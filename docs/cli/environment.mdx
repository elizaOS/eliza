...
| Variable | Purpose |
|----------|---------|
| `ELIZA_STATE_DIR` / `ELIZA_STATE_DIR` | Override state directory (config, credentials, caches). Default `~/.eliza/`. |
| `ELIZA_CONFIG_PATH` / `ELIZA_CONFIG_PATH` | Explicit path to `eliza.json`. |
| `ELIZA_NAMESPACE` | Namespace segment for default paths (`eliza` produces `~/.eliza/`). |
| `ELIZA_WORKSPACE_DIR` / `ELIZA_WORKSPACE_DIR` | Default agent workspace root. |
...
| `ELIZA_API_PORT` / `ELIZA_API_PORT` | 31337 | API port in dev mode (when API and UI run on separate ports). |
| `ELIZA_API_TOKEN` / `ELIZA_API_TOKEN` | (none) | API authentication token. When set, all API requests require `Authorization: Bearer <token>`. |
| `ELIZA_API_BIND` / `ELIZA_API_BIND` | `127.0.0.1` | API server bind address. Non-loopback addresses auto-generate a connection key if no token is set. |
...
| Variable | Purpose |
|----------|---------|
| `ELIZA_DISABLE_EDGE_TTS` / `ELIZA_DISABLE_EDGE_TTS` | Set to `1` to prevent auto-loading `@elizaos/plugin-edge-tts`. When the agent orchestrator is loaded, Eliza adds Edge TTS by default so swarm/PTY paths have a TTS handler -- this makes outbound calls to Microsoft's cloud. Disable if you want no network TTS. |
