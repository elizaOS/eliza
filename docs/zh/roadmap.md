...
- **Edge TTS disclosure** — 记录并暴露 **`ELIZA_DISABLE_EDGE_TTS`** / **`ELIZA_DISABLE_EDGE_TTS`**（注册表 + `docs/cli/environment.md` + TTS 文档）。**原因：** orchestrator 自动加载 Edge TTS → `node-edge-tts` → Microsoft；"无需 API 密钥"不等于"离线"。
