...
## Local audit - 2026-05-12

No final staged Eliza-1 bundle directories were found in this checkout at the expected release roots:
...
Therefore every required payload path listed in the tier sections below is missing locally under those roots. The only local HF-oriented model directories found were placeholder repo drafts under `reports/porting/2026-05-10/eliza-1-repos/`; their manifests record `status: "placeholder"`, `gguf.sha256: null`, and `gguf.sizeBytes: 0`, so they are not upload evidence and must not be treated as final hashes or weights.
