...
|---|---|---|---|---|
| **Offline `packages/training/` pipeline** | `packages/training/` | Fine-tunes a base Qwen3 causal-LM, applies the Eliza inference optimizations (PolarQuant / QJL / TurboQuant / DFlash), converts to GGUF | `elizaos/eliza-1/bundles/<tier>/` GGUF on HuggingFace + a runtime manifest | **Yes — this is the one.** |
