...
| X3 | `a11y.no-focus-visible-token` | **P0** | All onboarding buttons use Tailwind `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`. The `--ring` token's resolved value needs verification against the near-black backdrop — under the current theme `ring-offset-bg` is the same dark color as the page, so the offset gap is invisible and the ring effectively merges with the brand orange ring color. Result: keyboard focus is hard to see. Fix: explicit high-contrast focus token (e.g. white ring on dark, dark ring on light), independent of brand color. |
...
| X14 | `forms.unlabeled-inputs` | **P0** | Inputs on Step 6 (Display name) and Step 7 (Name, Relationship) have **no `id`, no `name`, no `aria-label`, no `aria-labelledby`** and their visible `<label>` elements have `for=""`. The label/input association is broken. Fix: stable `id` per input, `<label htmlFor={id}>`, or `aria-labelledby`. |
...
| 1.5 | `welcome.button-hierarchy` | P1 | "Grant microphone access" is full-width (476px) with the brand-orange treatment. "Continue" is small (91px) with the same orange treatment. Two visually identical primary buttons compete; users will not know which is "the" action. | Treat the mic button as the step's primary CTA (solid orange fill) and demote Continue to neutral/outline until it becomes meaningful (cf. 1.3). |
