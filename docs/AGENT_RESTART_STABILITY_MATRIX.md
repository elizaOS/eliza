...
| Packaged desktop | Electrobun shell plus app-core runtime process | User-visible relaunch or host-supervised restart, depending on package mode | Local app state, persisted runtime state, vault/config, content-addressed media | In-flight desktop bridge requests and transient renderer state | `bun run --cwd packages/app test:e2e:electrobun` and packaged relaunch specs under `packages/app/test/electrobun-packaged/` |
