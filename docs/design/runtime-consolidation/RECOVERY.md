...
| Electric test stack and cloud proxy | Deleted the unused SQL Electric Docker Compose stack, Caddy cloud proxy and retired sync/write-back environment settings. The supported PostgreSQL test stack remains. | No script/workflow references the removed artifacts; retained example variables are consumed by the PostgreSQL/PGlite implementation. |
