...
OpenAI unit/shape/loopback suite: **41 files / 591 cases pass**. The actual runtime HTTP retry suite passes five cases. Private keyless-runtime acceptance is still under investigation: its PGlite instance traps in WASM during a COMMIT loop. This is not counted as a passing integration check. The remaining SQL identity HTTP ownership, host contract curation and final combined acceptance remain open.
