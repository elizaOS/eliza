...
| M6 | Structured prompt + state | [packages/core/src/runtime/structured-prompt/executor.ts](../../../packages/core/src/runtime/structured-prompt/executor.ts); `stream-extractor.ts` | Parsed schema result or bounded recovery failure | core structured-output mechanism; do not truncate required context |
...
- Model handlers still return their typed model-specific results through `useModel`; the full model-outcome simplification and remaining coding-policy cleanup are not yet completed. The atlas describes those existing boundaries rather than claiming the proposal has shipped.
