...
- [ ] **Refresh tokens in localStorage.** XSS-reachable; defeats the HttpOnly cookie protection elsewhere in the system. Move refresh tokens to HttpOnly cookies; keep only short-lived access tokens in memory. — `cloud-frontend/src/lib/auth/*` — **M** — frontend + backend
