> September 22 correction: integration acceptance is OPEN. Notes port has 212 passing tests plus owning checks. Calendar port has 1,091 passing tests, four existing skips and one service suite blocked by unbuilt runtime dependencies; the host Calendar database suite passes all 37 cases. Calendar/host typechecks, lint and builds pass. Context/history recovery, acknowledgement delivery, preference parity and same-candidate UI acceptance remain open. Port 5288 still serves the separate saved demo. Root verify and native setup are not accepted.
...
- Branch/peer audit: `PEER-INTEGRATION-AUDIT-20260921.md`.
