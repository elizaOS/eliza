...
function resolveManifestPath(
  ...
  const manifestRelativePath = resolve(dirname(manifestPath), path);
  if (existsSync(manifestRelativePath)) return manifestRelativePath;
  const asWrittenPath = resolve(path);
  if (existsSync(asWrittenPath)) return asWrittenPath;
...
<body>
  ...
  <script>
    ...
    function hrefForPath(value) {
      ...
      if (!path) return null;
      if (new RegExp("^[a-z][a-z0-9+.-]*://", "i").test(path)) return path;
