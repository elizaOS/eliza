...
export const meetingProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    try {
      ...
      const activeParticipants = currentMeeting.participants.filter(
        (p) => p.isActive,
      ).length;
      const duration = currentMeeting.endTime
        ? Math.round(
            (currentMeeting.endTime.getTime() -
              currentMeeting.startTime.getTime()) /
              1000 /
              60,
          )
        : Math.round(
            (Date.now() - currentMeeting.startTime.getTime()) / 1000 / 60,
          );
