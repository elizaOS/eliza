...
export class GoogleAuthService extends Service {
  ...
  constructor(runtime?: IAgentRuntime) {
    ...
    const clientId = runtime.getSetting("GOOGLE_CLIENT_ID") as
      | string
      | undefined;
    const clientSecret = runtime.getSetting("GOOGLE_CLIENT_SECRET") as
      | string
      | undefined;
    const redirectUri =
    ...
    if (!clientId || !clientSecret) {
      throw new Error(
        "Google OAuth2 credentials not configured. Please set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET",
      );
    }
    ...
    const refreshToken = runtime.getSetting("GOOGLE_REFRESH_TOKEN") as
      | string
      | undefined;
    if (refreshToken) {
  ...
  async initialize(): Promise<void> {
    if (!this.authenticated) {
      logger.info(
        "Google OAuth2 client initialized. Use authenticateInteractive() to authenticate.",
      );
    }
  }
      ...
      try {
        ...
        if (url.pathname === "/oauth2callback") {
          ...
          if (code) {
            ...
            logger.info(
              "Save this refresh token in your .env file as GOOGLE_REFRESH_TOKEN to avoid re-authentication",
            );
        ...
        res.end(
          "<h1>Authentication failed!</h1><p>Check the logs for details.</p>",
        );
        server.close();
      }
    });
