...
If not available, try restarting Clawd Code, or manually add: `npx helius-mcp@latest  # configure in .clawd/settings.json or your MCP client` then restart. Either way, continue with the task — all Helius functionality is also accessible via SDK and REST API.
