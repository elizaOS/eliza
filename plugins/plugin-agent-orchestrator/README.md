...
| `ELIZA_CODEX_ACP_NO_LANDLOCK_SANDBOX_MODE` | unset (fail closed) | Explicit Codex ACP sandbox mode used when Linux Landlock is unavailable. Set this only when an independent container or VM sandbox enforces the workspace boundary; the runtime never silently widens a chat-started coding task to host access. |
| `ELIZA_CODEX_ACP_APPROVAL_POLICY` / `ELIZA_CODEX_APPROVAL_POLICY` | `never` for an explicit `danger-full-access` fallback, otherwise unset | Optional managed Codex ACP approval policy. Setting it requires an explicit sandbox mode; the successor supports the fixed pairs `read-only`/`on-request`, `workspace-write`/`on-request`, and `danger-full-access`/`never`. |
