...
with the `validating` transition. After a process restart, verification owned by
a confirmed dead local process becomes `interrupted`, with a retryable audit
...
identifiable owner; those require explicit operator review. The conditional
