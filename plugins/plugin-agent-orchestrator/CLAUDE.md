...
| `ELIZA_KIMI_ACP_COMMAND` | `kimi acp` | Official Kimi Code subscription ACP command. Spawns require `SpawnOptions.subscriptionExecutionMode: "user-attended"`; scheduled, unattended, and unspecified modes fail before workspace creation. Login is `kimi login`; logout is the interactive `/logout` because there is no top-level logout/status command. |
