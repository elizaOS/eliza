...
| `ELIZA_KIMI_ACP_COMMAND` | `kimi acp` | Official Kimi Code subscription ACP command. Requires explicit user-attended execution authority; scheduled, unattended, and unspecified modes fail before workspace creation. Login is `kimi login`; logout is interactive `/logout`. |
| `ELIZA_GROK_ACP_COMMAND` | `grok agent stdio` | Official Grok Build subscription ACP command. Login is `grok login` or `grok login --device-auth`; status is `grok models`; logout is `grok logout`. |
