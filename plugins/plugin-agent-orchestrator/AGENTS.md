...
It spawns local coding agents (Eliza Code, Pi Agent, Codex, and Claude) as
...
plugins/plugin-agent-orchestrator/
  src/
    ...
    services/
      ...
      elizaos-config.ts         Eliza Code-specific ACP configuration
...
| `ELIZA_ACP_DEFAULT_AGENT` | `elizaos` | Default agent type: `elizaos` (Eliza Code), `pi-agent`, `codex`, or `claude` |
...
| `ELIZA_ELIZAOS_ACP_COMMAND` | `eliza-code-acp` | Native Eliza Code ACP command |
