...
`packages/app/scripts/ensure-skills.mjs` resolves the shipped skill tree from `@elizaos/skills`, then falls back to `packages/skills/skills`, then legacy `skills/`.
...
- run `packages/app/scripts/ensure-skills.mjs` against a temporary `ELIZA_STATE_DIR`
