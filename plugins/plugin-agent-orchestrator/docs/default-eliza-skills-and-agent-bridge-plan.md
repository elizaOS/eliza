...
- Claude, Codex, and Eliza Code workers should receive those defaults through the ACP agent orchestrator.
...
`SKILLS.md` now tells workers to emit a standalone `USE_SKILL <slug> <json_args>` line. The orchestrator writes this manifest into the workspace and sets `ELIZA_SKILLS_MANIFEST`, so Codex/Claude/Eliza Code workers can discover skill protocol without hardcoding.
...
- Whether Eliza Code's vendored ACP path has enough smoke coverage against Cerebras.

Status: partially implemented. The first three are covered by code/tests; live Eliza Code/Cerebras validation remains credential-gated.
...
- Spawn Codex, Claude, and Eliza Code workers and verify they read `SKILLS.md`.
- Spawn Eliza Code through ACP using the vendored Eliza Code shim with Cerebras `gpt-oss-120b` and verify it can emit `USE_SKILL parent-agent ...`.
- Simulate:
  - action listing
...
Status: local unit/type/skills tests are implemented and passing. Live Eliza Code/Cerebras/SWE benchmarking requires running services and credentials, so it remains follow-up unless the environment already has them active.
...
- Live agent benchmarking against Eliza Code + Eliza Cloud + Cerebras requires credentials and running infrastructure.
