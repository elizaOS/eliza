...
- [x] Per-agent credential injection: claude → `CLAUDE_CODE_OAUTH_TOKEN`; codex → per-account `CODEX_HOME/auth.json` + minimal `config.toml`; direct API providers → their env key. Eliza Code consumes the runtime's authorized provider route instead of this external-CLI account pool.
...
- **Eliza Code is provider-routed by the runtime.** Its Cerebras or other OpenAI-compatible credential is forwarded through the authorized Eliza Code configuration, not rotated by the external Claude/Codex subscription selector.
