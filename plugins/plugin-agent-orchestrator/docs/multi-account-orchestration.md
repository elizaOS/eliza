...
- **Pi routes fail closed once accounts are linked.** A selector or availability failure refuses the spawn instead of falling back to ambient provider credentials. Private `models.json` files use Pi's `$ENV_VAR` interpolation and retain built-in provider model metadata; only OpenRouter materializes an arbitrary selected model id. That selected row explicitly uses `openai-completions` with OpenRouter’s `/api/v1` base URL, so a Claude catalog entry cannot substitute the Anthropic Messages protocol and append a second `/v1`. The [OpenRouter Chat Completions contract](https://openrouter.ai/docs/api/api-reference/chat/send-chat-completion-request) owns this wire format.
