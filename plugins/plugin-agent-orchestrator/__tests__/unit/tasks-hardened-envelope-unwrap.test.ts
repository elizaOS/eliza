...
describe("TASKS hardened-envelope unwrap", () => {
  ...
  it("create preserves a planner-supplied title after single-line normalization", async () => {
    const acp = serviceMock();
    ...
    expect(stored.title).toBe(blobTitle.replace(/\s+/gu, " ").trim());
    const emitted = emittedText(cb);
    expect(emitted).toContain(`[TASK:${THREAD_ID}]`);
    expect(emitted).toContain("x".repeat(300));
  });

  it("spawn_agent preserves a planner-supplied label after single-line normalization", async () => {
    const acp = serviceMock();
    ...
    expect(label).toBe(blobLabel.replace(/\s+/gu, " ").trim());
  });
