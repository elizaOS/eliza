...
describe("TASKS:create attaches spawned sessions to the minted task thread", () => {
  ...
  it("does NOT falsely promote the minted task to active for a single-turn create (real service)", {
    ...
    // subprocesses and completion-evidence assembly). Depending on runner
    // speed, validation may finish before this poll observes the intermediate
    // `validating` state.
    await vi.waitFor(
      async () => {
        expect((await taskService.getTask(taskId))?.activeSessionCount).toBe(0);
      },
    ...
    // through validation without falsely leaving it active.
