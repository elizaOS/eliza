...
describe("TASKS:create attaches spawned sessions to the minted task thread", () => {
  ...
  it("does NOT falsely promote the minted task to active for a single-turn create (real service)", {
    ...
    // subprocesses and completion-evidence assembly) before reaching its
    // terminal state; the default 1s waitFor bound flakes on loaded runners.
    await vi.waitFor(
      async () => {
    ...
    // is no longer live. With no unsatisfied criterion its task_complete event
    // advances the durable task through validation to done.
