...
class FakeExecutor implements TaskStepExecutor {
  ...
  async runTurn(ctx: TaskStepContext): Promise<TaskTurnResult> {
    ...
    const done = this.opts.doneOnTurn ? (ctx.turn ?? 0) >= this.opts.doneOnTurn : false;
    return { done };
  }
...
describe('runTaskWithSmithers (durable Smithers-backed coding task)', () => {
  it('completes a single-turn task', async () => {
  ...
  it('loops agent turns until done', async () => {
  ...
  it('stops at maxTurns and reports incomplete when never done', async () => {
  ...
  it('runs provision + approval(approved) + submit', async () => {
    ...
    expect(result.workspace).toMatchObject({ dir: expect.stringContaining('/tmp/ws-') });
    expect(result.submit).toMatchObject({ pr: expect.stringContaining('https://pr/') });
  }, TIMEOUT);

  it('skips submit and reports denied when approval is denied', async () => {
  ...
  it('fans out parallel agents and completes when all are done', async () => {
    const fake = new FakeExecutor({ doneOnTurn: 2 });
    const result = await runTaskWithSmithers(spec({ parallelAgents: 2 }), fake);
    expect(result.status).toBe('completed');
