/**
 * Gap (J): sub-agent model + auth selection is dispersed across private
 * `AcpService.buildEnv` branches and was never contract-tested as a single
 * resolved tuple.
 *
 ...
 *
 *  1. The Claude / Codex / Eliza Code / pi-agent CREDENTIAL-DROP +
...
// (PATH, ELIZA_*, elizaos autoupdate flags) is noise for the model/auth contract.
const AUTH_KEYS = [
  "ANTHROPIC_API_KEY",
...
// ===========================================================================
// 2. Claude/Codex/elizaos/elizaos/pi-agent credential-drop + injection
...
describe("model-chooser contract: elizaos + pi-agent are single-auth (bridge NOT consulted)", () => {
  ...
  // on agent-type branches that only apply to claude/codex/elizaos.
