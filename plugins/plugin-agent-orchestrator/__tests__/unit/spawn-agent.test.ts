...
describe("TASKS:spawn_agent", () => {
  it("rejects a list_agents alias on the promoted spawn tool before spawning", async () => {
    ...
    // `op` with the full ops enum (it carries the "spawn_agent" pin), so a
    // conflicting discriminator on the pinned virtual returns a structured
    // redirect instead of silently executing another operation.
