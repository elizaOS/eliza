...
const uniqueSession = () => ({ sessionId: `sess-${Math.random().toString(36).slice(2, 10)}` });
...
describe('runDurableTask', () => {
  it('drives the spawned session to completion and captures the response', async () => {
    const sendPrompt = vi.fn(async () => ({ stopReason: 'end_turn', finalText: 'all done' }));
    const service: AcpTaskService = { sendPrompt };
    const result = await runDurableTask(service, uniqueSession(), 'do the thing', {});
  ...
  it('falls back to sendToSession when sendPrompt is absent', async () => {
    const sendToSession = vi.fn(async () => ({ stopReason: 'end_turn', finalText: 'via sendToSession' }));
  ...
  it('propagates a prompt error (even when a single-turn loop would swallow it)', async () => {
    ...
    await expect(runDurableTask(service, uniqueSession(), 'x', {})).rejects.toThrow();
  }, TIMEOUT);
});
