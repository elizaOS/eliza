...
class FakeAcp implements AcpLike {
  ...
  constructor(private readonly promptResult: PromptOut = { stopReason: 'end_turn', finalText: 'done' }) {}
  ...
  async findResumableSessionByLabel(_label: string): Promise<{ sessionId: string } | null> {
...
describe('detectTurnDone', () => {
  it('treats a clean turn as done', () => {
    expect(detectTurnDone({ stopReason: 'end_turn', finalText: 'ok' })).toBe(true);
  });
...
describe('SmithersTaskExecutor', () => {
  it('spawns lazily, sends the initial prompt, reports done', async () => {
    ...
    const result = await executor.runTurn({ taskId: 't1', runId: 't1', turn: 1, prompt: 'fix the bug' });
    expect(acp.spawns).toHaveLength(1);
    expect(acp.spawns[0]).toMatchObject({ agentType: 'codex', label: 't1' });
    expect(acp.prompts[0]).toMatchObject({ sessionId: 'sess-1', text: 'fix the bug' });
  ...
  it('reuses one session across turns and sends the continue prompt later', async () => {
    ...
    const executor = new SmithersTaskExecutor(acp, { continuePrompt: 'keep going' });
  ...
  it('throws when a turn returns an error', async () => {
    ...
    const executor = new SmithersTaskExecutor(acp);
    await expect(executor.runTurn({ taskId: 't', runId: 't', turn: 1, prompt: 'P' })).rejects.toThrow(
      'agent failed'
    );
  });
