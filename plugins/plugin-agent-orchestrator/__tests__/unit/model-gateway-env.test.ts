...
const RAW_ELIZA_CODE_KEY = "csk-raw-elizaos-DO-NOT-LEAK";
...
const MANAGED_ENV_KEYS = [
  ...
  "OPENAI_BASE_URL",
  "ANTHROPIC_BASE_URL",
  // Host vars that steer Eliza Code provider selection — cleared so spawn
  // tests are deterministic on any developer/CI machine.
  "ELIZA_LLM_PROVIDER",
  "ELIZA_CODE_BASE_URL",
  "ELIZA_ELIZAOS_MODEL_POWERFUL",
  "ELIZA_ELIZAOS_MODEL_FAST",
  "CEREBRAS_BASE_URL",
  "CEREBRAS_MODEL",
] as const;
...
async function spawnAndCaptureEnv(
  agentType: "claude" | "codex" | "elizaos",
...
describe("applyModelGatewayEnv (pure env rewrite)", () => {
  it("deletes raw provider keys, then injects gateway base URLs + token", () => {
    const env: NodeJS.ProcessEnv = {
      ...
      ELIZA_CODE_API_KEY: RAW_ELIZA_CODE_KEY,
    ...
    expect(env.ELIZA_CODE_API_KEY).toBeUndefined();
  ...
  it("scrubs composite env values that embed a raw provider key (fail closed)", () => {
    const env: NodeJS.ProcessEnv = {
      CEREBRAS_API_KEY: RAW_CEREBRAS_KEY,
      // A composite carrier embedding the raw key guards future merge steps
      // that might smuggle a credential inside a non-excluded variable.
      CHILD_PROVIDER_CONFIG: JSON.stringify({
        provider: { cerebras: { options: { apiKey: RAW_CEREBRAS_KEY } } },
      }),
      PATH: "/usr/bin",
    };
    applyModelGatewayEnv(env, { url: GATEWAY_URL, token: GATEWAY_TOKEN });
    expect(env.CHILD_PROVIDER_CONFIG).toBeUndefined();
    expect(env.PATH).toBe("/usr/bin");
    expectNoRawKeyInDump(env);
  });
...
describe("gateway mode ON — spawned sub-agent env", () => {
  ...
  it("elizaos spawn uses the gateway's OpenAI-compatible route with raw keys excluded", async () => {
    setRawProviderKeys();
    enableGateway();
    const { env } = await spawnAndCaptureEnv("elizaos");
...
describe("gateway mode OFF — byte-identical legacy env", () => {
  ...
  function snapshotRelevantKeys(env: NodeJS.ProcessEnv) {
    return {
      ...
      ELIZA_CODE_API_KEY: env.ELIZA_CODE_API_KEY,
  ...
  it("both vars unset: elizaos keeps native provider settings untouched", async () => {
    setRawProviderKeys();
    const { env, logger } = await spawnAndCaptureEnv("elizaos");

    expect(env.ELIZA_CODE_API_KEY).toBe(RAW_ELIZA_CODE_KEY);
    expect(env.CEREBRAS_API_KEY).toBe(RAW_CEREBRAS_KEY);
