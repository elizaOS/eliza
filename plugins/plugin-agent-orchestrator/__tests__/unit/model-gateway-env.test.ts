...
const MANAGED_ENV_KEYS = [
  ...
  "OPENAI_BASE_URL",
  "ANTHROPIC_BASE_URL",
  // Host vars that steer provider selection — cleared so the spawn tests are
...
describe("applyModelGatewayEnv (pure env rewrite)", () => {
  ...
  it("scrubs composite env values that embed a raw provider key (fail closed)", () => {
    const env: NodeJS.ProcessEnv = {
      CEREBRAS_API_KEY: RAW_CEREBRAS_KEY,
      // A composite carrier: a JSON config blob embedding the raw key (guards
      // any future merge step that smuggles a raw value inside a non-excluded
      // var).
      TOOL_CONFIG_CONTENT: JSON.stringify({
        provider: { cerebras: { options: { apiKey: RAW_CEREBRAS_KEY } } },
      }),
      PATH: "/usr/bin",
    };
    applyModelGatewayEnv(env, { url: GATEWAY_URL, token: GATEWAY_TOKEN });
    expect(env.TOOL_CONFIG_CONTENT).toBeUndefined();
    expect(env.PATH).toBe("/usr/bin");
    expectNoRawKeyInDump(env);
  });
