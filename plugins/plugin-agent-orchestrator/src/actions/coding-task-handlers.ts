...
/**
 * Shared registry that maps spawned PTY session IDs to the generated skill
...
async function prepareSkillAwareness(
  ...
  const manifest = await buildSkillsManifest(runtime, {
    onlyEligible: true,
    recommendedSlugs: [PARENT_AGENT_BROKER_SLUG, ...recommendedSlugs],
    virtualSkills,
  });
...
function decorateTaskWithSkillHint(
  ...
  if (awareness.manifest.slugs.includes(PARENT_AGENT_BROKER_SLUG)) {
    lines.push(
      `To ask the running parent Eliza agent to use any loaded capability, send \`USE_SKILL parent-agent {"request":"..."}\`. To inspect available parent actions, send \`USE_SKILL parent-agent {"mode":"list-actions","query":"calendar"}\`.`,
    );
  }
  if (
...
export async function handleMultiAgent(
  ...
  // list so the bridge can reject directives for slugs outside this task.
  ensureSkillCallbackBridge({
    runtime,
    ptyService,
  ...
  for (const [i, plannedAgent] of plannedAgents.entries()) {
    ...
    try {
      ...
      // Register this session's generated skill allow-list so the skill
