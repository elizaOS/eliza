...
/** Default page size for issue listings — a caller-overridable window, not a
 * cap: the list path honors explicit `limit`/`offset` params and always echoes
 * `{total, offset, limit, hasMore}` so nothing is silently clipped. */
const ISSUE_RESULT_LIMIT = 25;

/** Named log preview: bounded head plus an explicit marker so a clipped log
 * line never reads as the complete value (log-only surface; the value itself
 * is deliberately discarded from the flow). */
function logPreview(value: string, max = 80): string {
  return value.length > max
    ? `${value.slice(0, max)}… (truncated, ${value.length} chars)`
    : value;
}

/** Canonical positive integer param, else the fallback (page-size parsing). */
...
async function runCreateLegacy(
  ...
  const baseLabel = baseLabelSane
    ? completeUserReferenceView(baseLabelSane)
    : undefined;
  ...
  // well-formed-normalized (task-label.ts), so only the params-derived branch
  // needs vetting.
...
export function deliverableSlugFromLabel(label: string): string {
  ...
  // Identifier derivation, not a content view (cap-audit: cosmetic): the slug
  // keeps at most 5 words for public-URL legibility; the complete label/task
  // text stays on the task record and label paths.
...
async function resolveRequestedRepo(
  ...
  if (rawParamRepo && !paramRepo) {
    logger(runtime).warn(
      `[TASKS] ignoring malformed planner repo param: ${logPreview(JSON.stringify(rawParamRepo))}`,
    );
  }
  ...
  if (paramRepo) {
    ...
    if (!grounded) {
      logger(runtime).warn(
        `[TASKS] ignoring ungrounded planner repo param (user text names no repo): ${logPreview(paramRepo)}`,
      );
  ...
  try {
    ...
    if (spawnCapRouter && spawnOriginKey) {
      ...
      if (spawnCapRouter.spawnCountForOrigin(spawnOriginKey) >= cap) {
        ...
        // it before its early returns too) — verbatim, it IS the answer. The
        // router's map is volatile (bounded, process-local); on a miss, fall
        // back to the durable canonicalBestResult the router persists on the
        // task at capture time.
        let bestText = (best?.deliverable ?? best?.text ?? "").trim();
        if (!bestText && spawnRootMessageId) {
          const taskServiceForBest = runtime.getService?.(
            OrchestratorTaskService.serviceType,
          ) as OrchestratorTaskService | null | undefined;
          try {
            const records = taskServiceForBest
              ? (
                  await taskServiceForBest.listTasks({ includeArchived: false })
                ).slice(0, 30)
              : [];
            for (const record of records) {
              const detail = await taskServiceForBest?.getTask(record.id);
              const canonical = (
                detail?.metadata as
                  | {
                      canonicalBestResult?: {
            ...
            // error-policy:J4 durable-fallback lookup is enrichment; a store
            // failure degrades to the honest exhausted report below.
          }
        }
        if (bestText) {
...
async function runHistory(
  ...
  const { limit, clampedFrom } = parseHistoryLimit(
    params.limit ?? content.limit,
    fallbackLimit,
  );
  const offset = parseHistoryOffset(params.offset ?? content.offset);
  ...
  if (taskService && typeof taskService.listTasks === "function") {
    try {
      ...
      // Explicit continuation window: pre-slice total + hasMore travel in
      // data, and the caller pages with offset — nothing is silently clipped.
      const {
        page: tasks,
        total: count,
        hasMore,
      } = paginateHistory(allTasks, offset, limit);
      const filterParts = [
      ...
      if (metric === "count") {
        ...
        // Named preview: show the first 3 hidden tasks and SAY how many more
        // the filters hid — the disclosure list must not itself silently
        // elide (the full set is reachable by re-querying without filters).
        const excludedPreview = excludedByFilters.slice(0, 3);
        const excludedRest = excludedByFilters.length - excludedPreview.length;
        responseText = inFlight
          ? `Nothing has finished yet — I'm still working on ${inFlight.name}.`
          : count > 0
            ? `No task threads at offset ${offset} — only ${count} match${filterSuffix}.`
            : excludedByFilters.length > 0
              ? [
                  `No task threads matched${filterSuffix}, but ${excludedByFilters.length} related task${excludedByFilters.length === 1 ? "" : "s"} exist outside those filters:`,
                  ...excludedPreview.map(
                    (task) => `- "${task.title}" — status=${task.status}`,
                  ),
                  ...(excludedRest > 0
                    ? [
                        `…and ${excludedRest} more hidden by the current filters — re-run without status/window filters to list them.`,
                      ]
                    : []),
                ].join("\n")
        ...
        responseText = [
          ...
          // Continuation contract: when the listing is a window, say so and
          // name the offset that fetches the next page.
          ...(hasMore || offset > 0
            ? [
                `(showing ${offset + 1}–${offset + tasks.length} of ${count}${hasMore ? `; pass offset=${offset + tasks.length} for more` : ""})`,
              ]
            : []),
        ].join("\n");
      }
      ...
      return {
        ...
        data: {
          ...
          filters: {
            ...
            limit,
            offset,
            // A clamped caller limit is reported, never silent: the ceiling
            // is MAX_HISTORY_LIMIT; page with `offset` for the rest.
  ...
  const matchingSessions = (await listSessionsWithin(service)).filter(
    (session) =>
      (!sessionId || session.id === sessionId) &&
      sessionMatchesHistoryFilters(session, statuses, windowFilters, search),
  );
  // Total is computed BEFORE the window (the pre-fix code counted after the
  // slice, telling the model "I found {page size}" against a larger store);
  // offset/limit/hasMore make the window an explicit continuation contract.
  const {
    page: sessions,
    total: count,
    hasMore,
  } = paginateHistory(matchingSessions, offset, limit);
  ...
  if (metric === "count") {
    ...
    responseText = [
      ...
      ...(hasMore || offset > 0
        ? [
            `(showing ${offset + 1}–${offset + sessions.length} of ${count}${hasMore ? `; pass offset=${offset + sessions.length} for more` : ""})`,
          ]
        : []),
    ].join("\n");
  }
  ...
  return {
    ...
    data: {
      ...
      limit,
      hasMore,
      ...(clampedFrom !== undefined ? { limitClampedFrom: clampedFrom } : {}),
...
async function runProvisionWorkspace(
  ...
  try {
    ...
    if (state) {
      // The path is a produced identifier and rides VERBATIM everywhere
      // (state, receipt, data) — truncating it mints a nonexistent path.
...
async function runSubmitWorkspace(
  ...
  try {
    ...
    // Named preview (cap-audit: cosmetic): standard short-hash display; the
    // COMPLETE hash rides in data.commitHash on this same ActionResult, and
    // the receipt binding pins the appendix text verbatim as rendered here.
...
/** Exported for deterministic unit coverage of the issue read/create seams
 * (pagination contract, durable body projection, bulk-create completeness). */
  ...
  try {
    switch (action.toLowerCase()) {
      ...
      case "list": {
        ...
        const labels = parseLabels(params.labels);
        const allIssues = await service.listIssues(repo, {
          state: stateFilter as "open" | "closed" | "all",
          labels: labels.length > 0 ? labels : undefined,
        });
        // Explicit pagination, never a silent clip: the caller's offset/limit
        // are honored verbatim (ISSUE_RESULT_LIMIT is only the default page
        // size) and total/hasMore are echoed in text AND data so the model
        // can page instead of reporting the window as the whole repo.
        const offset = parseHistoryOffset(params.offset);
        const limit = positiveIntParam(params.limit, ISSUE_RESULT_LIMIT);
        const {
          page: issues,
          total,
          hasMore,
        } = paginateHistory(allIssues, offset, limit);
        const listText =
          issues.length === 0
            ? total === 0
              ? `No ${stateFilter} issues found in ${repo}.`
              : `No ${stateFilter} issues at offset ${offset} in ${repo} — only ${total} exist.`
            : [
                `Issues in ${repo}${hasMore || offset > 0 ? ` (${offset + 1}–${offset + issues.length} of ${total})` : ""}:`,
                ...issues.map(
                  (i) =>
                ...
                ...(hasMore
                  ? [
                      `…and ${total - offset - issues.length} more — pass offset=${offset + issues.length} to continue.`,
                    ]
                  : []),
              ].join("\n");
        ...
        return {
          success: true,
          text: listText,
          data: { issues, total, offset, limit, hasMore },
        };
      }
...
async function runManageIssues(
  ...
  // security-notice lines. COMPLETE text, no truncation (prompt-integrity):
  // op inference, the repo-URL match, and extractBulkItems must see the whole
  // request — a pre-inference slice misinferred ops and silently dropped
  // trailing bulk items.
