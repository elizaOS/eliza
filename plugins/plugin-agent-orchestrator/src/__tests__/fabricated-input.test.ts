...
describe("fabricated input detection", () => {
  ...
  it("does not flag the script itself or unrelated writes", () => {
    ...
    expect(
      detectFabricatedInput(
        ...
        [
          {
            path: "/etc/nubs-secret-config.yaml",
            existed: true,
            sha256: "abc",
          },
        ],
  ...
  it("does not treat produced files as read targets; conviction needs an absent-at-baseline write", () => {
    expect(
      readTargetsFromTask("read sales.csv and write a summary to report.json"),
    ).toEqual(["sales.csv"]);
    // A same-named write with NO baseline is unknown — never a conviction.
    expect(
      detectFabricatedInput(
    ...
    // Captured absent before execution + written by the worker = fabricated.
    expect(
      detectFabricatedInput(
        "write a script that reads /ws/config.json and prints it",
        ["/ws/config.json"],
        [],
        [{ path: "/ws/config.json", existed: false }],
      ),
    ).toEqual({ target: "/ws/config.json", wrote: "/ws/config.json" });
  });
});
