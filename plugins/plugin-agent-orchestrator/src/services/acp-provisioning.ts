 ...
 * inherited file descriptor and a behavior-qualified `timeout` supervisor to
 * terminate the complete build process group at the shared deadline. Hosts
 * without those proven primitives
...
function findTimeoutSupervisorOnPath(timeoutMs: number): string | undefined {
  const timeout = findExecutableOnPath("timeout");
  if (!timeout || !Number.isFinite(timeoutMs) || timeoutMs <= 0)
    return undefined;
  // Qualify the behavior we depend on instead of a vendor/version banner.
  // Both GNU coreutils and uutils implement these semantics; a similarly named
  // binary that cannot escalate TERM to KILL within the deadline is rejected.
  const probe = spawnSync(
    ...
    ],
    {
      encoding: "utf8",
      env: { ...process.env, LC_ALL: "C" },
      timeout: Math.max(1, Math.floor(Math.min(1_500, timeoutMs))),
      killSignal: "SIGKILL",
    },
  );
  return probe.error === undefined &&
    (probe.status === 124 || probe.status === 137 || probe.signal === "SIGKILL")
    ? timeout
    : undefined;
}
...
export function provisionWorkspaceElizaCodeAcp(
  ...
  if (hooks.build) {
    ...
    const supervisor = findTimeoutSupervisorOnPath(deadline - now());
