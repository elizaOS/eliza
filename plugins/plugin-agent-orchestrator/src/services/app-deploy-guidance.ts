...
function elizaCloudGuidance(task?: string, monetized?: boolean): string {
  ...
  if (monetized === true || isMonetizedAppTask(task)) {
    lines.push(
      ...
      '- Only when the app needs a server-side runtime, deploy that SAME app with `apps.deploy` params `{"id":"<appId>","image":"ghcr.io/<approved-namespace>/<app>@sha256:<digest>"}` and poll `apps.deploy.status`. The app deploy path validates the image, owns the container identity, injects `ELIZA_APP_ID`, and injects the app database URL when isolated mode is selected. Do not create a parallel generic container with `containers.create`.',
    ...
    lines.push(
      ...
      "- Use `apps.database.update` + `apps.deploy` only if the product truly needs an isolated database and/or server runtime. Pass an operator-approved prebuilt `image` to `apps.deploy` and poll `apps.deploy.status`; the app-owned deploy path validates and persists the image and wires container identity plus database settings.",
