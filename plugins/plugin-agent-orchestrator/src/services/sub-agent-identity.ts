...
- Publish its static frontend: \`USE_SKILL parent-agent {"mode":"cloud-command","command":"apps.frontend.deploy","params":{"id":"<appId>","files":[{"path":"index.html","content":"..."}],"activate":true}}\`
- Deploy an app-owned backend when needed: \`USE_SKILL parent-agent {"mode":"cloud-command","command":"apps.deploy","params":{"id":"<appId>"}}\`
...
deploy one canonical app identity through the parent with \`apps.create\`,
\`apps.frontend.deploy\`, optional \`apps.database.update\`, and optional
\`apps.deploy\` instead of curling the Cloud API yourself or creating a parallel
generic container. Discovery is free, but
...
(\`apps.deploy\` is fixed-cost and may auto-authorize within that cap). You
cannot bypass either gate by declaring a price — the parent verifies
server-side. App code forwards the signed-in user's bearer token with
\`x-app-id\`; never request, bridge, or inject the owner's Cloud key into a child
or app container. Use this only when the task genuinely needs a parent
capability; a self-contained coding task never should.
`;
