...
interface BridgeDeps {
   ...
   * child with an error message listing the allowed slugs.
   *
...
export function installSkillCallbackBridge(deps: BridgeDeps): () => void {
  ...
  if (!useSkillAction || typeof useSkillAction.handler !== "function") {
    log.debug?.(
      `${LOG_PREFIX} USE_SKILL action not registered; disk-backed skill callbacks unavailable, virtual brokers still active`,
    );
  }
  ...
  const dispatchToParent = async (
    ...
    // Enforce the per-session skill allow-list when one is
    ...
    if (allowedSlugs && !allowedSlugs.includes(invocation.slug)) {
      const allowed =
        allowedSlugs.length > 0
          ...
          : "(none)";
      const text = `Skill \`${invocation.slug}\` is not on this task's allow-list. Allowed: ${allowed}.`;
      log.warn?.(
        `${LOG_PREFIX} session ${sessionId} requested non-allowed skill ${invocation.slug}; allow-list=[${allowedSlugs.join(",")}]`,
      );
    ...
    const sensitiveBroker =
      invocation.slug === LIFEOPS_CONTEXT_BROKER_SLUG ||
      invocation.slug === PARENT_AGENT_BROKER_SLUG;
    if (sensitiveBroker && !allowedSlugs?.includes(invocation.slug)) {
      const text = `Skill \`${invocation.slug}\` is sensitive and is only available when the parent explicitly allow-lists it for this spawned task.`;
      log.warn?.(
        {
          src: LOG_PREFIX,
          event: "sensitive_broker_denied",
          sessionId,
          slug: invocation.slug,
        },
        `${LOG_PREFIX} session ${sessionId} requested ${invocation.slug} without an allow-list grant`,
      );
      const reply = formatResultForChild(invocation.slug, {
        success: false,
        text,
    ...
    if (!useSkillAction || typeof useSkillAction.handler !== "function") {
      const reply = formatResultForChild(invocation.slug, {
        success: false,
        text: "Parent USE_SKILL action is not registered; only task-scoped virtual broker skills are available in this runtime.",
      });
      await ptyService.sendToSession(sessionId, reply);
      return;
    }
