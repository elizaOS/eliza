...
If an \`ELIZA_SKILLS_MANIFEST\` environment variable or \`SKILLS.md\` file is present, read it before assuming a capability is unavailable. The orchestrator can route approved task-scoped requests back to the running parent Eliza agent:

- \`USE_SKILL parent-agent {"request":"..."}\` asks the parent to use its loaded actions, providers, connectors, services, and confirmation flow.
- \`USE_SKILL parent-agent {"mode":"list-actions","query":"calendar"}\` lists matching parent actions.
- \`USE_SKILL lifeops-context {"taskId":"..."}\` reads task-scoped LifeOps context when that broker is listed in \`SKILLS.md\`.

Use these brokers for information or actions that only the parent Eliza runtime can perform, such as calendar lookup, GitHub connector access, browser actions, user-confirmed purchases, or plugin-specific capabilities.`;
}
