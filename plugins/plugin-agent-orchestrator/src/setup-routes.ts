...
function codingAgentRouteHandler(): LegacyRouteHandler {
    return async (req: RouteRequest, res: RouteResponse, agentRuntime: IAgentRuntime): Promise<void> => {
        ...
        if (!httpRes.headersSent) {
            ...
            httpRes.end(JSON.stringify({ error: "coding agent route not found", pathname }));
        }
    };
}
...
const codingAgentRoutes: Route[] = CODING_AGENT_ROUTE_PATHS.map((r) => ({
...
export const codingAgentRoutePlugin: Plugin = {
    ...
    description: "Coding-agent orchestrator HTTP routes (coding-agents, workspace, issues) " +
