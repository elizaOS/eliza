...
const BRIDGE_PATH = /^\/api\/coding-agents\/(pty-\d+-[0-9a-f]+)\/(parent-context|memory|active-workspaces)\/?$/;
...
async function handleParentContext(
	...
	const thread = task.threadId
		? await ctx.coordinator?.taskRegistry.getThread(task.threadId).catch(() => null)
		: null;
	sendJson(res, {
...
async function handleMemory(
	...
	const matched = memories
		.map((m) => {
			...
			return {
				speaker:
					m.entityId === ctx.runtime.agentId ? "agent" : "user",
