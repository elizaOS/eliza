...
import { TASK_AUDIT_EVENT } from "./services/audit.js";
...
import { appendAuditLine } from "./services/audit.js";
...
import { createTerminalUnsupportedTasksAction } from "./actions/sandbox-stub.js";
import { createUniqueUuid } from "@elizaos/core";
import { defaultAuditLogPath } from "./services/audit.js";
...
import { isSessionBusy } from "./services/active-session-forward.js";
...
import { type AcpToolCall } from "./services/types.js";
...
import { type TaskAuditPayload } from "./services/audit.js";
import { type ThreadHandle } from "@elizaos/core";
...
export { cloneLanePlan, collisionProviderFromWorkspaceService, createDeterministicLanePlan, type ExternalCollision, extractScopePaths, type LaneCollision, type LaneCollisionProvider, type LanePlan, type LanePlannerInput, LanePlannerService, type LaneReadiness, type LaneSpec, laneReadiness, sanitizeLaneBranchName, scopeSetsOverlap, shouldUseLanePlanner, validateLaneDependencyGraph, } from "./services/lane-planner.js";
...
export { sanitizeCompletionRelay, stripToolTranscript, } from "./services/transcript-sanitizer.js";
...
export function createAgentOrchestratorPlugin(): Plugin {
            ...
            ...promoteSubactionsToActions(tasksAction, {
                ...
                overrides: {
                    spawn_agent: {
                        ...
                        descriptionCompressed: "delegate ACP coding sub-agent elizaos|pi-agent|claude|codex; multi-step; prefer TASKS_SEND if active session exists on same workdir",
                    },
                },
            }),
        ]
    ...
    let taskAuditHandler: ((payload: TaskAuditPayload & {
    ...
    let activeSessionForwardHandler: ((payload: {
        message: Memory;
    }) => Promise<void>) | undefined;
    ...
    return {
        ...
        async init(_config: Record<string, string>, runtime: IAgentRuntime) {
            ...
            const auditLogPath = typeof auditLogSetting === "string" && auditLogSetting.length > 0
                ? auditLogSetting
                : defaultAuditLogPath();
            taskAuditHandler = async (payload) => {
                ...
                await appendAuditLine(auditLogPath, persisted).catch((err) => runtime.logger?.warn?.({
                    src: "@elizaos/plugin-agent-orchestrator",
                    err: err instanceof Error ? err.message : String(err),
                }, "Failed to append TASK_AUDIT entry"));
            };
            runtime.registerEvent<TaskAuditPayload & {
                runtime: IAgentRuntime;
            ...
            subAgentInbox.setOverflowObserver((sessionId, droppedNow, droppedTotal) => {
                runtime.logger?.warn?.({
                    src: "@elizaos/plugin-agent-orchestrator",
                    sessionId,
                    droppedNow,
                ...
                runtime.reportError?.("SubAgentInbox", new Error(`sub-agent inbox overflow for session ${sessionId}: dropped ${droppedNow} queued message(s) (${droppedTotal} total)`), { sessionId, droppedNow, droppedTotal });
            });
            ...
            activeSessionForwardHandler = createActiveSessionForwardHandler(runtime, subAgentInbox);
            runtime.registerEvent(EventType.MESSAGE_RECEIVED, activeSessionForwardHandler);
            ...
            setTimeout(() => {
                void (async () => {
                    for (const sType of types) {
                        ...
                        await runtime.getServiceLoadPromise(sType).catch((err: unknown) => runtime.logger?.warn?.({
                            src: "@elizaos/plugin-agent-orchestrator",
                            serviceType: sType,
                            err: err instanceof Error ? err.message : String(err),
                    ...
                    if (acp) {
                        ...
                        const scheduleFlush = (sessionId: string, tries = 0): void => {
                            if (subAgentInbox.size(sessionId) === 0)
                                return;
                            ...
                            if (tries === 0 && flushPending.has(sessionId))
                                return;
                            flushPending.add(sessionId);
                            const timer = setTimeout(() => {
                                flushTimers.delete(timer);
                                void (async () => {
                                    const svc = runtime.getService<AcpService>(AcpService.serviceType);
                    ...
                    void acp?.resumeOrphanedBusySessions?.().catch((err: unknown) => runtime.logger?.warn?.({
                        src: "@elizaos/plugin-agent-orchestrator",
                        err: err instanceof Error ? err.message : String(err),
        ...
        async dispose(runtime) {
            if (taskAuditHandler) {
                runtime.unregisterEvent?.<TaskAuditPayload & {
                    runtime: IAgentRuntime;
                }>(TASK_AUDIT_EVENT, taskAuditHandler);
                taskAuditHandler = undefined;
            }
            if (activeSessionForwardHandler) {
                runtime.unregisterEvent?.(EventType.MESSAGE_RECEIVED, activeSessionForwardHandler);
            ...
            const acp = runtime.getService<AcpService>(AcpService.serviceType);
            await acp?.stop();
            const taskService = runtime.getService<OrchestratorTaskService>(OrchestratorTaskService.serviceType);
            await taskService?.stop();
            const router = runtime.getService<SubAgentRouter>(SubAgentRouter.serviceType);
            await router?.stop();
            const coordinator = runtime.getService<SwarmCoordinatorService>(SwarmCoordinatorService.serviceType);
            await coordinator?.stop();
            const waveSupervisor = runtime.getService<WaveSupervisor>(WaveSupervisor.serviceType);
...
const SELF_HEAL_REPLACEMENT = "(Sub-agent state self-heals; respawning a fresh one automatically.)";
...
function isForbiddenCleanupSentence(sentence: string): boolean {
    ...
    const hasRecoveryVerb = containsWord(lower, "restart") ||
    ...
    if (containsWord(lower, "acpx") && hasRecoveryVerb)
        return true;
    if (containsWord(lower, "daemon") && hasRecoveryVerb)
        return true;
    const clears = containsWord(lower, "clear") ||
        containsWord(lower, "clean") ||
        containsWord(lower, "wipe");
    if (clears && lower.includes("stale session"))
        return true;
    return (lower.includes("manually clear") &&
...
function stripForbiddenCleanupSentences(text: string): string {
    ...
    for (let index = 0; index <= text.length; index += 1) {
        ...
        const boundary = index === text.length ||
            character === "." ||
            character === "!" ||
            character === "?" ||
            character === "\n";
        ...
        if (!isForbiddenCleanupSentence(segment))
            out.push(segment);
        if (character === "\n")
            out.push("\n");
...
function collapseWhitespaceRuns(text: string): string {
    ...
    while (cursor < text.length) {
        ...
        const start = cursor;
        while (cursor < text.length && text[cursor]?.trim() === "")
            cursor += 1;
...
function readProgressSetting(runtime: IAgentRuntime, key: string): string | undefined {
...
function parseProgressMode(value: string | undefined): SubAgentProgressMode {
    ...
    if (normalized === "ack" || normalized === "ack-only")
        return "ack";
    if (normalized === "thread" || normalized === "threaded")
        return "threaded";
    return "compact";
}
...
export function resolveSubAgentProgressPolicy(runtime: IAgentRuntime): SubAgentProgressPolicy {
    const mode = parseProgressMode(readProgressSetting(runtime, "ACPX_PROGRESS_MODE") ??
        readProgressSetting(runtime, "ELIZA_SUB_AGENT_PROGRESS_MODE"));
    return {
        mode,
        reactions: parseProgressReactions(readProgressSetting(runtime, "ACPX_PROGRESS_REACTIONS") ??
        ...
        delayMs: mode === "ack"
            ? 0
            : parseProgressDelayMs(readProgressSetting(runtime, "ACPX_PROGRESS_DELAY_MS") ??
 ...
 */
export function plannerAlreadyAckedSpawn(plannerReplyAtMs: number | undefined, sessionCreatedAtMs: number | undefined, lookbackMs: number): boolean {
    if (sessionCreatedAtMs === undefined)
        return false;
    if (plannerReplyAtMs === undefined)
        return false;
...
function formatToolCallForHuman(tc: AcpToolCall | undefined): string {
    ...
    const cmd = typeof input.command === "string" ? input.command.trim() : undefined;
    ...
    const noun = (() => {
        ...
        if (kind === "execute" || t.includes("terminal") || t.includes("bash"))
            return "Bash";
        if (kind === "read" || t.includes("read"))
            return "Read";
        if (kind === "edit" || t.includes("edit"))
            return "Edit";
    ...
    if (cmd)
        return `${noun}(${trimCmd(cmd)})`;
    if (filePath)
        return `${noun}(${shortPath(filePath)})`;
    if (pattern)
        return `${noun}("${trimCmd(pattern)}")`;
    if (url)
        return `${noun}(${url})`;
...
function registerProgressHook(runtime: IAgentRuntime): () => void {
    const acp = runtime.getService<AcpService>(AcpService.serviceType);
    runtime.logger?.debug?.({ src: "@elizaos/plugin-agent-orchestrator" }, `registerProgressHook acp=${acp ? "FOUND" : "MISSING"} onSessionEvent=${typeof acp?.onSessionEvent}`);
    if (!acp?.onSessionEvent) {
        runtime.logger?.warn?.({ src: "@elizaos/plugin-agent-orchestrator" }, "AcpService not available; sub-agent progress streaming disabled");
        return () => undefined;
    }
    ...
    // toolCallId.
    const toolHistory = new Map<string, Array<{
        id: string;
        formatted: string;
    }>>();
    ...
    const threadCacheKey = (source: string, roomId: string, label: string): string => `${source}::${roomId}::${label}`;
    ...
    const delayedProgressPayloads = new Map<string, {
        target: {
            source: string;
    ...
    if (!taggedRuntime.__orchestratorSendWrapped &&
        typeof runtime.sendMessageToTarget === "function") {
        const originalSend: SendMessageFn = runtime.sendMessageToTarget.bind(runtime) as SendMessageFn;
        const INTERNAL_SOURCES = new Set([
            "sub_agent_progress",
            "sub_agent_complete",
        ]);
        const wrapped: SendMessageFn = async (target, content) => {
            const contentSource = typeof (content as {
                source?: unknown;
            })?.source === "string"
                ? (content as {
                    source: string;
            ...
            if (lastPlannerReplyAtByRoom.size > 512) {
                ...
                if (oldest !== undefined)
                    lastPlannerReplyAtByRoom.delete(oldest);
            }
            ...
            if (!thread)
                return originalSend(target, content);
            try {
                const result = await runtime.postToThreadOnTarget(target, thread, content);
    ...
    const startHeartbeat = (sessionId: string, label: string, source: string, roomId: `${string}-${string}-${string}-${string}-${string}`): void => {
        ...
        const timer = setInterval(async () => {
            try {
                ...
                const raw = typeof acp.getSessionOutput === "function"
                ...
                if (cleaned.trim().length === 0 && tools.length === 0)
                    return;
                const toolsLine = tools.length > 0
                    ? `\nTools the sub-agent has called (in source order): ${tools.map((t) => t.formatted).join(", ")}`
                    : "";
                const filledPrompt = HEARTBEAT_SUMMARY_PROMPT.replace("{tail}", `${cleaned}${toolsLine}`.trim() || "(no narration captured yet)");
                ...
                if (prevSummary && norm(prevSummary) === norm(trimmedSummary))
                    return;
    ...
    const generateSpawnAck = async (sessionId: string, label: string): Promise<string> => {
        try {
            ...
            const task = typeof meta.initialTask === "string" && meta.initialTask.trim()
                ? meta.initialTask
                : label;
            return await withSpawnAckTimeout(runtime
                .useModel(ModelType.TEXT_SMALL, {
                system: buildSpawnAckSystemPrompt(runtime.character),
                prompt: buildSpawnAckUserPrompt(task),
                temperature: 0.7,
            })
                .then((raw) => sanitizeSpawnAck(typeof raw === "string" ? raw : "") ||
                SPAWN_ACK_FALLBACK), SPAWN_ACK_FALLBACK);
        }
        catch {
    ...
    function transientContent(text: string, source: "sub_agent_progress" | "sub_agent_complete"): {
        text: string;
        source: string;
        metadata: {
            transient: true;
    ...
    const resolveCanEdit = (source: string): boolean => hasCap(source, "edit_message");
    const resolveCanReact = (source: string): boolean => progressPolicy.mode === "threaded" &&
        progressPolicy.reactions &&
        hasCap(source, "react_message");
    const resolveCanThread = (source: string): boolean => progressPolicy.mode === "threaded" &&
        hasCap(source, "create_thread") &&
        hasCap(source, "post_to_thread");
    async function bestEffortReact(target: TargetInfo, messageId: string, emoji: string): Promise<void> {
        if (typeof runtime.addReactionOnTarget !== "function")
            return;
        try {
    ...
    async function resolveEmitTarget(target: {
        ...
        if (emitTargetCacheByKey.size > 512) {
            ...
            if (oldest !== undefined)
                emitTargetCacheByKey.delete(oldest);
        }
        return resolved;
    }
    ...
    async function emitProgress(sessionId: string, target: {
        source: string;
        roomId: `${string}-${string}-${string}-${string}-${string}`;
    }, rawText: string, label?: string): Promise<void> {
        if (progressPolicy.mode === "silent")
            return;
        ...
        if (!state && progressPolicy.delayMs > 0) {
            const firstSeenAt = delayedProgressFirstSeenAt.get(sessionId) ?? Date.now();
            ...
            if (elapsed < progressPolicy.delayMs) {
                if (!delayedProgressTimers.has(sessionId)) {
                    const timer = setTimeout(() => {
                        ...
                        if (!payload)
                            return;
                        void emitProgress(sessionId, payload.target, payload.rawText, payload.label);
        ...
        const displayText = progressPolicy.mode === "threaded" ? text : compactProgressText(text);
        ...
        try {
            ...
            if (state?.thread && typeof runtime.postToThreadOnTarget === "function") {
                ...
                await runtime.postToThreadOnTarget(sendTarget, state.thread, transientContent(threadText, "sub_agent_progress"));
            ...
            if (state?.canEdit &&
                ...
                if (state.lastText === displayText)
                    return;
                await runtime.editMessageOnTarget(sendTarget, state.mainMessageId, transientContent(displayText, "sub_agent_progress"));
                state.lastText = displayText;
                return;
            }
            // ── first emit OR fallback: send a fresh main-channel message ──
            const sessionLabel = state?.label ?? label ?? "sub-agent";
            const mainCacheKey = threadCacheKey(target.source, target.roomId, sessionLabel);
            if (!state && terminalSessions.has(sessionId))
                return;
            ...
            if (!state) {
                if (firstPostInFlight.has(sessionId))
                    return;
            ...
            const delivery = requireConfirmedSendHandlerDelivery(await runtime.sendMessageToTarget(sendTarget, transientContent(initialText, "sub_agent_progress")));
            ...
            if (!state &&
                ...
                if (canThread && typeof runtime.createThreadOnTarget === "function") {
                    const cacheKey = threadCacheKey(target.source, target.roomId, sessionLabel);
                    ...
                    if (thread) {
                        ...
                        await acp
                            ...
                            .catch((err: unknown) => runtime.logger?.warn?.({
                            src: "@elizaos/plugin-agent-orchestrator",
                            sessionId,
                            err: err instanceof Error ? err.message : String(err),
                        ...
                        if (displayText !== initialText &&
                            ...
                            try {
                                await runtime.postToThreadOnTarget(sendTarget, thread, transientContent(displayText, "sub_agent_progress"));
            ...
            if (alreadyWarned) {
                ...
                if (emitFailedSessions.size > 512) {
                    ...
                    if (oldest !== undefined)
                        emitFailedSessions.delete(oldest);
                }
                runtime.logger?.warn?.(logFields, "emitProgress failed");
            }
        }
    }
    async function markTaskComplete(sessionId: string, target: TargetInfo, summary: string): Promise<void> {
        const state = progressBySession.get(sessionId);
        if (!state)
            return;
        ...
        const sendTarget = target.roomId
            ? await resolveEmitTarget({
                source: target.source,
                roomId: target.roomId as `${string}-${string}-${string}-${string}-${string}`,
            })
            : target;
        const completionText = progressPolicy.mode === "threaded"
            ? `✅ [${state.label}] ${summary}`
        ...
        const suppressCompletionPost = progressPolicy.mode === "ack" || progressPolicy.mode === "silent";
        if (!suppressCompletionPost) {
            if (state.canEdit && typeof runtime.editMessageOnTarget === "function") {
                try {
                    await runtime.editMessageOnTarget(sendTarget, state.mainMessageId, transientContent(completionText, "sub_agent_complete"));
                }
                catch {
                ...
                try {
                    requireConfirmedSendHandlerDelivery(await runtime.sendMessageToTarget(sendTarget, transientContent(completionText, "sub_agent_complete")));
                }
                catch {
    ...
    async function markTaskFailed(sessionId: string, target: TargetInfo): Promise<void> {
        const state = progressBySession.get(sessionId);
        if (!state)
            return;
        const sendTarget = target.roomId
        ...
        const suppressFailurePost = progressPolicy.mode === "ack" || progressPolicy.mode === "silent";
        if (!state.canEdit && !suppressFailurePost) {
            try {
                requireConfirmedSendHandlerDelivery(await runtime.sendMessageToTarget(sendTarget, transientContent(progressPolicy.mode === "threaded"
                    ? `❌ [${state.label}] failed`
                    : `Failed ${state.label}.`, "sub_agent_complete")));
            }
            catch {
    ...
    async function flushMessageBuffer(sessionId: string, label: string, source: string, roomId: `${string}-${string}-${string}-${string}-${string}`): Promise<void> {
    ...
    runtime.logger?.debug?.({ src: "@elizaos/plugin-agent-orchestrator" }, "HOOK REGISTERED on AcpService");
    const unsubscribeSessionEvents = acp.onSessionEvent(async (sessionId, evName, data) => {
        ...
        try {
            ...
            const roomId = resolveOriginRoomId(meta) as `${string}-${string}-${string}-${string}-${string}` | undefined;
            if (!source || !roomId)
                return;
            const label = typeof meta.label === "string" && meta.label.trim().length > 0
                ? meta.label
                : `sub-agent ${sessionId}`;
            const isTerminalEvent = evName === "stopped" ||
                evName === "error" ||
            ...
            const isVerifyRetrySpawn = typeof meta.buildVerifyRetryCount === "number" &&
            ...
            if (progressPolicy.mode === "ack" &&
                ...
                const createdAtMs = session?.createdAt instanceof Date
                    ...
                    : undefined;
                const plannerAlreadyAcked = plannerAlreadyAckedSpawn(lastPlannerReplyAtByRoom.get(roomId), createdAtMs, PLANNER_ACK_LOOKBACK_MS);
            ...
            if (evName === "ready" || evName === "tool_running") {
                ...
                if (evName === "task_complete") {
                    const rawResponse = typeof (data as {
                        response?: unknown;
                    })?.response === "string"
                        ? (data as {
                            response: string;
                ...
                if (terminalState) {
                    const cacheKey = threadCacheKey(source, roomId, terminalState.label);
            ...
            if (evName === "tool_running") {
                ...
                const isNonInformative = trimmedFormatted.length === 0 ||
            ...
            switch (evName) {
                ...
                case "error": {
                    ...
                    if (!failureKind || !USER_ACTION_KINDS.has(failureKind))
                        return;
                    const msg = (data as {
                        message?: string;
                    })?.message ?? "error";
            ...
            if (!text)
                return;
            const dedupeKey = evName === "error" ||
                evName === "blocked" ||
                evName === "login_required"
                ? `${sessionId}:${evName}`
                : `${sessionId}:${evName}:${text}`;
            const dedupeWindow = evName === "error" ||
                evName === "blocked" ||
                evName === "login_required"
                ? 30000
                : POST_DEBOUNCE_MS;
    ...
    return () => {
        ...
        for (const timer of delayedProgressTimers.values())
            clearTimeout(timer);
...
export { type ChildDeliveryStatus, type ChildTerminalArtifactRef, type ChildTerminalResultEnvelope, type ChildTerminalStatus, type ChildVerificationStatus, deriveChildTerminalResult, } from "./services/child-terminal-result.js";
...
export { enforcePiProviderCredentialIsolation, isPiProviderRouteId, PI_PROVIDER_ROUTES, type PiApiDialect, type PiBillingMode, type PiProviderRoute, type PiProviderRouteId, type PiTermsPolicy, preparePiProviderRoute, } from "./services/pi-provider-config.js";
...
export { assertSubscriptionCodingAdapterReady, classifySubscriptionRuntimeFailure, isSubscriptionCodingAdapter, probeSubscriptionCodingAdapter, SUBSCRIPTION_CODING_ADAPTER_IDS, SUBSCRIPTION_CODING_ADAPTERS, type SubscriptionAdapterProbeStatus, type SubscriptionBillingSource, type SubscriptionCodingAdapterDescriptor, SubscriptionCodingAdapterError, type SubscriptionCodingAdapterErrorCode, type SubscriptionCodingAdapterId, type SubscriptionCodingAdapterProbe, type SubscriptionCodingAdapterProbeOptions, type SubscriptionExecutionMode, type SubscriptionLoginCommand, stripSubscriptionApiEnvironment, subscriptionCodingAdapterCommand, } from "./services/subscription-coding-adapters.js";
...
export { SWARM_COORDINATOR_SERVICE_TYPE, SwarmCoordinatorService, type SwarmEvent, type SwarmEventListener, } from "./services/swarm-coordinator-service.js";
...
export { type ActiveLaneScope, detectWaveCollisions, isSalvageEligible, NoopWaveRefillPlanner, type OpenPullRequestScope, type OpenPullRequestSource, readLaneDependencies, readLaneId, readWaveAttemptId, readWaveId, shouldRefillWave, WAVE_BUDGET_BREACH_CODE, WAVE_ID_METADATA_KEY, WAVE_REFILL_PLANNER_SERVICE_TYPE, WAVE_SUPERVISOR_SERVICE_TYPE, WAVE_SUPERVISOR_SETTING, WaveBudgetBreachError, type WaveCollision, WaveConcurrencyCapError, type WaveRefillPlanner, type WaveRefillRequest, type WaveReplacementSpec, type WaveStatus, WaveSupervisor, } from "./services/wave-supervisor.js";
