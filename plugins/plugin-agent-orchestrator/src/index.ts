...
export function registerProgressHook(runtime: IAgentRuntime): () => void {
  ...
  async function flushMessageBuffer(
    ...
    // evaluator builds. The full narration is durable (session transcript +
    // the completion summary), so a partial view names its continuation
    // reference instead of a bare "[+]" (#24262 close-out).
    const wellFormed = toWellFormedUnicode(trimmed);
    const narrationView = boundedContentView(
      wellFormed,
      800,
      orchestratorContentRef("session-output", sessionId),
    );
    const text = `💬 [${label}] ${narrationView.view}`;
