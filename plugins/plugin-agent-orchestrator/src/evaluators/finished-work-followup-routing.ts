...
export const finishedWorkFollowUpRoutingEvaluator: ResponseHandlerEvaluator = {
  ...
  evaluate: async ({ runtime, message, messageHandler }) => {
    ...
    const routed = (Array.isArray(sessions) ? sessions : []).filter((s) => {
      ...
      // garbage title into every successor (live 2026-08-21). The sniff scans
      // the FULL initialTask (no prefix slice): sprayed junk can sit past any
      // fixed offset, and the string is already in memory.
      const label = `${String(meta.label ?? "")} ${String(meta.initialTask ?? "")}`;
