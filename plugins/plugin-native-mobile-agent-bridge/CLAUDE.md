...
src/
  ...
  web.ts            Web/Electrobun fallback. startInboundTunnel always resolves to state:"error".
...
- **Web fallback always errors.** `startInboundTunnel` on the web implementation always resolves with `state: "error"`. This is intentional — do not make it a silent no-op.
