...
import { SAMServiceType, DEFAULT_SAM_OPTIONS, type SamTTSOptions, type HardwareBridgeService } from '../types';
...
export class SamTTSService extends Service {
  ...
  generateAudio(text: string, options: Partial<SamTTSOptions> = {}): Uint8Array {
    ...
    logger.info(`[SAM-TTS] Synthesizing: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`);
  ...
  async speakText(text: string, options: Partial<SamTTSOptions> = {}): Promise<Uint8Array> {
    ...
    const hardwareBridge = this.runtime.getService('hardwareBridge') as unknown as HardwareBridgeService | undefined;
