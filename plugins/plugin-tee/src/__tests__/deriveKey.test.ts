...
describe("DeriveKeyProvider", () => {
  ...
  describe("rawDeriveKey", () => {
    let _provider: PhalaDeriveKeyProvider;

    beforeEach(() => {
      _provider = new PhalaDeriveKeyProvider(TEEMode.LOCAL);
    });
    ...
    it("should handle errors during raw key derivation", async () => {
      ...
      vi.mocked(TappdClient).mockImplementationOnce(() => {
        ...
        instance.deriveKey = vi.fn();
        return instance;
      });

      const provider = new PhalaDeriveKeyProvider(TEEMode.LOCAL);
      await expect(provider.rawDeriveKey("path", "subject")).rejects.toThrow(
        mockError
      );
    });
  });
  ...
  describe("deriveEd25519Keypair", () => {
    ...
    it("should derive Ed25519 keypair successfully", async () => {
      ...
      const client = vi.mocked(TappdClient).mock.results[0].value;
      expect(client.deriveKey).toHaveBeenCalledWith(path, subject);
      expect(result.keypair.publicKey.toBase58()).toEqual(
