...
impl MigrationTracker {
    ...
    pub async fn ensure_tables(&self) -> Result<()> {
        let mut tx = self
            ...
            .begin()
            .await
            .context("Failed to begin migration tracking table setup")?;

        sqlx::query("SELECT pg_advisory_xact_lock(hashtext('elizaos_plugin_sql_migration_tables'))")
            .execute(&mut *tx)
            .await
            .context("Failed to lock migration tracking table setup")?;
        ...
        tx.commit()
            .await
            .context("Failed to commit migration tracking table setup")?;
