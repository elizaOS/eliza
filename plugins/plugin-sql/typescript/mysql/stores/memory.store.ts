/**
 * MySQL Memory Store - Batch-optimized CRUD operations for memories and embeddings
 * 
 * ARCHITECTURE:
 ...
 * - JSON_CONTAINS() for metadata filtering (MySQL 5.7.8+)
 * - Vector distance functions for similarity search (MySQL 9.0+ with vector type)
 * 
 * WHY BATCH-FIRST:
 * Same rationale as PostgreSQL store - memory operations are inherently multi-item.
 * 
 * MYSQL-SPECIFIC OPTIMIZATIONS:
 * - ON DUPLICATE KEY UPDATE for upserts (instead of PG's ON CONFLICT)
 * - JSON functions (JSON_EXTRACT, JSON_CONTAINS, JSON_MERGE_PATCH)
 * - No RETURNING clause - must re-query for created IDs
 * - CAST(... AS JSON) instead of ::jsonb cast syntax
 * 
 * PERFORMANCE CHARACTERISTICS:
 * - getMemories with metadata: O(N) without index (JSON_CONTAINS not well-optimized)
 * - Consider generated columns + B-tree index for frequently queried metadata fields
 * - createMemories: O(N) with multi-row INSERT
 * - updateMemories: O(N) with CASE expression
 */
import {
  type Memory,
  type MemoryMetadata,
...
export async function getMemories(
  ...
  // WHY: MySQL has no RLS. PG relies on RLS for entity scoping when
  // ENABLE_DATA_ISOLATION is set, but MySQL must filter explicitly.
  if (entityId) {
    conditions.push(eq(memoryTable.entityId, entityId));
  }

  // WHY: MySQL JSON_CONTAINS allows filtering by metadata fields without
  // fetching 50K records and filtering in JS (seen in plugin-knowledge).
  // The metadata param is a partial object — MySQL checks if the stored
  // metadata JSON contains all key-value pairs specified in the filter.
  if (metadata) {
    conditions.push(sql`JSON_CONTAINS(${memoryTable.metadata}, ${JSON.stringify(metadata)})`);
  }
...
/**
 * Retrieves cached embeddings from the database using CHAR_LENGTH difference
 * as a string similarity approximation (MySQL lacks native levenshtein).
 * @param {DrizzleDatabase} db - The database instance.
 ...
 * @param {number} opts.query_threshold - The threshold for the similarity score.
...
export async function getCachedEmbeddings(
  ...
  try {
    // MySQL doesn't have levenshtein() natively. We use a simpler string match
    // approach: find memories with content matching the input, then score by
    // character length difference as an approximation.
    const fieldPath = `$.${opts.query_field_sub_name}`;
    const results = await db.execute(sql`
      SELECT
        COALESCE(e.dim_384, e.dim_512, e.dim_768, e.dim_1024, e.dim_1536, e.dim_3072) as embedding,
        ABS(CHAR_LENGTH(JSON_UNQUOTE(JSON_EXTRACT(m.content, ${fieldPath}))) - CHAR_LENGTH(${opts.query_input})) as levenshtein_score
      FROM memories m
      LEFT JOIN embeddings e ON e.memory_id = m.id
      WHERE m.type = ${opts.query_table_name}
        AND JSON_EXTRACT(m.content, ${fieldPath}) IS NOT NULL
...
/**
 * Searches for memories by embedding vector using MySQL DISTANCE function with COSINE metric.
 * @param {DrizzleDatabase} db - The database instance.
 * @param {UUID} agentId - The ID of the agent.
 * @param {EmbeddingDimensionColumn} embeddingDimension - The embedding dimension column to use.
...
export async function searchMemoriesByEmbedding(
  ...
  // MySQL 9+ vector distance using DISTANCE function with COSINE metric
  const vectorStr = JSON.stringify(cleanVector);
  const similarity = sql<number>`(1 - DISTANCE(${embeddingTable[embeddingDimension]}, STRING_TO_VECTOR(${vectorStr}), 'COSINE'))`;
 ...
 * Uses CAST(... AS JSON) for MySQL JSON columns.
 * @param {DrizzleDatabase} db - The database instance (may be a transaction context).
 * @param {UUID} agentId - The ID of the agent.
 * @param {EmbeddingDimensionColumn} embeddingDimension - The embedding dimension column to use.
...
export async function createMemory(
  ...
  // WHY SELECT 1 instead of getMemoryById: the old code fetched the full row
  // (with a LEFT JOIN on embeddings) just to check existence. SELECT 1 with
  // LIMIT 1 is enough -- the full row is never used when we bail out early.
  ...
  // Ensure we always pass a JSON string to the SQL placeholder.
  ...
  await db.insert(memoryTable).values([
    {
      id: memoryId,
      type: tableName,
      content: sql`CAST(${contentToInsert} AS JSON)`,
      metadata: sql`CAST(${metadataToInsert} AS JSON)`,
      entityId: memory.entityId,
      roomId: memory.roomId,
      worldId: memory.worldId,
    ...
    await db.transaction(async (tx) => {
      ...
      if (memory.content) {
        ...
        await tx
          .update(memoryTable)
          .set({
            content: sql`CAST(${contentToUpdate} AS JSON)`,
            ...(memory.metadata && {
              metadata: sql`CAST(${metadataToUpdate} AS JSON)`,
            }),
          })
...
export async function deleteManyMemories(
  ...
  // WHY: Batch fragment lookup instead of N individual calls. The old code
  // called deleteMemoryFragments per memoryId (N queries to find fragments +
  // N deletes). Now we find ALL fragments for all document memories in one
  // query, then batch-delete their embeddings and the fragments themselves.
  await db.transaction(async (tx) => {
    const BATCH_SIZE = 100;
    for (let i = 0; i < memoryIds.length; i += BATCH_SIZE) {
      const batch = memoryIds.slice(i, i + BATCH_SIZE);

      // Batch-find all fragments for document memories in this batch
      const allFragments = await tx
        .select({ id: memoryTable.id })
        .from(memoryTable)
        .where(
          and(
            eq(memoryTable.agentId, agentId),
            sql`JSON_UNQUOTE(JSON_EXTRACT(${memoryTable.metadata}, '$.documentId')) IN (${sql.join(batch.map(id => sql`${id}`), sql`, `)})`
          )
        );

      if (allFragments.length > 0) {
        const fragmentIds = allFragments.map(f => f.id) as UUID[];
        // Delete fragment embeddings
 ...
 * Uses MySQL JSON_UNQUOTE(JSON_EXTRACT(...)) for JSON field access.
 * @param {DrizzleDatabase} db - The database instance (may be a transaction context).
 * @param {UUID} agentId - The ID of the agent.
...
export async function deleteAllMemories(
  ...
  await db.transaction(async (tx) => {
    ...
    // WHY: Batch fragment lookup instead of N individual deleteMemoryFragments calls.
    // Find ALL fragments whose documentId matches any of the memory IDs in one query.
    const allFragments = await tx
      .select({ id: memoryTable.id })
      .from(memoryTable)
      .where(
        and(
          eq(memoryTable.agentId, agentId),
          sql`JSON_UNQUOTE(JSON_EXTRACT(${memoryTable.metadata}, '$.documentId')) IN (${sql.join(ids.map(id => sql`${id}`), sql`, `)})`
        )
      );

    if (allFragments.length > 0) {
      const fragmentIds = allFragments.map(f => f.id) as UUID[];
      // Delete fragment embeddings in batch
      await tx.delete(embeddingTable).where(inArray(embeddingTable.memoryId, fragmentIds));
      // Delete fragments in batch
      await tx.delete(memoryTable).where(inArray(memoryTable.id, fragmentIds));
    }

    // 2) delete embeddings for the target memories in batch
...
/**
 * Counts the number of memories matching criteria (MySQL).
...
export async function countMemories(
  ...
  if (isObjectParams) {
    ...
    if (params.metadata) {
      // MySQL JSON_CONTAINS for metadata filtering
      conditions.push(sql`JSON_CONTAINS(${memoryTable.metadata}, ${JSON.stringify(params.metadata)})`);
    }
  } else {
...
//        embedding insert = 3 queries (plus per-memory similarity checks
//        when needed, which can't be batched).

/**
 * Creates multiple memories using multi-row INSERT.
 * Uses CAST(... AS JSON) for MySQL JSON columns.
 */
export async function createMemories(
  ...
  // Phase 2: Uniqueness resolution (per-memory similarity check, can't batch)
  ...
  // Phase 3a: Batch INSERT memories
  const memoryValues = toCreate.map(({ memory, tableName }) => {
    ...
    return {
      id: memory.id,
      type: tableName,
      content: sql`CAST(${contentStr} AS JSON)`,
      metadata: sql`CAST(${metadataStr} AS JSON)`,
      entityId: memory.entityId,
      roomId: memory.roomId,
      worldId: memory.worldId,
      agentId: memory.agentId || agentId,
  ...
  // MySQL: use onDuplicateKeyUpdate as equivalent to PG's onConflictDoNothing
  await db.insert(memoryTable).values(memoryValues)
    .onDuplicateKeyUpdate({ set: { id: sql`id` } });

  // Phase 3b: Batch INSERT embeddings
  ...
  if (embeddingRows.length > 0) {
    await db
      .insert(embeddingTable)
      .values(embeddingRows as unknown as (typeof embeddingTable.$inferInsert)[]);
  }
 ...
 *   Old: N separate UPDATEs (each with round-trip overhead)
 ...
 *
 * MySQL: uses CAST(... AS JSON) for JSON columns; STRING_TO_VECTOR for vectors.
 */
    ...
    await db.transaction(async (tx) => {
      ...
      if (contentMemories.length > 0) {
        ...
        const contentCases = contentMemories.map((m) => {
          ...
          return sql`WHEN ${memoryTable.id} = ${m.id} THEN CAST(${contentStr} AS JSON)`;
        });
        const metaCases = contentMemories.map((m) => {
          ...
          return sql`WHEN ${memoryTable.id} = ${m.id} THEN CAST(${metaStr} AS JSON)`;
        });
      ...
      if (embMemories.length > 0) {
        ...
        if (toInsert.length > 0) {
          await tx
            .insert(embeddingTable)
            .values(toInsert as unknown as (typeof embeddingTable.$inferInsert)[]);
        }

        // Batch UPDATE existing embeddings via CASE (1 query)
        // MySQL: use STRING_TO_VECTOR for vector columns; memoryId as string
        if (toUpdate.length > 0) {
          const dimCol = embeddingTable[embeddingDimension];
          const dimCases = toUpdate.map(
            (u) =>
              sql`WHEN ${embeddingTable.memoryId} = ${u.memoryId} THEN STRING_TO_VECTOR(${JSON.stringify(u.vector)})`
          );
 ...
 */
/**
 * WHY: Direct batch delete of embeddings + memories by ID. The old code called
 * deleteManyMemories(db, memoryIds) but that function expects (db, agentId,
 * memoryIds) — memoryIds landed in the agentId slot (pre-existing bug).
 *
 * Fragment cleanup is skipped here because this wrapper doesn't receive agentId.
 * Callers that delete document memories should call deleteManyMemories directly
 * with the agentId to ensure fragment cleanup.
 */
export async function deleteMemories(
  ...
  await db.delete(embeddingTable).where(inArray(embeddingTable.memoryId, memoryIds));
  // Delete the memories themselves
  await db.delete(memoryTable).where(inArray(memoryTable.id, memoryIds));
}

/**
 * Upserts multiple memories by ID (MySQL implementation).
 * 
 * WHY: MySQL uses ON DUPLICATE KEY UPDATE instead of ON CONFLICT.
 * No similarity checks (caller asserts known identity).
 */
export async function upsertMemories(
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      // Upsert memories: update content/metadata/unique on duplicate key
      await tx.insert(memoryTable).values(memoryValues).onDuplicateKeyUpdate({
        set: {
          // VALUES() deprecated in MySQL 8.0.20+ but still works
          content: sql.raw('VALUES(`content`)'),
          metadata: sql.raw('VALUES(`metadata`)'),
          unique: sql.raw('VALUES(`unique`)'),
      ...
      if (embeddingRows.length > 0) {
        // TRAP: embeddingDimension is a Drizzle camelCase property (e.g. "dim384")
        // but the actual SQL column is snake_case (e.g. "dim_384"). Drizzle handles
        // the left side via property→column mapping, but VALUES() is raw SQL.
        const sqlColumnName = embeddingDimension.replace(/(\d)/, '_$1');
        const updateSet: Record<string, unknown> = {};
        updateSet[embeddingDimension] = sql.raw(`VALUES(\`${sqlColumnName}\`)`);

        await tx.insert(embeddingTable).values(embeddingRows).onDuplicateKeyUpdate({
          set: updateSet,
        });
      }
    });
