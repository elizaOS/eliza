...
function toTimestampNumber(value: number | bigint | null | undefined): number {
...
export class LogStore implements Store {
  ...
  async getAgentRunSummaries(
    ...
    return this.ctx.withIsolationContext(params.entityId ?? null, async (tx) => {
      ...
      runs.sort((a, b) => {
        const aStarted = toTimestampNumber(a.startedAt);
        const bStarted = toTimestampNumber(b.startedAt);
