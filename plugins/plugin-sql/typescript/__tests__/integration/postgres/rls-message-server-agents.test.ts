...
describe.skipIf(!process.env.POSTGRES_URL)(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      await setupClientA.query("SELECT set_config('app.server_id', $1, false)", [serverAId]);
      await setupClientB.connect();
      await setupClientB.query("SELECT set_config('app.server_id', $1, false)", [serverBId]);
      ...
      await serverAClient.query("SELECT set_config('app.server_id', $1, false)", [serverAId]);
      await serverBClient.connect();
      await serverBClient.query("SELECT set_config('app.server_id', $1, false)", [serverBId]);
