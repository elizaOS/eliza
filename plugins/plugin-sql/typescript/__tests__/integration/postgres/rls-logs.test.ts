...
describe.skipIf(!process.env.POSTGRES_URL)("PostgreSQL RLS - Logs Isolation (STRICT)", () => {
  ...
  beforeAll(async () => {
    ...
    await setupClient.query("SELECT set_config('app.server_id', $1, false)", [serverId]);
    ...
    await aliceClient.query("SELECT set_config('app.server_id', $1, false)", [serverId]);
    ...
    await bobClient.query("SELECT set_config('app.server_id', $1, false)", [serverId]);
