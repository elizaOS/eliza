 ...
 * - withIsolationContext() is required for log insertion
 *
...
describe.skipIf(!process.env.POSTGRES_URL)(
  "PostgreSQL RLS - Logs Isolation (STRICT)",
  () => {
    ...
    beforeAll(async () => {
      ...
      await setupClient.query("SELECT set_config('app.server_id', $1, false)", [
        serverId,
      ]);
      ...
      await aliceClient.query("SELECT set_config('app.server_id', $1, false)", [
        serverId,
      ]);
      ...
      await bobClient.query("SELECT set_config('app.server_id', $1, false)", [
        serverId,
      ]);

      // Setup test data (with server context via set_config)
    ...
    afterAll(async () => {
      ...
      try {
        ...
        await setupClient.query("BEGIN");
        await setupClient.query(`SET LOCAL app.entity_id = '${bobId}'`);
        await setupClient.query(`DELETE FROM logs WHERE room_id = $1`, [
          bobPrivateRoomId,
        ]);
        await setupClient.query("COMMIT");
        ...
        await setupClient.query(
          `DELETE FROM participants WHERE room_id IN ($1, $2, $3)`,
          [sharedRoomId, alicePrivateRoomId, bobPrivateRoomId],
        );
        ...
        await setupClient.query(`DELETE FROM entities WHERE id IN ($1, $2)`, [
          aliceId,
          bobId,
        ]);
        await setupClient.query(`DELETE FROM agents WHERE id = $1`, [agentId]);
        await setupClient.query(`DELETE FROM servers WHERE id = $1`, [
          serverId,
        ]);
      } catch (err) {
    ...
    `,
      ...
      expect(aliceResult.rows.every((row) => row.entity_id === aliceId)).toBe(
        true,
      );
