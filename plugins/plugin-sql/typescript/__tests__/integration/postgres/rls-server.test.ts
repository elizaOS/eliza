...
describe.skipIf(!process.env.POSTGRES_URL)("PostgreSQL RLS Server Integration", () => {
  ...
  beforeAll(async () => {
    ...
    await setupClient1.query("SELECT set_config('app.server_id', $1, false)", [server1Id]);
    await setupClient2.connect();
    await setupClient2.query("SELECT set_config('app.server_id', $1, false)", [server2Id]);
    ...
    await userClient1.query("SELECT set_config('app.server_id', $1, false)", [server1Id]);
    await userClient2.connect();
    await userClient2.query("SELECT set_config('app.server_id', $1, false)", [server2Id]);
