 ...
 * - withIsolationContext() correctly sets entity context via parameterized set_config()
 *
 ...
 * production code path. This ensures the parameterized set_config() approach is tested.
 */
...
describe.skipIf(!process.env.POSTGRES_URL)(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      // Set server context for RLS via session variable (read by current_server_id())
      ...
      // This tests the actual production code path (withIsolationContext + set_config)
    ...
    it("should allow Alice to see room1 memories (tests withIsolationContext + set_config)", async () => {
      // This test verifies the production code path works:
      // withIsolationContext() -> set_config('app.entity_id', entityId, true)
    ...
    it("should combine Server RLS + Entity RLS (double isolation)", async () => {
      ...
      try {
        ...
        const result = await wrongServerManager.withIsolationContext(
          aliceId as UUID,
          async (tx) => {
            return await tx.execute(
