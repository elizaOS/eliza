...
- **Membership evidence is distinct from participant presence.** The participant row stores a generation-fenced observation whose transport source and account provenance were derived by a core-issued direct-plugin capability. Participant creation validates and row-locks the room and entity tenant; removal and singular room mutation include the adapter agent predicate. The SQL publication transaction row-locks and validates the persisted room/account and entity tenant before advancing participant evidence. Only fresh expiring `member` evidence is returned by `getCurrentRoomMemberships`; transport evidence has a hard 15-minute maximum TTL, and stale, absent, negative, indeterminate, unsupported, and unavailable observations fail closed while remaining inspectable. Document reads and mutations intersect supplied room ids with this evidence under a shared row lock inside their SQL transaction.
