...
export async function applyIdentityPersonLinkAttestationGuard(
  ...
  return db.transaction(async (tx) => {
    ...
    // Serialize refreshes before touching the shared function or trigger. The
    // table lock remains held until the complete replacement commits.
    await tx.execute(sql`
      LOCK TABLE public.identity_person_link_attestations IN ACCESS EXCLUSIVE MODE
    `);
