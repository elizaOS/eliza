...
import { type ConnectorAccountPatch } from "@elizaos/core";
import { type ConnectorAccountStorage } from "@elizaos/core";
import { type IAgentRuntime } from "@elizaos/core";
import { vi } from "vitest";
const { OAuth2Client } = Auth;
const TEST_OIDC_NONCE = "test-oidc-nonce";
import googlePlugin, { createGoogleConnectorAccountProvider, DefaultGoogleCredentialResolver, GOOGLE_CAPABILITIES, GOOGLE_MEET_API_SURFACE, GOOGLE_OAUTH_SCOPES, type GoogleApiClientFactory, GoogleCalendarClient, GoogleDriveClient, GoogleGmailClient, GoogleMeetClient, GoogleMeetStatus, GoogleWorkspaceService, getGoogleOAuthProviderConfig, normalizeGoogleCapabilities, scopesForGoogleCapabilities, } from "./index.js";
...
describe("google plugin", () => {
    ...
    it("registers the Google connector account provider on init", async () => {
        ...
        expect(getConnectorAccountManager(runtime).getProvider("google")).toEqual(expect.objectContaining({
            provider: "google",
    ...
    it("keeps provider capabilities aligned with the shared connector declaration", () => {
        expect(getConnectorAccountCatalogEntry("google")?.oauthCapabilities?.map((capability) => capability.id)).toEqual(GOOGLE_CAPABILITIES);
    });
    it("normalizes capability input and preserves opt-in OAuth metadata without incremental grants", () => {
        const config = getGoogleOAuthProviderConfig(normalizeGoogleCapabilities(["drive.read", "drive.read", "meet.read", "unknown"]));
    ...
    it("rejects unknown connector OAuth capabilities instead of widening access", async () => {
        ...
        await expect(provider.startOAuth?.({
            provider: "google",
        ...
        }, {} as never)).rejects.toThrow("Google OAuth capability or scope is not recognized: google.calendar.read");
    });
    it("rejects identity-only connector OAuth requests with no usable capability", async () => {
        ...
        }, {} as never)).rejects.toThrow("Google OAuth requires at least one Gmail, Calendar, Drive, or Meet capability.");
    });
    it.each([
        {
        ...
        }, {} as never)).rejects.toThrow("Google OAuth requires an explicit Gmail, Calendar, Drive, or Meet capability selection.");
    });
    it("defaults re-auth with accountId and omitted scopes to the account's recorded granted capabilities (#18543)", async () => {
        ...
        const requestedScopes = new Set((url.searchParams.get("scope") ?? "").split(" ").filter(Boolean));
        // Least privilege: exactly the recorded grant, never an expansion.
        expect(requestedScopes).toEqual(new Set([
            GOOGLE_OAUTH_SCOPES.profile.openid,
            GOOGLE_OAUTH_SCOPES.profile.email,
            GOOGLE_OAUTH_SCOPES.profile.profile,
            GOOGLE_OAUTH_SCOPES.gmail.read,
    ...
    it("keeps failing closed when the re-auth accountId is unknown", async () => {
        ...
        }, { getAccount: vi.fn(async () => null) } as never)).rejects.toThrow("Google OAuth cannot reauthorize a connector account that no longer exists.");
    });
    ...
    it("keeps account auth resolution explicit", async () => {
        ...
        await expect(service.searchMessages({ accountId: "acct_google_1", query: "from:example" })).rejects.toThrow("account acct_google_1");
    });
    ...
    it("resolves OAuth clients from connector credential refs and caches by credential version", async () => {
        ...
        const credentialStore = {
            get: vi.fn(async (vaultRef: string) => {
                if (vaultRef.endsWith(".access"))
                    return "access-token";
                if (vaultRef.endsWith(".refresh"))
    ...
    it("resolves OAuth clients from account metadata credential refs", async () => {
        ...
        expect(credentialStore.get).toHaveBeenCalledWith("connector.agent.google.acct_google_1.oauth_tokens", expect.objectContaining({ reveal: true }));
    });
    ...
    it("does not resolve OAuth clients from token-shaped account metadata", async () => {
        ...
        await expect(resolver.getAuthClient({
            provider: "google",
            accountId: "acct_google_1",
            capabilities: ["gmail.read"],
            scopes: scopesForGoogleCapabilities(["gmail.read"]),
    ...
    it("persists OAuth token material as vault-backed credential refs during callback", async () => {
        ...
        const runtime = {
            ...
            getService: (serviceType: string) => serviceType === "vault"
                ? {
        ...
        expect(credentialRef?.vaultRef).toMatch(new RegExp(`^connector\\.agent-1\\.google\\.${account.id}-[a-f0-9]+\\.oauth_tokens$`, "u"));
        expect(vault.get(credentialRef?.vaultRef ?? "")).toContain("google-access-token");
        expect(setCredentialRef).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("sanitizes provider-added scopes without rejecting a valid granted capability", async () => {
        ...
        const manager = createOAuthCallbackManager("google", "acct_google_incremental_grant", vi.fn(async () => undefined));
        const provider = createGoogleConnectorAccountProvider(runtime);
    ...
    it("does not record or re-request a compound capability from a partial provider grant", async () => {
        ...
        const manager = createOAuthCallbackManager("google", "acct_google_partial_compound", vi.fn(async () => undefined));
        const provider = createGoogleConnectorAccountProvider(runtime);
        const returnedScopes = [
            GOOGLE_OAUTH_SCOPES.profile.openid,
            GOOGLE_OAUTH_SCOPES.profile.email,
        ...
        const requestedScopes = new Set(new URL(reauthResult?.authUrl ?? "").searchParams.get("scope")?.split(" ") ?? []);
        expect(requestedScopes).toContain(GOOGLE_OAUTH_SCOPES.gmail.read);
    ...
    it("fails an OAuth callback that grants identity but no connector capability", async () => {
        ...
        const manager = createOAuthCallbackManager("google", "acct_google_identity_only", vi.fn(async () => undefined));
        vi.stubGlobal("fetch", vi.fn(async (url: string | URL | Request) => {
            const href = String(url);
            if (href.includes("oauth2.googleapis.com/token")) {
                return new Response(JSON.stringify({
        ...
        const provider = createGoogleConnectorAccountProvider(runtime);
        await expect(provider.completeOAuth?.({
            provider: "google",
            code: "oauth-code",
            query: {},
            flow: {
        ...
        }, manager as never)).rejects.toThrow("Google OAuth completed without a usable Gmail, Calendar, Drive, or Meet capability.");
    ...
    it.each([
        ...
        },
        {
            ...
            tokenFields: {},
            metadata: {},
            expectedMessage: "Google OAuth callback is missing the scopes bound to the authorization request.",
        },
        ...
        const manager = createOAuthCallbackManager("google", "acct_google_invalid_scope", vi.fn(async () => undefined));
        vi.stubGlobal("fetch", vi.fn(async (url: string | URL | Request) => {
            const href = String(url);
            if (href.includes("oauth2.googleapis.com/token")) {
                return new Response(JSON.stringify({
        ...
        }, manager as never)).rejects.toThrow(expectedMessage);
        expect(manager.upsertAccount).not.toHaveBeenCalled();
    });
    it("fails OAuth callback when no durable credential writer is available", async () => {
        ...
        const provider = createGoogleConnectorAccountProvider(runtime);
        const manager = createOAuthCallbackManager("google", "acct_google_durable_1", vi.fn(async () => undefined));
        await expect(provider.completeOAuth?.({
            provider: "google",
            code: "oauth-code",
            query: {},
        ...
        }, manager as never)).rejects.toThrow(/durable connector credential store|vault writer/i);
    });
    it("uses a fake Gmail client with selected Gmail read/send scopes", async () => {
        ...
        expect(factory.gmail).toHaveBeenNthCalledWith(1, expect.objectContaining({ accountId: "acct_google_1" }), ["gmail.read"], "gmail.searchMessages");
        expect(factory.gmail).toHaveBeenNthCalledWith(2, expect.objectContaining({ accountId: "acct_google_1" }), ["gmail.send"], "gmail.sendEmail");
    ...
    it("exposes rich Gmail management methods for LifeOps delegation", async () => {
        ...
        const client = new GoogleGmailClient(factory);
        await expect(client.searchGmailMessages({
            accountId: "acct_google_1",
            query: "in:inbox",
        ...
        await expect(client.getGmailMessageDetail({
            accountId: "acct_google_1",
            messageId: "msg_1",
            selfEmail: "grace@example.com",
        })).resolves.toMatchObject({
        ...
        await expect(client.getGmailSubscriptionHeaders({
            accountId: "acct_google_1",
            query: "unsubscribe",
            maxMessages: 1,
        })).resolves.toMatchObject([
        ...
        await expect(client.sendGmailReply({
            accountId: "acct_google_1",
            to: ["ada@example.com"],
            subject: "Need response",
            bodyText: "Done",
        ...
        await expect(client.createGmailFilterForSender({
            accountId: "acct_google_1",
    ...
    it("rejects hostile Gmail result limits before listing messages", async () => {
        ...
        await expect(client.listGmailUnrespondedThreads({
            accountId: "acct_google_1",
            olderThanDays: -4,
    ...
    it("escapes Drive folder IDs and preserves explicit trashed predicates", async () => {
        ...
        expect(fakeDrive.files.list).toHaveBeenNthCalledWith(1, expect.objectContaining({
            q: "'root\\' OR trashed = true OR \\'x' in parents and trashed = false",
            pageSize: 25,
        }));
        expect(fakeDrive.files.list).toHaveBeenNthCalledWith(2, expect.objectContaining({
    ...
    it("uses fake Calendar, Drive, and Meet clients with narrow capabilities", async () => {
        ...
        await expect(calendarClient.listEvents({ accountId: "acct_google_1", limit: 1 })).resolves.toMatchObject([
            {
                id: "event_1",
                calendarId: "primary",
                title: "Planning",
        ...
        await expect(calendarClient.getEvent({ accountId: "acct_google_1", eventId: "event_1" })).resolves.toMatchObject({
            id: "event_1",
            start: "2026-05-07T16:00:00.000Z",
        });
        await expect(calendarClient.updateEvent({
            accountId: "acct_google_1",
            eventId: "event_1",
        ...
        await expect(calendarClient.deleteEvent({ accountId: "acct_google_1", eventId: "event_1" })).resolves.toBeUndefined();
        await expect(driveClient.searchFiles({
            accountId: "acct_google_1",
            query: "name contains 'Plan'",
            limit: 1,
        })).resolves.toMatchObject([{ id: "file_1", name: "Plan", parents: ["root"] }]);
        await expect(driveClient.getFile({ accountId: "acct_google_1", fileId: "file_1" })).resolves.toMatchObject({ id: "file_1", name: "Plan", parents: ["root"] });
        await expect(driveClient.listDriveFiles({ accountId: "acct_google_1", folderId: "root", maxResults: 1 })).resolves.toMatchObject({
            files: [{ id: "file_1", name: "Plan", parents: ["root"] }],
            nextPageToken: null,
        });
        await expect(driveClient.getDocContent({ accountId: "acct_google_1", documentId: "doc_1" })).resolves.toEqual({ title: "Notes", plainText: "Hello" });
        await expect(driveClient.getSheetContent({ accountId: "acct_google_1", spreadsheetId: "sheet_1" })).resolves.toEqual({ title: "Sheet1", rows: [["A1"]] });
        await expect(driveClient.createDriveFile({
            accountId: "acct_google_1",
            name: "Notes",
            mimeType: "text/plain",
            content: "Hello",
        })).resolves.toMatchObject({ id: "file_2", name: "Notes" });
        await expect(driveClient.updateSheetCells({
            accountId: "acct_google_1",
            spreadsheetId: "sheet_1",
            range: "Sheet1!A1",
            values: [["A1"]],
        })).resolves.toEqual({ updatedRange: "Sheet1!A1", updatedCells: 1 });
        await expect(driveClient.appendToDoc({ accountId: "acct_google_1", documentId: "doc_1", text: "Later" })).resolves.toBeUndefined();
        await expect(meetClient.createMeeting({
            accountId: "acct_google_1",
            title: "Planning",
            accessType: "TRUSTED",
        })).resolves.toMatchObject({
        ...
        expect(fakeDrive.files.list).toHaveBeenCalledWith({
            ...
            fields: "nextPageToken,files(id,name,mimeType,createdTime,webViewLink,modifiedTime,size,parents)",
            supportsAllDrives: true,
            includeItemsFromAllDrives: true,
        });
        expect(factory.calendar).toHaveBeenNthCalledWith(1, expect.objectContaining({ accountId: "acct_google_1" }), ["calendar.read"], "calendar.listCalendars");
        expect(factory.calendar).toHaveBeenNthCalledWith(2, expect.objectContaining({ accountId: "acct_google_1" }), ["calendar.read"], "calendar.listEvents");
        expect(factory.calendar).toHaveBeenNthCalledWith(3, expect.objectContaining({ accountId: "acct_google_1" }), ["calendar.read"], "calendar.getEvent");
        expect(factory.calendar).toHaveBeenNthCalledWith(4, expect.objectContaining({ accountId: "acct_google_1" }), ["calendar.write"], "calendar.updateEvent");
        expect(factory.calendar).toHaveBeenNthCalledWith(5, expect.objectContaining({ accountId: "acct_google_1" }), ["calendar.write"], "calendar.deleteEvent");
        expect(factory.drive).toHaveBeenNthCalledWith(1, expect.objectContaining({ accountId: "acct_google_1" }), ["drive.read"], "drive.searchDriveFiles");
        expect(factory.drive).toHaveBeenNthCalledWith(2, expect.objectContaining({ accountId: "acct_google_1" }), ["drive.read"], "drive.getFile");
        expect(factory.drive).toHaveBeenNthCalledWith(3, expect.objectContaining({ accountId: "acct_google_1" }), ["drive.read"], "drive.listFiles");
        expect(factory.docs).toHaveBeenNthCalledWith(1, expect.objectContaining({ accountId: "acct_google_1" }), ["drive.read"], "drive.getDocContent");
        expect(factory.sheets).toHaveBeenNthCalledWith(1, expect.objectContaining({ accountId: "acct_google_1" }), ["drive.read"], "drive.getSheetContent");
        expect(factory.drive).toHaveBeenNthCalledWith(4, expect.objectContaining({ accountId: "acct_google_1" }), ["drive.write"], "drive.createFile");
        expect(factory.sheets).toHaveBeenNthCalledWith(2, expect.objectContaining({ accountId: "acct_google_1" }), ["drive.write"], "drive.updateSheetCells");
        expect(factory.docs).toHaveBeenNthCalledWith(2, expect.objectContaining({ accountId: "acct_google_1" }), ["drive.write"], "drive.appendToDoc");
        expect(factory.meet).toHaveBeenCalledWith(expect.objectContaining({ accountId: "acct_google_1" }), ["meet.create"], "meet.createMeeting");
    });
    it("passes RFC 5545 recurrence through create/patch and maps it on readback", async () => {
        ...
        expect(fakeCalendar.events.insert).toHaveBeenCalledWith(expect.objectContaining({
        ...
        expect(fakeCalendar.events.patch).toHaveBeenCalledWith(expect.objectContaining({
            eventId: "series_master",
...
function createOAuthCallbackManager(provider: string, durableAccountId: string, setCredentialRef: ReturnType<typeof vi.fn>) {
    ...
    return {
        ...
        upsertAccount: vi.fn(async (providerId: string, input: ConnectorAccountPatch & {
            provider?: string;
        }, accountId?: string): Promise<ConnectorAccount> => ({
