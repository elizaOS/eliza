...
export type GoogleOAuthCallbackConfigIssueCode = "missing" | "malformed" | "wrong_scheme" | "credentials" | "query" | "fragment" | "portless_loopback" | "wrong_path" | "served_origin_malformed" | "wrong_host" | "wrong_port";
...
function servedCallbackExample(options?: GoogleOAuthCallbackConfigOptions): string {
    ...
    try {
        const served = options.servedOrigin instanceof URL ? options.servedOrigin : new URL(options.servedOrigin);
        if ((served.protocol !== "http:" && served.protocol !== "https:") ||
            served.username !== "" ||
            served.password !== "") {
            return FALLBACK_CALLBACK_EXAMPLE;
        }
...
export function isPortlessLoopbackRedirectUrl(url: URL): boolean {
    if (!isLoopbackBindHost(url.hostname))
        return false;
    return url.port === "";
}
...
export function assessGoogleOAuthCallbackConfig(runtime: RuntimeWithSettings, options?: GoogleOAuthCallbackConfigOptions): GoogleOAuthCallbackConfigAssessment {
...
export function resolveGoogleConnectorOAuthCallbackUrl(runtime: RuntimeWithSettings, options?: GoogleOAuthCallbackConfigOptions): string {
