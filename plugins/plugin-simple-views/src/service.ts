 ...
 * allowed to mutate the durable workbench document; HTTP routes and view
...
import { SIMPLE_VIEWS_MAX_COLLECTION_ITEMS } from "./types.js";
import {
...
function capacityReached(kind: "notes" | "calendar events"): ElizaError {
  return new ElizaError(
    `Simple Views supports at most ${SIMPLE_VIEWS_MAX_COLLECTION_ITEMS} ${kind}.`,
    {
      code: "SIMPLE_VIEWS_CAPACITY_REACHED",
      context: { kind, limit: SIMPLE_VIEWS_MAX_COLLECTION_ITEMS },
      severity: "ephemeral",
    },
  );
}
...
export class SimpleViewsService extends Service {
  ...
  override capabilityDescription =
    "Durable developer-only Notes and Simple Calendar CRUD for view-switching QA.";
  ...
  async createNote(inputValue: unknown): Promise<StickyNote> {
    ...
    const transaction = await this.store.transact((draft) => {
      if (draft.notes.length >= SIMPLE_VIEWS_MAX_COLLECTION_ITEMS) {
        throw capacityReached("notes");
      }
  ...
  async createCalendarEvent(inputValue: unknown): Promise<SimpleCalendarEvent> {
    ...
    const transaction = await this.store.transact((draft) => {
      if (draft.events.length >= SIMPLE_VIEWS_MAX_COLLECTION_ITEMS) {
        throw capacityReached("calendar events");
      }
