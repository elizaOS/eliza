/**
 * Atomic JSON persistence for the developer view workbench. Each agent owns a
...
export class SimpleViewsStore {
  ...
  async initialize(): Promise<void> {
    ...
    const initialization = this.load().catch((error) => {
      // error-policy:J2 context-adding rethrow — every service instance that
      // shares this initialization must observe the same typed store failure.
