...
import { interact } from "./simpleViewsData.js";
...
describe("Simple Views browser interaction broker", () => {
  ...
  it("routes calendar capabilities through the Simple Calendar broker", async () => {
    transport.fetchWithCsrf.mockResolvedValueOnce(brokerResponse(5));

    await interact("select-calendar-date", { date: "2026-07-17" });
  ...
  it("rejects unknown capabilities before issuing a request", async () => {
    await expect(interact("invent-note")).rejects.toThrow(
      'Simple Views does not support capability "invent-note".',
    );
  ...
  it("rejects a malformed broker failure result", async () => {
    ...
    await expect(interact("clear-notes")).rejects.toThrow(
      "Simple Views returned an invalid broker failure result.",
    );
  });
  ...
  it("rejects a successful broker result with a malformed state", async () => {
    ...
    await expect(interact("clear-notes")).rejects.toThrow(
      "Simple Views state revision is invalid.",
    );
  });
});
