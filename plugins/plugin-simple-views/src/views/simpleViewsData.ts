...
import { isStickyColor, SIMPLE_VIEWS_MAX_COLLECTION_ITEMS } from "../types.js";
...
function parseSnapshot(value: unknown): SimpleViewsSnapshot {
  ...
  if (
    value.notes.length > SIMPLE_VIEWS_MAX_COLLECTION_ITEMS ||
    value.events.length > SIMPLE_VIEWS_MAX_COLLECTION_ITEMS
  ) {
    throw new Error("Simple Views state contains too many items.");
  }
  if (
