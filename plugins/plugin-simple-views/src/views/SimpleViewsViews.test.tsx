...
describe("Simple Views state labels", () => {
  ...
  it("disarms clear confirmation when authoritative state changes", async () => {
    const withNote = (revision: number): SimpleViewsSnapshot => ({
      ...snapshot(revision),
      notes: [
        {
          id: "note-test-1",
    ...
    stateHook.mockReturnValue(hookState({ snapshot: withNote(1) }));
    const view = render(<NotesView />);

    fireEvent.click(screen.getByRole("button", { name: "Clear" }));
    expect(screen.getByRole("button", { name: "Confirm clear" })).toBeTruthy();

    stateHook.mockReturnValue(hookState({ snapshot: withNote(2) }));
