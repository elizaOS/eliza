...
import {
  ...
  isStickyColor,
  SIMPLE_VIEWS_MAX_COLLECTION_ITEMS,
...
export function parseStickyColor(value: unknown, field = "color"): StickyColor {
  if (isStickyColor(value)) return value;
  throw validationError(
...
export function parseSimpleViewsDocument(value: unknown): SimpleViewsDocument {
  ...
  if (record.notes.length > SIMPLE_VIEWS_MAX_COLLECTION_ITEMS) {
    throw validationError(
      `notes must contain at most ${SIMPLE_VIEWS_MAX_COLLECTION_ITEMS} items.`,
      "notes",
    );
  }
  if (record.events.length > SIMPLE_VIEWS_MAX_COLLECTION_ITEMS) {
    throw validationError(
      `events must contain at most ${SIMPLE_VIEWS_MAX_COLLECTION_ITEMS} items.`,
      "events",
    );
  }
