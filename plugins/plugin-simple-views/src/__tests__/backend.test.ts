...
describe("SimpleViewsStore", () => {
  ...
  it("persists one document and restores notes, events, and selected date after restart", async () => {
    ...
    const event = await first.createCalendarEvent({
      ...
      date: "2026-07-20",
      time: "14:30",
      notes: "Open both developer views",
      color: "rose",
    });
  ...
  it("bounds both collections before accepting another durable item", async () => {
    const service = await serviceFor(await temporaryStateFile());
    const timestamp = "2026-07-16T12:00:00.000Z";
    await service.store.transact((draft) => {
      draft.notes = Array.from(
        { length: SIMPLE_VIEWS_MAX_COLLECTION_ITEMS },
        (_, index) => ({
          id: `note-capacity-${index}`,
          title: `Note ${index}`,
          body: "",
    ...
    await expect(
      service.createNote({ title: "One too many" }),
    ).rejects.toMatchObject({ code: "SIMPLE_VIEWS_CAPACITY_REACHED" });
    await expect(
      service.createCalendarEvent({ title: "One too many" }),
    ).rejects.toMatchObject({ code: "SIMPLE_VIEWS_CAPACITY_REACHED" });
    await service.stop();
  });
  ...
  it("surfaces corrupt persisted bytes as an error instead of healthy empty state", async () => {
    ...
    expect(() => second.snapshot()).toThrow("not valid JSON");
  });
});
