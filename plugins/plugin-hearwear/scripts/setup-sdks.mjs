#!/usr/bin/env node
/**
 * plugins/plugin-hearwear/scripts/setup-sdks.mjs
 *
...
if (existsSync(xrealLibs)) {
  const hasAar = existsSync(resolve(xrealLibs, "nrsdk3.aar")) ||
  ...
  if (hasAar) {
    ...
    info("Place nrsdk3.aar in plugins/plugin-hearwear/native/android/xreal/app/libs/");
  }
} else {
  warn("native/android/xreal/app/libs/ not found");
  info("Run the plugin-hearwear build first to create native app structure");
}

// ── Even Realities (BLE — no external SDK needed) ─────────────────────────────
header("Even Realities G1/G2 (BLE — built-in protocol)");
ok("Even Realities G1/G2 protocol is built into plugin-hearwear");
