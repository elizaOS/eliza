...
export const simulatorRoute: Route = {
  ...
  description:
    "Serves the XR device emulator bundle (IWER + camera injection) for Playwright testing. Build first: cd plugins/plugin-hearwear/emulator && bun run build",
    ...
    if (!existsSync(EMULATOR_BUNDLE)) {
      return {
        ...
        body: {
          ...
          hint: "Run: cd eliza/plugins/plugin-hearwear/emulator && bun run build",
        },
      };
    }
