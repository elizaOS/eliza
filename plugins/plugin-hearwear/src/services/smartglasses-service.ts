...
export class SmartglassesService extends Service {
  ...
  static async start(runtime: IAgentRuntime): Promise<SmartglassesService> {
    ...
    if (!service.transport) {
      logger.info(
        "[plugin-hearwear/smartglasses] no transport available; service loaded in offline/mockable mode",
      );
      return service;
    }
  ...
  startHeartbeatLoop(
    ...
    this.heartbeatTimer = setInterval(() => {
      void this.sendHeartbeat().catch((error) => {
        logger.warn(
          { error },
          "[plugin-hearwear/smartglasses] heartbeat failed",
        );
      });
    }, intervalMs);
  ...
  private async decodeAudioChunk(
    ...
    try {
      ...
      logger.warn(
        { error },
        "[plugin-hearwear/smartglasses] audio decoder failed; raw audio event preserved",
      );
      return null;
    }
  }
  ...
  private async emitPluginEvent(
    ...
    await (
      ...
      source: "@elizaos/plugin-hearwear",
      ...payload,
    });
  }
}
