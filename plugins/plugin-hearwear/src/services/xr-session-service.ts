...
export class XRSessionService extends Service {
  ...
  private async initialize(runtime: IAgentRuntime): Promise<void> {
    ...
    this.wss.on("error", (err) =>
      console.error("[plugin-hearwear/xr] WebSocket server error:", err),
    );

    console.info(
      `[plugin-hearwear/xr] WebSocket server listening on ws://localhost:${port}`,
    );
  }
  ...
  private onConnect(runtime: IAgentRuntime, ws: WebSocket): void {
    ...
    ws.on("message", (data, isBinary) => {
      try {
        ...
        console.error("[plugin-hearwear/xr] message handler error:", err);
      }
    });
    ...
    ws.on("close", () => {
      ...
      console.info(`[plugin-hearwear/xr] device disconnected: ${connId}`);
    });

    ws.on("error", (err) =>
      console.error(`[plugin-hearwear/xr] ws error on ${connId}:`, err),
    );
  }
  ...
  private handleTextMessage(
    ...
    if (msg.type === "hello") {
      ...
      console.info(
        `[plugin-hearwear/xr] ${msg.deviceType} connected: ${connId}`,
      );
      return;
    }
    ...
    if (msg.type === "view_ready") {
      console.info(
        `[plugin-hearwear/xr] view ready on ${connId}: ${msg.viewId}`,
      );
      return;
    }
    ...
    if (msg.type === "view_closed") {
      console.info(
        `[plugin-hearwear/xr] view closed on ${connId}: ${msg.viewId}`,
      );
      return;
    }
    ...
    if (msg.type === "view_event") {
      console.info(
        `[plugin-hearwear/xr] view event on ${connId}:`,
        msg.viewId,
        msg.event,
      );
      return;
  ...
  private async handleTranscript(
    ...
    const callback: HandlerCallback = async (
      ...
      try {
        ...
        console.error("[plugin-hearwear/xr] TTS error:", err);
      }
  ...
  private async ensureEntities(
    ...
    try {
      ...
      console.error("[plugin-hearwear/xr] entity setup error:", err);
    }
  }
}
