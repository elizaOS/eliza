/**
 * Playwright fixture and page wrapper for facewear emulator testing.
 *
 * Usage:
...
// Navigate up from emulator/src/ to emulator/dist/emulator.js
...
export class XREmulatorPage {
  ...
  async inject(): Promise<void> {
    if (!existsSync(EMULATOR_DIST)) {
      throw new Error(
        `Emulator bundle not found at ${EMULATOR_DIST}. Run: cd plugins/plugin-hearwear/emulator && bun run build`,
      );
    }
