...
describe("LifeOps package boundaries", () => {
  ...
  it("keeps health and screen-time actions as plugin-health wrappers", () => {
    ...
    expect(
      existsSync(resolve(packageRoot, "src/components/MobileSignalsSetupCard.tsx")),
    ).toBe(false);
    expect(
      existsSync(resolve(packageRoot, "src/components/LifeOpsAssistantSection.tsx")),
    ).toBe(false);
    expect(
      existsSync(resolve(packageRoot, "src/components/LifeOpsSettingsSection.tsx")),
    ).toBe(false);
    expect(uiEntrypoint).not.toContain("@elizaos/plugin-health/ui");
    expect(uiEntrypoint).not.toContain("HEALTH_ASSISTANT_COMMANDS");
    expect(uiEntrypoint).not.toContain("HEALTH_ASSISTANT_INTENTS");
    expect(uiEntrypoint).not.toContain(
    ...
    expect(uiEntrypoint).not.toContain("HealthProviderActionButton");
    expect(uiEntrypoint).not.toContain("HealthPendingAuthActions");
    expect(uiEntrypoint).not.toContain("HealthConnectorRedirectCard");
    expect(uiEntrypoint).not.toContain("health.connect(");
    expect(uiEntrypoint).not.toContain("health.disconnect(");
    expect(uiEntrypoint).not.toContain("health.sync(");
    ...
    expect(uiEntrypoint).not.toContain("GoogleManagementButton");
    expect(uiEntrypoint).not.toContain('dispatchFocusConnector("google")');
    expect(uiEntrypoint).not.toContain("GoogleConnectButton");
    expect(uiEntrypoint).not.toContain("GoogleAddAccountButton");
    expect(uiEntrypoint).not.toContain("GoogleDisconnectButton");
    expect(uiEntrypoint).not.toContain("GoogleAccountDisconnectButton");
    expect(uiEntrypoint).not.toContain("PendingAuthBanner");
    expect(uiEntrypoint).not.toContain("settings-google-${side}-connect");
    expect(uiEntrypoint).not.toContain("settings-google-${side}-disconnect");
    expect(uiEntrypoint).not.toContain("settings-google-${side}-add");
    expect(uiEntrypoint).not.toContain("settings-google-${side}-auth");
  ...
  it("imports browser bridge readiness policy from plugin-browser", () => {
    ...
    expect(
      existsSync(resolve(packageRoot, "src/components/BrowserBridgeStatusChip.tsx")),
    ).toBe(false);
    expect(uiEntrypoint).not.toContain("BrowserBridgeRedirectCard");
  ...
  it("keeps messaging account setup in connector plugins instead of LifeOps views", () => {
    ...
    expect(
      existsSync(resolve(packageRoot, "src/components/MessagingConnectorCards.tsx")),
    ).toBe(false);
