...
import { emitVoiceTurnObserved } from "../../plugin-local-inference/src/runtime/voice-entity-binding.js";
import { expect } from "vitest";
import { handleVoiceEntityBound } from "../../plugin-local-inference/src/runtime/voice-entity-binding.js";
import { handleVoiceProfilesManagementRoutes } from "../../plugin-local-inference/src/routes/voice-profiles-management-routes.js";
import { handleVoiceSpeakerProfileRoutes } from "../../plugin-local-inference/src/routes/voice-speaker-profile-routes.js";
import { handleVoiceTurnObserved } from "../src/lifeops/entities/voice-observer-bridge.js";
import { identifySpeakerAction } from "@elizaos/plugin-local-inference";
import { it } from "vitest";
import { localInferencePlugin } from "@elizaos/plugin-local-inference";
import { mkdtempSync } from "node:fs";
import { rmSync } from "node:fs";
import { setVoiceEntityBindingStore } from "../../plugin-local-inference/src/runtime/voice-entity-binding.js";
import { setVoiceProfilesManagementStore } from "../../plugin-local-inference/src/routes/voice-profiles-management-routes.js";
import { setVoiceSpeakerProfileStore } from "../../plugin-local-inference/src/routes/voice-speaker-profile-routes.js";
...
function buildSeamPlugin(): Plugin {
    const boundHandlers = localInferencePlugin.events?.[EventType.VOICE_ENTITY_BOUND];
    if (!boundHandlers?.length) {
        throw new Error("VOICE_ENTITY_BOUND handler is not registered on localInferencePlugin");
    }
    return {
        name: "voice-binding-seam-e2e",
        description: "Registers the real voice-binding event handlers from both plugins",
        events: {
...
describe("voice → entity binding round-trip (issue #8234)", () => {
    ...
    it("registers the binding seam on the local-inference plugin object", () => {
        ...
        expect(localInferencePlugin.events?.[EventType.VOICE_ENTITY_BOUND]).toContain(handleVoiceEntityBound);
        expect(localInferencePlugin.actions?.some((a) => a.name === "IDENTIFY_SPEAKER")).toBe(true);
        ...
        const routes = (localInferencePlugin.routes ?? []).map((r) => `${r.type} ${r.path}`);
        ...
        expect((localInferencePlugin.routes ?? []).every((r) => r.rawPath === true)).toBe(true);
    });
    it("binds a self-claimed speaker through the live event round-trip", async () => {
        ...
        expect(entity?.identities.some((i) => i.platform === "voice" && i.handle === "cluster_jill")).toBe(true);
        // Disk persistence: a fresh store instance sees the binding.
        const reloaded = new VoiceProfileStore({ rootDir: tmpRoot });
        await reloaded.init();
        expect((await reloaded.get(profile.profileId))?.entityId).toBe(bound?.entityId);
    });
    it("re-observing the same cluster resolves to the same entity (cross-session memory)", async () => {
        const before = await store.get((await store.list()).find((r) => r.imprintClusterId === "cluster_jill")
            ?.profileId ?? "");
        ...
        expect(entities.filter((e) => e.identities.some((i) => i.platform === "voice" && i.handle === "cluster_jill"))).toHaveLength(1);
    });
    it("IDENTIFY_SPEAKER binds the most recent unidentified voice by name", async () => {
        ...
        const result = await identifySpeakerAction.handler(runtime, message, undefined, undefined, async (content) => {
    ...
    it("applies correction provenance and merges duplicate named entities", async () => {
        ...
        expect(result?.success).toBe(true);
        const sarahs = (await entityStore.list()).filter((entity) => entity.preferredName === "Sarah");
        ...
        expect(recurringMatch?.profile.metadata?.speakerNameInference).toMatchObject({
            resolution: "confirmed",
            confidence: 1,
            provenance: [{ source: "user_correction", confidence: 1 }],
        });
        expect(recurringMatch
            ? voiceSpeakerFromImprintMatch({ match: recurringMatch }).displayName
            : undefined).toBe("Sarah");
    });
    it("serves HTTP bind/unbind from the plugin route handlers", async () => {
        ...
        const server = http.createServer((req, res) => {
            void (async () => {
                if (await handleVoiceSpeakerProfileRoutes(req, res))
                    return;
                if (await handleVoiceProfilesManagementRoutes(req, res))
                    return;
                res.statusCode = 404;
                res.end();
            })();
        ...
        try {
            ...
            const bindRes = await fetch(`${base}/v1/voice/speaker-profiles/${profile.profileId}/bind`, {
                method: "POST",
            ...
            expect((await store.get(profile.profileId))?.entityId).toBe(contact.entityId);
            ...
            expect(listDto.profiles.find((p) => p.profileId === profile.profileId)
                ?.entityId).toBe(contact.entityId);
            // Unbind via the management namespace (the VoiceProfileSection UI path).
            const unbindRes = await fetch(`${base}/api/voice/profiles/${profile.profileId}/unbind`, { method: "POST" });
