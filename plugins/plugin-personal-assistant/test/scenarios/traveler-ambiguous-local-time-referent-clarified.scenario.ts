  ...
  turns: [
    {
      ...
      body: {
        kind: "task",
        title: "Elena partner call before the outbound leg",
        timezone: "UTC",
        priority: 1,
        cadence: {
          kind: "once",
    ...
    },
    {
      ...
      room: "main",
      text: "gate's changed and i've lost track of myself. can you move the partner call to nine?",
      plannerExcludes: ["MESSAGE_SEND_CONFIRMED"],
      responseJudge: {
        minimumScore: 0.75,
        rubric:
          "The owner is mid-itinerary with a partner call on each side of the hop, and 'nine' carries no timezone and no side. The request is doubly ambiguous: which of the two calls, and nine o'clock where. Grade PASS only if the assistant asks about the underdetermined parts before moving anything — at minimum which zone 'nine' is in, and ideally which of the two calls she means. It fails if it assumes a timezone, silently picks one call, moves anything without asking, or claims the change is already made.",
      },
    },
    {
      kind: "message",
      ...
      responseJudge: {
        minimumScore: 0.75,
        rubric:
          "The owner has now pinned both variables: the post-landing call, at 09:00 in the destination zone rather than her current one. Grade PASS only if the assistant moves that call to 09:00 destination-local, leaves the pre-departure call untouched, and states the resulting time in a way that makes the zone unambiguous so a mistake would be visible. It fails if it moves the pre-departure call, applies her departure zone, or reports a bare time with no zone attached.",
      },
    },
  ],
  finalChecks: [
