...
import { registerAutomationNodeContributor } from "@elizaos/core/automation-node-contributors";
import { type AutomationNodeContributorContext } from "@elizaos/core/automation-node-contributors";
...
import { type LifeOpsDiscordConnectorStatus } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsGoogleConnectorStatus } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsTelegramConnectorStatus } from "@elizaos/core/contracts/personal-assistant";
...
async function resolveGoogleStatus(lifeOps: LifeOpsService): Promise<LifeOpsGoogleConnectorStatus | null> {
    try {
        return await lifeOps.getGoogleConnectorStatus(new URL("http://127.0.0.1/api/connectors/google/accounts"), undefined, "owner");
    }
    catch (error) {
...
async function resolveTelegramStatus(lifeOps: LifeOpsService): Promise<LifeOpsTelegramConnectorStatus | null> {
    try {
...
async function resolveDiscordStatus(lifeOps: LifeOpsService): Promise<LifeOpsDiscordConnectorStatus | null> {
    try {
...
export async function resolveNativeCalendarPermission(runtime: AutomationNodeContributorContext["runtime"]): Promise<PermissionState | null> {
    ...
    try {
        ...
        logger.warn(`[lifeops] Live native Calendar permission check failed; falling back to cached value: ${error instanceof Error ? error.message : String(error)}`);
        try {
...
function buildLifeOpsNode(id: string, label: string, description: string, enabled: boolean, disabledReason: string): AutomationNodeDescriptor {
    return {
        id,
        label,
        description,
...
function buildLifeOpsEventNode(eventKind: string, label: string, description: string, enabled: boolean, disabledReason: string): AutomationNodeDescriptor {
    return {
        id: `event:${eventKind}`,
        label,
        description,
...
async function buildLifeOpsAutomationNodes({ runtime, adminEntityId, }: AutomationNodeContributorContext): Promise<AutomationNodeDescriptor[]> {
    const lifeOps = new LifeOpsService(runtime, { ownerEntityId: adminEntityId });
    const [googleStatus, telegramStatus, discordStatus, calendarPermission] = await Promise.all([
    ...
    const hasGoogleCapability = (needle: string) => [...googleCapabilities].some((capability) => capability.includes(needle));
    ...
    return [
        ...
        buildLifeOpsNode("lifeops:calendar", "Calendar", "Owner-scoped calendar reading and event creation.", calendarConnected, calendarDisabledReason),
        ...
        buildLifeOpsEventNode("calendar.event.ended", "Calendar event ended", "Fires a workflow after a synced calendar event's end time has passed.", calendarConnected, calendarDisabledReason),
    ];
}
