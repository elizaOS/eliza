...
import { RESOURCE_CAPACITY_POLICY_VERSION } from "./types.js";
import { ResourceCapacityRepository } from "./repository.js";
import { ResourceCapacityService } from "./service.js";
import { SELF_ENTITY_ID } from "@elizaos/core/knowledge-graph/entity-types";
import { afterAll } from "vitest";
import { authenticatedHouseholdInboundIdentity } from "../household/inbound-approval.js";
...
import { getHouseholdCoordinationService } from "../household/service.js";
import { getScheduledTaskRunner } from "../scheduled-task/service.js";
import { it } from "vitest";
import { parseHouseholdInboundApprovalCommand } from "../household/inbound-approval.js";
import { processHouseholdInboundApproval } from "../household/inbound-approval.js";
import { randomUUID } from "node:crypto";
import { resolveKnowledgeGraphService } from "@elizaos/plugin-relationships";
import { type AgentRuntime } from "@elizaos/core";
import { type ApprovalQueue } from "../approval-queue.types.js";
import { type CapacityNeed } from "./types.js";
import { type CarSeatResourceDefinition } from "./types.js";
import { type CaregiverResourceDefinition } from "./types.js";
...
import { type HouseholdResourceDefinition } from "./types.js";
import { type Memory } from "@elizaos/core";
import { type RealTestRuntimeResult } from "../../../test/helpers/runtime.js";
import { type ResourceCapacityAssignment } from "./types.js";
import { type ResourceCapacityEvaluationInput } from "./types.js";
import { type ResourceCapacityPlan } from "./types.js";
import { type ResourceTransitionEvidence } from "./types.js";
import { type VehicleResourceDefinition } from "./types.js";
describe("household resource capacity — real PGlite", () => {
    ...
    async function upsertPerson(entityId: string, preferredName: string, verifiedTelegramHandle?: string): Promise<void> {
    ...
    function authorization(overrides?: Partial<HouseholdResourceDefinition["authorization"]>): HouseholdResourceDefinition["authorization"] {
        return {
            state: "authorized",
    ...
    function availability(resourceId: string, overrides?: Partial<HouseholdResourceDefinition["availability"][number]>): HouseholdResourceDefinition["availability"] {
        return [
            {
    ...
    function caregiverResource(householdId: string, resourceId: string, overrides?: Partial<CaregiverResourceDefinition>): CaregiverResourceDefinition {
        return {
    ...
    function vehicleResource(householdId: string, resourceId: string, carSeatResourceId: string, overrides?: Partial<VehicleResourceDefinition>): VehicleResourceDefinition {
        return {
            schemaVersion: RESOURCE_CAPACITY_POLICY_VERSION,
            resourceId,
            householdId,
    ...
    function carSeatResource(householdId: string, resourceId: string, vehicleResourceId: string, overrides?: Partial<CarSeatResourceDefinition>): CarSeatResourceDefinition {
        return {
            schemaVersion: RESOURCE_CAPACITY_POLICY_VERSION,
            resourceId,
            householdId,
    ...
    async function seedResourceSet(householdId: string, suffix: string): Promise<{
        ...
        await service.putResource({
            principalEntityId: SELF_ENTITY_ID,
            definition: vehicleResource(householdId, vehicleResourceId, carSeatResourceId),
            expectedRevision: 0,
        });
        await service.putResource({
            principalEntityId: SELF_ENTITY_ID,
            definition: carSeatResource(householdId, carSeatResourceId, vehicleResourceId),
            expectedRevision: 0,
        });
        return {
            caregiverResourceId,
            ...
            handoffs: [
                {
                    ...
                    kind: "pickup",
                    startsAt: new Date(Date.parse(input.startsAt) - 5 * 60 * 1000).toISOString(),
                    endsAt: new Date(Date.parse(input.startsAt) + 5 * 60 * 1000).toISOString(),
                ...
                },
                {
                    ...
                    kind: "dropoff",
                    startsAt: new Date(Date.parse(input.endsAt) - 5 * 60 * 1000).toISOString(),
                    endsAt: new Date(Date.parse(input.endsAt) + 5 * 60 * 1000).toISOString(),
                    locationRef: input.location,
                    requiredPrincipalEntityIds: [input.caregiverPrincipal ?? caregiverId],
                },
            ],
    ...
    function assignments(needs: readonly CapacityNeed[], resources: {
        caregiverResourceId: string;
        vehicleResourceId: string;
        carSeatResourceId: string;
    ...
    function plan(householdId: string, needs: readonly CapacityNeed[]): ResourceCapacityPlan {
        return {
            householdId,
    ...
    function transitions(resources: {
        caregiverResourceId: string;
        vehicleResourceId: string;
        carSeatResourceId: string;
    }, fromNeedId: string, toNeedId: string, minimumMinutes: number): ResourceTransitionEvidence[] {
    ...
    function evaluationInput(householdId: string, needs: readonly CapacityNeed[], resources: {
        caregiverResourceId: string;
        vehicleResourceId: string;
        carSeatResourceId: string;
    }, transitionEvidence: readonly ResourceTransitionEvidence[] = []): ResourceCapacityEvaluationInput {
        return {
    ...
    it("persists append-only resource revisions and rejects concurrent stale writers", async () => {
        ...
        const revisionRows = await executeRawSql(runtime, `SELECT revision
    ...
    it("rejects floating local timestamps before resource evidence is persisted", async () => {
        ...
        await expect(service.putResource({
            principalEntityId: SELF_ENTITY_ID,
            definition: caregiverResource(householdId, resourceId, {
                availability: [
                    {
    ...
    it("detects one caregiver, vehicle, and car-seat conflict across non-overlapping adult events", async () => {
        ...
        const result = await service.evaluate({
            principalEntityId: SELF_ENTITY_ID,
            evaluation: evaluationInput(householdId, [first, second], resources, transitions(resources, first.needId, second.needId, 25)),
        });
        expect(result.feasible).toBe(false);
        expect(result.conflicts.filter((conflict) => conflict.kind === "transition_time_insufficient")).toHaveLength(3);
        expect(result.conflicts.some((conflict) => conflict.kind === "direct_overlap")).toBe(false);
        expect(new Set(result.conflicts.flatMap((conflict) => conflict.resourceIds))).toEqual(new Set([
    ...
    it("finds a feasible distinct-restraint matching instead of depending on child order", async () => {
        ...
        await service.putResource({
            principalEntityId: SELF_ENTITY_ID,
            definition: vehicleResource(householdId, vehicleResourceId, flexibleSeatId, {
        ...
        await service.putResource({
            principalEntityId: SELF_ENTITY_ID,
            definition: carSeatResource(householdId, flexibleSeatId, vehicleResourceId, {
        ...
        await service.putResource({
            principalEntityId: SELF_ENTITY_ID,
            definition: carSeatResource(householdId, childSpecificSeatId, vehicleResourceId, {
                compatibleChildEntityIds: [childOneId],
            }),
            expectedRevision: 0,
        });
    ...
    it("drives the production owner action service through the real database without an external effect", async () => {
        ...
        const result = await action.handler(runtime, createMessageMemory({
            entityId: runtime.agentId,
            agentId: runtime.agentId,
            roomId: runtime.agentId,
            content: {
    ...
    it("fails closed for outsider access, self-granting caregivers, stale evidence, and incompatible constraints", async () => {
        ...
        await expect(service.putResource({
            ...
            definition: caregiverResource(householdId, "caregiver-self-grant-attempt"),
            expectedRevision: 0,
        })).rejects.toMatchObject({
        ...
        expect(result.conflicts.map((conflict) => conflict.kind)).toEqual(expect.arrayContaining([
            "source_stale",
    ...
    it("does not reuse one driver or restraint and rejects contradictory transition evidence", async () => {
        ...
        await service.putResource({
            principalEntityId: SELF_ENTITY_ID,
            definition: vehicleResource(householdId, secondVehicleResourceId, resources.carSeatResourceId),
            expectedRevision: 0,
        });
        await service.putResource({
            principalEntityId: SELF_ENTITY_ID,
            definition: carSeatResource(householdId, resources.carSeatResourceId, resources.vehicleResourceId, {
                compatibleChildEntityIds: [childOneId, childTwoId],
        ...
        expect(distinctResult.conflicts.map((conflict) => conflict.kind)).toEqual(expect.arrayContaining([
            "duplicate_assignment",
        ...
        const contradictory = await service.evaluate({
            ...
            evaluation: evaluationInput(householdId, [first, second], resources, [
                ...
                ...transitions(resources, first.needId, second.needId, 35).map((transition) => ({
                    ...transition,
                    sourceRef: `${transition.sourceRef}:conflicting`,
                })),
            ]),
        });
        expect(contradictory.conflicts.filter((conflict) => conflict.kind === "contradictory_transition_evidence")).toHaveLength(3);
    });
    ...
    it("creates one immutable review proposal, shared approvals, and one shared ScheduledTask under concurrent retry", async () => {
        ...
        const proposalRows = await executeRawSql(runtime, `SELECT proposal_id, status
       ...
       WHERE agent_id = '${runtime.agentId}'
        ...
        const approvalRows = await executeRawSql(runtime, `SELECT party_entity_id, approval_request_id
       ...
       ORDER BY party_entity_id`);
        ...
        const taskRows = await executeRawSql(runtime, `SELECT scheduled_task_id
       ...
       WHERE agent_id = '${runtime.agentId}'
        ...
        const task = tasks.find((candidate) => candidate.taskId === first.reviewTaskId);
        ...
        for (const approval of first.approvals) {
            const request = await approvals.byId(approval.approvalRequestId, approval.partyEntityId);
        ...
        expect(blocked.proposal.evaluation.conflicts.some((conflict) => conflict.kind === "pending_proposal_reservation")).toBe(true);
        expect(blocked.approvals).toEqual([]);
        expect(blocked.reviewTaskId).toBeNull();
        await expect(service.createProposal({
            ...input,
            evaluation: evaluationInput(householdId, [
                {
                    ...capacityNeed,
                    endsAt: "2027-03-12T16:30:00.000Z",
                },
    ...
    it("terminalizes rejected and cancelled reviews, releases overlapping resources, and dismisses shared watchers across restart", async () => {
        ...
        const rejectedResources = await seedResourceSet(rejectedHouseholdId, "rejected-terminal");
        ...
        const rejectedProposal = await service.createProposal({
            principalEntityId: SELF_ENTITY_ID,
            evaluation: evaluationInput(rejectedHouseholdId, [rejectedNeed], rejectedResources),
            requiredApproverEntityIds: [SELF_ENTITY_ID, partnerId],
            idempotencyKey: "capacity-rejected-terminal-v1",
            expiresAt: "2027-03-17T12:00:00.000Z",
        });
        const ownerApproval = rejectedProposal.approvals.find((approval) => approval.partyEntityId === SELF_ENTITY_ID);
        if (!ownerApproval)
            throw new Error("owner review approval missing");
        ...
        expect(concurrentEvaluation.conflicts.every((conflict) => conflict.kind !== "duplicate_assignment")).toBe(true);
        const releasedAfterRejection = await service.evaluate({
            principalEntityId: SELF_ENTITY_ID,
            evaluation: evaluationInput(rejectedHouseholdId, [rejectedNeed], rejectedResources),
        });
        expect(releasedAfterRejection.feasible).toBe(true);
        expect(releasedAfterRejection.conflicts.some((conflict) => conflict.kind === "pending_proposal_reservation")).toBe(false);
        const declined = await service.readProposal({
            principalEntityId: SELF_ENTITY_ID,
            proposalId: rejectedProposal.proposal.proposalId,
        });
        expect(declined.effectiveState).toBe("declined");
        expect(declined.approvals.map((approval) => approval.state).sort()).toEqual(["expired", "rejected"]);
        const cancelledHouseholdId = "capacity-household-cancelled-terminal";
        const cancelledResources = await seedResourceSet(cancelledHouseholdId, "cancelled-terminal");
        ...
        const cancellable = await service.createProposal({
            principalEntityId: SELF_ENTITY_ID,
            evaluation: evaluationInput(cancelledHouseholdId, [cancelledNeed], cancelledResources),
            requiredApproverEntityIds: [SELF_ENTITY_ID],
        ...
        await expect(cancelAction.handler(runtime, createMessageMemory({
            entityId: runtime.agentId,
            agentId: runtime.agentId,
            roomId: runtime.agentId,
            content: {
        ...
        expect(releasedAfterCancellation.conflicts.some((conflict) => conflict.kind === "pending_proposal_reservation")).toBe(false);
        const terminalRows = await executeRawSql(runtime, `SELECT proposal_id, terminal_state
       ...
       ORDER BY terminal_state`);
        ...
        for (const taskId of [
            ...
            expect(tasks.find((task) => task.taskId === taskId)?.state.status).toBe("dismissed");
        }
        const recoveryHouseholdId = "capacity-household-rejection-recovery";
        const recoveryResources = await seedResourceSet(recoveryHouseholdId, "rejection-recovery");
        ...
        const recoveryProposal = await service.createProposal({
            principalEntityId: SELF_ENTITY_ID,
            evaluation: evaluationInput(recoveryHouseholdId, [recoveryNeed], recoveryResources),
            requiredApproverEntityIds: [SELF_ENTITY_ID],
        ...
        if (!recoveryApproval)
            throw new Error("recovery approval missing");
        await approvals.reject(recoveryApproval.approvalRequestId, recoveryApproval.partyEntityId, {
            resolvedBy: SELF_ENTITY_ID,
            resolutionReason: "Simulated process death after the approval rejection commit.",
        });
        expect(await repository.getProposalTerminal(recoveryProposal.proposal.proposalId)).toBeNull();
        ...
        await expect(restarted.readProposal({
            principalEntityId: SELF_ENTITY_ID,
            proposalId: rejectedProposal.proposal.proposalId,
        ...
        expect((await getScheduledTaskRunner(runtime, {
            agentId: runtime.agentId,
        }).list()).find((task) => task.taskId === recoveryProposal.reviewTaskId)?.state
            .status).toBe("dismissed");
        await expect(restarted.evaluate({
            principalEntityId: SELF_ENTITY_ID,
            evaluation: evaluationInput(recoveryHouseholdId, [recoveryNeed], recoveryResources),
    ...
    it("invalidates review after resource drift and stops treating it as a pending reservation", async () => {
        ...
        await service.putResource({
            principalEntityId: SELF_ENTITY_ID,
            definition: caregiverResource(householdId, resources.caregiverResourceId, { label: "Caregiver with revised capacity evidence" }),
            expectedRevision: 1,
        });
        ...
        expect(replacement.conflicts.some((conflict) => conflict.kind === "pending_proposal_reservation")).toBe(false);
    });
    it("invalidates review when source evidence ages out and releases the proposal-only conflict", async () => {
        ...
        try {
            ...
            expect(invalidated.invalidationConflicts.some((conflict) => conflict.kind === "source_stale")).toBe(true);
            await expect(service.respondToProposal({
                principalEntityId: SELF_ENTITY_ID,
                proposalId: proposal.proposal.proposalId,
                proposalVersion: 1,
    ...
    it("accepts an exact verified co-parent review through the shared replay-receipted inbound path", async () => {
        ...
        const partnerApproval = proposal.approvals.find((approval) => approval.partyEntityId === partnerId);
        if (!partnerApproval)
            throw new Error("partner review approval missing");
        ...
        expect(await approvals.byId(partnerApproval.approvalRequestId, partnerApproval.partyEntityId)).toMatchObject({
            state: "approved",
    ...
    it("projects exact human review states without turning approval into execution", async () => {
        ...
        const ownerApproval = proposal.approvals.find((approval) => approval.partyEntityId === SELF_ENTITY_ID);
        if (!ownerApproval)
            throw new Error("owner review approval missing");
        await expect(service.respondToProposal({
            principalEntityId: SELF_ENTITY_ID,
        ...
        expect(reviewed.approvals.every((approval) => approval.state === "approved")).toBe(true);
    ...
    it("rejects unrelated or child reviewers before any approval artifact exists", async () => {
        ...
        const rows = await executeRawSql(runtime, `SELECT proposal_id
       FROM app_lifeops.life_resource_capacity_proposals
       WHERE agent_id = '${runtime.agentId}'
