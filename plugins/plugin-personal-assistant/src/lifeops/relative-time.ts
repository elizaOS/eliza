...
import { type LifeOpsDayBoundary } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsPersonalBaseline } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsRelativeTime } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsRelativeTimeAnchorSource } from "@elizaos/core/contracts/personal-assistant";
...
type RelativeTimeScheduleFields = Pick<LifeOpsScheduleInsight, "circadianState" | "stateConfidence" | "uncertaintyReason" | "awakeProbability" | "regularity" | "baseline" | "sleepConfidence" | "currentSleepStartedAt" | "lastSleepStartedAt" | "lastSleepEndedAt" | "wakeAt" | "firstActiveAt">;
...
function allowsProjectedBedtime(regularity: LifeOpsScheduleRegularity | null | undefined): boolean {
    return (regularity?.regularityClass === "regular" ||
        regularity?.regularityClass === "very_regular");
}
function allowsFallbackBedtimeFromLastSleep(regularity: LifeOpsScheduleRegularity | null | undefined): boolean {
...
function localHourInstantMs(args: {
    ...
    const anchorParts = getZonedDateParts(new Date(args.anchorMs ?? args.nowMs), args.timezone);
    ...
    for (let step = 0; step < 14; step += 1) {
        if (candidate >= args.nowMs - BEDTIME_TARGET_MAX_PAST_MS)
            break;
        candidate += DAY_MS;
    }
    for (let step = 0; step < 14; step += 1) {
        if (candidate <= args.nowMs + BEDTIME_TARGET_MAX_FUTURE_MS)
            break;
        candidate -= DAY_MS;
    }
    return candidate;
...
function baselineBedtimeHour(baseline: LifeOpsPersonalBaseline | null | undefined): number | null {
    return baseline?.medianBedtimeLocalHour ?? null;
}
function sourceConfidence(source: LifeOpsRelativeTimeAnchorSource | null): number {
    switch (source) {
        case "sleep_cycle":
            return 0.72;
        case "activity":
...
export function resolveLifeOpsRelativeTime(args: {
    ...
    const dayBoundary = args.dayBoundary ??
    ...
    const typicalBedtimeMs = !isUnclear &&
        ...
        : null;
    const fallbackBedtimeMs = !isUnclear &&
        ...
        ? localHourInstantMs({
            ...
            nowMs: args.nowMs,
            normalizedHour: (() => {
                const parts = getZonedDateParts(new Date(lastSleepStartedMs), args.timezone);
        ...
        : null;
    const bedtimeTargetMs = isAsleepState(args.schedule.circadianState) &&
        currentSleepStartedMs !== null
        ? currentSleepStartedMs
        : (typicalBedtimeMs ?? fallbackBedtimeMs);
    const bedtimeTargetSource: LifeOpsRelativeTimeAnchorSource | null = isAsleepState(args.schedule.circadianState) &&
        currentSleepStartedMs !== null
        ? "sleep_cycle"
        : typicalBedtimeMs !== null
            ? "typical_sleep"
    ...
    const minutesSinceWake = wakeAnchorMs !== null && wakeAnchorMs <= args.nowMs
        ? minutesBetween(wakeAnchorMs, args.nowMs)
        : null;
    const minutesAwake = isAwakeState(args.schedule.circadianState)
        ? minutesSinceWake
        : null;
    const minutesUntilBedtimeTarget = bedtimeTargetMs === null || bedtimeTargetMs < args.nowMs
        ? null
        : Math.round((bedtimeTargetMs - args.nowMs) / 60000);
    const minutesSinceBedtimeTarget = bedtimeTargetMs === null || bedtimeTargetMs > args.nowMs
        ? null
        : minutesBetween(bedtimeTargetMs, args.nowMs);
    return {
        computedAt: new Date(args.nowMs).toISOString(),
        localNowAt: formatInstantAsRfc3339InTimeZone(new Date(args.nowMs), args.timezone),
        ...
        minutesSinceWake,
        minutesAwake,
        bedtimeTargetAt: bedtimeTargetMs === null ? null : new Date(bedtimeTargetMs).toISOString(),
        bedtimeTargetSource,
        ...
        minutesUntilDayBoundaryEnd: Number.isFinite(endOfDayMs)
            ...
            : 0,
        confidence: roundConfidence(Math.max(awakeProbability.pAwake, awakeProbability.pAsleep, args.schedule.sleepConfidence, sourceConfidence(wakeAnchorSource), sourceConfidence(bedtimeTargetSource))),
    };
}
export function refreshLifeOpsRelativeTime<T extends RelativeTimeScheduleFields & {
    timezone: string;
}>(state: T, now: Date): T & {
