...
import { HOUSEHOLD_COORDINATION_SERVICE } from "./service.js";
import { HouseholdCoordinationError } from "./types.js";
...
import { assertFamilyWorkspaceReadable } from "../family-workflows/workspace-operation-store.js";
import { beginFamilyWorkspaceOperation } from "../family-workflows/workspace-operation-store.js";
...
import { generateAgreementReview } from "./agreement-review.js";
import { getHouseholdCoordinationService } from "./service.js";
import { normalizeHouseholdIdentifier } from "./types.js";
import { resolveKnowledgeGraphService } from "@elizaos/plugin-relationships";
import { settleFamilyWorkspaceOperation } from "../family-workflows/workspace-operation-store.js";
import { sqlInteger } from "../sql.js";
import { sqlQuote } from "../sql.js";
import { sqlText } from "../sql.js";
import { toNumber } from "../sql.js";
import { toText } from "../sql.js";
import { type AgreementReviewProposal } from "./agreement-review.js";
import { type AgreementReviewSource } from "./agreement-review.js";
import { type EntityStore } from "@elizaos/plugin-relationships";
import { type GeneratedAgreementReview } from "./agreement-review.js";
import { type HouseholdCoordinationService } from "./service.js";
...
import { validateAgreementReviewProposal } from "./agreement-review.js";
...
const agreementExtractionSchema = z.strictObject({
    ...
    pages: z.array(z.strictObject({
        ...
        nativeText: z.string(),
        nativePositionedText: z.array(z.strictObject({
...
export const HOUSEHOLD_AGREEMENT_KNOWLEDGE_SERVICE = "lifeops_household_agreement_knowledge";
...
function preparedReviewFromJson(value: unknown): PreparedReviewRecord {
    try {
        return preparedReviewSchema.parse(JSON.parse(requiredText(value, "decision_json")));
    }
    catch (cause) {
        // error-policy:J2 A corrupt durable review is unavailable, not a reason to regenerate it.
        throw new AgreementKnowledgeError("Prepared agreement review is invalid", "AGREEMENT_INVALID_CONTRACT", undefined, cause);
    }
}
...
export type ParentingAgreementGuestArtifact = Pick<ParentingAgreementArtifact, "id" | "version" | "title" | "originalFilename" | "mimeType" | "byteSize" | "pageCount" | "createdAt">;
...
export type ParentingAgreementGuestObligation = Pick<ParentingAgreementObligation, "id" | "title" | "obligationText" | "pageStart" | "pageEnd" | "citationText" | "status" | "decidedAt">;
...
type AgreementKnowledgeErrorCode = "AGREEMENT_ACCESS_DENIED" | "AGREEMENT_ARTIFACT_NOT_FOUND" | "AGREEMENT_DUPLICATE_CONTENT" | "AGREEMENT_INVALID_CONTRACT" | "AGREEMENT_OBLIGATION_CONFLICT" | "AGREEMENT_STORAGE_UNAVAILABLE" | "AGREEMENT_EXTRACTION_UNAVAILABLE" | "AGREEMENT_INGESTION_RECONCILIATION_REQUIRED" | "AGREEMENT_INGESTION_CLEANUP_FAILED";
export class AgreementKnowledgeError extends ElizaError {
    override readonly name = "AgreementKnowledgeError";
    constructor(message: string, code: AgreementKnowledgeErrorCode, context?: Record<string, unknown>, cause?: unknown) {
        super(message, {
            code,
            context,
            cause,
...
function requiredText(value: unknown, field: string): string {
    ...
    if (!text) {
        throw new AgreementKnowledgeError(`Persisted agreement row is missing ${field}`, "AGREEMENT_INVALID_CONTRACT", { field });
    }
    return text;
}
...
function positiveInteger(value: unknown, field: string): number {
    ...
    if (!Number.isSafeInteger(number) || number < 1) {
        throw new AgreementKnowledgeError(`Persisted agreement row has invalid ${field}`, "AGREEMENT_INVALID_CONTRACT", { field, value: toText(value) });
    }
    return number;
}
function obligationStatus(value: unknown): AgreementObligationStatus {
    ...
    throw new AgreementKnowledgeError("Persisted agreement obligation has an invalid status", "AGREEMENT_INVALID_CONTRACT", { status: toText(value) });
}
function artifactFromRow(row: Record<string, unknown>): ParentingAgreementArtifact {
    return {
        ...
        uploadedByEntityId: requiredText(row.uploaded_by_entity_id, "uploadedByEntityId"),
        createdAt: requiredText(row.created_at, "createdAt"),
    };
}
function obligationFromRow(row: Record<string, unknown>): ParentingAgreementObligation {
    return {
        id: requiredText(row.id, "id"),
        agentId: requiredText(row.agent_id, "agentId"),
        ...
        proposedByEntityId: requiredText(row.proposed_by_entity_id, "proposedByEntityId"),
...
function pinFromRow(row: Record<string, unknown>): HouseholdKnowledgePin {
    ...
    if (targetType !== "agent" && targetType !== "chat") {
        throw new AgreementKnowledgeError("Persisted knowledge pin has an invalid target type", "AGREEMENT_INVALID_CONTRACT", { targetType });
    }
    return {
        id: requiredText(row.id, "id"),
        agentId: requiredText(row.agent_id, "agentId"),
...
function grantFromRow(row: Record<string, unknown>): HouseholdKnowledgeGrant {
    return {
        ...
        householdId: requiredText(row.household_id, "householdId"),
        artifactId: requiredText(row.artifact_id, "artifactId"),
        principalEntityId: requiredText(row.principal_entity_id, "principalEntityId"),
...
export class AgreementKnowledgeRepository {
    ...
    private async executeAgreementMutation(statement: string) {
        return withActiveFamilyWorkspaceTransaction(this.runtime, [
    ...
    async readExportSnapshot(artifactId: string) {
      ...
      WHERE ${scoped} AND owner_type = 'parenting_agreement' AND owner_id = ${artifact}
        ...
        const rows = await executeRawSql(this.runtime, queries
            .map(([kind, table, condition]) => `SELECT ${sqlQuote(kind)} AS kind, to_jsonb(record)::text AS payload
       FROM app_lifeops.${table} AS record WHERE ${scoped} AND ${condition}`)
        ...
        const records = rows.map((row) => {
            try {
                ...
                throw new AgreementKnowledgeError("Agreement export snapshot is invalid", "AGREEMENT_INVALID_CONTRACT", { artifactId }, error);
            }
        });
        const source = records.find((record) => record.kind === "artifact");
        if (!source)
            throw new AgreementKnowledgeError("Parenting-agreement artifact was not found", "AGREEMENT_ARTIFACT_NOT_FOUND", { artifactId });
        const ofKind = (kind: string) => records
        ...
        return {
            ...
            audit: ofKind("audit").sort((left, right) => String(left.created_at).localeCompare(String(right.created_at)) ||
    ...
    async recordExport(input: {
        ...
        const rows = await withActiveFamilyWorkspaceTransaction(this.runtime, ["app_lifeops.life_audit_events"], (tx) => executeRawSqlTx(tx, `INSERT INTO app_lifeops.life_audit_events (
      ...
      ${sqlQuote(JSON.stringify({ manifestSha256: input.manifestSha256, archiveSha256: input.archiveSha256 }))}, 'owner', ${sqlQuote(input.createdAt)}) RETURNING id`), this.agentId);
        if (rows.length !== 1)
            throw new AgreementKnowledgeError("Agreement export audit was not persisted", "AGREEMENT_INVALID_CONTRACT");
    }
    async insertArtifact(input: Omit<ParentingAgreementArtifact, "version"> & {
        ...
        return await withTransaction(this.runtime, async (tx) => {
            const previousRows = await executeRawSqlTx(tx, `SELECT * FROM app_lifeops.life_household_agreement_artifacts
          ...
          FOR UPDATE`);
            ...
            if (previous?.contentSha256 === input.contentSha256) {
                throw new AgreementKnowledgeError("This agreement content is already the current immutable version", "AGREEMENT_DUPLICATE_CONTENT", { artifactId: previous.id, contentSha256: input.contentSha256 });
            }
            const version = (previous?.version ?? 0) + 1;
            const rows = await executeRawSqlTx(tx, agreementMutationSql(`INSERT INTO app_lifeops.life_household_agreement_artifacts (
           ...
           ${sqlQuote(input.uploadedByEntityId)}, ${sqlQuote(input.createdAt)}
            ...
            const row = rows[0];
            if (!row) {
                throw new AgreementKnowledgeError("Agreement version insert returned no persisted row", "AGREEMENT_INVALID_CONTRACT");
            }
            return artifactFromRow(row);
        });
    }
    async getArtifact(id: string): Promise<ParentingAgreementArtifact | null> {
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_household_agreement_artifacts
        WHERE agent_id = ${sqlQuote(this.agentId)} AND id = ${sqlQuote(id)}
        LIMIT 1`);
        return rows[0] ? artifactFromRow(rows[0]) : null;
    }
    async listArtifacts(householdId?: string): Promise<ParentingAgreementArtifact[]> {
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_household_agreement_artifacts
        WHERE agent_id = ${sqlQuote(this.agentId)}
    ...
    private obligationInsertSql(obligation: ParentingAgreementObligation): string {
        return agreementMutationSql(`INSERT INTO app_lifeops.life_household_agreement_obligations (
    ...
    async insertObligation(obligation: ParentingAgreementObligation): Promise<ParentingAgreementObligation> {
        const rows = await this.executeAgreementMutation(this.obligationInsertSql(obligation));
        const row = rows[0];
        if (!row) {
            throw new AgreementKnowledgeError("Agreement obligation insert returned no persisted row", "AGREEMENT_INVALID_CONTRACT");
        }
        return obligationFromRow(row);
    }
    /** Serializes correction retries with other review writes without resetting a saved decision. */
    async insertOwnerProposalOnce(obligation: ParentingAgreementObligation): Promise<{
        ...
        return withActiveFamilyWorkspaceTransaction(this.runtime, [
            ...
            const artifacts = await executeRawSqlTx(tx, `SELECT id FROM app_lifeops.life_household_agreement_artifacts
         WHERE agent_id = ${sqlQuote(this.agentId)} AND id = ${sqlQuote(obligation.artifactId)} FOR UPDATE`);
            if (artifacts.length !== 1)
                throw new AgreementKnowledgeError("Agreement source is unavailable", "AGREEMENT_ARTIFACT_NOT_FOUND");
            const existing = await executeRawSqlTx(tx, `SELECT * FROM app_lifeops.life_household_agreement_obligations
         WHERE agent_id = ${sqlQuote(this.agentId)} AND artifact_id = ${sqlQuote(obligation.artifactId)} AND id = ${sqlQuote(obligation.id)}`);
            ...
            const inserted = await executeRawSqlTx(tx, this.obligationInsertSql(obligation));
            if (inserted.length !== 1 || !inserted[0])
                throw new AgreementKnowledgeError("Owner proposal did not persist", "AGREEMENT_STORAGE_UNAVAILABLE");
            return { obligation: obligationFromRow(inserted[0]), created: true };
        });
    }
    async readPreparedReview(artifactId: string): Promise<PreparedReviewRecord | null> {
        const rows = await executeRawSql(this.runtime, `SELECT decision_json FROM app_lifeops.life_audit_events
      WHERE agent_id = ${sqlQuote(this.agentId)} AND owner_type = 'parenting_agreement'
        ...
        if (rows.length === 0)
            return null;
        if (rows.length !== 1)
            throw new AgreementKnowledgeError("Agreement has conflicting prepared review records", "AGREEMENT_INVALID_CONTRACT");
        return preparedReviewFromJson(rows[0]?.decision_json);
    }
    async commitPreparedReview(record: PreparedReviewRecord, obligations: ParentingAgreementObligation[]): Promise<PreparedReviewRecord> {
        if (record.obligationIds.length !== obligations.length ||
            obligations.some((item, index) => item.id !== record.obligationIds[index] ||
                item.artifactId !== record.artifactId ||
                item.agentId !== this.agentId ||
                item.status !== "proposed")) {
            throw new AgreementKnowledgeError("Prepared review batch does not match its source or proposal identities", "AGREEMENT_INVALID_CONTRACT");
        }
        return withActiveFamilyWorkspaceTransaction(this.runtime, [
            "app_lifeops.life_audit_events",
            "app_lifeops.life_household_agreement_artifacts",
        ...
        WHERE agent_id = ${sqlQuote(this.agentId)} AND id = ${sqlQuote(record.artifactId)} FOR UPDATE`);
            if (artifacts.length !== 1)
                throw new AgreementKnowledgeError("Agreement source is unavailable", "AGREEMENT_ARTIFACT_NOT_FOUND");
            const existing = await executeRawSqlTx(tx, `SELECT decision_json FROM app_lifeops.life_audit_events
        WHERE agent_id = ${sqlQuote(this.agentId)} AND owner_type = 'parenting_agreement'
          AND owner_id = ${sqlQuote(record.artifactId)} AND event_type = 'agreement_review_prepared'`);
            ...
            for (const obligation of obligations) {
                ...
                if (inserted.length !== 1)
                    throw new AgreementKnowledgeError("Review proposal did not persist", "AGREEMENT_STORAGE_UNAVAILABLE");
            }
            const saved = await executeRawSqlTx(tx, `INSERT INTO app_lifeops.life_audit_events
        (id, agent_id, event_type, owner_type, owner_id, reason, inputs_json, decision_json, actor, created_at)
        VALUES (${sqlQuote(`agreement_review_${crypto.randomUUID()}`)}, ${sqlQuote(this.agentId)}, 'agreement_review_prepared',
          ...
          ${sqlQuote(JSON.stringify(record))}, 'owner', ${sqlQuote(record.generatedAt)}) RETURNING id`);
            if (saved.length !== 1)
                throw new AgreementKnowledgeError("Prepared review did not persist", "AGREEMENT_STORAGE_UNAVAILABLE");
            return record;
        });
    }
    async decideObligation(input: {
        ...
        const rows = await this.executeAgreementMutation(agreementMutationSql(`UPDATE app_lifeops.life_household_agreement_obligations
      ...
      RETURNING *`, {
        ...
        const row = rows[0];
        if (!row) {
            throw new AgreementKnowledgeError("Obligation is missing or has already received a final decision", "AGREEMENT_OBLIGATION_CONFLICT", { obligationId: input.obligationId });
        }
        return obligationFromRow(row);
    }
    async listObligations(artifactId: string): Promise<ParentingAgreementObligation[]> {
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_household_agreement_obligations
        WHERE agent_id = ${sqlQuote(this.agentId)}
    ...
    async setPin(input: {
        ...
        const rows = await this.executeAgreementMutation(agreementMutationSql(`INSERT INTO app_lifeops.life_household_knowledge_pins (
       ...
       RETURNING *`, {
        ...
        const row = rows[0];
        if (!row) {
            throw new AgreementKnowledgeError("Knowledge pin insert returned no persisted row", "AGREEMENT_INVALID_CONTRACT");
        }
        return pinFromRow(row);
    }
    async listPins(artifactId: string): Promise<HouseholdKnowledgePin[]> {
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_household_knowledge_pins
        WHERE agent_id = ${sqlQuote(this.agentId)}
          AND artifact_id = ${sqlQuote(artifactId)}
    ...
    async listActivePinsForTargets(targets: ReadonlyArray<{
        targetType: KnowledgePinTargetType;
        targetId: string;
    }>): Promise<HouseholdKnowledgePin[]> {
        if (targets.length === 0)
        ...
        const targetSql = targets
            .map((target) => `(target_type = ${sqlQuote(target.targetType)} AND target_id = ${sqlQuote(target.targetId)})`)
            .join(" OR ");
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_household_knowledge_pins
        WHERE agent_id = ${sqlQuote(this.agentId)}
          AND unpinned_at IS NULL
    ...
    async removePin(input: {
        ...
        const rows = await this.executeAgreementMutation(agreementMutationSql(`UPDATE app_lifeops.life_household_knowledge_pins
      ...
      RETURNING *`, {
        ...
        const row = rows[0];
        if (!row) {
            throw new AgreementKnowledgeError("Knowledge pin is missing or already inactive", "AGREEMENT_INVALID_CONTRACT", { pinId: input.pinId });
        }
        return pinFromRow(row);
    }
    async upsertGrant(input: HouseholdKnowledgeGrant) {
        const rows = await this.executeAgreementMutation(agreementMutationSql(`INSERT INTO app_lifeops.life_household_knowledge_grants (
       ...
       RETURNING *`, {
        ...
        const row = rows[0];
        if (!row) {
            throw new AgreementKnowledgeError("Knowledge grant insert returned no persisted row", "AGREEMENT_INVALID_CONTRACT");
        }
        return grantFromRow(row);
    }
    async listGrants(artifactId: string, principalEntityId: string): Promise<HouseholdKnowledgeGrant[]> {
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_household_knowledge_grants
        WHERE agent_id = ${sqlQuote(this.agentId)}
          AND artifact_id = ${sqlQuote(artifactId)}
    ...
    async listArtifactGrants(artifactId: string): Promise<HouseholdKnowledgeGrant[]> {
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_household_knowledge_grants
        WHERE agent_id = ${sqlQuote(this.agentId)}
          AND artifact_id = ${sqlQuote(artifactId)}
        ORDER BY created_at ASC, id ASC`);
    ...
    async revokeGrant(input: {
        ...
        const rows = await this.executeAgreementMutation(agreementMutationSql(`UPDATE app_lifeops.life_household_knowledge_grants
      ...
      RETURNING *`, {
        ...
        const row = rows[0];
        if (!row) {
            throw new AgreementKnowledgeError("Knowledge grant is missing or already revoked", "AGREEMENT_ACCESS_DENIED", { grantId: input.grantId });
        }
        return grantFromRow(row);
    }
}
...
function guestArtifactProjection(artifact: ParentingAgreementArtifact): ParentingAgreementGuestArtifact {
    return {
        id: artifact.id,
...
function guestObligationProjection(obligation: ParentingAgreementObligation): ParentingAgreementGuestObligation {
    return {
        id: obligation.id,
...
function requirePositiveInteger(value: number, field: string): number {
    if (!Number.isSafeInteger(value) || value < 1) {
        throw new AgreementKnowledgeError(`${field} must be a positive integer`, "AGREEMENT_INVALID_CONTRACT", { field, value });
    }
    return value;
}
export class AgreementKnowledgeService {
    private readonly now: () => Date;
    private readonly reviewInFlight = new Map<string, Promise<PreparedAgreementReview>>();
    ...
    private requireOwner(actorEntityId: string): void {
        if (actorEntityId !== SELF_ENTITY_ID) {
            throw new AgreementKnowledgeError("Only the owner may mutate parenting-agreement knowledge", "AGREEMENT_ACCESS_DENIED", { actorEntityId });
        }
    }
    async listApprovedObligations(): Promise<ParentingAgreementObligation[]> {
    ...
    async prepareOwnerReview(input: {
        ...
        const artifactId = normalizeHouseholdIdentifier(input.artifactId, "artifactId");
    ...
    async readOwnerReview(input: {
        ...
        if (!record)
            return null;
        return this.preparedReviewResult(await this.reviewSource(artifact.id), record);
    }
    private async reviewSource(artifactId: string): Promise<AgreementReviewSource> {
        ...
        if (!documents)
            throw new AgreementKnowledgeError("Agreement document service is unavailable", "AGREEMENT_STORAGE_UNAVAILABLE");
        const document = await documents.getDocumentById(snapshot.artifact.documentId as UUID);
        const raw = document?.metadata && "agreementExtractionJson" in document.metadata
            ? document.metadata.agreementExtractionJson
            : undefined;
        const ingestion = snapshot.audit.find((event) => event.event_type === "agreement_ingested");
        try {
            ...
            if (extraction.pageCount !== snapshot.artifact.pageCount ||
            ...
            throw new AgreementKnowledgeError("Agreement review requires a complete, integrity-verified extraction", "AGREEMENT_INVALID_CONTRACT", { artifactId }, cause);
        }
    }
    private async prepareOwnerReviewOnce(artifactId: string): Promise<PreparedAgreementReview> {
        const source = await this.reviewSource(artifactId);
        let record = await this.deps.repository.readPreparedReview(artifactId);
        if (!record) {
            const generated: GeneratedAgreementReview = await withStandaloneTrajectory(this.deps.runtime, {
            ...
            const obligations: ParentingAgreementObligation[] = generated.proposals.map((proposal) => ({
                ...proposal,
    ...
    private async preparedReviewResult(source: AgreementReviewSource, record: PreparedReviewRecord): Promise<PreparedAgreementReview> {
        ...
        if (record.artifactId !== artifactId ||
            ...
            throw new AgreementKnowledgeError("Prepared review does not match this immutable source", "AGREEMENT_INVALID_CONTRACT", { artifactId });
        }
        ...
        const obligations = record.obligationIds.map((id) => {
            ...
            if (!obligation)
                throw new AgreementKnowledgeError("Prepared review obligation is unavailable", "AGREEMENT_STORAGE_UNAVAILABLE", { artifactId, obligationId: id });
            return obligation;
        });
        return {
            artifactId,
    ...
    private requireOwnerOrAgent(actorEntityId: string): void {
        if (actorEntityId !== SELF_ENTITY_ID &&
            ...
            throw new AgreementKnowledgeError("Only the owner or this agent may propose agreement obligations", "AGREEMENT_ACCESS_DENIED", { actorEntityId });
        }
    }
    private async settleIngestionOperation(operationId: string, priorFailure?: unknown): Promise<void> {
        try {
            ...
            const failure = new AgreementKnowledgeError("The ingestion operation could not be settled. Reconcile its claim before deletion or retry.", "AGREEMENT_INGESTION_RECONCILIATION_REQUIRED", { operationId }, priorFailure === undefined
                ? cause
    ...
    private async requireArtifact(id: string) {
        await this.requireReadableWorkspace();
        const artifact = await this.deps.repository.getArtifact(normalizeHouseholdIdentifier(id, "artifactId"));
        if (!artifact) {
            throw new AgreementKnowledgeError("Parenting-agreement artifact was not found", "AGREEMENT_ARTIFACT_NOT_FOUND", { artifactId: id });
        }
        await this.requireReadableWorkspace();
        return artifact;
    }
    async listOwnerAgreements(input: {
        ...
        const views = await Promise.all(artifacts.map(async (artifact) => ({
            artifact,
    ...
    async createAgreementVersion(input: {
        ...
        if (input.mimeType.trim().toLowerCase() !== "application/pdf") {
            throw new AgreementKnowledgeError("Parenting agreements must be uploaded as PDF bytes", "AGREEMENT_INVALID_CONTRACT", { mimeType: input.mimeType });
        }
        if (input.bytes.byteLength < 1) {
            throw new AgreementKnowledgeError("Parenting agreement PDF must not be empty", "AGREEMENT_INVALID_CONTRACT");
        }
        const bytes = Buffer.from(input.bytes);
        if (bytes.subarray(0, 5).toString("ascii") !== "%PDF-") {
            throw new AgreementKnowledgeError("Parenting agreement bytes do not have a PDF signature", "AGREEMENT_INVALID_CONTRACT");
        }
        const householdId = normalizeHouseholdIdentifier(input.householdId ?? DEFAULT_HOUSEHOLD_ID, "householdId");
        const agreementKey = nonEmpty(input.agreementKey, "agreementKey");
        const title = nonEmpty(input.title, "title");
        const originalFilename = nonEmpty(input.originalFilename, "originalFilename");
        ...
        if (existing) {
            throw new AgreementKnowledgeError("This agreement content already exists as an immutable version", "AGREEMENT_DUPLICATE_CONTENT", { artifactId: existing.id, contentSha256: expectedSha256 });
        }
        const fileStorage = this.deps.fileStorage();
        const documents = this.deps.documents();
        const pdf = this.deps.pdf();
        if (!fileStorage || !documents || !pdf) {
            throw new AgreementKnowledgeError("The runtime file-storage, document, or PDF service is unavailable", "AGREEMENT_STORAGE_UNAVAILABLE", {
        ...
        try {
            ...
            if (error instanceof ElizaError &&
                ...
                this.deps.runtime.reportError("AgreementKnowledge.extractCompleteDocument", error);
                throw new AgreementSourceUnchangedError("Document reading is temporarily unavailable. Check the model service, then retry this upload.", "AGREEMENT_EXTRACTION_UNAVAILABLE", error.context, error);
            }
            throw new AgreementSourceUnchangedError(`The complete parenting-agreement PDF could not be extracted: ${error instanceof Error ? error.message : String(error)}`, "AGREEMENT_INVALID_CONTRACT", undefined, error);
        }
        ...
        try {
            ...
            const failure = new AgreementKnowledgeError("Private PDF persistence could not be confirmed. Reconcile the source identity before retrying or deleting its claim.", "AGREEMENT_INGESTION_RECONCILIATION_REQUIRED", { operationId, artifactId, contentSha256: expectedSha256 }, cause);
            this.deps.runtime.reportError("AgreementKnowledge.ingestion", failure);
            throw failure;
        }
        let documentId: UUID | null = null;
        try {
            if (stored.hash !== expectedSha256 ||
                !stored.fileName.startsWith(`${expectedSha256}.`) ||
                stored.size !== bytes.byteLength) {
                throw new AgreementKnowledgeError("File storage returned metadata that does not match the agreement bytes", "AGREEMENT_INVALID_CONTRACT", { expectedSha256, storedHash: stored.hash });
            }
            ...
            if (documentId) {
                ...
                try {
                    ...
                    const failure = new AgreementKnowledgeError("Upload persistence could not be reconciled. Inspect the artifact before retrying or deleting its sources.", "AGREEMENT_INGESTION_RECONCILIATION_REQUIRED", {
                        artifactId,
                        documentId,
                        mediaFileName: stored.fileName,
                        operationId,
                ...
                if (persisted) {
                    const failure = new AgreementKnowledgeError("The agreement was persisted but the upload did not finish normally. Review the existing version before retrying.", "AGREEMENT_INGESTION_RECONCILIATION_REQUIRED", {
                        artifactId,
                        documentId,
                        mediaFileName: stored.fileName,
                        operationId,
            ...
            if (failures.length > 0) {
                const failure = new AgreementKnowledgeError("The upload failed and its private-source cleanup is incomplete. Resolve the reported storage failures before retrying.", "AGREEMENT_INGESTION_CLEANUP_FAILED", {
                    artifactId,
                    documentId,
                    mediaFileName: stored.fileName,
                    operationId,
    ...
    async readOwnerPdf(input: {
        ...
        const fileStorage = this.deps.fileStorage();
        if (!fileStorage) {
            throw new AgreementKnowledgeError("The runtime private file-storage service is unavailable", "AGREEMENT_STORAGE_UNAVAILABLE");
        }
        const bytes = await fileStorage.readPrivate(artifact.mediaFileName);
        if (!bytes) {
            throw new AgreementKnowledgeError("The immutable parenting-agreement PDF is unavailable", "AGREEMENT_STORAGE_UNAVAILABLE", { artifactId: artifact.id });
        }
        const hash = crypto.createHash("sha256").update(bytes).digest("hex");
        if (hash !== artifact.contentSha256 || bytes.length !== artifact.byteSize) {
            throw new AgreementKnowledgeError("The immutable parenting-agreement PDF failed integrity verification", "AGREEMENT_INVALID_CONTRACT", { artifactId: artifact.id });
        }
        await this.requireReadableWorkspace();
        return {
            bytes,
    ...
    async exportOwnerAgreement(input: {
        ...
        const snapshot = await this.deps.repository.readExportSnapshot(input.artifactId);
        const documents = this.deps.documents();
        if (!documents)
            throw new AgreementKnowledgeError("Agreement document service is unavailable", "AGREEMENT_STORAGE_UNAVAILABLE");
        const document = await documents.getDocumentById(snapshot.artifact.documentId as UUID);
        if (!document)
            throw new AgreementKnowledgeError("Agreement extraction document is unavailable", "AGREEMENT_STORAGE_UNAVAILABLE", { artifactId: input.artifactId });
        const extractionJson = document.metadata && "agreementExtractionJson" in document.metadata
            ? document.metadata.agreementExtractionJson
            : undefined;
        ...
        if (ingestion) {
            try {
                ...
                throw new AgreementKnowledgeError("Agreement ingestion provenance is invalid", "AGREEMENT_INVALID_CONTRACT", { artifactId: input.artifactId }, error);
            }
        }
        if (recordedExtractionSha256 &&
            (typeof extractionJson !== "string" ||
                crypto.createHash("sha256").update(extractionJson).digest("hex") !==
                    recordedExtractionSha256)) {
            throw new AgreementKnowledgeError("Agreement extraction failed integrity verification", "AGREEMENT_INVALID_CONTRACT", { artifactId: input.artifactId });
        }
        if (extractionJson === undefined) {
            extraction = {
                status: "unavailable",
                reason: "This version predates persisted extraction page maps; no historical extraction has been reconstructed.",
            };
        }
        else {
            try {
                ...
                const parsed = agreementExtractionSchema.parse(JSON.parse(extractionJson));
                if (parsed.pageCount !== snapshot.artifact.pageCount ||
                ...
                throw new AgreementKnowledgeError("Agreement extraction provenance is invalid", "AGREEMENT_INVALID_CONTRACT", { artifactId: input.artifactId }, error);
            }
        }
        ...
        const manifest = Buffer.from(`${JSON.stringify({
            ...
            extraction,
            auditCoverage: {
                status: snapshot.audit.some((event) => event.event_type === "agreement_ingested")
                    ? "recorded_from_ingestion"
                    : "partial_legacy_history",
                scope: "Persisted agreement events through the database snapshot. Earlier unrecorded activity is unavailable. This export preparation is recorded after archive construction; successful client receipt is not asserted.",
            },
        }, null, 2)}\n`, "utf8");
    ...
    async proposeObligation(input: {
        ...
        const pageEnd = requirePositiveInteger(input.pageEnd ?? pageStart, "pageEnd");
        if (pageEnd < pageStart || pageEnd > artifact.pageCount) {
            throw new AgreementKnowledgeError("Obligation citation pages must be ordered and inside the source PDF", "AGREEMENT_INVALID_CONTRACT", { pageStart, pageEnd, pageCount: artifact.pageCount });
        }
        const now = this.now().toISOString();
    ...
    async decideObligation(input: {
        ...
        return await this.deps.repository.decideObligation({
            obligationId: normalizeHouseholdIdentifier(input.obligationId, "obligationId"),
    ...
    async listPinTargets(ownerEntityId: string): Promise<AgreementPinTargets> {
        ...
        chats.sort((left, right) => (left.name ?? "").localeCompare(right.name ?? "") ||
    ...
    async pin(input: {
        ...
        const valid = input.targetType === "agent"
            ? targetId === targets.agent.id
            : input.targetType === "chat" &&
                targets.chats.some((chat) => chat.id === targetId);
        if (!valid) {
            throw new AgreementKnowledgeError("This pin destination is unavailable. Refresh the choices and select this agent or one of its current conversations.", "AGREEMENT_INVALID_CONTRACT", { targetType: input.targetType, targetId });
        }
    ...
    async listGuestAccessOptions(input: {
        ...
        for (const grant of householdGrants) {
            if (grant.principalEntityId === SELF_ENTITY_ID)
                continue;
            const principal = await this.deps.entityStore.get(grant.principalEntityId);
        ...
        for (const grant of await this.deps.repository.listArtifactGrants(artifact.id)) {
            if (grant.revokedAt)
                continue;
            const principal = await this.deps.entityStore.get(grant.principalEntityId);
    ...
    async previewGuestRead(input: {
        ...
        this.requireOwner(input.ownerEntityId);
        const artifact = await this.requireArtifact(input.artifactId);
        const principalEntityId = normalizeHouseholdIdentifier(input.principalEntityId, "principalEntityId");
        const householdGrantId = normalizeHouseholdIdentifier(input.householdGrantId, "householdGrantId");
        const base = {
            artifactId: artifact.id,
            principalEntityId,
            householdGrantId,
        ...
        try {
            ...
            if (!(error instanceof HouseholdCoordinationError))
                throw error;
            return {
                ...base,
                allowed: false,
    ...
    async activePinnedContext(input: {
        ...
        for (const artifactId of artifactIds) {
            ...
            const obligations = (await this.deps.repository.listObligations(artifact.id)).filter((obligation) => obligation.status === "approved");
            if (obligations.length > 0)
                views.push({ artifact, obligations });
        }
        await this.requireReadableWorkspace();
        return views;
    }
        ...
        for (const artifactId of artifactIds) {
            try {
                ...
                if ("contentSha256" in view.artifact) {
                    throw new AgreementKnowledgeError("Guest pin projection unexpectedly resolved owner knowledge", "AGREEMENT_ACCESS_DENIED", { artifactId, principalEntityId });
                }
                views.push(view);
            }
            catch (error) {
    ...
    async grantGuestRead(input: {
        ...
        const principal = await this.deps.entityStore.get(principalEntityId);
        if (!principal?.identities.some((identity) => identity.verified)) {
            throw new AgreementKnowledgeError("Agreement guests require at least one verified identity", "AGREEMENT_ACCESS_DENIED", { principalEntityId });
        }
        await this.deps.household.requireGrantActive({
            householdId: artifact.householdId,
    ...
    async readFor(input: {
        ...
        const grants = await this.deps.repository.listGrants(artifact.id, principalEntityId);
        ...
        throw new AgreementKnowledgeError("The principal has no active grant for this agreement version", "AGREEMENT_ACCESS_DENIED", { artifactId: artifact.id, principalEntityId });
    }
}
export function createAgreementKnowledgeService(runtime: IAgentRuntime, now?: () => Date): AgreementKnowledgeService {
    const graph = resolveKnowledgeGraphService(runtime);
    const household = getHouseholdCoordinationService(runtime);
    if (!graph || !household) {
        throw new AgreementKnowledgeError("Agreement knowledge requires graph and household services", "AGREEMENT_STORAGE_UNAVAILABLE", {
            graph: Boolean(graph),
    ...
    return new AgreementKnowledgeService({
        ...
        fileStorage: () => runtime.getService<IFileStorageService>(ServiceType.REMOTE_FILES),
        documents: () => runtime.getService<DocumentService>(DocumentService.serviceType),
...
export class AgreementKnowledgeRuntimeService extends Service {
    ...
    override capabilityDescription = "Immutable parenting-agreement versions, reviewed citations, pins, and bounded guest reads";
    readonly agreements: AgreementKnowledgeService;
    constructor(runtime?: IAgentRuntime) {
        super(runtime);
        if (!runtime) {
            throw new AgreementKnowledgeError("AgreementKnowledgeRuntimeService requires a runtime", "AGREEMENT_INVALID_CONTRACT");
        }
...
export function getAgreementKnowledgeService(runtime: IAgentRuntime): AgreementKnowledgeService | null {
    return (runtime.getService<AgreementKnowledgeRuntimeService>(HOUSEHOLD_AGREEMENT_KNOWLEDGE_SERVICE)?.agreements ?? null);
}
