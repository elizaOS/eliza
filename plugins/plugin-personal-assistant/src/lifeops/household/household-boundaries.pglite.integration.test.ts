...
import { type HouseholdScheduleProposal } from "./types.js";
import { type HouseholdScheduleTerms } from "./types.js";
...
describe("household authorization boundaries — real PGlite", () => {
    ...
    beforeAll(async () => {
        ...
        if (!graph) {
            throw new Error(`Knowledge graph service ${KNOWLEDGE_GRAPH_SERVICE} was not registered`);
        }
        ...
        return {
            ...
            custodyException: input.custody
                ? {
                    ...
                    substituteCustodianEntityId: input.custody.substituteCustodianEntityId,
                    authorityBaselineRelationshipId: input.custody.authorityBaselineRelationshipId,
                    reason: "The normal handoff changed for this interval.",
                }
                : null,
        };
    }
    async function approve(service: HouseholdCoordinationService, proposal: HouseholdScheduleProposal, partyEntityId: string): Promise<void> {
        const link = (await repository.listApprovalLinks(proposal.proposalId, proposal.version)).find((candidate) => candidate.partyEntityId === partyEntityId);
        if (!link)
            throw new Error(`approval link missing for ${partyEntityId}`);
    ...
    it("isolates one principal's roles, grants, heads, and schedule visibility by household", async () => {
        ...
        expect(exportA.roles).toEqual(expect.arrayContaining([
        ...
        expect(exportB.roles).toEqual(expect.arrayContaining([
            expect.objectContaining({
                householdId: householdB,
                entityId: principalId,
                role: "caregiver",
        ...
        await expect(service.requireScope({
            householdId: householdA,
            principalEntityId: principalId,
            subjectEntityId: childB,
            scope: "calendar.freebusy",
        })).rejects.toMatchObject({ code: "HOUSEHOLD_ACCESS_DENIED" });
        await expect(repository.getHead(householdA, sharedCoordinationId)).resolves.toMatchObject({ householdId: householdA });
        await expect(repository.getHead(householdB, sharedCoordinationId)).resolves.toMatchObject({ householdId: householdB });
    });
    it("pins custody authority revisions and invalidates approvals on revise and revoke", async () => {
        ...
        expect(firstProposal.terms.custodyException?.authorityBaselineRevisionSha256).toBe(firstAuthority.revisionSha256);
        ...
        expect(secondAuthority.revisionSha256).not.toBe(firstAuthority.revisionSha256);
        await expect(repository.getProposal(firstProposal.proposalId, firstProposal.version)).resolves.toMatchObject({ status: "invalidated" });
        const firstLinks = await repository.listApprovalLinks(firstProposal.proposalId, firstProposal.version);
        for (const link of firstLinks) {
            await expect(approvals.byId(link.approvalRequestId, link.partyEntityId)).resolves.toMatchObject({ state: "expired" });
        }
        await expect(service.finalizeProposal({
        ...
        expect(secondProposal.terms.custodyException?.authorityBaselineRevisionSha256).toBe(secondAuthority.revisionSha256);
        ...
        await expect(repository.getProposal(secondProposal.proposalId, secondProposal.version)).resolves.toMatchObject({ status: "invalidated" });
        await expect(service.finalizeProposal({
            proposalId: secondProposal.proposalId,
            proposalVersion: secondProposal.version,
        })).rejects.toMatchObject({ code: "HOUSEHOLD_PROPOSAL_CONFLICT" });
        await expect(service.setCustodyAuthority({
            householdId,
