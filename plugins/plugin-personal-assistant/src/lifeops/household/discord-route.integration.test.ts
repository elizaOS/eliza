...
import { registerConnectorRegistry } from "../connectors/index.js";
import { resolveKnowledgeGraphService } from "@elizaos/plugin-relationships";
import { type AgentRuntime } from "@elizaos/core";
import { type ConnectorSendPayload } from "../connectors/_helpers.js";
import { type ConnectorStatus } from "../connectors/index.js";
...
describe("household discord party route — real PGlite", () => {
    ...
    beforeAll(async () => {
        ...
        connectors.register({
            ...
            async send() {
                ...
                throw new Error("The dead telegram connector must never receive a household send.");
            },
        });
        connectors.register({
            kind: "discord",
    ...
    it("delivers the party prompt as a user-typed discord DM target, past the dead telegram route", async () => {
        ...
        const delivered = discordSends.find((send) => send.target === discordUserId);
        if (!delivered)
            throw new Error("discord approval prompt not delivered");
        ...
        expect(delivered.message).toContain(householdApprovalCommandText("approve", pending.approvalRequestId));
