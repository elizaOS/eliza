 ...
 */
import { FoodDomainError } from "./types.js";
import { FoodRepository } from "./repository.js";
import { InstacartProductsLinkClient } from "./instacart.js";
import { KNOWLEDGE_GRAPH_SERVICE } from "@elizaos/plugin-relationships";
import { SELF_ENTITY_ID } from "@elizaos/core/knowledge-graph/entity-types";
import { Service } from "@elizaos/core";
import { assertMealPlanSafe } from "./planner.js";
import { assertNonEmptyText } from "./types.js";
import { buildShoppingListContent } from "./planner.js";
import { createApprovalQueue } from "../approval-queue.js";
import { evaluateMealPlan } from "./planner.js";
import { resolveKnowledgeGraphService } from "@elizaos/plugin-relationships";
import { type ApprovalExecutionClaim } from "../approval-queue.types.js";
import { type ApprovalExecutionMutation } from "../approval-queue.types.js";
...
import { type EvaluateMealPlanInput } from "./planner.js";
import { type FoodChildView } from "./types.js";
import { type FoodHouseholdProfile } from "./types.js";
import { type FoodOwnerView } from "./types.js";
import { type FoodPreference } from "./types.js";
import { type FoodShoppingHandoff } from "./types.js";
import { type FoodView } from "./types.js";
import { type HardFoodConstraint } from "./types.js";
import { type IAgentRuntime } from "@elizaos/core";
import { type InventoryObservation } from "./types.js";
import { type MealPlanEvaluation } from "./types.js";
import { type PublishedMealPlan } from "./types.js";
import { type RelationshipStore } from "@elizaos/plugin-relationships";
...
function exactApprovalPayloadMatches(request: ApprovalRequest | null, handoff: FoodShoppingHandoff): boolean {
    return (request !== null &&
...
function relationshipRole(relationships: Awaited<ReturnType<RelationshipStore["list"]>>): string | null {
...
function relationshipSubjects(relationships: Awaited<ReturnType<RelationshipStore["list"]>>): string[] {
    return Array.from(new Set(relationships.flatMap((relationship) => {
        const subjects = relationship.metadata?.[HOUSEHOLD_SUBJECTS_METADATA_KEY];
        return Array.isArray(subjects)
            ? subjects.filter((subject): subject is string => typeof subject === "string")
            : [];
    })));
}
export class FoodDomainService {
    ...
    private assertOwnerPrincipal(principalEntityId: string): void {
        if (principalEntityId !== SELF_ENTITY_ID) {
            throw new FoodDomainError("Only the authenticated household owner may mutate food policy, inventory, or shopping handoffs", "FOOD_ACCESS_DENIED", { principalEntityId });
        }
    }
    private async requireProfile(householdId: string): Promise<FoodHouseholdProfile> {
        const profile = await this.deps.repository.getHouseholdProfile(householdId);
        if (!profile) {
            throw new FoodDomainError("Food household profile does not exist", "FOOD_INVALID_CONTRACT", { householdId });
        }
        return profile;
    }
    private async assertKnownEntity(entityId: string): Promise<void> {
        if (!(await this.deps.entityStore.get(entityId))) {
            throw new FoodDomainError(`Food-domain subject ${entityId} is not a canonical EntityStore entity`, "FOOD_ENTITY_NOT_FOUND", { entityId });
        }
    }
    private async assertHouseholdRelationship(entityId: string): Promise<void> {
        ...
        if (relationships.length === 0) {
            throw new FoodDomainError(`Food-domain member ${entityId} has no active household relationship`, "FOOD_RELATIONSHIP_REQUIRED", { entityId });
        }
    }
    private async assertProfileMember(profile: FoodHouseholdProfile, entityId: string): Promise<void> {
        if (!profile.memberEntityIds.includes(entityId)) {
            throw new FoodDomainError(`Entity ${entityId} is outside food household ${profile.householdId}`, "FOOD_ACCESS_DENIED", { entityId, householdId: profile.householdId });
        }
    ...
    async putConstraint(input: {
        ...
        if (input.constraint.appliesToEntityId !== null) {
            await this.assertProfileMember(profile, input.constraint.appliesToEntityId);
        }
        return this.deps.repository.putConstraint(input.constraint, input.expectedVersion, this.now().toISOString());
    }
    async putPreference(input: {
        ...
        if (input.preference.appliesToEntityId !== null) {
            await this.assertProfileMember(profile, input.preference.appliesToEntityId);
        }
        return this.deps.repository.putPreference(input.preference, input.expectedVersion, this.now().toISOString());
    }
    async recordInventoryObservation(input: {
        ...
        return this.deps.repository.recordInventoryObservation(input.observation, this.now().toISOString());
    }
    ...
    async publishMealPlan(input: {
        ...
        this.assertOwnerPrincipal(input.principalEntityId);
        const evaluation = assertMealPlanSafe(await this.currentCanonicalEvaluation(input.evaluation));
        return this.deps.repository.publishMealPlan({
            ...
            attendeeEntityIds: evaluation.participants.map((participant) => participant.entityId),
            evaluation,
            contentSha256: evaluation.contentSha256,
        }, this.now().toISOString());
    }
    private async currentCanonicalEvaluation(evaluation: MealPlanEvaluation): Promise<MealPlanEvaluation> {
        ...
        if (canonical.contentSha256 !== evaluation.contentSha256) {
            throw new FoodDomainError("Meal evaluation is stale or does not match its deterministic content hash", "FOOD_STALE_SOURCE", {
                householdId: evaluation.householdId,
                safetyPolicyChanged: canonical.safetyPolicySha256 !== evaluation.safetyPolicySha256,
                inventoryChanged: canonical.inventorySnapshotSha256 !==
    ...
    async requestShoppingHandoffApproval(input: {
        ...
        let handoff = await this.deps.repository.prepareHandoff({
            ...
            idempotencyKey: assertNonEmptyText(input.idempotencyKey, "idempotencyKey", 300),
            content,
            now: this.now().toISOString(),
        });
        if (handoff.approvalRequestId !== null) {
            const existing = await this.deps.approvalQueue.byId(handoff.approvalRequestId, handoff.requestedByEntityId);
            if (!existing || !exactApprovalPayloadMatches(existing, handoff)) {
                throw new FoodDomainError("Persisted shopping handoff approval does not match its exact content", "FOOD_APPROVAL_MISMATCH", { handoffId: handoff.handoffId });
            }
        ...
        const approvalRequest = await this.deps.approvalQueue.enqueue({
            ...
            channel: "internal",
            reason: "Create this exact external shopping-list link for human store, product-label, cart, and checkout review.",
            idempotencyKey: `food-handoff-approval:${handoff.handoffId}`,
            expiresAt: new Date(Date.parse(handoff.createdAt) + DEFAULT_APPROVAL_TTL_MS),
        });
        handoff = await this.deps.repository.bindApproval(handoff.handoffId, approvalRequest.id, this.now().toISOString());
        return { handoff, approvalRequest, replayed: false };
    }
    private async approvedRequestFor(handoff: FoodShoppingHandoff): Promise<ApprovalRequest> {
        if (handoff.approvalRequestId === null) {
            throw new FoodDomainError("Shopping handoff has no approval request", "FOOD_APPROVAL_REQUIRED", { handoffId: handoff.handoffId });
        }
        const request = await this.deps.approvalQueue.byId(handoff.approvalRequestId, handoff.requestedByEntityId);
        if (request?.state !== "approved" ||
            !exactApprovalPayloadMatches(request, handoff)) {
            throw new FoodDomainError("Shopping handoff requires approval of its exact current content", request && !exactApprovalPayloadMatches(request, handoff)
                ? "FOOD_APPROVAL_MISMATCH"
                : "FOOD_APPROVAL_REQUIRED", { handoffId: handoff.handoffId, approvalState: request?.state });
        }
        return request;
    }
    private async reconcileCompletedApproval(handoff: FoodShoppingHandoff): Promise<void> {
        if (handoff.approvalRequestId === null) {
            throw new FoodDomainError("Completed shopping handoff is missing its approval receipt", "FOOD_APPROVAL_MISMATCH", { handoffId: handoff.handoffId });
        }
        let request = await this.deps.approvalQueue.byId(handoff.approvalRequestId, handoff.requestedByEntityId);
        if (!request || !exactApprovalPayloadMatches(request, handoff)) {
            throw new FoodDomainError("Completed shopping handoff approval does not match its content", "FOOD_APPROVAL_MISMATCH", { handoffId: handoff.handoffId });
        }
        if (request.state === "done")
            return;
        if (request.state === "approved") {
            request = await this.deps.approvalQueue.claimExecution(this.executionClaimFor(request, handoff));
        }
        ...
        throw new FoodDomainError("Completed shopping handoff has a contradictory approval state", "FOOD_APPROVAL_MISMATCH", { handoffId: handoff.handoffId, approvalState: request.state });
    }
    private executionClaimFor(request: ApprovalRequest, handoff: FoodShoppingHandoff): ApprovalExecutionClaim {
        return {
    ...
    private executionMutationFor(request: ApprovalRequest, handoff: FoodShoppingHandoff): ApprovalExecutionMutation {
        if (!request.execution) {
            throw new FoodDomainError("Executing shopping handoff approval has no execution attempt", "FOOD_APPROVAL_MISMATCH", { handoffId: handoff.handoffId, approvalRequestId: request.id });
        }
        return {
            requestId: request.id,
            subjectUserId: handoff.requestedByEntityId,
    ...
    private persistedProviderReceipt(handoff: FoodShoppingHandoff): Readonly<Record<string, unknown>> {
        if (!handoff.providerLinkUrl || !handoff.providerResultKind) {
            throw new FoodDomainError("Completed shopping handoff is missing its provider link receipt", "FOOD_APPROVAL_MISMATCH", { handoffId: handoff.handoffId });
        }
        return {
    ...
    async materializeApprovedShoppingHandoff(input: {
        ...
        if (!current) {
            throw new FoodDomainError("Shopping handoff does not exist", "FOOD_INVALID_CONTRACT", { handoffId: input.handoffId });
        }
        ...
        if (policyHash !== current.content.safetyPolicySha256 ||
            inventoryHash !== current.content.inventorySnapshotSha256) {
            await this.deps.approvalQueue.markExpired(approval.id, approval.subjectUserId);
            await this.deps.repository.blockHandoff(current.handoffId, "source_changed_after_approval", this.now().toISOString());
            throw new FoodDomainError("Safety or inventory state changed after approval", "FOOD_STALE_SOURCE", {
                handoffId: current.handoffId,
                safetyPolicyChanged: policyHash !== current.content.safetyPolicySha256,
                inventoryChanged: inventoryHash !== current.content.inventorySnapshotSha256,
            });
        }
        const instacart = this.deps.instacart;
        if (!instacart) {
            throw new FoodDomainError("Instacart products-link capability is not configured", "FOOD_PROVIDER_UNAVAILABLE", { handoffId: current.handoffId });
        }
        ...
        let receipt: Awaited<ReturnType<InstacartProductsLinkClient["createProductsLink"]>>;
        try {
            ...
            const code = error instanceof FoodDomainError
                ? error.code
                : "FOOD_PROVIDER_UNAVAILABLE";
            const definitiveFailure = code === "FOOD_INVALID_CONTRACT" || code === "FOOD_PROVIDER_REJECTED";
            ...
            if (error instanceof FoodDomainError)
                throw error;
            throw new FoodDomainError("Shopping-list provider failed after the handoff attempt was claimed", "FOOD_PROVIDER_UNAVAILABLE", { handoffId: claim.handoff.handoffId }, error);
        }
        let dispatched: ApprovalExecutionMutation;
        try {
            const claimed = await this.deps.approvalQueue.claimExecution(this.executionClaimFor(approval, claim.handoff));
            ...
            throw new FoodDomainError("Approval state changed after the provider created a shopping link", "FOOD_APPROVAL_MISMATCH", { handoffId: claim.handoff.handoffId }, error);
        }
    ...
    async getView(input: {
        ...
        if (input.principalEntityId === SELF_ENTITY_ID) {
            const [constraints, preferences, inventory, mealPlans, handoffs] = await Promise.all([
                this.deps.repository.listConstraints(input.householdId),
                this.deps.repository.listPreferences(input.householdId),
                this.deps.repository.listInventory(input.householdId),
        ...
        if (role !== "child" ||
            ...
            throw new FoodDomainError("Food detail views are owner-only; only a child-safe meal projection is available to child principals", "FOOD_ACCESS_DENIED", { principalEntityId: input.principalEntityId, role });
        }
        ...
        const childView: FoodChildView = {
            ...
            meals: plans
                .filter((plan) => plan.attendeeEntityIds.includes(input.principalEntityId))
                .map((plan) => ({
                planId: plan.planId,
...
export function createFoodDomainService(runtime: IAgentRuntime): FoodDomainService {
    const graph = resolveKnowledgeGraphService(runtime);
    if (!graph) {
        throw new FoodDomainError("KnowledgeGraphService is required for the food domain", "FOOD_INVALID_CONTRACT");
    }
    ...
    if (environmentSetting !== undefined &&
        environmentSetting !== null &&
        environmentSetting !== "" &&
        environmentSetting !== "development" &&
        environmentSetting !== "production") {
        throw new FoodDomainError("INSTACART_ENVIRONMENT must be development or production", "FOOD_INVALID_CONTRACT");
    }
    const environment = environmentSetting === "development" ? "development" : "production";
    const instacart = typeof apiKeySetting === "string" && apiKeySetting.trim().length > 0
...
export class FoodDomainRuntimeService extends Service {
    ...
    override capabilityDescription = "Constraint-safe meal, inventory, and approval-bound shopping-list coordination";
    readonly food: FoodDomainService;
    constructor(runtime?: IAgentRuntime) {
        super(runtime);
        if (!runtime) {
            throw new FoodDomainError("FoodDomainRuntimeService requires a runtime", "FOOD_INVALID_CONTRACT");
        }
        this.food = createFoodDomainService(runtime);
    }
    static async start(runtime: IAgentRuntime): Promise<FoodDomainRuntimeService> {
...
export function getFoodDomainService(runtime: IAgentRuntime): FoodDomainService {
    const runtimeService = runtime.getService<FoodDomainRuntimeService>(FOOD_DOMAIN_SERVICE);
    if (!runtimeService) {
        throw new FoodDomainError("FoodDomainRuntimeService is required for food-domain operations", "FOOD_INVALID_CONTRACT");
    }
    return runtimeService.food;
}
export { DEFAULT_APPROVAL_TTL_MS, DEFAULT_PROVIDER_LEASE_MS, FOOD_APPROVAL_WORKFLOW_ID, };
