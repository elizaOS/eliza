...
import { type MealParticipant } from "./types.js";
...
describe("food domain — real PGlite and approval queue", () => {
    ...
    function provenance(sourceId: string, sourceRevision = 1, kind: InventoryObservation["lot"]["provenance"]["kind"] = "user_confirmed"): InventoryObservation["lot"]["provenance"] {
        return {
            kind,
            sourceId,
            sourceRevision,
    ...
    function mealParticipant(entityId: string, portionServings = 1): MealParticipant {
        return {
            entityId,
            portionServings,
    ...
    function inventoryObservation(input: {
        ...
        return {
            lot: {
                ...
                allergenTags: [],
                dietaryTags: [],
                provenance: provenance(`inventory-${input.lotId}`, input.sourceRevision, input.kind),
            },
        };
    }
    beforeAll(async () => {
        providerServer = http.createServer((request, response) => {
            void (async () => {
                ...
                response.end(JSON.stringify({
                    products_link_url: "https://www.instacart.com/store/shopping-list/food-test",
                }));
            })();
        });
    ...
    it("G23 rejects child and unknown-principal inventory or purchase mutations", async () => {
        ...
        await expect(service.recordInventoryObservation({
            principalEntityId: childEntityId,
            observation,
        })).rejects.toMatchObject({ code: "FOOD_ACCESS_DENIED" });
        await expect(service.requestShoppingHandoffApproval({
    ...
    it("G40 persists idempotent observations and refuses duplicate or out-of-order source corruption", async () => {
        ...
        const winningResult = concurrent.find((result) => result.status === "fulfilled");
        ...
        expect(await restartedRepository.listInventory(householdId)).toEqual(expect.arrayContaining([
        ...
        inferred.lot.provenance = provenance("consumption-model-bananas", 50, "consumption_inference");
        ...
        confirmed.lot.provenance = provenance("owner-count-bananas", 1, "user_confirmed");
        await Promise.all([
            service.recordInventoryObservation({
                principalEntityId: SELF_ENTITY_ID,
    ...
    it("G27 binds approval and content hashes, admits one concurrent provider call, and reuses the URL", async () => {
        ...
        expect(contentCacheReplay.approvalRequest.id).toBe(first.approvalRequest.id);
        expect(first.approvalRequest.state).toBe("pending");
        await expect(service.materializeApprovedShoppingHandoff({
            principalEntityId: SELF_ENTITY_ID,
            handoffId: first.handoff.handoffId,
        })).rejects.toMatchObject({ code: "FOOD_APPROVAL_REQUIRED" });
        expect(providerRequestCount).toBe(0);
        await approvals.approve(first.approvalRequest.id, first.approvalRequest.subjectUserId, {
        ...
        expect(completed).toEqual(expect.objectContaining({
            ...
            providerLinkUrl: "https://www.instacart.com/store/shopping-list/food-test",
        ...
        expect(await approvals.byId(first.approvalRequest.id, first.approvalRequest.subjectUserId)).toMatchObject({
            state: "done",
        });
        ...
        expect(providerRequestCount).toBe(1);
        expect(JSON.stringify(await repository.listHandoffs(householdId))).not.toContain("food-integration-provider-secret");
    });
    it("invalidates approval when inventory changes after the approved snapshot", async () => {
        ...
        await approvals.approve(prepared.approvalRequest.id, prepared.approvalRequest.subjectUserId, {
            resolvedBy: SELF_ENTITY_ID,
        ...
        expect(await approvals.byId(prepared.approvalRequest.id, prepared.approvalRequest.subjectUserId)).toMatchObject({
            state: "expired",
        });
        expect(await repository.getHandoff(prepared.handoff.handoffId)).toMatchObject({
            state: "blocked",
    ...
    it("persists ambiguous state before rejecting an expired provider lease retry", async () => {
        ...
        nowMs += 1001;
        await expect(repository.claimHandoff({
            handoffId: prepared.handoff.handoffId,
            now: currentDate().toISOString(),
            leaseMs: 1000,
    ...
    it("persists a rate limit as explicitly retryable without an automatic provider replay", async () => {
        ...
        await approvals.approve(prepared.approvalRequest.id, prepared.approvalRequest.subjectUserId, {
            resolvedBy: SELF_ENTITY_ID,
            resolutionReason: "Approve this exact list despite a possible rate limit.",
        });
