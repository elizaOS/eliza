 ...
 */
import { FOOD_DOMAIN_ACTION } from "./action.js";
import { FOOD_DOMAIN_SERVICE } from "./service.js";
import { SELF_ENTITY_ID } from "@elizaos/core/knowledge-graph/entity-types";
import { afterAll } from "vitest";
import { beforeAll } from "vitest";
import { createApprovalQueue } from "../approval-queue.js";
import { createFoodDomainAction } from "./action.js";
import { createLifeOpsTestRuntime } from "../../../test/helpers/runtime.js";
import { createMessageMemory } from "@elizaos/core";
import { describe } from "vitest";
import { expect } from "vitest";
import { getFoodDomainService } from "./service.js";
...
describe("HOUSEHOLD_FOOD action — real PGlite production wiring", () => {
    ...
    async function run(subaction: string, params: Record<string, unknown>): Promise<ActionResult> {
        const result = await action.handler?.(runtime, ownerMessage(), undefined, { parameters: { action: subaction, subaction, ...params } }, undefined);
    ...
    it("registers the umbrella, its promoted virtuals, and the runtime service in production composition", () => {
        const names = personalAssistantPlugin.actions?.map((candidate) => candidate.name);
        ...
        expect(personalAssistantPlugin.services?.map((candidate) => candidate.serviceType)).toContain(FOOD_DOMAIN_SERVICE);
    ...
    it("denies unauthenticated principals with a failed effect receipt", async () => {
        ...
        const result = await denied.handler?.(runtime, ownerMessage(), undefined, { parameters: { action: "view", subaction: "view", householdId } }, undefined);
        if (!result || typeof result === "boolean") {
    ...
    it("rejects malformed planner JSON with a typed contract error", async () => {
        await expect(run("put_constraint", {
            constraint: {
                id: "bad-constraint",
                householdId,
    ...
    it("drives the full authoring surface through the production service", async () => {
        ...
        expect((constraint.data as {
            constraint: HardFoodConstraint;
        }).constraint.kind).toBe("allergen_exclusion");
        ...
        expect((preference.data as {
            preference: FoodPreference;
        }).preference
            .preferredTags).toEqual(["family_favorite"]);
        ...
        const evaluation = (evaluated.data as {
        ...
        expect((published.data as {
            plan: PublishedMealPlan;
        }).plan.attendeeEntityIds).toContain(childEntityId);
        expect(published.effectReceipts?.[0]?.outcome).toBe("applied");
        const view = await run("view", { householdId });
        expect(view.success).toBe(true);
        const ownerView = (view.data as {
            view: FoodOwnerView;
        }).view;
        expect(ownerView.constraints).toHaveLength(1);
        expect(ownerView.mealPlans.map((plan) => plan.planId)).toContain("food-action-plan-tacos");
    });
    ...
    it("queues an approval-bound shopping handoff and replays the exact same intent idempotently", async () => {
        ...
        const firstHandoff = (first.data as {
            handoff: {
        ...
        const request = await approvals.byId(approvalRequestId, firstHandoff.handoff.requestedByEntityId);
        ...
        const replayedHandoff = (replayed.data as {
            handoff: {
                handoff: FoodShoppingHandoff;
                replayed: boolean;
            };
        ...
        expect(replayedHandoff.handoff.handoffId).toBe(firstHandoff.handoff.handoffId);
