...
import { type AlmanacEntryDefinition } from "./types.js";
import { type EntityStore } from "@elizaos/plugin-relationships";
import { type HouseholdObservationInput } from "./types.js";
import { type HouseholdSourceProvenance } from "./types.js";
import { type ItemReplacementThresholdDefinition } from "./types.js";
import { type Memory } from "@elizaos/core";
import { type OpportunityDefinition } from "./types.js";
...
describe("household operations — real PGlite and runtime graph", () => {
    ...
    function provenance(sourceId: string, sourceRevision = 1, overrides?: Partial<HouseholdSourceProvenance>): HouseholdSourceProvenance {
        return {
    ...
    function responsibility(recordId: string, subjectKey: string, overrides?: Partial<ResponsibilityAssignmentDefinition>): ResponsibilityAssignmentDefinition {
        return {
    ...
    it("returns commit proof tied to the real persisted household revision", async () => {
        ...
        const persisted = await repository.getCurrentRevision("vendor_profile", recordId);
    ...
    it("G29 preserves vendor/service history and blocks outreach until an exact calendar check", async () => {
        await service.putRevision({
            ...
            definition: responsibility(maintenanceAssignmentId, maintenanceSubjectKey),
            expectedRevision: 0,
        });
        ...
        const preparationBrief = await service.generateWeeklyBrief(preparationBriefInput);
        expect(preparationBrief.replayed).toBe(false);
        expect(preparationBrief.items.some((item) => item.subjectKey === maintenanceSubjectKey)).toBe(true);
        const preparationReplay = await service.generateWeeklyBrief(preparationBriefInput);
        ...
        const blockedItem = blockedBrief.items.find((item) => item.subjectKey === maintenanceSubjectKey);
        ...
        await expect(service.generateWeeklyBrief({
            principalEntityId: SELF_ENTITY_ID,
            householdId,
            window: {
                startsAt: "2027-03-10T00:00:00.000Z",
        ...
        const readyItem = readyBrief.items.find((item) => item.subjectKey === maintenanceSubjectKey);
        ...
        expect(await repository.listServiceEvents(householdId, {
            subjectKey: maintenanceSubjectKey,
        })).toEqual([]);
        await expect(service.recordServiceEvent({
            principalEntityId: SELF_ENTITY_ID,
            event: {
        ...
        const restartedRepository = new HouseholdOperationsRepository(runtime, runtime.agentId);
        const restartedService = new HouseholdOperationsService({
            runtime,
            agentId: runtime.agentId,
            entityStore: entities,
            relationshipStore: resolveKnowledgeGraphService(runtime)?.getRelationshipStore(runtime.agentId) ??
                (() => {
        ...
        const restartedHistory = await restartedRepository.listServiceEvents(householdId, { subjectKey: maintenanceSubjectKey });
    ...
    it("keeps append-only observation corrections, detects ambiguity, and rejects concurrent provenance corruption", async () => {
        ...
        expect(await service.resolveObservation({
            principalEntityId: SELF_ENTITY_ID,
            householdId,
            subjectKey: "home:roof-state",
            observationKind: "home_state",
    ...
    it("G30 preserves child size history, scopes it through graph roles, and proposes no purchase", async () => {
        ...
        await expect(service.listChildItemSizeHistory({
        ...
        const concurrentThreshold = (minimumUsableCount: number): ItemReplacementThresholdDefinition => ({
            ...threshold,
        ...
        expect(concurrent.filter((result) => result.status === "fulfilled")).toHaveLength(1);
        expect(concurrent.filter((result) => result.status === "rejected")).toHaveLength(1);
        await expect(service.putRevision({
            principalEntityId: SELF_ENTITY_ID,
            definition: {
                ...threshold,
                itemCategory: "coats",
    ...
    it("G31/G32 keeps waitlists out of coverage and preserves explicitly unstructured child time", async () => {
        ...
        expect(await service.evaluateOpportunity({
            principalEntityId: SELF_ENTITY_ID,
        ...
        expect(await repository.listRevisionHistory("opportunity", campOpportunity.recordId)).toHaveLength(3);
    });
    it("G38 proposes multi-party renegotiation after non-use and never returns responsibility to the owner", async () => {
        ...
        await expect(service.recordResponsibilitySignal({
        ...
        expect(proposal?.requiredApproverEntityIds).toEqual(expect.arrayContaining([SELF_ENTITY_ID, partnerEntityId]));
        ...
        const assignment = await repository.getCurrentRevision("responsibility_assignment", assignmentId);
        ...
        expect(await service.assessResponsibility({
            principalEntityId: SELF_ENTITY_ID,
            assignmentRecordId: assignmentId,
        })).toBeNull();
        ...
        expect(resolvedBrief.items.some((item) => item.kind === "responsibility_review" &&
            item.subjectKey === "routine:school-morning")).toBe(false);
    });
    it("builds a maximum-three-question brief and redacts owner-private maintenance from child and unrelated views", async () => {
        ...
        expect(brief.items.some((item) => item.kind === "maintenance_due" &&
            item.subjectKey === maintenanceSubjectKey)).toBe(true);
        expect(brief.items.some((item) => item.kind === "item_replacement")).toBe(true);
        const childView = await service.readWeeklyBrief({
            principalEntityId: childEntityId,
            briefId: brief.briefId,
        });
        expect(childView.items.some((item) => item.subjectKey === maintenanceSubjectKey)).toBe(false);
        expect(childView.items.some((item) => item.kind === "item_replacement")).toBe(true);
        await expect(service.readWeeklyBrief({
            principalEntityId: outsiderEntityId,
            briefId: brief.briefId,
        })).rejects.toMatchObject({
            code: "HOUSEHOLD_OPERATIONS_ACCESS_DENIED",
    ...
    it("binds every non-owner read and visibility principal to one canonical household relationship", async () => {
        ...
        await expect(service.listVisibleObservations({
            principalEntityId: partnerEntityId,
            householdId,
