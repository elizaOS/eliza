...
export function readNtfyConfigFromEnv(env: NodeJS.ProcessEnv = process.env): NtfyConfig {
    const baseUrl = env.NTFY_BASE_URL?.trim();
    const defaultTopic = env.NTFY_DEFAULT_TOPIC?.trim();
    if (!baseUrl) {
        throw new NtfyConfigError("Ntfy push is not configured. Set NTFY_BASE_URL (and optionally NTFY_DEFAULT_TOPIC).");
    }
    return {
 ...
 */
export async function sendPush(request: SendPushRequest, config?: NtfyConfig): Promise<SendPushResult> {
    ...
    if (!response.ok) {
        ...
        logger.warn({ boundary: "lifeops", integration: "ntfy", statusCode: response.status }, `[lifeops-push] Ntfy publish HTTP error: ${errorMsg}`);
    ...
    logger.info({ boundary: "lifeops", integration: "ntfy", topic, messageId }, `[lifeops-push] Push notification delivered to topic '${topic}'`);
