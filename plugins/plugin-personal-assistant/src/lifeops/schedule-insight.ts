 ...
 */
import { LIFEOPS_CIRCADIAN_STATES } from "@elizaos/core/contracts/personal-assistant";
import { MIN_STABILITY_WINDOW_MS } from "@elizaos/plugin-health";
import { SLEEP_ONSET_WINDOW_MS } from "@elizaos/plugin-health";
import { WAKE_CONFIRM_WINDOW_MS } from "@elizaos/plugin-health";
import { computeAwakeProbability } from "@elizaos/plugin-health";
import { computePersonalBaseline } from "@elizaos/plugin-health";
import { computeSleepRegularity } from "@elizaos/plugin-health";
...
import { persistSleepEpisodes } from "@elizaos/plugin-health";
...
import { resolveLifeOpsDayBoundary } from "@elizaos/plugin-health";
import { resolveLifeOpsRelativeTime } from "./relative-time.js";
import { resolveLifeOpsSleepCycle } from "@elizaos/plugin-health";
import { roundConfidence } from "@elizaos/core/lifeops-normalize/time-util";
import { scoreCircadianRules } from "@elizaos/plugin-health";
import { type CircadianScorerResult } from "@elizaos/plugin-health";
import { type IAgentRuntime } from "@elizaos/core";
import { type LifeOpsActivitySignal } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsActivityWindow } from "@elizaos/plugin-health";
import { type LifeOpsCircadianState } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsCircadianStateRow } from "./repository.js";
...
import { type LifeOpsSleepCycle } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsSleepEpisode } from "@elizaos/plugin-health";
...
const SIGNAL_ACTIVITY_PAD_MS = 3 * 60 * 1000;
const MERGE_ACTIVITY_GAP_MS = 5 * 60 * 1000;
const MEAL_GAP_MIN_MS = 15 * 60 * 1000;
const MEAL_GAP_MAX_MS = 90 * 60 * 1000;
...
const ACTIVITY_EVENT_MAX_WINDOW_MS = 20 * 60 * 1000;
type MealCandidate = {
...
function toDurationMinutes(startMs: number, endMs: number | null, nowMs: number): number {
    return Math.round(intervalDurationMs(startMs, endMs, nowMs) / 60000);
}
...
function firstActiveAfterWake(windows: LifeOpsActivityWindow[], wakeAtMs: number | null): number | null {
    if (windows.length === 0) {
        return null;
    }
    if (wakeAtMs === null) {
    ...
    const spansWake = windows.find((window) => window.startMs < wakeAtMs && window.endMs > wakeAtMs);
    return spansWake ? wakeAtMs : null;
}
function windowsFromActivityEvents(events: Awaited<ReturnType<typeof listActivityEvents>>, nowMs: number): LifeOpsActivityWindow[] {
    if (events.length === 0) {
        return [];
    }
    ...
    for (let index = 0; index < events.length; index += 1) {
        ...
        const hasNext = index + 1 < timestamps.length &&
...
function windowsFromScreenTimeSessions(sessions: Awaited<ReturnType<LifeOpsRepository["listScreenTimeSessionsOverlapping"]>>, nowMs: number): LifeOpsActivityWindow[] {
    const windows: LifeOpsActivityWindow[] = [];
    for (const session of sessions) {
        const startMs = Date.parse(session.startAt);
        const endMs = session.endAt && Number.isFinite(Date.parse(session.endAt))
...
function windowsFromSignals(signals: LifeOpsActivitySignal[], nowMs: number): LifeOpsActivityWindow[] {
    const windows: LifeOpsActivityWindow[] = [];
...
export function mergeActivityWindows(windows: LifeOpsActivityWindow[]): LifeOpsActivityWindow[] {
    if (windows.length === 0) {
        return [];
    }
...
export function inferMealCandidates(args: {
    ...
    for (let index = 0; index < args.windows.length - 1; index += 1) {
        ...
        const minutesSinceWake = args.wakeAtMs !== null ? (midpointMs - args.wakeAtMs) / 60000 : null;
        const continuityBonus = current.endMs - current.startMs >= 10 * 60 * 1000 &&
            next.endMs - next.startMs >= 10 * 60 * 1000
            ? 0.15
            : 0;
        const durationScore = 0.2 + clamp(1 - Math.abs(durationMinutes - 35) / 45, 0, 1) * 0.25;
        ...
        const winner = (Object.entries(scores) as Array<[
...
function predictNextMeal(args: {
    ...
    const latestMealMs = latestMeal !== undefined ? Date.parse(latestMeal.detectedAt) : Number.NaN;
    const minutesSinceWake = args.wakeAtMs !== null ? (args.nowMs - args.wakeAtMs) / 60000 : null;
    const minutesSinceMeal = Number.isFinite(latestMealMs)
        ? (args.nowMs - latestMealMs) / 60000
        : null;
    const buildWindow = (label: LifeOpsScheduleMealLabel, startMs: number, endMs: number, confidence: number) => ({
        nextMealLabel: label,
    ...
    if (!mealSet.has("breakfast") &&
        args.wakeAtMs !== null &&
        minutesSinceWake !== null &&
        minutesSinceWake >= 20 &&
        minutesSinceWake <= 240) {
        return buildWindow("breakfast", Math.max(args.nowMs, args.wakeAtMs + 20 * 60000), args.wakeAtMs + 4 * 60 * 60 * 1000, 0.6);
    }
    if (!mealSet.has("lunch") &&
        ((nowHour >= 11 && nowHour < 15) ||
            (minutesSinceMeal !== null && minutesSinceMeal >= 180))) {
        return buildWindow("lunch", args.nowMs, args.nowMs + 2 * 60 * 60 * 1000, 0.55);
    }
    if (!mealSet.has("dinner") &&
        ((nowHour >= 17 && nowHour < 22) ||
            (minutesSinceMeal !== null && minutesSinceMeal >= 240))) {
        return buildWindow("dinner", args.nowMs, args.nowMs + 3 * 60 * 60 * 1000, 0.52);
    }
    return {
        nextMealLabel: null,
...
function deriveRuleState(scorer: CircadianScorerResult): {
...
export function enforceStabilityWindow(args: {
    ...
    if (args.hasManualOverride || !args.prior)
    ...
    if (from === "waking" && to === "awake")
        required = WAKE_CONFIRM_WINDOW_MS;
    if ((from === "awake" || from === "winding_down") &&
...
function decideCircadianState(args: CircadianDecisionInputs): {
    ...
    if (sleepCycle.isProbablySleeping || ap.pAsleep >= 0.65) {
        return {
            decision: {
                circadianState: ruleState?.circadianState === "sleeping"
                    ? "sleeping"
                    : sleepCycle.cycleType === "nap"
                        ? "napping"
                        : "sleeping",
                stateConfidence: roundConfidence(Math.max(sleepCycle.sleepConfidence, ap.pAsleep)),
                uncertaintyReason: null,
            },
            hasManualOverride: false,
        };
    }
    if (args.wakeAtMs !== null && args.nowMs - args.wakeAtMs <= 90 * 60 * 1000) {
        return {
            decision: {
    ...
    const recentlyActive = args.lastActiveAtMs !== null &&
        args.nowMs - args.lastActiveAtMs <= 2 * 60 * 60 * 1000;
    ...
    const uncertaintyReason: LifeOpsUnclearReason = args.signalCount === 0 && args.windowCount === 0
        ? "no_signals"
 ...
 */
function deriveCircadianState(args: CircadianDecisionInputs & {
    priorState: {
        circadianState: LifeOpsCircadianState;
        enteredAtMs: number;
    } | null;
...
function toHistoricalSleepEpisodes(episodes: readonly LifeOpsSleepEpisode[]): SleepRegularityEpisodeLike[] {
    return episodes.map((episode) => ({
        startAt: new Date(episode.startMs).toISOString(),
        endAt: toIso(episode.endMs),
        cycleType: episode.endMs !== null &&
 ...
 */
export const STALE_CIRCADIAN_STATE_MS = 6 * 60 * 60 * 1000;
export function resolvePriorStateForDerivation(row: LifeOpsCircadianStateRow | null, nowMs: number): {
    circadianState: LifeOpsCircadianState;
    enteredAtMs: number;
} | null {
    if (!row)
        return null;
    if (row.circadianState === "unclear")
        return null;
    const enteredAtMs = Date.parse(row.enteredAt);
    if (!Number.isFinite(enteredAtMs))
        return null;
    const updatedAtMs = Date.parse(row.updatedAt);
    const ageMs = Number.isFinite(updatedAtMs) && updatedAtMs > enteredAtMs
        ? nowMs - updatedAtMs
        : nowMs - enteredAtMs;
    if (ageMs >= STALE_CIRCADIAN_STATE_MS)
        return null;
...
function analyzeLifeOpsScheduleInsight(args: {
    ...
    const wakeAtMs = sleepCycle.lastSleepEndedAt !== null
    ...
    const historicalEpisodes = args.historicalSleepEpisodes ??
    ...
    const scorerResult = scoreCircadianRules({
        ...
        currentSleepStartedAtMs: sleepCycle.currentSleepStartedAt !== null
            ? Date.parse(sleepCycle.currentSleepStartedAt)
            : null,
        lastSleepEndedAtMs: wakeAtMs,
        currentEpisodeLikelyNap: sleepCycle.cycleType === "nap",
    });
    const { circadianState, stateConfidence, uncertaintyReason } = deriveCircadianState({
        nowMs: args.nowMs,
        timezone: args.timezone,
        wakeAtMs,
        lastActiveAtMs,
...
export async function inspectLifeOpsSchedule(args: {
    ...
    const [signals, sessions, activityEvents] = await Promise.all([
        ...
        args.repository.listScreenTimeSessionsOverlapping(args.agentId, sinceAt, untilAt),
...
export async function readScheduleSummary(args: {
    ...
    const [mergedState, episodes] = await Promise.all([
        args.repository.getScheduleMergedState(args.agentId, "local", args.timezone),
        args.repository.listSleepEpisodesBetween(args.agentId, sevenDaysAgoIso, now.toISOString(), { includeOpen: true, limit: 200 }),
    ]);
