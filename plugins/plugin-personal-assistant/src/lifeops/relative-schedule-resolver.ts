...
import { type LifeOpsRegularityClass } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsScheduleMergedStateRecord } from "./repository.js";
import { type LifeOpsWorkflowSchedule } from "@elizaos/core/contracts/personal-assistant";
...
function zonedWeekday(ms: number, timezone: string): number {
    return new Date(new Date(ms).toLocaleString("en-US", { timeZone: timezone })).getDay();
}
function regularitySatisfied(actual: LifeOpsRegularityClass, required: LifeOpsRegularityClass | undefined): boolean {
    if (!required) {
        return true;
    }
    return REGULARITY_RANK[actual] >= REGULARITY_RANK[required];
}
function weekdayMatches(targetMs: number, timezone: string, allowedWeekdays: number[] | undefined): boolean {
...
type RelativeScheduleKind = Extract<LifeOpsWorkflowSchedule, {
    kind: "relative_to_wake" | "relative_to_bedtime" | "during_morning" | "during_night";
}>;
function isAnchorKind(schedule: RelativeScheduleKind): schedule is Extract<LifeOpsWorkflowSchedule, {
    kind: "relative_to_wake" | "during_morning";
}> {
    return (schedule.kind === "relative_to_wake" || schedule.kind === "during_morning");
}
function offsetMinutesFor(schedule: RelativeScheduleKind): number {
    if (schedule.kind === "relative_to_wake" ||
...
export function resolveNextRelativeScheduleInstant(args: {
    ...
    if (!regularitySatisfied(state.regularity.regularityClass, args.schedule.requireRegularityAtLeast)) {
        return null;
    }
    ...
    if (args.schedule.kind === "relative_to_wake" &&
