...
import { addDaysToLocalDate } from "./time.js";
import { asRecord } from "@elizaos/core/type-guards";
import { buildUtcDateFromLocalParts } from "./time.js";
import { getLocalDateKey } from "./time.js";
...
import { type LifeOpsAwakeProbability } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsCircadianState } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsPersonalBaseline } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsScheduleDeviceKind } from "@elizaos/plugin-elizacloud/cloud/lifeops-schedule-sync-contracts";
import { type LifeOpsScheduleInsight } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsScheduleInsightRecord } from "./repository.js";
import { type LifeOpsScheduleMealInsight } from "@elizaos/core/contracts/personal-assistant";
...
import { type LifeOpsScheduleObservation } from "@elizaos/plugin-elizacloud/cloud/lifeops-schedule-sync-contracts";
import { type LifeOpsScheduleObservationOrigin } from "@elizaos/plugin-elizacloud/cloud/lifeops-schedule-sync-contracts";
import { type LifeOpsScheduleObservationSnapshot } from "@elizaos/plugin-elizacloud/cloud/lifeops-schedule-sync-contracts";
import { type LifeOpsScheduleRegularity } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsScheduleStateScope } from "@elizaos/plugin-elizacloud/cloud/lifeops-schedule-sync-contracts";
import { type LifeOpsUnclearReason } from "@elizaos/core/contracts/personal-assistant";
import { type SyncLifeOpsScheduleObservationInput } from "@elizaos/plugin-elizacloud/cloud/lifeops-schedule-sync-contracts";
...
function bucketIso(value: string | null | undefined, timezone: string, mode: BucketMode = "nearest"): string | null {
...
function snapshotUncertainty(state: LifeOpsCircadianState, reason: LifeOpsUnclearReason | null | undefined): LifeOpsUnclearReason | null {
    return state === "unclear" ? (reason ?? "no_signals") : null;
}
function toObservationSnapshot(insight: LifeOpsScheduleInsight): LifeOpsScheduleObservationSnapshot {
    return {
...
function bucketSnapshot(snapshot: LifeOpsScheduleObservationSnapshot, timezone: string): LifeOpsScheduleObservationSnapshot {
    return {
        ...snapshot,
        ...
        currentSleepStartedAt: bucketIso(snapshot.currentSleepStartedAt, timezone, "floor"),
        lastSleepStartedAt: bucketIso(snapshot.lastSleepStartedAt, timezone, "floor"),
        ...
        nextMealWindowStartAt: bucketIso(snapshot.nextMealWindowStartAt, timezone, "floor"),
        nextMealWindowEndAt: bucketIso(snapshot.nextMealWindowEndAt, timezone, "ceil"),
...
export function resolveScheduleDeviceIdentity(): ResolvedScheduleDeviceIdentity {
    const envDeviceId = process.env.ELIZA_DEVICE_ID?.trim() ?? process.env.HOSTNAME?.trim();
    const deviceId = envDeviceId && envDeviceId.length > 0
        ? envDeviceId
        : `${process.platform}-${crypto.createHash("sha256").update(process.cwd()).digest("hex").slice(0, 8)}`;
    const envDeviceKind = process.env.ELIZA_DEVICE_KIND?.trim().toLowerCase() ?? "";
    if (envDeviceKind === "iphone" ||
        envDeviceKind === "ipad" ||
        envDeviceKind === "mac" ||
        envDeviceKind === "watch" ||
        envDeviceKind === "cloud") {
...
export function deriveLocalScheduleObservations(args: {
    ...
    const snapshot = bucketSnapshot(toObservationSnapshot(args.insight), args.timezone);
    ...
    if (snapshot.nextMealLabel &&
        ...
        observations.push(buildObservationRecord({
            ...
            windowEndAt: snapshot.nextMealWindowEndAt ?? snapshot.nextMealWindowStartAt,
            metadata: observationMetadata({
                snapshot,
                source: "schedule_insight",
                extra: { meal: true },
...
function recordFromSyncInput(args: {
    ...
    const bucketedWindowStartAt = bucketIso(args.input.windowStartAt, args.timezone, "floor") ??
        args.input.windowStartAt;
    const bucketedWindowEndAt = bucketIso(args.input.windowEndAt ?? null, args.timezone, "ceil");
    const circadianState = args.input.circadianState;
    const uncertaintyReason = snapshotUncertainty(circadianState, args.input.uncertaintyReason);
    const snapshotBase = {
        effectiveDayKey: typeof snapshotSource.effectiveDayKey === "string"
            ? snapshotSource.effectiveDayKey
            : getLocalDateKey(getZonedDateParts(new Date(args.observedAt), args.timezone)),
        localDate: typeof snapshotSource.localDate === "string"
            ? snapshotSource.localDate
            : getLocalDateKey(getZonedDateParts(new Date(args.observedAt), args.timezone)),
        phase: circadianState,
        circadianState,
        stateConfidence: roundConfidence(snapshotSource.stateConfidence ?? args.input.stateConfidence),
        uncertaintyReason,
        awakeProbability: snapshotSource.awakeProbability ??
        ...
        sleepConfidence: roundConfidence(snapshotSource.sleepConfidence ?? args.input.stateConfidence),
        currentSleepStartedAt: bucketIso(snapshotSource.currentSleepStartedAt, args.timezone, "floor") ??
            (isAsleepState(circadianState)
                ? bucketIso(args.input.windowStartAt, args.timezone, "floor")
                : null),
        lastSleepStartedAt: bucketIso(snapshotSource.lastSleepStartedAt, args.timezone, "floor"),
        lastSleepEndedAt: bucketIso(snapshotSource.lastSleepEndedAt, args.timezone, "nearest"),
        lastSleepDurationMinutes: normalizeDurationMinutes(snapshotSource.lastSleepDurationMinutes ?? null),
        wakeAt: bucketIso(snapshotSource.wakeAt, args.timezone, "nearest") ??
            (circadianState === "waking"
                ? bucketIso(args.input.windowStartAt, args.timezone, "nearest")
                : null),
        firstActiveAt: bucketIso(snapshotSource.firstActiveAt, args.timezone, "nearest"),
        lastActiveAt: bucketIso(snapshotSource.lastActiveAt, args.timezone, "nearest") ??
            (circadianState === "awake"
                ? bucketIso(args.input.windowStartAt, args.timezone, "nearest")
                : null),
        ...
        nextMealWindowStartAt: bucketIso(snapshotSource.nextMealWindowStartAt, args.timezone, "floor") ??
            (args.input.mealLabel ? bucketedWindowStartAt : null),
        nextMealWindowEndAt: bucketIso(snapshotSource.nextMealWindowEndAt, args.timezone, "ceil") ??
            (args.input.mealLabel ? bucketedWindowEndAt : null),
        nextMealConfidence: roundConfidence(snapshotSource.nextMealConfidence ??
...
export function recordsFromSyncRequest(args: {
    ...
    return args.request.observations.map((input) => recordFromSyncInput({
        agentId: args.agentId,
...
function observationSnapshot(observation: LifeOpsScheduleObservation): MergeObservationSnapshot | null {
...
function observationRelevant(observation: LifeOpsScheduleObservation, nowMs: number): boolean {
...
function latestSnapshotValue<T>(observations: LifeOpsScheduleObservation[], read: (snapshot: MergeObservationSnapshot) => T | null | undefined): T | null {
...
function isFutureIsoAt(value: string | null | undefined, nowMs: number): boolean {
    const parsed = parseIsoMs(value);
    return parsed !== null && parsed >= nowMs;
}
function pickFutureSnapshotValue(observations: LifeOpsScheduleObservation[], read: (snapshot: MergeObservationSnapshot) => string | null | undefined, nowMs: number): string | null {
    for (const observation of observations) {
        const snapshot = observationSnapshot(observation);
        const value = snapshot ? read(snapshot) : null;
...
function latestRelevantObservations(observations: LifeOpsScheduleObservation[], nowMs: number): LifeOpsScheduleObservation[] {
    return observations
...
function bestObservation(observations: LifeOpsScheduleObservation[], predicate: (observation: LifeOpsScheduleObservation) => boolean): LifeOpsScheduleObservation | null {
...
function mergedMeals(observations: LifeOpsScheduleObservation[]): LifeOpsScheduleMealInsight[] {
...
function resolveMergedCircadianState(relevant: LifeOpsScheduleObservation[]): {
    ...
    const candidates = relevant.filter((observation) => observation.circadianState !== "unclear");
    if (candidates.length === 0) {
        const fallback = relevant[0];
        return {
            circadianState: fallback?.circadianState,
            stateConfidence: fallback?.stateConfidence,
            uncertaintyReason: fallback?.uncertaintyReason ??
                (relevant.length === 0 ? "no_signals" : "contradictory_signals"),
        };
    }
    const [best] = candidates.sort((left, right) => {
        const rankDelta = STATE_RANK[right.circadianState] - STATE_RANK[left.circadianState];
        if (rankDelta !== 0) {
            return rankDelta;
        }
        if (right.stateConfidence !== left.stateConfidence) {
...
export function mergeScheduleObservations(args: {
    ...
    const { circadianState, stateConfidence, uncertaintyReason } = resolveMergedCircadianState(relevant);
    const currentSleep = bestObservation(relevant, (observation) => isAsleepState(observation.circadianState));
    const recentWake = bestObservation(relevant, (observation) => observation.circadianState === "waking");
    const mealWindow = bestObservation(relevant, (observation) => observation.mealLabel !== null);
    const currentSleepStartedAt = latestSnapshotValue(relevant, (snapshot) => snapshot.currentSleepStartedAt) ??
        currentSleep?.windowStartAt ??
        null;
    const lastSleepStartedAt = latestSnapshotValue(relevant, (snapshot) => snapshot.lastSleepStartedAt) ??
        currentSleepStartedAt;
    const lastSleepEndedAt = latestSnapshotValue(relevant, (snapshot) => snapshot.lastSleepEndedAt) ??
        null;
    const wakeAt = latestSnapshotValue(relevant, (snapshot) => snapshot.wakeAt) ??
        recentWake?.windowStartAt ??
        null;
    const firstActiveAt = latestSnapshotValue(relevant, (snapshot) => snapshot.firstActiveAt) ??
        wakeAt;
    const lastActiveAt = latestSnapshotValue(relevant, (snapshot) => snapshot.lastActiveAt) ??
        bestObservation(relevant, (observation) => observation.circadianState === "awake")?.windowStartAt ??
        null;
    const sleepStatus = isAsleepState(circadianState)
        ...
        : lastSleepEndedAt
            ...
            : stateConfidence >= 0.55
                ? "likely_missed"
                : "unknown";
    const sleepConfidence = roundConfidence(currentSleep?.stateConfidence ??
    ...
    const effectiveDayKey = latestSnapshotValue(relevant, (snapshot) => snapshot.effectiveDayKey) ??
        getLocalDateKey(getZonedDateParts(now, args.timezone));
    const localDate = latestSnapshotValue(relevant, (snapshot) => snapshot.localDate) ??
        getLocalDateKey(getZonedDateParts(now, args.timezone));
    const awakeProbability = latestSnapshotValue(relevant, (snapshot) => snapshot.awakeProbability) ??
        defaultAwakeProbability(mergedAt);
    const regularity = latestSnapshotValue(relevant, (snapshot) => snapshot.regularity) ??
        defaultScheduleRegularity();
    const baseline = latestSnapshotValue<LifeOpsPersonalBaseline | null>(relevant, (snapshot) => snapshot.baseline ?? null);
    ...
    const mealWindowStartFromObservation = mealWindow && isFutureIsoAt(mealWindow.windowStartAt, nowMs)
        ? mealWindow.windowStartAt
        : null;
    const mealWindowStartFromSnapshot = mealWindowStartFromObservation === null
        ? pickFutureSnapshotValue(relevant, (snapshot) => snapshot.nextMealWindowStartAt, nowMs)
        : null;
    const mealWindowSource: "observation" | "snapshot" | null = mealWindowStartFromObservation !== null
        ? "observation"
        : mealWindowStartFromSnapshot !== null
            ? "snapshot"
            : null;
    const nextMealWindowStartAt = mealWindowStartFromObservation ?? mealWindowStartFromSnapshot ?? null;
    const nextMealLabel = mealWindowSource === "observation"
        ? (mealWindow?.mealLabel ?? null)
        : mealWindowSource === "snapshot"
            ? (latestSnapshotValue(relevant, (snapshot) => snapshot.nextMealLabel) ?? null)
            : null;
    const nextMealWindowEndAt = mealWindowSource === "observation"
        ? (mealWindow?.windowEndAt ?? null)
        : mealWindowSource === "snapshot"
            ? (latestSnapshotValue(relevant, (snapshot) => snapshot.nextMealWindowEndAt) ?? null)
            : null;
    const nextMealConfidence = roundConfidence(mealWindowSource === "observation"
        ? (mealWindow?.stateConfidence ?? 0)
        : mealWindowSource === "snapshot"
            ? (latestSnapshotValue(relevant, (snapshot) => snapshot.nextMealConfidence) ?? 0)
            : 0);
    ...
    return {
        ...
        lastSleepStartedAt,
        lastSleepEndedAt,
        lastSleepDurationMinutes: latestSnapshotValue(relevant, (snapshot) => snapshot.lastSleepDurationMinutes) ?? null,
        wakeAt,
        firstActiveAt,
        lastActiveAt,
        meals,
...
export function isFreshCloudMergedState(state: LifeOpsScheduleMergedState | null | undefined, now: Date): boolean {
