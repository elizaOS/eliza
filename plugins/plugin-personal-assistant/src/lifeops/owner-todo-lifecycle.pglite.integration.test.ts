...
it("preserves one undated definition through owner completion and reopening with matching receipts", async () => {
  ...
  try {
    ...
    const completed = await dispatch({ action: "complete", target: id });
    ...
    expect((await service.getDefinition(id)).definition.status).toBe("active");
