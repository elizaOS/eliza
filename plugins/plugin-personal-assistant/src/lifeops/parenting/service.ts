...
import { HOUSEHOLD_ACCESS_SCOPES } from "../household/index.js";
import { HouseholdCoordinationError } from "../household/index.js";
import { PARENTING_AGE_BANDS } from "./types.js";
import { PARENTING_GUIDANCE_VERSION } from "./types.js";
import { PARENTING_TOPICS } from "./types.js";
...
import { getParentingHandoffResourceResolver } from "./resources.js";
import { getParentingSafetyClassifier } from "./safety-classifier.js";
import { graphBackedParentingSubjectLocationResolver } from "./subject-location.js";
...
import { type HouseholdRole } from "../household/index.js";
import { type HouseholdRoleBinding } from "../household/index.js";
import { type IAgentRuntime } from "@elizaos/core";
import { type Memory } from "@elizaos/core";
import { type ParentingAgeBand } from "./types.js";
import { type ParentingGuidanceDecision } from "./types.js";
import { type ParentingGuidanceRequest } from "./types.js";
import { type ParentingHandoffContact } from "./resources.js";
import { type ParentingHandoffResourceResolution } from "./resources.js";
import { type ParentingLocaleEvidence } from "./subject-location.js";
import { type ParentingRequesterRole } from "./types.js";
import { type ParentingSafetyClassificationStatus } from "./safety-classifier.js";
import { type ParentingTopic } from "./types.js";
export const PARENTING_GUIDANCE_SERVICE = "lifeops_parenting_guidance";
export const PARENTING_AGE_BAND_ATTRIBUTE = "lifeops.parenting.ageBand" as const;
export const PARENTING_RECORD_SCOPE_ATTRIBUTE = "lifeops.parenting.recordScope" as const;
export type ParentingGuidanceErrorCode = "PARENTING_GUIDANCE_ACCESS_DENIED" | "PARENTING_GUIDANCE_CONTEXT_INCOMPLETE" | "PARENTING_GUIDANCE_GRAPH_UNAVAILABLE" | "PARENTING_GUIDANCE_PERSISTED_DATA_INVALID";
export class ParentingGuidanceError extends ElizaError {
    override readonly name = "ParentingGuidanceError";
    constructor(message: string, code: ParentingGuidanceErrorCode, context?: Record<string, unknown>, cause?: unknown) {
        super(message, {
            code,
            context,
            cause,
            severity: code === "PARENTING_GUIDANCE_GRAPH_UNAVAILABLE" ||
...
export interface ParentingGuidanceServiceDependencies {
    ...
    readonly now?: () => Date;
    readonly resolveAuthenticatedPrincipal?: (runtime: IAgentRuntime, message: Memory) => Promise<string | null>;
    readonly resolveHousehold?: (runtime: IAgentRuntime) => HouseholdCoordinationService;
...
function requiredText(value: unknown, field: string, maximum?: number): string {
    if (typeof value !== "string" || !value.trim()) {
        throw new ParentingGuidanceError(`${field} is required`, "PARENTING_GUIDANCE_CONTEXT_INCOMPLETE", { field });
    }
    const normalized = value.trim();
    if (maximum !== undefined && normalized.length > maximum) {
        throw new ParentingGuidanceError(`${field} exceeds ${maximum} characters`, "PARENTING_GUIDANCE_CONTEXT_INCOMPLETE", { field, maximum });
    }
    return normalized;
}
function requesterRole(principalEntityId: string, binding: HouseholdRoleBinding | undefined, subjectEntityId: string): ParentingRequesterRole {
    if (principalEntityId === SELF_ENTITY_ID)
        return "owner";
    if (!binding) {
        throw new ParentingGuidanceError("The authenticated principal has no household role", "PARENTING_GUIDANCE_ACCESS_DENIED", { principalEntityId });
    }
    ...
    if (binding.role === "child" && principalEntityId !== subjectEntityId) {
        throw new ParentingGuidanceError("A child principal may request guidance only for their own subject record", "PARENTING_GUIDANCE_ACCESS_DENIED", { principalEntityId, subjectEntityId });
    }
...
function trustedAttribute(entity: Entity, key: string): unknown | undefined {
    ...
    if (attribute.confidence !== 1 ||
        ...
        throw new ParentingGuidanceError(`Entity ${entity.entityId} has untrusted ${key} metadata`, "PARENTING_GUIDANCE_PERSISTED_DATA_INVALID", { entityId: entity.entityId, key });
    }
    return attribute.value;
}
function ageBandFrom(entity: Entity, requested: ParentingAgeBand | undefined): ParentingAgeBand {
    ...
    if (stored !== undefined) {
        throw new ParentingGuidanceError(`Entity ${entity.entityId} has an invalid parenting age band`, "PARENTING_GUIDANCE_PERSISTED_DATA_INVALID", { entityId: entity.entityId });
    }
    if (requested && PARENTING_AGE_BANDS.includes(requested))
        return requested;
    throw new ParentingGuidanceError("The child's developmental age band is needed before source-grounded guidance can be selected", "PARENTING_GUIDANCE_CONTEXT_INCOMPLETE", { subjectEntityId: entity.entityId });
}
function recordScopeFrom(entity: Entity): "household_shared" | "adult_private" | "teen_private" {
    const stored = trustedAttribute(entity, PARENTING_RECORD_SCOPE_ATTRIBUTE);
    if (stored === undefined)
        return "household_shared";
    if (stored === "household_shared" ||
        stored === "adult_private" ||
        stored === "teen_private") {
        return stored;
    }
    throw new ParentingGuidanceError(`Entity ${entity.entityId} has an invalid parenting record scope`, "PARENTING_GUIDANCE_PERSISTED_DATA_INVALID", { entityId: entity.entityId });
}
function candidateSubjectId(input: {
    ...
    const childBindings = input.bindings.filter((binding) => binding.role === "child");
    const principalChild = childBindings.find((binding) => binding.entityId === input.principalEntityId);
    if (principalChild)
        return principalChild.entityId;
    const principalBinding = input.bindings.find((binding) => binding.entityId === input.principalEntityId);
    const accessibleChildIds = input.principalEntityId === SELF_ENTITY_ID
        ? childBindings.map((binding) => binding.entityId)
        : childBindings
            .filter((binding) => principalBinding?.subjectEntityIds.includes(binding.entityId))
            .map((binding) => binding.entityId);
    if (accessibleChildIds.length === 1 && accessibleChildIds[0]) {
        return accessibleChildIds[0];
    }
    throw new ParentingGuidanceError(accessibleChildIds.length === 0
        ? "No graph-backed child subject is available for this request"
        : "The request could refer to more than one child; select the intended child before guidance is generated", "PARENTING_GUIDANCE_CONTEXT_INCOMPLETE", { candidateCount: accessibleChildIds.length });
}
function validateTopic(topic: ParentingTopic): ParentingTopic {
    if (!PARENTING_TOPICS.includes(topic)) {
        throw new ParentingGuidanceError("A supported parenting topic is required", "PARENTING_GUIDANCE_CONTEXT_INCOMPLETE", { topic });
    }
    return topic;
}
function validateFramework(framework: "none" | "good_inside" | undefined): "none" | "good_inside" {
    if (framework === undefined)
        return "none";
    if (framework === "none" || framework === "good_inside")
        return framework;
    throw new ParentingGuidanceError("The requested parenting framework is unsupported", "PARENTING_GUIDANCE_CONTEXT_INCOMPLETE", { framework });
}
function guidanceUncertainty(decision: ParentingGuidanceDecision, resources: ParentingHandoffResourceResolution): string {
...
function humanNextStep(decision: ParentingGuidanceDecision, resources: ParentingHandoffResourceResolution): string {
    const locationUnavailable = decision.handoff !== null && resources.status === "unavailable";
...
export function renderParentingGuidanceDelivery(delivery: ParentingGuidanceDelivery): string {
    const lines: string[] = [];
    if (delivery.decision.status === "educational_options") {
        lines.push("Here are source-grounded educational options, not a diagnosis or therapy:");
        ...
        for (const reason of delivery.decision.reasons)
            lines.push(`- ${reason}`);
    ...
    if (delivery.handoffResources.status === "resolved") {
        lines.push("Verified handoff resources for the child's current jurisdiction:");
    ...
    else if (delivery.handoffResources.requestedKinds.length > 0 &&
        delivery.handoffResources.status === "unavailable") {
        lines.push(`Current-location-specific contacts are unavailable (${delivery.handoffResources.unavailableReason}); no contact was guessed from the owner's profile or travel location.`);
    }
...
export class ParentingGuidanceService {
    ...
    private readonly authenticate: NonNullable<ParentingGuidanceServiceDependencies["resolveAuthenticatedPrincipal"]>;
    private readonly resolveHousehold: NonNullable<ParentingGuidanceServiceDependencies["resolveHousehold"]>;
    ...
    constructor(private readonly deps: ParentingGuidanceServiceDependencies) {
        ...
        this.resolveHousehold =
            deps.resolveHousehold ??
                ((runtime) => getHouseholdCoordinationService(runtime) ??
    ...
    private async subjectContext(input: {
        ...
        if (!graph) {
            throw new ParentingGuidanceError("KnowledgeGraphService is required for parenting guidance", "PARENTING_GUIDANCE_GRAPH_UNAVAILABLE", { agentId: this.deps.runtime.agentId });
        }
        ...
        const childBinding = bindings.find((binding) => binding.entityId === subjectEntityId && binding.role === "child");
        if (!childBinding) {
            throw new ParentingGuidanceError("The selected subject is not a graph-backed household child", "PARENTING_GUIDANCE_ACCESS_DENIED", { subjectEntityId });
        }
        if (input.principalEntityId !== SELF_ENTITY_ID) {
            try {
                ...
                if (error instanceof HouseholdCoordinationError &&
                    ...
                    throw new ParentingGuidanceError("The authenticated principal lacks an active household visibility grant for this child", "PARENTING_GUIDANCE_ACCESS_DENIED", {
                        principalEntityId: input.principalEntityId,
                        subjectEntityId,
                    }, error);
                }
        ...
        const entity = await graph
            ...
            .get(subjectEntityId);
        if (!entity) {
            throw new ParentingGuidanceError("The selected child entity is unavailable", "PARENTING_GUIDANCE_PERSISTED_DATA_INVALID", { subjectEntityId });
        }
        const exported = await household.exportFor({
            principalEntityId: input.principalEntityId,
        });
        const principalBinding = bindings.find((binding) => binding.entityId === input.principalEntityId);
        return {
            subjectEntityId,
            ageBand: ageBandFrom(entity, input.requestedAgeBand),
            requesterRole: requesterRole(input.principalEntityId, principalBinding, subjectEntityId),
            grantedScopes: input.principalEntityId === SELF_ENTITY_ID
                ? [...HOUSEHOLD_ACCESS_SCOPES]
                : exported.effectiveScopes,
            recordScope: recordScopeFrom(entity),
        };
    }
    async advise(input: ParentingGuidanceConversationInput): Promise<ParentingGuidanceDelivery> {
        const text = requiredText(input.message.content.text, "message.content.text");
        const topic = validateTopic(input.topic);
        const requestedFramework = validateFramework(input.requestedFramework);
        const principalEntityId = await this.authenticate(this.deps.runtime, input.message);
        if (!principalEntityId) {
            throw new ParentingGuidanceError("A verified runtime or household principal is required", "PARENTING_GUIDANCE_ACCESS_DENIED");
        }
        ...
        const request: ParentingGuidanceRequest = {
            ...
            requester: {
                ...
                identityAssurance: principalEntityId === SELF_ENTITY_ID
                    ? "runtime_owner"
                    : "connector_verified",
        ...
        const handoffResources = await getParentingHandoffResourceResolver(this.deps.runtime).resolve({
...
export class ParentingGuidanceRuntimeService extends Service {
    ...
    override capabilityDescription = "Authenticated, source-grounded parenting education with structural safety/privacy gates and subject-location-bound human handoffs";
    readonly guidance: ParentingGuidanceService;
    constructor(runtime?: IAgentRuntime) {
        super(runtime);
        if (!runtime) {
            throw new ParentingGuidanceError("ParentingGuidanceRuntimeService requires a runtime", "PARENTING_GUIDANCE_GRAPH_UNAVAILABLE");
        }
        this.guidance = new ParentingGuidanceService({ runtime });
    }
    static async start(runtime: IAgentRuntime): Promise<ParentingGuidanceRuntimeService> {
        return new ParentingGuidanceRuntimeService(runtime);
    }
    async stop(): Promise<void> { }
    advise(input: ParentingGuidanceConversationInput): Promise<ParentingGuidanceDelivery> {
        return this.guidance.advise(input);
    }
}
export function getParentingGuidanceService(runtime: IAgentRuntime): ParentingGuidanceRuntimeService | null {
    return runtime.getService<ParentingGuidanceRuntimeService>(PARENTING_GUIDANCE_SERVICE);
}
