...
export const PARENTING_CURRENT_LOCATION_ATTRIBUTE = "lifeops.parenting.currentLocation" as const;
...
export type ParentingSubjectLocationSource = (typeof PARENTING_SUBJECT_LOCATION_SOURCES)[number];
...
export type ParentingSubjectLocationEvidence = {
    ...
    readonly unavailableReason: "location_missing" | "location_untrusted" | "location_stale";
};
...
function locationSource(value: unknown): ParentingSubjectLocationSource | null {
    if (typeof value === "string" &&
        PARENTING_SUBJECT_LOCATION_SOURCES.includes(value as ParentingSubjectLocationSource)) {
...
function unavailable(subjectEntityId: string, candidate: LocationCandidate, unavailableReason: "location_missing" | "location_untrusted" | "location_stale"): ParentingSubjectLocationEvidence {
    return {
        status: "unavailable",
        subjectEntityId,
        ...candidate,
...
function recordFrom(value: unknown): ParentingSubjectLocationRecord | null {
    ...
    const verificationEvidenceId = requiredString(value.verificationEvidenceId, 500);
    if (value.schemaVersion !== PARENTING_SUBJECT_LOCATION_VERSION ||
 ...
 * parameter.
 */
export function createParentingSubjectLocationAttribute(assertion: ParentingSubjectLocationAssertion, recordedAt: string = assertion.observedAt): EntityAttribute {
    return {
        value: {
            schemaVersion: PARENTING_SUBJECT_LOCATION_VERSION,
            assurance: "subject_current_location_verified",
...
export function evaluateParentingSubjectLocationEvidence(input: {
    ...
    const attribute = input.entity?.attributes?.[PARENTING_CURRENT_LOCATION_ATTRIBUTE];
    if (!attribute) {
        return unavailable(input.subjectEntityId, candidateFrom(null), "location_missing");
    }
...
async function verifierAuthorized(input: {
    ...
    if (!input.verifierEntityId || !input.verificationSource)
        return false;
    ...
    const verifierBinding = bindings.find((binding) => binding.entityId === input.verifierEntityId);
    if (verifierBinding?.role === "child" &&
    ...
    const sourceMatchesRole = input.verificationSource === "caregiver_presence_confirmation"
...
export class GraphBackedParentingSubjectLocationResolver implements ParentingSubjectLocationResolver {
    constructor(private readonly resolveHousehold: (runtime: IAgentRuntime) => HouseholdCoordinationService = (runtime) => getHouseholdCoordinationService(runtime) ??
    ...
    async resolve(input: {
        ...
        if (!graph) {
            throw new ElizaError("KnowledgeGraphService is required for subject location resolution", {
        ...
        const candidate = candidateFrom(entity?.attributes?.[PARENTING_CURRENT_LOCATION_ATTRIBUTE]?.value);
...
export const graphBackedParentingSubjectLocationResolver = new GraphBackedParentingSubjectLocationResolver();
