 ...
 */
import { PARENTING_AGE_BAND_ATTRIBUTE } from "./service.js";
import { PARENTING_CURRENT_LOCATION_ATTRIBUTE } from "./subject-location.js";
import { PARENTING_GUIDANCE_ACTION } from "./action.js";
import { PARENTING_GUIDANCE_SERVICE } from "./service.js";
import { PARENTING_RECORD_SCOPE_ATTRIBUTE } from "./service.js";
import { ParentingGuidanceService } from "./service.js";
import { SELF_ENTITY_ID } from "@elizaos/core/knowledge-graph/entity-types";
import { afterAll } from "vitest";
import { beforeAll } from "vitest";
import { createHouseholdCoordinationService } from "../household/index.js";
import { createLifeOpsTestRuntime } from "../../../test/helpers/runtime.js";
import { createMessageMemory } from "@elizaos/core";
import { createParentingGuidanceAction } from "./action.js";
import { createParentingSubjectLocationAttribute } from "./subject-location.js";
import { describe } from "vitest";
import { expect } from "vitest";
import { getHouseholdCoordinationService } from "../household/index.js";
import { getParentingGuidanceService } from "./service.js";
import { it } from "vitest";
import { personalAssistantPlugin } from "../../plugin.js";
import { randomUUID } from "node:crypto";
import { registerParentingSafetyClassifier } from "./safety-classifier.js";
...
import { type HouseholdCoordinationService } from "../household/index.js";
import { type Memory } from "@elizaos/core";
import { type OwnerFactProvenanceSource } from "../owner/fact-store.js";
import { type ParentingGuidanceDelivery } from "./service.js";
import { type ParentingRiskSignal } from "./types.js";
import { type ParentingSafetyAssessment } from "./types.js";
import { type ParentingSafetyClassification } from "./safety-classifier.js";
import { type ParentingSafetyClassifier } from "./safety-classifier.js";
import { type ParentingSubjectLocationSource } from "./subject-location.js";
import { type RealTestRuntimeResult } from "../../../test/helpers/runtime.js";
import { type UUID } from "@elizaos/core";
type RiskOverrides = Partial<Pick<ParentingSafetyAssessment, "immediateDanger" | "selfHarm" | "harmToOthers" | "suspectedAbuseOrNeglect" | "medicationOrDiagnosis" | "severeOrPersistentSymptoms" | "legalOrCustodyInterpretation">>;
function assessment(assessedAt: string, overrides: RiskOverrides = {}): ParentingSafetyAssessment {
...
describe("parenting guidance production wiring — real PGlite", () => {
    ...
    async function putChild(input: {
        ...
        await graph.getEntityStore(runtime.agentId).upsert({
            ...
            attributes: {
                ...
                [PARENTING_CURRENT_LOCATION_ATTRIBUTE]: createParentingSubjectLocationAttribute({
    ...
    async function setLocale(locale: string, options: {
        source?: OwnerFactProvenanceSource;
        recordedAt?: string;
    } = {}): Promise<void> {
        await resolveOwnerFactStore(runtime).update({ locale }, {
            source: options.source ?? "profile_save",
            recordedAt: options.recordedAt ?? new Date().toISOString(),
            note: "Owner confirmed locale for parenting handoff tests.",
        });
    }
    async function setSubjectLocation(subjectEntityId: string, options: {
        locale?: string;
        jurisdiction?: string;
        observedAt?: string;
        expiresAt?: string;
        ...
        if (!entity)
            throw new Error(`subject ${subjectEntityId} unavailable`);
        const attributes = { ...entity.attributes };
        if (options.remove) {
            delete attributes[PARENTING_CURRENT_LOCATION_ATTRIBUTE];
        }
        else {
            const observedAt = options.observedAt ?? new Date(Date.now() - 60 * 1000).toISOString();
    ...
    function useRisk(overrides: RiskOverrides): void {
        registerParentingSafetyClassifier(runtime, new StaticSafetyClassifier(overrides));
    }
    async function run(input: {
        text: string;
        childId?: string;
        topic?: "boundary_setting" | "routines" | "communication" | "emotion_coaching" | "independence" | "positive_discipline";
        ...
        const result = await action.handler(runtime, ownerMessage(input.text), undefined, {
            parameters: {
        ...
        if (!result)
            throw new Error("parenting action returned no result");
        const guidance = (result.data as {
            guidance?: ParentingGuidanceDelivery;
        } | undefined)?.guidance;
        if (!guidance)
            throw new Error("guidance artifact missing");
    ...
    beforeAll(async () => {
        ...
        await household.bindRole({
            ...
            subjectEntityIds: [schoolAgeChildId],
            boundByEntityId: SELF_ENTITY_ID,
            evidence: "Owner confirmed the co-parent relationship for subject-location tests.",
        });
    ...
    it("registers the conversational action and runtime service in production composition", () => {
        expect(personalAssistantPlugin.actions?.map((candidate) => candidate.name)).toContain(PARENTING_GUIDANCE_ACTION);
        expect(personalAssistantPlugin.services?.map((candidate) => candidate.serviceType)).toContain(PARENTING_GUIDANCE_SERVICE);
        ...
        expect(action.examples
            ?.flat()
            .map((example) => example.content.text)
            .join(" ")).toMatch(/turned into a whole thing|can’t tell whether they meant it/u);
    });
    it("returns cited ordinary options and a concrete human next step from vague language", async () => {
        ...
        expect(guidance.decision.sources.some((source) => source.evidenceTier === "named_framework_primary")).toBe(true);
    ...
    it("denies an ungranted household principal before child prose reaches the model classifier", async () => {
        ...
        await expect(deniedService.advise({
            message: ownerMessage("Private child-related prose must not cross the model boundary."),
    ...
    it("denies an otherwise authorized co-parent access to another child", async () => {
        ...
        await expect(childScopedService.advise({
            message: ownerMessage("This request is for a different child than the active grant."),
    ...
    it("resolves exact US emergency and crisis resources for urgent risk", async () => {
        ...
        expect(guidance.handoffResources.resources.flatMap((resource) => resource.contacts)).toEqual(expect.arrayContaining([
    ...
    ] as const)("routes $label language to its reviewed human boundary", async ({ risk, status, kind }) => {
        await setSubjectLocation(schoolAgeChildId);
        useRisk(risk);
        const { guidance } = await run({
    ...
    it("returns no guessed contacts when subject location is missing or unsupported", async () => {
        ...
        expect(JSON.stringify(unsupported.guidance.handoffResources)).not.toContain("911");
        await setSubjectLocation(schoolAgeChildId);
    });
    ...
    it("fails closed for stale, cross-child, or cross-tenant location assertions", async () => {
        ...
        expect(crossTenant.result.text).toMatch(/no contact was guessed from the owner's profile or travel location/u);
        await setSubjectLocation(schoolAgeChildId);
        await setSubjectLocation(teenChildId);
    });
    it("stops guidance without inventing a clinical handoff after source review expiry", async () => {
        ...
        expect(guidance.decision.options).toEqual([]);
        expect(guidance.decision.reasons.join(" ")).toMatch(/past their required review date/u);
