 ...
 * the X read domain. The composed service exposes all five methods, so they
 * are injected as typed callbacks rather than read off {@link LifeOpsContext}.
 */
...
export class InboxDomain {
  ...
  async getInbox(request: GetLifeOpsInboxRequest = {}): Promise<LifeOpsInbox> {
    const resolved = resolveInboxRequest(request);
    const ownerName = resolveOwnerName(this.ctx.runtime);
    ...
    const cached = await this.ctx.repository.listCachedInboxMessages(
      this.ctx.runtime.agentId,
      {
    ...
    const inbound = await fetchAllMessages(this.ctx.runtime, {
      sources: Array.from(resolved.allowed),
      limit: cacheWarmLimitFor(resolved),
      includeGmail: resolved.allowed.has("gmail"),
      gmailSource: this.deps,
      xDmSource: this.deps,
      gmailGrantId: resolved.gmailAccountId,
    });
    await this.ctx.repository.upsertCachedInboxMessages(
      this.ctx.runtime.agentId,
      toInboxMessages(inbound),
    );
    const inbox = await buildInboxWithLlm(this.ctx.runtime, inbound, resolved);
    await this.ctx.repository.upsertCachedInboxMessages(
      this.ctx.runtime.agentId,
  ...
  async markInboxEntryRead(inboxEntryId: string): Promise<LifeOpsInboxMessage> {
    ...
    const message = await this.ctx.repository.markCachedInboxMessageRead(
      this.ctx.runtime.agentId,
      id,
    );
    if (!message) {
