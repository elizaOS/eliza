...
import { SchoolSourceFactRepository } from "./repository.js";
import { Service } from "@elizaos/core";
import { actionBundleId } from "./repository.js";
import { authorityRank } from "./types.js";
import { canonicalSchoolJson } from "./types.js";
import { materialNoticeSha256 } from "./types.js";
import { normalizeResponsibilityAssignmentInput } from "./types.js";
import { normalizeSchoolNoticeExtraction } from "./types.js";
...
import { type ActionApprovalRequirement } from "./types.js";
...
import { type ChildReference } from "./types.js";
import { type ChildResolution } from "./types.js";
import { type ChildResolutionCandidate } from "./types.js";
...
import { type IngestSchoolNoticeResult } from "./types.js";
...
import { type SchoolNoticeNextAction } from "./types.js";
import { type SchoolNoticeResolution } from "./types.js";
...
function attributeStrings(entity: Entity, keys: readonly string[]): string[] {
    ...
    for (const key of keys) {
        ...
        if (typeof value === "string" && value.trim()) {
            ...
            values.push(...value.filter((entry): entry is string => typeof entry === "string" && Boolean(entry.trim())));
        }
    }
    return values;
}
...
function schoolFactValue(fact: SourceFact): PersistedSchoolFactValue {
    ...
    const question = record.childQuestion === null
        ? null
...
function schoolFactProjection(fact: SourceFact): SchoolFactProjection {
    ...
    if ((payload.childResolutionStatus === "resolved") !==
        (childEntityId !== null)) {
...
function consequenceItems(resolution: SchoolNoticeResolution): ActionBundleItem[] {
...
function bundleMatches(bundle: ActionBundle, input: Omit<ActionBundle, "id" | "revision" | "createdAt">): boolean {
    return (bundle.noticeKey === input.noticeKey &&
...
export class SchoolSourceFactService {
    ...
    static create(runtime: IAgentRuntime, options?: {
        agentId?: string;
        now?: () => Date;
    }): SchoolSourceFactService {
        const graph = resolveKnowledgeGraphService(runtime);
        if (!graph) {
            throw new SchoolSourceFactError("Runtime knowledge graph service is unavailable", "SCHOOL_GRAPH_UNAVAILABLE", { agentId: options?.agentId ?? runtime.agentId });
        }
        ...
        return new SchoolSourceFactService({
            ...
            entityStore,
            relationshipStore,
            repository: new SchoolSourceFactRepository(agentId, entityStore, relationshipStore),
            now: options?.now,
        });
    }
    async captureCandidates(artifactValue: SourceArtifactInput, candidateValues: readonly SourceFactCandidate[]): Promise<{
    ...
    async ingestSchoolNotice(inputValue: IngestSchoolNoticeInput): Promise<IngestSchoolNoticeResult> {
        ...
        const childEntityId = childResolution.status === "resolved" ? childResolution.entityId : null;
        ...
        const candidate = normalizeSourceFactCandidate({
            ...
            value: {
                extraction,
                childResolutionStatus: childResolution.status,
                childQuestion: childResolution.status === "resolved"
                    ? null
            ...
            supersedesStableFactKeys: extraction.kind === "correction" || extraction.kind === "cancellation"
            ...
            contradictsStableFactKeys: extraction.kind === "correction"
                ? [`${SCHOOL_NOTICE_FACT_PREFIX}${noticeKey}`]
                : [],
        });
        ...
        if (responsibility) {
            ...
            responsibilityAssignment = await this.deps.repository.putResponsibility(responsibility, this.now().toISOString());
        }
        const { resolution, bundle, replayed } = await this.reconcileNotice(noticeKey, responsibilityAssignment?.id ?? null);
        return {
            artifact,
            sourceFact,
            childResolution,
    ...
    async resolveChild(reference: ChildReference): Promise<ChildResolution> {
        ...
        const candidates = children
            ...
            .filter((candidate): candidate is ChildResolutionCandidate & {
            score: number;
        } => candidate !== null)
            .sort((left, right) => right.score - left.score ||
            left.entityId.localeCompare(right.entityId));
        if (reference.externalChildId) {
            const exact = candidates.filter((candidate) => candidate.matchedHints.includes("externalChildId"));
            if (exact.length === 1) {
                return {
                    status: "resolved",
            ...
            if (exact.length > 1) {
                return {
                    ...
                    question: "More than one household child has this school identifier. Confirm the child before writing any dependent record.",
                };
            }
            return {
                status: "unresolved",
                candidates: candidates.map(({ score: _score, ...candidate }) => candidate),
                question: "No household child matches this school identifier. Confirm the child before writing any dependent record.",
            };
        }
        if (candidates.length === 0) {
            return {
                status: "unresolved",
                candidates: [],
                question: "Which household child, school, or team does this notice apply to?",
            };
        }
        ...
        if (best.length !== 1) {
            return {
                ...
                question: "More than one household child matches these details. Confirm the child, school, grade, or team before writing any dependent record.",
            };
        }
        return {
            status: "resolved",
    ...
    async reconcileNotice(noticeKeyValue: string, explicitResponsibilityAssignmentId: string | null = null): Promise<{
        ...
        if (!noticeKey)
            return invalid("noticeKey must be a non-empty string");
        const facts = await this.deps.repository.listFacts(`${SCHOOL_NOTICE_FACT_PREFIX}${noticeKey}`);
        if (facts.length === 0) {
            throw new SchoolSourceFactError(`No source facts exist for school notice ${noticeKey}`, "SCHOOL_SOURCE_NOT_FOUND", { noticeKey });
        }
        const projections = facts.map(schoolFactProjection);
        const highestAuthority = Math.max(...projections.map((projection) => authorityRank(projection.fact.authority)));
        const authorityWinners = projections.filter((projection) => authorityRank(projection.fact.authority) === highestAuthority);
        const highestVersion = Math.max(...authorityWinners.map((projection) => projection.fact.version.sequence));
        const top = authorityWinners.filter((projection) => projection.fact.version.sequence === highestVersion);
        const distinctMaterials = new Set(top.map((projection) => projection.materialSha256));
        ...
        if (distinctMaterials.size > 1) {
            ...
            const winner = top[0];
            const current = projections.filter((projection) => projection.materialSha256 === winner.materialSha256);
            const currentIds = new Set(current.map((projection) => projection.fact.id));
            const superseded = projections.filter((projection) => !currentIds.has(projection.fact.id));
            const state = winner.payload.extraction.kind === "cancellation"
                ? "cancelled"
        ...
        const existingBundles = await this.deps.repository.listActionBundles(noticeKey);
        const responsibilityAssignmentId = explicitResponsibilityAssignmentId ??
            existingBundles
                .slice()
                .reverse()
        ...
        const bundleInput = {
            noticeKey,
            sourceFactIds: resolution.state === "conflicted"
                ? resolution.contradictionFactIds
                : resolution.currentFactIds,
            childEntityId: resolution.state === "conflicted" ? null : resolution.childEntityId,
            state,
            items,
            responsibilityAssignmentId,
        } satisfies Omit<ActionBundle, "id" | "revision" | "createdAt">;
        const replay = existingBundles.find((bundle) => bundleMatches(bundle, bundleInput));
        if (replay)
            return { resolution, bundle: replay, replayed: true };
        const revision = existingBundles.reduce((highest, bundle) => Math.max(highest, bundle.revision), 0) + 1;
        ...
        const persisted = await this.deps.repository.putActionBundle(bundle, existingBundles.map((existing) => existing.id));
    ...
    async listCurrentActionBundles(noticeKey: string): Promise<ActionBundle[]> {
        ...
        const supersededIds = new Set(relationships.map((relationship) => relationship.toEntityId));
    ...
    private async listHouseholdChildren(): Promise<Entity[]> {
        ...
        const childIds = Array.from(new Set(relationships
            ...
            .map((relationship) => relationship.toEntityId))).sort();
    ...
    private scoreChild(entity: Entity, reference: ChildReference): (ChildResolutionCandidate & {
        score: number;
    }) | null {
        ...
        if (reference.preferredName) {
            if (!includesNormalized(names, reference.preferredName))
                return null;
    ...
    private async requireSourceActor(artifact: SourceArtifactInput): Promise<void> {
    ...
    private async requireEntity(entityId: string): Promise<Entity> {
        ...
        if (!entity) {
            throw new SchoolSourceFactError(`Knowledge-graph entity ${entityId} does not exist`, "SCHOOL_SUBJECT_NOT_FOUND", { entityId });
        }
        return entity;
    }
    private effectiveFrom(extraction: SchoolNoticeExtraction, artifact: SourceArtifactInput): string | null {
    ...
    private async linkDeclaredFactRelationships(fact: SourceFact, occurredAt: string): Promise<void> {
...
export class SchoolSourceFactRuntimeService extends Service {
    ...
    override capabilityDescription = "Immutable school source facts, child disambiguation, correction reconciliation, safe action bundles, and structural C/P/E/M ownership";
    readonly school: SchoolSourceFactService;
    constructor(runtime?: IAgentRuntime) {
        super(runtime);
        if (!runtime) {
            throw new SchoolSourceFactError("SchoolSourceFactRuntimeService requires a runtime", "SCHOOL_INVALID_CONTRACT");
        }
        this.school = SchoolSourceFactService.create(runtime);
    }
    static async start(runtime: IAgentRuntime): Promise<SchoolSourceFactRuntimeService> {
        await runtime.getServiceLoadPromise(KNOWLEDGE_GRAPH_SERVICE);
        return new SchoolSourceFactRuntimeService(runtime);
    }
    async stop(): Promise<void> { }
    ingestSchoolNotice(input: IngestSchoolNoticeInput): Promise<IngestSchoolNoticeResult> {
        return this.school.ingestSchoolNotice(input);
    }
    captureCandidates(artifact: SourceArtifactInput, candidates: readonly SourceFactCandidate[]): Promise<{
        artifact: SourceArtifact;
        sourceFacts: SourceFact[];
    }> {
...
export function getSchoolSourceFactRuntimeService(runtime: IAgentRuntime): SchoolSourceFactRuntimeService | null {
    return runtime.getService<SchoolSourceFactRuntimeService>(SCHOOL_SOURCE_FACT_SERVICE);
}
