 ...
 */
import { ACTION_EFFECT_CLASSES } from "./types.js";
import { SCHOOL_NOTICE_ACTION_KINDS } from "./types.js";
import { SchoolSourceFactError } from "./types.js";
import { canonicalSchoolJson } from "./types.js";
import { normalizeResponsibilityAssignmentInput } from "./types.js";
import { normalizeSourceArtifactInput } from "./types.js";
import { normalizeSourceFactCandidate } from "./types.js";
import { sha256 } from "./types.js";
import { sourceFactRevisionSha256 } from "./types.js";
import { stableSchoolId } from "./types.js";
import { type ActionBundle } from "./types.js";
import { type ActionBundleItem } from "./types.js";
...
import { type ResponsibilityAssignment } from "./types.js";
import { type ResponsibilityAssignmentInput } from "./types.js";
import { type SourceArtifact } from "./types.js";
import { type SourceArtifactInput } from "./types.js";
import { type SourceFact } from "./types.js";
...
function invalidPersisted(message: string, context?: Record<string, unknown>): never {
    throw new SchoolSourceFactError(message, "SCHOOL_IMMUTABLE_CONFLICT", context);
}
...
function artifactFromEntity(entity: Entity): SourceArtifact {
    const contract = recordValue(attributeValue(entity, SOURCE_ARTIFACT_ATTRIBUTE), SOURCE_ARTIFACT_ATTRIBUTE);
...
function factFromEntity(entity: Entity): SourceFact {
    const contract = recordValue(attributeValue(entity, SOURCE_FACT_ATTRIBUTE), SOURCE_FACT_ATTRIBUTE);
    ...
    const fact: SourceFact = {
        ...
        revisionSha256: requiredText(contract.revisionSha256, "sourceFact.revisionSha256"),
    ...
    if (fact.revisionSha256 !== sourceFactRevisionSha256(fact.artifactId, candidate)) {
...
function actionItemFromValue(value: unknown, index: number): ActionBundleItem {
    ...
    const effectClassText = requiredText(record.effectClass, `actionBundle.items[${index}].effectClass`);
    const effectClass = ACTION_EFFECT_CLASSES.find((candidate) => candidate === effectClassText);
    if (!effectClass) {
    ...
    const approvalRequirement = requiredText(record.approvalRequirement, `actionBundle.items[${index}].approvalRequirement`);
    if (approvalRequirement !== "none" &&
        approvalRequirement !== "owner_approval" &&
        approvalRequirement !== "multi_party_approval") {
        return invalidPersisted("Persisted action bundle has invalid approvalRequirement", { index, approvalRequirement });
    }
    const state = requiredText(record.state, `actionBundle.items[${index}].state`);
    ...
    const kindText = requiredText(record.kind, `actionBundle.items[${index}].kind`);
    const kind = ACTION_BUNDLE_ITEM_KINDS.find((candidate) => candidate === kindText);
    if (!kind) {
    ...
    return {
        ...
        targetReference: nullableText(record.targetReference, `actionBundle.items[${index}].targetReference`),
        effectClass,
        approvalRequirement,
        state,
    };
...
function actionBundleFromEntity(entity: Entity): ActionBundle {
    const contract = recordValue(attributeValue(entity, ACTION_BUNDLE_ATTRIBUTE), ACTION_BUNDLE_ATTRIBUTE);
    if (!Array.isArray(contract.sourceFactIds)) {
        return invalidPersisted("Persisted actionBundle.sourceFactIds is not an array");
    }
    ...
    const sourceFactIds = contract.sourceFactIds.map((value, index) => requiredText(value, `actionBundle.sourceFactIds[${index}]`));
    const bundle: ActionBundle = {
        ...
        sourceFactIds,
        childEntityId: nullableText(contract.childEntityId, "actionBundle.childEntityId"),
        revision: requiredInteger(contract.revision, "actionBundle.revision"),
        state,
        items: contract.items.map(actionItemFromValue),
        responsibilityAssignmentId: nullableText(contract.responsibilityAssignmentId, "actionBundle.responsibilityAssignmentId"),
...
function responsibilityFromEntity(entity: Entity): ResponsibilityAssignment {
    const contract = recordValue(attributeValue(entity, RESPONSIBILITY_ATTRIBUTE), RESPONSIBILITY_ATTRIBUTE);
    ...
    if (assignment.id !== entity.entityId) {
        return invalidPersisted("Responsibility assignment id does not match graph entity", { entityId: entity.entityId, assignmentId: assignment.id });
    }
    return assignment;
}
...
function assertSafeActionBundle(bundle: ActionBundle): void {
    for (const item of bundle.items) {
        if (item.effectClass !== "read_only" &&
            item.effectClass !== "internal_reversible" &&
            item.approvalRequirement === "none") {
            throw new SchoolSourceFactError("Consequential action bundle items require typed approval", "SCHOOL_INVALID_CONTRACT", {
                bundleId: bundle.id,
                itemId: item.id,
        ...
        if (item.state !== "proposed" && item.state !== "blocked") {
            throw new SchoolSourceFactError("Source ingestion may only persist proposed or blocked actions", "SCHOOL_INVALID_CONTRACT", { bundleId: bundle.id, itemId: item.id, state: item.state });
        }
    }
}
export class SchoolSourceFactRepository {
    constructor(private readonly agentId: string, private readonly entityStore: EntityStore, private readonly relationshipStore: RelationshipStore) { }
    async putArtifact(artifactInput: SourceArtifactInput): Promise<SourceArtifact> {
        const input = normalizeSourceArtifactInput(artifactInput);
        if (sha256(input.untrustedContent) !== input.contentSha256) {
            throw new SchoolSourceFactError("Source artifact content does not match contentSha256", "SCHOOL_INVALID_CONTRACT", { stableReference: input.stableReference });
        }
        ...
        if (existing) {
            ...
            if (canonicalSchoolJson(persisted) !== canonicalSchoolJson(artifact)) {
                return invalidPersisted("Content-addressed source artifact changed after persistence", { artifactId: id });
            }
            return persisted;
        }
        const entity = await this.entityStore.upsert(entityInput({
            id,
            type: "concept",
    ...
    async getArtifact(id: string): Promise<SourceArtifact | null> {
        ...
        if (!entity.tags.includes(SOURCE_ARTIFACT_TAG)) {
            return invalidPersisted("Requested graph entity is not a source artifact", {
                entityId: id,
            });
        }
        return artifactFromEntity(entity);
    }
    async putFact(artifact: SourceArtifact, candidateInput: SourceFactCandidate): Promise<SourceFact> {
        ...
        if (existing) {
            ...
            if (canonicalSchoolJson(persisted) !== canonicalSchoolJson(fact)) {
                return invalidPersisted("Content-addressed source fact changed after persistence", { factId: id });
            }
            return persisted;
        }
        const entity = await this.entityStore.upsert(entityInput({
    ...
    async listFacts(stableFactKey?: string): Promise<SourceFact[]> {
        ...
        return entities
            .map(factFromEntity)
            .filter((fact) => stableFactKey === undefined || fact.stableFactKey === stableFactKey)
            .sort((left, right) => left.createdAt.localeCompare(right.createdAt) ||
            left.id.localeCompare(right.id));
    }
    async putResponsibility(inputValue: ResponsibilityAssignmentInput, createdAt: string): Promise<ResponsibilityAssignment> {
    ...
    async getResponsibility(id: string): Promise<ResponsibilityAssignment | null> {
        const entity = await this.entityStore.get(id);
        if (!entity)
            return null;
        if (!entity.tags.includes(RESPONSIBILITY_TAG)) {
            return invalidPersisted("Requested graph entity is not a responsibility assignment", { entityId: id });
        }
        return responsibilityFromEntity(entity);
    }
    async putActionBundle(bundle: ActionBundle, supersedesBundleIds: readonly string[]): Promise<ActionBundle> {
        assertSafeActionBundle(bundle);
        ...
        if (bundle.id !== expectedId) {
            throw new SchoolSourceFactError("Action bundle id does not match its immutable content", "SCHOOL_INVALID_CONTRACT", { bundleId: bundle.id, expectedId });
        }
    ...
    async listActionBundles(noticeKey?: string): Promise<ActionBundle[]> {
        ...
        return entities
            ...
            .filter((bundle) => noticeKey === undefined || bundle.noticeKey === noticeKey)
    ...
    private async ensureRelationship(input: {
        ...
        if (existing) {
            if (existing.fromEntityId !== input.fromEntityId ||
                existing.toEntityId !== input.toEntityId ||
                existing.type !== input.type) {
                return invalidPersisted("Content-addressed relationship changed after persistence", { relationshipId });
            }
            return existing;
        }
