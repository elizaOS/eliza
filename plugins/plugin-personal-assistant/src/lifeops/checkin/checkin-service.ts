 ...
 */
import { LIFEOPS_OCCURRENCE_STATES } from "@elizaos/core/contracts/personal-assistant";
import { ModelType } from "@elizaos/core";
import { buildBriefingSignals } from "./checkin-briefing-ranking.js";
import { buildUtcDateFromLocalParts } from "../time.js";
import { computeMissedOccurrenceStreak } from "../service-helpers-occurrence.js";
import { computeOccurrenceStreaks } from "../service-helpers-occurrence.js";
...
import { runWithTrajectoryPurpose } from "@elizaos/core";
import { sortBriefingItems } from "./checkin-briefing-ranking.js";
import { sqlQuote } from "../sql.js";
import { toFiniteNonNegativeNumber } from "./checkin-briefing-ranking.js";
import { toText } from "../sql.js";
import { toWellFormedUnicode } from "@elizaos/core";
import { type BriefingEngagement } from "./checkin-briefing-ranking.js";
import { type CheckinBriefingSection } from "./types.js";
import { type CheckinKind } from "./types.js";
import { type CheckinReport } from "./types.js";
import { type EscalationLevel } from "./types.js";
import { type GetLifeOpsCalendarFeedRequest } from "@elizaos/core/contracts/calendar";
import { type GetLifeOpsGmailTriageRequest } from "@elizaos/core/contracts/personal-assistant";
import { type GetLifeOpsInboxRequest } from "@elizaos/core/contracts/personal-assistant";
import { type HabitSummary } from "./types.js";
import { type IAgentRuntime } from "@elizaos/core";
import { type LifeOpsCadence } from "@elizaos/core/contracts/personal-assistant";
...
import { type LifeOpsInbox } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsOccurrence } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsOccurrenceState } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsXDm } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsXFeedItem } from "@elizaos/core/contracts/personal-assistant";
import { type LifeOpsXFeedType } from "@elizaos/core/contracts/personal-assistant";
import { type MeetingEntry } from "./types.js";
import { type OverdueTodo } from "./types.js";
import { type RecentWin } from "./types.js";
import { type RecordAcknowledgementRequest } from "./types.js";
import { type RunCheckinRequest } from "./types.js";
...
export function getCheckinSummaryTrajectoryPurpose(kind: CheckinKind): "morning_brief" | "health_checkin" {
...
export interface CheckinSourceService {
    ...
    syncXFeed?(feedType: LifeOpsXFeedType, opts?: {
        limit?: number;
        query?: string;
    }): Promise<{
        synced: number;
...
function logMissingOnce(key: string, message: string): void {
    if (loggedMissingSources.has(key))
        return;
...
function formatCheckinReportForPrompt(report: Omit<CheckinReport, "summaryText">): string {
 ...
 */
export function buildCheckinSummaryPrompt(report: Omit<CheckinReport, "summaryText">): string {
    const lines: string[] = [
        report.kind === "morning"
    ...
    if (report.kind === "night" && report.sleepRecap) {
        ...
        lines.push("", "Sleep recap (use these facts only — do not invent sleep numbers):", ...recapBullets, 'Include a short "Sleep recap" section in the summary using these numbers when present. If `regularityClass` is `irregular` or `very_irregular`, suggest one concrete step toward consistency. If it is `insufficient_data`, say so plainly and skip recommendations.');
    }
    lines.push("", "Report JSON:", formatCheckinReportForPrompt(report), "", "Summary:");
    return lines.join("\n");
}
...
function unavailableSection(key: CheckinBriefingSection["key"], title: string, message: string): CheckinBriefingSection {
    return {
        key,
        title,
...
const LIFEOPS_OCCURRENCE_STATE_SET: ReadonlySet<string> = new Set(LIFEOPS_OCCURRENCE_STATES);
...
function resolvePausedUntil(metadata: Record<string, unknown>, now: Date): string | null {
...
export function buildHabitSummary(args: {
    ...
    const quotaOccurrence = args.cadence.kind === "count_per_day"
        ? args.occurrences
            .filter((occurrence) => occurrence.dueAtMs >= args.now.getTime())
            .sort((left, right) => left.dueAtMs - right.dueAtMs)[0]
        : undefined;
    const completedCount = quotaOccurrence
        ? Math.min(quotaOccurrence.progressTotal, args.cadence.kind === "count_per_day" ? args.cadence.targetCount : 0)
        : 0;
    return {
        ...
        progress: args.cadence.kind === "count_per_day" && quotaOccurrence
            ? {
                completedCount,
                targetCount: args.cadence.targetCount,
                remainingCount: Math.max(args.cadence.targetCount - completedCount, 0),
...
async function collectHabitSummaries(runtime: IAgentRuntime, now: Date): Promise<CollectorResult<HabitSummary> & {
    pausedDefinitionIds: Set<string>;
}> {
    const agentId = String(runtime.agentId);
    try {
        const definitionRows = await executeRawSql(runtime, `SELECT id AS definition_id,
        ...
        const occurrencesRows = await executeRawSql(runtime, `SELECT definition_id,
        ...
        for (const row of occurrencesRows as HabitCollectorRow[]) {
            ...
            if (!Number.isFinite(rawProgressTotal)) {
                throw new Error(`Invalid quota progress total for LifeOps definition ${definitionId}`);
            }
        ...
        for (const row of definitionRows as HabitCollectorRow[]) {
            ...
            const cadence = parseJsonRecord(row.definition_cadence_json) as LifeOpsCadence;
        ...
        logMissingOnce("habit-summaries", `habit summaries collector unavailable: ${message}`);
        return { rows: [], error: message, pausedDefinitionIds: new Set() };
    }
}
async function collectOverdueTodos(runtime: IAgentRuntime, now: Date, pausedDefinitionIds: ReadonlySet<string>): Promise<CollectorResult<OverdueTodo>> {
    const agentId = String(runtime.agentId);
    const nowIso = now.toISOString();
    try {
        const rows = await executeRawSql(runtime, `SELECT occ.id AS id,
        ...
        const message = error instanceof Error ? error.message : String(error);
        logMissingOnce("overdue-todos", `overdue-todos collector unavailable (app_lifeops.life_task_occurrences not ready): ${message}`);
        return { rows: [], error: message };
    }
}
async function collectTodaysMeetings(runtime: IAgentRuntime, now: Date, timezone: string): Promise<CollectorResult<MeetingEntry>> {
    const agentId = String(runtime.agentId);
    const day = localDayWindow(now, timezone);
    try {
        const rows = await executeRawSql(runtime, `SELECT id, title, start_at, end_at
        ...
        const message = error instanceof Error ? error.message : String(error);
        logMissingOnce("todays-meetings", `meetings collector unavailable (app_calendar.life_calendar_events not ready): ${message}`);
        return { rows: [], error: message };
    }
}
async function collectCompletedWins(runtime: IAgentRuntime, kind: CheckinKind, now: Date, timezone: string): Promise<CollectorResult<RecentWin>> {
    const agentId = String(runtime.agentId);
    const day = kind === "morning"
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        logMissingOnce("yesterdays-wins", `completed-wins collector unavailable: ${message}`);
        return { rows: [], error: message };
    }
}
...
function resolveHabitEscalationLevel(summaries: readonly HabitSummary[]): EscalationLevel {
    const maxMissedStreak = summaries.reduce((max, summary) => Math.max(max, summary.missedOccurrenceStreak), 0);
    return clampEscalation(maxMissedStreak);
}
async function collectXDmSection(source: CheckinSourceService | undefined): Promise<CheckinBriefingSection> {
    if (!source?.syncXDms || !source.getXDms) {
        return unavailableSection("x_dms", "X DMs", "X DM reader is not registered on this runtime.");
    }
    try {
        await source.syncXDms({ limit: 30 });
        const dms = await source.getXDms({ limit: 30 });
        const items = sortBriefingItems(dms.map((dm) => {
        ...
        const actionNeeded = items.filter((item) => item.signals?.replyNeeded).length;
        return {
            key: "x_dms",
            title: "X DMs",
...
async function collectXFeedSection(source: CheckinSourceService | undefined, key: Extract<CheckinBriefingSection["key"], "x_timeline" | "x_mentions">, feedType: Extract<LifeOpsXFeedType, "home_timeline" | "mentions">, title: string): Promise<CheckinBriefingSection> {
    if (!source?.syncXFeed || !source.getXFeedItems) {
        return unavailableSection(key, title, "X feed reader is not registered on this runtime.");
    }
    try {
        await source.syncXFeed(feedType, { limit: 30 });
        const feedItems = await source.getXFeedItems(feedType, { limit: 30 });
        const items = sortBriefingItems(feedItems.map((feedItem) => {
            ...
            const engagement: BriefingEngagement = {
                ...
                totalCount: toFiniteNonNegativeNumber(metrics.like_count) +
...
async function collectInboxSection(source: CheckinSourceService | undefined): Promise<CheckinBriefingSection> {
    if (!source?.getInbox) {
        return unavailableSection("inbox", "Inbox", "Inbox reader is not registered on this runtime.");
    }
    try {
        const inbox = await source.getInbox({ limit: 50 });
        const counts = Object.entries(inbox.channelCounts)
            .filter(([, count]) => count.total > 0)
            .map(([channel, count]) => `${channel}: ${count.total}${count.unread > 0 ? `/${count.unread} unread` : ""}`);
        const items = sortBriefingItems(inbox.messages.map((message) => {
            const ranked = buildBriefingSignals({
...
async function collectGmailSection(source: CheckinSourceService | undefined, now: Date): Promise<CheckinBriefingSection> {
    if (!source?.getGmailTriage) {
        return unavailableSection("gmail", "Gmail", "Gmail triage reader is not registered on this runtime.");
    }
    try {
        const feed = await source.getGmailTriage(INTERNAL_URL, { maxResults: 25 }, now);
        const items = sortBriefingItems(feed.messages.map((message) => {
            const ranked = buildBriefingSignals({
                occurredAt: message.receivedAt,
...
async function collectCalendarChangeSection(runtime: IAgentRuntime, now: Date, timezone: string): Promise<CheckinBriefingSection> {
    const agentId = String(runtime.agentId);
    const day = localDayWindow(now, timezone);
    const sinceIso = new Date(now.getTime() - 24 * 60 * 60 * 1000).toISOString();
    try {
        const rows = await executeRawSql(runtime, `SELECT title, start_at, end_at, status, html_link, updated_at
         FROM app_calendar.life_calendar_events
        WHERE agent_id = ${sqlQuote(agentId)}
          AND side = 'owner'
          AND (
        ...
        const changedCount = rows.filter((row) => (parseMs(toText(row.updated_at)) ?? 0) >= Date.parse(sinceIso)).length;
        const items = sortBriefingItems(rows.map((row) => {
            ...
            const reason = status.toLowerCase() === "cancelled"
                ? "removed/cancelled"
                : isChanged
                    ? "added or updated"
                    : "on schedule";
        ...
        const message = error instanceof Error ? error.message : String(error);
        return unavailableSection("calendar_changes", "Calendar and schedule changes", message);
    }
}
async function collectGitHubSection(runtime: IAgentRuntime, now: Date, timezone: string): Promise<CheckinBriefingSection> {
    const agentId = String(runtime.agentId);
    const day = localDayWindow(now, timezone);
    const sinceIso = new Date(now.getTime() - 24 * 60 * 60 * 1000).toISOString();
    try {
        const [mailRows, screenRows] = await Promise.all([
            executeRawSql(runtime, `SELECT subject, from_display, snippet, received_at, html_link,
          ...
          LIMIT 12`),
            executeRawSql(runtime, `SELECT identifier, display_name, start_at, duration_seconds
...
async function collectContactSection(runtime: IAgentRuntime, now: Date, timezone: string): Promise<CheckinBriefingSection> {
    const agentId = String(runtime.agentId);
    const day = localDayWindow(now, timezone);
    try {
        ...
        const rows = await executeRawSql(runtime, `SELECT relationship_id,
              channel,
              direction,
              summary,
              occurred_at
        ...
        LIMIT 30`);
        const entityStore = resolveKnowledgeGraphService(runtime)?.getEntityStore(runtime.agentId);
        const nameByEntityId = new Map<string, string>();
        if (entityStore) {
            const entityIds = new Set(rows.map((row) => toText(row.relationship_id)).filter(Boolean));
        ...
        const message = error instanceof Error ? error.message : String(error);
        return unavailableSection("contacts", "Contacts and conversations", message);
    }
}
async function collectPromiseSection(runtime: IAgentRuntime, now: Date): Promise<CheckinBriefingSection> {
    try {
        ...
        const items = sortBriefingItems(digest.overdue.map((entry) => {
            const ranked = buildBriefingSignals({
            ...
            return {
                ...
                detail: clip(`Last contacted ${entry.lastContactedAt} (cadence ${entry.thresholdDays}d)`),
                occurredAt: entry.lastContactedAt,
                href: null,
        ...
        return {
            ...
            summary: digest.overdue.length === 0
        ...
        const message = error instanceof Error ? error.message : String(error);
        return unavailableSection("promises", "Promises, agreements, and follow-ups", message);
    }
}
async function collectBriefingSections(args: {
    ...
    return Promise.all([
        ...
        collectXFeedSection(args.source, "x_timeline", "home_timeline", "X timeline"),
...
export class CheckinService {
    constructor(private readonly runtime: IAgentRuntime, private readonly options: CheckinServiceOptions = {}) { }
    async runMorningCheckin(request: RunCheckinRequest = {}): Promise<CheckinReport> {
        return this.runCheckin("morning", request);
    }
    async runNightCheckin(request: RunCheckinRequest = {}): Promise<CheckinReport> {
    ...
    async getEscalationLevel(now: Date = new Date()): Promise<EscalationLevel> {
        ...
        const rows = await executeRawSql(this.runtime, `SELECT COUNT(*) AS unack_count
    ...
    async hasCheckinForLocalDay(args: {
        ...
        const rows = await executeRawSql(this.runtime, `SELECT id
         FROM ${CHECKIN_REPORTS_TABLE}
        WHERE agent_id = ${sqlQuote(agentId)}
    ...
    async recordCheckinAcknowledgement(request: RecordAcknowledgementRequest): Promise<void> {
        const reportId = request.reportId.trim();
        if (!reportId) {
            throw new Error("[CheckinService] recordCheckinAcknowledgement: reportId is required");
        }
        const agentId = String(this.runtime.agentId);
        await executeRawSql(this.runtime, `UPDATE ${CHECKIN_REPORTS_TABLE}
          SET acknowledged_at = ${sqlQuote(new Date().toISOString())}
        WHERE id = ${sqlQuote(reportId)}
          AND agent_id = ${sqlQuote(agentId)}`);
    }
    private async runCheckin(kind: CheckinKind, request: RunCheckinRequest): Promise<CheckinReport> {
        ...
        const [overdueTodos, todaysMeetings, completedWins, briefingSections] = await Promise.all([
        ...
        const habitEscalationLevel = resolveHabitEscalationLevel(habitCollector.rows);
        ...
        const sleepRecap: SleepRecap | null = kind === "night" ? (request.sleepRecap ?? null) : null;
    ...
    public async persistCheckinReport(report: CheckinReport, now = new Date(report.generatedAt)): Promise<void> {
        await this.persistReport(report, now);
    }
    private fallbackSummary(report: Omit<CheckinReport, "summaryText">): string {
        const prefix = report.kind === "morning" ? "Morning check-in" : "Night check-in";
        const winsLabel = report.kind === "morning" ? "yesterday's wins" : "wins today";
    ...
    private async renderSummary(report: Omit<CheckinReport, "summaryText">): Promise<string> {
        ...
        try {
            const response = await runWithTrajectoryPurpose(getCheckinSummaryTrajectoryPurpose(report.kind), () => this.runtime.useModel(ModelType.TEXT_LARGE, {
                prompt,
            }));
            ...
            logMissingOnce("checkin-summary-model", `summary model unavailable: ${error instanceof Error ? error.message : String(error)}`);
            return fallback;
        }
    }
    private async persistReport(report: CheckinReport, now: Date): Promise<void> {
        ...
        await executeRawSql(this.runtime, `INSERT INTO ${CHECKIN_REPORTS_TABLE}
