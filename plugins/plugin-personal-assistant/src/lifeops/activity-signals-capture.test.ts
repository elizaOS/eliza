 ...
 * tears down its listeners/intervals on stop — including the #16504 lifecycle
 * contract: idempotent singleton start, race-safe stop across every awaited
 * native startup step, failed-initialization cleanup, and pagehide teardown.
 *
...
// mock returns the same combined shape: client + isApiError (/api),
// isElectrobunRuntime (/bridge), loadDesktopWorkspaceSnapshot (/browser),
// lifecycle event names (/events). The object literal is inlined per call and
// reads only the hoisted `h` — any module-scope const would sit in its TDZ
// when the hoisted `vi.mock` and source import run.
...
// Fake timers stub setTimeout, so `settle()` would hang; microtasks are never
// faked, so a plain promise loop drains async chains under fake timers.
...
function useNativeMobileRuntime(): void {
  h.capacitorIsNative.mockReturnValue(true);
  h.capacitorGetPlatform.mockReturnValue("ios");
}

/** Manually resolvable promise for driving stop-during-await races. */
...
describe("startLifeOpsActivitySignalCapture", () => {
  ...
  it("is an idempotent singleton: a second start returns the active stop without duplicating listeners", async () => {
    const addDoc = vi.spyOn(document, "addEventListener");
    stop = startLifeOpsActivitySignalCapture(true);
    const again = startLifeOpsActivitySignalCapture(true);
    expect(again).toBe(stop);
    await settle();

    const visibilityListeners = addDoc.mock.calls.filter(
      ([event]) => event === "visibilitychange",
    );
    expect(visibilityListeners.length).toBe(1);

    // After stop, a fresh start claims a new session.
    stop();
    const fresh = startLifeOpsActivitySignalCapture(true);
    expect(fresh).not.toBe(stop);
    stop = fresh;
    addDoc.mockRestore();
  });
  ...
  it("keeps a network status-probe failure quiet but surfaces an unexpected probe failure", async () => {
    ...
    // A non-transport failure out of the probe is a defect, not "not ready":
    // it must surface as a capture_error status event (#16504).
    h.getStatus.mockRejectedValue(new Error("status route exploded"));
    h.isApiError.mockReturnValue(false);
    stop = startLifeOpsActivitySignalCapture(true);
    await settle();
  ...
  it("removes a listener installed after stop() landed mid-startup and never starts monitoring", async () => {
    ...
    // Stop while addListener is still in flight, then let it resolve late.
    stop();
    stop = undefined;
    pendingListener.resolve({ remove });
    await settle();
  ...
  it("halts monitoring that resolved after stop() and installs no health poller", async () => {
    vi.useFakeTimers();
    useNativeMobileRuntime();
    const pendingMonitoring = deferred<{
      enabled: boolean;
      supported: boolean;
      platform: string;
      snapshot: null;
    ...
    h.mobile.startMonitoring.mockImplementation(
      async () => pendingMonitoring.promise,
    );
    ...
    // No health poller survived: five minutes later, no snapshot poll fires.
    h.mobile.getSnapshot.mockClear();
    await vi.advanceTimersByTimeAsync(5 * 60_000);
    expect(h.mobile.getSnapshot).not.toHaveBeenCalled();
  });

  it("cleans up the installed listener when native startup fails part-way", async () => {
    useNativeMobileRuntime();
    const remove = vi.fn(async () => {});
    h.mobile.addListener.mockImplementation(async () => ({ remove }));
    h.mobile.startMonitoring.mockRejectedValue(
      new Error("monitoring exploded"),
    );

    stop = startLifeOpsActivitySignalCapture(true);
    await settle();

    // Failed initialization must undo the partial install and surface.
    expect(remove).toHaveBeenCalled();
    expect(h.dispatchStatus).toHaveBeenCalledWith(
      expect.objectContaining({
        status: "capture_error",
        message: "monitoring exploded",
      }),
    );
  });

  it("stops on a real pagehide but survives a bfcache round trip", async () => {
    stop = startLifeOpsActivitySignalCapture(true);
    await settle();
    h.captureLifeOpsActivitySignal.mockClear();

    // bfcache: persisted=true — capture stays alive.
    ...
    // Real teardown: persisted=false — capture stops itself.
    h.captureLifeOpsActivitySignal.mockClear();
