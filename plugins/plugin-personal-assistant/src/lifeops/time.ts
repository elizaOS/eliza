...
export function buildUtcDateFromLocalParts(timeZone: string, parts: ZonedDateParts): Date {
    ...
    const offsets = new Set(OFFSET_SAMPLE_HOURS.map((hours) => getTimeZoneOffsetMinutes(new Date(baseUtcMs + hours * HOUR_MS), timeZone)));
    ...
    const shiftedForward = candidates
        ...
        .filter(({ wallDeltaMs, candidate }) => Number.isFinite(wallDeltaMs) &&
        wallDeltaMs > 0 &&
        Number.isFinite(candidate.getTime()))
        .sort((left, right) => left.wallDeltaMs - right.wallDeltaMs ||
...
export function formatInstantAsRfc3339InTimeZone(value: Date | string, timeZone: string): string {
...
export function addDaysToLocalDate(dateOnly: Pick<ZonedDateParts, "year" | "month" | "day">, dayDelta: number): Pick<ZonedDateParts, "year" | "month" | "day"> {
...
export function getWeekdayForLocalDate(dateOnly: Pick<ZonedDateParts, "year" | "month" | "day">): number {
    return new Date(Date.UTC(dateOnly.year, dateOnly.month - 1, dateOnly.day, 12, 0, 0)).getUTCDay();
}
export function getLocalDateKey(dateOnly: Pick<ZonedDateParts, "year" | "month" | "day">): string {
