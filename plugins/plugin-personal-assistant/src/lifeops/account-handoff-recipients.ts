...
import { handoffRecipientBindingsSchema } from "./account-handoff-store.js";
import { normalizeEntityConnectorAccountId } from "@elizaos/core/knowledge-graph/entity-types";
import { resolveKnowledgeGraphService } from "@elizaos/plugin-relationships";
import { type AccountHandoffRecord } from "./account-handoff-store.js";
import { type IAgentRuntime } from "@elizaos/core";
import { z } from "zod";
type Destination = AccountHandoffRecord["review"]["messageDestinations"][number];
...
export class AccountHandoffRecipients {
    ...
    constructor(private readonly runtime: IAgentRuntime, ownerEntityId: string) {
        this.store = new AccountHandoffStore(runtime, ownerEntityId);
    }
    async capture(operationId: string, expectedRevision: number, recipientEntityIds: string[]): Promise<AccountHandoffRecord> {
        ...
        if (record.receipt.recipientReview) {
            const saved = handoffRecipientBindingsSchema.safeParse(record.receipt.recipientReview);
            if (!saved.success ||
                saved.data.length !== ids.data.length ||
                saved.data.some((binding, index) => binding.recipientEntityId !== ids.data[index]))
                throw this.changed();
            await this.verify(operationId, expectedRevision);
            return record;
        }
        const bindings = await this.resolve(record.review.messageDestinations, ids.data);
    ...
    async resolve(destinations: Destination[], recipientEntityIds: string[]): Promise<z.infer<typeof handoffRecipientBindingsSchema>> {
        if (destinations.length !== recipientEntityIds.length)
            throw this.changed();
        ...
        for (const [index, destination] of destinations.entries()) {
            ...
            if (!entity)
                throw this.changed();
            const identities = entity.identities.filter((identity) => identity.verified &&
                platforms[destination.channel].includes(identity.platform.toLowerCase()) &&
                this.sameHandle(destination.channel, identity.handle, destination.recipientId) &&
            ...
            bindings.push({
                ...
                identityConnectorAccountId: normalizeEntityConnectorAccountId(identity.connectorAccountId),
            });
        }
        return bindings;
    }
    async verify(operationId: string, expectedRevision: number): Promise<void> {
        const record = await this.requireRecord(operationId, expectedRevision);
        const parsed = handoffRecipientBindingsSchema.safeParse(record.receipt.recipientReview);
        if (!parsed.success ||
        ...
        for (const [index, binding] of parsed.data.entries()) {
            ...
            if (!entity)
                throw this.changed();
            if (!entity.identities.some((identity) => identity.verified &&
    ...
    private graph() {
        ...
        if (!graph)
            throw new ElizaError("Verified contacts are unavailable. Retry recipient review when the identity service is ready.", { code: "ACCOUNT_HANDOFF_RECIPIENT_GRAPH_UNAVAILABLE" });
    ...
    private sameHandle(channel: Destination["channel"], left: string, right: string): boolean {
        return channel === "email"
            ? left.toLowerCase() === right.toLowerCase()
            : left === right;
    }
    private changed(): ElizaError {
        return new ElizaError("A reviewed recipient identity changed or is not verified for the selected channel and account. Refresh the saved handoff review.", { code: "ACCOUNT_HANDOFF_RECIPIENT_CHANGED" });
    }
}
