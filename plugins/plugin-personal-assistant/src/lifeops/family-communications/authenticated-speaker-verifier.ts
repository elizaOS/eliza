...
import { FamilyCommunicationsError } from "./types.js";
import { FamilyVoiceSpeakerVerifierRuntimeService } from "./service.js";
...
import { type VerifiedVoiceSpeaker } from "./types.js";
import { type VoiceSpeakerAttestation } from "./types.js";
...
function unsignedAttestation(attestation: VoiceSpeakerAttestation): Omit<VoiceSpeakerAttestation, "proof"> {
...
export class AuthenticatedRuntimeSpeakerVerifierService extends FamilyVoiceSpeakerVerifierRuntimeService {
    ...
    override capabilityDescription = "Short-lived family speaker binding from authenticated runtime message identity";
    private readonly secret = randomBytes(32);
    private readonly issued = new Map<string, IssuedAttestation>();
    constructor(runtime?: IAgentRuntime) {
        super(runtime);
        if (!runtime) {
            throw new FamilyCommunicationsError("AuthenticatedRuntimeSpeakerVerifierService requires a runtime", "FAMILY_COMMUNICATIONS_SPEAKER_VERIFIER_UNAVAILABLE");
        }
    }
    static async start(runtime: IAgentRuntime): Promise<AuthenticatedRuntimeSpeakerVerifierService> {
        return new AuthenticatedRuntimeSpeakerVerifierService(runtime);
    }
    private proofFor(attestation: Omit<VoiceSpeakerAttestation, "proof">): string {
        return createHmac("sha256", this.secret)
            .update(stableStringify(attestation))
            .digest("base64url");
    }
    async issueForAuthenticatedMessage(message: Memory): Promise<AuthenticatedMessageSpeakerAttestation> {
        const principalEntityId = await resolveAuthenticatedFamilyPrincipal(this.runtime, message);
        if (!principalEntityId) {
            throw new FamilyCommunicationsError("A verified runtime message principal is required to issue a speaker attestation", "FAMILY_COMMUNICATIONS_ACCESS_DENIED");
        }
        if (!message.id) {
            throw new FamilyCommunicationsError("Authenticated message capture requires a persisted message ID", "FAMILY_COMMUNICATIONS_INVALID_CONTRACT");
        }
    ...
    async verify(attestation: VoiceSpeakerAttestation): Promise<VerifiedVoiceSpeaker> {
        const issued = this.issued.get(attestation.attestationId);
        if (attestation.issuer !== AUTHENTICATED_MESSAGE_ISSUER ||
            !issued ||
        ...
        if (!graph) {
            throw new FamilyCommunicationsError("KnowledgeGraphService is required to verify a family principal", "FAMILY_COMMUNICATIONS_SPEAKER_VERIFIER_UNAVAILABLE", { agentId: this.runtime.agentId });
        }
        const entityStore = graph.getEntityStore(this.runtime.agentId);
        const entity = issued.principalEntityId === SELF_ENTITY_ID
            ? await entityStore.ensureSelf()
            : await entityStore.get(issued.principalEntityId);
        if (!entity)
            return { kind: "unverified", reason: "revoked_binding" };
        return {
            kind: "verified",
...
export function getAuthenticatedRuntimeSpeakerVerifier(runtime: IAgentRuntime): AuthenticatedRuntimeSpeakerVerifierService | null {
    const service = runtime.getService<AuthenticatedRuntimeSpeakerVerifierService>(FAMILY_COMMUNICATIONS_SPEAKER_VERIFIER_SERVICE);
    return service instanceof AuthenticatedRuntimeSpeakerVerifierService
        ? service
        : null;
}
export async function issueAuthenticatedMessageSpeakerAttestation(runtime: IAgentRuntime, message: Memory): Promise<AuthenticatedMessageSpeakerAttestation> {
    const service = getAuthenticatedRuntimeSpeakerVerifier(runtime);
    if (!service) {
        throw new FamilyCommunicationsError("Authenticated runtime speaker verifier is unavailable", "FAMILY_COMMUNICATIONS_SPEAKER_VERIFIER_UNAVAILABLE", { agentId: runtime.agentId });
    }
