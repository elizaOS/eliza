...
import { HouseholdInboundApprovalError } from "../household/inbound-approval.js";
...
import { familySha256 } from "./types.js";
import { getHouseholdCoordinationService } from "../household/service.js";
import { hasRoleAccess } from "@elizaos/core";
import { requireFamilyText } from "./types.js";
import { resolveKnowledgeGraphService } from "@elizaos/plugin-relationships";
import { resolveVerifiedHouseholdInboundEntityId } from "../household/inbound-approval.js";
import { type FamilyWeekItemInput } from "./types.js";
...
export async function resolveAuthenticatedFamilyPrincipal(runtime: IAgentRuntime, message: Memory): Promise<string | null> {
    if (await hasRoleAccess(runtime, message, "OWNER"))
        return SELF_ENTITY_ID;
    const principalEntityId = requireFamilyText(message.entityId, "message.entityId", 200);
    const inboundIdentity = authenticatedHouseholdInboundIdentity(message);
    if (!inboundIdentity)
        return null;
    let verifiedInboundEntityId: string;
    try {
        verifiedInboundEntityId = await resolveVerifiedHouseholdInboundEntityId(runtime, inboundIdentity);
    }
    catch (error) {
        ...
        if (error instanceof HouseholdInboundApprovalError &&
            EXPECTED_IDENTITY_DENIAL_CODES.has(error.code)) {
            return null;
        }
        throw error;
    }
    if (verifiedInboundEntityId !== principalEntityId)
        return null;
    const graph = resolveKnowledgeGraphService(runtime);
    if (!graph) {
        throw new FamilyCommunicationsError("KnowledgeGraphService is required to authenticate a household principal", "FAMILY_COMMUNICATIONS_PERSISTED_DATA_INVALID", { agentId: runtime.agentId });
    }
    ...
    const role = (await household.listRoleBindings()).find((binding) => binding.entityId === principalEntityId);
    if (!role)
        return null;
    try {
...
function scheduleSourceRef(input: {
    ...
    const recordId = input.state === "agreement" ? input.agreementId : input.proposalId;
    const version = input.state === "agreement"
        ? input.agreementVersion
        : input.proposalVersion;
    if (!recordId || version === null) {
        throw new FamilyCommunicationsError("Household schedule export is missing its exact revision reference", "FAMILY_COMMUNICATIONS_PERSISTED_DATA_INVALID", { state: input.state });
    }
...
export async function resolveTrustedChildWeekItems(input: {
    ...
    if (sourceRefs.length !== 1 ||
        ...
        throw new FamilyCommunicationsError("Only the structurally scoped household child-week source is currently available", "FAMILY_COMMUNICATIONS_INVALID_CONTRACT", {
    ...
    return exported.schedules
        .filter((schedule) => schedule.subjectEntityIds.includes(input.childEntityId))
