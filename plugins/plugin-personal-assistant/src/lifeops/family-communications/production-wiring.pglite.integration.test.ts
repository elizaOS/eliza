 ...
 */
import { AuthenticatedRuntimeSpeakerVerifierService } from "./authenticated-speaker-verifier.js";
import { ChannelType } from "@elizaos/core";
import { FAMILY_COMMUNICATIONS_SERVICE } from "./service.js";
import { FAMILY_COMMUNICATIONS_SPEAKER_VERIFIER_SERVICE } from "./service.js";
import { FamilyCommunicationsRuntimeService } from "./service.js";
import { HOUSEHOLD_CHILD_WEEK_SOURCE_REF } from "./production-wiring.js";
import { SELF_ENTITY_ID } from "@elizaos/core/knowledge-graph/entity-types";
import { afterAll } from "vitest";
import { beforeAll } from "vitest";
import { createFamilyCommunicationsAction } from "./index.js";
import { createHouseholdCoordinationService } from "../household/service.js";
import { createLifeOpsTestRuntime } from "../../../test/helpers/runtime.js";
import { createMessageMemory } from "@elizaos/core";
import { describe } from "vitest";
import { expect } from "vitest";
import { getAuthenticatedRuntimeSpeakerVerifier } from "./authenticated-speaker-verifier.js";
import { getFamilyCommunicationsService } from "./index.js";
import { issueAuthenticatedMessageSpeakerAttestation } from "./authenticated-speaker-verifier.js";
...
import { resolveTrustedChildWeekItems } from "./production-wiring.js";
import { setEntityRole } from "@elizaos/core";
import { type AgentRuntime } from "@elizaos/core";
import { type HouseholdCoordinationService } from "../household/service.js";
...
describe("family communications production wiring — real PGlite", () => {
    ...
    it("registers the action and ordered runtime verifier/service in the production plugin", () => {
        const serviceTypes = (personalAssistantPlugin.services ?? []).map((service) => service.serviceType);
        expect(serviceTypes).toContain(AuthenticatedRuntimeSpeakerVerifierService.serviceType);
        expect(serviceTypes).toContain(FamilyCommunicationsRuntimeService.serviceType);
        expect(serviceTypes.indexOf(FAMILY_COMMUNICATIONS_SPEAKER_VERIFIER_SERVICE)).toBeLessThan(serviceTypes.indexOf(FAMILY_COMMUNICATIONS_SERVICE));
        expect(personalAssistantPlugin.actions?.map((action) => action.name)).toEqual(expect.arrayContaining([
    ...
    it("uses a verified direct-message identity to mint a short-lived attestation and run the inert action path", async () => {
        ...
        await expect(resolveAuthenticatedFamilyPrincipal(runtime, message)).resolves.toBe(verifiedChildId);
        const verifier = getAuthenticatedRuntimeSpeakerVerifier(runtime);
        if (!verifier)
            throw new Error("runtime speaker verifier unavailable");
        ...
        await expect(verifier.verify({ ...issued.attestation, proof: "tampered-proof" })).resolves.toEqual({ kind: "unverified", reason: "invalid_proof" });
        ...
        expect(result.effectReceipts?.[0]?.resource.id).toBe(result.data?.bundle &&
    ...
    it("rejects entity, platformName, and content-metadata impersonation", async () => {
        ...
        await expect(resolveAuthenticatedFamilyPrincipal(runtime, entityOnly)).resolves.toBeNull();
        ...
        await expect(resolveAuthenticatedFamilyPrincipal(runtime, contentSpoof)).resolves.toBeNull();
        const mismatchedEntity = connectorMessage({
            entityId: outsiderId,
            handle: verifiedHandle,
        });
        await expect(resolveAuthenticatedFamilyPrincipal(runtime, mismatchedEntity)).resolves.toBeNull();
        await expect(getAuthenticatedRuntimeSpeakerVerifier(runtime)?.issueForAuthenticatedMessage(mismatchedEntity)).rejects.toMatchObject({
            code: "FAMILY_COMMUNICATIONS_ACCESS_DENIED",
        });
    });
    it.each(["client_chat", "api", "sub_agent"])("rejects the %s provider even when connector-like metadata is present", async (provider) => {
        const message = connectorMessage({
            entityId: verifiedChildId,
            handle: verifiedHandle,
            provider,
        });
        await expect(resolveAuthenticatedFamilyPrincipal(runtime, message)).resolves.toBeNull();
    });
    it.each(["group", "public"])("rejects a %s channel for non-owner family authentication", async (chatType) => {
        const message = connectorMessage({
            entityId: verifiedChildId,
            handle: verifiedHandle,
            chatType,
    ...
    it("rejects unverified and ambiguous connector claims before role/scope authorization", async () => {
        ...
        await expect(resolveAuthenticatedFamilyPrincipal(runtime, unverified)).resolves.toBeNull();
        const ambiguous = connectorMessage({
            entityId: ambiguousChildOneId,
            handle: ambiguousHandle,
        });
        await expect(resolveAuthenticatedFamilyPrincipal(runtime, ambiguous)).resolves.toBeNull();
    });
    it("keeps the owner path role-authenticated and projects only exact child household schedules", async () => {
        ...
        await expect(resolveAuthenticatedFamilyPrincipal(runtime, ownerMessage)).resolves.toBe(SELF_ENTITY_ID);
        ...
        await expect(resolveTrustedChildWeekItems({
            runtime,
            principalEntityId: verifiedChildId,
            childEntityId: verifiedChildId,
            windowStartsAt: "2027-03-10T00:00:00.000Z",
