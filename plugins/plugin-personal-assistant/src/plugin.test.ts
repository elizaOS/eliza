...
import { ensureLifeOpsGooglePluginRegistered } from "./plugin.js";
import { ensureLifeOpsPdfPluginRegistered } from "./plugin.js";
...
describe("LifeOps Google plugin registration", () => {
    ...
    it("does not infer a private LifeOps audience from sender-role metadata", async () => {
        ...
        const result = await lifeOpsProvider.get({ agentId: "agent", reportError: vi.fn() } as unknown as IAgentRuntime, {
            entityId: "owner",
            agentId: "agent",
            roomId: "room",
            content: {
    ...
    it("declares plugin-google-workspace for app and route plugin dependency resolution", () => {
        expect(personalAssistantPlugin.dependencies).toContain("@elizaos/plugin-google-workspace");
        expect(personalAssistantRoutesPlugin.dependencies).toContain("@elizaos/plugin-google-workspace");
    });
    it("declares and registers the PDF service required by LifeOps document workflows", async () => {
        expect(personalAssistantPlugin.dependencies).toContain("@elizaos/plugin-pdf");
        const { runtime, plugins, registerPlugin } = createRuntimeWithPluginRegistration();
    ...
    it("registers generic Google connector routes without legacy LifeOps setup routes", () => {
        const routePaths = (personalAssistantRoutesPlugin.routes ?? []).map((route) => route.path);
