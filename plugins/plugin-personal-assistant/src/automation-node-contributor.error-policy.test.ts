...
function makeRuntime(registry: unknown): {
    ...
    const runtime = {
        getService: (serviceType: string) => serviceType === "eliza_permissions_registry" ? registry : null,
        reportError,
...
describe("resolveNativeCalendarPermission error policy", () => {
    ...
    it("reports the error and returns null when both the live check and the cached read fail", async () => {
        ...
        expect(reportError).toHaveBeenCalledWith("PersonalAssistant.calendarPermission", getError, { via: "check+get" });
    });
