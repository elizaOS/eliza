...
import { personalAssistantRoutesPlugin } from "./plugin.js";
import { requireLifeOpsRouteOwnerAdminAccess } from "./plugin.js";
...
function createRequest(url: string, headers: http.IncomingHttpHeaders = {}, options: {
    method?: string;
...
function createRuntime(options?: {
    ...
    return {
        agentId: "agent-1",
        getSetting: vi.fn((key: string) => key === "ELIZA_ADMIN_ENTITY_ID" ? (ownerId ?? undefined) : undefined),
...
function findRoute(type: Route["type"], path: string): Route & {
    handler: NonNullable<Route["handler"]>;
} {
    const route = personalAssistantRoutesPlugin.routes?.find((candidate) => candidate.type === type && candidate.path === path);
...
describe("LifeOps raw route owner/admin gate", () => {
    ...
    it("allows trusted local UI calls without an actor header", async () => {
        const res = createResponse();
        const allowed = await requireLifeOpsRouteOwnerAdminAccess({
            req: createRequest("/api/lifeops/app-state", {}, { remoteAddress: "127.0.0.1", host: "localhost:3000" }),
            res,
    ...
    it("denies private raw routes for explicit non-admin actors before the route handler runs", async () => {
        ...
        const res = createResponse();
        await route.handler(createRequest("/api/lifeops/app-state", {
            "x-eliza-entity-id": "user-1",
        }) as never, res as never, createRuntime({ roles: { "user-1": "USER" } }) as never);
        expect(res.statusCode).toBe(401);
    ...
    it("mounts every agreement mutation and preview behind the owner gate", () => {
        ...
        expect(findRoute("POST", "/api/lifeops/agreement-uploads").maxBodyBytes).toBe(AGREEMENT_UPLOAD_METADATA_BYTES);
        expect(findRoute("PUT", "/api/lifeops/agreement-uploads/:id/chunks/:index")
            .maxBodyBytes).toBe(AGREEMENT_UPLOAD_CHUNK_BYTES);
        expect(findRoute("POST", "/api/lifeops/agreements/grants").maxBodyBytes).toBeUndefined();
    });
    ...
    it.each([
        ...
        ["GET", "/api/lifeops/agreements/:id/review"],
        ["POST", "/api/lifeops/agreements/:id/review"],
    ] as const)("denies unauthenticated %s %s before accessing family data", async (method, path) => {
        const route = findRoute(method, path);
        const res = createResponse();
        await route.handler(createRequest(path.replace(":packetId", "packet-1").replace(":draftVersion", "1"), {
            "x-eliza-entity-id": "owner-1",
        }, { method }) as never, res as never, createRuntime() as never);
        expect(res.statusCode).toBe(401);
        expect(JSON.parse(res.body)).toEqual({ error: "Unauthorized" });
    });
    it("does not wrap public OAuth callback routes with the owner/admin gate", async () => {
        ...
        const res = createResponse();
        await route.handler(createRequest("/api/connectors/google/oauth/callback", {
            "x-eliza-entity-id": "user-1",
        }) as never, res as never, createRuntime({ roles: { "user-1": "USER" } }) as never);
    ...
    it("passes the frozen staging Cloud tuple after late env and runtime pollution", async () => {
        ...
        vi.mocked(runtime.getSetting).mockImplementation((key: string) => {
            if (key === "ELIZA_ADMIN_ENTITY_ID")
                return "owner-1";
            if (key.includes("API_KEY"))
                return "runtime-production-key";
            if (key.includes("BASE_URL"))
                return "https://api.eliza.app/api/v1";
            if (key.includes("SERVICE_KEY"))
                return "runtime-production-service-key";
            return undefined;
        });
        const route = findRoute("GET", "/api/cloud/features");
        await route.handler(createRequest("/api/cloud/features", {}, { remoteAddress: "127.0.0.1", host: "localhost:3000" }) as never, createResponse() as never, runtime as never);
    ...
    it("retains runtime-first Cloud proxy resolution without dev authority", async () => {
        ...
        vi.mocked(runtime.getSetting).mockImplementation((key: string) => {
            ...
            if (key === "ELIZAOS_CLOUD_SERVICE_KEY")
                return "runtime-service-key";
            return undefined;
        });
        const route = findRoute("GET", "/api/cloud/features");
        await route.handler(createRequest("/api/cloud/features", {}, { remoteAddress: "127.0.0.1", host: "localhost:3000" }) as never, createResponse() as never, runtime as never);
