...
import { getSelfControlStatus } from "@elizaos/plugin-blocker/services/website-blocker/index";
import { hasActiveHarshNoBypassRule } from "../website-blocker/chat-integration/harsh-mode-check.js";
import { isWebsiteBlockedByPolicy } from "@elizaos/plugin-blocker/services/website-blocker/index";
import { listCallerDefinitions } from "../lifeops/domains/definition-authorization.js";
import { logger } from "@elizaos/core";
import { parseSelfControlBlockRequest } from "@elizaos/plugin-blocker/services/website-blocker/index";
import { startSelfControlBlock } from "@elizaos/plugin-blocker/services/website-blocker/index";
import { stopSelfControlBlock } from "@elizaos/plugin-blocker/services/website-blocker/index";
import { syncWebsiteBlockerExpiryTask } from "@elizaos/plugin-blocker/services/website-blocker/index";
...
function buildBlockRequest(body: WebsiteBlockerRequestBody): ReturnType<typeof parseSelfControlBlockRequest> {
    const parameters: {
        websites?: string[] | string;
        durationMinutes?: number | string | null;
    } = {};
...
async function resolveRequiredTasksForHost(runtime: IAgentRuntime, ownerEntityId: string, host: string): Promise<{
    groupKey: string | null;
    requiredTasks: RequiredTaskInfo[];
}> {
    ...
    const definitions: LifeOpsTaskDefinition[] = await listCallerDefinitions(repo, {
        agentId: () => agentId,
    ...
    for (const definition of matchingDefinitions) {
        const occurrences: LifeOpsOccurrence[] = await repo.listOccurrencesForDefinition(agentId, definition.id);
...
export async function handleWebsiteBlockerRoutes(ctx: WebsiteBlockerRouteContext): Promise<boolean> {
    const { req, res, method, pathname, readJsonBody, json, error, runtime, ownerEntityId, } = ctx;
    ...
    if (method === "GET") {
        ...
        if (hostBlocked && runtime) {
            try {
                const tasks = await resolveRequiredTasksForHost(runtime, ownerEntityId ?? defaultOwnerEntityId(runtime), queriedHost);
                ...
                error(res, `Failed to resolve required tasks for blocked host: ${err instanceof Error ? err.message : String(err)}`, 500);
                return true;
            }
        }
        json(res, result);
    ...
    if (method === "DELETE") {
        ...
        if (runtime && (await hasActiveHarshNoBypassRule(runtime))) {
            json(res, {
                success: false,
                error: "A harsh-no-bypass block rule is active. The block cannot be removed manually — it releases only when the rule's gate is fulfilled.",
            }, 423);
            return true;
        }
