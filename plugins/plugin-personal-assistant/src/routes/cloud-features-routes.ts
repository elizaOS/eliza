...
import { ALL_FEATURE_KEYS } from "../lifeops/feature-flags.types.js";
import { CLOUD_LINKED_DEFAULT_ON } from "../lifeops/feature-flags.types.js";
import { createFeatureFlagService } from "../lifeops/feature-flags.js";
import { isCloudLinkedDefaultOnFeatureKey } from "../lifeops/feature-flags.types.js";
import { isLifeOpsFeatureKey } from "../lifeops/feature-flags.types.js";
import { logger } from "@elizaos/core";
import { normalizeCloudApiKey } from "@elizaos/plugin-elizacloud";
import { normalizeCloudSiteUrl } from "@elizaos/plugin-elizacloud";
import { resolveCloudApiKeyWithRuntimeOverride } from "@elizaos/plugin-elizacloud";
import { sendJson } from "@elizaos/core/api/http-helpers";
import { sendJsonError } from "@elizaos/core/api/http-helpers";
import { type AgentRuntime } from "@elizaos/core";
import { type CloudAuthApiKeyService } from "@elizaos/plugin-elizacloud";
import { type CloudProxyConfigLike } from "@elizaos/agent";
import { type FeatureFlagState } from "../lifeops/feature-flags.types.js";
import { type IAgentRuntime } from "@elizaos/core";
import { type LifeOpsFeatureFlagRowDto } from "../lifeops/feature-flags.types.js";
import { type LifeOpsFeatureFlagsResponse } from "../lifeops/feature-flags.types.js";
import { type LifeOpsFeatureFlagsSyncResponse } from "../lifeops/feature-flags.types.js";
import { type LifeOpsFeatureKey } from "../lifeops/feature-flags.types.js";
...
function resolveProxyApiKey(state: CloudFeaturesRouteState): string | null {
    const cloudAuth = state.runtime
        ...
        : null;
    const runtimeApiKey = cloudAuth?.isAuthenticated() === true
...
function parseCloudFeatures(payload: unknown): CloudFeatureRow[] {
    ...
    for (const entry of features) {
        ...
        if (!isLifeOpsFeatureKey(featureKeyRaw))
            continue;
        const enabled = entry.enabled === true;
        const packageId = typeof entry.packageId === "string" && entry.packageId.trim().length > 0
...
export async function fetchCloudFeatures(state: CloudFeaturesRouteState): Promise<FetchCloudFeaturesResult> {
...
async function handleGet(res: http.ServerResponse, state: CloudFeaturesRouteState): Promise<void> {
    if (!state.runtime) {
...
async function handleSync(res: http.ServerResponse, state: CloudFeaturesRouteState): Promise<void> {
    if (!state.runtime) {
        sendJsonError(res, "Runtime not available", 503);
        return;
    }
    ...
    logger.info(`[cloud-features] synced ${remote.rows.length} feature(s) from Eliza Cloud (${promotedKeys.size} promoted by Cloud-default policy)`);
    const list = await service.list();
...
export async function handleCloudFeaturesRoute(_req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, state: CloudFeaturesRouteState): Promise<boolean> {
