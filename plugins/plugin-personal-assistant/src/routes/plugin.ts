...
import { AGREEMENT_UPLOAD_CHUNK_BYTES } from "../lifeops/household/agreement-upload-limits.js";
import { AGREEMENT_UPLOAD_METADATA_BYTES } from "../lifeops/household/agreement-upload-limits.js";
import { DEV_REGISTRIES_ROUTE_PATHS } from "./scheduled-tasks.js";
...
import { ensureSessionForRequest } from "@elizaos/app/api/auth";
import { getCompatApiToken } from "@elizaos/app/api/auth";
import { getProvidedApiToken } from "@elizaos/app/api/auth";
...
import { resolveLifeOpsAuthenticatedPrincipal } from "./authenticated-entity-principal.js";
...
import { tokenMatches } from "@elizaos/app/api/auth";
import { type AgentRuntime } from "@elizaos/core";
import { type HttpPlugin as Plugin } from "@elizaos/shared";
import { type LegacyRouteHandler } from "@elizaos/shared";
import { type LifeOpsAuthenticatedPrincipal } from "./authenticated-entity-principal.js";
...
const requestPrincipals = new WeakMap<http.IncomingMessage, LifeOpsAuthenticatedPrincipal>();
...
function buildLifeOpsContext(req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime | null): LifeOpsRouteContext {
    ...
    return {
        ...
        state: {
            ...
            requestEntityId: requestPrincipals.get(req)?.entityId ?? routeOwnerEntityId(runtime),
...
function buildWebsiteBlockerContext(req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime | null): WebsiteBlockerRouteContext {
    const method = (req.method ?? "GET").toUpperCase();
    const url = new URL(req.url ?? "/", requestBaseUrl(req));
    return {
        req,
...
function runtimeSetting(runtime: AgentRuntime | null, key: string): string | undefined {
    const value = runtime?.getSetting?.(key);
    return typeof value === "string" && value.trim() ? value.trim() : undefined;
}
function buildCloudProxyConfig(runtime: AgentRuntime | null): CloudProxyConfigLike {
    if (resolveDevCloudEnvAuthority()) {
        ...
        return {
            cloud: {
                apiKey: authorityValue("ELIZAOS_CLOUD_API_KEY", "ELIZA_CLOUD_API_KEY", "ELIZACLOUD_API_KEY"),
                baseUrl: authorityValue("ELIZAOS_CLOUD_BASE_URL", "ELIZA_CLOUD_BASE_URL", "ELIZA_CLOUD_URL", "ELIZAOS_CLOUD_URL"),
                serviceKey: authorityValue("ELIZAOS_CLOUD_SERVICE_KEY", "ELIZA_CLOUD_SERVICE_KEY"),
            },
        };
    }
    return {
        cloud: {
            apiKey: runtimeSetting(runtime, "ELIZAOS_CLOUD_API_KEY") ??
                process.env.ELIZAOS_CLOUD_API_KEY,
            baseUrl: runtimeSetting(runtime, "ELIZAOS_CLOUD_BASE_URL") ??
                process.env.ELIZAOS_CLOUD_BASE_URL,
            serviceKey: runtimeSetting(runtime, "ELIZAOS_CLOUD_SERVICE_KEY") ??
...
const LIFEOPS_STATIC_ROUTES: RouteSpec[] = [
    ...
    { type: "POST", path: "/api/lifeops/money/plaid/sync" },
    {
        ...
        publicReason: "Plaid delivers Item/transaction webhooks directly to the callback URL registered at link time; Plaid cannot hold the local gate token.",
        publicWrite: "Authenticated out-of-band by the Plaid-Verification ES256 JWT: signature against Plaid's JWK (kid lookup through Eliza Cloud), exact raw-body SHA-256 pinning, and a bounded iat freshness window — all verified before any lookup or state change.",
...
const LIFEOPS_DYNAMIC_ROUTES: RouteSpec[] = [
    ...
    },
    {
        ...
        publicReason: "Health connector OAuth callbacks must accept provider redirects.",
    },
    {
        type: "GET",
        ...
        publicReason: "Health connector OAuth success landing must render after provider redirects.",
    },
...
const GOOGLE_CONNECTOR_ACCOUNT_ROUTES: RouteSpec[] = [
    ...
    },
    {
        ...
        publicReason: "Google OAuth callback POST must accept provider redirect exchanges.",
        publicWrite: "Provider redirect exchange POST authenticated by the OAuth state/code, not the local gate.",
    },
...
function withAuthenticatedEntityGate(handler: LegacyRouteHandler): LegacyRouteHandler {
    return async (req: unknown, res: unknown, runtime: unknown): Promise<void> => {
        const httpReq = req as http.IncomingMessage;
        const httpRes = res as http.ServerResponse;
        const agentRuntime = (runtime as AgentRuntime) ?? null;
        ...
        const principal = resolved.principal.kind === "owner"
            ? {
                ...resolved.principal,
                entityId: String(routeOwnerEntityId(agentRuntime) ?? SELF_ENTITY_ID),
            }
...
function buildRawRoutes(specs: readonly RouteSpec[], handler: LegacyRouteHandler): Route[] {
    return specs.map((spec): Route => {
        ...
        return {
            ...
            ...(spec.maxBodyBytes === undefined
                ? {}
                : { maxBodyBytes: spec.maxBodyBytes }),
            handler: spec.access === "authenticated_entity"
...
function lifeOpsRouteHandler(): LegacyRouteHandler {
    return async (req: unknown, res: unknown, runtime: unknown): Promise<void> => {
        const httpReq = req as http.IncomingMessage;
        const httpRes = res as http.ServerResponse;
        const ctx = buildLifeOpsContext(httpReq, httpRes, (runtime as AgentRuntime) ?? null);
        if (await handleEntityRoutes(ctx))
            return;
        if (await handleRelationshipRoutes(ctx))
            return;
        if (await handleAgreementKnowledgeRoutes(ctx))
            return;
...
function websiteBlockerRouteHandler(): LegacyRouteHandler {
    return async (req: unknown, res: unknown, runtime: unknown): Promise<void> => {
        const httpReq = req as http.IncomingMessage;
        const httpRes = res as http.ServerResponse;
        const ctx = buildWebsiteBlockerContext(httpReq, httpRes, (runtime as AgentRuntime) ?? null);
...
function cloudFeaturesRouteHandler(): LegacyRouteHandler {
    return async (req: unknown, res: unknown, runtime: unknown): Promise<void> => {
        ...
        const { handleCloudFeaturesRoute } = await import("./cloud-features-routes.js");
...
function travelProviderRelayRouteHandler(): LegacyRouteHandler {
    return async (req: unknown, res: unknown, runtime: unknown): Promise<void> => {
        ...
        const method = (httpReq.method ?? "GET").toUpperCase();
        const url = new URL(httpReq.url ?? "/", requestBaseUrl(httpReq));
        const { handleTravelProviderRelayRoute } = await import("@elizaos/plugin-elizacloud/routes/travel-provider-relay-routes");
        await handleTravelProviderRelayRoute(httpReq, httpRes, url.pathname, method, {
            config: buildCloudProxyConfig(agentRuntime),
            runtime: agentRuntime,
        });
    };
...
const lifeOpsPluginRoutes: Route[] = [
    ...
    ...buildRawRoutes(TRAVEL_PROVIDER_RELAY_ROUTES, travelProviderRelayRouteHandler()),
    ...buildRawRoutes(GOOGLE_CONNECTOR_ACCOUNT_ROUTES, googleConnectorAccountRouteHandler()),
...
export const personalAssistantRoutesPlugin: Plugin = {
    ...
    description: "LifeOps dashboard, Google Workspace, website blocker, and scheduling routes",
