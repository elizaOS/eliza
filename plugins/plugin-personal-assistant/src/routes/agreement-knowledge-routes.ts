...
import { acceptAgreementChunk } from "../lifeops/household/agreement-upload-session.js";
import { agreementUploadView } from "../lifeops/household/agreement-upload-session.js";
import { beginAgreementUpload } from "../lifeops/household/agreement-upload-session.js";
import { commitAgreementUpload } from "../lifeops/household/agreement-upload-session.js";
...
import { readAgreementUpload } from "../lifeops/household/agreement-upload-session.js";
import { readRequestBodyBuffer } from "@elizaos/core/api/http-helpers";
import { type LifeOpsRouteContext } from "./lifeops-routes.js";
import { type ParentingAgreementArtifact } from "../lifeops/household/agreement-knowledge.js";
type JsonObject = Record<string, unknown>;
function record(value: unknown): JsonObject {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
        throw new AgreementKnowledgeError("Request body must be a JSON object", "AGREEMENT_INVALID_CONTRACT");
    }
...
function stringField(body: JsonObject, field: string): string {
    ...
    if (typeof value !== "string" || !value.trim()) {
        throw new AgreementKnowledgeError(`${field} is required`, "AGREEMENT_INVALID_CONTRACT", { field });
    }
    return value.trim();
}
function numberField(body: JsonObject, field: string): number {
    ...
    if (typeof value !== "number" || !Number.isSafeInteger(value)) {
        throw new AgreementKnowledgeError(`${field} must be an integer`, "AGREEMENT_INVALID_CONTRACT", { field });
    }
    return value;
}
...
export async function handleAgreementKnowledgeRoutes(ctx: LifeOpsRouteContext): Promise<boolean> {
    if (!ctx.pathname.startsWith("/api/lifeops/agreements") &&
    ...
    try {
        ...
        const uploadStatus = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreement-uploads\/([^/]+)$/);
        if (ctx.method === "GET" && uploadStatus) {
            const manifest = await readAgreementUpload(runtime, uploadStatus[0] ?? "");
            ctx.json(ctx.res, { upload: agreementUploadView(manifest) });
            return true;
        }
        const uploadChunk = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreement-uploads\/([^/]+)\/chunks\/(\d+)$/);
        if (ctx.method === "PUT" && uploadChunk) {
            const bytes = await readRequestBodyBuffer(ctx.req, {
                maxBytes: AGREEMENT_UPLOAD_CHUNK_BYTES,
            });
            if (!bytes) {
                throw new AgreementKnowledgeError("Agreement chunk body is required", "AGREEMENT_INVALID_CONTRACT");
            }
            const shaHeader = ctx.req.headers["x-chunk-sha256"];
            const sha256 = Array.isArray(shaHeader) ? shaHeader[0] : shaHeader;
            if (!sha256) {
                throw new AgreementKnowledgeError("X-Chunk-Sha256 is required", "AGREEMENT_INVALID_CONTRACT");
            }
        ...
        const uploadCommit = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreement-uploads\/([^/]+)\/commit$/);
        if (ctx.method === "POST" && uploadCommit) {
            ...
            ctx.json(ctx.res, { artifact: committed.artifact }, committed.created ? 201 : 200);
            return true;
        }
        const ownerReview = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/([^/]+)\/review$/);
        if (ownerReview && (ctx.method === "GET" || ctx.method === "POST")) {
            const input = {
                artifactId: ownerReview[0] ?? "",
                ownerEntityId: SELF_ENTITY_ID,
            };
            const review = ctx.method === "POST"
        ...
        const artifactExport = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/([^/]+)\/export$/);
        if (ctx.method === "POST" && artifactExport) {
            ...
            ctx.res.setHeader("Content-Disposition", `attachment; filename*=UTF-8''${encodeURIComponent(file.fileName)}`);
            ctx.res.setHeader("Content-Length", String(file.bytes.length));
            ctx.res.end(file.bytes);
            return true;
        }
        const artifactDownload = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/([^/]+)\/download$/);
        ...
        const sharedRead = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/([^/]+)\/shared$/);
        if (ctx.method === "GET" && sharedRead) {
            ...
            if (!principalEntityId ||
                ...
                throw new AgreementKnowledgeError("A verified guest session is required for the shared agreement view", "AGREEMENT_ACCESS_DENIED");
            }
        ...
        const guestOptions = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/([^/]+)\/guest-options$/);
        if (ctx.method === "GET" && guestOptions) {
            ctx.res.setHeader("Cache-Control", "private, no-store, max-age=0");
            ctx.res.setHeader("Referrer-Policy", "no-referrer");
            ctx.json(ctx.res, await service.listGuestAccessOptions({
                artifactId: guestOptions[0] ?? "",
                ownerEntityId: SELF_ENTITY_ID,
            }));
            return true;
        }
        const artifactRead = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/([^/]+)$/);
        ...
        const guestProjection = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/([^/]+)\/guest-projection$/);
        if (ctx.method === "GET" && guestProjection) {
            const principalEntityId = ctx.url.searchParams.get("principalEntityId");
            if (!principalEntityId) {
                throw new AgreementKnowledgeError("principalEntityId is required", "AGREEMENT_INVALID_CONTRACT", { field: "principalEntityId" });
            }
            const agreement = await service.readFor({
        ...
        const obligationCreate = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/([^/]+)\/obligations$/);
        ...
        const decision = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/obligations\/([^/]+)\/decision$/);
        if (ctx.method === "POST" && decision) {
            ...
            if (requestedDecision !== "approve" && requestedDecision !== "reject") {
                throw new AgreementKnowledgeError("decision must be approve or reject", "AGREEMENT_INVALID_CONTRACT", { decision: requestedDecision });
            }
        ...
        const pins = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/([^/]+)\/pins$/);
        ...
        if (pins && ctx.method === "POST") {
            ...
            if (targetType !== "agent" && targetType !== "chat") {
                throw new AgreementKnowledgeError("targetType must be agent or chat", "AGREEMENT_INVALID_CONTRACT", { targetType });
            }
        ...
        const unpin = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/pins\/([^/]+)$/);
        ...
        const revoke = pathMatch(ctx.pathname, /^\/api\/lifeops\/agreements\/grants\/([^/]+)\/revoke$/);
