...
async function verifiedEntityForAuthIdentity(runtime: AgentRuntime, authIdentityId: string): Promise<string | null> {
    ...
    const exact = candidates.filter((candidate) => candidate.entity.identities.some((identity) => identity.platform === AUTH_SESSION_ENTITY_PLATFORM &&
...
export async function entityHasVerifiedMachineAuthBinding(runtime: AgentRuntime, entityId: string): Promise<boolean> {
    ...
    for (const identity of entity.identities) {
        if (identity.platform !== AUTH_SESSION_ENTITY_PLATFORM ||
            identity.connectorAccountId !== AUTH_SESSION_CONNECTOR_ACCOUNT ||
            !identity.verified) {
            continue;
        }
...
export async function resolveLifeOpsAuthenticatedPrincipal(args: {
    ...
    const entityId = await verifiedEntityForAuthIdentity(args.runtime, resolution.identityId);
    if (!entityId) {
        return {
            ok: false,
            status: 403,
...
export async function bindMachineAuthIdentityToEntity(args: {
    ...
    const existing = await verifiedEntityForAuthIdentity(args.runtime, args.authIdentityId);
    ...
    const retained = entity.identities.filter((identity) => !(identity.platform === AUTH_SESSION_ENTITY_PLATFORM &&
        identity.connectorAccountId === AUTH_SESSION_CONNECTOR_ACCOUNT &&
