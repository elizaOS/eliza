...
export const notesAction: Action = {
  ...
  description:
    "Durable notes the user can write and read back. action=create writes a note from one content field; action=list reads them, narrowed by content when supplied; action=update replaces the complete note found by its text; action=delete removes one found by its text. The first line is the note's label and later lines are its body. For a partial edit, preserve the other content, including the existing label, in the replacement. NOTES changes data, not the visible view: an explicit request to also open Notes needs its own navigation action (prefer VIEWS_SHOW when available).",
  descriptionCompressed:
    "notes: create, list/search, update full content (preserve unedited label/body), delete; opening the Notes view separately uses VIEWS_SHOW when available, otherwise VIEWS",
  routingHint:
