...
import { NOTES_SERVICE_TYPE } from "../service.js";
...
import { notesStateFilePath } from "../store.js";
import { promises as fs } from "node:fs";
import { serverInteract } from "../interact.js";
import { stringToUuid } from "@elizaos/core";
import { type IAgentRuntime } from "@elizaos/core";
import { type NotesInteractResult } from "../interact.js";
...
function expectAppliedMutationReceipt(result: NotesInteractResult, capability: string, resource: {
    kind: string;
    id: string;
}): void {
    if (!result.state)
        throw new Error("Mutation result state is required.");
    expect(result.effectReceipts).toHaveLength(1);
    const receipt = result.effectReceipts?.[0];
    if (!receipt)
        throw new Error("Mutation effect receipt is required.");
...
class ConnectorSetupTestService extends Service {
    ...
    override capabilityDescription = "Captures Notes websocket broadcasts in the backend harness.";
    readonly broadcasts: object[] = [];
    static override async start(runtime: IAgentRuntime): Promise<ConnectorSetupTestService> {
...
async function runtimeFor(service: NotesService): Promise<AgentRuntime> {
    ...
    class BoundNotesService extends NotesService {
        static override async start(_runtime: IAgentRuntime): Promise<NotesService> {
            return service;
        }
    }
...
function route(type: Route["type"], pathValue: string): Route {
    const match = notesRoutes.find((candidate) => candidate.type === type && candidate.path === pathValue);
    if (!match?.routeHandler) {
        throw new Error(`Missing ${type} ${pathValue} route handler.`);
    }
    return match;
}
async function invokeRoute(routeValue: Route, runtime: IAgentRuntime, options: {
    body?: unknown;
    params?: Record<string, string>;
    query?: Record<string, string | string[]>;
} = {}): Promise<RouteHandlerResult> {
    if (!routeValue.routeHandler)
...
describe("NotesStore", () => {
    it("persists one document and restores notes after restart", async () => {
        ...
        expect(Object.keys(persisted).sort()).toEqual(["notes", "persistedAt", "revision", "schemaVersion"].sort());
    });
    it("serializes concurrent writes without losing a mutation", async () => {
        const filePath = await temporaryStateFile();
        const service = await serviceFor(filePath);
        await Promise.all(Array.from({ length: 24 }, (_, index) => service.createNote({
    ...
    it("scopes default stores per agent and shares serialization for duplicate runtime construction", async () => {
        ...
        await Promise.all([
            ...Array.from({ length: 12 }, (_, index) => first.createNote({ title: `First ${index}` })),
            ...Array.from({ length: 12 }, (_, index) => duplicate.createNote({ title: `Duplicate ${index}` })),
            isolated.createNote({ title: "Other agent" }),
        ]);
        expect(first.store.filePath).toBe(duplicate.store.filePath);
        expect(first.store.filePath).toBe(notesStateFilePath(stateDir, testAgentId("agent-a")));
        expect(isolated.store.filePath).toBe(notesStateFilePath(stateDir, testAgentId("agent-b")));
    ...
    it("ignores malformed unscoped workbench state instead of importing it", async () => {
        ...
        await fs.writeFile(legacyPath, JSON.stringify({ schemaVersion: 1, revision: "not-a-number" }), "utf8");
...
describe("Notes capabilities", () => {
    it("dispatches server capabilities to the owning runtime service", async () => {
        ...
        await expect(serverInteract("create-note", { content: "First runtime\nA" }, { runtime: firstRuntime })).resolves.toMatchObject({ success: true });
        await expect(serverInteract("create-note", { content: "Second runtime\nB" }, { runtime: secondRuntime })).resolves.toMatchObject({ success: true });
    ...
    it("preserves colon punctuation through the create-note capability", async () => {
        ...
        const created = await interact("create-note", { content: "Demo Checklist: mic, charger, water" }, service);
    ...
    it("drives full note CRUD against the durable service", async () => {
        const service = await serviceFor(await temporaryStateFile());
        const createdNote = await interact("create-note", { content: "Workbench\nFirst draft", color: "yellow" }, service);
        expect(createdNote).toMatchObject({ success: true });
        const noteId = service.listNotes()[0]?.id;
        if (!noteId)
            throw new Error("Created note id is required.");
        expectAppliedMutationReceipt(createdNote, "create-note", {
            kind: "notes.note",
            id: noteId,
        });
        await expect(interact("get-notes", { query: "Workbench" }, service)).resolves.toMatchObject({
            success: true,
            data: { notes: [{ id: noteId, title: "Workbench" }] },
        });
        const originalSnapshot = service.snapshot();
        const updatedNote = await interact("update-note", {
        ...
        await expect(interact("update-note", {
            expectedRevision: polishedSnapshot.revision,
            query: "Workbench",
            content: "Workbench ready\nPolished draft",
        }, service)).resolves.toMatchObject({ success: true });
        const readNote = await interact("get-note", { query: "Workbench ready" }, service);
        expect(readNote).toMatchObject({
            success: true,
            data: { note: { id: noteId, title: "Workbench ready" } },
        });
        expect(readNote.effectReceipts).toBeUndefined();
        const deletedNote = await interact("delete-note", { query: "polished" }, service);
        ...
        const clearedNotes = await interact("clear-notes", { confirm: true, expectedRevision: service.snapshot().revision }, service);
    ...
    it("returns replay and multi-record outcomes for one logical note", async () => {
        const service = await serviceFor(await temporaryStateFile());
        const first = await interact("create-note", { content: "Milk\nBuy oat milk", color: "green" }, service);
        const note = (first.data as {
            note: StickyNote;
        }).note;
        const beforeReplay = service.snapshot();
        const persistedBeforeReplay = await fs.readFile(service.store.filePath, "utf8");
        const replay = await interact("create-note", { content: "Milk\nBuy oat milk", color: "green" }, service);
        ...
        expect(await fs.readFile(service.store.filePath, "utf8")).toBe(persistedBeforeReplay);
        await service.store.transact((draft) => {
            draft.notes.push({ ...note, id: "legacy-milk-2" }, { ...note, id: "legacy-milk-3" });
        });
        expect(service.snapshot().notes).toHaveLength(1);
        const originalSnapshot = service.snapshot();
        const updated = await interact("update-note", {
            expectedRevision: originalSnapshot.revision,
            query: "Milk",
            content: "Milk\nAlready bought",
        }, service);
        ...
        await service.store.transact((draft) => {
            ...
            if (!current)
                throw new Error("Updated note is required");
            draft.notes.push({ ...current, id: "legacy-updated-copy" });
        });
        const deleted = await interact("delete-note", { query: "Already bought" }, service);
    ...
    it("requires structural confirmation before clear-notes mutates the collection", async () => {
        const service = await serviceFor(await temporaryStateFile());
        await interact("create-note", { content: "Keep me", color: "yellow" }, service);
        ...
        await expect(interact("clear-notes", { confirm: false, expectedRevision: revision }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
        await expect(interact("clear-notes", { confirm: "true", expectedRevision: revision }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
        await expect(interact("clear-notes", { confirm: true }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
        await expect(interact("clear-notes", { confirm: true, expectedRevision: revision - 1 }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
        await expect(interact("clear-notes", { confirm: true, expectedRevision: revision, query: "Keep" }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
        expect(service.listNotes()).toHaveLength(1);
        const cleared = await interact("clear-notes", { confirm: true, expectedRevision: revision }, service);
    ...
    it("rejects stale clear-notes confirmation after another mutation", async () => {
        const service = await serviceFor(await temporaryStateFile());
        await interact("create-note", { content: "First", color: "yellow" }, service);
        const staleRevision = service.snapshot().revision;
        await interact("create-note", { content: "Second", color: "green" }, service);
        await expect(interact("clear-notes", { confirm: true, expectedRevision: staleRevision }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
        expect(service.listNotes()).toHaveLength(2);
        const cleared = await interact("clear-notes", { confirm: true, expectedRevision: service.snapshot().revision }, service);
        expect(cleared).toMatchObject({
            success: true,
            data: { cleared: 2 },
        });
    ...
    it("rejects a clear confirmed against a revision a concurrent create advances (issue #22122)", async () => {
        ...
        const clear = interact("clear-notes", { confirm: true, expectedRevision: confirmedRevision }, service);
    ...
    it("persists intentional clear-notes across service restart", async () => {
        const filePath = await temporaryStateFile();
        const first = await serviceFor(filePath);
        await interact("create-note", { content: "Persisted", color: "slate" }, first);
        const revision = first.snapshot().revision;
        await interact("clear-notes", { confirm: true, expectedRevision: revision }, first);
        await first.stop();
        const restarted = await serviceFor(filePath);
    ...
    it("fails closed when a title lookup is missing or ambiguous", async () => {
        const service = await serviceFor(await temporaryStateFile());
        await interact("create-note", { content: "Daily plan\nMorning", color: "yellow" }, service);
        await interact("create-note", { content: "Daily plan\nEvening", color: "rose" }, service);
        const originalSnapshot = service.snapshot();
        await expect(interact("update-note", {
            expectedRevision: originalSnapshot.revision,
            query: "Daily plan",
    ...
    it("delete-note with title selector deletes by exact first-line label", async () => {
        const service = await serviceFor(await temporaryStateFile());
        await interact("create-note", { content: "Shopping list\nMilk and eggs", color: "yellow" }, service);
        await interact("create-note", { content: "Todo\nFix the bug", color: "rose" }, service);
        // Exact title match deletes the right note.
        const deleted = await interact("delete-note", { title: "Shopping list" }, service);
        expect(deleted).toMatchObject({
            success: true,
    ...
    it("delete-note with title selector fails on ambiguous or unknown label", async () => {
        const service = await serviceFor(await temporaryStateFile());
        await interact("create-note", { content: "Meeting notes\nMorning sync", color: "yellow" }, service);
        await interact("create-note", { content: "Meeting notes\nAfternoon review", color: "rose" }, service);
        // Two notes share the same first-line label -> ambiguous.
        await expect(interact("delete-note", { title: "Meeting notes" }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_AMBIGUOUS_NOTE" },
        });
        // No note has this label -> not found.
        await expect(interact("delete-note", { title: "Nonexistent" }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_NOT_FOUND" },
        });
        // Nothing was deleted.
    ...
    it("returns the destructive name-mismatch fence as a structured failure", async () => {
        const service = await serviceFor(await temporaryStateFile());
        await interact("create-note", { content: "wifi password\nhunter2-not-really", color: "yellow" }, service);
        await expect(interact("delete-note", {
            title: "wifi password",
    ...
    it("delete-note rejects content parameter (fail-closed for payload contract)", async () => {
        const service = await serviceFor(await temporaryStateFile());
        await interact("create-note", { content: "Test note\nBody", color: "yellow" }, service);
        // content is not a declared delete selector — the boundary rejects
        // it with NOTES_VALIDATION_FAILED (fail-closed).
        await expect(interact("delete-note", { content: "Test note" }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
        expect(service.listNotes()).toHaveLength(1);
    ...
    it("get-note with title selector fails on ambiguous or unknown label", async () => {
        ...
        await interact("create-note", { content: "Meeting notes\nAfternoon review", color: "rose" }, service);
        // Two notes share the same first-line label -> ambiguous.
        await expect(interact("get-note", { title: "Meeting notes" }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_AMBIGUOUS_NOTE" },
        });
        // No note has this label -> not found.
        await expect(interact("get-note", { title: "Nonexistent" }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_NOT_FOUND" },
        });
    });
    it("get-note enforces exactly-one-of id/title/query", async () => {
        const service = await serviceFor(await temporaryStateFile());
        await interact("create-note", { content: "Test\nBody", color: "yellow" }, service);
        // Providing both title and query must fail validation.
        await expect(interact("get-note", { title: "Test", query: "Body" }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
    ...
    it("update-note with title selector updates by exact first-line label", async () => {
        ...
        expect(service.listNotes().map((n) => ({ title: n.title, body: n.body }))).toEqual(expect.arrayContaining([
    ...
    it("update-note with title selector fails on ambiguous or unknown label", async () => {
        ...
        // Nothing was mutated.
        expect(service.listNotes().map((n) => n.body)).toEqual(expect.arrayContaining(["Morning sync", "Afternoon review"]));
    });
    it("update-note enforces exactly-one-of id/title/query for selector", async () => {
        ...
        await expect(interact("update-note", { expectedRevision: originalSnapshot.revision, content: "Changed" }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
    });
    it("returns explicit failures for invalid input and rejects undeclared capabilities", async () => {
        const service = await serviceFor(await temporaryStateFile());
        await expect(interact("create-note", { content: "   " }, service)).resolves.toMatchObject({
            success: false,
            error: { code: "NOTES_VALIDATION_FAILED" },
        });
        expect(service.listNotes()).toEqual([]);
        await expect(interact("launch-confetti", {}, service)).rejects.toMatchObject({
...
describe("Notes authenticated routes", () => {
    ...
    it("returns unavailable instead of fabricating an empty snapshot", async () => {
        ...
        const runtime = await runtimeFor(service);
        await service.stop();
        const unavailable = await invokeRoute(route("GET", "/api/notes/state"), runtime);
