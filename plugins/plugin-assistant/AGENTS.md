...
Personal add_directive/remove_directive operations target only the requester; omission of scope cannot turn them into global changes. Removal requires the complete exact stored directive, preserving unrelated rules, traits and other slots. Store mutation and optional per-directive provenance checks share the slot lock; unknown legacy provenance is not inferred authority. Planner-owned directive changes return durable receipts and deferred reply grounding instead of a premature action callback.
