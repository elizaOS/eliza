/** Resolves conversational entity references and renders authorized entity context for assistant prompts. */
import { ElizaError } from "@elizaos/core";
import { ModelType } from "@elizaos/core";
import { composePrompt } from "@elizaos/shared";
import { logger } from "@elizaos/core";
import { memoizeTurnWork } from "@elizaos/core";
import { normalizeEntityMatchesStrict } from "./entity-matches";
import { readEntityResolutionField } from "./entity-matches";
import { stableStringify } from "@elizaos/core";
import { type Entity } from "@elizaos/core";
import { type EntityMatch } from "./entity-matches";
...
import { withVisibleComponents } from "@elizaos/core";
type EntityDetailsRecord = Pick<Entity, "id" | "agentId" | "names" | "metadata"> & {
    name?: string;
    data: string;
};
...
function parseEntityResolutionResponse(response: unknown): (ParsedResolution & {
    type?: string;
    entityId?: string;
}) | null {
    if (!response)
    ...
    if (parsedJson && typeof parsedJson === "object") {
        ...
        const malformedEntityId = (!isAbsentResolutionId(entityIdValue) &&
            !isUsableResolutionId(entityIdValue)) ||
            (!isAbsentResolutionId(resolvedIdValue) &&
                !isUsableResolutionId(resolvedIdValue));
        const conflictingEntityId = isUsableResolutionId(entityIdValue) &&
            isUsableResolutionId(resolvedIdValue) &&
...
function indexEntities(entities: Entity[]): IndexedEntity[] {
    return entities.map((entity) => {
        ...
        for (const component of entity.components ?? []) {
            const username = typeof component.data?.username === "string"
            ...
            const handle = typeof component.data?.handle === "string"
...
function indexedEntityMatches(entry: IndexedEntity, matchName: string, matchKey: string): boolean {
    return (entry.strippedNames.has(matchKey) ||
...
function entitiesMatchingReferent(indexed: IndexedEntity[], referent: string): Entity[] {
...
async function getRecentInteractions(sourceEntityId: UUID, candidateEntities: Entity[], recentMessages: Memory[], relationships: Relationship[]): Promise<{
    entity: Entity;
    interactions: Memory[];
    count: number;
}[]> {
    ...
    for (const entity of candidateEntities) {
        ...
        const directReplies = recentMessages.filter((msg) => {
            ...
            return ((msg.entityId === sourceEntityId && repliedToEntityId === entity.id) ||
                (msg.entityId === entity.id && repliedToEntityId === sourceEntityId));
        });
        interactions.push(...directReplies);
        const relationship = relationships.find((rel) => (rel.sourceEntityId === sourceEntityId &&
...
export async function findEntityByName(runtime: IAgentRuntime, message: Memory, state: State): Promise<Entity | null> {
    ...
    const counterpartIds = relationships.map((rel) => rel.sourceEntityId === message.entityId
        ? rel.targetEntityId
        : rel.sourceEntityId);
    const counterpartById = new Map((counterpartIds.length > 0
    ...
    const filteredEntities = await Promise.all(entitiesInRoom.map((entity) => withVisibleComponents(runtime, world, entity, message.entityId)));
    const filteredRelationshipEntities = await Promise.all(relationshipEntities.map((entity) => withVisibleComponents(runtime, world, entity, message.entityId)));
    ...
    if (contextualTarget) {
        const contextualId = contextualTarget === "sender" ? message.entityId : runtime.agentId;
        const contextualEntity = allEntities.find((entity) => entity.id === contextualId);
        if (contextualEntity) {
            return contextualEntity;
        }
        return null;
    }
    const uniqueReferentHits = entitiesMatchingReferent(indexedEntities, referent);
    ...
    const interactionData = await getRecentInteractions(message.entityId, allEntities, recentMessages, relationships);
    const senderEntity = allEntities.find((entity) => entity.id === message.entityId);
    const prompt = composePrompt({
        state: {
            ...
            relationshipEntities: JSON.stringify(filteredRelationshipEntities, null, 2),
    ...
    const candidateById = new Map(allEntities
    ...
    let invalidated = parsedResolution.malformedMatches === true ||
    ...
    if (suppliedEvidence && !invalidated && evidenceIds.size === 1) {
        ...
        if (resolved) {
            if (resolution.type === "RELATIONSHIP_MATCH") {
                const interactionInfo = interactionData.find((data) => data.entity.id === resolved.id);
...
export async function getEntityDetails({ runtime, roomId, }: {
    runtime: IAgentRuntime;
    roomId: UUID;
}) {
    return memoizeTurnWork(`entity-details:${runtime.agentId}:${roomId}`, async () => {
        ...
        for (const entity of roomEntities) {
            ...
            if (!entityId) {
                throw new ElizaError(`Room ${roomId} contains an entity without an id (names: ${entity.names?.length ? entity.names.join(", ") : "(none)"})`, {
...
function formatEntityNames(names: string[]): string {
    ...
    const renderedNames = uniqueNames.length > 0 ? `"${uniqueNames.join('" aka "')}"` : '"(unnamed)"';
