...
import { CONNECTOR_MESSAGE_RECEIVED_EVENT_TYPES } from "@elizaos/core";
...
import { advancedActions } from "../advanced-capabilities/index.ts";
import { advancedEvaluators } from "../advanced-capabilities/index.ts";
import { advancedProviders } from "../advanced-capabilities/index.ts";
import { advancedServices } from "../advanced-capabilities/index.ts";
...
import { autonomyStatusProvider } from "../autonomy/providers.ts";
import { botAwarenessProvider } from "./providers/botAwareness.ts";
import { bridgeActionCompletedToStreams } from "@elizaos/core";
import { bridgeActionStartedToStreams } from "@elizaos/core";
import { bridgeConnectorMessageReceivedToStreams } from "@elizaos/core";
import { bridgeEvaluatorCompletedToStreams } from "@elizaos/core";
import { bridgeEvaluatorStartedToStreams } from "@elizaos/core";
import { bridgeMessageReceivedToStreams } from "@elizaos/core";
import { bridgeRunEndedToStreams } from "@elizaos/core";
import { bridgeRunStartedToStreams } from "@elizaos/core";
...
import { disableAutonomousModeAction } from "../autonomy/action.ts";
import { enableAutonomousModeAction } from "../autonomy/action.ts";
...
import { imageDescriptionTemplate } from "./prompts.js";
...
import { platformChatContextProvider } from "./providers/platformContext.ts";
import { platformUserContextProvider } from "./providers/platformContext.ts";
import { postCreationTemplate } from "./prompts.js";
...
import { type ActionLogBody } from "@elizaos/core";
import { type BaseLogBody } from "@elizaos/core";
import { type Content } from "@elizaos/core";
import { type ControlMessagePayload } from "@elizaos/core";
import { type EntityPayload } from "@elizaos/core";
import { type EvaluatorEventPayload } from "@elizaos/core";
import { type EventPayload } from "@elizaos/core";
import { type HttpPlugin as Plugin } from "@elizaos/shared";
import { type IAgentRuntime } from "@elizaos/core";
import { type IControlTransportService } from "@elizaos/core";
import { type IMessageBusService } from "@elizaos/core";
import { type InvokePayload } from "@elizaos/core";
import { type JsonValue } from "@elizaos/core";
import { type Media } from "@elizaos/core";
import { type Memory } from "@elizaos/core";
import { type MentionContext } from "@elizaos/core";
import { type MessageMetadata } from "@elizaos/core";
...
import { type WorldPayload } from "@elizaos/core";
...
// ../advanced-capabilities/index.ts for the Bun.build mis-rewrite that
// requires bypassing barrels here too.
export { dedupeHygienicDialogueMessages, isHygienicDialogueMessage, recentMessagesProvider, } from "./providers/recentMessages.ts";
// Re-export advanced capability modules
export * from "../advanced-capabilities/actions/index.ts";
// Re-export advanced capabilities
export { advancedActions, advancedCapabilities, advancedEvaluators, advancedProviders, advancedServices, } from "../advanced-capabilities/index.ts";
...
export { coreCapabilities, secretsCapability, trustCapability, } from "../index.ts";
...
async function readBoundedMediaResponse(response: Response, label: string, url: string): Promise<Buffer> {
    const contentLength = response.headers.get("content-length");
    if (contentLength !== null) {
        const declared = Number(contentLength);
        if (Number.isFinite(declared) && declared > MAX_MEDIA_BYTES) {
            throw new MediaFetchError("max_bytes", `${label} exceeds size limit ${declared} > ${MAX_MEDIA_BYTES}: ${url}`);
        }
    }
...
function textContainsAgentName(text: string | undefined, names: Array<string | null | undefined>): boolean {
    if (!text) {
        return false;
    }
...
export async function fetchMediaData(attachments: Media[]): Promise<MediaData[]> {
    return Promise.all(attachments.map(async (attachment: Media) => {
        if (/^(http|https):\/\//.test(attachment.url)) {
            ...
            try {
                ...
                const mediaBuffer = await readBoundedMediaResponse(response, "Media", attachment.url);
 ...
 */
export async function processAttachments(attachments: Media[] | null | undefined, runtime: IAgentRuntime): Promise<Media[]> {
    ...
    for (const attachment of attachments) {
        ...
        if (attachment.contentType === ContentType.IMAGE &&
            ...
            const resolvedImageDescriptionPrompt = resolveOptimizedPromptForRuntime(runtime, "media_description", imageDescriptionTemplate);
            // Route through the shared content-addressed cache so identical bytes
            // reuse one description across all describe paths.
            const described = await describeImageCached(runtime, imageUrl, resolvedImageDescriptionPrompt);
            if (described) {
                ...
                runtime.logger.debug({
                    ...
                    descriptionPreview: described.description
                        ? truncateWellFormed(toWellFormedUnicode(described.description), 100)
                        : undefined,
                }, "Generated description");
            }
            else {
                runtime.logger.warn({ src: "basic-capabilities", agentId: runtime.agentId }, "Image description unavailable");
            }
        }
            ...
            if (isText) {
                ...
                const textContent = (await readBoundedMediaResponse(res, "Text document", url)).toString("utf8");
                ...
                runtime.logger.debug({
                    ...
                    textPreview: processedAttachment.text
                        ? truncateWellFormed(toWellFormedUnicode(processedAttachment.text), 100)
                        : undefined,
                ...
                const { convertPdfToTextFromBuffer } = await import("../documents/utils.ts");
                const pdfBuffer = await readBoundedMediaResponse(res, "PDF", url);
                const textContent = await convertPdfToTextFromBuffer(pdfBuffer, processedAttachment.title ?? undefined);
                processedAttachment.text = textContent;
                ...
                runtime.logger.warn({ src: "basic-capabilities", agentId: runtime.agentId, contentType }, "Skipping unsupported document type");
            }
        }
...
export function shouldRespond(runtime: IAgentRuntime, message: Memory, room?: Room, mentionContext?: MentionContext): {
    ...
    const respondChannels = new Set([...alwaysRespondChannels.map((t) => t.toString()), ...customChannels].map((s: string) => s.trim().toLowerCase()));
    const respondSources = [...alwaysRespondSources, ...customSources].map((s: string) => s.trim().toLowerCase());
    ...
    const textMentionsAgentByName = textContainsUserTag(message.content.text) &&
    ...
    const hasPlatformMention = !!(mentionContextIsMention || mentionContextIsReply);
...
const postGeneratedHandler = async ({ runtime, callback, worldId, userId, roomId, source }: InvokePayload, attempt = 1) => {
    const safeSource = source ?? "unknown";
    const safeUserId = (userId ?? runtime.agentId) as UUID;
    runtime.logger.info({ src: "basic-capabilities", agentId: runtime.agentId }, "Generating new post");
    ...
    const parsedJsonResponse = parseJSONObjectFromText(structuredResponseText) as PostCreationJson | null;
    ...
    if (RMDataRecentMessages) {
        for (const m of RMDataRecentMessages) {
            if (cleanedText === m.content.text) {
                runtime.logger.info({ src: "basic-capabilities", agentId: runtime.agentId, cleanedText }, "Already recently posted that, retrying");
    ...
    if (oaiRefusalRegex.test(cleanedText) ||
        anthropicRefusalRegex.test(cleanedText) ||
        googleRefusalRegex.test(cleanedText) ||
        generalRefusalRegex.test(cleanedText)) {
        runtime.logger.info({ src: "basic-capabilities", agentId: runtime.agentId, cleanedText }, "Got prompt moderation refusal, retrying");
        if (attempt >= MAX_POST_GENERATION_ATTEMPTS) {
            runtime.logger.warn({ src: "basic-capabilities", agentId: runtime.agentId, cleanedText }, "Post generation retry limit reached for moderation refusals");
            return;
        }
        await postGeneratedHandler({
            runtime,
 ...
 */
export function buildDmWorldMetadata(runtime: IAgentRuntime, entityId: string): Record<string, JsonValue> {
...
const syncSingleUser = async (entityId: UUID, runtime: IAgentRuntime, messageServerId: UUID, channelId: string, type: ChannelType, source: string) => {
 ...
 */
const handleServerSync = async ({ runtime, world, rooms, entities, source, onComplete, }: WorldPayload) => {
    runtime.logger.debug({
        src: "basic-capabilities",
        agentId: runtime.agentId,
...
const controlMessageHandler = async ({ runtime, message, }: ControlMessagePayload) => {
    ...
    const controlTransport = runtime.getService<IControlTransportService>(ServiceType.CONTROL_TRANSPORT);
    if (!controlTransport) {
        runtime.logger.error({ src: "basic-capabilities", agentId: runtime.agentId }, "No control transport service found to send control message");
        return;
    }
...
// ============================================================================
const connectorMessageReceivedEvents = Object.fromEntries(CONNECTOR_MESSAGE_RECEIVED_EVENT_TYPES.map((eventType) => [
    eventType,
    [
...
const events: PluginEvents = {
    ...
    [EventType.ENTITY_JOINED]: [
        async (payload: EntityPayload) => {
            ...
            if (!payload.worldId) {
                payload.runtime.logger.error({ src: "basic-capabilities", agentId: payload.runtime.agentId }, "No worldId provided for entity joined");
                return;
            }
            if (!payload.roomId) {
                payload.runtime.logger.error({ src: "basic-capabilities", agentId: payload.runtime.agentId }, "No roomId provided for entity joined");
                return;
            }
            const payloadMetadata = payload.metadata;
            if (!payloadMetadata?.type) {
                payload.runtime.logger.error({ src: "basic-capabilities", agentId: payload.runtime.agentId }, "No type provided for entity joined");
                return;
            }
            ...
            await syncSingleUser(payload.entityId, payload.runtime, payload.worldId, payload.roomId, channelType as ChannelType, safeSource);
        },
    ],
    ...
    [EventType.ACTION_STARTED]: [
        ...
        async (payload: ActionEventPayload) => {
            ...
            if (payloadContent?.source === MESSAGE_SOURCE_CLIENT_CHAT) {
                const messageBusService = payload.runtime.getService<IMessageBusService>("message-bus-service");
    ...
    [EventType.CONTROL_MESSAGE]: [
        async (payload: ControlMessagePayload) => {
            if (!payload.message) {
                payload.runtime.logger.warn({ src: "basic-capabilities" }, "CONTROL_MESSAGE received without message property");
                return;
            }
 ...
 */
export function createAssistantBehavior(): Plugin {
    return {
        name: "assistant",
        description: "Conversational actions, context, extraction and scheduling",
