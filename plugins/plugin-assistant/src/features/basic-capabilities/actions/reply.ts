...
import { mergeEffectReceipts } from "@elizaos/core";
...
import { resolveUserFacingEffectReceipts } from "@elizaos/core";
import { tagsRequireEffectReceipts } from "@elizaos/core";
...
function parseOption(raw: unknown, qIdx: number, oIdx: number): ReplyQuestionOption | {
    error: string;
} {
    ...
    if (typeof obj.preview === "string")
        out.preview = obj.preview;
    return out;
}
function parseQuestion(raw: unknown, idx: number): ReplyQuestion | {
    error: string;
} {
    if (!raw || typeof raw !== "object") {
...
export const replyAction = {
    ...
    similes: [
        ...(["GREET", "RESPOND", "RESPONSE"] as string[]),
        "ASK",
        "CLARIFY",
    ],
    ...
    parameters: [
        {
            name: "text",
            description: "Reply text. Omit with questions absent to compose from state.",
            required: false,
            schema: { type: "string" },
        },
        {
            name: "questions",
            description: "Every needed structured question: { question, header, options?: [{label, description?, preview?}], multiSelect? }. Returns requiresUserInteraction: true.",
            required: false,
            schema: {
                type: "array",
                items: {
    ...
    validate: async (_runtime: IAgentRuntime, message: Memory, state?: State) => hasActionContext(message, state, {
        contexts: ["general", "messaging"],
    }),
    handler: async (runtime: IAgentRuntime, message: Memory, state?: State, _options?: HandlerOptions, callback?: HandlerCallback, responses?: Memory[]): Promise<ActionResult> => {
        ...
        if (rawQuestions !== undefined) {
            ...
            if (rawQuestions.length < MIN_QUESTIONS ||
                ...
                return {
                    ...
                    error: new Error(`questions must contain ${MIN_QUESTIONS}-${MAX_QUESTIONS} items`),
                };
            }
        ...
        const allTurnEffectReceipts = mergeEffectReceipts(...previousResults.map((result) => result.effectReceipts));
        const mutationResults = previousResults.filter((result) => {
            const actionName = typeof result.data?.actionName === "string"
                ...
                : "";
            if (actionName.toUpperCase() === "REPLY")
                return false;
            const registeredAction = runtime.actions.find((action) => action.name.toUpperCase() === actionName.toUpperCase());
            return (result.effectReceipts !== undefined ||
        ...
        for (const previousResult of previousResults) {
            if ((previousResult.data as {
                actionName?: string;
            } | undefined)?.actionName === "REPLY" ||
                previousResult.userFacingText?.trim() !== text) {
                continue;
            }
            const boundReceipts = resolveUserFacingEffectReceipts(previousResult, allTurnEffectReceipts);
            if (boundReceipts) {
                groundedEffectReceiptIds = boundReceipts.map((receipt) => receipt.receiptId);
                break;
            }
        }
        if (!groundedEffectReceiptIds &&
            (mutationResults.length > 0 || replyClaimsCompletedSideEffect(text))) {
            const guidance = mutationResults.length > 0
