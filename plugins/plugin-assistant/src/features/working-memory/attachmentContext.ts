...
import { VISION_IMAGE_FETCH_TIMEOUT_MS } from "@elizaos/core/media";
import { VISION_IMAGE_MAX_BYTES } from "@elizaos/core/media";
import { buildAccessContext } from "@elizaos/core";
import { describeImageCached } from "@elizaos/core/media";
...
import { readResponseWithLimit } from "@elizaos/core/media";
import { resolveArtifactDisclosure } from "@elizaos/core";
import { selectDisclosedArtifactUrl } from "@elizaos/core";
import { trustedLocalMediaUrl } from "@elizaos/core/media";
...
const NATIVE_ATTACHMENT_REFERENCE = /^attachment:([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}):([0-9a-f]{64})$/i;
...
async function buildAttachmentAccessContext(runtime: IAgentRuntime, message: Memory): Promise<AccessContext | undefined> {
...
function selectAttachmentForRequester(memory: Memory, attachment: AttachmentWithInlineData, accessContext: AccessContext | undefined, agentId: UUID | undefined): AttachmentWithInlineData | null {
    ...
    const disclosure = resolveArtifactDisclosure({
        ...
        scopedEntityId: typeof scopedTo === "string"
            ? (scopedTo as UUID)
    ...
    if (!selected.redacted)
        return attachment;
    const { description: _description, text: _text, thumbnailUrl: _thumbnailUrl, _data, _mimeType, notProcessed: _notProcessed, ...rest } = attachment;
    return {
        ...rest,
        url: selected.url,
        redacted: true,
 ...
 */
async function inlineAttachmentImage(runtime: IAgentRuntime, rawUrl: string): Promise<string> {
    const localUrl = trustedLocalMediaUrl(rawUrl);
    if (!localUrl) {
        if (!/^(http|https):\/\//.test(rawUrl.trim())) {
            throw new MediaFetchError("fetch_failed", `attachment URL is neither a canonical media-store handle nor a remote http(s) URL: ${rawUrl}`);
        }
    ...
    if (!res.ok) {
        throw new MediaFetchError("http_error", `local media-store fetch failed (HTTP ${res.status}) for ${localUrl.pathname}`);
    }
...
async function describeImageAttachment(runtime: IAgentRuntime, attachment: AttachmentWithInlineData): Promise<string> {
    let imageUrl: string | null = null;
    if (typeof attachment._data === "string" &&
    ...
    const described = await describeImageCached(runtime, imageUrl, "Describe this attachment so an agent can reference it later.");
    return (described?.text || described?.description || "").trim();
}
async function readableAttachmentContent(runtime: IAgentRuntime, attachment: AttachmentWithInlineData): Promise<string> {
...
export async function listConversationAttachments(runtime: IAgentRuntime, message: Memory, options: {
    maxLookback?: number;
    ...
    if (!recentMessages ||
        ...
        return currentMessageAttachments
            .map((attachment) => selectAttachmentForRequester(message, {
            ...attachment,
            _createdAt: message.createdAt ?? Date.now(),
            _messageId: message.id,
        }, accessContext, agentId))
            .filter((attachment): attachment is AttachmentWithInlineData => Boolean(attachment));
    }
    const attachmentsById = new Map<string, AttachmentWithInlineData>();
    const rememberAttachment = (attachment: AttachmentWithInlineData, createdAt: number, messageId: UUID | undefined) => {
    ...
    for (const attachment of currentMessageAttachments) {
        const selected = selectAttachmentForRequester(message, attachment, accessContext, agentId);
        if (selected)
    ...
    for (const recentMessage of recentMessages) {
        ...
        for (const attachment of messageAttachments) {
            const selected = selectAttachmentForRequester(recentMessage, attachment, accessContext, agentId);
            if (selected)
                rememberAttachment(selected, createdAt, recentMessage.id);
        }
    }
    return Array.from(attachmentsById.values()).sort((left, right) => (right._createdAt ?? 0) - (left._createdAt ?? 0));
}
export async function resolveAttachmentSelection(_runtime: IAgentRuntime, message: Memory, attachments: Media[]): Promise<string | null> {
    const directId = typeof message.content.attachmentId === "string"
...
export async function readAttachmentRecord(runtime: IAgentRuntime, message: Memory, attachmentId?: string | null): Promise<ReadAttachmentResult | null> {
    const attachments = await listConversationAttachments(runtime, message);
    if (attachments.length === 0) {
        return null;
    }
    const selectedId = attachmentId?.trim() ||
...
export async function readAttachmentRecords(runtime: IAgentRuntime, message: Memory, attachmentId?: string | null): Promise<ReadAttachmentResult[]> {
    ...
    if (nativeReference) {
        ...
        const attachment = ((parent?.content.attachments ?? []) as AttachmentWithInlineData[]).find((candidate) => hashAttachmentIdForLocator(candidate.id) ===
