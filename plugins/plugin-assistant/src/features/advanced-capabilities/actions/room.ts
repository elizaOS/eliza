...
import { findKeywordTermMatch } from "@elizaos/core/i18n/keyword-matching-core";
import { getValidationKeywordTerms } from "@elizaos/core/i18n/keyword-matching-core";
import { logger } from "@elizaos/core";
import { parseBooleanFromText } from "@elizaos/core";
import { setRoomMuteUntil } from "../../../services/message/mute-state.ts";
import { setWorldMuteState } from "../../../services/message/mute-state.ts";
import { shouldFollowRoomTemplate } from "../prompts.js";
import { shouldMuteRoomTemplate } from "../prompts.js";
import { shouldUnfollowRoomTemplate } from "../prompts.js";
import { shouldUnmuteRoomTemplate } from "../prompts.js";
...
import { worldMuteActive } from "../../../services/message/mute-state.ts";
...
const OPS: Record<RoomOp, OpConfig> = {
    ...
    unmute: {
        ...
        startedThought: "I will now unmute this room and start considering it for responses again",
...
function normalizeOp(value: unknown): RoomOp | null {
    ...
    if (normalized === "mute" || normalized === "mute_chat")
        return "mute";
...
function inferOpFromText(text: string): RoomOp | null {
    if (findKeywordTermMatch(text, UNMUTE_TERMS) !== undefined)
        return "unmute";
    if (findKeywordTermMatch(text, MUTE_TERMS) !== undefined)
        return "mute";
    if (findKeywordTermMatch(text, FOLLOW_TERMS) !== undefined)
        return "follow";
    return null;
}
...
async function validateRoomOpAvailability(runtime: IAgentRuntime, message: Memory, _state?: State, options?: HandlerOptions, forcedOp?: RoomOp): Promise<boolean> {
    ...
    const current = (await runtime.getParticipantUserState(roomId, runtime.agentId)) as ParticipantState;
    return preconditionMet(op, current);
}
async function decide(runtime: IAgentRuntime, message: Memory, state: State, cfg: OpConfig, op: RoomOp): Promise<boolean> {
    ...
    const yes = parseBooleanFromText(response.trim()) ||
...
function roomMatchesTarget(args: {
    ...
    return candidates.some((candidate) => candidate === lookup || candidate.includes(lookup));
}
async function resolveTargetRoom(args: {
    ...
    const roomIds = await args.runtime.getRoomsForParticipant(args.runtime.agentId);
    for (const roomId of roomIds) {
        const room = await args.runtime.getRoom(roomId);
        if (roomMatchesTarget({
            room,
...
async function applyOp(args: {
    ...
    try {
        ...
        const untilIso = args.op === "mute"
            ? muteUntilIsoFromDuration(args.durationMinutes)
            : undefined;
        await args.runtime.updateParticipantUserState(args.roomId, args.runtime.agentId, args.cfg.nextState);
...
async function applyServerScopedOp(args: {
    ...
    if (!room?.worldId) {
        return failure("ROOM_SERVER_NOT_FOUND", "That room does not belong to a server I can mute.");
    }
    const world = await runtime.getWorld(room.worldId);
    if (!world) {
        return failure("ROOM_SERVER_NOT_FOUND", "That room does not belong to a server I can mute.");
    ...
    if (op === "mute" ? active : !active) {
        return failure(`ROOM_${op.toUpperCase()}_PRECONDITION_FAILED`, `Cannot ${op} server from state ${active ? "MUTED" : "NONE"}`);
    }
    const untilIso = op === "mute" ? muteUntilIsoFromDuration(args.durationMinutes) : undefined;
    await setWorldMuteState(runtime, world.id, op === "mute" ? { ...(untilIso ? { untilIso } : {}) } : null);
...
export const roomOpAction: Action = {
    ...
    description: "Room mute/unmute/follow/unfollow. Default current room. Use roomId or platform+chatName for connector chat. scope=server mutes/unmutes the whole server/guild. mute+durationMinutes auto-unmutes at the ISO time.",
    descriptionCompressed: "room mute|unmute|follow|unfollow; roomId or platform+chatName; scope room|server; durationMinutes",
    routingHint: "mute/unmute/follow/unfollow or join/leave a chat, channel, group, or thread -> ROOM; do NOT use to send/read messages in it -> MESSAGE, to reply in the current chat -> REPLY, or to publish to a public feed/timeline -> POST",
    parameters: [
        {
            name: "action",
            description: "Operation: mute | unmute | follow | unfollow. Infer if omitted.",
            required: false,
            schema: {
        ...
        {
            name: "platform",
            description: "Connector id (telegram, discord, ...) for chatName targeting.",
            required: false,
            schema: { type: "string" as const },
        },
        {
        ...
        },
        {
            ...
            description: "For action=mute: temporary mute minutes; auto-unmutes at the returned scheduleAutoUnmuteIso time.",
            required: false,
            schema: { type: "number" as const },
        },
        {
            name: "scope",
            description: "For mute/unmute: room (default) targets one room; server mutes/unmutes the entire server/guild the room belongs to.",
            required: false,
            schema: {
                type: "string" as const,
                enum: ["room", "server"],
    ...
    handler: async (runtime: IAgentRuntime, message: Memory, state?: State, options?: HandlerOptions, _callback?: HandlerCallback, _responses?: Memory[]): Promise<ActionResult> => {
        const params = readRoomOpParams(options);
        const op = normalizeOp(params.action) ??
        ...
        if (scope === "server") {
            ...
            const room = platform && (explicitRoomId || chatName)
        ...
        if (platform && (explicitRoomId || chatName)) {
            ...
            const current = (await runtime.getParticipantUserState(targetRoom.id, runtime.agentId)) as ParticipantState;
            ...
            const roomName = targetRoom.name ?? chatName ?? `Room-${String(targetRoom.id)}`;
...
function makeRoomOpChildAction(args: {
    ...
    return {
        ...
        parameters: roomOpAction.parameters?.filter((parameter) => parameter.name !== "op"),
        validate: (runtime, message, state, options) => validateRoomOpAvailability(runtime, message, state, options as HandlerOptions | undefined, args.op),
        handler: (runtime, message, state, options, callback, responses) => {
            const params = options?.parameters && typeof options.parameters === "object"
                ? (options.parameters as Record<string, unknown>)
                : {};
            const actionCallback: typeof callback = callback
                ? (response, actionName) => callback(response, actionName ?? args.name)
                : undefined;
            return roomOpAction.handler(runtime, message, state, {
                ...(options ?? {}),
                parameters: {
                    ...params,
                    op: args.op,
                },
            }, actionCallback, responses);
        },
    };
}
export const muteRoomAction = makeRoomOpChildAction({
    ...
    descriptionCompressed: "mute room/chat when not MUTED; optional roomId|platform+chatName|durationMinutes",
...
export const unmuteRoomAction = makeRoomOpChildAction({
    ...
    descriptionCompressed: "unmute room/chat only from MUTED participant state; optional roomId|platform+chatName",
...
export const followRoomAction = makeRoomOpChildAction({
    ...
    descriptionCompressed: "follow room/chat if state is neither FOLLOWED nor MUTED; optional roomId|platform+chatName",
...
export const unfollowRoomAction = makeRoomOpChildAction({
    ...
    descriptionCompressed: "unfollow room/chat only from FOLLOWED participant state; optional roomId|platform+chatName",
