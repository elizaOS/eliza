...
import { buildFactKeywordsForStorage } from "../features/advanced-capabilities/fact-keywords.ts";
import { factClaimsEquivalent } from "../features/advanced-capabilities/fact-keywords.ts";
...
import { scoreFactKeywordRelevance } from "../features/advanced-capabilities/fact-keywords.ts";
import { type ChatMessage } from "@elizaos/core";
import { type FactKind } from "@elizaos/core";
import { type FactVerificationStatus } from "@elizaos/core";
import { type IAgentRuntime } from "@elizaos/core";
import { type JSONSchema } from "@elizaos/core";
import { type Memory } from "@elizaos/core";
import { type MessageHandlerExtract } from "@elizaos/core";
import { type MessageHandlerExtractedRelationship } from "@elizaos/core";
import { type Relationship } from "@elizaos/core";
import { type State } from "@elizaos/core";
import { type ToolDefinition } from "@elizaos/core";
import { type UUID } from "@elizaos/core";
export const FACTS_AND_RELATIONSHIPS_TOOL_NAME = "FACTS_AND_RELATIONSHIPS_VALIDATE";
/**
...
export function createFactsAndRelationshipsTool(): ToolDefinition {
    return {
        ...
        description: "Return ONLY the candidate facts/relationships that are unique and worth persisting. Drop anything already covered by existing facts or relationships.",
        type: "function",
        strict: true,
...
const REMEMBER_PREFIX = /^(?:(?:hey|hi|ok|okay)[\s,]+)?(?:please\s+)?(?:remember|note|keep in mind|save)\s+(?:that\s+)?/i;
...
export function planNamesMemoryMutation(plan: {
    ...
    return names.some((name) => MEMORY_MUTATION_ACTION.test(normalizeMemoryActionName(name)));
}
function storedMemoryTexts(executedTools: readonly FactsStageExecutedTool[]): string[] {
    const texts: string[] = [];
    for (const { name, result } of executedTools) {
        const data = result.data;
        if (result.success !== true || !data)
            continue;
        const actionName = typeof data.actionName === "string" ? data.actionName : name;
        if (!MEMORY_MUTATION_ACTION.test(normalizeMemoryActionName(actionName)) &&
            !MEMORY_MUTATION_ACTION.test(normalizeMemoryActionName(name))) {
            continue;
        }
        const stored = data.op === "create"
            ? data.text
            : data.op === "update"
                ? (data.memory as {
                    content?: {
...
function residualUserClaim(runtime: IAgentRuntime, message: Memory): string {
    ...
    if (agentName) {
        text = text.replace(new RegExp(`^@?${escapeRegExp(agentName)}(?:\\s*\\(@\\d+\\))?[\\s,:!.-]*`, "i"), "");
    }
    return text.replace(REMEMBER_PREFIX, "").trim();
}
/** Only an identical stored claim proves coverage without semantic judgment. */
function storedTextsCoverClaim(claim: string, storedTexts: readonly string[]): boolean {
    return claim.length > 0 && storedTexts.some((text) => text === claim);
}
export async function runFactsAndRelationshipsStage(args: FactsAndRelationshipsRunArgs): Promise<FactsAndRelationshipsRunResult> {
    ...
    const candidateRelationships = filterCandidateRelationships(extract.relationships ?? []);
    ...
    if (candidateRelationships.length === 0) {
        ...
        if (storedTexts.length > 0 &&
            ...
            runtime.logger.info({ messageId: message.id, candidateFacts: candidateFacts.length }, "[FactsStage] skipped the model call: this turn's MEMORY action stored the whole message");
            return {
                parsed: {
                    facts: [],
                    relationships: [],
...
function buildFactsStageMessages(args: BuildMessagesArgs): ChatMessage[] {
    ...
    for (const entity of args.roomEntities) {
        ...
        if (entity.id && name)
            nameByEntityId.set(entity.id, name);
    }
    const speakerLabel = (entityId: string): string => entityId === args.runtime.agentId
        ? "agent"
    ...
    const currentText = typeof args.message.content.text === "string"
    ...
    const roomEntityLines = args.roomEntities.map((entity) => formatRoomEntityRef(entity));
    ...
    for (const rel of args.extract.relationships ?? []) {
        candidateLines.push(`- relationship: ${rel.subject} ${rel.predicate} ${rel.object}`);
    }
 ...
 */
async function fetchRoomEntities(runtime: IAgentRuntime, message: Memory): Promise<RoomEntityRef[]> {
    ...
    try {
        ...
        const refs = details
            ...
            const names = Array.isArray(entity.names)
                ? entity.names.filter((name: unknown): name is string => typeof name === "string")
                : [];
            const id = typeof entity.id === "string" ? asUuidOrNull(entity.id) : null;
...
async function searchSimilarFacts(runtime: IAgentRuntime, message: Memory, candidateFacts: readonly string[]): Promise<Memory[]> {
    ...
    if (!Array.isArray(results)) {
        throw new ElizaError("Facts store returned an invalid deduplication result", {
...
async function fetchExistingRelationships(runtime: IAgentRuntime, message: Memory): Promise<Relationship[]> {
    if (typeof runtime.getRelationships !== "function") {
        throw new ElizaError("Relationship deduplication requires a relationship reader", { code: "RELATIONSHIP_DEDUP_READER_UNAVAILABLE" });
    }
    const entityIds = [message.entityId, runtime.agentId].filter((id): id is `${string}-${string}-${string}-${string}-${string}` => typeof id === "string" && id.length > 0);
    ...
    try {
        ...
        // error-policy:J2 Preserve the store failure while classifying the dedup read.
        throw new ElizaError("Failed to read existing relationships for deduplication", {
    ...
    if (!Array.isArray(results)) {
        throw new ElizaError("Relationship store returned an invalid deduplication result", {
...
export function parseFactsAndRelationshipsOutput(raw: unknown): FactsAndRelationshipsResult {
    ...
    const relationships = parsed.relationships.map((entry, index): MessageHandlerExtractedRelationship => {
        if (!entry || typeof entry !== "object") {
 ...
 */
async function readSameMessageDurableFacts(runtime: IAgentRuntime, message: Memory): Promise<Memory[]> {
    if (!message.id || typeof runtime.getMemories !== "function")
        return [];
    ...
    return rows.filter((row) => {
        ...
        return (row.roomId === message.roomId &&
            meta?.messageId === message.id &&
            meta?.kind !== "current");
    });
}
function coveredBySameMessageDurableFact(fact: string, factEntityId: UUID, durableFacts: readonly Memory[]): boolean {
    return durableFacts.some((row) => {
        if (row.entityId !== factEntityId)
            return false;
        const rowText = typeof row.content.text === "string" ? row.content.text : "";
...
async function persistFactsAndRelationships(args: PersistArgs): Promise<{
    ...
    if (parsed.facts.length > 0 && typeof runtime.createMemory === "function") {
        const sameMessageDurableFacts = await readSameMessageDurableFacts(runtime, message);
        for (const factEntry of parsed.facts) {
            ...
            const resolvedSubjectEntityId = resolveRelationshipEntityId(factEntry.subject, roomEntities, runtime, message);
            ...
            if (resolvedSubjectEntityId !== undefined &&
                coveredBySameMessageDurableFact(sanitized, factEntityId, sameMessageDurableFacts)) {
                runtime.logger.debug({ messageId: message.id, fact: sanitized, factEntityId }, "[FactsStage] skipped a Stage-1 fact already stored durably for this message");
                continue;
            }
    ...
    if (parsed.relationships.length > 0 &&
        ...
        for (const rel of parsed.relationships) {
            const normalized = humanizeRelationshipEnds(resolveRedactedRelationshipEnds(normalizeRelationshipForPersistence(rel), runtime, message), roomEntities, runtime, message);
            if (!normalized)
                continue;
...
function filterCandidateFacts(runtime: IAgentRuntime, facts: readonly string[]): string[] {
    const seen = new Set<string>();
    const out: string[] = [];
    for (const fact of facts) {
        const sanitized = sanitizePersistedFact(runtime, fact);
        if (!sanitized || isLowSignalCandidate(sanitized))
            continue;
...
function filterCandidateRelationships(relationships: readonly MessageHandlerExtractedRelationship[]): MessageHandlerExtractedRelationship[] {
    const seen = new Set<string>();
    ...
    for (const relationship of relationships) {
        ...
        if (!normalized)
            continue;
        const key = normalizeForComparison(`${normalized.subject}:${normalized.predicate}:${normalized.object}`);
        if (!key || seen.has(key))
            continue;
        seen.add(key);
        out.push(normalized);
...
function normalizeRelationshipForPersistence(relationship: MessageHandlerExtractedRelationship): MessageHandlerExtractedRelationship | null {
    ...
    if (!subject || !object || !predicate)
        return null;
    if (containsSecretSignal(subject) ||
 ...
 */
function resolveRedactedRelationshipEnds(normalized: MessageHandlerExtractedRelationship | null, runtime: IAgentRuntime, message: Memory): MessageHandlerExtractedRelationship | null {
    if (!normalized)
        return null;
    if (REDACTION_PLACEHOLDER_PATTERN.test(normalized.object))
        return null;
    if (REDACTION_PLACEHOLDER_PATTERN.test(normalized.subject)) {
        if (normalized.subject !== "[REDACTED:ELIZA_ADMIN_ENTITY_ID]")
            return null;
 ...
 */
function humanizeRelationshipEnds(normalized: MessageHandlerExtractedRelationship | null, entities: readonly RoomEntityRef[], runtime: IAgentRuntime, message: Memory): (MessageHandlerExtractedRelationship & {
    sourceEntityId?: UUID;
    targetEntityId?: UUID;
}) | null {
    if (!normalized)
    ...
    const humanize = (value: string): {
        ...
        if (!uuid) {
            return {
                value,
                entityId: resolveRelationshipEntityId(value, entities, runtime, message),
                fromId: false,
            };
        }
...
function sanitizePersistedFact(runtime: IAgentRuntime, value: string): string {
    ...
    if (!cleaned)
        return "";
    if (containsSecretSignal(cleaned))
        return "";
...
function containsSecretSignal(value: string): boolean {
    return (/\b(?:api[_\s-]?key|secret|password|access[_\s-]?token|refresh[_\s-]?token|private[_\s-]?key)\b/i.test(value) ||
        /\b(?:sk|csk|pk|ghp|gho|ghu|ghs|github_pat)-[A-Za-z0-9_-]{16,}\b/.test(value));
}
function isLowSignalCandidate(value: string): boolean {
    const normalized = normalizeForComparison(value);
    return (normalized.length < 4 ||
        /^(?:by the way|remind me|can you|could you|please|thanks|thank you)\b/.test(normalized) ||
        /\b(?:conversation summary|compacted prior planner|compactor|summary mode)\b/.test(normalized) ||
...
function resolveRelationshipEntityId(value: string, entities: readonly RoomEntityRef[], runtime: IAgentRuntime, message: Memory): UUID | undefined {
    ...
    if (messageAuthorNames(message).some((name) => normalizeForComparison(name) === normalized)) {
        return message.entityId;
    }
    ...
    for (const entity of entities) {
        if (!entity.id)
            continue;
        if (entity.names.some((name) => normalizeForComparison(name) === normalized)) {
    ...
    if (matches.has(message.entityId))
        return message.entityId;
    return undefined;
}
