 ...
 */
import { VALIDATION_KEYWORD_DOCS } from "@elizaos/core/i18n/keyword-matching";
import { collectKeywordTermMatches } from "@elizaos/core/i18n/keyword-matching";
import { splitKeywordDoc } from "@elizaos/core/i18n/keyword-matching";
type KeywordDoc = {
    base?: string;
...
export function getActionSearchKeywordTerms(input: {
    ...
    return dedupeTerms(getActionSearchKeywordSources(input).flatMap((source) => source.terms));
}
export function countActionSearchKeywordMatches(texts: readonly string[], terms: readonly string[]): number {
    return collectKeywordTermMatches(texts, terms).size;
}
function collectKeywordSourcesUnderStem(stem: string, options: {
    includeAllLocales: boolean;
}): ActionSearchKeywordSource[] {
...
function collectKeywordDocs(node: KeywordTree | KeywordDoc, key: string, sources: ActionSearchKeywordSource[], options: {
    includeAllLocales: boolean;
}): void {
    if (isKeywordDoc(node)) {
        const terms = options.includeAllLocales
            ? splitKeywordDoc([node.base, ...Object.values(node.locales ?? {})]
...
function dedupeKeywordSources(sources: readonly ActionSearchKeywordSource[]): ActionSearchKeywordSource[] {
