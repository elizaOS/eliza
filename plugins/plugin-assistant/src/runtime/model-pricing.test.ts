...
import { MODEL_PRICES_USD_PER_M_TOKENS } from "./model-pricing";
import { PRICE_TABLE_ID } from "./model-pricing";
import { afterEach } from "vitest";
import { computeCallCostUsd } from "./model-pricing";
import { describe } from "vitest";
import { expect } from "vitest";
import { isLocalProvider } from "./model-pricing";
...
describe("MODEL_PRICES_USD_PER_M_TOKENS", () => {
    it("covers all required hosted providers", () => {
        ...
        expect(MODEL_PRICES_USD_PER_M_TOKENS["claude-opus-4-8"]?.provider).toBe("anthropic");
        expect(MODEL_PRICES_USD_PER_M_TOKENS["claude-opus-4-7"]?.provider).toBe("anthropic");
        expect(MODEL_PRICES_USD_PER_M_TOKENS["claude-sonnet-5"]?.provider).toBe("anthropic");
        expect(MODEL_PRICES_USD_PER_M_TOKENS["claude-sonnet-4-6"]?.provider).toBe("anthropic");
        expect(MODEL_PRICES_USD_PER_M_TOKENS["claude-haiku-4-5"]?.provider).toBe("anthropic");
        // OpenAI — the ship targets per CLAUDE.md
        expect(MODEL_PRICES_USD_PER_M_TOKENS["gpt-5.5"]?.provider).toBe("openai");
        expect(MODEL_PRICES_USD_PER_M_TOKENS["gpt-5.5-mini"]?.provider).toBe("openai");
        // Google / Groq / Cerebras / Eliza Cloud — every required hosted tier
        // in the W1-X1 spec ships at least one entry.
        const providers = new Set(Object.values(MODEL_PRICES_USD_PER_M_TOKENS).map((p) => p.provider));
    ...
    it("preserves the documented Anthropic rate card", () => {
        ...
        expect(MODEL_PRICES_USD_PER_M_TOKENS["claude-opus-4-7"]).toEqual(MODEL_PRICES_USD_PER_M_TOKENS["claude-opus-4-8"]);
    ...
    it("prices the pinned Cerebras default from the authoritative snapshot", () => {
        ...
        expect(DEFAULT_ELIZA_CLOUD_LARGE_TEXT_MODEL).toBe(DEFAULT_ELIZA_CLOUD_TEXT_MODEL);
        expect(MODEL_PRICES_USD_PER_M_TOKENS[DEFAULT_ELIZA_CLOUD_TEXT_MODEL]).toEqual({
            provider: "cerebras",
            input: 0.99,
            output: 1.49,
            cacheRead: 0,
...
describe("computeCallCostUsd", () => {
    ...
    it("returns undefined and warns when the model is unknown on a hosted provider", () => {
        const warn = vi.fn();
        const cost = computeCallCostUsd("never-heard-of-this-model", {
            promptTokens: 1000,
        ...
        expect((context as Record<string, unknown>).priceTableId).toBe(PRICE_TABLE_ID);
        expect((context as Record<string, unknown>).modelName).toBe("never-heard-of-this-model");
    });
    it("returns 0 with no warning when the provider is a local tier (Ollama)", () => {
        const warn = vi.fn();
        const cost = computeCallCostUsd("qwen-2.5-14b-some-local-tag", { promptTokens: 100000, completionTokens: 5000, totalTokens: 105000 }, { provider: "ollama", logger: { warn } });
        expect(cost).toBe(0);
        expect(warn).not.toHaveBeenCalled();
    });
    it("returns 0 with no warning when the provider is LM Studio", () => {
        const warn = vi.fn();
        expect(computeCallCostUsd("local-model", { promptTokens: 100, completionTokens: 100, totalTokens: 200 }, { provider: "lm-studio", logger: { warn } })).toBe(0);
        expect(warn).not.toHaveBeenCalled();
    });
    it("returns 0 with no warning when the provider is llama.cpp", () => {
        const warn = vi.fn();
        expect(computeCallCostUsd("phi-4-q4", { promptTokens: 1000, completionTokens: 100, totalTokens: 1100 }, { provider: "llama.cpp", logger: { warn } })).toBe(0);
        expect(warn).not.toHaveBeenCalled();
    });
    it("suppresses the missing-price warning when local capability is declared, even for a cloud-looking provider name", () => {
        const warn = vi.fn();
        expect(computeCallCostUsd("my-unpriced-model", { promptTokens: 100, completionTokens: 100, totalTokens: 200 }, { provider: "acme-edge", local: true, logger: { warn } })).toBe(0);
        expect(warn).not.toHaveBeenCalled();
    });
    it("still warns for a local-looking name when local capability is explicitly false and price is missing", () => {
        const warn = vi.fn();
        expect(computeCallCostUsd("unknown-model", { promptTokens: 100, completionTokens: 100, totalTokens: 200 }, { provider: "ollama", local: false, logger: { warn } })).toBeUndefined();
        expect(warn).toHaveBeenCalledTimes(1);
    });
    it("does not warn when logger is omitted (no noise in hot paths)", () => {
        // Should not throw even when logger is undefined.
        expect(() => computeCallCostUsd("unknown-model", { promptTokens: 100, completionTokens: 100, totalTokens: 200 }, { provider: "openai" })).not.toThrow();
    });
...
describe("env overrides (MODEL_PRICES_JSON / MODEL_CONTEXT_WINDOWS_JSON)", () => {
    ...
    it("prices an id absent from the static table via MODEL_PRICES_JSON", () => {
        vi.stubEnv("MODEL_PRICES_JSON", JSON.stringify({
            "acme-frontier-9000": { input: 4.0, output: 20.0 },
        }));
        const warn = vi.fn();
        const cost = computeCallCostUsd("acme-frontier-9000", {
            promptTokens: 1000000,
            completionTokens: 1000000,
            totalTokens: 2000000,
    ...
    it("env price keys participate in the substring fallback (versioned ids)", () => {
        vi.stubEnv("MODEL_PRICES_JSON", JSON.stringify({ "acme-frontier-9000": { input: 1.0, output: 2.0 } }));
    ...
    it("malformed override JSON fails fast", () => {
        ...
        expect(() => lookupModelPrice("claude-opus-4-8")).toThrow("MODEL_PRICES_JSON is not valid JSON");
    });
    it("an invalid entry rejects the whole override", () => {
        ...
        expect(() => lookupModelPrice("good-entry")).toThrow("MODEL_PRICES_JSON entry needs numeric input and output rates");
    });
    it("a truly unknown id has unknown cost and warns", () => {
        vi.stubEnv("MODEL_PRICES_JSON", JSON.stringify({ "some-other-model": { input: 1.0, output: 2.0 } }));
        const warn = vi.fn();
        const cost = computeCallCostUsd("claude-unknown-test-9", { promptTokens: 1000, completionTokens: 1000, totalTokens: 2000 }, { provider: "anthropic", logger: { warn } });
        expect(cost).toBeUndefined();
        expect(warn).toHaveBeenCalledTimes(1);
    });
});
