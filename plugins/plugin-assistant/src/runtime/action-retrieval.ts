 ...
 */
import { collectPreparedKeywordTermMatches } from "@elizaos/core/i18n/keyword-matching";
import { logger } from "@elizaos/core";
import { matchActionWildcardParts } from "@elizaos/core";
import { normalizeActionName } from "./action-catalog";
import { prepareKeywordTerms } from "@elizaos/core/i18n/keyword-matching";
import { type ActionCatalog } from "./action-catalog";
import { type ActionCatalogParent } from "./action-catalog";
import { type PreparedKeywordTerm } from "@elizaos/core/i18n/keyword-matching";
...
export function retrieveActions(input: RetrieveActionsInput): ActionRetrievalResponse {
    const candidateActions = dedupeNormalizedStrings(input.candidateActions);
    const catalogParentNames = new Set(input.catalog.parents.map((parent) => parent.normalizedName));
    const parentActionHints = dedupeNormalizedStrings([
        ...
        ...candidateActions.filter((actionName) => catalogParentNames.has(normalizeActionName(actionName))),
        ...candidateActions.flatMap((actionName) => {
            ...
            // is absent).
            if (catalogParentNames.has(normalizeActionName(actionName)))
                return [];
            const explicitAliases = explicitParentAliasesForCandidateAction(actionName);
            ...
            if (collectAmbiguousSimiles(input.catalog.parents).has(normalizeActionName(actionName))) {
                return [];
            }
    ...
    const recentConversationText = shouldUseRecentConversationForActionSearch(input.messageText ?? "")
        ? normalizeTextList(input.recentConversationText)
        : [];
    const candidateActionsForSearch = recentConversationText.length > 0
        ? candidateActions.filter((actionName) => parentAliasesForCandidateAction(actionName).length > 0)
        : candidateActions;
    const queryText = [
        input.messageText ?? "",
    ...
    const regexScores = scoreCandidateRegex(input.catalog.parents, candidateActionsForSearch);
    const keywordScores = scoreKeywordMatches(input.catalog.parents, keywordQueryTexts);
    const bm25Scores = scoreBm25(input.catalog.parents, queryTokens);
    const embeddingScores = scoreEmbeddingTieBreaker(input.catalog.parents, input.embedding);
    const isBareSingleTokenQuery = parentActionHints.length === 0 &&
        candidateActions.length === 0 &&
        queryTokens.length <= 1;
    const stageRankings: Partial<Record<RetrievalStageName, Map<string, number>>> = {
    ...
    const selectedContextSet = new Set((input.selectedContexts ?? []).map((c) => c.toLowerCase()));
    const results = input.catalog.parents.map((parent) => {
        ...
        const baseScore = Math.max(exact, regex, keyword > 0 ? 0.35 + keyword * 0.5 : 0, bm25 > 0 ? 0.28 + bm25 * (isBareSingleTokenQuery ? 0.38 : 0.49) : 0, embedding > 0 ? 0.25 + embedding * 0.45 : 0, rrf > 0 ? 0.2 + rrf * (isBareSingleTokenQuery ? 0.45 : 0.5) : 0);
        ...
        if (baseScore > 0 &&
            ...
            const intersect = parentContexts.some((c) => selectedContextSet.has(String(c).toLowerCase()));
            if (intersect) {
                contextBoost = 0.3;
    ...
    const needsMessageOnlyTieBreak = saturatedResults.length > 1 &&
    ...
    if (needsMessageOnlyTieBreak) {
        ...
        const messageOnlyKeywordScores = scoreKeywordMatches(input.catalog.parents, messageOnlyQueryTexts);
        const messageOnlyBm25Scores = scoreBm25(input.catalog.parents, tokenizeActionSearchText(messageOnlyQueryTexts.join("\n")));
        ...
        for (const parent of input.catalog.parents) {
            const keywordRaw = messageOnlyKeywordScores.get(parent.normalizedName) ?? 0;
            ...
            const keyword = maxMessageOnlyKeyword > 0 ? keywordRaw / maxMessageOnlyKeyword : 0;
            ...
            const parentContexts: readonly unknown[] = Array.isArray(parent.contexts)
                ? parent.contexts
                : [];
            const contextMatch = hasMessageEvidence &&
                selectedContextSet.size > 0 &&
                parentContexts.some((context) => selectedContextSet.has(String(context).toLowerCase()))
                ? 0.3
                : 0;
            messageEvidenceByParent.set(parent.normalizedName, roundScore(keyword + bm25 + embedding + contextMatch));
        }
    }
    results.sort((left, right) => {
        ...
        const saturatedHintCohortTie = needsMessageOnlyTieBreak && left.score === 1 && right.score === 1;
    ...
    if (input.measurementMode === true) {
        ...
        for (const parent of input.catalog.parents) {
            const parentContexts: readonly unknown[] = Array.isArray(parent.contexts)
                ? parent.contexts
                : [];
            if (selectedContextSetForMeasurement.size > 0 &&
                parentContexts.length > 0 &&
                parentContexts.some((c) => selectedContextSetForMeasurement.has(String(c).toLowerCase()))) {
...
function scoreExactHints(parents: ActionCatalogParent[], parentActionHints: string[]): Map<string, number> {
    const hints = new Set(parentActionHints.map(normalizeActionName).filter(Boolean));
...
function scoreCandidateRegex(parents: ActionCatalogParent[], candidateActions: string[]): Map<string, number> {
    ...
    for (const parent of parents) {
        ...
        for (const pattern of patterns) {
            const namespaceHit = pattern.namespace && pattern.namespace === parent.normalizedName;
            const nameHit = searchableNames.some((name) => pattern.regex.test(name));
            if (namespaceHit || nameHit) {
                scores.set(parent.normalizedName, Math.max(scores.get(parent.normalizedName) ?? 0, pattern.score));
            }
        }
    }
    return scores;
...
const parentScoringCache = new WeakMap<ActionCatalogParent, ParentScoringTokens>();
...
function scoreBm25(parents: ActionCatalogParent[], queryTokens: string[]): Map<string, number> {
    const scores = new Map<string, number>();
    ...
    const averageDocumentLength = documents.reduce((sum, document) => sum + document.scoring.length, 0) /
...
const preparedKeywordTermsCache = new WeakMap<ActionCatalogParent, PreparedKeywordTerm[]>();
function getPreparedKeywordTerms(parent: ActionCatalogParent): PreparedKeywordTerm[] {
    const cached = preparedKeywordTermsCache.get(parent);
    if (cached) {
        return cached;
    }
    const prepared = prepareKeywordTerms(parent.keywordText
        .split(/\n+/)
        .map((term) => term.trim())
        .filter(Boolean));
...
function scoreKeywordMatches(parents: ActionCatalogParent[], queryTexts: readonly string[]): Map<string, number> {
    const scores = new Map<string, number>();
    ...
    for (const parent of parents) {
        ...
        const score = collectPreparedKeywordTermMatches(distinctTexts, prepared).size;
        if (score > 0) {
...
function scoreEmbeddingTieBreaker(parents: ActionCatalogParent[], embedding?: ActionEmbeddingTieBreaker): Map<string, number> {
    const scores = new Map<string, number>();
    if (!embedding?.enabled || !embedding.scoresByParentName) {
        return scores;
    }
    for (const parent of parents) {
        const score = embedding.scoresByParentName[parent.name] ??
...
function reciprocalRankFusion(stageRankings: Partial<Record<RetrievalStageName, Map<string, number>>>, stageWeights?: Partial<Record<RetrievalStageName, number>>): Map<string, number> {
    const scores = new Map<string, number>();
...
function explicitParentAliasesForCandidateAction(actionName: string): string[] {
    ...
    if (/(?:^|[^A-Z0-9])(?:CALC(?:ULATE)?|MATH|ARITH(?:METIC)?|MULTIPLY|DIVIDE)(?:[^A-Z0-9]|$)/u.test(actionName.toUpperCase())) {
        return ["CALCULATE"];
    }
...
function looksLikeAppCandidateAction(normalizedActionName: string): boolean {
    ...
    return (hasAnyToken(tokens, APP_SURFACE_TOKENS) &&
...
function looksLikeSettingsPermissionCandidateAction(normalizedActionName: string): boolean {
    if (!normalizedActionName)
        return false;
    const tokens = new Set(normalizedActionName.split(/_+/).filter(Boolean));
    if (!hasAnyToken(tokens, SETTINGS_PERMISSION_OPERATION_TOKENS))
        return false;
    const namesAPermission = tokens.has("PERMISSION") || tokens.has("PERMISSIONS");
    const namesAScopedAccess = tokens.has("ACCESS") &&
...
function looksLikeViewCandidateAction(normalizedActionName: string): boolean {
    ...
    const hasGeneratedCapabilityShape = hasViewOperation && tokens.size >= 2 && !hasOnlyOperationTokens(tokens);
...
function hasOnlyOperationTokens(tokens: Set<string>): boolean {
    for (const token of tokens) {
        if (!VIEW_OPERATION_TOKENS.has(token))
            return false;
    }
    return true;
}
...
function collectAmbiguousSimiles(parents: readonly ActionCatalogParent[]): Set<string> {
...
function resolveSimileParentHints(parents: readonly ActionCatalogParent[], candidateActions: readonly string[]): string[] {
...
export function candidateNamespaceParentExists(parents: readonly Pick<ActionCatalogParent, "normalizedName">[], actionName: string): boolean {
    const normalized = normalizeActionName(actionName);
    ...
    const domainTokens = tokens.filter((token) => !VIEW_OPERATION_TOKENS.has(token) && token !== "VIEWS");
    return parents.some((parent) => domainTokens.some((token) => actionTokenMatchesParent(token, parent)));
}
function actionTokenMatchesParent(token: string, parent: Pick<ActionCatalogParent, "normalizedName">): boolean {
    const parentName = parent.normalizedName;
    return (parentName === token ||
        parentName === `${token}S` ||
        (parentName.endsWith("S") && parentName.slice(0, -1) === token));
}
function shouldUseRecentConversationForActionSearch(messageText: string): boolean {
    const normalized = messageText.toLowerCase().replace(/\s+/g, " ").trim();
    if (!normalized)
        return false;
    return (/\b(?:again|continue|redo|rerun|retry|same|another\s+one|one\s+more|also|too)\b/iu.test(normalized) ||
        /\b(?:do|run|make|build|check|try|send|show|open|fix|update|use|add|remove|delete|change|repeat)\b[\s\S]{0,80}\b(?:it|that|this|these|those|them|there|above|previous|last|same|one)\b/iu.test(normalized));
}
...
const CONTROL_BLOCK_MARKER_RE = /\[[ \t]*(?:FORM|CHOICE[^\]]*|FOLLOWUPS[^\]]*|TASK:[^\]]*|CHECKLIST)[ \t]*\][\s\S]*?\[[ \t]*\/[ \t]*(?:FORM|CHOICE|FOLLOWUPS|TASK|CHECKLIST)[ \t]*\]|\[CONFIG:[^\]]*\]/g;
...
function wildcardCandidateRegex(candidateAction: string): {
    ...
    const parts = rawSegments.map((rawSegment, index) => {
        ...
        const trail = index < lastIndex && /[^A-Za-z0-9]$/.test(rawSegment) ? "_" : "";
