...
const RUNTIME_TEMPLATE_CACHE = new Map<string, ReturnType<typeof compileTemplate>>();
...
export function getCompiledRuntimeTemplate(template: string, alreadyUpgraded = false): ReturnType<typeof compileTemplate> {
...
export function extractTemplatePlaceholderKeys(templateChunk: string): string[] {
...
export function isTemplateChunkStable(templateChunk: string): boolean {
    ...
    return placeholderKeys.every((key) => key !== "providers" && STABLE_PROMPT_TEMPLATE_KEYS.has(key));
}
export function getPromptProviderSegments(state: State): PromptSegment[] {
    const providerResults = state.data.providers as Record<string, {
        text?: string;
        providerName?: string;
    }> | undefined;
    if (!providerResults) {
    ...
    const providerOrder = Array.isArray(state.data.providerOrder)
        ...
        : Object.keys(providerResults).sort((left, right) => left.localeCompare(right));
...
export function renderPromptTemplateSegments(templateStr: string, context: Record<string, unknown>, state: State): PromptSegment[] {
    const upgradedTemplate = upgradeDoubleToTriple(templateStr);
    const templateWithMarkers = upgradedTemplate.replace(/\{\{\{?\s*providers\s*\}?\}\}/g, PROVIDERS_PROMPT_MARKER);
    const templateFunction = getCompiledRuntimeTemplate(templateWithMarkers, true);
    const renderedWithMarkers = cleanDynamicPromptTemplateOutput(templateFunction(context));
    if (!templateWithMarkers.includes(PROVIDERS_PROMPT_MARKER) ||
    ...
    if (providerSegments.length === 0) {
        return [
            {
                content: renderedWithMarkers.replaceAll(PROVIDERS_PROMPT_MARKER, String(context.providers ?? "")),
                stable: false,
            },
        ];
    }
...
export function joinPromptSegmentGroups(groups: PromptSegment[][]): PromptSegment[] {
...
export function mergePromptSegments(segments: PromptSegment[]): PromptSegment[] {
