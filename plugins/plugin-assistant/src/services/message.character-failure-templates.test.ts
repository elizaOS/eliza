...
import { INSUFFICIENT_CREDITS_REPLY } from "./message.ts";
...
import { asUUID } from "../../../../packages/core/src/types/primitives.ts";
...
import { type Content } from "../../../../packages/core/src/types/primitives.ts";
 ...
 */
function makeFailingRuntime(failure: Error, options: {
    ...
    return createMockRuntime({
        ...
        getModel: vi.fn(() => hasModelProvider
            ? async () => {
                throw failure;
            }
            : undefined),
...
async function runTurn(failure: Error, options: {
    hasModelProvider?: boolean;
    templates?: Partial<CharacterFailureTemplates>;
} = {}): Promise<string[]> {
    const runtime = makeFailingRuntime(failure, options);
    const deliveries: Content[] = [];
    await new DefaultMessageService().handleMessage(runtime, makeMessage(), async (content) => {
...
describe("character failure templates on the connector delivery path", () => {
    ...
    for (const testCase of CASES) {
        ...
        it(`does not leak other template keys into ${testCase.kind}`, async () => {
            ...
            const otherKeys = CASES.map((entry) => entry.key).filter((key) => key !== testCase.key);
            const visibleTexts = await runTurn(testCase.error(), {
                ...testCase.options,
                templates: TEMPLATES,
            });
    ...
    it("never uses transientFailureReply for a missing capability", async () => {
        ...
        expect(visibleTexts[0]?.toLowerCase()).not.toMatch(/\btry (that )?again\b|\bin a moment\b/);
    ...
    it("accepts a ({ state }) => string callback template", async () => {
        ...
        expect(deliveries.map((content) => content.text)).toContain("sentinel: callback rate limit reply.");
    });
});
