...
function stateSideEffectClaimHasLocalSubject(
  ...
  for (const match of text.matchAll(STATE_SIDE_EFFECT_CLAIM_PATTERN)) {
    ...
    // Pending work supplies the subject of a premature bare completion claim.
...
// common advisory imperative, not a report. A bare "Moved by ..." can be
// an adjectival introduction without a mutation object; it is not sufficient
// evidence of a completed move. "Moved the appointment by ..." still counts.
const SUBJECTLESS_PAST_SIDE_EFFECT_CLAIM_PATTERN =
  /(?:^|[.!?]\s+)(?:added|created|saved|scheduled|rescheduled|moved(?!\s+by\b)|postponed|shifted|booked|logged|deleted|removed|renamed|cancell?ed|arranged)\b/gi;
