...
function fixture(responses: unknown[]) {
  const state: State = {
    ...
    data: {
      providers: {
        NAMED_NOTES: {
          text: providerText,
          discoveryText: "context_discovery: NAMED_NOTES",
        },
      },
    },
  };
  ...
  const runtime = {
    ...
    providers: [{ name: "NAMED_NOTES", get: vi.fn() }],
    evaluators: [],
...
describe("invalid context read recovery", () => {
  it.each([ChannelType.DM, ChannelType.VOICE_DM])(
    ...
    async (channelType) => {
      ...
      expect(repair).toContain("NAMED_NOTES");
  ...
  it("does not process fields smuggled into an invalid read", async () => {
    const f = fixture([
      read(["NAMED_NOTES"], { facts: ["Do not persist this"] }),
      decision,
    ]);
  ...
  it("allows a corrected authorized read through fresh composition", async () => {
    const f = fixture([read(["files"]), read(["NAMED_NOTES"]), decision]);
    await runV5MessageRuntimeStage1({
      runtime: f.runtime,
      state: f.state,
      message,
