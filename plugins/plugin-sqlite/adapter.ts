...
export class SQLiteDatabaseAdapter extends InMemoryDatabaseAdapter {
  ...
  private facade: SQLiteDatabaseAdapter = this;

  private constructor(path: string, agentId: UUID) {
  ...
  static create(path: string, agentId: UUID): SQLiteDatabaseAdapter {
    const adapter = new SQLiteDatabaseAdapter(path, agentId);
    const storage = adapter.sqlite;
