...
import { reconcileTwitterWorld } from "./utils/memory";
...
import { type TwitterInteractionMemory } from "./types";
import { type TwitterInteractionPayload } from "./types";
import { type TwitterLikeReceivedPayload } from "./types";
import { type TwitterMemory } from "./types";
import { type TwitterProfile } from "./base";
import { type TwitterQuoteReceivedPayload } from "./types";
import { type TwitterRetweetReceivedPayload } from "./types";
...
export class TwitterInteractionClient {
    ...
    async start() {
        ...
        const handleTwitterInteractionsLoop = () => {
            ...
            logger.info(`Twitter interaction client will check in ${engagementIntervalMinutes.toFixed(1)} minutes`);
    ...
    async handleTwitterInteractions() {
        ...
        try {
            await this.client.withAuthenticatedSession(async (session) => {
                ...
                const repliesEnabled = (getSetting(this.runtime, "TWITTER_ENABLE_REPLIES") ??
                ...
                const targetUsersConfig = ((getSetting(this.runtime, "TWITTER_TARGET_USERS") ??
    ...
    private async handleMentions(session: TwitterAccountSession) {
        ...
        while (true) {
            const searchResult = await this.client.fetchSearchTweets(`@${twitterUsername}`, 20, SearchMode.Latest, cursor);
            let hitWatermark = false;
            for (const tweet of searchResult.tweets) {
                mentionCandidates.push(tweet);
                if (lastCheckedTweetId !== null &&
     ...
     */
    private async handleTargetUserPosts(targetUsersConfig: string, session: TwitterAccountSession) {
        try {
            ...
            for (const targetUser of targetUsers) {
                try {
                    ...
                    const searchResult = await this.client.fetchSearchTweets(searchQuery, 10, // Get up to 10 recent posts per user
                    SearchMode.Latest);
                    if (searchResult.tweets.length > 0) {
                        ...
                        await this.processTargetUserTweets(searchResult.tweets, normalizedUsername, session);
                    }
                }
                catch (error) {
                    if (isSessionRotationError(error))
                        throw error;
                    logger.error(`Error searching posts from @${targetUser}:`, errorMessage(error));
                }
            }
     ...
     */
    private async processTargetUserTweets(tweets: ClientTweet[], username: string, session: TwitterAccountSession) {
        const maxEngagementsPerRun = normalizePositiveInteger(getSetting(this.runtime, "TWITTER_MAX_ENGAGEMENTS_PER_RUN") ??
    ...
    private async processTimelineForEngagement(session: TwitterAccountSession) {
        try {
            ...
            try {
                ...
                logger.warn("Home timeline unavailable for engagement; falling back to popular search:", errorMessage(timelineError));
                const searchResult = await this.client.fetchSearchTweets("min_retweets:10 min_faves:20 -is:reply -is:retweet lang:en", 20, SearchMode.Latest);
            ...
            if (isSessionRotationError(error))
                throw error;
            logger.error("Error processing timeline for engagement:", errorMessage(error));
        }
    }
    /**
    ...
    private buildTweetMessage(tweet: ProcessableTweet): Memory {
        ...
        return {
            ...
            metadata: buildTwitterMessageMetadata(tweet, entityId, tweet.timestamp, this.client.accountId),
     ...
     * replies.
     */
    private async decideTweetActions(tweet: ProcessableTweet): Promise<ActionResponse> {
...
Choose any combination of [LIKE], [RETWEET], [QUOTE], and [REPLY] that are appropriate. Each action must be on its own line. Your response must only include the chosen actions.`;
    ...
    private async ensureTweetContext(tweet: ProcessableTweet) {
        try {
            ...
            const tweetMemory: Memory = {
                ...
                metadata: buildTwitterMessageMetadata(tweet, context.entityId, tweet.timestamp, this.client.accountId),
                createdAt: tweet.timestamp,
            };
            await createMemorySafe(this.runtime, tweetMemory, "messages");
        }
        catch (error) {
            logger.error(`Failed to ensure context for tweet ${tweet.id}:`, errorMessage(error));
            throw error;
        }
    }
    /**
     ...
     */
    private async engageWithTweet(tweet: ProcessableTweet, actions: ActionResponse, session: TwitterAccountSession): Promise<boolean> {
        let engaged = false;
        if (actions.like) {
     ...
     */
    private async handleLikeAction(tweet: ProcessableTweet, session: TwitterAccountSession): Promise<boolean> {
        try {
            if (this.isDryRun) {
     ...
     */
    private async handleRetweetAction(tweet: ProcessableTweet, session: TwitterAccountSession): Promise<boolean> {
        try {
            if (this.isDryRun) {
     ...
     */
    private async handleQuoteAction(tweet: ProcessableTweet, session: TwitterAccountSession): Promise<boolean> {
        try {
            const message = this.buildTweetMessage(tweet);
            const state = await this.runtime.composeState(message);
...
${tweet.text}`;
            ...
            if (this.isDryRun) {
                logger.info(`[DRY RUN] Would have quoted tweet ${tweet.id} with: ${post}`);
                return true;
            }
            await sendTextAsTweetThread(post, async (chunk, previousTweetId, index) => {
                this.assertCurrentSession(session);
                const result = await this.client.requestQueue.add(() => index === 0
                    ? this.client.twitterClient.sendQuoteTweet(chunk, tweet.id)
                    : this.client.twitterClient.sendTweet(chunk, previousTweetId), NO_REQUEST_RETRY);
     ...
     */
    private async handleReplyAction(tweet: ProcessableTweet, session: TwitterAccountSession): Promise<boolean> {
        try {
            this.assertCurrentSession(session);
            const message = this.buildTweetMessage(tweet);
     ...
     */
    async processMentionTweets(mentionCandidates: ClientTweet[], authenticatedProfile?: TwitterProfile): Promise<void> {
        return this.client.withAuthenticatedSession(async (session) => {
            if (authenticatedProfile &&
                authenticatedProfile.id !== session.profile.id) {
                throw new ElizaError("Authenticated X profile changed before mention processing", { code: "X_AUTH_SESSION_ROTATED" });
            }
            await this.processMentionTweetsForSession(mentionCandidates, session);
        });
    }
    private async processMentionTweetsForSession(mentionCandidates: ClientTweet[], session: TwitterAccountSession): Promise<void> {
        ...
        const autoRespondMentions = (getSetting(this.runtime, "TWITTER_AUTO_RESPOND_MENTIONS") ??
            process.env.TWITTER_AUTO_RESPOND_MENTIONS) !== "false";
        const autoRespondReplies = (getSetting(this.runtime, "TWITTER_AUTO_RESPOND_REPLIES") ??
        ...
        if (!autoRespondMentions || !autoRespondReplies) {
            const inReplyToIds = Array.from(new Set(uniqueTweetCandidates
                .map((tweet) => tweet.inReplyToStatusId)
                .filter((id): id is string => Boolean(id))));
            const parentTweetAuthorMap = new Map<string, string>();
            if (inReplyToIds.length > 0) {
                try {
                    const parentTweets = await this.client.twitterClient.getTweetsV2(inReplyToIds, {
                    ...
                    logger.warn("Unable to resolve parent tweet authors for mention/reply filtering", errorMessage(error));
                }
            }
            uniqueTweetCandidates = uniqueTweetCandidates.filter((tweet) => {
                const parentAuthorId = tweet.inReplyToStatus?.userId ||
        ...
        const maxInteractionsPerRun = normalizePositiveInteger(getSetting(this.runtime, "TWITTER_MAX_ENGAGEMENTS_PER_RUN") ??
            process.env.TWITTER_MAX_ENGAGEMENTS_PER_RUN, 10);
        ...
        for (const tweet of tweetsToProcess) {
            ...
            if (lastCheckedTweetId === null ||
                ...
                const hasExistingReply = existingReplies.some((memory) => memory.content.inReplyTo === tweetId ||
                    memory.content.inReplyTo === tweet.id);
                if (hasExistingReply) {
                    logger.log(`Already responded to tweet ${tweet.id} (found in conversation history), skipping`);
                    continue;
                }
    ...
    async handleInteraction(interaction: TwitterInteractionPayload) {
        if (interaction?.targetTweet?.conversationId) {
            const memory = this.createMemoryObject(interaction.type, `${interaction.id}-${interaction.type}`, interaction.userId, interaction.targetTweet.conversationId);
            await createMemorySafe(this.runtime, memory, "messages");
            ...
            this.runtime.emitEvent(EventType.REACTION_RECEIVED, {
                ...
                metadata: {
                    ...
                    userId: interaction.userId,
                    timestamp: Date.now(),
                    quoteText: interaction.type === "quote"
     ...
     */
    async handleTweet({ tweet, message, thread, session, }: {
        tweet: ClientTweet;
        message: Memory;
        thread: ClientTweet[];
        ...
        thread = thread.map((threadTweet) => normalizeTweet(threadTweet) ?? threadTweet);
        ...
        const callback: HandlerCallback = async (response: Content, tweetId?: string) => {
            ...
            if (egressAttempted) {
                logger.warn(`Suppressed duplicate reply attempt for mention ${tweet.id}`);
                return [];
            }
            ...
            try {
                responseMemories = await sendChunkedTweet(this.client, response, message.roomId, normalizedTweet.username, tweetToReplyTo);
            }
            catch (error) {
                deliveryError = error;
                throw error;
        ...
        const imageAttachments = await describeTweetPhotos(this.runtime, normalizedTweet);
