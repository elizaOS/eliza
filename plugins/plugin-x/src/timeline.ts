...
import { NO_REQUEST_RETRY } from "./base";
...
import { ensureTwitterContext } from "./utils/memory";
import { extractXWriteReceiptId } from "./utils/provider-receipt";
import { getEpochMs } from "./utils/time";
import { getSetting } from "./utils/settings";
import { isTweetProcessed } from "./utils/memory";
import { logger } from "@elizaos/core";
import { parseActionResponseFromText } from "./utils";
import { parseJSONObjectFromText } from "@elizaos/core/text/model-output";
import { parseTwitterInterval } from "./environment";
import { quoteTweetTemplate } from "./templates";
import { replyTweetTemplate } from "./templates";
...
import { type TwitterAccountSession } from "./base";
...
function normalizeTweet(tweet: Tweet): ActionableTweet | null {
    ...
    const username = typeof tweet.username === "string" && tweet.username.length > 0
        ? tweet.username
        : "unknown";
...
export class TwitterTimelineClient {
    ...
    constructor(client: ClientBase, runtime: IAgentRuntime, state: TwitterClientState) {
        this.client = client;
        ...
        const dryRunSetting: unknown = this.state?.TWITTER_DRY_RUN ??
        ...
        const timelineMode = getSetting(this.runtime, "TWITTER_TIMELINE_MODE") ??
    ...
    async start() {
        ...
        const handleTwitterTimelineLoop = () => {
            ...
            const engagementIntervalMinutes = parseTwitterInterval(this.state?.TWITTER_ENGAGEMENT_INTERVAL ||
    ...
    async getTimeline(count: number): Promise<ActionableTweet[]> {
        return this.client.withAuthenticatedSession(({ profile }) => this.getTimelineForProfile(count, profile));
    }
    private async getTimelineForProfile(count: number, profile: TwitterProfile): Promise<ActionableTweet[]> {
        const homeTimeline = this.timelineType === TIMELINE_TYPE.Following
    ...
    async describeTweetMedia(tweet: ActionableTweet): Promise<string> {
        ...
        if (typeof this.runtime.getModel(ModelType.IMAGE_DESCRIPTION) !== "function") {
            logger.debug(`No IMAGE_DESCRIPTION model registered; skipping media interpretation for tweet ${tweet.id}`);
            return "";
        }
        ...
        for (const imageUrl of mediaUrls) {
            try {
                ...
                logger.warn(`Failed to interpret media ${imageUrl} on tweet ${tweet.id}: ${errorMessage(error)}`);
            }
        }
    ...
    formMessage(runtime: IAgentRuntime, tweet: ActionableTweet): Memory {
        return {
            ...
            metadata: buildTwitterMessageMetadata(tweet, createUniqueUuid(runtime, tweet.userId), tweet.timestamp, this.client.accountId),
            createdAt: tweet.timestamp,
        };
    }
    async handleTimeline() {
        return this.client.withAuthenticatedSession((session) => this.handleTimelineForProfile(session.profile, session));
    }
    private async handleTimelineForProfile(profile: TwitterProfile, session: TwitterAccountSession) {
        ...
        const maxActionsPerCycle = parseInt((getSetting(this.runtime, "TWITTER_MAX_ENGAGEMENTS_PER_RUN") as string) ||
...
Choose any combination of [LIKE], [RETWEET], [QUOTE], and [REPLY] that are appropriate. Each action must be on its own line. Your response must only include the chosen actions.`;
                ...
                const parsedResponse = parseActionResponseFromText(actionResponse).actions;
                ...
                if (tweetDecisions.length >= maxActionsPerCycle)
                    break;
            }
            catch (error) {
                logger.error(`Error processing tweet ${tweet.id}:`, errorMessage(error));
            }
        }
        // Rank by the quality of the response
        const rankByActionRelevance = (arr: TweetDecision[]): TweetDecision[] => {
            return arr.sort((a, b) => {
                const countTrue = (obj: typeof a.actionResponse) => Object.values(obj).filter(Boolean).length;
        ...
        if (prioritizedTweets.length > 0) {
            const actionSummary = prioritizedTweets.map((td: TweetDecision) => {
                ...
                if (td.actionResponse.like)
                    actions.push("LIKE");
                if (td.actionResponse.retweet)
                    actions.push("RETWEET");
                if (td.actionResponse.quote)
                    actions.push("QUOTE");
                if (td.actionResponse.reply)
                    actions.push("REPLY");
    ...
    private async processTimelineActions(tweetDecisions: TweetDecision[], session: TwitterAccountSession): Promise<{
        tweetId: string;
        actionResponse: ActionResponse;
        ...
        for (const { tweet, actionResponse, tweetState: _tweetState, roomId, mediaDescriptions, } of tweetDecisions) {
            ...
            const tweetMemory: Memory = {
                ...
                roomId,
                metadata: buildTwitterMessageMetadata(tweet, createUniqueUuid(this.runtime, tweet.userId), tweet.timestamp, this.client.accountId),
                createdAt: tweet.timestamp,
            };
            try {
    ...
    private assertCurrentSession(session: TwitterAccountSession): void {
        if (!this.client.isAuthenticatedSessionCurrent(session)) {
            throw new ElizaError("X credentials rotated before a timeline action was executed", { code: "X_AUTH_SESSION_ROTATED" });
        }
    }
    private async ensureTweetWorldContext(tweet: ActionableTweet, _roomId: UUID, _worldId: UUID, _entityId: UUID) {
    ...
    async handleLikeAction(tweet: ActionableTweet, session?: TwitterAccountSession): Promise<boolean> {
        if (this.isDryRun) {
    ...
    async handleRetweetAction(tweet: ActionableTweet, session?: TwitterAccountSession): Promise<boolean> {
        if (this.isDryRun) {
    ...
    async handleQuoteAction(tweet: ActionableTweet, mediaDescriptions: string = "", session?: TwitterAccountSession): Promise<boolean> {
        try {
            const message = this.formMessage(this.runtime, tweet);
            const state = await this.runtime.composeState(message);
            const quotePrompt = composePromptFromState({
                state,
                template: this.runtime.character.templates?.quoteTweetTemplate ||
                    quoteTweetTemplate,
            }) +
                `
...
${tweet.text}${mediaDescriptions}`;
            ...
            const responseObject = (parseJSONObjectFromText(quoteResponse) as Record<string, unknown> | null) ?? {};
            if (responseObject.post) {
                if (this.isDryRun) {
                    logger.log(`[DRY RUN] Would have quoted tweet ${tweet.id} with: ${responseObject.post}`);
                    return true;
                }
                const receipts = await sendTextAsTweetThread(String(responseObject.post), async (chunk, previousTweetId, index) => {
                ...
                try {
                    ...
                    const memoryIds = receipts.map((receipt) => createUniqueUuid(this.runtime, receipt.id));
    ...
    async handleReplyAction(tweet: ActionableTweet, mediaDescriptions: string = "", session?: TwitterAccountSession): Promise<boolean> {
        try {
            const message = this.formMessage(this.runtime, tweet);
            const state = await this.runtime.composeState(message);
            const replyPrompt = composePromptFromState({
                state,
                template: this.runtime.character.templates?.replyTweetTemplate ||
                    replyTweetTemplate,
            }) +
                `
...
${tweet.text}${mediaDescriptions}`;
            ...
            const responseObject = (parseJSONObjectFromText(replyResponse) as Record<string, unknown> | null) ?? {};
            if (responseObject.post) {
                if (this.isDryRun) {
                    logger.log(`[DRY RUN] Would have replied to tweet ${tweet.id} with: ${responseObject.post}`);
                    return true;
                }
                const sendReply = () => sendChunkedTweet(this.client, { text: String(responseObject.post) }, message.roomId, tweet.username, tweet.id);
                if (session)
                    this.assertCurrentSession(session);
                ...
                if (responseMemories.length > 0) {
                    ...
                    try {
                        for (const [index, memory] of responseMemories.entries()) {
                            const tweetId = typeof memory.metadata === "object"
                                ? (memory.metadata as {
                        ...
                        this.runtime.reportError("XTimeline.replyReceipt", error, {
                            accountId: this.client.accountId,
                            tweetId: tweet.id,
                            replyId: (responseMemories[0]?.metadata as {
                                messageIdFull?: string;
                            } | undefined)?.messageIdFull,
                        });
                    }
