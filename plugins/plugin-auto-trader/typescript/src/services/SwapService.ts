...
export class SwapService extends Service {
  ...
  public readonly capabilityDescription = 'Executes token swaps on Solana via Jupiter DEX aggregator';
  ...
  private async initialize(): Promise<void> {
    ...
    if (privateKeyString) {
      ...
      logger.warn(`[${SwapService.serviceType}] No wallet private key configured - swaps will fail`);
    }
  ...
  public async getQuote(params: SwapParams): Promise<SwapQuote | null> {
    try {
      ...
      logger.info(`[SwapService] Quote: ${params.amount} ${params.inputMint.slice(0, 8)}... → ${params.outputMint.slice(0, 8)}...`);
  ...
  public async executeSwap(params: SwapParams): Promise<SwapResult> {
    ...
    try {
      ...
      logger.info(`[SwapService] Tx sent: ${signature.slice(0, 16)}... (https://solscan.io/tx/${signature})`);
      ...
      logger.info(`[SwapService] Swap complete: ${params.amount} → ${await this.smallestUnitToAmount(params.outputMint, quote.outAmount)}`);
  ...
  private async confirmTransaction(signature: string): Promise<boolean> {
    for (let attempt = 0; attempt < CONFIRMATION_CONFIG.MAX_ATTEMPTS; attempt++) {
      ...
      const confirmationStatus = status.value?.confirmationStatus as TransactionConfirmationStatus | undefined;
      if (confirmationStatus === 'confirmed' || confirmationStatus === 'finalized') {
        logger.info(`[SwapService] Confirmed: ${signature.slice(0, 16)}... (${confirmationStatus})`);
        return true;
      }

      const delay = CONFIRMATION_CONFIG.getDelayForAttempt(attempt);
      logger.debug(`[${SwapService.serviceType}] Waiting ${delay}ms for confirmation (attempt ${attempt + 1})`);
   ...
   */
  public async buy(tokenMint: string, solAmount: number, slippageBps?: number): Promise<SwapResult> {
   ...
   */
  public async sell(tokenMint: string, tokenAmount: number, slippageBps?: number): Promise<SwapResult> {
    return this.executeSwap({
      inputMint: tokenMint,
