...
export const getMarketAnalysisAction: Action = {
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory) => {
    ...
    const keywords = ['analyze', 'analysis', 'market', 'trending', 'outlook', 'sentiment', 'overview'];
  ...
  handler: async (
    ...
    const validator = runtime.getService('TokenValidationService') as TokenValidationService | undefined;

    if (!resolver) {
      callback?.({ text: '❌ Market data service not available. Configure BIRDEYE_API_KEY.' });
      return;
    }
  ...
  examples: [
    [
      ...
      { name: '{{agentName}}', content: { text: 'Here is the current market overview...' }}
    ],
    [
