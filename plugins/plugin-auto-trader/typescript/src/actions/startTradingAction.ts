...
async function extractTokens(text: string, resolver: TokenResolverService | undefined): Promise<string[]> {
...
export const startTradingAction: Action = {
  ...
  examples: [
    [
      { name: '{{user1}}', content: { text: 'Start trading with the LLM strategy' }},
      { name: '{{agentName}}', content: { text: '🚀 Auto-trading started!\nStrategy: LLM Trading Strategy\nMode: Analyzing trending tokens from Birdeye' }}
    ],
    [
      { name: '{{user1}}', content: { text: 'Start momentum trading on BONK with $500 max position' }},
      { name: '{{agentName}}', content: { text: '🚀 Auto-trading started!\nStrategy: Momentum Breakout\nTokens: BONK' }}
    ],
    [
      { name: '{{user1}}', content: { text: 'Trade PEPE and DOGE using AI strategy' }},
      { name: '{{agentName}}', content: { text: '🚀 Auto-trading started!\nStrategy: LLM Trading Strategy\nTokens: PEPE, DOGE' }}
    ],
    [
      { name: '{{user1}}', content: { text: 'Begin trading EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm' }},
      { name: '{{agentName}}', content: { text: '🚀 Auto-trading started!\nTokens: WIF (EKpQGSJ...)' }}
    ],
  ],
  ...
  handler: async (
    ...
    const autoTradingManager = runtime.getService('AutoTradingManager') as AutoTradingManager | undefined;
    const resolver = runtime.getService('TokenResolverService') as TokenResolverService | undefined;
    
    if (!autoTradingManager) {
      callback?.({ text: '❌ AutoTradingManager not found. Ensure plugin is loaded.', action: 'START_TRADING' });
      return;
    }
    ...
    if (percentMatch) {
      maxPositionSize = parseInt(percentMatch[1]) / 100;
    }
    
    const amountMatch = text.match(/\$(\d+)/);
    if (amountMatch) {
      maxPositionSize = Math.min(parseInt(amountMatch[1]) / 1000, 0.25);
    }
    ...
    if (slMatch) stopLossPercent = parseInt(slMatch[1]);

    const tpMatch = text.match(/take\s*profit\s*(?:at\s*)?(\d+)%?/);
    if (tpMatch) takeProfitPercent = parseInt(tpMatch[1]);
    ...
    if (intervalMatch) {
      ...
      else if (unit.startsWith('hour') || unit.startsWith('hr')) intervalMs = value * 60 * 60 * 1000;
    }
    ...
    if (tokens[0] === 'auto') {
      ...
      const names = await Promise.all(tokens.map(async addr => {
    ...
    const modeWarning = tradingMode === 'live' 
