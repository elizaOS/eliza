...
import { SwapService, KNOWN_TOKENS } from '../services/SwapService.ts';
...
const TRADE_KEYWORDS = ['live trade', 'real trade', 'execute', 'place order', 'buy', 'sell', 'swap', 'trade', 'purchase', 'exchange'];
...
async function parseTradeParams(
...
): Promise<{ isSell: boolean; amount: number; tokenSymbol: string; tokenAddress: string | null }> {
  ...
  if (addressMatch) {
    ...
    return { isSell, amount, tokenSymbol: address.slice(0, 8) + '...', tokenAddress: address };
  }
...
export const executeLiveTradeAction: Action = {
  ...
  similes: ['LIVE_TRADE', 'REAL_TRADE', 'EXECUTE_TRADE', 'PLACE_ORDER', 'MAKE_TRADE', 'SWAP', 'BUY_TOKEN', 'SELL_TOKEN'],
  ...
  handler: async (
    ...
    // Validate amount
    if (amount <= 0) {
      return respond(callback, `Please specify a valid amount. Example: "buy 0.5 SOL worth of ${tokenSymbol || 'BONK'}"`);
    }
    ...
    if (balances.solBalance < amount) {
      return respond(callback, `❌ **Insufficient SOL**\n\nNeed: ${amount} SOL\nHave: ${balances.solBalance.toFixed(4)} SOL`);
    }
