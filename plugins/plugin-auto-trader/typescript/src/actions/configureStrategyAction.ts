...
export const configureStrategyAction: Action = {
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory) => {
    ...
    return ['configure', 'config', 'set', 'adjust', 'parameter', 'setting'].some(kw => text.includes(kw));
  },
  ...
  handler: async (
    ...
    if (slMatch) {
      const value = parseInt(slMatch[1]);
    ...
    if (tpMatch) {
      const value = parseInt(tpMatch[1]);
...
Use "configure" to see current settings.`;
  ...
  examples: [
    [
      { name: '{{user1}}', content: { text: 'Set stop loss to 3% and take profit to 25%' }},
      { name: '{{agentName}}', content: { text: 'Settings updated: Stop loss 3%, Take profit 25%' }}
    ],
    [
      { name: '{{user1}}', content: { text: 'Configure minimum liquidity to $100k' }},
      { name: '{{agentName}}', content: { text: 'Minimum liquidity set to $100,000' }}
    ],
  ],
};
