...
export const compareStrategiesAction: Action = {
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory) => {
    ...
    return ['compare', 'strategies', 'which strategy', 'best strategy', 'list strategy'].some(kw => text.includes(kw));
  },
  ...
  handler: async (
    ...
    strategies.forEach((strategy, index) => {
      ...
      const emoji = strategy.id === 'llm' ? '🤖' : 
                    strategy.id.includes('momentum') ? '📈' :
                    strategy.id.includes('reversion') ? '↔️' :
...
Would you like to start trading with a specific strategy?`;
  ...
  examples: [
    [
      { name: '{{user1}}', content: { text: 'Compare the different trading strategies' }},
      { name: '{{agentName}}', content: { text: 'Here is a comparison of available strategies...' }}
    ],
    [
      { name: '{{user1}}', content: { text: 'Which strategy is best?' }},
      { name: '{{agentName}}', content: { text: 'Let me compare the strategies for you...' }}
    ],
  ],
};
