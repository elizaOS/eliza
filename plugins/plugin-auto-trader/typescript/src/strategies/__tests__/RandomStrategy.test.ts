...
describe("RandomStrategy", () => {
  ...
  describe("decide", () => {
    ...
    it("should use fixedTradeQuantity if availableCapital or currentPrice is missing/zero", async () => {
      ...
      let order = await strategy.decide({
      ...
      const psNoValue: PortfolioSnapshot = {
        ...portfolioSnapshot,
        totalValue: 0,
      };
      order = await strategy.decide({
        marketData,
        agentState,
        portfolioSnapshot: psNoValue,
      });
      expect(order?.quantity).toBe(5);
    });
    ...
    it("should return null if calculated quantity is zero or negative (below threshold)", async () => {
      strategy.configure({
        tradeAttemptProbability: 1,
        maxTradeSizePercentage: 0.000000001,
      }); // Very small percentage
      vi.spyOn(Math, "random").mockReturnValue(0);
      const smallPortfolio: PortfolioSnapshot = {
        ...portfolioSnapshot,
        totalValue: 1,
      };
      const order = await strategy.decide({
      ...
      const order2 = await strategy.decide({
        marketData,
        agentState,
        portfolioSnapshot,
      });
    ...
    it("should generate a valid TradeOrder structure with fixed quantity", async () => {
      ...
      strategy.configure({
        tradeAttemptProbability: 1,
        fixedTradeQuantity: 1.234567891,
      });
      vi.spyOn(Math, "random").mockReturnValue(0);
      const order = await strategy.decide({
        marketData,
