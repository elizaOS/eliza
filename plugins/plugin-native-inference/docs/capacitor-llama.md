...
- **Mobile token boundary:** omitted `maxTokens` uses the device ceiling of 256; an explicit larger request is rejected before decode, and reaching the ceiling is surfaced as incomplete instead of returning a partial response. Adjust `MOBILE_MAX_TOKENS_CAP` in `capacitor-llama-adapter.ts` only with device evidence.
