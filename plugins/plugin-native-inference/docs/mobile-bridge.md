...
Package exports:
- `./host-bridge` — host entry (re-exports all of the above)
...
```
src/
  bridge.ts                         Re-exports from android/, ios/, mobile-device-bridge-bootstrap, shared/
...
android-bridge/                     Native Capacitor manifest fragment + Kotlin computer-use services
...
```bash
bun run --cwd plugins/plugin-native-inference build           # tsup build (runs check:android-manifest first)
bun run --cwd plugins/plugin-native-inference check:android-manifest  # validate AndroidManifest.xml
bun run --cwd plugins/plugin-native-inference dev             # tsup --watch
bun run --cwd plugins/plugin-native-inference typecheck       # tsc against the mobile-boundary build config
bun run --cwd plugins/plugin-native-inference lint            # biome check --write --unsafe
bun run --cwd plugins/plugin-native-inference lint:check      # biome check (read-only)
bun run --cwd plugins/plugin-native-inference format          # biome format --write
bun run --cwd plugins/plugin-native-inference format:check    # biome format (read-only)
bun run --cwd plugins/plugin-native-inference clean           # rm -rf dist .turbo node_modules
```
...
- The bridge plugin owns the mobile service lifecycle; CLI helpers remain explicitly invoked by the agent host.
