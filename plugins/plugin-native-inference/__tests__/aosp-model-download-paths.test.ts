...
import { assertAospModelDownloadSize } from "../src/aosp-model-paths.js";
import { bundleSlugFromModelName } from "../src/aosp-model-paths.js";
import { describe } from "bun:test";
import { expect } from "bun:test";
import { fetchRecommendedAospModel } from "../src/aosp-model-paths.js";
import { findCatalogModel } from "@elizaos/plugin-native-inference/model-catalog/catalog";
import { it } from "bun:test";
import { resolveRecommendedAospModel } from "../src/aosp-model-paths.js";
...
describe("AOSP published model resolution", () => {
    it("resolves the first-run chat download through the shared catalog", () => {
        ...
        if (!catalogModel)
            throw new Error("first-run catalog model is missing");
        ...
        expect(resolved.ggufFile).toBe([catalogModel.hfPathPrefix, catalogModel.ggufFile]
            .filter(Boolean)
            .join("/"));
    });
    ...
    it("requests the pinned public BGE artifact and rejects a partial response", async () => {
        ...
        await fetchRecommendedAospModel({
            ...resolved,
            candidates: resolved.candidates.filter((candidate) => candidate.label === "direct"),
        }, async (url) => {
            requested.push(url);
        ...
        expect(() => assertAospModelDownloadSize(resolved, 10)).toThrow(/size 10 != expected 67308128/);
    });
    it("rejects a partial 200 response before it can become the live model", () => {
        const model = resolveRecommendedAospModel("chat");
        expect(() => assertAospModelDownloadSize(model, 4967494591)).toThrow(/size 4967494591 != expected 4967494592/);
