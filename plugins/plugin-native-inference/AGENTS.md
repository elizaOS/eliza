...
```bash
bun run --cwd plugins/plugin-native-inference build       # tsc --noCheck --noEmit (no type-checking, no output; parse validation only)
bun run --cwd plugins/plugin-native-inference typecheck   # tsgo full typecheck
bun run --cwd plugins/plugin-native-inference clean       # remove dist, .turbo, node_modules
bun run --cwd plugins/plugin-native-inference test        # bun test __tests__
```
