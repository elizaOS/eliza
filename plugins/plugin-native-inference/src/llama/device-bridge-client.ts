 ...
 * `@elizaos/app/src/services/local-inference/device-bridge.ts`.
...
export class DeviceBridgeClient {
    ...
    private connect(): void {
        ...
        try {
            ...
            this.config.onStateChange?.("error", err instanceof Error ? err.message : String(err));
    ...
    private async sendRegister(ws: WebSocket): Promise<void> {
        ...
        const gpu = useNativeOverride
            ? native?.gpu?.available
                ? ({
                    backend: native.gpu.backend === "metal" ||
    ...
    private async handleAgentMessage(ws: WebSocket, msg: AgentInbound): Promise<void> {
        if (msg.type === "ping") {
        ...
        if (msg.type === "load") {
            try {
                const capacitorLlama = await loadCapacitorLlama(msg.modelPath.split("/").pop() === BGE_EMBEDDING_MODEL.filename
                    ? "embedding"
                    : "chat");
        ...
        if (msg.type === "embed") {
            try {
                ...
                this.send(ws, {
                    ...
                    correlationId: msg.correlationId,
                    ok: false,
                    code: err instanceof ElizaError
                        ? err.code
        ...
        if (msg.type === "formatChat") {
            try {
                const capacitorLlama = await loadCapacitorLlama();
                const prompt = typeof capacitorLlama.formatChat === "function"
 ...
 */
export function startDeviceBridgeClient(config: DeviceBridgeClientConfig): DeviceBridgeClient {
