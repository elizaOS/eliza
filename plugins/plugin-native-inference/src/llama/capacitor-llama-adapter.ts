...
import { type EmbedOptions } from "./definitions.js";
import { type EmbedResult } from "./definitions.js";
import { type GenerateOptions } from "./definitions.js";
import { type GenerateResult } from "./definitions.js";
import { type GenerateStreamOptions } from "./definitions.js";
import { type GenerationEvent } from "./definitions.js";
import { type HardwareInfo } from "./definitions.js";
import { type LlamaAdapter } from "./definitions.js";
import { type LoadOptions } from "./definitions.js";
import { type NativeCompletionParams } from "llama-cpp-capacitor";
import { type NativeCompletionResult } from "llama-cpp-capacitor";
import { type NativeContextParams } from "llama-cpp-capacitor";
import { type NativeEmbeddingParams } from "llama-cpp-capacitor";
import { type NativeEmbeddingResult } from "llama-cpp-capacitor";
import { type NativeLlamaContext } from "llama-cpp-capacitor";
import { type PluginListenerHandle } from "@capacitor/core";
import { type SamplerStage } from "./definitions.js";
import { type SetSpecTypeArgs } from "./definitions.js";
...
type NativeCompletionProbability = NonNullable<NativeCompletionResult["completion_probabilities"]>[number];
...
interface LlamaCppPluginLike {
    ...
    addListener: (event: string, listener: (data: TokenEventPayload) => void) => Promise<PluginListenerHandle | undefined>;
}
...
function toPlainLlamaCppPlugin(plugin: LlamaCppPluginLike): LlamaCppPluginLike {
    return {
        ...
        generateText: typeof plugin.generateText === "function"
            ? (options) => plugin.generateText?.(options) as Promise<NativeCompletionResult>
            : undefined,
        ...
        tokenize: typeof plugin.tokenize === "function"
            ? (options) => plugin.tokenize?.(options) as Promise<{
                tokens: number[];
            }>
            : undefined,
...
function resolveMobileMaxTokens(requested?: number): number {
    ...
    if (!Number.isSafeInteger(requested) || requested <= 0) {
        throw new RangeError(`[capacitor-llama] maxTokens must be a positive safe integer; received ${String(requested)}`);
    }
    if (requested > MOBILE_MAX_TOKENS_CAP) {
        throw new RangeError(`[capacitor-llama] requested maxTokens=${requested} exceeds the mobile decode ceiling ${MOBILE_MAX_TOKENS_CAP}`);
    }
    return requested;
}
function incompleteOutputError(maxTokens: number): Error & {
    ...
    return Object.assign(new Error(`[capacitor-llama] native generation reached maxTokens=${maxTokens} before a stop condition; no partial response was returned`), { code: "MODEL_OUTPUT_INCOMPLETE" });
}
...
function fallbackHardwareInfo(platform = detectPlatform(), reason = "native hardware probe unavailable"): HardwareInfo {
...
function normalizeForkVariant(value: unknown): "buun-llama-cpp" | "stock-llama-cpp" | null | undefined {
...
function normalizeHardwareInfo(value: Partial<HardwareInfo> | null | undefined, platform = detectPlatform()): HardwareInfo {
    ...
    return {
        ...
        gpu,
        gpuSupported: booleanFromUnknown(value.gpuSupported) ?? fallback.gpuSupported,
        ...
        mtpReason: stringFromUnknown(value.mtpReason) ??
            (value.mtpSupported
                ? undefined
...
export class CapacitorLlamaAdapter implements LlamaAdapter {
    ...
    private serializeLifecycle<T>(operation: () => Promise<T>): Promise<T> {
        ...
        this.lifecycleQueue = result.then(() => undefined, () => {
    ...
    private async isIosSimulator(): Promise<boolean> {
        if (detectPlatform() !== "ios")
            return false;
        if (this.iosSimulatorProbe !== null)
            return this.iosSimulatorProbe;
        let result = false;
    ...
    async getHardwareInfo(): Promise<HardwareInfo> {
        ...
        try {
            ...
            const baseInfo = normalizeHardwareInfo(await plugin.getHardwareInfo?.(), platform);
            ...
            let forkVariant: HardwareInfo["forkVariant"] = baseInfo.forkVariant ?? "stock-llama-cpp";
    ...
    private async loadContext(options: LoadOptions): Promise<void> {
        ...
        if (options.modelPath.split("/").pop() === BGE_EMBEDDING_MODEL.filename) {
            ...
            if (typeof native.initBgeEmbedding !== "function") {
                throw new ElizaError("Install the fused mobile BGE bridge before embedding", { code: "EMBEDDING_BACKEND_UNAVAILABLE" });
            }
        ...
        const nativeGpuEnabled = resolveNativeGpuEnabled(options.useGpu) && !(await this.isIosSimulator());
     ...
     */
    private async runNativeCompletion(options: GenerateOptions, params: NativeGenerateParams): Promise<NativeCompletionResult> {
    ...
    async generate(options: GenerateOptions): Promise<GenerateResult> {
        ...
        let finishReason: "stop" | "length" | "tool" | "cancel" | "error" | null = null;
     ...
     */
    async *generateStream(options: GenerateStreamOptions): AsyncIterable<GenerationEvent> {
        ...
        if (completionState.error) {
            ...
            try {
                ...
                console.warn("[capacitor-llama] failed to unload after generation error", {
                    error: unloadErr instanceof Error
                        ? unloadErr.message
                        : String(unloadErr),
                });
            }
        ...
        if (completionState.result) {
            ...
            const duration = r.timings?.predicted_ms != null
            ...
            const finishReason: "stop" | "length" = r.tokens_predicted >= requested ? "length" : "stop";
    ...
    async setDrafter(drafterPath: string | null): Promise<void> {
        ...
        console.warn(`[capacitor-llama] setDrafter(${drafterPath ?? "null"}) not yet supported by native bridge; pass draftModelPath to load() instead`);
    }
    ...
    private async embedContext(options: EmbedOptions): Promise<EmbedResult> {
        if (this.bgeContext) {
            const prepared = prepareBgeEmbeddingInput(options.input, this.bgeContextLimit);
            if (options.expectedTokenIds !== undefined ||
                options.embeddingSpace !== undefined) {
                if (options.embeddingSpace !== BGE_SMALL_VECTOR_SPACE ||
                    !Array.isArray(options.expectedTokenIds)) {
                    throw new ElizaError("The relay must supply canonical BGE admission", { code: "EMBEDDING_TOKENIZER_MISMATCH" });
                }
            ...
            if (!tokenized || !Array.isArray(tokenized.tokens)) {
                throw new ElizaError("Mobile BGE tokenizer returned invalid admission data", { code: "EMBEDDING_BACKEND_UNAVAILABLE" });
            }
            ...
            return {
                embedding: identifyEmbeddingVector(result.embedding, BGE_SMALL_VECTOR_SPACE),
...
export class CapacitorLlamaLoaderRuntimeService {
    ...
    readonly capabilityDescription = "Owns isolated Capacitor llama.cpp chat and embedding contexts.";
    private readonly chatAdapter = new CapacitorLlamaAdapter();
    private readonly embeddingAdapter = new CapacitorLlamaAdapter();
    static async start(_runtime: unknown): Promise<CapacitorLlamaLoaderRuntimeService> {
    ...
    currentModelPath(): string | null {
        return (this.chatAdapter.currentModelPath() ??
