...
import { ELIZA_1_TIER_IDS } from "@elizaos/plugin-native-inference/model-catalog/catalog";
import { FIRST_RUN_DEFAULT_MODEL_ID } from "@elizaos/plugin-native-inference/model-catalog/catalog";
import { buildHuggingFaceResolveUrlCandidatesForPath } from "@elizaos/plugin-native-inference/model-catalog/catalog";
import { findCatalogModel } from "@elizaos/plugin-native-inference/model-catalog/catalog";
import { resolveHfDownloadBases } from "@elizaos/plugin-native-inference/model-catalog/hf-proxy";
import { tierBundleSlug } from "@elizaos/plugin-native-inference/model-catalog/catalog";
import { type HfResolveUrlCandidate } from "@elizaos/plugin-native-inference/model-catalog/catalog";
...
export type AospModelFetch = (input: string, init?: RequestInit) => Promise<Response>;
function isTransientDownloadStatus(status: number): boolean {
    return status === 408 || status === 429 || status >= 500;
}
const AOSP_CHAT_MODEL_SIZE_BYTES = 4967494592;
export function resolveRecommendedAospModel(role: "chat" | "embedding"): AospRecommendedModel {
    if (role === "embedding") {
    ...
    if (model?.category !== "chat") {
        throw new Error(`[aosp-local-inference] Catalog is missing ${role} source tier ${tierId}.`);
    }
    ...
    return {
        id: model.id,
        ggufFile,
        candidates: buildHuggingFaceResolveUrlCandidatesForPath(model, model.ggufFile),
        expectedSizeBytes: AOSP_CHAT_MODEL_SIZE_BYTES,
    };
}
export function assertAospModelDownloadSize(model: AospRecommendedModel, actualSizeBytes: number): void {
    if (model.expectedSizeBytes !== undefined &&
        actualSizeBytes !== model.expectedSizeBytes) {
        throw new Error(`[aosp-local-inference] Downloaded ${model.ggufFile} size ${actualSizeBytes} != expected ${model.expectedSizeBytes}.`);
    }
}
export async function fetchRecommendedAospModel(model: AospRecommendedModel, fetchImpl: AospModelFetch = fetch): Promise<{
    response: Response;
    ...
    for (let index = 0; index < model.candidates.length; index += 1) {
        ...
        try {
            ...
            if (isTransientDownloadStatus(response.status) &&
                index < model.candidates.length - 1) {
                await response.body?.cancel();
                lastError = new Error(`HTTP ${response.status} from ${candidate.label ?? candidate.base}`);
                continue;
            }
...
export function bundleSlugFromModelName(modelNameOrId: string): string {
    ...
    for (const id of [...ELIZA_1_TIER_IDS].reverse()) {
        ...
        if (lower.includes(`eliza-1-${stableSlug}`) ||
