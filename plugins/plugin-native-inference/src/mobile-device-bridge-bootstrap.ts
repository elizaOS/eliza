...
import { MODEL_DOWNLOAD_IDLE_TIMEOUT_MS } from "./shared/model-download-deadline.ts";
import { MODEL_DOWNLOAD_TOTAL_TIMEOUT_MS } from "./shared/model-download-deadline.ts";
import { MobileDeviceBridgeService } from "@elizaos/core";
import { ModelType } from "@elizaos/core";
import { ServiceType } from "@elizaos/core";
import { applyBackgroundInferenceBudget } from "@elizaos/core";
import { assertBgeTokenAgreement } from "@elizaos/plugin-native-inference/model-catalog/bge-input";
...
import { resolveBackgroundInferenceBudget } from "@elizaos/core";
...
import { type LocalInferencePriority } from "@elizaos/core";
import { type MobileDeviceBridgeStatus } from "@elizaos/core";
import { type Plugin } from "@elizaos/core";
import { type PreparedModelRequestGuard } from "@elizaos/core";
 ...
 */
function pairingTokenMatches(expected: string, provided: string | null | undefined): boolean {
    if (!provided)
        return false;
...
const ELIZA_1_LOAD_METADATA: Record<string, {
    contextSize: number;
    mtp?: {
        drafterFile: string;
        draftMin: number;
...
type GenerateTextHandler = (runtime: IAgentRuntime, params: GenerateTextParams) => Promise<string>;
type EmbeddingHandler = (runtime: IAgentRuntime, params: TextEmbeddingParams | string | null) => Promise<number[]>;
...
type RuntimeWithModelRegistration = AgentRuntime & {
    getModel: (modelType: string | number) => GenerateTextHandler | EmbeddingHandler | undefined;
    registerModel: (modelType: string | number, handler: GenerateTextHandler | EmbeddingHandler, provider: string, priority?: number) => void;
};
...
interface WssInstance {
    handleUpgrade(request: IncomingMessage, socket: Duplex, head: Buffer, cb: (ws: MinimalWebSocket) => void): void;
    on(event: "error", listener: (err: Error) => void): unknown;
...
class MobileDeviceBridge {
    ...
    private readonly heartbeatTimers = new Map<MinimalWebSocket, ReturnType<typeof setInterval>>();
    ...
    private readonly pendingFormatChats = new Map<string, Pending<string | null>>();
    private readonly expectedPairingToken = process.env.ELIZA_DEVICE_PAIRING_TOKEN?.trim() ||
    ...
    status(): MobileDeviceBridgeStatus {
        ...
        return {
            ...
            pendingRequests: this.pendingLoads.size +
    ...
    async attachToHttpServer(server: HttpServer): Promise<boolean> {
        ...
        if (!this.expectedPairingToken) {
            logger.warn("[mobile-device-bridge] Disabled: ELIZA_DEVICE_PAIRING_TOKEN is required when ELIZA_DEVICE_BRIDGE_ENABLED=1");
            return false;
        }
        ...
        this.loadTimeoutMs = resolveTimeoutMs("ELIZA_DEVICE_LOAD_TIMEOUT_MS", DEFAULT_LOAD_TIMEOUT_MS);
        this.generateTimeoutMs = resolveTimeoutMs("ELIZA_DEVICE_GENERATE_TIMEOUT_MS", DEFAULT_CALL_TIMEOUT_MS);
        this.embedTimeoutMs = resolveTimeoutMs("ELIZA_DEVICE_EMBED_TIMEOUT_MS", DEFAULT_CALL_TIMEOUT_MS);
        const serverCloseHandler = () => {
            ...
            void this.close().catch((error) => {
                logger.warn("[mobile-device-bridge] Server-owned transport teardown failed:", error instanceof Error ? error.message : String(error));
            });
        };
        ...
        const upgradeHandler: UpgradeHandler = (request, socket, head) => {
            ...
            if (url.pathname !== DEVICE_BRIDGE_PATH)
                return;
        ...
        logger.info(`[mobile-device-bridge] Listening for Capacitor device bridge at ${DEVICE_BRIDGE_PATH}`);
        return true;
    }
    private handleConnection(socket: MinimalWebSocket, WsCtor: WsConstructor, url: URL) {
        ...
        socket.on("message", (raw) => {
            ...
            if (!registeredDeviceId) {
                ...
                if (msg.payload.capabilities.platform === "ios") {
                    logger.warn("[mobile-device-bridge] Rejecting iOS registration: use native IPC");
                    socket.close(4003, "ios-ipc-required");
                    return;
                }
                if (!this.expectedPairingToken ||
                    !pairingTokenMatches(this.expectedPairingToken, msg.payload.pairingToken)) {
                    logger.warn("[mobile-device-bridge] Rejecting register: bad pairing token");
                    socket.close(4001, "unauthorized");
                    return;
                }
                ...
                logger.info(`[mobile-device-bridge] Device connected: ${registeredDeviceId} (${msg.payload.capabilities.platform})`);
        ...
        socket.on("close", () => {
            ...
            if (current?.socket === socket) {
                ...
                logger.info(`[mobile-device-bridge] Device disconnected: ${registeredDeviceId}`);
            }
        });
        socket.on("error", (err) => {
        ...
        const heartbeat = setInterval(() => {
            if (!registeredDeviceId || socket.readyState !== WsCtor.OPEN)
                return;
            try {
    ...
    private handleDeviceMessage(msg: DeviceOutbound): void {
        if (msg.type === "pong" || msg.type === "register")
            return;
        ...
        if (msg.type === "embedResult") {
            ...
            if (msg.ok === true) {
                ...
                pending.reject(new ElizaError(msg.error, {
    ...
    private rejectPending<T>(pending: Map<string, Pending<T>>, error: Error): void {
    ...
    async close(): Promise<void> {
        ...
        if (server && serverCloseHandler)
            server.off("close", serverCloseHandler);
        this.attachListeners.clear();
        const stopped = new Error("DEVICE_BRIDGE_STOPPED: mobile device bridge runtime stopped");
        ...
        if (closeErrors.length > 0) {
            throw new AggregateError(closeErrors, "Mobile device bridge transport teardown failed");
        }
    }
    private sendToPrimary<T>(pendingMap: Map<string, Pending<T>>, makeMessage: (correlationId: string) => AgentOutbound, timeoutMs: number, timeoutMessage: string, preparedRequest?: PreparedModelRequestGuard): Promise<T> {
        const device = this.primaryDevice();
        if (!device) {
            return Promise.reject(new Error("DEVICE_DISCONNECTED: no Capacitor llama device bridge attached"));
        }
    ...
    async loadModel(args: LocalInferenceLoadArgs): Promise<void> {
        const device = this.primaryDevice();
        if (device?.loadedPath === args.modelPath)
            return;
        return this.sendToPrimary<void>(this.pendingLoads, (correlationId) => ({
            type: "load",
            correlationId,
            ...args,
        }), this.loadTimeoutMs, "DEVICE_TIMEOUT: model load exceeded deadline");
    }
    async unloadModel(): Promise<void> {
        const device = this.primaryDevice();
        if (!device?.loadedPath)
            return;
        return this.sendToPrimary<void>(this.pendingUnloads, (correlationId) => ({ type: "unload", correlationId }), this.generateTimeoutMs, "DEVICE_TIMEOUT: unload exceeded deadline");
    }
    generate(args: {
        prompt: string;
        stopSequences?: string[];
        ...
        const preparedRequest = createNativeModelRequestGuard({
            provider: PROVIDER,
            model: args.model ??
                path.basename(this.primaryDevice()?.loadedPath ?? "capacitor-native-llama"),
        ...
        return this.sendToPrimary<string>(this.pendingGenerates, (correlationId) => ({
            type: "generate",
            correlationId,
            ...request,
        }), this.generateTimeoutMs, "DEVICE_TIMEOUT: no device responded within deadline", preparedRequest);
    }
    async embed(args: {
        input: string;
    }): Promise<number[]> {
        const prepared = prepareBgeEmbeddingInput(args.input);
        const response = await this.sendToPrimary<BgeWireResponse>(this.pendingEmbeds, (correlationId) => ({
            type: "embed",
            correlationId,
            input: prepared.text,
            expectedTokenIds: prepared.tokenIds,
            embeddingSpace: BGE_SMALL_VECTOR_SPACE,
        }), this.embedTimeoutMs, "DEVICE_TIMEOUT: no device returned embeddings within deadline");
    ...
    formatChat(messages: {
        ...
        return this.sendToPrimary<string | null>(this.pendingFormatChats, (correlationId) => ({
            type: "formatChat",
            correlationId,
            messages,
        }), this.loadTimeoutMs, "DEVICE_TIMEOUT: chat template format exceeded deadline");
    }
}
...
function resolveTimeoutMs(envKey: string, fallback: number): number {
    ...
    if (!raw)
        return fallback;
    if (!/^(?:0|[1-9]\d*)$/.test(raw)) {
        throw new ElizaError(`${envKey} must be a canonical decimal integer from 1 through ${MAX_TIMER_DELAY_MS}`, {
...
function resolveFromRegistry(slot: string): string | null {
    const assignments = readJsonFile<AssignmentsFile>(assignmentsPath())?.assignments;
...
function resolveAssignedRegistryModel(slot: string): {
    ...
    if (typeof matched?.path !== "string")
        return null;
    const resolvedPath = resolveStoredModelPath(matched.path, localInferenceRoot());
    if (!resolvedPath)
        return null;
    return {
        id: assigned,
...
function resolveManifestModel(slot: string): {
    ...
    const manifest = readJsonFile<BundledModelManifest>(path.join(modelsDir(), "manifest.json"));
...
function resolveGemmaDrafterPath(modelPath: string, drafterFile: string): string | null {
...
function applyManifestLoadHints(args: LocalInferenceLoadArgs, entry: BundledModelManifestEntry): LocalInferenceLoadArgs {
    const contextSize = positiveInteger(entry.contextSize);
    if (contextSize !== null)
        args.contextSize = contextSize;
    if (typeof entry.useGpu === "boolean")
        args.useGpu = entry.useGpu;
    ...
    if (draftContextSize !== null)
        args.draftContextSize = draftContextSize;
    const draftMin = positiveInteger(entry.draftMin);
    if (draftMin !== null)
        args.draftMin = draftMin;
    const draftMax = positiveInteger(entry.draftMax);
    if (draftMax !== null)
        args.draftMax = draftMax;
...
function resolveLocalLoadArgs(slot: string): LocalInferenceLoadArgs | null {
    if (slot === "TEXT_EMBEDDING") {
        ...
        const assignedPath = assigned && path.basename(assigned.path) === BGE_EMBEDDING_MODEL.filename
            ? assigned.path
            : undefined;
        ...
        if (path.basename(modelPath) !== BGE_EMBEDDING_MODEL.filename) {
            throw new ElizaError("The mobile embedding slot requires the pinned BGE-small model", { code: "EMBEDDING_MODEL_MISMATCH" });
        }
    ...
    if (registryModel)
        return buildLoadArgsFromRegistryModel(registryModel);
    const manifestModel = resolveManifestModel(slot);
    if (manifestModel)
        return buildLoadArgsFromManifestModel(manifestModel);
...
export { MODEL_DOWNLOAD_IDLE_TIMEOUT_MS as RECOMMENDED_MODEL_DOWNLOAD_IDLE_TIMEOUT_MS, MODEL_DOWNLOAD_TOTAL_TIMEOUT_MS as RECOMMENDED_MODEL_DOWNLOAD_TOTAL_TIMEOUT_MS, };
...
function buildRecommendedLoadArgs(slot: "TEXT_SMALL" | "TEXT_LARGE" | "TEXT_EMBEDDING", modelPath: string): LocalInferenceLoadArgs {
...
async function downloadRecommendedModelFor(slot: "TEXT_SMALL" | "TEXT_LARGE" | "TEXT_EMBEDDING"): Promise<string> {
    const model = RECOMMENDED_MODELS[slot];
    const dir = modelsDir();
    mkdirSync(dir, { recursive: true });
    const finalPath = path.join(dir, model.localFile ?? path.basename(model.ggufFile));
    if (existsSync(finalPath)) {
        ...
        logger.warn(`[mobile-device-bridge] ${model.ggufFile} present but size ${sz} != expected ${model.expectedSizeBytes}; re-downloading.`);
        try {
    ...
    const promise = (async () => {
        ...
        logger.info(`[mobile-device-bridge] Auto-downloading recommended ${slot} model ${model.id} from ${url}`);
        ...
        logger.info(`[mobile-device-bridge] Auto-download complete: ${finalPath} (${stagedSize} bytes)`);
        return finalPath;
    })();
...
async function resolveLoadArgsWithAutoDownload(slot: "TEXT_SMALL" | "TEXT_LARGE" | "TEXT_EMBEDDING"): Promise<LocalInferenceLoadArgs | null> {
...
function getBionicRequestTimeoutMs(): number {
    if (cachedBionicRequestTimeoutMs === undefined) {
        cachedBionicRequestTimeoutMs = resolveTimeoutMs("ELIZA_BIONIC_REQUEST_TIMEOUT_MS", 300000);
    }
...
function bionicSocketName(): string | null {
    if (process.env.ELIZA_BIONIC_HOST_DELEGATED?.trim() !== "1")
        return null;
 ...
 */
export function resolveBionicStopSequences(requested: readonly string[] | undefined): string[] {
...
export function deriveBionicBundleDir(modelPath: string): string {
    ...
    if (path.basename(dir) === "text")
        return path.dirname(dir);
    if (!FLAT_ELIZA_1_GGUF_RE.test(path.basename(modelPath)))
        return "";
    if (!existsSync(modelPath))
        return "";
    const modelName = path.basename(modelPath);
    const bundleRoot = path.join(dir, BIONIC_FLAT_BUNDLE_DIR, path.basename(modelName, path.extname(modelName)));
    ...
    try {
        ...
        logger.warn(`[mobile-device-bridge] could not stage bionic bundle view for flat model "${modelPath}": ${err instanceof Error ? err.message : String(err)}`);
    }
    return "";
}
...
function collectChatMlPromptMessages(prompt: string, system?: string): {
    role: string;
    content: string;
}[] | null {
...
function renderGemmaPromptMessages(messages: Array<{
    role: string;
    content: string;
}>): string {
    let out = "";
...
export function buildGemmaBionicPrompt(params: GenerateTextParams): string {
    ...
    if (trimmedPrompt.includes("<start_of_turn>") &&
...
function bionicHostGenerate(socketName: string, request: Record<string, unknown>): Promise<BionicGenerateResponse> {
    ...
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => finish(new Error("[mobile-device-bridge] bionic host timed out")), getBionicRequestTimeoutMs());
        ...
        sock.on("data", (d: Buffer) => {
            ...
            if (expected < 0 && chunks.length >= 4) {
                ...
                if (expected < 0 || expected > BIONIC_MAX_FRAME_BYTES) {
                    finish(new Error(`[mobile-device-bridge] bad bionic frame ${expected}`));
                    return;
                }
            }
            if (expected >= 0 && chunks.length >= 4 + expected) {
                try {
                    finish(null, JSON.parse(chunks.subarray(4, 4 + expected).toString("utf8")));
                }
                catch (e) {
                    finish(new Error(`[mobile-device-bridge] bad bionic JSON: ${(e as Error).message}`));
                }
            }
        });
        sock.on("error", (e: Error) => finish(new Error(`[mobile-device-bridge] bionic socket error: ${e.message}`)));
        sock.on("close", () => {
            if (!settled)
 ...
 */
function bionicHostGenerateStream(socketName: string, request: Record<string, unknown>, onToken: (text: string) => void): Promise<BionicGenerateResponse> {
    const payload = Buffer.from(JSON.stringify(request), "utf8");
    const frame = Buffer.allocUnsafe(4 + payload.length);
    frame.writeUInt32BE(payload.length, 0);
    payload.copy(frame, 4);
 ...
 * total).
 */
function resolveMobileLaneBudget(priority: LocalInferencePriority, prompt: string, maxTokens: number | undefined): {
    prompt: string;
    ...
    const budget = resolveBackgroundInferenceBudget(inferenceRamClassFromEnv() ?? "standard");
    const budgeted = applyBackgroundInferenceBudget({ prompt, maxTokens }, budget);
    return {
...
function makeGenerateHandler(slot: "TEXT_SMALL" | "TEXT_LARGE") {
    return async (_runtime: IAgentRuntime, params: GenerateTextParams) => {
        ...
        if (bionicSock) {
            ...
            const lane = resolveMobileLaneBudget(priority, buildGemmaBionicPrompt(params), params.maxTokens);
            const baseRequest = {
            ...
            const wireRequest = typeof onChunk === "function"
                ? {
                    ...baseRequest,
            ...
            const preparedRequest = createNativeModelRequestGuard({
                ...
                contextWindowTokens: installed?.contextSize ??
            ...
            if (!res.ok) {
                throw new Error(`[mobile-device-bridge] bionic host generate failed: ${res.error ?? "unknown"}`);
            }
            if (typeof res.tokens === "number" &&
                res.tokens >= baseRequest.maxTokens) {
                throw new ElizaError("Bionic local model output reached the decode boundary before a stop condition", {
            ...
            if (typeof res.tokS === "number") {
                logger.info(`[mobile-device-bridge] bionic GPU generate: ${res.tokens ?? "?"} tok @ ${res.tokS.toFixed(1)} tok/s`);
            }
            return res.text ?? "";
        }
        ...
        if (!loadArgs) {
            throw new Error(`[mobile-device-bridge] No local GGUF model installed under ${modelsDir()} and auto-download is disabled (ELIZA_DISABLE_MODEL_AUTO_DOWNLOAD=1). Install a model or unset the disable flag.`);
        }
        ...
        if (messagesForTemplate) {
            try {
                ...
                logger.warn(`[mobile-device-bridge] getFormattedChat failed, falling back to plain-text flatten: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
...
function collectRoleLabeledPromptMessages(prompt: string, system?: string): {
    role: string;
    content: string;
}[] | null {
...
function validateBgeResponse(response: BgeWireResponse, prepared: ReturnType<typeof prepareBgeEmbeddingInput>): number[] {
    const { embedding, tokenIds } = response;
    if (response.embeddingSpace !== BGE_SMALL_VECTOR_SPACE ||
        ...
        throw new ElizaError("Mobile BGE returned invalid vector data or provenance", { code: "EMBEDDING_VECTOR_INVALID" });
    }
    assertBgeTokenAgreement(prepared, tokenIds);
    return identifyEmbeddingVector(embedding, BGE_SMALL_VECTOR_SPACE);
}
function extractEmbeddingText(params: TextEmbeddingParams | string | null): string {
    if (params === null)
        return "";
...
function makeEmbeddingHandler(runtime: AgentRuntime): EmbeddingHandler {
    ...
    return async (_runtime, params) => {
        ...
        if (params === null) {
            ...
            return identifyEmbeddingVector(new Array(dimensions).fill(0), BGE_SMALL_VECTOR_SPACE);
        }
        ...
        try {
            ...
            if (!admitted && readiness === pending)
                readiness = undefined;
        }
        let loadArgs: LocalInferenceLoadArgs | null = resolveLocalLoadArgs("TEXT_EMBEDDING");
        let modelPath = loadArgs?.modelPath ?? null;
        if (!modelPath) {
            if (process.env.ELIZA_DISABLE_MODEL_AUTO_DOWNLOAD?.trim() === "1") {
                throw new Error(`[mobile-device-bridge] No local GGUF embedding model installed under ${modelsDir()} and auto-download is disabled.`);
            }
            modelPath = await downloadRecommendedModelFor("TEXT_EMBEDDING");
            loadArgs = buildRecommendedLoadArgs("TEXT_EMBEDDING", modelPath);
        }
        if (!loadArgs) {
            throw new Error(`[mobile-device-bridge] No local GGUF embedding model resolved for ${modelsDir()}.`);
        }
        ...
        if (bionicSock) {
            ...
            if (byteLength > 1 << 20)
                throw new ElizaError("Prepared embedding exceeds the Android host frame limit", {
...
function getBionicProbeTimeoutMs(): number {
    if (cachedBionicProbeTimeoutMs === undefined) {
        cachedBionicProbeTimeoutMs = resolveTimeoutMs("ELIZA_BIONIC_PROBE_TIMEOUT_MS", 2000);
    }
...
export async function getMobileDeviceBridgeServingStatus(): Promise<MobileDeviceBridgeServingStatus> {
    ...
    const bionicHostServing = registeredModelTrigger === "bionic-host" &&
        socketName !== null &&
        (await probeBionicHostSocket(socketName));
    return { registeredTrigger: registeredModelTrigger, bionicHostServing };
}
export async function loadMobileDeviceBridgeModel(modelPath: string, modelId?: string): Promise<void> {
    await mobileDeviceBridge.loadModel(modelId
...
export class CapacitorMobileDeviceBridgeService extends MobileDeviceBridgeService {
    capabilityDescription = "Relays on-device GPU inference to a paired mobile device over the device bridge.";
    static async start(runtime: IAgentRuntime): Promise<CapacitorMobileDeviceBridgeService> {
        return new CapacitorMobileDeviceBridgeService(runtime);
    }
    getMobileDeviceBridgeStatus(): MobileDeviceBridgeStatus {
        return mobileDeviceBridge.status();
    }
    async loadMobileDeviceBridgeModel(modelPath: string, modelId?: string): Promise<void> {
...
export const mobileDeviceBridgePlugin: Plugin = {
    ...
    description: "Registers the mobile device inference bridge as a runtime service.",
    services: [CapacitorMobileDeviceBridgeService],
};
export async function attachMobileDeviceBridgeToServer(server: HttpServer): Promise<boolean> {
...
function makeBionicImageDescriptionHandler() {
    return async (_runtime: IAgentRuntime, params: string | {
        imageUrl?: string;
        prompt?: string;
    }) => {
        const socketName = bionicSocketName();
        if (!socketName) {
            throw new Error("[mobile-device-bridge] IMAGE_DESCRIPTION requires the bionic host (ELIZA_BIONIC_HOST_DELEGATED=1)");
        }
        const url = typeof params === "string" ? params : params?.imageUrl;
        if (typeof url !== "string" || url.length === 0) {
            throw new Error("[mobile-device-bridge] IMAGE_DESCRIPTION requires a non-empty imageUrl");
        }
        const prompt = typeof params === "object" && params ? params.prompt : undefined;
        ...
        if (!res.ok) {
            throw new Error(`[mobile-device-bridge] bionic image describe failed: ${res.error ?? "unknown error"}`);
        }
        const raw = (res.text ?? "").trim();
        if (!raw) {
            throw new Error("[mobile-device-bridge] bionic image describe returned empty text");
        }
 ...
 */
function registerMobileDeviceBridgeModels(runtime: AgentRuntime, trigger: "bionic-host" | "device-bridge"): boolean {
    ...
    if (typeof runtimeWithRegistration.getModel !== "function" ||
        ...
        logger.error("[mobile-device-bridge] Runtime is missing getModel/registerModel; cannot wire handlers.");
        return false;
    }
    runtimeWithRegistration.registerModel(ModelType.TEXT_SMALL, makeGenerateHandler("TEXT_SMALL"), PROVIDER, LOCAL_INFERENCE_PRIORITY);
    runtimeWithRegistration.registerModel(ModelType.TEXT_LARGE, makeGenerateHandler("TEXT_LARGE"), PROVIDER, LOCAL_INFERENCE_PRIORITY);
    ...
    if (!resolveLocalLoadArgs("TEXT_SMALL") &&
        ...
        downloadRecommendedModelFor("TEXT_SMALL").catch((err) => logger.warn(`[mobile-device-bridge] Background chat-model download failed: ${(err as Error).message}`));
    }
    ...
    runtimeWithRegistration.registerModel(ModelType.TEXT_EMBEDDING, makeEmbeddingHandler(runtime), PROVIDER, LOCAL_INFERENCE_PRIORITY);
    ...
    // registered provider).
    if (bionicSocketName()) {
        runtimeWithRegistration.registerModel(ModelType.IMAGE_DESCRIPTION, makeBionicImageDescriptionHandler(), PROVIDER, LOCAL_INFERENCE_PRIORITY);
        logger.info("[mobile-device-bridge] Registered bionic IMAGE_DESCRIPTION handler (op=image)");
    }
    ...
    if (!embeddingModelPath &&
        ...
        downloadRecommendedModelFor("TEXT_EMBEDDING").catch((err) => logger.warn(`[mobile-device-bridge] Background embedding-model download failed: ${(err as Error).message}`));
    }
    logger.info(`[mobile-device-bridge] Registered ${PROVIDER} handlers for TEXT_SMALL / TEXT_LARGE${embeddingModelPath ? " / TEXT_EMBEDDING" : ""} at priority ${LOCAL_INFERENCE_PRIORITY} (via ${trigger})`);
    registeredRuntimes.add(runtime);
    registeredRuntimeCount += 1;
    registeredModelTrigger = trigger;
    return true;
}
export async function ensureMobileDeviceBridgeInferenceHandlers(runtime: AgentRuntime): Promise<boolean> {
    ...
    logger.warn("[mobile-device-bridge] No bionic host delegation and no device bridge attached — " +
    ...
    if (!deviceAttachUnsubscribers.has(runtime)) {
        ...
        deviceAttachUnsubscribers.set(runtime, mobileDeviceBridge.onDeviceAttached(registerOnAttach));
