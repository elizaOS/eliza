...
import { dispatchBufferedRequest } from "./dispatch.ts";
import { dispatchStreamingRequest } from "./dispatch.ts";
import { expect } from "vitest";
import { it } from "vitest";
import { stringToUuid as sqliteTestAgentId } from "@elizaos/core";
import { type AndroidCoreRouteDeps } from "./dispatch.ts";
import { type AndroidDispatchRoute } from "./dispatch.ts";
import { type AndroidElizaConfigLike } from "./dispatch.ts";
...
async function createNotificationRuntime(services: NonNullable<Plugin["services"]> = [], adapter: SQLiteDatabaseAdapter = SQLiteDatabaseAdapter.create(":memory:", sqliteTestAgentId("AndroidNotificationDispatchTest"))): Promise<{
...
function coreDeps(overrides: Partial<AndroidCoreRouteDeps> = {}): {
    ...
    const deps: AndroidCoreRouteDeps = {
        ...
        hasPersistedFirstRunState: (config) => (config as AndroidElizaConfigLike).meta?.firstRunComplete === true,
        ...overrides,
    };
    return { deps, saved };
}
describe("dispatchBufferedRequest", () => {
    it("runs authenticated Android internal wakes through TaskService", async () => {
        ...
        const wakeRuntime = {
            getService: (type: ServiceType) => type === ServiceType.TASK ? { runDueTasks } : null,
    ...
    it("returns a typed unavailable boundary and resets after TaskService failure", async () => {
        ...
        try {
            const unavailable = await dispatchBufferedRequest({ getService: () => null } as unknown as IAgentRuntime, route, payload);
            ...
            const recovered = await dispatchBufferedRequest(wakeRuntime, route, payload);
    ...
    it("coalesces concurrent wakes per runtime without skipping a replacement runtime", async () => {
        ...
        try {
            ...
            const replacement = await dispatchBufferedRequest(replacementRuntime, route, payload);
    ...
    it("persists first-run completion and fails closed on write errors", async () => {
        ...
        const ok = await dispatchBufferedRequest(runtime, route, {
            method: "POST",
            path: "/api/first-run",
        }, deps);
        ...
        const failed = await dispatchBufferedRequest(runtime, route, {
            method: "POST",
            path: "/api/first-run",
        }, failing);
        expect(failed.status).toBe(500);
    ...
    it("preserves real kernel health failures instead of reporting synthetic readiness", async () => {
        ...
        const response = await dispatchBufferedRequest(runtime, route, { method: "GET", path: "/api/health" }, deps);
    ...
    it("returns the loopback-shaped envelope for a JSON route", async () => {
        ...
        expect(Buffer.from(res.bodyBase64, "base64").toString("utf8")).toBe(res.body);
    ...
    it("rejects an absolute or unsafe path", async () => {
        ...
        await expect(dispatchBufferedRequest(runtime, route, {
            method: "GET",
            path: "http://evil/api",
    ...
    it("serves the /api/notifications inbox from the runtime service over the UDS (#13550)", async () => {
        ...
        const notifierRuntime = {
            getService: (type: string) => type === "notification"
                ? {
                    list: () => seeded,
                    getUnreadCount: () => 1,
                    markRead: (id: string) => {
        ...
        expect((calls[0] as {
            path?: string;
        }).path).toBe("/api/notifications/push-tokens");
        void push;
    });
    ...
    it("returns typed retryable 503 while the notification service is registering", async () => {
        ...
        const loading = registering.runtime.getServiceLoadPromise(ServiceType.NOTIFICATION);
        try {
            expect(registering.runtime.getServiceRegistrationStatus(ServiceType.NOTIFICATION)).toBe("registering");
            const { route } = fixedRoute(null);
    ...
    it("recovers a failed service once and then serves its persisted rows", async () => {
        ...
        const adapter = TransientCacheAdapter.create(":memory:", sqliteTestAgentId("AndroidNotificationDispatchTest"));
        const failedRuntime = await createNotificationRuntime([NotificationService], adapter);
        try {
            await expect(failedRuntime.runtime.getServiceLoadPromise(ServiceType.NOTIFICATION)).rejects.toThrow("failed to start");
            await failedRuntime.runtime.setCache(`notifications:${failedRuntime.runtime.agentId}`, [
                {
            ...
            const { route } = fixedRoute(null);
            const failed = await dispatchBufferedRequest(failedRuntime.runtime, route, {
                method: "GET",
                path: "/api/notifications",
            });
            expect(failed.status).toBe(503);
            expect(JSON.parse(failed.body).code).toBe("NOTIFICATION_SERVICE_FAILED");
            await failedRuntime.runtime.getServiceLoadPromise(ServiceType.NOTIFICATION);
            const available = await dispatchBufferedRequest(failedRuntime.runtime, route, { method: "GET", path: "/api/notifications" });
...
describe("dispatchStreamingRequest", () => {
    it("streams Android direct startup route responses without dispatchRoute", async () => {
        ...
        await dispatchStreamingRequest(runtime, route, { method: "GET", path: "/api/first-run/status" }, sink, deps);
        expect(events[0]).toMatchObject({ kind: "response", status: 200 });
        const body = JSON.parse(Buffer.from(events[1]?.dataBase64 as string, "base64").toString("utf8"));
    ...
    it("emits response head then base64 chunks for a return-shape stream", async () => {
        ...
        const { sink, events } = collectSink();
        await dispatchStreamingRequest(runtime, route, { method: "POST", path: "/api/conversations/c/messages/stream" }, sink);
        expect(events[0]).toMatchObject({ kind: "response", status: 200 });
        expect(events.slice(1).map((e) => e.kind)).toEqual(["chunk", "chunk"]);
        expect(Buffer.from(events[1]?.dataBase64 as string, "base64").toString("utf8")).toBe("data: hello\n\n");
    });
    it("forwards a legacy SSE handler's res.write fragments live via onChunk", async () => {
        ...
        expect(Buffer.from(chunks[0]?.dataBase64 as string, "base64").toString("utf8")).toBe("data: a\n\n");
    });
    it("streams a non-streaming buffered result as a single chunk", async () => {
        ...
        const { sink, events } = collectSink();
        await dispatchStreamingRequest(runtime, route, { method: "POST", path: "/api/agents/x/message" }, sink);
        expect(events[0]?.kind).toBe("response");
        const chunks = events.filter((e) => e.kind === "chunk");
        expect(chunks).toHaveLength(1);
        expect(JSON.parse(Buffer.from(chunks[0]?.dataBase64 as string, "base64").toString("utf8"))).toEqual({ done: true });
    });
    it("emits a 404 head + body when no route matches a stream", async () => {
        const { route } = fixedRoute(null);
        const { sink, events } = collectSink();
        await dispatchStreamingRequest(runtime, route, { method: "POST", path: "/api/nope/stream" }, sink);
