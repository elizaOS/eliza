...
import { cleanIosNativeConversationReply } from "./native-generation.ts";
import { closeDownloadWriter } from "../shared/download-writer.ts";
...
import { createWriteStream } from "../shared/fs-proxy.ts";
import { dispatchIosNativeGeneration } from "./native-generation.ts";
import { existsSync } from "../shared/fs-proxy.ts";
import { fileURLToPath } from "node:url";
import { getBootConfig } from "@elizaos/core/config/boot-config-store";
import { installMobileFsShim } from "../shared/fs-shim.ts";
import { mkdirSync } from "../shared/fs-proxy.ts";
import { parseCanonicalInteger } from "@elizaos/core/utils/number-parsing";
import { readAliasedEnv } from "@elizaos/core/utils/env";
import { readFileSync } from "../shared/fs-proxy.ts";
import { readdirSync } from "../shared/fs-proxy.ts";
import { renameSync } from "../shared/fs-proxy.ts";
import { resolveStoredModelPath } from "../shared/local-inference-stored-path.ts";
import { rmSync } from "../shared/fs-proxy.ts";
import { runModelGrind } from "./model-grind.ts";
import { setBootConfig } from "@elizaos/core/config/boot-config-store";
import { statSync } from "../shared/fs-proxy.ts";
import { stringToUuid } from "@elizaos/core";
import { stripReasoningBlocks } from "./native-generation.ts";
import { summarizeTranscript } from "@elizaos/core/transcripts";
import { teardownFailedDownload } from "../shared/download-writer.ts";
import { toStoredModelPath } from "../shared/local-inference-stored-path.ts";
...
import { type StdioBridgeRequestFrame as BridgeRequest } from "../shared/stdio-bridge.ts";
import { type StdioBridgeResponseFrame as BridgeResponse } from "../shared/stdio-bridge.ts";
...
import { writeDownloadChunk } from "../shared/download-writer.ts";
import { writeFileSync } from "../shared/fs-proxy.ts";
...
interface RuntimeMessageService {
    handleMessage: (runtime: IAgentRuntime, message: ReturnType<typeof createMessageMemory>, onResponse: (content: {
        text?: string;
    } | null | undefined) => Promise<unknown[]> | unknown[]) => Promise<unknown> | unknown;
}
...
type RuntimeWithModelRegistration = IAgentRuntime & {
    registerModel?: (modelType: string | number, handler: GenerateTextHandler, provider: string, priority?: number) => void;
};
...
const IOS_NATIVE_NO_THINK_SYSTEM = "Answer with final user-visible content only. Do not include private reasoning, analysis tags, or <think> blocks.";
...
function setProcessEnv(key: string, value: string | null | undefined, overwrite = true): void {
...
function hydrateIosEnvFromArgv(argv: string[] = process.argv): HydratedIosArgvEnv {
            ...
            if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
                for (const [key, value] of Object.entries(parsed)) {
                    if (typeof value === "string")
                        setProcessEnv(key, value, true);
                }
            }
        }
        catch (error) {
            console.error("[ios-bridge] failed to parse argv env envelope:", error instanceof Error ? error.message : String(error));
        }
    }
    const appSupportDir = argvValue(argv, "--eliza-ios-app-support-dir") ||
        process.env.ELIZA_IOS_APP_SUPPORT_DIR ||
        process.env.ELIZA_HOME ||
        null;
    const bundlePath = argvValue(argv, "--eliza-ios-agent-bundle") ||
    ...
    if (appSupportDir) {
        ...
        setProcessEnv("ELIZA_WORKSPACE_DIR", path.join(appSupportDir, "workspace"), true);
        setProcessEnv("PGLITE_DATA_DIR", path.join(appSupportDir, ".elizadb"), true);
    }
    if (bundlePath) {
...
function installIosBackendCrashGuards(): void {
    ...
    if (slot.__elizaIosCrashGuardsInstalled)
        return;
    slot.__elizaIosCrashGuardsInstalled = true;
    const format = (value: unknown): string => value instanceof Error ? (value.stack ?? value.message) : String(value);
    process.on("unhandledRejection", (reason) => {
        console.error("[ios-bridge] Unhandled promise rejection (non-fatal):", format(reason));
    });
    process.on("uncaughtException", (error) => {
        console.error("[ios-bridge] Uncaught exception (agent kept alive):", format(error));
    });
}
/**
 ...
 */
async function bootRuntimeWithRetry(boot: () => Promise<IAgentRuntime>): Promise<IAgentRuntime> {
    const maxAttempts = 3;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
            ...
            console.error(`[ios-bridge] runtime boot attempt ${attempt}/${maxAttempts} failed; retrying in ${backoffMs}ms:`, error instanceof Error ? error.message : String(error));
...
async function startIosBridgeBackend(): Promise<IosBridgeBackend> {
    ...
    const mobileWorkspaceRoot = process.env.MOBILE_WORKSPACE_ROOT ||
...
function maybeAutoRunModelGrind(): void {
    if (process.env.ELIZA_IOS_RUN_MODEL_GRIND !== "1")
        return;
    void (async () => {
        ...
        try {
            ...
            if (supportDir) {
                try {
                    writeFileSync(path.join(supportDir, "model-grind-report.json"), `${JSON.stringify(report, null, 2)}\n`);
                }
                catch (error) {
                    console.error("[model-grind] report write failed:", error instanceof Error ? error.message : error);
                }
            }
        }
        catch (error) {
            console.error("[model-grind] grind failed:", error instanceof Error ? error.message : error);
        }
    })();
}
...
function ensureIosBridgeBackendStarted(host: IosBridgeHost): Promise<IosBridgeBackend> {
    if (host.backendPromise)
        return host.backendPromise;
    host.backendPromise = startIosBridgeBackend().then((backend) => {
...
async function awaitIosBridgeBackend(host: IosBridgeHost, timeoutMs: number | undefined, label: string): Promise<IosBridgeBackend> {
    if (host.backend)
        return host.backend;
    if (host.bootError) {
    ...
    const result = await timeoutAfter(ensureIosBridgeBackendStarted(host), timeoutMs, `${label} backend startup`);
...
function bridgeStatus(values: Partial<Omit<BridgeStatusResult, "engine" | "transport" | "bridgeVersion">> = {}): BridgeStatusResult {
    return {
...
function timeoutAfter<T>(promise: Promise<T>, timeoutMs: number | undefined, label: string): Promise<T | {
    __timeout: true;
    timeoutMs: number;
    label: string;
}> {
...
async function fetchBackend(backend: IosBridgeBackend, payload: HttpRequestPayload): Promise<{
    status: number;
    statusText: string;
    headers: Record<string, string>;
    body: string;
    ...
    if (!rawPath || !isSafeLocalPath(rawPath)) {
        throw new Error("iOS bridge http_request requires a path that starts with / and is not an absolute URL");
    }
    ...
    const direct = await timeoutAfter(handleDirectCoreRoute(backend, method, rawPath, payload), timeoutMs, `${method} ${pathname}`);
    ...
    const result = await timeoutAfter(backend.dispatchRoute({
    ...
    if (result) {
        ...
        if (result.body == null) {
            ...
            if (!Object.keys(responseHeaders).some((k) => k.toLowerCase() === "content-type")) {
...
const CONVERSATION_STREAM_PATH = /^\/api\/conversations\/([^/]+)\/messages\/stream$/;
/**
 ...
 */
export async function fetchBackendStream(backend: IosBridgeBackend, payload: HttpStreamRequestPayload, streamId: string, emit: StreamEmitter): Promise<{
    streamId: string;
    done: true;
}> {
    const rawPath = typeof payload.path === "string" ? payload.path.trim() : "";
    if (!rawPath || !isSafeLocalPath(rawPath)) {
        throw new Error("iOS bridge http_request_stream requires a path that starts with / and is not an absolute URL");
    }
    const method = normalizeMethod(payload.method);
    ...
    await streamConversationMessageResponse(backend, conversationId, body, streamId, emit);
    return { streamId, done: true };
}
...
function bytesResponse(status: number, bytes: Uint8Array, headers: Record<string, string>): BufferedHttpResponse {
...
function queryParam(query: Record<string, string | string[]>, key: string): string | null {
    const raw = query[key];
...
function resolveMemoryTableFilter(typeParam: string | null): readonly string[] | undefined {
    if (!typeParam)
        return undefined;
...
async function fetchMemoriesFromTables(runtime: IAgentRuntime, params: {
    entityIds?: UUID[];
    roomId?: UUID;
    ...
    const perTableMemories = await Promise.all(tables.map(async (tableName) => {
        ...
        for (;;) {
            ...
            for (const memory of memories) {
                ...
                if (searchQuery &&
                    !matchesMemoryKeyword((tagged.content as {
                        text?: string;
...
async function handleMemoriesFeedRoute(runtime: IAgentRuntime, query: Record<string, string | string[]>): Promise<BufferedHttpResponse> {
    const requestedLimit = parsePositiveInteger(queryParam(query, "limit"), MEMORY_FEED_DEFAULT_LIMIT);
    ...
    const beforeId = beforeIdParam === null ? undefined : validateUuid(beforeIdParam);
...
async function handleMemoriesBrowseRoute(runtime: IAgentRuntime, query: Record<string, string | string[]>): Promise<BufferedHttpResponse> {
    const requestedLimit = parsePositiveInteger(queryParam(query, "limit"), MEMORY_BROWSE_DEFAULT_LIMIT);
...
async function handleMemoriesStatsRoute(runtime: IAgentRuntime): Promise<BufferedHttpResponse> {
    ...
    // is meant to remove.
    const entries = await Promise.all(MEMORY_TABLE_NAMES.map(async (tableName) => {
 ...
 */
function transcriptFromOwnedMemory(runtime: IAgentRuntime, row: Memory | null, id: UUID): Transcript | null {
    if (!row || row.agentId !== runtime.agentId)
        return null;
    if (row.metadata?.type !== "custom" ||
...
function buildTranscriptFromRequest(body: CreateTranscriptRequestBody, id: string, now: number): Transcript {
    const segments = Array.isArray(body.segments) ? body.segments : [];
    const createdAt = body.createdAt ?? now;
    return {
        id,
        title: body.title?.trim() || `Recording ${new Date(createdAt).toLocaleString()}`,
        createdAt,
        endedAt: now,
...
async function listTranscripts(runtime: IAgentRuntime, roomId?: UUID, limit = 100): Promise<TranscriptSummary[]> {
...
async function getTranscript(runtime: IAgentRuntime, id: UUID): Promise<Transcript | null> {
    const row = await runtime.getMemoryById(id);
    return transcriptFromOwnedMemory(runtime, row, id);
}
async function persistTranscript(runtime: IAgentRuntime, roomId: UUID, entityId: UUID, transcript: Transcript): Promise<Transcript> {
    const memory: Memory = {
...
async function handleTranscriptsRoute(runtime: IAgentRuntime, method: string, pathname: string, query: Record<string, string | string[]>, body: Record<string, unknown>): Promise<BufferedHttpResponse | null> {
    if (method === "GET" && pathname === "/api/transcripts") {
        const roomId = queryParam(query, "roomId") ?? undefined;
        const transcripts = await listTranscripts(runtime, roomId as UUID | undefined);
    ...
    if (method === "POST" && pathname === "/api/transcripts") {
        ...
        const transcript = buildTranscriptFromRequest(create, crypto.randomUUID(), Date.now());
        const saved = await persistTranscript(runtime, (create.roomId ?? agentId) as UUID, (create.entityId ?? agentId) as UUID, transcript);
...
function handleBrowserWorkspaceRoute(method: string, pathname: string, body: Record<string, unknown>): BufferedHttpResponse | null {
...
function _buildBufferedRoutePair(args: {
    ...
    const readable = Readable.from(args.bodyText ? [Buffer.from(args.bodyText, "utf8")] : []);
...
function createIosConversation(backend: IosBridgeBackend, input: Record<string, unknown> = {}): IosConversation {
...
function installHostCallProtocol(write: (frame: BridgeOutboundFrame) => void): void {
    hostProtocolWrite = write;
}
function tryHandleHostResultLine(line: string): boolean {
    if (!line.includes('"host_result"'))
        return false;
    ...
    const envelope = parsed.envelope && typeof parsed.envelope === "object"
        ? (parsed.envelope as Record<string, unknown>)
        : { ok: parsed.ok, result: parsed.result, error: parsed.error };
    if (envelope.ok === false) {
        pending.reject(new Error(typeof envelope.error === "string"
            ? envelope.error
...
function callIosHost(method: string, payload: unknown, timeoutMs = 120000): Promise<unknown> {
    const writeHostMessage = hostProtocolWrite;
    if (!writeHostMessage) {
        return Promise.reject(new Error("iOS native host-call protocol is not installed"));
    }
    ...
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
            ...
            reject(new Error(`Native iOS host call ${method} timed out after ${boundedTimeout}ms`));
        }, boundedTimeout);
...
function writeJsonObjectFile(filePath: string, value: Record<string, unknown>): void {
...
function iosNativeCatalogByFile(filePath: string): NativeCatalogModelEntry | null {
...
function nativeCatalogModelPayload(model: NativeCatalogModelEntry): Record<string, unknown> {
    return {
        id: model.id,
...
function installedModelForCatalogEntry(model: NativeCatalogModelEntry, filePath: string, sizeBytes: number, installedAt: string, source: string): InstalledModelEntry {
    return {
        id: model.id,
        displayName: model.displayName,
        path: filePath,
...
function serializeInstalledModelEntry(model: InstalledModelEntry): InstalledModelEntry | null {
...
function upsertInstalledModel(model: InstalledModelEntry): void {
    ...
    if (!storedModel) {
        throw new Error(`iOS local-inference model path must live under ${localInferenceRootPath()}: ${model.path}`);
    }
    const existing = readInstalledModels().filter((entry) => entry.id !== model.id &&
    ...
    const visit = (dir: string, depth: number): void => {
        ...
        for (const entry of entries) {
            ...
            if (stats.isDirectory()) {
                ...
                if (catalogModel) {
                    models.push(installedModelForCatalogEntry(catalogModel, fullPath, stats.size, new Date(stats.mtimeMs).toISOString(), "ios-bundled"));
                }
                else {
...
function normalizeInstalledModelPath(rawPath: string): string | null {
    ...
    return resolveStoredModelPath(rawPath, localInferenceRootPath(), (p) => existsSync(p));
}
function readInstalledModels(): InstalledModelEntry[] {
    ...
    const fromRegistry = rawModels
        ...
        return {
            ...
            installedAt: typeof record.installedAt === "string"
                ? record.installedAt
            ...
            bundleVerifiedAt: typeof record.bundleVerifiedAt === "string"
                ? record.bundleVerifiedAt
                : undefined,
            dimensions: positiveInteger(record.dimensions) ?? undefined,
            embeddingDimension: positiveInteger(record.embeddingDimension) ?? undefined,
            embeddingDimensions: positiveInteger(record.embeddingDimensions) ?? undefined,
        };
    })
        .filter((entry): entry is InstalledModelEntry => Boolean(entry));
...
function resolveAssignedModel(slot: string): InstalledModelEntry | null {
    const installed = readInstalledModels().filter((model) => !isEmbeddingModel(model));
    ...
    if (nativeLlamaState.modelPath) {
        const current = installed.find((entry) => entry.path === nativeLlamaState.modelPath);
        if (current)
            return current;
    }
    return (installed.sort((left, right) => {
        ...
        const rightBytes = typeof right.sizeBytes === "number" && Number.isFinite(right.sizeBytes)
            ? right.sizeBytes
            : 0;
        const leftBytes = typeof left.sizeBytes === "number" && Number.isFinite(left.sizeBytes)
            ? left.sizeBytes
            : 0;
...
function nativeLlamaContextSize(): number {
    return (positiveInteger(process.env.ELIZA_IOS_LLAMA_CONTEXT_SIZE) ??
 ...
 */
async function assertModelFitsDeviceMemory(model: InstalledModelEntry): Promise<void> {
    if (!model.sizeBytes || model.sizeBytes <= 0)
        return;
    const hardware = await nativeHardwareInfo();
    if (hardware.is_simulator === true)
        return;
    ...
    if (requiredBytes > availableBytes) {
        ...
        throw new Error(`[ios-native-llama] insufficient memory to load ${model.id}: needs ~${requiredMb} MB but only ~${availableMb} MB is available. Close other apps or install a smaller model.`);
    }
}
async function loadNativeLlamaModel(model: InstalledModelEntry, useGpu: boolean): Promise<Record<string, unknown>> {
    const result = await callIosHost("llama_load_model", {
        path: model.path,
        modelId: model.id,
...
async function ensureNativeModelLoaded(slot: string): Promise<NativeLlamaState> {
    const model = resolveAssignedModel(slot);
    if (!model) {
        throw new Error(`[ios-native-llama] No local GGUF model is installed under ${path.join(localInferenceRootPath(), "models")}. Download or install a model before using local generation.`);
    }
    if (nativeLlamaState.contextId != null &&
    ...
    try {
        ...
        try {
            ...
            if (!requestedGpu || !isMetalLoadError(error))
                throw error;
            requestedGpu = false;
...
async function unloadNativeLlamaModel(): Promise<void> {
    ...
    if (contextId != null) {
        ...
        await callIosHost("llama_free", { context_id: contextId }, 30000).catch(() => undefined);
    }
...
function collectChatMlPromptMessages(prompt: string, system?: string): Array<{
    role: string;
    content: string;
}> | null {
...
function renderGemmaPrompt(messages: Array<{
    role: string;
    content: string;
}>): string {
    const blocks: string[] = [];
    for (const message of messages) {
        ...
        if (content) {
            blocks.push(`<start_of_turn>${roleForGemmaPrompt(message.role)}\n${content}<end_of_turn>`);
        }
    }
    blocks.push("<start_of_turn>model\n");
    return blocks.join("\n");
}
async function maybeGenerateIosNativeConversationReply(runtime: IAgentRuntime, prompt: string, onToken?: StreamChunkCallback): Promise<Record<string, unknown> | null> {
    const installed = readInstalledModels().filter((model) => !isEmbeddingModel(model));
...
function isStructuredGenerationSlot(slot: string): boolean {
    return (slot === ModelType.RESPONSE_HANDLER ||
...
function mergeStopSequences(values: unknown): string[] {
    const requested = Array.isArray(values)
        ...
        : [];
    return Array.from(new Set([...requested, "<end_of_turn>", "<start_of_turn>", "<endoftext>"]));
}
function makeIosNativeGenerateHandler(slot: string): GenerateTextHandler {
    return async (_runtime, params) => {
        ...
        if (state.contextId == null) {
            throw new Error("[ios-native-llama] model load did not produce a context");
        }
        ...
        const text = await dispatchIosNativeGeneration({
            ...
            invoke: (request, timeoutMs) => callIosHost("llama_generate", request, timeoutMs),
        });
...
function installKeepAwakeBridge(): void {
    ...
    if (typeof g.__ELIZA_BRIDGE__.keep_awake_set === "function")
        return;
    g.__ELIZA_BRIDGE__.keep_awake_set = (enabled: unknown): boolean => {
        ...
        void callIosHost("keep_awake_set", { enabled: Boolean(enabled) }).catch((error) => {
            console.error("[ios-bridge] keep_awake_set failed:", error instanceof Error ? error.message : String(error));
        });
        return true;
    };
}
...
function installBackgroundDownloadBridge(): void {
    ...
    g.__ELIZA_BRIDGE__ = g.__ELIZA_BRIDGE__ ?? {};
    if (typeof g.__ELIZA_BRIDGE__.bg_download_start === "function")
        return;
    g.__ELIZA_BRIDGE__.bg_download_start = (args: unknown): Promise<unknown> => callIosHost("bg_download_start", args, 60000);
    g.__ELIZA_BRIDGE__.bg_download_status = (args: unknown): Promise<unknown> => callIosHost("bg_download_status", args, 60000);
    g.__ELIZA_BRIDGE__.bg_download_cancel = (args: unknown): Promise<unknown> => callIosHost("bg_download_cancel", args, 60000);
}
function installIosNativeLlamaHandlers(runtime: IAgentRuntime): void {
    ...
    if (flagged.__iosNativeLlamaHandlersInstalled)
        return;
    const runtimeWithRegistration = runtime as RuntimeWithModelRegistration;
    if (typeof runtimeWithRegistration.registerModel !== "function")
        return;
    for (const modelType of TEXT_GENERATION_MODEL_TYPES) {
        runtimeWithRegistration.registerModel(modelType, makeIosNativeGenerateHandler(modelType), IOS_NATIVE_LLAMA_PROVIDER, IOS_NATIVE_LLAMA_PRIORITY);
    }
...
async function nativeHardwareInfo(): Promise<Record<string, unknown>> {
    try {
        ...
        console.error("[ios-bridge] hardware info unavailable:", error instanceof Error ? error.message : String(error));
        return {
            backend: "unknown",
            total_ram_gb: 0,
            available_ram_gb: 0,
...
async function nativeLocalInferenceProviders(): Promise<Record<string, unknown>> {
...
function nativeDownloadJobs(): NativeDownloadJob[] {
    ...
    for (const model of readInstalledModels().filter((entry) => !isEmbeddingModel(entry))) {
        if (trackedModelIds.has(model.id))
            continue;
        jobs.push(nativeDownloadJobForInstalledModel(model));
    }
    return jobs;
}
function nativeDownloadStatus(model: InstalledModelEntry | null): Record<string, unknown> {
...
function nativeDownloadJobForInstalledModel(model: InstalledModelEntry): NativeDownloadJob {
...
function updateNativeDownloadJob(modelId: string, patch: Partial<Omit<NativeDownloadJob, "jobId" | "modelId" | "startedAt">>): NativeDownloadJob {
...
async function runNativeModelDownload(model: NativeCatalogModelEntry): Promise<void> {
    ...
    const partialPath = path.join(localInferenceDownloadsPath(), `${model.id}.part`);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to download ${model.id}: HTTP ${response.status} ${response.statusText}`);
        }
        if (!response.body) {
        ...
        while (true) {
            ...
            updateNativeDownloadJob(model.id, {
                received,
                bytesPerSec,
                etaMs: bytesPerSec > 0 ? Math.round((remaining / bytesPerSec) * 1000) : null,
            });
        }
        ...
        upsertInstalledModel(installedModelForCatalogEntry(model, targetPath, stats.size, installedAt, "eliza-download"));
        updateNativeDownloadJob(model.id, {
            state: "completed",
            received: stats.size,
            total: stats.size,
...
function startNativeModelDownload(modelId: string): NativeDownloadJob {
    ...
    if (!model)
        throw new Error(`Unsupported iOS local model: ${modelId}`);
...
function nativeTextReadiness(): Record<string, unknown> {
    ...
    for (const slot of ["TEXT_SMALL", "TEXT_LARGE"]) {
        ...
        const model = assignedModelId == null
            ? null
...
function hasNativeVoiceBundle(bundleDir: string): boolean {
    ...
    try {
        ...
        if (statSync(coreml).isDirectory() && statSync(voice).isFile())
            return true;
    }
    catch {
    ...
    try {
        for (const entry of readdirSync(ttsDir)) {
            if (/^omnivoice-base-.*\.gguf$/i.test(entry))
                return true; // OmniVoice (tier default)
            if (entry === "kokoro") {
                try {
                    for (const k of readdirSync(path.join(ttsDir, "kokoro"))) {
                        if (/\.gguf$/i.test(k) || k === "model_q4.onnx")
                            return true;
                    }
                }
                catch {
...
async function synthesizeNativeIosLocalTts(request: NativeLocalTtsRequest): Promise<Uint8Array> {
...
async function handleNativeIosLocalTtsRoute(method: string, rawPath: string, payload: HttpRequestPayload): Promise<BufferedHttpResponse | null> {
    const { pathname } = splitPathAndQuery(rawPath);
    ...
    if (voiceReadiness.status !== "ready" &&
...
async function transcribeNativeIosLocalAsr(request: NativeLocalAsrRequest): Promise<string> {
    const bundleDir = nativeVoiceBundleDir();
    if (!bundleDir) {
        throw new Error("No Eliza-1 voice bundle is installed");
    }
...
async function handleNativeIosLocalAsrRoute(method: string, rawPath: string, payload: HttpRequestPayload): Promise<BufferedHttpResponse | null> {
    const { pathname } = splitPathAndQuery(rawPath);
...
async function handleNativeIosLocalInferenceRoute(method: string, rawPath: string, payload: HttpRequestPayload): Promise<BufferedHttpResponse | null> {
    const { pathname } = splitPathAndQuery(rawPath);
    ...
    if (method === "POST" && pathname === "/api/local-inference/assignments") {
        ...
        if (modelId) {
            const installed = readInstalledModels();
            const known = installed.some((model) => model.id === modelId) ||
    ...
    const verifyMatch = pathname.match(/^\/api\/local-inference\/installed\/([^/]+)\/verify$/);
...
async function handleBufferedLocalInferenceRoute(method: string, rawPath: string, payload: HttpRequestPayload): Promise<BufferedHttpResponse | null> {
    const { pathname } = splitPathAndQuery(rawPath);
    ...
    const native = await handleNativeIosLocalInferenceRoute(method, rawPath, payload);
    if (native)
        return native;
    return null;
}
async function ensureConversationConnection(backend: IosBridgeBackend, conversation: IosConversation): Promise<UUID> {
...
async function handleDirectConversationMessage(backend: IosBridgeBackend, conversation: IosConversation, input: Record<string, unknown>, onToken?: (token: string, accumulated: string) => void): Promise<Record<string, unknown>> {
    ...
    const runtime = backend.runtime as IAgentRuntime & {
        createMemory?: (memory: ReturnType<typeof createMessageMemory>, tableName: string) => Promise<void> | void;
        messageService?: RuntimeMessageService;
    };
    const userId = await ensureConversationConnection(backend, conversation);
    const channelType = typeof input.channelType === "string" &&
    ...
    try {
        ...
        console.error("[ios-bridge] createMemory(messages) failed:", error instanceof Error ? error.message : String(error));
    }
    ...
    const emitToken = onToken
        ...
        : undefined;
    const nativeReply = await maybeGenerateIosNativeConversationReply(backend.runtime, prompt, emitToken ? (chunk) => emitToken(chunk) : undefined).catch((error) => ({
    ...
    try {
        ...
        chunks.push(err instanceof Error
...
function cachedConversationMessageResult(conversation: IosConversation, input: Record<string, unknown>): Record<string, unknown> | null {
    const prompt = typeof input.text === "string"
        ? input.text
        : typeof input.message === "string"
            ? input.message
            : typeof input.prompt === "string"
                ? input.prompt
                : "";
    if (!conversation.lastAssistantText ||
...
function bufferedConversationStreamResponse(result: Record<string, unknown>): BufferedHttpResponse {
    const text = typeof result.text === "string" ? result.text : "";
    const agentName = typeof result.agentName === "string" && result.agentName.trim()
        ? result.agentName
        : "Eliza";
    const body = [
 ...
 */
export async function streamConversationMessageResponse(backend: IosBridgeBackend, conversationId: string, body: Record<string, unknown>, streamId: string, emit: StreamEmitter): Promise<void> {
    ...
    if (!conversation) {
        ...
        await emit({
            streamId,
            kind: "chunk",
            dataBase64: Buffer.from(JSON.stringify({ error: "Conversation not found" }), "utf8").toString("base64"),
        });
        await emit({ streamId, kind: "complete", error: null });
        return;
    }
    ...
    const enqueueChunk = (payload: Record<string, unknown>): void => {
        ...
        emitTail = emitTail.then(() => emitSafely({ streamId, kind: "chunk", dataBase64 }));
    };
    ...
    try {
        ...
        if (cached) {
            ...
            result = await handleDirectConversationMessage(backend, conversation, body, (token, accumulated) => {
                streamedAny = true;
...
export async function handleDirectCoreRoute(backend: IosBridgeBackend, method: string, rawPath: string, payload: HttpRequestPayload): Promise<BufferedHttpResponse | null> {
    const { pathname } = splitPathAndQuery(rawPath);
    if (method === "GET" && pathname === "/api/health") {
        return jsonResponse(200, {
            ...
            database: "ok",
            plugins: {
                loaded: Array.isArray((backend.runtime as {
                    plugins?: unknown;
                }).plugins)
                    ? ((backend.runtime as {
                        plugins?: unknown[];
    ...
    if (localAsr)
        return localAsr;
    const localInference = await handleBufferedLocalInferenceRoute(method, rawPath, payload);
    if (localInference)
    ...
    if (method === "POST" && pathname === "/api/conversations") {
        const conversation = createIosConversation(backend, parseRequestBody(payload));
        return jsonResponse(200, { conversation });
    }
    const messageMatch = pathname.match(/^\/api\/conversations\/([^/]+)\/messages$/);
    const messageStreamMatch = pathname.match(/^\/api\/conversations\/([^/]+)\/messages\/stream$/);
    ...
    if (method === "POST" && messageStreamMatch) {
        ...
        const body = parseRequestBody(payload);
        const result = cachedConversationMessageResult(conversation, body) ??
    ...
    if (method === "POST" && messageMatch) {
        ...
        const result = await handleDirectConversationMessage(backend, conversation, parseRequestBody(payload));
    ...
    if (pathname === "/api/transcripts" ||
        ...
        const { query } = splitPathAndQuery(rawPath);
        const transcripts = await handleTranscriptsRoute(backend.runtime, method, pathname, query, parseRequestBody(payload));
        if (transcripts)
            return transcripts;
    }
    // ── Browser workspace (the app is the browser on iOS) ──────────────────
    if (pathname.startsWith("/api/browser-workspace")) {
        const browser = handleBrowserWorkspaceRoute(method, pathname, parseRequestBody(payload));
        if (browser)
            return browser;
    }
    return null;
}
async function sendMessage(backend: IosBridgeBackend, payload: unknown): Promise<Record<string, unknown>> {
    const input = payload && typeof payload === "object" && !Array.isArray(payload)
        ? (payload as Record<string, unknown>)
        : {};
    const message = typeof input.message === "string" ? input.message : "";
    if (!message.trim())
        throw new Error("send_message requires message");
    ...
    const result = await timeoutAfter(handleDirectConversationMessage(backend, conversation, {
        text: message,
        ...
        ...(input.metadata &&
            ...
            : {}),
    }), bridgeTimeoutMs(input.timeoutMs), "send_message");
    if (isTimeoutMarker(result)) {
        throw new Error(`${result.label} timed out after ${result.timeoutMs}ms`);
    }
    return { ...result, conversationId, response: result };
}
async function dispatchBridgeRequest(host: IosBridgeHost, request: BridgeRequest): Promise<unknown> {
        ...
        case "status":
            ...
            if (host.bootError) {
                return bridgeStatus({
                    ready: false,
                    phase: "error",
                    error: host.bootError instanceof Error
                        ? host.bootError.message
                        : String(host.bootError),
                });
            }
            if (payload.timeoutMs !== undefined) {
                await awaitIosBridgeBackend(host, bridgeTimeoutMs(payload.timeoutMs), "status");
                return bridgeStatus();
            }
            return bridgeStatus({ ready: false, phase: "starting" });
        case "http_request":
        case "http_fetch": {
            const backendForFetch = await awaitIosBridgeBackend(host, bridgeTimeoutMs(payload.timeoutMs), method);
            return fetchBackend(backendForFetch, (request.payload ?? {}) as HttpRequestPayload);
        }
        case "http_request_stream": {
            const streamPayload = (request.payload ?? {}) as HttpStreamRequestPayload;
            const streamId = typeof streamPayload.streamId === "string" &&
                streamPayload.streamId.trim()
                ? streamPayload.streamId.trim()
                : `ios-stream-${crypto.randomUUID()}`;
            const backendForStream = await awaitIosBridgeBackend(host, bridgeTimeoutMs(payload.timeoutMs), method);
            ...
            return fetchBackendStream(backendForStream, streamPayload, streamId, (frame) => callIosHost("stream_emit", frame, 30 * 60000).then(() => { }));
        }
        case "send_message": {
            const backendForMessage = await awaitIosBridgeBackend(host, bridgeTimeoutMs(payload.timeoutMs), method);
...
function reserveStdoutForBridgeProtocol(): () => void {
    ...
    const tryAssign = <T extends object, K extends keyof T>(target: T, key: K, value: T[K], restore: () => void): void => {
        try {
            target[key] = value;
    ...
    const writeToStderr = (chunk: string | Uint8Array, encoding?: BufferEncoding | ((err?: Error | null) => void), cb?: (err?: Error | null) => void): boolean => {
    ...
    const stdoutWriteToStderr = ((chunk: unknown, encoding?: unknown, cb?: unknown) => writeToStderr(chunk as string | Uint8Array, encoding as BufferEncoding, cb as ((err?: Error | null) => void) | undefined)) as typeof process.stdout.write;
    tryAssign(process.stdout, "write", stdoutWriteToStderr, () => {
        process.stdout.write = originalStdoutWrite;
    });
    tryAssign(console, "log", ((...args: unknown[]) => console.error(...args)) as typeof console.log, () => {
        console.log = originalConsoleLog;
    });
    tryAssign(console, "info", ((...args: unknown[]) => console.error(...args)) as typeof console.info, () => {
        console.info = originalConsoleInfo;
    });
    tryAssign(console, "debug", ((...args: unknown[]) => console.error(...args)) as typeof console.debug, () => {
...
export async function runIosBridgeCli(argv: string[] = process.argv): Promise<void> {
    ...
    process.on("unhandledRejection", (reason) => {
        console.error("[ios-bridge] unhandled rejection:", reason instanceof Error ? reason.stack || reason.message : reason);
    });
    process.on("uncaughtException", (error) => {
