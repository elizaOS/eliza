...
async function call(backend: IosBridgeBackend, method: string, rawPath: string, body?: unknown): Promise<{
    ...
    const res = await handleDirectCoreRoute(backend, method, rawPath, body === undefined ? {} : { body: JSON.stringify(body) });
    if (!res)
...
describe("iOS /api/transcripts/:id encoding", () => {
    ...
    it("canonical percent-encoded id still reaches transcript lookup", async () => {
        ...
        const { status, json } = await call(backend, "GET", `/api/transcripts/${id.replaceAll("-", "%2D")}`);
        expect(seen).toEqual([id]);
        expect(status).toBe(404);
        expect(json).toEqual({ error: "not found" });
    });
    it.each(["%", "%2", "%ZZ", "%E0%A4"])("rejects malformed %s with 400 before transcript lookup", async (token) => {
        const runtime = createFakeRuntime();
        ...
        for (const method of ["GET", "PUT", "DELETE"] as const) {
            const { status, json } = await call(backend, method, `/api/transcripts/${token}`, method === "PUT" ? { title: "x" } : undefined);
