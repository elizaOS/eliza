...
import { handleDirectCoreRoute } from "./bridge.ts";
import { it } from "vitest";
import { mkdtempSync } from "node:fs";
import { resetIosBrowserWorkspace } from "./bridge.ts";
import { tmpdir } from "node:os";
import { type IAgentRuntime } from "@elizaos/core";
import { type IosBridgeBackend } from "./bridge.ts";
    ...
    const runtime = {
        ...
        async getMemories(params: {
            ...
            if (params.textContains) {
                ...
                rows = rows.filter((m) => ((m.content as {
                    text?: string;
                }).text ?? "")
                    .toLowerCase()
                    .includes(needle));
        ...
        async updateMemory(memory: Partial<Memory> & {
            id: UUID;
        }): Promise<boolean> {
            for (const rows of tables.values()) {
...
async function call(backend: IosBridgeBackend, method: string, rawPath: string, body?: unknown): Promise<{
    status: number;
    json: Record<string, unknown>;
}> {
    const res = await handleDirectCoreRoute(backend, method, rawPath, body === undefined ? {} : jsonBody(body));
    if (!res)
        throw new Error(`route returned null: ${method} ${rawPath}`);
    return { status: res.status, json: JSON.parse(res.body) };
}
const seg = (text: string, endMs = 1000, speaker = "Speaker 1"): TranscriptSegment => ({
...
describe("iOS bridge — memories view routes", () => {
    ...
    async function seedMemory(table: string, text: string, createdAt: number, source?: string): Promise<void> {
    ...
    it("feed honors the limit + hasMore + before params", async () => {
        ...
        const beforeTexts = (before.json.memories as Array<{
            text: string;
        }>).map((m) => m.text);
        ...
        for (const cursor of [
            ...
            const invalid = await call(backend, "GET", `/api/memories/feed?before=${cursor}`);
    ...
    it("pages every tied-timestamp feed row exactly once with the id cursor", async () => {
        ...
        for (let pageNumber = 0; pageNumber < 3; pageNumber++) {
            ...
            if (page.json.hasMore === false || !last)
                break;
    ...
    it("rejects an id cursor without its timestamp pair", async () => {
        const result = await call(backend, "GET", `/api/memories/feed?beforeId=${crypto.randomUUID()}`);
    ...
    it("feed type filter scopes to a single table", async () => {
        ...
        const { json } = await call(backend, "GET", "/api/memories/feed?type=facts");
        const texts = (json.memories as Array<{
            text: string;
        }>).map((m) => m.text);
    ...
    it("browse paginates + keyword-filters with total/limit/offset", async () => {
        ...
        const texts = (search.json.memories as Array<{
            text: string;
        }>).map((m) => m.text);
        expect(texts).toEqual(["alpha echo", "alpha bravo"]);
        expect(search.json.total).toBe(2);
        const page = await call(backend, "GET", "/api/memories/browse?limit=1&offset=1");
    ...
    it("finds sparse matches beyond the first adapter window", async () => {
        ...
        const result = await call(backend, "GET", "/api/memories/browse?type=messages&q=needle%20missing&limit=20");
    ...
    it("fails closed when an adapter ignores the keyset cursor", async () => {
        ...
        runtime.getMemories = vi.fn((params) => cursorAware({ ...params, cursor: undefined }));
        await expect(call(backend, "GET", "/api/memories/browse?type=messages&q=needle%20missing&limit=20")).rejects.toMatchObject({ code: "MEMORY_BROWSE_CURSOR_NO_PROGRESS" });
        expect(runtime.getMemories).toHaveBeenCalledTimes(2);
    });
    it("fails closed at the shared request-wide sparse scan bound", async () => {
        let sequence = 0;
        const getMemories = vi.fn(async (params: {
            limit?: number;
        }): Promise<Memory[]> => Array.from({ length: params.limit ?? 0 }, () => {
        ...
        await expect(call(backend, "GET", "/api/memories/browse?type=messages&q=needle%20missing&limit=20")).rejects.toMatchObject({ code: "MEMORY_BROWSE_SCAN_LIMIT" });
        expect(getMemories.mock.calls.reduce((sum, [params]) => sum + (params.limit ?? 0), 0)).toBe(25000);
    });
    ...
    it("stats uses exact counts without materializing capped row windows", async () => {
        ...
        runtime.countMemories = vi.fn(async ({ tableName }) => values[tableName ?? "messages"] ?? 0);
        const { status, json } = await call(backend, "GET", "/api/memories/stats");
        expect(status).toBe(200);
    ...
    it("stats fails closed on an invalid adapter count", async () => {
        ...
        await expect(call(backend, "GET", "/api/memories/stats")).rejects.toMatchObject({ code: "MEMORY_STATS_INVALID_COUNT" });
    });
...
describe("iOS bridge — transcripts view routes", () => {
    ...
    it("create → list → get → update → delete round-trips", async () => {
        ...
        // Get by id
        const got = await call(backend, "GET", `/api/transcripts/${encodeURIComponent(id)}`);
        expect(got.status).toBe(200);
        expect((got.json.transcript as {
            id: string;
        }).id).toBe(id);
        // Update (PUT) — new title + longer segment
        const updated = await call(backend, "PUT", `/api/transcripts/${encodeURIComponent(id)}`, { title: "Standup (edited)", segments: [seg("hello there", 3000)] });
        expect(updated.status).toBe(200);
        expect(updated.json.transcript).toMatchObject({
            title: "Standup (edited)",
            durationMs: 3000,
        });
        expect((updated.json.transcript as {
            editedAt?: number;
        }).editedAt).toBeGreaterThan(0);
        // Delete
        const deleted = await call(backend, "DELETE", `/api/transcripts/${encodeURIComponent(id)}`);
    ...
    it("update rejects a body with neither title nor segments (400)", async () => {
        ...
        const id = (created.json.transcript as {
            id: string;
        }).id;
        const res = await call(backend, "PUT", `/api/transcripts/${encodeURIComponent(id)}`, {});
        expect(res.status).toBe(400);
    });
});
describe("iOS bridge — browser workspace routes", () => {
    ...
    it("starts in web mode with no tabs", async () => {
        const { status, json } = await call(backend, "GET", "/api/browser-workspace");
        expect(status).toBe(200);
    ...
    it("open → navigate → show/hide → close tab lifecycle", async () => {
        ...
        // Navigate
        const navigated = await call(backend, "POST", `/api/browser-workspace/tabs/${encodeURIComponent(id)}/navigate`, { url: "example.com" });
        expect((navigated.json.tab as {
            url: string;
        }).url).toBe("https://example.com");
        // Hide then show
        const hidden = await call(backend, "POST", `/api/browser-workspace/tabs/${encodeURIComponent(id)}/hide`);
        expect((hidden.json.tab as {
            visible: boolean;
        }).visible).toBe(false);
        const shown = await call(backend, "POST", `/api/browser-workspace/tabs/${encodeURIComponent(id)}/show`);
        expect((shown.json.tab as {
            visible: boolean;
        }).visible).toBe(true);
        // snapshot action returns empty data (web mode has no server screenshot)
        const snap = await call(backend, "GET", `/api/browser-workspace/tabs/${encodeURIComponent(id)}/snapshot`);
        expect(snap.json).toEqual({ data: "" });
        // Close
        const closed = await call(backend, "DELETE", `/api/browser-workspace/tabs/${encodeURIComponent(id)}`);
    ...
    it("opening a second visible tab hides the first", async () => {
        ...
        const first = tabs.find((t) => t.id === (a.json.tab as {
            id: string;
        }).id);
    ...
    it("acting on an unknown tab id 404s", async () => {
        const res = await call(backend, "POST", "/api/browser-workspace/tabs/nope/show");
...
describe("iOS bridge — conversation message failure surfacing", () => {
    ...
    function createMessageServiceRuntime(onCreateMemory: () => Promise<UUID>): IAgentRuntime {
        return {
            agentId: AGENT_ID,
            ...
            messageService: {
                async handleMessage(_runtime: IAgentRuntime, _message: Memory, onResponse: (content: {
                    text?: string;
    ...
    beforeEach(() => {
        ...
        process.env.ELIZA_STATE_DIR = mkdtempSync(path.join(tmpdir(), "ios-bridge-conv-"));
    });
    afterEach(() => {
    ...
    it("surfaces a rejected createMemory to stderr but still returns the reply", async () => {
        ...
        const runtime = createMessageServiceRuntime(() => Promise.reject(new Error("db offline")));
        const backend = makeBackend(runtime);
        seedConversation(backend, "conv-fail-1");
        const { status, json } = await call(backend, "POST", "/api/conversations/conv-fail-1/messages", { text: "hi there" });
        ...
        expect(errorSpy).toHaveBeenCalledWith("[ios-bridge] createMemory(messages) failed:", "db offline");
    });
    it("does not log when createMemory succeeds", async () => {
        const errorSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        const runtime = createMessageServiceRuntime(() => Promise.resolve(crypto.randomUUID() as UUID));
        const backend = makeBackend(runtime);
        seedConversation(backend, "conv-ok-1");
        const { status, json } = await call(backend, "POST", "/api/conversations/conv-ok-1/messages", { text: "hi there" });
        expect(status).toBe(200);
        expect(json.reply).toBe("hello from the local agent");
        expect(errorSpy).not.toHaveBeenCalledWith("[ios-bridge] createMemory(messages) failed:", expect.anything());
    });
});
describe("iOS bridge — unmatched routes still fall through", () => {
    it("returns null (→ eventual 404) for an unknown /api path", async () => {
        ...
        const res = await handleDirectCoreRoute(backend, "GET", "/api/does-not-exist", {});
        expect(res).toBeNull();
    });
    it("returns null for an unknown /api/memories subpath", async () => {
        const backend = makeBackend(createFakeRuntime());
        const res = await handleDirectCoreRoute(backend, "GET", "/api/memories/unknown", {});
        expect(res).toBeNull();
    });
    it("sorts conversations safely when updatedAt contains invalid date strings", async () => {
        ...
        const res = await handleDirectCoreRoute(backend, "GET", "/api/conversations", {});
