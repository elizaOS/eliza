...
- Because the string-prefix check cannot catch a symlink that escapes after normalization, `read()`/`list()` additionally realpath the full target through `assertRealPathWithinRoot()`. `write()` walks each parent component with `lstat` and creates only real directories — symlink ancestors are refused even when they resolve inside the workspace. Concurrent writers racing the same missing parent treat `EEXIST` as success only after a re-`lstat` still sees a real directory. When the final component already exists (checked via `lstat`), the target itself is realpath'd so an in-workspace symlink pointing outside the root is rejected instead of being followed and clobbered. Never drop the target-existence realpath in `write()`: `realpath` throws `ENOENT` on a to-be-created file, so the check is gated on `lstat` existence rather than run unconditionally.
