...
- Because the string-prefix check cannot catch symlink escapes, `read()` and `list()` realpath the full target through `assertRealPathWithinRoot()`. `write()` walks and canonicalizes each parent component, revalidates a concurrently created directory after `EEXIST`, and opens the final component with no-follow and exclusive-create semantics. It then truncates and writes through that same `FileHandle`, so a path replacement cannot redirect content I/O outside the root.
