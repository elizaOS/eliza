...
describe("isSafeBinUsage", () => {
  ...
  it("refuses when the safe-bin set is empty", () => {
    expect(isSafeBinUsage({ argv: ["ls"], resolution: res("ls"), safeBins: new Set(), fileExists: noFiles })).toBe(false);
  });

  it("refuses a non-allowlisted executable", () => {
    expect(isSafeBinUsage({ argv: ["rm", "-rf"], resolution: res("rm"), safeBins, fileExists: noFiles })).toBe(false);
  });

  it("refuses when the executable has no resolved path", () => {
    const unresolved: CommandResolution = { rawExecutable: "ls", executableName: "ls" };
    expect(isSafeBinUsage({ argv: ["ls"], resolution: unresolved, safeBins, fileExists: noFiles })).toBe(false);
  });

  it("approves an allowlisted bin with only flags", () => {
    expect(isSafeBinUsage({ argv: ["ls", "-la", "--color"], resolution: res("ls"), safeBins, fileExists: noFiles })).toBe(true);
  });

  it("refuses a path-like positional argument", () => {
    expect(isSafeBinUsage({ argv: ["cat", "/etc/passwd"], resolution: res("cat"), safeBins, fileExists: noFiles })).toBe(false);
  });

  it("refuses a positional that names an existing file", () => {
    expect(isSafeBinUsage({ argv: ["cat", "secrets"], resolution: res("cat"), safeBins, cwd: "/work", fileExists: () => true })).toBe(false);
  });

  it("refuses a --flag=<path> value", () => {
    expect(isSafeBinUsage({ argv: ["ls", "--width=/etc/passwd"], resolution: res("ls"), safeBins, fileExists: noFiles })).toBe(false);
  });

  it("approves a non-path positional that does not exist", () => {
    expect(isSafeBinUsage({ argv: ["ls", "subcmd"], resolution: res("ls"), safeBins, fileExists: noFiles })).toBe(true);
  });
});
