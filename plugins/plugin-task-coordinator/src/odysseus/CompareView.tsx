    ...
    return (
      ...
      >
        ...
        <CompareSelector
          ...
          onReset={resetConfig}
          onOpenPoolEditor={() => setPoolEditorOpen(true)}
...
function CompareSelector({
  ...
  onShuffleFill: () => void;
  onReset: () => void;
  onOpenPoolEditor: () => void;
  onOpenScoreboard: () => void;
  onStart: () => void;
  onClose: () => void;
}): ReactNode {
  const win = useWindowControls("win-compare-selector", { w: 520, h: 560 });
    ...
    <div className="od-search-panel od-cmp-selector" style={win.panelStyle}>
      ...
      <div className="od-cmp-sel-body">
        <p className="od-cmp-sel-desc">
          Select models to compare side-by-side. The same prompt goes to all.
        </p>

        {/* Mode toggles — blind / parallel / shuffle / reset (selector.js) */}
        <div className="od-cmp-sel-section">
          <div className="od-cmp-sel-label">Mode</div>
          <div className="od-cmp-sel-toggles">
            <button
              type="button"
              className={`od-cmp-toggle${blindMode ? " active" : ""}`}
              title="Blind mode — hide model names until you vote"
              aria-pressed={blindMode}
              onClick={() => onSetBlind(!blindMode)}
            >
              {blindMode ? <EyeOff size={15} /> : <Eye size={15} />}
              <span>Blind</span>
            </button>
            <button
              type="button"
              className={`od-cmp-toggle${parallel ? " active" : ""}`}
              title="Parallel runs all models at once; Sequential one at a time"
            ...
            <button
              type="button"
              className="od-cmp-toggle"
              title="Edit which models the shuffle can pick"
              onClick={onOpenPoolEditor}
            >
              <span>Pool</span>
            </button>
            <button
        ...
        {/* Type tabs — only Chat is faithfully representable on this runtime.
            Agent / Search / Research need a dual-model run + a search-provider
            backend that the single-agent runtime does not expose, so they are
            not shown as dead tabs. */}
        <div className="od-cmp-sel-section">
          <div className="od-cmp-sel-label">Type</div>
          <div className="od-cmp-sel-tabs">
            <button type="button" className="od-cmp-sel-tab active" disabled>
              Chat
            </button>
          </div>
        </div>
        ...
        {/* Model rows */}
        <div className="od-cmp-sel-section">
          <div className="od-cmp-sel-label">Models</div>
