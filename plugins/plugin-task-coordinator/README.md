...
**Orchestrator** — a multi-agent task orchestration workbench. Create task threads with goals and acceptance criteria, assign sub-agents (Eliza Code, Claude Code, Codex, Pi Agent), monitor their timeline of events and messages, fork tasks, validate results, and send messages to running tasks — all from one panel.
...
The settings panel manages configuration for Eliza Code, Pi Agent, Claude, and Codex.
