...
```bash
cmake -B plugins/plugin-local-inference/native/qjl-cpu/build -S plugins/plugin-local-inference/native/qjl-cpu
cmake --build plugins/plugin-local-inference/native/qjl-cpu/build -j
plugins/plugin-local-inference/native/qjl-cpu/build/qjl_int8_smoke
plugins/plugin-local-inference/native/qjl-cpu/build/qjl_avxvnni_smoke
...
Follow the repository-wide verification standard in the [root AGENTS.md](../../../../AGENTS.md). Review packed bytes, score diffs, selected SIMD lane, and built-fork graph evidence.
