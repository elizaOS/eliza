...
const dylib = arg("--dylib", defaultDylib(backend));
const bundle = arg("--bundle", `${HOME}/.eliza/local-inference/models/eliza-1-2b.bundle`);
const outPath = arg("--out", path.resolve(__dirname, "bench_results", `tts_asr_self_labelled_${new Date().toISOString().slice(0, 10)}.json`));
...
export interface LabelledSetEvidence {
    ...
    measurementClass: "self_labelled_tts_asr_loopback" | "external_generated_tts_loopback" | "external_labelled_unknown_provenance" | "real_recorded_labelled_speech";
...
export function normalizeWavDirProvenance(input: string): AudioProvenance {
    ...
    throw new Error(`[asr-bench] unknown --wav-dir-provenance "${input}" (expected generated-tts, external-unknown, or real-recorded)`);
}
export function labelledSetEvidenceFor(args: {
    ...
    if (args.source === "tts_loopback_self_labelled") {
        return {
            ...
            caveat: "Audio is synthesized from the same local bundle and labelled with the text that created it. " +
    ...
    if (args.realRecorded && explicit === "generated_tts") {
        throw new Error("[asr-bench] --real-recorded conflicts with --wav-dir-provenance generated-tts");
    }
    ...
    if (provenance === "generated_tts") {
        return {
            ...
            realRecordedWer: false,
            publishGateEligible: false,
            caveat: "WAV+txt inputs are generated TTS audio loaded from disk. This is generated-audio re-ASR " +
    ...
    return {
        ...
        realRecordedWer: false,
        publishGateEligible: false,
        caveat: "WAV+txt inputs did not declare real recorded provenance. Pass --real-recorded only for " +
...
export function publishGateEligibilityFor(args: {
    ...
    if (!Number.isInteger(minRealRecordedUtterances) || minRealRecordedUtterances < 1) {
        throw new Error(`[asr-bench] --min-real-recorded-utterances must be a positive integer; got ${minRealRecordedUtterances}`);
    }
    const meetsMinRealRecordedUtterances = args.utteranceCount >= minRealRecordedUtterances;
    if (!args.evidence.publishGateEligible) {
        return {
            publishGateEligible: false,
            meetsMinRealRecordedUtterances,
            minRealRecordedUtterances,
            reason: args.evidence.caveat ??
    ...
    if (!meetsMinRealRecordedUtterances) {
        return {
            ...
            meetsMinRealRecordedUtterances,
            minRealRecordedUtterances,
            reason: `real recorded ASR WER publish evidence requires >=${minRealRecordedUtterances} ` +
...
function readMonoPcm16Wav(file: string): {
    pcm: Float32Array;
    sampleRateHz: number;
} {
    ...
    for (let i = 0; i < n; i++)
        pcm[i] = Math.max(-1, buf.readInt16LE(dataOff + i * 2) / 32768);
...
function loadExternalWavDir(dir: string): Utterance[] {
    ...
    for (const name of readdirSync(dir).sort()) {
        ...
        if (!existsSync(txt)) {
            if (verbose)
                process.stderr.write(`[asr-bench] skip ${name}: no ${base}.txt reference\n`);
            continue;
        }
    ...
    if (out.length === 0)
        throw new Error(`[asr-bench] no WAV+.txt pairs found in ${dir}`);
    return out;
}
function loadAcceptedPrompts(): string[] {
    const explicit = args("--prompt").flatMap((v) => v
        .split("|")
        .map((s) => s.trim())
        .filter(Boolean));
...
async function startPromptServer(): Promise<{
    port: number;
    child: PromptServerProcess;
    textModel: string;
}> {
    const server = path.join(binDir, "llama-server");
    if (!existsSync(server))
        throw new Error(`[asr-bench] --generate-prompts needs ${server}`);
    const textModel = firstBundleTextModel(bundle);
    if (!textModel)
        throw new Error(`[asr-bench] --generate-prompts could not find a text/*.gguf in ${bundle}`);
...
async function generatePrompts(count: number): Promise<{
    prompts: string[];
    model: string | null;
    latencyMs: number | null;
}> {
    if (count <= 0)
        return { prompts: [], model: null, latencyMs: null };
    ...
    try {
        ...
        const res = await fetch(`http://127.0.0.1:${server.port}/completion`, {
            ...
            body: JSON.stringify({
                prompt: `Write ${count} short speech recognition test utterances, one per line. ` +
        ...
        if (!res.ok)
            throw new Error(`[asr-bench] /completion HTTP ${res.status}: ${(await res.text()).slice(0, 200)}`);
        const json = await res.json();
        const content = String(json.content ?? json.response ?? "");
        const prompts = parseGeneratedPrompts(content, count);
        if (prompts.length === 0)
            throw new Error("[asr-bench] local model generated no parseable prompts");
    ...
    try {
        ...
        if (wavDir) {
            ...
            for (let i = 0; i < phrases.length; i++) {
                ...
                if (written <= 0)
                    throw new Error(`[asr-bench] TTS produced ${written} samples for "${phrases[i]}"`);
                ...
                utterances.push({
                    ...
                    synthMs,
                    promptSource: i < acceptedPrompts.length
                        ? "accepted_prompt"
                ...
                if (verbose)
                    process.stderr.write(`[asr-bench] synthesized "${phrases[i]}" → ${written} samples in ${synthMs.toFixed(1)}ms\n`);
            }
        ...
        for (const u of utterances) {
            ...
            if (timedSupported) {
                ...
                if (verbose && validation.violations.length > 0) {
                    process.stderr.write(`[asr-bench] ${u.id}: ${validation.violations.length} word-timing violation(s): ${validation.violations
                        .slice(0, 3)
            ...
            if (verbose) {
                process.stderr.write(`[asr-bench] ${u.id}: ref="${nRef}" hyp="${nHyp}" wer=${wer.toFixed(3)} asr_rtf=${rtf.toFixed(2)} roundtrip_ms=${roundtripMs.toFixed(1)}\n`);
            }
        }
        ...
        const result = {
            ...
            labelledSet: {
                ...
                minRealRecordedUtterances: publishGateEligibility.minRealRecordedUtterances,
                meetsMinRealRecordedUtterances: publishGateEligibility.meetsMinRealRecordedUtterances,
                wavDir: wavDir || null,
                count: rows.length,
            ...
            aggregate: {
                ...
                rtf: aggregateRtf,
                asrRtf: aggregateRtf,
                meanSynthMs: rows.some((r) => r.synthMs !== null)
        ...
        if (evalOut) {
            ...
            const evalBlob = validMeasurement
                ...
                : {
                    ...
                    labelledSetProvenance: labelledSetEvidence.provenance,
                    utterances: rows.length,
                    reason: publishGateEligibility.reason ??
                        labelledSetEvidence.caveat ??
