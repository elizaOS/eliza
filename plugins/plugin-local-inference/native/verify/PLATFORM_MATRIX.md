...
> (`packages/app/scripts/build-llama-cpp-mtp.mjs`). For each target:
...
> here. The tracked hardware/readiness view was the pipeline test-matrix note
...
> bundle plan (`ELIZA_1_GGUF_PLATFORM_PLAN.json`) was likewise never
> committed.
...
Nothing regressed in this wave. `bun run typecheck` for `packages/app` is
clean; `bun test packages/app/src/services/local-inference/` is 603 pass /
