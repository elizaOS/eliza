...
```bash
cmake -B plugins/plugin-local-inference/native/polarquant-cpu/build -S plugins/plugin-local-inference/native/polarquant-cpu
cmake --build plugins/plugin-local-inference/native/polarquant-cpu/build -j
ctest --test-dir plugins/plugin-local-inference/native/polarquant-cpu/build --output-on-failure
python3 plugins/plugin-local-inference/native/polarquant-cpu/scripts/test_converter.py
...
Follow the repository-wide verification standard in the [root AGENTS.md](../../../../AGENTS.md). Review encoded bytes, round-trip error, dot-product diffs, dispatch selection, and hardware graph evidence.
