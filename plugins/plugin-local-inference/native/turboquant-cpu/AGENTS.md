...
```bash
cmake -B plugins/plugin-local-inference/native/turboquant-cpu/build -S plugins/plugin-local-inference/native/turboquant-cpu
cmake --build plugins/plugin-local-inference/native/turboquant-cpu/build -j
ctest --test-dir plugins/plugin-local-inference/native/turboquant-cpu/build --output-on-failure
...
Follow the repository-wide verification standard in the [root AGENTS.md](../../../../AGENTS.md). Review packed blocks, parity diffs, selected dispatch lane, converter output, and relevant real-hardware graph evidence.
