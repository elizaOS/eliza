...
```bash
cmake -B plugins/plugin-local-inference/native/silero-vad-cpp/build -S plugins/plugin-local-inference/native/silero-vad-cpp
cmake --build plugins/plugin-local-inference/native/silero-vad-cpp/build -j
python3 plugins/plugin-local-inference/native/silero-vad-cpp/scripts/silero_vad_to_gguf.py --output plugins/plugin-local-inference/native/silero-vad-cpp/build/silero-vad-v5.gguf
ctest --test-dir plugins/plugin-local-inference/native/silero-vad-cpp/build --output-on-failure
```

Run the Python parity test with the real upstream artifact when changing conversion or inference. Review probability traces over speech, silence, boundaries, and resets. Follow the repository-wide evidence standard in the [root AGENTS.md](../../../../AGENTS.md).
