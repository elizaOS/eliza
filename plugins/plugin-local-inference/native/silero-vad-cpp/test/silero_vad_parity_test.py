...
Run from anywhere:
  ...
  python3 plugins/plugin-local-inference/native/silero-vad-cpp/test/silero_vad_parity_test.py \
      --library plugins/plugin-local-inference/native/silero-vad-cpp/build/libsilero_vad.so \
      --gguf    plugins/plugin-local-inference/native/silero-vad-cpp/build/silero-vad-v5.gguf
...
def main() -> int:
    ...
    build_root = here.parents[1] / "build"
    default_lib = build_root / "libsilero_vad.so"
    default_gguf = build_root / "silero-vad-v5.gguf"
