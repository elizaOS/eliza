...
```bash
cmake -B plugins/plugin-local-inference/native/voice-classifier-cpp/build -S plugins/plugin-local-inference/native/voice-classifier-cpp
...
Parity tests can require external GGUF/reference fixtures; run them with real artifacts for any affected head. Review probabilities, embeddings, diarization segments, error codes, and EOT unavailability by hand. Follow the repository-wide evidence standard in the [root AGENTS.md](../../../../AGENTS.md).
