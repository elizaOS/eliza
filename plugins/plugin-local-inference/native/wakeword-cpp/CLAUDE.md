...
```bash
cmake -B plugins/plugin-local-inference/native/wakeword-cpp/build -S plugins/plugin-local-inference/native/wakeword-cpp
cmake --build plugins/plugin-local-inference/native/wakeword-cpp/build -j
ctest --test-dir plugins/plugin-local-inference/native/wakeword-cpp/build --output-on-failure
```
...
Follow the repository-wide verification standard in the [root AGENTS.md](../../../../AGENTS.md). Review score traces, latency, CPU use, resolver selection, and failure behavior.
