...
const KEYWORD_RULES = [
    // Image rules (most-common first).
    {
        ...
        strip: /^\s*(please\s+)?(draw|sketch|paint|illustrate)(\s+me)?(\s+an?)?\s+(of\s+)?/i,
    },
    {
        kind: "image",
        pattern: /\b(generate|create|make)\s+(an?\s+|the\s+)?(image|picture|photo|photograph|drawing|illustration)\b/i,
        strip: /^\s*(please\s+)?(generate|create|make)\s+(an?\s+|the\s+)?(image|picture|photo|photograph|drawing|illustration)(\s+of)?\s*/i,
    },
    {
        kind: "image",
    ...
    // Audio rules.
    {
        ...
        strip: /^\s*(please\s+)?(say|speak|read\s+aloud|read\s+out|narrate)(\s+aloud)?(\s+this)?(\s+in\s+\w+)?[:,]?\s+/i,
    },
    {
        kind: "audio",
        pattern: /\b(text\s*to\s*speech|tts|voice\s+this|voice\s+over)\b/i,
        strip: /^\s*(please\s+)?(do\s+)?(text\s*to\s*speech|tts|voice\s+this|voice\s+over)[:,]?\s*/i,
    },
    {
        kind: "audio",
        pattern: /\bgenerate\s+(an?\s+|some\s+)?(audio|speech|voice)\b/i,
        strip: /^\s*(please\s+)?generate\s+(an?\s+|some\s+)?(audio|speech|voice)\s+(of|for|saying)?\s*/i,
    },
    // Video rules. We detect them only to refuse cleanly.
    {
        kind: "video",
        pattern: /\b(video|animate|animation|movie|clip)\b/i,
        strip: /^\s*(please\s+)?(generate|create|make|render)?\s*(an?\s+|the\s+)?(video|animation|movie|clip)(\s+of)?\s*/i,
    },
];
function stripPrompt(rule, text) {
    if (!rule.strip)
        return text.trim();
 ...
 */
export async function detectMediaIntent(text, options = {}) {
...
function makeRuntimeClassifier(runtime) {
    return async (prompt) => {
...
function normalizeAudioBytes(value) {
    ...
    throw new Error("[generate-media] TEXT_TO_SPEECH backend returned non-binary audio output");
}
...
function detectImageMimeFromDataUrl(url) {
...
async function dispatchImage(runtime, prompt) {
    ...
    if (!first || typeof first.url !== "string" || first.url.length === 0) {
        throw new Error("[generate-media] IMAGE backend returned no result; expected ImageGenerationResult[]");
    }
...
async function dispatchAudio(runtime, text) {
    const raw = (await runtime.useModel(ModelType.TEXT_TO_SPEECH, {
        text,
    }));
    const bytes = normalizeAudioBytes(raw);
    if (bytes.length === 0) {
        throw new Error("[generate-media] TEXT_TO_SPEECH backend returned an empty buffer");
    }
...
function makeAttachment(success, prompt) {
    const contentType = success.kind === "image" ? ContentType.IMAGE : ContentType.AUDIO;
    return {
        id: uuidLike(),
        url: success.url,
...
export function buildGenerateMediaHandler(opts = {}) {
    const detect = opts.detectIntent ?? detectMediaIntent;
    const classifierFactory = opts.classifierFactory ?? makeRuntimeClassifier;
    return async function generateMediaHandler(runtime, message, _state, _options, callback) {
        const raw = extractMessageText(message);
        if (!raw.trim()) {
            const errText = "GENERATE_MEDIA requires a non-empty message describing what to generate.";
        ...
        if (!intent) {
            const errText = 'I couldn\'t tell whether you wanted an image, audio, or video. Try "draw me ...", "say ...", or describe the picture you want.';
            await callback?.({ text: errText });
            return {
                success: false,
                text: errText,
        ...
        try {
            const result = intent.kind === "image"
            ...
            const narration = result.kind === "image"
            ...
            return {
                ...
                data: {
                    source: "generate-media",
                    computerUseAction: result.kind === "image"
            ...
            const msg = err instanceof Error ? err.message : String(err);
            logger.warn({ err: msg, kind: intent.kind }, "[generate-media] dispatch failed");
            const errText = intent.kind === "image"
            ...
            return {
                ...
                data: {
                    source: "generate-media",
                    computerUseAction: intent.kind === "image"
...
// Validator
// ---------------------------------------------------------------------------
async function validate(_runtime, message) {
...
export const generateMediaAction = {
    ...
    description: "Generate an image, audio (TTS), or video from a natural-language prompt. Routes to the appropriate local model via the runtime model registry. Video is not yet supported and is refused cleanly.",
    descriptionCompressed: "GENERATE_MEDIA image|audio|future-video prompt; routes IMAGE|TEXT_TO_SPEECH",
    routingHint: "explicit ask to draw/picture/photo/say/speak/read-aloud/animate -> GENERATE_MEDIA; not for general text replies",
...
//# sourceMappingURL=generate-media.js.map