...
import { PII_ENTITY_RECOGNIZER_SERVICE } from "@elizaos/core";
...
import { detectPii } from "@elizaos/core";
import { hasRoleAccess } from "@elizaos/core";
import { logger } from "@elizaos/core";
import { transcriptCapturePrivacyState } from "@elizaos/core/transcripts";
import { transcriptConsentAllowsSharing } from "../services/voice/transcript-store.js";
import { transcriptPiiRecognizer } from "../services/voice/transcript-store.js";
...
import { type PiiEntityRecognizer } from "@elizaos/core";
import { type PiiEntityRecognizerService } from "@elizaos/core";
import { type PiiTextSpan } from "@elizaos/shared";
import { type ProviderDataRecord } from "@elizaos/core";
import { type Transcript } from "@elizaos/shared";
import { type TranscriptCaptureSharingState } from "@elizaos/shared";
import { type TranscriptServiceRuntime } from "../services/voice/transcript-service.js";
import { type TranscriptSharingState } from "@elizaos/shared";
import { type TranscriptStoreRuntime } from "../services/voice/transcript-store.js";
...
function parseInput(parameters: Record<string, unknown>, options: {
    allowTarget?: boolean;
    ...
    if (parameters.entityId !== undefined && !entityId)
        return null;
    if (parameters.roomId !== undefined && !roomId)
        return null;
    if (parameters.redactForAll !== undefined &&
        typeof parameters.redactForAll !== "boolean") {
        return null;
    }
    const redactForAll = parameters.redactForAll === true;
    const targetCount = Number(Boolean(entityId)) + Number(Boolean(roomId)) + Number(redactForAll);
    if (options.allowTarget && targetCount !== 1)
        return null;
    if (!options.allowTarget && targetCount !== 0)
        return null;
    const mode = parseMode(parameters.mode) ?? options.defaultMode;
    if (parameters.mode !== undefined && !parseMode(parameters.mode))
        return null;
    return {
...
function runtimePiiRecognizer(runtime: IAgentRuntime): PiiEntityRecognizer | undefined {
    const service = runtime.getService(PII_ENTITY_RECOGNIZER_SERVICE) as Partial<PiiEntityRecognizerService> | null;
    return service?.getRecognizer?.() ?? undefined;
}
async function transcriptPiiSpans(transcript: Transcript, recognizer?: PiiEntityRecognizer): Promise<PiiTextSpan[]> {
    ...
    for (const span of await transcriptPiiRecognizer(transcript, recognizer).recognize(corpus)) {
...
async function verifiedRedactedAudioUrl(runtime: IAgentRuntime, transcript: Transcript, recognizer?: PiiEntityRecognizer): Promise<string | undefined> {
    ...
    if (piiSpans.length === 0)
        return undefined;
    const service = runtime.getService(AUDIO_REDACTION_SERVICE_TYPE) as AudioRedactionServiceLike | null;
    if (!service) {
        throw new ElizaError("verified audio redaction service is unavailable; refusing to create an audio-bearing redacted variant", { code: "AUDIO_REDACTION_VERIFY_UNAVAILABLE" });
    }
...
function hasTranscriptRoleAccess(runtime: IAgentRuntime, message: Memory, requiredRole: RoleName): Promise<boolean> {
    return hasRoleAccess(runtime, message, requiredRole);
}
async function canManageTranscript(runtime: IAgentRuntime, message: Memory, transcriptId: UUID): Promise<boolean> {
    if (await hasTranscriptRoleAccess(runtime, message, "ADMIN"))
        return true;
    if (!(await hasTranscriptRoleAccess(runtime, message, "USER")))
        return false;
    const row = await (runtime as TranscriptStoreRuntime).getMemoryById(transcriptId);
    if (isRedactedVariantRow(row))
        return false;
    return row?.entityId === message.entityId;
}
async function accessContextForMessage(runtime: IAgentRuntime, message: Memory, transcriptId: UUID): Promise<AccessContext | undefined> {
...
function auditDenied(runtime: IAgentRuntime, action: string, message: Memory, input: TranscriptPermissioningInput | null): void {
...
function participantEntityIds(transcript: Transcript): UUID[] {
    ...
    for (const participant of participants) {
        if (!participant || typeof participant !== "object")
            continue;
        const entityId = parseUuid((participant as {
            entityId?: unknown;
        }).entityId);
        if (entityId)
            ids.add(entityId);
...
async function requireFullTranscript(runtime: IAgentRuntime, store: TranscriptStore, transcriptId: UUID, accessContext: AccessContext | undefined): Promise<ActionResult | null> {
    const transcript = await store.get(transcriptId, accessContext);
    const row = await (runtime as TranscriptStoreRuntime).getMemoryById(transcriptId);
    if (isRedactedVariantRow(row)) {
        return fail("TRANSCRIPT_REDACTED_VIEW", "Only an original transcript can be redacted or shared.");
    }
    if (!transcript) {
        return fail("TRANSCRIPT_NOT_ACCESSIBLE", "Transcript not found or not accessible.");
    }
    if (transcript.redacted) {
        return fail("TRANSCRIPT_REDACTED_VIEW", "Only a full transcript can be redacted or shared.");
    }
    return null;
}
export const redactTranscriptAction: Action = {
    ...
    description: "Create or refresh the redacted variant for a stored meeting transcript. The original stays unchanged; any variant audio is published only after fail-closed PII redaction verification.",
    routingHint: "user asks to redact/anonymize a meeting transcript -> REDACT_TRANSCRIPT; do not edit the original transcript text",
    ...
    validate: async () => true,
    handler: async (runtime: IAgentRuntime, message: Memory, _state?: unknown, options?: unknown, callback?: HandlerCallback): Promise<ActionResult> => {
        const input = parseInput(paramsFromOptions(options), {
            allowTarget: false,
        });
        if (!input) {
            const result = fail("REDACT_TRANSCRIPT_INVALID", "REDACT_TRANSCRIPT requires a valid transcriptId.");
        ...
        if (!(await canManageTranscript(runtime, message, input.transcriptId))) {
            ...
            const result = fail("REDACT_TRANSCRIPT_DENIED", "You do not have permission to redact that transcript.");
            await callback?.({ text: result.text, actions: ["REDACT_TRANSCRIPT"] });
            return result;
        }
        try {
            const store = transcriptStore(runtime);
            const accessContext = await accessContextForMessage(runtime, message, input.transcriptId);
            const denied = await requireFullTranscript(runtime, store, input.transcriptId, accessContext);
            if (denied) {
                auditDenied(runtime, "REDACT_TRANSCRIPT", message, input);
            ...
            const redactedAudioUrl = await verifiedRedactedAudioUrl(runtime, original, recognizer);
...
export const shareTranscriptAction: Action = {
    ...
    description: "Share a stored transcript with one entity or a persisted room-roster snapshot as full or redacted content. Admin redact-for-all snapshots the room and grants every participant the redacted variant.",
    routingHint: "user asks to share a meeting transcript -> SHARE_TRANSCRIPT with entityId and mode full|redacted; admin redacted-for-all uses redacted mode for non-privileged grants",
    roleGate: { minRole: "USER" },
    parameters: [
        {
            name: "transcriptId",
        ...
        {
            name: "roomId",
            description: "Meeting room id whose current persisted participant roster receives grants.",
            required: false,
            schema: { type: "string" as const },
        },
        {
            name: "redactForAll",
            description: "Admin-only: grant the redacted variant to every persisted meeting participant.",
            required: false,
            schema: { type: "boolean" as const, default: false },
        },
        {
            name: "mode",
            description: "Disclosure mode. Use redacted for ordinary participants; full requires admin access.",
            required: false,
            schema: {
    ...
    handler: async (runtime: IAgentRuntime, message: Memory, _state?: unknown, options?: unknown, callback?: HandlerCallback): Promise<ActionResult> => {
        ...
        if (!input?.mode || (input.redactForAll && input.mode !== "redacted")) {
            const result = fail("SHARE_TRANSCRIPT_INVALID", "SHARE_TRANSCRIPT requires a valid transcriptId, exactly one of entityId, roomId, or redactForAll, and mode full or redacted.");
        ...
        if (!canShare) {
            ...
            const result = fail("SHARE_TRANSCRIPT_DENIED", requiresAdmin
        ...
        try {
            ...
            if (!transcriptConsentAllowsSharing(original)) {
                const result = fail("SHARE_TRANSCRIPT_CONSENT_REQUIRED", "The persisted consent state does not permit sharing this transcript.");
                await callback?.({ text: result.text, actions: ["SHARE_TRANSCRIPT"] });
                return result;
            }
            const row = await (runtime as TranscriptStoreRuntime).getMemoryById(input.transcriptId);
            if (!row) {
                throw new ElizaError("transcript row disappeared during sharing", {
                    code: "TRANSCRIPT_NOT_ACCESSIBLE",
                });
            }
            const roomTarget = input.roomId ?? (input.redactForAll ? row.roomId : null);
            const roomEntityIds = roomTarget ? participantEntityIds(original) : [];
            if (roomTarget && row.roomId !== roomTarget) {
                const result = fail("SHARE_TRANSCRIPT_ROOM_MISMATCH", "The requested room does not own this transcript.");
                await callback?.({ text: result.text, actions: ["SHARE_TRANSCRIPT"] });
                return result;
            }
            if (roomTarget && roomEntityIds.length === 0) {
                const result = fail("SHARE_TRANSCRIPT_ROOM_EMPTY", "The persisted meeting roster has no resolved participant entities to share with.");
                await callback?.({ text: result.text, actions: ["SHARE_TRANSCRIPT"] });
                return result;
            }
            ...
            const result = ok(input.redactForAll
...
export const manageTranscriptPrivacyAction: Action = {
    ...
    description: "Change one retained meeting artifact's visibility, or permanently delete source audio while preserving the transcript.",
    routingHint: "user asks to make transcript/notes/source audio/generated artifacts private, restricted, shared, disabled, or asks to delete retained meeting audio",
    roleGate: { minRole: "USER" },
    parameters: [
        {
            name: "transcriptId",
        ...
        },
        {
            ...
            description: "Permanently delete retained source audio while keeping transcript text.",
            required: false,
            schema: { type: "boolean" as const, default: false },
        },
    ],
    ...
    handler: async (runtime: IAgentRuntime, message: Memory, _state?: unknown, options?: unknown, callback?: HandlerCallback): Promise<ActionResult> => {
        ...
        const artifact = MANAGEABLE_ARTIFACTS.find((value) => value === params.artifact);
        const state = MANAGEABLE_SHARING_STATES.find((value) => value === params.state) as TranscriptSharingState | undefined;
        const deleteSourceAudio = params.deleteSourceAudio === true;
        if (!transcriptId ||
            (deleteSourceAudio
                ? artifact !== undefined || state !== undefined
                : !artifact || !state)) {
            const result = fail("MANAGE_TRANSCRIPT_PRIVACY_INVALID", "Provide transcriptId and either artifact+state, or deleteSourceAudio=true.");
            await callback?.({
                text: result.text,
        ...
        if (!(await canManageTranscript(runtime, message, transcriptId))) {
            const result = fail("MANAGE_TRANSCRIPT_PRIVACY_DENIED", "You do not have permission to manage that transcript.");
            await callback?.({
                text: result.text,
                actions: ["MANAGE_TRANSCRIPT_PRIVACY"],
            });
        ...
        try {
            const privacy = new TranscriptPrivacyService(runtime as TranscriptServiceRuntime);
            ...
            const result = ok(deleteSourceAudio
                ...
                ...(transcriptCapturePrivacyState(transcript).retentionState
                    ? {
                        retentionState: transcriptCapturePrivacyState(transcript).retentionState,
                    }
                    : {}),
            });
            await callback?.({
            ...
            // error-policy:J1 action boundary returns a structured failure to the planner loop.
            const result = fail("MANAGE_TRANSCRIPT_PRIVACY_FAILED", error instanceof Error ? error.message : String(error));
            await callback?.({
                text: result.text,
                actions: ["MANAGE_TRANSCRIPT_PRIVACY"],
            });
