...
import { LOCAL_INFERENCE_MODEL_TYPES } from "./provider.js";
import { LOCAL_INFERENCE_PROVIDER_ID } from "./provider.js";
import { MODEL_CATALOG as SHARED_MODEL_CATALOG } from "@elizaos/plugin-native-inference/model-catalog/catalog";
import { ROUTING_POLICIES } from "@elizaos/plugin-native-inference/model-catalog/routing-policy";
import { buildHuggingFaceResolveUrl } from "@elizaos/plugin-native-inference/model-catalog/catalog";
import { classifyDeviceTier } from "./services/device-tier.js";
import { isMobilePlatform } from "@elizaos/core/runtime-env";
import { isRoutingPolicy } from "@elizaos/plugin-native-inference/model-catalog/routing-policy";
import { logger } from "@elizaos/core";
import { readJsonBody } from "@elizaos/core/api/http-helpers";
import { readRoutingPreferences } from "@elizaos/plugin-native-inference/model-catalog/routing-preferences";
...
import { setPolicy } from "@elizaos/plugin-native-inference/model-catalog/routing-preferences";
import { setPreferredProvider } from "@elizaos/plugin-native-inference/model-catalog/routing-preferences";
import { setTextRouting } from "@elizaos/plugin-native-inference/model-catalog/routing-preferences";
import { toLocalInferenceStoredPath } from "./services/paths.js";
import { type AgentModelSlot } from "@elizaos/core/contracts/local-inference";
import { type CatalogModel as SharedCatalogModel } from "@elizaos/core/contracts/local-inference";
...
async function prewarmLocalVoiceStackLazy(modelId: string): Promise<void> {
    const { prewarmLocalVoiceStackForModel } = await import("./services/voice-prewarm.js");
    await prewarmLocalVoiceStackForModel(modelId);
}
type ModelRole = "chat" | "embedding";
type DownloadState = "queued" | "downloading" | "completed" | "failed" | "cancelled";
type MobileDeviceBridgeApi = {
    ...
    loadMobileDeviceBridgeModel: (modelPath: string, modelId: string) => Promise<void>;
...
type AospLocalInferenceApi = {
    buildAospLoadModelArgs: (role: "chat" | "embedding", modelPath: string) => unknown;
...
function getMobileDeviceBridgeApi(): Promise<MobileDeviceBridgeApi> {
    mobileDeviceBridgeApiPromise ??= import("@elizaos/plugin-native-inference/mobile-device-bridge-bootstrap") as Promise<MobileDeviceBridgeApi>;
    return mobileDeviceBridgeApiPromise;
}
function getAospLocalInferenceApi(): Promise<AospLocalInferenceApi> {
    aospLocalInferenceApiPromise ??= import("@elizaos/plugin-native-inference") as Promise<AospLocalInferenceApi>;
...
export type LocalInferenceCommandIntent = "retry" | "resume" | "redownload" | "download" | "cancel" | "switch_smaller" | "status" | "use_cloud" | "use_local";
...
export interface LocalInferenceChatMetadata {
    ...
    status: "missing" | "downloading" | "loading" | "failed" | "no_space" | "idle" | "ready" | "cancelled" | "routing";
    modelId?: string | null;
...
type Assignments = Partial<Record<(typeof LOCAL_INFERENCE_MODEL_TYPES)[number], string>>;
...
export type LocalInferenceManagementOp = "start_download" | "cancel_download" | "set_active" | "clear_active" | "uninstall_model" | "verify_model" | "trigger_voice_model_update" | "pin_voice_model" | "set_voice_model_preferences" | "set_policy" | "set_preferred_provider" | "set_assignment";
...
function catalogRole(model: SharedCatalogModel): ModelRole {
    if ((model.category as string) === "embedding")
        return "embedding";
    return "chat";
}
...
function sanitizeAssignments(assignments: Assignments): Assignments {
    ...
    for (const [slot, modelId] of Object.entries(assignments) as Array<[
        ...
        if (!modelId || !ASSIGNMENT_SLOTS.has(slot))
            continue;
...
const mobileLookup: http.RequestOptions["lookup"] = (hostname, options, callback) => {
    mobileDnsResolver.resolve4(hostname, (error, addresses) => {
        ...
        if (options.all) {
            callback(null, addresses.map((address) => ({ address, family: 4 })), undefined as never);
            return;
        }
 ...
 */
export function reauthorizeRedirectHeaders(headers: Record<string, string>, nextUrl: string): Record<string, string> {
...
async function openDownloadResponse(url: string, headers: Record<string, string>, signal: AbortSignal, redirectCount = 0): Promise<http.IncomingMessage> {
    if (redirectCount > 5) {
    ...
    return new Promise((resolve, reject) => {
        const req = transport.get(parsed, {
            ...
            if (location && [301, 302, 303, 307, 308].includes(statusCode)) {
                ...
                resolve(openDownloadResponse(nextUrl, reauthorizeRedirectHeaders(headers, nextUrl), signal, redirectCount + 1));
                return;
            }
            resolve(response);
        });
...
async function withConfigFileLock<T>(filePath: string, task: () => Promise<T>): Promise<T> {
    const prior = configFileMutex.get(filePath) ?? Promise.resolve();
    const next = prior.then(task, task);
    configFileMutex.set(filePath, next.then(() => undefined, () => undefined));
    return next;
}
...
async function writeRegistry(models: InstalledModel[]): Promise<void> {
    await writeJsonFile(registryPath(), {
        ...
        models: models.map((model) => {
            ...
            if (!storedPath) {
                throw new Error("[local-inference] installed model path must live under the local-inference root");
            }
...
async function writeAssignments(assignments: Assignments): Promise<Assignments> {
...
async function updateAssignments(mutate: (assignments: Assignments) => void): Promise<Assignments> {
...
async function assignModel(model: CatalogModel, overwrite: boolean): Promise<void> {
...
async function downloadModel(model: CatalogModel, record: DownloadJob): Promise<void> {
    ...
    try {
        ...
        const response = await openDownloadResponse(downloadUrl, headers, abortController.signal);
        const statusCode = response.statusCode ?? 0;
...
async function isLocalInferenceRouted(): Promise<boolean | undefined> {
    const config = await readJsonFile<Record<string, unknown> | null>(path.join(stateDir(), "eliza.json"), null);
    if (!config)
        return undefined;
...
export async function getLocalInferenceActiveSnapshot(): Promise<{
    ...
    const withRouted = <T extends object>(state: T): T & {
        routed?: boolean;
    ...
    if (aospActive?.status === "ready" &&
        ...
        const installed = (await installedSnapshot()).find((model) => model.path === aospActive.path);
        return withRouted({
            modelId: installed?.id ?? path.basename(aospActive.path).replace(/\.gguf$/i, ""),
            loadedAt: typeof aospActive.loadedAt === "string" ? aospActive.loadedAt : null,
    ...
    if (!loadedPath)
        return withRouted(activeModelState);
...
function recommendedChatModel(): CatalogModel | null {
    ...
    const candidates = chatModels()
        ...
        .sort((left, right) => {
        const rightSize = typeof right.sizeGb === "number" && Number.isFinite(right.sizeGb)
            ? right.sizeGb
            : 0;
        const leftSize = typeof left.sizeGb === "number" && Number.isFinite(left.sizeGb)
            ? left.sizeGb
            : 0;
    ...
    return (candidates[0] ??
        chatModels().sort((a, b) => {
            const aSize = typeof a.sizeGb === "number" && Number.isFinite(a.sizeGb)
                ? a.sizeGb
                : 0;
            const bSize = typeof b.sizeGb === "number" && Number.isFinite(b.sizeGb)
                ? b.sizeGb
                : 0;
...
function progressForJob(job: DownloadJob): LocalInferenceChatMetadata["progress"] {
...
function progressText(progress: LocalInferenceChatMetadata["progress"] | undefined): string {
    if (!progress)
        return "";
...
function buildLocalInferenceChatResult(metadata: LocalInferenceChatMetadata, detail?: string): LocalInferenceChatResult {
...
async function resolveDefaultChatModel(prompt: string): Promise<CatalogModel | null> {
...
async function activateInstalledModel(installed: InstalledModel): Promise<LocalInferenceChatResult> {
    activeModelState = {
...
export async function getLocalInferenceChatStatus(intent: LocalInferenceCommandIntent = "status", error?: unknown): Promise<LocalInferenceChatResult> {
    ...
    if (active.status === "ready" && active.modelId) {
        const provider = (await localInferenceServiceLazy()).getActive().status === "ready"
...
export async function handleLocalInferenceChatCommand(intent: LocalInferenceCommandIntent, prompt: string): Promise<LocalInferenceChatResult> {
    if (intent === "status") {
...
export async function applyLocalInferenceManagementMutation(input: LocalInferenceManagementInput): Promise<LocalInferenceManagementResult> {
    switch (input.op) {
        case "start_download": {
        ...
        case "set_active": {
            const modelId = requireModelId(input.modelId, input.op);
            const installed = (await installedSnapshot()).find((model) => model.id === modelId);
            if (!installed)
            ...
            if (result.localInference.status === "failed") {
                throw new Error(result.localInference.error ?? "Failed to activate model");
            }
            return { op: input.op, modelId, active: activeModelState };
        }
        case "clear_active": {
            if (shouldUseAospLocalInference()) {
                const { clearAospLocalInferenceModel } = await getAospLocalInferenceApi();
                activeModelState = await clearAospLocalInferenceModel();
                return { op: input.op, active: activeModelState };
            }
            try {
                const { unloadMobileDeviceBridgeModel } = await getMobileDeviceBridgeApi();
                await unloadMobileDeviceBridgeModel();
            }
            catch (error) {
                // Clearing chat routing should still reset our state in headless tests.
                logger.debug(`[local-inference] clear_active ignored bridge unload failure: ${error instanceof Error ? error.message : String(error)}`);
            }
        ...
        case "set_voice_model_preferences": {
            const { applyVoiceModelManagementMutation } = await import("./routes/voice-models-routes.js");
        ...
        case "set_policy": {
            ...
            if (rawPolicy !== null && !isRoutingPolicy(rawPolicy)) {
                throw new Error(`Routing policy must be one of ${ROUTING_POLICIES.join(", ")}`);
            }
        ...
        case "set_preferred_provider": {
            ...
            const preferences = await setPreferredProvider(slot, input.provider?.trim() || null);
            return {
                op: input.op,
                slot,
        ...
        case "set_assignment": {
            ...
            if (modelId && !isCuratedCatalogModelId(modelId)) {
                throw new Error("Local inference assignments are limited to curated Eliza-1 tiers.");
            }
...
function decodeLocalInferencePathId(raw: string, res: http.ServerResponse, fieldName: string): string | null {
    try {
...
export async function handleLocalInferenceRoutes(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {
    ...
    if (pathname.startsWith("/api/voice/first-run/")) {
        const { handleVoiceFirstRunRoutes } = await import("./routes/voice-first-run-routes.js");
        if (await handleVoiceFirstRunRoutes(req, res))
            return true;
    }
    if (pathname === "/api/voice/native-pcm-turn") {
        const { handleNativePcmTurnRoute } = await import("./routes/native-pcm-turn-route.js");
        if (await handleNativePcmTurnRoute(req, res))
            return true;
    }
    // Family-member capture route lives under /v1/voice/first-run/family-member.
    if (pathname === "/v1/voice/first-run/family-member") {
        const { handleFamilyMemberRoute } = await import("./routes/family-member-route.js");
        if (await handleFamilyMemberRoute(req, res))
            return true;
    }
    ...
    if (pathname.startsWith("/v1/voice/speaker-profiles")) {
        const { handleVoiceSpeakerProfileRoutes } = await import("./routes/voice-speaker-profile-routes.js");
        if (await handleVoiceSpeakerProfileRoutes(req, res))
            return true;
    }
    ...
    if (pathname.startsWith("/api/local-inference/voice-models")) {
        const { handleVoiceModelsRoutes } = await import("./routes/voice-models-routes.js");
        if (await handleVoiceModelsRoutes(req, res))
            return true;
    }
    if (method === "GET" &&
    ...
    if (method === "GET" && pathname === "/api/local-inference/device-tier") {
        sendJson(res, {
            tier: classifyDeviceTier(await (await localInferenceServiceLazy()).getHardware()),
        });
        return true;
    }
    ...
    if (method === "GET" && pathname === "/api/local-inference/providers") {
        ...
        const serving = await getMobileDeviceBridgeApi()
            ...
            .catch((): MobileDeviceBridgeServingStatus => ({
    ...
    if (method === "POST" && pathname === "/api/local-inference/assignments") {
        ...
        const modelId = typeof body.modelId === "string" && body.modelId.trim()
            ...
            : null;
        if (modelId && !isCuratedCatalogModelId(modelId)) {
            sendJsonError(res, "Local inference assignments are limited to curated Eliza-1 tiers.", 400);
            return true;
        }
    ...
    if (method === "POST" && pathname === "/api/local-inference/routing/text") {
        ...
        if (!isRoutingPolicy(policy)) {
            sendJsonError(res, `policy must be one of ${ROUTING_POLICIES.join(", ")}`, 400);
            return true;
        }
    ...
    if (method === "POST" &&
        ...
        if (pathname.endsWith("/preferred")) {
            preferences = await setPreferredProvider(slot, typeof body.provider === "string" && body.provider.trim()
                ? body.provider.trim()
                : null);
        }
            ...
            sendJsonError(res, `policy must be one of ${ROUTING_POLICIES.join(", ")} or null`, 400);
            return true;
        }
        sendJson(res, { preferences });
        return true;
    ...
    if (method === "DELETE" && downloadMatch) {
        const modelId = decodeLocalInferencePathId(downloadMatch[1] ?? "", res, "model id");
        if (modelId === null)
            return true;
        activeDownloads.get(modelId)?.abortController.abort();
        activeDownloads.delete(modelId);
    ...
    if (method === "POST" && pathname === "/api/local-inference/active") {
        ...
        const { assertManifestEvalsPassed, CandidateModelActivationError } = await import("./services/active-model.js");
        try {
        ...
        try {
            ...
            if (shouldUseAospLocalInference()) {
                const { activateAospLocalInferenceModel, buildAospLoadModelArgs } = await getAospLocalInferenceApi();
    ...
    if (method === "POST" && verifyMatch) {
        const id = decodeLocalInferencePathId(verifyMatch[1] ?? "", res, "model id");
        if (id === null)
            return true;
    ...
    if (method === "DELETE" && installedMatch) {
        const id = decodeLocalInferencePathId(installedMatch[1] ?? "", res, "model id");
        if (id === null)
            return true;
