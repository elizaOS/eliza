...
const LEGACY_TTS_SPEAKER_EMBEDDING_ENV =
	"LOCAL_AI_LEGACY_TTS_SPEAKER_EMBEDDING_URL";
...
function isTextToAudioOutput(value: unknown): value is TextToAudioOutput {
	...
	return (
		...
		typeof output.sampling_rate === "number"
	);
}
	...
	private async initialize(): Promise<void> {
		...
		this.initializingPromise = (async () => {
			try {
				...
				const speakerEmbeddingUrl =
					process.env[LEGACY_TTS_SPEAKER_EMBEDDING_ENV]?.trim();
	...
	public async generateSpeech(text: string): Promise<Readable> {
		try {
			...
			logger.info(
				...
				"Audio data converted to 16-bit PCM Buffer:",
			);
