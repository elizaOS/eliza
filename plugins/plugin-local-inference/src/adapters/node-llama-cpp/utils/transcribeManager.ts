	...
	private async preprocessAudio(audioBuffer: Buffer): Promise<string> {
		...
		try {
			...
			const tempInputFile = path.join(
				this.cacheDir,
				`temp_input_${Date.now()}${extension}`,
			);
				...
				try {
					...
					if (
						...
						(stream?.bits_per_raw_sample === 16 ||
							stream?.bits_per_raw_sample === undefined)
					) {
		...
		try {
			...
			try {
				...
				if (
					...
					logger.error(
						"Whisper model not found. Please run: npx whisper-node download",
					);
					throw new Error(
			...
			if (!segments || !Array.isArray(segments)) {
				logger.warn(
					"Whisper returned no segments (likely silence or very short audio)",
				);
				return { text: "" };
			}
