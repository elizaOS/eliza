...
function createTextToSpeechHandler() {
    return async (runtime, params) => {
        ...
        if (typeof service.textToSpeech === "function") {
            return normalizeAudioBytes(await service.textToSpeech({ text, ...(signal ? { signal } : {}) }));
        }
...
function createImageDescriptionHandler() {
    return async (runtime, params) => {
        ...
        if (arbiter?.requestVisionDescribe) {
            ...
            const result = await arbiter.requestVisionDescribe({ modelKey, payload: request });
...
function createImageGenerationHandler() {
    return async (runtime, params) => {
        ...
        for (let i = 0; i < count; i += 1) {
            const seeded = typeof request.seed === "number" && i > 0
                ? { ...request, seed: request.seed + i }
                : request;
            const result = await arbiter.requestImageGen({ modelKey, payload: seeded });
