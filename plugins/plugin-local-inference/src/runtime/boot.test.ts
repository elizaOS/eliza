...
vi.mock("./ensure-local-inference-handler", () => ({
    ensureLocalInferenceHandler: (r: AgentRuntime) => ensureLocalInferenceHandler(r),
}));
vi.mock("./mobile-local-inference-gate", () => ({
    shouldEnableMobileLocalInference: () => shouldEnableMobileLocalInference(),
    warnIfMobileGateActiveWithoutPlatform: (_args: unknown) => warnIfMobileGateActiveWithoutPlatform(),
}));
