...
import { buildPiiScrubPrompt } from "./pii/scrub-handler.js";
...
import { manageTranscriptPrivacyAction } from "./actions/transcript-permissioning.js";
import { mergeElizaTurnStopSequences } from "./services/eliza-turn-stops.js";
import { parseScrubCompletion } from "./pii/scrub-handler.js";
...
import { redactTranscriptAction } from "./actions/transcript-permissioning.js";
import { resolveBackgroundInferenceBudget } from "@elizaos/core";
import { shareTranscriptAction } from "./actions/transcript-permissioning.js";
import { startTranscriptionAction } from "./actions/transcription-control.js";
import { stopTranscriptionAction } from "./actions/transcription-control.js";
import { transcriptsRoutes } from "./routes/transcripts-routes.js";
import { type AudioStreamResult } from "@elizaos/core";
...
import { type ImageGenerationResult } from "@elizaos/core";
import { type PiiScrubParams } from "@elizaos/core";
import { type PiiScrubResult } from "@elizaos/core";
...
export type LocalInferenceUnavailableReason = "backend_unavailable" | "capability_unavailable" | "invalid_input" | "invalid_output";
export class LocalInferenceUnavailableError extends Error {
    ...
    constructor(readonly modelType: string, readonly reason: LocalInferenceUnavailableReason, message: string, options?: {
        cause?: unknown;
    }) {
...
export function isLocalInferenceUnavailableError(error: unknown): error is LocalInferenceUnavailableError {
    return (error instanceof LocalInferenceUnavailableError ||
        (typeof error === "object" &&
            error !== null &&
            (error as {
                code?: unknown;
...
interface LocalInferenceTextToSpeechService {
    synthesizeSpeech?: (text: string, signal?: AbortSignal, voiceId?: string) => Promise<Uint8Array | ArrayBuffer | Buffer>;
    textToSpeech?: (args: {
        text: string;
        voice?: string;
        signal?: AbortSignal;
     ...
     */
    synthesizeSpeechStream?: (text: string, signal?: AbortSignal, voiceId?: string) => AsyncIterable<Uint8Array>;
}
interface LocalInferenceTranscriptionService {
    transcribe?: (params: unknown) => Promise<string | {
        text?: string;
    }>;
...
interface LocalInferenceRuntimeService extends LocalInferenceTextToSpeechService, LocalInferenceTranscriptionService, LocalInferenceArbiterAccessor {
    generate?: (args: LocalInferenceGenerateArgs) => Promise<string>;
    embed?: (args: {
        input: string;
    }) => Promise<number[] | LocalInferenceEmbedResult>;
    describeImage?: (params: ImageDescriptionParams | string) => Promise<ImageDescriptionResult | string>;
    imageDescription?: (params: ImageDescriptionParams | string) => Promise<ImageDescriptionResult | string>;
}
type RuntimeWithServices = IAgentRuntime & {
    getService?: (name: string) => unknown;
};
function serviceFromRuntime(runtime: IAgentRuntime): LocalInferenceRuntimeService | null {
    const withServices = runtime as RuntimeWithServices;
    if (typeof withServices.getService !== "function")
        return null;
    for (const name of [
...
function unavailable(modelType: string, reason: LocalInferenceUnavailableReason, message: string, cause?: unknown): LocalInferenceUnavailableError {
    return new LocalInferenceUnavailableError(modelType, reason, message, {
        cause,
    });
}
function requireService(runtime: IAgentRuntime, modelType: string): LocalInferenceRuntimeService {
...
function promptFromParams(params: GenerateTextParams): string {
    ...
    if (typeof prompt !== "string" || prompt.trim().length === 0) {
        throw unavailable(ModelType.TEXT_SMALL, "invalid_input", "[local-inference] TEXT generation requires a non-empty prompt");
    }
    return prompt;
}
function textGenerationArgsFromParams(params: GenerateTextParams): LocalInferenceGenerateArgs {
    return {
        ...
        onTextChunk: (params.stream === true || params.streamStructured === true) &&
...
function extractEmbeddingText(params: TextEmbeddingParams | string | null): string {
    ...
    throw unavailable(ModelType.TEXT_EMBEDDING, "invalid_input", "[local-inference] TEXT_EMBEDDING requires { text } or a non-empty string; null warmup probes are not served with fake vectors");
}
function extractSpeechText(params: TextToSpeechParams | string): string {
    ...
    throw unavailable(ModelType.TEXT_TO_SPEECH, "invalid_input", "[local-inference] TEXT_TO_SPEECH requires a string or { text } input");
}
function extractSpeechSignal(params: TextToSpeechParams | string): AbortSignal | undefined {
...
function ensureNonEmptyText(modelType: string, text: string): string {
    ...
    if (!trimmed) {
        throw unavailable(modelType, "invalid_input", `[local-inference] ${modelType} requires non-empty text`);
    }
    return trimmed;
}
function normalizeEmbeddingResult(result: number[] | LocalInferenceEmbedResult): number[] {
    const embedding = Array.isArray(result) ? result : result.embedding;
    if (!Array.isArray(embedding) ||
        embedding.some((value) => typeof value !== "number")) {
        throw unavailable(ModelType.TEXT_EMBEDDING, "invalid_output", "[local-inference] TEXT_EMBEDDING backend returned an invalid embedding");
    }
    return embedding;
}
function normalizeAudioBytes(result: Uint8Array | ArrayBuffer | Buffer): Uint8Array {
    ...
    throw unavailable(ModelType.TEXT_TO_SPEECH, "invalid_output", "[local-inference] TEXT_TO_SPEECH backend returned non-audio output");
}
...
function bufferedAudioStreamResult(bytes: Uint8Array, mimeType: string): AudioStreamResult {
...
function streamingAudioStreamResult(source: AsyncIterable<Uint8Array>, mimeType: string): AudioStreamResult {
...
function extractPcmTranscriptionParams(params: TranscriptionParams | Buffer | string | unknown): {
    pcm: Float32Array;
    sampleRate: number;
    signal?: AbortSignal;
} {
    if (!params || typeof params !== "object" || params instanceof Uint8Array) {
        throw unavailable(ModelType.TRANSCRIPTION, "invalid_input", "[local-inference] TRANSCRIPTION requires { pcm, sampleRateHz } when only transcribePcm is available");
    }
    ...
    if (!(record.pcm instanceof Float32Array)) {
        throw unavailable(ModelType.TRANSCRIPTION, "invalid_input", "[local-inference] TRANSCRIPTION requires Float32Array pcm when only transcribePcm is available");
    }
    const sampleRate = typeof record.sampleRateHz === "number"
        ? record.sampleRateHz
        : typeof record.sampleRate === "number"
            ? record.sampleRate
            : 0;
    if (!Number.isFinite(sampleRate) || sampleRate <= 0) {
        throw unavailable(ModelType.TRANSCRIPTION, "invalid_input", "[local-inference] TRANSCRIPTION { pcm } requires a positive sampleRateHz");
    }
...
function normalizeTranscript(result: string | {
    text?: string;
}): string {
    const text = typeof result === "string" ? result : result.text;
    if (typeof text !== "string") {
        throw unavailable(ModelType.TRANSCRIPTION, "invalid_output", "[local-inference] TRANSCRIPTION backend returned an invalid transcript");
    }
    return text;
}
function normalizeImageDescription(result: ImageDescriptionResult | string): ImageDescriptionResult {
    ...
    if (result &&
        ...
        return {
            ...
            description: ensureNonEmptyText(ModelType.IMAGE_DESCRIPTION, result.description),
        };
    }
    throw unavailable(ModelType.IMAGE_DESCRIPTION, "invalid_output", "[local-inference] IMAGE_DESCRIPTION backend returned an invalid description");
}
function createTextHandler(modelType: string) {
    return async (runtime: IAgentRuntime, params: GenerateTextParams): Promise<string> => {
        const service = requireService(runtime, modelType);
        const generate = service.generate;
        if (typeof generate !== "function") {
            throw unavailable(modelType, "capability_unavailable", `[local-inference] Active local backend does not implement ${modelType} generation`);
        }
        ...
        const configuredModel = runtime.getSetting?.(modelType === ModelType.TEXT_SMALL
            ? "LOCAL_SMALL_MODEL"
            : "LOCAL_LARGE_MODEL");
...
function createPiiScrubHandler() {
    return async (runtime: IAgentRuntime, params: PiiScrubParams): Promise<PiiScrubResult> => {
        const service = requireService(runtime, ModelType.PII_SCRUB);
        const generate = service.generate;
        if (typeof generate !== "function") {
            throw unavailable(ModelType.PII_SCRUB, "capability_unavailable", "[local-inference] Active local backend does not implement text generation for PII_SCRUB");
        }
        ...
        const modelId = typeof tier === "string" && tier.trim().length > 0
...
function createEmbeddingHandler() {
    return async (runtime: IAgentRuntime, params: TextEmbeddingParams | string | null): Promise<number[]> => {
        const service = serviceFromRuntime(runtime);
        if (!service) {
            throw unavailable(ModelType.TEXT_EMBEDDING, "backend_unavailable", "[local-inference] TEXT_EMBEDDING requires an active Eliza-1 backend or another embedding provider; refusing to synthesize zero-vectors.");
        }
        if (typeof service.embed !== "function") {
            throw unavailable(ModelType.TEXT_EMBEDDING, "capability_unavailable", "[local-inference] Active local backend does not implement TEXT_EMBEDDING");
        }
        const input = ensureNonEmptyText(ModelType.TEXT_EMBEDDING, extractEmbeddingText(params));
        return normalizeEmbeddingResult(await service.embed({ input }));
    };
}
function createTextToSpeechHandler() {
    return async (runtime: IAgentRuntime, params: TextToSpeechParams | string): Promise<Uint8Array | AudioStreamResult> => {
        const service = requireService(runtime, ModelType.TEXT_TO_SPEECH);
        const text = ensureNonEmptyText(ModelType.TEXT_TO_SPEECH, extractSpeechText(params));
        ...
        const wantsStream = typeof params === "object" &&
            params !== null &&
            (params as {
                audioStream?: boolean;
            }).audioStream === true;
        ...
        if (wantsStream && typeof service.synthesizeSpeechStream === "function") {
            return streamingAudioStreamResult(service.synthesizeSpeechStream(text, signal, voice), LOCAL_TTS_MIME);
        }
        const synthesizeBuffered = async (): Promise<Uint8Array> => {
            if (typeof service.synthesizeSpeech === "function") {
                return normalizeAudioBytes(await service.synthesizeSpeech(text, signal, voice));
            }
            if (typeof service.textToSpeech === "function") {
                return normalizeAudioBytes(await service.textToSpeech({
                    text,
                    ...(voice ? { voice } : {}),
                    ...(signal ? { signal } : {}),
                }));
            }
            throw unavailable(ModelType.TEXT_TO_SPEECH, "capability_unavailable", "[local-inference] Active local backend does not implement TEXT_TO_SPEECH");
        };
...
function createTranscriptionHandler() {
    return async (runtime: IAgentRuntime, params: TranscriptionParams | Buffer | string | unknown): Promise<string> => {
        ...
        if (typeof service.transcribePcm === "function") {
            ...
            const transcript = normalizeTranscript(await (signal
                ? service.transcribePcm(pcmParams, signal)
                : service.transcribePcm(pcmParams)));
            throwIfAborted(signal);
            return transcript;
        }
        throw unavailable(ModelType.TRANSCRIPTION, "capability_unavailable", "[local-inference] Active local backend does not implement TRANSCRIPTION");
    };
}
/**
...
function tryGetArbiter(service: LocalInferenceRuntimeService | null): ArbiterLike | null {
    ...
    if (typeof cand.hasCapability === "function" &&
...
function tryGetImageGenArbiter(service: LocalInferenceRuntimeService | null): ArbiterLike | null {
    if (!service?.getMemoryArbiter)
        return null;
    const arbiter = service.getMemoryArbiter();
    if (!arbiter || typeof arbiter !== "object")
...
function paramsToVisionRequest(params: ImageDescriptionParams | string): {
    ...
    if (typeof url !== "string" || !url) {
        throw unavailable(ModelType.IMAGE_DESCRIPTION, "invalid_input", "[local-inference] IMAGE_DESCRIPTION requires a non-empty imageUrl");
    }
    const prompt = typeof params === "object" ? params.prompt : undefined;
    const signal = typeof params === "object"
        ? (params as {
            signal?: AbortSignal;
        }).signal
        : undefined;
    ...
    const streamSink = wantsStream && typeof params === "object"
        ? (params as {
            onStreamChunk?: (chunk: string) => void | Promise<void>;
        })
            .onStreamChunk
        : undefined;
    const onTextChunk = typeof streamSink === "function"
...
function paramsWithPreparedVisionImage(params: ImageDescriptionParams | string, image: VisionImageInput): ImageDescriptionParams | string {
    if (image.kind !== "bytes") {
        return params;
    }
...
function createImageDescriptionHandler() {
    return async (runtime: IAgentRuntime, params: ImageDescriptionParams | string): Promise<ImageDescriptionResult> => {
        ...
        if (arbiter?.requestVisionDescribe) {
            ...
            const modelKeyCandidate = typeof params === "object"
                ? (params as {
                    modelKey?: unknown;
                }).modelKey
                : undefined;
            const modelKey = typeof modelKeyCandidate === "string" && modelKeyCandidate
                ? modelKeyCandidate
                : "gemma-vl";
            await augmentVisionRequest(request);
            const result = await arbiter.requestVisionDescribe<typeof request, ImageDescriptionResult | string>({ modelKey, payload: request });
        ...
        if (typeof service.describeImage === "function") {
            return normalizeImageDescription(await service.describeImage(preparedParams));
        }
        if (typeof service.imageDescription === "function") {
            return normalizeImageDescription(await service.imageDescription(preparedParams));
        }
        throw unavailable(ModelType.IMAGE_DESCRIPTION, "capability_unavailable", "[local-inference] Active local backend does not implement IMAGE_DESCRIPTION");
    };
}
/**
...
function paramsToImageGenRequest(params: ImageGenerationParams): ProviderImageGenRequest {
    if (typeof params.prompt !== "string" || !params.prompt.trim()) {
        throw unavailable(ModelType.IMAGE, "invalid_input", "[local-inference] IMAGE requires a non-empty prompt");
    }
    ...
    if (typeof extended.guidanceScale === "number" &&
...
function imageGenResultToUrls(result: ProviderImageGenResult): ImageGenerationResult[] {
    if (!(result.image instanceof Uint8Array) || result.image.length === 0) {
        throw unavailable(ModelType.IMAGE, "invalid_output", "[local-inference] IMAGE backend returned an empty image buffer");
    }
...
function createImageGenerationHandler() {
    return async (runtime: IAgentRuntime, params: ImageGenerationParams): Promise<ImageGenerationResult[]> => {
        ...
        if (!arbiter?.requestImageGen) {
            throw unavailable(ModelType.IMAGE, "capability_unavailable", "[local-inference] IMAGE generation requires the WS3 arbiter image-gen capability. Register it via createImageGenCapabilityRegistration at plugin init.");
        }
        ...
        const modelKeyCandidate = (params as ImageGenerationParams & {
            modelKey?: unknown;
        }).modelKey;
        const modelKey = typeof modelKeyCandidate === "string" && modelKeyCandidate
            ? modelKeyCandidate
        ...
        for (let i = 0; i < count; i += 1) {
            const seeded: ProviderImageGenRequest = typeof request.seed === "number" && i > 0
                ? { ...request, seed: request.seed + i }
                : request;
            const result = await arbiter.requestImageGen<ProviderImageGenRequest, ProviderImageGenResult>({ modelKey, payload: seeded });
...
export function createLocalInferenceModelHandlers(): NonNullable<Plugin["models"]> {
    return {
...
function createStaticPluginModelHandlers(): NonNullable<Plugin["models"]> {
    const { [ModelType.TEXT_EMBEDDING]: _embedding, ...handlers } = createLocalInferenceModelHandlers();
    return handlers;
}
export const localInferencePlugin: Plugin = {
    name: LOCAL_INFERENCE_PROVIDER_ID,
    description: "Eliza-1 local provider for text, embeddings, text-to-speech, and transcription.",
    ...
    async init(_config: unknown, runtime: IAgentRuntime) {
        ...
        const service = serviceFromRuntime(runtime);
        if (!service) {
            logger.info("[local-inference] Provider registered; no active backend service is exposed yet. Model calls will return LOCAL_INFERENCE_UNAVAILABLE until an Eliza-1 backend is activated.");
            return;
        }
        logger.info({
            generate: typeof service.generate === "function",
            embed: typeof service.embed === "function",
            textToSpeech: typeof service.synthesizeSpeech === "function" ||
                typeof service.textToSpeech === "function",
            imageDescription: typeof service.describeImage === "function" ||
                typeof service.imageDescription === "function",
            transcription: typeof service.transcribe === "function" ||
