...
import { getMobileDeviceBridgeStatus, loadMobileDeviceBridgeModel, unloadMobileDeviceBridgeModel, } from "@elizaos/plugin-capacitor-bridge";
import { buildHuggingFaceResolveUrl, MODEL_CATALOG as SHARED_MODEL_CATALOG, } from "@elizaos/shared";
import { LOCAL_INFERENCE_MODEL_TYPES, LOCAL_INFERENCE_PROVIDER_ID, LOCAL_INFERENCE_TEXT_MODEL_TYPES, } from "./provider.js";
import { assertManifestEvalsPassed, CandidateModelActivationError, } from "./services/active-model.js";
...
const CATALOG = SHARED_MODEL_CATALOG.map((model) => ({
    ...model,
...
function finalModelPath(model) {
    return path.join(modelsDir(), `${model.id.replace(/[^a-zA-Z0-9._-]/g, "_")}.gguf`);
}
function stagingPath(model) {
    return path.join(downloadsDir(), `${model.id.replace(/[^a-zA-Z0-9._-]/g, "_")}.part`);
}
...
const mobileLookup = (hostname, options, callback) => {
    mobileDnsResolver.resolve4(hostname, (error, addresses) => {
        ...
        if (options?.all) {
            callback(null, addresses.map((address) => ({ address, family: 4 })), undefined);
            return;
        }
    ...
    return new Promise((resolve, reject) => {
        const req = transport.get(parsed, {
            ...
            if (location && [301, 302, 303, 307, 308].includes(statusCode)) {
                response.resume();
                resolve(openDownloadResponse(new URL(location, parsed).toString(), headers, signal, redirectCount + 1));
                return;
            }
            resolve(response);
        });
...
async function readRegistry() {
    ...
    for (const model of models) {
        ...
        try {
            ...
            if (stat.isFile())
                installed.push({ ...model, sizeBytes: stat.size });
        }
        catch {
...
async function readAssignments() {
    const file = await readJsonFile(assignmentsPath(), {
        assignments: {},
    });
...
async function downloadModel(model, record) {
    ...
    try {
        ...
        const response = await openDownloadResponse(huggingFaceResolveUrl(model), headers, abortController.signal);
        const statusCode = response.statusCode ?? 0;
        ...
        for (const companionId of model.companionModelIds ?? []) {
            if (!activeDownloads.has(companionId)) {
                void startDownload(companionId).catch((error) => {
                    logger.warn(`[local-inference] Companion download failed for ${companionId}: ${error instanceof Error ? error.message : String(error)}`);
                });
            }
        }
        ...
        if (abortController.signal.aborted) {
            ...
            logger.warn(`[local-inference] Download failed for ${model.id}: ${record.error}`);
        }
        record.updatedAt = new Date().toISOString();
    }
    finally {
...
export async function getLocalInferenceActiveSnapshot() {
    const bridgeStatus = getMobileDeviceBridgeStatus();
    const loadedPath = bridgeStatus.devices.find((device) => Boolean(device.loadedPath))?.loadedPath;
    if (!loadedPath)
        return activeModelState;
    const installed = (await installedSnapshot()).find((model) => model.path === loadedPath);
    if (!installed)
        return activeModelState;
    const catalogModel = CATALOG.find((model) => model.id === installed.id);
    if (catalogModel?.role !== "chat")
        return activeModelState;
    return {
...
function isNoSpaceMessage(value) {
    const message = value instanceof Error
        ...
        : typeof value === "string"
            ? value
            : "";
    return /\b(?:enospc|no space left|disk full|not enough (?:disk )?space|insufficient storage)\b/i.test(message);
}
...
async function resolveDefaultChatModel(prompt) {
    ...
    const activeCatalog = active.modelId
        ? CATALOG.find((model) => model.id === active.modelId && model.role === "chat")
        : null;
    if (activeCatalog)
        return activeCatalog;
    const installedCatalog = installed
        .map((entry) => CATALOG.find((model) => model.id === entry.id && model.role === "chat"))
        .filter((model) => Boolean(model))
        .sort((a, b) => a.sizeGb - b.sizeGb)[0];
    return installedCatalog ?? recommendedChatModel();
}
async function setRoutingForChat(provider) {
    const current = await readJsonFile(routingPath(), defaultRoutingPreferences());
...
export async function getLocalInferenceChatStatus(intent = "status", error) {
    ...
    const installed = await installedSnapshot();
    const installedChat = installed.find((entry) => CATALOG.some((model) => model.id === entry.id && model.role === "chat"));
    if (installedChat) {
        return buildLocalInferenceChatResult({
            intent,
            status: "idle",
...
export async function handleLocalInferenceChatCommand(intent, prompt) {
    if (intent === "status") {
    ...
    if (intent === "use_local") {
        ...
        const installedModel = installed.find((entry) => entry.id === requested?.id);
        if (installedModel) {
    ...
    if (intent === "switch_smaller") {
        ...
        const smallerInstalled = installed
            .map((entry) => ({
            entry,
            catalog: CATALOG.find((model) => model.id === entry.id && model.role === "chat"),
        }))
            .filter((entry) => {
...
export async function handleLocalInferenceRoutes(req, res) {
    ...
    if (pathname.startsWith("/api/voice/onboarding/")) {
        const { handleVoiceOnboardingRoutes } = await import("./routes/voice-onboarding-routes.js");
        if (await handleVoiceOnboardingRoutes(req, res))
            return true;
    }
        ...
        sendJson(res, {
            providers: [
                ...
                },
                {
                    ...
                    registeredSlots: installed.length > 0 ? LOCAL_INFERENCE_MODEL_TYPES : [],
                },
            ],
        });
        return true;
    ...
    if (method === "GET" && pathname === "/api/local-inference/routing") {
        const preferences = await readJsonFile(routingPath(), defaultRoutingPreferences());
        sendJson(res, {
    ...
    if (method === "POST" && pathname === "/api/local-inference/downloads") {
        ...
        try {
            ...
            sendJsonError(res, error instanceof Error ? error.message : "Failed to start download", 400);
        }
        return true;
    }
    const downloadMatch = /^\/api\/local-inference\/downloads\/([^/]+)$/.exec(pathname);
    ...
    if (method === "POST" && pathname === "/api/local-inference/active") {
        ...
        const installed = (await installedSnapshot()).find((model) => model.id === body.modelId);
        if (!installed) {
        ...
        if (catalog)
            await assignModel(catalog, true);
        try {
            activeModelState = {
                modelId: installed.id,
                loadedAt: null,
            ...
            sendJsonError(res, error instanceof Error ? error.message : "Failed to load model", 503);
        }
        return true;
    }
    if (method === "DELETE" && pathname === "/api/local-inference/active") {
        try {
            ...
            sendJsonError(res, error instanceof Error ? error.message : "Failed to unload model", 503);
        }
        return true;
    }
    ...
    if (method === "POST" && verifyMatch) {
        ...
        const installed = (await installedSnapshot()).find((model) => model.id === id);
        if (!installed) {
    ...
    const installedMatch = /^\/api\/local-inference\/installed\/([^/]+)$/.exec(pathname);
...
//# sourceMappingURL=local-inference-routes.js.map