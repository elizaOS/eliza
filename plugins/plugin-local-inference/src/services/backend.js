...
export function decideBackend(input) {
    ...
    const unsatisfiedKernels = computeUnsatisfiedKernels(kernels, input.binaryKernels ?? null);
...
export function resolveCatalogForPlan(plan) {
    if (plan.catalog)
        return plan.catalog;
    if (plan.modelId)
        return findCatalogModel(plan.modelId);
    return undefined;
}
/**
...
export class BackendDispatcher {
    ...
    active = null;
    constructor(nodeLlamaCpp, llamaServer, probeLlamaServerAvailable, probeDflashRequired, 
    /**
    ...
    async load(plan) {
        ...
        if (decision.unsatisfiedKernels && decision.unsatisfiedKernels.length > 0) {
            ...
            if (localAllowStockKv()) {
                ...
                // loudly exactly once.
                warnReducedOptimizationLocalMode(`catalog model requires kernel(s) {${missing}}, not advertised by the installed llama-server binary`);
                if (plan.overrides &&
                ...
                throw new Error(`[local-inference] Catalog model requires kernel(s) {${missing}}, but the installed llama-server binary does not advertise them in CAPABILITIES.json. Rebuild the fork with the matching backend (e.g. node packages/app-core/scripts/build-llama-cpp-dflash.mjs --target <triple>), pick a different model, or set ELIZA_LOCAL_ALLOW_STOCK_KV=1 to load with stock f16 KV (reduced-optimization local mode — loud warning, not publishable).`);
            }
        }
        const target = decision.backend === "llama-server"
            ? this.llamaServer
            : this.nodeLlamaCpp;
    ...
    async generate(args) {
        if (!this.active) {
            throw new Error("[local-inference] No backend loaded. Call load() before generate().");
        }
        return this.active.generate(args);
    }
    async embed(args) {
        if (!this.active) {
            throw new Error("[local-inference] No backend loaded. Call load() before embed().");
        }
        if (!this.active.embed) {
            throw new Error(`[local-inference] Active backend (${this.active.id}) does not implement embed.`);
        }
