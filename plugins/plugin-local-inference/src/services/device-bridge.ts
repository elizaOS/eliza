...
import { computeGenerationThroughput } from "@elizaos/plugin-native-inference/model-catalog/throughput";
import { localInferenceRoot } from "./paths";
import { logger } from "@elizaos/core";
import { randomUUID } from "node:crypto";
import { timingSafeEqual } from "node:crypto";
import { type Duplex } from "node:stream";
import { type GenerationThroughput } from "@elizaos/plugin-native-inference/model-catalog/throughput";
 ...
 */
function scoreDevice(device: ConnectedDevice, opts: {
...
export class DeviceBridge {
    ...
    private readonly statusListeners = new Set<(status: DeviceBridgeStatus) => void>();
    private readonly generationMetricsListeners = new Set<(metrics: DeviceGenerationMetrics) => void>();
    ...
    private readonly expectedPairingToken: string | null = process.env.ELIZA_DEVICE_PAIRING_TOKEN?.trim() || null;
    ...
    private handleConnection(socket: MinimalWebSocket, WsCtor: WsConstructor, url: URL): void {
    ...
    private onDeviceRegistered(socket: MinimalWebSocket, WsCtor: WsConstructor, registration: DeviceRegistration): void {
    ...
    async unloadModel(): Promise<void> {
        ...
        if (targets.length === 0)
            return;
        await Promise.allSettled(targets.map((device) => new Promise<void>((resolve, reject) => {
            const correlationId = randomUUID();
            const timeout = setTimeout(() => {
    ...
    async embed(args: {
        ...
        const timeoutMs = resolveDeviceTimeoutMs("ELIZA_DEVICE_GENERATE_TIMEOUT_MS", DEFAULT_CALL_TIMEOUT_MS);
        const correlationId = randomUUID();
 ...
 */
export function buildDeviceResourceMetricsDevPayload(bridge: DeviceBridge = deviceBridge, limit = 50): DeviceResourceMetricsDevPayload {
    return {
