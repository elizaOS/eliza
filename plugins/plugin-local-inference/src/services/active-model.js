...
import { assessRamFit, defaultManifestLoader, pickFittingContextVariant, } from "./ram-budget";
import { recommendForFirstRun } from "./recommendation";
import { listInstalledModels, touchElizaModel } from "./registry";
import { assessVoiceBundleFits, VOICE_ENSEMBLE_BUDGETS, } from "./voice/voice-budget";
...
export async function resolveLocalInferenceLoadArgs(installed, overrides) {
    ...
    if (dflash) {
        ...
        const drafter = installedModels.find((model) => model.id === dflash.drafterModelId);
        if (drafter)
            args.draftModelPath = drafter.path;
    }
...
export function assertModelFitsHost(installed, hostRamMb, options = {}) {
    const catalog = findCatalogModel(installed.id);
    if (!catalog)
        return { level: "fits", minMb: 0, recommendedMb: 0 };
...
export class VoiceBundleDoesNotFitError extends Error {
    ...
    hostRamMb;
    constructor(args) {
        super(`[local-inference] The voice bundle for tier "${args.tierSlot}" needs ~${args.requiredSteadyStateMb} MB steady-state (+~${args.requiredPeakMb - args.requiredSteadyStateMb} MB transient TTS peak) but only ~${args.usableMb} MB are usable on this host (${args.hostRamMb} MB total, after the OS/runtime headroom reserve). Refusing to start local voice; the runtime should fall back to cloud TTS+ASR or refuse the user-facing action.`);
...
export class CandidateModelActivationError extends Error {
    ...
    failedEvals;
    constructor(args) {
        const evalSuffix = args.failedEvals.length > 0
            ? ` Failed evals: ${args.failedEvals.join(", ")}.`
            : "";
        super(`Model "${args.modelId}" is candidate-only — its manifest (version ${args.manifestVersion}) reports evals.textEval.passed=false. Refusing to activate.${evalSuffix} Wait for the publisher to flip the manifest off candidate/weights-staged and re-fetch the bundle.`);
 ...
 * silently otherwise.
 */
export function assertManifestEvalsPassed(installed, manifestLoader = defaultManifestLoader) {
    const manifest = manifestLoader(installed.id, installed);
    if (!manifest)
        return;
    if (manifest.evals.textEval.passed === true)
        return;
...
function collectFailedEvalNames(manifest) {
    ...
    if (evals.textEval.passed !== true)
        failed.push("textEval");
    if (evals.voiceRtf.passed !== true)
        failed.push("voiceRtf");
    if (evals.e2eLoopOk !== true)
        failed.push("e2eLoopOk");
    if (evals.thirtyTurnOk !== true)
        failed.push("thirtyTurnOk");
    if (evals.asrWer && evals.asrWer.passed !== true)
        failed.push("asrWer");
    ...
    if (evals.dflash && evals.dflash.passed !== true)
        failed.push("dflash");
    ...
    async switchTo(runtime, installed, overrides, opts = {}) {
        ...
        try {
            ...
            if (admission.level === "tight") {
                console.warn(`[local-inference] Loading "${installed.id}" with tight RAM headroom (~${admission.minMb} MB floor, ${admission.recommendedMb} MB recommended; ${hostRamMbFromProbe(probe)} MB host). Expect swapping under sustained load.`);
            }
