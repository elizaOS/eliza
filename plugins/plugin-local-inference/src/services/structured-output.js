 ...
 */
export { repairStructuredOutput, StructuredOutputRepairStream, } from "./structured-output/deterministic-repair";
/** True when `kind` is a span the model actually samples. */
function isFreeSpan(span) {
    return (span.kind === "free-string" ||
        span.kind === "free-json" ||
        span.kind === "number" ||
        span.kind === "boolean" ||
        (span.kind === "enum" &&
...
export function compileSkeletonToGbnf(skeletonInput) {
    ...
    if (!skeleton.spans.some(isFreeSpan))
        return null;
    const rules = new Map();
    const rootParts = [];
    let freeIdx = 0;
    ...
    for (let i = 0; i < skeleton.spans.length; i += 1) {
        ...
        if (span.kind === "free-string") {
            ...
            if (!rules.has(ruleName))
                rules.set(ruleName, GBNF_JSON_STRING);
            rootParts.push(ruleName);
            continue;
        }
        if (span.kind === "number") {
            ...
            if (!rules.has(ruleName)) {
                rules.set(ruleName, '"-"? ( [0-9] | [1-9] [0-9]* ) ( "." [0-9]+ )? ( [eE] [-+]? [0-9]+ )?');
            }
            rootParts.push(ruleName);
            continue;
        }
    ...
    if (needsJsonValue)
        lines.push(GBNF_JSON_VALUE);
    const source = lines.join("\n");
    if (triggerWord)
        return { source, lazy: true, triggers: [triggerWord] };
...
export function splitSkeletonAtFirstFree(skeleton) {
    let prefixLiteral = "";
    let idx = 0;
    while (idx < skeleton.spans.length &&
...
export function canonicalizeShortName(schema, name) {
    if (!schema)
        return name;
    if (Object.hasOwn(schema.longNames, name))
        return name; // already a short id
...
export function resolveGuidedDecodeForParams(params) {
    if (!params)
        return { grammar: null, prefillPlan: null, prefill: null };
    const schema = params.elizaSchema;
    if (schema) {
        const grammar = typeof schema.grammar === "string" && schema.grammar.trim().length > 0
        ...
        const prefill = typeof params.prefill === "string" && params.prefill.length > 0
            ? params.prefill
    ...
    return {
        ...
        prefillPlan: null,
        prefill: typeof params.prefill === "string" && params.prefill.length > 0
            ? params.prefill
            : null,
    };
}
