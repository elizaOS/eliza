...
export class KokoroOnnxRuntime implements KokoroRuntime {
	...
	async synthesize(args: KokoroRuntimeInputs): Promise<{ cancelled: boolean }> {
		...
		// KOKORO_MAX_PHONEME_LENGTH tokens (matches `kokoro-onnx` upstream
		...
		if (args.phonemes.ids.length > KOKORO_MAX_PHONEME_LENGTH) {
			throw new KokoroInputTooLongError(args.phonemes.ids.length);
		}
