 ...
 *
 * Two execution paths ship:
 *
 *   1. GGUF via the fork's llama-server / omnivoice path (default).
 *      The text-gen llama-server (built from
 *      `packages/inference/llama.cpp`) exposes the OpenAI-compatible
 *      `/v1/audio/speech` endpoint backed by the Kokoro head; PCM is
 *      streamed back. On AOSP / desktop the `omnivoice.cpp`
 *      cross-build (libomnivoice.{so,dylib,dll}) serves the same role,
 *      loaded via Capacitor on mobile and Bun FFI on desktop. This is
 *      the only production path.
 *
 *   2. Python subprocess (eval-only). Spawns `python -m kokoro_tts`
 *      from a known venv. Never the default in production — used only
 *      by the fine-tune evaluator that drives Apollo's training-loop
 *      quality gate (which already depends on the upstream Python eval
 *      suite).
 *
 ...
 *   - GGUF:          produced by the elizaOS/llama.cpp fork's
 *                    `omnivoice/tools/convert_kokoro_to_gguf.py`.
 *
 * After download, the runtime verifies SHA-256 against the value pinned
 * in `eliza-1.manifest.json` (or `KOKORO_MODEL_SHA256` env var for
 * ad-hoc builds). A mismatch raises `KokoroModelMissingError` and the
 * engine refuses to activate Kokoro — no silent downgrade.
 */
...
/** Pinned GGUF candidate location (produced by the omnivoice tools).
 *  The runtime references this only for diagnostics; the fork-side
 *  builder produces the file at this path. */
...
/**
 * Construction-time inputs for a runtime instance. The voice pack
 * contains the style tensor reference; the runtime is responsible for
 * resolving the bytes off `layout.voicesDir/<file>`.
 */
export interface KokoroRuntimeInputs {
	phonemes: KokoroPhonemeSequence;
	voice: KokoroVoicePack;
	/**
	 * Output sample budget. The runtime always honours the model's
	 * native sample rate (`layout.sampleRate`, usually 24 kHz) — this
	 * caps the total samples to prevent runaway generation. Defaults to
	 * 16 seconds at the layout sample rate.
	 */
	maxSamples?: number;
...
/** Shared runtime contract — `KokoroTtsBackend` depends on this, not
 *  the concrete classes. Tests inject a mock. */
...
// ---------------------------------------------------------------------------
// GGUF via llama-server / omnivoice path — production default.
// ---------------------------------------------------------------------------
...
function decodePhonemesForGgufBody(seq: KokoroPhonemeSequence): string {
	...
	// it the same way the GGUF graph does.
	return seq.phonemes;
}
 ...
 * synthesis call (no warm-pool — the Python path is the *eval* path,
 * not the realtime path). Production code paths never select this; the
...
// ---------------------------------------------------------------------------
// Mock runtime — synthesizes a sine sweep keyed to phoneme count so
// tests can observe deterministic PCM without loading the real model.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Back-compat: KokoroModelMissingError is re-thrown from the discovery layer
// when no GGUF / voice pack is staged. Re-export it from this module so
// callers that historically imported it from here keep compiling.
// ---------------------------------------------------------------------------
