...
export function projectVadToExpressiveEmotion(vad) {
    ...
    if (!Number.isFinite(vad.valence) ||
...
export class VoiceEmotionClassifier {
    ...
    constructor(options) {
        if (typeof options.modelPath !== "string" ||
            ...
            throw new VoiceEmotionClassifierError("[voice-emotion] modelPath is required");
        }
    ...
    async ensureLoaded() {
        ...
        try {
            ...
            if (err instanceof OnnxRuntimeUnavailableError)
                throw err;
            throw new VoiceEmotionClassifierError(`[voice-emotion] failed to load onnxruntime-node: ${err instanceof Error ? err.message : String(err)}`);
        }
        try {
            this.session = await ort.InferenceSession.create(this.modelPath);
        }
        catch (err) {
            throw new VoiceEmotionClassifierError(`[voice-emotion] failed to load model at ${this.modelPath}: ${err instanceof Error ? err.message : String(err)}`);
        }
        this.tensorCtor = ort.Tensor;
        this.inputName = this.session.inputNames[0] ?? null;
        if (!this.inputName) {
            throw new VoiceEmotionClassifierError("[voice-emotion] ONNX session reported zero input names");
        }
    }
    /**
    ...
    async classify(pcm) {
        if (!(pcm instanceof Float32Array)) {
            throw new VoiceEmotionClassifierError("[voice-emotion] pcm must be a Float32Array");
        }
        if (pcm.length < WAV2SMALL_MIN_SAMPLES) {
            throw new VoiceEmotionClassifierError(`[voice-emotion] pcm too short: ${pcm.length} samples < ${WAV2SMALL_MIN_SAMPLES}`);
        }
        ...
        if (!session || !Tensor || !inputName) {
            throw new VoiceEmotionClassifierError("[voice-emotion] session not loaded after ensureLoaded()");
        }
        const samples = pcm.length > WAV2SMALL_MAX_SAMPLES
        ...
        try {
            ...
            throw new VoiceEmotionClassifierError(`[voice-emotion] ONNX run failed: ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        if (!headName) {
            throw new VoiceEmotionClassifierError("[voice-emotion] ONNX session reported zero output names");
        }
        const out = outputs[headName];
        if (!out || !(out.data instanceof Float32Array)) {
            throw new VoiceEmotionClassifierError(`[voice-emotion] expected Float32 output, got ${out?.data?.constructor?.name ?? "undefined"}`);
        }
        if (out.data.length < 3) {
            throw new VoiceEmotionClassifierError(`[voice-emotion] expected ≥ 3 outputs (V,A,D), got ${out.data.length}`);
        }
        const vad = {
