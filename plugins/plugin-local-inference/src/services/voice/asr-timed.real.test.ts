...
import { type ElizaInferenceContextHandle } from "./ffi-bindings";
import { type ElizaInferenceFfi } from "./ffi-bindings";
import { validateAsrWordTimings } from "@elizaos/core/transcripts";
import os from "node:os";
import path from "node:path";
const isBun = typeof (globalThis as {
    Bun?: unknown;
}).Bun !== "undefined";
const LIB_PATH = resolveFusedLibraryPath(null, process.env) ??
    (() => {
        const built = fileURLToPath(new URL("../../../native/llama.cpp/build-static-fused/bin/libelizainference.so", import.meta.url));
...
const FREEMAN_WAV = fileURLToPath(new URL("../../../native/audio-fixtures/freeman.wav", import.meta.url));
...
describe.skipIf(!isBun || !LIB_PATH || !HAVE_BUNDLE)("fused ASR v12 per-word timings — real FFI on real audio", () => {
    ...
    it.skipIf(!HAVE_FREEMAN)("transcribes freeman.wav with well-formed per-word timings", () => {
        const { pcm, sampleRate } = decodeMonoPcm16Wav(new Uint8Array(readFileSync(FREEMAN_WAV)));
        ...
        expect((last as {
            endMs: number;
        }).endMs).toBeLessThanOrEqual(audioDurationMs + 1);
        expect((last as {
            endMs: number;
        }).endMs).toBeGreaterThan(audioDurationMs * 0.5);
    }, 
