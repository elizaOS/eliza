...
export class DesktopMicSource extends BaseMicSource {
    ...
    async start() {
        if (this._running)
            return;
        if (!this.program) {
            throw new Error(`[voice] No CLI mic recorder available on platform '${process.platform}'. ` +
        ...
        try {
            ...
            throw new Error(`[voice] Failed to spawn mic recorder '${this.program}': ${err instanceof Error ? err.message : String(err)}. Install it (Linux: alsa-utils/sox; macOS: sox or ffmpeg; Windows: ffmpeg) or use PushMicSource.`);
        }
        this.proc = proc;
        const stderrChunks = [];
        ...
        proc.on("error", (err) => {
            this.emitError(new Error(`[voice] Mic recorder '${this.program}' error: ${err.message}`));
        });
        proc.on("exit", (code, signal) => {
            ...
            if (this._running) {
                ...
                this.emitError(new Error(`[voice] Mic recorder '${this.program}' exited unexpectedly (code=${code} signal=${signal})${stderr ? `: ${stderr}` : ""}`));
            }
        });
...
export class PushMicSource extends BaseMicSource {
    ...
    constructor(opts = {}) {
        const sampleRate = opts.sampleRate ?? DEFAULT_SAMPLE_RATE;
        const frameSamples = opts.frameSamples ??
    ...
    push(pcm, timestampMs = now()) {
        if (!this._running)
            return;
        const mergedStartTimestampMs = this.pending.length > 0 ? this.pendingStartTimestampMs : timestampMs;
        ...
        while (merged.length - offset >= this.frameSamples) {
            ...
            const frameTimestampMs = mergedStartTimestampMs + (offset / this.sampleRate) * 1000;
