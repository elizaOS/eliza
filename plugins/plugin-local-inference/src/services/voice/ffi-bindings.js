...
export function loadElizaInferenceFfi(dylibPath) {
    if (!isBunRuntime()) {
        throw new VoiceLifecycleError("kernel-missing", `[ffi-bindings] Cannot load libelizainference: current runtime is not Bun. ` +
    ...
    if (!dylibPath || dylibPath.length === 0) {
        throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] loadElizaInferenceFfi: dylibPath is required");
    }
...
function bindWithBunFfi(dylibPath) {
    ...
    try {
        ...
        throw new VoiceLifecycleError("kernel-missing", `[ffi-bindings] Cannot load bun:ffi while opening ${dylibPath}: ${formatFfiError(err)}`);
    }
    const T = ffi.FFIType;
    ...
    if (lib === null) {
        throw new VoiceLifecycleError("kernel-missing", `[ffi-bindings] Failed to open libelizainference at ${dylibPath}: ${formatFfiError(lastOpenError)}`);
    }
    const loadedLib = lib;
    ...
    const reported = readCString(loadedLib.symbols.eliza_inference_abi_version(), ffi);
    const abiOk = reported === String(ELIZA_INFERENCE_ABI_VERSION) ||
        (reported === "3" && !referenceEncodeSymbolsAvailable);
    if (!abiOk) {
        loadedLib.close();
        throw new VoiceLifecycleError("kernel-missing", `[ffi-bindings] ABI mismatch: binding expected v${ELIZA_INFERENCE_ABI_VERSION}, ` +
    ...
    function takeError(outErrPtrBuf) {
        ...
        if (ptrValue === undefined || ptrValue === 0n)
            return null;
        const ptrNumber = Number(ptrValue);
        if (!Number.isSafeInteger(ptrNumber)) {
            throw new VoiceLifecycleError("kernel-missing", `[ffi-bindings] C diagnostic pointer ${ptrValue.toString()} exceeds JS safe integer range`);
        }
    ...
    function cstr(value) {
        if (value === null)
            return { ptr: null, bytes: 0, buffer: null };
    ...
    function failureCode(rc) {
        if (rc === ELIZA_ERR_OOM)
            return "ram-pressure";
        if (rc === ELIZA_ERR_FFI_FAULT)
            return "mmap-fail";
        if (rc === ELIZA_ERR_NOT_IMPLEMENTED)
            return "kernel-missing";
        if (rc === ELIZA_ERR_ABI_MISMATCH)
            return "kernel-missing";
        if (rc === ELIZA_ERR_BUNDLE_INVALID)
            return "kernel-missing";
        return "kernel-missing";
    }
    ...
    return {
        ...
        create(bundleDir) {
            ...
            const handle = loadedLib.symbols.eliza_inference_create(bundleArg.ptr, err.ptr);
            if (isNullPointer(handle)) {
                const message = takeError(err.buf) ??
        ...
        mmapAcquire(ctx, region) {
            ...
            const rc = loadedLib.symbols.eliza_inference_mmap_acquire(ctx, regionArg.ptr, err.ptr);
            if (rc !== ELIZA_OK) {
                const message = takeError(err.buf) ??
        ...
        mmapEvict(ctx, region) {
            const err = makeOutErr();
            const regionArg = cstr(region);
            const rc = loadedLib.symbols.eliza_inference_mmap_evict(ctx, regionArg.ptr, err.ptr);
            if (rc !== ELIZA_OK) {
                const message = takeError(err.buf) ??
        ...
        ttsSynthesize({ ctx, text, speakerPresetId, out }) {
            ...
            const rc = loadedLib.symbols.eliza_inference_tts_synthesize(ctx, textArg.ptr, BigInt(textArg.bytes), speakerArg.ptr, ffi.ptr(out), BigInt(out.length), err.ptr);
            if (rc < 0) {
                const message = takeError(err.buf) ??
        ...
        asrTranscribe({ ctx, pcm, sampleRateHz, maxTextBytes }) {
            ...
            const rc = loadedLib.symbols.eliza_inference_asr_transcribe(ctx, ffi.ptr(pcm), BigInt(pcm.length), sampleRateHz, ffi.ptr(outText), BigInt(cap), err.ptr);
            if (rc < 0) {
                const message = takeError(err.buf) ??
            ...
            return Buffer.from(outText.buffer, outText.byteOffset, len).toString("utf8");
        },
        ...
        ttsSynthesizeStream({ ctx, text, speakerPresetId, onChunk }) {
            ...
            const cb = new ffi.JSCallback(((pcmPtr, nSamples, isFinal) => {
            ...
            try {
                const rc = loadedLib.symbols.eliza_inference_tts_synthesize_stream(ctx, textArg.ptr, BigInt(textArg.bytes), speakerArg.ptr, BigInt(cb.ptr), 0n, err.ptr);
        ...
        encodeReferenceSupported() {
            return (typeof loadedLib.symbols.eliza_inference_encode_reference === "function");
        },
        encodeReference({ ctx, pcm, sampleRateHz }) {
            if (typeof loadedLib.symbols.eliza_inference_encode_reference !==
                "function" ||
                typeof loadedLib.symbols.eliza_inference_free_tokens !== "function") {
                throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] eliza_inference_encode_reference is not exported by this build");
            }
            if (sampleRateHz !== 24000) {
                throw new VoiceLifecycleError("kernel-missing", `[ffi-bindings] encodeReference: sampleRateHz must be 24000 (got ${sampleRateHz})`);
            }
            const err = makeOutErr();
            ...
            const rc = loadedLib.symbols.eliza_inference_encode_reference(ctx, ffi.ptr(pcm), BigInt(pcm.length), sampleRateHz, ffi.ptr(outK), ffi.ptr(outRefT), ffi.ptr(outTokensPtr), err.ptr);
            if (rc !== ELIZA_OK) {
                const message = takeError(err.buf) ??
            ...
            if (K <= 0 || refT <= 0 || tokensRaw === 0n) {
                throw new VoiceLifecycleError("kernel-missing", `[ffi-bindings] encodeReference returned empty result (K=${K}, refT=${refT})`);
            }
        ...
        setVerifierCallback(ctx, cbFn) {
            const err = makeOutErr();
            if (cbFn === null) {
                const rc = loadedLib.symbols.eliza_inference_set_verifier_callback(ctx, 0n, 0n, err.ptr);
                if (rc !== ELIZA_OK) {
                    const message = takeError(err.buf) ??
            ...
            const cb = new ffi.JSCallback(((evPtr) => {
                cbFn(readVerifierEvent(evPtr, ffi));
            }), { args: [T.ptr, T.ptr], returns: T.void });
            const rc = loadedLib.symbols.eliza_inference_set_verifier_callback(ctx, BigInt(cb.ptr), 0n, err.ptr);
            if (rc !== ELIZA_OK) {
                cb.close();
                const message = takeError(err.buf) ??
            ...
            return {
                close: () => {
                    ...
                    loadedLib.symbols.eliza_inference_set_verifier_callback(ctx, 0n, 0n, clearErr.ptr);
        ...
        vadOpen({ ctx, sampleRateHz }) {
            ...
            if (!nativeVadSymbolsAvailable || typeof open !== "function") {
                throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] eliza_inference_vad_open is not exported by this libelizainference build");
            }
            const err = makeOutErr();
        ...
        vadProcess({ vad, pcm }) {
            ...
            if (!nativeVadSymbolsAvailable || typeof process !== "function") {
                throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] eliza_inference_vad_process is not exported by this libelizainference build");
            }
            const err = makeOutErr();
            const outProbability = new Float32Array(1);
            const rc = process(vad, ffi.ptr(pcm), BigInt(pcm.length), ffi.ptr(outProbability), err.ptr);
            if (rc !== ELIZA_OK) {
                const message = takeError(err.buf) ??
        ...
        vadReset(vad) {
            ...
            if (!nativeVadSymbolsAvailable || typeof reset !== "function") {
                throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] eliza_inference_vad_reset is not exported by this libelizainference build");
            }
            const err = makeOutErr();
        ...
        asrStreamOpen({ ctx, sampleRateHz }) {
            const err = makeOutErr();
            const handle = loadedLib.symbols.eliza_inference_asr_stream_open(ctx, sampleRateHz, err.ptr);
            if (isNullPointer(handle)) {
                const message = takeError(err.buf) ??
        ...
        asrStreamFeed({ stream, pcm }) {
            const err = makeOutErr();
            const rc = loadedLib.symbols.eliza_inference_asr_stream_feed(stream, ffi.ptr(pcm), BigInt(pcm.length), err.ptr);
            if (rc < 0) {
                const message = takeError(err.buf) ??
        ...
        asrStreamPartial(args) {
            return readAsrStreamResult("partial", loadedLib.symbols.eliza_inference_asr_stream_partial, args);
        },
        asrStreamFinish(args) {
            return readAsrStreamResult("finish", loadedLib.symbols.eliza_inference_asr_stream_finish, args);
        },
        ...
        llmStreamSupported() {
            ...
            return (llmStreamSymbolsAvailable &&
        ...
        llmStreamOpen({ ctx, config }) {
            ...
            if (!llmStreamSymbolsAvailable || typeof open !== "function") {
                throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] eliza_inference_llm_stream_open is not exported by this build");
            }
            const err = makeOutErr();
        ...
        llmStreamPrefill({ stream, tokens }) {
            ...
            if (!llmStreamSymbolsAvailable || typeof prefill !== "function") {
                throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] eliza_inference_llm_stream_prefill is not exported by this build");
            }
            const err = makeOutErr();
            const rc = prefill(stream, ffi.ptr(tokens), BigInt(tokens.length), err.ptr);
            if (rc !== ELIZA_OK) {
                const message = takeError(err.buf) ??
        ...
        llmStreamNext({ stream, maxTokensPerStep, maxTextBytes }) {
            ...
            if (!llmStreamSymbolsAvailable || typeof next !== "function") {
                throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] eliza_inference_llm_stream_next is not exported by this build");
            }
            const err = makeOutErr();
            ...
            const rc = next(stream, ffi.ptr(tokensOut), BigInt(tokenCap), ffi.ptr(numTokensOut), ffi.ptr(textOut), BigInt(textCap), ffi.ptr(drafterDrafted), ffi.ptr(drafterAccepted), err.ptr);
            if (rc < 0) {
                const message = takeError(err.buf) ??
            ...
            const text = Buffer.from(textOut.buffer, textOut.byteOffset, len).toString("utf8");
            return {
                tokens,
                text,
                done: rc === 1,
        ...
        llmStreamSaveSlot({ stream, filename }) {
            ...
            if (!llmStreamSymbolsAvailable || typeof save !== "function") {
                throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] eliza_inference_llm_stream_save_slot is not exported by this build");
            }
            const err = makeOutErr();
        ...
        llmStreamRestoreSlot({ stream, filename }) {
            ...
            if (!llmStreamSymbolsAvailable || typeof restore !== "function") {
                throw new VoiceLifecycleError("kernel-missing", "[ffi-bindings] eliza_inference_llm_stream_restore_slot is not exported by this build");
            }
            const err = makeOutErr();
            const fnameArg = cstr(filename);
    ...
    function readAsrStreamResult(label, fn, args) {
        ...
        const ioNTokens = wantTokens
            ...
            : null;
        const rc = fn(args.stream, ffi.ptr(outText), BigInt(textCap), outTokens ? ffi.ptr(outTokens) : null, ioNTokens ? ffi.ptr(ioNTokens) : null, err.ptr);
        if (rc < 0) {
            const message = takeError(err.buf) ??
        ...
        const nul = outText.indexOf(0, 0);
        const len = nul >= 0 ? nul : rc;
        const partial = Buffer.from(outText.buffer, outText.byteOffset, len).toString("utf8");
