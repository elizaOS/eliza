 ...
 */
import { CompositeEntityRecognizer } from "@elizaos/core";
import { ElizaError } from "@elizaos/core";
import { GazetteerEntityRecognizer } from "@elizaos/core";
import { PseudonymSession } from "@elizaos/core";
import { RegexEntityRecognizer } from "@elizaos/core";
...
import { parseArtifactShareMetadata } from "@elizaos/core";
...
import { type ArtifactDisclosure } from "@elizaos/core";
import { type ArtifactRoomSnapshot } from "@elizaos/core";
import { type ArtifactShareGrant } from "@elizaos/core";
...
import { type TranscriptRetentionState } from "@elizaos/core/transcripts";
import { type TranscriptSharingState } from "@elizaos/core/transcripts";
 ...
 */
export function transcriptRowDisclosure(row: Memory, transcript: Pick<Transcript, "scope">, accessContext: AccessContext | undefined, agentId: UUID): ArtifactDisclosure {
    const metadata = row.metadata as Record<string, unknown> | undefined;
    const scopedTo = metadata?.scopedToEntityId;
    const scopedEntityId = typeof scopedTo === "string" ? (scopedTo as UUID) : row.entityId;
...
export function transcriptConsentAllowsSharing(transcript: Transcript): boolean {
...
function assertTranscriptConsentAllowsSharing(transcript: Transcript): void {
    if (transcriptConsentAllowsSharing(transcript))
        return;
    const state: TranscriptConsentState = transcriptCapturePrivacyState(transcript).consentState ?? "unknown";
    throw new ElizaError(`transcript consent state ${state} does not permit sharing`, {
 ...
 */
function serveRedactedVariant(variant: Transcript, original: Pick<Transcript, "id" | "createdAt" | "endedAt" | "source">): Transcript {
    return {
        ...variant,
        id: original.id,
...
function sharingAllowsGrantedViewer(state: TranscriptSharingState | undefined): boolean {
    return state === "restricted" || state === "shared" || state === "public";
}
function viewerOwnsTranscript(row: Memory, accessContext: AccessContext | undefined, agentId: UUID): boolean {
    if (!accessContext)
        return true;
    return (isAdminRank(accessContext.role) ||
...
function projectTranscriptArtifacts(row: Memory, transcript: Transcript, accessContext: AccessContext | undefined, agentId: UUID): Transcript {
    if (viewerOwnsTranscript(row, accessContext, agentId))
        return transcript;
...
function mergedTranscriptMetadata(transcript: Transcript, patch: {
    sharing?: Partial<TranscriptCaptureSharingState>;
    retention?: {
    ...
    const currentSharing = currentMetadata.sharing && typeof currentMetadata.sharing === "object"
        ? (currentMetadata.sharing as Record<string, unknown>)
        : {};
    const currentRetention = currentMetadata.retention && typeof currentMetadata.retention === "object"
...
function transcriptRosterEntries(transcript: Transcript): Array<{
    ...
    for (const participant of participants) {
        if (!participant || typeof participant !== "object")
            continue;
        const displayName = (participant as {
            displayName?: unknown;
        }).displayName;
 ...
 */
export function transcriptPiiRecognizer(transcript: Transcript, supplemental?: PiiEntityRecognizer): PiiEntityRecognizer {
    const recognizers: PiiEntityRecognizer[] = [new RegexEntityRecognizer()];
    const roster = transcriptRosterEntries(transcript);
    if (roster.length > 0) {
        recognizers.push(new GazetteerEntityRecognizer(roster, { name: "transcript-roster" }));
    }
    if (supplemental)
        recognizers.push(supplemental);
...
async function redactTranscript(original: Transcript, redactedAudioUrl?: string, seed?: string, supplementalRecognizer?: PiiEntityRecognizer): Promise<Transcript> {
    ...
    const safeText = (text: string): string => redactedText(pseudonyms.substituteText(text));
...
function mergedGrant(grants: readonly ArtifactShareGrant[], next: ArtifactShareGrant): ArtifactShareGrant[] {
    const out = grants.filter((grant) => grant.entityId !== next.entityId);
    out.push(next);
    return out;
}
function shareGrantsMetadata(grants: readonly ArtifactShareGrant[], roomSnapshot?: ArtifactRoomSnapshot): JsonObject {
    return {
...
export class TranscriptStore {
     ...
     */
    async list(roomId?: UUID, limit = 100, accessContext?: AccessContext): Promise<TranscriptSummary[]> {
        ...
        for (const row of rows) {
            ...
            if (!t || redactionOriginalId(row) !== null)
                continue;
            const disclosure = transcriptRowDisclosure(row, t, accessContext, this.runtime.agentId);
            if (disclosure === "full") {
                summaries.push(summarizeTranscript(projectTranscriptArtifacts(row, t, accessContext, this.runtime.agentId)));
            }
     ...
     */
    async get(id: UUID, accessContext?: AccessContext): Promise<Transcript | null> {
        ...
        if (originalId !== null) {
            ...
            const disclosure = transcriptRowDisclosure(gate, gateTranscript, accessContext, this.runtime.agentId);
            return disclosure === "full" ? transcript : null;
        }
        const disclosure = transcriptRowDisclosure(row, transcript, accessContext, this.runtime.agentId);
        if (disclosure === "full") {
            return projectTranscriptArtifacts(row, transcript, accessContext, this.runtime.agentId);
        }
     ...
     */
    async createRedactedVariant(input: CreateRedactedTranscriptVariantInput): Promise<Transcript> {
        ...
        const variantId = existingVariantId ??
            (stringToUuid(`transcript-redaction:${input.originalId}:${input.seed ?? ""}`) as UUID);
        const nowMs = input.nowMs ?? Date.now();
        const variant = {
            ...(await redactTranscript(original, input.redactedAudioUrl, input.seed, input.recognizer)),
            id: variantId,
            createdAt: nowMs,
            metadata: {
        ...
        const variantRow = await this.runtime.getMemoryById(variantId);
        const variantMeta = variantRow?.metadata as Record<string, unknown> | undefined;
    ...
    async share(input: ShareTranscriptGrantInput): Promise<void> {
        ...
        if (redactionOriginalId(row)) {
            throw new Error("share grants must be attached to the original transcript");
        }
        const transcript = rowToTranscript(row);
        if (!transcript) {
        ...
        const ok = await this.runtime.updateMemory({
            ...
            metadata: {
                ...
                type: "custom",
                source: TRANSCRIPT_METADATA_TYPE,
                share: shareGrantsMetadata(mergedGrant(share.grants, nextGrant), share.roomSnapshot),
            } as MemoryMetadata,
        });
        if (!ok) {
            throw new Error(`transcript ${input.transcriptId} not found`);
     ...
     */
    async shareRoomSnapshot(input: ShareTranscriptRoomSnapshotInput): Promise<void> {
        const row = await this.runtime.getMemoryById(input.transcriptId);
        if (!row) {
            throw new Error(`transcript ${input.transcriptId} not found`);
        }
    ...
    async revokeShare(input: RevokeTranscriptGrantInput): Promise<void> {
        ...
        if (redactionOriginalId(row)) {
            throw new Error("share grants must be revoked from the original transcript");
        }
        const metadata = row.metadata as Record<string, unknown> | undefined;
        const share = parseArtifactShareMetadata(metadata);
        const ok = await this.runtime.updateMemory({
            ...
            metadata: {
                ...
                type: "custom",
                source: TRANSCRIPT_METADATA_TYPE,
                share: shareGrantsMetadata(share.grants.filter((grant) => grant.entityId !== input.entityId), share.roomSnapshot),
            } as MemoryMetadata,
        });
        if (!ok) {
            throw new Error(`transcript ${input.transcriptId} not found`);
    ...
    async clearShares(transcriptId: UUID): Promise<void> {
        ...
        if (redactionOriginalId(row)) {
            throw new Error("share grants must be cleared from the original transcript");
        }
        const metadata = row.metadata as Record<string, unknown> | undefined;
        const ok = await this.runtime.updateMemory({
            id: transcriptId,
    ...
    async updateArtifactSharing(input: UpdateTranscriptArtifactSharingInput): Promise<Transcript> {
        const row = await this.runtime.getMemoryById(input.transcriptId);
        if (!row)
            throw new Error(`transcript ${input.transcriptId} not found`);
        if (redactionOriginalId(row)) {
            throw new Error("artifact sharing must be updated on the original transcript");
        }
        const transcript = rowToTranscript(row);
        if (!transcript) {
            throw new Error(`transcript ${input.transcriptId} is corrupt`);
        }
        return this.update(mergedTranscriptMetadata(transcript, { sharing: input.sharing }));
    }
    /**
     ...
     */
    async markSourceAudioDeletePending(input: MarkTranscriptSourceAudioDeleteInput): Promise<Transcript> {
        const row = await this.runtime.getMemoryById(input.transcriptId);
        if (!row)
            throw new Error(`transcript ${input.transcriptId} not found`);
        const transcript = rowToTranscript(row);
        ...
        const pending = mergedTranscriptMetadata({ ...transcript, audioUrl: undefined, audioContentType: undefined }, {
    ...
    async markSourceAudioDeleted(transcriptId: UUID): Promise<Transcript> {
        ...
        const transcript = rowToTranscript(row);
        if (!transcript)
            throw new Error(`transcript ${transcriptId} is corrupt`);
    ...
    async update(transcript: Transcript): Promise<Transcript> {
        ...
        const carriedShare = preserved?.share && typeof preserved.share === "object"
            ? (preserved.share as JsonObject)
            : undefined;
        const carriedVariantId = typeof preserved?.redactedVariantId === "string"
            ? preserved.redactedVariantId
            : undefined;
        const carriedRedactionOf = typeof preserved?.redactionOf === "string"
