...
export const DEFAULT_VOICE_PRESET_REL_PATH = path.join("cache", "voice-preset-default.bin");
/**
...
export function voicePresetPath(bundleRoot, voiceId) {
    ...
    if (!/^[A-Za-z0-9._-]+$/.test(voiceId) || voiceId.includes("..")) {
        throw new Error(`[voice] Invalid voiceId ${JSON.stringify(voiceId)} — must be a single path-safe segment.`);
    }
...
export class SpeakerPresetCache {
    ...
    maxVoices;
    constructor(opts = {}) {
        this.maxVoices = Math.max(1, Math.floor(opts.maxVoices ?? DEFAULT_MAX_VOICES));
    }
    /**
    ...
    loadFile(fullPath, voiceId) {
        ...
        if (!existsSync(fullPath)) {
            throw new Error(`[voice] Speaker preset for voice ${JSON.stringify(voiceId)} not found at ${fullPath}.`);
        }
