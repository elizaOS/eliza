...
import { computeDiarizationErrorRate } from "./diarization-error-rate";
import { normalizeWerText } from "@elizaos/core/voice-wer";
import { percentile } from "./metric-math";
import { round1 } from "./metric-math";
import { round4 } from "./metric-math";
import { type DiarizationSegment } from "./diarization-error-rate";
import { wordErrorRate } from "@elizaos/core/voice-wer";
export type VoiceE2eHarnessErrorCode = "missing-artifact" | "missing-measurement" | "invalid-measurement";
export class VoiceE2eHarnessError extends Error {
    readonly code: VoiceE2eHarnessErrorCode;
    readonly details?: unknown;
    constructor(code: VoiceE2eHarnessErrorCode, message: string, details?: unknown) {
        super(message);
...
export interface RequiredVoiceArtifact {
    kind: "bundle-root" | "speaker-preset" | "tts-model" | "tts-tokenizer" | "asr-model" | "asr-mmproj" | "ffi-library" | "server-binary";
    path: string;
    minBytes?: number;
    magic?: string;
}
...
export function assertRequiredVoiceArtifacts(artifacts: ReadonlyArray<RequiredVoiceArtifact>, probe: VoiceArtifactProbe): VerifiedVoiceArtifact[] {
    ...
    if (failures.length > 0) {
        const list = failures
            ...
            .join("\n");
        throw new VoiceE2eHarnessError("missing-artifact", `Missing required Eliza-1 voice artifact(s):\n${list}`, { failures });
    }
    return verified;
}
...
export function scoreTtsAsrRoundTrip(input: TtsAsrRoundTripInput): TtsAsrRoundTripResult {
...
export function scoreBargeInInterruption(input: BargeInInterruptionInput): BargeInInterruptionResult {
    const maxCancelMs = input.maxCancelMs ?? 250;
    const ttsCancelMs = optionalDuration("voiceDetectedAtMs", input.voiceDetectedAtMs, "ttsCancelledAtMs", input.ttsCancelledAtMs);
    const llmCancelMs = optionalDuration("voiceDetectedAtMs", input.voiceDetectedAtMs, "llmCancelledAtMs", input.llmCancelledAtMs);
    const audioDrainMs = optionalDuration("voiceDetectedAtMs", input.voiceDetectedAtMs, "audioDrainedAtMs", input.audioDrainedAtMs);
    if (ttsCancelMs === null) {
    ...
    const measured = [ttsCancelMs, llmCancelMs, audioDrainMs].filter((value): value is number => value !== null);
...
export function scorePauseContinuation(input: PauseContinuationInput): PauseContinuationResult {
    ...
    const maxRestartAfterContinuationMs = input.maxRestartAfterContinuationMs ?? 1000;
    const continuationGapMs = duration("speechPauseAtMs", input.speechPauseAtMs, "continuationAtMs", input.continuationAtMs);
    const speculativeStartAfterPauseMs = optionalDuration("speechPauseAtMs", input.speechPauseAtMs, "speculativeStartedAtMs", input.speculativeStartedAtMs);
    const abortAfterContinuationMs = duration("continuationAtMs", input.continuationAtMs, "speculativeAbortedAtMs", required(input.speculativeAbortedAtMs, "speculativeAbortedAtMs"));
    const restartAfterContinuationMs = duration("continuationAtMs", input.continuationAtMs, "finalRestartedAtMs", required(input.finalRestartedAtMs, "finalRestartedAtMs"));
    const committedBefore = input.committedBeforeContinuationAtMs !== null &&
    ...
    return {
        ...
        speculativeStartAfterPauseMs: speculativeStartAfterPauseMs === null
            ? null
...
export function scoreOptimisticRollbackRestart(input: OptimisticRollbackRestartInput): OptimisticRollbackRestartResult {
    const maxRestoreAfterContinuationMs = input.maxRestoreAfterContinuationMs ?? 300;
    const maxRestartAfterRestoreMs = input.maxRestartAfterRestoreMs ?? 1000;
    const saveAfterPauseMs = optionalDuration("speechPauseAtMs", input.speechPauseAtMs, "checkpointSavedAtMs", input.checkpointSavedAtMs);
    const abortAfterContinuationMs = duration("continuationAtMs", input.continuationAtMs, "speculativeAbortedAtMs", required(input.speculativeAbortedAtMs, "speculativeAbortedAtMs"));
    const restoreAfterContinuationMs = duration("continuationAtMs", input.continuationAtMs, "checkpointRestoredAtMs", required(input.checkpointRestoredAtMs, "checkpointRestoredAtMs"));
    const restartAfterRestoreMs = duration("checkpointRestoredAtMs", required(input.checkpointRestoredAtMs, "checkpointRestoredAtMs"), "restartedAtMs", required(input.restartedAtMs, "restartedAtMs"));
    return {
        kind: "optimistic-rollback-restart",
        saveAfterPauseMs: saveAfterPauseMs === null ? null : round1(saveAfterPauseMs),
        abortAfterContinuationMs: round1(abortAfterContinuationMs),
        restoreAfterContinuationMs: round1(restoreAfterContinuationMs),
        restartAfterRestoreMs: round1(restartAfterRestoreMs),
        passed: restoreAfterContinuationMs <= maxRestoreAfterContinuationMs &&
...
export function scoreFirstResponseLatency(input: FirstResponseLatencyInput): FirstResponseLatencyResult {
    const maxFirstAudioMs = input.maxFirstAudioMs ?? 1500;
    const asrFinalMs = optionalDuration("turnStartedAtMs", input.turnStartedAtMs, "asrFinalAtMs", input.asrFinalAtMs);
    const firstTokenMs = optionalDuration("turnStartedAtMs", input.turnStartedAtMs, "llmFirstTokenAtMs", input.llmFirstTokenAtMs);
    const firstAudioMs = duration("turnStartedAtMs", input.turnStartedAtMs, "ttsFirstAudioAtMs", required(input.ttsFirstAudioAtMs, "ttsFirstAudioAtMs"));
    const firstPlayedMs = optionalDuration("turnStartedAtMs", input.turnStartedAtMs, "audioFirstPlayedAtMs", input.audioFirstPlayedAtMs);
    return {
        kind: "first-response-latency",
...
export function scoreEotDecision(samples: ReadonlyArray<EotDecisionSample>, opts: {
    maxFalseTriggerRate?: number;
} = {}): EotDecisionResult {
...
export function scoreRespondDecision(samples: ReadonlyArray<RespondDecisionSample>, opts: {
    minAccuracy?: number;
    ...
    let should = 0;
    for (const s of samples) {
        if (s.responded === s.expectRespond)
            correct += 1;
        if (s.expectRespond)
            should += 1;
        else
...
export function scoreDiarization(samples: ReadonlyArray<DiarizationSample>, opts: {
    maxDer?: number;
} = {}): DiarizationResult {
    ...
    let misses = 0;
    for (const s of samples) {
        if (s.predictedLabel === null)
            misses += 1;
        else if (s.predictedLabel !== s.expectedLabel)
            confusions += 1;
    }
 ...
 */
export function scoreDiarizationTimeline(turns: ReadonlyArray<DiarizationTurnSample>, opts: {
    maxDer?: number;
} = {}): DiarizationResult {
    const maxDer = opts.maxDer ?? 0.2;
 ...
 */
export function scoreDiarizationSegments(reference: ReadonlyArray<DiarizationSegment>, hypothesis: ReadonlyArray<DiarizationSegment>, opts: {
    maxDer?: number;
} = {}): DiarizationResult {
    const maxDer = opts.maxDer ?? 0.2;
    const result = computeDiarizationErrorRate(reference, hypothesis);
    ...
    for (const segment of reference) {
        ...
        const active = hypothesis.filter((candidate) => candidate.startMs <= midpoint && candidate.endMs > midpoint);
        if (active.length === 0) {
            misses += 1;
        }
        else if (!active.some((candidate) => result.mapping[candidate.speaker] === segment.speaker)) {
            confusions += 1;
        }
    }
    return {
...
export function scoreEntityExtraction(input: EntityExtractionInput, opts: {
    minF1?: number;
    ...
    for (const e of inferred)
        if (expected.has(e))
            tp += 1;
    const precision = inferred.size > 0 ? tp / inferred.size : expected.size === 0 ? 1 : 0;
...
export function scoreVoiceEntityMatch(samples: ReadonlyArray<VoiceEntityMatchSample>, opts: {
    minMatchRate?: number;
    ...
    let correct = 0;
    for (const s of samples) {
        if (s.matchedEntityId === s.expectedEntityId)
            correct += 1;
    }
 ...
 */
export function scoreEchoRejection(samples: ReadonlyArray<EchoRejectionSample>, opts: {
    minRejectionRate?: number;
} = {}): EchoRejectionResult {
 ...
 */
export function scoreOwnerSecurity(samples: ReadonlyArray<OwnerSecuritySample>, opts: {
    minAccuracy?: number;
    maxImpostorAcceptRate?: number;
} = {}): OwnerSecurityResult {
    const minAccuracy = opts.minAccuracy ?? 0.9;
    ...
    let nonOwners = 0;
    for (const s of samples) {
        if (s.predictedOwner === s.expectedOwner)
            correct += 1;
        if (s.expectedOwner)
            owners += 1;
        else
 ...
 */
export function scoreBargeInGating(samples: ReadonlyArray<BargeInGatingSample>, opts: {
    maxCancelMs?: number;
} = {}): BargeInGatingResult {
    ...
    for (const s of samples) {
        ...
        if (s.expectCancel) {
            if (cancelled && (s.cancelMs as number) <= maxCancelMs)
                correct += 1;
            else
                missedCancels += 1;
            if (cancelled)
                legitCancelMs.push(s.cancelMs as number);
        }
        else {
            if (!cancelled)
                correct += 1;
    ...
    return {
        ...
        wrongCancels,
        missedCancels,
        worstCancelMs: legitCancelMs.length > 0 ? round1(Math.max(...legitCancelMs)) : null,
        maxCancelMs,
 ...
 */
export function scoreErle(samples: ReadonlyArray<{
    erleDb: number;
}>, opts: {
    minErleDb?: number;
} = {}): ErleResult {
 ...
 */
export function scorePartialMonotonicity(partials: ReadonlyArray<string>): PartialMonotonicityResult {
    let retractions = 0;
...
export function scoreMeasurementCoverage(metric: string, count: number): MeasurementCoverageResult {
    return {
        kind: "measurement-coverage",
        metric,
        count,
...
export type VoiceE2eCaseResult = TtsAsrRoundTripResult | BargeInInterruptionResult | BargeInGatingResult | PauseContinuationResult | OptimisticRollbackRestartResult | FirstResponseLatencyResult | EotDecisionResult | RespondDecisionResult | DiarizationResult | EntityExtractionResult | VoiceEntityMatchResult | EchoRejectionResult | OwnerSecurityResult | ErleResult | PartialMonotonicityResult | MeasurementCoverageResult;
export interface VoiceE2eSummary {
    passed: boolean;
    cases: VoiceE2eCaseResult[];
}
export function summarizeVoiceE2e(cases: ReadonlyArray<VoiceE2eCaseResult>): VoiceE2eSummary {
    return {
...
function optionalDuration(fromName: string, from: number, toName: string, to: number | null | undefined): number | null {
    if (to === null || to === undefined)
        return null;
    return duration(fromName, from, toName, to);
}
function duration(fromName: string, from: number, toName: string, to: number): number {
    if (!Number.isFinite(from))
        throw missingMeasurement(fromName);
    if (!Number.isFinite(to))
        throw missingMeasurement(toName);
    const delta = to - from;
    if (delta < 0) {
        throw new VoiceE2eHarnessError("invalid-measurement", `Invalid voice E2E measurement: ${toName} (${to}) is before ${fromName} (${from})`, { fromName, from, toName, to });
    }
    return delta;
}
function missingMeasurement(name: string): VoiceE2eHarnessError {
    return new VoiceE2eHarnessError("missing-measurement", `Missing required voice E2E measurement: ${name}`, { name });
}
