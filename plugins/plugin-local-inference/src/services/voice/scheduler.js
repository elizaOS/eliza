...
function isNativeCancelableTtsBackend(backend) {
    return (typeof backend.cancelTts ===
        "function");
}
...
export class VoiceScheduler {
    ...
    constructor(config, deps, events = {}) {
        this.chunker = new PhraseChunker(config.chunkerConfig, deps.phonemeTokenizer ?? null);
        this.preset = config.preset;
        this.backend = deps.backend;
        this.phraseCache = deps.phraseCache ?? new PhraseCache();
        this.sampleRate = config.sampleRate;
        this.sink = deps.sink ?? new InMemoryAudioSink();
        this.ringBuffer = new PcmRingBuffer(config.ringBufferCapacity, config.sampleRate, this.sink);
        this.events = events;
        this.maxInFlight = Math.max(1, config.maxInFlightPhrases ?? DEFAULT_MAX_IN_FLIGHT_PHRASES);
    ...
    markAgentSpeakingForAudio(samples, sampleRate) {
        ...
        this.agentSpeakingUntilMs = Math.max(this.agentSpeakingUntilMs, nowMs() + durationMs + 50);
    ...
    async dispatchPhrase(phrase) {
        ...
        this.lastCommittedTokenIndex = Math.max(this.lastCommittedTokenIndex, phrase.toIndex);
        ...
        void this.runPhraseSynthesis(phrase, cancelSignal).then(resolveDone, rejectDone);
    }
    async runPhraseSynthesis(phrase, cancelSignal) {
        try {
            ...
            if (this.streamingTtsActive && isStreamingTtsBackend(this.backend)) {
                const cancelled = await this.synthesizePhraseStream(phrase, cancelSignal);
    ...
    async synthesizePhraseStream(phrase, cancelSignal) {
        ...
        if (!isStreamingTtsBackend(backend))
            return false;
        const parts = [];
        let totalSamples = 0;
        let sampleRate = 0;
        ...
        const result = await backend.synthesizeStream({
            ...
            onChunk: (chunk) => {
                ...
                const chunkDurationMs = chunk.sampleRate > 0 ? (pcm.length / chunk.sampleRate) * 1000 : 0;
                ...
                inferenceTelemetry.record("tts.chunk_size_bytes", pcm.length * 4, // Float32: 4 bytes per sample
                { backend: ttsBackendName });
                ...
                this.commitAudio({
                    ...
                    pcm,
                    sampleRate: chunk.sampleRate,
                }, phrase, "synthesis", { emitFirstAudio: firstAudio, markPlayed: false });
                firstAudio = false;
                return cancelSignal.cancelled;
            },
        });
        ...
        if (this.events.onChunkMetrics) {
            const chunks = chunkSamples.map((c) => ({
                ...
                chunkDurationMs: c.sampleRate > 0 ? (c.samples / c.sampleRate) * 1000 : 0,
            }));
