...
export class TranscriptService {
    ...
    async create(input: CreateTranscriptInput): Promise<Transcript> {
        ...
        const knowledgeDocumentId = await this.mirrorToKnowledge(worldId, roomId, entityId, transcript);
    ...
    list(roomId?: UUID, limit?: number, accessContext?: AccessContext): Promise<TranscriptSummary[]> {
     ...
     */
    async update(id: UUID, input: UpdateTranscriptInput): Promise<Transcript | null> {
        ...
        const knowledgeDocumentId = await this.mirrorToKnowledge(input.worldId, input.roomId, input.entityId, next);
     ...
     */
    private async mirrorToKnowledge(worldId: UUID, roomId: UUID, entityId: UUID, transcript: Transcript): Promise<string | undefined> {
