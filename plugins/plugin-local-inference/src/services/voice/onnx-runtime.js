    ...
    if (!ortModulePromise) {
        ortModulePromise = (async () => {
            try {
                ...
                throw new OnnxRuntimeUnavailableError(`[voice] on-device ONNX models require the optional 'onnxruntime-node' dependency, which is not installed or failed to load (${err instanceof Error ? err.message : String(err)}).`);
            }
        })();
    }
