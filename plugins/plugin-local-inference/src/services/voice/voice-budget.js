...
export class BudgetExhaustedError extends Error {
    ...
    details;
    constructor(details) {
        super(`[voice-budget] Cannot fit ${(details.requestedBytes / BYTES_PER_MB).toFixed(0)} MB ` +
...
export function pickVoiceTierSlot(args) {
    ...
    if (id.includes("27b"))
        return "workstation-27b";
    if (id.includes("9b"))
        return "workstation-9b";
    if (id.includes("4b"))
        return "desktop-4b";
    if (id.includes("2b") || id.includes("1_7b"))
        return "desktop-2b";
...
export function assessVoiceBundleFits(args) {
    ...
    if (usableMb >= peakMb)
        level = "fits";
    else if (usableMb >= steadyStateMb)
        level = "tight";
    else
        level = "wontfit";
    return {
...
class VoiceBudgetImpl {
    ...
    async reserve(args) {
        ...
        while (this.freeBytes() < requestedBytes) {
            const candidate = this.lowestPriorityEvictableReservation(requestedPriorityRank);
            if (!candidate) {
                throw new BudgetExhaustedError({
                    requestedBytes,
                    freeBytes: this.freeBytes(),
    ...
    lowestPriorityEvictableReservation(requesterRank) {
        ...
        for (const entry of this._reservations.values()) {
            if (entry.released)
                continue;
            if (entry.priorityRank >= requesterRank)
                continue;
