...
const EMPTY_SECTION = Object.freeze({ offset: 0, length: 0 });
function checkSectionBounds(sec, fileLen, headerBytes) {
    if (sec.length === 0)
        return;
    if (sec.offset < headerBytes) {
        throw new VoicePresetFormatError(`voice preset section overlaps header (offset=${sec.offset} < header=${headerBytes})`, "bad-section-bounds");
    }
    if (sec.offset + sec.length > fileLen) {
        throw new VoicePresetFormatError(`voice preset section bounds exceed file length`, "bad-section-bounds");
    }
}
function readHeader(view) {
    if (view.byteLength < VOICE_PRESET_HEADER_BYTES_V1) {
        throw new VoicePresetFormatError(`voice preset file truncated: header needs ${VOICE_PRESET_HEADER_BYTES_V1} bytes, got ${view.byteLength}`, "truncated-header");
    }
    const magic = view.getUint32(0, true);
    if (magic !== VOICE_PRESET_MAGIC) {
        throw new VoicePresetFormatError(`voice preset bad magic: expected 0x${VOICE_PRESET_MAGIC.toString(16)}, got 0x${magic.toString(16)}`, "bad-magic");
    }
    const version = view.getUint32(4, true);
    if (version !== VOICE_PRESET_VERSION_V1 &&
        version !== VOICE_PRESET_VERSION_V2) {
        throw new VoicePresetFormatError(`voice preset unsupported version: ${version} (this build supports 1 and 2)`, "bad-version");
    }
    const headerBytes = version === VOICE_PRESET_VERSION_V2
        ? VOICE_PRESET_HEADER_BYTES_V2
        : VOICE_PRESET_HEADER_BYTES_V1;
    if (view.byteLength < headerBytes) {
        throw new VoicePresetFormatError(`voice preset file truncated: v${version} header needs ${headerBytes} bytes, got ${view.byteLength}`, "truncated-header");
    }
    const embedding = {
    ...
    if (version === VOICE_PRESET_VERSION_V2) {
        ...
        if (r0 !== 0 || r1 !== 0) {
            throw new VoicePresetFormatError(`voice preset v2 reserved header words must be 0 (got ${r0}, ${r1})`, "bad-section-bounds");
        }
    }
...
function copyInt32(bytes, relativeOffset, byteLength) {
    const aligned = new Uint8Array(byteLength);
    aligned.set(bytes.subarray(relativeOffset, relativeOffset + byteLength));
    return new Int32Array(aligned.buffer, 0, byteLength / 4);
}
function readEmbedding(bytes, sec) {
    if (sec.length === 0)
        return new Float32Array(0);
    if (sec.length % 4 !== 0) {
        throw new VoicePresetFormatError(`voice preset embedding length ${sec.length} is not a multiple of 4`, "bad-embedding-length");
    }
    return copyFloat32(bytes, sec.offset, sec.length);
}
function readRefAudioTokens(bytes, sec) {
    if (sec.length === 0) {
        return { K: 0, refT: 0, tokens: new Int32Array(0) };
    }
    if (sec.length < 8) {
        throw new VoicePresetFormatError(`voice preset ref_audio_tokens section truncated (need ≥ 8 bytes, got ${sec.length})`, "bad-ref-tokens");
    }
    ...
    if (tokenBytes % 4 !== 0) {
        throw new VoicePresetFormatError(`voice preset ref_audio_tokens payload bytes ${tokenBytes} is not a multiple of 4`, "bad-ref-tokens");
    }
    const expected = K * refT * 4;
    if (tokenBytes !== expected) {
        throw new VoicePresetFormatError(`voice preset ref_audio_tokens shape mismatch: K=${K}, ref_T=${refT}, expected ${expected} bytes, got ${tokenBytes}`, "bad-ref-tokens");
    }
    const tokens = tokenBytes === 0
        ? new Int32Array(0)
        : copyInt32(bytes, sec.offset + 8, tokenBytes);
    return { K, refT, tokens };
}
function readUtf8(bytes, sec) {
    if (sec.length === 0)
        return "";
    const slice = bytes.subarray(sec.offset, sec.offset + sec.length);
    return new TextDecoder("utf-8", { fatal: true }).decode(slice);
}
function readMetadata(bytes, sec) {
    if (sec.length === 0)
        return {};
    ...
    try {
        ...
        throw new VoicePresetFormatError(`voice preset metadata is not valid JSON: ${err.message}`, "bad-metadata");
    }
    if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
        throw new VoicePresetFormatError(`voice preset metadata must be a JSON object`, "bad-metadata");
    }
    return parsed;
}
function readPhrases(bytes, sec) {
    if (sec.length === 0)
        return [];
    const view = new DataView(bytes.buffer, bytes.byteOffset + sec.offset, sec.length);
    ...
    let pos = 0;
    if (sec.length < 4) {
        throw new VoicePresetFormatError("voice preset phrase section truncated before count", "truncated-section");
    }
    ...
    for (let i = 0; i < count; i++) {
        if (pos + 2 > sec.length) {
            throw new VoicePresetFormatError(`voice preset phrase #${i}: truncated before text length`, "bad-phrase-record");
        }
        const textLen = view.getUint16(pos, true);
        pos += 2;
        if (pos + textLen > sec.length) {
            throw new VoicePresetFormatError(`voice preset phrase #${i}: text overruns section`, "bad-phrase-record");
        }
        const textBytes = new Uint8Array(bytes.buffer, bytes.byteOffset + sec.offset + pos, textLen);
        const text = decoder.decode(textBytes);
        pos += textLen;
        if (pos + 8 > sec.length) {
            throw new VoicePresetFormatError(`voice preset phrase #${i}: truncated before sample_rate/pcm_len`, "bad-phrase-record");
        }
        ...
        pos += 4;
        if (pcmByteLen % 4 !== 0) {
            throw new VoicePresetFormatError(`voice preset phrase #${i}: pcm byte length ${pcmByteLen} is not a multiple of 4`, "bad-phrase-record");
        }
        if (pos + pcmByteLen > sec.length) {
            throw new VoicePresetFormatError(`voice preset phrase #${i}: pcm overruns section`, "bad-phrase-record");
        }
 ...
 */
export function readVoicePresetFile(bytes) {
...
export function writeVoicePresetFile(file) {
    ...
    for (let i = 0; i < file.phrases.length; i++) {
        ...
        if (t.byteLength > 0xffff) {
            throw new VoicePresetFormatError(`phrase #${i} text too long (${t.byteLength} bytes, max 65535)`, "bad-phrase-record");
        }
    ...
    // Embedding
    out.set(new Uint8Array(file.embedding.buffer, file.embedding.byteOffset, file.embedding.byteLength), embOff);
    // Phrases
    writePhraseSection(out, view, phrOff, file.phrases, encodedTexts);
    return out;
}
function writePhraseSection(out, view, startOff, phrases, encodedTexts) {
    let pos = startOff;
    ...
    for (let i = 0; i < phrases.length; i++) {
        ...
        pos += 4;
        out.set(new Uint8Array(phrase.pcm.buffer, phrase.pcm.byteOffset, phrase.pcm.byteLength), pos);
...
export function writeVoicePresetFileV2(file) {
    ...
    if (refAudioTokens.K * refAudioTokens.refT !== refAudioTokens.tokens.length) {
        throw new VoicePresetFormatError(`ref_audio_tokens shape mismatch: K=${refAudioTokens.K}, ref_T=${refAudioTokens.refT}, but tokens.length=${refAudioTokens.tokens.length}`, "bad-ref-tokens");
    }
    const encoder = new TextEncoder();
    ...
    const encodedMetadata = Object.keys(metadata).length === 0
        ? new Uint8Array(0)
    ...
    const refTokensBytes = refAudioTokens.tokens.length === 0 && refAudioTokens.K === 0
        ? 0
    ...
    if (embBytes > 0) {
        out.set(new Uint8Array(embedding.buffer, embedding.byteOffset, embBytes), embOff);
    }
    if (phrBytes > 0) {
    ...
    if (refTokensBytes > 0) {
        ...
        if (refAudioTokens.tokens.byteLength > 0) {
            out.set(new Uint8Array(refAudioTokens.tokens.buffer, refAudioTokens.tokens.byteOffset, refAudioTokens.tokens.byteLength), refTokensOff + 8);
        }
    }
...
//# sourceMappingURL=voice-preset-format.js.map