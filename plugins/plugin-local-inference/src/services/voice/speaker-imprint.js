...
export function normalizeVoiceEmbedding(embedding) {
    let sumSq = 0;
...
export function cosineSimilarity(left, right) {
    if (left.length !== right.length) {
        throw new Error(`[voice-imprint] embedding dimension mismatch: ${left.length} != ${right.length}`);
    }
...
export function updateVoiceImprintCentroid(args) {
    ...
    const observationWeight = clamp01(Math.min(args.maxObservationWeight ?? 1, args.observationConfidence ?? 1));
    ...
    if (args.centroidEmbedding.length !== observation.length) {
        throw new Error(`[voice-imprint] cannot update ${args.centroidEmbedding.length}-dim centroid with ${observation.length}-dim observation`);
    }
    const centroid = normalizeVoiceEmbedding(args.centroidEmbedding);
    const historicalWeight = Math.max(1, previousCount);
    const updated = centroid.map((value, i) => value * historicalWeight + observation[i] * observationWeight);
    return {
        centroidEmbedding: normalizeVoiceEmbedding(updated),
        sampleCount: previousCount + 1,
        confidence: clamp01(((args.confidence ?? 0) * previousCount + observationWeight) /
            (previousCount + 1)),
    };
}
export function matchVoiceImprint(args) {
    ...
    for (const profile of args.profiles) {
        ...
        if (similarity < threshold)
            continue;
        const confidence = clamp01(((similarity - threshold) / Math.max(0.0001, 1 - threshold)) *
...
export function attributeVoiceImprintObservations(args) {
    ...
    return {
        ...
        summary: {
            ...
            meanConfidence: confidenceCount === 0 ? 0 : clamp01(confidenceSum / confidenceCount),
            meanSimilarity: similarityCount === 0 ? null : similaritySum / similarityCount,
        },
    };
}
function selectPrimarySpeaker(segments) {
    ...
    for (const row of durations.values()) {
        if (!best || row.ms > best.ms)
            best = row;
    }
    return best?.speaker;
}
//# sourceMappingURL=speaker-imprint.js.map