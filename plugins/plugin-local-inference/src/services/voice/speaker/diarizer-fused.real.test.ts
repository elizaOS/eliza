...
describe.skipIf(!isBun || !LIB_PATH)("FusedDiarizer — real FFI", () => {
	...
	it("diarizeWindow() reduces real native labels into bounded segments", async () => {
		if (!diarizSupported) {
			throw new Error(
				`[test] the fused lib at ${LIB_PATH} (ABI v${ffi.libraryAbiVersion}) does not link the pyannote diarizer graph (eliza_inference_diariz_supported() == 0) — rebuild with the diarizer ABI to run this assertion.`,
			);
		}
