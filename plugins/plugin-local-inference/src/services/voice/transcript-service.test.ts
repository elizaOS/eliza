...
import { type CreateTranscriptInput } from "./transcript-service";
import { type Memory } from "@elizaos/core";
import { type Transcript } from "@elizaos/core/transcripts";
import { type TranscriptServiceRuntime } from "./transcript-service";
...
const access = (requesterEntityId: UUID, role: AccessContext["role"] = "USER"): AccessContext => ({
    requesterEntityId,
    worldId: WORLD,
    role,
    isOwner: role === "OWNER",
...
describe("TranscriptService", () => {
    it("mirrors the transcript into knowledge and links the document id", async () => {
        ...
        expect((opts.metadata as {
            transcriptId: string;
        }).transcriptId).toBe(t.id);
        ...
        expect(saved.knowledgeDocumentId).toBe("dddddddd-0000-0000-0000-000000000001");
        const got = await svc.get(t.id as UUID);
        expect(got?.knowledgeDocumentId).toBe("dddddddd-0000-0000-0000-000000000001");
    });
    ...
    it("applies an edit, re-derives metadata, and re-mirrors to knowledge", async () => {
        ...
        expect(updated?.knowledgeDocumentId).toBe("dddddddd-0000-0000-0000-000000000002");
    ...
    it("returns null when updating a transcript that does not exist", async () => {
        const rt = fakeRuntime({ withDocuments: true });
        const svc = new TranscriptService(rt);
        const result = await svc.update("99999999-0000-0000-0000-000000000000" as UUID, {
            worldId: WORLD,
            roomId: ROOM,
            entityId: ENTITY,
            patch: { title: "nope" },
    ...
    it("filters list/get by transcript scope when a requester context is supplied", async () => {
        ...
        expect((await svc.list(ROOM, undefined, access(ENTITY)))
            .map((t) => t.title)
            .sort()).toEqual(["Global", "User owned"]);
        expect((await svc.list(ROOM, undefined, access(OTHER_ENTITY)))
            .map((t) => t.title)
            .sort()).toEqual(["Global", "Other user"]);
        expect((await svc.list(ROOM, undefined, access("owner" as UUID, "OWNER"))).map((t) => t.title)).toHaveLength(5);
        expect((await svc.list(ROOM, undefined, access(rt.agentId, "USER"))).map((t) => t.title)).toHaveLength(5);
        expect(await svc.get(records[0].id as UUID, access(ENTITY))).toBeNull();
        expect((await svc.get(records[3].id as UUID, access(ENTITY)))?.title).toBe("User owned");
    });
});
