...
function resolveOpenVinoPython() {
...
export function resolveOpenVinoWhisperRuntime() {
    ...
    if (!modelDir)
        return null;
    const deviceChain = process.env.ELIZA_OPENVINO_WHISPER_DEVICE?.trim() ||
...
function bunOrThrow() {
    ...
    if (!bun || typeof bun.spawn !== "function") {
        throw new Error("[asr] OpenVINO whisper decoder requires the Bun runtime (Bun.spawn); production voice runs under Bun via Electrobun / Capacitor");
    }
    return bun;
}
/**
 ...
 */
export function makeOpenVinoWhisperDecoder(runtime) {
    ...
    proc.exited
        ...
        if (!disposed) {
            dead = true;
            deadReason = new Error(`[asr] OpenVINO whisper worker exited unexpectedly (code=${code})`);
        }
    })
        .catch(() => {
        /* ignore */
    });
    const decoder = (pcm16k) => {
        if (disposed) {
            return Promise.reject(new Error("[asr] OpenVINO whisper decoder has been disposed"));
        }
        if (dead) {
            return Promise.reject(deadReason ??
        ...
        const work = (async () => {
            ...
            const nBytes = new DataView(respHeader.buffer, respHeader.byteOffset, 4).getUint32(0, true);
            if (nBytes === 0)
                return "";
...
//# sourceMappingURL=openvino-whisper-asr.js.map