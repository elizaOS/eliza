...
export class TranscriptPrivacyService {
    ...
    async updateArtifactSharing(transcriptId: UUID, sharing: Partial<TranscriptCaptureSharingState>): Promise<Transcript> {
        ...
        else if (transcriptState === "restricted" ||
            transcriptState === "shared") {
            const grants = ((row.metadata as Record<string, unknown> | undefined)?.share as {
                grants?: Array<{
                    mode?: unknown;
                }>;
            } | undefined)?.grants;
            const requiredMode = transcriptState === "restricted" ? "redacted" : "full";
            if (!grants?.some((grant) => grant.mode === requiredMode)) {
                throw new ElizaError(`${transcriptState} visibility requires an existing ${requiredMode} grant`, {
    ...
    async deleteSourceAudio(transcriptId: UUID): Promise<Transcript> {
        ...
        if (!fileName) {
            ...
            throw new ElizaError("source audio is not a content-addressed media handle", { code: "TRANSCRIPT_SOURCE_AUDIO_NOT_STORED" });
        }
