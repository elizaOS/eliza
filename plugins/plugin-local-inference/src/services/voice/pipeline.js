...
export function splitTranscriptToTokens(transcript, startIndex = 0, tokenIds) {
    ...
    const ids = tokenIds && tokenIds.length === parts.length ? tokenIds : undefined;
    let i = startIndex;
...
export class VoicePipeline {
    ...
    constructor(deps, config, events = {}) {
        ...
        this.maxGeneratedTokens = Math.max(1, Math.floor(config.maxGeneratedTokens ?? DEFAULT_MAX_GENERATED_TOKENS));
        this.events = events;
    ...
    async run(audio) {
        if (this.active) {
            throw new Error("[voice-pipeline] a turn is already running; cancel() it or await the previous run() first");
        }
    ...
    async execute(audio, cancel) {
        ...
        let nextIndex = asrTokens.length > 0 ? asrTokens[asrTokens.length - 1].index + 1 : 0;
        let generated = 0;
        ...
        for (;;) {
            ...
            for (const t of indexedDraft) {
                if (generated + speculated >= this.maxGeneratedTokens)
                    break;
            ...
            const acceptedFromDraft = countMatchingPrefix(result.accepted, indexedDraft);
...
function countMatchingPrefix(accepted, draft) {
    ...
    let i = 0;
    while (i < n && accepted[i].text === draft[i].text)
        i++;
    return i;
}
