...
import { KokoroOnnxRuntime, KokoroTtsBackend, } from "@elizaos/shared/local-inference";
...
import { DEFAULT_PHRASE_CACHE_SEED, FIRST_AUDIO_FILLERS, PhraseCache, } from "./phrase-cache";
import { VoicePipeline, } from "./pipeline";
import { LlamaServerDraftProposer, LlamaServerTargetVerifier, MissingAsrTranscriber, } from "./pipeline-impls";
import { VoiceScheduler } from "./scheduler";
import { SharedResourceRegistry, } from "./shared-resources";
import { DEFAULT_VOICE_PRESET_REL_PATH, SpeakerPresetCache, } from "./speaker-preset-cache";
...
function ffiSpeakerPresetId(preset) {
    const hasV2Payload = (preset.instruct !== undefined && preset.instruct.length > 0) ||
...
export function isStreamingTtsBackend(backend) {
    return (typeof backend.synthesizeStream ===
        "function");
}
/**
...
export class StubOmniVoiceBackend {
    ...
    async synthesize(args) {
        this.calls++;
        args.onKernelTick?.();
        const samples = Math.floor((this.sampleRate * STUB_PCM_MS_PER_PHRASE) / 1000);
    ...
    async synthesizeStream(args) {
        this.streamCalls++;
        const totalSamples = Math.floor((this.sampleRate * STUB_PCM_MS_PER_PHRASE) / 1000);
        const perChunk = Math.max(1, Math.ceil(totalSamples / STUB_PCM_STREAM_CHUNKS));
        let cancelled = false;
            ...
            args.getContext ??
                (() => {
                    if (args.ctx === undefined) {
                        throw new VoiceStartupError("missing-fused-build", "[voice] FFI backend has no context provider");
                    }
                    return args.ctx;
                });
...
export class EngineVoiceBridge {
    ...
    activePipeline = null;
    constructor(scheduler, backend, bundleRoot, lifecycle, ffi, ffiContextRef, asrAvailable, phraseCache) {
    ...
    dispose() {
        if (this.ffi) {
            ...
            if (ctx !== null) {
                ...
                if (this.ffiContextRef)
                    this.ffiContextRef.current = null;
            }
            this.ffi.close();
        }
    }
    ...
    static start(opts) {
        if (opts.kokoroOnly) {
            if (opts.useFfiBackend || opts.backendOverride) {
                throw new VoiceStartupError("invalid-options", "[voice] kokoroOnly cannot be combined with useFfiBackend or backendOverride. Caller must pick exactly one backend path.");
            }
            return EngineVoiceBridge.startKokoroOnly(opts);
        }
        if (!opts.bundleRoot || !existsSync(opts.bundleRoot)) {
            throw new VoiceStartupError("missing-bundle-root", `[voice] Bundle root does not exist: ${opts.bundleRoot}`);
        }
        const presetPath = path.join(opts.bundleRoot, DEFAULT_VOICE_PRESET_REL_PATH);
        if (!existsSync(presetPath)) {
            throw new VoiceStartupError("missing-speaker-preset", `[voice] Bundle is missing required speaker preset at ${presetPath}. The default voice MUST ship as a precomputed embedding (AGENTS.md §4).`);
        }
        ...
        if (opts.backendOverride && opts.useFfiBackend) {
            throw new VoiceStartupError("missing-fused-build", "[voice] backendOverride cannot be combined with useFfiBackend=true. Voice-on production paths must load libelizainference and verify its ABI instead of bypassing the fused runtime.");
        }
        if (opts.backendOverride) {
            ...
            if (!existsSync(libPath)) {
                throw new VoiceStartupError("missing-ffi", `[voice] Fused omnivoice library not found under ${path.join(opts.bundleRoot, "lib")} (tried ${libraryFilenames().join(", ")}). Build via packages/app-core/scripts/build-llama-cpp-dflash.mjs (omnivoice-fuse target).`);
            }
            ...
            const contextRef = {
                current: null,
                    ...
                    if (!ffiHandle) {
                        throw new VoiceStartupError("missing-ffi", "[voice] FFI context requested without a loaded libelizainference handle");
                    }
        ...
        const config = {
            chunkerConfig: {
                maxTokensPerPhrase: opts.maxTokensPerPhrase ??
                    readPositiveIntEnv("ELIZA_VOICE_MAX_TOKENS_PER_PHRASE") ??
                    PHRASE_MAX_TOKENS_DEFAULT,
            },
            preset,
            ringBufferCapacity: opts.ringBufferCapacity ?? RING_BUFFER_CAPACITY_DEFAULT,
            sampleRate,
            maxInFlightPhrases: opts.maxInFlightPhrases ??
                readPositiveIntEnv("ELIZA_VOICE_MAX_IN_FLIGHT_PHRASES"),
        };
        const sinkOverride = opts.sink;
        const scheduler = new VoiceScheduler(config, sinkOverride
            ? { backend, sink: sinkOverride, phraseCache }
            : { backend, phraseCache }, opts.events ?? {});
        ...
        const loaders = opts.lifecycleLoaders ??
            defaultLifecycleLoaders(opts.bundleRoot, ffiHandle, ffiContextRef);
        const lifecycle = new VoiceLifecycle({ registry, loaders });
        return new EngineVoiceBridge(scheduler, backend, opts.bundleRoot, lifecycle, ffiHandle, ffiContextRef, asrAvailable, phraseCache);
    }
    /**
    ...
    static startKokoroOnly(opts) {
        if (!opts.kokoroOnly) {
            throw new VoiceStartupError("invalid-options", "[voice] startKokoroOnly called without `kokoroOnly` config — this is an internal error.");
        }
        const kokoro = opts.kokoroOnly;
        const sampleRate = opts.sampleRate ?? kokoro.layout.sampleRate;
        const workDir = opts.bundleRoot && existsSync(opts.bundleRoot)
            ? opts.bundleRoot
            : localInferenceRoot();
        ...
        const lifecycle = new VoiceLifecycle({ registry, loaders });
        return new EngineVoiceBridge(scheduler, backend, workDir, lifecycle, null, // no FFI handle on Kokoro-only
    ...
    async synthesizeTextToWav(text, signal) {
        ...
        if (!this.hasRealTtsBackend()) {
            throw new VoiceStartupError("missing-fused-build", "[voice] Direct speech synthesis requires a fused OmniVoice backend. The stub backend is only allowed in scheduler/unit tests.");
        }
    ...
    subscribeNativeVerifier(onEvent) {
        if (!this.ffi) {
            throw new VoiceStartupError("missing-ffi", "[voice] subscribeNativeVerifier requires a loaded fused libelizainference handle");
        }
        const ctx = this.ffiContextRef
            ? this.ffiContextRef.ensure()
            : (() => {
                throw new VoiceStartupError("missing-ffi", "[voice] subscribeNativeVerifier: no FFI context provider");
            })();
    ...
    assertVoiceOn(action) {
        ...
        if (state.kind === "voice-on")
            return;
        if (state.kind === "voice-error") {
            throw state.error;
        }
        throw new VoiceLifecycleError("illegal-transition", `[voice] Cannot ${action} while lifecycle is ${state.kind}. Call armVoice() and wait for voice-on first.`);
    }
}
...
export function decodeMonoPcm16Wav(bytes) {
    ...
    if (audioFormat !== 1 || channels !== 1 || bitsPerSample !== 16) {
        throw new Error(`[voice] Local transcription expects mono PCM16 WAV (format=1 channels=1 bits=16); got format=${audioFormat} channels=${channels} bits=${bitsPerSample}`);
    }
    let pos = 36;
...
function ensureContext(ref) {
    if (ref === null)
        return null;
    if (typeof ref === "object" && "ensure" in ref)
        return ref.ensure();
    return ref;
}
/**
...
function defaultLifecycleLoaders(bundleRoot, ffi, ctx) {
    return {
        loadTtsRegion: async () => bundleMmapRegion(path.join(bundleRoot, "tts"), "tts", ffi, ctx),
        loadAsrRegion: async () => bundleMmapRegion(path.join(bundleRoot, "asr"), "asr", ffi, ctx),
        loadVoiceCaches: async () => ({
...
function bundleMmapRegion(dir, kind, ffi, ctx) {
    if (!existsSync(dir)) {
        throw new Error(`[voice] mmap MAP_FAILED: ${kind} directory missing at ${dir}`);
    }
    if (!directoryHasRegularFile(dir)) {
        throw new Error(`[voice] mmap MAP_FAILED: ${kind} directory has no model files at ${dir}`);
    }
...
function locateBundleLibrary(bundleRoot) {
    ...
    return path.join(dirs[0] ?? path.join(bundleRoot, "lib"), libraryFilenames()[0] ?? "libelizainference.so");
}
