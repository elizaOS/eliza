...
import { TRANSCRIPTS_TABLE } from "./transcript-store";
...
import { type TranscriptStoreRuntime } from "./transcript-store";
...
function fakeRuntime(): TranscriptStoreRuntime & {
    ...
    return {
        ...
        async getMemories({ tableName, roomId, orderDirection }) {
            let out = [...rows.values()].filter((m) => tables.get(m.id as string) === tableName);
            if (roomId)
                out = out.filter((m) => m.roomId === roomId);
            out.sort((a, b) => orderDirection === "asc"
...
describe("TranscriptStore", () => {
    it("persists into the transcripts partition and round-trips the full record", async () => {
        ...
        expect(typeof (row.content as {
            transcript: string;
        }).transcript).toBe("string");
    ...
    it("returns null for a missing id and deletes a record", async () => {
        const rt = fakeRuntime();
        const store = new TranscriptStore(rt);
        expect(await store.get("deadbeef-0000-0000-0000-000000000000" as UUID)).toBeNull();
        const t = makeTranscript();
        await store.create({ roomId: ROOM, entityId: ENTITY, transcript: t });
    ...
    describe("per-viewer disclosure selection (#14781)", () => {
        ...
        it("projects source audio, notes, and generated artifacts independently", async () => {
            ...
            expect(viewer?.metadata?.retention).not.toHaveProperty("sourceAudioFileName");
            expect((await store.list(ROOM, 100, {
                requesterEntityId: VIEWER,
                role: "USER",
            }))[0]?.hasAudio).toBe(true);
            expect((await store.get(ORIGINAL_ID as UUID))?.metadata).toHaveProperty("notes");
        });
        ...
        it("share grants and variant links survive a text edit (update preserves metadata)", async () => {
            ...
            const viewer = { requesterEntityId: VIEWER, role: "USER" as const };
            expect((await store.get(ORIGINAL_ID as UUID, viewer))?.redacted).toBe(true);
        });
        it("creates a deterministic redacted variant without mutating the original or audio", async () => {
            ...
            const variantMeta = (rt.rows.get(first.id) as Memory).metadata as Record<string, unknown>;
        ...
        it("rejects share writes for missing, denied, or revoked consent", async () => {
            for (const state of [undefined, "denied", "revoked"] as const) {
                ...
                const metadata = rt.rows.get(ORIGINAL_ID)?.metadata as Record<string, unknown> | undefined;
