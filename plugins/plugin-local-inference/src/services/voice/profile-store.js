...
import { cosineSimilarity, DEFAULT_VOICE_IMPRINT_MATCH_THRESHOLD, } from "./speaker-imprint";
/** Canonical schema version for `vp_*.json` records. */
export const VOICE_PROFILE_RECORD_SCHEMA_VERSION = "eliza.voice_profile_record.v1";
...
function centroidToBuffer(centroid) {
    const arr = new Float32Array(centroid);
    return Buffer.from(arr.buffer, arr.byteOffset, arr.byteLength);
}
function deriveProfileId(centroid) {
...
export function welfordUpdate(args) {
    ...
    if ((args.mean.length !== 0 && args.mean.length !== dim) ||
...
export function welfordVariance(m2, count) {
...
export class VoiceProfileStore {
    ...
    constructor(options) {
        ...
        this.coldDiskMax = Math.max(this.hotCacheSize, options.coldDiskMax ?? DEFAULT_COLD_DISK);
        this.matchThreshold =
    ...
    async readIndex() {
        ...
        try {
            ...
            if (!parsed.entries)
                parsed.entries = [];
            if (!parsed.nextLruRank)
                parsed.nextLruRank = 1;
            parsed.version = 1;
    ...
    async readProfileFromDisk(profileId) {
        try {
    ...
    async writeProfileToDisk(record) {
    ...
    async upsertIndexEntry(record) {
        ...
        const existing = index.entries.findIndex((e) => e.profileId === record.profileId);
        const entry = {
    ...
    async enforceColdLimit(index) {
        if (index.entries.length <= this.coldDiskMax)
            return;
        ...
        while (index.entries.length > this.coldDiskMax &&
    ...
    async findBestMatch(args) {
        ...
        for (const entry of index.entries) {
            if (entry.embeddingModel !== args.embeddingModel)
                continue;
            if (entry.embeddingDim !== args.embedding.length)
                continue;
            ...
            if (similarity < this.matchThreshold)
                continue;
            const confidence = Math.max(0, Math.min(1, ((similarity - this.matchThreshold) /
    ...
    async refine(args) {
        ...
        if (record.embeddingDim !== args.embedding.length) {
            throw new Error(`[VoiceProfileStore.refine] embedding dim mismatch: ${record.embeddingDim} vs ${args.embedding.length}`);
        }
        ...
        let sumSq = 0;
        for (let i = 0; i < w.mean.length; i += 1)
            sumSq += w.mean[i] * w.mean[i];
        ...
        const updated = {
            ...
            confidence: Math.max(0, Math.min(1, (record.confidence * record.sampleCount +
    ...
    async deleteProfile(args) {
        ...
        if (record.entityId && !args.allowBoundEntity) {
            throw new Error(`[VoiceProfileStore.deleteProfile] refusing to delete ${args.profileId}: bound to entity ${record.entityId}`);
        }
