...
export class BargeInController {
    ...
    setAgentSpeaking(speaking) {
        if (this.agentSpeaking === speaking)
            return;
    ...
    onWordsDetected(args) {
        if (args.wordCount < 1)
            return;
        const withinConfirmWindow = this.wordConfirmExpiresAtMs != null &&
     ...
     * token until `reset()`.
     */
    hardStop(reason = "manual", timestampMs = this.lastEventTimestampMs || Date.now()) {
        this.clearWordConfirm();
        this.awaitingWordConfirm = false;
        if (!this.activeToken) {
    ...
    armWordConfirmDeadline(timestampMs) {
        ...
        if (this.wordConfirmDeadlineTimer &&
            typeof this.wordConfirmDeadlineTimer.unref ===
                "function") {
