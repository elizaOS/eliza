...
import { downloadedAssetName } from "./wake-word-staging";
...
import { planWakeWordStaging } from "./wake-word-staging";
import { stageWakeWordModel } from "./wake-word-staging";
...
describe("wake-word staging plan (#9880)", () => {
    ...
    it("maps each downloaded GGUF onto the loader's wake/<head>.<kind>.gguf layout", () => {
        if (!wake)
            throw new Error("wakeword version missing");
        const plan = planWakeWordStaging(wake, "/state/models/voice", "/state/local-inference/wake");
        ...
        expect(byDest["/state/local-inference/wake/hey-eliza.melspec.gguf"]).toBe("/state/models/voice/wakeword-0.3.0-hey-eliza.melspec.gguf");
        expect(byDest["/state/local-inference/wake/hey-eliza.embedding.gguf"]).toBe("/state/models/voice/wakeword-0.3.0-hey-eliza.embedding.gguf");
        expect(byDest["/state/local-inference/wake/hey-eliza.classifier.gguf"]).toBe("/state/models/voice/wakeword-0.3.0-hey-eliza.classifier.gguf");
    });
    it("returns no copies for a non-wakeword model", () => {
        const vad = latestVoiceModelVersion("vad");
        if (!vad)
            throw new Error("vad version missing");
    ...
    it("stageWakeWordModel copies the downloaded GGUFs into the wake dir", async () => {
        ...
        for (const asset of wake.ggufAssets) {
            await writeFile(path.join(bundleVoiceDir, downloadedAssetName(wake, asset.filename)), "GGUF");
        }
    ...
    it("downloadedAssetName prefixes id + version", () => {
        expect(downloadedAssetName({ id: "wakeword", version: "0.3.0" }, "voice/wakeword/hey-eliza.melspec.gguf")).toBe("wakeword-0.3.0-hey-eliza.melspec.gguf");
    });
});
