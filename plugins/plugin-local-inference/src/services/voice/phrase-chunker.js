 ...
 */
function resolveDefaultMaxAccumulationMs() {
...
const DEFAULT_CLOCK = () => globalThis.performance?.now?.() ?? Date.now();
export class PhraseChunker {
    ...
    clock;
    firstTokenAtMs = 0;
    constructor(config, tokenizer = null, clock = DEFAULT_CLOCK) {
        this.terminators = config.sentenceTerminators ?? DEFAULT_TERMINATORS;
        this.chunkOn = config.chunkOn ?? "punctuation";
        this.phonemesPerChunk = Math.max(1, config.phonemesPerChunk ?? DEFAULT_PHONEMES_PER_CHUNK);
        this.maxTokensPerPhrase = Math.max(1, config.maxTokensPerPhrase ?? DEFAULT_MAX_TOKENS_PER_PHRASE);
        ...
        if (this.chunkOn === "phoneme-stream" && this.tokenizer === null) {
            throw new Error("PhraseChunker: chunkOn='phoneme-stream' requires a PhonemeTokenizer (pass CharacterPhonemeStub for testing or a real tokenizer for production)");
        }
    }
    push(token) {
...
export function chunkTokens(tokens, config, acceptedAt = 0, tokenizer = null) {
...
//# sourceMappingURL=phrase-chunker.js.map