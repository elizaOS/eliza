...
- [~] iOS **physical device** — a device IS connected (Shaw's iPhone 15 Pro) + a dev cert (w/ private key) + an ASC API key exist; the full-Bun **engine + LlamaCpp xcframeworks compiled for device-arm64** and the build failed **only at code-signing** (`ios-device-build-signing.md`): Xcode has no Apple ID account configured (automatic signing can't generate profiles), the ASC key's issuer-id is a CI-only secret, and the milady app's FamilyControls/DeviceActivity extensions need authorized entitlements. **To finish:** sign into Xcode → Settings → Accounts (or wire the ASC issuer-id) + authorize the team for the screen-time entitlements. The on-device *inference* is not in question — the identical Metal fused engine + GGUF runs real voice models on this Mac's Apple Silicon GPU (same arch as the A17 Pro).
