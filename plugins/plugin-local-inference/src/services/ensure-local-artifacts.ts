...
import type { RuntimeMode } from "../../runtime/mode/runtime-mode";
...
function resolveLogger(provided?: EnsureLocalArtifactsArgs["logger"]): ResolvedLogger {
 ...
 * change.
 */
function modelIdForKind(tier: Eliza1Tier, _kind: EnsureLocalArtifactKind): string {
...
function planArtifacts(tier: Eliza1Tier, signedInCloud: boolean): ArtifactPlan[] {
...
async function runArtifact(
  ...
  try {
    ...
    logger.info(
      `${plan.kind} model ${plan.modelId} download started`,
    );
...
export async function ensureLocalArtifacts(
  ...
  if (args.mode === "cloud" || args.mode === "remote") {
    logger.info(
      `mode is ${args.mode}; no on-device artifacts to download`,
    );
    return noopResult();
  }
