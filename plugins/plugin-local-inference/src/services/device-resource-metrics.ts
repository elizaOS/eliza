...
export class DeviceResourceMetrics {
  ...
  recordResourceSample(sample: ResourceSample): void {
    ...
    if (sample.thermalState != null) {
      ...
      if (this.thermalInitial === null) this.thermalInitial = sample.thermalState;
      const known = sample.thermalState !== "unknown";
      if (known) {
        this.thermalKnownCount += 1;
        const rank = THERMAL_RANK[
    ...
    if (typeof sample.lowPowerMode === "boolean") {
      ...
      if (this.lowPowerLast !== null && this.lowPowerLast !== sample.lowPowerMode)
  ...
  private rssSummary(): RssSummary {
    ...
    const growthMb = firstMb !== null && lastMb !== null ? lastMb - firstMb : null;
    ...
    return { firstMb, lastMb, peakMb, steadyMb, samples: n, growthMb, leakSuspected };
  }
