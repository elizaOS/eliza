...
// ---------------------------------------------------------------------------
function isNonNegativeIntArray(value) {
 ...
 */
export function parseDflashStreamEvent(raw) {
    ...
    if (kind === "dflash-verify") {
        if (!isNonNegativeInt(obj.drafted_count) ||
            !isNonNegativeInt(obj.accept_count)) {
            return null;
        }
        if (obj.accept_count > obj.drafted_count)
            return null;
        if (typeof obj.reject_index !== "number" ||
            !Number.isInteger(obj.reject_index) ||
            obj.reject_index < -1) {
            return null;
        }
        if (obj.correction_token_id !== null) {
            if (!isNonNegativeInt(obj.correction_token_id))
                return null;
        }
        if (!isFiniteNumber(obj.verify_latency_ms) ||
            obj.verify_latency_ms < 0) {
            return null;
        }
        return {
            kind: "dflash-verify",
            drafted_count: obj.drafted_count,
            accept_count: obj.accept_count,
            reject_index: obj.reject_index,
            correction_token_id: obj.correction_token_id === null
                ? null
                : obj.correction_token_id,
            verify_latency_ms: obj.verify_latency_ms,
        };
    }
    ...
    switch (kind) {
        ...
        case "speculate-end": {
            ...
            if (obj.totalAccepted > obj.totalDrafted)
                return null;
            return {
                kind: "speculate-end",
                round: obj.round,
 ...
 * and falls back to the legacy synthesis path.
 */
export function expandDflashBatchEvent(batch) {
    ...
    if (batch.accept_count !== accepted.length)
        return [];
    if (accepted.length > drafted.length)
        return [];
    const timing = {
    ...
    if (range && range[1] >= range[0] && range[0] >= 0) {
        ...
        const correctedToken = accepted.length > 0 ? accepted[accepted.length - 1] : null;
 ...
 */
export function parseDflashBatchEvent(raw) {
    if (!raw || typeof raw !== "object")
        return null;
 ...
 */
export function parseDflashFieldFromSseChunk(parsed) {
    if (!parsed || typeof parsed !== "object")
        return [];
    const field = parsed.dflash;
 ...
 */
export function computeAcceptanceRate(events) {
    let drafted = 0;
    let accepted = 0;
...
export function summarizeEvents(events) {
    ...
    for (const event of events) {
        ...
        else if (event.kind === "speculate-start" ||
...
//# sourceMappingURL=dflash-event-schema.js.map