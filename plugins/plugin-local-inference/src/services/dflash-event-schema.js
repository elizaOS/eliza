...
export function parseDflashStreamEvent(raw) {
	...
	if (kind === "dflash-verify") {
		...
		if (obj.accept_count > obj.drafted_count) return null;
		if (
		...
		if (!isFiniteNumber(obj.verify_latency_ms) || obj.verify_latency_ms < 0) {
			return null;
		}
		return {
			kind: "dflash-verify",
			...
			correction_token_id:
				obj.correction_token_id === null ? null : obj.correction_token_id,
...
export function parseDflashBatchEvent(raw) {
	...
	if (raw.type !== "dflash_event") return null;
