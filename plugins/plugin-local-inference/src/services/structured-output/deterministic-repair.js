...
function scanJsonValueEnd(text, start) {
    ...
    const first = text[i];
    if (first === '"')
        return scanJsonStringEnd(text, i);
    if (first === "{" || first === "[") {
        const stack = [first];
        let inString = false;
        let escaped = false;
    ...
    const numberMatch = /^-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][-+]?\d+)?/.exec(literal);
    return numberMatch ? i + numberMatch[0].length : null;
}
function enumOutputs(span, prev, next) {
    const values = Array.isArray(span.enumValues) && span.enumValues.length > 0
        ? span.enumValues
    ...
    const rawInsideQuotes = prevLiteral.endsWith('"') && nextLiteral.startsWith('"');
    return values.map((value) => rawInsideQuotes ? String(value) : JSON.stringify(String(value)));
}
...
function deterministicValueForSchema(schema) {
    ...
    if (singleEnum !== null)
        return JSON.stringify(singleEnum);
    if (!isRecord(schema))
        return null;
...
function completeSkeleton(text, skeleton, schemasByKey) {
    ...
    for (let i = 0; i < spans.length; i += 1) {
        ...
        if (span.kind === "enum") {
            ...
            if (!match) {
                const exact = candidates.find((candidate) => remainder.startsWith(candidate));
                if (!exact)
                    return { text, status: "invalid", reason: "enum-mismatch" };
                pos += exact.length;
                continue;
            }
            if (!match.complete)
                out = out.slice(0, pos) + match.value;
        ...
        if (span.kind === "free-json" && span.key && schemasByKey?.[span.key]) {
            const repaired = repairJsonObjectAgainstSchema(out.slice(pos), schemasByKey[span.key]);
            if (repaired.text !== out.slice(pos) &&
...
export function repairStructuredOutput(text, options) {
    ...
    if (options.skeleton) {
        result = completeSkeleton(result.text, options.skeleton, options.jsonSchemasByKey);
    }
    return result;
}
...
export class StructuredOutputRepairStream {
    ...
    push(chunk) {
        ...
        if (!repaired.text.startsWith(this.text))
            return incoming;
