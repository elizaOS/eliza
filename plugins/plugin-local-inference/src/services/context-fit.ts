 ...
 * ceiling.
 */
export function computeRuntimeContextFit(input: RuntimeContextFitInput): RuntimeContextFit | null {
    ...
    if (!Number.isFinite(input.weightMb) ||
    ...
    const q8MaxFittingContext = roundDownToStep(kvBudgetBytes / kvBytesPerToken, step);
    if (q8MaxFittingContext < minContext)
        return null;
    ...
    if (input.preferAccurateKvWhenHeadroom) {
        ...
        const f16MaxFittingContext = roundDownToStep(kvBudgetBytes / f16BytesPerToken, step);
