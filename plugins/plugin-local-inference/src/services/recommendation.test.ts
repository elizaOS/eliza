...
describe("local inference recommendations", () => {
	...
	it("falls back to the smallest tier on minimal mobile", () => {
		// 2B needs 4 GB minRam; below that the ladder collapses to 0.8B.
	...
	it("rejects a tier when its bundle exceeds the mobile memory guardrail", () => {
		...
		const desktop = findCatalogModel("eliza-1-4b");

		if (!desktop) throw new Error("eliza-1-4b missing from catalog");
	...
	it("still recommends Eliza-1 text on a binary that advertises BOTH DFlash kernels AND OpenVINO", () => {
		...
		// spawn time; the recommender just trusts `requiresKernel`.
		const probe = hardware({
			totalRamGb: 64,
			freeRamGb: 48,
			gpu: { backend: "cuda", totalVramGb: 24, freeVramGb: 22 },
	...
	it("rejects Eliza-1 text on an OpenVINO-only binary (no DFlash kernels)", () => {
		...
		// drops the tier; the `unsupportedKernels` field is documentation
		// in this case.
		const probe = hardware({
			totalRamGb: 64,
			freeRamGb: 48,
