...
describe("classifyRecommendationPlatform", () => {
  it("maps each hardware shape to its platform class", () => {
    expect(
      classifyRecommendationPlatform(probe({ mobile: { platform: "android" } })),
    ).toBe("mobile");
    expect(
      classifyRecommendationPlatform(
    ...
    expect(
      classifyRecommendationPlatform(probe({ platform: "linux", gpu: cudaGpu })),
    ).toBe("linux-gpu");
    expect(classifyRecommendationPlatform(probe({ platform: "linux" }))).toBe(
      "linux-cpu",
    );
    expect(
      classifyRecommendationPlatform(probe({ platform: "win32", gpu: cudaGpu })),
    ).toBe("desktop-gpu");
...
describe("deviceCapsFromProbe", () => {
  it("includes cpu for x86 and adds the probed GPU backend, RAM in MB", () => {
    expect(deviceCapsFromProbe(probe({ totalRamGb: 8, gpu: metalGpu }))).toEqual({
  ...
  it("requires NEON for an arm CPU backend to count", () => {
    expect(
      deviceCapsFromProbe(
        probe({ arch: "arm64", cpuFeatures: { neon: true } }),
      ).availableBackends,
    ).toEqual(["cpu"]);
    expect(
...
describe("selectBestQuantizationVariant", () => {
  const variant = (id: string, status: "published" | "planned") =>
    ({ id, label: "4-bit", ggufFile: `${id}.gguf`, sizeGb: 1, minRamGb: 2, status });
  ...
  it("prefers the default variant, then published, then first, else null", () => {
    ...
    expect(
      selectBestQuantizationVariant({} as CatalogModel),
    ).toBeNull();
  });
});
