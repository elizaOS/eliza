...
import { bundleHasAsrModelFiles } from "./asr-provenance";
...
import { normalizeEmbeddingVector } from "../runtime/embedding-vector-space";
import { prepareBgeEmbeddingInput } from "@elizaos/plugin-native-inference/model-catalog/bge-input";
import { readBundleAsrProvenanceBlockers } from "./asr-provenance";
...
import { verifyBgeEmbeddingFile } from "../runtime/embedding-vector-space";
...
export function deriveBundleDir(modelPath: string): string {
    ...
    try {
        ...
        logger.warn(`[BionicHostLoader] could not stage bionic bundle view for flat model "${modelPath}": ${err instanceof Error ? err.message : String(err)}`);
    }
    return "";
}
export class BionicHostLoader implements LocalInferenceLoader {
    ...
    async prepareEmbeddingModel(modelPath?: string): Promise<void> {
        const configured = modelPath ??
            path.resolve(process.env.MODELS_DIR?.trim() || elizaModelsDir(), process.env.LOCAL_EMBEDDING_MODEL?.trim() ||
        ...
        if (this.embeddingModelPath === absolute)
            return;
        try {
            ...
            if (candidates.length !== 1 ||
                ...
                throw new ElizaError("Bionic embeddings require an isolated BGE bundle", { code: "EMBEDDING_ARTIFACT_INVALID" });
            }
            ...
            throw new ElizaError("Install the pinned BGE-small artifact before using Bionic embeddings", {
    ...
    async embed(args: {
        ...
        if (Buffer.from(args.input, "utf8").toString("utf8") !== args.input) {
            throw new ElizaError("Embedding input contains unpaired UTF-16 surrogates", { code: "EMBEDDING_INPUT_INVALID" });
        }
        if (!this.embeddingModelPath)
            await this.prepareEmbeddingModel();
        ...
        if (byteLength > 1 << 20) {
            throw new ElizaError("Complete embedding request exceeds the Android host frame limit; split the source into explicit lossless chunks", {
        ...
        if (!wireResponse ||
            ...
            throw new ElizaError("Bionic host returned a malformed embedding response", { code: "EMBEDDING_VECTOR_INVALID" });
        }
        const response = wireResponse as Record<string, unknown>;
        if (response.ok !== true) {
            throw new ElizaError(typeof response.error === "string"
                ? response.error
        ...
        if (response.embeddingSpace !== BGE_SMALL_VECTOR_SPACE ||
            ...
            !tokenIds.every((id): id is number => typeof id === "number" && Number.isInteger(id) && id >= 0) ||
            tokens < 1 ||
            tokens > BGE_EMBEDDING_MODEL.contextSize) {
            throw new ElizaError("Bionic host returned invalid BGE embedding provenance or data", { code: "EMBEDDING_VECTOR_INVALID" });
        }
        assertBgeTokenAgreement(prepared, tokenIds);
        return {
            embedding: identifyEmbeddingVector(normalizeEmbeddingVector(vector), BGE_SMALL_VECTOR_SPACE),
            tokens,
        };
    }
    async loadModel(args: LocalInferenceLoadArgs): Promise<void> {
        ...
        logger.info(`[BionicHostLoader] active model ${args.modelPath} (bundle ${this.bundleDir || "<host-default>"})`);
    }
    ...
    async generate(args: {
        ...
        const res = args.onTextChunk
            ? await this.streamRoundTrip(typeof args.maxTokensPerStep === "number" && args.maxTokensPerStep > 0
                ? {
                    op: "generateStream",
                    ...request,
                    streamStep: Math.floor(args.maxTokensPerStep),
                }
                : { op: "generateStream", ...request }, args.onTextChunk)
            : await this.roundTrip<BionicGenerateResponse>({
                op: "generate",
                ...request,
            });
        if (!res.ok) {
            throw new Error(`[BionicHostLoader] host generate failed: ${res.error ?? "unknown error"}`);
        }
        ...
        if (typeof res.tokS === "number") {
            logger.debug(`[BionicHostLoader] generated ${res.tokens ?? "?"} tok @ ${res.tokS.toFixed(1)} tok/s on the bionic GPU host`);
        }
        return res.text ?? "";
    }
    /**
    ...
    async transcribe(args: {
        ...
        if (!this.bundleDir || !bundleHasAsrModelFiles(this.bundleDir)) {
            throw new Error("[BionicHostLoader] host asr requires an active Gemma ASR-capable bundle; refusing to use the bionic host default bundle");
        }
        const blockers = readBundleAsrProvenanceBlockers(this.bundleDir);
        if (blockers.length > 0) {
            throw new Error(`[BionicHostLoader] host asr refused non-Gemma ASR provenance: ${blockers.join("; ")}`);
        }
        ...
        if (!res.ok) {
            throw new Error(`[BionicHostLoader] host asr failed: ${res.error ?? "unknown error"}`);
        }
        return res.text ?? "";
    }
    /**
    ...
    async describeImage(args: {
        ...
        if (!res.ok) {
            throw new Error(`[BionicHostLoader] host image describe failed: ${res.error ?? "unknown error"}`);
        }
        return res.text ?? "";
    }
    /**
    ...
    private roundTrip<T>(request: Record<string, unknown>): Promise<T> {
        ...
        return new Promise<T>((resolve, reject) => {
            ...
            const timer = setTimeout(() => finish(new Error("[BionicHostLoader] request timed out")), REQUEST_TIMEOUT_MS);
            ...
            sock.on("data", (d: Buffer) => {
                ...
                if (expected < 0 && chunks.length >= 4) {
                    ...
                    if (expected < 0 || expected > MAX_FRAME_BYTES) {
                        finish(new Error(`[BionicHostLoader] bad response frame length ${expected}`));
                        return;
                    }
                }
                if (expected >= 0 && chunks.length >= 4 + expected) {
                    ...
                    try {
                        ...
                        finish(new Error(`[BionicHostLoader] malformed response: ${e instanceof Error ? e.message : String(e)}`));
                    }
                }
            });
            sock.on("error", (e: Error) => finish(new Error(`[BionicHostLoader] socket error: ${e.message}`)));
            sock.on("close", () => {
                if (!settled)
                    finish(new Error("[BionicHostLoader] host closed the connection before responding"));
            });
        });
    }
    /**
     ...
     */
    private streamRoundTrip(request: Record<string, unknown>, onTextChunk: (chunk: string) => void | Promise<void>): Promise<BionicGenerateResponse> {
        const payload = Buffer.from(JSON.stringify(request), "utf8");
        const frame = Buffer.allocUnsafe(4 + payload.length);
        frame.writeUInt32BE(payload.length, 0);
        payload.copy(frame, 4);
        return new Promise<BionicGenerateResponse>((resolve, reject) => {
            ...
            const finish = (err: Error | null, value?: BionicGenerateResponse) => {
                ...
                void chunkChain.then(() => {
                    if (chunkFailure) {
                        reject(new Error(`[BionicHostLoader] onTextChunk failed: ${chunkFailure.message}`));
                    }
                    else {
            ...
            let timer = setTimeout(() => finish(new Error("[BionicHostLoader] stream request timed out")), REQUEST_TIMEOUT_MS);
            const bumpIdleTimer = () => {
                clearTimeout(timer);
                timer = setTimeout(() => finish(new Error("[BionicHostLoader] stream stalled (no frames)")), REQUEST_TIMEOUT_MS);
            };
            sock.on("connect", () => sock.write(frame));
            sock.on("data", (d: Buffer) => {
                chunks = Buffer.concat([chunks, d]);
                ...
                for (;;) {
                    ...
                    if (expected < 0 || expected > MAX_FRAME_BYTES) {
                        finish(new Error(`[BionicHostLoader] bad stream frame ${expected}`));
                        return;
                    }
                    ...
                    try {
                        ...
                        finish(new Error(`[BionicHostLoader] malformed stream frame: ${e instanceof Error ? e.message : String(e)}`));
                        return;
                    }
                    if (msg.type === "token") {
                        const text = msg.text;
            ...
            sock.on("close", () => {
                if (!settled)
                    finish(new Error("[BionicHostLoader] host closed the stream before the done frame"));
            });
        });
    }
}
