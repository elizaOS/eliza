...
export function resolveMemoryMonitorConfig(overrides = {}) {
    const intervalMs = Math.max(1_000, overrides.intervalMs ??
        envInt("ELIZA_LOCAL_MEMORY_MONITOR_INTERVAL_MS") ??
        DEFAULT_CONFIG.intervalMs);
    return {
        intervalMs,
        lowWaterMb: overrides.lowWaterMb ??
            envInt("ELIZA_LOCAL_MEMORY_LOW_WATER_MB") ??
            DEFAULT_CONFIG.lowWaterMb,
        lowWaterFraction: overrides.lowWaterFraction ??
            envFloat("ELIZA_LOCAL_MEMORY_LOW_WATER_FRACTION") ??
            DEFAULT_CONFIG.lowWaterFraction,
        evictionCooldownMs: Math.max(0, overrides.evictionCooldownMs ?? DEFAULT_CONFIG.evictionCooldownMs),
        exhaustedBackoffMs: Math.max(0, overrides.exhaustedBackoffMs ?? DEFAULT_CONFIG.exhaustedBackoffMs),
    };
}
export class MemoryMonitor {
    ...
    start() {
        ...
        const timer = setInterval(() => {
            void this.tick().catch((err) => {
                this.log?.warn?.(`[MemoryMonitor] tick failed: ${err instanceof Error ? err.message : String(err)}`);
            });
    ...
    async sample() {
        ...
        const reserveMb = Math.max(this.config.lowWaterMb, Math.round(totalMb * this.config.lowWaterFraction));
        const serverHeadroomMb = serverRssMb !== null
    ...
    lowWaterMb(totalMb) {
        return Math.max(this.config.lowWaterMb, Math.round(totalMb * this.config.lowWaterFraction));
    }
    isUnderPressure(sample) {
    ...
    async tick(now = Date.now()) {
        ...
        try {
            ...
            if (evicted) {
                ...
                this.exhaustedUntilMs = 0;
                this.log?.info?.(`[MemoryMonitor] RAM pressure (free ${sample.effectiveFreeMb} MB < ${this.lowWaterMb(sample.totalMb)} MB low-water) — evicted ${evicted.role} (~${evicted.estimatedMb} MB)`);
            ...
            if (now >= this.exhaustedUntilMs) {
                ...
                this.log?.warn?.(`[MemoryMonitor] RAM pressure (free ${sample.effectiveFreeMb} MB) but no evictable model role — only the text target is resident. Consider a smaller tier (ELIZA_LOCAL_RAM_HEADROOM_MB / model selection).`);
            }
 ...
 */
async function defaultServerRssMb() {
    try {
