...
export const Eliza1LineageSchema = z.object({
	...
	// `files.turn` ships the bundled `livekit/turn-detector` GGUF (the
	// ≤1.7B-tier `v1.2.2-en` SmolLM2 distill or the ≥4B-tier `v0.4.1-intl`
	// pruned Qwen2.5-0.5B), this records the upstream repo + license.
	turn: lineageEntry.optional(),
	// Voice Wave 2 (2026-05-14): acoustic-prosody emotion classifier lineage.
	// When `files.emotion` ships the bundled `wav2small-msp-dim-int8.gguf`
	...
	// via `packages/training/scripts/emotion/distill_wav2small.py`).
...
export const Eliza1FilesSchema = z.object({
	...
	// a Silero-VAD GGUF + an optional openWakeWord GGUF (replacing the
	...
	// runtime loads the GGUF via `LiveKitTurnDetector` (which dispatches to
	// the GGML-backed `LiveKitGgmlTurnDetector`) and pre-warms it at
	// voice-session start. Tier mapping is data-driven:
	// 0_8b/2b ship the EN-only SmolLM2-135M distill (~40 MB Q8 GGUF);
	// 4b/9b/27b ship the multilingual pruned Qwen2.5-0.5B (~280 MB Q8 GGUF).
	...
	// (Wav2Small student, ~120 KB int8 GGUF). Optional — when omitted, the
	...
	// the runtime loads the GGUF via `VoiceEmotionClassifier` (ggml-backed),
	// runs it on `isFinal` transcript snapshots, and fuses the output with the
	// Stage-1 text-emotion field via the single fusion point in
	// `emotion-attribution.ts`.
