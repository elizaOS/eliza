...
describe("catalog gpuProfile wiring", () => {
	it("eliza-1-27b-256k maps to the H200 profile and demands the max native-context kernel set", () => {
		const bundle = findCatalogModel("eliza-1-27b-256k");
		expect(bundle).toBeDefined();
		if (!bundle) return;
		expect(bundle.gpuProfile).toBe("rtx-5090");
		expect(bundle.contextLength).toBe(262_144);
		const required = bundle.runtime?.optimizations?.requiresKernel ?? [];
		// Every kernel needed for 256k-context quantized KV: dflash, turbo3/4,
	...
	it("h200 profile's primary bundle covers a 256k context window", () => {
