...
import { RUNTIME_LOCAL_INFERENCE_RECOMMENDATION_POLICY } from "@elizaos/plugin-native-inference/model-catalog/recommendation";
import { SUPPORTED_BACKENDS_BY_TIER } from "./manifest";
import { assessCatalogModelFit as assessSharedCatalogModelFit } from "@elizaos/plugin-native-inference/model-catalog/recommendation";
import { canSetAsDefault } from "./manifest";
...
import { defaultManifestLoader } from "./ram-budget";
...
import { type CatalogQuantizationVariant } from "./types";
...
import { type HardwareFitLevel } from "./types";
...
import { type TextGenerationSlot } from "./types";
export type { RecommendationPlatformClass, RecommendedModelSelection };
export { catalogDownloadSizeBytes, catalogDownloadSizeGb, classifyRecommendationPlatform, recommendForFirstRun, };
export interface RecommendationOptions {
    binaryKernels?: Partial<Record<string, boolean>> | null;
    installed?: ReadonlyArray<InstalledModel>;
    manifestLoader?: ManifestLoader;
}
function sharedOptions(catalog: readonly CatalogModel[], options: RecommendationOptions): SharedRecommendationOptions {
    const installed = options.installed ?? [];
    const manifestLoader = options.manifestLoader ?? defaultManifestLoader;
    const ramBudgets = Object.fromEntries(catalog.map((model) => {
        const budget = resolveRamBudget(model, installed.find((entry) => entry.id === model.id), manifestLoader);
        return [
            model.id,
...
export function assessCatalogModelFit(hardware: HardwareProbe, model: CatalogModel, catalog: CatalogModel[] = MODEL_CATALOG, options: {
    ...
    return assessSharedCatalogModelFit(hardware, model, catalog, sharedOptions(catalog, runtimeOptions));
}
export function selectRecommendedModelForSlot(slot: TextGenerationSlot, hardware: HardwareProbe, catalog: CatalogModel[] = MODEL_CATALOG, options: RecommendationOptions = {}): RecommendedModelSelection {
    return selectSharedRecommendedModelForSlot(slot, hardware, catalog, sharedOptions(catalog, options));
}
export function selectRecommendedModels(hardware: HardwareProbe, catalog: CatalogModel[] = MODEL_CATALOG, options: RecommendationOptions = {}): Record<TextGenerationSlot, RecommendedModelSelection> {
    return selectSharedRecommendedModels(hardware, catalog, sharedOptions(catalog, options));
}
export function chooseSmallerFallbackModel(currentModelId: string, hardware: HardwareProbe, slot: TextGenerationSlot = "TEXT_LARGE", catalog: CatalogModel[] = MODEL_CATALOG, options: RecommendationOptions = {}): CatalogModel | null {
    return chooseSharedSmallerFallbackModel(currentModelId, hardware, slot, catalog, sharedOptions(catalog, options));
}
export function selectBestQuantizationVariant(model: CatalogModel): CatalogQuantizationVariant | null {
    const quantization = model.quantization;
    if (!quantization)
        return null;
    return (quantization.variants.find((variant) => variant.id === quantization.defaultVariantId) ??
...
export function deviceCapsFromProbe(hardware: HardwareProbe): Eliza1DeviceCaps {
    const backends: Eliza1Backend[] = hardware.arch === "arm64" || hardware.arch === "arm"
        ? hardware.cpuFeatures?.neon === true
            ? ["cpu"]
            : []
        : ["cpu"];
    if (hardware.gpu)
        backends.push(hardware.gpu.backend);
    return {
...
export function canBundleBeDefaultOnDevice(installed: InstalledModel, hardware: HardwareProbe, options: {
    manifestLoader?: ManifestLoader;
    ...
    if (canSetAsDefault(manifest, caps))
        return { canBeDefault: true };
    ...
    const supported = new Set<Eliza1Backend>(SUPPORTED_BACKENDS_BY_TIER[manifest.tier]);
    const verifiedOnDeviceBackend = caps.availableBackends.some((backend) => supported.has(backend) &&
