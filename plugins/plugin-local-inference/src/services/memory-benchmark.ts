 ...
 */
import { ELIZA_1_CONTEXT_TARGET } from "@elizaos/plugin-native-inference/model-catalog/device-fit";
import { ELIZA_1_KV_QUANT } from "@elizaos/plugin-native-inference/model-catalog/device-fit";
import { dirname } from "node:path";
import { mkdir } from "node:fs/promises";
import { performance } from "node:perf_hooks";
import { selectBestEliza1Fit } from "@elizaos/plugin-native-inference/model-catalog/device-fit";
import { type ArbiterEvent } from "./memory-arbiter";
import { type CatalogModel } from "./types";
import { type HardwareProbe } from "./types";
import { type InstalledModel } from "./types";
import { type ModelHubSnapshot } from "./types";
...
function estimateResidentMb(model: CatalogModel, installed: InstalledModel | undefined): number {
...
function inferFit(hardware: HardwareProbe, model: CatalogModel): "fits" | "tight" | "wontfit" {
    if (typeof model.minRamGb !== "number")
        return "wontfit";
    if (hardware.freeRamGb >= model.minRamGb)
        return "fits";
...
export function buildMemoryBenchmarkPlan(snapshot: {
    ...
    const installedById = new Map(snapshot.installed.map((model) => [model.id, model]));
...
function eventCounts(events: ArbiterEvent[]): MemoryBenchmarkReport["telemetry"] {
    return {
        ...
        memoryPressureEvents: events.filter((event) => event.type === "memory_pressure").length,
        events,
    };
}
async function loadAndMeasure(model: InstalledModel, options: Required<Pick<MemoryBenchmarkOptions, "prompt" | "maxTokens">>, deps: {
    ...
    try {
        ...
        return {
            ...
            generatedTokens,
            generatedTokensPerSec: generatedTokensPerSec === null
                ? null
...
function hostFromHardware(hardware: HardwareProbe): MemoryBenchmarkReport["host"] {
    return {
...
function deviceFitFromHardware(hardware: HardwareProbe): MemoryBenchmarkReport["deviceFit"] {
...
export async function buildMemoryBenchmarkReport(snapshot: Pick<ModelHubSnapshot, "catalog" | "installed" | "hardware">, events: ArbiterEvent[] = [], loads: MemoryBenchmarkLoadResult[] = []): Promise<MemoryBenchmarkReport> {
    return {
...
export async function runMemoryBenchmark(options: MemoryBenchmarkOptions = {}): Promise<MemoryBenchmarkReport> {
    const [{ localInferenceService }, { localInferenceEngine }] = await Promise.all([import("./service"), import("./engine")]);
    ...
    try {
        if (options.loadInstalled) {
            for (const installed of snapshot.installed.filter((model) => model.source === "eliza-download")) {
                loads.push(await loadAndMeasure(installed, { prompt, maxTokens }, { service: localInferenceService, engine: localInferenceEngine }));
...
export function summarizeMemoryBenchmark(report: MemoryBenchmarkReport): string {
