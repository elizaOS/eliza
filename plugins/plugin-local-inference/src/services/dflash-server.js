...
export function readDflashBinaryCapabilities(
	binaryPath = resolveDflashBinary(),
) {
...
function assertGgufAvailable(buf, offset, bytes) {
	if (offset + bytes > buf.length) {
		throw new Error(
			`GGUF metadata exceeds ${Math.floor(GGUF_METADATA_READ_LIMIT_BYTES / 1024 / 1024)} MiB validation window`,
		);
	}
}
...
function readDflashGgufMetadata(file) {
	...
	for (let i = 0; i < kvCount; i += 1) {
		...
		if (DFLASH_TOKENIZER_HASH_KEYS.includes(key) && valueEnd >= valueStart) {
			hashes.set(
				key,
				crypto
					.createHash("sha256")
...
async function fetchStreamingChatCompletion(
	...
	try {
		...
		const consumeEvent = async (raw) => {
			...
			const timingRecord =
				parsed && typeof parsed === "object" ? parsed.timings : null;
