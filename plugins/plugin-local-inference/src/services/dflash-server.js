...
import { buildModelHash, DEFAULT_CACHE_TTLS, deriveSlotId, evictExpired, readCacheStats, slotCacheFileName, slotSavePath, } from "./cache-bridge";
import { ELIZA_1_PLACEHOLDER_IDS, findCatalogModel } from "./catalog";
import { parseDflashFieldFromSseChunk, } from "./dflash-event-schema";
import { DflashMetricsCollector, dflashTurnHistory, } from "./dflash-metrics-collector";
import { parseDflashVerifyEventsFromSseChunk, } from "./dflash-verify-event";
import { probeHardware } from "./hardware";
import { inferenceTelemetry } from "./inference-telemetry";
import { estimateQuantizedKvBytesPerToken, KV_SPILL_MIN_CONTEXT, planKvSpill, residentKvBudgetFromRamBudget, restoreClassForHardware, } from "./kv-spill";
...
import { grammarRequestFields, prefillPlanRequestFields, repairStructuredOutput, resolveGuidedDecodeForParams, StructuredOutputRepairStream, spanSamplerPlanRequestFields, } from "./structured-output";
 ...
 */
const CACHE_TYPE_REQUIRED_KERNEL = {
    turbo3: "turbo3",
    turbo3_0: "turbo3",
    tbq3_0: "turbo3",
    turbo4: "turbo4",
...
export function parseDflashMetrics(body) {
    ...
    const readFirst = (aliases) => {
        for (const alias of aliases) {
            ...
            if (bucket !== undefined)
                return bucket.unlabeled ?? bucket.labeledSum;
        }
        return null;
    };
...
function envWithBinaryLibraryPath(binaryPath) {
    const env = { ...process.env };
    const binaryDir = path.dirname(binaryPath);
    const prependPath = (name) => {
...
function allowZeroDraftForDiagnostics() {
 ...
 */
export function useNativeDflashEvents() {
    return readBool("ELIZA_NATIVE_DFLASH_EVENTS");
}
export function shouldRequireActiveDflashForRequest(plan, maxTokens, observedOutputTokens) {
    ...
    if (Number.isFinite(maxTokens) && maxTokens != null) {
        if (maxTokens < minDraftableTokens)
            return false;
    }
    if (Number.isFinite(observedOutputTokens) && observedOutputTokens != null) {
        if (observedOutputTokens < minDraftableTokens)
            return false;
    }
    if (!Number.isFinite(maxTokens) || maxTokens == null)
        return true;
...
export function attachDflashSpeculativeRequestFields(payload, plan) {
    if (!plan || plan.disableDrafter)
        return;
    payload["speculative.n_min"] = Math.max(0, plan.draftMin);
    payload["speculative.n_max"] = Math.max(Math.max(0, plan.draftMin), plan.draftMax);
    payload["speculative.type"] = "dflash";
}
export function estimateOutputTokensForDflashEvidence(usage, text) {
 ...
 */
export function dflashDevDisabled() {
 ...
 */
export function logDflashDevDisabledWarning() {
    if (!dflashDevDisabled())
        return;
    console.warn("[local-inference] ⚠️  ELIZA_DFLASH_DISABLE=1 — speculative decoding is OFF. " +
 ...
 * stock build dir.
 */
function accelBackendKey(suffix) {
    const forced = process.env.ELIZA_DFLASH_BACKEND?.trim().toLowerCase();
    if (forced)
        return `${process.platform}-${process.arch}-${forced}${suffix}`;
    if (process.platform === "darwin") {
    ...
    for (const backend of ["cuda", "vulkan", "rocm"]) {
        const dir = path.join(localInferenceRoot(), "bin", "dflash", `${process.platform}-${process.arch}-${backend}${suffix}`);
...
function fusedBackendKey() {
    return accelBackendKey("-fused");
}
function managedFusedDflashDir() {
 ...
 * process.
 */
export function resolveFusedDflashBinary() {
    if (readBool("ELIZA_DFLASH_DISABLE_FUSED_SERVER"))
        return null;
    ...
    if (!caps)
        return null;
    const fused = caps.fused === true ||
        (caps.binaries ?? []).some((b) => /omnivoice/i.test(b) || /libelizainference/i.test(b));
...
function readCapabilitiesAt(capsPath) {
    let raw;
    try {
 ...
 */
export function readDflashBinaryCapabilities(binaryPath = resolveDflashBinary()) {
...
function helpAdvertisesCacheType(binaryPath, value) {
    const help = llamaServerHelpText(binaryPath).toLowerCase();
    if (!help)
        return false;
    return new RegExp(`\\b${escapeRegExp(value.toLowerCase())}\\b`).test(help);
}
function helpAdvertisesDflashSpecType(binaryPath) {
    const help = llamaServerHelpText(binaryPath).toLowerCase();
    if (!help)
        return false;
    return help.includes("--spec-type") && /\bdflash\b/.test(help);
}
function capabilitiesAreFreshForBinary(binaryPath) {
    try {
        const capsStat = fs.statSync(path.join(path.dirname(binaryPath), "CAPABILITIES.json"));
...
function capabilitiesAdvertiseDflashSpecType(binaryPath) {
    const caps = readDflashBinaryCapabilities(binaryPath);
    if (!caps)
        return false;
    return Boolean(capabilitiesAreFreshForBinary(binaryPath) &&
...
function helpAdvertisesFlag(binaryPath, flag) {
    return llamaServerHelpText(binaryPath).includes(flag);
}
export function appendDflashDraftTuningFlags(args, opts) {
    if (helpAdvertisesFlag(opts.binaryPath, "--ctx-size-draft")) {
        args.push("--ctx-size-draft", String(opts.draftContextSize));
    }
    args.push(helpAdvertisesFlag(opts.binaryPath, "--spec-draft-n-min")
        ? "--spec-draft-n-min"
        : "--draft-min", String(opts.draftMin));
    args.push(helpAdvertisesFlag(opts.binaryPath, "--spec-draft-n-max")
        ? "--spec-draft-n-max"
        : "--draft-max", String(opts.draftMax));
}
function assertCacheTypeSupportedOnBackend(name, value, binaryPath) {
    const requiredKernel = CACHE_TYPE_REQUIRED_KERNEL[value.toLowerCase()];
    if (!requiredKernel)
        return; // stock cache type (f16/q8_0/...) — always ok
    const caps = readDflashBinaryCapabilities(binaryPath);
    if (caps && caps.kernels[requiredKernel] !== true) {
        throw new Error(`${name}=${value} requires the '${requiredKernel}' kernel, but the installed llama-server binary's CAPABILITIES.json reports it absent. Rebuild the fork with the matching kernel patches (node packages/app-core/scripts/build-llama-cpp-dflash.mjs --target <triple>) or use a stock KV cache type (f16/q8_0).`);
    }
    if (!helpAdvertisesCacheType(binaryPath, value)) {
        throw new Error(`${name}=${value} requires llama-server at ${binaryPath} to advertise '${value}' in --cache-type-k/v allowed values. CAPABILITIES.json alone is not trusted because stale installs can lie; rebuild the fork or select a stock KV cache type (f16/q8_0).`);
    }
}
function assertDflashSpecSupportedOnBackend(binaryPath) {
    const caps = readDflashBinaryCapabilities(binaryPath);
    if (caps && caps.kernels.dflash !== true) {
        throw new Error(`[dflash] ${binaryPath} has CAPABILITIES.json but kernels.dflash is false; refusing to launch target-only because Eliza-1 requires DFlash. Rebuild packages/app-core/scripts/build-llama-cpp-dflash.mjs --target <triple>.`);
    }
    if (!helpAdvertisesDflashSpecType(binaryPath)) {
        ...
        if (capabilitiesAdvertiseDflashSpecType(binaryPath))
            return;
        throw new Error(`[dflash] ${binaryPath} does not advertise '--spec-type ... dflash' in --help; refusing to launch because Eliza-1 requires DFlash drafting.`);
    }
}
/**
 ...
 */
const ctxCheckpointsProbeCache = new Map();
/**
 ...
 */
export function probeCtxCheckpointsSupported(binaryPath) {
    ...
    if (!supported) {
        console.warn(`[local-inference] llama-server at ${binaryPath} does not advertise --ctx-checkpoints; optimistic-rollback voice path will run without mid-prefill snapshots until the upstream merge lands.`);
    }
 ...
 */
export function __resetCtxCheckpointsProbeCacheForTests() {
    ctxCheckpointsProbeCache.clear();
}
export function __setCtxCheckpointsProbeCacheForTests(binaryPath, supported) {
    ctxCheckpointsProbeCache.set(binaryPath, supported);
}
 ...
 */
export function appendCtxCheckpointFlags(args, optimizations, binaryPath, enableCheckpoints = true) {
    ...
    if (!probeCtxCheckpointsSupported(binaryPath))
        return;
    const ckpt = optimizations?.ctxCheckpoints ?? DEFAULT_CTX_CHECKPOINTS;
    const interval = optimizations?.ctxCheckpointInterval ?? DEFAULT_CTX_CHECKPOINT_INTERVAL;
    if (helpAdvertisesFlag(binaryPath, "--ctx-checkpoints") &&
        typeof ckpt === "number" &&
        Number.isInteger(ckpt) &&
        ckpt > 0) {
        args.push("--ctx-checkpoints", String(ckpt));
    }
    if (helpAdvertisesFlag(binaryPath, "--ctx-checkpoint-interval") &&
...
const disableThinkingProbeCache = new Map();
const llamaServerHelpTextOverrideForTests = new Map();
function llamaServerHelpText(binaryPath) {
...
export function __setLlamaServerHelpTextForTests(binaryPath, helpText) {
    llamaServerHelpTextOverrideForTests.set(binaryPath, helpText);
}
export function __resetLlamaServerHelpTextForTests() {
    llamaServerHelpTextOverrideForTests.clear();
}
export function resolveDisableThinkingFlags(binaryPath) {
    ...
    if (flags.length === 0) {
        console.warn(`[local-inference] llama-server at ${binaryPath} does not advertise reasoning/chat-template controls; disableThinking requested but no compatible flag is available.`);
    }
    disableThinkingProbeCache.set(binaryPath, flags);
    return [...flags];
}
export function appendMetalSafeStartupFlags(args, binaryPath) {
    const caps = readDflashBinaryCapabilities(binaryPath);
    if (caps?.backend !== "metal")
        return;
    const help = llamaServerHelpText(binaryPath);
...
const metalCompressedKvWarnings = new Set();
export function resolveMetalRuntimeCacheTypes(opts) {
    ...
    const usesMetalCompressedKv = (kLower !== undefined && METAL_COMPRESSED_KV_UNSUPPORTED.has(kLower)) ||
    ...
    const cacheTypeK = kLower !== undefined && METAL_COMPRESSED_KV_UNSUPPORTED.has(kLower)
        ? METAL_COMPRESSED_KV_FALLBACK
        : k;
    const cacheTypeV = vLower !== undefined && METAL_COMPRESSED_KV_UNSUPPORTED.has(vLower)
        ? METAL_COMPRESSED_KV_FALLBACK
        : v;
    const reason = "Metal runtime graph dispatch for Qwen3.5 hybrid attention still routes compressed QJL/Polar KV tensors through generic attention/MUL_MAT in the built decoder graph; using q8_0 KV keeps the fused Metal+DFlash+voice path live until the graph selects the dedicated compressed-KV attention ops.";
    if (opts.emitWarning !== false) {
        ...
        if (!metalCompressedKvWarnings.has(warningKey)) {
            ...
            console.warn(`[local-inference] ${reason} Set ELIZA_DFLASH_METAL_COMPRESSED_KV=1 only for kernel-runtime experiments; standalone QJL/Polar Metal shaders are verified, but the current built-fork graph path is not production-safe.`);
        }
    }
    return {
        cacheTypeK,
...
function resolveDflashGpuLayers(overrides, fallback) {
    if (typeof overrides?.gpuLayers === "number")
        return overrides.gpuLayers;
    if (overrides?.gpuLayers === "auto")
        return "auto";
    if (overrides?.gpuLayers === "max")
        return "auto";
    ...
    if (overrides?.useGpu === false)
        return 0;
    return fallback;
}
function normalizeDflashKvOffloadMode(value) {
    const mode = value?.trim().toLowerCase();
    if (mode === "cpu" || mode === "gpu" || mode === "split")
        return mode;
    return null;
}
export function resolveDflashKvOffload(overrides) {
...
export function appendKvOffloadFlags(args, mode) {
    if (mode === "cpu") {
...
function isEliza1TierCatalogId(id) {
 ...
 */
export function findBundleOmnivoiceAssets(textModelPath) {
    const textDir = path.dirname(textModelPath);
    if (path.basename(textDir) !== "text")
        return null;
    ...
    const tokenizer = entries.find((e) => /^omnivoice-tokenizer-[^/]*\.gguf$/i.test(e));
    const model = entries.find((e) => /^omnivoice-[^/]*\.gguf$/i.test(e) && !/tokenizer/i.test(e));
    if (!model || !tokenizer)
        return null;
    return {
...
export async function resolveKvSpillPlan(args) {
    ...
    const residentKvBudgetBytes = ram
        ...
        : 1024 * 1024 * 1024;
    const bytesPerToken = estimateQuantizedKvBytesPerToken(args.catalog?.params ?? "27B");
    ...
    const cpuSpillAvailable = hardware.appleSilicon ||
 ...
 */
export function appendKvSpillFlags(args, plan) {
    if (!plan || plan.mode !== "spill")
        return args;
...
function assertGgufAvailable(buf, offset, bytes) {
    if (offset + bytes > buf.length) {
        throw new Error(`GGUF metadata exceeds ${Math.floor(GGUF_METADATA_READ_LIMIT_BYTES / 1024 / 1024)} MiB validation window`);
    }
}
function readGgufU64(buf, off) {
...
function readGgufString(buf, off) {
...
function skipGgufScalar(buf, off, type) {
    switch (type) {
        case 0:
        case 1:
        case 7:
...
function readGgufScalar(buf, off, type) {
    switch (type) {
        case 0: {
            assertGgufAvailable(buf, off.value, 1);
...
function skipGgufValue(buf, off, type) {
    if (type !== GGUF_ARRAY) {
...
function readDflashGgufMetadata(file) {
    ...
    for (let i = 0; i < kvCount; i += 1) {
        ...
        if (key === "general.architecture" ||
            ...
            if (skipped.arrayLength !== null)
                lengths.set(key, skipped.arrayLength);
        }
        const valueEnd = off.value;
        if (DFLASH_TOKENIZER_HASH_KEYS.includes(key) &&
            valueEnd >= valueStart) {
            hashes.set(key, crypto
                .createHash("sha256")
                .update(buf.subarray(valueStart, valueEnd))
                .digest("hex"));
        }
    }
    const tokenizerHashes = Object.fromEntries(DFLASH_TOKENIZER_HASH_KEYS.map((key) => [key, hashes.get(key) ?? null]));
    const tokenizerLengths = Object.fromEntries(DFLASH_TOKENIZER_HASH_KEYS.map((key) => [key, lengths.get(key) ?? null]));
...
function binarySupportsDflashDraftArchitecture(binaryPath) {
    const caps = readCapabilitiesAt(path.join(path.dirname(binaryPath), "CAPABILITIES.json"));
    if (!caps)
        return false;
    if (!capabilitiesAreFreshForBinary(binaryPath))
        return false;
    const raw = caps;
    if (raw.dflashDraftArchitecture === true)
        return true;
    const draftArchitectures = raw.draftArchitectures;
    if (Array.isArray(draftArchitectures) &&
        draftArchitectures.includes("dflash-draft")) {
        return true;
    }
    const supportedArchitectures = raw.supportedArchitectures;
    return (Array.isArray(supportedArchitectures) &&
...
export function validateDflashDrafterCompatibility(args) {
    ...
    const addMismatch = (mismatches, key) => {
        if (target.tokenizerHashes[key] !== drafter.tokenizerHashes[key]) {
            mismatches.push(key);
        }
    };
    const tokenizerMismatchKeys = [];
    ...
    const targetAddBos = target.tokenizerHashes["tokenizer.ggml.add_bos_token"] !== null &&
        target.tokenizerHashes["tokenizer.ggml.add_bos_token"] !==
            crypto
                .createHash("sha256")
                .update(Buffer.from([0]))
                .digest("hex");
    const drafterAddBos = drafter.tokenizerHashes["tokenizer.ggml.add_bos_token"] !== null &&
    ...
    const targetAddEos = target.tokenizerHashes["tokenizer.ggml.add_eos_token"] !== null &&
        target.tokenizerHashes["tokenizer.ggml.add_eos_token"] !==
            crypto
                .createHash("sha256")
                .update(Buffer.from([0]))
                .digest("hex");
    const drafterAddEos = drafter.tokenizerHashes["tokenizer.ggml.add_eos_token"] !== null &&
    ...
    const failures = [];
    if (drafter.architecture === "dflash-draft" &&
        !binarySupportsDflashDraftArchitecture(args.binaryPath)) {
        failures.push("drafter GGUF architecture is 'dflash-draft', but the installed llama-server does not advertise dflash-draft GGUF loader support");
    }
    ...
    if (tokenizerMismatches.length > 0) {
        failures.push(`target/drafter tokenizer metadata mismatch (${tokenizerMismatches
            .map((m) => m.key)
            .join(", ")})`);
    }
    return {
 ...
 */
export function maybeRepairGgufMerges(binaryPath, sourceWithMergesPath, strippedGgufPath) {
    if (readBool("ELIZA_DFLASH_REPAIR_DISABLED"))
        return strippedGgufPath;
    if (!fs.existsSync(sourceWithMergesPath) ||
...
async function fetchJson(url, init, timeoutMs = 60_000, externalSignal) {
...
export function extractStreamingChatDelta(json) {
 ...
 */
export function extractVerifierRejectRange(json) {
    if (!json || typeof json !== "object")
        return null;
    const verifier = json.verifier;
    if (!verifier || typeof verifier !== "object")
        return null;
    const rejected = verifier.rejected;
...
async function fetchStreamingChatCompletion(url, init, timeoutMs, callbacks, repairStream, externalSignal, startIndex = 0) {
    const controller = new AbortController();
    const abort = () => controller.abort(externalSignal?.reason);
    if (externalSignal?.aborted)
        abort();
    ...
    try {
        ...
        const consumeEvent = async (raw) => {
            ...
            const timingRecord = parsed && typeof parsed === "object"
                ? parsed.timings
                : null;
...
function derivePreferredParallel(args) {
    const bytesPerSlot = estimateQuantizedKvBytesPerToken(args.params) * args.contextSize;
 ...
 */
function resolveParallel(catalogParallel, budget, override) {
    ...
    if (budget)
        return derivePreferredParallel(budget);
    return DEFAULT_CACHE_PARALLEL;
}
/**
...
function appendPositiveIntFlag(args, flag, value) {
...
function appendBooleanFlag(args, enabledFlag, disabledFlag, value) {
    if (value === true)
        args.push(enabledFlag);
    if (value === false)
        args.push(disabledFlag);
}
export function appendOptimizationFlags(args, optimizations) {
    ...
    appendPositiveIntFlag(args, "--cache-reuse", readPositiveIntEnv("ELIZA_LOCAL_CACHE_REUSE") ?? optimizations?.cacheReuse);
    appendPositiveIntFlag(args, "--cache-ram", readPositiveIntEnv("ELIZA_LOCAL_CACHE_RAM_MB") ?? optimizations?.cacheRamMb);
    appendPositiveIntFlag(args, "--batch-size", readPositiveIntEnv("ELIZA_LOCAL_BATCH_SIZE") ?? optimizations?.batchSize);
    appendPositiveIntFlag(args, "--ubatch-size", readPositiveIntEnv("ELIZA_LOCAL_UBATCH_SIZE") ?? optimizations?.ubatchSize);
    appendBooleanFlag(args, "--cont-batching", "--no-cont-batching", readBoolFlag("ELIZA_LOCAL_CONT_BATCHING") ?? optimizations?.contBatching);
    appendBooleanFlag(args, "--kv-unified", "--no-kv-unified", readBoolFlag("ELIZA_LOCAL_KV_UNIFIED") ?? optimizations?.kvUnified);
    appendBooleanFlag(args, "--op-offload", "--no-op-offload", readBoolFlag("ELIZA_LOCAL_OP_OFFLOAD") ?? optimizations?.opOffload);
 ...
 */
export function applyGpuProfile(args, profile) {
...
function resolveKeepAliveIntervalMs() {
...
export class DflashLlamaServer {
     ...
     */
    nativeDflashEventsCapability = null;
    /**
     ...
     */
    verifyCollector = new DflashMetricsCollector();
    hasLoadedModel() {
     ...
     */
    loadedDrafterModelPath() {
        if (this.loadedPlan?.disableDrafter)
            return null;
    ...
    runningBinaryCapabilities() {
        if (this.loadedBinaryPath) {
            const caps = readCapabilitiesAt(path.join(path.dirname(this.loadedBinaryPath), "CAPABILITIES.json"));
            if (caps)
                return caps;
        }
     ...
     */
    audioSpeechRoute() {
        if (!this.baseUrl || !this.hasLoadedModel())
            return null;
    ...
    async synthesizeSpeech(args) {
        ...
        if (!route) {
            throw new Error("[dflash] synthesizeSpeech requires a fused omnivoice llama-server; " +
        ...
        try {
            ...
            if (!res.ok) {
                const body = await res.text().catch(() => "");
                throw new Error(`[dflash] /v1/audio/speech HTTP ${res.status}${body ? `: ${body}` : ""}`);
            }
     ...
     */
    async resizeParallel(targetParallel) {
        if (!this.hasLoadedModel() || !this.loadedPlan)
            return false;
        if (!Number.isFinite(targetParallel) ||
            targetParallel <= this.cacheParallel) {
            return false;
        }
        const clamped = Math.min(MAX_AUTO_PARALLEL, Math.floor(targetParallel));
        if (clamped === this.cacheParallel)
            return false;
        await this.start({ ...this.loadedPlan, parallelOverride: clamped }, this.lastOptimizations);
        return true;
    }
    /**
     ...
     */
    async restartWithoutDrafter() {
        if (!this.hasLoadedModel() || !this.loadedPlan)
            return false;
        if (this.loadedPlan.disableDrafter)
            return false;
        await this.start({ ...this.loadedPlan, disableDrafter: true }, this.lastOptimizations);
        return true;
    }
    ...
    async load(plan) {
        ...
        const cacheTypeK = typeof overrides?.cacheTypeK === "string"
            ? overrides.cacheTypeK
            : kvCache?.typeK;
        const cacheTypeV = typeof overrides?.cacheTypeV === "string"
        ...
        const ttsAssets = catalog && isEliza1TierCatalogId(catalog.id)
    ...
    async start(plan, optimizations) {
        ...
        const drafterModelPath = drafterEnabled
            ? maybeRepairDflashDrafter(status.binaryPath, plan.targetModelPath, plan.drafterModelPath)
        ...
        if (drafterEnabled) {
            try {
                ...
                if (!compatibility.compatible) {
                    throw new Error(`[dflash] DFlash drafter rejected before llama-server startup; refusing to launch target-only because Eliza-1 requires DFlash. ${compatibility.reason}`);
                }
            }
            catch (err) {
                ...
                throw new Error(`[dflash] DFlash drafter rejected before llama-server startup; refusing to launch target-only because Eliza-1 requires DFlash. ${disabledDrafterReason}`);
            }
        }
        const effectivePlan = {
            ...plan,
        ...
        const requestedCacheTypeK = process.env.ELIZA_DFLASH_CACHE_TYPE_K?.trim() || effectivePlan.cacheTypeK;
        const requestedCacheTypeV = process.env.ELIZA_DFLASH_CACHE_TYPE_V?.trim() || effectivePlan.cacheTypeV;
        ...
        const usableRamMb = Math.round(os.totalmem() / BYTES_PER_MB_DFLASH) - ramHeadroomReserveMb();
        const parallel = resolveParallel(optimizations?.parallel, effectivePlan.params
            ? {
                contextSize: effectivePlan.contextSize,
                params: effectivePlan.params,
                usableRamMb,
            }
            : undefined, effectivePlan.parallelOverride);
        ...
        if (cacheTypeK) {
            assertCacheTypeSupportedOnBackend(cacheTypeKSource, cacheTypeK, status.binaryPath);
            args.push("--cache-type-k", cacheTypeK);
        }
        if (cacheTypeV) {
            assertCacheTypeSupportedOnBackend(cacheTypeVSource, cacheTypeV, status.binaryPath);
        ...
        const runningBinaryIsFused = resolveFusedDflashBinary() !== null &&
            path.resolve(status.binaryPath) ===
                path.resolve(resolveFusedDflashBinary() ?? "");
        if (runningBinaryIsFused &&
        ...
        if (extra) {
            ...
            for (let i = 0; i < tokens.length; i += 1) {
                if ((tokens[i] === "--cache-type-k" || tokens[i] === "--cache-type-v") &&
                    i + 1 < tokens.length) {
                    assertCacheTypeSupportedOnBackend(tokens[i], tokens[i + 1], status.binaryPath);
                }
            }
            args.push(...tokens);
        }
    ...
    async evictIdleConversations() {
        ...
        const slotByConversation = new Map();
        ...
        if (dropped.length === 0)
            return;
        await Promise.all(dropped.map(async (conversationId) => {
    ...
    startKeepAliveTimer() {
        ...
        const timer = setInterval(() => {
            ...
            for (const [slotId, entry] of [...this.lastPrewarmBySlot]) {
                if (now - entry.touchedAtMs < staleThresholdMs)
                    continue;
    ...
    async persistAllConversationsBeforeStop(baseUrl, conversationDir) {
        ...
        const tasks = handles.map(async (handle) => {
            const targetPath = path.join(conversationDir, slotCacheFileName(handle.conversationId, "long"));
            try {
    ...
    async persistConversationKv(conversationId, slotId) {
        ...
        if (!baseUrl || !conversationDir || slotId < 0)
            return false;
        const targetPath = path.join(conversationDir, slotCacheFileName(conversationId, "long"));
        try {
    ...
    async restoreConversationKv(conversationId, slotId) {
        ...
        if (!baseUrl || !conversationDir || slotId < 0)
            return false;
        const sourcePath = path.join(conversationDir, slotCacheFileName(conversationId, "long"));
        try {
     ...
     */
    async probeNativeDflashEventsCapability() {
     ...
     */
    async nativeDflashEventsEnabled() {
    ...
    async runGenerate(baseUrl, dflashArgs, args, slotId) {
        const streaming = Boolean(args.onTextChunk ||
            dflashArgs.onVerifierEvent ||
            dflashArgs.onDflashEvent);
        const prefill = typeof dflashArgs.prefill === "string" && dflashArgs.prefill.length > 0
            ? dflashArgs.prefill
            : "";
        ...
        if (readBool("ELIZA_DFLASH_DEBUG_REQUEST")) {
            console.error("[dflash] request", JSON.stringify({
                streaming,
                slotId,
        ...
        if (streaming) {
            const repairStream = dflashArgs.responseSkeleton || dflashArgs.responseSchema
            ...
            const onDflashEvent = nativeEventsActive
                ...
                : undefined;
            const tail = await fetchStreamingChatCompletion(`${baseUrl}/v1/chat/completions`, {
                method: "POST",
                headers: { "content-type": "application/json" },
            ...
            if (firstTokenMs !== null) {
                ...
                inferenceTelemetry.record("inference.first_token_ms", firstTokenMs, telTags);
            }
        }
        else {
            json = (await fetchJson(`${baseUrl}/v1/chat/completions`, {
                method: "POST",
                headers: { "content-type": "application/json" },
                body: JSON.stringify(payload),
            }, 60_000, args.signal));
        ...
        if (streamingDflash &&
            ...
            usage = {
                ...
                dflash_acceptance_rate: streamingDflash.accepted / streamingDflash.drafted,
            };
        }
        const observedOutputTokens = estimateOutputTokensForDflashEvidence(usage, text);
        const maxTokens = typeof payload.max_tokens === "number" ? payload.max_tokens : null;
        const metricsCanProveDflashActivity = before.scrapeOk === true &&
            after.scrapeOk === true &&
            before.hasGenerationCounters === true &&
            after.hasGenerationCounters === true;
        if (shouldRequireActiveDflashForRequest(this.loadedPlan, maxTokens, observedOutputTokens) &&
            metricsCanProveDflashActivity &&
            (usage.dflash_drafted_tokens ?? 0) <= 0) {
            throw new Error("[dflash] speculative decoding was required for this Eliza-1 generation, " +
     ...
     */
    async prewarmConversation(promptPrefix, opts = {}) {
        const baseUrl = this.baseUrl;
        if (!baseUrl)
            return false;
        if (!promptPrefix || promptPrefix.length === 0)
            return false;
        const slotId = typeof opts.slotId === "number" && opts.slotId >= -1
            ? opts.slotId
        ...
        try {
            await fetchJson(`${baseUrl}/v1/chat/completions`, {
                method: "POST",
                headers: { "content-type": "application/json" },
                body: JSON.stringify(payload),
            }, 30_000);
 ...
 */
export function buildChatCompletionBody(args, slotId, streaming) {
    const guided = resolveGuidedDecodeForParams(args);
    const effectivePrefill = guided.prefill ??
...
//# sourceMappingURL=dflash-server.js.map