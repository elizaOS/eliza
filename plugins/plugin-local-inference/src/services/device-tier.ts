...
import { selectBestEliza1Fit as resolveBestEliza1FitForRam } from "@elizaos/plugin-native-inference/model-catalog/device-fit";
import { type Eliza1Fit } from "@elizaos/plugin-native-inference/model-catalog/device-fit";
 ...
 * cache).
 */
export function selectBestEliza1FitForDevice(probe: HardwareProbe): Eliza1Fit | null {
    const mobile = isMobileHost(probe);
    let memGb = effectiveModelMemoryGb(probe);
    if (mobile)
        memGb = Math.min(memGb, MOBILE_FIT_CEILING_GB);
    ...
    try {
        ...
        console.warn(`[device-tier] recommendedFit unavailable: ${err instanceof Error ? err.message : String(err)}`);
        return null;
    }
...
function hasAvx2Baseline(probe: HardwareProbe): boolean {
    ...
    if (probe.arch !== "x64")
        return false; // 32-bit ARM + others: unsupported
    return probe.cpuCores >= 4;
}
/**
...
function classifyRawTier(args: ClassifyArgs): DeviceTier {
    ...
    const meetsOkayEffective = effective >=
