...
export const MLX_BACKEND_ID = "mlx-server";
...
export function mlxOptIn() {
    return (envFlag("ELIZA_LOCAL_MLX") ||
...
export function resolveMlxPython() {
    ...
    for (const py of candidates) {
        try {
            const probe = spawnSync(py, ["-c", "import mlx_lm, mlx.core; print(mlx_lm.__version__)"], { encoding: "utf8", timeout: 8000 });
 ...
 */
export function looksLikeMlxModelDir(dir) {
    try {
 ...
 */
export function resolveMlxModelDir() {
    const explicit = process.env.ELIZA_MLX_MODEL_DIR?.trim();
    if (explicit && looksLikeMlxModelDir(explicit))
        return explicit;
...
export function mlxBackendEligible() {
    ...
    if (!modelDir) {
        return {
            eligible: false,
            reason: "no MLX model dir found (set ELIZA_MLX_MODEL_DIR or place one under <stateDir>/local-inference/mlx/eliza-1-*)",
            python,
            modelDir: null,
        };
    }
...
export class MlxLocalServer {
    ...
    async load(opts) {
        ...
        if (!python) {
            throw new Error("[mlx] no Python with `mlx-lm` importable — `pip install mlx-lm` on an Apple-Silicon mac");
        }
        if (!looksLikeMlxModelDir(opts.modelDir)) {
            throw new Error(`[mlx] ${opts.modelDir} does not look like an MLX model dir (need config.json + *.safetensors)`);
        }
        if (this.hasLoadedModel() && this.modelDir === opts.modelDir)
            return;
        if (this.child)
            await this.unload();
        ...
        await this.unload();
        throw new Error(`[mlx] mlx_lm.server did not become healthy within ${opts.healthTimeoutMs ?? 60_000}ms: ${String(lastErr)}`);
    }
    async unload() {
