...
export class DflashMetricsCollector {
     ...
     */
    acceptanceWindow = new Array(VERIFY_ACCEPTANCE_WINDOW).fill(Number.NaN);
    acceptanceWindowCursor = 0;
    acceptanceWindowFilled = 0;
    record(event) {
        if (this.finalized)
            return;
        ...
        if (event.kind === "dflash-verify") {
            ...
            const stepRate = event.drafted_count > 0
     ...
     */
    getAcceptanceRate() {
        if (this.acceptanceWindowFilled === 0)
            return Number.NaN;
        let sum = 0;
        for (let i = 0; i < this.acceptanceWindowFilled; i += 1) {
            const idx = (this.acceptanceWindowCursor -
...
export class DflashTurnHistory {
    ...
    constructor(limit = DEFAULT_HISTORY_LIMIT) {
        if (!Number.isInteger(limit) || limit < 1) {
            throw new Error(`[dflash] DflashTurnHistory limit must be a positive integer, got ${limit}`);
        }
        this.limit = limit;
    }
    ...
    async push(summary) {
        ...
        const verifyTail = summary.verifyTimeMs
            ...
            : "";
        logger.info(`[DflashMetricsCollector] turn summary drafted=${summary.drafted} accepted=${summary.accepted} rounds=${summary.rounds} acceptanceRate=${summary.acceptanceRate.toFixed(4)} events=${summary.eventCount} native=${summary.nativeEventCount}/${summary.eventCount} durationMs=${Math.round(summary.durationMs)}${verifyTail}`);
...
//# sourceMappingURL=dflash-metrics-collector.js.map