...
import { dflashLlamaServer, dflashRequired, getDflashRuntimeStatus, logDflashDevDisabledWarning, } from "./dflash-server";
import { MemoryMonitor } from "./memory-monitor";
import { mlxLocalServer } from "./mlx-server";
import { listInstalledModels } from "./registry";
import { DEFAULT_SESSION_KEY, resolveDefaultPoolSize, SessionPool, } from "./session-pool";
import { resolveGuidedDecodeForParams } from "./structured-output";
import { buildLocalEmbeddingRoute, EMBEDDING_FULL_DIM, isValidEmbeddingDim, } from "./voice/embedding";
...
import { createEvictableModelRole, SharedResourceRegistry, } from "./voice/shared-resources";
/**
...
function resolveVoiceSkeletonStreamFields(skeleton) {
    if (!skeleton)
        return [];
    const fields = [];
    const seen = new Set();
...
function skeletonHasFreeStringKey(skeleton, key) {
    return (skeleton?.spans.some((span) => span.kind === "free-string" && span.key === key) ?? false);
}
/**
...
export function resolveIdleUnloadMs() {
 ...
 */
export function resolveMaxConcurrentSpeculativeResponses(parallelSlots) {
 ...
 */
function resolveActiveEliza1Bundle(target) {
    if (!target?.bundleRoot)
        return null;
    if (!ELIZA_1_PLACEHOLDER_IDS.has(target.id))
        return null;
    return { root: target.bundleRoot, tierId: target.id };
}
/**
 ...
 */
function normalizeKvCacheTypeForBinding(name) {
 ...
 * cycle-free.
 */
function toBackendLoadOverrides(args) {
    const overrides = {};
    if (args.contextSize !== undefined)
        overrides.contextSize = args.contextSize;
    if (args.cacheTypeK !== undefined)
        overrides.cacheTypeK = args.cacheTypeK;
    if (args.cacheTypeV !== undefined)
        overrides.cacheTypeV = args.cacheTypeV;
    if (args.gpuLayers !== undefined)
        overrides.gpuLayers = args.gpuLayers;
    if (args.kvOffload !== undefined)
        overrides.kvOffload = args.kvOffload;
    if (args.flashAttention !== undefined) {
        overrides.flashAttention = args.flashAttention;
    }
    if (args.mmap !== undefined)
        overrides.mmap = args.mmap;
    if (args.mlock !== undefined)
        overrides.mlock = args.mlock;
    if (args.useGpu !== undefined)
        overrides.useGpu = args.useGpu;
    return overrides;
}
/**
 ...
 */
function resolveBindingGrammarSource(args) {
...
export class NodeLlamaCppBackend {
    ...
    async load(plan) {
        ...
        const optimizations = plan.catalog?.runtime?.optimizations ??
            (plan.modelId
        ...
        const poolSize = resolveDefaultPoolSize(process.env.ELIZA_LOCAL_SESSION_POOL_SIZE);
        ...
        const ctxOptions = { sequences: poolSize };
        ...
        if (overrides?.cacheTypeK !== undefined) {
            ctxOptions.experimentalKvCacheKeyType = normalizeKvCacheTypeForBinding(overrides.cacheTypeK);
        }
        if (overrides?.cacheTypeV !== undefined) {
            ctxOptions.experimentalKvCacheValueType = normalizeKvCacheTypeForBinding(overrides.cacheTypeV);
        }
    ...
    async generate(args) {
        ...
        const cacheKey = args.cacheKey && args.cacheKey.length > 0
            ? args.cacheKey
            : DEFAULT_SESSION_KEY;
        ...
        const grammar = grammarSource && this.bindingModule && this.llama
        ...
        const run = async () => {
            ...
            const promptText = prefill.length > 0 ? `${args.prompt}\n${prefill}` : args.prompt;
...
export class LocalInferenceEngine {
    nodeBackend = new NodeLlamaCppBackend();
    dispatcher = new BackendDispatcher(this.nodeBackend, dflashLlamaServer, () => getDflashRuntimeStatus().enabled, () => dflashRequired(), () => getDflashRuntimeStatus().capabilities?.kernels ?? null);
    /**
    ...
    registerResidentRoles() {
        ...
        if (this.drafterRoleId === null) {
            const role = createEvictableModelRole({
                role: "drafter",
                isResident: () => this.activeBackendId() === "llama-server" &&
    ...
    async deregisterResidentRoles() {
        const ids = [this.textTargetRoleId, this.drafterRoleId].filter((id) => id !== null);
    ...
    armIdleUnloadTimer() {
        ...
        const timer = setInterval(() => {
            if (!this.hasLoadedModel())
                return;
            if (Date.now() - this.lastActivityMs < idleMs)
                return;
            console.info(`[local-inference] No useModel activity for >${Math.round(idleMs / 1000)}s — unloading the active text model to reclaim RAM. It will reload on the next request.`);
            void this.unload().catch((err) => {
                console.warn(`[local-inference] idle-unload failed: ${err instanceof Error ? err.message : String(err)}`);
            });
     ...
     */
    maxConcurrentSpeculativeResponses() {
    ...
    async maybeAutoResizeParallel() {
        ...
        if (this.memoryMonitor.isUnderPressure(sample)) {
            console.warn(`[local-inference] Conversation high-water mark wants --parallel ${recommended} (running ${running}) but RAM is tight (free ${sample.effectiveFreeMb} MB) — not resizing. Slot thrashing may occur; consider a smaller tier or more RAM.`);
            return false;
        }
        try {
            const resized = await dflashLlamaServer.resizeParallel(recommended);
            if (resized) {
                console.info(`[local-inference] Resized llama-server --parallel ${running} → ${recommended} (conversation high-water mark grew).`);
            }
            return resized;
        }
        catch (err) {
            console.warn(`[local-inference] --parallel resize to ${recommended} failed: ${err instanceof Error ? err.message : String(err)}`);
            return false;
        }
    }
    async available() {
    ...
    async load(modelPath, resolved) {
        ...
        try {
            ...
            if (decision.backend === "llama-server" &&
                decision.reason === "preferred-backend" &&
                !dflashRequired()) {
                console.warn("[local-inference] llama-server backend unavailable; falling back to node-llama-cpp:", err instanceof Error ? err.message : String(err));
     ...
     * metrics).
     */
    async generateInConversation(handle, args) {
        if (handle.closed) {
            throw new Error(`[local-inference] Conversation ${handle.conversationId} has been closed; reopen before generating`);
        }
        this.markActivity();
     ...
     */
    async prewarmConversation(conversationOrId, promptPrefix, opts = {}) {
        if (this.activeBackendId() !== "llama-server")
            return false;
        this.markActivity();
        let slotId;
        ...
        if (typeof conversationOrId === "string") {
            const modelId = opts.modelId ?? this.currentModelPath() ?? "default-local-model";
            const handle = this.conversation(conversationOrId, modelId) ??
                this.openConversation({ conversationId: conversationOrId, modelId });
            slotId = handle.slotId;
            cacheKey = `conv:${handle.conversationId}`;
        }
        else {
            if (conversationOrId.closed)
                return false;
     ...
     */
    async closeConversation(handle) {
        if (handle.closed)
            return;
        if (this.activeBackendId() === "llama-server") {
     ...
     */
    conversation(conversationId, modelId) {
    ...
    startVoice(opts) {
        if (this.voiceBridge) {
            throw new VoiceStartupError("already-started", "[voice] Voice session is already active. Call stopVoice() before starting a new one.");
        }
    ...
    async armVoice() {
        const bridge = this.voiceBridge;
        if (!bridge) {
            throw new VoiceStartupError("not-started", "[voice] Cannot arm: no voice session active. Call startVoice() first.");
        }
        await bridge.arm();
    }
    /**
     ...
     */
    async ensureActiveBundleVoiceReady() {
        ...
        if (!bridge) {
            ...
            if (bundle) {
                ...
                if (!kokoro) {
                    throw new VoiceStartupError("missing-bundle-root", "[voice] Cannot start local voice: no active Eliza-1 bundle is loaded and no Kokoro artifacts are staged under ~/.eliza/local-inference/models/kokoro/. Install an Eliza-1 bundle, or stage the Kokoro ONNX + at least one voice .bin to enable local TTS.");
                }
                bridge = this.startVoice({
                    bundleRoot: "",
                    useFfiBackend: false,
    ...
    async startVoiceSession(opts) {
        ...
        if (bridge.lifecycle.current().kind !== "voice-on") {
            throw new VoiceStartupError("not-started", "[voice] Cannot start a voice session: voice lifecycle is not armed. Call armVoice() first.");
        }
        const backendId = bridge.backend.id;
        if (backendId === "stub") {
            throw new VoiceStartupError("missing-fused-build", "[voice] Cannot start a live voice session on the StubOmniVoiceBackend (it emits silence). Start the bridge with useFfiBackend:true or a real backendOverride.");
        }
        const [{ DesktopMicSource, pipeMicToRingBuffer }, vadMod, { VoiceTurnController }, { InMemoryAudioSink }, eotMod,] = await Promise.all([
            ...
            (await vadMod.createSileroVadDetector({
                ...
                ctx: bridge.ffi
                    ? () => {
                        ...
                        if (ctx === null) {
                            throw new VoiceStartupError("missing-ffi", "[voice] Cannot initialize native VAD: fused FFI context is not loaded.");
                        }
                        return ctx;
                    }
                    : undefined,
        ...
        const tierRevision = activeTier
            ...
            : undefined;
        const turnDetector = opts.turnDetector === false
            ? undefined
            : (opts.turnDetector ??
        ...
        if (turnDetector) {
            try {
                ...
                throw new VoiceStartupError("missing-turn-detector", `[voice] Cannot initialize semantic turn detector: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        ...
        const { unsubscribe: stopMicRing } = pipeMicToRingBuffer(micSource, new InMemoryAudioSink());
        ...
        let wakeWord = null;
        let feedWakeFrame = null;
        if (opts.wakeWord?.enabled) {
            const { isPlaceholderWakeWordHead, loadBundledWakeWordModel, OPENWAKEWORD_DEFAULT_HEAD, OpenWakeWordDetector, } = await import("./voice/wake-word");
            const headName = opts.wakeWord.head?.trim() || OPENWAKEWORD_DEFAULT_HEAD;
            if (isPlaceholderWakeWordHead(headName)) {
                console.warn(`[voice] wake word head '${headName}' is a PLACEHOLDER (the upstream openWakeWord "hey jarvis" head, renamed) — it fires on "hey jarvis", not the Eliza-1 wake phrase. Experimental, opt-in only; see packages/inference/reports/porting/2026-05-11/wakeword-head-plan.md.`);
            }
            ...
            if (model) {
                ...
                console.info("[voice] wake word requested but no openWakeWord model in this bundle — running VAD-gated only");
            }
        }
    ...
    async synthesizeSpeech(text, signal) {
        ...
        if (bridge.backend.id === "stub") {
            throw new VoiceStartupError("missing-fused-build", "[voice] Cannot synthesize speech with StubOmniVoiceBackend (it emits silence). Start voice with useFfiBackend:true or inject a real backend.");
        }
        return bridge.synthesizeTextToWav(text, signal);
    }
    async prewarmVoicePhrases(texts, opts = {}) {
        return this.requireVoiceBridge("prewarm voice phrases").prewarmPhrases(texts, opts);
    }
    /**
     ...
     */
    async prewarmIdleVoicePhrases(opts = {}) {
        return this.requireVoiceBridge("prewarm idle voice phrases").prewarmIdlePhrases(opts);
    }
    /**
    ...
    playFirstAudioFiller() {
        return this.requireVoiceBridge("play first-audio filler").playFirstAudioFiller();
    }
    async transcribePcm(args, signal) {
        ...
        const transcript = await this.requireVoiceBridge("transcribe audio").transcribePcm(args, signal);
        if (signal?.aborted) {
            throw signal.reason instanceof Error
                ? signal.reason
                : new DOMException("Aborted", "AbortError");
     ...
     */
    async runVoiceTurn(audio, opts = {}) {
        this.markActivity();
        const bridge = this.requireVoiceBridge("run a voice turn");
        return bridge.runVoiceTurn(audio, opts.textRunner ?? dflashTextRunner(dflashLlamaServer), {
    ...
    localEmbeddingRoute(args) {
        ...
        if (!bundleRoot || !tierId) {
            throw new VoiceStartupError("missing-bundle-root", "[embedding] no active Eliza-1 bundle; pass bundleRoot+tierId or load an Eliza-1 model first");
        }
     ...
     */
    async embed(input, dim = EMBEDDING_FULL_DIM) {
        this.markActivity();
        if (!isValidEmbeddingDim(dim)) {
            throw new Error(`[embedding] dim ${dim} is not a valid Matryoshka width (expected one of 64,128,256,512,768,1024)`);
        }
    ...
    requireVoiceBridge(action) {
        const bridge = this.voiceBridge;
        if (!bridge) {
            throw new VoiceStartupError("not-started", `[voice] Cannot ${action}: no voice session active. Call startVoice() and armVoice() first.`);
        }
        return bridge;
    }
    voiceStreamingArgs(args) {
        ...
        const structuredVoiceFields = args.streamStructured === true
            ? resolveVoiceSkeletonStreamFields(args.responseSkeleton)
            : [];
        const hasShouldRespondGate = args.streamStructured === true &&
            skeletonHasFreeStringKey(args.responseSkeleton, "shouldRespond");
        const extractorStreamFields = hasShouldRespondGate && !structuredVoiceFields.includes("shouldRespond")
        ...
        let shouldRespondAllowsVoice = hasShouldRespondGate
            ? null
            : true;
        const pendingStructuredReplyChunks = [];
        const pushStructuredVoiceChunk = (chunk) => {
            streamedAny = true;
            const token = { index: nextIndex++, text: chunk };
            structuredVoicePush = structuredVoicePush.then(() => bridge.pushAcceptedToken(token));
        };
        const structuredVoiceExtractor = structuredVoiceFields.length > 0 && args.responseSkeleton
            ? new ResponseSkeletonStreamExtractor({
                ...
                onChunk: (chunk, field) => {
                    ...
                    if (hasShouldRespondGate) {
                        if (shouldRespondAllowsVoice === false)
                            return;
     ...
     */
    async pushAcceptedTokens(tokens) {
    ...
    async dflashDrafterHandle() {
        ...
        const { createDflashDrafterHandle } = await import("./voice/shared-resources");
     ...
     */
    static async resolveDflashPlanForPath(modelPath) {
        const installed = await listInstalledModels();
        const target = installed.find((m) => m.path === modelPath);
        if (!target)
            return null;
