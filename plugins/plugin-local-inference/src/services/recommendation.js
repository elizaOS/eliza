...
function chatCandidates(catalog) {
    return catalog.filter((model) => !model.hiddenFromCatalog && model.runtimeRole !== "dflash-drafter");
}
...
function downloadSizeGuardrail(hardware, model, catalog, isMobile) {
    ...
    if (sizeGb > memGb * wontFitRatio)
        return "wontfit";
    if (sizeGb > memGb * tightRatio)
        return "tight";
    return null;
}
export function assessCatalogModelFit(hardware, model, catalog = MODEL_CATALOG, options = {}) {
    if (model.runtime?.dflash) {
        const byId = catalogById(catalog);
        if (!byId.has(model.runtime.dflash.drafterModelId))
            return "wontfit";
    }
    ...
    if (sizeLevel === "wontfit")
        return "wontfit";
    if (sizeLevel === "tight" || ramFit.level === "tight")
        return "tight";
    return "fits";
}
...
export function selectRecommendedModelForSlot(slot, hardware, catalog = MODEL_CATALOG, options = {}) {
    ...
    const eligible = ladder.filter((model) => canFit(hardware, model, catalog, budgetOptionsForModel(model, budget)) &&
    ...
    const alternatives = ranked.length > 0
        ? ranked
        : fallbackCandidates(slot, hardware, catalog, budget).filter((model) => kernelRequirementsSatisfied(model, binaryKernels));
    const model = alternatives[0] ?? null;
    const fit = model
        ? assessCatalogModelFit(hardware, model, catalog, budgetOptionsForModel(model, budget))
        : null;
    return {
        slot,
        platformClass,
...
export function canBundleBeDefaultOnDevice(installed, hardware, options = {}) {
    ...
    const verifiedOnDeviceBackend = caps.availableBackends.some((b) => supported.has(b) &&
...
export function recommendForFirstRun(catalog = MODEL_CATALOG) {
    const byId = catalogById(catalog);
    const isEligibleChat = (model) => !model.hiddenFromCatalog &&
...
export function chooseSmallerFallbackModel(currentModelId, hardware, slot = "TEXT_LARGE", catalog = MODEL_CATALOG, options = {}) {
    const byId = catalogById(catalog);
    ...
    const ladderFallback = modelsFromLadder(SLOT_LADDERS[platformClass][slot], catalog)
        ...
        .filter((model) => canFit(hardware, model, catalog, budgetOptionsForModel(model, budget)))[0];
    if (ladderFallback)
        return ladderFallback;
    return (fallbackCandidates(slot, hardware, catalog, budget)
        .filter((model) => model.id !== currentModelId)
