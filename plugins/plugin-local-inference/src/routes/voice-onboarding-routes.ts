...
export const ONBOARDING_SCRIPT: ReadonlyArray<OnboardingScriptStep> = [
	{
		id: "consent-1",
		role: "consent",
		prompt:
 ...
 * WeSpeaker int8 ONNX in `$ELIZA_STATE_DIR/voice-profiles/models/<model>.onnx`.
...
export function setVoiceOnboardingEncoderFactory(
...
export function setVoiceOnboardingProfileStore(
...
export function setVoiceOnboardingSettingsWriter(
...
function decodeFloat32(buf: Buffer): Float32Array {
	if (buf.byteLength % 4 !== 0) {
		throw new Error(
			`[voice-onboarding] PCM buffer length ${buf.byteLength} is not a multiple of 4`,
		);
	}
 ...
 */
export async function handleVoiceOnboardingRoutes(
	...
	if (!pathname.startsWith("/api/voice/onboarding/")) return false;
	...
	if (method === "POST" && pathname === "/api/voice/onboarding/complete") {
		...
		if (!settingsWriter) {
			sendJsonError(
				res,
				"voice onboarding settings writer not configured; runtime must call setVoiceOnboardingSettingsWriter()",
				503,
			);
			return true;
		}
		try {
			...
			logger.error(
				{ err },
				"[voice-onboarding] failed to write ELIZA_ADMIN_ENTITY_ID",
			);
			sendJsonError(
				res,
...
export function __resetVoiceOnboardingSessions(): void {
	sessions.clear();
}
