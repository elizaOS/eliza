...
import { SpeakerEncoderUnavailableError } from "../services/voice/speaker/encoder";
import { VoiceProfileStore } from "../services/voice/profile-store";
import { WESPEAKER_EMBEDDING_DIM } from "../services/voice/speaker/encoder";
import { WESPEAKER_MIN_SAMPLES } from "../services/voice/speaker/encoder";
import { WESPEAKER_RESNET34_LM_INT8_MODEL_ID } from "../services/voice/speaker/encoder";
import { WESPEAKER_SAMPLE_RATE } from "../services/voice/speaker/encoder";
import { averageEmbeddings } from "../services/voice/speaker/encoder";
...
import { type SpeakerEncoder } from "../services/voice/speaker/encoder";
...
export function setVoiceFirstRunEncoderFactory(factory: EncoderFactory | null): void {
...
async function loadFusedSpeakerEncoder(): Promise<SpeakerEncoder> {
    ...
    if (!libPath) {
        throw new SpeakerEncoderUnavailableError("library-missing", "[voice-first-run] fused libelizainference not found. Set $ELIZA_INFERENCE_LIBRARY (exact path) or $ELIZA_INFERENCE_LIB_DIR, or build it via packages/app/scripts/build-llama-cpp-mtp.mjs.");
    }
    const ffi = loadElizaInferenceFfi(libPath);
    if (!FusedSpeakerEncoder.isSupported(ffi)) {
        throw new SpeakerEncoderUnavailableError("native-missing", `[voice-first-run] the fused libelizainference at ${libPath} (ABI v${ffi.libraryAbiVersion}) lacks the speaker ABI (eliza_inference_speaker_supported() == 0). Rebuild with the WeSpeaker forward graph linked in.`);
    }
...
export function setVoiceFirstRunProfileStore(store: VoiceProfileStore | null): void {
...
export function setVoiceFirstRunSettingsWriter(writer: SettingsWriter | null): void {
...
function decodeFloat32(buf: Buffer): Float32Array {
    if (buf.byteLength % 4 !== 0) {
        throw new Error(`[voice-first-run] PCM buffer length ${buf.byteLength} is not a multiple of 4`);
    }
 ...
 */
export async function handleVoiceFirstRunRoutes(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {
    ...
    if (method === "POST" && pathname === "/api/voice/first-run/complete") {
        ...
        if (!settingsWriter) {
            sendJsonError(res, "voice first-run settings writer not configured; runtime must call setVoiceFirstRunSettingsWriter()", 503);
            return true;
        }
        try {
            ...
            logger.error({ err }, "[voice-first-run] failed to write ELIZA_ADMIN_ENTITY_ID");
