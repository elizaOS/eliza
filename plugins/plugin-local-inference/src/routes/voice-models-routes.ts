...
import { VoiceModelDownloadError } from "../services/voice-model-updater";
import { VoiceModelUpdater } from "../services/voice-model-updater";
import { downloadVoiceModel } from "../services/voice-model-updater";
...
const KNOWN_VOICE_MODEL_IDS: ReadonlySet<string> = new Set(VOICE_MODEL_VERSIONS.map((v) => v.id));
...
export interface VoiceModelManagementInput {
    op: "trigger_voice_model_update" | "pin_voice_model" | "set_voice_model_preferences";
    id?: string;
    pinned?: boolean;
...
async function readPreferences(): Promise<NetworkPolicyPreferences> {
    try {
        ...
        logger.warn({ err }, "[voice-models-routes] failed to read voice-update-prefs.json — using defaults");
        return DEFAULT_NETWORK_POLICY_PREFERENCES;
    }
}
...
function normalizePrefs(candidate: Partial<PreferencesFile> | null | undefined): NetworkPolicyPreferences {
    ...
    const quietHours = Array.isArray(candidate.quietHours)
        ...
        : def.quietHours;
    return {
        autoUpdateOnWifi: typeof candidate.autoUpdateOnWifi === "boolean"
            ? candidate.autoUpdateOnWifi
            : def.autoUpdateOnWifi,
        autoUpdateOnCellular: typeof candidate.autoUpdateOnCellular === "boolean"
            ? candidate.autoUpdateOnCellular
            : def.autoUpdateOnCellular,
        autoUpdateOnMetered: typeof candidate.autoUpdateOnMetered === "boolean"
...
async function readPins(): Promise<Set<VoiceModelId>> {
    try {
        ...
        if (!Array.isArray(parsed.pinned))
            return new Set();
        return new Set(parsed.pinned.filter((id): id is VoiceModelId => KNOWN_VOICE_MODEL_IDS.has(id)));
    }
    catch (err) {
        if ((err as NodeJS.ErrnoException).code === "ENOENT")
            return new Set();
        logger.warn({ err }, "[voice-models-routes] failed to read voice-update-pins.json — treating as empty");
        return new Set();
    }
}
...
export async function applyVoiceModelManagementMutation(input: VoiceModelManagementInput): Promise<VoiceModelManagementResult> {
    ...
    if (input.op === "set_voice_model_preferences") {
        ...
        const preferences = normalizePrefs({
            autoUpdateOnWifi: typeof input.preferences?.autoUpdateOnWifi === "boolean"
                ? input.preferences.autoUpdateOnWifi
                : current.autoUpdateOnWifi,
            autoUpdateOnCellular: typeof input.preferences?.autoUpdateOnCellular === "boolean"
                ? input.preferences.autoUpdateOnCellular
                : current.autoUpdateOnCellular,
            autoUpdateOnMetered: typeof input.preferences?.autoUpdateOnMetered === "boolean"
    ...
    const statuses = await updater.check({ installed, bundleVersion: resolveBundleVersion() }, { pinned: pins }, { force: true });
    const status = statuses.find((s) => s.id === id);
    if (!status?.latestKnown)
        throw new Error(`no candidate version for ${id}`);
    const prefs = await readPreferences();
    const totalBytes = status.latestKnown.ggufAssets.reduce((sum, a) => sum + a.sizeBytes, 0);
    ...
    for (let assetIndex = 0; assetIndex < status.latestKnown.ggufAssets.length; assetIndex++) {
        results.push(await getDownloader()({
...
export async function resolveInstalledVersions(dir: string = bundleVoiceDir()): Promise<Map<VoiceModelId, string>> {
    ...
    for (const entry of entries) {
        ...
        if (!KNOWN_VOICE_MODEL_IDS.has(id))
            continue;
...
export function setVoiceModelsBundleVersionForTest(bundleVersion: string | null): void {
    bundleVersionOverride = bundleVersion;
}
function resolveBundleVersion(): string {
    if (bundleVersionOverride !== null)
        return bundleVersionOverride;
    return process.env.ELIZA_BUNDLE_VERSION?.trim() ?? "0.0.0";
}
/* ----------------------------------------------------------------- *
 * Route handler                                                       *
 * ----------------------------------------------------------------- */
export async function handleVoiceModelsRoutes(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {
    ...
    if (method === "GET" && pathname === `${ROUTE_PREFIX}/check`) {
        ...
        if (requestedForceValues.length > 1 ||
            (requestedForce != null &&
                requestedForce !== "" &&
                requestedForce !== "1")) {
        ...
        try {
            statuses = await updater.check({ installed, bundleVersion: resolveBundleVersion() }, { pinned: pins }, { force });
        }
        catch (err) {
            logger.warn({ err }, "[voice-models-routes] updater.check failed — surfacing as 502");
            sendJsonError(res, err instanceof Error ? err.message : "updater check failed", 502);
            return true;
        }
        sendJson(res, {
    ...
    if (method === "POST" && pathname === `${ROUTE_PREFIX}/preferences`) {
        ...
        const candidate = normalizePrefs({
            autoUpdateOnWifi: typeof body.autoUpdateOnWifi === "boolean"
                ? body.autoUpdateOnWifi
                : current.autoUpdateOnWifi,
            autoUpdateOnCellular: typeof body.autoUpdateOnCellular === "boolean"
                ? body.autoUpdateOnCellular
                : current.autoUpdateOnCellular,
            autoUpdateOnMetered: typeof body.autoUpdateOnMetered === "boolean"
        ...
        if (!owner &&
            ...
            sendJsonError(res, "cellular + metered auto-update toggles are owner-only", 403);
            return true;
        }
    ...
    const idActionMatch = method === "POST"
        ? /^\/api\/local-inference\/voice-models\/([^/]+)\/(update|pin)$/.exec(pathname)
        : null;
    if (idActionMatch) {
        let rawId: string;
        try {
        ...
        let statuses: ReadonlyArray<VoiceModelStatus>;
        try {
            statuses = await updater.check({ installed, bundleVersion: resolveBundleVersion() }, { pinned: pins }, { force: true });
        }
        catch (err) {
            sendJsonError(res, err instanceof Error ? err.message : "updater check failed", 502);
            return true;
        ...
        if (!networkPolicy.allow) {
            sendJsonError(res, `network policy refused (reason=${networkPolicy.reason})`, 409);
            return true;
        }
        if (status.latestKnown.ggufAssets.length === 0) {
