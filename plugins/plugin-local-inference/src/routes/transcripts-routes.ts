...
import { TRANSCRIPT_SHARING_STATES } from "@elizaos/core/transcripts";
import { TranscriptPrivacyService } from "../services/voice/transcript-privacy.js";
import { TranscriptService } from "../services/voice/transcript-service.js";
...
function requesterCanManageRow(ctx: RouteHandlerContext, row: Pick<Memory, "entityId">): boolean {
    const access = ctx.accessContext;
    if (!access)
        return true;
    return (isAdminRank(access.role) ||
        access.isOwner === true ||
        access.requesterEntityId === row.entityId);
}
function runtimePiiRecognizer(ctx: RouteHandlerContext): PiiEntityRecognizer | undefined {
    const service = ctx.runtime.getService(PII_ENTITY_RECOGNIZER_SERVICE) as Partial<PiiEntityRecognizerService> | null;
 ...
 */
export function buildTranscriptFromRequest(body: CreateTranscriptRequest, id: string, now: number): Transcript {
    const segments = Array.isArray(body.segments) ? body.segments : [];
    const createdAt = body.createdAt ?? now;
    const artifactConsent = body.meetingArtifact?.meeting.consent.state;
    const metadata = {
        consent: {
            state: artifactConsent && artifactConsent !== "redacted"
                ? artifactConsent
                : "unknown",
        },
        ...(body.metadata ?? {}),
...
const listRoute: Route = {
    ...
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        ...
        const transcripts = await service(ctx).list(roomId as UUID | undefined, undefined, ctx.accessContext);
...
const getRoute: Route = {
    ...
    rawPath: true,
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        const transcript = await service(ctx).get(ctx.params.id as UUID, ctx.accessContext);
        if (!transcript)
...
const deleteRoute: Route = {
    ...
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        ...
        const row = await (ctx.runtime as TranscriptServiceRuntime).getMemoryById(ctx.params.id as UUID);
        if (!row)
            return { status: 404, body: { error: "not found" } };
...
function participantEntityIds(transcript: Transcript): UUID[] {
    ...
    for (const participant of participants) {
        if (!participant || typeof participant !== "object")
            continue;
        const entityId = requestUuid((participant as {
            entityId?: unknown;
        }).entityId);
        if (entityId)
            ids.add(entityId);
...
async function resolveTranscriptCreateScope(ctx: RouteHandlerContext, body: CreateTranscriptRequest): Promise<TranscriptWriteScopeResult> {
    ...
    if (!room.worldId) {
        ctx.runtime.reportError("transcripts.create-scope", new Error("Canonical transcript room has no worldId"), { roomId: requestedRoomId });
        return {
            ok: false,
            status: 503,
...
const updateRoute: Route = {
    ...
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        ...
        const existing = await service(ctx).get(ctx.params.id as UUID, ctx.accessContext);
        if (!existing)
            return { status: 404, body: { error: "not found" } };
        ...
        if ((body.worldId !== undefined && !editWorldId) ||
        ...
        if ((editWorldId && editWorldId !== row.worldId) ||
        ...
        if (!row.worldId) {
            ctx.runtime.reportError("transcripts.update-scope", new Error("Persisted transcript row has no worldId"), { transcriptId: ctx.params.id });
            return {
                status: 503,
...
const createRoute: Route = {
    ...
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        ...
        if (body.audioBase64 && !body.audioUrl) {
            body.audioUrl = persistTranscriptAudioWav(Buffer.from(body.audioBase64, "base64"));
            body.audioContentType = "audio/wav";
        }
        const transcript = buildTranscriptFromRequest(body, crypto.randomUUID(), Date.now());
...
const shareRoute: Route = {
    ...
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        ...
        if (body.redactForAll !== undefined &&
        ...
        const targetCount = Number(Boolean(entityId)) +
        ...
        if (roomTarget && roomEntityIds.length === 0) {
            return {
                status: 409,
                body: {
                    error: "persisted meeting roster has no resolved participant entities",
...
const updatePrivacyRoute: Route = {
    ...
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        ...
        for (const [key, value] of entries) {
            if (!ARTIFACT_SHARING_KEYS.includes(key as (typeof ARTIFACT_SHARING_KEYS)[number]) ||
        ...
        try {
            const updated = await new TranscriptPrivacyService(ctx.runtime as TranscriptServiceRuntime).updateArtifactSharing(ctx.params.id as UUID, body.sharing);
            return { status: 200, body: { transcript: updated } };
        }
        catch (error) {
...
const deleteSourceAudioRoute: Route = {
    ...
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        ...
        try {
            const updated = await new TranscriptPrivacyService(ctx.runtime as TranscriptServiceRuntime).deleteSourceAudio(ctx.params.id as UUID);
