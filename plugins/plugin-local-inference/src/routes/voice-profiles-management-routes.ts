...
export function setVoiceProfilesManagementStore(store: VoiceProfileStore | null): void {
...
function toDto(record: VoiceProfileRecord, owner: string | null, rootDir: string): VoiceProfileDto {
    ...
    const hasSample = (record.audioRefs?.length ?? 0) > 0 &&
        sampleWavPath(rootDir, record.profileId) !== null;
    return {
        id: record.profileId,
        entityId: record.entityId,
        displayName: metaString(meta, "displayName") ??
            metaString(meta, "label") ??
            record.profileId,
        relationshipLabel: metaString(meta, "relationship") ?? metaString(meta, "relationshipLabel"),
        isOwner,
        ...
        retentionDays: typeof meta?.retentionDays === "number" ? meta.retentionDays : null,
...
export async function handleVoiceProfilesManagementRoutes(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {
    const method = (req.method ?? "GET").toUpperCase();
    const url = new URL(req.url ?? "/", "http://localhost");
    const pathname = url.pathname;
    if (!pathname.startsWith("/api/voice/profiles"))
        return false;
    // Collection-level routes.
    if (pathname === "/api/voice/profiles") {
        if (method === "GET")
            return listProfiles(res);
        if (method === "DELETE") {
            const requestedOwnerValues = url.searchParams.getAll("includeOwner");
            const requestedOwner = requestedOwnerValues[0];
            if (requestedOwnerValues.length > 1 ||
                (requestedOwner != null &&
                    requestedOwner !== "" &&
                    requestedOwner !== "true" &&
                    requestedOwner !== "false")) {
                sendJsonError(res, 'includeOwner must be specified at most once as "true" or "false".', 400);
                return true;
            }
    ...
    if (!sub) {
        if (method === "PATCH")
            return patchProfile(req, res, id);
        if (method === "DELETE")
            return deleteProfile(res, id);
        return false;
    }
    if (sub === "sample" && method === "GET")
        return serveSample(res, id);
    if (sub === "merge" && method === "POST")
        return mergeProfile(req, res, id);
    if (sub === "split" && method === "POST")
        return splitProfile(req, res, id);
    if (sub === "bind" && method === "POST")
        return bindProfile(req, res, id);
    if (sub === "unbind" && method === "POST")
        return unbindProfile(res, id);
    return false;
}
// ---------------------------------------------------------------------------
// Handlers
...
async function patchProfile(req: http.IncomingMessage, res: http.ServerResponse, id: string): Promise<true> {
...
async function deleteProfile(res: http.ServerResponse, id: string): Promise<true> {
    const store = await getStore();
...
async function deleteAll(res: http.ServerResponse, includeOwner: boolean): Promise<true> {
    const store = await getStore();
    const owner = ownerEntityId();
    const records = await store.list();
    let deleted = 0;
    for (const record of records) {
        if (!includeOwner && owner && record.entityId === owner)
            continue;
...
async function mergeProfile(req: http.IncomingMessage, res: http.ServerResponse, id: string): Promise<true> {
    const body = await readJsonBody<Record<string, unknown>>(req, res);
    if (!body)
        return true;
...
async function splitProfile(req: http.IncomingMessage, res: http.ServerResponse, id: string): Promise<true> {
    const body = await readJsonBody<Record<string, unknown>>(req, res);
    if (!body)
        return true;
    ...
    const retainedSampleIds = new Set((record.audioRefs ?? []).map((sample) => sample.sampleId));
    const selectedSampleIds = new Set(utteranceIds.filter((sampleId) => retainedSampleIds.has(sampleId)));
    ...
    if (selectedSampleIds.size >= retainedSampleIds.size) {
        sendJsonError(res, "a split must leave at least one sample in the profile", 400);
        return true;
    }
...
async function bindProfile(req: http.IncomingMessage, res: http.ServerResponse, id: string): Promise<true> {
    const body = await readJsonBody<Record<string, unknown>>(req, res);
    if (!body)
        return true;
...
async function unbindProfile(res: http.ServerResponse, id: string): Promise<true> {
    const store = await getStore();
    const existing = await store.get(id);
    if (!existing) {
        sendJsonError(res, `profile not found: ${id}`, 404);
...
async function serveSample(res: http.ServerResponse, id: string): Promise<true> {
