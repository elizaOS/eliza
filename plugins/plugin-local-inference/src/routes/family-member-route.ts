...
import { SpeakerEncoderUnavailableError } from "../services/voice/speaker/encoder.js";
import { VoiceProfileStore } from "../services/voice/profile-store.js";
import { WESPEAKER_EMBEDDING_DIM } from "../services/voice/speaker/encoder.js";
import { WESPEAKER_MIN_SAMPLES } from "../services/voice/speaker/encoder.js";
import { WESPEAKER_RESNET34_LM_INT8_MODEL_ID } from "../services/voice/speaker/encoder.js";
import { WESPEAKER_SAMPLE_RATE } from "../services/voice/speaker/encoder.js";
...
import { type SpeakerEncoder } from "../services/voice/speaker/encoder.js";
// ---------------------------------------------------------------------------
...
export function setFamilyMemberEncoderFactory(factory: FamilyMemberEncoderFactory | null): void {
    encoderFactoryOverride = factory;
    cachedEncoder = null;
}
let profileStoreOverride: VoiceProfileStore | null = null;
export function setFamilyMemberProfileStore(store: VoiceProfileStore | null): void {
...
async function loadFusedSpeakerEncoder(): Promise<SpeakerEncoder> {
    ...
    if (!libPath) {
        throw new SpeakerEncoderUnavailableError("library-missing", "[family-member-route] fused libelizainference not found. Set $ELIZA_INFERENCE_LIBRARY (exact path) or $ELIZA_INFERENCE_LIB_DIR, or build it via packages/app/scripts/build-llama-cpp-mtp.mjs.");
    }
    const ffi = loadElizaInferenceFfi(libPath);
    if (!FusedSpeakerEncoder.isSupported(ffi)) {
        throw new SpeakerEncoderUnavailableError("native-missing", `[family-member-route] the fused libelizainference at ${libPath} (ABI v${ffi.libraryAbiVersion}) lacks the speaker ABI (eliza_inference_speaker_supported() == 0). Rebuild with the WeSpeaker forward graph linked in.`);
    }
...
function parseFamilyMemberBody(raw: Record<string, unknown>): FamilyMemberBody | string {
    const audioBase64 = typeof raw.audioBase64 === "string" ? raw.audioBase64.trim() : null;
    if (!audioBase64)
    ...
    const durationMs = typeof raw.durationMs === "number" && raw.durationMs > 0
        ...
        : null;
    if (durationMs === null)
        return "durationMs must be a positive number";
    const displayName = typeof raw.displayName === "string" && raw.displayName.trim().length > 0
        ? raw.displayName.trim()
        : null;
    if (!displayName)
        return "displayName is required";
    const relationship = typeof raw.relationship === "string" && raw.relationship.trim().length > 0
        ? raw.relationship.trim()
        : "family";
    const ownerEntityId = typeof raw.ownerEntityId === "string" && raw.ownerEntityId.trim()
 ...
 */
export async function handleFamilyMemberRoute(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {
    ...
    const { audioBase64, durationMs, displayName, relationship, ownerEntityId } = parsed;
    // Decode audio.
    ...
    if (pcm.length < WESPEAKER_MIN_SAMPLES) {
        sendJsonError(res, `capture too short: ${pcm.length} PCM samples (< ${WESPEAKER_MIN_SAMPLES} required for ${WESPEAKER_SAMPLE_RATE} Hz encoder). Minimum ~${Math.ceil((WESPEAKER_MIN_SAMPLES / WESPEAKER_SAMPLE_RATE) * 1000)}ms.`, 400);
        return true;
    }
    ...
    try {
        ...
        logger.error({ err, displayName, relationship }, "[family-member-route] failed to create voice profile");
        sendJsonError(res, err instanceof Error ? err.message : "failed to create voice profile", 500);
        return true;
    }
