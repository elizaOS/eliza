...
export async function handleNativePcmTurnRoute(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {
    const method = (req.method ?? "GET").toUpperCase();
    const pathname = new URL(req.url ?? "/", "http://localhost").pathname;
    if (method !== "POST" || pathname !== ROUTE_PATH)
        return false;
    ...
    try {
        ...
        const exitReason = await localInferenceEngine.runVoiceTurn({ pcm, sampleRate }, { events: voiceEventsForTurn({ turnId, nativeSignal: body.signal }) });
        ...
        sendJsonError(res, error instanceof Error ? error.message : "native PCM voice turn failed", 503);
    }
    return true;
}
