...
- **Fused desktop inference is an install invariant.** Root `bun install` initializes the pinned llama.cpp fork, provisions supported host build prerequisites, and builds or freshness-checks the staged `libelizainference`. Local embedding GGUFs remain runtime-managed and download during warmup. Optional sd.cpp and mflux binaries are capability-specific; `probe:sd-cpp` checks for sd.cpp.
