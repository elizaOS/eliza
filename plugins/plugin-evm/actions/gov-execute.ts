...
export const executeAction = {
  ...
  handler: async (
    ...
    try {
      ...
      if (!isConfirmed(options)) {
        const preview = `Review governance execution before submitting: proposal ${executeParams.proposalId} with ${executeParams.targets.length} target(s) on ${executeParams.chain}, governor ${executeParams.governor}. Re-invoke ${spec.name} with confirmed: true to submit.`;
  ...
  validate: createEvmActionValidator({
    keywords: ["execute", "proposal", "governor", "timelock", "run"],
    regex: /\b(?:execute|execution|run|proposal|governor|timelock)\b/i,
  }),
  examples: [
    [
      {
    ...
    [
      {
        name: "user",
        content: {
          text: "Ejecuta la propuesta 88 del governor en Base",
          action: "EXECUTE_PROPOSAL",
        },
      },
    ],
    [
      {
        name: "user",
        content: {
          text: "Execute queued proposal now on Ethereum governor contract",
          action: "EXECUTE_PROPOSAL",
        },
      },
    ],
