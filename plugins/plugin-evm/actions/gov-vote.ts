...
const spec = requireActionSpec("GOV_VOTE");
...
export const voteAction = {
  ...
  handler: async (
    ...
    try {
      ...
      if (!isConfirmed(options)) {
        const preview = `Review governance vote before submitting: proposal ${voteParams.proposalId} on ${voteParams.chain}, governor ${voteParams.governor}, support ${voteParams.support}. Re-invoke ${spec.name} with confirmed: true to submit.`;
  ...
  validate: createEvmActionValidator({
    keywords: ["vote", "proposal", "governor", "dao", "abstain"],
    regex: /\b(?:vote|proposal|governor|dao|yes|no|abstain)\b/i,
  }),
  examples: [
    [
      {
    ...
    [
      {
        name: "user",
        content: {
          text: "Vota sí en la propuesta 321 del DAO en Base",
          action: "GOVERNANCE_VOTE",
        },
      },
    ],
    [
      {
        name: "user",
        content: {
          text: "Please cast a no vote for proposal 901 on Arbitrum governor",
          action: "GOVERNANCE_VOTE",
        },
      },
    ],
