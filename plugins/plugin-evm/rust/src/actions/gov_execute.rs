...
/// Parameters for executing a governance proposal
...
impl ExecuteAction {
    ...
    pub async fn execute(&self, params: ExecuteParams) -> EVMResult<Transaction> {
        params.validate()?;
        ...
        // Function selector: 0x2656227d
        let selector: [u8; 4] = [0x26, 0x56, 0x22, 0x7d];
        ...
        let pending_tx = chain_provider.send_transaction(tx).await.map_err(|e| {
            EVMError::new(
                ...
                format!("Failed to send execute transaction: {e}"),
            )
        })?;
        ...
        if !receipt.status() {
            return Err(EVMError::new(
                ...
                format!("Execute transaction reverted: {tx_hash}"),
            ));
        }
