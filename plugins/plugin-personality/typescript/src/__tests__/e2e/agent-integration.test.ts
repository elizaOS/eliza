import type { Character, TestSuite } from "@elizaos/core";
...
export class AgentIntegrationTestSuite implements TestSuite {
  ...
  description =
    "Integration tests for self-modification plugin with packages/agent autonomy loop";
  ...
  tests = [
    {
      ...
      fn: async (runtime: any) => {
        ...
        const hasSelfModPlugin = pluginNames.includes(
        ...
        if (!hasAutonomous) {
          console.log(
            "Warning: Autonomous plugin not loaded, autonomy loop integration limited",
          );
        }
...

    {
      ...
      fn: async (runtime: any) => {
        ...
        const newBioLength = Array.isArray(updatedCharacter.bio)
          ? updatedCharacter.bio.length
          : 1;
        ...
        const hasStartupContent =
          bioText.toLowerCase().includes("startup") ||
          bioText.toLowerCase().includes("mentor");
...

    {
      ...
      fn: async (runtime: any) => {
        ...
        const hasEvolutionContext = state.text?.includes(
          "CHARACTER EVOLUTION CONTEXT",
        );
        ...
        const expectedBioCount = Array.isArray(character.bio)
...

    {
      ...
      fn: async (runtime: any) => {
        ...
        if (!validation.valid) {
          throw new Error(
            `Valid modification was rejected: ${validation.errors.join(", ")}`,
          );
        }

        // Test applying modification
        const applyResult =
          await fileManager.applyModification(validModification);
        if (!applyResult.success) {
          throw new Error(
            `Modification application failed: ${applyResult.error}`,
          );
        }
        ...
        const hasAgentContent =
          bioText.toLowerCase().includes("autonomous") ||
          bioText.toLowerCase().includes("agent");
...

    {
      ...
      fn: async (runtime: any) => {
        ...
        if (!hasAutonomous) {
          console.log(
            "Autonomous plugin not available, skipping autonomy integration test",
          );
          return;
        }
        ...
        if (shouldEvaluate) {
          ...
          console.log(
            "Evolution evaluator did not trigger (may be expected based on content)",
          );
        }
        ...
        if (evolutionMemories.length > 0) {
          console.log(
            `Found ${evolutionMemories.length} evolution suggestion(s)`,
          );
          const suggestion = evolutionMemories[0];
          console.log(
            "Evolution reasoning:",
            suggestion.content.metadata?.evolution?.reasoning,
          );
        }
