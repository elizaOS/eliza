"""
Error definitions for the N8n Plugin.
"""
...
class N8nError(Exception):
    """Base exception for N8n plugin errors."""
...
class ApiKeyError(ConfigError):
    ...
    def __init__(self, provider: str = "ANTHROPIC") -> None:
        ...
        super().__init__(
            f"{provider}_API_KEY",
            f"{provider}_API_KEY is not configured. Please set it to enable AI-powered plugin generation.",
        )


class ValidationError(N8nError):
    """Validation error for plugin specifications."""
...
class JobError(N8nError):
    """Error related to plugin creation jobs."""
