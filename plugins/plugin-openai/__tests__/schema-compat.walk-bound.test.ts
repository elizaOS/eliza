...
describe("normalizeSchemaForCerebras walk bound", () => {
  ...
  it("fails closed on a JSON.parse-legal over-deep properties nest", () => {
    ...
    try {
      ...
      expect(Date.now() - started).toBeLessThan(250);
      expect(isCerebrasSchemaUnbounded(error)).toBe(true);
      expect((error as ElizaError).context?.max).toBe(MAX_CEREBRAS_SCHEMA_WALK_DEPTH);
      expect(error).not.toBeInstanceOf(RangeError);
    }
  });
...
describe("normalizeSchemaForCerebras hostile caller inputs", () => {
  ...
  it("does not execute get/has/prototype traps on an honest object Proxy", () => {
    ...
    const result = normalizeCerebrasToolSchema(proxy) as Record<string, unknown>;
  ...
  it("walks sparse arrays from own length/index descriptors only", () => {
    ...
    const result = normalizeCerebrasToolSchema(schema) as Record<string, unknown>;
  ...
  it("accepts a schema at the exact depth budget and rejects one step past", () => {
    ...
    const result = normalizeCerebrasToolSchema(atBudget) as Record<string, unknown>;
    expect(result.type).toBe("object");

    try {
      normalizeCerebrasToolSchema(deepProperties(MAX_CEREBRAS_SCHEMA_WALK_DEPTH + 1));
      throw new Error("expected unbounded throw");
    } catch (error) {
      const typed = expectUnbounded(error);
  ...
  it("fails closed on an over-wide own-key node before cloning", () => {
    ...
    try {
      ...
      expect((typed.context?.visits as number) > MAX_CEREBRAS_SCHEMA_WALK_NODES).toBe(true);
    }
  });
});
...
describe("cloneSchemaForBoundedTransport", () => {
  ...
  it("accepts and rejects exactly what normalizeSchemaForCerebras does", () => {
    ...
    const cases: Array<[string, unknown]> = [
      ["shaw repro default", { type: "object", properties: {}, default: annotation }],
      ["20k default", { type: "object", properties: {}, default: deepAnnotation }],
      ["20k examples", { type: "object", properties: {}, examples: [deepAnnotation] }],
      ["20k extension", { type: "object", properties: {}, "x-vendor": deepAnnotation }],
      ...
      ["properties past budget", deepProperties(MAX_CEREBRAS_SCHEMA_WALK_DEPTH + 1)],
    ];
    ...
    for (const [label, schema] of cases) {
      expect([label, outcome(() => cloneSchemaForBoundedTransport(schema))]).toEqual([
        label,
        outcome(() => normalizeSchemaForCerebras(schema, true, { strict: true })),
      ]);
    }
  });
  ...
  it("fails closed past the schema depth budget", () => {
    expect(() =>
      cloneSchemaForBoundedTransport(deepProperties(MAX_CEREBRAS_SCHEMA_WALK_DEPTH))
    ).not.toThrow();
    try {
      cloneSchemaForBoundedTransport(deepProperties(MAX_CEREBRAS_SCHEMA_WALK_DEPTH + 1));
      throw new Error("expected unbounded throw");
    } catch (error) {
      const typed = expectUnbounded(error);
      expect(typed.context?.max).toBe(MAX_CEREBRAS_SCHEMA_WALK_DEPTH);
  ...
  it("reserves array width before allocating a huge sparse array", () => {
    ...
    try {
      ...
      expect(Date.now() - started).toBeLessThan(2000);
      expect(expectUnbounded(error).context?.maxNodes).toBe(MAX_CEREBRAS_SCHEMA_WALK_NODES);
    }
  });
