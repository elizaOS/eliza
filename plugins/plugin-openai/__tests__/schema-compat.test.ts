...
describe("normalizeSchemaForCerebras", () => {
  it.each(["integer", "number"])(
    ...
    (type) => {
      for (const nullFirst of [false, true]) {
        ...
        const normalized = normalizeSchemaForCerebras(schema, true) as typeof schema;
        const originalValidator = z.fromJSONSchema(schema);
        const normalizedValidator = z.fromJSONSchema(normalized);
        for (const value of [null, 1, 1.5, 50, 51, 0, -1, "50", "null", {}, []]) {
        ...
        expect(normalizeSchemaForCerebras(normalized, true)).toEqual(normalized);
        expect(normalizeSchemaForCerebras(schema, true, { strict: false })).toEqual(schema);
      }
    }
  );
  ...
  it("preserves nullable unions with sibling or branch constraints it cannot combine", () => {
    for (const limit of [
      ...
      {
        anyOf: [{ type: "integer", description: "Only whole records." }, { type: "null" }],
      },
    ]) {
      const schema = {
        type: "object",
  ...
  it("returns a closed empty object schema for a non-object root", () => {
    const result = normalizeSchemaForCerebras(undefined, true) as Record<string, unknown>;
  ...
  it("recurses into nested object properties", () => {
    ...
    const inner = (result.properties as Record<string, Record<string, unknown>>).inner;
  ...
  it("closes a bare nested object property", () => {
    ...
    const params = (result.properties as Record<string, Record<string, unknown>>).params;
  ...
  it("rewrites nested oneOf to anyOf under strict mode", () => {
    ...
    const recurrence = (result.properties as Record<string, unknown>).recurrence as Record<
      string,
      unknown
    >;
  ...
  it("walks every schema-bearing keyword", () => {
    ...
    expect((result.dependencies as Record<string, unknown>).names).toEqual(["direct"]);
  ...
  it("closes inferred and nullable object nodes", () => {
    ...
    expect(normalizeSchemaForCerebras({ type: ["object", "null"] })).toMatchObject({
      type: ["object", "null"],
      properties: {},
      additionalProperties: false,
    });
...
describe("sanitizeFunctionNameForCerebras", () => {
  it("rewrites dotted identifiers", () => {
    expect(sanitizeFunctionNameForCerebras("math.factorial")).toBe("math_factorial");
