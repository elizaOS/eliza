...
export function getRequiredSetting(runtime: IAgentRuntime, key: string, errorMessage?: string): string {
...
export function getNumericSetting(runtime: IAgentRuntime, key: string, defaultValue: number): number {
    const value = getSetting(runtime, key);
    if (value === undefined) {
        return defaultValue;
    }
    ...
    if (!Number.isSafeInteger(parsed) || parsed <= 0) {
        throw new Error(`Setting '${key}' must be a positive integer within JavaScript's safe range, got: ${value}`);
    }
    return parsed;
}
export function getBooleanSetting(runtime: IAgentRuntime, key: string, defaultValue: boolean): boolean {
    const value = getSetting(runtime, key);
    if (value === undefined) {
        return defaultValue;
    }
 ...
 */
export function getUsageProvider(runtime: IAgentRuntime): "cerebras" | "evolink" | "openai" | "openrouter" {
...
export function getAuthHeader(runtime: IAgentRuntime, forEmbedding = false): Record<string, string> {
 ...
 */
export function resolveOpenAIBaseURL(readSetting: EndpointSettingReader, options: {
    mockBaseURL?: string;
} = {}): string {
...
export function getBaseURL(runtime: IAgentRuntime): string {
    const baseURL = resolveOpenAIBaseURL((key) => {
        const runtimeValue = runtime.getSetting(key);
        const normalizedRuntime = normalizeEndpointSetting(runtimeValue === undefined || runtimeValue === null ? undefined : String(runtimeValue));
...
export function getSmallModel(runtime: IAgentRuntime): string {
    return (getSetting(runtime, "OPENAI_SMALL_MODEL") ??
...
export function getNanoModel(runtime: IAgentRuntime): string {
    return (getSetting(runtime, "OPENAI_NANO_MODEL") ??
        getCerebrasSmallModel(runtime) ??
        getEvoLinkModel(runtime) ??
...
export function getMediumModel(runtime: IAgentRuntime): string {
    return (getSetting(runtime, "OPENAI_MEDIUM_MODEL") ??
        getCerebrasSmallModel(runtime) ??
        getEvoLinkModel(runtime) ??
...
export function getLargeModel(runtime: IAgentRuntime): string {
    return (getSetting(runtime, "OPENAI_LARGE_MODEL") ??
...
export function getActionPlannerModel(runtime: IAgentRuntime): string {
    return (getSetting(runtime, "OPENAI_ACTION_PLANNER_MODEL") ??
