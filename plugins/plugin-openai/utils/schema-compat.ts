...
function failCerebrasSchemaUnbounded(context: Record<string, unknown>, cause?: unknown): never {
...
export function isCerebrasSchemaUnbounded(error: unknown): boolean {
  return error instanceof ElizaError && error.code === CEREBRAS_SCHEMA_UNBOUNDED;
}
...
function cloneOwnData(value: object, ctx: SchemaWalkContext): Record<string, unknown> {
...
function cloneSchemaTransportMap(
  ...
  for (const entry of ownDataEntries(value, ctx)) {
    defineOwnData(mapped, entry.key, cloneSchemaTransportWalk(entry.value, depth + 1, ctx));
  }
  return mapped;
}
...
function cloneSchemaTransportWalk(schema: unknown, depth: number, ctx: SchemaWalkContext): unknown {
...
export const JSON_SCHEMA_ARRAY_KEYWORDS = ["anyOf", "oneOf", "allOf", "prefixItems"] as const;
...
function describesObjectSchema(node: Record<string, unknown>): boolean {
  ...
  return (
    type === "object" ||
    (safeIsArray(type) && ownArrayValues(type as unknown[]).includes("object")) ||
...
function enforceStrictObjectShape(node: Record<string, unknown>): void {
  ...
  const hasProperties =
    isSchemaRecord(properties) && ownEnumerableStringKeys(properties).length > 0;
...
function normalizeNullableNumber(node: Record<string, unknown>): void {
  if (
    Object.keys(node).some((key) => !["anyOf", "description", "title"].includes(key)) ||
  ...
  const nullBranch = branches.find(
    (branch) => isSchemaRecord(branch) && branch.type === "null" && Object.keys(branch).length === 1
  );
  const numeric = branches.find(
    (branch) => isSchemaRecord(branch) && (branch.type === "number" || branch.type === "integer")
  );
  ...
  if (
    Object.entries(numeric).some(
      ([key, value]) =>
        key !== "type" &&
        (!bounds.includes(key) || typeof value !== "number" || !Number.isFinite(value))
    )
  ) {
    return;
  }
...
function mapSchemaRecord(
  ...
  for (const entry of ownDataEntries(value, ctx)) {
    defineOwnData(
      mapped,
      entry.key,
      normalizeSchemaForCerebrasWalk(entry.value, false, options, depth + 1, ctx)
    );
  }
  return mapped;
}
...
function walkSchemaChildren(
  ...
  if (isArrayRecord(items)) {
    ...
    node.items = normalizeSchemaForCerebrasWalk(items, false, options, depth + 1, ctx);
  }

  for (const key of SCHEMA_SINGLE_KEYS) {
    const value = node[key];
    if (!isSchemaRecord(value)) continue;
    node[key] = normalizeSchemaForCerebrasWalk(value, false, options, depth + 1, ctx);
  }
...
function normalizeSchemaForCerebrasWalk(
  ...
  if (!schema || typeof schema !== "object" || safeIsArray(schema)) {
    ...
    if (isRoot) {
      return options.strict === false ? { type: "object" } : closedEmptyObjectSchema();
    }
    return schema;
  }
  ...
  try {
    ...
    if (options.strict !== false) {
      ...
      if (hasNonEmptyOwnArray(node.oneOf)) {
        const existing = isArrayRecord(node.anyOf) ? ownArrayValues(node.anyOf) : [];
...
export function normalizeSchemaForCerebras(
  ...
  return normalizeSchemaForCerebrasWalk(schema, isRoot, options, 0, createSchemaWalkContext());
}
