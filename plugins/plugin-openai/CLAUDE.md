...
- **Cerebras mode.** Detected automatically from `ELIZA_PROVIDER=cerebras`, `OPENAI_BASE_URL` matching `*.cerebras.ai`, or presence of `CEREBRAS_API_KEY` without `OPENAI_API_KEY`. In Cerebras mode: caller-supplied response schemas use strict `json_schema` output without OpenAI's all-properties-required rewrite; explicit JSON-only requests retain `json_object`; `reasoning_effort` defaults to `"none"` for Qwen and `"low"` for GPT-OSS and GLM; `promptCacheRetention` is stripped (Cerebras rejects it); embeddings fall back to a deterministic local hash when no explicit embedding URL is set.
...
- **Cerebras optional tool arguments stay optional.** The shared sanitizer preserves declared `required` fields for Cerebras native tools, including nested objects and strict-safe record values. Other providers retain their all-properties-required strict contract. Do not make unused operation arguments mandatory: that forces invented values (such as an empty MEMORY snapshot) and unnecessary validation/replanning. Unsupported grammar constraints still become description hints, and the runtime still validates the original action schema.
