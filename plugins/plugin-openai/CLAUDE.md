...
- **Cerebras mode.** Detected automatically from `ELIZA_PROVIDER=cerebras`, `OPENAI_BASE_URL` matching `*.cerebras.ai`, or presence of `CEREBRAS_API_KEY` without `OPENAI_API_KEY`. For exact `qwen-3.8-27b` calls without native tools, supported plain response schemas use strict `json_schema`, retaining optional fields through the shared sanitizer and core normalizer. Schemas that need shape-changing normalization or unsupported grammar constructs retain the existing `json_object` contract and caller-side validation; opaque SDK output contracts, other models, native-tool calls, and schema-less JSON requests are unchanged. The [current Cerebras structured-output contract](https://inference-docs.cerebras.ai/capabilities/structured-outputs) supports optional fields and forbids assuming that tools and response formats can be combined. Qwen defaults to `reasoning_effort: "none"`; GPT-OSS and GLM retain their model-specific `"low"` defaults. `promptCacheRetention` is stripped; embeddings fall back to a deterministic local hash when no explicit embedding URL is set.
