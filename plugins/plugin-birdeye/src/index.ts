...
export const birdeyePlugin: Plugin = {
  ...
  init: async (_, runtime: IAgentRuntime) => {
    ...
    Promise.all(
      ['INTEL_DATAPROVIDER', BIRDEYE_SERVICE_NAME].map(
        p => runtime.getServiceLoadPromise(p as ServiceTypeName)
      )
    ).then(() => {
      const infoService = runtime.getService('INTEL_DATAPROVIDER') as any;

      // Guard against missing service
      if (!infoService) {
        runtime.logger?.warn('INTEL_DATAPROVIDER service not available, skipping data provider registration');
        return;
      }

      if (typeof infoService.registerDataProvder !== 'function') {
        runtime.logger?.warn('INTEL_DATAPROVIDER service does not have registerDataProvder method');
        return;
      }
