 ...
 */
export const extractChain = (text?: string, explicitChain?: string): BirdeyeSupportedChain => {
    ...
    if (explicitChain) {
        ...
        if (BIRDEYE_SUPPORTED_CHAINS.includes(normalizedChain as BirdeyeSupportedChain)) {
...
export const extractTimeRange = (text: string): { start: number; end: number } => {
    const now = Math.floor(Date.now() / 1000);

    // Check for specific date ranges
    const dateRangeMatch = text.match(/from\s+(\d{4}-\d{2}-\d{2})\s+to\s+(\d{4}-\d{2}-\d{2})/i);
    if (dateRangeMatch) {
...
export async function makeApiRequest<T>(
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new BirdeyeApiError(response.status, `HTTP error! status: ${response.status}`);
        }
...
export const formatTokenInfo = (
    ...
    const volume =
        token.volume_24h_usd != null ? `$${(token.volume_24h_usd / 1_000_000).toFixed(2)}M` : 'N/A';

    const liquidity =
        token.liquidity != null ? `$${(token.liquidity / 1_000_000).toFixed(2)}M` : 'N/A';

    const fdv = token.fdv != null ? `$${(token.fdv / 1_000_000).toFixed(2)}M` : 'N/A';
...
const formatSocialLinks = (data: TokenMetadataSingleResponse['data']): string => {
    ...
    if (extensions.coingecko_id) {
        links.push(`🦎 [CoinGecko](https://www.coingecko.com/en/coins/${extensions.coingecko_id})`);
    }
