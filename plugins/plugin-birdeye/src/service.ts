...
export class BirdeyeService extends Service {
  ...
  private async getTrendingTokensForChain(chain: Chain, options?: { notOlderThan?: number; total?: number }): Promise<CacheWrapper<IToken[]>> {
    ...
    try {
      ...
      this.runtime.logger?.warn(`getTrendingTokensForChain: ${error instanceof Error ? error.message : String(error)}`);
      return { data: [], setAt: Date.now() }
    }

    // Don't allow 'evm' as a trending chain (it's just a placeholder)
    if (validatedChain === 'evm') {
      this.runtime.logger?.warn(`getTrendingTokensForChain: 'evm' is not a specific chain. Use ethereum, arbitrum, polygon, etc.`);
      return { data: [], setAt: Date.now() }
    }

    const cacheKey = `tokens_v2_${validatedChain}`;
    const wrapper = await this.runtime.getCache<CacheWrapper<IToken[]>>(cacheKey);

    const freshEnough =
      !!wrapper &&
      (!options?.notOlderThan || Date.now() - wrapper.setAt <= options.notOlderThan);

    if (freshEnough) {
      this.runtime.logger.debug('birdyeSrv::getTrendingTokensForChain(' + chain + ') HIT')
      return wrapper
    }
    this.runtime.logger.debug('birdyeSrv::getTrendingTokensForChain(' + chain + ') MISS')
    ...
    const settled = await Promise.allSettled(
      OFFSETS.map(async (offset) => {
        ...
        return (data.tokens as any[]).map((token) => ({
    ...
    const fetched = settled
      .filter((r): r is PromiseFulfilledResult<IToken[]> => r.status === 'fulfilled')
      .flatMap((r) => r.value);
  ...
  async getTrendingTokens(chains: Chain[], options?: { notOlderThan?: number }): Promise<Record<Chain, IToken[]>> {
    try {
      ...
      this.runtime.logger?.error(`Failed to sync trending tokens: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async getTokenMarketData(tokenAddress: string): Promise<{
    ...
    try {
      ...
      if (!data.data) {
        this.runtime.logger.warn(`getTokenMarketData - cant save result for ${tokenAddress}: ${JSON.stringify(data)}`);
  ...
  */
  ...
  async getTokenTradeData(chain: string, tokenAddress: string, frames = '2h,8h,24h', options: GetCacheTimedOptions = {}): Promise<any> {
    ...
    const notOlderThan = options.notOlderThan ?? CACHE_DEFAULTS.TOKEN_TRADE_DATA_TTL;

    // Check cache
    const cached = await this.getCacheTimed<any>(key, { notOlderThan });
    if (cached) {
      return cached;
    }
    ...
    try {
      ...
      this.runtime.logger?.error(`birdeye:getTokenTradeData - ${e instanceof Error ? e.message : String(e)}`);
      return false;
    }
  }
  ...
  */
  ...
  // FIXME: caching
  async getTokensMarketData(chain: string, tokenAddresses: string[]): Promise<any> {
    const tokenDb: Record<string, any> = {};
    ...
    try {
      ...
      const batchesWithAddresses = hundos
        ...
        .filter((item): item is { addresses: string[], promise: Promise<Response> } => item !== undefined);

      const multipriceResps = await Promise.all(batchesWithAddresses.map(b => b.promise));
      const multipriceData = await Promise.all(
        ...
        for (const ca of batchAddresses) {
          ...
          if (t) {
            ...
            */
            ...
            this.runtime.logger?.debug(`birdeye:getTokensMarketData - caching token ${ca}`);
  ...
  async getTokenSecurityData(chain: string, tokenAddress: string, options: GetCacheTimedOptions = {}): Promise<any> {
    ...
    const notOlderThan = options.notOlderThan ?? CACHE_DEFAULTS.TOKEN_SECURITY_DATA_TTL;
    ...
    try {
      ...
      this.runtime.logger?.error(`birdeye:getTokenSecurityData - ${e instanceof Error ? e.message : String(e)}`);
      return false;
    }
  }
  ...
  */
  ...
  // lookup token
  async lookupToken(chain: string, ca: string, options: GetCacheTimedOptions = {}) {
    try {
  ...
  async lookupTokens(
    ...
    try {
      ...
      const results = await Promise.all(
        chainAndAddresses.map(cAA => this.lookupToken(cAA.chain, cAA.address, options))
      )
      ...
      this.runtime.logger?.error(`birdeyeSvr:lookupTokens - err: ${e instanceof Error ? e.message : String(e)}`);
      throw e
    }
  }

  async fetchSearchTokenMarketDataChain(chain: string, params: any, options: GetCacheTimedOptions = {}) {
    const birdeyeFetchOptions: RequestInit = this.getBirdeyeFetchOptions(chain)
    const res = await fetch(
  ...
  async lookupSymbolAllChains(
    ...
    const birdeyeFetchOptions: RequestInit = this.getBirdeyeFetchOptions('solana')
    const res = await fetch(
  ...
  async fetchWalletTokenList(chain: BirdeyeSupportedChain, publicKey: string, options: GetCacheTimedOptions = {}) {
  ...
  async fetchWalletTxList(chain: BirdeyeSupportedChain, publicKey: string, options: GetCacheTimedOptions = {}) {
    // set up cache
