...
import { extractSymbols, formatPercentChange, formatPrice, formatValue } from '../utils';
...
export const tokenSearchSymbolAction = {
    ...
    handler: async (
        ...
        try {
            ...
            const symbols = extractSymbols(message.content?.text ?? '', SYMBOL_SEARCH_MODE);

            elizaLogger.info(`Searching Birdeye provider for ${symbols.length} symbols`);
                ...
                r.data.items
                    ...
                    .flatMap((item) =>
                        (item.result as TokenResult[]).filter(
                            (r) => r.symbol?.toLowerCase() === symbols[i].toLowerCase()
                        )
                    )
            ) as TokenResult[][];
            ...
            if (validResults.length === 0) {
                callback?.({
                    text: 'No token results found for the symbols you searched.',
                });
                return;
            }
            ...
            const completeResults = `I performed a search for the token symbols you requested and found the following results (for more details search by contract address):\n\n${validResults
                .map((result, i) => `${formatTokenSummary(symbols[i], i, result)}`)
                .join('\n')}`;
...
const formatTokenSummary = (symbol: string, _index: number, tokens: TokenResult[]) => {
    return tokens
        .map((token, i) => {
            let output = '';
            if (i === 0) {
