...
export const tradePortfolioProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    try {
      ...
      if (!beService || typeof beService.fetchWalletTokenList !== 'function' || typeof beService.fetchWalletTxList !== 'function') {
      ...
      const [portfolio, trades] = await Promise.all([
        beService.fetchWalletTokenList(chain, walletAddr, { notOlderThan: 30 * 1000 }),
        beService.fetchWalletTxList(chain, walletAddr, { notOlderThan: 30 * 1000 }),
      ])

      // Guard against missing portfolio data
      if (!portfolio || !portfolio.wallet) {
