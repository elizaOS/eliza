...
import { CloudApiError } from "../utils/cloud-api";
...
function shapeTokenResponse(payload: unknown): {
    idToken: string;
} {
 ...
 */
export function decideRevalidation(prev: RevalidationState, probe: ApiKeyProbe, cfg: RevalidationConfig = DEFAULT_REVALIDATION_CONFIG): RevalidationDecision {
    if (probe === "valid") {
        const log = prev.keyState !== "valid"
        ...
        if (confirmed) {
            const log = prev.keyState !== "invalid"
                ? {
                    ...
                    message: "[CloudAuth] Eliza Cloud API key is REVOKED/INVALID (cloud reachable, key rejected). " +
...
export class CloudAuthService extends Service {
    ...
    private async runRevalidation(): Promise<void> {
        ...
        const probe = await this.probeApiKey(key).catch(() => "unreachable" as ApiKeyProbe);
