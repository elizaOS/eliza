...
} & Partial<Record<(typeof CLOUD_AUTH_CLEAR_METHODS)[number], (() => Promise<unknown>) | (() => unknown)>>;
...
function cloudCreditsHttpErrorMessage(status: number, creditResponse: {
    error?: unknown;
}): string {
...
export function getCloudAuth(runtime: AgentRuntime | null): CloudAuthLike | null {
...
function resolvePersistedCloudIdentity(runtime: AgentRuntime | null): {
    ...
    return {
        organizationId: normalizeEnvValue(runtimeWithCloud?.getSetting?.("ELIZA_CLOUD_ORGANIZATION_ID") as string | undefined) ??
            normalizeEnvValue(runtimeWithCloud?.character?.secrets?.ELIZA_CLOUD_ORGANIZATION_ID as string | undefined),
        userId: normalizeEnvValue(runtimeWithCloud?.getSetting?.("ELIZA_CLOUD_USER_ID") as string | undefined) ??
            normalizeEnvValue(runtimeWithCloud?.character?.secrets?.ELIZA_CLOUD_USER_ID as string | undefined),
    };
}
export function resolveCloudApiBaseUrl(rawBaseUrl?: string): string {
    return resolveCanonicalCloudApiBaseUrl(rawBaseUrl);
}
export function resolveCloudApiKey(config: Pick<ElizaConfig, "cloud"> | Record<string, unknown>, runtime?: {
    character?: {
    ...
    if (resolveDevCloudEnvAuthority()) {
        return normalizeEnvValue(resolveDevCloudAuthorityEnvValue("ELIZAOS_CLOUD_API_KEY"));
    }
    migrateLegacyRuntimeConfig(config as Record<string, unknown>);
    // 1. Config file (disk)
    const configApiKey = normalizeEnvValue((config as {
        cloud?: {
            apiKey?: string;
        };
    }).cloud?.apiKey);
    ...
    const runtimeSettingKey = normalizeEnvValue(runtime?.getSetting?.("ELIZAOS_CLOUD_API_KEY") as string | undefined);
    if (runtimeSettingKey)
        return runtimeSettingKey;
    // 5. Runtime character secrets (persisted in database, survives restarts)
    const runtimeKey = normalizeEnvValue(runtime?.character?.secrets?.ELIZAOS_CLOUD_API_KEY as string | undefined);
    if (runtimeKey)
        return runtimeKey;
    return undefined;
}
export function resolveCloudConnectionSnapshot(config: Partial<ElizaConfig>, runtime: AgentRuntime | null): CloudConnectionSnapshot {
    const devCloudAuthority = resolveDevCloudEnvAuthority();
    migrateLegacyRuntimeConfig(config as Record<string, unknown>);
    const _cloudRecord = config.cloud && typeof config.cloud === "object"
        ? (config.cloud as Record<string, unknown>)
        : undefined;
    const enabled = isCloudInferenceSelectedInConfig(config as Record<string, unknown>);
...
async function fetchCloudCreditsByApiKey(baseUrl: string, apiKey: string): Promise<number | null> {
    ...
    const creditResponse = (await response.json().catch((err: unknown) => {
        ...
        logger.warn(`[cloud-connection] Failed to parse credit balance response JSON: ${err instanceof Error ? err.message : String(err)}`);
        return {};
    })) as {
        balance?: unknown;
        data?: {
...
function withCreditFlags(balance: number, topUpUrl: string): CloudCreditsResponse {
    return {
        connected: true,
        balance,
...
export async function fetchCloudCredits(config: Partial<ElizaConfig>, runtime: AgentRuntime | null): Promise<CloudCreditsResponse> {
    ...
    if (snapshot.authConnected &&
        (typeof cloudClient?.requestData === "function" ||
            typeof cloudClient?.get === "function")) {
        try {
            const creditResponse = typeof cloudClient.requestData === "function"
                ? await cloudClient.requestData("GET", "/credits/balance")
                : await cloudClient.get?.("/credits/balance");
            const nestedData = typeof creditResponse === "object" && creditResponse !== null
                ? Reflect.get(creditResponse, "data")
                : undefined;
            const rawBalance = coerceCloudBalance(typeof creditResponse === "object" && creditResponse !== null
                ? Reflect.get(creditResponse, "balance")
                : undefined) ??
                coerceCloudBalance(typeof nestedData === "object" && nestedData !== null
...
export async function clearCloudAuthService(cloudAuth: CloudAuthLike | null): Promise<void> {
    if (!cloudAuth) {
        return;
    }
...
async function clearRuntimeCloudState(runtime: AgentRuntime | null): Promise<void> {
    const runtimeWithCloud = asRuntimeCloud(runtime);
    if (!runtimeWithCloud) {
        return;
    }
    ...
    if (runtimeWithCloud.character.settings &&
...
export function isCloudStatusReasonApiKeyOnly(reason: string | null | undefined): boolean {
