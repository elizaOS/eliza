...
import { handleCloudSttRoute } from "./server-cloud-tts";
...
let upstreamResponse: () => Response = () => new Response(new Uint8Array([73, 68, 51]), {
    status: 200,
 ...
 */
function fakeReqPreParsed(parsed: Record<string, unknown>, headers: Record<string, string> = {}): http.IncomingMessage {
    return {
        headers,
        body: parsed,
...
beforeEach(() => {
    ...
    upstream = [];
    upstreamResponse = () => new Response(new Uint8Array([73, 68, 51]), {
        status: 200,
        headers: { "content-type": "audio/mpeg" },
    });
...
afterAll(() => {
    ...
    if (prevApiKey === undefined)
        ...
        process.env.ELIZAOS_CLOUD_API_KEY = prevApiKey;
    if (prevConfigPath === undefined)
        delete process.env.ELIZA_CONFIG_PATH;
    else
        process.env.ELIZA_CONFIG_PATH = prevConfigPath;
    if (prevTtsDebug === undefined)
        delete process.env.ELIZA_TTS_DEBUG;
    else
        process.env.ELIZA_TTS_DEBUG = prevTtsDebug;
});
afterEach(() => {
    vi.useRealTimers();
    vi.unstubAllEnvs();
...
describe("handleCloudTtsPreviewRoute (/api/tts/cloud proxy)", () => {
    test("forwards the client's Idempotency-Key upstream (#16425)", async () => {
        ...
        await handleCloudTtsPreviewRoute(fakeReq(JSON.stringify({ text: "bill me once" }), {
    ...
    test("keeps deeply nested stage directions out of the upstream speech request", async () => {
        ...
        const { res, state } = fakeRes();
        await handleCloudTtsPreviewRoute(fakeReq(JSON.stringify({ text: `Say this. ${nestedDirection} Done.` })), res);
        expect(state.statusCode).toBe(200);
        expect(upstream).toHaveLength(1);
        expect(upstream[0].body).toMatchObject({ text: "Say this. Done." });
        expect(JSON.stringify(upstream[0].body)).not.toContain("secret-");
    });
    test.each(["*", "**"])("keeps deeply nested %s directions out of the upstream speech request", async (marker) => {
    ...
    test("honors a host pre-parsed JSON body when the stream is already drained (#16348)", async () => {
        const { res, state } = fakeRes();
        await handleCloudTtsPreviewRoute(fakeReqPreParsed({ text: "pre-parsed on the bare server" }, { "content-type": "application/json" }), res);
        expect(state.statusCode).toBe(200);
        expect(upstream.length).toBeGreaterThanOrEqual(1);
    ...
    test("pre-parsed body still forwards modelId/voiceId and the idempotency key (#16348)", async () => {
        const { res, state } = fakeRes();
        await handleCloudTtsPreviewRoute(fakeReqPreParsed({ text: "hi", modelId: "m-9", voiceId: "v-3" }, {
    ...
    test("forwards a non-retryable upstream error status and body", async () => {
        upstreamResponse = () => new Response(JSON.stringify({ error: "Insufficient credits" }), {
            status: 402,
    ...
    test("retries an explicit cache-warming 503 before returning audio", async () => {
        ...
        vi.useFakeTimers();
        const { res, state } = fakeRes();
        const handled = handleCloudTtsPreviewRoute(fakeReq(JSON.stringify({ text: "warm up" })), res);
    ...
    test("passes an abort signal on the default TTS timeout", async () => {
        const { res, state } = fakeRes();
        await handleCloudTtsPreviewRoute(fakeReq(JSON.stringify({ text: "signal me" })), res);
        expect(state.statusCode).toBe(200);
    ...
    test("aborts a hanging TTS upstream instead of waiting forever", async () => {
        ...
        try {
            const { res, state } = fakeRes();
            await handleCloudTtsPreviewRoute(fakeReq(JSON.stringify({ text: "do not hang" })), res);
...
describe("handleCloudSttRoute (/api/asr/cloud proxy)", () => {
    test.each([
    ...
    ])("sends speech to the login environment for dev source %s", async (devSource, expectedUrl) => {
        ...
        const { res, state } = fakeRes();
        await handleCloudSttRoute(fakeReq("RIFF-audio", { "content-type": "audio/wav" }), res);
    ...
    test("retries an explicit cache-warming 503 before returning a transcript", async () => {
        ...
        vi.useFakeTimers();
        const { res, state } = fakeRes();
        const handled = handleCloudSttRoute(fakeReq("RIFF-audio", { "content-type": "audio/wav" }), res);
        await vi.runAllTimersAsync();
        await handled;
        expect(state.statusCode).toBe(200);
        ...
        expect(upstream).toHaveLength(2);
        expect(upstream.every((request) => request.body instanceof FormData)).toBe(true);
    });
    test("does not retry a non-warming 503", async () => {
        upstreamResponse = () => new Response(JSON.stringify({ error: "Whisper unavailable" }), {
            status: 503,
        });
        const { res, state } = fakeRes();
        await handleCloudSttRoute(fakeReq("RIFF-audio", { "content-type": "audio/wav" }), res);
...
describe("ELIZA_TTS_DEBUG tracing on /api/tts/cloud", () => {
    ...
    const ttsLines = () => entries.filter((entry) => entry.msg.includes("[eliza][tts]"));
    beforeEach(() => {
        entries = [];
        unsubscribe?.();
    ...
    test("successful proxy emits proxy → upstream-ok → success phases", async () => {
        ...
        const { res, state } = fakeRes();
        await handleCloudTtsPreviewRoute(fakeReq(JSON.stringify({ text: "trace me please" }), {
            "x-elizaos-tts-message-id": "msg-42",
        }), res);
        expect(state.statusCode).toBe(200);
        const phases = ttsLines().map((entry) => entry.msg);
        expect(phases.some((m) => m.includes("[eliza][tts] server:cloud-tts:proxy"))).toBe(true);
        expect(phases.some((m) => m.includes("[eliza][tts] server:cloud-tts:upstream-ok"))).toBe(true);
        expect(phases.some((m) => m.includes("[eliza][tts] server:cloud-tts:success"))).toBe(true);
    ...
    test("missing cloud key emits a reject phase with the reason", async () => {
        ...
        await handleCloudTtsPreviewRoute(fakeReq(JSON.stringify({ text: "hi" })), res);
        expect(state.statusCode).toBe(401);
        const reject = ttsLines().find((entry) => entry.msg.includes("server:cloud-tts:reject"));
    ...
    test("flag unset → the same successful request emits zero tts lines", async () => {
        delete process.env.ELIZA_TTS_DEBUG;
        const { res, state } = fakeRes();
        await handleCloudTtsPreviewRoute(fakeReq(JSON.stringify({ text: "silent run" })), res);
        expect(state.statusCode).toBe(200);
