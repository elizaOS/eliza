...
export async function handleCloudRelayRoute(_req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, state: CloudRelayRouteState, helpers: RouteHelpers): Promise<boolean> {
    ...
    const service = (state.runtime.getService("CLOUD_MANAGED_GATEWAY_RELAY") ??
        ...
        state.runtime.getService("cloudManagedGatewayRelay")) as RelayServiceLike | null;
...
function readRuntimeSetting(runtime: CloudRelayRouteState["runtime"], key: string): string | null {
