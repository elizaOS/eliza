...
export const HOME_SATELLITE_ACCESS_SESSION_PARAM = "homeSatelliteSession";

export interface HomeSatelliteSshTunnel {
  command: string;
  localUrl: string;
}

export function buildHomeSatelliteAccessUrl(input: {
  ...
  try {
    ...
    url.search = "";
    url.hash = "";
    url.searchParams.set(HOME_SATELLITE_ACCESS_SESSION_PARAM, sessionId);
    return url.toString();
  } catch {
    return null;
  }
...
export function buildHomeSatelliteSshTunnel(input: {
  satelliteBaseUrl?: string | null;
  sshTarget?: string | null;
  sshIdentity?: string | null;
  localPort?: string | number | null;
}): HomeSatelliteSshTunnel | null {
  const sshTarget = normalizeSshTarget(input.sshTarget);
  if (!sshTarget) return null;

  let parsed: URL;
  try {
    parsed = new URL(input.satelliteBaseUrl?.trim() ?? "");
  } catch {
    return null;
  }
