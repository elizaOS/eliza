...
const TRAVEL_PROVIDER_PATH_RE = /^\/api\/cloud\/travel-providers\/([a-z0-9][a-z0-9-]*)(\/.*)$/;
...
function parseTravelProviderPath(pathname: string): {
    provider: string;
    providerPath: string;
} | null {
...
export async function handleTravelProviderRelayRoute(req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, state: TravelProviderRelayRouteState): Promise<boolean> {
    const parsed = parseTravelProviderPath(pathname);
    if (!parsed)
        return false;
...
async function forward402(res: http.ServerResponse, upstream: Response): Promise<void> {
