...
import { type CloudChainType } from "../cloud/bridge-client.js";
import { type CloudWalletDescriptor } from "../cloud/bridge-client.js";
import { type CloudWalletProvider } from "../cloud/bridge-client.js";
...
type CloudWalletRollbackEnvKey = (typeof CLOUD_WALLET_ROLLBACK_ENV_KEYS)[number];
...
function replaceMutableRoot<T extends object>(target: T, snapshot: T): void {
    ...
    Object.assign(targetRecord, structuredClone(snapshot as Record<string, unknown>));
}
...
async function restoreConfigEnvRollbackSnapshot(snapshot: ConfigEnvRollbackSnapshot): Promise<void> {
...
function getTelemetrySpan(state: CloudRouteState, meta: {
    boundary: "cloud";
    operation: string;
    timeoutMs?: number;
...
export async function handleCloudRoute(req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, state: CloudRouteState): Promise<boolean> {
    if (method === "POST" && pathname === "/api/cloud/login") {
        ...
        const sessionId = typeof createData === "object" && createData !== null
            ? (createData as {
                sessionId?: unknown;
            }).sessionId
            : undefined;
    ...
    if (method === "GET" && pathname.startsWith("/api/cloud/login/status")) {
        ...
        if (data.status === "authenticated" && data.apiKey) {
            ...
            const cloud = (state.config.cloud ?? {}) as NonNullable<CloudConfigLike["cloud"]>;
            ...
            const cloudInferenceSelected = isCloudInferenceSelectedInConfig(state.config as Record<string, unknown>);
            migrateLegacyRuntimeConfig(state.config as Record<string, unknown>);
            try {
                if (state.saveConfig) {
                    state.saveConfig(state.config);
            ...
            if (isCloudWalletEnabled()) {
                const rollbackConfigSnapshot = structuredClone(state.config as Record<string, unknown>) as CloudConfigLike;
                ...
                try {
                    ...
                    persistCloudWalletCache(state.config as Record<string, unknown>, descriptors);
                    const cloudCfg = (state.config.cloud ?? {}) as Record<string, unknown>;
    ...
    if (method === "POST" && pathname === "/api/cloud/disconnect") {
        ...
        if (state.runtime) {
            ...
            const secrets = character.secrets as Record<string, string | number | boolean>;
