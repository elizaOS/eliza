...
import { clearCloudAuthService } from "../lib/cloud-connection";
import { clearCloudSecrets } from "../lib/cloud-secrets";
import { disconnectCloudConnection } from "../lib/cloud-connection";
import { getCloudAuth } from "../lib/cloud-connection";
import { handleCloudCodingContainerRoute } from "./cloud-coding-container-routes";
import { handleCloudRoute as handleAutonomousCloudRoute } from "./cloud-routes-autonomous.js";
...
import { type CloudRouteState as AutonomousCloudRouteState } from "./cloud-routes-autonomous.js";
import { type ElizaConfig } from "../lib/config-like";
import { type RuntimeCloudLike } from "../lib/cloud-connection";
...
function isCloudConnectionMutationRoute(pathname: string, method: string): boolean {
    return ((method === "POST" &&
...
async function readRouteJsonBody(req: http.IncomingMessage, res: http.ServerResponse): Promise<Record<string, unknown> | null> {
    const preParsed = (req as http.IncomingMessage & {
        body?: unknown;
    }).body;
...
function getTelemetrySpan(meta: {
    ...
    return (meta.services.createIntegrationTelemetrySpan(meta) ??
        createNoopTelemetrySpan());
}
async function fetchCloudLoginStatus(sessionId: string, baseUrl: string): Promise<Response> {
...
export async function resolveAppliedCloudKeyFingerprint(state: {
    ...
    const resolved = typeof runtime?.getSetting === "function"
...
export function applyCloudProxyTextRouting(config: Record<string, unknown>): void {
    const existingRouting = config.serviceRouting && typeof config.serviceRouting === "object"
...
async function persistCloudLoginStatus(args: {
    ...
    if (cloudLoginMutationBlockReason()) {
        logger.warn("[cloud-login] Ignoring credential persistence that differs from the development launcher Cloud profile");
        return null;
    }
    if (args.epochAtPollStart !== undefined &&
    ...
    const cloud = { ...(args.state.config.cloud ?? {}) } as Record<string, unknown>;
    cloud.apiKey = args.apiKey;
    const cloudInferenceSelected = args.forceInferenceEnabled === true ||
        isCloudInferenceSelectedInConfig(args.state.config as Record<string, unknown>);
    ...
    try {
        ...
        logger.warn("[cloud-login] Cloud API key is stored in cleartext in ~/.eliza/eliza.json. " +
    ...
    const relayService = (runtime?.getService("CLOUD_MANAGED_GATEWAY_RELAY") ??
        ...
        runtime?.getService("cloudManagedGatewayRelay")) as StartableCloudRelayService | null;
    ...
    const resolvedKey = readRuntimeSetting(runtime as AgentRuntime, "ELIZAOS_CLOUD_API_KEY");
    if (resolvedKey !== args.apiKey) {
...
function readRuntimeSetting(runtime: AgentRuntime | null, key: string): string | null {
...
function toAutonomousState(state: CloudRouteState, services: CloudRouteServices): AutonomousCloudRouteState {
    return {
        ...state,
...
export async function handleCloudRoute(req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, state: CloudRouteState): Promise<boolean> {
    ...
    if (method === "GET" && pathname === "/api/cloud/relay-status") {
        const relayService = (state.runtime?.getService("CLOUD_MANAGED_GATEWAY_RELAY") ??
            state.runtime?.getService("cloud-managed-gateway-relay") ??
            state.runtime?.getService("cloudManagedGatewayRelay")) as RelayStatusService | null;
        ...
        try {
            ...
            sendJson(res, {
                ...
                ssh: buildHomeRemoteRunnerSshTunnel({
                    remoteBaseUrl: readRuntimeSetting(state.runtime, "ELIZA_HOME_REMOTE_RUNNER_URL") ??
                    ...
                    sshTarget: readRuntimeSetting(state.runtime, "ELIZA_HOME_REMOTE_RUNNER_SSH_TARGET") ??
                    ...
                    sshIdentity: readRuntimeSetting(state.runtime, "ELIZA_HOME_REMOTE_RUNNER_SSH_IDENTITY") ??
    ...
    if (method === "GET" && pathname.startsWith("/api/cloud/login/status")) {
        ...
        if (data.status === "authenticated" && typeof data.apiKey === "string") {
            const resolvedApiKey = await persistCloudLoginStatus({
                apiKey: data.apiKey,
                organizationId: typeof data.organizationId === "string"
                    ? data.organizationId
                    : undefined,
                services,
                state,
