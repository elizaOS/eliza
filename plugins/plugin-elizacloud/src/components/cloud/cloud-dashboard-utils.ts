...
export const STATUS_BADGE: Record<string, {
    i18nKey: string;
    className: string;
}> = {
    running: {
...
export function resolveCloudAccountIdDisplay(userId: string | null, statusReason: string | null, t: (key: string) => string): {
    mono: boolean;
    text: string;
} {
    if (userId) {
...
export function unwrapBillingData<T extends Record<string, unknown>>(value: T): T {
...
export function buildManagedDiscordSettingsReturnUrl(rawUrl: string): string | null {
    let url: URL;
    try {
        url = new URL(rawUrl);
    }
...
export function isManagedDiscordGatewayAgent(agent: CloudCompatAgent): boolean {
    ...
    return (agent.agent_name.trim().toLowerCase() ===
...
export function normalizeBillingSummary(raw: CloudBillingSummary): CloudBillingSummary {
...
export function normalizeBillingSettings(raw: CloudBillingSettings): CloudBillingSettings {
    const source = unwrapBillingData(raw);
    return {
        ...raw,
        ...source,
...
export function getBillingAutoTopUp(settings: CloudBillingSettings | null): Record<string, unknown> {
...
export function getBillingLimits(settings: CloudBillingSettings | null): Record<string, unknown> {
    const rawSettings = isRecord(settings?.settings) ? settings.settings : null;
    return isRecord(rawSettings?.limits) ? rawSettings.limits : {};
}
export function resolveCheckoutUrl(response: CloudBillingCheckoutResponse): string | null {
...
export function buildAutoTopUpFormState(billingSummary: CloudBillingSummary | null, billingSettings: CloudBillingSettings | null): AutoTopUpFormState {
    const autoTopUp = getBillingAutoTopUp(billingSettings);
    const minimumTopUp = readNumber((billingSummary as Record<string, unknown> | null)?.minimumTopUp) ?? 1;
...
export function autoTopUpFormReducer(state: AutoTopUpFormState, action: AutoTopUpFormAction): AutoTopUpFormState {
    switch (action.type) {
        case "hydrate":
