...
import { handleCloudSttRoute } from "./lib/server-cloud-tts";
import { handleCloudTtsPreviewRoute } from "./lib/server-cloud-tts";
import { handleXRelayRoute } from "./routes/x-relay-routes";
...
function getHostContext(runtime: unknown) {
    return getRuntimeRouteHostContext<Record<string, unknown>>(runtime && typeof runtime === "object" ? runtime : null);
}
        ...
        const cloudState: CloudRouteState = {
            ...
            services: {
                ...
                saveElizaConfig: (nextConfig) => {
                    hostContext?.saveConfig?.(nextConfig as Record<string, unknown>);
                },
            },
        };
        const handled = await handleCloudRoute(httpReq, httpRes, url.pathname, method, cloudState);
        if (!handled) {
            return;
        }
    };
...
async function cloudTtsPreviewHandler(req: unknown, res: unknown): Promise<void> {
    await handleCloudTtsPreviewRoute(req as http.IncomingMessage, res as http.ServerResponse);
}
async function cloudSttHandler(req: unknown, res: unknown): Promise<void> {
    await handleCloudSttRoute(req as http.IncomingMessage, res as http.ServerResponse);
}
...
export const elizaCloudRoutePlugin: Plugin = {
    ...
    description: "Eliza Cloud connection, login, status, credit, and relay routes (extracted from app/server.ts)",
    routes: cloudRoutes,
