...
const PROVISION_TIMEOUT_MS = 120000; // 2 minutes
 ...
 */
export async function checkCloudAvailability(baseUrl: string): Promise<string | null> {
    try {
 ...
 */
async function runCloudAuth(observer: CloudSetupObserver, baseUrl: string): Promise<CloudLoginResult | null> {
    try {
...
function describeCloudAuthError(err: unknown): string {
    ...
    if (msg.includes("login timed out") ||
    ...
    if (msg.includes("invalid cloud base url") ||
 ...
 */
async function provisionCloudAgent(observer: CloudSetupObserver, client: ElizaCloudClient, agentName: string, preset?: StylePreset): Promise<ProvisionOutcome> {
    ...
    while (Date.now() < deadline) {
        ...
        try {
            ...
            if (classification === "transient") {
                ...
                logger.warn(`[cloud-setup] Transient poll error, will retry: ${String(pollErr)}`);
                current = null;
            }
            else {
 ...
 */
export async function runCloudSetup(observer: CloudSetupObserver, agentName: string, preset?: StylePreset, baseUrl?: string): Promise<CloudSetupResult | null> {
 ...
 */
async function finishProvisioning(observer: CloudSetupObserver, baseUrl: string, authResult: CloudLoginResult, agentName: string, preset?: StylePreset): Promise<CloudSetupResult | null> {
    ...
    const pendingAgentId = provisionResult?.kind === "pending-after-timeout"
        ? provisionResult.agentId
        : undefined;
    observer.onNotice(pendingAgentId
