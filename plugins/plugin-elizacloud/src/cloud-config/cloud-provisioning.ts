 ...
 * `@elizaos/plugin-elizacloud/cloud-config/cloud-provisioning` so that `@elizaos/agent` (and other host-layer code) can
...
function readProvisioningEnv(key: string, authority: DevCloudEnvAuthority | null): string | undefined {
    return authority
...
function hasCloudApiKeyProvisioning(authority: DevCloudEnvAuthority | null): boolean {
    return (readProvisioningEnv("ELIZAOS_CLOUD_ENABLED", authority) === "true" &&
...
export function isCloudProvisionedContainer(): boolean {
    ...
    const hasCloudFlag = readProvisioningEnv("ELIZA_CLOUD_PROVISIONED", authority) === "1";
    return (hasCloudFlag &&
