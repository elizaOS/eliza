...
function getContainerId(message: Memory, options?: Record<string, unknown>): string | null {
  ...
  const meta = (message.metadata as Record<string, unknown> | undefined)?.actionParams as Record<string, unknown> | undefined;
...
export const freezeCloudAgentAction: Action = {
  ...
  parameters: [
    { name: "containerId", description: "ID of the container to freeze", required: true, schema: { type: "string" } },
  ],
  ...
  async handler(
    ...
    if (container.status !== "running") {
      return { success: false, error: `Container not running (status: ${container.status})` };
    }

    const notify = async (text: string) => { if (callback) await callback({ text, actions: ["FREEZE_CLOUD_AGENT"] }); };
