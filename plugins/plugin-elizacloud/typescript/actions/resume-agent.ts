...
/** Settings keys forwarded into the container. */
const FORWARDED_SETTINGS = [
  "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "GROQ_API_KEY",
  "ELIZAOS_CLOUD_API_KEY", "SMALL_MODEL", "LARGE_MODEL",
  "ELIZAOS_CLOUD_SMALL_MODEL", "ELIZAOS_CLOUD_LARGE_MODEL",
];
...
export const resumeCloudAgentAction: Action = {
  ...
  parameters: [
    { name: "name", description: "Name for the restored agent", required: true, schema: { type: "string" } },
    { name: "project_name", description: "Project identifier (must match original)", required: true, schema: { type: "string" } },
    { name: "snapshotId", description: "Specific snapshot ID (defaults to latest)", required: false, schema: { type: "string" } },
  ...
  async handler(
    ...
    const notify = async (text: string) => { if (callback) await callback({ text, actions: ["RESUME_CLOUD_AGENT"] }); };
    ...
    if (backup) {
      ...
      if (explicit) {
        ...
        const latest = await findLatestProjectSnapshot(backup, containerSvc, params.project_name as string);
        if (latest) {
