...
export const checkCloudCreditsAction: Action = {
  ...
  tags: ["cloud", "billing"],
  parameters: [
    { name: "detailed", description: "Include transaction history", required: false, schema: { type: "boolean" } },
  ],
  ...
  async handler(
    ...
    const { data: { balance } } = await client.get<CreditBalanceResponse>("/credits/balance");

    const running = containerSvc?.getTrackedContainers().filter((c) => c.status === "running").length ?? 0;
    ...
    if (detailed) {
      ...
      lines.push(`Total spent: $${data.totalSpent.toFixed(2)} | Total added: $${data.totalAdded.toFixed(2)}`);
      for (const tx of data.recentTransactions.slice(0, 10)) {
        const sign = tx.amount >= 0 ? "+" : "";
        lines.push(`  ${sign}$${tx.amount.toFixed(2)} — ${tx.description} (${new Date(tx.created_at).toLocaleDateString()})`);
      }
    }
    ...
    return {
      success: true,
      text,
      data: { balance, runningContainers: running, dailyCost, estimatedDaysRemaining: daysRemaining },
    };
  },
};
