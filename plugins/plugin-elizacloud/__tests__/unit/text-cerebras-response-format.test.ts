...
type RuntimeFixture = Pick<IAgentRuntime, "character" | "emitEvent" | "getSetting"> & Partial<IAgentRuntime>;
...
async function captureBody(modelName: string, params: Record<string, unknown> = { responseSchema: RESPONSE_SCHEMA }): Promise<Record<string, unknown> | null> {
    ...
    await generateNativeChatCompletion(runtime(), "TEXT_SMALL", { prompt: "hi", ...params } as never, { modelName, prompt: "hi" });
    return captured;
}
