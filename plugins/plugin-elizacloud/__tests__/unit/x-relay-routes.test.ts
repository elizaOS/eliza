...
describe("handleXRelayRoute", () => {
    it("forwards GET X relay routes to the Cloud API with auth headers", async () => {
        ...
        await expect(handleXRelayRoute(req as never, res as never, pathnameOf(req), "GET", {
            config: {
                cloud: {
                    apiKey: "eliza_test",
        ...
        expect(fetchMock).toHaveBeenCalledWith("https://cloud.example/api/v1/x/users/me?expansions=pinned_tweet_id", expect.objectContaining({
            method: "GET",
            redirect: "manual",
    ...
    it("forwards a POST body read fresh from the request stream", async () => {
        ...
        req.end(payload);
        await expect(handleXRelayRoute(req as never, res as never, pathnameOf(req), "POST", {
            config: { cloud: { apiKey: "eliza_test", baseUrl: "https://cloud.example" } },
        })).resolves.toBe(true);
        expect(fetchMock).toHaveBeenCalledWith("https://cloud.example/api/v1/x/tweets", expect.objectContaining({ method: "POST", body: payload }));
    ...
    it("forwards a POST body already cached by the plugin route pre-parse", async () => {
        ...
        (req as unknown as Record<symbol, unknown>)[CACHED_REQUEST_BODY] = Buffer.from(payload, "utf-8");
        const res = createResponse();
    ...
    it("preserves 402 payment-required responses and challenge headers", async () => {
        process.env.NODE_ENV = "development";
        vi.stubGlobal("fetch", vi.fn<typeof fetch>(async () => new Response("payment required", {
            status: 402,
            headers: {
    ...
    it("rejects unsupported methods with 405 before contacting the Cloud API", async () => {
        ...
        const res = createResponse();
        await expect(handleXRelayRoute(req as never, res as never, pathnameOf(req), "DELETE", {
            config: { cloud: { apiKey: "eliza_test", baseUrl: "https://cloud.example" } },
        })).resolves.toBe(true);
        expect(fetchMock).not.toHaveBeenCalled();
