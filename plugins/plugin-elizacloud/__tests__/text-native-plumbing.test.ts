...
describe("Eliza Cloud guidance plumbing (unit)", () => {
  ...
  it("forwards grammar, response skeleton, and span sampler headers for eliza-1 native tool calls", async () => {
    const responseSkeleton = {
      id: "planner-guidance",
      spans: [
        { kind: "literal", text: '{"action":' },
        { kind: "enum", key: "action", values: ["PLAN_ACTIONS"] },
      ],
    };
    ...
    await handleActionPlanner(
      runtime({
        ...
        ELIZAOS_CLOUD_ACTION_PLANNER_MODEL: "eliza-1-planner-test",
      }),
      {
        prompt: "fallback prompt",
        system: "You are a planner.",
        messages: [{ role: "user", content: "Plan a no-op." }],
        tools: [
          {
            type: "function",
            function: {
    ...
    expect(body.grammar).toBe('root ::= "{\\"actions\\":[]}"');
    expect(body.response_skeleton).toEqual(responseSkeleton);
    expect(body.eliza_guidance).toEqual({
      grammar: 'root ::= "{\\"actions\\":[]}"',
      response_skeleton: responseSkeleton,
    });
    expect(chatCall?.headers.get("x-eliza-span-samplers")).toBe(
      JSON.stringify({
  ...
  it("keeps generic provider requests free of eliza guidance extensions", async () => {
    await handleActionPlanner(
      runtime({
        ELIZAOS_CLOUD_API_KEY: "test-key",
        ELIZAOS_CLOUD_BASE_URL: "https://cloud.test/api/v1",
        ELIZAOS_CLOUD_ACTION_PLANNER_MODEL: "openai/gpt-5-mini",
      }),
      {
        prompt: "fallback prompt",
        messages: [{ role: "user", content: "Plan a no-op." }],
        ...
        grammar: 'root ::= "{}"',
        responseSkeleton: { spans: [{ kind: "literal", text: "{}" }] },
        spanSamplerPlan: {
    ...
    expect(body.response_skeleton).toBeUndefined();
    expect(body.eliza_guidance).toBeUndefined();
    expect(chatCall?.headers.has("x-eliza-span-samplers")).toBe(false);
  });
});
    ...
    beforeEach(() => {
      ...
      vi.spyOn(globalThis, "fetch").mockImplementation(
        async (input: RequestInfo | URL, init?: RequestInit) => {
          ...
          captured.push({ url, method, headers: new Headers(init?.headers), body });
