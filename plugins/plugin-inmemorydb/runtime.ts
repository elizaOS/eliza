...
function connectorCredentialKey(params: { accountId: UUID; credentialType: string }): string {
  return `${String(params.accountId)}::${params.credentialType}`;
}

function oauthFlowKey(params: { agentId: UUID; provider: string; stateHash: string }): string {
  return `${String(params.agentId)}::${params.provider}::${params.stateHash}`;
}

function connectorDateToMillis(value: number | Date | null | undefined): number | null | undefined {
...
export class InMemoryDatabaseAdapter extends DatabaseAdapter<Record<string, never>> {
  ...
  private cloneComponent(component: Component): Component {
    return {
      ...component,
      data: component.data ? ({ ...component.data } as Metadata) : component.data,
    };
  }
  ...
  private getStoredComponentsForEntity(
    ...
    for (const componentId of componentIds) {
      ...
      if (options?.worldId !== undefined && component.worldId !== options.worldId) {
        continue;
      }
      if (
  ...
  private indexComponent(component: Component): void {
    ...
    const entityComponents = this.componentIdsByEntity.get(entityKey) ?? new Set<string>();
  ...
  async ensureEmbeddingDimension(dimension: number): Promise<void> {
    if (this.embeddingSpace !== undefined && this.embeddingDimension !== dimension) {
      throw new ElizaError(
  ...
  async ensureEmbeddingSpace(spaceId: string): Promise<UUID[]> {
    if (!spaceId.trim() || spaceId !== spaceId.trim())
      throw new ElizaError("Embedding representation requires a canonical identifier", {
        code: "EMBEDDING_SPACE_INVALID",
      });
    if (this.embeddingSpace !== undefined && this.embeddingSpace !== spaceId)
      throw new ElizaError("Restart the adapter before changing its embedding representation", {
        code: "EMBEDDING_SPACE_CHANGED",
      });
    ...
    for (const memory of this.memoriesById.values()) {
      ...
      if (memory.id && memory.content.text && !memory.embedding?.length) pending.push(memory.id);
    }
    return pending;
  }
  ...
  async queryEntities(_params: {
    ...
    if (hasComponentQuery) {
      for (const component of this.components.values()) {
        ...
        if (_params.entityIds?.length && !_params.entityIds.includes(component.entityId)) {
          continue;
        }
        if (_params.worldId !== undefined && component.worldId !== _params.worldId) {
          continue;
        }
        if (_params.componentType !== undefined && component.type !== _params.componentType) {
          continue;
        }
        if (
          _params.componentDataFilter !== undefined &&
    ...
    if (matchedComponentsByEntity.size > 0) {
      entityIds = _params.entityIds?.length
        ? _params.entityIds.filter((entityId) => matchedComponentsByEntity.has(String(entityId)))
      ...
      for (const entity of this.entities.values()) {
        ...
        if (_params.entityIds?.length && !_params.entityIds.includes(entity.id)) {
          continue;
        }
    ...
    entityIds = entityIds.filter((entityId) => {
      ...
      return entity !== undefined && (!_params.agentId || entity.agentId === _params.agentId);
    });
    ...
    for (const entityId of entityIds) {
      ...
      const components = _params.includeAllComponents
        ? this.getStoredComponentsForEntity(entity.id).filter(
            (component) => !_params.agentId || component.agentId === _params.agentId
          )
        : matchedComponents;
    ...
    for (const roomId of roomIds) {
      ...
      if (participantSet) {
        for (const entityIdStr of participantSet) {
          ...
          if (entity) {
            entities.push(_includeComponents ? this.attachComponents(entity) : { ...entity });
          }
        }
      }
  ...
  async getComponentsByNaturalKeys(
    ...
    return keys.map((key) => {
      ...
      const component = componentId ? this.components.get(componentId) : undefined;
  ...
  async compareAndSwapDocument(
    ...
    await this.updateMemories([{ ...params.replacement, id: params.documentId }]);
    const updated = this.memoriesById.get(String(params.documentId));
    return updated ? { status: "updated", document: updated } : { status: "conflict" };
  }
  ...
  async getMemories(params: {
    ...
    if (params.entityId) {
      const participantRoomIds =
        this.roomsByParticipant.get(String(params.entityId)) ?? new Set<string>();
      const agentOwnedDocument =
        params.tableName === "documents" || params.tableName === "document_fragments";
      all = all.filter(
        (memory) =>
    ...
    if (textContains) {
      all = all.filter((memory) => {
        ...
        return typeof text === "string" && text.toLowerCase().includes(textContains);
      });
    }
  ...
  async getCachedEmbeddings(): Promise<{ embedding: number[]; levenshtein_score: number }[]> {
    return [];
  }
  ...
  async updateMemoryEmbedding(
    ...
    await this.updateMemories([{ id: update.id, embedding: [...update.embedding] }]);
    return true;
  }
  ...
  async countMemories(params: {
    ...
    if (roomIds.length === 0) {
      ...
      for (const [key, memories] of this.memoriesByRoom) {
        ...
        if (filterMeta) list = list.filter((memory) => memoryMatchesMetadata(memory, filterMeta));
  ...
  async compareAndSwapWorldMetadata(
    ...
    if (!worldMetadataValueEquals(existing.metadata ?? {}, params.expectedMetadata)) {
      return { status: "conflict" };
    }
    const storedRevision = getWorldMetadataRevision(existing.metadata as Metadata | undefined);
  ...
  async createRoomParticipants(entityIds: UUID[], roomId: UUID): Promise<UUID[]> {
    ...
    const participants = this.participantsByRoom.get(roomKey) ?? new Set<string>();
    const ids: UUID[] = [];
  ...
  async getRelationshipsByPairs(
    ...
    return pairs.map((pair) => {
      const relationship = Array.from(this.relationships.values()).find(
        (item) =>
          item.sourceEntityId === pair.sourceEntityId && item.targetEntityId === pair.targetEntityId
      );
  ...
  async getRelationships(params: {
    ...
    const filtered = Array.from(this.relationships.values()).filter((relationship) => {
      const matchesEntity =
        entitySet.has(relationship.sourceEntityId) || entitySet.has(relationship.targetEntityId);
      if (!matchesEntity) {
        return false;
      }
  ...
  async createRelationships(
    ...
    return relationships.map((relationship) => {
      ...
      this.relationships.set(id, {
        ...
        metadata: relationship.metadata ? ({ ...relationship.metadata } as Metadata) : {},
  ...
  async updateRelationships(relationships: Relationship[]): Promise<void> {
    for (const relationship of relationships) {
      ...
      this.relationships.set(String(relationship.id), {
        ...
        tags: relationship.tags ? [...relationship.tags] : [],
        metadata: relationship.metadata ? ({ ...relationship.metadata } as Metadata) : {},
        createdAt: relationship.createdAt ?? existing?.createdAt ?? new Date().toISOString(),
      });
    }
  }
    ...
    for (const query of queries) {
      ...
      for (const request of this.pairingRequests.values()) {
        if (
          ...
          (!query.createdAfter ||
            new Date(request.createdAt).getTime() >= query.createdAfter.getTime())
        ) {
  ...
  async getConnectorAccount(
    ...
    if (params.id) {
      ...
      account = accountId ? this.connectorAccountsById.get(accountId) : undefined;
    }
  ...
  async upsertConnectorAccount(
    ...
    if (existingByRequestedId?.deletedAt != null) {
      throw new Error("Connector account id already belongs to a deleted account");
    }

    let existingId: string | undefined;
    if (existingByRequestedId) {
      if (existingByAccountKeyId && existingByAccountKeyId !== existingByRequestedId.id) {
        throw new Error("Connector account id and account key resolve to different accounts");
      }
      if (existingByExternalRole && existingByExternalRole.id !== existingByRequestedId.id) {
        throw new Error("Connector account id and external identity resolve to different accounts");
      }
      existingId = existingByRequestedId.id;
    } else if (params.externalId != null) {
      if (existingByAccountKeyId && existingByAccountKeyId !== existingByExternalRole?.id) {
        throw new Error(
    ...
    const existing = existingId ? this.connectorAccountsById.get(existingId) : undefined;
    const now = Date.now();
    ...
    const record: ConnectorAccountRecord = {
      ...
      provider: params.provider,
      accountKey: params.accountKey,
      externalId: params.externalId !== undefined ? params.externalId : existing?.externalId,
      ...
      ownerBindingId:
        params.ownerBindingId !== undefined ? params.ownerBindingId : existing?.ownerBindingId,
      ownerIdentityId:
        params.ownerIdentityId !== undefined ? params.ownerIdentityId : existing?.ownerIdentityId,
      role: params.role ?? existing?.role ?? "OWNER",
      purpose: params.purpose ? [...params.purpose] : [...(existing?.purpose ?? ["messaging"])],
  ...
  async setConnectorAccountCredentialRef(
    ...
    const record: ConnectorAccountCredentialRefRecord = {
      ...
      lastVerifiedAt: lastVerifiedAt !== undefined ? lastVerifiedAt : existing?.lastVerifiedAt,
      createdAt: existing?.createdAt ?? now,
      updatedAt: now,
    };
  ...
  async getConnectorAccountCredentialRef(
    ...
    const credential = this.connectorCredentialRefs.get(connectorCredentialKey(params));
    return credential
      ? {
          ...credential,
  ...
  async appendConnectorAccountAuditEvent(
    ...
    const record: ConnectorAccountAuditEventRecord = {
      ...
      outcome: params.outcome ?? ("success" satisfies ConnectorAccountAuditOutcome),
  ...
  async createOAuthFlowState(params: CreateOAuthFlowStateParams): Promise<OAuthFlowRecord> {
    ...
    const now = Date.now();
    const expiresAt =
      connectorDateToMillis(params.expiresAt) ?? now + (params.ttlMs ?? 10 * 60_000);
  ...
  private async findOAuthFlowState(
    ...
    const now = connectorDateToMillis((params as GetOAuthFlowStateParams).now) ?? Date.now();
  ...
  async updateOAuthFlowState(params: UpdateOAuthFlowStateParams): Promise<OAuthFlowRecord | null> {
    ...
    const record: OAuthFlowRecord = {
      ...
      redirectUri: params.redirectUri !== undefined ? params.redirectUri : existing.redirectUri,
      codeVerifierRef:
        params.codeVerifierRef !== undefined ? params.codeVerifierRef : existing.codeVerifierRef,
      scopes: params.scopes ? [...params.scopes] : [...existing.scopes],
      metadata: {
        ...cloneConnectorJsonObject(existing.metadata),
      ...
      consumedAt: params.consumedAt !== undefined ? consumedAt : existing.consumedAt,
      consumedBy: params.consumedBy !== undefined ? params.consumedBy : existing.consumedBy,
    };
    this.oauthFlowsByStateHash.set(
      oauthFlowKey({
        agentId: record.agentId,
