...
- **Authorization serialization.** Membership evidence updates, room/account reassignment and world deletion, entity/participant mutation, and document reads, revisions, deletes, and direct-grant CAS operations share one storage-global `IStorage.runExclusive` key. The lock belongs to shared storage rather than one adapter or agent, so globally keyed room/entity ownership, membership publication, revocation, metadata writes, and deletion cascades cannot race across tenant adapters or restore stale evidence. Room/entity writes reject caller-agent mismatches and foreign-owned persisted IDs. Direct grants validate current-agent entities and the same OWNER/current-room-ADMIN policy as SQL.
- **Membership evidence parity.** Participant presence alone grants nothing. The adapter stores agent-scoped, source- and account-bound, generation-fenced observations and returns only fresh expiring `member` evidence as current, matching the persistent adapters without fabricating legacy membership. Every document operation intersects caller-supplied room ids with that authoritative current set before visibility or mutation checks.
