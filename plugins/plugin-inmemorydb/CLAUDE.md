...
bun run --cwd plugins/plugin-inmemorydb clean       # rm -rf dist .turbo *.tsbuildinfo
```
...
- **Default embedding dimension is 384.** Call `adapter.ensureEmbeddingDimension(n)` before writing memories with a different embedding size; it re-initializes the HNSW index (clearing existing vectors).
