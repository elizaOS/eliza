...
import type { Memory, PairingAllowlistEntry, PairingRequest, Task, UUID } from "@elizaos/core";
...
describe("InMemoryDatabaseAdapter secure record identities", () => {
  ...
  it("preserves caller-supplied stable IDs without consulting a CSPRNG", async () => {
    ...
    const taskIds = await adapter.createTasks([{ id: taskId, name: "stable task" } satisfies Task]);
