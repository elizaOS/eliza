/** Verifies the isolated in-memory adapter's portable task filter and pagination contract. */
...
describe("isolated in-memory task queries", () => {
  it("filters authority and returns stable created-at/id pages", async () => {
    ...
    await expect(adapter.getTasks({ ...query, limit: 0 })).resolves.toEqual([]);
    await expect(adapter.getTasks(query)).resolves.toHaveLength(3);
