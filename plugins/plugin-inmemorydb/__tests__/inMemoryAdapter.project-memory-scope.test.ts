...
import type { Memory, Room, UUID, World } from "@elizaos/core";
import {
...
async function setup() {
  ...
  const writeInProject = async (roomId: UUID, projectId: string, text: string) => {
    const base: Memory = {
...
describe("per-project memory scoping — real InMemoryDatabaseAdapter roundtrip", () => {
  ...
  it("(b): a read scoped to project A never returns project B's memories", async () => {
    const { adapter, worldA, worldB, roomA, roomB, writeInProject } = await setup();
