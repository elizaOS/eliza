...
async function seed(messages: Memory[]): Promise<InMemoryDatabaseAdapter> {
  ...
  await adapter.createMemories(messages.map((memory) => ({ memory, tableName: "messages" })));
  return adapter;
}
...
describe("InMemoryDatabaseAdapter — textContains", () => {
  it("uses entityId as participant-room context while count retains its row predicate", async () => {
    ...
    expect(offsetPast.map((memory) => memory.content.text)).toEqual(["requester-authored"]);
  ...
  it("matches the keyword literally — `%`/`_` are not wildcards", async () => {
    const adapter = await seed([msg("discount is 50% off", 1), msg("50x faster now", 2)]);
    const hits = await adapter.getMemories({
      roomId,
      tableName: "messages",
      textContains: "50%",
    });
    expect(hits.map((m) => (m.content as { text: string }).text)).toEqual(["discount is 50% off"]);
  });
  ...
  it("honors orderDirection for around-message paging (asc = oldest first)", async () => {
    ...
    expect(desc.map((m) => (m.content as { text: string }).text)).toEqual(["c", "b", "a"]);
    expect(asc.map((m) => (m.content as { text: string }).text)).toEqual(["a", "b", "c"]);
  });
  ...
  it("uses an exclusive tuple cursor that stays stable across earlier mutations", async () => {
    ...
    const adapter = await seed(ids.map((id, index) => msg(`m${index}`, 1_000, id)));
    ...
    await adapter.createMemories([
      {
        memory: msg("inserted ahead", 2_000, "00000000-0000-0000-0000-0000000000ff"),
        tableName: "messages",
      },
    ]);
  ...
  it("matches PostgreSQL UUID ordering across hexadecimal case at cursor boundaries", async () => {
    ...
    const adapter = await seed(ids.map((id, index) => msg(`case ${index}`, 1_000, id)));
  ...
  it("stays bounded: a keyword scan over many messages returns only matches, quickly", async () => {
    ...
    expect(hits.every((m) => (m.content as { text: string }).text.includes("needle"))).toBe(true);
