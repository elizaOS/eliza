...
import type { Agent, Component, Entity, Room, Task, UUID } from "@elizaos/core";
...
describe("InMemoryDatabaseAdapter", () => {
  ...
  it("handles agent batch creation, ordered lookup, replacement, and missing deletion", async () => {
    ...
    await expect(
      adapter.createAgents([{ id: AGENT_ID, name: "first", username: "first-user" }, withoutId])
    ...
    const ordered = await adapter.getAgentsByIds([OTHER_AGENT_ID, MISSING_ID, AGENT_ID]);
    expect(ordered.map((agent) => agent.id)).toEqual([OTHER_AGENT_ID, AGENT_ID]);
    ...
    await expect(adapter.deleteAgents([MISSING_ID, OTHER_AGENT_ID])).resolves.toBe(true);
    await expect(adapter.getAgents()).resolves.toEqual([{ id: AGENT_ID, name: "replacement" }]);
  });
  ...
  it("serializes cache values and safely ignores missing keys during reads and deletion", async () => {
    ...
    const values = await adapter.getCaches<typeof source>(["missing", "second", "first"]);
    ...
    await expect(adapter.deleteCaches(["missing", "first"])).resolves.toBe(true);
  ...
  it("maintains component indexes across updates and returns defensive copies", async () => {
    ...
    await expect(adapter.createComponents([original])).resolves.toEqual([COMPONENT_ID]);
    ...
    await expect(adapter.getComponentsByIds([COMPONENT_ID])).resolves.toMatchObject([
      { data: { label: "original" } },
    ]);
    ...
    await expect(adapter.getComponentsForEntities([ENTITY_ID])).resolves.toEqual([]);
  ...
  it("cascades entity deletion through components and room participation", async () => {
    ...
    await expect(adapter.getComponentsForEntities([ENTITY_ID, OTHER_ENTITY_ID])).resolves.toEqual(
      []
    );
    await expect(
    ...
    await expect(
      adapter.getParticipantUserStates([{ roomId: ROOM_ID, entityId: ENTITY_ID }])
  ...
  it("updates only queued pending tasks and preserves their stored identity", async () => {
    ...
    await expect(adapter.updatePendingTask(MISSING_ID, { name: "missing" })).resolves.toBe(false);
    await expect(adapter.updatePendingTask(OTHER_TASK_ID, { name: "not pending" })).resolves.toBe(
      false
    );
    await expect(adapter.updatePendingTask(notQueuedId, { name: "not queued" })).resolves.toBe(
      false
    );
    await expect(
  ...
  it("keeps participant indexes in sync and treats removal of a missing pair as success", async () => {
    ...
    ).resolves.toEqual([`${ROOM_ID}:${ENTITY_ID}`, `${ROOM_ID}:${OTHER_ENTITY_ID}`]);
    await expect(adapter.getParticipantsForRooms([ROOM_ID, OTHER_ROOM_ID])).resolves.toEqual([
    ...
    await expect(adapter.getRoomsForParticipants([OTHER_ENTITY_ID, ENTITY_ID])).resolves.toEqual([
      ROOM_ID,
    ]);
    await expect(
