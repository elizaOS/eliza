...
describe("InMemoryDatabaseAdapter pairing pagination", () => {
  it("keeps legacy ordering and returns deterministic bounded request pages", async () => {
    ...
    await adapter.createPairingRequests([request(1, 1_000), request(3, 2_000), request(2, 2_000)]);

    const [legacy] = await adapter.getPairingRequests([{ channel: "telegram", agentId: AGENT_ID }]);
    expect(legacy.requests.map((item) => item.id)).toEqual([id(1), id(3), id(2)]);
  ...
  it("bounds allowlist pages and rejects invalid direct adapter options", async () => {
    ...
    expect(legacy.entries.map((item) => item.id)).toEqual([id(4), id(6), id(5)]);
    ...
    await expect(
      adapter.getPairingAllowlists([{ channel: "telegram", agentId: AGENT_ID, limit: 101 }])
