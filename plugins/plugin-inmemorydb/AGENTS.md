...
- **Authorization serialization.** Membership evidence updates and document reads, revisions, deletes, and direct-grant CAS operations share one adapter lock, so a completed revocation cannot race a later document operation. Direct grants validate current-agent entities and the same OWNER/current-room-ADMIN policy as SQL.
- **Membership evidence parity.** Participant presence alone grants nothing. The adapter stores generation-fenced observations and returns only fresh `member` or `runtime:local` evidence as current, matching the persistent adapters without fabricating legacy membership. Every document operation intersects caller-supplied room ids with that authoritative current set before visibility or mutation checks.
