...
import { CHAIN_CONFIGS, PRIMARY_CHAIN_KEYS, resolveChainKey, } from "./chainConfig";
import { isBscChainName } from "./constants";
import { computeSingleChainFocus, matchesInventoryChainFilter, } from "./inventory-chain-filters";
function hasVisibleBalance(row) {
    return row.balanceRaw > 0 || row.valueUsd > 0;
}
function matchesSingleChainFocus(chainName, focus) {
...
function buildTokenRowsAllChains({ walletBalances, walletAddresses, walletConfig, }) {
    const rows = [];
...
export function useInventoryData({ walletBalances, walletAddresses, walletConfig, walletNfts, inventorySort, inventorySortDirection, inventoryChainFilters, }) {
    const singleChainFocus = useMemo(() => computeSingleChainFocus(inventoryChainFilters), [inventoryChainFilters]);
    const knownEvmAddr = walletAddresses?.evmAddress ?? walletConfig?.evmAddress;
    const _knownSolAddr = walletAddresses?.solanaAddress ?? walletConfig?.solanaAddress;
    const primaryChain = useMemo(() => {
        if (!walletBalances?.evm?.chains)
            return null;
        return (walletBalances.evm.chains.find((c) => isBscChainName(c.chain)) ?? null);
    }, [walletBalances]);
    ...
    const tokenRowsAllChains = useMemo(() => buildTokenRowsAllChains({
        walletBalances,
        walletAddresses,
        walletConfig,
    }), [walletBalances, walletAddresses, walletConfig]);
    const tokenRows = useMemo(() => tokenRowsAllChains.filter((row) => matchesInventoryChainFilter(row.chain, inventoryChainFilters)), [tokenRowsAllChains, inventoryChainFilters]);
    ...
    const chainErrors = useMemo(() => (walletBalances?.evm?.chains ?? []).filter((c) => c.error), [walletBalances]);
    const focusChainHasError = useMemo(() => {
        return chainErrors.some((c) => matchesInventoryChainFilter(c.chain, inventoryChainFilters));
    ...
    const allNfts = useMemo(() => {
        ...
        const filtered = items.filter((nft) => matchesInventoryChainFilter(nft.chain, inventoryChainFilters));
    ...
    const focusedChain = useMemo(() => {
        ...
        const chainConfig = CHAIN_CONFIGS[singleChainFocus];
        const evmChain = walletBalances?.evm?.chains.find((chain) => matchesSingleChainFocus(chain.chain, singleChainFocus)) ?? null;
        if (!chainConfig && !evmChain)
            return null;
        return {
    ...
    const primaryChainError = primaryChain?.error ??
        chainErrors.find((chain) => isBscChainName(chain.chain))?.error ??
        null;
    const primaryNativeBalance = primaryChain?.nativeBalance ?? null;
    const focusedRows = sortedRows;
    const visibleRows = sortedRows;
    const totalUsd = useMemo(() => tokenRows.reduce((sum, r) => sum + r.valueUsd, 0), [tokenRows]);
    const visibleChainErrors = useMemo(() => {
        return chainErrors.filter((chain) => matchesInventoryChainFilter(chain.chain, inventoryChainFilters));
    }, [chainErrors, inventoryChainFilters]);
    return {
        singleChainFocus,
        tokenRows,
