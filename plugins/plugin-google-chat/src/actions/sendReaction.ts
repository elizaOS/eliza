...
export const sendReaction: Action = {
  ...
  validate: async (_runtime: IAgentRuntime, message: Memory, _state?: State): Promise<boolean> => {
    ...
    const sourceOk = message.content.source === "google-chat";
    const keywordOk =
      text.includes("react") ||
      ...
      sourceOk;
    const regexOk =
      /\b(?:react|reaction|emoji|thumbs?\s?up|thumbs?\s?down|remove)\b/i.test(textRaw) || sourceOk;
    const inputOk = textRaw.trim().length > 0 || sourceOk;
  ...
  handler: async (
    ...
    const messageData = currentState.data?.message as Record<string, unknown> | undefined;
