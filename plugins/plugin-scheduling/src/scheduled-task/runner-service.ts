...
import { TASK_EXECUTION_PROFILES } from "./types.js";
import { createAnchorRegistry } from "./consolidation-policy.js";
import { createCodingAgentScheduleDispatcher } from "../coding-agent-schedules.js";
import { createCompletionCheckRegistry } from "./completion-check-registry.js";
import { createConsolidationRegistry } from "./consolidation-policy.js";
...
import { createInMemoryScheduledTaskStore } from "./runner.js";
import { createScheduledTaskRunner } from "./runner.js";
import { createSchedulingRecordStores } from "./record-store.js";
import { createSchedulingSqlScheduledTaskLogStore } from "./store.js";
...
import { ensureSchedulingRecordSchema } from "./record-store.js";
import { getRuntimeDb } from "./sql.js";
import { getScheduledTaskChannelDispatcher } from "./channel-dispatcher-registry.js";
import { getSchedulingRecordStore } from "./record-store.js";
import { isConnectorDispatchIntent } from "./connector-dispatch.js";
import { listScheduledTaskChannelDispatcherKeys } from "./channel-dispatcher-registry.js";
import { logger } from "@elizaos/core";
import { migrateSchedulingTables } from "./migration.js";
import { registerBuiltInCompletionChecks } from "./completion-check-registry.js";
import { registerBuiltInGates } from "./gate-registry.js";
import { registerDefaultEscalationLadders } from "./escalation.js";
import { registerFallbackAnchors } from "./consolidation-policy.js";
import { renderFailureDispatchResult } from "./dispatch-render.js";
import { renderScheduledDispatchMessage } from "./dispatch-render.js";
import { renderScheduledDispatchTitle } from "./dispatch-render.js";
import { resolveConnectorDispatchTarget } from "./connector-dispatch.js";
import { resolvePlatform } from "@elizaos/core/runtime-env";
import { runtimeHasMessageConnector } from "./connector-dispatch.js";
import { type ActivitySignalBusView } from "./types.js";
import { type AnchorRegistry } from "./consolidation-policy.js";
import { type CompletionCheckRegistry } from "./completion-check-registry.js";
import { type ConsolidationRegistry } from "./consolidation-policy.js";
import { type DispatchResult } from "../dispatch-types.js";
import { type EscalationLadderRegistry } from "./escalation.js";
import { type GlobalPauseView } from "./types.js";
...
import { type ScheduledTaskRunnerDeps } from "./runner.js";
import { type ScheduledTaskRunnerHandle } from "./runner.js";
import { type ScheduledTaskStore } from "./runner.js";
import { type SubjectStoreView } from "./types.js";
import { type TaskExecutionProfile } from "./types.js";
 ...
 */
export type ScheduledTaskRunnerDepsProvider = (runtime: IAgentRuntime, agentId: string) => ScheduledTaskRunnerDepsBundle;
const depsProvidersByRuntime = new WeakMap<IAgentRuntime, ScheduledTaskRunnerDepsProvider>();
/**
 ...
 * anchor registry.
 */
export function registerScheduledTaskRunnerDeps(runtime: IAgentRuntime, provider: ScheduledTaskRunnerDepsProvider): void {
    if (depsProvidersByRuntime.has(runtime)) {
        logger.debug({ src: SERVICE_TYPE, agentId: runtime.agentId }, "ScheduledTask runner deps provider already registered; keeping first-wins registration.");
        return;
    }
    depsProvidersByRuntime.set(runtime, provider);
}
export function getScheduledTaskRunnerDeps(runtime: IAgentRuntime): ScheduledTaskRunnerDepsProvider | null {
 ...
 */
export type ScheduledTaskRunnerBootHook = (service: ScheduledTaskRunnerService) => Promise<void> | void;
const bootHooksByRuntime = new WeakMap<IAgentRuntime, ScheduledTaskRunnerBootHook[]>();
const startedServiceByRuntime = new WeakMap<IAgentRuntime, ScheduledTaskRunnerService>();
function runBootHook(runtime: IAgentRuntime, service: ScheduledTaskRunnerService, hook: ScheduledTaskRunnerBootHook): Promise<void> {
 ...
 */
export function registerScheduledTaskRunnerBootHook(runtime: IAgentRuntime, hook: ScheduledTaskRunnerBootHook): void {
...
const ALL_PROFILES: ReadonlySet<TaskExecutionProfile> = new Set(TASK_EXECUTION_PROFILES);
const MOBILE_PROFILES: ReadonlySet<TaskExecutionProfile> = new Set<TaskExecutionProfile>(["foreground", "bg-light-30s", "notify-only"]);
/**
 ...
 */
function createDefaultScheduledTaskDispatcher(runtime: IAgentRuntime, store: ScheduledTaskStore): ScheduledTaskDispatcher {
    return {
        async dispatch(record): Promise<DispatchResult> {
            ...
            try {
                ...
                runtime.reportError("scheduling:scheduled-task:dispatch-render", error, {
            ...
            if (connectorTarget) {
                ...
                const dispatchIdempotencyKey = typeof existingKey === "string" && existingKey.trim().length > 0
                    ? existingKey.trim()
            ...
            const connectorResult = await dispatchViaMessageConnector(runtime, record, body);
            if (connectorResult)
            ...
            try {
                ...
                runtime.reportError("scheduling:scheduled-task:dispatch-render", error, { taskId: record.taskId, channelKey: record.channelKey });
                return renderFailureDispatchResult(error);
            }
            ...
            try {
                ...
                runtime.reportError("scheduling:scheduled-task:notification-dispatch", error, { taskId: record.taskId, channelKey: record.channelKey });
                return {
                    ok: false,
                    reason: "transport_error",
                    acceptance: "unknown",
...
function makeMissingActivityBusView(runtime: IAgentRuntime): ActivitySignalBusView {
    let warned = false;
    return {
        hasSignalSince() {
            if (!warned) {
                warned = true;
                logger.warn({ src: SERVICE_TYPE, agentId: runtime.agentId }, "ActivitySignalBus not registered; activity-dependent completion-checks report no-signal. A consumer can inject one via registerScheduledTaskRunnerDeps.");
            }
            return false;
        },
    };
...
function makeMissingSubjectStoreView(runtime: IAgentRuntime): SubjectStoreView {
    ...
    return {
        wasUpdatedSince() {
            if (!warned) {
                warned = true;
                logger.warn({ src: SERVICE_TYPE, agentId: runtime.agentId }, "SubjectStore not registered; subject_updated completion-checks report no-update. A consumer can inject one via registerScheduledTaskRunnerDeps.");
            }
            return false;
        },
    };
 ...
 */
function defaultRunnerDeps(runtime: IAgentRuntime, agentId: string): ScheduledTaskRunnerDepsBundle {
...
function buildRunner(runtime: IAgentRuntime, opts: GetScheduledTaskRunnerOptions): ScheduledTaskRunnerHandle {
    ...
    const completionChecks = deps.completionChecks ?? createCompletionCheckRegistry();
    ...
    const dispatcher: ScheduledTaskDispatcher = {
        dispatch(record) {
            const contributed = getScheduledTaskChannelDispatcher(runtime, record.channelKey);
            if (contributed)
                return contributed.dispatch(record);
    ...
    const channelAvailable = hostChannelAvailable
        ? (channelKey: string) => channelKey === PR_SHEPHERD_DISPATCH_CHANNEL ||
...
export class ScheduledTaskRunnerService extends Service {
    ...
    override capabilityDescription = "Long-lived ScheduledTask runner host. Builds the runner from the runtime-injected deps provider (or the built-in default deps) once per agent and caches it with a rebindable clock; the scheduler tick reads the cached runner instead of reconstructing it every minute.";
    ...
    static override async start(runtime: IAgentRuntime): Promise<ScheduledTaskRunnerService> {
        ...
        if (recordStore)
            await ensureSchedulingRecordSchema(recordStore);
        const runtimeDb = getRuntimeDb(runtime);
        if (!recordStore && runtimeDb) {
            await migrateSchedulingTables(await createDrizzleCarveOutDatabase(runtimeDb));
        }
        logger.debug({ src: SERVICE_TYPE, agentId: runtime.agentId }, "ScheduledTaskRunnerService started");
    ...
    getRunner(opts: GetScheduledTaskRunnerOptions): ScheduledTaskRunnerHandle {
        ...
        if (!entry) {
            const runtime = this.runtime;
            if (!runtime) {
                throw new Error("ScheduledTaskRunnerService: runtime is not bound; was the service started?");
            }
 ...
 */
export function getScheduledTaskRunner(runtime: IAgentRuntime, opts: GetScheduledTaskRunnerOptions): ScheduledTaskRunnerHandle {
    const service = runtime.getService(SERVICE_TYPE) as ScheduledTaskRunnerService | null | undefined;
    if (!service) {
        throw new ElizaError(`[${SERVICE_TYPE}] ScheduledTaskRunnerService is not registered on this runtime. Add @elizaos/plugin-scheduling to the agent's plugin list.`, {
