 ...
 */
import { SCHEDULED_TASKS_ROUTE_PATHS } from "./scheduled-tasks.js";
...
import { type SchedulingRouteContext } from "./scheduled-tasks.js";
...
function buildContext(req: IncomingMessage, res: ServerResponse): SchedulingRouteContext {
    ...
    return {
        ...
        json: sendJson,
        error: sendJsonError,
        readJsonBody: <T extends object>(r: IncomingMessage, s: ServerResponse) => httpReadJsonBody<T>(r, s),
    };
}
function scheduledTasksLegacyHandler(): LegacyRouteHandler {
    ...
    return async (req, res, runtime): Promise<void> => {
        if (!isIncomingMessage(req) || !isServerResponse(res)) {
            throw new TypeError("Scheduled-tasks legacy route requires Node HTTP request/response objects");
        }
