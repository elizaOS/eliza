...
import { disposeStandaloneTick } from "./scheduled-task/standalone-tick.js";
import { ensureStandaloneTickTask } from "./scheduled-task/standalone-tick.js";
import { getDefaultTaskPacks } from "./scheduled-task/seed-registry.js";
import { getScheduledTaskRunnerDeps } from "./scheduled-task/runner-service.js";
import { logger } from "@elizaos/core";
import { registerDefaultTaskPack } from "./scheduled-task/seed-registry.js";
import { registerScheduledTaskRunnerBootHook } from "./scheduled-task/runner-service.js";
import { registerStandaloneTickWorker } from "./scheduled-task/standalone-tick.js";
...
export const SCHEDULED_TASK_RUNNER_REGISTRATION_TIMEOUT = "SCHEDULED_TASK_RUNNER_REGISTRATION_TIMEOUT";
export const SCHEDULED_TASK_RUNNER_REGISTRATION_FAILED = "SCHEDULED_TASK_RUNNER_REGISTRATION_FAILED";
export const SCHEDULED_TASK_RUNNER_WAIT_STOPPED = "SCHEDULED_TASK_RUNNER_WAIT_STOPPED";
...
function runnerWaitStopped(runtime: IAgentRuntime, signal?: AbortSignal): boolean {
    const lifecycle = typeof runtime.getLifecycleState === "function"
...
function runnerWaitSignal(runtime: IAgentRuntime, ownerSignal?: AbortSignal): AbortSignal | undefined {
    const runtimeSignal = typeof runtime.getStopSignal === "function"
...
function requireDuration(value: number | undefined, fallback: number, name: string, allowZero: boolean): number {
 ...
 */
export async function waitForScheduledTaskRunnerService(runtime: IAgentRuntime, options: WaitForScheduledTaskRunnerServiceOptions = {}): Promise<ScheduledTaskRunnerService> {
    const timeoutMs = requireDuration(options.registrationTimeoutMs, DEFAULT_RUNNER_REGISTRATION_TIMEOUT_MS, "registrationTimeoutMs", true);
    const pollMs = requireDuration(options.registrationPollMs, DEFAULT_RUNNER_REGISTRATION_POLL_MS, "registrationPollMs", false);
    ...
    while (!runtime.hasService(serviceType)) {
        ...
        if (remainingMs <= 0) {
            throw new ElizaError("Scheduled task runner was not registered before the startup deadline", {
    ...
    return (await waitForPromise(runtime.getServiceLoadPromise(serviceType), signal)) as ScheduledTaskRunnerService;
}
export const schedulingPlugin: Plugin = {
    name: "@elizaos/plugin-scheduling",
    description: "Scheduling spine: the always-loaded ScheduledTask runtime primitive — runner host, REST surface, durable store, and default-pack seed registry. Owner/channel deps are injected by a host plugin; built-in defaults run when no host is present.",
    ...
    views: [
        {
            ...
            description: "Connect your model and accounts, then run a real LifeOps validation and watch it fire.",
            icon: "FlaskConical",
        ...
        registerScheduledTaskRunnerBootHook(runtime, async (service) => {
            try {
                ...
                if (!hasConsumerHost && !alreadyRegistered) {
                    registerDefaultTaskPack(runtime, buildFallbackDefaultPack({ agentId: runtime.agentId }));
                }
                ...
                logger.warn({ src: "scheduling:boot-seed", agentId: runtime.agentId, error }, "[scheduling] Default-pack boot seed failed; tasks can still be scheduled at runtime.");
            }
        });
    },
    dispose: async (runtime: IAgentRuntime) => {
        try {
            ...
            logger.warn({ src: "scheduling:dispose", agentId: runtime.agentId, error }, "[scheduling] Failed to remove the standalone tick task during teardown.");
        }
    },
};
