...
describe("conversationHistoryProvider", () => {
  ...
  describe("get", () => {
    it("should return conversation history when chat id is in content", async () => {
      ...
      mockBlooioService.getConversationHistory = vi
        .fn()
      ...
      expect(mockBlooioService.getConversationHistory).toHaveBeenCalledWith(
        "+18885551234",
        10,
      );
      expect(result.text).toContain(
        "Recent Blooio conversation with +18885551234:",
      );
    ...
    it("should extract chat id from text if not in chatId field", async () => {
      ...
      expect(result.text).toBe(
        "No recent conversation history with jane@example.com",
      );
    });
