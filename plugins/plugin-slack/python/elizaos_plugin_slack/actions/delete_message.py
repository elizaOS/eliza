...
async def handler(
    ...
    await slack_service.delete_message(channel_id, message_ts)
    
    if callback:
        await callback({"text": "Message deleted successfully.", "source": "slack"})
...
delete_message = {
    ...
    "examples": [
        [
            {"name": "{{user1}}", "content": {"text": "Delete that last message"}},
            {"name": "{{agent}}", "content": {"text": "I'll delete that message.", "actions": ["SLACK_DELETE_MESSAGE"]}},
        ],
    ],
}
