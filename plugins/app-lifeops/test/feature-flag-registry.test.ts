...
describe("FeatureFlagRegistry", () => {
  ...
  it("toggle-feature handler accepts a registered third-party key (validation path)", async () => {
    const { toggleFeatureAction } = await import(
      "../src/actions/toggle-feature.ts"
    );
    const runtime = makeRuntimeStub() as IAgentRuntime & {
    ...
    registry.register({
      key: ACME_KEY,
      label: "Acme experiment",
      description: "Synthetic flag for the toggle action test.",
      defaultEnabled: false,
      namespace: "third_party",
      metadata: { costsMoney: "false" },
    });

    // Stub `useModel` to return a JSON envelope picking the synthetic key.
    ...
    // Also stub the SQL service path via a fake CLOUD_AUTH + an in-memory
    // upsert that bypasses Postgres. The handler calls
    // `createFeatureFlagService(runtime)` → `executeRawSql(runtime, ...)`,
    // which would normally need a database. For this test we only verify
    // that the *validation* path accepts the registered key — we do that by
    // asserting the handler does NOT short-circuit with AMBIGUOUS_TOGGLE,
    // and instead reaches the SQL layer (which throws since there's no DB).
    let reachedSql = false;
    const runtimeWithDb = new Proxy(runtime, {
      get(target, prop, receiver) {
        if (prop === "getDatabaseAdapter" || prop === "db") {
          reachedSql = true;
    ...
    let threw = false;
    try {
      await toggleFeatureAction.handler(
        runtimeWithDb,
        {
          entityId: "test-user",
          content: { text: "enable acme" },
        } as unknown as Parameters<typeof toggleFeatureAction.handler>[1],
        undefined,
        undefined,
        async (response) => {
    ...
    // Either the handler threw because SQL is unreachable (validation
    // passed and we got past the registry check), or it returned cleanly
    // without producing the AMBIGUOUS_TOGGLE error message — both prove
    // the registered key was accepted by the registry validation.
    expect(resultText).not.toContain("could not match your request");
    expect(threw || reachedSql || resultText.length > 0).toBe(true);
    __resetFeatureFlagRegistryForTests(runtime);
  });

  it("toggle-feature handler rejects an unregistered key with AMBIGUOUS_TOGGLE", async () => {
    const { toggleFeatureAction } = await import(
      "../src/actions/toggle-feature.ts"
    );
    const runtime = makeRuntimeStub() as IAgentRuntime & {
    ...
    runtime.useModel = async () =>
      JSON.stringify({
        featureKey: "definitely.not.registered",
        enabled: true,
        reason: "test",
      });

    let resultText = "";
    const result = await toggleFeatureAction.handler(
      runtime,
      {
        entityId: "test-user",
        content: { text: "enable something fake" },
      } as unknown as Parameters<typeof toggleFeatureAction.handler>[1],
      undefined,
      undefined,
      async (response) => {
    ...
    expect(result.values?.error).toBe("AMBIGUOUS_TOGGLE");
    expect(resultText).toContain("could not match your request");
    __resetFeatureFlagRegistryForTests(runtime);
  });
});
