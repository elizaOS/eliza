...
function resolveCerebrasApiKey(role) {
    const apiKey = readEnv(role === "eval" ? "EVAL_CEREBRAS_API_KEY" : "TRAIN_CEREBRAS_API_KEY", "CEREBRAS_API_KEY", "ELIZA_E2E_CEREBRAS_API_KEY");
    if (!apiKey) {
        throw new Error(`[${role}-model] CEREBRAS_API_KEY is not set. ` +
...
function resolveTrainingModel() {
    return (readEnv("TRAIN_MODEL", "TRAINING_MODEL", "TRAIN_MODEL_NAME") ??
...
async function callCerebras(config, req) {
    ...
    if (!response.ok) {
        ...
        throw new Error(`[${config.role}-model] cerebras error ${response.status}: ${errBody.slice(0, 300)}`);
    }
