  ...
  async get(
    ...
    return {
      text: [
        ...
        "Use OWNER_INBOX for Gmail, email, and cross-channel executive-assistant inbox work: 'triage my Gmail inbox', 'summarize my unread emails', 'draft a reply to Sarah's latest email', 'send a reply to the last email from finance', 'triage my inbox', 'give me my inbox digest', daily briefs, drafts awaiting sign-off, missed-call repair, and group-chat handoff. Requests like 'if direct relaying gets messy here, suggest making a group chat handoff instead' are executable OWNER_INBOX policies and must not stay in plain REPLY. Do not use OWNER_INBOX just because the user mentioned email or messages while venting.",
        "Use OWNER_SEND_MESSAGE for owner-scoped outbound messages and drafts on the owner's behalf. Examples: 'send a Telegram message to Jane saying I am running late', 'send a Signal message to Priya saying thanks', 'email alice@example.com the notes', 'DM Bob on Discord', or 'text Sam that I am outside'. Always prefer OWNER_SEND_MESSAGE over OWNER_CALENDAR for relaying a message, even if the message text mentions a meeting.",
        ...
        "Route examples: sleep/no-call windows -> OWNER_CALENDAR; daily brief additions, missed-call repair, or group-chat handoff -> OWNER_INBOX; 'if direct relaying gets messy here, suggest making a group chat handoff instead' -> OWNER_INBOX; outbound Telegram/Signal/email/Discord/SMS drafts -> OWNER_SEND_MESSAGE; subscription audits or cancellations -> SUBSCRIPTIONS; travel preference memory -> UPDATE_OWNER_PROFILE; clinic-doc reminders or multi-device meeting ladders -> OWNER_DEVICE_INTENT; portal upload or browser filing -> LIFEOPS_COMPUTER_USE; if the agent gets stuck and should phone the owner -> CALL_USER.",
