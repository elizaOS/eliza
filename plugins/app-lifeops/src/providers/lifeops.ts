...
export const lifeOpsProvider: Provider = {
  name: "lifeops",
  description:
    "Owner and agent only. Provides LifeOps overview plus live calendar and Gmail context. Route executable personal follow-through like todos, habits, goals, reminders, alarms, and live todo-status questions to OWNER_LIFE; all owner calendar, scheduling, availability, and Calendly work to OWNER_CALENDAR; all owner inbox and Gmail/email work to TRIAGE_MESSAGES; morning/night self-review flows to OWNER_CHECKIN; stable owner profile or travel preferences only to OWNER_PROFILE; subscription audits, cancellations, and cancellation-status checks to OWNER_SUBSCRIPTIONS; meeting-prep and person-background briefs to OWNER_RELATIONSHIP; travel booking to OWNER_BOOK_TRAVEL; X/Twitter reads and search to OWNER_X; fixed-duration or generic focus blocks to OWNER_WEBSITE_BLOCK; task-gated focus blocks only to BLOCK_UNTIL_TASK_COMPLETE; browser-companion management to MANAGE_LIFEOPS_BROWSER; password-manager field fill on a trusted site to OWNER_AUTOFILL; pending approval decisions to OWNER_RESOLVE_REQUEST. Available in private owner conversations, including Discord.",
  ...
  async get(
    ...
    return {
      text: [
        "## Life Ops",
        "Use OWNER_LIFE for executable personal follow-through: todos, habits, goals, reminders, alarms, escalation, and live status questions like 'what's on my todo list today?'. Examples: 'add a todo', 'remember to call mom on Sunday', 'track my gym sessions three times a week', 'set a goal to save $5,000'. Do not use REPLY, UPDATE_ENTITY, or OWNER_PROFILE for these.",
        "Use OWNER_CALENDAR for live calendar reads, calendar writes, availability, proposed meeting times, scheduling preferences, and scheduling negotiation. Examples: 'what's my next meeting?', 'show me my calendar for today', 'what does my week look like?', 'schedule a dentist appointment next Tuesday at 3pm', 'find meeting options with Alice', or 'protect my sleep window from calls'. Do not answer these from provider context alone.",
        "Use TRIAGE_MESSAGES for Gmail, email, and cross-channel executive-assistant inbox work: 'triage my Gmail inbox', 'summarize my unread emails', 'draft a reply to Sarah's latest email', 'send a reply to the last email from finance', 'triage my inbox', 'give me my inbox digest', daily briefs, drafts awaiting sign-off, missed-call repair, and group-chat handoff. Requests like 'if direct relaying gets messy here, suggest making a group chat handoff instead' are executable TRIAGE_MESSAGES policies and must not stay in plain REPLY. Do not use TRIAGE_MESSAGES just because the user mentioned email or messages while venting.",
        "Use SEND_DRAFT for owner-scoped outbound messages and drafts on the owner's behalf. Examples: 'send a Telegram message to Jane saying I am running late', 'send a Signal message to Priya saying thanks', 'email alice@example.com the notes', 'DM Bob on Discord', or 'text Sam that I am outside'. Always prefer SEND_DRAFT over OWNER_CALENDAR for relaying a message, even if the message text mentions a meeting.",
        "Use OWNER_PASSWORD_MANAGER for credential lookup and saved-login requests. Examples: 'look up my GitHub password', 'show me my saved logins for github.com', 'copy my AWS password to clipboard'. Do not surface raw secrets in chat.",
        "Use OWNER_RELATIONSHIP for Rolodex contacts, follow-ups, and days-since-contact questions. Examples: 'who are my closest contacts?', 'remind me to follow up with David next week', 'how long has it been since I talked to David?'.",
        ...
        "Use OWNER_PROFILE only for stable owner-only profile details and reusable travel-preference checklists. Do not use it for goals, todos, reminders, temporary plans, or live task state.",
        "Use OWNER_X for X/Twitter reads and search: DMs, timeline, mentions, and topic search. Do not reply that X/Twitter access is unavailable when this action is available.",
        ...
        "Use OWNER_CHAT_THREAD for targeted connector chat mute/unmute when the owner names a Telegram/Discord/etc. room that is not the current chat, especially temporary mutes like 'mute the crypto signals Telegram group for 24 hours'. Do not fall back to generic MUTE_ROOM for named off-thread chat controls.",
        "Use OWNER_DEVICE_INTENT for multi-device reminders, push ladders, document-signing nudges, updated-ID interventions, and device-level warnings. Examples: 'for important meetings, remind me an hour before, ten minutes before, and right when they start on both my Mac and my phone', 'if missing this could trigger a cancellation fee, warn me clearly and offer to handle it now', or 'if the only ID on file is expired, ask me for an updated copy so the workflow can continue'. Do not stay in REPLY just because the exact reservation, workflow item, or upload is still unspecified. Use OWNER_COMPUTER_USE for portal uploads, Finder/Desktop work like taking screenshots or creating folders, browser workflows, and file-handling tasks on the owner's machine, including future instructions like 'when I send over the deck, upload it to the portal for me.'",
        ...
        "Use OWNER_RESOLVE_REQUEST when the owner is resolving a pending approval item. Examples: 'approve the pending travel booking request' or 'reject that pending approval request and say it needs changes'.",
        "Use OWNER_VOICE_CALL for phone-call escalation or booking calls. These actions can draft or request confirmation first; they do not require the dial to happen on the first turn. Requests like 'if you get stuck in the browser or on my computer, call me and let me jump in to unblock it' belong here. Requests like 'call the dentist and reschedule my appointment' or 'phone my cable company about the outage' also belong to OWNER_VOICE_CALL, not OWNER_CALENDAR, OWNER_LIFE, or SEND_DRAFT.",
        ...
        "Route examples: sleep/no-call windows -> OWNER_CALENDAR; daily brief additions, missed-call repair, or group-chat handoff -> TRIAGE_MESSAGES; 'if direct relaying gets messy here, suggest making a group chat handoff instead' -> TRIAGE_MESSAGES; outbound Telegram/Signal/email/Discord/SMS drafts -> SEND_DRAFT; subscription audits or cancellations -> OWNER_SUBSCRIPTIONS; travel preference memory -> OWNER_PROFILE; clinic-doc reminders or multi-device meeting ladders -> OWNER_DEVICE_INTENT; portal upload or browser filing -> OWNER_COMPUTER_USE; if the agent gets stuck and should phone the owner -> OWNER_VOICE_CALL.",
