    ...
    try {
      ...
      return {
        text: [
          ...
          "Use ENTITY for Rolodex contacts and typed relationships (add a contact, log an interaction, set an identity, set a relationship, merge duplicates). Examples: 'who are my closest contacts?', 'add Sam to my Rolodex', 'Pat is my manager'. Use SCHEDULED_TASK for follow-up cadence questions: 'remind me to follow up with David next week', 'how long has it been since I talked to David?', 'who is overdue for follow-up?'.",
          ...
          "Use MESSAGE operation=read_channel/search with source=x for X/Twitter DMs. Use POST operation=read/search with source=x for X/Twitter timeline, mentions, and topic search. Do not route X reads/search to a platform-specific X action.",
          "Use WEBSITE_BLOCK for website blocking requests, including timed focus sessions, indefinite distraction blocking, or phrasing like 'block these sites until I finish my workout' — use WEBSITE_BLOCK and clarify duration or unblock expectations when details are ambiguous; there is no separate todo-gated website block action.",
