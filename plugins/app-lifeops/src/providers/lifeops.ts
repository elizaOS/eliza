  ...
  async get(
    ...
    try {
      ...
      const connectorAccounts = await accountManager
        .listAccounts("google")
        .catch(() => []);
      const privacyByAccountKey = new Map<string, ReturnType<typeof getAccountPrivacy>>();
      ...
      try {
          ...
          for (const account of connectedAccounts.slice(0, MAX_ACCOUNT_LINES)) {
            ...
            const identityEmail =
              typeof (account.identity as Record<string, unknown> | null)
                ?.email === "string"
                ? String(
                    (account.identity as Record<string, unknown>).email,
                  )
                : null;
        ...
        if (status?.connected) {
          ...
          const statusIdentityEmail =
            typeof (status.identity as Record<string, unknown> | null)?.email ===
            "string"
