  ...
  [
    { name: "{{user1}}", content: { text: "Find my GitHub login" } },
    {
      ...
      content: {
        ...
        actions: ["OWNER_PASSWORD_MANAGER"],
      },
    },
  ],
  [
...
export const passwordManagerAction: Action & {
  ...
  descriptionCompressed:
    "password manager 1Password ProtonPass: search list inject_username inject_password; clipboard-only never displays creds; confirmed-true required for inject; owner",
  ...
  handler: async (runtime, message, state, options): Promise<ActionResult> => {
    ...
    const params = (await extractActionParamsViaLlm<PasswordManagerParameters>({
      ...
      message,
      state,
      actionName: "OWNER_PASSWORD_MANAGER",
    ...
    if (subaction === "inject_username" || subaction === "inject_password") {
      ...
      logger.info(
        {
          action: "OWNER_PASSWORD_MANAGER",
          subaction,
          itemId,
          field,
          fixtureMode: result.fixtureMode === true,
        },
        `[OWNER_PASSWORD_MANAGER] Copied ${field} for item ${itemId} to clipboard`,
      );
