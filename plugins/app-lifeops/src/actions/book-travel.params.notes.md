PERSONAL_ASSISTANT action=book_travel parameter typing rationale (2026-05-10):
...
- No top-level field marked required: action=book_travel is dual-mode — either an `offerId` is supplied to confirm an existing search, or origin/destination/departureDate trigger a fresh search. Marking either set required would break the other path.
