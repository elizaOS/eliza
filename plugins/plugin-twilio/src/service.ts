...
export class TwilioService extends Service implements TwilioServiceInterface {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        ...
        this.twilioConfig = {
            ...
            phoneNumber: runtime.getSetting("TWILIO_PHONE_NUMBER") as string,
            webhookUrl: runtime.getSetting("TWILIO_WEBHOOK_URL") as string,
            webhookPort: parseInt(runtime.getSetting("TWILIO_WEBHOOK_PORT") || "3000"),
        };
    ...
    private async setupWebhookServer(): Promise<void> {
        ...
        this.app.post(TWILIO_CONSTANTS.WEBHOOK_PATHS.STATUS, (req, res) => {
            ...
            logger.info("Status callback received:", { MessageSid, MessageStatus, CallSid, CallStatus });
            res.sendStatus(200);
        });
        ...
        this.wss.on("connection", (ws, req) => {
            ...
            ws.on("message", async (data) => {
                try {
                    ...
                    logger.error("Error processing voice stream message:", error);
                }
            });
    ...
    private async updatePhoneNumberWebhooks(): Promise<void> {
        try {
            ...
            logger.error("Error updating phone number webhooks:", error);
            throw error;
        }
    }
    ...
    async sendSms(to: string, body: string, mediaUrl?: string[]): Promise<TwilioMessage> {
        ...
        try {
            ...
            // Cache the message
            const conversationKey = CACHE_KEYS.CONVERSATION(to);
    ...
    private async processIncomingMessage(message: TwilioMessage): Promise<void> {
        try {
            ...
            const memory: Memory = {
                entityId: message.from as UUID,
                agentId: this.runtime.agentId,
                roomId: `twilio_${message.from}` as UUID,
                content: {
                    text: message.body,
                    source: "twilio",
            ...
            // Store in runtime database - use the correct method signature
            await this.runtime.createMemory(memory, "messages");

            // Process with agent - we need to manually handle the response
            // since processMessage doesn't exist, we'll emit an event for the runtime to handle
            if (this.runtime) {
                this.runtime.emitEvent("message_received", {
                    ...
                    callback: async (response: any) => {
                        ...
                        if (response && response.text) {
                            await this.sendSms(message.from, response.text);
                            
                            // Store agent's response as a memory
