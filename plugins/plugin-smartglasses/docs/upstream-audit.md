...
- Direct G1 BLE packet protocol and parsing: `src/protocol.ts`
...
- Eliza actions/providers: `src/actions/*.ts`, `src/providers/status.ts`
...
- `src/__tests__/protocol.test.ts`: packet encoding, pixel-aware display
  wrapping, display chunking, mic commands, tap/audio/serial parsing, settings,
  dashboard, navigation, translation, notifications, notes, voice notes, BMP,
  heartbeat, init modes, and app whitelist/setup.
