...
export class SmartglassesService extends Service {
  ...
  static async start(runtime: IAgentRuntime): Promise<SmartglassesService> {
    ...
    if (!service.transport) {
      logger.info(
        "[smartglasses] no transport available; service loaded in offline/mockable mode",
      );
      return service;
    }
    await service.connect();
    if (readBooleanSetting(runtime, SMARTGLASSES_AUTO_INIT_SETTING, true)) {
  ...
  startHeartbeatLoop(
    ...
    this.heartbeatTimer = setInterval(() => {
      void this.sendHeartbeat().catch((error) => {
        logger.warn({ error }, "[smartglasses] heartbeat failed");
      });
    }, intervalMs);
  ...
  private async decodeAudioChunk(
    ...
    try {
      ...
      logger.warn(
        { error },
        "[smartglasses] audio decoder failed; raw audio event preserved",
      );
      return null;
    }
  }
  ...
  private async emitPluginEvent(
    ...
    await (
      ...
      source: "@elizaos/plugin-smartglasses",
      ...payload,
    });
  }
}
...
async function chooseTransport(
  ...
  const preferred = normalizePreferredTransport(
    readSetting(runtime, SMARTGLASSES_TRANSPORT_SETTING),
  );
...
function readConnectionReadyModeSetting(
  ...
  const value = String(
    readSetting(runtime, SMARTGLASSES_INIT_MODE_SETTING) ?? "",
  )
    .trim()
    .toLowerCase();
  if (
