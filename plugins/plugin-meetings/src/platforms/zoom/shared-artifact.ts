 ...
 */
import { MEETING_ARTIFACT_SCHEMA_VERSION } from "@elizaos/core/meeting-artifacts";
import { assertValidMeetingArtifact } from "@elizaos/core/meeting-artifacts";
...
export function buildZoomBotMeetingArtifact(input: ZoomBotMeetingArtifactInput): MeetingArtifact {
    const streamId = `zoom-bot-mixed:${input.meetingId}`;
    const participantByName = new Map(input.participants.map((participant) => [
        participant.displayName.trim().toLowerCase(),
        participant,
    ]));
    const speakerRows = new Map<string, {
        label: string;
    ...
    const transcriptSpans = input.segments.map((segment) => {
        ...
        if (speakerId && label)
            speakerRows.set(speakerId, { label, participant });
        return {
            id: segment.id,
    ...
    const diarizedSpeakers = [...speakerRows].map(([id, { label, participant }]) => ({
        id,
        ...
        evidenceArtifacts: [
            {
                ...
                kind: "metrics",
                mediaRefId: input.audio.id,
                transcriptSpanIds: transcriptSpans.length > 0
                    ? transcriptSpans.map((span) => span.id)
                    : undefined,
                description: "Zoom browser capture retained mixed audio; per-participant audio was unavailable.",
            },
        ],
        provenance: {
