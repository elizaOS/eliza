...
import { transcriptPreview } from "@elizaos/core/transcripts";
...
import { type MemoryMetadata } from "@elizaos/core";
import { type Transcript } from "@elizaos/core/transcripts";
import { type TranscriptConsentState } from "@elizaos/core/transcripts";
 ...
 */
export function persistMeetingMedia(bytes: Buffer, extension: string): {
    id: string;
    url: string;
    checksum: string;
} {
...
export class MeetingTranscriptWriter {
    ...
    constructor(private readonly runtime: MeetingTranscriptRuntime, private readonly throttleMs: number = DEFAULT_WRITE_THROTTLE_MS, private readonly now: () => number = Date.now) {
    ...
    async start(input: StartMeetingTranscriptInput): Promise<Transcript> {
        ...
        logger.info({ transcriptId: this.transcriptId, sessionId: input.sessionId }, "[MeetingService] meeting transcript record created (recording)");
        return transcript;
    }
    /**
    ...
    updateSegments(segments: TranscriptSegment[]): void {
        if (this.finalized || !this.transcript)
            return;
    ...
    async finalize(input: FinalizeMeetingTranscriptInput): Promise<Transcript> {
        if (!this.transcript || !this.input) {
            throw new Error("[MeetingService] finalize called before transcript start");
        }
        if (this.finalized)
        ...
        if (input.audioWav && input.retainedAudio) {
            throw new Error("[MeetingService] finalize accepts captured or imported audio, not both");
        }
        ...
        if (knowledgeDocumentId)
            final.knowledgeDocumentId = knowledgeDocumentId;
        ...
        if (!ok) {
            throw new Error(`[MeetingService] transcript ${this.transcriptId} row vanished before finalize`);
        }
        logger.info({
            transcriptId: this.transcriptId,
     ...
     */
    private async mirrorToKnowledge(transcript: Transcript): Promise<string | undefined> {
        const documents = this.runtime.getService("documents") as DocumentsLike | null;
        if (!documents || !this.input)
            return undefined;
