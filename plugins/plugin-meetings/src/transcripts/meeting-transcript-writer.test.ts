 ...
 */
import { MeetingTranscriptWriter } from "./meeting-transcript-writer.js";
import { TRANSCRIPTS_TABLE } from "./meeting-transcript-writer.js";
...
import { persistMeetingAudioWav } from "./meeting-transcript-writer.js";
import { readTranscriptRow } from "./meeting-transcript-writer.js";
import { rmSync } from "node:fs";
import { segment } from "../test-support.js";
...
describe("MeetingTranscriptWriter — record shape golden", () => {
    it("persists a row the transcripts-routes reader parses at every lifecycle stage", async () => {
        ...
        expect(summarizeTranscript(recording as Transcript).id).toBe(writer.transcriptId);
...
describe("MeetingTranscriptWriter — throttling", () => {
    ...
    it("coalesces rapid segment updates to ~one write per throttle window", async () => {
        ...
        const live = transcriptsViewReader(fake.memories.get(writer.transcriptId) as Memory);
