...
import { DEFAULT_MEETING_AUTO_LEAVE } from "@elizaos/core/meetings";
import { DEFAULT_MEETING_MAX_DURATION_MS } from "@elizaos/core/meetings";
...
import { meetingArtifactToTranscriptSegments } from "@elizaos/core/meeting-artifacts";
...
import { type MeetingArtifact } from "@elizaos/core/meeting-artifacts";
import { type MeetingAudioSink } from "./types.js";
import { type MeetingAutoLeaveConfig } from "@elizaos/core/meetings";
import { type MeetingBillingError } from "./types.js";
import { type MeetingBillingSession } from "./types.js";
import { type MeetingBillingSessionInput } from "./types.js";
...
import { type MeetingGhostAttendanceContext } from "@elizaos/core/meetings";
...
import { type MeetingTranscriptionPipeline } from "./types.js";
import { type ResolvedMeetingBotConfig } from "./types.js";
import { type Transcript } from "@elizaos/core/transcripts";
import { type TranscriptSegment } from "@elizaos/core/transcripts";
import { type UUID } from "@elizaos/core";
import { type ZoomCloudImportInput } from "./platforms/zoom/cloud-import.js";
import { type ZoomCloudImportResult } from "./platforms/zoom/cloud-import.js";
...
export interface MeetingServiceDependencies {
    ...
    createBillingSession?(input: MeetingBillingSessionInput): MeetingBillingSession | null;
    importZoomCloudMeeting?(input: ZoomCloudImportInput): Promise<ZoomCloudImportResult>;
}
...
export type MeetingJoinErrorCode = "invalid_url" | "unsupported_platform" | "unsupported_host" | "policy_blocked" | "already_joined" | "invalid_duration_cap" | "insufficient_credits";
/** Validation/conflict failures of `requestJoin` — routes map these to 4xx. */
export class MeetingJoinError extends Error {
    constructor(readonly code: MeetingJoinErrorCode, message: string) {
        super(message);
...
export class MeetingService extends Service {
    ...
    capabilityDescription = "Joins Google Meet / Microsoft Teams / Zoom meetings as a notetaker bot and produces live, diarized transcripts";
    /**
     ...
     */
    static dependencyFactory: ((runtime: IAgentRuntime) => MeetingServiceDependencies) | null = null;
    private static readonly runtimeDependencyFactories = new WeakMap<IAgentRuntime, (runtime: IAgentRuntime) => MeetingServiceDependencies>();
    /** Install a dependency override for exactly one runtime, primarily for scenario boundaries. */
    static setRuntimeDependencyFactory(runtime: IAgentRuntime, factory: (runtime: IAgentRuntime) => MeetingServiceDependencies): void {
    ...
    constructor(runtime?: IAgentRuntime, deps?: MeetingServiceDependencies) {
        ...
        if (!resolved) {
            throw new Error("[MeetingService] no dependencies wired — import the plugin entry (src/index.ts) or inject MeetingServiceDependencies");
        }
        this.deps = resolved;
    ...
    async stop(): Promise<void> {
        const active = [...this.sessions.values()].filter((s) => !TERMINAL_STATUSES.has(s.status));
    ...
    async requestJoin(request: MeetingJoinRequest): Promise<MeetingSession> {
        ...
        if (!parsed) {
            throw new MeetingJoinError("invalid_url", `"${request.meetingUrl}" is not a recognizable Google Meet, Microsoft Teams, or Zoom meeting link`);
        }
        if (request.platform === "discord") {
            throw new MeetingJoinError("unsupported_platform", "Discord meetings are voice channels handled by the Discord connector — the browser meeting bot cannot join them");
        }
        const adapter = this.deps.adapters.get(parsed.platform);
        if (!adapter) {
            throw new MeetingJoinError("unsupported_platform", `no platform adapter available for ${MEETING_PLATFORM_LABELS[parsed.platform]}`);
        }
        ...
        if (!support.supported) {
            throw new MeetingJoinError("unsupported_host", support.reason ??
                "this host cannot run the meeting browser bot (no Chromium available)");
        }
        this.assertCapturePolicyAllowsJoin();
        const duplicate = [...this.sessions.values()].find((s) => !TERMINAL_STATUSES.has(s.status) &&
            s.platform === parsed.platform &&
            s.nativeMeetingId === parsed.nativeMeetingId);
        if (duplicate) {
            throw new MeetingJoinError("already_joined", `a bot is already in this meeting (session ${duplicate.id}, status ${duplicate.status})`);
        }
        ...
        const botName = request.botName?.trim() ||
        ...
        const pipeline = this.deps.createPipeline({
            ...
            ...(request.ghostAttendance?.attendees?.length
                ? {
                    calendarSpeakerEvidence: request.ghostAttendance.attendees.map((attendee, index) => ({
                        source: "calendar_attendee" as const,
                        name: attendee.name,
                        confidence: 0.82,
                        evidenceId: request.calendarEventId ??
        ...
        try {
            ...
            try {
                ...
                logger.error({
                    ...
                    platform: parsed.platform,
                    nativeMeetingId: parsed.nativeMeetingId,
                    error: billingErr instanceof Error
                        ? billingErr.message
                        : String(billingErr),
            ...
            if (billingReleaseFailure !== undefined) {
                throw new AggregateError([err, billingReleaseFailure], "Meeting join setup and billing release both failed");
            }
    ...
    listSessions(options?: {
        ...
        return all.sort((a, b) => {
            const bReq = typeof b.requestedAt === "number" && Number.isFinite(b.requestedAt)
                ? b.requestedAt
                : 0;
            const aReq = typeof a.requestedAt === "number" && Number.isFinite(a.requestedAt)
                ? a.requestedAt
                : 0;
    ...
    async waitForSessionCompletion(sessionId: UUID): Promise<MeetingSession> {
        ...
        if (!terminal) {
            throw new Error(`[MeetingService] session ${sessionId} completed without a terminal snapshot`);
        }
        return terminal;
    }
    ...
    async importZoomMeeting(request: ZoomMeetingImportRequest): Promise<ZoomMeetingImportResult> {
        ...
        if (!accessToken) {
            throw new ZoomCloudImportError("invalid_request", "Zoom access token is required in the private request or ELIZA_ZOOM_ACCESS_TOKEN.", 400);
        }
        ...
        const roomId = createUniqueUuid(this.runtime, `zoom-import:${imported.artifact.meeting.id}`);
        const worldId = await this.ensureMeetingsWorld();
        await this.runtime.ensureRoomExists({
            id: roomId,
            name: imported.artifact.meeting.title ??
        ...
        await writer.start({
            ...
            roomId,
            entityId: this.runtime.agentId,
            title: imported.artifact.meeting.title ??
                `Zoom meeting ${imported.artifact.meeting.id}`,
            platform: "zoom",
            meetingUrl: `https://zoom.us/j/${encodeURIComponent(request.meetingId)}`,
            nativeMeetingId: imported.artifact.meeting.nativeMeetingId ??
                imported.artifact.meeting.id,
            consentState: imported.artifact.meeting.consent.state === "redacted"
                ? "unknown"
                : imported.artifact.meeting.consent.state,
        });
        const segments = meetingArtifactToTranscriptSegments(imported.artifact);
        const retainedAudio = imported.artifact.media.find((media) => media.mimeType.startsWith("audio/"));
        const transcript = await writer.finalize({
            segments,
            endReason: "normal_completion",
            participants: imported.artifact.platformParticipants.map((participant) => ({
                id: participant.id,
    ...
    private rosterTrackingSink(session: InternalSession): MeetingAudioSink {
        ...
        return {
            pushSpeakerAudio: (speakerKey, samples) => pipeline.pushSpeakerAudio(speakerKey, samples),
            setSpeakerName: (speakerKey, displayName) => pipeline.setSpeakerName(speakerKey, displayName),
    ...
    private trackParticipantJoined(session: InternalSession, participant: MeetingParticipant): MeetingParticipant {
        ...
        const entityId = createUniqueUuid(this.runtime, `meeting-participant:${session.platform}:${participant.displayName.toLowerCase()}`);
    ...
    private applyStatus(session: InternalSession, status: MeetingSessionStatus): void {
        ...
        logger.info({ sessionId: session.id, status }, "[MeetingService] session status");
        this.emitter.emitStatus(this.toDto(session));
    }
    /** Full adapter lifecycle: run → finalize pipeline + transcript → terminal. */
    private async runSession(session: InternalSession, adapter: MeetingPlatformAdapter, botSession: MeetingBotSession): Promise<void> {
        ...
        try {
            ...
            logger.error({ sessionId: session.id, error: errorMessage }, "[MeetingService] platform adapter failed");
        }
        finally {
            if (capTimer)
    ...
    private async finishSession(session: InternalSession, endReason: MeetingEndReason, errorMessage: string | undefined): Promise<void> {
        ...
        try {
            ...
            errorMessage =
                errorMessage ?? (err instanceof Error ? err.message : String(err));
            logger.error({ sessionId: session.id, error: errorMessage }, "[MeetingService] pipeline finalize failed");
        }
        try {
            ...
            errorMessage =
                errorMessage ?? (err instanceof Error ? err.message : String(err));
            logger.error({ sessionId: session.id, error: errorMessage }, "[MeetingService] transcript finalize failed");
        }
        try {
            ...
            errorMessage =
                errorMessage ?? (err instanceof Error ? err.message : String(err));
            logger.error({ sessionId: session.id, error: errorMessage }, "[MeetingService] billing reconciliation failed");
        }
    ...
    private async emitTranscriptFinalized(session: InternalSession, dto: MeetingSession, transcript: Transcript | null): Promise<void> {
        if (!transcript || session.status !== "ended")
            return;
    ...
    private async reconcileBillingOnce(session: InternalSession, endReason: MeetingEndReason): Promise<void> {
        if (!session.billing)
            return;
    ...
    private assertCapturePolicyAllowsJoin(): void {
        const configured = this.settingString("ELIZA_MEETINGS_CAPTURE_POLICY")?.toLowerCase();
        const cloudEnabled = this.settingString("ELIZAOS_CLOUD_ENABLED")?.toLowerCase();
        const organizationManaged = Boolean(this.settingString("ELIZAOS_CLOUD_ORG_ID") ??
        ...
        if (configured === "allow")
            return;
        if (configured === "deny") {
            throw new MeetingJoinError("policy_blocked", "meeting capture is blocked by organization policy");
        }
        if (configured !== undefined || organizationManaged) {
            throw new MeetingJoinError("policy_blocked", "meeting capture policy is unavailable; capture was not started");
        }
    }
    private resolveMaxDurationMs(requested: number | undefined): number {
        const configured = this.settingPositiveInteger("ELIZA_MEETINGS_MAX_DURATION_MS");
        const maximum = configured ?? DEFAULT_MEETING_MAX_DURATION_MS;
        if (requested !== undefined &&
            (!Number.isSafeInteger(requested) || requested <= 0)) {
            throw new MeetingJoinError("invalid_duration_cap", "maxDurationMs must be a positive integer");
        }
        if (requested !== undefined && requested > maximum) {
            throw new MeetingJoinError("invalid_duration_cap", `maxDurationMs exceeds the configured maximum (${maximum}ms)`);
        }
