...
import { type MeetingJoinRequest } from "@elizaos/shared";
...
function sessionForViewer(ctx: RouteHandlerContext, session: MeetingSession): Promise<MeetingSession> {
...
const listRoute: Route = {
    ...
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        ...
        const active = activeParam === "1" || activeParam === "true" ? true : undefined;
        const sessions = await Promise.all(svc
            .listSessions({ active })
...
const importZoomRoute: Route = {
    ...
    routeHandler: async (ctx): Promise<RouteHandlerResult> => {
        ...
        if (body.retainRecordings !== undefined &&
