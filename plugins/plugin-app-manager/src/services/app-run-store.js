...
function normalizeHealthFacet(value, fallbackMessage) {
    if (isRecord(value)) {
        return {
...
function deriveHealthStateFromStatus(status) {
...
function normalizeHealth(value, status, summary) {
    if (isRecord(value)) {
...
function normalizeViewerConfig(value) {
...
function normalizeSessionState(value) {
    ...
    return {
        ...
        ...(Array.isArray(value.controls)
            ? {
                controls: value.controls.filter((control) => control === "pause" || control === "resume"),
            }
            : {}),
        ...
        ...(Array.isArray(value.suggestedPrompts)
            ? {
                suggestedPrompts: value.suggestedPrompts.filter((prompt) => typeof prompt === "string"),
            }
            : {}),
        ...(isRecord(value.telemetry) || value.telemetry === null
            ? {
                telemetry: value.telemetry === null
                    ? null
                    : normalizeSessionJsonValue(value.telemetry),
            }
            : {}),
    };
}
function normalizeSessionJsonValue(value) {
    if (value === null)
        return null;
...
function normalizeRunEvent(value) {
    if (!isRecord(value))
        return null;
...
function normalizeAwaySummary(value) {
    if (!isRecord(value))
        return null;
...
function deriveRunHealthFacetState(availability) {
...
function normalizeRun(input) {
    ...
    const supportsBackground = typeof run.supportsBackground === "boolean" ? run.supportsBackground : true;
    const supportsViewerDetach = typeof run.supportsViewerDetach === "boolean"
        ? run.supportsViewerDetach
        : supportsBackground;
    const characterId = typeof run.characterId === "string"
        ? run.characterId
        : (session?.characterId ?? null);
    const agentId = typeof run.agentId === "string" ? run.agentId : (session?.agentId ?? null);
    const chatAvailability = typeof run.chatAvailability === "string"
        ? normalizeAvailability(run.chatAvailability)
        : deriveChatAvailability(session);
    const controlAvailability = typeof run.controlAvailability === "string"
    ...
    const baseRun = {
        ...
        startedAt,
        updatedAt,
        lastHeartbeatAt: typeof run.lastHeartbeatAt === "string"
            ? run.lastHeartbeatAt
            : session
                ? updatedAt
                : null,
        ...
        chatAvailability,
        controlAvailability,
        viewerAttachment: viewerAttachmentValue === "attached" ||
    ...
    return {
        ...baseRun,
        awaySummary: normalizeAwaySummary(run.awaySummary) ?? buildAwaySummary(baseRun),
        healthDetails: isRecord(run.healthDetails)
            ? {
                checkedAt: typeof run.healthDetails.checkedAt === "string"
                    ? run.healthDetails.checkedAt
                    : derivedHealthDetails.checkedAt,
                auth: normalizeHealthFacet(run.healthDetails.auth, derivedHealthDetails.auth.message),
                runtime: normalizeHealthFacet(run.healthDetails.runtime, derivedHealthDetails.runtime.message),
                viewer: normalizeHealthFacet(run.healthDetails.viewer, derivedHealthDetails.viewer.message),
                chat: normalizeHealthFacet(run.healthDetails.chat, derivedHealthDetails.chat.message),
                control: normalizeHealthFacet(run.healthDetails.control, derivedHealthDetails.control.message),
                message: typeof run.healthDetails.message === "string"
...
export function resolveAppRunStoreFilePath(stateDir = resolveStateDir()) {
    return path.join(stateDir, "apps", "runs.v2.json");
}
export function resolveLegacyAppRunStoreFilePath(stateDir = resolveStateDir()) {
    return path.join(stateDir, "apps", "runs.v1.json");
}
function readAppRunStoreFile(filePath) {
...
export function readAppRunStore(stateDir = resolveStateDir()) {
