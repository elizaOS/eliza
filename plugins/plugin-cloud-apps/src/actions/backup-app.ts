...
export const backupAppAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      logger.warn(`[BACKUP_APP] failed: ${err instanceof Error ? err.message : String(err)}`);
      const msg = "I couldn't export that backup right now.";
      await callback?.({ text: msg, actions: ["BACKUP_APP"] });
      return { success: false, text: "Backup failed.", userFacingText: msg, error: err instanceof Error ? err : new Error(String(err)), data: { reason: "error" } };
    }
  },
  ...
  examples: [
    [
      ...
      { name: "{{agent}}", content: { text: 'Backed up "Acme Bot" — a config snapshot (v1, no secrets) you can save and restore later. Monetization: off.', actions: ["BACKUP_APP"] } },
    ],
  ],
};
