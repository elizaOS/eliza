...
import { extractAppReference, getCloudClient, resolveApp, resolveCloudApiKey } from "../client.js";
...
export function selectRollbackTarget(
  ...
  const restorable = deployments
    .filter((d) => d.id !== activeId && (d.status === "superseded" || d.status === "ready" || d.status === "active"))
    .sort((a, b) => b.version - a.version);
  if (version !== undefined) return restorable.find((d) => d.version === version) ?? null;
  return restorable[0] ?? null;
}

export const listFrontendDeploymentsAction: Action = {
  name: "LIST_FRONTEND_DEPLOYMENTS",
  similes: ["SHOW_FRONTEND_VERSIONS", "FRONTEND_HISTORY", "APP_FRONTEND_DEPLOYMENTS"],
  description:
  ...
  descriptionCompressed: "List an app's frontend deployment versions + the live one.",
  ...
  handler: async (runtime, message, _state, options, callback): Promise<ActionResult> => {
    ...
    if (!client) {
      await callback?.({ text: NO_KEY_MESSAGE, actions: ["LIST_FRONTEND_DEPLOYMENTS"] });
    ...
    const { app, available } = reference ? await resolveApp(client, reference) : { app: null, available: [] as string[] };
    if (!app) {
      const msg = available.length === 0 ? "You don't have any apps yet." : `Which app? Your apps: ${available.join(", ")}.`;
    ...
    try {
      const { deployments, active_deployment_id } = await client.listAppFrontendDeployments(app.id);
      const reply =
        deployments.length === 0
          ...
          : `"${app.name}" frontend versions:\n${deployments
              .slice()
              .sort((a, b) => b.version - a.version)
              .map((d) => `• v${d.version} (${d.status})${d.id === active_deployment_id ? " ← live" : ""}`)
              .join("\n")}`;
      await callback?.({ text: reply, actions: ["LIST_FRONTEND_DEPLOYMENTS"] });
      return { success: true, text: `Listed ${deployments.length} frontend deployments.`, userFacingText: reply, verifiedUserFacing: true, data: { count: deployments.length, activeId: active_deployment_id } };
    } catch (err) {
      logger.warn(`[LIST_FRONTEND_DEPLOYMENTS] failed: ${err instanceof Error ? err.message : String(err)}`);
      const msg = "I couldn't list the frontend deployments right now.";
      await callback?.({ text: msg, actions: ["LIST_FRONTEND_DEPLOYMENTS"] });
      return { success: false, text: "Failed to list.", userFacingText: msg, error: err instanceof Error ? err : new Error(String(err)), data: { reason: "error" } };
    }
  },

  examples: [
    [
      { name: "{{user}}", content: { text: "show my Acme Bot frontend versions" } },
      { name: "{{agent}}", content: { text: '"Acme Bot" frontend versions:\n• v3 (active) ← live\n• v2 (superseded)', actions: ["LIST_FRONTEND_DEPLOYMENTS"] } },
    ],
  ],
};
...
export const rollbackFrontendAction: Action = {
  ...
  similes: ["REVERT_FRONTEND", "RESTORE_FRONTEND_VERSION", "UNDO_FRONTEND_DEPLOY"],
  description:
  ...
  suppressPostActionContinuation: true,
  parameters: [
    { name: "appName", description: "Name/slug/id of the app to roll back.", required: false, schema: { type: "string" } },
    { name: "version", description: "Specific frontend version number to restore. Omit to roll back to the previous one.", required: false, schema: { type: "number" } },
  ],
  ...
  handler: async (runtime, message, _state, options, callback): Promise<ActionResult> => {
    const client = getCloudClient(runtime);
    if (!client) {
      await callback?.({ text: NO_KEY_MESSAGE, actions: ["ROLLBACK_FRONTEND"] });
      return { success: false, text: "No Cloud API key.", userFacingText: NO_KEY_MESSAGE, data: { reason: "no_key" } };
    }
    const reference = extractAppReference(message, options);
    const { app, available } = reference ? await resolveApp(client, reference) : { app: null, available: [] as string[] };
    ...
    try {
      const { deployments, active_deployment_id } = await client.listAppFrontendDeployments(app.id);
      const target = selectRollbackTarget(deployments, active_deployment_id, version);
      if (!target) {
        const msg =
          version !== undefined
        ...
        await callback?.({ text: msg, actions: ["ROLLBACK_FRONTEND"] });
        return { success: false, text: "No rollback target.", userFacingText: msg, data: { reason: "no_target" } };
      }
      ...
      return {
        ...
        data: { app: { id: app.id, name: app.name }, activatedVersion: target.version, activatedId: target.id },
      };
    } catch (err) {
      logger.warn(`[ROLLBACK_FRONTEND] failed: ${err instanceof Error ? err.message : String(err)}`);
      const msg = "I couldn't roll back the frontend right now.";
      await callback?.({ text: msg, actions: ["ROLLBACK_FRONTEND"] });
      return { success: false, text: "Rollback failed.", userFacingText: msg, error: err instanceof Error ? err : new Error(String(err)), data: { reason: "error" } };
    }
  },

  examples: [
    [
      { name: "{{user}}", content: { text: "roll back the Acme Bot site, the new one is broken" } },
      { name: "{{agent}}", content: { text: 'Rolled "Acme Bot" frontend back to v2 — it\'s live now.', actions: ["ROLLBACK_FRONTEND"] } },
    ],
  ],
};
