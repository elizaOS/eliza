...
import { DEVICE_REGISTRY, type FacewearDeviceType } from "../devices/registry.ts";
...
export const facewearConnectAction: Action = {
  ...
  similes: ["CONNECT_GLASSES", "CONNECT_HEADSET", "PAIR_DEVICE", "CONNECT_FACEWEAR"],
  examples: [
    [
      { name: "{{user1}}", content: { text: "How do I connect my Quest 3?" } },
      { name: "{{user2}}", content: { text: "To connect your Meta Quest 3, open a browser in VR, navigate to the pairing URL, and tap Allow on the camera/mic prompts." } },
    ],
  ],
  ...
  handler: async (
    ...
    else if (text.includes("even") || text.includes("g1") || text.includes("g2")) deviceType = "even-realities";
    else if (text.includes("vision") || text.includes("apple") || text.includes("avp")) deviceType = "apple-vision-pro";
    ...
    if (profile.connectionType === "ble") {
      ...
      instructions += "3. The elizaOS agent will auto-detect via Noble BLE or Web Bluetooth\n";
      instructions += "4. For native Android: install the Even Realities companion app\n";
    } else {
      instructions += "**Connection method:** WebXR over WebSocket\n";
      instructions += "1. Start the elizaOS agent (this server)\n";
      instructions += "2. Visit `/api/xr/connect` for the QR code + pairing URL\n";
