...
- **`eval()` uses postMessage with a 5 s timeout.** The web view must handle `eliza:eval` messages and reply with `eliza:evalResult`. If the page does not implement this, `eval()` rejects. Each `eliza:eval` includes a `messageId`; echo it on the `eliza:evalResult` reply to correlate concurrent calls to their own results per call. Replies that omit it fall back to in-order FIFO matching (oldest pending call wins).
