...
All CI workflows use `N8N_HOST` and `N8N_API_KEY` from GitHub Secrets, pointing to the `service-n8n@elizalabs.ai` n8n account.
...
|---|---|
| `N8N_HOST` | `https://elizaapp.app.n8n.cloud` |
| `N8N_API_KEY` | 1Password — search for the `service-n8n@elizalabs.ai` n8n API key |
| OAuth credentials | Pull from Vercel (see below) or 1Password |
| [Bun](https://bun.sh) | Runtime for running scripts locally |

### Step 0: Set up `.env`

The script needs `N8N_HOST` + `N8N_API_KEY` to talk to n8n, and optionally OAuth client ID/secret per platform (so credentials are pre-populated instead of using n8n defaults).
...
vercel env pull /path/to/plugin-n8n-workflow/.env --yes
```
...
```bash
N8N_HOST=... N8N_API_KEY=... bun run scripts/crawl-triggers-live.ts [options]
```
