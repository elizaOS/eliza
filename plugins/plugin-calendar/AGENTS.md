...
Conversational attendee display names or mailbox names are not evidence of an email address. An explicit address in authoritative user text can be retained; a named guest with only a model-proposed address pauses creation with CALENDAR_ATTENDEE_IDENTITY_REQUIRED. Do not silently create an attendee-free event for that unresolved guest. This pause has rejected acceptance and awaits user input; unrelated model-invented guests remain excluded. Contact resolution must supply independently grounded address evidence before it can bypass clarification.
