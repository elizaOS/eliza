...
const MockCalendarService = vi.hoisted(() => Object.assign(function CalendarService() { }, {
    serviceType: "calendar",
}));
const MockCalendarMigrationService = vi.hoisted(() => Object.assign(function CalendarMigrationService() { }, {
...
describe("calendar plugin HTTP routes", () => {
    ...
    it("serves linked-event names through the resolved owner route service", async () => {
        ...
        await calendarRouteHandler()(makeRequest({
            method: "GET",
            url: "/api/lifeops/calendar/links?view=events",
        }) as never, response as never, makeRuntime(service) as never);
    ...
    it("keeps the host adapter available for the owner-gated LifeOps route", async () => {
        ...
        }) as never, res as never, runtime as never);
