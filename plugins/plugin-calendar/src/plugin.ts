...
export const calendarPlugin: Plugin = {
    name: "calendar",
    description: "Multi-provider calendar feeds, scheduling, and event management for Eliza agents.",
    ...
    init: async (_config: Record<string, string>, runtime: IAgentRuntime): Promise<void> => {
        ...
        logger.info({ src: "plugin:calendar:microsoft-oauth" }, "Registered Microsoft connector account provider");
    },
    views: [
        ...
        // runtime export reaches the bundle build.
        {
            ...
            description: "Chat-first calendar over unified Google, Microsoft, Apple, and ICS events.",
            icon: "CalendarDays",
            path: "/calendar",
            modalities: ["gui"],
            ...
            scopedActions: [
                {
                    ...
                    description: "Show the agenda for a day in the currently displayed calendar grid. Requires date in YYYY-MM-DD format. Reading or searching events does not select a day in the UI; use this action to show it. For a date outside the displayed grid, navigate the calendar month controls first.",
                    parameters: ["date"],
