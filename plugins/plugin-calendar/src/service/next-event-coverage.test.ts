...
describe("next-event read coverage", () => {
    it.each([
    ...
    ] as const)("preserves $state source health without claiming exhaustive event coverage (empty=$empty)", async ({ state, empty }) => {
        const now = new Date("2026-09-24T10:00:00.000Z");
        const events: LifeOpsCalendarEvent[] = (empty ? [] : [11, 13]).map((hour) => ({
            id: `event-${hour}`,
        ...
        const result = await service.getNextCalendarEventContext(new URL("http://localhost/"), { timeZone: "UTC" }, now);
        ...
        vi.spyOn(service, "getNextCalendarEventContext").mockResolvedValue(result);
        ...
        expect(outcome.effectReceipts?.[0].resource.kind).toBe("calendar.next_event");
        const agenda = await action.handler(runtime, {
            ...message,
