...
function event(id: string, title: string, startAt: string): LifeOpsCalendarEvent {
    return {
        id,
        title,
        startAt,
...
const dentistFriday = event("e1", "Dentist appointment", "2026-09-11T22:00:00.000Z");
const dentistSaturday = event("e2", "Dentist appointment", "2026-09-12T22:00:00.000Z");
    ...
    it("separates dates in the title from the source-day constraint", () => {
        ...
        for (const action of ["update", "delete"] as const) {
            for (const [day, expected] of [
                ...
                expect(resolveCalendarMutationCandidates({
                    action,
                    events: [target],
                    titleHint: query,
                    texts: [query],
