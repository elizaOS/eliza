...
import { getMcpJsonSchemaBudgetError } from "@elizaos/shared";
import { toolSelectionArgumentSchema } from "./schemas";
import { toolSelectionNameSchema } from "./schemas";
import { type McpProviderData } from "../types";
import { type McpServerInfo } from "../types";
import { type ResourceSelection } from "./schemas";
import { type State } from "@elizaos/core";
import { type ToolSelectionArgument } from "./schemas";
import { type ToolSelectionName } from "./schemas";
import { type ValidationResult } from "../types";
import { validateJsonSchema } from "@elizaos/shared";
...
function formatWorkerErrors(errors: unknown): string {
    ...
    return errors
        ...
        const rawPath = typeof error.instancePath === "string"
            ? error.instancePath
...
async function validateUntrustedToolArguments(data: unknown, schema: Readonly<Record<string, unknown>>): Promise<ValidationResult<unknown>> {
    const schemaBudgetError = getMcpJsonSchemaBudgetError(schema);
    if (schemaBudgetError)
        return { success: false, error: schemaBudgetError };
    ...
    try {
        return await new Promise<ValidationResult<unknown>>((resolve) => {
            ...
            worker.on("message", (message: SchemaWorkerResult & {
                ready?: boolean;
                errors?: unknown;
            }) => {
                if (message.ready) {
...
export function validateToolSelectionName(parsed: unknown, state: State): ValidationResult<ToolSelectionName> {
...
export async function validateToolSelectionArgument(parsed: unknown, toolInputSchema: Readonly<Record<string, unknown>>): Promise<ValidationResult<ToolSelectionArgument>> {
    const normalizedParsed = isRecord(parsed) &&
        typeof parsed.toolArguments === "string" &&
        ["", "{}"].includes(parsed.toolArguments.trim())
        ? { ...parsed, toolArguments: {} }
        : parsed;
    const basicResult = validateJsonSchema<ToolSelectionArgument>(normalizedParsed, toolSelectionArgumentSchema);
    if (basicResult.success === false) {
        return { success: false, error: basicResult.error };
    }
    const data = basicResult.data;
    const validationResult = await validateUntrustedToolArguments(data.toolArguments, toolInputSchema);
...
export function createToolSelectionFeedbackPrompt(originalResponse: string, errorMessage: string, composedState: State, userMessage: string): string {
...
function createFeedbackPrompt(originalResponse: string, errorMessage: string, itemType: string, itemsDescription: string, userMessage: string): string {
