...
import { resourceAnalysisTemplate } from "@elizaos/shared";
import { toolReasoningTemplate } from "@elizaos/shared";
...
export function processResourceResult(result: ResourceResult, uri: string): {
...
export function processToolResult(result: ToolResult, serverName: string, toolName: string, runtime: IAgentRuntime, messageEntityId: string): {
    toolOutput: string;
...
export async function handleResourceAnalysis(runtime: IAgentRuntime, message: Memory, uri: string, serverName: string, resourceContent: string, resourceMeta: string, callback?: HandlerCallback): Promise<void> {
...
export async function handleToolResponse(runtime: IAgentRuntime, message: Memory, serverName: string, toolName: string, toolArgs: Readonly<Record<string, unknown>>, toolOutput: string, hasAttachments: boolean, attachments: readonly Media[], state: State, mcpProvider: McpProviderArg, callback?: HandlerCallback, isError = false): Promise<Memory> {
    await createMcpMemory(runtime, message, "tool", serverName, toolOutput, {
        toolName,
        arguments: toolArgs,
        isToolCall: true,
    });
    const reasoningPrompt = createReasoningPrompt(state, mcpProvider, toolName, serverName, message.content.text ?? "", toolOutput, hasAttachments, isError);
...
function createReasoningPrompt(state: State, mcpProvider: McpProviderArg, toolName: string, serverName: string, userMessage: string, toolOutput: string, hasAttachments: boolean, toolErrored: boolean): string {
    const enhancedState: State = {
        ...state,
        values: {
            ...state.values,
