...
export class McpError extends Error {
    ...
    static connectionError(serverName: string, details?: string): McpError {
        return new McpError(`Failed to connect to server '${serverName}'${details ? `: ${details}` : ""}`, "CONNECTION_ERROR");
    }
    ...
    static resourceNotFound(uri: string, serverName: string): McpError {
        return new McpError(`Resource '${uri}' not found on server '${serverName}'`, "RESOURCE_NOT_FOUND");
    }
    ...
    static serverError(serverName: string, details?: string): McpError {
        return new McpError(`Server error from '${serverName}'${details ? `: ${details}` : ""}`, "SERVER_ERROR");
    }
}
