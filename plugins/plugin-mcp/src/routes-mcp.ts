...
import { getMcpServerDetails } from "./mcp-marketplace.js";
import { logger } from "@elizaos/core";
import { searchMcpMarketplace } from "./mcp-marketplace.js";
...
export interface McpRouteContext {
    ...
    resolveMcpTerminalAuthorizationRejection: (req: http.IncomingMessage, servers: Record<string, unknown>, body: {
...
function createRequestAbortTracker(req: http.IncomingMessage, res: http.ServerResponse, operation: string): RequestAbortTracker {
    ...
    const register = (source: AbortEventSource | null | undefined, event: string, listener: () => void) => {
...
function parseClampedInteger(value: string | null | undefined, options: ParseClampedIntegerOptions = {}): number | undefined {
...
export async function handleMcpRoutes(ctx: McpRouteContext): Promise<boolean> {
    ...
    if (method === "GET" && pathname === "/api/mcp/marketplace/search") {
        ...
        try {
            ...
            if (abortTracker.isAborted() || !canWriteMarketplaceResponse(res))
                return true;
            ...
            if (abortTracker.isAborted() || !canWriteMarketplaceResponse(res))
                return true;
            await error(res, `MCP marketplace search failed: ${err instanceof Error ? err.message : err}`, 502);
            abortTracker.markCompleted();
        }
        finally {
    ...
    if (method === "GET" && marketplaceDetailsPrefix) {
        const serverName = ctx.decodePathComponent(pathname.slice(marketplaceDetailsPrefix.length), res, "server name");
        if (serverName === null)
            return true;
        ...
        try {
            ...
            if (abortTracker.isAborted() || !canWriteMarketplaceResponse(res))
                return true;
            await error(res, `Failed to fetch server details: ${err instanceof Error ? err.message : err}`, 502);
            abortTracker.markCompleted();
        }
        finally {
            abortTracker.dispose();
    ...
    if (method === "POST" && pathname === "/api/mcp/config/server") {
        ...
        state.config.mcp.servers[serverName] = sanitized as NonNullable<NonNullable<typeof state.config.mcp>["servers"]>[string];
    ...
    if (method === "PUT" && pathname === "/api/mcp/config") {
        ...
        if (body.servers !== undefined) {
            ...
            const mcpRejection = await ctx.resolveMcpServersRejection(body.servers as Record<string, unknown>);
            if (mcpRejection) {
                error(res, mcpRejection, 400);
                return true;
            }
            const mcpTerminalRejection = ctx.resolveMcpTerminalAuthorizationRejection(req, body.servers as Record<string, unknown>, body);
            if (mcpTerminalRejection) {
                error(res, `Configuring stdio MCP servers requires terminal authorization. ${mcpTerminalRejection.reason}`, mcpTerminalRejection.status);
                return true;
            }
            const sanitized = ctx.cloneWithoutBlockedObjectKeys(body.servers);
            state.config.mcp.servers = sanitized as NonNullable<NonNullable<typeof state.config.mcp>["servers"]>;
        }
        // error-policy:J4 a config write failure is visible in logs while the in-memory update remains usable.
        try {
            ctx.saveElizaConfig(state.config);
