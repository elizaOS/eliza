...
import { createResourceSelectionFeedbackPrompt } from "../utils/validation";
import { createToolSelectionArgument } from "../utils/selection";
import { createToolSelectionName } from "../utils/selection";
import { handleMcpError } from "../utils/error";
import { handleNoToolAvailable } from "../utils/handler";
import { handleResourceAnalysis } from "../utils/processing";
import { handleToolResponse } from "../utils/processing";
import { processResourceResult } from "../utils/processing";
import { processToolResult } from "../utils/processing";
import { resourceSelectionTemplate } from "@elizaos/shared";
import { sendInitialResponse } from "../utils/processing";
...
import { validateResourceSelection } from "../utils/validation";
...
function getDirectResourceSelection(options?: unknown): ResourceSelection | null {
    ...
    return {
        serverName,
        uri,
        reasoning: typeof params.reasoning === "string" && params.reasoning.trim()
...
async function handleCallTool(runtime: IAgentRuntime, message: Memory, options: HandlerOptions | Record<string, unknown> | undefined, callback: HandlerCallback | undefined): Promise<ActionResult> {
    ...
    try {
        ...
        const { toolOutput, hasAttachments, attachments, isError } = processToolResult(result, serverName, toolName, runtime, message.entityId);
        const replyMemory = await handleToolResponse(runtime, message, serverName, toolName, toolArguments, toolOutput, hasAttachments, attachments, composedState, mcpProvider, callback, isError);
        const data = {
            actionName: "MCP",
        ...
        if (isError) {
            return {
                ...
                data,
                success: false,
                error: new McpError(`MCP tool '${serverName}/${toolName}' returned an error result: ${toolOutput}`, "TOOL_EXECUTION_ERROR"),
            };
        }
        return {
...
async function handleReadResource(runtime: IAgentRuntime, message: Memory, options: HandlerOptions | Record<string, unknown> | undefined, callback: HandlerCallback | undefined): Promise<ActionResult> {
    const composedState = await runtime.composeState(message, ["RECENT_MESSAGES", "MCP"]);
    const mcpService = runtime.getService<McpService>(MCP_SERVICE_NAME);
    if (!mcpService) {
        throw new Error("MCP service not available");
    ...
    try {
        ...
        const parsedSelection = getDirectResourceSelection(options) ??
            (await (async () => {
                ...
                return withModelRetry<ResourceSelection>({
                    ...
                    createFeedbackPromptFn: (originalResponse, errorMessage, state, userMessage) => createResourceSelectionFeedbackPrompt(typeof originalResponse === "string"
                        ? originalResponse
                        : JSON.stringify(originalResponse), errorMessage, state, userMessage),
...
export function getMcpRouteForTest(message: Memory, options?: HandlerOptions | Record<string, unknown>): McpOp | null {
...
export const mcpAction: Action = {
    ...
    routingHint: "call a tool or read a resource on a connected external MCP server -> MCP; do NOT use to invoke an agent skill -> USE_SKILL, or to run a local shell command / edit files -> BASH / FILE",
    parameters: [
        {
            name: "action",
    ...
    handler: async (runtime: IAgentRuntime, message: Memory, _state?: State, options?: HandlerOptions | Record<string, unknown>, callback?: HandlerCallback): Promise<ActionResult> => {
