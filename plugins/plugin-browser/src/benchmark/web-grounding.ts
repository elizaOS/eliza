 ...
 * (the same `ChromiumBenchmarkEngine` the MiniWoB++ real lane uses) and reading
 * each target's true screen bbox via the browser — never a hand-written bbox. As
 * in the computeruse harness the large/licensed real dataset is not vendored; a
 * grounder + these synthetic-but-real-rendered samples exercise the scorer.
 */
