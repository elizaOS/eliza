...
import { handleBrowserBridgeRoutes } from "../bridge.js";
...
function createContext(method: string, pathname: string): BrowserBridgeRouteContext & {
...
function bridgeRoutes(): Route[] {
    return (browserPlugin.routes ?? []).filter((route) => route.path.startsWith("/api/browser-bridge/"));
}
describe("browser bridge route registration", () => {
    it("registers the companion action-lease begin callback as a public write", () => {
        const route = bridgeRoutes().find((candidate) => candidate.path ===
        ...
        expect((route as {
            publicWrite?: string;
        }).publicWrite).toBeTruthy();
    });
    it("registers the authenticated companion preflight callback as a public write", () => {
        const route = bridgeRoutes().find((candidate) => candidate.path === "/api/browser-bridge/companions/preflight");
        expect(route).toBeDefined();
        expect(route?.type).toBe("POST");
        expect((route as {
            public?: boolean;
    ...
    it("resolves every advertised bridge path to a handler branch", async () => {
        ...
        const sweepable = bridgeRoutes().filter((route) => !route.path.startsWith("/api/browser-bridge/packages"));
    ...
    it("does not claim paths outside the advertised bridge surface", async () => {
        const ctx = createContext("POST", "/api/browser-bridge/companions/sessions/session-1/actions/finish");
