 ...
 */
import { assertBrowserWorkspaceCommandConnectorAccountGate } from "./workspace-account-gate.js";
import { assertBrowserWorkspaceConnectorAccountGate } from "./workspace-account-gate.js";
import { assertBrowserWorkspaceUserScriptAllowed } from "../workspace/browser-workspace-helpers.js";
import { closeBrowserWorkspaceTab } from "../workspace/index.js";
import { createBrowserWorkspaceError } from "../workspace/browser-workspace-errors.js";
import { evaluateBrowserWorkspaceTab } from "../workspace/index.js";
import { executeBrowserWorkspaceCommand } from "../workspace/index.js";
import { getBrowserWorkspaceSnapshot } from "../workspace/index.js";
import { getBrowserWorkspaceUnavailableMessage } from "../workspace/index.js";
import { hideBrowserWorkspaceTab } from "../workspace/index.js";
import { isBrowserWorkspaceBridgeConfigured } from "../workspace/index.js";
import { isBrowserWorkspaceError } from "../workspace/browser-workspace-errors.js";
import { listBrowserWorkspaceTabs } from "../workspace/index.js";
import { navigateBrowserWorkspaceTab } from "../workspace/index.js";
import { normalizeBrowserWorkspaceCommand } from "../workspace/browser-workspace-helpers.js";
import { openBrowserWorkspaceTab } from "../workspace/index.js";
import { requestBrowserWorkspace } from "../workspace/browser-workspace-desktop.js";
import { showBrowserWorkspaceTab } from "../workspace/index.js";
import { snapshotBrowserWorkspaceTab } from "../workspace/index.js";
import { type BrowserWorkspaceCommand } from "../workspace/index.js";
import { type BrowserWorkspaceErrorCode } from "../workspace/browser-workspace-errors.js";
...
function statusFromBrowserWorkspaceErrorCode(code: BrowserWorkspaceErrorCode, message: string): number {
    switch (code) {
        case "invalid_url":
...
function statusFromBrowserWorkspaceError(error: unknown, message: string): number {
    ...
    if (isBrowserWorkspaceError(error)) {
        return statusFromBrowserWorkspaceErrorCode(error.browserWorkspaceErrorCode, message);
    }
...
function connectorReferenceFromSearchParams(url: URL | undefined): BrowserWorkspaceConnectorReference {
    return {
...
function isBrowserWorkspaceRouteBodyObject(value: unknown): value is Record<string, unknown> {
    return Boolean(value && typeof value === "object" && !Array.isArray(value));
}
function validateBrowserWorkspaceCommandTree(value: unknown, path = "command", depth = 0, state = { commandCount: 0 }): string | null {
    ...
    for (let index = 0; index < value.steps.length; index += 1) {
        ...
        const nestedError = validateBrowserWorkspaceCommandTree(value.steps[index], `${path}.steps[${index}]`, depth + 1, state);
        if (nestedError) {
            return nestedError;
        }
    }
...
function validateBrowserWorkspaceBatchSteps(command: BrowserWorkspaceCommand, path = "command"): string | null {
    if (typeof command.subaction !== "string" ||
    ...
    for (let index = 0; index < command.steps.length; index += 1) {
        const nestedError = validateBrowserWorkspaceBatchSteps(command.steps[index], `${path}.steps[${index}]`);
        if (nestedError) {
            return nestedError;
        }
    }
...
function rejectMalformedBrowserWorkspacePayload(ctx: BrowserWorkspaceRouteContext): true {
...
async function assertBrowserWorkspaceTabConnectorAccountGate(ctx: BrowserWorkspaceRouteContext, tabId: string, reference: BrowserWorkspaceConnectorReference, operation: string): Promise<void> {
...
export async function handleBrowserWorkspaceRoutes(ctx: BrowserWorkspaceRouteContext): Promise<boolean> {
    const { req, res, method, pathname, readJsonBody, json } = ctx;
    if (pathname !== "/api/browser-workspace" &&
    ...
    try {
        ...
        if (pathname === "/api/browser-workspace/events" && method === "GET") {
            if (!isBrowserWorkspaceBridgeConfigured()) {
                throw createBrowserWorkspaceError("desktop_only", "events", getBrowserWorkspaceUnavailableMessage());
            }
            json(res, await requestBrowserWorkspace<BrowserWorkspaceEventLogSnapshot>(buildBrowserWorkspaceEventsBridgePath(ctx.url)));
            return true;
        }
        if (pathname === "/api/browser-workspace/command" && method === "POST") {
            const body = (await readJsonBody<BrowserWorkspaceCommandBody>(req, res)) ?? null;
        ...
        if (pathname === "/api/browser-workspace/tabs" && method === "POST") {
            const body = (await readJsonBody<OpenBrowserWorkspaceBody>(req, res)) ?? null;
            if (!isBrowserWorkspaceRouteBodyObject(body)) {
                return rejectMalformedBrowserWorkspacePayload(ctx);
            }
        ...
        const match = pathname.match(/^\/api\/browser-workspace\/tabs\/([^/]+)(?:\/(navigate|eval|show|hide|snapshot))?$/);
        if (!match) {
            return false;
        }
        ...
        if (!action && method === "DELETE") {
            await assertBrowserWorkspaceTabConnectorAccountGate(ctx, tabId, connectorReferenceFromSearchParams(ctx.url), "close browser workspace tab");
        ...
        if (action === "show" && method === "POST") {
            await assertBrowserWorkspaceTabConnectorAccountGate(ctx, tabId, connectorReferenceFromSearchParams(ctx.url), "show browser workspace tab");
            json(res, { tab: await showBrowserWorkspaceTab(tabId) });
            return true;
        }
        if (action === "hide" && method === "POST") {
            await assertBrowserWorkspaceTabConnectorAccountGate(ctx, tabId, connectorReferenceFromSearchParams(ctx.url), "hide browser workspace tab");
            json(res, { tab: await hideBrowserWorkspaceTab(tabId) });
            return true;
        }
        if (action === "snapshot" && method === "GET") {
            await assertBrowserWorkspaceTabConnectorAccountGate(ctx, tabId, connectorReferenceFromSearchParams(ctx.url), "snapshot browser workspace tab");
        ...
        if (action === "navigate" && method === "POST") {
            ...
            await assertBrowserWorkspaceTabConnectorAccountGate(ctx, tabId, body, "navigate browser workspace tab");
            json(res, {
        ...
        if (action === "eval" && method === "POST") {
            ...
            await assertBrowserWorkspaceTabConnectorAccountGate(ctx, tabId, body, "evaluate browser workspace tab");
