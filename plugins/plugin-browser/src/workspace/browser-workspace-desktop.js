import { assertBrowserWorkspaceConnectorSecretsNotExported, createBrowserWorkspaceCommandTargetError, DEFAULT_TIMEOUT_MS, normalizeEnvValue, resolveBrowserWorkspaceCommandElementRefs, } from "./browser-workspace-helpers.js";
import { appendBrowserWorkspaceProfilerEntry, appendBrowserWorkspaceTraceEntry, getBrowserWorkspaceRuntimeState, registerBrowserWorkspaceElementRefs, } from "./browser-workspace-state.js";
...
export async function requestBrowserWorkspace(path, init, env = process.env) {
...
export async function evaluateBrowserWorkspaceTab(request, env = process.env) {
    if (!isBrowserWorkspaceBridgeConfigured(env)) {
        throw new Error("Eliza browser workspace eval is only available in the desktop app.");
    }
...
export async function snapshotBrowserWorkspaceTab(id, env = process.env) {
    if (!isBrowserWorkspaceBridgeConfigured(env)) {
        throw new Error("Eliza browser workspace snapshot is only available in the desktop app.");
    }
    return await requestBrowserWorkspace(`/tabs/${encodeURIComponent(id)}/snapshot`, undefined, env);
}
export function createDesktopBrowserWorkspaceCommandScript(command) {
    return `
(() => {
...
export function createDesktopBrowserWorkspaceUtilityScript(command) {
    return `
(() => {
  const command = ${JSON.stringify(command)};
  const normalize = (value) => String(value ?? "").replace(/\\s+/g, " ").trim();
...
export async function executeDesktopBrowserWorkspaceUtilityCommand(command, env) {
    const id = await resolveDesktopBrowserWorkspaceTargetTabId(command, env);
    if (command.subaction === "cookies" ||
        command.subaction === "storage" ||
        (command.subaction === "set" &&
            (command.setAction === "credentials" || command.setAction === "headers"))) {
        await assertDesktopBrowserWorkspaceCanAccessProfileSecrets(id, env, command.subaction);
    }
...
export async function getDesktopBrowserWorkspaceSnapshotRecord(command, env) {
    const id = await resolveDesktopBrowserWorkspaceTargetTabId(command, env);
    const result = await evaluateBrowserWorkspaceTab({
        id,
        script: `
...
export async function getDesktopBrowserWorkspaceSessionState(command, env) {
    const id = await resolveDesktopBrowserWorkspaceTargetTabId(command, env);
...
export async function loadDesktopBrowserWorkspaceSessionState(command, payload, env) {
    const id = await resolveDesktopBrowserWorkspaceTargetTabId(command, env);
    await assertDesktopBrowserWorkspaceCanAccessProfileSecrets(id, env, "state");
...
export async function executeDesktopBrowserWorkspaceDomCommand(command, env) {
    const id = await resolveDesktopBrowserWorkspaceTargetTabId(command, env);
    const startedAt = Date.now();
...
export function resolveBrowserWorkspaceCurrentTab(tabs) {
    if (tabs.length === 0) {
        return null;
    }
...
export async function resolveDesktopBrowserWorkspaceTargetTabId(command, env) {
    if (command.id?.trim()) {
