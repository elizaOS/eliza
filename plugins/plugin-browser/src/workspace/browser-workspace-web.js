...
import { browserWorkspaceTextMatches, buildBrowserWorkspaceElementSelector, collectBrowserWorkspaceInspectElements, createBrowserWorkspaceElementSummary, findClosestBrowserWorkspaceForm, getBrowserWorkspaceElementBox, getBrowserWorkspaceElementStyles, getBrowserWorkspaceElementValue, isBrowserWorkspaceElementVisible, mergeBrowserWorkspaceSelectorCommand, queryAllBrowserWorkspaceSelector, resolveBrowserWorkspaceElement, resolveBrowserWorkspaceFindElement, resolveWebBrowserWorkspaceCommandDocument, } from "./browser-workspace-elements.js";
import { activateWebBrowserWorkspaceElement, clearWebBrowserWorkspaceTabElementRefs, cloneWebBrowserWorkspaceTabState, ensureBrowserWorkspaceCheckboxElement, ensureBrowserWorkspaceFormControlElement, ensureLoadedWebBrowserWorkspaceTabDocument, scrollWebBrowserWorkspaceTarget, setBrowserWorkspaceControlValue, submitWebBrowserWorkspaceForm, } from "./browser-workspace-forms.js";
import { assertBrowserWorkspaceConnectorSecretsNotExported, createBrowserWorkspaceCommandTargetError, createBrowserWorkspaceNotFoundError, DEFAULT_TIMEOUT_MS, DEFAULT_WAIT_INTERVAL_MS, normalizeBrowserWorkspaceText, resolveBrowserWorkspaceCommandElementRefs, sleep, writeBrowserWorkspaceFile, } from "./browser-workspace-helpers.js";
import { applyBrowserWorkspaceDomSettings, ensureBrowserWorkspaceDom, } from "./browser-workspace-jsdom.js";
import { fetchBrowserWorkspaceTrackedResponse, normalizeBrowserWorkspaceHeaders, } from "./browser-workspace-network.js";
import { applyBrowserWorkspaceStateToWebDocument, buildBrowserWorkspaceDocumentSnapshotText, createBrowserWorkspacePdfBuffer, createBrowserWorkspaceSnapshotRecord, createBrowserWorkspaceSyntheticScreenshotData, diffBrowserWorkspaceSnapshots, readBrowserWorkspaceCookies, readBrowserWorkspaceStorage, } from "./browser-workspace-snapshots.js";
import { browserWorkspaceClipboardText, getBrowserWorkspaceRuntimeState, getBrowserWorkspaceTimestamp, registerBrowserWorkspaceElementRefs, setBrowserWorkspaceClipboardText, webWorkspaceState, withWebStateLock, } from "./browser-workspace-state.js";
export function getWebBrowserWorkspaceTabIndex(tabId) {
    return webWorkspaceState.tabs.findIndex((tab) => tab.id === tabId);
}
export function getWebBrowserWorkspaceTabState(tabId) {
...
export function getCurrentWebBrowserWorkspaceTabState() {
...
export function findWebBrowserWorkspaceTargetTabId(command) {
    if (command.id?.trim()) {
...
export async function executeWebBrowserWorkspaceUtilityCommand(command) {
        ...
        switch (command.subaction) {
            ...
            case "clipboard": {
                ...
                if (action === "copy") {
                    ...
                    setBrowserWorkspaceClipboardText(target && "value" in target
                        ? String(target.value ?? "")
            ...
            case "set": {
                ...
                if (action === "credentials" || action === "headers") {
                    assertBrowserWorkspaceConnectorSecretsNotExported(tab.partition, `set:${action}`);
                }
                if (action === "viewport") {
            ...
            case "cookies": {
                assertBrowserWorkspaceConnectorSecretsNotExported(tab.partition, "cookies");
            ...
            case "storage": {
                assertBrowserWorkspaceConnectorSecretsNotExported(tab.partition, "storage");
            ...
            case "state": {
                assertBrowserWorkspaceConnectorSecretsNotExported(tab.partition, "state");
                if (command.stateAction === "load") {
                    ...
                    setBrowserWorkspaceClipboardText(typeof payload.clipboard === "string"
                        ? payload.clipboard
...
export async function executeWebBrowserWorkspaceDomCommand(command) {
    return withWebStateLock(async () => {
        const id = findWebBrowserWorkspaceTargetTabId(command);
