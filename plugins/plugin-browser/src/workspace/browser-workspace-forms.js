import { buildBrowserWorkspaceElementSelector, findClosestBrowserWorkspaceForm, } from "./browser-workspace-elements.js";
import { assertBrowserWorkspaceUrl, inferBrowserWorkspaceTitle, normalizeBrowserWorkspaceText, } from "./browser-workspace-helpers.js";
import { ensureBrowserWorkspaceDom, getJSDOMClass, installBrowserWorkspaceWebRuntime, } from "./browser-workspace-jsdom.js";
import { fetchBrowserWorkspaceTrackedResponse } from "./browser-workspace-network.js";
import { getBrowserWorkspaceRuntimeState, getBrowserWorkspaceTimestamp, resetBrowserWorkspaceRuntimeNavigationState, } from "./browser-workspace-state.js";
export function ensureBrowserWorkspaceFormControlElement(element, subaction) {
...
export function ensureBrowserWorkspaceCheckboxElement(element, subaction) {
    if (element.tagName === "INPUT") {
        const input = element;
...
export function setBrowserWorkspaceControlValue(control, nextValue) {
...
export async function activateWebBrowserWorkspaceElement(tab, element, subaction) {
...
export function scrollWebBrowserWorkspaceTarget(dom, element, direction, pixels) {
...
export async function submitWebBrowserWorkspaceForm(tab, form) {
...
export function clearWebBrowserWorkspaceTabElementRefs(tabId) {
    clearBrowserWorkspaceElementRefs("web", tabId);
}
export function cloneWebBrowserWorkspaceTabState(tab) {
    return {
        id: tab.id,
        title: tab.title,
        url: tab.url,
...
export function pushWebBrowserWorkspaceHistory(tab, nextUrl) {
...
export async function loadWebBrowserWorkspaceTabDocument(tab) {
    const state = getBrowserWorkspaceRuntimeState("web", tab.id);
...
export async function ensureLoadedWebBrowserWorkspaceTabDocument(tab) {
