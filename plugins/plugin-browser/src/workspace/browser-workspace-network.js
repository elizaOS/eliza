import { browserWorkspacePageFetch, DEFAULT_TIMEOUT_MS, } from "./browser-workspace-helpers.js";
import { getBrowserWorkspaceTimestamp } from "./browser-workspace-state.js";
export function browserWorkspacePatternMatches(pattern, value) {
...
export function normalizeBrowserWorkspaceHeaders(headers) {
    if (!headers) {
        return {};
    }
...
export function findBrowserWorkspaceNetworkRoute(state, url) {
    return ([...state.networkRoutes]
        .reverse()
        .find((route) => browserWorkspacePatternMatches(route.pattern, url)) ??
        null);
}
export function recordBrowserWorkspaceNetworkRequest(state, request) {
    const entry = {
        ...request,
...
export async function fetchBrowserWorkspaceTrackedResponse(state, url, init = {}, resourceType) {
