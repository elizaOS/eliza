...
export function getBrowserCaptureExecutablePath() {
    return CHROME_PATH;
}
export function isBrowserCaptureSupported() {
...
export async function startBrowserCapture(config) {
    ...
    if (!isBrowserCaptureSupported()) {
        throw new Error(`Google Chrome not found at ${CHROME_PATH}. Install Chrome or update browser-capture before enabling screen capture.`);
    }
    ...
    activeCaptureLoop = (async () => {
        while (!stopSignal) {
            try {
                ...
                if (!stopSignal) {
                    logger.warn(`[browser-capture] frame capture failed: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            if (!stopSignal) {
    ...
    logger.info(`[browser-capture] Screenshot loop active (${fps} fps), saving to ${FRAME_FILE}`);
}
