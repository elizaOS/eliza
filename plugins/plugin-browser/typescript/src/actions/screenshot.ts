  ...
  handler: async (
    ...
    try {
      ...
      if (!service) {
        ...
        handleBrowserError(error, callback as (content: { text: string; error?: boolean }) => Promise<unknown>, 'take screenshot');
        return {
      ...
      logger.error(`Error in BROWSER_SCREENSHOT action: ${errorMessage}`);

      const browserError = caughtError instanceof ActionError
        ? caughtError
        : new ActionError('screenshot', 'page', caughtError instanceof Error ? caughtError : undefined);
