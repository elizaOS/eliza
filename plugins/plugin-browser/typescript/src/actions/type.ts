  ...
  handler: async (
    ...
    try {
      ...
      if (!service) {
        ...
        handleBrowserError(error, callback as (content: { text: string; error?: boolean }) => Promise<unknown>, 'type text');
        return {
      ...
      logger.error(`Error in BROWSER_TYPE action: ${errorMessage}`);

      const browserError = caughtError instanceof ActionError
        ? caughtError
        : new ActionError('type', 'input field', caughtError instanceof Error ? caughtError : undefined);
