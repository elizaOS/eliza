  ...
  handler: async (
    ...
    try {
      ...
      if (!service) {
        ...
        handleBrowserError(error, callback as (content: { text: string; error?: boolean }) => Promise<unknown>, 'select option');
        return {
      ...
      logger.error(`Error in BROWSER_SELECT action: ${errorMessage}`);

      const browserError = caughtError instanceof ActionError
        ? caughtError
        : new ActionError('select', 'dropdown', caughtError instanceof Error ? caughtError : undefined);
