  ...
  handler: async (
    ...
    try {
      ...
      if (!service) {
        ...
        handleBrowserError(error, callback as (content: { text: string; error?: boolean }) => Promise<unknown>, 'click on element');
        return {
      ...
      logger.error(`Error in BROWSER_CLICK action: ${errorMessage}`);

      const browserError = caughtError instanceof ActionError
        ? caughtError
        : new ActionError('click', 'element', caughtError instanceof Error ? caughtError : undefined);
