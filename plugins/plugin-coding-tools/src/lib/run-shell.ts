...
import { applyHostExecutionBaseline } from "@elizaos/core/host-execution-env";
import { createWriteStream } from "node:fs";
import { detectTerminalSupport } from "./terminal-capabilities.js";
import { execFileSync } from "node:child_process";
import { existsSync } from "node:fs";
import { getCapabilityRouter } from "@elizaos/core";
import { missingToolForCommand } from "./terminal-capabilities.js";
import { missingToolMessage } from "./terminal-capabilities.js";
import { mkdtempSync } from "node:fs";
import { normalizeWorkspaceDeltaReceipt } from "@elizaos/core";
import { resolveHostExecutable } from "@elizaos/core/host-execution-env";
import { resolveHostShell } from "./terminal-capabilities.js";
...
import { type ShellCaptureProjection } from "./shell-streaming-capture.js";
import { type ShellOutputArtifact } from "./shell-output-artifact.js";
import { type WorkspaceDeltaReceipt } from "@elizaos/core";
import { type WriteStream } from "node:fs";
import * as importPath from "node:path";
import process from "node:process";
export type ShellSandboxBackend = "host" | "capability-router" | "docker" | "apple-container" | "wsl2" | "appcontainer" | "none";
...
function hasValidUpstreamCaptureAttestation(result: {
    ...
    return Boolean(capture?.complete === true &&
...
async function finalizeAttestedUpstreamResult(runtime: IAgentRuntime, opts: RunShellOptions, result: ShellResult): Promise<ShellResult> {
    if (!opts.captureScope) {
        throw new Error("foreground shell artifact capture requires an owner scope");
    }
...
function getRuntimeSandboxManager(runtime: IAgentRuntime): RuntimeSandboxManager | null {
    const candidate = (runtime as {
        getSandboxManager?: () => RuntimeSandboxManager | null;
    }).getSandboxManager?.();
    return candidate ?? null;
}
function backendForManager(manager: RuntimeSandboxManager): Exclude<ShellSandboxBackend, "host" | "capability-router"> {
    ...
    if (engineType === "docker")
        return "docker";
    if (engineType === "apple-container")
        return "apple-container";
    return "none";
}
function toSandboxWorkdir(cwd: string): string | undefined {
    const root = process.cwd();
    const relative = importPath.relative(importPath.resolve(root), importPath.resolve(cwd));
    if (relative === "")
        return "/workspace";
...
function killHostProcess(pid: number | undefined, signal: NodeJS.Signals, useProcessGroup: boolean, proc: HostShellProcess): void {
    try {
...
interface BunHostRuntime {
    spawn(options: {
        ...
        stderr: "pipe";
        detached: boolean;
        onExit: (proc: BunHostSubprocess, code: number, signal: NodeJS.Signals | null, error?: Error) => void;
    }): BunHostSubprocess;
}
...
function startBunHostProcess(opts: {
    ...
    const stdinFifo = opts.stdin === "pipe" && process.platform !== "win32"
        ? createStdinFifo()
        : null;
...
function runOnHost(runtime: IAgentRuntime, opts: RunShellOptions): Promise<ShellResult> {
    return runOnHostWithShell(runtime, opts, resolveHostShell()).then(async (result) => {
...
function assertHostBackgroundSupported(runtime: IAgentRuntime, command: string, cwd: string): void {
    if (getCapabilityRouter(runtime)) {
        throw new Error("Background shell sessions are not supported by the capability-router backend.");
    }
    ...
    if (mode === "local-safe") {
        throw new Error("Background shell sessions require a managed sandbox backend with session support; this runtime only exposes one-shot sandbox exec.");
    }
    const support = detectTerminalSupport();
    if (!support.supported) {
        throw new Error(support.message ?? "Local terminal execution is unavailable.");
    }
...
export function startBackgroundShellOnHost(runtime: IAgentRuntime, opts: {
    command: string;
    cwd: string;
    env?: NodeJS.ProcessEnv;
...
export function signalHostProcessGroup(proc: HostShellProcess, signal: NodeJS.Signals): void {
    killHostProcess(proc.pid, signal, process.platform !== "win32", proc);
}
function resolveExecutableForHost(name: string, fallback: string): string | undefined {
    return resolveHostExecutable(name) ?? resolveHostExecutable(fallback);
}
async function runOnHostWithShell(runtime: IAgentRuntime, opts: RunShellOptions, shell: ReturnType<typeof resolveHostShell>): Promise<ShellResult> {
    ...
    return new Promise<ShellResult>((resolve, reject) => {
        if (!shell.available) {
            void capture.abort().finally(() => resolve({
                exitCode: -1,
                signal: null,
                stdout: "",
        ...
        proc.on("close", (code, signal) => {
            ...
            void capture
                ...
                .then(({ artifact, projection }) => resolve({
                exitCode,
                signal,
...
async function runThroughCapabilityRouter(runtime: IAgentRuntime, opts: RunShellOptions): Promise<ShellResult | null> {
    ...
    try {
        ...
        if (workspaceDeltaReceipt &&
            ...
            throw new Error("routed workspace receipt does not match its attested execution scope");
        }
 ...
 */
export async function runShell(runtime: IAgentRuntime, opts: RunShellOptions): Promise<ShellResult> {
    const mode = resolveRuntimeExecutionMode(runtime);
    ...
    if (routed) {
        ...
        return {
            ...
            sourceLoss: {
                ...
                message: "capability-router returned a full string without a streaming completeness contract",
            },
        };
    }
    if (mode === "cloud") {
    ...
    if (mode === "local-safe") {
        ...
        if (!manager) {
            throw new Error("local-safe mode requires SandboxManager, but no sandbox manager is available for command execution.");
        }
        const sandboxWorkdir = toSandboxWorkdir(opts.cwd);
        if (!sandboxWorkdir) {
            throw new Error(`local-safe mode can only execute inside the sandbox workspace; cwd is outside process workspace: ${opts.cwd}`);
        }
        ...
        return {
            ...
            sourceLoss: {
                ...
                message: "sandbox manager returned full strings without a streaming completeness contract",
            },
        };
    }
