...
function runtimeWithRouter(router: ElizaCapabilityRouter): IAgentRuntime {
    return {
        getService: (serviceType: string) => serviceType === CAPABILITY_ROUTER_SERVICE_TYPE ? router : null,
    } as IAgentRuntime;
}
...
describe("plugin-coding-tools runShell mobile routing", () => {
    ...
    it("rejects iOS coding commands when no Remote capability router is available", async () => {
        process.env.ELIZA_PLATFORM = "ios";
        process.env.ELIZA_RUNTIME_MODE = "local-yolo";
        await expect(runShell(hostRuntime(), {
            command: "codex exec 'touch changed.txt'",
            cwd: "/workspace",
            timeoutMs: 10000,
            captureScope: TEST_CAPTURE_SCOPE,
        })).rejects.toThrow("Local coding tools are unavailable on iOS because the runtime does not expose shell, coding, or orchestrator subprocess capabilities.");
    });
});
