/**
 * MLX text generation for ElizaOS.
 *
 * ## Why this module looks like Ollama's / LM Studio's, not OpenAI's
 *
 * `mlx-lm.server` is an OpenAI-compatible local server, so the bytes-on-the-wire
 * are very similar to `plugin-openai`. But the orchestration concerns are closer
 * to `plugin-ollama` and `plugin-lmstudio`:
 *
 * - Models are picked up from what the user has loaded locally — there is no
 *   canonical "gpt-4o-class" identifier. Callers either override per-tier env
 *   vars (`MLX_SMALL_MODEL`, `MLX_LARGE_MODEL`) or we fall back to the first id
 *   returned by `GET /v1/models`.
 * - There's no need for OpenAI's reasoning / image / Cerebras branches.
 *
 * So we mirror the same structure (single `handleTextWithModelType`, structured
 * output via `Output.object`, optional streaming) but build on
 * `@ai-sdk/openai-compatible`.
 *
 * ## Errors
 *
 * `AI_SDK` errors carry `responseBody` / `statusCode` / `url`.
 * `summarizeAiSdkError` surfaces these so operators get mlx-lm's actual
 * response body in logs instead of a generic "Internal Server Error" — local
 * servers commonly fail with "no model loaded" or OOM, both of which are only
 * visible in the body.
 */
...
import type { MlxModelInfo } from "../types";
...
/**
 * `GenerateTextParams` widened with the fields ElizaOS core sends alongside the
 * documented prompt parameters — messages, tools, toolChoice and an optional
 * response schema. Typed loosely (matching plugin-ollama / plugin-lmstudio)
 * because callers cross several runtime versions.
 */
 ...
 *
 * 1. `MLX_<TIER>_MODEL` env (e.g. `MLX_SMALL_MODEL`).
 * 2. Generic `<TIER>_MODEL` env fallback.
 * 3. First model returned by `GET /v1/models` — mlx-lm always returns at least
 *    the currently-loaded model, so this gives a useful default without
 *    per-install config.
 *
 * The detection result is cached per runtime so we don't hit `/v1/models` on
 * every call.
 */
async function resolveModelForType(
  ...
  if (!pending) {
    pending = (async (): Promise<string | null> => {
      ...
      const first: MlxModelInfo = result.models[0] as MlxModelInfo;
      return first.id;
    })();
  ...
  throw new Error(
    "[MLX] No model configured and `GET /v1/models` returned no entries. Set MLX_SMALL_MODEL / MLX_LARGE_MODEL or start mlx_lm.server with a default model."
  );
}
...
function summarizeAiSdkError(error: unknown, depth = 0): Record<string, unknown> {
  ...
  if (typeof e.responseBody === "string") out.mlxResponseBody = e.responseBody;
...
function logTextFailure(
  ...
  logger.error(
    ...
    `[MLX] ${phase} failed (${modelType}, model=${modelId}).`
  );
}
...
export function normalizeNativeTools(tools: unknown): ToolSet | undefined {
  ...
  for (const rawTool of tools) {
    ...
    if (!name) {
      throw new Error("[MLX] Native tool definition is missing a name.");
    }
...
async function handleTextWithModelType(
  ...
  try {
    ...
    logger.log(`[MLX] Using ${modelType} model: ${model}`);
    ...
    if (tools && outputSpec) {
      logger.debug(
        "[MLX] tools and responseSchema both present — omitting structured output for this call."
      );
    ...
    const result = await generateText(baseArgs as Parameters<typeof generateText>[0]);
    const usage =
