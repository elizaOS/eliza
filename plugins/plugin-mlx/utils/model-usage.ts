/**
 * Token usage normalization and `MODEL_USED` event emission for mlx-lm calls.
 *
 * `mlx-lm.server`'s OpenAI-compatible API returns usage with the same field
 * names as OpenAI (`prompt_tokens`, `completion_tokens`, `total_tokens`), while
 * the AI SDK normalizes them to `inputTokens` / `outputTokens` / `totalTokens`
 * before handing them to us. We accept either shape and fall back to a
 * length-based estimate when a model omits usage.
 */
