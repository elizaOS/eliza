...
/** Effective account leverage = total notional / account value. */
function effectiveLeverage(
  summary: HyperliquidAccountSummary | null,
): number | null {
  if (!summary) return null;
  const notional = toNumber(summary.totalNotionalPosition);
  const accountValue = toNumber(summary.accountValue);
  if (notional === null || accountValue === null || accountValue <= 0) {
    return null;
  }
  return notional / accountValue;
}

/**
 * Distance to liquidation as a percent of the mark price. For a long, liq sits
 * below mark, so distance = (mark - liq) / mark; for a short it's inverted.
 * Bigger = safer. Null when mark/liq are missing. The sign of the position is
 * read from `size` (Hyperliquid `szi`: negative = short).
 */
function distanceToLiquidationPct(
  position: HyperliquidPosition,
): number | null {
  const mark = position.entryPx ? Number(position.entryPx) : null;
  const liq = position.liquidationPx ? Number(position.liquidationPx) : null;
  ...
  const isLong = size >= 0;
  const distance = isLong ? (mark - liq) / mark : (liq - mark) / mark;
  return distance * 100;
}
...
function AccountHealthStrip({
  ...
  const leverage = effectiveLeverage(summary);
...
function PositionRow({ position }: { position: HyperliquidPosition }) {
  ...
  const liqDistance = distanceToLiquidationPct(position);
