        ...
        if structured_data:
            ...
            try:
                ...
                if parsed.get("teamKey"):
                    ...
                    team = next(
                        (t for t in teams if t["key"].lower() == parsed["teamKey"].lower()),
                        None
                    )
                    if team:
                ...
                if parsed.get("labels") and isinstance(parsed["labels"], list):
                    ...
                    for label_name in parsed["labels"]:
                        if label_name:
                            label = next(
                                (l for l in labels if l["name"].lower() == label_name.lower()),
                                None
                            )
                            if label:
...
create_issue_action = create_action(
    ...
    examples=[
        [
            ActionExample(
                name="User",
                content={"text": "Create a new issue: Fix login button not working on mobile devices"},
            ),
            ActionExample(
                name="Assistant",
                content={"text": "I'll create that issue for you in Linear.", "actions": ["CREATE_LINEAR_ISSUE"]},
            ),
        ],
        [
            ActionExample(
            ...
            ActionExample(
                name="Assistant",
                content={"text": "I'll create a bug report for the engineering team.", "actions": ["CREATE_LINEAR_ISSUE"]},
            ),
        ],
    ],
    validate=validate,
