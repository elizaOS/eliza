                ...
                try:
                    ...
                    if parsed.get("issueId"):
                        ...
                        if not issues:
                            ...
                            if callback:
                                await callback({"text": error_msg, "source": message.get("content", {}).get("source")})
                        ...
                        if len(issues) == 1:
                            ...
                            clarify_msg = f"Found multiple issues matching \"{parsed['issueDescription']}\":\n" + "\n".join(issue_list) + "\n\nPlease specify by ID."
                            if callback:
                            ...
                            return {
                                ...
                                "data": {
                                    ...
                                    "issues": [{"id": i["id"], "identifier": i["identifier"], "title": i["title"]} for i in issues],
...
create_comment_action = create_action(
    ...
    examples=[
        [
            ActionExample(name="User", content={"text": "Comment on ENG-123: This looks good to me"}),
            ActionExample(name="Assistant", content={"text": "I'll add your comment to issue ENG-123.", "actions": ["CREATE_LINEAR_COMMENT"]}),
        ],
    ],
    validate=validate,
    handler=handler,
