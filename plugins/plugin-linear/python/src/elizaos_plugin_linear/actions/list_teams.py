...
async def handler(
    ...
    try:
        ...
        if not teams:
            no_teams_msg = (
                f'No team found matching "{specific_team}".' if specific_team
                else f'No teams found matching "{name_filter}".' if name_filter
                else "No teams found in Linear."
            )
            if callback:
                await callback({"text": no_teams_msg, "source": message.get("content", {}).get("source")})
        ...
        header_text = (
            "📋 Team Details:" if specific_team and len(teams) == 1
            else f'📋 Found {len(teams)} team{"s" if len(teams) != 1 else ""} matching "{name_filter}":' if name_filter
        ...
        return {
            ...
            "data": {
                "teams": [
                    {"id": t["id"], "name": t["name"], "key": t["key"], "description": t.get("description")}
                    for t in teams
                ],
                "count": len(teams),
...
list_teams_action = create_action(
    ...
    examples=[
        [
            ...
            ActionExample(name="Assistant", content={"text": "I'll list all the teams in Linear.", "actions": ["LIST_LINEAR_TEAMS"]}),
        ],
    ],
    validate=validate,
    handler=handler,
