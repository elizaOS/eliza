...
```text
plugins/plugin-native-bun-runtime/engine/artifacts/ElizaBunEngine.xcframework
```
...
`plugins/plugin-native-bun-runtime/engine/artifacts/` before CocoaPods runs. The Podspec only
...
bun run --cwd plugins/plugin-native-bun-runtime engine:verify:app-store -- --target=device

# Verify a signed .app bundle and its embedded ElizaBunEngine framework.
bun run --cwd plugins/plugin-native-bun-runtime engine:verify:app-store -- --app=/path/to/App.app
```
...
bun run --cwd plugins/plugin-native-bun-runtime engine:check

# Build the simulator engine from a fork checkout.
ELIZA_BUN_IOS_SOURCE_DIR=/path/to/elizaos-bun \
  bun run --cwd plugins/plugin-native-bun-runtime engine:build:sim
...
`plugins/plugin-native-bun-runtime/engine/vendor/bun` or `ELIZA_BUN_IOS_SOURCE_DIR`. The public
