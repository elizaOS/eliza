...
export class MeteoraLpService extends Service {
  ...
  public async getPools(
    ...
    try {
      ...
      console.error(
        "[MeteoraLpService] Error fetching pools from Meteora API:",
        error,
      );
      return [];
    }
  ...
  public async addLiquidity(params: {
    ...
    try {
      ...
      const totalLiquidity = position.positionData.positionBinData.reduce(
        (acc: anchor.BN, bin: { binLiquidity: string }) =>
          acc.add(new BN(bin.binLiquidity)),
        new BN(0),
      );
  ...
  public async removeLiquidity(params: {
    ...
    try {
      ...
      if (userPositions.length === 0) {
        throw new Error(
          "No positions found for this user in the specified pool.",
        );
      }

      const poolInfo = (await this.getPools()).find(
      ...
      const preBalanceA = await this.getTokenBalance(
        params.userVault.publicKey,
        tokenAMint,
      );
      const preBalanceB = await this.getTokenBalance(
        params.userVault.publicKey,
        tokenBMint,
      );
      ...
      const txs = Array.isArray(removeLiquidityTx)
        ? removeLiquidityTx
        : [removeLiquidityTx];

      let lastSignature = "";
      for (const tx of txs) {
        lastSignature = await sendTransaction(
          this.connection,
          tx.instructions,
          params.userVault,
        );
      ...
      const postBalanceA = await this.getTokenBalance(
        params.userVault.publicKey,
        tokenAMint,
      );
      const postBalanceB = await this.getTokenBalance(
        params.userVault.publicKey,
        tokenBMint,
      );
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const { userPositions } =
            await dlmmPool.getPositionsByUserAndLbPair(userPubKey);
          if (userPositions.length > 0) {
  ...
  public async getMarketDataForPools(
    ...
    try {
      ...
      console.error(
        "[MeteoraLpService] Error getting market data for pools:",
        error,
      );
      return {};
    }
  ...
  private async getTokenBalance(
    ...
    try {
      ...
      if (tokenAccounts.value.length > 0) {
        const balance =
          tokenAccounts.value[0].account.data.parsed.info.uiAmount;
        const decimals =
