  ...
  tests: [
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        assert(
          ...
          "Pool should have tokenA and tokenB",
        );
        assert(
          typeof pool.tokenA.mint === "string",
          "Token should have mint address",
        );
        assert(
          typeof pool.tokenA.symbol === "string",
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const filteredPools = await meteoraService.getPools(
          solUsdcMint,
          usdcMint,
        );
        ...
        assert(
          Array.isArray(filteredPools),
          "Filtered search should return array",
        );
        ...
        if (filteredPools.length > 0) {
          ...
          const hasCorrectPair =
            (pool.tokenA.mint === solUsdcMint &&
              pool.tokenB.mint === usdcMint) ||
            (pool.tokenA.mint === usdcMint && pool.tokenB.mint === solUsdcMint);
          assert(
            hasCorrectPair,
            "Filtered pool should match requested token pair",
          );
        }
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        assert(
          ...
          "Market data should be an object",
        );
        ...
        assert(
          foundData,
          "Should have found market data for at least one pool",
        );
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        assert(
          typeof meteoraService.stop === "function",
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const positionDetails = await meteoraService.getLpPositionDetails(
          randomUser,
          poolId,
        );
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const marketData = await meteoraService.getMarketDataForPools([
          testPool.id,
        ]);
        assert(
          marketData[testPool.id] !== undefined,
          "Should get market data for test pool",
        );
        ...
        console.log(
          `✅ Full workflow test passed with ${pools.length} pools discovered`,
        );
        console.log(
          `Test pool: ${testPool.tokenA.symbol}-${testPool.tokenB.symbol}`,
        );
      },
    },
  ],
