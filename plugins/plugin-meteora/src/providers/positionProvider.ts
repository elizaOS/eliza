...
export const meteoraPositionProvider: Provider = {
  ...
  description:
    "Provides Meteora LP position information for the current wallet",
  get: async (runtime: IAgentRuntime, message: Memory, state?: State) => {
    ...
    try {
      const { address: ownerAddress, connection } = await loadWallet(
        runtime,
        false,
      );
      if (!ownerAddress) {
...
const fetchPositions = async (
  ...
  try {
    ...
    for (const poolAddress of POOL_ADDRESSES) {
      ...
      const { userPositions } =
        await dlmmPool.getPositionsByUserAndLbPair(ownerAddress);
      ...
      for (const position of userPositions) {
        ...
        const binRange =
          Math.max(...positionBinIds) - Math.min(...positionBinIds);

        const centerBinId = Math.floor(
          (Math.max(...positionBinIds) + Math.min(...positionBinIds)) / 2,
        );

        const distanceFromActiveBinBps =
          Math.abs(centerBinId - activeBin.binId) * 100;
