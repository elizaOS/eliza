...
class SignalApiClient {
	...
	private rpcOptions(timeoutMs?: number): {
		...
		return timeoutMs
			? { baseUrl: this.baseUrl, timeoutMs }
	...
	async getContacts(): Promise<SignalContact[]> {
		return signalListContacts(this.accountNumber, this.rpcOptions()) as Promise<
			SignalContact[]
		>;
	}

	async getGroups(): Promise<SignalGroup[]> {
		return signalListGroups(this.accountNumber, this.rpcOptions()) as Promise<
			SignalGroup[]
		>;
	}
	...
	private async pollMessages(): Promise<void> {
		...
		try {
			...
			for (const raw of rawMessages) {
				try {
					const msg = SignalService.unwrapEnvelope(
		...
		return memories
			...
			.map((memory) => {
				...
				const isGroup =
					...
					Boolean(
						(room?.metadata as Record<string, unknown> | undefined)?.isGroup,
					);
