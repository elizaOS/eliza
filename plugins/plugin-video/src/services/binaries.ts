...
import { promises as fsp, createWriteStream, constants as fsConstants } from "node:fs";
...
export class BinaryResolver {
    ...
    static instance(): BinaryResolver {
        if (!this._instance) this._instance = new BinaryResolver();
    ...
    constructor(opts: BinaryResolverOptions = {}) {
        ...
        this.disableAutoUpdate = opts.disableAutoUpdate ?? envBool("MILADY_DISABLE_YTDLP_AUTOUPDATE");
        this.preferSystemPath = opts.preferSystemPath ?? envBool("MILADY_YT_DLP_PREFER_PATH");
    ...
    private async fetchExpectedSha(sumsUrl: string, assetName: string): Promise<string> {
...
function errorMessage(err: unknown): string {
    ...
    if (typeof anyErr.stderr === "string" && anyErr.stderr.length > 0) return anyErr.stderr;
