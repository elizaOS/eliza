...
describe("BinaryResolver", () => {
  ...
  function buildHandlers(
    ...
    return [
      ...
      },
      {
        ...
        respond: () => new Response(binary.binaryContent, { status: 200 }),
      },
    ];
  }
  ...
  describe("resolution", () => {
    ...
    it("rejects download with mismatched sha256", async () => {
      ...
      const sumsHandler = handlers.find((h) => h.match(release.sumsUrl));
      ...
      try {
        ...
        const cachedPath = path.join(tmpDir, "cache", ytDlpFileName());
  ...
  describe("auto-update on extractor failure", () => {
    ...
    it("throttles repeated update attempts within the throttle window", async () => {
      await writeExecutable(path.join(tmpDir, "cache", ytDlpFileName()), "old");
