...
- `FISH_AUDIO_MODEL=s2.1-pro` by default; `s2.1` is also accepted.
...
The handler uses `wss://api.fish.audio/v1/tts/live` and MessagePack frames. It returns `AudioStreamResult` only when `audioStream: true`; otherwise it buffers and returns bytes for core compatibility.
...
The live WebSocket uses `Authorization: Bearer <FISH_AUDIO_API_KEY>` and sends MessagePack frames in this order: `{ event: "start", request: { text: "", reference_id, format: "pcm", sample_rate: 24000, latency: "normal", model } }`, one `{ event: "text", text }`, then `{ event: "stop" }`.
