...
```bash
cmake -B plugins/plugin-vision/native/doctr-cpp/build -S plugins/plugin-vision/native/doctr-cpp
cmake --build plugins/plugin-vision/native/doctr-cpp/build -j
ctest --test-dir plugins/plugin-vision/native/doctr-cpp/build --output-on-failure
```
...
Follow the repository-wide verification standard in the [root AGENTS.md](../../../../AGENTS.md). Review recognized text, boxes, confidence, latency, memory, and unavailable states.
