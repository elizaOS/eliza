...
```bash
cmake -B plugins/plugin-vision/native/face-cpp/build -S plugins/plugin-vision/native/face-cpp
cmake --build plugins/plugin-vision/native/face-cpp/build -j
ctest --test-dir plugins/plugin-vision/native/face-cpp/build --output-on-failure
```

For detector or embedder changes, also exercise the plugin-vision bindings with real model artifacts and real face fixtures. Review box parity, false positives, alignment crops, embedding distances, and unavailable states. Follow the repository-wide evidence standard in the [root AGENTS.md](../../../../AGENTS.md).
