...
export class VisionModels {
  private objectDetectionModel: Awaited<ReturnType<NonNullable<typeof cocoSsd>["load"]>> | null = null;
  private poseDetector: Awaited<ReturnType<NonNullable<typeof poseDetection>["createDetector"]>> | null = null;
  ...
  async initialize(config: VisionModelConfig): Promise<void> {
    ...
    if (!this.tfAvailable || !tf || !cocoSsd || !poseDetection) {
      ...
      logger.info("[VisionModels] Initialized without TensorFlow (fallback mode)");
      return;
    }
  ...
  async detectObjects(
    ...
    try {
      ...
      const squeezed = batched.squeeze([0]) as ReturnType<typeof imageTensor.expandDims> extends infer T ? T : never;
      const predictions = await this.objectDetectionModel.detect(squeezed as Parameters<typeof this.objectDetectionModel.detect>[0]);
      squeezed.dispose();
  ...
  convertPosesToPersonInfo(poses: Array<{ keypoints: Array<{ name?: string; x: number; y: number; score?: number }>; score?: number }>): PersonInfo[] {
  ...
  private determinePoseFromKeypoints(keypoints: Array<{ name?: string; x: number; y: number; score?: number }>): Pose {
