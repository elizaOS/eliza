...
export class ErrorRecoveryManager {
  ...
  private registerDefaultStrategies(): void {
    this.registerStrategy("CAMERA_ERROR", {
      ...
      recover: async (error) => {
        logger.warn(
          "[ErrorRecovery] Attempting camera recovery:",
          error.message,
        );
    ...
    this.registerStrategy("API_ERROR", {
      canRecover: (error) => {
        ...
        const count =
          this.errorCounts.get(`${error.code}_${apiError.endpoint}`) || 0;
        return count < this.maxRetries;
      },
      recover: async (error) => {
        const apiError = error as APIError;
  ...
  async handleError(error: VisionError): Promise<boolean> {
    logger.error(
      `[ErrorRecovery] Handling ${error.name}:`,
      error.message,
      error.context,
    );
    ...
    if (!strategy) {
      logger.warn(
        "[ErrorRecovery] No recovery strategy for error code:",
        error.code,
      );
      return false;
    }
    ...
    if (!strategy.canRecover(error)) {
      logger.error(
        "[ErrorRecovery] Recovery strategy cannot handle this error",
      );
      return false;
    }
