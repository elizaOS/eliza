...
export const visionPlugin: Plugin = {
    ...
    init: async (_config, _runtime) => {
        ...
        try {
            const li = (await dynamicImport("@elizaos/plugin-local-inference/services")) as LocalInferenceServicesModule;
            if (typeof li.registerVisionContextAugmenter === "function") {
                const { createDefaultVisionAugmenter } = await import("./vision-context-augmenter.js");
                li.registerVisionContextAugmenter(createDefaultVisionAugmenter());
                logger.info("[vision] registered vision-context augmenter (OCR+object+face fusion) into IMAGE_DESCRIPTION");
            }
        }
        catch (err) {
            logger.debug(`[vision] local-inference vision-augment seam not available; describe runs unaugmented (${err instanceof Error ? err.message : String(err)})`);
        }
        try {
            const mod = (await import("@elizaos/plugin-computeruse/mobile/ocr-provider")) as {
            ...
            if (typeof mod.registerCoordOcrProvider === "function") {
                wireComputerUseOcrBridge(mod.registerCoordOcrProvider as (provider: unknown) => void);
                logger.info("[vision] registered coord-OCR bridge into plugin-computeruse scene seam");
            }
            ...
            if (typeof mod.registerSetOfMarksProvider === "function") {
                wireComputerUseSetOfMarksBridge(mod.registerSetOfMarksProvider as (provider: unknown) => void);
                logger.info("[vision] registered Set-of-Marks bridge into plugin-computeruse detect_elements seam");
            }
        }
        catch (err) {
            logger.debug(`[vision] plugin-computeruse OCR seam not available; running standalone (${err instanceof Error ? err.message : String(err)})`);
        }
    },
