...
export class Florence2Model {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      logger.info(
        "[Florence2] Initializing local Florence-2 model with TensorFlow.js...",
      );
  ...
  async analyzeTile(tile: ScreenTile): Promise<Florence2Result> {
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          "[Florence2] Local model analysis failed, falling back:",
          _modelError,
        );
      }
      ...
      logger.debug(
        `[Florence2] Mock analyzed tile ${tile.id}: ${result.caption}`,
      );
      return result;
    } catch (error) {
  ...
  > {
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          "[Florence2] Local model failed for UI detection, using fallback",
        );
  ...
  private async mockAnalyzeBuffer(
    _imageBuffer: Buffer,
  ...
  async generateSceneGraph(tiles: ScreenTile[]): Promise<{
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        const relation = this.inferSpatialRelation(
          nodes[i].position,
          nodes[j].position,
        );
        if (relation) {
