...
export class VisionWorkerManager {
  ...
  constructor(config: VisionConfig) {
    ...
    this.florence2ResultsBuffer = new SharedArrayBuffer(
      this.FLORENCE2_RESULTS_SIZE,
    );
  ...
  private updateFlorence2Cache(msg: { tileId: string }): void {
    ...
    try {
      ...
      logger.error(
        "[VisionWorkerManager] Failed to update Florence2 cache:",
        error,
      );
    }
  }
  ...
  private readFlorence2Result(tileId: string): Florence2Result | null {
    try {
      ...
      logger.error(
        "[VisionWorkerManager] Failed to read Florence2 result:",
        error,
      );
      return null;
    }
  ...
  getLatestEnhancedScene(): EnhancedSceneDescription {
    ...
    const _tiles = this.generateTiles(
      screenCapture?.width || 0,
      screenCapture?.height || 0,
    );
  ...
  private async restartFlorence2Worker(): Promise<void> {
    ...
    if (attempts >= this.MAX_RESTART_ATTEMPTS) {
      logger.error(
        "[VisionWorkerManager] Max restart attempts reached for Florence2 worker",
      );
      return;
    }
    ...
    logger.info(
      `[VisionWorkerManager] Restarting Florence2 worker (attempt ${attempts + 1})`,
    );
    ...
    try {
      ...
      logger.error(
        "[VisionWorkerManager] Failed to restart Florence2 worker:",
        error,
      );
    }
  }
