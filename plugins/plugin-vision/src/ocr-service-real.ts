...
export class RealOCRService {
      ...
      this.worker = await Tesseract.createWorker("eng", 1, {
        logger: (m) => {
          if (m.status === "recognizing text") {
            logger.debug(
              `[RealOCR] Progress: ${(m.progress * 100).toFixed(1)}%`,
            );
          }
        },
      });
  ...
  private async preprocessImage(imageBuffer: Buffer): Promise<Buffer> {
    try {
      ...
      logger.warn(
        "[RealOCR] Image preprocessing failed, using original:",
        error,
      );
      return imageBuffer;
    }
  ...
  private detectTables(
    ocrResult: OCRResult,
  ): Array<{ rows: string[][]; bbox: BoundingBox }> {
    ...
    for (const [y, rowBlocks] of sortedRows) {
      if (rowBlocks.length > 1) {
        ...
        if (!tableBounds) {
          ...
          tableBounds.width =
            Math.max(...rowBlocks.map((b) => b.bbox.x + b.bbox.width)) -
            tableBounds.x;
  ...
  private detectLists(
    ocrResult: OCRResult,
  ): Array<{ items: string[]; bbox: BoundingBox }> {
    ...
    for (const block of ocrResult.blocks) {
      ...
      const numberedMatch = block.text.match(
        /^(\d+\.|\d+\)|\(\d+\)|[a-zA-Z]\.)\s+(.+)$/,
      );

      if (listMatch || numberedMatch) {
        const itemText = listMatch?.[1] ?? numberedMatch?.[2];
        if (!itemText) {
          continue;
        }
