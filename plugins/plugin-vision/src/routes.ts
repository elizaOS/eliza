 ...
 */
import { OCR_BRIDGE_SERVICE_TYPE } from "./ocr-bridge";
import { SCREEN_CAPTURE_BRIDGE_SERVICE_TYPE } from "./screen-capture-bridge";
import { normalizeScreenCaptureFailureContract } from "@elizaos/core/contracts/screen-capture";
import { normalizeScreenCaptureFrameContract } from "@elizaos/core/contracts/screen-capture";
import { type OcrBridgeService } from "./ocr-bridge";
import { type OcrBridgeWord } from "./ocr-bridge";
import { type Route } from "@elizaos/shared";
import { type ScreenCaptureBridgeService } from "./screen-capture-bridge";
...
export const captureRequestsRoute: Route = {
    ...
    routeHandler: async (ctx) => {
        const bridge = ctx.runtime.getService<ScreenCaptureBridgeService>(SCREEN_CAPTURE_BRIDGE_SERVICE_TYPE);
        if (!bridge)
...
export const screenFrameRoute: Route = {
    ...
    routeHandler: async (ctx) => {
        ...
        const ok = bridge.submitFrame(frame.requestId, frame.base64, frame.format, frame.width, frame.height, frame.capturedAt);
        return ok
            ? jsonResult(200, { ok: true })
            : jsonResult(404, { ok: false, error: "unknown_request" });
    },
...
export const ocrRequestsRoute: Route = {
    ...
    rawPath: true,
    routeHandler: async (ctx) => {
        const bridge = ctx.runtime.getService<OcrBridgeService>(OCR_BRIDGE_SERVICE_TYPE);
        if (!bridge)
            return jsonResult(200, { requests: [] });
        return jsonResult(200, { requests: bridge.takeRequests() });
    },
...
export const ocrResultRoute: Route = {
    ...
    routeHandler: async (ctx) => {
        ...
        if ((body as Record<string, unknown>).error !== undefined) {
            const failed = bridge.failRequest(requestId, String((body as Record<string, unknown>).error));
            return failed
                ? jsonResult(200, { ok: true })
                : jsonResult(404, { ok: false, error: "unknown_request" });
        }
