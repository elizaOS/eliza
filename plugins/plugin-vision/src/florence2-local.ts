...
export class Florence2Local {
  private model: { predict: (input: ReturnType<NonNullable<typeof tf>["tensor"]>) => ReturnType<NonNullable<typeof tf>["tensor"]>; dispose: () => void } | null = null;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.model = (await tf.loadGraphModel(this.config.modelUrl)) as typeof this.model;
  ...
  async analyzeImage(imageBuffer: Buffer): Promise<Florence2Result> {
    ...
    try {
      ...
      return this.parseModelOutput(predictions as Parameters<typeof this.parseModelOutput>[0]);
    } catch (error) {
  ...
  private async runInference(input: { expandDims: (axis: number) => { dispose: () => void } }) {
    if (!this.model) {
    ...
    // Run model
    const output = this.model.predict(batched as Parameters<typeof this.model.predict>[0]);
  ...
  private async parseModelOutput(predictions: { array: () => Promise<number[][]>; dispose: () => void }): Promise<Florence2Result> {
