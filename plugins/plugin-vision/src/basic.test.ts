...
function mockRuntime(serviceReturn: unknown = null): IAgentRuntime {
  return {
...
function mockMessage(text = "test"): Memory {
  return {
    content: { text },
    roomId: "room-1",
    worldId: "world-1",
...
describe("Config", () => {
  ...
  describe("VisionConfigSchema", () => {
    ...
    it("rejects invalid visionMode", () => {
      expect(() => VisionConfigSchema.parse({ visionMode: "INVALID" })).toThrow();
    });

    it("rejects pixelChangeThreshold out of range", () => {
      expect(() => VisionConfigSchema.parse({ pixelChangeThreshold: 150 })).toThrow();
      expect(() => VisionConfigSchema.parse({ pixelChangeThreshold: -1 })).toThrow();
    });

    it("rejects tileSize out of range", () => {
      expect(() => VisionConfigSchema.parse({ tileSize: 10 })).toThrow();
      expect(() => VisionConfigSchema.parse({ tileSize: 2048 })).toThrow();
    });
        ...
        people: [
          {
            ...
            facing: "camera",
            confidence: 0.85,
            boundingBox: { x: 50, y: 10, width: 60, height: 150 },
          },
        ],
        sceneChanged: true,
