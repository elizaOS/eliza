 ...
 * Capacitor host.
 */
import { ScreenCaptureBridgeService } from "./screen-capture-bridge.js";
...
import { type ScreenCaptureFrame } from "./screen-capture-bridge.js";
function makeBridge(timeoutMs = 30000): ScreenCaptureBridgeService {
...
async function submitFrameRoute(bridge: ScreenCaptureBridgeService, body: unknown) {
    const routeHandler = screenFrameRoute.routeHandler;
    if (!routeHandler)
        throw new Error("screen frame route handler is missing");
    return routeHandler({
        body,
        params: {},
        query: {},
...
describe("ScreenCaptureBridgeService", () => {
    ...
    it("submitFrame resolves the matching pending promise with decoded bytes", async () => {
        ...
        const [request] = bridge.takeRequests();
        const accepted = bridge.submitFrame(request.requestId, SAMPLE_BASE64, "png", 10, 20);
    ...
    it("submitFrame returns false for an unknown requestId", () => {
        const bridge = makeBridge();
        expect(bridge.submitFrame("does-not-exist", SAMPLE_BASE64, "png", 1, 1)).toBe(false);
    });
    it("submitFrame returns false the second time (request already consumed)", async () => {
        const bridge = makeBridge();
        const framePromise = bridge.requestFrame();
        const [request] = bridge.takeRequests();
        expect(bridge.submitFrame(request.requestId, SAMPLE_BASE64, "png", 1, 1)).toBe(true);
        await framePromise;
        expect(bridge.submitFrame(request.requestId, SAMPLE_BASE64, "png", 1, 1)).toBe(false);
    });
