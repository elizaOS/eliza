...
class Florence2Worker {
  ...
  private async processFrame(): Promise<void> {
    ...
    for (let i = 0; i < tilesToProcess.length; i++) {
      ...
      try {
        ...
        const tileBuffer = await this.extractTileFromSharedBuffer(
          tile,
          metadata,
        );
        ...
        logger.error(
          `[Florence2Worker] Failed to analyze tile ${tile.id}:`,
          error,
        );
      }
    }
  ...
  private async extractTileFromSharedBuffer(
    ...
    const tileData = Buffer.allocUnsafe(
      tile.width * tile.height * bytesPerPixel,
    );
    ...
    for (let row = 0; row < tile.height; row++) {
      ...
      const sourceOffset =
        this.DATA_OFFSET + sourceY * rowStride + tile.x * bytesPerPixel;
  ...
  private async writeResultToBuffer(
    ...
    for (
      let i = 0;
      i < Math.min(resultBytes.length, this.MAX_RESULT_SIZE - 4);
      i++
    ) {
