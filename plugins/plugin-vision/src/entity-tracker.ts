import { type IAgentRuntime, createUniqueUuid, logger, type UUID } from "@elizaos/core";
      ...
      for (const entity of frameEntities) {
        ...
        try {
          ...
          if (!existing) {
            ...
            logger.debug(`[EntityTracker] Created entity ${entity.id} (${entityUuid}) in runtime`);
          } else {
            await runtime.updateEntity({
              ...elizaEntity,
              id: entityUuid,
            });
            logger.debug(`[EntityTracker] Updated entity ${entity.id} (${entityUuid}) in runtime`);
          }
        } catch (err) {
          logger.debug(
