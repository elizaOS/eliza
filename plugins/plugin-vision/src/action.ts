...
export const describeSceneAction: Action = {
  ...
  description:
    "Analyzes the current visual scene and provides a detailed description of what the agent sees through the camera. Returns scene analysis data including people count, objects, and camera info for action chaining.",
    	...
    	const __avVisionService = runtime?.getService?.("VISION") as { isActive?: () => boolean } | null | undefined;
    	...
    	const __avKeywordOk =
    		...
    		(__avKeywords.some((kw) => kw.length > 0 && __avText.includes(kw)) || __avLegacyContextOk);
    	...
    	const __avLegacyValidate = async (runtime: IAgentRuntime, _message: Memory, _state?: State): Promise<boolean> => {
