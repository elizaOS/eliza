...
/** A single vision capture entry (success or error) */
interface VisionCaptureEntry {
  index: number;
  timestamp: string;
  timestampMs?: number;
  elapsedMs?: number;
  ...
  tests = [
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const captureData = {
          ...
          captures: [] as VisionCaptureEntry[],
          statistics: {
            totalFrames: 0,
            totalSceneChanges: 0,
        ...
        while (Date.now() - startTime < totalDuration) {
          ...
          try {
            ...
            if (scene) {
              ...
              captureData.statistics.totalObjectsDetected +=
            ...
            if (
              elapsedSeconds > lastLoggedProgress &&
              elapsedSeconds % 5 === 0
            ) {
              ...
              if (scene) {
                console.log(
                  `    Last scene: "${scene.description.substring(0, 60)}..."`,
                );
...
`;
        ...
        console.log(
          `   - Scene changes: ${captureData.statistics.totalSceneChanges}`,
        );
        console.log(
          `   - Average change: ${captureData.statistics.averageChangePercentage.toFixed(2)}%`,
        );
        console.log(
          `   - Objects detected: ${captureData.statistics.totalObjectsDetected}`,
        );
        console.log(
          `   - People detected: ${captureData.statistics.totalPeopleDetected}`,
        );
