  ...
  tests = [
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        const visionService = runtime.getService<VisionService>(
          "VISION" as ServiceTypeName,
        );
        if (!visionService) {
          throw new Error(
            "Vision service not available - service must be registered",
          );
        }
        ...
        if (!isActive || !cameraInfo) {
          ...
          console.warn(
            "⚠️  No camera detected. Service initialized but not active.",
          );
          console.log(
            "   This is acceptable in CI/CD environments without cameras.",
          );
        } else {
          console.log("✓ Vision service initialized and active");
          console.log(
            `✓ Connected to camera: ${cameraInfo.name} (ID: ${cameraInfo.id})`,
          );
        }
      },
    },

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (!visionService?.isActive()) {
          ...
          console.log(
            "  Action validation correctly returned false (vision not active)",
          );
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (state.values.visionAvailable === undefined) {
          throw new Error(
            "Vision provider data missing - provider not registered or failed",
          );
        }
...

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (!visionService.isActive()) {
          console.warn(
            "⚠️  Vision service not active - skipping scene change test",
          );
        ...
        console.log(
          `  Initial scene: ${initialScene ? "Available" : "Pending..."}`,
        );
        ...
        console.log(
          `  Scene timestamp: ${new Date(updatedScene.timestamp).toISOString()}`,
        );
        console.log(
          `  Description: ${updatedScene.description.substring(0, 100)}...`,
        );

        if (updatedScene.changePercentage !== undefined) {
          console.log(
            `  Last change: ${updatedScene.changePercentage.toFixed(1)}%`,
          );
        }
      },
    },

    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (!visionService.isActive()) {
          console.warn(
            "⚠️  Vision service not active - skipping detection test",
          );
          console.log("   This is acceptable in environments without cameras");
          return;
        }
        ...
        if (scene.changePercentage > 10) {
          ...
          if (scene.objects.length === 0 && scene.people.length === 0) {
            console.warn(
              "  ⚠️  Significant scene change but no objects/people detected",
            );
            console.log(
              "     This might indicate the motion detection threshold needs adjustment",
            );
          }
        }
      },
