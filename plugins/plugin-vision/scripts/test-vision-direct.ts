...
const testRuntime: Partial<IAgentRuntime> = {
	...
	useModel: async (type: ModelType, input: unknown) => {
		...
		return {
			description: "I see a computer screen with a terminal and code editor",
		};
	},
	logger,
};
...
async function testVisionService() {
	...
	try {
		...
		console.log(
			"  - Camera:",
			service.getCameraInfo() ? "Connected" : "Not connected",
		);
		...
		if (screen) {
			...
			if (analyzedTiles.length > 0) {
				...
				if (tile.analysis?.florence2) {
					console.log(
						"    - Florence2:",
						tile.analysis.florence2.caption || "No caption",
					);
				}
				if (tile.analysis?.ocr) {
					console.log(
						"    - OCR Text:",
						`${tile.analysis.ocr.fullText.substring(0, 100)}...`,
					);
				}
			}
		} else {
		...
		if (enhanced) {
			...
			if (enhanced.screenAnalysis) {
				...
				console.log(
					"  - Focused App:",
					enhanced.screenAnalysis.focusedApp || "Unknown",
				);
			}
		}
