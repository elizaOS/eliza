...
const models = [
	...
	{
		folder: "age_gender_model",
		file: "age_gender_model-weights_manifest.json",
	},
		...
		https
			.get(url, (response) => {
				if (response.statusCode === 302 || response.statusCode === 301) {
					...
					https
						.get(response.headers.location, (redirectResponse) => {
							if (redirectResponse.statusCode !== 200) {
								reject(
									new Error(
										`Failed to download ${modelInfo.file}: ${redirectResponse.statusCode}`,
									),
								);
								return;
							}
					...
					reject(
						new Error(
							`Failed to download ${modelInfo.file}: ${response.statusCode}`,
						),
					);
				}
			})
...
async function downloadAllModels() {
	...
	console.log(
		"These models are required for face recognition functionality.\n",
	);
