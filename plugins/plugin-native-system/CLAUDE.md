...
Exposes Android system capabilities — role status (home, dialer, SMS, assistant), screen brightness, and audio-volume control — to TypeScript code running inside a Capacitor-based Eliza agent on Android. On web/browser it provides a bounded fallback implementation: read methods return non-Android defaults and Android-only mutations reject with descriptive errors. This package is a Capacitor plugin, not an elizaOS plugin that registers actions/services with `AgentRuntime`; it is consumed by higher-level elizaOS packages that need native Android access.
...
| `getDeviceSettings()` | Android + web | Brightness (0–1), brightness mode, WRITE_SETTINGS permission flag, and volume levels for all streams. Web returns fixed non-Android fallback values. |
...
plugins/plugin-native-system/
  src/
    ...
    web.ts            Web fallback (SystemWeb extends WebPlugin); read methods return defaults and Android-only methods throw
  android/
    src/main/
...
bun run --cwd plugins/plugin-native-system test           # vitest coverage for the web fallback
```

The package does not include Android emulator tests; Kotlin changes require manual device or emulator verification through a host Capacitor app.
...
2. Add the web fallback in `src/web.ts` (`SystemWeb` class) — throw a descriptive error for Android-only behavior or return a safe non-Android default for read methods.
