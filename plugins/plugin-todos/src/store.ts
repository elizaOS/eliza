...
export interface TodoStore {
    ...
    importMutationRecords(input: TodoMutationImportInput): Promise<TodoMutationImportResult>;
    create(input: CreateTodoInput): Promise<Todo>;
    get(scope: TodoScope, id: string): Promise<Todo | null>;
    list(filter: TodoFilter): Promise<Todo[]>;
    update(scope: TodoScope, id: string, patch: UpdateTodoInput): Promise<Todo | null>;
...
export const TODO_SCOPE_CONVERGENCE_ERROR_CODE = "TODO_SCOPE_CONVERGENCE_CONFLICT";
/** Return the first repeated persisted id in a desired todo list. */
export function findDuplicateTodoId(todos: ReadonlyArray<{
    id?: string;
}>): string | null {
