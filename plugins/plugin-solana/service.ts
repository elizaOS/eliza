...
export class SolanaWalletService extends Service {
	...
	constructor(runtime?: IAgentRuntime) {
		super(runtime);
		if (!runtime)
			throw new Error("runtime is required for SolanaWalletService");
	}
	...
	public async transferSol(
		...
		try {
			...
			if (!payerKey) {
				throw new Error(
					"SolanaService is not initialized with a fee payer key",
				);
			}
...
export class SolanaService extends Service {
	...
	private async birdeyeFetchWithRetry(
		...
		throw (
			lastError ??
			new Error(
				`Failed to fetch ${resolvedUrl} after ${PROVIDER_CONFIG.MAX_RETRIES} retries`,
			)
		);
	}
	...
	private parseToken2022SymbolFromMintOrPtr = (
		...
		const ptrExtBuffer = getExtensionData(
	...
	public async parseTokenAccounts(
		...
		allMintKeys.forEach((mk, i) => {
			...
			const isT22 =
				infoOwner?.toBase58 &&
				infoOwner.toBase58() === TOKEN_2022_PROGRAM_ID.toBase58();
		...
		(async () => {
			...
			this.runtime.logger.error(
				{ err },
				"solana:parseTokenAccounts - cache save failed",
			),
		);
		...
		try {
			...
			if (birdeyeApiKey) {
				try {
					...
					this.runtime.logger.error(
						{ err: e },
						"solana::updateWalletData - exception",
					);
				}
			}
	...
	public async getTokenAccountsByKeypair(
		...
		try {
			...
			if (options.notOlderThan !== undefined && options.notOlderThan !== 0) {
				const check =
					await this.runtime.getCache<CacheWrapper<KeyedParsedTokenAccount[]>>(
						key,
					);
				if (check) {
	...
	public async getWalletBalances(
		...
		const ataPairs = mints.map((mint) => {
			const ataTokenV1 = getAssociatedTokenAddressSync(
				mint,
				owner,
				false,
				TOKEN_PROGRAM_ID,
	...
	public async executeSwap(
		...
		for (const wallet of wallets) {
			...
			try {
				...
				const estimateLamportsNeeded =
					this.jupiterService.estimateLamportsNeeded;
				...
				const totalLamportsNeededForSwap =
					quoteWithLamports.totalLamportsNeeded;
				if (
					totalLamportsNeededForSwap &&
					totalLamportsNeededForSwap > availableLamports
				) {
					swapResponses[pubKey] = {
						success: false,
						error: "not enough SOL",
				...
				const otherAmountThresholdNum = Number(
					initialQuote.otherAmountThreshold,
				);
				const impliedSlippageBps: number =
					((outAmountNum - otherAmountThresholdNum) / outAmountNum) * 10_000;

				const { amount } = await this.calculateOptimalBuyAmount2(
					initialQuote,
					wallet.amount,
				);
				...
				swapResponses[pubKey] = {
					success: true,
					outAmount:
						typeof outAmount === "string" ? outAmount : String(outAmount || 0),
					signature: txid,
					fees: fees
						? {
