...
export class AgentSkillsService extends Service {
   ...
   */
  private parseDirectoryList(
    config: string | string[] | unknown,
  ): string[] {
    if (Array.isArray(config)) {
      return config.filter((d): d is string => typeof d === "string" && d.trim().length > 0);
    }
   ...
   */
  private async refreshSkillsIfChanged(since: number): Promise<void> {
   ...
   */
  async getIneligibleSkills(): Promise<Array<{
  ...
  private async getLockfileVersion(slug: string): Promise<string | null> {
...
export { AgentSkillsService as ClawHubService };
