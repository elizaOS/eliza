...
export const uninstallSkillAction = {
    ...
    description: "Uninstall non-bundled skill. Bundled skills cannot be removed. Provide slug, e.g. uninstall weather.",
    ...
    validate: createAgentSkillsActionValidator({
        ...
        regex: /\b(?:uninstall|remove|delete)\b.*\bskill\b|\bskill\b.*\b(?:uninstall|remove|delete)\b/i,
    }),
    handler: async (runtime, message, _state, _options, callback) => {
        ...
        if (!slug) {
            const errorText = "I couldn't determine which skill to uninstall. " +
        ...
        const match = loadedSkills.find((s) => s.slug === slug || s.name.toLowerCase() === slug.toLowerCase()) ??
            loadedSkills.find((s) => s.slug.includes(slug) || s.name.toLowerCase().includes(slug));
        if (!match) {
        ...
        if (match.source === "bundled" || match.source === "plugin") {
            const errorText = `Skill **${match.name}** (\`${match.slug}\`) is a ${match.source} skill and cannot be uninstalled. ` +
        ...
        if (decision.status === "cancelled") {
            ...
            if (callback)
                await callback({ text: cancelText });
