...
export const toggleSkillAction = {
    ...
    description: "Enable/disable installed skill.",
    ...
    handler: async (runtime, message, _state, _options, callback) => {
        ...
        if (!slug) {
            const errorText = "I couldn't determine which skill to toggle. " +
        ...
        if (enable === null) {
            const errorText = "I couldn't determine whether to enable or disable the skill. " +
        ...
        const exactMatch = loadedSkills.find((s) => s.slug === slug || s.name.toLowerCase() === slug);
        ...
        if (!toggled && enable) {
            ...
            const findingCount = report
                ...
                : 0;
            const errorText = `Cannot enable "${fuzzyMatch.name}" — it has ${findingCount} security finding(s) ` +
...
//# sourceMappingURL=toggle-skill.js.map