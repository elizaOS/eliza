...
export const installSkillAction = {
    ...
    description: "Install skill from ClawHub registry. Security-scanned before activation. Provide slug/search term, e.g. install weather.",
    ...
    handler: async (runtime, message, _state, _options, callback) => {
        ...
        if (!slug) {
            const errorText = "I couldn't determine which skill to install. " +
        ...
        const existing = loadedSkills.find((s) => s.slug === slug || s.name.toLowerCase() === slug.toLowerCase());
        if (existing) {
        ...
        const bestMatch = searchResults.find((r) => r.slug === slug || r.displayName.toLowerCase() === slug.toLowerCase()) ?? searchResults[0];
        if (!bestMatch) {
        ...
        if (!success) {
            ...
            const errorText = `Failed to install skill "${installSlug}". ` +
        ...
        if (callback)
            await callback({ text: boundedResultText });
        return {
            success: true,
