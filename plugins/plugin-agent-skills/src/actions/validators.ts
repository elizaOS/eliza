...
export function createAgentSkillsActionValidator(
  ...
  return async (
    ...
    const keywordMatch = config.keywords.some((keyword) => keyword.length > 0 && text.includes(keyword));
    const regexMatch = config.regex.test(text);
    const source = String(message.content?.source ?? "");
    const sourceMatch = Boolean(source || state || runtime.agentId || runtime.getService);
