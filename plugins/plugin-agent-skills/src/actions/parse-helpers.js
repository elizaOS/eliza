...
const FILLER_WORDS = /\b(please|can\s+you|could\s+you|the|skill|called|named|for\s+me)\b/g;
/** Action verbs that indicate enable/install/uninstall intent. */
const ACTION_VERBS = /\b(enable|disable|turn\s+on|turn\s+off|activate|deactivate|start|stop|install|download|add|get|fetch|uninstall|remove|delete)\b/g;
/**
 ...
 */
export function extractSlugFromMessage(text) {
 ...
 */
export function detectEnableIntent(text) {
    const normalized = text.toLowerCase();
    if (/\b(enable|turn\s+on|activate|start)\b/.test(normalized))
        return true;
    if (/\b(disable|turn\s+off|deactivate|stop)\b/.test(normalized))
        return false;
    return null;
}
//# sourceMappingURL=parse-helpers.js.map