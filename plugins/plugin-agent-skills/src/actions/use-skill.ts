...
export const useSkillAction: Action = {
  ...
  handler: async (
    ...
    if (!enabled) {
      const errorText =
        `Skill \`${skill.slug}\` is disabled. Use ENABLE_SKILL to enable it first.`;
      if (callback) await callback({ text: errorText });
      return { success: false, error: new Error(errorText) };
    }
    ...
    if (effectiveMode === "script") {
      if (!hasScripts) {
        const errorText =
          `Skill \`${skill.slug}\` has no executable scripts; request mode='guidance' instead.`;
        if (callback) await callback({ text: errorText });
        return { success: false, error: new Error(errorText) };
      }
    ...
    if (!instructions) {
      const errorText =
        `No instructions available for skill \`${skill.slug}\`.`;
      if (callback) await callback({ text: errorText });
      return { success: false, error: new Error(errorText) };
    }
