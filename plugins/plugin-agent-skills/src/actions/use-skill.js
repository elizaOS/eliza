...
const USE_SKILL_PARAMETERS = [
    {
        name: "slug",
        description: "Enabled skill slug. Must match enabled_skills provider.",
        required: true,
        schema: { type: "string" },
    },
    {
        name: "mode",
        description: "Invoke mode: script runs executable, guidance loads SKILL.md, auto picks by scripts.",
        required: false,
        schema: {
            type: "string",
    ...
    {
        name: "script",
        description: "Script filename for mode=script/auto when multiple scripts. Default first script.",
        required: false,
        schema: { type: "string" },
    },
    {
        name: "args",
        description: "Script args: string array or JSON object values as positional args.",
        required: false,
        schema: { type: "object" },
    },
];
function pickMode(requested, hasScripts) {
    if (requested === "script")
        return "script";
...
function isSkillTruncationMarker(marker) {
 ...
 */
async function recordSkillInvocation(runtime, parentStepId, params) {
    if (!parentStepId)
        return;
...
export const useSkillAction = {
    ...
    handler: async (runtime, _message, _state, options, callback) => {
        ...
        const opts = rawOptions.parameters &&
        ...
        if (!rawSlug) {
            const errorText = "USE_SKILL requires a `slug` parameter naming the skill to invoke.";
            if (callback)
                await callback({ text: errorText });
            return { success: false, error: new Error(errorText) };
        }
        ...
        if (!skill) {
            const installed = service
                ...
                .map((s) => s.slug)
                .slice(0, 10);
            const errorText = `Skill \`${rawSlug}\` is not installed. ` +
        ...
        if (!eligibility.eligible) {
            ...
            const errorText = `Skill \`${skill.slug}\` is not eligible to run. Missing dependencies:\n` +
                reasonLines.join("\n");
            if (callback)
                await callback({ text: errorText });
            return { success: false, error: new Error(errorText) };
        }
        const requestedMode = opts.mode === "script" || opts.mode === "guidance" || opts.mode === "auto"
            ? opts.mode
            : undefined;
        ...
        const hasActiveStep = typeof activeStepId === "string" && activeStepId.trim() !== "";
        if (hasActiveStep) {
            await annotateActiveTrajectoryStep(runtime, {
                stepId: activeStepId,
        ...
        if (effectiveMode === "script") {
            ...
            const requestedScript = typeof opts.script === "string" && opts.script.trim()
                ? opts.script.trim()
                : skill.scripts[0];
            const scriptPath = service.getScriptPath(skill.slug, requestedScript);
            if (!scriptPath) {
                const errorText = `Script \`${requestedScript}\` not found in skill \`${skill.slug}\`. ` +
                    `Available scripts: ${skill.scripts.join(", ") || "(none)"}.`;
                if (callback)
                    await callback({ text: errorText });
                return { success: false, error: new Error(errorText) };
            }
            runtime.logger.info(`[AgentSkills] USE_SKILL invoking ${skill.slug}/${requestedScript}`);
        ...
        const maxLen = 3500;
        const truncatedBody = instructions.body.length > maxLen
            ? `${instructions.body.substring(0, maxLen)}\n\n...[truncated]`
            : instructions.body;
        const text = `## ${skill.name}\n\n${skill.description}\n\n### Instructions\n\n${truncatedBody}`;
        if (callback)
            await callback({ text, actions: ["USE_SKILL"] });
        await recordSkillInvocation(runtime, hasActiveStep ? activeStepId : null, {
            skillSlug: skill.slug,
            args: { mode: "guidance" },
            result: {
...
//# sourceMappingURL=use-skill.js.map