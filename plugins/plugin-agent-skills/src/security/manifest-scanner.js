...
function hasHiddenComponent(filePath) {
    return filePath
        .split("/")
        .some((p) => p.startsWith(".") && !ALLOWED_DOTFILES.has(p));
}
export function scanManifest(entries, skillDirPath) {
    const findings = [];
    let totalSize = 0;
    ...
    for (const entry of entries) {
        ...
        if (getFileName(entry.relativePath) === "SKILL.md")
            hasSkillMd = true;
...
export async function buildManifestEntriesFromDisk(dirPath) {
...
export function buildManifestEntriesFromMemory(files) {
    const entries = [];
