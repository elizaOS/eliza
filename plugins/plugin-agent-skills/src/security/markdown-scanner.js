...
function buildExternalUrlPattern(extra = []) {
    const all = [...DEFAULT_SAFE_DOMAINS, ...extra];
    const lookahead = all.map((d) => d.replace(/\./g, "\\.")).join("|");
    return new RegExp(`https?:\\/\\/(?!(${lookahead})\\b)[a-zA-Z0-9][^\\s)\\]"'<>]*`);
}
export function buildMarkdownRules(additionalSafeDomains = []) {
    return [
        ...
        },
        {
            ...
            pattern: /\b(new\s+instructions|override\s+instructions|disregard\s+(all|previous|prior)|forget\s+(everything|all|previous))\b/i,
        },
...
export function scanMarkdownSource(source, filePath, additionalSafeDomains = []) {
