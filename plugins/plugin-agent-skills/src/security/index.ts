...
import type { SkillScanFinding, SkillScanOptions, SkillScanReport, SkillScanStatus } from "./types";
...
export type { ManifestFinding, SkillScanFinding, SkillScanOptions, SkillScanReport, SkillScanSeverity, SkillScanStatus } from "./types";
...
const BLOCKING_RULE_IDS = new Set(["binary-file", "symlink-escape", "missing-skill-md"]);
...
function buildReport(
  ...
  return { scannedAt: new Date().toISOString(), status, summary: { scannedFiles, critical, warn, info }, findings, manifestFindings, skillPath };
}

// Need the import for buildReport's parameter type
import type { ManifestFinding } from "./types";

export async function scanSkillDirectory(dirPath: string, options: SkillScanOptions = {}): Promise<SkillScanReport> {
  ...
  for (const entry of manifestEntries) {
    ...
    let content: string;
    try { content = await fs.readFile(path.join(dirPath, entry.relativePath), "utf-8"); }
    catch { continue; }

    scannedCount++;
    if (isCode) allFindings.push(...scanCodeSource(content, entry.relativePath));
    if (isMd) allFindings.push(...scanMarkdownSource(content, entry.relativePath, safeDomains));
  }
...
export function scanSkillPackage(
  ...
  for (const [relativePath, file] of files) {
    ...
    if (isMd) allFindings.push(...scanMarkdownSource(content, relativePath, safeDomains));
  }

  return buildReport(skillPath, scannedCount, allFindings, manifestFindings);
}

export async function saveScanReport(skillDirPath: string, report: SkillScanReport): Promise<void> {
  const fs = await import("node:fs/promises");
  const path = await import("node:path");
  await fs.writeFile(path.join(skillDirPath, SCAN_REPORT_FILENAME), JSON.stringify(report, null, 2), "utf-8");
}

export async function loadScanReport(skillDirPath: string): Promise<SkillScanReport | null> {
  const fs = await import("node:fs/promises");
  const path = await import("node:path");
  try {
    ...
    if (typeof parsed.scannedAt !== "string" || !Array.isArray(parsed.findings) || !Array.isArray(parsed.manifestFindings)) return null;
    return parsed;
  } catch {
    return null;
  }
