...
const NODE_MANAGERS = ["bun", "npm", "yarn"];
// ============================================================
// PLATFORM UTILITIES
// ============================================================
/**
 ...
 * 4. yarn
 */
export async function getPreferredNodeManager() {
...
async function resolveNodeManager(command) {
    ...
    if (!manager) {
        throw new Error("No Node.js package manager found (tried bun, npm, yarn)");
    }
 ...
 * ```
 */
export async function installSkillDependency(options) {
 ...
 */
export async function findBestInstallOption(options) {
    const platform = detectPlatform();
 ...
 */
export async function getAvailableInstallOptions(options) {
    const available = [];
 ...
 */
export async function installSkillDependencies(skill, options = {}) {
