...
export class AgentSkillsService extends Service {
  ...
  private scanStatusMap: Map<string, import("../security/types").SkillScanStatus> = new Map();
  ...
  private async loadSkillFromStorageWithSource(
    ...
    return {
      ...
      description: typeof frontmatter.description === "string" ? frontmatter.description : String(frontmatter.description || ""),
      version,
      content,
      frontmatter,
      ...
      do {
        ...
        if (!response.ok) {
          if (response.status === 429) {
            ...
            const retrySecs = retryAfterHeader ? Number(retryAfterHeader) : null;
            const cooldownSecs =
  ...
  private async scanInstalledSkill(slug: string): Promise<SkillScanReport> {
    ...
    const report = scanSkillPackage(
      pkg.files as Map<string, { content: string | Uint8Array; isText: boolean }>,
