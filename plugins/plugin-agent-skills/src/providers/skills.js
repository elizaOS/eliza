 ...
 */
export const skillsSummaryProvider = {
    name: "agent_skills",
        ...
        try {
            ...
            for (const skill of listedSkills) {
                ...
                if (scanStatus && scanStatus !== "clean") {
                    scanAnnotations.push(`- \`${skill.slug}\`: security scan status = **${scanStatus}** (requires acknowledgment to enable)`);
                }
            }
            const scanSection = scanAnnotations.length > 0
                ? `\n\n### Security Notices\n${scanAnnotations
 ...
 */
export const skillInstructionsProvider = {
    ...
    get: async (runtime, message, state) => {
        try {
            ...
            const maxChars = 4000;
            const truncatedBody = instructions.body.length > maxChars
 ...
 */
export const catalogAwarenessProvider = {
    ...
    get: async (runtime, message, _state) => {
        try {
            ...
            let categoryText = "";
            for (const [category, skills] of Object.entries(categories).slice(0, MAX_CATALOG_CATEGORIES)) {
                const skillNames = skills
                    .slice(0, MAX_CATALOG_SKILLS_PER_CATEGORY)
                    .map((s) => s.name)
                    .join(", ");
                const more = skills.length > MAX_CATALOG_SKILLS_PER_CATEGORY
...
function calculateSkillRelevance(skill, context) {
    ...
    // Exact slug match
    if (slug && contextLower.includes(slug.toLowerCase()))
        score += 10;
    // Exact name match
    if (name && contextLower.includes(name.toLowerCase()))
        score += 8;
    ...
    if (triggerMatch) {
        ...
        for (const trigger of triggerWords) {
            if (trigger && contextLower.includes(trigger))
                score += 3;
        }
    }
    return score;
