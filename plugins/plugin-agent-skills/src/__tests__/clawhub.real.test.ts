...
`;
		...
		console.log(`Has Otto requires: ${JSON.stringify(ottoMeta?.requires)}`);
	});
...
When asked a question, identify which skill (if any) is most relevant. Be brief.`,
		...
		it("should recognize Otto dependencies in skills", () => {
			...
			console.log(`  git-workflow requires: ${gitMeta?.requires?.bins}`);
			console.log(`  code-review requires: ${reviewMeta?.requires?.bins}`);
		});
	},
);
