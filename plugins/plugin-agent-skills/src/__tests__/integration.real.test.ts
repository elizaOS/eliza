...
function findSkillRoots(): string[] {
  const possiblePaths = [
    ...
    path.resolve(__dirname, "../../../../../../../eliza/packages/skills/skills"),
  ...
  return possiblePaths.filter((candidate, index) =>
...
async function loadRealSkill(skillName: string): Promise<LoadedSkill | null> {
  const skillPath = SKILL_ROOTS
    .map((root) => path.join(root, skillName, "SKILL.md"))
