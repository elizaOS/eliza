...
async function benchmarksPanelHandler(
  req: any,
  res: any,
  ...
  try {
    ...
    logger.error(
      "[ACTION-BENCH PANEL] Error serving benchmarks UI:",
...
async function frontendAssetHandler(
  ...
  try {
    ...
    if (assetsStartIndex !== -1) {
      assetName = assetRequestPath.substring(
    ...
    if (!assetName) {
      logger.warn(
        "[ACTION-BENCH ASSETS] No asset name found in path:",
        assetRequestPath,
      );
