...
export const testRoute: Route = {
  ...
  handler: async (req: any, res: any, runtime: IAgentRuntime) => {
    ...
    try {
      ...
      if (!useMultiStep) {
        throw new Error('Multi-step functionality is required for benchmark tests. Please set USE_MULTI_STEP=true in your environment. The benchmark UI only supports multi-step mode.');
      }
      ...
      if (!channelId) {
        throw new Error('channelId is required - test must be run with an existing channel');
      }
    ...
    const sendStepMessage = async (message: string, channelId: UUID, clientId: UUID, baseUrl: string) => {
      try {
        ...
        socket.on('messageBroadcast', async (data: any) => {
          try {
            const { senderId, text, roomId: messageChannelId, actions, rawMessage } = data;
