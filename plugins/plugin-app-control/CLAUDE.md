...
This opt-in plugin registers nine actions, no natural-language pre-LLM shortcuts, three evaluators,
...
| `viewCommandShortcutEvaluator` | `src/evaluators/view-command-shortcut.ts` | Post-Stage-1 executor for exact navigation phrases. It runs only after the model has selected `VIEWS`, resolves the target deterministically, and avoids a redundant full-planner round. |
