...
|---|---|---|
| `APP` | `src/actions/app.ts` | Unified local app control. Sub-modes: `launch`, `relaunch`, `load_from_directory`, `list`, `create`. `list` returns a structured machine result without writing a callback message so the planner owns the sole prose reply. `create` runs a multi-turn scaffold+coding-agent flow. Owner-gated. |
