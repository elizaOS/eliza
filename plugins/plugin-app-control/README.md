...
Single-view navigation can reuse the typed decision from the existing response-handler call. For client-chat text and voice direct messages, when that decision covers the entire request, the regular executor performs `VIEWS_SHOW`; a successful matching receipt can release the held model-authored destination confirmation without another inference. Missing or unsuitable prose keeps the reply-only round. Compound or uncertain requests keep the planner. Voice retains originating-client delivery; without a completed-action delivery receipt it keeps the post-tool confirmation call. All role, destination, transport and recovery checks still apply.
...
Optional visual continuation preserves an already completed simple answer when the typed plan has no pending intents, action candidates, parent hints or tool requirement, and its reply effect is `none`. It does not infer completion from the user's wording. Explicit navigation, pending domain work and missing answers keep normal planning and receipt checks.
