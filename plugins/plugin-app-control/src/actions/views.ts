...
function readCapabilityParams(
	...
	messageText?: string,
	registeredViews: readonly ViewSummary[] = [],
	...
	if (
		...
		!targetMatchesRegisteredView(unresolvedTarget, registeredViews) &&
		!params.title &&
...
function targetMatchesRegisteredView(
	target: string,
	views: readonly ViewSummary[],
): boolean {
	return resolveViewTarget(target, views) !== null;
}
...
function resolveLayoutTargets(
	...
	// A specific structured target can contain another view's whole generic name
	// ("Simple Calendar" contains "Calendar"). Drop only that shadow match; keep
	// genuinely additional text targets so a partially populated planner payload
	// can still be completed from the user's words.
	const textUnique = uniqueByViewId(textResolved).filter(
		(view) =>
			explicitUnique.some((explicitView) => explicitView.id === view.id) ||
			!explicitUnique.some((explicitView) =>
...
function viewNameIsContainedBy(
	...
	return candidateNames.some((candidateName) =>
		specificNames.some(
			(specificName) =>
				specificName.length > candidateName.length &&
				textMentionsTerm(specificName, candidateName),
		),
	);
}
