...
function escapePatternCharacter(value: string): string {
	return /[\\^$.*+?()[\]{}|]/.test(value) ? `\\${value}` : value;
}

function grammarPattern(value: string): string {
...
function exactValuePattern(value: string): string {
	return Array.from(value, escapePatternCharacter).join("");
}
...
export function isBoundRuntimeManagementConfirmation(
	...
	const fieldPattern = fields
		.map(
			([name, value]) =>
				`\\s+${grammarPattern(name)}=${exactValuePattern(value)}`,
		)
		.join("");
	const trailingGrammarWhitespace = fields.length === 0 ? "\\s*" : "";
	const pattern = new RegExp(
		`^\\s*(?:${grammarPattern("yes")},?\\s+)?${grammarPattern("confirm")}\\s+${grammarPattern(operation)}${fieldPattern}${trailingGrammarWhitespace}$`,
		"u",
	);
