...
function rankMatches(intent, apps) {
    const intentTokens = new Set(tokenize(intent));
    if (intentTokens.size === 0)
        return [];
    const ranked = [];
    for (const app of apps) {
        const haystack = tokenize(`${app.name} ${app.displayName} ${app.pluginName}`);
        let score = 0;
...
function renderChoiceBlock(choiceId, matches) {
    ...
    matches.forEach((match, idx) => {
        lines.push(`edit-${idx + 1} = Edit existing: ${match.app.displayName} (${match.app.name})`);
    });
...
async function findFreeWorkdir(repoRoot, baseName) {
    ...
    while (await fs.stat(candidate).then(() => true, () => false)) {
        ...
        suffix += 1;
        if (suffix > 50) {
            throw new Error(`Could not find a free app directory under ${baseDir} for "${baseName}"`);
        }
    }
    return { workdir: candidate, appDirName };
}
const KEBAB_RE = /^[a-z][a-z0-9-]{1,38}[a-z0-9]$/;
function fallbackNamesFromIntent(intent) {
...
async function extractNames(runtime, intent) {
    ...
    try {
        ...
        logger.warn(`[plugin-app-control] APP/create extractNames LLM failed: ${err instanceof Error ? err.message : String(err)} — using fallback`);
        return fallback;
    }
...
async function dispatchCodingAgent({ runtime, prompt, label, workdir, appName, originRoomId, callback, }) {
    const createTask = runtime.actions.find((a) => a.name === "START_CODING_TASK") ??
    ...
    const result = await spawnWithTrajectoryLink(runtime, {
        ...
        if (trajectory.parentStepId) {
            ...
            const existingMetadata = parameters.metadata &&
        ...
        const createTaskResult = await createTask.handler(runtime, fakeMessage, undefined, handlerOptions, callback);
...
function buildCreatePrompt(intent, appName, displayName, workdir) {
    return [
...
function buildEditPrompt(intent, app, workdir) {
    return [
        "task: edit_eliza_app",
        `appName: ${app.name}`,
...
async function resolveMinAppTemplateDir(repoRoot) {
...
async function deleteIntentTask(runtime, taskId) {
    await runtime
        .deleteTask(taskId)
        .catch((err) => {
        logger.warn(`[plugin-app-control] APP/create failed to delete intent task ${taskId}: ${err instanceof Error ? err.message : String(err)}`);
    });
}
async function locateInstalledAppWorkdir(repoRoot, app) {
...
async function createNewApp({ runtime, intent, repoRoot, originRoomId, callback, }) {
    ...
    if (!templateSrc) {
        const tried = MIN_APP_TEMPLATE_CANDIDATES.map((rel) => path.join(repoRoot, rel)).join(", ");
    ...
    await callback?.({ text });
    logger.info(`[plugin-app-control] APP/create new name=${name} workdir=${workdir} dir=${appDirName} session=${task.sessionId}`);
    return {
        success: true,
        text,
        values: {
...
async function editExistingApp({ runtime, intent, app, repoRoot, originRoomId, callback, }) {
    ...
    await callback?.({ text });
    logger.info(`[plugin-app-control] APP/create edit appName=${app.name} workdir=${workdir} session=${task.sessionId}`);
    return {
        success: true,
        text,
        values: {
...
export function isChoiceReply(text) {
 ...
 */
export async function runCreate({ runtime, client, message, options, callback, repoRoot, }) {
    ...
    if (explicitEditTarget) {
        ...
        const target = installed.find((a) => a.name === explicitEditTarget ||
    ...
    await callback?.({ text });
    logger.info(`[plugin-app-control] APP/create offered ${matches.length} edit choices for room=${roomId}`);
    return {
        success: true,
 ...
 */
export async function hasPendingIntent(runtime, roomId) {
    const existing = await findExistingIntentTask(runtime, roomId);
