...
export async function runRelaunch({ runtime, client, message, options, callback, }) {
    const explicitRunId = readStringOption(options, "runId");
    const target = readStringOption(options, "app") ??
        readStringOption(options, "name") ??
        extractLaunchTarget(message, options);
    if (!target && !explicitRunId) {
        const text = 'I need an app name or runId to relaunch. Try: "relaunch shopify".';
        await callback?.({ text });
    ...
    if (explicitRunId) {
        await client.stopAppRun(explicitRunId).catch((err) => {
            logger.warn(`[plugin-app-control] APP/relaunch stopAppRun ${explicitRunId} failed: ${err instanceof Error ? err.message : String(err)}`);
        });
    }
    else if (appName) {
        const runs = await client.listAppRuns();
        const matched = runs.filter((r) => r.appName === appName ||
            r.displayName === appName ||
            r.appName.endsWith(appName));
        for (const run of matched) {
            await client.stopAppRun(run.runId).catch((err) => {
                logger.warn(`[plugin-app-control] APP/relaunch stopAppRun ${run.runId} failed: ${err instanceof Error ? err.message : String(err)}`);
            });
        }
    }
    if (!appName) {
    ...
    let verifySection = "";
    const wantsVerify = options?.verify === true || readStringOption(options, "verify") === "true";
    if (wantsVerify) {
        ...
        if (!workdir) {
            ...
            const service = runtime.getService("app-verification");
            if (!service) {
                verifySection =
                ...
                verifySection = `\nVerify (fast): ${verifyResult.verdict}${verifyResult.verdict === "fail"
    ...
    await callback?.({ text });
    logger.info(`[plugin-app-control] APP/relaunch ${appName} newRunId=${newRunId ?? "<none>"} verify=${wantsVerify}`);
    return {
        success: true,
        text,
        values: {
...
//# sourceMappingURL=app-relaunch.js.map