...
export async function runLaunch({ client, message, options, callback, }) {
    const target = extractLaunchTarget(message, options);
    if (!target) {
        const text = 'I need the app name to launch. Try: "launch shopify" or pass { app: "companion" }.';
        await callback?.({ text });
    ...
    logger.info(`[plugin-app-control] APP/launch ${appName} runId=${runId ?? "<none>"}`);
    await callback?.({ text });
    return {
        success: true,
        text,
