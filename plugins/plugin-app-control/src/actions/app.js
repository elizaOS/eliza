...
const CREATE_VERBS = /\b(create|build|make|new|scaffold|generate|spin up)\b.*?\b(app|application|game|tool|widget|dashboard)\b/i;
const PLUGIN_ONLY = /\bplugin\b/i;
const APP_NOUN = /\b(app|apps|application|applications|mini)\b/i;
const LOAD_FROM_DIR = /\b(load|register|import|scan)\b.*\b(directory|folder|dir|path)\b/i;
...
function inferMode(text, options) {
    ...
    if (!trimmed)
        return null;
    if (LOAD_FROM_DIR.test(trimmed))
        return "load_from_directory";
    if (CREATE_VERBS.test(trimmed) && !PLUGIN_ONLY.test(trimmed)) {
        return "create";
    }
    // "what apps are open" / "list running"
    if (LIST_VERBS.test(trimmed) && APP_NOUN.test(trimmed))
        return "list";
    if (RELAUNCH_VERBS.test(trimmed) && APP_NOUN.test(trimmed))
        return "relaunch";
    ...
    if (LAUNCH_VERBS.test(trimmed) && APP_NOUN.test(trimmed))
        return "launch";
    return null;
}
...
export function createAppAction(deps = {}) {
    ...
    return {
        ...
        parameters: [
            ...
            {
                name: "app",
                description: "App name, slug, or display name (launch / relaunch / create-edit).",
                required: false,
                schema: { type: "string" },
            },
            {
            ...
            {
                name: "runId",
                description: "Specific run id to stop before relaunching (relaunch mode).",
                required: false,
                schema: { type: "string" },
            },
            {
                name: "verify",
                description: "When true, runs AppVerificationService (fast profile) after relaunch.",
                required: false,
                schema: { type: "boolean", default: false },
            },
            {
                name: "workdir",
                description: "Absolute workdir for verify (relaunch) or for explicit edit (create).",
                required: false,
                schema: { type: "string" },
            },
            {
            ...
            {
                name: "intent",
                description: "Free-form description of the app to build (create mode). Defaults to the user message text.",
                required: false,
                schema: { type: "string" },
            },
            {
                name: "editTarget",
                description: "Skip the picker and edit this installed app directly (create mode).",
                required: false,
                schema: { type: "string" },
            },
            {
        ...
        handler: async (runtime, message, _state, options, callback) => {
            ...
            if (!mode) {
                const reply = 'Tell me which app to control. Try: "launch shopify", "list running apps", "create a new note-taking app".';
...
export const appAction = createAppAction();
