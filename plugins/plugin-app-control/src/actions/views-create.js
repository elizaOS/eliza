...
function rankViewMatches(intent, views) {
    const intentTokens = new Set(tokenize(intent));
    if (intentTokens.size === 0)
        return [];
    const ranked = [];
    for (const view of views) {
        const haystack = tokenize(`${view.id} ${view.label} ${view.description ?? ""} ${(view.tags ?? []).join(" ")}`);
        let score = 0;
...
async function extractNames(runtime, intent) {
    ...
    try {
        ...
        logger.warn(`[plugin-app-control] VIEWS/create extractNames LLM failed: ${err instanceof Error ? err.message : String(err)} — using fallback`);
        return fallback;
    }
...
async function resolveTemplateDir(repoRoot) {
...
async function findFreePluginWorkdir(repoRoot, baseName) {
    for (const candidate of PLUGINS_DIR_CANDIDATES) {
        ...
        while (await fs.stat(dir).then(() => true, () => false)) {
            ...
            suffix += 1;
            if (suffix > 50)
                throw new Error(`Could not find a free plugin directory for "${baseName}"`);
        }
        return dir;
    }
...
async function dispatchCodingAgent({ runtime, prompt, label, workdir, pluginName, originRoomId, callback, }) {
    ...
    const result = await spawnWithTrajectoryLink(runtime, {
        ...
        if (trajectory.parentStepId) {
            ...
            const existingMeta = parameters.metadata &&
...
function buildEditPrompt(intent, view, workdir) {
    return [
...
async function deleteIntentTask(runtime, taskId) {
    await runtime
        .deleteTask(taskId)
        .catch((err) => {
        logger.warn(`[plugin-app-control] VIEWS/create failed to delete intent task ${taskId}: ${err instanceof Error ? err.message : String(err)}`);
    });
}
...
// ---------------------------------------------------------------------------
async function locatePluginSourceDir(repoRoot, view) {
...
// Create / edit helpers
// ---------------------------------------------------------------------------
async function createNewViewPlugin({ runtime, intent, repoRoot, originRoomId, callback, }) {
...
async function editExistingViewPlugin({ runtime, intent, view, repoRoot, originRoomId, callback, }) {
...
// Public entry
// ---------------------------------------------------------------------------
export async function runViewsCreate({ runtime, message, options, views, callback, repoRoot, }) {
    ...
    if (explicitEditTarget) {
        const target = views.find((v) => v.id === explicitEditTarget ||
...
export async function hasPendingViewsCreateIntent(runtime, roomId) {
    const existing = await findExistingIntentTask(runtime, roomId);
    return existing !== null;
}
//# sourceMappingURL=views-create.js.map