 ...
 */
function formatTable(installed, runs) {
    ...
    if (installed.length === 0) {
        ...
        for (const app of installed) {
            ...
            lines.push(`  ${app.name},${app.displayName},${live.map((r) => r.runId).join("|") || "none"}`);
        }
    }
    const orphanRuns = runs.filter((r) => !installed.some((app) => app.name === r.appName));
...
export async function runList({ client, callback, }) {
