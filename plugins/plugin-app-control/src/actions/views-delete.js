...
// ---------------------------------------------------------------------------
function resolveTargetView(target, views) {
    ...
    const byPlugin = views.find((v) => v.pluginName.toLowerCase() === q ||
        v.pluginName.replace(/^@[^/]+\//, "").toLowerCase() === q);
    if (byPlugin)
        return { kind: "match", view: byPlugin };
    const scored = views
...
function extractDeleteTarget(message, options) {
    return (readStringOption(options, "view") ??
...
function extractTargetFromText(text) {
    ...
    for (const verb of DELETE_VERBS) {
        ...
        const tokens = rest
            ...
            .filter((t) => t.length > 0);
        let i = 0;
        while (i < tokens.length && FILLER.has(tokens[i].toLowerCase()))
            i++;
        const candidate = tokens.slice(i).join(" ").toLowerCase();
        if (candidate && !FILLER.has(candidate))
            return candidate;
    }
    return null;
}
...
// Protection check
// ---------------------------------------------------------------------------
async function checkProtection(pluginName, repoRoot) {
...
// ---------------------------------------------------------------------------
async function findConfirmTask(runtime, roomId) {
    ...
    const matching = tasks
        ...
        const aAt = typeof aMeta?.confirmedAt === "string"
            ? 0
            : Date.parse(String(aMeta?.roomId ?? "")) || 0;
        const bAt = typeof bMeta?.confirmedAt === "string"
            ? 0
...
async function persistConfirmTask(runtime, metadata) {
...
async function deleteConfirmTask(runtime, taskId) {
    await runtime
        .deleteTask(taskId)
        .catch((err) => {
        logger.warn(`[plugin-app-control] VIEWS/delete failed to delete confirm task ${taskId}: ${err instanceof Error ? err.message : String(err)}`);
    });
}
// ---------------------------------------------------------------------------
// Plugin unload
...
export function isDeleteCancellation(text) {
    return NO_RE.test(text);
}
// ---------------------------------------------------------------------------
// Public entry
// ---------------------------------------------------------------------------
export async function runViewsDelete({ runtime, message, options, views, callback, repoRoot, }) {
    ...
    if (existingConfirm) {
        if (isDeleteConfirmation(userText)) {
            ...
            logger.info(`[plugin-app-control] VIEWS/delete confirmed viewId=${metadata.viewId} pluginName=${metadata.pluginName}`);
    ...
    const targetStr = readStringOption(options, "confirm") === "true"
        ? (readStringOption(options, "view") ??
            readStringOption(options, "viewId") ??
            userText)
        : extractDeleteTarget(message, options);
    if (!targetStr) {
        const text = 'Tell me which view to delete. Try: "delete the wallet view" or "remove the LifeOps plugin".';
        await callback?.({ text });
        return { success: false, text };
    }
    ...
    if (explicitConfirm === "true" || explicitConfirm === "yes") {
        logger.info(`[plugin-app-control] VIEWS/delete explicit-confirm viewId=${view.id} pluginName=${view.pluginName}`);
    ...
    await callback?.({ text });
    logger.info(`[plugin-app-control] VIEWS/delete awaiting confirmation viewId=${view.id} pluginName=${view.pluginName} room=${roomId}`);
    return {
        success: true,
        text,
        values: {
...
export async function hasPendingDeleteConfirm(runtime, roomId) {
    const existing = await findConfirmTask(runtime, roomId);
    return existing !== null;
}
//# sourceMappingURL=views-delete.js.map