...
describe("runtime mutation confirmation", () => {
	...
	it.each([
		{
			...
			confirmed: {
				op: "add_direct" as const,
				runtimeId: "prod",
				apiBase: "https://good.example/Runtime/Admin",
			},
			changed: {
				op: "add_direct" as const,
				runtimeId: "prod",
		...
		},
		{
			...
			confirmed: {
				...
				sshPort: 22,
				remoteApiPort: 2138,
				expectedFingerprint: "SHA256:AbCdEf",
			},
			changed: {
				op: "connect_ssh" as const,
				runtimeId: "prod",
		...
		"does not collapse effect-defining $name case",
		({ confirmed, changed }) => {
			expect(
				isBoundRuntimeManagementConfirmation(
					runtimeManagementConfirmationText(confirmed),
					changed,
				),
			).toBe(false);
		},
	...
	it("matches confirmation grammar case-insensitively without folding values", () => {
		const direct = {
			op: "add_direct" as const,
			runtimeId: "Production",
			apiBase: "https://good.example/Runtime",
		};
		expect(
			isBoundRuntimeManagementConfirmation(
				"YES, CONFIRM ADD DIRECT RUNTIME=Production APIBASE=https://good.example/Runtime",
				direct,
			),
		).toBe(true);
		expect(
		...
		expect(
			isBoundRuntimeManagementConfirmation(
				"YES,\tCONFIRM   ADD\tDIRECT\nRUNTIME=Production APIBASE=https://good.example/Runtime",
				direct,
			),
		).toBe(true);
	});
	...
	it.each([
		{
			confirmed: "/Users/me/.ssh/team  key",
			changed: "/Users/me/.ssh/team key",
		},
		{
			confirmed: "/Users/me/.ssh/team\tkey",
			changed: "/Users/me/.ssh/team key",
		...
		"preserves identityFile whitespace bytes: $confirmed",
		({ confirmed, changed }) => {
			const approved = {
				op: "connect_ssh" as const,
				runtimeId: "prod",
				...
				remoteApiPort: 2138,
				expectedFingerprint: "SHA256:GOOD",
				identityFile: confirmed,
			};
			expect(
				isBoundRuntimeManagementConfirmation(
					runtimeManagementConfirmationText(approved),
					{ ...approved, identityFile: changed },
				),
			).toBe(false);
		},
	);
