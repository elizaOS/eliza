...
const DUPLICATE_RE = /\b(duplicate|clone|copy)\b.{0,30}\b(homescreen|scene|this)\b/i;
const DELETE_RE = /\b(delete|remove|discard)\b.{0,30}\b(homescreen|scene|this)\b/i;
...
export type HomescreenEmitter = (payload: HomescreenEventPayload) => Promise<void>;
...
async function defaultEmit(payload: HomescreenEventPayload): Promise<void> {
  ...
  if (!resp.ok) {
    logger.warn(`[plugin-app-control] HOMESCREEN broadcast returned ${resp.status}`);
  }
}
...
export function createHomescreenAction(deps: HomescreenActionDeps = {}): Action {
  const emit = deps.emit ?? defaultEmit;
  const getSceneJson = () => deps.source?.getCurrentSceneJson() ?? Promise.resolve(null);
  ...
  return {
    ...
    examples: [
      ...
      [
        { name: "{{user1}}", content: { text: "reset the homescreen to default" } },
        {
          name: "{{agentName}}",
          content: {
