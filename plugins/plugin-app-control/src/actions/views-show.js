...
function extractViewTarget(message, options) {
    // Explicit option wins.
    const explicit = readStringOpt(options, "view") ??
...
function readStringOpt(options, key) {
    if (!options)
        return null;
    const v = options[key];
    if (typeof v !== "string")
...
async function navigateToView(view, requestedViewType) {
    ...
    try {
        const resp = await fetch(`${base}/api/views/${encodeURIComponent(view.id)}/navigate${requestedViewType ? `?viewType=${requestedViewType}` : ""}`, {
            method: "POST",
        ...
        if (resp.status === 501)
            return `Opened ${view.label}.`;
        // 404 for the route itself means the agent doesn't expose this endpoint.
        if (resp.status === 404)
            return `Opened ${view.label}.`;
        const body = await resp.text().catch(() => "");
        logger.warn(`[plugin-app-control] VIEWS/show navigate returned ${resp.status}: ${body}`);
    }
    catch {
...
export async function runViewsShow({ client, message, options, viewType, callback, }) {
    const target = extractViewTarget(message, options);
    if (!target) {
        const text = 'Tell me which view to open. Try: "open wallet" or "show settings".';
        await callback?.({ text });
    ...
    logger.info(`[plugin-app-control] VIEWS/show viewId=${view.id} viewType=${view.viewType ?? "gui"}`);
...
//# sourceMappingURL=views-show.js.map