...
function parseViewSummary(entry) {
    ...
    const viewType = entry.viewType === "gui" || entry.viewType === "tui"
        ? entry.viewType
        : undefined;
    ...
    const heroImageUrl = typeof entry.heroImageUrl === "string" ? entry.heroImageUrl : undefined;
    const visibleInManager = typeof entry.visibleInManager === "boolean"
        ? entry.visibleInManager
        : undefined;
    const developerOnly = typeof entry.developerOnly === "boolean" ? entry.developerOnly : undefined;
...
function parseCurrentView(body) {
    ...
    const action = typeof currentView.action === "string" ? currentView.action : undefined;
...
export function createViewsClient() {
    return {
        async listViews(opts = {}) {
            ...
            if (opts.viewType)
                params.set("viewType", opts.viewType);
