...
async function requestJson(path, init, parse, errorContext) {
...
function parseInstalledApps(body) {
    ...
    return body.map((entry) => {
        ...
        if (typeof name !== "string" ||
            ...
            throw new Error("Malformed installed app entry: missing required string fields");
        }
        return { name, displayName, pluginName, version, installedAt };
    });
}
function parseAppRunSummary(entry) {
    ...
    if (typeof runId !== "string" ||
        ...
        throw new Error("Malformed app run summary: missing required string fields");
    }
...
function parseLaunchResult(body) {
    ...
    if (typeof displayName !== "string" || typeof launchType !== "string") {
        throw new Error("Malformed launch result: missing displayName or launchType");
    }
    const launchUrl = entry.launchUrl;
    const run = entry.run && typeof entry.run === "object"
        ? parseAppRunSummary(entry.run)
        : null;
    return {
...
function parseStopResult(body) {
    ...
    if (stopScope !== "plugin-uninstalled" &&
...
export function createAppControlClient() {
    return {
        async listInstalledApps() {
            return requestJson("/api/apps/installed", { method: "GET" }, parseInstalledApps, "Failed to list installed apps");
        },
        async listAppRuns() {
            return requestJson("/api/apps/runs", { method: "GET" }, parseAppRuns, "Failed to list running apps");
        },
        async launchApp(name) {
            return requestJson("/api/apps/launch", {
                method: "POST",
                body: JSON.stringify({ name }),
            }, parseLaunchResult, `Failed to launch app ${name}`);
        },
        async stopAppRun(runId) {
            return requestJson(`/api/apps/runs/${encodeURIComponent(runId)}/stop`, { method: "POST" }, parseStopResult, `Failed to stop app run ${runId}`);
        },
        async stopAppByName(name) {
            return requestJson("/api/apps/stop", {
                method: "POST",
                body: JSON.stringify({ name }),
            }, parseStopResult, `Failed to stop app ${name}`);
        },
    };
}
