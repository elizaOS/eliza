...
describe("extractLaunchTarget", () => {
  it("prefers options.app, then options.name, then the message verb", () => {
    expect(extractLaunchTarget(msg("launch calculator"), { app: "wallet" })).toBe(
      "wallet",
    );
    expect(extractLaunchTarget(msg("launch calculator"), { name: "wallet" })).toBe(
      "wallet",
    );
  ...
  it("peels filler words and lowercases the resolved name", () => {
    expect(extractLaunchTarget(msg("fire up the mini Wallet overlay"), undefined)).toBe(
      "wallet",
    );
  });
  ...
  it("returns null when no verb, no option, or only fillers follow", () => {
    ...
    expect(extractLaunchTarget(msg("open the app please"), undefined)).toBeNull();
...
describe("extractCloseTarget", () => {
  it("splits runId from the app name and honors close verbs", () => {
    expect(extractCloseTarget(msg("close calculator"), { runId: "r-1" })).toEqual({
      runId: "r-1",
      appName: "calculator",
    });
