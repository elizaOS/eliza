...
export const availableAppsProvider = {
    name: "available_apps",
    description: "Installed Eliza apps with running-run counts; use this to pick targets for APP launch / relaunch / create. Read-only list/status is exposed here.",
    ...
    get: async (_runtime, _message, _state) => {
        try {
            ...
            if (listedInstalled.length > 0) {
                lines.push(`apps[${listedInstalled.length}]{name,displayName,pluginName,running}:`);
                for (const app of listedInstalled) {
                    const running = runsByApp.get(app.name) ?? 0;
                    lines.push(`  ${app.name},${app.displayName},${app.pluginName},${running}`);
                }
                if (overflow > 0) {
            ...
            if (orphanRuns.length > 0) {
                lines.push(`otherRuns[${orphanRuns.length}]{runId,appName,displayName,status}:`);
                for (const run of orphanRuns) {
                    lines.push(`  ${run.runId},${run.appName},${run.displayName},${run.status}`);
                }
            }
            return {
                text: lines.join("\n"),
