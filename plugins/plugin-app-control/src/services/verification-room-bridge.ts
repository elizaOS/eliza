...
async function buildPassMessage(
	...
	if (isApp) {
		...
		if (launchable) {
			return {
				fallback: `${payload.targetName} app built, verified, and installed — reply 'launch ${payload.targetName}' to open it.`,
				receipt: {
					verdict: "pass",
					targetType: "app",
					targetName: payload.targetName,
					status: "built_verified_installed",
					nextStep: `Ask me to launch ${payload.targetName}.`,
				},
			};
		}
