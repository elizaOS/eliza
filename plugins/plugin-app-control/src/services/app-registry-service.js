...
async function readGrants(file) {
	...
	for (const [slug, value] of Object.entries(rawGrants)) {
		...
		const namespaces = value.namespaces.filter(
			(n) =>
				typeof n === "string" && RECOGNISED_PERMISSION_NAMESPACES.includes(n),
		);
		grants[slug] = {
			namespaces,
...
export class AppRegistryService extends Service {
	...
	async writeGrant(slug, namespaces, actor) {
		...
		const previousSorted = previous ? [...previous.namespaces].sort() : [];
