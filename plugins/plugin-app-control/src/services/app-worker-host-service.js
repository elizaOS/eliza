...
const SOURCE_WORKER_ENTRY = path.resolve(__dirname, "../workers/app-worker-entry.ts");
const DIST_WORKER_ENTRY = path.resolve(__dirname, "workers/app-worker-entry.js");
...
function readStringFromExports(value) {
    ...
    if (!value || typeof value !== "object" || Array.isArray(value))
        return null;
    const record = value;
    return (readString(record.import) ??
...
async function resolvePluginEntryPath(entry) {
    ...
    const pkg = parsed;
    const exportsEntry = readStringFromExports(pkg.exports) ??
        (pkg.exports &&
...
export class AppWorkerHostService extends Service {
    ...
    capabilityDescription = "Spawns and manages Bun workers for apps declaring isolation:'worker'. Phase 2 enforcement substrate.";
    workers = new Map();
    stateDir;
    constructor(runtime) {
        super(runtime);
    ...
    async stop() {
        ...
        await Promise.all(slugs.map((slug) => this.stopWorker(slug).catch(() => { })));
    }
    /**
    ...
    async spawn(options) {
        ...
        spawned.readyPromise = new Promise((resolve, reject) => {
            const onMessage = (raw) => {
                ...
                if (msg.id === 0) {
                    if (msg.ok === true) {
                        ...
                        reject(new Error(msg.reason ?? "Worker boot failed (no reason supplied)"));
                    }
                    return;
                }
                ...
                if (msg.ok) {
                    ...
                    pending.reject(new Error(msg.reason ?? "Worker returned ok:false with no reason"));
                }
            };
            worker.on("message", onMessage);
            worker.on("error", (raw) => {
                const error = raw instanceof Error ? raw : new Error(String(raw));
                logger.error(`[app-worker-host] worker for slug=${options.slug} errored: ${error.message}`);
                if (spawned.readyAt === null)
                    reject(error);
            ...
            worker.on("exit", (code) => {
                ...
                const exitErr = new Error(`Worker for slug=${options.slug} exited (code=${code})`);
                for (const pending of spawned.pending.values()) {
    ...
    async stopWorker(slug) {
        ...
        const settled = await Promise.race([
            ...
            new Promise((resolve) => setTimeout(() => resolve("timeout"), SHUTDOWN_GRACE_MS)),
        ]);
        if (settled === "timeout") {
            logger.warn(`[app-worker-host] worker for slug=${slug} did not exit in ${SHUTDOWN_GRACE_MS}ms; terminating`);
    ...
    async bootstrapRegisteredWorkers() {
        ...
        for (const entry of entries) {
            if (entry.isolation !== "worker")
                continue;
            const result = await this.startForRegisteredApp(entry.slug).catch((error) => ({
                ok: false,
                reason: error instanceof Error ? error.message : String(error),
            }));
            if (!result.ok) {
                logger.warn(`[app-worker-host] bootstrap spawn failed for slug=${entry.slug}: ${result.reason}`);
            }
        }
    }
    snapshot(spawned) {
        return {
            ...
            readyMs: spawned.readyAt !== null ? spawned.readyAt - spawned.bootedAt : null,
        };
    }
}
