...
const calc = app({ name: "calc", displayName: "Calculator", pluginName: "@x/calc" });
const notes = app({ name: "notes", displayName: "Notes", pluginName: "@x/notes" });
...
describe("resolveRunByName", () => {
  it("matches on runId as well as the name fields", () => {
    const r1 = run({ appName: "calc", displayName: "Calculator", runId: "r-123" });
...
describe("candidate formatting", () => {
  it("formats app and run candidate lists", () => {
    ...
    expect(
      formatRunCandidates([run({ displayName: "Calculator", runId: "r-1", status: "running" })]),
