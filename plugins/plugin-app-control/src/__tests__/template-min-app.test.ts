/**
 * Template validation: scaffolds the real `packages/elizaos/templates/min-project` and
 * `packages/elizaos/templates/min-plugin` trees into a tempdir, replaces the
...
// __tests__ → src → plugin-app-control → plugins → eliza
const ELIZA_ROOT = path.resolve(HERE, "..", "..", "..", "..");
const APP_TEMPLATE_DIR = path.join(
	ELIZA_ROOT,
	"packages/elizaos/templates/min-project",
);
const PLUGIN_TEMPLATE_DIR = path.join(
	ELIZA_ROOT,
...
describe.skipIf(skip)(
	"packages/elizaos/templates/min-project — scaffolds into a verifiable workspace",
	() => {
...
describe.skipIf(skip)(
	"packages/elizaos/templates/min-plugin — scaffolds into a verifiable workspace",
	() => {
		const service = new AppVerificationService();
