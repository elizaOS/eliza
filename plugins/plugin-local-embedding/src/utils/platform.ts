...
export class PlatformManager {
  ...
  private async queryWindowsGpuName(): Promise<string | null> {
    // Prefer PowerShell + CIM. wmic.exe is removed in Windows 11 24H2 /
    ...
    try {
      ...
      try {
        ...
        if (message.includes("lspci") && message.includes("not found")) {
          logger.debug("Linux GPU detection skipped: lspci not installed");
        } else {
          logger.warn({ error: message }, "Linux GPU detection failed");
        }
        return null;
      }
    }
