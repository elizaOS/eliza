...
src/
  ...
  plugin.ts              Builds Route[] + exports knowledgePlugin (Plugin object)
...
```bash
bun run --cwd plugins/plugin-knowledge build               # build:js + build:views + build:types
bun run --cwd plugins/plugin-knowledge build:js            # tsup bundling
bun run --cwd plugins/plugin-knowledge build:views         # Vite Knowledge view bundle
bun run --cwd plugins/plugin-knowledge build:types         # tsc --noCheck type emit
bun run --cwd plugins/plugin-knowledge typecheck           # source TypeScript check
bun run --cwd plugins/plugin-knowledge clean               # rm -rf dist
bun run --cwd plugins/plugin-knowledge test                # vitest run (unit tests)
bun run --cwd plugins/plugin-knowledge test:e2e:manual     # vitest run live e2e tests
```

Use `lint:check` for read-only package lint validation.
