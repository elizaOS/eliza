...
import { DetailSkeleton } from "@elizaos/ui/components";
...
import { Textarea } from "@elizaos/ui/components";
import { TranscriptPlayer } from "@elizaos/ui/components/transcripts/TranscriptPlayer";
import { canShareFiles } from "@elizaos/ui/utils/download-share";
import { client } from "@elizaos/ui/api";
import { confirmDesktopAction } from "@elizaos/ui/utils/desktop-dialogs";
import { downloadAttachment } from "@elizaos/ui/utils/download-share";
import { filenameForMime } from "@elizaos/ui/utils/download-share";
...
import { shareAttachment } from "@elizaos/ui/utils/download-share";
import { type DocumentDetail } from "@elizaos/ui/api";
import { type DocumentFragmentRecord } from "@elizaos/ui/api";
import { type ReactNode } from "react";
import { type Transcript } from "@elizaos/core/transcripts";
import { type TranscriptCaptureSharingState } from "@elizaos/core/transcripts";
...
function documentLoadIssue(error: unknown, t: (key: string, options?: Record<string, unknown>) => string): DocumentLoadIssue {
...
/* ── Document Viewer ────────────────────────────────────────────────── */
export function DocumentViewer({ documentId, initialSeekMs, onUpdated, }: {
    ...
    useEffect(() => {
        ...
        async function load() {
            ...
            if (!docRes?.document) {
                throw new Error(t("documentsview.DocumentUnavailable", {
                    defaultValue: "This document is no longer available.",
                }));
            }
    ...
    const handleUpdateTranscriptPrivacy = (sharing: Partial<TranscriptCaptureSharingState>) => {
        if (!transcriptId)
            return;
        ...
        client
            ...
            setPrivacyError(privacyUpdateError instanceof Error
    ...
    const handleDeleteTranscriptSourceAudio = async () => {
        ...
        const confirmed = await confirmDesktopAction({
            ...
            message: "The transcript text will remain, but audio replay cannot be restored.",
            type: "warning",
        ...
        setPrivacyError(null);
        try {
            const { transcript: updated } = await client.deleteTranscriptSourceAudio(transcriptId);
            setTranscript(updated);
        }
        catch (sourceAudioDeleteError) {
            // error-policy:J4 source deletion failures remain visible in the artifact reader.
            setPrivacyError(sourceAudioDeleteError instanceof Error
    ...
    const scopeLabel = doc?.scope === "owner-private"
        ...
        : doc?.scope === "user-private"
            ...
            : doc?.scope === "agent-private"
                ...
                : null;
    const readableText = previewText || fragments.map((fragment) => fragment.text).join("\n\n");
    ...
    const servedFileUrl = doc?.transcriptId && transcript
    ...
    const handleDownloadFile = async () => {
        ...
        try {
            ...
            setActionNotice(t("documentsview.FailedToDownload", {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setActionNotice(t("documentsview.DocumentUpdated", {
            ...
            setActionNotice(t("documentsview.FailedToUpdateDocument", {
    ...
    if (doc) {
        if (readerKind === "transcript") {
            mediaBlock = transcript ? (<PagePanel variant="inset" className="p-4">
          <TranscriptPlayer transcript={transcript} audioUrl={mediaUrl || undefined} initialSeekMs={initialSeekMs}/>
        </PagePanel>) : mediaUrl ? (
            // biome-ignore lint/a11y/useMediaCaption: the searchable text copy below is the caption.
            <audio data-testid="reader-audio" controls src={mediaUrl} className="w-full"/>) : null;
        }
        else if (readerKind === "image" && mediaUrl) {
            mediaBlock = (<Button type="button" data-testid="reader-image" onClick={() => setFullscreen(true)} variant="mediaZoom" size="content" className="mx-auto">
          <img src={mediaUrl} alt={doc.filename} className="max-h-[28rem] w-auto object-contain"/>
        </Button>);
        }
        else if (readerKind === "audio" && mediaUrl) {
            mediaBlock = (
            // biome-ignore lint/a11y/useMediaCaption: user-uploaded audio carries no caption track.
            <audio data-testid="reader-audio" controls src={mediaUrl} className="w-full"/>);
        }
        else if (readerKind === "video" && mediaUrl) {
            mediaBlock = (
            // biome-ignore lint/a11y/useMediaCaption: user-uploaded video carries no caption track.
            <video data-testid="reader-video" controls src={mediaUrl} className="max-h-[28rem] w-full rounded-sm bg-black"/>);
        }
        else if (readerKind === "pdf" && mediaUrl) {
            mediaBlock = (<iframe data-testid="reader-pdf" src={mediaUrl} title={doc.filename} 
            ...
            sandbox="allow-same-origin" className="h-[32rem] w-full rounded-sm border border-border/40 bg-white"/>);
        }
    }
    const hasDocumentActions = Boolean(doc &&
    ...
    const documentMeta = doc
        ...
        : [];
    return (<PagePanel className="settings-surface flex min-h-0 flex-1 flex-col overflow-hidden">
      <div className="custom-scrollbar eliza-chat-scroll min-h-0 flex-1 overflow-y-auto pb-4">
        {loading ? (<div role="status" aria-label={t("appsview.Loading")} className="rounded-[16px] border border-[color:var(--settings-hairline)] bg-[var(--settings-panel)] p-5">
            <DetailSkeleton />
          </div>) : null}

        {error ? (<div className="rounded-[16px] border border-[color:var(--settings-hairline)] bg-[var(--settings-panel)]">
            <PagePanel.ContentState state="error" placement="inset" tone="warning" icon={<AlertTriangle className="size-5"/>} title={error.title} description={error.description} action={error.retryable ? (<Button variant="outline" size="touch" onClick={() => setReloadToken((current) => current + 1)}>
                    {t("common.retry")}
                  </Button>) : undefined}/>
          </div>) : null}

        {!loading && !error && !doc ? (<div className="rounded-[16px] border border-[color:var(--settings-hairline)] bg-[var(--settings-panel)]">
            <PagePanel.ContentState state="empty" placement="inset" title={t("documentsview.NoDocumentSelected", {
        ...
        {!loading && !error && doc ? (<div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
            <SettingsGroup title={t("documentsview.Document", { defaultValue: "Document" })}>
              <div data-slot="settings-row" className="flex min-h-20 items-center gap-3 px-5 py-4">
                <span aria-hidden className="flex size-10 shrink-0 items-center justify-center rounded-[12px] bg-[var(--settings-fill)] text-accent">
              ...
              {hasDocumentActions ? (<div data-slot="settings-row" className="flex min-h-14 flex-wrap items-center gap-2 px-4 py-2">
                  {mediaUrl && readerKind !== "transcript" ? (<Button type="button" variant="outline" size="sm" data-testid="document-fullscreen" aria-pressed={fullscreen} onClick={() => setFullscreen((current) => !current)}>
                  ...
                  {servedFileUrl ? (<Button type="button" variant="outline" size="sm" data-testid="document-download" onClick={() => void handleDownloadFile()}>
                      <Download className="size-4" aria-hidden/>
                      {t("documentsview.Download", {
                        defaultValue: "Download",
                    })}
                    </Button>) : null}
                  {servedFileUrl && shareSupported ? (<Button type="button" variant="outline" size="sm" data-testid="document-share" onClick={() => void handleShareFile()}>
                  ...
                  {doc.canEditText ? (<>
                      ...
                      {editing ? (<Button type="button" size="sm" onClick={() => void handleSave()} disabled={saving || draftText.trim().length === 0}>
            ...
            {mediaBlock ? (<SettingsGroup title={t("documentsview.Preview", { defaultValue: "Preview" })}>
                <div data-testid="reader-media" className="p-4">
                  {mediaBlock}
                </div>
              </SettingsGroup>) : null}

            {transcript?.source === "meeting" && !transcript.redacted ? (<ArtifactPrivacyControls transcript={transcript} onUpdate={handleUpdateTranscriptPrivacy} onDeleteSourceAudio={() => void handleDeleteTranscriptSourceAudio()} saving={privacySaving} error={privacyError}/>) : null}

            {readerKind === "transcript" && transcript ? null : (<SettingsGroup title={t("documentsview.Content", { defaultValue: "Content" })}>
                <div className="p-5">
                  {editing ? (<Textarea value={draftText} rows={16} onChange={(event) => setDraftText(event.target.value)} variant="documentEditor" density="tall"/>) : readableText ? (<div className="whitespace-pre-wrap break-words text-[15px] leading-7 text-[color:var(--settings-foreground)]">
                      {readableText}
                    </div>) : (<PagePanel.ContentState state="empty" placement="inset" title={t("documentsview.NoPreview", {
      ...
      {fullscreen && mediaBlock ? (
        ...
        <div data-testid="reader-fullscreen" className="fixed inset-0 z-50 flex flex-col bg-black/90 p-4 sm:p-8" onClick={() => setFullscreen(false)}>
          <div className="flex justify-end">
            <Button type="button" variant="outline" size="sm" data-testid="reader-fullscreen-close" onClick={(event) => {
