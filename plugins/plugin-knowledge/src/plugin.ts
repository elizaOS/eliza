/**
 * Knowledge plugin document HTTP routes.
 *
 ...
 */
export function resolveTrustedLocalDocumentAccessContext(ctx: Pick<RouteHandlerContext, "accessContext" | "isTrustedLocal" | "runtime">): AccessContext | undefined {
...
function buildCapturedResponse(): {
    ...
    const setHeader = (name: string, value: string | number | readonly string[]): void => {
...
function capturedToResult(captured: CapturedResponse): RouteHandlerResult {
    ...
    const isJson = typeof contentTypeHeader === "string" &&
...
function documentRouteHandler(): (ctx: RouteHandlerContext) => Promise<RouteHandlerResult> {
    return async (ctx: RouteHandlerContext): Promise<RouteHandlerResult> => {
        ...
        await handleDocumentsRoutes({
            ...
            json,
            error,
            readJsonBody: async <T extends object>(req: http.IncomingMessage, _res: http.ServerResponse, options?: Parameters<typeof httpReadJsonBody>[2]): Promise<T | null> => {
...
const DOCUMENT_ROUTES: Array<{
    ...
    { type: "GET", path: "/api/documents/:id/pins" },
    { type: "PATCH", path: "/api/documents/:id/pins" },
    { type: "DELETE", path: "/api/documents/:id" },
    { type: "GET", path: "/api/documents/:id/fragments" },
];
export const documentsRoutes: Route[] = DOCUMENT_ROUTES.map((route) => ({
