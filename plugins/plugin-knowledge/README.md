...
Provides the Knowledge app-shell views and document management REST API in one elizaOS plugin.

The package is `@elizaos/plugin-knowledge`. The document API remains at
`/api/documents`, and the Knowledge app remains at `/documents`. The canonical
runtime export is `knowledgePlugin`; `documentsPlugin` is an alias for callers
that use the document-specific export name.
...
Add `@elizaos/plugin-knowledge` to the agent's plugin list in the character configuration or register it programmatically:
