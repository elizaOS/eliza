...
import { detectCommand, hasCommand, normalizeCommandBody } from "../src/parser";
import {
	findCommandByAlias,
	findCommandByKey,
	getCommands,
...
describe("Command Registry", () => {
	...
	describe("getEnabledCommands", () => {
		it("excludes disabled commands", () => {
			...
			const disabledCount = allCommands.filter(
				(c) => c.enabled === false,
			).length;
		...
		for (const cmd of commands) {
			for (const alias of cmd.textAliases) {
				if (!alias.startsWith("/") && !alias.startsWith("!")) {
					throw new Error(
						`Alias '${alias}' for command '${cmd.key}' should start with / or !`,
					);
				}
			}
		}
