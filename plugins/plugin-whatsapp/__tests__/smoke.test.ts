...
import { sendMessageAction, WHATSAPP_SEND_MESSAGE_ACTION } from "../src/actions/sendMessage";
import { sendReactionAction, WHATSAPP_SEND_REACTION_ACTION } from "../src/actions/sendReaction";
...
describe("@elizaos/plugin-whatsapp", () => {
  ...
  it("does not callback a generic success message after reacting", async () => {
    ...
    const runtime = createRuntime(JSON.stringify({ messageId: "wamid.1", emoji: "👍" }));
    const callback: HandlerCallback = vi.fn(async () => []);
