...
beforeEach(async () => {
  ...
  service = (await LocalFileStorageService.start(
    makeRuntime(storagePath) as unknown as Parameters<typeof LocalFileStorageService.start>[0]
...
describe("LocalFileStorageService", () => {
  ...
  it("uploadBytes writes a file and downloadBytes reads it back unchanged", async () => {
    ...
    expect(uploadResult.url).toBe(`file://${path.join(storagePath, "subdir", "binary.bin")}`);
    const onDisk = await fsp.readFile(path.join(storagePath, "subdir", "binary.bin"));
    expect(onDisk.equals(payload)).toBe(true);

    const fetched = await service.downloadBytes("ignored-bucket", "subdir/binary.bin");
  ...
  it("uploadBytes accepts Uint8Array", async () => {
    ...
    const result = await service.uploadBytes(payload, "u8.bin", "application/octet-stream");
  ...
  it("delete removes a previously-stored object", async () => {
    await service.uploadBytes(Buffer.from("transient"), "doomed.txt", "text/plain");
  ...
  it("uploadJson round-trips through downloadBytes", async () => {
    const result = await service.uploadJson({ foo: "bar", n: 42, list: [1, 2, 3] }, "data.json");
    expect(result.success).toBe(true);
    ...
    expect(JSON.parse(buf.toString("utf8"))).toEqual({ foo: "bar", n: 42, list: [1, 2, 3] });
  });

  it("downloadBytes throws when key is missing", async () => {
    await expect(service.downloadBytes("ignored", "no-such-key")).rejects.toThrow(/not found/);
  });

  it("downloadFile writes the bytes to the requested local path", async () => {
    const payload = Buffer.from("download-me");
    await service.uploadBytes(payload, "to-download.bin", "application/octet-stream");
  ...
  it("generateSignedUrl returns a file:// absolute path under the storage root", async () => {
    ...
    expect(url).toBe(`file://${path.join(storagePath, "nested/dir/asset.png")}`);
  });
});
  ...
  it("falls back to LOCAL_STORAGE_PATH env when getSetting returns nothing", async () => {
    ...
    try {
      ...
      const svc = (await LocalFileStorageService.start(
        runtime as unknown as Parameters<typeof LocalFileStorageService.start>[0]
      )) as LocalFileStorageService;
