...
import { listTelegramPollerHealth } from "./poller-lock";
...
import { type SetupState } from "@elizaos/core";
...
function sendSetupError(res: RouteResponse, status: number, code: string, message: string): void {
...
function isConnectorSetupService(service: unknown): service is ConnectorSetupService {
    ...
    return (typeof candidate.getConfig === "function" &&
...
function readSavedToken(setupService: ConnectorSetupService | null, runtime: IAgentRuntime): string | null {
    if (setupService) {
        ...
        const connectors = (config.connectors ?? {}) as Record<string, Record<string, unknown> | undefined>;
...
function isConfiguredPollerConnected(runtime: IAgentRuntime, token: string | null): boolean {
    const poller = token ? getTelegramPollerClaim(token) : undefined;
    return Boolean(poller?.ok &&
        poller.connected &&
...
async function currentStatus(setupService: ConnectorSetupService | null, runtime: IAgentRuntime): Promise<SetupStatusResponse> {
    const telegram = (setupService?.getConfig().connectors as Record<string, Record<string, unknown>> | undefined)?.telegram;
    ...
    const token = await resolveTelegramBotCredential(runtime, savedToken, "telegram-setup-status");
...
function exclusiveSetup(handler: NonNullable<Route["handler"]>): Route["handler"] {
    return async (req, res, runtime) => {
        if (setupMutations.has(runtime)) {
            sendSetupError(res, 409, "setup_pending", "Another Telegram setup operation is running. Wait and retry.");
            return;
        }
...
function isBotDisconnector(service: unknown): service is BotDisconnector {
    ...
    return (typeof candidate.assertDefaultBotDisconnect === "function" &&
...
async function handleDisconnect(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const body = await readJsonBody<{
        expectedBotId?: number;
    }>(req);
    const expectedBotId = body?.expectedBotId;
    if (typeof expectedBotId !== "number" ||
        !Number.isSafeInteger(expectedBotId) ||
        expectedBotId <= 0) {
        sendSetupError(res, 400, "bad_request", "The displayed bot identity is required. Refresh before disconnecting.");
        return;
    }
    const matchesDisplayedBot = (token: string) => Number(token.split(":", 1)[0]) === expectedBotId;
    const setup = getSetupService(runtime);
    if (!setup) {
        sendSetupError(res, 503, "setup_unavailable", "Restore connector configuration access before disconnecting.");
        return;
    }
    try {
        const connectors = setup.getConfig().connectors as Record<string, Record<string, unknown>> | undefined;
        const config = connectors?.telegram;
        if (!config || config.accounts || config.personal) {
            sendSetupError(res, 409, "disconnect_scope_unsupported", "This control disconnects a managed default bot. Review named and personal accounts separately.");
            return;
        }
        const saved = config.botToken;
        const token = await resolveTelegramBotCredential(runtime, typeof saved === "string" ? saved : null, "telegram-disconnect");
        if (token && !matchesDisplayedBot(token)) {
            sendSetupError(res, 409, "disconnect_target_changed", "The connected bot changed. Refresh before disconnecting.");
            return;
        }
        ...
        if ((runtime.hasService("telegram") &&
            runtime.getServiceRegistrationStatus("telegram") !== "registered") ||
            (!drainer &&
                (service || runtime.hasService("telegram") || hasOwnedPoller()))) {
            sendSetupError(res, 503, "disconnect_service_unavailable", "Load the Telegram bot service before disconnecting so pending work can be drained.");
            return;
        }
        if (!token) {
            if (config.enabled === false && !saved && !hasOwnedPoller()) {
                ...
                sendSetupError(res, 409, "disconnect_identity_unavailable", "The managed bot credential is unavailable. Restore it before disconnecting.");
            }
            return;
        }
        ...
        if (claim &&
            (claim.ownerId !== String(runtime.agentId) ||
                ...
                claim.mode !== "full")) {
            sendSetupError(res, 409, "disconnect_scope_unsupported", "The credential belongs to another polling session. Disconnect it through its owning service.");
            return;
        }
        const updateManaged = (pending: boolean, removeToken = false) => setup.updateConfig((latest) => {
            const current = (latest.connectors as Record<string, Record<string, unknown>> | undefined)?.telegram;
            if (!current ||
                current.botToken !== saved ||
                current.accounts ||
                current.personal) {
                throw new ElizaError("Telegram configuration changed. Refresh before retrying.", { code: "TELEGRAM_DISCONNECT_CONFIG_CHANGED" });
            }
            current.enabled = false;
            if (pending)
                current.disconnectPending = true;
            else
                delete current.disconnectPending;
            if (removeToken)
                delete current.botToken;
        });
        // Persist the restart barrier before draining; failure keeps the token for retry.
        if (drainer)
            drainer.assertDefaultBotDisconnect(token);
        updateManaged(true);
        if (drainer)
            await drainer.disconnectDefaultBot(token);
        ...
        const credentialRetained = typeof saved === "string" && saved.startsWith("vault://")
        ...
        sendSetupError(res, 503, "disconnect_incomplete", "Telegram disconnect did not complete. Refresh its status and retry.");
    }
}
// ── GET /api/setup/telegram/status ──────────────────────────────────
async function handleStatus(_req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    ...
    try {
        ...
        sendSetupError(res, 503, "credential_unavailable", "Telegram readiness could not be checked. Restore access to the configured credential and retry.");
    }
}
// ── POST /api/setup/telegram/start ──────────────────────────────────
async function handleStart(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const body = await readJsonBody<{
        token?: string;
    }>(req);
    ...
    if (!/^\d+:[A-Za-z0-9_-]{30,}$/.test(token)) {
        sendSetupError(res, 400, "bad_request", "Token format invalid. Expected format: 123456:ABC-DEF...");
        return;
    }
    let apiRes: Response;
    try {
        ...
        sendSetupError(res, 502, "upstream_unreachable", `Failed to reach Telegram API: ${message}`);
        return;
    }
    if (!apiRes.ok) {
        sendSetupError(res, 502, "upstream_unreachable", `Telegram API returned ${apiRes.status}. Check that the token is correct.`);
        return;
    }
    ...
    if (!data.ok || !data.result) {
        sendSetupError(res, 502, "upstream_unreachable", "Telegram API returned unexpected response");
        return;
    }
    ...
    if (setupService) {
        ...
        logger.warn("[telegram-setup] connector-setup service not available — token saved to runtime only");
    }
    sendStatus(res, {
        connector: "telegram",
        state: "configuring",
...
async function handleCancel(_req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const setupService = getSetupService(runtime);
    if (setupService) {
