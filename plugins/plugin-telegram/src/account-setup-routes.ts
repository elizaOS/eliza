 ...
 */
import { TelegramAccountAuthSession } from "./account-auth-service.js";
import { clearTelegramAccountAuthState } from "./account-auth-service.js";
import { clearTelegramAccountSession } from "./account-auth-service.js";
import { defaultTelegramAccountDeviceModel } from "./account-auth-service.js";
import { defaultTelegramAccountSystemVersion } from "./account-auth-service.js";
import { resolveTelegramAppCredentials } from "./account-credentials.js";
import { telegramAccountAuthStateExists } from "./account-auth-service.js";
import { telegramAccountSessionExists } from "./account-auth-service.js";
...
import { type TelegramAccountAuthSessionLike } from "./account-auth-service.js";
import { type TelegramAccountAuthSnapshot } from "./account-auth-service.js";
import { type TelegramAccountSessionScope } from "./account-auth-service.js";
...
export async function stopTelegramAccountAuthSession(runtime?: IAgentRuntime): Promise<void> {
...
// ── Config helpers ─────────────────────────────────────────────────────
function readConnectorConfig(config: Record<string, unknown>): Record<string, unknown> {
...
function hasConfiguredTelegramAccount(connConfig: Record<string, unknown>): boolean {
    return Boolean(typeof connConfig.phone === "string" &&
...
function resolveConfiguredPhone(runtime: IAgentRuntime, connConfig: Record<string, unknown>): string | null {
    if (typeof connConfig.phone === "string" &&
...
function resolveService(runtime: IAgentRuntime): TelegramAccountRuntimeServiceLike | null {
    const service = runtime.getService("telegram-account");
    return ((service as TelegramAccountRuntimeServiceLike | null | undefined) ?? null);
}
function isServiceConnected(service: TelegramAccountRuntimeServiceLike | null): boolean {
    if (!service) {
        return false;
    }
...
function setupStateFromFlow(flowStatus: string, configured: boolean, sessionExists: boolean, serviceConnected: boolean): SetupState {
    if (flowStatus === "error")
        return "error";
    if (serviceConnected)
        return "paired";
    if (flowStatus === "waiting_for_provisioning_code" ||
...
function statusFromState(runtime: IAgentRuntime, config: Record<string, unknown>): TelegramAccountStatusResponse {
    ...
    const serviceAccount = typeof service?.getAccountSummary === "function"
        ? service.getAccountSummary()
        : null;
    const fallbackPhone = resolveConfiguredPhone(runtime, connectorConfig);
    let flowStatus = (serviceError ? "error" : authSnapshot?.status) ??
        (serviceConnected
            ? "connected"
    ...
    const state = setupStateFromFlow(flowStatus, configured, sessExists, serviceConnected);
    return {
        connector: "telegram-account",
        state,
        detail: {
            ...
            hasAppCredentials: Boolean((typeof connectorConfig.appId === "string" ||
...
function ensureConnectorBlock(config: Record<string, unknown>): Record<string, unknown> {
...
function createSessionOptions(config: Record<string, unknown>, runtime: IAgentRuntime): {
    ...
    return {
        ...
        deviceModel: typeof connectorConfig.deviceModel === "string" &&
            connectorConfig.deviceModel.trim().length > 0
            ? connectorConfig.deviceModel.trim()
            : defaultTelegramAccountDeviceModel(),
        systemVersion: typeof connectorConfig.systemVersion === "string" &&
            connectorConfig.systemVersion.trim().length > 0
            ? connectorConfig.systemVersion.trim()
            : defaultTelegramAccountSystemVersion(),
    };
}
function ensureAuthSession(config: Record<string, unknown>, runtime: IAgentRuntime): TelegramAccountAuthSessionLike | null {
    const existing = authSessions.get(runtime);
    if (existing)
        return existing;
    if (!telegramAccountAuthStateExists(sessionScope(runtime)))
        return null;
    const session = new TelegramAccountAuthSession(createSessionOptions(config, runtime));
...
async function handleStart(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    ...
    if (!phone) {
        sendSetupError(res, 400, "bad_request", "telegram phone number is required");
        return;
    }
    await resolveService(runtime)?.stopAccount?.();
    await authSessions.get(runtime)?.stop();
    const telegramAccountAuthSession = new TelegramAccountAuthSession(createSessionOptions(config, runtime));
    ...
    try {
        ...
        sendSetupError(res, 500, "internal_error", err instanceof Error ? err.message : String(err));
    }
}
async function handleSubmitCode(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const body = (req.body ?? {}) as {
    ...
    if (!telegramAccountAuthSession) {
        sendSetupError(res, 400, "bad_request", "telegram login session has not been started");
        return;
    }
    try {
...
async function handleCancel(_req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const setupService = getSetupService(runtime);
    if (!setupService) {
        sendSetupError(res, 503, "setup_unavailable", "Restore connector configuration storage before disconnecting this account.");
        return;
    }
...
function serializeAuthRoute(handler: typeof handleStart): typeof handleStart {
    return async (req, res, runtime) => {
        const pending = (authOperations.get(runtime) ?? Promise.resolve()).then(() => handler(req, res, runtime));
        // error-policy:J5 HTTP handler awaits the original rejection; tail only serializes subsequent requests.
        authOperations.set(runtime, pending.then(() => undefined, () => undefined));
        await pending;
    };
}
