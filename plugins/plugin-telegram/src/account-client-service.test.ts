...
import { InMemoryConnectorAccountStorage } from "@elizaos/core";
...
import { loadTelegramAccountSessionString } from "./account-auth-service";
import { readBigIntFromBuffer } from "telegram/Helpers.js";
import { saveTelegramAccountSessionString } from "./account-auth-service";
...
import { type TelegramAccountClientDeps } from "./account-client-service";
...
afterEach(() => {
    ...
    for (const key of [
        ...
        if (oldEnvironment[key] === undefined)
            delete process.env[key];
        else
            process.env[key] = oldEnvironment[key];
    }
    for (const directory of directories.splice(0))
        fs.rmSync(directory, { recursive: true, force: true });
});
async function harness(binding = true, options: {
    ...
    const deps: TelegramAccountClientDeps = {
        createClient(saved, credentials) {
            const client = new TelegramClient(saved, credentials.apiId, credentials.apiHash, {});
            ...
            vi.spyOn(client, "getMe").mockResolvedValue(options.identity ??
                new Api.User({
                    id: userId,
                    phone: "15551234567",
                    firstName: "Owner",
                }));
            vi.spyOn(client, "getInputEntity").mockResolvedValue(new Api.InputPeerUser({ userId, accessHash: userId }));
            vi.spyOn(client, "invoke").mockImplementation(async (request) => {
                ...
                return new Api.messages.Messages({
                    messages: rows.map((id) => new Api.Message({
                        id,
                        date: 1700000000 + id,
    ...
    vi.spyOn(runtime, "getService").mockImplementation((type) => type === "telegram-account" ? service : original(type));
...
describe("personal service account-bound history", () => {
    it("connects a saved session and reaches the provider final page without bot cache", async () => {
        ...
        const result = await connector.fetchMessages({ runtime, accountId: "me:personal", target }, { target });
        ...
        expect(result.every((row) => row.metadata?.accountId === "me:personal")).toBe(true);
        expect(loadTelegramAccountSessionString({
    ...
    it("refuses an unbound owner before history RPC", async () => {
        ...
        await service.refreshAccount("me:personal");
        await expect(service.fetchConnectorMessages({ runtime, accountId: "me:personal", target }, { target })).rejects.toMatchObject({ code: "TELEGRAM_ACCOUNT_READ_DENIED" });
    ...
    it("rejects conflicting account selection and refuses reads after stop", async () => {
        const { runtime, service, clients } = await harness();
        await service.refreshAccount("me:personal");
        await expect(service.fetchConnectorMessages({ runtime, accountId: "another:personal", target }, { target })).rejects.toMatchObject({ code: "TELEGRAM_ACCOUNT_MISMATCH" });
        expect(clients[0].invoke).not.toHaveBeenCalled();
        await service.stop();
        await expect(service.fetchConnectorMessages({ runtime, accountId: "me:personal", target }, { target })).rejects.toMatchObject({ code: "TELEGRAM_ACCOUNT_NOT_CONNECTED" });
    });
    it("reconstructs the service from encrypted session storage and retains provider identities", async () => {
        ...
        await service.refreshAccount("me:personal");
        const first = await service.fetchConnectorMessages({ runtime, accountId: "me:personal", target }, { target });
        await service.stop();
        ...
        if (!fallback)
            throw new Error("Runtime fixture lookup required");
        vi.spyOn(runtime, "getService").mockImplementation((type) => type === "telegram-account" ? replacement : fallback(type));
        await replacement.refreshAccount("me:personal");
        const second = await replacement.fetchConnectorMessages({ runtime, accountId: "me:personal", target }, { target });
    ...
    it("does not return a partial result if disconnect commits during an RPC", async () => {
        ...
        if (!original)
            throw new Error("RPC fixture implementation required");
    ...
    it("searches the complete peer before applying the requested match count", async () => {
        ...
        await service.refreshAccount("me:personal");
        const result = await service.searchConnectorMessages({ runtime, accountId: "me:personal", target }, { target, query: "final-page", limit: 1 });
    ...
    it("does not turn an unsupported room-only scope into an account-wide search", async () => {
        ...
        await expect(service.searchConnectorMessages({ runtime, accountId: "me:personal" }, { roomId: "unmapped-room", query: "secret" })).rejects.toMatchObject({ code: "TELEGRAM_HISTORY_TARGET_INVALID" });
    ...
    it("rejects conflicting context and request targets before history RPCs", async () => {
        ...
        try {
            ...
            for (const { contextTarget, params, kind } of cases) {
                await expect(service.fetchConnectorMessages({ runtime, accountId: "me:personal", target: contextTarget }, params)).rejects.toMatchObject({
            ...
            const rows = await service.fetchConnectorMessages({ runtime, accountId: "me:personal", target: matching }, {
                target: matching,
            ...
            expect(clients[0].invoke).toHaveBeenCalledWith(expect.any(Api.messages.GetReplies));
        }
        finally {
            await service.stop();
        }
    ...
    ])("rejects a mismatched $field before registering or reading history", async ({ identity }) => {
    ...
    it("persists personal disconnect across reconstruction despite fallback character credentials", async () => {
        ...
        if (!stateDir)
            throw new Error("Fixture state directory required");
        ...
        const setup = {
            getConfig: (): Record<string, unknown> => JSON.parse(fs.readFileSync(configPath, "utf8")),
            persistConfig: (config: Record<string, unknown>) => fs.writeFileSync(configPath, JSON.stringify(config)),
        ...
        if (!lookup)
            throw new Error("Runtime service fixture lookup required");
        vi.mocked(runtime.getService).mockImplementation((type) => type === "connector-setup" ? (setup as never) : lookup(type));
        ...
        const route = telegramAccountRoutes.find((entry) => entry.path === "/api/setup/telegram-account/cancel");
        if (!route)
            throw new Error("Personal cancel route required");
        let status = 0;
        const response = {
            status(code: number) {
                status = code;
        ...
        try {
            await route.handler({} as RouteRequest, response as RouteResponse, runtime);
    ...
    it("keeps identical peer/message numbers distinct across explicitly configured accounts", async () => {
        ...
        const first = await service.fetchConnectorMessages({ runtime, accountId: "me:personal", target }, { target, limit: 1 });
        const otherTarget = { ...target, accountId: "other:personal" };
        const second = await service.fetchConnectorMessages({ runtime, accountId: "other:personal", target: otherTarget }, { target: otherTarget, limit: 1 });
        expect(first[0].metadata?.platformMessageId).toBe(second[0].metadata?.platformMessageId);
