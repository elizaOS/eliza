...
- **ConnectorAccountManager**: registered in `init()`. Telegram bot-token accounts do not use OAuth; `startOAuth`/`completeOAuth` are unsupported for this provider. Single-account env configs are surfaced as a synthetic `"default"` account.
