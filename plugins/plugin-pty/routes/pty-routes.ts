 ...
 */
import { ELIZA_CLOUD_DEFAULT_BASE_URL } from "../lib/eliza-code-spec";
import { buildClaudeCliSpec } from "../lib/vendor-cli-spec";
import { buildCodexCliSpec } from "../lib/vendor-cli-spec";
import { buildElizaCodeCerebrasSpec } from "../lib/eliza-code-spec";
import { logger } from "@elizaos/core";
import { resolveClaudeCliBin } from "../lib/vendor-cli-spec";
import { resolveCodexCliBin } from "../lib/vendor-cli-spec";
import { resolveElizaCodeBin } from "../lib/eliza-code-spec";
import { resolveElizaCodeCloudTuple } from "../lib/eliza-code-spec";
import { timingSafeEqual } from "node:crypto";
import { type ElizaCodeCloudTuple } from "../lib/eliza-code-spec";
import { type IAgentRuntime } from "@elizaos/core";
import { type PtyService } from "../services/pty-service";
import { type PtySpawnSpec } from "../services/pty-types";
import { type PtyVendorCliKind } from "../lib/vendor-cli-spec";
...
function allowedBaseUrls(runtime: IAgentRuntime): Set<string> {
    ...
    return new Set([ELIZA_CLOUD_DEFAULT_BASE_URL, ...configured].map((url) => normalizeBaseUrl(url)));
}
function resolveAllowedBaseUrl(runtime: IAgentRuntime, requested: string | undefined): string | undefined {
    if (!requested)
        return undefined;
    const normalized = normalizeBaseUrl(requested);
    if (!allowedBaseUrls(runtime).has(normalized)) {
        throw new Error(`Unsupported PTY baseUrl "${requested}". Configure PTY_ALLOWED_BASE_URLS to allow it explicitly.`);
    }
    return normalized;
}
function providedTerminalToken(ctx: RouteHandlerContext, body: Record<string, unknown> = {}): string | undefined {
    return (header(ctx, "x-eliza-terminal-token") ??
        header(ctx, "x-pty-terminal-token") ??
        bodyToken(body) ??
        query(ctx, "terminalToken") ??
        query(ctx, "ptyToken"));
}
function ptyAccessRejection(ctx: RouteHandlerContext, body: Record<string, unknown> = {}): RouteHandlerResult | null {
    const expected = getStr(ctx.runtime, "ELIZA_TERMINAL_RUN_TOKEN") ??
    ...
    if (!expected) {
        if (ctx.inProcess || ctx.isTrustedLocal)
            return null;
        return json(403, {
            error: "Interactive PTY routes require a terminal token (ELIZA_TERMINAL_RUN_TOKEN) for HTTP access.",
        });
    }
    if (ctx.isTrustedLocal)
        return null;
    const provided = providedTerminalToken(ctx, body);
    if (!provided) {
        return json(401, {
            error: "Missing terminal token. Provide X-Eliza-Terminal-Token or terminalToken.",
        });
    }
...
function resolveRequestCloudTuple(runtime: IAgentRuntime, body: Record<string, unknown>): ElizaCodeCloudTuple {
    ...
    if (launcherTuple.authority)
        return launcherTuple;
 ...
 */
function elizaCodeSpecFromRequest(runtime: IAgentRuntime, body: Record<string, unknown>, cwd: string, cloud: ElizaCodeCloudTuple): PtySpawnSpec {
    ...
    return buildElizaCodeCerebrasSpec({
        ...
        fastModel: str(body.fastModel) ?? getStr(runtime, "PTY_ELIZA_CLOUD_FAST_MODEL"),
        smartModel: str(body.smartModel) ?? getStr(runtime, "PTY_ELIZA_CLOUD_SMART_MODEL"),
    });
}
/**
 ...
 */
function vendorCliSpecFromRequest(runtime: IAgentRuntime, kind: PtyVendorCliKind, cwd: string): PtySpawnSpec {
    if (kind === "claude") {
...
async function spawnHandler(ctx: RouteHandlerContext): Promise<RouteHandlerResult> {
    ...
    if (!interactiveEnabled(runtime)) {
        return json(403, {
            error: "Interactive PTY sessions are disabled (PTY_INTERACTIVE_ENABLED=false or store build).",
        });
    }
    const svc = getService(ctx);
    if (!svc)
        return json(503, { error: "PTY_SERVICE is not available." });
    ...
    if (kind !== "eliza-code" && !vendorCliEnabled(runtime)) {
        return json(403, {
            error: `Interactive "${kind}" CLI sessions are an experimental tier that is off by default ` +
    ...
    try {
        ...
        logger.info(`[plugin-pty] spawned interactive session ${session.sessionId} kind=${kind} label=${spec.label} cwd=${cwd}`);
...
async function bufferedOutputHandler(ctx: RouteHandlerContext): Promise<RouteHandlerResult> {
    const rejection = ptyAccessRejection(ctx);
    if (rejection)
        return rejection;
    const svc = getService(ctx);
