...
class SearchFilesAction:
    ...
    async def handler(self, message: Message, state: dict, service: CoderService | None = None) -> ActionResult:
        ...
        max_matches = int(state.get("max_matches", 50)) if str(state.get("max_matches", "")).strip() else 50

        if not pattern:
            return ActionResult(False, "Missing pattern.", "missing_pattern")

        ok, res = await service.search_files(conversation_id(message), pattern, dirpath, max_matches)
        if not ok:
