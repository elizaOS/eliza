...
function buildStatusResponse(runtime: IAgentRuntime): SetupStatusResponse<DiscordServiceStatusShape> {
...
async function handleStart(_req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const service = resolveService(runtime);
    if (!service) {
        res
            .status(503)
    ...
    try {
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `failed to authorize discord-local: ${err instanceof Error ? err.message : String(err)}`));
    }
}
...
async function handleCancel(_req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    ...
    try {
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `failed to disconnect discord-local: ${err instanceof Error ? err.message : String(err)}`));
    }
}
/**
