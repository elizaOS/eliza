...
function isDiscordLocalServiceLike(service: unknown): service is DiscordLocalServiceLike {
    if (!service || typeof service !== "object")
        return false;
    const candidate = service as Record<string, unknown>;
...
function resolveService(runtime: IAgentRuntime): DiscordLocalServiceLike | null {
...
function getConnectorConfig(setupService: ConnectorSetupService): ConnectorConfig {
    ...
    const connectors = (config.connectors as Record<string, ConnectorConfig> | undefined) ??
...
async function handleGuilds(_req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    ...
    try {
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `failed to list discord-local guilds: ${err instanceof Error ? err.message : String(err)}`));
    }
}
// ── GET /api/discord/channels ───────────────────────────────────────────
async function handleChannels(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const service = resolveService(runtime);
    if (!service) {
        res
            .status(503)
    ...
    const url = new URL((req as {
        url?: string;
    }).url ?? "/api/discord/channels", "http://localhost");
    ...
    if (!isValidSnowflake(guildId)) {
        res
            .status(400)
            .json(buildSetupError("bad_request", "guildId must be a Discord snowflake"));
        return;
    }
    try {
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `failed to list discord-local channels: ${err instanceof Error ? err.message : String(err)}`));
    }
}
// ── POST /api/discord/subscriptions ─────────────────────────────────────
async function handleSubscriptions(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const service = resolveService(runtime);
    if (!service) {
        res
            .status(503)
    ...
    if (invalidChannelIds.length > 0) {
        res
            .status(400)
            .json(buildSetupError("bad_request", "channelIds must contain only Discord snowflakes"));
        return;
    }
    try {
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `failed to update discord-local subscriptions: ${err instanceof Error ? err.message : String(err)}`));
    }
}
/**
