 ...
 */
export function getMultiAccountConfig(runtime: IAgentRuntime): DiscordMultiAccountConfig {
...
function mergeDiscordAccountConfig(
  ...
  const envListenChannelIds = runtime.getSetting("DISCORD_LISTEN_CHANNEL_IDS") as
    | string
    | undefined;

  const envConfig: DiscordAccountConfig = {
    shouldIgnoreBotMessages:
      (runtime.getSetting("DISCORD_SHOULD_IGNORE_BOT_MESSAGES") as string)?.toLowerCase() ===
      "true",
    shouldIgnoreDirectMessages:
      (runtime.getSetting("DISCORD_SHOULD_IGNORE_DIRECT_MESSAGES") as string)?.toLowerCase() ===
      "true",
    shouldRespondOnlyToMentions:
      (runtime.getSetting("DISCORD_SHOULD_RESPOND_ONLY_TO_MENTIONS") as string)?.toLowerCase() ===
      "true",
...
export function resolveDiscordAccount(
  ...
  const tokenResolution = resolveDiscordToken(runtime, { accountId: normalizedAccountId });
 ...
 */
export function listEnabledDiscordAccounts(runtime: IAgentRuntime): ResolvedDiscordAccount[] {
