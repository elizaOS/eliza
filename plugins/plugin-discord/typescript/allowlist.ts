...
function resolveChannelEntryMatch(
  ...
  keys: string[],
  parentKeys?: string[]
): { entry: DiscordGuildChannelConfig; matchKey: string; matchSource: ChannelMatchSource } | null {
...
export function validateMessageAllowed(params: {
  ...
  const { accountConfig, isDirectMessage, isGroupDm, guild, channelId, channelName, author } =
    params;
