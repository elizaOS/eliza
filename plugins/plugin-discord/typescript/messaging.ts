 ...
 */
export function resolveTimestampMs(timestamp?: string | null): number | undefined {
  if (!timestamp) {
    return undefined;
  }
 ...
 */
export function truncateUtf16Safe(text: string, maxLength: number, ellipsis = "…"): string {
  if (text.length <= maxLength) {
    return text;
  }
