...
function route(path: string, type: string) {
    const found = discordDataRoutes.find((candidate) => candidate.path === path && candidate.type === type);
    if (!found) {
...
function makeRuntime(service: Record<string, unknown>): IAgentRuntime {
    return {
        getService: vi.fn((serviceName: string) => serviceName === DISCORD_LOCAL_SERVICE_NAME ? service : null),
...
describe("discordDataRoutes", () => {
    it("rejects malformed guildId before calling the local service", async () => {
        ...
        await route("/api/discord/channels", "GET").handler({ url: "/api/discord/channels?guildId=../../etc/passwd" } as RouteRequest, res, makeRuntime(service));
    ...
    it("passes a valid guild snowflake through to listChannels", async () => {
        ...
        } as RouteRequest, res, makeRuntime(service));
