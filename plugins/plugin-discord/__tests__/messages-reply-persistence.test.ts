...
describe("Discord reply persistence keeps one row per reply", () => {
	...
	it.each([undefined, "invalid-response-id"])(
		"falls back to a delivery-derived row for unusable canonical id %s",
		async (responseId) => {
			const { writes, sentIds } = await deliver(
				{ text: "Sure thing, legacy row.", channelType: "DM", responseId },
				null,
			);
			...
			expect(writes.created[0].metadata).not.toHaveProperty(
				"platformMessageIds",
			);
		},
	);
