...
export type DiscordProfileSyncOptions = Pick<FetchMediaOptions, "fetchImpl" | "lookupFn" | "pinnedFetchImpl">;
function resolveProfileSyncStatePath(env: NodeJS.ProcessEnv = process.env): string {
    return path.join(resolveStateDir(env), "cache", PROFILE_SYNC_STATE_FILE);
}
async function readPersistedProfileSyncState(env: NodeJS.ProcessEnv = process.env): Promise<PersistedDiscordProfileSyncState> {
    try {
...
async function writePersistedProfileSyncState(state: PersistedDiscordProfileSyncState, env: NodeJS.ProcessEnv = process.env): Promise<void> {
...
function normalizeDesiredDiscordName(runtime: IAgentRuntime, settings: DiscordSettings): string | undefined {
...
function readNestedOptionalString(value: unknown, pathSegments: string[]): string | undefined {
...
function normalizeDesiredDiscordAvatarSource(runtime: IAgentRuntime, settings: DiscordSettings): DesiredDiscordAvatar {
    ...
    const fromIdentity = readNestedOptionalString(character, ["identity", "avatar"]) ??
        readNestedOptionalString(character, ["settings", "identity", "avatar"]);
    if (fromIdentity) {
        return { source: fromIdentity, explicit: true };
    }
    const fromCharacter = readNestedOptionalString(character, ["avatar"]) ??
...
async function readAvatarBytesFromLocalCandidates(source: string): Promise<Buffer> {
    ...
    for (const [candidateIndex, candidate] of candidates.entries()) {
        try {
            ...
            const fsCode = error instanceof Error &&
                "code" in error &&
            ...
            const sanitizedCause = Object.assign(new Error(fsCode
                ? `Filesystem read failed (${fsCode}).`
                : "Filesystem read failed."), fsCode ? { code: fsCode } : {});
            throw new ElizaError("A Discord profile avatar candidate could not be read.", {
    ...
    throw new ElizaError(`Discord profile avatar was not found in ${candidates.length} local candidate path(s).`, {
        code: "DISCORD_PROFILE_AVATAR_NOT_FOUND",
        context: { candidateCount: candidates.length },
        severity: "ephemeral",
    });
}
async function loadDiscordProfileAvatarBytes(source: string, fetchOptions: DiscordProfileSyncOptions): Promise<{
    bytes: Buffer;
    hash: string;
} | null> {
    const trimmed = source.trim();
...
export async function syncDiscordClientProfile(runtime: IAgentRuntime, clientUser: Pick<ClientUser, "username"> & {
    setAvatar?: (avatar: Buffer | string | null) => Promise<unknown>;
    setUsername?: (username: string) => Promise<unknown>;
}, settings: DiscordSettings, options: DiscordProfileSyncOptions = {}): Promise<void> {
    ...
    if (desiredAvatar.source) {
        let avatar: Awaited<ReturnType<typeof loadDiscordProfileAvatarBytes>> = null;
        try {
            avatar = await loadDiscordProfileAvatarBytes(desiredAvatar.source, options);
        }
        catch (error) {
            if (desiredAvatar.explicit ||
            ...
            runtime.logger.debug({ src: "plugin:discord", agentId: runtime.agentId }, "No bundled default Discord avatar in this checkout; skipping avatar sync");
        }
