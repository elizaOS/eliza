...
describe("UserLpProfileService with In-Memory Storage", () => {
  ...
  const _defaultAutoRebalanceConfig: UserLpProfile["autoRebalanceConfig"] = {
    enabled: false,
    minGainThresholdPercent: 0.5,
    maxSlippageBps: 50,
    preferredDexes: [],
    cycleIntervalHours: 1,
    maxGasFeeLamports: "100000",
  };
  ...
  describe("updateProfile", () => {
    it("should update profile and merge autoRebalanceConfig", async () => {
      ...
      const initialProfile = await profileService.ensureProfile(
        testUserId1,
        testVaultPk1,
        testEncryptedKey1,
  ...
  describe("getTrackedPositions", () => {
    it("should return tracked positions for a user", async () => {
      ...
      const pos2: TrackedLpPositionInput = {
        positionIdentifier: "p2",
        dex: "d2",
        poolAddress: "pa2",
      };
