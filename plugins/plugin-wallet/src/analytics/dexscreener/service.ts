...
import { type DexScreenerChainParams } from "./types";
import { type DexScreenerConfig } from "./types";
import { type DexScreenerNewPairsParams } from "./types";
import { type DexScreenerOrder } from "./types";
import { type DexScreenerPair } from "./types";
import { type DexScreenerPairParams } from "./types";
import { type DexScreenerProfile } from "./types";
import { type DexScreenerSearchParams } from "./types";
import { type DexScreenerServiceResponse } from "./types";
import { type DexScreenerTokenParams } from "./types";
import { type DexScreenerTrendingParams } from "./types";
...
export class DexScreenerService extends Service {
    ...
    static async start(runtime: IAgentRuntime): Promise<Service> {
        ...
        if (customBase.length > 0) {
            ...
            const cloud = cloudServiceApisBaseUrl(toWalletCloudRoutingSettings(runtime), "dexscreener");
            if (cloud !== null) {
    ...
    async search(params: DexScreenerSearchParams): Promise<DexScreenerServiceResponse<DexScreenerPair[]>> {
        try {
    ...
    async getTokenPairs(params: DexScreenerTokenParams): Promise<DexScreenerServiceResponse<DexScreenerPair[]>> {
        try {
            await this.rateLimit();
            const data = await this.get<{
                pairs?: DexScreenerPair[];
    ...
    async getPair(params: DexScreenerPairParams): Promise<DexScreenerServiceResponse<DexScreenerPair>> {
        try {
            await this.rateLimit();
            const data = await this.get<{
    ...
    async getTrending(params: DexScreenerTrendingParams = {}): Promise<DexScreenerServiceResponse<DexScreenerPair[]>> {
        try {
            await this.rateLimit();
            // DexScreener has no direct trending endpoint; use top boosted tokens
            // as a proxy signal.
            const responseData = await this.get<DexScreenerBoostedToken[] | DexScreenerBoostedToken>(`/token-boosts/top/v1`);
            ...
            const pairPromises = boostedTokens
                ...
                .map(async (token) => {
                try {
                    const pairData = await this.get<TokensV1Wire>(`/tokens/v1/${token.chainId}/${token.tokenAddress}`);
            ...
            return {
                success: false,
                error: dexScreenerErrorMessage(caught) || "Failed to get trending pairs",
            };
        }
    }
    async getPairsByChain(params: DexScreenerChainParams): Promise<DexScreenerServiceResponse<DexScreenerPair[]>> {
        try {
            await this.rateLimit();
            ...
            if (params.sortBy) {
                pairs.sort((a, b) => {
                    switch (params.sortBy) {
                        ...
                        case "txns":
                            return ((b.txns.h24.buys + b.txns.h24.sells || 0) -
            ...
            return {
                success: false,
                error: dexScreenerErrorMessage(caught) || "Failed to get pairs by chain",
            };
        }
    }
    async getNewPairs(params: DexScreenerNewPairsParams = {}): Promise<DexScreenerServiceResponse<DexScreenerPair[]>> {
        try {
            await this.rateLimit();
            // DexScreener has no direct new-pairs endpoint; use the latest token
            // profiles as a proxy for newly listed tokens.
            const responseData = await this.get<DexScreenerProfile[] | DexScreenerProfile>(`/token-profiles/latest/v1`);
            ...
            const pairPromises = filteredProfiles
                ...
                .map(async (profile) => {
                try {
                    const pairData = await this.get<TokensV1Wire>(`/tokens/v1/${profile.chainId}/${profile.tokenAddress}`);
    ...
    async getTokenProfile(tokenAddress: string): Promise<DexScreenerServiceResponse<DexScreenerProfile>> {
        try {
            await this.rateLimit();
    ...
    async getMultipleTokens(chainId: string, tokenAddresses: string[]): Promise<DexScreenerServiceResponse<DexScreenerPair[]>> {
        try {
            ...
            const data = await this.get<DexScreenerPair[] | DexScreenerPair>(`/tokens/v1/${chainId}/${addresses}`);
            return {
                success: true,
            ...
            return {
                success: false,
                error: dexScreenerErrorMessage(caught) || "Failed to get multiple tokens",
            };
        }
    }
    async getLatestTokenProfiles(): Promise<DexScreenerServiceResponse<DexScreenerProfile[]>> {
        try {
            await this.rateLimit();
            const data = await this.get<DexScreenerProfile[] | DexScreenerProfile>(`/token-profiles/latest/v1`);
            return {
                success: true,
            ...
            return {
                success: false,
                error: dexScreenerErrorMessage(caught) ||
                    "Failed to get latest token profiles",
            };
        }
    }
    async getLatestBoostedTokens(): Promise<DexScreenerServiceResponse<DexScreenerBoostedWire[]>> {
        try {
            await this.rateLimit();
            const data = await this.get<DexScreenerBoostedWire[] | DexScreenerBoostedWire>(`/token-boosts/latest/v1`);
            return {
                success: true,
                data: Array.isArray(data) ? data : [data],
            };
    ...
    async getTopBoostedTokens(): Promise<DexScreenerServiceResponse<DexScreenerBoostedWire[]>> {
        try {
            await this.rateLimit();
            const data = await this.get<DexScreenerBoostedWire[] | DexScreenerBoostedWire>(`/token-boosts/top/v1`);
            return {
                success: true,
                data: Array.isArray(data) ? data : [data],
            };
            ...
            return {
                success: false,
                error: dexScreenerErrorMessage(caught) || "Failed to get top boosted tokens",
            };
        }
    }
    async checkOrderStatus(chainId: string, tokenAddress: string): Promise<DexScreenerServiceResponse<DexScreenerOrder[]>> {
        try {
            await this.rateLimit();
            const data = await this.get<DexScreenerOrder[] | DexScreenerOrder>(`/orders/v1/${chainId}/${tokenAddress}`);
            return {
                success: true,
                data: Array.isArray(data) ? data : data ? [data] : [],
            };
            ...
            return {
                success: false,
                error: dexScreenerErrorMessage(caught) || "Failed to check order status",
            };
        }
    }
    async getTokenPairsByChain(chainId: string, tokenAddress: string): Promise<DexScreenerServiceResponse<DexScreenerPair[]>> {
        try {
            await this.rateLimit();
            const data = await this.get<DexScreenerPair[] | DexScreenerPair>(`/token-pairs/v1/${chainId}/${tokenAddress}`);
            return {
                success: true,
                data: Array.isArray(data) ? data : data ? [data] : [],
            };
