...
export class BirdeyeProvider {
    ...
    public async fetchTokenListV2_POST(params: FetchParams<Record<string, never>>) {
     ...
     */
    public async fetchWalletSupportedNetworks(options: { headers?: Record<string, string> } = {}) {
        return this.fetchWithCacheAndRetry<DefiNetworksResponse>({
            url: BIRDEYE_ENDPOINTS.defi.networks,
            headers: options.headers,
        });
