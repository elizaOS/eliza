...
import { parseTransferParams } from "../types";
...
export async function buildTransferDetails(state: State, message: Memory, runtime: IAgentRuntime, wp: WalletProvider): Promise<TransferParams> {
    ...
    if (!parsedResponse) {
        throw new EVMError(EVMErrorCode.INVALID_PARAMS, "Failed to parse structured response from LLM for transfer details.");
    }
    const rawParams = {
    ...
    if (!existingChain) {
        throw new EVMError(EVMErrorCode.CHAIN_NOT_CONFIGURED, `Chain "${transferDetails.fromChain}" not configured. Available chains: ${chains.toString()}`);
    }
