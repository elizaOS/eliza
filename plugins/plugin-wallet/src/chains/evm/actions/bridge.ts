...
import { confirmationRequired, createEvmActionValidator, isConfirmed } from "./helpers";
...
export const bridgeAction = {
  ...
  handler: async (
    ...
    if (!isConfirmed(options)) {
      const recipientText = bridgeOptions.toAddress ? ` to ${bridgeOptions.toAddress}` : "";
      const preview = `Review EVM bridge before submitting: ${bridgeOptions.amount} ${bridgeOptions.fromToken} from ${bridgeOptions.fromChain} to ${bridgeOptions.toToken} on ${bridgeOptions.toChain}${recipientText}. Re-invoke ${spec.name} with confirmed: true to submit.`;
  ...
  validate: createEvmActionValidator({
    keywords: ["bridge", "cross-chain", "cross chain", "from", "to"],
    regex: /\b(?:bridge|cross[-\s]?chain|transfer)\b/i,
  }),
    ...
    [
      {
        ...
        user: "user",
        content: {
          text: "Please move 250 USDC cross-chain from Arbitrum to Optimism",
          action: "CROSS_CHAIN_TRANSFER",
        },
      },
    ],
    ...
    [
      {
        ...
        user: "user",
        content: {
          text: "Transfiere 100 USDT de Polygon a Arbitrum por bridge",
          action: "CROSS_CHAIN_TRANSFER",
        },
      },
    ],
    [
      {
        ...
        user: "user",
        content: {
          text: "Bridge all my ETH from Base to OP",
          action: "CROSS_CHAIN_TRANSFER",
        },
      },
    ],
