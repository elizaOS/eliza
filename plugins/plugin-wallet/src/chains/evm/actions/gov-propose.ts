...
export const proposeAction = {
  ...
  handler: async (
    ...
    try {
      ...
      if (!isConfirmed(options)) {
        const preview = `Review governance proposal before submitting: ${proposeParams.targets.length} target(s) on ${proposeParams.chain}, governor ${proposeParams.governor}. Re-invoke ${spec.name} with confirmed: true to submit.`;
  ...
  validate: createEvmActionValidator({
    keywords: ["propose", "proposal", "governance", "dao", "submit"],
    regex: /\b(?:propose|proposal|governance|dao|submit)\b/i,
  }),
  examples: [
    [
      {
    ...
    [
      {
        name: "user",
        content: {
          text: "Propón una actualización del fee en el governor de Base",
          action: "PROPOSE",
        },
      },
    ],
    [
      {
        name: "user",
        content: {
          text: "Create a proposal to pause emissions for one epoch on Ethereum DAO",
          action: "PROPOSE",
        },
      },
    ],
