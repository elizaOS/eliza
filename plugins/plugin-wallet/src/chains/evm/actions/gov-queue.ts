...
const spec = requireActionSpec("GOV_QUEUE");
...
export const queueAction = {
  ...
  handler: async (
    ...
    try {
      ...
      if (!isConfirmed(options)) {
        const preview = `Review governance queue before submitting: ${queueParams.targets.length} target(s) on ${queueParams.chain}, governor ${queueParams.governor}. Re-invoke ${spec.name} with confirmed: true to submit.`;
  ...
  validate: createEvmActionValidator({
    keywords: ["queue", "proposal", "timelock", "governor"],
    regex: /\b(?:queue|queued|timelock|proposal|governor)\b/i,
  }),
  examples: [
    [
      {
    ...
    [
      {
        name: "user",
        content: {
          text: "Pon en cola la propuesta 33 para ejecución en Base",
          action: "QUEUE_PROPOSAL",
        },
      },
    ],
    [
      {
        name: "user",
        content: {
          text: "Queue this approved proposal in timelock before execution",
          action: "QUEUE_PROPOSAL",
        },
      },
    ],
