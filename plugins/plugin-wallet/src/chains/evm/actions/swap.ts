 ...
 */
import { BEBOP_CHAIN_MAP } from "../constants";
import { BebopRouteSchema } from "../types";
import { DEFAULT_SLIPPAGE_PERCENT } from "../constants";
import { EVMError } from "../types";
import { EVMErrorCode } from "../types";
import { GAS_BUFFER_MULTIPLIER } from "../constants";
import { GAS_PRICE_MULTIPLIER } from "../constants";
import { KYBERSWAP_CHAIN_MAP } from "../constants";
import { KYBERSWAP_NATIVE_SENTINEL } from "../constants";
import { ModelType } from "@elizaos/core";
import { NATIVE_TOKEN_ADDRESS } from "../constants";
import { TX_CONFIRMATION_TIMEOUT_MS } from "../constants";
import { buildSendTxParams } from "./helpers";
import { composePromptFromState } from "@elizaos/shared";
import { createConfig } from "@lifi/sdk";
import { createEvmActionValidator } from "./helpers";
import { encodeFunctionData } from "viem";
import { gateWalletFinancialExecution } from "../../../security/wallet-financial-confirmation.js";
...
import { parseSwapParams } from "../types";
...
import { type BebopRoute } from "../types";
import { type ExtendedChain } from "@lifi/sdk";
import { type HandlerCallback } from "@elizaos/core";
import { type Hex } from "viem";
import { type IAgentRuntime } from "@elizaos/core";
import { type KyberSwapRouteData } from "../types";
import { type KyberSwapRouteSummary } from "../types";
...
import { walletFinancialGateActionResult } from "../../../security/wallet-financial-confirmation.js";
...
export class SwapAction {
    ...
    private async resolveTokenAddress(tokenSymbolOrAddress: string, chainId: number): Promise<string> {
    ...
    async swap(params: SwapParams): Promise<Transaction> {
        ...
        const slippageLevels = params.slippageBps === undefined ? [0.01, 0.015, 0.02] : [params.slippageBps / 10000];
        ...
        throw new EVMError(EVMErrorCode.CONTRACT_REVERT, `All swap attempts failed after ${attemptCount} tries. ${lastError?.message ?? "Unknown error"}`);
    }
    private isRecoverableError(error: Error): boolean {
        const message = error.message;
        return (message.includes("price movement") ||
            message.includes("Return amount is not enough") ||
            message.includes("reverted") ||
            message.includes("MEV frontrunning") ||
            message.includes("TRANSFER_FROM_FAILED"));
    }
    private async getSortedQuotes(fromAddress: Address, params: SwapParams, slippage: number = DEFAULT_SLIPPAGE_PERCENT): Promise<SwapQuote[]> {
        const decimalsAbi = parseAbi(["function decimals() view returns (uint8)"]);
        let fromTokenDecimals: number;
        const chainConfig = this.walletProvider.getChainConfigs(params.chain);
        if (params.fromToken.toUpperCase() === chainConfig.nativeCurrency.symbol.toUpperCase() ||
    ...
    private async getLifiQuote(fromAddress: Address, params: SwapParams, fromTokenDecimals: number, slippage: number = DEFAULT_SLIPPAGE_PERCENT): Promise<SwapQuote | undefined> {
        try {
    ...
    private async getBebopQuote(fromAddress: Address, params: SwapParams, fromTokenDecimals: number): Promise<SwapQuote | undefined> {
        try {
            ...
            const buyTokenInfo = buyTokens[resolvedToToken] ??
    ...
    private async getKyberSwapQuote(fromAddress: Address, params: SwapParams, fromTokenDecimals: number, slippage: number): Promise<SwapQuote | undefined> {
        try {
            const chainSlug = KYBERSWAP_CHAIN_MAP[params.chain];
            if (!chainSlug)
                return undefined;
            const fromToken = params.fromToken === NATIVE_TOKEN_ADDRESS ? KYBERSWAP_NATIVE_SENTINEL : params.fromToken;
            const toToken = params.toToken === NATIVE_TOKEN_ADDRESS ? KYBERSWAP_NATIVE_SENTINEL : params.toToken;
            ...
            if (!res.ok)
                throw new Error(`KyberSwap API error: ${res.status}`);
            ...
            logger.error("Error in getKyberSwapQuote:", error instanceof Error ? error.message : String(error));
            return undefined;
        }
    }
    private async executeLifiQuote(quote: SwapQuote): Promise<Transaction | undefined> {
        ...
        const chainName = Object.keys(this.walletProvider.chains).find((name) => this.walletProvider.getChainConfigs(name as SupportedChain).id === chainId);
        if (!chainName) {
        ...
        if (fromToken.address !== NATIVE_TOKEN_ADDRESS) {
            await this.handleTokenApproval(publicClient, walletClient, fromToken.address as Address, txRequest.to as Address, BigInt(route.fromAmount));
        }
        const hash = await walletClient.sendTransaction(buildSendTxParams({
            account,
    ...
    private async executeBebopQuote(quote: SwapQuote, params: SwapParams): Promise<Transaction | undefined> {
        ...
        if (resolvedFromToken !== NATIVE_TOKEN_ADDRESS) {
            await this.handleTokenApproval(publicClient, walletClient, resolvedFromToken as Address, bebopRoute.approvalTarget, BigInt(bebopRoute.sellAmount));
        }
        const hash = await walletClient.sendTransaction(buildSendTxParams({
            account,
    ...
    private async executeKyberSwapQuote(quote: SwapQuote, params: SwapParams): Promise<Transaction | undefined> {
        ...
        const buildRes = await fetch(`https://aggregator-api.kyberswap.com/${ks.chainSlug}/api/v1/route/build`, {
            signal: AbortSignal.timeout(DEFAULT_EVM_SWAP_FETCH_TIMEOUT_MS),
            method: "POST",
            headers: {
                "X-Client-Id": "elizaos",
        ...
        if (ks.fromToken.toLowerCase() !== KYBERSWAP_NATIVE_SENTINEL.toLowerCase()) {
            await this.handleTokenApproval(publicClient, walletClient, ks.fromToken as Address, tx.routerAddress as Address, BigInt(ks.amountIn));
        }
        const hash = await walletClient.sendTransaction(buildSendTxParams({
            account,
    ...
    private async handleTokenApproval(publicClient: ReturnType<WalletProvider["getPublicClient"]>, walletClient: ReturnType<WalletProvider["getWalletClient"]>, tokenAddress: Address, spenderAddress: Address, requiredAmount: bigint): Promise<void> {
        const account = walletClient.account;
        if (!account) {
        ...
        const approvalTx = await walletClient.sendTransaction(buildSendTxParams({
            account,
            to: tokenAddress,
            value: 0n,
            data: approvalData,
        ...
        if (approvalReceipt.status === "reverted") {
            throw new EVMError(EVMErrorCode.CONTRACT_REVERT, `Token approval failed. Hash: ${approvalTx}`);
        }
        logger.info(`Token approval confirmed`);
    }
}
export async function buildSwapDetails(state: State, message: Memory, runtime: IAgentRuntime, wp: WalletProvider): Promise<SwapParams> {
    ...
    if (!parsedResponse) {
        throw new EVMError(EVMErrorCode.INVALID_PARAMS, "Failed to parse structured response from LLM for swap details.");
    }
    ...
    const amount = amountMode === "absolute"
    ...
    if (!wp.chains[swapDetails.chain]) {
        throw new EVMError(EVMErrorCode.CHAIN_NOT_CONFIGURED, `Chain ${swapDetails.chain} not configured. Available: ${chains.join(", ")}`);
    }
    return swapDetails;
}
 ...
 */
function resolveRelativeAmount(mode: Exclude<AmountMode, "absolute">, rawPercent: unknown, balance: string | undefined): string {
    if (balance === undefined) {
        throw new EVMError(EVMErrorCode.INVALID_PARAMS, `Cannot resolve a relative swap amount: unknown balance for the selected chain.`);
    }
    ...
    if (!Number.isFinite(percent) || percent < 1 || percent > 100) {
        throw new EVMError(EVMErrorCode.INVALID_PARAMS, `Swap percentage must be between 1 and 100, received: ${String(rawPercent)}`);
    }
