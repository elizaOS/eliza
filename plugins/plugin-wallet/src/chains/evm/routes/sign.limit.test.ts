...
describe("EVM sign chainId validation", () => {
    ...
    it.each([
        ...
        "12px",
        "9007199254740992",
    ])("rejects non-canonical chainId %s before resolving the backend", async (chainId) => {
        const response = res();
        await route("wallet-evm-sign-transaction")(req({
            chainId,
    ...
    it.each([0, -1, 1.5, Number.NaN, Number.POSITIVE_INFINITY])("rejects invalid numeric chainId %s before resolving the backend", async (chainId) => {
        const response = res();
        await route("wallet-evm-sign-transaction")(req({
            chainId,
            tx: { to: "0x0000000000000000000000000000000000000000" },
    ...
    it.each(["1", "0x1"])("still accepts canonical chainId %s and reaches the backend", async (chainId) => {
