...
function route(name: string) {
    ...
    if (!found.handler)
        throw new Error(`route ${name} has no handler`);
...
describe("EVM browser signing routes", () => {
    ...
    it("does not mark browser signing routes as public", () => {
        ...
        expect(evmSignRoutes.every((candidate) => candidate.public !== true)).toBe(true);
    });
    it("closes the surface when the signing token is missing or too short", async () => {
        for (const token of [null, "short-token"]) {
            const response = res();
            await route("wallet-evm-address").handler(req({ authorization: "Bearer short-token" }), response, runtime(token));
    ...
    it("never reflects an arbitrary cross-origin or sends credentialed CORS", async () => {
        const response = res();
        await route("wallet-evm-personal-sign").handler(req({
            method: "OPTIONS",
    ...
    it("allows a loopback origin without credentialed CORS", async () => {
        const response = res();
        await route("wallet-evm-personal-sign").handler(req({ method: "OPTIONS", origin: "http://localhost:31337" }), response, runtime("1234567890abcdef"));
        expect(response.statusCode).toBe(204);
        expect(response.headers["Access-Control-Allow-Origin"]).toBe("http://localhost:31337");
        expect(response.headers["Access-Control-Allow-Credentials"]).toBeUndefined();
    });
    it("handles OPTIONS without touching wallet backends", async () => {
        const response = res();
        await route("wallet-evm-personal-sign").handler(req({ method: "OPTIONS", origin: "http://127.0.0.1:2138" }), response, runtime("1234567890abcdef"));
        expect(response.statusCode).toBe(204);
        expect(response.body).toEqual({});
    ...
    it("gates every EVM signing route behind the browser signing token", async () => {
        ...
        for (const routeName of signingRouteNames) {
            const response = res();
            await route(routeName).handler(req({ authorization: "Bearer caller-token", body: {} }), response, runtime(null));
            expect(response.statusCode).toBe(503);
            expect(walletBackendMocks.resolveWalletBackend).not.toHaveBeenCalled();
        }
    });
    it("rejects malformed chain ids before resolving the backend", async () => {
        const response = res();
        await route("wallet-evm-sign-transaction").handler(req({
    ...
    it("rejects unsupported chain ids before resolving the backend", async () => {
        const response = res();
        await route("wallet-evm-send-transaction").handler(req({
            authorization: "Bearer 1234567890abcdef",
            body: {
                chainId: -1,
                tx: { to: "0x0000000000000000000000000000000000000000" },
