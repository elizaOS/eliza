...
describe("Solana browser signing routes", () => {
    ...
    it("closes the surface when the signing token is missing or too short", async () => {
        for (const token of [null, "short-token"]) {
            const response = res();
            await route("wallet-solana-pubkey").handler(req({ authorization: "Bearer short-token" }), response, runtime(token));
    ...
    it("never reflects an arbitrary cross-origin or sends credentialed CORS", async () => {
        const response = res();
        await route("wallet-solana-sign-message").handler(req({ method: "OPTIONS", origin: "https://attacker.example" }), response, runtime("1234567890abcdef"));
    ...
    it("allows a loopback origin without credentialed CORS", async () => {
        const response = res();
        await route("wallet-solana-sign-message").handler(req({ method: "OPTIONS", origin: "http://127.0.0.1:2138" }), response, runtime("1234567890abcdef"));
        expect(response.statusCode).toBe(204);
    ...
    it("validates message body shape before resolving wallet backend", async () => {
        const response = res();
        await route("wallet-solana-sign-message").handler(req({
    ...
    it("rejects invalid transaction bytes before resolving wallet backend", async () => {
        const response = res();
        await route("wallet-solana-sign-transaction").handler(req({
            authorization: "Bearer 1234567890abcdef",
