 ...
 */
import { BIRDEYE_ROUTE_SPEC } from "./analytics/birdeye/service.js";
import { BirdeyeService } from "./analytics/birdeye/service.js";
...
function concatServices(...chunks: (readonly ServiceClass[] | undefined)[]): ServiceClass[] {
...
async function initBirdeyeAnalytics(runtime: IAgentRuntime): Promise<void> {
    const birdeyeRoute = resolveCloudRoute(toWalletCloudRoutingSettings(runtime), BIRDEYE_ROUTE_SPEC);
    registerBirdeyeSearchCategories(runtime, {
        enabled: birdeyeRoute.source !== "disabled",
        disabledReason: birdeyeRoute.source === "disabled"
            ? "BIRDEYE_API_KEY or Eliza Cloud route is not configured."
            : undefined,
    });
    if (birdeyeRoute.source === "disabled") {
        runtime.logger.log("birdeye: no BIRDEYE_API_KEY and Eliza Cloud not connected, skipping plugin-birdeye init");
        return;
    }
    ...
    const beNoTrending = parseBooleanFromText(String(runtime.getSetting("BIRDEYE_NO_TRENDING") ?? ""));
    if (!beNoTrending) {
...
export const walletPlugin: Plugin = {
    name: "wallet",
    description: "Non-custodial wallet for elizaOS — EVM + Solana, Steward/local backends, x402, CCTP, and venue routing.",
    services: concatServices(coreWalletPlugin.services, evmPlugin.services as ServiceClass[] | undefined, solanaPlugin.services as ServiceClass[] | undefined, analyticsServices),
    ...
    async dispose(runtime: IAgentRuntime) {
        ...
        const walletBackend = runtime.getService<WalletBackendService>(WALLET_BACKEND_SERVICE_TYPE);
        await walletBackend?.stop();
        const stewardTrading = runtime.getService<StewardTradingService>(StewardTradingService.serviceType);
