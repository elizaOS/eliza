 ...
 */
import { STEWARD_TRADING_SERVICE_TYPE } from "./steward-trading-service.js";
import { StewardTradingService } from "./steward-trading-service.js";
...
function configuredService(fetchMock: typeof fetch, maxRetries = 3, apiUrl = "https://steward.local") {
...
describe("StewardTradingService", () => {
    it("registers under the intended service type and reports configured capability", () => {
        ...
        expect(StewardTradingService.serviceType).toBe(STEWARD_TRADING_SERVICE_TYPE);
    ...
    it("allows bracketed IPv6 loopback Steward sidecar URLs", async () => {
        const fetchMock = vi.fn(async () => jsonResponse(200, stewardFixtures.tokenStatusObserved));
        const service = configuredService(fetchMock as unknown as typeof fetch, 3, "http://[::1]:8787");
        expect(service.capability()).toMatchObject({
            kind: "steward-self",
            canTrade: true,
            agentId: "agent-fixture",
        ...
        expect(fetchMock.mock.calls[0]?.[0]).toBe("http://[::1]:8787/v1/trade/token-status?agentId=agent-fixture");
    });
    ...
    it("opens sessions through the versioned route with Steward request names", async () => {
        const fetchMock = vi.fn(async () => jsonResponse(201, stewardFixtures.openHyperliquidSession));
        ...
        expect(fetchMock.mock.calls[0]?.[0]).toBe("https://steward.local/v1/trade/sessions");
    ...
    it("uses only versioned trade routes for session lifecycle and token status", async () => {
        const fetchMock = vi
            .fn()
            .mockResolvedValueOnce(jsonResponse(200, stewardFixtures.tokenStatusObserved))
            .mockResolvedValueOnce(jsonResponse(200, stewardFixtures.openHyperliquidSession))
            .mockResolvedValueOnce(jsonResponse(200, { ok: true, data: { revoked: true } }));
        const service = configuredService(fetchMock as unknown as typeof fetch);
        await service.tokenStatus();
    ...
    it("submits Hyperliquid orders with one idempotency key reused across retries", async () => {
        const fetchMock = vi
            .fn()
            .mockResolvedValueOnce(jsonResponse(429, stewardFixtures.rateLimited, { "Retry-After": "0" }))
            .mockResolvedValueOnce(jsonResponse(503, { ok: false, error: "transient upstream failure" }))
            .mockResolvedValueOnce(jsonResponse(200, stewardFixtures.hyperliquidOrderAccepted));
        const service = configuredService(fetchMock as unknown as typeof fetch);
        const result = await service.submitOrder({
            venue: "hyperliquid",
            sessionId: "sess_hl_fixture",
    ...
    it("does not retry status-unknown submit responses", async () => {
        const fetchMock = vi.fn(async () => jsonResponse(502, stewardFixtures.unknownSubmit));
        const service = configuredService(fetchMock as unknown as typeof fetch);
        const result = await service.submitOrder({
            venue: "polymarket",
    ...
    it("reports exhausted 5xx retries as an unknown submission", async () => {
        const fetchMock = vi.fn(async () => jsonResponse(503, { ok: false, error: "transient upstream failure" }));
        const service = configuredService(fetchMock as unknown as typeof fetch);
        const result = await service.submitOrder({
            venue: "hyperliquid",
            sessionId: "sess_hl_fixture",
    ...
    it("maps critical Steward failures into outcome classes and retry flags", async () => {
        ...
        for (const c of cases) {
            const fetchMock = vi.fn(async () => jsonResponse(c.status, c.body, { "Retry-After": "0" }));
    ...
    it("keeps venue instrument token rejections out of credential handling", async () => {
        const fetchMock = vi.fn(async () => jsonResponse(400, { ok: false, error: "token not supported" }));
        const service = configuredService(fetchMock as unknown as typeof fetch, 1);
        const result = await service.submitOrder({
            venue: "polymarket",
            sessionId: "sess_pm_fixture",
    ...
    it("resolves an active governed account from token and session state", async () => {
        const fetchMock = vi
            .fn()
            .mockResolvedValueOnce(jsonResponse(200, stewardFixtures.tokenStatusObserved))
            .mockResolvedValueOnce(jsonResponse(200, stewardFixtures.activeHyperliquidSession));
        const service = new StewardTradingService(runtime({
            STEWARD_API_URL: "https://steward.local",
            STEWARD_AGENT_ID: "agent-fixture",
            STEWARD_AGENT_TOKEN: "token-fixture",
    ...
    it("exposes exact neutral methods and fail-closed unavailable routes", async () => {
        ...
        await expect(service.cancelOrder({ venue: "polymarket", orderId: "order-fixture" })).resolves.toMatchObject({
            ok: false,
            outcome: "not_attempted",
            error: "ROUTE_NOT_FOUND",
            retryable: false,
...
describe("StewardTradingService dev launcher authority", () => {
    ...
    beforeEach(() => {
        ...
        saved = Object.fromEntries(AUTHORITY_ENV_KEYS.map((key) => [key, process.env[key]])) as Record<(typeof AUTHORITY_ENV_KEYS)[number], string | undefined>;
        for (const key of AUTHORITY_ENV_KEYS)
            delete process.env[key];
        stateDir = mkdtempSync(path.join(os.tmpdir(), "steward-trading-authority-"));
    ...
    it("does not trade from persisted or runtime production credentials in default staging", async () => {
        writeFileSync(path.join(stateDir, "steward-credentials.json"), JSON.stringify({
