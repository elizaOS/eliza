...
import { type CancelOrderRequest } from "../types/trade.js";
import { type CancelResult } from "../types/trade.js";
import { type IAgentRuntime } from "@elizaos/core";
import { type OpenOrder } from "../types/trade.js";
import { type OpenSessionRequest } from "../types/trade.js";
import { type OrderResult } from "../types/trade.js";
import { type PolicyDenyReason } from "../types/trade.js";
import { type Position } from "../types/trade.js";
import { type SubmitOrderRequest } from "../types/trade.js";
import { type TradeEnvelope } from "../types/trade.js";
import { type TradeSession } from "../types/trade.js";
import { type TradeTokenStatus } from "../types/trade.js";
import { type TradingAccount } from "../types/trade.js";
import { type TradingCapability } from "../types/trade.js";
import { type Venue } from "../types/trade.js";
...
function readJsonCredentialsFile(credentialsPath: string): PersistedStewardCredentials | null {
    try {
...
function readPersistedStewardCredentials(): PersistedStewardCredentials | null {
    return readJsonCredentialsFile(path.join(resolveStateDir(), "steward-credentials.json"));
}
...
function detailFromBody(body: unknown, fallback: string): string {
    ...
    return (normalizeOptionalString(record.reason) ??
...
function mapPolicyDenyReason(reason: string | undefined): PolicyDenyReason {
    ...
    if (/session.*not active|active .*session required|session-not-active/.test(normalized)) {
...
function isStewardCredentialFailure(status: number, detail: string): boolean {
    if (status === 401)
        return true;
    return (/\bagent\s+jwt\s+(?:required|missing|invalid|expired)\b/i.test(detail) ||
        /\bauth(?:entication)?[-_\s]?token\s+(?:required|missing|invalid|expired)\b/i.test(detail) ||
        /\b(?:required|missing|invalid|expired)\s+auth(?:entication)?[-_\s]?token\b/i.test(detail));
}
...
function toOrderResult(venue: Venue, body: unknown, idempotencyKey: string): OrderResult | null {
    const data = isResponseEnvelope(body) ? body.data : body;
    const record = data as Record<string, unknown>;
    ...
    return {
        ...
        orderId,
        status,
        filledQty: typeof record.filledQty === "number" ? record.filledQty : undefined,
        avgPrice: typeof record.avgPrice === "number" ? record.avgPrice : undefined,
        txHash: typeof record.txHash === "string" || record.txHash === null
            ? record.txHash
            : undefined,
        notionalUsd: typeof record.notionalUsd === "number" ? record.notionalUsd : undefined,
        builderPerp: typeof record.builderPerp === "boolean" ? record.builderPerp : undefined,
        idempotencyKey,
    };
}
...
function mapFailure<T>(response: JsonResponse, fallback: string): TradeEnvelope<T> {
...
export class StewardTradingService extends Service {
    ...
    override capabilityDescription = "Governed Steward trading HTTP client for Hyperliquid and Polymarket";
    ...
    constructor(runtime?: IAgentRuntime, options: StewardTradingServiceOptions = {}) {
        super(runtime);
    ...
    static override async start(runtime: IAgentRuntime): Promise<StewardTradingService> {
    ...
    capability(): TradingCapability {
        if (!this.tradingConfig) {
            return {
                kind: "none",
                canTrade: false,
                reason: "STEWARD_API_URL, STEWARD_AGENT_ID, and a Steward auth token are required.",
            };
        }
        const isCloud = normalizeOptionalString(this.runtime.getSetting("ELIZA_CLOUD_PROVISIONED")) === "1" || process.env.ELIZA_CLOUD_PROVISIONED === "1";
        return {
    ...
    async tokenStatus(): Promise<TradeTokenStatus> {
        ...
        const response = await this.request(`/v1/trade/token-status?agentId=${encodeURIComponent(config.agentId)}`, { method: "GET" }, false);
        if (response.status >= 200 && response.status < 300) {
            return toTokenStatus(response.body);
        }
        throw new Error(detailFromBody(response.body, "Steward token status failed"));
    }
    async openSession(req: OpenSessionRequest): Promise<TradeEnvelope<TradeSession>> {
        const config = this.requireConfig();
        const response = await this.request("/v1/trade/sessions", {
            method: "POST",
    ...
    async getSession(id: string): Promise<TradeEnvelope<TradeSession>> {
        const response = await this.request(`/v1/trade/sessions/${encodeURIComponent(id)}`, { method: "GET" }, false);
        if (response.status >= 200 && response.status < 300) {
            const session = toSessionData(response.body);
            return {
                ok: true,
    ...
    async revokeSession(id: string): Promise<TradeEnvelope<{
        revoked: true;
    }>> {
        const response = await this.request(`/v1/trade/sessions/${encodeURIComponent(id)}/revoke`, { method: "POST" }, false);
        if (response.status >= 200 && response.status < 300) {
            return { ok: true, data: { revoked: true }, audit: { sessionId: id } };
        }
        return mapFailure(response, "Steward session revoke failed");
    }
    async submitOrder(req: SubmitOrderRequest): Promise<TradeEnvelope<OrderResult>> {
        ...
        const response = await this.requestWithRetry(buildOrderRoute(req), {
            method: "POST",
            body,
            idempotencyKey,
        }, idempotencyKey);
        if (response.status >= 200 && response.status < 300) {
            const orderResult = toOrderResult(req.venue, response.body, idempotencyKey);
            if (!orderResult) {
                return {
                    ok: false,
                    outcome: "not_attempted",
    ...
    async cancelOrder(req: CancelOrderRequest): Promise<TradeEnvelope<CancelResult>> {
        return routeNotFound(`Steward does not expose a ${req.venue} cancel-order HTTP route yet.`);
    }
    async resolveAccount(venue: Venue): Promise<TradeEnvelope<TradingAccount>> {
        const settingName = venue === "hyperliquid"
        ...
        const sessionId = authoritative
            ? (normalizeOptionalString(resolveDevCloudAuthorityEnvValue(settingName)) ??
                normalizeOptionalString(resolveDevCloudAuthorityEnvValue("STEWARD_TRADE_SESSION_ID")))
            : (normalizeOptionalString(this.runtime.getSetting(settingName)) ??
                normalizeOptionalString(process.env[settingName]) ??
                normalizeOptionalString(this.runtime.getSetting("STEWARD_TRADE_SESSION_ID")) ??
        ...
        const config = this.requireConfig();
        const tokenResponse = await this.request(`/v1/trade/token-status?agentId=${encodeURIComponent(config.agentId)}`, { method: "GET" }, false);
    ...
    async listOrders(venue: Venue): Promise<TradeEnvelope<OpenOrder[]>> {
        return routeNotFound(`Steward does not expose a ${venue} list-orders HTTP route yet.`);
    }
    async listPositions(venue: Venue): Promise<TradeEnvelope<Position[]>> {
        return routeNotFound(`Steward does not expose a ${venue} positions HTTP route yet.`);
    }
    private resolveConfig(): StewardTradingConfig | null {
        ...
        const apiUrl = normalizeOptionalString(this.runtime.getSetting("STEWARD_API_URL")) ??
            normalizeOptionalString(process.env.STEWARD_API_URL) ??
            persisted?.apiUrl;
        const agentId = normalizeOptionalString(this.runtime.getSetting("STEWARD_AGENT_ID")) ??
            normalizeOptionalString(this.runtime.getSetting("ELIZA_STEWARD_AGENT_ID")) ??
            normalizeOptionalString(process.env.STEWARD_AGENT_ID) ??
            normalizeOptionalString(process.env.ELIZA_STEWARD_AGENT_ID) ??
            persisted?.agentId;
        const agentToken = normalizeOptionalString(this.runtime.getSetting("STEWARD_AGENT_TOKEN")) ??
            normalizeOptionalString(process.env.STEWARD_AGENT_TOKEN) ??
            persisted?.agentToken;
        const apiKey = normalizeOptionalString(this.runtime.getSetting("STEWARD_API_KEY")) ??
            normalizeOptionalString(process.env.STEWARD_API_KEY) ??
            persisted?.apiKey;
        const tenantId = normalizeOptionalString(this.runtime.getSetting("STEWARD_TENANT_ID")) ??
    ...
    private requireConfig(): StewardTradingConfig {
        if (!this.tradingConfig) {
            throw new Error("Steward trading is not configured. Set Steward API URL, agent ID, and auth token.");
        }
    ...
    private headers(idempotencyKey?: string): Record<string, string> {
        ...
        if (config.tenantId)
            headers["X-Steward-Tenant"] = config.tenantId;
        if (idempotencyKey)
            headers["Idempotency-Key"] = idempotencyKey;
        return headers;
    }
    private async requestWithRetry(route: string, init: {
        readonly method: "GET" | "POST";
        readonly body?: unknown;
        readonly idempotencyKey?: string;
    }, idempotencyKey: string): Promise<JsonResponse> {
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt += 1) {
            try {
                ...
                await this.sleep(retryAfterMs(response.headers) ?? retryDelayMs(attempt, this.random));
            }
            catch (error) {
                ...
                if (!(error instanceof StewardTransportError))
                    throw error;
                last = unknownSubmissionResponse();
                if (error.timedOut || attempt === this.maxRetries)
                    return last;
    ...
    private async request(route: string, init: {
        readonly method: "GET" | "POST";
        readonly body?: unknown;
        readonly idempotencyKey?: string;
    }, throwTransportErrors: boolean): Promise<JsonResponse & {
        retryAfterMs?: number;
    }> {
        const config = this.requireConfig();
