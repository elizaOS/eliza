...
import { clearAutomationNodeContributorsForTests } from "@elizaos/core/automation-node-contributors";
import { describe } from "vitest";
import { expect } from "vitest";
import { it } from "vitest";
import { listAutomationNodeContributors } from "@elizaos/core/automation-node-contributors";
...
function context(runtime: Partial<{
    actions: Array<{
        name: string;
        similes?: string[];
    }>;
...
describe("plugin-wallet automation node contributor", () => {
    ...
    it("emits disabled wallet nodes when no wallet capability is loaded", async () => {
        ...
        expect(nodes.find((node) => node.id === "crypto:evm.swap")?.disabledReason).toBe("Load the EVM plugin with swap support.");
    });
    ...
    it("enables the solana node when a solana swap action is registered", async () => {
        registerWalletAutomationNodeContributor();
        const [contributor] = listAutomationNodeContributors();
        const nodes = await contributor(context({ actions: [{ name: "SWAP_SOLANA" }] }));
        expect(nodes.find((node) => node.id === "crypto:solana.swap")?.availability).toBe("enabled");
        expect(nodes.find((node) => node.id === "crypto:evm.swap")?.availability).toBe("disabled");
    });
});
