...
import { PostWalletGenerateRequestSchema } from "@elizaos/core/contracts/wallet-routes";
import { PostWalletImportRequestSchema } from "@elizaos/core/contracts/wallet-routes";
import { PostWalletPrimaryRequestSchema } from "@elizaos/core/contracts/wallet-routes";
...
import { type WalletChainKind } from "@elizaos/core/contracts/wallet-types";
...
import { type WalletEntry } from "@elizaos/core/contracts/wallet-types";
import { type WalletExportRejection as WalletExportRejectionLike } from "@elizaos/core/contracts/wallet-types";
import { type WalletExportRequestBody } from "@elizaos/core/contracts/wallet-types";
import { type WalletNftsResponse } from "@elizaos/core/contracts/wallet-types";
import { type WalletPrimaryMap } from "@elizaos/core/contracts/wallet-types";
...
import { type WalletSource } from "@elizaos/core/contracts/wallet-types";
...
type CloudHelperBundle = {
    ...
    persistCloudWalletCache: (config: unknown, descriptors: Partial<Record<WalletChainKind, CloudWalletDescriptor>>) => void;
    provisionCloudWalletsBestEffort: (bridge: unknown, args: {
        agentId: string;
    ...
    resolveCloudApiKey: (config: ElizaConfig, runtime: AgentRuntime | null) => string | null;
};
...
function resolveWalletConfigUpdateRequest(body: unknown, currentSelections: WalletRpcSelections): WalletConfigUpdateRequest | null {
    ...
    if (record.selections &&
        ...
        const walletNetwork = record.walletNetwork === "testnet" || record.walletNetwork === "mainnet"
            ? record.walletNetwork
            : undefined;
        const credentials = record.credentials &&
            typeof record.credentials === "object" &&
            !Array.isArray(record.credentials)
            ? Object.fromEntries(Object.entries(record.credentials as Record<string, unknown>).filter(([, value]) => typeof value === "string"))
            : undefined;
        return {
            selections: normalizeWalletRpcSelections(record.selections as Partial<Record<keyof WalletRpcSelections, string>>),
            walletNetwork,
            credentials: credentials as WalletConfigUpdateRequest["credentials"],
        };
    }
    const compatCredentials = Object.fromEntries(Object.entries(record).filter(([key, value]) => WALLET_CONFIG_COMPAT_KEYS.has(key) && typeof value === "string"));
    if (Object.keys(compatCredentials).length === 0) {
        return null;
    }
    return {
        selections: currentSelections,
        walletNetwork: record.walletNetwork === "testnet" || record.walletNetwork === "mainnet"
            ? record.walletNetwork
            : undefined,
...
function readCloudWalletCache(config: ElizaConfig): Partial<Record<WalletChainKind, CachedCloudWalletDescriptor>> {
    const wallet = config.wallet;
    if (!wallet || typeof wallet !== "object")
        return {};
    const cloud = (wallet as {
        cloud?: unknown;
    }).cloud;
...
function readPrimaryMap(config: ElizaConfig): WalletPrimaryMap {
    const wallet = config.wallet;
    const raw = wallet && typeof wallet === "object"
        ? (wallet as {
            primary?: unknown;
        }).primary
        : undefined;
 ...
 */
function buildDualWalletShape(config: ElizaConfig, addresses: {
    evmAddress: string | null;
    solanaAddress: string | null;
}, isCloudWalletEnabled: () => boolean): {
    wallets: WalletEntry[];
...
function readCloudWalletAddress(descriptor: CachedCloudWalletDescriptor | undefined): string | null {
    if (typeof descriptor?.walletAddress !== "string")
        return null;
    const trimmed = descriptor.walletAddress.trim();
    return trimmed.length > 0 ? trimmed : null;
}
function readCachedCloudWalletDescriptor(config: ElizaConfig, chain: WalletChainKind): CloudWalletDescriptor | null {
    const descriptor = readCloudWalletCache(config)[chain];
    const walletAddress = readCloudWalletAddress(descriptor);
    if (!walletAddress)
        return null;
    return {
        agentWalletId: typeof descriptor?.agentWalletId === "string" &&
...
function readCachedCloudWalletDescriptors(config: ElizaConfig): Partial<Record<WalletChainKind, CloudWalletDescriptor>> {
...
function resolvePrimaryWalletAddresses(config: ElizaConfig, addresses: {
    evmAddress: string | null;
    solanaAddress: string | null;
}): {
    evmAddress: string | null;
    ...
    const cloud = readCloudWalletCache(config);
    return {
        evmAddress: primary.evm === "cloud"
            ? readCloudWalletAddress(cloud.evm)
            : addresses.evmAddress,
        solanaAddress: primary.solana === "cloud"
            ? readCloudWalletAddress(cloud.solana)
            : addresses.solanaAddress,
    };
}
function persistPrimarySelection(config: ElizaConfig, chain: WalletChainKind, source: WalletSource): void {
    if (!config.wallet) {
        config.wallet = {};
    }
    const wallet = config.wallet;
...
async function triggerWalletRuntimeReload(ctx: WalletRouteContext, reason: string): Promise<boolean> {
...
const SOLANA_PKCS8_DER_PREFIX = Buffer.from("302e020100300506032b657004220420", "hex");
...
function safeParseBrowserBigInt(value: string): bigint {
    try {
        ...
        throw new Error(`Invalid transaction value: expected an integer or hex string, got "${value}"`);
    }
}
...
function resolveLocalBrowserSolanaSeed(deriveSolanaAddress: WalletRouteDependencies["deriveSolanaAddress"]): {
    address: string;
    seed: Buffer;
} {
    ...
    const seed = decoded.length === 64
        ? decoded.subarray(0, 32)
        : decoded.length === 32
            ? decoded
            : null;
    if (!seed) {
        throw new Error(`Invalid Solana private key length: expected 32 or 64 bytes, got ${decoded.length}.`);
    }
    return {
...
function resolveBrowserSolanaMessageBytes(body: Record<string, unknown>): Buffer {
...
function resolveBrowserWalletMessagePayload(message: string): string | Uint8Array {
...
async function signLocalBrowserWalletMessage(message: string): Promise<{
    ...
    return {
        mode: "local-key",
        signature: await wallet.signMessage(resolveBrowserWalletMessagePayload(message)),
    };
}
async function signLocalBrowserSolanaMessage(body: Record<string, unknown>, deriveSolanaAddress: WalletRouteDependencies["deriveSolanaAddress"]): Promise<{
    address: string;
    mode: "local-key";
    ...
    const signature = crypto.sign(null, resolveBrowserSolanaMessageBytes(body), privateKey);
    return {
        address,
        mode: "local-key",
...
async function signLocalBrowserSolanaTransaction(body: Record<string, unknown>, deriveSolanaAddress: WalletRouteDependencies["deriveSolanaAddress"]): Promise<{
    address: string;
    mode: "local-key";
    ...
    const { address, seed } = resolveLocalBrowserSolanaSeed(deriveSolanaAddress);
    const { Keypair, VersionedTransaction, Transaction, Connection } = await loadBrowserSolanaWeb3();
    ...
    try {
        ...
        if (broadcast) {
            const connection = new Connection(browserSolanaClusterRpcUrl(cluster), "confirmed");
...
function resolvePreferredBrowserRpcUrl(config: ElizaConfig, chainId: number, resolveWalletRpcReadiness: WalletRouteDependencies["resolveWalletRpcReadiness"]): string | null {
...
async function sendLocalBrowserWalletTransaction(config: ElizaConfig, request: BrowserEvmTransactionRequest, resolveWalletRpcReadiness: WalletRouteDependencies["resolveWalletRpcReadiness"]): Promise<{
    approved: true;
    mode: "local-key";
    pending: false;
    txHash: string;
    ...
    if (request.broadcast === false) {
        throw new Error("Local browser wallet signing currently requires broadcast=true.");
    }
    const rpcUrl = resolvePreferredBrowserRpcUrl(config, request.chainId, resolveWalletRpcReadiness);
    if (!rpcUrl) {
...
export async function handleWalletRoutes(ctx: WalletRouteContext): Promise<boolean> {
    const { req, res, method, pathname, config, saveConfig, readJsonBody, json, error, deps, } = ctx;
    const { isCloudWalletEnabled, persistConfigEnv, createIntegrationTelemetrySpan, resolveWalletRpcReadiness, resolveWalletNetworkMode, resolveWalletCapabilityStatus, getStoredWalletRpcSelections, applyWalletRpcConfigUpdate, deriveSolanaAddress, setSolanaWalletEnv, } = deps;
        ...
        if (addresses.solanaAddress && rpcReadiness.solanaBalanceReady) {
            ...
            try {
                const solanaData = heliusKey
                    ...
                    : await deps.fetchSolanaNativeBalanceViaRpc(addresses.solanaAddress, rpcReadiness.solanaRpcUrls);
    ...
    if (method === "POST" && pathname === "/api/wallet/import") {
        ...
        if (!parsedImport.success) {
            error(res, parsedImport.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        ...
        const stewardWarning = (stewardAuthority
        ...
        if (configSaveWarning)
            warnings.push(configSaveWarning);
        if (stewardWarning)
            warnings.push(stewardWarning);
        ...
        try {
            ...
            error(res, `Failed to persist ${walletSourceEnvKey}: ${String(err)}`, 500);
            return true;
        }
        const restarted = await triggerWalletRuntimeReload(ctx, "Wallet configuration updated");
        json(res, {
            ok: true,
            chain,
    ...
    if (method === "POST" && pathname === "/api/wallet/generate") {
        ...
        if (!parsedGen.success) {
            error(res, parsedGen.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        ...
        if (stewardConnection && requestedSource !== "local") {
            try {
                ...
                try {
                    const agentRes = await fetch(`${stewardConnection.apiUrl}/agents/${encodeURIComponent(stewardConnection.agentId)}`, { headers: { ...headers }, signal: AbortSignal.timeout(15000) });
                    if (agentRes.ok) {
                        agentExists = true;
                ...
                if (!agentExists) {
                    ...
                    logger.info(`[wallet] Created steward agent "${stewardConnection.agentId}" with wallets`);
                }
        ...
        if (requestedSource === "steward" && !stewardConnection) {
            error(res, "Steward wallet generation is unavailable for this launch target.", 503);
            return true;
        }
    ...
    if (method === "POST" &&
        ...
        if (!body)
            return true;
        const hasLocalEvmKey = Boolean(normalizeBrowserString(process.env.EVM_PRIVATE_KEY));
        const hasLocalSolanaKey = Boolean(normalizeBrowserString(process.env.SOLANA_PRIVATE_KEY));
        ...
        if (pathname === "/api/wallet/browser-solana-transaction") {
            ...
            try {
                json(res, await signLocalBrowserSolanaTransaction(body, deriveSolanaAddress));
            }
            catch (err) {
                error(res, err instanceof Error ? err.message : String(err), 503);
            }
        ...
        const request: BrowserEvmTransactionRequest = {
            ...
            chainId: typeof body.chainId === "number" && Number.isFinite(body.chainId)
                ? body.chainId
                : Number.NaN,
        ...
        try {
            json(res, await sendLocalBrowserWalletTransaction(config, request, resolveWalletRpcReadiness));
        }
        catch (err) {
            error(res, err instanceof Error ? err.message : String(err), 503);
        }
    ...
    if (method === "POST" && pathname === "/api/wallet/primary") {
        ...
        if (!parsedPrimary.success) {
            error(res, parsedPrimary.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        ...
        const envKey = chain === "evm" ? "WALLET_SOURCE_EVM" : "WALLET_SOURCE_SOLANA";
        try {
        ...
        const restarted = previousPrimary === source
            ? false
    ...
    if (method === "POST" && pathname === "/api/wallet/refresh-cloud") {
        ...
        const { ElizaCloudClient, getOrCreateClientAddressKey, normalizeCloudSiteUrl, persistCloudWalletCache, provisionCloudWalletsBestEffort, resolveCloudApiKey, } = cloudHelpers;
        ...
        try {
            ...
            const chainsToProvision = (["evm", "solana"] as const).filter((chain) => !cachedDescriptors[chain]);
            const descriptors: Partial<Record<WalletChainKind, CloudWalletDescriptor>> = { ...cachedDescriptors };
            const warnings: string[] = [];
            const previousPrimary = readPrimaryMap(config);
            const previousEvmAddress = readCloudWalletAddress(cachedDescriptors.evm);
            const previousSolanaAddress = readCloudWalletAddress(cachedDescriptors.solana);
            if (chainsToProvision.length > 0) {
                ...
                for (const [index, failure] of provisionResult.failures.entries()) {
                    ...
                    if (cached) {
                        ...
                        warnings.push(`Reused cached ${failure.chain} cloud wallet after refresh failed: ${detail}`);
                        continue;
                    }
                    warnings.push(provisionResult.warnings[index] ??
                        `Cloud ${failure.chain} wallet import failed`);
                }
            }
            if (!descriptors.evm && !descriptors.solana) {
                throw new Error(warnings[0] ?? "Failed to provision any cloud wallet descriptors");
            }
            ...
            const walletBindingChanged = previousPrimary.evm !== nextPrimary.evm ||
    ...
    if (method === "PUT" && pathname === "/api/wallet/config") {
        ...
        if (!body)
            return true;
        const updateRequest = resolveWalletConfigUpdateRequest(body, getStoredWalletRpcSelections(config));
        if (!updateRequest) {
            error(res, "Invalid wallet config update");
            return true;
        }
        applyWalletRpcConfigUpdate(config, updateRequest);
        const selectedProviders = normalizeWalletRpcSelections(updateRequest.selections);
        const shouldEnableCloudWallet = Object.values(selectedProviders).every((provider) => provider === "eliza-cloud");
        if (shouldEnableCloudWallet) {
            ...
            try {
                ...
                error(res, `Failed to persist ENABLE_CLOUD_WALLET: ${String(err)}`, 500);
                return true;
            }
        }
        let configSaveWarning: string | undefined;
    ...
    if (method === "GET" && pathname === "/api/wallet/approvals/stream") {
        error(res, "Wallet approval streaming requires the Steward wallet route bridge.", 503);
        return true;
    }
    const approvalDecision = /^\/api\/wallet\/approvals\/([^/]+)\/decision$/.exec(pathname);
    if (method === "POST" && approvalDecision) {
        error(res, "Wallet approval decisions require the Steward wallet route bridge.", 503);
        return true;
    }
    // POST /api/wallet/export — removed (no plaintext key export from the agent API).
    if (method === "POST" && pathname === "/api/wallet/export") {
        error(res, "Private key export has been removed. Use Steward or OS-backed custody flows.", 410);
        return true;
    }
    return false;
}
