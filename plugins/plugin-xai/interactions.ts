...
import {
	createMemorySafe,
	ensureXContext as ensureContext,
	isPostProcessed,
} from "./utils/memory";
...
export class XInteractionClient {
	...
	constructor(
		...
		const usernameSetting =
			getSetting(this.runtime, "X_USERNAME") || this.state?.X_USERNAME;
		this.xUsername =
			typeof usernameSetting === "string"
				? usernameSetting
	...
	async handleXInteractions() {
		...
		try {
			...
			const repliesEnabled =
				(getSetting(this.runtime, "X_ENABLE_REPLIES") ??
					process.env.X_ENABLE_REPLIES) !== "false";
			...
			const targetUsersConfig =
				((getSetting(this.runtime, "X_TARGET_USERS") ??
					process.env.X_TARGET_USERS) as string) || "";
	...
	private async handleMentions(xUsername: string) {
		try {
			...
			const cachedCursor: string | undefined =
			...
			for (const targetUser of targetUsers) {
				try {
					...
					if (searchResult.posts.length > 0) {
						logger.info(
							`Found ${searchResult.posts.length} posts from @${normalizedUsername}`,
						);

						// Process these posts for potential engagement
						await this.processTargetUserPosts(
							searchResult.posts,
							normalizedUsername,
						);
					}
	...
	private async processTimelineForEngagement() {
		try {
			...
			if (relevantPosts.length > 0) {
				logger.info(
					`Found ${relevantPosts.length} relevant posts from timeline`,
				);
...
Response (YES/NO):`;
	...
	async processMentionPosts(mentionCandidates: ClientPost[]) {
		logger.log(
			"Completed checking mentioned posts:",
		...
		if (targetUsersConfig?.trim()) {
			uniquePostCandidates = uniquePostCandidates.filter((post) => {
				...
				if (!shouldTarget) {
					logger.log(
						`Skipping post from @${post.username} - not in target users list`,
					);
				}
				return shouldTarget;
			});
		...
		for (const post of postsToProcess) {
			if (
				...
				const hasExistingReply = existingReplies.some(
					(memory) =>
						memory.content.inReplyTo === postId ||
						memory.content.inReplyTo === post.id,
				);
	...
	async handlePost({
		...
		const callback: HandlerCallback = async (
			...
			try {
				...
				if (this.isDryRun) {
					logger.info(
						`[DRY RUN] Would have replied to ${post.username} with: ${response.text}`,
					);
					return [];
				}
				...
				const postResult = await sendPost(
					this.client,
					response.text,
					[],
					postToReplyTo,
				...
				const responsePostId =
					postResult.id || postResult.data?.id || Date.now().toString();
