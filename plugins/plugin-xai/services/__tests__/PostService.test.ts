...
describe("XPostService", () => {
	...
	describe("deletePost", () => {
		...
		it("should throw error on failure", async () => {
			mockClient.xClient.deletePost.mockRejectedValue(
				new Error("Delete failed"),
			);

			await expect(
				service.deletePost("post-123", TEST_AGENT_ID),
	...
	describe("likePost", () => {
		...
		it("should throw error on failure", async () => {
			...
			await expect(service.likePost("post-123", TEST_AGENT_ID)).rejects.toThrow(
				"Like failed",
			);
		});
	});
	...
	describe("repost", () => {
		...
		it("should throw error on failure", async () => {
			...
			await expect(service.repost("post-123", TEST_AGENT_ID)).rejects.toThrow(
				"Repost failed",
			);
		});
	});
