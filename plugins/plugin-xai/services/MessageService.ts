...
export class XMessageService implements IMessageService {
	...
	private async extractResultId(
		result: PostResponse | unknown,
			...
			const messages: Message[] = searchResult.posts
				.filter((post) => {
					if (options.roomId && post.conversationId) {
						const postRoomId = createUniqueUuid(
							this.client.runtime,
							post.conversationId,
						);
	...
	async sendMessage(options: SendMessageOptions): Promise<Message> {
		try {
			...
			if (options.type === MessageType.DIRECT_MESSAGE) {
				...
				result = await this.client.xClient.sendPost(
					options.text,
					options.replyToId,
				);
			}
