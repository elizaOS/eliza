...
export class XPostService implements IPostService {
	...
	async createPost(options: CreatePostOptions): Promise<Post> {
		try {
			...
			if (!postId) {
				...
				logger.error(
					"X createPost: could not extract post id from API result",
					safeResult,
				);
				throw new Error(
		...
		try {
			...
			const post: Post = {
				...
				roomId: createUniqueUuid(
					this.client.runtime,
					rawPost.conversationId || rawPost.id,
				),
	...
	async getPosts(options: GetPostsOptions): Promise<Post[]> {
		...
		function hasRequiredFields(post: PartialPost): post is PartialPost & {
			id: string;
			userId: string;
			username: string;
			text: string;
		...
		try {
			...
			if (options.userId) {
				...
				_posts = await this.client.fetchHomeTimeline(
					options.limit || 20,
					false,
				);
			}
