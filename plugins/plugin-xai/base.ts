...
export class ClientBase {
	...
	async getPost(postId: string): Promise<Post> {
		...
		const post = await this.requestQueue.add(() =>
			this.xClient.getPost(postId),
		);
	...
	constructor(runtime: IAgentRuntime, state: Record<string, unknown>) {
		...
		if (
			typeof reuseKey === "string" &&
			reuseKey &&
			ClientBase._xClients[reuseKey]
		) {
	...
	async fetchOwnPosts(count: number): Promise<Post[]> {
		...
		const homeTimeline = await this.xClient.getUserPosts(
			this.profile.id,
			count,
		);
			...
			const someCachedPostsExist = cachedTimeline.some((post) =>
				post.id
					? existingMemoryIds.has(
							createUniqueUuid(this.runtime, post.id as string),
						)
					: false,
			);
			...
			if (someCachedPostsExist) {
				...
				for (const post of postsToSave) {
					...
					await this.runtime.ensureWorldExists({
						...
						messageServerId: createUniqueUuid(
							this.runtime,
							post.userId,
						) as UUID,
						metadata: {
				...
				logger.log(
					`Populated ${postsToSave.length} missing posts from the cache.`,
				);
				return;
			}
		}
	...
	async cacheTimeline(timeline: Post[]) {
		...
		await this.runtime.setCache<Post[]>(
			`x/${this.profile.username}/timeline`,
			timeline,
		);
	}
	...
	async cacheMentions(mentions: Post[]) {
		...
		await this.runtime.setCache<Post[]>(
			`x/${this.profile.username}/mentions`,
			mentions,
		);
	}
	...
	async fetchInteractions() {
		...
		return mentionsResponse.posts.map((post: Post) =>
			this.formatPostToInteraction(post),
		);
	}
	...
	formatPostToInteraction(post: Post): XInteractionPayload | null {
		...
		const type: "quote" | "repost" | "like" = isQuote
			? "quote"
			: isRepost
				? "repost"
				: "like";
