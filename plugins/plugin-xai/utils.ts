...
async function _handleNotePost(
	...
	const result = await client.xClient.sendPost(
		content,
		postId,
		convertedMediaData,
	);
...
export async function sendPost(
	...
	try {
		...
		result = await client.xClient.sendPost(
			postText,
			postToReplyTo,
			convertedMediaData,
		);
	...
	try {
		...
		const postId =
			rawResult && ("id" in rawResult ? rawResult.id : rawResult.data?.id);
		if (postId) {
			if (
				client.lastCheckedPostId &&
			...
			const postText =
				rawResult && ("text" in rawResult ? rawResult.text : undefined);
			if (postId && postText) {
				await client.cachePost({
					id: postId,
					text: postText,
					...rawResult,
				} as Post);
...
export const parseActionResponseFromText = (
	...
	for (const line of lines) {
		...
		if (trimmed === "[REPOST]" || trimmed === "[RETWEET]")
			actions.repost = true;
