  ...
  try {
    ...
    for await (const post of searchIterator) {
      ...
      const convertedPost: Post = {
        ...
        name:
          searchIterator.includes?.users?.find((u) => u.id === post.author_id)
            ?.name || "",
        username:
          searchIterator.includes?.users?.find((u) => u.id === post.author_id)
            ?.username || "",
        ...
        videos: [],
        isRepost:
          post.referenced_tweets?.some((rt) => rt.type === "retweeted") ||
          false,
        isReply:
          post.referenced_tweets?.some((rt) => rt.type === "replied_to") ||
          false,
        isQuoted:
          post.referenced_tweets?.some((rt) => rt.type === "quoted") || false,
        isPin: false,
