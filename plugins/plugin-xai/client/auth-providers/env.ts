...
export class EnvAuthProvider implements XOAuth1Provider {
  ...
  async getOAuth1Credentials(): Promise<OAuth1Credentials> {
    const apiKeyRaw =
      this.state?.X_API_KEY ?? getSetting(this.runtime, "X_API_KEY");
    ...
    const apiSecretKey =
      typeof apiSecretKeyRaw === "string" ? apiSecretKeyRaw : undefined;
    const accessToken =
      typeof accessTokenRaw === "string" ? accessTokenRaw : undefined;
    const accessTokenSecret =
      typeof accessTokenSecretRaw === "string"
        ? accessTokenSecretRaw
        : undefined;
    ...
    if (missing.length) {
      throw new Error(
        `Missing required X env credentials: ${missing.join(", ")}`,
      );
    }
