...
export class XTimelineClient {
  ...
  constructor(
    ...
    const timelineMode =
      getSetting(this.runtime, "X_TIMELINE_MODE") ??
      process.env.X_TIMELINE_MODE;
    this.timelineType =
  ...
  formMessage(runtime: IAgentRuntime, post: Post) {
    if (!post.id || !post.userId || !post.conversationId) {
      throw new Error(
        "Post missing required fields: id, userId, or conversationId",
      );
    }
    return {
      ...
      try {
        ...
        const actionRespondPrompt =
          composePromptFromState({
            state,
            template:
              this.runtime.character.templates?.xActionTemplate ||
              xActionTemplate,
          }) +
          `
Post:
...
Choose any combination of [LIKE], [REPOST], [QUOTE], and [REPLY] that are appropriate. Each action must be on its own line. Your response must only include the chosen actions.`;
    ...
    if (prioritizedPosts.length > 0) {
      const actionSummary = prioritizedPosts.map(
        (td: (typeof postDecisions)[0]) => {
          const actions = [];
    ...
    try {
      ...
      const quotePrompt =
        composePromptFromState({
          state,
          template:
            this.runtime.character.templates?.quotePostTemplate ||
            quotePostTemplate,
        }) +
        `
...
${post.text}`;
      ...
      if (postText && typeof postText === "string") {
        if (this.isDryRun) {
          logger.log(
            `[DRY RUN] Would have quoted post ${post.id} with: ${postText}`,
          );
          return;
        }
        ...
        const result = await this.client.requestQueue.add(
          async () =>
            await this.xClient.sendQuotePost(postTextValue, postIdValue),
        );
        ...
        const postResult =
          body?.data?.create_post?.post_results?.result || body?.data || body;
        if (postResult) {
    ...
    try {
      ...
      const replyPrompt =
        composePromptFromState({
          state,
          template:
            this.runtime.character.templates?.replyPostTemplate ||
            replyPostTemplate,
        }) +
        `
...
${post.text}`;
      ...
      if (responseObject?.post && typeof responseObject.post === "string") {
        ...
        if (result) {
          ...
          // Create memory for our response
          const replyPostId =
            result.id || result.data?.id || Date.now().toString();
