...
export class XPostClient {
  ...
  async start() {
    ...
    const generateNewPostLoop = async () => {
      ...
      logger.info(
        `Next post scheduled in ${postIntervalMinutes.toFixed(1)} minutes`,
      );
  ...
  async generateNewPost(): Promise<boolean> {
    ...
    try {
      ...
      logger.info(
        `Generating post for user: ${this.client.profile?.username} (${userId})`,
      );
...
Generate a single post that sounds like YOU would actually write it:`;
      ...
      if (postText.length > 280) {
        logger.warn(
          `Generated post too long (${postText.length} chars), truncating...`,
        );
        ...
        const finalPost =
          truncated.trim() || `${postText.substring(0, 277)}...`;
        ...
        logger.info(
          "Post created successfully (memory saving disabled due to room constraints)",
        );
        return true;
      }
  ...
  private async postToX(
    ...
    if (isDuplicate) {
      logger.warn(
        "Post is a duplicate of a recent post. Skipping to avoid duplicate.",
      );
      return null;
    }
