 ...
 */
import { ENTITY_OPS } from "../types.js";
...
import { describeUserReference } from "@elizaos/core";
...
export const entityAction: Action = {
    ...
    description: "Direct CRUD over the runtime knowledge graph (entities + typed edges): create | read | list | log_interaction | set_relationship. Identity claims and merges require the deterministic identity authority and are not agent actions.",
    descriptionCompressed: "KNOWLEDGE_GRAPH create|read|list|log_interaction|set_relationship",
    tags: [
    ...
    validate: async (runtime: IAgentRuntime, message: Memory, _state?: State): Promise<boolean> => {
        if (!resolveKnowledgeGraphService(runtime))
            return false;
        return hasRoleAccess(runtime, message, "OWNER");
    },
    handler: async (runtime: IAgentRuntime, message: Memory, _state: State | undefined, options: HandlerOptions | undefined, callback: HandlerCallback | undefined): Promise<ActionResult> => {
        ...
        if (!op) {
            const text = "Tell me which knowledge-graph op: create, read, list, log_interaction, or set_relationship. Identity verification and merging require the deterministic identity authority.";
            await callback?.({
                text,
                source: "action",
                action: RELATIONSHIPS_ACTION_NAME,
        ...
        const reply = async (result: ActionResult & {
            text: string;
        ...
        logger.info(`${RELATIONSHIPS_LOG_PREFIX} ${RELATIONSHIPS_ACTION_NAME} op=${op}`);
        switch (op) {
            case "create": {
            ...
            case "list": {
                ...
                const unfilteredPage = kind && entities.length === 0
