...
import { type DurableRecordStore } from "@elizaos/core";
...
export function graphRecordRepository(runtime: IAgentRuntime, agentId: string): GraphRecordRepository | null {
    ...
    if (storage.version !== 1 ||
        ...
        throw failure("Canonical graph records require this runtime's agent-bound version 1 database");
...
export class GraphRecordRepository {
    ...
    transaction<T>(operation: () => Promise<T>): Promise<T> {
        return this.storage.transaction(async () => {
            ...
            if (version !== null && version !== 1)
                throw failure("Canonical graph record schema requires an explicit migration");
            if (version === null)
                await this.storage.set(SCHEMA, "version", 1);
            return operation();
        });
    }
    ...
    async putEntity(entity: Entity): Promise<Entity> {
        ...
        for (const identity of entity.identities) {
            const normalized = {
                ...identity,
                connectorAccountId: normalizeEntityConnectorAccountId(identity.connectorAccountId),
            };
    ...
    async putRelationship(relationship: Relationship): Promise<Relationship> {
        await this.storage.set(RELATIONSHIPS, relationship.relationshipId, relationship);
        return structuredClone(relationship);
    }
    async listRelationships(filter?: RelationshipFilter): Promise<Relationship[]> {
        const rows = (await this.storage.getAll<Relationship>(RELATIONSHIPS)).filter((row) => {
            if (!filter?.includeRetired && row.status !== "active")
                return false;
    ...
    async retargetRelationships(sourceId: string, targetId: string, now: string): Promise<void> {
        for (const row of await this.storage.getAll<Relationship>(RELATIONSHIPS)) {
            ...
            await this.putRelationship({
                ...row,
                fromEntityId: row.fromEntityId === sourceId ? targetId : row.fromEntityId,
    ...
    async listAudit(id: string): Promise<Omit<GraphAuditRecord, "relationshipId">[]> {
