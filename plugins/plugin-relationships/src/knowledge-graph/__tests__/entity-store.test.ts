 ...
 */
import { AUTO_MERGE_CONFIDENCE_THRESHOLD } from "../entity-store.ts";
...
function likePattern(sql: string, column: string): string | undefined {
    const escaped = column.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const match = sql.match(new RegExp(`${escaped}\\s+LIKE\\s+'((?:[^']|'')*)'`, "i"));
    return match?.[1]?.replace(/''/g, "'");
}
...
function parseInsert(sql: string): {
    ...
    const values = splitSqlList(afterCols.slice(valOpen + 1, valClose)).map(decodeSqlLiteral);
...
function inList(sql: string): string[] | undefined {
    ...
    return splitSqlList(sql.slice(open + 1, close)).map((item) => String(decodeSqlLiteral(item)));
}
class MemoryKnowledgeGraph {
    ...
    execute = async (query: QueryChunks): Promise<{
        rows: Array<Record<string, unknown>>;
    }> => {
        return { rows: this.dispatch(extractSql(query)) };
    };
    private upsert(table: Array<Record<string, unknown>>, incoming: Record<string, unknown>, keyCols: string[], updateCols: string[]): void {
        const existing = table.find((row) => keyCols.every((col) => row[col] === incoming[col]));
        if (!existing) {
            table.push({ ...incoming });
            return;
        }
    ...
    private dispatch(sql: string): Array<Record<string, unknown>> {
        if (/INSERT INTO app_lifeops\.life_entities\b/i.test(sql)) {
            ...
            this.upsert(this.entities, parsed.row, ["agent_id", "entity_id"], parsed.conflictUpdateColumns);
            return [];
        }
        if (/INSERT INTO app_lifeops\.life_entity_identities\b/i.test(sql)) {
            const parsed = parseInsert(sql);
            this.upsert(this.identities, parsed.row, ["agent_id", "entity_id", "platform", "connector_account_id", "handle"], parsed.conflictUpdateColumns);
            return [];
        }
        if (/INSERT INTO app_lifeops\.life_entity_attributes\b/i.test(sql)) {
            const parsed = parseInsert(sql);
            this.upsert(this.attributes, parsed.row, ["agent_id", "entity_id", "key"], parsed.conflictUpdateColumns);
            return [];
        }
        if (/DELETE FROM app_lifeops\.life_entity_identities\b/i.test(sql)) {
            const where = parseWhereEquals(sql);
            this.identities = this.identities.filter((row) => !(row.agent_id === where.agent_id && row.entity_id === where.entity_id));
            return [];
        }
        if (/DELETE FROM app_lifeops\.life_entity_attributes\b/i.test(sql)) {
            const where = parseWhereEquals(sql);
            this.attributes = this.attributes.filter((row) => !(row.agent_id === where.agent_id && row.entity_id === where.entity_id));
            return [];
        }
        if (/DELETE FROM app_lifeops\.life_entities\b/i.test(sql)) {
            const where = parseWhereEquals(sql);
            this.entities = this.entities.filter((row) => !(row.agent_id === where.agent_id && row.entity_id === where.entity_id));
            return [];
        }
        if (/UPDATE app_lifeops\.life_entities\b/i.test(sql)) {
            ...
            for (const row of this.entities) {
                if (row.agent_id === where.agent_id &&
        ...
        if (/UPDATE app_lifeops\.life_relationships_v2\b/i.test(sql)) {
            ...
            for (const row of this.relationships) {
                if (row.agent_id !== where.agent_id)
                    continue;
                if (where.from_entity_id !== undefined &&
                    row.from_entity_id === where.from_entity_id) {
                    Object.assign(row, assignments);
                }
                if (where.to_entity_id !== undefined &&
        ...
        if (/SELECT \* FROM app_lifeops\.life_entities\b/i.test(sql)) {
            ...
            const rows = this.entities.filter((row) => row.agent_id === agentId && row.entity_id === entityId);
        ...
        if (/SELECT \* FROM app_lifeops\.life_entity_identities\b/i.test(sql)) {
            ...
            return this.identities
                .filter((row) => row.agent_id === agentId && ids.includes(String(row.entity_id)))
                .sort((a, b) => String(a.added_at ?? "").localeCompare(String(b.added_at ?? "")));
        }
        if (/SELECT \* FROM app_lifeops\.life_entity_attributes\b/i.test(sql)) {
            const agentId = quotedEquals(sql, "agent_id");
            const ids = inList(sql) ?? [];
            return this.attributes.filter((row) => row.agent_id === agentId && ids.includes(String(row.entity_id)));
        }
    ...
    private selectEntities(sql: string): Array<Record<string, unknown>> {
        const agentId = quotedEquals(sql, "e.agent_id") ?? quotedEquals(sql, "agent_id");
        ...
        if (tagLike !== undefined) {
            rows = rows.filter((row) => sqlLike(String(row.tags_json ?? ""), tagLike));
        }
        ...
        if (hasIdentityExists) {
            rows = rows.filter((row) => this.identities.some((identity) => {
                if (identity.agent_id !== row.agent_id ||
        ...
        rows = [...rows].sort((a, b) => String(a.preferred_name ?? "").localeCompare(String(b.preferred_name ?? "")));
...
function identity(overrides: Partial<EntityIdentity> & Pick<EntityIdentity, "platform" | "handle">): EntityIdentity {
    return {
...
describe("EntityStore", () => {
    ...
    it("throws when the runtime has no database adapter", async () => {
        ...
        await expect(store.get("missing")).rejects.toThrow("runtime database adapter unavailable");
    });
    ...
    it("throws when an upsert cannot be read back", async () => {
        ...
        await expect(store.upsert({
            ...
            visibility: "owner_agent_admin",
            state: {},
        })).rejects.toThrow("[EntityStore] failed to read back upserted entity ent-ghost");
    });
    ...
    it("auto-merges a single candidate at the confidence threshold", async () => {
        ...
        expect(result.entity.identities[0]?.confidence).toBe(AUTO_MERGE_CONFIDENCE_THRESHOLD);
    ...
    it("silently no-ops recordInteraction for a missing entity", async () => {
        const { store } = createHarness();
        await expect(store.recordInteraction("missing", {
            platform: "slack",
            direction: "inbound",
            summary: "gone",
            occurredAt: T1,
    ...
    it("throws when merge target is missing, including the empty-source path", async () => {
        const { store } = createHarness();
        await expect(store.merge("missing", [])).rejects.toThrow("[EntityStore.merge] target missing not found");
        await expect(store.merge("missing", ["ent-x"])).rejects.toThrow("[EntityStore.merge] target missing not found");
    });
    ...
    it("refuses to delete the self entity and no-ops delete of a missing id", async () => {
        ...
        await expect(store.deleteForTest(SELF_ENTITY_ID)).rejects.toThrow("[EntityStore] cannot delete self entity");
