...
function rowMatches(row: Record<string, unknown>, whereSql: string): boolean {
    for (const cond of splitAndClauses(whereSql)) {
        ...
        if (!eq)
            return false;
        if (row[eq[1]] !== decodeSqlValue(eq[2]))
            return false;
    }
    return true;
}
...
function executeSql(sqlText: string, tables: GraphTables): {
    rows: Array<Record<string, unknown>>;
} {
    const trimmed = sqlText.trim();
    const insertRel = trimmed.match(/^INSERT\s+INTO\s+app_lifeops\.life_relationships_v2\s*\(([\s\S]+?)\)\s*VALUES\s*\(([\s\S]+?)\)\s*(ON\s+CONFLICT[\s\S]*)?$/i);
    if (insertRel) {
    ...
    const insertAudit = trimmed.match(/^INSERT\s+INTO\s+app_lifeops\.life_relationship_audit_events\s*\(([\s\S]+?)\)\s*VALUES\s*\(([\s\S]+?)\)$/i);
    if (insertAudit) {
    ...
    const selectRel = trimmed.match(/^SELECT\s+\*\s+FROM\s+app_lifeops\.life_relationships_v2\s+WHERE\s+([\s\S]+?)(?:\s+ORDER\s+BY\s+updated_at\s+DESC)?(?:\s+LIMIT\s+(\d+))?\s*$/i);
    if (selectRel) {
        let result = Array.from(tables.relationships.values()).filter((row) => rowMatches(row, selectRel[1].trim()));
    ...
    const selectAudit = trimmed.match(/^SELECT\s+\*\s+FROM\s+app_lifeops\.life_relationship_audit_events\s+WHERE\s+([\s\S]+?)\s+ORDER\s+BY\s+created_at\s+ASC\s*$/i);
    if (selectAudit) {
    ...
    const updateRel = trimmed.match(/^UPDATE\s+app_lifeops\.life_relationships_v2\s+SET\s+([\s\S]+?)\s+WHERE\s+([\s\S]+)$/i);
    if (updateRel) {
        const assignments = parseAssignments(updateRel[1]);
        for (const row of tables.relationships.values()) {
            if (!rowMatches(row, updateRel[2].trim()))
                continue;
...
function createRuntime(agentId: string, tables: GraphTables): IAgentRuntime {
    return {
        ...
        adapter: {
            db: {
                execute: async (query: unknown) => executeSql(extractSql(query), tables),
            },
        },
...
describe("RelationshipStore", () => {
    ...
    it("returns null for a missing relationship id", async () => {
        const store = new RelationshipStore(createRuntime("agent-1", createTables()), "agent-1");
    ...
    it("creates an edge with a generated rel_ id and omits empty metadata", async () => {
        ...
        expect(created.relationshipId).toMatch(/^rel_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i);
    ...
    it("preserves createdAt and status on conflict, and omits zero interactionCount", async () => {
        ...
        const store = new RelationshipStore(createRuntime("agent-1", tables), "agent-1");
        await store.upsert(edgeInput({
            relationshipId: "rel_keep",
            status: "active",
            state: { interactionCount: 0, sentimentTrend: "neutral" },
        }));
        vi.setSystemTime(new Date("2026-06-01T12:00:05.000Z"));
        const updated = await store.upsert(edgeInput({
            relationshipId: "rel_keep",
            fromEntityId: "from-z",
            type: "friend_of",
            confidence: 0.9,
    ...
    it("keeps an existing retired status when the upsert omits status", async () => {
        const store = new RelationshipStore(createRuntime("agent-1", createTables()), "agent-1");
        await store.upsert(edgeInput({ relationshipId: "rel_ret", status: "retired" }));
        const updated = await store.upsert(edgeInput({ relationshipId: "rel_ret", confidence: 0.2 }));
    ...
    it("throws when the inserted row cannot be read back", async () => {
        ...
        await expect(store.upsert(edgeInput())).rejects.toThrow(/failed to read back upserted relationship/);
    });
    it("scopes reads to the store agentId", async () => {
        const tables = createTables();
        const storeA = new RelationshipStore(createRuntime("agent-a", tables), "agent-a");
        const storeB = new RelationshipStore(createRuntime("agent-b", tables), "agent-b");
    ...
    it("lists the empty set, a single element, and DESC updated_at order", async () => {
        ...
        await store.upsert(edgeInput({ relationshipId: "rel_new", toEntityId: "to-c" }));
    ...
    it("excludes retired edges unless includeRetired is set", async () => {
        ...
        await store.upsert(edgeInput({ relationshipId: "rel_gone", toEntityId: "to-x" }));
    ...
    it("filters by from, to, type string, type array, and quoted identifiers", async () => {
        ...
        expect((await store.list({ fromEntityId: "o'reilly" })).map((rel) => rel.relationshipId)).toEqual(["rel_work", "rel_quote"]);
        expect((await store.list({ toEntityId: "pat's" })).map((rel) => rel.relationshipId)).toEqual(["rel_quote"]);
        expect((await store.list({ type: "works_at" })).map((rel) => rel.relationshipId)).toEqual(["rel_work"]);
        expect((await store.list({ type: ["friend_of", "knows"] })).map((rel) => rel.relationshipId)).toEqual(["rel_other", "rel_quote"]);
    ...
    it("applies a finite integer limit in SQL and ignores non-finite limits", async () => {
        ...
        vi.setSystemTime(new Date("2026-06-01T12:00:01.000Z"));
        await store.upsert(edgeInput({ relationshipId: "rel_b", toEntityId: "to-c" }));
        vi.setSystemTime(new Date("2026-06-01T12:00:02.000Z"));
        await store.upsert(edgeInput({ relationshipId: "rel_c", toEntityId: "to-d" }));
        expect((await store.list({ limit: 1 })).map((rel) => rel.relationshipId)).toEqual(["rel_c"]);
        expect(await store.list({ limit: 0 })).toEqual([]);
        expect((await store.list({ limit: 1.9 })).map((rel) => rel.relationshipId)).toEqual(["rel_c"]);
        expect(await store.list({ limit: Number.NaN })).toHaveLength(3);
        expect(await store.list({ limit: Number.POSITIVE_INFINITY })).toHaveLength(3);
    });
    it("matches metadata in memory after the SQL limit, including omitted empty metadata", async () => {
        ...
        vi.setSystemTime(new Date("2026-06-01T12:00:02.000Z"));
        await store.upsert(edgeInput({ relationshipId: "rel_empty", toEntityId: "to-d" }));
        expect((await store.list({ metadataMatch: { role: "keep" } })).map((rel) => rel.relationshipId)).toEqual(["rel_keep"]);
        expect(await store.list({ metadataMatch: { role: "keep", city: "sf" } })).toHaveLength(1);
        expect(await store.list({ metadataMatch: { role: "keep", city: "ny" } })).toEqual([]);
        expect(await store.list({ metadataMatch: { role: "keep" }, limit: 1 })).toEqual([]);
        expect((await store.list({ metadataMatch: { missing: null } })).map((rel) => rel.relationshipId)).toEqual(["rel_skip", "rel_keep"]);
    });
    it("filters cadence-overdue edges, including the invalid-asOf empty-queue path", async () => {
        ...
        expect((await store.list({ cadenceOverdueAsOf: asOf }))
            .map((rel) => rel.relationshipId)
            .sort()).toEqual(["rel_never", "rel_overdue"]);
    ...
    it("logs observe-on-retired without flipping status or timestamps", async () => {
        ...
        if (!retired)
            throw new Error("expected retired edge");
    ...
    it("throws when retiring a missing id and returns no audit rows for it", async () => {
        const store = new RelationshipStore(createRuntime("agent-1", createTables()), "agent-1");
        await expect(store.retire("rel_missing", "gone")).rejects.toThrow(/relationship rel_missing not found/);
    ...
    it("throws when the runtime database adapter is unavailable", async () => {
        const store = new RelationshipStore({ adapter: {} } as unknown as IAgentRuntime, "agent-1");
        await expect(store.get("rel_x")).rejects.toThrow("runtime database adapter unavailable");
    });
});
