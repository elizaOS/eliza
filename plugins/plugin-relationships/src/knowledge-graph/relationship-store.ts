...
import { parseJsonRecord } from "./sql.ts";
...
import { type RelationshipFilter } from "@elizaos/core/knowledge-graph/relationship-types";
import { type RelationshipSentiment } from "@elizaos/core/knowledge-graph/relationship-types";
import { type RelationshipSource } from "@elizaos/core/knowledge-graph/relationship-types";
import { type RelationshipState } from "@elizaos/core/knowledge-graph/relationship-types";
import { type RelationshipStatus } from "@elizaos/core/knowledge-graph/relationship-types";
...
export class RelationshipStore {
    ...
    async upsert(input: Omit<Relationship, "relationshipId" | "createdAt" | "updatedAt" | "status"> & {
        relationshipId?: string;
        status?: RelationshipStatus;
    }): Promise<Relationship> {
        return this.operation(() => this.upsertOperation(input));
    }
    private async upsertOperation(input: Omit<Relationship, "relationshipId" | "createdAt" | "updatedAt" | "status"> & {
        relationshipId?: string;
        status?: RelationshipStatus;
    }): Promise<Relationship> {
        const now = isoNow();
        ...
        await executeRawSql(this.runtime, `INSERT INTO app_lifeops.life_relationships_v2 (
       ...
       ON CONFLICT (relationship_id) DO UPDATE SET
        ...
        if (!fetched) {
            throw new Error(`[RelationshipStore] failed to read back upserted relationship ${relationshipId}`);
        }
        return fetched;
    }
    ...
    private async getOperation(relationshipId: string): Promise<Relationship | null> {
        if (this.records)
            return this.records.getRelationship(relationshipId);
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_relationships_v2
    ...
    private async listOperation(filter?: RelationshipFilter): Promise<Relationship[]> {
        ...
        if (filter?.metadataMatch) {
            results = results.filter((rel) => {
                if (!rel.metadata)
                    return false;
                return Object.entries(filter.metadataMatch ?? {}).every(([key, value]) => JSON.stringify(rel.metadata?.[key] ?? null) ===
    ...
    private async observeOperation(obs: {
        ...
        if (active) {
            const mergedEvidence = Array.from(new Set([...active.evidence, ...obs.evidence]));
    ...
    private async retireOperation(relationshipId: string, reason: string): Promise<void> {
        const existing = await this.getOperation(relationshipId);
        if (!existing) {
            throw new Error(`[RelationshipStore.retire] relationship ${relationshipId} not found`);
        }
        const now = isoNow();
        if (this.records) {
        ...
        await executeRawSql(this.runtime, `UPDATE app_lifeops.life_relationships_v2
          SET status = 'retired',
    ...
    async listAuditEvents(relationshipId: string): Promise<Array<{
        id: string;
        kind: string;
    ...
    private async listAuditEventsOperation(relationshipId: string): Promise<Array<{
        id: string;
        kind: string;
        details: Record<string, unknown>;
        createdAt: string;
        ...
        if (this.records)
            return this.records.listAudit(relationshipId);
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_relationship_audit_events
        WHERE agent_id = ${sqlQuote(this.agentId)}
          AND relationship_id = ${sqlQuote(relationshipId)}
    ...
    private async appendAudit(relationshipId: string, kind: string, details: Record<string, unknown>): Promise<void> {
        if (this.records)
        ...
        await executeRawSql(this.runtime, `INSERT INTO app_lifeops.life_relationship_audit_events (
