...
import { AUTO_MERGE_CONFIDENCE_THRESHOLD } from "@elizaos/core/knowledge-graph/merge";
import { SELF_ENTITY_ID } from "@elizaos/core/knowledge-graph/entity-types";
import { confirmEmailRecipient } from "./confirmed-email-recipient.ts";
import { decideIdentityOutcome } from "@elizaos/core/knowledge-graph/merge";
import { executeRawSql } from "./sql.ts";
import { findIdentityMatches } from "@elizaos/core/knowledge-graph/merge";
import { foldIdentity } from "@elizaos/core/knowledge-graph/merge";
import { graphRecordRepository } from "./record-repository.ts";
import { mergeEntities } from "@elizaos/core/knowledge-graph/merge";
import { normalizeEntityConnectorAccountId } from "@elizaos/core/knowledge-graph/entity-types";
import { parseJsonArray } from "./sql.ts";
import { parseJsonValue } from "./sql.ts";
import { sqlInteger } from "./sql.ts";
import { sqlJson } from "./sql.ts";
import { sqlNumber } from "./sql.ts";
import { sqlQuote } from "./sql.ts";
import { sqlText } from "./sql.ts";
import { toBoolean } from "./sql.ts";
import { toNumber } from "./sql.ts";
import { toText } from "./sql.ts";
import { type ConfirmEmailRecipientInput } from "./confirmed-email-recipient.ts";
import { type ConfirmedEmailRecipient } from "./confirmed-email-recipient.ts";
import { type Entity } from "@elizaos/core/knowledge-graph/entity-types";
import { type EntityAttribute } from "@elizaos/core/knowledge-graph/entity-types";
import { type EntityFilter } from "@elizaos/core/knowledge-graph/entity-types";
import { type EntityIdentity } from "@elizaos/core/knowledge-graph/entity-types";
import { type EntityIdentityAddedVia } from "@elizaos/core/knowledge-graph/entity-types";
import { type EntityResolveCandidate } from "@elizaos/core/knowledge-graph/entity-types";
import { type EntityState } from "@elizaos/core/knowledge-graph/entity-types";
import { type EntityVisibility } from "@elizaos/core/knowledge-graph/entity-types";
import { type GraphRecordRepository } from "./record-repository.ts";
...
function identityRowToIdentity(row: Record<string, unknown>): EntityIdentity {
    ...
    return {
        ...
        connectorAccountId: normalizeEntityConnectorAccountId(toText(row.connector_account_id)),
        ...(displayName ? { displayName } : {}),
        verified: toBoolean(row.verified),
        confidence: toNumber(row.confidence, 0),
        addedAt: toText(row.added_at),
        addedVia: toText(row.added_via, "platform_observation") as EntityIdentityAddedVia,
        evidence,
    };
}
...
export class EntityStore {
    ...
    constructor(private readonly runtime: IAgentRuntime, private readonly agentId: string) {
        this.records = graphRecordRepository(runtime, agentId);
    }
    confirmEmailRecipient(input: ConfirmEmailRecipientInput): Promise<ConfirmedEmailRecipient> {
    ...
    async upsert(input: Omit<Entity, "entityId" | "createdAt" | "updatedAt"> & {
        entityId?: string;
    }): Promise<Entity> {
        return this.operation(() => this.upsertInternal(input));
    }
    private async upsertInternal(input: Omit<Entity, "entityId" | "createdAt" | "updatedAt"> & {
        entityId?: string;
    }): Promise<Entity> {
        const now = isoNow();
        ...
        await executeRawSql(this.runtime, `INSERT INTO app_lifeops.life_entities (
       ...
       ON CONFLICT (agent_id, entity_id) DO UPDATE SET
        ...
        await executeRawSql(this.runtime, `DELETE FROM app_lifeops.life_entity_identities
        ...
        await executeRawSql(this.runtime, `DELETE FROM app_lifeops.life_entity_attributes
        WHERE agent_id = ${sqlQuote(this.agentId)}
          AND entity_id = ${sqlQuote(entityId)}`);
    ...
    private async persistIdentity(entityId: string, identity: EntityIdentity): Promise<void> {
        await executeRawSql(this.runtime, `INSERT INTO app_lifeops.life_entity_identities (
    ...
    private async persistAttribute(entityId: string, key: string, attribute: EntityAttribute): Promise<void> {
        await executeRawSql(this.runtime, `INSERT INTO app_lifeops.life_entity_attributes (
    ...
    private async getOperation(entityId: string): Promise<Entity | null> {
        if (this.records)
            return this.records.getEntity(entityId);
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_entities
        WHERE agent_id = ${sqlQuote(this.agentId)}
          AND entity_id = ${sqlQuote(entityId)}
        LIMIT 1`);
        const row = rows[0];
    ...
    private async listOperation(filter?: EntityFilter): Promise<Entity[]> {
        if (this.records)
            return this.records.listEntities(filter);
        ...
        if (filter?.nameContains) {
            ...
            clauses.push(`(LOWER(e.preferred_name) LIKE ${needle} OR LOWER(COALESCE(e.full_name, '')) LIKE ${needle})`);
        }
        if (filter?.hasPlatform || filter?.hasConnectorAccountId) {
            ...
            if (filter.hasPlatform) {
                identityClauses.push(`LOWER(i.platform) = ${sqlQuote(filter.hasPlatform.toLowerCase())}`);
            }
            if (filter.hasConnectorAccountId) {
                identityClauses.push(`i.connector_account_id = ${sqlQuote(normalizeEntityConnectorAccountId(filter.hasConnectorAccountId))}`);
            }
            clauses.push(`EXISTS (SELECT 1 FROM app_lifeops.life_entity_identities i
                  WHERE ${identityClauses.join("\n                    AND ")})`);
        }
        const limitClause = typeof filter?.limit === "number" && Number.isFinite(filter.limit)
            ? `LIMIT ${sqlInteger(filter.limit)}`
            : "";
        const rows = await executeRawSql(this.runtime, `SELECT e.* FROM app_lifeops.life_entities e
    ...
    private async loadIdentities(entityIds: string[]): Promise<Map<string, EntityIdentity[]>> {
        if (entityIds.length === 0)
            return new Map();
        const idList = entityIds.map((id) => sqlQuote(id)).join(", ");
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_entity_identities
        WHERE agent_id = ${sqlQuote(this.agentId)}
    ...
    private async loadAttributes(entityIds: string[]): Promise<Map<string, Record<string, EntityAttribute>>> {
        if (entityIds.length === 0)
            return new Map();
        const idList = entityIds.map((id) => sqlQuote(id)).join(", ");
        const rows = await executeRawSql(this.runtime, `SELECT * FROM app_lifeops.life_entity_attributes
        WHERE agent_id = ${sqlQuote(this.agentId)}
          AND entity_id IN (${idList})`);
    ...
    private async observeIdentityOperation(obs: {
        ...
        const connectorAccountId = normalizeEntityConnectorAccountId(obs.connectorAccountId);
        ...
        if (outcome.kind === "merge") {
            const target = candidates.find((c) => c.entityId === outcome.targetEntityId);
            if (!target) {
    ...
    private async resolveOperation(query: {
        ...
        if (query.name)
            filters.nameContains = query.name;
        if (query.identity) {
            filters.hasPlatform = query.identity.platform;
            filters.hasConnectorAccountId = normalizeEntityConnectorAccountId(query.identity.connectorAccountId);
        }
        ...
        if (query.identity) {
            ...
            const accountKey = normalizeEntityConnectorAccountId(query.identity.connectorAccountId);
            entities = entities.filter((entity) => entity.identities.some((identity) => identity.platform.toLowerCase() === platformKey &&
        ...
        return entities
            ...
            if (query.identity) {
                const match = entity.identities.find((identity) => identity.platform.toLowerCase() ===
                    query.identity?.platform.toLowerCase() &&
                    normalizeEntityConnectorAccountId(identity.connectorAccountId) ===
                        normalizeEntityConnectorAccountId(query.identity.connectorAccountId) &&
            ...
            if (query.name && entity.preferredName) {
                const exact = entity.preferredName.toLowerCase() === query.name.toLowerCase();
     ...
     * applicable.
     */
    async recordInteraction(entityId: string, interaction: {
        platform: string;
        direction: "inbound" | "outbound";
        summary: string;
        occurredAt: string;
    }): Promise<void> {
        return this.operation(() => this.recordInteractionOperation(entityId, interaction));
    }
    private async recordInteractionOperation(entityId: string, interaction: {
        platform: string;
        direction: "inbound" | "outbound";
        summary: string;
        occurredAt: string;
    }): Promise<void> {
        const directionColumn = interaction.direction === "inbound"
        ...
        await executeRawSql(this.runtime, `UPDATE app_lifeops.life_entities
    ...
    private async mergeOperation(targetId: string, sourceIds: string[]): Promise<Entity> {
        ...
        for (const source of sources) {
            if (this.records) {
                await this.records.retargetRelationships(source.entityId, targetId, now);
                await this.records.deleteEntity(source.entityId);
                continue;
            }
            const rewriteSql = (column: string) => `UPDATE app_lifeops.life_relationships_v2
          ...
          WHERE agent_id = ${sqlQuote(this.agentId)}
            AND entity_id = ${sqlQuote(source.entityId)}`);
            await executeRawSql(this.runtime, `DELETE FROM app_lifeops.life_entities
          WHERE agent_id = ${sqlQuote(this.agentId)}
            AND entity_id = ${sqlQuote(source.entityId)}`);
        }
        return persisted;
    ...
    private async deleteForTestOperation(entityId: string): Promise<void> {
        ...
        if (this.records)
            return this.records.deleteEntity(entityId);
        await executeRawSql(this.runtime, `DELETE FROM app_lifeops.life_entity_identities
        WHERE agent_id = ${sqlQuote(this.agentId)}
          AND entity_id = ${sqlQuote(entityId)}`);
        await executeRawSql(this.runtime, `DELETE FROM app_lifeops.life_entity_attributes
