...
function entity(id: string, identities: EntityIdentity[] = []): Omit<Entity, "createdAt" | "updatedAt"> {
    return {
        entityId: id,
        type: "person",
        preferredName: id,
...
function identity(handle: string, connectorAccountId = "default"): EntityIdentity {
    return {
        platform: "email",
        handle,
        connectorAccountId,
...
afterEach(async () => {
    for (const adapter of opened.splice(0))
        await adapter.close();
...
it("preserves full graph records, identity evidence, retirement and audit after native close/reopen", async () => {
    ...
    expect(await reopened.relationships.get(edge.relationshipId)).toEqual(retired);
    expect(await reopened.relationships.listAuditEvents(edge.relationshipId)).toEqual(audit);
    expect(audit).toMatchObject([
        { kind: "retire", details: { reason: "synthetic withdrawal" } },
    ]);
    expect(await reopened.relationships.list()).toEqual([]);
    expect(await reopened.entities.list({
        hasPlatform: "EMAIL",
...
it("serializes concurrent identity observations and edge strengthening without losing evidence", async () => {
    ...
    const observations = await Promise.all(Array.from({ length: 12 }, (_, i) => entities.observeIdentity({
        platform: "email",
    ...
    const edges = await Promise.all(Array.from({ length: 12 }, (_, i) => relationships.observe({
        fromEntityId: "self",
...
it("merges through the shared engine, retargets both edge ends and rolls back the whole graph on failure", async () => {
    ...
    const target = await entities.upsert(entity("target", [identity("target@example.test")]));
    ...
    await expect(adapter.recordStore.transaction(async () => {
...
it("confirms a recipient once under concurrency and rejects ambiguity, stale review and conflicting identity", async () => {
    ...
    const replies = await Promise.all(Array.from({ length: 6 }, () => entities.confirmEmailRecipient(request)));
    ...
    await expect(entities.confirmEmailRecipient({ ...request, name: "Changed name" })).rejects.toMatchObject({ code: "ENTITY_RECIPIENT_REVIEW_STALE" });
    await entities.upsert(entity("other"));
    await expect(entities.confirmEmailRecipient({ ...request, entityId: "other" })).rejects.toMatchObject({ code: "ENTITY_RECIPIENT_IDENTITY_CONFLICT" });
    ...
    await expect(entities.confirmEmailRecipient({ ...request, address: "bad\naddress" })).rejects.toMatchObject({ code: "ENTITY_RECIPIENT_INVALID" });
});
