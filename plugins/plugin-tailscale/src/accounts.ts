...
function readRawField(record: RawAccountRecord, keys: readonly string[]): unknown {
  const credentials =
...
function normalizeBackend(value: unknown): 'local' | 'cloud' | 'auto' | undefined {
  return value === 'local' || value === 'cloud' || value === 'auto' ? value : undefined;
}

function accountFromRecord(record: RawAccountRecord): TailscaleAccountConfig | null {
  const accountId = normalizeTailscaleAccountId(record.accountId ?? record.id ?? record.name);
  const account: TailscaleAccountConfig = {
    accountId,
    authKey: nonEmptyString(
      readRawField(record, ['TAILSCALE_AUTH_KEY', 'authKey', 'accessToken', 'access']),
    ),
    tags: readRawField(record, ['TAILSCALE_TAGS', 'tags']) as string | string[] | undefined,
    funnel: readRawField(record, ['TAILSCALE_FUNNEL', 'funnel']) as string | boolean | undefined,
    defaultPort: readRawField(record, ['TAILSCALE_DEFAULT_PORT', 'defaultPort']) as
      | string
      | number
      | undefined,
    backend: normalizeBackend(readRawField(record, ['TAILSCALE_BACKEND', 'backend'])),
...
export function readTailscaleAccounts(runtime: IAgentRuntime): TailscaleAccountConfig[] {
  ...
  for (const record of parseAccountsJson(readSetting(runtime, 'TAILSCALE_ACCOUNTS'))) {
  ...
  addAccount(accounts, {
    ...
    authKeyExpirySeconds: readSetting(runtime, 'TAILSCALE_AUTH_KEY_EXPIRY_SECONDS'),
...
export function resolveTailscaleAccount(
  ...
  return (
    ...
    accounts.find((account) => account.accountId === DEFAULT_TAILSCALE_ACCOUNT_ID) ??
    accounts[0] ??
    null
  );
}
