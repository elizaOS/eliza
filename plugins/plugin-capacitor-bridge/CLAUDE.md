...
- **Canonical Plugin object.** `mobileDeviceBridgePlugin` owns the bridge service and can be registered through the standard elizaOS plugin lifecycle. The agent bundle still calls `ensureMobileDeviceBridgeInferenceHandlers()` directly so serving handlers remain gated on a live bionic host or attached device.
