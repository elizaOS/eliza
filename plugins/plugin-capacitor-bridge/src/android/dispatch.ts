...
export async function dispatchStreamingRequest(
	...
	// emitted as a response head plus a single body chunk. The plugin kernel does
	// not own the wake or inbox routes, so skipping an inline handler makes that
	// route fail on the streaming channel even though the buffered channel serves
	// it; keeping the chain identical also keeps wake validation at one boundary.
