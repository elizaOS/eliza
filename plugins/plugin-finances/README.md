...
| `TIMEZONE` | No | Agent setting used for bill dueness ("today" as a calendar day) when no owner zone is registered through `registerCalendarTimeZoneResolver` from `@elizaos/shared`. An invalid zone, or an owner resolver that fails, is a `CalendarTimeZoneError` translated into an explicit action or HTTP failure, never a silent fallback zone. With neither configured the host zone is used. |
