...
//!
//! Async HTTP client for Twitter API v2 interactions using reqwest.
...
impl TwitterClient {
    ...
    fn generate_signature(
        ...
        let signing_key = format!(
            "{}&{}",
            urlencoding::encode(&self.config.api_secret_key),
    ...
    fn parse_tweet(&self, tweet_data: &serde_json::Value, includes: &serde_json::Value) -> Tweet {
        ...
        let metrics = &tweet_data["public_metrics"];
        let entities = &tweet_data["entities"];
        let attachments = &tweet_data["attachments"];
        ...
        let refs = tweet_data["referenced_tweets"].as_array();
        let is_reply = refs
        ...
        let id = tweet_data["id"].as_str().unwrap_or("").to_string();
        ...
        Tweet {
            id: id.clone(),
            text: tweet_data["text"].as_str().unwrap_or("").to_string(),
            author_id: tweet_data["author_id"].as_str().map(String::from),
            conversation_id: tweet_data["conversation_id"].as_str().map(String::from),
            created_at,
            language: tweet_data["lang"].as_str().map(String::from),
            username: author
            ...
            is_retweet,
            is_quote,
            is_sensitive: tweet_data["possibly_sensitive"].as_bool().unwrap_or(false),
            permanent_url: format!("https://twitter.com/i/status/{}", id),
    ...
    pub async fn get_tweet(&self, tweet_id: &str) -> Result<Option<Tweet>> {
        ...
        match self.check_response(response).await {
            Ok(response) => {
                ...
                Ok(Some(self.parse_tweet(&data["data"], includes)))
            }
            Err(TwitterError::ApiError { status: 404, .. }) => Ok(None),
            Err(e) => Err(e),
        }
    }

    /// Post a new tweet.
    pub async fn post_tweet(&self, text: &str) -> Result<TweetCreateResult> {
        if self.config.dry_run {
            debug!("Dry run: would post tweet: {}", text);
            return Ok(TweetCreateResult {
    ...
    pub async fn post_reply(&self, text: &str, reply_to_id: &str) -> Result<TweetCreateResult> {
        if self.config.dry_run {
    ...
    pub async fn delete_tweet(&self, tweet_id: &str) -> Result<bool> {
        if self.config.dry_run {
            debug!("Dry run: would delete tweet: {}", tweet_id);
            return Ok(true);
        }
    ...
    /// Like a tweet.
    pub async fn like_tweet(&mut self, tweet_id: &str) -> Result<bool> {
        if self.config.dry_run {
            debug!("Dry run: would like tweet: {}", tweet_id);
            return Ok(true);
        }
    ...
    /// Retweet a tweet.
    pub async fn retweet(&mut self, tweet_id: &str) -> Result<bool> {
        if self.config.dry_run {
            debug!("Dry run: would retweet: {}", tweet_id);
            return Ok(true);
        }
    ...
    pub async fn get_home_timeline(
        ...
        let includes = &data["includes"];
        let tweets: Vec<Tweet> = data["data"]
            .as_array()
            .map(|arr| arr.iter().map(|t| self.parse_tweet(t, includes)).collect())
            .unwrap_or_default();

        Ok(QueryTweetsResponse {
            tweets,
