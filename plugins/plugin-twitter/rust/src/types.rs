...
impl TwitterConfig {
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        let api_key = std::env::var("TWITTER_API_KEY")
            .map_err(|_| anyhow::anyhow!("TWITTER_API_KEY is required"))?;
        let api_secret_key = std::env::var("TWITTER_API_SECRET_KEY")
            .map_err(|_| anyhow::anyhow!("TWITTER_API_SECRET_KEY is required"))?;
        let access_token = std::env::var("TWITTER_ACCESS_TOKEN")
            .map_err(|_| anyhow::anyhow!("TWITTER_ACCESS_TOKEN is required"))?;
        let access_token_secret = std::env::var("TWITTER_ACCESS_TOKEN_SECRET")
            .map_err(|_| anyhow::anyhow!("TWITTER_ACCESS_TOKEN_SECRET is required"))?;

        let mut config = Self::new(&api_key, &api_secret_key, &access_token, &access_token_secret);

        if let Ok(bearer) = std::env::var("TWITTER_BEARER_TOKEN") {
            config.bearer_token = Some(bearer);
        }

        if let Ok(dry_run) = std::env::var("TWITTER_DRY_RUN") {
