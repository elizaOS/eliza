...
Async HTTP client for Twitter API v2 interactions using httpx.
...
class TwitterClient:
    ...
    def _generate_oauth_signature(
        ...
        signing_key = "&".join([
            urllib.parse.quote(self._config.api_secret_key, safe=""),
    ...
    def _get_headers(self, method: str = "GET", url: str = "") -> dict[str, str]:
        ...
        msg = "No valid authentication configured"
        raise TwitterClientError(msg)
        ...
        *,
        ...
        if not response.is_success:
            ...
            raise TwitterClientError(
                f"Twitter API error ({response.status_code}): {error_text}",
    ...
    def _parse_tweet(self, tweet_data: dict, includes: dict | None = None) -> Tweet:
        """Parse a tweet from API v2 response."""
        ...
        for poll_id in attachments.get("poll_ids", []):
            ...
            if p:
                poll = PollData(
                    ...
                    options=[
                        PollOption(position=o.get("position"), label=o.get("label", ""), votes=o.get("votes"))
        ...
        return Tweet(
            ...
            poll=poll,
            place=place,
            in_reply_to_status_id=next((r["id"] for r in refs if r.get("type") == "replied_to"), None),
            quoted_status_id=next((r["id"] for r in refs if r.get("type") == "quoted"), None),
            retweeted_status_id=next((r["id"] for r in refs if r.get("type") == "retweeted"), None),
    ...
    async def get_tweet(self, tweet_id: str) -> Tweet | None:
        ...
        try:
            data = await self._request("GET", f"/tweets/{tweet_id}", params={
            ...
            return self._parse_tweet(tweet_data, data.get("includes"))
    ...
    async def post_tweet(
        ...
        *,
        ...
        if self._config.dry_run:
            return TweetCreateResult(id="dry-run", text=text)
    ...
    async def delete_tweet(self, tweet_id: str) -> bool:
        ...
        data = await self._request("DELETE", f"/tweets/{tweet_id}")
    ...
    async def like_tweet(self, tweet_id: str) -> bool:
        ...
        me = await self.me()
        data = await self._request("POST", f"/users/{me.id}/likes", json={"tweet_id": tweet_id})
    ...
    async def unlike_tweet(self, tweet_id: str) -> bool:
        ...
        me = await self.me()
        data = await self._request("DELETE", f"/users/{me.id}/likes/{tweet_id}")
        return not data.get("data", {}).get("liked", True)
    ...
    async def retweet(self, tweet_id: str) -> bool:
        ...
        me = await self.me()
        data = await self._request("POST", f"/users/{me.id}/retweets", json={"tweet_id": tweet_id})
    ...
    async def unretweet(self, tweet_id: str) -> bool:
        ...
        me = await self.me()
        data = await self._request("DELETE", f"/users/{me.id}/retweets/{tweet_id}")
        return not data.get("data", {}).get("retweeted", True)
    ...
    async def get_home_timeline(
        ...
        tweets = [self._parse_tweet(t, data.get("includes")) for t in data.get("data", [])]

        return QueryTweetsResponse(
            tweets=tweets,
        ...
        *,
        ...
        while count < max_results:
            ...
            for tweet_data in data.get("data", []):
                yield self._parse_tweet(tweet_data, data.get("includes"))
                count += 1
    ...
    async def follow_user(self, user_id: str) -> bool:
        ...
        me = await self.me()
        data = await self._request("POST", f"/users/{me.id}/following", json={"target_user_id": user_id})
    ...
    async def unfollow_user(self, user_id: str) -> bool:
        ...
        return not data.get("data", {}).get("following", True)
