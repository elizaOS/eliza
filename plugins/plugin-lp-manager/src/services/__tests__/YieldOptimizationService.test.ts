...
describe("YieldOptimizationService", () => {
  ...
  const testUserProfile: UserLpProfile = {
    userId: testUserId,
    vaultPublicKey: "dummyPk",
    encryptedSecretKey: "dummyKey",
    autoRebalanceConfig: {
      enabled: true,
  ...
  const tokenA_SOL: TokenBalance = {
    address: solMint,
    balance: "1000000000",
    decimals: 9,
    symbol: "SOL",
  };
  const tokenB_USDC: TokenBalance = {
    address: usdcMint,
    balance: "100000000",
    decimals: 6,
    symbol: "USDC",
  };
  const mockPoolBasicSolUsdc: PoolInfo = {
    id: "basic-sol-usdc",
    dex: "basic",
    tokenA: { mint: solMint, symbol: "SOL" },
    tokenB: { mint: usdcMint, symbol: "USDC" },
    apr: 0.1,
    tvl: 2000000,
    displayName: "Basic SOL/USDC",
  };
  const mockPoolHighYieldSolUsdc: PoolInfo = {
    id: "high-yield-sol-usdc",
    dex: "high-yield",
    tokenA: { mint: solMint, symbol: "SOL" },
    tokenB: { mint: usdcMint, symbol: "USDC" },
    apr: 0.15,
    tvl: 1500000,
    displayName: "High Yield SOL/USDC",
  };
  const mockPoolSuperHighYieldSolUsdc: PoolInfo = {
    id: "super-high-yield",
    dex: "super-yield",
    tokenA: { mint: solMint, symbol: "SOL" },
    tokenB: { mint: usdcMint, symbol: "USDC" },
    apr: 0.25,
    tvl: 1000000,
    displayName: "Super High Yield SOL/USDC",
  };
  const currentPositionBasic: LpPositionDetails = {
    poolId: "basic-sol-usdc",
    dex: "basic",
    lpTokenBalance: {
      address: "lpBasic",
      balance: "100",
    ...
    valueUsd: 1000,
    metadata: { apr: 0.1, poolDisplayName: "Current SOL/USDC Basic" },
  };
  ...
  describe("findBestYieldOpportunities", () => {
    ...
    it("should not suggest rebalancing if net gain is below threshold after costs", async () => {
      ...
      const poolFailsAfterCost: PoolInfo = {
        ...
        dex: "fail-dex",
        tokenA: { mint: solMint },
        tokenB: { mint: usdcMint },
        apr: targetAprToFail,
        tvl: 500000,
        displayName: "Pool Fails After Cost",
      };
      (mockDexInteractionService.getPools as Mock).mockResolvedValue([
        mockPoolBasicSolUsdc,
        poolFailsAfterCost,
