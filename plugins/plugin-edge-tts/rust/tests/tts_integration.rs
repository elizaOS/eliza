...
use elizaos_plugin_edge_tts::{EdgeTTSParams, EdgeTTSPlugin, EdgeTTSService, EdgeTTSSettings};

/// Check if a network error occurred (to skip test gracefully).
fn is_network_error(err: &elizaos_plugin_edge_tts::EdgeTTSError) -> bool {
    ...
    async fn should_convert_text_to_speech() {
        ...
        match service.text_to_speech(test_text).await {
            Ok(audio_data) => {
                ...
                let is_id3 = audio_data.len() >= 3 && &audio_data[..3] == b"ID3";
    ...
    async fn should_generate_with_speed() {
        ...
        match service.text_to_speech_with_params(&params).await {
            Ok(audio_data) => {
                assert!(
                    !audio_data.is_empty(),
                    "Speed adjustment returned empty data"
                );
                println!("SUCCESS: Speed 1.5x generated {} bytes", audio_data.len());
            }
            Err(ref e) if is_network_error(e) => {
                eprintln!("Skipping: network unavailable");
            }
    ...
    async fn should_generate_with_rate() {
        ...
        match service.text_to_speech_with_params(&params).await {
            Ok(audio_data) => {
                assert!(
                    !audio_data.is_empty(),
                    "Rate adjustment returned empty data"
                );
    ...
    async fn should_generate_with_volume() {
        ...
        match service.text_to_speech_with_params(&params).await {
            Ok(audio_data) => {
                ...
                println!("SUCCESS: Volume +20% generated {} bytes", audio_data.len());
            }
            Err(ref e) if is_network_error(e) => {
                eprintln!("Skipping: network unavailable");
            }
    ...
    async fn should_generate_with_custom_settings() {
        ...
        match plugin.text_to_speech("Testing custom settings.").await {
            Ok(audio_data) => {
                assert!(
                    !audio_data.is_empty(),
                    "Custom settings returned empty audio"
                );
                println!(
