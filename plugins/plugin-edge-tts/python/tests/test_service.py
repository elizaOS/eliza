...
class TestEdgeTTSServiceLiveTTS:
    ...
    async def test_voice_preset(self) -> None:
        ...
        service = EdgeTTSService()
        try:
            audio_data = await service.text_to_speech("Testing voice preset.", voice="alloy")
            assert len(audio_data) > 0
        except Exception as e:
            error_msg = str(e).lower()
            if "network" in error_msg or "connect" in error_msg:
    ...
    async def test_rate_adjustment(self) -> None:
        ...
        service = EdgeTTSService()
        try:
            audio_data = await service.text_to_speech("Testing rate adjustment.", rate="+20%")
            assert len(audio_data) > 0
        except Exception as e:
            error_msg = str(e).lower()
            if "network" in error_msg or "connect" in error_msg:
    ...
    async def test_pitch_adjustment(self) -> None:
        ...
        service = EdgeTTSService()
        try:
            audio_data = await service.text_to_speech("Testing pitch adjustment.", pitch="+10Hz")
            assert len(audio_data) > 0
        except Exception as e:
            error_msg = str(e).lower()
            if "network" in error_msg or "connect" in error_msg:
    ...
    async def test_volume_adjustment(self) -> None:
        ...
        service = EdgeTTSService()
        try:
            audio_data = await service.text_to_speech("Testing volume adjustment.", volume="+20%")
            assert len(audio_data) > 0
        except Exception as e:
            error_msg = str(e).lower()
            if "network" in error_msg or "connect" in error_msg:
