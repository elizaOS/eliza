...
export class SecretsService extends Service {
  ...
  private async migrateAliasedKeys(): Promise<number> {
    ...
    for (const [alias, canonical] of Object.entries(SECRET_KEY_ALIASES)) {
      ...
      if (success) {
        ...
        // Also sync to process.env under canonical name
  ...
  async set(
    ...
    const success = await this.storage.set(canonicalKey, value, context, config);
    ...
    if (success) {
      ...
      this.logAccess(canonicalKey, "write", context, false, "Storage operation failed");
    }
  ...
  async delete(key: string, context: SecretContext): Promise<boolean> {
    ...
    if (success) {
      ...
      this.logAccess(canonicalKey, "delete", context, false, "Secret not found");
    }
