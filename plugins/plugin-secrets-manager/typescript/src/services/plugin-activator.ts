...
export class PluginActivatorService extends Service {
  ...
  private secretsReadyListeners: Map<string, Array<(runtime: IAgentRuntime) => Promise<void>>> =
    new Map();

  /** Listeners for secret changed events */
  private secretChangedListeners: Map<
    string,
    Array<(key: string, value: string | null, runtime: IAgentRuntime) => Promise<void>>
  > = new Map();
