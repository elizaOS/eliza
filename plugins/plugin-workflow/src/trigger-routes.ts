...
import { type TriggerConfig } from "@elizaos/core";
import { type TriggerKind } from "@elizaos/core";
import { type TriggerLastStatus } from "@elizaos/core";
import { type TriggerRunRecord as CoreTriggerRunRecord } from "@elizaos/core";
import { type TriggerType } from "@elizaos/core";
import { type TriggerWakeMode } from "@elizaos/core";
...
export interface TriggerTaskMetadata {
    ...
    [key: string]: string | number | boolean | string[] | number[] | Record<string, string | number | boolean> | undefined | TriggerConfig | CoreTriggerRunRecord[];
}
...
export interface TriggerRouteContext extends RouteRequestContext {
    ...
    executeTriggerTask: (runtime: IAgentRuntime, task: Task, options: TriggerExecutionOptions) => Promise<TriggerExecutionResult>;
...
function parseTriggerKind(value: unknown): TriggerKind | undefined {
    if (value === 'workflow' || value === 'prompt')
        return value;
    return undefined;
}
...
async function findTask(runtime: IAgentRuntime, id: string, listTriggerTasks: (runtime: IAgentRuntime) => Promise<Task[]>, readTriggerConfig: (task: Task) => TriggerConfig | null): Promise<Task | null> {
...
export async function handleTriggerRoutes(ctx: TriggerRouteContext): Promise<boolean> {
    const { method, pathname, req, res, runtime, readJsonBody, json, error, executeTriggerTask, getTriggerHealthSnapshot, getTriggerLimit, listTriggerTasks, readTriggerConfig, readTriggerRuns, taskToTriggerSummary, triggersFeatureEnabled, buildTriggerConfig, buildTriggerMetadata, normalizeTriggerDraft, DISABLED_TRIGGER_INTERVAL_MS, TRIGGER_TASK_NAME, TRIGGER_TASK_TAGS, } = ctx;
    ...
    if (method === 'POST' && pathname === '/api/triggers') {
        ...
        const workflowId = requestedKind === 'workflow' ? parseNonEmptyString(body.workflowId) : undefined;
        const workflowName = requestedKind === 'workflow' ? parseNonEmptyString(body.workflowName) : undefined;
        ...
        const normalized = normalizeTriggerDraft({
            input: inputDraft,
            fallback: {
                displayName: typeof body.displayName === 'string' && trim(body.displayName)
                    ? trim(body.displayName)
                    : 'New Trigger',
                instructions: typeof body.instructions === 'string' ? trim(body.instructions) : '',
                triggerType: typeof body.triggerType === 'string' ? (body.triggerType as TriggerType) : 'interval',
                wakeMode: typeof body.wakeMode === 'string' ? (body.wakeMode as TriggerWakeMode) : 'inject_now',
        ...
        const duplicate = existingTasks.find((task) => {
            ...
            return (existingTrigger?.enabled &&
        ...
        const roomId = (runtime.getService('AUTONOMY') as {
            getAutonomousRoomId?(): UUID;
        } | null)?.getAutonomousRoomId?.();
    ...
    if (method === 'PUT') {
        ...
        const nextWorkflowId = nextKind === 'workflow'
            ? (parseNonEmptyString(body.workflowId) ?? currentWorkflowId)
            : undefined;
        const nextWorkflowName = nextKind === 'workflow'
        ...
        const mergedInput: TriggerDraftInput = {
            ...
            scheduledAtIso: typeof body.scheduledAtIso === 'string' ? body.scheduledAtIso : current.scheduledAtIso,
            cronExpression: typeof body.cronExpression === 'string' ? body.cronExpression : current.cronExpression,
