 ...
 */
function toWorkflowPayload(workflow: WorkflowDefinition): Record<string, unknown> {
  return {
    name: workflow.name,
 ...
 */
function toNodePayload(node: WorkflowNode): Record<string, unknown> {
...
export class WorkflowApiClient {
  ...
  async createWorkflow(workflow: WorkflowDefinition): Promise<WorkflowDefinitionResponse> {
    return this.request<WorkflowDefinitionResponse>('POST', '/workflows', toWorkflowPayload(workflow));
  }
  ...
  async listWorkflows(params?: {
    ...
    return this.request<{ data: WorkflowDefinitionResponse[]; nextCursor?: string }>(
      'GET',
  ...
  async getWorkflow(id: string): Promise<WorkflowDefinitionResponse> {
    return this.request<WorkflowDefinitionResponse>('GET', `/workflows/${id}`);
  }
  ...
  async createCredential(credential: {
    ...
    return this.request<WorkflowCredential>('POST', '/credentials', credential);
  }
  ...
  async listExecutions(params?: {
    ...
    return this.request<{ data: WorkflowExecution[]; nextCursor?: string }>(
      'GET',
  ...
  async getExecution(id: string): Promise<WorkflowExecution> {
    return this.request<WorkflowExecution>('GET', `/executions/${id}`);
  }
  ...
  async listTags(): Promise<{ data: WorkflowTag[] }> {
    return this.request<{ data: WorkflowTag[] }>('GET', '/tags');
  }

  /** @see POST /tags */
  async createTag(name: string): Promise<WorkflowTag> {
    return this.request<WorkflowTag>('POST', '/tags', { name });
  }
  ...
  async getOrCreateTag(name: string): Promise<WorkflowTag> {
    ...
    try {
      ...
      if (error instanceof WorkflowApiError && error.statusCode === 409) {
  ...
  private async request<T = void>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    try {
      ...
      throw new WorkflowApiError(message, response.status, errorData);
    } catch (error) {
