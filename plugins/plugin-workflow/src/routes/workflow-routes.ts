 ...
 *
 * Exposes:
 *   GET    /api/workflow/status                          — mode + sidecar state
 *   POST   /api/workflow/sidecar/start                   — fire-and-forget sidecar boot
 *   GET    /api/workflow/workflows                       — list workflows
 *   POST   /api/workflow/workflows                       — create workflow
 *   POST   /api/workflow/workflows/generate              — generate + create/update workflow
...
export interface WorkflowDefinitionNodeLike {
  id?: string;
  name?: string;
  type?: string;
  typeVersion?: number;
...
export interface WorkflowDefinition {
  ...
  active: boolean;
  description?: string;
  nodes?: WorkflowDefinitionNodeLike[];
  nodeCount: number;
...
type WorkflowDefinitionConnections = NonNullable<WorkflowDefinition['connections']>;

interface WorkflowDefinitionWriteNode {
  id?: string;
  name: string;
  type: string;
  typeVersion: number;
...
interface WorkflowDefinitionWritePayload {
  name: string;
  nodes: WorkflowDefinitionWriteNode[];
  connections: WorkflowDefinitionConnections;
...
function sanitizeNode(n: unknown): WorkflowDefinitionNodeLike {
 ...
 */
function sanitizeNodeFull(n: unknown): WorkflowDefinitionNodeLike {
  if (!n || typeof n !== 'object') {
    return {};
  }
  const obj = n as Record<string, unknown>;
 ...
 */
function normalizeWorkflowFull(raw: unknown): WorkflowDefinition | null {
  if (!raw || typeof raw !== 'object') {
    return null;
  }
  const obj = raw as Record<string, unknown>;
  ...
  const connections =
    obj.connections && typeof obj.connections === 'object'
      ? (obj.connections as WorkflowDefinition['connections'])
      : undefined;
 ...
 * `@elizaos/plugin-workflow` — duplicated here so this route doesn't
...
function normalizeWorkflowWriteNode(value: unknown, index: number): WorkflowDefinitionWriteNode | null {
...
function normalizeWorkflowConnections(value: unknown): WorkflowDefinitionConnections {
  const raw = asRecord(value);
  if (!raw) {
    return {};
  }

  const connections: WorkflowDefinitionConnections = {};
...
function normalizeWorkflowWritePayload(body: Record<string, unknown>): {
  ...
  const nodes = nodesRaw
    ...
    .filter((node): node is WorkflowDefinitionWriteNode => node !== null);
  if (nodes.length === 0) {
...
function propagateError(ctx: WorkflowRouteContext, upstream: { status: number; body: unknown }): void {
...
export async function handleWorkflowRoutes(ctx: WorkflowRouteContext): Promise<boolean> {
  ...
  if (method === 'POST' && pathname === '/api/workflow/sidecar/start') {
    if (native) {
      sendJson(ctx, 409, {
  ...
  if (method === 'POST' && pathname === '/api/workflow/workflows/generate') {
    return handleGenerateWorkflow(ctx);
  }

  if (method === 'POST' && pathname === '/api/workflow/workflows/resolve-clarification') {
...
async function handleStatus(
  ...
  const payload: WorkflowStatusResponse = {
    mode,
    host,
    status,
    cloudConnected,
...
async function handleListWorkflows(
  ...
  const workflows = list.map(normalizeWorkflow).filter((w): w is WorkflowDefinition => w !== null);
...
/**
 * GET /api/workflow/workflows/:id — single-workflow fetch with full graph payload.
 *
...
function getWorkflowService(ctx: WorkflowRouteContext): WorkflowServiceLike | null {
  const service = ctx.runtime?.getService?.('n8n_workflow') as WorkflowServiceLike | undefined;
  if (
...
async function handleGenerateWorkflow(ctx: WorkflowRouteContext): Promise<boolean> {
  const body = await readCompatJsonBody(ctx.req, ctx.res);
  if (!body) {
    return true;
  }
...
async function handleResolveClarification(ctx: WorkflowRouteContext): Promise<boolean> {
  const body = await readCompatJsonBody(ctx.req, ctx.res);
  if (!body) {
    return true;
  }
