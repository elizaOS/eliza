...
import { PostWorkbenchTodoCompleteRequestSchema } from "@elizaos/core/contracts/workbench-routes";
import { PostWorkbenchTodoRequestSchema } from "@elizaos/core/contracts/workbench-routes";
import { PutWorkbenchTodoRequestSchema } from "@elizaos/core/contracts/workbench-routes";
import { WORKBENCH_TODO_TAG } from "../lib/automations-types";
import { isObject } from "../lib/automations-types";
import { isWorkbenchTodoTask } from "../lib/automations-types";
import { logger } from "@elizaos/core";
import { normalizeStringArray } from "../lib/automations-types";
import { readTaskCompleted } from "../lib/automations-types";
import { readTaskMetadata } from "../lib/automations-types";
...
function decodePathComponent(raw: string, res: http.ServerResponse, fieldName: string): string | null {
    try {
...
export function toWorkbenchTodoView(task: Task): WorkbenchTodoView | null {
    ...
    return {
        ...
        description: typeof todoMeta.description === 'string'
...
function getAgentEventEmitter(runtime: AgentRuntime): AgentEventEmitterLike | null {
    ...
    const service = runtimeWithServices.getService?.('agent_event') ??
...
function emitWorkbenchTodoChanged(runtime: AgentRuntime, operation: WorkbenchTodoMutation, todoId: string, todo?: WorkbenchTodoView): void {
    ...
    try {
        ...
        logger.warn({ src: 'plugin:workflow:workbench-todos', error, operation, todoId }, 'Failed to emit workbench todo change event');
    }
}
/**
 ...
 */
export async function handleWorkbenchTodosRoutes(ctx: WorkbenchTodosRouteContext): Promise<boolean> {
