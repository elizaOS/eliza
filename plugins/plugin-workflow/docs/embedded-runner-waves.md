...
- Ownership boundary: all n8n and workflow runtime code lives in `plugins/plugin-workflow`. App-core and agent can keep calling the plugin routes/services they already call, but they should not own n8n execution internals.
- Backend switch: default remains HTTP n8n. Embedded mode is opt-in with one of:
  ...
  - `WORKFLOW_EMBEDDED_ENABLED=true`
  - `WORKFLOW_HOST=embedded://local`
