...
describe('n8n compatibility shim', () => {
  ...
  test('rejects unknown nodes by their original (unshimmed) name', async () => {
    await withDb(async (db) => {
      const runtime = makeRuntime(db);
      const service = await EmbeddedWorkflowService.start(runtime);
      ...
      await expect(service.createWorkflow(workflow)).rejects.toThrow(
        /n8n-nodes-base\.thisDoesNotExist/
      );
