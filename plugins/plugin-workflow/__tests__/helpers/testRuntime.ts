 ...
 *
 * Creates a lightweight runtime with REAL WorkflowService and WorkflowApiClient,
...
/**
 * Setup a real WorkflowService with mock fetch
 *
...
export function createE2ERuntime(config: E2ETestConfig = {}) {
  ...
  const apiClient = new WorkflowApiClient("https://n8n.test", "test-api-key");
  ...
  const runtime: IAgentRuntime = {
    ...
    getService: mock((type: string) => {
      if (type === WORKFLOW_SERVICE_TYPE) return service;
      return null;
    }),
