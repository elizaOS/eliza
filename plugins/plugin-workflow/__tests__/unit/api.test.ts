...
describe("WorkflowApiClient constructor", () => {
  test("sets base URL and strips trailing slash", () => {
    const client = new WorkflowApiClient("https://n8n.example.com/", "key-123");
  ...
  test("sends API key in X-N8N-API-KEY header", async () => {
    const client = new WorkflowApiClient("https://n8n.example.com", "my-secret-key");
    mockFetch = mock(() => Promise.resolve(mockResponse(200, { data: [] })));
    globalThis.fetch = mockFetch as any;
...
describe("WorkflowApiClient workflows", () => {
  const client = new WorkflowApiClient("https://n8n.test", "key-123");
...
describe("WorkflowApiClient credentials", () => {
  const client = new WorkflowApiClient("https://n8n.test", "key-123");
...
describe("WorkflowApiClient executions", () => {
  const client = new WorkflowApiClient("https://n8n.test", "key-123");
...
describe("WorkflowApiClient tags", () => {
  const client = new WorkflowApiClient("https://n8n.test", "key-123");
...
describe("WorkflowApiClient error handling", () => {
  const client = new WorkflowApiClient("https://n8n.test", "key-123");

  test("throws WorkflowApiError on 404", async () => {
    mockFetch = mock(() =>
      Promise.resolve(
    ...
    try {
      ...
      expect((error as WorkflowApiError).message).toContain("Workflow not found");
    }
  });

  test("throws WorkflowApiError on 401", async () => {
    mockFetch = mock(() =>
      Promise.resolve(
    ...
    try {
      ...
      expect(error).toBeInstanceOf(WorkflowApiError);
      expect((error as WorkflowApiError).statusCode).toBe(401);
    }
  });

  test("throws WorkflowApiError on 500", async () => {
    mockFetch = mock(() =>
      Promise.resolve(
        mockResponse(
          500,
    ...
    try {
      ...
      expect(error).toBeInstanceOf(WorkflowApiError);
      expect((error as WorkflowApiError).statusCode).toBe(500);
    }
  });

  test("throws WorkflowApiError on network failure", async () => {
    ...
    try {
      ...
      expect(error).toBeInstanceOf(WorkflowApiError);
      expect((error as WorkflowApiError).message).toContain("ECONNREFUSED");
      expect((error as WorkflowApiError).statusCode).toBeUndefined();
    }
  });
  ...
  test("includes status text in error when no message in body", async () => {
    ...
    try {
      ...
      expect(error).toBeInstanceOf(WorkflowApiError);
      expect((error as WorkflowApiError).message).toContain("Forbidden");
    }
  });
});
...
describe("WorkflowApiClient request format", () => {
  const client = new WorkflowApiClient("https://n8n.test", "key-123");
