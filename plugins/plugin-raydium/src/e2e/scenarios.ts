...
): Promise<{ lpService: RaydiumLpService; owner: Keypair }> {
  assert(
    SECRET_KEY_BASE58,
    "SOLANA_PRIVATE_KEY must be set in .env for e2e tests",
  );
  ...
  tests: [
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        if (pools.length === 0) {
          console.warn(
            "WARNING: No pools returned from Raydium API. This might be a temporary API issue.",
          );
          console.log(
            "Skipping specific pool verification due to empty pool list.",
          );
        ...
        if (!testPool) {
          console.warn(
            `Test pool ${TEST_POOL_ID} not found in ${pools.length} pools. First 5 pool IDs:`,
            pools.slice(0, 5).map((p) => p.id),
          );
        } else {
          assert.equal(testPool.dex, "raydium", "Pool DEX should be raydium");
          console.log(
            "Successfully found and verified the test pool on mainnet.",
          );
        }
      },
    },
    ...
    },
    {
      ...
      fn: async (runtime: IAgentRuntime) => {
        ...
        console.log(
          "Running Add Liquidity to get a position NFT for removal test...",
        );
        ...
        assert.equal(
          addResult.success,
          true,
          `Add liquidity failed, cannot proceed to remove liquidity test: ${addResult.error}`,
        );
        ...
        assert(
          positionNftMint,
          "Position NFT mint should be defined after adding liquidity",
        );
        ...
        assert.equal(
          ...
          true,
          `Remove liquidity failed: ${removeResult.error}`,
        );
        assert(
          removeResult.transactionId,
          "Expected a transaction ID on removal",
        );
      },
    },
  ],
