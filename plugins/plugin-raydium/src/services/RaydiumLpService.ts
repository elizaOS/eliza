...
export class RaydiumLpService extends Service /* implements ILpService */ {
  ...
  public readonly capabilityDescription = "Provides a Raydium implementation of the ILpService for LP management.";
  ...
  private get sdkService(): RaydiumSdkService {
    if (!this._sdkService) {
      this._sdkService = this.runtime.getService(RaydiumSdkService.serviceType) as RaydiumSdkService;
      if (!this._sdkService) {
        throw new Error("RaydiumLpService requires the RaydiumSdkService to be registered.");
      }
    }
  ...
  static async stop(runtime: IAgentRuntime): Promise<void> {
    const service = runtime.getService(RaydiumLpService.serviceType) as RaydiumLpService;
    if (service) {
        await service.stop();
    }
  }
  ...
  public async getPools(tokenAMint?: string, tokenBMint?: string): Promise<PoolInfo[]> {
    try {
      ...
      if (pools.data && Array.isArray(pools.data)) {
        ...
        allPools = [...(pools.clmm || []), ...(pools.standard || []), ...(pools.cpmm || [])];
      }

      logger.info(`Total pools to format: ${allPools.length}`);

      if (allPools.length === 0) {
        logger.warn('No pools returned from API. The Raydium API might be temporarily unavailable.');
      ...
      const formattedPools: PoolInfo[] = allPools.map((p: ApiV3PoolInfoStandardItem | ApiV3PoolInfoConcentratedItem) => ({
          id: p.id,
          dex: 'raydium',
          tokenA: { mint: p.mintA.address, symbol: p.mintA.symbol, decimals: p.mintA.decimals },
          tokenB: { mint: p.mintB.address, symbol: p.mintB.symbol, decimals: p.mintB.decimals },
      ...
      if (tokenAMint && tokenBMint) {
          return formattedPools.filter(p => 
              (p.tokenA.mint === tokenAMint && p.tokenB.mint === tokenBMint) ||
              (p.tokenA.mint === tokenBMint && p.tokenB.mint === tokenAMint)
          );
      }
      if (tokenAMint) {
          return formattedPools.filter(p => p.tokenA.mint === tokenAMint || p.tokenB.mint === tokenAMint);
      }
  ...
  public async addLiquidity(params: {
    ...
    const { poolId, tokenAAmountLamports, tickLowerIndex, tickUpperIndex } = params;
    ...
    let poolInfo = (await raydium.api.fetchPoolById({ ids: poolId }))[0] as ApiV3PoolInfoConcentratedItem;
    ...
    const { tick: lowerTick } = tickLowerIndex !== undefined ? { tick: tickLowerIndex } : TickUtils.getPriceAndTick({
        poolInfo,
        price: new Decimal(poolInfo.price).mul(0.8), // Default to 20% below current price for wider range
        baseIn: true,
    });

    const { tick: upperTick } = tickUpperIndex !== undefined ? { tick: tickUpperIndex } : TickUtils.getPriceAndTick({
        poolInfo,
  ...
  public async removeLiquidity(params: {
    ...
    const positionPubKey = getPdaPersonalPositionAddress(CLMM_PROGRAM_ID, new PublicKey(positionNftMint)).publicKey;
    const positionAccountInfo = await raydium.connection.getAccountInfo(positionPubKey);
    if (!positionAccountInfo) {
        throw new Error(`Position account not found for NFT mint ${positionNftMint}`);
    }
    const position = PositionInfoLayout.decode(positionAccountInfo.data);

    const poolInfo = (await raydium.api.fetchPoolById({ ids: position.poolId.toBase58() }))[0] as ApiV3PoolInfoConcentratedItem;
    if (!poolInfo) throw new Error(`Pool with ID ${position.poolId.toBase58()} not found for the position.`);
  ...
  public async getLpPositionDetails(userAccountPublicKey: string, poolOrPositionIdentifier: string): Promise<LpPositionDetails | null> {
    ...
    try {
        ...
        const position = PositionInfoLayout.decode(positionAccountInfo.data);
        const poolInfo = (await raydium.api.fetchPoolById({ ids: position.poolId.toBase58() }))[0] as ApiV3PoolInfoConcentratedItem;
        const rpcPoolInfo = await raydium.clmm.getRpcClmmPoolInfo({ poolId: position.poolId });
        ...
        const { tick: currentTick } = TickUtils.getPriceAndTick({ poolInfo, price: new Decimal(poolInfo.price), baseIn: true });

        const underlyingTokens: TokenBalance[] = [
            { address: poolInfo.mintA.address, balance: amountA.amount.toString(), decimals: poolInfo.mintA.decimals, symbol: poolInfo.mintA.symbol },
            { address: poolInfo.mintB.address, balance: amountB.amount.toString(), decimals: poolInfo.mintB.decimals, symbol: poolInfo.mintB.symbol },
        ];
        ...
        return {
            poolId: poolInfo.id,
            dex: 'raydium',
            lpTokenBalance: { address: positionNftMint, balance: '1', decimals: 0, symbol: 'Raydium Position NFT' },
            underlyingTokens,
            metadata: {
                tickLower: position.tickLower,
                tickUpper: position.tickUpper,
                liquidity: position.liquidity.toString(),
                inRange: position.tickLower <= currentTick && position.tickUpper >= currentTick,
            }
        };

    } catch (error: any) {
        logger.error(`Failed to get LP position details for ${positionNftMint}:`, error);
        return null;
    }
  }

  public async getMarketDataForPools(poolIds: string[]): Promise<Record<string, Partial<PoolInfo>>> {
    const data = await this.sdkService.sdk.api.fetchPoolById({ ids: poolIds.join(',') });
