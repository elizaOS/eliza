...
describe("RaydiumLpService", () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(mockSdkService, "owner", "get").mockReturnValue(
      Keypair.generate(),
    );
    (mockSdkService as any).isInitialized = true;
    vi.spyOn(mockSdkService, "ensureTokenAccounts").mockResolvedValue(
      undefined,
    );
    ...
    it("should fetch and format pools correctly", async () => {
      const mockApiPools = {
        clmm: [
          {
            id: "clmm1",
            mintA: { address: "a", symbol: "A" },
            mintB: { address: "b", symbol: "B" },
            apr24h: 0.1,
            tvl: 1000,
            programId: "clmm_prog",
          },
        ...
        standard: [
          {
            id: "std1",
            mintA: { address: "c", symbol: "C" },
            mintB: { address: "d", symbol: "D" },
            apr24h: 0.05,
            tvl: 2000,
            programId: "amm_prog",
          },
  ...
  describe("removeLiquidity", () => {
    it("should call decreaseLiquidity and return a transaction result", async () => {
      ...
      (mockRaydiumSdk.api.fetchPoolById as Mock).mockResolvedValue([poolInfo]);
      (mockRaydiumSdk.clmm.decreaseLiquidity as Mock).mockResolvedValue({
        execute: () => Promise.resolve(executeRes),
      });
  ...
  describe("getLpPositionDetails", () => {
    it("should return formatted position details", async () => {
      const position = {
        ...
        nftMint: new PublicKey(1),
        tickLower: 10,
        tickUpper: 100,
        liquidity: new BN(500),
      };
      const poolInfo = {
        id: "pool1",
        price: 1.2,
      ...
      const amounts = {
        amountA: { amount: new BN(100) },
        amountB: { amount: new BN(200) },
      };
      vi.spyOn(PositionInfoLayout, "decode").mockReturnValue(position as any);
      (mockRaydiumSdk.api.fetchPoolById as Mock).mockResolvedValue([poolInfo]);
      vi.spyOn(PositionUtils, "getAmountsFromLiquidity").mockReturnValue(
        amounts as any,
      );
      vi.spyOn(TickUtils, "getPriceAndTick").mockReturnValue({
        tick: 50,
      ...
      const details = await lpService.getLpPositionDetails(
        "userPk",
