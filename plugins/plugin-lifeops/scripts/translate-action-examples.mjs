 ...
 * `plugins/plugin-lifeops/src/lifeops/i18n/prompt-registry.ts`) into a CLI flow
 * that:
 ...
 *
 * Usage:
 *   bun plugins/plugin-lifeops/scripts/translate-action-examples.mjs \
 *       plugins/plugin-lifeops/src/actions/life.ts \
 *       --target-locale=es \
 *       --provider=cerebras \
 *       --max-examples=3 \
 *       --action-name=life \
 *       --output=plugins/plugin-lifeops/src/lifeops/i18n/generated/life.es.ts
 *
 * Environment:
...
function renderRegistryPack({ actionName, locale, translations }) {
  const lines = [];
  lines.push(
    "// AUTOGENERATED by plugins/plugin-lifeops/scripts/translate-action-examples.mjs",
  );
