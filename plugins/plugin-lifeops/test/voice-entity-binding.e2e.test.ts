 ...
 *       `appLifeOpsPlugin.events`) → `VoiceObserver` → real `EntityStore`
 ...
 * `appLifeOpsPlugin.events` registers (asserted by the registration test in
...
describe("voice → entity binding round-trip (issue #8234)", () => {
  ...
  it("registers the binding seam on the local-inference plugin object", () => {
    // (The lifeops side — appLifeOpsPlugin.events[VOICE_TURN_OBSERVED] —
