...
export function getApiKey(runtime: IAgentRuntime): string | undefined {
  return getSetting(runtime, 'NVIDIA_CLOUD_API_KEY') ?? getSetting(runtime, 'NVIDIA_API_KEY');
}
...
function getPositiveIntSetting(runtime: IAgentRuntime, key: string, defaultValue: number): number {
...
export function getEmbeddingInputType(runtime: IAgentRuntime): 'passage' | 'query' | undefined {
  const model = getEmbeddingModel(runtime).toLowerCase();
  const needs =
    model.includes('nv-embed') || model.includes('nv-embedqa') || model.includes('embedqa');
  if (!needs) {
    return undefined;
  }
  const v = getSetting(runtime, 'NVIDIA_EMBEDDING_INPUT_TYPE', 'passage') ?? 'passage';
