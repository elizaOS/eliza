#!/usr/bin/env bun
/**
 * Build script for @elizaos/plugin-s3-storage TypeScript package
 *
 * Produces:
...
const externalDeps = ["@elizaos/core", "@aws-sdk/client-s3", "@aws-sdk/s3-request-presigner"];
...
async function build(): Promise<void> {
  ...
  const nodeResult = await Bun.build({
    ...
    sourcemap: "external",
    minify: false,
    external: [...externalDeps, "node:fs", "node:path"],
  });
