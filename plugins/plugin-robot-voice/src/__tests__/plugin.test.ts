...
import { robotVoicePlugin, SamTTSService } from "../index";
...
describe("RobotVoicePlugin", () => {
  it("has correct metadata", () => {
    expect(robotVoicePlugin.name).toBe("@elizaos/plugin-robot-voice");
    expect(robotVoicePlugin.description).toContain("SAM");
  });

  it("registers SAY_ALOUD action", () => {
    expect(robotVoicePlugin.actions).toHaveLength(1);
    const firstAction = robotVoicePlugin.actions?.[0];
    expect(firstAction?.name).toBe("SAY_ALOUD");
  });

  it("registers SamTTSService", () => {
    expect(robotVoicePlugin.services).toHaveLength(1);
    expect(robotVoicePlugin.services?.[0]).toBe(SamTTSService);
  });
});

describe("SayAloudAction", () => {
  let runtime: IAgentRuntime;
  const action = robotVoicePlugin.actions?.[0];
