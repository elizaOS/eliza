...
function makeFakeSdk(scripts: TurnScript[]): {
  ...
  return { sdk, starts: () => startCount, queryOptions: () => seenOptions };
}
...
describe("ClaudeSdkSession — TEXT mode", () => {
  ...
  it("passes a scoped subprocess env to the SDK query options", async () => {
    const subprocessEnv = { PATH: "/bin", CLAUDE_CODE_OAUTH_TOKEN: "selected-token" };
    const { session, queryOptions } = makeSession([{ text: "hello", subtype: "success" }], {
      subprocessEnv,
    });
    expect(await session.generate("hi")).toBe("hello");
    expect(queryOptions()[0]?.env).toEqual(subprocessEnv);
    await session.dispose();
  });
