 ...
 */
import { buildSetupError } from "@elizaos/core";
import { parseIMessageContactId } from "./contact-path.js";
import { type IAgentRuntime } from "@elizaos/core";
import { type ParsedContactId } from "./contact-path.js";
...
interface IMessageServiceLike {
    handleBlooioWebhook?(rawBody: string, signature: string | undefined): Promise<"accepted" | "ignored" | "unauthorized">;
    ...
    updateContact(personId: string, patch: {
        firstName?: string;
        lastName?: string;
        addPhones?: Array<{
            label?: string;
...
async function handleBlooioWebhook(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    ...
    const result = await service.handleBlooioWebhook(req.rawBody, readHeader(req, "x-blooio-signature"));
...
function isIMessageServiceLike(service: unknown): service is IMessageServiceLike {
    ...
    return (typeof candidate.getRecentMessages === "function" &&
...
function rejectContactId(res: RouteResponse, parsed: ParsedContactId): parsed is {
    ok: true;
    id: string;
} {
    if (parsed.ok)
...
async function handleMessages(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const service = resolveService(runtime);
    if (!service) {
    ...
    try {
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `failed to read messages: ${err instanceof Error ? err.message : String(err)}`));
    }
}
// ── POST /api/imessage/messages ────────────────────────────────────
async function handleSendMessage(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const service = resolveService(runtime);
    if (!service) {
        res.status(503).json(buildSetupError("service_unavailable", "imessage service not registered"));
        return;
    ...
    try {
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `sendMessage threw: ${err instanceof Error ? err.message : String(err)}`));
    }
}
// ── GET /api/imessage/chats ─────────────────────────────────────────
async function handleChats(_req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const service = resolveService(runtime);
    if (!service) {
        res.status(503).json(buildSetupError("service_unavailable", "imessage service not registered"));
        return;
    ...
    try {
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `failed to read chats: ${err instanceof Error ? err.message : String(err)}`));
    }
}
// ── GET /api/imessage/contacts ──────────────────────────────────────
async function handleListContacts(_req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const service = resolveService(runtime);
    if (!service) {
        res.status(503).json(buildSetupError("service_unavailable", "imessage service not registered"));
        return;
    ...
    try {
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `failed to read contacts: ${err instanceof Error ? err.message : String(err)}`));
    }
}
// ── POST /api/imessage/contacts ─────────────────────────────────────
async function handleCreateContact(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const service = resolveService(runtime);
    if (!service) {
        res.status(503).json(buildSetupError("service_unavailable", "imessage service not registered"));
        return;
    ...
    if (!body.firstName && !body.lastName && !body.phones?.length && !body.emails?.length) {
        res
            .status(400)
            .json(buildSetupError("bad_request", "at least one of firstName, lastName, phones, or emails is required"));
        return;
    }
    try {
        ...
        if (!id) {
            res
                .status(500)
                .json(buildSetupError("internal_error", "contact creation failed — see server logs. Common cause: Contacts write permission not granted yet."));
            return;
        }
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `addContact threw: ${err instanceof Error ? err.message : String(err)}`));
    }
}
// ── PATCH /api/imessage/contacts/:id ────────────────────────────────
async function handleUpdateContact(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    ...
    try {
        ...
        if (!ok) {
            res
                .status(500)
                .json(buildSetupError("internal_error", "contact update failed — see server logs. Contact may not exist, or write permission may be denied."));
            return;
        }
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `updateContact threw: ${err instanceof Error ? err.message : String(err)}`));
    }
}
// ── DELETE /api/imessage/contacts/:id ───────────────────────────────
async function handleDeleteContact(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
    const pathname = req.url ?? "";
    const parsed = parseIMessageContactId(pathname.split("?")[0]);
    if (!rejectContactId(res, parsed)) {
        return;
    ...
    try {
        ...
        if (!ok) {
            res
                .status(500)
                .json(buildSetupError("internal_error", "contact delete failed — see server logs. Contact may not exist, or write permission may be denied."));
            return;
        }
        ...
        res
            .status(500)
            .json(buildSetupError("internal_error", `deleteContact threw: ${err instanceof Error ? err.message : String(err)}`));
    }
}
export const imessageDataRoutes: Route[] = [
    {
        ...
        publicWrite: "Inbound Blooio webhook POST authenticated by the X-Blooio-Signature header, not the local gate.",
    },
    {
        type: "GET",
