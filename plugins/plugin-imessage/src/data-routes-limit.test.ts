...
const messagesRoute = imessageDataRoutes.find((route) => route.type === "GET" && route.path === "/api/imessage/messages");
...
describe("GET /api/imessage/messages limit query", () => {
    ...
    it.each(["1e2", "12px", "007", "0", "abc", "-1", "50abc", " 10", "10 ", " "])("rejects prefix-coerced limit=%s with 400 before the service", async (limit) => {
        const queries: MessageQuery[] = [];
        const captured = await getMessages(`/api/imessage/messages?limit=${encodeURIComponent(limit)}`, queries);
