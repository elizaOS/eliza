...
const patchRoute = imessageDataRoutes.find((route) => route.type === "PATCH" && route.path === "/api/imessage/contacts/:id");
const deleteRoute = imessageDataRoutes.find((route) => route.type === "DELETE" && route.path === "/api/imessage/contacts/:id");
const listRoute = imessageDataRoutes.find((route) => route.type === "GET" && route.path === "/api/imessage/contacts");
...
async function patchContact(url: string, calls: ServiceCalls): Promise<Captured> {
    ...
    await patchRoute.handler?.({ url, method: "PATCH", body: { firstName: "Ada" } } as RouteRequest, mockRes(captured), mockRuntime(calls));
    return captured;
}
async function deleteContact(url: string, calls: ServiceCalls): Promise<Captured> {
    const captured: Captured = {};
    await deleteRoute.handler?.({ url, method: "DELETE" } as RouteRequest, mockRes(captured), mockRuntime(calls));
    return captured;
}
async function legacyContactRoute(method: "PATCH" | "DELETE", url: string, calls: ServiceCalls): Promise<Captured> {
    const captured: Captured = {};
    ...
    await handleIMessageRoute({ method, url } as http.IncomingMessage, res, url, method, { runtime: mockRuntime(calls) }, helpers);
    return captured;
}
describe("PATCH /api/imessage/contacts/:id encoding", () => {
    ...
    it("GET contacts list is untouched", async () => {
        const calls: ServiceCalls = { updates: [], deletes: [], lists: 0 };
        const captured: Captured = {};
        await listRoute.handler?.({ url: "/api/imessage/contacts", method: "GET" } as RouteRequest, mockRes(captured), mockRuntime(calls));
        expect(captured.status).toBe(200);
