...
async function request(url: string, observedLimits: number[]) {
    ...
    const handled = await handleIMessageRoute({ url } as http.IncomingMessage, {} as http.ServerResponse, "/api/imessage/messages", "GET", state, helpers);
...
describe("GET /api/imessage/messages limit", () => {
    ...
    it.each(["1e2", "12px", "abc", "50abc", "0x10"])("rejects malformed limit %s before reading messages", async (limit) => {
        const observedLimits: number[] = [];
        const helpers = await request(`/api/imessage/messages?limit=${encodeURIComponent(limit)}`, observedLimits);
        expect(helpers.error).toHaveBeenCalledWith(expect.anything(), "limit must be an integer", 400);
