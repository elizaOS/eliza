 ...
 */
export async function handleIMessageRoute(req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, state: IMessageRouteState, helpers: RouteHelpers): Promise<boolean> {
    ...
    if (method === "PATCH" && pathname.startsWith("/api/imessage/contacts/")) {
        ...
        if (!parsedId.ok) {
            helpers.error(res, parsedId.reason === "malformed"
                ? "Invalid contact id: malformed URL encoding"
                : "contact id is required in the path", 400);
            return true;
        }
