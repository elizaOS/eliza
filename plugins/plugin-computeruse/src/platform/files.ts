...
const CREDENTIAL_PATTERNS: Array<{ pattern: RegExp; label: string }> = [
  ...
  {
    pattern: /^\/\.config\/gcloud\/credentials\.db$/i,
    label: "GCP credentials",
  },
  {
    pattern: /^\/\.config\/gcloud\/application_default_credentials\.json$/i,
    label: "GCP default credentials",
  },
  ...
  {
    pattern:
      /\/(?:Google\/Chrome|Microsoft\\?\/Edge|BraveSoftware\/Brave-Browser)\/.*\/(?:Login Data|Cookies)$/i,
    label: "browser credential store",
  },
  {
    pattern:
  ...
  {
    pattern: /\/AppData\/(?:Roaming|Local)\/Microsoft\/Credentials\//i,
    label: "Windows Credential Store",
  },
  {
    pattern: /\/AppData\/Local\/Microsoft\/Vault\//i,
...
const SYSTEM_DIR_PATTERNS_WIN32: Array<{ pattern: RegExp; label: string }> = [
  ...
  { pattern: /^[A-Z]:\/Recovery\//i, label: "Recovery directory" },
  {
    ...
    label: "System Volume Information",
  },
...
const SYSTEM_DIR_PATTERNS_UNIX: Array<{ pattern: RegExp; label: string }> = [
  ...
  {
    pattern: /^\/etc\/(?:shadow|sudoers|pam\.d|master\.passwd)/,
...
function validateFilePath(
  ...
  if (resolved.startsWith("//")) {
    return {
      allowed: false,
      reason:
        "Network (UNC) paths are blocked. The agent can only access local files.",
    };
  }
...
export async function readFile(params: {
  ...
  try {
    const content = await read(params.path, {
      encoding: (params.encoding || "utf-8") as BufferEncoding,
    });
    return {
      success: true,
      path: params.path,
...
export async function appendFile(params: {
  path: string;
  content: string;
}): Promise<Record<string, unknown>> {
  const check = validateFilePath(params.path, "write");
