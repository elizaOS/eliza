...
import { classifyPermissionDeniedError, isPermissionDeniedError } from "./permissions.js";
...
export function desktopClick(x: number, y: number): void {
  runWithAccessibilityPermission("desktop_click", () => {
...
export function desktopClickWithModifiers(
  ...
  runWithAccessibilityPermission("desktop_click_with_modifiers", () => {
    const sx = validateInt(x);
    const sy = validateInt(y);
    const os = currentPlatform();
...
`;
    ...
    if (os === "linux") {
      ...
      const keys = normalizedModifiers.map((modifier) => modMap[modifier] ?? modifier);
      runCommand(
        "xdotool",
        [
    ...
    if (os === "win32") {
      ...
      const keyDown = normalizedModifiers
        ...
        .map((code) => `[Win32.Win32Keyboard]::keybd_event(${code}, 0, 0, 0)`);
      const keyUp = [...normalizedModifiers]
        .reverse()
        .map((modifier) => vkMap[modifier])
        .filter((code): code is number => Number.isFinite(code))
        .map((code) => `[Win32.Win32Keyboard]::keybd_event(${code}, 0, 2, 0)`);
      const ps = [
        "Add-Type -AssemblyName System.Windows.Forms",
        `[System.Windows.Forms.Cursor]::Position = New-Object System.Drawing.Point(${sx},${sy})`,
...
export function desktopDoubleClick(x: number, y: number): void {
  runWithAccessibilityPermission("desktop_double_click", () => {
    const sx = validateInt(x);
    const sy = validateInt(y);
    const os = currentPlatform();
...
export function desktopRightClick(x: number, y: number): void {
  runWithAccessibilityPermission("desktop_right_click", () => {
    const sx = validateInt(x);
    const sy = validateInt(y);
    const os = currentPlatform();
...
export function desktopMouseMove(x: number, y: number): void {
  runWithAccessibilityPermission("desktop_mouse_move", () => {
    const sx = validateInt(x);
    const sy = validateInt(y);
    const os = currentPlatform();
...
export function desktopDrag(
  ...
  runWithAccessibilityPermission("desktop_drag", () => {
...
export function desktopScroll(
  ...
  runWithAccessibilityPermission("desktop_scroll", () => {
    const sx = validateInt(x);
    const sy = validateInt(y);
...
export function desktopType(text: string): void {
  runWithAccessibilityPermission("desktop_type", () => {
...
export function desktopKeyPress(key: string): void {
  runWithAccessibilityPermission("desktop_key_press", () => {
...
export function desktopKeyCombo(combo: string): void {
  runWithAccessibilityPermission("desktop_key_combo", () => {
...
function normalizeModifiers(modifiers: string[]): string[] {
  ...
  for (const modifier of normalized) {
    if (!["cmd", "command", "meta", "super", "ctrl", "control", "alt", "option", "shift"].includes(modifier)) {
