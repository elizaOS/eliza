...
|---|---|---|---|---|---|
| screenshot / capture / displays | verified (`capture.ts` / desktop driver path) | code-parity (`screencapture`; multi-monitor checklist still manual) | code-parity (PowerShell `System.Drawing`; local Windows evidence exists) | code-parity (ReplayKit foreground; broadcast extension flaky on iOS 26 beta) | code-parity (MediaProjection, requires user consent) |
...
| clipboard | verified (`wl-clipboard` / `xclip` / `xsel`) | code-parity (`pbpaste` / `pbcopy`) | code-parity (PowerShell `Get-Clipboard` / `Set-Clipboard`) | unavailable (no mobile clipboard bridge method yet) | unavailable (no mobile clipboard bridge method yet) |
| accessibility tree (a11y) | code-parity (`LinuxAccessibilityProvider`) | code-parity (`DarwinAccessibilityProvider`, TCC-gated) | code-parity (`WindowsAccessibilityProvider`, UIAutomation) | code-parity (`accessibilitySnapshot` returns own-app tree only) | code-parity (AccessibilityService `getRootInActiveWindow`) |
