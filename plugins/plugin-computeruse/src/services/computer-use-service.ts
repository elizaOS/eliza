 ...
 * browser control, screenshots, and human approval state.
 *
...
export class ComputerUseService extends Service {
  ...
  static async start(runtime: IAgentRuntime): Promise<Service> {
    ...
    try {
      ...
      logger.warn(
        `[computeruse] Could not detect screen size, using defaults: ${String(err)}`,
      );
    }
    ...
    if (!instance.capabilities.screenshot.available) {
      logger.warn(
        `[computeruse] Screenshot not available: ${instance.capabilities.screenshot.tool}`,
      );
    }
    if (!instance.capabilities.computerUse.available) {
      logger.warn(
        `[computeruse] Mouse/keyboard not available: ${instance.capabilities.computerUse.tool}`,
      );
    }
    ...
    try {
      ...
      switch (params.action) {
        ...
        case "key_combo":
          if (!params.key) {
            throw new Error(
              "key is required for key_combo action (e.g. 'ctrl+c')",
            );
          }
      ...
      if (
        ...
        try {
          ...
          logger.warn(
            `[computeruse] Screenshot capture failed after action: ${String(err)}`,
          );
        }
      }
    ...
    try {
      ...
      switch (params.action) {
        ...
        case "scroll":
          ...
          result = {
            success: true,
            content: `Scrolled ${params.direction ?? "down"}.`,
          };
          break;
        case "screenshot":
          ...
          break;
        case "dom":
          result = { success: true, content: await getBrowserDom() };
          break;
        case "clickables": {
        ...
        case "execute":
          ...
          result = {
            success: true,
            content: await executeBrowser(params.code),
          };
          break;
        case "state": {
        ...
        case "close_tab":
          ...
          result = {
            success: true,
            content: `Closed tab ${params.tabId}.`,
          };
          break;
        case "switch_tab":
          if (!params.tabId) {
            throw new Error("tabId is required for switch_tab");
          }
          result = {
            success: true,
            content: `Switched to tab: ${(await switchBrowserTab(params.tabId)).url}`,
          };
          break;
        default:
  ...
  async executeWindowAction(
    ...
    try {
      ...
      switch (params.action) {
        ...
        case "focus":
          if (!params.windowId) {
            throw new Error("windowId is required for focus");
          }
          focusWindow(params.windowId);
          break;
        case "minimize":
          if (!params.windowId) {
            throw new Error("windowId is required for minimize");
          }
          minimizeWindow(params.windowId);
          break;
        case "maximize":
          if (!params.windowId) {
            throw new Error("windowId is required for maximize");
          }
          maximizeWindow(params.windowId);
          break;
        case "close":
          if (!params.windowId) {
            throw new Error("windowId is required for close");
          }
