...
import { sendJson as sendJsonResponse } from "@elizaos/core/api/http-helpers";
...
export async function handleSandboxRoute(req: IncomingMessage, res: ServerResponse, pathname: string, method: string, state: SandboxRouteState): Promise<boolean> {
    ...
    if (os === "darwin") {
        ...
        tell application "System Events"
          ...
          return windowList as text
            ...
            logger.warn(`[sandbox-routes] macOS window enumeration failed: ${err instanceof Error ? err.message : String(err)}`);
            return [];
        }
    }
    if (os === "linux") {
        try {
            ...
            logger.warn(`[sandbox-routes] X11 window enumeration failed: ${err instanceof Error ? err.message : String(err)}`);
            return [];
        }
    }
    if (os === "win32") {
        try {
            ...
            logger.warn(`[sandbox-routes] PowerShell window enumeration failed: ${err instanceof Error ? err.message : String(err)}`);
            return [];
        }
    }
    return [];
...
function detectCapabilities(): Record<string, {
    available: boolean;
    tool: string;
}> {
    const os = platform();
...
function readJsonBody<T = unknown>(req: IncomingMessage, res: ServerResponse): Promise<T | null> {
