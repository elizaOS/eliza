...
- Android `android-cloud-debug` builds, installs on the emulator and a physical
  Pixel 6a, and passes the local chat smoke launcher. Direct `VIEW` intents for
  `elizaos://assistant`, `elizaos://lifeops/task/new`, and
  `elizaos://feature/open` resolve to `app.eliza/.MainActivity` on both.
...
- On the physical Pixel 6a, Android reports
  `com.google.android.googlequicksearchbox` as the active
  `android.app.role.ASSISTANT` holder. Forcing `app.eliza` into that role fails
  because the Play/Pixel cloud APK intentionally does not qualify as an Android
  voice-interaction assistant service.
...
- Android consumer App Actions still require a spoken or Assistant/Gemini UI
  fulfillment test; the physical Pixel now confirms static shortcuts and direct
  deep-link dispatch, but not Google Assistant natural-language matching.
