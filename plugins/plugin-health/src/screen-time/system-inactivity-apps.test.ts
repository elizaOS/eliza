...
describe("isSystemInactivityApp", () => {
  it("detects macOS lock and screen saver identities", () => {
    expect(
      isSystemInactivityApp({ bundleId: "com.apple.loginwindow" }),
    ).toBe(true);
  ...
  it("detects Windows and Linux lock screen executables", () => {
    expect(isSystemInactivityApp({ executableName: "LockApp.exe" })).toBe(
      true,
    );
    expect(isSystemInactivityApp({ executableName: "kscreenlocker_greet" })).toBe(
      true,
    );
  });
