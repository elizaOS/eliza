...
export const minecraftConnectAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      return { text: content.text ?? "", success: true, data: { botId: session.botId }, values: { connected: true } };
    } catch (err) {
      ...
      await callback?.(content);
      return { text: content.text ?? "", success: false, data: { error: msg }, values: { connected: false } };
    }
  },
};
