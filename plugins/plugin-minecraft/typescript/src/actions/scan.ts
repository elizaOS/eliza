...
function parseScan(text: string): { blocks?: string[]; radius?: number; maxResults?: number } {
...
export const minecraftScanAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      return { text: content.text ?? "", success: true, data, values: { count: blocks.length } };
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      const content: Content = { text: `Scan failed: ${msg}`, actions: ["MC_SCAN"], source: message.content.source };
      await callback?.(content);
