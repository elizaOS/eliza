...
export const minecraftAttackAction: Action = {
  ...
  handler: async (
    ...
    try {
      ...
      const content: Content = { text: `Attacked entity ${entityId}.`, actions: ["MC_ATTACK"], source: message.content.source };
      await callback?.(content);
      return { text: content.text ?? "", success: true };
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      const content: Content = { text: `Failed to attack: ${msg}`, actions: ["MC_ATTACK"], source: message.content.source };
      await callback?.(content);
