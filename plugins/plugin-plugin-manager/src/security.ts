 ...
 * create a layer cycle — `@elizaos/agent` already depends on this plugin).
 *
 ...
 * without monkey-patching the `@elizaos/core` module (bun's `mock.module`
 * persists across test files in the same run, which would contaminate
 * unrelated suites).
 */
...
export async function hasOwnerAccess(
  ...
  const resolveOwnerFn =
    deps.resolveCanonicalOwnerIdForMessage ?? defaultResolveCanonicalOwnerIdForMessage;
