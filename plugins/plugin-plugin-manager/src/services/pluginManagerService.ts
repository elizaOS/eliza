...
export class PluginManagerService extends Service implements PluginRegistry {
  ...
  async installPlugin(
    ...
    return this.serialiseInstall(async () => {
      ...
      // Try cloning if requested or if npm install fails
      if (shouldClone) {
        try {
          // Redirect install to ejected path if cloning
          ...
          // Symlink or just return success with ejected path?
          // If we clone, it's effectively an ejected plugin.
          // But existing logic might expect it in 'installed'.
          // For simplify, we can skip symlinking and rely on the loader checking 'ejected' dirs too,
          // or we can symlink 'installed' -> 'ejected'.
          // Let's symlink for compatibility.
          await fs.remove(targetDir); // remove the empty dir we just created
      ...
      if (!installed) {
        try {
          ...
          if (!shouldClone) {
            // If we didn't already try cloning
            try {
  ...
  async searchRegistry(query: string, limit?: number): Promise<PluginSearchResult[]> {
    const { searchPluginsByContent } = await import("./pluginRegistryService");
  ...
  async syncPlugin(pluginId: string): Promise<SyncResult> {
    return this.serialiseEject(async () => {
      // Need to find ejected path first.
      // Simplified: scan ejected dir for folder matching query
      const base = this.getEjectedBaseDir();
