...
import { resolveStateDir } from "../utils/paths";
import { getRegistryEntry } from "./pluginRegistryService";
...
export class CoreManagerService extends Service {
  ...
  override capabilityDescription = "Manages the core ElizaOS installation (eject, sync, reinject)";
  ...
  private async readCorePackageVersion(packageDir = this.corePackageDir()): Promise<string> {
    try {
  ...
  private async resolveInstalledCoreVersion(): Promise<string> {
    ...
    try {
      // Try to find local package.json for @elizaos/core
      // Since this plugin runs in the agent, @elizaos/core should be obtainable via require reference or node_modules
  ...
  private async readUpstreamMetadata(): Promise<UpstreamMetadata | null> {
    try {
      ...
      return {
        $schema: "milaidy-upstream-v1",
        source: typeof parsed.source === "string" ? parsed.source : "github:elizaos/eliza",
        gitUrl: parsed.gitUrl,
        branch: parsed.branch,
        commitHash: parsed.commitHash,
        ejectedAt:
          typeof parsed.ejectedAt === "string" ? parsed.ejectedAt : new Date().toISOString(),
  ...
  private async writeUpstreamMetadata(metadata: UpstreamMetadata): Promise<void> {
  ...
  private async writeTsconfigCorePaths(targetDistPath: string | null): Promise<void> {
    ...
    if (!targetDistPath) {
      // Reset to default
      // Note: This logic assumes specific defaults for the user's workspace.
      // It might be safer to remove the paths if we are reinjecting, or restore from a backup.
      // The original code reset to internal paths, but that might not be correct if we are just using npm.
      // If we reinject (go back to npm), we should typically remove the path override.
  ...
  async ejectCore(): Promise<CoreEjectResult> {
    return this.serialise(async () => {
      ...
      try {
        logger.info(`Installing dependencies and building ${CORE_PACKAGE_NAME}...`);
  ...
  async syncCore(): Promise<CoreSyncResult> {
    return this.serialise(async () => {
      ...
      const localChanges =
        (await this.gitStdout(["status", "--porcelain"], monorepoDir).catch(() => "")).length > 0;
      ...
      const commitHash = await this.gitStdout(["rev-parse", "HEAD"], monorepoDir);
      // const localCommits = ... (omitted for brevity, can implement if needed)
  ...
  async getCoreStatus(): Promise<CoreStatus> {
    ...
    const commitHash = await this.gitStdout(["rev-parse", "HEAD"], monorepoDir).catch(() => null);
    const localChanges =
      (await this.gitStdout(["status", "--porcelain"], monorepoDir).catch(() => "")).length > 0;
