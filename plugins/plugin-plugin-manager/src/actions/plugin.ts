/**
 * @module plugin-plugin-manager/actions/plugin
 *
...
import { runCoreStatus } from "./plugin-handlers/core-status";
import {
  hasPendingPluginCreateIntent,
  isPluginCreateChoiceReply,
  runCreate,
} from "./plugin-handlers/create";
import { runEject } from "./plugin-handlers/eject";
import { runInstall } from "./plugin-handlers/install";
import { runList } from "./plugin-handlers/list";
import { runListEjected } from "./plugin-handlers/list-ejected";
import { runReinject } from "./plugin-handlers/reinject";
import { runSearch } from "./plugin-handlers/search";
import { runSync } from "./plugin-handlers/sync";
...
function readNestedParameters(options: ActionOptions | undefined): ActionOptions | undefined {
...
function readOptionValue(options: ActionOptions | undefined, key: string): unknown {
...
function readStringOption(options: ActionOptions | undefined, key: string): string | undefined {
...
function readSourceOption(options: ActionOptions | undefined): "npm" | "git" | undefined {
  const source = readStringOption(options, "source");
  if (source === "npm" || source === "git") return source;
  return undefined;
}

function inferMode(text: string, options?: Record<string, unknown>): PluginMode | null {
  ...
  if (LIST_VERBS.test(trimmed) && EJECTED_NOUN.test(trimmed)) return "list_ejected";
...
export function createPluginAction(deps: PluginActionDeps = {}): Action {
  ...
  const canManagePlugins = async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
