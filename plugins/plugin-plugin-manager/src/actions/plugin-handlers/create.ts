...
async function resolveMinPluginTemplateDir(repoRoot: string): Promise<string | undefined> {
...
async function createNewPlugin({
  ...
  const templateSrc = await resolveMinPluginTemplateDir(repoRoot);
  if (!templateSrc) {
    const tried = MIN_PLUGIN_TEMPLATE_CANDIDATES.map((rel) => path.join(repoRoot, rel)).join(", ");
    const text = `min-plugin template not found (tried: ${tried}); cannot scaffold a new plugin.`;
    await callback?.({ text });
