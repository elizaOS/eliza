/**
 * @module plugin-plugin-manager/actions/plugin-handlers/search
 *
...
export async function runSearch({ runtime, query, callback }: SearchInput): Promise<ActionResult> {
  ...
  const lines: string[] = [`Found ${results.length} plugin(s) matching "${query}":`, ""];
  results.forEach((plugin, idx) => {
    const score = plugin.score ? ` (match: ${(plugin.score * 100).toFixed(0)}%)` : "";
