/**
 * @module plugin-plugin-manager/actions/plugin-handlers/core-status
 *
...
import type { CoreManagerService, CoreStatus } from "../../services/coreManagerService";
...
export async function runCoreStatus({ runtime, callback }: CoreStatusInput): Promise<ActionResult> {
  const service = runtime.getService("core_manager") as CoreManagerService | null;
  if (!service) {
