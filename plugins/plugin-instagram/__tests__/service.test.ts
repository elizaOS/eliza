...
describe("InstagramService", () => {
  ...
  describe("start/stop", () => {
    it("should throw when starting without initialization", async () => {
      await expect(service.startService()).rejects.toThrow("Instagram service not initialized");
    });
  });
  ...
  describe("operations without running", () => {
    ...
    it("should throw when liking media without running", async () => {
      await expect(service.likeMedia(12345)).rejects.toThrow("Instagram service is not running");
    });

    it("should throw when following user without running", async () => {
      await expect(service.followUser(12345)).rejects.toThrow("Instagram service is not running");
    });
  });
});
...
describe("splitMessage", () => {
  ...
  it("should split multiline messages correctly", () => {
    const lines = Array.from({ length: 100 }, (_, i) => `Line ${i}: Some content`);
