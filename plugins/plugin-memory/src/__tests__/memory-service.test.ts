...
describe('MemoryService', () => {
  ...
  describe('initialization', () => {
    it('should initialize with default config', async () => {
      ...
      expect(config.shortTermSummarizationThreshold).toBe(16);
      // Note: shortTermRetainRecent may vary based on environment variables
      expect(config.shortTermRetainRecent).toBeGreaterThan(0);
  ...
  describe('configuration', () => {
    it('should update configuration', async () => {
      ...
      expect(config.longTermExtractionEnabled).toBe(false);
      // shortTermRetainRecent should remain at its initialized value (not updated)
      expect(config.shortTermRetainRecent).toBeGreaterThan(0);
    });
  });
  ...
  describe('long-term memory storage', () => {
    ...
    it('should store long-term memory', async () => {
      ...
      expect(memory.category).toBe(LongTermMemoryCategory.SEMANTIC);
    ...
    it('should retrieve long-term memories', async () => {
      ...
      expect(memories[0].category).toBe(LongTermMemoryCategory.SEMANTIC);
    });
