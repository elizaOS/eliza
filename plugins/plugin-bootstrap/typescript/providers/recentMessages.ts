...
export const recentMessagesProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory) => {
    try {
      ...
      const isPostFormat = (room && room.type)
          ...
          .map(([runId, memories]) => {
            ...
            const runText = sortedMemories
              .map((mem: Memory) => {
                ...
                const error = (memContent && memContent.error) || "";
      ...
      const foundEntity = entitiesData.find((entity: Entity) => entity.id === message.entityId);
      const senderName =
        (foundEntity && foundEntity.names && foundEntity.names[0]) ||
        (metaData && metaData.entityName) ||
        "Unknown User";
      const receivedMessageContent = message.content.text;

      const hasReceivedMessage = !!(receivedMessageContent && receivedMessageContent.trim());
      ...
      const getRecentMessageInteractions = async (
        ...
        const formattedInteractions = recentInteractionsData.map((message) => {
          ...
          if (isSelf) {
            ...
            const interactionMetadata = interactionEntity && interactionEntity.metadata;
            sender =
              (interactionMetadata && interactionMetadata.userName as string) || "unknown";
          }
