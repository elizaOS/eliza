...
import { type RealTestRuntimeResult } from "../../../packages/app/test/helpers/real-runtime.ts";
import { type RouteHandlerContext } from "@elizaos/shared";
import { type RouteHandlerResult } from "@elizaos/shared";
import { type TriageEntry } from "../src/inbox/types.ts";
...
function inbound(overrides: Partial<InboundMessage> & {
    id: string;
    text: string;
}): InboundMessage {
    return {
...
describe("inbox routes e2e — real plugin on real PGLite runtime", () => {
    ...
    async function call(method: "GET" | "POST", path: string, opts: {
        body?: unknown;
        ...
        const route = getHttpRuntime(runtime).routes.find((candidate) => {
            if (candidate.type !== method)
                return false;
            const pattern = new RegExp(`^${candidate.path.replace(/:[^/]+/g, "[^/]+")}$`);
            return pattern.test(path);
        });
        expect(route?.routeHandler, `route ${method} ${path} must be registered on the runtime`).toBeDefined();
        ...
        if (!result)
            throw new Error(`route ${method} ${path} returned nothing`);
        return result;
    }
    beforeAll(async () => {
        ...
        runtime.registerModel(ModelType.TEXT_SMALL as ModelTypeName, async (_rt, params) => deterministicTriageModel(String((params as {
            prompt?: string;
        }).prompt)), "inbox-routes-e2e", 100);
    ...
    it("standalone plugin-inbox registers INBOX plus the promoted INBOX_* virtuals and both providers", () => {
        ...
        const providerNames = new Set(runtime.providers.map((provider) => provider.name));
    ...
    it("POST triage classifies through the real service and persists real app_inbox rows", async () => {
        ...
        const bySource = new Map(unresolved.map((entry) => [entry.sourceMessageId, entry]));
        expect(bySource.get("route-msg-urgent")?.classification).toBe("urgent");
        expect(bySource.get("route-msg-urgent")?.suggestedResponse).toBe("On it — will handle right away.");
        expect(bySource.get("route-msg-question")?.classification).toBe("needs_reply");
        ...
        expect(unresolved[unresolved.length - 1]?.sourceMessageId).toBe("route-msg-newsletter");
    });
    it("GET triage filters by classification and respects limit", async () => {
        ...
        const urgentEntries = (urgent.body as {
            entries: TriageEntry[];
        }).entries;
        ...
        expect((bogus.body as {
            entries: TriageEntry[];
        }).entries.length).toBeGreaterThanOrEqual(2);
    });
    it("POST :id/snooze hides the entry until the timestamp; includeSnoozed reveals it", async () => {
        const unresolved = await repo.getUnresolved({ limit: 50 });
        const target = unresolved.find((entry) => entry.sourceMessageId === "route-msg-question");
        if (!target)
            throw new Error("unreachable");
        const until = new Date(Date.now() + 60 * 60 * 1000).toISOString();
        const snooze = await call("POST", `/api/lifeops/inbox/${target.id}/snooze`, { params: { id: target.id }, body: { snoozedUntil: until } });
        ...
        const defaultIds = (defaultRead.body as {
            entries: TriageEntry[];
        ...
        const snoozedIds = (withSnoozed.body as {
            entries: TriageEntry[];
        }).entries.map((entry) => entry.id);
    ...
    it("snooze validates its inputs: unknown entry and bad timestamp both fail loudly", async () => {
        const missing = await call("POST", "/api/lifeops/inbox/00000000-0000-0000-0000-000000000000/snooze", {
            params: { id: "00000000-0000-0000-0000-000000000000" },
            body: { snoozedUntil: new Date(Date.now() + 1000).toISOString() },
        });
        expect(missing.status).toBe(404);
        expect(String((missing.body as {
            error: string;
        }).error)).toContain("was not found");
        const unresolved = await repo.getUnresolved({ limit: 1 });
        const anyEntry = unresolved[0];
        if (!anyEntry)
            throw new Error("unreachable");
        const badTs = await call("POST", `/api/lifeops/inbox/${anyEntry.id}/snooze`, { params: { id: anyEntry.id }, body: { snoozedUntil: "not-a-date" } });
        expect(badTs.status).toBe(400);
        expect(String((badTs.body as {
            error: string;
        }).error)).toContain("snooze timestamp");
    });
    it("POST :id/approve fails cleanly when the entry has no draft or suggested response", async () => {
        ...
        const noDraft = unresolved.find((entry) => !entry.draftResponse && !entry.suggestedResponse);
        if (!noDraft)
            throw new Error("unreachable");
        const approve = await call("POST", `/api/lifeops/inbox/${noDraft.id}/approve`, { params: { id: noDraft.id }, body: {} });
        expect(approve.status).toBe(400);
        expect(String((approve.body as {
            error: string;
        }).error)).toContain("no draft or suggested response");
    });
    it("POST :id/reply requires a body and a real entry", async () => {
        const noEntry = await call("POST", "/api/lifeops/inbox/11111111-1111-1111-1111-111111111111/reply", {
        ...
        expect(String((noBody.body as {
            error: string;
        }).error)).toContain("reply body is required");
    });
    it("re-triaging the same source message ids dedupes instead of duplicating rows", async () => {
        ...
        expect(after.filter((entry) => entry.sourceMessageId === "route-msg-urgent")).toHaveLength(1);
