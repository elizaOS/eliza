...
import { executeInboxQueueOperation } from "../actions/inbox.ts";
import { type IAgentRuntime } from "@elizaos/core";
import { type InboundMessage } from "../inbox/types.ts";
import { type InboxQueueOperationResult } from "../actions/inbox.ts";
...
import { type TriageClassification } from "../inbox/types.ts";
...
function routeParams(ctx: RouteHandlerContext, operation?: InboxRouteOperation): Record<string, unknown> {
...
function classifyInboxError(error: unknown): {
    ...
    const message = error instanceof Error ? error.message : "Inbox operation failed.";
...
async function runOperation(runtime: IAgentRuntime, operation: InboxRouteOperation, params: Record<string, unknown>): Promise<RouteHandlerResult> {
...
async function handleTriageRead(ctx: RouteHandlerContext): Promise<RouteHandlerResult> {
...
async function handleTriageWrite(ctx: RouteHandlerContext): Promise<RouteHandlerResult> {
    const body = bodyRecord(ctx.body);
...
async function inboxRouteHandler(ctx: RouteHandlerContext): Promise<RouteHandlerResult> {
