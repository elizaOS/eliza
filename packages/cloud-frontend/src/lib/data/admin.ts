...
export function useAdminOrgs() {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<{ orgs: AdminOrgRow[]; total: number }>("/api/v1/admin/orgs").then(
        (r) => r.orgs,
      ),
...
export function useAdminUsers() {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<{ users: AdminUserRow[]; total: number }>("/api/v1/admin/users").then(
        (r) => r.users,
      ),
    enabled: gate.enabled,
  });
...
export function useAdminAiPricing() {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<{ pricing: AdminAiPricingRow[] }>("/api/v1/admin/ai-pricing").then(
        (r) => r.pricing,
      ),
    enabled: gate.enabled,
  });
...
export function useAdminServicePricing(serviceId: string | undefined) {
  ...
  return useQuery({
    queryKey: authenticatedQueryKey(
      ["admin", "service-pricing", serviceId ?? null],
      gate,
    ),
    queryFn: () =>
...
export function useAdminInfrastructure() {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<AdminInfrastructureSummary>("/api/v1/admin/infrastructure"),
    enabled: gate.enabled,
  });
}
...
export function useAdminInfrastructureContainers() {
  ...
  return useQuery({
    queryKey: authenticatedQueryKey(
      ["admin", "infrastructure", "containers"],
      gate,
    ),
    queryFn: () =>
      api<{ containers?: AdminDockerContainer[] }>(
...
export function useAdminDockerNodes() {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<{ nodes?: AdminDockerNode[] }>("/api/v1/admin/docker-nodes").then(
        (r) => r.nodes ?? [],
      ),
    enabled: gate.enabled,
  });
...
export function useAdminHeadscale() {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<{ nodes?: AdminHeadscaleNode[] }>("/api/v1/admin/headscale").then(
        (r) => r.nodes ?? [],
      ),
    enabled: gate.enabled,
  });
