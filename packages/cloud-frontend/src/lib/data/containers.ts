...
export function useContainerDetail(
  ...
  const gate = useAuthenticatedQueryGate(Boolean(id));
  const includeKey =
    include.length === 0 ? null : [...include].sort().join(",");
  return useQuery({
    queryKey: authenticatedQueryKey(["container-detail", id, includeKey], gate),
    queryFn: async () => {
      const qs = includeKey ? `?include=${includeKey}` : "";
      const res = await api<ApiEnvelope<ContainerDetailData>>(
        `/api/v1/containers/${id}${qs}`,
      );
      return res.data;
    },
    enabled: gate.enabled,
...
export function useContainerDeployments(id: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<ApiEnvelope<unknown>>(`/api/v1/containers/${id}/deployments`),
    enabled: gate.enabled,
  });
}
...
export function useContainerMetrics(id: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<ApiEnvelope<unknown>>(`/api/v1/containers/${id}/metrics?period=60`),
    enabled: gate.enabled,
  });
}
...
export function useContainerCredentials() {
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<ApiEnvelope<unknown>>("/api/v1/containers/credentials", {
        method: "POST",
      }),
    enabled: gate.enabled,
  });
