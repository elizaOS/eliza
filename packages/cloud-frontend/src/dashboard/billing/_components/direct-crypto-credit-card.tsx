...
export function DirectCryptoCreditCard({
  ...
  const cardClassName = isCloudSurface
    ? "border-black/10 bg-white text-black shadow-none sm:border-white/12 sm:bg-black/78 sm:text-white sm:shadow-2xl sm:backdrop-blur-xl"
    : "border-border bg-card text-card-fg";
  const mutedTextClassName = isCloudSurface
    ? "text-black/60 sm:text-white/60"
    : "text-muted";
  const titleClassName = isCloudSurface
    ? "text-black sm:text-white"
    : "text-txt-strong";
  const dividerClassName = isCloudSurface
    ? "border-t border-black/10 sm:border-white/10"
    : "border-t border-border/60";
  const iconBoxClassName = isCloudSurface
    ? "border-black/10 bg-black text-white sm:border-white/12 sm:bg-white/[0.06] sm:text-[#FF5800]"
    : "border-accent/20 bg-accent-subtle text-accent";
  const segmentClassName = isCloudSurface
    ? "border-black/10 bg-black/[0.03] sm:border-white/10 sm:bg-white/[0.04]"
    : "border-border bg-bg-muted";
  const selectedSegmentClassName = isCloudSurface
    ? "bg-black text-white sm:bg-[#FF5800] sm:text-black"
    : "bg-accent text-accent-foreground";
  const unselectedSegmentClassName = isCloudSurface
    ? "text-black/58 hover:bg-black/[0.06] hover:text-black sm:text-white/58 sm:hover:bg-white/[0.06] sm:hover:text-white"
    : "text-muted-foreground hover:bg-bg-hover hover:text-txt";
  const infoTileClassName = isCloudSurface
    ? "border-black/10 bg-black/[0.03] sm:border-white/10 sm:bg-white/[0.05]"
    : "border-border bg-bg-muted";
  const infoValueClassName = isCloudSurface
    ? "text-black sm:text-white"
    : "text-txt-strong";
  const promoClassName = isCloudSurface
    ? "border-black bg-black text-white sm:border-[#FF5800]/30 sm:bg-[#FF5800]/12 sm:text-[#FFB087]"
    : "border-warn/25 bg-warn-subtle text-warn";
  const surfaceButtonClassName = isCloudSurface
    ? "border-black bg-black text-white hover:bg-black/82 sm:border-white/14 sm:bg-white/[0.06] sm:hover:bg-white/10"
    : undefined;
  const payButtonClassName = isCloudSurface
    ? "min-w-[172px] bg-black text-white hover:bg-black/82 sm:bg-[#FF5800] sm:text-black sm:hover:bg-white"
    : "min-w-[172px]";
