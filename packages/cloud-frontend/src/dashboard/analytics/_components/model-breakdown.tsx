      ...
      <Card className="border-border/70 bg-background/60 shadow-sm">
        <CardHeader className="p-6 pb-5">
          <CardTitle className="text-base font-semibold">
            {t("cloud.analytics.modelBreakdown.title", { defaultValue: "Model breakdown" })}
          </CardTitle>
          <p className="text-sm text-muted-foreground">
            {t("cloud.analytics.modelBreakdown.noData", { defaultValue: "No model data available for the selected period." })}
          </p>
        </CardHeader>
      </Card>
    );
      ...
      <CardHeader className="flex flex-col gap-3 p-6 pb-5">
        <div className="flex items-center gap-3">
          ...
          <Badge variant="outline" className="rounded-full text-xs">
            {t("cloud.analytics.modelBreakdown.count", { defaultValue: "{{n}} model{{plural}}", n: models.length, plural: models.length !== 1 ? "s" : "" })}
          </Badge>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("cloud.analytics.modelBreakdown.subtitle", { defaultValue: "Detailed usage statistics and cost analysis per model with efficiency metrics." })}
        </p>
      </CardHeader>
              ...
              <thead>
                <tr className="border-b border-border/50">
                  <th className="pb-3 text-left font-medium text-muted-foreground">
                    {t("cloud.analytics.modelBreakdown.col.model", { defaultValue: "Model" })}
                  </th>
                  <th className="pb-3 text-left font-medium text-muted-foreground">
                    {t("cloud.analytics.modelBreakdown.col.provider", { defaultValue: "Provider" })}
                  </th>
                  <th className="pb-3 text-right font-medium text-muted-foreground">
                    {t("cloud.analytics.modelBreakdown.col.requests", { defaultValue: "Requests" })}
                  </th>
                  <th className="pb-3 text-right font-medium text-muted-foreground">
                    {t("cloud.analytics.modelBreakdown.col.cost", { defaultValue: "Cost" })}
                  </th>
                  <th className="pb-3 text-right font-medium text-muted-foreground">
                    {t("cloud.analytics.modelBreakdown.col.tokens", { defaultValue: "Tokens" })}
                  </th>
                  <th className="pb-3 text-right font-medium text-muted-foreground">
                    {t("cloud.analytics.modelBreakdown.col.success", { defaultValue: "Success" })}
                  </th>
                </tr>
              </thead>
              <tbody>
              ...
              >
                {expanded ? (
                  <>
                    ...
                    {t("cloud.analytics.modelBreakdown.showLess", { defaultValue: "Show less" })}
                  </>
                ) : (
                  <>
                    <ChevronDown className="h-4 w-4" />
                    {t("cloud.analytics.modelBreakdown.showMore", { defaultValue: "Show {{n}} more", n: models.length - displayLimit })}
                  </>
                )}
              </Button>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 rounded-sm border border-border/60 bg-muted/30 p-4">
            <div className="space-y-1">
              <p className="text-xs font-medium text-muted-foreground">
                {t("cloud.analytics.modelBreakdown.totalRequests", { defaultValue: "Total requests" })}
              </p>
            ...
            <div className="space-y-1">
              <p className="text-xs font-medium text-muted-foreground">
                {t("cloud.analytics.modelBreakdown.totalCost", { defaultValue: "Total cost" })}
              </p>
              <p className="text-lg font-semibold text-foreground">
