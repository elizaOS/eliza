...
/// Pattern ported from openclaw-classic iOS VoiceWakeManager.
...
/// (e.g. "cloud" for trigger "clawd") still fires.
private enum WakeGate {
    ...
    /// Supports fuzzy matching so "cloud" matches trigger "clawd".
    ...
    /// Threshold: ceil(maxLen / 3). e.g. "clawd" (5) ↔ "cloud" (5) → threshold 2, distance 2 → match.
 ...
 * Speech framework. Ported from openclaw-classic VoiceWakeManager (iOS) and
 * VoiceWakeRuntime (macOS).
 *
...
public class SwabblePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    struct PluginConfig {
        ...
        init(from obj: JSObject) {
            self.triggers = (obj["triggers"] as? [String]) ?? ["clawd", "claude"]
