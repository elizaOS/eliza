...
import { emojiStrip } from "../emoji-strip.ts";

let DkgClient: any = null;

function stripEmojisFromAllStrings(value: unknown): unknown {
	if (typeof value === "string") {
		return emojiStrip(value);
	}
	if (Array.isArray(value)) {
		return value.map((item) => stripEmojisFromAllStrings(item));
	}
	if (value && typeof value === "object") {
		const result: Record<string, unknown> = {};
		for (const [key, val] of Object.entries(value as Record<string, unknown>)) {
			result[key] = stripEmojisFromAllStrings(val);
		}
		return result;
	}
	return value;
...
export const dkgInsert: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            const sanitizedMemoryKnowledgeGraph = stripEmojisFromAllStrings(
                memoryKnowledgeGraph,
            ) as any;

            elizaLogger.log(
                `KA: ${JSON.stringify(sanitizedMemoryKnowledgeGraph, null, 2)}`,
            );

            createAssetResult = await DkgClient.asset.create(
                {
                    public: sanitizedMemoryKnowledgeGraph,
                },
                {
                    epochsNum: 12,
            ...
            if (
                ...
                try {
                    ...
                    const sanitizedData = stripEmojisFromAllStrings(data);
                    createAssetResult = await DkgClient.asset.create(
                        {
