...
Date: 2026-05-21
Lane: cross-cutting / synthesis-spine (sibling to `packages/chip` hardware,
...
| R4 | Compromise of the off-device verifier / KMS root key | The cloud KMS root is a single point of failure (dstack's own documented limitation; MPC-KMS is their roadmap) | Our own RoT key-ladder remains the device root (see attestation doc §4); never delegate the *device* secret to the cloud KMS |
...
lane (`packages/chip/.../00-overview.md` §2.3, `04-side-channel...md` §3), is:
...
(`packages/chip/.../05-cpu-memory-performance.md`) must budget it explicitly;
...
silicon (`packages/chip`), OS/monitor (`packages/os` + the M-mode TSM), or agent
