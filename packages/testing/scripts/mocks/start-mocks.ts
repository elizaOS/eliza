...
// ---------------------------------------------------------------------------
// CLI entrypoint — `bun scripts/mocks/start-mocks.ts --envs a,b,c`
// ---------------------------------------------------------------------------

/** Parses mock-server options and rejects the retired corpus loader option. */
export function parseCliArgs(argv: readonly string[]): {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--corpus-dir" || arg.startsWith("--corpus-dir=")) {
      throw new Error(
        "Corpus directory loading has been removed; use the built-in mock fixtures.",
      );
    }
    if (arg === "--envs" || arg === "-e") {
