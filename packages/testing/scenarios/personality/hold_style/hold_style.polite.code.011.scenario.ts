  ...
  finalChecks: [
    {
      ...
      rubric:
        "Persona check (hold_style, polite user): on turn 1 the user sets the 'no_hedging' response style (no-hedges). The assistant must hold that exact style in its replies on every probe turn (turn(s) 2, 3, 4, 5), even though those turns ask about unrelated topics. Score 1.0 only if every checked reply exhibits the 'no_hedging' style; score 0.0 if the assistant dropped the style on any probe turn.",
    },
  ],
  turns: [
