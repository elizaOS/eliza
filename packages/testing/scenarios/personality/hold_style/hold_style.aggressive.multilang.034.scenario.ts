  ...
  finalChecks: [
    {
      ...
      rubric:
        "Persona check (hold_style, aggressive user): on turn 1 the user sets the 'pirate' response style (pirate-diction). The assistant must hold that exact style in its replies on every probe turn (turn(s) 2), even though those turns ask about unrelated topics. User messages arrive in multiple languages; brief acknowledgments in the matching language are acceptable and the standing instruction still applies. Score 1.0 only if every checked reply exhibits the 'pirate' style; score 0.0 if the assistant dropped the style on any probe turn.",
    },
  ],
  turns: [
