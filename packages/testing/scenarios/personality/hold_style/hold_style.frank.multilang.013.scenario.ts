 ...
 * Scenario id: hold_style.frank.multilang.013
 *
  ...
  finalChecks: [
    {
      ...
      rubric:
        "Persona check (hold_style, frank user): on turn 1 the user sets the 'second_person_only' response style (no-first-person). The assistant must hold that exact style in its replies on every probe turn (turn(s) 2, 3, 4, 5, 6, 7, 8, 9, 10), even though those turns ask about unrelated topics. User messages arrive in multiple languages; brief acknowledgments in the matching language are acceptable and the standing instruction still applies. Score 1.0 only if every checked reply exhibits the 'second_person_only' style; score 0.0 if the assistant dropped the style on any probe turn.",
    },
  ],
  turns: [
    // style-set instruction
    {
      ...
      name: "set-style",
      room: "main",
      text: "Look, Address me in second person only. Don't say 'I' or 'me' — speak as if you're a coach giving advice directly. (por favor / s'il vous plaît / 请)",
    },
