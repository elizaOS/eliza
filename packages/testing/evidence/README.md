...
certification contract live in this README and the package `AGENTS.md`.
...
`{}`; `toJSON` is not honored — callers pre-serialize. See `evidence/canonical.ts`.
...
`packages/testing/scenario-runner/reports/` have no live writer and are not part of
...
```bash
bun run --cwd packages/testing bundle:create -- --tier cpu [--out evidence/runs] [--repo-root <dir>]
bun run --cwd packages/testing bundle:snapshot -- --repo-root <dir> --out <snapshot.json>
bun run --cwd packages/testing bundle:create -- --tier cpu --baseline <snapshot.json>
...
```bash
bun run --cwd packages/testing test:evidence        # vitest, real tmp-dir filesystem
