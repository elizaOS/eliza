...
- **`evidence/schema.ts` is a frozen contract.** `ArtifactEntry`, `BundleManifest`,
...
- **Manifest bytes are signed.** `finalize()` must stay byte-stable: artifacts
  ...
  newline — `evidence/canonical.ts`; non-plain objects throw, `toJSON` is not
...
- **One producer inventory.** Ingestors (`evidence/ingest.ts`) name every canonical
...
- **Fail fast, typed.** Throw `EvidenceError`/`EvidenceValidationError`
  ...
  these entrypoints must run without loading runtime fixtures in CI/vast containers). No `?? <default>` for
...
`evidence/certify/` turns a finalized bundle into a signed, offline-verifiable
...
```bash
bun run --cwd packages/testing test:evidence         # vitest suite
bun run --cwd packages/testing typecheck:evidence    # tsc --noEmit
bun run --cwd packages/testing lint         # biome
bun run --cwd packages/testing bundle:create -- --tier cpu
bun run --cwd packages/testing bundle:snapshot -- --repo-root <dir> --out <snapshot.json>
bun run --cwd packages/testing bundle:verify -- evidence/runs/<run-id>
bun run evidence:review:no-open -- --bundle=evidence/runs/<run-id>
bun run --cwd packages/testing certify:keygen -- [--print-private-key]
bun run --cwd packages/testing certify:rollup -- --bundle <dir> [--requirements <file>] [--out <file>]
bun run --cwd packages/testing certify:sign -- --bundle <dir> --verdicts <file> --reviewer-id <id> --reviewer-kind <agent|human>
bun run --cwd packages/testing certify:verify -- --cert <file> --bundle <dir> --pubkey <pem> [--requirements <file>] [--json]
...
bun run --cwd packages/testing video:walkthrough -- --def all --bundle <dir>
bun run --cwd packages/testing video:ingest -- --file x.webm --granularity feature --slug send-message --bundle <dir>
bun run --cwd packages/testing vision-qa -- --help
bun run --cwd packages/testing evidence:gpu-queue -- --help
```
...
```
evidence/schema.ts       frozen schema-1 types + zod validation (J3 typed invalid)
evidence/canonical.ts    canonical JSON bytes (signed form)
evidence/bundle.ts       EvidenceBundle builder + verifyBundle integrity report
evidence/provenance.ts   git facts (fail loud), runner kind, env fingerprint allowlist
evidence/ingest.ts       silo definitions + ingestAllSilos / ingestNamedSilo
evidence/cli.ts          thin argv/formatting layer over the lib (J1 boundary)
evidence/errors.ts       EvidenceError / EvidenceValidationError
evidence/analyzers/     ARIA, brand, color, diff, OCR, pHash, and keyframe analysis
evidence/certify/
...
evidence/ffmpeg-binaries.ts  ffmpeg/ffprobe resolver: env → PATH → installed static packages
evidence/queue/          durable file-backed GPU analysis queue and result merge
evidence/vision-qa/      backend selection, caching, batched visual questions, QA records
evidence/video/          video evidence lanes (#14545, stacked on the #14542 analyzers)
