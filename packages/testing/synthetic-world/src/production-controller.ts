...
import { SYNTHETIC_WORLD_CAPABILITIES } from "./types";
...
export interface SyntheticProductionRuntimeModule {
    ...
    shutdownRuntime(runtime: SyntheticProductionRuntime, context: string, options: {
        fast: true;
    }): Promise<void>;
}
function isProductionRuntimeModule(value: unknown): value is SyntheticProductionRuntimeModule {
    if (typeof value !== "object" || value === null)
        return false;
    const candidate = value as Record<string, unknown>;
    return (typeof candidate.startEliza === "function" &&
...
export type ProductionSyntheticWorldFailureStage = "input" | "claim" | "initialization" | "proof" | "teardown";
...
function controllerError(code: string, message: string, cause?: unknown): ElizaError {
...
function validateInput(input: ProductionSyntheticWorldBootInput): void {
    if (input.runtimeName.trim().length === 0 ||
        input.runtimeName !== input.runtimeName.trim() ||
        !path.isAbsolute(input.pgliteDataDir) ||
        input.pgliteDataDir.trim() !== input.pgliteDataDir) {
        throw controllerError("SYNTHETIC_CONTROLLER_INVALID_INPUT", "Runtime name must be trimmed and PGlite data directory must be an absolute path");
    }
}
...
class OwnedProductionSyntheticWorldController implements ProductionSyntheticWorldController {
    private stopPromise: Promise<void> | null = null;
    constructor(runtime: SyntheticProductionRuntime, readonly proof: ProductionSyntheticWorldRuntimeProof, stopRuntime: (runtime: SyntheticProductionRuntime) => Promise<void>) {
    ...
    async stop(): Promise<void> {
        this.stopPromise ??= this.stopRuntime().catch((error: unknown) => {
            ...
            throw controllerError("SYNTHETIC_CONTROLLER_TEARDOWN_FAILED", "Production synthetic controller teardown failed", error);
        });
 ...
 */
export async function bootProductionSyntheticWorldController(input: ProductionSyntheticWorldBootInput): Promise<ProductionSyntheticWorldBootResult> {
    return bootWithProductionRuntime(input, async () => {
        ...
        if (!isProductionRuntimeModule(productionRuntime)) {
            throw controllerError("SYNTHETIC_CONTROLLER_RUNTIME_MODULE_INVALID", "The production agent runtime does not expose its boot and teardown contract");
        }
        return productionRuntime;
    });
}
/** Internal adversarial seam; not exported from `@elizaos/testing/synthetic-world`. */
export async function bootProductionSyntheticWorldControllerWithModule(input: ProductionSyntheticWorldBootInput, productionRuntime: SyntheticProductionRuntimeModule): Promise<ProductionSyntheticWorldBootResult> {
    return bootWithProductionRuntime(input, async () => productionRuntime);
}
async function bootWithProductionRuntime(input: ProductionSyntheticWorldBootInput, loadProductionRuntime: () => Promise<SyntheticProductionRuntimeModule>): Promise<ProductionSyntheticWorldBootResult> {
    let runtime: SyntheticProductionRuntime | undefined;
    let stopProductionRuntime: ((runtime: SyntheticProductionRuntime, context: string) => Promise<void>) | undefined;
    let stage: ProductionSyntheticWorldFailureStage = "input";
    try {
        validateInput(input);
        stage = "claim";
        const claim = await input.journal.execute(input.authority, {
        ...
        `);
            const inserted = database.run(`INSERT INTO synthetic_world_controller_boot_claims
           ...
           VALUES (?, ?, ?, ?, ?)`, [
            ...
            if (inserted.changes !== 1) {
                throw controllerError("SYNTHETIC_CONTROLLER_CLAIM_NOT_DURABLE", "Controller boot claim did not insert exactly one row");
            }
            return {
                generation: input.authority.generation,
                runtimeName: input.runtimeName,
        ...
        if (claim.replayed) {
            throw controllerError("SYNTHETIC_CONTROLLER_BOOT_ALREADY_CLAIMED", "The durable boot claim was already consumed; a new command ID is required");
        }
        stage = "initialization";
        const productionRuntime = await loadProductionRuntime();
        const { shutdownRuntime, startEliza } = productionRuntime;
        const productionStop = (ownedRuntime: SyntheticProductionRuntime, context: string) => shutdownRuntime(ownedRuntime, context, { fast: true });
        ...
        if (!bootedRuntime) {
            if (runtime) {
                throw controllerError("SYNTHETIC_CONTROLLER_INITIALIZATION_INCOMPLETE", "Production boot created a runtime but resolved without returning it");
            }
            return {
                status: "unavailable",
                failure: {
        ...
        if (adapter !== "PgliteDatabaseAdapter" ||
            agentEntity?.id !== runtime.agentId ||
            runtimeName !== input.runtimeName ||
            runtimePluginNames.some((name) => name.length === 0 || name !== name.trim())) {
            throw controllerError("SYNTHETIC_CONTROLLER_REPOSITORY_PROOF_FAILED", "Production PGlite adapter and agent entity readback did not match");
        }
        ...
        return {
            status: "available",
            controller: new OwnedProductionSyntheticWorldController(runtime, proof, (ownedRuntime) => productionStop(ownedRuntime, "synthetic production controller")),
            proof,
            capabilities: SYNTHETIC_WORLD_CAPABILITIES,
        };
    }
        ...
        if (stage === "claim" &&
            ...
            failure = controllerError("SYNTHETIC_CONTROLLER_BOOT_ALREADY_CLAIMED", "The durable boot claim is already owned by another execution", error);
        }
        if (runtime && stopProductionRuntime) {
            try {
                await stopProductionRuntime(runtime, "failed synthetic production boot");
            }
            catch (teardownError) {
                // error-policy:J2 Preserve teardown failure on the explicit failed result.
                failure = controllerError("SYNTHETIC_CONTROLLER_BOOT_TEARDOWN_FAILED", "Production boot failed and its partial runtime could not be stopped", { bootError: error, teardownError });
                stage = "teardown";
            }
        }
        ...
        return {
            ...
            stage,
            failure: {
                code: failure instanceof ElizaError
                    ? failure.code
