...
class SqliteSyntheticCommandJournalRepository implements SyntheticCommandJournalRepository<Database> {
    ...
    find(database: Database, identity: SyntheticCommandJournalIdentity): SyntheticCommandJournalRow | null {
        const row = database
            .query<SqliteCommandRow, [
            string,
            string
    ...
    insert(database: Database, row: SyntheticCommandJournalRow): number {
        return database.run(`INSERT INTO synthetic_world_commands (
    ...
    compareAndSet(database: Database, identity: SyntheticCommandJournalIdentity, expected: SyntheticCommandJournalExpected, patch: SyntheticCommandJournalPatch): number {
        const assignments: string[] = [];
        const values: Array<string | number | null> = [];
        const columns: Array<[
            keyof SyntheticCommandJournalPatch,
            string
        ]> = [
        ...
        values.push(identity.namespace, identity.commandId, expected.generation, expected.phase, expected.executionToken, expected.executionToken, expected.revision);
        return database.run(`UPDATE synthetic_world_commands SET ${assignments.join(", ")}
       WHERE namespace = ? AND command_id = ? AND generation = ? AND phase = ?
         AND ((execution_token = ?) OR (execution_token IS NULL AND ? IS NULL))
         AND revision = ?`, values).changes;
    }
}
