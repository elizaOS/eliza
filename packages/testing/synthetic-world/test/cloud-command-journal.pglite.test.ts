...
import { LeaseFencedSyntheticCommandJournal } from "../src";
import { SYNTHETIC_WORLD_COMMAND_VERSION } from "../src";
...
import { type SyntheticCommandJournalRepository } from "../src";
...
const repository: SyntheticCommandJournalRepository<DbTransaction> = new CloudSyntheticCommandJournalRepository();
const journal = new LeaseFencedSyntheticCommandJournal(leaseStore, repository);
beforeAll(async () => {
    const { apply } = await pushSchema({ agentTable, syntheticEnvironmentLeases, syntheticWorldCommands } as never, dbWrite as never);
    await apply();
}, 60000);
beforeEach(async () => {
...
async function acquire(namespace: string, leaseDurationMs = 5000): Promise<SyntheticEnvironmentLeaseAuthority> {
    return (await leaseStore.acquire({
        namespace,
        owner: {
...
function command(authority: SyntheticEnvironmentLeaseAuthority, commandId: string, payload: null | Record<string, string> = null) {
    return {
...
describe("Cloud synthetic command journal on PGlite", () => {
    test("commits production agent readback with COMMITTED and replays without a second callback", async () => {
        ...
        const first = await journal.execute(authority, command(authority, "ensure-agent", { agentId }), async (tx) => {
            callbackCount += 1;
            await agentsRepository.ensure({ id: agentId, name: "Synthetic Cloud Agent" }, tx);
            ...
            async onCheckpoint(checkpoint) {
                ...
                const [record] = await dbWrite
                    ...
                    .where(and(eq(syntheticWorldCommands.namespace, authority.namespace), eq(syntheticWorldCommands.command_id, "ensure-agent")));
                if (agent && record)
        ...
        const restartedJournal = new LeaseFencedSyntheticCommandJournal(new CloudSyntheticEnvironmentLeaseStore(), new CloudSyntheticCommandJournalRepository());
        const replay = await restartedJournal.execute(authority, command(authority, "ensure-agent", { agentId }), async () => {
            callbackCount += 1;
        ...
        await expect(journal.execute(authority, command(authority, "ensure-agent", { agentId: "changed" }), async () => null)).rejects.toMatchObject({ code: "SYNTHETIC_COMMAND_ID_CONFLICT" });
    });
    test("serializes a colliding caller and invokes the production mutation once", async () => {
        ...
        const first = journal.execute(authority, command(authority, "colliding-agent", { agentId }), async (tx) => {
            callbackCount += 1;
            mutationEntered.resolve();
            await mutationReleased.promise;
            await agentsRepository.ensure({ id: agentId, name: "Colliding Agent" }, tx);
            return { agentId };
        });
        await mutationEntered.promise;
        const second = journal.execute(authority, command(authority, "colliding-agent", { agentId }), async () => {
            callbackCount += 1;
            return { agentId };
        });
        await Bun.sleep(20);
        mutationReleased.resolve();
        const collisionOutcome = await second.then((execution) => ({ replayed: execution.replayed }), (error: unknown) => ({
    ...
    test("rolls back the production row and COMMITTED transition when mutation fails", async () => {
        ...
        await expect(journal.execute(authority, command(authority, "rollback-agent"), async (tx) => {
            await agentsRepository.ensure({ id: agentId, name: "Must Roll Back" }, tx);
    ...
    test("rejects stale authority and rolls back an expiry during mutation", async () => {
        ...
        await expect(journal.execute(stale, command(stale, "stale-command"), async () => null)).rejects.toMatchObject({ code: "SYNTHETIC_LEASE_LOST" });
        ...
        await expect(journal.execute(expiring, command(expiring, "expired-agent"), async (tx) => {
            await agentsRepository.ensure({ id: agentId, name: "Expired Agent" }, tx);
            ...
            const [lease] = await tx
                ...
                .where(eq(syntheticEnvironmentLeases.namespace, expiring.namespace));
            if (!lease?.expiresAt)
            ...
            while (true) {
                const remaining = lease.expiresAt.getTime() - (await repository.now(tx));
                if (remaining <= 0)
                    break;
        ...
        expect(await agentsRepository.findById(agentId)).toBeNull();
        expect(await dbWrite
            .select({
            phase: syntheticWorldCommands.phase,
            result: syntheticWorldCommands.result_json,
        })
            .from(syntheticWorldCommands)
            .where(eq(syntheticWorldCommands.command_id, "expired-agent"))).toEqual([{ phase: "EXECUTING", result: null }]);
    }, 15000);
    test("does not steal same-generation active execution during recovery", async () => {
        ...
        await journal.execute(authority, command(authority, "active-command"), async () => null, {
            async onCheckpoint(checkpoint) {
                if (checkpoint.phase !== "EXECUTING")
                    return;
    ...
    test("classifies commit-before-response as DIRTY after generation recovery", async () => {
        ...
        await expect(journal.execute(authority, command(authority, "ambiguous-agent"), async (tx) => {
            await agentsRepository.ensure({ id: agentId, name: "Ambiguous Agent" }, tx);
            return { agentId };
        }, {
        ...
        expect(await journal.inspect(next.authority, "ambiguous-agent")).toMatchObject({
            phase: "DIRTY",
            outcome: "UNKNOWN",
        });
    });
    test("fails closed on corrupt JSON and a future-generation row", async () => {
        ...
        await journal.execute(authority, command(authority, "corrupt-command"), async () => ({
            ok: "yes",
        }));
        await dbWrite
        ...
        await expect(journal.inspect(authority, "corrupt-command")).rejects.toMatchObject({
            code: "SYNTHETIC_COMMAND_STORAGE_FAILURE",
        });
        await journal.execute(authority, command(authority, "future-command"), async () => null);
        await dbWrite
            .update(syntheticWorldCommands)
            .set({ generation: authority.generation + 1 })
            .where(eq(syntheticWorldCommands.command_id, "future-command"));
        await expect(journal.execute(authority, command(authority, "future-command"), async () => null)).rejects.toMatchObject({ code: "SYNTHETIC_COMMAND_STORAGE_FAILURE" });
    });
    test("inspect and recover fail closed on a terminal row from a future generation", async () => {
        ...
        await journal.execute(authority, command(authority, "future-terminal-command"), async () => ({ ok: "yes" }));
        await dbWrite
            .update(syntheticWorldCommands)
            .set({ generation: authority.generation + 1 })
            .where(eq(syntheticWorldCommands.command_id, "future-terminal-command"));
        await expect(journal.inspect(authority, "future-terminal-command")).rejects.toMatchObject({ code: "SYNTHETIC_COMMAND_STORAGE_FAILURE" });
