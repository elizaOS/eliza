...
import { SqliteSyntheticCommandJournal } from "../../src";
...
const authority = JSON.parse(authorityJson) as SyntheticEnvironmentLeaseAuthority;
const store = new SqliteSyntheticEnvironmentLeaseStore(databasePath);
const journal = new SqliteSyntheticCommandJournal(store);
try {
    const execution = await journal.execute(authority, {
        ...
        database.run("CREATE TABLE IF NOT EXISTS synthetic_test_writes (command_id TEXT PRIMARY KEY)");
        database.run("INSERT INTO synthetic_test_writes (command_id) VALUES (?)", [commandId]);
        if (crashAt === "MUTATION")
            process.exit(86);
        return { written: commandId };
    }, {
        onCheckpoint(checkpoint) {
            if (checkpoint.phase === crashAt)
                process.exit(86);
        },
    });
