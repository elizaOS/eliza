...
function loadConfiguredCloudApiKey(): string {
    ...
    const configuredPath = resolveAliasedEnvValue("ELIZA_CONFIG_PATH")?.trim() ||
...
export function selectLiveProvider(preferredProvider?: LiveProviderName): LiveProviderConfig | null {
    ...
    if (cloudApiKey && (!preferredProvider || preferredProvider === "openai")) {
        ...
        const largeModel = process.env.OPENAI_LARGE_MODEL?.trim() ||
 ...
 */
export function requireLiveProvider(preferredProvider?: LiveProviderName): LiveProviderConfig {
...
export function availableProviderNames(): LiveProviderName[] {
    const providers = new Set<LiveProviderName>(PROVIDERS.filter((def) => def.keyEnvVars.some((k) => process.env[k]?.trim())).map((def) => def.name));
    if (process.env.ELIZAOS_CLOUD_API_KEY?.trim() ||
        process.env.ELIZA_CLOUD_API_KEY?.trim() ||
        getConfiguredCloudApiKey()) {
