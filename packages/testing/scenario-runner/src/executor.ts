...
import { DEFAULT_SCENARIO_EXECUTION_PROFILE } from "@elizaos/testing";
import { ElizaError } from "@elizaos/core";
import { JudgeModelObserver } from "./judge-model-observer.ts";
import { JudgeParseError } from "./judge.ts";
...
import { assertProviderQualifiedPluginPackages } from "./required-plugins.ts";
import { assertScenarioBackgroundMemoryIdle } from "./scenario-background-memory";
import { attachInterceptor } from "./interceptor.ts";
import { attestDeliveryAudienceFromCanonicalRoom } from "@elizaos/core";
...
import { deterministicJudgeFixturesActive } from "./judge-independence.ts";
import { drainPostDeliveryTasks } from "@elizaos/core";
...
import { executeVoiceTurn } from "./voice-turn.ts";
import { getHttpRuntime } from "@elizaos/core/api/http-plugin-runtime";
import { invalidateRelatedEntityIds } from "@elizaos/core";
import { isJudgeIndependent } from "./judge-independence.ts";
import { isLoopbackUrl } from "./utils.js";
import { judgeIndependenceRequired } from "./judge-independence.ts";
import { judgeTextWithLlm } from "./judge.ts";
import { loadScenarioRequiredPlugin } from "./required-plugins.ts";
import { logger } from "@elizaos/core";
import { pluginPackageIsRegistered } from "./required-plugins.ts";
import { postDeliveryTaskQuarantineReason } from "@elizaos/core";
import { redactForScenarioReport } from "./redaction.ts";
import { resolveRequiredFixturePlugins } from "./required-plugins.ts";
import { resolveRequiredPluginPackages } from "./required-plugins.ts";
import { resolveScenarioTurnSender } from "./turn-sender.ts";
import { revalidateOwnerExclusiveDisclosure } from "@elizaos/core";
import { runFinalCheck } from "./final-checks/index.ts";
import { scenarioLane } from "@elizaos/testing";
import { scenarioModelFixtureMode } from "./model-fixtures.ts";
...
import { type FinalCheckReport } from "./types.ts";
import { type JudgeEvidence } from "./judge.ts";
import { type JudgeResult } from "./judge.ts";
import { type Memory } from "@elizaos/core";
import { type RouteBodyValue } from "@elizaos/shared";
...
import { type ScenarioExecutionProfile } from "@elizaos/testing";
import { type ScenarioFinalCheck } from "@elizaos/testing";
import { type ScenarioJudgeRubric } from "@elizaos/testing";
import { type ScenarioLane } from "@elizaos/testing";
...
import { voiceTurnAssertionFailures } from "./voice-turn.ts";
 ...
 */
export function executorFetch(input: RequestInfo | URL, init?: RequestInit, timeoutMs: number = EXECUTOR_REQUEST_TIMEOUT_MS): Promise<Response> {
 ...
 */
export function skippedFinalCheckFailure(lane: ScenarioLane, result: Pick<FinalCheckReport, "status" | "label" | "detail">, executionProfile: ScenarioExecutionProfile = "simulated"): string | null {
...
function scenarioHasIndependentJudgeWork(scenario: ScenarioDefinition): boolean {
    if (scenario.turns.some((turn) => turn.responseJudge !== undefined &&
        typeof turn.responseJudge.rubric === "string" &&
        turn.responseJudge.rubric.trim().length > 0)) {
        return true;
    }
    return (scenario.finalChecks ?? []).some((check) => check.type === "judgeRubric" &&
 ...
 */
export function providerQualifiedScenarioProblems(scenario: ScenarioDefinition): string[] {
    ...
    if (requiredFixturePlugins.length > 0) {
        problems.push(`provider-qualified scenarios cannot declare seed fixture plugins: ${requiredFixturePlugins.join(", ")}`);
    }
    if ((scenario.seed?.length ?? 0) > 0) {
        problems.push("provider-qualified scenarios cannot declare seed steps; observe production state instead of injecting fixtures or logical time");
    }
    if (scenario.isolation !== "per-scenario") {
        problems.push("provider-qualified scenarios must declare isolation=per-scenario");
    }
    if (scenario.deferred !== undefined) {
        problems.push("provider-qualified scenarios cannot be deferred while claiming executable evidence");
    }
    if ((scenario.mockoon?.length ?? 0) > 0) {
        problems.push("provider-qualified scenarios cannot declare Mockoon services");
    }
    if ((scenario.rooms?.length ?? 0) > 0) {
        problems.push("provider-qualified target principals, rooms, and ingress belong in the operator-signed run manifest, not scenario-authored rooms");
    }
    ...
    if (forbiddenTurns.length > 0) {
        problems.push(`provider-qualified scenarios accept only explicit message turns through authenticated production ingress (${forbiddenTurns.join(", ")})`);
    }
    const executableTurnKeys = scenario.turns.flatMap((turn) => Object.keys(turn)
        .filter((key) => !PROVIDER_QUALIFIED_TURN_KEYS.has(key))
        .map((key) => `${turn.name}.${key}`));
    if (executableTurnKeys.length > 0) {
        problems.push(`provider-qualified message turns contain fields outside the data-only ingress contract (${executableTurnKeys.join(", ")})`);
    }
    if ((scenario.cleanup?.length ?? 0) > 0) {
        problems.push("provider-qualified cleanup must run after signed evidence finalization through the external operator, not scenario callbacks");
    }
    ...
    if (untrustedChecks.length > 0) {
        problems.push(`provider-qualified final checks must be independent semantic or trusted-observer checks (${untrustedChecks.join(", ")})`);
    }
    if (!scenarioHasIndependentJudgeWork(scenario)) {
        problems.push("provider-qualified scenarios require at least one responseJudge or judgeRubric evaluated by the independent judge");
    }
    if (!(scenario.finalChecks ?? []).some((check) => TRUSTED_PROVIDER_CHECK_TYPES.has(check.type))) {
        problems.push("provider-qualified scenarios require providerEffectObserved or providerNoEffectObserved as a final check");
    }
    return problems;
}
const DEFAULT_TURN_TIMEOUT_MS = 120000;
type TurnMatcher = string | RegExp;
function responsePatternMatches(pattern: unknown, responseText: string): boolean {
...
function isSynthesizedReplyAction(action: ScenarioTurnExecution["actionsCalled"][number]): boolean {
...
async function attestScenarioTurnAudience(runtime: AgentRuntime, message: Memory, room: ScenarioRoomDefinition): Promise<void> {
    ...
    if (room.channelType !== ChannelType.DM)
        return;
    ...
    const configuredOwnerEntityId = validateUuid(runtime.getSetting("ELIZA_ADMIN_ENTITY_ID"));
    if (configuredOwnerEntityId !== null &&
...
async function restoreScenarioConnectorTopology(runtime: AgentRuntime, room: ScenarioRoomDefinition): Promise<void> {
...
type ScenarioRouteRequest = http.IncomingMessage & RouteRequest & {
 ...
 */
async function resetSharedSchedulingState(runtime: AgentRuntime): Promise<void> {
    if (!pluginPackageIsRegistered(runtime, "@elizaos/plugin-personal-assistant")) {
        return;
    }
    const { resetLifeOpsScenarioState } = (await import("@elizaos/plugin-personal-assistant/plugin")) as {
        resetLifeOpsScenarioState: (runtime: AgentRuntime) => Promise<void>;
    };
    await resetLifeOpsScenarioState(runtime);
}
function assertScenarioModelFixturesConsumed(runtime: AgentRuntime): string | undefined {
...
function captureScenarioModelFixtureDiagnostics(runtime: AgentRuntime): DeterministicModelDiagnostics | undefined {
    return (runtime as RuntimeWithScenarioModelFixtures).getScenarioModelFixtureDiagnostics?.();
}
...
function summarizeActionForJudge(action: ScenarioTurnExecution["actionsCalled"][number]): string {
    ...
    if (action.result) {
        ...
        if (browserTask) {
            lines.push(`Browser task: completed=${browserTask.completed === true}, needsHuman=${browserTask.needsHuman === true}`);
        ...
        if (intervention) {
            lines.push(`Intervention: status=${typeof intervention.status === "string" ? intervention.status : "unknown"}`);
        }
...
function buildExecutionJudgeCandidate(turn: ScenarioTurn, execution: ScenarioTurnExecution): string {
    ...
    if (execution.actionsCalled.length > 0) {
        sections.push(`Observed action trace:\n${execution.actionsCalled
            .map((action) => summarizeActionForJudge(action))
            .join("\n\n")}`);
    }
    return sections.join("\n\n");
}
function buildScenarioJudgeCandidate(scenario: ScenarioDefinition, ctx: RunnerContext): string {
    const sections: string[] = [];
...
async function drainScenarioPostDeliveryTasks(runtime: AgentRuntime, opts: ExecutorOptions): Promise<string | undefined> {
    const timeoutMs = opts.postDeliveryTimeoutMs ?? opts.turnTimeoutMs;
    const controller = new AbortController();
    const readDeadline = AbortSignal.timeout(timeoutMs);
    const timeout = setTimeout(() => {
        controller.abort(new Error(`post-delivery drain timed out after ${timeoutMs}ms`));
    }, timeoutMs);
    const abortFromCaller = () => {
        controller.abort(opts.abortSignal?.reason ?? new Error("scenario execution was aborted"));
    };
    ...
    try {
        ...
        await drainScenarioBackgroundMemory(runtime, controller.signal, readDeadline);
        return undefined;
    }
    catch (error) {
...
function resolveScenarioRooms(scenario: ScenarioDefinition): ScenarioRoomDefinition[] {
    const defaultWorldId = stringToUuid(`scenario-runner-world:${scenario.id}`);
    const maybeRooms = (scenario as {
        rooms?: unknown;
    }).rooms;
    ...
    const resolved = rooms
        ...
        const logicalEntityId = typeof raw.entity === "string" && raw.entity.trim().length > 0
            ? raw.entity.trim()
            : account;
        const explicitEntity = typeof raw.entity === "string" && raw.entity.trim().length > 0;
        const logicalWorldId = typeof raw.world === "string" && raw.world.trim().length > 0
            ? raw.world.trim()
            : "default";
        const userName = typeof raw.title === "string" && raw.title.trim().length > 0
            ? raw.title.trim()
            : account;
        return {
            id,
            ...
            worldId: logicalWorldId === "default"
                ? defaultWorldId
                : stringToUuid(`scenario-world:${scenario.id}:${logicalWorldId}`),
            source: typeof raw.source === "string" && raw.source.trim().length > 0
                ? raw.source.trim()
                : "scenario-runner",
...
function resolveTurnRoom(turn: ScenarioTurn, rooms: readonly ScenarioRoomDefinition[]): ScenarioRoomDefinition {
    const defaultRoom = getDefaultScenarioRoom(rooms);
    const requestedRoom = typeof turn.room === "string" && turn.room.trim().length > 0
        ? turn.room.trim()
        : null;
    if (!requestedRoom) {
        return defaultRoom;
 ...
 */
async function establishScenarioIdentityTopology(runtime: AgentRuntime, scenarioId: string, rooms: readonly ScenarioRoomDefinition[]): Promise<void> {
    ...
    for (const [canonicalEntityId, group] of byCanonical) {
        const sourcePrincipalIds = Array.from(new Set(group.map((room) => room.userId))).filter((entityId) => entityId !== canonicalEntityId);
        if (sourcePrincipalIds.length === 0)
            continue;
        ...
        for (const sourcePrincipalId of sourcePrincipalIds) {
            const exists = existingRelationships.some((relationship) => {
                const samePair = (relationship.sourceEntityId === canonicalEntityId &&
        ...
        const authority = runtime.getService<PrincipalService>(PrincipalService.serviceType);
        if (!authority)
            continue;
        const unresolved: UUID[] = [];
        let generation = 0;
        for (const sourcePrincipalId of sourcePrincipalIds) {
            const resolution = await authority.resolveCanonicalPrincipal(runtime.agentId, sourcePrincipalId);
...
function getDefaultScenarioRoom(rooms: readonly ScenarioRoomDefinition[]): ScenarioRoomDefinition {
...
function attachResponseHelpers(res: http.ServerResponse): ScenarioRouteResponse {
...
async function augmentRequest(req: http.IncomingMessage, url: URL, params: Record<string, string>): Promise<ScenarioRouteRequest> {
    ...
    (request as unknown as Record<symbol, Buffer>)[Symbol.for("eliza.http.cachedRequestBody")] = Buffer.from(rawBody, "utf8");
...
async function startScenarioApiServer(runtime: AgentRuntime): Promise<ScenarioApiServer> {
    const server = http.createServer(async (req, res) => {
        ...
        for (const route of getHttpRuntime(runtime).routes ?? []) {
            ...
            const params = route.path === url.pathname
                ? {}
            ...
            try {
                ...
                logger.error("[scenario-runner] route handler error", error instanceof Error ? error.message : String(error));
            }
            return;
        }
        res.statusCode = 404;
        ...
        const multiplier = unit.toLowerCase() === "m"
            ...
            : unit.toLowerCase() === "h"
                ...
                : unit.toLowerCase() === "d"
                    ? 24 * 60 * 60000
                    : 7 * 24 * 60 * 60000;
        resolved.setTime(resolved.getTime() + (sign === "+" ? amount : -amount) * multiplier);
    }
...
function indexResponseIdentifiers(body: unknown, variables: ScenarioVariableState): void {
    const record = toRecord(body);
    const definition = toRecord(record?.definition);
    const definitionId = typeof definition?.id === "string" ? definition.id : undefined;
    const definitionTitle = typeof definition?.title === "string" ? definition.title : undefined;
    ...
    for (const collection of occurrenceCollections) {
        ...
        for (const item of collection) {
            ...
            const occurrenceId = typeof occurrence?.id === "string" ? occurrence.id : undefined;
            const occurrenceTitle = typeof occurrence?.title === "string" ? occurrence.title : undefined;
...
function captureResponseFields(turn: ScenarioTurn, body: unknown, variables: ScenarioVariableState): void {
    const captures = turn.captures && typeof turn.captures === "object"
        ? turn.captures
        : undefined;
    if (!captures)
        return;
    for (const [name, path] of Object.entries(captures)) {
        ...
        if (!captureName) {
            throw new Error(`[executor] api turn '${turn.name}' has an empty capture name`);
        }
        if (typeof path !== "string" || path.trim().length === 0) {
            throw new Error(`[executor] api turn '${turn.name}' capture '${captureName}' is missing a response path`);
        }
        ...
        if (value === undefined ||
            (typeof value !== "string" &&
                typeof value !== "number" &&
                typeof value !== "boolean")) {
            throw new Error(`[executor] api turn '${turn.name}' could not capture '${captureName}' from response path '${path}'`);
        }
...
async function lookupDefinitionIdByTitle(args: {
    ...
    const response = await executorFetch(`${args.apiServer.baseUrl}/api/lifeops/definitions`);
...
async function lookupOccurrenceIdByTitle(args: {
    ...
    const response = await executorFetch(`${args.apiServer.baseUrl}/api/lifeops/overview`);
    const body = await response.json();
...
async function resolveTemplateValue(args: {
    ...
    if (Array.isArray(args.value)) {
        return await Promise.all(args.value.map((item) => resolveTemplateValue({
            value: item,
            apiServer: args.apiServer,
            variables: args.variables,
        })));
    ...
    if (args.value && typeof args.value === "object") {
        const entries = await Promise.all(Object.entries(args.value as Record<string, unknown>).map(async ([key, value]) => [
            key,
...
function createScenarioComputerUseService(): ScenarioComputerUseService {
    const run = async (params: Record<string, unknown>) => {
        ...
        const isPortalWorkflow = /portal|upload|browser|resume|blocked|file/.test(blob);
        const needsHuman = /help|blocked|resume|auth|login|sign in/.test(blob) || isPortalWorkflow;
...
async function runCustomSeeds(scenario: ScenarioDefinition, runtime: AgentRuntime, ctx: RunnerContext, initialNow: Date): Promise<SeedRunResult> {
    const seeds = (scenario as {
        seed?: unknown;
    }).seed;
    ...
    for (const seed of seeds) {
        ...
        if (type === "advanceClock") {
            ...
            try {
                currentNow = addClockOffset(currentNow, (resolvedSeed as {
                    by: string;
                }).by);
        ...
        if (type === "custom" && typeof apply === "function") {
            try {
                const result = await (apply as (c: ScenarioContext) => unknown)(scenarioCtx);
        ...
        try {
            const result = await applyScenarioSeedStep(scenarioCtx, resolvedSeed as Exclude<ScenarioDefinition["seed"], undefined>[number]);
            if (typeof result === "string" && result.length > 0) {
                return {
                    now: currentNow,
                    error: `seed ${name ?? "(unnamed)"}: ${result}`,
...
async function deleteMockGmailDrafts(): Promise<string | undefined> {
    ...
    for (const draft of drafts) {
        ...
        const deleteResponse = await executorFetch(`${baseUrl}/gmail/v1/users/me/drafts/${encodeURIComponent(id)}`, { method: "DELETE" });
...
async function clearSelfControlBlocks(): Promise<string | undefined> {
    const { stopSelfControlBlock } = await import("@elizaos/plugin-blocker/services/website-blocker/index");
...
async function runScenarioCleanups(scenario: ScenarioDefinition, runtime: AgentRuntime, ctx: RunnerContext): Promise<string[]> {
    const cleanups = (scenario as {
        cleanup?: unknown;
    }).cleanup;
    ...
    for (const cleanup of cleanups) {
        ...
        try {
            if (step.type === "gmailDeleteDrafts") {
                ...
                const customResult = await (step.apply as (c: ScenarioContext) => unknown)(scenarioCtx);
                result =
            ...
            failures.push(`cleanup ${String(step.name ?? step.type)} threw: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    return failures;
}
async function executeMessageTurn(runtime: AgentRuntime, turn: ScenarioTurn, room: ScenarioRoomDefinition, currentNow: Date, turnTimeoutMs: number, scenarioId: string, runId?: string): Promise<{
    responseText: string;
    durationMs: number;
    syntheticFailure: boolean;
}> {
    ...
    const turnContent = turn.content !== null && typeof turn.content === "object"
    ...
    const messageService = (runtime as {
        messageService?: {
            handleMessage: (rt: AgentRuntime, memory: Memory, cb: (content: SyntheticFailureAwareContent) => Promise<unknown>, options?: Record<string, unknown>) => Promise<{
    ...
    if (!messageService) {
        throw new Error("[executor] runtime.messageService is not initialized — cannot send messages");
    }
    ...
    const callback = async (content: SyntheticFailureAwareContent): Promise<unknown[]> => {
        if (content.text)
            responseText += content.text;
        if (content.elizaSyntheticFailure === true)
            syntheticFailure = true;
        return [];
    };
    const timeoutMs = typeof turn.timeoutMs === "number" ? turn.timeoutMs : turnTimeoutMs;
    const result = await withTimeout(messageService.handleMessage(runtime, message, callback, {}), timeoutMs, `handleMessage(${turn.name})`);
...
async function executeActionTurn(runtime: AgentRuntime, turn: ScenarioTurn, room: ScenarioRoomDefinition, currentNow: Date, turnTimeoutMs: number, hiddenActionNames: readonly string[] = []): Promise<{
    validation: NonNullable<ScenarioTurnExecution["validation"]>;
    responseText: string;
    responseBody: unknown;
    durationMs: number;
}> {
    const actionName = typeof turn.actionName === "string" && turn.actionName.trim().length > 0
        ? turn.actionName.trim()
        : turn.content !== null &&
            typeof turn.content === "object" &&
            typeof (turn.content as {
                action?: unknown;
            }).action === "string"
            ? String((turn.content as {
                action: string;
            }).action).trim()
            : "";
    if (!actionName) {
        throw new Error(`[executor] action turn '${turn.name}' is missing actionName`);
    }
    const action = runtime.actions.find((candidate: Action) => candidate.name === actionName);
    if (!action) {
        ...
        if (hiddenActionNames.includes(actionName)) {
            throw new Error(`[executor] action turn '${turn.name}' requested '${actionName}', which is hidden from this scenario because only another scenario in this run declared the plugin that provides it; add that package to this scenario's requires.plugins`);
        }
        throw new Error(`[executor] action turn '${turn.name}' requested unknown action '${actionName}'`);
    }
    const text = typeof turn.text === "string"
        ? String(resolveScenarioTemplates(turn.text, currentNow))
        : actionName;
    ...
    const options = turn.options !== null && typeof turn.options === "object"
    ...
    const timeoutMs = typeof turn.timeoutMs === "number" ? turn.timeoutMs : turnTimeoutMs;
    const validated = await withTimeout(action.validate(runtime, message, undefined, options as never), timeoutMs, `validateAction(${turn.name})`);
    ...
    if (!validated) {
        ...
        throw new Error(`[executor] action turn '${turn.name}' failed validation for '${actionName}'`);
    }
    if (expectedValidation === "rejected") {
        throw new Error(`[executor] action turn '${turn.name}' expected validation rejection for '${actionName}', but validation accepted the input`);
    }
    const result = await withTimeout(action.handler(runtime, message, undefined, options as never, callback as never), timeoutMs, `executeAction(${turn.name})`);
...
async function executeApiTurn(args: {
    ...
    const method = typeof args.turn.method === "string" && args.turn.method.trim().length > 0
        ? args.turn.method.trim().toUpperCase()
        : "GET";
    const rawPath = typeof args.turn.path === "string" && args.turn.path.trim().length > 0
        ? args.turn.path.trim()
        : null;
    if (!rawPath) {
    ...
    const body = args.turn.body === undefined
        ? undefined
        : await resolveTemplateValue({
    ...
    const configuredHeaders = args.turn.headers
        ...
        : undefined;
    const startedAt = Date.now();
    const timeoutMs = typeof args.turn.timeoutMs === "number"
        ? args.turn.timeoutMs
        : args.turnTimeoutMs;
    ...
    const timeoutId = setTimeout(() => {
        controller.abort(new Error(`api(${args.turn.name}) timed out after ${timeoutMs}ms`));
    }, timeoutMs);
    const abortFromCaller = () => {
        controller.abort(args.abortSignal?.reason ?? new Error("scenario execution was aborted"));
    };
    ...
    try {
        ...
        const explicitRedactions = Array.isArray(args.turn.redactResponseFields)
            ? args.turn.redactResponseFields.filter((field): field is string => typeof field === "string")
            : [];
        const reportResponseBody = redactForScenarioReport(responseBody, explicitRedactions);
        return {
            apiStatus: response.status,
            apiBody: responseBody,
            statusCode: response.status,
            responseBody,
            responseText: typeof responseBody === "string"
                ? responseBody
                : JSON.stringify(responseBody ?? ""),
            reportResponseText: typeof reportResponseBody === "string"
                ? reportResponseBody
...
async function executeTickTurn(args: {
    ...
    const worker = typeof args.turn.worker === "string" && args.turn.worker.trim().length > 0
        ? args.turn.worker.trim()
        : null;
    if (worker !== "lifeops_scheduler") {
        throw new Error(`[executor] tick turn '${args.turn.name}' has unsupported worker '${worker ?? "(missing)"}'`);
    }
    ...
    const now = typeof args.turn.now === "string"
        ...
        : undefined;
    const startedAt = Date.now();
    const { executeLifeOpsSchedulerTask } = (await import("@elizaos/plugin-personal-assistant/plugin")) as {
        executeLifeOpsSchedulerTask: (runtime: AgentRuntime, options: Record<string, unknown>) => Promise<Record<string, unknown>>;
    };
    const result = await withTimeout(executeLifeOpsSchedulerTask(args.runtime, {
        ...(toRecord(options) ?? {}),
        ...(now ? { now } : {}),
    }), typeof args.turn.timeoutMs === "number"
        ? args.turn.timeoutMs
        : args.turnTimeoutMs, `tick(${args.turn.name})`);
...
async function executeWaitTurn(turn: ScenarioTurn, turnTimeoutMs: number, runtime: AgentRuntime, ctx: RunnerContext): Promise<{
    statusCode: number;
    responseBody: unknown;
    responseText: string;
    durationMs: number;
    ...
    if (until === undefined &&
        (typeof durationMs !== "number" ||
            ...
            durationMs < 0)) {
        throw new Error(`[executor] wait turn '${turn.name}' requires non-negative durationMs`);
    }
    if (until !== undefined && typeof until !== "function") {
        throw new Error(`[executor] wait turn '${turn.name}' has a non-callable until predicate`);
    }
    const timeoutMs = typeof turn.timeoutMs === "number" ? turn.timeoutMs : turnTimeoutMs;
    const startedAt = Date.now();
    if (until) {
        ...
        if (!Number.isFinite(pollIntervalMs) || pollIntervalMs <= 0) {
            throw new Error(`[executor] wait turn '${turn.name}' requires a positive pollIntervalMs`);
        }
        const deadline = startedAt + timeoutMs;
        while (true) {
            const predicateBudgetMs = deadline - Date.now();
            if (predicateBudgetMs <= 0) {
                throw new Error(`waitUntil(${turn.name}) timed out after ${timeoutMs}ms`);
            }
            if (await withTimeout(Promise.resolve(until({ ...ctx, runtime })), predicateBudgetMs, `waitUntil(${turn.name})`)) {
                break;
            }
            ...
            await new Promise<void>((resolve) => setTimeout(resolve, Math.min(pollIntervalMs, sleepBudgetMs)));
        }
    }
    else {
        await withTimeout(new Promise((resolve) => setTimeout(resolve, durationMs)), timeoutMs, `wait(${turn.name})`);
    }
    const responseBody = {
        success: true,
...
async function runTurnAssertions(turn: ScenarioTurn, execution: ExecutedTurn, runtime: AgentRuntime, minJudgeScore: number): Promise<TurnAssertionResult> {
    const failures: string[] = [];
    ...
    if (execution.syntheticFailure === true) {
        failures.push("runtimeFailureReply: the runtime returned a synthetic model/runtime failure reply (rate-limit, auth, credits, or generic apology); this cannot satisfy scenario evidence");
    }
    if (typeof turn.assertResponse === "function") {
        const result = turnUsesStatusResponse(kind)
            ? await (turn.assertResponse as (status: number, body: unknown) => unknown)(execution.statusCode ?? 0, execution.responseBody)
            : await (turn.assertResponse as (text: string) => unknown)(execution.responseText ?? "");
        if (typeof result === "string" && result.length > 0) {
            failures.push(`assertResponse: ${result}`);
        }
    }
    if (turnUsesStatusResponse(kind)) {
        const expectedStatus = (turn as {
            expectedStatus: number;
        }).expectedStatus;
        if (typeof expectedStatus === "number" &&
            execution.statusCode !== expectedStatus) {
            failures.push(`expectedStatus: expected ${expectedStatus}, saw ${execution.statusCode ?? "unknown"}`);
        }
    }
    if (kind === "voice") {
        ...
        failures.push(...voiceTurnAssertionFailures(execution.responseBody as VoiceWorkbenchScenarioRun | undefined, { allowVoiceSkip: turn.allowVoiceSkip === true }));
    }
    ...
    const expectedActions = stringList((turn as {
        expectedActions?: unknown;
    }).expectedActions);
    if (expectedActions.length > 0) {
        const realActions = execution.actionsCalled.filter((action) => !isSynthesizedReplyAction(action));
        const ok = realActions.some((action) => actionMatchesScenarioExpectation(action.actionName, expectedActions));
        if (!ok) {
            const realActionNames = realActions.map((action) => action.actionName).join(",") || "(none)";
            const capturedActionNames = execution.actionsCalled.map((action) => action.actionName).join(",") ||
                "(none)";
            const capturedDetail = capturedActionNames === realActionNames
                ? ""
                : `; captured actions: [${capturedActionNames}]`;
            failures.push(`expectedActions: expected action in [${expectedActions.join(",")}], saw actions [${realActionNames}]${capturedDetail}`);
        }
    }
    // responseIncludesAny / responseIncludesAll / responseExcludes / forbiddenActions (inline)
    const includesAny = (turn as {
        responseIncludesAny?: unknown;
    })
        .responseIncludesAny;
    if (Array.isArray(includesAny) && includesAny.length > 0) {
        const text = execution.responseText ?? "";
        const ok = includesAny.some((pattern) => responsePatternMatches(pattern, text));
        if (!ok) {
            failures.push(`responseIncludesAny: expected response to include any of [${includesAny.join(",")}], saw ${JSON.stringify(execution.responseText ?? "")}`);
        }
    }
    const includesAll = (turn as {
        responseIncludesAll?: unknown;
    })
        .responseIncludesAll;
    if (Array.isArray(includesAll) && includesAll.length > 0) {
        const text = execution.responseText ?? "";
        const missing = includesAll.filter((pattern) => !responsePatternMatches(pattern, text));
        if (missing.length > 0) {
            failures.push(`responseIncludesAll: expected response to include all of [${includesAll.join(",")}], missing [${missing.join(",")}], saw ${JSON.stringify(execution.responseText ?? "")}`);
        }
    }
    const excludes = (turn as {
        responseExcludes?: unknown;
    }).responseExcludes;
    if (Array.isArray(excludes) && excludes.length > 0) {
        const text = execution.responseText ?? "";
        const hits = excludes.filter((pattern) => responsePatternMatches(pattern, text));
        if (hits.length > 0) {
            failures.push(`responseExcludes: response included forbidden pattern(s) [${hits.join(",")}], saw ${JSON.stringify(execution.responseText ?? "")}`);
        }
    }
    const forbidden = (turn as {
        forbiddenActions?: unknown;
    }).forbiddenActions;
    if (Array.isArray(forbidden) && forbidden.length > 0) {
        const hits = execution.actionsCalled.filter((a) => forbidden.includes(a.actionName));
        if (hits.length > 0) {
            failures.push(`forbiddenActions triggered: ${hits.map((h) => h.actionName).join(",")}`);
        }
    }
    const plannerIncludesAll = toTurnMatcherArray((turn as {
        plannerIncludesAll?: unknown;
    }).plannerIncludesAll);
    const plannerIncludesAny = toTurnMatcherArray((turn as {
        plannerIncludesAny?: unknown;
    }).plannerIncludesAny);
    const plannerExcludes = toTurnMatcherArray((turn as {
    ...
    if (plannerIncludesAll.length > 0 ||
        ...
        if (plannerIncludesAll.length > 0) {
            const missing = plannerIncludesAll.filter((pattern) => !matchesTurnMatcher(plannerBlob, pattern));
            if (missing.length > 0) {
                failures.push(`plannerIncludesAll: expected planner trace to include ${formatTurnMatcher(missing[0] as TurnMatcher)}, saw ${plannerEvidence}`);
            }
        }
        if (plannerIncludesAny.length > 0) {
            const ok = plannerIncludesAny.some((pattern) => matchesTurnMatcher(plannerBlob, pattern));
            if (!ok) {
                failures.push(`plannerIncludesAny: expected planner trace to include any of [${formatTurnMatchers(plannerIncludesAny)}], saw ${plannerEvidence}`);
            }
        }
        if (plannerExcludes.length > 0) {
            const hits = plannerExcludes.filter((pattern) => matchesTurnMatcher(plannerBlob, pattern));
            if (hits.length > 0) {
                failures.push(`plannerExcludes: expected planner trace to exclude [${formatTurnMatchers(hits)}], saw ${plannerEvidence}`);
            }
        }
    }
    if (turn.responseJudge) {
        ...
        try {
            const judged = await judgeTextWithLlm(runtime, buildExecutionJudgeCandidate(turn, execution), rubric.rubric);
            judgment = judged;
            judgeScore = judged.score;
            if (judged.score < threshold) {
                failures.push(`responseJudge: score ${judged.score.toFixed(2)} < ${threshold}: ${judged.reason}`);
            }
        }
        catch (err) {
            // error-policy:J1 expose invalid judgment evidence in the scenario report.
            if (err instanceof JudgeParseError)
                judgeFailure = err.evidence;
            failures.push(`responseJudge: judge failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    return {
        failures,
...
async function runJudgeRubricFinalCheck(check: ScenarioFinalCheck, scenario: ScenarioDefinition, runtime: AgentRuntime, ctx: RunnerContext, minJudgeScore: number): Promise<FinalCheckReport> {
...
async function runObservedScenario(scenario: ScenarioDefinition, runtime: AgentRuntime, opts: ExecutorOptions): Promise<ScenarioReport> {
    const startedAt = Date.now();
    const executionProfile = opts.executionProfile ?? DEFAULT_SCENARIO_EXECUTION_PROFILE;
    ...
    const report: ScenarioReport = {
        ...
        executionProfile,
        evidence: executionProfile === "simulated"
            ? {
                schemaVersion: 1,
    ...
    if (opts.postDeliveryTimeoutMs !== undefined &&
    ...
    try {
        await assertScenarioBackgroundMemoryIdle(runtime, AbortSignal.timeout(opts.postDeliveryTimeoutMs ?? opts.turnTimeoutMs));
    }
    catch (error) {
    ...
    if (scenario.executionProfile !== undefined &&
    ...
    if (executionProfile === "provider-qualified") {
        ...
        preflightProblems.push("the in-process scenario executor cannot establish authenticated production ingress or an independent signed observer boundary; use the provider-qualified controller rather than relabeling this runtime");
        const missingPlugins = resolveRequiredPluginPackages(scenario).filter((packageName) => !pluginPackageIsRegistered(runtime, packageName));
        if (missingPlugins.length > 0) {
            preflightProblems.push(`declared production plugin(s) not registered before execution: ${missingPlugins.join(", ")}`);
        }
        if (runtime.getService("lifeops_scheduled_task_runner") === null ||
            runtime.getService("lifeops_scheduled_task_runner") === undefined) {
            preflightProblems.push("production scheduled-task runner service is not registered and active");
        }
        if (!(await isJudgeIndependent())) {
            preflightProblems.push("independent judge identity has not been observed; credentials alone cannot qualify this execution");
        }
        if (!opts.runDir) {
            preflightProblems.push("provider-qualified execution requires runDir so exact trajectory files can be hashed");
        }
    ...
    ctx.worldIds = Object.fromEntries(rooms.map((room) => [room.logicalWorldId, room.worldId]));
    ctx.entityIds = Object.fromEntries(rooms.map((room) => [room.logicalEntityId, room.canonicalEntityId]));
    ctx.accountEntityIds = Object.fromEntries(rooms.map((room) => [room.accountId, room.userId]));
    ctx.roomWorldIds = Object.fromEntries(rooms.map((room) => [room.id, room.worldId]));
    ctx.roomEntityIds = Object.fromEntries(rooms.map((room) => [room.id, room.userId]));
    ...
    const actionScope = enterScenarioActionScope(runtime, scenario, opts.batchPluginPackages ?? [], opts.scenarioDeclaredActionNames);
    if (actionScope.hiddenActionNames.length > 0) {
        logger.info(`[scenario-runner] ${scenario.id}: hiding ${actionScope.hiddenActionNames.length} action(s) declared only by batch peers: ${actionScope.hiddenActionNames.join(", ")}`);
    }
    try {
        beginScenarioModelFixtureAttempt(runtime, scenario, opts.attemptId ?? `${scenario.id}:attempt-1`, opts.worldId);
        await resetSharedSchedulingState(runtime);
        runtime.setSetting("ELIZA_ADMIN_ENTITY_ID", primaryRoom.canonicalEntityId, false);
        ...
        runtime.setSetting("ELIZA_OWNER_CONTACTS_JSON", JSON.stringify(Object.fromEntries(rooms
            .filter((room) => room.canonicalEntityId === primaryRoom.canonicalEntityId)
            .map((room) => [
            room.accountId,
        ...
        for (const pkg of requiredPluginPackages) {
            if (pluginPackageIsRegistered(runtime, pkg))
                continue;
            try {
        ...
        const missing = [
            ...requiredPluginPackages.filter((plugin) => !pluginPackageIsRegistered(runtime, plugin) &&
                !autoLoaded.has(plugin)),
            ...requiredFixturePlugins.filter((plugin) => !pluginPackageIsRegistered(runtime, plugin)),
        ];
        if (missing.length > 0) {
        ...
        for (const turn of scenario.turns) {
            ...
            const execution: ExecutedTurn = kind === "voice"
                ? {
                    actionsCalled: [],
                ...
                : kind === "api"
                    ...
                    : kind === "tick"
                        ...
                        : kind === "action"
                            ? {
                                actionsCalled: [],
                                ...(await executeActionTurn(runtime, turn, resolveTurnRoom(turn, rooms), logicalNow, opts.turnTimeoutMs || DEFAULT_TURN_TIMEOUT_MS, actionScope.hiddenActionNames)),
                            }
                            : kind === "wait"
                                ? {
                                    actionsCalled: [],
                                    ...(await executeWaitTurn(turn, opts.turnTimeoutMs || DEFAULT_TURN_TIMEOUT_MS, runtime, ctx)),
                                }
                                : {
                                    actionsCalled: [],
                                    ...(await executeMessageTurn(runtime, turn, resolveTurnRoom(turn, rooms), logicalNow, opts.turnTimeoutMs || DEFAULT_TURN_TIMEOUT_MS, scenario.id, ctx.runId)),
                                };
            ...
            const { failures: failedAssertions, judgeScore: turnJudgeScore, judgment, judgeFailure, } = await runTurnAssertions(turn, execution, runtime, opts.minJudgeScore);
            if (turnJudgeScore !== undefined) {
                judgeScores.push(turnJudgeScore);
            }
            const voiceRun = kind === "voice"
            ...
            report.turns.push({
                ...
                kind,
                text: typeof turn.text === "string" ? turn.text : undefined,
                responseText: execution.reportResponseText ?? execution.responseText ?? "",
            ...
            if (kind === "message" &&
                ...
                const drainFailure = await drainScenarioPostDeliveryTasks(runtime, opts);
                if (drainFailure) {
                    report.status = "failed";
                    report.failedAssertions.push({
        ...
        const finalChecks = Array.isArray((scenario as {
            finalChecks?: unknown;
        }).finalChecks)
            ? ((scenario as {
        ...
        for (const check of finalChecks) {
            ...
            if (type === "judgeRubric") {
                result = await runJudgeRubricFinalCheck(check, scenario, runtime, ctx, opts.minJudgeScore);
            }
            else {
            ...
            if (result.status === "failed") {
                ...
                const failure = skippedFinalCheckFailure(scenarioLane(scenario), result, executionProfile);
                if (failure) {
                    report.status = "failed";
                    report.failedAssertions.push({
                        label: result.label,
                    ...
                    logger.warn(`[scenario-runner] ${scenario.id} finalCheck "${result.label}" skipped — ${result.detail}. This check proved nothing this run.`);
                }
            }
        }
    }
        ...
        const preCleanupDrainFailure = await drainScenarioPostDeliveryTasks(runtime, opts);
        ...
        const postCleanupDrainFailure = await drainScenarioPostDeliveryTasks(runtime, opts);
    ...
    if (judgeScores.length > 0) {
        ...
        const judgments = [
            ...
            ...report.finalChecks.flatMap((check) => check.judgment ? [check.judgment] : []),
        ];
        if (!deterministicJudgeFixturesActive()) {
            report.judgeIndependence =
                judgments.length &&
                    judgments.every((result) => result.evidence.independence === "independent")
                    ? "independent"
                    : judgments.some((result) => result.evidence.independence === "self-graded")
                        ? "self-graded"
                        : "unknown";
        }
        if (!deterministicJudgeFixturesActive() &&
            ...
            logger.warn(`[scenario-runner] ${scenario.id}: judge independence was not established from observed model identities; inspect judgment evidence`);
            if (judgeIndependenceRequired()) {
                report.status = "failed";
                report.failedAssertions.push({
                    label: "judgeIndependence",
                    detail: "SCENARIO_JUDGE_REQUIRE_INDEPENDENT=1: observed identities do not establish independence from the model under test; CEREBRAS_API_KEY alone is insufficient",
                });
            }
        }
    }
...
export async function runScenario(scenario: ScenarioDefinition, runtime: AgentRuntime, opts: ExecutorOptions): Promise<ScenarioReport> {
