...
import { finalMessageUserText } from "@elizaos/testing";
...
import githubPlugin, { GitHubService, } from "../../../../../plugins/plugin-github/src/index.ts";
...
const ISSUE_CREATE_PREVIEW = 'About to create octo/repo issue: "Deterministic issue" [labels: scenario] [assignees: hubot] as agent. Re-invoke with confirmed: true to proceed.';
type JsonRecord = Record<string, unknown>;
type RuntimeWithGithubScenario = IAgentRuntime & RuntimeWithScenarioModelFixtures & {
...
async function ensureGithubPlugin(runtime: RuntimeWithGithubScenario): Promise<GitHubService> {
    const registered = (runtime.plugins ?? []).some((plugin) => plugin.name === githubPlugin.name);
    if (!registered) {
    ...
    getHttpRuntime(runtime).routes = routes.filter((route) => route.__scenarioGithubRoute !== true);
    ...
    const service = ((await runtime.getServiceLoadPromise?.(GitHubService.serviceType)) as GitHubService | undefined) ??
...
function githubActionParameters(action: string, parameters: Record<string, unknown> = {}): Record<string, unknown> {
    return {
        action,
        repo: REPO,
        as: "agent",
...
const githubPreviewEvaluator: DeterministicModelFixture = {
    ...
    response: {
        success: false,
        decision: "FINISH",
        thought: "The issue mutation is awaiting the user's confirmation; no issue was created.",
        messageToUser: ISSUE_CREATE_PREVIEW,
    },
    times: 1,
};
function registerGithubStrictFixtures(runtime: RuntimeWithGithubScenario): void {
    ...
    runtime.scenarioModelFixtures?.register(route, planner, githubPreviewEvaluator);
}
function expectGithubPreview(execution: ScenarioTurnExecution): string | undefined {
...
function expectGithubConfirmation({ actionName, expectedLedgerLength, expectedPreview, expectedSuccess, }: {
    actionName: string;
    ...
    return (execution) => {
        if (execution.validation?.actionName !== actionName ||
...
function expectGithubCreate(execution: ScenarioTurnExecution): string | undefined {
    const action = firstAction(execution, "GITHUB_ISSUE_CREATE");
    if (typeof action === "string")
        return action;
...
function expectGithubIssueAssign(execution: ScenarioTurnExecution): string | undefined {
...
function expectGithubIssueState(actionName: "GITHUB_ISSUE_CLOSE" | "GITHUB_ISSUE_REOPEN", op: "close" | "reopen", responseText: string): (execution: ScenarioTurnExecution) => string | undefined {
    return (execution) => {
...
function expectGithubIssueComment(execution: ScenarioTurnExecution): string | undefined {
...
function expectGithubIssueLabel(execution: ScenarioTurnExecution): string | undefined {
...
function expectGithubPrList(execution: ScenarioTurnExecution): string | undefined {
...
function expectGithubParentPrList(execution: ScenarioTurnExecution): string | undefined {
...
function expectGithubPrReview(execution: ScenarioTurnExecution): string | undefined {
...
function expectGithubNotificationTriage(execution: ScenarioTurnExecution): string | undefined {
...
function expectGithubTokenRoute(status: number, body: unknown): string | undefined {
    if (status !== 200)
        return `expected status 200, saw ${status}`;
    return expectEqual(body, { connected: false, deviceFlowAvailable: false }, "GitHub token route body");
}
async function finalGithubCheck(): Promise<string | undefined> {
    const service = scenarioRuntime?.getService<GitHubService>(GitHubService.serviceType);
    ...
    const ledgerFailure = expectEqual(githubLedger, expectedLedger, "GitHub Octokit ledger");
...
export default scenario({
    ...
    seed: [
        transientTurnEvaluationSeed(strictGithubRoutes.map((route) => ({
            input: route.input,
            action: route.actionName,
            completed: route.input !== "create deterministic GitHub issue preview",
            actionSuccess: route.input !== "create deterministic GitHub issue preview",
            reason: route.input === "create deterministic GitHub issue preview"
    ...
    turns: [
        ...
        },
        {
            ...
            assertTurn: expectGithubIssueState("GITHUB_ISSUE_CLOSE", "close", `Closed ${REPO}#17: ${ISSUE_TITLE}`),
        },
        {
            kind: "action",
        ...
        },
        {
            ...
            assertTurn: expectGithubIssueState("GITHUB_ISSUE_REOPEN", "reopen", `Reopened ${REPO}#17: ${ISSUE_TITLE}`),
        },
        {
            kind: "action",
        ...
        },
        {
            ...
            assertTurn: expectGithubConfirmation({
                ...
                expectedPreview: buildReviewPreview("approve", REPO, 17, REVIEW_BODY, "user"),
