...
import { WORKFLOW_SERVICE_TYPE } from "../../../../../plugins/plugin-workflow/src/services/index.ts";
...
import { type WorkflowService } from "../../../../../plugins/plugin-workflow/src/services/index.ts";
import workflowPlugin, { workflowRoutePlugin, } from "../../../../../plugins/plugin-workflow/src/index.ts";
...
const workflowModelFixtures = strictActionRouteFixtures(strictWorkflowRoutes[0]);
...
function expectWorkflowActionOptions(action: CapturedAction): string | undefined {
    const actual = actionParameters(action);
    if (!expectEqual(actual, workflowExecutionParameters, "WORKFLOW handler options")) {
        return undefined;
    }
    const nested = isRecord(actual.parameters) ? actual.parameters : null;
    if (nested &&
        !expectEqual(nested, workflowExecutionParameters, "WORKFLOW nested handler parameters")) {
        return undefined;
    }
...
function expectSeededExecution(execution: unknown): string | undefined {
    ...
    for (const [path, expected] of Object.entries({
        ...
        const failure = expectEqual(readPath(item, path), expected, `input.${path}`);
        if (failure)
            return failure;
    }
...
async function ensureWorkflowPlugin(runtime: RuntimeWithWorkflowScenario): Promise<void> {
    ...
    getHttpRuntime(runtime).routes = routes.filter((route) => route.__scenarioWorkflowRoute !== true);
...
async function workflowServices(runtime: RuntimeWithWorkflowScenario): Promise<{
    ...
    const service = ((await runtime.getServiceLoadPromise?.(WORKFLOW_SERVICE_TYPE)) as WorkflowService | undefined) ??
        runtime.getService<WorkflowService>(WORKFLOW_SERVICE_TYPE);
    if (!embedded)
        throw new Error("EmbeddedWorkflowService was not registered");
    if (!service)
        throw new Error("WorkflowService was not registered");
...
async function seedWorkflow(ctx: ScenarioContext): Promise<string | undefined> {
    ...
    try {
        ...
        if (!ctx.primaryUserId)
            return "scenario primary user was not available";
        const ownerTag = await embedded.getOrCreateTag(await getUserTagName(runtime, ctx.primaryUserId));
...
function expectWorkflowAction(execution: ScenarioTurnExecution): string | undefined {
    ...
    for (const [path, expected] of Object.entries({
        ...
        const failure = expectEqual(readPath(executions[0], path), expected, `captured execution ${path}`);
        if (failure)
            return failure;
    }
    return undefined;
}
function expectWorkflowRoute(status: number, body: unknown): string | undefined {
    if (status !== 200)
...
function expectExecutionsRoute(status: number, body: unknown): string | undefined {
    if (status !== 200)
        return `expected status 200, saw ${status}`;
...
async function finalWorkflowCheck(ctx: ScenarioContext): Promise<string | undefined> {
    const runtime = ctx.runtime as RuntimeWithWorkflowScenario | undefined;
...
export default scenario({
    ...
    seed: [
        transientTurnEvaluationSeed(strictWorkflowRoutes.map((route) => ({
            input: route.input,
