...
`bun run --cwd packages/testing test:pr:e2e` runs the zero-cost PR
...
```bash
bun run --cwd packages/testing test:live:e2e
```
...
- `live-help-knowledge` covers the deleted-Help-view replacement: a real model
  ...
  `eliza-scenarios run packages/testing/scenario-runner/test/scenarios --scenario live-help-knowledge --report <out> --run-dir <dir>`
...
- `live-lifeops-task-filter-due-window` and `live-plugin-enable-toggle-verb`
  ...
  used. Run both with
  `eliza-scenarios run packages/testing/scenario-runner/test/scenarios --lane live-only '**/live-lifeops-task-filter-due-window.scenario.ts' '**/live-plugin-enable-toggle-verb.scenario.ts' --report <out> --run-dir <dir>`.
