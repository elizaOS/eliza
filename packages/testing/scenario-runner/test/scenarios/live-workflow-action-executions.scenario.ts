 ...
 * lane.
 */
import { EMBEDDED_WORKFLOW_SERVICE_TYPE } from "../../../../../plugins/plugin-workflow/src/services/index.ts";
...
import { type EmbeddedWorkflowService } from "../../../../../plugins/plugin-workflow/src/services/index.ts";
...
async function embeddedService(runtime: RuntimeWithWorkflow): Promise<EmbeddedWorkflowService> {
    const embedded = ((await runtime.getServiceLoadPromise?.(EMBEDDED_WORKFLOW_SERVICE_TYPE)) as EmbeddedWorkflowService | undefined) ??
        runtime.getService<EmbeddedWorkflowService>(EMBEDDED_WORKFLOW_SERVICE_TYPE);
    if (!embedded)
        throw new Error("EmbeddedWorkflowService was not registered");
    return embedded;
}
async function seedWorkflow(ctx: ScenarioContext): Promise<string | undefined> {
    ...
    if (!runtime?.db)
        return "scenario runtime db was not available";
    const registered = (runtime.plugins ?? []).some((plugin) => plugin.name === workflowPlugin.name);
    if (!registered)
        await runtime.registerPlugin?.(workflowPlugin);
    // Mount the plugin's routes so the executions op resolves the same way the
    // running app does.
    const existing = (getHttpRuntime(runtime).routes ?? []).filter((route) => route.__scenarioWorkflowRoute !== true);
    ...
    const ownerId = runtime.getSetting?.("ELIZA_ADMIN_ENTITY_ID") as string | undefined;
    if (!ownerId)
        return "scenario owner entity id was not available";
...
function expectWorkflowExecutionsAction(execution: ScenarioTurnExecution): string | undefined {
    const action = execution.actionsCalled.find((candidate) => candidate.actionName === "WORKFLOW");
    if (!action) {
...
async function finalWorkflowState(ctx: ScenarioContext): Promise<string | undefined> {
    const runtime = ctx.runtime as RuntimeWithWorkflow | undefined;
    if (!runtime)
        return "scenario runtime unavailable in final check";
    const embedded = await embeddedService(runtime);
