 ...
 */
import { MCP_SERVICE_NAME } from "../../../../../plugins/plugin-mcp/src/types.ts";
...
import { registerStrictActionRouteFixtures } from "@elizaos/testing";
import { resolve } from "node:path";
import { scenario } from "@elizaos/testing";
import { strictActionRouteFixtures } from "@elizaos/testing";
import { type CapturedAction } from "@elizaos/testing";
import { type DeterministicModelCall } from "@elizaos/testing";
import { type HttpPlugin as Plugin } from "@elizaos/shared";
import { type IAgentRuntime } from "@elizaos/core";
import { type McpServer } from "../../../../../plugins/plugin-mcp/src/types.ts";
...
import mcpPlugin, { handleMcpRoutes, type McpRouteConfig, } from "../../../../../plugins/plugin-mcp/src/index.ts";
...
const parentListConnectionsInput = "Fetch deterministic MCP connections through the parent action.";
...
const strictMcpRoutes = [
    ...
    },
    {
        ...
        messageToUser: "MCP op=list_connections is only available in the cloud runtime.",
    },
    {
        ...
        messageToUser: "MCP op=search_actions is only available in the cloud runtime.",
    },
    {
...
function matchesUnsupportedMcpEvaluation(expectedInput: string, op: string, phase: "evaluation" | "final-reply" = "evaluation") {
    const route = strictMcpRoutes.find((candidate) => candidate.input === expectedInput);
    if (!route)
        throw new Error("Missing declared MCP route");
    return (call: DeterministicModelCall) => {
        ...
        if (!messages.some((message) => message.role === "system" &&
            ...
            message.content.includes(phase === "evaluation" ? "evaluator_stage:\n" : "planner_stage:\n")))
            return false;
        ...
        if (phase === "final-reply") {
            ...
            if (finalMessage?.role !== "user" ||
                ...
                !finalMessage.content.startsWith(`The ${route.actionName} step failed and the turn is ending without a usable result. Recorded failure cause: MCP op=${op} is only available in the cloud runtime. Do not call any tool and do not claim the failed step succeeded.`))
                return false;
        }
...
type RuntimeWithMcpScenario = IAgentRuntime & RuntimeWithScenarioModelFixtures & {
    plugins?: Plugin[];
...
function expectActionParameters(action: CapturedAction, expectedParameters: JsonRecord): string | undefined {
    const actual = actionParameters(action);
    const parameters = isRecord(actual.parameters) ? actual.parameters : actual;
    return expectEqual(parameters, expectedParameters, `${action.actionName} handler parameters`);
}
function firstAction(execution: ScenarioTurnExecution, actionName: string): CapturedAction | string {
    const action = execution.actionsCalled.find((candidate) => candidate.actionName === actionName);
    return (action ??
        `expected ${actionName} action, saw ${execution.actionsCalled.map((candidate) => candidate.actionName).join(", ") || "none"}`);
}
function expectMcpReadResourceAction(execution: ScenarioTurnExecution): string | undefined {
    const action = firstAction(execution, "MCP_READ_RESOURCE");
    if (typeof action === "string")
        return action;
    const parametersFailure = expectActionParameters(action, readResourceParameters);
    ...
    if (Number(readPath(action.result, "data.contentLength")) !==
...
function expectMcpCallToolAction(execution: ScenarioTurnExecution): string | undefined {
...
function expectUnsupportedMcpCloudOp(actionName: "MCP" | "MCP_SEARCH_ACTIONS" | "MCP_LIST_CONNECTIONS", op: "search_actions" | "list_connections", expectedParameters: JsonRecord): (execution: ScenarioTurnExecution) => string | undefined {
    return (execution) => {
        const action = firstAction(execution, actionName);
        if (typeof action === "string")
            return action;
        const parametersFailure = expectActionParameters(action, expectedParameters);
        if (parametersFailure)
            return parametersFailure;
        const text = `MCP op=${op} is only available in the cloud runtime.`;
...
async function readJsonBody<T extends object>(req: http.IncomingMessage, res: http.ServerResponse): Promise<T | null> {
    const chunks: Buffer[] = [];
...
async function scenarioMcpRouteHandler(req: RouteRequest, res: RouteResponse, runtime: IAgentRuntime): Promise<void> {
...
async function seedMcp(ctx: ScenarioContext): Promise<string | undefined> {
    ...
    if (!agentRuntime)
        return "scenario runtime was not available";
    ...
    runtime.getService = ((serviceType: string) => serviceType === MCP_SERVICE_NAME
        ? scenarioMcpService
    ...
    registerStrictActionRouteFixtures(runtime, strictMcpRoutes.filter((route) => route.actionName === "MCP_READ_RESOURCE" ||
        route.actionName === "MCP_CALL_TOOL"));
    for (const route of strictMcpRoutes.filter((route) => route.actionName !== "MCP_READ_RESOURCE" &&
        route.actionName !== "MCP_CALL_TOOL")) {
    ...
    runtime.scenarioModelFixtures?.register({
        ...
        match: {
            ...
            prompt: (prompt: string) => prompt.includes(RESOURCE_URI) && prompt.includes(RESOURCE_TEXT),
        },
    ...
    // model selection call is skipped entirely.
    {
        ...
        match: {
            ...
            prompt: (prompt: string) => prompt.includes("# TASK: Select the Most Appropriate Tool and Server") &&
        ...
        match: {
            modelType: ModelType.TEXT_LARGE,
            prompt: (prompt: string) => prompt.includes("# TASK: Generate Tool Arguments for Tool Execution") &&
                prompt.includes(TOOL_NAME) &&
        ...
        match: {
            modelType: ModelType.TEXT_SMALL,
            prompt: (prompt: string) => prompt.includes(`Synthesize the result from the "${TOOL_NAME}" tool`) && prompt.includes(TOOL_OUTPUT),
        },
        response: TOOL_REASONING_TEXT,
        times: 1,
    }, unsupportedMcpEvaluationFixture(parentListConnectionsInput, "list_connections"), unsupportedMcpEvaluationFixture(searchActionsInput, "search_actions"), unsupportedMcpEvaluationFixture(listConnectionsInput, "list_connections"), unsupportedMcpPostToolFixture(parentListConnectionsInput, "list_connections"), unsupportedMcpPostToolFixture(searchActionsInput, "search_actions"), unsupportedMcpPostToolFixture(listConnectionsInput, "list_connections"));
    const registered = (runtime.plugins ?? []).some((plugin) => plugin.name === mcpPlugin.name);
    if (!registered) {
    ...
    if (!server)
        return `MCP server ${MCP_SERVER_NAME} was not registered`;
...
function expectMcpStatus(status: number, body: unknown): string | undefined {
    ...
    const server = servers.find((candidate) => readPath(candidate, "name") === MCP_SERVER_NAME);
    if (!server) {
...
async function finalMcpCheck(ctx: ScenarioContext): Promise<string | undefined> {
    const runtime = (ctx.runtime as RuntimeWithMcpScenario | undefined) ?? scenarioRuntime;
    if (!runtime)
        return "scenario runtime was not available in final check";
    const service = runtime.getService<McpService>(MCP_SERVICE_NAME);
    if (!service)
        return "McpService was not registered";
    const server = service
        .getServers()
        .find((candidate: McpServer) => candidate.name === MCP_SERVER_NAME);
    if (!server)
        return `expected ${MCP_SERVER_NAME}, saw no MCP servers`;
...
export default scenario({
    ...
    turns: [
        ...
        },
        {
            ...
            assertTurn: expectUnsupportedMcpCloudOp("MCP", "list_connections", parentListConnectionsParameters),
        },
        {
            kind: "message",
            name: "MCP search-actions virtual action reports local-only boundary",
            text: searchActionsInput,
            assertTurn: expectUnsupportedMcpCloudOp("MCP_SEARCH_ACTIONS", "search_actions", searchActionsParameters),
        },
        {
            kind: "message",
            name: "MCP list-connections virtual action reports local-only boundary",
            text: listConnectionsInput,
            assertTurn: expectUnsupportedMcpCloudOp("MCP_LIST_CONNECTIONS", "list_connections", listConnectionsParameters),
        },
        {
            kind: "api",
