...
packages/testing/scenario-runner/
  bin/
    eliza-scenarios          # Bun CLI entry shim — invokes src/cli.ts
  schema/
...
```ts
// Execution
import { runScenario }       from "@elizaos/testing/scenario-runner";
import { attachInterceptor } from "@elizaos/testing/scenario-runner";
import { judgeTextWithLlm }  from "@elizaos/testing/scenario-runner";
...
import type { NativeBoundaryRow, ScenarioNativeExportManifest } from "@elizaos/testing/scenario-runner";
...
`runScenario(scenario, runtime, opts)` is the core API. The CLI calls it for each discovered file. The runtime factory (`createScenarioRuntime`) is defined in `runtime-factory.ts` and is **not** part of the package's main entry; import it from the `@elizaos/testing/scenario-runner/runtime-factory` subpath (or relatively, e.g. `./src/runtime-factory.ts`, when working inside this repo).
...
(`@elizaos/testing/scenario-runner/schema`):
...
```bash
bun run --cwd packages/testing typecheck:runner
bun run --cwd packages/testing test:runner
bun run --cwd packages/testing typecheck:runner
bun run --cwd packages/testing test:deterministic:e2e
bun run --cwd packages/testing test:live:e2e
bun run --cwd packages/testing test:pr:e2e
bun run --cwd packages/testing clean
```
...
- **Schema vs dist exports.** `ScenarioDefinition` and schema types come from `@elizaos/testing/scenario-runner/schema` (the `schema/` directory, not `dist/`). Do not import them from `@elizaos/testing/scenario-runner` directly.
...
The runner is part of the private `@elizaos/testing` workspace. Public subpaths
resolve source directly, and the `eliza-scenarios` Bun shim invokes the real CLI.
Use `test:runner` and `typecheck:runner` from `packages/testing`.
