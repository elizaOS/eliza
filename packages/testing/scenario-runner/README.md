...
bun run --cwd packages/testing eval:multi-agent-arena -- \
...
ELIZA_CHAT_VIA_CLI=codex \
ELIZA_CLI_CODEX_BIN=/absolute/path/to/codex \
bun run --cwd packages/testing eval:sales-lighthouse -- \
...
ELIZA_CHAT_VIA_CLI=codex \
ELIZA_CLI_CODEX_BIN=/absolute/path/to/codex \
bun run --cwd packages/testing eval:sales-lighthouse-autonomous -- \
...
```bash
bun run --cwd packages/testing eval:timing:merge -- \
...
```bash
bun run --cwd packages/testing eval:timing:matrix -- \
  --input=/path/to/finetune_test_dialogue.jsonl \
...
- Schema types (`ScenarioDefinition`, `CapturedAction`, etc.) come from `@elizaos/testing/scenario-runner/schema`, not from the main export.
- Scenarios starting with `_` or in directories starting with `_` are skipped by the loader.

The runner is part of the private `@elizaos/testing` workspace. Its exports and
Bun CLI resolve source directly; no standalone publication or build is required.
TypeScript loads authored `.scenario.ts` files at runtime.
