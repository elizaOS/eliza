...
function expectViewsInteract(
  ...
  // Live variant: a real model phrases its final chat reply freely, so we do
  // NOT assert exact responseText. We assert the model read the Active-View
  // block and emitted the correct structured tool call (capability + element
  // id + value); the domain side-effect is verified by the serverInteract
  // finalCheck below.
...
export default scenario({
  id: "live-active-view-agent-surface",
  lane: "live-only",
  title: "Live active-view agent-surface planner->id->interact trajectory",
