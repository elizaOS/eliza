/**
 * Validates that scenario final checks prove an observable action effect.
 *
 ...
 * this guard is a GUARD like echo-assertion-guard: the count may only go
...
describe("scenario action-effect integrity (#9310)", () => {
  it("rejects actionCalled-only scenarios", () => {
    expect(
      flagged,
      `A finalChecks array containing only actionCalled does not prove an effect. Offenders:\n${flagged.join("\n")}`,
    ).toEqual([]);
  });
});
