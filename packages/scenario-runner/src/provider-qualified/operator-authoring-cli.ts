...
export const PROVIDER_OPERATOR_HELP = `Usage:
  ...
  eliza-provider-operator export-signing-request <authoring-directory> <new-request-file>
  eliza-provider-operator import-signature <authoring-directory> <request-file> <raw-signature-file> <public-spki-file> <new-authorization.json>
  eliza-provider-operator prepare-run <authoring-directory> <new-run-directory>
  eliza-provider-operator help

Commands:
  init       Create one private 0700 directory with six 0600 files; editable starters are deliberately invalid.
  preflight  Validate file modes, closed documents, one operation, two probes, and all 13 static canary snapshots.
  export-signing-request Preflight and atomically emit exact canonical manifest bytes for an offline signer.
  import-signature Verify one detached 64-byte Ed25519 signature and pinned SPKI, then atomically write authorization.json.
...
This tool never accepts private PEM, private-key paths, credentials, tokens, or
execution flags; it never runs provider ingress or invokes a signer. The request
contains no provider credentials or private keys.
...
export async function runProviderOperatorCli(
  ...
  if (command === "export-signing-request") {
    if (!directory || directory.startsWith("--"))
      fail("export-signing-request requires <authoring-directory>");
    const [requestFile, ...extra] = rest;
    if (!requestFile || requestFile.startsWith("--") || extra.length > 0) {
      fail("export-signing-request requires exactly one <new-request-file>");
    }
    ...
    io.stdout(
      `${JSON.stringify({ status: "signing-request-exported", scenarioId: request.scenarioId, manifestSha256: request.manifestSha256, keyId: request.keyId })}\n`,
    );
    return 0;
  }
  if (command === "import-signature") {
    if (!directory || directory.startsWith("--"))
      fail("import-signature requires <authoring-directory>");
    const [
      requestFile,
      signatureFile,
      publicKeyFile,
    ...
    if (
      ...
      !publicKeyFile ||
      !authorizationFile ||
      [requestFile, signatureFile, publicKeyFile, authorizationFile].some(
        (value) => value.startsWith("--"),
      ) ||
      extra.length > 0
    ) {
      fail(
        "import-signature requires <request-file> <raw-signature-file> <public-spki-file> <new-authorization.json>",
      );
    }
    const authorization = await importProviderManifestSignature({
      authoringDirectory: directory,
      requestFile,
      signatureFile,
      publicKeyFile,
    ...
    io.stdout(
      `${JSON.stringify({ status: "authorization-imported", scenarioId: authorization.manifest.scenario.id, manifestSha256: authorization.manifest.manifestSha256, keyId: authorization.manifestSignature.keyId })}\n`,
    );
    return 0;
  }
