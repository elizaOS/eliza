...
export const REMOTE_EVIDENCE_SIGN_REQUEST_SCHEMA =
  "eliza.remote-evidence-sign-request.v2" as const;
...
function sourceSha(value: unknown, path: string): string {
  ...
  if (!SOURCE_SHA_PATTERN.test(result))
    fail(`${path} must be a 40- or 64-character source digest`);
  return result;
}
...
function correlation(
  ...
  return {
    ...
    repositorySha: sourceSha(payload.repositorySha, "payload.repositorySha"),
    deploymentSha: sourceSha(payload.deploymentSha, "payload.deploymentSha"),
...
>(
  ...
  const responseIdentity = {
    endpoint: normalized.pin.endpoint,
    organizationId: normalized.pin.organizationId,
    administrativeDomain: normalized.pin.organizationId,
    keyId: normalized.pin.keyId,
  };
  let verifiedResult: CanonicalJsonValue;
  try {
    verifiedResult = verifyProviderServiceResponse({
      value: decoded,
      pin: {
        ...responseIdentity,
        publicKeyPem: normalized.pin.publicKeyPem,
        serviceIdentitySha256: providerServiceIdentitySha256(responseIdentity),
      },
      expectedRole:
        role === "observer" ? "observer-sign" : "semantic-judge-sign",
      ...
      completedAtIso: new Date(completionNowMs).toISOString(),
    });
  } catch {
    return fail("signed response envelope is invalid");
  }
  ...
  if (envelope.keyId !== normalized.pin.keyId)
    fail("response envelope key was substituted");
  if (
    canonicalJson(
