...
function responseSignerFixture() {
  return {
    signer: signerFixture().signer,
    organizationId: "service-operator",
    administrativeDomain: "service-operator",
  };
}
...
describe("provider canary service host", () => {
  test("executes one exactly authorized controller request and durably refuses its replay", async () => {
    ...
    const host = createProviderCanaryServiceHost({
      responseSigner: responseSignerFixture(),
  ...
  test("refuses to sign a response completed after the request expiry", async () => {
    let clockReads = 0;
    const host = createProviderCanaryServiceHost({
      responseSigner: responseSignerFixture(),
      authorizer: createStaticProviderServiceRoleAuthorizer([
      ...
      now: () =>
        new Date(clockReads++ === 0 ? NOW : "2026-08-20T18:04:01.000Z"),
    });
    const response = await post(
      host,
      serviceRequest("controller-execute", {}, "T".repeat(32)),
    );
    expect(response.status).toBe(500);
  });
  ...
  test("observer signer refuses before completion and signs only its exact recorded material", async () => {
    ...
    const material = Object.freeze({
      deploymentAttestation: {
        ...
        statement: {
          ...
          subject: providerDeploymentAttestationSubject(
            providerDeploymentWorkloadSha256(CORRELATION),
          ),
          audience: providerDeploymentAttestationAudience(
            CORRELATION.scenarioId,
          ),
          runId: CORRELATION.runId,
          runNonce: "run-nonce-001",
          repositorySha: CORRELATION.repositorySha,
          deploymentSha: CORRELATION.deploymentSha,
          workloadSha256: providerDeploymentWorkloadSha256(CORRELATION),
          platformEvidenceSha256: "2".repeat(64),
          issuedAtIso: "2026-08-20T17:59:50.000Z",
          expiresAtIso: "2026-08-20T18:03:50.000Z",
        },
    ...
    const validation = vi.fn(
      async ({ expectedValidationSha256, deploymentAttestationSha256 }) => ({
    ...
    const evidence: ProviderObserverEvidencePayload = {
      ...
      repositorySha: CORRELATION.repositorySha,
      deploymentSha: CORRELATION.deploymentSha,
      rawControllerMaterialSha256: "3".repeat(64),
  ...
  test("semantic judge cannot sign verdicts other than its recorded evaluation", async () => {
    ...
    const payload: SemanticJudgeEvidencePayload = {
      ...
      deploymentSha: CORRELATION.deploymentSha,
      rawControllerMaterialSha256: "3".repeat(64),
      observerEvidenceSha256: "4".repeat(64),
      observerDeploymentAttestationSha256: "5".repeat(64),
      scenarioEndedAtIso: "2026-08-20T17:59:30.000Z",
      trajectoryVerifiedAtIso: "2026-08-20T17:59:40.000Z",
      signedAtIso: NOW,
      trajectorySetSha256: TRAJECTORY_HASH,
  ...
  test("cleanup service returns unsigned state and only the observer can validate and sign it", async () => {
    ...
    const cleanupResult = {
      ...
      runId: CORRELATION.runId,
      scenarioId: CORRELATION.scenarioId,
      operationKind: CORRELATION.operationKind,
      runNonce: "run-nonce-001",
    ...
    expect(cleanupEnvelope.payload.result).toEqual(cleanupResult);
    expect(cleanupEnvelope.payload.result).not.toHaveProperty("signature");
    ...
    const signedResponse = await post(
      ...
      serviceRequest(
        ...
        { cleanupResult: cleanupEnvelope.payload.result },
        "J".repeat(32),
      ),
    );
    ...
    expect(
      verifyProviderCleanupProof({
        proof: signedEnvelope.payload.result,
