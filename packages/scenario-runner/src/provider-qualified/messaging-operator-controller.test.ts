...
describe("messaging provider-canary operator", () => {
  it.each(Object.keys(CASES) as MessagingCanaryKind[])(
    ...
    async (kind) => {
      ...
      const pre = preflightMessagingOperatorCanary(authored);
      ...
      expect(() =>
        reverifyPortableRawControllerMaterial({
          value: {
            ...receipt,
            failureProbes: receipt.failureProbes.map((probe, index) =>
              index === 0
                ? {
                    ...probe,
                    providerStateAfterSha256: digest("tampered-state"),
                  }
                : probe,
            ),
          },
          scenarioId: pre.scenarioId,
          operationKind: pre.operation.kind,
          manifest: pre.authorization.manifest,
          providerTarget: authored.providerTarget,
          operationInput: authored.operationInput,
          failureProbeMaterials: authored.failureProbes,
          failureProbeBindings: pre.execution.failureProbeBindings,
          now: new Date(receipt.collectedAtIso),
        }),
      ).toThrow(/changed provider state/);
    },
  );
