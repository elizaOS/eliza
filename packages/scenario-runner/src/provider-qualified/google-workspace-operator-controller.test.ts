...
describe("Google Workspace provider-canary operator", () => {
  it.each(Object.keys(CASES) as GoogleWorkspaceCanaryKind[])(
    ...
    async (kind) => {
      const authored = fixture(kind);
      const ready = preflightGoogleWorkspaceOperatorCanary(authored);
          ...
          value: {
            ...
            replay: {
              ...
              binding: {
                ...
                operationSha256: digest("nested-operation-tamper"),
              },
            },
          },
          scenarioId: ready.scenarioId,
          operationKind: ready.operation.kind,
          manifest: ready.authorization.manifest,
          providerTarget: authored.providerTarget,
          operationInput: authored.operationInput,
          failureProbeMaterials: authored.failureProbes,
          failureProbeBindings: ready.execution.failureProbeBindings,
          now: new Date(receipt.collectedAtIso),
        }),
      ).toThrow(/operationSha256 mismatch/);
    },
  );
