...
function harness() {
  ...
  const deploymentAttestationIssuerPair = generateKeyPairSync("ed25519");
  ...
  const deploymentAttestationIssuerPublicKeyPem =
    deploymentAttestationIssuerPair.publicKey.export({
      type: "spki",
      format: "pem",
    });
  ...
  const deploymentAttestation = {
    ...
    statement: {
      ...
      workloadSha256,
      platformEvidenceSha256: hash("platform-native evidence"),
      issuedAtIso: "2026-08-20T17:01:08.000Z",
      expiresAtIso: "2026-08-20T17:04:08.000Z",
    },
    signature: "pending",
  } as const;
  const signedDeploymentAttestation = {
    ...deploymentAttestation,
    signature: signPayload(
      null,
      providerDeploymentAttestationSigningBytes(
        deploymentAttestation.statement,
      ),
      deploymentAttestationIssuerPair.privateKey,
    ).toString("base64url"),
  };
  const observerPayload: ProviderObserverEvidencePayload = {
    ...
    deploymentAttestation: signedDeploymentAttestation,
  ...
  it("publishes one genuinely qualified and reverified artifact after cleanup", async () => {
    ...
    expect(() =>
      reverifyProviderQualificationArtifact(
        result.artifact,
        createTestReleaseTrustPolicyForArtifact(result.artifact),
      ),
    ).not.toThrow();
  });
