/**
 * Binds one portable v5 qualification artifact to a separately signed cleanup
...
import {
  authorizeProviderCleanupSignerForRelease,
...
export const PROVIDER_QUALIFICATION_PUBLICATION_SCHEMA =
  "eliza.provider-qualification-publication.v2" as const;
...
function reverifyCore(
  ...
  if (
    ...
    publication.observerDeploymentAttestationSha256 !==
...
export function reverifyProviderQualificationPublication(
  ...
  return reverifyCore(publication, releaseTrustPolicy);
}
  ...
  const core = canonicalJsonValue(
    {
      ...
      observerDeploymentAttestationSha256:
        input.artifact.observerDeploymentAttestationSha256,
