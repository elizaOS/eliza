...
import {
  ...
  validateProviderDeploymentAttestationEnvelope,
...
export const SEMANTIC_JUDGE_EVIDENCE_SCHEMA =
  "eliza.provider-qualified-semantic-evidence.v2" as const;

/** Independently signed statement binding the run to the inspected workload. */
export type ProviderDeploymentWorkloadAttestation =
...
export interface ProviderObserverEvidencePayload {
  ...
  deploymentAttestation: ProviderDeploymentWorkloadAttestation;
...
export interface DeriveProviderQualificationInput {
  ...
  /** External verifier roots; absence cannot be inferred from observer pins. */
...
const SEMANTIC_PAYLOAD_KEYS = [
  ...
  "observerDeploymentAttestationSha256",
  "scenarioEndedAtIso",
  "trajectoryVerifiedAtIso",
  "signedAtIso",
  "trajectorySetSha256",
...
function runtimeSourceSha(value: unknown, path: string): string {
  const sha = runtimeString(value, path);
  if (!SOURCE_SHA_PATTERN.test(sha)) {
    throw new Error(
      `${path} must be a lowercase 40- or 64-character source digest`,
    );
  }
  return sha;
}

function validateDeploymentAttestation(
  value: unknown,
  _path: string,
): ProviderDeploymentWorkloadAttestation {
  return validateProviderDeploymentAttestationEnvelope(value);
}
...
function parseSignedEvidenceRuntime(
  ...
  runtimeSourceSha(
    payload.repositorySha,
    "signedEvidence.payload.repositorySha",
  );
  runtimeSourceSha(
    payload.deploymentSha,
    "signedEvidence.payload.deploymentSha",
  );
  validateDeploymentAttestation(
    payload.deploymentAttestation,
    "signedEvidence.payload.deploymentAttestation",
  );
  for (const key of [
    "scenarioStartedAtIso",
    "scenarioEndedAtIso",
...
function parseSemanticEvidenceRuntime(
  ...
  runtimeSourceSha(
    payload.repositorySha,
    "signedSemanticEvidence.payload.repositorySha",
  );
  runtimeSourceSha(
    payload.deploymentSha,
    "signedSemanticEvidence.payload.deploymentSha",
  );
  for (const key of [
    "scenarioEndedAtIso",
    "trajectoryVerifiedAtIso",
...
/** Stable digest the semantic judge must bind for the observer's workload proof. */
export function providerDeploymentAttestationSha256(
  attestation: ProviderDeploymentWorkloadAttestation,
): string {
  return canonicalSha256(
    validateDeploymentAttestation(
      canonicalJsonValue(attestation, "providerDeploymentAttestation"),
...
/** Exact public workload identity covered by an observer attestation. */
export function providerDeploymentWorkloadSha256(input: {
  repositorySha: string;
  deploymentSha: string;
}): string {
  runtimeSourceSha(
    input.repositorySha,
    "providerDeploymentWorkload.repositorySha",
  );
  runtimeSourceSha(
    input.deploymentSha,
    "providerDeploymentWorkload.deploymentSha",
  );
  return canonicalSha256(
    {
...
export function deriveProviderQualification(
  ...
  const pinnedDeploymentAttestationIssuerPublicKeysPem = canonicalJsonValue(
    inputDataProperty(
      rawInput,
  ...
  const deploymentAttestationIssuerKeyIds = pinnedKeyIds(
  ...
  if (
    [...deploymentAttestationIssuerKeyIds].some(
      (keyId) =>
        observerKeyIds.has(keyId) ||
        semanticKeyIds.has(keyId) ||
        authorityKeyIds.has(keyId),
    )
  ) {
    reasons.push("deployment-attestation:issuer-not-independent");
  }
  if (
  ...
  let verifiedDeploymentAttestation:
    | ReturnType<typeof verifyProviderDeploymentAttestation>
    | undefined;
  try {
    verifiedDeploymentAttestation = verifyProviderDeploymentAttestation({
      envelope: payload.deploymentAttestation,
      trustedIssuerPublicKeysPem:
        input.pinnedDeploymentAttestationIssuerPublicKeysPem,
      expected: {
        ...
        workloadSha256: providerDeploymentWorkloadSha256(manifest.run),
      },
      nowIso: input.nowIso,
      maxLifetimeMs: maxSignatureAgeMs,
      maxClockSkewMs: clockSkewMs,
    });
  } catch {
    // error-policy:J3 untrusted attestation failures become an explicit reason.
    reasons.push("deployment-attestation:invalid-or-untrusted");
  }
  if (
    ...
    payload.repositorySha !== input.manifest.run.repositorySha ||
    payload.deploymentSha !== input.manifest.run.deploymentSha
  ) {
    reasons.push("observer-correlation:mismatch");
  }
  const observerEvidenceSha256 = canonicalSha256(
    signedEvidence,
    "signedObserverEvidence",
  );
  const observerDeploymentAttestationSha256 =
    providerDeploymentAttestationSha256(payload.deploymentAttestation);
  if (
    ...
    semanticPayload.repositorySha !== input.manifest.run.repositorySha ||
    semanticPayload.deploymentSha !== input.manifest.run.deploymentSha ||
    semanticPayload.rawControllerMaterialSha256 !==
      payload.rawControllerMaterialSha256 ||
    semanticPayload.observerEvidenceSha256 !== observerEvidenceSha256 ||
    semanticPayload.observerDeploymentAttestationSha256 !==
      observerDeploymentAttestationSha256 ||
  ...
  const scenarioEndedAt = timestamp(
    payload.scenarioEndedAtIso,
    "signedEvidence.payload.scenarioEndedAtIso",
  );
  if (
    verifiedDeploymentAttestation !== undefined &&
    (timestamp(
      verifiedDeploymentAttestation.issuedAtIso,
      "deploymentAttestation.issuedAtIso",
    ) <
      scenarioEndedAt - clockSkewMs ||
      timestamp(
        verifiedDeploymentAttestation.issuedAtIso,
        "deploymentAttestation.issuedAtIso",
      ) >
        signedAt + clockSkewMs)
  ) {
    reasons.push("deployment-attestation:stale-or-out-of-order");
  }
  if (
