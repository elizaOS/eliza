...
function deploymentStatement(key: KeyObject) {
  return {
    issuerKeyId: pin(key).keyId,
...
function artifact(
  ...
  const [authorityKey, observerKey, attestationIssuerKey, judgeKey] = trustKeys;
  if (!authorityKey || !observerKey || !attestationIssuerKey || !judgeKey) {
    throw new Error("artifact fixture requires four trust keys");
  }
  ...
  const core = {
    ...
    rawControllerMaterialSha256: "2".repeat(64),
    observerDeploymentAttestationSha256: "3".repeat(64),
    ...
    reverification: {
      ...
      publicKeyPins: {
        ...
        deploymentAttestationIssuers: [pin(attestationIssuerKey)],
      ...
      signedObserverEvidence: {
        payload: {
          ...
          deploymentAttestation: {
            statement: deploymentStatement(attestationIssuerKey),
          },
        },
      },
...
describe("provider qualification publication capsule", () => {
  ...
  const attestationIssuer = generateKeyPairSync("ed25519");
    ...
    const core = {
      ...
      organizations: {
        manifestAuthority: {
          ...
          keys: [pin(authority.publicKey)],
        },
        providerObserver: {
          organizationId: "observer.example",
          keys: [pin(observer.publicKey)],
          allowedWorkloadSha256s: ["4".repeat(64)],
          allowedStatementSha256s: [
            canonicalSha256(
              deploymentStatement(attestationIssuer.publicKey),
        ...
        deploymentAttestationIssuer: {
          ...
          keys: [pin(attestationIssuer.publicKey)],
        },
        semanticJudge: {
          organizationId: "judge.example",
          keys: [pin(judge.publicKey)],
        },
        cleanup: {
          organizationId: "observer.example",
          keys: [pin(observer.publicKey)],
  ...
  it("rejects missing or accessor-backed capsule fields", () => {
    ...
    expect(() =>
      reverifyProviderQualificationPublication(accessor, releaseTrustPolicy()),
    ).toThrow(/cleanupProof must not be an accessor property/);
  });
  ...
  it("rejects tampered raw material and the wrong signer pin", () => {
    ...
    expect(() =>
      reverifyProviderQualificationPublication(rawTamper, releaseTrustPolicy()),
    ...
    ).toThrow(/cleanup signer is not authorized/);
  });
