...
export const PROVIDER_QUALIFICATION_VERIFY_CONFIG_SCHEMA =
  "eliza.provider-qualification-verify-config.v4" as const;
export const PROVIDER_QUALIFICATION_CATALOG_CONFIG_SCHEMA =
  "eliza.provider-qualification-catalog-config.v4" as const;
...
export function parseProviderQualificationVerifyConfig(
  ...
  return {
    ...
    deploymentAttestationIssuerPublicKeyFiles: stringArray(
    ...
    releaseTrustPolicyFile: requiredString(
...
export async function verifyProviderQualificationFromConfig(
  ...
  const releaseTrustPolicy = validateProviderQualificationReleaseTrustPolicy(
    readJson(
      resolveFrom(baseDir, config.releaseTrustPolicyFile),
  ...
  const artifact = assembleProviderQualificationArtifact({
    ...
    pinnedDeploymentAttestationIssuerPublicKeysPem: readPublicKeys(
      baseDir,
...
export function reverifyProviderQualificationArtifactFile(
  ...
  reverifyProviderQualificationArtifact(
    artifact,
    validateProviderQualificationReleaseTrustPolicy(
      readJson(path.resolve(releaseTrustPolicyFile), "release trust policy"),
    ),
  );
  return artifact;
}
...
export async function runProviderQualificationCli(
  ...
  if (
    ...
    process.stderr.write(
      "usage: eliza-provider-qualification <verify|catalog> <config.json>\n       eliza-provider-qualification <reverify|reverify-publication> <capsule.json> <release-trust-policy.json>\n",
    );
    return 2;
  }
  if (argv[0] === "catalog") {
  ...
  if (argv[0] === "reverify") {
    ...
    const artifact = reverifyProviderQualificationArtifactFile(
      argv[1],
      argv[2],
    );
  ...
  if (argv[0] === "reverify-publication") {
    if (argv.length !== 3) return 2;
    const policy = validateProviderQualificationReleaseTrustPolicy(
      readJson(path.resolve(argv[2]), "release trust policy"),
    );
    const publication = reverifyProviderQualificationPublicationFile(
      argv[1],
      argv[2],
    );
    process.stdout.write(
      renderProviderQualificationPublicationMarkdown(publication, policy),
    );
    return 0;
  }
  if (argv.length !== 2) return 2;
