...
import providerCanaryDefinitions from "../../schema/provider-canary-definitions.json" with {
  type: "json",
};
import type {
...
import { createTestReleaseTrustPolicyForArtifact } from "./release-trust-policy-test-fixtures.ts";
...
function scenario(): ScenarioDefinition {
  const definition = providerCanaryDefinitions.scenarios.find(
    ({ id }) => id === "provider.google-calendar.create",
  );
  if (definition === undefined) {
    throw new Error("canonical Google Calendar provider canary is missing");
  }
...
function bindings(
  ...
  const accountRefSha256 = hash("operator-google-calendar-canary-account");
  ...
  return {
    ...
    trust: {
      ...
      observerSigners: [
        {
          observerId: "google-calendar-provider-observer",
          keyId: observerKeyId,
        },
      ],
    },
...
function fixture(): DeriveProviderQualificationInput & {
  ...
  deploymentAttestationIssuerPrivateKey: KeyObject;
  ...
  const deploymentAttestationIssuerKeyPair = generateKeyPairSync("ed25519");
  ...
  const deploymentAttestationIssuerPublicKeyPem =
    deploymentAttestationIssuerKeyPair.publicKey.export({
      type: "spki",
      format: "pem",
    });
  ...
  const observation: ProviderEffectObservation = {
    ...
    source: {
      ...
      accountRefSha256: hash("operator-google-calendar-canary-account"),
    },
  ...
  const workloadSha256 = providerDeploymentWorkloadSha256(manifest.run);
  const deploymentAttestation: SignedProviderDeploymentAttestation = {
    schema: PROVIDER_DEPLOYMENT_ATTESTATION_ENVELOPE_SCHEMA,
    statement: {
      schema: PROVIDER_DEPLOYMENT_ATTESTATION_STATEMENT_SCHEMA,
      issuerKeyId: providerObserverKeyId(
        deploymentAttestationIssuerPublicKeyPem,
      ),
      subject: providerDeploymentAttestationSubject(workloadSha256),
      audience: providerDeploymentAttestationAudience(manifest.scenario.id),
      runId: manifest.run.runId,
      runNonce: manifest.run.nonce,
      repositorySha: manifest.run.repositorySha,
      deploymentSha: manifest.run.deploymentSha,
      workloadSha256,
      platformEvidenceSha256: hash("platform-native attestation evidence"),
      issuedAtIso: "2026-05-23T00:01:08.000Z",
      expiresAtIso: "2026-05-23T00:04:08.000Z",
    },
    signature: "pending",
  };
  deploymentAttestation.signature = signPayload(
    null,
    providerDeploymentAttestationSigningBytes(deploymentAttestation.statement),
    deploymentAttestationIssuerKeyPair.privateKey,
  ).toString("base64url");
  const payload: ProviderObserverEvidencePayload = {
    ...
    repositorySha: manifest.run.repositorySha,
    deploymentSha: manifest.run.deploymentSha,
    rawControllerMaterialSha256: hash("raw controller material"),
  ...
  const semanticPayload: SemanticJudgeEvidencePayload = {
    ...
    observerDeploymentAttestationSha256: canonicalSha256(
  ...
  return {
    ...
    deploymentAttestationIssuerPrivateKey:
      deploymentAttestationIssuerKeyPair.privateKey,
...
function resignDeploymentAttestation(
  input: DeriveProviderQualificationInput & {
    deploymentAttestationIssuerPrivateKey: KeyObject;
  },
): void {
  input.signedEvidence.payload.deploymentAttestation.signature = signPayload(
    null,
    providerDeploymentAttestationSigningBytes(
      input.signedEvidence.payload.deploymentAttestation.statement,
    ),
    input.deploymentAttestationIssuerPrivateKey,
  ).toString("base64url");
}
...
function resignSemantic(
  ...
  input.signedSemanticEvidence.payload.repositorySha =
    input.signedEvidence.payload.repositorySha;
  input.signedSemanticEvidence.payload.deploymentSha =
    input.signedEvidence.payload.deploymentSha;
  input.signedSemanticEvidence.payload.rawControllerMaterialSha256 =
    input.signedEvidence.payload.rawControllerMaterialSha256;
  input.signedSemanticEvidence.payload.observerEvidenceSha256 = canonicalSha256(
    input.signedEvidence,
    "signedObserverEvidence",
  );
  input.signedSemanticEvidence.payload.observerDeploymentAttestationSha256 =
    canonicalSha256(
      input.signedEvidence.payload.deploymentAttestation,
      "providerDeploymentAttestation",
    );
...
describe("deriveProviderQualification", () => {
  ...
  it("rejects recomputed source/deployment evidence for a different manifest workload", () => {
    const input = fixture();
    input.signedEvidence.payload.repositorySha = "9".repeat(40);
    input.signedEvidence.payload.deploymentAttestation.statement.repositorySha =
      "9".repeat(40);
    input.signedEvidence.payload.deploymentAttestation.statement.workloadSha256 =
      providerDeploymentWorkloadSha256({
        repositorySha: "9".repeat(40),
        deploymentSha: input.signedEvidence.payload.deploymentSha,
      });
    resignDeploymentAttestation(input);
    resignProvider(input);
    resignSemantic(input);
  ...
  it("rejects a re-signed raw controller digest not bound by semantic evidence", () => {
    const input = fixture();
    input.signedEvidence.payload.rawControllerMaterialSha256 = hash(
      "substituted raw controller material",
    );
    resignProvider(input);

    expect(deriveProviderQualification(input).qualification).toMatchObject({
      status: "unqualified",
      reasons: expect.arrayContaining(["semantic-correlation:mismatch"]),
    });
  });

  it.each(["issuerKeyId", "workloadSha256"] as const)(
    "rejects a recomputed deployment attestation with a substituted %s",
    (field) => {
      const input = fixture();
      input.signedEvidence.payload.deploymentAttestation.statement[field] =
        hash(`substituted:${field}`);
      resignDeploymentAttestation(input);
      resignProvider(input);
      resignSemantic(input);
      ...
      expect(deriveProviderQualification(input).qualification).toMatchObject({
        status: "unqualified",
        reasons: expect.arrayContaining([
          "deployment-attestation:invalid-or-untrusted",
        ]),
      });
    },
  );

  it("rejects a recomputed semantic envelope with another observer envelope digest", () => {
    const input = fixture();
    input.signedSemanticEvidence.payload.observerEvidenceSha256 = hash(
      "another observer envelope",
    );
    input.signedSemanticEvidence.signature = signPayload(
      null,
      semanticEvidenceSigningBytes(input.signedSemanticEvidence.payload),
  ...
  it("emits a transcript-free capsule that independently reverifies", () => {
    ...
    const artifact = assembleProviderQualificationArtifact({
      ...
      pinnedDeploymentAttestationIssuerPublicKeysPem:
        input.pinnedDeploymentAttestationIssuerPublicKeysPem,
    ...
    const releaseTrustPolicy =
      createTestReleaseTrustPolicyForArtifact(artifact);
    expect(
      reverifyProviderQualificationArtifact(artifact, releaseTrustPolicy),
    ).toEqual(artifact.decision);
    ...
    expect(() =>
      reverifyProviderQualificationArtifact(tampered, releaseTrustPolicy),
    ).toThrow(/decision does not reverify/);

    const substitutedPolicy = structuredClone(releaseTrustPolicy);
    const substituteKey = generateKeyPairSync("ed25519").publicKey.export({
      type: "spki",
      format: "pem",
    });
    substitutedPolicy.organizations.providerObserver.keys = [
      {
        keyId: providerObserverKeyId(substituteKey),
        algorithm: "ed25519",
        spkiPem: substituteKey,
      },
    ];
    const { policySha256: _policyDigest, ...policyCore } = substitutedPolicy;
    substitutedPolicy.policySha256 = canonicalSha256(
      policyCore,
      "providerQualificationReleaseTrustPolicy",
    );
    expect(() =>
      reverifyProviderQualificationArtifact(artifact, substitutedPolicy),
    ).toThrow(/not authorized by the external policy/);

    const bogusWorkloadPolicy = structuredClone(releaseTrustPolicy);
    bogusWorkloadPolicy.organizations.providerObserver.allowedStatementSha256s =
      [hash("different externally authorized workload statement")];
    const {
      policySha256: _bogusWorkloadPolicyDigest,
      ...bogusWorkloadPolicyCore
    } = bogusWorkloadPolicy;
    bogusWorkloadPolicy.policySha256 = canonicalSha256(
      bogusWorkloadPolicyCore,
      "providerQualificationReleaseTrustPolicy",
    );
    expect(() =>
      reverifyProviderQualificationArtifact(artifact, bogusWorkloadPolicy),
    ).toThrow(/workload attestation is not authorized by the external policy/);

    const weakened = structuredClone(artifact);
    weakened.reverification.scenarioDefinition = {
      ...providerCanaryDefinitions.scenarios[0],
      title: "Weakened same-ID canary",
    } as ScenarioDefinition;
    const { artifactSha256: _weakenedDigest, ...weakenedCore } = weakened;
    weakened.artifactSha256 = canonicalSha256(
      weakenedCore,
      "providerQualificationArtifact",
    );
    expect(() =>
      reverifyProviderQualificationArtifact(weakened, releaseTrustPolicy),
    ).toThrow(/differs from the repository-owned canonical definition/);
  });
});
