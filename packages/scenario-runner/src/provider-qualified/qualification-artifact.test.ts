...
function verifyConfig(): Record<string, unknown> {
  return {
    ...
    deploymentAttestationIssuerPublicKeyFiles: ["attestation-issuer.pem"],
  ...
  it("rejects a runner report bound to another scenario before qualification", () => {
    ...
    expect(() =>
      assembleProviderQualificationArtifact({
        ...
        pinnedDeploymentAttestationIssuerPublicKeysPem: ["unused"],
    ...
    const artifact = {
      ...
      reverification: {
        publicKeyPins: {
          ...
          deploymentAttestationIssuers: [{ keyId: HASH }],
