...
export const REFERENCE_OPERATOR_CONFIG_SCHEMA =
  "eliza.provider-canary-reference-operator-config.v4" as const;
export const REFERENCE_OPERATOR_SECRET_REQUEST_SCHEMA =
  "eliza.provider-canary-secret-request.v2" as const;
export const REFERENCE_OPERATOR_SERVICE_REQUEST_SCHEMA =
  "eliza.provider-canary-service-request.v3" as const;
...
interface AuthenticatedServiceConfig {
  ...
  responseServiceIdentitySha256: string;
}
...
export interface ReferenceOperatorConfig {
  ...
  secretBrokerAdministrativeDomain: string;
  secretBrokerOrganizationId: string;
  secretBrokerPublicKeyPem: string;
  secretBrokerKeyId: string;
  secretBrokerServiceIdentitySha256: string;
...
export interface ReferenceOperatorSecretResolver {
  resolve(input: {
    ...
    responsePin: ProviderServiceResponsePin;
...
function requireSourceSha(value: unknown, path: string): string {
  const candidate = boundedString(value, path, 64);
  if (!SOURCE_SHA_PATTERN.test(candidate))
...
function serviceConfig(
  ...
  exactKeys(
    ...
    [
      ...
      "responseKeyId",
      "responseServiceIdentitySha256",
    ],
    path,
  );
  const endpoint = httpsEndpoint(record.endpoint, `${path}.endpoint`);
  const administrativeDomain = boundedString(
    record.administrativeDomain,
    `${path}.administrativeDomain`,
    256,
  );
  const responseOrganizationId = boundedString(
    record.responseOrganizationId,
    `${path}.responseOrganizationId`,
    256,
  );
  if (responseOrganizationId !== administrativeDomain)
    fail(`${path} response organization must own its administrative domain`);
  const responsePublicKeyPem = ed25519PublicKey(
    record.responsePublicKeyPem,
    `${path}.responsePublicKeyPem`,
  );
  const responseKeyId = requireHash(
    record.responseKeyId,
    `${path}.responseKeyId`,
  );
  if (providerObserverKeyId(responsePublicKeyPem) !== responseKeyId)
    fail(`${path}.responseKeyId does not match responsePublicKeyPem`);
  const responseServiceIdentitySha256 = requireHash(
    record.responseServiceIdentitySha256,
    `${path}.responseServiceIdentitySha256`,
  );
  if (
    responseServiceIdentitySha256 !==
    providerServiceIdentitySha256({
    ...
    fail(`${path} response identity does not match its exact endpoint`);
  ...
  const base = serviceConfig(
    {
      ...
      responseOrganizationId: record.responseOrganizationId,
      responsePublicKeyPem: record.responsePublicKeyPem,
      responseKeyId: record.responseKeyId,
      responseServiceIdentitySha256: record.responseServiceIdentitySha256,
    },
    path,
  );
...
function deploymentConfig(
  ...
  const observerAttestationStatementSha256 = requireHash(
    record.observerAttestationStatementSha256,
    `${path}.observerAttestationStatementSha256`,
  );
  ...
  const cleanup = serviceConfig(record.cleanup, `${path}.cleanup`);
  ...
  const responseKeys = [controller, observer, semanticJudge, cleanup].map(
    (service) => service.responseKeyId,
  );
  if (new Set(responseKeys).size !== responseKeys.length)
    fail(`${path} service response keys must be distinct`);
  ...
  const pinnedDeploymentAttestationIssuerPublicKeysPem = publicKeys(
    record.pinnedDeploymentAttestationIssuerPublicKeysPem,
    `${path}.pinnedDeploymentAttestationIssuerPublicKeysPem`,
  );
  if (
...
export function parseReferenceOperatorConfig(
  ...
  exactKeys(
    ...
    [
      ...
      "secretBrokerAdministrativeDomain",
      "secretBrokerOrganizationId",
      "secretBrokerPublicKeyPem",
      "secretBrokerKeyId",
      "secretBrokerServiceIdentitySha256",
      "deployments",
    ],
    "config",
  );
  ...
  const secretBrokerEndpoint = httpsEndpoint(
    record.secretBrokerEndpoint,
    "config.secretBrokerEndpoint",
  );
  const secretBrokerAdministrativeDomain = boundedString(
    record.secretBrokerAdministrativeDomain,
    "config.secretBrokerAdministrativeDomain",
    256,
  );
  const secretBrokerOrganizationId = boundedString(
    record.secretBrokerOrganizationId,
    "config.secretBrokerOrganizationId",
    256,
  );
  if (secretBrokerAdministrativeDomain !== secretBrokerOrganizationId)
    fail("secret broker organization must own its administrative domain");
  const secretBrokerPublicKeyPem = ed25519PublicKey(
    record.secretBrokerPublicKeyPem,
    "config.secretBrokerPublicKeyPem",
  );
  const secretBrokerKeyId = requireHash(
    record.secretBrokerKeyId,
    "config.secretBrokerKeyId",
  );
  if (providerObserverKeyId(secretBrokerPublicKeyPem) !== secretBrokerKeyId)
    fail("config.secretBrokerKeyId does not match secretBrokerPublicKeyPem");
  const secretBrokerServiceIdentitySha256 = requireHash(
    record.secretBrokerServiceIdentitySha256,
    "config.secretBrokerServiceIdentitySha256",
  );
  if (
    secretBrokerServiceIdentitySha256 !==
    providerServiceIdentitySha256({
      endpoint: secretBrokerEndpoint,
      organizationId: secretBrokerOrganizationId,
      administrativeDomain: secretBrokerAdministrativeDomain,
      keyId: secretBrokerKeyId,
    })
  )
    fail("secret broker response identity does not match its exact endpoint");
  for (const deployment of Object.values(parsed)) {
    const services = [
    ...
    if (
      services.some(
        (service) =>
          service.administrativeDomain === secretBrokerAdministrativeDomain ||
          endpointOrigin(service.endpoint) ===
            endpointOrigin(secretBrokerEndpoint) ||
          service.responseKeyId === secretBrokerKeyId,
      )
    )
      fail(
        "secret broker administration, origin, and response key must be independent",
      );
  }
  return Object.freeze({
  ...
  return Object.freeze({
    async resolve({
      ...
      const expiresAtIso = new Date(
        Date.parse(requestedAtIso) + 60_000,
      ).toISOString();
      const body = {
      ...
      const requestSha256 = createHash("sha256")
        .update(canonicalJson(canonicalJsonValue(body, "secretRequest")))
        .digest("hex");
      ...
      const verified = verifyProviderServiceResponse({
        value: decoded,
        pin: responsePin,
        expectedRole: "secret-resolve",
      ...
      const values = plainRecord(verified, "secretBrokerResponse.values");
...
function validateFactoryInput(
  ...
  return Object.freeze({
    ...
    repositorySha: requireSourceSha(
      record.repositorySha,
      "factoryInput.repositorySha",
    ),
    deploymentSha: requireSourceSha(
      record.deploymentSha,
      "factoryInput.deploymentSha",
    ),
  });
}
...
function serviceCaller(input: {
  ...
  return async (payload) => {
    ...
    const request: ServiceRequest = {
      ...
      repositorySha: input.factory.repositorySha,
      deploymentSha: input.factory.deploymentSha,
    ...
    return verifyProviderServiceResponse({
      value: decoded,
      pin: {
        endpoint: input.config.endpoint,
        organizationId: input.config.responseOrganizationId,
        administrativeDomain: input.config.administrativeDomain,
        publicKeyPem: input.config.responsePublicKeyPem,
        keyId: input.config.responseKeyId,
        serviceIdentitySha256: input.config.responseServiceIdentitySha256,
      },
      expectedRole: input.role,
      expectedRequestNonce: request.requestNonce,
      expectedRequestSha256: requestSha256,
      requestedAtIso,
      expiresAtIso,
      completedAtIso: new Date().toISOString(),
...
export async function createReferenceExternalProviderCanaryCapabilities(
  ...
  const resolvedSecrets = await (
    ...
    responsePin: {
      endpoint: config.secretBrokerEndpoint,
      organizationId: config.secretBrokerOrganizationId,
      administrativeDomain: config.secretBrokerAdministrativeDomain,
      publicKeyPem: config.secretBrokerPublicKeyPem,
      keyId: config.secretBrokerKeyId,
      serviceIdentitySha256: config.secretBrokerServiceIdentitySha256,
    },
  });
  ...
  const observerCleanupSign = serviceCaller({
    factory,
    config: deployment.observer,
    bearerToken: token(secrets, deployment.observer),
    fetchImpl,
    role: "observer-cleanup-sign",
  });
  ...
  const cleanup: RemoteProviderCleanupClient = Object.freeze({
    ...
    cleanupAndSign: async (
      ...
      const cleanupResult = await cleanupCall(input);
      return (await observerCleanupSign({
        cleanupResult,
