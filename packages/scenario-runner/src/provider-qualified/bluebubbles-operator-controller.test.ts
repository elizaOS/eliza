...
describe("BlueBubbles provider-canary operator", () => {
  ...
  it("returns only correlated unsigned source receipts", async () => {
    ...
    const preflight = preflightBlueBubblesOperatorCanary(authored);
    ...
    expect(() =>
      reverifyPortableRawControllerMaterial({
        value: {
          ...result,
          readback: { ...result.readback, chatGuid: "iMessage;-;substituted" },
        },
        ...
        now: new Date(result.collectedAtIso),
      }),
    ).toThrow(/readback does not match/);
  });
