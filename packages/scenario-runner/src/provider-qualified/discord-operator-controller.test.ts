...
import {
  reverifyPortableRawControllerMaterial,
  reverifyVerifiedDeployedCanaryExecution,
} from "./portable-raw-receipt-reverifier.ts";
...
describe("Discord provider-canary operator", () => {
  ...
  it("executes a complete deployed seam and verifies real trajectory bytes", async () => {
    ...
    const preflight = preflightDiscordOperatorCanary(authored);
    ...
    const portableInput = {
      ...
      operationKind: preflight.execution.targetBinding.kind,
      manifest: preflight.authorization.manifest,
      providerTarget: authored.providerTarget,
      operationInput: authored.operationInput,
      failureProbeMaterials: authored.failureProbes,
      failureProbeBindings: preflight.execution.failureProbeBindings,
    ...
    expect(() =>
      reverifyVerifiedDeployedCanaryExecution({
        ...
        value: {
          ...result,
          failureProbes: result.failureProbes.map((probe, index) =>
            index === 0 ? { ...probe, providerEffectCountAfter: 2 } : probe,
          ),
        },
      }),
    ).toThrow(/changed provider effect count/);
    const providerReadback = {
      schema: "eliza.discord-provider-canary-raw-readback.v1" as const,
      collectedAtIso: new Date(now + 50_000).toISOString(),
      channelId,
      observerIdentity: {
        userId: botId,
        bot: true as const,
        rawResponseSha256: hash("discord-observer-identity"),
      },
      humanIngress: {
        messageId: "123456789012345678",
        channelId,
        guildId,
        author: { id: humanId, bot: false },
        timestamp: new Date(now + 1_000).toISOString(),
        content: humanIngressContent,
        contentSha256: hash(humanIngressContent),
      },
      providerEffect: {
        ...
        content: providerEffectContent,
        contentSha256: hash(providerEffectContent),
      },
      qualificationClaimed: false as const,
    };
    const composite = {
      schema:
        "eliza.provider-canary-deployed-composite-raw-material.v1" as const,
      scenarioId: preflight.scenarioId,
      operationKind: "discord.message-send" as const,
      controllerFamily: "discord" as const,
      deployedExecution: result,
      providerReadback,
      qualificationClaimed: false as const,
    };
    expect(
      ...
      reverifyPortableRawControllerMaterial({
        ...
        value: {
          ...
          providerReadback: {
            ...providerReadback,
            providerEffect: {
              ...providerReadback.providerEffect,
              content: "substituted effect",
            },
          },
        },
      }),
    ).toThrow(/providerEffect.*invalid|signed message/);
