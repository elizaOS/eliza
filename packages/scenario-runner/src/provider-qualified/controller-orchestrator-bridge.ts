...
function validateControllerRun(
  ...
  const raw = reverifyPortableRawControllerMaterial({
    value: value.rawControllerMaterial,
    scenarioId: contract.scenarioId,
    operationKind: contract.operationKind,
    manifest: context.preflight.authorization.manifest,
    providerTarget: context.providerTarget,
    operationInput: context.operationInput,
    failureProbeMaterials: context.failureProbes,
    failureProbeBindings: context.preflight.failureProbeBindings,
    now,
  }) as BridgeableRawControllerMaterial;
...
function assertEvidenceCorrelation(
  ...
  for (const payload of payloads) {
    if (
      ...
      payload.repositorySha !== correlation.repositorySha ||
      payload.deploymentSha !== correlation.deploymentSha ||
      payload.rawControllerMaterialSha256 !== rawControllerMaterialSha256 ||
  ...
  if (
    observer.payload.deploymentAttestation.statement.repositorySha !==
      correlation.repositorySha ||
    observer.payload.deploymentAttestation.statement.deploymentSha !==
      correlation.deploymentSha ||
    observer.payload.deploymentAttestation.statement.workloadSha256 !==
      providerDeploymentWorkloadSha256(correlation) ||
    (semantic !== undefined &&
      (semantic.payload.observerEvidenceSha256 !==
        canonicalSha256(observer, "providerBridge.observerEvidence") ||
        semantic.payload.observerDeploymentAttestationSha256 !==
          providerDeploymentAttestationSha256(
            observer.payload.deploymentAttestation,
          )))
  ) {
    fail("remote signer returned cross-deployment observer material");
  }
}
...
function buildObserverPayload(input: {
  ...
  return Object.freeze({
    ...
    repositorySha: input.correlation.repositorySha,
    deploymentSha: input.correlation.deploymentSha,
    rawControllerMaterialSha256: input.run.rawControllerMaterialSha256,
...
function buildSemanticPayload(input: {
  ...
  return Object.freeze({
    ...
    observerDeploymentAttestationSha256: providerDeploymentAttestationSha256(
      input.observerEvidence.payload.deploymentAttestation,
    ),
    scenarioEndedAtIso: input.run.trajectories.scenarioEndedAtIso,
    trajectoryVerifiedAtIso: input.run.trajectories.verifiedAtIso,
    signedAtIso: input.signedAtIso,
        ...
        return {
          async complete({
            ...
            assertEvidenceCorrelation(
              activeCorrelation,
              run.trajectories,
              run.rawControllerMaterialSha256,
              evidence,
              undefined,
            );
            return evidence;
      ...
      async judge({ context, ingress, trajectories, observerEvidence }) {
        ...
        const verdicts = await input.semanticJudge.evaluate({
          ...
          observerDeploymentAttestationSha256:
            providerDeploymentAttestationSha256(
              observerEvidence.payload.deploymentAttestation,
            ),
        });
