...
export type ExternalProviderCanaryProgressStage =
  | "observer-starting"
  | "observer-started"
  | "ingress-starting"
  | "ingress-completed"
...
export interface ExecuteExternalProviderCanaryInput {
  ...
  onProgress?: (
    stage: ExternalProviderCanaryProgressStage,
  ) => void | Promise<void>;
}
...
export async function executeExternalProviderCanary(
  ...
  try {
    await input.onProgress?.("observer-starting");
    ...
    await input.onProgress?.("observer-started");

    await input.onProgress?.("ingress-starting");
    const ingress = await input.capabilities.ingress.execute(context);
    completedStages.push("ingress-completed");
    await input.onProgress?.("ingress-completed");
    ...
    validateProviderQualificationArtifact(artifact);
    ...
    await input.onProgress?.("proofs-completed");
  } catch (error) {
  ...
  try {
    ...
    await input.onProgress?.("cleanup-completed");
  } catch (cleanupError) {
