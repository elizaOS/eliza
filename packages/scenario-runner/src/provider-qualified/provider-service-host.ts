...
function sourceSha(value: unknown): string {
  const candidate = boundedString(value, 64);
  if (!SOURCE_SHA_PATTERN.test(candidate)) refuse(400);
  return candidate;
}
...
function correlationFromService(
  ...
  return Object.freeze({
    ...
    repositorySha: sourceSha(request.repositorySha),
    deploymentSha: sourceSha(request.deploymentSha),
...
function validateGrant(input: {
  ...
  if (input.request.requestedSecretRefs) {
    if (
      ...
      grant.repositorySha !== undefined ||
      grant.deploymentSha !== undefined ||
...
async function signedSuccess(input: {
  ...
  const signer = signerSnapshot(input.options.responseSigner.signer);
  const identity = {
    endpoint: input.endpoint,
    organizationId: boundedString(
      input.options.responseSigner.organizationId,
      256,
    ),
    administrativeDomain: boundedString(
      input.options.responseSigner.administrativeDomain,
      256,
    ),
    keyId: signer.keyId,
  };
  try {
    const respondedAtMs = (input.options.now ?? (() => new Date()))().getTime();
    if (!Number.isFinite(respondedAtMs)) refuse(500);
    return success(
      await signProviderServiceResponse({
        pin: {
          ...identity,
        ...
        requestNonce: input.requestNonce,
        requestSha256: input.requestSha256,
        respondedAtIso: new Date(respondedAtMs).toISOString(),
        expiresAtIso: input.expiresAtIso,
        result: input.result,
      }),
    );
  } catch {
    // error-policy:J1 response-signing failure becomes a fixed transport refusal.
    return refuse(500);
  }
}
  ...
  return canonicalJsonValue(
    {
      ...
      deploymentAttestation: payload.deploymentAttestation,
    },
...
function parseRemoteSignRequest(value: unknown): RemoteSignRequest {
  ...
  sourceSha(request.repositorySha);
  sourceSha(request.deploymentSha);
...
function signCorrelation(
  ...
  if (
    ...
    payload.deploymentSha !== request.deploymentSha ||
    payload.rawControllerMaterialSha256 !==
      request.rawControllerMaterialSha256 ||
...
async function handleRemoteSign(input: {
  ...
  if (role === "observer-sign") {
    ...
    const observerPayload = payload as ProviderObserverEvidencePayload;
    if (
      observerPayload.deploymentAttestation.statement.workloadSha256 !==
        providerDeploymentWorkloadSha256(correlation) ||
      observerPayload.deploymentAttestation.statement.repositorySha !==
        correlation.repositorySha ||
      observerPayload.deploymentAttestation.statement.deploymentSha !==
    ...
    if (
      ...
      canonicalJson(material) !==
        canonicalJson(evidenceProjection(observerPayload))
    )
      refuse(403);
    ...
    const deploymentAttestationSha256 = providerDeploymentAttestationSha256(
    ...
    if (
      validation.validationSha256 !== expectedValidationSha256 ||
      validation.deploymentAttestationSha256 !== deploymentAttestationSha256
    )
      refuse(403);
    const bytes = providerEvidenceSigningBytes(observerPayload);
    const signature = await signAndVerify({
      signer,
    ...
    return success(
      await signProviderServiceResponse({
        pin: {
          ...
          organizationId:
            request.role === "observer"
              ? (input.options.observer?.organizationId ?? "")
              : (input.options.semanticJudge?.organizationId ?? ""),
          administrativeDomain:
            request.role === "observer"
              ? (input.options.observer?.organizationId ?? "")
              : (input.options.semanticJudge?.organizationId ?? ""),
          keyId: signer.keyId,
          serviceIdentitySha256: providerServiceIdentitySha256({
            endpoint: input.endpoint,
            organizationId:
              request.role === "observer"
                ? (input.options.observer?.organizationId ?? "")
...
async function handleSecret(input: {
  ...
  return signedSuccess({
    ...
    requestNonce,
    requestSha256: digest,
    expiresAtIso: boundedString(request.expiresAtIso, 64),
    result: values,
  });
}
...
export function createProviderCanaryServiceHost(
  ...
  signerSnapshot(options.responseSigner.signer);
  boundedString(options.responseSigner.organizationId, 256);
  boundedString(options.responseSigner.administrativeDomain, 256);
      ...
      const match = snapshot.find((entry) => {
        ...
        if (request.correlation) {
          return (
            ...
            entry.policy.repositorySha === request.correlation.repositorySha &&
            entry.policy.deploymentSha === request.correlation.deploymentSha &&
