...
- build/create/make a website/web page/app/site, or update/edit/fix/redeploy a named existing app/site when the user wants it opened, previewed, hosted, or live -> contexts=["code","general"], candidateActions=["APP"]. APP action=create owns the verified create-or-edit lifecycle and final link; never replace it with raw TASKS/SPAWN_AGENT, which has no app registration, verification bridge, or launch-link contract.
- imperative code/repo/file changes that are not an app lifecycle request ("edit this Python file", "fix the parser", "add a test") -> code (SPAWN_AGENT / TASKS spawn_agent); NOT tasks/automation/settings/scheduled. This is hands-on coding work, not a scheduled task or a focus block.
