...
test("Railpack runtime resolves workspace dependencies and serves health without the checkout", async () => {
  ...
  try {
    ...
    for (const name of ["login", "core", "common"]) {
    ...
    const recipe = JSON.parse(
      await readFile(join(build, "packages/login/railpack.json"), "utf8"),
    );
    ...
    server = Bun.spawn(
      [
        ...
        "packages/login/src/server/embedded.ts",
      ],
      {
        cwd: runtime,
        env: {
    ...
    expect(health).toEqual({ status: "ok", name: "@elizaos/login" });
    server.kill("SIGTERM");
