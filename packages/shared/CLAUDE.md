...
packages/shared/
  src/
    ...
    config/
      types.ts                   Master ElizaConfig shape (200+ config keys)
      types.eliza.ts             Re-export used to avoid agent↔shared ESM cycle
    ...
    utils/
      ...
      eliza-root.ts              elizaRoot() state-dir resolver
...
**Config types** — `ElizaConfig` and 200+ sub-types from `src/config/types.ts` (exported individually via `export type { AgentConfig, … }`).
...
**Add a new config key:** edit `src/config/types.ts` (the main config shape). Update the Zod schema in `src/config/zod-schema.agent-runtime.ts` if runtime validation is required. Export via the existing `export type { … }` block in `src/index.ts`.
...
- **Cycle guard in `src/config/config.ts` and `src/config/env-vars.ts`.** Both files are stubs with comments explaining that the real exports were moved to avoid `agent ↔ shared` ESM cycles. If you add imports from `@elizaos/agent` here you will break the bench-server boot.
