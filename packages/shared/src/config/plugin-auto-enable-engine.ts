// Plugin auto-enable engine. Lives in @elizaos/shared so it is reachable from
// both the agent runtime and other shared callers without inverting the
// dependency direction (shared must never import from agent).
//
// Two pieces of behaviour that depended on agent-specific helpers in earlier
// versions are now injected via `ApplyPluginAutoEnableParams`:
//   - `subscriptionProviderMap` — maps a subscription-provider id (e.g.
//     "anthropic-subscription") to the provider plugin short id (e.g.
//     "anthropic"). Provided by agent because the subscription model is an
//     agent-runtime concept.
//   - `evmAutoEnableReason` — returns a string reason when plugin-wallet
//     should be auto-enabled because EVM signing is available, else null.
//     Provided by agent because the resolver depends on agent-runtime services.
//
// Plugins that adopt the declarative `Plugin.autoEnable` field on their plugin
// object can be auto-enabled without any central map entry — see
// `applyPluginSelfDeclaredAutoEnable`. Migrating each plugin to that path is
// the long-term direction; the maps below remain as a fallback while that
...
export interface ApplyPluginAutoEnableParams {
  ...
  /**
   * Maps subscription-provider ids → provider plugin short ids. Injected by
   * agent since the subscription model is agent-specific. Omit (or pass an
   * empty record) to disable subscription-based auto-enable.
   */
  subscriptionProviderMap?: Readonly<Record<string, string>>;
  /**
   * Returns a non-empty reason string when plugin-wallet should be
   * auto-enabled because an EVM signing path is available. Omit to skip the
   * EVM auto-enable check entirely (Solana key + cloud-Steward fallbacks
   * still apply).
   */
  evmAutoEnableReason?: (env: NodeJS.ProcessEnv) => string | null;
}
...
export const CONNECTOR_PLUGINS: Record<string, string> = {
  ...
  // App-local connector (not on npm) — kept here so we don't need a separate
  // shared/agent wrapper just to inject one entry.
...
export const AUTH_PROVIDER_PLUGINS: Record<string, string> = {
  ...
  // NOTE: EVM signing auto-enable is NOT via this map — see the
  // `evmAutoEnableReason` injected callback on ApplyPluginAutoEnableParams.
...
export function applyPluginAutoEnable(
  ...
  const { config, env, subscriptionProviderMap, evmAutoEnableReason } = params;
  ...
  const subscriptionPluginId =
    subscriptionIsRuntimeApplicable && subscriptionProviderMap
      ? subscriptionProviderMap[subscriptionProvider as string]
      : undefined;
  ...
  const evmReason = evmAutoEnableReason ? evmAutoEnableReason(env) : null;
  ...
  if (env.ELIZA_AGENT_WALLET_AUTO_ENABLE !== "0") {
    ...
    const agentWalletReason =
      evmReason ?? solanaKeyReason ?? cloudStewardReason;
