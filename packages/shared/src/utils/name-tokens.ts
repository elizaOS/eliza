...
// `{{agentName}}` substitution, `$`-safe replacer function) and re-exported
// here so existing `@elizaos/shared` consumers keep their import path. The core
// symbol is exported from both the node and browser barrels, so this re-export
// resolves in browser bundles too.
