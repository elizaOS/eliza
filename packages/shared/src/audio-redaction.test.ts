...
describe("matchPiiSpansToWords", () => {
  ...
  it("matches Mandarin and Japanese PII without Latin-token assumptions", () => {
    const words: TranscriptWord[] = [
      { text: "我的", startMs: 0, endMs: 200 },
      { text: "名字", startMs: 200, endMs: 400 },
      { text: "是王小明", startMs: 400, endMs: 800 },
      { text: "です", startMs: 800, endMs: 1000 },
      { text: "電話番号", startMs: 1000, endMs: 1300 },
      { text: "090-1234-5678", startMs: 1300, endMs: 1800 },
    ];
    const { matches, unmatched } = matchPiiSpansToWords(words, [
      { text: "王 小明", label: "PERSON" },
      { text: "090 1234 5678", label: "PHONE" },
    ]);
    expect(unmatched).toEqual([]);
    expect(matches).toEqual([
      { startMs: 400, endMs: 800, labels: ["PERSON"] },
      { startMs: 1300, endMs: 1800, labels: ["PHONE"] },
    ]);
  });
...
describe("mergeRedactionSpans", () => {
  ...
  it("rejects windows entirely past the end of the audio", () => {
    expect(() =>
