...
// these (issue #15976).
const EXPECTED_BUNDLE_SLUGS: Record<string, string> = {
  "eliza-1-2b": "e2b",
  "eliza-1-4b": "e4b",
  "eliza-1-9b": "12b",
  "eliza-1-27b": "31b",
...
describe("Eliza-1 runtime quant metadata", () => {
  ...
  it("keeps stable ids but exposes requested size-cased display names", () => {
    for (const id of ELIZA_1_TIER_IDS) {
      ...
      expect(entry?.ggufFile).toContain(tierBundleSlug(id));
    }
  });

  it("separates bundle directories from manifest-backed published tiers", () => {
    expect(ELIZA_1_BUNDLE_SLUGS).toEqual(EXPECTED_BUNDLE_SLUGS);
    expect(ELIZA_1_PUBLISHED_TIER_IDS).toEqual(["eliza-1-2b", "eliza-1-4b"]);
    expect(ELIZA_1_PUBLISHED_SLUGS).toEqual({
      "eliza-1-2b": "e2b",
      "eliza-1-4b": "e4b",
    });
    for (const id of ELIZA_1_TIER_IDS) {
      expect(isEliza1TierPublished(id)).toBe(
        ELIZA_1_PUBLISHED_TIER_IDS.includes(
          id as (typeof ELIZA_1_PUBLISHED_TIER_IDS)[number],
        ),
      );
      expect(tierBundleSlug(id)).toBe(EXPECTED_BUNDLE_SLUGS[id]);
      const entry = MODEL_CATALOG.find((model) => model.id === id);
      expect(entry?.hfPathPrefix).toBe(`bundles/${EXPECTED_BUNDLE_SLUGS[id]}`);
  ...
  it("points every voice-enabled tier at the bundled Silero VAD GGUF", () => {
    for (const id of ELIZA_1_TIER_IDS) {
      ...
      expect(entry?.sourceModel?.components.vad?.file).toBe(
        `bundles/${tierBundleSlug(id)}/vad/silero-vad-v5.gguf`,
      );
    }
  });
