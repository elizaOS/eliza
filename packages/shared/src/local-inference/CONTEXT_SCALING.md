...
| `eliza-1-27b`†    | (aspirational)    |     16‡ |        4 |      256 |                       128k → 256k (ext) | ~16.8 GB (Q4_K_M)   |
...
- **0.8B / 2B on a 16-24 GB GPU are wildly under-utilizing the device.**
  ...
  f16). Even at f16 KV the 0.8B's 32k window costs 3.5 GiB — fits 16 GB four
...
- 0.8B / 2B: the binding limit is the base model's `max_position_embeddings`
  ...
  0.8B, ~0.93 GiB for 2B. (Past 40960 needs RoPE extension at build time;
...
| 96 GB+ / GH200                                  | 27B-256k | qjl+polarq4 / TCQ trellis (`turbo3_tcq` for the K-cache at 256k) | 262144                | 9B drafter, mlock on                    | 8                                       | host RAM headroom |
