---
title: Published Project Domains
description: Configure system and custom domains for projects published on Eliza Cloud.
---

# Published Project Domains

Every published project receives a system URL and can add custom domains with
automatic SSL and DNS management. Domain settings belong to the project's bound
Cloud app record; the stable API paths therefore remain `/api/v1/apps/*`.

## Overview

Domain management in elizaOS Cloud supports:

- **Managed frontend URL**: Automatic
  `<slug>.sites.elizacloud.ai` URL for the active static deployment
- **Container URL**: `*.apps.elizacloud.ai` for eligible backend containers
...
- **Multi-tenant Routing**: Route traffic to the correct published project based on domain

## Quick Start

### Dashboard

Open **Projects**, select a project, then choose **Publish → Domains**. Domain
controls appear only when Eliza Cloud is connected and the project has been
published.

### API

```bash
# Add a custom domain to the published project's Cloud record
...
<Steps>
<Step title="Project publication">

When you publish a project, it automatically receives a subdomain like
`myapp.sites.elizacloud.ai` for its managed frontend. A separately deployed
container can also receive an `apps.elizacloud.ai` backend URL.
...
Requests are routed to the published project based on the incoming domain.
...
Every published project automatically gets a subdomain under
`sites.elizacloud.ai` when its managed frontend is active:

```
https://myapp.sites.elizacloud.ai
```
...
Refresh all domain statuses for a published project:
...
Remove a custom domain from a published project:
...
<Warning>
  After removing a domain, users will no longer be able to access the published
  project through that URL. The subdomain at `*.apps.elizacloud.ai` remains
  active.
</Warning>
...
1. Add both `example.com` and `www.example.com` to the published project
...
If the same published project is accessible through both its subdomain and a
custom domain:
...
- Existing domain conflicts with other published projects
...
<CardGroup>
  <CardGroup.Card title="Publishing Projects" href="/cloud/apps">
    Publish and configure projects
  </CardGroup.Card>
