...
| `bun run dev:macos` | Same stack with the macOS assistant experience: one tray-owned bottom pill; Workspace opens only on request | Assistant/pill workflow |
| `bun run dev:macos:watch` | macOS assistant experience + Vite dev server/HMR | macOS pill and shared ChatOverlay workflow |
| `bun run build:macos` | Production Electrobun package with the normal full-window Workspace experience | Local packaged macOS app candidate |
| `bun run build:macos:assistant` | Production Electrobun package with the opt-in macOS assistant/pill experience | Local packaged assistant experiment |
...
| Variable | Purpose |
|----------|---------|
| `ELIZA_DESKTOP_EXPERIENCE` | Runtime startup contract: `workspace` (ordinary full-window startup and the packaged macOS default) or `macos-assistant` (the explicit tray-owned assistant/pill experiment). This is intentionally separate from `ELIZA_DESKTOP_PROFILE`, which controls packaging contents. |
