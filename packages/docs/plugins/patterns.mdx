...
The bootstrap plugin is built into `@elizaos/core` and automatically registered during runtime initialization. It provides core functionality through:

```typescript
// The bootstrap plugin is created internally using createBootstrapPlugin()
// It provides:
...
<Note>
You don't need to import or configure the bootstrap plugin - it's automatically included when you create an AgentRuntime.
</Note>
