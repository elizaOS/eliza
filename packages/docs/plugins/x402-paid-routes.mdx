...
- **`X402_FACILITATOR_URL`** — optional facilitator base URL for standard `PAYMENT-SIGNATURE` flows. The agent posts to **`/verify`** and **`/settle`** under this URL. If unset, the default is Eliza’s hosted x402 API base (`https://x402.elizaos.ai/api/v1/x402`).
...
```typescript
import { registerX402Config } from "@elizaos/agent/middleware/x402";
