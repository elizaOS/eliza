...
`ELIZA_CONFIG_PATH` takes precedence over `ELIZA_STATE_DIR`. Both support `~` expansion for the home directory. For backwards compatibility, the legacy names `ELIZA_STATE_DIR` and `ELIZA_CONFIG_PATH` are also accepted as fallbacks.
