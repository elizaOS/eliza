...
PGLite 将数据存储在本地目录中。Eliza 在启动时固定数据目录：
...
Eliza 包含一个可配置的向量内存搜索系统。配置位于 `agents.defaults.memorySearch` 下或在 `agents.list[n].memorySearch` 中按代理配置：
