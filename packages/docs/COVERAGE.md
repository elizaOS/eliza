...
| `packages/feed/`                 | Feed      | `tracks/training/feed.mdx`                                           |
| `packages/benchmarks/`           | Runtime   | `tracks/training/benchmarks.mdx`                                     |
| `packages/vault/`                | Runtime   | `guides/wallet.md`, `guides/platform-secure-store.md`                |

Hardware and embodiment research — the Eliza E1 chip, the robot stack, and the Alberta RL framework — moved to the [elizaOS/research](https://github.com/elizaOS/research) repository (mounted here as the `packages/research/` submodule). Its docs live in that repo, so it has no page on this docs site.
