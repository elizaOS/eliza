...
You already have Shakespeare in `src/character.ts` from the previous guide. Now let's add another agent to our project so they can interact. Create a fresh character file for Hemingway by copying the generated character file:

```bash Terminal
mkdir -p src/agents
cp src/character.ts src/agents/hemingway.ts
```
