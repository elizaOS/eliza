...
export interface FeedTrustScenario {
  id: string;
  name: string;
  description: string;
...
export interface FeedTrustBenchmarkResult {
  scenario: FeedTrustScenario;
...
function validateTrustScenario(scenario: FeedTrustScenario): string[] {
  const issues: string[] = [];
...
export class TrustScenarioLoader {
  ...
  private readonly cache = new Map<string, FeedTrustScenario>();
...
export class FeedTrustBenchmark {
  ...
  static async runScenario(
    ...
    scenarioId: TrustScenarioId | string
  ): Promise<FeedTrustBenchmarkResult> {
