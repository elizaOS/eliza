...
For the current Feed stack, raw VRAM is not the only constraint. The RL path benefits from running vLLM and the trainer at the same time, so split-GPU configurations are often better than the biggest single card.
...
```bash
cd /Users/shawwalters/feed-workspace/feed/packages/training/deploy/docker
...
```bash
cd /Users/shawwalters/feed-workspace/feed/packages/training/deploy/runpod
...
- `/workspace/feed-output/pipeline_report.json`
- `/workspace/feed-output/training_manifest.json`
- `/workspace/feed-output/served_eval.json`
- `/workspace/feed-output/scambench_results.json`
- `/workspace/feed-output/adapters/`
- `/workspace/feed-output/materialized_models/`
