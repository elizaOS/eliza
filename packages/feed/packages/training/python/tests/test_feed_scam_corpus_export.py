...
SCRIPT_PATH = PYTHON_ROOT / "scripts" / "export_feed_scam_corpus.py"

if not SCRIPT_PATH.exists():
    pytest.skip(f"script not found: {SCRIPT_PATH.name}", allow_module_level=True)

SPEC = importlib.util.spec_from_file_location("export_feed_scam_corpus", SCRIPT_PATH)
assert SPEC and SPEC.loader
module = importlib.util.module_from_spec(SPEC)
sys.modules["export_feed_scam_corpus"] = module
SPEC.loader.exec_module(module)


def test_export_feed_rows_to_canonical_corpus(tmp_path: Path) -> None:
...
def test_export_feed_rows_skips_invalid_metadata_and_steps(tmp_path: Path) -> None:
    source_dir = tmp_path / "source"
    source_dir.mkdir()
...
def test_export_feed_rows_derives_benign_defaults_and_normalizes_judge_scores(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "source"
    source_dir.mkdir()
