#!/usr/bin/env python3
"""
Export Feed trajectory data into the canonical scam-defense corpus contract.
"""
...
def resolved_available_actions(
    ...
    *,
    ...
    return action_catalog_for_key(
        f"feed::{trajectory_id}::{step_index}",
...
def build_training_row(
    ...
    row = {
        "record_id": (
            f"feed::{trajectory_id or 'unknown-trajectory'}::{step_index}::{call_index}"
        ),
        "group_id": scenario_id or "feed-group",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Export Feed trajectories into the canonical scam-defense corpus."
    )
    parser.add_argument(
        "--source-dir", required=True, help="Directory containing Feed trajectory exports."
    )
    parser.add_argument(
