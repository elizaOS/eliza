/**
 * Local Babylon Agent - Works with local A2A server and anvil
 *
...
async function runAgent() {
  console.log('');
  console.log('🤖 Babylon Local Agent Starting...');
  console.log('================================');
  ...
  const baseUrl = process.env.BABYLON_API_URL || 'http://localhost:3001';
  const tickInterval = parseInt(process.env.TICK_INTERVAL || '5000');
  const agentName = process.env.AGENT_NAME || 'Demo Agent';
  const agentDescription =
    process.env.AGENT_DESCRIPTION || 'Autonomous Babylon agent';
