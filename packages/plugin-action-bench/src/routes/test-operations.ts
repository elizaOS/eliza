...
export const createChannelRoute: Route = {
  ...
  handler: async (req: any, res: any, runtime: IAgentRuntime) => {
    ...
    try {
      const { testType, clientId, channelId: predeterminedChannelId, baseUrl }: CreateChannelRequest = req.body || {};
      
      console.log('📦 Create Channel Request:', { testType, clientId, predeterminedChannelId, baseUrl });
      ...
      console.log('🔗 Creating DM channel for test:', testType, predeterminedChannelId ? `with predetermined ID: ${predeterminedChannelId}` : 'with auto-generated ID');
      const channelId = await createTestChannel(userId, agentId, testType, baseUrl, predeterminedChannelId as UUID);
...
export const testRoute: Route = {
  ...
  handler: async (req: any, res: any, runtime: IAgentRuntime) => {
    ...
    try {
      ...
      const testResult = await executeActionBenchmarkTest(runtime, testType, userId, testChannelId as UUID, baseUrl);
 ...
 */
async function createTestChannel(userId: UUID, agentId: UUID, testType: string, baseUrl?: string, predeterminedChannelId?: UUID): Promise<UUID> {
  try {
    ...
    console.log('📦 Channel payload:', predeterminedChannelId ? 
      `Using predetermined ID: ${predeterminedChannelId}` : 'Auto-generating ID', channelPayload);
...
async function executeActionBenchmarkTest(
  ...
  console.log(`🚀 Starting action benchmark test: ${testDefinition.name} (${testDefinition.steps.length} steps)`);
  ...
  return new Promise<TestResult>((resolve, reject) => {
    ...
    const sendStepMessage = async (message: string, channelId: UUID, clientId: UUID, baseUrl: string) => {
      try {
        ...
        socket.on('messageBroadcast', async (data: any) => {
          try {
            const { senderId, senderName, text, roomId: messageChannelId, actions, rawMessage } = data;
            ...
            console.log('📨 Agent message received:', { senderId, text, actions, actionStatus: rawMessage?.actionStatus });
            ...
            if (isStepComplete) {
              ...
              // Move to next step
              currentStepIndex++;
              console.log(`➡️ Moving to step ${currentStepIndex + 1} of ${testDefinition.steps.length}`);
...
function extractActionsFromMessage(messageData: any): string[] {
  ...
  if (messageData.actions && Array.isArray(messageData.actions)) {
    return messageData.actions.map((action: any) => {
