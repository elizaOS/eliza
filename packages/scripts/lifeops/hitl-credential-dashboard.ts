 ...
 * ~/.eliza/.env, env-layers.ts); every field shows its
 ...
 * Run: node packages/scripts/lifeops/hitl-credential-dashboard.ts (add --open to launch the macOS browser).
 */
...
import { HOME_ENV_PATH, loadLayeredEnv, writeSecret } from "./env-layers.ts";
import {
...
async function handleSiweLogin(body) {
  ...
  const run = await runCommand(
    "bun",
    ["packages/scripts/cloud/siwe-test-login.ts", "--json"],
    {
      timeoutMs: 90_000,
      env: childEnv,
    },
