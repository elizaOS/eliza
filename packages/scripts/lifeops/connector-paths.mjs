...
export const CONNECTOR_PATHS = [
  ...
  definePath({
    ...
    oneClick: {
      type: "siwe",
      detail:
        "bun packages/scripts/cloud/siwe-test-login.mjs [--json] — SIWE nonce/sign/verify returns an apiKey (Bearer); PRIVATE_KEY pins the wallet, SIWE_BASE overrides the API base",
    },
    availability: { type: "always" },
    notes:
  ...
  definePath({
    ...
    availability: {
      ...
      reason:
        "no Discord user-OAuth app configured (the in-tree exchange at packages/app/src/api/auth/discord-exchange.ts is Activity-iframe-only)",
    },
  }),
