 ...
 * ownerVars/agentVars.
 *
 * probeId values reference credential-probes.ts families plus exactly two
...
import { PROBE_FAMILIES } from "./credential-probes.ts";
...
/** Probe ids valid on paths beyond the credential-probes.ts family sweep. */
...
export const CONNECTOR_PATHS = [
  ...
  definePath({
    ...
    oneClick: {
      type: "siwe",
      detail:
        "bun packages/scripts/cloud/siwe-test-login.ts [--json] — SIWE nonce/sign/verify returns an apiKey (Bearer); PRIVATE_KEY pins the wallet, SIWE_BASE overrides the API base",
    },
    availability: { type: "always" },
    notes:
