 ...
 *
 * Usage:
 *   bun packages/scripts/ensure-plugin-test-conventions.ts     # apply to all plugins
 *   bun packages/scripts/ensure-plugin-test-conventions.ts --dry-run   # print what would change
 *   bun packages/scripts/ensure-plugin-test-conventions.ts --check     # exit 1 if any would change (CI)
 */
...
import { GUARDED_REAL_LIVE_SUITES } from "./lib/real-live-suites.ts";
...
const REGISTERED_NATIVE_RUNNER_ROOTS = new Map([
  [
    ...
    [
      "__tests__",
      "package test invokes packages/scripts/plugins/plugin-workflow/run-isolated-tests.ts, whose recursive discovery root is __tests__",
    ],
  ],
]);
 ...
 * packages/scripts/lib/script-test-inventory.ts.
 */
  ...
  if (orphans.length > 0) {
    console.error(
      `[ensure-plugin-test-conventions] ${orphans.length} orphaned plugin test file(s): no registered test lane reaches them after effective excludes.\n` +
        ...
        "\nRegister the owning Vitest/native runner, remove the orphaning exclude, or add a dated, reasoned entry to ORPHANED_PLUGIN_TEST_EXCEPTIONS in packages/scripts/ensure-plugin-test-conventions.ts.",
    );
    return false;
  }
  return true;
