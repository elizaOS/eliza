...
function collectFailingTestFiles(line: string, files: Set<string>) {
...
export interface CapturedTestOutput {
  failingTestFiles: Set<string>;
  omittedChars: number;
  pendingLines: Map<string, string>;
  retained: string;
}

export function createCapturedTestOutput(): CapturedTestOutput {
  return {
...
export function appendCapturedTestOutput(
  capture: CapturedTestOutput,
  chunk: string | Buffer,
  source = "combined",
) {
...
export function retainedCapturedTestOutput(capture: CapturedTestOutput) {
  return capture.retained;
}

export function formatCapturedTestOutput(
  capture: CapturedTestOutput,
  label: string,
) {
