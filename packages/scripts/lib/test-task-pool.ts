...
import { parseCanonicalInt } from "./cli-numbers.ts";
 ...
 * @returns {boolean}
 */
export function isParallelSafeTask({
  scriptName,
  lane,
  packageName,
}: {
...
export function partitionTasks<
  T extends { scriptName: string; packageName?: string },
>(tasks: T[], lane: string) {
  const parallel = [];
  const serial = [];
  ...
  async function drain() {
    while (true) {
      ...
      try {
        ...
        // error-policy:J4: retain each worker failure for the caller to report.
 ...
 */
export function parseShardSpec(spec: string | undefined | null) {
  if (!spec) {
    return null;
  }
 ...
 * @returns {boolean}
 */
export function taskBelongsToShard(
  taskKey: string,
  shardCfg: { index: number; total: number } | null,
) {
  if (!shardCfg) {
    return true;
  }
