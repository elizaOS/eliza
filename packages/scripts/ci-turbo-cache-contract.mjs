...
export function runContract(repoRoot = DEFAULT_REPO_ROOT) {
  ...
  assert(
    /name:\s*Setup Bun[\s\S]*?HOME:\s*\$\{\{\s*runner\.temp\s*\}\}\/bun-home-\$\{\{\s*github\.run_id\s*\}\}-\$\{\{\s*github\.run_attempt\s*\}\}-\$\{\{\s*github\.job\s*\}\}-\$\{\{\s*runner\.name\s*\}\}[\s\S]*?bun-version:/.test(
      workspaceSetup,
    ),
    `${SETUP_WORKSPACE_PATH}: setup-bun HOME must be isolated by run, attempt, job, and runner`,
  );
