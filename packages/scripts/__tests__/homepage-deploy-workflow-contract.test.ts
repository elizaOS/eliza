...
function assertHomepageDeployContract(source: string): void {
  ...
  invariant(
    triggers.workflow_call?.inputs?.source_sha?.required === true &&
      triggers.workflow_call.inputs.source_sha.type === "string",
    "Release calls must provide the exact source commit.",
  );
  invariant(
  ...
  invariant(
    resolveScript.includes('[ "$source_sha" = "$EXPECTED_SOURCE_SHA" ]'),
    "Release tags must resolve to the caller's exact source commit.",
  );
  invariant(
    resolveScript.includes(
    ...
    "Resolved source commits must belong to protected develop history.",
  );
  invariant(
  ...
  invariant(
    verify.run?.includes('[ "$actual_digest" = "$expected_digest" ]') &&
      ...
      verify.run.includes('if [ "$SOURCE_REF" != "$AUTHORITY_REF" ]; then') &&
      verify.run.includes(
        '[ "$(resolve_remote_oid "$remote" "$SOURCE_REF")" = "$SOURCE_REF_OID" ]',
      ),
    "Deployment must reject artifact mismatch, divergent source history, and moved release tags.",
  );
  invariant(
    !verify.run.includes(
      '[ "$(resolve_remote_oid "$remote" "$AUTHORITY_REF")" = "$AUTHORITY_REF_OID" ]',
    ),
    "Unrelated develop commits must not starve an otherwise valid deployment.",
  );
...
describe("homepage deploy workflow contract", () => {
  ...
  test("rejects removal of the post-build source movement check", () => {
    const mutated = replaceRequired(
      workflowSource,
      'verify_develop_ancestry "$SOURCE_SHA" "$current_authority_oid"',
      ": # ancestry verification removed",
    );
    expect(() => assertHomepageDeployContract(mutated)).toThrow();
  });

  test("rejects removal of the caller source SHA binding", () => {
    const mutated = replaceRequired(
      workflowSource,
