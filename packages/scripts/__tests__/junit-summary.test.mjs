/**
 * Exercises semantic JUnit reconciliation against Bun and Vitest document shapes.
 */
...
describe("parseJunitSummary", () => {
  ...
  test("rejects declared counts that do not match testcase results", () => {
    expect(() =>
      parseJunitSummary(bunNested.replace('skipped="1">', 'skipped="0">')),
    ).toThrow("does not match nested skipped=1");
  });

  test("rejects entity-bearing documents before parsing testcase content", () => {
    expect(() =>
      parseJunitSummary(
  ...
  test("rejects testcase elements smuggled outside a suite", () => {
    expect(() =>
      parseJunitSummary(
