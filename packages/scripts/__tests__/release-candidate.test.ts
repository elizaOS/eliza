...
describe("immutable release candidate", () => {
  ...
  test("rewrites workspace ranges into packed manifests and restores the source tree", () => {
    const fixture = makeFixture();
    const aManifestPath = path.join(
      fixture.repoRoot,
    ...
    const { plan } = createCandidate(fixture, "candidate-workspace");
    const aRecord = plan.packages.find(
      ({ name }) => name === "@release-fixture/a",
    );
    expect(aRecord?.internalDependencies).toContainEqual({
      name: "@release-fixture/b",
    ...
    const tarballPath = path.join(
      fixture.base,
      "candidate-workspace",
      aRecord!.tarball.path,
    );
    const extractDirectory = path.join(fixture.base, "extract-a");
    fs.mkdirSync(extractDirectory);
    execFileSync("tar", ["-xzf", tarballPath, "-C", extractDirectory]);
    const packedManifest = JSON.parse(
      fs.readFileSync(
        path.join(extractDirectory, "package/package.json"),
        "utf8",
      ),
    );
    expect(packedManifest.dependencies["@release-fixture/b"]).toBe("1.2.3");

    const restoredSource = fs.readFileSync(aManifestPath, "utf8");
    expect(JSON.parse(restoredSource).dependencies["@release-fixture/b"]).toBe(
      "workspace:*",
    );
    expect(
