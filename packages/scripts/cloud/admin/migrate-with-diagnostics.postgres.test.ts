...
const CATALOG_GUARD_CREATED_AT = 1_785_614_400_001;
...
describe.skipIf(!ENABLED)(
  ...
  () => {
    ...
    test("applies the append-only fix-forward once and passes the reusable catalog preflight", async () => {
      ...
      expect(first.output).toContain("pending migrations: 4");
    ...
    test("accepts historical production hash drift but enforces hashes from the checkpoint forward", async () => {
      ...
      expect(migrated.output).toContain("pending migrations: 4");
      ...
      expect(checkpointMismatch.output).toContain(
        "Migration ledger hash mismatch for 0188_job_execution_interruptions_catalog_guard",
      );
      await database.client.end();
    }, 30_000);
