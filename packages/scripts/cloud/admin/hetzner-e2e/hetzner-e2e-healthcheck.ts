...
async function main(): Promise<void> {
  ...
  if (!agentId) {
    throw new Error("state file missing agent_id; deploy-agent step must run first");
  }

  const response = await fetch(`${baseUrl}/api/v1/eliza/agents/${agentId}/bridge`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`Healthcheck HTTP ${response.status}: ${text.slice(0, 300)}`);
  }
  const body = JSON.parse(text) as { result?: { ready?: boolean }; error?: unknown };
  if (body.error) {
    throw new Error(`Healthcheck JSON-RPC error: ${JSON.stringify(body.error).slice(0, 300)}`);
  }
  if (body.result?.ready !== true) {
    throw new Error(`Healthcheck not ready: ${JSON.stringify(body.result).slice(0, 300)}`);
  }
