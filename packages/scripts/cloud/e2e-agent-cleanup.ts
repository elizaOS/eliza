 ...
 *
 * Usage:
 *   bun packages/scripts/cloud/e2e-agent-cleanup.ts                    # dry run vs https://api.eliza.app
 *   bun packages/scripts/cloud/e2e-agent-cleanup.ts --report /tmp/cleanup-dry-run.json
 *   bun packages/scripts/cloud/e2e-agent-cleanup.ts --apply --wait \
 *     --candidate <reviewed-id> --expected-address <wallet> \
 *     --expected-org <org-id> --report /tmp/cleanup-receipt.json
 *   bun packages/scripts/cloud/e2e-agent-cleanup.ts --base <url> --keep 2 --min-age-minutes 10
 *   bun packages/scripts/cloud/e2e-agent-cleanup.ts --protect <agentId> --report <path>
...
if (has("--help") || has("-h")) {
  console.log(
    [
      "Usage: bun packages/scripts/cloud/e2e-agent-cleanup.ts [options]",
