 ...
 *
 *   node packages/scripts/portable-linux-fused-inference.ts --out /absolute/output
 *
...
const TRACKED_BUILD_FILES = Object.freeze([
  ...
  "packages/app/scripts/build-helpers/verify-fused-symbols.ts",
...
export function mmdebstrapArguments({
  ...
  const buildCommand =
    ...
    "node packages/app/scripts/stage-desktop-fused-lib.ts " +
...
function finalizeProvenance({ audit, hostStage, includeDigest, jobs }) {
  ...
  const provenance = {
    schemaVersion: 1,
    builder: "packages/scripts/portable-linux-fused-inference.ts",
