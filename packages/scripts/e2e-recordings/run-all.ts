 ...
 *
 * Usage:
 *   node packages/scripts/e2e-recordings/run-all.ts
 *
 * Options:
 *   --packages=<comma-list>   Run only the named packages (e.g. --packages=homepage,app)
...
import { resolveRequireEvidence } from "../../app/scripts/lib/capture-output.ts";
import { RECORDINGS_DIR, REPO_ROOT, UI_E2E_SUITES } from "./suites.ts";
...
async function main() {
  ...
  if (!skipSheets) {
    ...
    const sheetsScript = path.join(SCRIPTS_DIR, "generate-contact-sheets.ts");
    if (fs.existsSync(sheetsScript)) {
      const code = runScript(sheetsScript);
      if (code !== 0) {
        console.warn(
          `[warn] generate-contact-sheets.ts exited with code ${code}`,
        );
      }
    } else {
      console.warn("[warn] generate-contact-sheets.ts not found — skipping");
    }
  } else {
  ...
  if (!skipViewer) {
    ...
    const viewerScript = path.join(SCRIPTS_DIR, "generate-viewer.ts");
    if (fs.existsSync(viewerScript)) {
      const code = runScript(viewerScript);
      if (code !== 0) {
        console.warn(`[warn] generate-viewer.ts exited with code ${code}`);
      }
    } else {
      console.warn("[warn] generate-viewer.ts not found — skipping");
    }
  } else {
  ...
  if (review) {
    ...
    if (fs.existsSync(reviewScript)) {
      ...
      console.warn("[warn] evidence-review/generate.ts not found — skipping");
    }
  }
