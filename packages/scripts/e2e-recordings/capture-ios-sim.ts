...
} from "../../app/scripts/lib/ios-simulator-capture.ts";
import {
  argValue,
  copyArtifact,
  createCaptureLog,
...
async function main() {
  ...
  try {
    ...
    await runCommandWithLog(
      process.execPath,
      [
        "packages/app/scripts/ios-onboarding-smoke.ts",
        "--no-video",
        "--api-base",
      ...
      ],
      {
        ...
        logSink,
        label:
          "node packages/app/scripts/ios-onboarding-smoke.ts --no-video --api-base http://127.0.0.1:31337",
      },
    );
