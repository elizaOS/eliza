...
import { conditionalSkipBearingFiles } from "./audit-focused-skipped-tests.ts";
import {
  atomicWriteJsonSync,
...
const ISOLATED_TEST_DRIVER = "packages/scripts/run-script-test-files.ts";
 ...
 * classifier in audit-focused-skipped-tests.ts is the single authority for
    ...
    if (unauthorized.length > 0) {
      throw new Error(
        `JUnit artifact contains skipped test(s) in ${unauthorized.join(", ")}; ` +
          ...
          "classified as runtime-conditional (see packages/scripts/audit-focused-skipped-tests.ts)",
      );
    }
  }
  return {
...
function printUsage() {
  process.stdout.write(
    "Usage: node packages/scripts/run-script-tests.ts [--inventory] [--report <path>] [--junit <path>]\n",
  );
}
