...
const RM_PATH_RECURSIVE_SCRIPT = path.join(SCRIPT_DIR, "rm-path-recursive.mjs");
...
function rmRecursive(pathToRemove) {
  ...
  if (result.status !== 0) {
    ...
    throw new Error(
      `failed to recursively remove benchmark artifact mirror ${pathToRemove}: ${reason}`,
    );
  }
}
