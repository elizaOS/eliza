 ...
 *   - ci.yml owns the consolidated PR + branch-health gate (push+PR on
 *     develop and main, plus merge_group/workflow_call/workflow_dispatch).
...
export function runContract(repoRoot = DEFAULT_REPO_ROOT) {
  ...
  // ci.yml is the consolidated PR + branch-health gate for both branches.
  const ci = read(".github/workflows/ci.yml");
  assertDeepEqual(
    parseInlineBranches(ci, "push", ".github/workflows/ci.yml"),
    ["develop", "main"],
    "ci.yml push branches",
  );
  assertDeepEqual(
    parseInlineBranches(ci, "pull_request", ".github/workflows/ci.yml"),
    ["develop", "main"],
    "ci.yml PR branches",
  );
  ...
  // either directly via setup-bun-workspace or by reusing ci.yml (which wires
  // it internally) through workflow_call.
  for (const workflowRel of [
    ".github/workflows/nightly.yml",
    ".github/workflows/release.yaml",
  ]) {
    const body = read(workflowRel);
    assert(
      body.includes("setup-bun-workspace") ||
        body.includes(".github/workflows/ci.yml"),
      `${workflowRel} GitHub-native turbo cache: expected setup-bun-workspace or a ci.yml workflow_call reuse`,
    );
  }
