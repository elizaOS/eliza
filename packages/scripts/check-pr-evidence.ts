...
import { pngMetadata } from "./lib/png-metadata.ts";
...
interface EvaluationFinding extends EvidenceRow {
  status: string;
  detail?: string;
}
...
interface TrustedArtifact extends UrlReference {
  identity: string;
  extension: string;
  kind: string;
}
interface ArtifactReference extends TrustedArtifact, EvidenceRow {
  expectedKind: string | null;
}
type ArtifactResult = Pick<
  ArtifactVerificationFinding,
  "status" | "detail" | "contentSha256"
>;
type ArtifactFetch = (
...
export function hasMatchingEvidenceHead(body: unknown, headSha: unknown) {
 ...
 */
export function requiresSurfaceArtifactsFromFiles(files: unknown) {
  return surfaceFiles(files).length > 0;
}

/** The rendered-UI source files within a changed-file list. */
export function surfaceFiles(files: unknown) {
 ...
 * prose.
 */
export function beforeScreenshotImpossible(
  changedFiles: unknown,
  addedFiles: unknown,
) {
...
export function requiresSurfaceArtifacts(labels: unknown) {
  const labelSet = new Set(parseLabels(labels));
  return SURFACE_EVIDENCE_LABELS.some((label) => labelSet.has(label));
}

export function hasOcrEvidenceReference(rows: Map<string, string>) {
...
export function hasNaWithReason(text: string) {
...
function urlReferences(text: string) {
  const source = String(text ?? "");
  const references: UrlReference[] = [];
  const seen = new Set();
  const add = (rawUrl: string, presentation: string) => {
...
function trustedArtifact(reference: UrlReference): TrustedArtifact | null {
  let url: URL;
  try {
...
function trustedArtifacts(text: string) {
  const artifacts = new Map<string, TrustedArtifact>();
...
export function hasArtifactReference(text: string) {
  return trustedArtifacts(text).length > 0;
}

function artifactCanBeEvidenceFile({
  extension,
  kind,
  presentation,
}: TrustedArtifact) {
...
export function hasEvidenceFileReference(text: string) {
...
function hasSubstantiveInlineLog(text: string) {
  const source = String(text ?? "");
...
function inlineTranscriptMeasurements(text: string) {
  const source = String(text ?? "");
  const blocks = [
...
function inlineTranscriptMeetsFloor({
  lines,
  chars,
}: {
  lines: number;
 ...
 */
export function hasInlineTranscriptEvidence(text: string) {
 ...
 */
export function describeInlineTranscriptShortfall(text: string) {
  let best = null;
...
function fencedBlockDelimiter(line: string) {
...
function closesFencedBlock(line: string, fence: Fence) {
...
function substantiveTrajectoryValue(value: unknown, minimumLength: number) {
  const serialized =
...
function hasMeaningfulTrajectoryKey(
  value: unknown,
...
function hasTrajectorySemantics(value: unknown) {
  return (
...
function parseStructuredTrajectory(text: string) {
...
export function hasSubstantiveInlineTrajectory(text: string) {
  const source = String(text ?? "");
  const blocks = [
    ...source.matchAll(/<details[\s\S]*?<\/details>/gi),
    ...source.matchAll(/<pre[\s\S]*?<\/pre>/gi),
 ...
 */
export function hasVisualArtifactReference(text: string, expected = "media") {
...
export function parseChangedFiles(value: unknown): string[] {
  if (Array.isArray(value))
...
export function findRetiredRepoEvidenceFiles(files: unknown) {
  return parseChangedFiles(files).filter((file) =>
    file.replaceAll("\\", "/").startsWith(`${RETIRED_REPO_EVIDENCE_PATH}/`),
  );
}

export function isChecked(rowText: string) {
  return /^\s*[-*]\s*\[\s*[xX]\s*\]/m.test(rowText);
}

export function isRowSatisfied(rowText: string) {
  return (
...
export function isRowSatisfiedForContext(
  rowText: string,
  { artifactRequired = false }: { artifactRequired?: boolean } = {},
) {
  if (artifactRequired) return hasArtifactReference(rowText);
  return isRowSatisfied(rowText);
}

function isEvidenceRowSatisfied(id: string, rowText: string) {
...
export function boundRowBlock(block: string) {
...
export function extractEvidenceRows(body: string) {
  const source = body ?? "";
...
function duplicateArtifactsByRow(
  rows: Map<string, string>,
  requiredRows: readonly EvidenceRow[],
) {
  const owners = new Map<string, { rows: Set<string>; url: string }>();
  ...
  const duplicates = new Map<string, { rowIds: string[]; url: string }[]>();
...
export function evaluatePrEvidence(
  body: string,
  requiredRows: readonly EvidenceRow[] = REQUIRED_EVIDENCE_ROWS,
  ...
  const findings: EvaluationFinding[] = requiredRows.map(({ id, label }) => {
    ...
    const rowText = rows.get(id);
    if (rowText === undefined) return { id, label, status: "missing" };
...
function usefulImageDimensions(width: number, height: number) {
  return width >= MIN_SCREENSHOT_WIDTH && height >= MIN_SCREENSHOT_HEIGHT;
}

function jpegMetadata(bytes: Uint8Array, complete: boolean) {
...
function readGifSubBlocks(bytes: Uint8Array, start: number) {
  let offset = start;
  let payloadBytes = 0;
...
function gifMetadata(bytes: Uint8Array, complete: boolean) {
...
function webpMetadata(bytes: Uint8Array, complete: boolean) {
  if (
    bytes.length < 30 ||
...
function isoChildBoxes(bytes: Uint8Array, start: number, end: number) {
  const boxes = [];
  let offset = start;
  while (offset < end) {
...
function isoChild(boxes: IsoBox[] | null, type: string) {
  return boxes?.find((box) => box.type === type) ?? null;
}

function hasIsoVideoTrack(bytes: Uint8Array, moov: IsoBox) {
...
function isoBmffVideoMetadata(bytes: Uint8Array, complete: boolean) {
...
function ebmlChildren(bytes: Uint8Array, start: number, end: number) {
  const elements = [];
  let offset = start;
  while (offset < end) {
...
function ebmlUnsigned(bytes: Uint8Array, element: EbmlElement | undefined) {
...
function webmTrackDimensions(bytes: Uint8Array, tracks: EbmlElement) {
  const trackEntries =
...
function webmClusterHasPayload(bytes: Uint8Array, cluster: EbmlElement) {
...
function webmMetadata(bytes: Uint8Array) {
...
function mediaMetadataFromBytes(bytes: Uint8Array, complete: boolean) {
  return (
    pngMetadata(bytes, complete, MIN_SCREENSHOT_WIDTH, MIN_SCREENSHOT_HEIGHT) ??
...
function parseJsonLines(text: string, complete: boolean): unknown[] | null {
  const lines = text
    .split(/\r?\n/)
...
async function readResponsePrefix(
  ...
  const parseByteCount = (value: string | null) => {
...
function trustedDownloadUrl(rawUrl: string) {
  let url: URL;
  try {
    url = new URL(rawUrl);
  } catch {
...
async function verifyArtifact(
  ...
  token: string,
  timeoutMs: number,
  contentDigestLimitBytes: number | null,
...
export async function verifyReferencedArtifacts(
  ...
  const uniqueArtifacts = new Map<string, ArtifactReference>();
  const limitFindings: ArtifactVerificationFinding[] = [];
  ...
  const referencesByIdentity = new Map<string, ArtifactReference[]>();
  ...
  const findings = references.map(({ id, identity, kind, label, url }) => {
    ...
    if (!result)
      throw new Error(`Missing artifact verification result: ${identity}`);
    return {
      id,
      label,
      url,
...
export function planReferencedArtifacts(
  body: string,
  requiredRows: readonly EvidenceRow[] = REQUIRED_EVIDENCE_ROWS,
  options: Pick<VerificationOptions, "allowedArtifactKinds"> = {},
) {
  const rows = extractEvidenceRows(String(body ?? ""));
  const allowedArtifactKinds = options.allowedArtifactKinds
    ? new Set(options.allowedArtifactKinds)
...
function combineVerificationFindings(
  evaluationFindings: EvaluationFinding[],
  remoteFindings: ArtifactVerificationFinding[],
) {
  const failuresByRow = new Map<string, ArtifactVerificationFinding[]>();
...
function readFileListArg(args: string[], flag: string) {
...
function usage() {
  console.log(`Usage: node packages/scripts/check-pr-evidence.ts [options]
...
function buildFixtureBody(overrides: Record<string, string> = {}) {
    ...
    const { ok } = evaluatePrEvidence(
      ...
      REQUIRED_EVIDENCE_ROWS,
      {
        changedFiles: [
          ...
          "packages/app/src/services/thing.ts",
