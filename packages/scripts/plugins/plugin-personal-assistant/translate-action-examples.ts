 ...
 *
 * Usage:
 *   bun packages/scripts/plugins/plugin-personal-assistant/translate-action-examples.ts \
...
function parseArgs(argv) {
  ...
  if (args.length === 0) {
    fail(
      "Usage: translate-action-examples.ts <action-file> --target-locale=<locale> [--provider=cerebras] [--max-examples=N] [--action-name=NAME] [--output=PATH] [--dry-run]",
    );
  }
  const result = {
    file: args[0],
...
function renderRegistryPack({ actionName, locale, translations }) {
  const lines = [];
  lines.push(
    "// AUTOGENERATED by packages/scripts/plugins/plugin-personal-assistant/translate-action-examples.ts",
  );
