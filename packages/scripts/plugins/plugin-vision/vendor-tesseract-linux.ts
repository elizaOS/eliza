 ...
 *
 * Usage: node packages/scripts/plugins/plugin-vision/vendor-tesseract-linux.ts [--out <dir>] [--langs eng,...]
...
const outDir = arg(
  "--out",
  join(here, "../../../../plugins/plugin-vision/vendor"),
);
...
function lddClosure(binPath: string) {
  let out = "";
  try {
    ...
    // error-policy:J2: annotate and rethrow dependency discovery failure.
    ...
    throw new Error(
      `[vendor-tesseract] ldd failed (${err instanceof Error ? err.message : String(err)}); cannot compute the shared-lib closure on this host`,
    );
  }
  const resolved = [];
