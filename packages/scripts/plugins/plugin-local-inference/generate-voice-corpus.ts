...
import {
  generateVoiceCorpus,
  writeVoiceCorpus,
} from "../../../../plugins/plugin-local-inference/src/services/voice/corpus-generator.ts";
import {
  MEETING_STRESS_CORPUS_SCHEMA_VERSION,
  type MeetingStressAudioSourceMode,
} from "../../../../plugins/plugin-local-inference/src/services/voice/meeting-acoustic-stress-evaluator.ts";
import {
  buildMeetingAcousticStressMatrix,
  type MeetingAcousticStressCase,
} from "../../../../plugins/plugin-local-inference/src/services/voice/meeting-acoustic-stress-matrix.ts";
import { VOICE_WORKBENCH_SCENARIOS } from "../../../../plugins/plugin-local-inference/src/services/voice/workbench-scenarios.ts";
...
async function main(): Promise<void> {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        schemaVersion: meetingStress ? MEETING_STRESS_CORPUS_SCHEMA_VERSION : 1,
        ...
        ...(stressMatrix
          ? {
              meetingAcousticStressMatrix: {
                ...
                cases: stressMatrix.cases.map(
                  ({ scenario: _scenario, ...entry }) => entry,
                ),
              },
            }
          : {}),
