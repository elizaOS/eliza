#!/usr/bin/env node
/**
 * probe-sd-cpp.ts — WS3 image-gen onboarding probe.
 *
...
async function main() {
  ...
  if (!versionResult.ok) {
    const reason =
      versionResult.error?.code === "ENOENT" || versionResult.code === null
        ? "binary_missing"
...
function emit(payload) {
  if (ARG_HUMAN && !ARG_JSON) {
    ...
    if (payload.available) {
      ...
      lines.push(`accelerators: ${(payload.accelerators ?? []).join(", ")}`);
