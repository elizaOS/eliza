...
import { localInferenceRoot } from "../../../../plugins/plugin-local-inference/src/services/paths";
import type { BenchCorpusEntry } from "../../../../plugins/plugin-local-inference/src/services/voice/bench-utils";
import { decodeMonoPcm16Wav } from "../../../../plugins/plugin-local-inference/src/services/voice/engine-bridge";
import {
...
export function makeBenchGates(
  tag: string,
  requireEnvName: string,
): BenchGates {
  ...
  if (existsSync(manifestPath)) {
    ...
    if (existing !== JSON.stringify(manifest, null, "\t")) {
      ...
      for (const e of entries)
        rmSync(path.join(dir, `${e.id}.wav`), { force: true });
    }
  }

  const missing = entries.filter(
    (e) => !existsSync(path.join(dir, `${e.id}.wav`)),
  );
  if (missing.length > 0) {
    ...
    if (failures.length > 0) {
      gates.fail(
        `corpus ${name}: Kokoro synthesis failed for ${failures.join(", ")}`,
      );
    }
...
export function defaultReportDir(): string {
  return (
    ...
    path.resolve(
      __dirname,
      "../../../../plugins/plugin-local-inference/voice-bench-output",
    )
  );
}
