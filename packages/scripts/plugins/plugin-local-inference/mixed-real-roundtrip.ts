...
if (!lib || !existsSync(lib))
  skip("set ELIZA_INFERENCE_LIBRARY to the fused dylib");
...
async function elevenTts(text: string, fmt: string): Promise<Uint8Array> {
  ...
  if (!r.ok)
    fail(`ElevenLabs TTS ${r.status}: ${(await r.text()).slice(0, 200)}`);
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a concise voice assistant. Reply in one short sentence.",
        },
...
console.log(
  `[mixed-roundtrip] (cloud TTS question) ${tQ}ms, ${pcm16.length} bytes pcm`,
);
...
console.log(
  `[mixed-roundtrip] (LOCAL STT, eliza-1-asr) ${asrMs}ms → "${transcript}"`,
);
...
console.log(
  `[mixed-roundtrip] (cloud LLM, Cerebras ${cbModel}) ${llmMs}ms → "${reply}"`,
);
...
console.log(
  `[mixed-roundtrip] (cloud TTS reply) ${ttsMs}ms, ${replyAudio.length} bytes mp3`,
);
