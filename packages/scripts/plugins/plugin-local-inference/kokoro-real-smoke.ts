...
import { resolveKokoroTtfaBudgetMs } from "../../../../plugins/plugin-local-inference/src/services/voice/kokoro/kokoro-ttfa-budget";
import type { Phrase } from "../../../../plugins/plugin-local-inference/src/services/voice/types";
...
console.log(
  `[kokoro-real-smoke] lib=${libPath} (ABI v${ffi.libraryAbiVersion})`,
);
console.log(
...
try {
  ...
  const ttfa =
    firstAudibleMs === null ? Number.NaN : Math.round(firstAudibleMs);
  console.log(
