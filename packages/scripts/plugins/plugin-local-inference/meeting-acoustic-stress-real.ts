...
import { runVoiceWorkbenchHeadless } from "../../../../plugins/plugin-local-inference/src/services/voice/workbench-headless-runner.ts";
...
import { defaultReportDir, writeBenchReport } from "./voice-bench-shared.ts";
...
function measuredMetrics(
  ...
  const negativeTurnsByScenario = new Map(
    corpus.entries.map((entry) => [
      ...
      entry.corpus.groundTruth.turns.filter((turn) => !turn.expectRespond)
        .length,
    ]),
  );
  ...
  for (const run of runs) {
    const respond = run.cases.find(
      (entry) => entry.kind === "respond-decision",
    );
    const scenarioNegativeTurns =
      negativeTurnsByScenario.get(run.scenarioId) ?? 0;
      ...
      [
        ...
        ...(corpus.nonPublishableReasons.length > 0
          ? [
              ...
              ...corpus.nonPublishableReasons.map((reason) => `- ${reason}`),
            ]
          : []),
      ].join("\n"),
    );
