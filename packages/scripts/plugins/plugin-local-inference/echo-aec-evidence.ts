...
} from "../../../../plugins/plugin-local-inference/src/services/voice/__test-helpers__/synthetic-speech.js";
import { extractTimbreEmbedding } from "../../../../plugins/plugin-local-inference/src/services/voice/acoustic-speaker-attribution.js";
import {
  applyGainDb,
  applyReverb,
} from "../../../../plugins/plugin-local-inference/src/services/voice/corpus-augment.js";
import {
...
function roomEcho(
  ...
  for (let i = 0; i < attenuated.length; i++)
    near[delaySamples + i] += attenuated[i];
...
async function main(): Promise<void> {
  ...
  const conditions = [
    {
      ...
      delayMs: 20,
      farFieldDb: 6,
      reflections: [] as ReadonlyArray<readonly [number, number]>,
    },
    {
    ...
    },
    {
      ...
      delayMs: 380,
      farFieldDb: 9,
      reflections: [[80, 0.3]] as ReadonlyArray<readonly [number, number]>,
    },
  ];
  ...
  for (let i = 0; i < 5; i++) {
    ...
    agentProbes.push({
      seed: 0x4000 + i,
      similarity:
        similarity === null ? Number.NaN : Number(similarity.toFixed(3)),
      rejected: await imprint.isAgentSelfVoice(emb),
    });
  }
  const humanProbes = [] as {
    seed: number;
    index: number;
    similarity: number;
    rejected: boolean | null;
  ...
  const desktopPass = (
    report.desktopFarEndReference as { meetsCeiling: boolean }
  ).meetsCeiling;
  const selfVoicePass =
    (
      report.selfVoiceImprint as {
  ...
  // ── print ────────────────────────────────────────────────────────────────
  console.log(
    "=== #12256 echo/AEC domain evidence (real DSP over synthetic corpus) ===\n",
  );
