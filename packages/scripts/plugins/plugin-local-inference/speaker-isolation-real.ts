...
import { OnlineSpeakerClusterer } from "../../../../plugins/plugin-local-inference/src/services/voice/acoustic-speaker-attribution";
import {
...
import { measureRms } from "../../../../plugins/plugin-local-inference/src/services/voice/corpus-augment";
import {
  computeDiarizationErrorRate,
  type DiarizationSegment,
} from "../../../../plugins/plugin-local-inference/src/services/voice/diarization-error-rate";
import { FusedDiarizer } from "../../../../plugins/plugin-local-inference/src/services/voice/speaker/diarizer-fused";
import { FusedSpeakerEncoder } from "../../../../plugins/plugin-local-inference/src/services/voice/speaker/encoder-fused";
import { voiceSpeakerDistance } from "../../../../plugins/plugin-local-inference/src/services/voice/speaker/encoder-ggml";
import {
  BENCH_SAMPLE_RATE,
  bootFusedFfi,
  defaultReportDir,
...
if (!diarizGguf || !existsSync(diarizGguf)) {
  gates.skip(
    "no diarizer GGUF (set ELIZA_DIARIZ_GGUF to pyannote-segmentation-3.0.gguf)",
  );
}
if (!speakerGguf || !existsSync(speakerGguf)) {
  gates.skip(
    "no speaker-encoder GGUF (set ELIZA_SPEAKER_GGUF to wespeaker-resnet34-lm.gguf)",
  );
}
if (!FusedDiarizer.isSupported(ffi)) {
  gates.skip(
    "fused lib does not link the pyannote diarizer (eliza_inference_diariz_*)",
  );
}
if (!FusedSpeakerEncoder.isSupported(ffi)) {
  gates.skip(
    "fused lib does not link the WeSpeaker encoder (eliza_inference_speaker_*)",
  );
}
log(`lib=${libPath}`);
...
const turns = await ensureKokoroCorpus(
  "two-speaker",
  TWO_SPEAKER_DIALOGUE,
  gates,
  log,
...
try {
  ...
  const clusters = [
    ...new Set(assigned.map((a) => a.cluster).filter(Boolean)),
  ] as string[];
  let mfccBest = 0;
  ...
  log(
    `mfcc attributor (#9427): best-mapping accuracy ${mfccBest.toFixed(3)} (${clusters.length} clusters)`,
  );

  // --- slice 2: diarizer over 5 s windows + encoder globalization --------------
  const enrollment = new Map<string, number[]>();
  for (const e of embedded) {
    if (!enrollment.has(e.speaker))
      enrollment.set(e.speaker, Array.from(e.embedding));
  }
  ...
  for (let off = 0; off < timeline.pcm.length; off += WINDOW_SAMPLES) {
    ...
    window.set(
      timeline.pcm.subarray(
        off,
        Math.min(off + WINDOW_SAMPLES, timeline.pcm.length),
      ),
    );
  ...
  for (let i = 0; i < WINDOW_SAMPLES; i++) {
    overlapPcm[i] =
      (i < a.length ? a[i] : 0) + (i < b.length ? b[i] * scaleB : 0);
  }
...
if (report.der.der > MAX_DER) {
  failures.push(
    `diarizer DER ${report.der.der.toFixed(3)} > SPEAKER_ISO_MAX_DER ${MAX_DER}`,
  );
}
