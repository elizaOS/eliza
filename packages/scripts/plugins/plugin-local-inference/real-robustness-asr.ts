...
const CONDITIONS: Array<{
  ...
  {
    name: "far-field 12dB",
    spec: { farFieldDb: 12, reverb: 0.4, noiseSnrDb: 14, seed: 1 },
  },
...
async function ttsFloat32(text: string): Promise<Float32Array> {
  ...
  if (!r.ok)
    skip(`ElevenLabs TTS ${r.status}: ${(await r.text()).slice(0, 160)}`);
...
for (const cond of CONDITIONS) {
  ...
  const mean = arr.length
    ...
    : Number.NaN;
  console.log(
    `  ${cond.name.padEnd(16)} mean WER = ${mean.toFixed(3)}  (n=${arr.length})`,
  );
}
