...
import {
  ...
  STT_BENCH_CORPUS,
  type WerAtSnr,
} from "../../../../plugins/plugin-local-inference/src/services/voice/bench-utils";
import {
  addNoise,
  type NoiseKind,
} from "../../../../plugins/plugin-local-inference/src/services/voice/corpus-augment";
import {
  BENCH_SAMPLE_RATE,
  bootFusedFfi,
  defaultReportDir,
...
try {
  // Clean baseline first.
  {
    const utts = corpus.map((item) => {
      ...
      return {
        id: item.id,
        wer: wordErrorRate(item.text, transcript),
        transcript,
      };
    });
