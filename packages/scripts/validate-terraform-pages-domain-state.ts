...
function main() {
  ...
  if (
    ...
    console.error(
      "::error::Usage: validate-terraform-pages-domain-state.ts <pages-domains-json> <canonical-edge-json> <railway-tunnel-json> <redirect-dns-json> <legacy-certificate-packs-json> <staging|production>",
    );
    process.exitCode = 1;
    return;
  }
