 ...
 * streams each lane's status through the human-speed reporter (reporter.ts),
...
import { createMatrixReporter, renderMatrixSummary } from "./reporter.ts";
...
export const MATRIX_STEPS = [
  ...
  },
  {
    ...
    command: ["node", "packages/scripts/run-content-context.ts"],
    tags: ["tests", "content-context"],
  },
  {
    id: "ios-sim-capture",
    label: "iOS simulator capture",
    command: ["node", "packages/scripts/e2e-recordings/capture-ios-sim.ts"],
    tags: ["device", "ios"],
  ...
  },
  {
    ...
    command: ["node", "packages/scripts/e2e-recordings/capture-android-emu.ts"],
    tags: ["device", "android"],
...
function printHelp() {
  console.log(`Usage: node packages/scripts/evidence-review/run-matrix.ts [options]
