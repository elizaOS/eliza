...
const USAGE = `Usage: node packages/scripts/vast/run-certification.ts --sha <commit> [options]
...
export function buildOnstart(opts) {
  const lines = [
    ...
    "  node packages/scripts/gpu-vision/serve.ts --vlm --verify || fail gpu-vision-serve",
    "else",
