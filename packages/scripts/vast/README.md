...
| `run-certification.test.mjs` | Unit tests for the pure pieces (`bun run test:vast` / `node --test packages/scripts/vast/*.test.mjs`) |
...
node packages/scripts/vast/run-certification.mjs --sha <full-sha> --tier full
```

Always available with **zero API calls and zero secrets**:

```bash
node packages/scripts/vast/run-certification.mjs --sha <sha> --dry-run
```
...
tier; a CUDA box with `llama-server` + `node --test packages/scripts/gpu-vision/*.test.mjs` models for
gpu/full):

```bash
export ELIZA_CERT_SIGNING_KEY=...   # from the keyholder; never committed
node packages/scripts/vast/local-certify.mjs --tier full
```
...
`packages/scripts/vast/Dockerfile.gpu` bakes CUDA `llama-server` (pinned to
llama.cpp `b8525`, the minimum `packages/scripts/gpu-vision` accepts), the
sha256-pinned OCR/VLM GGUFs via `packages/scripts/gpu-vision/setup.mjs --with-vlm`,
