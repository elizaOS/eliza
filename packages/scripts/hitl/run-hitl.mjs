 ...
 * (through `packages/scripts/e2e-recordings/run-all.mjs`, which already extracts frames,
 ...
 *
 * Usage:
 *   node packages/scripts/hitl/run-hitl.mjs                # plan + report from existing recordings
 *   node packages/scripts/hitl/run-hitl.mjs --record       # also (re)run the frame suites first
 *   node packages/scripts/hitl/run-hitl.mjs --groups=onboarding,login
 *   node packages/scripts/hitl/run-hitl.mjs --out=<dir>
 *
...
lines.push(
  `Contact sheets: ${haveContactSheets ? "present (open e2e-recordings/viewer)" : "NOT present — run `node packages/scripts/hitl/run-hitl.mjs --record`"}`,
);
lines.push("");
