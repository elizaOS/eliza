 ...
 *
 * The per-route AI-QA reviewer (`review-screenshots.ts`) only ever sees an
 * isolated route at a time. This adapter binds the SAME vision review library
 * (`review-lib.ts`) to the ordered walkthrough steps: it reads each
 ...
 *
 * Usage:
 *   node packages/scripts/ai-qa/review-walkthrough.ts [--run-dir reports/walkthrough/<id>]
...
import { parseReviewerArgs } from "./reviewer-args.ts";
import { resolveReviewerBackend } from "./reviewer-preflight.ts";
...
export function buildVerdictMarkdown({ runId, model, lane, totals, results }) {
  ...
  lines.push("");
  lines.push(
    "Generated by `packages/scripts/ai-qa/review-walkthrough.ts`. Each row is a per-step",
