...
export function runI18nCheck(options = {}) {
  ...
  const { literalKeys, prefixWildcards, dynamicSites } = scanSources(scanDirs);
  ...
  // 2. Every source-locale key must be referenced in source (literal,
  //    wildcard prefix, or allowlist) — an unused source key is dead copy —
  //    and a key present in a locale but absent from the source catalog is an
  //    orphaned translation. Both are errors.
  ...
  const unusedSource = Object.keys(locales[sourceLocale]).filter(
    (key) =>
      !allowedKeys.has(key) && !isCoveredByPrefixes(key, allowedPrefixes),
  );
  if (unusedSource.length > 0) {
    errors.push(
      `[i18n] ${sourceLocale}.json has ${unusedSource.length} unused key(s) (not referenced in source or allowlist):`,
    );
    for (const key of unusedSource.slice(0, 50)) errors.push(`  - ${key}`);
    if (unusedSource.length > 50) {
      errors.push(`  ... ${unusedSource.length - 50} more`);
    }
  }
  for (const lang of langs) {
    if (lang === sourceLocale) continue;
  ...
  // 4. Surface dynamic call sites the allowlist doesn't cover (informational).
  if (
    dynamicSites.length > 0 &&
    allowlist.keys.length === 0 &&
    allowlist.prefixes.length === 0
  ) {
    warnings.push(
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (result.ok) {
    ...
    console.log(
      `[i18n] OK — ${s.literalKeys} literal keys, ${s.wildcardPrefixes} wildcard prefixes, ${s.dynamicSites} dynamic sites; ${s.sourceKeys} source keys × ${s.locales} locales (translation gaps above are warnings; --strict-translations upgrades them).`,
    );
    process.exit(0);
  }
  process.exit(1);
