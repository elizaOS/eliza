...
import { shouldNormalizeBunStatus99 } from "./test-cloud-run-helpers.ts";
...
export const PREFLIGHT_STEPS = {
  coreBuild: {
    ...
    script: ["packages", "scripts", "build-core.ts"],
  },
};
    ...
    if (!step) {
      throw new Error(
        `[test:cloud] no preflight step named "${stepName}" — keep ` +
          ...
          "packages/scripts/test-cloud-run.ts.",
      );
    }
    log(
    ...
    if (stillMissing.length > 0) {
      throw new Error(
        `[test:cloud] ${step.label} completed but the required artifact(s) are still missing:\n` +
          ...
          "update computeRequiredRuntimeArtifacts in packages/scripts/test-cloud-run.ts.",
      );
    }
  }
}
  ...
  if (missing.length > 0) {
    console.error(
      `[test:cloud] test root(s) not found — the gate would silently run no tests:\n  ${missing.join("\n  ")}\n` +
        "Update packages/scripts/test-cloud-run.ts to match the current package layout.",
    );
    process.exit(1);
  }
  ...
  if (cloudApiUnitTests.length === 0) {
    console.error(
      `[test:cloud] no colocated cloud/api unit tests found under ${cloudApiRoot} — ` +
        "the gate would silently run zero api tests. Update packages/scripts/test-cloud-run.ts " +
  ...
  if (cloudServicesTests.length === 0) {
    console.error(
      `[test:cloud] no cloud/services tests found under ${cloudServicesRoot} — ` +
        "the gate would silently run zero services tests. Update packages/scripts/test-cloud-run.ts " +
        "to match the current package layout.",
    );
    process.exit(1);
  }
  ...
  if (allTestFiles.length === 0) {
    console.error(
      "[test:cloud] enumerated zero test files across all roots — the gate would " +
        "silently pass. Update packages/scripts/test-cloud-run.ts to match the layout.",
    );
    process.exit(1);
  }
  ...
  if (missingIsolatedTestFiles.length > 0) {
    console.error(
      `[test:cloud] fresh-process test file(s) not found in the unit manifest:\n  ${missingIsolatedTestFiles.join("\n  ")}\n` +
        "Update TEST_FILES_REQUIRING_FRESH_PROCESS in packages/scripts/test-cloud-run.ts.",
    );
    process.exit(1);
  }
