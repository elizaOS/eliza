...
function readStagedFiles(): string[] {
  ...
  if (proc.exitCode !== 0) throw new Error("Failed to get staged files");
  return new TextDecoder().decode(proc.stdout).split("\0").filter(Boolean);
}

function assertNoGeneratedAppPlatformsStaged(stagedFiles: string[]) {
...
try {
  ...
  assertNoGeneratedAppPlatformsStaged(stagedFiles);
  ...
  // error-policy:J1 report the CLI failure and exit unsuccessfully.
  console.error(
    "Error during pre-commit linting:",
