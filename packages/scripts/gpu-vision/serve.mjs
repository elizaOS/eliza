 ...
 *
 * Usage:
 *   node packages/scripts/gpu-vision/serve.mjs [--vlm] [--parallel N] [--port P] [--verify]
 *   node packages/scripts/gpu-vision/serve.mjs --stop [--vlm]
 */
...
async function serve({ setKey, parallel, requestedPort, verify }) {
  ...
  for (const p of [modelPath, mmprojPath]) {
    await fs.access(p).catch(() => {
      throw new Error(
        `[gpu-vision] missing model file: ${p}\n  Run: node packages/scripts/gpu-vision/setup.mjs${setKey === "vlm" ? " --with-vlm" : ""}`,
      );
    });
  }
    ...
    if (entryIsLive(existing)) {
      throw new Error(
        `[gpu-vision] a ${setKey} server is already running (pid ${existing.pid}, port ${existing.port}).\n` +
          "  Stop it first: node packages/scripts/gpu-vision/serve.mjs --stop" +
          (setKey === "vlm" ? " --vlm" : ""),
      );
    }
    process.stdout.write(
  ...
  process.stdout.write(
    `[gpu-vision] stop with: node packages/scripts/gpu-vision/serve.mjs --stop${setKey === "vlm" ? " --vlm" : ""}\n`,
  );
