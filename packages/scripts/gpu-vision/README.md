...
node packages/scripts/gpu-vision/setup.mjs
#    …or also fetch the Qwen3-VL VLM:
node packages/scripts/gpu-vision/setup.mjs --with-vlm

# 2. Serve it (resident, OpenAI-compatible). Prints the base URL + PID file.
node packages/scripts/gpu-vision/serve.mjs --parallel 2
#    …or serve the VLM instead (second instance, its own PID entry):
node packages/scripts/gpu-vision/serve.mjs --vlm
...
node packages/scripts/gpu-vision/serve.mjs --verify

# 3. Prove it actually transcribes pixels.
node packages/scripts/gpu-vision/smoke.mjs

# Stop the resident server.
node packages/scripts/gpu-vision/serve.mjs --stop        # add --vlm to stop the VLM instance

# One-shot: setup (if needed) → serve → smoke → stop.
node packages/scripts/gpu-vision/smoke.mjs --start
```
...
`node --test packages/scripts/gpu-vision/*.test.mjs` covers the pure logic: lockfile
