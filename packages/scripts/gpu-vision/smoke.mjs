 ...
 *
 * Usage:
 *   node packages/scripts/gpu-vision/smoke.mjs [--url http://127.0.0.1:PORT] [--vlm]
 *   node packages/scripts/gpu-vision/smoke.mjs --start [--vlm] [--with-vlm]
 */
...
async function resolveBaseUrl(flags, setKey) {
  ...
  throw new Error(
    "[gpu-vision] no running server found. Pass --url, set ELIZA_GPU_VISION_PORT, " +
      "or start one: node packages/scripts/gpu-vision/serve.mjs" +
      (setKey === "vlm" ? " --vlm" : ""),
  );
}
