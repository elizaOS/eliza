 ...
 *
 *   node packages/scripts/pr-evidence.mjs attach <pr> <files...>        upload + print URLs
 *   node packages/scripts/pr-evidence.mjs rows <pr> --row id=<file|url|"N/A - reason"> …
 *                                                              patch body rows + verify
 *   node packages/scripts/pr-evidence.mjs verify <pr>                   run the gate locally
 *
...
function usage() {
  console.log(`Usage:
  node packages/scripts/pr-evidence.mjs attach <pr> <files...>
  ...
  node packages/scripts/pr-evidence.mjs rows <pr> [--dry-run] --row <id>=<value> [...]
  ...
  node packages/scripts/pr-evidence.mjs verify <pr>
...
export function createOverflowReleaseIfAbsent(tag, run = gh) {
  ...
  const notes = `Overflow continuation of the '${PRIMARY_RELEASE_TAG}' evidence asset store. GitHub caps a release at ${MAX_ASSETS_PER_RELEASE} assets, so headless-agent evidence uploads (packages/scripts/pr-evidence.mjs) roll into '${tag}' once '${prEvidenceTagForIndex(index - 1)}' fills. Files are named '<pr>-<artifact>'; embed the asset download URLs in the PR evidence rows. Never delete assets referenced by an open PR.`;
  try {
    run(
      [
        "release",
