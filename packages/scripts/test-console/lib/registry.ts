 ...
 * task list is `run-all-tests.ts --plan=json --all` verbatim; live gating is
 * `GUARDED_REAL_LIVE_SUITES` + `computeRealLiveAccounting` from
 * packages/scripts/lib/real-live-suites.ts (the same accounting the
 * post-merge lane prints); connection ownership comes from connections.ts.
...
function runPlanDiscovery() {
  return spawnSync(
    "node",
    [
      path.join(REPO_ROOT, "packages/scripts/run-all-tests.ts"),
      "--plan=json",
      "--all",
    ],
    {
