...
REPORT = ROOT / "build/reports/e1x_attn_out_sampled_k_real_weight_rows.json"
...
def main() -> int:
    ...
    paths = (PLACEMENT, FULL_OUTPUT_WORKPLAN, REPAIRED_REAL_WEIGHT)
    missing = [str(path.relative_to(ROOT)) for path in paths if not path.is_file()]
    status, detail = pass_fail(
        not missing,
        "attn-out sampled-K real-weight row inputs present",
        "missing inputs: " + ", ".join(missing),
    )
    checks.append({"id": "e1x_attn_out_sampled_k_real_weight_rows_inputs_present", "status": status, "detail": detail})
    ...
    repaired = load_json(REPAIRED_REAL_WEIGHT) if REPAIRED_REAL_WEIGHT.is_file() else {}
    ...
    deps_ok = (
        ...
        and repaired.get("status") == "PASS"
        and repaired.get("summary", {}).get("residual_blocker")
        == "full_output_real_weight_checksum_missing"
    )
    status, detail = pass_fail(
        deps_ok,
        "placement, full-output workplan, and repaired real-weight execution are linked",
        "attn-out sampled-K dependency mismatch",
    )
    checks.append({"id": "e1x_attn_out_sampled_k_real_weight_rows_dependencies_pass", "status": status, "detail": detail})

    layers = [
        layer
        for layer in placement.get("layers", [])
        if isinstance(layer, dict) and str(layer.get("kind")) == "attn_out_proj"
    ]
    total_rows = 0
    total_macs = 0
    ...
    execution_ok = (
        ...
        and total_macs == 13_107_200
        and represented_full_k_macs == 1_048_576_000
        and 0.07 < row_fraction < 0.09
        and 0.001 < executed_mac_fraction < 0.002
        and 0.08 < represented_mac_fraction < 0.09
        and aggregate_checksum != FNV64_OFFSET
    )
    status, detail = pass_fail(
        execution_ok,
        f"executed every attn_out_proj row over sampled K={SAMPLED_K} for {total_macs} real MACs",
        "attn-out sampled-K real-weight execution mismatch",
    )
    checks.append({"id": "e1x_attn_out_sampled_k_real_weight_rows_execute_all_rows", "status": status, "detail": detail})

    boundary_ok = int(layers[0]["cols"]) > SAMPLED_K if layers else False
    status, detail = pass_fail(
        boundary_ok,
        "attn-out sampled-K execution improves matmul row coverage while preserving missing full-K/full-output blocker",
        "attn-out sampled-K claim boundary mismatch",
    )
    checks.append({"id": "e1x_attn_out_sampled_k_real_weight_rows_preserve_blocker", "status": status, "detail": detail})

    failures = [check for check in checks if check["status"] != "pass"]
    summary = {
        "check_count": len(checks),
        "failing_check_count": len(failures),
        "executed_layer_kind": "attn_out_proj",
        "executed_layer_count": len(layers),
        "sampled_k": SAMPLED_K,
        "executed_attn_out_output_row_count": total_rows,
        "executed_attn_out_sampled_k_mac_count": total_macs,
        "represented_attn_out_full_k_mac_count": represented_full_k_macs,
        ...
        "attn_out_sampled_k_real_weight_checksum": int(aggregate_checksum),
        "attn_out_sampled_k_result_sha256": canonical_sha256(layer_results),
    ...
    report = {
        ...
        "gate": "e1x-attn-out-sampled-k-real-weight-rows",
        "status": "PASS" if not failures else "BLOCKED",
        "as_of": datetime.now(UTC).isoformat(),
        "generated_utc": utc_now(),
        "subsystem": "e1x",
        "claim_boundary": (
            "Deterministic W4A8 real-weight execution for every attn_out_proj "
        ...
        "evidence_paths": [
            ...
            "scripts/check_e1x_attn_out_sampled_k_real_weight_rows.py",
        ],
        "checks": checks,
        "summary": summary,
    }
    ...
    if failures:
        print(
            "BLOCKED: E1X attn-out sampled-K real-weight rows failed: "
            + ", ".join(c["id"] for c in failures)
        )
        return 1
    print(f"PASS: E1X attn-out sampled-K real-weight rows; report {REPORT.relative_to(ROOT)}")
    return 0
