...
def test_e1x_execution_coverage_ladder_gate_passes() -> None:
    ...
    assert summary["deterministic_window_row_count"] == 18_112
    assert summary["deterministic_window_lane_mac_count"] == 418_880
    assert summary["deterministic_window_remaining_row_count"] == 2_590_528
    assert summary["row_coverage_gain_vs_real_sample"] == 16.0
    assert summary["lane_mac_gain_vs_real_sample"] == 16.0
