...
def write_yosys_script(
    *,
    ...
    lines.append(
        f"read_slang --top {module} --compat-mode "
        f"--unroll-limit {unroll_limit}{param_args} {read_args}"
    )
    passes = [
    ...
    passes.extend(
        [
            ...
            f"abc -liberty {lib_path}{abc_clock_arg}",
            "opt_clean",
            f"stat -liberty {lib_path} -tee {stat_log_path}",
            f"stat -liberty {lib_path} -json {json_stat_path}",
...
def main() -> int:
    ...
    lib_paths = [p.resolve() for p in args.lib]
    for p in lib_paths:
        if not p.is_file():
    ...
    abc_match = re.search(
        r"abc:\s+Path\s+\d+:\s+Delay\s+=\s+([0-9.]+)\s+ps", yosys_log.read_text(encoding="utf-8")
    )
    if abc_match:
        abc_delay_ps = float(abc_match.group(1))
