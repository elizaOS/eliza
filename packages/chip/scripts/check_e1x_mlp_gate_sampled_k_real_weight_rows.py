...
REPORT = ROOT / "build/reports/e1x_mlp_gate_sampled_k_real_weight_rows.json"
...
ATTN_QKV_SAMPLED_K = ROOT / "build/reports/e1x_attn_qkv_sampled_k_real_weight_rows.json"
...
def main() -> int:
    ...
    paths = (PLACEMENT, FULL_OUTPUT_WORKPLAN, ATTN_QKV_SAMPLED_K)
    missing = [str(path.relative_to(ROOT)) for path in paths if not path.is_file()]
    status, detail = pass_fail(
        not missing,
        "mlp-gate sampled-K real-weight row inputs present",
        "missing inputs: " + ", ".join(missing),
    )
    checks.append({"id": "e1x_mlp_gate_sampled_k_real_weight_rows_inputs_present", "status": status, "detail": detail})
    ...
    attn_qkv = load_json(ATTN_QKV_SAMPLED_K) if ATTN_QKV_SAMPLED_K.is_file() else {}
    ...
    deps_ok = (
        ...
        and attn_qkv.get("summary", {}).get("residual_blocker")
        == "full_output_real_weight_checksum_missing"
    )
    status, detail = pass_fail(
        deps_ok,
        "placement, full-output workplan, and attn-qkv sampled-K evidence are linked",
        "mlp-gate sampled-K dependency mismatch",
    )
    checks.append({"id": "e1x_mlp_gate_sampled_k_real_weight_rows_dependencies_pass", "status": status, "detail": detail})

    layers = [
        layer
        for layer in placement.get("layers", [])
        if isinstance(layer, dict) and str(layer.get("kind")) == "mlp_gate_proj"
    ]
    total_rows = 0
    total_macs = 0
    ...
    execution_ok = (
        len(layers) == 40
        and total_rows == 552_960
        and total_macs == 17_694_720
        and represented_full_k_macs == 2_831_155_200
        and 0.21 < row_fraction < 0.22
        and 0.001 < executed_mac_fraction < 0.002
        and 0.21 < represented_mac_fraction < 0.22
        and aggregate_checksum != FNV64_OFFSET
    )
    status, detail = pass_fail(
        execution_ok,
        f"executed every mlp_gate_proj row over sampled K={SAMPLED_K} for {total_macs} real MACs",
        "mlp-gate sampled-K real-weight execution mismatch",
    )
    checks.append({"id": "e1x_mlp_gate_sampled_k_real_weight_rows_execute_all_rows", "status": status, "detail": detail})

    boundary_ok = int(layers[0]["cols"]) > SAMPLED_K if layers else False
    status, detail = pass_fail(
        boundary_ok,
        "mlp-gate sampled-K execution improves matmul row coverage while preserving missing full-K/full-output blocker",
        "mlp-gate sampled-K claim boundary mismatch",
    )
    checks.append({"id": "e1x_mlp_gate_sampled_k_real_weight_rows_preserve_blocker", "status": status, "detail": detail})

    failures = [check for check in checks if check["status"] != "pass"]
    summary = {
        "check_count": len(checks),
        "failing_check_count": len(failures),
        "executed_layer_kind": "mlp_gate_proj",
        "executed_layer_count": len(layers),
        "sampled_k": SAMPLED_K,
        "executed_mlp_gate_output_row_count": total_rows,
        "executed_mlp_gate_sampled_k_mac_count": total_macs,
        "represented_mlp_gate_full_k_mac_count": represented_full_k_macs,
        ...
        "mlp_gate_sampled_k_real_weight_checksum": int(aggregate_checksum),
        "mlp_gate_sampled_k_result_sha256": canonical_sha256(layer_results),
    ...
    report = {
        ...
        "gate": "e1x-mlp-gate-sampled-k-real-weight-rows",
        "status": "PASS" if not failures else "BLOCKED",
        "as_of": datetime.now(UTC).isoformat(),
        "generated_utc": utc_now(),
        "subsystem": "e1x",
        "claim_boundary": (
            "Deterministic W4A8 real-weight execution for every mlp_gate_proj "
        ...
        "evidence_paths": [
            ...
            "scripts/check_e1x_mlp_gate_sampled_k_real_weight_rows.py",
        ],
        "checks": checks,
        "summary": summary,
    }
    ...
    if failures:
        print(
            "BLOCKED: E1X mlp-gate sampled-K real-weight rows failed: "
            + ", ".join(c["id"] for c in failures)
        )
        return 1
    print(f"PASS: E1X mlp-gate sampled-K real-weight rows; report {REPORT.relative_to(ROOT)}")
    return 0
