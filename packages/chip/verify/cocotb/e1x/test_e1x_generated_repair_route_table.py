...
@cocotb.test()
async def generated_high_failure_repair_rom_programs_route_table_lookups(dut):
    ...
    set_lookup(dut, 0, 0xFFFF, 0xFFFE)
    dut.lookup_valid.value = 0x1
    await Timer(1, units="ns")
    assert int(dut.lookup_hit.value) & 0x1 == 0
    assert get_lookup_dir(dut, 0) == 7
    assert get_lookup_hops(dut, 0) == 0
