...
| **Tier-split manifest (logic tier 0 vs memory tier 1) + per-tier open-PDK status** | `scripts/generate_e1x3d_tier_split_manifest.py` (`eliza.e1x3d.tier_split_manifest.v1`); gate `scripts/check_e1x3d_3d_split.py` (`build/reports/e1x3d_3d_split.json`) | architecture-simulation (split feasibility); the gate records cross-tier 3D-DRC/LVS as a BLOCKED escalation | cross-tier 3D DRC/LVS over the bonded interface is commercial-only (Siemens Calibre 3D-LVS/3D-DRC, Cadence Integrity 3D-IC); the logic-tier open-PDK run is still incomplete (see next line). | `.venv/bin/python scripts/check_e1x3d_3d_split.py` |
| **Single-tier E1X3D tile PD on Sky130 (planar 2D RTL-to-GDS proxy of one wafer-mesh tile)** | `pd/openlane/config.e1x3d-tile.sky130.json`; gate `scripts/check_e1x3d_pd_signoff.py` (`build/reports/e1x3d_pd_signoff.json`) | open-PDK physical (Sky130) DRC/LVS/STA — **target level; currently BLOCKED pending a completed run** | a **completed** OpenLane2 `e1x3d_tile` run (`final/metrics.json` with clean DRC/LVS/STA + GDS/DEF/netlist). Only mid-flight runs exist on disk, so the gate fails closed. This is a planar tile proxy — it is **not** the 3D stack. | `OPENLANE_CONFIG=pd/openlane/config.e1x3d-tile.sky130.json scripts/run_openlane.sh --config pd/openlane/config.e1x3d-tile.sky130.json && .venv/bin/python scripts/check_e1x3d_pd_signoff.py` |
...
- **Open-PDK physical signoff actually achieved today:** none of E1X3D is yet at
  a *passing* open-PDK level. The single-tier Sky130 tile PD is the **only**
  open-PDK signoff *target* in scope, and its gate currently reports **BLOCKED**
  because no `e1x3d_tile` run has completed `final/metrics.json` (only mid-flight
  runs exist). When that run lands clean, it signs off **one planar tile in 2D
  on Sky130** — not the 3D stack. The ASAP7 variant is predictive
...
.venv/bin/python scripts/check_e1x3d_pd_signoff.py       # single-tier Sky130 tile PD (BLOCKED until a run completes)
...
exactly as designed). The open-PDK Sky130 tile PD is BLOCKED until a run
completes. The 3D-stack signoff, the M3D PDK, and measured silicon are
