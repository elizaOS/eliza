...
bun run --cwd packages/cloud/test-mocks test       # local provider contracts
```

No standalone build script is defined; this package is consumed or executed from source.

Provider contract tests run locally without live provider credentials and are included in the required server test lane.
