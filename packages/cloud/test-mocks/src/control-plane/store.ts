 ...
 */
import { type SharedTodoCutoverRecord } from "@elizaos/shared";
import { type SharedTodoCutoverSnapshot } from "@elizaos/shared";
import { type SharedTodoMutationCutoverRecord } from "@elizaos/shared";
export type SandboxStatus = "provisioning" | "running" | "stopped" | "error" | "deletion_pending" | "deleted" | "deletion_failed";
...
export type ContainerStatus = "pending" | "running" | "restarting" | "deleting" | "deleted" | "error";
...
type TodoImportReceipt = Pick<ConversationImportResult, "sourceTodoCount" | "importedTodos" | "repairedTodos" | "skippedTodos" | "removedStaleTodos" | "sourceTodoMutationCount" | "importedTodoMutations" | "skippedTodoMutations" | "sourceTodoDigest" | "targetTodoDigest">;
interface StoredTodoCutoverState {
    todos: SharedTodoCutoverRecord[];
    mutations: SharedTodoMutationCutoverRecord[];
}
function stageTodoCutoverImport(previousState: StoredTodoCutoverState | undefined, snapshot: SharedTodoCutoverSnapshot): {
    receipt: TodoImportReceipt;
    state: StoredTodoCutoverState;
} {
    const previousTodos = previousState?.todos ?? [];
    const previousById = new Map(previousTodos.map((todo) => [todo.sourceId, todo]));
    ...
    const removedStaleTodos = previousTodos.filter((todo) => !nextIds.has(todo.sourceId)).length;
    ...
    const mutationsByKey = new Map(mutations.map((mutation) => [mutation.idempotencyKey, mutation]));
    const mutationsById = new Map(mutations.map((mutation) => [mutation.mutationId, mutation]));
    ...
    for (const mutation of snapshot.mutations) {
        ...
        if (previous) {
            if (JSON.stringify(previous) !== JSON.stringify(mutation)) {
                throw new Error("Todo mutation idempotency key belongs to a different record");
            }
            skippedTodoMutations += 1;
            continue;
        }
        if (mutationsById.has(mutation.mutationId)) {
            throw new Error("Todo mutation id belongs to a different idempotency key");
        }
...
export class ControlPlaneStore {
    ...
    private readonly scheduledTasks = new Map<string, Map<string, StoredScheduledTask>>();
    ...
    resolveContainerActions(): {
        ...
        for (const container of [...this.containers.values()]) {
            if (container.pendingActionAt !== undefined &&
    ...
    bindSandboxRuntimeToken(id: string, token: string): void {
        ...
        if (!trimmed)
            throw new Error("runtime token must be non-empty");
     ...
     */
    importConversation(sandboxId: string, conversationId: string, messages: ImportedMessage[], scheduledTasks: ImportedScheduledTask[] = [], cutoverToken = "", activateScheduledTasks = false, todoSnapshot?: SharedTodoCutoverSnapshot): ConversationImportResult {
        ...
        for (const task of scheduledTasks) {
            ...
            if (prior) {
                if (prior.sourceAgentId !== conversationId ||
            ...
            if (activateScheduledTasks) {
                ...
                if (!stored)
                    throw new Error("scheduled task import did not persist");
                if (stored.active)
    ...
    getConversation(sandboxId: string, conversationId: string): ImportedMessage[] {
        return (this.conversations.get(this.convKey(sandboxId, conversationId)) ?? []);
    }
    /** Read the Dedicated task receipts that back cutover assertions. */
    getScheduledTasks(sandboxId: string, conversationId: string): StoredScheduledTask[] {
    ...
    getTodos(sandboxId: string, conversationId: string): SharedTodoCutoverRecord[] {
        return structuredClone(this.todoSnapshots.get(this.convKey(sandboxId, conversationId))?.todos ??
            []);
    }
    /** Read the durable Todo replay authority materialized by Dedicated. */
    getTodoMutations(sandboxId: string, conversationId: string): SharedTodoMutationCutoverRecord[] {
        return structuredClone(this.todoSnapshots.get(this.convKey(sandboxId, conversationId))
            ?.mutations ?? []);
    }
    /**
     ...
     */
    appendConversationTurn(sandboxId: string, conversationId: string, text: string, clientMessageId?: string): {
        reply: string;
        replayed: boolean;
    } | null {
        const key = this.convKey(sandboxId, conversationId);
     ...
     */
    getConversationByAgent(agentId: string, conversationId: string): ImportedMessage[] {
        const sandbox = [...this.sandboxes.values()].find((s) => s.agentId === agentId);
        if (!sandbox)
            return [];
        return this.getConversation(sandbox.id, conversationId);
    }
    getScheduledTasksByAgent(agentId: string, conversationId: string): StoredScheduledTask[] {
        const sandbox = [...this.sandboxes.values()].find((candidate) => candidate.agentId === agentId);
        if (!sandbox)
            return [];
        return this.getScheduledTasks(sandbox.id, conversationId);
    }
    getTodosByAgent(agentId: string, conversationId: string): SharedTodoCutoverRecord[] {
        const sandbox = [...this.sandboxes.values()].find((candidate) => candidate.agentId === agentId);
        if (!sandbox)
            return [];
        return this.getTodos(sandbox.id, conversationId);
    }
    getTodoMutationsByAgent(agentId: string, conversationId: string): SharedTodoMutationCutoverRecord[] {
        const sandbox = [...this.sandboxes.values()].find((candidate) => candidate.agentId === agentId);
        if (!sandbox)
            return [];
    ...
    stuckProvisioningSandboxes(cutoff: Date): Sandbox[] {
        return [...this.sandboxes.values()].filter((s) => s.status === "provisioning" && s.createdAt.getTime() < cutoff.getTime());
    }
