...
import { SYNTHETIC_ENVIRONMENT_LEASE_VERSION } from "@elizaos/core/contracts/synthetic-environment-lease";
import { SYNTHETIC_ENVIRONMENT_NAMESPACE_MAX_LENGTH } from "@elizaos/core/contracts/synthetic-environment-lease";
import { chmodSync } from "node:fs";
import { isSyntheticEnvironmentNamespace } from "@elizaos/core/contracts/synthetic-environment-lease";
import { lstatSync } from "node:fs";
import { mkdirSync } from "node:fs";
import { randomUUID } from "node:crypto";
import { type AcquireSyntheticEnvironmentLeaseInput } from "@elizaos/core/contracts/synthetic-environment-lease";
import { type RefreshSyntheticEnvironmentLeaseInput } from "@elizaos/core/contracts/synthetic-environment-lease";
import { type SyntheticEnvironmentLeaseAuthority } from "@elizaos/core/contracts/synthetic-environment-lease";
import { type SyntheticEnvironmentLeaseOwner } from "@elizaos/core/contracts/synthetic-environment-lease";
import { type SyntheticEnvironmentLeaseReceipt } from "@elizaos/core/contracts/synthetic-environment-lease";
import { type SyntheticEnvironmentLeaseSnapshot } from "@elizaos/core/contracts/synthetic-environment-lease";
...
function storageFailure(message: string, namespace: string | null, cause: unknown): ElizaError {
    return new ElizaError(message, {
...
function validateNamespace(value: unknown, field: string): string {
    if (!isSyntheticEnvironmentNamespace(value)) {
        throw invalidInput(`${field} must contain 1-${SYNTHETIC_ENVIRONMENT_NAMESPACE_MAX_LENGTH} non-control characters`);
    }
    return value;
}
...
function validateAuthority(authority: SyntheticEnvironmentLeaseAuthority): SyntheticEnvironmentLeaseAuthority {
...
function rowOwner(row: LeaseRow): SyntheticEnvironmentLeaseOwner | null {
    if (row.owner_id === null || row.owner_host === null)
        return null;
    return {
...
function snapshot(row: LeaseRow, nowMs: number): SyntheticEnvironmentLeaseSnapshot {
    const status = row.lease_id === null
        ? "released"
...
function assertAuthorityMatches(row: LeaseRow | null, authority: SyntheticEnvironmentLeaseAuthority, nowMs: number): LeaseRow {
    if (row === null ||
        ...
        throw new ElizaError("Synthetic environment lease authority is stale, expired, or owned elsewhere", {
            code: "SYNTHETIC_LEASE_LOST",
            severity: "fatal",
            context: {
...
export class SqliteSyntheticEnvironmentLeaseStore implements SyntheticEnvironmentLeaseStore<Database> {
    ...
    constructor(databasePath: string) {
        ...
        if (process.platform !== "win32" && (parent.mode & 0o022) !== 0) {
            throw invalidInput("databasePath parent must not be writable by group or other users");
        }
        ...
        try {
            ...
            if (!existing.isFile() || existing.isSymbolicLink()) {
                throw invalidInput("databasePath must be a regular file, not a symbolic link");
            }
    ...
    close(): void {
        ...
        if (this.pendingTransactions > 0) {
            throw storageFailure("Cannot close synthetic lease storage while a transaction is pending", null, new Error("transaction in progress"));
        }
        this.closed = true;
        this.database.close(false);
    }
    async acquire(input: AcquireSyntheticEnvironmentLeaseInput): Promise<SyntheticEnvironmentLeaseReceipt> {
        ...
        return this.transaction(namespace, async (nowMs) => {
            ...
            const operation = current?.lease_id === null || current === null ? "acquire" : "recover";
            this.database
                .query(`
        ...
        `)
                .run(namespace, generation, leaseId, owner.ownerId, owner.processId, owner.host, nowMs, nowMs, nowMs + duration);
    ...
    async heartbeat(input: RefreshSyntheticEnvironmentLeaseInput): Promise<SyntheticEnvironmentLeaseReceipt> {
        if (typeof input !== "object" || input === null) {
        ...
        return this.transaction(authority.namespace, async (nowMs) => {
            assertAuthorityMatches(this.select(authority.namespace), authority, nowMs);
            this.database
                .query(`
        ...
        `)
            ...
            return this.receipt("heartbeat", this.requireRow(authority.namespace), nowMs);
        });
    }
    async rollover(input: RefreshSyntheticEnvironmentLeaseInput): Promise<SyntheticEnvironmentLeaseReceipt> {
        if (typeof input !== "object" || input === null) {
        ...
        const duration = validateDuration(input.leaseDurationMs);
        return this.transaction(authority.namespace, async (nowMs) => {
            const row = assertAuthorityMatches(this.select(authority.namespace), authority, nowMs);
            const leaseId = randomUUID();
            this.database
                .query(`
          UPDATE synthetic_environment_leases
          ...
          WHERE namespace = ?
        `)
                .run(row.generation + 1, leaseId, nowMs, nowMs, nowMs + duration, authority.namespace);
            return this.receipt("rollover", this.requireRow(authority.namespace), nowMs);
        });
    }
    async release(uncheckedAuthority: SyntheticEnvironmentLeaseAuthority): Promise<SyntheticEnvironmentLeaseReceipt> {
    ...
    async withActiveGeneration<T>(uncheckedAuthority: SyntheticEnvironmentLeaseAuthority, write: (database: Database) => T | Promise<T>): Promise<{
        value: T;
        ...
        return this.transaction(authority.namespace, async (nowMs) => {
            ...
            const row = assertAuthorityMatches(this.select(authority.namespace), authority, committedAt);
            return {
                value,
    ...
    private receipt(operation: SyntheticEnvironmentLeaseReceipt["operation"], row: LeaseRow, nowMs: number): SyntheticEnvironmentLeaseReceipt {
        return {
            operation,
            authority: authorityFromRow(row),
    ...
    private assertOpen(): void {
        if (this.closed) {
            throw storageFailure("Synthetic lease storage is closed", null, new Error("database closed"));
        }
    }
    private async transaction<T>(namespace: string, operation: (nowMs: number) => Promise<T>): Promise<T> {
        this.assertOpen();
        ...
        try {
            try {
                ...
                throw storageFailure("Synthetic lease transaction could not begin", namespace, error);
            }
            try {
                ...
                try {
                    ...
                    throw storageFailure("Synthetic lease transaction commit was not confirmed", namespace, error);
                }
                return result;
            }
            catch (error) {
                ...
                try {
                    ...
                    throw storageFailure("Synthetic lease transaction rollback was not confirmed", namespace, new AggregateError([error, rollbackError]));
                }
                throw error;
            }
        }
