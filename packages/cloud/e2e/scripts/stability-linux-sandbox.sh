...
# The caller supplies validated ports, canonical paths, caller identity, a private
# environment file outside retained artifacts, and the executable plus arguments.
# The wrapper consumes that file before creating the sandbox UID; descriptor 3
# carries the seccomp program only. Teardown removes UID processes before access.
...
run() {
  ...
  local environment_real output_real environment_dir
  ...
  environment_dir="$(/usr/bin/dirname -- "$environment_real")"
  [ "$environment_real" = "$environment_file" ] || die "sandbox environment must be canonical"
  [[ "$environment_dir" = */eliza-stability-env-* && "$environment_real" = "$environment_dir"/environment.bin ]] || die "sandbox environment is outside its private directory"
  [[ "$environment_real" != "$output_real"/* ]] || die "sandbox environment cannot enter artifact directory"
  [ "$(/usr/bin/stat -c %a "$environment_dir")" = "700" ] || die "sandbox environment directory mode must be 0700"
  [ "$(/usr/bin/stat -c %u "$environment_dir")" = "$caller_uid" ] || die "sandbox environment directory owner mismatch"
