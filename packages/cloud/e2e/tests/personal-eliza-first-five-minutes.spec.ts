...
>({
  // The strict model fixture binds before the stack boots, so the Worker env
  // can name its ephemeral loopback route without any port collision.
  scriptedModel: [
    ...
    async ({}, use) => {
      const model = await startMockLlm({
        scenarioId: "personal-eliza-first-five-minutes",
        fixtures: [
          {
            name: "shared-notes-capability-wall",
            times: 1,
            match: (call) => {
...
test.describe("personal Eliza first five minutes", () => {
  test("keeps first contact rowless and free, replays it, then claims the same account and history", async ({
    ...
    expect(scriptedModel.diagnostics().unexpectedCalls).toEqual([]);
    scriptedModel.assertFixturesConsumed();
