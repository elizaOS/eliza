...
test.each([
  {
    name: "fetch",
    expression: 'await fetch("https://example.com/forbidden")',
    method: "GET",
  },
  {
    name: "preconnect",
    expression: 'fetch.preconnect("https://example.com/forbidden")',
    method: "PRECONNECT",
  },
])(
  "production Bun preload blocks child $name before external egress",
  async ({ expression, method }) => {
    ...
    try {
      ...
      const child = Bun.spawn(
        [process.execPath, "--preload", guard, "-e", expression],
        {
          cwd: directory,
          env: {
