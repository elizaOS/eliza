...
function parseArtifactManifest(value: unknown): CloudStabilityArtifactManifest {
  ...
  const { reportSha256, planOutputRoot, ...laneManifest } =
    unsignedArtifactManifest;
  if (
    typeof planOutputRoot !== "string" ||
    !path.isAbsolute(planOutputRoot) ||
    path.resolve(planOutputRoot) !== planOutputRoot
  ) {
    throw new Error("Cloud stability manifest plan root is not canonical");
  }
  if (
  ...
  if (
    manifestSha256 !==
    canonicalCloudStabilitySha256({ ...manifest, reportSha256, planOutputRoot })
  ) {
    throw new Error("Cloud stability manifest checksum does not match");
  }
  return { ...manifest, manifestSha256, reportSha256, planOutputRoot };
}
...
async function verifyCloudStabilityArtifactsAt(
  ...
  const report = parseCloudStabilityExecutionReport(
    reportValue,
    manifest,
    manifest.planOutputRoot,
  );
  return { report, manifest, reportSha256 };
}

/** Verifies retained bytes at the supplied directory against the sealed writer plan.
 * Writer paths establish logical identity only; relocated reads never access them. */
export async function verifyCloudStabilityArtifacts(
  outputRoot: string,
): Promise<VerifiedCloudStabilityArtifacts> {
  return verifyCloudStabilityArtifactsAt(outputRoot);
}
...
async function persistCloudStabilityArtifacts(
  ...
  const unsignedArtifactManifest = {
    ...manifest,
    reportSha256,
    planOutputRoot: path.resolve(outputRoot),
  };
  ...
  try {
    ...
    await verifyCloudStabilityArtifactsAt(stagingRoot);
