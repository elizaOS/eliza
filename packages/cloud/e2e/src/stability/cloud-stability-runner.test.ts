...
describe("Cloud stability manifest", () => {
  ...
  test("downloaded artifacts verify after the original writer directory is removed", async () => {
    const original = await mkdtemp(path.join(tmpdir(), "stability-writer-"));
    const downloaded = await mkdtemp(
      path.join(tmpdir(), "stability-download-"),
    );
    directories.push(original, downloaded);
    ...
    const verified = await verifyCloudStabilityArtifacts(downloaded);
    expect(verified.report.status).toBe("failed");
    expect(verified.manifest.planOutputRoot).toBe(original);
    await writeFile(path.join(downloaded, "stability.json"), "{}");
    await expect(verifyCloudStabilityArtifacts(downloaded)).rejects.toThrow();
  });
