...
export async function writeSandboxEnvironment(
  ...
  const privateDirectory = await realpath(
    await mkdtemp(path.join(tmpdir(), "eliza-stability-env-")),
  );
  const relative = path.relative(await realpath(outputDir), privateDirectory);
  if (
    relative === "" ||
    ...
    throw new Error("sandbox environment must be outside the artifact tree");
  }
  const environmentPath = path.join(privateDirectory, "environment.bin");
  try {
    ...
    // error-policy:J2 Preserve failed private environment creation after cleanup.
