...
export function buildSharedEnv(
  ...
  return {
    ...
    // The fixture spawns cloud-api-e2e-server.ts directly, so it doesn't get
    // the `--var` injection cloud-api-dev.ts does — set it here instead.
    ELIZA_CF_REGISTRAR_DEV_STUB: process.env.ELIZA_CF_REGISTRAR_DEV_STUB ?? "1",
    // PGlite via TCP bridge (cloud-api-dev.ts handles this)
