...
to UID 0 only inside a new user namespace, with capabilities initially dropped, a new PID and
...
to overwrite a prior bundle. The sealed `planOutputRoot` preserves the writer's
logical attempt identities when an artifact is downloaded elsewhere; verification
reads files only from the supplied artifact directory.
