...
const localProfile = process.env.ELIZA_CLOUD_LOCAL_PROFILE?.trim();
if (
  localProfile !== undefined &&
  localProfile !== "staging" &&
  localProfile !== "production"
) {
  throw new Error(
    `ELIZA_CLOUD_LOCAL_PROFILE must be staging or production, received ${JSON.stringify(localProfile)}`,
  );
}
const outputPath = path.join(
  apiDir,
  localProfile ? `.dev.vars.${localProfile}` : ".dev.vars",
);
...
const localRuntimeOverrideKeys = new Set([
  ...
  "ELIZA_KMS_BACKEND",
  "ELIZA_LOCAL_DOCKER_HOST_SUFFIX",
  "ELIZA_LOCAL_DOCKER_PROVIDER",
...
const content = [
  ...
  `# Local${localProfile ? ` ${localProfile}` : ""} profile only. Do not commit.`,
...
console.log(
  `[sync-api-dev-vars] wrote ${path.relative(repoRoot, outputPath)} (${entries.length} keys)`,
);
