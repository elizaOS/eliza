 ...
 * test-cloud-run.ts precedent — so a `.cmd`-shimmed bun still resolves.
 */
  ...
  if (missing.length > 0) {
    console.error(
      `[run-bun-tests] quarantined suite(s) not found on disk:\n  ${missing.join("\n  ")}\n` +
        "Update DEFAULT_QUARANTINED_SUITES in scripts/run-bun-tests-helpers.ts to match the layout.",
    );
    return 1;
  }
