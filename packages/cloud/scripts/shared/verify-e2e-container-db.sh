...
#   bash ../scripts/shared/verify-e2e-container-db.sh
set -uo pipefail
...
bun run ../scripts/shared/_e2e-provision.ts > /tmp/e2e-dsns.json || { echo "provisioning FAILED"; cat /tmp/e2e-dsns.json; exit 1; }
