...
IMAGE_REF=$(REGISTRY=apps-local APP_ID="$APP_ID" CONTEXT=../scripts/shared/e2e-sample-app \
  bun run ../scripts/shared/_e2e-build.ts) || { echo "build FAILED"; exit 1; }
...
bun run ../scripts/shared/_e2e-provision.ts > /tmp/e2e-dsns.json || { echo "provision FAILED"; cat /tmp/e2e-dsns.json; exit 1; }
