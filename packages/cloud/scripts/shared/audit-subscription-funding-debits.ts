...
import type { SubscriptionDebitSignal } from "../../shared/src/lib/services/subscription-funding-policy";
...
const SIGNAL_PATTERNS: Readonly<Record<SubscriptionDebitSignal, RegExp>> = {
  ...
  raw_credit_transaction_sql_insert:
    /\bINSERT\s+INTO\s+"?credit_transactions"?/gi,
...
function listProductionTypeScript(directory: string): string[] {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      const relativePath = relative(CLOUD_ROOT, absolutePath)
        .split(sep)
        .join("/");
    ...
    listProductionTypeScript(CLOUD_ROOT)
      .map((relativePath) => [
        ...
        scanSubscriptionDebitSignals(
          readFileSync(resolve(CLOUD_ROOT, relativePath), "utf8"),
        ),
      ])
...
if (import.meta.main) {
  process.stdout.write(
    `${JSON.stringify(scanProductionSubscriptionDebitInventory(), null, 2)}\n`,
  );
}
