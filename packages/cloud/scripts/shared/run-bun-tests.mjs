...
const packageDir = path.resolve(here, "../../shared");
const repoRoot = path.resolve(packageDir, "../../..");
const rawPassthroughArgs = process.argv.slice(2);
const passthroughArgs = withDefaultTestTimeout(rawPassthroughArgs);

const UPSTREAM_TEMPLATE =
  "packages/cloud/scripts/shared/bun-pglite-crash-upstream-report.md";
...
const PGLITE_SOURCE_PATTERN =
  /(?:@electric-sql\/pglite|pglite:\/\/|\bPGlite\b)/i;
...
function discoverTestFiles() {
  ...
  if (process.env.ELIZA_BUN_TEST_FILES_JSON) {
    ...
    if (
      ...
      throw new Error(
        "[run-bun-tests] ELIZA_BUN_TEST_FILES_JSON must be a non-empty string array",
      );
    }
    if (new Set(parsed).size !== parsed.length) {
      throw new Error(
        "[run-bun-tests] ELIZA_BUN_TEST_FILES_JSON contains duplicate test files",
      );
    }
    const missing = parsed.filter((file) => !existsSync(resolveTestFile(file)));
    if (missing.length > 0) {
      throw new Error(
        `[run-bun-tests] sharded test file(s) not found:\n  ${missing.join("\n  ")}`,
      );
    }
    return [...parsed].sort();
  }
  ...
  const visit = (directory) => {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      if (entry.isDirectory()) {
        if (!DISCOVERY_SKIP_DIRECTORIES.has(entry.name))
...
function isPgliteTestFile(file) {
  return PGLITE_SOURCE_PATTERN.test(
    readFileSync(resolveTestFile(file), "utf8"),
  );
}
  ...
  if (bin) {
    ...
    if (env.ELIZA_BUN_TEST_BIN_ARGS) {
      ...
      if (
        ...
        prefixArgs.some((a) => typeof a !== "string")
      ) {
        throw new Error(
          "[run-bun-tests] ELIZA_BUN_TEST_BIN_ARGS must be a JSON array of strings",
        );
      }
    }
...
function writeCrashCapture({ attempt, maxAttempts, args, result, reason }) {
  ...
  const file = path.join(
    dir,
    `tenant-db-pglite-crash-${stamp}-attempt${attempt}.log`,
  );
  const header = [
...
async function main() {
  ...
  const shardingOn =
    resolveTestShardingMode(process.env) &&
    !hasExplicitTestFileFilter(rawPassthroughArgs);
  ...
  if (!quarantineOn && !shardingOn) {
    ...
    if (result.signal) {
      console.error(
        `[run-bun-tests] bun test terminated by signal ${result.signal}`,
      );
      return 1;
    }
  ...
  const quarantinedSuites = quarantineOn
    ? resolveQuarantinedSuites(process.env)
    : [];
  const maxAttempts = quarantineOn ? resolveMaxAttempts(process.env) : 1;
  const attemptTimeoutMs = quarantineOn
    ? resolveAttemptTimeoutMs(process.env)
    : 0;

  // Fail loud on a stale quarantine list (e.g. a renamed suite) — otherwise
  // the quarantined pass would silently run nothing (#13620).
  const missing = quarantinedSuites.filter(
    (suite) => !existsSync(path.join(packageDir, suite)),
  );
  if (missing.length > 0) {
    console.error(
  ...
  if (shardingOn) {
    ...
    const mainFiles = discovered.filter(
      (file) =>
        !quarantinedSuites.some((suite) =>
          matchesPackageRelativeFile(file, suite),
        ),
    );
    ...
    const quarantinedBasenames = quarantinedSuites.map((suite) =>
  ...
  const quarantineArgs = buildQuarantinePassArgs(
    quarantinedSuites,
    passthroughArgs,
  );
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const result = await runBunTest(quarantineArgs, {
