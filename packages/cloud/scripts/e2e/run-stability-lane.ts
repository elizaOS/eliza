...
import { SyntheticControlClient } from "@elizaos/testing/synthetic-control";
import { ScenarioStabilitySubprocessAdapter } from "@elizaos/testing/scenario-runner";
import { testOutputPath } from "../../../scripts/lib/test-output.ts";
import cloudStabilityScenario from "../../e2e/scenarios/cloud-stability-agent.scenario.ts";
import { authorityChildEnvironment } from "../../e2e/src/stability/cloud-stability-environment.ts";
import {
  type CloudStabilityMode,
  canonicalCloudStabilitySha256,
  parseCloudStabilityManifest,
  runCloudStabilityLane,
} from "../../e2e/src/stability/cloud-stability-runner.ts";
...
const outputRoot = path.resolve(
  option("output") ?? testOutputPath("cloud-stability", runId),
);
...
try {
  ...
  const adapter = new ScenarioStabilitySubprocessAdapter({
    ...
    args: () => [
      "--conditions=eliza-source",
      path.join(repoRoot, "packages/cloud/scripts/e2e/stability-attempt.ts"),
    ],
    cwd: outputRoot,
