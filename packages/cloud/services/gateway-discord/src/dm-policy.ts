 ...
 */
import { isDiscordDmSenderAllowed } from "@elizaos/shared";
import { type DiscordDmPolicyMetadata } from "@elizaos/shared";
...
export const INVALID_DISCORD_DM_POLICY_STATE: DiscordConnectionDmPolicyState = Object.freeze({ status: "invalid" });
function isRecord(value: unknown): value is Record<string, unknown> {
    return value !== null && typeof value === "object" && !Array.isArray(value);
}
function isDmPolicy(value: unknown): value is NonNullable<DiscordConnectionDmMetadata["dmPolicy"]> {
    return (value === "open" ||
        value === "disabled" ||
        value === "allowlist" ||
        value === "pairing");
...
function isSnowflakeArray(value: unknown): value is string[] {
    return (Array.isArray(value) &&
        value.every((entry) => typeof entry === "string" && DISCORD_SNOWFLAKE_PATTERN.test(entry)));
}
/** Normalize the untrusted assignment envelope; every malformed shape is invalid. */
export function parseDiscordConnectionDmPolicyState(value: unknown): DiscordConnectionDmPolicyState {
    if (!isRecord(value))
...
export function isDmSenderAllowed(state: DiscordConnectionDmPolicyState | undefined, authorId: string): boolean {
