...
| [U07] | **RESOLVED — `ghost-container-charges`:** the advertised deployment, image, storage, bandwidth, and extra-instance prices are removed from the product contract (catalogue constants and public docs); the static monthly-reference constant is gone and the shutdown-warning email derives its monthly display from the settled daily rate (x30). Only the audited daily CPU/memory/count rate remains, pinned by `pricing.test.ts`. Readmission of any removed charge requires idempotent metering with a receipt. | Constants advertised several charges, while the audited billing callers established only running CPU/memory/count proration. [N01] [N02] | Implemented by [#22957][U07]. [#22553][i22553] remains the adjacent billing-selection goal and is not duplicated. |
