...
src/
  ...
  lib/                     @elizaos/cloud-shared/lib — SERVER-ONLY services + use-cases
    services/              service modules, including OAuth and connector-binding policy
...
- **Connector bindings are the Cloud execution authority.** `agent_connector_bindings` joins a canonical `user_characters.id` to a platform credential. Public projections omit the credential locator. A binding selects a subset of the credential's granted Google products and capabilities and can be revoked without deleting the shared credential.
- **Google MCP is direct and per product.** Cloud runtime config fans a connected Google binding into the fixed official product endpoints and injects only a short-lived bearer resolved from the exact bound vault credential. There is no OAuth mode, execution mode, Google broker route, or raw refresh-token export.
- **Google's OAuth application registration is deployment-owned.** `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` identify the elizaOS Cloud product and come from trusted deployment secret bindings; never copy them into an organization/user vault. Per-user access and refresh tokens remain encrypted organization credentials, and the one-time PKCE verifier is a short-lived vault value.
