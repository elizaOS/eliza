...
describe.skipIf(!postgresUrl)("generic purchaser runtime with PostgreSQL and Stripe HTTP", () => {
  beforeAll(async () => {
    ...
    for (const tag of [
      ...
      "0408_app_billing_provider_verifications",
      "0409_app_billing_notification_endpoints",
      "0410_app_subscription_outbox_delivery",
      "0411_app_billing_webhook_recovery",
      "0412_app_billing_checkout_expiry",
      "0413_app_billing_membership_authority",
      "0414_app_billing_seat_mutations",
      ...
      "0421_app_billing_sales_fence",
      "0422_app_billing_refund_commands",
      ...
      "0427_app_billing_deletion_dispositions",
      "0428_app_billing_deletion_disposition_guards",
      "0429_app_billing_customer_closures",
      "0430_app_billing_customer_closure_guards",
      "0431_app_billing_resume_payment_progress",
      "0432_app_billing_paid_resume_progress",
      "0433_app_billing_deletion_checkout",
      "0434_app_billing_applied_revision",
      "0435_app_billing_completed_checkout",
      "0436_app_billing_deletion_cancellation",
      ...
      "0442_billing_financial_history_retention",
      "0443_app_billing_customer_terminal_obligations",
      "0444_app_billing_customer_command_shape",
      "0445_app_billing_terminal_buyer_commands",
      "0446_app_billing_customer_command_identity",
      "0447_app_billing_customer_execution_lease",
      "0448_app_billing_customer_completion_guard",
      "0449_app_billing_customer_command_guard",
      "0450_app_billing_refund_observations",
      "0451_app_billing_refund_recovery_authority",
      "0452_app_billing_refund_observation_guards",
      "0453_app_billing_customer_retention",
      "0454_app_billing_refund_actor_recovery",
      "0455_app_billing_observed_customer_retention",
      "0456_app_billing_refund_phase_obligations",
      "0457_app_billing_completion_owner_locks",
      "0458_app_billing_completion_scope_decisions",
      "0459_app_billing_completion_validations",
      "0460_app_billing_completion_inventory",
      "0461_app_billing_completion_validation_locks",
      "0462_app_billing_completion_validation_guards",
      "0463_app_billing_completion_commit_validation",
      "0464_app_billing_completion_mutation_fence",
      "0465_app_billing_customer_receipt_digests",
      "0466_app_billing_cancellation_evidence",
      "0467_app_billing_checkout_cleanup_receipts",
      "0468_app_billing_checkout_cleanup_evidence",
    ]) {
  ...
  test("app subscriptions stay out of developer organization billing reads", async () => {
    ...
    const { readPrimaryOrganizationSubscription } = await import(
      "../../db/repositories/account-billing-snapshot-subscription"
    );
    const primary = await dbWrite.transaction((tx) => readPrimaryOrganizationSubscription(tx, org));
    expect(primary).toEqual({ state: "none" });
    const { subscriptionEntitlementsRepository } = await import(
      "../../db/repositories/subscription-entitlements"
    );
    expect(await subscriptionEntitlementsRepository.find(org)).toMatchObject({
  ...
  test("Stripe deletion preserves shared purchaser billing and refuses unclosed last-administrator obligations", async () => {
    ...
    expect(await adapter.inspect(context)).toMatchObject({ state: "complete" });
    ...
    for (const outcome of ["paid", "void"] as const) {
      test(`${purchaserState} purchaser pending update ${outcome} recovers the original payment without repeating a purchase`, async () => {
        ...
        if (purchaserState === "deleting") {
          ...
          expect(await deletionAdapter.inspect(deletionContext)).toMatchObject({
            state: "complete",
          });
          expect(
            (
  ...
  for (const state of ["prepared", "open_checkout"] as const) {
    test(`shared purchaser deletion retires ${state} intent without creating a purchase`, async () => {
      ...
      expect(await adapter.inspect(context)).toMatchObject(expected);
      expect(await adapter.inspect(context)).toMatchObject(expected);
      expect(fixture.requests.filter((row) => row.method === "POST")).toHaveLength(
