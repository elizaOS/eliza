...
import { AGENT_SNAPSHOT_CAPTURE_TRANSIENT_CODE } from "./contracts.js";
...
import { MAX_BACKUPS } from "./contracts.js";
import { MAX_RESTORABLE_AGENT_BACKUP_BYTES } from "@elizaos/core/agent-backup-limits";
import { SNAPSHOT_AUTHORITY_CHANGED } from "./authority.js";
import { SNAPSHOT_CAPTURE_TRANSIENT } from "./contracts.js";
import { SNAPSHOT_ENDPOINT_UNSUPPORTED } from "./contracts.js";
import { SNAPSHOT_FETCH_TIMEOUT_MS } from "./contracts.js";
import { SNAPSHOT_MAX_RAW_BYTES } from "./transfer-limits.js";
import { SNAPSHOT_RESTORE_TIMEOUT_MS } from "./contracts.js";
import { SandboxTransport } from "../bridge/transport.js";
import { SnapshotAuthorityCapture } from "./authority.js";
import { SnapshotPayloadTooLargeError } from "@elizaos/core/agent-backup-limits";
import { SnapshotResult } from "./contracts.js";
import { agentSandboxBackups } from "../../../../db/schemas/agent-sandboxes";
import { agentSandboxes } from "../../../../db/schemas/agent-sandboxes";
import { agentSandboxesRepository } from "../../../../db/repositories/agent-sandboxes";
import { and } from "drizzle-orm";
import { assertSnapshotExpandedBudgets } from "./transfer-limits.js";
import { computeStateHash } from "../../agent-backup-diff";
import { dbWrite } from "../../../../db/helpers";
import { eq } from "drizzle-orm";
import { estimateDeltaBytes } from "../../agent-backup-diff";
import { hydrateAgentSandboxBackup } from "../../../../db/repositories/agent-sandboxes";
import { inArray } from "drizzle-orm";
import { incrementalChainDepth } from "../../agent-backup-diff";
import { logger } from "../../../utils/logger";
import { planIncrementalBackup } from "../../agent-backup-diff";
import { prepareAgentBackupInsertData } from "../../../../db/repositories/agent-sandboxes";
import { readBodyWithinBudget } from "./transfer-limits.js";
import { readErrorBodyExcerpt } from "./transfer-limits.js";
import { resolveBackupChainBytes } from "../../agent-backup-diff";
import { snapshotAuthorityRejection } from "./authority.js";
import { snapshotCaptureStillCanonical } from "./authority.js";
import { sql } from "drizzle-orm";
import { toWellFormedUnicode } from "@elizaos/core";
import { truncateWellFormed } from "@elizaos/core";
import { type AgentBackupSnapshotType } from "../../../../db/repositories/agent-sandboxes";
import { type AgentBackupStateData } from "../../../../db/schemas/agent-sandboxes";
import { type AgentSandbox } from "../../../../db/repositories/agent-sandboxes";
import { type AgentSandboxBackupMetadata } from "../../../../db/repositories/agent-sandboxes";
import { type NewAgentSandboxBackup } from "../../../../db/schemas/agent-sandboxes";
...
export interface SandboxBackupHost {
    ...
    getAgentForLifecycleMutation(tx: LifecycleTx, agentId: string, orgId: string): Promise<AgentSandbox | undefined>;
    fetchAgentApi(...args: Parameters<SandboxTransport["fetchAgentApi"]>): ReturnType<SandboxTransport["fetchAgentApi"]>;
    getSafeBridgeEndpoint(...args: Parameters<SandboxTransport["getSafeBridgeEndpoint"]>): ReturnType<SandboxTransport["getSafeBridgeEndpoint"]>;
    getAgentJsonHeaders(...args: Parameters<SandboxTransport["getAgentJsonHeaders"]>): ReturnType<SandboxTransport["getAgentJsonHeaders"]>;
}
export class SandboxBackup {
    constructor(private readonly host: SandboxBackupHost) { }
    // Snapshots
    async snapshot(agentId: string, orgId: string, type: AgentBackupSnapshotType = "manual"): Promise<SnapshotResult> {
        ...
        const persisted = await dbWrite.transaction(async (tx) => {
            ...
            const storedBackup = await this.persistAuthorizedSnapshotWithinTransaction(tx, current, orgId, type, plannedInput);
     ...
     */
    async buildBackupInput(sandboxRecordId: string, type: AgentBackupSnapshotType, stateData: AgentBackupStateData, sizeBytes: number): Promise<NewAgentSandboxBackup> {
            ...
            try {
                ...
                if (baseState) {
                    ...
                    if (plan.kind === "incremental") {
                        ...
                        logger.info("[agent-sandbox] Storing a full backup: an incremental would exceed the restorable chain budget", {
                            sandboxRecordId,
                            existingChainBytes,
                            deltaBytes,
    ...
    async listBackups(agentId: string, orgId: string, limit?: number): Promise<AgentSandboxBackupMetadata[]> {
    ...
    async fetchSnapshotState(rec: Pick<AgentSandbox, "id" | "bridge_url" | "health_url" | "node_id" | "bridge_port" | "web_ui_port" | "headscale_ip" | "sandbox_id" | "environment_vars">): Promise<{
    ...
    async persistAuthorizedSnapshotWithinTransaction(tx: LifecycleTx, rec: SnapshotAuthorityCapture, organizationId: string, type: AgentBackupSnapshotType, plannedInput: NewAgentSandboxBackup): Promise<StoredAgentSandboxBackup> {
        const [sandbox] = await tx
            .update(agentSandboxes)
            .set({ last_backup_at: new Date(), updated_at: new Date() })
            .where(and(eq(agentSandboxes.id, rec.id), eq(agentSandboxes.organization_id, organizationId), eq(agentSandboxes.status, "running"), eq(agentSandboxes.execution_tier, rec.execution_tier), sql `${agentSandboxes.pool_status} IS NULL`, sql `${agentSandboxes.deleted_at} IS NULL`, sql `${agentSandboxes.deletion_attempt_id} IS NULL`, sql `${agentSandboxes.sandbox_id} IS NOT DISTINCT FROM ${rec.sandbox_id}`, sql `${agentSandboxes.node_id} IS NOT DISTINCT FROM ${rec.node_id}`, sql `${agentSandboxes.container_name} IS NOT DISTINCT FROM ${rec.container_name}`, sql `${agentSandboxes.bridge_url} IS NOT DISTINCT FROM ${rec.bridge_url}`, sql `${agentSandboxes.health_url} IS NOT DISTINCT FROM ${rec.health_url}`, sql `${agentSandboxes.bridge_port} IS NOT DISTINCT FROM ${rec.bridge_port}`, sql `${agentSandboxes.web_ui_port} IS NOT DISTINCT FROM ${rec.web_ui_port}`, sql `${agentSandboxes.headscale_ip} IS NOT DISTINCT FROM ${rec.headscale_ip}`, eq(agentSandboxes.environment_revision, rec.environment_revision), eq(agentSandboxes.lifecycle_revision, rec.lifecycle_revision)))
    ...
    async persistSnapshotWithinTransaction(tx: LifecycleTx, sandboxRecordId: string, organizationId: string, type: AgentBackupSnapshotType, stateData: AgentBackupStateData, sizeBytes: number): Promise<{
        backupId: string;
        lifecycleRevision: number;
    }> {
        const [backup] = await tx
            .insert(agentSandboxBackups)
            .values(await prepareAgentBackupInsertData({
            sandbox_record_id: sandboxRecordId,
            snapshot_type: type,
            state_data: stateData,
            size_bytes: sizeBytes,
        ...
        const [sandbox] = await tx
            .update(agentSandboxes)
            .set({ last_backup_at: new Date(), updated_at: new Date() })
            .where(and(eq(agentSandboxes.id, sandboxRecordId), eq(agentSandboxes.organization_id, organizationId), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS])))
    ...
    async pushState(sandboxOrBridgeUrl: Pick<AgentSandbox, "id" | "bridge_url" | "health_url" | "node_id" | "bridge_port" | "web_ui_port" | "headscale_ip" | "sandbox_id" | "environment_vars"> | string, state: AgentBackupStateData, options?: {
        trusted?: boolean;
        ...
        const res = typeof sandboxOrBridgeUrl === "string"
            ? await fetch(await this.host.getSafeBridgeEndpoint(sandboxOrBridgeUrl, "/api/restore", options), {
                ...requestInit,
