...
test("failed adopted provision fences its exact generation and refuses stale ownership", async () => {
  ...
  await fixture.query(
    `INSERT INTO agent_sandboxes(id,organization_id,status,execution_tier,lifecycle_revision,environment_revision,sandbox_id,node_id,container_name,bridge_url,health_url,bridge_port,web_ui_port)
     VALUES($1,$2,'running','dedicated-always',1,1,$3,'failed-provision-node',$3,'http://fixture','http://fixture/health',2138,2139)`,
    [agentId, org, name],
  );
  const { agentSandboxesRepository: repository } = await import(
    "../../db/repositories/agent-sandboxes"
  );
  const adopted = await repository.findByIdAndOrg(agentId, org);
  if (!adopted) throw new Error("Missing adopted provision fixture");
  const handle = {
    sandboxId: name,
    bridgeUrl: "http://fixture",
    healthUrl: "http://fixture/health",
    metadata: {
      provider: "docker" as const,
      agentId,
      nodeId: "failed-provision-node",
      hostname: "fixture",
      containerName: name,
      bridgePort: 2138,
      webUiPort: 2139,
  ...
  const cleanup = new SandboxReplacementCleanup({
    ...
    isReplacementCleanupSweepEligibleTx:
      authority.isReplacementCleanupSweepEligibleTx.bind(authority),
    getProvider: async () => {
      throw new Error("Fencing must not perform remote effects");
    },
  });
  const fence = () =>
    cleanup.persistAdoptedFailedProvisionCleanupFence(
      agentId,
      org,
      adopted,
      handle,
  ...
  await fixture.query("UPDATE agent_sandboxes SET lifecycle_revision=2 WHERE id=$1", [agentId]);
  await expect(fence()).rejects.toThrow(/generation changed/);
  expect(await repository.findByIdAndOrg(agentId, org)).toMatchObject({
    status: "running",
    sandbox_id: name,
    replacement_cleanup_container_id: null,
  });
  await fixture.query("UPDATE agent_sandboxes SET lifecycle_revision=1 WHERE id=$1", [agentId]);
  await expect(
    cleanup.persistAdoptedFailedProvisionCleanupFence(
      agentId,
      crypto.randomUUID(),
  ...
  expect(await repository.findByIdAndOrg(agentId, org)).toMatchObject({
    ...
    bridge_port: null,
    web_ui_port: null,
    replacement_cleanup_container_id: handle.metadata.containerId,
    replacement_cleanup_attempt_id: handle.metadata.replacementAttemptId,
    replacement_cleanup_node_id: handle.metadata.nodeId,
  ...
  await expect(
    cleanup.persistAdoptedFailedProvisionCleanupFence(
      ...
      {
        ...handle,
        metadata: { ...handle.metadata, containerId: "c".repeat(64) },
      },
      "different candidate",
    ),
