...
export { type CloudCodingContainerSession, type CloudCodingContainerStatus, type CloudCodingPromotion, type CloudCodingSyncResult, type PromoteVfsToCloudContainerRequest, type PromoteVfsToCloudContainerResponse, type RequestCodingAgentContainerRequest, type RequestCodingAgentContainerResponse, type SyncCloudCodingContainerRequest, type SyncCloudCodingContainerResponse } from "@elizaos/core/contracts/cloud-coding-containers";
export { PromoteVfsToCloudContainerRequestSchema, RequestCodingAgentContainerRequestSchema, SyncCloudCodingContainerRequestSchema } from "@elizaos/core/contracts/cloud-coding-containers";
import { containersEnv } from "../config/containers-env";
import { describeImageReference } from "./containers/image-rollout-status";
import { type CloudCodingContainerSession } from "@elizaos/core/contracts/cloud-coding-containers";
import { type CloudCodingContainerStatus } from "@elizaos/core/contracts/cloud-coding-containers";
import { type CloudCodingPromotion } from "@elizaos/core/contracts/cloud-coding-containers";
import { type CloudCodingSyncResult } from "@elizaos/core/contracts/cloud-coding-containers";
import { type CloudVfsBundle } from "@elizaos/core/contracts/cloud-coding-containers";
import { type PromoteVfsToCloudContainerRequest } from "@elizaos/core/contracts/cloud-coding-containers";
import { type RequestCodingAgentContainerRequest } from "@elizaos/core/contracts/cloud-coding-containers";
import { type SyncCloudCodingContainerRequest } from "@elizaos/core/contracts/cloud-coding-containers";
...
function sourceSlug(request: RequestCodingAgentContainerRequest): string {
    ...
    return slugify(source?.projectId ?? source?.workspaceId ?? source?.snapshotId ?? request.promotionId, "workspace");
}
export function resolveCodingWorkspacePath(request: RequestCodingAgentContainerRequest, fallbackId = "workspace"): string {
    const fallback = `/workspace/${sourceSlug(request) || slugify(fallbackId, "workspace")}`;
    return normalizeContainerPath(trimOptional(request.workspacePath) ?? trimOptional(request.source?.rootPath) ?? fallback, fallback);
}
...
export function buildCodingContainerCreatePayload(request: RequestCodingAgentContainerRequest): CodingContainerCreatePayload {
    const workspacePath = resolveCodingWorkspacePath(request);
    const projectName = slugify(request.container?.name ?? request.promotionId ?? request.source?.projectId, `coding-${request.agent}`);
    const image = trimOptional(request.container?.image) ??
    ...
    if (promotionId)
        environmentVars.ELIZA_CODING_PROMOTION_ID = promotionId;
    if (prompt)
        environmentVars.ELIZA_CODING_PROMPT = prompt;
 ...
 */
export function isCodingContainerImageAllowed(image: string, allowlist: readonly string[]): boolean {
...
function normalizeStatus(value: unknown): CloudCodingContainerStatus {
    ...
    if (value === "pending" || value === "deploying")
        return "pending";
    return "requested";
}
export function buildCodingContainerSessionResponse({ request, createPayload, upstreamData, now = new Date(), }: CodingContainerSessionBuildInput): CloudCodingContainerSession {
...
export function buildCodingPromotionResponse(request: PromoteVfsToCloudContainerRequest, options: CodingContainerPromotionBuildOptions = {}): CloudCodingPromotion {
    const promotionId = options.id ?? randomId("ccpromo");
    const sourceName = request.source.projectId ?? request.source.workspaceId ?? request.source.snapshotId;
    const workspacePath = trimOptional(request.target?.workspacePath) ??
...
export function buildCodingSyncResponse(containerId: string, request: SyncCloudCodingContainerRequest, options: CodingContainerSyncBuildOptions = {}): CloudCodingSyncResult {
    return {
