...
describe("reconcile refund-then-throw + settler re-invoke (#11512)", () => {
  ...
  test("settler fallback after mid-reconcile throw applies exactly ONE refund (no mint)", async () => {
    ...
    expect(refunds[0]?.stripe_payment_intent_id).toBe("reconcile-refund:req-11512-settler");
  ...
  test("reconcile refund replay (no fault) refunds the org exactly once — ALS key fallback", async () => {
    if (!pgliteReady) return;
    const { appId, payerUserId, payerOrgId } = await seed();

    // No explicit metadata key: the chargeKey must fall back to the
    // per-request ALS idempotency key, exactly like the earnings dedup.
    const runReconcile = () =>
      runWithRequestContext({ idempotencyKey: "req-11512-als" }, async () => {
    ...
    await runWithRequestContext({ idempotencyKey: "req-11512-als" }, async () => {
    ...
    // Full settlement replay of the SAME request: the refund must dedupe
    // (before #11512 the org was credited 0.04 again → 100.02).
    await runReconcile();
    expect(await orgBalance(payerOrgId)).toBeCloseTo(99.98, 6);
    const refunds = await orgTransactions(payerOrgId, "refund");
    expect(refunds).toHaveLength(1);
    expect(refunds[0]?.stripe_payment_intent_id).toBe("reconcile-refund:req-11512-als");
  });
  ...
  test("overage-charge replay debits the org exactly once (symmetric key)", async () => {
    ...
    const reconcileOverage = () =>
      appCreditsService.reconcileCredits({
        ...
        metadata: { idempotencyKey: "req-11512-overage" },
      });
    ...
    const overageRows = (await orgTransactions(payerOrgId, "debit")).filter(
      (row) => row.stripe_payment_intent_id === "reconcile-charge:req-11512-overage",
    );
