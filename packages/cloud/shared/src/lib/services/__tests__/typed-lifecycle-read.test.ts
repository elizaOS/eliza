...
describe("typed lifecycle reads and exact sandbox generations", () => {
  ...
  test(
    "disconnected recovery loses to a same-status ABA after its bridge observation",
    async () => {
      const { orgId, userId } = await seedOwner();
      const seeded = await seedRunningAgent(orgId, userId);
      const fixedTimestamp = new Date("2026-07-30T12:00:00.123Z");
      const [observed] = await dbWrite
        .update(agentSandboxes)
        .set({
          status: "disconnected",
          bridge_url: "http://127.0.0.1:2138",
          updated_at: fixedTimestamp,
        })
        .where(eq(agentSandboxes.id, seeded.id))
        .returning();

      await dbWrite.execute(sql`
        UPDATE ${agentSandboxes}
        SET error_count = error_count + 1, updated_at = ${fixedTimestamp}
      ...
      const current = await agentSandboxesRepository.findByIdAndOrg(observed.id, orgId);
  ...
  test(
    "stuck-provisioning recovery loses to a same-status ABA after its health observation",
    async () => {
      const { orgId, userId } = await seedOwner();
      const seeded = await seedRunningAgent(orgId, userId);
      const fixedTimestamp = new Date("2026-07-30T12:00:00.123Z");
        ...
        observed = (
          await dbWrite
            ...
            .set({
              bridge_url: `http://127.0.0.1:${port}`,
              health_url: `http://127.0.0.1:${port}`,
              sandbox_id: "agent-sleep-fence",
              node_id: "test-node",
              container_name: "agent-sleep-fence",
              bridge_port: port,
              headscale_ip: "127.0.0.1",
            })
            .where(eq(agentSandboxes.id, sandbox.id))
  ...
  test(
    ...
    async () => {
      ...
      if (!afterRawWrite) throw new Error("sandbox disappeared after raw lifecycle write");
      ...
      const fresh = await agentSandboxesRepository.update(
        sandbox.id,
        { status: "sleeping" },
        { ...observed, lifecycleRevision: afterRawWrite.lifecycle_revision },
      );
