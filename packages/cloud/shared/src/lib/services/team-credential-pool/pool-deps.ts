...
import { type LinkedAccountHealth } from "@elizaos/core/contracts/service-routing";
import { type PoolProviderId } from "./account-pool-contract";
...
export class DrizzleAccountPoolDeps implements AccountPoolDeps {
    ...
    async writeAccount(account: LinkedAccountConfig): Promise<void> {
        ...
        const updated = await pooledCredentialsRepository.updatePoolStateForOrganization(account.id, this.organizationId, {
    ...
    async deleteAccount(providerId: PoolProviderId, accountId: string): Promise<void> {
        const row = this.rowsById.get(accountId) ??
            (await pooledCredentialsRepository.findByIdForOrganization(accountId, this.organizationId));
        const deleted = await pooledCredentialsRepository.deleteForOrganization(accountId, this.organizationId);
