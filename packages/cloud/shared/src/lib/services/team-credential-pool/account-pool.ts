 ...
 */
import { type AccountPool } from "./account-pool-contract";
...
import { type SelectInput } from "./account-pool-contract";
import { type Strategy } from "./account-pool-contract";
...
export class TeamCredentialAccountPool implements AccountPool {
    ...
    private applyStrategy(strategy: Strategy, eligible: LinkedAccountConfig[], providerId: PoolProviderId): LinkedAccountConfig | null {
        if (eligible.length === 0)
            return null;
        ...
        switch (strategy) {
            ...
            case "quota-aware": {
                const underQuota = eligible.filter((account) => accountSessionPct(account) < QUOTA_AWARE_SKIP_PCT);
