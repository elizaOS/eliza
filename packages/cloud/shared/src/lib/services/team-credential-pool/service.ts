 ...
 */
import { POOLED_PROVIDER_SECRET_PROVIDER } from "./provider-map";
import { getTeamPoolRegistry } from "./registry";
import { isPooledDirectProvider } from "./provider-map";
import { isSubscriptionProviderId } from "./provider-map";
import { keyLast4 } from "./provider-map";
...
import { type AuditContext } from "../secrets/secrets";
...
import { type PooledCredentialWithContributor } from "../../../db/repositories/pooled-credentials";
...
function toSummary(row: PooledCredential, contributor?: {
    name: string | null;
} | null, callsToday = 0): PooledCredentialSummary {
    return {
        id: row.id,
...
export async function contributePooledCredential(params: ContributePooledCredentialParams): Promise<PooledCredentialSummary> {
    const provider = params.provider.trim();
    if (isSubscriptionProviderId(provider)) {
        throw new TeamCredentialPoolError("Subscription accounts (Claude Max / ChatGPT) cannot be pooled — only provider API keys.");
    }
    ...
    if (!probe.ok) {
        throw new TeamCredentialPoolError(`Key failed live validation against ${provider} (status ${probe.status}). Not added.`);
    }
    ...
    try {
        ...
        await secretsService
            ...
            logger.warn("[TeamCredentialPool] orphaned-secret cleanup failed after row insert failure", {
                organizationId: params.organizationId,
                secretId: secret.id,
...
export async function listPooledCredentials(organizationId: string): Promise<PooledCredentialSummary[]> {
    ...
    return rows.map((row: PooledCredentialWithContributor) => toSummary(row, { name: row.contributor_name }, todayTotals.get(row.id) ?? 0));
}
export async function getPooledCredential(id: string, organizationId?: string): Promise<PooledCredential | undefined> {
...
export async function updatePooledCredential(params: UpdatePooledCredentialParams): Promise<PooledCredentialSummary> {
    const updated = await pooledCredentialsRepository.updatePoolStateForOrganization(params.credentialId, params.organizationId, {
...
export async function removePooledCredential(params: {
    ...
    const row = await pooledCredentialsRepository.deleteForOrganization(params.credentialId, params.organizationId);
    if (!row) {
        throw new TeamCredentialPoolError("Credential not found", 404);
    }
    getTeamPoolRegistry().invalidate(params.organizationId);
