...
function hasLegacyDefaultBio(config: Record<string, unknown>): boolean {
    ...
    return knownDefaults.some((known) => bio.length === known.length && bio.every((entry, index) => entry === known[index]));
}
/**
 ...
 */
export function withDefaultAgentCharacter(agentConfig?: Record<string, unknown> | null): Record<string, unknown> {
    const config = asRecord(agentConfig) ?? {};
    if (agentConfigHasCharacter(config) && !hasLegacyDefaultBio(config))
        return { ...config };
    const carried = Object.fromEntries(Object.entries(config).filter(([key]) => !(PERSONA_KEYS as readonly string[]).includes(key)));
