...
export async function runSealedPromotion(input: {
  ...
  try {
    ...
    for (const [index, batch] of batches.entries()) {
      await postJson(fetchImpl, new URL("/api/memory/transfer/stage", target).toString(), headers, {
        seal,
        batch_index: index,
    ...
    return { epoch: active.epoch, seal, published, skipped_existing: skipped, resumed };
  } catch (error) {
