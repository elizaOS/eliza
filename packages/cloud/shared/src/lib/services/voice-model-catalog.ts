...
export async function signVoiceModelCatalog(args: {
    ...
    const key = await crypto.subtle.importKey("pkcs8", toArrayBufferView(pkcs8), { name: "Ed25519" }, false, ["sign"]);
    const sig = await crypto.subtle.sign({ name: "Ed25519" }, key, toArrayBufferView(new TextEncoder().encode(args.bodyText)));
