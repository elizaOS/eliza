...
export const CLOUD_MEMORY_BIO = "Remembers what people care about, and months later she'll bring up the project, the worry, the trip.";
/**
...
const RECALL_REPLY = "Not seeing anything about your sister in my stored memories. Tell me again and I'll hold onto it this time.";
/**
 ...
 */
export function toCloudCharacterMessageExamples(groups: readonly (readonly PresetTurn[])[], agentName: string): Array<Array<{
    name: string;
    content: {
        text: string;
    };
}>> {
    return groups.map((group) => group.map((turn) => ({
        name: turn.user === "{{agentName}}"
            ? agentName
            : turn.user === "{{user1}}"
                ? "{{name1}}"
                : turn.user,
