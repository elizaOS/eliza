...
export type BrowserOriginCheck = {
    ok: true;
} | {
    ok: false;
    reason: string;
...
export function isPermittedElizaBrowserOrigin(origin: string | null, requestHost: string | null, isProduction: boolean): boolean {
    if (!origin)
        return false;
    if (ELIZA_BROWSER_ORIGIN_HOSTS.has(origin))
        return true;
    if (requestHost && origin === requestHost)
        return true;
    return !isProduction && LOCAL_DEV_ORIGIN_HOSTS.has(origin);
}
export function checkElizaMutatingRequestOrigin(req: RequestHeaderReader, isProduction: boolean): BrowserOriginCheck {
