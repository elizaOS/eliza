...
import { commitPersonalProvisionalHistoryConvergence } from "./services/shared-runtime/conversation-coordinator";
...
import { isValidE164 } from "./utils/phone-normalization";
...
import { preparePersonalProvisionalHistoryConvergence } from "./services/shared-runtime/conversation-coordinator";
import { releasePersonalProvisionalHistoryConvergence } from "./services/shared-runtime/conversation-coordinator";
import { settleOffResponsePath } from "./utils/settle-off-response-path";
import { type CommitPhoneTelegramConvergenceResult } from "../db/repositories/users";
import { type PersonalProvisionalHistoryConvergence } from "./services/shared-runtime/conversation-coordinator";
import { type PreparedPersonalProvisionalHistoryConvergence } from "./services/shared-runtime/conversation-coordinator";
...
function extractErrorMetadata(candidate: unknown): {
    ...
    return {
        ...
        constraint: typeof typedCandidate.constraint === "string" ? typedCandidate.constraint : undefined,
        detail: typeof typedCandidate.detail === "string" ? typedCandidate.detail : undefined,
        message: typeof typedCandidate.message === "string" ? typedCandidate.message : String(candidate),
    };
}
...
export function isUniqueViolation(error: unknown): boolean {
    if (!error || typeof error !== "object")
        return false;
    if (extractErrorMetadata(error).code === "23505")
        return true;
...
export function describeSyncError(error: unknown): string {
    ...
    const causeMeta = error && typeof error === "object" && "cause" in error
...
function isRecoverableStewardProjectionConflict(error: unknown): boolean {
    ...
    const hasExactStewardConstraint = errorMetadata.constraint === STEWARD_IDENTITY_UNIQUE_CONSTRAINT ||
...
function isOrganizationSlugConflict(error: unknown): boolean {
    ...
    const causeMetadata = "cause" in error ? extractErrorMetadata(error.cause) : { message: "" };
    const isUniqueViolation = errorMetadata.code === "23505" || causeMetadata.code === "23505";
    const hasExactSlugConstraint = errorMetadata.constraint === ORGANIZATION_SLUG_UNIQUE_CONSTRAINT ||
        causeMetadata.constraint === ORGANIZATION_SLUG_UNIQUE_CONSTRAINT;
    return isUniqueViolation && hasExactSlugConstraint;
}
async function recoverCanonicalStewardUser(expectedUserId: string, stewardUserId: string, context: "invite" | "signup", error: unknown): Promise<boolean> {
...
async function rollbackCreatedUserSafely(userId: string, context: "invite" | "signup", originalError: unknown): Promise<void> {
    try {
...
async function restorePreviousStewardUserIdSafely(userId: string, previousStewardUserId: string, originalError: unknown): Promise<void> {
    try {
...
function reportDeferredSignupProvisioningFailure(operation: DirectSignupProvisioningOperation["name"], userId: string, organizationId: string, error: unknown): void {
    const detail = describeSyncError(error);
    if (operation === "Steward tenant") {
        logger.warn(`[StewardSync] Eager Steward tenant provisioning failed for new org ${organizationId}; signup proceeds and agent-provision will retry: ${detail}`);
        return;
    }
    logger.error(`[StewardSync] Deferred ${operation} provisioning failed for new user ${userId} in org ${organizationId}: ${detail}`);
}
    ...
    await settleOffResponsePath(executionCtx, async () => {
        ...
        for (const [index, outcome] of outcomes.entries()) {
            if (outcome.status === "rejected") {
                reportDeferredSignupProvisioningFailure(operations[index].name, userId, organizationId, outcome.reason);
            }
        }
...
type CommittedPhoneTelegramConvergence = Extract<CommitPhoneTelegramConvergenceResult, {
...
async function completePhoneTelegramHistoryConvergence(input: {
    ...
    if (receipt.source_agent_id !== expectedSourceAgentId ||
    ...
    const completed = await usersRepository.markPhoneTelegramPersonalAccountAliasComplete(receipt.token);
    if (!completed) {
        throw new Error("Personal account convergence receipt disappeared during recovery");
    }
    return { ...user, organization };
}
type ConvergenceConflictStatus = Exclude<CommitPhoneTelegramConvergenceResult["status"], "committed" | "already_committed">;
function throwConvergenceConflict(status: ConvergenceConflictStatus): never {
    if (status === "phone_account_mature" ||
 ...
 */
async function findUserByStoredWalletAddress(walletAddress: string): Promise<UserWithOrganization | undefined> {
...
export async function syncUserFromSteward(params: StewardSyncParams): Promise<StewardSyncedUser> {
    ...
    if (params.telegramContinuation && !claimedTelegramUser) {
        let claim: Awaited<ReturnType<typeof import("./services/eliza-app/onboarding-chat").inspectTelegramPersonalAccountContinuation>>;
        try {
            const { inspectTelegramPersonalAccountContinuation } = await import("./services/eliza-app/onboarding-chat");
    ...
    if (verifiedPhone && !user) {
        ...
        if (promotion.status === "promoted" || promotion.status === "already_promoted") {
            ...
            try {
                ...
                logger.warn(`[StewardSync] Phone-account tenant provisioning failed for org ${promotion.organization.id}; sign-in proceeds: ${describeSyncError(error)}`);
            }
    ...
    if (user) {
        ...
        if (shouldUpdate) {
            try {
                ...
                if (!isUniqueViolation(error)) {
                    logger.error(`[StewardSync] Existing-user profile refresh failed for user ${user.id}: ${describeSyncError(error)}`);
                    throw error;
                }
                logger.error(`[StewardSync] Existing-user profile refresh conflicts with another row for user ${user.id} — continuing sign-in with the stored profile: ${describeSyncError(error)}`);
            }
        }
        ...
        if (user.organization_id && !claimedTelegramUser) {
            ...
            await settleOffResponsePath(params.executionCtx, async () => {
                try {
                    ...
                    logger.warn(`[StewardSync] Sign-in tenant self-heal failed for org ${organizationId}; sign-in proceeds and the next attempt retries: ${describeSyncError(error)}`);
                }
            });
        }
        return user;
    ...
    if (email) {
        ...
        if (pendingInvite) {
            ...
            try {
                ...
                if (!recovered) {
                    logger.error(`[StewardSync] Invited-user creation failed for ${stewardUserId}: ${describeSyncError(error)}`);
                    throw error;
                }
            }
            const userWithOrg = await usersService.getByStewardIdForWrite(stewardUserId);
            if (!userWithOrg) {
                throw new Error(`Failed to fetch newly created user (steward: ${stewardUserId}) after accepting invite`);
            }
    ...
    if (email) {
        ...
        if (existingByEmail && existingByEmail.steward_user_id !== stewardUserId) {
            logger.info(`[StewardSync] Linking Steward account for ${email}: ${existingByEmail.steward_user_id} → ${stewardUserId}`);
            ...
            try {
                ...
                logger.error(`[StewardSync] Identity projection upsert failed while email-linking user ${existingByEmail.id}: ${describeSyncError(error)}`);
                throw error;
            }
    ...
    if (walletAddress && !email) {
        ...
        if (existingByWallet && existingByWallet.steward_user_id !== stewardUserId) {
            logger.info(`[StewardSync] Linking Steward wallet account for ${walletAddress}: ${existingByWallet.steward_user_id} → ${stewardUserId}`);
            if (verifiedPhone) {
            ...
            if (!existingByWallet.wallet_verified ||
            ...
            try {
                ...
                await restorePreviousStewardUserIdSafely(existingByWallet.id, existingByWallet.steward_user_id, error);
                logger.error(`[StewardSync] Identity projection upsert failed while wallet-linking user ${existingByWallet.id}: ${describeSyncError(error)}`);
                throw error;
            }
            const linkedUser = await usersService.getByStewardIdForWrite(stewardUserId);
            if (!linkedUser) {
                throw new Error(`Failed to fetch user after Steward wallet account linking for ${walletAddress}`);
            }
            return linkedUser;
        }
    }
    ...
    const generateOrganizationSlug = (): string => {
        ...
        if (walletAddress)
            return generateSlugFromWallet(walletAddress);
        if (name)
    ...
    let createdUser: Awaited<ReturnType<typeof usersService.createFreshStewardSignupUser>> | undefined;
    try {
        ...
        if (isUniqueViolation(error)) {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                ...
                if (existingUser) {
                    ...
                    if (existingUser.steward_user_id !== stewardUserId) {
                        ...
                        logger.info(`[StewardSync] Linking Steward account for ${email}: ${existingUser.steward_user_id} → ${stewardUserId}`);
            ...
            logger.error(`[StewardSync] Duplicate key error but user (steward: ${stewardUserId}) not found after ${maxRetries} retries`);
            await organizationsService.delete(organization.id);
        }
        logger.error(`[StewardSync] Failed to create user for ${stewardUserId}: ${describeSyncError(error)}`);
        throw error;
    }
    if (!createdUser) {
    ...
    try {
        ...
        const recovered = await recoverCanonicalStewardUser(createdUser.id, stewardUserId, "signup", error);
        if (!recovered) {
            if (isInferenceRevocationBoundaryUnavailable(error)) {
                ...
                logger.warn(`[StewardSync] Fresh Steward binding activation unavailable; preserving recoverable identity for user ${createdUser.id}: ${describeSyncError(error)}`, { organizationId: organization.id });
                ...
                await organizationsService.delete(organization.id);
                logger.error(`[StewardSync] Identity projection upsert failed for new user ${createdUser.id}: ${describeSyncError(error)}`);
            }
            throw error;
        }
    }
 ...
 */
export async function ensureDefaultCharacter(userId: string, organizationId: string): Promise<void> {
