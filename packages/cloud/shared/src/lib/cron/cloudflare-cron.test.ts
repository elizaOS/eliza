...
describe("payment-request cron fanout", () => {
  ...
  test("dispatches the user-session telemetry cleanup on the daily retention schedule", async () => {
    const requests: Request[] = [];
    let work: Promise<unknown> | undefined;
    const scheduled = makeCronHandler(async (request) => {
      requests.push(request);
    ...
    await scheduled(
      { cron: "0 2 * * *", scheduledTime: Date.now() },
      {
        CRON_SECRET: "cron-secret",
        NEXT_PUBLIC_APP_URL: "https://api.example.test",
      } as never,
    ...
    const cleanupRequest = requests.find(
      (request) => new URL(request.url).pathname === "/api/cron/cleanup-user-sessions",
    );
    expect(cleanupRequest?.method).toBe("POST");
    expect(cleanupRequest?.headers.get("x-cron-secret")).toBe("cron-secret");
  });
});
