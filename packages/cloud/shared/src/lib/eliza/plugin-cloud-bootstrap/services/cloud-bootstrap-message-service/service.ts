...
import { RunTerminalOwner } from "@elizaos/core";
import { SINGLE_SHOT_TEMPLATE } from "./types";
import { TRANSPARENT_META_ACTIONS } from "../../types";
import { asUUID } from "@elizaos/core";
import { attachAvailableContexts } from "../../utils/context-routing";
import { cleanupLatestResponseId } from "../../utils/race-tracking";
...
import { getActiveRoutingContexts } from "../../utils/context-routing";
import { getAvailableActionNames } from "../../utils/native-planner-guards";
import { getContextRoutingFromMessage } from "../../utils/context-routing";
import { getLatestResponseId } from "../../utils/race-tracking";
...
import { isLatestResponseId } from "../../utils/race-tracking";
import { isStateValue } from "./types";
import { logger } from "@elizaos/core";
import { nativePlannerTemplate } from "../../templates/native-planner";
import { nativeResponseTemplate } from "../../templates/native-planner";
import { parseBooleanFromText } from "@elizaos/core";
import { parseContextRoutingMetadata } from "../../utils/context-routing";
import { parseNativePlannerDecision } from "../../utils/native-planner-guards";
import { parseStructuredModelObject } from "./retry";
import { refreshStateAfterAction } from "../../utils/state";
import { resolveActionPlannerStepModel } from "./model-resolution";
import { resolveResponseStepModel } from "./model-resolution";
import { resolveShouldRespondStepModel } from "./model-resolution";
import { runWithStreamingContext } from "@elizaos/core";
import { setContextRoutingMetadata } from "../../utils/context-routing";
import { setLatestResponseId } from "../../utils/race-tracking";
import { shouldRespondTemplate } from "../../templates/native-planner";
import { stripAugmentationForPersistence } from "@elizaos/core";
import { toNativeActionParams } from "../../utils/native-planner-guards";
import { truncateToCompleteSentence } from "@elizaos/core";
import { type CloudMessageOptions } from "../../types";
import { type Content } from "@elizaos/core";
import { type ContextRoutingDecision } from "../../utils/context-routing";
import { type HandlerCallback } from "@elizaos/core";
import { type IAgentRuntime } from "@elizaos/core";
import { type IMessageService } from "@elizaos/core";
import { type Media } from "@elizaos/core";
import { type Memory } from "@elizaos/core";
import { type MentionContext } from "@elizaos/core";
import { type MessageProcessingResult } from "./types";
...
import { type RuntimeWithEvaluators } from "./types";
import { type State } from "@elizaos/core";
import { type StrategyResult } from "../../types";
import { type UUID } from "@elizaos/core";
import { type ValidatedNativePlannerDecision } from "../../utils/native-planner-guards";
import { v4 } from "uuid";
import { validateNativePlannerDecision } from "../../utils/native-planner-guards";
import { withActionResultsMetadata } from "./types";
import { withRetry } from "./retry";
import { withScopedTextModel } from "./model-resolution";
export class CloudBootstrapMessageService implements IMessageService {
    private async evaluateShouldRespond(runtime: IAgentRuntime, message: Memory): Promise<{
        responseObject: Record<string, unknown> | null;
        routing: ContextRoutingDecision;
    }> {
        let evalState = await runtime.composeState(message, ["RECENT_MESSAGES", "CHARACTER", "ENTITIES"], true);
        ...
        logger.info("==============================================");
        const response = await withScopedTextModel("small", resolveShouldRespondStepModel(runtime), async () => await runtime.useModel(ModelType.TEXT_SMALL, {
    ...
    async handleMessage(runtime: IAgentRuntime, message: Memory, callback?: HandlerCallback, options?: CloudMessageOptions): Promise<MessageProcessingResult> {
        ...
        const responseId = v4();
        try {
            logger.info(`[CloudBootstrap] Message received from ${message.entityId} in room ${message.roomId}`);
            ...
            terminal = new RunTerminalOwner(runtime, runId, message, startTime, options?.roomHandlerLease);
            ...
            const timeoutPromise = new Promise<never>((_, reject) => {
                timeoutId = setTimeout(() => {
                    ...
                    reject(timeoutError);
                    const timeoutEmission = Promise.resolve().then(() => runtime.emitEvent(EventType.RUN_TIMEOUT, {
                        runtime,
                        runId,
                        messageId: message.id!,
                        roomId: message.roomId,
            ...
            const processingPromise = runWithStreamingContext({ ...getStreamingContext(), messageId: message.id, abortSignal: signal }, () => this.processMessage(runtime, message, callback, responseId, startTime, turnOptions));
            ...
            terminal?.request(signal.aborted
    ...
    private async processMessage(runtime: IAgentRuntime, message: Memory, callback: HandlerCallback | undefined, responseId: string, startTime: number, options?: CloudMessageOptions): Promise<MessageProcessingResult> {
        ...
        logger.debug(`[CloudBootstrap] Processing: ${truncateToCompleteSentence(message.content.text || "", 50)}...`);
        ...
        const isMuted = agentUserState === "MUTED" &&
        ...
        logger.debug(`[CloudBootstrap] shouldRespond: ${respondDecision.shouldRespond} (${respondDecision.reason})`);
        ...
        if (respondDecision.skipEvaluation) {
            ...
            logger.debug(`[CloudBootstrap] LLM decided: ${shouldRespondToMessage ? "RESPOND" : "IGNORE"}`);
        }
        ...
        const useNativePlanner = options?.useNativePlanner ??
        ...
        if (hasEvaluatorStorage) {
            ...
            try {
                await runtimeWithEvaluators.evaluate?.(message, state, true, async (content: Content) => {
                    if (responseContent) {
                ...
                logger.warn(`[CloudBootstrap] Evaluators failed after response generation: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        else {
     ...
     */
    private async runNativePlannerCore(runtime: IAgentRuntime, message: Memory, state: State, responseId: string, callback?: HandlerCallback, options?: CloudMessageOptions): Promise<StrategyResult> {
        ...
        const maxConsecutiveFailures = parseInt(String(runtime.getSetting("NATIVE_PLANNER_MAX_CONSECUTIVE_FAILURES") ?? "2"));
        ...
        try {
            ...
            const cachedStableData: State["data"] = accumulatedState.data.actionsData !== undefined
            ...
            const streamThinking = async (phase: string, content: string): Promise<void> => {
                if (options?.onReasoningChunk) {
                    await options.onReasoningChunk(content, phase as "planning" | "actions" | "response" | "thinking", message.id as UUID);
                }
            };
            while (iterationCount < maxIterations) {
                ...
                const actionOnlyState = await runtime.composeState(messageWithResults, ["ACTION_STATE", "CHARACTER"], true);
                ...
                logger.info(`========== LLM CALL: nativePlanner (iteration ${iterationCount}/${maxIterations}) ==========`);
                ...
                const maxParseRetries = parseInt(String(runtime.getSetting("NATIVE_PLANNER_PARSE_RETRIES") ?? "2"));
                let stepResultRaw = "";
                let parsedStep: ValidatedNativePlannerDecision | null = null;
                for (let parseAttempt = 1; parseAttempt <= maxParseRetries; parseAttempt++) {
                    try {
                        logger.debug(`[NativePlanner] Decision model call attempt ${parseAttempt}/${maxParseRetries}`);
                        stepResultRaw = await withScopedTextModel("small", resolveActionPlannerStepModel(runtime), async () => await runtime.useModel(ModelType.TEXT_SMALL, {
                            prompt,
                        }));
                        logger.info(`[LLM:nativePlanner] Response (attempt ${parseAttempt}):\n${stepResultRaw}`);
                        const rawParsedStep = parseNativePlannerDecision(stepResultRaw);
                        if (rawParsedStep) {
                            const validation = validateNativePlannerDecision(rawParsedStep, getAvailableActionNames(accumulatedState.data.actionsData));
                            if (validation.error) {
                                logger.warn(`[NativePlanner] Invalid planner output on attempt ${parseAttempt}/${maxParseRetries}: ${validation.error}`);
                        ...
                        if (parsedStep) {
                            ...
                            logger.warn(`[NativePlanner] Failed to parse planner JSON on attempt ${parseAttempt}/${maxParseRetries}`);
                            if (parseAttempt < maxParseRetries) {
                                const delay = getRetryDelay(parseAttempt);
                                await new Promise((resolve) => setTimeout(resolve, delay));
                            }
                        ...
                        logger.error(`[NativePlanner] Error during model call attempt ${parseAttempt}:`, errorMessage);
                        if (parseAttempt >= maxParseRetries) {
                            throw error;
                        }
                        const delay = getRetryDelay(parseAttempt);
                ...
                if (action && dedupKey === lastActionKey) {
                    logger.warn(`[NativePlanner] Duplicate action detected: ${action} with same params. Forcing completion.`);
                    traceActionResult.push({
                ...
                if (action === "FINISH") {
                    ...
                    logger.info(`[NativePlanner] Terminal response returned at iteration ${iterationCount}, response length: ${finishResponse.length}`);
                    await streamThinking("response", "\n--- Final response ---\n");
                    break;
                }
                if (action === "REPLY" || action === "NONE") {
                    const actionParams = parameters || {};
                    const replyText = typeof actionParams.response === "string"
                    ...
                    if (replyText) {
                        ...
                        logger.info(`[NativePlanner] ${action} treated as final response, length: ${replyText.length}`);
                    }
                    else {
                ...
                try {
                    ...
                    if (action && hasActionParams) {
                        ...
                        logger.info(`[NativePlanner] Stored parameters for ${action}: ${JSON.stringify(actionParams)}`);
                    }
                    ...
                    const result = await executePlannedToolCall(runtime, {
                    ...
                    const resultText = typeof result.text === "string" && result.text.length > 0
                        ? result.text
                    ...
                    accumulatedState = await refreshStateAfterAction(runtime, message, accumulatedState, traceActionResult);
                ...
                if (consecutiveFailures >= maxConsecutiveFailures) {
                    ...
                    logger.warn(`[NativePlanner] Failure budget exhausted after ${consecutiveFailures} consecutive failures`);
                    break;
                }
                // Compatibility fallback for older planners that set isFinish separately.
                if (isFinish) {
                    logger.info(`[NativePlanner] Task complete (isFinish fallback) at iteration ${iterationCount}`);
                    break;
                }
            }
            ...
            const summaryFreshState = await runtime.composeState(summaryMessageWithResults, [
                "RECENT_MESSAGES",
                "ACTION_STATE",
                "ACTIONS",
                "CHARACTER",
            ...
            logger.info("==============================================");
            const maxSummaryRetries = parseInt(String(runtime.getSetting("NATIVE_RESPONSE_PARSE_RETRIES") ?? "2"));
            let finalOutput = "";
            ...
            for (let summaryAttempt = 1; summaryAttempt <= maxSummaryRetries; summaryAttempt++) {
                try {
                    ...
                    finalOutput = await withScopedTextModel("large", resolveResponseStepModel(runtime), async () => await runtime.useModel(ModelType.TEXT_LARGE, {
                    ...
                    if (summary?.text) {
                        ...
                        logger.warn(`[NativePlanner] Failed to parse JSON summary on attempt ${summaryAttempt}/${maxSummaryRetries}`);
                    ...
                    const errorMessage = error instanceof Error ? error.message : String(error);
                    logger.error(`[NativePlanner] Summary generation error on attempt ${summaryAttempt}:`, errorMessage);
                    if (summaryAttempt >= maxSummaryRetries) {
    ...
    private async runNativeSinglePassCore(runtime: IAgentRuntime, message: Memory, state: State, callback?: HandlerCallback, options?: CloudMessageOptions): Promise<StrategyResult> {
        ...
        try {
            state = await runtime.composeState(message, ["RECENT_MESSAGES", "ACTIONS", "CHARACTER"], true);
            ...
            const parsedResponse = await withRetry(async () => {
                const response = String(await withScopedTextModel("small", resolveActionPlannerStepModel(runtime), async () => await runtime.useModel(ModelType.TEXT_SMALL, {
                    prompt,
                })));
                logger.info(`[LLM:singleShot] Response:\n${response}`);
                return parseStructuredModelObject(response);
            }, (result) => !!(result?.text || result?.thought), maxRetries, "singleShot");
            if (!parsedResponse) {
        ...
        return Promise.all(attachments.map(async (attachment) => {
            ...
            if (contentType.startsWith("image/")) {
                try {
                    ...
                    logger.warn(`[CloudBootstrap] Failed to generate image description for ${label}: ${error}`);
    ...
    async deleteMessage(runtime: IAgentRuntime, message: Memory): Promise<void> {
        ...
        logger.info(`[CloudBootstrap] Deleting memory for message ${message.id} from room ${message.roomId}`);
        await runtime.deleteMemory(message.id);
    }
    async clearChannel(runtime: IAgentRuntime, roomId: UUID, channelId: string): Promise<void> {
        logger.info(`[CloudBootstrap] Clearing message memories from channel ${channelId} -> room ${roomId}`);
        ...
        logger.info(`[CloudBootstrap] Cleared ${deletedCount}/${memories.length} memories from channel ${channelId}`);
    }
}
