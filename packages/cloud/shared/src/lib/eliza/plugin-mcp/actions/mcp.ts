...
function inferOpFromText(text: string): CloudMcpOp | null {
    ...
    if (/\b(connection|connected|oauth|connected services|integrations linked)\b/i.test(t) ||
...
async function handleReadResource(runtime: IAgentRuntime, message: Memory, state: State | undefined, callback?: HandlerCallback): Promise<ActionResult> {
    ...
    try {
        ...
        if (typeof params.serverName === "string" && typeof params.uri === "string") {
            ...
            parsedSelection = await withModelRetry<ResourceSelection>({
                ...
                createFeedbackPromptFn: (originalResponse, errorMessage, st, userMessage) => createResourceSelectionFeedbackPrompt(originalResponse as string, errorMessage, st, userMessage),
        ...
        const { resourceContent, resourceMeta } = processResourceResult(result as {
            contents: Array<{
                uri: string;
                mimeType?: string;
                text?: string;
...
async function handleSearchActions(runtime: IAgentRuntime, message: Memory, state: State | undefined): Promise<ActionResult> {
...
async function handleListConnections(runtime: IAgentRuntime, message: Memory, state: State | undefined, callback?: HandlerCallback): Promise<ActionResult> {
...
export const mcpAction: ActionWithParams = {
    ...
    description: "Single MCP entry point. op=read_resource reads an MCP resource; op=search_actions discovers new tool actions across connected platforms; op=list_connections lists OAuth connections.",
    similes: [
        // search_actions
        "SEARCH_ACTIONS",
        "FIND_ACTIONS",
    ...
    parameters: defineActionParameters({
        ...
        query: {
            type: "string",
            description: "For op=search_actions: keyword search query (use specific verbs and nouns from the request). For op=read_resource: optional natural-language description of the resource.",
            required: false,
        },
        platform: {
            type: "string",
            description: "For op=search_actions / list_connections: filter to a single platform name. Omit to search all.",
            required: false,
        },
        limit: {
            type: "number",
        ...
        offset: {
            type: "number",
            description: "For op=search_actions: skip first N results for pagination when initial search didn't find what you need.",
            required: false,
            default: 0,
        },
    }),
    validate: async (runtime: IAgentRuntime, message: Memory, state?: State) => {
        ...
        if (!svc)
            return false;
        if (!checkMcpOAuthAccess(runtime))
            return false;
