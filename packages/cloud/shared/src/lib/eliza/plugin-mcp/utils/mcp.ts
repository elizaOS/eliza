...
export function checkMcpOAuthAccess(runtime: IAgentRuntime, serverName?: string): boolean {
    ...
    if (!enabled.includes(serverName)) {
        logger.debug({ serverName, enabled }, "[MCP] OAuth check denied: server not in MCP_ENABLED_SERVERS");
        return false;
    }
    return true;
}
export async function createMcpMemory(runtime: IAgentRuntime, message: Memory, type: string, serverName: string, content: string, metadata: Record<string, unknown>): Promise<void> {
