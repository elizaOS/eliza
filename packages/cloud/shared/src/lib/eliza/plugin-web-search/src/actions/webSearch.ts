...
function toCallbackData(searchResponse: SearchResponse, reasoning: string, searchMetadata: Record<string, ContentValue>): Record<string, ContentValue> {
    return {
 ...
 */
async function extractSearchParams(runtime: IAgentRuntime, message: Memory, _state?: State): Promise<WebSearchParams> {
    ...
    if (messageParams?.query?.trim()) {
        logger.info({ src: "webSearch:extractParams", source: "message.params" }, "Using params from action message");
    ...
    const matchedStateParams = stateParamSources.find((candidate) => candidate.value && typeof candidate.value === "object" && !Array.isArray(candidate.value));
    ...
    try {
        ...
        const conversationContext = (typeof conversationLog === "string" && conversationLog.trim()) ||
            (typeof recentMessages === "string" && recentMessages.trim()) ||
            "";
        const contextSource = typeof conversationLog === "string" && conversationLog.trim()
            ? "conversationLog"
            : "recentMessages";
        ...
        if (parsed?.query) {
            ...
            logger.info({ src: "webSearch:extractParams", query: extractedParams.query }, "Extracted query via LLM");
            return extractedParams;
        }
    }
    catch (err) {
        logger.warn({ src: "webSearch:extractParams", error: (err as Error).message }, "LLM extraction failed, falling back to message text");
    }
    // Final fallback: use the message text directly as the query
    const messageText = message.content?.text?.trim();
    if (messageText) {
        logger.info({ src: "webSearch:extractParams", source: "messageText" }, "Using message text as query");
...
export const webSearch: Action & Record<string, unknown> = {
    ...
    description: "Search the web using hosted Google Search grounding via Gemini. Supports general web search and finance topics (crypto/DeFi/markets). Use when other actions/providers can't provide accurate or current info.\n\n" +
    ...
    parameters: defineActionParameters({
        ...
        search_depth: {
            type: "string",
            description: "Compatibility field from older search providers. Accepted but ignored by hosted Google search.",
            required: false,
        },
        time_range: {
            type: "string",
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory, _state?: State) => {
        try {
            ...
            logger.warn({ src: "webSearch:validate", error: (err as Error).message }, "WebSearchService not available");
            return false;
        }
    },
    handler: async (runtime: IAgentRuntime, message: Memory, state?: State, _options?: {
        ...
        try {
            ...
            const hasResults = Array.isArray(searchResponse?.results) && searchResponse.results.length > 0;
            if (searchResponse && (hasResults || searchResponse.answer)) {
                const responseList = searchResponse.answer
                    ? `${searchResponse.answer}${hasResults
