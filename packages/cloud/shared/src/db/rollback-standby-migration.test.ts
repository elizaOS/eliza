...
const migrationUrl = new URL("./migrations/0189_rollback_standby_state.sql", import.meta.url);
...
describe("0188 rollback standby state", () => {
  test("is registered once in the migration journal", () => {
    ...
    const rollbackStandbyEntries = journal.entries.filter(
      (entry) => entry.tag === "0189_rollback_standby_state",
    );
    ...
    expect(
      journal.entries.findIndex((entry) => entry.tag === "0189_rollback_standby_state"),
    ).toBeGreaterThan(
