...
import { REMOTE_COMMAND_CLOCK_SKEW_MS } from "@elizaos/core/contracts/remote-control";
import { REMOTE_COMMAND_MAX_TTL_MS } from "@elizaos/core/contracts/remote-control";
...
import { type EncryptedRemoteCommandEnvelope } from "@elizaos/core/contracts/remote-control";
import { type EncryptedRemoteCommandResultEnvelope } from "@elizaos/core/contracts/remote-control";
import { type EncryptedRemoteCommandStartReceiptEnvelope } from "@elizaos/core/contracts/remote-control";
import { type RemoteCommandEnvelope } from "../schemas/remote-command-envelopes";
import { type StoredRemoteControlEnvelope } from "../schemas/remote-command-envelopes";
...
function sameEnvelope(left: StoredRemoteControlEnvelope | null, right: StoredRemoteControlEnvelope): boolean {
    return (left !== null && canonicalizeRemoteControlValue(left) === canonicalizeRemoteControlValue(right));
}
...
function validateEnvelopeKind(envelope: StoredRemoteControlEnvelope, messageKind: StoredRemoteControlEnvelope["messageKind"]): void {
    ...
    if (envelope.senderKeyId !== (targetOriginated ? envelope.targetKeyId : envelope.controllerKeyId) ||
...
function envelopeMatchesCommand(envelope: StoredRemoteControlEnvelope, command: RemoteCommandEnvelope): boolean {
    return (envelope.ownerId === command.user_id &&
...
async function terminalizeSessionCommands(tx: DbTransaction, sessionId: string, now: Date, preStartStatus: "cancelled" | "expired"): Promise<void> {
    const commands = await tx
        ...
        .where(and(eq(remoteCommandEnvelopes.session_id, sessionId), inArray(remoteCommandEnvelopes.status, ["pending", "claimed", "started"])))
...
export class RemoteCommandEnvelopesRepository {
    ...
    async enqueue(input: {
        ...
        if (Number.isNaN(expiresAt.getTime()))
            throw invalidInput("envelope expiresAt is invalid");
        return this.database.transaction(async (tx) => {
            const [host] = await tx
                .select()
                .from(remoteHosts)
                .where(and(eq(remoteHosts.id, scope.targetRuntimeId), eq(remoteHosts.organization_id, input.organizationId), eq(remoteHosts.user_id, input.ownerId), eq(remoteHosts.status, "active")))
                .for("update");
            if (!host || host.runtime_key_id !== scope.targetKeyId)
                return { kind: "not_found" };
            const [session] = await tx
                .select()
                .from(remoteSessions)
                .where(and(eq(remoteSessions.id, scope.sessionId), eq(remoteSessions.grant_id, scope.grantId), eq(remoteSessions.grant_revision, scope.grantRevision), eq(remoteSessions.host_id, host.id), eq(remoteSessions.organization_id, input.organizationId), eq(remoteSessions.user_id, input.ownerId)))
                .for("update");
            if (!session ||
            ...
            if (session.status !== "active") {
                await terminalizeSessionCommands(tx, session.id, now, session.status === "expired" ? "expired" : "cancelled");
            ...
            const [existing] = await tx
                .select()
                .from(remoteCommandEnvelopes)
                .where(and(eq(remoteCommandEnvelopes.session_id, session.id), eq(remoteCommandEnvelopes.command_id, scope.commandId)))
                .limit(1);
            if (existing) {
                const duplicate = existing.sequence === input.envelope.sequence &&
            ...
            if (input.envelope.sequence <= session.last_sequence)
                return { kind: "replay" };
            if (input.envelope.sequence !== session.last_sequence + 1)
                return { kind: "sequence_gap" };
    ...
    async claimNext(input: {
        ...
        return this.database.transaction(async (tx) => {
            const [host] = await tx
                .select()
                .from(remoteHosts)
                .where(and(eq(remoteHosts.id, input.hostId), eq(remoteHosts.host_token_hash, tokenHash), eq(remoteHosts.status, "active")))
                .for("update");
            if (!host)
                return { kind: "not_found" };
            const now = await readPostLockDatabaseNow(tx);
            ...
            const [session] = await tx
                .select()
                .from(remoteSessions)
                .where(and(eq(remoteSessions.id, input.sessionId), eq(remoteSessions.host_id, host.id), eq(remoteSessions.organization_id, host.organization_id), eq(remoteSessions.user_id, host.user_id)))
                .for("update");
            if (!session)
                return { kind: "not_found" };
            if (session.status !== "active") {
            ...
            const staleStarted = await tx
                ...
                .from(remoteCommandEnvelopes)
                .where(and(eq(remoteCommandEnvelopes.session_id, session.id), eq(remoteCommandEnvelopes.status, "started"), sql `${remoteCommandEnvelopes.expires_at} <= ${now}`))
                .orderBy(asc(remoteCommandEnvelopes.sequence))
                .limit(100)
                .for("update", { skipLocked: true });
            if (staleStarted.length > 0) {
                await tx
                    .update(remoteCommandEnvelopes)
                    .set({ status: "execution_ambiguous", terminal_at: now, updated_at: now })
                    .where(inArray(remoteCommandEnvelopes.id, staleStarted.map((command) => command.id)));
            }
            await tx
                .update(remoteCommandEnvelopes)
                .set({
                ...
                .where(and(eq(remoteCommandEnvelopes.session_id, session.id), eq(remoteCommandEnvelopes.status, "claimed"), gt(remoteCommandEnvelopes.expires_at, now), sql `${remoteCommandEnvelopes.claim_expires_at} <= ${now}`));
            await tx
                .update(remoteCommandEnvelopes)
                .set({
                status: "expired",
                ...
                .where(and(eq(remoteCommandEnvelopes.session_id, session.id), inArray(remoteCommandEnvelopes.status, ["pending", "claimed"]), sql `${remoteCommandEnvelopes.expires_at} <= ${now}`));
            const [candidate] = await tx
                .select()
                .from(remoteCommandEnvelopes)
                .where(and(eq(remoteCommandEnvelopes.session_id, session.id), eq(remoteCommandEnvelopes.host_id, host.id), eq(remoteCommandEnvelopes.status, "pending"), gt(remoteCommandEnvelopes.expires_at, now)))
                .orderBy(asc(remoteCommandEnvelopes.sequence))
                .limit(1)
                .for("update", { skipLocked: true });
            if (!candidate)
            ...
            const claimExpiresAt = new Date(Math.min(now.getTime() + leaseMs, candidate.expires_at.getTime()));
            const [claimed] = await tx
                ...
                .where(and(eq(remoteCommandEnvelopes.id, candidate.id), eq(remoteCommandEnvelopes.status, "pending")))
                .returning();
            if (!claimed) {
    ...
    async recordStart(input: {
        ...
        return this.withHostSessionCommand(input, async ({ tx, command, now }) => {
            if (!envelopeMatchesCommand(input.startReceipt, command))
                return { kind: "not_found" };
            ...
            const [started] = await tx
                ...
                .where(and(eq(remoteCommandEnvelopes.id, command.id), eq(remoteCommandEnvelopes.status, "claimed"), eq(remoteCommandEnvelopes.claim_token, input.claimToken), eq(remoteCommandEnvelopes.attempts, input.claimAttempt)))
                .returning();
            if (!started)
                return { kind: "claim_lost" };
    ...
    async complete(input: {
        ...
        return this.withHostSessionCommand(input, async ({ tx, command, now }) => {
            if (!envelopeMatchesCommand(input.resultEnvelope, command))
                return { kind: "not_found" };
            ...
            if (command.expires_at.getTime() <= now.getTime()) {
                const [ambiguous] = await tx
                    .update(remoteCommandEnvelopes)
                    .set({ status: "execution_ambiguous", terminal_at: now, updated_at: now })
                    .where(and(eq(remoteCommandEnvelopes.id, command.id), eq(remoteCommandEnvelopes.status, "started")))
                    .returning();
                if (!ambiguous)
                    return { kind: "claim_lost" };
            ...
            const [completed] = await tx
                ...
                .where(and(eq(remoteCommandEnvelopes.id, command.id), eq(remoteCommandEnvelopes.status, "started"), eq(remoteCommandEnvelopes.claim_token, input.claimToken), eq(remoteCommandEnvelopes.attempts, input.claimAttempt)))
                .returning();
            if (!completed)
                return { kind: "claim_lost" };
    ...
    async readOwnedResult(input: {
        ...
        const [row] = await this.database
            ...
            .from(remoteCommandEnvelopes)
            .innerJoin(remoteSessions, and(eq(remoteSessions.id, remoteCommandEnvelopes.session_id), eq(remoteSessions.organization_id, remoteCommandEnvelopes.organization_id), eq(remoteSessions.user_id, remoteCommandEnvelopes.user_id)))
            .where(and(eq(remoteCommandEnvelopes.session_id, input.sessionId), eq(remoteCommandEnvelopes.command_id, input.commandId), eq(remoteCommandEnvelopes.organization_id, input.organizationId), eq(remoteCommandEnvelopes.user_id, input.ownerId)))
            .limit(1);
        return row?.command;
    }
    private async withHostSessionCommand<T>(input: {
        sessionId: string;
        commandId: string;
        hostId: string;
        hostToken: string;
        ...
        return this.database.transaction(async (tx) => {
            ...
            if (session.status !== "active" ||
                ...
                await terminalizeSessionCommands(tx, session.id, now, session.status === "active" || session.status === "expired" ? "expired" : "cancelled");
                return { kind: "not_found" };
            }
            const [command] = await tx
                .select()
                .from(remoteCommandEnvelopes)
                .where(and(eq(remoteCommandEnvelopes.session_id, session.id), eq(remoteCommandEnvelopes.command_id, input.commandId), eq(remoteCommandEnvelopes.host_id, host.id), eq(remoteCommandEnvelopes.organization_id, host.organization_id), eq(remoteCommandEnvelopes.user_id, host.user_id)))
                .for("update");
            if (!command)
                return { kind: "not_found" };
