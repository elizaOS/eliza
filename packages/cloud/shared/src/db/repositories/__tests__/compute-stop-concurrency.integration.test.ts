  ...
  await dbWrite.execute(
    sql.raw(`
      ...
      CREATE TABLE agent_sandboxes (
        ...
        lifecycle_revision bigint NOT NULL DEFAULT 0,
        billing_status text NOT NULL DEFAULT 'active', scheduled_shutdown_at timestamptz,
        execution_tier text NOT NULL DEFAULT 'shared', pool_status text,
        deletion_attempt_id uuid, deleted_at timestamptz,
...
realPostgres("compute stop concurrency", () => {
  ...
  test("manual cancellation and agent billing execution share lifecycle-first order", async () => {
    ...
    await dbWrite.execute(sql`INSERT INTO agent_sandboxes
      (id, organization_id, user_id, lifecycle_revision)
      VALUES (${agentId}, ${organizationId}, ${userId}, 5)`);
  ...
  test("terminal agent stop authority remains visible to the independent recovery scan", async () => {
    ...
    await dbWrite.execute(sql`INSERT INTO users(id, organization_id, steward_user_id)
