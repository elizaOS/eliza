...
async function createReplacementAttemptFixture(): Promise<ReplacementAttemptFixture> {
  ...
  const providerServerId = `1${String(Math.floor(Math.random() * 1e14)).padStart(14, "0")}`;
  const [nodeOccurrence] = await dbWrite
    ...
    .returning({ id: agentNodeIncarnationHistories.id });
  await dbWrite.insert(dockerNodes).values({
    ...
    provider_server_id: providerServerId,
    node_incarnation: nodeIncarnation,
    current_node_history_id: nodeOccurrence.id,
  });
  ...
  return {
    ...
    locator: {
      ...
      nodeRecordId,
      nodeIncarnation,
      nodeHistoryId: nodeOccurrence.id,
      nodeHostname,
      nodeSshPort: 22,
      nodeSshUser: "root",
