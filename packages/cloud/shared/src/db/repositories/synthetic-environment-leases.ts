...
import { type GuardedSyntheticEnvironmentWriteResult } from "@elizaos/core/contracts/synthetic-environment-lease";
import { type RefreshSyntheticEnvironmentLeaseInput } from "@elizaos/core/contracts/synthetic-environment-lease";
import { type SyntheticEnvironmentLease } from "../schemas/synthetic-environment-leases";
...
function snapshot(row: SyntheticEnvironmentLease, databaseNow: Date): SyntheticEnvironmentLeaseSnapshot {
...
function assertAuthorityMatches(row: SyntheticEnvironmentLease | undefined, authority: SyntheticEnvironmentLeaseAuthority, databaseNow: Date): SyntheticEnvironmentLease {
    if (!row ||
...
async function lockRow(tx: DbTransaction, namespace: string): Promise<SyntheticEnvironmentLease | undefined> {
    const [row] = await tx
        .select()
...
function receipt(operation: SyntheticEnvironmentLeaseReceipt["operation"], row: SyntheticEnvironmentLease, databaseNow: Date): SyntheticEnvironmentLeaseReceipt {
...
export class CloudSyntheticEnvironmentLeaseStore implements SyntheticEnvironmentLeaseStore<DbTransaction> {
    ...
    async heartbeat(input: RefreshSyntheticEnvironmentLeaseInput): Promise<SyntheticEnvironmentLeaseReceipt> {
        ...
        return dbWrite.transaction(async (tx) => {
            ...
            const [updated] = await tx
                ...
                .returning();
            if (!updated) {
                throw storageFailure("Synthetic lease heartbeat update was not committed", authority.namespace);
            }
    ...
    async rollover(input: RefreshSyntheticEnvironmentLeaseInput): Promise<SyntheticEnvironmentLeaseReceipt> {
        ...
        return dbWrite.transaction(async (tx) => {
            ...
            const [updated] = await tx
                ...
                .returning();
            if (!updated) {
                throw storageFailure("Synthetic lease rollover update was not committed", authority.namespace);
            }
    ...
    async release(uncheckedAuthority: SyntheticEnvironmentLeaseAuthority): Promise<SyntheticEnvironmentLeaseReceipt> {
        ...
        return dbWrite.transaction(async (tx) => {
            ...
            const [released] = await tx
                ...
                .returning();
            if (!released) {
                throw storageFailure("Synthetic lease release update was not committed", authority.namespace);
            }
            return {
                operation: "release",
    ...
    async withActiveGeneration<T>(uncheckedAuthority: SyntheticEnvironmentLeaseAuthority, write: (transaction: DbTransaction) => T | Promise<T>): Promise<GuardedSyntheticEnvironmentWriteResult<T>> {
        const authority = validateAuthority(uncheckedAuthority);
        return dbWrite.transaction(async (tx) => {
            ...
            const active = assertAuthorityMatches(await lockRow(tx, authority.namespace), authority, committedNow);
