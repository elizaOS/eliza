...
import { CONTAINER_BACKED_EXECUTION_TIERS } from "../schemas/agent-sandboxes";
...
import { PROVISIONING_RECONCILIATION_BATCH_SIZE } from "../../lib/services/provisioning-job-types";
...
import { UPGRADE_FAILURE_TARGET_MARKER_PREFIX } from "../schemas/agent-sandboxes";
import { WARM_POOL_ORG_ID } from "../schemas/agent-sandboxes";
import { WARM_POOL_USER_ID } from "../schemas/agent-sandboxes";
...
import { applyBackupDelta } from "../../lib/services/agent-backup-diff";
...
import { decryptAgentBackupStateData } from "../crypto/agent-backups";
import { deleteLegacyObject } from "../../lib/storage/object-store";
import { desc } from "drizzle-orm";
import { dockerNodes } from "../schemas/docker-nodes";
import { elizaProvisionAdvisoryLockSql } from "../../lib/services/eliza-provision-lock";
import { elizaTryProvisionAdvisoryLockSql } from "../../lib/services/eliza-provision-lock";
import { encryptAgentBackupStateData } from "../crypto/agent-backups";
import { ensureAgentSandboxSchema } from "../ensure-agent-sandbox-schema";
import { eq } from "drizzle-orm";
import { getObjectText } from "../../lib/storage/object-store";
import { gt } from "drizzle-orm";
import { gte } from "drizzle-orm";
import { imageRepo } from "../utils/docker-image-ref";
import { imageRepoSql } from "../utils/docker-image-ref";
import { inArray } from "drizzle-orm";
import { isDigestPinnedImageSql } from "../utils/docker-image-ref";
...
import { pinnedImageDigestSql } from "../utils/docker-image-ref";
import { randomUUID } from "node:crypto";
import { readOrganizationQuotaPolicyInTransaction } from "../../lib/services/organization-quota-policy";
import { requireBackupDelta } from "../../lib/services/agent-backup-diff";
import { requireBackupStateData } from "../../lib/services/agent-backup-diff";
import { requireOrganizationResourceLimit } from "../../lib/services/organization-quota-policy";
import { selectPrunableBackupIds } from "../../lib/services/agent-backup-diff";
import { sql } from "drizzle-orm";
import { sqlRows } from "../execute-helpers";
import { type AgentBackupSnapshotType } from "../schemas/agent-sandboxes";
import { type AgentBackupStateData } from "../schemas/agent-sandboxes";
import { type AgentBackupStoredStateData } from "../schemas/agent-sandboxes";
import { type AgentExecutionTier } from "../schemas/agent-sandboxes";
import { type AgentSandbox } from "../schemas/agent-sandboxes";
import { type AgentSandboxBackup } from "../schemas/agent-sandboxes";
import { type AgentSandboxStatus } from "../schemas/agent-sandboxes";
import { type BackupChainNode } from "../../lib/services/agent-backup-diff";
import { type Database } from "../client";
import { type DbTransaction } from "../client";
import { type NewAgentSandbox } from "../schemas/agent-sandboxes";
import { type NewAgentSandboxBackup } from "../schemas/agent-sandboxes";
import { type ProvisioningJobType } from "../../lib/services/provisioning-job-types";
import { type SQL } from "drizzle-orm";
import { type StoredAgentSandboxBackup } from "../schemas/agent-sandboxes";
export type { AgentBackupSnapshotType, AgentSandbox, AgentSandboxBackup, AgentSandboxStatus, NewAgentSandbox, NewAgentSandboxBackup, };
 ...
 */
export type AgentSandboxBackupMetadata = Pick<StoredAgentSandboxBackup, "id" | "sandbox_record_id" | "snapshot_type" | "state_data_storage" | "state_data_key" | "size_bytes" | "backup_kind" | "parent_backup_id" | "content_hash" | "verification_status" | "verified_at" | "verification_error" | "recovery_organization_id" | "recovery_agent_id" | "recovery_deletion_attempt_id" | "recovery_expires_at" | "created_at">;
/**
 ...
 */
export type WarmPoolRuntimeGeneration = Pick<AgentSandbox, "id" | "organization_id" | "status" | "environment_revision" | "sandbox_id" | "node_id" | "container_name" | "bridge_url" | "health_url" | "docker_image" | "image_digest" | "pool_ready_at">;
/**
 ...
 */
export type ProvisioningAdmissionCapture = Pick<AgentSandbox, "id" | "organization_id" | "status" | "lifecycle_job_id" | "lifecycle_execution_generation" | "execution_tier" | "pool_status" | "deleted_at" | "deletion_attempt_id" | "lifecycle_revision">;
/**
 ...
 */
export type ProvisioningRecoveryCapture = Pick<AgentSandbox, "id" | "organization_id" | "status" | "execution_tier" | "sandbox_id" | "node_id" | "container_name" | "bridge_url" | "health_url" | "headscale_ip" | "environment_revision" | "lifecycle_revision" | "lifecycle_job_id" | "lifecycle_execution_generation" | "pool_status" | "deleted_at" | "deletion_attempt_id">;
/** Exact row generation whose bridge was probed by disconnected recovery. */
export type DisconnectedRecoveryCapture = ProvisioningRecoveryCapture & Pick<AgentSandbox, "previous_image_digest" | "error_message">;
...
function backupVisibleToLegacyReaders(): SQL {
    return or(isNull(agentSandboxBackups.catalog_state), eq(agentSandboxBackups.catalog_state, "legacy_unmigrated")) as SQL;
}
...
async function getStoredBackupById(backupId: string, database: Database = dbWrite): Promise<StoredAgentSandboxBackup | undefined> {
...
async function backupOrganizationId(sandboxRecordId: string): Promise<string> {
    const [sandbox] = await dbWrite
        ...
        .limit(1);
    if (!sandbox)
        throw new Error(`Agent sandbox not found: ${sandboxRecordId}`);
    return sandbox.organizationId;
}
export async function hydrateAgentSandboxBackup(backup: StoredAgentSandboxBackup): Promise<AgentSandboxBackup> {
 ...
 */
export async function reconstructStoredAgentSandboxBackupChain(capturedTargetToBase: readonly StoredAgentSandboxBackup[]): Promise<{
    state: AgentBackupStateData;
    target: AgentSandboxBackup;
}> {
    if (capturedTargetToBase.length === 0)
    ...
    for (const stored of [...capturedTargetToBase].reverse()) {
        ...
        if (stored.id === capturedTargetToBase[0]?.id)
            target = hydrated;
        if (hydrated.backup_kind === "full") {
            ...
            if (!state)
                throw new Error(`Incremental ${hydrated.id} reached before a full backup`);
        ...
        if (!hydrated.content_hash || computeStateHash(state) !== hydrated.content_hash) {
            throw new Error(`Backup ${hydrated.id} content digest does not match its reconstructed state`);
        }
    }
    if (!state || !target)
        throw new Error("Backup chain did not produce a restorable state");
    return { state, target };
}
export async function prepareAgentBackupInsertData(data: NewAgentSandboxBackup, organizationId?: string): Promise<NewAgentSandboxBackup> {
    if (data.state_data_storage === "r2")
        return data;
    const id = data.id ?? randomUUID();
    const createdAt = data.created_at ?? new Date();
    const effectiveOrganizationId = organizationId ?? (await backupOrganizationId(data.sandbox_record_id));
    const encryptedStateData = await encryptAgentBackupStateData(effectiveOrganizationId, id, data.state_data);
...
async function assertProvisionQuota(tx: DbTransaction, id: string, organizationId?: string, eligibility?: SQL): Promise<void> {
    const [identity] = await tx
        .select({ organizationId: agentSandboxes.organization_id })
        .from(agentSandboxes)
        .where(and(eq(agentSandboxes.id, id), organizationId ? eq(agentSandboxes.organization_id, organizationId) : undefined));
    if (!identity)
        return;
    ...
    const [candidate] = await tx
        ...
        .from(agentSandboxes)
        .where(and(eq(agentSandboxes.id, id), eq(agentSandboxes.organization_id, identity.organizationId), eligibility))
        .for("update");
    ...
    if (policy.authority.source === "legacy")
        return;
    const ceiling = requireOrganizationResourceLimit(policy, "sandboxes");
    const [population] = await tx
        .select({ count: sql<number> `count(*)::int` })
        .from(agentSandboxes)
        .where(and(eq(agentSandboxes.organization_id, candidate.organizationId), isNull(agentSandboxes.pool_status), inArray(agentSandboxes.status, QUOTA_COUNTED_STATUSES)));
    if (!population)
        throw new ElizaError("Sandbox population is unavailable", {
            code: "RESOURCE_POLICY_UNAVAILABLE",
        });
    const required = BigInt(population.count) + (QUOTA_COUNTED_STATUSES.includes(candidate.status) ? 0n : 1n);
    if (required > ceiling)
...
export class AgentSandboxesRepository {
    ...
    async wasStoppedByUser(id: string, orgId: string): Promise<boolean> {
        const [latest] = await dbWrite
            ...
            .where(and(eq(agentComputeStopIntents.agent_id, id), eq(agentComputeStopIntents.organization_id, orgId), eq(agentComputeStopIntents.status, "provider_confirmed"), isNotNull(agentComputeStopIntents.provider_confirmed_at)))
            .orderBy(desc(agentComputeStopIntents.provider_confirmed_at), desc(agentComputeStopIntents.id))
            .limit(1);
    ...
    async listByNodeId(nodeId: string): Promise<AgentSandbox[]> {
        ...
        return dbRead
            .select()
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.node_id, nodeId), notInArray(agentSandboxes.status, terminalStatuses)));
    }
    /**
     ...
     * (#22548).
     */
    async listRunning(): Promise<Array<{
        id: string;
        ...
        return dbRead
            ...
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.status, "running"), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.pool_status)));
    }
    /**
     ...
     */
    async summarizeDedicatedFleet(): Promise<Array<{
        ...
        return dbRead
            ...
            .from(agentSandboxes)
            .where(and(inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.pool_status)))
     ...
     */
    async listRecoverable(limit = 100): Promise<Array<{
        id: string;
        organization_id: string;
        user_id: string;
        ...
        return dbRead
          ...
          )`, eq(agentSandboxes.execution_tier, "dedicated-always"), sql `${agentSandboxes.deleted_at} IS NULL`))
            .limit(limit);
    }
    /**
     ...
     */
    async listStuckProvisioningWithContainer(cutoff: Date, limit = 50): Promise<Array<{
        id: string;
        organization_id: string;
        user_id: string;
        agent_name: string | null;
        ...
        return dbRead
            ...
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.status, "provisioning"), lt(agentSandboxes.updated_at, cutoff), sql `${agentSandboxes.sandbox_id} IS NOT NULL`, sql `${agentSandboxes.pool_status} IS NULL`, sql `${agentSandboxes.deleted_at} IS NULL`, hasNoProvisioningStatusOwnerJob()))
            .limit(limit);
    }
    /**
     ...
     */
    async listSharedBridgeReapCandidates(cutoff: Date, limit: number): Promise<Array<{
        id: string;
        organization_id: string;
        user_id: string;
        agent_name: string | null;
        ...
        return dbRead
            ...
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.execution_tier, "shared"), sql `${agentSandboxes.deleted_at} IS NULL`, lt(agentSandboxes.created_at, cutoff)))
     ...
     */
    async listLiveDedicatedTwins(organizationIds: string[]): Promise<Array<{
        organization_id: string;
        user_id: string;
        agent_name: string | null;
        created_at: Date;
        ...
        return dbRead
            ...
            .from(agentSandboxes)
            .where(and(inArray(agentSandboxes.organization_id, organizationIds), eq(agentSandboxes.status, "running"), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), sql `${agentSandboxes.deleted_at} IS NULL`));
    }
    /**
     ...
     */
    async listRunningWithDigestOtherThan(targetDigest: string, targetImage: string, limit: number): Promise<Array<{
        id: string;
        organization_id: string;
        user_id: string;
        ...
        return dbRead
            ...
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.status, "running"), sql `${agentSandboxes.deleted_at} IS NULL`, sql `${agentSandboxes.image_digest} IS DISTINCT FROM ${targetDigest}`, 
        ...
        sql `${agentSandboxes.node_id} IS NOT NULL`, sql `${agentSandboxes.container_name} IS NOT NULL`))
            .limit(limit);
    }
    /**
     ...
     */
    async listLegacyWarmClaimRecoveryCandidates(limit: number): Promise<Array<{
        id: string;
        organization_id: string;
        user_id: string;
        image_digest: string | null;
        ...
        return dbWrite
            ...
            .from(agentSandboxes)
            .where(and(inArray(agentSandboxes.status, ["running", "provisioning", "stopped", "error"]), isNotNull(agentSandboxes.claimed_at), sql `${agentSandboxes.warm_claim_credential_state} IS NULL`, sql `${agentSandboxes.pool_status} IS NULL`, sql `${agentSandboxes.deleted_at} IS NULL`))
     ...
     */
    async listStrandedWarmClaimRecoveryCandidates(cutoff: Date, limit: number): Promise<Array<{
        id: string;
        organization_id: string;
        user_id: string;
    }>> {
        return dbWrite
            ...
            .from(agentSandboxes)
            .where(and(inArray(agentSandboxes.status, ["running", "provisioning", "stopped", "error"]), isNotNull(agentSandboxes.claimed_at), sql `${agentSandboxes.warm_claim_credential_state} IN ('pending', 'attested')`, sql `${agentSandboxes.pool_status} IS NULL`, sql `${agentSandboxes.deleted_at} IS NULL`, lt(agentSandboxes.updated_at, cutoff), sql `NOT EXISTS (
            SELECT 1 FROM ${jobs}
            WHERE  ${jobs.agent_id} = ${agentSandboxes.id}::text
            AND    ${jobs.organization_id} = ${agentSandboxes.organization_id}
     ...
     */
    async listFailedWarmClaimCredentialCleanupCandidates(limit: number): Promise<Array<{
        id: string;
        organization_id: string;
    }>> {
        return dbWrite
            ...
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.warm_claim_credential_state, "failed"), sql `${agentSandboxes.warm_claim_cleanup_completed_at} IS NULL`, sql `${agentSandboxes.deleted_at} IS NULL`))
            .orderBy(asc(agentSandboxes.updated_at), asc(agentSandboxes.id))
            .limit(limit);
    }
    /**
     ...
     * actually roll back.
     */
    async listRollbackEligibleForDigest(currentDigest: string, targetImage: string, limit: number): Promise<Array<{
        id: string;
        organization_id: string;
        user_id: string;
        image_digest: string | null;
        ...
        return dbRead
            ...
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.status, "running"), sql `${agentSandboxes.deleted_at} IS NULL`, eq(agentSandboxes.image_digest, currentDigest), isNotNull(agentSandboxes.previous_image_digest), 
        ...
        sql `(${agentSandboxes.docker_image} IS NULL OR ${imageRepoSql(agentSandboxes.docker_image)} = ${imageRepo(targetImage)})`, sql `${agentSandboxes.pool_status} IS NULL`, sql `${agentSandboxes.node_id} IS NOT NULL`, sql `${agentSandboxes.container_name} IS NOT NULL`))
            .limit(limit);
    }
    async findRunningSandbox(id: string, orgId: string): Promise<AgentSandbox | undefined> {
        ...
        const [r] = await dbWrite
            .select()
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.id, id), eq(agentSandboxes.organization_id, orgId), eq(agentSandboxes.status, "running"), inArray(agentSandboxes.execution_tier, [...RUNNING_SANDBOX_EXECUTION_TIERS])))
            .limit(1);
        return r;
    }
    async findByManagedDiscordGuildId(guildId: string): Promise<AgentSandbox[]> {
        ...
        const rows = await sqlRows<AgentSandbox>(dbWrite, sql `
      SELECT *
      FROM ${agentSandboxes}
    ...
    async create(data: NewAgentSandbox): Promise<AgentSandbox> {
        ...
        if (!r)
            throw new Error("Failed to create Agent sandbox record");
        return r;
    }
    async markStuckProvisioningWithoutActiveJobAsError(cutoff: Date): Promise<ReconciliationBatchResult<{
        agentId: string;
        ...
        const candidates = await dbWrite
            ...
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.status, "provisioning"), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), lt(agentSandboxes.updated_at, cutoff), hasNoProvisioningStatusOwnerJob()))
            .orderBy(asc(agentSandboxes.updated_at), asc(agentSandboxes.id))
        ...
        for (const candidate of candidates) {
            const updated = await dbWrite.transaction(async (tx) => {
                ...
                const [row] = await tx
                    ...
                    .where(and(eq(agentSandboxes.id, candidate.agentId), eq(agentSandboxes.organization_id, candidate.organizationId), eq(agentSandboxes.status, "provisioning"), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), lt(agentSandboxes.updated_at, cutoff), hasNoProvisioningStatusOwnerJob()))
                    .returning({
                    agentId: agentSandboxes.id,
     ...
     */
    async markOrphanedPendingWithoutJobAsError(cutoff: Date): Promise<ReconciliationBatchResult<{
        agentId: string;
        agentName: string | null;
        organizationId: string;
        createdAt: Date | null;
        ...
        const candidates = await dbWrite
            ...
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.status, "pending"), sql `${agentSandboxes.pool_status} IS NULL`, lt(agentSandboxes.created_at, cutoff), noProvisionJob()))
        ...
        for (const candidate of candidates) {
            const updated = await dbWrite.transaction(async (tx) => {
                ...
                const [row] = await tx
                    ...
                    .set({
                    status: "error",
                    error_message: "Provisioning never started: no agent_provision job was enqueued " +
                        "(orphaned pending). Please retry.",
                    updated_at: new Date(),
                })
                    .where(and(eq(agentSandboxes.id, candidate.agentId), eq(agentSandboxes.organization_id, candidate.organizationId), eq(agentSandboxes.status, "pending"), sql `${agentSandboxes.pool_status} IS NULL`, lt(agentSandboxes.created_at, cutoff), noProvisionJob()))
                    .returning({
                    agentId: agentSandboxes.id,
                    agentName: agentSandboxes.agent_name,
                    organizationId: agentSandboxes.organization_id,
    ...
    async markProvisionFailed(expected: AgentSandbox, message: string): Promise<AgentSandbox | undefined> {
        await ensureAgentSandboxSchema();
        const execution = expected.lifecycle_job_id !== null && expected.lifecycle_execution_generation !== null;
        const [updated] = await dbWrite
            ...
            .where(and(eq(agentSandboxes.id, expected.id), eq(agentSandboxes.organization_id, expected.organization_id), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), inArray(agentSandboxes.status, ["provisioning", "running"]), eq(agentSandboxes.environment_revision, expected.environment_revision), isNull(agentSandboxes.deletion_attempt_id), isNull(agentSandboxes.deleted_at), execution
            ? and(eq(agentSandboxes.lifecycle_job_id, expected.lifecycle_job_id!), eq(agentSandboxes.lifecycle_execution_generation, expected.lifecycle_execution_generation!))
            : and(isNull(agentSandboxes.lifecycle_job_id), isNull(agentSandboxes.lifecycle_execution_generation), eq(agentSandboxes.lifecycle_revision, expected.lifecycle_revision))))
            .returning();
        return updated;
    }
    async update(id: string, data: Partial<NewAgentSandbox>, expectedRunningGeneration?: {
        organizationId: string;
        ...
        await ensureAgentSandboxSchema();
        const updateData = data.environment_vars === undefined
        ...
        if (data.environment_vars !== undefined) {
            predicates.push(sql `COALESCE(${agentSandboxes.warm_claim_credential_state}, '') NOT IN ('pending', 'attested')`);
        }
        if (expectedRunningGeneration) {
            predicates.push(eq(agentSandboxes.organization_id, expectedRunningGeneration.organizationId), eq(agentSandboxes.status, "running"), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), eq(agentSandboxes.environment_revision, expectedRunningGeneration.environmentRevision), sql `${agentSandboxes.sandbox_id} IS NOT DISTINCT FROM ${expectedRunningGeneration.sandboxId}`, sql `${agentSandboxes.node_id} IS NOT DISTINCT FROM ${expectedRunningGeneration.nodeId}`, sql `${agentSandboxes.container_name} IS NOT DISTINCT FROM ${expectedRunningGeneration.containerName}`, 
            ...
            eq(agentSandboxes.lifecycle_revision, expectedRunningGeneration.lifecycleRevision), hasNoProvisioningOwnerJob([...EXCLUSIVE_AGENT_LIFECYCLE_JOB_TYPES]));
            ...
            return dbWrite.transaction(async (tx) => {
                await configureElizaLifecycleTransaction(tx);
                await tx.execute(elizaProvisionAdvisoryLockSql(expectedRunningGeneration.organizationId, id));
    ...
    async trySetProvisioning(id: string): Promise<AgentSandbox | undefined> {
        await ensureAgentSandboxSchema();
        return dbWrite.transaction(async (tx) => {
            const eligibility = and(eq(agentSandboxes.id, id), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), sql `${agentSandboxes.replacement_cleanup_sandbox_id} IS NULL`, sql `(
     ...
     */
    async trySetProvisioningFromRestoreCapture(capture: ProvisioningAdmissionCapture): Promise<AgentSandbox | undefined> {
        const isAdmissibleStatus = (RESTORE_PROVISIONING_ADMISSIBLE_STATUSES as readonly AgentSandboxStatus[]).includes(capture.status);
        const isCanonicalContainerTier = (CONTAINER_BACKED_EXECUTION_TIERS as readonly string[]).includes(capture.execution_tier);
        if (!isAdmissibleStatus ||
        ...
        return dbWrite.transaction(async (tx) => {
            ...
            const eligibility = and(eq(agentSandboxes.id, capture.id), eq(agentSandboxes.organization_id, capture.organization_id), eq(agentSandboxes.status, capture.status), inArray(agentSandboxes.status, [...RESTORE_PROVISIONING_ADMISSIBLE_STATUSES]), eq(agentSandboxes.execution_tier, capture.execution_tier), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), eq(agentSandboxes.lifecycle_revision, capture.lifecycle_revision), isNull(agentSandboxes.lifecycle_job_id), isNull(agentSandboxes.lifecycle_execution_generation), hasNoProvisioningOwnerJob([...EXCLUSIVE_AGENT_LIFECYCLE_JOB_TYPES]), isNull(agentSandboxes.pool_status), isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.deletion_attempt_id), isNull(agentSandboxes.replacement_cleanup_sandbox_id), isNull(agentSandboxes.replacement_cleanup_node_id), isNull(agentSandboxes.replacement_cleanup_container_name), isNull(agentSandboxes.replacement_cleanup_attempt_id), isNull(agentSandboxes.replacement_cleanup_container_id), isNull(agentSandboxes.replacement_cleanup_vpn_node_id), isNull(agentSandboxes.replacement_cleanup_vpn_node_name), isNull(agentSandboxes.replacement_cleanup_preserved_vpn_node_id), isNull(agentSandboxes.replacement_cleanup_vpn_registration_started_at), isNull(agentSandboxes.replacement_cleanup_allocation_counted), isNull(agentSandboxes.replacement_cleanup_created_at), sql `${agentSandboxes.warm_claim_credential_state} IS DISTINCT FROM 'failed'`);
     ...
     */
    async markReconnectedFromDisconnected(expected: DisconnectedRecoveryCapture, repairedIngress?: RepairedDisconnectedIngress): Promise<AgentSandbox | undefined> {
        const recoverableStatus = expected.status === "disconnected" || expected.status === "error";
        const isCanonicalContainerTier = (CONTAINER_BACKED_EXECUTION_TIERS as readonly string[]).includes(expected.execution_tier);
        ...
        return dbWrite.transaction(async (tx) => {
            ...
            if (!lock?.acquired)
                return undefined;
            const eligibility = and(eq(agentSandboxes.id, expected.id), eq(agentSandboxes.organization_id, expected.organization_id), eq(agentSandboxes.status, expected.status), inArray(agentSandboxes.status, ["disconnected", "error"]), eq(agentSandboxes.execution_tier, expected.execution_tier), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), sql `${agentSandboxes.sandbox_id} IS NOT DISTINCT FROM ${expected.sandbox_id}`, sql `${agentSandboxes.node_id} IS NOT DISTINCT FROM ${expected.node_id}`, sql `${agentSandboxes.container_name} IS NOT DISTINCT FROM ${expected.container_name}`, sql `${agentSandboxes.bridge_url} IS NOT DISTINCT FROM ${expected.bridge_url}`, sql `${agentSandboxes.health_url} IS NOT DISTINCT FROM ${expected.health_url}`, sql `${agentSandboxes.headscale_ip} IS NOT DISTINCT FROM ${expected.headscale_ip}`, eq(agentSandboxes.environment_revision, expected.environment_revision), eq(agentSandboxes.lifecycle_revision, expected.lifecycle_revision), isNull(agentSandboxes.lifecycle_job_id), isNull(agentSandboxes.lifecycle_execution_generation), hasNoProvisioningOwnerJob([...EXCLUSIVE_AGENT_LIFECYCLE_JOB_TYPES]), isNull(agentSandboxes.pool_status), sql `${agentSandboxes.previous_image_digest} IS NOT DISTINCT FROM ${expected.previous_image_digest}`, sql `${agentSandboxes.error_message} IS NOT DISTINCT FROM ${expected.error_message}`, sql `${expected.status} != 'error' OR (${agentSandboxes.previous_image_digest} IS NOT NULL AND ${agentSandboxes.error_message} IS NULL)`, isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.deletion_attempt_id));
     ...
     * matched nothing.
     */
    async markRunningFromProvisioning(expected: ProvisioningRecoveryCapture): Promise<AgentSandbox | undefined> {
        const isCanonicalContainerTier = (CONTAINER_BACKED_EXECUTION_TIERS as readonly string[]).includes(expected.execution_tier);
        if (expected.status !== "provisioning" ||
            !isCanonicalContainerTier ||
            !expected.sandbox_id?.trim() ||
            !expected.node_id?.trim() ||
            !expected.container_name?.trim() ||
        ...
        return dbWrite.transaction(async (tx) => {
            ...
            const [updated] = await tx
                ...
                .where(and(eq(agentSandboxes.id, expected.id), eq(agentSandboxes.organization_id, expected.organization_id), eq(agentSandboxes.status, expected.status), eq(agentSandboxes.execution_tier, expected.execution_tier), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), sql `${agentSandboxes.sandbox_id} IS NOT DISTINCT FROM ${expected.sandbox_id}`, sql `${agentSandboxes.node_id} IS NOT DISTINCT FROM ${expected.node_id}`, sql `${agentSandboxes.container_name} IS NOT DISTINCT FROM ${expected.container_name}`, sql `${agentSandboxes.bridge_url} IS NOT DISTINCT FROM ${expected.bridge_url}`, sql `${agentSandboxes.health_url} IS NOT DISTINCT FROM ${expected.health_url}`, sql `${agentSandboxes.headscale_ip} IS NOT DISTINCT FROM ${expected.headscale_ip}`, eq(agentSandboxes.environment_revision, expected.environment_revision), eq(agentSandboxes.lifecycle_revision, expected.lifecycle_revision), isNull(agentSandboxes.lifecycle_job_id), isNull(agentSandboxes.lifecycle_execution_generation), isNull(agentSandboxes.pool_status), isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.deletion_attempt_id), hasNoProvisioningStatusOwnerJob()))
                .returning();
            return updated;
        });
    }
    ...
    async countNonTerminalByOrganization(organizationId: string): Promise<number> {
        ...
        const [row] = await dbRead
            .select({ count: sql<number> `count(*)::int` })
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.organization_id, organizationId), sql `${agentSandboxes.pool_status} is null`, sql `${agentSandboxes.status} in ('pending', 'provisioning', 'running')`));
        return row?.count ?? 0;
    }
    /**
    ...
    async countRetainedByOrganization(organizationId: string): Promise<number> {
        ...
        const [row] = await dbRead
            .select({ count: sql<number> `count(*)::int` })
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.organization_id, organizationId), sql `${agentSandboxes.pool_status} is null`, sql `${agentSandboxes.deleted_at} is null`));
        return row?.count ?? 0;
    }
    async countUserProvisionsSince(sinceMs: number): Promise<number> {
        ...
        const [row] = await dbRead
            .select({ count: sql<number> `count(*)::int` })
            .from(agentSandboxes)
            .where(and(gte(agentSandboxes.created_at, since), sql `${agentSandboxes.organization_id} <> ${WARM_POOL_ORG_ID}`, sql `${agentSandboxes.pool_status} is null`));
        return row?.count ?? 0;
    }
    /**
     ...
     */
    async listWarmPoolReconciliationCandidates(minimumGenerationAgeMs: number): Promise<WarmPoolReconciliationCandidate[]> {
        await ensureAgentSandboxSchema();
        ...
        return dbWrite
            ...
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.organization_id, WARM_POOL_ORG_ID), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed"), eq(agentSandboxes.status, "running"), lt(agentSandboxes.created_at, createdBefore), isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.deletion_attempt_id), sql `NOT (${and(...warmPoolRuntimeReadyConditions())})`))
    ...
    async listAllRunningPoolEntries(): Promise<AgentSandbox[]> {
        ...
        return dbWrite
            .select()
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.organization_id, WARM_POOL_ORG_ID), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed"), eq(agentSandboxes.status, "running"), isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.deletion_attempt_id)))
            .orderBy(agentSandboxes.pool_ready_at);
    }
    /**
    ...
    async listPoolEntriesForRollout(): Promise<AgentSandbox[]> {
        ...
        return dbWrite
            .select()
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.organization_id, WARM_POOL_ORG_ID), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed"), isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.deletion_attempt_id)))
    ...
    async findStuckPoolProvisioning(staleThresholdMs: number): Promise<AgentSandbox[]> {
        ...
        return dbWrite
            .select()
            .from(agentSandboxes)
            .where(and(eq(agentSandboxes.organization_id, WARM_POOL_ORG_ID), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed"), inArray(agentSandboxes.status, ["pending", "provisioning", "error", "deletion_failed"]), lt(agentSandboxes.updated_at, cutoff)));
    }
    /**
    ...
    async claimWarmContainer(params: {
        ...
        return dbWrite.transaction(async (tx) => {
            ...
            const poolRows = await sqlRows<AgentSandbox>(tx, sql `
          SELECT *
          FROM ${agentSandboxes}
          WHERE ${claimablePool}
        ...
        `);
          ...
          `);
                ...
                if (skippedRow && skippedRow.count > 0) {
                    logger.warn("[agent-sandbox] Warm-pool claim skipped unclaimable entries; falling through to cold path", {
            ...
            const [userRow] = await tx
                .select()
                .from(agentSandboxes)
                .where(and(eq(agentSandboxes.id, params.userAgentId), eq(agentSandboxes.organization_id, params.organizationId), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), isNull(agentSandboxes.pool_status), isNull(agentSandboxes.deleted_at)))
                .for("update")
                .limit(1);
            if (!userRow)
                return null;
            ...
            if (userRow.database_status === "ready" || userRow.database_uri)
                return null;
            if (userRow.status === "running")
                return null;
            if (userRow.deletion_attempt_id !== null ||
            ...
            if (params.expectedLifecycleRevision !== undefined &&
            ...
            const [updated] = await tx
                ...
                environment_vars: mergeWarmClaimEnvironmentVars(userRow.environment_vars as Record<string, string> | null, pool.environment_vars as Record<string, string> | null),
                environment_revision: sql `${agentSandboxes.environment_revision} + 1`,
                ...
                .where(and(eq(agentSandboxes.id, params.userAgentId), eq(agentSandboxes.organization_id, params.organizationId), inArray(agentSandboxes.execution_tier, [...CONTAINER_BACKED_EXECUTION_TIERS]), isNull(agentSandboxes.pool_status), isNull(agentSandboxes.deleted_at), ...(params.expectedLifecycleRevision === undefined
                ? []
                : [eq(agentSandboxes.lifecycle_revision, params.expectedLifecycleRevision)]), sql `${agentSandboxes.deletion_attempt_id} IS NULL`, sql `${agentSandboxes.status} NOT IN ('deletion_pending', 'deletion_failed')`))
                .returning();
            if (!updated)
                return null;
    ...
    async createPoolEntry(data: Omit<NewAgentSandbox, "organization_id" | "user_id" | "pool_status" | "execution_tier" | "billing_status" | "last_billed_at" | "hourly_rate" | "shutdown_warning_sent_at" | "scheduled_shutdown_at">): Promise<AgentSandbox> {
        await ensureAgentSandboxSchema();
        const [row] = await dbWrite
            ...
            .returning();
        if (!row)
            throw new Error("Failed to create warm pool entry");
        return row;
    }
    ...
    async deletePoolEntry(id: string): Promise<boolean> {
        ...
        const r = await dbWrite
            .delete(agentSandboxes)
            .where(and(eq(agentSandboxes.id, id), eq(agentSandboxes.organization_id, WARM_POOL_ORG_ID), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed")))
            .returning({ id: agentSandboxes.id });
        return r.length > 0;
    }
    /**
     ...
     */
    async commitPoolEntryReady(expected: WarmPoolRuntimeGeneration): Promise<AgentSandbox | undefined> {
        await ensureAgentSandboxSchema();
        ...
        return dbWrite.transaction(async (tx) => {
            ...
            const [ready] = await tx
                ...
                .where(and(...warmPoolGenerationConditions(expected), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed"), eq(agentSandboxes.status, "provisioning"), isNull(agentSandboxes.pool_ready_at), ...warmPoolRuntimeLocatorConditions()))
                .returning();
            return ready;
        });
    }
     ...
     */
    async promoteStrandedPoolEntryReady(expected: WarmPoolRuntimeGeneration): Promise<AgentSandbox | undefined> {
        await ensureAgentSandboxSchema();
        if (expected.organization_id !== WARM_POOL_ORG_ID || expected.pool_ready_at !== null) {
            return undefined;
        }
        ...
        const [ready] = await dbWrite
            ...
            .where(and(...warmPoolGenerationConditions(expected), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed"), eq(agentSandboxes.status, "running"), isNull(agentSandboxes.pool_ready_at), ...warmPoolRuntimeLocatorConditions()))
            .returning();
        return ready;
    }
    /**
     ...
     */
    async reserveUnclaimablePoolEntryForReap(expected: WarmPoolRuntimeGeneration, reason: string): Promise<AgentSandbox | undefined> {
        await ensureAgentSandboxSchema();
        if (expected.organization_id !== WARM_POOL_ORG_ID)
            return undefined;
        const [reserved] = await dbWrite
            ...
            .where(and(...warmPoolGenerationConditions(expected), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed"), eq(agentSandboxes.status, "running"), isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.deletion_attempt_id), sql `NOT (${and(...warmPoolRuntimeReadyConditions())})`))
            .returning();
        return reserved;
    }
    /**
     ...
     */
    async reserveStalePoolEntryForRollout(expected: WarmPoolRuntimeGeneration, targetDigest: string): Promise<AgentSandbox | undefined> {
        await ensureAgentSandboxSchema();
        ...
        const [reserved] = await dbWrite
            ...
            .where(and(...warmPoolGenerationConditions(expected), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed"), sql `${agentSandboxes.image_digest} IS DISTINCT FROM ${targetDigest}`, isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.deletion_attempt_id)))
            .returning();
        return reserved;
    }
    /**
     ...
     */
    async reserveStuckPoolEntryForReap(expected: WarmPoolRuntimeGeneration, staleThresholdMs: number): Promise<AgentSandbox | undefined> {
        await ensureAgentSandboxSchema();
        if (expected.organization_id !== WARM_POOL_ORG_ID)
            return undefined;
        const cutoff = new Date(Date.now() - staleThresholdMs);
        const [reserved] = await dbWrite
            ...
            .where(and(...warmPoolGenerationConditions(expected), eq(agentSandboxes.user_id, WARM_POOL_USER_ID), eq(agentSandboxes.pool_status, "unclaimed"), inArray(agentSandboxes.status, ["pending", "provisioning", "error", "deletion_failed"]), lt(agentSandboxes.updated_at, cutoff), isNull(agentSandboxes.deleted_at), isNull(agentSandboxes.deletion_attempt_id)))
            .returning();
        return reserved;
    }
    // Backups
     ...
     */
    async validateAttachedPreDeleteBackupForDeletion(tx: DbTransaction, params: {
        backupId: string;
        ...
        const [backup] = await tx
            ...
            .from(agentSandboxBackups)
            .where(and(eq(agentSandboxBackups.id, params.backupId), eq(agentSandboxBackups.sandbox_record_id, params.sandboxRecordId), eq(agentSandboxBackups.snapshot_type, "pre-delete"), eq(agentSandboxBackups.backup_kind, "full"), isNull(agentSandboxBackups.parent_backup_id), gte(agentSandboxBackups.created_at, params.deletionStartedAt), isNull(agentSandboxBackups.recovery_organization_id), isNull(agentSandboxBackups.recovery_agent_id), isNull(agentSandboxBackups.recovery_deletion_attempt_id), isNull(agentSandboxBackups.recovery_expires_at), backupVisibleToLegacyReaders()))
            .limit(1);
     ...
     */
    async retainPreDeleteBackupForDeletedAgent(tx: DbTransaction, params: {
        backupId: string;
        sandboxRecordId: string;
        organizationId: string;
    ...
    async getPreDeleteRecoveryBackup(organizationId: string, deletedAgentId: string, now = new Date()): Promise<AgentSandboxBackup | undefined> {
        const [row] = await dbRead
            .select()
            .from(agentSandboxBackups)
            .where(and(isNull(agentSandboxBackups.sandbox_record_id), eq(agentSandboxBackups.snapshot_type, "pre-delete"), eq(agentSandboxBackups.recovery_organization_id, organizationId), eq(agentSandboxBackups.recovery_agent_id, deletedAgentId), gt(agentSandboxBackups.recovery_expires_at, now), backupVisibleToLegacyReaders()))
     ...
     */
    async cleanupExpiredPreDeleteRecoveryBackups(now = new Date(), limit = PRE_DELETE_BACKUP_CLEANUP_BATCH_SIZE): Promise<PreDeleteBackupCleanupResult> {
        ...
        const candidates = await dbRead
            ...
            .from(agentSandboxBackups)
            .where(and(isNull(agentSandboxBackups.sandbox_record_id), eq(agentSandboxBackups.snapshot_type, "pre-delete"), isNotNull(agentSandboxBackups.recovery_organization_id), isNotNull(agentSandboxBackups.recovery_agent_id), isNotNull(agentSandboxBackups.recovery_deletion_attempt_id), lte(agentSandboxBackups.recovery_expires_at, now), backupVisibleToLegacyReaders()))
        ...
        for (const candidate of candidates) {
            if (candidate.stateDataStorage === "r2") {
                if (!candidate.stateDataKey) {
                    ...
                    invalidRows += 1;
                    logger.error("Expired recovery backup is missing its object-storage key; discarding invalid row", { backupId: logger.redact.id(candidate.id) });
                }
                else {
                    try {
                        ...
                        failedRows += 1;
                        logger.error("Failed to delete expired recovery backup object; retaining row for retry", {
            ...
            try {
                const removed = await dbWrite
                    ...
                    .where(and(eq(agentSandboxBackups.id, candidate.id), isNull(agentSandboxBackups.sandbox_record_id), lte(agentSandboxBackups.recovery_expires_at, now)))
                    .returning({ id: agentSandboxBackups.id });
    ...
    async listBackups(sandboxRecordId: string, limit = 10): Promise<AgentSandboxBackup[]> {
        const rows = await dbRead
            .select()
            .from(agentSandboxBackups)
            .where(and(eq(agentSandboxBackups.sandbox_record_id, sandboxRecordId), backupVisibleToLegacyReaders()))
    ...
    async listBackupMetadata(sandboxRecordId: string, limit = 10): Promise<AgentSandboxBackupMetadata[]> {
        return await dbRead
            .select({
            id: agentSandboxBackups.id,
     ...
     */
    async getLatestBackupByType(sandboxRecordId: string, snapshotType: AgentBackupSnapshotType): Promise<AgentSandboxBackup | undefined> {
        const [r] = await dbWrite
            .select()
            .from(agentSandboxBackups)
            .where(and(eq(agentSandboxBackups.sandbox_record_id, sandboxRecordId), eq(agentSandboxBackups.snapshot_type, snapshotType), backupVisibleToLegacyReaders()))
            .orderBy(desc(agentSandboxBackups.created_at))
            .limit(1);
        return r ? await hydrateAgentSandboxBackup(r) : undefined;
    }
    ...
    async getLatestStoredBackup(sandboxRecordId: string): Promise<StoredAgentSandboxBackup | undefined> {
     ...
     */
    async stampBackupVerification(backupId: string, outcome: {
    ...
    async pruneBackups(sandboxRecordId: string, keep: number): Promise<number> {
        const all = await dbRead
            ...
            .from(agentSandboxBackups)
            .where(and(eq(agentSandboxBackups.sandbox_record_id, sandboxRecordId), backupVisibleToLegacyReaders(), eq(agentSandboxBackups.state_data_storage, "inline")));
        if (all.length <= keep)
            return 0;
    ...
    async getReconstructedBackupState(backupId: string): Promise<AgentBackupStateData | undefined> {
        ...
        while (cursor) {
            ...
            if (chain.length > MAX_RECONSTRUCTED_BACKUP_CHAIN_DEPTH) {
                throw new Error(`Backup chain for ${backupId} exceeds ${MAX_RECONSTRUCTED_BACKUP_CHAIN_DEPTH} rows`);
            }
            const parentBackupId = cursor.parent_backup_id;
            cursor = await getStoredBackupById(parentBackupId);
            if (!cursor)
                throw new Error(`Backup chain references missing backup ${parentBackupId}`);
        }
        chain.reverse();
        let state: AgentBackupStateData | undefined;
     ...
     */
    private async spendDeletionAllocation(nodeId: string, claimWhere: SQL): Promise<DeletionAllocationSpendResult> {
        ...
        return dbWrite.transaction(async (tx) => {
            ...
            if (!claimed)
                return { outcome: "not-owned", lifecycleRevision: null };
            ...
            if (decremented.length === 0) {
                ...
                logger.warn(`[agent-sandboxes] Deletion allocation ownership consumed for node ${nodeId} but allocated_count was not decremented — counter already at 0 or node row missing`);
                return {
     ...
     */
    async tryReleaseDeletionAllocation(agentId: string, orgId: string, deletionAttemptId: string, nodeId: string): Promise<DeletionAllocationRelease> {
        const result = await this.spendDeletionAllocation(nodeId, and(eq(agentSandboxes.id, agentId), eq(agentSandboxes.organization_id, orgId), eq(agentSandboxes.deletion_attempt_id, deletionAttemptId), eq(agentSandboxes.node_id, nodeId)) as SQL);
     ...
     */
    async tryReleaseDeletionAllocationForCommit(agentId: string, orgId: string, deletionAttemptId: string, nodeId: string, expectedLifecycleRevision: number): Promise<DeletionAllocationSpendResult> {
        return this.spendDeletionAllocation(nodeId, and(eq(agentSandboxes.id, agentId), eq(agentSandboxes.organization_id, orgId), eq(agentSandboxes.deletion_attempt_id, deletionAttemptId), eq(agentSandboxes.node_id, nodeId), eq(agentSandboxes.lifecycle_revision, expectedLifecycleRevision)) as SQL);
    }
    /**
     ...
     */
    async releaseDeletionAllocationOnReap(agentId: string, nodeId: string): Promise<DeletionAllocationRelease> {
        const result = await this.spendDeletionAllocation(nodeId, and(eq(agentSandboxes.id, agentId), eq(agentSandboxes.node_id, nodeId)) as SQL);
        ...
        await dbWrite
            ...
            .where(and(eq(agentSandboxes.id, agentId), eq(agentSandboxes.node_id, nodeId), inArray(agentSandboxes.status, ["deletion_pending", "deletion_failed"])));
        return result.outcome;
    }
}
