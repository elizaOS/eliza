...
import { generateRemoteHostToken } from "../crypto/remote-host-token";
...
import { type RemoteRelayScope } from "./remote-command-envelopes";
...
function envelope(messageKind: "command", overrides?: Record<string, unknown>): EncryptedRemoteCommandEnvelope;
function envelope(messageKind: "start_receipt", overrides?: Record<string, unknown>): EncryptedRemoteCommandStartReceiptEnvelope;
function envelope(messageKind: "result", overrides?: Record<string, unknown>): EncryptedRemoteCommandResultEnvelope;
function envelope(messageKind: "command" | "start_receipt" | "result", overrides: Record<string, unknown> = {}): EncryptedRemoteControlEnvelope {
...
async function enrollAndPair(activate = true): Promise<void> {
    ...
    expect(await hosts.createOwned({
        id: hostId,
    ...
    const code = "123456";
    const pairingExpiry = new Date(Date.now() + 5 * 60000);
    const grantExpiry = new Date(Date.now() + 60 * 60000);
    const verifier = await deriveRemotePairingCodeVerifier(pairingSecret, { organizationId, userId: ownerId, hostId, sessionId }, code, pairingExpiry);
    expect(await sessions.createPendingForOwnedHost({
        id: sessionId,
        organization_id: organizationId,
        user_id: ownerId,
        host_id: hostId,
    ...
    if (activate) {
        expect(await sessions.activatePendingHost({
            sessionId,
            hostId,
            hostToken,
            code,
...
describe("secure remote relay repositories", () => {
    it("keeps managed enrollment non-authoritative until durable activation", async () => {
        ...
        expect(await sessions.createPendingForOwnedHost({
            ...
            status: "pending",
            requester_identity: ownerId,
            pairing_token_hash: await deriveRemotePairingCodeVerifier(pairingSecret, { organizationId, userId: ownerId, hostId, sessionId }, "123456", pairingExpiry),
            controller_device_id: controllerDeviceId,
            controller_key_id: controllerKeyId,
            controller_display_name: "Controller",
            controller_platform: "linux",
            ...
            target_key_id: targetKeyId,
            expires_at: pairingExpiry,
            grant_expires_at: new Date(Date.now() + 60 * 60000),
        })).toBeUndefined();
        ...
        expect(await hosts.listManagedCleanupCandidates({
    ...
    it("recovers a lost host token only for the exact active public identity", async () => {
        ...
        expect(await hosts.recoverCredential({
            hostId,
            organizationId,
            userId: ownerId,
    ...
    it("runs target challenge claim, confirmation, expiry, denial, replay, and no-resurrection", async () => {
        ...
        const challenge = async (id: string, code: string) => sessions.createPendingForAuthenticatedHost({
            id,
            hostId,
            hostToken,
            ...
            grantExpiresAt: new Date(Date.now() + 60 * 60000),
        });
        ...
        expect(await sessions.claimPendingHostForOwner({
            organizationId,
            userId: otherOwnerId,
            sessionId,
            code: "123456",
        ...
        expect(await sessions.readAuthenticatedHostPairing({
            sessionId,
            hostId,
            hostToken: generateRemoteHostToken(),
        })).toEqual({ kind: "not_found" });
        ...
        expect(await new RemoteSessionsRepository(database).listByOwnedHost(hostId, organizationId, ownerId)).toEqual([]);
    ...
    `);
        ...
        expect(await sessions.compensateHostActivation({
    ...
    it("discovers one host-bound session from only its six-digit code", async () => {
        await enrollAndPair(false);
        expect(await sessions.activatePendingHostByCode({
            hostId,
            hostToken,
            code: "000000",
            pairingSecret,
    ...
    it("commits an exact staged activation idempotently without admitting pre-commit commands", async () => {
        ...
        await expect(sessions.commitHostActivation({
            sessionId,
            hostId,
            hostToken: `rhost_v1_${"Z".repeat(43)}`,
        })).resolves.toEqual({ kind: "not_found" });
        await expect(sessions.commitHostActivation({ sessionId, hostId, hostToken })).resolves.toMatchObject({
            kind: "committed",
            session: { id: sessionId, status: "active" },
            alreadyCommitted: false,
        });
        await expect(sessions.commitHostActivation({ sessionId, hostId, hostToken })).resolves.toMatchObject({ kind: "committed", alreadyCommitted: true });
    });
    ...
    it("compensates an exact activation idempotently under host finalization", async () => {
        ...
        await expect(sessions.compensateHostActivation({
            sessionId,
            hostId,
            hostToken: `rhost_v1_${"Z".repeat(43)}`,
        })).resolves.toEqual({ kind: "not_found" });
        ...
        await expect(sessions.compensateHostActivation({ sessionId, hostId, hostToken })).resolves.toMatchObject({
            kind: "compensated",
            session: { id: sessionId, status: "denied" },
    ...
    it("retries only an expired pre-start claim and fences its stale attempt", async () => {
        ...
        const first = await commands.claimNext({ sessionId, hostId, hostToken, leaseMs: 1000 });
    ...
    `);
        const second = await commands.claimNext({ sessionId, hostId, hostToken, leaseMs: 1000 });
        ...
        expect((await commands.recordStart({
            sessionId,
    ...
    it("binds completion to the exact claim attempt and durable start receipt", async () => {
        ...
        if (claim.kind !== "claimed")
            throw new Error("expected claim");
        expect((await commands.complete({
            sessionId,
            commandId: scope.commandId,
            hostId,
            hostToken,
        ...
        expect(await commands.readOwnedResult({
            organizationId,
            ownerId,
            sessionId,
            commandId: scope.commandId,
    ...
    it("lets the exact host bearer revoke and continue its own bounded cleanup", async () => {
        await enrollAndPair();
        await expect(hosts.revokeAuthenticated(hostId, `rhost_v1_${"B".repeat(43)}`)).resolves.toBeUndefined();
