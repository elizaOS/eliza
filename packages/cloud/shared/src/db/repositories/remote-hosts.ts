...
import { type NewRemoteHost } from "../schemas/remote-hosts";
import { type RemoteConnectionMode } from "@elizaos/core/contracts/remote-control";
import { type RemoteHost } from "../schemas/remote-hosts";
...
export class RemoteHostsRepository {
    ...
    async getOwned(hostId: string, organizationId: string, userId: string): Promise<RemoteHost | undefined> {
        const [host] = await this.database
            .select()
            .from(remoteHosts)
            .where(and(eq(remoteHosts.id, hostId), eq(remoteHosts.organization_id, organizationId), eq(remoteHosts.user_id, userId)))
            .limit(1);
        return host;
    }
    async authenticate(hostId: string, token: string): Promise<RemoteHost | undefined> {
        ...
        const [host] = await this.database
            .select()
            .from(remoteHosts)
            .where(and(eq(remoteHosts.id, hostId), eq(remoteHosts.host_token_hash, tokenHash), eq(remoteHosts.status, "active")))
            .limit(1);
        return host;
    }
    /**
     ...
     */
    async authenticateManagedEnrollment(hostId: string, token: string): Promise<RemoteHost | undefined> {
        let tokenHash: string;
        try {
            tokenHash = await hashRemoteHostToken(token);
        }
        ...
        const [host] = await this.database
            .select()
            .from(remoteHosts)
            .where(and(eq(remoteHosts.id, hostId), eq(remoteHosts.host_token_hash, tokenHash), inArray(remoteHosts.status, ["pending", "active"]), eq(remoteHosts.headscale_cleanup_pending, true), isNotNull(remoteHosts.headscale_hostname), isNotNull(remoteHosts.headscale_preauth_key_id)))
            .limit(1);
        return host;
    }
    async recordManagedEnrollment(input: {
        ...
        const [host] = await this.database
            ...
            .where(and(eq(remoteHosts.id, input.hostId), eq(remoteHosts.organization_id, input.organizationId), eq(remoteHosts.user_id, input.userId), eq(remoteHosts.status, "pending")))
            .returning();
        if (!host) {
    ...
    async activateManagedEnrollment(input: {
        ...
        const [host] = await this.database
            ...
            .where(and(eq(remoteHosts.id, input.hostId), eq(remoteHosts.organization_id, input.organizationId), eq(remoteHosts.user_id, input.userId), eq(remoteHosts.status, "pending"), eq(remoteHosts.headscale_cleanup_pending, true), isNotNull(remoteHosts.headscale_hostname), isNotNull(remoteHosts.headscale_preauth_key_id)))
            .returning();
        if (!host) {
    ...
    async listManagedCleanupCandidates(input: {
        ...
        return this.database
            .select()
            .from(remoteHosts)
            .where(and(eq(remoteHosts.headscale_cleanup_pending, true), or(eq(remoteHosts.status, "revoked"), and(eq(remoteHosts.status, "pending"), lte(remoteHosts.updated_at, input.pendingUpdatedBefore)))))
    ...
    async recordManagedCleanupFailure(input: {
        ...
        const [host] = await this.database
            ...
            .where(and(eq(remoteHosts.id, input.hostId), eq(remoteHosts.organization_id, input.organizationId), eq(remoteHosts.user_id, input.userId)))
            .returning({ id: remoteHosts.id });
        if (!host) {
     ...
     */
    async recoverCredential(input: RecoverRemoteHostCredentialInput): Promise<RecoverRemoteHostCredentialResult> {
        if (!input.hostId ||
        ...
        return this.database.transaction(async (tx) => {
            ...
            if (current.device_id !== input.deviceId ||
     ...
     */
    async revoke(hostId: string, organizationId: string, userId: string): Promise<RevokeRemoteHostResult | undefined> {
        return this.revokeMatching(hostId, and(eq(remoteHosts.organization_id, organizationId), eq(remoteHosts.user_id, userId)));
    }
    /**
     ...
     */
    async revokeAuthenticated(hostId: string, token: string): Promise<RevokeRemoteHostResult | undefined> {
        let tokenHash: string;
        try {
            tokenHash = await hashRemoteHostToken(token);
        }
    ...
    private async revokeMatching(hostId: string, ownership: SQL<unknown> | undefined): Promise<RevokeRemoteHostResult | undefined> {
        return this.database.transaction(async (tx) => {
            const [current] = await tx
                .select()
                .from(remoteHosts)
            ...
            if (!alreadyRevoked) {
                const [revoked] = await tx
                    ...
                    .where(and(eq(remoteHosts.id, hostId), inArray(remoteHosts.status, ["pending", "active"])))
                    .returning();
                if (!revoked) {
            ...
            const sessions = await tx
                ...
                .from(remoteSessions)
                .where(and(eq(remoteSessions.host_id, hostId), eq(remoteSessions.organization_id, organizationId), eq(remoteSessions.user_id, userId), inArray(remoteSessions.status, ["pending", "activating", "active"])))
            ...
            if (sessions.length > 0) {
                await tx
                    ...
                    .where(inArray(remoteSessions.id, sessions.map((session) => session.id)));
            }
            const commands = await tx
                ...
                .where(and(eq(remoteCommandEnvelopes.host_id, hostId), inArray(remoteCommandEnvelopes.status, ["pending", "claimed", "started"])))
            ...
            const [remainingSession] = await tx
                .select({ id: remoteSessions.id })
                .from(remoteSessions)
                .where(and(eq(remoteSessions.host_id, hostId), inArray(remoteSessions.status, ["pending", "activating", "active"])))
                .limit(1);
