...
function requireNullableIsoTimestamp(value: unknown, field: string): Date | null {
  ...
  if (!Number.isFinite(parsed.getTime())) {
    throw invalidInput(`${field} must be a valid timestamp`, field);
  }
  return parsed;
}
 ...
 */
export async function startAgentSandboxReplacementAttempt(
          ...
          .where(
            and(
              ...
              eq(
                ...
                restore.sourceActivationGeneration,
              ),
...
async function recordLocatorStage(
  ...
  return await dbWrite.transaction(async (tx) => {
    ...
    if (!locator.containerId) {
      throw invalidInput(
        "Created replacement locator is missing containerId",
        "locator.containerId",
      );
    }
    ...
    if (stage === "created") {
      ...
      if (!recorded) {
        throw conflict(
          "Replacement Docker enrichment lost its state CAS",
          reference,
          current.state,
        );
      }
...
/** Persist the complete pre-create placement before remote effects can begin. */
export async function recordAgentSandboxReplacementIntent(
  reference: AgentSandboxReplacementAttemptReference,
  locator: AgentSandboxReplacementLocatorInput,
): Promise<AgentSandboxReplacementAttemptWriteResult> {
  return await recordLocatorStage(reference, locator, "intent");
}
...
export async function recordAgentSandboxReplacementCreated(
  ...
  return await recordLocatorStage(reference, locator, "created");
}
...
export async function recordAgentSandboxReplacementVpnRegistered(
  ...
  return await recordLocatorStage(reference, locator, "vpn");
}
...
/** Convenience boundary for callers that do not also mutate lifecycle state. */
export async function recordAgentSandboxReplacementLifecycleCommitted(
  input: CommitAgentSandboxReplacementLifecycleAdoptionInput,
): Promise<AgentSandboxReplacementAttemptWriteResult> {
  return await dbWrite.transaction((tx) =>
    commitAgentSandboxReplacementLifecycleAdoptionInTransaction(tx, input),
  );
}

/**
 * Terminally retain exact cleanup proof. It may close an unresolved call or a
 * provider-success candidate rejected before lifecycle adoption.
 */
export async function recordAgentSandboxReplacementCleanupProven(
  referenceInput: AgentSandboxReplacementAttemptReference,
  receiptDigestInput: string,
): Promise<AgentSandboxReplacementAttemptWriteResult> {
  const reference = validateReference(referenceInput);
