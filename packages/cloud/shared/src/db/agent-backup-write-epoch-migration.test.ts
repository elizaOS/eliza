...
const migrationUrl = new URL("./migrations/0187_agent_backup_write_epochs.sql", import.meta.url);
...
describe("0187 backup write epochs", () => {
  test("is registered once in the migration journal", () => {
    ...
    expect(
      journal.entries.filter((entry) => entry.tag === "0187_agent_backup_write_epochs"),
    ).toHaveLength(1);
  });
