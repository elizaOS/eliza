...
export const remoteCommandEnvelopes = pgTable("remote_command_envelopes", {
    ...
    sessionQueueIdx: index("remote_command_envelopes_session_queue_idx").on(table.session_id, table.status, table.sequence),
    hostQueueIdx: index("remote_command_envelopes_host_queue_idx").on(table.host_id, table.status, table.created_at),
    commandUnique: uniqueIndex("remote_command_envelopes_session_command_unique").on(table.session_id, table.command_id),
    sequenceUnique: uniqueIndex("remote_command_envelopes_session_sequence_unique").on(table.session_id, table.sequence),
    nonceUnique: uniqueIndex("remote_command_envelopes_session_nonce_unique").on(table.session_id, table.nonce),
    positiveSequence: check("remote_command_envelopes_positive_sequence_check", sql `${table.sequence} > 0 AND ${table.attempts} >= 0 AND ${table.grant_revision} > 0`),
    lifecycleShape: check("remote_command_envelopes_lifecycle_shape_check", sql `(
