  ...
  (table) => ({
    ...
    endpoint_check: check(
      ...
      sql`((${table.purpose} = 'restore'
          ...
          AND jsonb_typeof(${table.endpoint_envelope}->'runtimeAgentId') = 'string'
          AND ${table.endpoint_envelope}->>'runtimeAgentId' ~
            '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
          ...
          AND octet_length(${table.endpoint_envelope}->>'registryUrl') =
            length(${table.endpoint_envelope}->>'registryUrl')
          AND octet_length(${table.endpoint_envelope}->>'bridgeUrl') =
            length(${table.endpoint_envelope}->>'bridgeUrl')
          AND octet_length(${table.endpoint_envelope}->>'healthUrl') =
            length(${table.endpoint_envelope}->>'healthUrl')
          AND ${table.endpoint_sha256} = pg_catalog.encode(pg_catalog.sha256(
            pg_catalog.convert_to('{"version":1,"generation":' ||
              pg_catalog.to_json(${table.endpoint_envelope}->>'generation')::text ||
              ',"kind":"dedicated-sandbox","serverName":' ||
              pg_catalog.to_json(${table.endpoint_envelope}->>'serverName')::text ||
              ',"runtimeAgentId":' ||
              pg_catalog.to_json(${table.endpoint_envelope}->>'runtimeAgentId')::text ||
              ',"registryUrl":' ||
              pg_catalog.to_json(${table.endpoint_envelope}->>'registryUrl')::text ||
              ',"bridgeUrl":' ||
              pg_catalog.to_json(${table.endpoint_envelope}->>'bridgeUrl')::text ||
              ',"healthUrl":' ||
              pg_catalog.to_json(${table.endpoint_envelope}->>'healthUrl')::text || '}',
              'UTF8')), 'hex'))
