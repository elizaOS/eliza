...
import { agentBackupGcOutbox } from "./agent-backup-catalog";
...
export const agentBackupAdmissionEnrollmentShards = pgTable(
  ...
  (table) => ({
    pk: primaryKey({ columns: [table.work_kind, table.shard_id] }),
    bounds_check: check(
    ...
    scan_shape_check: check(
      ...
      sql`(
        ...
        AND (
          (${table.scan_cursor_due_at} IS NULL AND ${table.scan_cursor_id} IS NULL)
          OR (${table.scan_cursor_due_at} IS NOT NULL AND ${table.scan_cursor_id} IS NOT NULL)
        )
      )`,
    ),
    lease_shape_check: check(
      ...
      sql`(
        ...
        ${table.lease_owner} IS NOT NULL
        AND btrim(${table.lease_owner}) <> ''
        AND octet_length(${table.lease_owner}) <= 128
...
export const agentBackupAdmissionWork = pgTable(
  "agent_backup_admission_work",
  {
    ...
    gc_outbox_id: uuid("gc_outbox_id"),
    node_history_id: uuid("node_history_id").references(() => agentNodeIncarnationHistories.id, {
      onDelete: "restrict",
    }),
  ...
  (table) => ({
    ...
    gc_tenant_fk: foreignKey({
      name: "agent_backup_admission_work_gc_tenant_fkey",
      columns: [table.gc_outbox_id, table.organization_id],
      foreignColumns: [agentBackupGcOutbox.id, agentBackupGcOutbox.organization_id],
    }).onDelete("restrict"),
    schedule_uidx: uniqueIndex("agent_backup_admission_work_schedule_uidx")
      .on(table.sandbox_id, table.source_due_at)
      .where(sql`${table.work_kind} = 'schedule_capture'`),
    operation_uidx: uniqueIndex("agent_backup_admission_work_operation_uidx")
      .on(table.backup_id)
      .where(sql`${table.work_kind} = 'catalog_operation'`),
    gc_uidx: uniqueIndex("agent_backup_admission_work_gc_uidx")
      .on(table.gc_outbox_id)
      .where(sql`${table.work_kind} = 'gc_object'`),
    ...
    reference_shape_check: check(
      ...
      sql`(
        ...
        AND ${table.gc_outbox_id} IS NULL
      ) OR (
        ...
        AND ${table.sandbox_id} IS NULL
        AND ${table.backup_id} IS NULL
        AND ${table.gc_outbox_id} IS NOT NULL
      )`,
    ),
    priority_check: check(
      ...
      sql`(
        ...
        ${table.work_kind} <> 'gc_object'
        OR ${table.priority_class} = 'garbage_collection'
      )`,
    ),
    lane_check: check(
      "agent_backup_admission_work_lane_check",
      sql`(
        NOT ${table.requires_node_lane}
        OR ${table.node_history_id} IS NOT NULL
      ) AND (
        ${table.work_kind} <> 'schedule_capture'
        OR (${table.requires_node_lane} AND ${table.node_history_id} IS NOT NULL)
      ) AND (
        ${table.work_kind} <> 'gc_object'
        OR (NOT ${table.requires_node_lane} AND ${table.node_history_id} IS NULL)
      )`,
    ),
    ...
    counters_check: check(
      ...
      sql`${table.ready_cohort} >= 0
        ...
        AND ${table.shard_id} = agent_backup_admission_expected_shard(
          COALESCE(${table.sandbox_id}, ${table.backup_id}, ${table.gc_outbox_id})
        )
