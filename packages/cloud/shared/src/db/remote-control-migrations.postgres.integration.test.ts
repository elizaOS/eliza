...
realPostgresTest(
  ...
  async () => {
    ...
    try {
      ...
      const afterTwoPhase = await client.query<{ definition: string }>(`
        SELECT pg_get_constraintdef(oid) AS definition
        FROM pg_constraint
        WHERE conname = 'remote_sessions_status_check'
      `);
      expect(afterTwoPhase.rows).toHaveLength(1);
      expect(afterTwoPhase.rows[0]?.definition).toContain("activating");

      const managedColumnsBeforeUpgrade = await client.query<{ count: string }>(`
        SELECT count(*)::text AS count
        FROM information_schema.columns
        WHERE table_schema = 'public'
          AND table_name = 'remote_hosts'
          AND column_name = 'headscale_preauth_key_id'
      `);
      expect(managedColumnsBeforeUpgrade.rows).toEqual([{ count: "0" }]);

      await applyMigration(client, "0312_remote_host_managed_network");
