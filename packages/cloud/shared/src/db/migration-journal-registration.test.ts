...
describe("migrations/meta/_journal.json registration", () => {
  ...
  test("keeps the deployed 0320-0328 stack and appends the 0329-0330 tail", () => {
    const entries = journalEntries();
    const sharedConsent = entries.find(
      ({ tag }) => tag === "0320_personal_shared_multi_principal_consent",
    );
    const replacementTail = entries.find(
    ...
    const backupAdmissionCursors = entries.find(
      ({ tag }) => tag === "0330_agent_backup_admission_cursors",
    );
    ...
    expect(backupAdmissionCursors).toMatchObject({ idx: 313, when: 1794254400021 });
  });
