...
function insertOperation(overrides = ""): string {
  return `INSERT INTO agent_backup_restore_operations (
    ...
    lease_owner_id, catalog_epoch, copy_role, expected_manifest_sha256,
    expected_activation_generation, expected_lifecycle_revision
  ) VALUES (
    '${ORG}', '${AGENT}', '${BACKUP}', '${ATTEMPT}', '${LEASE}', '${FENCE}',
    'restore-worker', 9, 'primary', '${SHA}', '${GENERATION}', 18446744073709551615
  ) ${overrides}`;
}
...
describe("0251-0252 restore operation spine", () => {
  test("accepts an exact reserved operation and refuses a second open one per backup", async () => {
    ...
    try {
      ...
      await database.exec(`INSERT INTO agent_backup_restore_leases
        VALUES ('${LEASE.replace(/1$/, "2")}', '${ORG}', '${BACKUP}',
          '${ATTEMPT.replace(/1$/, "2")}', '${FENCE.replace(/1$/, "2")}')`);
      await expect(
        database.exec(
          insertOperation()
  ...
  test("refuses a fence or attempt that belongs to another tenant", async () => {
    const database = await prerequisiteDatabase();
    try {
      await database.exec(`INSERT INTO agent_backup_catalog_authorities
        VALUES ('${ORG_B}', '${AGENT}')`);
      await expect(
        database.exec(insertOperation().replace(`'${ORG}'`, `'${ORG_B}'`)),
      ).rejects.toThrow(/attempt_authority_fkey/);
      await expect(
        database.exec(insertOperation().replace(`'${FENCE}'`, `'${GENERATION}'`)),
      ).rejects.toThrow(/fence_authority_fkey/);
    } finally {
      await database.close();
    }
  }, 60_000);
