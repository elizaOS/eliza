...
CREATE OR REPLACE FUNCTION "guard_agent_backup_admission_claim_shard_update"()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
  old_active boolean := OLD."cycle_observed_at" IS NOT NULL;
  new_active boolean := NEW."cycle_observed_at" IS NOT NULL;
  first_pass smallint := 0;
  final_pass smallint := CASE NEW."work_kind"
    WHEN 'schedule_capture' THEN 3 WHEN 'catalog_operation' THEN 5 ELSE 6 END;
  old_at_high_water boolean := OLD."scan_cursor_id" IS NOT NULL AND
    (OLD."scan_cursor_cohort", OLD."scan_cursor_ordinal", OLD."scan_cursor_id") =
    (OLD."cycle_max_cohort", OLD."cycle_max_ordinal", OLD."cycle_max_id");
...
BEGIN
  ...
  END IF;
  IF new_active AND (
    "agent_backup_admission_expected_shard"(NEW."cycle_max_id") <> NEW."shard_id"
    OR (NEW."scan_cursor_id" IS NOT NULL AND
      "agent_backup_admission_expected_shard"(NEW."scan_cursor_id") <> NEW."shard_id")
  ) THEN
  ...
  IF old_active AND new_active THEN
    IF NEW."cycle_observed_at" IS DISTINCT FROM OLD."cycle_observed_at"
      OR NEW."cycle_max_cohort" IS DISTINCT FROM OLD."cycle_max_cohort"
      OR NEW."cycle_max_ordinal" IS DISTINCT FROM OLD."cycle_max_ordinal"
      OR NEW."cycle_max_id" IS DISTINCT FROM OLD."cycle_max_id"
    ...
    restarting_after_admission :=
      ...
      AND NEW."last_admitted_work_id" IS DISTINCT FROM OLD."last_admitted_work_id"
      AND EXISTS (
        SELECT 1
        FROM "agent_backup_admission_work" AS admitted
        WHERE admitted."id" = NEW."last_admitted_work_id"
          AND admitted."work_kind" = NEW."work_kind"
          AND admitted."shard_id" = NEW."shard_id"
          AND admitted."state" = 'leased'
          AND admitted.xmin = pg_current_xact_id()::xid
          AND (admitted."ready_cohort", admitted."cohort_ordinal", admitted."id") <=
            (NEW."cycle_max_cohort", NEW."cycle_max_ordinal", NEW."cycle_max_id")
      );
    IF NEW."last_admitted_work_id" IS DISTINCT FROM OLD."last_admitted_work_id"
      AND NOT restarting_after_admission THEN
      RAISE EXCEPTION 'backup admission claim restart requires an exact current admission' USING ERRCODE = '55000';
    END IF;
    IF restarting_after_admission THEN
      NULL;
    ELSIF NEW."priority_pass" = OLD."priority_pass" THEN
      IF NEW."scan_cursor_id" IS NULL OR (
        OLD."scan_cursor_id" IS NOT NULL AND
        (NEW."scan_cursor_cohort", NEW."scan_cursor_ordinal", NEW."scan_cursor_id") <=
        (OLD."scan_cursor_cohort", OLD."scan_cursor_ordinal", OLD."scan_cursor_id")
      ) THEN
  ...
  ELSIF NOT old_active AND new_active THEN
    IF NEW."priority_pass" <> first_pass OR NEW."scan_cursor_id" IS NOT NULL
      OR NEW."last_admitted_work_id" IS NOT NULL THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'agent_backup_admission_claim_shard_update_guard' AND tgrelid = 'agent_backup_admission_claim_shards'::regclass) THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'agent_backup_admission_claim_shard_delete_guard' AND tgrelid = 'agent_backup_admission_claim_shards'::regclass) THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'agent_backup_admission_claim_shard_truncate_guard' AND tgrelid = 'agent_backup_admission_claim_shards'::regclass) THEN
