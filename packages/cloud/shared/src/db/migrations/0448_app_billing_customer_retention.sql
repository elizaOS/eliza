-- Retention permits no provider write and is not deletion phase completion evidence.
...
BEGIN
  ...
  )) THEN RAISE EXCEPTION 'Every sharing scope requires its current canonical retention or close decision'; END IF;
