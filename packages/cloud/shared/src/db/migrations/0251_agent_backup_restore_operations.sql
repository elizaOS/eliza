-- Durable per-attempt restore coordination. The catalogue row is per-backup and
-- singular; a restore is per-attempt and has one side effect per phase, so the
-- expected_* columns pre-record each effect's exact identity and a lost response
-- is re-verified instead of re-executed. The fencing token is the lease's own
-- generation -- a second token would be a second source of truth.
...
CREATE TABLE IF NOT EXISTS "agent_backup_restore_operations" (
  ...
  CONSTRAINT "agent_backup_restore_operations_attempt_authority_fkey"
    FOREIGN KEY ("organization_id", "restore_attempt_id")
    REFERENCES "agent_backup_restore_leases" ("organization_id", "restore_attempt_id")
    ON DELETE RESTRICT,
  CONSTRAINT "agent_backup_restore_operations_fence_authority_fkey"
    FOREIGN KEY ("organization_id", "backup_id", "lease_generation")
    REFERENCES "agent_backup_restore_leases" ("organization_id", "backup_id", "generation")
    ON DELETE RESTRICT,
  CONSTRAINT "agent_backup_restore_operations_catalog_authority_fkey"
    FOREIGN KEY ("organization_id", "agent_id")
    REFERENCES "agent_backup_catalog_authorities" ("organization_id", "agent_id")
    ON DELETE RESTRICT,
  CONSTRAINT "agent_backup_restore_operations_phase_check" CHECK ((
  ...
  CONSTRAINT "agent_backup_restore_operations_claim_shape_check" CHECK ((
  ...
  CONSTRAINT "agent_backup_restore_operations_receipt_shape_check" CHECK ((
  ...
  CONSTRAINT "agent_backup_restore_operations_failure_replay_check" CHECK ((
