...
CREATE INDEX IF NOT EXISTS "agent_backup_admission_claim_shards_turn_idx"
  ON "agent_backup_admission_claim_shards" ("work_kind", "last_turn", "shard_id");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_backup_admission_work_claim_scan_idx"
  ON "agent_backup_admission_work" (
    "work_kind", "shard_id", "ready_cohort", "cohort_ordinal", "id"
  ) WHERE "state" = 'queued';
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_backup_admission_work_deferred_ready_shard_idx"
  ON "agent_backup_admission_work" (
    "work_kind", "shard_id", "not_before", "id"
  ) WHERE "state" = 'deferred';
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_backup_admission_work_expired_lease_shard_idx"
  ON "agent_backup_admission_work" (
    "work_kind", "shard_id", "lease_expires_at", "id"
  ) WHERE "state" = 'leased';
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_sandbox_backups_admission_active_org_idx"
...
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_sandbox_backups_admission_capture_history_idx"
...
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_sandbox_backups_admission_capture_fallback_idx"
