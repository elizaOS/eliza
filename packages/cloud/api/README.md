...
A codegen step (`src/_generate-router.mjs`) walks the package, finds every `route.ts` / `route.tsx`, and writes `src/_router.generated.ts`, which exports `mountRoutes(app)`. Hono-shaped leaves are mounted when they import from `hono`, or import the shared `createMcpsTransportApp` factory (the `mcps/*/[transport]` routes), unless an exact approved first-line `// route-codegen: skip` directive keeps the route dormant. Run `bun run codegen` after adding or removing a route.
...
- `bun run check:router-contract` — verify generated mount parity and approved dormant routes
