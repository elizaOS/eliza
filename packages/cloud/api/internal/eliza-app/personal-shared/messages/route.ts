...
type GroupMessage = Extract<SharedMessage, { chatType: "group" | "supergroup" }>;
...
function groupPolicyCommand(message: string): "mention_only" | "ambient" | null {
  const match = message.match(
...
function isGroupLeaveCommand(message: string): boolean {
  return /^(?:\/eliza_leave(?:@[a-z0-9_]{5,32})?|eliza\s+leave)$/i.test(message);
}
  ...
  try {
    ...
    if (isGroupMessage(parsed.data)) {
      ...
      if (claimCode) {
        ...
        const claimed = await personalSharedGroupsRepository.consumeClaimAndBind({
        ...
        if (claimed.status !== "bound") {
          const code = `group_claim_${claimed.status}`;
          return c.json({
            success: true,
            data: {
              code,
      ...
      const ownerControl =
        groupPolicyCommand(parsed.data.message) !== null ||
        isGroupLeaveCommand(parsed.data.message);
      if (
        ownerControl &&
        ...
        return c.json({
          ...
          data: {
            ...
            reply: "Only the owner who linked Eliza can change this group's response policy.",
          },
        });
      }
      ...
      if (isGroupLeaveCommand(parsed.data.message)) {
        ...
        return c.json({
          ...
          data: {
            ...
            reply: "This group is disconnected from your Eliza. Remove the bot/account here, or DM Eliza `/group` later to reconnect.",
          },
        });
      }
      if (
        ...
        return c.json({ success: true, data: { code: "group_silent", reply: "" } });
      }
    ...
    if (groupConversationId && groupPersonalAgentId !== agent.id) {
      throw new Error("Personal Shared group binding does not match its canonical owner");
    }
    ...
    const startPersonalPrewarm = () => {
      ...
      const timing = prewarmPersonalSharedAgentTurnCaches(
        ...
        worker.namespace,
        {
          warmConversation: isNewPersonalAccount || Boolean(groupConversationId),
    ...
    if (dedicated && !gatewayNativeReminderTurn) {
      ...
      if (response.error?.message === "Bridge returned HTTP 404") {
        ...
        const history = await coordinateSharedHistory(agent.id, conversationId, {
