...
import { ELIZA_TRACE_ID_HEADER } from "@/lib/observability/http-telemetry";
import { Hono } from "hono";
import { KNOWN_ROUTE_SHARD_KEYS } from "./_router-shard-keys.generated";
import { PERSONAL_TELEGRAM_DELIVERY_EPOCH } from "./personal-telegram-delivery";
import { PLAYWRIGHT_TEST_SESSION_COOKIE_NAME } from "@elizaos/cloud-shared/lib/auth/playwright-test-session";
import { canonicalCloudPathForLegacyDashboard } from "@elizaos/plugin-elizacloud/cloud-config/domain-contract";
import { canonicalElizaServiceHostname } from "@elizaos/plugin-elizacloud/cloud-config/domain-contract";
import { classifyElizaHostname } from "@elizaos/plugin-elizacloud/cloud-config/domain-contract";
import { cloneRequestWithScheduledCronMetadata } from "@/lib/cron/cloudflare-cron";
...
import { isPersonalTelegramDeliveryEpoch1CompatEnabled } from "./personal-telegram-delivery";
import { isPlaywrightTestAuthEnabled } from "@elizaos/cloud-shared/lib/auth/playwright-test-session";
import { isStorageReadCapabilityPath } from "./blob-host";
...
import { setHttpTelemetryHeaders } from "@/lib/observability/http-telemetry";
...
export { isThinStewardEmailAuthPath, isThinStewardPasskeyLoginOptionsPath, isThinStewardPath, isThinStewardPublicPath, } from "./steward/public-paths";
...
const CRYPTO_PAYMENT_CONFIRM_PATH_RE = /^\/api\/crypto\/payments\/[^/]+\/confirm\/?$/;
...
async function handleCryptoPaymentConfirmBeforeShard(request: Request, env: AppEnv["Bindings"], ctx: ExecutionContext | HonoExecutionContext): Promise<Response | null> {
    ...
    const hasPossibleSession = Boolean(readStewardAccessCookieFromHeader(request.headers.get("cookie"), env.ENVIRONMENT) ||
        STRUCTURAL_JWT_RE.test(bearer) ||
        (isPlaywrightTestAuthEnabled(env) &&
            getCookieValueFromHeader(request.headers.get("cookie"), PLAYWRIGHT_TEST_SESSION_COOKIE_NAME)));
...
const STAGING_SESSION_UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
...
function exactInferenceRoute(pathname: string, load: InferenceRouteSpec["load"]): InferenceRouteSpec {
    return {
        key: pathname,
        mountPath: pathname,
...
const INFERENCE_ROUTES: readonly InferenceRouteSpec[] = [
    exactInferenceRoute("/api/v1/chat/completions", () => import("../v1/chat/completions/route")),
    exactInferenceRoute("/api/v1/messages", () => import("../v1/messages/route")),
    exactInferenceRoute("/api/v1/responses", () => import("../v1/responses/route")),
    exactInferenceRoute("/api/v1/embeddings", () => import("../v1/embeddings/route")),
    exactInferenceRoute("/api/v1/chat", () => import("../v1/chat/route")),
    exactInferenceRoute("/api/v1/voice/stt", () => import("../v1/voice/stt/route")),
    exactInferenceRoute("/api/v1/voice/tts", () => import("../v1/voice/tts/route")),
    exactInferenceRoute("/api/v1/generate-image", () => import("../v1/generate-image/route")),
    exactInferenceRoute("/api/v1/generate-video", () => import("../v1/generate-video/route")),
    exactInferenceRoute("/api/v1/generate-music", () => import("../v1/generate-music/route")),
    exactInferenceRoute("/api/v1/generate-sfx", () => import("../v1/generate-sfx/route")),
    exactInferenceRoute("/api/v1/generate-prompts", () => import("../v1/generate-prompts/route")),
    {
        key: "app-chat",
    ...
    },
    {
        ...
        matches: (pathname) => /^\/api\/v1\/apps\/[^/]+\/generate-image$/.test(pathname),
...
const DEFAULT_AGENT_BASE_DOMAIN = ELIZA_DOMAIN_CONTRACTS.production.dedicatedAgentHostnameSuffix.slice(1);
const FRONTEND_ALIAS_TARGETS: Record<string, {
    appHost: string;
    apiHost: string;
}> = {
    "cloud.eliza.app": {
...
type AgentDomainBindings = Pick<AppEnv["Bindings"], "AGENT_ROUTER_ORIGIN_HOST" | "ELIZA_CLOUD_AGENT_BASE_DOMAIN">;
function getAppForPath(pathname: string): Promise<Hono<AppEnv>> {
    ...
    if (!promise) {
        promise = import("./bootstrap-app").then((m) => m.createApp({ requestPath: pathname }));
...
export function decorateFullAppDispatchResponse(response: Response, traceId: string, dispatchMs: number, moduleInitMs: number | null): Response {
    if (!shouldDecorateHttpTelemetryStatus(response.status))
        return response;
...
export async function dispatchFullApp(request: Request, env: AppEnv["Bindings"], ctx: ExecutionContext | HonoExecutionContext, loadFullApp?: () => Promise<Hono<AppEnv>>): Promise<Response> {
    ...
    const cryptoConfirmAuthRejection = await handleCryptoPaymentConfirmBeforeShard(request, env, ctx);
    if (cryptoConfirmAuthRejection)
        return cryptoConfirmAuthRejection;
    const startedAt = performance.now();
    const requestPathname = new URL(request.url).pathname;
    const moduleWasInitialized = loadFullApp
        ? true
        : fullAppPromises.has(knownRouteShardKey(requestPathname, knownRouteShards));
    ...
    try {
        ...
        return decorateFullAppDispatchResponse(response, traceId, dispatchMs, moduleWasInitialized ? null : moduleInitMs);
    }
    finally {
        ...
        if (status === null ||
            ...
            durationMs >= 1000 ||
            (moduleInitMs ?? 0) >= 250) {
            logger.warn("[CloudEntrypoint] full app dispatch slow or failed", logContext);
        }
        else {
...
async function getStewardThinApp(): Promise<Hono<AppEnv>> {
    stewardThinAppPromise ??= import("./steward/thin-app").then((m) => m.createStewardThinApp());
    return stewardThinAppPromise;
}
async function getCliSessionThinApp(): Promise<Hono<AppEnv>> {
    cliSessionThinAppPromise ??= import("./cli-session-app").then((m) => m.createCliSessionThinApp());
    return cliSessionThinAppPromise;
}
const ELIZA_APP_WEBHOOK_PATH = /^\/api\/eliza-app\/webhook\/(?:blooio|discord|telegram|twilio|whatsapp)(?:\/|$)/;
...
async function getWebhookApp(): Promise<Hono<AppEnv>> {
    webhookAppPromise ??= import("./webhook-app").then((module) => module.createWebhookApp());
    return webhookAppPromise;
}
async function dispatchWebhook(request: Request, env: AppEnv["Bindings"], ctx: ExecutionContext): Promise<Response | null> {
    const pathname = new URL(request.url).pathname;
    if (!isElizaAppWebhookPath(pathname))
        return null;
    const startedAt = performance.now();
    ...
    if (response.status >= 500 || dispatchMs >= 1000 || moduleInitMs >= 250) {
        logger.warn("[CloudEntrypoint] webhook dispatch slow or failed", logContext);
    }
    else {
...
async function getDiscordGatewayApp(): Promise<Hono<AppEnv>> {
    discordGatewayAppPromise ??= import("./discord-gateway-app").then((module) => module.createDiscordGatewayApp());
    return discordGatewayAppPromise;
}
async function dispatchDiscordGateway(request: Request, env: AppEnv["Bindings"], ctx: ExecutionContext): Promise<Response | null> {
    const pathname = new URL(request.url).pathname;
    if (!isInternalDiscordGatewayPath(pathname))
        return null;
    const startedAt = performance.now();
    ...
    if (response.status >= 500 || dispatchMs >= 1000 || moduleInitMs >= 250) {
        logger.warn("[CloudEntrypoint] Discord gateway dispatch slow or failed", logContext);
    }
    else {
        logger.info("[CloudEntrypoint] Discord gateway dispatch completed", logContext);
    }
    return new Response(response.body, {
        status: response.status,
        statusText: response.statusText,
...
async function dispatchCliSession(request: Request, env: AppEnv["Bindings"], ctx: ExecutionContext): Promise<Response | null> {
    const pathname = new URL(request.url).pathname;
    if (!isThinCliSessionPath(request.method, pathname))
        return null;
    ...
    headers.append("Server-Timing", `entry_dispatch;dur=${dispatchMs.toFixed(1)}`);
    if (!moduleWasInitialized) {
        headers.append("Server-Timing", `cli_session_module_init;dur=${moduleInitMs.toFixed(1)}`);
    }
    return new Response(response.body, {
        status: response.status,
        statusText: response.statusText,
...
async function dispatchThinSteward(request: Request, env: AppEnv["Bindings"], ctx: ExecutionContext): Promise<Response | null> {
    const pathname = new URL(request.url).pathname;
    if (!isThinStewardPath(request.method, pathname))
        return null;
    const dispatchStartedAt = performance.now();
    ...
    headers.append("Server-Timing", `entry_dispatch;dur=${dispatchMs.toFixed(1)}`);
    if (!moduleWasInitialized) {
        headers.append("Server-Timing", `steward_module_init;dur=${moduleInitMs.toFixed(1)}`);
    }
    return new Response(response.body, {
        status: response.status,
        statusText: response.statusText,
...
async function getInferenceApp(spec: InferenceRouteSpec): Promise<Hono<AppEnv>> {
    let promise = inferenceAppPromises.get(spec.key);
    if (!promise) {
        promise = Promise.all([import("./inference-app"), spec.load()]).then(([shell, route]) => shell.createInferenceApp(spec.mountPath, route.default));
        inferenceAppPromises.set(spec.key, promise);
    }
    return promise;
}
export function isThinInferenceEnabled(env: Pick<AppEnv["Bindings"], "THIN_INFERENCE_ENTRY_ENABLED">): boolean {
...
async function dispatchInference(request: Request, env: AppEnv["Bindings"], ctx: ExecutionContext): Promise<Response | null> {
    if (!isThinInferenceEnabled(env)) {
        return null;
    }
    const pathname = new URL(request.url).pathname;
    const route = INFERENCE_ROUTES.find((candidate) => candidate.matches(pathname));
    if (!route)
        return null;
    const dispatchStartedAt = performance.now();
    ...
    response.headers.append("Server-Timing", `entry_dispatch;dur=${dispatchMs.toFixed(1)}`);
    if (!moduleWasInitialized) {
        response.headers.append("Server-Timing", `inference_module_init;dur=${moduleInitMs.toFixed(1)}`);
    }
    return response;
}
function healthResponse(env: AppEnv["Bindings"]): Response {
    const personalSharedTelegramEdgeEnabled = isPersonalSharedTelegramEdgeEnabled(env);
    const stagingSessionVersion = env.STAGING_SESSION_EXCHANGE_VERSION?.trim() || null;
    const stagingSessionSigningSecret = env.STAGING_SESSION_EXCHANGE_SIGNING_SECRET?.trim() ?? "";
    const stagingSessionEnabled = env.NODE_ENV === "production" &&
        env.ENVIRONMENT === "staging" &&
        env.STAGING_SESSION_EXCHANGE_ENABLED === "true" &&
        env.STAGING_SESSION_EXCHANGE_VERSION === "v1";
    const stagingSessionConfigured = env.NODE_ENV === "production" &&
        env.ENVIRONMENT === "staging" &&
        env.STAGING_SESSION_EXCHANGE_VERSION === "v1" &&
        ...
        STAGING_SESSION_KEY_ID_RE.test(env.STAGING_SESSION_EXCHANGE_SIGNING_KEY_ID?.trim() ?? "") &&
        Boolean(env.STEWARD_TENANT_ID?.trim()) &&
        hasExactStagingSessionUuidList(env.STAGING_SESSION_EXCHANGE_ALLOWED_API_KEY_IDS) &&
        hasExactStagingSessionUuidList(env.STAGING_SESSION_EXCHANGE_ALLOWED_USER_IDS) &&
        hasExactStagingSessionUuidList(env.STAGING_SESSION_EXCHANGE_ALLOWED_ORGANIZATION_IDS);
    const e2eRunReceipt = env.NODE_ENV === "test" && env.CLOUD_E2E === "1"
    ...
    return Response.json({
        ...
        personalTelegramDelivery: {
            ...
            legacyEpoch1CompatEnabled: isPersonalTelegramDeliveryEpoch1CompatEnabled(env),
        },
        ...
        stagingSessionExchange: env.ENVIRONMENT === "staging"
            ? {
...
export function getGeneratedAgentId(url: URL, env: AgentDomainBindings): string | null {
    const baseDomain = normalizeHostname(env.ELIZA_CLOUD_AGENT_BASE_DOMAIN) ??
        DEFAULT_AGENT_BASE_DOMAIN;
    const suffix = `.${baseDomain}`;
    const hostname = normalizeHostname(url.hostname);
    if (hostname?.endsWith(suffix)) {
        const subdomain = hostname.slice(0, -suffix.length);
        if (AGENT_ID_RE.test(subdomain))
            return subdomain;
    }
    return null;
}
export function redirectFrontendHost(url: URL, _env: AgentDomainBindings): Response | null {
    const hostname = normalizeHostname(url.hostname);
    if (!hostname)
        return null;
    ...
    if (hostname === "docs.elizacloud.ai") {
        return Response.redirect(ELIZA_DOMAIN_CONTRACTS.production.marketingOrigin, 308);
    }
    ...
    if (hostname === "www.eliza.app") {
        canonicalHostname = new URL(ELIZA_DOMAIN_CONTRACTS.production.marketingOrigin).hostname;
    }
    else if (classified.role === "legacy-marketing") {
        const contract = ELIZA_DOMAIN_CONTRACTS[classified.environment ?? "production"];
    ...
    else if (classified.role === "legacy-dedicated-agent" &&
        classified.agentId &&
...
export function isUnsupportedLegacyWildcardHostname(rawHostname: string): boolean {
    const hostname = normalizeHostname(rawHostname);
    if (!hostname?.endsWith(".elizacloud.ai"))
        return false;
    if (hostname === "docs.elizacloud.ai" || hostname === "os.elizacloud.ai") {
        return false;
    }
    if (canonicalElizaServiceHostname(hostname))
        return false;
    const classified = classifyElizaHostname(hostname);
    if (classified.role === "legacy-marketing" ||
        classified.role === "legacy-cloud-app" ||
        classified.role === "legacy-cloud-api") {
        return false;
    }
...
function rejectUnsupportedLegacyWildcardHost(url: URL): Response | null {
    if (!isUnsupportedLegacyWildcardHostname(url.hostname))
        return null;
...
export function getFrontendAliasApiProxyTarget(url: URL): URL | null {
    ...
    if (!hostname)
        return null;
    const target = FRONTEND_ALIAS_TARGETS[hostname];
    if (!target || !isFrontendAliasBackendPath(url))
        return null;
    const targetUrl = new URL(url);
    targetUrl.hostname = target.apiHost;
    return targetUrl;
}
function proxyFrontendAliasRequest(request: Request, url: URL): Promise<Response> | null {
...
function createFrontendAliasProxyInit(request: Request, url: URL): RequestInit {
    ...
    for (const [name, value] of request.headers) {
        ...
        if (FRONTEND_ALIAS_PROXY_HEADER_DENYLIST.has(headerName) ||
...
function proxyGeneratedAgentRequest(request: Request, env: AppEnv["Bindings"], url: URL): Promise<Response> | null {
    ...
    return import("./dedicated-agent-proxy").then((m) => m.handleDedicatedAgentProxy(request, env, url, agentId));
}
/**
 ...
 */
export function getHostedFrontendServeRewrite(url: URL, env: {
...
const scheduled = makeCronHandler(async (request, env, ctx) => dispatchFullApp(request, env, ctx));
export default {
    fetch: async (request: Request, env: AppEnv["Bindings"], ctx: ExecutionContext) => {
        const nativeSelectionError = nativeApplicationSelectionSurfaceError(request);
        if (nativeSelectionError)
            return nativeSelectionError;
        const url = new URL(request.url);
        ...
        if (isStorageReadCapabilityPath(url)) {
            return ((await serveBlobHostRequest(request, url, env)) ??
                Response.json({ success: false, error: "Not found", code: "resource_not_found" }, { status: 404, headers: { "cache-control": "private, no-store" } }));
        }
        ...
        if (frontendAliasApiTarget) {
            ...
            const apiRequest = new Request(frontendAliasApiTarget.toString(), createFrontendAliasProxyInit(request, url));
            const webhookResponse = await dispatchWebhook(apiRequest, env, ctx);
            if (webhookResponse)
                return webhookResponse;
            const discordGatewayResponse = await dispatchDiscordGateway(apiRequest, env, ctx);
            if (discordGatewayResponse)
                return discordGatewayResponse;
            const cliSessionThinResponse = await dispatchCliSession(apiRequest, env, ctx);
            if (cliSessionThinResponse)
                return cliSessionThinResponse;
            const stewardThinResponse = await dispatchThinSteward(apiRequest, env, ctx);
            if (stewardThinResponse)
                return stewardThinResponse;
        ...
        if (frontendAliasResponse)
            return frontendAliasResponse;
        ...
        if (unsupportedLegacyHost)
            return unsupportedLegacyHost;
        ...
        if (webhookResponse)
            return webhookResponse;
        const discordGatewayResponse = await dispatchDiscordGateway(request, env, ctx);
        if (discordGatewayResponse)
            return discordGatewayResponse;
        ...
        if (p.startsWith("/v1/") ||
            ...
            const rewrittenInferenceResponse = await dispatchInference(rewrittenRequest, env, ctx);
            if (rewrittenInferenceResponse)
                return rewrittenInferenceResponse;
