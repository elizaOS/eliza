...
v1/eliza/agents/[agentId]/connectors/
                           Canonical-agent connector binding CRUD. Google MCP execution is direct from the runtime.
...
- Agent connector routes may accept a sandbox ID for compatibility, but must resolve and persist the canonical character/runtime ID. They are control-plane routes only and must never accept or return a platform credential locator or OAuth token; the runtime calls official Google MCP resources directly from a binding-scoped short-lived bearer.
