/** Exercises real session-manager and cookie mutation boundaries with controlled primary membership and undo-cancellation service collaborators. */
...
mock.module("@/lib/services/subscription-cancellation", () => ({
  submitOrganizationSubscriptionCancellationUndo: async (
  ...
  readOrganizationSubscriptionCancellationUndo: async (input: {
...
app.route("/api/v1/subscriptions/cancel/undo", submit);
app.route("/api/v1/subscriptions/cancel/undo/:commandId", poll);
const url = "https://api.eliza.app/api/v1/subscriptions/cancel/undo";
function request(
  body: unknown = {
    subscriptionId: SUB,
    expectedSubscriptionRevision: 1,
