/**
 * POST /api/v1/apps/:id/domains/status used to let c.req.json() throw into
...
describe("POST /api/v1/apps/:id/domains/status malformed JSON", () => {
  ...
  test("canonical JSON still returns stored status", async () => {
    const response = await app.request("/", {
      method: "POST",
      headers: { "content-type": "application/json" },
