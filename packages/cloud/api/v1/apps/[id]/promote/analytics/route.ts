...
async function __hono_GET(request: Request, { params }: RouteContext<{
    id: string;
}>) {
    try {
        ...
        const attribution = await conversionTrackingService.getCampaignAttribution(user.organization_id!, { appId: id });
