...
assert.equal(parsePaginationParam("9007199254740991", "offset", 0), 9007199254740991);

for (const value of ["0", "-1", "+1", "1.5", "1e2", "007", "501", "12px"]) {
  const result = parsePaginationParam(value, "limit", 50);
  assert.equal(typeof result, "string", `limit ${JSON.stringify(value)} must fail`);
  assert.match(result, /limit/);
  assert.ok(result.includes(value));
}
for (const value of ["-1", "+1", "1.5", "1e2", "007", "12px", "9007199254740992"]) {
  const result = parsePaginationParam(value, "offset", 0);
  assert.equal(typeof result, "string", `offset ${JSON.stringify(value)} must fail`);
...
for (const file of routeFiles) {
  ...
  assert.match(source, /function parsePaginationParam\(/, `${file}: parser missing`);
  assert.match(source, /\^\(\?:0\|\[1-9\]\\d\*\)\$/, `${file}: canonical grammar missing`);
  assert.match(source, /Number\.isSafeInteger\(parsed\)/, `${file}: safe-integer guard missing`);
  assert.match(source, /parameter === "limit" \? 500/, `${file}: limit ceiling missing`);
...
const billing = readFileSync(new URL("billing-ledger-route.ts", import.meta.url), "utf8");
assert.match(billing, /parsePaginationParam\(c\.req\.query\("limit"\), "limit", 50\)/);

for (const file of ["ballots-route.ts", "oauth-intents-route.ts"]) {
  const source = readFileSync(new URL(file, import.meta.url), "utf8");
  assert.match(source, /parsePaginationParam\(c\.req\.query\("limit"\), "limit", 50\)/);
  assert.match(source, /parsePaginationParam\(c\.req\.query\("offset"\), "offset", 0\)/);
}

const gallery = readFileSync(new URL("gallery-route.ts", import.meta.url), "utf8");
assert.match(gallery, /parsePaginationParam\(c\.req\.query\("limit"\), "limit", 100\)/);
assert.match(gallery, /parsePaginationParam\(c\.req\.query\("offset"\), "offset", 0\)/);
