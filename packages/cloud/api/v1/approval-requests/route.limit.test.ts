/**
 * GET /api/v1/approval-requests `limit` is collection page-size identity,
 * leftover tax after approval-request `public` (#21135) and approvals
 * `state` (#20899). Stock develop used z.coerce.number(), which treated
 * `1e2` / `007` / `0x10` as a page size instead of a 400. offset /
 * status / challengeKind stay untouched. Missing / empty still means
 * the service default (no limit).
 */
import { beforeEach, describe, expect, mock, test } from "bun:test";
import { Hono } from "hono";

const list = mock(async () => []);

mock.module("@/lib/auth/workers-hono-auth", () => ({
  requireUserOrApiKeyWithOrg: async () => ({
    id: "user-1",
    organization_id: "org-1",
  }),
}));
mock.module("@/lib/middleware/rate-limit-hono-cloudflare", () => ({
  RateLimitPresets: { STANDARD: {} },
  rateLimit: () => async (_c: unknown, next: () => Promise<void>) => next(),
}));
mock.module("@/lib/api/cloud-worker-errors", () => ({
  failureResponse: (_c: unknown, error: unknown) => {
    throw error;
  },
}));
mock.module("@/lib/utils/logger", () => ({
  logger: {
    warn: () => undefined,
    info: () => undefined,
    error: () => undefined,
  },
}));
mock.module("@/db/repositories/approval-requests", () => ({
  approvalRequestsRepository: {},
}));
mock.module("@/lib/services/approval-requests", () => ({
  createApprovalRequestsService: () => ({
    list,
    create: mock(async () => ({})),
  }),
}));

const { default: route } = await import("./route");
const app = new Hono().route("/", route);

describe("GET /api/v1/approval-requests limit identity", () => {
  beforeEach(() => {
    list.mockClear();
  });

  test.each(["", "?limit=", "?limit"])(
    "accepts %s as the service-default approval-requests page",
    async (query) => {
      const response = await app.request(`/${query}`);
      expect(response.status).toBe(200);
      expect(list).toHaveBeenCalledTimes(1);
      expect(list).toHaveBeenCalledWith(
        "org-1",
        expect.objectContaining({ limit: undefined }),
      );
    },
  );

  test("accepts limit=10 as an exact approval-requests page size", async () => {
    const response = await app.request("/?limit=10");
    expect(response.status).toBe(200);
    expect(list).toHaveBeenCalledWith(
      "org-1",
      expect.objectContaining({ limit: 10 }),
    );
  });

  test("rejects a canonical oversize limit before list", async () => {
    const response = await app.request("/?limit=201");
    expect(response.status).toBe(400);
    const body = (await response.json()) as { error: string };
    expect(body.error).toBe("Invalid limit");
    expect(list).not.toHaveBeenCalled();
  });

  test.each([
    "1e2",
    "12px",
    "007",
    "0",
    "abc",
    "-1",
    "50abc",
    " 10",
    "10 ",
    "0x10",
  ])(
    "rejects prefix-coerced limit=%s before approvalRequestsService.list",
    async (token) => {
      const response = await app.request(
        `/?limit=${encodeURIComponent(token)}`,
      );
      expect(response.status).toBe(400);
      const body = (await response.json()) as { error: string };
      expect(body.error).toBe("Invalid limit");
      expect(list).not.toHaveBeenCalled();
    },
  );

  test.each([
    "?limit=10&limit=10",
    "?limit=10&limit=20",
    "?limit=&limit=10",
    "?limit=foo&limit=10",
  ])(
    "rejects duplicate limit values in %s before approvalRequestsService.list",
    async (query) => {
      const response = await app.request(`/${query}`);
      expect(response.status).toBe(400);
      const body = (await response.json()) as { error: string };
      expect(body.error).toBe("Invalid limit");
      expect(list).not.toHaveBeenCalled();
    },
  );
});
