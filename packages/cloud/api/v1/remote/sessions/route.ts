...
app.post("/", async (c) => {
    try {
        ...
        const session = await remoteSessionsRepository.createPendingForAuthenticatedHost({
            id: sessionId,
...
app.get("/", async (c) => {
    try {
        ...
        if (!isRemotePairingUuid(agentId || hostId)) {
            return c.json({ success: false, error: "Pairing target must be a UUID" }, 400);
        }
        const sessions = hostId
            ? await remoteSessionsRepository.listByOwnedHost(hostId, user.organization_id, user.id)
            : await remoteSessionsRepository.listActiveByOwnedAgent(agentId, user.organization_id, user.id);
        if (!sessions) {
