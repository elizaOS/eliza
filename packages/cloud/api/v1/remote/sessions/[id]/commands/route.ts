...
app.post("/", async (c) => {
    try {
        ...
        const rawEnvelope = typeof value === "object" && value !== null && !Array.isArray(value)
        ...
        if (envelope?.messageKind !== "command" ||
            ...
            return c.json({ success: false, error: "Command envelope is invalid" }, 400);
        }
        ...
        if (result.kind === "expired") {
            return c.json({ success: false, error: "Remote grant or command expired" }, 410);
        }
        ...
        if (result.kind === "session_capacity") {
            return c.json({
                success: false,
                error: "Remote session replay capacity is exhausted; create a new session",
