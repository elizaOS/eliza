...
app.post("/", async (c) => {
    try {
        ...
        if (!isRemotePairingUuid(sessionId) || !commandId) {
            return c.json({ success: false, error: "Session or command id is invalid" }, 400);
        }
        ...
        if (!Number.isSafeInteger(body.claimAttempt) ||
            !isRemotePairingUuid(typeof body.claimToken === "string" ? body.claimToken : "") ||
            !startReceipt ||
