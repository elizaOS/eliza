...
import { enrollManagedNetwork } from "../managed-network";
import { failureResponse } from "@/lib/api/cloud-worker-errors";
import { generateRemoteHostToken } from "@/db/crypto/remote-host-token";
import { hashRemoteHostToken } from "@/db/crypto/remote-host-token";
import { isRemoteConnectionMode } from "@elizaos/core/contracts/remote-control";
...
import { managedNetworkConfig } from "../managed-network";
...
app.get("/", async (c) => {
    try {
        ...
        const hosts = await remoteHostsRepository.listOwned(user.organization_id, user.id);
        return c.json({
            success: true,
            data: {
                ownerId: user.id,
...
app.post("/", async (c) => {
    try {
        ...
        if (body.managedNetwork !== undefined &&
            typeof body.managedNetwork !== "boolean") {
            return c.json({ success: false, error: "Managed network must be a boolean" }, 400);
        }
        const managedNetworkRequested = body.managedNetwork === true;
        const recoveryHostId = typeof body.recoveryHostId === "string" ? body.recoveryHostId.trim() : "";
        if (recoveryHostId && !isRemotePairingUuid(recoveryHostId)) {
            return c.json({ success: false, error: "Recovery host id must be a UUID" }, 400);
        }
        if (recoveryHostId && managedNetworkRequested) {
            return c.json({
                success: false,
                error: "Recover the host credential first; managed-network enrollment is one-use.",
            }, 409);
        }
        ...
        if (managedNetworkRequested) {
            try {
                networkConfig = managedNetworkConfig(c.env as unknown as Record<string, unknown>);
            }
            catch (cause) {
                return c.json({
                    success: false,
        ...
        if (!isRemoteControlIdentifier(body.deviceId) ||
            !isRemoteConnectionMode(connectionMode) ||
            !isRemoteTargetPublicIdentity(identity)) {
            return c.json({ success: false, error: "Remote host identity is invalid" }, 400);
        }
        ...
        let managedNetworkEnrollment: Awaited<ReturnType<typeof enrollManagedNetwork>> | null = null;
        ...
        if (result.kind === "created" && networkConfig) {
            try {
                ...
                try {
                    await remoteHostsRepository.revoke(result.host.id, user.organization_id, user.id);
                }
                catch (revokeCause) {
                    throw new AggregateError([cause, revokeCause], "Managed-network enrollment failed; host revocation is pending retry.", { cause });
                }
                throw cause;
            }
        }
