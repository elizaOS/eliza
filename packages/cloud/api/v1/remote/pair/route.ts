...
app.post("/", async (c) => {
    try {
        ...
        const claimSessionId = typeof body.sessionId === "string" ? body.sessionId.trim() : "";
        const claimCode = typeof body.code === "string" ? body.code.trim() : "";
        if (claimCode || claimSessionId) {
            if (Boolean(claimSessionId) === Boolean(hostId) ||
                agentId ||
                ...
                return c.json({ success: false, error: "Pairing claim is invalid" }, 400);
            }
            ...
            if (!isRemoteControllerPublicIdentity(controllerIdentity)) {
                return c.json({ success: false, error: "Controller identity is invalid" }, 400);
            }
            const claim = await remoteSessionsRepository.claimPendingHostForOwner({
                ...
                controllerEncryptionPublicJwk: controllerIdentity.encryptionPublicKeyJwk,
            });
        ...
        if (hostId) {
            if (typeof body.controller !== "object" ||
                body.controller === null ||
                Array.isArray(body.controller)) {
                return c.json({ success: false, error: "controller identity is required" }, 400);
            }
            const controller = body.controller as Record<string, unknown>;
            ...
            if (!Number.isSafeInteger(grantTtlSeconds) ||
            ...
            const grantExpiresAt = new Date(Date.now() + (grantTtlSeconds as number) * 1000);
            const tokenHash = await deriveRemotePairingCodeVerifier(pairingSecret, {
                organizationId: user.organization_id,
                userId: user.id,
                hostId,
                sessionId,
            ...
            const session = await remoteSessionsRepository.createPendingForOwnedHost({
                ...
                controller_encryption_public_jwk: controllerIdentity.encryptionPublicKeyJwk,
