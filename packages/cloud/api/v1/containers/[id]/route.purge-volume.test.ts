...
type HetznerContainersClient = ReturnType<
  typeof import("@/lib/services/containers/hetzner-client/client").getHetznerContainersClient
>;

const deleteContainer = mock(
  async (..._args: Parameters<HetznerContainersClient["deleteContainer"]>) =>
    undefined,
);
const stopContainer = mock(
  async (..._args: Parameters<HetznerContainersClient["stopContainer"]>) => ({
    id: CONTAINER_ID,
    status: "stopped",
  }),
);
