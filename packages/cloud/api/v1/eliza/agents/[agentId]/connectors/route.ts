...
const bindSchema = z.object({
  ...
  oauthMode: z.enum(["eliza_managed", "bring_your_own"]),
  executionTarget: z.enum(["cloud_broker", "agent_host"]),
  selectedProducts: stringList,
  allowedCapabilities: stringList,
...
app.post("/", async (c) => {
  try {
    ...
    const binding = await agentConnectorBindingsService.bind({
      ...
      oauthMode: parsed.data.oauthMode,
