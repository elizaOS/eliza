...
describe("sandbox-backed shared-agent deletion", () => {
  ...
  test("a whitespace-only body retains the legacy provisioning conflict", async () => {
    const response = await deleteRequest(PROVISIONING_AGENT, " \n\t ");
    expect(response.status).toBe(409);
    expect(errorSchema.parse(await response.json()).error).toBe(
      "Agent provisioning is in progress",
    );
  ...
  test("an exact conditional identity lets the delete-wins queue retire provisioning", async () => {
    const { elizaSandboxService } = await import(
      "@/lib/services/eliza-sandbox"
    );
    const agent = await elizaSandboxService.getAgent(PROVISIONING_AGENT, ORG_A);
    expect(agent).toBeDefined();

    const response = await deleteRequest(PROVISIONING_AGENT, {
      expectedAgentName: agent?.agent_name,
      expectedCreatedAt: agent?.created_at.toISOString(),
      expectedExecutionTier: agent?.execution_tier,
    });
    expect(response.status).toBe(202);
    const body = acceptedDeleteSchema.parse(await response.json());
    expect(body.data.agentId).toBe(PROVISIONING_AGENT);
    expect(await deleteJobsFor(PROVISIONING_AGENT)).toHaveLength(1);
    expect(await sandboxStatus(PROVISIONING_AGENT)).toBe("deletion_pending");
  });

  test("an invalid conditional identity is rejected before queueing", async () => {
    const response = await deleteRequest(DETAIL_AGENT, {
      expectedAgentName: "Detail shared agent",
      expectedCreatedAt: "not-a-timestamp",
      expectedExecutionTier: "shared",
    });
    expect(response.status).toBe(400);
    expect(await deleteJobsFor(DETAIL_AGENT)).toHaveLength(0);
  });

  test("exposes cancellation success and conflict at the authenticated route", async () => {
    const { elizaSandboxService } = await import(
      "@/lib/services/eliza-sandbox"
    );
    ...
    try {
      const success = await patchRequest(DETAIL_AGENT, {
        action: "cancel_deletion",
      });
      expect(success.status).toBe(200);
      await expect(success.json()).resolves.toMatchObject({
        success: true,
        data: { agentId: DETAIL_AGENT, status: "running" },
      });
      ...
      const conflict = await patchRequest(DETAIL_AGENT, {
        action: "cancel_deletion",
      });
      expect(conflict.status).toBe(409);
      expect(errorSchema.parse(await conflict.json()).error).toBe(
        "Agent deletion is already executing",
      );
    } finally {
