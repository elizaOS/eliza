...
import { mintStewardTokenFromClaims } from "@/lib/auth/steward-client";
...
function stewardRefreshRequestContext(c: {
...
}, isProduction: boolean): StewardRefreshRequestContext {
    ...
    const localClientIp = !isProduction && isLoopback
...
async function callStewardRefresh(baseUrl: string, refreshToken: string, pinnedTenantId?: string, signingSecret?: string, requestContext: StewardRefreshRequestContext = {}): Promise<{
    kind: "ok";
    ...
    if (requestContext.origin)
        headers.set("Origin", requestContext.origin);
    ...
    if (typeof signingSecret === "string" && signingSecret.length > 0) {
        await signStewardMutatingRequest(signingSecret, "POST", `${refreshUrl.pathname}${refreshUrl.search}`, headers, bodyBytes);
    }
...
app.post("/", async (c) => {
    ...
    if (bearerToken) {
        ...
        const refreshed = await mintStewardTokenFromClaims(c.env, claims, BEARER_REFRESH_TTL_SECONDS);
        if (!refreshed) {
    ...
    const refresh = await callStewardRefresh(stewardBaseUrl, refreshToken, c.env.STEWARD_TENANT_ID, c.env.STEWARD_REQUEST_SIGNING_SECRET, stewardRefreshRequestContext(c, isProduction));
    ...
    if (refresh.kind === "error") {
        ...
        if (refresh.status === 429) {
            return c.json(errorBody("Too many refresh attempts. Wait a moment and try again.", "internal_error"), 429, refresh.retryAfter ? { "Retry-After": refresh.retryAfter } : undefined);
        }
        return c.json(errorBody(refresh.data.error || "Refresh failed", "internal_error"), 502);
    }
