...
import { RateLimitPresets } from "@/lib/middleware/rate-limit-hono-cloudflare";
import { STEWARD_ACCESS_TOKEN_TTL_SECONDS } from "@/lib/auth/steward-client";
import { consumeSsoBridgeCode } from "@/lib/services/sso-bridge-codes";
import { getIpKey } from "@/lib/middleware/rate-limit-hono-cloudflare";
import { isBlockedBySsoBridgeLogout } from "@/lib/services/sso-bridge-codes";
import { issueSsoBridgeCode } from "@/lib/services/sso-bridge-codes";
import { logger } from "@/lib/utils/logger";
import { looksLikeSsoBridgeChallenge } from "@/lib/services/sso-bridge-codes";
import { looksLikeSsoBridgeCode } from "@/lib/services/sso-bridge-codes";
 ...
 */
function checkBridgeOrigin(c: {
    req: {
        header: (name: string) => string | undefined;
    };
}, allowedHosts: ReadonlySet<string>, isProduction: boolean): boolean {
    const origin = originHost(c.req.header("origin"));
    if (!origin)
        return false;
    if (allowedHosts.has(origin))
        return true;
    if (!isProduction && LOCAL_DEV_ORIGIN_HOSTS.has(origin))
        return true;
    return false;
}
...
app.post("/mint", async (c) => {
    try {
        ...
        const codeChallenge = typeof body.codeChallenge === "string" ? body.codeChallenge : null;
        if (!looksLikeSsoBridgeChallenge(codeChallenge)) {
            ...
            return c.json(errorBody("Code challenge required", "missing_challenge"), 400);
        }
...
app.post("/exchange", async (c) => {
    try {
        ...
        if (await isBlockedBySsoBridgeLogout(record.stewardUserId, record.tokenIssuedAt)) {
            return c.json(errorBody("Session was signed out", "session_ended"), 401);
        }
        ...
        const remainingSeconds = Math.min(STEWARD_ACCESS_TOKEN_TTL_SECONDS, record.tokenExpiresAt - Math.floor(Date.now() / 1000));
        if (remainingSeconds <= 0) {
            return c.json(errorBody("Session no longer valid", "invalid_token"), 401);
        }
        const minted = await mintStewardTokenFromClaims(c.env, { ...record.claims, bridged: true }, remainingSeconds);
        if (!minted) {
            return c.json(errorBody("Steward verification not configured on server", "server_secret_missing"), 503);
        }
