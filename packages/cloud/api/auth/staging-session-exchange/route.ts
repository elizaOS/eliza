...
import { STAGING_SESSION_MAX_TTL_SECONDS } from "@/lib/auth/staging-session-binding";
import { StagingSessionConfigurationError } from "@/lib/auth/staging-session-binding";
import { StagingSessionEligibilityError } from "@/lib/auth/staging-session-binding";
import { consumeStagingSessionCode } from "@/lib/services/staging-session-exchange-codes";
import { getIpKey } from "@/lib/middleware/rate-limit-hono-cloudflare";
import { isStagingSessionExchangeEnabled } from "@/lib/auth/staging-session-binding";
import { isStagingSessionSigningConfigured } from "@/lib/auth/staging-session-binding";
import { issueStagingSessionCode } from "@/lib/services/staging-session-exchange-codes";
import { loadExistingStagingSessionSubjectForMint } from "@/lib/auth/staging-session-binding";
import { logger } from "@/lib/utils/logger";
import { looksLikeStagingSessionChallenge } from "@/lib/services/staging-session-exchange-codes";
import { looksLikeStagingSessionCode } from "@/lib/services/staging-session-exchange-codes";
...
import { type StewardTokenClaims } from "@/lib/auth/steward-client";
import { type StewardVerifyEnv } from "@/lib/auth/steward-client";
import { validateStagingSessionBinding } from "@/lib/auth/staging-session-binding";
...
function exactStagingRequestSurface(c: {
    ...
    if (!isStagingSessionExchangeEnabled(c.env))
        return false;
    const url = new URL(c.req.url);
    return (url.origin === STAGING_API_ORIGIN &&
...
function readSingleApiKeyCredential(c: {
    ...
    const bearerKey = authorization.startsWith("Bearer ")
        ...
        : "";
    if (bearerKey && !bearerKey.startsWith("eliza_"))
        return null;
...
app.post("/mint", async (c) => {
    ...
    if (!exactStagingAppOrigin(c)) {
        return c.json(errorBody("Forbidden", "forbidden_origin"), 403, noStoreHeaders());
    }
    if (!stewardSignerConfigured(c.env)) {
        return c.json(errorBody("Staging session signer unavailable", "server_secret_missing"), 503, noStoreHeaders());
    }
    const presentedApiKey = readSingleApiKeyCredential(c);
    if (!presentedApiKey) {
        return c.json(errorBody("API key required", "api_key_required"), 401, noStoreHeaders());
    }
    const body = (await c.req.json().catch(() => ({}))) as {
        codeChallenge?: unknown;
    };
    const codeChallenge = typeof body.codeChallenge === "string" ? body.codeChallenge : undefined;
    if (!looksLikeStagingSessionChallenge(codeChallenge)) {
        return c.json(errorBody("Code challenge required", "missing_challenge"), 400, noStoreHeaders());
    }
    try {
        ...
        return c.json({ ok: true, code: issued.code, expiresIn: issued.expiresIn }, 200, noStoreHeaders());
    }
    catch (error) {
        if (error instanceof StagingSessionEligibilityError) {
            return c.json(errorBody("Staging session subject is not eligible", "subject_not_eligible"), 403, noStoreHeaders());
        }
        if (error instanceof StagingSessionConfigurationError) {
            logger.error("[staging-session-exchange] configuration unavailable");
            return c.json(errorBody("Staging session exchange unavailable", "exchange_unavailable"), 503, noStoreHeaders());
        }
        if (error instanceof ApiError &&
            (error.status === 401 || error.status === 403)) {
            return c.json(error.status === 401
                ? errorBody("API key required or invalid", "invalid_api_key")
                : errorBody("Staging session subject is not eligible", "subject_not_eligible"), error.status, noStoreHeaders());
        }
...
app.post("/exchange", async (c) => {
    ...
    if (!looksLikeStagingSessionCode(code)) {
        return c.json(errorBody("Code required", "missing_code"), 400, noStoreHeaders());
    }
    ...
    try {
        ...
        if (!record?.claims.stagingSessionBinding) {
            return c.json(errorBody("Invalid or expired code", "invalid_code"), 401, noStoreHeaders());
        }
        ...
        const remainingSeconds = Math.min(STAGING_SESSION_MAX_TTL_SECONDS, record.tokenExpiresAt - Math.floor(Date.now() / 1000));
