...
import { browserOriginHost } from "@/lib/auth/browser-origin-policy";
import { checkElizaMutatingRequestOrigin } from "@/lib/auth/browser-origin-policy";
import { cookieDomainForHost } from "@/lib/auth/cookie-domain";
import { describeSyncError } from "@/lib/steward-sync";
import { hasElizaNonSimpleRequestMarker } from "@/lib/auth/browser-origin-policy";
import { isPermittedElizaBrowserOrigin } from "@/lib/auth/browser-origin-policy";
...
import { type StewardVerifyEnv } from "@/lib/auth/steward-client";
...
function shouldReturnClientToken(c: {
    req: {
        header: (name: string) => string | undefined;
    };
}, isProduction: boolean): boolean {
    const origin = browserOriginHost(c.req.header("origin")) ??
...
function errorBody(message: string, code: StewardSessionErrorCode): {
    error: string;
 ...
 */
async function callStewardExchange(baseUrl: string, body: {
    code: string;
    redirect_uri: string;
    tenant_id: string | null;
    code_verifier?: string;
}, pinnedTenantId?: string, signingSecret?: string | null): Promise<{
    kind: "ok";
    ...
    if (typeof signingSecret === "string" && signingSecret.length > 0) {
        await signStewardMutatingRequest(signingSecret, "POST", `${exchangeUrl.pathname}${exchangeUrl.search}`, headers, bodyBytes);
    }
...
app.post("/", async (c) => {
    ...
    const redirectUri = typeof body.redirectUri === "string"
        ...
        : typeof body.redirect_uri === "string"
            ...
            : "";
    const rawTenant = typeof body.tenantId === "string"
        ? body.tenantId.trim()
    ...
    const tenantId = rawTenant.length > 0 ? rawTenant : envTenant.length > 0 ? envTenant : null;
    ...
    if (!codeVerifier) {
        ...
        return c.json(errorBody("codeVerifier required", "missing_code_verifier"), 400);
    }
    if (body.telegramContinuation !== undefined) {
        logExchange("telegram-claim-not-permitted");
        return c.json(errorBody("Account confirmation required", "telegram_claim_conflict"), 409);
    }
    if (!stewardSecretConfigured(c.env)) {
        logExchange("server-secret-missing");
        return c.json(errorBody("Steward verification not configured on server", "server_secret_missing"), 503);
    }
    const stewardBaseUrl = resolveStewardBaseUrl(c.env);
    if (!stewardBaseUrl) {
        logExchange("upstream-not-configured");
        return c.json(errorBody("Steward upstream not configured", "steward_upstream_unavailable"), 503);
    }
    ...
    }, c.env.STEWARD_TENANT_ID, c.env.STEWARD_REQUEST_SIGNING_SECRET);
    if (exchange.kind === "transport") {
        ...
        return c.json(errorBody("Steward upstream unavailable", "steward_upstream_unavailable"), 502);
    }
    if (exchange.kind === "error") {
        ...
        const mapped: StewardSessionErrorCode = upstreamCode === "code_expired" ||
        ...
        return c.json(errorBody(exchange.data.error || "Code exchange failed", mapped), status);
    }
    ...
    try {
        ...
        logger.error(`[steward-nonce-exchange] Failed to sync Steward user before setting cookie (stewardUserId=${claims.userId}): ${describeSyncError(error)}`);
        return c.json(errorBody("Could not sync Steward user", "steward_user_sync_failed"), 500);
    }
