...
import { type CloudPairExchangeResponse } from "@elizaos/core/contracts/cloud-pair";
...
function nativePairingError(error: string, code: (typeof NATIVE_PAIRING_ERROR_CODE)[keyof typeof NATIVE_PAIRING_ERROR_CODE]) {
...
app.post("/", async (c) => {
    try {
        ...
        const agentId = isRecord(body) && typeof body.agentId === "string"
            ? body.agentId.trim()
            : "";
        ...
        if (!isLoopbackHttpOrigin(origin)) {
            return c.json({
                error: "Managed browser pairing must be completed at the agent's Eliza Cloud address",
            }, 403);
        }
...
app.post("/native", async (c) => {
    try {
        ...
        if (!authorization?.startsWith("Bearer ") ||
            ...
            return c.json(nativePairingError("Cloud authentication required", NATIVE_PAIRING_ERROR_CODE.cloudAuthRequired), 401);
        }
        ...
        const body: unknown = await c.req.json().catch(() => null);
        if (!isRecord(body)) {
            return c.json(nativePairingError("Invalid native pairing request", NATIVE_PAIRING_ERROR_CODE.invalidRequest), 400);
        }
        const token = typeof body.token === "string" ? body.token.trim() : "";
        const agentId = typeof body.agentId === "string" ? body.agentId.trim() : "";
        const expectedOrigin = typeof body.expectedOrigin === "string"
        ...
        if (claim.status === "sandbox-credential-unavailable") {
            ...
            return c.json(nativePairingError("Pairing failed", NATIVE_PAIRING_ERROR_CODE.sandboxCredentialUnavailable), 503);
        }
        if (claim.status === "invalid") {
            return c.json(nativePairingError("Invalid or expired pairing code", NATIVE_PAIRING_ERROR_CODE.pairingTokenInvalid), 410);
        }
        const response: CloudPairExchangeResponse = {
            message: "Paired successfully",
            apiKey: claim.apiKey,
