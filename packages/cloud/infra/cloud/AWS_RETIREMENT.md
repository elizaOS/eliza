...
| `s3-storage` plugin registry entry (`AWS_*` config fields) | `packages/app/src/registry/entries/plugins/s3-storage.json` | Generic user-facing S3 plugin. The `AWS_REGION` / `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_S3_ENDPOINT` schema fields are how the broader S3 ecosystem labels these creds, regardless of provider. Keep. |
