...
| Deployment capacity | repeated production deploys are pending/cancelled; no completed positive deploy proof | Operational blocker; retain exact-SHA run until completion |
...
| Full validation | unrelated merge conflict in `eliza-sse-bridge.ts` blocks Cloud-wide typecheck | Worktree blocker; focused lanes used, conflict left untouched |
...
The public health beacons reported production at
...
- environment metadata showed `DATABASE_URL` and production SSH target
  credentials, but no `HEADSCALE_API_KEY`, `HEADSCALE_PUBLIC_URL`,
  `WARM_POOL_ENABLED`, or `ELIZA_AGENT_IMAGE`; and
- staging metadata did not expose the provisioning host/key pair needed by the
  deploy job.

The newest completed production workflow marked successful contained only the
`Stop agent containers (production)` job; it was not positive evidence of a
worker deploy. A production dispatch from a non-main test ref correctly failed
the protected-source guard. These results show deployment activity in flight,
but do not prove the worker process, DB identity, warm pool, node fleet, or one
Dedicated chat is healthy.
