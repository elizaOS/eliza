...
import { aeadDecrypt, aeadEncrypt } from "./crypto/aead.ts";
import { hkdfSha256 } from "./crypto/hkdf.ts";
import { parseKeyId, withVersion } from "./key-namespace.ts";
import {
  type EncryptResult,
 ...
 *
 * Wraps `@elizaos/auth/vault`'s master-key resolution (OS keychain → scrypt passphrase)
...
export interface LocalKmsOptions {
  /** 32-byte root key. Caller resolves via `@elizaos/auth/vault` master-key API. */
  rootKey: Uint8Array;
}
