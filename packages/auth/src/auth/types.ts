 ...
 */
import { codingProviderEnrollmentAvailability } from "@elizaos/core/contracts/coding-agent-capabilities";
import { codingProviderSubscriptionAuthMode } from "@elizaos/core/contracts/coding-agent-capabilities";
import { codingProviderSubscriptionBillingMode } from "@elizaos/core/contracts/coding-agent-capabilities";
...
export type SubscriptionProvider = "anthropic-subscription" | "openai-codex" | "gemini-cli" | "zai-coding" | "kimi-coding" | "deepseek-coding";
export type OAuthSubscriptionProvider = "anthropic-subscription" | "openai-codex";
...
export type DirectAccountProvider = "anthropic-api" | "openai-api" | "deepseek-api" | "zai-api" | "moonshot-api" | "cerebras-api" | "openrouter-api" | "xai-api";
export type AccountCredentialProvider = SubscriptionProvider | DirectAccountProvider;
...
export function isDirectAccountProvider(value: unknown): value is DirectAccountProvider {
    return (typeof value === "string" &&
        (DIRECT_ACCOUNT_PROVIDER_IDS as readonly string[]).includes(value));
}
export function isOAuthSubscriptionProvider(value: unknown): value is OAuthSubscriptionProvider {
    return (typeof value === "string" &&
        (OAUTH_SUBSCRIPTION_PROVIDER_IDS as readonly string[]).includes(value));
}
export function isCodingPlanKeySubscriptionProvider(value: unknown): value is CodingPlanKeySubscriptionProvider {
    return (typeof value === "string" &&
        (CODING_PLAN_KEY_SUBSCRIPTION_PROVIDER_IDS as readonly string[]).includes(value));
}
export function isExternalCliSubscriptionProvider(value: unknown): value is ExternalCliSubscriptionProvider {
    return (typeof value === "string" &&
        (EXTERNAL_CLI_SUBSCRIPTION_PROVIDER_IDS as readonly string[]).includes(value));
}
export function isUnavailableSubscriptionProvider(value: unknown): value is UnavailableSubscriptionProvider {
    return (typeof value === "string" &&
        (UNAVAILABLE_SUBSCRIPTION_PROVIDER_IDS as readonly string[]).includes(value));
}
export function isAccountCredentialProvider(value: unknown): value is AccountCredentialProvider {
    return (typeof value === "string" &&
        (ACCOUNT_CREDENTIAL_PROVIDER_IDS as readonly string[]).includes(value));
}
export const DIRECT_ACCOUNT_PROVIDER_ENV: Record<DirectAccountProvider, string> = {
...
export const OPENAI_COMPAT_BASE_BY_DIRECT_PROVIDER: Readonly<Partial<Record<DirectAccountProvider, string>>> = {
...
export const CODING_PLAN_PROVIDER_BASE_URL: Record<CodingPlanKeySubscriptionProvider, string> = {
    "zai-coding": "https://api.z.ai/api/coding/paas/v4",
    "kimi-coding": "https://api.kimi.com/coding/v1",
};
export type SubscriptionProviderAuthMode = "oauth" | "external-cli" | "coding-plan-key" | "unavailable";
export type SubscriptionProviderBillingMode = "subscription-coding-plan" | "subscription-coding-cli";
export type SubscriptionProviderAvailability = "available" | "external" | "unavailable";
...
export const SUBSCRIPTION_PROVIDER_METADATA: Record<SubscriptionProvider, SubscriptionProviderMetadata> = {
    "anthropic-subscription": {
        providerId: "anthropic-subscription",
        displayName: "Claude Subscription",
        selectionIds: ["anthropic-subscription"],
        allowedClient: "Claude Code CLI",
        billingMode: codingProviderSubscriptionBillingMode("anthropic-subscription"),
        authMode: codingProviderSubscriptionAuthMode("anthropic-subscription"),
        availability: codingProviderEnrollmentAvailability("anthropic-subscription"),
    ...
    "gemini-cli": {
        ...
        setupHint: "Run gemini auth login; tokens are not imported into API env vars.",
    },
    "zai-coding": {
        ...
        setupHint: "Add a z.ai Coding Plan credential for the dedicated coding endpoint.",
    ...
    "kimi-coding": {
        ...
        setupHint: "Add a Kimi coding-endpoint key for model inference. Kimi ACP uses a separate kimi login OAuth session.",
    ...
    "deepseek-coding": {
        ...
        setupHint: "Use the DeepSeek direct API-key provider if you have API billing.",
        directProviderId: "deepseek-api",
        availabilityReason: "No first-party DeepSeek coding subscription surface is available to integrate without substituting general API billing.",
    },
} as const;
export function getSubscriptionProviderMetadata(provider: SubscriptionProvider): SubscriptionProviderMetadata {
