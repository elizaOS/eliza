...
import { SUBSCRIPTION_PROVIDER_IDS } from "./types.ts";
import { SUBSCRIPTION_PROVIDER_MAP } from "./types.ts";
import { SUBSCRIPTION_PROVIDER_METADATA } from "./types.ts";
import { accountRefreshMutex } from "./refresh-mutex.ts";
import { carryForwardIdToken } from "./account-storage.ts";
import { commitAccountDeletions } from "./account-storage.ts";
import { deleteAccount } from "./account-storage.ts";
...
import { isCodingPlanKeySubscriptionProvider } from "./types.ts";
import { isExternalCliSubscriptionProvider } from "./types.ts";
import { isOAuthSubscriptionProvider } from "./types.ts";
import { isSubscriptionProvider } from "./types.ts";
import { isUnavailableSubscriptionProvider } from "./types.ts";
import { listAccounts } from "./account-storage.ts";
import { loadAccount } from "./account-storage.ts";
import { logger } from "@elizaos/core";
import { preflightProviderAccountDeletions } from "./account-storage.ts";
...
import { saveAccount } from "./account-storage.ts";
import { type AccountCredentialProvider } from "./types.ts";
import { type AccountCredentialRecord } from "./account-storage.ts";
import { type AccountDeletionPlan } from "./account-storage.ts";
import { type AccountStoragePolicy } from "./account-storage.ts";
import { type LoadedAccountCredentialRecord } from "./account-storage.ts";
import { type OAuthCredentials } from "./types.ts";
import { type StoredCredentials } from "./types.ts";
...
import { updateAccountCredentialsIfUnchanged } from "./account-storage.ts";
const DEFAULT_ACCOUNT_ID = "default";
/** Buffer before expiry to trigger refresh (5 minutes) */
const REFRESH_BUFFER_MS = 5 * 60 * 1000;
/** Stable failure categories for callers that manage account-pool health. */
export type AccessTokenFailureKind = "auth" | "transient" | "insufficient-lifetime";
...
function tokenFailure(kind: AccessTokenFailureKind, message: string, extra: {
    expiresAt?: number;
    minRemainingMs?: number;
} = {}): Extract<AccessTokenOutcome, {
    ok: false;
}> {
    return {
        ok: false,
...
function classifyRefreshError(err: unknown): AccessTokenFailureKind {
    ...
    if (/\b(?:401|403|invalid[_ ]?grant|invalid[_ ]?token|unauthor|forbidden|re-?auth|revoked|(?:access |refresh |oauth |jwt |session |credential )?token (?:has |is )?expired|expired[_ ]?(?:access[_ ]?|refresh[_ ]?|oauth[_ ]?|jwt[_ ]?)?token|(?:credential|jwt|session) (?:has |is )?expired)\b/i.test(message)) {
        return "auth";
    }
    if (/\b(?:5\d\d|timeout|timed? ?out|fetch failed|network|ECONNRESET|ETIMEDOUT|EAI_AGAIN|socket hang up|service unavailable|bad gateway)\b/i.test(message)) {
        return "transient";
    }
    return "transient";
}
 ...
 */
export function saveCredentials(provider: SubscriptionProvider, credentials: OAuthCredentials, accountId: string, storagePolicy: AccountStoragePolicy): LoadedAccountCredentialRecord {
 ...
 */
export function loadCredentials(provider: SubscriptionProvider, accountId: string = DEFAULT_ACCOUNT_ID, storagePolicy?: AccountStoragePolicy): StoredCredentials | null {
 ...
 */
export function deleteCredentials(provider: SubscriptionProvider, accountId: string, storagePolicy: AccountStoragePolicy): void {
    deleteAccount(provider, accountId, storagePolicy);
}
export function preflightProviderCredentialDeletion(providers: readonly AccountCredentialProvider[], storagePolicy: AccountStoragePolicy): AccountDeletionPlan {
    return preflightProviderAccountDeletions(providers, storagePolicy);
}
/**
 * Delete every stored credential account for a provider.
 */
export function deleteProviderCredentials(provider: AccountCredentialProvider, storagePolicy: AccountStoragePolicy): number {
 ...
 */
export function hasValidCredentials(provider: AccountCredentialProvider, accountId: string = DEFAULT_ACCOUNT_ID, storagePolicy?: AccountStoragePolicy): boolean {
    const record = loadAccount(provider, accountId, storagePolicy);
    if (!record)
        return false;
 ...
 */
export function listProviderAccounts(provider: AccountCredentialProvider, storagePolicy?: AccountStoragePolicy): AccountCredentialRecord[] {
 ...
 */
export function getAccessToken(provider: AccountCredentialProvider, accountId: string, opts: GetAccessTokenOutcomeOptions): Promise<AccessTokenOutcome>;
export function getAccessToken(provider: AccountCredentialProvider, accountId?: string, opts?: GetAccessTokenOptions): Promise<string | null>;
export async function getAccessToken(provider: AccountCredentialProvider, accountId: string = DEFAULT_ACCOUNT_ID, opts?: GetAccessTokenOptions | GetAccessTokenOutcomeOptions): Promise<string | null | AccessTokenOutcome> {
    const returnOutcome = (opts as GetAccessTokenOutcomeOptions | undefined)?.outcome === true;
    const finish = (outcome: AccessTokenOutcome): string | null | AccessTokenOutcome => returnOutcome ? outcome : outcome.ok ? outcome.accessToken : null;
    ...
    const effectiveBufferMs = typeof raw === "number" && Number.isFinite(raw) && raw > REFRESH_BUFFER_MS
        ? raw
        : REFRESH_BUFFER_MS;
    ...
    if (!isSubscriptionProvider(provider)) {
        ...
        if (direct.credentials.expires <= Date.now()) {
            return finish(tokenFailure("auth", "Stored credential is expired", {
    ...
    if (isCodingPlanKeySubscriptionProvider(provider)) {
        ...
        return finish(initial.credentials.expires > Date.now()
            ? tokenFailure("insufficient-lifetime", "Credential cannot be refreshed to satisfy the requested lifetime", {
                expiresAt: initial.credentials.expires,
    ...
    if (isExternalCliSubscriptionProvider(provider) ||
        isUnavailableSubscriptionProvider(provider)) {
        logger.info(`[auth] ${provider} is not an importable OAuth credential; use its first-party coding client or supported coding endpoint.`);
        return finish(tokenFailure("auth", `${provider} cannot provide importable OAuth tokens`));
    }
    return accountRefreshMutex.acquire(`${provider}:${accountId}`, async () => {
        ...
        if (!opts?.storagePolicy) {
            throw new ElizaError("Refreshing a stored credential requires an explicit account storage policy", {
        ...
        try {
            if (provider === "anthropic-subscription") {
                ...
                return finish(tokenFailure("auth", `Unknown credential provider: ${provider}`));
            }
            else {
                logger.error(`[auth] Refresh unsupported for provider: ${provider}`);
                return finish(tokenFailure("auth", `Refresh unsupported for provider: ${provider}`));
            }
        }
        catch (err) {
            ...
            return finish(tokenFailure(classifyRefreshError(err), err instanceof Error ? err.message : String(err)));
        }
        ...
        const commit = updateAccountCredentialsIfUnchanged(provider, accountId, stored.credentialGeneration, refreshed, opts.storagePolicy);
        if (commit.kind === "missing") {
            logger.warn(`[auth] Discarded ${provider} refresh for "${accountId}": the account was removed while the refresh was in flight`);
            return finish(tokenFailure("auth", "Credential was removed while the refresh was in flight"));
        }
        if (commit.kind === "changed") {
            logger.warn(`[auth] Discarded ${provider} refresh for "${accountId}": the stored credential was replaced while the refresh was in flight`);
            ...
            return finish(current.expires > Date.now()
                ? tokenFailure("insufficient-lifetime", "Replacement credential does not satisfy the requested lifetime", { expiresAt: current.expires, minRemainingMs: effectiveBufferMs })
                : tokenFailure("auth", "Replacement credential is expired", {
                    expiresAt: current.expires,
                }));
        }
        if (refreshed.expires <= Date.now() + effectiveBufferMs) {
            return finish(tokenFailure("insufficient-lifetime", "Refreshed token does not satisfy the requested lifetime", { expiresAt: refreshed.expires, minRemainingMs: effectiveBufferMs }));
        }
        return finish({
            ok: true,
...
function readConfiguredAnthropicSetupToken(): string | null {
    ...
    try {
        ...
        logger.debug(`[auth] Anthropic setup token config unavailable: ${String(error)}`);
        return null;
    }
}
...
function subscriptionStatusMetadata(provider: SubscriptionProvider): Pick<SubscriptionAccountStatus, "available" | "allowedClient" | "loginHint" | "billingMode"> & {
    availabilityReason?: string;
} {
 ...
 */
function hasConfiguredExternalCredential(provider: SubscriptionProvider): boolean {
    const discovered = getSubscriptionAuthProvider(provider)?.detectExternalCredentials?.();
    if (discovered == null)
        return false;
    ...
    for (const provider of SUBSCRIPTION_PROVIDER_IDS) {
        ...
        for (const account of accounts) {
            rows.push({
                ...
                source: isCodingPlanKeySubscriptionProvider(provider) &&
        ...
        if (provider === "anthropic-subscription") {
            ...
            if (importedClaudeAuth) {
                ...
                const blobValid = claudeBlob
                    ...
                    : true;
                const accountId = claudeSource === "claude-code-cli"
                    ? "claude-code-cli"
                    : "setup-token";
                const label = claudeSource === "claude-code-cli"
                    ? "Claude Code CLI"
                    : "Setup Token";
                rows.push({
                    ...metadata,
...
function readClaudeCodeOAuthBlob(): ClaudeCodeCredentialBlob | null {
    const parse = (raw: string, source: string): ClaudeCodeCredentialBlob | null => {
        try {
            ...
            if (!isRecord(oauth))
                return null;
            const accessTokenFields = [oauth.accessToken, oauth.access_token].filter((value) => value !== undefined);
            if (accessTokenFields.length === 0 ||
                accessTokenFields.some((value) => typeof value !== "string" || !value.trim())) {
                return null;
            }
            ...
            if (refreshTokenFields.some((value) => value !== null && typeof value !== "string")) {
                return null;
            }
            const refreshTokenValue = oauth.refreshToken ?? oauth.refresh_token ?? null;
            const refreshToken = typeof refreshTokenValue === "string" ? refreshTokenValue : null;
            const expiresAtFields = [oauth.expiresAt, oauth.expires_at].filter((value) => value !== undefined);
            if (expiresAtFields.some((value) => value !== null &&
    ...
    try {
        ...
        logger.debug(`[auth] Claude Code credential file unavailable: ${String(error)}`);
    }
    return null;
}
...
function isSubscriptionCredentialApplicationDisabled(): boolean {
    const disabled = process.env.ELIZA_DISABLE_SUBSCRIPTION_CREDENTIALS?.trim().toLowerCase();
    return (disabled === "1" ||
        disabled === "true" ||
        disabled === "yes" ||
        disabled === "on");
 ...
 */
export function applySubscriptionCredentialsLocal(config?: SubscriptionCredentialConfig): void {
    if (isSubscriptionCredentialApplicationDisabled()) {
        logger.info("[auth] Subscription credential application disabled by ELIZA_DISABLE_SUBSCRIPTION_CREDENTIALS");
        return;
    }
    ensureBuiltinSubscriptionAuthProviders();
    ...
    if (codexAccounts.length > 0) {
        const labels = codexAccounts
            .map((a) => `"${a.label}" (${a.id})`)
            .join(", ");
        logger.info(`[auth] OpenAI Codex subscription accounts configured: ${labels} — available for Codex CLI-backed coding/model providers. ` +
        ...
        if (hasConfiguredExternalCredential("openai-codex")) {
            logger.info("[auth] OpenAI Codex CLI auth detected — available for Codex CLI-backed coding/model providers. " +
                "Not applied to OPENAI_API_KEY; add a direct OpenAI API key for @elizaos/plugin-openai runtime inference.");
        }
    }
    ...
    if (geminiAccounts.length > 0 ||
        ...
        logger.info("[auth] Gemini CLI subscription surface detected/configured — available through the external Gemini CLI, but not wired to the coding-agent orchestrator. " +
    ...
    for (const provider of ["zai-coding", "kimi-coding"] as const) {
        ...
        const envName = provider === "zai-coding" ? "ZAI_API_KEY" : "MOONSHOT_API_KEY";
        logger.info(`[auth] ${provider} coding-plan accounts configured: ${labels} — available only for the provider's dedicated coding endpoint. ` +
    ...
    if (config?.agents?.defaults) {
        ...
        const provider = defaults.subscriptionProvider as keyof typeof SUBSCRIPTION_PROVIDER_MAP;
        if (provider) {
 ...
 */
export async function applySubscriptionCredentials(config?: SubscriptionCredentialConfig): Promise<void> {
...
export async function applySubscriptionCredentialsDeferred(): Promise<void> {
    if (isSubscriptionCredentialApplicationDisabled())
        return;
    if (listProviderAccounts("anthropic-subscription").length > 0)
        return;
    if (readClaudeCodeOAuthBlob()) {
        logger.info("[auth] Detected local Claude Code CLI credentials. The CLI manages refresh. " +
