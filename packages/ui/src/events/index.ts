 ...
 */
import { CONNECT_EVENT } from "@elizaos/core/events";
...
import { type ElizaDocumentEventName as SharedDocumentEventName } from "@elizaos/core/events";
import { type ElizaWindowEventName as SharedWindowEventName } from "@elizaos/core/events";
import { type NavigateViewDetail } from "@elizaos/core/events";
import { type NavigateViewEvent } from "@elizaos/core/events";
export { AGENT_READY_EVENT, APP_EMOTE_EVENT, APP_PAUSE_EVENT, APP_RESUME_EVENT, type AppEmoteEventDetail, BRIDGE_READY_EVENT, CHAT_AVATAR_VOICE_EVENT, type ChatAvatarVoiceEventDetail, COMMAND_PALETTE_EVENT, CONNECT_EVENT, createNavigateViewEvent, dispatchAppEmoteEvent, dispatchElizaCloudStatusUpdated, dispatchNavigateViewEvent, ELIZA_CLOUD_STATUS_UPDATED_EVENT, type ElizaCloudStatusUpdatedDetail, EMOTE_PICKER_EVENT, FIRST_RUN_VOICE_PREVIEW_AWAIT_TELEPORT_EVENT, MOBILE_RUNTIME_MODE_CHANGED_EVENT, NAVIGATE_VIEW_EVENT, type NavigateViewDetail, type NavigateViewEvent, type NavigateViewType, NETWORK_STATUS_CHANGE_EVENT, type NetworkStatusChangeDetail, PUSH_TO_TALK_HOLD_EVENT, PUSH_TO_TALK_TOGGLE_EVENT, type PushToTalkHoldDetail, SELF_STATUS_SYNC_EVENT, SHARE_TARGET_EVENT, STOP_EMOTE_EVENT, TRAY_ACTION_EVENT, VOICE_CONFIG_UPDATED_EVENT, VRM_TELEPORT_COMPLETE_EVENT } from "@elizaos/core/events";
 ...
 */
export type CloudHandoffPhase = "migrating" | "switched" | "switched-empty" | "timed-out" | "failed" | "insufficient-credits";
 ...
 * one listener.
 */
export const OPEN_NOTIFICATION_CENTER_EVENT = "eliza:notifications:open" as const;
...
export type ElizaDocumentEventName = SharedDocumentEventName | typeof FOCUS_CONNECTOR_EVENT;
export type ElizaWindowEventName = SharedWindowEventName | typeof VOICE_CONTROL_EVENT | typeof CHAT_PREFILL_EVENT | typeof CLOUD_HANDOFF_PHASE_EVENT | typeof CLOUD_HANDOFF_RETRY_EVENT | typeof ELIZA_BACK_INTENT_EVENT;
export type ElizaEventName = ElizaDocumentEventName | ElizaWindowEventName;
// ── Helpers ──────────────────────────────────────────────────────────────
/** Dispatch a typed custom event on `document`. */
export function dispatchAppEvent(name: ElizaDocumentEventName, detail?: unknown): void {
...
type ConnectRequestListener = (detail: ConnectRequestDetail) => ConnectRequestResult | void
...
function connectRequestState(request: ConnectRequestDetail): ConnectRequestState {
 ...
 */
export function dispatchConnectRequest(detail: ConnectRequestDetail): Promise<ConnectRequestResult> {
 ...
 */
export function listenForConnectRequests(listener: ConnectRequestListener): () => void {
    const handle = (event: Event): void => {
        ...
        try {
            ...
            void Promise.resolve(result).then((outcome) => {
                complete(outcome ?? {
                    status: "failed",
                    message: "The connection handler did not confirm completion. Try connecting again.",
                });
            }, failed);
        }
        catch (error) {
...
type NavigateViewRequestListener = (event: NavigateViewEvent) => boolean | undefined;
...
const navigateViewRequestClaims = new WeakMap<object, NavigateViewRequestClaim>();
const navigateViewRequestResolvers = new WeakMap<object, (applied: boolean) => void>();
...
function drainNavigateViewRequests(): void {
    if (drainingNavigateViewRequests || typeof window === "undefined")
        return;
    ...
    try {
        while (pendingNavigateViewRequests.length > 0) {
            ...
            if (pendingNavigateViewRequests[0] === request)
                break;
        }
    }
    finally {
...
function dropOldestPendingNavigateViewRequest(): void {
    ...
    logger.warn({ viewId: dropped.viewId, viewPath: dropped.viewPath }, `[navigate-view-request] dropped oldest pending request past the ${MAX_PENDING_NAVIGATE_VIEW_REQUESTS}-item bound`);
 ...
 */
export function dispatchNavigateViewRequest(detail: NavigateViewDetail): Promise<boolean> {
    if (typeof window === "undefined")
 ...
 */
export function listenForNavigateViewRequests(listener: NavigateViewRequestListener): () => void {
    if (typeof window === "undefined")
        return () => { };
    const handle = (event: Event): void => {
        const detail = (event as CustomEvent<unknown>).detail;
        ...
        try {
            ...
            logger.warn({ error }, "[navigate-view-request] listener threw while applying a navigation intent; leaving it unclaimed for retry");
            return;
        }
        if (applied)
            claim?.commit();
...
export function dispatchWindowEvent(name: ElizaWindowEventName, detail?: unknown): void {
    if (typeof window === "undefined")
        return;
 ...
 */
export function dispatchCloudHandoffPhase(detail: CloudHandoffPhaseDetail): void {
...
export function dispatchCloudHandoffRetry(detail: CloudHandoffRetryDetail): void {
