 ...
 */
export { formatByteSize, formatDateTime, formatDurationMs, formatShortDate, formatTime, formatUptime } from "@elizaos/ui/utils/format";
type RelativeTimeTranslator = (key: string, vars?: Record<string, string | number | boolean | null | undefined>) => string;
 ...
 */
export function formatRelativeTimeShort(value: string | number | Date, t?: RelativeTimeTranslator): string {
    const date = value instanceof Date ? value : new Date(value);
    const time = date.getTime();
    if (!Number.isFinite(time))
        return "now";
    const { future, absMs, mins, hours, days } = relativeTimeParts(Date.now() - time);
    if (absMs < 60000)
        return "now";
    if (absMs >= WEEK_MS)
 ...
 */
export function formatRelativeTime(value: string | number | Date, t?: RelativeTimeTranslator): string {
    const date = value instanceof Date ? value : new Date(value);
    const time = date.getTime();
    if (!Number.isFinite(time)) {
