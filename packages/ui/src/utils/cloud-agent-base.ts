...
import { buildElizaDedicatedAgentOrigin } from "@elizaos/plugin-elizacloud/cloud-config/domain-contract";
 ...
 */
export function directCloudSharedAgentIdFromBase(value: string | null | undefined): string | null {
    if (!value?.trim())
        return null;
...
export function isPersonalSharedElizaId(value: string): boolean {
    return /^personal:[0-9a-f]{8}-[0-9a-f]{4}-5[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value);
}
/**
 ...
 */
export function isManagedCloudSharedAgentBase(value: string | null | undefined): boolean {
    if (directCloudSharedAgentIdFromBase(value) === null || !value)
        return false;
    const url = normalizeHttpUrl(value.trim());
...
const PROD_CLOUD_ENVIRONMENT_BASE = ELIZA_DOMAIN_CONTRACTS.production.cloudAppOrigin;
const STAGING_CLOUD_ENVIRONMENT_BASE = ELIZA_DOMAIN_CONTRACTS.staging.cloudAppOrigin;
/**
 ...
 */
export function buildCloudSharedAgentApiBase(cloudApiBase: string, agentId: string): string {
 ...
 */
export function buildDedicatedCloudAgentApiBase(agentId: string | null | undefined, cloudApiBase?: string | null): string | null {
    const label = agentId?.trim().toLowerCase() ?? "";
    if (!/^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/.test(label)) {
        return null;
    }
    const cloudUrl = cloudApiBase ? normalizeHttpUrl(cloudApiBase.trim()) : null;
    const environment = cloudUrl && isStagingCloudHostname(cloudUrl.hostname)
        ? "staging"
        : "production";
 ...
 */
export function isCloudAgentsCollectionBase(value: string | null | undefined): boolean {
    if (!value?.trim())
        return true;
    const url = normalizeHttpUrl(value.trim());
    if (!url)
 ...
 * "Backend Unreachable".
 */
export function isElizaCloudControlPlaneAgentlessBase(value: string | null | undefined): boolean {
    if (!value)
        return false;
    const url = normalizeHttpUrl(value.trim());
    if (!url)
 ...
 */
export function isTrustedHostedCloudOnboardingBase(value: string | null | undefined, cloudOnlyBranding: boolean): boolean {
    if (isElizaCloudControlPlaneAgentlessBase(value))
        return true;
    if (!cloudOnlyBranding || !value?.trim())
        return false;
    const url = normalizeHttpUrl(value.trim());
 ...
 */
export function isDedicatedCloudAgentBase(value: string | null | undefined): boolean {
    if (!value?.trim())
        return false;
    const url = normalizeHttpUrl(value.trim());
    if (!url)
    ...
    if (isElizaDedicatedAgentHostname(host))
        return true;
    const path = stripTrailingSlash(url.pathname);
    const match = /^\/api\/v1\/eliza\/agents\/([0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})(?:\/api)?$/i.exec(path);
    if (!match)
        return false;
 ...
 */
export function dedicatedCloudAgentIdFromBase(value: string | null | undefined): string | null {
    ...
    if (classified)
        return classified;
    const match = /^\/api\/v1\/eliza\/agents\/([0-9a-f-]+)(?:\/api)?\/?$/i.exec(url.pathname);
...
export const PROD_ELIZA_APP_ORIGIN = ELIZA_DOMAIN_CONTRACTS.production.cloudAppOrigin;
...
export const STAGING_ELIZA_APP_ORIGIN = ELIZA_DOMAIN_CONTRACTS.staging.cloudAppOrigin;
/**
 ...
 */
export function resolveElizaAppOrigin(hostname: string | null | undefined): string {
