...
### Tier 2 - Setup progress (ongoing, self-hiding)
Only active setup work belongs here, such as `model-download.status` and
`agent-provisioning.status`. Pending-response prompts stay in conversation and
notification flows instead of becoming resident Home cards. Continuous activity
streams, app-run lists, and running workflow lists stay in the launcher/sidebar
or routed views.
