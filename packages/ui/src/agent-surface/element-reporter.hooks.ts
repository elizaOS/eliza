...
/** Post one registry snapshot using the shell transport's WebSocket identity. */
export async function reportAgentSurfaceElementSnapshot(
  registry: ViewAgentRegistry,
): Promise<void> {
  const { viewId, viewType, elements } = buildPayload(registry);
  if (elements.length === 0) return;
  const [{ client }, { fetchWithCsrf }, { resolveApiUrl }] = await Promise.all([
    import("../api/client"),
  ...
  await fetchWithCsrf(
    resolveApiUrl(`/api/views/${encodeURIComponent(viewId)}/elements`),
    {
      ...
      body: JSON.stringify({ viewType, elements }),
    },
  );
}
...
export function useAgentSurfaceElementReporter(
  ...
  useEffect(() => {
    ...
    const flush = () => {
      const { elements } = buildPayload(registry);
