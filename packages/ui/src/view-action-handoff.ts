...
export function findViewActionHandoff(actionResults: readonly ChatActionResultSummary[] | undefined): ViewActionHandoff | null {
    ...
    for (let index = actionResults.length - 1; index >= 0; index--) {
        ...
        const actionName = readString(readOwnValue(result, "actionName"))?.toUpperCase();
        const values = readOwnValue(result, "values");
        const mode = readString(readOwnValue(values, "mode"))?.toLowerCase();
        const subaction = readString(readOwnValue(values, "subaction"))?.toLowerCase();
        const targetId = readString(readOwnValue(values, "targetId"))?.toLowerCase();
        ...
        const isBrowserWorkspaceHandoff = (actionName === "BROWSER" ||
            ...
            (subaction === "open" ||
                ...
                subaction === "show") &&
            viewId === "browser";
        if ((isViewsHandoff || isAppBrowserHandoff || isBrowserWorkspaceHandoff) &&
            viewId) {
            ...
            const viewPath = isBrowserWorkspaceHandoff && actionUrl
                ? `/browser?browse=${encodeURIComponent(actionUrl)}`
                : declaredViewPath;
            const subview = readString(readOwnValue(values, "subview"));
            const completedActionHandoffId = normalizeCompletedActionHandoffId(readOwnValue(values, "completedActionHandoffId"));
            return {
                viewId,
                ...(viewPath ? { viewPath } : {}),
                ...(subview ? { subview } : {}),
    ...
    return {
        ...
        currentView: {
            ...
            ...(Array.isArray(record.views)
                ? {
                    views: record.views.filter((view): view is string => typeof view === "string"),
                }
                : {}),
...
async function fetchCurrentViewResponse(fetchCurrentView?: () => Promise<Response>): Promise<CurrentViewResponse> {
    const response = await (fetchCurrentView?.() ??
        fetchWithCsrf("/api/views/current"));
    if (!response.ok) {
        throw new ElizaError(`GET /api/views/current returned HTTP ${response.status}`, {
...
export async function dispatchViewActionHandoff(actionResults: readonly ChatActionResultSummary[] | undefined, dependencies: {
    ...
    if (!handoff)
        return false;
    const { currentView: current } = await fetchCurrentViewResponse(dependencies.fetchCurrentView);
    if (!current) {
        throw new ElizaError("The VIEWS action completed without a current view", {
            code: "VIEW_HANDOFF_CURRENT_VIEW_MISSING",
        });
    ...
    if (current.viewId !== handoff.viewId) {
        throw new ElizaError(`VIEWS action selected "${handoff.viewId}" but current view is "${current.viewId}"`, {
 ...
 */
export function dispatchViewActionHandoffDirect(actionResults: readonly ChatActionResultSummary[] | undefined, dispatch: typeof dispatchNavigateViewEvent = dispatchNavigateViewEvent): boolean {
    const handoff = findViewActionHandoff(actionResults);
    if (!handoff)
        return false;
    const detail = {
...
export async function recoverMissedCurrentView(dependencies: {
    fetchCurrentView?: () => Promise<Response>;
    dispatch?: typeof dispatchNavigateViewEvent;
    currentPath?: () => string;
} = {}): Promise<boolean> {
    ...
    const { currentView: current, justSwitched } = await fetchCurrentViewResponse(dependencies.fetchCurrentView);
    if (!current || !justSwitched || current.source !== "agent")
        return false;
    ...
    if (readPath() !== pathBeforeFetch)
        return false;
    const targetPath = current.viewPath ?? `/apps/${current.viewId}`;
    if (pathBeforeFetch === targetPath && !current.subview)
        return false;
    const dispatch = dependencies.dispatch ?? dispatchNavigateViewEvent;
