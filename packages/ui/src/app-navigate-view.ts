...
export function consumeNavigateViewPayload<T = unknown>(viewId: string): T | null {
    if (!pendingNavigateViewPayloads.has(viewId))
        return null;
    const payload = pendingNavigateViewPayloads.get(viewId) as T;
    pendingNavigateViewPayloads.delete(viewId);
    return payload;
}
export function __setNavigateViewPayloadForTests(viewId: string, payload: unknown): void {
    pendingNavigateViewPayloads.set(viewId, payload);
}
function storeNavigateViewPayload(detail: NavigateViewDetail): void {
    if (!detail.viewId || detail.payload === undefined)
        return;
    pendingNavigateViewPayloads.set(detail.viewId, detail.payload);
}
export type DesktopTabOpen = (view: ViewRegistryEntry, options?: {
    pinned?: boolean;
}) => void;
...
export function pathForNavigateViewDetail(detail: NavigateViewDetail, views: readonly ViewRegistryEntry[] = []): string | null {
    if (detail.viewPath)
        return detail.viewPath;
    if (!detail.viewId)
        return null;
    const entry = desktopEntryForDetail(views, detail.viewId);
    return entry?.path ?? `/apps/${detail.viewId}`;
}
export function directTabForNavigateView(detail: NavigateViewDetail, path: string): "views" | "apps" | null {
    if (path === "/views")
        return "views";
...
export function desktopEntryForDetail(views: readonly ViewRegistryEntry[], viewId: string): ViewRegistryEntry | undefined {
...
export function createNavigateViewHandler({ availableViewsForDesktopTabs, closeDesktopTab, desktopTabs = [], invokeDesktopBridgeRequest, navigatePath = navigateBrowserPath, openDesktopTab, setActiveDesktopTabId, setTab, setTabForPath, setViewLayout, }: {
    ...
    return (event: Event) => {
        ...
        if (detail.action === "split-view" || detail.action === "tile-views") {
            ...
            for (const viewId of viewIds) {
                const entry = desktopEntryForDetail(availableViewsForDesktopTabs, viewId);
                if (!entry)
                    continue;
        ...
        const path = pathForNavigateViewDetail(detail, availableViewsForDesktopTabs);
        if (!path)
            return false;
        setViewLayout?.(null);
        ...
        if (detail.action === "open-window" && detail.viewId) {
            const entry = desktopEntryForDetail(availableViewsForDesktopTabs, detail.viewId);
            ...
            void invokeDesktopBridgeRequest<{
                ...
                // observable.
                logger.warn({ err, viewPath }, "[app-navigate-view] desktop openAppWindow failed; opening in-shell");
                activateTabForPath(viewPath);
                navigatePath(viewPath);
            });
            return true;
