...
import { resetUiRegistryHostForTests } from "@elizaos/ui/registry-host";
import { resolveInitialTabForPath } from "./navigation";
...
function AppWithRealNavigation() {
    ...
    const [tab, setTabRaw] = React.useState<Tab>(() => resolveInitialTabForPath(getWindowNavigationPath(), "chat"));
    const [, setAppsSubTab] = React.useState<"browse" | "running" | "games">("browse");
    ...
    return (<NavigationHarnessContext.Provider value={{ tab, setTab, navigation }}>
      <App />
...
const authStatusMock = vi.hoisted(() => ({
    phase: "authenticated" as "authenticated" | "unauthenticated" | "server_unavailable",
    refetch: vi.fn(),
    use: vi.fn(),
}));
...
const dynamicViewLoaderMock = vi.hoisted(() => ({
    render: vi.fn(({ bundleUrl, frameUrl, surface, viewId, viewType, }: {
        bundleUrl?: string;
        frameUrl?: string;
        surface?: {
            capabilities?: string[];
        ...
        viewId: string;
        viewType?: string;
    }) => (<div data-bundle-url={bundleUrl ?? ""} data-frame-url={frameUrl ?? ""} data-surface-capabilities={surface?.capabilities?.join(",") ?? ""} data-testid="dynamic-view-loader" data-view-id={viewId} data-view-type={viewType ?? ""}/>)),
}));
...
function resetMockAvailableViews() {
    mockAvailableViews.splice(0, mockAvailableViews.length, remoteLedgerView, viewsManagerView, projectBoardView, calendarView, sharedCanvasView, documentsView);
}
...
vi.mock("./hooks/useAuthStatus", () => ({
    useAuthStatus: (options: {
        ...
        return {
            state: authStatusMock.phase === "authenticated"
    ...
    getAuthStatusSnapshot: () => authStatusMock.phase === "authenticated"
        ? authenticatedAuthStatus
...
vi.mock("./state", async () => {
    ...
    return {
        ACCENT_PRESETS,
        useApp: useAppValue,
        useAppSelector: <T,>(selector: (s: ReturnType<typeof useAppValue>) => T): T => selector(useAppValue()),
        useAppSelectorShallow: <T,>(selector: (s: ReturnType<typeof useAppValue>) => T): T => selector(useAppValue()),
    };
});
...
vi.mock("./components/character/CharacterEditor", () => ({
    CharacterEditor: ({ initialPage }: {
        initialPage?: string;
    }) => (<div data-initial-page={initialPage ?? ""} data-testid={initialPage === "documents" ? "documents-view" : "character-editor"}/>),
}));
...
describe("App navigate-view event wiring", () => {
    ...
    async function mountedRemoteNavigation() {
        ...
        await waitFor(() => expect(getActiveSurfaceRealmScope()?.viewId).toBe(remoteLedgerView.id));
        const actual = await vi.importActual<typeof import("./components/views/DynamicViewLoader")>("./components/views/DynamicViewLoader");
    ...
    it.each(["view", "policy", "bundle", "plugin"] as const)("revokes a mounted remote handle after actual %s replacement", async (change) => {
        const { rendered, navigate } = await mountedRemoteNavigation();
        await act(async () => {
            if (change === "view") {
    ...
    it.each(["component", "loader"] as const)("revokes old host navigation after registered %s replacement", async (change) => {
        ...
        await waitFor(() => expect(getActiveSurfaceRealmScope()?.viewId).toBe("owned-host"));
        const actual = await vi.importActual<typeof import("./components/views/DynamicViewLoader")>("./components/views/DynamicViewLoader");
        const external = await actual.hostImport("@elizaos/ui/app-navigate-view");
        const navigate = external.navigateBrowserPath;
        if (typeof navigate !== "function")
    ...
    it.each(["base", "credential"] as const)("revokes a remote handle on actual client %s authority replacement", async (change) => {
        ...
        try {
            await act(async () => {
                if (change === "base")
                    ...
                    actualClient.setToken("synthetic-scope-owner-b");
            });
            expect(() => navigate("/inventory")).toThrow(SurfaceRealmDeniedError);
            expect(window.location.pathname).not.toBe("/inventory");
        }
    ...
    it("keeps the exact branded staging Pages alias inside first-run onboarding", () => {
        ...
        window.localStorage.setItem("steward_session_token", "existing-steward-session");
        render(<BrandingContext.Provider value={{ ...DEFAULT_BRANDING, cloudOnly: true }}>
        <App />
      </BrandingContext.Provider>);
        expect(authStatusMock.use).toHaveBeenCalledWith(expect.objectContaining({ skip: true }));
    ...
    it.each([
        ...
        authStatusMock.phase = "unauthenticated";
        window.localStorage.setItem("steward_session_token", "existing-steward-session");
        render(<BrandingContext.Provider value={{ ...DEFAULT_BRANDING, cloudOnly }}>
        <App />
      </BrandingContext.Provider>);
        expect(authStatusMock.use).toHaveBeenCalledWith(expect.objectContaining({ skip: false }));
    ...
    it("renders Files with its canonical shell-owned page scroller", async () => {
        ...
        const { container } = render(<App />);
        await waitFor(() => {
            expect(container.querySelector('[data-page-kind="content"]')).not.toBeNull();
        });
        ...
        expect(frame
            ...
            ?.getAttribute("data-page-gutter")).toBe("standard");
        expect(container.querySelectorAll("[data-scroll-owner]")).toHaveLength(1);
    ...
    it("renders Automations with one framed gutter, clearance, and view scroller", async () => {
        ...
        const { container } = render(<App />);
        const automations = await screen.findByTestId("automations-layout", {}, { timeout: 10000 });
        const frame = automations.closest<HTMLElement>("[data-page-kind]");
        const pageContent = frame?.querySelector<HTMLElement>(":scope > [data-page-content]");
        ...
        const clearanceOwners = Array.from(frame?.querySelectorAll<HTMLElement>("*") ?? []).filter((element) => element
            .getAttribute("class")
        ...
        expect(automations.className).not.toContain("pe-[var(--eliza-chat-side-clearance,0px)]");
        expect(automations.className).not.toContain("[@media(orientation:landscape)_and_(max-height:520px)]:pb-0");
        ...
        expect(frame?.querySelectorAll('[data-shell-scroll-region="true"]')).toHaveLength(0);
        expect(container.querySelectorAll("[data-scroll-owner]")).toHaveLength(1);
    });
    ...
    it("renders an explicit unavailable state instead of healthy Home for an absent surface", async () => {
        ...
        const { container } = render(<App />);
        await waitFor(() => {
            expect(container.querySelector('[data-view-status="unavailable"][data-view-id="phone"]')).toBeTruthy();
        });
    ...
    it("frames the immersive background editor exactly once", async () => {
        ...
        const { container } = render(<App />);
        await waitFor(() => {
            expect(container.querySelector('[data-page-kind="immersive"]')).not.toBeNull();
        });
    ...
    it("passes settings navigate payloads into SettingsView for targeted permission priming", async () => {
        ...
        render(<App />);
        fireEvent(window, createNavigateViewEvent({
            viewId: "settings",
            viewPath: "/settings",
            subview: "permissions",
            payload,
        ...
        await waitFor(() => {
            expect(settingsViewMock.render).toHaveBeenCalledWith(expect.objectContaining({
                initialSection: "permissions",
                navigatePayload: payload,
                navigateSequence: 1,
            }));
        });
        const settingsPageContent = (await screen.findByTestId("settings-view")).closest<HTMLElement>("[data-page-content]");
        expect(settingsPageContent).not.toBeNull();
        expect(settingsPageContent?.getAttribute("data-page-gutter")).toBe("none");
        expect(settingsPageContent
            ?.closest<HTMLElement>("[data-page-kind]")
            ?.getAttribute("data-page-width")).toBe("full");
        expect(settingsPageContent?.className).toContain("pb-[var(--eliza-chat-clearance,5.25rem)]");
        expect(settingsPageContent?.className).not.toContain("pe-[var(--eliza-chat-side-clearance,0px)]");
        ...
        expect(screen
            ...
            .closest<HTMLElement>("[data-app-shell-root]")?.style.paddingTop).toBe("0px");
    });
    ...
    it("renders a remote module route through DynamicViewLoader in the mounted App", async () => {
        ...
        await waitFor(() => {
            expect(dynamicViewLoaderMock.render).toHaveBeenCalledWith(expect.objectContaining({
                bundleUrl: "/api/views/remote-ledger/bundle.js",
        ...
        expect(loader.getAttribute("data-bundle-url")).toBe("/api/views/remote-ledger/bundle.js");
        ...
        expect(container
            ...
            ?.className.includes("pb-[var(--eliza-chat-clearance")).toBe(true);
        expect(container
            .querySelector('[data-shell-content-region="true"] [data-page-content]')
            ?.className.includes("pe-[var(--eliza-chat-side-clearance")).toBe(false);
    ...
    it("keeps an in-process page's own header without adding a duplicate shell header", async () => {
        registerAppShellPage({
            ...
            Component: () => (<section data-testid="signed-normal-content">
          <h1>Signed Normal</h1>
        </section>),
        });
        appState.tab = "apps";
    ...
    it.each([
    ...
    ])("returns a registered fixture to the launcher using its own back control ($entry, StrictMode=$strictMode)", async ({ strictMode, entry }) => {
        ...
        window.history.replaceState(null, "", entry === "direct" ? "/notes" : "/views");
        render(strictMode ? (<React.StrictMode>
            <AppWithRealNavigation />
          </React.StrictMode>) : (<AppWithRealNavigation />));
        ...
        expect(() => window.history.pushState(null, "", "/views")).toThrow(SurfaceRealmDeniedError);
        ...
        await waitFor(() => {
            ...
            expect(screen.queryByRole("region", { name: "Notes fixture" })).toBeNull();
    ...
    it("keeps modal remote pages headerless without treating them as fullscreen", async () => {
        ...
        expect(container
            .querySelector('[data-shell-content-region="true"] [data-page-content]')
            ?.className.includes("pb-[var(--eliza-chat-clearance")).toBe(true);
        expect(container.querySelector<HTMLElement>("[data-app-shell-root]")?.style
            .paddingTop).not.toBe("0px");
    });
    it.each(["/documents", "/knowledge"])("keeps the legacy Knowledge route %s on the canonical plugin surface", async (path) => {
        registerAppShellPage({
            id: "documents",
        ...
        expect(await findByTestId("documents-view", undefined, { timeout: 5000 })).toBeTruthy();
    ...
    it("prefers an exact remote plugin route over its native wallet fallback", async () => {
        ...
        await waitFor(() => getByTestId("dynamic-view-loader"));
        expect(getByTestId("dynamic-view-loader").getAttribute("data-view-id")).toBe(walletMarketView.id);
    ...
    it("keeps signed-out remote Cloud rendering and capability ownership together for the plugin audit fixture", async () => {
        ...
        expect(getByTestId("dynamic-view-loader").getAttribute("data-view-id")).toBe("remote-cloud-imposter");
        await waitFor(() => {
            expect(getActiveSurfaceRealmScope()?.viewId).toBe("remote-cloud-imposter");
        });
        expect([
    ...
    it.each([
    ...
    ])("gives an authenticated Steward session authoritative %s Cloud dashboard ownership", async (_label, path) => {
        registerAppShellPage({
            id: "cloud",
            pluginId: "@elizaos/ui",
            label: "Cloud",
        ...
        expect(getActiveSurfaceRealmScope()?.manifest.layout.topology).toBe("ambient");
    });
    it.each([
        "restoring-session",
        "polling-backend",
        ...
        "first-run-required",
        "ready",
    ])("keeps authenticated account management available during agent %s", async (phase) => {
        registerAppShellPage({
            id: "cloud",
            pluginId: "@elizaos/ui",
            label: "Cloud",
    ...
    ])("keeps agent startup gates for $owner with Cloud auth=$authenticated", ({ authenticated, owner }) => {
        registerAppShellPage({
            id: "cloud",
            pluginId: owner,
            label: "Cloud",
    ...
    it("gives an in-process wallet page a live agent-surface registry", async () => {
        registerAppShellPage({
            ...
            path: "/inventory",
            tabAffinity: "inventory",
            Component: () => (<AgentButton agentId="wallet-refresh">Refresh wallet</AgentButton>),
        });
        appState.tab = "inventory";
        ...
        expect(getViewRegistry("wallet.inventory", "gui")?.describe("wallet-refresh")
            ?.label).toBe("Refresh wallet");
        const walletButton = screen.getByRole("button", {
            name: "Refresh wallet",
        });
        const walletContentRegion = walletButton.closest<HTMLElement>('[data-shell-content-region="true"]');
        expect(walletContentRegion).not.toBeNull();
        expect(walletContentRegion?.querySelector('[data-shell-scroll-region="true"]')).toBeNull();
        const routedMain = walletButton.closest("main");
        expect(routedMain?.className).not.toContain("px-2");
        expect(routedMain?.className).not.toContain("pt-[var(--view-pad-top)]");
        expect(walletButton.closest<HTMLElement>("[data-app-shell-root]")?.style
            .paddingTop).toBe("0px");
    });
    it("routes the registered native Contacts read through App without granting native writes or DOM control", async () => {
        ...
        const registration = listAppShellPages().find((entry) => entry.id === "contacts");
        if (!registration)
            throw new Error("Native Contacts page did not register");
        const onClick = vi.fn();
        registerAppShellPage({
            ...registration,
            Component: () => (<AgentButton agentId="create-contact" onClick={onClick}>
          Create contact
        </AgentButton>),
        });
        ...
        try {
            ...
            await expect(invokeViewInteract("contacts", "gui", "list-contacts")).resolves.toEqual({
                query: "",
            ...
            for (const operation of [
                ...
                await expect(invokeViewInteract("contacts", "gui", operation, {
                    id: "create-contact",
    ...
    it("hands a cold wallet deep link to a deferred app-shell registration", async () => {
        ...
        window.history.replaceState(null, "", "/wallet");
        render(<App />);
        expect((await screen.findByTestId("dynamic-plugin-page-loading")).textContent).toBe("Loading wallet.inventory…");
        act(() => {
            registerAppShellPage({
                id: "wallet.inventory",
                pluginId: "@elizaos/plugin-wallet:ui",
                ...
                path: "/inventory",
                tabAffinity: "inventory",
                Component: () => (<div data-testid="deferred-wallet-page">Wallet ready</div>),
            });
        });
        expect((await screen.findByTestId("deferred-wallet-page")).textContent).toBe("Wallet ready");
        expect(screen.queryByTestId("dynamic-plugin-page-loading")).toBeNull();
    });
    it.each(["/inventory", "/wallet/activity", "/wallet/markets"])("does not canonicalize a cold exact wallet-family route through tab affinity: %s", async (path) => {
        appState.tab = "views";
        window.history.replaceState(null, "", path);
        const registrations = [
            {
        ...
        const owningRegistration = registrations.find((registration) => registration.path === path);
    ...
    it("mounts an exact signed native renderer when stale registry metadata still advertises a remote bundle", async () => {
        ...
        try {
            ...
            expect(frame
                ?.querySelector("[data-page-content]")
                ?.getAttribute("data-page-gutter")).toBe("none");
            expect(container.querySelectorAll("[data-page-kind]")).toHaveLength(1);
        }
        finally {
            platform.mockRestore();
    ...
    it("lets a fullscreen plugin view fill behind the floating composer", async () => {
        ...
        expect(container
            .querySelector('[data-shell-content-region="true"] [data-page-content]')
            ?.className.includes("pb-[var(--eliza-chat-clearance")).toBe(false);
        expect(container
            .querySelector('[data-shell-content-region="true"] [data-page-content]')
            ?.className.includes("pe-[var(--eliza-chat-side-clearance")).toBe(false);
        expect(container.querySelector<HTMLElement>("[data-app-shell-root]")?.style
    ...
    it("routes frame-only sandboxed views through DynamicViewLoader with frameUrl", async () => {
        ...
        expect(loader.getAttribute("data-frame-url")).toBe("/api/views/sandboxed-frame/frame.html");
    });
    it("keeps an unavailable registered route in its loader for retry instead of the view manager", async () => {
        ...
        render(<App />);
        await waitFor(() => {
            expect(dynamicViewLoaderMock.render).toHaveBeenCalledWith(expect.objectContaining({ viewId: "unavailable-ledger" }), undefined);
        });
    ...
    it("reports user desktop-tab clicks to the agent without a navigation echo", async () => {
        ...
        await waitFor(() => {
            expect(globalThis.fetch).toHaveBeenCalledWith("http://agent.local/api/views/remote-ledger/navigate", expect.objectContaining({
                method: "POST",
                body: JSON.stringify({
                    source: "user",
                    path: "/apps/remote-ledger",
    ...
    it("replays a failed cold-start view report once after reconnect and still clears Home", async () => {
        ...
        const rendered = render(<App />);
        const viewNavigationCalls = () => fetchMock.mock.calls.filter(([input]) => String(input).includes("/api/views/"));
    ...
    it("renders split-view events as a live dynamic view layout", async () => {
        ...
        expect(loaders.map((loader) => loader.getAttribute("data-view-id"))).toEqual(["project-board", "calendar"]);
        expect(loaders[0]?.getAttribute("data-surface-capabilities")).toBe("agent-surface");
        ...
        expect(desktopTabsMock.openTab).toHaveBeenCalledWith(projectBoardAgentSurfaceView, {
            pinned: false,
        });
    ...
    it("renders registered documents bundles inside split-view when registry wins", async () => {
        ...
        expect(getAllByTestId("dynamic-view-loader").map((loader) => loader.getAttribute("data-view-id"))).toEqual(["documents", "calendar"]);
    ...
    it("lands on the designed not-found state for a navigate-view id nothing serves (#17033)", async () => {
        ...
        expect(await screen.findByTestId("app-route-not-found", {}, { timeout: 4000 })).toBeTruthy();
