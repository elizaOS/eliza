 ...
 */
export type FirstRunLocalInference = "all-local" | "cloud-inference" | "configure-later";
...
export function normalizeFirstRunName(value: string | null | undefined): string {
...
export function firstRunRuntimeTarget(runtime: FirstRunRuntime, localInference: FirstRunLocalInference = "all-local"): FirstRunRuntimeTarget {
    if (runtime === "cloud")
        return "elizacloud";
    if (runtime === "remote")
        return "remote";
 ...
 * place.
 */
export function firstRunNeedsCloudConnect(draft: Pick<FirstRunProfileDraft, "runtime" | "localInference">, elizaCloudConnected: boolean): boolean {
    if (elizaCloudConnected)
        return false;
    if (draft.runtime === "cloud")
        return true;
    return (draft.runtime === "local" && draft.localInference === "cloud-inference");
}
/**
 ...
 */
export function firstRunDownloadsLocalModel(localInference: FirstRunLocalInference): boolean {
...
export function validateFirstRunSubmitDraft(draft: FirstRunProfileDraft): FirstRunSubmitValidation {
...
export function buildFirstRunSubmitPlan(args: {
    ...
    const serverTarget = firstRunRuntimeTarget(args.draft.runtime, args.draft.localInference);
    const cloudInference = args.draft.runtime === "cloud" || serverTarget === "elizacloud-hybrid";
    ...
    const systemPrompt = style.system?.replace(/\{\{name\}\}/g, agentName) ??
