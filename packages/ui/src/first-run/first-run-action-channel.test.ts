 ...
 * dispatched while a conductor is active, a non-prefixed value is never
 * intercepted, and a reserved-prefix value is identifiable even after
 * onboarding finished and the handler is gone (leftover transcript widgets
 * stay inert instead of reaching the server).
 */
...
describe("isFirstRunAction (the send funnel's routing contract)", () => {
  ...
  it("does not reserve ordinary chat text", () => {
    expect(isFirstRunAction("hello")).toBe(false);
  });
});
