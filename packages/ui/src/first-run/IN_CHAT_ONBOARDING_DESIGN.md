...
- `choice-__first_run__:runtime:{cloud|local|remote}` in developer builds that
  explicitly enable the runtime chooser. Production onboarding is Cloud-only.
  "Bring your own keys" is not a location; it lives on the provider axis below.
- `choice-__first_run__:provider:{on-device|elizacloud|other}`
- `choice-__first_run__:tutorial:{start|skip}`

Those choices route into the headless first-run finish path. App-shell targets
produce one guarded `POST /api/first-run`; direct shared/dedicated Cloud agent
hosts produce none because they do not own that route. Tests should assert the
real chat overlay plus transcript choices and keep negative assertions for
deleted surfaces such as
...
## The sign-in-first onboarding surface
...
## Flow model and confused-user guards
...
- **One explicit phase.** `first-run-flow.ts` represents runtime choice,
  provider choice, sign-in, provisioning, recovery, wrap-up, and completion as
  a discriminated union. Busy, provisioned, and silent-entry behavior is
  derived from that phase instead of independent boolean latches.
- **One action table.** Each phase lists the action groups it accepts. A stale
  runtime widget cannot provision during wrap-up, and a second tap cannot start
  a concurrent flow while the phase is `provisioning`.
- **Strict values.** The shared parser canonicalizes reserved values. Malformed
  values and groups that are illegal for the current phase are consumed, never
  acted on. Cloud-only policy is enforced in that same router.
- **The prefix is reserved forever.** `isFirstRunAction` (the send
...
- **"Other / configure in Settings" stays honest.** The `provider:other` pick
  starts the local runtime without inventing a provider or downloading a model.
  Setup completes normally, and the no-provider chat surface points the user to
  Settings. `error:settings` remains the explicit escape when setup itself
  fails.
