      ...
      <ShellControllerProvider>
        ...
        >
          {/* BOTTOM-BAR / SAFE-AREA FLOOR (do not remove): a viewport-filling
              ...
              bottom inset (and every unpainted zone) reads as the dark
              BACKGROUND token — never accent — regardless of route or shader
              state.

              R3.4 (standalone-PWA bottom-bar fix): on SHARED-background routes
              ...
              wallpaper sits above it — the residual r3.3 bar. Going transparent
              on wallpaper routes lets the full-bleed wallpaper own the whole
              screen down to the bottom edge; the FOUC/orange guard is still
              covered because the wallpaper layer is opaque cover-fit. On
              OPAQUE/overlay routes (no wallpaper) the floor keeps `bg-bg` so the
              orange guard never shows. */}
          <div
            aria-hidden="true"
