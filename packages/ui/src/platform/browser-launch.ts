...
function isTrustedCloudLaunchHost(host: string): boolean {
    ...
    return (isElizaCloudControlPlaneHostname(normalized) ||
        isElizaDedicatedAgentHostname(normalized) ||
        isConfiguredCloudHost(normalized));
}
function normalizeLaunchApiBase(apiBase: string, options: {
    kind?: "cloud" | "remote";
} = {}): string {
    ...
    try {
        ...
        if (isTrustedRestoreApiBaseUrl(parsed.toString()) ||
            (options.kind === "cloud" &&
...
async function exchangeCloudLaunchSession(cloudBaseUrl: string, sessionId: string): Promise<{
    agentId: string;
    apiBase: string;
    token: string;
}> {
...
export function applyLaunchConnection(args: {
    ...
    const cloudAgentId = kind === "cloud" ? args.cloudAgentId?.trim() || null : null;
    if (kind === "cloud" &&
...
export async function applyLaunchConnectionFromUrl(): Promise<boolean> {
    ...
    if (launchSession && launchBase) {
        const connection = await exchangeCloudLaunchSession(normalizeLaunchBaseUrl(launchBase), launchSession);
