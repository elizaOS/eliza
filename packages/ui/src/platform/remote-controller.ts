...
function isOpenedResult(value: unknown): value is {
    ...
    return (isRecord(value) &&
        ["completed", "rejected", "cancelled", "execution_ambiguous"].includes(String(value.status)) &&
...
function nativeRemoteController(): NativeRemoteControllerPlugin {
    cachedNativeRemoteController ??=
        Capacitor.registerPlugin<NativeRemoteControllerPlugin>("RemoteControllerIdentity");
...
export async function getOrCreateRemoteControllerIdentity(input: {
    ...
    if (isUnsupportedNativeIOS()) {
        throw new Error("Secure mobile pairing is unavailable until the native iOS plugin is installed.");
    }
    ...
    const identity = await invokeDesktopBridgeRequest<RemoteControllerPublicIdentity>({
    ...
    if (!identity) {
        throw new Error("Secure device pairing requires the Eliza desktop app so private keys stay in OS credential storage.");
    }
    return identity;
}
export async function createRemoteCommand(input: {
    ...
    if (isUnsupportedNativeIOS()) {
        throw new Error("Secure mobile command signing is unavailable until the native iOS plugin is installed.");
    }
    if (isNativeController()) {
    ...
    if (!result)
        throw new Error("Secure remote command signing is unavailable.");
    return result;
}
export async function acknowledgeRemoteCommandEnqueue(input: {
    ...
    if (isUnsupportedNativeIOS()) {
        throw new Error("Secure mobile enqueue acknowledgement is unavailable until the native iOS plugin is installed.");
    }
    if (isNativeController()) {
...
export async function openRemoteCommandResult(input: {
    ...
    if (isUnsupportedNativeIOS()) {
        throw new Error("Secure mobile result decryption is unavailable until the native iOS plugin is installed.");
    }
    if (isNativeController()) {
...
export async function openRemoteCommandStartReceipt(input: {
    ...
    if (isUnsupportedNativeIOS()) {
        throw new Error("Secure mobile start receipt verification is unavailable until the native iOS plugin is installed.");
    }
    if (isNativeController()) {
        const result = await nativeRemoteController().openStartReceipt(input);
        if (!isOpenedStartReceipt(result))
            throw new Error("Secure mobile start receipt verification is unavailable.");
        return result;
    }
    const result = await invokeDesktopBridgeRequest<{
        startedAt: number;
...
export async function clearRemoteControllerSessionState(input: {
    ...
    if (isUnsupportedNativeIOS()) {
        throw new Error("Secure mobile session cleanup is unavailable until the native iOS plugin is installed.");
    }
    if (isNativeController()) {
