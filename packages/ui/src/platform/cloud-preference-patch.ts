...
import { readString } from "../state/config-readers";
...
import { type CloudPreferencePatchState as PatchState } from "./types";
...
function hasRemoteConnection(config: StorageConfig | null | undefined): boolean {
    return (resolveDeploymentTargetInConfig(config as Record<string, unknown>)
        .runtime === "remote");
}
function cloudHandlesInference(config: StorageConfig | null | undefined): boolean {
    return resolveElizaCloudTopology(config as Record<string, unknown>).services
        .inference;
}
function hasInactiveCloudSignals(config: StorageConfig | null | undefined): boolean {
    return isElizaCloudLinkedInConfig(config as Record<string, unknown>);
}
export function shouldPreferLocalProviderConfig(config: StorageConfig | null | undefined): boolean {
    if (!config)
        return false;
    ...
    return Boolean(!hasRemoteConnection(config) &&
        !cloudHandlesInference(config) &&
        hasInactiveCloudSignals(config));
}
export function normalizeConfigForLocalProviderPreference(config: StorageConfig | null | undefined): StorageConfig | null | undefined {
    ...
    if (apiKey)
        nextCloud.apiKey = apiKey;
    return { ...config, cloud: nextCloud };
}
export function installLocalProviderCloudPreferencePatch(client: ClientLike): () => void {
    ...
    client.getConfig = (async () => {
        const config = (await originalGetConfig()) as StorageConfig | null | undefined;
        return normalizeConfigForLocalProviderPreference(config) as Record<string, unknown>;
