 ...
 */
import { getAppBlockerPlugin } from "../bridge/native-plugins";
import { getAppleCalendarPlugin } from "../bridge/native-plugins";
import { getCameraPlugin } from "../bridge/native-plugins";
import { getContactsPlugin } from "../bridge/native-plugins";
import { getLocationPlugin } from "../bridge/native-plugins";
import { getMessagesPlugin } from "../bridge/native-plugins";
import { getMobileSignalsPlugin } from "../bridge/native-plugins";
import { getPhonePlugin } from "../bridge/native-plugins";
import { getPlaySettingsPlugin } from "../bridge/native-plugins";
import { getPushNotificationsPlugin } from "../bridge/native-plugins";
import { getScreenCapturePlugin } from "../bridge/native-plugins";
import { getSystemPlugin } from "../bridge/native-plugins";
...
import { type AppBlockerPermissionResult } from "../bridge/native-plugins";
import { type AppBlockerPluginLike } from "../bridge/native-plugins";
import { type AppleCalendarPermissionStatus } from "../bridge/native-plugins";
import { type AppleCalendarPluginLike } from "../bridge/native-plugins";
import { type CameraPermissionStatus } from "../bridge/native-plugins";
import { type CameraPluginLike } from "../bridge/native-plugins";
import { type ContactsPermissionStatus } from "../bridge/native-plugins";
import { type ContactsPluginLike } from "../bridge/native-plugins";
import { type IPermissionsRegistry } from "@elizaos/core/contracts/permissions";
import { type LocationPermissionStatus } from "../bridge/native-plugins";
import { type LocationPluginLike } from "../bridge/native-plugins";
import { type MessagesPermissionStatus } from "../bridge/native-plugins";
import { type MessagesPluginLike } from "../bridge/native-plugins";
import { type MobileSignalsOpenSettingsResult } from "../bridge/native-plugins";
import { type MobileSignalsPermissionStatus } from "../bridge/native-plugins";
import { type MobileSignalsPluginLike } from "../bridge/native-plugins";
import { type MobileSignalsScreenTimeStatus } from "../bridge/native-plugins";
import { type MobileSignalsSettingsTarget } from "../bridge/native-plugins";
import { type MobileSignalsSetupAction } from "../bridge/native-plugins";
import { type PermissionFeatureRef } from "@elizaos/core/contracts/permissions";
...
import { type PhonePermissionStatus } from "../bridge/native-plugins";
import { type PlaySettingsPluginLike } from "../bridge/native-plugins";
import { type PushNotificationPermissionStatus } from "../bridge/native-plugins";
import { type PushNotificationsPluginLike } from "../bridge/native-plugins";
import { type ScreenCapturePermissionStatus } from "../bridge/native-plugins";
import { type ScreenCapturePluginLike } from "../bridge/native-plugins";
import { type SystemPluginLike } from "../bridge/native-plugins";
import { type TalkModePermissionStatus } from "../bridge/native-plugins";
import { type TalkModePluginLike } from "../bridge/native-plugins";
type MobilePermissionId = Extract<PermissionId, "calendar" | "health" | "screentime" | "notifications" | "contacts" | "microphone" | "camera" | "location" | "screen-recording" | "speech-recognition" | "photos" | "phone" | "messages" | "wifi" | "bluetooth" | "app-blocking" | "usage-access" | "overlay" | "write-settings" | "local-network" | "battery-optimization">;
...
function defaultMobileState(id: PermissionId, status: PermissionStatus = "not-applicable", options: Partial<Omit<PermissionState, "id" | "status" | "lastChecked">> = {}): PermissionState {
    return {
        id,
        status,
...
type PromptLikePermissionState = "granted" | "denied" | "prompt" | "prompt-with-rationale" | "limited" | "not_supported" | undefined | null;
function statusFromPromptLike(value: PromptLikePermissionState): PermissionStatus {
    if (value === "granted")
        return "granted";
    if (value === "limited")
        return "limited";
...
function stateFromPromptLike(id: PermissionId, value: PromptLikePermissionState, reason?: string): PermissionState {
    ...
    return defaultMobileState(id, status, {
        ...
        restrictedReason: status === "not-applicable" ? "platform_unsupported" : undefined,
        reason: value === "limited" && !reason
            ? "Limited access is enabled; manage selected items in system settings."
            : reason,
    });
}
function stateFromCamera(id: Extract<MobilePermissionId, "camera" | "microphone" | "photos">, permissions: CameraPermissionStatus): PermissionState {
    if (id === "camera")
        return stateFromPromptLike(id, permissions.camera);
    if (id === "microphone") {
        return stateFromPromptLike(id, permissions.microphone);
    }
    return stateFromPromptLike(id, permissions.photos);
}
function stateFromTalkMode(id: Extract<MobilePermissionId, "microphone" | "speech-recognition">, permissions: TalkModePermissionStatus): PermissionState {
    if (id === "speech-recognition") {
        return stateFromPromptLike(id, permissions.speechRecognition);
    }
    return stateFromPromptLike(id, permissions.microphone);
}
function stateFromLocation(id: Extract<MobilePermissionId, "location" | "wifi">, permissions: LocationPermissionStatus): PermissionState {
    const reason = id === "wifi"
...
async function verifyForegroundLocationRead(state: PermissionState, locationPlugin: LocationPluginLike): Promise<PermissionState> {
    ...
    try {
        ...
        return {
            ...state,
            reason: "Location permission is enabled, but Android could not provide a current location. Check Location Services and try again.",
        };
    }
}
function stateFromScreenCapture(permissions: ScreenCapturePermissionStatus): PermissionState {
    return stateFromPromptLike("screen-recording", permissions.screenCapture, permissions.screenCapture === "prompt"
        ? "Screen capture shows an OS picker each time recording starts."
        : undefined);
}
function stateFromContacts(permissions: ContactsPermissionStatus): PermissionState {
...
function stateFromMessages(permissions: MessagesPermissionStatus): PermissionState {
    return stateFromPromptLike("messages", permissions.sms);
}
function stateFromAppBlocker(id: Extract<MobilePermissionId, "app-blocking" | "overlay">, permissions: AppBlockerPermissionResult): PermissionState {
    return defaultMobileState(id, permissions.status, {
        canRequest: permissions.canRequest,
        reason: permissions.reason,
    });
}
function stateFromWriteSettings(status: Awaited<ReturnType<NonNullable<SystemPluginLike["getDeviceSettings"]>>>): PermissionState {
    return defaultMobileState("write-settings", status.canWriteSettings ? "granted" : "not-determined", {
...
function normalizeMobileSignalsStatus(status: MobileSignalsPermissionStatus["status"]): PermissionStatus {
    return status;
}
function statusFromSetupAction(action: MobileSignalsSetupAction | null): PermissionStatus {
    if (!action)
        return "not-applicable";
...
function findSetupAction(permissions: MobileSignalsPermissionStatus, ids: readonly MobileSignalsSetupAction["id"][]): MobileSignalsSetupAction | null {
    return (permissions.setupActions.find((action) => ids.includes(action.id)) ?? null);
}
function restrictedReasonForScreenTime(screenTime: MobileSignalsScreenTimeStatus): PermissionState["restrictedReason"] {
...
function stateFromScreenTime(permissions: MobileSignalsPermissionStatus): PermissionState {
    ...
    const hasUsableCapability = screenTime.reportAvailable ||
    ...
    return defaultMobileState("screentime", screenTime.supported ? "restricted" : "not-applicable", {
        canRequest: false,
...
function stateFromHealth(permissions: MobileSignalsPermissionStatus): PermissionState {
    return defaultMobileState("health", normalizeMobileSignalsStatus(permissions.status), {
        canRequest: permissions.canRequest,
        reason: permissions.reason,
    });
}
function stateFromNotifications(permissions: MobileSignalsPermissionStatus): PermissionState {
...
function stateFromUsageAccess(permissions: MobileSignalsPermissionStatus): PermissionState {
...
function stateFromSetupAction(id: Extract<MobilePermissionId, "battery-optimization" | "local-network">, permissions: MobileSignalsPermissionStatus, actionIds: readonly MobileSignalsSetupAction["id"][]): PermissionState {
...
function stateFromPushNotifications(permissions: PushNotificationPermissionStatus): PermissionState {
...
function stateFromAppleCalendar(permissions: AppleCalendarPermissionStatus): PermissionState {
    const status: PermissionStatus = permissions.calendar === "prompt"
        ? "not-determined"
...
function stateFromMobileSignals(id: MobilePermissionId, permissions: MobileSignalsPermissionStatus): PermissionState {
    if (id === "calendar")
        return defaultMobileState("calendar");
    if (id === "health")
        return stateFromHealth(permissions);
    if (id === "screentime")
        return stateFromScreenTime(permissions);
    if (id === "notifications")
        return stateFromNotifications(permissions);
    if (id === "usage-access")
        return stateFromUsageAccess(permissions);
...
function mobileSettingsTargetFor(id: PermissionId): MobileSignalsSettingsTarget {
    if (id === "health")
...
export async function openMobilePermissionSettings(id: PermissionId, plugin: MobileSignalsPluginLike = getMobileSignalsPlugin(), systemPlugin: SystemPluginLike = getSystemPlugin(), playSettingsPlugin: PlaySettingsPluginLike = getPlaySettingsPlugin()): Promise<MobileSignalsOpenSettingsResult | undefined> {
    if (id === "write-settings" &&
    ...
    if ((id === "notifications" || id === "location") &&
...
export function createMobileSignalsPermissionsRegistry(plugin: MobileSignalsPluginLike = getMobileSignalsPlugin(), fallbackClient?: PermissionClientLike, appleCalendarPlugin: AppleCalendarPluginLike = getAppleCalendarPlugin(), pushNotificationsPlugin: PushNotificationsPluginLike = getPushNotificationsPlugin(), nativePlugins: NativePermissionPlugins = {}, initPushRegistrationAfterGrant: () => Promise<void> = initPushRegistration): IPermissionsRegistry {
    ...
    const checkMobilePermission = async (id: MobilePermissionId) => {
        if (id === "calendar") {
            if (typeof appleCalendarPlugin.checkPermissions !== "function") {
                return commit(defaultMobileState("calendar", "not-applicable", {
                    restrictedReason: "platform_unsupported",
                }));
            }
            return commit(stateFromAppleCalendar(await appleCalendarPlugin.checkPermissions()));
        }
        if (id === "contacts") {
            if (typeof native.contacts.checkPermissions !== "function") {
                return commit(defaultMobileState("contacts"));
            }
            return commit(stateFromContacts(await native.contacts.checkPermissions()));
        }
        if (id === "phone") {
        ...
        if (id === "messages") {
            ...
            return commit(stateFromMessages(await native.messages.checkPermissions()));
        }
        if (id === "camera" || id === "photos") {
            if (typeof native.camera.checkPermissions !== "function") {
                return commit(defaultMobileState(id));
            }
            return commit(stateFromCamera(id, await native.camera.checkPermissions()));
        }
        if (id === "microphone") {
            if (typeof native.talkMode.checkPermissions === "function") {
                return commit(stateFromTalkMode("microphone", await native.talkMode.checkPermissions()));
            }
            if (typeof native.camera.checkPermissions === "function") {
                return commit(stateFromCamera("microphone", await native.camera.checkPermissions()));
            }
        ...
        if (id === "speech-recognition") {
            ...
            return commit(stateFromTalkMode("speech-recognition", await native.talkMode.checkPermissions()));
        }
        if (id === "location" || id === "wifi") {
            if (typeof native.location.checkPermissions !== "function") {
                return commit(defaultMobileState(id));
            }
            return commit(stateFromLocation(id, await native.location.checkPermissions()));
        }
        if (id === "screen-recording") {
            ...
            return commit(stateFromScreenCapture(await native.screenCapture.checkPermissions()));
        }
        if (id === "app-blocking" || id === "overlay") {
            if (typeof native.appBlocker.checkPermissions !== "function") {
                return commit(defaultMobileState(id));
            }
            return commit(stateFromAppBlocker(id, await native.appBlocker.checkPermissions()));
        }
        if (id === "write-settings") {
            if (typeof native.system.getDeviceSettings !== "function") {
                return commit(defaultMobileState("write-settings"));
            }
            return commit(stateFromWriteSettings(await native.system.getDeviceSettings()));
        }
        if (id === "bluetooth") {
            return commit(defaultMobileState("bluetooth", "not-determined", {
                canRequest: true,
                reason: "Bluetooth permission is granted by the OS when a nearby accessory flow asks for it.",
            }));
        }
        if (id === "notifications") {
            if (typeof pushNotificationsPlugin.checkPermissions === "function") {
                return commit(stateFromPushNotifications(await pushNotificationsPlugin.checkPermissions()));
            }
        }
    ...
    return {
        ...
        async check(id) {
            if (isMobilePermissionId(id))
                return checkMobilePermission(id);
        ...
        async request(id, opts) {
            ...
            if (id === "calendar") {
                ...
                if (current.canRequest &&
                    ...
                    requestedState = stateFromContacts(await native.contacts.requestPermissions());
                }
                else {
                ...
                if (current.canRequest &&
                    ...
                    requestedState = stateFromPhone(await native.phone.requestPermissions());
                }
                else {
                    await openMobilePermissionSettings(id, plugin, native.system);
                }
                ...
                if (current.canRequest &&
                    ...
                    requestedState = stateFromMessages(await native.messages.requestPermissions());
                }
                else {
                    await openMobilePermissionSettings(id, plugin, native.system);
                }
                ...
                if (current.canRequest &&
                    ...
                    requestedState = stateFromCamera(id, await native.camera.requestPermissions());
                }
                else {
                    await openMobilePermissionSettings(id, plugin, native.system);
                }
                ...
                if (current.canRequest &&
                    ...
                    requestedState = stateFromTalkMode("microphone", await native.talkMode.requestPermissions());
                }
                else if (current.canRequest &&
                    typeof native.camera.requestPermissions === "function") {
                    requestedState = stateFromCamera("microphone", await native.camera.requestPermissions());
                }
                else {
                    await openMobilePermissionSettings(id, plugin, native.system);
                }
                ...
                if (current.canRequest &&
                    typeof native.talkMode.requestPermissions === "function") {
                    requestedState = stateFromTalkMode("speech-recognition", await native.talkMode.requestPermissions());
                }
                else {
                    await openMobilePermissionSettings(id, plugin, native.system);
                }
                ...
                if (current.canRequest &&
                    ...
                    requestedState = stateFromLocation(id, await native.location.requestPermissions());
                    if (id === "location") {
                        requestedState = await verifyForegroundLocationRead(requestedState, native.location);
                    }
                }
                else {
                    await openMobilePermissionSettings(id, plugin, native.system);
                ...
                if (current.canRequest &&
                    ...
                    requestedState = stateFromScreenCapture(await native.screenCapture.requestPermissions());
                }
                else {
                    await openMobilePermissionSettings(id, plugin, native.system);
                }
                ...
                if (current.canRequest &&
                    ...
                    requestedState = stateFromPushNotifications(await pushNotificationsPlugin.requestPermissions());
                }
                else if (current.canRequest &&
                ...
                if (current.canRequest &&
                    ...
                    requestedState = stateFromAppBlocker(id, await native.appBlocker.requestPermissions());
                }
                else {
                    await openMobilePermissionSettings(id, plugin, native.system);
                }
            ...
            if (id === "notifications" && next.status === "granted") {
                try {
                    ...
                    next = {
                        ...next,
                        reason: "Notifications are enabled on this device, but remote push registration is currently unavailable.",
                    };
                }
            }
            return commit({
        ...
        async openSettings(id) {
            const result = await openMobilePermissionSettings(id, plugin, native.system);
            return result?.opened === true;
        },
        recordBlock(id, feature: PermissionFeatureRef) {
            const current = states.get(id) ?? defaultMobileState(id, "not-determined");
            commit({
                ...current,
                lastBlockedFeature: {
                    ...feature,
        ...
        pending() {
            return Array.from(states.values()).filter((state) => state.status === "not-determined" ||
