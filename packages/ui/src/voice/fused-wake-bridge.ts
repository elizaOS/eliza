 ...
 */
import { FUSED_WAKE_EVENT } from "@elizaos/core/events";
...
export function emitFusedWake(event: FusedWakeEvent): void {
    if (typeof window === "undefined")
        return;
    window.dispatchEvent(new CustomEvent<FusedWakeEvent>(FUSED_WAKE_EVENT, { detail: event }));
}
/**
 ...
 */
export function subscribeFusedWake(listener: (event: FusedWakeEvent) => void): () => void {
    if (typeof window === "undefined")
        return () => { };
