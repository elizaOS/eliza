 ...
 */
import { DEFAULT_DOWNLINK_CODEC } from "./voice-session-protocol";
import { DEFAULT_UPLINK_CODEC } from "./voice-session-protocol";
import { INITIAL_VOICE_SESSION_STATE } from "./voice-session-state";
import { VOICE_SESSION_PROTOCOL_VERSION } from "./voice-session-protocol";
import { VOICE_SESSION_SAMPLE_RATE } from "./voice-session-protocol";
import { VoiceMicCaptureError } from "./voice-session-mic-capture";
import { applyClientAction } from "./voice-session-state";
import { applyServerEvent } from "./voice-session-state";
import { beginListening } from "./voice-session-state";
import { createVoiceSessionPlayback } from "./voice-session-playback";
import { encodeClientControl } from "./voice-session-protocol";
import { isUsableMintResponse } from "./voice-session-protocol";
import { loopToListening } from "./voice-session-state";
import { negotiateCodec } from "./voice-session-protocol";
import { parseServerControl } from "./voice-session-protocol";
import { parseVoiceUiContext } from "@elizaos/core/voice";
import { startVoiceMicCapture } from "./voice-session-mic-capture";
import { toContinuousStatus } from "./voice-session-state";
import { type MicAudioContextLike } from "./voice-session-mic-capture";
...
import { type VoiceMicCapture } from "./voice-session-mic-capture";
import { type VoiceSessionCodec } from "./voice-session-protocol";
import { type VoiceSessionMachineState } from "./voice-session-state";
import { type VoiceSessionMintResponse } from "./voice-session-protocol";
import { type VoiceSessionPlayback } from "./voice-session-playback";
import { type VoiceSessionPlaybackStatsEvent } from "./voice-session-playback";
...
export function createVoiceSessionClient(options: VoiceSessionClientOptions): VoiceSessionClient {
    ...
    const wsFactory = options.webSocketFactory ??
    ...
    const reconnectBudgetResetMs = Math.max(0, options.reconnectBudgetResetMs ?? 30000);
    ...
    let playbackEnd: Extract<ServerControlFrame, {
        t: "speaking_end";
    }> | null = null;
    ...
    const isLifecycleCurrent = (generation: number): boolean => !disposed && lifecycleGeneration === generation;
    ...
    const waitForRetry = async (attempt: number, generation: number) => {
        await new Promise<void>((resolve) => setTimeout(resolve, preLiveRetryDelayMs * attempt));
        assertLifecycleCurrent(generation);
    };
    const waitForReconnectBackoff = async (attempt: number, generation: number) => {
    ...
    async function mintOnce(generation: number): Promise<VoiceSessionMintResponse> {
        ...
        try {
            ...
            assertLifecycleCurrent(generation);
            throw new VoiceSessionConsentError("could not obtain realtime voice consent", cause);
        }
        assertLifecycleCurrent(generation);
        if (!consentNonce?.trim()) {
            throw new VoiceSessionConsentError("could not obtain realtime voice consent");
        }
        let res: Response;
        try {
            ...
            assertLifecycleCurrent(generation);
            throw new VoiceSessionMintError(0, "voice session mint transport failed", undefined, cause);
        }
        assertLifecycleCurrent(generation);
        if (!res.ok) {
    ...
    async function mint(generation: number): Promise<VoiceSessionMintResponse> {
        ...
        for (let attempt = 1; attempt <= preLiveMaxAttempts; attempt += 1) {
            try {
                ...
                lastError = typed;
                const retryable = (typed instanceof VoiceSessionMintError && typed.isTransient) ||
                    typed instanceof VoiceSessionConsentError;
                if (!retryable || attempt >= preLiveMaxAttempts)
                    throw typed;
        ...
        try {
            if (uiContextSupported && options.getUiContext) {
                ...
                if (serialized !== lastUiContext) {
                    ws.send(encodeClientControl({ t: "ui_context", context: context ?? {} }));
                    lastUiContext = serialized;
                    if (!context)
                        console.warn("[voice] Invalid renderer view context; cleared stale context");
                }
            }
            ...
            const uplink = microphoneMuted || state.phase === "speaking"
    ...
    function handleServerFrame(data: unknown, generation: number, socket: VoiceWebSocketLike): void {
        ...
        if (ArrayBuffer.isView(data)) {
            ...
            playback?.enqueue(new Uint8Array(view.buffer, view.byteOffset, view.byteLength));
            notifyPlaybackUnlockState();
            mark("downlink_audio", state.traceId);
            return;
        }
        ...
        switch (event.t) {
            ...
            case "assistant_playing":
                mark(event.active ? "assistant_playing" : "assistant_playing_end", event.traceId);
                break;
    ...
    async function startCapture(generation: number, socket: VoiceWebSocketLike): Promise<void> {
        if (mic ||
        ...
        try {
            ...
            if (!isLifecycleCurrent(generation) || ws !== socket)
                return;
            const error = err instanceof VoiceMicCaptureError
                ? err
            ...
            if (captureSocket === socket)
                captureSocket = null;
            if (captureAbort === captureController)
                captureAbort = null;
        }
    }
    async function openConnection(minted: VoiceSessionMintResponse, generation: number): Promise<void> {
        assertLifecycleCurrent(generation);
        ...
        socket.addEventListener("open", () => {
            ...
            try {
                socket.send(encodeClientControl({
                    t: "hello",
                    token: minted.token,
     ...
     */
    function scheduleRotation(minted: VoiceSessionMintResponse, generation: number): void {
        clearRotationTimer();
    ...
    function attemptRotation(generation: number): void {
        if (!isLifecycleCurrent(generation) || intentionalClose)
            return;
    ...
    function requestRecovery(reason: string, generation: number, socket: VoiceWebSocketLike, recoveryOptions?: {
        budgeted?: boolean;
        cleanBye?: boolean;
    }): void {
        ...
        try {
            socket.close(recoveryOptions?.cleanBye ? 1000 : 1012, recoveryOptions?.cleanBye ? "token rotation" : "re-mint");
        }
        catch (ignoredError) {
        ...
        if (recoveryPromise)
            return;
        const pending = handleTransportLoss(reason, generation, recoveryOptions?.budgeted ?? true);
        recoveryPromise = pending;
        void pending.finally(() => {
            if (recoveryPromise === pending)
                recoveryPromise = null;
        });
    }
    async function handleTransportLoss(reason: string, generation: number, budgetedInitial: boolean): Promise<void> {
        if (!isLifecycleCurrent(generation) || intentionalClose)
            return;
        clearRotationTimer();
        ...
        while (true) {
            if (budgeted && reconnectsUsed >= reconnectCap) {
                mark(`not_reached(reconnect_exhausted:${currentReason})`, state.traceId);
            ...
            try {
                ...
                if (attempt > 1)
                    await waitForReconnectBackoff(attempt, generation);
                ...
                if (!isLifecycleCurrent(generation))
                    return;
                if (err instanceof VoiceSessionLifecycleCancelledError)
                    return;
                const transient = (err instanceof VoiceSessionMintError && err.isTransient) ||
                ...
                if (transient) {
                    ...
                    if (reconnectsUsed < reconnectCap)
                        continue;
                    mark(`not_reached(reconnect_exhausted:${currentReason})`, state.traceId);
                }
                ...
                emitError(new Error(`voice session lost: ${currentReason}`, { cause: err }));
                await stop();
                return;
            }
        }
    ...
    return {
        ...
        async start() {
            if (connPhase !== "idle" && connPhase !== "closed")
                return;
            ...
            try {
                ...
                // traceId exists yet.
                mark(playback.unlocked
                    ? "playback_unlocked"
                    : "not_reached(playback_unlock_required)", state.traceId);
                await openConnection(minted, generation);
            }
            catch (err) {
                if (!isLifecycleCurrent(generation))
        ...
        setMicrophoneMuted(muted) {
            if (disposed || microphoneMuted === muted)
                return;
...
export class VoiceSessionMintError extends Error {
    constructor(readonly status: number, message?: string, readonly code?: string, options?: unknown) {
        super(message ?? `voice session mint failed (${status})`, options === undefined ? undefined : { cause: options });
    ...
    get isTransient(): boolean {
        return (this.status === 0 || this.status >= 500 || this.code === "agent_not_found");
    }
}
