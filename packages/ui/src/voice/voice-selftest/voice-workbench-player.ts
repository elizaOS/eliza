...
export interface VoiceWorkbenchOptions {
    ...
    playReply?: (reply: string, turnIndex: number, messageId: string) => Promise<VoicePlaybackEvidenceEvent[]>;
     ...
     */
    resolvePredictedSpeakerLabel?: (turn: WorkbenchTurn, index: number, wav: Uint8Array) => Promise<string | null> | string | null;
...
async function synthesizeReply(opts: VoiceWorkbenchOptions, reply: string): Promise<{
    ok: boolean;
 ...
 */
async function runTurn(opts: VoiceWorkbenchOptions, turn: WorkbenchTurn, index: number, conversationId: string): Promise<VoiceWorkbenchTurnReport> {
    const t0 = now();
    ...
    const speakerLabelOk = !speakerAttributionRan || predictedSpeakerLabel === expectedSpeakerLabel;
    let transcript = "";
    try {
    ...
    try {
        const send = await opts.client.sendConversationMessageStream(conversationId, transcript, () => { }, "VOICE_DM", opts.signal);
    ...
    if (responded) {
        try {
            if (opts.playReply) {
                ...
                const queued = playbackEvidence.filter((event) => event.kind === "queued");
                const started = playbackEvidence.filter((event) => event.kind === "source-started");
                const terminals = playbackEvidence.filter((event) => event.kind === "terminal");
                const selectedBuffer = playbackEvidence.find((event) => event.kind === "decoded" && event.bufferId === started[0]?.bufferId);
                const encoded = playbackEvidence.filter((event) => event.kind === "encoded");
    ...
    if (noResponseReason)
        detail.noResponseReason = noResponseReason;
    if (turn.pausesMs?.length) {
        detail.pausesMs = turn.pausesMs.join(",");
    }
    if (turn.expectedEntity)
        detail.expectedEntity = turn.expectedEntity;
    return {
...
export function scoreWorkbenchDiarization(turns: ReadonlyArray<VoiceWorkbenchTurnReport>, maxDer = 0.2): VoiceWorkbenchReport["diarization"] {
...
export async function runVoiceWorkbench(opts: VoiceWorkbenchOptions): Promise<VoiceWorkbenchReport> {
    const { scenario } = opts;
    const startedAt = new Date().toISOString();
    const base: Omit<VoiceWorkbenchReport, "overall" | "finishedAt" | "turns" | "diarization"> = {
        schemaVersion: 1,
    ...
    const { conversation } = await opts.client.createConversation(`voice-workbench:${scenario.id}`);
    ...
    const allSkipped = turns.length > 0 && turns.every((t) => t.status === "skipped");
