...
import { isLocalInferenceAsrReady } from "../local-asr-transcribe";
...
export type VoiceSelfTestMode = "wav-direct" | "mic-capture" | "inject-transcript";
...
export interface VoiceSelfTestOptions {
    ...
    onArtifact?: (kind: "microphone" | "reference" | "tts", bytes: Uint8Array) => void;
    client: ElizaClient;
...
async function captureMicWav(opts: VoiceSelfTestOptions): Promise<{
    ...
    try {
        await sleep(300);
        const fixture = new Uint8Array(await (await fetch(opts.fixtureUrl, { signal: opts.signal })).arrayBuffer());
        opts.onArtifact?.("reference", fixture);
        const buffer = await opts.audioCtx.decodeAudioData(fixture.slice().buffer);
        const reference = await playThroughDestination(opts.audioCtx, buffer, opts.signal, opts.speakerDeviceId);
 ...
 */
async function playThroughDestination(ctx: AudioContext, buffer: AudioBuffer, signal?: AbortSignal, speakerDeviceId?: string): Promise<{
    started: boolean;
    ...
    try {
        if (ctx.state === "suspended") {
            ...
            if (ctx.state === "suspended") {
                throw new Error("AudioContext remained suspended before playback probe");
            }
        }
        ...
        if (speakerDeviceId) {
            ...
            if (selectable.sinkId !== speakerDeviceId) {
                throw new Error(`Speaker device binding mismatch: requested ${speakerDeviceId}, ` +
        ...
        const ended = new Promise<void>((resolve, reject) => {
            const timeout = window.setTimeout(() => reject(new Error("audio output did not finish")), Math.min(30000, Math.ceil(buffer.duration * 1000) + 2000));
...
export async function runVoiceSelfTest(opts: VoiceSelfTestOptions): Promise<VoiceSelfTestReport> {
    ...
    // ---- Stage ASR: known audio phrase -> transcript ------------------------
    {
        ...
        try {
            if (mode === "inject-transcript") {
                ...
                if (!transcript)
                    throw new Error("injectedTranscript is empty");
                stages.push({
                    stage: "asr",
                    status: "pass",
                ...
                const capture = mode === "mic-capture" ? await captureMicWav(opts) : null;
                const wav = capture?.wav ??
                    new Uint8Array(await (await fetch(opts.fixtureUrl, { signal: opts.signal })).arrayBuffer());
    ...
    if (asrUsable) {
        const t0 = now();
        try {
            const { conversation } = await opts.client.createConversation("voice-selftest");
            let tokenCount = 0;
            const send = await opts.client.sendConversationMessageStream(conversation.id, transcript, () => {
                tokenCount += 1;
            }, "VOICE_DM", opts.signal);
            ...
            sendBackend = send.localInference
                ? `local-inference:${send.localInference.activeModelId ??
    ...
    if (reply.length > 0) {
        ...
        try {
            ...
            const playback = await playThroughDestination(opts.audioCtx, audioBuffer, opts.signal, opts.speakerDeviceId);
