...
} | ClientHelloFrame | ClientAudioMetaFrame | ClientAudioCapabilitiesFrame | ClientBargeInFrame | ClientByeFrame;
...
export type ServerControlFrame = {
    t: "reply_complete";
    traceId: string;
} | ServerReadyEvent | ServerSttPartialEvent | ServerSttEagerEotEvent | ServerSttFinalEvent | ServerLlmFirstTextEvent | ServerProgressEvent | ServerSpeakingStartEvent | ServerSpeakingEndEvent | ServerAssistantPlayingEvent | ServerVoiceMetricEvent | ServerHandoffRequestedEvent | ServerHandoffCompletedEvent | ServerNavigateViewEvent | ServerInterruptedEvent | ServerErrorEvent | ServerUsageEvent;
...
export function parseServerControl(raw: string): ServerControlFrame | null {
    ...
    if (t === "navigate_view") {
        const viewId = readBoundedString((parsed as {
            viewId?: unknown;
        }).viewId);
        const traceId = readBoundedString((parsed as {
            traceId?: unknown;
        }).traceId);
        const rawViewPath = (parsed as {
            viewPath?: unknown;
        }).viewPath;
        const viewPath = rawViewPath === undefined ? null : readBoundedString(rawViewPath);
        const rawSubview = (parsed as {
            subview?: unknown;
        }).subview;
        const subview = rawSubview === undefined ? null : readBoundedString(rawSubview);
        if (!viewId ||
            !traceId ||
...
export function isUsableMintResponse(value: unknown): value is VoiceSessionMintResponse {
 ...
 */
export function negotiateCodec(preferred: VoiceSessionCodec, offered: VoiceSessionCodec[] | undefined): VoiceSessionCodec | null {
