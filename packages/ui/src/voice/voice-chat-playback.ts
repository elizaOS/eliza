...
import { MOUTH_OPEN_STEP } from "./voice-chat-types";
...
export function shouldCacheGeneratedSpeech(_input: string, segment: SpeechSegmentKind): boolean {
 ...
 */
const HIDDEN_VOICE_BLOCK_RE = /<(think|thought|analysis|reasoning|tool_calls?|tools?)\b[^>]*>[\s\S]*?(?:<\/\1>|$)/gi;
...
export function shelterUrls(input: string): {
    ...
    return {
        text,
        restore: (s: string) => s.replace(/__URL(\d+)__/g, (_, i) => urls[Number(i)] ?? _),
    };
}
/**
...
export function isRealSentenceEnd(value: string, matchIndex: number): boolean {
    ...
    const nextChar = matchIndex + 1 < value.length ? value[matchIndex + 1] : undefined;
...
export function splitFirstSentence(text: string): {
    ...
    while (true) {
        ...
        if (punctChar === ".") {
            if (match[1]?.length >= 3)
                continue;
            if (!isRealSentenceEnd(sheltered, match.index))
                continue;
        }
...
export function isCommittedSpeechPrefix(fullText: string, prefix: string): boolean {
...
export function remainderAfter(fullText: string, firstSentence: string): string {
    const full = collapseWhitespace(fullText);
    ...
    throw new ElizaError("The reply changed after speech was queued. Play the completed reply again.", {
