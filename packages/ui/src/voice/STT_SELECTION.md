...
- **macOS/iOS `SFSpeechRecognizer`** — `node packages/app/scripts/voice/stage-b-stt-bench.mjs`
...
- **Android `SpeechRecognizer` (NNAPI)** — needs a port of `stage-b-stt-bench`
  ...
  device lifecycle). Application E2E fixtures do not supply acoustic evidence.
...
- The combined decision cell `stt.stage-b.evaluation` goes green only when
  `packages/app/scripts/voice/voice-stage-b-eval.mjs` validates a reviewed
  ...
  and power telemetry:
  `ELIZA_VOICE_STAGE_B_REPORT=<report.json> node packages/app/scripts/voice/voice-matrix.mjs --run --platform stt.stage-b.evaluation`.
