...
function resolveStoredVoiceConfig(config: Record<string, unknown>): VoiceConfig | null {
    const messages = asRecord(config.messages);
    const tts = asRecord(messages?.tts);
    return tts ? (tts as VoiceConfig) : null;
}
function resolveSelectedCharacterVoiceId(config: Record<string, unknown>, uiLanguage: string): {
    characterId: string;
    voiceId: string;
} | null {
    const ui = asRecord(config.ui);
    const presetId = readString(ui, "presetId");
    const preset = resolveStylePresetById(presetId, uiLanguage) ??
...
function isExplicitElevenLabsChoice(config: VoiceConfig | null): boolean {
    ...
    const hasStoredKeyEvidence = apiKey.toUpperCase() === "[REDACTED]" ||
        apiKey.toUpperCase() === "REDACTED";
    return Boolean(config?.provider === "elevenlabs" &&
        (config.mode === "cloud" ||
            config.mode === "own-key" ||
...
export function resolveCharacterVoiceConfigFromAppConfig(args: {
    ...
    const releaseLegacyProvider = storedVoiceConfig?.provider === "elevenlabs" &&
    ...
    const providerNeutralConfig = releaseLegacyProvider
        ...
        : storedVoiceConfig;
    const currentVoiceId = typeof storedVoiceConfig?.elevenlabs?.voiceId === "string"
        ? storedVoiceConfig.elevenlabs.voiceId.trim()
        : "";
    const legacyVoiceId = resolveLegacyVoiceId(selectedCharacterVoice.characterId);
    const shouldUpdatePresetVoice = selectedCharacterVoice.voiceId !== currentVoiceId &&
        (!currentVoiceId ||
    ...
    return {
        ...
        elevenlabs: {
            ...
            modelId: providerNeutralConfig?.elevenlabs?.modelId ??
 ...
 */
export function applyVoiceProviderDefaults(config: VoiceConfig | null, defaults: DefaultVoiceProviderResult, resolvedTtsProvider?: VoiceConfig["provider"]): VoiceConfig {
    const base = config ?? {};
    return {
        ...base,
