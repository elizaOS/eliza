 ...
 */
import type { AppBlockerSettingsCardProps, StewardPolicyResult, WebsiteBlockerSettingsCardProps } from "@elizaos/shared";
...
export type AppBootStewardTxStatus = "pending" | "approved" | "rejected" | "signed" | "broadcast" | "confirmed" | "failed";
...
export interface StewardApprovalQueueProps {
    ...
    approveStewardTx: (txId: string) => Promise<AppBootStewardApprovalActionResponse>;
    rejectStewardTx: (txId: string, reason?: string) => Promise<AppBootStewardApprovalActionResponse>;
...
export interface AppBootConfig {
    ...
    resolveCompanionInferenceNotice?: (args: ResolveCompanionInferenceNoticeArgs) => CompanionInferenceNotice | null;
...
export declare function syncBrandEnvToEliza(aliases: readonly (readonly [string, string])[]): void;
/** Sync Eliza env vars → brand equivalents. Server-side only. */
export declare function syncElizaEnvToBrand(aliases: readonly (readonly [string, string])[]): void;
