...
export { runAsPrivilegedShell, shellHistory, shellLocalStorage, } from "./surface-realm-channel";
/**
...
export class SurfaceRealmDeniedError extends Error {
    constructor(readonly viewId: string, readonly vector: "storage" | "navigate", readonly detail: string) {
...
export function isShellReservedStorageKey(key: string): boolean {
    return SHELL_RESERVED_STORAGE_PREFIXES.some((prefix) => key.startsWith(prefix));
}
...
function hostScopedStorage(backing: Storage, viewId: string): ScopedStorage {
    const assertWritable = (key: string): void => {
        if (isShellReservedStorageKey(key)) {
            throw new SurfaceRealmDeniedError(viewId, "storage", `reserved shell key "${key}" is not writable by a view`);
        }
    };
    return {
        get length() {
 ...
 */
export function brokerSurfaceStorage(manifest: ResolvedSurfaceManifest, backing: Storage, viewId: string): ScopedStorage {
 ...
 */
export function brokerSurfaceNavigate(manifest: ResolvedSurfaceManifest, viewId: string, navigate: (path: string) => void): (path: string) => void {
    return (path: string) => {
        if (!surfaceGrants(manifest, "navigate")) {
            throw new SurfaceRealmDeniedError(viewId, "navigate", `no "navigate" grant; blocked navigation to "${path}"`);
        }
        navigate(path);
    };
}
...
function isShellOwnedRootVar(name: string): boolean {
    return (SHELL_OWNED_ROOT_VARS.has(name) ||
...
export class SurfaceRealmScope {
    ...
    constructor(readonly manifest: ResolvedSurfaceManifest, readonly viewId: string, backing: Storage, navigate: (path: string) => void, memberViewIds: readonly string[] = [viewId]) {
        ...
        if (typeof document === "undefined") {
            ...
            this.shellRootVarBaseline = new Map(rootVars
...
export function subscribeActiveSurfaceRealmScope(listener: () => void): () => void {
...
export function setActiveSurfaceRealmScope(scope: SurfaceRealmScope | null): void {
...
function assertRawStorageWriteAllowed(op: string, key: string): void {
    ...
    throw new SurfaceRealmDeniedError(scope.viewId, "storage", `raw localStorage ${op} on reserved shell key "${key}" — views use the ` +
...
function guardedLocalStorage(backing: Storage): Storage {
    ...
    return new Proxy(backing, {
        get(target, prop) {
            if (prop === STORAGE_GUARD_TARGET)
                return target;
            if (prop === "setItem")
                return guardedSetItem;
            if (prop === "removeItem")
                return guardedRemoveItem;
            if (prop === "clear")
                return guardedClear;
...
function assertRawHistoryMutationAllowed(op: "pushState" | "replaceState", url: string | URL | null | undefined): void {
    if (isPrivilegedShellActive())
        return;
    const scope = activeScope;
    if (scope === null)
    ...
    if (url === undefined || url === null || url === "")
        return;
    if (surfaceGrants(scope.manifest, "navigate"))
        return;
    let next: URL;
    try {
    ...
    throw new SurfaceRealmDeniedError(scope.viewId, "navigate", `raw history.${op} to "${next.pathname}" without the "navigate" grant — ` +
        `views use scope.navigate; shell code uses shellHistory`);
}
let historyGuardInstalled = false;
function installHistoryGuard(): void {
    if (historyGuardInstalled || typeof History === "undefined")
        return;
    ...
    History.prototype.pushState = function pushState(data: unknown, unused: string, url?: string | URL | null): void {
        assertRawHistoryMutationAllowed("pushState", url);
        rawPushState.call(this, data, unused, url);
    };
    History.prototype.replaceState = function replaceState(data: unknown, unused: string, url?: string | URL | null): void {
...
export function ensureHostRealmGuards(): void {
    ...
    try {
        ...
        if (current?.[STORAGE_GUARD_TARGET] !== undefined)
            return;
        ...
        logger.warn({ error }, "[SurfaceRealmBroker] raw localStorage guard unavailable; facade enforcement only");
    }
}
