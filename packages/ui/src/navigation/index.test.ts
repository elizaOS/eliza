...
import { developerShellUrl } from "./developer-route";
import {
  ALL_TAB_GROUPS,
  BUILTIN_ROUTE_IDS,
...
describe.each(["/dev", "/dev2"])(
  "local developer shell routing at %s",
  (route) => {
    const location = {
      protocol: "http:",
      hostname: "localhost",
    ...
    it("starts at chat and reads inner app routes without leaving the developer shell", () => {
      ...
      expect(
        isDeveloperWorkspaceRoute({ ...location, pathname: `${route}/` }),
      ).toBe(true);
    });
    it.each([
      "/chat",
    ...
    it.each(["127.0.0.1", "localhost", "[::1]"])(
      "recognizes the developer tab on loopback host %s",
      (hostname) => {
        expect(isDeveloperWorkspaceRoute({ ...location, hostname })).toBe(true);
      },
    );
    it("keeps app navigation in the current developer route and retains both query strings", () => {
      vi.stubGlobal("window", {
        location: new URL(`http://localhost${route}?reader=1#/chat`),
      });
      expect(developerShellUrl("/notes?folder=work#note-1")).toBe(
        `${route}?reader=1#/notes?folder=work#note-1`,
      );
      expect(developerShellUrl("https://eliza.example/notes")).toBe(
        "https://eliza.example/notes",
      );
      expect(developerShellUrl(`${route}#/calendar`)).toBe(
        `${route}#/calendar`,
      );
      expect(developerShellUrl(null)).toBeNull();
    });
  },
);
