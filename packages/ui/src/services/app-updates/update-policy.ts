...
import { type AgentUpdateAuthority } from "@elizaos/core/contracts/update-status";
import { type AgentUpdateStatus } from "@elizaos/core/contracts/update-status";
...
export function resolveAppUpdatePolicy(input: AppUpdatePolicyInput): AppUpdatePolicy {
    if (input.platform === "desktop") {
        if (input.buildVariant === "store") {
            return {
                ...
                detail: "This build must receive application updates through its desktop store.",
                actionLabel: null,
            };
        }
        return {
    ...
    if (input.platform === "ios") {
        if (input.buildVariant === "store") {
            return {
                ...
                detail: "iOS App Store builds cannot download executable app updates outside the App Store.",
                actionLabel: null,
            };
        }
        return {
            ...
            detail: "This iOS build can point to GitHub releases, but installing a new binary still goes through the sideloading toolchain.",
            actionLabel: null,
        };
    }
    if (input.platform === "android") {
        if (input.elizaOS) {
            return {
                ...
                detail: "AOSP system builds update with the device image or privileged package channel.",
                actionLabel: null,
            };
        }
        if (input.buildVariant === "store") {
            return {
                ...
                detail: "Google Play builds cannot self-update or download executable code outside Play.",
                actionLabel: null,
            };
        }
        return {
            ...
            detail: "This sideload build can link to GitHub APK releases, but Android installation still requires user-controlled package install consent.",
            actionLabel: null,
        };
    }
    return {
...
function resolveAgentAuthority(installMethod: string): Pick<ConnectedAgentUpdateSnapshot, "authority" | "authorityLabel" | "detail"> {
    switch (installMethod) {
        case "npm-global":
            return {
                authority: "npm",
                authorityLabel: "npm global",
                detail: "The connected agent is updated with npm on the host running the agent.",
            };
        case "bun-global":
            return {
                authority: "bun",
                authorityLabel: "Bun global",
                detail: "The connected agent is updated with Bun on the host running the agent.",
            };
        case "homebrew":
            return {
                authority: "homebrew",
                authorityLabel: "Homebrew",
                detail: "The connected agent is managed by Homebrew on the host running the agent.",
            };
        case "snap":
            return {
                authority: "snap",
                authorityLabel: "Snap",
                detail: "The connected agent is managed by Snap on the host running the agent.",
            };
        case "apt":
            return {
                authority: "apt",
                authorityLabel: "Debian apt",
                detail: "The connected agent is managed by the Debian package manager on the host.",
            };
        case "flatpak":
            return {
                authority: "flatpak",
                authorityLabel: "Flatpak",
                detail: "The connected agent is managed by Flatpak on the host running the agent.",
            };
        case "local-dev":
            return {
                authority: "local-dev",
                authorityLabel: "Local development checkout",
                detail: "The connected agent is a local development checkout and should be updated with workspace tooling.",
            };
        default:
            return {
                authority: "unknown",
...
export function mapAgentUpdateStatusToSnapshot(status: AgentUpdateStatus | null | undefined): ConnectedAgentUpdateSnapshot | null {
    if (!status)
        return null;
...
export async function readNativeAppInfo(): Promise<NativeAppInfo | null> {
    ...
    try {
        ...
        /* @vite-ignore */ capacitorAppPackage)) as CapacitorAppModule;
