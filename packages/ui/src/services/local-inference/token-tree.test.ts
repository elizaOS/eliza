...
describe("buildTokenTreeDescriptor", () => {
	...
	it("sorts leaves by name for byte-stable output", () => {
		...
		const d1 = buildTokenTreeDescriptor("action", ["ZULU", "ALPHA", "MIKE"], map);
		const d2 = buildTokenTreeDescriptor("action", ["MIKE", "ALPHA", "ZULU"], map);
...
describe("TokenizerClient", () => {
	function makeStubFetch(
		...
		const stub = vi.fn(async (url: RequestInfo | URL, init?: RequestInit) => {
			calls.count += 1;
	...
	it("throws a useful error when /tokenize returns a non-2xx", async () => {
		const stub = vi.fn(async () => new Response("nope", { status: 500 })) as
			unknown as typeof fetch;
		const client = new TokenizerClient({ fetch: stub });
		await expect(
			client.tokenize("model-a", "http://x", "FOO"),
