...
describe("local inference recommendations", () => {
  ...
  it("still recommends Eliza-1 text on a binary that advertises BOTH DFlash kernels AND OpenVINO", () => {
    // Intel hosts ship binaries that link both the DFlash/W4-B kernels
    // (for text) and the OpenVINO backend (for ASR/Whisper). The
    // `unsupportedKernels: ["openvino"]` annotation on text tiers must
    // NOT reject these — DFlash is present and is the right route. The
    // dispatcher steers OpenVINO off at spawn time.
    const probe = hardware({
      totalRamGb: 64,
      freeRamGb: 48,
      gpu: { backend: "cuda", totalVramGb: 24, freeVramGb: 22 },
