...
describe("OmiFrameReassembler", () => {
  it("does not lock frame-id mode on a same-id retransmit of the previous payload", () => {
    const r = new OmiFrameReassembler();
    const results = pushAll(r, [
      // Sequence-contract frame start.
      notif(10, 0, [1, 2, 3]),
      // Link-layer retransmit: same frame id, bumped chunk index, identical
      // payload. Must be ignored as a duplicate, not proof of frame-id mode.
      notif(10, 1, [1, 2, 3]),
      // Genuine sequence-contract continuation and next frame boundary.
      notif(11, 1, [4, 5, 6]),
      notif(12, 0, [7, 8, 9]),
    ]);
