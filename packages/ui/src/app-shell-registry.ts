 ...
 */
import { getAllOverlayApps } from "@elizaos/ui/apps/overlay-app-registry";
...
export function registerAppShellPage(registration: AppShellPageRegistration): void {
...
export function appShellPageMatchesPath(page: AppShellPageRegistration, navigationPath: string): boolean {
...
export function appShellPageIsAvailable(page: AppShellPageRegistration, runtime: {
...
function requireStableAgentViewId(value: string, kind: RegisteredAgentSurfaceKind, ownerId: string): string {
    const viewId = value.trim();
    if (viewId.length === 0) {
        throw new Error(`Registered ${kind} surface "${ownerId}" resolved an empty agent view id`);
    }
    return viewId;
}
...
export function appShellAgentSurfaceDescriptor(page: AppShellPageRegistration): RegisteredAgentSurfaceDescriptor {
    return {
        kind: "app-shell",
        viewId: requireStableAgentViewId(page.agentViewId ?? page.id, "app-shell", page.pluginId),
        ownerId: page.pluginId,
        path: page.path,
    };
}
/** The bridge descriptor generated for one overlay-app registration. */
export function overlayAgentSurfaceDescriptor(app: OverlayApp): RegisteredAgentSurfaceDescriptor {
    const viewId = requireStableAgentViewId(overlayAgentViewId(app.name), "overlay", app.name);
    return {
        kind: "overlay",
        viewId,
        ownerId: app.name,
 ...
 */
export function buildRegisteredAgentSurfaceInventory(appShellPages: readonly AppShellPageRegistration[], overlayApps: readonly OverlayApp[]): RegisteredAgentSurfaceDescriptor[] {
    const descriptors = [
    ...
    for (const descriptor of descriptors) {
        ...
        if (existing) {
            throw new Error(`Agent surface view id "${descriptor.viewId}" is registered by both ` +
                `${existing.kind}:${existing.ownerId} and ${descriptor.kind}:${descriptor.ownerId}`);
        }
        ownersByViewId.set(descriptor.viewId, descriptor);
    }
    return descriptors.sort((left, right) => left.viewId.localeCompare(right.viewId));
}
...
export function listRegisteredAgentSurfaceInventory(): RegisteredAgentSurfaceDescriptor[] {
    return buildRegisteredAgentSurfaceInventory(listAppShellPages(), getAllOverlayApps());
}
/**
 ...
 */
export function requireRegisteredAgentSurface(expected: RegisteredAgentSurfaceDescriptor): RegisteredAgentSurfaceDescriptor {
    const descriptor = listRegisteredAgentSurfaceInventory().find((candidate) => candidate.kind === expected.kind &&
        candidate.viewId === expected.viewId &&
        candidate.ownerId === expected.ownerId);
    if (!descriptor) {
        throw new Error(`Agent surface ${expected.kind}:${expected.viewId} is not present in the live registry inventory`);
    }
    return descriptor;
}
/**
...
function getHostExternalImporterStore(): Map<string, HostExternalImporter> {
    ...
    const existing = globalObject[registryKey] as Map<string, HostExternalImporter> | undefined;
    if (existing)
        return existing;
 ...
 */
export function registerHostExternalImporter(specifier: string, importer: HostExternalImporter): void {
    getHostExternalImporterStore().set(specifier, importer);
}
/** Resolve a registered host-external importer, or `undefined` if none. */
export function resolveRegisteredHostExternalImporter(specifier: string): HostExternalImporter | undefined {
