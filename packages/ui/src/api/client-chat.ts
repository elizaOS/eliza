...
import { isRemoteRelayRestAdapterBase } from "./client-base";
import { type AccountConnectRequest } from "./client-types";
import { type ApiError } from "./client-types";
import { type ChatActionResultSummary } from "./client-types";
import { type ChatFailureKind } from "./client-types";
import { type ChatTerminalFailure } from "./client-types";
import { type ChatTokenUsage } from "./client-types";
import { type ChatToolCallEvent } from "./client-types";
import { type ChatTurnStatus } from "./client-types";
import { type ConnectionTestResult } from "./client-types";
import { type ContentBlock } from "./client-types";
import { type ContextInspectorResponse } from "./client-types";
import { type Conversation } from "./client-types";
import { type ConversationChannelType } from "./client-types";
import { type ConversationGreeting } from "./client-types";
import { type ConversationMessage } from "./client-types";
import { type ConversationMessageSearchResponse } from "./client-types";
import { type ConversationMetadata } from "./client-types";
import { type CreateConversationOptions } from "./client-types";
import { type DatabaseConfigResponse } from "./client-types";
import { type DatabaseProviderType } from "@elizaos/core/contracts/config";
import { type DatabaseStatus } from "./client-types";
import { type DocumentBulkUploadResult } from "./client-types";
import { type DocumentDetail } from "./client-types";
import { type DocumentFacetCountsResponse } from "./client-types";
import { type DocumentFragmentsResponse } from "./client-types";
import { type DocumentScope } from "./client-types";
import { type DocumentSearchResponse } from "./client-types";
import { type DocumentStats } from "./client-types";
import { type DocumentUpdateResult } from "./client-types";
import { type DocumentUploadResult } from "./client-types";
import { type DocumentsResponse } from "./client-types";
import { type ImageAttachment } from "./client-types";
import { type LocalInferenceChatMetadata } from "./client-types";
import { type McpMarketplaceResult } from "./client-types";
import { type McpRegistryServerDetail } from "./client-types";
import { type McpServerConfig } from "./client-types";
import { type McpServerStatus } from "./client-types";
import { type MemoryBrowseQuery } from "./client-types";
import { type MemoryBrowseResponse } from "./client-types";
import { type MemoryFeedQuery } from "./client-types";
import { type MemoryFeedResponse } from "./client-types";
import { type MemoryRememberResponse } from "./client-types";
import { type MemorySearchResponse } from "./client-types";
import { type MemoryStatsResponse } from "./client-types";
import { type PostInboxMessageRequest } from "@elizaos/core/contracts/inbox-routes";
import { type PostWorkbenchVfsPromoteToCloudRequest } from "./client-types";
import { type PromoteVfsToCloudContainerRequest } from "./client-types";
import { type PromoteVfsToCloudContainerResponse } from "./client-types";
import { type QueryResult } from "./client-types";
import { type QuickContextResponse } from "./client-types";
import { type RequestCodingAgentContainerRequest } from "./client-types";
import { type RequestCodingAgentContainerResponse } from "./client-types";
import { type ShareIngestItem } from "./client-types";
import { type ShareIngestPayload } from "./client-types";
import { type SyncCloudCodingContainerRequest } from "./client-types";
import { type SyncCloudCodingContainerResponse } from "./client-types";
import { type TableInfo } from "./client-types";
import { type TableRowsResponse } from "./client-types";
import { type TrajectoryConfig } from "./client-types";
import { type TrajectoryDetailResult } from "./client-types";
import { type TrajectoryExportOptions } from "./client-types";
import { type TrajectoryListOptions } from "./client-types";
import { type TrajectoryListResult } from "./client-types";
import { type TrajectoryStats } from "./client-types";
import { type WorkbenchLoadedVfsPlugin } from "./client-types";
import { type WorkbenchOverview } from "./client-types";
import { type WorkbenchTask } from "./client-types";
import { type WorkbenchTodo } from "./client-types";
import { type WorkbenchVfsCompileResult } from "./client-types";
import { type WorkbenchVfsDiffEntry } from "./client-types";
import { type WorkbenchVfsEntry } from "./client-types";
import { type WorkbenchVfsProject } from "./client-types";
import { type WorkbenchVfsQuota } from "./client-types";
import { type WorkbenchVfsSnapshot } from "./client-types";
...
function setPositiveNumberParam(params: URLSearchParams, key: string, value: number | undefined): void {
    if (typeof value === "number" && value > 0)
        params.set(key, String(value));
}
function setTruthyNumberParam(params: URLSearchParams, key: string, value: number | undefined): void {
    if (value)
        params.set(key, String(value));
}
function setDefinedNumberParam(params: URLSearchParams, key: string, value: number | undefined): void {
    if (value !== undefined)
        params.set(key, String(value));
}
function setNonEmptyStringParam(params: URLSearchParams, key: string, value: string | undefined): void {
    if (typeof value === "string" && value.length > 0)
        params.set(key, value);
}
function setTruthyStringParam(params: URLSearchParams, key: string, value: string | undefined): void {
    if (value)
        params.set(key, value);
}
function appendTagsParam(params: URLSearchParams, tags: string[] | undefined): void {
...
function buildInboxMessagesParams(options: InboxMessagesOptions | undefined): URLSearchParams {
...
function buildInboxMessagesRpcParams(options: InboxMessagesOptions | undefined): InboxMessagesOptions {
...
function buildInboxChatsRpcParams(options: InboxChatsOptions | undefined): InboxChatsOptions {
    return options?.sources && options.sources.length > 0
        ? { sources: options.sources }
        : {};
}
function appendDocumentFilterParams(params: URLSearchParams, options: DocumentListOptions | DocumentSearchOptions | undefined): void {
...
function buildDocumentListParams(options: DocumentListOptions | undefined): URLSearchParams {
    const params = new URLSearchParams();
...
function buildDocumentSearchParams(query: string, options: DocumentSearchOptions | undefined): URLSearchParams {
...
function buildTrajectoryParams(options: TrajectoryListOptions | undefined): URLSearchParams {
    const params = new URLSearchParams();
    setTruthyNumberParam(params, "limit", options?.limit);
    setTruthyNumberParam(params, "offset", options?.offset);
    setTruthyStringParam(params, "roomId", options?.roomId);
...
declare module "./client-base" {
    interface ElizaClient {
        sendChatRest(text: string, channelType?: ConversationChannelType): Promise<{
            text: string;
            agentName: string;
        ...
        sendChatStream(text: string, onToken: (token: string, accumulatedText?: string) => void, channelType?: ConversationChannelType, signal?: AbortSignal): Promise<{
            text: string;
            agentName: string;
            completed: boolean;
            noResponseReason?: "ignored";
        ...
        getConversationMessages(id: string, options?: {
            signal?: AbortSignal;
            /**
         ...
         */
        searchConversationMessages(query: string, options?: {
            limit?: number;
            offset?: number;
            since?: number;
            until?: number;
         ...
         */
        deleteConversationMessage(id: string, messageId: string): Promise<{
            ok: boolean;
            deletedCount: number;
        }>;
        retryConversationReply(id: string, messageId: string): Promise<{
            text: string;
            agentName: string;
            messageId: string;
        ...
        sendConversationMessage(id: string, text: string, channelType?: ConversationChannelType, images?: ImageAttachment[], metadata?: Record<string, unknown>): Promise<{
            text: string;
            agentName: string;
            interrupted?: boolean;
        ...
        sendConversationMessageStream(id: string, text: string, onToken: (token: string, accumulatedText?: string, 
        /** True for in-flight action-callback text the final reply may
         *  replace — voice output must hold it until the turn completes. */
        provisional?: boolean) => void, channelType?: ConversationChannelType, signal?: AbortSignal, images?: ImageAttachment[], metadata?: Record<string, unknown>, 
        ...
        onReplyReady?: (actionResults: ChatActionResultSummary[]) => void): Promise<{
            text: string;
            agentName: string;
            completed: boolean;
            interrupted?: boolean;
        ...
        abortConversationTurn(roomId: string, reason?: string): Promise<{
            aborted: boolean;
            roomId: string;
            reason: string;
        }>;
        ...
        renameConversation(id: string, title: string, options?: {
            generate?: boolean;
        }): Promise<{
            conversation: Conversation;
        }>;
        ...
        getDocumentFacetCounts(options?: DocumentListOptions): Promise<DocumentFacetCountsResponse>;
        ...
        updateDocumentPins(documentId: string, data: {
            agent: boolean;
            roomIds: string[];
        ...
        updateDocumentAccess(documentId: string, data: {
            directGrantEntityIds: string[];
        ...
        uploadDocumentFromUrl(url: string, options?: DocumentUrlUploadOptions): Promise<DocumentUploadResult>;
        searchDocuments(query: string, options?: DocumentSearchOptions): Promise<DocumentSearchResponse>;
        getDocumentFragments(documentId: string): Promise<DocumentFragmentsResponse>;
        ...
        getMemoriesByEntity(entityId: string, query?: MemoryBrowseQuery): Promise<MemoryBrowseResponse>;
        ...
        listWorkbenchVfsFiles(projectId: string, options?: {
            path?: string;
            recursive?: boolean;
        }): Promise<{
            files: WorkbenchVfsEntry[];
        }>;
        readWorkbenchVfsFile(projectId: string, path: string, options?: {
        ...
        writeWorkbenchVfsFile(projectId: string, data: {
            path: string;
            content: string;
            encoding?: "utf-8" | "base64";
        }): Promise<{
        ...
        deleteWorkbenchVfsFile(projectId: string, path: string): Promise<{
            ok: boolean;
        }>;
        listWorkbenchVfsSnapshots(projectId: string): Promise<{
            snapshots: WorkbenchVfsSnapshot[];
        }>;
        createWorkbenchVfsSnapshot(projectId: string, data?: {
            note?: string;
        }): Promise<{
            snapshot: WorkbenchVfsSnapshot;
        }>;
        getWorkbenchVfsDiff(projectId: string, snapshotId: string): Promise<{
            diff: WorkbenchVfsDiffEntry[];
        }>;
        rollbackWorkbenchVfs(projectId: string, snapshotId: string): Promise<{
            rollback: unknown;
        }>;
        compileWorkbenchVfsPlugin(projectId: string, data: {
            entry: string;
            outFile?: string;
            format?: "esm" | "cjs";
            target?: string;
        ...
        loadWorkbenchVfsPlugin(projectId: string, data: {
            entry: string;
            outFile?: string;
        ...
        unloadWorkbenchVfsPlugin(projectId: string, pluginName: string): Promise<{
            pluginName: string;
            unloaded: boolean;
        }>;
        promoteWorkbenchVfsToCloud(projectId: string, data?: PostWorkbenchVfsPromoteToCloudRequest): Promise<PromoteVfsToCloudContainerResponse>;
        promoteVfsToCloudContainer(data: PromoteVfsToCloudContainerRequest): Promise<PromoteVfsToCloudContainerResponse>;
        requestCloudCodingContainer(data: RequestCodingAgentContainerRequest): Promise<RequestCodingAgentContainerResponse>;
        syncCloudCodingContainerChanges(containerId: string, data: SyncCloudCodingContainerRequest): Promise<SyncCloudCodingContainerResponse>;
        refreshRegistry(): Promise<void>;
        getTrajectories(options?: TrajectoryListOptions, init?: RequestInit): Promise<TrajectoryListResult>;
        getTrajectoryDetail(trajectoryId: string, options?: RequestInit & {
            includePayloads?: boolean;
        }): Promise<TrajectoryDetailResult>;
        getTrajectoryStats(): Promise<TrajectoryStats>;
        getContextInspector(conversationId: string, options?: {
            offset?: number;
            limit?: number;
...
function writeLegacyChatConversationId(client: ElizaClient, conversationId: string | null): void {
    if (typeof window === "undefined") {
        return;
    }
...
async function ensureLegacyChatConversationId(client: ElizaClient): Promise<string> {
...
ElizaClient.prototype.sendChatRest = async function (this: ElizaClient, text, channelType = "DM") {
    const sendToConversation = async (conversationId: string) => this.sendConversationMessage(conversationId, text, channelType, undefined);
...
ElizaClient.prototype.sendChatMessage = function (this: ElizaClient, text, channelType = "DM") {
...
ElizaClient.prototype.sendChatStream = async function (this: ElizaClient, text, onToken, channelType = "DM", signal?) {
    const streamConversation = async (conversationId: string) => this.sendConversationMessageStream(conversationId, text, onToken, channelType, signal, undefined);
    const conversationId = await ensureLegacyChatConversationId(this);
    try {
...
function withConversationListDefaults<T extends {
...
async function invokeLocalDesktopChatRpc<T>(baseUrl: string, options: {
    rpcMethod: string;
    ipcChannel: string;
    params?: unknown;
}): Promise<T | null> {
    if (!isDesktopLocalApiBaseUrl(baseUrl) ||
...
ElizaClient.prototype.listConversations = async function (this: ElizaClient, options) {
    ...
    return withConversationListDefaults(await this.fetch<{
        conversations: Conversation[];
    }>("/api/conversations", {
        signal: options?.signal,
    }));
};
ElizaClient.prototype.createConversation = async function (this: ElizaClient, title?, options?) {
...
ElizaClient.prototype.getConversationMessages = async function (this: ElizaClient, id, options) {
    let response: {
        messages: ConversationMessage[];
        hasMore?: boolean;
    } | null = null;
    ...
    response ??= await this.fetch<{
        messages: ConversationMessage[];
        hasMore?: boolean;
    }>(`/api/conversations/${encodeURIComponent(id)}/messages${query}`, options?.signal ? { signal: options.signal } : undefined);
    return {
...
ElizaClient.prototype.searchConversationMessages = async function (this: ElizaClient, query, options) {
    const params = new URLSearchParams({ q: query });
    if (options?.limit !== undefined)
        params.set("limit", String(options.limit));
    ...
    return this.fetch<ConversationMessageSearchResponse>(`/api/conversations/messages/search?${params}`, options?.signal ? { signal: options.signal } : undefined);
};
ElizaClient.prototype.getInboxMessages = async function (this: ElizaClient, options) {
...
ElizaClient.prototype.getInboxChats = async function (this: ElizaClient, options) {
    const params = buildSourcesParams(options?.sources);
    const query = params.toString();
...
ElizaClient.prototype.setInboxChatMute = async function (this: ElizaClient, data) {
    return this.fetch<{
        ok: boolean;
        roomId: string;
        action: "mute" | "unmute";
...
ElizaClient.prototype.sendInboxMessage = async function (this: ElizaClient, data) {
    return this.fetch<{
        ok: boolean;
        message?: ConversationMessage & {
            roomId: string;
...
ElizaClient.prototype.truncateConversationMessages = async function (this: ElizaClient, id, messageId, options?) {
...
ElizaClient.prototype.deleteConversationMessage = async function (this: ElizaClient, id, messageId) {
    return this.fetch(`/api/conversations/${encodeURIComponent(id)}/messages/${encodeURIComponent(messageId)}`, { method: "DELETE" });
};
ElizaClient.prototype.retryConversationReply = async function (this: ElizaClient, id, messageId) {
    return this.fetch(`/api/conversations/${encodeURIComponent(id)}/messages/${encodeURIComponent(messageId)}/retry-reply`, { method: "POST", body: JSON.stringify({}) });
};
ElizaClient.prototype.sendConversationMessage = async function (this: ElizaClient, id, text, channelType = "DM", images?, metadata?) {
    const response = await this.fetch<{
        text: string;
        agentName: string;
        interrupted?: boolean;
...
ElizaClient.prototype.sendConversationMessageStream = async function (this: ElizaClient, id, text, onToken, channelType = "DM", signal?, images?, metadata?, onStatus?, onToolEvent?, clientMessageId?, onReplyReady?) {
    return this.streamChatEndpoint(`/api/conversations/${encodeURIComponent(id)}/messages/stream`, text, onToken, channelType, signal, images, metadata, onStatus, onToolEvent, clientMessageId, onReplyReady);
};
ElizaClient.prototype.abortConversationTurn = async function (this: ElizaClient, roomId, reason = "ui-abort") {
...
ElizaClient.prototype.requestGreeting = async function (this: ElizaClient, id, lang?) {
...
ElizaClient.prototype.renameConversation = async function (this: ElizaClient, id, title, options?) {
    return this.updateConversation(id, {
        title,
        generate: options?.generate,
    });
};
ElizaClient.prototype.updateConversation = async function (this: ElizaClient, id, data) {
...
ElizaClient.prototype.deleteConversation = async function (this: ElizaClient, id) {
    return this.fetch(`/api/conversations/${encodeURIComponent(id)}`, {
        method: "DELETE",
    });
};
ElizaClient.prototype.cleanupEmptyConversations = async function (this: ElizaClient, options?) {
...
ElizaClient.prototype.listDocuments = async function (this: ElizaClient, options?) {
    const params = buildDocumentListParams(options);
    const query = params.toString();
    return this.fetch(`/api/documents${query ? `?${query}` : ""}`);
};
ElizaClient.prototype.getDocumentFacetCounts = async function (this: ElizaClient, options?) {
...
ElizaClient.prototype.getDocument = async function (this: ElizaClient, documentId) {
    return this.fetch(`/api/documents/${encodeURIComponent(documentId)}`);
};
ElizaClient.prototype.getDocumentAccess = async function (this: ElizaClient, documentId) {
    return this.fetch(`/api/documents/${encodeURIComponent(documentId)}/access`);
};
ElizaClient.prototype.getDocumentPins = async function (this: ElizaClient, documentId) {
    return this.fetch(`/api/documents/${encodeURIComponent(documentId)}/pins`);
};
ElizaClient.prototype.updateDocumentPins = async function (this: ElizaClient, documentId, data) {
    return this.fetch(`/api/documents/${encodeURIComponent(documentId)}/pins`, {
        method: "PATCH",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.updateDocumentAccess = async function (this: ElizaClient, documentId, data) {
    return this.fetch(`/api/documents/${encodeURIComponent(documentId)}/access`, {
        method: "PATCH",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.updateDocument = async function (this: ElizaClient, documentId, data) {
    return this.fetch(`/api/documents/${encodeURIComponent(documentId)}`, {
        method: "PATCH",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.deleteDocument = async function (this: ElizaClient, documentId) {
    return this.fetch(`/api/documents/${encodeURIComponent(documentId)}`, {
        method: "DELETE",
    });
};
ElizaClient.prototype.uploadDocument = async function (this: ElizaClient, data) {
    return this.fetch("/api/documents", {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.uploadDocumentsBulk = async function (this: ElizaClient, data) {
    return this.fetch("/api/documents/bulk", {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.uploadDocumentFromUrl = async function (this: ElizaClient, url, options?) {
    const metadata = {
        ...(options?.metadata ?? {}),
...
ElizaClient.prototype.searchDocuments = async function (this: ElizaClient, query, options?) {
    const params = buildDocumentSearchParams(query, options);
    return this.fetch(`/api/documents/search?${params}`);
};
ElizaClient.prototype.getDocumentFragments = async function (this: ElizaClient, documentId) {
    return this.fetch(`/api/documents/${encodeURIComponent(documentId)}/fragments`);
};
ElizaClient.prototype.rememberMemory = async function (this: ElizaClient, text) {
    return this.fetch("/api/memory/remember", {
        method: "POST",
        body: JSON.stringify({ text }),
    });
};
ElizaClient.prototype.searchMemory = async function (this: ElizaClient, query, options?) {
    const params = new URLSearchParams({ q: query });
    if (options?.limit !== undefined)
        params.set("limit", String(options.limit));
    return this.fetch(`/api/memory/search?${params}`);
};
ElizaClient.prototype.quickContext = async function (this: ElizaClient, query, options?) {
    const params = new URLSearchParams({ q: query });
    if (options?.limit !== undefined)
        params.set("limit", String(options.limit));
    return this.fetch(`/api/context/quick?${params}`);
};
ElizaClient.prototype.getMemoryFeed = async function (this: ElizaClient, query?) {
    const params = new URLSearchParams();
    if (query?.type)
...
ElizaClient.prototype.browseMemories = async function (this: ElizaClient, query?) {
    const params = new URLSearchParams();
    if (query?.type)
        params.set("type", query.type);
    if (query?.entityId)
...
ElizaClient.prototype.getMemoriesByEntity = async function (this: ElizaClient, entityId, query?) {
    const params = new URLSearchParams();
    if (query?.type)
        params.set("type", query.type);
    if (typeof query?.limit === "number")
...
ElizaClient.prototype.searchMcpMarketplace = async function (this: ElizaClient, query, limit) {
    const params = new URLSearchParams({ q: query, limit: String(limit) });
    return this.fetch(`/api/mcp/marketplace/search?${params}`);
};
ElizaClient.prototype.getMcpServerDetails = async function (this: ElizaClient, name) {
    return this.fetch(`/api/mcp/marketplace/${encodeURIComponent(name)}`);
};
ElizaClient.prototype.addMcpServer = async function (this: ElizaClient, name, config) {
    await this.fetch("/api/mcp/servers", {
        method: "POST",
        body: JSON.stringify({ name, config }),
    });
};
ElizaClient.prototype.removeMcpServer = async function (this: ElizaClient, name) {
    await this.fetch(`/api/mcp/servers/${encodeURIComponent(name)}`, {
        method: "DELETE",
    });
};
ElizaClient.prototype.ingestShare = async function (this: ElizaClient, payload) {
...
ElizaClient.prototype.getWorkbenchOverview = async function (this: ElizaClient) {
...
ElizaClient.prototype.getWorkbenchTask = async function (this: ElizaClient, taskId) {
    return this.fetch(`/api/workbench/tasks/${encodeURIComponent(taskId)}`);
};
ElizaClient.prototype.createWorkbenchTask = async function (this: ElizaClient, data) {
    return this.fetch("/api/workbench/tasks", {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.updateWorkbenchTask = async function (this: ElizaClient, taskId, data) {
    return this.fetch(`/api/workbench/tasks/${encodeURIComponent(taskId)}`, {
        method: "PUT",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.deleteWorkbenchTask = async function (this: ElizaClient, taskId) {
    return this.fetch(`/api/workbench/tasks/${encodeURIComponent(taskId)}`, {
        method: "DELETE",
    });
};
...
ElizaClient.prototype.getWorkbenchTodo = async function (this: ElizaClient, todoId) {
    return this.fetch(`/api/workbench/todos/${encodeURIComponent(todoId)}`);
};
ElizaClient.prototype.createWorkbenchTodo = async function (this: ElizaClient, data) {
    return this.fetch("/api/workbench/todos", {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.updateWorkbenchTodo = async function (this: ElizaClient, todoId, data) {
    return this.fetch(`/api/workbench/todos/${encodeURIComponent(todoId)}`, {
        method: "PUT",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.setWorkbenchTodoCompleted = async function (this: ElizaClient, todoId, isCompleted) {
    await this.fetch(`/api/workbench/todos/${encodeURIComponent(todoId)}/complete`, {
        method: "POST",
        body: JSON.stringify({ isCompleted }),
    });
};
ElizaClient.prototype.deleteWorkbenchTodo = async function (this: ElizaClient, todoId) {
    return this.fetch(`/api/workbench/todos/${encodeURIComponent(todoId)}`, {
        method: "DELETE",
    });
};
ElizaClient.prototype.createWorkbenchVfsProject = async function (this: ElizaClient, projectId) {
    return this.fetch("/api/workbench/vfs/projects", {
        method: "POST",
        body: JSON.stringify({ projectId }),
    });
};
ElizaClient.prototype.getWorkbenchVfsQuota = async function (this: ElizaClient, projectId) {
    return this.fetch(`/api/workbench/vfs/projects/${encodeURIComponent(projectId)}/quota`);
};
ElizaClient.prototype.listWorkbenchVfsFiles = async function (this: ElizaClient, projectId, options = {}) {
    const params = new URLSearchParams();
    if (options.path)
...
ElizaClient.prototype.readWorkbenchVfsFile = async function (this: ElizaClient, projectId, path, options = {}) {
...
ElizaClient.prototype.writeWorkbenchVfsFile = async function (this: ElizaClient, projectId, data) {
    return this.fetch(`/api/workbench/vfs/projects/${encodeURIComponent(projectId)}/file`, {
        method: "PUT",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.deleteWorkbenchVfsFile = async function (this: ElizaClient, projectId, path) {
    const params = new URLSearchParams({ path });
    return this.fetch(`/api/workbench/vfs/projects/${encodeURIComponent(projectId)}/file?${params.toString()}`, { method: "DELETE" });
};
ElizaClient.prototype.listWorkbenchVfsSnapshots = async function (this: ElizaClient, projectId) {
    return this.fetch(`/api/workbench/vfs/projects/${encodeURIComponent(projectId)}/snapshots`);
};
ElizaClient.prototype.createWorkbenchVfsSnapshot = async function (this: ElizaClient, projectId, data = {}) {
    return this.fetch(`/api/workbench/vfs/projects/${encodeURIComponent(projectId)}/snapshots`, {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.getWorkbenchVfsDiff = async function (this: ElizaClient, projectId, snapshotId) {
...
ElizaClient.prototype.rollbackWorkbenchVfs = async function (this: ElizaClient, projectId, snapshotId) {
    return this.fetch(`/api/workbench/vfs/projects/${encodeURIComponent(projectId)}/rollback`, {
        method: "POST",
        body: JSON.stringify({ snapshotId }),
    });
};
ElizaClient.prototype.compileWorkbenchVfsPlugin = async function (this: ElizaClient, projectId, data) {
    return this.fetch(`/api/workbench/vfs/projects/${encodeURIComponent(projectId)}/compile-plugin`, {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.loadWorkbenchVfsPlugin = async function (this: ElizaClient, projectId, data) {
    return this.fetch(`/api/workbench/vfs/projects/${encodeURIComponent(projectId)}/load-plugin`, {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.listWorkbenchVfsPlugins = async function (this: ElizaClient) {
    return this.fetch("/api/workbench/vfs/plugins");
};
ElizaClient.prototype.unloadWorkbenchVfsPlugin = async function (this: ElizaClient, projectId, pluginName) {
...
ElizaClient.prototype.promoteWorkbenchVfsToCloud = async function (this: ElizaClient, projectId, data = {}) {
    return this.fetch(`/api/workbench/vfs/projects/${encodeURIComponent(projectId)}/promote-to-cloud`, {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.promoteVfsToCloudContainer = async function (this: ElizaClient, data) {
    return this.fetch("/api/cloud/coding-containers/promotions", {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.requestCloudCodingContainer = async function (this: ElizaClient, data) {
    return this.fetch("/api/cloud/coding-containers", {
        method: "POST",
        body: JSON.stringify(data),
    });
};
ElizaClient.prototype.syncCloudCodingContainerChanges = async function (this: ElizaClient, containerId, data) {
...
ElizaClient.prototype.getTrajectories = async function (this: ElizaClient, options?, init?) {
    const params = buildTrajectoryParams(options);
    const query = params.toString();
    return this.fetch(`/api/trajectories${query ? `?${query}` : ""}`, init);
};
ElizaClient.prototype.getTrajectoryDetail = async function (this: ElizaClient, trajectoryId, options?) {
    const { includePayloads = true, ...init } = options ?? {};
    return this.fetch(`/api/trajectories/${encodeURIComponent(trajectoryId)}${includePayloads ? "" : "?includePayloads=0"}`, init);
};
ElizaClient.prototype.getTrajectoryStats = async function (this: ElizaClient) {
    return this.fetch("/api/trajectories/stats");
};
ElizaClient.prototype.getContextInspector = async function (this: ElizaClient, conversationId, options = {}) {
...
ElizaClient.prototype.updateTrajectoryConfig = async function (this: ElizaClient, config) {
    return this.fetch("/api/trajectories/config", {
        method: "PUT",
        body: JSON.stringify(config),
    });
};
ElizaClient.prototype.exportTrajectories = async function (this: ElizaClient, options) {
...
ElizaClient.prototype.deleteTrajectories = async function (this: ElizaClient, trajectoryIds) {
    return this.fetch("/api/trajectories", {
        method: "DELETE",
        body: JSON.stringify({ trajectoryIds }),
    });
};
ElizaClient.prototype.clearAllTrajectories = async function (this: ElizaClient) {
    return this.fetch("/api/trajectories", {
        method: "DELETE",
...
ElizaClient.prototype.saveDatabaseConfig = async function (this: ElizaClient, config) {
    return this.fetch("/api/database/config", {
        method: "PUT",
        body: JSON.stringify(config),
    });
};
ElizaClient.prototype.testDatabaseConnection = async function (this: ElizaClient, creds) {
...
ElizaClient.prototype.getDatabaseRows = async function (this: ElizaClient, table, opts?) {
    const params = new URLSearchParams();
    if (opts?.offset != null)
...
ElizaClient.prototype.insertDatabaseRow = async function (this: ElizaClient, table, data) {
    return this.fetch(`/api/database/tables/${encodeURIComponent(table)}/rows`, {
        method: "POST",
        body: JSON.stringify({ data }),
    });
};
ElizaClient.prototype.updateDatabaseRow = async function (this: ElizaClient, table, where, data) {
    return this.fetch(`/api/database/tables/${encodeURIComponent(table)}/rows`, {
        method: "PUT",
        body: JSON.stringify({ where, data }),
    });
};
ElizaClient.prototype.deleteDatabaseRow = async function (this: ElizaClient, table, where) {
    return this.fetch(`/api/database/tables/${encodeURIComponent(table)}/rows`, {
        method: "DELETE",
        body: JSON.stringify({ where }),
    });
};
ElizaClient.prototype.executeDatabaseQuery = async function (this: ElizaClient, sql, readOnly = true) {
