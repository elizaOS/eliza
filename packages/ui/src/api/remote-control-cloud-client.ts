...
export interface RemotePairingClaimReceipt {
    ...
    grantExpiresAt: string;
    capabilities: string[];
    host: Pick<RemoteHostSummary, "id" | "deviceId" | "displayName" | "platform" | "runtimeKeyId" | "signingPublicKeyJwk" | "encryptionPublicKeyJwk" | "createdAt">;
}
export type RemoteRelayCommandStatus = "pending" | "claimed" | "started" | "completed" | "failed" | "expired" | "cancelled" | "execution_ambiguous";
...
function exactEnum<const T extends string>(value: unknown, field: string, values: readonly T[]): T {
...
function parseHost(value: unknown): RemoteHostSummary {
    ...
    if (!item)
        throw new Error("Cloud response contains an invalid remote host.");
    const platform = requiredString(item.platform, "host platform");
    if (!["macos", "windows", "linux", "ios", "android", "web"].includes(platform)) {
    ...
    return {
        ...
        encryptionPublicKeyJwk: publicJwk(item.encryptionPublicKeyJwk, "encryption key"),
...
function parseSession(value: unknown, expectedHostId: string, expectedOwnerId?: string): RemoteSessionSummary {
    const item = record(value);
    if (!item) {
        throw new Error("Cloud response contains an invalid remote session.");
    }
    const hostId = uuid(item.hostId, "host id");
    const targetRuntimeId = uuid(item.targetRuntimeId ?? item.hostId, "target runtime id");
    ...
    return {
        ...
        controllerDeviceId: identifier(item.controllerDeviceId, "controller device id"),
...
export class RemoteControlCloudClient {
    ...
    private readonly requestImpl: (url: string, init: RequestInit) => Promise<Response>;
    constructor(options: RemoteControlCloudClientOptions) {
        ...
        if (!this.authToken)
            throw new RemoteControlAuthenticationRequiredError();
        this.requestImpl =
            options.request ??
                (async (url, init) => {
                    const transport = desktopHttpTransportForUrl(url) ?? fetchAgentTransport;
    ...
    private async request<T>(path: string, init: RequestInit = {}): Promise<T> {
        ...
        if (response.status !== 204) {
            try {
                ...
                throw new RemoteCloudRequestError("Eliza Cloud returned an unreadable response. Try again.", response.status);
            }
        }
        const envelope = record(payload);
        if (!response.ok || envelope?.success === false) {
            const cloudMessage = typeof envelope?.error === "string" &&
                envelope.error.length <= 256 &&
                !/[\r\n]/.test(envelope.error)
                ? envelope.error
                : "The remote device request failed. Try again.";
            throw new RemoteCloudRequestError(cloudMessage, response.status, typeof envelope?.code === "string" ? envelope.code : undefined);
        }
    ...
    async listSessions(hostId: string, ownerId?: string): Promise<RemoteSessionSummary[]> {
        ...
        return data.sessions.map((session) => parseSession(session, expectedHostId, expectedOwnerId));
    }
    async createPairing(input: {
        ...
        const data = await this.request<Record<string, unknown>>("/api/v1/remote/pair", {
            method: "POST",
            cache: "no-store",
            body: JSON.stringify({
                hostId: input.hostId,
        ...
        if (ownerId !== input.controller.ownerId ||
            ...
            throw new Error("Cloud pairing authority does not match the requested host.");
        }
        const expiresAt = isoDate(data.expiresAt, "pairing expiration");
        const derivedTtlSeconds = Math.ceil((Date.parse(expiresAt) - Date.now()) / 1000);
        const ttlSeconds = data.ttlSeconds === undefined
            ? derivedTtlSeconds
    ...
    async claimPairing(input: {
        ...
        if (!host)
            throw new Error("Cloud response is missing the target Mac.");
        const ownerId = uuid(data.ownerId, "owner id");
        ...
        if (!Array.isArray(capabilities) ||
            ...
            capabilities.some((capability, index) => capability !== REMOTE_TARGET_PAIRING_CAPABILITIES[index])) {
        ...
        return {
            ...
            host: {
                ...
                encryptionPublicKeyJwk: publicJwk(host.encryptionPublicKeyJwk, "encryption key"),
    ...
    async revokeSession(sessionId: string): Promise<void> {
        ...
        for (let page = 0; page < MAX_REVOCATION_CLEANUP_PAGES; page += 1) {
            ...
            const commands = nonNegativeInteger(cleanup.commands, "session cleanup command count");
            const more = requiredBoolean(cleanup.more, "session cleanup continuation state");
            if (!more)
                return;
            if (commands === 0) {
                throw new Error("The session is revoked, but Cloud cleanup made no progress. Try revoking again.");
            }
        }
        throw new Error("The session is revoked, but Cloud cleanup exceeded the safe continuation limit. Try revoking again.");
    }
    async revokeHost(hostId: string): Promise<void> {
        ...
        for (let page = 0; page < MAX_REVOCATION_CLEANUP_PAGES; page += 1) {
            ...
            const sessions = nonNegativeInteger(cleanup.sessions, "host cleanup session count");
            const commands = nonNegativeInteger(cleanup.commands, "host cleanup command count");
            const more = requiredBoolean(cleanup.more, "host cleanup continuation state");
            if (!more)
                return;
            if (sessions === 0 && commands === 0) {
                throw new Error("The host is revoked, but Cloud cleanup made no progress. Try revoking again.");
            }
        }
        throw new Error("The host is revoked, but Cloud cleanup exceeded the safe continuation limit. Try revoking again.");
    }
    async enqueueCommand(input: {
        sessionId: string;
        envelope: EncryptedRemoteControlEnvelope;
    }): Promise<void> {
        await this.request(`/api/v1/remote/sessions/${encodeURIComponent(input.sessionId)}/commands`, { method: "POST", body: JSON.stringify({ envelope: input.envelope }) });
    }
    async readCommand(input: {
        ...
        const data = await this.request<Record<string, unknown>>(`/api/v1/remote/sessions/${encodeURIComponent(input.sessionId)}/commands/${encodeURIComponent(input.commandId)}`);
        ...
        const envelope = (value: unknown, kind: "start_receipt" | "result"): EncryptedRemoteControlEnvelope | null => {
            if (value === null || value === undefined)
                return null;
            if (!isEncryptedRemoteControlEnvelope(value) ||
                value.messageKind !== kind ||
                value.sessionId !== input.sessionId ||
                value.commandId !== input.commandId) {
                throw new Error(`Cloud response contains an invalid ${kind.replace("_", " ")} envelope.`);
            }
            return value;
        };
        return {
