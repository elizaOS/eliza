...
function isNativeCloudHttpsUrl(url: string): boolean {
    const parsed = parseUrl(url);
    if (!parsed || !Capacitor.isNativePlatform())
        return false;
    if (parsed.protocol !== "https:")
        return false;
    const host = parsed.hostname.toLowerCase();
...
function isNativeTrustedRemoteAgentUrl(url: string): boolean {
    ...
    if (!parsed || !Capacitor.isNativePlatform())
        return false;
    try {
        if (globalThis.localStorage?.getItem("eliza:mobile-runtime-mode") !==
            "remote-mac") {
            return false;
        }
    }
...
type NativeWebFetch = (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>;
/**
...
function nativeRequestData(body: BodyInit | null | undefined, headers: HeadersInit | undefined): unknown {
...
const nativeCloudHttpTransport: AgentRequestTransport = {
    async request(url, init, context) {
        ...
        const useBinaryBody = wantsBinary &&
            result.status >= 200 &&
            result.status < 300 &&
            typeof result.data === "string";
        return new Response(useBinaryBody ? responseBytes(result.data) : responseBody(result.data), {
            status: result.status,
            headers: result.headers,
        });
    },
};
export function nativeCloudHttpTransportForUrl(url: string): AgentRequestTransport | null {
    ...
    if (isNativeCloudHttpsUrl(url))
        return nativeCloudHttpTransport;
    if (isNativeTrustedRemoteAgentUrl(url))
        return nativeCloudHttpTransport;
    return null;
}
