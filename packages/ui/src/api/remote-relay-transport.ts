 ...
 */
import { acknowledgeRemoteCommandEnqueue } from "../platform/remote-controller";
import { bodyToString } from "./transport";
import { createRemoteCommand } from "../platform/remote-controller";
...
import { openRemoteCommandResult } from "../platform/remote-controller";
import { openRemoteCommandStartReceipt } from "../platform/remote-controller";
import { parseRemoteAgentRequest } from "@elizaos/core/contracts/remote-agent-request";
import { type AgentProfile } from "../state/agent-profile-types";
import { type AgentRequestTransport } from "./transport";
import { type RemoteAgentRequest } from "@elizaos/core/contracts/remote-agent-request";
...
type RemoteRelayCloudClient = Pick<RemoteControlCloudClient, "enqueueCommand" | "readCommand">;
...
function relayProfileForUrl(url: string): AgentProfile | null {
    ...
    if (parsed.protocol !== "eliza-remote:" ||
        parsed.host !== "session" ||
        parsed.username ||
        parsed.password ||
        parsed.search ||
    ...
    return (loadAgentProfileRegistry().profiles.find((profile) => profile.connectionMode === "relay" &&
        profile.remoteRelay?.sessionId === sessionId) ?? null);
}
async function withSessionEnqueue<T>(sessionId: string, operation: () => Promise<T>): Promise<T> {
...
async function sendCommand(cloud: RemoteRelayCloudClient, profile: AgentProfile, action: RemoteCommandAction, payload: RemoteJsonValue, signal?: AbortSignal, dependencies: RemoteRelayCommandDependencies = remoteRelayCommandDependencies): Promise<RemoteJsonValue | undefined> {
    throwIfAborted(signal);
    const relay = profile.remoteRelay;
    if (!relay)
        throw new Error("Remote relay authority is missing.");
    ...
    if (!Number.isSafeInteger(target.createdAt) || target.createdAt <= 0) {
        throw new Error("This remote profile is missing trusted host metadata. Remove it and pair again.");
    }
    const created = await withSessionEnqueue(relay.sessionId, async () => {
        ...
        for (;;) {
            ...
            if (!acknowledged) {
                throw new Error("The remote enqueue acknowledgement was not persisted.");
            }
            if (!next.recoveredPending)
                return next;
        }
    });
    ...
    while (dependencies.now() <= resultDeadline) {
        ...
        if (current.status === "started" && !verifiedStartReceipt) {
            throw new Error("The remote runtime reported a start without a verifiable receipt. The command was not retried.");
        }
        if (current.status === "execution_ambiguous") {
            throw new Error("The remote runtime started this command, but its final result is unknown. It was not retried.");
        }
    ...
    throw new Error(crossedStartBoundary
        ? "The remote command may still be running. It was not retried."
        : "The remote command timed out before starting.");
}
export function remoteRelayTransportForUrl(url: string, cloudFactory: () => RemoteControlCloudClient | Promise<RemoteControlCloudClient> = defaultCloudClient): AgentRequestTransport | null {
    const profile = relayProfileForUrl(url);
    if (!profile)
        return null;
    return {
        async request(requestUrl, init) {
            const request = normalizeRelayAgentRequest(requestUrl, init, profile.remoteRelay?.sessionId);
            ...
            const result = await sendCommand(await cloudFactory(), profile, "agent.request", payload, init.signal ?? undefined);
            return responseFromRemoteResult(result);
        },
    };
}
function normalizeRelayAgentRequest(requestUrl: string, init: RequestInit, expectedSessionId?: string): RemoteAgentRequest {
...
function responseFromRemoteResult(result: RemoteJsonValue | undefined): Response {
    const response = result && typeof result === "object" && !Array.isArray(result)
        ? (result as Record<string, RemoteJsonValue>)
        : {};
    const validStatus = typeof response.status === "number" &&
        Number.isInteger(response.status) &&
        response.status >= 100 &&
        response.status <= 599;
    const remoteHeaders = response.headers &&
        typeof response.headers === "object" &&
        !Array.isArray(response.headers)
        ? (response.headers as Record<string, RemoteJsonValue>)
        : {};
    const contentType = remoteHeaders["content-type"];
    const contentTypeHasControl = typeof contentType === "string" &&
        Array.from(contentType).some((character) => {
            const code = character.charCodeAt(0);
            return code <= 0x1f || code === 0x7f;
        });
    const headers = typeof contentType === "string" &&
        contentType.length <= 256 &&
        !contentTypeHasControl
        ? { "content-type": contentType }
        : { "content-type": "application/json" };
    return new Response(validStatus && typeof response.body === "string" ? response.body : "", {
