 ...
 */
import { AGENT_MODEL_SLOTS } from "@elizaos/core/contracts/local-inference";
import { COINGECKO_MARKET_PROVIDER } from "@elizaos/ui/wallet/market-overview";
import { MODEL_CATALOG } from "@elizaos/plugin-native-inference/model-catalog/catalog";
import { POLYMARKET_MARKET_PROVIDER } from "@elizaos/ui/wallet/market-overview";
import { asRecord } from "@elizaos/core/type-guards";
import { assessCatalogModelFit } from "../services/local-inference/recommendation";
import { buildCoinGeckoMarketsUrl } from "@elizaos/ui/wallet/market-overview";
import { buildMarketMovers } from "@elizaos/ui/wallet/market-overview";
import { buildMarketPriceSnapshots } from "@elizaos/ui/wallet/market-overview";
import { buildMobileLoadOptions } from "./ios-local-agent-mobile-policy";
import { catalogDownloadSizeGb } from "../services/local-inference/recommendation";
import { chooseSmallerFallbackModel } from "../services/local-inference/recommendation";
import { createTimeoutSignal } from "./timeout-signal";
import { fallbackMobileTotalRamGb } from "./ios-local-agent-mobile-policy";
...
import { gpuBackendForMobile } from "./ios-local-agent-mobile-policy";
...
import { mobileRecommendedBucket } from "./ios-local-agent-mobile-policy";
import { normalizeMobilePlatform } from "./ios-local-agent-mobile-policy";
import { parseCanonicalInteger } from "@elizaos/core/utils/number-parsing";
import { parseCoinGeckoMarkets } from "@elizaos/ui/wallet/market-overview";
import { positiveFiniteNumber } from "./ios-local-agent-mobile-policy";
...
import { summarizeTranscript } from "@elizaos/core/transcripts";
import { transcriptDurationMs } from "@elizaos/core/transcripts";
import { transcriptSpeakerCount } from "@elizaos/core/transcripts";
import { type ActiveModelState } from "@elizaos/core/contracts/local-inference";
...
import { type DownloadJob } from "@elizaos/core/contracts/local-inference";
...
import { type ModelAssignments } from "@elizaos/core/contracts/local-inference";
import { type ProviderStatus } from "@elizaos/core/contracts/local-inference-providers";
import { type RoutingPreferences } from "@elizaos/plugin-native-inference/model-catalog/routing-policy";
...
import { type TranscriptSource } from "@elizaos/core/transcripts";
...
const DEFAULT_SYSTEM_PROMPT = "You are Eliza, a private on-device assistant. Answer directly and concisely.";
...
const WALLET_MARKET_OVERVIEW_CACHE_TTL_MS = 120000;
const WALLET_MARKET_OVERVIEW_FETCH_TIMEOUT_MS = 8000;
...
export const CLOUD_BRIDGE_REQUEST_TIMEOUT_MS = 60000;
...
export const IOS_BUNDLE_MANIFEST_TIMEOUT_MS = 30000;
const EMPTY_ROUTING_PREFERENCES: RoutingPreferences = {
    preferredProvider: {},
    policy: {},
};
const IOS_LOCAL_BACKGROUND_UNAVAILABLE_REASON = "iOS local mode uses the WebView ITTP route kernel. Capacitor BackgroundRunner wakes in a separate JSContext and cannot call that WebView kernel while the app is suspended.";
...
type CapacitorLlamaLoadOptions = Parameters<NonNullable<CapacitorLlamaAdapter["load"]>>[0];
type LlamaCppModule = {
...
export function resolveIosLocalCloudApiBase(activeApiBase?: string | null, options: {
    ...
    const pageHostname = options.pageHostname ??
        (typeof window !== "undefined"
            ...
            : "");
    const bootCloudApiBase = options.bootCloudApiBase ?? getBootConfig().cloudApiBase;
    return resolveDirectCloudAuthApiBase(resolveCloudEnvironmentBase({
        pageHostname,
...
function readIosCloudPairing(): IosCloudPairing {
    const activeServer = readJson<Record<string, unknown> | null>(ACTIVE_SERVER_STORAGE_KEY, null);
    const apiBase = resolveIosLocalCloudApiBase(stringValue(activeServer?.apiBase));
...
function isIosVoiceAssetPath(rawPath: string): boolean {
    ...
    if (!/\.(bin|codec|gguf|json)$/i.test(path))
        return false;
    if (/(^|\/)(asr|tts|vad|voice|voices|wakeword)\//.test(path))
        return true;
    return /(^|\/)cache\/[^/]*voice[^/]*\.(bin|codec|gguf|json)$/i.test(path);
}
function inferVoiceReadiness(installed: InstalledModel[], bundles: Record<string, IosBundleRecord>): LocalVoiceReadinessSnapshot {
    const installedVoicePaths = new Set<string>();
    for (const model of installed) {
        if (isIosVoiceAssetPath(model.path))
            installedVoicePaths.add(model.path);
    }
    ...
    for (const record of Object.values(bundles)) {
        ...
        if (paths.length > 0) {
            ...
            for (const path of paths)
                installedVoicePaths.add(path);
        }
    }
    const modelId = [...bundleIdsWithVoice][0] ??
    ...
    return {
        ...
        installedFiles,
        modelId,
        message: "Eliza-1 voice assets are installed. Voice engine will warm on first playback.",
    };
}
function sanitizeLocalSpeechText(input: string): string {
    let text = input.normalize("NFKC");
    text = text.replace(/<think\b[^>]*>[\s\S]*?(?:<\/think>|$)/gi, " ");
    text = text.replace(/<(analysis|reasoning|tool_calls?|tools?)\b[^>]*>[\s\S]*?(?:<\/\1>|$)/gi, " ");
...
function buildLocalTranscript(body: Record<string, unknown>, id: string, now: number): Transcript {
    ...
    return {
        id,
        title: stringFromUnknown(body.title)?.trim() ||
...
async function handleLocalTranscriptsRoute(request: Request, method: string, pathname: string, url: URL): Promise<Response | null> {
    ...
    if (method === "POST" && pathname === "/api/transcripts") {
        ...
        const transcript = buildLocalTranscript(body, randomId("transcript"), Date.now());
    ...
    if (!id || id.includes("/"))
        return null;
    if (method === "GET") {
        const record = readTranscriptStore().records.find((r) => r.transcript.id === id);
        if (!record)
...
function handleLocalMemoriesRoute(method: string, pathname: string, url: URL): Response | null {
    if (method !== "GET")
        return null;
    if (pathname === "/api/memories/feed") {
        const limit = Math.min(Math.max(positiveIntegerParam(url.searchParams.get("limit"), MEMORY_FEED_DEFAULT_LIMIT), 1), MEMORY_FEED_MAX_LIMIT);
        const before = parseCanonicalInteger(url.searchParams.get("before"));
        if (before === "invalid") {
            return json({ error: "before must be a Unix timestamp in milliseconds" }, 400);
        }
        ...
        if (beforeIdParam !== null &&
            ...
            return json({ error: "beforeId must be a non-empty ID paired with before" }, 400);
        }
    ...
    if (pathname === "/api/memories/browse") {
        const limit = Math.min(Math.max(positiveIntegerParam(url.searchParams.get("limit"), MEMORY_BROWSE_DEFAULT_LIMIT), 1), MEMORY_BROWSE_MAX_LIMIT);
        ...
        if (searchQuery) {
            items = items.filter((item) => localMemoryMatchesKeyword(item.text, searchQuery));
        }
        return json({
    ...
    if (pathname.startsWith("/api/memories/by-entity/")) {
        const entityId = decodePathSegment(pathname.slice("/api/memories/by-entity/".length));
        if (entityId === null)
            return json({ error: "malformed URL encoding" }, 400);
        if (!entityId)
            return json({ error: "Missing entity identifier." }, 400);
        const limit = Math.min(Math.max(positiveIntegerParam(url.searchParams.get("limit"), MEMORY_BROWSE_DEFAULT_LIMIT), 1), MEMORY_BROWSE_MAX_LIMIT);
        const offset = positiveIntegerParam(url.searchParams.get("offset"), 0);
...
function normalizeBrowserWorkspaceKind(value: unknown): "internal" | "standard" | undefined {
...
async function openBrowserWorkspaceTab(request: Request): Promise<Response> {
    ...
    const tab: LocalBrowserWorkspaceTab = {
        ...
        partition: typeof body.partition === "string" && body.partition.trim()
            ? body.partition.trim()
...
async function handleBrowserWorkspaceTabRoute(request: Request, pathname: string): Promise<Response | null> {
    const method = request.method.toUpperCase();
    const match = pathname.match(/^\/api\/browser-workspace\/tabs\/([^/]+)(?:\/(navigate|show|hide|snapshot))?$/);
    if (!match)
        return null;
    ...
    if (action === "show" && method === "POST") {
        const now = nowIso();
        store.tabs = store.tabs.map((tab) => tab.id === tabId
    ...
    if (action === "navigate" && method === "POST") {
        ...
        store.tabs = store.tabs.map((tab) => tab.id === tabId ? store.tabs[index] : { ...tab, visible: false });
        writeBrowserWorkspaceStore(store);
...
function unavailableLocalBackendRoute(error: string, details: Record<string, unknown> = {}, status = 503): Response {
    return json({
        ok: false,
        error,
        mode: "ios-local",
...
function isWalletMarketOverview(value: unknown): value is Record<string, unknown> {
    const record = asRecord(value);
...
function readCachedWalletMarketOverview(): CachedWalletMarketOverview | null {
    const parsed = readJson<CachedWalletMarketOverview | null>(WALLET_MARKET_OVERVIEW_KEY, null);
    if (!parsed ||
...
function writeCachedWalletMarketOverview(response: Record<string, unknown>): void {
    const cacheTtlSeconds = typeof response.cacheTtlSeconds === "number" && response.cacheTtlSeconds > 0
...
function staleWalletMarketOverview(response: Record<string, unknown>): Record<string, unknown> {
...
function resolveCloudWalletMarketOverviewUrl(): string {
    ...
    const cloudApiBase: string = typeof rawBase === "string"
        ? rawBase
...
async function fetchCoinGeckoWalletMarketOverview(): Promise<Record<string, unknown>> {
...
let walletMarketOverviewInFlight: Promise<Record<string, unknown>> | null = null;
async function refreshWalletMarketOverview(): Promise<Record<string, unknown>> {
    if (!walletMarketOverviewInFlight) {
        walletMarketOverviewInFlight = (async () => {
            const cloudPayload = await fetchJsonWithTimeout(resolveCloudWalletMarketOverviewUrl()).catch(() => fetchCoinGeckoWalletMarketOverview());
...
function emptyWalletMarketOverview(error = "Market data is unavailable in local iOS mode."): Record<string, unknown> {
    const unavailable = (provider: typeof COINGECKO_MARKET_PROVIDER | typeof POLYMARKET_MARKET_PROVIDER) => ({
        ...provider,
        available: false,
        stale: false,
        error,
...
async function localWalletMarketOverview(): Promise<Record<string, unknown>> {
    ...
    try {
        ...
        return emptyWalletMarketOverview(error instanceof Error ? error.message : "Market data is unavailable.");
    }
}
function emptyWalletTradingProfile(url: URL): Record<string, unknown> {
    const windowParam = url.searchParams.get("window");
    const sourceParam = url.searchParams.get("source");
    const selectedWindow = windowParam === "24h" ||
        windowParam === "7d" ||
        windowParam === "30d" ||
        windowParam === "all"
        ? windowParam
        : "30d";
    const selectedSource = sourceParam === "agent" || sourceParam === "manual" || sourceParam === "all"
        ? sourceParam
        : "all";
    return {
...
async function loadCapacitorLlama(): Promise<CapacitorLlamaAdapter | null> {
    llamaAdapterPromise ??= (async () => {
        try {
            ...
            /* @vite-ignore */ packageName)) as CapacitorLlamaModule | null;
...
function buildHuggingFaceResolveUrlForPath(model: CatalogModel, filePath: string): string {
...
function publicLocalModelDownloadError(error: unknown): string | null {
    try {
        if (!(error instanceof PublicLocalModelDownloadError))
            return null;
...
function normalizeNativeHashResult(result: Awaited<ReturnType<NonNullable<LlamaCppModule["hashFile"]>>>): {
    sha256: string;
    sizeBytes?: number;
} {
    if (typeof result === "string") {
        ...
        if (!sha256)
            throw new PublicLocalModelDownloadError("Native hashFile returned an invalid SHA256");
        return { sha256 };
    }
    const sha256 = normalizedSha256(result.sha256 ?? result.hash);
    if (!sha256)
        throw new PublicLocalModelDownloadError("Native hashFile returned an invalid SHA256");
    const sizeBytes = typeof result.sizeBytes === "number"
        ? result.sizeBytes
...
async function hashNativeBundleFile(llama: LlamaCppModule, filePath: string, label: string): Promise<{
    sha256: string;
    sizeBytes?: number;
}> {
    if (!llama.hashFile) {
        throw new PublicLocalModelDownloadError(`Native Eliza-1 downloader cannot verify SHA256 for ${label}; refusing bundle install.`);
    }
    return normalizeNativeHashResult(await llama.hashFile(filePath));
}
async function verifyNativeBundleFile(llama: LlamaCppModule, filePath: string, expectedSha256: string, label: string): Promise<{
    sha256: string;
    sizeBytes?: number;
}> {
    const hashed = await hashNativeBundleFile(llama, filePath, label);
    if (hashed.sha256 !== expectedSha256) {
        throw new PublicLocalModelDownloadError(`SHA256 mismatch for ${label}: expected ${expectedSha256}, got ${hashed.sha256}`);
    }
    return hashed;
}
...
function parseIosBundleManifest(input: unknown, model: CatalogModel): IosBundleManifest {
    if (!input || typeof input !== "object") {
        throw new PublicLocalModelDownloadError("Invalid Eliza-1 manifest: expected object");
    }
    const raw = input as Partial<IosBundleManifest>;
    if (raw.id !== model.id) {
        throw new PublicLocalModelDownloadError(`Invalid Eliza-1 manifest id for ${model.id}`);
    }
    if (raw.defaultEligible !== true || typeof raw.version !== "string") {
        throw new PublicLocalModelDownloadError("Invalid Eliza-1 manifest metadata");
    }
    ...
    for (const kind of [
        ...
        if (!Array.isArray(raw.files[kind])) {
            throw new PublicLocalModelDownloadError(`Invalid Eliza-1 manifest files.${kind}`);
        }
    }
    for (const kind of ["text", "voice", "asr", "cache", "vad"] as const) {
        if (raw.files[kind].length === 0) {
            throw new PublicLocalModelDownloadError(`Invalid Eliza-1 manifest files.${kind} must be non-empty`);
        }
    }
    if (!raw.files.text.some((entry) => entry.path === model.ggufFile)) {
        throw new PublicLocalModelDownloadError(`Eliza-1 manifest missing text file ${model.ggufFile}`);
    }
    return raw as IosBundleManifest;
}
function collectIosBundleFiles(manifest: IosBundleManifest): IosBundleFileEntry[] {
...
function _companionInstalled(installed: InstalledModel[], modelId: string): InstalledModel | undefined {
...
async function hardwareProbe(): Promise<HardwareProbe> {
    ...
    const totalRamGb = positiveFiniteNumber(hardware?.totalRamGb) ??
        fallbackMobileTotalRamGb(platform);
    const availableRamGb = positiveFiniteNumber(hardware?.availableRamGb) ?? totalRamGb;
...
async function validateMobileModelFit(model: CatalogModel): Promise<string | null> {
    ...
    if (typeof freeStorageGb === "number" && freeStorageGb > 0) {
        ...
        if (requiredGb > freeStorageGb * 0.9) {
            return `Not enough free storage for ${model.displayName}: needs about ${requiredGb.toFixed(1)} GB including companions, ${freeStorageGb.toFixed(1)} GB available.`;
        }
    }
    return null;
}
...
type LocalReply = {
    ...
    localInference?: {
        intent?: "retry" | "resume" | "redownload" | "download" | "cancel" | "switch_smaller" | "status";
        status: "missing" | "downloading" | "loading" | "failed" | "no_space" | "idle" | "ready" | "cancelled";
        modelId?: string | null;
...
async function localModelStatusReply(text: string): Promise<LocalReply | null> {
    ...
    if (intent === "switch_smaller") {
        const smaller = chooseSmallerFallbackModel(model.id, hardware, "TEXT_LARGE", MODEL_CATALOG);
        if (smaller)
            model = smaller;
    ...
    const state = activeState.status === "loading"
        ? "loading"
 ...
 */
type IosLocalGenerateFallbackReason = "local-unavailable" | "local-overloaded" | "local-error" | "local-aborted-pre-completion";
...
const CLOUD_PAIRED_PROBE_TIMEOUT_MS = 2000;
type FetchWithOptionalPreconnect = typeof fetch & {
    preconnect?: (...args: unknown[]) => unknown;
};
const fetchWithOptionalPreconnect = fetch as FetchWithOptionalPreconnect;
const fetchIosKernelRoute = Object.assign(async function fetchIosKernelRoute(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    const request = input instanceof Request ? input : new Request(input, init);
    return handleIosLocalAgentRequest(request);
}, {
    preconnect: (...args: unknown[]) => fetchWithOptionalPreconnect.preconnect?.(...args),
}) as typeof fetch;
async function probeAgentCloudPaired(fetchImpl: typeof fetch = fetchIosKernelRoute): Promise<CloudPairedProbe> {
    const controller = new AbortController();
    const timer = globalThis.setTimeout(() => controller.abort(), CLOUD_PAIRED_PROBE_TIMEOUT_MS);
    ...
    const cloudProvisioned = (body as {
...
async function sendPromptToIosCloud(prompt: string): Promise<CloudForwardResult> {
    ...
    const { signal: cloudBridgeSignal, dispose: disposeCloudBridgeSignal } = createTimeoutSignal(CLOUD_BRIDGE_REQUEST_TIMEOUT_MS);
    try {
        const response = await fetch(`${pairing.apiBase}/api/v1/eliza/agents/${encodeURIComponent(pairing.agentId)}/bridge`, {
            method: "POST",
            headers: {
        ...
        const body = asRecord(await response.json().catch((err: unknown) => {
        ...
        if (error) {
            throw new Error(stringValue(error.message) ?? "Cloud bridge returned an error.");
        }
...
async function handleIosCloudChat(request: Request): Promise<Response> {
    ...
    try {
        ...
        logger.error({ error, diagnostic: formatError(error) }, "[ios-local-agent] cloud chat request failed");
        return json({
            error: notPaired
 ...
 */
async function forwardToAgentCloudChat(prompt: string, fetchImpl: typeof fetch = fetchIosKernelRoute): Promise<CloudForwardResult> {
    ...
    if (!response.ok) {
        throw new Error(`[ios-local-agent] Cloud fallback failed: HTTP ${response.status} ${response.statusText}`);
    }
    ...
    if (typeof textValue !== "string") {
        throw new Error("[ios-local-agent] Cloud fallback: response missing string 'text'");
    }
...
async function generateLocalReply(conversation: LocalConversation, text: string): Promise<LocalReply> {
    ...
    if (!llama?.generate) {
        ...
        const err = new Error("Native Eliza-1 runtime is not available on this build.");
    ...
    try {
        ...
        if (result.finishReason === "length") {
            throw new Error("[ios-local-agent] local generation ended at its decode boundary before completion");
        }
        ...
        if (probe.kind === "paired") {
            try {
                ...
                const honest = new Error(`Local model failed (${cls.reason}) and the paired cloud fallback also failed: ${cloudErr instanceof Error ? cloudErr.message : String(cloudErr)}`);
        ...
        // do NOT swallow.
        const honest = new Error("Local model failed and cloud is not paired. Pair Eliza Cloud or download a smaller model.");
...
async function generateLocalGreeting(): Promise<LocalReply> {
    const setupReply = await localModelStatusReply("download the default local model");
...
function aggregateProgress(jobs: DownloadJob[]): {
    ...
    const received = jobs.reduce((sum, job) => sum + Math.max(0, job.received), 0);
    const total = jobs.reduce((sum, job) => sum + Math.max(0, job.total), 0);
    const bytesPerSec = jobs.reduce((sum, job) => sum + Math.max(0, job.bytesPerSec), 0);
    ...
    const etaMs = total > received && bytesPerSec > 0
...
function classifyLocalModelIntent(text: string): "redownload" | "download" | "cancel" | "switch_smaller" | "status" | null {
    const normalized = text.toLowerCase();
    if (/\b(cancel|stop|pause|abort)\b/.test(normalized))
        return "cancel";
    if (/\b(redownload|re-download|download again|fresh copy|retry download)\b/.test(normalized)) {
        return "redownload";
    }
    if (/\b(smaller|lighter|tiny|low memory|less memory|save space|not enough space)\b/.test(normalized)) {
        return "switch_smaller";
    }
    if (/\b(download|install|fetch|pull|resume|retry)\b/.test(normalized))
        return "download";
    if (/\b(status|progress|percent|percentage|eta|what.*happen|how.*long)\b/.test(normalized)) {
        return "status";
    }
    return null;
}
function statusLine(model: CatalogModel, jobs: DownloadJob[], detail?: string): string {
    const progress = aggregateProgress(jobs);
    const templates = [
        () => `I'm still downloading ${model.displayName}: ${progress.percent ?? 0}% (${formatGb(progress.received)}/${formatGb(progress.total || Math.round(catalogDownloadSizeGb(model, MODEL_CATALOG) * 1024 ** 3))}). Please hold on.`,
        () => `${model.displayName} is still downloading: ${progress.percent ?? 0}% complete, ${formatGb(progress.received)} of ${formatGb(progress.total || Math.round(catalogDownloadSizeGb(model, MODEL_CATALOG) * 1024 ** 3))}.`,
        () => `Local inference is still downloading. ${model.displayName} is at ${progress.percent ?? 0}% (${formatGb(progress.received)}/${formatGb(progress.total ||
            Math.round(catalogDownloadSizeGb(model, MODEL_CATALOG) * 1024 ** 3))}).`,
    ];
    ...
    if (detail)
        return detail;
    const index = Math.abs(Math.floor(Date.now() / 10000)) % templates.length;
    return templates[index]();
}
async function queueCompanionDownloads(model: CatalogModel): Promise<void> {
    void model;
}
async function downloadNativeModelFile(llama: LlamaCppModule, url: string, filename: string): Promise<string> {
    if (!llama.downloadModel) {
        throw new PublicLocalModelDownloadError("Native Eliza-1 downloader is unavailable.");
    }
    const result = await llama.downloadModel(url, filename);
    return typeof result === "string" ? result : (result.path ?? filename);
}
async function downloadIosBundle(model: CatalogModel, llama: LlamaCppModule, job: DownloadJob): Promise<string> {
    if (!model.bundleManifestFile) {
        throw new PublicLocalModelDownloadError(`${model.displayName} does not declare an Eliza-1 manifest.`);
    }
    const manifestUrl = buildHuggingFaceResolveUrlForPath(model, model.bundleManifestFile);
    ...
    const manifest = await (async () => {
        const { signal, dispose } = createTimeoutSignal(IOS_BUNDLE_MANIFEST_TIMEOUT_MS);
        try {
            ...
            if (!manifestResponse.ok) {
                throw new PublicLocalModelDownloadError(`HTTP ${manifestResponse.status} while fetching ${model.displayName} manifest`);
            }
            ...
            if (isTimeoutAbortError(err)) {
                throw new PublicLocalModelDownloadError(`Timed out fetching ${model.displayName} manifest after ${Math.round(IOS_BUNDLE_MANIFEST_TIMEOUT_MS / 1000)} s`, { cause: err });
            }
            throw err;
        }
        finally {
    ...
    const manifestPath = await downloadNativeModelFile(llama, manifestUrl, iosBundleFilename(model, model.bundleManifestFile));
    files[model.bundleManifestFile] = manifestPath;
    const manifestHash = await hashNativeBundleFile(llama, manifestPath, model.bundleManifestFile);
    ...
    updateDownload(job, {
        received: Math.min(job.total, Math.round((completedSteps / totalSteps) * job.total)),
    });
    for (const entry of entries) {
        const url = buildHuggingFaceResolveUrlForPath(model, entry.path);
        const downloadedPath = await downloadNativeModelFile(llama, url, iosBundleFilename(model, entry.path));
        files[entry.path] = downloadedPath;
        const fileHash = await verifyNativeBundleFile(llama, downloadedPath, entry.sha256, entry.path);
    ...
    if (!textPath) {
        throw new PublicLocalModelDownloadError(`Eliza-1 bundle did not install ${model.ggufFile}`);
    }
    writeBundleRecord({
        modelId: model.id,
            ...
            void (async () => {
                while (polling && ["queued", "downloading"].includes(job.state)) {
                    try {
                        ...
                        if (progress) {
                            const received = typeof progress.downloaded === "number"
                                ? progress.downloaded
                            ...
                            const total = typeof progress.total === "number" && progress.total > 0
                                ? progress.total
                                : job.total;
                            const bytesPerSec = typeof progress.bytesPerSec === "number"
                            ...
                            updateDownload(job, {
                                ...
                                total,
                                bytesPerSec,
                                etaMs: typeof progress.etaMs === "number" || progress.etaMs === null
                                    ? progress.etaMs
...
async function activateModel(modelId: string, knownPath?: string): Promise<ActiveModelState> {
    const catalog = findCatalogModel(modelId);
    const bundle = catalog ? readBundleIndex()[catalog.id] : undefined;
    const installed = await listInstalledModels();
    const model = installed.find((entry) => entry.id === modelId) ??
        (knownPath
            ? ({
                id: modelId,
    ...
    try {
        ...
        if (catalog) {
            ...
            if (fit === "wontfit") {
                throw new Error(`${catalog.displayName} is above this device's local inference minspec. Switch to a smaller model.`);
            }
        }
        const loadOptions = buildMobileLoadOptions(model, hardware);
        await llama.load(loadOptions);
...
export async function handleIosLocalAgentRequest(request: Request, _context: IttpAgentRequestContext = {}): Promise<Response> {
    ...
    if (method === "GET" && pathname.startsWith("/api/documents/")) {
        if (pathname.endsWith("/fragments")) {
            const documentId = decodePathSegment(pathname.slice("/api/documents/".length, -"/fragments".length));
            if (documentId === null) {
                return json({ error: "malformed URL encoding" }, 400);
            }
    ...
    if (pathname === "/api/transcripts" ||
        ...
        const response = await handleLocalTranscriptsRoute(request, method, pathname, url);
        if (response)
            return response;
    }
    ...
    if (method === "POST" && pathname === "/api/tts/local-inference") {
        const body = await requestJson(request);
        const text = typeof body.text === "string" ? sanitizeLocalSpeechText(body.text) : "";
        if (!text)
            return json({ error: "Missing text" }, 400);
        const voiceReadiness = inferVoiceReadiness(await listInstalledModels(), readBundleIndex());
        return json({
            error: voiceReadiness.message,
            code: voiceReadiness.status === "unavailable"
    ...
    if (method === "POST" && pathname === "/api/local-inference/downloads") {
        ...
        if (validationError)
            return json({ error: validationError }, 409);
        return json({ job: startDownload(catalog) });
    }
    const downloadMatch = pathname.match(/^\/api\/local-inference\/downloads\/([^/]+)$/);
    if (downloadMatch) {
        ...
        if (method === "DELETE") {
            if (job && ["queued", "downloading"].includes(job.state)) {
                ...
                if (catalog) {
                    ...
                    await llama
                        ?.cancelDownload?.(buildHuggingFaceResolveUrlForPath(catalog, cancelPath))
                        .catch(() => false);
                }
                updateDownload(job, { state: "cancelled", etaMs: 0 });
            }
    ...
    if (method === "POST" && pathname === "/api/local-inference/assignments") {
        ...
        if (modelId !== null) {
            ...
            if (!catalog ||
                ...
                return json({
                    error: "Local inference assignments are limited to verified Eliza-1 bundles.",
                }, 400);
            }
        }
    ...
    const installedMatch = pathname.match(/^\/api\/local-inference\/installed\/([^/]+)(?:\/verify)?$/);
    if (installedMatch) {
        ...
        if (method === "GET")
            return json({ model });
        if (method === "DELETE") {
            return json({ ok: false, error: "Uninstall is not supported on iOS yet." }, 400);
        }
    ...
    if (method === "POST" && pathname === "/api/conversations") {
        const body = await requestJson(request);
        const conversation = createConversation(typeof body.title === "string" ? body.title : undefined);
        const store = readStore();
    ...
    const messageMatch = pathname.match(/^\/api\/conversations\/([^/]+)\/messages(?:\/stream|\/truncate)?$/);
    if (messageMatch) {
        ...
        if (method === "POST" && pathname.endsWith("/messages/truncate")) {
            ...
            if (index < 0)
                return json({ ok: true, deletedCount: 0 });
    ...
    if (conversationMatch) {
        ...
        const store = readStore();
        const index = store.conversations.findIndex((entry) => entry.id === conversationId);
        if (index < 0)
            return json({ error: "Conversation not found" }, 404);
        if (method === "DELETE") {
