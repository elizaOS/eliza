 ...
 */
import type { ChatFailureKind, ChatTokenUsage, ConnectionStateInfo, ConversationChannelType, ImageAttachment, LocalInferenceChatMetadata, WsEventHandler } from "./client-types";
...
export declare class ElizaClient {
    ...
    rawRequest(path: string, init?: RequestInit, options?: {
        allowNonOk?: boolean;
        timeoutMs?: number;
    }): Promise<Response>;
    fetch<T>(path: string, init?: RequestInit, options?: {
        allowNonOk?: boolean;
        timeoutMs?: number;
    }): Promise<T>;
    connectWs(): void;
    ...
    streamChatEndpoint(path: string, text: string, onToken: (token: string, accumulatedText?: string) => void, channelType?: ConversationChannelType, signal?: AbortSignal, images?: ImageAttachment[], metadata?: Record<string, unknown>): Promise<{
        text: string;
        agentName: string;
        completed: boolean;
