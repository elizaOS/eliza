...
import { MOBILE_LOCAL_AGENT_PORT } from "../first-run/mobile-runtime-mode";
...
import { installElizaBridge } from "../bridge/eliza-window-bridge";
import { isCommittedOnDeviceMobileRuntimeMode } from "../first-run/mobile-runtime-mode";
import { isMobileLocalAgentIpcUrl } from "../first-run/mobile-runtime-mode";
import { isMobileLocalAgentUrl as isConfiguredMobileLocalAgentUrl } from "../first-run/mobile-runtime-mode";
import { isStoreBuild } from "../build-variant";
import { isStreamingRequest } from "./transport";
import { mobileLocalAgentPathFromUrl } from "../first-run/mobile-runtime-mode";
import { nativeHttpResultToResponse } from "./native-http-codec";
import { normalizeMobileRuntimeMode } from "../first-run/mobile-runtime-mode";
import { registerElizaBridgeCapability } from "../bridge/eliza-window-bridge";
import { registerPlugin } from "@capacitor/core";
import { reportRendererDiagnostic } from "../utils/renderer-diagnostics";
import { runAbortableRequest } from "./abortable-request";
import { startIosLocalAgentKernel } from "./ios-local-agent-kernel";
import { toWellFormedUnicode } from "@elizaos/core/utils/unicode";
import { truncateWellFormed } from "@elizaos/core/utils/unicode";
...
let fullBunRuntime: Promise<FullBunRuntimePlugin | null> | PrimedFullBunRuntime | null = null;
 ...
 */
const IOS_CLOUD_MODE_LOCAL_IPC_POLICY_MESSAGE = "iOS cloud builds cannot use local-agent IPC unless local runtime mode is active";
const IOS_REMOTE_MODE_LOCAL_IPC_POLICY_MESSAGE = "iOS remote-agent modes cannot use local-agent IPC";
/**
 ...
 * backend deadline.
 */
export function isTerminalIosNativeAgentBootErrorMessage(message: string | null | undefined): boolean {
    if (!message)
        return false;
    return TERMINAL_IOS_NATIVE_AGENT_BOOT_ERROR_FRAGMENTS.some((fragment) => message.includes(fragment));
}
 ...
 */
export function appendIosBootTrace(stage: string, detail: Record<string, unknown> = {}): void {
    const bridge = resolveBootTraceBridge();
    if (!bridge || bootTraceDisabled)
        return;
    if (bootTraceEntryCount >= IOS_BOOT_TRACE_MAX_ENTRIES_PER_SESSION)
        return;
    ...
    try {
        ...
        safeDetail = JSON.parse(JSON.stringify({
    ...
    bridge
        ...
        if (/not implemented|is not available|method not found/i.test(message) ||
...
export function recordIosNativeAgentBootPhase(phase: IosNativeAgentBootPhase, error?: string | null): void {
    const now = Date.now();
    iosAgentBootProgress = {
        phase,
        startedAt: phase === "starting" ? now : (iosAgentBootProgress.startedAt ?? now),
        lastHeartbeatAt: now,
        lastError: error ?? (phase === "error" ? iosAgentBootProgress.lastError : null),
    };
...
export function isIosNativeAgentBootInProgress(now = Date.now()): boolean {
    ...
    if (progress.phase === "ready") {
        return (progress.lastHeartbeatAt !== null &&
...
interface FullBunRuntimePlugin {
    ...
    addListener?: (eventName: string, listener: (event: unknown) => void) => Promise<{
...
function isPrimedFullBunRuntime(value: typeof fullBunRuntime): value is PrimedFullBunRuntime {
    return (!!value &&
        typeof value === "object" &&
        (value as {
            kind?: unknown;
...
const IOS_RESTART_LISTENER_WINDOW_KEY = "__ELIZA_IOS_LOCAL_AGENT_RESTART_LISTENER_INSTALLED__";
...
declare global {
    interface Window {
        ...
        __ELIZA_IOS_LOCAL_AGENT_REQUEST__?: (options: IosLocalAgentNativeRequestOptions) => Promise<IosLocalAgentNativeRequestResult>;
    }
}
...
function isFullBunRuntimeBuiltIn(): boolean {
    const env = viteEnv();
    return (isTruthyBuildFlag(env.VITE_ELIZA_IOS_FULL_BUN_AVAILABLE) ||
...
function readRuntimeMode(): string | null {
    ...
    if (persisted)
        return persisted;
    const env = viteEnv();
    const iosRuntimeMode = typeof env.VITE_ELIZA_IOS_RUNTIME_MODE === "string"
...
function shouldRequireFullBunRuntime(): boolean {
    ...
    if (isRemoteMacRuntimeMode(runtimeMode))
        return false;
    if (runtimeMode === "cloud" || runtimeMode === "cloud-hybrid")
        return false;
    const fullBunBuiltIn = isFullBunRuntimeBuiltIn();
    return (isTruthyBuildFlag(env.VITE_ELIZA_IOS_FULL_BUN_STRICT) ||
        isTruthyBuildFlag(env.VITE_ELIZA_IOS_FULL_BUN_SMOKE) ||
...
function hasIosFullBunSmokeRequest(): boolean {
    try {
        return (globalThis.localStorage?.getItem("eliza:ios-full-bun-smoke:request") ===
            "1");
    }
    catch {
...
function fullBunStartupError(message: string, cause?: unknown): Error {
    ...
    return new Error(`[ios-local-agent] Full Bun iOS runtime required but ${message}${causeMessage ? `: ${causeMessage}` : ""}`);
}
...
function isNativeIosCloudRuntime(): boolean {
    if (!isNativeIos())
        return false;
    const runtimeMode = readRuntimeMode();
    if (!runtimeMode && isTruthyBuildFlag(viteEnv().PROD))
        return true;
...
function isIosOnDeviceAgentHttpUrl(value: string): boolean {
    if (!isMobileLocalAgentUrl(value))
        return false;
...
function iosRuntimeHasOnDeviceAgent(): boolean {
    ...
    return (mode === "tunnel-to-mobile" || isCommittedOnDeviceMobileRuntimeMode(mode));
}
...
function isCloudRuntimeAllowedLocalAgentPath(path: string): boolean {
    ...
    return (pathname === "/api/local-inference" ||
...
function wrapFullBunRuntime(runtime: FullBunRuntimePlugin): FullBunRuntimePlugin {
    return {
...
export function isIosInProcessLocalAgentBase(baseUrl: string | null | undefined): boolean {
    if (!baseUrl)
        return false;
...
function normalizeNativeResult(value: unknown): IosLocalAgentNativeRequestResult | null {
    ...
    return {
        ...
        bodyEncoding: typeof record.bodyEncoding === "string" ? record.bodyEncoding : undefined,
    };
}
...
function readStartupTraceId(): string | undefined {
    if (typeof window === "undefined")
        return undefined;
    const startupTrace = (window as Window & {
        [STARTUP_TRACE_WINDOW_KEY]?: {
            traceId?: unknown;
        };
    })[STARTUP_TRACE_WINDOW_KEY];
    return (normalizeStartupTraceId(window[STARTUP_TRACE_ID_WINDOW_KEY]) ??
...
async function startFullBunRuntime(runtime: FullBunRuntimePlugin, source = "startup"): Promise<void> {
    ...
    if (!status.ready || status.engine !== "bun") {
        throw new Error(`runtime status was ready=${String(status.ready)} engine=${status.engine ?? "unknown"}`);
    }
...
async function getFullBunRuntime(): Promise<FullBunRuntimePlugin | null> {
    ...
    if (!isNativeIos() && !strict)
        return null;
    if (!strict && !isFullBunRuntimeBuiltIn())
        return null;
    if (!pluginAvailable) {
        if (strict) {
...
async function importFullBunRuntimePlugin(): Promise<FullBunRuntimePlugin> {
    ...
    try {
        mod = (await import("@elizaos/capacitor-bun-runtime")) as Partial<FullBunRuntimeModule>;
    }
    catch {
    ...
    return wrapFullBunRuntime(mod?.ElizaBunRuntime ??
...
async function restartIosFullBunRuntimeFromWatchdog(): Promise<void> {
    ...
    restartRequestInFlight = (async () => {
        if (!isNativeIos())
            return;
        if (["cloud", "remote-mac"].includes(readRuntimeMode() ?? ""))
            return;
...
function installIosLocalAgentRestartRequestListener(): void {
    if (restartRequestListenerInstalled)
        return;
    if (typeof window === "undefined")
        return;
    if (typeof window.addEventListener !== "function")
        return;
    if (window[IOS_RESTART_LISTENER_WINDOW_KEY])
        return;
    window.addEventListener("eliza:local-agent-restart-requested", (event: Event) => {
...
async function tryFullBunNativeRequest(options: IosLocalAgentNativeRequestOptions, signal?: AbortSignal): Promise<IosLocalAgentNativeRequestResult | null> {
...
async function requestToNativeBridgeOptions(request: Request, context?: {
    timeoutMs?: number;
...
function nativeResultToResponse(result: IosLocalAgentNativeRequestResult): Response {
 ...
 */
async function tryFullBunStreamingResponse(options: IosLocalAgentNativeRequestOptions, signal: AbortSignal): Promise<Response | null> {
    const runtime = await getFullBunRuntime();
    signal.throwIfAborted();
    if (!runtime?.addListener)
        return null;
    const plugin = createIosStreamingAgentPlugin({ call: runtime.call, addListener: runtime.addListener }, (error) => {
        reportRendererDiagnostic({
...
async function dispatchIosLocalAgentRequest(request: Request, context?: {
    timeoutMs?: number;
}): Promise<Response> {
    return runAbortableRequest(request.signal, () => dispatchIosRequest(request, context));
}
async function dispatchIosRequest(request: Request, context?: {
    timeoutMs?: number;
}): Promise<Response> {
    ...
    const response = nativeResultToResponse(await handleIosLocalAgentNativeRequest(options, request.signal));
    return abortableResponse(response, request.signal);
}
let tracedNativeRequests = 0;
export async function handleIosLocalAgentNativeRequest(options: IosLocalAgentNativeRequestOptions, signal?: AbortSignal): Promise<IosLocalAgentNativeRequestResult> {
    ...
    if (!path || !isSafeLocalPath(path)) {
        throw new Error("iOS local Agent.request requires a path that starts with / and is not an absolute URL");
    }
    ...
    if (fullBunResult)
        return fullBunResult;
    if (isNativeIosStoreBuild()) {
        throw fullBunStartupError("the foreground ITTP compatibility transport is disabled for iOS store builds");
    }
    if (!canUseJsContextCompatibilityFallback()) {
        throw fullBunStartupError("the JSContext compatibility transport is disabled outside iOS development builds");
    }
    startIosLocalAgentKernel();
    const response = await handleIosLocalAgentRequest(new Request(`${IOS_LOCAL_AGENT_IPC_BASE}${path}`, {
        method,
        headers: options.headers,
...
export function installIosLocalAgentNativeRequestBridge(): void {
    ...
    if (typeof window === "undefined")
        return;
    registerElizaBridgeCapability("iosLocalAgentRequest", handleIosLocalAgentNativeRequest);
    installElizaBridge();
...
function shouldBridgeFetchUrl(url: URL): boolean {
    ...
    if (isNativeIosStoreBuild() && isLoopbackLocalAgentUrl(url.toString())) {
        throw new TypeError("iOS store builds must use eliza-local-agent://ipc for local-agent requests");
    }
    if (isIosLocalAgentIpcUrl(url) && !canUseIosLocalAgentIpc()) {
        if (isCloudRuntimeAllowedIpcPath(url))
            return true;
        throw new TypeError(IOS_CLOUD_MODE_LOCAL_IPC_POLICY_MESSAGE);
    }
    if (usesStrictIosNetworkPolicy() &&
        ...
        throw new TypeError("iOS store/cloud builds block cleartext loopback or private-network requests");
    }
    if (isIosOnDeviceAgentHttpUrl(url.toString()))
        return true;
    if (isNativeIosCloudRuntime())
        return false;
    if ((url.pathname || "").startsWith("/api/")) {
        return (shouldRequireFullBunRuntime() ||
...
export function installIosLocalAgentFetchBridge(): void {
    ...
    const bridgedFetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
...
export async function iosInProcessAgentTransportForUrl(url: string): Promise<AgentRequestTransport | null> {
    if (!isIosInProcessLocalAgentUrl(url))
        return null;
    installIosLocalAgentNativeRequestBridge();
    installIosLocalAgentFetchBridge();
    transport ??= createIttpAgentTransport((request, context) => dispatchIosLocalAgentRequest(request, context));
    return transport;
}
