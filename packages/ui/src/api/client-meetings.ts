...
import { type MeetingStatusEvent } from "@elizaos/core/meetings";
...
ElizaClient.prototype.requestMeetingBot = async function (this: ElizaClient, input: MeetingJoinRequest) {
    return this.fetch("/api/meetings", {
        method: "POST",
        body: JSON.stringify(input),
    });
};
ElizaClient.prototype.listMeetings = async function (this: ElizaClient, options?: ListMeetingsOptions) {
    const q = options?.active ? "?active=1" : "";
    return this.fetch(`/api/meetings${q}`);
};
ElizaClient.prototype.getMeeting = async function (this: ElizaClient, id: string) {
    return this.fetch(`/api/meetings/${encodeURIComponent(id)}`);
};
ElizaClient.prototype.stopMeeting = async function (this: ElizaClient, id: string) {
    return this.fetch(`/api/meetings/${encodeURIComponent(id)}`, {
        method: "DELETE",
    });
};
...
export function parseMeetingTranscriptEvent(data: Record<string, unknown>): MeetingTranscriptEvent | null {
...
export function parseMeetingStatusEvent(data: Record<string, unknown>): MeetingStatusEvent | null {
