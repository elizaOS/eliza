...
export type AuthSetupResult = {
    ...
    reason: "weak_password" | "invalid_display_name" | "insecure_transport" | "already_initialized" | "rate_limited" | "server_error";
    message: string;
};
export type AuthLoginResult = {
    ...
    reason: "invalid_credentials" | "insecure_transport" | "rate_limited" | "server_error";
    message: string;
};
export type AuthMeResult = {
    ...
    ok: false;
    status: 401 | 429 | 503;
    reason?: "remote_auth_required" | "remote_password_not_configured" | "rate_limited" | "server_error" | "cloud_unavailable";
    access?: AuthAccessInfo;
...
export type AuthChangePasswordResult = {
    ...
    reason: "weak_password" | "invalid_credentials" | "insecure_transport" | "owner_not_found" | "rate_limited" | "server_error";
    message: string;
};
// ── API base helper ───────────────────────────────────────────────────────────
/**
...
async function resolvePairingFallback(base: string): Promise<AuthMeResult | null> {
    ...
    if (status.required !== true || status.pairingEnabled !== true)
        return null;
    return {
        ok: false,
        status: 401,
...
export async function authSetup(params: {
    ...
    if (!isPasswordAuthTransportConfidential(base)) {
        return {
            ...
            message: "Owner password setup requires HTTPS or a local in-process connection.",
        };
    }
    let res: Response;
    try {
...
export async function authLoginPassword(params: {
    ...
    if (!isPasswordAuthTransportConfidential(base)) {
        return {
            ...
            status: 400,
            reason: "insecure_transport",
            message: "Owner password login requires HTTPS or a local in-process connection. Pair this device instead.",
        };
    }
    let res: Response;
    try {
    ...
    return {
        ...
        reason: res.status === 401 || res.status === 400
            ? "invalid_credentials"
            : "server_error",
        message: res.status === 401 || res.status === 400
...
export async function authMe(): Promise<AuthMeResult> {
    ...
    const requestIsCurrent = () => authBase() === requestBase && getBootConfig().apiToken === requestToken;
    ...
    if (isManagedCloudSharedAgentBase(authBase())) {
        ...
        const hasNativeOwnerApiKey = (isNative || isElectrobunRuntime()) &&
            Boolean(normalizeCloudApiKeyToken(getBootConfig().apiToken) ??
    ...
    try {
        const viaRpc = !isDesktopLocalApiBaseUrl(requestBase) ||
        ...
        if (viaRpc) {
            ...
            if (viaRpc.unauthorized) {
                ...
                return {
                    ok: false,
                    status: 401,
                    reason: reason === "remote_password_not_configured"
    ...
    if (res.status === 401) {
        ...
        const result: AuthMeResult = {
            ok: false,
            status: 401,
            reason: body.reason === "remote_password_not_configured"
                ? "remote_password_not_configured"
        ...
        if (result.reason === "remote_auth_required" &&
        ...
        if (result.reason !== "server_error" || result.access)
            return result;
 ...
 */
export async function authRevokeSession(sessionId: string): Promise<AuthRevokeResult> {
    let res: Response;
    try {
...
export async function authChangePassword(params: {
    ...
    if (!isPasswordAuthTransportConfidential(base)) {
        return {
            ...
            status: 400,
            reason: "insecure_transport",
            message: "Changing the owner password requires HTTPS or a local in-process connection.",
        };
    }
    let res: Response;
    try {
