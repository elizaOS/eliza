...
import { type DeviceBridgeStatus } from "./local-inference-response-types";
...
import { type ModelBucket } from "@elizaos/core/contracts/local-inference";
...
import { type PublicRegistration } from "./local-inference-response-types";
import { type RoutingPolicy } from "@elizaos/plugin-native-inference/model-catalog/routing-policy";
...
export const LOCAL_INFERENCE_HARDWARE_RESPONSE_INVALID_CODE = "LOCAL_INFERENCE_HARDWARE_RESPONSE_INVALID";
...
function invalidHardware(path: string, expected: string): never {
    throw new ElizaError("Hardware details are unavailable because the agent returned invalid data. Retry after the device check finishes.", {
...
function assertSupportedString(value: unknown, supported: ReadonlySet<string>, path: string, expected: string): asserts value is string {
...
function assertHardwareProbe(value: unknown): asserts value is HardwareProbe {
    if (!isRecord(value))
        invalidHardware("response.hardware", "an object");
    assertFiniteNonNegativeNumber(value.totalRamGb, "response.hardware.totalRamGb");
    ...
    assertSupportedString(value.recommendedBucket, MODEL_BUCKETS, "response.hardware.recommendedBucket", "a supported model bucket");
    assertSupportedString(value.source, HARDWARE_PROBE_SOURCES, "response.hardware.source", "a supported hardware probe source");
    if (value.gpu !== null) {
        if (!isRecord(value.gpu)) {
            invalidHardware("response.hardware.gpu", "an object or null");
        }
        assertSupportedString(value.gpu.backend, GPU_BACKENDS, "response.hardware.gpu.backend", "a supported GPU backend");
        assertFiniteNonNegativeNumber(value.gpu.totalVramGb, "response.hardware.gpu.totalVramGb");
        assertFiniteNonNegativeNumber(value.gpu.freeVramGb, "response.hardware.gpu.freeVramGb");
    }
}
...
function parseModelHubSnapshotHardware(value: unknown): ModelHubSnapshot {
    if (!isRecord(value))
        invalidHardware("response", "an object");
 ...
 */
export function classifyDeviceTierFromProbe(probe: HardwareProbe): DeviceTierResult {
...
declare module "./client-base" {
    interface ElizaClient {
        ...
        searchHuggingFaceGguf(query: string, limit?: number, hub?: "huggingface" | "modelscope"): Promise<{
            models: CatalogModel[];
        }>;
        ...
        setLocalInferenceTextRouting(provider: string, policy?: RoutingPolicy): Promise<{
            preferences: RoutingPreferences;
        }>;
...
ElizaClient.prototype.getLocalInferenceHub = async function (this: ElizaClient) {
    localInferenceHubRequest ??= this.fetch<unknown>("/api/local-inference/hub", undefined, { timeoutMs: 30000 })
...
ElizaClient.prototype.getLocalInferenceHardware = async function (this: ElizaClient) {
    const hardware = await this.fetch<unknown>("/api/local-inference/hardware");
    assertHardwareProbe(hardware);
    return hardware;
};
ElizaClient.prototype.getLocalInferenceDeviceTier = async function (this: ElizaClient) {
...
ElizaClient.prototype.getLocalInferenceCatalog = async function (this: ElizaClient) {
    return this.fetch("/api/local-inference/catalog");
};
ElizaClient.prototype.getLocalInferenceInstalled = async function (this: ElizaClient) {
    return this.fetch("/api/local-inference/installed");
};
ElizaClient.prototype.startLocalInferenceDownload = async function (this: ElizaClient, modelId: string) {
    return this.fetch("/api/local-inference/downloads", {
        method: "POST",
        body: JSON.stringify({ modelId }),
    });
};
ElizaClient.prototype.searchHuggingFaceGguf = async function (this: ElizaClient, query: string, limit?: number, hub: "huggingface" | "modelscope" = "huggingface") {
...
ElizaClient.prototype.cancelLocalInferenceDownload = async function (this: ElizaClient, modelId: string) {
    return this.fetch(`/api/local-inference/downloads/${encodeURIComponent(modelId)}`, { method: "DELETE" });
};
ElizaClient.prototype.getLocalInferenceActive = async function (this: ElizaClient) {
    return this.fetch("/api/local-inference/active");
};
ElizaClient.prototype.setLocalInferenceActive = async function (this: ElizaClient, modelId: string) {
    return this.fetch("/api/local-inference/active", {
        method: "POST",
        body: JSON.stringify({ modelId }),
    });
};
ElizaClient.prototype.clearLocalInferenceActive = async function (this: ElizaClient) {
    return this.fetch("/api/local-inference/active", {
        method: "DELETE",
    });
};
ElizaClient.prototype.uninstallLocalInferenceModel = async function (this: ElizaClient, id: string) {
    return this.fetch(`/api/local-inference/installed/${encodeURIComponent(id)}`, { method: "DELETE" });
};
ElizaClient.prototype.getLocalInferenceDeviceStatus = async function (this: ElizaClient) {
    return this.fetch("/api/local-inference/device");
};
ElizaClient.prototype.getLocalInferenceProviders = async function (this: ElizaClient) {
    return this.fetch("/api/local-inference/providers");
};
ElizaClient.prototype.getLocalInferenceAssignments = async function (this: ElizaClient) {
    return this.fetch("/api/local-inference/assignments");
};
ElizaClient.prototype.setLocalInferenceAssignment = async function (this: ElizaClient, slot: AgentModelSlot, modelId: string | null) {
    return this.fetch("/api/local-inference/assignments", {
        method: "POST",
        body: JSON.stringify({ slot, modelId }),
    });
};
ElizaClient.prototype.verifyLocalInferenceModel = async function (this: ElizaClient, id: string) {
    return this.fetch(`/api/local-inference/installed/${encodeURIComponent(id)}/verify`, { method: "POST" });
};
ElizaClient.prototype.getLocalInferenceRouting = async function (this: ElizaClient) {
    return this.fetch("/api/local-inference/routing");
};
ElizaClient.prototype.setLocalInferencePreferredProvider = async function (this: ElizaClient, slot: AgentModelSlot, provider: string | null) {
    return this.fetch("/api/local-inference/routing/preferred", {
        method: "POST",
        body: JSON.stringify({ slot, provider }),
    });
};
ElizaClient.prototype.setLocalInferenceTextRouting = async function (this: ElizaClient, provider: string, policy: RoutingPolicy = "manual") {
    return this.fetch("/api/local-inference/routing/text", {
        method: "POST",
        body: JSON.stringify({ provider, policy }),
    });
};
ElizaClient.prototype.setLocalInferencePolicy = async function (this: ElizaClient, slot: AgentModelSlot, policy: RoutingPolicy | null) {
