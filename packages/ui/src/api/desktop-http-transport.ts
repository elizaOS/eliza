...
import { methodAllowsBody } from "./transport";
    ...
    async request(url, init, context) {
        ...
        const interrupted = new Promise<never>((_resolve, reject) => {
            ...
            if (context?.timeoutMs !== undefined) {
                timeoutId = setTimeout(() => reject(new DOMException("The desktop HTTP request timed out", "TimeoutError")), context.timeoutMs);
            }
        });
...
export function desktopHttpTransportForUrl(url: string): AgentRequestTransport | null {
