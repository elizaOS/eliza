...
import { LINKED_ACCOUNT_ACCOUNT_SOURCES } from "@elizaos/core/contracts/service-routing-types";
import { LINKED_ACCOUNT_HEALTH_STATES } from "@elizaos/core/contracts/service-routing-types";
import { LINKED_ACCOUNT_PROVIDER_IDS } from "@elizaos/core/contracts/service-routing-types";
import { SERVICE_ROUTE_ACCOUNT_STRATEGIES } from "@elizaos/core/contracts/service-routing-types";
import { codingAgentSpawnCapabilityForProvider } from "@elizaos/core/contracts/coding-agent-capabilities";
import { codingProviderCredentialPathForProvider } from "@elizaos/core/contracts/coding-agent-capabilities";
...
function invalid(path: string, expected: string): never {
    throw new ElizaError(`Invalid /api/accounts response at ${path}: expected ${expected}`, {
...
function assertUsage(value: unknown, path: string): void {
    ...
    if (value.weeklyModelBuckets === undefined)
        return;
...
function assertRuntimeCapability(value: unknown, path: string): void {
    ...
    if (value.credentialPath !== undefined &&
...
function assertRuntimeEligibility(value: unknown, providerId: LinkedAccountProviderId, path: string): void {
    if (!isRecord(value))
        invalid(path, "an object");
    ...
    if (chat.available !== descriptor.inferenceSupport) {
        invalid(`${path}.chat.available`, `the canonical inference availability for provider "${providerId}"`);
    }
    if (chat.credentialPath !== expectedChatCredentialPath) {
        invalid(`${path}.chat.credentialPath`, `the canonical credential path "${expectedChatCredentialPath}"`);
    }
    const canonical = codingAgentSpawnCapabilityForProvider(providerId);
    if (codingAgent.available !== canonical.available) {
        invalid(`${path}.codingAgent.available`, `the canonical spawn availability for provider "${providerId}"`);
    }
    if (canonical.available && codingAgent.backend !== canonical.backend) {
        invalid(`${path}.codingAgent.backend`, `the canonical backend "${canonical.backend}"`);
    }
    if (!canonical.available && codingAgent.backend !== undefined) {
        invalid(`${path}.codingAgent.backend`, "no backend for an unavailable provider");
    }
    const expectedCodingCredentialPath = canonical.available
        ? credentialPath
        : "none";
    if (codingAgent.credentialPath !== expectedCodingCredentialPath) {
        invalid(`${path}.codingAgent.credentialPath`, `the canonical credential path "${expectedCodingCredentialPath}"`);
    }
}
function assertAccount(value: unknown, providerId: string, path: string): string {
    if (!isRecord(value))
        invalid(path, "an object");
    ...
    if (value.prioritySource !== undefined &&
...
function assertAccountsListResponse(value: unknown): asserts value is AccountsListResponse {
    if (!isRecord(value))
        invalid("response", "an object");
    ...
    for (const [providerIndex, rawProvider] of value.providers.entries()) {
        ...
        if (rawProvider.runtimeEligibility !== undefined) {
            assertRuntimeEligibility(rawProvider.runtimeEligibility, providerId, `${path}.runtimeEligibility`);
        }
...
export function parseAccountsListResponse(value: unknown): AccountsListResponse {
