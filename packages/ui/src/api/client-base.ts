...
import { isCloudAgentGoneError } from "./client-types";
...
import { isIosInProcessLocalAgentBase } from "./ios-local-agent-transport";
...
import { mergeStreamingText } from "@elizaos/ui/utils/streaming-text";
...
import { saveAgentProfileRegistry } from "../state/agent-profiles";
...
import { type ConnectionStateInfo } from "./client-types";
...
import { type NetworkStatusChangeDetail } from "../events";
import { type WebSocketConnectionState } from "./client-types";
import { type WsEventHandler } from "./client-types";
...
export function isStreamGenerationError(value: unknown): value is StreamGenerationError {
    return value instanceof StreamGenerationError;
}
const CHAT_TURN_STATUS_KINDS: ReadonlySet<ChatTurnStatus["kind"]> = new Set<ChatTurnStatus["kind"]>([
    "thinking",
    "streaming",
    "running_action",
    "running_tool",
...
const CHAT_TOOL_PHASES: ReadonlySet<ChatToolCallEvent["phase"]> = new Set<ChatToolCallEvent["phase"]>(["call", "result", "error"]);
...
function parseChatToolCallEvent(parsed: StreamChatEvent): ChatToolCallEvent | null {
    if (!parsed.phase || !CHAT_TOOL_PHASES.has(parsed.phase))
        return null;
...
function isElizaCloudControlPlaneBase(value: string | null | undefined): boolean {
    ...
    if (!normalized)
        return false;
    try {
        return isElizaCloudControlPlaneHostname(new URL(normalized).hostname.toLowerCase());
    }
    catch {
...
function requestHeadersToRecord(headers: HeadersInit | undefined): Record<string, string> {
    if (!headers)
        return {};
...
function sseEventName(lines: readonly string[]): string | undefined {
    ...
    for (const line of lines) {
        if (line.startsWith("event:"))
            name = line.slice(6).trim() || undefined;
    }
    return name;
}
function parseStreamChatDataLine(line: string, eventName?: string): StreamChatEvent | null {
...
function applyStreamChatTokenEvent(parsed: StreamChatEvent, state: StreamChatState, onToken: (token: string, accumulatedText?: string, provisional?: boolean) => void): boolean {
    ...
    const nextFullText = fullText !== null
...
function parseStreamActionResults(value: unknown): ChatActionResultSummary[] | undefined {
    if (!Array.isArray(value) ||
        !value.every((result) => result !== null &&
            typeof result === "object" &&
            !Array.isArray(result) &&
            typeof result.success === "boolean" &&
            ["actionName", "text", "error"].every((key) => result[key] === undefined || typeof result[key] === "string") &&
...
function applyStreamChatDoneEvent(parsed: StreamChatEvent, state: StreamChatState): boolean {
...
function applyStreamChatDataLine(line: string, state: StreamChatState, onToken: (token: string, accumulatedText?: string, provisional?: boolean) => void, onStatus?: (status: ChatTurnStatus) => void, onToolEvent?: (event: ChatToolCallEvent) => void, eventName?: string, onReplyReady?: (actionResults: ChatActionResultSummary[]) => void): boolean {
    ...
    if (parsed.type === "reply_ready") {
        ...
        applyStreamChatTokenEvent({ ...parsed, provisional: false }, state, onToken);
...
function isSharedRuntimeRestAdapterBase(value: string | null | undefined): boolean {
    const normalized = normalizeBaseUrl(value);
    if (!normalized)
        return false;
    try {
...
function isRemoteRelayApiUrl(value: string | null | undefined): boolean {
    ...
    try {
        const url = new URL(normalized);
        return (url.protocol === "eliza-remote:" &&
            url.host === "session" &&
            !url.username &&
            !url.password &&
...
export function isRemoteRelayRestAdapterBase(value: string | null | undefined): boolean {
    const normalized = normalizeBaseUrl(value);
    if (!normalized || !isRemoteRelayApiUrl(normalized))
        return false;
    try {
        const url = new URL(normalized);
        return (/^\/[A-Za-z0-9._-]{1,256}$/.test(url.pathname) && !url.search && !url.hash);
    }
    catch {
...
function shouldTreatAsConnectedWithoutWebSocket(value: string | null | undefined): boolean {
...
function shouldUseRestOnlyForInsecureWebSocket(wsProtocol: "ws:" | "wss:", host: string): boolean {
    if (wsProtocol !== "ws:")
        return false;
    if (typeof window === "undefined")
        return false;
    ...
    if (isTrustedPairedHost && isAndroidLocalSideloadBuild())
        return false;
    return true;
}
/**
...
function isNetworkStatusChangeEvent(ev: Event): ev is CustomEvent<NetworkStatusChangeDetail> {
    if (!("detail" in ev))
        return false;
 ...
 */
export function onNetworkStatusChange(listener: (connected: boolean) => void): () => void {
...
function warmingRetryDelayMs(retryAfterSeconds: number | undefined): number {
    const ms = retryAfterSeconds !== undefined && Number.isFinite(retryAfterSeconds)
...
function sleepUnlessAborted(ms: number, signal?: AbortSignal | null): Promise<void> {
...
export class ElizaClient {
    ...
    private connectionStateListeners = new Set<(state: ConnectionStateInfo) => void>();
    ...
    constructor(baseUrl?: string, token?: string) {
        ...
        const initialBase = normalizeBaseUrl(baseUrl ?? bootBase ?? injectedBase ?? storedBase ?? "");
    ...
    setToken(token: string | null): void {
        if (this.pinnedRemoteApiBase &&
            ...
            logger.warn("[ElizaClient] ignored a credential not bound to the build-pinned remote target");
            return;
        }
    ...
    private isPinnedRemoteCredential(token: string): boolean {
        if (!this.pinnedRemoteApiBase)
            return true;
        const activeServer = loadPersistedActiveServer();
        return (activeServer?.kind === "remote" &&
    ...
    private notifyAuthorityChange(): void {
        for (const listener of this.authorityChangeListeners) {
            try {
                ...
                logger.error({ err }, "[ElizaClient] onAuthorityChange listener threw; other listeners still notified");
            }
        }
    }
    /**
    ...
    private installToken(token: string | null, notify: boolean): void {
        ...
        const tokenChanged = previousEffectiveToken !== this.apiToken ||
    ...
    setBaseUrl(baseUrl: string | null, options?: {
        persist?: boolean;
    }): void {
        const normalized = normalizeBaseUrl(baseUrl);
        if (this.pinnedRemoteApiBase && normalized !== this.pinnedRemoteApiBase) {
            logger.warn("[ElizaClient] ignored a base change outside the build-pinned remote target");
            return;
        }
    ...
    private notifyBaseUrlChange(): void {
        for (const listener of this.baseUrlChangeListeners) {
            try {
                ...
                logger.error({ err }, "[ElizaClient] onBaseUrlChange listener threw; other listeners still notified");
            }
        }
    }
    /**
    ...
    repointBaseUrl(baseUrl: string, token?: string | null): void {
        ...
        if (!normalized)
            return;
        if (this.pinnedRemoteApiBase && normalized !== this.pinnedRemoteApiBase) {
            logger.warn("[ElizaClient] ignored a repoint outside the build-pinned remote target");
            return;
        }
        if (this.pinnedRemoteApiBase &&
            token?.trim() &&
            !this.isPinnedRemoteCredential(token)) {
            logger.warn("[ElizaClient] ignored a repoint credential not bound to the build-pinned remote target");
            return;
        }
        ...
        if (installsToken)
            this.installToken(token ?? null, false);
        this._userSetBase = normalized.length > 0;
        this._baseUrl = normalized;
        this.persistBaseUrlFailClosed(normalized);
        this.notifyBaseUrlChange();
     ...
     */
    private async repointAfterPersonalElizaCutover(response: Response, requestBase: string, authToken: string | null, signal?: AbortSignal | null): Promise<boolean> {
        if (response.status !== 409 || !authToken)
            return false;
        let payload: unknown;
        try {
        ...
        const repoint = (async (): Promise<boolean> => {
            try {
                ...
                if (!savePersistedActiveServer(server)) {
                    logger.warn("[ElizaClient] Dedicated runtime resolved but active-server persistence was unavailable");
                }
                ...
                logger.warn({ error }, "[ElizaClient] failed to resolve authoritative personal Eliza runtime after cutover");
                return false;
            }
        })();
    ...
    // --- REST API ---
    async rawRequest(path: string, init?: RequestInit, options?: {
        allowNonOk?: boolean;
        timeoutMs?: number;
        ...
        let token = this.apiToken ?? (await hydrateAndroidLocalAgentTokenForUrl(requestUrl));
        ...
        const logicalRequestDeadline = Date.now() + (options?.timeoutMs ?? defaultFetchTimeoutMs(path, init));
        let requestAttempt = 0;
        const requestOnce = () => this.rawRequestOnce(path, requestUrl, init, options, token, ++requestAttempt);
        ...
        if (await this.repointAfterPersonalElizaCutover(res, requestBase, token, init?.signal)) {
        ...
        while (true) {
            ...
            if (res.ok)
                return res;
            const rawText = await this.readBodyText(res, path, options?.timeoutMs, init).catch((error: unknown) => {
            ...
            const featureStarting = res.status === 503 &&
                code === "feature_starting" &&
                body.retryable === true;
            if (featureStarting && !options?.allowNonOk && !init?.signal?.aborted) {
                const now = Date.now();
                const lifecycleBound = init?.signal != null && options?.timeoutMs === undefined;
                if (lifecycleBound || now < logicalRequestDeadline) {
                    notifyWaiting();
                    await sleepUnlessAborted(lifecycleBound
                        ? warmingRetryDelayMs(retryAfter)
                        : Math.min(warmingRetryDelayMs(retryAfter), logicalRequestDeadline - now), init?.signal);
                    if (!init?.signal?.aborted &&
            ...
            if (res.status === 503 &&
                ...
                if (now < warmingDeadline) {
                    ...
                    // dots.
                    notifyWaiting();
                    const delay = Math.min(warmingRetryDelayMs(retryAfter), warmingDeadline - now);
     ...
     */
    private releaseStaleCloudAgentBindingIfGone(error: ApiError, requestBase: string): void {
        if (!isCloudAgentGoneError(error))
            return;
        const base = normalizeBaseUrl(requestBase);
        if (!base)
            return;
        if (!isDedicatedCloudAgentBase(base) &&
            !isSharedRuntimeRestAdapterBase(base)) {
            return;
        }
        if (this._releasedGoneAgentBase === base)
            return;
        ...
        try {
            ...
            if (remaining.length !== registry.profiles.length) {
                const activeStillPresent = remaining.some((profile) => profile.id === registry.activeProfileId);
    ...
    private async rawRequestOnce(path: string, requestUrl: string, init: RequestInit | undefined, options: {
        allowNonOk?: boolean;
        timeoutMs?: number;
    } | undefined, token: string | null, requestAttempt: number): Promise<Response> {
        ...
        try {
            const requestInit = this.rawRequestInit(init, abortController, token, requestUrl, requestAttempt);
    ...
    private rawRequestInit(init: RequestInit | undefined, abortController: AbortController, token: string | null, requestUrl: string, requestAttempt: number): RequestInit {
        ...
        const hasCsrfHeader = Object.keys(headers).some((name) => name.toLowerCase() === CSRF_HEADER_NAME);
        ...
        if (isDedicatedCloudRequest) {
            for (const key of Object.keys(headers)) {
                ...
                if (DEDICATED_CLOUD_CORS_BLOCKED_HEADERS.has(normalized) ||
    ...
    private async rawRequestTransport(requestUrl: string): Promise<AgentRequestTransport> {
    ...
    private throwRawRequestError(err: unknown, path: string, timeoutMs: number, timedOut: boolean, abortController: AbortController): never {
        if (timedOut) {
            throw new ApiError({
                kind: "timeout",
                path,
     ...
     */
    private async readBodyText(res: Response, path: string, timeoutMs?: number, init?: RequestInit): Promise<string> {
        ...
        try {
            ...
            try {
                await reader.cancel(terminalReason === "timeout"
                ...
                logger.debug({ path, error: cancelError }, "[ElizaClient] failed to cancel interrupted JSON response body");
            }
            throw error;
        }
        finally {
    ...
    async fetch<T>(path: string, init?: RequestInit, options?: {
        allowNonOk?: boolean;
        timeoutMs?: number;
    ...
    // --- WebSocket ---
    private rememberReplayableWsEvent(type: string, data: Record<string, unknown>): void {
    ...
    private replayBackloggedWsEvents(type: string, handler: WsEventHandler): void {
        const backlog = this.wsEventBacklog.get(type);
        if (!backlog?.length)
            return;
    ...
    connectWs(): void {
        ...
        const effectiveBase = this.baseUrl ||
            (getInjectedWsBase()
                ? ""
        ...
        const explicitHttpBase = this._userSetBase && this.baseUrl
            ? (() => {
                try {
        ...
        if (wsBase && explicitHttpBase) {
            try {
                ...
                const toHttpOrigin = (value: string): string => value
                    .replace(/^wss:\/\//i, "https://")
                    .replace(/^ws:\/\//i, "http://");
        ...
        socket.onclose = () => {
            ...
            if (this.reconnectAttempt >= this.maxReconnectAttempts) {
                ...
                if (isDedicatedCloudAgentBase(effectiveBase) ||
    ...
    resetConnection(): void {
        ...
        if (existingReadyState === WebSocket.OPEN ||
    ...
    sendWsMessage(data: Record<string, unknown>): void {
        const message: Record<string, unknown> = typeof data.msgId === "string" && data.msgId.length > 0
            ? data
    ...
    normalizeAssistantText(text: string, options?: {
        interrupted?: boolean;
    }): string {
    ...
    async streamChatEndpoint(path: string, text: string, onToken: (token: string, accumulatedText?: string, 
    ...
    provisional?: boolean) => void, channelType: ConversationChannelType = "DM", signal?: AbortSignal, images?: ImageAttachment[], metadata?: Record<string, unknown>, 
    ...
    onReplyReady?: (actionResults: ChatActionResultSummary[]) => void): Promise<{
        ...
        const resolvedClientMessageId = clientMessageId ?? ElizaClient.generateMessageId();
            ...
            while (eventBreak) {
                ...
                for (const line of eventLines) {
                    if (!line.startsWith("data:"))
                        continue;
                    if (applyStreamChatDataLine(line, streamState, onToken, onStatus, onToolEvent, eventName, notifyReplyReady)) {
                        buffer = "";
        ...
        if (!streamState.receivedDone && buffer.trim()) {
            ...
            for (const line of trailingLines) {
                if (line.startsWith("data:")) {
                    applyStreamChatDataLine(line, streamState, onToken, onStatus, onToolEvent, trailingEventName, notifyReplyReady);
                }
            }
        }
