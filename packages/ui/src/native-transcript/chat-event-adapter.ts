...
export function isNativeChatFailureRetryable(failureKind: ChatFailureKind, terminalFailure?: ChatTerminalFailure): boolean {
...
export function publishNativeToolState(event: ChatToolCallEvent, turnId?: string): void {
    const detail = toolDetail(event);
    publishNativeTranscriptEvent({
        type: "tool.state",
        callId: event.callId,
        name: event.toolName,
        phase: event.phase === "call"
            ? "started"
            : event.phase === "result"
                ? "succeeded"
                : "failed",
...
export interface NativeChatTranscriptTurnPublisher {
    ...
    publishFailureKind(failureKind: ChatFailureKind, message?: string, terminalFailure?: ChatTerminalFailure): void;
...
export function createNativeChatTranscriptTurnPublisher(options: {
    ...
    const publishUserFinal = (text: string, at: number): void => {
        if (!options.enabled || userPublished)
            return;
        userPublished = true;
        publishNativeTranscriptEvent({
            type: "stt.final",
    ...
    const publishAgentSnapshot = (text: string, final = false): void => {
        ...
        if (agentFinal)
            return;
        if (text === lastAgentText && final === agentFinal)
            return;
        lastAgentText = text;
        agentFinal = final;
    ...
    const publishFailureKind = (failureKind: ChatFailureKind, message?: string, terminalFailure?: ChatTerminalFailure): void => {
        publishError({
    ...
    const publishCancel = (reason: string): void => {
        if (!options.enabled || terminalPublished || cancelled)
            return;
        cancelled = true;
        publishNativeTranscriptEvent({
            type: "cancel",
    ...
    return {
        ...
        publishTerminal(result) {
            if (terminalPublished)
                return;
            publishAgentSnapshot(result.text || result.streamedText, result.completed !== false);
            if (result.failureKind) {
                publishFailureKind(result.failureKind, undefined, result.terminalFailure);
            }
            ...
            if (result.completed === false)
                publishCancel("generation-incomplete");
