...
describe("primeAuthStatusProbe + activation reuse", () => {
  ...
  it("defers one private-runtime startup 401 without deleting the device credential", async () => {
    setBootConfig({
      branding: {},
    ...
    expect(loadPersistedActiveServer()?.accessToken).toBe("device-token");
    const { result } = renderHook(() => useAuthStatus({ pollIntervalMs: 0 }));
    await waitFor(() =>
      expect(result.current.state.phase).toBe("authenticated"),
    );
  ...
  it("scrubs a private-runtime credential after the activated probe confirms the 401", async () => {
    setBootConfig({
      branding: {},
      apiBase: "http://127.0.0.1:31338",
      apiToken: "bad-device-token",
    });
    savePersistedActiveServer(
      createPersistedActiveServer({
        kind: "remote",
        apiBase: "http://127.0.0.1:31338",
        accessToken: "bad-device-token",
      }),
    );
    const unauthorized = {
      reason: "remote_auth_required",
    ...
    expect(loadPersistedActiveServer()?.accessToken).toBe("bad-device-token");
    const { result } = renderHook(() => useAuthStatus({ pollIntervalMs: 0 }));
    await waitFor(() =>
      expect(result.current.state).toMatchObject({
        phase: "unauthenticated",
