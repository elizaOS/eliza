...
import { type AccountRefreshUsageResult } from "../api/client-agent";
import { type AccountStrategy } from "../api/client-agent";
import { type AccountTestResult } from "../api/client-agent";
...
function replaceAccount(list: AccountsListResponse | null, providerId: LinkedAccountProviderId, next: LinkedAccountConfig): AccountsListResponse | null {
    if (!list)
        return list;
    return {
        providers: list.providers.map((p) => {
            ...
            return {
                ...
                accounts: (existing
                    ...
                    : [...p.accounts, merged]).sort((a, b) => (Number.isFinite(a.priority) ? a.priority : 0) -
...
export function useAccounts(opts: UseAccountsOptions = {}): UseAccountsResult {
    ...
    const nextAccountVersion = useCallback((providerId: LinkedAccountProviderId, accountId: string) => {
    ...
    const notify = useCallback((prefix: string, err: unknown) => {
        setActionNotice?.(describeError(prefix, err), "error", 6000);
    }, [setActionNotice]);
    const refresh = useCallback<UseAccountsResult["refresh"]>(async (created) => {
        if (created) {
            ...
            setData((previous) => replaceAccount(previous, created.providerId, created.account));
        }
    ...
    const createApiKey = useCallback<UseAccountsResult["createApiKey"]>(async (providerId, body) => {
        stateVersionRef.current += 1;
            ...
            setData((prev) => {
                ...
                return {
                    providers: prev.providers.map((p) => {
                        ...
                        ].sort((a, b) => (Number.isFinite(a.priority) ? a.priority : 0) -
                            (Number.isFinite(b.priority) ? b.priority : 0));
                        return { ...p, accounts };
                    }),
                };
    ...
    const patch = useCallback<UseAccountsResult["patch"]>(async (providerId, accountId, body) => {
        stateVersionRef.current += 1;
                ...
                providers: prev.providers.map((p) => {
                    ...
                    return {
                        ...
                        accounts: p.accounts
                            ...
                            .sort((x, y) => (Number.isFinite(x.priority) ? x.priority : 0) -
    ...
    const remove = useCallback<UseAccountsResult["remove"]>(async (providerId, accountId) => {
        stateVersionRef.current += 1;
    ...
    const test = useCallback<UseAccountsResult["test"]>(async (providerId, accountId) => {
        markSaving(`test:${accountId}`, true);
        try {
            const result = await client.testAccount(providerId, accountId);
            if (result.ok) {
                const catalogUnavailable = result.modelCatalogUnavailable || result.modelCatalogTruncated;
                const catalogSuffix = catalogUnavailable
                    ? "; model catalog unavailable"
                    : "";
                setActionNotice?.(`Connection OK${typeof result.latencyMs === "number"
                    ? ` (${result.latencyMs}ms)`
                    : ""}${catalogSuffix}`, catalogUnavailable ? "info" : "success", catalogUnavailable ? 6000 : 3000);
            }
            else {
    ...
    const refreshUsage = useCallback<UseAccountsResult["refreshUsage"]>(async (providerId, accountId) => {
        stateVersionRef.current += 1;
        const request = nextAccountVersion(providerId, accountId);
        markSaving(`usage:${accountId}`, true);
        try {
            const result: AccountRefreshUsageResult = await client.refreshAccountUsage(providerId, accountId);
            if (accountVersionRef.current.get(request.key) === request.version) {
                stateVersionRef.current += 1;
            ...
            try {
                ...
                logger.warn({ error: reconcileError, providerId, accountId }, "[useAccounts] post-probe reconciliation failed");
            }
            throw err;
        }
        finally {
    ...
    const setStrategy = useCallback<UseAccountsResult["setStrategy"]>(async (providerId, strategy) => {
        stateVersionRef.current += 1;
