...
import { ASSISTANT_TTS_DEBOUNCE_MS } from "../voice/voice-chat-types";
import { ASSISTANT_TTS_FINAL_ONLY } from "../voice/voice-chat-types";
import { ASSISTANT_TTS_FIRST_FLUSH_CHARS } from "../voice/voice-chat-types";
import { ASSISTANT_TTS_MIN_CHUNK_CHARS } from "../voice/voice-chat-types";
import { BufferedVoiceEvidence } from "../voice/voice-playback-evidence";
import { Capacitor } from "@capacitor/core";
import { CloudSttError } from "../voice/local-asr-transcribe";
import { DEFAULT_ELEVEN_MODEL } from "../voice/voice-chat-types";
import { DEFAULT_ELEVEN_VOICE } from "../voice/voice-chat-types";
import { ElizaError } from "@elizaos/core/errors";
import { PlaybackFramePump } from "../voice/playback-frame-pump";
import { TALKMODE_STOP_SETTLE_MS } from "../voice/voice-chat-types";
import { collapseWhitespace } from "../voice/voice-chat-playback";
import { configuredCloudVoiceOrigin } from "../voice/shared-runtime-voice";
import { currentSharedRuntimeVoiceOrigin } from "../voice/shared-runtime-voice";
import { describeTtsCloudFetchTargetForDebug } from "../voice/voice-chat-types";
import { describeTtsFetchTargetForDebug } from "../voice/voice-chat-types";
import { ensurePlaybackContextRunning } from "../voice/playback-frame-pump";
import { fetchWithCsrf } from "../api/csrf-client";
import { formatNamedVoiceError } from "./voice-error-preview";
import { formatVoiceErrorPreview } from "./voice-error-preview";
...
import { getElizaPlayVoicePlugin } from "../bridge/native-plugins";
import { getSpeechRecognitionCtor } from "../voice/voice-chat-types";
import { getTalkModePlugin } from "../bridge/native-plugins";
import { hasConfiguredApiKey } from "../voice";
import { invokeDesktopBridgeRequest } from "../bridge/electrobun-rpc";
import { isAbortError } from "../voice/voice-chat-types";
import { isCommittedSpeechPrefix } from "../voice/voice-chat-playback";
import { isLocalAsrCaptureSupported } from "../voice/local-asr-capture";
import { isLocalInferenceAsrReady } from "../voice/local-asr-transcribe";
import { isSilentWav } from "../voice/local-asr-capture";
import { isTtsDebugEnabled } from "../utils/tts-debug";
import { localePrefix } from "../voice/voice-chat-types";
import { logger } from "@elizaos/ui/logger";
import { matchesVoiceLocale } from "../voice/voice-chat-types";
import { mergeTranscriptWindows } from "../voice/voice-chat-recording";
import { nextIdleMouthOpen } from "../voice/voice-chat-playback";
import { normalizeCacheText } from "../voice/voice-chat-playback";
import { normalizeMouthOpen } from "../voice/voice-chat-playback";
import { normalizeSpeechLocale } from "../voice/voice-chat-types";
import { observeVoiceRequest } from "../voice/voice-playback-evidence";
import { playDecodedVoiceAudio } from "../voice/voice-chat-audio-playback";
import { queueableSpeechPrefix } from "../voice/voice-chat-playback";
import { readCachedAudio } from "../voice/voice-chat-types";
import { remainderAfter } from "../voice/voice-chat-playback";
import { rememberCachedAudio } from "../voice/voice-chat-types";
import { reportRendererDiagnostic } from "../utils/renderer-diagnostics";
import { requestViaAgentTransport } from "../api/csrf-client";
import { resolveApiUrl } from "../utils";
import { resolveEffectiveVoiceConfig } from "../voice/voice-chat-types";
import { resolveForcedCloudTtsRoute } from "../voice/shared-runtime-voice";
import { resolveVoiceMode } from "../voice/voice-chat-types";
import { resolveVoiceProxyEndpoint } from "../voice/voice-chat-types";
import { resolveWavAsrRoute } from "../voice/voice-provider-defaults";
import { resumeAudioContextForPlayback } from "../voice/playback-frame-pump";
import { shouldCacheGeneratedSpeech } from "../voice/voice-chat-playback";
import { splitFirstSentence } from "../voice/voice-chat-playback";
import { stableSpeechPrefix } from "../voice/voice-chat-playback";
import { startLocalAsrRecorder } from "../voice/local-asr-capture";
import { toArrayBuffer } from "../voice/voice-chat-types";
import { toSpeakableText } from "../voice/voice-chat-playback";
import { transcribeCloudWav } from "../voice/local-asr-transcribe";
import { transcribeLocalInferenceWav } from "../voice/local-asr-transcribe";
import { ttsDebug } from "../utils/tts-debug";
import { ttsDebugTextPreview } from "../utils/tts-debug";
import { type AssistantSpeechState } from "../voice/voice-chat-types";
...
import { type QueueAssistantSpeechOptions } from "../voice/voice-chat-types";
import { type SpeakTask } from "../voice/voice-chat-types";
import { type SpeechRecognitionInstance } from "../voice/voice-chat-types";
import { type SpeechRecognitionResultEvent } from "../voice/voice-chat-types";
import { type TalkModeErrorEvent } from "../bridge/native-plugins";
import { type TalkModeStateEvent } from "../bridge/native-plugins";
import { type TalkModeTranscriptEvent } from "../bridge/native-plugins";
import { type VoiceCaptureMode } from "../voice/voice-chat-types";
import { type VoiceChatOptions } from "../voice/voice-chat-types";
import { type VoiceChatState } from "../voice/voice-chat-types";
import { type VoiceConfig } from "../api/client";
import { type VoicePlaybackStartEvent } from "../voice/voice-chat-types";
import { type VoiceSessionMode } from "../voice/voice-chat-types";
import { type VoiceSpeakerMetadata } from "../voice/voice-chat-types";
import { type VoiceTranscriptEvent } from "../voice/voice-chat-types";
import { type VoiceTranscriptPreviewEvent } from "../voice/voice-chat-types";
import { type VoiceTtsError } from "../voice/voice-chat-types";
import { type VoiceTurn } from "../voice/voice-chat-types";
...
import { warmPlaybackWorklet } from "../voice/playback-frame-pump";
import { webSpeechVoiceDebugFields } from "../voice/voice-chat-types";
...
const MAX_PREFETCH_TEXT_CHARACTERS = 1000;
...
const CLOUD_TTS_TIMEOUT_MS = 60000;
const LOCAL_INFERENCE_TTS_TIMEOUT_MS = 60000;
...
function warnDirectCloudTtsFallback(target: string, error: unknown): void {
    ...
    if (warnedDirectCloudTtsTargets.has(target))
        return;
    warnedDirectCloudTtsTargets.add(target);
    logger.warn(`[useVoiceChat] Direct cloud TTS to ${target} failed (${message}); falling back to the on-device /api/tts/cloud proxy`);
}
...
function normalizeActiveVoiceSessionMode(mode: unknown): Exclude<VoiceSessionMode, "idle"> | null {
...
export function useVoiceChat(options: VoiceChatOptions): VoiceChatState {
    ...
    const sttBackendRef = useRef<"browser" | "local-inference" | "cloud" | "talkmode" | null>(null);
    ...
    const emitTranscript = useEffectEvent((text: string, event: VoiceTranscriptEvent) => {
        options.onTranscript(text, event);
    });
    const emitTranscriptPreview = useEffectEvent((text: string, event: VoiceTranscriptPreviewEvent) => {
    ...
    const stopPlaybackFrameTap = useCallback((options: {
        reset?: boolean;
        drain?: boolean;
    } = {}) => {
    ...
    const assistantTtsDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    ...
    const unlockAudio = useCallback(() => {
        ...
        const errorEngine: VoiceTtsError["engine"] = currentProvider === "elevenlabs"
            ? "elevenlabs"
    ...
    const rememberCachedSegment = useCallback((key: string, bytes: Uint8Array) => {
        rememberCachedAudio(key, bytes);
    }, []);
    const makeElevenCacheKey = useCallback((text: string, config: NonNullable<VoiceConfig["elevenlabs"]>) => {
    ...
    const makeLocalInferenceCacheKey = useCallback((text: string) => ["local-inference", normalizeCacheText(text)].join("|"), []);
    const makeElizaCloudCacheKey = useCallback((text: string) => ["eliza-cloud", normalizeCacheText(text)].join("|"), []);
    const updateMouthOpen = useCallback((value: number | ((previousValue: number) => number)) => {
    ...
    useEffect(() => {
        ...
        const syncVoiceSupport = async () => {
            ...
            const localAsrSupported = shouldUseLocalInferenceAsr(voiceConfigRef.current) &&
            ...
            const cloudAsrSupported = shouldUseCloudAsr(voiceConfigRef.current) &&
                isLocalAsrCaptureSupported();
    ...
    const applyTranscriptUpdate = useCallback((transcript: string, isFinal: boolean, metadata: VoiceTranscriptUpdateMetadata = {}) => {
    ...
    const ensureTalkModeListeners = useCallback(async () => {
        ...
        const registration = (async () => {
            ...
            const transcriptHandle = await talkMode.addListener("transcript", (event: TalkModeTranscriptEvent) => {
            ...
            const errorHandle = await talkMode.addListener("error", (event: TalkModeErrorEvent) => {
                if (sttBackendRef.current === "talkmode" ||
            ...
            const stateHandle = await talkMode.addListener("stateChange", (event: TalkModeStateEvent) => {
    ...
    const transcribeLocalInferenceAudio = useCallback(async (audio: Uint8Array, signal?: AbortSignal): Promise<string> => {
        const { text } = await transcribeLocalInferenceWav(audio, { signal });
        return text;
    }, []);
    const transcribeCloudAudio = useCallback(async (audio: Uint8Array, signal?: AbortSignal): Promise<string> => {
    ...
    const startLocalInferenceRecognition = useCallback(async (mode: Exclude<VoiceCaptureMode, "idle">) => {
        try {
    ...
    const startCloudRecognition = useCallback(async (mode: Exclude<VoiceCaptureMode, "idle">) => {
        try {
            const recorder = await startLocalAsrRecorder();
            localAsrRecorderRef.current = recorder;
            captureStartedAtRef.current = Date.now();
    ...
    const startBrowserRecognition = useCallback((mode: Exclude<VoiceCaptureMode, "idle">) => {
    ...
    const startTalkModeRecognition = useCallback(async (mode: Exclude<VoiceCaptureMode, "idle">) => {
        if (!shouldPreferNativeTalkMode()) {
            return false;
        }
        try {
    ...
    const finalizeRecognition = useCallback((submit: boolean) => {
        const mode = normalizeActiveVoiceSessionMode(listeningModeRef.current) ?? "compose";
    ...
    const startListening = useCallback(async (mode: Exclude<VoiceCaptureMode, "idle"> = "compose") => {
        ...
        const localInferenceReady = shouldUseLocalInferenceAsr(voiceConfigRef.current) && captureSupported
        ...
        if (wavRoute === "local-inference") {
            ...
            if (cloudStarted) {
                voiceCaptureDebug("provider:cloud", {
                    mode,
                    note: asrProvider === "local-inference"
            ...
            if (recorder) {
                try {
                    ...
                    applyTranscriptUpdate(transcript, true, {
                        mode: normalizeActiveVoiceSessionMode(mode) ??
    ...
    const cancelPlayback = useCallback(() => {
        ...
        if (Capacitor.isNativePlatform()) {
            ...
            const talkMode = getTalkModePlugin();
            const stopNativePlayback = typeof talkMode.stopSpeaking === "function"
                ? talkMode.stopSpeaking()
                : getElizaPlayVoicePlugin().stop?.();
            void stopNativePlayback?.catch((error: unknown) => {
                logger.warn({ error }, "[useVoiceChat] Native speech teardown failed during barge-in");
            });
        }
        clearSpeechTimers();
    ...
    // ── ElevenLabs TTS ────────────────────────────────────────────────
    const prepareElevenLabs = useCallback(async (text: string, elConfig: NonNullable<VoiceConfig["elevenlabs"]>, task: QueuedSpeechTask, controller: AbortController) => {
        ...
        if (!audioBytes) {
            ...
            const shouldFallbackFromCloudProxy = (status: number): boolean => status === 400 ||
                status === 401 ||
                status === 403 ||
                status === 404 ||
                status === 405 ||
            ...
            const fetchViaBestAvailableProxy = async (): Promise<Response> => {
                ...
                try {
                    const cloudRes = await observeVoiceRequest(task.evidence, "cloud-proxy", proxyRequestBody, () => fetchWithCsrf(cloudTarget, makeProxyRequestInit(), {
                ...
                return observeVoiceRequest(task.evidence, "elevenlabs-proxy", proxyRequestBody, () => fetchWithCsrf(resolveApiUrl("/api/tts/elevenlabs"), makeProxyRequestInit(), {
                    responseType: "arraybuffer",
                    timeoutMs: CLOUD_TTS_TIMEOUT_MS,
                }));
            };
            ...
            if (hasDirectKey) {
                try {
                    ...
                    res = await observeVoiceRequest(task.evidence, "elevenlabs-direct", JSON.stringify(requestBody), () => fetch(url.toString(), {
                        method: "POST",
                        headers: {
            ...
            if (!res.ok) {
                ...
                throw new Error(`ElevenLabs ${res.status}: ${formatVoiceErrorPreview(body, 200)}`);
            }
    ...
    const prepareElizaCloud = useCallback(async (text: string, task: QueuedSpeechTask, controller: AbortController) => {
        const cacheKey = task.cacheKey ??
            (shouldCacheGeneratedSpeech(text, task.segment)
        ...
        if (!audioBytes) {
            const timeoutId = setTimeout(() => {
                controller.abort(new DOMException("Eliza Cloud TTS timed out", "TimeoutError"));
            ...
            try {
                ...
                const debugHeaders: Record<string, string> = isTtsDebugEnabled() && dbg
                    ? {
                        "x-elizaos-tts-message-id": encodeURIComponent(dbg.messageId),
                        "x-elizaos-tts-clip-segment": encodeURIComponent(task.segment),
                        "x-elizaos-tts-full-preview": encodeURIComponent(dbg.fullAssistTextPreview),
                ...
                const fetchViaProxy = (url: string, bearer: string | null) => observeVoiceRequest(task.evidence, "cloud-proxy", JSON.stringify({ text }), () => fetchWithCsrf(url, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                ...
                if (route.via === "direct-cloud") {
                    ...
                    try {
                        ...
                        // preflight passes.
                        const directRes = await observeVoiceRequest(task.evidence, "cloud-direct", JSON.stringify({
                            text,
                            ...(route.voiceId ? { voiceId: route.voiceId } : {}),
                            ...(route.modelId ? { modelId: route.modelId } : {}),
                        }), () => requestViaAgentTransport(route.url, {
                            method: "POST",
                            headers: {
                                "Content-Type": "application/json",
                                "Idempotency-Key": ttsUtteranceKey,
                        ...
                        if (!directRes.ok) {
                            ...
                            throw new Error(`direct cloud TTS ${directRes.status}: ${formatVoiceErrorPreview(preview, 120)}`);
                        }
                        res = directRes;
                    }
                    catch (error) {
            ...
            if (!res.ok) {
                ...
                throw new Error(`Eliza Cloud TTS ${res.status}: ${formatVoiceErrorPreview(body, 200)}`);
            }
            selectedResponse = res;
    ...
    // ── Local inference TTS ───────────────────────────────────────────────
    const prepareLocalInference = useCallback(async (text: string, task: QueuedSpeechTask, controller: AbortController) => {
        const cacheKey = task.cacheKey ??
            (shouldCacheGeneratedSpeech(text, task.segment)
        ...
        if (!audioBytes) {
            const timeoutId = setTimeout(() => {
                controller.abort(new DOMException("Local inference TTS timed out", "TimeoutError"));
            }, LOCAL_INFERENCE_TTS_TIMEOUT_MS);
            let res: Response;
            try {
                res = await observeVoiceRequest(task.evidence, "local-inference", JSON.stringify({ text }), () => fetchWithCsrf(resolveApiUrl("/api/tts/local-inference"), {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            if (!res.ok) {
                const body = await res.text().catch(() => "");
                throw new Error(`Local inference TTS ${res.status}: ${formatVoiceErrorPreview(body, 200)}`);
            }
            selectedResponse = res;
            audioBytes = new Uint8Array(await res.arrayBuffer());
            if (cacheKey) {
    ...
    const speakBrowser = useCallback((text: string, task: SpeakTask, generation: number) => {
    ...
    const prepareBufferedSpeech = useCallback(async (task: QueuedSpeechTask, generation: number, lookahead: boolean): Promise<PreparedSpeech> => {
        ...
        try {
            ...
            const prepared = provider === "elevenlabs" && task.voiceConfig?.elevenlabs
                ? await prepareElevenLabs(task.text, task.voiceConfig.elevenlabs, task, controller)
                : provider === "eliza-cloud"
                    ...
                    : provider === "local-inference"
                        ...
                        : null;
            if (!prepared)
                throw new ElizaError("Speech provider cannot prepare buffered audio", { code: "VOICE_BUFFERED_PROVIDER_REQUIRED" });
            if (generation !== generationRef.current || controller.signal.aborted)
                throw new DOMException("Speech cancelled", "AbortError");
            const context = sharedAudioCtx ?? new AudioContext({ latencyHint: "interactive" });
            if (!sharedAudioCtx) {
                sharedAudioCtx = context;
                warmPlaybackWorklet(context);
            }
            const audioBuffer = await context.decodeAudioData(toArrayBuffer(prepared.audioBytes));
            if (generation !== generationRef.current || controller.signal.aborted)
                throw new DOMException("Speech cancelled", "AbortError");
    ...
    const prepareNextBufferedSpeech = useCallback(() => {
        if (!bufferedPlaybackActiveRef.current ||
        ...
        const result = prepareBufferedSpeech(task, generation, true).then<SpeechPreparation, SpeechPreparation>((audio) => ({ ok: true, audio }), (error) => ({ ok: false, error }));
        prefetchedSpeechRef.current = { task, generation, result };
    }, [prepareBufferedSpeech]);
    const playBufferedSpeech = useCallback(async (task: QueuedSpeechTask, generation: number) => {
        ...
        if (generation !== generationRef.current)
            return;
        await ensurePlaybackContextRunning(audio.context, audio.provider, markAudioBlocked);
        if (generation !== generationRef.current)
            return;
        const audioBuffer = audio.data.kind === "decoded"
            ? audio.data.audioBuffer
            : await audio.context.decodeAudioData(toArrayBuffer(audio.data.audioBytes));
        if (generation !== generationRef.current)
            return;
        const bufferId = audio.data.kind === "decoded"
            ? audio.bufferId
    ...
    const processQueue = useCallback(() => {
        ...
        void (async () => {
            ...
            const failClosed = (engine: VoiceTtsError["engine"], error: unknown): void => {
                const message = error instanceof Error ? error.message : String(error);
                reportRendererDiagnostic({
    ...
    const enqueueSpeech = useCallback((task: SpeakTask) => {
        ...
        const queuedTask: QueuedSpeechTask = {
            ...
            telemetry: task.telemetry
                ? {
                    ...task.telemetry,
                    queuedAtMs: typeof performance !== "undefined"
                        ? performance.now()
                        : Date.now(),
                }
                : undefined,
        ...
        if (options.onPlaybackEvidence &&
            !Capacitor.isNativePlatform() &&
            (provider === "eliza-cloud" ||
                provider === "local-inference" ||
                (provider === "elevenlabs" && queuedTask.voiceConfig?.elevenlabs))) {
            const evidence = new BufferedVoiceEvidence(options.onPlaybackEvidence, () => observedTasksRef.current.delete(evidence));
            ...
            queueRef.current.push(queuedTask);
        if (enqueueGeneration !== generationRef.current)
            return;
    ...
    // ── Public speak APIs ─────────────────────────────────────────────
    const speak = useCallback((text: string, speakOptions?: Parameters<VoiceChatState["speak"]>[1]) => {
        if (assistantTtsDebounceRef.current != null) {
            clearTimeout(assistantTtsDebounceRef.current);
            assistantTtsDebounceRef.current = null;
        }
    ...
    const flushPendingAssistantTts = useCallback(() => {
        ...
        if (!state || state.finalQueued || state.revisionRejected)
            return;
        const latest = state.latestSpeakable;
        if (!latest)
            return;
        const stable = stableSpeechPrefix(latest);
        if (stable.length <= state.queuedSpeakablePrefix.length)
            return;
        ...
        const cacheKey = voiceConfigRef.current?.provider === "elevenlabs" &&
            elConfig &&
    ...
    const queueAssistantSpeech = useCallback((messageId: string, text: string, isFinal: boolean, queueOptions?: QueueAssistantSpeechOptions) => {
        if (!messageId)
            return;
        ...
        const promotesActiveMessage = current != null &&
        ...
        const state = assistantSpeechRef.current;
        if (!state || state.revisionRejected)
            return;
        if (state.queuedSpeakablePrefix &&
        ...
        const boundaryUnsent = boundaryPrefix.length > state.queuedSpeakablePrefix.length
        ...
        const targetPrefix = boundaryUnsent
            ...
            : thresholdFlush
                ...
                : "";
        const unsent = targetPrefix.length > state.queuedSpeakablePrefix.length
