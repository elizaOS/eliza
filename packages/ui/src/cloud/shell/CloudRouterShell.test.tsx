...
import { CLOUD_MANAGEMENT_COMPAT_REDIRECTS } from "./CloudRouterShell";
import { LEGACY_DASHBOARD_REDIRECTS } from "./CloudRouterShell";
...
import { resolveLegacyCloudSettingsTarget } from "./CloudRouterShell";
import { savePersistedActiveServer } from "../../state/persistence";
import { screen } from "@testing-library/react";
import { setPrivateCloudLoadForTests } from "../private-cloud-registration";
...
function renderCatchAll(initialPath = "/"): void {
    render(<MemoryRouter initialEntries={[initialPath]}>
      <Routes>
        ...
        <Route path="*" element={<AppCatchAllRoute appElement={<div data-testid="agent-app"/>}/>}/>
      </Routes>
    </MemoryRouter>);
}
    ...
    render(<QueryClientProvider client={queryClient}>
      <MemoryRouter initialEntries={[initialPath]}>
        <Routes>
          <Route path="/login" element={<div data-testid="login-page"/>}/>
          <Route path="/cloud" element={<div data-testid="console-home"/>}/>
          <Route path="/cloud/agents" element={<div data-testid="instances-page"/>}/>
...
function installFetchRecorder(respond?: (url: string, init?: RequestInit) => Response): void {
    fetchLog = [];
...
describe("CloudRouterShell app-mode catch-all (app.elizacloud.ai)", () => {
    ...
    it("sends an unauthenticated app-host visitor through the login flow, not the agent app", async () => {
        ...
        expect(await screen.findByTestId("login-page", {}, { timeout: 5000 })).toBeTruthy();
        expect(screen.queryByTestId("agent-app")).toBeNull();
        expect(fetchLog).toEqual([]);
        await flushMicrotasks();
...
describe("CloudRouterShell retired dashboard redirects", () => {
    it("keeps every shell redirect in parity with the shared edge contract", () => {
        ...
        for (const { from, to } of LEGACY_DASHBOARD_REDIRECTS) {
            ...
            expect(canonicalCloudPathForLegacyDashboard(concreteSource), concreteSource).toBe(concreteTarget);
        }
        for (const tab of ["connections", "billing", "organization", "agents"]) {
            const search = `?tab=${encodeURIComponent(tab)}&return=1`;
            expect(resolveLegacyCloudSettingsTarget(search), tab).toBe(canonicalCloudPathForLegacyDashboard("/dashboard/settings", search));
        }
        const unknownSearch = "?tab=unknown&return=1";
        expect(canonicalCloudPathForLegacyDashboard("/dashboard/settings", unknownSearch)).toBe(resolveLegacyCloudSettingsTarget(unknownSearch));
    });
    it("lets the generic dashboard fallback own direct surface migrations", () => {
        ...
        for (const r of LEGACY_DASHBOARD_REDIRECTS) {
            expect(standalone.has(r.from), `unexpected redirect for ${r.from}`).toBe(false);
        }
    });
    it("resolves legacy earnings + affiliates links to the monetization console page", () => {
        const targets = Object.fromEntries(LEGACY_DASHBOARD_REDIRECTS.map((r) => [r.from, r.to]));
    ...
    it("keeps retired cloud earnings aliases on the canonical monetization page", () => {
        const targets = Object.fromEntries(CLOUD_MANAGEMENT_COMPAT_REDIRECTS.map((route) => [route.from, route.to]));
    ...
    it("maps legacy settings tabs to canonical managed Cloud pages", () => {
        expect(resolveLegacyCloudSettingsTarget("?tab=connections")).toBe("/cloud/connectors");
        expect(resolveLegacyCloudSettingsTarget("?tab=billing&payment=success")).toBe("/cloud/billing");
        expect(resolveLegacyCloudSettingsTarget("?tab=organization")).toBe("/cloud/organization");
        expect(resolveLegacyCloudSettingsTarget("?tab=agents")).toBe("/cloud/agents");
