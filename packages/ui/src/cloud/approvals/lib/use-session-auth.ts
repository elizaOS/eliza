/**
 * Synchronous "is the owner logged in" answer for the in-app Approvals pane.
 *
 * Copied from the documents domain's `use-session-auth.ts` (which itself was
 * ported from `@elizaos/cloud-frontend/src/hooks/use-session-auth.ts`). Reads
 * the Steward auth context exposed by the cloud shell
 * (`LocalStewardAuthContext` in `../../shell/StewardProvider`) and falls back to
 * decoding the JWT directly from `localStorage` so the pane can gate on
 * `{ ready, authenticated, user }` without forcing the heavy `@stwd/*` runtime
 * to mount.
 *
 * Scoped to the approvals domain. The shell can promote this to shared cloud
 * infra once enough domains share it; until then each domain keeps its own copy
 * (per the documents-domain precedent + the migration scope rules).
 */
