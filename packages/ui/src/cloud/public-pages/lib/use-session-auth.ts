/**
 * Steward session-auth hook for the app-hosted cloud public pages.
 *
 * Ported from `@elizaos/cloud-frontend/src/hooks/use-session-auth.ts`. Reads the
 * Steward auth context provided by the shell's {@link LocalStewardAuthContext}
 * (which the `CloudRouterShell` wraps non-public routes in), with a localStorage
 * fallback so public/token pages still see a logged-in session even when the
 * heavy `@stwd/*` runtime hasn't mounted. Returns a fallback when no provider is
 * present (Rules-of-Hooks-safe: reads the context directly, no try/catch).
 */
 ...
 * provider is mounted (public token routes render without the Steward runtime).
 */
