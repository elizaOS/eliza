...
const SECOND_AGENT_BASE = "https://agent2.elizacloud.ai";
const PAIRING_TOKEN_URL =
  "https://api.elizacloud.ai/api/v1/eliza/agents/agent1/pairing-token";
const SECOND_PAIRING_TOKEN_URL =
  "https://api.elizacloud.ai/api/v1/eliza/agents/agent2/pairing-token";
const PAIR_REDIRECT_URL = `${AGENT_BASE}/pair?token=one-time-t`;
const SECOND_PAIR_REDIRECT_URL = `${SECOND_AGENT_BASE}/pair?token=second-one-time-t`;
...
describe("repairDedicatedAgentCredential", () => {
  ...
  it("scopes the completed-result cache by agent base", async () => {
    const firstClient = new ElizaClient(AGENT_BASE);
    const secondClient = new ElizaClient(SECOND_AGENT_BASE);
    localStorage.setItem(STEWARD_TOKEN_KEY, makeStewardJwt(3600));

    const seen = installFetchRouter([
      mintRoute(() => jsonResponse(401, { error: "Unauthorized" })),
      {
        match: (url) => url.startsWith(SECOND_PAIRING_TOKEN_URL),
        respond: () =>
          jsonResponse(200, {
            success: true,
            data: {
              token: "second-one-time-t",
              redirectUrl: SECOND_PAIR_REDIRECT_URL,
            },
          }),
      },
      {
        match: (url) => url.startsWith(`${SECOND_AGENT_BASE}/pair`),
        respond: () => jsonResponse(200, { apiKey: "agent2_fresh_key" }),
      },
    ]);

    await expect(repairDedicatedAgentCredential(firstClient)).resolves.toBe(
      false,
    );
    await expect(repairDedicatedAgentCredential(secondClient)).resolves.toBe(
      true,
    );
    expect(seen.map((request) => request.url)).toEqual([
      PAIRING_TOKEN_URL,
      SECOND_PAIRING_TOKEN_URL,
      expect.stringContaining(`${SECOND_AGENT_BASE}/pair`),
    ]);
    expect(secondClient.getRestAuthToken()).toBe("agent2_fresh_key");
  });
});
