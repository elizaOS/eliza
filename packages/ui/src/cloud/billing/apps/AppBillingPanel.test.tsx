...
describe("independent app subscription UI", () => {
  it("restores subscription controls after returning from the billing portal", async () => {
    ...
    fixture.snapshot = {
      ...
      subscription: {
        ...
        appId: fixture.catalog.appId,
        environment: "test",
        billingAccountId: fixture.snapshot.account.id,
        productFamilyKey: "workspace",
    ...
    await user.click(
      await screen.findByRole("button", {
        name: "Payment methods and invoices",
      }),
    );
    await user.click(
      await screen.findByRole("button", { name: "Done with portal · Refresh" }),
    );
    await waitFor(() =>
      expect(
        screen
