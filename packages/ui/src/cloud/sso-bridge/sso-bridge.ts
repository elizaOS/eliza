...
import { clearStaleStewardSession } from "../shell/StewardProviderShared";
import { configuredSessionEndpoint } from "../shell/StewardProviderShared";
...
function pairForHostname(hostname: string): SsoBridgePair | null {
    ...
    for (const pair of SSO_BRIDGE_PAIRS) {
        if (pair.appHost === host || pair.mintHosts.includes(host))
            return pair;
    }
    return null;
}
...
export function pairedAppOrigin(mintHostname: string): string | null {
    ...
    if (!pair || pair.appHost === mintHostname.toLowerCase())
        return null;
 ...
 */
export function sanitizeBridgeReturnTo(value: string | null | undefined): string {
    if (!value || value.length > RETURN_TO_MAX_LENGTH)
        return "/";
    if (!value.startsWith("/"))
        return "/";
 ...
 */
export function buildSsoBridgeErrorUrl(reason: "auth_failed" | "sync_failed", returnTo: string): string {
 ...
 */
export function pairedAppLoginUrlForMintHost(hostname: string, returnTo: string): string | null {
...
export function isWellFormedSsoState(value: string | null | undefined): value is string {
    return typeof value === "string" && SSO_STATE_RE.test(value);
}
/** Challenge/verifier share the state's 64-hex shape (32 random bytes). */
export function isWellFormedSsoChallenge(value: string | null | undefined): value is string {
 ...
 */
export function buildBridgeMintUrl(appHostname: string, state: string, challenge: string, returnTo: string): string | null {
    const pair = pairForHostname(appHostname);
    if (!pair || pair.appHost !== appHostname.toLowerCase())
        return null;
...
export function buildBridgeExchangeUrl(mintHostname: string, code: string, state: string, returnTo: string): string | null {
    const pair = pairForHostname(mintHostname);
    if (!pair || pair.appHost === mintHostname.toLowerCase())
        return null;
    if (!isWellFormedSsoState(state))
 ...
 */
export function shouldAutoBridgeToSso(hostname: string = window.location.hostname, now: number = Date.now()): boolean {
    if (ssoBridgeRoleForHostname(hostname) !== "exchange")
        return false;
    if (isSsoLoggedOut())
        return false;
 ...
 * the ordinary login.
 */
export async function redirectToSsoBridge(returnTo: string, hostname: string = window.location.hostname): Promise<boolean> {
    const handshake = await createSsoBridgeHandshake();
    if (!handshake)
        return false;
    const url = buildBridgeMintUrl(hostname, handshake.state, handshake.challenge, returnTo);
    if (!url)
        return false;
...
export function isWellFormedSsoCode(value: string | null | undefined): value is string {
 ...
 */
export async function mintSsoCode(hostname: string, challenge: string, fetchFn: typeof fetch = fetch): Promise<SsoMintResult> {
    ...
    if (!token)
        return { ok: false, error: "No local session" };
    try {
 ...
 */
export async function performSsoExchange(code: string, verifier: string, hostname: string, fetchFn: typeof fetch = fetch): Promise<SsoExchangeResult> {
    const base = apiBaseForHostname(hostname);
    if (!base)
        return { ok: false, error: "Host cannot exchange SSO codes" };
 ...
 */
export function burnSsoBridgeCode(code: string, hostname: string = window.location.hostname, fetchFn: typeof fetch = fetch): void {
    const base = apiBaseForHostname(hostname);
    if (!base || !isWellFormedSsoCode(code))
        return;
    try {
 ...
 */
export async function signOutFromSsoBridgedHost(hostname: string = window.location.hostname, fetchFn: typeof fetch = fetch): Promise<void> {
    ...
    if (response && !response.ok) {
        throw new ElizaError(`Eliza Cloud could not end the browser session (${response.status}).`, {
 ...
 * account again.
 */
export async function prepareSsoAccountSwitch(hostname: string = window.location.hostname, fetchFn: typeof fetch = fetch): Promise<void> {
    const stewardToken = readStoredStewardToken();
    invalidateStewardServerCookieSyncMarker();
    markSsoLoggedOut();
    const base = apiBaseForHostname(hostname);
    if (!base) {
        throw new Error("Eliza Cloud account switching is unavailable on this host.");
    }
    ...
    const response = await serverLogout;
    if (!response.ok) {
        throw new Error(`Eliza Cloud could not end the previous browser session (${response.status}).`);
    }
}
