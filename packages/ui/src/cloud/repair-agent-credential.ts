...
const inflightRepairs = new Map<string, Promise<boolean>>();
const completedRepairs = new Map<
  string,
  { completedAt: number; result: boolean }
>();
...
function repairCacheKey(targetClient: ElizaClient): string {
  return (
    targetClient.getBaseUrl().trim() ||
    (typeof window !== "undefined" ? window.location.origin : "")
  );
...
export async function repairDedicatedAgentCredential(
  ...
  const cacheKey = repairCacheKey(targetClient);
  const inflightRepair = inflightRepairs.get(cacheKey);
  if (inflightRepair) return inflightRepair;

  const completedRepair = completedRepairs.get(cacheKey);
  if (
    completedRepair &&
    Date.now() - completedRepair.completedAt < REPAIR_RESULT_CACHE_MS
  ) {
    return completedRepair.result;
  }
  ...
  const repair = (async () => {
    ...
    .finally(() => {
      if (inflightRepairs.get(cacheKey) === repair) {
        inflightRepairs.delete(cacheKey);
      }
    });

  inflightRepairs.set(cacheKey, repair);
  return repair;
}
