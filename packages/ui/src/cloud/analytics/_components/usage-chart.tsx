...
import { ChartContainer } from "../../../cloud-ui";
import { ChartTooltip } from "../../../cloud-ui";
import { ChartTooltipContent } from "../../../cloud-ui";
import { XAxis } from "recharts";
import { YAxis } from "recharts";
import { format } from "date-fns";
import { formatUsd } from "@elizaos/ui/utils/format";
...
export function UsageChart({ data, granularity }: UsageChartProps) {
    ...
    return (<div className="flex flex-col gap-7 lg:gap-8">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          {(Object.keys(chartConfig) as MetricKey[]).map((metric) => {
            ...
            return (<Button key={metric} variant="selection" size="sm" shape="circle" data-state={isActive ? "on" : "off"} onClick={() => setActiveMetric(metric)}>
      ...
      <ChartContainer config={chartConfig} className="h-[340px] w-full rounded-sm border border-border/70 bg-background/70 p-5 sm:p-6">
        <AreaChart data={chartData}>
          ...
          <ChartTooltip cursor={{ strokeDasharray: "4 4" }} content={<ChartTooltipContent hideIndicator formatter={(value) => {
          ...
          <Area type="monotone" dataKey={activeMetric} stroke={activeColor} fill={`url(#fill-${activeMetric})`} strokeWidth={2} dot={false}/>
        </AreaChart>
      </ChartContainer>
    </div>);
}
