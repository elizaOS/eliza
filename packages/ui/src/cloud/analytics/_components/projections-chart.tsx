...
import { Alert } from "../../../cloud-ui";
import { AlertDescription } from "../../../cloud-ui";
import { AlertTitle } from "../../../cloud-ui";
import { AlertTriangle } from "lucide-react";
import { Area } from "recharts";
import { AreaChart } from "recharts";
import { Badge } from "../../../cloud-ui";
import { Card } from "../../../cloud-ui";
import { CardContent } from "../../../cloud-ui";
import { CardHeader } from "../../../cloud-ui";
import { CardTitle } from "../../../cloud-ui";
...
import { formatUsd as formatCurrency } from "@elizaos/ui/utils/format";
...
export function ProjectionsChart({ data }: ProjectionsChartProps) {
    ...
    const lastHistoricalDate = todayIndex >= 0
    ...
    return (<div className="space-y-6">
      <Card variant="reportPanel">
        <CardHeader className="flex flex-col gap-3 p-6 pb-5">
          <div className="flex items-center gap-3">
            ...
            <Badge variant="outline" className="rounded-full text-xs" title={t("cloud.projectionsChart.predictiveAnalytics", {
        ...
        <CardContent className="border-t border-border/60 p-6">
          <div className="space-y-6">
              ...
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="fill-historical" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <stop offset="95%" stopColor={chartConfig.historical.color} stopOpacity={0.05}/>
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" vertical={false}/>
                <XAxis dataKey="date" tickLine={false} axisLine={false} minTickGap={24}/>
                <YAxis tickLine={false} axisLine={false} width={70} tickFormatter={(value) => {
            if (value >= 1000) {
            ...
            return `$${value.toFixed(0)}`;
                ...
                if (source &&
                    ...
                    if (isProjected && confidence) {
                        return t("cloud.projectionsChart.confidenceLabel", {
                            date: fullDate,
                            confidence,
                            defaultValue: "{{date}} ({{confidence}}% confidence)",
                        });
                    }
                    return fullDate;
                }
                ...
                <Area type="monotone" dataKey="cost" stroke={chartConfig.historical.color} fill="url(#fill-historical)" strokeWidth={2} dot={false} name={t("cloud.projectionsChart.cost", {
            defaultValue: "Cost",
        })}/>

                <Line type="monotone" dataKey="cost" stroke={chartConfig.projected.color} strokeWidth={2} strokeDasharray="5 5" dot={false} connectNulls={false}/>

                {lastHistoricalDate && (<ReferenceLine x={lastHistoricalDate} stroke="var(--muted)" strokeDasharray="2 2" label={{
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs">
              <div className="flex items-center gap-2">
                <div className="size-3 rounded-full" style={{ backgroundColor: chartConfig.historical.color }}/>
            ...
            defaultValue: "Historical data",
              ...
              <div className="flex items-center gap-2">
                <div className="size-3 rounded-full opacity-70" style={{ backgroundColor: chartConfig.projected.color }}/>
                <span className="text-muted-foreground">
      ...
      {alerts.length > 0 && (<Card variant="reportPanel">
          ...
          <CardContent className="border-t border-border/60 p-6">
            <div className="space-y-3">
              {alerts.map((alert, index) => {
                ...
                return (<Alert key={alert.eventId ?? index} data-alert-event-id={alert.eventId} data-alert-severity={severity} variant={getAlertVariant(alert.type)}>
                    <Icon className="size-4"/>
                    <AlertTitle>{alert.title}</AlertTitle>
                    <AlertDescription className="mt-2">
                      {alert.message}
                      {alert.projectedValue !== undefined && (<span className="ml-2 font-medium">
