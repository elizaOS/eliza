...
import { parsePersonalWorkspaceCapabilityHandoff } from "@elizaos/core/capability-catalog";
...
export function parseCapabilityHandoff(value: unknown, expectedAgentId?: string | null): CapabilityHandoffRequest | null {
...
export function findCapabilityHandoff(actionResults: readonly ChatActionResultSummary[] | undefined, expectedAgentId?: string | null): CapabilityHandoffRequest | null {
    if (!Array.isArray(actionResults))
        return null;
    for (let index = actionResults.length - 1; index >= 0; index -= 1) {
        const parsed = parseCapabilityHandoff(actionResults[index]?.values?.capabilityHandoff, expectedAgentId);
        if (parsed)
            return parsed;
    }
    return null;
...
function writeStored(key: string, handoff: CapabilityHandoffRequest, storage?: Storage): void {
    try {
        const target = storageOrNull(storage);
        if (!target)
            return;
        runAsPrivilegedShell(() => target.setItem(key, JSON.stringify({
...
function readStored(key: string, storage?: Storage, expectedAgentId?: string | null): CapabilityHandoffRequest | null {
    const target = storageOrNull(storage);
    if (!target)
        return null;
    try {
...
export function rememberCapabilityHandoff(messageId: string, handoff: CapabilityHandoffRequest, storage?: Storage): void {
    if (!messageId.trim())
        return;
...
export function restoreCapabilityHandoffs(messages: readonly ConversationMessage[], storage?: Storage, expectedAgentId?: string | null): ConversationMessage[] {
    let changed = false;
    const restored = messages.map((message) => {
        if (message.capabilityHandoff) {
            const parsed = parseCapabilityHandoff(message.capabilityHandoff, expectedAgentId);
            if (parsed) {
        ...
        const handoff = readStored(`${STORED_HANDOFF_PREFIX}${message.id}`, storage, expectedAgentId);
        if (!handoff || message.role !== "assistant")
            return message;
        changed = true;
...
export function rememberPendingCapabilityHandoff(handoff: CapabilityHandoffRequest, storage?: Storage): void {
...
export function markPendingCapabilityReady(agentId: string, storage?: Storage): boolean {
    const target = storageOrNull(storage);
    ...
    try {
        runAsPrivilegedShell(() => target.setItem(PENDING_READY_AGENT_KEY, agentId));
        return true;
    }
    catch {
...
export function readPendingCapabilityReadyAgentId(storage?: Storage): string | null {
    const target = storageOrNull(storage);
    if (!target)
        return null;
    try {
        const readyAgentId = boundedText(target.getItem(PENDING_READY_AGENT_KEY), 256);
        if (!readyAgentId)
            return null;
...
export function consumePendingCapabilityIntent(agentId: string, storage?: Storage): string | null {
    const target = storageOrNull(storage);
    const handoff = readStored(PENDING_HANDOFF_KEY, storage);
    const readyAgentId = readPendingCapabilityReadyAgentId(storage);
    if (!target || !handoff || readyAgentId !== agentId)
        return null;
    ...
    if (sourceAgentId !== agentId || !originalIntent)
        return null;
    try {
        runAsPrivilegedShell(() => {
            target.removeItem(PENDING_HANDOFF_KEY);
