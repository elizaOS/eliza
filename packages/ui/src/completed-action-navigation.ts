...
export function captureCompletedActionNavigationFence(): () => boolean {
    ...
    return () => observedWindow === initialWindow &&
...
function rememberPendingHandoff(id: string, snapshot: {
    navigationEpoch: number;
    path: string;
}): void {
    pendingHandoffs.delete(id);
...
export function dispatchCompletedActionNavigation(detail: NavigateViewDetail): boolean {
    if (typeof window === "undefined")
        return false;
    observeNavigationEpoch();
    const id = normalizeCompletedActionHandoffId(detail.completedActionHandoffId);
    if (id && handledHandoffs.has(id))
        return false;
    ...
    if (id) {
        if (event.defaultPrevented)
            ...
            rememberPendingHandoff(id, snapshot);
    }
    return true;
}
/** Mark a handoff handled from a shell listener before performing navigation. */
export function markCompletedActionNavigationHandled(event: Event, detail: NavigateViewDetail | undefined): void {
    if (!event.cancelable ||
