...
import { isRealtimeVoiceFlagEnabled } from "../../hooks/useRealtimeVoiceSession";
import { isTranscriptionExitPhrase } from "../../voice/transcription-exit";
import { isTranscriptionStartPhrase } from "../../voice/transcription-exit";
...
import { queryMicrophonePermission } from "../../voice/local-asr-capture";
...
import { stripExitPhrase } from "../../voice/transcription-exit";
...
import { type ConversationNavDirection } from "./conversation-nav";
...
import { type VoiceCaptureBackend } from "../../voice/voice-capture-factory";
import { type VoiceCaptureHandle } from "../../voice/voice-capture-factory";
import { type VoiceCaptureState } from "../../voice/voice-capture-factory";
...
import { useChatComposer } from "../../state";
import { useChatTurnStatus } from "../../state";
import { useConversationMessages } from "../../state";
...
export type { ConversationNav, ConversationNavDirection, } from "./conversation-nav";
export { buildConversationNav, resolveAdjacentConversationId, } from "./conversation-nav";
...
export interface ShellController {
    ...
    setTranscriptSessionSink: (sink: ((segments: TranscriptSegment[], startedAtMs: number, audioWav: Uint8Array | null) => void) | null) => void;
...
function isMicPermissionDenialError(err: unknown): boolean {
    ...
    return (haystack.includes("notallowed") ||
...
export function describeCaptureFailure(err: unknown): string {
    ...
    if (haystack.includes("cloudstterror") ||
...
export function isNoSpeechCaptureFailure(err: unknown): boolean {
    ...
    return (normalized.includes("no microphone audio was captured") ||
...
function describeRealtimeVoiceFailure(outcome: Exclude<RealtimeVoiceStartOutcome, {
    kind: "live";
}>, surfacedError: string | null): string {
...
export function useShellController(): ShellController {
    const { tab, chatFirstTokenReceived, sendChatText, sendActionMessage, agentStatus, characterData, uiLanguage, elizaCloudConnected, elizaCloudVoiceProxyAvailable, handleNewConversation, ensureActiveConversation, handleSelectConversation, activeConversationId, conversations, setTab, handleChatStop, setActionNotice, chatAgentVoiceMuted, setState, handleInteractiveCloudLogin, } = useAppSelectorShallow(selectShellController);
    ...
    const requestSignIn = React.useCallback(() => {
        ...
        void handleInteractiveCloudLogin()
            ...
            setActionNotice(error instanceof Error
                ? error.message
                : "Could not start Cloud login.", "error", 5000);
        })
            .finally(() => {
    ...
    const recoverGatedCapture = React.useCallback(() => {
        ...
        setActionNotice(authGate.phase === "unavailable"
            ? "Unable to reach Eliza Cloud. Retrying the connection."
            : "Checking your Eliza Cloud session. Try push-to-talk again in a moment.", authGate.phase === "unavailable" ? "error" : "info", 4000);
    ...
    const wakeCharacterName = characterData?.name?.trim() || agentStatus?.agentName?.trim() || "eliza";
    ...
    const handleRealtimeVoiceServerEvent = React.useCallback((event: ServerControlFrame) => {
    ...
    const { agentId: realtimeVoiceAgentId, getConsentNonce } = useRealtimeVoiceMint({ conversationId: activeConversationId });
    ...
    const realtimeVoiceCapabilityEnabled = realtimeVoiceBuildEnabled && Boolean(realtimeVoiceAgentId);
    ...
    const realtimeVoiceEnabled = realtimeVoiceCapabilityEnabled && realtimeVoice.available;
    ...
    const realtimeVoiceCanStart = realtimeVoiceEnabled ||
        (realtimeVoiceCapabilityEnabled && !activeConversationId?.trim());
    const [realtimeVoiceBatchFallback, setRealtimeVoiceBatchFallback] = React.useState(!realtimeVoiceEnabled);
    const realtimeVoiceSelected = realtimeVoiceEnabled && !realtimeVoiceBatchFallback;
    ...
    const realtimeVoiceOwnsMedia = realtimeVoiceBuildEnabled &&
    ...
    const [realtimeVoiceBoundaryError, setRealtimeVoiceBoundaryError] = React.useState<string | null>(null);
    const conversationIdentityWaitersRef = React.useRef(new Set<{
    ...
    const ensureActiveConversationForVoice = React.useCallback(async () => {
        ...
        if (!conversationId || !realtimeVoiceWantedRef.current)
            return null;
    ...
    React.useEffect(() => {
        ...
        for (const waiter of conversationIdentityWaitersRef.current) {
            ...
            waiter.resolve(committedId === waiter.conversationId ? committedId : null);
        }
    ...
    const openSettings = React.useCallback(() => {
        if (isElectrobunRuntime()) {
            void openDesktopSettingsWindow().catch((error: unknown) => {
                ...
                setActionNotice(`Settings could not open. ${error instanceof Error ? error.message : String(error)}`, "error", 6000);
            });
            return;
        }
        setTab("settings");
    ...
    const runWithConversationLoading = React.useCallback((task: () => Promise<unknown>) => {
        ...
        const watchdog = setTimeout(clearLoadingForSeq, CONVERSATION_LOADING_MAX_MS);
    ...
    const selectAdjacentConversation = React.useCallback((direction: ConversationNavDirection) => {
        if (conversationTransitionBusyRef.current) {
            return;
        }
        const targetId = resolveAdjacentConversationId(conversationsRef.current, activeConversationIdRef.current, direction);
        if (targetId) {
    ...
    const conversationNav = React.useMemo<ConversationNav>(() => {
        const nav = buildConversationNav(conversations, activeConversationId, selectConversation);
        return {
            ...nav,
    ...
    const [micPermission, setMicPermission] = React.useState<MicrophonePermissionState>("unknown");
    ...
    const setDictationSink = React.useCallback((sink: ((text: string) => void) | null) => {
    ...
    const transcriptSessionRef = React.useRef<TranscriptSessionAccumulator | null>(null);
    const transcriptSessionStartRef = React.useRef(0);
    const onTranscriptSessionRef = React.useRef<((segments: TranscriptSegment[], startedAtMs: number, audioWav: Uint8Array | null) => void) | null>(null);
    const setTranscriptSessionSink = React.useCallback((sink: ((segments: TranscriptSegment[], startedAtMs: number, audioWav: Uint8Array | null) => void) | null) => {
    ...
    const beginTranscriptSession = React.useCallback(() => {
        ...
        transcriptSessionRef.current = new TranscriptSessionAccumulator(transcriptSessionStartRef.current);
    }, []);
    ...
    const finalizeTranscriptSession = React.useCallback(() => {
        ...
        if (!session || session.count === 0)
            return;
        onTranscriptSessionRef.current?.(session.build(), transcriptSessionStartRef.current, session.buildAudioWav());
    }, []);
    ...
    const shellMessageCacheRef = React.useRef<Map<string, ShellMessage>>(new Map());
    ...
    React.useEffect(() => {
        if (!recordReplyIntoTranscriptRef.current)
            return;
        if (!transcriptionModeRef.current)
            return;
        if (chatSending)
            return; // wait for the reply to finish streaming
    ...
    const send = React.useCallback((text: string, options?: {
        channelType?: "DM" | "VOICE_DM";
        images?: ImageAttachment[];
        metadata?: Record<string, unknown>;
        clientMessageId?: string;
        ...
        // attachment to send.
        if (!trimmed && !options?.images?.length)
            return;
    ...
    const sendFirstRunText = React.useCallback((text: string) => {
        const trimmed = text.trim();
        if (!trimmed)
            return;
        void sendActionMessage(trimmed);
    }, [sendActionMessage]);
    const stopCaptureAndDrain = React.useCallback(async (options?: {
        immediateUiReset?: boolean;
    }) => {
    ...
    const stopCapture = React.useCallback((options?: {
        immediateUiReset?: boolean;
    }) => {
    ...
    const startCapture = React.useCallback((intent?: CaptureIntent) => {
        ...
        const aggregator = intent === "dictate" || intent === "transcription" || intent === "ptt"
            ? null
            : new TurnAggregator({
                onCommit: (turn) => {
                    ...
                    const voiceTurnSignal = buildVoiceTurnSignal(turn, respondContext);
                    send(turn, {
                    ...
                    if (wake.matched) {
                        ...
                        send(command, {
                            ...
                            metadata: {
                                ...
                                voiceTurnSignal: buildVoiceTurnSignal(command, respondContext),
                            },
                        });
                        return;
                    }
    ...
    const recheckMicPermission = React.useCallback(async (): Promise<MicrophonePermissionState> => {
        ...
        if (state === "denied") {
            ...
            if (!captureFailureNoticedRef.current) {
                captureFailureNoticedRef.current = true;
                setActionNotice("Microphone access is off. Re-enable microphone permission in your browser or system settings to use voice.", "error", 6000);
            }
    ...
    const gateEngageOnMicPermission = React.useCallback((onProceed: () => void): void => {
        if (micPermissionRef.current !== "denied") {
        ...
        void recheckMicPermission().then((state) => {
            ...
            if (authGateRef.current.gated || captureRef.current)
                return;
            onProceed();
        });
    ...
    React.useEffect(() => {
        ...
        if (realtimeVoiceOwnsMedia)
            return;
        if (autoEngagedHandsFreeRef.current)
            return;
    ...
    React.useEffect(() => {
        if (bootPermissionProbedRef.current)
            return;
    ...
    const realtimeVoiceResponding = realtimeVoiceOwnsMedia &&
        (realtimeVoice.status === "thinking" ||
            realtimeVoice.status === "speaking" ||
            realtimeVoice.status === "interrupting");
    const realtimeVoiceListening = realtimeVoiceOwnsMedia &&
        (realtimeVoice.connecting ||
            realtimeVoice.status === "listening" ||
            realtimeVoice.status === "transcribing");
    const realtimeVoiceRecording = realtimeVoiceOwnsMedia &&
        (realtimeVoice.active || realtimeVoice.connecting);
    const responding = chatSending || voiceOutput.speaking || realtimeVoiceResponding;
    ...
    // no-provider gate.
    const noProviderConfigured = latestAssistantNoProvider && agentStatus?.canRespond === false;
    ...
    React.useEffect(() => {
        ...
        if (noProviderNavigatedRef.current)
            return;
    ...
    const startRealtimeVoice = React.useCallback(async () => {
        ...
        if (!realtimeVoiceCanStart)
            return;
        if (captureRef.current || recording || sttPending)
            return;
        ...
        if (prior !== "always-on")
            priorContinuousModeRef.current = prior;
        ...
        if (authGateRef.current.gated || !realtimeVoiceWantedRef.current)
            return;
        if (!conversationId) {
            const message = "Cartesia voice needs an active conversation. Tap Talk to retry.";
            realtimeVoiceWantedRef.current = false;
            saveContinuousChatMode(priorContinuousModeRef.current);
            setHandsFree(false);
            handsFreeRef.current = false;
        ...
        if (outcome.kind === "live")
            return;
        const message = describeRealtimeVoiceFailure(outcome, realtimeVoiceRef.current.error?.message || null);
        realtimeVoiceWantedRef.current = false;
        saveContinuousChatMode(priorContinuousModeRef.current);
        setHandsFree(false);
        handsFreeRef.current = false;
    ...
    React.useEffect(() => {
        ...
        if (wasEnabled && !realtimeVoiceEnabled) {
            const shouldContinue = realtimeVoiceWantedRef.current ||
                realtimeVoice.active ||
                realtimeVoice.connecting;
            realtimeVoiceWantedRef.current = false;
            realtimeVoiceWasActiveRef.current = false;
            ...
            if (shouldContinue) {
                stopRealtimeVoice();
                const message = "Cartesia voice connection changed. Tap Talk to reconnect.";
                setRealtimeVoiceBoundaryError(message);
                setActionNotice(message, "error", 6000);
            }
        }
        if (!realtimeVoiceEnabled || !realtimeVoiceBatchFallback)
            return;
        if (authGate.gated ||
            handsFree ||
            realtimeVoice.active ||
    ...
    React.useEffect(() => {
        if (realtimeVoice.active)
            realtimeVoiceWasActiveRef.current = true;
    }, [realtimeVoice.active]);
    React.useEffect(() => {
        if (!realtimeVoiceSelected)
            return;
        ...
        if (realtimeVoice.active || realtimeVoice.connecting)
            return;
        if (!realtimeVoiceWasActiveRef.current)
            return;
        if (!realtimeVoiceWantedRef.current)
            return;
        realtimeVoiceWasActiveRef.current = false;
    ...
    React.useEffect(() => {
        if (!realtimeVoiceSelected || autoEngagedHandsFreeRef.current)
            return;
        if (!ready || chatSending || realtimeVoice.connecting)
            return;
        if (loadContinuousChatMode() !== "always-on")
            return;
        autoEngagedHandsFreeRef.current = true;
    ...
    useViewEvent(VOICE_SETTINGS_APPLY_EVENT, React.useCallback((event) => {
    ...
    // ../../voice/VOICE_UX.md.
    const wakeAlreadyAlwaysOn = handsFree && loadContinuousChatMode() === "always-on";
    ...
    useWakeListenWindow({
        ...
        onOpen: React.useCallback(() => {
            ...
            handsFreeRef.current = true;
            voiceOutput.unlockAudio();
            if (!responding && !captureRef.current)
                startCapture("converse");
    ...
    const toggleTranscriptionMode = React.useCallback(async () => {
        ...
        if (transcriptionModeRef.current) {
            ...
            if (captureRef.current)
                await stopCaptureAndDrain();
            if (authGateRef.current.gated || !transcriptionModeRef.current)
                return;
    ...
    React.useEffect(() => {
        ...
        return () => window.removeEventListener(VOICE_CONTROL_EVENT, onVoiceControl);
    }, []);
    ...
    React.useEffect(() => {
        if (!transcriptionMode || !ready)
            return;
        if (recording || captureRef.current)
            return;
        if (chatSending || voiceOutput.speaking)
            return;
    ...
    React.useEffect(() => {
        ...
        const onResume = (): void => {
            ...
            if (realtimeVoiceOwnsMedia)
                return;
            const shouldReArm = (wasCapturingAtSuspendRef.current || handsFreeRef.current) &&
    ...
    const waveformMode = phase === "listening"
        ? "listening"
        : phase === "responding"
            ? "responding"
            : "idle";
    let realtimeVoiceErrorMessage = realtimeVoiceBoundaryError;
    if (!realtimeVoiceErrorMessage &&
        realtimeVoiceOwnsMedia &&
        realtimeVoice.error) {
    ...
    React.useEffect(() => {
        ...
        if (captureRef.current)
            cancelCapture();
        if (authGate.phase === "needs-auth" && isOpen)
            setIsOpen(false);
        stopRealtimeVoiceRef.current();
    }, [authGate.gated, authGate.phase, cancelCapture, isOpen]);
    const startRecording = React.useCallback((intent?: CaptureIntent) => {
        if (authGate.gated) {
            recoverGatedCapture();
            return;
        }
    ...
    return {
        ...
        setComposerHasDraft,
        transcript: realtimeVoice.active || realtimeVoice.connecting
            ? realtimeVoice.status === "listening" ||
                realtimeVoice.status === "transcribing"
        ...
        needsAudioUnlock: (realtimeVoiceOwnsMedia &&
            (realtimeVoice.active || realtimeVoice.connecting) &&
