...
const stubResolver = {
    ...
    setup(b) {
        ...
        // in the whole app-state surface (@elizaos/core, AppContext, …) which is
...
// @elizaos/core: the WidgetHost + the (dead-in-browser) @elizaos/core graph
...
// barrel pulls @elizaos/logger / @elizaos/core, which import node builtins) -
...
async function swipeLeft(locator) {
    ...
    if (!box)
        throw new Error("missing swipe target bounds");
...
async function dragVertical(locator, dy) {
    const box = await locator.boundingBox();
    if (!box)
        throw new Error("missing vertical drag target bounds");
...
async function touchSlowDragLeft(page, testId) {
    ...
    if (!viewportWidth)
        throw new Error("missing viewport width for slow drag");
    await touchSwipe(page, `[data-testid="${testId}"]`, -viewportWidth * 0.35, 0, {
        steps: 10,
...
async function sameTaskFastFlickLeft(page) {
    return page.evaluate(async () => {
        const target = document.querySelector('[data-testid="home-launcher-home-page"]');
...
async function sameTaskVerticalDrag(page) {
    return page.evaluate(async () => {
        ...
        return {
            ...
            railX: !transform || transform === "none"
                ? 0
                : new DOMMatrixReadOnly(transform).m41,
        };
    });
...
async function installCoarsePointerMedia(page) {
    await page.addInitScript(() => {
        ...
        window.matchMedia = (query) => /hover:\s*hover|pointer:\s*fine/.test(query)
            ? coarsePointer(query)
            : real(query);
    });
}
...
async function assertQuietHome(page, label) {
    ...
    await page.waitForFunction((attentionIds) => {
        const host = document.querySelector('[data-testid="widget-host-home"]');
        if (!(host instanceof HTMLElement))
            return false;
        if (host.childElementCount !== 0)
            return false;
        return attentionIds.every((testId) => document.querySelector(`[data-testid="${testId}"]`) == null);
    }, ATTENTION_HOME_TEST_IDS, { timeout: 15000 });
    assert((await page.getByTestId("home-time-widget").count()) === 1, `${label}: time widget remains visible`);
    assert((await page.getByTestId("home-weather").count()) === 1, `${label}: weather widget remains visible`);
    assert((await page.getByTestId("widget-host-home").locator(":scope > *").count()) ===
        0, `${label}: no ranked attention cards render healthy-empty chrome`);
    for (const testId of ATTENTION_HOME_TEST_IDS) {
        assert((await page.getByTestId(testId).count()) === 0, `${label}: ${testId} self-hides when data is healthy-empty`);
    }
}
async function waitForSurfacePageSettled(p, pageName) {
    await p.waitForFunction((expectedPage) => {
        ...
        if (surface.getAttribute("data-page") !== expectedPage)
            return false;
        const surfaceRect = surface.getBoundingClientRect();
        const railRect = rail.getBoundingClientRect();
        const expectedLeft = expectedPage === "launcher"
...
async function waitForRenderedHomeSettled(page) {
    ...
    await page.waitForFunction(async (expectedViewportWidth) => {
        const sample = () => {
            const surface = document.querySelector('[data-testid="home-launcher-surface"]');
            const rail = document.querySelector('[data-testid="home-launcher-rail"]');
            const home = document.querySelector('[data-testid="home-launcher-home-page"]');
        ...
        if (!second)
            return false;
        const stable = ["railLeft", "homeLeft", "homeRight"].every((key) => Math.abs(first[key] - second[key]) < 0.5);
        return (stable &&
...
async function measureRailSwipeWindow(page) {
    ...
    try {
        ...
        const effectiveDroppedFrames = deltas.filter((delta) => Number.isFinite(delta) &&
            delta > summary.budgetMs + DROPPED_FRAME_EPSILON_MS).length;
        const droppedFrameRatio = effectiveDroppedFrames / Math.max(1, summary.sampleCount);
...
try {
    ...
    const initialNotificationCenter = mobile.getByTestId("home-notification-center");
    const initialNotificationList = initialNotificationCenter.getByTestId("home-notification-list");
    await initialNotificationList.waitFor({ state: "visible", timeout: 5000 });
    assert((await initialNotificationList.getAttribute("data-shade-mode")) ===
        "expanded", "populated notifications open as the full gesture-only inbox");
    await touchSwipe(mobile, '[data-testid="home-notification-list"]', 0, -180, {
        steps: 10,
        stepDelayMs: 16,
    });
    await initialNotificationCenter
        .locator('[data-testid="home-notification-list"][data-shade-mode="rested"]')
    ...
    assert((await mobile.getByTestId("home-launcher-surface").getAttribute("data-page")) === "home", "mobile coarse-pointer: reload returns to the home half");
    assert((await mobile.getByTestId("first-session-swipe-hint").count()) === 0, "mobile coarse-pointer: retired lesson stays absent after reload");
    ...
    assert((await mobile.getByTestId("rail-pager-edge-prev").count()) === 0 &&
        ...
        (await mobile.getByTestId("launcher-pager-edge-next").count()) === 0, "mobile coarse-pointer: no rail or launcher edge buttons on home");
    assert((await mobile.getByTestId("home-launcher-surface").getAttribute("data-page")) === "home", "combined surface starts on Home");
    assert((await mobile.getByTestId("home-clock").count()) === 0, "no clock (home kept minimal)");
    ...
    assert((await homeWidgetHost.getAttribute("data-slot")) === "home", "home WidgetHost is mounted for the home slot");
    ...
    for (const testId of [
        ...
        assert((await homeWidgetHost.getByTestId(testId).count()) === 0, `removed/demoted home widget ${testId} stays absent`);
    }
    ...
    // ErrorBoundary catch is invisible to the page-error guard, so assert it here.
    {
        const errorCards = await mobile
            ...
            .allTextContents();
        assert(errorCards.length === 0, `no home widget hit its error boundary (${errorCards.length})`);
    }
    ...
    // live travel and settled-state ownership without redundant global chrome.
    {
        ...
        assert((await mobile
            ...
            .count()) === 0, "the notification inbox is inline on the home column, outside the ranked WidgetHost");
        assert((await center.getByTestId("notification-row").count()) === 1, "the seeded notification renders as a single row");
        assert((await center.getByTestId("notification-group-label").count()) === 0, "no group header eyebrows render — grouping is physical only");
        assert((await center.getByText("Payment failed", { exact: false }).count()) > 0, "the notification row shows the seeded title");
        assert((await center.getByTestId("notifications-count").count()) === 0 &&
            (await center.getByTestId("notifications-count-button").count()) === 0 &&
            (await center.getByTestId("notifications-clear-all").count()) === 0 &&
            (await center.getByTestId("notifications-collapse").count()) === 0, "the gesture-only shade renders no redundant global controls");
        const notificationRow = center.getByTestId("notification-row");
        const restedRowBox = await notificationRow.boundingBox();
        if (!restedRowBox)
            throw new Error("missing notification row bounds");
        const partialPull = await touchDragHold(mobile, '[data-testid="home-notification-list"]', 0, 48, { steps: 6, stepDelayMs: 16 });
        await mobile.evaluate(() => new Promise((resolve) => requestAnimationFrame(resolve)));
        const heldRowBox = await notificationRow.boundingBox();
        if (!heldRowBox)
            throw new Error("missing pulled notification row bounds");
        const heldRowTravel = heldRowBox.y - restedRowBox.y;
        assert(Math.abs(heldRowTravel) > 1 && Math.abs(heldRowTravel) < 28, `a partial pull moves the notification continuously (${heldRowTravel.toFixed(2)}px)`);
        ...
        assert(releasePeak <= Math.abs(heldRowTravel) + 1.5, `a cancelled pull returns without bouncing farther from rest (${releasePeak.toFixed(2)}px peak)`);
        assert(Math.abs(releaseFinal) < 0.75, `the notification row settles back at rest (${releaseFinal.toFixed(2)}px)`);
        await touchSwipe(mobile, '[data-testid="home-notification-list"]', 0, 180, { steps: 10, stepDelayMs: 16 });
        await center
            .locator('[data-testid="home-notification-list"][data-shade-mode="expanded"]')
            .waitFor({ state: "visible", timeout: 5000 });
        assert((await center.getByTestId("notification-row").count()) === 1, "a downward gesture opens the full inbox without duplicating rows");
        await touchSwipe(mobile, '[data-testid="home-notification-list"]', 0, -180, { steps: 10, stepDelayMs: 16 });
        await center
            .locator('[data-testid="home-notification-list"][data-shade-mode="rested"]')
            .waitFor({ state: "visible", timeout: 5000 });
        assert((await center.getByTestId("notification-row").count()) === 1, "an upward gesture restores the rested triage row");
    }
    ...
    assert((await mobile.getByTestId("home-tiles").count()) === 0, "mobile native mode keeps app tiles off the Home page");
    ...
    // fixed-position overlays whose boxes are outside the card's grid geometry.
    {
        ...
        for (let i = 0; i < items.length; i += 1) {
            for (let j = i + 1; j < items.length; j += 1) {
                ...
                assert(!(xOverlap > TOLERANCE && yOverlap > TOLERANCE), `home widgets ${items[i].testId} and ${items[j].testId} do not overlap (x ${Math.round(xOverlap)}px, y ${Math.round(yOverlap)}px)`);
            }
        }
    }
    await snap(mobile, "mobile-home");
    {
        ...
        assert(darkForegrounds.length === 0, `home card foregrounds stay readable on the dark ember field (${JSON.stringify(darkForegrounds.slice(0, 5))})`);
    }
    ...
    const shifts = await mobile.evaluate(() => window.__ELIZA_LAYOUT_SHIFTS__ ?? []);
    const stability = summarizeStability(shifts, [], { maxCls: 0.1 });
    assert(!stability.flagged, `home settle is layout-stable (CLS ${stability.cls.toFixed(4)} ≤ 0.1, ${stability.shiftCount} shifts)`);
    await waitForSurfacePageSettled(mobile, "home");
    ...
    assert(Math.abs(fastFlick.immediateX + 90) < 3, `same-task flick starts its settle at the release frame (${fastFlick.immediateX.toFixed(2)}px)`);
    assert(fastFlick.frameX.length >= 4 &&
        fastFlick.frameX.every((value, index, values) => index === 0 || value <= values[index - 1] + 1), `same-task flick advances monotonically (${fastFlick.frameX.map((value) => value.toFixed(1)).join(", ")})`);
    assert(fastFlick.frameX.every((value, index, values) => index === 0 ||
        Math.abs(value - values[index - 1]) < fastFlick.viewportWidth * 0.25), "same-task flick contains no one-frame release jump");
    assert(fastFlick.clickDefaultPrevented && !fastFlick.clickBubbled, "committed same-task flick suppresses its synthesized click");
    await waitForSurfacePageSettled(mobile, "launcher");
    assert((await mobile.getByTestId("home-launcher-surface").getAttribute("data-page")) === "launcher", "same-task fast flick commits exactly the adjacent launcher page");
    await touchSwipeRight(mobile, "home-launcher-launcher-page");
    await waitForSurfacePageSettled(mobile, "home");
    const verticalDrag = await sameTaskVerticalDrag(mobile);
    assert(verticalDrag.page === "home" && Math.abs(verticalDrag.railX) < 1, "same-task vertical drag is rejected without paging or displacing the rail");
    ...
    await waitForSurfacePageSettled(mobile, "launcher");
    assert((await mobile.getByTestId("home-launcher-surface").getAttribute("data-page")) === "launcher", "deliberate one-second thumb drag opens the launcher before half-screen travel");
    await touchSwipeRight(mobile, "home-launcher-launcher-page");
    await waitForSurfacePageSettled(mobile, "home");
    ...
    assert((await mobile.getByTestId("rail-pager-edge-prev").count()) === 0 &&
        (await mobile.getByTestId("rail-pager-edge-next").count()) === 0 &&
        (await mobile.getByTestId("launcher-pager-edge-prev").count()) === 0 &&
        (await mobile.getByTestId("launcher-pager-edge-next").count()) === 0, "mobile coarse-pointer: no rail or launcher edge buttons on launcher");
    // ── Curated apps page - the everyday apps render as tiles, in curated order.
    for (const id of ["wallet", "automations", "browser", "settings"]) {
        assert(await mobile.getByTestId(`launcher-tile-${id}`).isVisible(), `curated app "${id}" renders on the launcher apps page`);
    }
    // ── No dock: the featured-views dock was removed, so there is no
    // `launcher-dock` element competing with the curated page grid.
    assert((await mobile.getByTestId("launcher-dock").count()) === 0, "the launcher renders no dock (featured-views header removed)");
    ...
    assert((await mobile.getByTestId("launcher-tile-wallet").count()) === 1, "duplicate wallet registrations collapse to one tile");
    ...
    for (const id of ["wallet", "automations", "browser", "character"]) {
        ...
        assert((await visual.count()) === 1 && (await visual.isVisible()), `curated app "${id}" renders its glyph icon plate`);
        assert((await mobile.getByTestId(`launcher-image-${id}`).count()) === 0, `curated app "${id}" renders no hero <img> (glyph-only launcher)`);
    }
    ...
    assert((await mobile
        ...
        .count()) === 0, "the page indicator is removed (no colliding dots)");
    assert((await mobile.locator('[aria-label^="Page "]').count()) === 0, "the inner Launcher dot strip is absent too");
    ...
    assert(visualCount >= 5, `launcher renders multiple glyph tiles (${visualCount})`);
    assert((await mobile.locator('[data-testid^="launcher-image-"]').count()) === 0, "no launcher tile renders a hero <img> (glyph-only launcher)");
    const launcherGlyphs = await mobile.$$eval("[data-view-visual]", (els) => els.map((el) => {
    ...
    assert(launcherGlyphs.every(({ kind, ionicon }) => kind === "ionicon" && Boolean(ionicon)), "every launcher plate carries a named official Ionicon");
    const distinctIonicons = new Set(launcherGlyphs.map(({ ionicon }) => ionicon));
    assert(distinctIonicons.size >= 3, `launcher uses distinct Ionicons, not one placeholder (${distinctIonicons.size} distinct)`);
    ...
    assert((await mobile
        ...
        .count()) === 0, "a stationary long-press does NOT enter edit mode (curated launcher is read-only)");
    ...
    assert((await mobile
        ...
        .getAttribute("data-page")) === "home", "swipe-back from the launcher returns HOME");
    await touchSwipeLeft(mobile, "home-launcher-home-page");
    await waitForSurfacePageSettled(mobile, "launcher");
    ...
    for (const id of [
        ...
        assert((await mobile
            ...
            .count()) === 1, `developer tool "${id}" renders on the single launcher page`);
    }
    assert((await mobile.getByTestId("launcher-page-1").count()) === 0, "there is no second launcher page (single curated page of views)");
    ...
    assert((await mobile
        ...
        (await mobile.getByTestId("launcher-page-1").count()) === 0, "a left-swipe on the single page rubber-bands (no page 2, stays on launcher)");
    ...
    assert((await mobile.getByTestId("home-edit-toggle").count()) === 0, "no Edit toggle (clean dashboard)");
    assert((await mobile.getByText("Pinned", { exact: true }).count()) === 0, 'no "Pinned" label');
    await mobile.goto(`${url}?homeData=quiet`);
    ...
    // shared frame-budget detector used by the chat performance gates.
    {
        ...
        for (let attempt = 0; attempt < RAIL_SWIPE_ATTEMPTS; attempt += 1) {
            ...
            console.log(`  [rail-swipe ${attempt + 1}/${RAIL_SWIPE_ATTEMPTS}] ` +
            ...
            assert(result.sampleCount >= MIN_FRAME_SAMPLES, `rail-swipe window ${attempt + 1} captured ≥${MIN_FRAME_SAMPLES} frames ` +
                `(got ${result.sampleCount})`);
        }
        const budgetMs = attempts[0]?.budgetMs ?? 1000 / FRAME_BUDGET.targetFps;
        const medianP95FrameMs = medianNumber(attempts.map((attempt) => attempt.p95FrameMs));
        const medianDroppedFrameRatio = medianNumber(attempts.map((attempt) => attempt.droppedFrameRatio));
        const medianDroppedPct = 100 * medianDroppedFrameRatio;
        const overP95Budget = medianP95FrameMs > budgetMs * FRAME_GATE.p95BudgetFactor;
        const overDroppedBudget = medianDroppedFrameRatio >= FRAME_GATE.droppedFrameRatio;
        console.log(`  [rail-swipe median] p95=${medianP95FrameMs.toFixed(1)}ms ` +
            `dropped=${medianDroppedPct.toFixed(0)}% attempts=${attempts.length}`);
        assert(!overP95Budget && !overDroppedBudget, `rail swipe median stays within the frame budget (p95 ${medianP95FrameMs.toFixed(1)}ms ≤ ` +
    ...
    assert(await shortQuietHome.getByTestId("notifications-empty").isVisible(), "short quiet home still reveals its empty notification shade through a pull gesture");
    ...
    assert((await desktop.getByTestId("home-tiles").count()) === 0, "no pinned tiles off-AOSP (grid omitted)");
    assert((await desktop.getByTestId("home-tile-phone").count()) === 0, "phone tile hidden when native disabled");
    // Desktop uses the same inline, control-free directional shade.
    {
        const center = desktop.getByTestId("home-notification-center");
        await center.waitFor({ state: "visible", timeout: 5000 });
        assert((await center.getByTestId("notification-row").count()) === 1, "desktop home renders the inline notification inbox with the seeded row");
        const list = center.getByTestId("home-notification-list");
        assert((await list.getAttribute("data-shade-mode")) === "expanded", "desktop notifications open as the full inbox");
        ...
        await center
            .locator('[data-testid="home-notification-list"][data-shade-mode="expanded"]')
            .waitFor({ state: "visible", timeout: 5000 });
        assert((await center.getByTestId("notification-row").count()) === 1, "desktop directional gestures preserve the single notification row");
        await dragVertical(list, -180);
        await center
            .locator('[data-testid="home-notification-list"][data-shade-mode="rested"]')
            .waitFor({ state: "visible", timeout: 5000 });
    ...
    await finePointer.addInitScript(() => {
        ...
        window.matchMedia = (query) => /hover: hover|pointer: fine/.test(query) ? stub(query) : real(query);
    });
    ...
    // first view).
    assert((await finePointer.getByTestId("rail-pager-edge-next").count()) === 1, "desktop fine-pointer: `>` edge button present on home");
    assert((await finePointer.getByTestId("rail-pager-edge-prev").count()) === 0, "desktop fine-pointer: no `<` edge button on the first (home) view");
    ...
    assert((await finePointer.getByTestId("rail-pager-edge-prev").count()) === 1, "desktop fine-pointer: `<` edge button (→ home) present on the launcher");
