 ...
 *
 * Reads `/sol-build-info.json` from the web root (best-effort; renders nothing
...
const BUILD_INFO_URL = "/sol-build-info.json";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Best-effort: no build info available (real builds without the dev
        // server stamp). Stay hidden silently.
      }
    })();
    return () => {
      cancelled = true;
