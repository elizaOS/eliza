...
describe("TopicChipsBar", () => {
	...
	it("renders nothing when there are no topics", () => {
		...
		expect(container.querySelector('[data-testid="topic-chips-bar"]')).toBeNull();
	});
});
...
describe("TopicGroup", () => {
	...
	it("renders children + a divider header when expanded", () => {
		render(
			<TopicGroup topic="billing" count={2} collapsed={false} onCollapsedChange={() => {}}>
	...
	it("renders an untitled run with no header/pill", () => {
		render(
			<TopicGroup topic={null} count={1} collapsed={false} onCollapsedChange={() => {}}>
