...
import { CHAT_CLOSE_EVENT } from "../../events";
import { CHAT_OPEN_EVENT } from "../../events";
import { CHAT_OVERLAY_RESTING_WINDOW_HEIGHT } from "./chat-overlay-window-bounds";
import { CHAT_OVERLAY_RESTING_WINDOW_WIDTH } from "./chat-overlay-window-bounds";
import { CHAT_PREFILL_EVENT } from "../../events";
import { CHAT_UPLOAD_ACCEPT } from "../../utils/image-attachment";
import { CONFIG_SELECT_FLOATING_LAYER_Z_INDEX } from "../../lib/floating-layers";
...
import { ELIZA_BACK_INTENT_EVENT } from "../../events";
import { FIRST_RUN_SIGN_IN_FALLBACK_DELAY_MS } from "./chat-overlay-transcript";
import { FileText } from "lucide-react";
import { Film } from "lucide-react";
import { GLASS_SHEET_BACKDROP_FILTER } from "../../glass/tokens";
import { GLASS_SHEET_FILL } from "../../glass/tokens";
import { House } from "lucide-react";
import { Input } from "../ui/input";
import { KEYBOARD_INTRUSION_THRESHOLD_PX } from "../../platform/standalone-bottom-reclaim";
import { LAYOUT_SHIFT_INTENT_ATTR } from "../../hooks/useLayoutShiftMonitor";
import { LAYOUT_SHIFT_INTENT_TRANSIENT } from "../../hooks/useLayoutShiftMonitor";
...
import { TOUCH_TAP_MOVE_SLOP as OUTSIDE_SHEET_TAP_SLOP } from "../../gestures";
...
import { bytesToMb } from "../../utils/image-attachment";
import { chatUploadKind } from "../../utils/image-attachment";
import { clamp01 } from "./chat-overlay-motion";
import { clearChatDraft } from "../../state/ChatComposerContext.hooks";
...
import { getPhysicalScreenVerticalExtent } from "../../platform/standalone-bottom-reclaim";
import { goHome } from "../../state/shell-surface-store";
import { grabberBarOpacity } from "./chat-overlay-motion";
import { highlightSearchMatches } from "./search-match-highlight";
import { highlightSearchMessage } from "./search-match-highlight";
import { intakeAttachmentFiles } from "../../utils/image-attachment";
import { isAndroid } from "../../platform/init";
import { isFirstRunShellMessage } from "./chat-overlay-transcript";
...
import { isShortLandscapeViewport } from "./chat-panel-layout";
import { isStandalonePwa } from "../../platform/init";
...
import { measureSafeAreaInsetTop } from "./chat-panel-layout";
import { motion } from "motion/react";
import { pillHandleCounterScale } from "./chat-overlay-motion";
import { pillMorphScale } from "./chat-overlay-motion";
import { registerPendingFirstRunTextConsumer } from "../../first-run/first-run-pending-text";
import { renderOverlayMessageBody } from "./chat-overlay-transcript";
import { reportComposerActivity } from "../../chat/report-composer-activity";
import { resolveChatKeyboardHiddenViewportBaseline } from "./chat-panel-layout";
import { resolveChatNativeKeyboardLift } from "./chat-panel-layout";
import { resolveChatPanelHalfDetentHeight } from "./chat-panel-layout";
import { resolveChatPanelLayout } from "./chat-panel-layout";
import { selectFirstRunDisplayMessages } from "./chat-overlay-transcript";
import { selectSemanticNewestFirstRunMessage } from "./chat-overlay-transcript";
import { setChatComposerAccessoryBarHidden } from "./ios-chat-accessory-bar";
import { sheetBlackoutProgress } from "./chat-overlay-motion";
import { shellToChatMessageData } from "./chat-overlay-transcript";
import { shouldInstallStandaloneBottomReclaim } from "../../platform/standalone-bottom-reclaim";
import { summarizeDroppedAttachments } from "../../utils/image-attachment";
...
import { type BackIntentEventDetail } from "../../events";
import { type ChatMessageData } from "../composites/chat/chat-types";
import { type ChatMessageRenderContext } from "../composites/chat/chat-types";
import { type ChatPrefillEventDetail } from "../../events";
import { type ConversationMessageSearchResult } from "../../api/client-types-chat";
import { type ConversationNav } from "./useShellController";
import { type ImageAttachment } from "../../api/client-types-chat";
import { type MotionStyle } from "motion/react";
import { type MotionValue } from "motion/react";
import { type NavigateViewDetail } from "../../events";
...
import { useComposerPaste } from "../../chat/composer-core";
...
const SCREEN_KEYBOARD_SIGNAL_ACTIVE = typeof window !== "undefined" &&
...
export type ChatState = "CLOSED" | "INPUT" | "OPEN_UNDER_HALF" | "OPEN_HALF_OR_OVER" | "MAXIMIZED";
...
function detentHaptic(): void {
    try {
        ...
        if (cap?.isNativePlatform?.()) {
            void Promise.resolve(cap.Plugins?.Haptics?.impact?.({ style: "LIGHT" })).catch(() => {
...
export { grabberBarOpacity, PILL_MORPH_MIN_SCALE, pillHandleCounterScale, pillMorphScale, sheetBlackoutProgress, } from "./chat-overlay-motion";
...
function Glyph({ d, className, }: {
    ...
    return (<svg viewBox="0 0 36 36" className={cn("h-[26px] w-[26px]", className)} aria-hidden="true">
...
export function SoftButton({ glyph, icon: Icon, label, onClick, onPointerDown, onPointerUp, onPointerCancel, onPointerLeave, disabled, active, pressed, pulse, testId, }: {
    ...
    return (<Button variant="transparent" size="icon" data-testid={testId} aria-label={label} aria-pressed={pressed ?? active} data-state={active ? "on" : "off"} 
    ...
    aria-disabled={disabled} onClick={disabled ? undefined : onClick} onPointerDown={disabled ? undefined : onPointerDown} onPointerUp={disabled ? undefined : onPointerUp} onPointerCancel={disabled ? undefined : onPointerCancel} onPointerLeave={disabled ? undefined : onPointerLeave} className={cn(
        ...
        "relative grid shrink-0 place-items-center [&_svg]:size-5", "text-muted-strong hover:text-txt data-[state=on]:text-inverse data-[state=on]:hover:text-inverse aria-[disabled=true]:pointer-events-none aria-[disabled=true]:opacity-40", 
      ...
      {Icon ? (<Icon aria-hidden={true}/>) : glyph ? (
        // Match the lucide marks: the parent [&_svg] rule governs the box, and
        // the widened glyph paths fill the same fraction of it.
        <Glyph d={glyph} className="size-5"/>) : null}
    </Button>);
}
function ComposerControlTransition({ children, controlKey, reduceMotion, }: {
    children: React.ReactNode;
    controlKey: string;
    reduceMotion: boolean;
}): React.JSX.Element {
    const present = useIsPresent();
    return (<motion.div data-composer-control={controlKey} aria-hidden={!present || undefined} inert={!present || undefined} initial={reduceMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{
            duration: reduceMotion ? 0 : 0.16,
            ease: OVERLAY_EASE,
        }} className="absolute inset-0 grid place-items-center" style={{ pointerEvents: present ? "auto" : "none" }}>
      {children}
    </motion.div>);
}
function ComposerControlSlot({ children, controlKey, reduceMotion, slot, }: {
    children: React.ReactNode;
    controlKey: string | null;
    reduceMotion: boolean;
    slot: "left" | "right";
}): React.JSX.Element {
    return (<div data-testid={`chat-composer-control-slot-${slot}`} className="relative size-10 shrink-0 pointer-coarse:size-11">
      <AnimatePresence initial={false}>
        {controlKey ? (<ComposerControlTransition key={controlKey} controlKey={controlKey} reduceMotion={reduceMotion}>
            {children}
          </ComposerControlTransition>) : null}
      </AnimatePresence>
    </div>);
}
...
function ComposerMicActivity({ analyser, finishing, reduceMotion, transcript, }: {
    ...
    React.useEffect(() => {
        if (!analyser || finishing || reduceMotion)
            return;
        ...
        const renderFrame = () => {
            ...
            barsRef.current.forEach((bar, index) => {
                if (!bar)
                    return;
                const segmentStart = Math.floor((index * samples.length) / COMPOSER_MIC_BARS.length);
                const segmentEnd = Math.max(segmentStart + 1, Math.floor(((index + 1) * samples.length) / COMPOSER_MIC_BARS.length));
                let energy = 0;
    ...
    return (<div role="status" aria-label={finishing ? "Finishing transcription" : "Live microphone activity"} data-testid="chat-composer-mic-activity" className="relative flex min-h-10 min-w-0 flex-1 items-center justify-between gap-1 px-2 text-white/85">
      <span className="sr-only" aria-live="polite">
        {finishing
            ? "Finishing transcription"
            : transcript.trim() || "Listening"}
      </span>
      <Separator aria-hidden="true" tone="subtle40" className="pointer-events-none absolute inset-x-2 top-1/2 -translate-y-1/2"/>
      {COMPOSER_MIC_BARS.map(({ id, height }, index) => (<Card asChild surface="inverseForeground" border="none" radius="full" key={id}>
          <span 
        // Stable bar ids keep imperative analyser writes independent of React.
        ref={(node) => {
                barsRef.current[index] = node;
            }} aria-hidden="true" className={cn("relative z-10 w-0.5 origin-center transition-transform duration-75 sm:w-1", !finishing &&
                !analyser &&
                "animate-pulse motion-reduce:animate-none")} style={{ height, transform: "scaleY(0.32)" }}/>
        </Card>))}
    </div>);
}
type RealtimeVoiceStatus = NonNullable<ShellController["realtimeVoice"]>["status"];
...
type RealtimeVoiceVisualPhase = RealtimeVoiceStatus | "connecting" | "paused" | "error";
const REALTIME_VOICE_ORB: Record<RealtimeVoiceVisualPhase, {
    state: OrbState;
    speed: number;
    paused?: boolean;
}> = {
...
function ComposerRealtimeVoiceWaveform({ phase, reduceMotion, }: {
    phase: RealtimeVoiceVisualPhase;
    reduceMotion: boolean;
}): React.JSX.Element {
    const visual = REALTIME_VOICE_ORB[phase];
    const paused = reduceMotion || visual.paused === true;
    return (<span aria-hidden="true" data-phase={phase} data-orb-state={visual.state} data-testid="chat-composer-realtime-waveform" className={cn("flex h-5 w-6 shrink-0 items-center justify-center", phase === "error" ? "opacity-60" : "opacity-90")}>
      <ThinkingOrb aria-hidden="true" data-testid="chat-composer-thinking-orb" state={visual.state} size={20} speed={paused ? 0 : visual.speed} paused={paused} theme="dark"/>
    </span>);
}
/** Realtime voice occupies the composer's normal text lane, never a second card. */
function ComposerRealtimeVoiceActivity({ connecting, error, needsAudioUnlock, onUnlockAudio, paused, microphoneMuted, reduceMotion, status, transcript, }: {
    connecting: boolean;
    error: string | null;
    ...
    return (<div role="status" aria-live="polite" aria-label={liveTranscript ? `${phaseLabel}: ${liveTranscript}` : phaseLabel} data-status={status} data-testid="chat-composer-realtime-voice" className="flex min-h-10 min-w-0 flex-1 items-center gap-2 px-1.5">
      <ComposerRealtimeVoiceWaveform phase={visualPhase} reduceMotion={reduceMotion}/>
      <div className="flex h-10 min-w-0 flex-1 items-center overflow-hidden">
        <span ref={copyRef} data-testid="chat-composer-realtime-copy" className={cn("block max-h-10 w-full min-w-0 overflow-hidden whitespace-pre-wrap text-start text-sm leading-5 [overflow-wrap:anywhere]", error
            ? "text-danger"
            : liveTranscript
                ? "text-txt"
                : "text-white/75", shimmerPhase &&
            "shimmer shimmer-duration-1200 motion-reduce:shimmer-none")} title={visibleCopy}>
          {visibleCopy}
        </span>
      </div>
      {needsAudioUnlock ? (<Button variant="warningOutline" size="tiny" onClick={onUnlockAudio} data-testid="chat-composer-voice-audio-unlock" shape="circle" className="shrink-0">
          Enable sound
        </Button>) : null}
    </div>);
}
 ...
 */
function SheetGrabber({ open, onOpen, onClose, binding, breathing, opacity, pilled, locked = false, }: {
    open: boolean;
    onOpen: () => void;
    onClose: () => void;
    ...
    return (<Button asChild variant="chatGestureTarget" size="content">
      <motion.button style={{ opacity, pointerEvents: disabled ? "none" : "auto" }} 
    ...
    tabIndex={disabled ? -1 : undefined} aria-hidden={disabled || undefined} 
    ...
    type="button" aria-expanded={open} aria-disabled={locked || undefined} aria-label={open ? "drag down to close chat" : "drag up to open chat"} data-testid="chat-sheet-grabber" data-open={open ? "true" : "false"} onClick={(event) => {
    ...
    onTouchEnd={(e) => {
            ...
            binding.onPointerDown(event);
        }} className={cn("appearance-none text-left", 
        ...
        <Card asChild surface="transparent" border="none" radius="full" overlayHandle>
 ...
 */
export function PillHandle({ binding, counterScale, onOpen, breathing, pilled, desktopOverlayHost = false, }: {
    binding: PullGestureBinding;
    ...
    if (desktopOverlayHost) {
        return (<motion.div className="h-1.5 w-12 origin-bottom" style={{
                scale: counterScale,
        ...
        <Button variant="transparent" size="content" shape="circle" data-testid="chat-pill" aria-label="open chat" style={{
                width: CHAT_OVERLAY_RESTING_WINDOW_WIDTH,
                height: CHAT_OVERLAY_RESTING_WINDOW_HEIGHT,
            }} onKeyDown={(event) => {
                if (event.key === "Enter" ||
                ...
                if (event.cancelable)
                    event.preventDefault();
            }} {...binding} tabIndex={pilled ? 0 : -1} aria-hidden={pilled ? undefined : true} className={cn("shrink-0 cursor-grab touch-none select-none active:scale-95 active:cursor-grabbing focus-visible:ring-2 focus-visible:ring-inverse/70 focus-visible:ring-offset-2 focus-visible:ring-offset-transparent", pilled ? "pointer-events-auto" : "pointer-events-none")}>
          <Card asChild surface="transparent" border="none" radius="full" overlayHandle>
            <span aria-hidden="true" data-testid="chat-pill-mark" className={cn("pointer-events-none h-3 w-16 opacity-100", breathing && "eliza-chat-handle-breathe")}/>
          </Card>
        </Button>
      </motion.div>);
    }
    return (<Button variant="chatGestureTarget" size="content" data-testid="chat-pill" aria-label="open chat" onKeyDown={(e) => {
    ...
    onTouchEnd={(event) => {
            if (event.cancelable)
                event.preventDefault();
        }} {...binding} tabIndex={pilled ? undefined : -1} aria-hidden={pilled ? undefined : true} className={cn("h-auto w-full px-8 pb-1.5 pt-10", 
      ...
      <Card asChild surface="transparent" border="none" radius="full" overlayHandle layoutStyle={{ transformOrigin: "bottom center" }}>
        <motion.span aria-hidden="true" className={cn(
...
function MessageScrollerSendFollow({ request }: {
    request: number;
}) {
...
type ScrollToTranscriptMessage = ReturnType<typeof useMessageScroller>["scrollToMessage"];
/** Exposes the scroller's coordinated jump API to search result handling. */
function MessageScrollerSearchBridge({ scrollToMessageRef, }: {
...
export function ChatOverlay({ controller, agentName = "Eliza", firstRunOpen = false, acceptPendingFirstRunText = false, initialMode = "input", releaseFirstRunToFull = false, fillHostAtHalf = false, onFirstRunReleaseHandled, onPilledChange, onDetentChange, onStateChange, }: {
    ...
    const { messages, phase, responding, turnStatus, send, sendFirstRunText, canSend, recording, analyser, transcript, handsFree, toggleHandsFree, transcriptionMode, toggleTranscriptionMode, stopTranscriptionAndMic, setDictationSink, setTranscriptSessionSink, setComposerHasDraft, needsAudioUnlock, ttsError, unlockAudio, openSettings, navigateHome, currentTab, stop, speak, stopSpeaking, speaking, } = controller;
    const realtimeVoice = controller.realtimeVoice;
    const realtimeVoiceComposerVisible = Boolean(realtimeVoice?.enabled &&
        !realtimeVoice.error &&
        (handsFree ||
            realtimeVoice.active ||
            realtimeVoice.connecting ||
            realtimeVoice.status !== "idle"));
    const realtimeVoiceControlsVisible = Boolean(realtimeVoice?.enabled &&
        handsFree &&
    ...
    const firstRunComposerPlaceholder = React.useMemo(() => {
        ...
        if (/opening your personal eliza|setting up your agent|connecting to eliza cloud/i.test(latestStatus)) {
    ...
    // handlers.
    const { handleChatEdit, handleChatRetry, elizaCloudLoginFallbackUrl, cloudLoginHasError, handleSelectConversation, loadConversationMessagesAround, } = useAppSelectorShallow((s) => ({
    ...
    const handleSpeakMessage = React.useCallback((id: string, text: string) => {
    ...
    const handleEditMessage = React.useCallback(async (id: string, text: string): Promise<boolean> => {
        stopSpeaking?.();
        setPlayingMessageId(null);
    ...
    const handleRetry = React.useCallback((assistantId: string) => {
        const list = messagesRef.current;
        const assistantIdx = list.findIndex((m) => m.id === assistantId && m.role === "assistant");
        if (assistantIdx < 0)
            return;
    ...
    const { removeConversationMessage, conversationMessages, prependConversationMessages, } = useConversationMessages();
    const handleDismissSuggestion = React.useCallback((messageId: string) => {
        removeConversationMessage(messageId);
    }, [removeConversationMessage]);
    const handleAcceptSuggestion = React.useCallback((m: ChatMessageData) => {
    ...
    const { chatInput: draft, setChatInput: setDraft, chatPendingImages: pendingImages, setChatPendingImages: setPendingImages, chatReplyTarget, setChatReplyTarget, } = useChatComposerOrLocal();
    const activeConversationId = conversationNav.activeId;
    const sentMessageHistory = React.useMemo(() => messages.flatMap((message) => {
    ...
    React.useEffect(() => {
        if (messageHistoryConversationRef.current === activeConversationId)
            return;
    ...
    const [transcriptionFinishing, setTranscriptionFinishing] = React.useState(false);
    const transcriptionComposerActive = transcriptionMode || transcriptionFinishing;
    const cloudLoginWaiting = React.useMemo(() => {
        ...
        return (activeMessage?.id === "first-run:cloud-login-waiting" &&
    ...
    const [mode, setMode] = React.useState<ChatMode>(pinnedOpen ? "half" : initialMode);
    ...
    const [inputIdleEpoch, noteInputActivity] = React.useReducer((epoch: number) => epoch + 1, 0);
    React.useEffect(() => () => {
    ...
    React.useLayoutEffect(() => {
        if (!transcriptionComposerActive && !realtimeVoiceComposerVisible)
            return;
    ...
    const animateThreadHeight = React.useCallback((target: number) => {
        stopThreadAnimation();
        setSheetSettled(false);
        ...
        controls.finished
            ...
            if (draggingRef.current)
                return; // a new drag started meanwhile
    ...
    const animateOpenProgress = React.useCallback((target: number) => {
    ...
    const [panelElement, setPanelElement] = React.useState<HTMLFieldSetElement | null>(null);
    ...
    const getPanelElement = React.useCallback(() => {
        ...
        if (typeof document === "undefined")
            return null;
        return document.querySelector<HTMLFieldSetElement>('[data-testid="chat-sheet"]');
    }, [panelElement]);
    ...
    const markLayoutShiftIntent = React.useCallback(() => {
        ...
        const scheduleClear = (delay: number) => {
            layoutShiftIntentTimerRef.current = window.setTimeout(() => {
                const since = performance.now() - layoutShiftIntentLastMotionRef.current;
                if (since < 180) {
    ...
    const renderableMessages = React.useMemo(() => filterRenderableShellMessages(messages, responding ? "responding" : phase), [messages, phase, responding]);
    ...
    const hasFirstRunMessages = React.useMemo(() => renderableMessages.some(isFirstRunShellMessage), [renderableMessages]);
    const [firstRunFallbackReady, setFirstRunFallbackReady] = React.useState(false);
    React.useEffect(() => {
    ...
    const visibleMessages = React.useMemo(() => {
        if (firstRunOpen)
            return selectFirstRunDisplayMessages(renderableMessages, firstRunFallbackReady);
    ...
    const cloudSignInRequired = React.useMemo(() => firstRunOpen &&
        visibleMessages.some((message) => message.content.includes("__first_run__:runtime:cloud=Sign in to Eliza Cloud")), [firstRunOpen, visibleMessages]);
    ...
    const handleThreadScroll = React.useCallback((event: React.UIEvent<HTMLDivElement>) => {
    ...
    React.useLayoutEffect(() => {
        ...
        const preservePinnedEnd = () => {
            if (!threadPinnedToEndRef.current)
                return;
            const end = Math.max(0, viewport.scrollHeight - viewport.clientHeight);
    ...
    const runMessageSearch = React.useCallback(async (query: string, signal: AbortSignal) => {
        const { results } = await client.searchConversationMessages(query, {
            signal,
        });
        if (!signal.aborted)
            completedSearchQueryRef.current = query.trim();
        return results;
    }, []);
    ...
    const waitForSearchAnchor = React.useCallback((anchorId: string, maxFrames: number): Promise<HTMLElement | null> => new Promise((resolve) => {
    ...
    const searchScrollToMessageRef = React.useRef<ScrollToTranscriptMessage | null>(null);
    const scrollAndFlashSearchAnchor = React.useCallback((el: HTMLElement, messageId: string, query: string, isUser: boolean) => {
        ...
        const bubble = el.querySelector<HTMLElement>('[data-chat-message-bubble="true"]') ??
            el;
        const paintTarget = bubble.querySelector<HTMLElement>('[data-chat-selectable="true"]') ??
            bubble;
        ...
        const cleanupTimer = window.setTimeout(clearSearchHighlight, reducedMotion ? 1050 : 1550);
    ...
    const handleSearchJump = React.useCallback((result: ConversationMessageSearchResult) => {
        ...
        void (async () => {
            ...
            if (!el) {
                ...
                const loaded = await loadConversationMessagesAround(result.conversationId, result.messageId);
                if (loaded) {
                    el = await waitForSearchAnchor(anchorId, 20);
                }
            }
            if (el)
                scrollAndFlashSearchAnchor(el, result.messageId, query, result.role === "user");
        })();
    }, [
    ...
    const renderRowBody = React.useCallback((m: ChatMessageData, ctx: ChatMessageRenderContext | undefined) => renderOverlayMessageBody(m, ctx, openSettings), [openSettings]);
    ...
    const handleReplyMessage = React.useCallback((message: ChatMessageData) => {
    ...
    const renderThreadLine = React.useCallback((m: ShellMessage, index: number) => {
        ...
        const renderContext: ChatMessageRenderContext | undefined = isLastAssistant
            ...
            : undefined;
        return (<MessageScrollerItem key={m.id} messageId={m.id} className={cn("w-full", firstRunOpen && index > 0 && "mt-2")}>
          {m.role === "assistant" && m.planningAcknowledgment ? (<p className="px-2 pt-2 text-sm text-muted" data-testid="chat-acknowledgment">
              {m.planningAcknowledgment}
            </p>) : null}
          <ChatMessage actionAccessory={m.id === speakingSourceMessageId ? (<SpeakingStatusAccessory />) : undefined} appearance="glass" 
        ...
        agentName={agentName} message={messageData} reduceMotion={reduce} onCopy={handleCopyMessage} onLongPressCopy={handleLongPressCopy} onSpeak={handleSpeakMessage} onEdit={handleEditMessage} onReply={isInFlight ? undefined : handleReplyMessage} onRetry={handleRetry} playing={speaking && playingMessageId === m.id} renderContent={renderRowBody} renderContext={renderContext} onAcceptSuggestion={handleAcceptSuggestion} onDismissSuggestion={handleDismissSuggestion}/>
        </MessageScrollerItem>);
    }, [
        visibleMessages.length,
    ...
    const generationOwnsTrailingControl = !recording && responding && Boolean(turnStatus) && !playingMessageId;
    React.useEffect(() => {
    ...
    const submitText = React.useCallback((text: string, images: ImageAttachment[] = []) => {
    ...
    const finishTranscription = React.useCallback(async () => {
        if (transcriptionFinishingRef.current)
            return;
    ...
    const readViewport = React.useCallback(() => {
        ...
        let keyboardInset = 0;
        if (vv) {
            const insetFromInner = Math.max(0, innerHeight - vv.height - vv.offsetTop);
            let screenKeyboard = 0;
            if (SCREEN_KEYBOARD_SIGNAL_ACTIVE) {
                const screenHeight = getPhysicalScreenVerticalExtent();
                const insetFromScreen = screenHeight > 0
    ...
    React.useEffect(() => {
        if (typeof window === "undefined")
            return undefined;
        const measure = () => setSafeAreaTop((prev) => {
    ...
    const keyboardBlocksMaximize = effectiveKeyboardInset >= KEYBOARD_INTRUSION_THRESHOLD_PX;
    ...
    const fullBleed = maximizeAllowed && maximized && expanded && sheetOpen && !pilled;
    ...
    const shortLandscape = !fillHostAtHalf &&
        isShortLandscapeViewport(viewport.innerWidth, viewport.innerHeight);
    const compactLanding = shortLandscape &&
        !sheetOpen &&
        !fullBleed &&
        !composerFocused &&
        !hasDraft &&
    ...
    React.useEffect(() => {
        ...
        const publish = () => {
            ...
            if (draggingRef.current || threadHeight.get() > 0)
                return;
            const h = panel.getBoundingClientRect().height + CHAT_CLEARANCE_REST_GAP_PX;
            ...
            if (h > 0)
                root.style.setProperty("--eliza-chat-clearance", `${Math.min(Math.ceil(h), CHAT_CLEARANCE_MAX_PX)}px`);
        };
        publish();
    ...
    const desktopHostPanelMaxH = Math.max(0, fullPanelMaxH - DESKTOP_HOST_TOP_BREATHING_ROOM_PX);
    ...
    const nativeSurfaceState: ChatSurfaceState = chatActionsOpen && chatState === "INPUT" ? "INPUT_MENU" : chatState;
    React.useEffect(() => {
    ...
    const evalHeaderVisible = React.useCallback((h: number) => threadPresented && !pilled && (fullBleed || h >= halfH - 1), [threadPresented, pilled, fullBleed, halfH]);
    ...
    const threadFlexBasis = useTransform(threadHeight, (height) => `${Math.round(height)}px`);
    ...
    const animateFullBleedTo = React.useCallback((target: number) => {
    ...
    const settleFullBleed = React.useCallback(() => animateFullBleedTo(fullBleedRef.current ? 1 : 0), [animateFullBleedTo]);
    ...
    const wrapperBaseMaxW = compactLanding
        ...
        : 768;
    const wrapperMaxW = useTransform(fullBleedT, (t) => `${wrapperBaseMaxW + Math.max(0, viewport.innerWidth - wrapperBaseMaxW) * t}px`);
    ...
    const panelCapH = useTransform([threadHeight, fullBleedT, overpullCapT] as MotionValue<number>[], ([h, t, pin]: number[]) => Math.min(fullPanelMaxH, Math.max(restingPanelMaxH + maxOverPull * Math.max(t, pin), h)));
    ...
    const morphRadius = useTransform(fullBleedT, (t: number) => `${PANEL_RADIUS_PX * (1 - t)}px`);
    ...
    const surfaceBlackout = useTransform([threadHeight, fullBleedT] as MotionValue<number>[], ([h, t]: number[]) => sheetBlackoutProgress(h, halfH, t));
    ...
    const surfaceBackdropFilter = useTransform(surfaceBlackout, (t: number): string => (t >= 1 ? "none" : GLASS_SHEET_BACKDROP_FILTER));
    ...
    const composerCapsuleShadow = useTransform(fullBleedT, (t: number) => liquidGlassEdgeShadow(t));
    ...
    const rimOpacity = useTransform([glassOpacity, fullBleedT] as MotionValue<number>[], ([open, bleed]: number[]) => open * (1 - bleed));
    ...
    const grabberOpacity = useTransform([openProgress, fullBleedT] as MotionValue<number>[], ([p, t]: number[]) => grabberBarOpacity(p, t));
    ...
    const headerMaxHMorph = useTransform([headerMaxH, fullBleedT] as MotionValue<number>[], 
    ...
    const settleDrag = React.useCallback(() => {
        ...
        const liveDetentH = !liveSheetOpen
            ...
            : liveMode === "full"
                ? openH
                : halfH;
        const liveBaseH = liveFreeH != null ? Math.min(liveFreeH, panelMaxH) : liveDetentH;
    ...
    React.useEffect(() => {
        const idleInput = fillHostAtHalf &&
            !firstRunOpen &&
            effectiveMode === "input" &&
            !hasDraft &&
            !hasImages &&
        ...
        if (!idleInput)
            return undefined;
        const timeout = window.setTimeout(collapseToPill, DESKTOP_INPUT_IDLE_COLLAPSE_MS);
    ...
    const collapseFromRelease = React.useCallback((collapseHighStart = true) => {
        ...
        if ((!pilled && openProgress.get() <= PILL_COMMIT_PROGRESS) ||
            (collapseHighStart &&
    ...
    const onSheetWheel = React.useCallback((e: React.WheelEvent) => {
        ...
        if (firstRunOpen || draggingRef.current || searchOpen)
            return;
        ...
        if (now < wheelStepCooldownRef.current)
            return;
        ...
        const freeAboveHalf = freeH != null && freeH > halfH + SHEET_DETENT_MAGNET;
        if (grow) {
            if (pilled)
                ...
                goToDetent(freeBelowHalf ? "half" : "full");
            else if (!maximized && maximizeAllowed)
                maximizeFromPull();
        }
        else {
            if (maximized)
                ...
                goToDetent(freeAboveHalf ? "half" : "collapsed");
            else if (!pilled)
                collapseToPill();
        }
    }, [
    ...
    React.useEffect(() => {
        ...
        const preserveFocusedComposer = (event: Event) => {
            ...
            const staysInShell = action !== "close" &&
                action !== "close-all" &&
                action !== "open-window";
            const shouldPreserve = staysInShell &&
        ...
        return () => window.removeEventListener(NAVIGATE_VIEW_EVENT, preserveFocusedComposer);
    }, []);
    ...
    React.useEffect(() => {
        ...
        const preserveFocus = preserveUntil > 0 && preserveUntil >= performance.now();
    ...
    const expandCore = React.useCallback((snapshotPreFocus: boolean) => {
        if (!hasRevealableThread) {
    ...
    const voiceConversationActive = Boolean(realtimeVoice?.enabled &&
    ...
    React.useEffect(() => {
        ...
        if (!voiceConversationActive || wasActive || pinnedOpen)
            return;
        preFocusCollapsedRef.current = false;
        setFreeH(null);
        goToDetent("half");
    ...
    const expandFromTyping = React.useCallback(() => expandCore(false), [expandCore]);
    ...
    React.useEffect(() => {
        if (!hasRevealableThread || !pendingExpandOnRevealRef.current)
            return;
        pendingExpandOnRevealRef.current = false;
        if (typeof document === "undefined" ||
            document.activeElement !== inputRef.current) {
    ...
    React.useEffect(() => {
        ...
        const onPrefill = (event: Event) => applyPrefill((event as CustomEvent<ChatPrefillEventDetail>).detail);
        window.addEventListener(CHAT_PREFILL_EVENT, onPrefill);
        const unregister = !firstRunOpen && acceptPendingFirstRunText
    ...
    React.useEffect(() => {
        if (!acceptPendingFirstRunText || firstRunOpen)
            return;
    ...
    React.useEffect(() => {
        ...
        return () => window.removeEventListener(OS_INTENT_COMPOSER_PREFILL_EVENT, consumePrefill);
        ...
        setTranscriptSessionSink((segments, startedAtMs, audioWav) => {
            ...
            if (audioWav && hasAudio) {
                ...
                if (audioWav.byteLength > MAX_CHAT_MEDIA_RAW_BYTES) {
                    setImageError(`Recording too large to attach (max ${bytesToMb(MAX_CHAT_MEDIA_RAW_BYTES)}MB) — transcript kept.`);
                }
                else {
                    ...
                    setPendingImages((prev) => [...prev, recording].slice(0, MAX_CHAT_IMAGES));
                }
            }
            if (text || hasAudio) {
                expand();
    ...
    const cycleMessageHistory = React.useCallback((direction: -1 | 1, event: React.KeyboardEvent<HTMLTextAreaElement>): boolean => {
        if (event.altKey ||
            event.ctrlKey ||
            event.metaKey ||
            event.shiftKey ||
    ...
    const handleComposerPaste = useComposerPaste<HTMLTextAreaElement>({
        ...
        attachText: (attachment) => setPendingImages((prev) => [...prev, attachment].slice(0, MAX_CHAT_IMAGES)),
    });
    ...
    // stays OUTSIDE.
    const isOverlayControlTarget = React.useCallback((target: EventTarget | null): boolean => {
    ...
    React.useEffect(() => {
        ...
        const isAboveShellOverlay = (target: EventTarget | null): boolean => target instanceof Element &&
            !!target.closest('[data-above-shell-overlay], [role="dialog"], [data-notif-row], [data-notif-control]');
        const onPointerDown = (event: PointerEvent) => {
        ...
        const onPointerMove = (event: PointerEvent) => {
            ...
            if (!start || start.pointerId !== event.pointerId)
                return;
            if (Math.hypot(event.clientX - start.startX, event.clientY - start.startY) >
    ...
    React.useEffect(() => {
        ...
        const onKey = (e: KeyboardEvent) => {
            if (e.key === "Escape") {
                ...
                if (document.querySelector('[role="dialog"][data-state="open"], [data-testid="transcript-viewer"], [data-testid="thread-line-edit-input"]')) {
                    return;
                }
                e.preventDefault();
                collapse();
    ...
    React.useEffect(() => {
        ...
        const onBackIntent = (event: Event) => {
            ...
            if (!detail || detail.handled)
                return;
            if (!sheetOpen || pinnedOpen)
                return;
            detail.handled = true;
            collapse();
        };
        window.addEventListener(ELIZA_BACK_INTENT_EVENT, onBackIntent);
        return () => window.removeEventListener(ELIZA_BACK_INTENT_EVENT, onBackIntent);
    ...
    React.useEffect(() => {
        ...
        return () => window.removeEventListener(NAVIGATE_VIEW_EVENT, onNavigateHome);
    }, []);
    ...
    const onDragOffset = React.useCallback((offset: number) => {
        ...
        if (!draggingRef.current) {
            ...
            if (sheetOpen && typeof document !== "undefined") {
                const threadEl = document.querySelector<HTMLElement>('[data-testid="chat-thread"]');
                const actualThreadH = threadEl?.getBoundingClientRect().height;
                if (actualThreadH != null &&
                    actualThreadH > 0 &&
        ...
        const cont = Math.min(fullPanelMaxH, Math.max(-PILL_OPEN_DISTANCE, dragContRef.current + dy));
        ...
        const restorePinnedInsideSlop = restoreGestureRef.current && !restoreDidEngageRef.current;
        ...
        if (cont > 0 && cont > dragStartContRef.current) {
            ...
            if (top != null) {
                ...
                if (dragPinnedRef.current) {
                    measuredOverpullT = clamp01((cont - dragOffAtPinRef.current) /
        ...
        const measuredPanelReach = measuredPanelH > 0 && measuredPanelTop != null
            ? viewportH - Math.max(0, measuredPanelTop)
            : 0;
        const livePanelH = Math.max(measuredPanelH, measuredPanelReach, Math.max(0, cont));
        const crossedFullscreenLine = fullscreenCrossContRef.current != null
            ? cont >=
        ...
        const pillCommitCont = dragStartHRef.current > 0
    ...
    const maybeMaximizeOnRelease = React.useCallback((): boolean => {
        ...
        if (pinnedOpen)
            return false;
        if (maximizeReversedRef.current)
            return false;
        ...
        if (keyboardBlocksMaximize)
            return false;
        const peak = maxPullRawRef.current;
        const longHaulFromBottom = dragLastOffsetRef.current >= viewportH * 0.8 &&
    ...
    const pullBinding: PullGestureBinding = usePullGesture({
        ...
        onSettleFree: (direction) => {
            ...
            if (!sheetOpen && direction === "down") {
                if (dragContRef.current <= -PILL_OPEN_DISTANCE / 2)
                    collapseToPill();
                else
                    settleDrag();
                return;
    ...
    const grabberBinding = React.useMemo(() => withPressLatch(pullBinding, grabberPressRef), [pullBinding]);
    ...
    // full-bleed ceiling.
    const onRestoreDrag = React.useCallback((offset: number) => {
        if (pinnedOpen)
            return;
        ...
        if (!restoreDidEngageRef.current &&
    ...
    const restoreZoneBinding = React.useMemo(() => withPressLatch(maximizeRestoreBinding, restorePressRef), [maximizeRestoreBinding]);
    ...
    const firstRunProbe = React.useMemo(() => {
        ...
        for (let i = messages.length - 1; i >= 0; i--) {
            const region = findChoiceRegions(messages[i].content).find((r) => r.scope === "first-run" || r.scope.startsWith("first-run"));
            if (region) {
                return {
                    step: region.id,
    ...
    return (<motion.div ref={overlayRef} className={cn("pointer-events-none fixed inset-x-0 bottom-0 isolate flex w-full min-w-0 flex-col", 
    ...
    style={{
        ...
        }} data-testid="chat-overlay" data-chat-overlay="" data-chat-gesture-surface="" data-open={sheetOpen ? "true" : undefined}>
      ...
      <motion.div aria-hidden="true" data-testid="chat-sheet-backdrop" data-active={sheetOpen ? "true" : "false"} className="fixed inset-0" style={{
            visibility: scrimVisibility,
            pointerEvents: "none",
        }}/>

      {ttsError ? (<div className="pointer-events-auto relative mb-2 w-full max-w-3xl">
      ...
      {needsAudioUnlock &&
            !realtimeVoiceComposerVisible &&
            !realtimeVoice?.error ? (<div role="status" aria-live="polite" className="pointer-events-none relative mb-2 flex w-full justify-center">
          <Button variant="warningOutline" size="pillDense" onClick={unlockAudio} data-testid="overlay-voice-audio-unlock" className={cn("pointer-events-auto", WALLPAPER_FLOAT_SHADOW)}>
    ...
    style={{ maxWidth: wrapperMaxW }} className="pointer-events-none relative flex w-full flex-col items-center">
        ...
        <SheetGrabber open={sheetOpen} onOpen={openFromGrabber} onClose={collapse} binding={grabberBinding} 
        ...
        breathing={(listening || responding) && !recording} opacity={grabberOpacity} pilled={pilled} locked={firstRunOpen}/>) : null}
        <Card asChild variant="transparentSquare">
          <motion.fieldset ref={bindPanelRef} aria-label="Chat composer" data-testid="chat-sheet" onWheel={onSheetWheel} data-variant={sheetOpen ? "open" : "closed"} data-detent={detentLabel} data-maximized={fullBleed ? "true" : undefined} data-revealed={threadPresented ? "true" : "false"} data-chat-state={chatState} data-header-shown={headerVisible ? "true" : "false"} data-theme="dark" 
    ...
    data-conversation-id={conversationNav.activeId ?? undefined} data-conversation-index={conversationNav.index} 
    ...
    style={{
        ...
        "relative m-0 flex w-full min-w-0 flex-col overflow-visible p-0")}>
          ...
          paint-order contract visually true as well. */}
            <Card asChild surface="transparent" border="none" radius="none" visualStyle={{
            ...
            boxShadow: "var(--chat-sheet-shadow, none)",
              ...
              <motion.div ref={glassSurfaceRef} aria-hidden="true" data-testid="chat-sheet-surface" data-glass-tier={nativeSheetTier} className="pointer-events-none absolute inset-0 z-0" style={{
            ...
            ...({
                ...
                "--chat-sheet-background": firstRunOpen || nativeInsetSheet
                    ? "var(--bg)"
                    : surfaceBackgroundColor,
                "--chat-sheet-backdrop-filter": firstRunOpen || nativeInsetSheet
                    ? "none"
          ...
          tier system's silent J4 degrades). */}
            ...
            {firstRunProbe ? (<span className="sr-only" data-testid="onboarding-state-probe">
          ...
          behind the pill capsule. */}
            <Card asChild surface="transparent" border="none" radius="none" visualStyle={{ borderRadius: "var(--chat-sheet-radius)" }}>
              <motion.div ref={contentRef} data-testid="chat-content" inert={pilled || undefined} 
    ...
    onDragOver={(event) => {
        ...
        stays WCAG 2.1.1 operable. */}
            ...
            !pinnedOpen ? (<div className="pointer-events-auto absolute inset-x-0 top-0 z-[15]" style={{
                height: `calc(var(--safe-area-top, 0px) + ${MAXIMIZE_RESTORE_ZONE_PX}px)`,
            }}>
                    <Button {...restoreZoneBinding} type="button" variant="publicRow" size="fill" data-testid="chat-maximize-restore-zone" aria-label="drag down to exit full screen" onKeyDown={(e) => {
                if (e.key === "Enter" ||
                    e.key === " " ||
                    e.key === "ArrowDown") {
                    e.preventDefault();
      ...
      the safe-area top inset at full-bleed and host the transcribe badge. */}
                {threadPresented ? (<motion.div data-testid="chat-sheet-header" 
        ...
        inert={!sheetOpen || !headerVisible || undefined} style={{
            ...
            top inset at full-bleed and host the transcription badge. */}
                    {transcriptionComposerActive ? (<Badge variant="statusMuted" size="pill" data-testid="chat-transcribing-badge" className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap">
    ...
    is unmounted, so there is no hidden transcript layer. */}
                {threadPresented ? (<motion.div data-testid="chat-thread" className={cn(
        ...
        style={{
            ...
            composer. */}
                    {searchOpen && sheetOpen ? (<div data-testid="chat-message-search" data-keyboard-open={keyboardLiftActive ? "true" : undefined} 
            ...
            className="absolute inset-0 z-30 flex flex-col overflow-hidden px-4 pb-3 pt-2">
                        <MessageSearchPanel search={runMessageSearch} onJump={handleSearchJump} onClose={closeSearch} layout="keyboard-anchored"/>
                      </div>) : null}
                    <MessageScrollerProvider key={activeConversationId ?? "unbound"} autoScroll={!firstRunOpen} defaultScrollPosition={firstRunOpen ? "start" : "end"} scrollEdgeThreshold={MESSAGE_SCROLLER_END_THRESHOLD_PX}>
                      <MessageScrollerSendFollow request={scrollToEndRequest}/>
                      <MessageScrollerSearchBridge scrollToMessageRef={searchScrollToMessageRef}/>
                      <MessageScroller>
                        <motion.div inert={searchOpen || undefined} className="flex size-full min-h-0 flex-col" style={{
                ...
                filter: searchOpen ? "opacity(0.4)" : undefined,
                          ...
                          <MessageScrollerViewport id="continuous-thread" data-testid="chat-thread-scroll" 
        // Match the composer fade at the grabber edge in every detent.
        fade="both" ref={threadRef} preserveScrollOnPrepend={false} onScroll={handleThreadScroll} aria-label="conversation history" aria-hidden={!sheetOpen || searchOpen ? true : undefined} tabIndex={sheetOpen && !searchOpen ? 0 : -1} onKeyDown={(e) => {
                if (e.key === "Escape") {
                    e.preventDefault();
                    collapse();
                }
      ...
      wait; first-run owns its own empty state. */}
                ...
                !firstRunOpen &&
                (conversationLoading || booting) ? (<div data-testid="chat-thread-loading" className="pointer-events-none absolute inset-0 grid place-items-center">
  ...
  the first turn. */}
                            <MessageScrollerContent ref={threadContentRef} aria-busy={responding} className={cn("flex flex-col gap-0", firstRunOpen
                ? "shrink-0 pt-8"
      ...
      preserves the reader's anchor so the thread never jumps. */}
                ...
                visibleMessages.length > 0 ? (<div ref={topSentinelRef} data-testid="chat-transcript-top-sentinel" aria-hidden="true" className="pointer-events-none h-px w-full shrink-0"/>) : null}
        ...
        chronological transcript. */}
                ...
                !(visibleMessages.at(-1)?.role === "assistant" &&
                    !lastContent.trim()) ? (<TurnStatus status={turnStatus}/>) : null}
            ...
            latest turn glides clear without moving the sheet. */}
                        <AnimatePresence initial={false}>
                          {chatReplyTarget ? (<motion.div key="chat-reply-target" data-testid="chat-reply-lane" initial={reduce
                    ? false
                    : {
                        height: 0,
                        opacity: 0,
                    ...
                    ease: OVERLAY_EASE,
                              ...
                              <div className="px-3 pb-2 pt-1">
                                <ChatReplyPill appearance="glass" target={chatReplyTarget} onCancel={() => setChatReplyTarget(null)}/>
                              </div>
                            </motion.div>) : null}
                        </AnimatePresence>
                      </MessageScroller>
        ...
        44px-class target for each file. */}
                {hasImages || imageError ? (<div data-testid="chat-pending-attachments" className="pointer-events-none relative z-10 flex shrink-0 flex-col gap-1.5 px-3 pt-2">
                    {hasImages ? (<div data-testid="chat-pending-attachment-list" {...pullBinding} className="pointer-events-auto flex touch-none flex-wrap gap-2">
                        {pendingImages.map((img, i) => {
                    const kind = chatUploadKind(img.mimeType);
                    const removeButton = (<Button variant="outlineMuted" size="icon-sm" aria-label={`remove ${img.name}`} onPointerDown={(event) => event.stopPropagation()} onClick={() => removeImage(i)} 
                    ...
                    shape="circle" className="pointer-events-auto absolute -bottom-1.5 -right-1.5 z-30 before:absolute before:-inset-3 before:content-['']">
                              ×
                            </Button>);
                    const tileKey = `${img.name}-${img.mimeType}-${img.data.length}`;
                    if (kind === "image") {
                        return (<div key={tileKey} className="group relative size-14 shrink-0">
                                <Card asChild surface="transparent" border="strong" radius="large" className="overflow-hidden">
                                  <img src={`data:${img.mimeType};base64,${img.data}`} alt={img.name} className="size-14 object-cover"/>
                                </Card>
                                {removeButton}
                              </div>);
                    }
                    const KindIcon = kind === "audio"
                        ...
                        : kind === "video"
                            ? Film
                            : FileText;
                    return (<Card surface="raised" border="strong" radius="large" tone="text" key={tileKey} className="group relative flex h-14 min-w-[3.5rem] max-w-[10rem] shrink-0 items-center gap-2 px-2.5" title={img.name}>
                    ...
                    {imageError ? (<p role="alert" className={cn("text-xs", WALLPAPER_TEXT.danger, WALLPAPER_FLOAT_SHADOW)}>
                        {imageError}
                      </p>) : null}
                  </div>) : null}
                <Input ref={fileInputRef} type="file" accept={CHAT_UPLOAD_ACCEPT} multiple className="hidden" onChange={(e) => {
            if (e.target.files)
    ...
    row needs no separate entrance — it just sits at the panel base. */}
                <Card asChild surface="transparent" border="standard" radius="full" visualStyle={{
            backgroundColor: "var(--chat-composer-background)",
            borderColor: "var(--chat-composer-border)",
            boxShadow: "var(--chat-composer-shadow, none)",
        }}>
                  <motion.div data-testid="chat-composer-row" onPointerDownCapture={noteInputActivity} className={cn(
    ...
    style={{
      ...
      pure client affordance. */}
                    {!transcriptionComposerActive ? (<DropdownMenu open={chatActionsOpen} onOpenChange={setChatActionsOpen}>
                        <DropdownMenuTrigger asChild>
                          <Button variant="overlayEdge" size="icon" aria-label="chat actions" disabled={firstRunOpen} data-testid="chat-composer-plus" onPointerDown={(event) => {
        ...
        className="relative shrink-0 data-[state=open]:text-txt [&_svg]:size-5">
                        ...
                        <DropdownMenuContent data-chat-overlay-control side="top" align="start" sideOffset={10} 
        ...
        glass className="min-w-[13rem]">
                          {currentTab !== "chat" ? (<DropdownMenuItem className="cursor-pointer gap-2.5 data-[highlighted]:bg-bg-hover" onSelect={() => {
                    navigateHome?.();
                }}>
                              <House className="size-4 shrink-0 text-muted" aria-hidden/>
                              Back to Home
                            </DropdownMenuItem>) : null}
                          <DropdownMenuItem className="cursor-pointer gap-2.5 data-[highlighted]:bg-bg-hover" onSelect={() => openSearch()}>
                            <Search className="size-4 shrink-0 text-muted" aria-hidden/>
                            Search chat…
                          </DropdownMenuItem>
                          <DropdownMenuItem className="cursor-pointer gap-2.5 data-[highlighted]:bg-bg-hover" disabled={pendingImages.length >= MAX_CHAT_IMAGES} onSelect={() => fileInputRef.current?.click()}>
                            <Paperclip className="size-4 shrink-0 text-muted" aria-hidden/>
                            Upload file
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>) : null}
                    {transcriptionComposerActive ? (<ComposerMicActivity analyser={analyser} finishing={transcriptionFinishing} reduceMotion={reduce} transcript={transcript}/>) : realtimeVoiceComposerVisible && realtimeVoice ? (<ComposerRealtimeVoiceActivity connecting={realtimeVoice.connecting} error={realtimeVoice.error} needsAudioUnlock={needsAudioUnlock} onUnlockAudio={unlockAudio} paused={realtimeVoice.paused} microphoneMuted={realtimeVoice.microphoneMuted} reduceMotion={reduce} status={realtimeVoice.status} transcript={transcript}/>) : (<Textarea ref={inputRef} rows={1} value={draft} 
        ...
        disabled={cloudSignInRequired} readOnly={cloudLoginWaiting} onChange={(e) => {
                ...
                suppressExpandOnFocusRef.current = false;
            }} onPaste={handleComposerPaste} onKeyDown={(event) => {
                noteInputActivity();
                            ...
                            : noProviderConfigured
                                ...
                                : modelBlocksSend
                                    ...
                                    : booting
                                        ...
                                        : "Hey Eliza…"} aria-label="message" data-testid="chat-composer-textarea" aria-describedby={firstRunOpen
                ? "cc-first-run-hint"
        ...
        className="scrollbar-hide min-w-0 flex-1 resize-none self-center leading-relaxed disabled:pointer-events-none disabled:opacity-60"/>)}
            ...
            booting &&
            !noProviderConfigured &&
            !firstRunOpen ? (<span id="cc-booting-hint" className="sr-only">
                        {agentName} is waking up. You can type now; your message
                        sends and the reply arrives in a moment.
                      </span>) : null}
                    {firstRunOpen ? (<span id="cc-first-run-hint" className="sr-only">
                        Your answer stays in setup until your agent is ready.
                      </span>) : null}
                    {/* Trailing controls. */}
                    <div data-testid="chat-composer-trailing-controls" className={cn("grid shrink-0 items-center", realtimeVoiceControlsVisible
            ? "grid-cols-2"
            : "grid-cols-1")}>
                      {realtimeVoiceControlsVisible && realtimeVoice ? (<ComposerControlSlot slot="left" reduceMotion={reduce} controlKey={realtimeVoice.microphoneMuted
                ? "voice-microphone-muted"
                : "voice-microphone-live"}>
                          <SoftButton icon={realtimeVoice.microphoneMuted ||
                realtimeVoice.status === "speaking"
                ? MicOff
                : Mic} label={realtimeVoice.status === "speaking"
                ? "Microphone paused while speaking"
                : realtimeVoice.microphoneMuted
                    ? "unmute microphone"
                    : "mute microphone"} disabled={realtimeVoice.status === "speaking"} active={realtimeVoice.microphoneMuted} pressed={realtimeVoice.microphoneMuted} onClick={realtimeVoice.toggleMicrophoneMute} testId="chat-composer-voice-mute"/>
                        </ComposerControlSlot>) : null}
      ...
      held pointer has exactly the same Cartesia action as a tap. */}
                      <ComposerControlSlot slot="right" reduceMotion={reduce} controlKey={transcriptionComposerActive
            ? "transcription-stop"
     ...
     every other composer pixel until capture finishes. */
        <SoftButton glyph={STOP_GLYPH} label={transcriptionFinishing
                ? "finishing transcription"
                : "stop transcription"} disabled={firstRunOpen || transcriptionFinishing} onPointerDown={(event) => event.preventDefault()} onClick={() => void finishTranscription()} testId="chat-composer-transcription-stop"/>) : draftOwnsTrailingControl ? (<SoftButton icon={SendHorizontal} label={firstRunOpen
                ? "send to setup assistant"
                : !canSend
                    ? "send (agent stopped)"
                    : responding
                        ? "send another"
                        : "send"} disabled={firstRunOpen
                ? cloudLoginWaiting || !sendFirstRunText
                : !canSend} onPointerDown={(event) => event.preventDefault()} onClick={submit} testId="chat-composer-action"/>) : generationOwnsTrailingControl ? (<SoftButton glyph={STOP_GLYPH} label={turnStatus?.kind === "speaking"
                ? "stop speaking"
                : "stop generating"} onClick={() => stop()} testId="chat-composer-stop"/>) : (<SoftButton glyph={handsFree ? STOP_GLYPH : undefined} icon={handsFree ? undefined : AudioLines} label={realtimeVoice?.error
                ...
                : realtimeVoice?.connecting
                    ...
                    : handsFree
                        ...
                        : recording
                            ? "stop listening"
                            : "talk"} disabled={firstRunOpen} active={handsFree} pressed={recording || handsFree} pulse={recording && !handsFree} onClick={handleMicClick} testId="chat-composer-mic"/>)}
                      </ComposerControlSlot>
                    </div>
                  </motion.div>
                </Card>
            ...
            <Card asChild surface="transparent" border="strong" radius="none" visualStyle={{ borderRadius: "var(--chat-sheet-radius)" }}>
              <motion.div data-testid="chat-sheet-rim" aria-hidden="true" className="pointer-events-none absolute inset-0 z-40" style={{
            opacity: rimOpacity,
            ...({
                "--chat-sheet-radius": morphRadius,
            } satisfies ChatSheetMotionStyle),
          ...
          forms. Interactive only while pilled; sits over the (faded) input. */}
            <motion.div className="absolute inset-x-0 bottom-0 z-30 flex justify-center" style={{
            opacity: pillOpacity,
            pointerEvents: pilled ? "auto" : "none",
        }}>
              {pilled && responding && turnStatus && !fillHostAtHalf ? (<div className={cn("pointer-events-none absolute bottom-7 max-w-[calc(100vw-2rem)] overflow-hidden whitespace-nowrap", "[&_[data-slot=marker-content]]:truncate", WALLPAPER_FLOAT_SHADOW)} data-testid="chat-pill-turn-status">
                  <TurnStatus status={turnStatus} showLabel={false}/>
                </div>) : null}
              <PillHandle binding={pullBinding} counterScale={pillCounterScale} onOpen={openFromPill} 
    ...
    breathing={listening || responding || recording} pilled={pilled} desktopOverlayHost={fillHostAtHalf}/>
            </motion.div>
          </motion.fieldset>
        </Card>
      </motion.div>
