...
// Lowered 0.3 → 0.22 so maximize commits a little EARLIER in the over-pull zone:
// the finger reaches edge-to-edge before it has to travel all the way into the
// status-bar / notch band at the very top, where the OS owns the pull gesture
// (notification shade / control center). Keep the hysteresis gap above RELEASE.
const MAXIMIZE_COMMIT_T = 0.22;
const MAXIMIZE_RELEASE_T = 0.12;
...
// Lowered 24 → 16 so a pull-DOWN from maximized drops full-bleed a little
// EARLIER — the sheet leaves the top edge (and the OS notification-shade /
// control-center pull zone) promptly instead of after a longer downward drag.
// Still enough slop to absorb touch jitter so a held-at-top hand doesn't flap.
const RESTORE_UNMAX_SLOP = 16;
...
export function ChatOverlay({
  ...
  // path can tell an over-pull past the 72%-viewport maximize threshold from a
  ...
  // The `"native"` tier is one value for two very different materials: iOS a
  // UIGlassEffect liquid frost, Android a flat Material sheet. The chosen
  // material must be STABLE across every detent. Keying it on the platform
  // (constant) + tier (never downgrades) — NOT on the transient
  // open/maximize/collapse state — is what stops the sheet BLIPPING from native
  // material to the CSS liquid-glass fallback for the frames a drag/collapse
  // leaves an open-state gate momentarily false. `nativeBackdropActive` is the
  // one non-detent input (installed once at boot), so it does not flip per drag.
  const glassPlatform = glassTier === "native" ? nativeGlassPlatform() : null;
  // Android's Material sheet is OPAQUE — it masks on its own and never refracts,
  // so it does not depend on the native backdrop and is stable the moment the
  // tier resolves. iOS's frost is translucent and reads the UIGlassEffect behind
  // the webview, so it requires the backdrop to be installed to look right.
  const androidNative = glassPlatform === "android";
  const iosNative = glassPlatform === "ios" && nativeBackdropActive;
  // Only iOS anchors a real native glass PANEL behind the sheet: its surface is
  // mostly transparent so the UIGlassEffect below the webview is the visible
  // frost. Android's Material sheet is opaque (see the fill below), so a panel
  // behind it is fully occluded — attaching/detaching one per detent was pure
  // native-view churn (new View + GradientDrawable + LayoutParams every open,
  ...
  // the 72%-of-viewport threshold rises to the FULL detent and drops the inset,
  ...
  // peak raw pull clears 72% of the viewport height.
  ...
  const onDragOffset = React.useCallback(
    (offset: number) => {
      ...
      const liveLongHaul = livePeakTravel >= viewportH * 0.72;
  ...
  // upward travel (maxPullRawRef, pre-clamp/pre-pin) cleared 72% of the viewport
  ...
  const maybeMaximizeOnRelease = React.useCallback((): boolean => {
    ...
    //  - LONG HAUL: the drag swept ≥72% of the screen — "grabbed it and threw
    //    it to the top". Short free-rest flicks step to FULL because the raw
    //    travel stays below this threshold.
    // The 72% is measured against the LAYOUT viewport (screen space), not the
    // keyboard-shrunk visual viewport: with a soft keyboard up, 72% of the
    ...
    const longHaul = peakTravel >= screenH * 0.72;
  ...
  const pullBinding: PullGestureBinding = usePullGesture({
    ...
    onPullUp: () => {
      ...
      if (pilled) {
        ...
        // long pull from the pill lands FULL (or commits maximize past the 72%
      ...
      // Over-pull past the 72%-viewport threshold maximizes from ANY open state
    ...
    onSettleFree: (direction) => {
      ...
      // A slow upward over-pull past the 72%-viewport threshold maximizes
      ...
      >
        ...
        >
          ...
          <motion.div
            ...
            data-native-glass={androidNative || iosNative ? "on" : undefined}
            ...
            data-glass-rim={firstRunOpen || androidNative ? undefined : "on"}
            className={cn(
