...
import { getShellSurface } from "../../../state/shell-surface-store";
...
import { type VoiceCaptureFactoryOptions } from "../../../voice/voice-capture-factory";
...
const realtimeVoiceMock = vi.hoisted(() => {
    const holder = {
        ...
        start: vi.fn(async (): Promise<RealtimeVoiceStartOutcome> => ({ kind: "live" })),
        stop: vi.fn(async () => { }),
        unlock: vi.fn(async () => { }),
        bargeIn: vi.fn(),
    ...
    return Object.assign(holder, {
        state: {
            ...
            error: null as {
                kind: "permission" | "no-device" | "mint" | "consent" | "transport" | "unknown";
                message: string;
                actionable: boolean;
            } | null,
            fallbackReason: null,
...
const { useAppSelectorShallowMock } = vi.hoisted(() => ({
    useAppSelectorShallowMock: (selector: (value: typeof appMock.value) => unknown) => selector(appMock.value),
}));
vi.mock("../../../state", () => ({
...
vi.mock("../../../voice/local-asr-capture", async (importOriginal) => {
    const actual = await importOriginal<typeof import("../../../voice/local-asr-capture")>();
    return {
        ...actual,
...
afterEach(() => {
    ...
    appMock.value.ensureActiveConversation = vi.fn(async () => appMock.value.activeConversationId ?? null);
    ...
    micPermissionMock.query.mockImplementation(async () => micPermissionMock.state);
...
describe("useShellController", () => {
    ...
    it("sends through immediately even while warming — the server holds the turn", () => {
        ...
        expect(appMock.value.sendChatText.mock.calls[0]?.[0]).toBe("hello while booting");
    });
...
describe("useShellController — conversation loading watchdog", () => {
    ...
    it("force-clears the loading spinner when the new-chat create hangs", async () => {
        ...
        appMock.value.handleNewConversation = vi.fn(() => new Promise<void>((r) => {
            resolveCreate = () => r();
        }));
        const { result } = renderHook(() => useShellController());
    ...
    it("drops stale swipe callbacks while a conversation switch is pending", async () => {
        ...
        appMock.value.handleSelectConversation = vi.fn(() => new Promise<void>((resolve) => {
        ...
        expect(appMock.value.handleSelectConversation).toHaveBeenCalledExactlyOnceWith("c");
        expect(result.current.conversationLoading).toBe(true);
        await act(async () => {
            resolveSwitch?.();
            await vi.advanceTimersByTimeAsync(0);
    ...
    it("re-resolves a stale swipe callback against the latest active conversation", async () => {
        ...
        expect(appMock.value.handleSelectConversation).toHaveBeenCalledExactlyOnceWith("b");
        expect(result.current.conversationLoading).toBe(false);
    });
});
...
describe("useShellController — conversation-nav interleaving (#9954)", () => {
    ...
    function wireMutableConversations(initialIds: string[]): void {
        ...
        appMock.value.handleSelectConversation = vi.fn((id: string) => {
            ...
            const neighbours = [
                resolveAdjacentConversationId(appMock.value.conversations, active, "prev"),
                resolveAdjacentConversationId(appMock.value.conversations, active, "next"),
            ];
    ...
    async function drive(result: {
        current: Ctl;
    }, rerender: () => void, action: Action, rng: () => number): Promise<void> {
        await act(async () => {
            if (action === "next")
                result.current.conversationNav.goNext();
            else if (action === "prev")
                result.current.conversationNav.goPrev();
            else if (action === "new")
    ...
    function assertInvariants(ctl: Ctl): void {
        ...
        expect(nav.hasNext).toBe(expectedIndex >= 0 && expectedIndex < list.length - 1);
    ...
    it("named sequence swipe-back → new → forward → new → forward → swipe-back stays index-consistent", async () => {
        ...
        for (const action of sequence) {
            ...
            const atIndex0Boundary = action === "prev" && result.current.conversationNav.index === 0;
        ...
        expect(appMock.value.conversations.some((c) => c.id === appMock.value.activeConversationId)).toBe(true);
    });
    it("seeded random walks keep the nav invariants on every step", async () => {
        ...
        for (let seed = 1; seed <= 12; seed++) {
            ...
            const { result, rerender, unmount } = renderHook(() => useShellController());
    ...
    it("rapid swipe bursts never resolve against a stale index (#10042 regression)", async () => {
        ...
        const fire = (ctl: Ctl, action: Action): void => {
            if (action === "next")
                ctl.conversationNav.goNext();
            else if (action === "prev")
                ctl.conversationNav.goPrev();
            else if (action === "new")
...
describe("useShellController — turnStatus derivation", () => {
    ...
    it("surfaces a waking server status even before chatSending settles", () => {
        composerMock.value.chatSending = false;
        appMock.serverTurnStatus = { kind: "waking" } as {
            kind: string;
        };
        const { result } = renderHook(() => useShellController());
    ...
    it("speaking (voice output) wins over the server status", () => {
        ...
        composerMock.value.chatSending = true;
        appMock.serverTurnStatus = { kind: "streaming" } as {
            kind: string;
        };
        const { result } = renderHook(() => useShellController());
...
function fireFinalTranscript(text: string, extra: Partial<Parameters<NonNullable<CaptureOpts["onTranscript"]>>[0]> = {}): void {
...
describe("useShellController — voice capture routing", () => {
    ...
    it("engage does not open the mic when the mic grant is known-denied", async () => {
        ...
        expect(appMock.value.setActionNotice).toHaveBeenCalledWith(expect.stringContaining("Microphone access is off"), "error", expect.any(Number));
    });
    ...
    it("a denied to re-enable tap engages on the first retry via a fresh probe", async () => {
        ...
        expect(appMock.value.setActionNotice).not.toHaveBeenCalledWith(expect.stringContaining("Microphone access is off"), "error", expect.any(Number));
        // And always-on is persisted so a reload restores the recovered loop.
        expect(window.localStorage.getItem("eliza:voice:continuous-chat-mode")).toBe("always-on");
    });
    ...
    it("voice-settings apply always-on engages the mounted hands-free shell", async () => {
        ...
        expect(result.current.handsFree).toBe(false);
        await act(async () => {
            emitViewEvent(VOICE_SETTINGS_APPLY_EVENT, { continuous: "always-on" }, "agent");
            await Promise.resolve();
        });
        expect(result.current.handsFree).toBe(true);
        expect(result.current.isOpen).toBe(true);
    ...
    it("wake word DURING transcription sends one inline reply and KEEPS recording (#9880)", async () => {
        ...
        expect(appMock.value.sendChatText).toHaveBeenCalledTimes(1);
        expect(appMock.value.sendChatText.mock.calls[0]?.[0]).toBe("what is on my calendar");
    ...
    it("HOLDS a slow-speaker mid-clause turn and sends only the completed turn (EOT)", async () => {
        ...
        expect(appMock.value.sendChatText).toHaveBeenCalledTimes(1);
        expect(appMock.value.sendChatText.mock.calls[0]?.[0]).toBe("schedule a meeting with bob tomorrow");
    });
    ...
    it("restores a persisted always-on mode by engaging the loop on mount", async () => {
        ...
        window.localStorage.setItem("eliza:voice:continuous-chat-mode", "always-on");
        const { result } = renderHook(() => useShellController());
        await act(async () => {
            await vi.advanceTimersByTimeAsync(0);
        });
    ...
    it("persists always-on on tap and restores the prior mode on tap-off", async () => {
        ...
        window.localStorage.setItem("eliza:voice:continuous-chat-mode", "vad-gated");
        const { result } = renderHook(() => useShellController());
        await act(async () => {
            result.current.toggleHandsFree();
        });
        ...
        expect(result.current.handsFree).toBe(false);
        expect(window.localStorage.getItem("eliza:voice:continuous-chat-mode")).toBe("vad-gated");
    });
...
describe("useShellController — transcription mode", () => {
    ...
    it("starts/stops transcription on a voice-control window event (agent action)", () => {
        ...
        expect(result.current.transcriptionMode).toBe(false);
        act(() => {
            window.dispatchEvent(new CustomEvent("eliza:voice-control", {
        ...
        expect(result.current.transcriptionMode).toBe(true);
        act(() => {
            window.dispatchEvent(new CustomEvent("eliza:voice-control", { detail: { command: "stop" } }));
        });
        expect(result.current.transcriptionMode).toBe(false);
    });
    ...
    function sinkSessions(result: {
        ...
        act(() => result.current.setTranscriptSessionSink((segments, startedAt, audioWav) => sessions.push({
    ...
    it("drains the hands-free recorder through transcription before opening its replacement", async () => {
        ...
        await act(async () => {
            enterTranscription = Promise.resolve(result.current.toggleTranscriptionMode());
            await Promise.resolve();
        });
        expect(createVoiceCaptureMock).toHaveBeenCalledTimes(1);
        await act(async () => {
...
describe("useShellController — conversation change stops TTS + resets voice latch", () => {
    ...
    it("swiping to another conversation stops in-flight TTS and clears lastTurnVoice", async () => {
        ...
        act(() => result.current.send("what's the weather", { channelType: "VOICE_DM" }));
    ...
    it("clearing the conversation stops in-flight TTS and clears lastTurnVoice", async () => {
        const { result } = renderHook(() => useShellController());
        act(() => result.current.send("remind me later", { channelType: "VOICE_DM" }));
        expect(voiceOutputMock.lastTurnVoiceSeen).toBe(true);
        voiceOutputMock.stopSpeaking.mockClear();
        await act(async () => {
            result.current.clearConversation();
...
describe("useShellController — mounted Cartesia Talk ownership", () => {
    ...
    it("routes a negative conversation probe to batch capture and output", async () => {
        ...
        expect(appMock.value.sendChatText).toHaveBeenCalledWith("continue in batch", expect.objectContaining({ channelType: "VOICE_DM" }));
    });
    ...
    it("stops Talk on identity loss without batch capture or automatic mic recovery", async () => {
        ...
        try {
            ...
            expect(createVoiceCaptureMock).not.toHaveBeenCalled();
            expect(voiceOutputMock.realtimeVoiceEnabledSeen).toBe(false);
            expect(appMock.value.setActionNotice).toHaveBeenCalledWith(expect.stringContaining("Tap Talk to reconnect"), "error", 6000);
            expect(window.localStorage.getItem("eliza:voice:continuous-chat-mode")).toBe("off");
            await act(async () => {
                realtimeVoiceMintMock.agentId = "eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee";
                realtimeVoiceMock.state.available = true;
                rerender();
            ...
            expect(result.current.realtimeVoice?.enabled).toBe(true);
            expect(voiceOutputMock.realtimeVoiceEnabledSeen).toBe(true);
            expect(appMock.value.setActionNotice).not.toHaveBeenCalledWith(expect.stringContaining("Cartesia voice is not ready"), "error", expect.any(Number));
        }
        finally {
            vi.useRealTimers();
        }
    ...
    it.each(["active", "connecting"] as const)("routes an %s realtime owner through realtime teardown", async (ownerState) => {
    ...
    it("parks Talk visibly OFF when a LIVE session dies past the client's recovery budget", async () => {
        ...
        expect(result.current.handsFree).toBe(false);
        expect(appMock.value.setActionNotice).toHaveBeenCalledWith("Voice connection dropped. Tap the mic to try again.", "error", 6000);
    ...
    it("surfaces a retryable Cartesia error instead of falling back to batch", async () => {
        ...
        expect(appMock.value.setActionNotice).toHaveBeenCalledWith(expect.stringContaining("Tap Talk to retry"), "error", 6000);
    });
    it("preserves a classified voice setup error instead of replacing it with a consent diagnosis", async () => {
        ...
        expect(result.current.handsFree).toBe(false);
        expect(createVoiceCaptureMock).not.toHaveBeenCalled();
        expect(appMock.value.setActionNotice).toHaveBeenCalledWith(message, "error", 6000);
    });
    it("surfaces the precise browser microphone setup timeout from realtime voice", async () => {
        realtimeVoiceMock.startOutcome = {
            kind: "fallback-to-batch",
            reason: "transport",
            message: "Microphone setup timed out. Check browser microphone permission, then tap Talk to retry.",
        };
        const { result } = renderHook(() => useShellController());
        await act(async () => {
            result.current.toggleHandsFree();
        ...
        expect(result.current.handsFree).toBe(false);
        expect(result.current.realtimeVoice?.error).toContain("Microphone setup timed out");
        expect(appMock.value.setActionNotice).toHaveBeenCalledWith("Microphone setup timed out. Check browser microphone permission, then tap Talk to retry.", "error", 6000);
    });
    it("waits for recovered history before using a provisional conversation UUID", async () => {
        appMock.value.activeConversationId = null;
        let finishGreeting: (() => void) | null = null;
        appMock.value.ensureActiveConversation = vi.fn(() => new Promise<string | null>((resolve) => {
    ...
    it("waits for startup hydration instead of creating an orphan conversation", async () => {
        ...
        appMock.value.ensureActiveConversation = vi.fn(() => new Promise((resolve) => {
    ...
    it("reconciles gateway-written voice turns through the canonical conversation loader", () => {
        ...
        try {
            renderHook(() => useShellController());
            const onServerEvent = realtimeVoiceMock.options?.clientOptions?.onServerEvent;
...
describe("useShellController cloud-only auth gate", () => {
    ...
    it("opens chat while checking but keeps PTT closed and retries auth visibly", () => {
        ...
        expect(appMock.value.setActionNotice).toHaveBeenCalledWith("Checking your Eliza Cloud session. Try push-to-talk again in a moment.", "info", 4000);
    });
    it("opens chat while Cloud is unavailable and retries PTT with an error", () => {
        ...
        expect(createVoiceCaptureMock).not.toHaveBeenCalled();
        expect(authStatusMock.revalidate).toHaveBeenCalledTimes(1);
        expect(appMock.value.setActionNotice).toHaveBeenCalledWith("Unable to reach Eliza Cloud. Retrying the connection.", "error", 4000);
    });
    ...
    it("sign-out is terminal for hands-free: no auto re-listen after re-login", async () => {
        ...
        try {
            ...
            expect(result.current.recording).toBe(false);
            expect(result.current.isOpen).toBe(false);
            expect(window.localStorage.getItem("eliza:voice:continuous-chat-mode")).not.toBe("always-on");
    ...
    it("sign-out invalidates a transcription handoff waiting on recorder drain", async () => {
        ...
        act(() => result.current.toggleHandsFree());
        expect(createVoiceCaptureMock).toHaveBeenCalledTimes(1);
        captureHandles[0]?.stop.mockImplementation(() => new Promise<void>((resolve) => {
            resolveStop = resolve;
        }));
    ...
    it("does not restore persisted always-on after auth gates during permission probe", async () => {
        ...
        micPermissionMock.query.mockImplementationOnce(() => new Promise((resolve) => {
