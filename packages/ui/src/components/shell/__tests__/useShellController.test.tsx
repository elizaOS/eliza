...
describe("useShellController — transcription mode", () => {
  ...
  it("waits for hands-free startup before switching its single recorder to transcription", async () => {
    ...
    let switchToTranscription: Promise<void> | undefined;
    await act(async () => {
      switchToTranscription = Promise.resolve(
        result.current.toggleTranscriptionMode(),
      );
      await Promise.resolve();
    });
    ...
    act(() => handsFreeOptions?.onStateChange?.("stopped"));
    expect(result.current.recording).toBe(true);
  });

  it("surfaces a recorder drain failure through the shell notice boundary", async () => {
    appMock.value.setActionNotice.mockClear();
    const { result } = renderHook(() => useShellController());
    await act(async () => result.current.toggleTranscriptionMode());
    captureHandles[0]?.stop.mockRejectedValueOnce(
      new Error("ASR drain failed after recorder stop"),
    );

    await act(async () => result.current.toggleTranscriptionMode());

    expect(appMock.value.setActionNotice).toHaveBeenCalledWith(
      "Voice capture failed. Check your microphone and connection, then try again.",
      "error",
      6000,
    );
    expect(captureHandles[0]?.dispose).toHaveBeenCalledTimes(1);
