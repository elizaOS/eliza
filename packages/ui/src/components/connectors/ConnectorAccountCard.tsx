...
import { presentConnectorAccountStatus } from "../capabilities/connected-capability-presentation";
import { presentConnectorCapabilityChips } from "../capabilities/connected-capability-presentation";
import { readConnectorAccountCapabilityAccess } from "../capabilities/connected-capability-presentation";
import { type ConnectorAccountRecord } from "../../api/client-agent";
import { type ConnectorAccountStatus } from "../../api/client-agent";
import { type ConnectorAccountUpdateInput } from "../../api/client-agent";
...
function formatRelativeTime(epochMs: number | undefined, t: TranslateFn): string {
    if (!epochMs)
...
function deriveStatus(status: ConnectorAccountStatus | undefined, t: TranslateFn): StatusInfo {
    switch (status) {
        case "connected":
            return {
...
export function ConnectorAccountCard({ account, isDefault = account.isDefault === true, selected = false, saving = false, testBusy = false, refreshBusy = false, onSelect, onUpdate, onTest, onRefresh, onDelete, onMakeDefault, declaredCapabilities, onGrantCapability, grantBusyCapabilityId = null, onReconnect, reconnectBusy = false, }: ConnectorAccountCardProps) {
    ...
    const capabilityChips = declaredCapabilities && declaredCapabilities.length > 0
        ? presentConnectorCapabilityChips(capabilityAccess, declaredCapabilities)
        : undefined;
    const showReconnect = statusPresentation.needsReconnect && Boolean(onReconnect);
    ...
    return (<Card border={selected ? "strong" : "subtle"} flow="column" gap="default" padding="default" surface={selected ? "raised" : enabled ? "card" : "backgroundSubtle"} className="transition-opacity">
      <div className="flex flex-wrap items-start gap-3">
        <Card variant="connectorAvatar" className="flex size-9 shrink-0 items-center justify-center">
          {account.avatarUrl ? (<img src={account.avatarUrl} alt="" width={36} height={36} className="h-full w-full object-cover"/>) : (initials(account.label))}
        </Card>

        <div className="min-w-[180px] flex-1 space-y-1">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <StatusBadge label={status.label} tone={statusPresentation.tone} withDot/>
            <EditableAccountLabel value={account.label} disabled={saving} onSubmit={(label) => onUpdate({ label })} inputAriaLabel={t("connectoraccount.labelAria", {
            defaultValue: "Connector account label",
        })}/>
            {isDefault ? (<Badge variant="outline" size="compact" className="shrink-0">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-2xs text-muted">
            {displayHandle ? (<span className="max-w-[220px] truncate">{displayHandle}</span>) : null}
            <span>{formatRelativeTime(account.lastSyncedAt, t)}</span>
            {account.statusDetail ? (<span className="max-w-[260px] truncate text-warn">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-1.5">
          {onSelect ? (<Button type="button" variant={selected ? "default" : "outline"} size="sm" disabled={saving || selected} onClick={onSelect}>
              {selected
                ? t("connectoraccount.selected", { defaultValue: "Selected" })
                : t("connectoraccount.use", { defaultValue: "Use" })}
            </Button>) : null}
          {showReconnect ? (<Button type="button" variant="default" size="sm" disabled={saving || reconnectBusy} onClick={() => onReconnect?.()} aria-label={t("connectoraccount.reconnect", {
                defaultValue: "Reconnect account",
            })}>
              {reconnectBusy ? (<Spinner className="size-3"/>) : (<KeyRound className="size-3.5" aria-hidden/>)}
              {t("connectoraccount.reconnectLabel", {
                defaultValue: "Reconnect",
            })}
            </Button>) : null}
          <Button type="button" variant="ghost" size="icon-sm" disabled={saving || isDefault || defaultBusy} onClick={() => void handleMakeDefault()} aria-label={t("connectoraccount.makeDefault", {
            defaultValue: "Make default account",
        })} title={t("connectoraccount.makeDefault", {
            defaultValue: "Make default account",
        })}>
            {defaultBusy ? (<Spinner className="size-3"/>) : (<Star className="size-3.5" aria-hidden/>)}
          </Button>
          <Button type="button" variant="outline" size="sm" disabled={saving || testBusy} onClick={() => void onTest()} aria-label={t("connectoraccount.test", {
            defaultValue: "Test connector account",
        })}>
            {testBusy ? (<Spinner className="size-3"/>) : (t("connectoraccount.test", { defaultValue: "Test" }))}
          </Button>
          <Button type="button" variant="outline" size="icon-sm" disabled={saving || refreshBusy} onClick={() => void onRefresh()} aria-label={t("connectoraccount.refresh", {
            defaultValue: "Refresh connector account",
        })} title={t("connectoraccount.refresh", {
            defaultValue: "Refresh connector account",
        })}>
            {refreshBusy ? (<Spinner className="size-3"/>) : (<RefreshCw className="size-3.5" aria-hidden/>)}
          </Button>
          <Button type="button" variant="destructive" size="icon-sm" disabled={saving} onClick={deleteModal.open} aria-label={t("connectoraccount.delete", {
      ...
      {capabilityChips !== undefined ? (<ConnectedCapabilityChips chips={capabilityChips} grantBusyCapabilityId={grantBusyCapabilityId} onGrantCapability={onGrantCapability}/>) : null}

      <div className="flex flex-wrap items-center gap-3">
        <ConnectorAccountPurposeSelector value={account.role} disabled={saving} accountLabel={account.label} onChange={(role, confirmation) => {
            void onUpdate({ role, confirmation });
        }}/>
        <ConnectorAccountPrivacySelector value={account.privacy} disabled={saving} accountLabel={account.label} onChange={(privacy, confirmation) => onUpdate({ privacy, confirmation })}/>
        <div className="inline-flex items-center gap-1.5 text-xs text-muted">
          <Checkbox checked={enabled} disabled={saving} onCheckedChange={(checked) => {
      ...
      <Dialog open={confirmingDelete} onOpenChange={(open) => {
            if (!open && !deleteBusy)
                deleteModal.close();
        }}>
        <DialogContent>
          <DialogHeader>
            ...
            defaultValue: "Removing the account deletes its connector metadata and may revoke stored auth state once backend support is enabled. This cannot be undone.",
        })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2">
            <Button type="button" variant="ghost" disabled={deleteBusy} onClick={deleteModal.close}>
              {t("connectoraccount.removeDialog.cancel", {
            defaultValue: "Cancel",
        })}
            </Button>
            <Button type="button" variant="destructive" disabled={deleteBusy} onClick={handleDelete}>
              {deleteBusy ? (<Spinner className="size-3"/>) : (t("connectoraccount.removeDialog.confirm", {
