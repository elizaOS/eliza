...
export function ConnectorOAuthCapabilityPicker({ capabilities, selected, onChange, }: ConnectorOAuthCapabilityPickerProps) {
    return (<Card asChild variant="transparent" border="subtle" padding="default">
      <fieldset className="grid gap-2 md:grid-cols-2">
        ...
        {capabilities.map((capability) => {
            ...
            return (<div key={capability.id} className="flex items-start gap-2">
              <Checkbox id={inputId} checked={selected.has(capability.id)} onCheckedChange={(value) => onChange(capability.id, value === true)}/>
