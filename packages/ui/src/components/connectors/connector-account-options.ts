 ...
 */
import { CONNECTOR_ACCOUNT_CATALOG } from "@elizaos/core/connector-account-catalog";
import { getConnectorAccountCatalogEntry } from "@elizaos/core/connector-account-catalog";
import { normalizeConnectorCatalogId as normalizeConnectorCatalogIdShared } from "@elizaos/core/connector-account-catalog";
import { type ConnectorAccountCatalogEntry } from "@elizaos/core/connector-account-catalog";
...
export const CONNECTOR_ACCOUNT_MANAGEMENT_PANEL_PREFIX = "connector-account-management";
 ...
 */
const CONNECTOR_PLUGIN_MANAGED_PRESENTATION: Readonly<Record<string, {
    title: string;
    description: string;
}>> = {
    telegram: {
        title: "Telegram accounts",
        description: "Manage Telegram bot accounts through @elizaos/plugin-telegram account inventory.",
    },
    google: {
        title: "Google accounts",
        description: "Manage Google Workspace accounts through @elizaos/plugin-google-workspace OAuth account inventory.",
    },
    x: {
        title: "X accounts",
        description: "Manage X/Twitter accounts through @elizaos/plugin-x account inventory.",
    },
    slack: {
        title: "Slack accounts",
        description: "Manage Slack workspace accounts through @elizaos/plugin-slack OAuth account inventory.",
    },
    whatsapp: {
        title: "WhatsApp accounts",
        description: "Manage WhatsApp account records through @elizaos/plugin-whatsapp account inventory.",
    },
};
/**
 ...
 */
function toPluginManagedAccountOption(entry: ConnectorAccountCatalogEntry): ConnectorPluginManagedAccountOption {
 ...
 */
export const CONNECTOR_PLUGIN_MANAGED_ACCOUNT_OPTIONS: readonly ConnectorPluginManagedAccountOption[] = CONNECTOR_ACCOUNT_CATALOG.map(toPluginManagedAccountOption);
const CONNECTOR_PLUGIN_MANAGED_ACCOUNT_OPTIONS_BY_ID = new Map(CONNECTOR_PLUGIN_MANAGED_ACCOUNT_OPTIONS.flatMap((option) => [
...
export function getConnectorPluginManagedAccountOption(connectorId: string | undefined): ConnectorPluginManagedAccountOption | null {
    if (!connectorId)
        return null;
    ...
    if (!entry)
        return null;
    return (CONNECTOR_PLUGIN_MANAGED_ACCOUNT_OPTIONS_BY_ID.get(entry.connectorId) ??
        null);
}
export function hasConnectorPluginManagedAccounts(connectorId: string | undefined): boolean {
    return getConnectorPluginManagedAccountOption(connectorId) !== null;
}
export function connectorAccountManagementPanelPluginId(connectorId: string): string | null {
...
export function parseConnectorAccountManagementPanelPluginId(pluginId: string): {
    provider: string;
    connectorId: string;
} | null {
...
export function getConnectorPluginManagedAccountCreateInput(connectorId: string): ConnectorAccountCreateInput | undefined {
    const option = getConnectorPluginManagedAccountOption(connectorId);
    if (!option || option.supportsOAuth)
        return undefined;
    return {
...
export function getConnectorPurposeOption(value: ConnectorAccountRole | undefined): ConnectorAccountOption<ConnectorAccountRole> {
    return (CONNECTOR_ACCOUNT_PURPOSE_OPTIONS.find((option) => option.value === value) ?? CONNECTOR_ACCOUNT_PURPOSE_OPTIONS[0]);
}
export function getConnectorPrivacyOption(value: ConnectorAccountPrivacy | undefined): ConnectorAccountOption<ConnectorAccountPrivacy> {
    return (CONNECTOR_ACCOUNT_PRIVACY_OPTIONS.find((option) => option.value === value) ?? CONNECTOR_ACCOUNT_PRIVACY_OPTIONS[0]);
}
export function getConnectorPrivacyConfirmationRequirement(current: ConnectorAccountPrivacy | undefined, next: ConnectorAccountPrivacy): ConnectorPrivacyConfirmationRequirement {
    const resolvedCurrent = current ?? "owner_only";
    if (next === resolvedCurrent)
        return "none";
    if (next === "public" && resolvedCurrent !== "public")
        return "public";
...
export function isConnectorPrivacyConfirmationSatisfied(requirement: ConnectorPrivacyConfirmationRequirement, typedValue: string, publicAcknowledged: boolean): boolean {
    ...
    return (normalized === CONNECTOR_PRIVACY_PUBLIC_CONFIRMATION && publicAcknowledged);
}
export function getConnectorRoleConfirmationRequirement(current: ConnectorAccountRole | undefined, next: ConnectorAccountRole): ConnectorRoleConfirmationRequirement {
    return next === "OWNER" && (current ?? "OWNER") !== "OWNER"
        ? "owner"
        : "none";
}
export function isConnectorRoleConfirmationSatisfied(requirement: ConnectorRoleConfirmationRequirement, typedValue: string): boolean {
    if (requirement === "none")
        return true;
