...
import { CLOUD_PAIR_LEGACY_STORAGE_KEY } from "@elizaos/core/contracts/cloud-pair";
...
export function getCloudPairTokenFromLocation(locationLike: Pick<Location, "pathname" | "search"> | null = typeof window ===
    "undefined"
    ? null
    : window.location): string | null {
    if (!locationLike)
        return null;
    if (locationLike.pathname.replace(/\/+$/, "") !== "/pair")
        return null;
    const token = new URLSearchParams(locationLike.search).get("token")?.trim();
    return token || null;
}
export function isElizaCloudHostedLocation(locationLike: Pick<Location, "hostname" | "protocol"> | null = typeof window === "undefined" ? null : window.location): boolean {
    if (!locationLike)
        return false;
...
export function resolveNativeCloudPairExchangeUrl(cloudApiBase?: string): string {
    const url = new URL(resolveCloudPairExchangeUrl(cloudApiBase));
    url.pathname = `${url.pathname.replace(/\/+$/, "")}/native`;
    return url.toString();
}
async function readCloudPairResponse(response: Response): Promise<CloudPairRelaySession> {
    ...
    if (!session) {
        throw new CloudPairExchangeError("Cloud did not return an agent session.", 502, "invalid_pairing_response");
    }
    return session;
}
export async function exchangeCloudPairToken(token: string, options: {
    signal?: AbortSignal;
    fetchFn?: typeof fetch;
    cloudApiBase?: string;
} = {}): Promise<CloudPairRelaySession> {
    const fetchFn = options.fetchFn ?? fetch;
    const response = await fetchFn(resolveCloudPairExchangeUrl(options.cloudApiBase), {
        method: "POST",
 ...
 */
export async function exchangeAuthenticatedNativeCloudPairToken(token: string, options: {
    cloudToken: string;
    agentId: string;
    ...
    const fetchFn = options.fetchFn ?? fetch;
    const response = await fetchFn(resolveNativeCloudPairExchangeUrl(options.cloudApiBase), {
        method: "POST",
        headers: {
...
function tryPersistBrowserStorage(storage: Storage | undefined, agentKey: string, apiToken: string): boolean {
    if (!storage)
        return false;
    try {
...
export function installCloudPairApiTokenForSession(apiToken: string): void {
    ...
    if (!token)
        throw new Error("Missing cloud pair API token.");
 ...
 */
export function persistCloudPairApiToken(apiToken: string, agentId: string): void {
    const token = apiToken.trim();
    if (!token)
        throw new Error("Missing cloud pair API token.");
    const owner = agentId.trim();
    if (!owner)
        throw new Error("Missing cloud pair token owner agent id.");
    const agentKey = cloudPairTokenKeyForAgent(owner);
    const persistedInSession = tryPersistBrowserStorage(typeof window === "undefined" ? undefined : window.sessionStorage, agentKey, token);
    const persistedDurably = tryPersistBrowserStorage(typeof window === "undefined" ? undefined : window.localStorage, agentKey, token);
...
export function resolveCloudHostedAgentUrl(locationLike: Pick<Location, "hostname"> | null = typeof window ===
    "undefined"
    ? null
    ...
    if (classified.role === "cloud-app" ||
        classified.role === "legacy-cloud-app") {
        return `${base}/join`;
    }
    const agentId = classified.agentId ?? "";
    const agentPath = agentId &&
        !["www", "app", "app-staging", "api", "api-staging", "staging"].includes(agentId)
...
export type CloudPairExchangeFn = (token: string, options?: {
    signal?: AbortSignal;
}) => Promise<CloudPairRelaySession>;
...
function describePairFailure(error: unknown): Exclude<CloudPairStatus, {
    phase: "pairing";
}> {
...
export function CloudHostedAgentAuthNotice({ onNativeReauth, onNativeRetry, nativeRecoveryMode = "reauth", }: CloudHostedAgentAuthNoticeProps = {}) {
    const reopenUrl = resolveCloudHostedAgentUrl();
    const [activeNativeAction, setActiveNativeAction] = useState<"primary" | "retry" | null>(null);
    const [reauthError, setReauthError] = useState<string | null>(null);
    const handleNativeAction = async (action: (() => Promise<void>) | undefined, actionName: "primary" | "retry") => {
        if (!action || activeNativeAction)
            return;
        ...
        try {
            ...
            setReauthError(error instanceof Error
                ? error.message
    ...
    const ctaClass = "mt-7 inline-flex min-h-11 items-center justify-center rounded-md bg-[#f3a51f] px-5 text-sm font-semibold text-[#101010] transition hover:bg-[#c97710] disabled:cursor-wait disabled:opacity-70";
    return (<main className="flex min-h-[100dvh] flex-col items-center overflow-y-auto bg-[#08090b] px-6 text-center font-body text-white">
      <div className="my-auto w-full max-w-[25rem]">
        ...
        {onNativeReauth ? (<Button variant="default" size="touch" disabled={activeNativeAction !== null} onClick={() => void handleNativeAction(onNativeReauth, "primary")} type="button">
          ...
          </Button>) : (<a className={ctaClass} href={reopenUrl} rel="noopener" target="_top">
            Re-open from Eliza Cloud
          </a>)}
        {nativeRecoveryMode === "manage" && onNativeRetry ? (<Button variant="outlineMuted" size="touch" className="mt-3" disabled={activeNativeAction !== null} onClick={() => void handleNativeAction(onNativeRetry, "retry")} type="button">
            {activeNativeAction === "retry"
                ? "Reconnecting…"
                : "I fixed it — reconnect"}
          </Button>) : null}
        {reauthError ? (<p className="mt-4 text-sm leading-6 text-[#f4b55a]" role="alert">
            {reauthError}
          </p>) : null}
      </div>
    </main>);
...
export function CloudPairRelay({ token, exchangeFn = exchangeCloudPairToken, persistFn = persistCloudPairApiToken, onPaired = redirectToAgentRoot, }: CloudPairRelayProps) {
    ...
    useEffect(() => {
        ...
        exchangeFn(token, { signal: controller.signal })
            ...
            if (originOwner && originOwner !== agentId) {
                throw new CloudPairExchangeError("Cloud returned a session for a different agent.", 502, "pairing_owner_mismatch");
            }
    ...
    <main className="flex min-h-[100dvh] flex-col items-center overflow-y-auto bg-[#08090b] px-6 text-center font-body text-white">
      <div className="my-auto w-full max-w-[24rem]">
        ...
        <p className="mt-3 text-sm leading-6 text-white/60" role={status.phase === "session-only" ? "status" : undefined}>
          {message}
        </p>
        {status.phase === "session-only" ? (<Button size="touch" className="mt-7" onClick={() => onPaired()} type="button">
