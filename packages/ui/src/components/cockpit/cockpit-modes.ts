...
import { type CodingAgentTaskProviderPolicy } from "../../api/client-types-cloud";
...
export type CockpitModeOptionId = "eliza-cloud" | "claude" | "codex" | "claude-experimental" | "codex-experimental";
...
export function visibleCockpitModeOptions(experimentalEnabled: boolean): CockpitModeOption[] {
    return COCKPIT_MODE_OPTIONS.filter((o) => experimentalEnabled || !o.experimental);
}
/** Map a concrete config back to the picker option id it represents. */
export function optionIdForConfig(config: CockpitModeConfig): CockpitModeOptionId {
    switch (config.mode) {
        case "eliza-cloud":
            return "eliza-cloud";
        case "subscription":
...
export function cockpitModeProviderSource(config: CockpitModeConfig): ProviderSource {
    switch (config.mode) {
        case "eliza-cloud":
            return "eliza-cloud";
        case "subscription":
...
export function cockpitModeModel(config: CockpitModeConfig): string | undefined {
 ...
 */
export function cockpitModeToProviderPolicy(config: CockpitModeConfig): CodingAgentTaskProviderPolicy {
 ...
 */
export function normalizeCockpitSpawnTarget(raw: CockpitSpawnTarget): CockpitSpawnTarget | undefined {
