...
import { getElizaCuratedAppCatalogOrder } from "@elizaos/core/contracts/apps";
import { getInternalToolAppCatalogOrder } from "./internal-tool-apps";
import { isElizaCuratedAppName } from "@elizaos/core/contracts/apps";
import { isInternalToolApp } from "./internal-tool-apps";
import { isViewVisible } from "@elizaos/core/views/view-kind";
import { normalizeElizaCuratedAppName } from "@elizaos/core/contracts/apps";
import { packageNameToAppRouteSlug } from "@elizaos/core/contracts/apps";
import { type EnabledViewKinds } from "@elizaos/core";
import { type RegistryAppInfo } from "../../api";
export const DEFAULT_VIEWER_SANDBOX = "allow-scripts allow-same-origin allow-popups";
...
const APPS_VIEW_HIDDEN_APP_NAME_SET = new Set<string>(APPS_VIEW_HIDDEN_APP_NAMES);
/**
...
function isDeclarableCatalogSection(value: string | undefined): value is AppCatalogSectionKey {
...
function getConfiguredDefaultAppNames(): ReadonlySet<string> {
    return new Set((getBootConfig().defaultApps ?? []).filter((name): name is string => typeof name === "string" && name.length > 0));
}
function isFeaturedApp(app: Pick<RegistryAppInfo, "name" | "featured">): boolean {
...
export function isCuratedGameApp(app: Pick<RegistryAppInfo, "category" | "name">): boolean {
    void app.category;
    return isElizaCuratedAppName(app.name);
}
export function shouldShowAppInAppsView(app: Pick<RegistryAppInfo, "category" | "name" | "catalogSection" | "featured" | "defaultHidden" | "scope">, options: {
    isProd?: boolean;
    showAllApps?: boolean;
    walletEnabled?: boolean;
} = {}): boolean {
    const { isProd = typeof import.meta.env.PROD === "boolean"
        ? import.meta.env.PROD
        : Boolean(import.meta.env.PROD), showAllApps, walletEnabled = false, } = options;
    void isProd;
    if (isHiddenFromAppsView(app.name)) {
        return false;
    }
    const configuredDefaultAppNames = getConfiguredDefaultAppNames();
    if (!configuredDefaultAppNames.has(app.name) &&
    ...
    if (sectionKey === "games") {
        return (app.defaultHidden === false ||
            configuredDefaultAppNames.has(app.name) ||
            configuredDefaultAppNames.has(canonicalName));
    }
    return true;
}
export function filterAppsForCatalog(apps: RegistryAppInfo[], { activeAppNames = new Set<string>(), isProd, searchQuery = "", showAllApps, showActiveOnly = false, walletEnabled, enabledKinds = { developer: false, preview: false }, }: AppsCatalogFilterOptions = {}): RegistryAppInfo[] {
    const normalizedSearch = searchQuery.trim().toLowerCase();
    const seenCanonicalNames = new Set<string>();
    const sortedApps = [...apps].sort((left, right) => {
        const toolOrderDiff = getInternalToolAppCatalogOrder(left.name) -
...
export function getDefaultAppsCatalogSelection(apps: RegistryAppInfo[], options: {
    isProd?: boolean;
    showAllApps?: boolean;
    walletEnabled?: boolean;
} = {}): string | null {
...
export function getAppCatalogSectionKey(app: Pick<RegistryAppInfo, "name" | "displayName" | "description" | "category" | "catalogSection" | "featured">): AppCatalogSectionKey {
    ...
    if (/commerce|shop|store|finance|wallet|market|trade|sales|business|team/.test(searchBlob)) {
        return "finance";
    }
    if (/debug|viewer|plugin|skill|memory|trajectory|runtime|database|log|sql/.test(searchBlob)) {
        return "developerUtilities";
    }
    return "other";
}
export function getAppCatalogSectionLabel(app: Pick<RegistryAppInfo, "name" | "displayName" | "description" | "category" | "catalogSection" | "featured">): string {
    return APP_CATALOG_SECTION_LABELS[getAppCatalogSectionKey(app)];
}
export function groupAppsForCatalog(apps: RegistryAppInfo[], { favoriteAppNames = new Set<string>(), }: {
    ...
    const featuredApps = apps.filter((app) => isFeaturedApp(app) && !favoriteAppNames.has(app.name));
...
export function getAppIconName(app: RegistryAppInfo): string {
    ...
    if (sectionKey === "games")
        return "Gamepad2";
    if (sectionKey === "developerUtilities")
        return "Wrench";
    if (sectionKey === "finance")
        return "Wallet";
    return "Package";
}
export function getAppSessionModeLabel(app: Pick<RegistryAppInfo, "session">): string | null {
    const mode = app.session?.mode;
    if (!mode)
        return null;
    return SESSION_MODE_LABELS[mode] ?? mode;
}
export function getAppSessionFeatureLabels(app: Pick<RegistryAppInfo, "session">): string[] {
    return (app.session?.features ?? []).map((feature) => SESSION_FEATURE_LABELS[feature] ?? feature);
}
/* ── App URL slugs ──────────────────────────────────────────────────── */
/**
...
export function findAppBySlug(apps: readonly RegistryAppInfo[], slug: string): RegistryAppInfo | undefined {
