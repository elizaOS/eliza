...
export type { BrowserWorkspaceSolanaMessageSignatureResult, BrowserWorkspaceSolanaTransactionResult, BrowserWorkspaceWalletMessageSignatureResult, BrowserWorkspaceWalletTransactionResult, } from "../../api/client-types-wallet";
...
const SUPPORTED_BROWSER_WORKSPACE_EVM_CHAIN_ID_SET = new Set<number>(SUPPORTED_BROWSER_WORKSPACE_EVM_CHAIN_IDS);
export type BrowserWorkspaceWalletMode = "steward" | "local" | "blocked" | "none";
...
export type BrowserWorkspaceWalletRpcMethod = "eth_accounts" | "eth_requestAccounts" | "eth_chainId" | "eth_sendTransaction" | "personal_sign" | "eth_sign" | "eth_signTypedData" | "eth_signTypedData_v3" | "eth_signTypedData_v4" | "wallet_switchEthereumChain";
export type BrowserWorkspaceSolanaMethod = "solana_connect" | "solana_signMessage" | "solana_signTransaction" | "solana_signAndSendTransaction";
export type BrowserWorkspaceWalletMethod = "getState" | "requestAccounts" | "sendTransaction" | BrowserWorkspaceWalletRpcMethod | BrowserWorkspaceSolanaMethod;
...
export function getBrowserWorkspaceWalletAddress(walletAddresses: WalletAddresses | null, walletConfig: WalletConfigStatus | null, stewardStatus: StewardStatusResponse | null): string | null {
...
export function getBrowserWorkspaceSolanaAddress(walletAddresses: WalletAddresses | null, walletConfig: WalletConfigStatus | null, stewardStatus: StewardStatusResponse | null): string | null {
    return (stewardStatus?.walletAddresses?.solana ??
        walletAddresses?.solanaAddress ??
        walletConfig?.solanaAddress ??
        null);
}
export function resolveBrowserWorkspaceWalletMode(stewardStatus: StewardStatusResponse | null, evmAddress: string | null, solanaAddress: string | null, walletConfig: WalletConfigStatus | null): BrowserWorkspaceWalletMode {
...
export function isBrowserWorkspaceWalletRequest(value: unknown): value is BrowserWorkspaceWalletRequest {
...
export function getUnsupportedBrowserWorkspaceEvmChainError(chainId: number): string {
    return `Unsupported EVM chain ${chainId}. Supported chain IDs: ${SUPPORTED_BROWSER_WORKSPACE_EVM_CHAIN_IDS.join(", ")}.`;
}
export function resolveBrowserWorkspaceSignMessage(params: unknown, address: string | null): string | null {
