...
import { type ConversationMetadata } from "../../api/client-types-chat";
import { type PageScope } from "@elizaos/core/contracts/page-scope";
export { PAGE_SCOPES, type PageScope } from "@elizaos/core/contracts/page-scope";
const PAGE_SCOPE_ROUTING_CONTEXTS: Record<PageScope, {
...
export function isPageScopedConversation(conversation: Pick<Conversation, "metadata"> | null | undefined): boolean {
    const scope = conversation?.metadata?.scope;
    return typeof scope === "string" && scope.startsWith("page-");
}
export function isPageScopedConversationMetadata(metadata: ConversationMetadata | null | undefined): boolean {
    const scope = metadata?.scope;
    return typeof scope === "string" && scope.startsWith("page-");
}
export function buildPageScopedConversationMetadata(scope: PageScope, options: {
 ...
 */
export function buildPageScopedRoutingMetadata(scope: PageScope, options: {
    sourceConversationId?: string;
    pageId?: string;
...
function findPageScopedConversation(conversations: Conversation[], scope: PageScope, pageId?: string): Conversation | null {
    const matching = conversations.filter((conversation) => conversation.metadata?.scope === scope &&
...
function findPageScopedConversations(conversations: Conversation[], scope: PageScope, pageId?: string): Conversation[] {
    return conversations
        .filter((conversation) => conversation.metadata?.scope === scope &&
        (conversation.metadata?.pageId ?? undefined) === (pageId ?? undefined))
        .sort((left, right) => new Date(right.updatedAt).getTime() -
...
export async function resolvePageScopedConversation(params: {
    ...
    if (existing) {
        ...
        const metadataMatches = existing.metadata?.scope === scope &&
...
export async function resetPageScopedConversation(params: {
    ...
    if (matching.length > 0) {
        await Promise.allSettled(matching.map((conversation) => client.deleteConversation(conversation.id)));
    }
    const { conversation } = await client.createConversation(title, {
        metadata: desiredMetadata,
    });
