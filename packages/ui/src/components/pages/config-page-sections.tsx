...
export function CloudRpcStatus({ connected, loginBusy, onLogin, }: CloudRpcStatusProps) {
    const t = useAppSelector((s) => s.t);
    if (connected) {
        return null;
    }
    return (<div className="flex justify-start">
      <Button variant="default" size="compact" onClick={() => void onLogin()} disabled={loginBusy}>
        {loginBusy
...
function buildRpcRendererConfig(t: TranslateFn, selectedProvider: string, providerConfigs: RpcSectionConfigMap, rpcFieldValues: Record<string, string>) {
...
export function RpcConfigSection<T extends string>({ title, description, options, selectedProvider, onSelect, providerConfigs, rpcFieldValues, onRpcFieldChange, cloud, containerClassName, t, }: RpcSectionProps<T>) {
    ...
    return (<div>
      ...
      {renderRpcProviderButtons(options, selectedProvider, onSelect, containerClassName, (key: string) => {
      ...
      <div className="mt-3">
        {selectedProvider === "eliza-cloud" ? (<CloudRpcStatus connected={cloud.connected} loginBusy={cloud.loginBusy} onLogin={() => void cloud.onLogin()}/>) : rpcConfig ? (<ConfigRenderer schema={rpcConfig.schema} hints={rpcConfig.hints} values={rpcConfig.values} setKeys={rpcConfig.setKeys} registry={defaultRegistry} onChange={onRpcFieldChange}/>) : null}
      </div>
    </div>);
}
/* ── renderRpcProviderButtons ──────────────────────────────────────── */
function renderRpcProviderButtons<T extends string>(options: readonly RpcProviderOption<T>[], selectedProvider: T, onSelect: (provider: T) => void, containerClassName: string, tFallback?: (key: string) => string) {
    return (<div className={containerClassName}>
      {options.map((provider) => {
            const active = selectedProvider === provider.id;
            return (<Button variant="choice" size="touch" data-state={active ? "on" : "off"} key={provider.id} onClick={() => onSelect(provider.id)}>
...
function isCloudServiceRouteSelected(route: unknown): boolean {
    ...
    return (routeRecord.transport === "cloud-proxy" &&
...
export function CloudServicesSection() {
    ...
    useEffect(() => {
        ...
        client
            ...
            if (cancelled)
                return;
            const routing = cfg.serviceRouting &&
            ...
            if (cancelled)
                return;
            setActionNotice(t("configpageview.CloudServicesLoadFailed", {
    ...
    const handleToggle = useCallback(async (key: CloudServiceKey, newValue: boolean) => {
        const updated = { ...services, [key]: newValue };
        setServices(updated);
        setSaving(true);
        try {
            const cfg = await client.getConfig();
            const existingRouting = cfg.serviceRouting &&
                typeof cfg.serviceRouting === "object" &&
                !Array.isArray(cfg.serviceRouting)
                ? (cfg.serviceRouting as Record<string, unknown>)
                : {};
            ...
            setActionNotice(t("configpageview.CloudServicesSaveFailed", {
    ...
    if (!loaded)
        return null;
    return (<SettingsStack className="mt-6">
      <SettingsGroup title={t("configpageview.CloudServices", {
            defaultValue: "Cloud Services",
        })} action={needsRestart ? (<span className="text-xs-tight font-medium text-accent">
              {t("configpageview.RestartRequired", {
                defaultValue: "Restart required",
            })}
            </span>) : undefined}>
        {CLOUD_SERVICE_DEFS.map(({ key, labelKey, labelDefault, descriptionKey, descriptionDefault, }) => (<SettingsSwitchRow key={key} agentId={`config-cloud-service-${key}`} group="config-cloud-services" label={t(labelKey, { defaultValue: labelDefault })} description={t(descriptionKey, {
                defaultValue: descriptionDefault,
            })} checked={services[key]} disabled={saving} onCheckedChange={(checked) => void handleToggle(key, checked)}/>))}
      </SettingsGroup>
    </SettingsStack>);
}
