...
export interface LauncherSurfaceProps {
  /** Place the grid inline in another vertical scroller instead of owning a viewport. */
  embedded?: boolean;
}
    ...
    <div
      className={cn(
        embedded
          ...
          : "absolute inset-0 flex min-h-0 flex-col px-0 pb-[calc(var(--eliza-mobile-nav-offset,0px)+max(var(--safe-area-bottom,0px),var(--android-gesture-inset-bottom,0px))+var(--eliza-continuous-chat-clearance,5.25rem)+1.75rem)]",
      )}
      data-testid={embedded ? "home-apps-grid" : undefined}
    >
      <Launcher
        entries={page}
        loading={loading}
