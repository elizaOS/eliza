...
import { type CreateTriggerRequest } from "../../api/client";
...
import { type TriggerWakeMode } from "../../api/client";
import { type UpdateTriggerRequest } from "../../api/client";
...
export function getTemplateName(template: TriggerTemplate, t: (key: string, options?: Record<string, unknown>) => string): string {
    return template.nameKey
        ? t(template.nameKey, { defaultValue: template.name })
        : template.name;
}
export function getTemplateInstructions(template: TriggerTemplate, t: (key: string, options?: Record<string, unknown>) => string): string {
...
export function scheduleLabel(trigger: TriggerSummary, t: TranslateFn, locale?: string): string {
...
export function buildCreateRequest(form: TriggerFormState): CreateTriggerRequest {
...
export function buildUpdateRequest(form: TriggerFormState): UpdateTriggerRequest {
 ...
 */
export function nextRunsForInterval(intervalMs: number, count: number, from = new Date()): Date[] {
 ...
 */
export function nextRunsForCron(expr: string, count: number, from = new Date()): Date[] {
    const trimmed = expr.trim();
    if (!trimmed)
        return [];
    try {
...
export function validateTriggerKind(form: TriggerFormState, t: TranslateFn): string | null {
    if (!form.workflowId) {
        return t("triggers.workflowPlaceholder");
    }
    return null;
}
export function validateForm(form: TriggerFormState, t: TranslateFn): string | null {
    ...
    if (form.triggerType === "once") {
        ...
        if (!raw)
            return t("triggersview.validationScheduledTimeRequired");
    ...
    if (form.triggerType === "cron") {
        ...
        if (!cronTrimmed)
            return t("triggersview.validationCronRequired");
...
export function toneForLastStatus(status?: string): "success" | "warning" | "danger" | "muted" {
    if (!status)
        return "muted";
...
export function localizedExecutionStatus(status: string, t: TranslateFn): string {
    switch (status) {
        case "success":
