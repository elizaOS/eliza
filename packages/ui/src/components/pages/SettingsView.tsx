...
import { backFromConnectorDetail } from "../settings/settings-sections";
import { buildSettingsNavigationGroups } from "../settings/settings-navigation-model";
import { cn } from "../../lib/utils";
import { getAllSettingsSections } from "../settings/settings-sections";
import { getBootConfig } from "../../config/boot-config-store";
import { getFrontendPlatform } from "../../platform/platform-guards";
import { getSettingsSectionRegistryVersion } from "../settings/settings-section-registry";
import { getWindowNavigationPath } from "../../navigation";
import { groupSettingsSections } from "../settings/settings-sections";
...
import { navigateBackToLauncher } from "../shared/ViewHeader";
import { parseSettingsHash } from "../settings/settings-sections";
import { readSettingsHashRoute } from "../settings/settings-sections";
import { readSettingsHashSection } from "../settings/settings-sections";
import { replaceConnectorDetailHash } from "../settings/settings-sections";
import { replaceSettingsHash } from "../settings/settings-sections";
import { replaceSettingsHashRoute } from "../settings/settings-sections";
import { reportUserViewSwitch } from "../../chat/view-navigation-report";
import { resolvePrimingSet } from "../permissions/permission-priming";
import { resolveSettingsExperience } from "../settings/settings-experience";
import { resolveSettingsRuntimeCapabilities } from "../settings/settings-runtime-capabilities";
import { settingsRuntimeHasCapability } from "../settings/settings-runtime-capabilities";
import { settingsSectionIsAvailable } from "../settings/settings-section-registry";
import { settingsSectionLabel } from "../settings/settings-sections";
import { settingsSectionMatchesExperience } from "../settings/settings-experience";
import { settingsSectionTitle } from "../settings/settings-sections";
import { subscribeSettingsSections } from "../settings/settings-section-registry";
import { type GroupedSettingsSections } from "../settings/settings-sections";
import { type PermissionId } from "@elizaos/core/contracts/permissions";
import { type SettingsRoute } from "../settings/settings-sections";
import { type SettingsRuntimeCapabilities } from "../settings/settings-runtime-capabilities";
import { type SettingsSectionDef } from "../settings/settings-sections";
...
function readSettingsPermissionRequest(payload: unknown): PermissionId | null {
    if (!payload || typeof payload !== "object")
        return null;
    const permissionRequest = (payload as {
    ...
    const permission = (permissionRequest as {
        permission?: unknown;
    }).permission;
 ...
 */
function SettingsSectionLoading({ title }: {
    title: string;
}) {
    return (<div aria-busy="true" aria-label={`Loading ${title}`} className="min-h-24 space-y-3 py-1" role="status">
 ...
 */
function SettingsSectionContent({ section, t, anchored = true, compactInset = false, }: {
    ...
    return (<div id={anchored ? section.id : undefined} data-slot="settings-section-content" className={cn(section.bodyClassName, compactInset && "!px-4")}>
      <ErrorBoundary key={section.id} fallback={(error, reset) => (<SettingsSectionFallback title={title} error={error} onRetry={reset} t={t}/>)}>
 ...
 */
function SettingsSectionFallback({ title, error, onRetry, t, }: {
    title: string;
    error: Error;
    onRetry: () => void;
    t: Translate;
}) {
    return (<div role="alert" data-testid="settings-section-error" className="flex flex-col items-start gap-2 rounded-md border border-warn/30 bg-warn/12 p-4 text-left">
      ...
      <Button variant="outlineAccent" size="regularCompact" onClick={onRetry} className="mt-1">
 ...
 */
function SettingsSectionSurfaceAnchor({ section, label, active, onSelect, }: {
    section: SettingsSectionDef;
    label: string;
    ...
    return (<Button ref={ref} type="button" aria-hidden tabIndex={-1} className="hidden" onClick={() => onSelect(section.id)} {...agentProps} 
...
export function SettingsView({ inModal, initialSection, navigatePayload, navigateSequence = 0, runtimeCapabilities: injectedRuntimeCapabilities, }: {
    inModal?: boolean;
    onClose?: () => void;
    ...
    const managedCloudRuntime = isManagedCloudRuntime(runtimeTarget) || cloudOnlyBranding;
    ...
    const isNativeCompactSettings = frontendPlatform === "ios" || frontendPlatform === "android";
    ...
    const runtimeCapabilities = useMemo(() => injectedRuntimeCapabilities ?? resolveSettingsRuntimeCapabilities(), [injectedRuntimeCapabilities]);
    const detachedSettingsShell = !inModal &&
        settingsRuntimeHasCapability(runtimeCapabilities, "detached-settings-shell");
    const registryVersion = useSyncExternalStore(subscribeSettingsSections, getSettingsSectionRegistryVersion, getSettingsSectionRegistryVersion);
    ...
    const allSections = useMemo(() => getAllSettingsSections(), [registryVersion]);
    const availableSections = useMemo(() => allSections.filter((section) => (!section.androidCloudOnly || androidCloudBuild) &&
        settingsSectionIsAvailable(section, runtimeCapabilities)), [allSections, androidCloudBuild, runtimeCapabilities]);
    const availableSectionIds = useMemo(() => new Set(availableSections.map((section) => section.id)), [availableSections]);
    ...
    const [settingsRoute, setSettingsRoute] = useState<SettingsRoute>(() => readSettingsHashRoute());
    const [primePermission, setPrimePermission] = useState<PermissionId | null>(null);
    const visibleSections = useMemo(() => availableSections.filter((section) => {
        if (section.id === "wallet-rpc" && walletEnabled === false)
            return false;
        if (!settingsSectionMatchesExperience(section, settingsExperience)) {
            return false;
        }
        if (!isViewVisible(section, enabledKinds))
            return false;
        if (section.hideOnCloud && androidCloudBuild)
            return false;
        return true;
    }), [
        androidCloudBuild,
    ...
    const grouped: GroupedSettingsSections = groupSettingsSections(visibleSections);
    const navigationGrouped = buildSettingsNavigationGroups(grouped, activeSection);
    useEffect(() => {
        void loadPlugins();
    }, [loadPlugins]);
    ...
    useEffect(() => {
        ...
        const connectorsPath = path.match(/^\/(?:settings\/)?connectors(?:\/([a-z0-9-]+))?\/?$/i);
        if (!connectorsPath)
            return;
    ...
    useEffect(() => {
        ...
        if (!permission) {
            if (navigateSequence > 0)
                setPrimePermission(null);
            return;
        }
    ...
    useEffect(() => {
        const requestedSection = settingsRoute.kind === "hub" ? null : settingsRoute.sectionId;
        if ((!activeSection || availableSectionIds.has(activeSection)) &&
    ...
    const desktopSectionDef = activeSectionDef ?? navigationGrouped[0]?.items[0] ?? null;
    ...
    const connectorDetailId = settingsRoute.kind === "connector-detail"
    ...
    const backLabel = connectorDetailId
        ? "Back to Connectors"
        : "Back to Settings";
    const desktopSidebar = isWideSettings ? (<DesktopSettingsNavigation grouped={navigationGrouped} activeId={desktopSectionDef?.id ?? null} onSelect={openSection} onBack={detachedSettingsShell ? undefined : navigateBackToLauncher} settingsLabel={settingsTitle} label={(labelKey, fallback) => t(labelKey, { defaultValue: fallback })}/>) : null;
    return (<ShellViewAgentSurface viewId="settings">
      {detachedSettingsShell ? (<div aria-hidden="true" className="settings-window-drag-strip" data-window-titlebar="true"/>) : null}
      <ContentLayout inModal={inModal} className={cn("settings-surface settings-canvas", !inModal && !isWideSettings && "min-h-0", detachedSettingsShell && "pt-8")} contentClassName="!overflow-hidden !px-0 !pb-0 !pt-0">
        <div data-testid="settings-shell" data-settings-presentation={isNativeCompactSettings ? "compact-native" : "workspace"} className={cn("flex h-full min-h-0 w-full overflow-hidden", isWideSettings ? "flex-row" : "flex-col")}>
          {desktopSidebar}
          {/* Agent-surface anchors: the agent addresses every section by
            `section-<id>` regardless of which one is shown. */}
          <div className="hidden">
            {visibleSections.map((section) => (<SettingsSectionSurfaceAnchor key={section.id} section={section} label={settingsSectionLabel(section, t)} active={section.id === displayedSectionDef?.id} onSelect={openSection}/>))}
          </div>

          {!isWideSettings ? (<div className={cn(isNativeCompactSettings && "pt-[var(--safe-area-top,0px)]")}>
              {activeSectionDef || connectorDetailId ? (<nav aria-label={settingsTitle} data-testid="view-header" className="flex shrink-0 items-center px-1.5 py-2 sm:px-1.5">
                  <ViewBackButton onBack={onBack} label={backLabel}/>
                </nav>) : null}
            </div>) : null}

          <div data-scroll-cert-scroller={!isWideSettings ? "" : undefined} data-testid="settings-scroll-region" className={cn("min-w-0 flex-1", isWideSettings
            ...
            {isWideSettings ? (<PagePanel.ContentRail data-testid="desktop-settings-work-area" width="compact" className="pb-12 pt-12">
                {desktopSectionDef ? (
            ...
            <section id={desktopSectionDef.id} aria-labelledby={`settings-section-title-${desktopSectionDef.id}`}>
                    <h1 id={`settings-section-title-${desktopSectionDef.id}`} className="sr-only">
                      {settingsSectionTitle(desktopSectionDef, t)}
                    </h1>
                    <SettingsSectionContent section={desktopSectionDef} t={t} anchored={false}/>
                  </section>) : null}
              </PagePanel.ContentRail>) : activeSectionDef ? (<SettingsSectionContent section={activeSectionDef} t={t} compactInset/>) : (
        /* The hub IS the mobile main screen. Tapping a row swaps in
           the section subview; the shared header returns here. */
        <SettingsHubList grouped={navigationGrouped} onSelect={openSection} label={(labelKey, fallback) => t(labelKey, { defaultValue: fallback })}/>)}
          </div>
          {primePermission ? (<PermissionPrimingModal ids={[primePermission]} open onComplete={() => setPrimePermission(null)}/>) : null}
        </div>
      </ContentLayout>
