 ...
 *   2. Secrets (modal)
 */
import { BSC_RPC_OPTIONS } from "./config-page-sections.helpers";
import { Button } from "../ui/button";
import { Check } from "lucide-react";
import { CloudServicesSection } from "./config-page-sections";
import { EVM_RPC_OPTIONS } from "./config-page-sections.helpers";
import { RpcConfigSection } from "./config-page-sections";
import { SOLANA_RPC_OPTIONS } from "./config-page-sections.helpers";
import { ShellViewAgentSurface } from "../views/ShellViewAgentSurface";
import { buildWalletRpcUpdateRequest } from "@elizaos/core/contracts/wallet";
import { claimCloudLoginWindow } from "../../state/cloud-login-launch";
import { navigateBrowserPath } from "../../app-navigate-view";
import { openExternalUrl } from "../../utils";
import { resolveInitialWalletRpcSelections } from "@elizaos/core/contracts/wallet";
import { type RpcProviderOption } from "./config-page-sections";
import { type RpcSectionConfigMap } from "./config-page-sections";
...
function areCloudRpcSelections(selections: WalletRpcSelections) {
    return (selections.evm === "eliza-cloud" &&
        selections.bsc === "eliza-cloud" &&
        selections.solana === "eliza-cloud");
}
function firstCustomRpcProvider<T extends string>(options: readonly RpcProviderOption<T>[], fallback: T): T {
    return options.find((option) => option.id !== "eliza-cloud")?.id ?? fallback;
}
function resolveCustomRpcSelections(current: WalletRpcSelections): WalletRpcSelections {
    return {
        evm: current.evm === "eliza-cloud"
            ? firstCustomRpcProvider(EVM_RPC_OPTIONS, current.evm)
            : current.evm,
        bsc: current.bsc === "eliza-cloud"
            ? firstCustomRpcProvider(BSC_RPC_OPTIONS, current.bsc)
            : current.bsc,
        solana: current.solana === "eliza-cloud"
            ? firstCustomRpcProvider(SOLANA_RPC_OPTIONS, current.solana)
            : current.solana,
    };
}
function CloudLoginFallbackLink({ browserUrl }: {
    browserUrl: string;
}) {
    return (
    ...
    <div className="w-full max-w-sm p-2 text-left">
      ...
      <Button aria-label="Open Eliza Cloud sign-in in your browser" variant="externalLink" className="block w-full whitespace-normal break-all" onClick={() => void openExternalUrl(browserUrl)}>
        {browserUrl}
      </Button>
    </div>);
}
export function ConfigPageView({ embedded = false, onWalletSaveSuccess, }: {
    embedded?: boolean;
    onWalletSaveSuccess?: () => void;
}) {
    const { t, elizaCloudConnected, elizaCloudLoginBusy, elizaCloudLoginFallbackUrl, walletConfig, walletApiKeySaving, handleWalletApiKeySave, handleInteractiveCloudLogin, } = useAppSelectorShallow((s) => ({
        t: s.t,
    ...
    const [rpcMode, setRpcMode] = useState<"cloud" | "custom">(allCloud ? "cloud" : "custom");
    ...
    const [selectedEvmRpc, setSelectedEvmRpc] = useState<WalletRpcSelections["evm"]>(initialSelectedRpc.evm);
    const [selectedBscRpc, setSelectedBscRpc] = useState<WalletRpcSelections["bsc"]>(initialSelectedRpc.bsc);
    const [selectedSolanaRpc, setSelectedSolanaRpc] = useState<WalletRpcSelections["solana"]>(initialSelectedRpc.solana);
    useEffect(() => {
    ...
    const legacyRpcWarning = legacyRpcChains.length > 0
    ...
    const filterCloudOption = <T extends string>(options: readonly RpcProviderOption<T>[]) => options.filter((o) => o.id !== "eliza-cloud");
    ...
    return (<ShellViewAgentSurface viewId="config">
      <div>
        {!embedded && (<>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
          <Button ref={cloudModeEl.ref} variant="choice" size="card" align="start" data-testid="wallet-rpc-mode-cloud" {...cloudModeEl.agentProps} data-state={rpcMode === "cloud" ? "on" : "off"} onClick={() => handleModeChange("cloud")} className="relative">
            <div className="flex items-center gap-2">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={rpcMode === "cloud" ? "text-inherit" : "text-muted"}>
                <title>
            ...
            <span className={`text-xs-tight leading-snug ${rpcMode === "cloud" ? "text-inherit" : "text-muted"}`}>
              {t("configpageview.CloudModeDesc", {
            defaultValue: "Managed RPC for EVM, BSC, and Solana via Eliza Cloud, with Helius on Solana.",
        })}
            </span>
            {rpcMode === "cloud" && (<span className="absolute top-3 right-3 flex size-5 items-center justify-center rounded-full bg-accent text-2xs font-bold text-accent-fg">
                <Check className="size-3" aria-hidden/>
              </span>)}
          </Button>

          <Button ref={customModeEl.ref} variant="choice" size="card" align="start" {...customModeEl.agentProps} data-state={rpcMode === "custom" ? "on" : "off"} onClick={() => handleModeChange("custom")} className="relative">
            <div className="flex items-center gap-2">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={rpcMode === "custom" ? "text-inherit" : "text-muted"}>
                <title>
            ...
            <span className={`text-xs-tight leading-snug ${rpcMode === "custom" ? "text-inherit" : "text-muted"}`}>
              {t("configpageview.CustomModeDesc", {
            defaultValue: "Bring your own API keys. Configure per chain.",
        })}
            </span>
            {rpcMode === "custom" && (<span className="absolute top-3 right-3 flex size-5 items-center justify-center rounded-full bg-accent text-2xs font-bold text-accent-fg">
                <Check className="size-3" aria-hidden/>
              </span>)}
          </Button>
        </div>
        ...
        {rpcMode === "cloud" && (<div>
            {elizaCloudConnected ? (<>
                ...
                ].map((chain) => (<div key={chain.label} className="flex items-center gap-3 px-3 py-2.5">
                ...
                <Button ref={cloudConnectEl.ref} variant="default" size="denseWide" {...cloudConnectEl.agentProps} onClick={() => {
                    claimCloudLoginWindow();
                    void handleInteractiveCloudLogin();
                ...
                {elizaCloudLoginBusy && elizaCloudLoginFallbackUrl ? (<CloudLoginFallbackLink browserUrl={elizaCloudLoginFallbackUrl}/>) : null}
              </div>)}

            <div className="flex justify-end mt-4">
              <Button ref={saveEl.ref} variant="default" size="dense" data-testid="wallet-rpc-save" {...saveEl.agentProps} onClick={() => {
                void handleWalletSaveAll();
        ...
        {rpcMode === "custom" && (<div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Button ref={secretsEl.ref} variant="outlineMuted" size="formAction" {...secretsEl.agentProps} onClick={handleOpenVault}>
            ...
            <div className="space-y-5">
              <RpcConfigSection title={t("configpageview.EVM", { defaultValue: "EVM" })} description={t("configpageview.EVMDesc", {
                defaultValue: "Ethereum, Base, Arbitrum",
            })} options={filterCloudOption(EVM_RPC_OPTIONS)} selectedProvider={selectedEvmRpc === "eliza-cloud"
                ? (EVM_RPC_OPTIONS.find((o) => o.id !== "eliza-cloud")
                    ?.id ?? selectedEvmRpc)
                : selectedEvmRpc} onSelect={(provider) => setSelectedEvmRpc(provider)} providerConfigs={evmRpcConfigs} rpcFieldValues={rpcFieldValues} onRpcFieldChange={handleRpcFieldChange} cloud={cloudStatusProps} containerClassName="flex flex-wrap gap-1.5" t={t}/>
              <div className="py-1"/>
              <RpcConfigSection title={t("configpageview.BSC", { defaultValue: "BSC" })} description={t("configpageview.BSCDesc", {
                defaultValue: "BNB Smart Chain",
            })} options={filterCloudOption(BSC_RPC_OPTIONS)} selectedProvider={selectedBscRpc === "eliza-cloud"
                ? (BSC_RPC_OPTIONS.find((o) => o.id !== "eliza-cloud")
                    ?.id ?? selectedBscRpc)
                : selectedBscRpc} onSelect={(provider) => setSelectedBscRpc(provider)} providerConfigs={bscRpcConfigs} rpcFieldValues={rpcFieldValues} onRpcFieldChange={handleRpcFieldChange} cloud={cloudStatusProps} containerClassName="flex flex-wrap gap-1.5" t={t}/>
              <div className="py-1"/>
              <RpcConfigSection title={t("configpageview.Solana", { defaultValue: "Solana" })} description={t("configpageview.SolanaDesc", {
                defaultValue: "Solana mainnet",
                ...
                : selectedSolanaRpc} onSelect={(provider) => setSelectedSolanaRpc(provider)} providerConfigs={solanaRpcConfigs} rpcFieldValues={rpcFieldValues} onRpcFieldChange={handleRpcFieldChange} cloud={cloudStatusProps} containerClassName="flex flex-wrap gap-1.5" t={t}/>
            </div>

            {legacyRpcWarning && (<div className="mt-4 rounded-sm border border-warn bg-warn-subtle px-3 py-2 text-xs-tight text-txt">
                {legacyRpcWarning}
              </div>)}
