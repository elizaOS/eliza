        ...
        <span className="flex w-full min-w-0 items-center gap-2">
          <span
            className={`inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[11px] font-semibold ${
              isAgent
                ? "border border-border/70 bg-bg-muted text-txt"
                : "bg-card text-muted"
            }`}
          >
            {tab.hasSessionFocus ? (
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="z-[8810] gap-4 rounded-3xl border-border/60 bg-bg shadow-[0_24px_80px_rgba(0,0,0,.48)] max-sm:-translate-y-1/2 max-sm:rounded-3xl"
        style={{
