...
const { handleViewsRoutes, clearCurrentViewState } = await import(join(agentSrc, "api", "views-routes.ts"));
const { registerBuiltinViews } = await import(join(agentSrc, "api", "views-registry.ts"));
const { handleInteractionsRoutes } = await import(join(agentSrc, "api", "interactions-routes.ts"));
const { routeAutonomyTextToUser } = await import(join(agentSrc, "api", "server-helpers-swarm.ts"));
const { registerProactiveInteractionDecider, PROACTIVE_INTERACTION_SOURCE } = await import(join(agentSrc, "services", "proactive-interaction-decider.ts"));
const { ProactiveInteractionGate } = await import(join(agentSrc, "services", "proactive-interaction-gate.ts"));
...
registerProactiveInteractionDecider(runtime, {
    gate,
    route: (text) => routeAutonomyTextToUser(state, text, PROACTIVE_INTERACTION_SOURCE),
    now: () => vnow,
});
const noopRes = {};
const jsonNoop = () => { };
...
const proactiveFrames = () => frames.filter((f) => f.type === "proactive-message");
...
assert(proactiveFrames().length === 1 &&
    proactiveFrames()[0].message.source === PROACTIVE_INTERACTION_SOURCE &&
    proactiveFrames()[0].message.text ===
        "Want me to pull your latest balances?", "s1: wallet switch produced a governed proactive-message frame");
assert(createdMemories.length === 1, "s1: the admitted comment was persisted to the conversation");
assert(voiceCalls === 1, "s1: the admitted comment passed through the humanness voice gate");
...
await flushDebounce();
assert(judgeCalls === 1, "s2: burst switch was suppressed pre-judge (no model call spent)");
assert(proactiveFrames().length === 1, "s2: burst switch was rate-limited (no second frame)");
assert(gateDecisions.at(-1)?.admitted === false &&
    gateDecisions.at(-1)?.reason === "global cooldown", `s2: gate precheck recorded the suppression reason (${gateDecisions.at(-1)?.reason})`);
// s4 — control shortcut: denied before the judge (gesture, not intent).
await postShortcut("focus-composer");
await sleep(50);
assert(judgeCalls === 1 && proactiveFrames().length === 1, "s4: control shortcut (focus-composer) never reached the judge");
// s5 — agent-initiated navigation: already acknowledged, no comment.
await postNavigate("settings", { source: "agent" });
await flushDebounce();
assert(judgeCalls === 1 && proactiveFrames().length === 1, "s5: agent-initiated switch produced no proactive comment");
assert(navigationFrames.length === 1 &&
    navigationFrames[0].viewId === "settings", "s5: agent-initiated navigation reached its originating view client");
...
assert(proactiveFrames().length === 2 &&
    proactiveFrames()[1].message.text === "Want a hand finding something?", "s6: clock-advanced shortcut was admitted (second frame)");
assert(voiceCalls === 2, "s6: the second admitted comment also passed through the voice gate");
...
assert(proactiveFrames().length === 2 &&
    ...
    gateDecisions.at(-1)?.reason === "per-surface cooldown", `s7: per-surface cooldown suppressed the repeat (${gateDecisions.at(-1)?.reason})`);
...
await postNavigate("calendar", { source: "user" });
await flushDebounce();
assert(judgeCalls === 2 && proactiveFrames().length === 2, "s8: chattiness=off suppressed the interaction before the judge");
chattiness = "subtle";
...
assert(proactiveFrames().length === 2 &&
    gateDecisions.at(-1)?.admitted === false &&
    gateDecisions.at(-1)?.reason === "duplicate of a recent comment", `s9: textual dedup suppressed the identical offer (${gateDecisions.at(-1)?.reason})`);
assert(errors400.length === 0, `phase 1: no route rejected a valid report (${JSON.stringify(errors400)})`);
console.log("  gate ledger:");
for (const d of gateDecisions) {
    console.log(`    t+${String(d.atMs / 1000).padStart(5)}s ${d.admitted ? "ADMIT   " : "SUPPRESS"} [${d.surface}] ${d.reason}`);
}
await writeFile(join(outDir, "phase1-pipeline.json"), `${JSON.stringify({ frames, gateDecisions, judgeCalls, memories: createdMemories.length }, null, 2)}\n`);
...
// The fixture's server-only edges (the @elizaos/core barrel and the
...
try {
    ...
    assert(await p
        ...
        .isVisible(), "the bubble shows the governed comment text");
    assert(await p.getByText("Suggestion", { exact: true }).isVisible(), 'the bubble carries the "Suggestion" chip (not a normal reply)');
    assert((await p.getByRole("button", { name: "Do it" }).count()) === 1 &&
        (await p.getByRole("button", { name: "Dismiss suggestion" }).count()) ===
            1, 'the bubble exposes the "Do it" and dismiss affordances');
    await snap(p, "suggestion-rendered");
    // 2. Malformed frame (no message id) → real parser rejects, UI unchanged.
    const rBad = await p.evaluate((f) => window.__deliverWsFrame?.(f), {
        type: "proactive-message",
        conversationId: CONVERSATION_ID,
        message: { role: "assistant", text: "broken", timestamp: 1 },
    });
    assert(rBad?.delivered === false && (await bubbleCount()) === 1, `malformed frame is rejected by the real parser (${rBad?.reason})`);
    // 3. Frame for another conversation → unread marker, not a bubble.
    const rOther = await p.evaluate((f) => window.__deliverWsFrame?.(f), {
        ...captured[0],
        conversationId: "conv-2",
        message: { ...captured[0].message, id: randomUUID() },
    });
    const unread = await p.evaluate(() => window.__unreadConversations);
    assert(rOther?.delivered === false &&
        (await bubbleCount()) === 1 &&
        unread?.includes("conv-2"), "a frame for an inactive conversation goes to unread, not the transcript");
    ...
    assert(rDup?.reason === "id-deduped" && (await bubbleCount()) === 2, "replaying an already-delivered frame does not duplicate the bubble");
    ...
    assert((await p.evaluate(() => window.__sentTexts))?.length === 0, "dismiss sends nothing to the agent");
    ...
    assert(Array.isArray(sent) && sent.length === 1 && sent[0] === "Yes, let's do it.", '"Do it" sends the acceptance as a normal chat turn');
