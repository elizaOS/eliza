...
await cp(join(here, "../../views/view-icons/ionicons"), join(outDir, "view-icons/ionicons"), { recursive: true, force: true });
let failures = 0;
...
// @elizaos/core barrel, which transitively reaches @elizaos/core / node
...
const stubNodeBuiltins = {
    ...
    setup(b) {
        ...
        b.onLoad({ filter: /.*/, namespace: "node-stub" }, () => ({
            contents: "const n=()=>noop;const noop=new Proxy(n,{get:()=>noop});module.exports=noop;",
            loader: "js",
        }));
    },
};
...
const focusPolicyStart = productCss.indexOf("/* Product policy: focus rings are intentionally disabled globally. */");
const focusPolicyEndMarker = "/* biome-ignore-end lint/complexity/noImportantStyles: end focus-ring ban override */";
const focusPolicyEnd = productCss.indexOf(focusPolicyEndMarker, focusPolicyStart);
if (focusPolicyStart < 0 || focusPolicyEnd < 0) {
    throw new Error("could not find the product focus-policy block in styles.css");
}
const focusPolicyCss = productCss.slice(focusPolicyStart, focusPolicyEnd + focusPolicyEndMarker.length);
...
const readTelemetry = (p) => p.evaluate(() => window.__ELIZA_VIEW_INTERACTION_TELEMETRY__ ?? []);
...
function surfaceChanged(before, after) {
    return SURFACE_PROPERTIES.some((property) => before[property] !== after[property]);
}
function geometryStayedPut(before, after, tolerance = 0.5) {
    return GEOMETRY_PROPERTIES.every((property) => Math.abs(before[property] - after[property]) <= tolerance);
}
function hasNeutralMotion(state) {
    return Object.values(state.parts).every((part) => part.transform === "none" && part.animationName === "none");
}
...
function tileGeometryStayedPut(before, after) {
    return Object.keys(before.parts).every((part) => geometryStayedPut(before.parts[part].rect, after.parts[part].rect));
}
const errors = [];
...
async function captureViewport(name, viewport, deviceScaleFactor, { theme = "dark", interactions = false } = {}) {
    ...
    const renderedColorScheme = await page.evaluate(() => getComputedStyle(document.documentElement).colorScheme);
    assert(renderedColorScheme === theme, `${name}: real theme CSS renders in ${theme} mode (color-scheme=${renderedColorScheme})`);
    ...
    assert(visuals === tiles, `${name}: every launcher tile has a glyph visual (${visuals}/${tiles})`);
    const glyphImages = await page
        .locator("[data-view-visual] img[data-launcher-glyph]")
        .count();
    assert(glyphImages === tiles, `${name}: every launcher visual uses one glyph image (${glyphImages}/${tiles})`);
    const ionicons = await page
        .locator('[data-launcher-glyph-kind="ionicon"][data-ionicon]:not([data-ionicon=""])')
        .count();
    const ioniconsLoaded = await page
        .locator('[data-launcher-glyph-kind="ionicon"]')
        .evaluateAll((images) => images.every((image) => image instanceof HTMLImageElement &&
        image.complete &&
        image.naturalWidth > 0));
    const customImages = await page
        .locator('[data-launcher-glyph-kind="image"]:not([data-ionicon]):not([data-testid^="launcher-image-"])')
        .count();
    ...
    // compatible.
    assert(ionicons + customImages === glyphImages, `${name}: glyph images are Ionicon or third-party custom assets (ionicon=${ionicons}, custom=${customImages})`);
    const heroImages = await page
        .locator('[data-testid^="launcher-image-"]')
        .count();
    assert(heroImages === 0, `${name}: no launcher hero image nodes render (${heroImages})`);
    const uncontractedImages = await page
        .locator("[data-view-visual] img:not([data-launcher-glyph])")
        .count();
    assert(uncontractedImages === 0, `${name}: no image bypasses the launcher glyph contract (${uncontractedImages})`);
    for (const entryWithHero of ["wallet", "companion"]) {
        const deterministicGlyph = await page
            ...
            .count();
        assert(deterministicGlyph === 1, `${name}: ${entryWithHero} ignores its hero URL and keeps one deterministic glyph`);
    }
    // Read-only: no per-tile edit/pin/delete affordances anywhere.
    const editAffordances = await page
        .locator('[data-testid^="launcher-fav-"], [data-testid^="launcher-edit-"], [data-testid^="launcher-delete-"]')
        .count();
    assert(editAffordances === 0, `${name}: read-only launcher renders no edit/pin/delete affordances (${editAffordances})`);
    ...
    assert(pageWindow === 1 && legacyPage1 === 0, `${name}: single launcher page window (window=${pageWindow}, legacy-page-1=${legacyPage1})`);
    ...
    if (interactions) {
        ...
        assert(surfaceChanged(restState.surface, hoverState.surface), `${name}: hover visibly changes the icon surface`);
        assert(tileGeometryStayedPut(restState, hoverState), `${name}: hover feedback does not move or resize the tile`);
        ...
        const focusedTestId = await page.evaluate(() => document.activeElement
            ?.closest("[data-testid]")
            ?.getAttribute("data-testid") ?? null);
        assert(focusedTestId === "launcher-tile-chat" && focusState.focusVisible, `${name}: Tab puts visible keyboard focus on the first launcher tile`);
        assert(surfaceChanged(unfocusedState.surface, focusState.surface), `${name}: keyboard focus visibly changes the icon surface`);
        assert(tileGeometryStayedPut(unfocusedState, focusState), `${name}: keyboard focus feedback does not move or resize the tile`);
        await snap(page, `${name}-keyboard-focus`);
    }
    await page.close();
}
try {
    await captureViewport("desktop-dark", { width: 1180, height: 900 }, undefined, { theme: "dark", interactions: true });
    await captureViewport("desktop-light", { width: 1180, height: 900 }, undefined, { theme: "light" });
    await captureViewport("mobile-dark", { width: 390, height: 844 }, 2, { theme: "dark" });
}
catch (err) {
...
assert(Array.isArray(callsAfterLaunch.launch) &&
    callsAfterLaunch.launch.includes(launchTarget), `tap launches the tile (onLaunch fired with "${launchTarget}")`);
const launchInRing = (await readTelemetry(page)).some((e) => e.action === "launch");
...
//    or move. The held release must also be consumed rather than ghost-launch.
{
    ...
    assert(hasNeutralMotion(beforePress) &&
        hasNeutralMotion(afterPointerDown) &&
        hasNeutralMotion(whileHeld) &&
        afterPointerDown.activeAnimations === 0 &&
        whileHeld.activeAnimations === 0, "pointer-down and long-press add no animation or transform to the tile");
    assert(tileGeometryStayedPut(beforePress, afterPointerDown) &&
        tileGeometryStayedPut(beforePress, whileHeld), "pointer-down and long-press do not move or resize the button, plate, or glyph");
    assert(!surfaceChanged(beforePress.surface, afterPointerDown.surface) &&
        !surfaceChanged(beforePress.surface, whileHeld.surface), "pointer-down and long-press keep launcher-owned paint and inset depth stable");
    const affordances = await page
        .locator('[data-testid^="launcher-fav-"], [data-testid^="launcher-edit-"], [data-testid^="launcher-delete-"], button.animate-pulse')
        .count();
    assert(affordances === 0, `a long-press never enters edit mode (read-only launcher; ${affordances} affordances)`);
    const callsAfterHold = await readCalls(page);
    assert(!Array.isArray(callsAfterHold.launch) ||
        !callsAfterHold.launch.includes("wallet"), "a long-press release does not ghost-launch the held tile");
}
...
//    inner launcher pager owns no horizontal gesture.
{
    ...
    assert(midDrag !== null && Math.abs(midDrag - expectedMid) <= 2, `mid-drag rail tracks the pointer 1:1 (m41=${midDrag}, expected ≈${expectedMid})`);
...
assert(vids.length === 1, `walkthrough emits exactly one video artifact (${vids.length})`);
if (vids.length === 1) {
