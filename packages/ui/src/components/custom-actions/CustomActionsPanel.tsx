...
function handlerTypeLabel(type: string, t: (key: string, options?: Record<string, string | number>) => string): string {
    switch (type) {
        case "http":
...
export function CustomActionsPanel({ open, onClose, onOpenEditor, }: CustomActionsPanelProps) {
    ...
    const loadActions = useCallback(async () => {
        try {
            ...
            if (!mountedRef.current)
                return;
            setError(t("customactionspanel.LoadFailed", {
    ...
    const filteredActions = useMemo(() => {
        ...
        return actions.filter((action) => {
            ...
            const hasDescription = typeof action.description === "string" &&
                action.description.toLowerCase().includes(searchTerm);
            const hasAlias = (action.similes ?? []).some((alias) => alias.toLowerCase().includes(searchTerm));
    ...
    const formatParameterCount = useCallback((count: number) => {
        return count === 1
    ...
    const formatAliasCount = useCallback((count: number) => {
        return count === 1
    ...
    const handleToggleEnabled = async (action: CustomActionDef) => {
        try {
            ...
            setActions((prev) => prev.map((item) => item.id === action.id
                ? {
                    ...item,
                    enabled: next,
                }
            ...
            setError(t("customactionspanel.UpdateFailed", {
    ...
    const handleDelete = async (action: CustomActionDef) => {
        ...
        try {
            ...
            setError(t("customactionspanel.DeleteFailed", {
    ...
    return (<div className={`bg-card flex flex-col transition-all duration-200 ${open ? "w-80" : "w-0 overflow-hidden"}`}>
      {open && (<>
          {/* Header */}
          <div className="flex items-start justify-between p-4">
            ...
            <Button variant="ghostMuted" size="icon-sm" onClick={onClose} aria-label={t("aria.closePanel")}>
          ...
          <div className="space-y-3 p-3">
            <Button variant="default" size="regularCompact" onClick={handleCreate} className="w-full">
              {t("customactionspanel.NewCustomAction")}
            </Button>

            <div className="relative">
              <Input variant="surface" density="denseResponsive" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("customactionspanel.SearchByNameDesc")} className="w-full"/>
            </div>

            {error && (<div className="text-xs text-danger bg-danger/10 border border-danger/30 px-2 py-1 rounded-sm">
                {error}
              </div>)}
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {loading ? (<div className="text-center text-muted text-xs py-8">
                {t("customactionspanel.LoadingYourActions")}
              </div>) : filteredActions.length === 0 ? (<div className="text-center text-muted text-xs py-8">
                {search
                    ? t("customactionspanel.NoActionsMatchSearch", {
                        defaultValue: "Nothing matches that search.",
                    })
                    : t("customactionspanel.NoActionsYet", {
                        defaultValue: "No custom actions yet. Make one to get started.",
                    })}
              </div>) : (filteredActions.map((action) => (<div key={action.id} className="border border-border bg-surface rounded-sm p-2 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className={`text-2xs px-1.5 py-0.5 rounded-sm whitespace-nowrap ${HANDLER_TYPE_COLORS[action.handler.type] ||
                    "bg-surface text-muted"}`}>
                      {handlerTypeLabel(action.handler.type, t)}
                    </span>
                  </div>

                  {action.description && (<p className="text-xs text-muted line-clamp-2 break-words">
                      {action.description}
                    </p>)}
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    <label className="flex items-center gap-1 cursor-pointer text-xs text-muted">
                      <Switch checked={action.enabled} onCheckedChange={() => handleToggleEnabled(action)} className="scale-75"/>
                      <span>{t("common.enabled")}</span>
                    </label>

                    <div className="ml-auto flex items-center gap-2">
                      <Button variant="ghostMuted" size="micro" onClick={() => handleEdit(action)} title={t("customactionspanel.EditAction")}>
                        {t("common.edit")}
                      </Button>
                      <Button variant="dangerGhost" size="micro" onClick={() => handleDelete(action)} title={t("customactionspanel.DeleteAction")}>
                        {t("common.delete")}
                      </Button>
                    </div>
                  </div>
