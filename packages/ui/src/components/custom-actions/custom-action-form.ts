...
/* ── CSS class names ───────────────────────────────────────────────── */
export const editorDialogContentClassName = "w-[min(calc(100%_-_2rem),48rem)] max-h-[min(90vh,56rem)] overflow-hidden rounded-sm border border-border/70 bg-card/96 p-0";
export const editorFieldLabelClassName = "text-xs text-muted";
export const editorInputClassName = "rounded-sm border-border bg-surface text-txt placeholder:text-muted/50 ";
...
export const editorSectionCardClassName = "flex flex-col gap-3 rounded-sm border border-border/70 bg-bg/20 p-3";
...
export function parseGeneratedAction(payload: unknown): {
    ...
    const hTypeRaw = toNonEmptyString((raw as {
        handlerType?: unknown;
    }).handlerType) ??
        toNonEmptyString((handlerSource as {
            type?: unknown;
        }).type);
