...
import { HTTP_METHODS } from "./custom-action-form";
...
import { editorFieldLabelClassName } from "./custom-action-form";
import { normalizeActionName } from "./custom-action-form";
import { normalizeMethod } from "./custom-action-form";
import { normalizeParamName } from "./custom-action-form";
import { parseGeneratedAction } from "./custom-action-form";
import { parseSimilesInput } from "./custom-action-form";
...
import { type HandlerType } from "./custom-action-form";
import { type HeaderRow } from "./custom-action-form";
import { type HttpMethod } from "./custom-action-form";
import { type ParamDef } from "./custom-action-form";
import { type ParsedGeneration } from "./custom-action-form";
import { useAppSelector } from "../../state";
import { useEffect } from "react";
import { useState } from "react";
import { validateParameters } from "./custom-action-form";
...
export function CustomActionEditor({ open, action, onSave, onClose, }: CustomActionEditorProps) {
    ...
    const applyGenerated = (parsed: ParsedGeneration) => {
        ...
        if (parsed.handlerType === "http") {
            ...
            setHttpUrl(handler.url);
            setHttpBody(handler.bodyTemplate || "");
            setHttpHeaders(handler.headers
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (!parsed.ok || !parsed.action) {
                setFormError(parsed.errors.length > 0
                    ? parsed.errors.join(" ")
    ...
    const updateParameter = (index: number, field: keyof ParamDef, value: string | boolean) => {
        setParameters((prevParameters) => prevParameters.map((parameter, i) => {
            if (i !== index) {
                return parameter;
            }
            if (field === "name") {
    ...
    const updateHeader = (index: number, field: "key" | "value", value: string) => {
        setHttpHeaders((prevHeaders) => prevHeaders.map((header, i) => i === index ? { ...header, [field]: value } : header));
        setFormError("");
    };
    ...
    const handleTest = async () => {
        ...
        if (!actionId) {
            try {
                ...
                if (!actionId) {
                    setTestResult({
                        error: "Failed to save action before testing. Please save manually first.",
                    });
                    setTesting(false);
                    return;
                }
    ...
    if (!open)
        return null;
    return (<Dialog open={open} onOpenChange={(nextOpen: boolean) => {
            if (!nextOpen)
                onClose();
        }}>
      <DialogContent showCloseButton={false} className="w-[min(calc(100%_-_2rem),48rem)] max-h-[min(90vh,56rem)] p-0">
        {/* Header */}
        <DialogHeader className="flex flex-row items-center px-5 py-4">
          ...
          <Button variant="ghostMuted" size="icon-sm" onClick={onClose} aria-label={t("common.close")}>
            <X aria-hidden="true"/>
          </Button>
        </DialogHeader>
        ...
        <div className="flex max-h-[min(72vh,44rem)] flex-col gap-4 overflow-y-auto px-5 py-4">
          ...
          {!action && (<Card variant="panel">
            ...
            }} placeholder={t("customactioneditor.eGCheckIfAWebs")} variant="surface" className="flex-1"/>
                  <Button variant="default" size="sm" className="whitespace-nowrap" onClick={handleGenerate} disabled={generating || !aiPrompt.trim()}>
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input type="text" value={name} onChange={(e) => setNormalizedName(e.target.value)} placeholder={t("customactioneditor.MYACTION")} variant="surface" className="flex-1"/>
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <Textarea value={description} onChange={(e) => setDescriptionValue(e.target.value)} placeholder={t("customactioneditor.WhatDoesThisActio")} rows={2} variant="modal" className="flex-1 resize-none"/>
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input type="text" value={similesInput} onChange={(e) => {
            setSimilesInput(e.target.value);
            setFormError("");
        }} placeholder={t("customactioneditor.SYNONYMONESYNONYM")} variant="surface" className="flex-1"/>
            <span className="text-xs text-muted">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex gap-2">
              {(["http", "shell", "code"] as const).map((type) => (<Button variant="choice" size="compact" data-state={handlerType === type ? "on" : "off"} key={type} onClick={() => {
                setHandlerType(type);
                setFormError("");
            }}>
                  {type === "http"
          ...
          {/* Handler Config */}
          {handlerType === "http" && (<Card variant="panel">
              <div className="flex flex-col gap-3 p-3">
                <div className="flex gap-2">
                  <Select value={httpMethod} onValueChange={(value: string) => setHttpMethod(value as HttpMethod)}>
                    <SelectTrigger aria-label="HTTP method" variant="modal" className="w-auto min-w-[6.5rem]">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {HTTP_METHODS.map((method) => (<SelectItem key={method} value={method}>
                          {method}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
                  <Input type="text" value={httpUrl} onChange={(e) => {
                setHttpUrl(e.target.value);
                setFormError("");
            }} placeholder={t("customactioneditor.httpsApiExample")} variant="surface" className="flex-1"/>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="mutedLink" size="content" onClick={addHeader}>
                      {t("customactioneditor.Add")}
                    </Button>
                  </div>
                  {httpHeaders.map((header, i) => (<div key={`${header.key}:${header.value}`} className="flex gap-2">
                      <Input type="text" value={header.key} onChange={(e) => updateHeader(i, "key", e.target.value)} placeholder={t("customactioneditor.HeaderName")} variant="surface" className="flex-1"/>
                      <Input type="text" value={header.value} onChange={(e) => updateHeader(i, "value", e.target.value)} placeholder={t("customactioneditor.valueOrParam")} variant="surface" className="flex-1"/>
                      <Button variant="ghostMuted" size="inlineIcon" onClick={() => removeHeader(i)} aria-label={`Remove header ${i + 1}`}>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Textarea value={httpBody} onChange={(e) => {
                setHttpBody(e.target.value);
                setFormError("");
            }} placeholder={'{"key": "{{param}}"}'} rows={3} variant="modal" className="resize-none font-mono"/>
                </div>
              </div>
            </Card>)}

          {handlerType === "shell" && (<div className="flex flex-col gap-1">
              <span className={editorFieldLabelClassName}>
                {t("customactioneditor.CommandTemplate")}
              </span>
              <Textarea value={shellCommand} onChange={(e) => {
                setShellCommand(e.target.value);
                setFormError("");
            }} placeholder={t("customactioneditor.echoMessage")} rows={4} variant="modal" className="resize-none font-mono"/>
              <span className="text-xs text-muted">
          ...
          {handlerType === "code" && (<div className="flex flex-col gap-1">
              <span className={editorFieldLabelClassName}>
              ...
              <Textarea value={code} onChange={(e) => {
                ...
                setFormError("");
            }} placeholder={t("customactioneditor.AvailableParams")} rows={6} variant="modal" className="resize-none font-mono"/>
            </div>)}
          ...
          <div className="flex flex-col gap-2 pt-3">
            ...
            {parameters.map((param, paramIdx) => (<div key={`${param.name}-${param.description ?? ""}`} className="flex gap-2 items-start">
                <Input type="text" value={param.name} onChange={(e) => updateParameter(paramIdx, "name", e.target.value)} placeholder={t("customactioneditor.paramName")} variant="surface" className="w-32"/>
                <Input type="text" value={param.description} onChange={(e) => updateParameter(paramIdx, "description", e.target.value)} placeholder={t("common.description")} variant="surface" className="flex-1"/>
                <span className="flex items-center gap-1 text-xs text-muted cursor-pointer">
                  <Checkbox checked={param.required} aria-label={t("secretsview.Required")} onCheckedChange={(checked: boolean | "indeterminate") => updateParameter(paramIdx, "required", !!checked)}/>

                  {t("secretsview.Required")}
                </span>
                <Button variant="ghostMuted" size="inlineIcon" onClick={() => removeParameter(paramIdx)} aria-label={`Remove parameter ${param.name || paramIdx + 1}`}>
                  {t("bugreportmodal.Times")}
                </Button>
              </div>))}
          </div>
          ...
          {/* Test Section */}
          <div className="flex flex-col gap-2 pt-3">
            <Button variant="disclosureMuted" size="content" onClick={() => setTestExpanded((expanded) => !expanded)}>
              <span>{t("customactioneditor.TestAction")}</span>
              <span>
                {testExpanded ? (<ChevronDown className="size-3"/>) : (<ChevronRight className="size-3"/>)}
              </span>
            </Button>
            {testExpanded && (<div className="flex gap-2">
                <Separator orientation="vertical" className="h-auto self-stretch"/>
                <div className="flex min-w-0 flex-1 flex-col gap-2">
                  {parameters
                .filter((p) => p.name.trim())
                .map((param) => (<div key={param.name} className="flex flex-col gap-1">
                        <span className={editorFieldLabelClassName}>
                          {param.name}
                        </span>
                        <Input type="text" value={testParams[param.name] || ""} onChange={(e) => setTestParams({
                    ...testParams,
                    [param.name]: e.target.value,
                })} placeholder={param.description || "value"} variant="surface"/>
                      </div>))}
                  {testResult && (<Card variant="insetCompact">
                      <div className="font-mono text-xs">
                        {testResult.error && (<div className="text-status-danger">
                            {t("customactioneditor.Error")} {testResult.error}
                          </div>)}
                        {testResult.output && (<pre className="text-txt whitespace-pre-wrap">
                            {testResult.output}
                          </pre>)}
                        {testResult.duration !== undefined && (<div className="text-muted mt-1">
        ...
        <DialogFooter className="px-5 py-4 sm:justify-end sm:space-x-2">
          {testExpanded && (<Button variant="outline" size="sm" onClick={handleTest} disabled={testing || saving}>
              {testing
                ? t("common.testing", {
