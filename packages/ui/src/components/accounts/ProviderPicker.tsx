...
import { type AccountProviderCategory } from "./account-provider-options";
...
export function capabilityLine(option: AccountProviderOption): string {
    ...
    const authentication = descriptor.authMode === "oauth"
        ? "browser login"
...
export function ProviderPicker({ onPick }: ProviderPickerProps) {
    ...
    return (<div className="grid gap-2">
      <div className="relative">
        ...
        <Input ref={inputRef} adornment="leading" density="compact" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={onKeyDown} placeholder={t("accounts.add.search", {
      ...
      <div ref={listRef} className="max-h-[min(52vh,420px)] overflow-y-auto" role="listbox" aria-label={t("accounts.add.chooseTitle", {
            defaultValue: "Add a provider account",
        })}>
        {flat.length === 0 ? (<p className="px-1 py-6 text-center text-xs text-muted">
            {t("accounts.add.noMatch", {
                defaultValue: "No providers match your search.",
            })}
          </p>) : (flat.map((option, index) => {
            const showHeader = option.category !== lastCategory;
            lastCategory = option.category;
            const active = index === activeIndex;
            return (<div key={option.id}>
                {showHeader ? (<div className="px-1 pb-1 pt-2 text-2xs font-semibold uppercase tracking-[0.16em] text-muted first:pt-0">
                    {CATEGORY_LABEL[option.category]}
                  </div>) : null}
                <Button variant="selection" size="row" align="start" data-state={active ? "on" : "off"} type="button" data-index={index} role="option" aria-selected={active} onMouseMove={() => setActiveIndex(index)} onClick={() => onPick(option.id)}>
                  <Badge asChild variant={active ? "providerMarkActive" : "providerMark"} size="providerMark">
                    <span>
                  ...
                  {active ? (<Badge asChild variant="keyHint">
