...
export function readSubscriptionOAuth(providerId: LinkedAccountProviderId): PersistedSubscriptionOAuth | null {
    ...
    try {
        ...
        const valid = value.providerId === providerId &&
...
export function writeSubscriptionOAuth(value: PersistedSubscriptionOAuth): void {
    if (typeof window === "undefined")
        return;
    try {
...
export function clearSubscriptionOAuth(providerId: LinkedAccountProviderId): void {
    if (typeof window === "undefined")
        return;
    try {
        shellLocalStorage.removeItem(key(providerId));
