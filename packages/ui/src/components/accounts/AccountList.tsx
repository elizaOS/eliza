...
export function AccountList({ providerId }: AccountListProps) {
    ...
    const [addDialogOpen, setAddDialogOpen] = useState(() => readSubscriptionOAuth(providerId) !== null);
    ...
    useEffect(() => {
        const restorePendingDialog = () => {
            if (readSubscriptionOAuth(providerId))
    ...
    const sorted: AccountWithCredentialFlag[] = useMemo(() => providerEntry
        ? [...providerEntry.accounts].sort((a, b) => (Number.isFinite(a.priority) ? a.priority : 0) -
            (Number.isFinite(b.priority) ? b.priority : 0))
        : [], [providerEntry]);
    const handleMove = useCallback(async (accountId: string, direction: "up" | "down") => {
    ...
    const listState = accounts.loading && !accounts.data
        ? {
        ...
        : accounts.error && !accounts.data
            ? {
                ...
                action: (<Button type="button" variant="outline" size="sm" onClick={() => void accounts.refresh()} className="shrink-0">
    ...
    return (<>
      ...
      <AddAccountDialog open={addDialogOpen} providerId={providerId} credentialRepairAccount={credentialRepairAccount} onClose={() => {
