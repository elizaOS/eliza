...
const STRATEGY_OPTIONS: readonly StrategyOption[] = [
    ...
    },
    {
        ...
        descriptionFallback: "Spend the account whose weekly limit resets first; hold freshly-reset accounts in reserve.",
    },
    {
        ...
        descriptionFallback: "Prefer hand-set priority, then the account whose relevant weekly limit resets soonest.",
    },
];
export function RotationStrategyPicker({ providerId, value, onChange, disabled, }: RotationStrategyPickerProps) {
    const t = useAppSelector((s) => s.t);
    const resolved: AccountStrategy = value ??
    ...
    return (<div className="flex items-center gap-2">
      ...
      <Select value={resolved} onValueChange={(next) => {
            if (next !== resolved)
                onChange(next as AccountStrategy);
        }} disabled={disabled}>
        <SelectTrigger variant="accountCompact" aria-label={t("accounts.strategy.label", {
            defaultValue: "Strategy",
        })} id={`rotation-strategy-${providerId}`} className="w-[160px] gap-1 truncate whitespace-nowrap [&>span]:truncate">
          <SelectValue placeholder={t("accounts.strategy.choose", {
            defaultValue: "Choose strategy",
        })}/>
        </SelectTrigger>
        <SelectContent position="popper" sideOffset={4} align="end" collisionPadding={16} className="w-[min(18rem,calc(100vw-2rem))]">
          {STRATEGY_OPTIONS.map((option) => (<SelectItem key={option.id} value={option.id} description={t(option.descriptionKey, {
