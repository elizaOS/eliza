...
interface AccountManagementPanelProps {
    ...
    onSelectSubscription?: (providerId: SubscriptionProviderSelectionId) => Promise<void> | void;
}
function RowSkeleton() {
    return (<div className="flex items-center gap-3 rounded-lg border border-border/50 bg-card/40 px-3 py-2.5">
...
export function AccountManagementPanel({ activeSubscriptionId = null, activeChatProviderId = null, cloudCallsDisabled = false, onSelectChatProvider, onSelectSubscription, }: AccountManagementPanelProps) {
    const t = useAppSelector((s) => s.t);
    const setActionNotice = useAppSelector((s) => s.setActionNotice);
    const accounts = useAccounts({ setActionNotice });
    const [pendingProviderId, setPendingProviderId] = useState<LinkedAccountProviderId | undefined>(() => ACCOUNT_PROVIDER_OPTIONS.find((option) => readSubscriptionOAuth(option.id))?.id);
    const [addDialogOpen, setAddDialogOpen] = useState(() => Boolean(pendingProviderId));
    const [credentialRepairAccount, setCredentialRepairAccount] = useState<AccountWithCredentialFlag | null>(null);
    const [expanded, setExpanded] = useState<Set<string>>(() => new Set());
    const [showAvailable, setShowAvailable] = useState(false);
    const providerMap = useMemo(() => {
        if (!accounts.data)
            return new Map();
        return new Map(accounts.data.providers.map((provider) => [
            provider.providerId,
            provider,
        ]));
    }, [accounts.data]);
    ...
    const openCredentialRepair = useCallback((providerId: LinkedAccountProviderId, account: AccountWithCredentialFlag) => {
        setCredentialRepairAccount(account);
        setPendingProviderId(providerId);
        setAddDialogOpen(true);
    }, []);
    const handleMove = useCallback(async (providerId: LinkedAccountProviderId, sorted: AccountWithCredentialFlag[], accountId: string, direction: "up" | "down") => {
    ...
    const rowHandlers = {
        ...
        onStrategyChange: (providerId: LinkedAccountProviderId, strategy: AccountStrategy) => {
    ...
    if (accounts.loading && !accounts.data) {
        return (<div className="grid gap-2" aria-busy data-testid="account-management-panel" data-state="loading">
    ...
    if (accounts.error && !accounts.data) {
        return (<div className="flex flex-col items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-4" data-testid="account-management-panel" data-state="error">
        ...
        <Button type="button" variant="outline" size="sm" onClick={() => void accounts.refresh()}>
    ...
    return (<div className="grid gap-3" data-testid="account-management-panel" data-state="ready">
      {/* Header: one line of intent + the primary add affordance. */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-xs leading-5 text-muted">
          {t("accounts.management.summary", {
            defaultValue: "Provider accounts feed chat and coding agents. Chat defaults are chosen in Intelligence above; here you manage the accounts behind them.",
        })}
        </p>
        <Button type="button" variant="default" size="sm" onClick={() => openAdd(undefined)} className="shrink-0">
      ...
      {nothingConnected ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border/50 bg-bg-accent/20 px-6 py-8 text-center">
          ...
          <div className="grid gap-1">
            ...
            <p className="mx-auto max-w-sm text-xs leading-5 text-muted">
              {t("accounts.empty.description", {
                defaultValue: "Connect an API key for chat, or sign in to a coding subscription to power task agents. Add several to any provider and they'll rotate automatically.",
            })}
            </p>
          </div>
          <Button type="button" variant="default" size="sm" onClick={() => openAdd(undefined)}>
            <Plus className="size-3.5" aria-hidden/>
          ...
          {connectedOptions.map((option) => (<ProviderAccountRow key={option.id} option={option} provider={providerMap.get(option.id)} expanded={expanded.has(option.id)} onToggle={() => toggleExpanded(option.id)} {...rowHandlers}/>))}
        </div>)}

      {/* ── Available-to-connect disclosure (kills the empty-card stack) ── */}
      {availableOptions.length > 0 ? (<div className="grid gap-2">
          <Button type="button" variant="ghost" size="content" align="start" onClick={() => setShowAvailable((v) => !v)} className="w-full" aria-expanded={showAvailable}>
            <ChevronRight className={cn("size-3.5 shrink-0 transition-transform", showAvailable && "rotate-90")} aria-hidden/>
            {nothingConnected
          ...
          {showAvailable ? (<div className="grid gap-2">
              {availableOptions.map((option) => (<ProviderAccountRow key={option.id} option={option} provider={providerMap.get(option.id)} expanded={false} onToggle={() => toggleExpanded(option.id)} {...rowHandlers}/>))}
            </div>) : null}
        </div>) : null}
      ...
      <AddAccountDialog open={addDialogOpen} providerId={pendingProviderId} credentialRepairAccount={credentialRepairAccount} onClose={() => {
