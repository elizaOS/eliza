...
export function resolveProviderEligibility(option: AccountProviderOption, runtime: ProviderRuntimeEligibility | undefined): ResolvedEligibility {
    if (runtime) {
        return {
 ...
 */
export function eligibilityChips(eligibility: ResolvedEligibility): EligibilityChip[] {
...
export function runtimeEligibilityFor(provider: AccountsListProvider | undefined): ProviderRuntimeEligibility | undefined {
    return provider?.runtimeEligibility;
}
export type ProviderConnectionState = "connected-healthy" | "connected-attention" | "disconnected";
/**
 ...
 */
export function providerConnectionState(accounts: {
    enabled: boolean;
    health: string;
}[]): ProviderConnectionState {
    if (accounts.length === 0)
        return "disconnected";
    const needsAttention = accounts.some((a) => a.health === "needs-reauth" ||
        a.health === "invalid" ||
        a.health === "expired");
    if (needsAttention)
