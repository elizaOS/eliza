...
import { SUBSCRIPTION_PROVIDER_SELECTIONS } from "../../providers";
...
import { type ProviderCategory } from "./ProviderCard";
...
export function sortAiProviders(plugins: PluginInfo[]): PluginInfo[] {
    return [...plugins.filter((p) => p.category === "ai-provider")].sort((left, right) => {
        const leftCatalog = getFirstRunProviderOption(normalizeAiProviderPluginId(left.id));
        const rightCatalog = getFirstRunProviderOption(normalizeAiProviderPluginId(right.id));
...
export function computeAvailableProviderIds(allAiProviders: PluginInfo[]): Set<string> {
    return new Set([
        ...allAiProviders.map((provider) => getFirstRunProviderOption(normalizeAiProviderPluginId(provider.id))
            ?.id),
        ...FIRST_RUN_PROVIDER_CATALOG.filter((option) => option.authMode === "api-key" &&
            getDirectAccountProviderForFirstRunProvider(option.id)).map((option) => option.id),
...
export function useProviderEntries({ allAiProviders, elizaCloudConnected, cloudCallsDisabled, isCloudSelected, isCloudConfigured, resolvedSelectedId, subscriptionStatus, anthropicCliDetected, t, }: UseProviderEntriesArgs): UseProviderEntriesResult {
    const apiProviderChoices = useMemo<ApiProviderChoice[]>(() => {
        const pluginChoices = allAiProviders
            .map((provider) => {
            const option = getFirstRunProviderOption(normalizeAiProviderPluginId(provider.id));
        ...
        const accountManagedChoices = FIRST_RUN_PROVIDER_CATALOG.filter((option) => option.authMode === "api-key" &&
            getDirectAccountProviderForFirstRunProvider(option.id) &&
            !seen.has(option.id)).map((option) => ({
            id: option.id,
            label: option.name,
            provider: {
                id: option.id,
        ...
        return [...pluginChoices, ...accountManagedChoices].sort((left, right) => {
            const leftOrder = getFirstRunProviderOption(left.id)?.order ?? Number.MAX_SAFE_INTEGER;
            const rightOrder = getFirstRunProviderOption(right.id)?.order ?? Number.MAX_SAFE_INTEGER;
    ...
    const servingLocalFallback = isCloudConfigured && !elizaCloudConnected && !cloudCallsDisabled;
    const getProviderStatus = useCallback((entryId: ProviderPanelId): ProviderStatus => {
        ...
        if (isSubscriptionProviderSelectionId(entryId)) {
            const subSelection = SUBSCRIPTION_PROVIDER_SELECTIONS.find((provider) => provider.id === entryId);
            const statuses = subscriptionStatus.filter((entry) => entry.provider === entryId ||
                (subSelection
            ...
            if (entryId === "gemini-subscription" &&
                statuses.some((status) => status.source === "gemini-cli" &&
                    status.configured &&
                    status.valid)) {
                return { tone: "ok", label: "CLI detected" };
            }
