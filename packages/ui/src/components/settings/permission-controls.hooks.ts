...
import { type AllPermissionsState } from "../../api";
...
// ---------------------------------------------------------------------------
type RendererPermissionId = Extract<PermissionId, "camera" | "microphone" | "location" | "notifications">;
...
function isPermissionState(value: unknown, id: PermissionId): value is PermissionState {
...
function mapRendererMediaPermissionState(state: "granted" | "denied" | "prompt" | "default" | undefined): PermissionStatus | null {
    if (state === "granted") {
        return "granted";
    }
    if (state === "denied") {
...
async function queryRendererPermission(id: RendererPermissionId): Promise<PermissionStatus | null> {
...
async function inferRendererMediaPermissionFromDevices(id: Extract<RendererPermissionId, "camera" | "microphone">): Promise<PermissionStatus | null> {
    ...
    try {
        ...
        return devices.some((device) => device.kind === kind && Boolean(device.label?.trim()))
            ? "granted"
            : null;
    }
    catch {
...
async function probeRendererMediaPermission(id: RendererPermissionId): Promise<PermissionStatus | null> {
...
async function requestRendererPermission(id: PermissionId): Promise<PermissionStatus | null> {
    ...
    if (id === "location" && navigator.geolocation) {
        const requestedStatus = await new Promise<PermissionStatus | null>((resolve) => {
    ...
    if (id === "notifications" && typeof Notification !== "undefined") {
        return mapRendererMediaPermissionState(await Notification.requestPermission());
    }
    return probeRendererMediaPermission(id);
}
...
export async function reconcileRendererMediaPermissions(snapshot: DesktopPermissionsSnapshot): Promise<DesktopPermissionsSnapshot> {
    ...
    for (const id of RENDERER_PERMISSION_IDS) {
        ...
        if (!isRendererPermissionAuthoritative(id, current.platform, snapshot.nativeBridgeAvailable)) {
            continue;
        }
        ...
        if (current.status === rendererStatus &&
...
async function mergeRuntimePermissionsIntoSnapshot(snapshot: DesktopPermissionsSnapshot): Promise<DesktopPermissionsSnapshot> {
    let nextPermissions = snapshot.permissions;
    let changed = false;
...
export function useDesktopPermissionsState() {
    ...
    const loadPermissionsSnapshot = useCallback(async (forceRefresh = false): Promise<DesktopPermissionsSnapshot> => {
        ...
        const shellEnabled = bridgedShellEnabled === null
        ...
        const snapshot = {
            ...
            shellEnabled,
            nativeBridgeAvailable: bridgedPermissions !== null ||
                bridgedShellEnabled !== null ||
                bridgedPlatform !== null,
        };
        const runtimeMergedSnapshot = await mergeRuntimePermissionsIntoSnapshot(snapshot);
        return reconcileRendererMediaPermissions(runtimeMergedSnapshot);
    }, []);
    const replaceSnapshot = useCallback(async (forceRefresh = false): Promise<DesktopPermissionsSnapshot> => {
    ...
    const scheduleSettingsRefreshes = useCallback(() => {
        ...
        for (const delayMs of SETTINGS_REFRESH_DELAYS_MS) {
            ...
            timerId = window.setTimeout(() => {
                settingsRefreshTimersRef.current =
                    settingsRefreshTimersRef.current.filter((currentTimerId) => currentTimerId !== timerId);
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            try {
                ...
                logger.warn({ err }, "[permission-controls] permission snapshot load failed");
                if (!cancelled) {
                    setPermissions(null);
                    setPlatform("unknown");
                }
    ...
    const handleRequest = useCallback(async (id: PermissionId) => {
        try {
            ...
            if (isRendererPermissionId(id) &&
                isRendererPermissionAuthoritative(id, bridged?.platform ??
                    (platform as PermissionState["platform"] | undefined), bridged !== null)) {
    ...
    const handleOpenSettings = useCallback(async (id: PermissionId) => {
        try {
            if (isRuntimePermissionId(id)) {
