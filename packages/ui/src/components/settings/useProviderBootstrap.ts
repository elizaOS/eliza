...
export function useProviderBootstrap(selection: ReturnType<typeof useProviderSelection>, cloudModel: ReturnType<typeof useCloudModelConfig>, enabled = true): ProviderBootstrapState {
    const [subscriptionStatus, setSubscriptionStatus] = useState<SubscriptionProviderStatus[]>([]);
    ...
    useEffect(() => {
        const anthStatuses = subscriptionStatus.filter((s) => s.provider === "anthropic-subscription");
        const oaiStatuses = subscriptionStatus.filter((s) => s.provider === "openai-subscription" || s.provider === "openai-codex");
        ...
        const anthAppConnected = anthStatuses.some((status) => status.configured && status.valid && status.source === "app");
        setAnthropicConnected(anthAppConnected);
        setAnthropicCliDetected(anthStatuses.some((status) => status.configured &&
            status.valid &&
            status.source === "claude-code-cli"));
        setOpenaiConnected(oaiStatuses.some((status) => status.configured && status.valid));
    }, [subscriptionStatus]);
    return {
