...
import { normalizeSubscriptionProviderSelectionId } from "@elizaos/core/contracts/first-run-options";
...
function readRememberedProviderPanel(elizaCloudConnected: boolean): ProviderPanelId | null {
    if (typeof window === "undefined")
        return null;
    try {
        const remembered = new URLSearchParams(window.location.search).get("provider") ??
        ...
        if (remembered === "__cloud__" && !elizaCloudConnected)
            return null;
        return remembered;
    }
    catch {
 ...
 */
export function resolveDefaultIntelligencePanelId({ cloudCallsDisabled, cloudRuntimeLocked, elizaCloudConnected, isCloudSelected, resolvedSelectedId, }: {
    ...
    if (cloudRuntimeLocked && elizaCloudConnected)
        return "__cloud__";
    if (cloudCallsDisabled)
        return "__local__";
    if (isCloudSelected && !elizaCloudConnected)
        return "__local__";
...
function readSubscriptionProvider(cfg: Record<string, unknown>): SubscriptionProviderSelectionId | null {
    const agents = asRecord(cfg.agents);
    const defaults = asRecord(agents?.defaults);
    return normalizeSubscriptionProviderSelectionId(defaults?.subscriptionProvider);
}
...
export function useProviderSelection(availableProviderIds: Set<string>, notifySelectionFailure: (prefix: string, err: unknown) => void, elizaCloudConnected: boolean = false): ProviderSelection {
    const { setActionNotice, handleCloudDisconnect } = useAppSelectorShallow((s) => ({
        setActionNotice: s.setActionNotice,
        handleCloudDisconnect: s.handleCloudDisconnect,
    }));
    const branding = useBranding();
    const cloudRuntimeLocked = branding.cloudOnly === true || isElizaCloudRuntimeLocked();
    ...
    const [selectedProviderPanelId, setSelectedProviderPanelId] = useState<ProviderPanelId | null>(() => readRememberedProviderPanel(elizaCloudConnected));
    const readCloudCallsDisabled = useCallback((cfg: Record<string, unknown>): boolean => {
        const llmText = resolveServiceRoutingInConfig(cfg)?.llmText;
        if (llmText?.transport === "cloud-proxy" ||
    ...
    const initializeFromConfig = useCallback((cfg: Record<string, unknown>) => {
        ...
        const nextSelectedId = llmText?.transport === "cloud-proxy" && providerId === "elizacloud"
            ? "__cloud__"
            : llmText?.transport === "direct"
                ? (providerId ?? null)
    ...
    const resolvedSelectedId = useMemo(() => selectedProviderId === "__cloud__"
        ? "__cloud__"
        : selectedProviderId &&
            (availableProviderIds.has(selectedProviderId) ||
                isSubscriptionProviderSelectionId(selectedProviderId))
            ? selectedProviderId
            : null, [availableProviderIds, selectedProviderId]);
    const restoreSelection = useCallback((previousSelectedId: string | null, previousManualSelection: boolean) => {
        hasManualSelection.current = previousManualSelection;
        setSelectedProviderId(previousSelectedId);
    }, []);
    const handleSwitchProvider = useCallback(async (newId: string, providerId: string) => {
    ...
    const handleSelectSubscription = useCallback(async (providerId: SubscriptionProviderSelectionId, activate: boolean = true) => {
        if (!cloudCallsDisabled && resolvedSelectedId === providerId)
            return;
        const previousSelectedId = resolvedSelectedId;
        const previousManualSelection = hasManualSelection.current;
        const previousCloudCallsDisabled = cloudCallsDisabled;
    ...
    const handleSelectCloud = useCallback(async () => {
        if (!cloudCallsDisabled && resolvedSelectedId === "__cloud__")
            return;
        const previousSelectedId = resolvedSelectedId;
        const previousManualSelection = hasManualSelection.current;
        const previousCloudCallsDisabled = cloudCallsDisabled;
    ...
    const handleSelectLocalOnly = useCallback(async () => {
        if (cloudRuntimeLocked) {
            setActionNotice?.("Eliza Cloud is required while this app is running in cloud mode.", "error", 6000);
            return;
        }
        const previousSelectedId = resolvedSelectedId;
        const previousManualSelection = hasManualSelection.current;
    ...
    const isCloudConfigured = resolvedSelectedId === "__cloud__" || resolvedSelectedId === null;
    ...
    const effectiveCloudCallsDisabled = configuredCloudCallsDisabled ||
    ...
    const visibleProviderPanelId: ProviderPanelId = cloudRuntimeLocked &&
        elizaCloudConnected &&
 ...
 */
export function resolveProviderIdForSwitch(newId: string, aiProviders: AiProviderLike[]): string {
    const target = aiProviders.find((provider) => (getFirstRunProviderOption(normalizeAiProviderPluginId(provider.id))
        ?.id ?? normalizeAiProviderPluginId(provider.id)) === newId) ?? null;
    return (getFirstRunProviderOption(normalizeAiProviderPluginId(target?.id ?? newId))
        ?.id ?? newId);
}
