...
export function ProviderRoutingPanel({ largeModelOptions, cloudModelSchema, modelValues, currentLargeModel, modelSaving, modelSaveSuccess, onModelFieldChange, showCloudControls, elizaCloudConnected, }: ProviderRoutingPanelProps) {
    const t = useAppSelector((s) => s.t);
    const hasModelControls = elizaCloudConnected &&
        (largeModelOptions.length > 0 || cloudModelSchema !== null);
    if (!showCloudControls || !hasModelControls)
        return null;
    return (<div className="flex flex-col">
      {largeModelOptions.length > 0 ? (<SettingsSelectRow agentId="routing-primary-model" label={t("providerswitcher.model", {
                defaultValue: "Primary model",
            })} value={currentLargeModel || ""} onValueChange={(v) => onModelFieldChange("large", v)} placeholder={t("providerswitcher.chooseModel", {
                defaultValue: "Choose a model",
            })} options={largeModelOptions.map((model) => ({
                value: model.id,
                label: model.name,
            }))} triggerClassName="w-full"/>) : null}
      {cloudModelSchema ? (<AdvancedSettingsDisclosure title="Model overrides">
          <ConfigRenderer schema={cloudModelSchema.schema} hints={cloudModelSchema.hints} values={modelValues.values} setKeys={modelValues.setKeys} registry={defaultRegistry} onChange={onModelFieldChange}/>
        </AdvancedSettingsDisclosure>) : null}
      {modelSaving ? (<span className="inline-flex items-center py-2 text-muted" title={t("providerswitcher.savingRestarting")} role="status" aria-label={t("providerswitcher.savingRestarting")}>
          <Loader2 className="size-3.5 animate-spin"/>
        </span>) : null}
      {modelSaveSuccess ? (<span className="inline-flex items-center py-2 text-ok" title={t("providerswitcher.savedRestartingAgent")} role="status" aria-label={t("providerswitcher.savedRestartingAgent")}>
