...
import { CloudSourceModeToggle } from "../cloud/CloudSourceControls";
import { Input } from "../ui/input";
import { PREMADE_VOICES } from "../../voice";
...
import { isCloudVoiceRunnable } from "../../voice";
import { isElectrobunRuntime } from "../../bridge";
import { normalizeForWake } from "../../voice";
import { sanitizeApiKey } from "../../voice";
import { type AsrProvider } from "../../api";
import { type SwabbleConfig } from "../../bridge/native-plugins";
import { type VoiceConfig } from "../../api";
import { type VoiceMode } from "../../api";
import { type VoiceProvider } from "../../api";
...
export function DesktopTalkModePanel() {
    ...
    const { ref: refreshRef, agentProps: refreshAgentProps } = useAgentElement<HTMLButtonElement>({
        id: "voice-talkmode-refresh",
        role: "button",
        label: t("common.refresh"),
        group: "voice-talkmode",
        onActivate: () => void runAction("voice-talkmode-refresh", async () => { }, t("voiceconfigview.TalkModeStateRefreshed")),
    });
    const { ref: phraseRef, agentProps: phraseAgentProps } = useAgentElement<HTMLInputElement>({
    ...
    const { ref: startStopRef, agentProps: startStopAgentProps } = useAgentElement<HTMLButtonElement>({
        ...
        onActivate: () => void runAction("voice-talkmode-start-stop", async () => {
            ...
            if (result?.available === false) {
                throw new Error(result.reason || t("voiceconfigview.TalkModeUnavailable"));
            }
        }, panelState.enabled
            ? t("voiceconfigview.TalkModeStopped")
            : t("voiceconfigview.TalkModeStarted")),
    });
    const { ref: speakRef, agentProps: speakAgentProps } = useAgentElement<HTMLButtonElement>({
        id: "voice-talkmode-speak",
        role: "button",
        label: t("voiceconfigview.SpeakTestPhrase"),
        group: "voice-talkmode",
        status: phrase.trim() ? "active" : "inactive",
        onActivate: () => void runAction("voice-talkmode-speak", async () => {
            await invokeDesktopBridgeRequest<void>({
                rpcMethod: "talkmodeSpeak",
                ipcChannel: "talkmode:speak",
                params: { text: phrase },
            });
        }, t("voiceconfigview.SpeechRequested"), false),
    });
    const { ref: stopSpeakingRef, agentProps: stopSpeakingAgentProps } = useAgentElement<HTMLButtonElement>({
        id: "voice-talkmode-stop-speaking",
        role: "button",
        label: t("voiceconfigview.StopSpeaking"),
        group: "voice-talkmode",
        onActivate: () => void runAction("voice-talkmode-stop-speaking", async () => {
            await invokeDesktopBridgeRequest<void>({
    ...
    return (<Card variant="panel">
      <CardHeader className="p-4 pb-0">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Button ref={refreshRef} variant="outline" onClick={() => void runAction("voice-talkmode-refresh", async () => { }, t("voiceconfigview.TalkModeStateRefreshed"))} disabled={loading || busyAction === "voice-talkmode-refresh"} {...refreshAgentProps}>
            {t("common.refresh")}
          </Button>
        </div>
      </CardHeader>
      ...
      <CardContent className="flex flex-col gap-4 px-4 pb-4">
        {(error || message) && (<div className={`rounded-sm border px-3 py-2.5 text-xs-tight leading-5 ${error
        ...
        <Input ref={phraseRef} type="text" value={phrase} onChange={(event) => setPhrase(event.target.value)} placeholder={t("voiceconfigview.testPhrase")} {...phraseAgentProps}/>

        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
          <Button ref={startStopRef} variant="outline" onClick={() => void runAction("voice-talkmode-start-stop", async () => {
            if (panelState.enabled) {
                await invokeDesktopBridgeRequest<void>({
                    rpcMethod: "talkmodeStop",
                    ipcChannel: "talkmode:stop",
            ...
            ? t("voiceconfigview.TalkModeStopped")
            : t("voiceconfigview.TalkModeStarted"))} disabled={busyAction === "voice-talkmode-start-stop" || loading} {...startStopAgentProps}>
            {panelState.enabled
            ? t("voiceconfigview.StopTalkMode")
            : t("voiceconfigview.StartTalkMode")}
          </Button>
          <Button ref={speakRef} variant="outline" onClick={() => void runAction("voice-talkmode-speak", async () => {
            await invokeDesktopBridgeRequest<void>({
                rpcMethod: "talkmodeSpeak",
                ipcChannel: "talkmode:speak",
                params: { text: phrase },
            });
        }, t("voiceconfigview.SpeechRequested"), false)} disabled={!phrase.trim() || busyAction === "voice-talkmode-speak"} {...speakAgentProps}>
            {t("voiceconfigview.SpeakTestPhrase")}
          </Button>
          <Button ref={stopSpeakingRef} variant="outline" onClick={() => void runAction("voice-talkmode-stop-speaking", async () => {
            await invokeDesktopBridgeRequest<void>({
                rpcMethod: "talkmodeStopSpeaking",
                ipcChannel: "talkmode:stopSpeaking",
            });
        }, t("voiceconfigview.StoppedCurrentSpeechOutput"))} disabled={busyAction === "voice-talkmode-stop-speaking"} {...stopSpeakingAgentProps}>
...
function RemoveTriggerButton({ trigger, onRemove, label, }: {
    trigger: string;
    onRemove: () => void;
    label: string;
}) {
    ...
    return (<Button ref={ref} variant="ghost" size="icon-sm" shape="circle" className="ml-1" onClick={onRemove} aria-label={label} {...agentProps}>
      ×
    </Button>);
}
function ModelSizeButton({ id, active, hint, onSelect, }: {
    id: NonNullable<SwabbleConfig["modelSize"]>;
    active: boolean;
    hint?: string;
    onSelect: () => void;
    ...
    return (<Button ref={ref} variant={active ? "default" : "outline"} size="tile" onClick={onSelect} {...agentProps}>
...
export function WakeWordSection({ serverConfig, }: {
    ...
    const characterName = useAppSelector((s) => s.characterData?.name?.trim() ||
    ...
    const [modelSize, setModelSize] = useState<NonNullable<SwabbleConfig["modelSize"]>>("base");
        ...
        void (async () => {
            try {
                ...
                if (resolved) {
                    ...
                    if (resolved.modelSize)
                        setModelSize(resolved.modelSize);
                }
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                const h = await getSwabblePlugin().addListener("audioLevel", (evt: {
                    level: number;
                }) => {
    ...
    const buildConfig = useCallback((): SwabbleConfig => ({
        triggers,
    ...
    const addTrigger = useCallback((raw: string) => {
    ...
    const handleModelSizeChange = useCallback(async (size: NonNullable<SwabbleConfig["modelSize"]>) => {
        setModelSize(size);
        try {
    ...
    const { ref: enableRef, agentProps: enableAgentProps } = useAgentElement<HTMLButtonElement>({
    ...
    const { ref: addTriggerRef, agentProps: addTriggerAgentProps } = useAgentElement<HTMLInputElement>({
    ...
    const { ref: gapRef, agentProps: gapAgentProps } = useAgentElement<HTMLInputElement>({
    ...
    return (<div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex min-h-11 items-center gap-2 rounded-md border border-border bg-surface px-3">
          ...
          <Switch ref={enableRef} checked={enabled} onCheckedChange={() => void handleToggle()} aria-label={enabled
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex min-h-10 flex-wrap gap-1.5 rounded-sm border border-border/60 bg-bg p-2">
          {triggers.map((trigger) => (<span key={trigger} className="flex min-h-7 items-center gap-1 rounded-full border border-accent/30 bg-accent/10 px-2 py-1 text-xs text-txt">
              {trigger}
              {triggers.length > 1 && (<RemoveTriggerButton trigger={trigger} onRemove={() => removeTrigger(trigger)} label={t("voiceconfigview.RemoveTrigger", {
                    defaultValue: 'Remove trigger "{{trigger}}"',
                    trigger,
                })}/>)}
            </span>))}
          <Input ref={addTriggerRef} type="text" variant="embeddedToken" className="min-w-[120px] flex-1" density="compact" placeholder={t("voiceconfigview.AddTrigger")} value={triggerInput} onChange={(e) => setTriggerInput(e.target.value)} onKeyDown={(e) => {
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Input ref={gapRef} type="range" variant="nativeRange" min={0.1} max={2.0} step={0.05} value={postTriggerGap} aria-label={t("voiceconfigview.PostTriggerGap", {
            defaultValue: "Post-trigger gap",
        })} onChange={(e) => void handlePostTriggerGapChange(parseFloat(e.target.value))} {...gapAgentProps}/>
      </div>
      <div className="flex flex-col gap-1.5">
        <span className="text-xs font-semibold">
        ...
        <div className="grid grid-cols-2 gap-1.5 xl:grid-cols-5">
          {MODEL_SIZES.map((m) => (<ModelSizeButton key={m.id} id={m.id} active={modelSize === m.id} hint={m.hintKey ? t(m.hintKey) : undefined} onSelect={() => void handleModelSizeChange(m.id)}/>))}
        </div>
      </div>
      <div className="flex flex-col gap-1">
        ...
        <div className="h-2 w-full overflow-hidden rounded-full bg-border/70">
          <div ref={meterRef} className="h-full w-full origin-left rounded-full bg-ok transition-transform duration-75" style={{ transform: "scaleX(0)" }}/>
        </div>
      </div>
    </div>);
}
function AsrProviderButton({ id, label, hint, active, onSelect, }: {
    id: AsrProvider;
    label: string;
    hint: string;
    active: boolean;
 ...
 */
function AsrAdvancedSection({ currentAsrProvider, onChange, defaultAsrProvider, }: {
    ...
    return (<div className="flex flex-col gap-3">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        {ASR_PROVIDERS.map((p) => (<AsrProviderButton key={p.id} id={p.id} label={p.label} hint={p.hint} active={currentAsrProvider === p.id} onSelect={() => onChange(p.id)}/>))}
      </div>
      {currentAsrProvider === "local-inference" && localStatusBusy && (<p className="text-xs leading-5 text-warn">
          {t("voiceconfigview.AsrDownloading", {
                defaultValue: "Downloading the local model. Using cloud until it's ready.",
            })}
        </p>)}
      {currentAsrProvider === "openai" && (<p className="text-xs leading-5 text-muted">
          {t("voiceconfigview.AsrUsesOpenAiKey", {
                defaultValue: "Uses your OpenAI API key from the Providers section.",
            })}
        </p>)}
    </div>);
}
function TtsProviderButton({ id, label, hint, active, onSelect, }: {
    id: VoiceProvider;
    label: string;
    hint: string;
    active: boolean;
...
function PremadeVoiceButton({ voiceId, name, hint, active, onSelect, }: {
    voiceId: string;
    name: string;
    hint: string;
    active: boolean;
    ...
    return (<Button ref={ref} variant="choice" size="row" align="start" data-state={active ? "on" : "off"} onClick={onSelect} {...agentProps}>
...
export function VoiceConfigView() {
    ...
    const elizaCloudVoiceProxyAvailable = useAppSelector((s) => s.elizaCloudVoiceProxyAvailable);
    const [voiceConfig, setVoiceConfig] = useState<VoiceConfig>({});
    const [swabbleServerConfig, setSwabbleServerConfig] = useState<Partial<SwabbleConfig> | null>(null);
    ...
    const hasElevenLabsApiKey = hasConfiguredApiKey(voiceConfig.elevenlabs?.apiKey);
    ...
    const currentAsrProvider: AsrProvider = voiceConfig.asr?.provider ?? providerDefaults.asr;
    ...
    const resolvedTtsDefaultLabel = resolvedTtsDefault === "local-inference"
    ...
    const isConfigured = (() => {
        if (currentProvider === "eliza-cloud")
            return cloudVoiceAvailable;
    ...
    const performSave = useCallback(async () => {
        ...
        const normalizedElevenLabs = provider === "elevenlabs"
            ? {
    ...
    if (loading) {
        return (<div className="rounded-sm border border-border/60 bg-card/92 px-4 py-6 text-center text-xs text-muted ">
    ...
    return (<SettingsStack>
      <SettingsGroup bare title={t("voiceconfigview.TTSProvider")} footer={<span className="flex items-center gap-2">
            ...
            <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${isConfigured
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {VOICE_PROVIDERS.map((p) => (<TtsProviderButton key={p.id} id={p.id} label={t(p.labelKey, { defaultValue: p.label })} hint={t(p.hintKey, { defaultValue: p.hint })} active={currentProvider === p.id} onSelect={() => handleProviderChange(p.id)}/>))}
        </div>
      </SettingsGroup>

      {currentProvider === "elevenlabs" && (<SettingsGroup title={t("voiceconfigview.APISource")} action={<CloudSourceModeToggle mode={currentMode} onChange={handleModeChange}/>}>
          {currentMode === "cloud" ? (<SettingsRow label={t("voiceconfigview.APISource")} stacked>
              <CloudConnectionStatus connected={elizaCloudConnected} disconnectedText={t("elizaclouddashboard.ElizaCloudNotConnected")}/>
            </SettingsRow>) : null}
          {currentMode === "own-key" ? (<SettingsInputRow agentId="voice-tts-elevenlabs-key" group="voice-tts" type="password" label={t("settings.voice.elevenLabsApiKey", {
                    defaultValue: "ElevenLabs API key",
                })} description={<>
                  {t("voiceconfigview.GetYourKeyAt")}{" "}
                  <a href="https://elevenlabs.io" target="_blank" rel="noopener noreferrer" className="text-txt underline decoration-accent underline-offset-2 hover:opacity-80">
                    {t("voiceconfigview.elevenlabsIo")}
                  </a>
                  {" · "}
                  {t("voiceconfigview.FastPathDefaultE")}
                  <code>{DEFAULT_ELEVEN_FAST_MODEL}</code>
                </>} value={apiKeyDraft} onValueChange={(next) => {
                    setApiKeyDraft(next);
                    handleApiKeyChange(next);
                }} placeholder={voiceConfig.elevenlabs?.apiKey
                    ? t("mediasettingssection.ApiKeySetLeaveBlank")
                    : t("mediasettingssection.EnterApiKey")} inputClassName="w-full"/>) : null}
          <SettingsRow label={t("common.voice")} stacked>
            <div className="grid gap-1.5 sm:grid-cols-2 xl:grid-cols-3">
              {PREMADE_VOICES.map((preset) => (<PremadeVoiceButton key={preset.id} voiceId={preset.voiceId} name={preset.nameKey
                    ? t(preset.nameKey, { defaultValue: preset.name })
                    : preset.name} hint={preset.hintKey
                    ? t(preset.hintKey, { defaultValue: preset.hint })
                    : preset.hint} active={selectedVoiceId === preset.voiceId} onSelect={() => handleVoiceSelect(preset.voiceId)}/>))}
            </div>
          </SettingsRow>
          {selectedPreset ? (<SettingsRow label={t("voiceconfigview.Microphone")} stacked>
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <Button variant="outline" size="touch" disabled={testing} onClick={() => handleTestVoice(selectedPreset.previewUrl)}>
                  {testing
                ...
                {testing ? (<Button variant="outline" size="touch" onClick={() => {
                        if (audioRef.current) {
                            audioRef.current.pause();
                            setTesting(false);
                        }
      ...
      {currentProvider === "edge" && (<SettingsGroup bare>
          <p className="py-1 text-xs leading-5 text-muted">
            {t("voiceconfigview.EdgeTTSUsesMicros")}
          </p>
        </SettingsGroup>)}
      {currentProvider === "robot-voice" && (<SettingsGroup bare>
          <p className="py-1 text-xs leading-5 text-muted">
      ...
      <SettingsGroup>
        <SettingsRow label={t("voiceconfigview.advancedSettings", {
            defaultValue: "Advanced settings",
        })} control={<AdvancedToggle label="Advanced"/>}/>
      </SettingsGroup>

      {advancedEnabled && (<SettingsGroup bare>
          <AsrAdvancedSection currentAsrProvider={currentAsrProvider} onChange={handleAsrProviderChange} defaultAsrProvider={providerDefaults.asr}/>
        </SettingsGroup>)}
      ...
      <SaveFooter dirty={dirty} saving={saving} saveError={saveError} saveSuccess={saveSuccess} onSave={() => void handleSave()}/>
    </SettingsStack>);
}
