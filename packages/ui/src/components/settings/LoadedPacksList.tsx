...
function LoadedPackRow({ pack, isActive, activeLabel, onToggle, }: {
    ...
    return (<ActionListRow element="button" title={pack.manifest.name} description={pack.manifest.description ?? undefined} selected={isActive} onClick={onToggle} buttonRef={ref} {...agentProps} trailing={isActive ? (<span className="inline-flex size-6 shrink-0 items-center justify-center text-accent" title={activeLabel} role="img" aria-label={activeLabel}>
            <Check className="size-4" aria-hidden/>
          </span>) : null}/>);
}
export function LoadedPacksList({ loadedPacks, activePackId, onToggle, }: LoadedPacksListProps) {
    ...
    return (<SettingsGroup title={t("settings.appearance.loadedPacks", {
            defaultValue: "Loaded content packs",
        })}>
      {loadedPacks.map((pack) => (<LoadedPackRow key={pack.manifest.id} pack={pack} isActive={activePackId === pack.manifest.id} activeLabel={activeLabel} onToggle={() => onToggle(pack)}/>))}
    </SettingsGroup>);
}
