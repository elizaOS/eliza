 ...
 */
import { DEFAULT_ACTION_PLANNER_MODEL } from "./cloud-model-schema";
import { DEFAULT_ELIZA_CLOUD_TEXT_MODEL } from "@elizaos/core/contracts/service-routing";
import { DEFAULT_RESPONSE_HANDLER_MODEL } from "./cloud-model-schema";
import { buildCloudModelSchema } from "./cloud-model-schema";
...
function readConfigString(source: Record<string, unknown> | null | undefined, key: string): string {
...
export function useCloudModelConfig(onSaveError: (prefix: string, err: unknown) => void): CloudModelConfig {
    const { setTimeout } = useTimeout();
    const [modelOptions, setModelOptionsState] = useState<FirstRunOptions["models"] | null>(null);
    ...
    const initializeFromConfig = useCallback((cfg: Record<string, unknown>, elizaCloudEnabledCfg: boolean) => {
        ...
        setCurrentNanoModel(readConfigString(models, "nano") ||
            llmText?.nanoModel ||
            envFor("NANO_MODEL") ||
            cloudDefault);
        setCurrentSmallModel(readConfigString(models, "small") ||
            llmText?.smallModel ||
            envFor("SMALL_MODEL") ||
            cloudDefault);
        setCurrentMediumModel(readConfigString(models, "medium") ||
            llmText?.mediumModel ||
            envFor("MEDIUM_MODEL") ||
            cloudDefault);
        setCurrentLargeModel(readConfigString(models, "large") ||
            llmText?.largeModel ||
            envFor("LARGE_MODEL") ||
            cloudDefault);
        setCurrentMegaModel(readConfigString(models, "mega") ||
            llmText?.megaModel ||
            envFor("MEGA_MODEL") ||
            cloudDefault);
    ...
    const handleModelFieldChange = useCallback((key: string, value: unknown) => {
        ...
        if (key === "nano")
            setCurrentNanoModel(val);
        if (key === "small")
            setCurrentSmallModel(val);
        if (key === "medium")
            setCurrentMediumModel(val);
        if (key === "large")
            setCurrentLargeModel(val);
        if (key === "mega")
            setCurrentMegaModel(val);
