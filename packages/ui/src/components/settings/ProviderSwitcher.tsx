...
import { ApiKeyPanel } from "./ProviderPanels";
import { CloudPanel } from "./ProviderPanels";
import { FIRST_RUN_PROVIDER_CATALOG } from "../../providers";
import { IntelligenceServingSummary } from "./IntelligenceServingSummary";
import { LocalProviderPanel } from "./ProviderPanels";
...
import { computeAvailableProviderIds } from "./useProviderEntries";
import { describeUnsignedCloudChat } from "./ProviderPanels";
import { getDirectAccountProviderForFirstRunProvider } from "../../providers";
...
import { isRealtimeVoiceSelfHostedEnabled } from "../../voice/realtime-voice-build-flags";
import { isSubscriptionProviderSelectionId } from "../../providers";
import { resolveEffectiveVoiceConfig } from "../../voice/voice-chat-types";
import { resolveProviderIdForSwitch } from "./useProviderSelection";
import { sortAiProviders } from "./useProviderEntries";
...
interface ProviderSwitcherProps {
    ...
    handlePluginConfigSave?: (pluginId: string, values: Record<string, unknown>) => void | Promise<void>;
...
export function ProviderSwitcher(props: ProviderSwitcherProps = {}) {
    ...
    const t = app.t;
    const realtimeVoiceEnabled = isRealtimeVoiceForceEnabled() || isRealtimeVoiceSelfHostedEnabled();
    ...
    const voiceProvider = VOICE_PROVIDERS.find((provider) => provider.id === effectiveVoiceConfig?.provider);
    const plugins = Array.isArray(props.plugins)
        ? props.plugins
        : Array.isArray(app.plugins)
            ? app.plugins
            : [];
    const pluginSaving = props.pluginSaving ??
        (app.pluginSaving instanceof Set ? app.pluginSaving : new Set<string>());
    const pluginSaveSuccess = props.pluginSaveSuccess ??
    ...
    const notifySelectionFailure = useCallback((prefix: string, err: unknown) => {
    ...
    const availableProviderIds = useMemo(() => computeAvailableProviderIds(allAiProviders), [allAiProviders]);
    const selection = useProviderSelection(availableProviderIds, notifySelectionFailure, elizaCloudConnected);
    const cloudModel = useCloudModelConfig(notifySelectionFailure);
    const bootstrap = useProviderBootstrap(selection, cloudModel, !selection.cloudRuntimeLocked);
    const { apiProviderChoices, providerEntries, servingLocalFallback } = useProviderEntries({
        allAiProviders,
        elizaCloudConnected,
    ...
    const settingsContentReady = bootstrap.routingConfigResolved || selection.cloudRuntimeLocked;
    ...
    const displayedProviderEntries = useMemo(() => reconcileProviderEntriesWithServingAxes(providerEntries, servingAxes), [providerEntries, servingAxes]);
    const activeEntry = useMemo(() => displayedProviderEntries.find((entry) => entry.current) ?? null, [displayedProviderEntries]);
    const activeChatCatalogProvider = resolveActiveChatCatalogProvider(resolvedSelectedId, elizaCloudConnected);
    const selectedPanelProvider = useMemo(() => {
        if (visibleProviderPanelId === "__cloud__" ||
            visibleProviderPanelId === "__local__" ||
            isSubscriptionProviderSelectionId(visibleProviderPanelId)) {
            return null;
        }
        return (apiProviderChoices.find((choice) => choice.id === visibleProviderPanelId)
            ?.provider ?? null);
    }, [apiProviderChoices, visibleProviderPanelId]);
    const apiKeyPanelLabel = apiProviderChoices.find((choice) => choice.id === visibleProviderPanelId)
        ?.label ??
    ...
    const onSwitchProvider = useCallback((id: string) => {
        void selection.handleSwitchProvider(id, resolveProviderIdForSwitch(id, allAiProviders));
    }, [allAiProviders, selection]);
    const activeChatProviderId = getDirectAccountProviderForFirstRunProvider(resolvedSelectedId);
    const onSelectChatProvider = useCallback((accountProviderId: LinkedAccountProviderId) => {
        const provider = FIRST_RUN_PROVIDER_CATALOG.find((candidate) => getDirectAccountProviderForFirstRunProvider(candidate.id) ===
            accountProviderId);
        if (!provider) {
            setActionNotice?.("This account provider cannot be selected for chat.", "error", 6000);
            return;
        }
        void selection.handleSwitchProvider(provider.id, resolveProviderIdForSwitch(provider.id, allAiProviders));
    ...
    const intelligenceEntries = displayedProviderEntries.filter((entry) => entry.category === "cloud" ||
        (entry.category === "local" && !selection.cloudRuntimeLocked));
    const keyEntries = displayedProviderEntries.filter((entry) => entry.category === "key");
    const renderChip = (entry: ProviderListEntry) => (<ProviderCard key={entry.id} id={entry.id} icon={entry.icon} label={entry.label} category={entry.category} status={entry.status} current={entry.current} selected={visibleProviderPanelId === entry.id} onSelect={selection.handleProviderPanelSelect}/>);
    ...
    const intelligenceDescription = (entry: ProviderListEntry) => entry.category === "cloud"
        ? elizaCloudConnected
            ? t("providerswitcher.cloudTileDescription", {
                defaultValue: "Managed models through your Eliza Cloud account. No setup — sign in and it works.",
            })
            : describeUnsignedCloudChat(servingAxes, t, "tile")
        : servingAxes.runtime === "remote"
            ? t("providerswitcher.remoteLocalTileDescription", {
                defaultValue: "Runs with your remote agent. Private to that host and available while it stays online.",
            })
            : t("providerswitcher.localTileDescription", {
                defaultValue: "Use an installed model to process chat on this device.",
            });
    return (<SettingsStack>
      <SettingsGroup title={t("providerswitcher.intelligenceGroupTitle", {
        ...
        {!selection.cloudRuntimeLocked &&
            activeEntry &&
            activeEntry.category === "subscription" ? (<ActiveProviderSummary entry={activeEntry} t={t}/>) : null}
        {!selection.cloudRuntimeLocked ? (<div className="grid gap-2 sm:grid-cols-2">
            {intelligenceEntries.map((entry) => (<ProviderCard key={entry.id} id={entry.id} icon={entry.icon} label={entry.label} category={entry.category} status={entry.status} current={entry.current} selected={visibleProviderPanelId === entry.id} onSelect={selection.handleProviderPanelSelect} variant="tile" description={intelligenceDescription(entry)}/>))}
          </div>) : null}

        {bootstrap.routingConfigResolved &&
            visibleProviderPanelId === "__local__" &&
            !selection.cloudRuntimeLocked ? (<LocalProviderPanel cloudCallsDisabled={selection.cloudCallsDisabled && servingAxes.inference === "local"} routingModeSaving={selection.routingModeSaving} onSelectLocalOnly={() => void selection.handleSelectLocalOnly()} runtime={servingAxes.runtime} servingFallback={Boolean(servingLocalFallback)}/>) : null}

        {bootstrap.routingConfigResolved &&
            visibleProviderPanelId === "__cloud__" &&
            !selection.cloudRuntimeLocked ? (<CloudPanel cloudCallsDisabled={selection.cloudCallsDisabled} isCloudSelected={selection.isCloudConfigured} routingModeSaving={selection.routingModeSaving} onSelectCloud={() => void selection.handleSelectCloud()} onSignIn={handleCloudSignIn} elizaCloudConnected={elizaCloudConnected} largeModelOptions={cloudModel.largeModelOptions} cloudModelSchema={cloudModel.cloudModelSchema} modelValues={cloudModel.modelValues} currentLargeModel={cloudModel.currentLargeModel} modelSaving={cloudModel.modelSaving} modelSaveSuccess={cloudModel.modelSaveSuccess} onModelFieldChange={cloudModel.handleModelFieldChange} servingAxes={servingAxes}/>) : null}
      </SettingsGroup>

      {/* Per-role model configuration (small/large chat brains + coding
            sub-agent), driven by the validated /api/models catalog. */}
      {settingsContentReady && !selection.cloudRuntimeLocked ? (<ModelConfigurationPanel activeChatProvider={activeChatCatalogProvider} showChatModels={!isSubscriptionProviderSelectionId(resolvedSelectedId)}/>) : null}

      {settingsContentReady && !selection.cloudRuntimeLocked ? (<SettingsGroup title={t("providerswitcher.accountsGroupTitle", {
                defaultValue: "Accounts",
            })} description={t("providerswitcher.accountsGroupDescription", {
                defaultValue: "Connect provider accounts without scattering provider pickers across the page.",
            })} bare>
          <AccountManagementPanel activeChatProviderId={activeChatProviderId} activeSubscriptionId={isSubscriptionProviderSelectionId(resolvedSelectedId)
                ? resolvedSelectedId
                : null} cloudCallsDisabled={selection.cloudCallsDisabled} onSelectChatProvider={onSelectChatProvider} onSelectSubscription={selection.handleSelectSubscription}/>
        </SettingsGroup>) : null}

      {settingsContentReady ? (<SettingsGroup title={t("providerswitcher.voiceGroupTitle", {
                defaultValue: "Voice",
            })} bare>
          <SettingsRow icon={Mic} label={t("providerswitcher.speechPlaybackLabel", {
            ...
            })} control={<span className="text-xs text-txt-strong">
                {voiceProvider
          ...
          {realtimeVoiceEnabled && !selection.cloudRuntimeLocked ? (<SettingsRow icon={Mic} label={t("providerswitcher.realtimeVoiceRowLabel", {
                    defaultValue: "Cartesia (realtime)",
                })} description={servingAxes.runtime === "remote"
                    ? t("providerswitcher.remoteRealtimeVoiceRowDescription", {
                        defaultValue: "Connection not verified. Cartesia handles speech recognition and playback when connected. Your agent stays on the remote host.",
                    })
                    : t("providerswitcher.realtimeVoiceRowDescription", {
                        defaultValue: "Connection not verified. Cartesia handles speech recognition and playback when connected. Your agent stays on this device.",
                    })} control={<span className="text-xs text-muted">
      ...
      {settingsContentReady && !selection.cloudRuntimeLocked ? (<SettingsGroup title={t("providerswitcher.advancedGroupTitle", {
                defaultValue: "Advanced",
            })} bare>
          <AdvancedSettingsDisclosure title={t("providerswitcher.advancedDisclosureTitle", {
                defaultValue: "Custom providers & model overrides",
            })} lazy>
            <div className="flex flex-col gap-3">
              {keyEntries.length > 0 ? (<div className="flex flex-wrap gap-2">
                  {keyEntries.map(renderChip)}
                </div>) : null}

              {selectedPanelProvider ? (<ApiKeyPanel selectedProvider={selectedPanelProvider} panelLabel={apiKeyPanelLabel} visibleProviderPanelId={visibleProviderPanelId} resolvedSelectedId={resolvedSelectedId} cloudCallsDisabled={selection.cloudCallsDisabled} onSwitchProvider={onSwitchProvider} pluginSaving={pluginSaving} pluginSaveSuccess={pluginSaveSuccess} handlePluginConfigSave={handlePluginConfigSave} loadPlugins={loadPlugins}/>) : null}
 ...
 */
export function reconcileProviderEntriesWithServingAxes(entries: ProviderListEntry[], axes: ServingAxes): ProviderListEntry[] {
    ...
    return entries.map((entry) => {
        ...
        const current = axes.inference === "external" &&
            entry.category === "key" &&
            entry.id.trim().toLowerCase() === providerId;
        const selectedInferenceTile = entry.category === "local" || entry.category === "cloud";
        return {
            ...entry,
            current,
            ...(selectedInferenceTile && entry.status.label === "Active"
                ? {
                    status: {
                        tone: "muted" as const,
                        label: axes.inference === "unknown" ? "Unconfirmed" : "Not serving",
                    },
                }
                : {}),
        };
 ...
 */
export function resolveActiveChatCatalogProvider(resolvedSelectedId: string | null, elizaCloudConnected: boolean): "elizacloud" | "cerebras" | "claude-chat" | undefined {
    if (resolvedSelectedId === "__cloud__") {
        return elizaCloudConnected ? "elizacloud" : undefined;
    }
    if (resolvedSelectedId === "cerebras")
        return "cerebras";
    if (resolvedSelectedId === "anthropic")
        return "claude-chat";
    return undefined;
}
/**
 ...
 * @internal Exported for testing only.
 */
export function ActiveProviderSummary({ entry, t, }: {
    ...
    const codingAgentsOnly = entry.category === "subscription" && entry.id !== "openai-subscription";
    return (<SettingsRow label={<span className="flex items-center gap-2">
          <Icon className="size-[18px] shrink-0 text-accent" aria-hidden/>
          {entry.label}
        </span>} description={codingAgentsOnly
            ? t("providerswitcher.codingSubscriptionChatNote", {
                defaultValue: "Powers coding agents & workflows only — chat replies keep using your selected Intelligence provider (Cloud or Local).",
            })
            : undefined} control={<span className="text-xs text-accent">
          {codingAgentsOnly
