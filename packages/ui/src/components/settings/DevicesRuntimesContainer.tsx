...
import { RemoteCloudRequestError } from "../../api/remote-control-cloud-client";
import { RemoteControlAuthenticationRequiredError } from "../../api/remote-control-cloud-client";
...
import { getRemoteTargetIdentity } from "../../platform/remote-target";
import { getRemoteTargetStatus } from "../../platform/remote-target";
import { getSshRuntimeStatus } from "../../platform/ssh-runtime";
import { isElectrobunRuntime } from "../../bridge/electrobun-runtime";
import { loadAgentProfileRegistry } from "../../state";
import { readRemoteTargetPairingChallenge } from "../../platform/remote-target";
import { removeProfileWithoutStaleSelection as removeProfileCanonically } from "../../platform/runtime-management";
...
import { type ControllerPairingClaimView } from "./DevicesRuntimesSection";
import { type DevicePairingView } from "./DevicesRuntimesSection";
import { type DeviceRuntimeTarget } from "./DevicesRuntimesSection";
import { type LinuxRemoteTargetView } from "./DevicesRuntimesSection";
import { type RemoteControllerPublicIdentity } from "@elizaos/core/contracts/remote-control";
import { type RemoteHostDirectory } from "../../api/remote-control-cloud-client";
import { type RemoteHostSummary } from "../../api/remote-control-cloud-client";
import { type RemotePairingClaimReceipt } from "../../api/remote-control-cloud-client";
import { type RemoteSessionSummary } from "../../api/remote-control-cloud-client";
import { type SshConnectInput } from "./DevicesRuntimesSection";
import { type SshHostInspection } from "../../platform/ssh-runtime";
import { type SshRuntimeStatus } from "../../platform/ssh-runtime";
...
function isCloudAuthenticationRequired(cause: unknown): boolean {
    return (cause instanceof RemoteControlAuthenticationRequiredError ||
        (cause instanceof RemoteCloudRequestError && cause.status === 401));
}
async function startSshWithCredentialCleanup(runtimeId: string, input: SshConnectInput, dependencies: {
    ...
    try {
        ...
        if (input.accessToken) {
            try {
                ...
                throw new AggregateError([cause, cleanupCause], "SSH connection failed and its stored credential could not be removed.", { cause });
            }
        }
...
function controllerClaimView(claim: RemotePairingClaimReceipt): ControllerPairingClaimView {
    return {
...
function restoredRelayProfile(host: RemoteHostSummary, session: RemoteSessionSummary, controller: RemoteControllerPublicIdentity): Omit<AgentProfile, "id" | "createdAt"> {
    if (session.targetKeyId !== host.runtimeKeyId) {
        throw new Error("Cloud session target key does not match the enrolled remote host.");
    }
    return {
        kind: "remote",
...
function profileTarget(profile: AgentProfile, activeId: string | null, sshStatuses: ReadonlyMap<string, SshRuntimeStatus>, directory: RemoteHostDirectory | null, sessions: ReadonlyMap<string, RemoteSessionSummary[]>): DeviceRuntimeTarget {
    ...
    if (profile.connectionMode === "relay" && profile.remoteRelay) {
        ...
        const host = directory?.hosts.find((item) => item.id === relay.targetRuntimeId);
        const session = (sessions.get(relay.targetRuntimeId) ?? []).find((item) => item.id === relay.sessionId);
        const invalid = Boolean(!Number.isSafeInteger(relay.targetCreatedAt) ||
        ...
        const offline = !invalid && Boolean(directory && host?.status === "offline");
        return {
            id: profile.id,
            label: profile.label,
...
function hostTarget(host: RemoteHostSummary, sessions: ReadonlyMap<string, RemoteSessionSummary[]>, controller: RemoteControllerPublicIdentity | null): DeviceRuntimeTarget {
    const active = (sessions.get(host.id) ?? []).find((session) => session.status === "active");
    const activeHere = (sessions.get(host.id) ?? []).find((session) => session.status === "active" &&
...
function relayAuthorityFromProfile(profile: AgentProfile): RelayRevocationAuthority | null {
    const relay = profile.remoteRelay;
    if (profile.connectionMode !== "relay" || !relay)
        return null;
    return {
...
function resolveRelayRevocationAuthority(targetId: string, profiles: readonly AgentProfile[], sessions: ReadonlyMap<string, RemoteSessionSummary[]>, controller: RemoteControllerPublicIdentity | null): RelayRevocationAuthority | null {
    const directProfile = profiles.find((profile) => profile.id === targetId);
    if (directProfile)
        return relayAuthorityFromProfile(directProfile);
    if (!targetId.startsWith("host:") || !controller)
        return null;
    const hostId = targetId.slice("host:".length);
    const session = (sessions.get(hostId) ?? []).find((candidate) => candidate.status === "active" &&
        candidate.controllerDeviceId === controller.deviceId &&
        candidate.controllerKeyId === controller.keyId);
    if (!session)
        return null;
    const profile = profiles.find((candidate) => candidate.remoteRelay?.sessionId === session.id) ?? null;
    return {
        sessionId: session.id,
        ownerId: session.ownerId,
...
function buildRuntimeTargets(registry: AgentProfileRegistry, sshStatuses: ReadonlyMap<string, SshRuntimeStatus>, directory: RemoteHostDirectory | null, sessions: ReadonlyMap<string, RemoteSessionSummary[]>, controller: RemoteControllerPublicIdentity | null): DeviceRuntimeTarget[] {
    ...
    const localProfiles = registry.profiles.filter((profile) => profile.kind === "local");
    const visibleLocalProfile = localProfiles.find((profile) => profile.id === registry.activeProfileId) ??
        localProfiles[localProfiles.length - 1];
    const visibleProfiles = registry.profiles.filter((profile) => profile.kind !== "local" || profile.id === visibleLocalProfile?.id);
    const profiles = visibleProfiles.map((profile) => profileTarget(profile, registry.activeProfileId, sshStatuses, directory, sessions));
    const representedHostIds = new Set(visibleProfiles.flatMap((profile) => profile.remoteRelay ? [profile.remoteRelay.targetRuntimeId] : []));
...
async function revokeRelayAuthorityWithCleanup(authority: RelayRevocationAuthority, dependencies: {
    ...
    if (authority.profile)
        dependencies.removeProfile(authority.profile.id);
}
...
async function removeRuntimeWithAuthority(profile: AgentProfile, dependencies: RuntimeRemovalDependencies): Promise<void> {
...
async function revokeLinuxHostCloudFirst(hostId: string, dependencies: {
    ...
    if (!(await dependencies.finalizeLocal(hostId))) {
        throw new Error("Cloud revoked the Linux host, but local credential cleanup needs to be retried.");
    }
}
export function DevicesRuntimesContainer({ className, }: {
    className?: string;
}) {
    ...
    const [controller, setController] = useState<RemoteControllerPublicIdentity | null>(null);
    const [sessions, setSessions] = useState<Map<string, RemoteSessionSummary[]>>(() => new Map());
    const [sshStatuses, setSshStatuses] = useState<Map<string, SshRuntimeStatus>>(() => new Map());
    const [linuxTarget, setLinuxTarget] = useState<LinuxRemoteTargetView | null>(null);
    const [pairing, setPairing] = useState<DevicePairingView | null>(null);
    const [controllerClaim, setControllerClaim] = useState<ControllerPairingClaimView | null>(null);
    const [sshInspection, setSshInspection] = useState<SshHostInspection | null>(null);
    const pendingSshId = useRef(crypto.randomUUID());
    const [busy, setBusy] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [cloudState, setCloudState] = useState<"loading" | "available" | "signed-out" | "error">("loading");
    const refresh = useCallback(async () => {
        setError(null);
        setCloudState("loading");
        const nextRegistry = loadAgentProfileRegistry();
        setRegistry(nextRegistry);
        const sshProfiles = nextRegistry.profiles.filter((profile) => profile.connectionMode === "ssh");
        const statuses = await Promise.all(sshProfiles.map(async (profile) => [profile.id, await getSshRuntimeStatus(profile.id)] as const));
        ...
        try {
            ...
            await Promise.all(nextDirectory.hosts.map(async (host) => {
                nextSessions.set(host.id, await cloud.listSessions(host.id, nextDirectory.ownerId));
            }));
            ...
            for (const host of nextDirectory.hosts) {
                for (const session of nextSessions.get(host.id) ?? []) {
                    if (session.status !== "active")
                        continue;
                    if (session.controllerDeviceId !== nextController.deviceId ||
                        session.controllerKeyId !== nextController.keyId) {
                        continue;
                    }
                    const existing = nextRegistry.profiles.some((profile) => profile.remoteRelay?.sessionId === session.id);
                    if (existing)
                        continue;
    ...
    const targets = useMemo(() => {
        return buildRuntimeTargets(registry, sshStatuses, directory, sessions, controller);
    }, [controller, directory, registry, sessions, sshStatuses]);
    const onSelect = (id: string) => run(async () => {
        const result = switchRuntimeNonDestructive(id);
        if (!result.ok)
            throw new Error("That runtime could not be selected. Check its connection and try again.");
    });
    const onPair = (targetId: string, code: string) => run(async () => {
        ...
        setControllerClaim(controllerClaimView(outcome.data?.claim as RemotePairingClaimReceipt));
    });
    const onRevoke = (targetId: string) => run(async () => {
        const outcome = await executeRuntimeManagementCommand({
            op: "revoke",
            targetId,
        });
    ...
    const onRemove = (id: string) => run(async () => {
        const outcome = await executeRuntimeManagementCommand({
            op: "remove",
            runtimeId: id,
        });
    ...
    const onRetry = (id: string) => run(async () => {
        const outcome = await executeRuntimeManagementCommand({
            op: "retry",
            runtimeId: id,
        });
    ...
    const onConnectSsh = (input: SshConnectInput) => run(async () => {
    ...
    const onEnrollLinuxTarget = (managedNetwork: boolean) => run(async () => {
    ...
    const onCreateTargetPairing = () => run(async () => {
        const outcome = await executeRuntimeManagementCommand({
            op: "create_pairing",
        });
        if (!outcome.ok)
        ...
        setPairing({
            ...
            hostLabel: linuxTarget?.platform === "macos"
                ? "This Mac"
    ...
    const onConfirmTargetPairing = (sessionId: string) => run(async () => {
        const outcome = await executeRuntimeManagementCommand({
            op: "confirm_pairing",
            sessionId,
        });
    ...
    const onDenyTargetPairing = (sessionId: string) => run(async () => {
        const outcome = await executeRuntimeManagementCommand({
            op: "deny_pairing",
            sessionId,
        });
    ...
    const onClaimControllerPairing = useCallback((input: {
        sessionId: string;
        code: string;
    }) => run(async () => {
        const outcome = await executeRuntimeManagementCommand({
    ...
    useEffect(() => subscribeRemoteControllerPairingIntents((intent) => onClaimControllerPairing({
        sessionId: intent.sessionId,
        code: intent.code,
    })), [onClaimControllerPairing]);
    const onSetLinuxTargetRunning = (running: boolean) => run(async () => {
        const outcome = await executeRuntimeManagementCommand({
    ...
    const onRevokeLinuxTarget = () => run(async () => {
        const outcome = await executeRuntimeManagementCommand({
            op: "revoke_host",
        });
        if (!outcome.ok)
    ...
    return (<DevicesRuntimesSection className={className} targets={targets} pairing={pairing} controllerClaim={controllerClaim} sshInspection={sshInspection} linuxTarget={linuxTarget} busy={busy} error={error} cloudState={cloudState} onRefresh={() => run(refresh)} onSelect={onSelect} onRetry={onRetry} onPair={onPair} onRevoke={onRevoke} onRemove={onRemove} onInspectSsh={onInspectSsh} onConnectSsh={onConnectSsh} onEnrollLinuxTarget={onEnrollLinuxTarget} onCreateTargetPairing={onCreateTargetPairing} onConfirmTargetPairing={onConfirmTargetPairing} onDenyTargetPairing={onDenyTargetPairing} onSetLinuxTargetRunning={onSetLinuxTargetRunning} onRevokeLinuxTarget={onRevokeLinuxTarget}/>);
}
