...
export function TranscriptPlayer({ transcript, audioUrl, initialSeekMs, className, }: TranscriptPlayerProps): React.JSX.Element {
    const audio = useAudioElement();
    const durationMs = audio.durationMs || transcript.durationMs;
    React.useEffect(() => {
        if (!audioUrl || initialSeekMs === undefined)
            return;
    ...
    return (<div className={cn("flex flex-col gap-4", className)}>
      {audioUrl ? (
        ...
        <audio ref={audio.audioRef} src={audioUrl} preload="metadata"/>) : null}

      {audioUrl ? (<div className="flex items-center gap-3">
          <Button variant="surfaceAccent" size="icon" shape="circle" data-testid="transcript-play" aria-label={audio.playing ? "pause" : "play"} onClick={audio.toggle} className="shrink-0">
            {audio.playing ? (<Pause className="size-5" aria-hidden/>) : (<Play className="size-5 translate-x-px" aria-hidden/>)}
          </Button>
          <Input type="range" variant="nativeRange" data-testid="transcript-scrub" aria-label="seek" min={0} max={Math.max(1, durationMs)} value={Math.min(audio.currentMs, durationMs)} onChange={(e) => audio.seekMs(Number(e.target.value))} className="flex-1"/>
          <span className="shrink-0 tabular-nums text-xs text-muted">
            {formatMs(audio.currentMs)} / {formatMs(durationMs)}
          </span>
        </div>) : null}

      <TranscriptBody transcript={transcript} currentTimeMs={audio.currentMs} onSeekMs={audioUrl ? audio.seekMs : undefined}/>
    </div>);
}
