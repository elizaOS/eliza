...
import { type SpeakerNameEvidenceSource } from "@elizaos/core/speaker-name-inference";
...
function decisionLabel(attribution: SpeakerNameAttribution): string {
    ...
    if (attribution.resolution === "needs_confirmation")
        return "Review";
    if (attribution.resolution === "withheld")
        return "Withheld";
    return "Unknown";
}
function provenanceLabels(attribution: SpeakerNameAttribution): string[] {
    return [
        ...new Set(attribution.provenance.map((item) => SOURCE_LABELS[item.source])),
    ];
}
export function SpeakerNameAttributionBadge({ attribution, className, }: {
    ...
    const detail = [
        ...
        .filter(Boolean)
        .join("; ");
    return (<StatusBadge data-testid="speaker-name-attribution" data-resolution={attribution.resolution} role="note" className={className} aria-label={detail} title={detail} tone={attribution.resolution === "confirmed" ? "info" : "muted"} label={<>
          <span>{decision}</span>
