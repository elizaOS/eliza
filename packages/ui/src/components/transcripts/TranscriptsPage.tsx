...
export function TranscriptsPage(): React.JSX.Element {
    const [transcripts, setTranscripts] = React.useState<MeetingAwareTranscriptSummary[]>([]);
    ...
    const loadTranscript = React.useCallback((id: string, showLoading = false) => {
        if (showLoading)
        ...
        return client
            ...
            setSelected(null);
            setSelectedError(e instanceof Error ? e.message : "Transcript is unavailable");
        })
            .finally(() => {
            if (showLoading)
    ...
    const refresh = React.useCallback(async () => {
        ...
        if (meetingsResult.status === "fulfilled") {
            ...
            setMeetingError(meetingsResult.reason instanceof Error
    ...
    const onUpdatePrivacy = React.useCallback((sharing: Partial<TranscriptCaptureSharingState>) => {
        if (!selectedId)
            return;
        ...
        client
            ...
            setPrivacyError(e instanceof Error
                ? e.message
    ...
    const onDeleteSourceAudio = React.useCallback(() => {
        ...
        if (typeof globalThis.confirm === "function" &&
            !globalThis.confirm("Delete the retained source audio? The transcript text will remain, but audio replay cannot be restored.")) {
            return;
        }
        setPrivacySaving(true);
        setPrivacyError(null);
        client
            ...
            setPrivacyError(e instanceof Error ? e.message : "Failed to delete source audio");
        })
            .finally(() => setPrivacySaving(false));
    }, [refresh, selectedId]);
    ...
    return (<div className="flex h-full min-h-0 w-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-hidden">
        <TranscriptsView transcripts={transcripts} selectedId={selectedId} selected={selected} onSelect={onSelect} loading={loading} error={error} selectedLoading={selectedLoading} selectedError={selectedError} onUpdatePrivacy={onUpdatePrivacy} onDeleteSourceAudio={onDeleteSourceAudio} privacySaving={privacySaving} privacyError={privacyError} activeMeetings={activeMeetings} onJoinMeeting={onJoinMeeting} onStopMeeting={onStopMeeting} joiningMeeting={joiningMeeting} meetingError={meetingError}/>
      </div>
    </div>);
}
