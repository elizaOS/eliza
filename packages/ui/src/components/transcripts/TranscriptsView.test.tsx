...
describe("TranscriptsView", () => {
    it("lists recordings and selects on click", () => {
        ...
        render(<TranscriptsView transcripts={summaries} selectedId={null} selected={null} onSelect={onSelect}/>);
        expect(screen.getByTestId("transcript-row-t1").textContent).toContain("Standup");
        expect(screen.getByTestId("transcript-row-t1").textContent).toContain("2 speakers");
        // processing status surfaces; ready does not add a label.
        expect(screen.getByTestId("transcript-row-t2").textContent).toContain("Processing");
    ...
    it("shows the player for the selected transcript", () => {
        render(<TranscriptsView transcripts={summaries} selectedId="t1" selected={selected} onSelect={vi.fn()}/>);
        expect(screen.getByTestId("transcript-row-t1").getAttribute("data-active")).toBe("true");
    ...
    it("shows an empty hint when there are no recordings", () => {
        render(<TranscriptsView transcripts={[]} selectedId={null} selected={null} onSelect={vi.fn()}/>);
    ...
    it("renders platform badge, participant count, and LIVE on a live meeting row", () => {
        render(<TranscriptsView transcripts={[meetingSummary]} selectedId={null} selected={null} onSelect={vi.fn()}/>);
        expect(screen.getByTestId("transcript-platform-m1").textContent).toBe("Google Meet");
        expect(screen.getByTestId("transcript-participants-m1").textContent).toBe("2 participants");
        expect(screen.getByTestId("transcript-live-m1").textContent).toContain("LIVE");
        // no "Recording" status label when the LIVE dot already says so
        expect(screen.getByTestId("transcript-row-m1").textContent).not.toContain("Recording");
    });
    it("omits meeting affordances on an archived meeting row", () => {
        render(<TranscriptsView transcripts={[{ ...meetingSummary, status: "ready" }]} selectedId={null} selected={null} onSelect={vi.fn()}/>);
    ...
    it("shows meeting metadata + the player on an archived meeting detail", () => {
        ...
        render(<TranscriptsView transcripts={[{ ...meetingSummary, status: "ready" }]} selectedId="m1" selected={archivedMeeting} onSelect={vi.fn()}/>);
        expect(screen.getByTestId("meeting-detail-platform").textContent).toBe("Google Meet");
        expect(screen.getByTestId("meeting-detail-participants").textContent).toBe("Alice, Bob");
    ...
    it("shows capture, consent, retention, and sharing states from meeting metadata", () => {
        ...
        expect(state.textContent).toContain("Retention: Audio deleted, transcript retained");
    ...
    it("exposes independent artifact controls and source-audio deletion", () => {
        ...
        render(<TranscriptsView transcripts={[{ ...meetingSummary, status: "ready" }]} selectedId="m1" selected={archivedMeeting} onSelect={vi.fn()} onUpdatePrivacy={onUpdatePrivacy} onDeleteSourceAudio={onDeleteSourceAudio}/>);
        expect(screen.getByTestId("transcript-artifact-privacy-controls")).toBeTruthy();
        ...
        expect(screen.getByLabelText("Generated artifacts visibility")).toBeTruthy();
        fireEvent.click(screen.getByRole("button", { name: "Delete source audio" }));
    ...
    it("renders distinct selected loading, unavailable, and mutation error states", () => {
        const { rerender } = render(<TranscriptsView transcripts={summaries} selectedId="t1" selected={null} selectedLoading onSelect={vi.fn()}/>);
        expect(screen.getByTestId("transcripts-detail-loading")).toBeTruthy();
        rerender(<TranscriptsView transcripts={summaries} selectedId="t1" selected={null} selectedError="Transcript unavailable" onSelect={vi.fn()}/>);
        expect(screen.getByTestId("transcripts-detail-error").textContent).toContain("Transcript unavailable");
    });
    it("renders the join bar and forwards a join request", () => {
        const onJoinMeeting = vi.fn();
        render(<TranscriptsView transcripts={[]} selectedId={null} selected={null} onSelect={vi.fn()} activeMeetings={[]} onJoinMeeting={onJoinMeeting} onStopMeeting={vi.fn()}/>);
