...
afterEach(cleanup);
function renderBar(props: Partial<React.ComponentProps<typeof MeetingJoinBar>> = {}) {
    const onJoin = vi.fn();
    const onStop = vi.fn();
    render(<MeetingJoinBar activeMeetings={[]} onJoin={onJoin} onStop={onStop} {...props}/>);
...
describe("MeetingJoinBar", () => {
    it("disables submit and shows the invalid hint for an unrecognized URL", () => {
        ...
        expect((screen.getByTestId("meeting-join-submit") as HTMLButtonElement).disabled).toBe(true);
    });
    ...
    it.each([
        ...
        expect(screen.getByTestId("meeting-platform-hint").textContent).toContain(label);
        expect(screen.queryByTestId("meeting-url-invalid")).toBeNull();
        expect((screen.getByTestId("meeting-join-submit") as HTMLButtonElement).disabled).toBe(false);
    });
    it("submits the canonical URL, platform, and optional bot name", () => {
        ...
        expect((screen.getByTestId("meeting-url-input") as HTMLInputElement).value).toBe("");
    });
    ...
    it("lists active sessions with a Stop control", () => {
        ...
        expect(screen.getByTestId("active-meeting-m1").textContent).toContain("Zoom");
        expect(screen.getByTestId("active-meeting-m1").textContent).toContain("In meeting");
    ...
    it("exposes the live meeting form through the agent bridge", async () => {
        const onJoin = vi.fn();
        render(<AgentSurfaceProvider viewId="transcripts" viewType="gui">
        <MeetingJoinBar activeMeetings={[]} onJoin={onJoin} onStop={vi.fn()}/>
      </AgentSurfaceProvider>);
        const registry = getViewRegistry("transcripts", "gui");
        if (!registry)
            throw new Error("transcripts registry missing");
        expect(handleAgentSurfaceCapability(registry, "list-elements", undefined)).toEqual(expect.arrayContaining([
        ...
        await act(async () => {
            expect(handleAgentSurfaceCapability(registry, "agent-fill", {
                id: "meeting-url",
                value: "https://meet.google.com/abc-defg-hij",
            })).toMatchObject({ ok: true });
        });
        await act(async () => {
            expect(handleAgentSurfaceCapability(registry, "agent-click", {
                id: "meeting-join",
            })).toMatchObject({ ok: true });
        });
        expect(onJoin).toHaveBeenCalledWith({
