...
const clientMock = vi.hoisted(() => ({
    ...
    stopMeeting: vi.fn(),
    onWsEvent: vi.fn((_type: string, _handler: (data: unknown) => void): (() => void) => () => { }),
}));
...
const parseMeetingStatusEventMock = vi.hoisted(() => vi.fn((_data: unknown) => null as unknown));
...
describe("TranscriptsPage active-meetings fetch failure (three-state)", () => {
    ...
    it("surfaces a failed active-meetings fetch instead of a healthy empty strip, while still rendering transcripts", async () => {
        ...
        clientMock.listMeetings.mockRejectedValue(new Error("meetings backend unreachable"));
        render(<TranscriptsPage />);
        // The meetings-fetch failure is surfaced (not swallowed into empty state).
        await waitFor(() => {
            expect(screen.getByRole("alert").textContent).toContain("meetings backend unreachable");
        });
    ...
    it("falls back to a generic message when the meetings rejection is not an Error", async () => {
        ...
        render(<TranscriptsPage />);
        await waitFor(() => {
            expect(screen.getByRole("alert").textContent).toContain("Failed to load active meetings");
        });
    });
    ...
    it("clears a stale meetings-error banner once a later refresh succeeds", async () => {
        ...
        clientMock.onWsEvent.mockImplementation((_type: string, handler: (data: unknown) => void) => {
            wsHandler = handler;
            return () => { };
        });
