 ...
 */
import { MEETING_PLATFORMS } from "@elizaos/core/meetings";
 ...
 */
export function applyMeetingTranscriptEvent(state: LiveTranscriptState, event: MeetingTranscriptEvent): LiveTranscriptState {
    const seen = new Set(state.confirmed.map((s) => s.id));
    const appended = event.confirmed.filter((s) => !seen.has(s.id));
    return {
        confirmed: appended.length > 0 ? [...state.confirmed, ...appended] : state.confirmed,
 ...
 */
export function applyPolledTranscript(state: LiveTranscriptState, transcript: Transcript): LiveTranscriptState {
    if (transcript.segments.length <= state.confirmed.length)
        return state;
...
export function meetingTranscriptMeta(transcript: {
    ...
    const participants: MeetingParticipant[] = Array.isArray(raw)
        ? raw.filter((p): p is MeetingParticipant => typeof p === "object" &&
            p !== null &&
            typeof (p as {
                displayName?: unknown;
            }).displayName === "string")
