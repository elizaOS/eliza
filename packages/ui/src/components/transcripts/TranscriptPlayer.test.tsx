...
describe("TranscriptPlayer", () => {
    it("renders transport + the transcript words when audio is present", () => {
        ...
        expect(screen.getByTestId("transcript-play").getAttribute("aria-label")).toBe("play");
    ...
    it("seeks once to an anchored document-search offset", () => {
        render(<TranscriptPlayer transcript={transcript} audioUrl="/a.wav" initialSeekMs={1250}/>);
        const audio = document.querySelector("audio") as HTMLAudioElement;
        expect(audio.currentTime).toBeCloseTo(1.25, 3);
        expect((screen.getByTestId("transcript-scrub") as HTMLInputElement).value).toBe("1250");
    });
