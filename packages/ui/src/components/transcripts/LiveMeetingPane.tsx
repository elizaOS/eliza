...
import { applyMeetingTranscriptEvent } from "./meeting-live";
import { applyPolledTranscript } from "./meeting-live";
import { client } from "../../api/client";
import { cn } from "../../lib/utils";
import { parseMeetingTranscriptEvent } from "../../api/client-meetings";
import { type LiveTranscriptState } from "./meeting-live";
...
function SegmentBlock({ segment, muted, testId, }: {
    segment: TranscriptSegment;
    muted: boolean;
    testId: string;
}): React.JSX.Element {
    return (<div data-testid={testId} className={cn(muted && "text-muted")}>
      {segment.speakerLabel || segment.speakerNameAttribution ? (<div className="mb-0.5 flex flex-wrap items-center gap-1.5 text-xs font-medium text-muted">
          {segment.speakerLabel ? <span>{segment.speakerLabel}</span> : null}
          <SpeakerNameAttributionBadge attribution={segment.speakerNameAttribution}/>
        </div>) : null}
      <p className="leading-relaxed">{segment.text}</p>
    </div>);
}
export function LiveMeetingPane({ transcript, className, }: LiveMeetingPaneProps): React.JSX.Element {
    ...
    React.useEffect(() => {
        return client.onWsEvent("meeting-transcript", (data) => {
            ...
            if (!event || event.transcriptId !== transcriptId)
                return;
    ...
    React.useEffect(() => {
        const timer = setInterval(() => {
            if (Date.now() - lastWsEventAtRef.current < WS_FRESHNESS_MS)
                return;
            client
                .getTranscript(transcriptId)
                .then((r) => setLive((prev) => applyPolledTranscript(prev, r.transcript)))
                .catch(() => {
    ...
    return (<div ref={scrollRef} onScroll={onScroll} data-testid="live-meeting-pane" className={cn("min-h-0 flex-1 space-y-4 overflow-y-auto text-txt", className)} aria-live="polite">
      {segmentCount === 0 ? (<p className="text-sm text-muted">Listening…</p>) : null}
      {live.confirmed.map((seg, i) => (<SegmentBlock key={seg.id} segment={seg} muted={false} testId={`live-confirmed-${i}`}/>))}
      {live.pending.map((seg, i) => (<SegmentBlock key={`pending-${seg.id}`} segment={seg} muted testId={`live-pending-${i}`}/>))}
    </div>);
}
