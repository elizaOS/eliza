...
function isRecordingSession(session: MeetingSession): boolean {
    return (!TERMINAL_MEETING_STATUSES.has(session.status) &&
...
export function LiveMeetingPage(): React.JSX.Element {
    ...
    const [liveTranscripts, setLiveTranscripts] = React.useState<Record<string, Transcript>>({});
    // Per-transcript load errors (codex P2): a broken GET /api/transcripts/:id
    // must read as an error, not an indefinite "Connecting…".
    const [transcriptErrors, setTranscriptErrors] = React.useState<Record<string, string>>({});
    ...
    const loadLiveTranscript = React.useCallback((transcriptId: string, force = false) => {
        return client
            .getTranscript(transcriptId)
            .then((r) => {
            setLiveTranscripts((prev) => !force && prev[transcriptId]
                ? prev
            ...
            setTranscriptErrors((prev) => ({
                ...prev,
                [transcriptId]: e instanceof Error ? e.message : "Failed to load transcript",
            }));
        });
    }, []);
    ...
    const refresh = React.useCallback(async () => {
        try {
            ...
            setMeetingError(e instanceof Error ? e.message : "Failed to load active meetings");
        }
    }, []);
    React.useEffect(() => {
        void refresh();
    ...
    React.useEffect(() => {
        return client.onWsEvent("meeting-status", (data) => {
            ...
            if (!event)
                return;
            const { session } = event;
            if (TERMINAL_MEETING_STATUSES.has(session.status) &&
    ...
    const onJoinMeeting = React.useCallback((input: MeetingJoinRequest) => {
        setJoiningMeeting(true);
        setMeetingError(null);
        client
            .requestMeetingBot(input)
            .then(() => refresh())
            .catch((e) => setMeetingError(e instanceof Error ? e.message : "Failed to join meeting"))
            .finally(() => setJoiningMeeting(false));
    }, [refresh]);
    const onStopMeeting = React.useCallback((sessionId: string) => {
        setMeetingError(null);
        client
            .stopMeeting(sessionId)
            .then(() => refresh())
            .catch((e) => setMeetingError(e instanceof Error ? e.message : "Failed to stop meeting"));
    }, [refresh]);
    ...
    return (<div className="flex h-full min-h-0 w-full flex-col">
      <ShellViewAgentSurface viewId="transcripts">
        <div data-testid="live-meeting-page" className="flex h-full min-h-0 w-full flex-col gap-4">
          <MeetingJoinBar activeMeetings={activeMeetings} onJoin={onJoinMeeting} onStop={onStopMeeting} joining={joiningMeeting} error={meetingError}/>
          <div className="min-h-0 flex-1 overflow-y-auto">
            {recordingSessions.length === 0 ? (<div data-testid="live-meeting-empty" className="grid h-full place-items-center px-6 text-center">
                ...
                const transcriptError = session.transcriptId
                    ...
                    : undefined;
                return (<section key={session.id} data-testid={`live-meeting-${session.id}`} className="flex min-h-0 flex-col gap-2">
                      <h2 className="text-sm font-semibold text-txt">
                        {transcript?.title ?? session.botName}
                      </h2>
                      {transcript ? (<LiveMeetingPane transcript={transcript}/>) : transcriptError ? (<p data-testid={`live-meeting-error-${session.id}`} className="text-sm text-muted" role="alert">
                          {transcriptError}
