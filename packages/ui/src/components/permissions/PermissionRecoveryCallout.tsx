...
export function PermissionRecoveryCallout({ permission, title, description, retryLabel = "Try again", settingsLabel = "Open Settings", settingsErrorLabel = "Couldn’t open Settings. Open System Settings manually, then re-check.", openingLabel = "Opening…", checkingLabel = "Checking…", onOpenSettings, onRetry, className, testId = "permission-recovery-callout", }: PermissionRecoveryCalloutProps): React.JSX.Element {
    ...
    return (<Alert variant="warningStrong" data-testid={testId} className={cn("p-3 text-left", className)}>
      ...
      {openError ? (<p className="mt-2 text-sm leading-snug text-danger" data-testid={`${testId}-settings-error`}>
          {settingsErrorLabel}
        </p>) : null}
      <div className="mt-3 flex flex-wrap gap-2">
        <Button type="button" size="sm" variant="default" onClick={() => void handleOpenSettings()} disabled={opening} data-testid={`${testId}-settings`}>
          {opening ? openingLabel : settingsLabel}
        </Button>
        {onRetry ? (<Button type="button" size="sm" variant="outline" onClick={() => void handleRetry()} disabled={retrying} data-testid={`${testId}-retry`}>
