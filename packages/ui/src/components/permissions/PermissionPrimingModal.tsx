...
import { appNameInterpolationVars } from "../../config/branding";
 ...
 */
export function PermissionPrimingModal(props: PermissionPrimingModalProps): React.JSX.Element {
    return props.controllerOverride ? (<PermissionPrimingModalView controller={props.controllerOverride} open={props.open} onComplete={props.onComplete}/>) : (<PermissionPrimingModalLive {...props}/>);
}
function PermissionPrimingModalLive({ ids, open, onComplete, }: PermissionPrimingModalProps): React.JSX.Element {
    const controller = usePermissionPriming(ids);
    return (<PermissionPrimingModalView controller={controller} open={open} onComplete={onComplete}/>);
}
function PermissionPrimingModalView({ controller, open, onComplete, }: {
    controller: PermissionPrimingController;
    open: boolean;
    onComplete: () => void;
}): React.JSX.Element {
    const { active, currentStep, totalSteps, ready, done, request, skip, openSettings, recheck, skipAll, } = controller;
    ...
    return (<Dialog open={open} onOpenChange={(next) => {
      ...
      <DialogContent showCloseButton data-testid="permission-priming-modal" aria-describedby="permission-priming-subtitle" 
    ...
    className="z-[9500]" overlayClassName="z-[9490]">
        <DialogHeader>
        ...
        {!ready ? (<div className="py-8 text-center text-sm text-muted" data-testid="permission-priming-loading">
            {t("permissionpriming.checking", {
                defaultValue: "Checking permissions…",
            })}
          </div>) : active ? (<PrimingCard key={active.id} id={active.id} status={active.status} canRequest={active.canRequest} requesting={active.requesting} requestError={active.requestError} recheckError={active.recheckError} currentStep={currentStep} totalSteps={totalSteps} onEnable={() => void request(active.id)} onSkip={() => skip(active.id)} onOpenSettings={() => openSettings(active.id)} onRecheck={() => void recheck(active.id)} onSkipAll={skipAll} cloudOnly={branding.cloudOnly === true}/>) : (<div className="py-8 text-center text-sm text-muted" data-testid="permission-priming-done">
...
function PrimingCard({ id, status, canRequest, requesting, requestError, recheckError, currentStep, totalSteps, onEnable, onSkip, onOpenSettings, onRecheck, onSkipAll, cloudOnly, }: PrimingCardProps): React.JSX.Element {
    const t = useAppSelector((s) => s.t);
    ...
    const rationale = copy
        ? t(id === "microphone" && cloudOnly
            ? "permissionpriming.microphone.cloudRationale"
            : copy.rationaleKey, {
            defaultValue: id === "microphone" && cloudOnly
        ...
        : t("permissionpriming.generic.rationale", {
            defaultValue: "Enable this permission so I can complete the request you just made.",
            permission: fallbackName,
        });
    const denied = status === "denied";
    const needsRecovery = denied || requestError || recheckError;
    return (<div className="flex flex-col gap-4" data-testid={`priming-card-${id}`}>
      ...
      {needsRecovery ? (<PermissionRecoveryCallout permission={id} title={requestError
                ...
                ? t("permissionpriming.requestFailedDescription", {
                    defaultValue: "The system request failed. Try again, or open Settings and re-check.",
                })
                : recheckError
                    ? t("permissionpriming.recheckFailedDescription", {
                        defaultValue: "Eliza couldn’t re-check the system setting. Try again, or open Settings and confirm it is enabled.",
                    })
                    : canRequest
                        ? t("permissionpriming.deniedRetry", {
                            defaultValue: "You can try again, or turn it on later in Settings.",
                        })
                        : t("permissionpriming.deniedSettings", {
                            defaultValue: "To enable it, open Settings and allow access, then re-check.",
                        })} retryLabel={requestError || (canRequest && !recheckError)
                ...
                defaultValue: "Couldn’t open Settings. Open System Settings manually, then re-check.",
            })} openingLabel={t("permissionpriming.openingSettings", {
                defaultValue: "Opening…",
            })} checkingLabel={t("permissionpriming.checking", {
                defaultValue: "Checking permissions…",
            })} onOpenSettings={onOpenSettings} onRetry={requestError || (canRequest && !recheckError) ? onEnable : onRecheck} testId={`priming-recovery-${id}`}/>) : null}
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {needsRecovery ? (<Button type="button" size="sm" variant="ghost" onClick={onSkip} data-testid={`priming-skip-${id}`}>
              {t("permissionpriming.continue", { defaultValue: "Continue" })}
            </Button>) : (<>
              <Button type="button" size="sm" variant="ghost" onClick={onSkip} disabled={requesting} data-testid={`priming-skip-${id}`}>
                {t("permissionpriming.notNow", { defaultValue: "Not now" })}
              </Button>
              <Button type="button" size="sm" variant="default" onClick={onEnable} disabled={requesting} data-testid={`priming-enable-${id}`}>
                {requesting
      ...
      <Button type="button" variant="mutedLink" className="self-center" onClick={onSkipAll} data-testid="priming-skip-all">
