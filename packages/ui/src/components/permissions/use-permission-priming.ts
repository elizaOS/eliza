 ...
 */
import { checkDesktopPermissionFresh } from "../../platform";
...
const PRIMING_REASON = "Requested during onboarding so the assistant is ready to use this feature.";
...
export function usePermissionPriming(ids: readonly PermissionId[]): PermissionPrimingController {
    ...
    const patch = React.useCallback((id: PermissionId, next: Partial<PrimingItem>) => {
        setItems((current) => current.map((item) => (item.id === id ? { ...item, ...next } : item)));
    }, []);
    const request = React.useCallback(async (id: PermissionId) => {
        if (requestingRef.current.has(id))
            return;
