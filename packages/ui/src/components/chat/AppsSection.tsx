...
import { isOverlayApp } from "@elizaos/ui/apps/overlay-app-registry";
...
export function AppsSection({ headerAction }: AppsSectionProps = {}) {
    const { favoriteApps: favoriteAppsValue, appRuns, setTab, setState, setActionNotice, t, } = useAppSelectorShallow((s) => ({
    ...
    const { orderedApps, runByName } = useMemo(() => {
        ...
        const favOnlyItems = catalogApps.filter((app) => favoriteApps.includes(app.name) && !runningSet.has(app.name));
        return {
    ...
    // -------------------------------------------------------------------------
    const handleRelaunch = useCallback(async (app: RegistryAppInfo) => {
        try {
            await client.fetch("/api/apps/relaunch", {
                method: "POST",
                body: JSON.stringify({ name: app.name }),
            });
            setActionNotice(`${app.displayName ?? app.name} relaunched.`, "success", 3000);
        }
        catch (err) {
    ...
    const handleEdit = useCallback(async (app: RegistryAppInfo) => {
        try {
            await client.fetch("/api/apps/create", {
                method: "POST",
                body: JSON.stringify({ intent: "edit", editTarget: app.name }),
            });
            setActionNotice(`Editing ${app.displayName ?? app.name}…`, "info", 3500);
        }
        catch (err) {
            // error-policy:J4 failure surfaces as an action notice
            setActionNotice(err instanceof Error
                ? err.message
                : `Couldn't start an edit for ${app.displayName ?? app.name}.`, "error", 4000);
        }
    }, [setActionNotice]);
    const handleStop = useCallback(async (app: RegistryAppInfo) => {
        try {
            await client.stopApp(app.name);
            setActionNotice(`${app.displayName ?? app.name} stopped.`, "success", 3000);
        }
        catch (err) {
            // error-policy:J4 failure surfaces as an action notice
            setActionNotice(err instanceof Error
                ? err.message
                : `Couldn't stop ${app.displayName ?? app.name}.`, "error", 4000);
        }
    }, [setActionNotice]);
    // -------------------------------------------------------------------------
    // Launch handler (identical logic to FavoriteAppsBar)
    // -------------------------------------------------------------------------
    const handleLaunch = useCallback(async (app: RegistryAppInfo) => {
        ...
        try {
            ...
            if (targetUrl) {
                try {
                    ...
                    if (await openExternalUrl(targetUrl)) {
                        setActionNotice(t("appsview.OpenedInNewTab", {
                            name: app.displayName ?? app.name,
                        }), "success", 2600);
                    }
                    else {
                        setActionNotice(t("appsview.PopupBlockedOpen", {
                            name: app.displayName ?? app.name,
                        }), "error", 4200);
                    }
                }
            ...
            setActionNotice(t("appsview.LaunchedNoViewer", {
                name: app.displayName ?? app.name,
            }), "error", 4000);
        }
        catch (err) {
    ...
    if (orderedApps.length === 0 && !headerAction)
        return null;
    return (<WidgetSection title={t("nav.apps", { defaultValue: "Apps" })} icon={<LayoutGrid className="size-4"/>} action={headerAction} testId="chat-widget-apps-section">
      {orderedApps.length > 0 ? (<div className="flex flex-wrap items-center gap-2">
          {orderedApps.map((app) => {
                const run = runByName.get(app.name);
                const displayName = app.displayName ?? getAppShortName(app);
                const isRunning = Boolean(run);
                return (<div key={app.name} className="group relative" data-testid={`apps-section-tile-${app.name}`}>
                <Button title={displayName} aria-label={t("chatsidebar.launchApp", {
                        defaultValue: `Launch ${displayName}`,
                        name: displayName,
                    })} variant={isRunning ? "outlineAccent" : "transparent"} size="content" onClick={() => void handleLaunch(app)}>
                  <AppIdentityTile app={app} active={isRunning} size="sm" imageOnly/>
                </Button>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button aria-label={`Actions for ${displayName}`} data-testid={`apps-section-kebab-${app.name}`} onClick={(event) => event.stopPropagation()} variant="ghost" size="icon-sm" className="absolute -right-1 -top-1 opacity-0 transition-opacity group-hover:opacity-100">
                      <MoreHorizontal className="size-3" aria-hidden/>
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" sideOffset={4} className="w-36" onClick={(event: React.MouseEvent) => event.stopPropagation()}>
                    <DropdownMenuItem data-testid={`apps-section-launch-${app.name}`} onSelect={() => void handleLaunch(app)}>
                      {t("settings.sections.apps.launch", {
                        defaultValue: "Launch",
                    })}
                    </DropdownMenuItem>
                    <DropdownMenuItem data-testid={`apps-section-relaunch-${app.name}`} onSelect={() => void handleRelaunch(app)}>
                      {t("settings.sections.apps.relaunch", {
                        defaultValue: "Relaunch",
                    })}
                    </DropdownMenuItem>
                    <DropdownMenuItem data-testid={`apps-section-edit-${app.name}`} onSelect={() => void handleEdit(app)}>
                      {t("settings.sections.apps.edit", {
                        defaultValue: "Edit",
                    })}
                    </DropdownMenuItem>
                    {isRunning ? (<DropdownMenuItem data-testid={`apps-section-stop-${app.name}`} className="text-danger " onSelect={() => void handleStop(app)}>
