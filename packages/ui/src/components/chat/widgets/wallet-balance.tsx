...
import { selectDefaultPriceRows } from "./wallet-price-holdings";
import { selectPricedHoldings } from "./wallet-price-holdings";
import { type PricedHolding } from "./wallet-price-holdings";
...
export function WalletBalanceWidget(props: Partial<WidgetProps>): React.JSX.Element | null {
    ...
    const refresh = useCallback(async () => {
        ...
        const [balancesResult, overviewResult] = await Promise.all([
            ...
            client
                ...
                .then<RefreshResult<Awaited<ReturnType<typeof client.getWalletMarketOverview>>>>((value) => ({ ok: true, value }))
                .catch<RefreshResult<Awaited<ReturnType<typeof client.getWalletMarketOverview>>>>(() => ({
                // error-policy:J4 overview failure ⇒ no prices at all ⇒ widget hides
                ok: false,
            })),
        ]);
        if (!activeRef.current || seq !== refreshSeqRef.current)
            return;
        if (!overviewResult.ok || !balancesResult.ok) {
            ...
            setDisplayState((previous) => previous.status === "ready" ? previous : { status: "unavailable" });
            return;
        }
        const held = selectPricedHoldings(balancesResult.value, overviewResult.value.prices);
        setDisplayState({
            status: "ready",
            holdings: held.length > 0 ? held : selectDefaultPriceRows(overviewResult.value),
        });
    }, []);
    useEffect(() => {
        if (!authenticated)
    ...
    useIntervalWhenDocumentVisible(() => {
        void refresh();
    }, REFRESH_INTERVAL_MS, authenticated);
    if (!authenticated)
        return null;
    // First load pending: a quiet placeholder keeps the grid cell stable.
    if (displayState.status === "loading") {
        return (<div data-testid="chat-widget-wallet-balance-loading" aria-busy="true" className={`${spanClassName} h-12 animate-pulse`}/>);
    }
    if (displayState.status === "unavailable") {
        return (<div className={spanClassName}>
        <Button data-testid="chat-widget-wallet-unavailable" aria-label="Wallet data unavailable. Open wallet." onClick={() => nav.openView("/wallet", "wallet")} variant="surface" size="row" align="start">
    ...
    if (holdings.length === 0)
        return null;
    return (<div className={spanClassName}>
      <Button data-testid="chat-widget-wallet-prices" aria-label={`Wallet prices: ${holdings
            .map((h) => `${h.symbol} ${formatPrice(h.priceUsd)}`)
            .join(", ")}. Open wallet.`} onClick={() => nav.openView("/wallet", "wallet")} variant="homeWidget" size="card" align="start" className="w-full">
        <span className="flex items-center gap-2 text-xs text-[color:color-mix(in_srgb,var(--brand-white)_68%,transparent)] [&>svg]:h-3.5 [&>svg]:w-3.5">
          <Wallet />
          Wallet
        </span>
        {holdings.map((h) => {
            ...
            return (<span key={h.symbol} data-testid={`wallet-price-row-${h.symbol}`} className="flex items-baseline justify-between gap-2 text-sm">
              ...
              <span className="flex shrink-0 items-baseline gap-1.5">
                ...
                {change ? (<span className="tabular-nums text-xs text-[color:color-mix(in_srgb,var(--brand-white)_82%,transparent)]">
                    {change}
                  </span>) : null}
              </span>
            </span>);
