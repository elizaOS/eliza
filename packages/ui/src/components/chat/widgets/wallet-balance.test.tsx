...
vi.mock("./home-widget-card", () => ({
    HOME_WIDGET_SOLID_TILE_CLASS: "group relative flex h-auto w-full overflow-hidden rounded-2xl border border-[color:color-mix(in_srgb,var(--brand-white)_20%,var(--brand-black))] bg-[var(--brand-black)] text-left text-[var(--brand-white)]",
...
describe("WalletBalanceWidget (price-only, #10706)", () => {
    ...
    it("renders a loading placeholder until the data resolves", () => {
        ...
        expect(screen.getByTestId("chat-widget-wallet-balance-loading")).toBeTruthy();
    });
    it("renders price-only rows for held assets - no amount or holding value", async () => {
        getWalletBalances.mockResolvedValue(balances([
        ...
        render(<WalletBalanceWidget spanClassName="col-span-2 row-span-1"/>);
        await waitFor(() => expect(screen.getByTestId("chat-widget-wallet-prices")).toBeTruthy());
    ...
    it("skips holdings under $1 and renders nothing when none qualify", async () => {
        getWalletBalances.mockResolvedValue(balances([{ symbol: "SHIB", valueUsd: "0.40" }]));
        getWalletMarketOverview.mockResolvedValue(overview([{ symbol: "SHIB", priceUsd: 0.00001 }]));
        const { container } = render(<WalletBalanceWidget />);
    ...
    it("opens the wallet view on tap", async () => {
        getWalletBalances.mockResolvedValue(balances([{ symbol: "USDC", valueUsd: "100" }]));
        getWalletMarketOverview.mockResolvedValue(overview([{ symbol: "USDC", priceUsd: 1 }]));
    ...
    it("keeps the last-good rows when a later balance refresh fails", async () => {
        ...
        expect(screen.getAllByTestId(/^wallet-price-row-/).map((r) => r.dataset.testid)).toEqual([
            "wallet-price-row-BTC",
            "wallet-price-row-SOL",
            "wallet-price-row-ETH",
        ]);
    ...
    it("does not render no-holdings defaults when balances are unavailable", async () => {
        ...
        render(<WalletBalanceWidget />);
        await waitFor(() => expect(getWalletBalances).toHaveBeenCalled());
        const unavailable = await screen.findByTestId("chat-widget-wallet-unavailable");
    ...
    it("refreshes prices on the 60s visibility-gated interval (#14344)", async () => {
        ...
        expect(screen.getByTestId("chat-widget-wallet-prices").textContent).toContain("$64,000.00");
        ...
        expect(screen.getByTestId("chat-widget-wallet-prices").textContent).toContain("$70,000.00");
    });
    it("does not poll while the document is hidden", async () => {
        vi.useFakeTimers();
        setVisibility("hidden");
        getWalletBalances.mockResolvedValue({ evm: null, solana: null });
        getWalletMarketOverview.mockResolvedValue(overview([{ symbol: "BTC", priceUsd: 64000 }]));
        render(<WalletBalanceWidget />);
        await act(async () => {
            await vi.advanceTimersByTimeAsync(0);
        });
    ...
    it("renders unavailable when the overview cannot load", async () => {
        ...
        expect(await screen.findByTestId("chat-widget-wallet-unavailable")).toBeTruthy();
    });
});
