...
|---|---|---|---|---|---|---|---|
| **Task** | `[TASK:<threadId>]<title>[/TASK]` | `TASKS_CREATE` → `plugin-agent-orchestrator/src/actions/tasks.ts:725` | `message-task-parser.ts` | `task-widget.tsx` `TaskWidget` | none (display-only; whole-card navigate to `/orchestrator?taskId=`) | both¹ | ✅ wired + verified |
...
¹ The Task widget is registered by `plugin-task-coordinator` (`registerTaskWidget()`), **not** auto-loaded in `inline-builtins`. It renders on both surfaces only when the orchestrator UI is loaded — by design (`MessageContent` knows nothing about tasks).
...
|---|---|---|---|---|---|
| `notifications.recent` | home | notification-store ← WS `agent_event` `stream:"notification"` | `notifications.tsx` | ViewCatalog (home) | ✅ |
| `messages.recent` | home | conversation store | `messages.tsx` | ViewCatalog (home) | ✅ |
| `agent-orchestrator.activity` | chat-sidebar + home | `useActivityEvents` ← WS `pty-session-event` / `proactive-message` / `agent_event` | `agent-orchestrator.tsx` `OrchestratorActivityWidget` | TasksEventsPanel + ViewCatalog | ✅ |
| `agent-orchestrator.apps` | chat-sidebar + home | poll `listAppRuns()` 5s | `agent-orchestrator.tsx` `AppRunsWidget` | TasksEventsPanel + ViewCatalog | ✅ |
| `agent-orchestrator.accounts` | chat-sidebar | poll `listAccounts()`/`getOrchestratorAccounts()`/`getOrchestratorRooms()` 15s | `agent-orchestrator-accounts-view.tsx` | TasksEventsPanel | ✅ |
| `browser.status` | chat-sidebar | browser-workspace status | `browser-status.tsx` | TasksEventsPanel | ✅ |
| `music-player.stream` | chat-sidebar | music-player state | `music-player.tsx` | TasksEventsPanel | ✅ |
| `todo.items` | home | todo store | `todo.tsx` | ViewCatalog | ✅ |
| `inbox.unread` | home | inbox store | `inbox-unread.tsx` | ViewCatalog | ✅ |
| `relationships.attention` | home | relationships store | `relationships-attention.tsx` | ViewCatalog | ✅ |
| `calendar.upcoming` | home | calendar store | `calendar-upcoming.tsx` | ViewCatalog | ✅ |
| `goals.attention` | home | goals store | `goals-attention.tsx` | ViewCatalog | ✅ |
| `finances.alerts` | home | finances store | `finances-alerts.tsx` | ViewCatalog | ✅ |
| `health.sleep` | home | health store | `health-sleep.tsx` | ViewCatalog | ✅ |
| `music-library.playlists` | character | music-library state | — | CharacterHubView | ✅ |

### Slots & host mounts

| Slot | Host mounted? | Widgets registered? | Verdict |
|---|---|---|---|
| `home` | ✅ ViewCatalog | ✅ many | active |
| `chat-sidebar` | ✅ TasksEventsPanel | ✅ 5 | active |
| `character` | ✅ CharacterHubView | ✅ 1 | active |
| `heartbeats` | ✅ HeartbeatsView | ❌ none | **empty mount — remove host or register a widget** |
| `chat-inline` | ❌ never mounted | ❌ none | **dead slot — the inline-marker registry IS this concept; prune the enum value** |
| `wallet` | ❌ never mounted | ❌ none | **dead slot** |
| `browser` | ❌ never mounted | ❌ none | **dead slot** (browser uses `chat-sidebar`) |
| `settings` | ❌ never mounted | ❌ none | **dead slot** |
| `nav-page` | ❌ never mounted | ❌ none | **dead slot** |
| `automations` | ❌ never mounted | ❌ none | **dead slot** |
