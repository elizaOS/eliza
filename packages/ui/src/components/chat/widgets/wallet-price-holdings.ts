...
import { type WalletMarketMover } from "@elizaos/core/contracts/wallet-types";
import { type WalletMarketOverviewResponse } from "@elizaos/core/contracts/wallet-types";
import { type WalletMarketPriceSnapshot } from "@elizaos/core/contracts/wallet-types";
/**
 ...
 */
export function selectPricedHoldings(balances: WalletBalancesResponse | null | undefined, prices: readonly WalletMarketPriceSnapshot[] | null | undefined): PricedHolding[] {
    if (!balances)
        return [];
    ...
    for (const p of prices ?? []) {
        ...
        if (key && !priceBySymbol.has(key))
            priceBySymbol.set(key, p);
    }
    ...
    const ranked = [...heldValueBySymbol.entries()]
        ...
        .sort((a, b) => {
        if (b[1] !== a[1])
            return b[1] - a[1]; // holding value desc
...
type MarketOverviewLike = Pick<WalletMarketOverviewResponse, "prices" | "movers">;
function toPricedRow(s: WalletMarketPriceSnapshot | WalletMarketMover): PricedHolding | null {
    if (!Number.isFinite(s.priceUsd))
        return null;
    return {
        symbol: s.symbol,
        priceUsd: s.priceUsd,
 ...
 */
export function selectDefaultPriceRows(overview: MarketOverviewLike | null | undefined): PricedHolding[] {
    if (!overview)
        return [];
    const priceBySymbol = new Map<string, WalletMarketPriceSnapshot>();
    ...
    const push = (row: PricedHolding | null) => {
        if (!row || rows.length >= MAX_DEFAULT_ROWS)
            return;
