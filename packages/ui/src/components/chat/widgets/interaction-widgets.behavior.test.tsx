...
describe("ChoiceWidget — pick an option", () => {
	...
	it("reports the chosen value", () => {
		...
		render(<ChoiceWidget id="c1" scope="approve" options={options} onChoose={onChoose} />);
	...
	it("locks every option after the first pick (one decision per prompt)", () => {
		...
		expect((screen.getByTestId("choice-cancel") as HTMLButtonElement).disabled).toBe(true);
...
describe("ChoiceWidget — put their own in (allowCustom)", () => {
	...
	it("has no custom affordance unless allowCustom is set", () => {
		render(<ChoiceWidget id="c1" scope="s" options={options} onChoose={vi.fn()} />);
	...
	it("lets the user type their own answer and submits it", () => {
		const onChoose = vi.fn();
		render(
			<ChoiceWidget id="c1" scope="s" options={options} onChoose={onChoose} allowCustom />,
		);
