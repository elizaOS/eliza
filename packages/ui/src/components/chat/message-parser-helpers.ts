...
import { type PatchOp } from "@elizaos/core/config/ui-spec";
...
export const HIDDEN_TAG_BLOCK_RE = /<(think|analysis|reasoning|tool_calls?|tools?)\b[^>]*>[\s\S]*?(?:<\/\1>|$)/gi;
/**
 ...
 */
export function collectSegmentRegions(targetText: string, analysisMode: boolean): SegmentRegion[] {
    parserWork.regionScanChars += targetText.length;
    const regions: SegmentRegion[] = [];
    if (analysisMode) {
        const XML_RE = /<(thought|analysis|reasoning|tool_calls?|tools?|action|providers?|response|text)\b[^>]*>([\s\S]*?)(?:<\/\1>|$)/gi;
    ...
    for (const patch of findPatchRegions(targetText)) {
        ...
        const overlaps = regions.some((r) => patch.start < r.end && patch.end > r.start);
        if (!overlaps) {
            regions.push({
                start: patch.start,
                end: patch.end,
    ...
    const overlapsExisting = (start: number, end: number): boolean => regions.some((r) => start < r.end && end > r.start);
 ...
 */
export function interleaveSegments(targetText: string, regions: SegmentRegion[], startCursor = 0): Segment[] {
 ...
 */
export function conversationTranscriptText(messages: ReadonlyArray<ConversationTranscriptMessage>, options: {
    agentName?: string;
    userName?: string;
} = {}): string {
...
export function buildInlinePluginConfigModel(plugin: PluginInfo | null, values: Record<string, unknown>): {
    ...
    if (pluginParams.some((p) => p.required)) {
        for (const param of pluginParams) {
            if (param.required)
                continue;
            if (plugin.configUiHints?.[param.key]?.advanced === false)
                continue;
...
export function sensitiveRequestTitleLabel(key: string): string {
    ...
    const words = parts.map((part, index) => {
        ...
        if (override)
            return override;
        if (upper === "KEY" && index === parts.length - 1)
            return "key";
...
export function sensitiveRequestStatusLabel(status: NonNullable<ConversationMessage["secretRequest"]>["status"]): string {
    switch (status) {
        case "saved":
        case "submitted":
        case "fulfilled":
