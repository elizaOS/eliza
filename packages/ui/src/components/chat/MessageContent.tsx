...
import { buildInlinePluginConfigModel } from "./message-parser-helpers";
...
import { defaultConnectorWidgetModeId } from "./inline-connector-modes";
...
import { isSafeNormalizedPluginId } from "./message-parser-helpers";
import { memo } from "react";
import { normalizePluginId } from "./message-parser-helpers";
...
import { parseFormSubmitDisplay } from "./message-parser-helpers";
import { renderPermissionCardFromPayload } from "../composites/chat/permission-card.render";
import { sensitiveRequestStatusLabel } from "./message-parser-helpers";
import { sensitiveRequestTitleLabel } from "./message-parser-helpers";
import { splitInlineCode } from "./message-parser-helpers";
...
import { type UiActionDispatchMetadata } from "../config-ui/ui-renderer";
...
function renderInlineText(text: string): ReactNode {
    ...
    return parts.map((part) => {
        ...
        return part.kind === "code" ? (<CodeBlock key={key} variant="inline" value={part.code} data-testid="inline-code"/>) : (<span key={key}>{part.text}</span>);
    });
}
/**
 ...
 */
function MessageTextBody({ text, userMessage, }: {
    text: string;
    userMessage: boolean;
}) {
...
export function FormSubmitReceipt({ label }: {
    label: string;
}) {
    return (<div className="inline-flex max-w-full items-center py-1 text-xs font-medium text-muted-strong" data-testid="form-submit-receipt">
      Submitted {label}
    </div>);
}
// ── InlinePluginConfig ──────────────────────────────────────────────
...
export const InlinePluginConfig = memo(function InlinePluginConfig({ pluginId: rawPluginId, }: {
    pluginId: string;
}) {
    ...
    const { setActionNotice, loadPlugins, t, elizaCloudConnected } = useAppSelectorShallow((s) => ({
    ...
    const fetchPlugin = useCallback(async () => {
        try {
            ...
            if (mountedRef.current) {
                setError(t("messagecontent.LoadPluginInfoFailed", {
    ...
    const modes = useMemo(() => connectorWidgetModes(pluginId, {
        elizaCloudConnected: Boolean(elizaCloudConnected),
    }), [pluginId, elizaCloudConnected]);
    const selectedModeId = modeChoice ?? defaultConnectorWidgetModeId(pluginId, modes);
    ...
    const handleOAuthSignIn = useCallback(async () => {
        ...
        setError(null);
        try {
            const result = await client.startConnectorAccountOAuth(pluginId, pluginId, {});
            const authUrl = result.authUrl;
            if (!isHttpsAuthorizationUrl(authUrl)) {
                throw new Error(result.error ??
                    t("messagecontent.OAuthNoAuthUrl", {
                        defaultValue: "The connector did not return an authorization link.",
                    }));
            }
    ...
    const handleToggle = useCallback(async (enable: boolean) => {
        ...
        try {
            ...
            if (enable && mountedRef.current) {
                const tabLabel = plugin?.category === "feature"
                ...
                setPlugin((prev) => prev ? { ...prev, enabled: true, configured: true } : prev);
            }
    ...
    if (loading) {
        return (<div className="my-2 py-2 text-xs text-muted italic">
    ...
    const showConfigForm = schema && hasConfigurableParams && selectedMode?.kind !== "oauth";
    return (<ChatWidgetShell testId="inline-plugin-config" complete={connected} icon={plugin.icon ? (<span className="text-sm">{plugin.icon}</span>) : (<span className="text-sm opacity-60">{"\u2699\uFE0F"}</span>)} title={t("messagecontent.PluginConfigurationTitle", {
            defaultValue: "{{name}} Configuration",
            name: plugin.name,
        })} status={<>
          {plugin.configured && (<span className="text-2xs text-ok font-medium">
              {t("config-field.Configured")}
            </span>)}
          <span className={`text-2xs font-medium ${isEnabled ? "text-ok" : "text-muted"}`}>
            {isEnabled
                ? t("common.active", {
                    defaultValue: "Active",
                })
        ...
        </>} summary={<span className="text-ok">
      ...
      {modes.length > 1 && (
        ...
        <div className="flex flex-wrap items-center gap-1.5 px-3 pt-3" role="group" aria-label={t("messagecontent.SetupModeLabel", {
                defaultValue: "Setup method",
            })} data-testid="inline-plugin-config-modes">
          {modes.map((mode) => {
                const active = mode.id === selectedModeId;
                return (<Button key={mode.id} type="button" aria-pressed={active} title={mode.description} data-testid={`inline-plugin-config-mode-${mode.id}`} variant="selection" size="tiny" data-state={active ? "on" : "off"} onClick={() => {
      ...
      {selectedMode?.kind === "oauth" && (<div className="py-1.5" data-testid="inline-plugin-config-oauth">
          <Button variant="default" size="denseWide" onClick={() => void handleOAuthSignIn()} disabled={signingIn} data-testid="inline-plugin-config-oauth-btn">
            {signingIn
          ...
          {apiKeyModeId && (<Button type="button" onClick={() => {
                    setModeChoice(apiKeyModeId);
                    setError(null);
                }} data-testid="inline-plugin-config-use-apikey" variant="mutedLink" size="content" className="mt-2 block">
      ...
      {selectedMode?.kind === "local" && localSignIn && (<div className="py-1.5" data-testid="inline-plugin-config-local">
          {selectedMode.description && (<p className="mb-2 text-2xs text-muted">
              {selectedMode.description}
            </p>)}
          <Button variant="default" size="denseWide" onClick={() => void handleLocalSignIn()} disabled={signingIn} data-testid="inline-plugin-config-local-btn">
            {signingIn
      ...
      {showConfigForm ? (<div className="py-1.5">
          {selectedMode?.description &&
                selectedMode.kind === "local" &&
                !localSignIn && (<p className="mb-2 text-2xs text-muted">
                {selectedMode.description}
              </p>)}
          <ConfigRenderer schema={schema} hints={hints} values={mergedValues} setKeys={setKeys} registry={defaultRegistry} pluginId={plugin.id} onChange={handleChange}/>
        </div>) : selectedMode?.kind === "oauth" ||
            (selectedMode?.kind === "local" && localSignIn) ? null : (<div className="py-1.5 text-xs text-muted italic">
          {t("messagecontent.NoConfigurablePara")}
        </div>)}

      {/* Footer */}
      <div className="flex items-center gap-2 px-3 py-2 flex-wrap">
        {showConfigForm && (<Button variant="default" size="tinyWide" onClick={handleSave} disabled={saving || enabling || Object.keys(values).length === 0}>
            {saving
                ? t("common.saving", {
                    defaultValue: "Saving...",
                })
        ...
        {!isEnabled ? (<Button variant="outlineAccent" size="tinyWide" onClick={() => void handleToggle(true)} disabled={enabling || saving}>
            {enabling
          ...
          </Button>) : (<Button variant="dangerOutline" size="tinyWide" onClick={() => void handleToggle(false)} disabled={enabling || saving}>
            {enabling
...
// ── UiSpec block ────────────────────────────────────────────────────
export function MessageUiSpecBlock({ spec, raw, }: {
    spec: UiSpec;
    raw: string;
}) {
    ...
    const handleAction = useCallback((action: string, params?: Record<string, unknown>, metadata?: UiActionDispatchMetadata) => {
        // Plugin actions are handled directly via the API instead of
        // being sent back as chat messages.
        if (action === "plugin:save") {
            if (!params?.pluginId) {
                void sendActionMessage("[Failed to save plugin config: pluginId is required]");
                return;
            }
            ...
            if (Object.keys(config).length === 0) {
                void sendActionMessage("[Failed to save plugin config: no configuration values were provided]");
                return;
            }
            void client
                ...
                .catch((err: unknown) => sendActionMessage(`[Failed to save plugin config: ${err instanceof Error ? err.message : "unknown error"}]`));
            return;
        }
        if (action === "plugin:enable" && params?.pluginId) {
            void client
                ...
                // error-policy:J4 the failure is surfaced as a chat action message
                .catch((err: unknown) => sendActionMessage(`[Failed to enable plugin: ${err instanceof Error ? err.message : "unknown error"}]`));
            return;
        }
        ...
        if (action === "plugin:configure" && params?.pluginId) {
            void sendActionMessage(`Please show me the configuration form for the ${params.pluginId} plugin`);
            return;
        }
    ...
    return (<div className="my-2 overflow-hidden">
      ...
      {showRaw && (
        ...
        <Card variant="codePane" className="overflow-x-auto overscroll-x-contain">
      ...
      <div className="py-1.5">
        ...
        */}
        <ErrorBoundary fallback={() => (<Card surface="destructiveSubtle" border="destructive" padding="default" tone="muted" className="text-xs">
              <span className="font-semibold text-destructive">
                Couldn't render this widget.
              </span>{" "}
              <Button type="button" className="underline underline-offset-2" onClick={() => setShowRaw((v) => !v)}>
...
export function SensitiveRequestBlock({ request, }: {
    ...
    const isSaved = status === "saved" || status === "submitted" || status === "fulfilled";
    const handleSubmit = useCallback(async (event: FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        if (!canCollectSecret || !canSubmit)
            return;
        ...
        try {
            if (isRemoteConnect) {
                ...
                if (currentRequest.current !== submittedRequest)
                    return;
    ...
    return (<Card variant="insetPadded" stack="compact" data-testid="sensitive-request" className="my-2">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="break-words font-medium">{requestLabel}</div>
          {canCollectSecret && !tunnel && !isRemoteConnect && (<div className="mt-0.5 text-xs text-muted-strong">
              Masked input. It never lands in the transcript.
            </div>)}
        </div>
        <div data-testid="sensitive-request-status" className={cn("flex shrink-0 items-center gap-1 text-xs", isSaved ? "text-ok" : "text-muted-strong")}>
          {isSaved && <Check className="size-3.5" aria-hidden/>}
          {sensitiveRequestStatusLabel(status)}
        </div>
      </div>
      {request.reason && (<div className="text-xs text-muted-strong">{request.reason}</div>)}
      {request.delivery?.instruction && (<div className="text-xs text-muted-strong">
          {request.delivery.instruction}
        </div>)}
      {canCollectSecret && (<SemanticForm className="space-y-3" onSubmit={handleSubmit}>
          {fields.map((field) => {
                const label = field.label ?? field.name;
                const inputId = `sensitive-request-${field.name.replace(/[^a-zA-Z0-9_-]/g, "-")}`;
                const isUpload = field.input === "image" || field.input === "file";
                if (isUpload) {
                    const accept = field.mimeTypes && field.mimeTypes.length > 0
                        ? field.mimeTypes.join(",")
                        : field.input === "image"
                            ? "image/*"
                            : undefined;
                    const hasValue = Boolean(values[field.name]);
                    return (<label key={field.name} htmlFor={inputId} className="block text-xs space-y-1">
                  <span className="font-medium">{label}</span>
                  <Input id={inputId} aria-label={label} data-testid={`sensitive-request-file-${field.name}`} variant="secret" density="short" type="file" accept={accept} 
                    // Mobile: prefer the rear camera for image capture (2FA QR/seed).
                    capture={field.input === "image" ? "environment" : undefined} required={field.required && !hasValue} onChange={(event) => {
                            ...
                            if (field.maxBytes && file.size > field.maxBytes) {
                                setError(`${label} is too large (max ${Math.round(field.maxBytes / 1024)} KB).`);
                            ...
                            reader.onerror = () => setError(`Could not read ${label}.`);
                ...
                return (<label key={field.name} htmlFor={inputId} className="block text-xs space-y-1">
                <span className="font-medium">{label}</span>
                <Input id={inputId} aria-label={label} variant="secret" density="short" type={field.input === "secret" ? "password" : "text"} value={values[field.name] ?? ""} onChange={(event) => {
          ...
          <Button type="submit" size="sm" disabled={saving || !canSubmit} data-testid="sensitive-request-submit">
            {saving
                ? "Saving…"
                : (request.form?.submitLabel ??
                    (isRemoteConnect ? "Connect" : "Save securely"))}
          </Button>
          {!isRemoteConnect && (<div className="flex items-center gap-1.5 text-xs text-muted-strong" data-testid="sensitive-request-security-note">
      ...
      {canStartOAuth && request.form?.kind === "oauth" && (<OAuthRequestPanel form={request.form} authorizing={authorizing} onStart={() => {
                ...
                try {
                    ...
                    if (typeof window === "undefined")
                        return;
                    const popup = window.open(url, "eliza-oauth", "width=520,height=720,noreferrer");
                    if (!popup) {
                        ...
                        setError("Pop-up blocked. Allow pop-ups for this site to continue.");
                        return;
                    }
                    try {
                        popup.opener = null;
...
export function MessagePermissionCard({ payload, }: {
    payload: PermissionCardPayload;
}) {
    const { sendActionMessage } = useAppSelectorShallow((s) => ({
        sendActionMessage: s.sendActionMessage,
    }));
    const permissionRegistry = useMemo(() => isNative && !isDesktopPlatform()
        ? createMobileSignalsPermissionsRegistry(undefined, client)
        : createClientPermissionsRegistry(client), []);
    const handlePermissionFallback = useCallback((feature: string, permission: string) => {
        void sendActionMessage(`__permission_card__:use_fallback feature=${feature} permission=${permission}`);
    }, [sendActionMessage]);
    const handlePermissionGranted = useCallback((feature: string, permission: string) => {
        void sendActionMessage(`__permission_card__:granted feature=${feature} permission=${permission}`);
    }, [sendActionMessage]);
    return renderPermissionCardFromPayload(payload, {
        ...
        onFallback: ({ feature, permission }) => handlePermissionFallback(feature, permission),
        onGranted: () => handlePermissionGranted(payload.feature, payload.permission),
    });
}
function OAuthRequestPanel({ form, authorizing, onStart, }: {
    ...
    return (<div data-testid="sensitive-request-oauth" className="space-y-2">
      {form.scopes && form.scopes.length > 0 && (<div className="text-xs text-muted">
          Scopes: {form.scopes.join(", ")}
        </div>)}
      <Button type="button" size="sm" onClick={onStart} disabled={authorizing} data-testid="sensitive-request-oauth-start">
...
export function MessageContent({ message, analysisMode = false, }: MessageContentProps) {
    ...
    const { sendActionMessage, setTab, handleChatRetry } = useAppSelectorShallow((s) => ({
        sendActionMessage: s.sendActionMessage,
        setTab: s.setTab,
    ...
    const [localDownloadState, setLocalDownloadState] = useState<"idle" | "busy" | "queued" | "failed">("idle");
    ...
    const inlineWidgetCtx = useInlineWidgetContext(sendActionMessage, setChatInput);
    ...
    const handleDownloadDefaultLocalModel = useCallback(async () => {
        ...
        try {
            ...
            setLocalDownloadError(error instanceof Error ? error.message : "Failed to start download");
    ...
    if (message.capabilityHandoff) {
        ...
        {message.text.trim() ? (<MessageTextBody text={message.text} userMessage={false}/>) : null}
        <CapabilityHandoffBlock request={message.capabilityHandoff}/>
      </div>);
    }
    if (message.localInference &&
        ...
        <AlertDescription className="gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="button" size="sm" onClick={handleDownloadDefaultLocalModel} disabled={downloading || localDownloadState === "busy"}>
              {downloading
                ? "Downloading"
            ...
            {!canStartDownload ? (<Button type="button" size="sm" onClick={handleOpenSettings}>
                Open Local Models
              </Button>) : null}
          </div>
          {localDownloadError ? (<div className="text-xs text-danger">{localDownloadError}</div>) : null}
        </AlertDescription>
      </Alert>);
    }
    if (message.role === "assistant" && message.replyRecoveryAvailable === true) {
        ...
        <AlertDescription>
          <div className="whitespace-pre-wrap">{message.text}</div>
          <Button type="button" size="sm" disabled={replyRecoveryPending} onClick={async () => {
                if (!message.id || replyRecoveryPending)
                    return;
    ...
    const hasAssistantExtras = message.role === "assistant" &&
        Boolean(message.reasoning?.trim() || message.toolEvents?.length);
    if (!hasAssistantExtras &&
        segments.length === 1 &&
        segments[0].kind === "text") {
        return (<MessageTextBody text={segments[0].text} userMessage={message.role === "user"}/>);
    }
    return (<div>
      {message.role === "assistant" && message.reasoning?.trim() ? (<ThinkingBlock reasoning={message.reasoning}/>) : null}
      {message.role === "assistant" && message.toolEvents?.length ? (<div className="mb-2 flex flex-col gap-1.5">
          {message.toolEvents.map((event) => (<ToolCallEventLog key={event.callId ?? event.id} event={event}/>))}
        </div>) : null}
      {(() => {
            ...
            return segments.map((seg) => {
                ...
                switch (seg.kind) {
                    case "text":
                        return (<MessageTextBody key={segmentKey} text={seg.text} userMessage={message.role === "user"}/>);
                    case "code":
                        return (<CodeBlock key={segmentKey} className="my-2" value={seg.code} wrap copyable data-testid="code-block" {...(seg.lang ? { "data-lang": seg.lang } : {})}/>);
                    case "analysis-xml":
                        return (<Card key={segmentKey} variant="insetPadded" stack="compact" className="my-2">
                  ...
                  <pre className="m-0 overflow-x-auto whitespace-pre-wrap break-words font-mono text-xs text-muted-strong overscroll-x-contain">
                    ...
                    case "config":
                        ...
                        return (<InlinePluginConfig key={segmentKey} pluginId={seg.pluginId}/>);
                    case "ui-spec":
                        return (<MessageUiSpecBlock key={segmentKey} spec={seg.spec} raw={seg.raw}/>);
                    case "widget": {
                    ...
                    case "permission":
                        return (<MessagePermissionCard key={segmentKey} payload={seg.payload}/>);
                    default:
                        return null;
                }
            });
      ...
      {analysisMode && message.actionName && (<Card variant="insetPadded" stack="compact" className="my-2">
      ...
      {analysisMode &&
            ...
            message.actionCallbackHistory.length > 0 && (<Card variant="insetPadded" stack="compact" className="my-2">
            ...
            <div className="space-y-1 font-mono text-xs text-muted-strong">
              {(() => {
                ...
                return message.actionCallbackHistory.map((log) => {
                    ...
                    return (<div key={`${message.id}:action-callback:${n}:${log}`} className="break-words">
                      {log}
                    </div>);
                });
            })()}
