...
function resolveUrl(url: string): string {
    ...
    if (!trimmed || ABSOLUTE_URL.test(trimmed))
        return trimmed;
 ...
 */
async function readInlineText(att: MessageAttachment, signal: AbortSignal): Promise<{
    text: string;
    loadFailed?: boolean;
}> {
    if (att.text?.trim())
    ...
    try {
        const text = await fetchWithDeadline(src, { method: "GET" }, async (response) => {
            if (!response.ok) {
 ...
 */
export function segmentsFromEditedText(text: string, original: TranscriptSegment[]): TranscriptSegment[] {
    const lines = text
        .split("\n")
        .map((l) => l.trim())
        .filter((l) => l.length > 0);
    ...
    const totalMs = original.length > 0
...
export function TranscriptViewerOverlay({ attachment, onClose, }: TranscriptViewerOverlayProps): React.JSX.Element | null {
    ...
    const audioUrl = load.status === "ready" && load.audioUrl ? resolveUrl(load.audioUrl) : null;
    const dirty = value !== pristine;
    const title = load.status === "ready"
        ? load.title
    ...
    React.useEffect(() => {
        ...
        return () => {
            live = false;
            controller.abort(new DOMException("Transcript load superseded", "AbortError"));
        };
    }, [attachment]);
    ...
    const handleGrantShare = React.useCallback(async () => {
        ...
        try {
            ...
            setShareStatus({
                kind: "error",
                message: err instanceof Error ? err.message : "Couldn't share transcript.",
            });
        }
    ...
    const handleRevokeShare = React.useCallback(async () => {
        if (load.status !== "ready" || !load.transcriptId)
            return;
        const entityId = shareStatus.kind === "success"
    ...
    return createPortal(<div role="dialog" aria-modal="true" aria-label={`Transcript: ${title}`} data-testid="transcript-viewer" className="fixed inset-0 flex items-center justify-center p-4 sm:p-6" style={{
            zIndex: Z_SHELL_OVERLAY + 10,
            paddingTop: "calc(var(--safe-area-top, 0px) + 1rem)",
        }}>
      <Button aria-label="Close transcript" onClick={onClose} variant="publicRow" size="content" className="absolute inset-0 cursor-default"/>
      <Card flow="column" surface="card" border="standard" radius="large" tone="text" className="relative max-h-full w-full max-w-2xl overflow-hidden">
        {/* Header: title + close */}
        <Card variant="attachmentHeader" flow="row" gap="compact" className="px-4 py-3">
          <h2 className="min-w-0 flex-1 truncate text-sm font-semibold text-txt-strong">
            {title}
          </h2>
          <Badge variant="outline" size="compact" className="hidden shrink-0 sm:inline-flex">
            <LockKeyhole className="size-3" aria-hidden/>
            Private
          </Badge>
          {load.status === "ready" && load.redacted ? (<Badge variant="secondary" size="compact" className="shrink-0" data-testid="transcript-redacted-badge">
              <ShieldCheck className="size-3" aria-hidden/>
              Redacted
            </Badge>) : null}
          <Button aria-label="Close" onClick={onClose} variant="ghostMuted" size="icon-sm" shape="circle" className="active:scale-[0.96] motion-reduce:active:scale-100">
        ...
        <div className="mb-3 space-y-1.5">
              <audio src={audioUrl} controls preload="metadata" data-testid="transcript-audio" className="w-full">
                <track kind="captions"/>
              </audio>
              <div className="flex items-center gap-1 text-xs text-muted">
                <FileAudio className="size-3.5 shrink-0" strokeWidth={1.5} aria-hidden/>
                <span>Recording retained</span>
              </div>
            </div>) : null}
          {load.status === "loading" ? (<div className="flex items-center gap-2 py-8 text-sm text-muted">
              <Spinner size={16}/> Loading transcript…
            </div>) : load.status === "error" ? (<p className="py-8 text-sm text-danger" data-testid="transcript-load-error">
              {load.message}
            </p>) : editing ? (<Textarea value={value} onChange={(e) => setValue(e.target.value)} aria-label="Edit transcript" data-testid="transcript-editor" variant="form" density="editor" className="resize-none leading-relaxed" autoFocus/>) : (<pre data-testid="transcript-text" className="whitespace-pre-wrap break-words font-sans text-xs-tight leading-relaxed text-txt">
              {value || "(empty transcript)"}
            </pre>)}
          {saveError ? (<p className="mt-2 text-xs text-danger" data-testid="transcript-save-error">
              {saveError}
            </p>) : null}
          {shareOpen ? (<Card surface="backgroundSubtle" border="standard" padding="default" className="mt-4" data-testid="transcript-share-sheet">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto]">
                <label htmlFor="transcript-share-target" className="grid gap-1 text-xs text-muted">
                  Recipient entity ID
                  <Input id="transcript-share-target" value={shareTarget} onChange={(event) => {
                setShareTarget(event.target.value);
                setShareStatus({ kind: "idle" });
            }} placeholder="Entity ID" density="compact" data-testid="transcript-share-target" variant="config"/>
                </label>
                <div className="grid content-end gap-1">
                  <Card asChild surface="card" border="standard" className="inline-flex p-1">
                    <fieldset>
                      <legend className="sr-only">
                        Transcript disclosure mode
                      </legend>
                      <Button variant={shareMode === "redacted" ? "default" : "ghost"} size="dense" onClick={() => {
                setShareMode("redacted");
                setShareStatus({ kind: "idle" });
            }} data-testid="transcript-share-mode-redacted">
                        Redacted
                      </Button>
                      <RoleGate minRole="ADMIN" fallback={<Button variant="ghostMuted" size="dense" disabled data-testid="transcript-share-mode-full-disabled">
                            Full
                          </Button>}>
                        <Button variant={shareMode === "full" ? "default" : "ghost"} size="dense" onClick={() => {
                setShareMode("full");
                setShareStatus({ kind: "idle" });
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <Button variant="default" size="sm" onClick={() => void handleGrantShare()} disabled={shareStatus.kind === "submitting" ||
                load.status !== "ready" ||
                !load.transcriptId ||
                !shareTarget.trim()} data-testid="transcript-grant-share">
                  {shareStatus.kind === "submitting" ? (<Loader2 className="mr-1.5 size-4 animate-spin"/>) : (<Share2 className="mr-1.5 size-4"/>)}
                  Grant
                </Button>
                <Button variant="ghost" size="sm" onClick={() => void handleRevokeShare()} disabled={shareStatus.kind === "submitting" ||
                load.status !== "ready" ||
                !load.transcriptId ||
                !shareTarget.trim()} data-testid="transcript-revoke-share">
              ...
              {shareStatus.kind === "error" ? (<p className="mt-2 break-words text-xs text-danger" data-testid="transcript-share-error">
                  {shareStatus.message}
                </p>) : shareStatus.kind === "success" ? (<p className="mt-2 break-words text-xs text-status-success" data-testid="transcript-share-success">
                  {shareStatus.message}
                </p>) : null}
            </Card>) : null}
        </div>
        ...
        {/* Action bar */}
        <Card variant="topDivider" flow="row" gap="compact" className="flex-wrap px-4 pt-3 pb-[calc(var(--safe-area-bottom,0px)+0.75rem)]">
          {!editing ? (<Button variant="ghost" size="sm" onClick={() => setEditing(true)} data-testid="transcript-edit">
              <Pencil className="mr-1.5 size-4" strokeWidth={1.5}/> Edit
            </Button>) : (<Button variant="ghost" size="sm" onClick={() => setValue(pristine)} disabled={!dirty} data-testid="transcript-undo">
              <Undo2 className="mr-1.5 size-4" strokeWidth={1.5}/> Undo
            </Button>)}
          <Button variant={copyStatus === "failed" ? "dangerGhost" : "ghostMuted"} size="sm" onClick={() => void handleCopy()} data-testid="transcript-copy">
            {copyStatus === "copied" ? (<Check className="mr-1.5 size-4 text-status-success" strokeWidth={1.5}/>) : (<Copy className="mr-1.5 size-4" strokeWidth={1.5}/>)}
          ...
          <Button variant="ghost" size="sm" onClick={handleSaveToFiles} data-testid="transcript-save-to-files">
            <Download className="mr-1.5 size-4" strokeWidth={1.5}/> Download
          </Button>
          {resolvedId || audioUrl ? (<Button variant="dangerGhost" size="sm" onClick={handleOpenInKnowledge} data-testid="transcript-open-in-knowledge">
              <Library className="mr-1.5 size-4" strokeWidth={1.5}/> Open in
              Knowledge
            </Button>) : null}
          {resolvedId ? (<Button variant="ghost" size="sm" onClick={() => void handleDelete()} data-testid="transcript-delete">
          ...
          <div className="ml-auto flex items-center gap-2">
            <Button variant="ghost" size="sm" onClick={onClose} data-testid="transcript-cancel">
              Cancel
            </Button>
            <Button variant="default" size="sm" onClick={() => void handleSaveAndExit()} disabled={saving || (editing && !dirty && canPersist)} data-testid="transcript-save-exit">
