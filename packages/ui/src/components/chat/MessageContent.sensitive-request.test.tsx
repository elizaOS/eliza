...
import { SensitiveRequestBlock } from "./MessageContent";
...
import { type ConnectRequestResult } from "../../events";
...
function permissionState(overrides: Partial<PermissionState> = {}): PermissionState {
    return {
        id: "reminders",
...
function renderWithApp(message: ConversationMessage, sendActionMessage = vi.fn()) {
    const appValue = {
        t: (key: string) => key,
        sendActionMessage,
    } as never;
    ...
    render(<AppContext.Provider value={appValue}>
...
describe("MessageContent sensitive requests", () => {
    afterEach(() => {
        cleanup();
        for (const stop of connectionCleanups.splice(0))
            stop();
    ...
    beforeEach(() => {
        ...
        clientMock.requestPermission.mockResolvedValue(permissionState({ status: "granted", canRequest: false }));
        clientMock.openPermissionSettings.mockResolvedValue(undefined);
    });
    it("renders public requests as status-only without an input", () => {
        render(<MessageContent message={baseMessage({ secretRequest: pendingPublicSecretRequest() })}/>);
        expect(screen.getByTestId("sensitive-request-status").textContent).toBe("Pending");
        expect(screen.queryByLabelText("OPENAI_API_KEY")).toBeNull();
        expect(screen.getByTestId("sensitive-request").textContent).toContain("Open the owner app");
        expect(screen.queryByText("Fallback text that should not render for a sensitive request.")).toBeNull();
    });
    it("renders owner-private inline requests as a private form descriptor", () => {
        render(<MessageContent message={baseMessage({
        ...
        expect(screen.getByTestId("sensitive-request").textContent).not.toContain("will not be sent");
        ...
        expect(screen.getByTestId("sensitive-request").textContent).toContain("Masked input. It never lands in the transcript.");
        expect(screen.getByTestId("sensitive-request-security-note").textContent).toContain("Sent directly to the agent. Never posted to chat.");
        expect(screen.getByTestId("sensitive-request-security-note").textContent).not.toContain("encrypted");
    });
    it("labels the submit button 'Save securely' when the form omits submitLabel", () => {
        const request = pendingOwnerInlineSecretRequest();
        if (request?.form)
            request.form.submitLabel = undefined;
        render(<MessageContent message={baseMessage({ secretRequest: request })}/>);
    ...
    it("describes the tunnel delivery (send-once, not stored) on tunneled requests", () => {
        render(<MessageContent message={baseMessage({ secretRequest: pendingTunnelSecretRequest() })}/>);
        expect(screen.getByTestId("sensitive-request-security-note").textContent).toContain("Sent once to the waiting session");
        ...
        expect(screen.getByTestId("sensitive-request").textContent).not.toContain("Masked input. It never lands in the transcript.");
    });
    it("shows success status without rendering the submitted value", async () => {
        ...
        const { container } = render(<MessageContent message={baseMessage({
                secretRequest: pendingOwnerInlineSecretRequest(),
            })}/>);
        ...
        await waitFor(() => {
            expect(screen.getByTestId("sensitive-request-status").textContent).toBe("Saved");
        });
    ...
    it("submits tunneled sub-agent credentials through the tunnel endpoint instead of saving global secrets", async () => {
        ...
        const { container } = render(<MessageContent message={baseMessage({ secretRequest: pendingTunnelSecretRequest() })}/>);
    ...
    it("renders an image field as a file input and delivers it via updateSecrets (#8910)", async () => {
        ...
        render(<MessageContent message={baseMessage({ secretRequest: pendingImageSecretRequest() })}/>);
        const input = screen.getByTestId("sensitive-request-file-seed_photo") as HTMLInputElement;
        ...
        await waitFor(() => {
            expect((screen.getByTestId("sensitive-request-submit") as HTMLButtonElement)
                .disabled).toBe(false);
        });
        ...
        const payload = updateSecretsMock.mock.calls[0]?.[0] as Record<string, string>;
    ...
    it("renders a non-image file field as a file input without camera capture (#8910)", async () => {
        ...
        render(<MessageContent message={baseMessage({ secretRequest: request })}/>);
        const input = screen.getByTestId("sensitive-request-file-doc") as HTMLInputElement;
        expect(input.type).toBe("file");
    ...
    it("rejects an upload over maxBytes and does not submit (#8910)", async () => {
        ...
        const field = request?.form?.fields?.[0];
        if (field)
            field.maxBytes = 3; // 3 bytes — a 4-byte file must be rejected.
        render(<MessageContent message={baseMessage({ secretRequest: request })}/>);
        const input = screen.getByTestId("sensitive-request-file-seed_photo") as HTMLInputElement;
        ...
        await waitFor(() => {
            expect(screen.getByTestId("sensitive-request").textContent).toContain("too large");
        });
        expect((screen.getByTestId("sensitive-request-submit") as HTMLButtonElement)
            .disabled).toBe(true);
        expect(updateSecretsMock).not.toHaveBeenCalled();
    ...
    it("renders an OAuth request with a Connect button and never shows the URL in chat", () => {
        const { container } = render(<MessageContent message={baseMessage({ secretRequest: pendingOAuthRequest() })}/>);
        ...
        expect(container.textContent?.includes("example.test/oauth/authorize")).toBe(false);
    ...
    it("opens the authorization URL in a popup when the Connect button is clicked", () => {
        ...
        render(<MessageContent message={baseMessage({ secretRequest: pendingOAuthRequest() })}/>);
        fireEvent.click(screen.getByTestId("sensitive-request-oauth-start"));
        expect(openMock).toHaveBeenCalledTimes(1);
        expect(openMock.mock.calls[0]?.[0]).toBe("https://example.test/oauth/authorize?state=abc");
        ...
        expect(screen.getByTestId("sensitive-request-oauth-start").textContent).toContain("Authorizing");
        window.open = originalOpen;
    });
    it.each(["https://agent.example.com:31337/", "agent.example.com:31337/"])("remote_connect submits %s without writing the secret store", async (address) => {
        connectionCleanups.push(listenForConnectRequests(() => ({ status: "connected" })));
        ...
        const urlInput = screen.getByLabelText("Remote agent URL") as HTMLInputElement;
    ...
    it("remote_connect omits an empty token from the CONNECT_EVENT detail", async () => {
        ...
        document.addEventListener(CONNECT_EVENT, onConnect);
        render(<MessageContent message={baseMessage({ secretRequest: pendingRemoteConnectRequest() })}/>);
    ...
    it("keeps remote credentials editable until adoption succeeds and permits retry after failure", async () => {
        ...
        connectionCleanups.push(listenForConnectRequests((detail) => {
        ...
        expect(screen.getByText("Start the remote agent, then try again.")).toBeTruthy();
        expect((screen.getByLabelText("Remote agent URL") as HTMLInputElement).value).toBe("https://paused.example");
        expect((screen.getByLabelText("Access token (optional)") as HTMLInputElement)
            .value).toBe("synthetic-owner");
        expect((screen.getByRole("button", { name: "Connect" }) as HTMLButtonElement)
            .disabled).toBe(false);
        fireEvent.click(screen.getByRole("button", { name: "Connect" }));
        await act(async () => {
    ...
    it("does not apply an old completion or finally to a replaced form request", async () => {
        ...
        connectionCleanups.push(listenForConnectRequests(() => new Promise<ConnectRequestResult>((resolve) => {
            completions.push(resolve);
        })));
        const first = pendingRemoteConnectRequest();
        const second = pendingRemoteConnectRequest();
        if (!first || !second)
            throw new Error("Remote form fixture required");
        ...
        expect(screen.getByTestId("sensitive-request-status").textContent).toBe("Pending");
        expect((screen.getByLabelText("Remote agent URL") as HTMLInputElement).value).toBe("https://b.example");
    ...
    ])("remote_connect rejects invalid URL %s and keeps the form editable", async (address, message) => {
        const connectEvents: unknown[] = [];
        const onConnect = (event: Event) => {
            connectEvents.push((event as CustomEvent).detail);
        };
    ...
    it("degrades a blocked OAuth popup to same-tab navigation on plain web (#15143)", () => {
        ...
        expect(assignSpy).toHaveBeenCalledWith("https://example.test/oauth/authorize?state=abc");
...
describe("MessageContent permission cards", () => {
    ...
    it("renders permission_request as an inline card and hides the JSON block", async () => {
        const text = "I need access before I can add that.\n```json\n" +
            JSON.stringify({
        ...
        expect(screen.getByText("I need access before I can add that.")).toBeTruthy();
        expect(document.body.textContent).not.toContain("permission_request");
        expect(screen.getByTestId("permission-card-fallback").textContent).toContain("Use internal reminder");
    });
    it("sends fallback and granted action messages back through chat", async () => {
        ...
        expect(sendActionMessage).toHaveBeenCalledWith("__permission_card__:use_fallback feature=lifeops.reminders.create permission=reminders");
        cleanup();
        renderWithApp(baseMessage({ text }), sendActionMessage);
        fireEvent.click(await screen.findByTestId("permission-card-primary"));
        await waitFor(() => expect(sendActionMessage).toHaveBeenCalledWith("__permission_card__:granted feature=lifeops.reminders.create permission=reminders"));
    });
});
