...
describe("CapabilityHandoffBlock", () => {
    ...
    it("shows concise value and preserves explicit review after same-app setup", async () => {
        ...
        fireEvent.click(screen.getByRole("button", { name: "Set up personal workspace" }));
        ...
        expect(window.sessionStorage.getItem("eliza:capability-handoff:pending")).toContain("Move tomorrow's meeting to 3.");
        stopListening();
    });
    it("clears the continuation when contained setup is unavailable", async () => {
        render(<CapabilityHandoffBlock request={request}/>);
        fireEvent.click(screen.getByRole("button", { name: "Set up personal workspace" }));
        expect((await screen.findByRole("alert")).textContent).toContain("Setup can’t open inside this app yet");
        expect(window.sessionStorage.getItem("eliza:capability-handoff:pending")).toBeNull();
    });
});
