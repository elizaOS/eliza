...
function tokenInput(container: HTMLElement) {
    return container.querySelector('input[data-config-key="TELEGRAM_BOT_TOKEN"]') as HTMLInputElement | null;
}
beforeEach(() => {
    withFrozenClock();
    withSeededRandom();
...
describe("connector-setup card — minimal + Advanced split", () => {
    it("renders required params immediately and moves optional params behind Advanced", async () => {
        ...
        const { container } = withApp(<MessageContent message={assistant("[CONFIG:telegram]")}/>);
        ...
        expect(container.querySelector('input[data-config-key="TELEGRAM_API_ROOT"]')).toBeNull();
        expect(container.querySelector('input[data-config-key="TELEGRAM_ALLOWED_CHATS"]')).toBeNull();
    ...
    it("the Advanced toggle reveals the optional fields", async () => {
        ...
        expect(container.querySelector('input[data-config-key="TELEGRAM_API_ROOT"]')).toBeTruthy();
        expect(container.querySelector('input[data-config-key="TELEGRAM_ALLOWED_CHATS"]')).toBeTruthy();
...
describe("connector-setup card — collapse-on-connect", () => {
    it("mounts collapsed to a compact status row when already connected", async () => {
        ...
        expect(screen.getByTestId("inline-plugin-config-summary").textContent).toContain("Telegram is enabled.");
    ...
    it("preserves in-progress field edits across a collapse/expand round-trip", async () => {
        ...
        expect((tokenInput(container) as HTMLInputElement).value).toBe("12345:draft-token");
    });
});
