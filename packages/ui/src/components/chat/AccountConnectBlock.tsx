 ...
 */
function providerLabel(providerId: LinkedAccountProviderId, t: (key: string, vars?: Record<string, unknown>) => string): string {
    switch (providerId) {
...
export function AccountConnectBlock({ request, }: {
    request: AccountConnectRequest;
}) {
    const t = useAppSelector((s) => s.t);
    const accounts = useAccounts();
    const [openProvider, setOpenProvider] = useState<LinkedAccountProviderId | null>(null);
    ...
    return (<Card variant="insetPadded" className="text-sm" data-testid="account-connect">
      ...
      <div className="text-muted-strong whitespace-pre-wrap mb-3">
        {request.reason?.trim()
            ...
            : t("accounts.connect.subheading", {
                defaultValue: "Pick a provider to sign into another account. Your accounts rotate automatically.",
            })}
      </div>
      <div className="flex flex-col gap-2">
        {request.providers.map((providerId) => {
            const count = countByProvider.get(providerId) ?? 0;
            return (<Card variant="configRow" flow="rowBetween" gap="default" padding="compact" key={providerId} data-testid={`account-connect-row-${providerId}`}>
              ...
              <Button type="button" variant="default" size="sm" data-testid={`account-connect-add-${providerId}`} onClick={() => setOpenProvider(providerId)} className="shrink-0">
                {t("accounts.add.button", { defaultValue: "Add account" })}
              </Button>
            </Card>);
        })}
      </div>
      {openProvider ? (<AddAccountDialog open providerId={openProvider} onClose={() => setOpenProvider(null)} onCreated={() => {
