...
import { CONFIG_RE } from "./message-parser-helpers";
...
import { type PluginMutationResult } from "../../api/client-types-config";
...
import { withFrozenClock } from "../../../test/determinism";
import { withSeededRandom } from "../../../test/determinism";
...
function plugin(over: Partial<PluginInfo> & {
    id: string;
}): PluginInfo {
    return {
        name: over.id,
...
function mutationResult(over: Partial<PluginMutationResult> = {}): PluginMutationResult {
    return { ok: true, ...over };
}
...
function withApp(node: React.ReactElement) {
    ...
    const t = (key: string, vars?: Record<string, unknown>) => {
        ...
        return template.replace(/\{\{(\w+)\}\}/g, (whole, name) => vars && name in vars ? String(vars[name]) : whole);
    };
    const appValue = {
        t,
        setActionNotice,
    ...
    const utils = render(<AppContext.Provider value={appValue}>{node}</AppContext.Provider>);
...
describe("[CONFIG] marker parsing", () => {
    it("CONFIG_RE matches a bare plugin id and a scoped npm id", () => {
        ...
        const ids = Array.from("[CONFIG:weather] and [CONFIG:@elizaos/plugin-x]".matchAll(CONFIG_RE)).map((m) => m[1]);
    ...
    it("parseSegments lifts [CONFIG:weather] into a config segment", () => {
        ...
        const config = segments.find((s): s is Extract<Segment, {
            kind: "config";
        }> => s.kind === "config");
        expect(config?.pluginId).toBe("weather");
        // The surrounding prose survives as its own text segment.
        expect(segments.some((s) => s.kind === "text" && s.text.includes("Configure it"))).toBe(true);
    });
    ...
    it("rejects a prototype-pollution id via isSafeNormalizedPluginId", () => {
        expect(isSafeNormalizedPluginId(normalizePluginId("__proto__"))).toBe(false);
        expect(isSafeNormalizedPluginId(normalizePluginId("constructor"))).toBe(false);
        expect(isSafeNormalizedPluginId(normalizePluginId("prototype"))).toBe(false);
        // A normal id passes.
        expect(isSafeNormalizedPluginId(normalizePluginId("@elizaos/plugin-x"))).toBe(true);
    });
    it("does not render an InlinePluginConfig for a prototype-pollution id", () => {
        ...
        const { container } = withApp(<MessageContent message={assistant("[CONFIG:__proto__]")}/>);
...
describe("MessageContent → InlinePluginConfig states", () => {
    ...
    it("shows the loading placeholder while the plugin fetch is pending", () => {
        ...
        const { container } = withApp(<MessageContent message={assistant("[CONFIG:weather]")}/>);
        expect(container.textContent ?? "").toContain("Loading weather configuration...");
    });
    ...
    it("renders the configurable params and the plugin title for a found plugin", async () => {
        ...
        expect(container.querySelector('input[data-config-key="WEATHER_API_KEY"]')).toBeTruthy();
    });
    it("saves edited config via updatePlugin and surfaces the saved state", async () => {
        ...
        const { container } = withApp(<MessageContent message={assistant("[CONFIG:weather]")}/>);
        const input = (await screen.findByText("Weather Configuration")) &&
    ...
    it("surfaces the server error message when save fails", async () => {
        ...
        const input = container.querySelector('input[data-config-key="WEATHER_API_KEY"]');
        fireEvent.change(input as HTMLInputElement, {
            target: { value: "bad" },
        });
        fireEvent.click(screen.getByRole("button", { name: "common.save" }));
    ...
    it("enabling a disabled plugin calls updatePlugin({enabled:true}), refreshes, notifies, and dismisses", async () => {
        ...
        clientMock.updatePlugin.mockResolvedValue(mutationResult());
        const { setActionNotice, loadPlugins } = withApp(<MessageContent message={assistant("[CONFIG:weather]")}/>);
        ...
        await waitFor(() => {
            expect(setActionNotice).toHaveBeenCalledWith(expect.stringContaining("Weather is on."), "success", 4000);
        });
    ...
    it("disabling an enabled plugin calls updatePlugin({enabled:false}) and does not dismiss", async () => {
        ...
        clientMock.updatePlugin.mockResolvedValue(mutationResult());
        const { setActionNotice } = withApp(<MessageContent message={assistant("[CONFIG:weather]")}/>);
