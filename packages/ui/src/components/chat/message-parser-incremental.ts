 ...
 */
import { SEGMENT_TRIGGER_RE } from "./message-parser-helpers";
import { collectSegmentRegions } from "./message-parser-helpers";
import { getInlineWidgetOpenTokens } from "./widgets/inline-registry";
import { interleaveSegments } from "./message-parser-helpers";
import { normalizeDisplayCore } from "./message-parser-helpers";
import { normalizeDisplayText } from "./message-parser-helpers";
import { parseSegments } from "./message-parser-helpers";
import { parserWork } from "./message-parser-helpers";
import { stripAssistantStageDirections } from "@elizaos/core/utils/assistant-text";
import { stripHiddenDisplayContent } from "./message-parser-helpers";
import { type Segment } from "./message-parser-helpers";
import { type SegmentRegion } from "./message-parser-helpers";
...
const HIDDEN_BLOCK_RE = /<(think|analysis|reasoning|tool_calls?|tools?)\b[^>]*>[\s\S]*?(?:<\/\1>|$)/gi;
/**
...
export function computeSafeNormCut(raw: string, fromCut: number): number {
    ...
    for (let m = HIDDEN_BLOCK_RE.exec(window); m !== null; m = HIDDEN_BLOCK_RE.exec(window)) {
    ...
    const insideClosedBlock = (i: number): boolean => closed.some((b) => i > b.start && i < b.end);
 ...
 */
function hasCoupledFencedUiSpecRisk(tailRegions: readonly SegmentRegion[], target: string): boolean {
    const firstFence = target.indexOf("```");
    if (firstFence === -1)
        return false;
    for (const r of tailRegions) {
        if (r.segment.kind !== "ui-spec")
            continue;
        if (target.slice(r.start, r.start + 3) !== "```")
            continue; // patch ui-spec
        if (firstFence < r.start)
            return true; // a fence precedes this UiSpec
    }
    return false;
}
/**
 ...
 */
function proseHasOpenMarker(text: string, from: number, to: number, openTokens: readonly string[]): boolean {
    if (from >= to)
        return false;
    const gap = text.slice(from, to);
    if (!gap.includes("["))
        return false;
    if (gap.includes("[CONFIG"))
        return true;
    for (const token of openTokens)
        if (gap.includes(token))
            return true;
    return false;
}
/**
...
function isMergeablePatchTail(r: SegmentRegion, target: string): boolean {
    if (r.segment.kind !== "ui-spec")
        return false;
    if (!target.slice(r.start, r.end).trimStart().startsWith("{"))
        return false;
...
function isUnsealedFenceTail(r: SegmentRegion, target: string): boolean {
    ...
    if (after.length === 0)
        return true; // a ` ``` ` opener could still arrive
    if (after.startsWith("```"))
        return true; // adjacent fence opener confirmed
 ...
 */
export function parseSegmentsStreaming(text: string, analysisMode: boolean, cache: StreamingParseCache | null): StreamingParseResult {
    if (!cache ||
    ...
    const windowCore = normRawCut > cache.normRawCut
    ...
    const permissionMode = cache.permissionMode || target.includes(PERMISSION_MARKER);
    if (permissionMode) {
    ...
    for (const r of rawTail) {
        ...
        if (start < prevCut)
            continue; // already covered by stableSegments
        tailRegions.push({
            start,
            end: r.end + scanStart,
    ...
    const segments = cache.stableSegments.length === 0 && sortedTail.length === 0
    ...
    for (let idx = 0; idx < sortedTail.length; idx++) {
        ...
        if (fenceParity % 2 !== 0)
            break;
        if (proseHasOpenMarker(target, cursor, r.start, openTokens))
            break;
        cursor = r.end;
        if (r.end >= target.length)
            break; // no content after ⇒ closer not confirmed
        if (isMergeablePatchTail(r, target))
            break;
        if (isUnsealedFenceTail(r, target))
            break;
        newCut = r.end;
    ...
    if (newCut > prevCut) {
        ...
        stableSegments = cache.stableSegments.concat(interleaveSegments(target.slice(0, newCut), sortedTail.slice(0, cutRegionCount), prevCut));
