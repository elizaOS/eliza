 ...
 */
import { APP_EMOTE_EVENT } from "../../events/index";
import { Button } from "../ui/button";
import { CharacterExamplesPanel } from "./CharacterEditorPanels";
import { CharacterHubView } from "./CharacterHubView";
import { CharacterIdentityPanel } from "./CharacterEditorPanels";
import { CharacterRoster } from "./CharacterRoster";
import { CharacterStylePanel } from "./CharacterEditorPanels";
import { DEFAULT_ELEVEN_FAST_MODEL } from "./character-voice-config";
...
import { FramedPageBody } from "../../layouts/framed-page";
import { Input } from "../ui/input";
import { PREMADE_VOICES } from "@elizaos/core/voice";
import { ShellViewAgentSurface } from "../views/ShellViewAgentSurface";
import { VOICE_CONFIG_UPDATED_EVENT } from "../../events/index";
import { buildCharacterDraftFromPreset } from "./character-editor-helpers";
import { buildVoiceConfigForCharacterEntry } from "./character-voice-config";
import { client } from "../../api/client";
import { createCustomPackRosterEntry } from "./CharacterRoster.helpers";
import { dispatchWindowEvent } from "../../events/index";
import { getFirstRunPresetStyles } from "./character-editor-helpers";
...
import { resolveRosterEntries } from "./CharacterRoster.helpers";
import { sanitizeApiKey } from "@elizaos/core/voice";
...
import { type FirstRunPreset } from "./character-editor-helpers";
import { type KeyboardEvent as ReactKeyboardEvent } from "react";
import { type ReactNode } from "react";
import { useAgentElement } from "../../agent-surface";
import { useAppSelectorShallow } from "../../state";
import { useCallback } from "react";
import { useChatAvatarVoiceBridge } from "../../hooks";
...
const DownloadIcon = ({ className }: {
    className?: string;
}) => (<Icon className={className} d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3"/>);
const UploadIcon = ({ className }: {
    className?: string;
}) => (<Icon className={className} d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M17 8l-5-5-5 5M12 3v12"/>);
...
const idleSaveBtnStyle = {
    background: "linear-gradient(180deg, rgba(var(--accent-rgb),0.16) 0%, rgba(var(--accent-rgb),0.1) 100%)",
 ...
 */
function CharacterPageTabButton({ page, label, isActive, agentLabel, onSelect, children, className, style, onKeyDown, }: {
    ...
    return (<Button ref={ref} variant="selection" size="content" data-state={isActive ? "on" : "off"} id={`character-editor-tab-${page}`} role="tab" aria-selected={isActive} aria-current={isActive ? "page" : undefined} aria-controls={`character-editor-panel-${page}`} tabIndex={isActive ? 0 : -1} className={className} style={style} onClick={() => onSelect(page)} onKeyDown={onKeyDown} {...agentProps}>
      {children}
    </Button>);
}
/**
 ...
 * as the click.
 */
function CharacterAgentButton({ agentId, agentLabel, agentGroup, agentDescription, agentStatus, onActivate, ...buttonProps }: {
    agentId: string;
    agentLabel: string;
    agentGroup?: string;
...
/* ── Component ─────────────────────────────────────────────────────── */
export function CharacterEditor({ initialPage, sceneOverlay = false, inModal: _inModal = false, onHeaderActionsChange, pageChrome, }: {
    ...
    const { tab, setTab, characterData, characterDraft, characterLoading, characterSaving, characterSaveSuccess, chatAgentVoiceMuted: _chatAgentVoiceMuted, characterSaveError, handleCharacterFieldInput, handleCharacterStyleInput, handleSaveCharacter, loadCharacter, setState, firstRunOptions, selectedVrmIndex, customVrmUrl: _customVrmUrl, customVrmPreviewUrl, customCatchphrase, customVoicePresetId, activePackId, t, uiLanguage, registryStatus: _registryStatus, registryLoading: _registryLoading, registryRegistering: _registryRegistering, registryError: _registryError, dropStatus: _dropStatus, loadRegistryStatus, registerOnChain: _registerOnChain, syncRegistryProfile: _syncRegistryProfile, loadDropStatus, walletConfig: _walletConfig, elizaCloudConnected, elizaCloudVoiceProxyAvailable, } = useAppSelectorShallow((s) => ({
        tab: s.tab,
        setTab: s.setTab,
    ...
    const handleFieldEdit = useCallback((field: string, value: unknown) => {
        if (!suppressDirtyRef.current)
            setFieldsEdited(true);
        handleCharacterFieldInput(field as keyof CharacterData, value as CharacterData[keyof CharacterData]);
    ...
    const [activePage, setActivePage] = useState<CharacterEditorPage>(initialPage ?? "personality");
    ...
    /* ── Style entry state ──────────────────────────────────────────── */
    const [pendingStyleEntries, setPendingStyleEntries] = useState<Record<string, string>>({ all: "", chat: "", post: "" });
    const [styleEntryDrafts, setStyleEntryDrafts] = useState<Record<string, string[]>>({ all: [], chat: [], post: [] });
    /* ── Roster state ───────────────────────────────────────────────── */
    ...
    const firstRunPresetStyles = useMemo(() => getFirstRunPresetStyles(firstRunOptions), [firstRunOptions]);
    ...
    const handleChatAvatarSpeakingChange = useCallback((isSpeaking: boolean) => {
    ...
    useEffect(() => {
        ...
        if (firstRunPresetStyles.length) {
            const merged = firstRunPresetStyles.map((serverPreset) => {
                const localMeta = localizedPresets.find((p) => p.id === serverPreset.id ||
    ...
    const baseRosterEntries = useMemo(() => {
        ...
        if (activePackId && _customVrmUrl) {
            const customFirstRunName = typeof characterData?.name === "string" && characterData.name.trim()
                ? characterData.name
                : "Custom";
            base.unshift(createCustomPackRosterEntry({
                id: activePackId,
    ...
    const characterRoster = useMemo(() => {
        ...
        const draftName = typeof characterDraft.name === "string" ? characterDraft.name.trim() : "";
        if (!activeId || !draftName)
            return baseRosterEntries;
        return baseRosterEntries.map((entry) => entry.id === activeId ? { ...entry, name: draftName } : entry);
    }, [
        baseRosterEntries,
        selectedCharacterId,
        savedCharacterId,
    ...
    const fallbackCharacterName = (typeof d.name === "string" && d.name.trim()) ||
    ...
    const normalizedMessageExamples = Array.isArray(d.messageExamples)
        ? hasValidMessageExamplesShape(d.messageExamples)
            ? (d.messageExamples as ReturnType<typeof normalizeCharacterMessageExamples>)
            : normalizeCharacterMessageExamples(d.messageExamples, fallbackCharacterName)
        : [];
    const bioText = typeof d.bio === "string"
        ...
        : Array.isArray(d.bio)
            ...
            : "";
    const hasCharacterContent = (c: unknown) => Boolean(c && Object.keys(c as Record<string, unknown>).length > 0);
    ...
    const activeCharacterRosterEntry: CharacterRosterEntry | null = useMemo(() => {
        ...
        if (!currentCharacter)
            return null;
        const currentName = typeof currentCharacter.name === "string"
    ...
    const hasPendingChanges = fieldsEdited ||
    ...
    useEffect(() => {
        ...
        if (hasValidMessageExamplesShape(d.messageExamples))
            return;
        const normalized = normalizeCharacterMessageExamples(d.messageExamples, fallbackCharacterName);
    ...
    useEffect(() => {
        void (async () => {
            ...
            try {
                ...
                if (tts) {
                    const serverElevenlabsVoiceId = typeof tts.elevenlabs === "object" ? tts.elevenlabs.voiceId : null;
                    setVoiceConfig((prev) => {
                        if (!voicePresetAppliedRef.current) {
                            return tts;
                        }
                        const serverElevenlabs = typeof tts.elevenlabs === "object" ? tts.elevenlabs : {};
                        const currentElevenlabs = typeof prev.elevenlabs === "object" ? prev.elevenlabs : {};
                    ...
                    if (serverElevenlabsVoiceId && !voicePresetAppliedRef.current) {
                        const preset = PREMADE_VOICES.find((p) => p.voiceId === serverElevenlabsVoiceId);
                ...
                setVoiceSaveError("Voice settings are unavailable until configuration finishes loading.");
            }
            setVoiceLoading(false);
        })();
    }, []);
    /* ── Voice helpers ──────────────────────────────────────────────── */
    const applyVoicePresetForEntry = useCallback((entry: CharacterRosterEntry) => {
        setVoiceSaveError(null);
    ...
    /* ── Character defaults ─────────────────────────────────────────── */
    const applyCharacterDefaults = useCallback((entry: CharacterRosterEntry) => {
    ...
    const commitCharacterSelection = useCallback((entry: CharacterRosterEntry, applyDefaults: boolean) => {
        ...
        if (!voiceSelectionLocked && isNewCharacter) {
            ...
            if (persistedVoiceConfig) {
                ...
                void persistCharacterVoiceSelection({
                    ...
                    setVoiceSaveError("Could not save the selected character voice. Use Save to retry.");
                    logger.warn({ err }, "[CharacterEditor] voice config early persist failed");
                });
            }
        }
        if (applyDefaults) {
    ...
    const requestPageChange = useCallback((page: CharacterEditorPage) => {
        if (page === activePage)
            return;
        if (hasPendingChanges) {
    ...
    const requestCharacterSelection = useCallback((entry: CharacterRosterEntry) => {
        if (entry.id === selectedCharacterId)
            return;
        if (hasPendingChanges) {
    ...
    const handleSelectCharacter = useCallback((entry: CharacterRosterEntry) => {
    ...
    useEffect(() => {
        ...
        const isNamed = typeof currentCharacter.name === "string" &&
            currentCharacter.name.trim().length > 0;
        const hasBioOrSystem = Boolean(currentCharacter.bio ||
        ...
        const entry = activeCharacterRosterEntry ??
        ...
        const applyDefaults = shouldApplyPresetDefaults(hasMeaningfulContent, currentCharacter.name, entry.name);
    ...
    useEffect(() => {
        ...
        const handler = () => {
            ...
            if (greeting.characterId !== activeCharacterIdRef.current)
                return;
    ...
    const persistVoiceConfig = useCallback(async () => {
        setVoiceSaveError(null);
        const provider = voiceConfig.provider ?? (useElevenLabs ? "eliza-cloud" : "edge");
        ...
        if (provider === "edge") {
            ...
            const hasElevenLabsApiKey = hasConfiguredApiKey((voiceConfig.elevenlabs as Record<string, string> | undefined)?.apiKey);
            const defaultVoiceMode = typeof voiceConfig.mode === "string"
                ? voiceConfig.mode
            ...
            const normalized: Record<string, string> = {
                ...
                modelId: (voiceConfig.elevenlabs as Record<string, string> | undefined)
                    ?.modelId ?? DEFAULT_ELEVEN_FAST_MODEL,
            };
            const sanitizedKey = sanitizeApiKey(normalized?.apiKey);
            if (sanitizedKey)
                normalized.apiKey = sanitizedKey;
            else
    ...
    const handleExportCharacter = useCallback(() => {
        ...
        if (!data)
            return;
        const fileName = `${typeof data.name === "string" && data.name.trim()
    ...
    const resolvePendingNavigation = useCallback(async (shouldSave: boolean) => {
    ...
    const renderContentActionButtons = (uploadInputId: string) => (<div className="flex flex-wrap items-center justify-end gap-2">
      <CharacterAgentButton agentId="action-upload-vrm" agentLabel={t("aria.upload", { defaultValue: "Upload VRM" })} agentGroup="character-actions" agentDescription="Upload a custom VRM avatar file" type="button" variant="outline" size="icon" className="size-9 rounded-sm" onActivate={() => document.getElementById(uploadInputId)?.click()} onClick={() => document.getElementById(uploadInputId)?.click()} title={t("aria.upload", {
      ...
      <CharacterAgentButton agentId="action-export-json" agentLabel={t("charactereditor.ExportJSON", {
            defaultValue: "Export JSON",
        })} agentGroup="character-actions" agentDescription="Download the current character as a JSON file" type="button" variant="outline" size="icon" className="size-9 rounded-sm" onActivate={handleExportCharacter} onClick={handleExportCharacter} disabled={!currentCharacter} title={t("charactereditor.ExportJSON", {
            defaultValue: "Export JSON",
      ...
      <CharacterAgentButton agentId="action-reset" agentLabel={t("common.reset", { defaultValue: "Reset" })} agentGroup="character-actions" agentDescription="Reset this character to its default values" type="button" variant="outline" size="sm" className="h-9 rounded-sm px-4 text-xs-tight font-semibold" onActivate={handleResetToDefaults} onClick={handleResetToDefaults} disabled={!activeCharacterRosterEntry || !currentCharacter} title={t("charactereditor.ResetToDefaults", {
            defaultValue: "Reset to Defaults",
        })}>
        {t("common.reset", { defaultValue: "Reset" })}
      </CharacterAgentButton>
      <CharacterAgentButton agentId="action-save" agentLabel={t("common.save", { defaultValue: "Save" })} agentGroup="character-actions" agentDescription="Save character and voice settings" agentStatus={hasPendingChanges ? "active" : "inactive"} size="sm" className="h-9 rounded-sm px-6 text-sm font-bold tracking-[0.05em] transition-[background-color,border-color,color,box-shadow,transform] duration-200 disabled:opacity-50" style={hasPendingChanges ? accentGradientStyle : idleSaveBtnStyle} disabled={characterSaving ||
            voiceSaving ||
            !hasPendingChanges ||
            !currentCharacter} onActivate={() => void handleSaveAll()} onClick={() => void handleSaveAll()}>
    ...
    const handlePendingStyleEntryChange = useCallback((key: string, value: string) => {
        setPendingStyleEntries((prev) => ({ ...prev, [key]: value }));
    }, []);
    const handleAddStyleEntry = useCallback((key: string) => {
    ...
    const handleRemoveStyleEntry = useCallback((key: string, index: number) => {
        const nextItems = [...(d.style?.[key as "all" | "chat" | "post"] ?? [])];
        nextItems.splice(index, 1);
        handleStyleEdit(key as "all" | "chat" | "post", nextItems.join("\n"));
    }, [d.style, handleStyleEdit]);
    const handleReorderStyleEntries = useCallback((key: string, items: string[]) => {
        handleStyleEdit(key as "all" | "chat" | "post", items.join("\n"));
    }, [handleStyleEdit]);
    const handleStyleEntryDraftChange = useCallback((key: string, index: number, value: string) => {
    ...
    const handleCommitStyleEntry = useCallback((key: string, index: number) => {
    ...
    if (characterLoading && !characterData) {
        const loadingState = (<div className={sceneOverlay
                ...
                : "flex flex-col w-full flex-1 items-center justify-center"} data-no-camera-zoom={sceneOverlay ? "true" : undefined} data-no-camera-drag={sceneOverlay ? "true" : undefined} data-testid={sceneOverlay ? "companion-character-editor" : undefined}>
        ...
        return sceneOverlay ? (loadingState) : (<FramedPage gutterOwner="framed-page">
        {pageChrome}
        <FramedPageBody scroll="page">{loadingState}</FramedPageBody>
      </FramedPage>);
    }
    /* ── Render ─────────────────────────────────────────────────────── */
    return (<ShellViewAgentSurface viewId="character">
      <div className={sceneOverlay
            ...
            : "flex flex-col w-full flex-1 min-h-0 gap-4"} data-no-camera-zoom={sceneOverlay ? "true" : undefined} data-no-camera-drag={sceneOverlay ? "true" : undefined} data-testid={sceneOverlay ? "companion-character-editor" : undefined} onWheel={sceneOverlay ? (e) => e.stopPropagation() : undefined}>
        <div className={sceneOverlay
          ...
          {sceneOverlay && !customizing && (<div className="shrink min-h-0 overflow-hidden flex flex-col items-center justify-end w-full relative max-[600px]:!overflow-visible pointer-events-auto">
              <CharacterRoster entries={characterRoster} selectedId={selectedCharacterId ?? activeCharacterRosterEntry?.id ?? null} onSelect={handleSelectCharacter}/>
            </div>)}

          {/* ── Companion overlay: tabbed editor (identity | style | examples) */}
          {sceneOverlay && customizing && (<section className="flex flex-col flex-1 min-h-0 gap-2 overflow-hidden" aria-label={t("charactereditor.TabbedEditorGroupLabel", {
                defaultValue: "Character editor — tabbed sections",
            })}>
              <div className="flex flex-wrap items-center gap-3 shrink-0">
                <div 
        /* Flat — no card/border. Active tab carries the only fill. */
        className="flex shrink-0 items-center gap-1" role="tablist" aria-label={t("charactereditor.TabbedEditorGroupLabel", {
                defaultValue: "Character editor sections",
            })}>
                  {CHARACTER_EDITOR_PAGES.map((page) => {
                const pageLabel = page === "personality"
                ...
                return (<CharacterPageTabButton key={page} page={page} label={pageLabel} agentLabel={pageLabel} isActive={activePage === page} onSelect={requestPageChange} className="flex-initial cursor-pointer rounded-sm bg-transparent px-[0.6rem] py-1.5 text-center text-2xs font-bold uppercase tracking-[0.1em] text-txt transition-[background,color,box-shadow] duration-150 hover:bg-bg-hover hover:text-txt-strong" style={activePage === page ? accentGradientStyle : undefined} onKeyDown={(event) => {
                        ...
                        event.preventDefault();
                        const currentIndex = CHARACTER_EDITOR_PAGES.indexOf(activePage);
              ...
              <div id={`character-editor-panel-${activePage}`} role="tabpanel" aria-labelledby={`character-editor-tab-${activePage}`} className="flex flex-col flex-1 min-h-0 overflow-hidden">
                <div className={`custom-scrollbar flex flex-col flex-1 gap-3 min-h-0 overflow-y-auto pr-1 [scrollbar-gutter:stable]${activePage !== "personality" ? " hidden" : ""}`}>
                  <CharacterIdentityPanel nameText={typeof d.name === "string" ? d.name : ""} systemText={typeof d.system === "string" ? d.system : ""} bioText={bioText} handleFieldEdit={handleFieldEdit} t={t}/>
                </div>
                <div className={`custom-scrollbar flex flex-col flex-1 gap-3 min-h-0 overflow-y-auto pr-1 [scrollbar-gutter:stable]${activePage !== "style" && activePage !== "examples" ? " hidden" : ""}`}>
                  <div style={{
                display: rightTab === "style" ? undefined : "none",
            }}>
                    <CharacterStylePanel d={d} pendingStyleEntries={pendingStyleEntries} styleEntryDrafts={styleEntryDrafts} handlePendingStyleEntryChange={handlePendingStyleEntryChange} handleAddStyleEntry={handleAddStyleEntry} handleRemoveStyleEntry={handleRemoveStyleEntry} handleStyleEntryDraftChange={handleStyleEntryDraftChange} handleCommitStyleEntry={handleCommitStyleEntry} handleReorderStyleEntries={handleReorderStyleEntries} t={t}/>
                  </div>
                  <div style={{
                display: rightTab === "examples" ? undefined : "none",
            }}>
                    <CharacterExamplesPanel d={d} normalizedMessageExamples={normalizedMessageExamples} handleFieldEdit={handleFieldEdit} t={t}/>
                  </div>
                </div>
              </div>
            </section>)}
          ...
          {!sceneOverlay && (<CharacterHubView pageChrome={pageChrome} d={d} bioText={bioText} normalizedMessageExamples={normalizedMessageExamples} pendingStyleEntries={pendingStyleEntries} styleEntryDrafts={styleEntryDrafts} applyFieldEdit={(field, value) => {
                handleCharacterFieldInput(field as keyof CharacterData, value as CharacterData[keyof CharacterData]);
            }} handlePendingStyleEntryChange={handlePendingStyleEntryChange} applyStyleEdit={handleCharacterStyleInput} handleStyleEntryDraftChange={handleStyleEntryDraftChange} characterSaveError={characterSaveError}/>)}
        </div>

        {/* ── Footer (companion overlay only) ────────────────────────── */}
        {sceneOverlay && (<div className="flex flex-col gap-2 pt-2 shrink-0 pointer-events-auto">
            {(characterSaveSuccess || combinedSaveError) && (<div className="flex flex-wrap items-center justify-center gap-2">
                {characterSaveSuccess && (<span className="rounded-sm border border-status-success/20 bg-status-success-bg px-3 py-1 text-xs font-bold text-status-success">
                    {characterSaveSuccess}
                  </span>)}
                {combinedSaveError && (<span className="rounded-sm border border-status-danger/20 bg-status-danger-bg px-3 py-1 text-xs font-medium text-status-danger">
                    {combinedSaveError}
                  </span>)}
              </div>)}

            <div className="flex min-h-9 items-center justify-end">
              <Input type="file" id="ce-vrm-upload" accept=".vrm" variant="nativeFileDisplayNone" style={{ display: "none" }} onChange={(e: ChangeEvent<HTMLInputElement>) => {
              ...
              <CharacterAgentButton agentId="action-toggle-customize" agentLabel={customizing
                ? t("charactereditor.SelectBtn", { defaultValue: "Select" })
                : t("charactereditor.CustomizeBtn", {
                    defaultValue: "Customize",
                })} agentGroup="character-actions" agentDescription="Toggle between the character roster and the customize editor" agentStatus={customizing ? "active" : "inactive"} type="button" variant="default" size="sm" className="h-9 rounded-sm px-6 text-sm font-bold tracking-[0.05em] transition-[background-color,border-color,color,box-shadow,transform] duration-200 disabled:opacity-50" style={accentGradientStyle} onActivate={() => {
                if (customizing) {
        ...
        <Dialog open={pendingNavigation !== null} onOpenChange={(open: boolean) => {
            if (!open)
                setPendingNavigation(null);
        }}>
          <DialogContent className="max-w-md rounded-sm border-border/60 bg-bg">
            ...
            defaultValue: "You have unsaved changes. Save before switching?",
        })}
            ...
            : pendingNavigation?.kind === "page"
                ? `\n${t("charactereditor.SwitchSectionPrompt", {
                    defaultValue: "Switch to {{name}}?",
                    name: pendingNavigation.page === "personality"
                        ? t("charactereditor.TabPersonality", {
                            defaultValue: "Personality",
                        })
            ...
            <DialogFooter className="gap-2 sm:gap-2">
              <Button type="button" onClick={() => void resolvePendingNavigation(true)} disabled={characterSaving || voiceSaving}>
                {t("common.save", { defaultValue: "Save" })}
              </Button>
              <Button type="button" variant="outline" onClick={() => void resolvePendingNavigation(false)}>
        ...
        <Dialog open={resetConfirmOpen} onOpenChange={(open: boolean) => {
            if (!open)
          ...
          <DialogContent className="max-w-md rounded-sm border-border/60 bg-bg">
            ...
            defaultValue: "This will discard all unsaved changes and restore this character to its default values.",
        })}
              </DialogDescription>
            </DialogHeader>
            <DialogFooter className="gap-2 sm:gap-2">
