...
export function getFirstRunPresetStyles(options: unknown): readonly FirstRunPreset[] {
    if (!options || typeof options !== "object")
        return [];
    const styles = (options as {
        styles?: unknown;
    }).styles;
...
export function buildCharacterDraftFromPreset(entry: CharacterRosterEntry) {
    ...
    return {
        ...
        messageExamples: p.messageExamples.map((convo: Array<{
            user: string;
            content: {
                text: string;
            };
            ...
            examples: convo.map((msg: {
                ...
                name: msg.user === "{{agentName}}"
                    ? name
                    : replaceCharacterToken(msg.user, name),
                content: { text: replaceCharacterToken(msg.content.text, name) },
            })),
        })),
        postExamples: p.postExamples.map((ex: string) => replaceCharacterToken(ex, name)),
    };
}
/**
 ...
 */
export function shouldApplyPresetDefaults(hasMeaningfulContent: boolean, savedCharacterName: string | null | undefined, rosterEntryName: string): boolean {
    if (!hasMeaningfulContent)
        return true;
    const savedNorm = typeof savedCharacterName === "string"
