...
import { createSandboxHostFacilities } from "./SandboxedViewFrame";
...
function mountFrame(surface: SurfaceManifest) {
    render(<SandboxedViewFrame viewId={VIEW_ID} surface={surface} srcDoc="<!doctype html><title>probe</title>" title="probe"/>);
    const iframe = screen.getByTestId(`sandboxed-view-frame-${VIEW_ID}`) as HTMLIFrameElement;
    const frameWindow = iframe.contentWindow;
    if (!frameWindow)
        throw new Error("iframe contentWindow missing in jsdom");
    const postSpy = vi
...
function postFromFrame(frameWindow: Window, capability: string, payload: unknown, requestId: string) {
...
describe("SandboxedViewFrame — real isolation path (#14180)", () => {
    ...
    it("renders a REAL sandbox: allow-scripts without allow-same-origin", () => {
        render(<SandboxedViewFrame viewId={VIEW_ID} surface={GRANTED} srcDoc="<!doctype html><title>x</title>" title="x"/>);
        const iframe = screen.getByTestId(`sandboxed-view-frame-${VIEW_ID}`) as HTMLIFrameElement;
    ...
    it("DENIES storage without the grant — no key is written, typed denial to the frame", async () => {
        const { frameWindow, postSpy } = mountFrame(UNGRANTED);
        postFromFrame(frameWindow, "storage", { op: "set", key: "secret", value: "pwn" }, "req-store-deny");
        await waitFor(() => expect(postSpy).toHaveBeenCalled());
    ...
    it("SERVICES storage with the grant — writes ONLY the view-namespaced key", async () => {
        const { frameWindow, postSpy } = mountFrame(GRANTED);
        postFromFrame(frameWindow, "storage", { op: "set", key: "draft", value: "hello" }, "req-store-ok");
        await waitFor(() => expect(postSpy).toHaveBeenCalled());
        expect(lastResponse(postSpy).ok).toBe(true);
    ...
    it("keeps colon-bearing view IDs and storage keys in distinct namespaces", async () => {
        ...
        expect(window.localStorage.getItem(sandboxStorageKey("alpha:beta", "draft"))).toBe("one");
        expect(window.localStorage.getItem(sandboxStorageKey("alpha", "beta:draft"))).toBe("two");
