...
import { brokerSandboxedViewRequest } from "./sandboxed-view-broker";
import { dispatchNavigateViewEvent } from "@elizaos/core/events";
import { logger } from "@elizaos/ui/logger";
import { parseSandboxedViewRequest } from "./sandboxed-view-broker";
...
import { type SandboxHostFacilities } from "./sandboxed-view-broker";
 ...
 */
export function createSandboxHostFacilities(viewId: string): SandboxHostFacilities {
    return {
 ...
 */
export function SandboxedViewFrame({ viewId, surface, src, srcDoc, sandboxExtra, title, }: SandboxedViewFrameProps) {
    ...
    const sandbox = useMemo(() => resolveSandboxTokens(sandboxExtra), [sandboxExtra]);
    const facilities = useMemo(() => createSandboxHostFacilities(viewId), [viewId]);
    useEffect(() => {
        const onMessage = (event: MessageEvent) => {
            ...
            if (!request)
                return;
            void brokerSandboxedViewRequest(viewId, resolvedManifest, request, facilities).then((response) => {
                if (!response.ok) {
                    logger.warn(`[SandboxedViewFrame] denied "${request.capability}" for view "${viewId}": ${response.error}`);
                }
    ...
    return (<Card asChild variant="sandboxFrame">
      <iframe ref={frameRef} data-testid={`sandboxed-view-frame-${viewId}`} title={title} sandbox={sandbox} src={src} srcDoc={srcDoc} className="h-full w-full"/>
    </Card>);
}
