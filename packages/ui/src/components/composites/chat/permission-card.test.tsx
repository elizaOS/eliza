...
function makeRegistry(initial: PermissionState, overrides: Partial<IPermissionsRegistry> = {}): IPermissionsRegistry {
    return {
        get: vi.fn(() => initial),
...
function state(overrides: Omit<PermissionState, "platform"> & Partial<Pick<PermissionState, "platform">>): PermissionState {
...
describe("PermissionCard", () => {
    it("renders the friendly title and reason for not-determined state", () => {
        render(<PermissionCard {...baseProps} initialState={{
                id: "reminders",
        ...
        expect((screen.getByTestId("permission-card-primary") as HTMLButtonElement)
            .textContent).toContain("Grant access");
    });
    it("calls registry.request and reports granted on success", async () => {
        ...
        render(<PermissionCard {...baseProps} registry={registry} onGranted={onGranted}/>);
    ...
    it("renders 'Open System Settings' when denied and canRequest is false", () => {
        ...
        expect((screen.getByTestId("permission-card-primary") as HTMLButtonElement)
            .textContent).toContain("Open System Settings");
    });
    it("opens settings when not-determined cannot be requested directly", () => {
        const onOpenSettings = vi.fn();
        render(<PermissionCard {...baseProps} permission="screentime" initialState={{
                id: "screentime",
                status: "not-determined",
                lastChecked: 0,
                canRequest: false,
        ...
        const button = screen.getByTestId("permission-card-primary") as HTMLButtonElement;
    ...
    it("renders disabled 'Coming soon' when restricted by entitlement", () => {
        render(<PermissionCard {...baseProps} permission="health" initialState={{
                id: "health",
                status: "restricted",
        ...
        const btn = screen.getByTestId("permission-card-primary") as HTMLButtonElement;
    ...
    it("renders write-only calendar access as limited until an upgrade succeeds", async () => {
        ...
        const onGranted = vi.fn();
        render(<PermissionCard {...baseProps} permission="calendar" feature="lifeops.calendar.read" registry={registry} initialState={limitedState} onGranted={onGranted}/>);
        expect(screen.queryByTestId("permission-card-granted")).toBeNull();
        expect(screen.getByTestId("permission-card").dataset.status).toBe("limited");
        expect(screen.getByText("limited")).toBeTruthy();
        expect(screen.getByText(/add-only access: new events can go to the default/i)).toBeTruthy();
        const button = screen.getByTestId("permission-card-primary") as HTMLButtonElement;
    ...
    it("sends non-requestable limited access to system settings", () => {
        const onOpenSettings = vi.fn();
        render(<PermissionCard {...baseProps} permission="calendar" initialState={{
                id: "calendar",
                status: "limited",
                lastChecked: 1,
                canRequest: false,
    ...
    it("emits fallback choice when offered and clicked", () => {
        ...
        render(<PermissionCard {...baseProps} fallbackOffered fallbackLabel="Use internal reminders instead" initialState={{
                id: "reminders",
                status: "not-determined",
                lastChecked: 0,
                canRequest: true,
