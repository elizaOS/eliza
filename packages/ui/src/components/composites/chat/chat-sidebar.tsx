...
function ChatSidebarRoot({
  ...
  const newChatAction = isGameModal ? (
    <Button
      variant="outline"
      className="h-11 w-full rounded-sm border-[color:var(--first-run-accent-border)] bg-[color:var(--first-run-accent-bg)] px-3 py-2 text-sm font-medium text-[color:var(--first-run-text-strong)] hover:border-[color:var(--first-run-accent-border-hover)] hover:bg-[color:var(--first-run-accent-bg-hover)] active:scale-[0.98]"
      onClick={() => {
        onCreate();
        onClose?.();
      }}
