...
export function defaultStateFor(id: PermissionId): PermissionState {
    const platform = typeof navigator !== "undefined" && /Win/i.test(navigator.platform ?? "")
        ? "win32"
...
export function createClientPermissionsRegistry(clientLike: PermissionClientLike): IPermissionsRegistry {
...
export function parsePermissionRequestFromText(text: string): {
    ...
    if (!text)
        return null;
    const fenced = text.match(/```(?:json)?\s{0,32}\n?(\{[\s\S]*?\})\s{0,32}\n?```/);
