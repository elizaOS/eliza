...
import { defaultStateFor } from "./permission-card.helpers";
import { getPermissionLabel } from "./permission-card.helpers";
import { openPermissionSettings } from "@elizaos/core/utils/permission-deep-links";
import { parseFeatureRef } from "./permission-card.helpers";
import { type IPermissionsRegistry } from "@elizaos/core/contracts/permissions";
import { type PermissionCardFallbackChoice } from "./permission-card.helpers";
import { type PermissionCardLabels } from "./permission-card.helpers";
...
export function PermissionCard({ permission, reason, feature, fallbackOffered = false, fallbackLabel, registry, initialState, onDismiss, onFallback, onGranted, onOpenSettings, labels = {}, className, }: PermissionCardProps): React.ReactElement | null {
    const { appName } = useBranding();
    const [state, setState] = useState<PermissionState>(initialState ?? registry?.get(permission) ?? defaultStateFor(permission));
    ...
    if (state.status === "granted") {
        return (<Badge variant="outline" tone="success" data-testid="permission-card-granted" className={cn("mt-2", className)}>
        {labels.granted ?? "Access granted"} ✓
      </Badge>);
    }
    const isRestrictedEntitlement = state.status === "restricted" &&
        state.restrictedReason === "entitlement_required";
    const isRestrictedUnavailable = state.status === "restricted" && !isRestrictedEntitlement;
    const isLimited = state.status === "limited";
    const canOpenSettingsInstead = state.canRequest === false &&
        (state.status === "denied" ||
    ...
    const resolvedFallbackLabel = fallbackLabel ??
        (permission === "reminders" ? "Use internal reminder" : "Use fallback");
    return (<Card variant="insetPadded" stack="compact" role="region" data-testid="permission-card" data-permission={permission} data-feature={feature} data-status={state.status} aria-label={`Permission request: ${title}`} className={cn("mt-2", className)}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {isRestrictedEntitlement ? (<Button variant="default" size="sm" disabled data-testid="permission-card-primary" title="Coming soon: requires app entitlement.">
            {labels.comingSoon ?? "Coming soon: requires app entitlement"}
          </Button>) : isRestrictedUnavailable || state.status === "not-applicable" ? (<Button variant="default" size="sm" disabled data-testid="permission-card-primary">
            {labels.unavailable ?? "Unavailable on this platform"}
          </Button>) : canOpenSettingsInstead ? (<Button variant="default" size="sm" onClick={handleOpenSettings} data-testid="permission-card-primary">
            {labels.openSettings ?? "Open System Settings"}
          </Button>) : (<Button variant="default" size="sm" onClick={() => void handleGrant()} disabled={requesting || !registry} data-testid="permission-card-primary">
            {requesting
        ...
        {registry ? (<Button variant="outline" size="sm" onClick={() => void handleCheckAgain()} disabled={checking || requesting} data-testid="permission-card-check-again">
            {checking ? "Checking…" : "Check again"}
          </Button>) : null}
        {fallbackOffered ? (<Button variant="outline" size="sm" onClick={handleFallback} data-testid="permission-card-fallback">
            {resolvedFallbackLabel}
          </Button>) : null}
        <Button variant="mutedLink" size="content" onClick={handleDismiss} data-testid="permission-card-dismiss" className="ml-auto">
...
function platformSettingsLabel(platform: PermissionState["platform"], appName: string): string {
    if (platform === "darwin")
    ...
    if (platform === "win32")
        return "Windows privacy settings";
    if (platform === "linux")
        return "system privacy settings";
    return "browser settings";
}
function permissionGuidance(permission: PermissionId, state: PermissionState, appName: string): {
    primary: string;
    secondary: string;
} {
    const title = getPermissionLabel(permission);
    ...
    if (state.status === "limited") {
        if (permission === "calendar") {
            return {
                primary: "Apple Calendar has add-only access: new events can go to the default calendar.",
                secondary: "Existing calendars and events remain private. Upgrade to full access only if this feature must read, update, or delete them.",
            };
        }
        return {
    ...
    if (state.status === "denied" || state.canRequest === false) {
        return {
            ...
            secondary: "After enabling it, return here and choose Check again. If you changed your mind, you can leave it off and use any offered fallback.",
        };
    }
    if (state.status === "restricted") {
        return {
            primary: `${title} is blocked by the current OS policy or app entitlement.`,
            secondary: "This device may need a different build, entitlement, profile, or administrator setting before the feature can work.",
        };
    }
    if (state.status === "not-applicable") {
        return {
            primary: `${title} is not available on this platform.`,
            secondary: "The agent can continue only if there is a fallback that does not use this device capability.",
        };
    }
    if (permission === "usage-access") {
        return {
            primary: `Open Android Usage Access and allow ${appName}.`,
            secondary: "This lets app blocking and Screen Time features read foreground app usage. Return here and choose Check again when done.",
        };
    }
    if (permission === "overlay") {
        return {
            primary: `Allow ${appName} to draw over other apps in Android settings.`,
            secondary: "The blocking screen needs this to appear above distracting apps. If you cancel, press Grant access again.",
        };
    }
    if (permission === "write-settings") {
        return {
            primary: `Open Android Write Settings and allow ${appName}.`,
            secondary: "Android requires this separate settings screen for brightness and device-setting changes.",
        };
    }
    return {
