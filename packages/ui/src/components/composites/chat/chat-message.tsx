...
import { COPY_HOLD_MS } from "../../../gestures";
import { ChatBubble } from "./chat-bubble";
import { ChatMessageActionSurface } from "./chat-message-actions";
import { ChatMessageActions } from "./chat-message-actions";
import { ChatVoiceSpeakerBadge } from "./chat-source";
import { Check } from "lucide-react";
import { GLASS_EASE } from "./chat-bubble";
import { LoaderCircle } from "lucide-react";
import { Message as MessageRow } from "../../ui/message";
import { MessageContent as MessageRowContent } from "../../ui/message";
import { MessageFooter as MessageRowFooter } from "../../ui/message";
...
import { renderChatReactionEmoji } from "./chat-source.helpers";
import { resolveChatVoiceSpeakerLabel } from "./chat-source.helpers";
import { type ChatMessageData } from "./chat-types";
import { type ChatMessageLabels } from "./chat-types";
import { type ChatMessageReaction } from "./chat-types";
import { type ChatMessageRenderContext } from "./chat-types";
...
import { usePointerPressAndHold } from "../../../gestures";
...
export interface ChatMessageProps {
    ...
    renderContent?: (message: ChatMessageData, ctx?: ChatMessageRenderContext) => React.ReactNode;
...
const HOVER_MEDIA_QUERY = "(min-width: 768px) and (hover: hover) and (pointer: fine)";
...
function subscribeSupportsHover(listener: () => void): () => void {
    ...
    if (mediaQuery && hoverSupportListeners.size === 1) {
        const notify = () => {
            for (const current of hoverSupportListeners)
                current();
        };
        if (typeof mediaQuery.addEventListener === "function") {
            mediaQuery.addEventListener("change", notify);
            hoverMediaQueryUnsubscribe = () => mediaQuery.removeEventListener("change", notify);
        }
        else {
...
function useSupportsHover(): boolean {
    return useSyncExternalStore(subscribeSupportsHover, readSupportsHover, () => true);
}
/**
 ...
 */
export function buildReplyTargetFromMessage(message: ChatMessageData, agentName: string): {
    messageId: string;
    senderName: string;
    snippet: string;
} {
...
function resolveSenderHandle(message: ChatMessageData, displayName: string | null): string | null {
    const handle = normalizeSenderHandle(message.fromUserName);
    if (!handle)
        return null;
    if (displayName?.replace(/^@/, "").toLowerCase() ===
        handle.slice(1).toLowerCase()) {
        return null;
    }
    return handle;
}
function resolveReplySenderDisplayName(message: ChatMessageData, replyTarget?: ChatMessageData | null): string | null {
    if (replyTarget) {
        const targetDisplayName = resolveSenderDisplayName(replyTarget);
        if (targetDisplayName)
            return targetDisplayName;
    }
    const replyToSenderName = typeof message.replyToSenderName === "string"
...
function normalizeMessageReactions(reactions: ChatMessageReaction[] | undefined): ChatMessageReaction[] {
    if (!Array.isArray(reactions)) {
        return [];
    }
    return reactions.filter((reaction) => typeof reaction?.emoji === "string" &&
...
function ReactionStrip({ alignRight, reactions, }: {
    alignRight: boolean;
    ...
    return (<div className={cn("mt-2 flex flex-wrap gap-1.5", alignRight ? "justify-end" : "justify-start")}>
      {reactions.map((reaction) => {
            const title = Array.isArray(reaction.users) && reaction.users.length > 0
                ? reaction.users.join(", ")
                : undefined;
            return (<Badge variant="outline" key={`${reaction.emoji}:${reaction.count}`} data-testid="chat-reaction-badge" title={title}>
...
function isNestedInteractiveTarget(currentTarget: HTMLElement, target: EventTarget | null): boolean {
...
function messageHasInteractiveWidget(content: string): boolean {
    return (findChoiceRegions(content).length > 0 ||
...
function useCopiedFlash(durationMs: number): [
    boolean,
    () => void
] {
...
function arePropsEqual(prev: ChatMessageProps, next: ChatMessageProps): boolean {
    const sharedEqual = prev.isGrouped === next.isGrouped &&
...
export const ChatMessage = memo(function ChatMessage({ message, actionAccessory, afterBubbleContent, appearance = "panel", isGrouped = false, agentName = "Agent", children, enterOnMount = false, labels = {}, onCopy, onSpeak, onEdit, onDismissSuggestion, onAcceptSuggestion, onLongPressCopy, onRetry, onReply, playing = false, reduceMotion = false, replyTarget = null, renderContent, renderContext, userMessagesOnRight = true, }: ChatMessageProps) {
    ...
    const canPlay = Boolean(!isUser && !isFirstRun && typeof onSpeak === "function" && trimmedText);
    ...
    const showVoiceSpeakerBadge = isUser &&
        !isGrouped &&
        Boolean(message.voiceSpeaker) &&
        Boolean(voiceSpeakerLabel) &&
        voiceSpeakerLabel !== senderDisplayName;
    const replyTargetId = typeof message.replyToMessageId === "string"
    ...
    const showReplyReference = Boolean(!isEditing && replyTargetId && normalizedSource);
    const showSenderHeader = isUser && !isGrouped && Boolean(senderDisplayName || senderHandle);
    ...
    const focusMessageSurface = useCallback(() => {
        ...
        const focusTarget = glass
            ? messageElement?.querySelector<HTMLElement>('[data-chat-message-bubble="true"]')
            : messageElement;
    ...
    const canHoldCopy = glass && isAssistant && !!onLongPressCopy && trimmedText.length > 0;
    ...
    const handleStartEditing = useCallback(() => {
        if (!canEdit || savingEdit)
            return;
        const bubble = articleRef.current?.querySelector<HTMLElement>('[data-chat-message-bubble="true"]');
    ...
    const handleTapReveal = useCallback((event: TouchEvent<HTMLElement>) => {
        const tapStart = tapStartRef.current;
        tapStartRef.current = null;
        if (supportsHover || isEditing)
            return;
    ...
    const handleActionsMouseLeave = useCallback((event: MouseEvent<HTMLElement>) => {
        const activeElement = event.currentTarget.ownerDocument.activeElement;
        const keyboardFocusWithin = activeElement instanceof HTMLElement &&
    ...
    const handleActionsPointerMove = useCallback((event: React.PointerEvent<HTMLElement>) => {
    ...
    const handleReplyReferenceClick = useCallback((event: MouseEvent<HTMLAnchorElement>) => {
        if (!replyTargetId || typeof document === "undefined")
            return;
        const target = document.getElementById(getChatMessageAnchorId(replyTargetId));
        if (!target)
            return;
        event.preventDefault();
        target.scrollIntoView({ behavior: "smooth", block: "center" });
    }, [replyTargetId]);
    const editSaveDisabled = savingEdit || !draftText.trim() || draftText.trim() === message.text.trim();
    const inlineEditControls = (<ChatMessageActionSurface bare={glass} data-testid={glass ? "thread-line-edit-controls" : "chat-message-edit-controls"} className={cn(glass ? "gap-0.5" : "gap-0 px-1.5 py-0.5", !glass && "absolute right-0 top-full z-30 mt-1")}>
      {glass ? (<>
          <Button variant="ghostMuted" size="icon-sm" aria-label={labels.cancel ?? "Cancel"} title={labels.cancel ?? "Cancel"} data-testid="thread-line-edit-cancel" onClick={(event) => {
                event.stopPropagation();
                handleCancelEditing();
            }} disabled={savingEdit} className="keyboard-focus-emphasis transition-[color,transform] duration-150 active:scale-95">
            <X className="size-3.5"/>
          </Button>
          <Button variant="ghostMuted" size="icon-sm" aria-label={savingEdit
                ? (labels.saving ?? "Saving...")
                : (labels.send ?? "Send")} title={savingEdit ? undefined : (labels.send ?? "Send")} data-testid="thread-line-edit-save" onClick={(event) => {
                event.stopPropagation();
                void handleSaveEdit();
            }} disabled={editSaveDisabled} className="keyboard-focus-emphasis transition-[color,transform] duration-150 active:scale-95">
            {savingEdit ? (<LoaderCircle aria-hidden="true" className="size-3.5 animate-spin motion-reduce:animate-none"/>) : (<Check className="size-3.5"/>)}
          </Button>
        </>) : (<>
          <Button variant="ghostMuted" size="tiny" onClick={handleCancelEditing} disabled={savingEdit} className="keyboard-focus-emphasis transition-colors duration-150">
            {labels.cancel ?? "Cancel"}
          </Button>
          <Separator aria-hidden orientation="vertical" className="mx-0.5 h-3.5"/>
          <Button variant="ghostMuted" size="tiny" onClick={() => void handleSaveEdit()} disabled={editSaveDisabled} className="keyboard-focus-emphasis transition-colors duration-150">
            {savingEdit
                ? (labels.saving ?? "Saving…")
    ...
    const inlineEditor = (<div data-testid={glass ? "thread-line-inline-editor" : "chat-message-inline-editor"} className="relative min-w-0">
      <Textarea ref={editTextareaRef} aria-label={labels.edit ?? "Edit message"} data-testid={glass ? "thread-line-edit-input" : "chat-message-edit-input"} value={draftText} onChange={(event) => setDraftText(event.target.value)} onKeyDown={handleEditKeyDown} rows={Math.min(6, Math.max(1, draftText.split("\n").length))} variant="mobileComposer" density="singleLine" disabled={savingEdit}/>
    ...
    if (glass) {
        ...
        if (isAssistant &&
            ...
            return (<motion.div ref={articleRef} id={getChatMessageAnchorId(message.id)} data-testid="thread-line" data-role={message.role} data-failure={message.failureKind} data-interrupted={message.interrupted ? "true" : undefined} initial={initial} animate={{ opacity: 1 }} transition={transition} className="mb-2.5 flex w-full justify-start">
        ...
        const accessoryVisible = actionsVisible || isEditing || Boolean(actionAccessory);
        const hiddenActionLane = {
            opacity: 0,
            y: reduceMotion ? 0 : 4,
            scale: reduceMotion ? 1 : 0.98,
        };
        const timestampAccessory = typeof message.timestamp === "number" &&
            Number.isFinite(message.timestamp) ? (<RelativeTime ts={message.timestamp} short data-testid="thread-line-timestamp" className="inline-block min-w-[3ch] whitespace-nowrap text-left text-xs-tight tabular-nums text-white/45"/>) : null;
        const trailingAccessory = timestampAccessory || actionAccessory ? (<div className="flex min-w-0 items-center gap-1.5">
          {actionAccessory}
          {timestampAccessory}
        </div>) : undefined;
        if (isEditing)
            accessoryModeRef.current = "edit";
        else if (actionsVisible)
            accessoryModeRef.current = "actions";
        ...
        const hasInteractiveWidget = isAssistant && messageHasInteractiveWidget(message.text);
        ...
        const handleBubbleClick = (e: MouseEvent<HTMLDivElement>) => {
            if (!bubbleInteractive)
                return;
            if (isNestedInteractiveTarget(e.currentTarget, e.target))
                return;
        ...
        const bubbleContent = isUser && isEditing ? (inlineEditor) : (<>
          {isSuggestion ? (
            ...
            <div className="mb-1.5 flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                {onAcceptSuggestion ? (<Button variant="surfaceAccent" size="badge" data-testid="thread-line-suggestion-accept" title="Do it" aria-label="Do it" onClick={(e) => {
                        e.stopPropagation();
                        onAcceptSuggestion(message);
                    }}>
                    Do it
                  </Button>) : null}
                {onDismissSuggestion ? (<Button variant="ghostMuted" size="icon-sm" data-testid="thread-line-suggestion-dismiss" title="Dismiss suggestion" aria-label="Dismiss suggestion" onClick={(e) => {
                        e.stopPropagation();
          ...
          <div data-chat-selectable="true" className={cn(isFirstRun &&
                "flex w-full flex-col gap-4 whitespace-normal text-chat-lead")}>
            {renderContent?.(message, renderContext) ??
                children ??
                message.text}
          </div>
          {isAssistant && message.interrupted ? (<div className="mt-2">
        ...
        const bubbleAppearance = isFirstRun
            ...
            : isSuggestion
                ? "suggestion"
                : "default";
        return (<MotionMessageRow ref={articleRef} id={getChatMessageAnchorId(message.id)} align={isUser ? "end" : "start"} data-testid="thread-line" data-role={message.role} data-failure={isAssistant ? message.failureKind : undefined} data-interrupted={isAssistant && message.interrupted ? "true" : undefined} 
        // A very short opacity-only entrance keeps fast-model turns immediate
        // without fighting the scroller's bottom anchor.
        initial={initial} animate={{ opacity: 1 }} transition={transition} className="mb-0" onPointerMove={supportsHover && hasActions ? handleActionsPointerMove : undefined} onMouseLeave={supportsHover && hasActions ? handleActionsMouseLeave : undefined} onFocusCapture={hasActions ? handleActionsFocus : undefined} onBlurCapture={hasActions
                ? (event) => {
        ...
        <MessageRowContent className={cn("relative flex flex-col", 
            ...
            hasActionLane &&
                "transition-[padding-bottom] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:duration-100", hasActionLane &&
                (supportsHover
                    ? "pb-6"
                    : accessoryVisible
                ...
                : isUser
                    ? "max-w-[80%] items-end"
                    : "w-full items-start")}>
          {bubbleInteractive ? (<ChatBubble appearance={bubbleAppearance} variant="glass" source={normalizedSource} bare={isFlatAssistant} tone={isUser ? "user" : "assistant"} {...(holdHandlers ?? {})} role="button" tabIndex={0} aria-label={actionsVisible ? "Hide message actions" : "Show message actions"} aria-expanded={actionsVisible} onClick={handleBubbleClick} onKeyDown={handleBubbleKeyDown} className={bubbleExtraClassName} style={isEditing && editBubbleWidth
                    ? { width: editBubbleWidth, maxWidth: "100%" }
                    : undefined} data-chat-message-bubble="true" data-proactive-suggestion={isSuggestion ? "true" : undefined}>
              {bubbleContent}
            </ChatBubble>) : (<ChatBubble appearance={bubbleAppearance} variant="glass" source={normalizedSource} bare={isFlatAssistant} tone={isUser ? "user" : "assistant"} tabIndex={-1} aria-label={`${isUser ? "Your" : agentName} message`} {...(holdHandlers ?? {})} className={bubbleExtraClassName} style={isEditing && editBubbleWidth
                    ? { width: editBubbleWidth, maxWidth: "100%" }
                    : undefined} data-chat-message-bubble="true" data-proactive-suggestion={isSuggestion ? "true" : undefined}>
              {bubbleContent}
            </ChatBubble>)}
          {afterBubbleContent ? (<div className="mt-2 w-full max-w-[13.5rem] pointer-events-auto" data-testid="thread-line-after-bubble">
              {afterBubbleContent}
            </div>) : null}
          {hasActionLane ? (<motion.div data-testid="thread-line-actions" aria-hidden={!accessoryVisible} inert={!accessoryVisible} initial={accessoryVisible ? false : hiddenActionLane} animate={accessoryVisible
                    ? { opacity: 1, y: 0, scale: 1 }
                    : hiddenActionLane} transition={{
                    duration: reduceMotion ? 0.1 : 0.2,
                    ease: GLASS_EASE,
                }} className={cn("absolute bottom-0 z-10 min-w-0", isUser ? "right-0 origin-top-right" : "left-0 origin-top-left", 
              ...
              <MessageRowFooter className="flex items-center p-0 text-white/70">
                <motion.div key={accessoryMode} className="flex" initial={reduceMotion
                    ? false
                    : { opacity: 0, transform: "translateY(2px)" }} animate={{ opacity: 1, transform: "translateY(0px)" }} transition={{
                    duration: reduceMotion ? 0.08 : 0.14,
                    ease: GLASS_EASE,
                }}>
                  {accessoryMode === "edit" ? (inlineEditControls) : (<ChatMessageActions appearance="glass-row" canEdit={canEdit} canPlay={canPlay} canReply={canReply} copied={copied} labels={labels} onCopy={canRowCopy ? handleCopy : undefined} onEdit={handleStartEditing} onPlay={() => onSpeak?.(message.id, message.text)} onReply={handleReply} playing={playing} trailingAccessory={trailingAccessory}/>)}
                </motion.div>
              </MessageRowFooter>
            </motion.div>) : null}
          {/* Recovery stays visible beside the failed turn. */}
          {canRetry ? (<Button variant="surfaceAccent" size="badge" data-testid="thread-line-retry" aria-label={message.replyRecoveryAvailable ? "Regenerate reply" : "Retry"} disabled={retryPending} onClick={async (e) => {
                    e.stopPropagation();
                    if (retryPending)
                        return;
    ...
    return (<MessageRow ref={articleRef} id={getChatMessageAnchorId(message.id)} role="article" align={isRightAligned ? "end" : "start"} className={`items-start sm:gap-3 ${isGrouped ? "mt-0.5" : "mt-1.5"} ${enterOnMount
            ? "motion-safe:animate-[chat-turn-in_320ms_cubic-bezier(0.22,1,0.36,1)]"
            : ""}`} data-testid="chat-message" data-role={message.role} data-interrupted={isAssistant && message.interrupted ? "true" : undefined} tabIndex={isFirstRun ? undefined : 0} onPointerMove={supportsHover ? handleActionsPointerMove : undefined} onMouseLeave={supportsHover ? handleActionsMouseLeave : undefined} onFocusCapture={!isFirstRun ? handleActionsFocus : undefined} onBlurCapture={!isFirstRun
            ? (event) => {
                if (!event.currentTarget.contains(event.relatedTarget)) {
                    setShowActions(false);
                }
            }
            : undefined} onTouchStart={handleTapStart} onTouchEnd={handleTapReveal} aria-label={`${isUser && showSenderHeader
            ? senderPrimaryLabel
            : isUser
                ? userMessagesOnRight
                    ? "Your"
                    : senderPrimaryLabel
                : agentName} message`}>
      <div className={`max-w-[88%] min-w-0 sm:max-w-[80%] ${isRightAligned ? "mr-1" : "ml-1"}`}>
        {!isUser && !isGrouped ? (<div className={cn("text-xs font-semibold text-accent", isRightAligned ? "text-right" : "text-left")}>
            {agentName}
          </div>) : null}
        {isUser && !isGrouped && !showSenderHeader ? (<div className={cn("flex items-center gap-1.5 text-xs font-semibold text-accent", isRightAligned ? "justify-end" : "justify-start")}>
            <span>You</span>
            {showVoiceSpeakerBadge ? (<ChatVoiceSpeakerBadge speaker={message.voiceSpeaker} data-testid={`chat-message-voice-speaker-${message.id}`}/>) : null}
          </div>) : null}
        {showSenderHeader ? (<div className={cn("flex items-center gap-2", isRightAligned ? "justify-end" : "justify-start")}>
            <div className={cn("min-w-0", isRightAligned ? "text-right" : "text-left")}>
              ...
              {senderHandle ? (<div className="truncate text-xs-tight text-muted">
                  {senderHandle}
                </div>) : null}
            </div>
          </div>) : null}
        <ChatBubble appearance={isSuggestion ? "suggestion" : "default"} tone={isUser ? "user" : "assistant"} source={normalizedSource} className="relative group py-1 font-chat text-chat-body whitespace-pre-wrap break-words" style={isEditing && editBubbleWidth
            ? { width: editBubbleWidth, maxWidth: "100%" }
            : undefined} data-chat-message-bubble="true" data-proactive-suggestion={isSuggestion ? "true" : undefined}>
          {isSuggestion && !isEditing ? (<div className="mb-1.5 flex items-center justify-between gap-2">
              <span className="inline-flex items-center gap-1 text-xs-tight font-medium text-accent/85">
                <Sparkles className="size-3.5" aria-hidden="true"/>
                {labels.suggestion ?? "Suggestion"}
              </span>
              <div className="flex items-center gap-1">
                {onAcceptSuggestion ? (<Button variant="surfaceAccent" size="micro" onClick={() => onAcceptSuggestion(message)} title={labels.acceptSuggestion ?? "Do it"} aria-label={labels.acceptSuggestion ?? "Do it"}>
                    {labels.acceptSuggestion ?? "Do it"}
                  </Button>) : null}
                {onDismissSuggestion ? (<Button variant="ghostMuted" size="icon-sm" onClick={() => onDismissSuggestion(message.id)} title={labels.dismiss ?? "Dismiss suggestion"} aria-label={labels.dismiss ?? "Dismiss suggestion"}>
                    <X className="size-3.5"/>
                  </Button>) : null}
              </div>
            </div>) : null}
          {showReplyReference ? (<Button asChild variant="externalLink" size="content" className="mb-2 block">
              <a href={`#${getChatMessageAnchorId(replyTargetId)}`} onClick={handleReplyReferenceClick}>
                {replyReferenceLabel}
              </a>
            </Button>) : null}
          {isEditing
          ...
          {!isUser && message.interrupted ? (<div className="mt-2">
          ...
          {!isEditing && !isFirstRun ? (<div data-testid="chat-message-action-rail" aria-hidden={!actionsVisible} inert={!actionsVisible} className={cn("absolute top-0 flex items-center gap-1 transition-opacity duration-200", 
            ...
            isRightAligned
                ...
                : "right-1 sm:right-0 sm:translate-x-full", actionsVisible
                ? "opacity-100"
                : "pointer-events-none opacity-0")}>
              <ChatMessageActions canEdit={canEdit} canPlay={canPlay} canReply={canReply} copied={copied} labels={labels} onCopy={handleCopy} onEdit={handleStartEditing} onPlay={() => onSpeak?.(message.id, message.text)} onReply={handleReply}/>
            </div>) : null}
        </ChatBubble>
        <ReactionStrip alignRight={isRightAligned} reactions={visibleReactions}/>
      </div>
    </MessageRow>);
}, arePropsEqual);
