...
import { exchangeAuthenticatedNativeCloudPairToken } from "../components/auth/CloudPairRelay";
...
export type AgentSessionRecoveryResult = {
    ...
    ok: false;
    reason: "not-ready" | "unauthorized" | "manage-required" | "cancelled" | "error";
    message: string;
};
export interface RunAgentSessionRecoveryDeps {
    ...
    exchangePairToken?: (token: string, binding: {
        cloudToken: string;
        agentId: string;
...
function pairTokenFromRedirectUrl(redirectUrl: string): string | null {
    try {
        ...
        if (parsed.pathname.replace(/\/+$/, "") !== "/pair")
            return null;
...
function classifyNativePairExchangeError(error: unknown): Extract<AgentSessionRecoveryResult, {
    ok: false;
}> {
    ...
    if (error.code === "sandbox_credential_unavailable" ||
 ...
 */
export async function runAgentSessionRecovery(deps: RunAgentSessionRecoveryDeps): Promise<AgentSessionRecoveryResult> {
    const { cloudApiBase, agentId, cloudToken, navigate, consumeRedirectInProcess = false, exchangePairToken = (token, binding) => exchangeAuthenticatedNativeCloudPairToken(token, {
        ...binding,
        cloudApiBase,
    }), persistPairApiToken = persistCloudPairApiToken, clearStalePairCredentials, isRecoveryTargetCurrent, commitPairedInProcess, onPairedInProcess, fetchFn = fetch, sleepFn = realSleep, nowFn = Date.now, signal, } = deps;
    ...
    while (nowFn() < deadline) {
        ...
        const requiresCloudManagement = res.status === 402 ||
            res.status === 403 ||
            res.status === 404 ||
        ...
        if (!res.ok) {
            return {
                ok: false,
                reason: "error",
                message: data.error || `Failed to generate pairing token (HTTP ${res.status})`,
            };
        }
            ...
            if (consumeRedirectInProcess) {
                ...
                try {
                    ...
                    if (pairedSession.agentId.toLowerCase() !== agentId.toLowerCase()) {
                        throw new CloudPairExchangeError("Cloud paired a different agent than the requested recovery target.", 403, "pairing_agent_mismatch");
                    }
