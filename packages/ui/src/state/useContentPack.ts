 ...
 */
import { applyColorScheme } from "../content-packs";
import { applyContentPack } from "../content-packs";
import { loadContentPackFromFiles } from "../content-packs";
import { loadContentPackFromUrl } from "../content-packs";
import { loadPersistedActivePackUrl } from "./persistence";
import { releaseLoadedContentPack } from "../content-packs";
import { savePersistedActivePackUrl } from "./persistence";
...
export function useContentPack(): UseContentPackResult {
    const { setState, activePackId, selectedVrmIndex, customVrmUrl, customVrmPreviewUrl, customBackgroundUrl, customWorldUrl, firstRunName, firstRunStyle, } = useAppSelectorShallow((s) => ({
        setState: s.setState,
    ...
    const activate = useCallback((pack: ResolvedContentPack) => {
    ...
    const toggle = useCallback((pack: ResolvedContentPack) => {
    ...
    const loadFromUrl = useCallback(async (url: string) => {
        ...
        try {
            ...
            if (controller.signal.aborted)
                return;
            setError(`Failed to load pack: ${err instanceof Error ? err.message : "Unknown error"}`);
        }
        finally {
            if (urlLoadControllerRef.current === controller) {
                urlLoadControllerRef.current = null;
    ...
    const activePack = useMemo(() => activePackId
        ? (loadedPacks.find((p) => p.manifest.id === activePackId) ?? null)
        : null, [activePackId, loadedPacks]);
    return {
        activePack,
        loadedPacks,
        error,
