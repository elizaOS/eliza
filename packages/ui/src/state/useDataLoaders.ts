...
import { buildAutonomyGapReplayRequests } from "./autonomy";
import { clearSettledPendingChatTurns } from "./pending-chat-turns";
import { client } from "../api";
import { filterRenderableConversationMessages } from "./internal";
import { hasPendingAutonomyGaps } from "./autonomy";
import { logger } from "@elizaos/ui/logger";
import { markConversationHistoryApplied } from "./conversation-hydration-readiness";
import { markPendingAutonomyGapsPartial } from "./autonomy";
import { mergeAutonomyEvents } from "./autonomy";
import { normalizeConversationList } from "./chat-conversation-guards";
import { normalizeOwnerName } from "@elizaos/ui/utils/owner-name";
...
import { shouldKeepConversationMessage } from "./internal";
...
import { type BscTradeExecuteRequest } from "../api";
import { type BscTradeExecuteResponse } from "../api";
import { type BscTradePreflightResponse } from "../api";
import { type BscTradeQuoteRequest } from "../api";
import { type BscTradeQuoteResponse } from "../api";
import { type BscTradeTxStatusResponse } from "../api";
import { type BscTransferExecuteRequest } from "../api";
import { type BscTransferExecuteResponse } from "../api";
import { type CharacterData } from "../api";
import { type Conversation } from "../api";
import { type ConversationMessage } from "../api";
import { type LoadConversationMessagesResult } from "./internal";
import { type RefObject } from "react";
import { type StewardWebhookEventType } from "../api";
import { type StreamEventEnvelope } from "../api";
import { type StreamingTextModification } from "./internal";
import { type StylePreset } from "../api";
import { type UiLanguage } from "../i18n";
import { type UpdateStatus } from "../api";
import { type WalletTradingProfileResponse } from "../api";
import { type WalletTradingProfileSourceFilter } from "../api";
import { type WalletTradingProfileWindow } from "../api";
import { type WorkbenchOverview } from "../api";
...
function hasConversationBootstrapMessage(messages: ConversationMessage[]): boolean {
    return messages.some((message) => message.role === "assistant" && shouldKeepConversationMessage(message));
}
function localConversationMessageLineage(message: ConversationMessage): string | null {
...
function indexConversationMessagesByLineage(messages: readonly ConversationMessage[]): Map<string, ConversationMessage> {
    const indexed = new Map<string, ConversationMessage>();
    for (const message of messages) {
        const lineage = localConversationMessageLineage(message);
        if (lineage)
            indexed.set(lineage, message);
    }
    return indexed;
}
function captureRegisteredConversationMessageOverlay(overlay: ConversationMessageOverlay, currentMessages: readonly ConversationMessage[]): void {
...
function rememberResolvedOverlayUser(record: ConversationMessageOverlayRecord, message: ConversationMessage): void {
...
function overlayMessages(overlay: ConversationMessageOverlay | undefined): ConversationMessage[] {
    if (!overlay)
        return [];
...
function snapshotOverlayRevisions(overlay: ConversationMessageOverlay | undefined): Map<string, number> {
    return new Map(overlay
        ? [...overlay].map(([lineage, record]) => [lineage, record.revision])
        : []);
}
const LOCAL_TURN_MATCH_SLACK_MS = 60000;
function findMatchingServerMessageIndex(serverMessages: ConversationMessage[], localMessage: ConversationMessage, usedIndexes: Set<number>, options?: {
    afterIndex?: number;
}): number {
    ...
    const startIndex = typeof options?.afterIndex === "number" ? options.afterIndex + 1 : 0;
    let bestIndex = -1;
    ...
    for (let index = startIndex; index < serverMessages.length; index += 1) {
        ...
        if (!serverMessage || serverMessage.role !== localMessage.role)
            continue;
        if (serverMessage.timestamp < minTimestamp)
            continue;
        if (serverMessage.timestamp > maxTimestamp)
            continue;
        if (serverMessage.text.trim() !== text)
            continue;
        const timestampDelta = Math.abs(serverMessage.timestamp - localMessage.timestamp);
...
function isLocalOverlayMessage(message: ConversationMessage, overlayLineages: ReadonlySet<string>): boolean {
    const lineage = localConversationMessageLineage(message);
    return lineage !== null && overlayLineages.has(lineage);
}
function findNearestPriorLocalOverlayUser(messages: ConversationMessage[], fromIndex: number, overlayLineages: ReadonlySet<string>): ConversationMessage | null {
...
function findNearestPriorUser(messages: ConversationMessage[], fromIndex: number): ConversationMessage | null {
    for (let index = fromIndex - 1; index >= 0; index -= 1) {
        const message = messages[index];
        if (message?.role === "user")
            return message;
...
function resolvedLocalOverlayServerIndexes(serverMessages: ConversationMessage[], currentMessages: ConversationMessage[], overlay: ConversationMessageOverlay): Map<string, number> {
    ...
    currentMessages.forEach((message) => {
        if (isLocalOverlayMessage(message, overlayLineages))
            return;
    ...
    currentMessages.forEach((message) => {
        ...
        if (precedingIds) {
            ...
            for (const index of currentServerUserIndexes)
                excludedIndexes.add(index);
        }
        const serverIndex = findMatchingServerMessageIndex(serverMessages, message, excludedIndexes);
        if (serverIndex < 0)
            return;
    ...
    currentMessages.forEach((message, index) => {
        ...
        const pairedUser = findNearestPriorLocalOverlayUser(currentMessages, index, overlayLineages);
        const matchedUserIndex = pairedUser
            ? serverUserIndexByLocalId.get(pairedUser.id)
            : undefined;
        const serverIndex = findMatchingServerMessageIndex(serverMessages, message, usedServerAssistantIndexes, typeof matchedUserIndex === "number"
...
function mergeMessagesChronologically(serverMessages: ConversationMessage[], localOverlay: ConversationMessage[], causalServerPredecessors?: ReadonlyMap<string, number>): ConversationMessage[] {
    if (localOverlay.length === 0)
        return serverMessages;
    const merged: Array<{
        message: ConversationMessage;
        serverIndex: number | null;
    }> = serverMessages.map((message, serverIndex) => ({ message, serverIndex }));
    const orderedOverlay = [...localOverlay].sort((left, right) => left.timestamp - right.timestamp);
    for (const message of orderedOverlay) {
        let insertionIndex = merged.findIndex((candidate) => candidate.message.timestamp > message.timestamp);
        if (insertionIndex < 0)
        ...
        const replyToPosition = message.role === "assistant" && message.replyToMessageId
            ? merged.findIndex((candidate) => candidate.message.role === "user" &&
        ...
        const lineage = localConversationMessageLineage(message);
        const predecessorServerIndex = lineage && !message.replyToMessageId
            ? causalServerPredecessors?.get(lineage)
            : undefined;
        if (typeof predecessorServerIndex === "number") {
            const predecessorPosition = merged.findIndex((candidate) => candidate.serverIndex === predecessorServerIndex);
...
function reconcileConversationMessagesWithOverlay(serverMessages: ConversationMessage[], overlay: ConversationMessageOverlay | undefined, options?: {
    ...
    const resolvedOverlayServerIndexes = options?.localContext && options.allowTextFallback
        ? resolvedLocalOverlayServerIndexes(serverMessages, options.localContext, overlay)
    ...
    if (localContext) {
        localContext.forEach((message, index) => {
            if (message.role !== "assistant")
                return;
            const lineage = localConversationMessageLineage(message);
            if (!lineage || !overlay.has(lineage))
                return;
            const precedingUser = findNearestPriorUser(localContext, index);
            if (!precedingUser)
                return;
            const predecessorServerIndex = serverIndexById.get(precedingUser.id) ??
    ...
    const preserveOwnedMessage = (serverIndex: number, lineage: string, ownedMessage: ConversationMessage) => {
    ...
    for (const [lineage, record] of overlay) {
        ...
        const newestRequestCanConsumeRevision = fullNewestRequestRevisions !== undefined &&
        ...
        if (typeof fallbackServerIndex === "number") {
            ...
            if (fullNewestRequestRevisions !== undefined &&
            ...
            matchedLineages.add(lineage);
            if (newestRequestCanConsumeRevision) {
                rememberResolvedOverlayUser(record, serverMessages[fallbackServerIndex]);
                overlay.delete(lineage);
            }
            else {
                preserveOwnedMessage(fallbackServerIndex, lineage, message);
        ...
        if (record.consecutiveNewestMisses >= OVERLAY_NEWEST_MISS_RETIREMENT_COUNT) {
            overlay.delete(lineage);
        }
    }
    return mergeMessagesChronologically(visibleServerMessages, overlayMessages(overlay).filter((message) => {
        const lineage = localConversationMessageLineage(message);
        return lineage === null || !matchedLineages.has(lineage);
    }), causalServerPredecessors);
}
function buildLocalizedCharacterPayload(preset: StylePreset, name?: string | null): CharacterData {
...
export function useDataLoaders(deps: DataLoadersDeps) {
    const { autonomousStoreRef, autonomousEventsRef, autonomousLatestEventIdRef, autonomousRunHealthByRunIdRef, autonomousReplayInFlightRef, setAutonomousEvents, setAutonomousLatestEventId, setAutonomousRunHealthByRunId, activeConversationIdRef, conversationMessagesRef, greetingFiredRef, setConversations, setActiveConversationId, setConversationMessages, loadWalletConfig, agentStatus, characterData, characterDraft, loadCharacter, selectedVrmIndex, firstRunComplete, uiLanguage, setOwnerNameState, } = deps;
    ...
    // ── Autonomy ────────────────────────────────────────────────────────
    const applyAutonomyEventMerge = useCallback((incomingEvents: StreamEventEnvelope[], replay = false) => {
    ...
    const fetchAutonomyReplay = useCallback(async () => {
        ...
        try {
            ...
            const gapResults = await Promise.allSettled(gapReplays.map((request) => client.getAgentEvents({
                runId: request.runId,
            ...
            for (const result of gapResults) {
                if (result.status === "rejected") {
                    logger.debug({ error: result.reason }, "[useDataLoaders] autonomy gap replay failed; retried on next poll cycle");
                    continue;
                }
    ...
    const conversationMessageCacheRef = useRef<Map<string, ConversationMessage[]>>(new Map());
    const conversationMessageOverlayRef = useRef<Map<string, ConversationMessageOverlay>>(new Map());
    ...
    const unownedConversationMessageOverlayRef = useRef<ConversationMessageOverlay>(new Map());
    ...
    const visibleConversationMessagesContentOwnerRef = useRef<string | null | undefined>(null);
    ...
    const canonicalNewestConversationMessageContentOwnerRef = useRef<string | null>(null);
    ...
    const textFallbackBlockedConversationIdsRef = useRef<Map<string, "around" | "mutation">>(new Map());
    ...
    const activeMessageLoadFenceRef = useRef<ConversationMessageLoadFence | null>(null);
    ...
    const conversationMessageCacheRequestTokenRef = useRef<Map<string, number>>(new Map());
    ...
    const invalidateConversationMessageCacheRequest = useCallback((id: string) => {
        conversationMessageCacheRequestSequenceRef.current += 1;
        conversationMessageCacheRequestTokenRef.current.delete(id);
    }, []);
    const finishConversationMessageCacheRequest = useCallback((id: string, requestToken: number) => {
        if (conversationMessageCacheRequestTokenRef.current.get(id) === requestToken) {
            conversationMessageCacheRequestTokenRef.current.delete(id);
        }
    }, []);
    const cacheConversationMessages = useCallback((id: string, serverMessages: ConversationMessage[], requestToken: number) => {
        if (conversationMessageCacheRequestTokenRef.current.get(id) !== requestToken) {
            return;
        }
    ...
    const captureVisibleConversationMessageOverlay = useCallback((owner: string | null) => {
        if (visibleConversationMessagesOwnerRef.current !== owner)
            return;
        if (visibleConversationMessagesContentOwnerRef.current !== owner)
            return;
        const overlay = owner === null
            ? unownedConversationMessageOverlayRef.current
            : conversationMessageOverlayRef.current.get(owner);
        if (!overlay)
            return;
        captureRegisteredConversationMessageOverlay(overlay, conversationMessagesRef.current);
        if (owner !== null && overlay.size === 0) {
            conversationMessageOverlayRef.current.delete(owner);
        }
    }, [conversationMessagesRef]);
    const isCurrentConversationMessageFence = useCallback((fence: ConversationMessageLoadFence): boolean => activeMessageLoadFenceRef.current === fence &&
        ...
        visibleConversationMessageRequestGenerationRef.current ===
            fence.visibleRequestGeneration, [activeConversationIdRef]);
    ...
    const claimConversationMessagesOwnership = useCallback((conversationId: string | null): number => {
        ...
        const targetOverlay = conversationId === null
            ? unownedConversationMessageOverlayRef.current
        ...
        const currentByLineage = indexConversationMessagesByLineage(conversationMessagesRef.current);
        const targetOwnsVisibleRegisteredRows = targetHasVisibleOverlay &&
            currentByLineage.size > 0 &&
            [...currentByLineage].every(([lineage, message]) => targetOverlay?.get(lineage)?.message === message);
        ...
        else if (conversationMessagesRef.current.length === 0 &&
    ...
    const registerConversationMessageOverlay = useCallback((conversationId: string | null, lineages: readonly string[], explicitMessages?: readonly ConversationMessage[]) => {
        const overlay = conversationId === null
            ? unownedConversationMessageOverlayRef.current
            : (conversationMessageOverlayRef.current.get(conversationId) ??
        ...
        const cached = conversationId === null
            ? undefined
            : conversationMessageCacheRef.current.get(conversationId);
        const visible = visibleConversationMessagesContentOwnerRef.current === conversationId
            ? conversationMessagesRef.current
            : [];
        const requestedLineages = new Set(lineages);
        const precedingUsers = [...(cached ?? []), ...visible].filter((message) => message.role === "user" &&
            !requestedLineages.has(localConversationMessageLineage(message) ?? ""));
        const precedingServerUserIds = (cached !== undefined ||
            conversationId === null ||
            ...
            (conversationId === null ||
                textFallbackBlockedConversationIdsRef.current.get(conversationId) !==
                    "mutation")
            ? new Set(precedingUsers
                .filter((message) => !message.id.startsWith("temp-"))
                .map((message) => message.id))
            : undefined;
        const precedingUserRecords = [...overlay]
            .filter(([lineage, record]) => !requestedLineages.has(lineage) && record.message?.role === "user")
            .map(([, record]) => record);
        const precedingUserContext = () => ({
            precedingServerUserIds: precedingServerUserIds === undefined
                ? undefined
        ...
        // visible transcript.
        const explicitByLineage = indexConversationMessagesByLineage(explicitMessages ?? []);
        for (const lineage of lineages) {
    ...
    const getConversationMessagesSnapshot = useCallback((conversationId: string): ConversationMessage[] | undefined => {
    ...
    const applyConversationMessageStream = useCallback((conversationId: string, changed: ConversationMessage[], removed: string[]) => {
        ...
        for (const row of changed)
            rows.set(row.id, row);
        const orderedRows = [...rows.values()].sort((a, b) => a.timestamp - b.timestamp);
        setConversationMessages(mergeMessagesChronologically(orderedRows.filter((row) => row.assistantEphemeral !== true), orderedRows.filter((row) => row.assistantEphemeral === true)));
        ...
        registerConversationMessageOverlay(conversationId, changed.flatMap((row) => {
    ...
    const isConversationMessagesOwnershipCurrent = useCallback((conversationId: string | null, generation: number): boolean => visibleConversationMessagesOwnerRef.current === conversationId &&
        conversationMessageOwnerGenerationRef.current === generation, []);
    const getConversationMessagesOwnershipGeneration = useCallback((): number => conversationMessageOwnerGenerationRef.current, []);
    const applyConversationMessageOverlayModification = useCallback((conversationId: string | null, lineage: string, modification: StreamingTextModification, options?: {
        onlyIfEmpty?: boolean;
    }) => {
        const overlay = conversationId === null
            ? unownedConversationMessageOverlayRef.current
        ...
        const nextMessage = nextMessages.find((message) => localConversationMessageLineage(message) === lineage) ?? null;
        if (nextMessage === record.message)
            return;
        if (!nextMessage) {
            overlay.delete(lineage);
            if (conversationId !== null && overlay.size === 0) {
    ...
    const removeConversationMessageStateMessages = useCallback((conversationId: string, mutation: ConversationMessageStateMutation) => {
        const { removedMessages } = mutation;
        const removedIds = new Set(removedMessages.map((message) => message.id));
        const removedLineages = new Set(removedMessages
            .map(localConversationMessageLineage)
            .filter((lineage): lineage is string => lineage !== null));
        if (mutation.mode === "delete-exact" &&
            removedIds.size === 0 &&
        ...
        textFallbackBlockedConversationIdsRef.current.set(conversationId, "mutation");
        if (canonicalNewestConversationMessageContentOwnerRef.current ===
            conversationId) {
        ...
        if (mutation.mode === "truncate") {
            ...
            if (cached) {
                conversationMessageCacheRef.current.set(conversationId, cached.filter((message) => {
        ...
        if (overlay) {
            const truncateCutoff = mutation.mode === "truncate"
                ? Math.min(...removedMessages.map((message) => message.timestamp))
                : null;
            for (const [lineage, record] of overlay) {
                const message = record.message;
                if (removedLineages.has(lineage) ||
        ...
        if (visibleConversationMessagesOwnerRef.current === conversationId) {
            ...
            else if (visibleConversationMessagesContentOwnerRef.current === conversationId) {
    ...
    const discardConversationMessageState = useCallback((conversationId?: string) => {
        ...
        if (activeMessageLoadFenceRef.current?.conversationId === conversationId) {
            activeMessageLoadFenceRef.current.controller.abort();
            activeMessageLoadFenceRef.current = null;
        }
        conversationMessageCacheRef.current.delete(conversationId);
    ...
    const loadConversationMessages = useCallback(async function loadNewestConversationMessages(convId: string, confirmGuardedRead = true): Promise<LoadConversationMessagesResult> {
        ...
        const visibleRequestGeneration = ++visibleConversationMessageRequestGenerationRef.current;
        ...
        if (cached) {
            ...
            const mergedCached = reconcileConversationMessagesWithOverlay(restoredServerMessages, overlay);
        ...
        captureVisibleConversationMessageOverlay(convId);
        const textFallbackBarrier = textFallbackBlockedConversationIdsRef.current.get(convId);
        const textFallbackContextIsCanonical = (canonicalNewestConversationMessageContentOwnerRef.current === convId ||
            loadedConversationIdRef.current === convId) &&
            visibleConversationMessagesContentOwnerRef.current === convId &&
        ...
        const fence: ConversationMessageLoadFence = {
            ...
            requestKind: "newest",
            overlayRevisionsAtStart: snapshotOverlayRevisions(conversationMessageOverlayRef.current.get(convId)),
        };
        ...
        try {
            ...
            if (!isCurrentConversationMessageFence(fence))
                return { ok: true };
            const serverMessages = restoreCapabilityHandoffs(filterRenderableConversationMessages(messages));
            captureVisibleConversationMessageOverlay(convId);
            const currentOverlay = conversationMessageOverlayRef.current.get(convId);
            const nextMessages = reconcileConversationMessagesWithOverlay(serverMessages, currentOverlay, {
            ...
            if (confirmGuardedRead &&
                !textFallbackContextIsCanonical &&
                textFallbackBarrier !== "mutation" &&
                currentOverlay &&
                isCurrentConversationMessageFence(fence)) {
                const candidates = resolvedLocalOverlayServerIndexes(serverMessages, nextMessages, currentOverlay);
                const hasUnresolvedUserCopy = [...currentOverlay].some(([lineage, record]) => record.message?.role === "user" &&
            ...
            if (status === 404) {
                ...
                const refreshed = await client
                    ...
                    logger.warn({ err: refreshErr }, "[useDataLoaders] conversation-list refresh after 404 failed");
                    return null;
                });
                ...
                if (refreshed) {
                    const normalized = normalizeConversationList(refreshed.conversations);
                    setConversations(normalized);
    ...
    const loadConversationMessagesAround = useCallback(async (convId: string, messageId: string): Promise<boolean> => {
        if (activeConversationIdRef.current !== convId ||
        ...
        try {
            ...
            const nextMessages = reconcileConversationMessagesWithOverlay(serverMessages, conversationMessageOverlayRef.current.get(convId), {
                localContext: conversationMessagesRef.current,
            ...
            logger.debug({ error }, `[useDataLoaders] around-window load failed for ${convId}`);
            return false;
        }
        finally {
            if (activeMessageLoadFenceRef.current === fence) {
    ...
    const prefetchConversationMessages = useCallback((ids: readonly string[]) => {
        for (const id of ids) {
            if (!id)
                continue;
            if (conversationMessageCacheRef.current.has(id))
                continue;
            if (prefetchAbortRef.current.has(id))
                continue;
            const controller = new AbortController();
            ...
            void client
                ...
                if (controller.signal.aborted)
                    return;
                cacheConversationMessages(id, filterRenderableConversationMessages(messages), cacheRequestToken);
            })
                .catch(() => {
    ...
    const getBscTradePreflight = useCallback(async (tokenAddress?: string): Promise<BscTradePreflightResponse> => client.getBscTradePreflight(tokenAddress), []);
    const getBscTradeQuote = useCallback(async (request: BscTradeQuoteRequest): Promise<BscTradeQuoteResponse> => client.getBscTradeQuote(request), []);
    const getBscTradeTxStatus = useCallback(async (hash: string): Promise<BscTradeTxStatusResponse> => client.getBscTradeTxStatus(hash), []);
    const getStewardStatus = useCallback(async () => client.getStewardStatus(), []);
    const getStewardAddresses = useCallback(async () => client.getStewardAddresses(), []);
    const getStewardBalance = useCallback(async (chainId?: number) => client.getStewardBalance(chainId), []);
    const getStewardTokens = useCallback(async (chainId?: number) => client.getStewardTokens(chainId), []);
    const getStewardWebhookEvents = useCallback(async (opts?: {
        event?: StewardWebhookEventType;
        since?: number;
    }) => client.getStewardWebhookEvents(opts), []);
    const getStewardHistory = useCallback(async (opts?: {
        status?: string;
        limit?: number;
        offset?: number;
    }) => client.getStewardHistory(opts), []);
    const getStewardPending = useCallback(async () => client.getStewardPending(), []);
    const approveStewardTx = useCallback(async (txId: string) => client.approveStewardTx(txId), []);
    const rejectStewardTx = useCallback(async (txId: string, reason?: string) => client.rejectStewardTx(txId, reason), []);
    const loadWalletTradingProfile = useCallback(async (window: WalletTradingProfileWindow = "30d", source: WalletTradingProfileSourceFilter = "all"): Promise<WalletTradingProfileResponse> => client.getWalletTradingProfile(window, source), []);
    const executeBscTrade = useCallback(async (request: BscTradeExecuteRequest): Promise<BscTradeExecuteResponse> => client.executeBscTrade(request), []);
    const executeBscTransfer = useCallback(async (request: BscTransferExecuteRequest): Promise<BscTransferExecuteResponse> => client.executeBscTransfer(request), []);
    ...
    useEffect(() => {
        ...
        const preset = resolveStylePresetByName(characterName, uiLanguage) ??
        ...
        void (async () => {
            try {
                await client.updateCharacter(buildLocalizedCharacterPayload(preset, resolvedName));
                await loadCharacter();
            }
            catch {
    ...
    // ── Channel change ──────────────────────────────────────────────────
    const handleChannelChange = useCallback(async (channel: "stable" | "beta" | "nightly") => {
