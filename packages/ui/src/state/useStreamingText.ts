...
import { type AccountConnectRequest } from "../api";
 ...
 */
function withProvisional(message: ConversationMessage, provisional: boolean): ConversationMessage {
    if (provisional) {
 ...
 */
function computeNextMessage(message: ConversationMessage, mod: StreamingTextModification): ConversationMessage | null {
    switch (mod.mode) {
        case "append": {
            ...
            if (nextText === message.text &&
                ...
                return null;
            return withProvisional({ ...message, text: nextText }, mod.provisional === true);
        }
        case "replace": {
            if (mod.fullText === message.text &&
                !mod.provisional === !message.provisional)
                return null;
            return withProvisional({ ...message, text: mod.fullText }, mod.provisional === true);
        }
        case "complete": {
            const sameText = message.text === mod.fullText;
            const sameInterruption = (message.interrupted === true) === (mod.interrupted === true);
            const sameFailure = message.failureKind === mod.failureKind;
            const sameTerminalFailure = message.terminalFailure === mod.terminalFailure;
            const sameAccountConnect = message.accountConnect === mod.accountConnect;
            const sameCapabilityHandoff = message.capabilityHandoff === mod.capabilityHandoff;
            const sameReasoning = mod.reasoning === undefined || message.reasoning === mod.reasoning;
            const sameAssistantEphemeral = message.assistantEphemeral === mod.assistantEphemeral;
            const sameId = mod.persistedMessageId === undefined ||
                message.id === mod.persistedMessageId;
            const sameReplyTo = mod.replyToMessageId === undefined ||
 ...
 */
export function applyStreamingTextModification(setMessages: StreamingTextSetter, mod: StreamingTextModification): void {
    setMessages((prev: ConversationMessage[]) => {
        ...
        if ((mod.mode === "complete" || mod.mode === "rekey") &&
