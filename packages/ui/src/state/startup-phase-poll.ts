...
import { MOBILE_LOCAL_AGENT_IPC_BASE } from "../first-run/mobile-runtime-mode";
...
import { appendIosBootTrace } from "../api/ios-local-agent-transport";
import { asApiLikeError } from "./internal";
...
import { deriveFirstRunResumeFieldsFromConfig } from "./internal";
import { describeStoppedDedicatedCloudAgent } from "./dedicated-cloud-agent-error";
import { formatStartupErrorDetail } from "./internal";
...
import { isIosNativeAgentBootInProgress } from "../api/ios-local-agent-transport";
...
import { persistMobileRuntimeMode } from "../first-run/mobile-runtime-mode";
import { readMobileRuntimeBuildTruth } from "../first-run/reconcile-mobile-runtime-mode";
import { readPersistedMobileRuntimeMode } from "../first-run/mobile-runtime-mode";
import { requestAndroidLocalAgentStartForUrl } from "../api/android-native-agent-transport";
...
import { type PlatformPolicy } from "./startup-coordinator";
...
import { unwrapStartupProbe } from "./startup-probe";
 ...
 */
const NATIVE_CONSECUTIVE_FAILURE_BUDGET_MS = STARTUP_TIMING_POLICY.nativeConsecutiveFailureBudgetMs;
/**
...
export function shouldFallBackToLocalOrigin(args: {
    ...
    if (typeof asApiLikeError(args.error)?.status === "number")
        return false;
    ...
    if (isDedicatedCloudAgentBase(args.clientBaseUrl) ||
...
export function isRecoverableRemoteBase(args: {
    ...
    if (!base)
        return false; // already same-origin / local
    try {
        const url = new URL(base);
        // Never recover to where we already are (no pointless self-recovery loop).
        if (args.pageOrigin && url.origin === args.pageOrigin)
            return false;
...
export function resolveStartupCloudControlPlaneBase(agentBase: string, options: {
...
async function sharedCloudAgentIsMissingFromRunningSet(base: string): Promise<boolean> {
    ...
    if (!isManagedCloudSharedAgentBase(base))
        return false;
    ...
    if (isPersonalSharedElizaId(agentId))
        return false;
    if (!getCloudAuthToken(client))
        return false;
    const priorBaseUrl = client.getBaseUrl();
    ...
    try {
        ...
        if (!res.success)
            return false;
        return !res.data.some((agent) => agent.agent_id === agentId && agent.status === "running");
    }
    catch {
...
function applyFirstRunResumeFields(rf: ReturnType<typeof deriveFirstRunResumeFieldsFromConfig>, deps: Pick<PollingBackendDeps, "setFirstRunRuntimeTarget" | "setFirstRunProvider" | "setFirstRunRemoteConnected" | "setFirstRunRemoteApiBase" | "setFirstRunRemoteToken">): void {
 ...
 */
export async function runPollingBackend(deps: PollingBackendDeps, dispatch: (event: StartupEvent) => void, policy: PlatformPolicy, ctx: RestoringSessionCtx | null, effectRunId: number, effectRunRef: React.MutableRefObject<number>, cancelled: {
    current: boolean;
}, tidRef: {
    current: ReturnType<typeof setTimeout> | null;
}, target: RuntimeTarget = "embedded-local"): Promise<void> {
    ...
    const nativeFailureBudgetMs = policy.nativeConsecutiveFailureBudgetMs ??
        NATIVE_CONSECUTIVE_FAILURE_BUDGET_MS;
    let nativeFailureStreakStartedAt: number | null = null;
    const remoteNativeFailureBudgetMs = policy.remoteNativeConsecutiveFailureBudgetMs ??
    ...
    const agentUnreachableBudgetMs = policy.agentUnreachableFailureBudgetMs ??
    ...
    const recoverToLocalOrigin = (why: string) => {
        ...
        logger.warn({ staleBase: client.getBaseUrl(), reason: why }, "[startup-phase-poll] abandoning the saved server; falling back to the local origin");
    ...
    const canRecoverToOnDeviceLocalAgent = (): boolean => {
        ...
        if (isMobileLocalAgentIpcBase(client.getBaseUrl()))
            return false;
        try {
    ...
    const recoverToOnDeviceLocalAgent = (why: string) => {
        ...
        logger.warn({ staleBase: client.getBaseUrl(), reason: why }, "[startup-phase-poll] persisted cloud agent is dead; falling back to the on-device local agent");
    ...
    const recoverToAgentSelection = (why: string) => {
        logger.warn({ staleBase: client.getBaseUrl(), reason: why }, "[startup-phase-poll] abandoning the saved cloud agent; routing to agent selection");
    ...
    const advanceManagedCloudAuthRejection = (why: string): boolean => {
        if (!client.hasToken())
            return false;
        const activeServer = ctx?.persistedActiveServer ?? ctx?.restoredActiveServer ?? null;
    ...
    const routeToOfflineFirstRun = (why: string) => {
        logger.warn({ reason: why }, "[startup-phase-poll] backend is unavailable; routing to first-run without server options");
        clearPersistedActiveServer();
        client.setBaseUrl(null);
        client.setToken(null);
    ...
    const cloudOnlyDesktopRenderer = typeof window !== "undefined" &&
        (window as {
            __ELIZA_DESKTOP_RUNTIME_MODE__?: unknown;
        })
            .__ELIZA_DESKTOP_RUNTIME_MODE__ === "cloud";
    const freshCloudOnlyTarget = policy.defaultTarget === "cloud-managed" || cloudOnlyDesktopRenderer;
    if (!cancelled.current &&
        ...
        routeToOfflineFirstRun("fresh cloud-only desktop has no saved agent; opening Cloud sign-in onboarding");
        return;
    }
    ...
    const restoredManagedCloud = ctx?.restoredActiveServer?.kind === "cloud" ||
        ctx?.persistedActiveServer?.kind === "cloud";
    if (cloudOnlyDesktopRenderer &&
    ...
    const traceIfStalled = <T>(promise: Promise<T>, label: string): Promise<T> => {
        if (stallTracersArmed >= 3)
            return promise;
        stallTracersArmed += 1;
    ...
    const boundedProbe = <T>(promise: Promise<T>): Promise<T> => {
        ...
        const remoteNativeProbeCapMs = isCapacitorNative() &&
        ...
        return new Promise<T>((resolve, reject) => {
            const hangTid = setTimeout(() => {
                reject(new ApiHangTimeoutError(`Startup probe did not settle within ${Math.round(capMs / 1000)}s — the request transport is hung (see the on-device boot trace, stage "probe-stalled"); retrying`));
            }, capMs);
            promise.then((value) => {
    ...
    while (!cancelled.current && effectRunRef.current === effectRunId) {
        ...
        if (polledBase) {
            void requestAndroidLocalAgentStartForUrl(polledBase).then((requested) => {
                if (requested) {
                    logger.info("[startup-phase-poll] requested native local-agent start for the polled base");
        ...
        try {
            const auth = await traceIfStalled(boundedProbe(client.getAuthStatus()), "auth-status");
            latestAuth = auth;
            ...
            if (auth.required && !auth.authenticated && !client.hasToken()) {
                ...
                if (!fellBackToLocal &&
                    ...
                    recoverToLocalOrigin("saved remote requires auth but pairing is disabled (dead end)");
                    continue;
                }
            ...
            if (auth.required && !auth.authenticated && client.hasToken()) {
                if (advanceManagedCloudAuthRejection("auth status rejected the saved Cloud agent credential")) {
                    return;
                }
                deps.setAuthRequired(false);
                deps.setFirstRunComplete(true);
            ...
            const firstRunStatusRes = await traceIfStalled(boundedProbe(client.getFirstRunStatus()), "first-run-status");
            const { complete, cloudProvisioned } = firstRunStatusRes;
            if (cancelled.current)
                return;
            deps.setFirstRunCloudProvisionedContainer(Boolean(cloudProvisioned));
            let sessionComplete = complete || deps.firstRunCompletionCommittedRef.current;
            ...
            if (!complete &&
                ...
                ctx?.hadPriorFirstRun) {
                const status = await traceIfStalled(boundedProbe(client.getStatus()), "first-run-runtime-status");
                ...
                while (!cancelled.current && effectRunRef.current === effectRunId) {
                    ...
                    try {
                        ...
                        if (ae?.status === 401 && client.hasToken()) {
                            if (advanceManagedCloudAuthRejection("first-run setup routes rejected the saved Cloud agent credential")) {
                                return;
                            }
                            // Transient 401: retry. /api/auth/status is the auth gate.
                            optErr = err;
                            await new Promise<void>((r) => {
                                tidRef.current = setTimeout(r, STARTUP_TIMING_POLICY.runtimePollIntervalMs);
                            });
                            continue;
                        }
                        if (ae?.status === 404) {
                            if (isDirectCloudSharedAgentBase(client.getBaseUrl())) {
                                if (await sharedCloudAgentIsMissingFromRunningSet(client.getBaseUrl())) {
                                    recoverToAgentSelection("saved shared cloud agent is missing from the running set");
                                    return;
                                }
                            ...
                            if (isDedicatedCloudAgentBase(client.getBaseUrl())) {
                                ...
                                if (await dedicatedCloudAgentIsGone(client.getBaseUrl())) {
                                    recoverToAgentSelection("saved dedicated cloud agent is deleted / unreachable");
                                    return;
                                }
            ...
            if (tracedPollFailures <= 5 || tracedPollFailures % 10 === 0) {
                const failureMessage = ae?.message ?? (err instanceof Error ? err.message : String(err));
            ...
            const terminalMessage = ae?.message ?? (err instanceof Error ? err.message : String(err));
            if (isCapacitorNative() &&
                isTerminalIosNativeAgentBootErrorMessage(terminalMessage)) {
                logger.warn({ message: terminalMessage, path: ae?.path }, "[startup-phase-poll] terminal native agent/transport error during backend poll; surfacing the startup error");
            ...
            if (isTerminalDedicatedCloudAgentErrorState({
                status: ae?.status,
                code: ae?.code,
                message: terminalMessage,
                data: ae?.data,
                ...
                if (canRecoverToOnDeviceLocalAgent()) {
                    recoverToOnDeviceLocalAgent("saved dedicated cloud agent is in a terminal error state");
                    continue;
                }
                recoverToAgentSelection("saved dedicated cloud agent is in a terminal error state");
                return;
            }
            if (ae?.status === 401 &&
                client.hasToken() &&
                advanceManagedCloudAuthRejection("backend poll rejected the saved Cloud agent credential")) {
                return;
            }
            if (ae?.status === 401 && !client.hasToken()) {
            ...
            if ((ae?.status === 401 || ae?.status === 429) &&
                client.hasToken() &&
                latestAuth.authenticated) {
                if (ae.status === 401 &&
                    advanceManagedCloudAuthRejection("runtime routes rejected the saved Cloud agent credential")) {
                    return;
                }
            ...
            if (ae?.status === 404) {
                ...
                if (isDedicatedCloudAgentBase(client.getBaseUrl())) {
                    ...
                    if (await dedicatedCloudAgentIsGone(client.getBaseUrl())) {
                        if (canRecoverToOnDeviceLocalAgent()) {
                            recoverToOnDeviceLocalAgent("saved dedicated cloud agent is deleted / unreachable");
                            continue;
                        }
                        recoverToAgentSelection("saved dedicated cloud agent is deleted / unreachable");
                        return;
            ...
            if (isViteDevUiShell() &&
                ...
                routeToOfflineFirstRun(ae?.status === undefined
            ...
            if (!isCapacitorNative()) {
                ...
                const isConnectionLevelFailure = err instanceof ApiHangTimeoutError ||
                    ae?.kind === "network" ||
                    ae?.kind === "timeout";
                if (isConnectionLevelFailure &&
                    ...
                    if (failingForMs >= agentUnreachableBudgetMs) {
                        ...
                        logger.warn({ failingForMs, agentHost, detail }, "[startup-phase-poll] dedicated cloud agent host is unreachable at the connection level; surfacing the startup error");
                        deps.setStartupError({
                            reason: "backend-unreachable",
                            phase: "starting-backend",
            ...
            if (isCapacitorNative()) {
                ...
                const remoteTarget = target !== "embedded-local" &&
                    !isMobileLocalAgentIpcBase(client.getBaseUrl());
                const isConnectionLevelFailure = err instanceof ApiHangTimeoutError ||
                    ae?.kind === "network" ||
                    ae?.kind === "timeout";
                if (remoteTarget && isConnectionLevelFailure) {
                    ...
                    if (failingForMs >= remoteNativeFailureBudgetMs) {
                        const detail = formatStartupErrorDetail(err) ?? String(err);
                        logger.warn({ failingForMs, target, detail }, "[startup-phase-poll] native remote target exceeded the connection-failure budget; surfacing the startup error");
                        deps.setStartupError({
                            reason: "backend-unreachable",
                            phase: "starting-backend",
                            message: "Could not reach your saved agent. Check your connection and retry; your sign-in and chat history are preserved.",
                            detail,
                            status: ae?.status,
                            path: ae?.path,
                        });
                ...
                const androidLocalAgentIpc = isMobileLocalAgentIpcBase(client.getBaseUrl()) &&
                ...
                const androidNativeBootProgress = androidBootState.state === "booting" ||
                    androidBootState.state === "restarting" ||
                    androidBootState.state === "listening";
                const legacyAndroidLocalAgentBooting = androidBootState.state === "unknown" &&
                    androidLocalAgentIpc &&
                    (err instanceof ApiHangTimeoutError ||
                        (err as {
                            status?: number;
                        } | undefined)?.status === undefined);
                const localAgentBootProgress = !remoteTarget &&
                ...
                if (localAgentBootProgress) {
                    ...
                    if (failingForMs >= nativeFailureBudgetMs) {
                        ...
                        if (canRecoverToOnDeviceLocalAgent()) {
                            ...
                            recoverToOnDeviceLocalAgent(`native poll dead-ended after ${Math.round(failingForMs / 1000)}s: ${detail}`);
                            continue;
                        }
                        logger.warn({ failingForMs, detail }, "[startup-phase-poll] native backend poll exceeded the consecutive-failure budget; surfacing the startup error");
