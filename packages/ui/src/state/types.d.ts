...
export interface NavigationEventsApi {
    subscribeTabCommitted: (listener: (detail: TabCommittedDetail) => void) => () => void;
    /**
...
export type FlaminaGuideTopic = "provider" | "rpc" | "permissions" | "voice" | "features";
export interface OnboardingNextOptions {
    allowPermissionBypass?: boolean;
    omitRuntimeProvider?: boolean;
    skipTask?: string;
}
export declare const ONBOARDING_PERMISSION_LABELS: Record<SystemPermissionId, string>;
...
export type StartupErrorReason = "backend-timeout" | "backend-unreachable" | "agent-timeout" | "agent-error" | "asset-missing" | "unknown";
...
export interface StartupCoordinatorView {
    state: {
        phase: "splash" | "restoring-session" | "resolving-target" | "polling-backend" | "pairing-required" | "onboarding-required" | "starting-runtime" | "hydrating" | "ready" | "error";
        [key: string]: unknown;
    };
    dispatch: (event: {
        type: string;
...
export interface AppActions {
     ...
     */
    completeOnboarding: (landingTab?: Tab, options?: CompleteOnboardingOptions) => void;
