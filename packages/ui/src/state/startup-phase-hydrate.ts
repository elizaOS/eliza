...
import { isRouteRootPath } from "../navigation";
import { isRuntimeManagementOperation } from "@elizaos/core/contracts/runtime-management";
import { isTransientOptionalFetchFailure } from "../utils";
import { loadAgentProfileRegistry } from "./agent-profiles";
import { loadAvatarIndex } from "./internal";
import { logger } from "@elizaos/ui/logger";
import { mapServerTasksToSessions } from "../chat/coding-agent-session-state";
import { normalizeAvatarIndex } from "./internal";
import { normalizeShellNavigateViewPayload } from "@elizaos/core/events";
import { parseAgentStatusEvent } from "./internal";
import { parseProactiveMessageEvent } from "./internal";
import { parseStreamEventEnvelopeEvent } from "./internal";
...
import { resolveAgentProfileByQuery } from "./agent-profiles";
import { resolveDefaultLandingTab } from "../navigation";
...
import { type WalletAddresses } from "../api";
...
function normalizeAppEmoteEvent(data: Record<string, unknown>): AppEmoteEventDetail | null {
 ...
 */
export async function runHydrating(deps: HydratingDeps, dispatch: (event: StartupEvent) => void, _cancelled: {
    current: boolean;
}): Promise<void> {
    ...
    const reconcileDirectCloudConversation = async (): Promise<void> => {
        const persistedConversationId = window.localStorage.getItem(ACTIVE_CONVERSATION_STORAGE_KEY);
 ...
 */
export function bindReadyPhase(depsRef: React.MutableRefObject<ReadyPhaseDeps | undefined>): () => void {
    let ptyPollInterval: ReturnType<typeof setInterval> | null = null;
    let handleVis: (() => void) | null = null;
    const doHydratePty = () => {
        if (!depsRef.current?.codingAgentsEnabledRef.current)
            return;
        ...
        client
            ...
            logger.debug({ err }, "[startup-phase-hydrate] coding-agent status hydrate failed");
        });
    };
    ...
    const hydratePty = () => {
        if (depsRef.current?.agentRunningRef.current)
            doHydratePty();
    };
    ...
    const reconcileConversationAfterReconnect = (running: boolean): void => {
        if (!running || !conversationResyncPending)
            return;
    ...
    ptyPollInterval = setInterval(() => {
        ...
        if (running && depsRef.current?.hasPtySessionsRef.current)
            doHydratePty();
    }, 5000);
    client.connectWs();
    const unbindEmotes = client.onWsEvent("emote", (data: Record<string, unknown>) => {
        const e = normalizeAppEmoteEvent(data);
        if (e)
            dispatchAppEmoteEvent(e);
    });
    const unbindWsReconnect = client.onWsEvent("ws-reconnected", () => {
        const conversationId = depsRef.current?.activeConversationIdRef.current ?? null;
    ...
    const unbindStatus = client.onWsEvent("status", (data: Record<string, unknown>) => {
        ...
        if (typeof data.pendingRestart === "boolean")
            d.setPendingRestart((p: boolean) => p === data.pendingRestart ? p : (data.pendingRestart as boolean));
    ...
    const unbindRestart = client.onWsEvent("restart-required", (data: Record<string, unknown>) => {
        if (Array.isArray(data.reasons)) {
            depsRef.current?.setPendingRestartReasons(data.reasons.filter((e): e is string => typeof e === "string"));
            depsRef.current?.setPendingRestart(true);
            depsRef.current?.showRestartBanner();
        }
    });
    const unbindShellNavigateView = client.onWsEvent(SHELL_NAVIGATE_VIEW_WS_EVENT, (data: Record<string, unknown>) => {
    ...
    const unbindModelSwitch = client.onWsEvent("shell:model-switch", (data: Record<string, unknown>) => {
        ...
        depsRef.current?.setActionNotice(notice, "success", undefined, false, status === "downloading" || status === "loading");
    });
    ...
    const unbindSwitchAgent = client.onWsEvent("shell:switch-agent", (data: Record<string, unknown>) => {
        const requestId = typeof data.requestId === "string" ? data.requestId : null;
        const query = typeof data.profile === "string" ? data.profile : "";
        if (!requestId)
            return;
        const originBase = typeof client.getBaseUrl === "function" ? client.getBaseUrl() : "";
        ...
        const profile = resolveAgentProfileByQuery(query, loadAgentProfileRegistry());
        if (!profile) {
        ...
        if (!result.ok) {
            ...
            if (result.reason === "untrusted-remote") {
                depsRef.current?.setActionNotice(`Refused to switch to "${profile.label}" — untrusted remote address.`, "error");
            }
            else if (result.reason === "untrusted-cloud") {
                depsRef.current?.setActionNotice(`Refused to switch to "${profile.label}" — invalid Cloud agent address.`, "error");
            }
            else if (result.reason === "persistence-failed") {
                depsRef.current?.setActionNotice(`Couldn't switch to "${profile.label}" because browser storage is unavailable.`, "error");
            }
            return;
        }
        reportResult({
        ...
        depsRef.current?.setActionNotice(`Switched the app to "${result.profile.label}".`, "success");
    });
    ...
    const unbindManageRuntime = client.onWsEvent("shell:manage-runtime", (data: Record<string, unknown>) => {
        const requestId = typeof data.requestId === "string" ? data.requestId : null;
        const rawRequest = data.request !== null &&
        ...
        const originBase = typeof client.getBaseUrl === "function" ? client.getBaseUrl() : "";
        void (async () => {
            const claimResponse = await fetchWithCsrf(`${originBase}/api/runtime/manage/claim`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            try {
                const { executeRuntimeManagementCommand } = await import("../platform/runtime-management");
            ...
            const resultResponse = await fetchWithCsrf(`${originBase}/api/runtime/manage/result`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    requestId,
            ...
            depsRef.current?.setActionNotice(result.ok
                ? "Devices & Runtimes updated."
                : (result.error ?? "The runtime operation failed."), result.ok ? "success" : "error");
        })().catch((cause) => {
            // error-policy:J4 surface bridge failure as a visible error state.
            depsRef.current?.setActionNotice(cause instanceof Error && cause.message.trim()
                ? cause.message
                : "The runtime management bridge failed.", "error");
            logger.warn(`[startup-phase-hydrate] runtime management bridge failed: ${cause instanceof Error ? cause.message : String(cause)}`);
        });
    });
    const unbindViewEvent = client.onWsEvent("view:event", (data: Record<string, unknown>) => {
        const viewEventType = typeof data.viewEventType === "string" ? data.viewEventType : null;
        if (!viewEventType)
            return;
    ...
    const unbindViewInteract = client.onWsEvent("view:interact", (data: Record<string, unknown>) => {
        ...
        const viewType = data.viewType === "gui" ||
            data.viewType === "tui" ||
            data.viewType === "xr"
            ? data.viewType
            : undefined;
        ...
        if (!viewId || !capability || !requestId || !installationId)
            return;
        ...
        import("../components/views/view-interact-registry")
            .then(({ dispatchViewInteract }) => dispatchViewInteract(viewId, viewType, capability, params, requestId, installationId))
            .catch(() => {
            // No renderer claimed execution. The owning host times out without replay.
        });
    });
    const unbindAgent = client.onWsEvent("agent_event", (data: Record<string, unknown>) => {
    ...
    const unbindHb = client.onWsEvent("heartbeat_event", (data: Record<string, unknown>) => {
        const e = parseStreamEventEnvelopeEvent(data);
        if (e) {
            depsRef.current?.appendAutonomousEvent(e);
    ...
    const unbindProactive = client.onWsEvent("proactive-message", (data: Record<string, unknown>) => {
        ...
        if (cid === d.activeConversationIdRef.current)
            ...
            d.setUnreadConversations((prev: Set<string>) => new Set([...prev, cid]));
        if (msg.source &&
        ...
        if (conv?.id)
            depsRef.current?.setConversations((prev: Conversation[]) => {
                const u = prev.map((c) => {
                    ...
                    const isGenericTitle = !incomingTitle ||
                ...
                if (eventType === "blocked" || eventType === "escalation")
                    return prev.map((s) => s.sessionId === sid
                        ? {
                            ...
                            lastActivity: eventType === "escalation"
                                ? "Escalated — needs attention"
                                : "Waiting for input",
                        }
                        : s);
                if (eventType === "tool_running") {
                    const td = (dd?.description as string) ??
