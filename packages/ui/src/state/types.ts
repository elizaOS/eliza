...
import { type AppRunSummary } from "../api/client";
import { type AppSessionState } from "../api/client";
import { type AppViewerAuthMessage } from "../api/client";
import { type BackgroundConfig } from "./ui-preferences";
import { type BscTradeExecuteRequest } from "../api/client";
import { type BscTradeExecuteResponse } from "../api/client";
import { type BscTradePreflightResponse } from "../api/client";
import { type BscTradeQuoteRequest } from "../api/client";
import { type BscTradeQuoteResponse } from "../api/client";
import { type BscTradeTxStatusResponse } from "../api/client";
import { type BscTransferExecuteRequest } from "../api/client";
import { type BscTransferExecuteResponse } from "../api/client";
import { type CharacterData } from "../api/client";
import { type ChatTokenUsage } from "../api/client";
import { type CodingAgentSession } from "../api/client";
import { type Conversation } from "../api/client";
import { type ConversationChannelType } from "../api/client";
import { type ConversationMessage } from "../api/client";
import { type CreateTriggerRequest } from "../api/client";
import { type Dispatch } from "react";
import { type DropStatus } from "../api/client";
import { type FirstRunOptions } from "../api/client";
import { type FirstRunRuntimeTarget } from "../first-run/runtime-target";
import { type ImageAttachment } from "../api/client";
import { type LogEntry } from "../api/client";
import { type McpMarketplaceResult } from "../api/client";
import { type McpRegistryServerDetail } from "../api/client";
import { type McpServerConfig } from "../api/client";
import { type McpServerStatus } from "../api/client";
import { type MintResult } from "../api/client";
import { type PluginInfo } from "../api/client";
import { type RegistryPlugin } from "../api/client";
import { type RegistryStatus } from "../api/client";
import { type ReleaseChannel } from "../api/client";
import { type SetStateAction } from "react";
import { type SkillInfo } from "../api/client";
import { type SkillScanReportSummary } from "../api/client";
import { type StewardApprovalActionResponse } from "../api/client";
import { type StewardBalanceResponse } from "../api/client";
import { type StewardHistoryResponse } from "../api/client";
import { type StewardPendingResponse } from "../api/client";
import { type StewardStatusResponse } from "../api/client";
import { type StewardTokenBalancesResponse } from "../api/client";
import { type StewardWalletAddressesResponse } from "../api/client";
import { type StewardWebhookEventType } from "../api/client";
import { type StewardWebhookEventsResponse } from "../api/client";
import { type StreamEventEnvelope } from "../api/client";
import { type Tab } from "../navigation";
import { type TriggerHealthSnapshot } from "../api/client";
...
import { type WalletAddresses } from "../api/client";
import { type WalletBalancesResponse } from "../api/client";
import { type WalletChainKind } from "@elizaos/core/contracts/wallet-types";
import { type WalletConfigStatus } from "../api/client";
import { type WalletConfigUpdateRequest } from "../api/client";
import { type WalletEntry } from "@elizaos/core/contracts/wallet-types";
import { type WalletExportResult } from "../api/client";
import { type WalletNftsResponse } from "../api/client";
...
import { type WalletTradingProfileResponse } from "../api/client";
import { type WalletTradingProfileSourceFilter } from "../api/client";
import { type WalletTradingProfileWindow } from "../api/client";
import { type WhitelistStatus } from "../api/client";
import { type WorkbenchOverview } from "../api/client";
...
export const LIFECYCLE_MESSAGES: Record<LifecycleAction, {
    inProgress: string;
    progress: string;
    success: string;
    verb: string;
 ...
 */
export function shouldAwaitAgentReadiness(agentStatus: AgentStatus | null): boolean {
    const state = agentStatus?.state;
    return (!deriveAgentReady(agentStatus) &&
        state !== "error" &&
        state !== "stopped" &&
        state !== "not_started");
}
export type StartupPhase = "starting-backend" | "initializing-agent" | "ready";
export type StartupErrorReason = "backend-timeout" | "backend-unreachable" | "agent-timeout" | "agent-error" | "agent-stopped" | "asset-missing" | "unknown";
...
export interface StartupCoordinatorView {
    state: {
        phase: "restoring-session" | "resolving-target" | "polling-backend" | "pairing-required" | "first-run-required" | "starting-runtime" | "hydrating" | "ready" | "error";
        [key: string]: unknown;
    };
    dispatch: (event: {
        type: string;
    ...
    statusMessageKey: "startupshell.Starting" | "startupshell.InitializingAgent" | "startupshell.Loading";
    error: {
        reason: StartupErrorReason;
        message: string;
        timedOut: boolean;
...
export type WalletResourceStatus = "idle" | "loading" | "ready" | "unavailable" | "error";
...
export interface AppActions {
     ...
     */
    loadConversationMessagesAround: (conversationId: string, messageId: string) => Promise<boolean>;
    ...
    handlePluginConfigSave: (pluginId: string, config: Record<string, string>) => Promise<boolean>;
    // Skills
