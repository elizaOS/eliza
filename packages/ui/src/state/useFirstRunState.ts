...
function loadInitialServerSelection(): Pick<FirstRunState, "serverTarget" | "remote" | "remoteApiBase" | "remoteToken"> {
    ...
    if (activeServer.kind === "cloud") {
        const serverTarget = readPersistedMobileRuntimeMode() === "cloud-hybrid"
            ? "elizacloud-hybrid"
...
function firstRunReducer(state: FirstRunState, action: FirstRunAction): FirstRunState {
    switch (action.type) {
        case "ADD_DEFERRED_TASK":
            if (state.deferredTasks.includes(action.task))
                return state;
            return {
                ...state,
...
export function useFirstRunState(cloudOnly?: boolean): FirstRunStateHook {
    const [state, dispatch] = useReducer(firstRunReducer, cloudOnly, (co) => createInitialState(co));
    ...
    const completionCommittedRef = useRef(loadPersistedFirstRunComplete(cloudOnly) && !isOnboardingReplayRequested());
    return {
        state,
        dispatch,
