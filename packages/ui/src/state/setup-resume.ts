...
export function hasPartialSetupConnectionConfig(config: Record<string, unknown> | null | undefined): boolean {
...
export function deriveFirstRunResumeFieldsFromConfig(config: Record<string, unknown> | null | undefined): FirstRunResumeFields {
    const deploymentTarget = resolveDeploymentTargetInConfig(config);
    ...
    const cloudServerTarget = pinnedRuntimeMode === "cloud-hybrid" ? "elizacloud-hybrid" : "elizacloud";
    ...
    const firstRunRuntimeTarget = pinnedRuntimeMode === "local"
        ? "local"
    ...
    const firstRunProvider = llmText &&
    ...
    return {
        ...
        firstRunProvider,
        firstRunRemoteConnected: deploymentTarget.runtime === "remote" &&
