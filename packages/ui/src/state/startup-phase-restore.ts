...
import { ANDROID_LOCAL_AGENT_LABEL } from "../first-run/mobile-runtime-mode";
import { ANDROID_LOCAL_AGENT_SERVER_ID } from "../first-run/mobile-runtime-mode";
import { IOS_LOCAL_AGENT_IPC_BASE } from "../first-run/mobile-runtime-mode";
import { MOBILE_LOCAL_AGENT_LABEL } from "../first-run/mobile-runtime-mode";
import { MOBILE_LOCAL_AGENT_SERVER_ID } from "../first-run/mobile-runtime-mode";
import { STARTUP_TIMING_POLICY } from "./startup-timing-policy";
import { buildCloudSharedAgentApiBase } from "../utils/cloud-agent-base";
import { buildDedicatedCloudAgentApiBase } from "../utils/cloud-agent-base";
import { buildStaticFirstRunOptions } from "./startup-first-run-options";
import { clearForceFreshFirstRun } from "../platform";
...
import { hydratePersistedFirstRunCompleteFromNativeStore } from "./persistence";
import { invokeDesktopBridgeRequestWithTimeout } from "../bridge";
import { isAndroid } from "../platform";
import { isCloudPairAgentId } from "@elizaos/core/contracts/cloud-pair";
import { isCloudPairLoopbackOrigin } from "@elizaos/core/contracts/cloud-pair";
...
import { isForceFreshFirstRunEnabled } from "../platform";
...
import { resolveDirectCloudAuthApiBase } from "../api/client-cloud";
import { runStartupProbeWithTimeout } from "./startup-probe";
import { savePersistedActiveServer } from "./persistence";
import { savePersistedFirstRunComplete } from "./persistence";
...
import { wasForceFreshResetApplied } from "../platform";
import { writeStoredStewardToken } from "@elizaos/plugin-elizacloud/steward-session-client";
const DESKTOP_RESTORE_RPC_TIMEOUT_MS = STARTUP_TIMING_POLICY.desktopRestoreRpcTimeoutMs;
/**
 ...
 */
const STEWARD_RESTORE_REFRESH_TIMEOUT_MS = STARTUP_TIMING_POLICY.stewardRestoreRefreshTimeoutMs;
/** Bound the non-blocking legacy runtime-tier repair lookup. */
const CLOUD_AGENT_TIER_PROBE_TIMEOUT_MS = STARTUP_TIMING_POLICY.cloudAgentTierProbeTimeoutMs;
...
function recoverCloudAgentId(active: PersistedActiveServer): string | null {
    ...
    const rawId = active.id?.startsWith("cloud:")
        ...
        : "";
    if (isCloudPairAgentId(rawId) || isPersonalSharedElizaId(rawId))
        return rawId;
 ...
 */
async function reconcileLegacyDedicatedCloudApiBase(active: PersistedActiveServer, ownerToken: string | null): Promise<PersistedActiveServer | null> {
    if (!ownerToken || !isDedicatedCloudAgentBase(active.apiBase))
        return null;
    const agentId = recoverCloudAgentId(active);
    if (!agentId)
        return null;
    const pageHostname = typeof window !== "undefined" ? window.location.hostname : "";
    const cloudApiBase = resolveDirectCloudAuthApiBase(resolveCloudEnvironmentBase({
        pageHostname,
    ...
    try {
        const response = await fetch(`${cloudApiBase}/api/v1/eliza/agents/${encodeURIComponent(agentId)}`, {
            headers: {
 ...
 */
function backfillCloudApiBase(active: PersistedActiveServer): PersistedActiveServer {
    ...
    const dedicatedApiBase = isCloudPairAgentId(agentId)
        ...
        : null;
    const sharedApiBase = buildCloudSharedAgentApiBase(resolveDirectCloudAuthApiBase(cloudApiBase), agentId);
    const managedBase = !active.apiBase ||
    ...
    if (!managedBase)
        return active;
    const useSharedApiBase = active.cloudRuntime === "shared" ||
        isDirectCloudSharedAgentBase(active.apiBase);
    const repairedApiBase = useSharedApiBase ? sharedApiBase : dedicatedApiBase;
    if (!repairedApiBase)
        return active;
    if (active.apiBase === repairedApiBase)
        return active;
...
function isMobileLocalActiveServer(server: PersistedActiveServer, mobileRuntimeMode = readPersistedMobileRuntimeMode()): boolean {
    ...
    if (mobileRuntimeMode === "remote-mac")
        return false;
...
function reconcilePersistedApiBaseWithLive(apiBase: string | undefined): string | undefined {
    if (!apiBase)
        return apiBase;
    const live = getElizaApiBase();
    if (!live || live === apiBase)
        return apiBase;
    try {
        const persisted = new URL(apiBase);
        if (!isLoopbackHostname(persisted.hostname))
            return apiBase;
        const liveUrl = new URL(live);
        if (!isLoopbackHostname(liveUrl.hostname))
            return apiBase;
        return live;
    }
    catch {
...
function mobileLocalActiveServer(platform: MobileNativePlatform = isAndroid ? "android" : "ios"): PersistedActiveServer {
...
export function reconcileMobileRestoredActiveServer(args: {
    ...
    const expectedMobileIpcBase = platform === "android"
...
function resolveRestoreStewardRefreshEndpoint(): string | undefined {
    if (!isNative && !isElectrobunRuntime())
        return undefined;
    const cloudBase = getBootConfig().cloudApiBase?.trim() ||
...
async function resolveRestoredStewardToken(): Promise<string | null> {
    ...
    if (!stored) {
        ...
        if (typeof window !== "undefined" && hasStewardAuthedCookie()) {
            const refreshProbe = await runStartupProbeWithTimeout(() => refreshCloudStewardSession({
            ...
            if (refreshProbe.kind !== "ok") {
                logger.warn({ error: refreshProbe.error, kind: refreshProbe.kind }, "[startup-phase-restore] Steward cookie refresh did not complete");
            }
            if (recovered?.token) {
    ...
    if (refreshProbe.kind !== "ok") {
        logger.warn({ error: refreshProbe.error, kind: refreshProbe.kind }, "[startup-phase-restore] stored Steward token refresh did not complete");
    }
    if (refreshed?.token) {
...
async function dropShadowingStewardToken(reason: string): Promise<void> {
    try {
        ...
        logger.error({ error, reason }, "[startup-phase-restore] denied delete left a shadowing Steward JWT in protected storage");
    }
}
export async function applyRestoredConnection(args: {
    ...
    if (restoredActiveServer.kind === "cloud") {
        ...
        if (!isTrustedCloudApiBaseUrl(resolved.apiBase, agentId)) {
            logger.warn(`[startup-phase-restore] dropping persisted cloud active-server with untrusted apiBase host: ${resolved.apiBase ?? "(none)"}`);
        ...
        const nativeOwnerApiKey = isNative || isElectrobunRuntime()
            ? (normalizeCloudApiKeyToken(resolved.accessToken) ??
                normalizeCloudApiKeyToken(getElizaApiToken()))
            : null;
        const usesLocalDockerCredential = isCloudPairLoopbackOrigin(resolved.apiBase);
        const isAgentlessControlPlane = isElizaCloudControlPlaneAgentlessBase(resolved.apiBase);
        const isManagedSharedControlPlane = isManagedCloudSharedAgentBase(resolved.apiBase);
            ...
            if (secs !== null && secs < STEWARD_RESTORE_REFRESH_AHEAD_SECS) {
                ...
                stewardTokenPromise = refreshCloudStewardSession()
                    ...
                    if (fresh)
                        ...
                        await dropShadowingStewardToken("rotation-returned-no-token");
                    return fresh;
                })
                    .catch(async () => {
        ...
        const tierRepairPromise = !isManagedSharedControlPlane &&
        ...
        clientRef.setToken(usesLocalDockerCredential
            ? resolved.accessToken || null
        ...
        void tierRepairPromise.then((repaired) => {
            if (!repaired || repaired.apiBase === resolved.apiBase)
                return;
            if (!isTrustedCloudApiBaseUrl(repaired.apiBase, agentId))
                return;
    ...
    if (isMobileLocalAgentIpcUrl(restoredActiveServer.apiBase) ||
    ...
    const reconciled = reconcilePersistedApiBaseWithLive(restoredActiveServer.apiBase);
    ...
    if (!isTrustedRestoreApiBaseUrl(reconciled)) {
        logger.warn(`[startup-phase-restore] dropping persisted remote active-server with untrusted apiBase host: ${reconciled ?? "(none)"}`);
        clearPersistedActiveServer();
        return;
    }
...
function activeServerToTarget(server: PersistedActiveServer): "embedded-local" | "cloud-managed" | "remote-backend" {
    if (isMobileLocalActiveServer(server))
...
export function canRestoreActiveServer(args: {
    ...
    if (args.server.apiBase) {
        ...
        if (args.server.kind === "cloud") {
            return isTrustedCloudApiBaseUrl(args.server.apiBase, recoverCloudAgentId(args.server));
        }
        return true;
    }
    ...
    if (args.server.kind === "cloud") {
        ...
        const rawId = args.server.id?.startsWith("cloud:")
            ...
            : "";
        return (isCloudPairAgentId(args.server.cloudRuntimeAgentId) ||
...
function preserveCloudAuthTokenForFirstRun(server: PersistedActiveServer): void {
    ...
    client.setBaseUrl(resolveDirectCloudAuthApiBase(getBootConfig().cloudApiBase || RESTORE_DEFAULT_DIRECT_CLOUD_BASE_URL));
 ...
 */
export async function runRestoringSession(deps: RestoringSessionDeps, dispatch: (event: StartupEvent) => void, ctxRef: React.MutableRefObject<RestoringSessionCtx | null>, cancelled: {
    current: boolean;
}): Promise<void> {
    ...
    if (forceFreshFirstRun) {
        ...
        const preservePostResetConnection = resetAlreadyClearedPriorServer && persistedActiveServer !== null;
    ...
    const desktopRuntimeMode = (): Promise<{
        ...
        desktopRuntimeModePromise ??= getDesktopRuntimeModeForStartup().catch(() => null);
    ...
    // single-shot.
    const committedMobileOnDeviceMode = (isAndroid || isIOS) &&
        isCommittedOnDeviceMobileRuntimeMode(readPersistedMobileRuntimeMode());
    const shouldProbeExistingInstall = !forceFreshFirstRun && !persistedActiveServer;
    ...
    if (shouldProbeExistingInstall) {
        try {
            ...
            logger.error(`[startup-phase-restore] existing-install probe failed for a committed on-device runtime: ${err instanceof Error ? err.message : String(err)}`);
            probed = {
    ...
    if (cancelled.current)
        return;
    let restoredActiveServer = persistedActiveServer ?? (probed ? probed.activeServer : null);
    ...
    const preserveCompleted = hadPrior &&
    ...
    await applyRestoredConnection({
        ...
        startLocalRuntime: async () => {
            try {
                ...
                logger.warn(`[startup-phase-restore] desktop agent bridge request failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        },
    });
    if (isManagedCloudSharedAgentBase(restoredActiveServer.apiBase) &&
