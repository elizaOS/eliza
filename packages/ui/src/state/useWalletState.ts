...
import { loadBrowserEnabled } from "./persistence";
import { loadComputerUseEnabled } from "./persistence";
import { loadWalletEnabled } from "./persistence";
import { logger } from "@elizaos/ui/logger";
import { saveBrowserEnabled } from "./persistence";
import { saveComputerUseEnabled } from "./persistence";
import { saveWalletEnabled } from "./persistence";
import { type DropStatus } from "../api";
...
import { type RegistryStatus } from "../api";
...
import { type WalletExportResult } from "../api";
...
import { type WalletResourceStatus } from "./types";
import { type WalletSource } from "@elizaos/core/contracts/wallet-types";
import { type WhitelistStatus } from "../api";
...
// ── Hook ──────────────────────────────────────────────────────────────
export function useWalletState({ setActionNotice, promptModal, agentName, characterName, hydrateServerConfig = true, }: WalletStateParams) {
    ...
    const isCurrentAuthority = useCallback((token: WalletAuthorityToken) => {
        return (authorityTokenRef.current === token &&
    ...
    const syncCapability = useCallback((name: "wallet" | "browser" | "computerUse", v: boolean) => {
        ...
        void client
            ...
            if (!isCurrentAuthority(requestAuthority))
                return;
            logger.error({ err, capability: name, value: v }, "[useWalletState] capability sync to server failed");
            setActionNotice(`Failed to sync ${name} setting to the agent — it may revert on reload`, "error");
        });
    ...
    useEffect(() => {
        if (!hydrateServerConfig)
            return;
        const requestAuthority = authorityTokenRef.current;
        if (requestAuthority.authority !== authority)
            return;
        let cancelled = false;
        void client
            .getConfig()
            .then((cfg) => {
            if (cancelled || !isCurrentAuthority(requestAuthority))
                return;
            ...
            if (!isCurrentAuthority(requestAuthority))
                return;
            logger.warn({ err }, "[useWalletState] capability hydration from server config failed; keeping local values");
        });
        return () => {
            cancelled = true;
        };
    ...
    const [walletConfigStatus, setWalletConfigStatus] = useState<WalletResourceStatus>("idle");
    const [walletConfigError, setWalletConfigError] = useState<string | null>(null);
    const [walletBalancesStatus, setWalletBalancesStatus] = useState<WalletResourceStatus>("idle");
    const [walletBalancesError, setWalletBalancesError] = useState<string | null>(null);
    const [walletNftsStatus, setWalletNftsStatus] = useState<WalletResourceStatus>("idle");
    ...
    const [walletPrimary, setWalletPrimaryMap] = useState<WalletPrimaryMap | null>(null);
    const [walletPrimaryRestarting] = useState<Partial<Record<WalletChainKind, boolean>>>({});
    const [walletPrimaryPending, setWalletPrimaryPending] = useState<Partial<Record<WalletChainKind, boolean>>>({});
    ...
    const [inventoryChainFilters, setInventoryChainFilters] = useState<InventoryChainFilters>({
        ethereum: true,
        base: true,
        bsc: true,
        avax: true,
    ...
    const walletExportTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    const walletConfigRequestRef = useRef(0);
    const walletBalancesRequestRef = useRef(0);
    const walletNftsRequestRef = useRef(0);
    useEffect(() => {
        if (walletStateAuthorityRef.current === authority)
            return;
    ...
    const fetchWalletConfig = useCallback(async () => {
        ...
        try {
            ...
            if (isCurrentAuthority(requestAuthority) &&
                walletConfigRequestRef.current === requestId) {
                setWalletConfigError(`Failed to load wallet config: ${err instanceof Error ? err.message : "network error"}`);
                setWalletConfigStatus("error");
            }
            throw err;
        }
    }, [applyWalletConfig, isCurrentAuthority]);
    const hasWalletSource = useCallback((config: WalletConfigStatus | null | undefined, chain: WalletChainKind, source: WalletSource) => (config?.wallets ?? []).some((wallet: WalletEntry) => wallet.chain === chain &&
    ...
    const summarizeCloudWalletImport = useCallback((config: WalletConfigStatus | null | undefined, warnings: string[] | undefined): {
        text: string;
        tone: "success" | "info";
    } => {
    ...
    const loadBalances = useCallback(async () => {
        ...
        try {
            ...
            if (isCurrentAuthority(requestAuthority) &&
                walletBalancesRequestRef.current === requestId) {
                setWalletBalancesError(`Failed to fetch balances: ${err instanceof Error ? err.message : "network error"}`);
    ...
    const loadNfts = useCallback(async () => {
        ...
        try {
            ...
            if (isCurrentAuthority(requestAuthority) &&
                walletNftsRequestRef.current === requestId) {
                const unavailable = isApiError(err) &&
                    (err.status === 404 ||
                        err.status === 501 ||
                ...
                if (unavailable) {
                    ...
                    setWalletNftsError(`Failed to fetch NFTs: ${err instanceof Error ? err.message : "network error"}`);
    ...
    const handleWalletApiKeySave = useCallback(async (config: WalletConfigUpdateRequest) => {
        const requestAuthority = authorityTokenRef.current;
        if (!isCurrentAuthority(requestAuthority))
            return false;
    ...
    const setWalletPrimary = useCallback(async (chain: WalletChainKind, source: WalletSource) => {
        const requestAuthority = authorityTokenRef.current;
        if (!isCurrentAuthority(requestAuthority))
            return;
        ...
        setWalletError(null);
        try {
            let currentConfig = walletStateAuthorityRef.current === requestAuthority.authority
                ? walletConfig
                : null;
            if (!currentConfig) {
    ...
    const handleExportKeys = useCallback(async () => {
        ...
        if (!confirmed || !isCurrentAuthority(requestAuthority))
            return;
        ...
        if (exportToken === null || !isCurrentAuthority(requestAuthority))
            return;
        if (!exportToken.trim()) {
    ...
    // ── Return ─────────────────────────────────────────────────────────
    const walletStateIsCurrent = walletStateAuthorityRef.current === authority &&
        getActiveAgentAuthority() === authority;
    const setWalletAddressesForAuthority = useCallback((next: WalletAddresses | null) => {
