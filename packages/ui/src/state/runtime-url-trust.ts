...
const REMOTE_FALLBACK_RUNTIME_GLOBAL = "__ELIZA_BUILD_CONFIGURED_REMOTE_API_BASE__";
...
function configuredRemoteFallbackApiBase(): string | undefined {
    ...
    const runtimeValue = (globalThis as RemoteFallbackRuntimeGlobal)[REMOTE_FALLBACK_RUNTIME_GLOBAL];
...
export function installBuildConfiguredRemoteApiBaseUrl(apiBase: string): void {
    ...
    if (!resolved) {
        throw new Error("[runtime-url-trust] build-configured remote target must be a root HTTPS origin");
    }
    ...
    if (typeof current === "string" && current !== resolved) {
        throw new Error("[runtime-url-trust] build-configured remote target is already locked");
    }
 ...
 */
export function getBuildConfiguredRemoteApiBaseUrl(configuredBase = configuredRemoteFallbackApiBase()): string | null {
 ...
 */
export function isTrustedBuildConfiguredRemoteApiBaseUrl(apiBase: string | undefined, configuredBase = configuredRemoteFallbackApiBase()): boolean {
    if (!apiBase)
        return false;
...
export function isTrustedRestoreApiBaseUrl(apiBase: string | undefined): boolean {
    if (!apiBase)
        return false;
    ...
    if (isMobileLocalAgentIpcBase(apiBase))
        return true;
    let parsed: URL;
    try {
    ...
    if (isTrustedBuildConfiguredRemoteApiBaseUrl(apiBase))
        return true;
    const host = parsed.hostname.toLowerCase().replace(/^\[|\]$/g, "");
    if (isLoopbackHostname(host) || host === "0.0.0.0")
        return true;
...
function agentIdMatches(candidate: string, expectedAgentId: string | null): boolean {
 ...
 */
export function isTrustedCloudApiBaseUrl(apiBase: string | undefined, expectedAgentId?: string | null): boolean {
    if (!apiBase)
        return false;
    const hasExpectedAgentId = expectedAgentId !== undefined && expectedAgentId !== null;
    const normalizedExpectedAgentId = expectedAgentId?.trim().toLowerCase() ?? "";
    if (hasExpectedAgentId &&
    ...
    if (isCloudPairLoopbackOrigin(url.origin)) {
        ...
        const match = /^\/api\/v1\/eliza\/agents\/([^/]+)(?:\/bridge|\/api)?$/.exec(normalizedPath);
    ...
    if ((classified.role !== "dedicated-agent" &&
    ...
    if (!isCloudPairAgentId(candidate))
        return false;
