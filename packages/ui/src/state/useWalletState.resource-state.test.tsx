...
function renderWalletState() {
    return renderHook(() => useWalletState({
...
beforeEach(() => {
    ...
    mocks.client.getAuthorityRevision.mockImplementation(() => mocks.authority.revision);
...
describe("useWalletState resource lifecycle", () => {
    it("hides the prior wallet and drops stale responses for a same-host profile switch", async () => {
        ...
        act(() => {
            ...
            for (const onChange of mocks.authority.listeners)
                onChange();
        });
    ...
    it("classifies an unsupported NFT endpoint without faking an empty response", async () => {
        mocks.client.getWalletNfts.mockRejectedValueOnce(new ApiError({
            kind: "http",
