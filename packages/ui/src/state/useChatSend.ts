...
import { UNDELIVERED_TURN_NOTICE } from "./chat-send-failures";
import { VIEW_EVENTS } from "../views/view-event-types";
import { applyStreamingTextModification } from "./internal";
import { asRecord } from "@elizaos/core/type-guards";
import { buildChatViewMetadata } from "./chat-view-routing";
import { buildSendFailureNotice } from "./chat-send-failures";
import { captureCompletedActionNavigationFence } from "../completed-action-navigation";
import { clearChatDraft } from "./ChatComposerContext.hooks";
import { clearPendingChatTurn } from "./pending-chat-turns";
import { client } from "../api";
import { consumePendingCapabilityIntent } from "../capability-handoff";
...
import { findCapabilityHandoff } from "../capability-handoff";
import { findViewActionHandoff } from "../view-action-handoff";
import { formatSearchBullet } from "./internal";
import { generateChatClientMessageId } from "../api/client-base";
import { getSendValidationFailureMessage } from "./chat-send-failures";
...
import { markPendingCapabilityReady } from "../capability-handoff";
import { mergeStreamingText } from "./internal";
import { persistPendingChatTurn } from "./pending-chat-turns";
import { readPendingCapabilityReadyAgentId } from "../capability-handoff";
import { rememberCapabilityHandoff } from "../capability-handoff";
import { resolveAbortRoomId } from "./chat-send-failures";
import { sentUserTurnPresent } from "./chat-send-failures";
import { shouldApplyFinalStreamText } from "./internal";
...
import { type ConversationChannelType } from "../api";
import { type ConversationMessage } from "../api";
import { type ConversationMessageStateMutation } from "./useDataLoaders";
import { type ImageAttachment } from "../api";
import { type LoadConversationMessagesResult } from "./internal";
import { type MessageAttachmentContentType } from "../api";
import { type MutableRefObject } from "react";
import { type StreamingTextModification } from "./internal";
...
type ConversationStreamResult = Awaited<ReturnType<typeof client.sendConversationMessageStream>>;
...
export { buildSendFailureNotice, getSendValidationFailureMessage, resolveAbortRoomId, UNDELIVERED_TURN_NOTICE, } from "./chat-send-failures";
function createCompletedViewHandoff(isTurnCurrent: () => boolean, showFailure: (message: string) => void): (actionResults: ChatActionResultSummary[] | undefined) => boolean {
    ...
    return (actionResults) => {
        ...
        try {
            ...
            if (viewHandoff.completedActionHandoffId ||
                ...
                dispatchViewActionHandoffDirect(actionResults, (detail) => dispatchCompletedActionNavigation({
                    ...detail,
                    completedActionHandoffId: viewHandoff.completedActionHandoffId ?? fallbackHandoffId,
                }));
            }
        }
        catch (err) {
            ...
            logger.warn({ err }, "[useChatSend] completed VIEWS action could not reach the renderer");
            showFailure("The agent chose a view, but the app couldn't open it. Try opening the view again.");
        }
        return true;
    };
}
async function handoffCompletedAction(actionResults: ChatActionResultSummary[] | undefined, handoffView: ReturnType<typeof createCompletedViewHandoff>): Promise<void> {
    ...
    if (successfulActions.length > 0) {
        ...
        emitViewEvent(VIEW_EVENTS.VIEW_REFRESH, {
            actionNames: successfulActions.flatMap((result) => result.actionName ? [result.actionName] : []),
        }, "agent");
    }
    if (handoffView(actionResults))
        return;
    if (dispatchDoorDashHumanHandoff(actionResults))
        return;
...
function optimisticAttachmentKind(mimeType: string): MessageAttachmentContentType {
...
function captureCompletedTurnForHistoryRefresh(messages: readonly ConversationMessage[], ids: {
    userReceiptId: string;
    ...
    return {
        ...
        user: messages.find((message) => message.role === "user" &&
            (message.id === ids.userReceiptId ||
                message.id === ids.optimisticUserMessageId ||
                message.clientRenderId === ids.optimisticUserMessageId)) ?? null,
        assistant: messages.find((message) => message.role === "assistant" &&
            (message.id === ids.assistantReceiptId ||
                ...
                message.clientRenderId === ids.optimisticAssistantMessageId)) ?? null,
    };
}
/**
 ...
 */
function hasNewerUserTurn(messages: readonly ConversationMessage[], origin: AssistantTurnOrigin): boolean {
    const originIds = new Set([
        origin.optimisticUserMessageId,
        ...(origin.persistedUserMessageId ? [origin.persistedUserMessageId] : []),
    ]);
    let originIndex = messages.findIndex((message) => message.role === "user" &&
    ...
    return messages.some((message) => message.role === "user" && message.timestamp > origin.sentAt);
}
function abortServerConversationTurn(roomId: string | null | undefined, reason: string): void {
    if (!roomId)
        return;
    ...
    void client.abortConversationTurn(roomId, reason).catch((err) => {
        logger.warn(`[useChatSend] abortConversationTurn(${roomId}) failed: ${err instanceof Error ? err.message : String(err)}`);
    });
}
...
function createOptimisticTurn(clientMessageId: string): QueuedChatSend["optimisticTurn"] {
    return {
...
function createOptimisticUserMessage(turn: Pick<QueuedChatSend, "rawInput" | "images" | "optimisticTurn">): ConversationMessage {
    ...
    return {
        ...
        role: "user",
        text: turn.images?.length && !rawText
...
export interface UseChatSendDeps {
    ...
    isConversationMessagesOwnershipCurrent: (conversationId: string | null, generation: number) => boolean;
    registerConversationMessageOverlay: (conversationId: string | null, lineages: readonly string[], explicitMessages?: readonly ConversationMessage[]) => void;
    applyConversationMessageOverlayModification: (conversationId: string | null, lineage: string, modification: StreamingTextModification, options?: {
        onlyIfEmpty?: boolean;
    }) => void;
    removeConversationMessageStateMessages?: (conversationId: string, mutation: ConversationMessageStateMutation) => void;
...
// ── Hook ────────────────────────────────────────────────────────────
export async function createConversationForFirstSend(chatClient: Pick<typeof client, "createConversation" | "getBaseUrl">, lang: string, title?: string): Promise<{
    conversation: Conversation;
}> {
    const sharedAgentId = directCloudSharedAgentIdFromBase(chatClient.getBaseUrl());
    if (sharedAgentId) {
...
export async function prewarmSharedChatScope(chatClient: Pick<typeof client, "getBaseUrl" | "getStatus">): Promise<void> {
    if (!directCloudSharedAgentIdFromBase(chatClient.getBaseUrl()))
        return;
...
export function useChatSend(deps: UseChatSendDeps) {
    const { t, uiLanguage, tab, activeConversationId, chatInput = "", firstRunComplete = true, ptySessionsRef, setChatInput, setChatSending, setChatFirstTokenReceived, setServerTurnStatus, setChatLastUsage, setChatPendingImages, setConversations, setActiveConversationId, setCompanionMessageCutoffTs, setConversationMessages, setUnreadConversations, setChatReplyTarget, setActionNotice, activeConversationIdRef, chatInputRef, chatPendingImagesRef, chatReplyTargetRef, conversationsRef, conversationMessagesRef, conversationHydrationEpochRef, chatAbortRef, chatSendBusyRef, chatSendNonceRef, loadConversations, loadConversationMessages, claimConversationMessagesOwnership, isConversationMessagesOwnershipCurrent, registerConversationMessageOverlay, applyConversationMessageOverlayModification, removeConversationMessageStateMessages, discardConversationMessageState, settleConversationHydrationForSend, elizaCloudEnabled, elizaCloudConnected, pollCloudCredits, } = deps;
    ...
    const isConversationCommitActive = useCallback((conversationId: string | null): boolean => activeConversationIdRef.current === conversationId, [activeConversationIdRef]);
    const createTurnViewHandoff = useCallback((owner: ChatViewHandoffOwner) => createCompletedViewHandoff(() => !unmountingRef.current &&
        ...
        isConversationMessagesOwnershipCurrent(owner.conversationId, owner.generation), (message) => setActionNotice(message, "error", 8000)), [
        activeConversationIdRef,
        isConversationMessagesOwnershipCurrent,
        setActionNotice,
    ]);
    const setConversationMessagesForConversation = useCallback((conversationId: string | null, value: ConversationMessage[] | ((prev: ConversationMessage[]) => ConversationMessage[])) => {
    ...
    const applyStreamingModificationForConversation = useCallback((conversationId: string | null, modification: Parameters<typeof applyStreamingTextModification>[1]) => {
        if (!isConversationCommitActive(conversationId)) {
            applyConversationMessageOverlayModification(conversationId, modification.messageId, modification);
            return;
        }
    ...
    const reconcileTerminalStream = useCallback((conversationId: string, assistantMessageId: string, streamedAssistantText: string, data: ConversationStreamResult, options: {
        ...
        const capabilityHandoff = findCapabilityHandoff(data.actionResults, directCloudSharedAgentIdFromBase(client.getBaseUrl()));
        if (capabilityHandoff) {
            rememberCapabilityHandoff(data.messageId ?? assistantMessageId, capabilityHandoff);
        }
        ...
        if (data.completed && data.messageId && !data.assistantEphemeral) {
            setConversationMessagesForConversation(conversationId, (prev) => {
                const next = prev.filter((message) => message.assistantEphemeral !== true);
        ...
        if (data.interrupted === true || !data.completed) {
            const interruptedText = data.interrupted === true
                ? data.text
    ...
    const setServerTurnStatusForConversation = useCallback((conversationId: string | null, status: ChatTurnStatus | null) => {
        if (!isConversationCommitActive(conversationId))
            return;
    ...
    const commitStreamingBuffer = useCallback(() => {
        ...
        if (buffer.pendingText !== null) {
            ...
            if (commitVisible) {
                ...
                applyConversationMessageOverlayModification(buffer.conversationId, buffer.messageId, modification);
            }
            committed = true;
        }
        ...
        if (committed)
            buffer.lastFlushAtMs = performance.now();
    }, [
        applyConversationMessageOverlayModification,
        isConversationCommitActive,
        setConversationMessages,
    ...
    const startStreamingTurn = useCallback((conversationId: string, messageId: string) => {
        const buffer = streamingFlushRef.current;
    ...
    const ensureStreamingFlush = useCallback(() => {
        ...
        const commitScheduled = () => {
            if (buffer.flushGeneration !== generation)
                return;
            buffer.flushTimer = null;
            buffer.flushScheduled = false;
            commitStreamingBuffer();
        };
        const delayMs = streamingRenderDelayMs(buffer.lastFlushAtMs, performance.now());
        if (delayMs === 0) {
    ...
    const scheduleStreamingText = useCallback((conversationId: string, messageId: string, fullText: string, provisional = false) => {
    ...
    const scheduleServerTurnStatus = useCallback((conversationId: string, messageId: string, status: ChatTurnStatus | null) => {
        startStreamingTurn(conversationId, messageId);
    ...
    const scheduleToolEvent = useCallback((conversationId: string, messageId: string, event: ChatToolCallEvent) => {
        startStreamingTurn(conversationId, messageId);
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (activeTurn?.abortServerTurn) {
                activeTurn.controller.signal.removeEventListener("abort", activeTurn.abortServerTurn);
            }
    ...
    const resolveQueuedChatSends = useCallback((conversationId: string | null): RestoredQueuedDraft => {
        ...
        chatSendQueueRef.current.splice(0, chatSendQueueRef.current.length, ...retained);
        if (queued.length === 0)
            return { text: "", images: [] };
        const cancelledMessageIds = new Set(queued.flatMap((turn) => [
            turn.optimisticTurn.userMsgId,
            turn.optimisticTurn.assistantMsgId,
        ]));
        setConversationMessagesForConversation(conversationId, (prev) => prev.filter((message) => !cancelledMessageIds.has(message.id)));
        ...
        if (restored || restoredImages.length > 0) {
            setActionNotice(restoredImages.length > 0
                ? "Your unsent message and attachments were restored to the input."
                : "Your unsent message was restored to the input.", "info", 6000);
        }
    ...
    const interruptActiveChatPipelineWithDraft = useCallback((): RestoredQueuedDraft => {
        sendCancellationGenerationRef.current += 1;
        const activeTurn = activeChatTurnRef.current;
        const restoredQueuedDraft = resolveQueuedChatSends(activeConversationIdRef.current ?? activeTurn?.conversationId ?? null);
    ...
    const appendLocalCommandTurn = useCallback((userText: string, assistantText: string, conversationId: string | null, ownershipGeneration: number | null) => {
        if (ownershipGeneration === null ||
            !isConversationMessagesOwnershipCurrent(conversationId, ownershipGeneration)) {
            return;
        }
        const now = Date.now();
        const nonce = Math.random().toString(36).slice(2, 8);
        setConversationMessagesForConversation(conversationId, (prev: ConversationMessage[]) => [
            ...prev,
            {
    ...
    const tryHandlePrefixedChatCommand = useCallback(async (rawText: string, conversationId: string | null, ownershipGeneration: number | null): Promise<{
        handled: boolean;
        rewrittenText?: string;
    }> => {
        const commitLocalCommandTurn = (userText: string, assistantText: string) => appendLocalCommandTurn(userText, assistantText, conversationId, ownershipGeneration);
        if (rawText.startsWith("#")) {
            const commandBody = rawText.slice(1).trim();
            if (!commandBody) {
                commitLocalCommandTurn(rawText, "Usage: #remember <text>, #memory <query>, #documents <query>, or #<query>.");
                return { handled: true };
            }
            ...
            const documentLines = documentResult?.results.map((item, index) => `${index + 1}. ${item.text.replace(/\s+/g, " ").trim()} (sim ${item.similarity.toFixed(2)})`) ?? [];
            commitLocalCommandTurn(rawText, [
                scope === "memory"
                    ? "Memory search"
                    : scope === "documents"
                        ? "Knowledge search"
        ...
        if (rawText.startsWith("$")) {
            ...
            if (queryRaw) {
                commitLocalCommandTurn(rawText, "Use bare `$` only. `$ <text>` is not supported.");
                return { handled: true };
            }
            ...
            const documentLines = quick.documents.map((item, index) => `${index + 1}. ${item.text.replace(/\s+/g, " ").trim()} (sim ${item.similarity.toFixed(2)})`);
            commitLocalCommandTurn(rawText, [
                quick.answer,
                "",
    ...
    const dropEmptyAssistantPlaceholder = useCallback((conversationId: string | null, assistantMsgId: string) => {
        if (!isConversationCommitActive(conversationId)) {
            applyConversationMessageOverlayModification(conversationId, assistantMsgId, { messageId: assistantMsgId, mode: "drop" }, { onlyIfEmpty: true });
            return;
        }
        setConversationMessagesForConversation(conversationId, (prev) => prev.filter((message) => !(message.id === assistantMsgId && !message.text.trim())));
    }, [
        applyConversationMessageOverlayModification,
        isConversationCommitActive,
        setConversationMessagesForConversation,
    ...
    const reattachInterruptedPartial = useCallback((conversationId: string | null, partialText: string, assistantEphemeral = false) => {
    ...
    const restoreEvictedUserTurn = useCallback((conversationId: string | null, turn: {
        userMsgId: string;
        assistantMsgId: string;
        text: string;
        timestamp: number;
        ...
        if (!sentText)
            return;
        setConversationMessagesForConversation(conversationId, (prev) => {
            if (sentUserTurnPresent(prev, sentText, turn.timestamp))
                return prev;
            return [
                ...prev,
                {
    ...
    // truth.
    const preserveCompletedTurnAfterHistoryRefresh = useCallback((conversationId: string | null, turn: {
        ...
        setConversationMessagesForConversation(conversationId, (prev) => {
            const userIds = new Set([turn.userReceiptId, turn.user.id, turn.user.clientRenderId].filter((value): value is string => Boolean(value)));
            const userIndex = prev.findIndex((message) => message.role === "user" &&
            ...
            let assistantIndex = assistant
                ? prev.findIndex((message) => message.role === "assistant" &&
            ...
            if (assistant && assistantIndex < 0 && !turn.assistantReceiptId) {
                assistantIndex = prev.findIndex((message) => message.role === "assistant" &&
    ...
    const runQueuedChatSend = useCallback(async (turn: Omit<QueuedChatSend, "resolve" | "reject">) => {
        ...
        const optimisticOwnerConversationId = turn.conversationId ?? activeConversationIdRef.current ?? null;
        ...
        const optimisticOwnerGeneration = activeConversationIdRef.current === optimisticOwnerConversationId
        ...
        const restoreUnsentSetup = (): void => {
            ...
            setConversationMessagesForConversation(optimisticOwnerConversationId, (previous) => previous.filter((message) => !ids.includes(message.id)));
            for (const messageId of ids) {
                applyConversationMessageOverlayModification(optimisticOwnerConversationId, messageId, { messageId, mode: "drop" });
            }
            if (optimisticOwnerGeneration !== null &&
                activeConversationIdRef.current === optimisticOwnerConversationId &&
                isConversationMessagesOwnershipCurrent(optimisticOwnerConversationId, optimisticOwnerGeneration)) {
        ...
        if (rawText) {
            ...
            try {
                commandResult = await tryHandlePrefixedChatCommand(rawText, optimisticOwnerConversationId, optimisticOwnerGeneration);
            }
            catch (err) {
                if (discardCancelledSetup())
                    return;
                appendLocalCommandTurn(rawText, `Command failed: ${err instanceof Error ? err.message : "unknown error"}`, optimisticOwnerConversationId, optimisticOwnerGeneration);
                return;
            }
            if (discardCancelledSetup())
                return;
        ...
        if (optimisticOwnerGeneration !== null &&
            !isConversationMessagesOwnershipCurrent(optimisticOwnerConversationId, optimisticOwnerGeneration)) {
            setConversationMessagesForConversation(optimisticOwnerConversationId, (prev) => prev.filter((message) => message.id !== turn.optimisticTurn.userMsgId &&
                message.id !== turn.optimisticTurn.assistantMsgId));
            applyConversationMessageOverlayModification(optimisticOwnerConversationId, turn.optimisticTurn.userMsgId, { messageId: turn.optimisticTurn.userMsgId, mode: "drop" });
            applyConversationMessageOverlayModification(optimisticOwnerConversationId, turn.optimisticTurn.assistantMsgId, { messageId: turn.optimisticTurn.assistantMsgId, mode: "drop" });
            return;
        }
        ...
        setConversationMessagesForConversation(optimisticOwnerConversationId, (prev: ConversationMessage[]) => {
            const userIndex = prev.findIndex((message) => message.id === userMsgId);
            const assistantIndex = prev.findIndex((message) => message.id === assistantMsgId);
            if (userIndex >= 0) {
                if (assistantIndex >= 0)
                    return prev;
                return [
        ...
        registerConversationMessageOverlay(optimisticOwnerConversationId, [userMsgId, assistantMsgId], [optimisticUserMessage, optimisticAssistantMessage]);
        if (isConversationCommitActive(optimisticOwnerConversationId)) {
            setChatFirstTokenReceived(false);
        }
        ...
        if (!convId) {
            try {
                const { conversation: rawConversation } = await createConversationForFirstSend(client, uiLanguage);
                if (discardCancelledSetup())
                    return;
                ...
                const shouldActivateCreatedConversation = optimisticOwnerGeneration !== null &&
                    activeConversationIdRef.current === optimisticOwnerConversationId &&
                    isConversationMessagesOwnershipCurrent(optimisticOwnerConversationId, optimisticOwnerGeneration);
                if (shouldActivateCreatedConversation) {
                    viewHandoffOwner.generation = claimConversationMessagesOwnership(conversation.id);
                ...
                restoreUnsentSetup();
                setActionNotice(`Couldn't start the conversation — check your connection and try again. ${imagesToSend?.length ? "Your message and attachments were restored." : "Your message was restored."}`, "error", 8000);
                return;
            }
        }
        ...
        if (activeConv &&
            ...
            const fallbackTitle = text.length > 15 ? `${text.slice(0, 15)}...` : text;
            setConversations((prev) => prev.map((c) => c.id === convId ? { ...c, title: fallbackTitle } : c));
        }
        ...
        try {
            const data = await client.sendConversationMessageStream(convId, text, (token, accumulatedText, provisional) => {
                ...
                // it (double-speak fix).
                scheduleStreamingText(convId, assistantMsgId, nextText, provisional === true);
            }, channelType, controller.signal, imagesToSend, turn.metadata, 
            ...
            const interruptedPartial = reconcileTerminalStream(convId, assistantMsgId, streamedAssistantText, data, {
            ...
            const completedTurnSnapshot = isConversationCommitActive(convId) &&
                data.completed &&
                data.userMessageId
                ? captureCompletedTurnForHistoryRefresh(conversationMessagesRef.current, {
            ...
            if (activeConversationIdRef.current === convId &&
                ...
                if (completedTurnSnapshot?.user) {
                    preserveCompletedTurnAfterHistoryRefresh(convId, {
                        ...
                        ...(completedTurnSnapshot.assistantReceiptId
                            ? {
                                assistantReceiptId: completedTurnSnapshot.assistantReceiptId,
                            }
                            : {}),
                    });
                }
                ...
                if (interruptedPartial) {
                    reattachInterruptedPartial(convId, interruptedPartial, data.assistantEphemeral === true);
                }
            ...
            const userMessageCount = isConversationCommitActive(convId)
                ? conversationMessagesRef.current.filter((message) => message.role === "user" && !message.id.startsWith("temp-")).length
                : null;
            if (userMessageCount === 1 &&
                ...
                void client
                    ...
                    // fresh either way.
                    logger.warn(`[useChatSend] conversation title generation failed: ${err instanceof Error ? err.message : String(err)}`);
                    void loadConversations();
                });
            }
            else {
            ...
            if (status === 404) {
                ...
                try {
                    ...
                    const createStatus = (createErr as {
                        status?: number;
                    }).status;
                    ...
                    if (createStatus === 404 && isCloudAgentBase(client.getBaseUrl())) {
                        setActionNotice("This agent is no longer reachable — it may have been deleted. Your message was kept; pick another agent and try again.", "error", 10000);
                        dropEmptyAssistantPlaceholder(convId, assistantMsgId);
                        return;
                    }
                ...
                try {
                    const nextCutoffTs = Date.now();
                    const shouldActivateReplay = activeConversationIdRef.current === convId &&
                        viewHandoffOwner.generation !== null &&
                        isConversationMessagesOwnershipCurrent(convId, viewHandoffOwner.generation);
                    ...
                    setConversationMessagesForConversation(conversation.id, replayMessages);
                    registerConversationMessageOverlay(conversation.id, [replayUserId, replayAssistantId], replayMessages);
                    let replayStreamedText = "";
                    const retryData = await client.sendConversationMessageStream(conversation.id, text, (token, accumulatedText, provisional) => {
                        const nextText = typeof accumulatedText === "string"
                            ? accumulatedText
                            : mergeStreamingText(replayStreamedText, token);
                        if (nextText === replayStreamedText)
                            return;
                        replayStreamedText = nextText;
                        if (isConversationCommitActive(conversation.id)) {
                            setChatFirstTokenReceived(true);
                        }
                        scheduleStreamingText(conversation.id, replayAssistantId, nextText, provisional === true);
                    }, channelType, controller?.signal, imagesToSend, turn.metadata, (serverStatus) => scheduleServerTurnStatus(conversation.id, replayAssistantId, serverStatus), (event) => scheduleToolEvent(conversation.id, replayAssistantId, event), 
                    ...
                    reconcileTerminalStream(conversation.id, replayAssistantId, replayStreamedText, retryData, {
                        includeReasoning: true,
                        includeAccountConnect: true,
                        origin: {
                    ...
                    if ((replayErr as Error).name !== "AbortError" &&
                ...
                if (getSendValidationFailureMessage(err) !== null) {
                    ...
                    setActionNotice(`${buildSendFailureNotice(err)} ${restored}`, "error", 10000);
                }
                else {
            ...
            setServerTurnStatusForConversation(convId || optimisticOwnerConversationId, null);
        ...
        const onPhase = (event: Event) => {
            ...
            if ((detail.phase === "switched" || detail.phase === "switched-empty") &&
                ...
                if (originalIntent) {
                    ...
                    setActionNotice("Your workspace is ready. Review your request, then send it when you want.", "success");
                }
            }
    ...
    const sendChatTextInternal = useCallback(async (rawInput: string, options?: ChatSendTextInternalOptions) => {
        ...
        if (!options?.[CHAT_SEND_HYDRATION_SETTLED] &&
        ...
        const metadata = replyTarget && !asRecord(options?.metadata)?.replyToMessageId
        ...
        const clientMessageId = identityOverride?.clientMessageId ??
            options?.clientMessageId ??
            generateChatClientMessageId();
        const optimisticTurn = identityOverride?.optimisticTurn ??
            createOptimisticTurn(clientMessageId);
        const conversationId = options?.conversationId ?? activeConversationIdRef.current ?? null;
        const queuedTurn = {
            rawInput,
        ...
        if (!isDrainPaintedCommand(rawInput)) {
            ...
            setConversationMessagesForConversation(conversationId, (prev: ConversationMessage[]) => {
                ...
                const current = prev.filter((message) => message.assistantEphemeral !== true);
                return current.some((message) => message.id === optimisticTurn.userMsgId)
                    ? current
    ...
    const sendChatText = useCallback((rawInput: string, options?: ChatSendTextOptions): Promise<void> => sendChatTextInternal(rawInput, options), [sendChatTextInternal]);
    const handleChatSend = useCallback(async (channelType: ConversationChannelType = "DM", options?: {
        metadata?: Record<string, unknown>;
    }) => {
        ...
        const admissionGeneration = sendCancellationGenerationRef.current;
        if ((await settleConversationHydrationForSend?.()) === false)
            return;
        if (admissionGeneration !== sendCancellationGenerationRef.current)
            return;
    ...
    const sendActionMessage = useCallback(async (text: string) => {
        ...
        let convRoomId: string | null = null;
        try {
            const optimisticOwnerConversationId = activeConversationIdRef.current ?? activeConversationId ?? null;
            viewHandoffOwner.conversationId = optimisticOwnerConversationId;
            viewHandoffOwner.generation = claimConversationMessagesOwnership(optimisticOwnerConversationId);
            let convId: string = optimisticOwnerConversationId ?? "";
            if (!convId) {
                try {
                    ...
                    const { conversation: rawConversation } = await createConversationForFirstSend(client, uiLanguage, actionTitle || t("common.newChat"));
                    if (admissionGeneration !== sendCancellationGenerationRef.current)
                        return;
                    if (!isConversationRecord(rawConversation)) {
                        throw new Error("Conversation creation returned an invalid payload.");
                    ...
                    if (activeConversationIdRef.current ===
                        ...
                        isConversationMessagesOwnershipCurrent(optimisticOwnerConversationId, viewHandoffOwner.generation)) {
                        viewHandoffOwner.generation = claimConversationMessagesOwnership(conversation.id);
                        viewHandoffOwner.conversationId = conversation.id;
                        setActiveConversationId(conversation.id);
                        activeConversationIdRef.current = conversation.id;
                    ...
                    // runQueuedChatSend).
                    setActionNotice("Couldn't start the conversation — check your connection and try again.", "error", 8000);
                    return;
                }
            }
            if (activeConversationIdRef.current === convId) {
            ...
            setConversationMessagesForConversation(convId, (prev: ConversationMessage[]) => [...prev, ...actionMessages]);
            registerConversationMessageOverlay(convId, [userMsgId, assistantMsgId], actionMessages);
            if (isConversationCommitActive(convId)) {
                setChatSending(true);
                setChatFirstTokenReceived(false);
            }
            ...
            let streamedAssistantText = "";
            try {
                const data = await client.sendConversationMessageStream(convId, trimmed, (token, accumulatedText, provisional) => {
                    const nextText = typeof accumulatedText === "string"
                        ? accumulatedText
                        : mergeStreamingText(streamedAssistantText, token);
                    if (nextText === streamedAssistantText)
                    ...
                    scheduleStreamingText(convId, assistantMsgId, nextText, provisional === true);
                }, "DM", controller.signal, undefined, buildChatViewMetadata(tab), 
                ...
                undefined, (event) => scheduleToolEvent(convId, assistantMsgId, event), undefined, handoffView);
                // Commit any token parked by the throttle before the terminal
                // drop/complete/fail/interrupt — no streamed tokens may be lost.
                flushStreamingText();
                if (data.userMessageId) {
    ...
    const handleChatStop = useCallback(() => {
        ...
        for (const session of ptySessionsRef.current) {
            ...
            client.stopCodingAgent(session.sessionId).catch((err) => {
                logger.warn(`[useChatSend] stopCodingAgent(${session.sessionId}) failed: ${err instanceof Error ? err.message : String(err)}`);
            });
        }
    ...
    const handleChatRetry = useCallback(async (assistantMsgId: string) => {
        ...
        const assistantIdx = currentMessages.findIndex((m) => m.id === assistantMsgId && m.role === "assistant");
        if (assistantIdx < 0)
            return;
        const assistantMessage = currentMessages[assistantIdx];
        const convId = activeConversationIdRef.current;
        const recoveryKey = `${convId}:${assistantMsgId}`;
        if (replyRecoveriesRef.current.has(recoveryKey))
            return;
        if (assistantMessage.replyRecoveryAvailable === true) {
            if (!convId || assistantMsgId.startsWith("temp-"))
                return;
            replyRecoveriesRef.current.add(recoveryKey);
            try {
                const reply = await client.retryConversationReply(convId, assistantMsgId);
                ...
                setActionNotice(`Could not regenerate reply: ${err instanceof Error ? err.message : "network error"}`, "error", 4200);
            }
            finally {
        ...
        if (assistantMessage.terminalFailure?.transient === false)
            return;
        ...
        if (!retryText)
            return;
        const canTruncate = Boolean(convId) &&
        ...
        if (canTruncate && convId) {
            ...
            try {
                ...
                await loadConversationMessages(convId);
                setActionNotice(`Failed to retry message: ${err instanceof Error ? err.message : "network error"}`, "error", 4200);
            }
            return;
        }
        ...
        const clientMessageId = optimisticUserId.startsWith("temp-")
            ...
            : "";
        setConversationMessages((prev) => prev.filter((m) => m.id !== assistantMsgId &&
    ...
    const handleChatEdit = useCallback(async (messageId: string, text: string): Promise<boolean> => {
        const convId = activeConversationIdRef.current;
    ...
    const handleChatDelete = useCallback(async (messageId: string): Promise<boolean> => {
        const convId = activeConversationIdRef.current;
        if (!convId)
            return false;
        const currentMessages = conversationMessagesRef.current;
        ...
        try {
            ...
            setActionNotice(`Failed to delete message: ${err instanceof Error ? err.message : "network error"}`, "error", 4200);
            return false;
        }
    }, [
        activeConversationIdRef,
    ...
    useEffect(() => {
        if (!firstRunComplete || chatInput.trim())
            return;
