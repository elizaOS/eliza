 ...
 */
import { LIFECYCLE_MESSAGES } from "./internal";
import { alertDesktopMessage } from "../utils";
import { clearAvatarIndex } from "./internal";
import { clearPersistedActiveServer } from "./internal";
...
import { loadPersistedActiveServer } from "./internal";
import { logger } from "@elizaos/ui/logger";
import { parseAgentStatusFromMainMenuResetPayload } from "./internal";
...
function logResetWarn(message: string, detail?: unknown): void {
    logger.warn(detail != null && typeof detail === "object"
        ? (detail as Record<string, unknown>)
        : {}, `${RESET_LOG_PREFIX} ${message}`);
}
async function waitForLifecycleIdle(lifecycleBusyRef: MutableRefObject<boolean>, timeoutMs: number): Promise<boolean> {
...
function publishElizaCloudVoiceSnapshot(setCloudVoiceProxyAvailable: (value: boolean) => void, setHasPersistedKey: (value: boolean) => void, snapshot: {
...
export function useChatLifecycle(deps: UseChatLifecycleDeps) {
    ...
    const { agentStatus, setAgentStatus, pollAgentReadiness = true, lifecycleAction, beginLifecycleAction, finishLifecycleAction, lifecycleBusyRef, lifecycleActionRef, setActionNotice, pendingRestart, pendingRestartReasons, setPendingRestart, setPendingRestartReasons, resetBackendConnection, loadPlugins, hydrateInitialConversationState, requestGreetingWhenRunning, interruptActiveChatPipelineWithDraft, resetConversationDraftState, setChatInput, setChatPendingImages, setActiveConversationId, setConversationMessages, setConversations, activeConversationIdRef, conversationHydrationEpochRef, claimConversationMessagesOwnership, discardConversationMessageState, elizaCloudPreferDisconnectedUntilLoginRef, setElizaCloudEnabled, setElizaCloudConnected, setElizaCloudVoiceProxyAvailable, setElizaCloudHasPersistedKey, setElizaCloudCredits, setElizaCloudCreditsLow, setElizaCloudCreditsCritical, setElizaCloudAuthRejected, setElizaCloudCreditsError, setElizaCloudTopUpUrl, setElizaCloudUserId, setElizaCloudStatusReason, setElizaCloudLoginError, firstRunCompletionCommittedRef, setFirstRunUiRevealNonce, setFirstRunLoading, setFirstRunComplete, setFirstRunDeferredTasks, setPostFirstRunChecklistDismissed, setFirstRunName, setFirstRunStyle, setFirstRunRuntimeTarget, setFirstRunProvider, setFirstRunRemoteConnected, setFirstRunRemoteApiBase, setFirstRunRemoteToken, setFirstRunOptions, setSelectedVrmIndex, setCustomVrmUrl, setCustomBackgroundUrl, setPlugins, setSkills, setLogs, coordinatorResetRef, } = deps;
    ...
    const handleStart = useCallback(async () => {
        if (!beginLifecycleAction("start"))
            return;
        setActionNotice(LIFECYCLE_MESSAGES.start.progress, "info", 300000, false, true);
        try {
    ...
    const handleStop = useCallback(async () => {
        if (!beginLifecycleAction("stop"))
            return;
        setActionNotice(LIFECYCLE_MESSAGES.stop.progress, "info", 120000, false, true);
        try {
    ...
    const handleRestart = useCallback(async () => {
        if (!beginLifecycleAction("restart"))
            return;
        setActionNotice(LIFECYCLE_MESSAGES.restart.progress, "info", 300000, false, true);
        try {
            setAgentStatus({
                ...(agentStatus ?? {
                    agentName: "Eliza",
    ...
    const showDesktopNotification = useCallback(async (options: {
        title: string;
        body?: string;
        ...
        await deliverSystemNotification({
            ...
            title: options.title,
            body: options.body,
            priority: options.urgency === "critical"
                ? "urgent"
                : options.urgency === "low"
                    ? "low"
                    : "normal",
    ...
    const notifyHeartbeatEvent = useCallback((event: StreamEventEnvelope) => {
        ...
        const eventTs = typeof payload.ts === "number"
            ? payload.ts
        ...
        const duration = typeof payload.durationMs === "number"
    ...
    const awaitingAgentReadiness = pollAgentReadiness && shouldAwaitAgentReadiness(agentStatus);
    useEffect(() => {
        if (!awaitingAgentReadiness)
            return;
        let active = true;
        ...
        const refresh = async () => {
            ...
            try {
                ...
                if (signature === readinessPollSignatureRef.current)
                    return;
    ...
    useEffect(() => {
        ...
        const signature = pendingRestartReasons.length > 0
        ...
        const summary = pendingRestartReasons.length === 1
    ...
    const completeResetLocalStateAfterServerWipe = useCallback(async (postResetAgentStatus: AgentStatus | null): Promise<void> => {
        await runCompleteResetLocalStateAfterServerWipe(postResetAgentStatus, {
            ...
            clearElizaCloudSessionUi: async () => {
                ...
                publishElizaCloudVoiceSnapshot(setElizaCloudVoiceProxyAvailable, setElizaCloudHasPersistedKey, {
                    apiConnected: false,
                    enabled: false,
    ...
    const handleResetAppliedFromMain = useCallback(async (payload: unknown) => {
        await handleResetAppliedFromMainCore(payload, {
            performanceNow: () => performance.now(),
            isLifecycleBusy: () => lifecycleBusyRef.current,
            getActiveLifecycleAction: () => lifecycleActionRef.current ?? lifecycleAction ?? "reset",
            beginLifecycleAction,
            finishLifecycleAction,
            setActionNotice,
    ...
    const completeConnectedAgentStateAfterServerWipe = useCallback(async (postResetAgentStatus: AgentStatus | null): Promise<void> => {
    ...
    const handleReset = useCallback(async () => {
        ...
        const resetTargetName = resetTarget.kind === "local"
            ? "local agent"
        ...
        if (lifecycleBusyRef.current) {
            ...
            setActionNotice(`Waiting for current agent action to finish (${LIFECYCLE_MESSAGES[activeAction].inProgress}).`, "info", 12000, false, true);
            const idle = await waitForLifecycleIdle(lifecycleBusyRef, 10000);
            if (!idle) {
                logResetInfo("handleReset: skipped — lifecycle remained busy", {
                    activeAction,
                });
                setActionNotice(`Agent action already in progress (${LIFECYCLE_MESSAGES[activeAction].inProgress}). Please wait.`, "info", 4200);
                return;
            }
        }
        ...
        if (!beginLifecycleAction("reset")) {
            logResetInfo("handleReset: beginLifecycleAction raced with another action — waiting");
            const idle = await waitForLifecycleIdle(lifecycleBusyRef, 10000);
            if (!idle || !beginLifecycleAction("reset")) {
                logResetInfo("handleReset: forcing lifecycle lock clear after confirmed reset", {
                    idle,
                    activeAction: lifecycleActionRef.current,
                });
                finishLifecycleAction();
                if (!beginLifecycleAction("reset")) {
                    logResetInfo("handleReset: aborted — could not begin lifecycle after forced clear");
                    setActionNotice("Another agent operation is still running. Wait for it to finish, then try Reset again.", "info", 4200);
                    return;
                }
            }
        }
        if (lifecycleActionRef.current !== "reset") {
            logResetInfo("handleReset: lifecycle action ref was not reset after begin; continuing reset", { activeAction: lifecycleActionRef.current });
        }
        setActionNotice(LIFECYCLE_MESSAGES.reset.progress, "info", 120000, false, true);
        ...
        logResetInfo("handleReset: tip — reset logs also appear in this window (filter [eliza][reset]); API terminal only shows server-side routes");
        try {
            ...
            if (resetTarget.kind !== "local") {
                ...
                try {
                    ...
                    logResetDebug("handleReset: connected-agent restartAndWait completed", {
                        state: postResetAgentStatus.state,
                        port: postResetAgentStatus.port,
                    });
                }
                catch (httpErr) {
                    logResetWarn("handleReset: connected-agent restartAndWait failed — preserving connection and clearing local lists", httpErr);
                }
            ...
            logResetDebug("handleReset: applying local UI reset before local restart wait");
            ...
            const isMobileLocalInProcessReset = resetTarget.kind === "local" &&
            ...
            let postResetAgentStatus: AgentStatus | null = null;
            logResetDebug("handleReset: invoking desktop bridge agentRestartClearLocalDb");
            ...
            try {
                postResetAgentStatus = await Promise.race([
                    ...
                    new Promise<AgentStatus | null>((_, reject) => {
                        window.setTimeout(() => {
                            reject(Object.assign(new Error(`agentRestartClearLocalDb exceeded ${BRIDGE_RESTART_MS / 1000}s`), { name: "ResetBridgeTimeout" }));
                        }, BRIDGE_RESTART_MS);
                    }),
                ]);
                ...
                if (postResetAgentStatus == null && isElectrobunRuntime()) {
                    logResetWarn("handleReset: agentRestartClearLocalDb RPC returned null — bridge request missing; will rely on HTTP restart path");
                }
            }
            catch (bridgeErr) {
                postResetAgentStatus = null;
                if (bridgeErr instanceof Error &&
                    bridgeErr.name === "ResetBridgeTimeout") {
                    logResetWarn("handleReset: agentRestartClearLocalDb timed out — falling back to HTTP restart", bridgeErr);
                }
                else {
                    logResetWarn("handleReset: bridge agentRestartClearLocalDb threw (will try HTTP restart)", bridgeErr);
                }
            }
            const embeddedRestartedOk = postResetAgentStatus != null &&
            ...
            if (!embeddedRestartedOk) {
                logResetInfo("handleReset: calling client.restartAndWait(120s) — external API or bridge inactive");
                try {
                    postResetAgentStatus = await client.restartAndWait(120000);
                    ...
                    postResetAgentStatus = null;
                    logResetWarn("handleReset: client.restartAndWait failed — UI may be stale until manual restart", httpErr);
                }
            }
            if (postResetAgentStatus != null) {
                setAgentStatus(postResetAgentStatus);
