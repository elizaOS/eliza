 ...
 */
import { DEFAULT_ACCENT_ID } from "./ui-preferences";
import { DEFAULT_BACKGROUND_COLOR } from "./ui-preferences";
import { DEFAULT_BACKGROUND_CONFIG } from "./ui-preferences";
...
import { normalizeAccentId } from "./ui-preferences";
...
export function saveHomeTimeWidgetHidden(hidden: boolean): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(HOME_TIME_WIDGET_HIDDEN_STORAGE_KEY, hidden ? "1" : "0");
    }, undefined);
}
export function saveUiThemeMode(mode: UiThemeMode): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(UI_THEME_MODE_STORAGE_KEY, normalizeUiThemeMode(mode));
    }, undefined);
}
...
export function normalizeBackgroundConfig(value: unknown): BackgroundConfig {
    ...
    const rawImageUrl = typeof record.imageUrl === "string" && record.imageUrl.length > 0
        ? record.imageUrl
        : undefined;
    ...
    if (record.mode === "glsl") {
        ...
        const preset = getShaderPreset(typeof shaderRecord?.presetId === "string"
...
export function loadBackgroundConfig(): BackgroundConfig {
    return tryLocalStorage(() => {
        ...
        const migrated = localStorage.getItem(UI_BACKGROUND_DEFAULT_MIGRATION_KEY);
        if (!migrated) {
...
export function saveBackgroundConfig(config: BackgroundConfig): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(UI_BACKGROUND_STORAGE_KEY, JSON.stringify(normalizeBackgroundConfig(config)));
    }, undefined);
}
/**
...
export function saveBackgroundHistory(history: BackgroundConfig[]): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(UI_BACKGROUND_HISTORY_STORAGE_KEY, JSON.stringify(normalizeBackgroundHistory(history)));
    }, undefined);
}
...
export function saveBackgroundRedo(redo: BackgroundConfig[]): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(UI_BACKGROUND_REDO_STORAGE_KEY, JSON.stringify(normalizeBackgroundHistory(redo)));
    }, undefined);
}
/**
...
const CLOUD_ONLY_FIRST_RUN_COMPLETE_STORAGE_KEY = "eliza:first-run-complete:cloud-only:v1";
/**
...
export function saveUiLanguage(language: UiLanguage): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(UI_LANGUAGE_STORAGE_KEY, normalizeLanguage(language));
    }, undefined);
}
...
export function saveUiShellMode(mode: UiShellMode): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(UI_SHELL_MODE_STORAGE_KEY, normalizeUiShellMode(mode));
    }, undefined);
}
...
export function saveLastNativeTab(tab: Tab): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(LAST_NATIVE_TAB_STORAGE_KEY, normalizeLastNativeTab(tab));
    }, undefined);
}
/* ── Avatar persistence ───────────────────────────────────────────────── */
...
export function saveAvatarIndex(index: number): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(AVATAR_INDEX_KEY, String(normalizeAvatarIndex(index)));
    }, undefined);
}
...
export function saveFavoriteApps(apps: string[]): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(FAVORITE_APPS_KEY, JSON.stringify(sanitizeFavoriteApps(apps)));
    }, undefined);
}
/**
 ...
 */
export async function replaceServerFavoriteApps(favoriteAppNames: string[]): Promise<string[] | null> {
    try {
 ...
 */
export async function toggleServerFavoriteApp(appName: string, isFavorite: boolean): Promise<string[] | null> {
    try {
        const resp = await fetchWithCsrf("/api/apps/favorites", {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
...
export function saveRecentApps(apps: string[]): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(RECENT_APPS_KEY, JSON.stringify(apps.slice(0, RECENT_APPS_MAX)));
    }, undefined);
}
...
const OS_INTENT_TRANSCRIPTION_AUTO_START_KEY = "eliza:voice:os-intent-auto-start-transcription";
...
export function loadOsIntentAutoStartConsent(): OsIntentAutoStartConsent {
    return tryLocalStorage(() => ({
        ...
        transcription: localStorage.getItem(OS_INTENT_TRANSCRIPTION_AUTO_START_KEY) === "true",
    }), { voice: false, transcription: false });
}
export function saveOsIntentAutoStartConsent(consent: OsIntentAutoStartConsent): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(OS_INTENT_VOICE_AUTO_START_KEY, String(consent.voice));
        shellLocalStorage.setItem(OS_INTENT_TRANSCRIPTION_AUTO_START_KEY, String(consent.transcription));
    }, undefined);
}
...
export function saveCompanionMessageCutoffTs(value: number): void {
    tryLocalStorage(() => {
        shellLocalStorage.setItem(COMPANION_MESSAGE_CUTOFF_TS_KEY, String(Math.max(0, Math.trunc(value))));
    }, undefined);
}
...
function normalizePersistedActiveServer(value: unknown): PersistedActiveServer | null {
    const record = asRecord(value);
    if (!record) {
        return null;
    }
    ...
    const cloudRuntime = record.cloudRuntime === "shared" || record.cloudRuntime === "dedicated"
        ? record.cloudRuntime
        : undefined;
    return {
        id,
...
function persistedRecordNeedsControlPlaneRepair(raw: unknown, normalized: PersistedActiveServer | null): boolean {
    if (!normalized)
        return false;
...
export function savePersistedActiveServer(server: PersistedActiveServer): boolean {
    if (typeof localStorage === "undefined") {
        return false;
    }
    ...
    if (pinnedRemoteApiBase &&
        ...
        logger.warn("[persistence] rejected active-server change outside the build-pinned remote target");
        return false;
    }
    ...
    try {
        shellLocalStorage.setItem(ACTIVE_SERVER_STORAGE_KEY, JSON.stringify(server));
        return true;
    }
    catch (err) {
...
export function clearPersistedActiveServer(): void {
    ...
    if (pinnedRemoteApiBase) {
        ...
        const preserved = current?.kind === "remote" &&
