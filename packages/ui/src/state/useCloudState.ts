 ...
 */
import { ANDROID_CLOUD_AUTH_RESULT_EVENT } from "../android-cloud/android-cloud-auth";
import { ANDROID_CLOUD_AUTH_STARTED_EVENT } from "../android-cloud/android-cloud-auth";
import { CLOUD_LOGIN_POPUP_NAME } from "./cloud-login-launch";
import { Capacitor } from "@capacitor/core";
import { beginAndroidCloudSignIn } from "../android-cloud/android-cloud-auth";
import { bindDirectCloudLoginToPersonalAgent } from "./bind-direct-cloud-login";
import { cancelAndroidCloudSignIn } from "../android-cloud/android-cloud-auth";
import { clearAndroidCloudAccountSwitchPending } from "../android-cloud/android-cloud-auth";
...
import { closeExternalBrowser } from "../utils";
...
import { getInjectedEthereumProvider } from "./cloud-siwe-login";
import { hasDirectCloudAccountTransport } from "../api/client-cloud";
import { hasStewardLoginLauncher } from "./cloud-steward-login";
import { hasUsableStoredStewardToken } from "./cloud-steward-login";
import { invokeDesktopBridgeRequestWithTimeout } from "../bridge";
import { isAndroidCloudAccountSwitchPending } from "../android-cloud/android-cloud-auth";
import { isAndroidCloudBuild } from "../platform/android-runtime";
import { isAndroidLauncherBuild } from "../platform/android-runtime";
...
import { isLoopbackStagingStewardDevelopment } from "./cloud-login-launch";
import { isPrivateNetworkHost } from "./private-network-host";
import { isSafeNavigationUrl } from "../utils";
import { isViteDevUiShell } from "../platform/vite-dev-ui-shell";
import { launchStewardLogin } from "./cloud-steward-login";
import { listenForExternalBrowserFinished } from "../utils";
import { loadPersistedActiveServer } from "./persistence";
import { logger } from "@elizaos/ui/logger";
import { markAndroidCloudAccountSwitchPending } from "../android-cloud/android-cloud-auth";
import { navigateAndroidCloudSignInInApp } from "../android-cloud/android-cloud-auth";
import { navigatePreOpenedWindow } from "../utils";
import { navigateToSameTabCloudLogin } from "./cloud-login-launch";
...
import { shouldUseSameTabCloudLogin } from "./cloud-login-launch";
...
import { siweLoginWithInjectedWallet } from "./cloud-siwe-login";
import { supportsFullAppShellRoutes } from "../api/app-shell-capabilities";
import { takeClaimedCloudLoginWindow } from "./cloud-login-launch";
import { takeLatestAndroidCloudCompletion } from "../android-cloud/android-cloud-auth";
import { takePreparedDesktopCloudLoginSession } from "./cloud-login-launch";
import { type AndroidCloudAuthResult } from "../android-cloud/android-cloud-auth";
import { type CloudCredits } from "../api";
import { type CloudLoginOptions } from "./types";
import { type CloudStatus } from "../api";
import { useCallback } from "react";
import { useEffect } from "react";
import { useRef } from "react";
import { useState } from "react";
import { verifyDirectCloudStewardSession } from "../api/client-cloud";
import { writeStoredStewardToken } from "@elizaos/plugin-elizacloud/steward-session-client";
import { yieldHttpAfterNativeMessageBox } from "../utils";
// ── Constants ──────────────────────────────────────────────────────────────
...
const CLOUD_SESSION_VERIFICATION_TRANSIENT_MESSAGE = "Eliza Cloud is temporarily unavailable. Retry in a moment.";
...
function publishElizaCloudVoiceSnapshot(setHasPersistedKey: (value: boolean) => void, snapshot: {
...
function isTrustedCloudAuthMessageOrigin(origin: string, cloudApiBase: string): boolean {
    if (!origin)
        return false;
    try {
...
function isMatchingCloudAuthCompleteMessage(data: unknown, sessionId: string): boolean {
    ...
    return (message.type === "eliza-cloud-auth-complete" &&
...
function hasCloudLoginBackend(): boolean {
    ...
    if (explicitBase) {
        if (isCloudOnlyElectrobunAssetBase(explicitBase))
            return false;
        return (!isConfiguredCloudSiteBase(explicitBase) &&
            !isCapacitorAssetBase(explicitBase));
    }
    if (isDevUiPortWithoutEmbeddedBackend())
        return false;
    if (isCloudOnlyElectrobunAssetBase(window.location.origin))
        return false;
...
function canPollCloudStatus(): boolean {
    ...
    if (hasDirectCloudAccountTransport(client))
        return true;
    if (isCapacitorNativeRuntime() || isElectrobunRuntime())
        return true;
    if (explicitBase && isConfiguredCloudSiteBase(explicitBase))
        return true;
...
function resolveStewardRefreshEndpoint(): string | undefined {
    if (!isCapacitorNativeRuntime() && !isElectrobunRuntime())
        return undefined;
    const cloudBase = getBootConfig().cloudApiBase?.trim() || DEFAULT_DIRECT_CLOUD_BASE_URL;
...
// ── Hook ───────────────────────────────────────────────────────────────────
export function useCloudState({ setActionNotice, loadWalletConfig, t, disconnectLocked = false, }: CloudStateParams) {
    // ── State ──────────────────────────────────────────────────────────
    const [elizaCloudEnabled, setElizaCloudEnabled] = useState(false);
    const [elizaCloudVoiceProxyAvailable, setElizaCloudVoiceProxyAvailable] = useState(false);
    const [elizaCloudConnected, setElizaCloudConnected] = useState(false);
    const [elizaCloudHasPersistedKey, setElizaCloudHasPersistedKey] = useState(false);
    const [elizaCloudCredits, setElizaCloudCredits] = useState<number | null>(null);
    ...
    const [elizaCloudCreditsError, setElizaCloudCreditsError] = useState<string | null>(null);
    ...
    const [elizaCloudStatusReason, setElizaCloudStatusReason] = useState<string | null>(null);
    const [cloudDashboardView, setCloudDashboardView] = useState<"overview" | "billing">("overview");
    const [elizaCloudStatusLoading, setElizaCloudStatusLoading] = useState(true);
    const cloudStatusPollsInFlight = useRef(0);
    const [elizaCloudStatusUnavailable, setElizaCloudStatusUnavailable] = useState(false);
    const [elizaCloudLoginBusy, setElizaCloudLoginBusy] = useState(false);
    const [elizaCloudLoginError, setElizaCloudLoginError] = useState<string | null>(null);
    /**
     ...
     */
    const [elizaCloudLoginFallbackUrl, setElizaCloudLoginFallbackUrl] = useState<string | null>(null);
    ...
    // ── Callbacks ──────────────────────────────────────────────────────
    async function runCloudPoll(intent: PollIntent = "ambient"): Promise<boolean> {
        ...
        if (intent === "session-verification" && buildPinnedRemoteApiBase) {
            ...
            const cloudApiBase = getBootConfig().cloudApiBase?.trim() || DEFAULT_DIRECT_CLOUD_BASE_URL;
            if (!stewardToken)
                return false;
            const verification = await verifyDirectCloudStewardSession({
                ...
                logger.warn({ err }, "[useCloudState] direct Cloud session verification failed");
        ...
        let cloudVoiceProxyAvailable = Boolean(cloudStatus.cloudVoiceProxyAvailable ?? false);
        ...
        setElizaCloudStatusReason(isConnected &&
        ...
        if (isConnected) {
            ...
            const credits = prefetchedCloudCredits !== undefined
        ...
        else if ((!isConnected || !canScheduleAmbientPolling) &&
            elizaCloudPollInterval.current) {
            clearInterval(elizaCloudPollInterval.current);
            elizaCloudPollInterval.current = null;
        }
    ...
    const pollCloudCredits = useCallback(async (intent: PollIntent = "ambient") => {
    ...
    const reconcileAndroidCloudSession = useCallback(async (cloudApiBase?: string): Promise<boolean> => {
        const token = readStoredStewardToken()?.trim();
        if (!token)
            return false;
        const authenticatedCloudApiBase = resolveDirectCloudAuthApiBase(cloudApiBase ??
        ...
        try {
            ...
            logger.warn({ err }, "[useCloudState] wallet config unavailable after Cloud auth");
        }
        if (!connected)
            return false;
        ...
        const reconcile = async (apiBase?: string) => {
            try {
                ...
                if (!cancelled && !connected && readStoredStewardToken()?.trim()) {
                    setElizaCloudLoginError("Could not verify your Eliza Cloud session. Please sign in again.");
                }
            }
            catch (err) {
                ...
                logger.warn({ err }, "[useCloudState] Android Cloud session reconciliation failed");
                if (!cancelled) {
                    setElizaCloudLoginError(err instanceof CloudSessionVerificationTransientError
    ...
    const handleCloudLogin = useCallback(async (prePoppedWindow: Window | null = null, options: CloudLoginOptions = {}) => {
        rememberCloudLoginPopup(prePoppedWindow);
        const closePrePoppedWindow = () => {
            closeCloudLoginPopup(prePoppedWindow);
        };
        let cloudAuthMessageHandler: ((event: MessageEvent) => void) | null = null;
        ...
        const hasRequiredClientAuth = () => !options.requireClientAuth || Boolean(getCloudAuthToken());
        ...
        if (options.forceReauth) {
            ...
            try {
                ...
                setElizaCloudLoginError(error instanceof Error
                    ? error.message
        ...
        if (isAndroidCloudBuild() && Capacitor.isNativePlatform()) {
            const cloudApiBase = getBootConfig().cloudApiBase ?? DEFAULT_DIRECT_CLOUD_BASE_URL;
            ...
            try {
                ...
                const authResult = new Promise<AndroidCloudAuthResult>((resolve) => {
                    ...
                    const onResult = (event: Event) => {
                        ...
                        if (result.retryable) {
                            ...
                            setElizaCloudLoginError(result.error ??
                    ...
                    window.addEventListener(ANDROID_CLOUD_AUTH_RESULT_EVENT, onResult);
                    removeResultListener = () => window.removeEventListener(ANDROID_CLOUD_AUTH_RESULT_EVENT, onResult);
                });
                ...
                const onCallbackStarted = (event: Event) => {
                    const startedAttemptId = (event as CustomEvent<{
                        attemptId?: string;
                    }>).detail?.attemptId;
                    if (startedAttemptId === attemptId)
                        callbackStarted = true;
                };
                window.addEventListener(ANDROID_CLOUD_AUTH_STARTED_EVENT, onCallbackStarted);
                removeCallbackStartedListener = () => window.removeEventListener(ANDROID_CLOUD_AUTH_STARTED_EVENT, onCallbackStarted);
                removeBrowserFinishedListener =
                    await listenForExternalBrowserFinished(() => {
                        ...
                        browserFinishTimer = window.setTimeout(() => {
                            ...
                            if (callbackStarted || callbackIsRetrying || !attemptId)
                                return;
                            resolveAuthResult({
                                attemptId,
                ...
                if (!result.ok) {
                    await cancelAndroidCloudSignIn(attempt.state);
                    throw new Error(result.error ?? "Eliza Cloud sign-in could not be completed.");
                }
                const connected = await reconcileAndroidCloudSession(result.apiBase ?? cloudApiBase);
                if (!connected) {
                    throw new Error("Could not verify your Eliza Cloud session. Please sign in again.");
                }
                try {
                    ...
                    logger.warn({ error }, "[useCloudState] Could not clear Android account-switch marker");
                }
            }
            catch (error) {
                androidLoginError = error;
                setElizaCloudLoginError(error instanceof Error ? error.message : "Eliza Cloud login failed");
            }
            finally {
                if (browserFinishTimer !== null) {
        ...
        if (!hasUsableStoredStewardToken() &&
            ...
            try {
                ...
                logger.warn({ err }, "[useCloudState] SIWE wallet login failed; falling through");
            }
        }
        ...
        const usesHostedLoopbackStagingSession = isLoopbackStagingStewardDevelopment();
        ...
        if (hasBackend) {
            ...
            const alreadyAuthenticated = isCloudStatusAuthenticated(Boolean(cloudStatus?.connected), cloudStatus?.reason);
            if (!options.forceReauth &&
                ...
                await loadWalletConfig().catch((err: unknown) => {
                    ...
                    logger.warn({ err }, "[useCloudState] wallet config refresh failed after cloud login");
                });
                setElizaCloudLoginError(null);
        ...
        const shouldBindClientToDirectCloud = useDirectAuth && !(usesHostedLoopbackStagingSession && hasBackend);
        ...
        if (shouldUseSameTabCloudLogin(prePoppedWindow, {
        ...
        try {
            ...
            if (sessionId && typeof window !== "undefined") {
                cloudAuthMessageHandler = (event: MessageEvent) => {
                    if (!isTrustedCloudAuthMessageOrigin(event.origin, authenticatedCloudApiBase)) {
                        return;
                    }
            ...
            if (resp.browserUrl && isSafeNavigationUrl(resp.browserUrl)) {
                ...
                // OAuth redirect_uri.
                if (usesHostedLoopbackStagingSession &&
                ...
                if (isElectrobunRuntime()) {
                    ...
                    if (!opened) {
                        setElizaCloudLoginError(`Couldn't open the sign-in browser. Open this link to log in: ${resp.browserUrl}`);
                    }
                }
                ...
                if (resp.browserUrl) {
                    ...
                    // failed login start.
                    setElizaCloudLoginError("The login link returned by the server is not a valid URL.");
            ...
            elizaCloudLoginPollTimer.current = window.setInterval(async () => {
                ...
                try {
                    ...
                    if (poll.status === "authenticated") {
                        ...
                        if (useDirectAuth) {
                            if (!poll.token) {
                                stopCloudLoginPolling("Eliza Cloud login completed, but the cloud session did not return a session token.");
                                return;
                            }
                            if (isElectrobunRuntime()) {
    ...
    useEffect(() => {
        ...
        const requestedReturn = sanitizeLoginReturnTo(new URL(window.location.href).searchParams.get("elizaCloudLoginReturnTo"));
        const accountReturn = isLoopbackStagingStewardDevelopment() &&
            requestedReturn &&
        ...
        void (async () => {
            ...
            if (cancelled || elizaCloudLoginBusyRef.current)
                return;
            clearCloudLoginReturnParams();
            elizaCloudLoginBusyRef.current = true;
            setElizaCloudLoginBusy(true);
            ...
            try {
                while (!cancelled && Date.now() < deadline) {
                    const poll = await client.cloudLoginPollDirect(authenticatedCloudApiBase, sessionId);
                    if (cancelled)
                        return;
                    if (poll.status === "authenticated") {
                        if (!poll.token) {
                        ...
                        const preservesLocalBackend = isLoopbackStagingStewardDevelopment() && hasCloudLoginBackend();
                        ...
                        void closeExternalBrowser();
                        if (accountReturn)
                            window.location.replace(accountReturn);
                        return;
                    }
                ...
                if (!cancelled) {
                    setElizaCloudLoginError(lastError ??
                        "Eliza Cloud login did not finish. Please sign in again.");
                }
            }
            catch (err) {
                if (!cancelled) {
                    setElizaCloudLoginError(err instanceof Error
                        ? err.message
                        : "Eliza Cloud login did not finish. Please sign in again.");
                }
            }
     ...
     */
    const handleInteractiveCloudLogin = useCallback((options?: CloudLoginOptions): Promise<void> => {
    ...
    const handleCloudLoginRecovery = useCallback((options?: CloudLoginOptions): Promise<void> => handleCloudLogin(null, options), [handleCloudLogin]);
    const handleCloudDisconnect = useCallback(async (opts?: {
        ...
        if (disconnectLocked || isElizaCloudRuntimeLocked()) {
            setActionNotice("Eliza Cloud is required while this app is running in cloud mode.", "error");
            return;
        }
            ...
            if (needRendererDisconnect) {
                await Promise.race([
                    ...
                    new Promise<never>((_, reject) => {
                        window.setTimeout(() => {
                            reject(new Error(`Disconnect timed out after ${RENDERER_DISCONNECT_MS / 1000}s`));
    ...
    const handleCloudSignOut = useCallback(async (): Promise<void> => {
        ...
        try {
            ...
            const nativeAndroidCloud = isAndroidCloudBuild() && Capacitor.isNativePlatform();
    ...
    useEffect(() => {
        ...
        const checkAndRefresh = async () => {
            ...
            if (secs === null)
                return;
            if (secs >= STEWARD_REFRESH_AHEAD_SECS)
                return;
            ...
            try {
                ...
                if (disposed)
                    return;
                if (result?.token) {
                    const replaced = await replaceStoredStewardTokenIfCurrent(storedToken, result.token);
