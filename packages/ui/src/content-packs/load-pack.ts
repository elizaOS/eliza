 ...
 */
import { CONTENT_PACK_MANIFEST_FILENAME } from "@elizaos/core/contracts/content-pack";
import { type ContentPackManifest } from "@elizaos/core/contracts/content-pack";
import { type ContentPackSource } from "@elizaos/core/contracts/content-pack";
import { type ResolvedContentPack } from "@elizaos/core/contracts/content-pack";
import { validateContentPackManifest } from "@elizaos/core/contracts/content-pack";
...
function cancelManifestBody(body: Pick<ReadableStream<Uint8Array>, "cancel"> | undefined | null, reason: unknown): void {
    if (!body)
        return;
...
async function cancelManifestReader(reader: ReadableStreamDefaultReader<Uint8Array>, reason: unknown): Promise<void> {
...
async function readBoundedManifestJson<T>(response: Response, signal: AbortSignal, sizeAbort: AbortController, maxBytes: number): Promise<T> {
    if (!Number.isSafeInteger(maxBytes) || maxBytes < 1) {
        throw new RangeError("Content pack manifest byte limit must be a positive safe integer");
    }
    ...
    if (!reader)
        throw new Error("Content pack manifest response has no body");
    ...
    try {
        ...
        bodyFailed = true;
        if (!bodyComplete)
            await cancelManifestReader(reader, error);
        throw error;
    }
    finally {
...
export async function getContentPackManifestJsonWithFetch<T>(url: string, fetchImpl: typeof fetch, timeoutMs: number = CONTENT_PACK_MANIFEST_FETCH_TIMEOUT_MS, maxBytes: number = CONTENT_PACK_MANIFEST_MAX_BYTES, callerSignal?: AbortSignal): Promise<T> {
    ...
    return await readBoundedManifestJson<T>(response, signal, sizeAbort, maxBytes);
}
export class ContentPackLoadError extends Error {
    constructor(message: string, public readonly source: ContentPackSource, public readonly cause?: unknown) {
        super(message);
 ...
 */
export async function loadContentPackFromUrl(baseUrl: string, options: {
    signal?: AbortSignal;
    ...
    let raw: unknown;
    try {
        raw = await getContentPackManifestJsonWithFetch(manifestUrl, globalThis.fetch, CONTENT_PACK_MANIFEST_FETCH_TIMEOUT_MS, CONTENT_PACK_MANIFEST_MAX_BYTES, options.signal);
    }
    catch (err) {
        ...
        throw new ContentPackLoadError(err instanceof ContentPackManifestTooLargeError
            ? err.message
            : `Failed to fetch pack manifest from ${manifestUrl}`, source, err);
    }
    const errors = validateContentPackManifest(raw);
    if (errors.length > 0) {
        throw new ContentPackLoadError(`Invalid pack manifest: ${errors.map((e) => `${e.field}: ${e.message}`).join("; ")}`, source);
    }
 ...
 */
export async function loadContentPackFromFiles(files: File[]): Promise<ResolvedContentPack> {
    const packFile = files.find((file) => file.webkitRelativePath.endsWith(CONTENT_PACK_MANIFEST_FILENAME) ||
        file.name === CONTENT_PACK_MANIFEST_FILENAME);
    if (!packFile) {
        throw new ContentPackLoadError("Could not find pack.json in the selected folder.", { kind: "file", path: "local-folder" });
    }
    let raw: unknown;
    try {
        ...
        throw new ContentPackLoadError("Failed to parse pack.json", { kind: "file", path: "local-folder" }, err);
    }
    const errors = validateContentPackManifest(raw);
    if (errors.length > 0) {
        throw new ContentPackLoadError(`Invalid pack manifest: ${errors.map((e) => `${e.field}: ${e.message}`).join("; ")}`, { kind: "file", path: "local-folder" });
    }
    const manifest = raw as ContentPackManifest;
    ...
    const resolveBlobUrl = (path: string | undefined): string | undefined => {
        ...
        const fileMatch = files.find((file) => {
            const relativeSegments = file.webkitRelativePath
                ...
                : [file.name];
            if (relativeSegments.length !== targetSegments.length)
                return false;
    ...
    const folderPath = packFile.webkitRelativePath
 ...
 */
export function resolveContentPackFromManifest(manifest: ContentPackManifest, baseUrl: string, source: ContentPackSource): ResolvedContentPack {
    const normalizedBase = baseUrl.endsWith("/") ? baseUrl : `${baseUrl}/`;
    return resolvePackAssets(manifest, normalizedBase, source);
}
function resolvePackAssets(manifest: ContentPackManifest, baseUrl: string, source: ContentPackSource): ResolvedContentPack {
    const { assets } = manifest;
    const resolve = (path: string | undefined) => path ? `${baseUrl}${path}` : undefined;
    return {
        manifest,
 ...
 */
export function loadBundledContentPack(manifest: ContentPackManifest, packsBaseUrl = "/packs"): ResolvedContentPack {
