...
async function postComposerActivity(args: {
    ...
    if (!res.ok) {
        throw new Error(`POST /api/interactions/composer returned HTTP ${res.status}`);
    }
...
export function reportComposerActivity(report: ComposerActivityReport): void {
    try {
        ...
        if (!base || typeof fetch === "undefined")
            return;
        if (!supportsFullAppShellRoutes(base))
            return;
        ...
        void postComposerActivity({ base, token, report }).catch((err) => {
            ...
            logger.warn(`[reportComposerActivity] composer report failed: ${err instanceof Error ? err.message : String(err)}`);
        });
    }
    catch (err) {
        // error-policy:J7 same guard for synchronous setup failures — telemetry
        // must never break composer input.
        logger.warn(`[reportComposerActivity] composer report setup failed: ${err instanceof Error ? err.message : String(err)}`);
    }
}
