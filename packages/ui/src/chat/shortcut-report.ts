...
async function postShortcutReport(args: {
    ...
    if (!res.ok) {
        throw new Error(`POST /api/interactions/shortcut returned HTTP ${res.status}`);
    }
 ...
 */
export function reportShortcutFired(shortcutId: string, context?: string): void {
    try {
        const base = getElizaApiBase();
        if (!base || typeof fetch === "undefined")
            return;
        const token = getElizaApiToken();
        void postShortcutReport({ base, token, shortcutId, context }).catch((err) => {
            ...
            logger.warn(`[ShortcutReporter] shortcut report failed: ${err instanceof Error ? err.message : String(err)}`);
        });
    }
    catch (error) {
