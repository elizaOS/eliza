...
import { catalogEntryToConfig } from "../state/ui-preferences";
...
import { mergeUniforms } from "./shader-schema";
import { resolveCatalogEntry } from "../state/ui-preferences";
import { type BackgroundApplyPayload } from "@elizaos/core/events";
import { type ShaderUniformValues } from "./shader-schema";
import { useBackgroundConfig } from "../state/useBackgroundConfig";
import { useViewEvent } from "../hooks/useViewEvent";
export { type BackgroundApplyOp, type BackgroundApplyPayload } from "@elizaos/core/events";
export { BACKGROUND_APPLY_EVENT };
/** Pull a Partial<ShaderUniformValues> out of an untrusted payload field. */
function readUniformPatch(value: unknown): Partial<Record<keyof ShaderUniformValues, unknown>> | undefined {
...
export function useBackgroundApplyChannel(): void {
    const { backgroundConfig, setBackgroundConfig, undoBackgroundConfig, redoBackgroundConfig, } = useBackgroundConfig();
    useViewEvent(BACKGROUND_APPLY_EVENT, (event) => {
        ...
        const imageUrl = typeof payload.imageUrl === "string" && payload.imageUrl.length > 0
            ? payload.imageUrl
            : undefined;
        ...
        const catalogId = typeof payload.catalogId === "string" ? payload.catalogId : undefined;
        if (catalogId) {
            ...
            if (!entry) {
                ...
                entry = loadUserBackgroundCatalog().find((e) => e.id.toLowerCase() === needle || e.label.toLowerCase() === needle);
            }
            if (entry) {
                const config = catalogEntryToConfig(entry, (id) => getShaderPreset(id)?.source);
                if (config)
        ...
        if (wantsGlsl) {
            ...
            if (!presetId &&
                ...
                current.mode === "glsl" &&
                current.shader) {
                setBackgroundConfig(makeGlslConfig({
