...
import { APPEARANCE_APPLY_EVENT } from "@elizaos/core/events";
import { UI_LANGUAGES } from "../i18n";
import { type AppearanceApplyPayload } from "@elizaos/core/events";
...
export function useAppearanceApplyChannel(): void {
    ...
    const setHomeTimeWidgetHidden = useAppSelector((state) => state.setHomeTimeWidgetHidden);
    useViewEvent(APPEARANCE_APPLY_EVENT, (event) => {
        ...
        if (themeMode)
            setUiThemeMode(themeMode);
        const accentId = readAccentId(payload.accentId);
        if (accentId)
            setUiAccent(accentId);
