...
import { chatDraftStorageKey } from "./state/ChatComposerContext.hooks";
...
vi.mock("./state", async () => {
    const { ACCENT_PRESETS } = await vi.importActual<typeof import("./state/ui-preferences")>("./state/ui-preferences");
    const getAppValue = () => ({
        actionNotice: null,
    ...
    return {
        ACCENT_PRESETS,
        useApp: () => getAppValue(),
        useAppSelector: <T,>(selector: (s: ReturnType<typeof getAppValue>) => T): T => selector(getAppValue()),
        useAppSelectorShallow: <T,>(selector: (s: ReturnType<typeof getAppValue>) => T): T => selector(getAppValue()),
        useTranslation: () => ({
...
vi.mock("./components/shell/ShellControllerContext", () => ({
    ShellControllerProvider: ({ children }: {
        children: React.ReactNode;
    }) => (<>{children}</>),
...
describe("App in-process host-realm mutation isolation (#14179)", () => {
    ...
    async function navigate(rerender: (ui: React.ReactElement) => void, tab: string, path: string): Promise<void> {
        await act(async () => {
            appState.tab = tab;
    ...
    it("replaces the scope when the mounted view's navigation grant changes", async () => {
        ...
        await act(async () => {
            mockAvailableViews = mockAvailableViews.map((view) => view.id === "iso-navigate"
        ...
        expect(() => current?.navigate("/settings")).toThrow(SurfaceRealmDeniedError);
    ...
    it("replaces the scope when a view ID moves to a different route", async () => {
        ...
        await navigate(rerender, "views", "/iso-navigate");
        const previous = getActiveSurfaceRealmScope();
        mockAvailableViews = mockAvailableViews.map((view) => view.id === "iso-navigate" ? { ...view, path: "/second-route" } : view);
    ...
    it("publishes only rendered layout members and revokes changed child policy", async () => {
        const { rerender } = render(<App />);
        await act(async () => {
            window.dispatchEvent(createNavigateViewEvent({
                action: "split-view",
                viewId: "iso-navigate",
        ...
        await act(async () => {
            mockAvailableViews = mockAvailableViews.filter((view) => view.id !== "iso-storage");
            rerender(<App />);
        });
    ...
    it("declares the Database vector child only under its builtin owner", async () => {
        ...
        await navigate(rerender, "settings", "/settings");
        expect(getActiveSurfaceRealmScope()?.ownsView("vector-browser")).toBe(false);
    }, 60000);
    it("scopes/blocks all four host-realm vectors across a fuzzed cross-view walk", async () => {
        for (const seed of [1, 7, 42]) {
            ...
            for (const route of order) {
                ...
                if (grantsStorage) {
                    expect(() => scope.storage.setItem(SHELL_STORAGE_KEY, "pwn")).toThrow(SurfaceRealmDeniedError);
                }
                else {
                    scope.storage.setItem(SHELL_STORAGE_KEY, "pwn");
                    // The namespaced write landed under the view keyspace, not the shell.
                    expect(window.localStorage.getItem(`${surfaceViewStoragePrefix(scope.viewId)}${SHELL_STORAGE_KEY}`)).toBe("pwn");
                }
                // Either way the shell key is intact.
                expect(window.localStorage.getItem(SHELL_STORAGE_KEY), `${route.path}: shell storage key intact`).toBe(SHELL_STORAGE_VALUE);
                ...
                expect(() => window.localStorage.setItem(SHELL_STORAGE_KEY, "pwn"), `${route.path}: raw reserved setItem denied`).toThrow(SurfaceRealmDeniedError);
                expect(() => window.localStorage.removeItem(SHELL_STORAGE_KEY), `${route.path}: raw reserved removeItem denied`).toThrow(SurfaceRealmDeniedError);
                expect(() => {
                    (window.localStorage as Record<string, unknown>)[SHELL_STORAGE_KEY] =
                        "pwn";
                }, `${route.path}: raw indexed reserved write denied`).toThrow(SurfaceRealmDeniedError);
                expect(window.localStorage.getItem(SHELL_STORAGE_KEY), `${route.path}: shell storage key intact after raw attack`).toBe(SHELL_STORAGE_VALUE);
                ...
                expect(window.localStorage.getItem(rawClearKey), `${route.path}: raw clear removed reachable non-reserved keys`).toBeNull();
                expect(window.localStorage.getItem(SHELL_STORAGE_KEY), `${route.path}: raw clear spared reserved shell keys`).toBe(SHELL_STORAGE_VALUE);
                // (d) navigation: a non-`navigate` view is denied; the route never moves.
                const grantsNavigate = route.path === "/iso-navigate";
                if (!grantsNavigate) {
                    expect(() => scope.navigate("/rogue-route")).toThrow(SurfaceRealmDeniedError);
                    ...
                    expect(() => window.history.pushState(null, "", "/rogue-raw"), `${route.path}: raw pushState denied`).toThrow(SurfaceRealmDeniedError);
                    expect(() => window.history.replaceState(null, "", "/rogue-raw"), `${route.path}: raw replaceState denied`).toThrow(SurfaceRealmDeniedError);
                    window.history.replaceState(null, "", `${route.path}#in-view-hash`);
                    expect(window.location.pathname, `${route.path}: route not hijacked`).toBe(route.path);
                }
                ...
                expect(document.documentElement.classList.contains(rogueRootClass), `${route.path}: injected root class did not survive`).toBe(false);
                expect(document.body.classList.contains(rogueBodyClass), `${route.path}: injected body class did not survive`).toBe(false);
                expect(document.documentElement.style.getPropertyValue(rogueVar), `${route.path}: injected CSS var did not survive`).toBe("");
            }
            cleanup();
        }
    }, 120000);
    it("a storage-granted view reaches the host keyspace (the grant is a real switch)", async () => {
        ...
        expect(window.localStorage.getItem(SHELL_STORAGE_KEY)).toBe(SHELL_STORAGE_VALUE);
    }, 60000);
    ...
    it("the privileged shell channel stays writable while a no-grant view is active", async () => {
        ...
        expect(window.localStorage.getItem(SHELL_STORAGE_KEY)).toBe("shell-updated");
    ...
    it("real shell persisters keep working under an active view scope (regression guard for missed migrations)", async () => {
        ...
        expect(window.localStorage.getItem(chatDraftStorageKey(conversationId))).toBe("half-typed message");
