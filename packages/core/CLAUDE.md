...
- Planner action retrieval ranks every authorized catalog parent without top-K omission. Relevance tiers are diagnostic only: every authorized parent and child remains callable, coding turns use the ordinary execution gates rather than a size-driven allowlist, and tool/provider/parameter descriptions plus examples render completely. Legacy limit and compressed-description fields are compatibility metadata, never model-facing caps.
