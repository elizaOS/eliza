...
- Participant association is not room authorization. `RoomMembershipEvidence` is generation-fenced transport evidence; only a fresh `member` observation (or `runtime:local` structural membership) belongs in document requester rooms. `nonmember`, `indeterminate`, `unsupported`, `unavailable`, missing, and expired evidence all deny room-derived access without erasing the association or its last observation.
