...
Stage-1 source annotations are advisory. Legacy selected/full mode outputs remain parseable, but the planner and completion evaluator receive every original dialogue source and complete routing evidence. A model's relevance judgment is not proof that omitted context is irrelevant. Authorized deferred-provider reads retain their explicit read contract and cannot remove dialogue or execute accompanying effects. Stage 1 receives complete authorized action descriptions, contexts and aliases before capability classification; subsequent exact-name schema reads retain canonical permission admission.
...
An explicitly registered historyRetention evaluator may annotate complete original dialogue through the existing incremental post-turn worker. Its committed source-bound checkpoint remains available for diagnostics and explicit authorized rereads. Every authorized original remains in the foreground model request, including sources previously marked deferred. Review annotations never authorize source omission or semantic projection. Recompose through current authorization before processing any reread; preserve cold-review failures as diagnostics. The optional advanced-memory plugin owns registration; group, voice and coding paths remain complete.

Stage 1 uses the complete-context schema when a legacy history reference index is available. Source annotations are advisory. A reread decision cannot execute draft fields or effects.
