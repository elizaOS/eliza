...
- Document reads use `roomId` as the single room entitlement and join it to the requester's current room set inside the adapter. Transport membership evidence has a hard 15-minute maximum TTL; participant presence is never authority. `directGrantEntityIds` is a bounded, validated read exception that remains valid without room membership; it never opens `agent-private` documents and never grants mutation authority. Only the dedicated adapter CAS may replace grants: OWNER on any valid document, or a current room ADMIN on global and user-private documents, with every grantee validated in the current agent tenant. Malformed or duplicate grant arrays make the parent unreadable.
- Participant association is not room authorization. `RoomMembershipEvidence` is generation-fenced transport evidence published through a direct plugin's source- and account-bound capability. Registration binds one stable account ID; the adapter verifies room source, persisted `metadata.connectorAccountId`, entity tenant, participant association, and generation in one transaction/lock, while provenance is derived rather than caller-authored. Room, world-room, and entity deletion atomically cascade participant state and evidence so deleted identifiers cannot retain or revive entitlement. The adapter CAS remains the trusted host/test storage boundary; direct plugins are in-process trusted code, not a sandbox. A publisher is inactive until plugin registration completes, revoked on failed init, and revoked plus drained before unload completes. Only a fresh `member` observation belongs in document requester rooms. `nonmember`, `indeterminate`, `unsupported`, `unavailable`, missing, and expired evidence all deny room-derived access without erasing the association or its last observation.
