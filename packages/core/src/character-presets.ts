...
import { DEFAULT_CHARACTER_LANGUAGE as DEFAULT_LANGUAGE } from "./character-language.js";
import { SHARED_STYLE_RULES } from "./character-presets.shared.js";
import { addLanguageRule } from "./character-language.js";
...
function resolveCharacterVariant(definition: CharacterDefinition, language: CharacterLanguage): StylePreset {
...
const STYLE_PRESET_CACHE = Object.fromEntries(CHARACTER_LANGUAGES.map((language) => [
    language,
    CHARACTER_DEFINITIONS.map((definition) => resolveCharacterVariant(definition, language)),
])) as Record<CharacterLanguage, StylePreset[]>;
const CHARACTER_DEFINITION_BY_ID = new Map(CHARACTER_DEFINITIONS.map((definition) => [
    definition.id.toLowerCase(),
    definition,
]));
const CHARACTER_DEFINITION_BY_NAME = new Map(CHARACTER_DEFINITIONS.map((definition) => [
...
const CHARACTER_DEFINITION_BY_AVATAR_INDEX = new Map<number, CharacterDefinition>();
for (const definition of CHARACTER_DEFINITIONS) {
    if (!CHARACTER_DEFINITION_BY_AVATAR_INDEX.has(definition.avatarIndex)) {
        CHARACTER_DEFINITION_BY_AVATAR_INDEX.set(definition.avatarIndex, definition);
    }
}
...
export function getStylePresets(language: unknown = DEFAULT_LANGUAGE): StylePreset[] {
...
export function getDefaultStylePreset(language: unknown = DEFAULT_LANGUAGE): StylePreset {
...
export function resolveStylePresetById(id: string | null | undefined, language: unknown = DEFAULT_LANGUAGE): StylePreset | undefined {
    if (!id) {
        return undefined;
    }
...
export function resolveStylePresetByName(name: string | null | undefined, language: unknown = DEFAULT_LANGUAGE): StylePreset | undefined {
    if (!name) {
        return undefined;
    }
    const normalized = normalizeCharacterLanguage(language);
    ...
    const lookupName = DEFAULT_AGENT_NAME_OVERRIDE &&
...
export function resolveStylePresetByAvatarIndex(avatarIndex: number | null | undefined, language: unknown = DEFAULT_LANGUAGE): StylePreset | undefined {
...
export const CHARACTER_PRESET_META: Record<string, {
    id: string;
    name: string;
    avatarIndex: number;
    voicePresetId?: string;
    catchphrase: string;
}> = Object.fromEntries(STYLE_PRESETS.map((preset) => [
    preset.catchphrase,
    {
        id: preset.id,
        name: preset.name,
...
export function getPresetNameMap(language: unknown = DEFAULT_LANGUAGE): Record<string, string> {
...
export function buildElizaCharacterCatalog(): {
    ...
    const assetsByAvatarIndex = new Map<number, {
        id: number;
        slug: string;
        title: string;
        sourceName: string;
    ...
    const assets = [...assetsByAvatarIndex.values()].sort((left, right) => left.id - right.id);
