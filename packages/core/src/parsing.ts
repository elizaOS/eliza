...
export function parseJSONObjectFromText(
    ...
    if (jsonBlockMatch) {
        // Use the inner JSON block content for parsing
        text = cleanJsonResponse(jsonBlockMatch[1]);
        try {
        ...
        if (objectMatch) {
            // Use the matched object text when present
            text = cleanJsonResponse(objectMatch[0]);
            try {
                jsonData = JSON.parse(text);
            } catch (e) {
                console.error("Error parsing JSON:", e);
    ...
    if (
        ...
        // Convert numeric primitive values to strings (recursively) to match historical behavior/tests
        const coerceNumbersToStrings = (obj: any) => {
            if (obj === null || typeof obj !== "object") return obj;
            if (Array.isArray(obj)) {
                return obj.map((v) =>
                    typeof v === "number" ? String(v) : coerceNumbersToStrings(v)
                );
            }
            Object.keys(obj).forEach((k) => {
                ...
                if (typeof v === "number") {
                    ...
                    obj[k] = coerceNumbersToStrings(v);
                }
            });
            return obj;
        };

        coerceNumbersToStrings(jsonData);
