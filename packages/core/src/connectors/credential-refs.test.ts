...
describe("connector credential persistence", () => {
	...
	it("rolls back vaulted secrets when the ref write fails", async () => {
		const secrets = new Map<string, string>();
		const runtime = runtimeWithServices({
			vault: {
				has: async (key: string) => secrets.has(key),
			...
			connector_account_storage: {
				setConnectorAccountCredentialRef: vi.fn(async () => {
					throw new Error("ref storage down");
				}),
			},
		});
		...
		await expect(
			persistConnectorCredentialRefs({
				...
				provider: "github",
				accountIdForRef: "owner",
				storageAccountId: "account-rollback",
				caller: "oauth.callback",
				credentials: [{ credentialType: "oauth.tokens", value: "secret" }],
			}),
		).rejects.toThrow(/Failed to persist connector credential refs/);
		expect(secrets.size).toBe(0);
	});

	it("preserves an existing credential when reconnect metadata fails", async () => {
		const vaultRef = buildConnectorCredentialVaultRef({
			agentId: "agent/reuse",
			provider: "github",
			accountId: "owner",
			credentialType: "oauth.tokens",
		});
		const secrets = new Map([[vaultRef, "existing-token"]]);
		const runtime = runtimeWithServices({
			vault: {
				has: async (key: string) => secrets.has(key),
		...
		await expect(
			persistConnectorCredentialRefs({
				...
				caller: "oauth.callback",
				credentials: [
					{ credentialType: "oauth.tokens", value: "replacement-token" },
				],
			}),
		).rejects.toThrow();
		expect(secrets.get(vaultRef)).toBe("existing-token");
	});

	it("serializes reconnect rollback before a newer reconnect commits", async () => {
		const secrets = new Map<string, string>();
		...
		const persist = (value: string) =>
			persistConnectorCredentialRefs({
				...
				storageAccountId: "account",
				caller: "oauth.callback",
				credentials: [{ credentialType: "oauth.tokens", value }],
			});
		const first = persist("first-token");
		const rejected = expect(first).rejects.toThrow();
		await entered;
		const second = persist("second-token");
		releaseFailure();
		await rejected;
		const result = await second;
		expect(secrets.get(result.refs[0].vaultRef)).toBe("second-token");
	});
