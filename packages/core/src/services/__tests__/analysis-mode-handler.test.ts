...
describe("isAnalysisModeAllowed", () => {
  it("honors the explicit env gate", () => {
    expect(isAnalysisModeAllowed({ ELIZA_ENABLE_ANALYSIS_MODE: "1" })).toBe(true);
    expect(isAnalysisModeAllowed({ ELIZA_ENABLE_ANALYSIS_MODE: "0" })).toBe(false);
  });
...
describe("maybeHandleAnalysisActivation", () => {
  ...
  it("enables analysis mode for a room", () => {
    const result = maybeHandleAnalysisActivation({ text: "analysis", roomId: "r1" }, env);
