		...
		events: [
			...context.events,
			{
				...
				segment: {
					id: "history-read-evidence",
					stable: false,
					content: `Completed current-turn conversation reads: ${JSON.stringify({ scope: "current_conversation", sourceSetId: bound.sourceSetId, matchMode: "case-insensitive literal substring", results: projection.searchResults.map((result) => ({ ...result, matchedSources: result.matchedSourceIds.length })) })}\nRuntime receipts, not a generated reply or an app-record lookup. Exact matches refer to original source IDs, not inferred facts or permission. An optional speaker field restricts that receipt to the named role; zero matches proves only literal absence within its scanned scope. These reads cover only this conversation, not other stored conversations. Even history:all has this boundary. For missing earlier conversations, use the authorized memory retrieval advertised by providers before claiming the history does not exist. These reads do not satisfy other pending tool work.`,
				},
			},
		],
	};
