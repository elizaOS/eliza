...
function looksLikeDelegationExcludedAsk(text: string): boolean {
	...
	if (
		/\b(?:do not|don't|dont|without)\s+(?:spawn|delegate|use|start)\s+(?:a\s+)?(?:sub[- ]?agent|task[- ]?agent|coding agent|elizaos|codex|claude)\b/iu.test(
			normalized,
		)
	) {
		return true;
...
function looksLikeExplicitDelegationRequest(text: string): boolean {
	const normalized = text.toLowerCase();
	return (
		/\b(?:spawn|delegate|use|start|ask|have)\b[\s\S]{0,80}\b(?:sub[- ]?agent|task[- ]?agent|coding agent|elizaos|codex|claude)\b/iu.test(
			normalized,
		) ||
		/\b(?:sub[- ]?agent|task[- ]?agent|coding agent|elizaos|codex|claude)\b[\s\S]{0,80}\b(?:build|create|make|implement|write|scaffold|fix|edit|modify|verify)\b/iu.test(
			normalized,
		)
	);
}
