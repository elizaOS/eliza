...
export function normalizeImageDescription(response: unknown): CachedImageDescription | null {
...
export async function getCachedImageDescription(runtime: IAgentRuntime, imageUrl: string): Promise<CachedImageDescription | undefined> {
...
export async function setCachedImageDescription(runtime: IAgentRuntime, imageUrl: string, value: CachedImageDescription): Promise<void> {
    if (!value.description && !value.text)
        return;
    await runtime
 ...
 */
export async function describeImageCached(runtime: IAgentRuntime, imageUrl: string, prompt: string): Promise<CachedImageDescription | null> {
