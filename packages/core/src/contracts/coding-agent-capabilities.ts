...
export type CodingProviderAuthMode = "oauth" | "direct-api-key" | "coding-plan-key" | "external-cli" | "unavailable";
export type CodingProviderBillingMode = "subscription-coding-plan" | "subscription-coding-cli" | "usage" | "api-payg" | "api-credits-or-byok";
export type CodingProviderSubscriptionAuthMode = "oauth" | "external-cli" | "coding-plan-key" | "unavailable";
export type CodingProviderSubscriptionBillingMode = "subscription-coding-plan" | "subscription-coding-cli";
export type CodingProviderDiscoveryPolicy = "bundled-or-configured-command" | "configured-command-or-path" | "none";
...
} as const satisfies Readonly<Record<CodingAgentBackend, CodingAgentBackendPreflight>>;
...
function descriptor<const ProviderId extends string, const AccountKind extends CodingProviderAccountKind, const AuthMode extends CodingProviderAuthMode, const BillingMode extends CodingProviderBillingMode, const InferenceSupport extends boolean, const Backend extends CodingAgentBackend | null>(providerId: ProviderId, accountKind: AccountKind, authMode: AuthMode, billingMode: BillingMode, enrollmentSupport: boolean, inferenceSupport: InferenceSupport, backend: Backend, unsupportedReason: string | null): CodingProviderDescriptor & {
...
export const CODING_PROVIDER_DESCRIPTORS = {
    "anthropic-subscription": descriptor("anthropic-subscription", "subscription", "oauth", "subscription-coding-cli", true, false, "claude", null),
    "openai-codex": descriptor("openai-codex", "subscription", "oauth", "subscription-coding-cli", true, true, "codex", null),
    "gemini-cli": descriptor("gemini-cli", "subscription", "external-cli", "subscription-coding-cli", false, false, null, "Gemini CLI is not wired to a supported coding-agent spawn backend."),
    "zai-coding": descriptor("zai-coding", "subscription", "coding-plan-key", "subscription-coding-plan", true, true, "pi-agent", null),
    "kimi-coding": descriptor("kimi-coding", "subscription", "coding-plan-key", "subscription-coding-plan", true, true, "pi-agent", null),
    "deepseek-coding": descriptor("deepseek-coding", "subscription", "unavailable", "subscription-coding-plan", false, false, null, "No first-party DeepSeek coding subscription surface or supported coding-agent spawn backend is available."),
    "anthropic-api": descriptor("anthropic-api", "api-key", "direct-api-key", "usage", true, true, null, "Anthropic API keys can serve model inference, but the coding-account bridge only supports Claude subscription accounts today."),
    "openai-api": descriptor("openai-api", "api-key", "direct-api-key", "usage", true, true, null, "OpenAI API keys can serve model inference, but the coding-account bridge only supports Codex subscription accounts today."),
    "deepseek-api": descriptor("deepseek-api", "api-key", "direct-api-key", "usage", true, true, "pi-agent", null),
    "zai-api": descriptor("zai-api", "api-key", "direct-api-key", "usage", true, true, "pi-agent", null),
    "moonshot-api": descriptor("moonshot-api", "api-key", "direct-api-key", "usage", true, true, "pi-agent", null),
    "cerebras-api": descriptor("cerebras-api", "api-key", "direct-api-key", "usage", true, true, null, "Cerebras API keys can serve model inference, but no supported coding-agent spawn backend consumes them."),
    "openrouter-api": descriptor("openrouter-api", "api-key", "direct-api-key", "api-credits-or-byok", true, true, "pi-agent", null),
    "xai-api": descriptor("xai-api", "api-key", "direct-api-key", "api-payg", true, true, "pi-agent", null),
} as const satisfies Readonly<Record<LinkedAccountProviderId, CodingProviderDescriptor>>;
...
export const CODING_AGENT_BACKEND_PROVIDERS = Object.fromEntries(CODING_AGENT_BACKENDS.map((backend) => [
    backend,
...
])) as unknown as Readonly<Record<CodingAgentBackend, readonly CodingProviderId[]>>;
export type CodingAgentAccountProviderId = (typeof CODING_AGENT_BACKEND_PROVIDERS)[CodingAgentBackend][number];
...
export function codingProviderDescriptorForProvider(providerId: string): CodingProviderDescriptor | undefined {
    return CODING_PROVIDER_DESCRIPTORS[providerId as CodingProviderId];
}
export type CodingProviderEnrollmentAvailability = "available" | "external" | "unavailable";
/** Resolve whether this app can enroll the provider, independently of use. */
export function codingProviderEnrollmentAvailability(providerId: string): CodingProviderEnrollmentAvailability {
    const provider = codingProviderDescriptorForProvider(providerId);
    if (!provider)
        return "unavailable";
    if (provider.authMode === "external-cli")
        return "external";
    return provider.enrollmentSupport ? "available" : "unavailable";
}
export type CodingSubscriptionProviderId = {
    [ProviderId in CodingProviderId]: (typeof CODING_PROVIDER_DESCRIPTORS)[ProviderId]["accountKind"] extends "subscription" ? ProviderId : never;
}[CodingProviderId];
export function isCodingSubscriptionProvider(providerId: string): providerId is CodingSubscriptionProviderId {
    return (codingProviderDescriptorForProvider(providerId)?.accountKind ===
        "subscription");
}
/** Maps a subscription descriptor to the auth package's enrollment contract. */
export function codingProviderSubscriptionAuthMode(providerId: CodingSubscriptionProviderId): CodingProviderSubscriptionAuthMode {
...
export function codingProviderSubscriptionBillingMode(providerId: CodingSubscriptionProviderId): CodingProviderSubscriptionBillingMode {
    const descriptor = CODING_PROVIDER_DESCRIPTORS[providerId];
...
export function codingAgentBackendForProvider(providerId: string): CodingAgentBackend | undefined {
 ...
 */
export function codingAgentSpawnCapabilityForProvider(providerId: string): CodingAgentSpawnCapability {
    ...
    return {
        available: false,
        unavailableReason: descriptor?.unsupportedReason ??
...
export type ProviderCredentialPath = Exclude<NonNullable<ProviderRuntimeCapability["credentialPath"]>, "none">;
/** Resolve where runtime consumers may obtain a linked provider credential. */
export function codingProviderCredentialPathForProvider(providerId: string): ProviderCredentialPath | undefined {
    const descriptor = codingProviderDescriptorForProvider(providerId);
    if (!descriptor)
        return undefined;
    if (descriptor.authMode === "external-cli")
        return "external-cli";
