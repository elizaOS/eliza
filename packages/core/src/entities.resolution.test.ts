...
describe("findEntityByName referent and candidate containment", () => {
	...
	it("preserves relationship context order and repeated counterparts when batch rows are unordered", async () => {
		const prompts: string[] = [];
		const missing = "00000000-0000-0000-0000-000000000099" as UUID;
		const counterparts = [STRANGER, ALICE, missing, STRANGER];
		await findEntityByName(
			runtime({
				prompts,
				getRelationships: async () =>
					counterparts.map((targetEntityId) => ({
						sourceEntityId: BOB,
						targetEntityId,
						agentId: AGENT,
						tags: [],
				...
				getEntitiesByIds: async () => [
					...
					structuredClone(stranger),
				],
			}),
			message("Which contact did I mean?"),
			state,
		);
		expect(prompts).toHaveLength(1);
		const prompt = prompts[0];
		const firstEve = prompt.indexOf('"Eve"');
		const alicePosition = prompt.indexOf('"Alice Smith"', firstEve);
		const secondEve = prompt.indexOf('"Eve"', firstEve + 1);
		expect(firstEve).toBeGreaterThan(-1);
		expect(alicePosition).toBeGreaterThan(firstEve);
		expect(secondEve).toBeGreaterThan(alicePosition);
