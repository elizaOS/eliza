...
describe('Utils Comprehensive Tests', () => {
  ...
  */
  ...
  describe('XmlTextStreamExtractor', () => {
    it('should extract text content from complete XML', () => {
      const extractor = new XmlTextStreamExtractor();

      const result = extractor.push('<response><thought>thinking</thought><text>Hello world!</text></response>');

      expect(result).toBe('Hello world!');
      expect(extractor.done).toBe(true);
    });

    it('should handle streaming chunks before <text> tag', () => {
      const extractor = new XmlTextStreamExtractor();

      // First chunk: before text tag
      let result = extractor.push('<response><thought>');
      expect(result).toBe('');
      expect(extractor.done).toBe(false);

      // Second chunk: still before text tag
      result = extractor.push('thinking</thought>');
      expect(result).toBe('');
      expect(extractor.done).toBe(false);

      // Third chunk: text tag starts
      result = extractor.push('<text>Hello');
      expect(result).toBe(''); // Buffer is too short, keeps last 7 chars
      expect(extractor.done).toBe(false);

      // Fourth chunk: more content
      result = extractor.push(' world!');
      expect(result).toBe('Hello'); // Now safe to stream "Hello"
      expect(extractor.done).toBe(false);

      // Fifth chunk: closing tag
      result = extractor.push('</text></response>');
    ...
    it('should handle <text> tag split across chunks', () => {
      ...
      // Split <text> across chunks
      let result = extractor.push('<response><te');
      expect(result).toBe('');

      result = extractor.push('xt>Content here</text>');
    ...
    it('should return empty string after done', () => {
      ...
      // After done, should return empty
    ...
    it('should handle large content before <text> tag without memory issues', () => {
      ...
      // Push large content before <text> (simulates large thought/actions)
      const largeContent = 'a'.repeat(150 * 1024); // 150KB
      let result = extractor.push(`<response><thought>${largeContent}</thought>`);
      expect(result).toBe('');
      expect(extractor.done).toBe(false);

      // Now push text content
    ...
    it('should stream content incrementally for long text', () => {
      ...
      chunks.push(extractor.push('Hello '));
      chunks.push(extractor.push('beautiful '));
      chunks.push(extractor.push('world! '));
      chunks.push(extractor.push('How are you today?'));
      chunks.push(extractor.push('</text>'));
    ...
    it('should handle special characters in text content', () => {
      ...
      const result = extractor.push('<text>Special chars: &lt;tag&gt; &amp; "quotes"</text>');
      expect(result).toBe('Special chars: &lt;tag&gt; &amp; "quotes"');
      expect(extractor.done).toBe(true);
    });
    ...
    it('should handle newlines in text content', () => {
      ...
      const result = extractor.push('<text>Line 1\nLine 2\nLine 3</text>');
      expect(result).toBe('Line 1\nLine 2\nLine 3');
      expect(extractor.done).toBe(true);
    });
  });
