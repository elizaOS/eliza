 ...
 *
 * Each consumer supplies {@link LocalRequestTrustPolicy} matching its historical
...
export function isTrustedLocalRequest(
	...
	options: LocalRequestTrustPolicy,
): boolean {
	if (options.localAuthRequired) return false;
	if (options.cloudProvisioned) return false;
