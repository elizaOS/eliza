...
describe("probe3", () => {
  it("runs", async () => {
    ...
    {
      const s = new PseudonymSession({ salt: "x", recognizer: new GazetteerEntityRecognizer([{ kind: "person", value: "Bob" }]) });
      await s.learn("Contact Bob about it");   // learns surface "Bob"
      const swapped = s.substituteText("Email BOB and bob and Bob now");
      log("[CASE-LEAK] entry", s.entries.map(e=>e.value), "swapped", swapped);
      log("[CASE-LEAK] BOB-leaked", /\bBOB\b/.test(swapped), "bob-leaked", /\bbob\b/.test(swapped));
    }
    // UNICODE-MID corruption: real value substituted inside an unrelated accented word -> real word Anaïs destroyed
    {
      const s = new PseudonymSession({ salt: "x", recognizer: new GazetteerEntityRecognizer([{ kind: "person", value: "Ana" }]) });
      const text = "Anaïs Nin wrote to Ana";
      await s.learn(text);
      const swapped = s.substituteText(text);
      log("[UNI-MID] swapped", swapped, "'Anaïs'-intact", swapped.includes("Anaïs"), "restore", s.restoreText(swapped), "restore-eq", s.restoreText(swapped)===text);
    }
    ...
    {
      const s = new PseudonymSession({ salt: "x", recognizer: new GazetteerEntityRecognizer([{ kind: "person", value: "Bo" }]) });
      const text = "Bornïte and Bo"; // "Bo" inside "Bornïte"? no, 'r' is a word char after Bo -> boundary blocks. good control.
      await s.learn(text);
      const swapped = s.substituteText(text);
      log("[UNI-CTRL] swapped", swapped, "restore-eq", s.restoreText(swapped)===text, "size", s.size);
    }
    ...
    // then the 21st must fallback. Confirm 21st surrogate is fallback-shaped and unique + restores.
    {
      ...
      const doc = vals.join(" ");
      s.learnSpans(doc, vals.map(v=>({kind:"location", value:v})) as any);
      const surrs = s.entries.map(e=>e.surrogate);
      const swapped = s.substituteText(doc);
      log("[EXH-512] size", s.size, "distinct", new Set(surrs.map(x=>x.toLowerCase())).size, "restore-eq", s.restoreText(swapped)===doc);
      // any surrogate NOT a bare city (contains a space+base36 tail) => fallback used
      log("[EXH-512] fallbacks", surrs.filter(x=>x.split(" ").length>1).length);
    }
    ...
    // IDEMPOTENCY when a surrogate token equals another entry's real value AND appears standalone (multi-call chain restore direction)
    {
      ...
      s.learnSpans("Priya", [{kind:"person", value:"Priya"}] as any);
      const surr = s.entries[0]!.surrogate;
      // now a person literally named like the surrogate arrives
      s.learnSpans(surr, [{kind:"person", value:surr}] as any);
      const doc = `Priya | ${surr}`;
      const sw1 = s.substituteText(doc);
      const sw2 = s.substituteText(sw1);
      log("[IDEM-CHAIN] idempotent", sw1===sw2, "restore-eq", s.restoreText(sw1)===doc, "sw1", sw1);
    }
    writeFileSync("/private/tmp/claude-501/-Users-shawwalters-eliza-workspace-milady-eliza/6d6de187-237a-4fc9-9689-34ed10a5d6e7/scratchpad/probe-out3.txt", out.join("\n"));
  });
});
