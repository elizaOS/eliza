...
import { readActionFailureProvenance } from "../types/action-failure.ts";
import { ModelType } from "../types/model.ts";
import { isProviderContextOverflowFailure } from "../utils/model-errors.ts";
import {
	findNextCloseTag,
	REASONING_TAG_NAMES,
...
/** A provider exhausted its own retry budget; do not retry another registration
 * owned by that provider in this dispatch. Other providers remain eligible. */
...
/** Recognizes a provider-owned exhausted retry budget through SDK wrappers. */
export function isModelProviderRetryBudgetExhaustedError(
	error: unknown,
): boolean {
	return (
