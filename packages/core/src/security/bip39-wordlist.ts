...
/**
 * Locate a valid BIP-39 mnemonic window inside a longer run of words and return
 * the exact matched substring (or null). A bare regex over a word run grabs the
 * surrounding words ("seed: abandon … about. then") and fails the checksum on
 * the wrong word count, so this slides over the candidate tokens — longest count
 * first — and requires consecutive words be whitespace-adjacent so a skipped
 * long word cannot bridge two non-adjacent fragments into a false positive.
 */
	...
	for (let i = 0; i < tokens.length; i += 1) {
		for (const count of [24, 21, 18, 15, 12]) {
			...
			if (mnemonicValid(phrase)) {
				...
				return text.slice(first.index ?? 0, (last.index ?? 0) + last[0].length);
			}
		}
	}
	return null;
