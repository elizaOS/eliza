...
// aliases), ordered. `@elizaos/core` statically imports this (browser-safe, no
...
function main(): void {
	...
	if (check) {
		for (const [path, expected] of artifacts) {
			...
			if (current !== expected) {
				console.error(
					`[registry/generate] ${path} is stale. Run \`bun run --cwd packages/core generate:first-party\` and commit the result.`,
				);
				process.exit(1);
			}
		}
