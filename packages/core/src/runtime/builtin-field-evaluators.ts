...
> = {
	...
	description:
		"Likely UPPER_SNAKE_CASE action names. Prefer available_actions; confident unlisted names ok. UI navigation plus view-backed Notes, Calendar, and native-device operations -> VIEWS. Life-management (goals/todos/reminders/routines) -> the matching AVAILABLE action (OWNER_REMINDERS, TRIGGER); hint, not a claim. Empty when no action likely.",
	descriptionCompressed:
		"Likely UPPER_SNAKE_CASE action names. UI navigation and view-backed Notes/Calendar/device operations -> VIEWS. Empty only when no action is needed.",
	priority: 50,
	schema: {
		type: "array",
		items: { type: "string" },
