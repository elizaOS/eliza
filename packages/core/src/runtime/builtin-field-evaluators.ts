...
> = {
	...
	description:
		...
		"Prefer exact available children. Sticky notes: NOTES_CREATE; NOTES_GET reads one named note (planner resolves its ID); NOTES_LIST searches/lists; NOTES_PATCH edits fields or exact text (NOTES_UPDATE for legacy full rewrites); NOTES_DELETE. Stored messages/saved facts: MEMORY_SEARCH for source contents; MEMORY_COUNT for fresh inventory totals, categories and newest timestamps (do not reuse old dialogue counts); writes: MEMORY_CREATE/MEMORY_UPDATE/MEMORY_DELETE. Calendar: CALENDAR_NEXT_EVENT=next event; CALENDAR_FEED=all events/counts in a date range without content filters; CALENDAR_SEARCH_EVENTS=title/attendee/location/keyword filter, optional date bounds; CALENDAR_PROPOSE_TIMES=verified free-time suggestions; CALENDAR_CHECK_AVAILABILITY=free/busy for an interval. Filtered searches cannot prove availability. Writes: CALENDAR_CREATE_EVENT/CALENDAR_UPDATE_EVENT/CALENDAR_DELETE_EVENT. One known app view: VIEWS_SHOW; other view/layout/native-device work: VIEWS. Navigation needs data actions only for requested data work. Preserve the original resource across clarification answers: calendar reminders stay CALENDAR_CREATE_EVENT, not OWNER_REMINDERS_CREATE; accepted alternative slots resume the original create or update operation unless the user changes it. Life management: matching available OWNER_* or TRIGGER. Umbrellas only for unresolved operations or unknown suitable children; further tools remain discoverable. " +
