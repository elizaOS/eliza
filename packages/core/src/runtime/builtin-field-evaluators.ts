...
> = {
	...
	description:
		"Source references for planning, execution and the final completion evaluator, never a summary. Review ALL [hN] user and assistant history blocks for every dependency of the FINAL CURRENT REQUEST, including action constraints and disambiguation, not just final prose. The normal result of a completed relevance review is mode=selected: echo completion_source_set as sourceSetId, set complete=true, and list every applicable exact h source ID. relevantSourceIds carries factual background; constraintSourceIds carries every applicable standing preference, permission, prohibition and correction; referentSourceIds resolves this/that/it and follow-ups; pendingIntentSourceIds carries unfinished work referenced by the current request. List each source ID once, in its most specific category; the runtime retains the union of all four lists. There is no cap. Do not discard a standing constraint merely because it is old or another request is newer. Completed unrelated one-off tasks are not pending instructions. If no prior source applies after review, selected/complete=true with four empty lists is valid. Full/complete=false is an unresolved-relevance fallback, not an empty or N/A value: use it if an applicable constraint, referent or dependency cannot be resolved, if the FINAL CURRENT REQUEST needs exhaustive history/count/recall, or if no source set is supplied. Long history and old unrelated recall requests alone do not require full mode. Include assistant proposals, exact IDs, receipts and pending work whenever referenced. Current request and standing provider constraints are always retained; future tool receipts are appended automatically, so not having tool results yet does not make this source review incomplete.",
	descriptionCompressed:
		"Review all h sources for the FINAL CURRENT REQUEST. Selected/complete=true with sourceSetId retains every applicable fact, standing constraint/correction, referent and referenced pending intent; reviewed empty lists are valid. List each source once across categories; all categories are retained together. No summaries/caps. Full/false is unresolved-context fallback, not N/A: use if applicable context is uncertain, current request needs exhaustive recall, or no source set. Current request/providers remain; future receipts arrive later.",
	priority: 16,
...
export const replyTextFieldEvaluator: ResponseHandlerFieldEvaluator<string> = {
	...
	description:
		...
		'User-facing reply. Populate when shouldRespond=RESPOND. contexts includes "simple" => whole answer. Other planning/tool work => brief ack ("On it.", "Spawning the sub-agent now.", "Looking into it."); planner sends grounded follow-up. IGNORE => empty. No thinking/reasoning.\n\nNEVER refuse in replyText on planning path. If `contexts` or `candidateActionNames` != "simple", planner handles work; no capability gatekeeping. Ban refusal openings: "I cannot...", "I am unable to...", "I don\'t have the ability to...", "Sorry, I can\'t...". Tools exist (FILE, BASH, TASKS_SPAWN_AGENT, etc.). If no tool can attempt, use shouldRespond=RESPOND, `contexts: ["simple"]`, explain.',
	descriptionCompressed:
