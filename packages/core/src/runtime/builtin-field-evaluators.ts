...
> = {
	...
	description:
		...
		"Prefer exact available children: NOTES_CREATE creates sticky notes; NOTES_LIST reads/searches/lists them; NOTES_UPDATE edits and NOTES_DELETE deletes. Calendar reads use CALENDAR_NEXT_EVENT, CALENDAR_FEED or CALENDAR_SEARCH_EVENTS; writes use CALENDAR_CREATE_EVENT, CALENDAR_UPDATE_EVENT or CALENDAR_DELETE_EVENT. One known app view -> VIEWS_SHOW; other view/layout/native-device work -> VIEWS. Navigation needs no data action unless data work is also requested. Life management -> matching available OWNER_* or TRIGGER operation. Use umbrellas only for unresolved operations or unknown suitable children; further tools remain discoverable. " +
