...
> = {
	...
	description:
		"Likely UPPER_SNAKE_CASE action names. Prefer available_actions; confident unlisted names ok. Opening or switching UI views -> OPEN_VIEW; view-backed Notes and native-device operations -> VIEWS; calendar-event reads/writes -> CALENDAR. Life-management (goals/todos/reminders/routines) -> the matching AVAILABLE action (OWNER_REMINDERS, TRIGGER); hint, not a claim. Empty when no action likely.",
	descriptionCompressed:
		"Likely UPPER_SNAKE_CASE action names. Open/switch UI -> OPEN_VIEW; Notes/device operations -> VIEWS; calendar events -> CALENDAR. Empty only when no action is needed.",
	priority: 50,
	schema: {
		type: "array",
		items: { type: "string" },
