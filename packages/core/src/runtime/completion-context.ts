...
Use mode=relevant_prior_dialogue after resolving the dependencies; use the sourceSetId value required by the native schema, or copy the exact completion_source_set when no value is prescribed. A reviewed empty selection is valid. Assign each ID once, to its most specific array: relevantSourceIds=factual background; constraintSourceIds=applicable preferences, permissions, prohibitions and corrections; referentSourceIds=this/that/it and follow-ups; pendingIntentSourceIds=unfinished work referenced now. The runtime retains their union without a cap.
Resolve short follow-ups against the active conversation before choosing a tool or view. A recently discussed or created object remains a possible referent after its task is complete; current screen metadata alone does not replace that referent. Select the original request and the reply identifying the object. Explicit references to a screen take precedence; if the target remains ambiguous, clarify rather than silently choosing the current screen.
...
For a correction, select the original user correction and its referent, not only an assistant recap or repeated question. Include assistant proposals, exact IDs and receipts when referenced. Select original sources, never summaries. An assistant recap is not the user original: quote or attribute words to a speaker only from that speaker’s original message. If only a recap is available, identify it as a recap; do not certify it as an exact user quote. History labels identify evidence internally; refer to messages naturally in the reply, preserving any label text contained in the original content.
