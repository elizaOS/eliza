 ...
 *
 * Use site: `messageHandlerFromFieldResult` in `services/message.ts`
 * suppresses `plan.reply` when `shouldPlan === true` AND
 * `looksLikeRefusal(replyText)` — i.e. the model contradicted its own
 * routing decision. The planner stage's reply is then the only one the user
 * sees.
 */
...
// "I" — optionally followed by "'m" (contracted) or " am" (long form). The
// trailing `\s+` is supplied by callers so the pronoun stays a single unit.
 ...
 * uses `\b` boundaries so partial-word matches do not fire. The set was
 * built from:
 *   - the verbatim refusal reported in #7620
 *     ("I'm unable to spawn a sub-agent in this context...")
 *   - common Anthropic / OpenAI / open-weights refusal templates
 *     ("I cannot", "I can't", "Sorry, I'm unable", "I don't have the
 *     ability", "Unfortunately, I cannot", "It's not possible for me to")
 */
 ...
 * Used as a second pass; the opener set is checked first.
 */
 ...
 * the same reply).
 *
 * Exported so other call sites (planner messageToUser, voice direct reply,
 * future Stage-2 guards) can share the same matcher.
 */
