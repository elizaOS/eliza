...
const MANDATORY_PLANNER_POLICY = [
	...
	`- native toolCalls: every tool requires the reserved arg \`eliza_turn_scope\` (stripped before execution); use the same batch scope on every call. ${plannerBatchScopeDescription}`,
].join("\n");
...
// The batch-scope rule is stated once in the planner instructions; repeating
// it per tool cost ~405 chars per exposed tool per planner call (2026-09-13).
