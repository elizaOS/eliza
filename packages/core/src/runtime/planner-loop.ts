...
async function runPlannerLoopIterations(
	...
	for (let iteration = 1; ; iteration++) {
		...
		if (
			...
			latestResult.data?.capabilityDiscovery !== undefined
		) {
			// Discovery mutates the append-only tool/context surface for the next
			// model call. It is never task completion and must not pass through the
			// completion gate or evaluator, both of which could terminate before the
			// newly loaded schema is usable. A model cannot validly batch an unknown
			// tool with discovery, so discard any residual queue and re-plan.
			trajectory.plannedQueue.length = 0;
			continue;
		}
...
const MANDATORY_PLANNER_POLICY = [
	...
	'- candidateActions naming a tool that is not in this turn\'s exposed tools list is a discovery hint — do not invent SHELL/BROWSER/TASKS workarounds to fulfill it. First scan the exposed tools for a genuine equivalent. If none fits, call DISCOVER_CAPABILITIES with a precise search query; matching actions become callable on the next planner iteration. Only explain that a capability is unavailable after an appropriate search returned no match. Never emit echo-placeholder SHELL commands such as: echo "<intent-name>" / echo "placeholder for <ACTION>" / echo "search <X>" as a way to "trigger" a missing capability — placeholder echoes burn cost and produce no progress.',
	"- DISCOVER_CAPABILITIES searches the complete authorization-filtered catalog without exposing denied metadata. Search results are explicit pages; follow nextCursor while hasMore is true when exhaustive traversal is required. Loading a context expands its authorized actions and providers for this same turn.",
