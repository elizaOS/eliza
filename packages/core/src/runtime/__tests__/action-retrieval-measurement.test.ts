 ...
 * reciprocal-rank-fusion ordering, preserves every result despite the legacy
 * tierOverrides.topK hint, and honors per-stage weights. Runs
...
describe("action-retrieval measurement mode", () => {
	...
	it("keeps every result when a legacy tierOverrides.topK is supplied", () => {
		const catalog = buildActionCatalog(actions);
		...
		expect(capped.results).toHaveLength(wide.results.length);
		expect(capped.results.map((r) => r.name)).toEqual(
			wide.results.map((r) => r.name),
		);
	});
