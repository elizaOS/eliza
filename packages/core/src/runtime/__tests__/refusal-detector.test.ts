...
describe("looksLikeRefusal", () => {
	describe("matches the verbatim refusals reported in elizaOS/eliza#7620", () => {
		// Both Cerebras-hosted models (gpt-oss-120b AND qwen-3-235b-instruct)
		// emit the SAME refusal openings on `spawn a sub-agent` prompts. Each
		// one must be caught so the suppression path fires in
	...
	describe("matches common refusal openings", () => {
		const refusals = [
			...
			// "As an AI ..." prefix is a classic alignment-tune opener
			"As an AI assistant, I cannot spawn sub-processes.",
			"As an AI, I'm unable to write files on your behalf.",
			// Hedge-then-refusal mid-sentence path (must still catch)
	...
	describe("does NOT match legitimate acknowledgements", () => {
		// Zero false positives on the planning-path-acknowledgement texts we
		// expect the model to actually emit when it correctly chose to route
		// to the planner.
	...
	describe("does NOT match incidental 'can't' / 'unable' mid-sentence", () => {
		// Mid-sentence 'can't' is fine — it's only a refusal opener when it
		// anchors the reply. These are realistic planning-path acks.
		const mid = [
	...
	describe("guards on empty / short / non-string inputs", () => {
		...
		it("returns false for short strings (< 12 chars)", () => {
			// "I cannot." alone would technically open a refusal but at 9
			// chars it does not carry enough signal to act on — keep the
			// guard so we don't suppress voice-style "I can't." replies.
