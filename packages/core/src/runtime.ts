...
export class AgentRuntime implements IAgentRuntime {
  ...
  async ensureConnection({
    ...
    await Promise.all([
      this.ensureParticipantInRoomOptimized(entityId, roomId, entity),
      this.ensureParticipantInRoomOptimized(this.agentId, roomId, undefined),
    ]);
  ...
  async ensureParticipantInRoom(entityId: UUID, roomId: UUID) {
    return this.ensureParticipantInRoomOptimized(entityId, roomId, undefined);
  }

  private async ensureParticipantInRoomOptimized(
    entityId: UUID,
    roomId: UUID,
    cachedEntity: Entity | undefined | null
  ) {
    const entity = cachedEntity !== undefined ? cachedEntity : await this.getEntityById(entityId);
