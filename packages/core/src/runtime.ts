...
export class AgentRuntime implements IAgentRuntime {
    ...
    async processActions(
        ...
        for (const response of responses) {
            ...
            // Diagnostic: log registered action names to help debugging
            try {
                const registered = this.actions.map((a) => a.name).join(",");
                elizaLogger.debug(`Registered actions: ${registered}`);
            } catch (e) {
                // ignore
            }
            ...
            if (!action) {
                // Fast-path: treat IGNORE as a harmless no-op rather than an error
                if (normalizedAction === "ignore" || normalizedAction.includes("ignore")) {
                    elizaLogger.info("Received IGNORE action from model; skipping action execution.");
                    continue;
                }
