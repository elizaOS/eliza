...
export const RUNTIME_EXECUTION_MODE_DEFINITIONS: Record<RuntimeExecutionMode, RuntimeExecutionModeDefinition> = {
    cloud: {
        mode: "cloud",
        local: false,
        cloud: true,
...
export function normalizeRuntimeExecutionMode(value: unknown): RuntimeExecutionMode | null {
...
export function runtimeExecutionModeForDeploymentTarget(deploymentTarget: DeploymentTargetConfig | null | undefined): RuntimeExecutionMode {
    return deploymentTarget?.runtime === "cloud" ? "cloud" : "local-safe";
}
export function readRuntimeExecutionModeConfig(config: RuntimeExecutionModeConfigSource | null | undefined): RuntimeExecutionMode {
 ...
 */
export function resolveRuntimeExecutionMode(source?: RuntimeExecutionModeSource | null): RuntimeExecutionMode {
    const clampForPlatform = (mode: RuntimeExecutionMode): RuntimeExecutionMode => isIosMobile() && mode === "local-yolo" ? "local-safe" : mode;
...
export function resolveLocalExecutionMode(source?: RuntimeExecutionModeSource | null): LocalExecutionMode {
    return resolveRuntimeExecutionMode(source) === "local-safe"
        ? "local-safe"
        : "local-yolo";
}
export function shouldUseSandboxExecution(source?: RuntimeExecutionModeSource | null): boolean {
    return resolveRuntimeExecutionMode(source) === "local-safe";
}
export function isCloudExecutionMode(source?: RuntimeExecutionModeSource | null): boolean {
