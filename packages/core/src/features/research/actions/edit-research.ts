import type {
	Action,
	HandlerCallback,
	HandlerOptions,
	IAgentRuntime,
	Memory,
	State,
} from "../../../types/index.ts";
import { logger } from "../../../types/index.ts";
import type { UUID } from "../../../types/primitives.ts";
import { getResearchService } from "../services/researchService.ts";
import type { EditResearchInput, ResearchStatus } from "../types.ts";

function readParams(options?: HandlerOptions): Record<string, unknown> {
	return options?.parameters && typeof options.parameters === "object"
		? (options.parameters as Record<string, unknown>)
		: {};
}

function extractId(message: Memory, options?: HandlerOptions): string | null {
	const params = readParams(options);
	const raw =
		params.id ?? message.content.id ?? message.content.researchId;
	return typeof raw === "string" && raw.trim() ? raw.trim() : null;
}

function resolveStatus(value: unknown): ResearchStatus | undefined {
	if (value === "open" || value === "resolved" || value === "archived") {
		return value;
	}
	return undefined;
}

export const editResearchAction: Action = {
	name: "EDIT_RESEARCH",
	contexts: ["research", "agent_internal"],
	description:
		"Edit an existing research thread. Can update the title or status.",
	similes: ["UPDATE_RESEARCH", "MODIFY_RESEARCH", "CHANGE_RESEARCH"],

	validate: async (
		_runtime: IAgentRuntime,
		message: Memory,
		_state?: State,
		options?: HandlerOptions,
	): Promise<boolean> => {
		return extractId(message, options) !== null;
	},

	handler: async (
		runtime: IAgentRuntime,
		message: Memory,
		_state: State | undefined,
		options: HandlerOptions | undefined,
		callback?: HandlerCallback,
	) => {
		const id = extractId(message, options);
		if (!id) {
			const msg = "Could not edit research: an id is required.";
			if (callback) {
				await callback({
					text: msg,
					actions: ["EDIT_RESEARCH_FAILED"],
					source: message.content.source,
				});
			}
			return { success: false, text: msg };
		}

		const params = readParams(options);
		const patch: EditResearchInput = {};

		if (typeof params.title === "string" && params.title.trim()) {
			patch.title = params.title.trim();
		}
		const status = resolveStatus(params.status);
		if (status !== undefined) {
			patch.status = status;
		}

		if (Object.keys(patch).length === 0) {
			const msg =
				"No changes provided. Supply at least one of: title, status.";
			if (callback) {
				await callback({
					text: msg,
					actions: ["EDIT_RESEARCH_FAILED"],
					source: message.content.source,
				});
			}
			return { success: false, text: msg };
		}

		try {
			const agentId = runtime.agentId as UUID;
			const userId =
				typeof message.entityId === "string"
					? (message.entityId as UUID)
					: agentId;

			const service = getResearchService(runtime);
			const research = await service.edit(agentId, userId, id as UUID, patch);

			const successMsg = `Updated research "${research.title}" (id: ${research.id}).`;
			if (callback) {
				await callback({
					text: successMsg,
					actions: ["EDIT_RESEARCH_SUCCESS"],
					source: message.content.source,
				});
			}
			return {
				success: true,
				text: successMsg,
				data: {
					id: research.id,
					title: research.title,
					status: research.status,
				},
			};
		} catch (error) {
			const errorMsg = error instanceof Error ? error.message : String(error);
			logger.error("[EditResearch] Error:", errorMsg);
			if (callback) {
				await callback({
					text: `Failed to edit research: ${errorMsg}`,
					actions: ["EDIT_RESEARCH_FAILED"],
					source: message.content.source,
				});
			}
			return { success: false, text: `Failed to edit research: ${errorMsg}` };
		}
	},

	parameters: [
		{
			name: "id",
			description: "ID of the research thread to edit.",
			required: true,
			schema: { type: "string" as const, minLength: 1 },
		},
		{
			name: "title",
			description: "New title.",
			required: false,
			schema: { type: "string" as const },
		},
		{
			name: "status",
			description: "New status.",
			required: false,
			schema: {
				type: "string" as const,
				enum: ["open", "resolved", "archived"],
			},
		},
	],
	examples: [],
};

export default editResearchAction;
