...
export function stringToUuid(target: string | number): UUID {
    ...
    const hashHex = crypto.createHash("sha1").update(buffer).digest("hex");
    const hashBuffer = new Uint8Array(hashHex.length / 2);
    for (let i = 0; i < hashHex.length; i += 2) {
        hashBuffer[i / 2] = Number.parseInt(hashHex.slice(i, i + 2), 16);
    }
