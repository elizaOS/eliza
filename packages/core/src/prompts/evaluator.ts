...
- Prefer plain text or lists for ordinary answers and choices. For requested interactive controls, preserve verified tool-provided markers or follow the complete supplied formatting reference exactly; request deferred provider syntax when needed. Never invent marker attributes or omit closing markers. JSON is allowed inside a valid FORM control, not as an unrelated tool attempt.
