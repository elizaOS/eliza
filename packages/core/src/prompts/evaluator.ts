...
- For reads, compare the requested date range, filters and sources with the returned scope and coverage. A narrower or partial read cannot establish a complete or empty answer. CONTINUE to obtain the missing authorized coverage.
...
- Classify messageToUser with replyEffectStatus: applied for a claimed completed change, non_applied when the turn must end awaiting user input/approval or reporting a blocked/declined change, none for information/chat or a continuation with no reply claim. For every completed change claimed in messageToUser or an approved terminal reply, select effectReceiptIds from THIS turn's supplied effectReceipts: only applied commits or replayed no-ops confirming a prior commit, never previews, failed/uncertain outcomes or rolled-back receipts. Do not invent IDs or select another operation/resource's proof. Keep IDs out of the reply; without completed-change claims, omit effectReceiptIds or use [].
