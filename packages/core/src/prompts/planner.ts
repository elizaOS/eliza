...
- native toolCalls: every tool requires the reserved arg \`eliza_turn_scope\` (stripped before execution). It describes whether ANOTHER ACTION BATCH is needed; use the same value on every call in this batch. Use "final" when the requested actions fit in this batch. You do not need to know their results yet: the evaluator receives the results and writes the complete user-facing answer, including read-dependent details and summaries. Do not reserve another batch just to answer via REPLY. Use "more_work_pending" only when the results must ground a later non-terminal action, such as looking up an item before updating it. Calls already queued in this batch do not count as later work. Result verification still happens for a final batch.
