...
- native toolCalls: every tool requires the reserved arg \`eliza_turn_scope\` (stripped before the tool runs). It describes the entire batch, not each call's position: use the same value on every call in one batch. Use "final" on every call when this batch covers the full request, even if it contains several calls; the evaluator still verifies their results. Use "more_work_pending" on every call only when a later batch is needed — including a list/get/search whose results are needed to ground a later write. Do not mark an earlier call pending merely because another call is already queued in this batch. A final conversational confirmation does not itself require another tool batch.
