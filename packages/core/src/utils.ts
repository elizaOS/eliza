...
/**
 * Extracts text content from an XML stream, only emitting content inside <text>...</text> tags.
 * Used for streaming LLM responses that are wrapped in XML structure.
 */
export class XmlTextStreamExtractor {
  /** Maximum buffer size before <text> tag is found (100KB) */
  private static readonly MAX_PRE_TEXT_BUFFER = 100 * 1024;

  private buffer = '';
  private insideText = false;
  private finished = false;

  /** Returns true when </text> has been found and streaming is complete */
  get done(): boolean {
  ...
  /**
   * Push a chunk into the extractor.
   * @returns The text content to stream to the client (may be empty string)
   */
  push(chunk: string): string {
    ...
    // Look for <text> opening tag
    if (!this.insideText && this.buffer.includes('<text>')) {
      this.insideText = true;
      this.buffer = this.buffer.split('<text>')[1];
    }

    // Prevent unbounded buffer growth if <text> tag never appears
    if (!this.insideText && this.buffer.length > XmlTextStreamExtractor.MAX_PRE_TEXT_BUFFER) {
      // Truncate buffer but keep last portion to detect <text> if it spans chunks
      this.buffer = this.buffer.slice(-1024);
    }

    if (!this.insideText) return '';

    // Check for </text> closing tag
    if (this.buffer.includes('</text>')) {
      const content = this.buffer.split('</text>')[0];
      this.finished = true;
      this.buffer = '';
      return content;
    }

    // Stream safe content (keep last 7 chars to handle split </text>)
    if (this.buffer.length > 7) {
      const toStream = this.buffer.slice(0, -7);
