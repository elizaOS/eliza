...
The optional `historyRetention` evaluator annotates original dialogue through the existing background memory worker. Every authorized original remains in the foreground request, including sources marked deferred in older checkpoints. Source-bound `history:hN`, `history:all` and literal `history:search:phrase` requests remain available for explicit authorized rereads. They refresh current authorization before any draft processing or effect; they cannot omit or summarize other originals. The advanced-memory feature owns registration, and its initial background review consumes separately measured model quota. Group, voice and coding requests preserve complete history.

Source annotations use the complete-context schema. A contradictory simple/none reply with pending intents receives the existing response-contract validation. A reread or invalid response cannot dispatch draft fields or effects.
