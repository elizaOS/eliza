...
Direct voice conversations retain their complete context and spoken-response path;
reviewed-history source selection remains limited to direct text.
...
Keyless web search treats an explicit Parallel zero-result envelope as an unavailable result. Complete MCP text blocks are preserved; unknown result formats are not classified as empty.
