...
import { createOllamaModelHandlers } from "@elizaos/testing";
import { detectInferenceProviders } from "@elizaos/testing";
...
const TEST_CHARACTER: Character = {
    name: "E2ETestAgent",
    system: "You are a concise, helpful assistant used for end-to-end testing. " +
...
async function importWorkspacePlugin(relativeFromHere: string, bareSpecifier: string): Promise<Record<string, unknown> | null> {
    try {
        ...
        console.warn(`[e2e] workspace import failed for ${relativeFromHere}: ${(err as Error).message}; falling back to bare specifier`);
        try {
...
async function resolveProviderPlugin(providerName: string): Promise<Plugin | null> {
    switch (providerName) {
        case "openai": {
            const mod = await importWorkspacePlugin("../../../../plugins/plugin-openai/index.ts", "@elizaos/plugin-openai");
            if (!mod)
                return null;
            return ((mod.openaiPlugin ?? mod.default) as Plugin | undefined) ?? null;
        }
        case "anthropic": {
            const mod = await importWorkspacePlugin("../../../../plugins/plugin-anthropic/index.ts", "@elizaos/plugin-anthropic");
            if (!mod)
                return null;
            return (((mod.anthropicPlugin ?? mod.default) as Plugin | undefined) ?? null);
        }
        case "groq": {
            const mod = await importWorkspacePlugin("../../../../plugins/plugin-groq/index.ts", "@elizaos/plugin-groq");
            if (!mod)
                return null;
...
function applyProviderSettings(runtime: AgentRuntime, providerName: string): void {
    switch (providerName) {
        case "openai": {
            ...
            const isCerebras = explicitElizaProvider?.toLowerCase() === "cerebras" ||
                /^csk-/i.test(openAiKey || cerebrasKey) ||
                /(^|\.)cerebras\.ai(\/|$)/i.test(explicitBase ?? "");
            runtime.setSetting("OPENAI_API_KEY", openAiKey || (isCerebras ? cerebrasKey : ""), true);
            if (cerebrasKey) {
            ...
            if (explicitBase) {
                ...
                runtime.setSetting("OPENAI_BASE_URL", "https://api.cerebras.ai/v1", true);
            ...
            if (isCerebras) {
                const cerebrasModel = process.env.OPENAI_LARGE_MODEL?.trim() ||
                    process.env.OPENAI_SMALL_MODEL?.trim() ||
                    process.env.LARGE_MODEL?.trim() ||
                    process.env.SMALL_MODEL?.trim() ||
                    DEFAULT_CEREBRAS_TEXT_MODEL;
                runtime.setSetting("OPENAI_SMALL_MODEL", process.env.OPENAI_SMALL_MODEL?.trim() || cerebrasModel);
                runtime.setSetting("OPENAI_LARGE_MODEL", process.env.OPENAI_LARGE_MODEL?.trim() || cerebrasModel);
                runtime.setSetting("SMALL_MODEL", process.env.SMALL_MODEL?.trim() ||
                    process.env.OPENAI_SMALL_MODEL?.trim() ||
                    cerebrasModel);
                runtime.setSetting("LARGE_MODEL", process.env.LARGE_MODEL?.trim() ||
...
export default async function globalSetup(): Promise<void> {
    ...
    if (provider.name === "ollama" && !providerPlugin) {
        ...
        for (const [modelType, handler] of Object.entries(handlers)) {
            if (handler) {
                runtime.registerModel(modelType, handler as (rt: AgentRuntime, p: Record<string, unknown>) => Promise<unknown>, "ollama");
            }
        }
    }
        ...
        try {
            ...
            if (req.method === "POST" && req.url === "/chat") {
                ...
                if (!runtime.messageService) {
                    res.writeHead(500);
                    res.end(JSON.stringify({ error: "messageService unavailable on runtime" }));
                    return;
                }
