...
import { encryptObjectValues, getSalt } from "./settings";
import type {
  Character,
  Content,
...
export class elizaOS extends EventTarget implements IElizaOS {
  ...
  private initFunctions: Map<UUID, (runtime: IAgentRuntime) => Promise<void>> =
    new Map();
  ...
  async addAgents(
    ...
    const promises = agents.map(async (agent) => {
      ...
      if (agent.databaseAdapter) {
        (runtime as IAgentRuntime).registerDatabaseAdapter(
          agent.databaseAdapter,
        );
      }
  ...
  async handleMessage(
    ...
    if (isAsyncMode) {
      ...
      const processing =
        await handleMessageWithEntityContext<MessageProcessingResult>(
          () => messagePromise,
        );
  ...
  async handleMessages(
    ...
    Array<{ agentId: UUID; result: HandleMessageResult; error?: Error }>
  > {
