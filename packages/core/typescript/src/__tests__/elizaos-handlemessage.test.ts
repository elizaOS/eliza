...
import { elizaOS as ElizaOS } from "../elizaos";
import type {
...
describe("ElizaOS.handleMessage", () => {
  ...
  describe("SYNC Mode", () => {
    it("should send message and wait for response", async () => {
      ...
      expect(result.processing?.responseContent?.text).toBe(
  ...
  describe("ASYNC Mode", () => {
    it("should return immediately with messageId when onResponse provided", async () => {
      const onResponseMock = mock(async (_content: Content) => {});
    ...
    it("should call onError callback if error occurs in callback", async () => {
      ...
      const onErrorMock = mock(async (_error: Error) => {});
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle metadata in message", async () => {
      ...
      expect((result.userMessage.metadata as any).customField).toBe(
        "custom value",
      );
    });
    ...
    it("should preserve content structure with attachments", async () => {
      ...
      expect(result.userMessage.content.attachments?.[0].url).toBe(
