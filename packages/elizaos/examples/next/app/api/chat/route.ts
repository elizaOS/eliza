  ...
  initPromise = (async () => {
    try {
      ...
      if (message.includes('Extension bundle not found') || message.includes('migrations')) {
        console.log('⚠️ PGLite extensions not compatible with Next.js bundling.');
        console.log('💡 Falling back to classic ELIZA mode.');
        console.log('💡 For full LLM mode, set POSTGRES_URL or run `elizaos start` separately.');
        useClassicFallback = true;
        initError = 'PGLite extensions not compatible with Next.js. Using classic mode.';
      } else {
        initError = message;
      }
      return null;
...
export async function POST(request: Request) {
  try {
    const body = await request.json() as { action?: string; message?: string; userId?: string };
        ...
        try {
          if (rt) {
            ...
            await rt.messageService?.handleMessage(
              ...
              async (content) => {
                if (content?.text) {
                  controller.enqueue(encoder.encode(`data: ${JSON.stringify({ text: content.text })}\n\n`));
                }
                return [];
              }
            );
