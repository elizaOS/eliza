...
- Package-root [`README.md`](../../README.md) and [`ROADMAP.md`](../../ROADMAP.md) for npm/registry discovery; cross-links to [`../../../typescript/docs/PROMPT_OPTIMIZATION.md`](../../../typescript/docs/PROMPT_OPTIMIZATION.md) for core vs plugin **WHY** table. In-source file-level comments on plugin `index.ts` and `disk-hooks.ts`. **Why:** Pipeline docs under `src/optimization/` are contributor-deep; operators need a short entry point without opening the monorepo tree blindly.
