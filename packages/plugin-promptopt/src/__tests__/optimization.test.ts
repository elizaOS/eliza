...
import {
	...
	flattenSchemaFieldsForAxOutputs,
	resolveNestedValue,
} from "../optimization/adapters/bridge.ts";
import {
	InstrumentationLogger,
	parseInstrumentationLevel,
} from "../optimization/adapters/instrumentation.ts";
import { readOptimizationAIConfig } from "../optimization/adapters/optimization-ai.ts";
import {
	isMergedTemplate,
...
import { ScoreCard } from "@elizaos/core";
...
describe("Phase 4 optimization AI settings and bridge", () => {
	it("readOptimizationAIConfig returns null when required keys missing", () => {
		const runtime = {
			getSetting: () => undefined,
		} as unknown as IAgentRuntime;
		expect(readOptimizationAIConfig(runtime)).toBeNull();
	});

	it("readOptimizationAIConfig returns config when student trio present", () => {
		const map: Record<string, string> = {
			OPTIMIZATION_AI_PROVIDER: "openai",
			OPTIMIZATION_AI_API_KEY: "sk-test",
			OPTIMIZATION_AI_MODEL: "gpt-4o-mini",
			OPTIMIZATION_TEACHER_MODEL: "gpt-4o",
		};
		const runtime = {
			getSetting: (k: string) => map[k],
		} as unknown as IAgentRuntime;
		expect(readOptimizationAIConfig(runtime)).toEqual({
			provider: "openai",
			apiKey: "sk-test",
			model: "gpt-4o-mini",
			teacherProvider: undefined,
			teacherApiKey: undefined,
			teacherModel: "gpt-4o",
		});
	});

	it("readOptimizationAIConfig inherits OLLAMA_* model and placeholder api key", () => {
		const map: Record<string, string> = {
			OLLAMA_SMALL_MODEL: "lfm2:24b",
		};
		const runtime = {
			getSetting: (k: string) => map[k],
		} as unknown as IAgentRuntime;
	...
	it("readOptimizationAIConfig inherits OPENAI_* when prefixed model is set", () => {
		const map: Record<string, string> = {
	...
	it("readOptimizationAIConfig fills OPENAI_API_KEY when provider+model are explicit", () => {
		const map: Record<string, string> = {
			OPTIMIZATION_AI_PROVIDER: "openai",
			OPTIMIZATION_AI_MODEL: "gpt-4o-mini",
			OPENAI_API_KEY: "sk-only-in-env",
		};
		const runtime = {
			getSetting: (k: string) => map[k],
		} as unknown as IAgentRuntime;
	...
	it("readOptimizationAIConfig OPTIMIZATION_AI_INHERIT=false requires explicit trio", () => {
		const map: Record<string, string> = {
			OPTIMIZATION_AI_INHERIT: "false",
			OPENAI_API_KEY: "sk-x",
			OPENAI_SMALL_MODEL: "gpt-4o-mini",
		};
		const runtime = {
	...
	it("parseInstrumentationLevel normalizes values", () => {
		expect(parseInstrumentationLevel(undefined)).toBe("full");
		expect(parseInstrumentationLevel("SUMMARY")).toBe("summary");
		expect(parseInstrumentationLevel("off")).toBe("off");
	});

	it("elizaMetricFn uses AxMetricFn object shape and _scoreCard", () => {
		const card = new ScoreCard();
		card.add({ source: "dpe", kind: "parseSuccess", value: 1.0 });
		const s = elizaMetricFn({
			prediction: { x: 1 },
			example: { _scoreCard: card.toJSON() },
		});
		expect(s).toBeCloseTo(1.0, 5);
	});

	it("elizaMetricFn respects signalWeights like the pipeline metricFn", () => {
		const card = new ScoreCard();
		card.add({ source: "dpe", kind: "parseSuccess", value: 0.0 });
		card.add({ source: "dpe", kind: "schemaValid", value: 1.0 });
		const ex = { _scoreCard: card.toJSON() };
		const defaultObjective = elizaMetricFn({
			prediction: {},
			example: ex,
		});
		const skewed = elizaMetricFn(
			{ prediction: {}, example: ex },
			{ "dpe:parseSuccess": 0.01, "dpe:schemaValid": 100 },
		);
		expect(defaultObjective).toBeCloseTo(0.4, 5);
		expect(skewed).toBeGreaterThan(0.95);
	});

	it("createElizaAxMetricFn returns a one-arg AxMetricFn", () => {
		const card = new ScoreCard();
		card.add({ source: "dpe", kind: "parseSuccess", value: 1.0 });
		const fn = createElizaAxMetricFn({ "dpe:parseSuccess": 10 });
		expect(fn({ prediction: {}, example: { _scoreCard: card.toJSON() } })).toBe(
			1.0,
		);
	});

	it("extractGEPAInstructions prefers optimizedProgram instruction", () => {
		const text = extractGEPAInstructions({
			paretoFront: [],
			bestScore: 0,
			optimizedProgram: { instruction: "  Be brief.  " },
		});
		expect(text).toBe("Be brief.");
	});

	it("extractGEPAInstructions falls back to Pareto configuration strings", () => {
		const text = extractGEPAInstructions({
			paretoFront: [
				{
	...
	it("extractACEPlaybook renders markdown sections", () => {
		const md = extractACEPlaybook({
			version: 1,
			description: "Rules",
			sections: {
				general: [{ id: "b1", content: "Check facts" }],
			},
			stats: {
				bulletCount: 1,
				helpfulCount: 0,
		...
		expect(md).toContain("Check facts");
	});

	it("flattenSchemaFieldsForAxOutputs skips object parents", () => {
		const rows = [
			{
				field: "outer",
				description: "Outer",
				...
				properties: [
					{ field: "inner", description: "Inner", type: "string" as const },
				],
			},
		];
		const leaves = flattenSchemaFieldsForAxOutputs(rows);
		expect(leaves.map((l) => l.path)).toEqual(["outer.inner"]);
	});

	it("resolveNestedValue reads dotted and array paths", () => {
		const obj = {
			a: { b: [{ c: 1 }, { c: 2 }] },
		} as Record<string, unknown>;
		expect(resolveNestedValue(obj, "a.b[].c")).toEqual([1, 2]);
	});

	it("buildAxTypedExamples aligns flat paths to trace.response", () => {
		const traces = [
			makeTrace({
				response: { reply: { text: "hello" } },
			}),
		];
		const schema = [
			{
				...
				type: "object" as const,
				properties: [
					{ field: "text", description: "T", type: "string" as const },
				],
			},
		];
		const ex = buildAxTypedExamples(traces, schema);
		expect(ex[0]?.["reply.text"]).toBe("hello");
	});

	it("InstrumentationLogger appends JSONL when level is not off", async () => {
		const dir = await mkdtemp(join(tmpdir(), "instr-"));
		const log = new InstrumentationLogger(dir, "minimal");
		await log.log({ test: true });
		const content = await readFile(join(dir, "instrumentation.jsonl"), "utf-8");
		const row = JSON.parse(content.trim()) as { test: boolean; level: string };
		expect(row.test).toBe(true);
		expect(row.level).toBe("minimal");
		await rm(dir, { recursive: true, force: true });
	});

	it("buildAxProgram returns AxGen when @ax-llm/ax is installed", async () => {
		...
		const program = buildAxProgram(ax, schema, "Task description");
		expect(typeof program.getSignature).toBe("function");
	});
});
