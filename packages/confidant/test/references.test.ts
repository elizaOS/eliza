...
describe("references", () => {
  it("parses every supported scheme", () => {
    ...
    expect(
      parseReference("keyring://elizaos/llm.openrouter.apiKey").source,
    ).toBe("keyring");
    expect(parseReference("op://Personal/OpenRouter/api-key").source).toBe(
      "1password",
    );
    expect(parseReference("pass://default/openrouter").source).toBe(
      "protonpass",
    );
    expect(parseReference("env://OPENROUTER_API_KEY").source).toBe(
      "env-legacy",
    );
  ...
  it("rejects unknown schemes", () => {
    expect(() => parseReference("unknown://value")).toThrow(
      InvalidReferenceError,
    );
  });

  it("rejects missing scheme", () => {
    expect(() => parseReference("just-a-string")).toThrow(
      InvalidReferenceError,
    );
