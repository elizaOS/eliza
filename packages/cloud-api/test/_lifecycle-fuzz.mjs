...
async function mintKey() {
  let nonce = null;
  for (let i = 0; i < 8; i++) {
    const r = await j("GET", "/api/auth/siwe/nonce?chainId=1", { noAuth: true });
    if (r.s === 200 && r.d?.nonce) { nonce = r.d; break; }
    await sleep(2500);
  }
  ...
  const v = await j("POST", "/api/auth/siwe/verify", { noAuth: true, body: { message: msg, signature: sig } });
...
async function createAgent(label) {
  const c = await j("POST", "/api/v1/eliza/agents", { body: { agentName: label, alwaysOn: true } });
...
async function waitReady(id, budgetMs = 5 * 60 * 1000) {
  ...
  while (Date.now() - t0 < budgetMs) {
    let st = 0;
    try { st = (await j("GET", "/api/status", { base: pub })).s; } catch {}
    if (st === 200) return { ready: true, sec: Math.round((Date.now() - t0) / 1000) };
    await sleep(8000);
  }
...
async function talk(id, text) {
  ...
  const chat = await j("POST", `/api/conversations/${cid}/messages`, { base: pub, body: { text, channelType: "DM" } });
...
async function waitGone(id, budgetMs = 3 * 60 * 1000) {
  ...
  while (Date.now() - t0 < budgetMs) {
    ...
    if (r.s === 404) return { gone: true, sec: Math.round((Date.now() - t0) / 1000) };
    await sleep(6000);
  }
...
async function listAgentIds() {
  ...
  return (Array.isArray(arr) ? arr : []).map((a) => a.id || a.agentId).filter(Boolean);
}
...
async function main() {
  ...
  const ra = await waitReady(A.id); check("A ready", ra.ready, `${ra.sec}s`);
  // 2. TALK A
  const ta = await talk(A.id, "Say: A-one");
  check("talk A", ta.status === 200 && ta.reply.length > 0, `status=${ta.status} reply="${ta.reply.slice(0, 40)}"`);
  // 3. DELETE A
  const da = await del(A.id); check("delete A accepted", da.status >= 200 && da.status < 300, `status=${da.status}`);
  const ga = await waitGone(A.id); check("A gone (404)", ga.gone, `${ga.sec}s`);
  // INVARIANT: talking to deleted A fails
  const tadead = await talk(A.id, "still there?");
  check("talk deleted A fails (no zombie)", tadead.status >= 400, `status=${tadead.status}`);

  // 4 & 5. CREATE B, CREATE C (concurrent)
  const [B, C] = await Promise.all([createAgent("fuzz-B"), createAgent("fuzz-C")]);
  ...
  check("B ready", rb.ready, `${rb.sec}s`); check("C ready", rc.ready, `${rc.sec}s`);

  // 6. SWITCH (to B) + talk B — switching = choosing which agent to address
  const tb = await talk(B.id, "Say: B-switched");
  check("switch→talk B", tb.status === 200 && tb.reply.length > 0, `status=${tb.status} reply="${tb.reply.slice(0, 40)}"`);
  // INVARIANT: both B and C present in the list (switch didn't drop one)
  const midList = await listAgentIds();
  check("B & C both listed", midList.includes(B.id) && midList.includes(C.id), `count=${midList.length}`);

  // 7. DELETE B
  const db = await del(B.id); check("delete B accepted", db.status >= 200 && db.status < 300, `status=${db.status}`);
  const gb = await waitGone(B.id); check("B gone (404)", gb.gone, `${gb.sec}s`);

  // 8. TALK C (after B deleted — C must be unaffected)
  const tc = await talk(C.id, "Say: C-survives");
  check("talk C after B deleted", tc.status === 200 && tc.reply.length > 0, `status=${tc.status} reply="${tc.reply.slice(0, 40)}"`);

  // 9. DELETE C + idempotent re-delete
  const dc = await del(C.id); check("delete C accepted", dc.status >= 200 && dc.status < 300, `status=${dc.status}`);
  const gc = await waitGone(C.id); check("C gone (404)", gc.gone, `${gc.sec}s`);
  const dc2 = await del(C.id);
  check("re-delete C is safe (404/2xx, no 500)", dc2.status === 404 || (dc2.status >= 200 && dc2.status < 300), `status=${dc2.status}`);

  // FINAL: no fuzz agents leaked
  const finalList = await listAgentIds();
  const leaked = [A.id, B.id, C.id].filter((id) => finalList.includes(id));
  check("no agents leaked", leaked.length === 0, leaked.length ? `leaked=${leaked.join(",")}` : "clean");
