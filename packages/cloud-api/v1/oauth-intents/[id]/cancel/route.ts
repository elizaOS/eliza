...
app.post("/", async (c) => {
  try {
    ...
    const oauthIntent = await service.cancel(id, user.organization_id, parsed.data.reason);

    return c.json({ success: true, oauthIntent: redactOAuthIntentForPublic(oauthIntent) });
  } catch (error) {
