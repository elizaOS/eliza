 ...
 */
export function createX402Middleware(routeConfig: Record<string, { price?: string; description?: string; inputSchema?: Record<string, unknown>; outputSchema?: Record<string, unknown> }>): (req: Request, res: Response, next: NextFunction) => void {
    ...
    if (!config.enabled) {
        return (req: Request, res: Response, next: NextFunction) => {
            ...
            if (!apiKey || apiKey !== serverAuthToken) {
                logger.warn(`[x402] Unauthorized access attempt: Missing or invalid X-API-KEY from ${req.ip}`);
    ...
    if (config.useMainnet) {
        if (!process.env.CDP_API_KEY_ID || !process.env.CDP_API_KEY_SECRET) {
            logger.error('[x402] X402_USE_MAINNET is true but CDP_API_KEY_ID or CDP_API_KEY_SECRET are not set');
            throw new Error('Mainnet facilitator requires CDP_API_KEY_ID and CDP_API_KEY_SECRET environment variables');
        }
    }
    ...
    const enhancedRouteConfig: Record<string, { price: string; network: Network; config?: Record<string, unknown> }> = {};
