...
```bash
bun run --cwd packages/app/deploy/cloud-agent-template start
```

This command expects the surrounding app deployment layout because the entrypoint imports `../cloud-agent-shared.ts`.

## Change rules

- Put shared cloud-agent behavior in `packages/app/deploy/cloud-agent-shared.ts`.
