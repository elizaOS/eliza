...
const logger = {
    info(message: string, context?: Record<string, unknown>): void {
        if (context)
            console.log(`[cloud-agent] ${message}`, context);
        else
            console.log(`[cloud-agent] ${message}`);
    },
    warn(message: string, context?: Record<string, unknown>): void {
        if (context)
            console.warn(`[cloud-agent] ${message}`, context);
        else
            console.warn(`[cloud-agent] ${message}`);
    },
    error(message: string, context?: Record<string, unknown>): void {
        if (context)
            console.error(`[cloud-agent] ${message}`, context);
        else
            console.error(`[cloud-agent] ${message}`);
    },
};
...
export function warnGeneratedBridgeSecret(): void {
    logger.warn("CRITICAL: No BRIDGE_SECRET configured — generated ephemeral secret and bound to 127.0.0.1 only");
}
/**
...
export function publicDatabaseLiveness(payload: DatabaseLivenessPayload): Omit<DatabaseLivenessPayload, "message"> {
    return {
        ok: payload.ok,
...
function readProbeDiagnosticProperty(value: unknown, property: PropertyKey): unknown {
    if (value === null ||
...
function describeDatabaseProbeError(error: unknown): string {
    ...
    let text = typeof message === "string" && message.trim() ? message : undefined;
    if (text === undefined) {
        try {
            text = String(error);
        }
...
function isTerminalDatabaseProbeError(error: unknown): boolean {
    ...
    while (current && !seen.has(current)) {
        ...
        const message = typeof diagnosticMessage === "string"
            ? diagnosticMessage
            : typeof current === "string"
                ? current
                : "";
        if (TERMINAL_DATABASE_LIVENESS_PATTERNS.some((pattern) => pattern.test(message))) {
            return true;
        }
...
async function probeDatabaseHandle(handle: unknown): Promise<void> {
    if (handle && typeof handle === "object") {
        ...
        if (typeof executable.execute === "function") {
            await (executable.execute as (query: unknown) => Promise<unknown>)(sql `SELECT 1`);
            return;
        }
    }
    throw new Error("database connection does not expose query or execute");
}
export async function checkRuntimeDatabaseLiveness(runtime: RuntimeWithDatabaseLiveness | null): Promise<DatabaseLivenessPayload> {
    if (!runtime)
    ...
    try {
        if (typeof runtime.checkDatabaseLiveness === "function") {
            ...
            if (!DATABASE_LIVENESS_STATUSES.has(result.status) ||
...
export function appendBridgeCallbackContent(result: BridgeMessageResult, content: BridgeCallbackContent): BridgeMessageResult {
...
export function normalizeBridgeMessage(params?: BridgeRpcParams): NormalizedBridgeMessage {
    const trimmedRoomId = typeof params?.roomId === "string" && params.roomId.trim().length > 0
        ? params.roomId.trim()
        : "default";
    const source = typeof params?.source === "string" && params.source.trim().length > 0
        ? params.source.trim()
        : "cloud-bridge";
    const sender = params?.sender && typeof params.sender === "object"
        ? {
...
interface AgentRuntime {
    ...
    processMessageStream: (params: BridgeRpcParams, onChunk: (chunk: string) => void) => Promise<BridgeMessageResult>;
...
export function startCloudAgent(userConfig: CloudAgentConfig = {}): void {
    ...
    const BRIDGE_PORT = userConfig.bridgePort ?? Number(process.env.BRIDGE_PORT ?? "18790");
    ...
    function readBody(req: http.IncomingMessage): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            ...
            const invalidEncoding = (cause: unknown) => {
                failed = true;
                reject(new ElizaError("Request body must contain complete valid UTF-8", {
    ...
    async function initRuntime(): Promise<void> {
        const elizaAvailable = await import("@elizaos/core")
            ...
            .catch(() => false);
        if (elizaAvailable) {
            const { AgentRuntime: AgentRuntimeCtor, createCharacter, createMessageMemory, stringToUuid, ChannelType, } = await import("@elizaos/core");
            ...
            const runtimeWithBridge = runtime as typeof runtime & {
                ...
                ensureParticipantInRoom?: (entityId: ReturnType<typeof stringToUuid>, roomId: ReturnType<typeof stringToUuid>) => Promise<void>;
                getEntityById?: (entityId: ReturnType<typeof stringToUuid>) => Promise<Record<string, unknown> | null>;
            ...
            const ensureBridgeContext = async (params: BridgeRpcParams) => {
                ...
                const worldId = stringToUuid(`${normalized.source}-${normalized.channelType.toLowerCase()}-world`);
                const serverId = stringToUuid(`${normalized.source}-bridge-server`);
                const roomId = stringToUuid(`${normalized.source}-bridge-room-${normalized.roomKey}`);
                const entityKey = normalized.sender?.id ??
                    normalized.sender?.username ??
                    `${normalized.source}-bridge-user`;
                const entityId = stringToUuid(`${normalized.source}-bridge-user-${entityKey}`);
                const channelType = normalized.channelType === "GROUP"
                    ? ChannelType.GROUP
                    : ChannelType.DM;
                const displayName = normalized.sender?.displayName ??
                    normalized.sender?.username ??
                    "BridgeUser";
                ...
                const entityMetadata = normalized.sender?.metadata &&
                ...
                const entityPayload = {
                    id: entityId,
                    agentId: runtime.agentId,
                    names: Array.from(new Set([displayName, normalized.sender?.username].filter((value): value is string => Boolean(value)))),
                    ...(entityMetadata ? { metadata: entityMetadata } : {}),
                };
                try {
                    if (typeof runtimeWithBridge.getEntityById === "function" &&
                        typeof runtimeWithBridge.updateEntity === "function") {
                        const existingEntity = await runtimeWithBridge.getEntityById(entityId);
                        if (existingEntity) {
                            await runtimeWithBridge.updateEntity({
                                ...existingEntity,
                                ...entityPayload,
                                names: entityPayload.names.length > 0
                                    ? entityPayload.names
            ...
            agentRuntime = {
                processMessage: async (params: BridgeRpcParams): Promise<BridgeMessageResult> => {
                    const { normalized, entityId, roomId, channelType } = await ensureBridgeContext(params);
                    ...
                    await runtime.messageService?.handleMessage(runtime, message, async (content) => {
                        appendBridgeCallbackContent(response, content as BridgeCallbackContent);
                        return [];
                    });
                ...
                processMessageStream: async (params: BridgeRpcParams, onChunk: (chunk: string) => void): Promise<BridgeMessageResult> => {
                    const { normalized, entityId, roomId, channelType } = await ensureBridgeContext(params);
                    const message = createMessageMemory({
                        id: crypto.randomUUID() as ReturnType<typeof stringToUuid>,
                        entityId,
    ...
    const healthServer = http.createServer(async (req, res) => {
        if (req.method === "GET" &&
            ...
            const lastActivityAge = Date.now() - new Date(state.lastActivityAt).getTime();
            const possiblyHung = runtimeReady &&
    ...
    // ─── Bridge HTTP server ───────────────────────────────────────────────
    const handleBridgeRequest = async (req: http.IncomingMessage, res: http.ServerResponse) => {
        ...
        const importMatch = req.method === "POST"
        ...
        if (importMatch) {
            ...
            const tasks = Array.isArray(body.scheduledTasks)
                ? body.scheduledTasks
                : [];
            const todo = body.todoSnapshot && typeof body.todoSnapshot === "object"
                ? (body.todoSnapshot as Record<string, unknown>)
                : null;
            const todos = todo && Array.isArray(todo.todos) ? todo.todos : [];
            const mutations = todo && Array.isArray(todo.mutations) ? todo.mutations : [];
            const digest = todo && typeof todo.digest === "string" ? todo.digest : null;
            if (!messages ||
                tasks.length > 0 ||
                todos.length > 0 ||
                mutations.length > 0 ||
            ...
            const existingSourceIds = new Set(state.memories
            ...
            for (const value of messages) {
                ...
                const sourceId = typeof message.sourceId === "string" ? message.sourceId.trim() : "";
                if (!role || !text || !sourceId)
                    continue;
        ...
        if (req.method === "POST" && req.url === "/api/restore") {
            ...
            if (incoming.memories)
                state.memories = incoming.memories;
            if (incoming.config)
                state.config = incoming.config;
        ...
        if (req.method === "POST" && req.url === "/bridge/stream") {
            ...
            const response = await agentRuntime.processMessageStream(rpc.params ?? {}, (chunk: string) => {
    ...
    const bridgeServer = http.createServer(async (req, res) => {
        try {
            ...
            const invalidUtf8 = error instanceof ElizaError &&
                error.code === "CLOUD_AGENT_INVALID_UTF8";
            if (!invalidUtf8)
                logger.error("Bridge request failed", {
                    errorName: error instanceof ElizaError
                        ? "ElizaError"
                        : error instanceof TypeError
                            ...
                            : error instanceof Error
                                ? "Error"
                                : "NonError",
                    code: error instanceof ElizaError &&
            ...
            res.end(JSON.stringify(invalidUtf8
                ? {
