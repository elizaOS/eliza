...
cd deploy
bash ../eliza/packages/app/deploy/docker-setup.sh

# Load the current image onto configured nodes.
cd deploy
bash ../eliza/packages/app/deploy/deploy-to-nodes.sh --status
```
