...
`packages/app/scripts/package-electrobun-flatpak.mjs`. The resulting
...
```bash
bun run --cwd packages/app build:flatpak:direct
```
...
```bash
bun run --cwd packages/app verify:linux-distribution
```
...
```bash
node packages/app/scripts/package-electrobun-linux.mjs \
  --build-dir=packages/app/platforms/electrobun/build/dev-linux-x64/Eliza-dev
node packages/app/scripts/package-electrobun-flatpak.mjs \
  --build-dir=packages/app/platforms/electrobun/build/dev-linux-x64/Eliza-dev
```
...
```bash
bundle=packages/app/platforms/electrobun/artifacts/Eliza-2.0.3-beta.7-linux-x86_64.flatpak
...
`bun run --cwd packages/app build:flatpak:store` deliberately fails
