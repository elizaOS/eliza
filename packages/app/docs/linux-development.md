...
```bash
bash packages/scripts/bootstrap-linux-dev.sh
```
...
```bash
bash packages/scripts/bootstrap-linux-dev.sh --doctor-only
bun packages/scripts/linux-dev-doctor.mjs --json
bash packages/scripts/bootstrap-linux-dev.sh --skip-install
```

The bootstrap's repository-local PATH exists only for that command. Running
`bun packages/scripts/linux-dev-doctor.mjs` directly is still useful when intentionally auditing the
...
`packages/app/platforms/electrobun/build/dev-linux-<arch>/Eliza-dev/`.
...
```bash
bun run --cwd packages/app/platforms/electrobun build -- --build-fused-lib
```
...
- Desktop API/UI ports are dynamically allocated by the app platform
