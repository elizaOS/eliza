...
import { isMobileLocalAgentIpcUrl } from "@elizaos/ui/first-run/mobile-runtime-mode";
...
export function isTrustedCloudOnlyApiBaseUrl(parsed: URL, cloudOnly: boolean): boolean {
    return (cloudOnly &&
...
export function createUrlTrustPolicy(ctx: UrlTrustPolicyContext) {
    ...
    function isTrustedApiBaseUrl(parsed: URL): boolean {
        ...
        if (ctx.isPopoutWindow && parsed.protocol === "https:")
            return true;
