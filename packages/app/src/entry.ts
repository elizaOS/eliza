...
import { applyCliProfileEnv } from "./cli/profile";
...
import { parseCliProfileArgs } from "./cli/profile";
...
if (promoteLauncherScopedDevCloudApiKey(process.env)) {
    ...
    process.stderr.write("[entry] launcher-scoped Cloud development credential promoted to ELIZAOS_CLOUD_API_KEY\n");
}
...
if (process.env.DATABASE_URL && !process.env.POSTGRES_URL) {
    ...
    // Stderr only — logger isn't initialized yet at this point in boot.
    process.stderr.write("[entry] DATABASE_URL detected: bridged to POSTGRES_URL for plugin-sql\n");
}
...
import("./cli/run-main")
    ...
    .catch((error) => {
    console.error(`${getLogPrefix()} Failed to start CLI:`, formatErrorWithStack(error));
    process.exit(1);
});
