 ...
 */
import { AGENT_READY_EVENT } from "@elizaos/ui/events";
import { APP_BRANDING_BASE } from "./app-config";
import { APP_CHARACTER_CATALOG } from "./character-catalog";
import { APP_CONFIG } from "./app-config";
import { APP_ENV_ALIASES } from "./brand-env";
import { APP_ENV_PREFIX } from "./brand-env";
import { APP_LOG_PREFIX } from "./app-config";
import { APP_NAMESPACE } from "./app-config";
import { APP_URL_SCHEME } from "./app-config";
...
import { COMMAND_PALETTE_EVENT } from "@elizaos/ui/events";
import { Capacitor } from "@capacitor/core";
import { ELIZA_DEFAULT_THEME } from "@elizaos/ui/themes";
import { ErrorBoundary } from "@elizaos/ui";
import { FIRST_RUN_CLOUD_LOGIN_ACTION } from "@elizaos/ui/first-run/first-run-action-channel";
import { IOS_LOCAL_AGENT_IPC_BASE } from "@elizaos/ui/first-run/mobile-runtime-mode";
import { MOBILE_LOCAL_AGENT_API_BASE } from "@elizaos/ui/first-run/mobile-runtime-mode";
import { MOBILE_RUNTIME_MODE_CHANGED_EVENT } from "@elizaos/ui/events";
import { MOBILE_RUNTIME_MODE_STORAGE_KEY } from "@elizaos/ui/first-run/mobile-runtime-mode";
import { PHONE_COMPANION_AGENT_VIEW_ID } from "./renderer-shell-scope";
import { PUSH_TO_TALK_HOLD_EVENT } from "@elizaos/ui/events";
import { PUSH_TO_TALK_TOGGLE_EVENT } from "@elizaos/ui/events";
import { Preferences } from "@capacitor/preferences";
import { RenderTelemetryProfiler } from "@elizaos/ui/cloud-ui/runtime/render-telemetry";
import { SHARE_TARGET_EVENT } from "@elizaos/ui/events";
...
import { apiBaseToDeviceBridgeUrl } from "./ios-runtime";
...
import { applyRuntimeChooserOverrideFromUrl } from "./runtime-chooser-override";
import { applyUiTheme } from "@elizaos/ui/state/persistence";
import { assertSupportedIosRuntimeConfig } from "./ios-runtime";
import { buildAssistantLaunchHashRoute } from "./deep-link-routing";
import { cachedDynamicImport } from "./app-module-cache";
import { clearStandaloneBottomReclaim } from "@elizaos/ui/platform/standalone-bottom-reclaim";
...
import { createPersistedActiveServer } from "@elizaos/ui/state/persistence";
import { createTranslator } from "@elizaos/ui/i18n";
import { decideChatOverlayToggle } from "./desktop-hotkey";
import { dedicatedCloudAgentIdFromBase } from "@elizaos/ui/utils/cloud-agent-base";
import { dispatchAppEvent } from "@elizaos/ui/events";
import { dispatchConnectRequest } from "@elizaos/ui/events";
import { dispatchNavigateViewRequest } from "@elizaos/ui/events";
import { dispatchOpenNotificationCenter } from "@elizaos/ui/events";
import { dispatchRemoteControllerPairingIntent } from "@elizaos/ui/platform/remote-target-pairing-intent";
import { extractIosLivenessChallengeToken } from "./ios-cloud-onboarding-smoke";
import { getBootConfig } from "@elizaos/ui/config";
import { getChatOverlayHotkey } from "@elizaos/ui/state/useChatOverlayHotkey";
import { getMobileRemoteFallbackApiBase } from "./mobile-remote-fallback";
...
import { getWindowNavigationPath } from "@elizaos/ui/navigation";
...
import { initStartupTrace } from "@elizaos/ui/state/startup-telemetry";
...
import { installIosLocalAgentFetchBridge } from "@elizaos/app/api/ios-local-agent-transport";
import { installIosLocalAgentNativeRequestBridge } from "@elizaos/app/api/ios-local-agent-transport";
...
import { installMobileRemoteFallback } from "./mobile-remote-fallback";
...
import { installStandaloneBottomReclaim } from "@elizaos/ui/platform/standalone-bottom-reclaim";
import { invokeDesktopBridgeRequest } from "@elizaos/ui/bridge";
import { isAndroidCloudBuild } from "@elizaos/ui/platform/android-runtime";
import { isAppWindowRoute } from "@elizaos/ui/navigation";
import { isChatOverlayWindowShell } from "@elizaos/ui/platform/window-shell";
...
import { isDedicatedCloudAgentBase } from "@elizaos/ui/utils/cloud-agent-base";
import { isDetachedWindowShell } from "@elizaos/ui/platform/window-shell";
import { isDeveloperWorkspaceRoute } from "@elizaos/ui/navigation";
...
import { isEmbedPath } from "./embed-bootstrap";
import { isIosCloudOnboardingComplete } from "./ios-cloud-onboarding-smoke";
import { isIosLivenessReplyRow as isIosLivenessReplyRowFromContract } from "./ios-cloud-onboarding-smoke";
...
import { isStandaloneWindowShell } from "@elizaos/ui/platform/window-shell";
import { isTrustedAppLink } from "./deep-link-handler";
import { isTrustedCloudOnlyApiBaseUrl } from "./url-trust-policy";
import { lazy } from "react";
import { loadPersistedActiveServer } from "@elizaos/ui/state/persistence";
import { loadUiLanguage } from "@elizaos/ui/state/persistence";
import { loadUiThemeMode } from "@elizaos/ui/state/persistence";
import { logger } from "@elizaos/core/logger";
import { markStartup } from "@elizaos/ui/state/startup-telemetry";
import { measureStartup } from "@elizaos/ui/state/startup-telemetry";
import { normalizeMobileRuntimeMode } from "@elizaos/ui/first-run/mobile-runtime-mode";
import { parseFirstRunRemoteConnectDeepLink } from "@elizaos/ui/first-run/deep-link-handler";
import { parseIosCloudOnboardingSmokeRequest as parseIosCloudOnboardingSmokeRequestFromContract } from "./ios-cloud-onboarding-smoke";
import { parseRemoteControllerPairingDeepLink } from "@elizaos/ui/platform/remote-target-pairing-intent";
...
import { removeUrlParameter } from "./runtime-chooser-override";
...
import { resolveUiTheme } from "@elizaos/ui/state/persistence";
import { resolveWindowShellRoute } from "@elizaos/ui/platform/window-shell";
...
import { savePersistedActiveServer } from "@elizaos/ui/state/persistence";
import { setBootConfig } from "@elizaos/ui/config";
...
import { shouldInstallMainWindowFirstRunPatches } from "@elizaos/ui/platform/window-shell";
import { shouldInstallStandaloneBottomReclaim } from "@elizaos/ui/platform/standalone-bottom-reclaim";
...
import { subscribeDesktopBridgeEvent } from "@elizaos/ui/bridge";
import { syncDetachedShellLocation } from "@elizaos/ui/platform/window-shell";
import { tryHandleFirstRunAction } from "@elizaos/ui/first-run/first-run-action-channel";
import { type AndroidDeepLinkBuffer } from "./mobile-lifecycle";
import { type AppBlockerSettingsCardProps } from "@elizaos/core/contracts/personal-assistant";
import { type AppBootConfig } from "@elizaos/ui/config";
import { type BrandingConfig } from "@elizaos/ui/config";
import { type CodingAgentTasksPanelProps } from "@elizaos/ui/config";
import { type ComponentType } from "react";
import { type DeepLinkNavigationIntent } from "./deep-link-routing";
...
import { type IosCloudOnboardingSmokeRequest } from "./ios-cloud-onboarding-smoke";
import { type IosRuntimeConfig } from "./ios-runtime";
import { type MobileLifecycle } from "./mobile-lifecycle";
import { type PushToTalkHoldDetail } from "@elizaos/ui/events";
import { type ReactNode } from "react";
import { type ShareTargetPayload } from "@elizaos/ui/platform";
import { type SideEffectAppModuleLoader } from "./plugin-registrations";
...
function importPersonalAssistant() {
    return cachedDynamicImport("@elizaos/plugin-personal-assistant", () => import("@elizaos/plugin-personal-assistant"));
}
function importAppPhone() {
    return cachedDynamicImport("@elizaos/plugin-native-phone", () => import("@elizaos/plugin-native-phone"));
}
function importAppTaskCoordinator() {
    return cachedDynamicImport("@elizaos/plugin-agent-orchestrator", () => import("@elizaos/plugin-agent-orchestrator/ui"));
}
function importAppTaskCoordinatorRegister() {
    return cachedDynamicImport("@elizaos/plugin-agent-orchestrator/ui/register", () => import("@elizaos/plugin-agent-orchestrator/ui/register"));
}
function lazyNamedComponent<TProps>(load: () => Promise<ComponentType<TProps>>): ComponentType<TProps> {
...
const AppWindowRenderer = lazyNamedComponent<{
    slug: string;
}>(async () => {
...
const ShellViewAgentSurface = lazyNamedComponent<{
    ...
    const mod = await import("@elizaos/ui/components/views/ShellViewAgentSurface");
    return mod.ShellViewAgentSurface;
});
/** Desktop-only shell widgets — never static-import into the login entry. */
const DesktopSurfaceNavigationRuntime = lazyNamedComponent<Record<string, never>>(async () => {
    const mod = await import("@elizaos/app/desktop-shell");
    return mod.DesktopSurfaceNavigationRuntime;
});
const DesktopTrayRuntime = lazyNamedComponent<Record<string, never>>(async () => {
    const mod = await import("@elizaos/app/desktop-shell");
    return mod.DesktopTrayRuntime;
});
const DetachedShellRoot = lazyNamedComponent<DetachedShellRootProps>(async () => {
    const mod = await import("@elizaos/app/desktop-shell");
    return mod.DetachedShellRoot;
});
const PhoneCompanionApp = lazyNamedComponent<Record<string, never>>(async () => (await importAppPhone()).PhoneCompanionApp);
async function runIosFullBunSmokeFromDesktopShell(): Promise<boolean> {
    const mod = await import("@elizaos/app/desktop-shell");
    return mod.runIosFullBunSmokeIfRequested();
}
async function buildLocalizedTrayMenuAsync(...args: Parameters<typeof import("@elizaos/app/desktop-shell").buildLocalizedTrayMenu>) {
    const mod = await import("@elizaos/app/desktop-shell");
    return mod.buildLocalizedTrayMenu(...args);
}
const AppBlockerSettingsCard = lazyNamedComponent<AppBlockerSettingsCardProps>(async () => (await importPersonalAssistant()).AppBlockerSettingsCard);
const WebsiteBlockerSettingsCard = lazyNamedComponent<WebsiteBlockerSettingsCardProps>(async () => (await importPersonalAssistant()).WebsiteBlockerSettingsCard);
const CodingAgentControlChip = lazyNamedComponent<Record<string, never>>(async () => (await importAppTaskCoordinator()).CodingAgentControlChip);
const CodingAgentSettingsSection = lazyNamedComponent<Record<string, never>>(async () => (await importAppTaskCoordinator()).CodingAgentSettingsSection);
const CodingAgentTasksPanel = lazyNamedComponent<CodingAgentTasksPanelProps>(async () => (await importAppTaskCoordinator()).CodingAgentTasksPanel);
...
function getLegacyInjectedAppApiBase(): string | undefined {
    const brandedApiBase: unknown = Reflect.get(window, BRANDED_WINDOW_KEYS.apiBase);
    return (window.__ELIZA_APP_API_BASE__ ??
...
function getInjectedDesktopRuntimeMode(): string | undefined {
    if (typeof window !== "undefined") {
        const injected: unknown = Reflect.get(window, "__ELIZA_DESKTOP_RUNTIME_MODE__");
...
const APP_BRANDING: Partial<BrandingConfig> = {
    ...
    cloudOnly: resolveAppCloudOnlyBranding({
        ...
        bootApiBase: getBootConfig().apiBase ?? getMobileRemoteFallbackApiBase() ?? undefined,
        legacyInjectedApiBase: typeof window === "undefined" ? undefined : getLegacyInjectedAppApiBase(),
        isNativePlatform: Capacitor.isNativePlatform(),
        nativeRuntimeMode: isAndroidCloudBuild() && !getMobileRemoteFallbackApiBase()
            ? "cloud"
            : undefined,
...
const isStoreBuild = typeof __ELIZA_BUILD_VARIANT__ === "string" &&
...
const BACKGROUND_RUNNER_CONFIG_RETRY_MS = 5000;
...
const IOS_CLOUD_ONBOARDING_SMOKE_REQUEST_KEY = "eliza:ios-cloud-onboarding-smoke:request";
const IOS_CLOUD_ONBOARDING_SMOKE_RESULT_KEY = "eliza:ios-cloud-onboarding-smoke:result";
...
const IOS_ONBOARDING_RELAUNCH_SMOKE_REQUEST_KEY = "eliza:ios-onboarding-relaunch-smoke:request";
const IOS_ONBOARDING_RELAUNCH_SMOKE_RESULT_KEY = "eliza:ios-onboarding-relaunch-smoke:result";
const IOS_MIXED_CONTENT_SMOKE_REQUEST_KEY = "eliza:ios-mixed-content-smoke:request";
const IOS_MIXED_CONTENT_SMOKE_RESULT_KEY = "eliza:ios-mixed-content-smoke:result";
const IOS_ONBOARDING_SMOKE_TIMEOUT_MS = 120000;
...
function applyCloudPairSessionToken(): void {
    ...
    if (isDedicatedCloudAgentBase(currentOrigin)) {
        ...
        if (configuredBase && isDedicatedCloudAgentBase(configuredBase)) {
            ...
            if (!isCloudPairAgentId(ownerHint))
                return;
            apiBase = currentOrigin;
            agentId = ownerHint;
    ...
    if (!token) {
        ...
        if (legacyToken) {
            try {
                ...
                const ownedByTarget = activeServer !== null &&
    ...
    if (usedLocalOwnerHint) {
        ...
        const sessionDurable = persisted !== null &&
...
function shouldEnableElectrobunMacWindowDrag(): boolean {
    if (!isElectrobunRuntime() || typeof document === "undefined")
        return false;
...
if (shouldEnableElectrobunMacWindowDrag()) {
    document.documentElement.classList.add("eliza-electrobun-frameless", "eliza-electrobun-macos-titlebar");
}
...
function importSideEffectAppModule(key: string, loader: () => Promise<unknown>) {
...
function scheduleAppModuleIdleWork(work: () => void): void {
    ...
    if (typeof w.requestIdleCallback === "function") {
        w.requestIdleCallback(work, { timeout: 3000 });
        return;
    }
...
function scheduleAppModuleIdleLoads(loaders: readonly SideEffectAppModuleLoader[]): void {
    if (loaders.length === 0)
        return;
    let nextIndex = 0;
    let activeCount = 0;
    const pump = () => {
        while (activeCount < SIDE_EFFECT_APP_MODULE_LOAD_CONCURRENCY &&
...
function installRendererServiceHost(): void {
    ...
    startRendererServiceHost({
        ...
        reportError: (serviceId, error, phase) => {
            console.error(`${APP_LOG_PREFIX} renderer service "${serviceId}" ${phase} failed:`, error);
        },
    });
}
function scheduleDeferredAppModuleLoadsAfterPaint(): void {
    if (deferredAppModuleLoadsScheduled)
        return;
...
function buildAppBootConfig(): AppBootConfig {
    ...
    return {
        ...
        assetBaseUrl: (import.meta.env.VITE_ASSET_BASE_URL as string | undefined)?.trim() ||
            undefined,
        ...
        clientMiddleware: {
            forceFreshFirstRun: shouldInstallMainWindowFirstRunPatches(windowShellRoute),
...
function getShareQueue(): ShareTargetPayload[] {
    const brandedQueue: unknown = Reflect.get(window, BRANDED_WINDOW_KEYS.shareQueue);
    const existing = window.__ELIZA_APP_SHARE_QUEUE__ ??
...
function logNativePluginUnavailable(pluginName: string, error: unknown): void {
    console.warn(`${APP_LOG_PREFIX} ${pluginName} plugin not available:`, error instanceof Error ? error.message : error);
}
async function writeIosOnboardingSmokeResult(result: Record<string, unknown>): Promise<void> {
    await writeIosPreferenceSmokeResult(IOS_ONBOARDING_SMOKE_RESULT_KEY, result);
}
async function writeIosCloudOnboardingSmokeResult(result: Record<string, unknown>): Promise<void> {
    await writeIosPreferenceSmokeResult(IOS_CLOUD_ONBOARDING_SMOKE_RESULT_KEY, result);
}
async function writeIosOnboardingRelaunchSmokeResult(result: Record<string, unknown>): Promise<void> {
    await writeIosPreferenceSmokeResult(IOS_ONBOARDING_RELAUNCH_SMOKE_RESULT_KEY, result);
}
async function writeIosMixedContentSmokeResult(result: Record<string, unknown>): Promise<void> {
    await writeIosPreferenceSmokeResult(IOS_MIXED_CONTENT_SMOKE_RESULT_KEY, result);
}
async function writeIosAuthCallbackSmokeResult(result: Record<string, unknown>): Promise<void> {
...
async function writeIosPreferenceSmokeResult(key: string, result: Record<string, unknown>): Promise<void> {
    ...
    await boundedPreferenceWrite(() => Preferences.set({
        key,
        value,
    }));
}
async function boundedPreferenceWrite(operation: () => Promise<unknown>): Promise<void> {
    try {
        await Promise.race([
            operation(),
...
function parseIosOnboardingSmokeRequest(raw: string | null): {
    ...
    try {
        ...
        return {
            ...
            livenessPrompt: typeof parsed.livenessPrompt === "string" &&
...
async function readIosMixedContentSmokeRequest(fallbackApiBase?: string): Promise<{
    apiBase: string;
} | null> {
    let rawRequest: string | null = null;
    try {
        rawRequest = window.localStorage.getItem(IOS_MIXED_CONTENT_SMOKE_REQUEST_KEY);
    }
    catch {
    ...
    if (!rawRequest) {
        rawRequest = await boundedPreferenceGet(IOS_MIXED_CONTENT_SMOKE_REQUEST_KEY);
    }
    if (!rawRequest && !fallbackApiBase)
        return null;
    return parseIosOnboardingSmokeRequest(rawRequest ?? JSON.stringify({ apiBase: fallbackApiBase }));
}
async function waitForIosOnboardingElement<T extends Element>(selector: string, options?: {
    timeoutMs?: number;
    visible?: boolean;
}): Promise<T> {
    ...
    throw new Error(`Timed out waiting for iOS onboarding selector ${selector}${lastElement ? " to become visible" : ""}`);
}
function readIosOnboardingSmokeStorageSnapshot(): Record<string, string | null> {
    const keys = [
...
function readIosCloudOnboardingSmokeStorageSnapshot(): Record<string, string | boolean | null> {
...
async function waitForIosOnboardingSmokeStorageSnapshot(apiBase: string): Promise<Record<string, string | null>> {
    ...
    throw new Error(`Timed out waiting for iOS onboarding active server ${apiBase}: ${JSON.stringify(snapshot)}`);
}
const IOS_LIVENESS_ASSISTANT_SELECTOR = '[data-role="assistant"], [data-testid="chat-message-assistant"], [data-testid="thread-line"][data-role="assistant"]';
...
async function driveIosLivenessChatTurn(prompt: string): Promise<string> {
    const composer = await waitForIosOnboardingElement<HTMLTextAreaElement>('[data-testid="chat-composer-textarea"]', { visible: true });
    const priorReplies = document.querySelectorAll(IOS_LIVENESS_ASSISTANT_SELECTOR).length;
    const expectedToken = extractIosLivenessChallengeToken(prompt);
    composer.focus();
    setReactTextareaValue(composer, prompt);
    const send = document.querySelector<HTMLButtonElement>('[data-testid="chat-composer-action"], button[aria-label="Send"], button[aria-label="Send message"]');
    ...
    while (Date.now() < deadline) {
        const replies = document.querySelectorAll<HTMLElement>(IOS_LIVENESS_ASSISTANT_SELECTOR);
        for (let index = priorReplies; index < replies.length; index += 1) {
            ...
            if (expectedToken) {
                ...
                if (text.toLowerCase().includes(expectedToken))
                    return text;
            }
            else {
                return text;
    ...
    throw new Error("iOS liveness chat turn: assistant never produced a reply within the timeout");
}
/**
...
function parseIosCloudOnboardingSmokeRequest(raw: string | null): IosCloudOnboardingSmokeRequest {
...
async function waitForIosCloudSignInGreeting(): Promise<boolean> {
    ...
    while (Date.now() < deadline) {
        ...
        if (/Sign in to Eliza(?: Cloud)?/i.test(text))
            return true;
        await new Promise((resolve) => window.setTimeout(resolve, 250));
    }
...
async function triggerIosCloudSignInAction(): Promise<void> {
    const deadline = Date.now() + IOS_ONBOARDING_SMOKE_TIMEOUT_MS;
    while (Date.now() < deadline) {
        if (tryHandleFirstRunAction(FIRST_RUN_CLOUD_LOGIN_ACTION))
            return;
        await new Promise((resolve) => window.setTimeout(resolve, 250));
    }
...
async function waitForIosCloudOnboardingHome(): Promise<{
    ...
    const home = await waitForIosOnboardingElement<HTMLElement>('[data-testid="home-launcher-surface"][data-page="home"]', { visible: true, timeoutMs: IOS_ONBOARDING_SMOKE_TIMEOUT_MS });
    const composer = await waitForIosOnboardingElement<HTMLElement>('[data-testid="chat-composer-textarea"]', { visible: true, timeoutMs: IOS_ONBOARDING_SMOKE_TIMEOUT_MS });
...
async function runIosCloudOnboardingSmokeIfRequested(): Promise<boolean> {
    ...
    let rawRequest: string | null = null;
    try {
        rawRequest = window.localStorage.getItem(IOS_CLOUD_ONBOARDING_SMOKE_REQUEST_KEY);
    }
    catch {
        // error-policy:J3 unavailable storage reads as "no request"; the
        // Preferences fallback below still serves the simulator harness
    ...
    if (!rawRequest) {
        rawRequest = await boundedPreferenceGet(IOS_CLOUD_ONBOARDING_SMOKE_REQUEST_KEY);
    }
    if (!rawRequest)
        return false;
    ...
    try {
        ...
        const cloudActiveServer = typeof activeServer === "string" &&
            activeServer.includes('"kind":"cloud"');
        const onboardingHidden = !document.querySelector('[data-testid="first-run-chat"], [data-testid="startup-first-run-background"]');
        ...
        const livenessReply = await driveIosLivenessChatTurn(request.livenessPrompt);
        await writeIosCloudOnboardingSmokeResult({
        ...
        try {
            ...
            logger.debug({ error }, "[iOSCloudOnboardingSmoke] localStorage request cleanup failed");
        }
        await boundedPreferenceWrite(() => Preferences.remove({ key: IOS_CLOUD_ONBOARDING_SMOKE_REQUEST_KEY }));
    }
    return true;
}
...
async function runIosMixedContentSmokeIfRequested(options?: {
    ...
    const clientBaseUrl = typeof client.getBaseUrl === "function" ? client.getBaseUrl() : "";
    try {
        ...
        client.connectWs();
        const connectionState = typeof client.getConnectionState === "function"
            ? client.getConnectionState()
            : null;
        const restHealth = await fetchIosMixedContentHealth(request.apiBase);
        const bodyText = document.body?.innerText ?? "";
        const lostBackendOverlayAbsent = !/Lost backend connection/i.test(bodyText) &&
            !document.querySelector('[data-testid="connection-lost-overlay"]');
        await writeIosMixedContentSmokeResult({
            ok: restHealth.ok === true &&
            ...
            clientBaseUrl,
            expectedInsecureWebSocketUrl: new URL("/ws", request.apiBase).href.replace(/^http:/, "ws:"),
            mixedContentWouldBlockWebSocket: window.location.protocol === "https:" &&
        ...
        await writeIosMixedContentSmokeResult({
            ...
            clientBaseUrl,
            webSocketConstructorCalls: wsConstructorCalls,
            connectionState: typeof client.getConnectionState === "function"
                ? client.getConnectionState()
                : null,
            error: error instanceof Error ? error.message : String(error),
            storage: readIosOnboardingSmokeStorageSnapshot(),
        ...
        await boundedPreferenceWrite(() => Preferences.remove({ key: IOS_MIXED_CONTENT_SMOKE_REQUEST_KEY }));
    }
    return true;
}
async function runIosOnboardingSmokeIfRequested(): Promise<boolean> {
    if (!isIOS || iosOnboardingSmokeStarted)
        return iosOnboardingSmokeStarted;
    let rawRequest: string | null = null;
    try {
    ...
    try {
        ...
        const home = await waitForIosOnboardingElement<HTMLElement>('[data-testid="home-launcher-surface"][data-page="home"]', { visible: true });
        const composer = await waitForIosOnboardingElement<HTMLElement>('[data-testid="chat-composer-textarea"]', { visible: true });
        const onboardingHidden = !document.querySelector('[data-testid="first-run-chat"], [data-testid="startup-first-run-background"]');
        const storage = await waitForIosOnboardingSmokeStorageSnapshot(request.apiBase);
        ...
        await boundedPreferenceWrite(() => Preferences.remove({ key: IOS_ONBOARDING_SMOKE_REQUEST_KEY }));
    }
    return true;
}
async function runIosOnboardingRelaunchSmokeIfRequested(): Promise<boolean> {
    ...
    let rawRequest: string | null = null;
    try {
        rawRequest = window.localStorage.getItem(IOS_ONBOARDING_RELAUNCH_SMOKE_REQUEST_KEY);
    }
    catch {
        // error-policy:J3 unavailable storage reads as "no request"; the
        // Preferences fallback below still serves the simulator harness
    ...
    if (!rawRequest) {
        rawRequest = await boundedPreferenceGet(IOS_ONBOARDING_RELAUNCH_SMOKE_REQUEST_KEY);
    }
    if (!rawRequest)
        return false;
    ...
    try {
        ...
        await boundedPreferenceWrite(() => Preferences.remove({ key: IOS_ONBOARDING_RELAUNCH_SMOKE_REQUEST_KEY }));
    }
    return true;
}
async function initializeAgent(): Promise<void> {
    try {
        ...
        console.warn(`${APP_LOG_PREFIX} Agent not available:`, err instanceof Error ? err.message : err);
    }
}
async function initializePlatform(): Promise<void> {
    ...
    void runIosAttachmentSmokeIfRequested({
        ...
        removePreference: (key) => boundedPreferenceWrite(() => Preferences.remove({ key })),
    ...
    if (isIOS) {
        void import("./ios-voice-selftest-smoke").then(({ runIosVoiceSelfTestSmokeIfRequested }) => runIosVoiceSelfTestSmokeIfRequested({
            isIOS,
            client,
            getPreference: boundedPreferenceGet,
            removePreference: (key) => boundedPreferenceWrite(() => Preferences.remove({ key })),
...
async function registerMobileBlockerBackends(): Promise<void> {
    try {
        ...
        blocker.registerNativeWebsiteBlockerBackend(websiteNative.createNativeWebsiteBlockerBackend(websiteNative.WebsiteBlocker));
        blocker.registerNativeAppBlockerBackend(appNative.createNativeAppBlockerBackend(appNative.AppBlocker));
    }
    catch (error) {
...
function connectFirstRunRemoteDeepLink(rawApiBase: string): void {
    ...
    if (validatedUrl.protocol !== "https:" && validatedUrl.protocol !== "http:") {
        console.error(`${APP_LOG_PREFIX} Invalid first-run remote URL protocol:`, validatedUrl.protocol);
        return;
    }
    if (!isTrustedDeepLinkApiBaseUrl(validatedUrl)) {
        console.warn(`${APP_LOG_PREFIX} Rejected untrusted first-run remote host:`, validatedUrl.hostname);
        return;
    }
    ...
    void setStorageValue("elizaos:active-server", activeServer).catch((error) => {
        console.warn(`${APP_LOG_PREFIX} Failed to persist first-run remote active server:`, error);
    });
    dispatchConnect();
}
async function recordIosAuthCallbackSmoke(parsed: URL, path: string, url: string, outcome: AuthCallbackDeepLinkOutcome, activeServerBefore: string): Promise<void> {
...
async function handleAuthCallbackDeepLink(parsed: URL, path: string, url: string): Promise<boolean> {
    if (isAndroid && isAndroidCloudBuild()) {
        try {
            ...
            console.warn(`${APP_LOG_PREFIX} Android Cloud sign-in callback failed:`, error instanceof Error ? error.message : error);
            if (shouldAcknowledgeAndroidCloudCallback(error))
                return true;
            throw error;
        }
    }
    ...
    await recordIosAuthCallbackSmoke(parsed, path, url, outcome, activeServerBefore);
    return true;
}
/**
...
function handleDeepLink(url: string): undefined | Promise<boolean> {
    const remotePairing = parseRemoteControllerPairingDeepLink(url, APP_URL_SCHEME);
    if (remotePairing) {
        if (!isRemoteControllerPairingRuntimeAllowed({
            ...
            nativePluginAvailable: Capacitor.isPluginAvailable?.("RemoteControllerIdentity") === true,
        })) {
            console.warn(`${APP_LOG_PREFIX} Remote controller pairing requires the enrolled Linux desktop shell or this iPhone's secure native controller bridge`);
            return;
        }
    ...
    if (parsed.protocol !== `${APP_URL_SCHEME}:` && !isAppLink)
        return;
    ...
    if (path === "first-run/runtime/remote") {
        const rawApiBase = parsed.searchParams.get("api")?.trim() ||
    ...
    const navigationIntent = resolveDeepLinkNavigationIntent(path, parsed.searchParams);
    if (navigationIntent) {
        return dispatchDeepLinkNavigation(navigationIntent);
    }
    const assistantLaunchHashRoute = buildAssistantLaunchHashRoute(path, parsed.searchParams);
            ...
            if (gatewayUrl) {
                try {
                    ...
                    if (validatedUrl.protocol !== "https:" &&
                        ...
                        console.error(`${APP_LOG_PREFIX} Invalid gateway URL protocol:`, validatedUrl.protocol);
                        break;
                    }
                    if (!isTrustedDeepLinkApiBaseUrl(validatedUrl)) {
                        console.warn(`${APP_LOG_PREFIX} Rejected untrusted gateway URL host:`, validatedUrl.hostname);
                        break;
                    }
 ...
 */
function dispatchDeepLinkNavigation(intent: DeepLinkNavigationIntent): Promise<boolean> {
    return dispatchNavigateViewRequest(intent);
}
async function initializeDesktopShell(): Promise<void> {
    document.body.classList.add("desktop");
    const version = await invokeDesktopBridgeRequest<{
        runtime: string;
    }>({
        rpcMethod: "desktopGetVersion",
        ipcChannel: "desktop:getVersion",
    });
    const desktopNativeReady = version !== null &&
    ...
    if (commandPaletteRegistration?.success !== true) {
        throw new Error("[desktop-shell] Operating system rejected the command-palette shortcut");
    }
    ...
    if (chatOverlayHotkey.enabled) {
        ...
        if (chatOverlayRegistration?.success !== true) {
            throw new Error(`[desktop-shell] Operating system rejected the chat-overlay shortcut ${chatOverlayHotkey.accelerator}`);
        }
    }
    ...
    if (pushToTalkRegistration?.success !== true) {
        console.warn("[desktop-shell] Operating system rejected the push-to-talk shortcut; the pill hold gesture remains available");
    }
    ...
    subscribeDesktopBridgeEvent({
        ...
        listener: (payload: unknown) => {
            ...
            if (!detail || typeof detail.held !== "boolean")
                return;
    ...
    if (fnHoldStart?.status === "started") {
        if (fnHoldStart.fnSystemUsageType !== 0) {
            console.warn("[desktop-shell] fn-hold push-to-talk is active but the macOS 'Press 🌐 key to' action is also enabled — a quick fn tap will trigger the system action; set it to 'Do Nothing' in System Settings → Keyboard");
        }
    }
    else if (fnHoldStart?.status === "permission-missing") {
        console.warn("[desktop-shell] fn-hold push-to-talk needs Accessibility permission (System Settings → Privacy & Security → Accessibility); falling back to the toggle hotkey");
    }
...
function setupPlatformStyles(): void {
    ...
    if (shouldInstallStandaloneBottomReclaim({
    ...
    console.info(`[shell] window shell mode: ${windowShellRoute.mode} (search="${typeof window !== "undefined" ? window.location.search : ""}")`);
...
const DeveloperWorkspace = lazy(async () => {
    const mod = await import("@elizaos/ui/components/developer/DeveloperWorkspace");
...
function mountReactApp(): void {
    ...
    const isSpecialWindowShell = phoneCompanion || detachedShell || appWindowSlug !== null;
    ...
    const appSubtree = (<>
      ...
      <ShellModalityProvider modality="gui">
        <ShellRoleProvider>
          {developerWorkspaceEnabled && !isSpecialWindowShell ? (<DeveloperWorkspace readerMode={/^\/dev2\/?$/.test(window.location.pathname)}>
              <App />
            </DeveloperWorkspace>) : (<App />)}
        </ShellRoleProvider>
      </ShellModalityProvider>
    </>);
    const mainTree = __ELIZA_CHAT_UI_HARNESS__ === true ? (<ChatWidgetHarness />) : shouldMountWebShell() && !isSpecialWindowShell ? (<CloudRouterShell cloudManagementElement={<ManagedCloudPage />} appElement={<AppProvider branding={APP_BRANDING}>{appSubtree}</AppProvider>}/>) : (<AppProvider branding={APP_BRANDING}>
        {phoneCompanion ? (<ShellViewAgentSurface viewId={PHONE_COMPANION_AGENT_VIEW_ID} surfaceKind="app-shell">
            <PhoneCompanionApp />
          </ShellViewAgentSurface>) : detachedShell ? (<div className="flex h-[100dvh] min-h-0 w-full max-w-full flex-col overflow-hidden bg-bg">
            <DetachedShellRoot route={windowShellRoute}/>
          </div>) : appWindowSlug ? (<div className="flex h-[100dvh] min-h-0 w-full max-w-full flex-col overflow-hidden bg-bg">
            <AppWindowRenderer slug={appWindowSlug}/>
          </div>) : (appSubtree)}
      </AppProvider>);
    markStartup("react-mount:start");
    rendererBootstrap.root ??= createRoot(rootEl);
    rendererBootstrap.root.render(<ErrorBoundary>
      <StrictMode>
...
function isPrivateOrLoopbackApiHost(host: string): boolean {
    ...
    return (isLoopbackApiHost(normalized) ||
        (normalized.includes(":") &&
...
function allowsIosSimulatorLoopbackApiBase(parsed: URL): boolean {
    return (isNative &&
        ...
        isTruthyBuildFlag(import.meta.env.VITE_ELIZA_IOS_ALLOW_SIMULATOR_LOOPBACK) &&
...
function isTrustedNativeWebSocketUrl(value: string): boolean {
    try {
        ...
        if (!usesStrictIosNetworkPolicy())
            return true;
        return (parsed.protocol === "wss:" && !isPrivateOrLoopbackApiHost(parsed.hostname));
    }
    catch {
...
function validateAndSetApiBase(apiBase: string): void {
    try {
        ...
        if (isTrustedApiBaseUrl(parsed)) {
            ...
            console.warn(`${APP_LOG_PREFIX} Rejected non-local apiBase:`, parsed.hostname);
        }
    }
    catch {
        ...
        if (apiBase.startsWith("/") && !apiBase.startsWith("//")) {
            ...
            console.warn(`${APP_LOG_PREFIX} Rejected invalid relative apiBase:`, apiBase);
        }
    }
}
...
function getCurrentIosRuntimeConfig(): IosRuntimeConfig {
    ...
    if (typeof window !== "undefined") {
        try {
            const mode = normalizeMobileRuntimeMode(window.localStorage.getItem(MOBILE_RUNTIME_MODE_STORAGE_KEY));
            if (mode)
                config = { ...config, mode };
        }
        catch (error) {
            ...
            if (!(error instanceof DOMException) || error.name !== "SecurityError")
                throw error;
            console.warn(`${APP_LOG_PREFIX} Runtime preference storage unavailable`, error);
        }
    }
...
function applyBuildTimeIosConnection(): void {
    ...
    setBootConfig(next);
    if (isIOS && IOS_RUNTIME_ENV_CONFIG.mode === "local")
        return;
...
async function getOrCreateDeviceBridgeId(): Promise<string> {
    ...
    const readPersisted = async (): Promise<string | undefined> => {
        try {
            const fromPrefs = (await Preferences.get({ key: DEVICE_BRIDGE_ID_KEY })).value?.trim();
            if (fromPrefs)
                return fromPrefs;
        }
        catch {
        ...
        return (globalThis.localStorage?.getItem(DEVICE_BRIDGE_ID_KEY)?.trim() ||
            undefined);
    };
...
async function configureMobileBackgroundRunner(retry = 0): Promise<void> {
    ...
    let authToken = bootConfig.apiToken?.trim() || runtimeConfig.apiToken?.trim() || undefined;
    ...
    try {
        ...
        console.warn(`${APP_LOG_PREFIX} Background runner unavailable:`, error instanceof Error ? error.message : error);
    }
    if (isAndroid && runtimeConfig.mode === "local" && !authToken && retry < 2) {
        window.setTimeout(() => void configureMobileBackgroundRunner(retry + 1), BACKGROUND_RUNNER_CONFIG_RETRY_MS * (retry + 1));
    }
}
async function initializeMobileDeviceBridge(): Promise<void> {
    ...
    if (mobileDeviceBridgeClient)
        return;
    if (mobileDeviceBridgeStartPromise)
        return;
    ...
    mobileDeviceBridgeStartPromise = (async () => {
        try {
            ...
            const pairingToken = runtimeConfig.deviceBridgeToken?.trim() ||
                (isAndroid && runtimeConfig.mode === "local"
                    ? await readAndroidLocalAgentToken()
                    : undefined);
            if (isAndroid && runtimeConfig.mode === "local" && !pairingToken) {
                window.setTimeout(() => void initializeMobileDeviceBridge(), BACKGROUND_RUNNER_CONFIG_RETRY_MS);
                return;
            }
            mobileDeviceBridgeClient = startDeviceBridgeClient({
                ...
                onStateChange: (state, detail) => {
                    console.info(`${APP_LOG_PREFIX} Device bridge ${state}`, detail ?? "");
                },
            });
            ...
            if (isAndroid && !cameraBridgeResponderStop) {
                const { startCameraBridgeResponder } = await import("./camera-bridge-responder");
            ...
            // error-policy:J4 optional native module — absence logged, app degrades
            console.warn(`${APP_LOG_PREFIX} Device bridge unavailable:`, error instanceof Error ? error.message : error);
        }
        finally {
...
function initializeMobileRuntimeModeListener(): void {
    if (!isNative || mobileRuntimeModeListenerInstalled)
        return;
...
function initVisionBridgesIfEnabled(): void {
    if (import.meta.env.VITE_ELIZA_VISION_BRIDGES !== "1")
        return;
...
async function main(): Promise<void> {
    ...
    if (await runIosFullBunEntrypoint({
        isIOS,
    ...
    try {
        ...
        console.error(`${APP_LOG_PREFIX} Failed to apply managed cloud launch session:`, err instanceof Error ? err.message : err);
    }
