...
import { REMOTE_CONTROL_ENVELOPE_ALGORITHM } from "@elizaos/core/contracts/remote-control";
import { REMOTE_CONTROL_SIGNATURE_ALGORITHM } from "@elizaos/core/contracts/remote-control";
...
import { isSignedRemoteCommandResult } from "@elizaos/core/contracts/remote-control";
import { isSignedRemoteCommandStartReceipt } from "@elizaos/core/contracts/remote-control";
import { randomBytes } from "node:crypto";
import { sign } from "node:crypto";
import { type EncryptedRemoteControlEnvelope } from "@elizaos/core/contracts/remote-control";
import { type RemoteCommandBinding } from "@elizaos/core/contracts/remote-control";
import { type RemoteCommandBody } from "@elizaos/core/contracts/remote-control";
import { type RemoteCommandResult } from "@elizaos/core/contracts/remote-control";
import { type RemoteCommandStartReceipt } from "@elizaos/core/contracts/remote-control";
import { type RemoteControlMessageKind } from "@elizaos/core/contracts/remote-control";
...
export type RemoteCommandRejection = "malformed" | "unknown_controller" | "wrong_owner" | "wrong_grant" | "stale_grant" | "wrong_session" | "wrong_controller" | "wrong_target" | "wrong_target_key" | "revoked" | "expired" | "issued_in_future" | "ttl_too_long" | "payload_digest_mismatch" | "invalid_signature";
...
export type OpenedRemoteControlMessage = SignedRemoteCommand | SignedRemoteCommandStartReceipt | SignedRemoteCommandResult;
function remoteCryptoError(message: string, code: string, context?: Record<string, unknown>, cause?: unknown): ElizaError {
...
export function digestRemoteResultValue(result: RemoteJsonValue | undefined, errorCode: string | undefined): string {
    return digestRemoteControlValue({ result, errorCode });
}
function signatureFor(value: unknown, privateKeyJwk: JsonWebKey): string {
    return sign("sha256", Buffer.from(canonicalizeRemoteControlValue(value)), createPrivateKey({ key: privateKeyJwk, format: "jwk" })).toString("base64url");
}
function hasValidSignature(value: unknown, signature: string, publicKeyJwk: JsonWebKey): boolean {
    try {
        return verify("sha256", Buffer.from(canonicalizeRemoteControlValue(value)), createPublicKey({ key: publicKeyJwk, format: "jwk" }), Buffer.from(signature, "base64url"));
    }
    catch {
...
export function signRemoteCommand(body: RemoteCommandBody, signingPrivateKeyJwk: JsonWebKey): SignedRemoteCommand {
    return {
        body,
...
export function signRemoteCommandStartReceipt(body: RemoteCommandStartReceipt, signingPrivateKeyJwk: JsonWebKey): SignedRemoteCommandStartReceipt {
    return {
        body,
        signatureAlgorithm: REMOTE_CONTROL_SIGNATURE_ALGORITHM,
        signature: signatureFor(body, signingPrivateKeyJwk),
...
export function signRemoteCommandResult(body: RemoteCommandResult, signingPrivateKeyJwk: JsonWebKey): SignedRemoteCommandResult {
    return {
        body,
        signatureAlgorithm: REMOTE_CONTROL_SIGNATURE_ALGORITHM,
        signature: signatureFor(body, signingPrivateKeyJwk),
...
function bindingMatches(actual: RemoteCommandBinding, expected: RemoteCommandBinding): boolean {
...
export function verifyRemoteCommandAuthenticity(options: VerifyRemoteCommandAuthenticityOptions): RemoteCommandAuthenticity {
    ...
    if (body.ownerId !== options.expectedOwnerId ||
    ...
    if (body.sessionId !== options.expectedSessionId ||
        grant.sessionId !== body.sessionId) {
        return { ok: false, reason: "wrong_session" };
    }
    if (body.targetRuntimeId !== options.expectedTargetRuntimeId ||
    ...
    if (targetIdentity.ownerId !== body.ownerId ||
...
export function verifyRemoteCommandStartReceipt(signed: SignedRemoteCommandStartReceipt, targetIdentity: RemoteTargetPublicIdentity, command: SignedRemoteCommand): boolean {
    return (isSignedRemoteCommandStartReceipt(signed) &&
        ...
        hasValidSignature(signed.body, signed.signature, targetIdentity.signingPublicKeyJwk));
}
/** Verifies a target-signed terminal result and all original command bindings. */
export function verifyRemoteCommandResult(signed: SignedRemoteCommandResult, targetIdentity: RemoteTargetPublicIdentity, command: SignedRemoteCommand): boolean {
    return (isSignedRemoteCommandResult(signed) &&
        targetIdentity.role === "target" &&
        targetIdentity.runtimeId === signed.body.targetRuntimeId &&
        targetIdentity.keyId === signed.body.targetKeyId &&
        targetIdentity.ownerId === signed.body.ownerId &&
...
function envelopeHeader(scope: RemoteControlEnvelopeExpectation, message: OpenedRemoteControlMessage): RemoteControlEnvelopeHeader {
    const common = {
    ...
    if (scope.messageKind === "command") {
        if (!isSignedRemoteCommand(message)) {
            throw remoteCryptoError("Remote control command envelope requires a command", "REMOTE_ENVELOPE_KIND_INVALID");
        }
        return {
            ...common,
            messageKind: "command",
...
function authenticatedEnvelopeHeader(envelope: EncryptedRemoteControlEnvelope): RemoteControlEnvelopeHeader {
    const common = {
...
function messageMatchesKind(messageKind: RemoteControlMessageKind, message: unknown): message is OpenedRemoteControlMessage {
    if (messageKind === "command")
        return isSignedRemoteCommand(message);
    if (messageKind === "start_receipt") {
        return isSignedRemoteCommandStartReceipt(message);
    }
    return isSignedRemoteCommandResult(message);
}
function envelopeKeyDirectionMatches(scope: RemoteControlEnvelopeExpectation): boolean {
...
export function sealRemoteControlMessage(message: OpenedRemoteControlMessage, scope: RemoteControlEnvelopeExpectation, recipientEncryptionPublicKeyJwk: JsonWebKey): EncryptedRemoteControlEnvelope {
    if (!messageMatchesKind(scope.messageKind, message)) {
        throw remoteCryptoError("Remote control message does not match envelope kind", "REMOTE_ENVELOPE_KIND_INVALID");
    }
    if (!envelopeKeyDirectionMatches(scope)) {
        throw remoteCryptoError("Remote control envelope key direction is invalid", "REMOTE_ENVELOPE_KEY_DIRECTION_INVALID");
    }
    if (!bindingMatches(message.body, scope)) {
        throw remoteCryptoError("Remote control message does not match envelope scope", "REMOTE_ENVELOPE_SCOPE_INVALID");
    }
    ...
    const key = Buffer.from(hkdfSync("sha256", sharedSecret, salt, createHash("sha256")
...
function expectationMatches(envelope: EncryptedRemoteControlEnvelope, expected: RemoteControlEnvelopeExpectation): boolean {
    return (envelope.messageKind === expected.messageKind &&
 ...
 */
export function openRemoteControlMessage(envelope: EncryptedRemoteControlEnvelope, recipientEncryptionPrivateKeyJwk: JsonWebKey, expected: RemoteControlEnvelopeExpectation): OpenedRemoteControlMessage {
    if (!isEncryptedRemoteControlEnvelope(envelope) ||
        !expectationMatches(envelope, expected)) {
        throw remoteCryptoError("Remote control envelope scope mismatch", "REMOTE_ENVELOPE_SCOPE_INVALID");
    }
    const sharedSecret = diffieHellman({
    ...
    const aad = envelopeAad(header);
    const key = Buffer.from(hkdfSync("sha256", sharedSecret, Buffer.from(envelope.salt, "base64url"), createHash("sha256")
        .update("eliza-remote-control-v1\0")
        .update(aad)
        .digest(), 32));
    const combined = Buffer.from(envelope.ciphertext, "base64url");
    if (combined.length <= 16) {
        throw remoteCryptoError("Remote control ciphertext is invalid", "REMOTE_CIPHERTEXT_INVALID");
    }
    ...
    try {
        ...
        throw remoteCryptoError("Remote control plaintext is invalid JSON", "REMOTE_PLAINTEXT_INVALID", undefined, cause);
    }
    if (!messageMatchesKind(envelope.messageKind, parsed) ||
        !bindingMatches(parsed.body, envelope) ||
        digestRemoteControlValue(parsed) !== envelope.messageDigest) {
        throw remoteCryptoError("Remote control plaintext binding mismatch", "REMOTE_PLAINTEXT_BINDING_INVALID");
    }
    if (envelope.messageKind === "command" &&
        ...
        throw remoteCryptoError("Remote control command routing metadata mismatch", "REMOTE_ROUTING_METADATA_INVALID");
    }
    return parsed;
}
