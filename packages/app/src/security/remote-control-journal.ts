...
import { REMOTE_CONTROL_MAX_ACTIVE_SESSIONS } from "@elizaos/core/contracts/remote-control";
...
export type RemoteCommandJournalStatus = "reserved" | "started" | RemoteCommandResultStatus;
...
export type RemoteCommandAdmissionRejection = "unknown_grant" | "revoked" | "expired" | "wrong_owner" | "wrong_session" | "wrong_controller" | "wrong_target" | "stale_grant" | "replay" | "command_digest_mismatch" | "command_conflict" | "capacity";
export type RemoteCommandAdmission = {
    ...
    ok: false;
    reason: RemoteCommandAdmissionRejection;
};
...
export type RemoteCommandCompletion = {
    ...
    ok: false;
    reason: "not_found" | "not_started" | "execution_conflict" | "result_conflict";
};
...
export interface DurableRemoteCommandJournal {
    ...
    beginExecution(commandId: string, startedAt: number): Promise<RemoteCommandStart>;
    /** Idempotently records one terminal outcome for the matching execution. */
    completeExecution(input: CompleteRemoteCommandInput): Promise<RemoteCommandCompletion>;
    /** Converts orphaned starts to explicit ambiguity; it never retries them. */
    recoverInterruptedExecutions(recoveredAt: number): Promise<RemoteCommandJournalRecord[]>;
    /** Clears replay state and fences all nonterminal work for a dead session. */
    terminateSession(sessionId: string, terminatedAt: number): Promise<RemoteCommandSessionTermination>;
...
function remoteJournalError(message: string, code: string, context?: Record<string, unknown>, cause?: unknown): ElizaError {
...
function copyRecord(record: RemoteCommandJournalRecord): RemoteCommandJournalRecord {
    return { ...record };
}
...
function sameBinding(record: RemoteCommandJournalRecord, command: SignedRemoteCommand): boolean {
    const body = command.body;
    return (record.version === body.version &&
...
export class InMemoryDurableRemoteCommandJournal implements DurableRemoteCommandJournal {
    ...
    async installAuthority(grant: RemoteControllerGrant): Promise<void> {
        return this.transact(() => {
            if (!isRemoteControllerGrant(grant)) {
                throw remoteJournalError("Remote authority is malformed", "REMOTE_AUTHORITY_MALFORMED");
            }
            const current = this.authorities.get(grant.grantId);
            if (current && grant.revision < current.revision) {
                throw remoteJournalError("Remote authority revision cannot move backward", "REMOTE_AUTHORITY_REVISION_REGRESSION", { grantId: grant.grantId });
            }
            if (current?.revokedAt !== null &&
                current?.revokedAt !== undefined &&
                grant.revokedAt === null) {
                throw remoteJournalError("Remote authority revocation cannot be reversed", "REMOTE_AUTHORITY_REVOCATION_REVERSAL", { grantId: grant.grantId });
            }
            if (current &&
                grant.revision === current.revision &&
                canonicalizeRemoteControlValue(current) !==
                    canonicalizeRemoteControlValue(grant)) {
                throw remoteJournalError("Remote authority revision cannot be replaced", "REMOTE_AUTHORITY_REVISION_CONFLICT", { grantId: grant.grantId });
            }
    ...
    async authorizeAndReserve(input: {
        ...
        return this.transact(() => {
            ...
            if (authority.controllerDeviceId !== body.controllerDeviceId ||
            ...
            if (session.ownerId !== body.ownerId ||
            ...
            if (session.nonces.has(body.nonce) ||
                body.sequence <= session.lastSequence) {
                return { ok: false, reason: "replay" };
            }
            if (session.nonces.size >= REMOTE_CONTROL_MAX_REPLAY_ENTRIES_PER_SESSION) {
                return { ok: false, reason: "capacity" };
            }
    ...
    async beginExecution(commandId: string, startedAt: number): Promise<RemoteCommandStart> {
        return this.transact(() => {
    ...
    async completeExecution(input: CompleteRemoteCommandInput): Promise<RemoteCommandCompletion> {
        return this.transact(() => {
    ...
    async recoverInterruptedExecutions(recoveredAt: number): Promise<RemoteCommandJournalRecord[]> {
        return this.transact(() => {
    ...
    async terminateSession(sessionId: string, terminatedAt: number): Promise<RemoteCommandSessionTermination> {
        return this.transact(() => {
 ...
 */
export async function executeReservedRemoteCommand(journal: DurableRemoteCommandJournal, input: {
    commandId: string;
    startedAt: number;
    completedAt: () => number;
    effect: (executionId: string) => Promise<RemoteCommandEffectOutcome>;
}): Promise<RemoteCommandExecutionOutcome> {
    const started = await journal.beginExecution(input.commandId, input.startedAt);
    if (!started.ok)
    ...
    if (!executionId) {
        throw remoteJournalError("Durable remote start did not allocate an execution ID", "REMOTE_EXECUTION_ID_MISSING", { commandId: input.commandId });
    }
    ...
    try {
        ...
        if (!ambiguous.ok) {
            throw remoteJournalError("Could not persist ambiguous remote execution", "REMOTE_AMBIGUOUS_RESULT_PERSIST_FAILED", { commandId: input.commandId }, cause);
        }
    ...
    if (!completed.ok) {
        throw remoteJournalError(`Could not persist remote command result: ${completed.reason}`, "REMOTE_RESULT_PERSIST_FAILED", { commandId: input.commandId, reason: completed.reason });
    }
