...
// TDZ-hardening (see also packages/app/src/services/vault-mirror.ts).
...
// agent → app → … → this module) re-enters those functions before this
 ...
 */
function stewardOsPairs(): ReadonlyArray<readonly [
 ...
 */
async function migrateOsStoreWalletKeysIntoVault(envKeys: ReadonlyArray<keyof NodeJS.ProcessEnv>): Promise<string[]> {
    if (envKeys.length === 0)
        return [];
...
export async function hydrateWalletKeysFromNodePlatformSecureStore(): Promise<void> {
    ...
    if (missingWalletKeys.length > 0) {
        try {
            ...
            if (migrated.length > 0) {
                logger.info(`[wallet][vault] migrated ${migrated.length} key(s) from OS keystore: ${migrated.join(", ")}`);
            }
        }
        catch (err) {
    ...
    if (resolveDevCloudStewardOperationalTuple()) {
        for (const [envKey] of stewardOsPairs()) {
            ...
            if (launchValue?.trim())
                process.env[envKey] = launchValue;
            else
