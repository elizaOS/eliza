 ...
 */
export function resolveElizaStateDir(env: RuntimeEnvRecord = process.env): string {
...
function parseEnvelope(raw: string): JwksCacheEnvelope | null {
    ...
    if (!isFiniteNumber(candidate.fetchedAt) ||
 ...
 */
export async function readCachedJwks(issuer: string, options: {
    ...
    if (envelope.issuer !== issuer)
        return null;
    if (now - envelope.fetchedAt > ttlMs)
        return null;
 ...
 */
export async function writeCachedJwks(issuer: string, jwks: JwksDocument, options: {
    env?: RuntimeEnvRecord;
    now?: number;
} = {}): Promise<void> {
    const env = options.env ?? process.env;
