...
import { claudeMinRemainingMs } from "./claude-token-refresh.js";
import { configuredAccountStrategyForProvider } from "./account-pool.js";
...
import { loadAccount } from "@elizaos/auth/auth/account-storage";
...
import { resolveClaudeExpectedRunMs } from "./claude-token-refresh.js";
import { resolveStateDir } from "@elizaos/core";
import { rmSync } from "node:fs";
import { saveCredentials } from "@elizaos/auth/auth/credentials";
import { selectionForProvider } from "./account-pool.js";
import { setCodingAgentSelectorBridge } from "@elizaos/core";
import { statSync } from "node:fs";
import { type AccessTokenOutcome } from "@elizaos/auth/auth/credentials";
...
import { type CodingProviderAvailability } from "@elizaos/core";
...
import { type Strategy } from "./account-pool.js";
...
function getEnvCodingStrategy(): Strategy | undefined {
    ...
    if (!env)
        return undefined;
    if (VALID_CODING_STRATEGIES.has(env as Strategy))
        return env as Strategy;
    logger.warn(`[coding-account-bridge] ignoring invalid ELIZA_CODING_ACCOUNT_STRATEGY=${JSON.stringify(env)}; using the provider default`);
    return undefined;
}
/**
 ...
 */
const AGENT_PROVIDER_CANDIDATES: Readonly<Record<string, readonly LinkedAccountProviderId[]>> = {
 ...
 */
const AUTH_FAILURE_PATTERN = /\b(40[13]|invalid[_ ]?grant|invalid[_ ]?token|unauthor|forbidden|re-?auth|revoked|expired)\b/i;
...
function accessTokenFailureIsAuth(outcome: AccessTokenOutcome | undefined, err?: unknown): boolean {
    if (outcome && !outcome.ok)
        return outcome.kind === "auth";
    return isAuthFailure(err);
}
function codexHomeDir(accountId: string): string {
    return path.join(process.env.ELIZA_HOME || resolveStateDir(), "auth", "_codex-home", accountId);
}
...
function parseMaterializedCodexAuth(raw: string, authPath: string): CodexAuthParseResult {
    let value: unknown;
    try {
    ...
    if (typeof value !== "object" || value === null) {
        return {
            ok: false,
            error: new ElizaError(`Codex auth file has the wrong shape: ${authPath}`, {
                code: "CODEX_AUTH_FILE_INVALID",
                context: { authPath },
                severity: "fatal",
            }),
    ...
    if (typeof tokens !== "object" || tokens === null) {
        return {
            ok: false,
            error: new ElizaError(`Codex auth file is missing its token pair: ${authPath}`, {
                code: "CODEX_AUTH_FILE_INVALID",
                context: { authPath },
                severity: "fatal",
            }),
    ...
    if (typeof accessToken !== "string" ||
        ...
        return {
            ok: false,
            error: new ElizaError(`Codex auth file contains an invalid token generation: ${authPath}`, {
                code: "CODEX_AUTH_FILE_INVALID",
                context: { authPath },
                severity: "fatal",
            }),
...
async function readStableCodexAuth(homeDir: string): Promise<MaterializedCodexAuthCandidate> {
    ...
    for (let attempt = 0; attempt < CODEX_AUTH_STABLE_READ_ATTEMPTS; attempt++) {
        ...
        await new Promise<void>((resolve) => setTimeout(resolve, CODEX_AUTH_STABLE_READ_DELAY_MS));
    }
    throw new ElizaError(`Codex auth file did not reach a valid stable generation: ${authPath}`, {
...
function listCodexHomeCandidates(accountId: string): string[] {
    ...
    const homes: string[] = [];
    if (existsSync(path.join(accountHome, "auth.json")))
...
function publishJsonExclusive(filePath: string, value: unknown): void {
    ...
    try {
        ...
        throw new ElizaError(`Could not publish Codex auth generation: ${filePath}`, {
...
function canonicalCodexAuth(accountId: string, record: NonNullable<ReturnType<typeof loadAccount>>): MaterializedCodexAuthJson & Record<string, unknown> {
    const { access, refresh, idToken } = record.credentials;
    if (!access || !refresh) {
        throw new ElizaError(`openai-codex account "${accountId}" is missing a complete token pair`, {
...
async function createCanonicalCodexHome(accountId: string, record: NonNullable<ReturnType<typeof loadAccount>>, storagePolicy: AccountStoragePolicy): Promise<MaterializedCodexAuthCandidate> {
    const refreshToken = record.credentials.refresh;
    if (!refreshToken) {
        throw new ElizaError(`openai-codex account "${accountId}" is missing a refresh token`, {
            code: "CODEX_CANONICAL_CREDENTIAL_MISSING",
            context: { accountId },
            severity: "fatal",
        });
    ...
    const authPath = path.join(homeDir, "auth.json");
    withAccountStorageMutation(storagePolicy, "materialize-codex-auth-generation", () => {
    ...
    if (candidate.auth.tokens.refresh_token !== refreshToken) {
        throw new ElizaError(`Codex auth generation does not match its canonical generation: ${authPath}`, {
...
async function reconcileCodexTokensLocked(accountId: string, storagePolicy: AccountStoragePolicy): Promise<CodexReconciliation> {
    ...
    const matching = candidates.filter((candidate) => candidate.auth.tokens.refresh_token === record.credentials.refresh);
    const divergent = candidates.filter((candidate) => candidate.auth.tokens.refresh_token !== record.credentials.refresh);
    for (const candidate of divergent) {
        if (candidate.lastRefreshMs === null) {
            throw new ElizaError(`Codex auth generation cannot be ordered safely: ${candidate.authPath}`, {
    ...
    const newer = divergent
        .filter((candidate) => candidate.lastRefreshMs !== null &&
        candidate.lastRefreshMs > record.updatedAt)
        .sort((left, right) => (right.lastRefreshMs ?? Number.NEGATIVE_INFINITY) -
        (left.lastRefreshMs ?? Number.NEGATIVE_INFINITY));
    const newest = newer[0];
    const equallyNew = newest
        ? newer.find((candidate) => candidate !== newest &&
    ...
    if (newest && equallyNew) {
        throw new ElizaError(`Codex auth generations have an ambiguous refresh order for account "${accountId}"`, {
            code: "CODEX_AUTH_GENERATION_AMBIGUOUS",
            context: {
                accountId,
    ...
    if (newest) {
        ...
        }, accountId, storagePolicy);
        const adoptedRecord = loadAccount("openai-codex", accountId, storagePolicy);
        if (!adoptedRecord) {
            throw new ElizaError(`Adopted Codex credentials could not be reloaded for account "${accountId}"`, {
                code: "CODEX_CANONICAL_CREDENTIAL_MISSING",
                context: { accountId },
                severity: "fatal",
            });
        }
        logger.info(`[coding-account-bridge] adopted rotated Codex tokens from CODEX_HOME for account "${accountId}" (CLI self-refresh)`);
        return { adopted: true, candidate: newest, record: adoptedRecord };
    }
    const candidate = matching.sort((left, right) => (right.lastRefreshMs ?? Number.NEGATIVE_INFINITY) -
        (left.lastRefreshMs ?? Number.NEGATIVE_INFINITY))[0];
 ...
 */
export async function adoptRotatedCodexTokens(accountId: string): Promise<boolean> {
    const storagePolicy = accountStoragePolicy();
    return accountRefreshMutex.acquire(`openai-codex:${accountId}`, async () => {
        const reconciled = await reconcileCodexTokensLocked(accountId, storagePolicy);
...
function resolveCodexConfig(): string {
    ...
    for (const key of ["ELIZA_CODEX_MODEL", "ELIZA_CODEX_MODEL_POWERFUL"]) {
        ...
        if (!/^[\w.:/-]+$/.test(candidate)) {
            logger.warn(`[coding-account-bridge] ignoring malformed ${key}=${JSON.stringify(candidate)}`);
            continue;
        }
        model = candidate;
        break;
    ...
    if (!model) {
        ...
        if (existsSync(machineConfig)) {
            ...
            const match = readFileSync(machineConfig, "utf-8").match(/^\s*model\s*=\s*["']([^"']+)["']/m);
            if (match?.[1])
                model = match[1];
        }
    }
    ...
    if (effort && !CODEX_EFFORT_VALUES.has(effort)) {
        ...
        logger.warn(`[coding-account-bridge] ignoring invalid ELIZA_CODEX_EFFORT=${JSON.stringify(effort)} (expected low|medium|high|xhigh|max|ultra)`);
        effort = undefined;
    }
        ...
        logger.warn(`[coding-account-bridge] ELIZA_CODEX_EFFORT=${JSON.stringify(effort)} is not supported by the managed codex-acp contract (supported: low|medium|high|xhigh); omitting model_reasoning_effort so config.toml stays loadable`);
        effort = undefined;
    }
    return `model = "${model || "gpt-5.6-sol"}"\ncli_auth_credentials_store = "file"\n${effort ? `model_reasoning_effort = "${effort}"\n` : ""}`;
}
function materializeRequiredTextFile(targetPath: string, contents: string, storagePolicy: AccountStoragePolicy, errorCode: "CODEX_CONFIG_MATERIALIZATION_FAILED" | "CODEX_ACTIVE_HOME_MATERIALIZATION_FAILED", description: string): void {
    withAccountStorageMutation(storagePolicy, `materialize-codex-${description}`, () => {
        ...
        try {
            ...
            if (readFileSync(targetPath, "utf-8") !== contents) {
                throw new ElizaError(`Codex ${description} verification failed after materialization: ${targetPath}`, {
                    code: errorCode,
                    context: { targetPath },
                    severity: "fatal",
                });
            ...
            throw new ElizaError(`Could not materialize required Codex ${description}: ${targetPath}`, {
                code: errorCode,
                cause,
                context: { targetPath },
                severity: "fatal",
...
function materializeRequiredCodexConfig(homeDir: string, storagePolicy: AccountStoragePolicy): void {
    materializeRequiredTextFile(path.join(homeDir, "config.toml"), resolveCodexConfig(), storagePolicy, "CODEX_CONFIG_MATERIALIZATION_FAILED", "config");
}
function publishActiveCodexHome(accountId: string, homeDir: string, storagePolicy: AccountStoragePolicy): void {
    const accountHome = codexHomeDir(accountId);
    const relativeHome = path.relative(accountHome, homeDir);
    if (relativeHome.startsWith(`..${path.sep}`) ||
        relativeHome === ".." ||
    ...
    materializeRequiredTextFile(path.join(accountHome, CODEX_ACTIVE_HOME_FILE), `${relativeHome || "."}\n`, storagePolicy, "CODEX_ACTIVE_HOME_MATERIALIZATION_FAILED", "active-home pointer");
}
/**
...
async function materializeCodexHome(accountId: string): Promise<string> {
    ...
    return accountRefreshMutex.acquire(`openai-codex:${accountId}`, async () => {
        ...
        const candidate = reconciled.candidate ??
            (await createCanonicalCodexHome(accountId, reconciled.record, storagePolicy));
...
async function buildEnvPatch(providerId: LinkedAccountProviderId, accessToken: string): Promise<Record<string, string>> {
    switch (providerId) {
        ...
        default: {
            ...
            const envKey = DIRECT_ACCOUNT_PROVIDER_ENV[providerId as DirectAccountProvider];
...
function makeBridge(pool: AccountPool): CodingAgentSelectorBridge {
    return {
        describe() {
            ...
            const now = Date.now();
            for (const [agentType, providers] of Object.entries(AGENT_PROVIDER_CANDIDATES)) {
                out[agentType] = providers.map((providerId) => {
                    ...
                    return {
                        ...
                        healthy: accounts.filter((a) => a.enabled && isAccountSelectableNow(a, now)).length,
                    };
                });
            }
            return out;
        ...
        async select(agentType, opts) {
            ...
            for (const providerId of candidates) {
                if (opts?.providerId && providerId !== opts.providerId)
                    continue;
                const excluded = [
                    ...(opts?.exclude ?? []),
                ...
                if (providerId === "openai-codex") {
                    ...
                    const resolveOpts = providerId === "anthropic-subscription"
                        ? {
                            minRemainingMs: claudeMinRemainingMs(resolveClaudeExpectedRunMs((key) => process.env[key])),
                        }
                        : undefined;
                    ...
                    try {
                        ...
                        if (accessToken === null &&
                            ...
                            if (stillValid.ok) {
                                logger.info(`[coding-account-bridge] proactive refresh for ${providerId}/${account.id} did not yield a fresh token; using the still-valid shorter-TTL token (a long run may hit the typed expiry signal)`);
                        ...
                        resolveError = err;
                        logger.warn(`[coding-account-bridge] token resolve failed for ${providerId}/${account.id}: ${String(err)}`);
                    }
                    if (!accessToken) {
                        ...
                        if (accessTokenFailureIsAuth(resolveOutcome, resolveError)) {
                            await pool.markNeedsReauth(account.id, "No valid credential / token refresh failed", { providerId });
                        }
                        continue;
                    }
                ...
                const source = providerId === "zai-coding" || providerId === "kimi-coding"
                    ? "coding-plan-key"
                    : isSubscriptionProvider(providerId)
                        ? "oauth"
                        : "api-key";
                logger.info(`[coding-account-bridge] ${agentType} → ${providerId} account "${account.label}" (${account.id}) via ${strategy}`);
                return {
                    providerId,
        ...
        markRateLimited(providerId: LinkedAccountProviderId, accountId, untilMs, detail) {
            return pool.markRateLimited(accountId, untilMs, detail, { providerId });
        },
        async markNeedsReauth(providerId: LinkedAccountProviderId, accountId, detail) {
                ...
                if (tokenOutcome.ok) {
                    ...
                    if (isSubscriptionProvider(providerId)) {
                        ...
                        if (!probe.ok) {
                            ...
                            logger.info(`[coding-account-bridge] ${providerId}/${accountId} auth-failure verify was inconclusive (probe status ${probe.status}${probe.error ? `: ${probe.error}` : ""}) — leaving rotation state to the keep-alive sweep`);
                            return;
                        }
                    }
                    logger.info(`[coding-account-bridge] ${providerId}/${accountId} reported an auth failure but its credential verifies — keeping it in rotation (injected token likely expired mid-session)${detail ? `: ${detail}` : ""}`);
                    return;
                }
                if (tokenOutcome.kind !== "auth") {
                    logger.info(`[coding-account-bridge] ${providerId}/${accountId} auth-failure verify did not produce a reauth failure (${tokenOutcome.kind}) — leaving rotation state to the keep-alive sweep`);
                    return;
                }
            }
            catch (err) {
                if (!isAuthFailure(err)) {
                    logger.info(`[coding-account-bridge] ${providerId}/${accountId} auth-failure verify hit a transient error (${String(err)}) — leaving rotation state to the keep-alive sweep`);
                    return;
                }
            }
            return pool.markNeedsReauth(accountId, detail, { providerId });
