...
import { isAuthFailure } from "./coding-account-bridge.js";
...
import { type AccountPoolBrokerAccountSnapshot } from "@elizaos/core";
import { type AccountPoolBrokerFailoverSnapshot } from "@elizaos/core";
import { type AccountPoolBrokerLastReportedStatus } from "@elizaos/core";
import { type AccountPoolBrokerProviderSnapshot } from "@elizaos/core";
...
import { type PoolProviderId } from "./account-pool.js";
...
export function parseBrokerLeaseRequest(body: Record<string, unknown>): AccountPoolBrokerLeaseRequest | null {
    ...
    if (body.exclude !== undefined && exclude === undefined)
        return null;
    return {
        providerId,
...
export function parseBrokerReportRequest(body: Record<string, unknown>): AccountPoolBrokerReportRequest | null {
...
export function parseBrokerReleaseRequest(body: Record<string, unknown>): AccountPoolBrokerReleaseRequest | null {
...
function observabilityAccountKey(providerId: PoolProviderId, accountId: string): string {
    return `${providerId}:${accountId}`;
}
function pendingFailoverKey(providerId: PoolProviderId, sessionKeyHash: string): string {
    return `${providerId}:${sessionKeyHash}`;
}
function retryUntilMs(now: number, retryAfterMs: number | undefined): number | null {
...
function reportIsAuthFailure(report: AccountPoolBrokerReportRequest): boolean {
    if (report.httpStatus === 401 || report.httpStatus === 403)
        return true;
    return report.errorCode ? isAuthFailure(report.errorCode) : false;
}
function reportErrorCodeMatches(report: AccountPoolBrokerReportRequest, pattern: RegExp): boolean {
...
function reportIsTransient(report: AccountPoolBrokerReportRequest): boolean {
    if (typeof report.httpStatus === "number" &&
        report.httpStatus >= 500 &&
        report.httpStatus <= 599) {
        return true;
    }
    return reportErrorCodeMatches(report, /\b(timeout|timed.?out|overload|unavailable|reset|network)\b/i);
}
function normalizeReportCause(report: AccountPoolBrokerReportRequest): AccountPoolBrokerFailoverSnapshot["cause"] | null {
    ...
    if (reportIsTransient(report)) {
        ...
        return {
            category: "transient",
            reason: typeof status === "number" && status >= 500 && status <= 599
                ? "http_5xx"
...
function lastReportedStatusFromReport(report: AccountPoolBrokerReportRequest, atMs: number): AccountPoolBrokerLastReportedStatus {
...
export async function resolveBrokerAccessToken(providerId: PoolProviderId, accountId: string): Promise<{
    accessToken: string;
    accessExpiresAt: number;
}> {
    if (providerId === "openai-codex") {
        try {
            ...
            logger.warn(`[AccountPoolBroker] Codex token adoption failed for account ${accountId}: ${String(err)}`);
        }
    }
    const opts = providerId === "anthropic-subscription"
        ? {
...
export class AccountPoolBroker {
    ...
    private readonly accountObservability = new Map<string, AccountObservabilityState>();
    private readonly lastSelectionByProvider = new Map<PoolProviderId, NonNullable<AccountPoolBrokerProviderSnapshot["lastSelection"]>>();
    private readonly recentFailoversByProvider = new Map<PoolProviderId, AccountPoolBrokerFailoverSnapshot[]>();
    private readonly pendingFailoversBySession = new Map<string, PendingFailoverSignal>();
    constructor(deps: AccountPoolBrokerDeps = {}) {
        ...
        this.leaseTtlMs = normalizeLeaseTtlMs(deps.leaseTtlMs ?? readLeaseTtlFromEnv());
    }
    async lease(request: AccountPoolBrokerLeaseRequest): Promise<AccountPoolBrokerLeaseResponse | null> {
        this.pruneExpired();
        const now = this.now();
    ...
    async report(report: AccountPoolBrokerReportRequest): Promise<{
        ...
        const observability = this.ensureAccountObservability(lease.providerId, lease.accountId);
        if (report.model) {
        ...
        const successPredatesFailure = report.ok &&
            observability.lastFailureAtMs !== null &&
            lease.atMs <= observability.lastFailureAtMs;
        if (!successPredatesFailure) {
            observability.lastReportedStatus = lastReportedStatusFromReport(report, reportAt);
        }
        if (!report.ok)
            observability.lastFailureAtMs = reportAt;
        await this.pool.recordCall(lease.accountId, {
            ok: report.ok,
        ...
        if (report.ok) {
            const pendingKey = pendingFailoverKey(lease.providerId, lease.sessionKeyHash);
        ...
        const failoverCause = normalizeReportCause(report);
        if (failoverCause) {
            this.pendingFailoversBySession.set(pendingFailoverKey(lease.providerId, lease.sessionKeyHash), {
                providerId: lease.providerId,
        ...
        if (reportIsRateLimit(report)) {
            ...
            if (untilMs === null) {
                await this.pool.markRateLimitedUnknown(lease.accountId, report.errorCode ?? "rate_limited", { providerId: lease.providerId });
            }
            else {
                await this.pool.markRateLimited(lease.accountId, untilMs, report.errorCode ?? "rate_limited", { providerId: lease.providerId });
            }
            this.deleteLease(lease);
            return { ok: true };
        }
        if (reportIsAuthFailure(report)) {
            await this.pool.markNeedsReauth(lease.accountId, report.errorCode ?? "auth_failed", { providerId: lease.providerId });
            this.deleteLease(lease);
            return { ok: true };
        }
        if (reportIsTransient(report)) {
    ...
    health(): {
        ...
        const providers = new Set<PoolProviderId>(this.pool.list().map((account) => account.providerId));
        return {
            ok: true,
            enabled: true,
            activeLeases: this.byLeaseId.size,
            accounts: Object.fromEntries(Object.entries(snapshot.accounts).map(([accountId, account]) => [
                accountId,
                {
            ...
            providers: [...providers].sort().map((providerId) => {
                ...
                return {
                    ...
                    selectable: accounts.filter((account) => account.enabled &&
                        (account.health === "ok" ||
                            (account.health === "rate-limited" &&
                                typeof account.healthDetail?.until === "number" &&
                                account.healthDetail.until < now))).length,
                };
            }),
        };
    }
    ...
    private observeLease(lease: LeaseEntry, reason: string | undefined): void {
        const state = this.ensureAccountObservability(lease.providerId, lease.accountId);
        state.lastLease = {
        ...
        this.recentFailoversByProvider.set(lease.providerId, next.slice(-MAX_RECENT_FAILOVERS));
    }
    private ensureAccountObservability(providerId: PoolProviderId, accountId: string): AccountObservabilityState {
    ...
    private resolveSessionPin(sessionKey: string): LeaseEntry | null {
        ...
        if (!lease || lease.expiresAt <= this.now()) {
            if (lease)
                ...
                this.bySessionKey.delete(sessionKey);
            return null;
        }
        return lease;
    }
