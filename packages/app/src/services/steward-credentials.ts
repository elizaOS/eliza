...
import { type SecureStoreSecretKind } from "../security/platform-secure-store";
...
type StewardCredentialsMetadata = Omit<PersistedStewardCredentials, StewardCredentialSecretField> & Partial<Pick<PersistedStewardCredentials, "apiUrl" | "tenantId" | "agentId">>;
...
function createStewardSecureStore(options: StewardCredentialPersistenceOptions = {}): PlatformSecureStore {
    return options.secureStore ?? createNodePlatformSecureStore();
}
function readCredentialsFile(): (Partial<PersistedStewardCredentials> & StewardCredentialsMetadata) | null {
    const credPath = resolveCredentialsPath();
    try {
        if (!fs.existsSync(credPath)) {
            return null;
        }
        return JSON.parse(fs.readFileSync(credPath, "utf-8")) as Partial<PersistedStewardCredentials> & StewardCredentialsMetadata;
    }
    catch {
...
function writeCredentialsMetadata(credentials: PersistedStewardCredentials | StewardCredentialsMetadata): void {
    const credPath = resolveCredentialsPath();
    ...
    if (credentials.apiUrl)
        data.apiUrl = credentials.apiUrl;
    if (credentials.tenantId)
        data.tenantId = credentials.tenantId;
    if (credentials.agentId)
        data.agentId = credentials.agentId;
    ...
    try {
        descriptor = fs.openSync(tmp, fs.constants.O_WRONLY |
...
async function readStewardSecret(store: PlatformSecureStore, vaultId: string, field: StewardCredentialSecretField): Promise<string | null> {
    const got = await store.get(vaultId, STEWARD_SECRET_KINDS[field]);
    return got.ok && got.value.trim() ? got.value.trim() : null;
}
async function writeStewardSecret(store: PlatformSecureStore, vaultId: string, field: StewardCredentialSecretField, value: string, clearEmpty = false): Promise<void> {
    ...
    if (!trimmed) {
        ...
        if (!removed.ok) {
            throw new Error(`secure store rejected clearing ${field}: ${removed.message ?? removed.reason}`);
        }
    ...
    if (!result.ok) {
        throw new Error(`secure store rejected ${field}: ${result.message ?? result.reason}`);
    }
    const verified = await store.get(vaultId, STEWARD_SECRET_KINDS[field]);
    if (!verified.ok || verified.value.trim() !== trimmed) {
        throw new Error(`secure store could not verify ${field}; plaintext credentials were retained for recovery`);
    }
}
async function snapshotStewardSecrets(store: PlatformSecureStore, vaultId: string): Promise<Map<StewardCredentialSecretField, string | null>> {
    const snapshot = new Map<StewardCredentialSecretField, string | null>();
    for (const field of Object.keys(STEWARD_SECRET_KINDS) as StewardCredentialSecretField[]) {
...
async function restoreStewardSecrets(store: PlatformSecureStore, vaultId: string, snapshot: ReadonlyMap<StewardCredentialSecretField, string | null>, attempted: readonly StewardCredentialSecretField[]): Promise<void> {
    for (const field of [...attempted].reverse()) {
        const previous = snapshot.get(field) ?? null;
        if (previous === null) {
            const removed = await store.delete(vaultId, STEWARD_SECRET_KINDS[field]);
            if (!removed.ok)
                throw new Error(`rollback could not clear ${field}`);
            const verified = await store.get(vaultId, STEWARD_SECRET_KINDS[field]);
            if (verified.ok || verified.reason !== "not_found") {
        ...
        const restored = await store.set(vaultId, STEWARD_SECRET_KINDS[field], previous);
        if (!restored.ok)
            throw new Error(`rollback could not restore ${field}`);
        const verified = await store.get(vaultId, STEWARD_SECRET_KINDS[field]);
...
async function migrateLegacyFileSecrets(store: PlatformSecureStore, vaultId: string, parsed: Partial<PersistedStewardCredentials> & StewardCredentialsMetadata): Promise<void> {
 ...
 */
export async function loadStewardCredentials(options: StewardCredentialPersistenceOptions = {}): Promise<PersistedStewardCredentials | null> {
    const parsed = readCredentialsFile();
    if (!parsed)
        return null;
    const store = createStewardSecureStore(options);
    const hasLegacySecrets = (Object.keys(STEWARD_SECRET_KINDS) as StewardCredentialSecretField[]).some((field) => {
        const value = parsed[field];
    ...
    if (await store.isAvailable()) {
        ...
        const secureValues: Partial<Pick<PersistedStewardCredentials, StewardCredentialSecretField>> = {};
        for (const field of Object.keys(STEWARD_SECRET_KINDS) as StewardCredentialSecretField[]) {
    ...
    if (hasLegacySecrets) {
        throw new Error("platform secure store is unavailable; plaintext Steward credentials were retained for recovery");
    }
 ...
 */
export async function saveStewardCredentials(credentials: PersistedStewardCredentials, options: StewardCredentialPersistenceOptions = {}): Promise<void> {
    const store = createStewardSecureStore(options);
    if (!(await store.isAvailable())) {
        throw new Error("platform secure store is unavailable; Steward credentials were not persisted");
    }
    const vaultId = deriveStewardVaultId();
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Steward credential save failed and rollback was incomplete: ${rollbackCause instanceof Error ? rollbackCause.message : String(rollbackCause)}`, { cause });
        }
        // error-policy:J2 preserve the original secure-store or metadata failure.
        throw new Error("Steward credential save failed; prior values were restored", {
            cause,
        });
    }
}
 ...
 */
export async function resolveEffectiveStewardConfig(env: NodeJS.ProcessEnv = process.env, options: StewardCredentialPersistenceOptions = {}): Promise<PersistedStewardCredentials | null> {
    ...
    const agentId = env.STEWARD_AGENT_ID?.trim() ||
        env.ELIZA_STEWARD_AGENT_ID?.trim() ||
        persisted?.agentId ||
        null;
    const apiKey = env.STEWARD_API_KEY?.trim() || persisted?.apiKey || "";
    const agentToken = env.STEWARD_AGENT_TOKEN?.trim() || persisted?.agentToken || "";
    return {
        apiUrl,
        tenantId: tenantId || "",
        agentId: agentId || "",
