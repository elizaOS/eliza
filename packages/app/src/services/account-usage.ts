...
function utilizationToPct(value: unknown, scaleFractional = true): number | undefined {
...
function invalidAnthropicUsageShape(field: string): never {
    throw new ElizaError(`Anthropic usage response field "${field}" was invalid`, {
...
function parseNullableRecord(value: unknown, field: string): Record<string, unknown> | null | undefined {
    if (value === null)
        return null;
    return parseOptionalRecord(value, field);
}
function parseAnthropicUsageWindow(value: unknown, field: string): AnthropicUsageWindow | undefined {
...
function parseAnthropicUsageLimits(value: unknown): AnthropicUsageLimit[] | undefined {
    if (value === undefined || value === null)
        return undefined;
    if (!Array.isArray(value))
        return invalidAnthropicUsageShape("limits");
    return value.map((entry, index) => {
        if (!isRecord(entry))
            return invalidAnthropicUsageShape(`limits[${index}]`);
        const scope = parseNullableRecord(entry.scope, `limits[${index}].scope`);
        const model = parseOptionalRecord(scope?.model, `limits[${index}].scope.model`);
        return {
            kind: entry.kind,
            group: entry.group,
...
function parseAnthropicUsagePayload(value: unknown): AnthropicUsagePayload {
    if (!isRecord(value))
        return invalidAnthropicUsageShape("root");
    return {
 ...
 */
export async function pollAnthropicUsage(accessToken: string, fetchImpl: FetchLike = fetch): Promise<UsageSnapshot> {
    ...
    if (Array.isArray(payload.limits)) {
        for (const limit of payload.limits) {
            ...
            const modelName = typeof displayName === "string" ? displayName.trim() : undefined;
            if (modelName) {
 ...
 */
export async function pollCodexUsage(accessToken: string, accountId: string, fetchImpl: FetchLike = fetch): Promise<UsageSnapshot> {
...
function counterFile(providerId: string, accountId: string, ts: number = Date.now()): string {
    return path.join(resolveStateDir(), "usage", providerId, accountId, `${dayStamp(ts)}.jsonl`);
}
/**
 ...
 */
export function recordCall(providerId: string, accountId: string, entry: Omit<UsageEntry, "ts">): void {
    const ts = Date.now();
 ...
 */
export function readTodayCounters(providerId: string, accountId: string): DailyCounters {
