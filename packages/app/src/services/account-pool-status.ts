...
import { getAccountPoolConsumerUsageSummary } from "./account-pool-consumer-metering.js";
import { getDefaultAccountPool } from "./account-pool.js";
import { isAccountSelectableNow } from "./account-pool.js";
...
import { selectionForProvider } from "./account-pool.js";
import { type AccountPool } from "./account-pool.js";
import { type AccountPoolConsumerUsageBreakdown } from "./account-pool-consumer-metering.js";
import { type AccountPoolConsumerUsageTotals } from "./account-pool-consumer-metering.js";
import { type LinkedAccountConfig } from "@elizaos/core/contracts/service-routing";
import { type queryAccountPoolConsumerUsage } from "./account-pool-consumer-metering.js";
...
function atomicWriteText(filePath: string, value: string): void {
    ensureDir(filePath);
    const tmp = path.join(path.dirname(filePath), `.${path.basename(filePath)}.${process.pid}.${randomBytes(8).toString("hex")}.tmp`);
        ...
        try {
            ...
            snapshots.push({
                ...
                accounts: parsed.accounts
                    ...
                    weeklyResetAt: typeof account.weeklyResetAt === "number"
            ...
            logger.warn(`[account-pool-status] skipping invalid snapshot line: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return snapshots;
}
...
function getModelBuckets(usage: UsageCompat): InternalPoolModelBuckets {
    ...
    for (const [key, value] of Object.entries(source)) {
        ...
        if (/fable/i.test(key))
            fable = bucket;
        if (/sonnet/i.test(key))
            sonnet = bucket;
    }
    if (fable || sonnet)
        return { available: true, fable, sonnet };
    return {
        available: false,
        note: "per-model buckets pending account usage support",
    };
}
function calculateBurn(row: InternalPoolStatusAccount, snapshots: readonly StatusSnapshot[], now: number): BurnEstimate {
    ...
    for (const snapshot of snapshots) {
        const old = snapshot.accounts.find((account) => account.snapshotKey === row.snapshotKey);
        if (!old || snapshot.ts >= now)
            continue;
        if (old.weeklyResetAt === null ||
...
function applyUrgency(status: InternalPoolStatus, snapshots: readonly StatusSnapshot[]): void {
    const now = nowMs();
    let totalRate = 0;
    let knownRates = 0;
    for (const row of status.perAccount) {
        ...
        const exhaustMs = row.fableUsedPct !== null && rate !== null && rate > 0
    ...
    const poolMs = totalRate > 0 ? (status.fable.leftPct / totalRate) * 3600000 : null;
    status.urgency = {
        burnRatePctPerHour: knownRates > 0 ? round2(totalRate) : null,
        projectedDepletionIn: durationText(poolMs),
        depletionMessage: knownRates === 0
            ? "pool-wide burn: estimating"
            : totalRate === 0
                ? "pool-wide burn is currently flat"
                : `pool-wide capacity at current burn: ${durationText(poolMs)}`,
        nextRefill: next && next.weeklyResetAt !== null
            ? {
                account: next.name,
                in: durationText(next.weeklyResetAt - now) ?? "unknown",
                capacityAddedPct: round2(next.fableUsedPct ?? next.weeklyUsedPct ?? 0),
            }
            : null,
    };
}
function consumerTotals(totals: AccountPoolConsumerUsageTotals["totals"]): PublicPoolConsumerTotals {
    return {
...
function publicEdgeUsage(usage: AccountPoolConsumerUsageBreakdown, now: number): PublicPoolStatus["publicEdge"] {
    const today = usage.byDay[currentDayStamp(now)] ??
        ({
            requests: 0,
            tokens: 0,
            input_tokens: 0,
...
function buildStatus(pool: AccountPool, consumerUsage: AccountPoolConsumerUsageBreakdown, snapshots: readonly StatusSnapshot[]): InternalPoolStatus {
    const now = nowMs();
    ...
    const accounts = pool
        ...
        .filter((account) => configuredIds === null || configuredIds.has(account.id))
        .filter((account) => isAccountSelectableNow(account, now));
    if (accounts.length === 0) {
        throw new Error("no selectable public pool accounts");
    }
    const selection = pool.selectionState(PROVIDER_ID, configured.strategy ?? "drain-soonest-reset", configured.accountIds ? { accountIds: configured.accountIds } : undefined);
    let fableLeft = 0;
    let allLeft = 0;
    ...
    const rows: InternalPoolStatusAccount[] = accounts.map((account, index) => {
        ...
        const fableUsed = modelBuckets.available && modelBuckets.fable
        ...
        const fallbackWeeklyResetAt = typeof usage.resetsAt === "number"
            ? usage.resetsAt
            : typeof usage.weeklyResetsAt === "number"
                ? usage.weeklyResetsAt
                : null;
        const weeklyResetAt = modelBuckets.available && modelBuckets.fable
            ? (modelBuckets.fable.resetAt ?? fallbackWeeklyResetAt)
            : fallbackWeeklyResetAt;
        const exhausted = fableUsed !== null && fableUsed >= 100;
        const serving = selection.activeAccountId === account.id &&
        ...
        return {
            ...
            sessionResetAt: typeof usage.sessionResetsAt === "number"
    ...
    const status: InternalPoolStatus = {
        ...
        pool: {
            ...
            selectableAccounts: accounts.filter((account) => isAccountSelectableNow(account, now)).length,
        },
        fable: {
...
function withHealth(status: InternalPoolStatus, stale: boolean): InternalPoolStatus {
    const now = nowMs();
...
function publicModelBuckets(buckets: InternalPoolModelBuckets): PublicPoolModelBuckets {
...
export function serializePublicPoolStatus(status: InternalPoolStatus): PublicPoolStatus {
    return {
...
export async function getPublicAccountPoolStatus(): Promise<PublicPoolStatus> {
    ...
    if (!inflight) {
        inflight = (async () => {
            ...
            const usage = await (depsOverride.queryConsumerUsage ?? getAccountPoolConsumerUsageSummary)();
            ...
            .catch((error) => {
            logger.warn(`[AccountPoolStatus] status refresh failed: ${error instanceof Error ? error.message : String(error)}`);
            if (cache)
                return withHealth(cache.status, true);
            throw error;
        })
            .finally(() => {
            inflight = null;
...
export function __resetAccountPoolStatusForTests(deps: AccountPoolStatusDeps = {}): void {
    depsOverride = deps;
    cache = null;
    inflight = null;
}
