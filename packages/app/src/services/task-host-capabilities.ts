 ...
 * on every fire.
 */
export function getHostExecutionCapabilities(runtime: IAgentRuntime): ReadonlySet<TaskExecutionProfile> {
    ...
    const isCapacitor = typeof capacitor === "object" &&
        capacitor !== null &&
    ...
    const hasBackgroundRunner = plugins != null &&
    ...
    const hasElizaTasks = plugins != null &&
        typeof plugins === "object" &&
...
export function describeHostExecutionCapabilities(runtime: IAgentRuntime): {
    ...
    const isCapacitor = typeof capacitor === "object" &&
        ...
        typeof (capacitor as CapacitorGlobalLike).isNativePlatform === "function" &&
        (capacitor as CapacitorGlobalLike).isNativePlatform?.() === true;
    const plugins = isCapacitor && capacitor != null
        ? (capacitor as CapacitorGlobalLike).Plugins
        : undefined;
    const hasBackgroundRunner = plugins != null &&
        typeof plugins === "object" &&
        plugins.BackgroundRunner != null;
    const hasElizaTasksPlugin = plugins != null &&
        typeof plugins === "object" &&
        plugins.ElizaTasks != null;
    const getSetting = (runtime as {
        getSetting?: (k: string) => unknown;
