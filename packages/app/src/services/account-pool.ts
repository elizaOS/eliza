 ...
 */
import { ACCOUNT_CREDENTIAL_PROVIDER_IDS } from "@elizaos/auth/auth/types";
import { DIRECT_ACCOUNT_PROVIDER_ENV } from "@elizaos/auth/auth/types";
import { DIRECT_ACCOUNT_PROVIDER_IDS } from "@elizaos/auth/auth/types";
import { ElizaError } from "@elizaos/core";
import { OPENAI_COMPAT_BASE_BY_DIRECT_PROVIDER } from "@elizaos/auth/auth/types";
import { adoptRotatedCodexTokens } from "./coding-account-bridge.js";
...
import { getAccessToken as getAccountAccessToken } from "@elizaos/auth/auth/credentials";
...
import { isSubscriptionProvider } from "@elizaos/auth/auth/types";
import { listProviderAccounts } from "@elizaos/auth/auth/credentials";
import { logger } from "@elizaos/core";
import { openSync } from "node:fs";
import { pollAnthropicUsage } from "./account-usage.js";
import { pollCodexUsage } from "./account-usage.js";
import { randomUUID } from "node:crypto";
import { readFileSync } from "node:fs";
import { recordCall as recordUsageEntry } from "./account-usage.js";
import { renameSync } from "node:fs";
import { resolveStateDir } from "@elizaos/core";
import { rmSync } from "node:fs";
import { setAnthropicAccountPoolBridge } from "@elizaos/core";
import { type AccountCredentialRecord } from "@elizaos/auth/auth/account-storage";
import { type AccountStoragePolicy } from "@elizaos/auth/auth/account-storage";
...
import { type LinkedAccountHealthDetail } from "@elizaos/core/contracts/service-routing";
...
import { type LinkedAccountsConfig } from "@elizaos/core/contracts/service-routing";
import { withAccountStorageMutation } from "@elizaos/auth/auth/account-storage";
import { writeFileSync } from "node:fs";
import path from "node:path";
export type Strategy = "priority" | "round-robin" | "least-used" | "quota-aware" | "reset-soonest" | "drain-soonest-reset";
...
function accountWeeklyBucket(account: LinkedAccountConfig, requestedModel?: string): {
    pct: number;
    resetsAt?: number;
} {
    ...
    if (key && buckets) {
        for (const [name, bucket] of Object.entries(buckets)) {
            if (modelKeysMatch(key, normalizeModelKey(name)))
                return bucket;
        }
    }
    return {
 ...
 */
function bySoonestReset(a: LinkedAccountConfig, b: LinkedAccountConfig): number {
...
function subscriptionEndBoost(account: LinkedAccountConfig, now: number): number {
    if (typeof account.subscriptionEndsAt !== "number" ||
...
function isAccountExpired(account: LinkedAccountConfig, now: number = Date.now()): boolean {
    return (typeof account.subscriptionEndsAt === "number" &&
        account.subscriptionEndsAt <= now);
}
...
export class AccountPool {
     ...
     */
    selectionState(providerId: PoolProviderId, strategy: Strategy = "priority", opts?: {
        model?: string;
        accountIds?: string[];
    }): {
        ...
        return this.selectionStateFromAccounts(this.deps.readAccounts(), providerId, strategy, opts);
    }
    ...
    readSnapshot(): {
        ...
        selectionState(providerId: PoolProviderId, strategy?: Strategy): {
            activeAccountId: string | null;
            reason: string | null;
        };
    } {
        const all = this.deps.readAccounts();
        return {
            list: (providerId?: PoolProviderId): LinkedAccountConfig[] => Object.values(all).filter((account) => !providerId || account.providerId === providerId),
            selectionState: (providerId: PoolProviderId, strategy: Strategy = "priority") => this.selectionStateFromAccounts(all, providerId, strategy),
        };
    }
    private selectionStateFromAccounts(all: Record<string, LinkedAccountConfig>, providerId: PoolProviderId, strategy: Strategy, opts?: {
        model?: string;
        accountIds?: string[];
    }): {
        activeAccountId: string | null;
        ...
        if (eligible.length === 0)
            return { activeAccountId: null, reason: null };
    ...
    private async markExpiredAccounts(providerId: PoolProviderId | undefined, all: Record<string, LinkedAccountConfig>): Promise<number> {
        const now = Date.now();
        let changed = 0;
        for (const account of Object.values(all)) {
            ...
            changed += 1;
            logger.warn(`[AccountPool] account expired providerId=${account.providerId} accountId=${account.id} subscriptionEndsAt=${account.subscriptionEndsAt}`);
    ...
    private filterEligible(all: Record<string, LinkedAccountConfig>, input: SelectInput): LinkedAccountConfig[] {
    ...
    private applyStrategy(strategy: Strategy, eligible: LinkedAccountConfig[], providerId: PoolProviderId, opts: {
        model?: string;
        ...
        switch (strategy) {
            ...
            case "least-used": {
                return ([...eligible].sort((a, b) => this.byLeastUsedEffective(a, b))[0] ??
                    null);
            }
            case "quota-aware": {
                const underQuota = eligible.filter((a) => accountSessionPct(a) < QUOTA_AWARE_SKIP_PCT);
            ...
            case "drain-soonest-reset": {
                ...
                const underSessionCap = eligible.filter((a) => accountSessionPct(a) < QUOTA_AWARE_SKIP_PCT);
                const pool = underSessionCap.length > 0 ? underSessionCap : eligible;
                return ([...pool].sort((a, b) => byDrainSoonestReset(a, b, opts.model))[0] ??
                    null);
            }
            default:
    ...
    private effectiveLastUsed(account: LinkedAccountConfig): number {
        ...
        return Math.max(accountLastUsedAt(account), recentSelection === undefined ? 0 : recentSelection);
    }
    ...
    async upsert(account: LinkedAccountConfig): Promise<void> {
        ...
        await this.deps.writeAccount({
            ...account,
            prioritySource: account.prioritySource ?? prior?.prioritySource ?? "generated",
        });
    }
    async deleteMetadata(providerId: PoolProviderId, accountId: string): Promise<void> {
        if (!this.deps.deleteAccount)
            return;
        await this.deps.deleteAccount(providerId, accountId);
    }
    // Mutations.
    async recordCall(accountId: string, result: {
        tokens?: number;
        latencyMs?: number;
        ok: boolean;
        errorCode?: string;
    ...
    async refreshUsage(accountId: string, accessToken: string, opts?: {
        ...
        if (account.providerId === "anthropic-subscription") {
            ...
            if (!account.email) {
                try {
                    ...
                    logger.warn(`[AccountPool] Anthropic profile backfill failed for account ${accountId}: ${String(err)}`);
                }
            }
        }
        ...
        const fresh = findAccountById(this.deps.readAccounts(), accountId, opts?.providerId);
        if (!fresh)
            return;
    ...
    async markRateLimited(accountId: string, untilMs: number, detail?: string, opts?: {
        providerId?: PoolProviderId;
    }): Promise<void> {
        const account = findAccountById(this.deps.readAccounts(), accountId, opts?.providerId);
        if (!account)
        ...
        const providerResetMs = account.providerId === "openai-codex"
            ? account.usage?.resetsAt
            : undefined;
        const heuristicUntil = Number.isFinite(untilMs) && untilMs > Date.now()
            ? untilMs
            : Date.now() + DEFAULT_RATE_LIMIT_BACKOFF_MS;
        const healthDetail: LinkedAccountHealthDetail = {
            until: typeof providerResetMs === "number" && providerResetMs > Date.now()
                ? providerResetMs
                : heuristicUntil,
    ...
    async markRateLimitedUnknown(accountId: string, detail?: string, opts?: {
        providerId?: PoolProviderId;
    }): Promise<void> {
        const account = findAccountById(this.deps.readAccounts(), accountId, opts?.providerId);
        if (!account)
    ...
    async markNeedsReauth(accountId: string, detail?: string, opts?: {
        providerId?: PoolProviderId;
    }): Promise<void> {
        const account = findAccountById(this.deps.readAccounts(), accountId, opts?.providerId);
        if (!account)
        ...
        if (account.health !== "needs-reauth") {
            logger.warn(`[account-pool] ${account.providerId} account "${account.label ?? account.id}" (${account.id}) → needs-reauth: ${detail ?? "no detail provided"}`);
        }
        await this.deps.writeAccount({
            ...account,
            health: "needs-reauth",
    ...
    async markInvalid(accountId: string, detail?: string, opts?: {
        providerId?: PoolProviderId;
    }): Promise<void> {
        const account = findAccountById(this.deps.readAccounts(), accountId, opts?.providerId);
        if (!account)
    ...
    async markHealthy(accountId: string, opts?: {
        providerId?: PoolProviderId;
    }): Promise<void> {
        const account = findAccountById(this.deps.readAccounts(), accountId, opts?.providerId);
        if (!account)
    ...
    async markUsagePrimed(accountId: string, lastPrimedAt: number = Date.now(), opts?: {
        providerId?: PoolProviderId;
    }): Promise<void> {
        const account = findAccountById(this.deps.readAccounts(), accountId, opts?.providerId);
        if (!account)
 ...
 */
export function isAccountSelectableNow(account: LinkedAccountConfig, now: number = Date.now()): boolean {
    if (isAccountExpired(account, now))
        return false;
    if (account.health === "ok")
        return true;
    return (account.health === "rate-limited" &&
...
function findAccountById(all: Record<string, LinkedAccountConfig>, accountId: string, providerId?: PoolProviderId): LinkedAccountConfig | null {
    if (providerId) {
        const scoped = all[poolRecordKey(providerId, accountId)];
        if (scoped)
            return scoped;
        return (Object.values(all).find((account) => account.id === accountId && account.providerId === providerId) ?? null);
    }
...
function byExplicitPriority(a: LinkedAccountConfig, b: LinkedAccountConfig): number {
...
function byDrainSoonestReset(a: LinkedAccountConfig, b: LinkedAccountConfig, requestedModel?: string): number {
    ...
    if (aBucket.pct !== bBucket.pct)
        return aBucket.pct - bBucket.pct;
    const now = Date.now();
...
function _byLeastUsedThenPriority(a: LinkedAccountConfig, b: LinkedAccountConfig): number {
    const aPct = accountSessionPct(a);
    const bPct = accountSessionPct(b);
    if (aPct !== bPct)
        return aPct - bPct;
...
function writeMetaStore(store: PoolMetaStore, storagePolicy: AccountStoragePolicy): void {
    const file = metadataFile(storagePolicy);
    const dir = path.dirname(file);
    const tmp = `${file}.${process.pid}.${randomUUID()}.tmp`;
    let descriptor: number | undefined;
    try {
        descriptor = openSync(tmp, constants.O_WRONLY |
            constants.O_CREAT |
            constants.O_EXCL |
...
function recordToLinked(record: AccountCredentialRecord, meta: PoolMetaFields | undefined, providerId: PoolProviderId, defaultPriority: number): LinkedAccountConfig {
    return {
        id: record.id,
        providerId,
        ...
        prioritySource: meta?.prioritySource ??
...
function loadAllAccounts(storagePolicy: AccountStoragePolicy): Record<string, LinkedAccountConfig> {
...
async function persistAccount(account: LinkedAccountConfig, storagePolicy: AccountStoragePolicy): Promise<void> {
    if (!isLinkedAccountProviderId(account.providerId))
        return;
    withAccountStorageMutation(storagePolicy, "persist-account-pool-metadata", () => {
...
async function deleteAccountMeta(providerId: PoolProviderId, accountId: string, storagePolicy: AccountStoragePolicy): Promise<void> {
    withAccountStorageMutation(storagePolicy, "delete-account-pool-metadata", () => {
        const store = readMetaStore(storagePolicy);
        const bucket = store[providerId];
        if (!bucket)
            return;
        if (!(accountId in bucket))
            return;
...
function normalizeAccountIdsFromRoute(route: AccountPoolSelectionRoute | undefined): string[] | undefined {
    if (!route)
        return undefined;
    const fromList = Array.isArray(route.accountIds)
        ? route.accountIds
            ...
            .filter(Boolean)
        : [];
    const single = typeof route.accountId === "string" && route.accountId.trim()
        ? [route.accountId.trim()]
        : [];
    const ids = fromList.length > 0 ? fromList : single;
    return ids.length > 0 ? ids : undefined;
}
function routeTargetsProvider(route: AccountPoolSelectionRoute | undefined, providerId: PoolProviderId): boolean {
    if (!route?.backend)
        return false;
    const directProvider = DIRECT_PROVIDER_BY_BACKEND[route.backend];
    if (directProvider === providerId)
        return true;
 ...
 */
export function selectionForProvider(providerId: PoolProviderId, opts: {
    ...
    const routeSelection = routeTargetsProvider(route, providerId)
        ...
        : {};
    return {
        strategy: routeSelection.strategy ??
...
export function configuredAccountStrategyForProvider(providerId: PoolProviderId): Strategy | undefined {
    const route = defaultSelectionConfig.serviceRouting?.llmText;
    return ((routeTargetsProvider(route, providerId)
        ? normalizeStrategy(route?.strategy)
        : undefined) ??
        normalizeStrategy(defaultSelectionConfig.accountStrategies?.[providerId]));
}
export function configureDefaultAccountPoolSelection(config: AccountPoolSelectionConfig = {}): void {
...
function defaultRuntimeStoragePolicy(): AccountStoragePolicy {
    if (!cachedRuntimeStoragePolicy) {
        cachedRuntimeStoragePolicy = createRuntimeAccountStoragePolicy(process.env.ELIZA_HOME || resolveStateDir());
    }
...
export function getDefaultAccountPool(): AccountPool {
    if (!cachedDefaultPool) {
        ...
        cachedDefaultPool = new AccountPool({
            ...
            deleteAccount: (providerId, accountId) => deleteAccountMeta(providerId, accountId, storagePolicy),
        });
...
export interface DirectProviderCredentialExportDeps {
    ...
    listAccounts: (providerId: DirectAccountProvider) => readonly Pick<AccountCredentialRecord, "id">[];
    getToken: (providerId: DirectAccountProvider, accountId: string) => Promise<string | null>;
 ...
 */
export type DirectProviderEnvLedger = Map<DirectAccountProvider, {
    token: string;
 ...
 */
const displacedDirectProviderEnv = new WeakMap<DirectProviderEnvLedger, Map<string, DisplacedEnvValue>>();
const directProviderExportGeneration = new WeakMap<DirectProviderEnvLedger, number>();
function nextDirectProviderExportGeneration(ledger: DirectProviderEnvLedger): number {
    const generation = (directProviderExportGeneration.get(ledger) ?? 0) + 1;
    directProviderExportGeneration.set(ledger, generation);
    return generation;
}
function assignExportedEnv(env: NodeJS.ProcessEnv, ledger: DirectProviderEnvLedger, key: string, value: string): void {
    ...
    if (!displaced.has(key)) {
        ...
        displaced.set(key, previous === undefined
            ? { present: false }
            : { present: true, value: previous });
    }
    env[key] = value;
}
function restoreExportedEnv(env: NodeJS.ProcessEnv, ledger: DirectProviderEnvLedger, key: string, ownedValue: string): void {
    if (env[key] !== ownedValue)
        return;
    const displaced = displacedDirectProviderEnv.get(ledger)?.get(key);
    if (displaced?.present)
        env[key] = displaced.value;
    else
        delete env[key];
}
function retractExportedDirectProviderEnv(providerId: DirectAccountProvider, env: NodeJS.ProcessEnv, ledger: DirectProviderEnvLedger): void {
...
function retractAllExportedDirectProviderEnvInternal(env: NodeJS.ProcessEnv, ledger: DirectProviderEnvLedger): void {
...
export function retractAllExportedDirectProviderEnv(env: NodeJS.ProcessEnv, ledger: DirectProviderEnvLedger): void {
 ...
 */
export async function applyDirectProviderCredentialsToEnv(activeBackend: string | null | undefined, deps: DirectProviderCredentialExportDeps): Promise<void> {
    ...
    if (directProviderExportGeneration.get(ledger) !== generation)
        return;
...
export async function applyAccountPoolApiCredentials(opts: {
    ...
    await applyDirectProviderCredentialsToEnv(opts.activeBackend, {
        ...
        getToken: (providerId, accountId) => getAccountAccessToken(providerId, accountId, {
...
function accountPoolUsagePrimingEnabled(): boolean {
    const value = process.env.ELIZA_ACCOUNT_POOL_USAGE_PRIMING?.trim().toLowerCase();
    return (value !== "0" && value !== "false" && value !== "no" && value !== "off");
}
function accountNeedsUsagePriming(account: LinkedAccountConfig | null, now: number): boolean {
    if (!account?.enabled)
        return false;
    ...
    if (typeof resetsAt === "number")
        return false;
    return (typeof lastPrimedAt !== "number" ||
        lastPrimedAt + USAGE_PRIMING_DEBOUNCE_MS <= now);
}
async function probeAnthropicUsagePriming(accessToken: string, fetchImpl: typeof fetch): Promise<void> {
    ...
    if (!response.ok) {
        ...
        throw new Error(`Anthropic usage priming probe failed: HTTP ${response.status}`);
    }
    await response.body?.cancel();
}
async function primeAnthropicUsageThenRefresh(pool: AccountPool, record: AccountCredentialRecord, token: string, deps: Required<AccountPoolKeepAliveDeps>): Promise<void> {
    ...
    try {
        ...
        // request metadata.
        logger.warn(`[AccountPool] Anthropic usage priming failed for account ${record.id}; usage refresh will continue.`);
    }
...
function resolveKeepAliveDeps(deps: AccountPoolKeepAliveDeps = {}): Required<AccountPoolKeepAliveDeps> {
    const envDelay = Number.parseInt(process.env.ELIZA_ACCOUNT_POOL_USAGE_PRIMING_RETRY_DELAY_MS ?? "", 10);
    const usagePrimingRetryDelayMs = deps.usagePrimingRetryDelayMs ??
...
export async function sweepAccountPoolKeepAlive(deps: AccountPoolKeepAliveDeps = {}): Promise<AccountPoolKeepAliveResult> {
    ...
    for (const providerId of ACCOUNT_CREDENTIAL_PROVIDER_IDS) {
        ...
        for (const record of listProviderAccounts(providerId)) {
            ...
            if ((pooled?.health === "needs-reauth" || pooled?.health === "invalid") &&
            ...
            try {
                if (providerId === "anthropic-subscription" &&
                    ...
                    accountNeedsUsagePriming(pool.get(record.id, "anthropic-subscription"), keepAliveDeps.now())) {
                    await primeAnthropicUsageThenRefresh(pool, record, token, keepAliveDeps);
                }
                else {
                    await pool.refreshUsage(record.id, token, {
                        providerId,
                ...
                if (/\b(?:HTTP\s*)?(?:401|403)\b|unauthoriz/i.test(message)) {
                    ...
                    await pool.markRateLimited(record.id, Date.now() + DEFAULT_RATE_LIMIT_BACKOFF_MS, message, { providerId });
                }
                else {
                    logger.warn(`[AccountPool] usage refresh failed for ${providerId}/${record.id} without proving credential failure: ${message}`);
                }
...
export function startAccountPoolKeepAlive(intervalMs: number = KEEP_ALIVE_INTERVAL_MS): void {
    const disabled = process.env.ELIZA_ACCOUNT_POOL_KEEPALIVE?.trim().toLowerCase();
    if (disabled === "0" ||
        disabled === "false" ||
        disabled === "no" ||
        disabled === "off") {
...
function installAnthropicBridge(pool: AccountPool): void {
    const bridge: AnthropicAccountPoolBridge = {
        ...
        getAccessToken: (providerId, accountId) => getAccountAccessToken(providerId, accountId, {
            storagePolicy: defaultRuntimeStoragePolicy(),
        }),
...
export function resetDefaultAccountPoolAfterCredentialReset(): void {
    ...
    retractAllExportedDirectProviderEnv(process.env, defaultDirectProviderEnvLedger);
    cachedDefaultPool = null;
    cachedRuntimeStoragePolicy = null;
}
export const __resetDefaultAccountPoolForTests = resetDefaultAccountPoolAfterCredentialReset;
