...
import { CREDENTIALS_FILE } from "./steward-sidecar/types";
import { DEFAULT_MAX_RESTARTS } from "./steward-sidecar/types";
import { DEFAULT_PORT } from "./steward-sidecar/types";
import { ElizaError } from "@elizaos/core";
import { INITIAL_BACKOFF_MS } from "./steward-sidecar/types";
import { MAX_BACKOFF_MS } from "./steward-sidecar/types";
...
import { findStewardEntryPoint } from "./steward-sidecar/process-management";
import { fingerprintRandomToken } from "./steward-sidecar/helpers";
import { generateApiKey } from "./steward-sidecar/helpers";
import { loadOrCreateLoginMasterPassword } from "./steward-sidecar/master-password";
import { logger } from "@elizaos/core";
import { pipeOutput } from "./steward-sidecar/process-management";
import { readAliasedEnv } from "@elizaos/core/utils/env";
import { resolveDataDir } from "./steward-sidecar/helpers";
import { type StewardCredentials } from "./steward-sidecar/types";
import { type StewardSidecarConfig } from "./steward-sidecar/types";
import { type StewardSidecarStatus } from "./steward-sidecar/types";
...
export class StewardSidecar {
    private config: Required<Pick<StewardSidecarConfig, "dataDir" | "port" | "maxRestarts">> & StewardSidecarConfig;
    ...
    private terminationPromises = new WeakMap<StewardProcessHandle, Promise<void>>();
    ...
    async start(): Promise<StewardSidecarStatus> {
        ...
        if (this.process) {
            const error = new ElizaError("Cannot start Steward while the previous child has not confirmed exit", {
    ...
    private async startLifecycle(generation: number): Promise<StewardSidecarStatus> {
        this.stopping = false;
        ...
        try {
            ...
            const credentials = await ensureWalletSetup(this.credentials, this.getApiBase(), this.config.masterPassword, this.config.dataDir, (p) => {
                if (this.isLifecycleActive(generation)) {
                    this.updateStatus(p);
                }
            ...
            if (!this.isLifecycleActive(generation)) {
                if (cleanupError) {
                    ...
                    logger.warn({ error: cleanupError, generation }, "[StewardSidecar] Spawned-child cleanup failed after the start lifecycle was superseded");
                }
                return this.status;
            }
            if (cleanupError) {
                const error = new ElizaError("Steward startup failed and spawned-child cleanup did not complete", {
    ...
    private async ensureDataDir(): Promise<void> {
        ...
        const targetHasData = fs.existsSync(path.join(targetDataDir, "PG_VERSION")) ||
    ...
    private async loadOrCreateCredentials(): Promise<void> {
        ...
        if (fs.existsSync(credPath)) {
            try {
                ...
                this.config.masterPassword = loadOrCreateLoginMasterPassword(path.join(this.config.dataDir, "data"), this.credentials.masterPassword || this.config.masterPassword, Boolean(this.credentials.masterPassword || this.config.masterPassword));
                this.credentials.masterPassword = this.config.masterPassword;
                this.updateStatus({
                ...
                throw new ElizaError("Local login credentials could not be loaded. Restore the existing credentials file before restarting.", {
        ...
        this.config.masterPassword = loadOrCreateLoginMasterPassword(path.join(this.config.dataDir, "data"), this.config.masterPassword);
    }
    private async spawnProcess(generation: number): Promise<boolean> {
        ...
        if (!entryPoint) {
            throw new Error("Login API entry point not found. Install the @elizaos/auth workspace or set STEWARD_ENTRY_POINT to its embedded entry.");
        }
        ...
        const masterPw = this.credentials?.masterPassword || this.config.masterPassword;
        if (masterPw) {
        ...
        if (bun) {
            ...
            const child = childProcess.spawn(this.config.stewardEntryPoint ? "node" : "bun", this.config.stewardEntryPoint ? [entryPoint] : ["run", entryPoint], {
                env,
                cwd: path.dirname(entryPoint),
    ...
    private async handleCrash(exitCode: number | null): Promise<void> {
        ...
        const backoff = Math.min(INITIAL_BACKOFF_MS * 2 ** (this.status.restartCount - 1), MAX_BACKOFF_MS);
            ...
            try {
                ...
                if (!this.isLifecycleActive(generation)) {
                    if (cleanupError) {
                        ...
                        logger.warn({ error: cleanupError, generation }, "[StewardSidecar] Spawned-child cleanup failed after the restart lifecycle was superseded");
                    }
                    return;
                }
                if (cleanupError) {
                    const error = new ElizaError("Steward restart failed and spawned-child cleanup did not complete", {
                        code: "STEWARD_RESTART_CLEANUP_FAILED",
                        cause: new AggregateError([err, cleanupError]),
                        context: { generation, pid: spawned?.pid ?? null },
                        severity: "fatal",
                    });
                    logger.error({ error }, "[StewardSidecar] Crash restart cleanup failed");
                    this.updateStatus({ state: "error", error: error.message });
                    return;
                }
                const error = err instanceof Error ? err.message : String(err);
     ...
     */
    private async discardSpawnedProcess(spawned: StewardProcessHandle | null, reason: string): Promise<void> {
    ...
    private observeProcessExit(processHandle: StewardProcessHandle, exitCode: number): void {
        if (this.process !== processHandle) {
            return;
        }
        this.process = null;
        if (!this.stopping && !this.discardedProcesses.has(processHandle)) {
            logger.warn(`[StewardSidecar] Process exited unexpectedly (code ${exitCode})`);
            void this.handleCrash(exitCode);
        }
    }
    private async terminateProcess(processHandle: StewardProcessHandle, reason: string): Promise<void> {
    ...
    private async terminateProcessOnce(processHandle: StewardProcessHandle, reason: string): Promise<void> {
        ...
        errors.push(new Error(`Steward child did not exit within ${PROCESS_TERMINATION_GRACE_MS}ms of SIGTERM`));
        ...
        errors.push(new Error(`Steward child did not exit within ${PROCESS_TERMINATION_GRACE_MS}ms of SIGKILL`));
        throw new ElizaError("Steward child failed to confirm exit after SIGTERM and SIGKILL", {
    ...
    private requestTerminationSignal(processHandle: StewardProcessHandle, signal: "SIGTERM" | "SIGKILL", reason: string, errors: unknown[]): void {
        try {
            const delivered = processHandle.kill(signal);
            if (delivered === false) {
                const error = new Error(`Steward child rejected ${signal}`);
                errors.push(error);
                logger.warn({ error, pid: processHandle.pid ?? null, reason, signal }, "[StewardSidecar] Child termination signal was not delivered");
            }
        }
        catch (error) {
            ...
            errors.push(error);
            logger.warn({ error, pid: processHandle.pid ?? null, reason, signal }, "[StewardSidecar] Child termination signal failed");
        }
    }
    private async waitForProcessExit(processHandle: StewardProcessHandle): Promise<ProcessExitWaitResult> {
        let timeout: ReturnType<typeof setTimeout> | null = null;
        try {
            return await Promise.race([
                processHandle.exited.then((): ProcessExitWaitResult => ({ exited: true }), (error: unknown): ProcessExitWaitResult => ({ exited: false, error })),
                new Promise<ProcessExitWaitResult>((resolve) => {
                    timeout = setTimeout(() => resolve({ exited: false }), PROCESS_TERMINATION_GRACE_MS);
                }),
            ]);
        }
        finally {
 ...
 */
export function createDesktopStewardSidecar(overrides?: Partial<StewardSidecarConfig>): StewardSidecar {
    const home = process.env.HOME || process.env.USERPROFILE || "";
    ...
    return new StewardSidecar({
        dataDir: process.env.STEWARD_DATA_DIR ||
            overrides?.dataDir ||
            path.join(stateHome, namespace, "steward"),
        port: parseInt(process.env.STEWARD_PORT || "", 10) ||
            overrides?.port ||
            DEFAULT_PORT,
        masterPassword: process.env.STEWARD_MASTER_PASSWORD || overrides?.masterPassword,
        stewardEntryPoint: process.env.STEWARD_ENTRY_POINT || overrides?.stewardEntryPoint,
