...
import { type DeliveryResult } from "@elizaos/core";
import { type DispatchSensitiveRequest as SensitiveRequest } from "@elizaos/core";
...
function readSetting(runtime: unknown, key: string): string | undefined {
    const candidate = (runtime as RuntimeWithSettings | null | undefined)?.getSetting?.(key);
...
function defaultResolveCloudBase(runtime: unknown): string | null {
    ...
    if (authoritySnapshot) {
        if (authoritySnapshot.authority === "staging-default" ||
    ...
    if (!apiKey)
        return null;
    const rawBase = readSetting(runtime, "ELIZAOS_CLOUD_BASE_URL") ??
...
function readPaymentAppId(request: SensitiveRequest): string | undefined {
    ...
    const callbackAppId = (request.callback as Record<string, unknown> | undefined)?.appId;
    if (typeof callbackAppId === "string" && callbackAppId.trim()) {
        return callbackAppId.trim();
    }
    return undefined;
}
function buildUrl(cloudBase: string, request: SensitiveRequest): {
    url: string;
} | {
    error: string;
} {
...
export function createCloudLinkSensitiveRequestAdapter(deps: CloudLinkAdapterDeps = {}): SensitiveRequestDeliveryAdapter {
...
export const cloudLinkSensitiveRequestAdapter: SensitiveRequestDeliveryAdapter = createCloudLinkSensitiveRequestAdapter();
