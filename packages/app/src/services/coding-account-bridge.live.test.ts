 ...
 */
import { __resetDefaultAccountPoolForTests } from "./account-pool.js";
...
d("multi-account live (real linked accounts)", () => {
    it("rotates least-used across distinct real accounts and resolves tokens", async () => {
        ...
        if (!bridge)
            return;
        const multiProvider = CODING_PROVIDERS.find((p) => pool.list(p).filter((a) => a.enabled).length >= 2);
        if (!multiProvider) {
            throw new Error("Connect ≥2 accounts for a Claude subscription or Codex provider, then re-run.");
        }
        const agentType = multiProvider === "anthropic-subscription" ? "claude" : "codex";
    ...
    it("reports live session/weekly usage for each connected account", async () => {
        ...
        for (const provider of CODING_PROVIDERS) {
            for (const account of pool.list(provider).filter((a) => a.enabled)) {
                const { getAccessToken } = await import("@elizaos/auth/auth/credentials");
