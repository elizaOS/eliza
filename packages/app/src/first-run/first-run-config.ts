...
import { buildElizaCloudServiceRoute } from "@elizaos/core/contracts/service-routing";
import { isElizaCloudFirstRunTarget } from "./runtime-target";
import { normalizeFirstRunProviderId } from "@elizaos/core/contracts/first-run-options";
import { requiresAdditionalRuntimeProvider } from "@elizaos/core/contracts/first-run-options";
import { type DeploymentTargetConfig } from "@elizaos/core/contracts/service-routing";
import { type FirstRunCredentialInputs } from "@elizaos/core/contracts/first-run-options";
import { type FirstRunLocalProviderId } from "@elizaos/core/contracts/first-run-options";
...
function resolveLocalProviderId(provider: string): FirstRunLocalProviderId | null {
    const normalized = normalizeFirstRunProviderId(provider);
    return normalized && normalized !== "elizacloud" ? normalized : null;
}
function resolveArgsServerTarget(args: Pick<BuildFirstRunConnectionArgs, "firstRunRuntimeTarget">): FirstRunRuntimeTarget {
...
export function buildFirstRunRuntimeConfig(args: BuildFirstRunConnectionArgs): BuildFirstRunRuntimeConfigResult {
    const serverTarget = resolveArgsServerTarget(args);
    const persistRuntimeOnConnectedRemote = serverTarget === "remote" && args.firstRunRemoteConnected;
    ...
    const responseHandlerModel = trimToUndefined(args.firstRunResponseHandlerModel ?? "");
    const actionPlannerModel = trimToUndefined(args.firstRunActionPlannerModel ?? "");
    ...
    const deploymentTarget: DeploymentTargetConfig = persistRuntimeOnConnectedRemote
        ? { runtime: "local" }
        : serverTarget === "remote"
            ? {
                runtime: "remote",
    ...
    const shouldConfigureRuntimeProvider = !args.omitRuntimeProvider &&
        !requiresAdditionalRuntimeProvider(args.firstRunProvider);
    if (args.firstRunProvider === "elizacloud" &&
    ...
    const cloudDefaultsSelected = args.firstRunProvider === "elizacloud" ||
        (deploymentTarget.runtime === "cloud" &&
            deploymentTarget.provider === "elizacloud");
    if (cloudDefaultsSelected) {
        Object.assign(serviceRouting, buildDefaultElizaCloudServiceRouting({
            base: serviceRouting,
            includeInference: shouldConfigureRuntimeProvider &&
            ...
            excludeServices: deploymentTarget.runtime === "cloud" ? undefined : ["embeddings"],
            nanoModel,
            smallModel,
            mediumModel,
            largeModel,
    ...
    const hasFeatures = args.firstRunFeatureTelegram ||
