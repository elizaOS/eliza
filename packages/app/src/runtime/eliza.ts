/**
 * App-core Eliza runtime loader: the single boot chokepoint every app agent
...
import { CUSTOM_PLUGINS_DIRNAME } from "@elizaos/agent";
import { applyCloudConfigToEnv as upstreamApplyCloudConfigToEnv } from "@elizaos/agent";
import { attemptPgliteAutoReset } from "./startup/pglite-recovery.js";
import { bootElizaRuntime as upstreamBootElizaRuntime } from "@elizaos/agent";
import { collectPluginNames as upstreamCollectPluginNames } from "@elizaos/agent";
import { createRuntimeBootResources } from "./startup/app-runtime-host.js";
import { ensureBundledFusedLibDir } from "./bundled-fused-lib.js";
import { ensureDefaultEmbeddingDimension } from "./startup/local-model-warmup.js";
import { failRuntimeRepair } from "./startup/app-runtime-host.js";
import { getPgliteRecoveryRetrySkipPlugins } from "./startup/pglite-recovery.js";
import { installAgentHostBridge } from "./install-agent-host-bridge.js";
import { normalizePgliteStartupError } from "./startup/pglite-recovery.js";
import { prepareLocalEmbeddingWarmup } from "./startup/local-model-warmup.js";
import { readAliasedEnv } from "@elizaos/core/utils/env";
import { repairRuntimeAfterBoot } from "./startup/app-runtime-host.js";
import { resolvePackageEntry } from "@elizaos/agent";
import { scanDropInPlugins } from "@elizaos/agent";
import { shutdownRuntime } from "./startup/app-runtime-host.js";
import { startDeferredLocalEmbeddingWarmup } from "./startup/local-model-warmup.js";
import { startEliza as upstreamStartEliza } from "@elizaos/agent";
import { startServerOnlyHost } from "./startup/server-only-host.js";
import { stopRuntimeBootResources } from "./startup/app-runtime-host.js";
import { type BootElizaRuntimeOptions } from "@elizaos/agent";
import { type EmbeddingProgressCallback } from "./startup/local-model-warmup.js";
import { type StartElizaOptions } from "@elizaos/agent";
...
export { drainBootHookContributors, resolveBootHookContributors, } from "@elizaos/agent/runtime/boot-hooks";
export { __loadAppRoutePluginFromSpecifierForTest, drainRuntimeHookContributors, getDeferAppRoutesEnabled, getSkippedAppRoutePluginIds, normalizeAppRoutePluginId, } from "./startup/app-contributors.js";
export { createRuntimeBootResources, type PostReadyBootSteps, type RuntimeBootResources, runPostReadyBootTail, shutdownRuntime, } from "./startup/app-runtime-host.js";
export function collectPluginNames(...args: Parameters<typeof upstreamCollectPluginNames>): ReturnType<typeof upstreamCollectPluginNames> {
    return upstreamCollectPluginNames(...args);
}
export function applyCloudConfigToEnv(...args: Parameters<typeof upstreamApplyCloudConfigToEnv>): ReturnType<typeof upstreamApplyCloudConfigToEnv> {
...
export async function bootElizaRuntime(opts: BootElizaRuntimeOptionsExt = {}): Promise<Awaited<ReturnType<typeof upstreamBootElizaRuntime>>> {
    ...
    try {
        ...
        // error-policy:J2 a failed app repair cannot leave the upstream
...
export interface StartElizaOptionsExt extends StartElizaOptions {
    ...
    onServerOnlyHostReady?: (host: import("./server-only-process").ServerOnlyHost) => void;
}
async function upstreamStartElizaWithPgliteCompat(options?: StartElizaOptions): Promise<Awaited<ReturnType<typeof upstreamStartEliza>>> {
    ensureBundledFusedLibDir();
    try {
...
export async function startEliza(options?: StartElizaOptionsExt): Promise<Awaited<ReturnType<typeof upstreamStartEliza>>> {
        ...
        return await startServerOnlyHost({
            ...
            bootRuntime: async (onPostReadyPhase) => {
                ...
                if (!booted)
                    return booted;
                try {
                    return await repairRuntimeAfterBoot(booted, bootResources, onPostReadyPhase);
                }
                catch (error) {
                    // error-policy:J2 the server host never publishes a runtime whose
                    // app repair failed, and cleanup preserves the failure cause.
