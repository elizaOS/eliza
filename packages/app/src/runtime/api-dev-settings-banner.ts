...
import { ELIZA_RUNTIME_ENV_KEYS } from "@elizaos/core/runtime-env";
...
export function shouldShowApiDevSettingsBanner(env: Record<string, string | undefined> = process.env): boolean {
    const logLevel = (env.ELIZA_DEV_LOG_LEVEL ?? env.LOG_LEVEL ?? "")
        .trim()
        .toLowerCase();
    return (env.ELIZA_DEV_SHOW_SETTINGS === "1" ||
 ...
 */
export function formatApiDevSettingsBannerText(actualPort: number, options?: {
    ...
    const originsWin = firstWinningEnvString(env, ELIZA_RUNTIME_ENV_KEYS.allowedOrigins);
    const hostsWin = firstWinningEnvString(env, ELIZA_RUNTIME_ENV_KEYS.allowedHosts);
    const rows: DevSettingsRow[] = [
        ...
        },
        {
            ...
            change: "export ELIZA_API_TOKEN=<secret> or unset; ELIZA_DISABLE_AUTO_API_TOKEN=1 disables auto token",
        },
        {
        ...
        },
        {
            ...
            source: env.ELIZA_RENDERER_URL?.trim()
                ...
                : "default (unset)",
            change: "export ELIZA_RENDERER_URL=http://127.0.0.1:<vite>/ (desktop dev)",
        },
        {
            ...
            source: env.ELIZA_DESKTOP_DEV_LOG_PATH?.trim()
                ...
                : "default (unset)",
            change: "set by dev orchestrator; GET /api/dev/console-log tails this file",
        },
        {
            setting: "Dev hooks",
            effective: "/api/dev/stack, /api/dev/console-log, /api/dev/cursor-screenshot",
            source: "derived — dev server",
            change: `GET http://127.0.0.1:${actualPort}/api/dev/stack etc.; docs/apps/desktop-local-development.md`,
        },
    ];
    return prependDevSubsystemFigletHeading("api", formatDevSettingsTable("API — effective settings (after listen)", rows));
}
