/**
 * Combined dev-server / desktop-agent process entry. Binds the app API
...
import { formatError } from "null/browser-contracts";
import { formatUncaughtError } from "@elizaos/core/error-classification";
import { getLogPrefix } from "@elizaos/core/utils/log-prefix";
import { isRoutineDevMemoryHeartbeatEnabled } from "./dev-memory-heartbeat.js";
import { logRoutineDevMemoryHeartbeat } from "./dev-memory-heartbeat.js";
import { logger } from "@elizaos/core";
import { mergedRecoverySkipPlugins } from "./skip-plugins-env.js";
...
import { restoreOperatorSkipPlugins } from "./skip-plugins-env.js";
...
import { shouldShowApiDevSettingsBanner } from "./api-dev-settings-banner.js";
...
console.log(`${getLogPrefix()} Script starting... (timing: ${STARTUP_TIMING_SOURCE}; pre-body/import delay: ${STARTUP_TIMESTAMP
...
console.log(`${getLogPrefix()} Static imports complete (${elapsedSinceStartupTimingStart()}ms since ${STARTUP_TIMING_SOURCE}; module body ${elapsedSinceModuleBodyStart()}ms)`);
// Load .env files for parity with CLI mode (which loads via run-main.ts).
const { config: loadDotenv } = await import("dotenv");
loadDotenv({ quiet: true });
console.log(`${getLogPrefix()} dotenv loaded (${elapsedSinceStartupTimingStart()}ms since ${STARTUP_TIMING_SOURCE}; module body ${elapsedSinceModuleBodyStart()}ms)`);
...
let apiUpdateStartup: ((update: {
    phase?: string;
    attempt?: number;
    lastError?: string;
    lastErrorAt?: number;
    nextRetryAt?: number;
    state?: "not_started" | "starting" | "running" | "paused" | "stopped" | "restarting" | "error";
}) => void) | null = null;
...
async function bootstrapRuntime(reason: string): Promise<void> {
    if (isShuttingDown || isRestarting || runtimeBootInProgress)
        return;
    ...
    try {
        ...
        try {
            const { applySavedTokenToEnv } = await import("../services/github-credentials.js");
            const result = await applySavedTokenToEnv();
            if (result.applied) {
                logger.info(`${getLogPrefix()} Applied saved GitHub token to runtime env (user=@${result.username})`);
            }
            else if (result.envAlreadySet) {
                logger.info(`${getLogPrefix()} GITHUB_TOKEN already set in env — leaving untouched`);
            }
        }
        catch (err) {
            ...
            logger.warn(`${getLogPrefix()} Failed to apply saved GitHub token (runtime continues without it): ${formatError(err)}`);
        }
        ...
        if (isShuttingDown) {
            try {
                ...
                logger.warn(`${getLogPrefix()} Runtime teardown failed during shutdown race: ${formatError(error)}`);
            }
            return;
        }
        if (apiUpdateRuntime) {
        ...
        logger.info(`${getLogPrefix()} Runtime ready — agent: ${agentName} (total: ${Date.now() - bootstrapStart}ms)`);
        (await loadElizaRuntimeModule()).startDeferredLocalEmbeddingWarmup();
    }
    catch (err) {
        if (!runtimeBootPgliteAutoResetAttempted) {
            try {
                const { attemptPgliteAutoReset, getPgliteRecoveryRetrySkipPlugins } = await loadElizaRuntimeModule();
                ...
                if (backupDir) {
                    ...
                    if (runtimeBootPgliteRecoverySkipPlugins.length > 0) {
                        process.env.ELIZA_SKIP_PLUGINS = mergedRecoverySkipPlugins(operatorSkipPlugins, runtimeBootPgliteRecoverySkipPlugins);
                        logger.warn(`${getLogPrefix()} Skipping previously failed plugins on the recovery retry: ${runtimeBootPgliteRecoverySkipPlugins.join(", ")}.`);
                    }
                    apiUpdateStartup?.({
                        phase: "runtime-bootstrap",
                        attempt: 1,
                    ...
                    logger.warn(`${getLogPrefix()} Quarantined corrupt PGlite data dir at ${backupDir}. Retrying runtime bootstrap once.`);
                ...
                logger.error(`${getLogPrefix()} PGlite auto-reset failed (${formatError(recoveryErr)})`);
            }
        }
        const now = Date.now();
        ...
        if (failure.shouldRetry && failure.delayMs !== undefined) {
            logger.error(`${getLogPrefix()} Runtime bootstrap failed (${failure.lastError}). Retrying in ${Math.round(failure.delayMs / 1000)}s${failure.state === "error" ? " (UI state set to error)" : ""}`);
            scheduleRuntimeBootstrap(failure.delayMs, "retry");
        }
        else {
            logger.error(`${getLogPrefix()} Runtime bootstrap failed (${failure.lastError}). Startup halted until the PGlite issue is fixed.`);
        }
    }
    finally {
 ...
 */
let trajectoryRecoveryIpc: ReturnType<typeof import("./dev-trajectory-recovery-ipc").createDevTrajectoryRecoveryIpc> | null = null;
const devTrajectoryRecoveryRequested = process.env.ELIZA_DEV_TRAJECTORY_RECOVERY === "1";
...
async function createRuntime(): Promise<AgentRuntime> {
    ...
    if (devTrajectoryRecoveryRequested) {
        const { createDevTrajectoryRecoveryIpc } = await import("./dev-trajectory-recovery-ipc");
        trajectoryRecoveryIpc ??= createDevTrajectoryRecoveryIpc();
        try {
            await runtimeModule.prepareDevTrajectoryRecovery(result, trajectoryRecoveryIpc);
        }
        catch (error) {
            ...
            throw new Error("Development trajectory recovery failed before runtime readiness", { cause: error });
        }
    }
...
async function handleRestart(reason?: string): Promise<void> {
    ...
    restartPromise = (async () => {
        ...
        try {
            ...
            logger.info(`${getLogPrefix()} Restart requested${reason ? ` (${reason})` : ""} — bouncing runtime…`);
                ...
                try {
                    process.send({ type: "eliza:runtime-restart" }, (error) => {
                        if (error) {
                            ...
                            logger.warn(`${getLogPrefix()} Parent restart notification failed: ${formatError(error)}`);
...
async function shutdown(): Promise<void> {
    ...
    if (currentRuntime) {
        try {
            ...
            logger.warn(`${getLogPrefix()} Error stopping runtime during shutdown: ${formatError(err)}`);
        }
        currentRuntime = null;
    }
...
async function main() {
    ...
    const { port: actualPort, updateRuntime, updateStartup, } = await startApiServer({
        port,
    ...
    if (actualPort !== port) {
        logger.error(`${getLogPrefix()} [CRITICAL] API bound to port ${actualPort} but orchestrator expected ${port}. ` +
    ...
    setImmediate(() => {
        try {
            ...
            if (sweep.stale.length > 0) {
                logger.warn(`[eliza] ${sweep.stale.length} of ${sweep.distLoaded} dist-loaded workspace plugin(s) have stale output; ` +
            ...
            logger.warn(`[eliza] Plugin dist staleness check unavailable: ${formatError(error)}`);
        }
    });
    ...
    console.log(`${getLogPrefix()} API ready: http://localhost:${actualPort} (${apiReady - apiStart}ms)`);
    if (apiToken) {
        console.log(`${getLogPrefix()} Connection key: ${"*".repeat(Math.max(0, apiToken.length - 4)) + apiToken.slice(-4)}`);
    }
    if (pairing) {
...
process.on("unhandledRejection", (reason) => {
    if (shouldIgnoreUnhandledRejection(reason)) {
        logger.warn(`${getLogPrefix()} Provider credits appear exhausted; request failed without output. Top up credits and retry.`);
        return;
    }
    // In dev mode (bun --watch), log but do NOT exit — let the watcher restart.
    logger.error(`${getLogPrefix()} Unhandled rejection: ${formatUncaughtError(reason)}`);
});
