...
import { AppStartupStateMachine } from "../startup-state.js";
...
import { isRuntimeBootDeferred } from "../../api/deferred-runtime-boot.js";
import { logger } from "@elizaos/core";
import { readAliasedEnv } from "@elizaos/core/utils/env";
import { registerDeferredRuntimeBoot } from "../../api/deferred-runtime-boot.js";
...
import { shouldDeferRuntimeBootUntilOnboarding } from "../../api/deferred-runtime-boot.js";
import { startApiServer } from "../../api/server.js";
import { syncResolvedApiPort } from "@elizaos/core/runtime-env";
import { triggerDeferredRuntimeBoot } from "../../api/deferred-runtime-boot.js";
import { type AgentRuntime } from "@elizaos/core";
import { type AppStartupPhase } from "../startup-state.js";
...
export interface StartServerOnlyHostOptions {
    ...
    bootRuntime: (onPostReadyPhase: (phase: PostReadyPhase) => void) => Promise<AgentRuntime | undefined>;
    stopRuntime: (runtime: AgentRuntime, reason: string) => Promise<unknown>;
    stopWithoutRuntime: () => void;
}
export async function startServerOnlyHost({ options, bootRuntime, stopRuntime, stopWithoutRuntime, }: StartServerOnlyHostOptions): Promise<AgentRuntime | undefined> {
    ...
    const publishPostReadyPhase = (phase: "pending" | "complete" | "failed"): void => {
        postReadyPhase = phase;
        if (!runtimePublished)
            return;
        publishStartup(phase === "pending"
            ? "features-starting"
            : phase === "complete"
                ? "ready"
                : "degraded");
    ...
    const deferRuntimeBootUntilOnboarding = shouldDeferRuntimeBootUntilOnboarding();
    ...
    let updateRuntime: Awaited<ReturnType<typeof startApiServer>>["updateRuntime"] | undefined;
    let updateStartup: Awaited<ReturnType<typeof startApiServer>>["updateStartup"] | undefined;
    ...
    const skipApiListen = options.localAgentMode === true &&
        resolveApiExposePort(process.env) !== true;
    if (skipApiListen) {
        bootLap("startEliza:local-agent IPC mode — skipping API TCP bind");
        logger.info("[eliza] Local-agent IPC mode: initializing route kernel without a TCP listener (set ELIZA_API_EXPOSE_PORT=1 to re-open the port)");
    }
    ...
    if (!skipApiListen) {
        ...
        logger.info("[eliza] Local-agent IPC mode: route kernel ready (no TCP listener bound)");
    ...
    if (deferRuntimeBootUntilOnboarding) {
        ...
        logger.info("[eliza] Fresh install — agent runtime boot deferred until onboarding commits (onboarding API routes are live)");
    ...
    const { buildSandboxRegistryFromEnv } = await import("@elizaos/agent/sandbox-registry");
