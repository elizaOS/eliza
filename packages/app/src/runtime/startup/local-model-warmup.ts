...
import { configureLocalEmbeddingPlugin } from "@elizaos/agent";
...
import { shouldWarmupVoice } from "../voice-warmup.js";
...
import { warmVoiceModels } from "../voice-warmup.js";
export type EmbeddingProgressCallback = (phase: EmbeddingWarmupPhase, detail?: string) => void;
let localInferenceRuntime: typeof import("@elizaos/plugin-local-inference/runtime") | undefined;
...
async function getLocalInferenceRuntime() {
    ...
    try {
        ...
        if (code !== "MODULE_NOT_FOUND" &&
            ...
            throw error;
        logger.info("[eliza] Local embedding warmup unavailable: optional local-inference runtime is not installed.");
        return undefined;
    }
    localInferenceRuntime ??= await import("@elizaos/plugin-local-inference/runtime");
    return localInferenceRuntime;
}
function isLocalEmbeddingWarmupDeferredByEnv(): boolean {
    const raw = process.env.ELIZA_DEFER_LOCAL_EMBEDDING_WARMUP?.trim().toLowerCase();
    return !(raw === "0" || raw === "false" || raw === "no" || raw === "off");
}
function startLocalEmbeddingWarmup(onProgress?: EmbeddingProgressCallback): void {
    void warmupEmbeddingModel(onProgress).catch((error) => {
        ...
        logger.error(`[eliza] Local embedding warmup preparation failed: ${formatError(error)}`);
    });
}
/** Starts eager warmup only when the operator has disabled default deferral. */
export function prepareLocalEmbeddingWarmup(onProgress?: EmbeddingProgressCallback): void {
...
export function startDeferredLocalEmbeddingWarmup(onProgress?: EmbeddingProgressCallback): boolean {
    if (!isLocalEmbeddingWarmupDeferredByEnv())
        return false;
...
async function warmupEmbeddingModel(onProgress?: EmbeddingProgressCallback): Promise<void> {
    if (warmupInFlight)
...
async function warmupEmbeddingModelImpl(onProgress?: EmbeddingProgressCallback): Promise<void> {
    if (isMobilePlatform()) {
    ...
    if (!localInference.isEmbeddingWarmupReuseDisabled() &&
        !localInference.embeddingGgufFilePresent(modelsDir, model)) {
        const reuse = localInference.findExistingEmbeddingModelForWarmupReuse(modelsDir);
        if (reuse) {
    ...
    try {
        await localInference.ensureModel(modelsDir, modelRepo, model, false, progressCallback);
    }
    catch (error) {
        ...
        logger.warn(`[eliza] Embedding model warmup failed (will retry on first use): ${formatError(error)}`);
    }
}
function isExplicitDesktopCloudOnlyRuntime(env: NodeJS.ProcessEnv = process.env): boolean {
...
export async function startDeferredVoiceWarmup(runtime: AgentRuntime): Promise<void> {
    ...
    await warmVoiceModels(runtime as Parameters<typeof warmVoiceModels>[0], {
