...
import { CONNECTOR_TARGET_SOURCE_REGISTRY_SERVICE } from "@elizaos/core";
import { ElizaError } from "@elizaos/core";
import { configureAutonomy } from "./autonomy.js";
import { createRuntimeBootResources } from "./post-ready.js";
...
import { getDeferAppRoutesEnabled } from "./app-contributors.js";
...
import { registerAppRoutePlugins } from "./app-contributors.js";
import { registerCoreSensitiveRequestAdapters } from "../../services/sensitive-requests/index.js";
import { registerRuntimeHooks } from "./app-contributors.js";
...
import { runPostReadyBootTail } from "./post-ready.js";
import { shutdownRuntime as upstreamShutdownRuntime } from "@elizaos/agent";
import { startDeferredVoiceWarmup } from "./local-model-warmup.js";
import { type AgentRuntime } from "@elizaos/core";
import { type PostReadyBootSteps } from "./post-ready.js";
import { type RuntimeBootResources } from "./post-ready.js";
import { type TargetSource } from "@elizaos/core";
const runtimeBootResources = new WeakMap<AgentRuntime, RuntimeBootResources>();
export async function repairRuntimeAfterBoot(runtime: AgentRuntime, resources: RuntimeBootResources, onPostReadyPhase?: (phase: "pending" | "complete" | "failed") => void): Promise<AgentRuntime> {
    ...
    if (getDeferAppRoutesEnabled()) {
        void runPostReadyBootTail(runtime, createPostReadyBootSteps(resources), resources).then(() => onPostReadyPhase?.("complete"), (err: unknown) => {
            ...
            logger.error(`[eliza] post-ready boot tail failed: ${formatErrorWithStack(err)}`);
    ...
    try {
        await runPostReadyBootTail(runtime, createPostReadyBootSteps(resources), resources);
        onPostReadyPhase?.("complete");
    }
    catch (err) {
 ...
 */
function createPostReadyBootSteps(resources: RuntimeBootResources): PostReadyBootSteps {
    return {
        ...
        ensureTriggerEventBridge: (runtime) => ensureTriggerEventBridge(runtime, resources),
        ensureConnectorTargetCatalog: (runtime) => ensureConnectorTargetCatalog(runtime, resources),
        startDeferredVoiceWarmup,
    };
}
export { createRuntimeBootResources, type PostReadyBootSteps, type RuntimeBootResources, runPostReadyBootTail, };
const CONNECTOR_TARGET_CATALOG_SERVICE_TYPE = "connector_target_catalog";
async function ensureTriggerEventBridge(runtime: AgentRuntime, resources: RuntimeBootResources): Promise<void> {
    if (resources.triggerEventBridge) {
        resources.triggerEventBridge.stop();
        resources.triggerEventBridge = null;
    }
    const { startTriggerEventBridge } = await import("../../services/trigger-event-bridge.js");
    resources.triggerEventBridge = startTriggerEventBridge(runtime);
    logger.debug("[eliza] trigger event bridge armed");
}
async function ensureConnectorTargetCatalog(runtime: AgentRuntime, resources: RuntimeBootResources): Promise<void> {
    ...
    const catalog = createElizaConnectorTargetCatalog({
        ...
        listSources: () => {
            const registry = runtime.getService(CONNECTOR_TARGET_SOURCE_REGISTRY_SERVICE) as {
                list(): TargetSource[];
            } | null;
...
export function stopRuntimeBootResources(resources: RuntimeBootResources): void {
    ...
    if (resources.triggerEventBridge) {
        try {
            ...
            logger.warn(`[eliza] Trigger event bridge stop failed during shutdown: ${formatError(error)}`);
        }
        resources.triggerEventBridge = null;
    }
    if (resources.connectorTargetCatalog) {
        try {
            ...
            logger.warn(`[eliza] Connector target catalog stop failed during shutdown: ${formatError(error)}`);
        }
        resources.connectorTargetCatalog = null;
    }
}
export async function shutdownRuntime(...args: Parameters<typeof upstreamShutdownRuntime>): Promise<Awaited<ReturnType<typeof upstreamShutdownRuntime>>> {
...
export async function failRuntimeRepair(runtime: AgentRuntime, scope: "boot" | "server-only-boot" | "start", repairError: unknown): Promise<never> {
    try {
