...
import { formatDocsLink } from "@elizaos/app/terminal/links";
...
export function loadConfig(configPath: string): Record<string, unknown> {
    ...
    try {
        ...
        throw new Error(`Cannot parse config at ${configPath}: ${reason}. Fix or remove the file before running setup.`);
    }
...
export function hasModelKey(env: Record<string, string | undefined>): string | null {
    const keys = [
        "ANTHROPIC_API_KEY",
        "CLAUDE_API_KEY",
        "OPENAI_API_KEY",
...
// ---------------------------------------------------------------------------
export async function runProviderWizard(configPath: string, options: ProviderWizardOptions = {}): Promise<void> {
    ...
    const combinedEnv = { ...env, ...envSection } as Record<string, string | undefined>;
...
export function registerSetupCommand(program: Command) {
    program
        ...
        .option("--key <value>", "Unsafe: API key or URL via argv (prefer --key-stdin)")
        ...
        await runCommandWithRuntime(defaultRuntime, async () => {
            ...
            const { ensureAgentWorkspace, resolveDefaultAgentWorkspaceDir } = await import("@elizaos/agent");
            ...
            if (opts.provider && keyValue) {
                ...
                const providerEntry = PROVIDERS.find((p) => p.label.toLowerCase().includes(providerQuery) ||
                    (p.key ?? "").toLowerCase().includes(providerQuery));
                const envKey = providerEntry?.key ??
                ...
                if (opts.key) {
                    console.log(`${theme.warn("⚠")} ${theme.muted("Passing secrets via --key exposes them in shell history and process lists. Prefer --key-stdin.")}`);
                }
            }
