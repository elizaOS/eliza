 ...
 */
export async function runAuthAdoptCodex(params: AdoptCodexCliParams = {}): Promise<AdoptCodexCliResult> {
    ...
    if (!params.yes) {
        ...
        log(`This RETIRES the Codex CLI login (auth.json is renamed out of the CLI's read path) and stores its tokens as pool account ${theme.command(`openai-codex/${accountId}`)}.`);
        log("After adoption the `codex` CLI is logged out on this machine — the pool becomes the only refresher of this chain.");
        log(theme.error("Stop every running codex process first, then re-run with --yes to proceed."));
        return {
            ok: false,
    ...
    const { adoptCodexCliLogin } = await import("@elizaos/auth/auth/subscription-auth/adopt-codex-cli-login");
    const { createRuntimeAccountStoragePolicy } = await import("@elizaos/auth/auth/account-storage");
    ...
    try {
        ...
        log(`${theme.success("✓")} adopted Codex login as ${theme.command(`openai-codex/${result.accountId}`)}`);
        log(`${theme.success("✓")} source retired to ${theme.command(result.retiredTo)} — the codex CLI can no longer refresh this chain`);
...
export function registerAuthAdoptCodexSubcommand(program: Command): void {
    ...
    auth
        .command("adopt-codex")
        .description("Adopt the Codex CLI login into the account pool and retire the source (destructive; requires --yes)")
        .option("--account <id>", "Pool account id to create", "default")
        .option("--codex-home <path>", "CODEX_HOME to adopt from (default $CODEX_HOME or ~/.codex)")
