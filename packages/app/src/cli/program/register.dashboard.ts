 ...
 */
export function createDevServerTeardown(child: Pick<ChildProcess, "pid">, spawnSync: typeof import("node:child_process").spawnSync, platform: NodeJS.Platform = process.platform): () => void {
    const killGroup = (signal: NodeJS.Signals): void => {
        ...
        try {
            ...
            if (code !== "ESRCH") {
                console.log(theme.muted(`Dev server teardown (${signal}) skipped: ${code}`));
            }
        }
    };
    let cleaned = false;
...
export function registerDashboardCommand(program: Command) {
    ...
    program
        ...
        const { resolveElizaPackageRootSync } = await import("../../utils/eliza-root");
        ...
        if (!appDir) {
            ...
            console.log(theme.muted("Start the agent with `eliza start` and use the URL printed at startup."));
            process.exitCode = 1;
            return;
        }
