...
async function voiceModelsAction(opts: {
    ...
    const { VOICE_MODEL_VERSIONS, latestVoiceModelVersion } = await import("@elizaos/plugin-native-inference/model-catalog/voice-models");
    console.log(`\n${theme.heading("Eliza voice sub-models")}\n`);
    console.log(theme.muted("Per-sub-model versioning (R5-versioning §3). Auto-update gate: newer\n" +
    ...
    for (const v of VOICE_MODEL_VERSIONS) {
        ...
        if (!latest)
            continue;
        const sizeMb = latest.ggufAssets.length === 0
            ? "(unpublished)"
            : `${(latest.ggufAssets.reduce((s, a) => s + a.sizeBytes, 0) / 1048576).toFixed(1)} MB`;
        console.log(`  ${theme.accent(v.id.padEnd(24))} ${theme.success(latest.version.padEnd(8))} ${theme.muted(sizeMb)}`);
...
async function updateAction(opts: {
    ...
    const { detectInstallMethod, getUpdateActionPlan, performUpdate } = await import("@elizaos/agent");
...
async function statusAction(): Promise<void> {
    ...
    const { resolveChannel, fetchAllChannelVersions } = await import("@elizaos/agent");
    const { detectInstallMethod, getUpdateActionPlan } = await import("@elizaos/agent");
    ...
    console.log(`  Can run:    ${updatePlan.canExecuteFromContext ? "yes" : "no"}`);
...
export function registerUpdateCommand(program: Command): void {
    const updateCmd = program
        ...
        .option("-c, --channel <channel>", "Switch release channel (stable, beta, nightly)")
        .option("--check", "Check for updates without installing")
        .option("--force", "Force update check (bypass interval cache)")
        .option("--voice-models", "List voice sub-model versions (R5-versioning auto-updater)")
        .action(updateAction);
    updateCmd
        .command("status")
        .description("Show current version and available updates across all channels")
        .action(statusAction);
    updateCmd
