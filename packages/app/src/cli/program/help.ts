 ...
 * no command behavior.
 */
import { formatCliBannerLine } from "../banner";
import { formatDocsLink } from "@elizaos/app/terminal/links";
import { hasEmittedCliBanner } from "../banner";
import { isRich } from "@elizaos/app/terminal/theme";
import { replaceCliName } from "../cli-name";
...
export function configureProgramHelp(program: Command, programVersion: string) {
    program
        ...
        .option("--dev", "Dev profile: isolate state under a named XDG state-dir namespace with separate config and ports")
        .option("--profile <name>", "Use a named profile with isolated state and config");
