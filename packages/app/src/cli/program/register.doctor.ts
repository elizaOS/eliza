...
import { type CheckCategory } from "../doctor/checks";
import { type CheckResult } from "../doctor/checks";
import { type CheckStatus } from "../doctor/checks";
...
function attemptFix(result: CheckResult): boolean {
    ...
    if (!result.fix.startsWith("eliza "))
        return false;
    ...
    const bin = process.env.ELIZA_BIN ??
...
export function registerDoctorCommand(program: Command) {
    program
        ...
        await runCommandWithRuntime(defaultRuntime, async () => {
            const { runMtpDoctor } = await import("@elizaos/plugin-local-inference/services");
            ...
            for (const check of report.checks) {
                const icon = check.status === "pass"
                    ...
                    : check.status === "warn"
                        ? theme.warn("⚠")
                        : theme.error("✗");
                console.log(`  ${icon} ${check.label.padEnd(28)} ${theme.muted(check.detail)}`);
    ...
    program
        ...
        await runCommandWithRuntime(defaultRuntime, async () => {
            ...
            if (opts.json) {
                ...
                process.stdout.write(`${JSON.stringify({ summary, checks: results }, null, 2)}\n`);
                if (summary.fail > 0)
                    process.exit(1);
                return;
            }
            ...
            if (failures.length === 0 && warnings.length === 0) {
                console.log(`  ${theme.success("Everything looks good.")} Ready to run ${theme.command("eliza start")}.`);
            }
                ...
                console.log(`  ${theme.warn(`${warnings.length} warning${warnings.length === 1 ? "" : "s"}. Things should still work.`)}`);
            }
            // ── --fix pass ────────────────────────────────────────────────────
            if (opts.fix) {
