...
function resolveDefaultAgentApiBase(): string {
    return (readAliasedEnv("ELIZA_AGENT_API_BASE") ??
        `http://127.0.0.1:${resolveDesktopApiPort(process.env)}`);
}
export type CapabilityRouterEndpointProvider = "direct" | "home-machine" | "mobile-companion" | "desktop-companion";
...
export type CapabilityRouterConformanceSurface = "action" | "provider" | "route" | "view-asset" | "model" | "lifecycle" | "event" | "service" | "app-bridge" | "evaluator" | "response-handler-evaluator" | "response-handler-field-evaluator";
export function registerCapabilityRouterCommand(program: Command) {
    ...
    capabilityRouter
        ...
        .option("--provider <provider>", "Endpoint provider: direct, home-machine, mobile-companion, desktop-companion", "direct")
        .option("--id <id>", "Endpoint id for direct/provider connects")
        .option("--token <token>", "Bearer token for direct/provider endpoint calls")
        .option("--allowed-module <moduleId>", "Restrict sync to a remote module id; repeatable", collectValues, [])
        .option("--no-persist", "Do not persist the connected endpoint")
        .option("--request-timeout-ms <ms>", "Capability-router RPC request timeout in milliseconds")
        .option("--cloud", "Provision an Eliza Cloud capability endpoint")
        .option("--cloud-api-base <url>", "Eliza Cloud API base URL", "https://api.eliza.app")
        .option("--cloud-auth-token <token>", "Eliza Cloud API token")
        .option("--cloud-name <name>", "Name for the provisioned Cloud agent")
        .option("--cloud-bio <line>", "Bio line for the provisioned Cloud agent; repeatable", collectValues, [])
        ...
        .option("--api-base <url>", "Running agent API base URL", resolveDefaultAgentApiBase())
        .action(async (baseUrl: string | undefined, opts: CapabilityRouterConnectOptions & {
        apiBase: string;
    }) => {
    ...
    capabilityRouter
        ...
        .description("Validate a remote endpoint against the capability-router plugin protocol")
        .option("--token <token>", "Bearer token for endpoint calls")
        .option("--request-timeout-ms <ms>", "Capability-router RPC request timeout in milliseconds")
        .option("--require <surface>", "Required surface: action, provider, route, view-asset, model, lifecycle, event, service, app-bridge, evaluator, response-handler-evaluator, response-handler-field-evaluator; repeatable", collectValues, [])
        .action(async (baseUrl: string, opts: CapabilityRouterConformanceOptions) => {
...
export async function runCapabilityRouterConformance(baseUrl: string, opts: CapabilityRouterConformanceOptions = {}): Promise<Record<string, unknown>> {
    const endpoint = normalizeApiBase(requireNonEmpty(baseUrl, "baseUrl"));
    const requestTimeoutMs = parseOptionalPositiveInteger(opts.requestTimeoutMs, "requestTimeoutMs") ??
        60000;
    const requiredSurfaces = normalizeConformanceSurfaces(opts.require);
    const availability = await requestCapabilityJson(endpoint, "GET", "/v1/capabilities", undefined, opts.token, requestTimeoutMs);
    ...
    const moduleResult = await invokeCapability(endpoint, "plugin.modules.list", {}, opts.token, requestTimeoutMs);
    ...
    if (requiredSurfaces.includes("action")) {
        ...
        if (!target)
            throw new Error("Endpoint did not expose a remote action.");
        const action = target.actions[0];
        await invokeCapability(endpoint, "plugin.action.invoke", {
            moduleId: target.id,
            action: action.name,
            content: { text: "capability-router conformance action" },
        }, opts.token, requestTimeoutMs);
    ...
    if (requiredSurfaces.includes("provider")) {
        ...
        if (!target)
            throw new Error("Endpoint did not expose a remote provider.");
        const provider = target.providers[0];
        await invokeCapability(endpoint, "plugin.provider.get", { moduleId: target.id, provider: provider.name, state: {} }, opts.token, requestTimeoutMs);
    ...
    if (requiredSurfaces.includes("route")) {
        ...
        if (!target)
            throw new Error("Endpoint did not expose a remote route.");
        const route = target.routes[0];
        const result = await invokeCapability(endpoint, "plugin.route.call", {
            moduleId: target.id,
            method: route.method,
            path: route.path,
            headers: {},
    ...
    if (requiredSurfaces.includes("view-asset")) {
        const target = modules.find((module) => module.views.find((view) => view.bundlePath));
        if (!target) {
        ...
        const result = await invokeCapability(endpoint, "plugin.asset.get", { moduleId: target.id, path: view.bundlePath }, opts.token, requestTimeoutMs);
    ...
    if (requiredSurfaces.includes("model")) {
        ...
        if (!target)
            throw new Error("Endpoint did not expose a remote model.");
        const model = target.models[0];
        await invokeCapability(endpoint, "plugin.model.invoke", {
            moduleId: target.id,
    ...
    if (requiredSurfaces.includes("lifecycle")) {
        ...
        await invokeCapability(endpoint, "plugin.lifecycle.call", { moduleId: target.id, hook, context: { conformance: true } }, opts.token, requestTimeoutMs);
    ...
    if (requiredSurfaces.includes("event")) {
        ...
        await invokeCapability(endpoint, "plugin.event.handle", {
            moduleId: target.id,
    ...
    if (requiredSurfaces.includes("service")) {
        const target = modules.find((module) => module.services.find((service) => service.methods[0]));
        if (!target) {
        ...
        await invokeCapability(endpoint, "plugin.service.call", {
            moduleId: target.id,
    ...
    if (requiredSurfaces.includes("app-bridge")) {
        ...
        await invokeCapability(endpoint, "plugin.appBridge.call", {
            moduleId: target.id,
            hook,
            context: {
                method: "GET",
    ...
    if (requiredSurfaces.includes("evaluator")) {
        ...
        if (!target)
            throw new Error("Endpoint did not expose a remote evaluator.");
        ...
        await invokeCapability(endpoint, "plugin.evaluator.shouldRun", common, opts.token, requestTimeoutMs);
        const prepare = await invokeCapability(endpoint, "plugin.evaluator.prepare", common, opts.token, requestTimeoutMs);
        const prepared = isRecord(prepare) ? prepare.prepared : undefined;
        await invokeCapability(endpoint, "plugin.evaluator.prompt", { ...common, ...(prepared === undefined ? {} : { prepared }) }, opts.token, requestTimeoutMs);
        await invokeCapability(endpoint, "plugin.evaluator.process", {
            ...common,
    ...
    if (requiredSurfaces.includes("response-handler-evaluator")) {
        ...
        await invokeCapability(endpoint, "plugin.responseHandlerEvaluator.shouldRun", common, opts.token, requestTimeoutMs);
        await invokeCapability(endpoint, "plugin.responseHandlerEvaluator.evaluate", common, opts.token, requestTimeoutMs);
        exercised.responseHandlerEvaluator = `${target.id}:${evaluator.name}`;
    }
    if (requiredSurfaces.includes("response-handler-field-evaluator")) {
        const target = modules.find((module) => module.responseHandlerFieldEvaluators[0]);
        if (!target) {
            throw new Error("Endpoint did not expose a remote response-handler field evaluator.");
        }
        ...
        await invokeCapability(endpoint, "plugin.responseHandlerFieldEvaluator.shouldRun", common, opts.token, requestTimeoutMs);
        const parse = await invokeCapability(endpoint, "plugin.responseHandlerFieldEvaluator.parse", { ...common, value: { raw: true } }, opts.token, requestTimeoutMs);
        const parsed = isRecord(parse) ? parse.value : undefined;
        await invokeCapability(endpoint, "plugin.responseHandlerFieldEvaluator.handle", {
            ...common,
            value: { raw: true },
...
export function buildCapabilityRouterConnectPayload(baseUrl: string | undefined, opts: CapabilityRouterConnectOptions): Record<string, unknown> {
...
function normalizeProvider(value: CapabilityRouterConnectOptions["provider"]): CapabilityRouterEndpointProvider {
    ...
    throw new Error("provider must be one of direct, home-machine, mobile-companion, or desktop-companion.");
}
function collectValues(value: string, previous: string[]): string[] {
    return [...previous, value];
}
function normalizeConformanceSurfaces(value: string[] | undefined): CapabilityRouterConformanceSurface[] {
    const values = normalizeStringList(value);
    const surfaces = values.length === 0
        ? [
            "action",
            "provider",
            "route",
    ...
    return surfaces.map((surface) => {
        ...
        throw new Error("require must be one of action, provider, route, view-asset, model, lifecycle, event, service, app-bridge, evaluator, response-handler-evaluator, or response-handler-field-evaluator.");
    });
}
function normalizeStringList(value: string[] | undefined): string[] {
    return [...new Set((value ?? []).map((item) => item.trim()).filter(Boolean))];
}
function optionalStringList(value: string[] | undefined, key: string): Record<string, string[]> {
    const values = normalizeStringList(value);
...
function optionalPositiveInteger(value: string | undefined, key: string): Record<string, number> {
...
async function requestCapabilityJson(baseUrl: string, method: "GET" | "POST", path: string, body: Record<string, unknown> | undefined, token: string | undefined, requestTimeoutMs: number): Promise<unknown> {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Capability request failed with HTTP ${response.status}: ${text}`);
        }
        return parsed;
    }
    finally {
...
async function invokeCapability(baseUrl: string, method: string, params: Record<string, unknown>, token: string | undefined, requestTimeoutMs: number): Promise<unknown> {
    const response = await requestCapabilityJson(baseUrl, "POST", "/v1/capabilities/invoke", { method, params }, token, requestTimeoutMs);
...
function readConformanceModules(value: unknown): ConformanceModule[] {
    ...
    return value.modules.map((item) => {
        ...
        return {
            ...
            responseHandlerFieldEvaluators: readNamedList(item.responseHandlerFieldEvaluators),
        };
    });
}
...
function readRouteList(value: unknown): Array<{
    ...
    return value
        .filter((item): item is Record<string, unknown> => isRecord(item))
        .filter((item) => typeof item.method === "string" &&
        item.method.trim() &&
...
function readModelList(value: unknown): Array<{
    ...
    return value
        .filter((item): item is Record<string, unknown> => isRecord(item))
        .filter((item) => typeof item.modelType === "string" && item.modelType.trim())
...
function readLifecycleHooks(value: unknown): string[] {
    if (!isRecord(value) || !Array.isArray(value.hooks))
        return [];
    return value.hooks.filter((item): item is string => typeof item === "string" && item.trim().length > 0);
}
...
function readServiceList(value: unknown): Array<{
    ...
    return value
        .filter((item): item is Record<string, unknown> => isRecord(item))
        .filter((item) => typeof item.serviceType === "string" && item.serviceType.trim())
        .map((item) => ({
        serviceType: item.serviceType as string,
        methods: Array.isArray(item.methods)
            ? item.methods.filter((method): method is string => typeof method === "string" && method.trim().length > 0)
            : [],
    }));
}
...
function readAssetResult(value: unknown): {
    ...
    if (!isRecord(value) ||
        ...
        throw new Error("plugin.asset.get result must include path, contentType, and bodyBase64.");
    }
