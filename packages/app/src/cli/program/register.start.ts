...
async function startAction() {
    ...
    await runCommandWithRuntime(defaultRuntime, async () => {
        ...
        const { installServerOnlyProcessOwner } = await import("../../runtime/server-only-process");
        const { ensureAuthPairingCodeForRemoteAccess } = await import("../../api/auth-pairing-routes");
        ...
        if (connectionKey) {
            console.log(`│  Connection key: ${("*".repeat(Math.max(0, connectionKey.length - 4)) + connectionKey.slice(-4)).padEnd(22)}│`);
        }
        if (pairing) {
...
export function registerStartCommand(program: Command) {
    const registerCommand = (name: string, description: string): void => {
        const command = program
            ...
            .option("--connection-key [key]", "Set or auto-generate a connection key for remote access");
        if (name === "start") {
            command.addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/getting-started", "docs.eliza.ai/getting-started")}\n`);
        }
        command.action(async (opts: {
            connectionKey?: string | boolean;
        }) => {
