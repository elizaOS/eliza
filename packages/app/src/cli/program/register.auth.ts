...
interface SqlPluginModule {
    createDatabaseAdapter: (cfg: {
        dataDir: string;
    }, id: `${string}-${string}-${string}-${string}-${string}`) => unknown;
...
async function openAuthStoreFromCli(): Promise<{
    ...
    const adapter = createDatabaseAdapter({ dataDir }, "00000000-0000-0000-0000-000000000001" as `${string}-${string}-${string}-${string}-${string}`) as RuntimeAdapter;
 ...
 */
async function waitForProofMatch(options: ProofChallengeOptions): Promise<boolean> {
    const interval = options.pollIntervalMs ?? 500;
    const timeoutMs = options.timeoutMs ?? 5 * 60 * 1000;
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
        const seen = await options.reader();
        if (seen !== null && seen.trim() === options.challenge)
            return true;
 ...
 */
export async function runElizaAuthReset(params: RunResetParams = {}): Promise<RunResetResult> {
    ...
    log("");
    const reader = params.proofReader ??
        (async () => {
            try {
 ...
 */
export async function runDevWalletLogin(params: DevWalletLoginParams = {}): Promise<DevWalletLoginResult> {
    const log = params.log ?? ((line: string) => console.log(line));
    ...
    if (params.save !== false) {
        try {
            const { resolveConfigPath, loadConfig, saveConfig } = await import("./register.setup");
            ...
            log(theme.error(`WARNING: failed to save ELIZAOS_CLOUD_API_KEY to the eliza config: ${saveError}`));
            log(theme.error("Your API key was NOT saved — the agent will not use Eliza Cloud until you save it."));
            log(`${theme.muted("Save it manually with:")} ${theme.command(`export ELIZAOS_CLOUD_API_KEY=${apiKey}`)}`);
            log(`${theme.muted("or re-run")} ${theme.command("eliza auth dev-login")} ${theme.muted("after fixing the config path above.")}`);
        }
    }
    return {
        ok: true,
...
export function registerAuthCommand(program: Command) {
    ...
    auth
        ...
        .option("--cloud <url>", "Cloud API base (default https://api.eliza.app or $ELIZAOS_CLOUD_BASE_URL)")
        ...
        await runCommandWithRuntime(defaultRuntime, async () => {
            ...
            if (result.savedTo) {
                ...
                console.log(result.saveError
                    ? theme.error(`✗ ELIZAOS_CLOUD_API_KEY was NOT saved (${result.saveError}) — save it manually (see above)`)
