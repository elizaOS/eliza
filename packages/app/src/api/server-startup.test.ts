/**
 * Colocated coverage for app server startup helpers. Drives the real
 * `isSafeResetStateDir`, `findOwnPackageRoot`, and re-exported
 * `resolveCorsOrigin` — no mocks of the module under test. Pins the
 * under-home + allowed-segment reset guard (including app's extra
...
describe("findOwnPackageRoot", () => {
  ...
  function makeTree(): string {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "app-pkg-root-"));
    trees.push(dir);
    return dir;
  }
  ...
  it("walks upward and returns the nearest matching ancestor", () => {
    ...
    const nested = path.join(root, "packages", "app", "src", "api");
    fs.mkdirSync(nested, { recursive: true });
    writePackageJson(path.join(root, "packages", "app"), {
      name: "@elizaos/app",
    });
    expect(findOwnPackageRoot(nested)).toBe(root);
  });
  ...
  it("still walks ancestors even when ELIZA_MOBILE_PLATFORM is set (app does not short-circuit)", () => {
    const root = makeTree();
    writePackageJson(root, { name: "eliza" });
