...
import { executeRawSql } from "@elizaos/plugin-sql/database-utils/sql-compat";
import { quoteIdent } from "@elizaos/plugin-sql/database-utils/sql-compat";
import { recordCacheHit } from "./perf-instrument";
import { recordCacheMiss } from "./perf-instrument";
import { sanitizeIdentifier } from "@elizaos/plugin-sql/database-utils/sql-compat";
import { sendJson as sendJsonResponse } from "./response";
import { sendJsonError as sendJsonErrorResponse } from "./response";
import { sqlLiteral } from "@elizaos/plugin-sql/database-utils/sql-compat";
...
function rememberTableIntrospection(key: string, resolvedSchema: string, columns: string[], nowMs: number): void {
...
export async function handleDatabaseRowsCompatRoute(req: http.IncomingMessage, res: http.ServerResponse, state: CompatRuntimeState, deps: DatabaseRowsCompatRouteDeps = {}): Promise<boolean> {
    ...
    if (decoded === null) {
        sendJsonErrorResponse(res, 400, "invalid table name: malformed URL encoding");
        return true;
    }
    ...
    if (cachedIntrospection && cachedIntrospection.expiresAt > nowMs) {
        ...
        if (!resolvedSchema) {
          ...
          ORDER BY CASE WHEN table_schema = 'public' THEN 0 ELSE 1 END,
            ...
            if (schemas.length > 1 && !schemas.includes("public")) {
                sendJsonErrorResponse(res, 409, `Table "${tableName}" exists in multiple schemas; specify ?schema=<name>.`);
                return true;
            }
            resolvedSchema = schemas.includes("public") ? "public" : schemas[0];
        }
        const columnResult = await executeRawSql(runtime, `SELECT column_name
        ...
        columns = columnResult.rows
            ...
            .filter((value): value is string => typeof value === "string");
        if (columns.length === 0) {
            sendJsonErrorResponse(res, 404, `No readable columns found for ${resolvedSchema}.${tableName}`);
            return true;
        }
    ...
    if (search) {
        ...
        filters.push(`(${columns
            .map((columnName) => `CAST(${quoteIdent(columnName)} AS TEXT) ILIKE ${searchLiteral}`)
            .join(" OR ")})`);
    }
    const whereClause = filters.length > 0 ? `WHERE ${filters.join(" AND ")}` : "";
    const orderBy = sortColumn && columns.includes(sortColumn)
