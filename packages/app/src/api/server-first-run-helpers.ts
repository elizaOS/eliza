...
import { applyFirstRunCredentialPersistence } from "@elizaos/agent";
import { deriveFirstRunCredentialPersistencePlan } from "@elizaos/core/contracts/first-run-options";
...
import { normalizeFirstRunCredentialInputs } from "@elizaos/core/contracts/first-run-options";
import { normalizeLinkedAccountFlagsConfig } from "@elizaos/core/contracts/service-routing";
...
function resolveCompatFirstRunStyle(body: Record<string, unknown>, language: string) {
...
export function hasDeprecatedFirstRunRequestFields(body: Record<string, unknown>): boolean {
    return DEPRECATED_FIRST_RUN_REQUEST_KEYS.some((key) => Object.hasOwn(body, key));
}
/**
 ...
 */
export async function extractAndPersistFirstRunApiKey(body: Record<string, unknown>, onConfigWrite?: FirstRunConfigWriteObserver, observeEnvironmentMutation?: <T>(mutation: () => T) => T): Promise<string | null> {
    const credentialInputs = normalizeFirstRunCredentialInputs(body.credentialInputs);
    ...
    logger.info(`[first-run] extractAndPersistFirstRunApiKey: credentialInputs=${credentialInputs ? "present" : "missing"}, keys=${Object.keys(body).join(",")}`);
    ...
    if (!llmSelection && !initialPlan.cloudApiKey) {
        logger.warn("[first-run] No first-run credentials resolved from request body");
        return null;
    }
    logger.info(`[first-run] Resolved selection: transport=${llmSelection?.transport ?? "none"}, provider=${llmSelection?.backend ?? "N/A"}, hasKey=${Boolean(llmSelection?.apiKey)}, hasCloudKey=${Boolean(initialPlan.cloudApiKey)}`);
    ...
    if (llmSelection?.transport === "direct" &&
        ...
        if (resolved && resolved.authType === "subscription") {
            ...
            logger.info(`[first-run] Using subscription auth for ${resolved.providerId}`);
        }
        else if (resolved) {
            effectiveCredentialInputs = {
                ...(effectiveCredentialInputs ?? {}),
            ...
            logger.info(`[first-run] Resolved real key for ${llmSelection.backend} via credential-resolver`);
        }
        else if (!llmSelection.apiKey || llmApiKeyMasked) {
            logger.warn(`[first-run] No real key available for ${llmSelection.backend} (input key ${llmApiKeyMasked ? "masked" : "missing"}) — cannot persist`);
            return null;
        }
...
export function persistFirstRunDefaults(body: Record<string, unknown>, onConfigWrite?: FirstRunConfigWriteObserver): string | null {
...
export function deriveFirstRunReplayBody(body: Record<string, unknown>): {
    ...
    const explicitDeploymentTarget = normalizeDeploymentTargetConfig(body.deploymentTarget);
    const explicitCredentialInputs = normalizeFirstRunCredentialInputs(body.credentialInputs);
    const deploymentTarget: DeploymentTargetConfig | undefined = explicitDeploymentTarget ?? undefined;
    const linkedAccounts: LinkedAccountFlagsConfig | undefined = normalizeLinkedAccountFlagsConfig(body.linkedAccounts) ?? undefined;
    const serviceRouting: ServiceRoutingConfig | undefined = normalizeServiceRoutingConfig(body.serviceRouting) ?? undefined;
