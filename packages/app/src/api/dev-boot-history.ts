...
import { getLastFailedPluginDetails } from "@elizaos/agent";
import { isDevApiWatchEnabled } from "@elizaos/core/runtime-env";
import { resolveStateDir } from "@elizaos/core";
import { type FailedPluginDetail } from "@elizaos/agent";
...
export async function buildBootHistoryPayload(env: NodeJS.ProcessEnv = process.env): Promise<BootHistoryPayload> {
    const tel = (...segments: string[]): string => path.join(resolveStateDir(env), "telemetry", ...segments);
