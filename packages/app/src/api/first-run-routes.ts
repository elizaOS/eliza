...
import { applyCanonicalFirstRunConfig } from "@elizaos/agent";
import { createHash } from "node:crypto";
import { deriveFirstRunReplayBody } from "./server-first-run-helpers";
...
import { hasDeprecatedFirstRunRequestFields } from "./server-first-run-helpers";
import { isRuntimeBootDeferred } from "./deferred-runtime-boot";
...
import { triggerDeferredRuntimeBoot } from "./deferred-runtime-boot";
import { type CompatRuntimeState } from "./compat-route-shared";
import { type DeploymentTargetRuntime } from "@elizaos/core/contracts/service-routing";
...
class FirstRunCommitError extends ElizaError {
    constructor(readonly result: Extract<FirstRunCommitResult, {
        ok: false;
    }>) {
        super(result.error, {
...
async function syncFirstRunConfigState(req: http.IncomingMessage, config: Record<string, unknown>, includeCloudTopology = true): Promise<void> {
    ...
    if (!response.ok) {
        throw new Error(`Loopback config sync failed (${response.status}): ${await response.text()}`);
    }
}
/**
...
function scheduleCloudApiKeyResave(apiKey: string): void {
    ...
    setTimeout(() => {
        try {
            ...
            logger.warn(`[api] Defensive cloud.apiKey resave failed: ${err instanceof Error ? err.message : String(err)}`);
        }
 ...
 */
function resolveCloudApiKeyForFirstRun(config: Record<string, unknown>, devCloudAuthority = resolveDevCloudEnvAuthority()): string | undefined {
    if (devCloudAuthority) {
        const launchValue = resolveDevCloudAuthorityEnvValue("ELIZAOS_CLOUD_API_KEY")?.trim();
...
function withoutAuthorityOwnedCloudCredential(body: Record<string, unknown>, devCloudAuthority: ReturnType<typeof resolveDevCloudEnvAuthority>): Record<string, unknown> {
    if (!devCloudAuthority)
        return body;
...
export async function handleFirstRunRoute(req: http.IncomingMessage, res: http.ServerResponse, state: CompatRuntimeState): Promise<boolean> {
    const method = (req.method ?? "GET").toUpperCase();
    const url = new URL(req.url ?? "/", "http://localhost");
    const activationMatch = method === "GET"
    ...
    if (activationMatch) {
        ...
        if (operation?.intent.kind !== "restart" ||
        ...
        sendJsonResponse(res, 200, {
            ...
            error: operation.status === "failed" || operation.status === "rolled-back"
    ...
    const persist = async (commitBody: Record<string, unknown>): Promise<FirstRunCommitResult> => {
        try {
            if (hasDeprecatedFirstRunRequestFields(commitBody)) {
                return {
                    ok: false,
                    status: 400,
                    error: "deprecated first-run payloads are no longer supported; send deploymentTarget, linkedAccounts, serviceRouting, and credentialInputs",
                };
            }
            const connectorPreparation = prepareFirstRunConnectors(loadElizaConfig(), commitBody);
            if (!connectorPreparation.ok) {
                return { ok: false, status: 400, error: connectorPreparation.error };
            }
            await extractAndPersistFirstRunApiKey(withoutAuthorityOwnedCloudCredential(commitBody, devCloudAuthority), configRollback.record, configRollback.observeEnvironmentMutation);
            ...
            const { replayBody: replayBodyRecord } = deriveFirstRunReplayBody(commitBody);
            ...
            const replayLinkedAccounts = normalizeLinkedAccountFlagsConfig(replayBodyRecord.linkedAccounts);
            const replayServiceRouting = normalizeServiceRoutingConfig(replayBodyRecord.serviceRouting);
            const cloudInferenceSelected = Boolean(replayServiceRouting?.llmText?.transport === "cloud-proxy" &&
                normalizeFirstRunProviderId(replayServiceRouting.llmText.backend) ===
                    "elizacloud");
            const shouldResolveCloudApiKey = replayDeploymentTarget?.runtime === "cloud" ||
            ...
            try {
                ...
                if (shouldResolveCloudApiKey) {
                    resolvedCloudApiKey = resolveCloudApiKeyForFirstRun(config as Record<string, unknown>, devCloudAuthority);
                    if (!resolvedCloudApiKey) {
                        logger.warn(devCloudAuthority
                        ...
                        logger.info("[api] Cloud-linked first-run: resolved API key, injecting into replay body");
                    }
                    capturedCloudApiKey = resolvedCloudApiKey;
                }
                const currentConnectorPreparation = prepareFirstRunConnectors(config, commitBody);
                ...
                logger.error(`[api] Failed to persist first-run state: ${err instanceof Error ? err.message : String(err)}`);
                return {
                    ok: false,
                    status: 500,
            ...
            logger.error(`[api] First-run helper failed after valid JSON: ${err instanceof Error ? err.message : String(err)}`);
            return {
                ok: false,
                status: 500,
    ...
    const commit = async (): Promise<FirstRunCommitResult> => {
        ...
        const provider = getDirectAccountProviderForFirstRunProvider(routing?.llmText?.backend);
        const key = normalizeFirstRunCredentialInputs(body.credentialInputs)?.llmApiKey;
        const rollback = async (): Promise<void> => {
            ...
            if (configRollback.hasWrites) {
                ...
                if (!state.reloadConfigFromDisk) {
                    throw new ElizaError("First-run rollback cannot refresh host configuration", {
                        code: "FIRST_RUN_ROLLBACK_UNAVAILABLE",
                    });
                }
                state.reloadConfigFromDisk();
            }
            if (typeof persistenceBody.name === "string" &&
            ...
            const routes = normalizeServiceRoutingConfig(currentConfig.serviceRouting);
            const concurrentlyReferenced = Object.values(routes ?? {}).some((route) => route?.accountId === owned.account.id ||
                route?.accountIds?.includes(owned.account.id));
            if (concurrentlyReferenced) {
                logger.info("[api] Retained first-run account selected by a concurrent settings change");
            }
            else
                await owned.rollback();
            adoption = null;
            const { syncDirectProviderCredentials } = await import("@elizaos/agent/api/accounts-routes");
            await syncDirectProviderCredentials({ state: { config: loadElizaConfig(), runtime: state.current } }, owned.account.providerId);
        };
        try {
            if (routing?.llmText?.transport === "direct" &&
                key &&
                ...
                if (routing.llmText.accountId || routing.llmText.accountIds?.length) {
                    return {
                        ok: false,
                        status: 400,
                        error: "Choose an existing account or enter a new provider key, not both.",
                    };
                }
                const { adoptFirstRunDirectAccount } = await import("@elizaos/agent/api/first-run-direct-account");
            ...
            if (!result.ok)
                throw new FirstRunCommitError(result);
            if (adoption) {
                const { syncDirectProviderCredentials } = await import("@elizaos/agent/api/accounts-routes");
                await syncDirectProviderCredentials({ state: { config: loadElizaConfig(), runtime: state.current } }, adoption.account.providerId);
            }
            ...
            try {
                ...
                return {
                    ok: false,
                    status: 500,
                    error: "First-run setup and rollback failed. Inspect runtime diagnostics before retrying.",
                };
            }
            if (err instanceof FirstRunCommitError)
                return err.result;
            logger.error({ err }, "[api] First-run provider account activation failed");
            return {
                ok: false,
                status: err instanceof ElizaError &&
                    err.code === "FIRST_RUN_DIRECT_CREDENTIAL_INVALID"
                    ? 400
                    : 500,
                error: err instanceof ElizaError &&
                    err.code === "FIRST_RUN_DIRECT_CREDENTIAL_INVALID"
    ...
    const requiresActivation = state.current !== null &&
    ...
    if (requiresActivation) {
        ...
        if (!operations) {
            sendJsonResponse(res, 503, {
                error: "Runtime activation is not available. Retry after the local host is ready.",
            });
            return true;
        }
        ...
        try {
            ...
            if (outcome.kind === "rejected-busy") {
                sendJsonResponse(res, 409, {
                    error: "A runtime operation is already in progress. Retry setup when it finishes.",
            ...
            sendJsonResponse(res, 202, {
                ...
                activation: {
                    ...
                    error: outcome.operation.status === "failed" ||
            ...
            if (err instanceof FirstRunCommitError) {
                ...
                logger.error({ err }, "[api] First-run runtime activation could not be scheduled");
    ...
    if (isRuntimeBootDeferred() &&
        ...
        triggerDeferredRuntimeBoot("first-run onboarding committed").catch((err) => {
            ...
            logger.error(`[api] Deferred runtime boot after first-run commit failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
        });
    }
    return true;
}
