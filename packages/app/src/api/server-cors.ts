...
import { resolveAllowedOrigins } from "@elizaos/core/runtime-env";
import { resolveRuntimePorts } from "@elizaos/core/runtime-env";
...
export function getAllowedRemoteOrigins(): Set<string> {
    ...
    return new Set(resolveAllowedOrigins(process.env).map((origin) => {
        try {
 ...
 */
export function isAllowedOrigin(urlStr: string, allowedPorts?: Set<string>, allowedRemoteOrigins?: Set<string>): boolean {
