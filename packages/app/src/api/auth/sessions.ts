...
import { type AppendAuditEventInput } from "../../services/auth-store";
import { type AuthRepository } from "../../services/auth-store";
import { type AuthSessionRow } from "../../services/auth-store";
 ...
 * `eliza_csrf` cookie.
 */
export async function createBrowserSession(store: AuthRepository, options: CreateBrowserSessionOptions): Promise<SessionWithCsrf> {
 ...
 */
export async function createMachineSession(store: AuthRepository, options: CreateMachineSessionOptions): Promise<SessionWithCsrf> {
    const now = options.now ?? Date.now();
    const id = generateSessionId();
    const csrfSecret = generateCsrfSecret();
    ...
    return (error) => {
        logger.error({
            ...
            cause: error instanceof Error && error.cause
                ? String(error.cause instanceof Error
                    ? error.cause.message
                    : error.cause).slice(0, 240)
                : "(no cause)",
 ...
 */
export async function findActiveSession(store: AuthRepository, sessionId: string, now: number = Date.now()): Promise<AuthSessionRow | null> {
...
export async function revokeSession(sessionId: string, options: RevokeSessionOptions): Promise<boolean> {
    const now = options.now ?? Date.now();
...
export async function revokeAllSessionsForIdentity(options: RevokeAllSessionsOptions): Promise<number> {
    const now = options.now ?? Date.now();
 ...
 */
export function verifyCsrfToken(session: {
    id: string;
    csrfSecret: string;
 ...
 * `x-eliza-csrf` header.
 */
export function serializeCsrfCookie(session: {
    id: string;
    csrfSecret: string;
    expiresAt: number;
}, options: SerializeSessionCookieOptions = {}): string {
...
export function serializeSessionExpiryCookie(options: SerializeSessionCookieOptions = {}): string {
    const env = options.env ?? process.env;
    const parts = [
        `${SESSION_COOKIE_NAME}=`,
        "Path=/",
...
export function serializeCsrfExpiryCookie(options: SerializeSessionCookieOptions = {}): string {
    const env = options.env ?? process.env;
 ...
 */
export function parseSessionCookie(req: Pick<http.IncomingMessage, "headers">): string | null {
