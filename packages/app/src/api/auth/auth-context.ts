...
import { findActiveSession } from "./sessions.js";
...
import { parseSessionCookie } from "./sessions.js";
import { proxyClientHeaderBlocksLocalTrust } from "@elizaos/agent/api/loopback-trust";
import { type AuthIdentityRow } from "../../services/auth-store";
...
function sessionAllowedForRequest(session: AuthSessionRow, req: Pick<http.IncomingMessage, "headers" | "socket">): boolean {
    if (!session.scopes.includes(DESKTOP_LOOPBACK_SESSION_SCOPE))
        return true;
 ...
 * for sending the 401.
 */
export async function ensureSessionForRequest(req: Pick<http.IncomingMessage, "headers" | "socket">, _res: http.ServerResponse, options: EnsureSessionOptions): Promise<ResolvedAuthContext | null> {
    const { store } = options;
    const now = options.now ?? Date.now();
    const allowBootstrap = options.allowBootstrapBearer ?? true;
    const handleStoreFailure = (error: unknown): null => {
        if (options.storeFailureMode === "throw")
            throw error;
        return null;
    };
    // 1. cookie session
    ...
    if (cookieSessionId) {
        const session = await findActiveSession(store, cookieSessionId, now).catch(handleStoreFailure);
    ...
    if (bearer) {
        ...
        const session = await findActiveSession(store, bearer, now).catch(handleStoreFailure);
        if (session && sessionAllowedForRequest(session, req)) {
            const identity = await store
                .findIdentity(session.identityId)
                .catch(handleStoreFailure);
