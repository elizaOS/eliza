...
import { readCachedJwks } from "../../services/cloud-jwks-store";
import { type AuthRepository } from "../../services/auth-store";
import { type JwksDocument } from "../../services/cloud-jwks-store";
import { type RuntimeEnvRecord } from "@elizaos/core/runtime-env";
import { writeCachedJwks } from "../../services/cloud-jwks-store";
...
async function loadJwks(issuer: string, options: VerifyOptions): Promise<JwksDocument | null> {
 ...
 */
export async function verifyBootstrapToken(token: string, options: VerifyOptions): Promise<VerifyBootstrapResult> {
    const env = options.env ?? process.env;
