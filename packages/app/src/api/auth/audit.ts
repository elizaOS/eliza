 ...
 */
export function redactMetadata(metadata: Record<string, string | number | boolean>): Record<string, string | number | boolean> {
...
export function resolveAuditLogRotatedPath(env: RuntimeEnvRecord = process.env): string {
    return path.join(resolveElizaStateDir(env), "auth", AUDIT_LOG_ROTATE_FILENAME);
}
 ...
 * swallowed.
 */
export async function appendAuditEvent(input: AuditEventInput, options: AuditEmitterOptions): Promise<void> {
