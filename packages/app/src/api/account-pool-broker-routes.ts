...
import { AccountPoolBroker } from "../services/account-pool-broker.js";
import { createAccountPoolConsumerKey } from "../services/account-pool-consumer-metering.js";
import { isLoopbackRemoteAddress } from "@elizaos/agent/api/loopback-trust";
import { listAccountPoolConsumerKeys } from "../services/account-pool-consumer-metering.js";
import { parseBrokerLeaseRequest } from "../services/account-pool-broker.js";
import { parseBrokerReleaseRequest } from "../services/account-pool-broker.js";
import { parseBrokerReportRequest } from "../services/account-pool-broker.js";
import { parseCanonicalInteger } from "@elizaos/core/utils/number-parsing";
import { queryAccountPoolConsumerUsage } from "../services/account-pool-consumer-metering.js";
import { readCompatJsonBody } from "./compat-route-shared.js";
import { rotateAccountPoolConsumerKey } from "../services/account-pool-consumer-metering.js";
import { sendJson } from "./response.js";
import { type AccountPoolBrokerSnapshot } from "@elizaos/core";
import { updateAccountPoolConsumerKey } from "../services/account-pool-consumer-metering.js";
...
function brokerEnabled(): boolean {
    const enabled = process.env.ELIZA_ACCOUNT_POOL_BROKER_ENABLED?.trim().toLowerCase();
    if (enabled !== "1" &&
        enabled !== "true" &&
        enabled !== "yes" &&
        enabled !== "on") {
...
function sendBrokerJson(res: http.ServerResponse, status: number, body: unknown): void {
    res.setHeader("Cache-Control", "no-store");
    sendJson(res, status, body);
}
function methodAllowed(method: string, expected: "GET" | "POST" | "PATCH", res: http.ServerResponse): boolean {
    if (method === expected)
        return true;
...
export async function handleAccountPoolBrokerRoute(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {
    ...
    const consumerKeyMatch = url.pathname.match(new RegExp(`^${ROUTE_PREFIX}/consumer-keys/([^/]+)(/rotate)?$`));
    if (consumerKeyMatch) {
        ...
        if (!methodAllowed(method, "PATCH", res))
            return true;
        const body = await readCompatJsonBody(req, res);
        if (body === null)
            return true;
