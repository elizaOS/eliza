...
function parseInferenceTimingLog(log: Log): InferenceTurnSummary | null {
    ...
    const spans = Array.isArray(metadata.spans)
        ? metadata.spans.flatMap((value) => {
            ...
            const meta = Object.fromEntries(rawMetaEntries.filter((entry): entry is [
                string,
    ...
    return {
        ...
        anomalies: Array.isArray(metadata.anomalies)
            ? metadata.anomalies.filter((value): value is string => typeof value === "string")
            : [],
    };
}
/**
 ...
 */
export async function handleDevCompatRoutes(req: http.IncomingMessage, res: http.ServerResponse, state: CompatRuntimeState): Promise<boolean> {
    ...
    if (method === "GET" && url.pathname === "/api/dev/cursor-screenshot") {
        ...
        try {
            ...
            if (!r.ok) {
                ...
                sendJsonResponse(res, r.status === 401 || r.status === 403 ? r.status : 502, {
    ...
    if (method === "GET" && url.pathname === "/api/dev/console-log") {
        ...
        if (maxLines === "invalid" || maxBytes === "invalid") {
            sendJsonErrorResponse(res, 400, "maxLines and maxBytes must be canonical positive integers");
            return true;
        }
    ...
    if (method === "GET" && url.pathname === "/api/dev/voice-latency") {
        ...
        if (limit === "invalid") {
            sendJsonErrorResponse(res, 400, "limit must be a canonical positive integer");
            return true;
        }
    ...
    if (method === "GET" && url.pathname === "/api/dev/device-resource-metrics") {
        ...
        const { buildDeviceResourceMetricsDevPayload } = await import("@elizaos/plugin-local-inference/services");
    ...
    if (method === "GET" && url.pathname === "/api/dev/inference-timing") {
        ...
        const persistedTurns = state.current
            ? (await state.current.getLogs({
                ...
                .filter((summary): summary is InferenceTurnSummary => summary !== null)
            : [];
