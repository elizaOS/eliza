...
import { MODEL_PROVIDER_SECRETS } from "@elizaos/core";
import { SECRET_KEY_ALIASES } from "@elizaos/core";
...
import { getDirectAccountProviderForFirstRunProvider } from "@elizaos/core/contracts/first-run-options";
import { getFirstRunProviderOption } from "@elizaos/core/contracts/first-run-options";
import { getStoredSubscriptionProviderForRequest } from "@elizaos/core/contracts/first-run-options";
import { isDirectAccountProvider } from "@elizaos/auth/auth/types";
import { listProviderAccounts } from "@elizaos/auth/auth/credentials";
...
function sourceForProvider(providerId: string): CredentialSource | null {
    ...
    if (!canonicalEnvVar?.endsWith("_API_KEY"))
        return null;
    return {
...
function directAccountProviderForRequest(providerId: string): DirectAccountProvider | null {
    const normalized = providerId.trim().toLowerCase();
    if (isDirectAccountProvider(normalized))
        return normalized;
    const firstRunProvider = normalizeFirstRunProviderId(providerId);
    if (!firstRunProvider)
        return null;
    const directProvider = getDirectAccountProviderForFirstRunProvider(firstRunProvider);
 ...
 */
export function resolveProviderCredential(providerId: string): ResolvedCredential | null {
    const source = sourceForProvider(providerId);
    if (!source)
        return null;
    const resolved = readFirstEnvValue(source.envVars);
    if (resolved) {
        logger.info(`[credential-resolver] Resolved ${resolved.envVar} for ${providerId} (${resolved.value.length} chars, ${source.authType})`);
        return {
 ...
 */
export async function resolveProviderCredentialMulti(providerId: string, opts?: {
    sessionKey?: string;
    exclude?: string[];
}): Promise<ResolvedCredential | null> {
    const subscriptionProvider = subscriptionProviderForRequest(providerId);
    if (subscriptionProvider) {
        logger.info(`[credential-resolver] Refusing to expose ${providerId} as a direct API credential; subscription coding plans must use their first-party coding surface.`);
        return null;
    }
