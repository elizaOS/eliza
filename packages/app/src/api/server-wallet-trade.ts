...
import { type WalletExportRejection as CompatWalletExportRejection } from "@elizaos/core/contracts/wallet-types";
import { type WalletExportRequestBody } from "@elizaos/core/contracts/wallet-types";
type UpstreamRejectionFn = (req: http.IncomingMessage, body: WalletExportRequestBody) => CompatWalletExportRejection | null;
...
const pendingExportNonces = new Map<string, {
    issuedAt: number;
    ip: string;
}>();
...
export function normalizeCompatRejection<T extends {
    status: number;
    reason: string;
} | null>(rejection: T): T {
    if (!rejection) {
...
export function runWithCompatAuthContext<T>(req: Pick<http.IncomingMessage, "headers">, operation: () => T): T {
...
function validateExportNonce(nonce: string, ip: string): {
    valid: true;
} | {
    reason: string;
    valid: false;
...
function createHardenedExportGuard(upstream: UpstreamRejectionFn): (req: http.IncomingMessage, body: HardenedExportRequestBody) => CompatWalletExportRejection | null {
    return (req: http.IncomingMessage, body: HardenedExportRequestBody): CompatWalletExportRejection | null => {
...
function resolveCompatWalletExportRejection(...args: Parameters<typeof upstreamResolveWalletExportRejection>): CompatWalletExportRejection | null {
    const [req] = args;
    return runWithCompatAuthContext(req, () => normalizeCompatRejection(upstreamResolveWalletExportRejection(...args)));
}
const hardenedGuard = createHardenedExportGuard(resolveCompatWalletExportRejection);
export function resolveWalletExportRejection(...args: Parameters<typeof upstreamResolveWalletExportRejection>): CompatWalletExportRejection | null {
    const [req] = args;
