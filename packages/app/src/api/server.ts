...
import { AGENT_EVENT_ALLOWED_STREAMS } from "@elizaos/agent";
import { CONFIG_WRITE_ALLOWED_TOP_KEYS } from "@elizaos/agent";
import { CORS_ALLOWED_HEADERS } from "./server-cors";
import { DIRECT_ACCOUNT_PROVIDER_ENV } from "@elizaos/auth/auth/types";
import { PLUGIN_REGISTRY_LOAD_DEADLINE_MS } from "./plugin-registry-load-deadline";
...
import { clearCloudSecrets } from "@elizaos/plugin-elizacloud/cloud-config/cloud-secrets";
import { clearCompatRuntimeRestart } from "./compat-route-shared";
import { clearPersistedFirstRunConfig } from "@elizaos/agent";
import { cloneWithoutBlockedObjectKeys } from "@elizaos/agent";
import { createRequire } from "node:module";
import { createRuntimeAccountStoragePolicy } from "@elizaos/auth/auth/account-storage";
import { decodePathComponent } from "@elizaos/agent";
import { deleteWalletSecretsFromOsStore } from "../security/wallet-os-store-actions";
import { discoverInstalledPlugins } from "@elizaos/agent";
import { discoverPluginsFromManifest } from "@elizaos/agent";
import { enforceCompatRouteAuthPolicy } from "./route-auth-policy";
import { ensureCloudTtsApiKeyAlias } from "@elizaos/plugin-elizacloud/cloud-config/server-cloud-tts";
...
import { extractAuthToken } from "@elizaos/agent";
...
import { getConfiguredCompatAgentName } from "./compat-route-shared";
import { getCorsAllowedPorts } from "./server-cors";
...
import { handleCloudBillingRoute } from "@elizaos/agent";
import { handleCloudCompatRoute } from "@elizaos/agent";
...
import { handleRuntimeModePreDispatch } from "@elizaos/agent";
import { handleRuntimeModeRemoteForward } from "@elizaos/agent";
...
import { isAllowedHost } from "@elizaos/agent";
import { isAllowedOrigin } from "./server-cors";
import { isAuthorized } from "@elizaos/agent";
import { isCredentialedCorsOrigin } from "@elizaos/agent";
...
import { isFeatureRouteHandlerAvailable } from "./feature-route-readiness.js";
import { isNodePlatformSecureStoreDefaultAvailable } from "../security/platform-secure-store-node";
import { isPerfInstrumentEnabled } from "./perf-instrument";
import { loadEffectiveElizaConfig } from "@elizaos/agent";
import { loadElizaConfig } from "@elizaos/agent";
import { logger } from "@elizaos/core";
import { mirrorCompatHeaders } from "@elizaos/plugin-elizacloud/cloud-config/server-cloud-tts";
import { normalizeRouteKey } from "./perf-instrument";
import { normalizeWsClientId } from "@elizaos/agent";
import { persistConversationRoomTitle } from "@elizaos/agent";
import { readCookie } from "./auth";
import { recordRouteTiming } from "./perf-instrument";
...
import { resolveMcpServersRejection } from "@elizaos/agent";
import { resolvePluginConfigMutationRejections } from "@elizaos/agent";
import { resolveSessionTokenRole } from "./auth";
import { resolveStateDir } from "@elizaos/core";
import { resolveUserPath } from "@elizaos/agent";
import { resolveWithinDeadline } from "./plugin-registry-load-deadline";
import { routeAutonomyTextToUser } from "@elizaos/agent";
import { runCompatRouteChain } from "./compat-route-shared";
import { saveElizaConfig } from "@elizaos/agent";
import { sendJson as sendJsonResponse } from "./response";
import { settingsDebugCloudSummary } from "@elizaos/core/settings-debug";
import { startApiServer as upstreamStartApiServer } from "@elizaos/agent";
import { streamResponseBodyWithByteLimit } from "@elizaos/agent";
import { type AgentRuntime } from "@elizaos/core";
import { type CompatRouteChainEntry } from "./compat-route-shared";
import { type CompatRouteContext } from "./compat-route-shared";
import { type CompatRuntimeState } from "./compat-route-shared";
import { type ConversationMeta } from "@elizaos/agent";
import { type ElizaConfig } from "@elizaos/agent";
import path from "node:path";
// Override the wallet export rejection function with the hardened version
// that adds rate limiting, audit logging, and a forced confirmation delay.
export { __resetCloudBaseUrlCache, ensureCloudTtsApiKeyAlias, resolveCloudTtsBaseUrl, resolveElevenLabsApiKeyForCloudMode, } from "@elizaos/plugin-elizacloud/cloud-config/server-cloud-tts";
...
let _localInferenceRoutes: typeof import("@elizaos/plugin-local-inference/routes") | undefined;
async function getLocalInferenceRoutes() {
    if (!_localInferenceRoutes) {
        _localInferenceRoutes = await import("@elizaos/plugin-local-inference/routes");
    }
...
function hydrateWalletOsStoreFlagFromConfig(): void {
    ...
    const persistedEnv = config.env && typeof config.env === "object" && !Array.isArray(config.env)
 ...
 */
async function clearCompatPgliteDataDir(runtime: AgentRuntime | null, config: ElizaConfig): Promise<void> {
            ...
            await Promise.race([
                ...
                new Promise<void>((_resolve, reject) => {
                    timeoutHandle = setTimeout(() => {
                        reject(new Error(`runtime.stop() exceeded ${RUNTIME_STOP_RESET_TIMEOUT_MS}ms`));
...
function resolveCompatStatusAgentName(state: CompatRuntimeState): string | null {
...
function mergeEmbeddingIntoStatusPayload(payload: Record<string, unknown>): void {
...
function rewriteCompatStatusBody(bodyText: string, state: CompatRuntimeState): string {
    ...
    try {
        ...
        const upstreamPendingRestartReasons = Array.isArray(payload.pendingRestartReasons)
            ? payload.pendingRestartReasons.filter((value): value is string => typeof value === "string")
            : [];
        const pendingRestartReasons = Array.from(new Set([
            ...upstreamPendingRestartReasons,
            ...state.pendingRestartReasons,
        ]));
        if (pendingRestartReasons.length > 0 ||
...
function patchCompatStatusResponse(req: http.IncomingMessage, res: http.ServerResponse, state: CompatRuntimeState): void {
    ...
    res.end = ((chunk?: string | Uint8Array, encoding?: unknown, cb?: unknown) => {
        ...
        return originalEnd(rewriteCompatStatusBody(bodyText, state), "utf8", resolvedCallback);
    }) as typeof res.end;
}
/**
...
function resolveCloudConfig(runtime?: unknown): ElizaConfig {
    ...
    const cloudRec = config.cloud && typeof config.cloud === "object"
        ? (config.cloud as Record<string, unknown>)
        : undefined;
    if (isElizaSettingsDebugEnabled()) {
        logger.debug(`[eliza][settings][compat] resolveCloudConfig disk cloud=${JSON.stringify(settingsDebugCloudSummary(cloudRec))} topKeys=${Object.keys(config as object)
            .sort()
            .join(",")}`);
    }
    if (isDevCloudConfigAuthorityView(config)) {
        if (isElizaSettingsDebugEnabled()) {
            logger.debug("[eliza][settings][compat] resolveCloudConfig using launcher-owned ephemeral Cloud view");
        }
        return config;
    }
    const linkedAccounts = resolveLinkedAccountsInConfig(config as Record<string, unknown>);
    if (linkedAccounts?.elizacloud?.status === "unlinked") {
        ...
        if (isElizaSettingsDebugEnabled()) {
            logger.debug("[eliza][settings][compat] resolveCloudConfig skip backfill (linkedAccounts.elizacloud.status===unlinked)");
        }
        return config;
    }
    if (!config.cloud?.apiKey) {
        // Try multiple sources: sealed secrets → process.env → runtime character secrets
        const backfillKey = getCloudSecret("ELIZAOS_CLOUD_API_KEY") ||
        ...
        if (backfillKey) {
            if (isElizaSettingsDebugEnabled()) {
                logger.debug("[eliza][settings][compat] resolveCloudConfig backfilling cloud.apiKey from env/secrets/runtime");
            }
            if (!config.cloud) {
...
async function handleCompatRoute(req: http.IncomingMessage, res: http.ServerResponse, state: CompatRuntimeState): Promise<boolean> {
    if (!isPerfInstrumentEnabled()) {
        return handleCompatRouteInner(req, res, state);
    }
    const start = performance.now();
    const url = new URL(req.url ?? "/", "http://localhost");
    const routeKey = normalizeRouteKey((req.method ?? "GET").toUpperCase(), url.pathname);
...
async function handleCompatRouteInner(req: http.IncomingMessage, res: http.ServerResponse, state: CompatRuntimeState): Promise<boolean> {
    const method = (req.method ?? "GET").toUpperCase();
    const url = new URL(req.url ?? "/", "http://localhost");
    ...
    if (await handleRuntimeModePreDispatch(req, res, state.current))
        return true;
    const authPolicyDecision = await enforceCompatRouteAuthPolicy(req, res, state, method, url.pathname);
    if (authPolicyDecision === "denied")
        return true;
    if (authPolicyDecision === "unmanaged")
        return false;
    ...
    if (await handleRuntimeModeRemoteForward(req, res))
        return true;
...
const COMPAT_ROUTE_CHAIN: readonly CompatRouteChainEntry[] = [
    ...
    },
    {
        ...
        handler: ({ req, res, state }) => handleDesktopAuthBootstrapRoute(req, res, state),
    },
    {
        // Must precede the auth-pairing handler so the rate-limited route owns
        // /api/auth/bootstrap/exchange.
        id: "auth-bootstrap",
        handler: ({ req, res, state }) => handleAuthBootstrapRoutes(req, res, state),
    },
    {
    ...
    },
    {
        ...
        id: "auth-pairing",
        handler: ({ req, res, state }) => handleAuthPairingCompatRoutes(req, res, state),
    },
    {
    ...
    },
    {
        ...
        handler: ({ req, res, state }) => handleSensitiveRequestRoutes(req, res, state),
    },
    {
        // Public-safe anonymous capacity status for the first-class account pool.
        id: "account-pool-status",
        handler: ({ req, res, method, url }) => handleAccountPoolStatusRoute(req, res, method, url.pathname),
    },
    {
        id: "credential-tunnel",
        handler: ({ req, res, state }) => handleCredentialTunnelRoute(req, res, state),
    },
    {
        id: "background-tasks",
        handler: ({ req, res, state }) => handleBackgroundTasksRoute(req, res, state),
    },
    {
    ...
    },
    {
        ...
        handler: async ({ req, res, state }) => {
            const { handleLiveDiarizationRoute, handleLocalInferenceAsrRoute, handleLocalInferenceCompatRoutes, handleLocalInferenceTtsRoute, } = await getLocalInferenceRoutes();
            if (await handleLocalInferenceCompatRoutes(req, res, state))
                return true;
            if (await handleLocalInferenceAsrRoute(req, res, state))
                return true;
            if (await handleLocalInferenceTtsRoute(req, res, state))
                return true;
    ...
    },
    {
        ...
        handler: async ({ req, res, state, method, url }) => {
            ...
            if (await handleSecretsInventoryRoute(req, res, url.pathname, method, state)) {
                return true;
            }
    ...
    },
    {
        ...
        id: "drop-status",
        handler: ({ req, res, method, url }) => handleDropStatusCompatRoute(req, res, method, url.pathname),
    },
    {
        id: "agent-reset",
        handler: async ({ req, res, state, method, url }) => {
            ...
            try {
                logger.info("[eliza][reset] POST /api/agent/reset: loading config, will clear first-run state, persisted provider config, and cloud keys (GGUF / MODELS_DIR untouched)");
                const config = loadElizaConfig();
                logger.info("[eliza][reset] Skipping loopback API cleanup; runtime stop plus PGlite data-dir removal clears conversations, knowledge, and trajectories without re-entering the HTTP server.");
                const runtimeBeforeReset = state.current;
                await clearCompatPgliteDataDir(runtimeBeforeReset, config);
                state.current = null;
                clearPersistedFirstRunConfig(config, createRuntimeAccountStoragePolicy(resolveStateDir()));
                ...
                logger.info("[eliza][reset] POST /api/agent/reset: eliza.json saved; renderer should restart API process if embedded/third-party dev");
    ...
    },
    {
        ...
        handler: async ({ req, res, state, method, url }) => {
            ...
            sendJsonResponse(res, 200, _filterConfigEnvForResponse(loadElizaConfig() as Record<string, unknown>));
            return true;
        },
    },
];
export async function handleElizaCompatRoute(req: http.IncomingMessage, res: http.ServerResponse, state: CompatRuntimeState): Promise<boolean> {
    return handleCompatRoute(req, res, state);
}
async function runCompatRequestPipeline(req: http.IncomingMessage, res: http.ServerResponse, state: CompatRuntimeState, next: () => Promise<void>): Promise<void> {
    ...
    }
    {
        ...
        try {
            ...
            // error-policy:J1 HTTP middleware boundary — translate an app
    ...
    const readinessFailure = resolveFeatureRouteReadinessFailure(pathname, state.current !== null, deferredBoot.phases, isFeatureRouteHandlerAvailable({
...
export async function startApiServer(...args: Parameters<typeof upstreamStartApiServer>): Promise<Awaited<ReturnType<typeof upstreamStartApiServer>>> {
    ...
    const server = await upstreamStartApiServer({
        ...
        authorizeWebSocket: async (request, url) => {
            ...
            const sessionToken = url.searchParams.get("token")?.trim() ||
            ...
            if (sessionToken) {
                if (compatState.current?.adapter) {
                    try {
                        ...
                        logger.error(`[eliza][auth] WebSocket session lookup failed: ${error instanceof Error ? error.message : String(error)}`);
                        compatState.current?.reportError("appCore.webSocketSessionAuth", error, { phase: "upgrade" });
                    }
                }
            }
    ...
    logger.info(`[eliza-api] upstreamStartApiServer took ${Date.now() - upstreamStart}ms`);
    compatState.runtimeOperations = server.runtimeOperations;
    compatState.reloadConfigFromDisk = server.reloadConfigFromDisk;
    const originalUpdateRuntime = server.updateRuntime as (runtime: AgentRuntime) => void;
