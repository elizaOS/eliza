...
import { CSRF_HEADER_NAME } from "./auth/sessions.js";
import { authStoreForRuntime } from "../services/auth-store.js";
import { denyOnAuthStoreError } from "./auth/sessions.js";
import { extractHeaderValue } from "./auth/tokens.js";
import { findActiveSession } from "./auth/sessions.js";
import { getProvidedApiToken } from "./auth/tokens.js";
import { isTrustedLocalRequest } from "./compat-route-shared.js";
import { readEmbedSessionSecretSetting } from "./auth/embed-session-token.js";
import { resolveApiToken } from "@elizaos/core/runtime-env";
import { resolveEmbedSessionSecret } from "./auth/embed-session-token.js";
import { roleRank } from "@elizaos/core";
import { sendJsonError } from "./response.js";
import { tokenMatches } from "./auth/tokens.js";
import { type AuthIdentityRow } from "../services/auth-store.js";
import { type AuthRepository } from "../services/auth-store.js";
import { type AuthRuntimeSource } from "../services/auth-store.js";
import { type EmbedSessionClaims } from "./auth/embed-session-token.js";
import { type EmbedSessionSecretRuntime } from "./auth/embed-session-token.js";
import { type RoleGateRole } from "@elizaos/core";
import { verifyCsrfToken } from "./auth/sessions.js";
import { verifyEmbedSessionToken } from "./auth/embed-session-token.js";
// AuthStore is statically imported elsewhere in the package; the dynamic
// import below was INEFFECTIVE_DYNAMIC_IMPORT.
export { type AuthContextSource, type EnsureSessionOptions, ensureSessionForRequest, type ResolvedAuthContext, } from "./auth/auth-context.js";
export { extractHeaderValue, getProvidedApiToken, tokenMatches, } from "./auth/tokens.js";
 ...
 * its own env source.
 */
export function resolveEmbedPrincipal(req: Pick<http.IncomingMessage, "headers">, now?: number, read: (key: string) => unknown = (key) => process.env[key]): EmbedSessionClaims | null {
 ...
 */
export function embedBoundaryRole(claims: EmbedSessionClaims | null): RoleGateRole | null {
    if (!claims)
        return null;
 ...
 */
export function ensureCompatApiAuthorized(req: Pick<http.IncomingMessage, "headers" | "socket">, res: http.ServerResponse): boolean {
 ...
 * once per handler.
 */
export async function ensureCompatApiAuthorizedAsync(req: Pick<http.IncomingMessage, "headers" | "socket" | "method">, res: http.ServerResponse, options: {
 ...
 */
export function readCookie(req: Pick<http.IncomingMessage, "headers">, name: string): string | null {
 ...
 */
export function ensureAuthSessionOrBootstrap(req: Pick<http.IncomingMessage, "headers" | "socket">): AuthSessionOrBootstrapResult {
    const ip = req.socket.remoteAddress ?? null;
    if (isAuthRateLimited(ip)) {
 ...
 * `"NONE"` (rank 0).
 */
function resolveBoundaryRole(req: Pick<http.IncomingMessage, "headers" | "socket">, env: NodeJS.ProcessEnv = process.env): RoleGateRole {
    if (isTrustedLocalRequest(req)) {
        return "OWNER";
    }
 ...
 */
function ensureMinRole(req: Pick<http.IncomingMessage, "headers" | "socket">, minRole: RoleGateRole, env: NodeJS.ProcessEnv = process.env): boolean {
 ...
 */
export function roleForIdentityKind(kind: AuthIdentityRow["kind"] | null | undefined): RoleGateRole {
    if (kind === "owner")
        return "OWNER";
    if (kind === "machine")
        return "USER";
...
async function resolveSessionRole(store: AuthRepository, identityId: string): Promise<RoleGateRole> {
 ...
 */
export async function resolveSessionTokenRole(provided: string, options: SessionTokenRoleOptions): Promise<Extract<RouteRoleResolution, {
    ok: true;
}> | null> {
    const store = options.store ?? authStoreForRuntime(options.state?.current);
    if (!store)
        return null;
    const session = await findActiveSession(store, provided, options.now).catch(denyOnAuthStoreError(options.scope ?? "resolveSessionTokenRole/bearerSession"));
    if (!session)
        return null;
    return {
        ok: true,
...
export async function resolveAuthorizedRouteRole(req: Pick<http.IncomingMessage, "headers" | "socket" | "method">, options: AuthorizedRouteRoleOptions): Promise<RouteRoleResolution> {
    ...
    const store = "store" in options && options.store
        ? options.store
        : authStoreForRuntime(state?.current);
    const method = (req.method ?? "GET").toUpperCase();
    const csrfRequired = !options.skipCsrf && CSRF_REQUIRED_METHODS.has(method);
    const sessionCookie = !store || options.allowCookieAuth === false
        ? null
        : readCookie(req, SESSION_COOKIE_NAME);
    if (store && sessionCookie) {
        const session = await findActiveSession(store, sessionCookie, options.now).catch(denyOnAuthStoreError("resolveAuthorizedRouteRole/cookieSession"));
        if (session) {
            if (csrfRequired) {
                const csrfHeader = extractHeaderValue((req.headers as http.IncomingHttpHeaders)[CSRF_HEADER_NAME]);
    ...
    const provided = options.allowBearerAuth === false ? null : getProvidedApiToken(req);
    if (store && provided) {
    ...
    if (provided) {
        ...
        const embedPrincipal = resolveEmbedPrincipal(req, options.now, state
 ...
 */
export async function ensureRouteMinRole(req: Pick<http.IncomingMessage, "headers" | "socket" | "method">, res: http.ServerResponse, state: CompatStateLike, minRole: RoleGateRole, options: {
    skipCsrf?: boolean;
    now?: number;
} = {}): Promise<boolean> {
 ...
 */
export function ensureCompatSensitiveRouteAuthorized(req: Pick<http.IncomingMessage, "headers" | "socket">, res: http.ServerResponse): boolean {
 ...
 */
export async function ensureRouteAuthorized(req: Pick<http.IncomingMessage, "headers" | "socket" | "method">, res: http.ServerResponse, state: CompatStateLike, options: {
    skipCsrf?: boolean;
    now?: number;
} = {}): Promise<boolean> {
