...
describe("compat route auth policy table", () => {
  ...
  it("declares auth tiers for representative compat routes", () => {
    ...
    expect(
      resolveCompatRouteAuthPolicy("POST", "/api/auth/guest-pair-code"),
    ).toMatchObject({ id: "auth.guest-pair-code", tier: "session" });
  ...
  it("fails closed for undeclared app-managed routes", async () => {
