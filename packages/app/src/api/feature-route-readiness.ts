...
export function isFeatureRouteHandlerAvailable(options: {
    ...
    if (HOST_OWNED_FEATURE_ROUTE_PREFIXES.some((prefix) => matchesPathPrefix(options.pathname, prefix)) ||
        HOST_OWNED_FEATURE_ROUTES.some((route) => route.method === method && route.pathname === options.pathname)) {
        return true;
    }
    return Boolean(options.runtimeRoutes?.some((route) => route.type === method &&
...
export function resolveFeatureRouteReadinessFailure(pathname: string, runtimeAvailable: boolean, phases: Readonly<Partial<Record<FeatureRouteBootPhase, DeferredBootPhaseStatus>>>, routeHandlerAvailable = false): FeatureRouteReadinessFailure | null {
    if (!DEFERRED_FEATURE_ROUTE_PREFIXES.some((prefix) => matchesPathPrefix(pathname, prefix))) {
        return null;
    }
    if (!runtimeAvailable) {
        return {
    ...
    if (routeHandlerAvailable)
        return null;
    const pendingPhase = FEATURE_ROUTE_BOOT_PHASES.find((phase) => phases[phase] === "pending");
    if (pendingPhase) {
        return {
            error: "feature_starting",
            code: "feature_starting",
    ...
    const failedPhase = FEATURE_ROUTE_BOOT_PHASES.find((phase) => phases[phase] === "failed");
    if (failedPhase) {
        return {
