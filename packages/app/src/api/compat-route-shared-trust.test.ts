/**
 * Verifies that app's `isTrustedLocalRequest` wrapper wires the correct
...
/**
 * Pins that app's `isTrustedLocalRequest` wrapper binds its exact policy
...
describe("app isTrustedLocalRequest wrapper (policy gates)", () => {
  beforeEach(clearEnv);
  afterEach(clearEnv);
