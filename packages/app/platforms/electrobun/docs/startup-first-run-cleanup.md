...
- `packages/app/platforms/electrobun/src/native/agent.ts` owns embedded runtime lifecycle, diagnostics files, health polling, retry, restart, and bug-report bundle inputs.
- `packages/app/platforms/electrobun/src/boot-progress.ts` composes typed `bootProgress` from AgentManager status and `/api/health`.
- `packages/app/platforms/electrobun/src/config-and-auth-rpc.ts` composes typed auth gate snapshots.
- `packages/app/platforms/electrobun/src/first-run-rpc.ts` composes typed first-run status and options snapshots.
