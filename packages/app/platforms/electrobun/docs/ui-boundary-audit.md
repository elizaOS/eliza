...
- `packages/app/src`
- `packages/app/src`
- `packages/app/platforms/electrobun/src/dynamic-views`
- `packages/app/platforms/electrobun/src/trace`
- `packages/app/platforms/electrobun/src/voice`
- `packages/agent/src`
...
- `packages/app/src/runtime/desktop/*` renders detached desktop windows and tray behavior using `@elizaos/ui`.
...
The sweep covered `packages/app`, `packages/app`, `packages/app/platforms/electrobun`, `packages/agent`, `packages/ui`, and plugin view surfaces.

Findings:

- `packages/app` has three renderer entry files and correctly remains production app composition.
- `packages/app/src/runtime/desktop` has four `.tsx` renderer roots. These are Electrobun desktop shell composition, not a second production app, but they should stay thin and compose `@elizaos/ui`.
...
| Local model renderers | `packages/ui/src/components/local-inference` | Keep UI renderers; data/control comes from app or runtime APIs. |
...
- Future reusable pieces extracted from `packages/app/platforms/electrobun/src/trace/views/agent-run-trace.html`

Trace state remains in Electrobun/app. `packages/ui` should render trace events and latency summaries, not own trace storage or dynamic view sessions.
...
| Local model status | `packages/ui/src/components/local-inference` | `eliza.local-model`, app, plugin-local-inference |
...
| Logs/database tables | `LogsView`, `DatabaseView` renderer pieces | Runtime/app APIs |
...
| Electrobun renderer bridge | `packages/ui/src/bridge/electrobun-rpc.ts` | Keep renderer-safe adapter; main-process execution stays in app/Electrobun. |
...
- `packages/app/src/runtime/desktop/AppWindowRenderer.tsx`
- `packages/app/src/runtime/desktop/DetachedShellRoot.tsx`
- `packages/app/src/runtime/desktop/DesktopTrayRuntime.tsx`

Do not move these now. The audit only records that product composition is distributed across `packages/app`, `packages/ui`, and `packages/app`.
...
| Hidden detached desktop renderers | `packages/app/src/runtime/desktop` | Keep as Electrobun shell composition, not product UI replacement. |
...
`packages/app/src/main.tsx` is the production app composition root. It imports `@elizaos/ui`, `@elizaos/app`, Capacitor bridges, and app/plugin bundles like companion, phone, LifeOps, task coordinator, training, and stewardship views.
...
`packages/app` is mixed by design:
...
- app may own platform bootstrap and Electrobun shell composition.
- app should not become a second production app.
- Desktop renderer roots should compose `@elizaos/ui` components and bridge to app/Electrobun services.
...
| --- | --- | --- | --- |
| `packages/ui/src/services/local-inference` execution-heavy mirror files | medium | no | Local fs, downloader, device bridge, and service code should not live in the rendering package once shared/app sources cover the contracts. |
...
- Should app desktop renderer roots remain in app or move into `packages/app` behind Electrobun-only adapters?
