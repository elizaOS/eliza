 ...
 */
function resolveElizaConfigPath(env: Record<string, string | undefined>): string {
 ...
 */
export function readPersistedDeployment(env: Record<string, string | undefined> = process.env as Record<string, string | undefined>): PersistedDeployment | null {
    ...
    try {
        ...
        logger.warn(`[Deployment] Could not read ${configPath}: ${err instanceof Error ? err.message : String(err)}`);
        return null;
    }
    let parsed: unknown;
    try {
        ...
        logger.warn(`[Deployment] Could not parse ${configPath}: ${err instanceof Error ? err.message : String(err)}`);
        return null;
    }
    const deploymentTarget = normalizeDeploymentTargetConfig((parsed as {
        deploymentTarget?: unknown;
    } | null)?.deploymentTarget);
    if (!deploymentTarget) {
        return null;
    }
    return {
