...
import { SurfaceWindowManager } from "./surface-windows";
import { appendChatOverlayShellModeParam } from "./desktop-bottom-bar-config";
import { appendKioskShellModeParam } from "./kiosk-mode";
import { appendShellModeParam } from "./kiosk-mode";
import { buildApplicationMenu } from "./application-menu";
import { buildBunRpcHandlers } from "./rpc-handlers";
import { buildMainMenuResetApiCandidates } from "./menu-reset-from-main";
import { checkWebGpuSupport } from "./native/webgpu-browser-support";
import { classifyDeepLinkRoute } from "./desktop-deep-link-events";
import { clearCurrentMainWindow } from "./main-window-runtime";
import { computeBottomBarFrame } from "./desktop-bottom-bar-config";
import { configureDesktopLocalApiAuth } from "./native/agent";
import { createDesktopSessionGenerationTracker } from "./lifecycle/desktop-session-prime";
import { createElectrobunBrowserWindow } from "./electrobun-window-options";
import { createRendererApiProxyRequestInit } from "./renderer-api-proxy";
import { createServer as createNetServer } from "node:net";
import { desktopRehydrateSshRuntimes } from "./ssh-runtime-rpc";
import { desktopShutdownSshRuntimes } from "./ssh-runtime-rpc";
import { disableBackForwardNavigationGestures } from "./native/mac-window-effects";
import { disposeNativeModules } from "./native/index";
import { findAppMenuEntryBySlug } from "./application-menu";
import { findViewMenuEntryById } from "./application-menu";
import { formatError } from "@elizaos/core";
import { getAgentManager } from "./native/agent";
import { getBrandConfig } from "./brand-config";
import { getDesktopManager } from "./native/desktop";
import { getDiagnosticLogPath } from "./native/agent";
import { getPermissionManager } from "./native/permissions";
import { getPersistedDeployment } from "./persisted-deployment";
import { getRendererAssetContentType } from "./renderer-static";
import { getStartupDiagnosticLogTail } from "./native/agent";
import { getStartupDiagnosticsSnapshot } from "./native/agent";
import { getStartupStatusPath } from "./native/agent";
import { hasKnownMacosStatusItemSceneRegression } from "./desktop-tray-config";
import { hydrateCloudOnlyEnv } from "./cloud-only-boot";
import { initializeNativeModules } from "./native/index";
import { isAgentReady } from "./agent-ready-state";
import { isDetachedSurface } from "./surface-windows";
import { isKioskShellMode } from "./kiosk-mode";
import { isRendererApiProxyPath } from "./renderer-api-proxy";
import { isStewardLocalEnabled } from "./native/steward";
import { logger } from "./logger";
import { markDesktopSessionStale } from "./lifecycle/desktop-session-prime";
import { mergeRuntimePermissionStates } from "./runtime-permissions";
import { onAgentReadyChange } from "./agent-ready-state";
import { onStewardStatusChange } from "./native/steward";
import { parseSettingsWindowAction } from "./application-menu";
import { parseViewWindowAction } from "./application-menu";
import { pathToFileURL } from "node:url";
import { pickReachableMenuResetApiBase } from "./menu-reset-from-main";
import { primeDesktopSessionAuth } from "./lifecycle/desktop-session-prime";
...
import { readRendererShellMode } from "./kiosk-mode";
import { readResolvedPreloadScript } from "./runtime-layout";
import { recordStartupPhase } from "./startup-trace";
...
import { resetSteward } from "./native/steward";
import { resolveApiToken } from "@elizaos/core/runtime-env";
import { resolveBootstrapShellRenderer } from "./main-window-session";
import { resolveBootstrapViewRenderer } from "./main-window-session";
import { resolveDesktopApiPort } from "@elizaos/core/runtime-env";
import { resolveDesktopApiRequestToken } from "./runtime-preflight";
import { resolveDesktopBundleVersion } from "./windows-cef-profile";
import { resolveDesktopRuntimeForBoot } from "./runtime-preflight";
import { resolveDesktopRuntimeModeWithDeployment } from "./api-base";
import { resolveDesktopShellWindowPresentation } from "./desktop-bottom-bar-config";
import { resolveDesktopWorkspaceWindowOptions } from "./application-menu";
import { resolveInitialApiBase } from "./api-base";
import { resolveMainWindowPartition } from "./main-window-session";
import { resolveNamespaceFromEnv } from "./brand-env-reads";
import { resolveQualifiedExternalToken } from "./runtime-preflight";
import { resolveRendererAsset } from "./renderer-static";
import { resolveRendererAssetByteRange } from "./renderer-static";
import { resolveRendererAssetDir } from "./runtime-layout";
import { resolveRendererFacingApiBase } from "./api-base";
import { resolveRendererProxyIdleTimeoutSeconds } from "./renderer-api-proxy";
import { resolveRendererUrlFromEnv } from "./brand-env-reads";
import { resolveStartupBundlePath } from "./startup-trace";
import { restartSteward } from "./native/steward";
import { resumeDesktopRemoteTarget } from "./remote-target-rpc";
import { runMainMenuResetAfterApiBaseResolved } from "./menu-reset-from-main";
...
import { setCurrentMainWindow } from "./main-window-runtime";
import { setNativeDragRegion } from "./native/mac-window-effects";
import { setStewardSendToWebview } from "./native/steward";
import { setTrafficLightsPosition } from "./native/mac-window-effects";
import { setWindowShadow } from "./native/mac-window-effects";
import { shouldCreateDesktopTray } from "./desktop-tray-config";
import { shouldEnableTrayPopover } from "./desktop-tray-config";
import { shouldForceMainWindowCef } from "./main-window-session";
import { shouldProxyToApiBase } from "./renderer-api-proxy";
import { shouldResetWindowsCefProfile } from "./windows-cef-profile";
import { shouldStartTrayFirst } from "./desktop-tray-config";
import { shouldUseIsolatedMainView } from "./main-window-session";
import { shouldWriteWindowsCefProfileMarker } from "./windows-cef-profile";
...
import { startSteward } from "./native/steward";
import { stopSteward } from "./native/steward";
import { type BoundsStore } from "./surface-windows";
import { type BrowserWindow } from "electrobun/bun";
import { type DesktopRuntimeBootResolution } from "./runtime-preflight";
import { type ElizaDesktopRPCSchema } from "./rpc-schema";
import { type ManagedWindowFrame } from "./surface-windows";
import { type ManagedWindowLike } from "./surface-windows";
import { type SendToWebview } from "./types.js";
import { updateCurrentMainWindowEffectsState } from "./main-window-runtime";
import { wireBrowserWorkspaceCaller } from "./rpc-handlers";
import os from "node:os";
import path from "node:path";
import Electrobun, { ApplicationMenu, BrowserView, BuildConfig, Screen, Updater, Utils, WGPU, webgpu, } from "electrobun/bun";
...
function resolveDesktopAppIconPath(): string {
    return path.join(import.meta.dir, process.platform === "win32"
        ? "../assets/appIcon.ico"
        : "../assets/appIcon.png");
}
/**
...
function resolveDesktopTrayIconOptions(): {
    ...
    if (process.platform === "darwin") {
        const templateIconPath = path.join(import.meta.dir, "../assets/trayIconTemplate.png");
...
function resolveDesktopRuntime(): DesktopRuntimeBootResolution {
    return (preparedDesktopRuntime ??
        resolveDesktopRuntimeModeWithDeployment(process.env as Record<string, string | undefined>, getPersistedDeployment()));
}
...
function buildApiRequestHeaders(targetUrl: string, contentType?: string): Record<string, string> {
...
async function resolveReachableApiBaseForMainReset(): Promise<string | null> {
    ...
    if (base) {
        logger.info(`[Main][reset] Using reachable API base ${base} (tried: ${candidates.join(", ")})`);
    }
    else {
        logger.warn(`[Main][reset] No reachable API base among candidates (tried: ${candidates.join(", ")})`);
    }
    return base;
}
/**
...
async function resetTheAppFromApplicationMenu(): Promise<void> {
    logger.info(`[Main][reset] App menu: Reset ${BRAND.appName} — confirm + POST /api/agent/reset + restart (main process)`);
    await getDesktopManager()
        .showWindow()
        .catch((err: unknown) => {
        logger.warn(`[Main][reset] showWindow failed (continuing): ${err instanceof Error ? err.message : String(err)}`);
    });
    const autoConfirm = process.env.ELIZA_DESKTOP_TEST_AUTO_CONFIRM_DIALOGS === "1" ||
    ...
    const response = autoConfirm
        ...
        : await Utils.showMessageBox({
            type: "warning",
            title: "Reset Agent",
            message: "This will reset the agent: config, cloud keys, and local agent database (conversations / memory).",
            detail: "Downloaded GGUF embedding models are kept. You will return to first-run runtime setup.",
            buttons: ["Reset", "Cancel"],
            defaultId: 0,
            cancelId: 1,
        }).then((box) => box && typeof box === "object" && "response" in box
            ? (box as {
                response: number;
            }).response
            : typeof box === "number"
    ...
    try {
        ...
        await runMainMenuResetAfterApiBaseResolved({
            ...
            pushEmbeddedApiBaseToRenderer: (port, apiToken) => {
                if (currentWindow) {
                    const base = port
                        ? resolveRendererFacingApiBase(process.env as Record<string, string | undefined>, port)
                        : (resolveLoopbackApiBase() ??
                            resolveInitialApiBase(process.env as Record<string, string | undefined>) ??
                            apiBase);
                    if (base) {
        ...
        logger.info("[Main][reset] Pushed menu-reset-app-applied to renderer with /api/status snapshot");
    }
    catch (err) {
        logger.error(`[Main][reset] Main-process reset failed: ${err instanceof Error ? err.message : String(err)}`);
        Utils.showNotification({
            title: "Reset Failed",
 ...
 */
function applyMacOSWindowEffects(win: BrowserWindow, nativeShadow: boolean, nativeInteractiveChrome: boolean): void {
    if (process.platform !== "darwin")
        return;
    const ptr = (win as {
        ptr?: unknown;
    ...
    const shadowConfigured = setWindowShadow(ptr as Parameters<typeof setWindowShadow>[0], nativeShadow);
    updateCurrentMainWindowEffectsState({
        vibrancyEnabled: false,
        shadowEnabled: shadowConfigured ? nativeShadow : null,
    });
    const alignButtons = () => setTrafficLightsPosition(ptr as Parameters<typeof setTrafficLightsPosition>[0], MAC_TRAFFIC_LIGHTS_X, MAC_TRAFFIC_LIGHTS_Y);
    const alignDragRegion = () => setNativeDragRegion(ptr as Parameters<typeof setNativeDragRegion>[0], MAC_NATIVE_DRAG_REGION_X, MAC_NATIVE_DRAG_REGION_HEIGHT);
    ...
    const disableSwipeBackGesture = () => disableBackForwardNavigationGestures(ptr as Parameters<typeof disableBackForwardNavigationGestures>[0]);
    const alignChrome = () => {
...
function scheduleStateSave(statePath: string, win: BrowserWindow): void {
    ...
    saveTimer = setTimeout(() => {
        try {
            ...
            if (width < 200 || height < 200 || x < -16000)
                return;
            const dir = path.dirname(statePath);
            if (!fs.existsSync(dir))
                fs.mkdirSync(dir, { recursive: true });
            fs.writeFileSync(statePath, JSON.stringify({ x, y, width, height }), "utf8");
        }
        catch (err) {
...
function requestAppQuit(): Promise<void> {
    ...
    quitRequestPromise = (async () => {
        await runShutdownCleanup("explicit-quit").catch((err) => {
            logger.warn(`[Main] Shutdown cleanup failed before explicit quit: ${formatError(err)}`);
        });
        Utils.quit();
    })();
    return quitRequestPromise;
...
async function openBrowserDevtoolsFallback(targetWindow: ManagedWindowLike | BrowserWindow | null): Promise<void> {
    const currentUrl = (targetWindow?.webview as {
        url?: string | null;
    } | undefined)?.url;
...
function sendToActiveRenderer(message: string, payload?: unknown): void {
    ...
    if (!currentSendToWebview) {
        const level = message === "desktopTrayMenuClick" ? console.warn : console.debug;
...
function shouldRestoreWindowBeforeMenuAction(action: string | undefined): boolean {
...
async function startRendererServer(): Promise<string> {
    ...
    const getPort = (start: number): Promise<number> => new Promise((resolve) => {
    ...
    const initialApiBase = initialRuntime.mode === "external" && initialRuntime.externalApi.base
        ? initialRuntime.externalApi.base
        : resolveInitialApiBase(process.env as Record<string, string | undefined>);
    const initialApiToken = initialRuntime.mode === "local"
    ...
    const resolveRendererCacheControl = (pathname: string, mimeExt: string): string => {
    ...
    const rendererProxyIdleTimeoutSeconds = resolveRendererProxyIdleTimeoutSeconds(process.env);
    Bun.serve({
        port,
        hostname: "127.0.0.1",
        ...
        async fetch(req) {
            ...
            try {
                ...
                const byteRange = isGzipped
                    ? null
                    : resolveRendererAssetByteRange(req.headers.get("range"), content.byteLength);
                if (byteRange) {
...
async function resolveRendererUrl(): Promise<string> {
    ...
    if (!rendererUrl) {
        ...
        rendererUrl = pathToFileURL(path.join(resolveRendererAssetDir(import.meta.dir), "index.html")).href;
        logger.warn("[Main] Falling back to file:// renderer URL — CORS issues possible");
    }
    return rendererUrl;
}
...
async function resolveRendererUrlForCurrentRuntime(): Promise<string> {
    ...
    if (runtime.mode === "external" && runtime.externalApi.base) {
        const externalRendererUrl = appendApiBaseParam(rendererUrl, runtime.externalApi.base);
...
function resolveBottomBarFrame(): {
    ...
    try {
        ...
        logger.warn(`[main-window] bottom-bar Screen.getPrimaryDisplay() failed; using default geometry: ${err instanceof Error ? err.message : String(err)}`);
    }
...
async function createMainWindow(rpc: ElizaDesktopRpc): Promise<BrowserWindow> {
    ...
    try {
        ...
        logger.error(`[Main] Failed to read preload script (dev fallback): ${err instanceof Error ? err.message : String(err)}`);
    ...
    const forceMainWindowCef = shouldForceMainWindowCef(process.env, process.platform);
    ...
    if (forceMainWindowCef && !canUseCefView) {
        logger.warn("[Main] ELIZA_DESKTOP_FORCE_CEF=1 requested, but this Electrobun build does not bundle the CEF renderer. Falling back to the native renderer.");
    }
    ...
    if (useIsolatedMainView) {
        ...
        logger.info(`[Main] Using partitioned main BrowserView ${mainWindowPartition}`);
    }
    else {
        win = createElectrobunBrowserWindow({
            title: BRAND.appName,
    ...
    if (kiosk) {
        try {
            ...
            logger.warn(`[main-window] kiosk setFullScreen() failed: ${err instanceof Error ? err.message : String(err)}`);
        }
        return win;
    }
    ...
    if (bottomBar) {
        try {
            ...
            logger.warn(`[main-window] bottom-bar setAlwaysOnTop() failed: ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        if (process.platform === "darwin") {
            try {
                ...
                logger.warn(`[main-window] bottom-bar setVisibleOnAllWorkspaces() failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        applyMacOSWindowEffects(win, presentation.nativeShadow, presentation.nativeInteractiveChrome);
    ...
    if (state.shouldMaximize === MAXIMIZE_ON_LAUNCH_SENTINEL) {
        try {
            ...
            logger.warn(`[main-window] maximize() failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    return win;
}
function attachMainWindow(win: BrowserWindow, rpc: ElizaDesktopRpc, sendToWebview: SendToWebview, releaseShellSync: () => void): BrowserWindow {
    ...
    win.on("close", (event: unknown) => {
        ...
        if (!isQuitting &&
            ...
            logger.info("[Main] Window close on Linux with no tray — quitting (no surface to restore from)");
            void requestAppQuit();
            return;
        }
        if (!isQuitting && process.env.ELIZAOS_CLOSE_MINIMIZES_TO_TRAY !== "0") {
            ...
            if (typeof closeEvent?.preventDefault === "function") {
                ...
                void getDesktopManager()
                    .hideWindow()
                    .catch((err: unknown) => {
                    logger.warn(`[Main] Failed to minimize window on close: ${err instanceof Error ? err.message : String(err)}`);
                });
                logger.info("[Main] Window close requested - minimized to tray");
                showBackgroundRunNoticeOnce();
                return;
            }
            logger.info("[Main] Window close requested - agent continues in background");
        }
...
async function restoreWindow(): Promise<void> {
    ...
    backgroundWindowPromise = (async () => {
        ...
        const win = attachMainWindow(await createMainWindow(rpc), rpc, sendToWebview, releaseShellSync);
        injectApiBase(win);
...
function showBackgroundRunNoticeOnce(): void {
    try {
        ...
        logger.warn(`[Main] Failed to persist background notice marker: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
function trackFocusedWindow(window: ManagedWindowLike): void {
    ...
    window.on("focus", () => {
        lastFocusedWindow = window;
        const windowId = (window as {
            id?: number;
        }).id;
        if (process.platform === "darwin" &&
            typeof windowId === "number" &&
            macOpenedDevtoolsWindowIds.has(windowId)) {
            scheduleDevtoolsLayoutRefresh(window as Parameters<typeof scheduleDevtoolsLayoutRefresh>[0]);
        }
    });
    window.on("close", () => {
        const windowId = (window as {
...
function toggleFocusedWindowDevTools(): void {
    ...
    if (typeof webview?.toggleDevTools === "function") {
        ...
        scheduleDevtoolsLayoutRefresh(targetWindow as Parameters<typeof scheduleDevtoolsLayoutRefresh>[0]);
        return;
    }
 ...
 * and `send` proxies.
 */
type ElizaDesktopRpc = ReturnType<typeof BrowserView.defineRPC<ElizaDesktopRPCSchema>>;
/**
...
function asRpcSend(send: unknown): (message: string, payload?: unknown) => void {
...
function createDesktopRpc(label: string): {
    ...
    const sendToWebview: SendToWebview = (message, payload) => {
        if (!rpc) {
            logger.warn(`[sendToWebview:${label}] RPC not yet initialised; dropping message: ${message}`);
            return;
        }
        try {
            ...
            logger.warn(`[sendToWebview:${label}] send(${message}) failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    };
    type BunRpcRequestsHandlers = NonNullable<Parameters<typeof BrowserView.defineRPC<ElizaDesktopRPCSchema>>[0]["handlers"]>["requests"];
 ...
 */
function wireMainWindowAfterCreate(win: BrowserWindow, rpc: ElizaDesktopRpc, sendToWebview: SendToWebview): void {
...
function injectApiBase(win: BrowserWindow): void {
    ...
    if (runtimeResolution.externalApi.invalidSources.length > 0) {
        logger.warn(`[Main] Invalid API base env vars: ${runtimeResolution.externalApi.invalidSources.join(", ")}`);
    }
    if (runtimeResolution.mode === "external" &&
        runtimeResolution.externalApi.base) {
        apiBaseOwner.notifyChange(win, runtimeResolution.externalApi.base, resolveApiToken(process.env) ??
            resolveQualifiedExternalToken(runtimeResolution, runtimeResolution.externalApi.base) ??
            "");
    ...
    if (runtimeResolution.mode === "disabled") {
        ...
        apiBaseOwner.notifyChange(win, resolveInitialApiBase(process.env as Record<string, string | undefined>) ?? `http://127.0.0.1:${resolveDesktopApiPort(process.env)}`, resolveApiToken(process.env) ?? "");
        setAgentReady(true);
        return;
    }
    ...
    apiBaseOwner.notifyChange(win, resolveRendererFacingApiBase(process.env as Record<string, string | undefined>, port), apiToken);
    setAgentReady(true);
}
 ...
 */
async function syncPermissionsToRestApi(port: number, startup = false): Promise<void> {
    try {
        const permissions = await mergeRuntimePermissionStates(port, await getPermissionManager().checkAllPermissions());
        ...
        logger.warn(`[Main] Permission sync failed: ${err instanceof Error ? err.message : String(err)}`);
    }
}
...
async function _startAgent(): Promise<void> {
    ...
    if (runtimeResolution.mode !== "local") {
        logger.info(`[Main] Skipping embedded agent startup (${runtimeResolution.mode} mode)`);
        ...
        if (externalApiBase && isDesktopLoopbackApiBase(externalApiBase)) {
            ...
            const rendererBase = resolveRendererFacingApiBase(process.env as Record<string, string | undefined>, Number(externalUrl.port || (externalUrl.protocol === "https:" ? 443 : 80)));
            const sessionPrimed = await primeDesktopSessionAuth(externalApiBase, rendererBase);
    ...
    try {
        ...
        if (status.state === "running" && status.port) {
            ...
            const rendererBase = resolveRendererFacingApiBase(process.env as Record<string, string | undefined>, status.port);
            ...
            const sessionPrimed = await primeDesktopSessionAuth(apiBase, rendererBase);
            ...
            void resumeDesktopRemoteTarget({ apiBase, apiToken })
                .then((result) => {
                if (result.resumed) {
                    logger.info("[RemoteTarget] Resumed enrolled Linux target after desktop startup");
                }
            })
                .catch((error) => {
                ...
                logger.error(`[RemoteTarget] Startup resume blocked: ${error instanceof Error ? error.message : String(error)}`);
            });
        ...
        logger.error(`[Main] Agent start failed: ${err instanceof Error ? err.message : String(err)}`);
    }
}
async function setupUpdater(): Promise<void> {
        ...
        try {
            ...
            if (!updaterState.canAutoUpdate) {
                if (updaterState.autoUpdateDisabledReason) {
                    logger.info(`[Updater] Skipping auto-update check: ${updaterState.autoUpdateDisabledReason}`);
                    if (notifyOnNoUpdate) {
                        Utils.showNotification({
            ...
            if (updateResult.updateAvailable) {
                Updater.downloadUpdate().catch((err: unknown) => {
                    logger.warn(`[Updater] Download failed: ${err instanceof Error ? err.message : String(err)}`);
                });
                return;
            }
            if (notifyOnNoUpdate) {
    ...
    try {
        ...
        const handleUpdateAndConfigMenuAction = async (action: string | undefined): Promise<boolean> => {
        ...
        const handleMainWindowMenuAction = (action: string | undefined): boolean => {
        ...
        const handleSurfaceMenuAction = (action: string | undefined): boolean => {
            ...
            if (workspaceOptions) {
                void getDesktopManager()
                    ...
                    logger.error(`[Main] Desktop workspace launch failed: ${formatError(error)}`);
                    Utils.showNotification({
        ...
        const handleStewardMenuAction = (action: string | undefined): boolean => {
            if (action === "restart-steward" && isStewardLocalEnabled()) {
                restartSteward().catch((err: unknown) => {
                    logger.error(`[Main] Steward restart failed: ${err instanceof Error ? err.message : String(err)}`);
                    Utils.showNotification({
            ...
            if (action === "reset-steward" && isStewardLocalEnabled()) {
                resetSteward().catch((err: unknown) => {
                    logger.error(`[Main] Steward reset failed: ${err instanceof Error ? err.message : String(err)}`);
                    Utils.showNotification({
        ...
        const handleAppEntryMenuAction = async (action: string | undefined): Promise<boolean> => {
        ...
        const handleRuntimeMenuAction = (action: string | undefined): boolean => {
            ...
            if (action === "restart-agent") {
                getAgentManager()
                    .restart()
                    .catch((err: unknown) => {
                    logger.error(`[Main] Agent restart failed: ${err instanceof Error ? err.message : String(err)}`);
                });
                return true;
            }
            if (action === "quit") {
        ...
        const handleApplicationMenuAction = async (action: string | undefined): Promise<void> => {
            if (!currentWindow && shouldRestoreWindowBeforeMenuAction(action)) {
                await restoreWindow();
            }
            if (await handleUpdateAndConfigMenuAction(action))
                return;
            if (handleMainWindowMenuAction(action))
                return;
            if (handleSettingsMenuAction(action))
                return;
            if (handleSurfaceMenuAction(action))
                return;
            if (handleStewardMenuAction(action))
                return;
            if (await handleAppEntryMenuAction(action))
                return;
        ...
        Electrobun.events.on("tray-clicked", (e: {
            data?: {
                action?: string;
            };
        }) => {
...
async function runShutdownCleanup(reason: string): Promise<void> {
    ...
    shutdownCleanupPromise = (async () => {
        ...
        const cleanupResults = await Promise.allSettled(cleanupFnsToRun.map((cleanupFn) => Promise.resolve().then(cleanupFn)));
        for (const result of cleanupResults) {
            if (result.status === "rejected") {
                logger.warn(`[Main] Shutdown cleanup callback failed: ${result.reason instanceof Error
        ...
        try {
            ...
            logger.warn(`[Main] Native module disposal failed during shutdown: ${error instanceof Error ? error.message : String(error)}`);
        }
    })();
    return shutdownCleanupPromise;
}
...
async function loadTheAppEnvFilesForMain(): Promise<void> {
    ...
    try {
        ...
        const repoRootGuess = path.resolve(normalizedModuleDir, "..", "..", "..", "..");
...
function initializeBundledWebGPU(): void {
    if (!WGPU.native.available) {
        logger.info("[WebGPU] Native Dawn runtime not bundled for this run; renderer-side WebGPU remains available through the webview/browser path.");
        return;
    }
    webgpu.install();
...
function checkWebGpuBrowserSupport(rendererType: "native" | "cef"): void {
    ...
    if (status.available) {
        ...
        if (status.chromeBetaPath) {
            logger.info(`[WebGPU Browser] Chrome Beta found at: ${status.chromeBetaPath}`);
        }
        else if (status.downloadUrl) {
            logger.info(`[WebGPU Browser] Download Chrome Beta: ${status.downloadUrl}`);
        }
    }
...
async function main(): Promise<void> {
    ...
    const cloudOnlyHydration = hydrateCloudOnlyEnv(BRAND.cloudOnly, BRAND.cloudApiBase);
    if (cloudOnlyHydration.applied.length > 0) {
        console.log(`[Env] cloud-only brand flag raised: ${cloudOnlyHydration.applied.join(", ")}`);
    }
    const desktopEnv = process.env as Record<string, string | undefined>;
    const persistedDeployment = getPersistedDeployment();
    const unverifiedDesktopRuntime = resolveDesktopRuntimeModeWithDeployment(desktopEnv, persistedDeployment);
    preparedDesktopRuntime = await resolveDesktopRuntimeForBoot({
        env: desktopEnv,
        deployment: persistedDeployment,
    });
    if (unverifiedDesktopRuntime.mode === "external" &&
        preparedDesktopRuntime.mode === "local" &&
        persistedDeployment?.runtime !== "local") {
        logger.warn("[Main] Persisted external target is not a ready Eliza agent; using the embedded runtime for this launch without changing the saved deployment target.");
    }
    else if (preparedDesktopRuntime.externalReachability === "unavailable") {
        logger.warn("[Main] Persisted external target is unavailable and this package has no embedded runtime; keeping the external topology unavailable without exposing its credential.");
    }
    ...
    console.log(`[Env] platform=${process.platform} arch=${process.arch} bun=${Bun.version} ` +
    ...
    console.log(`[Env] desktopRuntimeMode=${runtimeResolution.mode} externalApi=${runtimeResolution.externalApi.base ?? "none"}`);
    printElectrobunDevSettingsBanner(process.env as Record<string, string | undefined>);
    ...
    void maybePromptStartupCrashReport()
        ...
        .catch((err) => {
        logger.warn(`[Main] Startup crash prompt failed: ${err instanceof Error ? err.message : String(err)}`);
    });
    ...
    if (process.platform === "win32") {
        try {
            ...
            const cefVersionMarker = path.join(cefDir, BRAND.cefVersionMarkerFileName);
            const currentVersion = resolveDesktopBundleVersion(import.meta.dir) ?? "unknown";
            ...
            if (shouldResetWindowsCefProfile({
                currentVersion,
                previousVersion,
                cefDirExists: fs.existsSync(cefDir),
            })) {
                logger.info(`[Main] CEF version mismatch (${previousVersion ?? "none"} → ${currentVersion}), clearing stale CEF profile`);
                // Remove everything except the version marker we're about to write.
                for (const entry of fs.readdirSync(cefDir)) {
                    if (entry === BRAND.cefVersionMarkerFileName)
                        continue;
                    ...
                    try {
                        ...
                        logger.warn(`[Main] Could not remove ${entryPath}: ${err instanceof Error ? err.message : String(err)}`);
                    }
                }
            }
            ...
            logger.warn(`[Main] CEF profile cleanup failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    const browserWorkspaceBridgeStop = startBrowserWorkspaceBridgeServer()
        ...
        .catch((err) => {
        logger.warn(`[Main] Browser-workspace bridge startup failed: ${err instanceof Error ? err.message : String(err)}`);
        return undefined;
    });
    ...
    try {
        const stopScreenCaptureBridgeServer = await startScreenCaptureBridgeServer();
        ...
        logger.warn(`[Main] Screen-capture bridge startup failed: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    const desktopSessionGenerationChanged = createDesktopSessionGenerationTracker();
    cleanupFns.push(getAgentManager().onStatusChange((status) => {
        if (status.port) {
            const apiBase = `http://127.0.0.1:${status.port}`;
            const rendererBase = resolveRendererFacingApiBase(process.env as Record<string, string | undefined>, status.port);
            if (desktopSessionGenerationChanged(status)) {
                ...
                void primeDesktopSessionAuth(apiBase, rendererBase).then(async (sessionPrimed) => {
                    if (!recoveringExistingRenderer)
                        return;
                    await reloadRendererAfterDesktopSessionPrime({
                        sessionPrimed,
                        backendGeneration: `${status.port}:${status.startedAt ?? "running"}`,
    ...
    const hasBrokenStatusItemScene = hasKnownMacosStatusItemSceneRegression(process.platform, os.release());
    const dockless = docklessRequested && !hasBrokenStatusItemScene;
    if (hasBrokenStatusItemScene && docklessRequested) {
        logger.warn(`[Main] macOS ${os.release()} has the Control Center status-item scene regression; keeping the Dock icon available`);
        getDesktopManager().setTrayFirstMode(false);
    }
    else if (dockless) {
        logger.info("[Main] Dockless startup requested — verifying the menu-bar icon before hiding the Dock icon");
    }
    recordStartupPhase("creating_window", {
        pid: process.pid,
    });
    const { rpc: mainRpc, sendToWebview: mainSendToWebview, releaseShellSync: releaseMainShellSync, } = createDesktopRpc("main");
    const mainWin: BrowserWindow | null = attachMainWindow(await createMainWindow(mainRpc), mainRpc, mainSendToWebview, releaseMainShellSync);
    ...
    void desktopRehydrateSshRuntimes()
        ...
        if (restored.length > 0) {
            logger.info(`[SSH runtime] Restored ${restored.length} tunnel(s) after restart`);
        }
        for (const item of blocked) {
            logger.warn(`[SSH runtime] Kept ${item.runtimeId} stopped after restart: ${item.error}`);
        }
    })
        .catch((error) => {
        logger.warn(`[SSH runtime] Failed to read restart intents; all SSH tunnels remain stopped: ${error instanceof Error ? error.message : String(error)}`);
    });
    ...
    surfaceWindowManager = new SurfaceWindowManager({
        ...
        wireRpc: (window) => {
            ...
            if (!rpc) {
                logger.warn("[surface-windows] wireRpc called for window with no tracked rpc; skipping browser-workspace caller setup");
                return;
            }
            wireSettingsRpcAfterCreate(rpc);
        },
        injectApiBase: (window) => injectApiBase(window as BrowserWindow & ManagedWindowLike),
        ...
        onRegistryChanged: () => {
            ...
            getDesktopManager().setManagedWindowsPresent((surfaceWindowManager?.listWindows().length ?? 0) > 0);
        },
    ...
    getDesktopManager().setOpenSurfaceWindowCallback((surface, browse, alwaysOnTop) => {
        if (!surfaceWindowManager) {
            throw new Error("Surface window manager is not ready.");
        }
        return surfaceWindowManager.openSurfaceWindow(surface, browse, alwaysOnTop === true);
    });
    ...
    if (mainWin && process.argv.includes("--hidden")) {
        try {
            ...
            logger.warn(`[Main] Failed to minimize window on --hidden startup: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    setupDeepLinks();
    setupDockReopen();
    ...
    if (shouldCreateDesktopTray(process.env)) {
        try {
            ...
            if (dockless) {
                if (desktop.hasVisibleTrayStatusItem()) {
                    ...
                    logger.info("[Main] Menu-bar icon visible — pill-only dockless mode enabled");
                }
                else {
                    ...
                    logger.warn("[Main] Menu-bar icon has no visible bounds; keeping Dock icon available while macOS recovers");
                    const trayRecoveryTimer = setInterval(() => {
                        if (!desktop.hasVisibleTrayStatusItem())
                            return;
                        clearInterval(trayRecoveryTimer);
                        desktop.setTrayFirstMode(true);
                        logger.info("[Main] Menu-bar icon recovered — pill-only dockless mode enabled");
                    }, 5000);
            ...
            logger.warn(`[Main] Tray creation failed: ${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        if (shouldEnableTrayPopover()) {
            try {
                ...
                logger.warn(`[Main] Tray popover configuration failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    }
    else {
    ...
    if (isStewardLocalEnabled()) {
        ...
        cleanupFns.push(onStewardStatusChange((status) => {
            sendToActiveRenderer("stewardStatusUpdate", status);
        }));
        try {
            const stewardResult = await startSteward();
            if (stewardResult.state === "running") {
                logger.info(`[Main] Steward sidecar ready on port ${stewardResult.port}, wallet: ${stewardResult.walletAddress ?? "pending"}`);
            }
            else {
                logger.warn(`[Main] Steward sidecar in state "${stewardResult.state}": ${stewardResult.error ?? "unknown"}`);
    ...
    if (rt.mode === "external" || rt.mode === "local") {
        ...
        _startAgent().catch((err) => {
            logger.error(`[Main] Agent auto-start failed: ${err instanceof Error ? err.message : String(err)}`);
            const error = err instanceof Error ? err.message : String(err);
...
function resolveStartupCrashReportPath(): string {
    return path.join(path.dirname(getDiagnosticLogPath()), STARTUP_CRASH_REPORT_FILE);
}
function resolveStartupCrashPromptMarkerPath(): string {
    return path.join(path.dirname(getDiagnosticLogPath()), STARTUP_CRASH_PROMPT_MARKER_FILE);
}
...
function persistStartupCrashReport(options: {
    ...
    try {
        ...
        logger.warn(`[Main] Failed to write startup crash report: ${err instanceof Error ? err.message : String(err)}`);
        try {
            ...
            logger.warn(`[Main] Failed to write fallback startup crash report: ${fallbackErr instanceof Error ? fallbackErr.message : String(fallbackErr)}`);
        }
    }
...
async function maybePromptStartupCrashReport(): Promise<void> {
    if (process.env.ELIZA_DESKTOP_SKIP_STARTUP_CRASH_PROMPT === "1" ||
        process.env.ELIZA_DESKTOP_TEST_AUTO_CONFIRM_DIALOGS === "1") {
        return;
    }
    const diagnostics = getStartupDiagnosticsSnapshot();
    const looksLikeStartupFailure = diagnostics.state === "error" &&
    ...
    const dialog = await Utils.showMessageBox({
        ...
        message: "The previous launch failed. A crash report is ready to share with support.",
        detail: "Choose Copy Report, paste into Discord, and ping @iono. You can also open logs.",
        buttons: ["Copy Report", "Open Logs Folder", "Continue"],
        defaultId: 0,
        cancelId: 2,
    });
    const response = dialog && typeof dialog === "object" && "response" in dialog
        ? (dialog as {
            response: number;
        }).response
    ...
    if (response === 0) {
        try {
            ...
            logger.warn(`[Main] Failed to copy startup crash report: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    else if (response === 1) {
        try {
            ...
            logger.warn(`[Main] Failed to open startup logs folder: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
}
main().catch((err) => {
    ...
    try {
        ...
        fs.writeFileSync(getStartupStatusPath(), `${JSON.stringify({
            state: "error",
            phase: "fatal_startup",
