...
import { digestRemotePayload } from "../../../src/security/remote-control-crypto";
import { generateKeyPairSync } from "node:crypto";
import { isEncryptedRemoteControlEnvelope } from "@elizaos/core/contracts/remote-control";
import { isRemoteCommandAction } from "@elizaos/core/contracts/remote-control";
...
import { signRemoteCommand } from "../../../src/security/remote-control-crypto";
...
import { type RemoteCommandBody } from "@elizaos/core/contracts/remote-control";
import { type RemoteControllerPlatform } from "@elizaos/core/contracts/remote-control";
...
import { type SignedRemoteCommandResult } from "@elizaos/core/contracts/remote-control";
import { type SignedRemoteCommandStartReceipt } from "@elizaos/core/contracts/remote-control";
import { verifyRemoteCommandResult } from "../../../src/security/remote-control-crypto";
import { verifyRemoteCommandStartReceipt } from "../../../src/security/remote-control-crypto";
...
async function withControllerMutation<T>(vaultId: string, operation: () => Promise<T>): Promise<T> {
...
function parseStoredIdentity(value: string): StoredControllerIdentity {
    ...
    if (!isRemoteControllerPublicIdentity(identity) ||
        ...
        canonicalizeRemoteControlValue(identity.encryptionPublicKeyJwk) !==
            canonicalizeRemoteControlValue(publicJwk(record.encryptionPrivateKeyJwk)) ||
        identity.keyId !==
            identityKeyId(identity.signingPublicKeyJwk, identity.encryptionPublicKeyJwk)) {
        throw new Error("Stored controller identity is corrupt.");
    }
    const sessionSequences = record.sessionSequences;
    if (sessionSequences !== undefined &&
        (typeof sessionSequences !== "object" ||
            ...
            Object.entries(sessionSequences).some(([sessionId, entry]) => !isIdentifier(sessionId) ||
                typeof entry !== "object" ||
                entry === null ||
                typeof Reflect.get(entry, "bindingDigest") !== "string" ||
                !/^[A-Za-z0-9_-]{43}$/.test(String(Reflect.get(entry, "bindingDigest"))) ||
                ...
                (() => {
                    ...
                    return (typeof Reflect.get(pending, "requestDigest") !== "string" ||
                        !/^[A-Za-z0-9_-]{43}$/.test(String(Reflect.get(pending, "requestDigest"))) ||
                        ...
                        canonicalizeRemoteControlValue(copyRemoteCommandBinding(command.body)) !==
                            canonicalizeRemoteControlValue(copyRemoteCommandBinding(envelope)));
                })()) ||
            Object.values(sessionSequences).some((entry) => typeof entry !== "object" ||
                entry === null ||
                typeof Reflect.get(entry, "bindingDigest") !== "string" ||
                !Number.isSafeInteger(Reflect.get(entry, "sequence")) ||
                Number(Reflect.get(entry, "sequence")) < 1))) {
    ...
    return {
        ...
        ...(sessionSequences
            ? {
                sessionSequences: sessionSequences as StoredControllerIdentity["sessionSequences"],
            }
            : {}),
    };
}
...
function identityKeyId(signingPublicKeyJwk: JsonWebKey, encryptionPublicKeyJwk: JsonWebKey): string {
    return `p256:${createHash("sha256")
        .update(canonicalizeRemoteControlValue({
        signingPublicKeyJwk,
...
function readIdentityRequest(params: unknown): {
    ...
    if (!isIdentifier(ownerId) ||
        ...
        !["macos", "windows", "linux", "ios", "android", "web"].includes(String(platform))) {
...
async function nativeControllerDeviceId(nativeStore: PlatformSecureStore): Promise<string> {
    return withControllerMutation(CONTROLLER_DEVICE_VAULT_ID, async () => {
        const existing = await nativeStore.get(CONTROLLER_DEVICE_VAULT_ID, "runtime.agent_profiles");
        if (existing.ok) {
        ...
        const persisted = await nativeStore.set(CONTROLLER_DEVICE_VAULT_ID, "runtime.agent_profiles", created);
        if (!persisted.ok) {
            throw new Error("Secure controller device storage is unavailable.");
        }
        return created;
...
async function loadControllerIdentity(ownerId: string, deviceId: string, nativeStore: PlatformSecureStore = store): Promise<StoredControllerIdentity | null> {
    const result = await nativeStore.get(controllerVaultId(ownerId, deviceId), "runtime.agent_profiles");
    if (!result.ok) {
    ...
    if (stored.identity.ownerId !== ownerId ||
        stored.identity.deviceId !== deviceId) {
        throw new Error("Stored controller identity binding is invalid.");
    }
    return stored;
}
export async function desktopGetOrCreateControllerIdentity(params: unknown, nativeStore: PlatformSecureStore = store): Promise<RemoteControllerPublicIdentity> {
    const request = readIdentityRequest(params);
    const deviceId = request.deviceId ?? (await nativeControllerDeviceId(nativeStore));
    const vaultId = controllerVaultId(request.ownerId, deviceId);
    return withControllerMutation(vaultId, async () => {
        const existing = await loadControllerIdentity(request.ownerId, deviceId, nativeStore);
        if (existing)
        ...
        const result = await nativeStore.set(vaultId, "runtime.agent_profiles", JSON.stringify(stored));
        if (!result.ok) {
            throw new Error("Secure controller identity storage is unavailable.");
        }
        return identity;
...
export async function desktopCreateRemoteCommand(params: unknown, nativeStore?: PlatformSecureStore): Promise<{
    commandId: string;
    expiresAt: number;
    command: SignedRemoteCommand;
    envelope: EncryptedRemoteControlEnvelope;
...
export async function desktopCreateRemoteCommand(params: unknown, nativeStore: PlatformSecureStore = store): Promise<{
    commandId: string;
    expiresAt: number;
    command: SignedRemoteCommand;
    envelope: EncryptedRemoteControlEnvelope;
    ...
    if (!identifiers.every(isIdentifier) ||
    ...
    return withControllerMutation(vaultId, async () => {
        const stored = await loadControllerIdentity(ownerId, controllerDeviceId, nativeStore);
        ...
        const requestDigest = createHash("sha256")
            .update(canonicalizeRemoteControlValue({ action: record.action, payload }))
            .digest("base64url");
        ...
        const sequence = previous?.bindingDigest === bindingDigest ? previous.sequence + 1 : 1;
        ...
        const envelope = sealRemoteControlMessage(command, scope, record.targetEncryptionPublicKeyJwk as JsonWebKey);
        ...
        const persisted = await nativeStore.set(vaultId, "runtime.agent_profiles", JSON.stringify({ ...stored, sessionSequences: sequences }));
        if (!persisted.ok) {
...
export async function desktopAcknowledgeRemoteCommandEnqueue(params: unknown, nativeStore: PlatformSecureStore = store): Promise<{
    ...
    return withControllerMutation(vaultId, async () => {
        ...
        if (!entry?.pending)
            return { acknowledged: false };
        if (entry.bindingDigest !== bindingDigest ||
            entry.pending.commandId !== commandId) {
            throw new Error("Remote enqueue acknowledgement does not match the pending command.");
        }
        ...
        const persisted = await nativeStore.set(vaultId, "runtime.agent_profiles", JSON.stringify({ ...stored, sessionSequences }));
        if (!persisted.ok) {
...
export async function desktopClearRemoteSessionState(params: unknown, nativeStore: PlatformSecureStore = store): Promise<{
    cleared: boolean;
}> {
    if (typeof params !== "object" || params === null || Array.isArray(params)) {
    ...
    return withControllerMutation(vaultId, async () => {
        ...
        if (entry.pending) {
            throw new Error("A remote command is still awaiting enqueue acknowledgement. Resolve or revoke it before clearing this session.");
        }
        const sessionSequences = { ...stored.sessionSequences };
...
export async function desktopOpenRemoteCommandResult(params: unknown): Promise<{
    ...
    const record = params as Record<string, unknown>;
    if (!isIdentifier(record.ownerId) ||
    ...
    const stored = await loadControllerIdentity(record.ownerId, record.controllerDeviceId);
    if (!stored || stored.identity.keyId !== envelope.recipientKeyId) {
        throw new Error("Controller identity is unavailable or changed.");
    }
    const opened = openRemoteControlMessage(envelope, stored.encryptionPrivateKeyJwk, {
        ...command.body,
        messageKind: "result",
...
export async function desktopOpenRemoteCommandStartReceipt(params: unknown): Promise<{
    startedAt: number;
    executionId: string;
}> {
    if (typeof params !== "object" || params === null || Array.isArray(params)) {
    ...
    if (!verifyRemoteCommandStartReceipt(opened, targetIdentity, command)) {
        throw new Error("Remote start receipt signature or command binding is invalid.");
    }
    return {
