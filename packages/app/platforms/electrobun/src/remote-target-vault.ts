...
import { isRemoteTargetPublicIdentity } from "@elizaos/core/contracts/remote-control";
...
type RemoteTargetVaultRecord = PendingRemoteTargetVaultRecord | EnrolledRemoteTargetVaultRecord;
...
function isPrivateP256Jwk(value: unknown): value is JsonWebKey {
    ...
    return (Reflect.get(value, "kty") === "EC" &&
...
function isManagedNetworkRecord(value: unknown): boolean {
    ...
    try {
        return (typeof value === "object" &&
            ...
            /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/.test(Reflect.get(value, "hostname") as string) &&
            canonicalManagedNetworkLoginServer(Reflect.get(value, "loginServer") as string) === Reflect.get(value, "loginServer"));
    }
    catch {
...
export function remoteTargetVaultId(canonicalStateDir = resolveCanonicalStateDir()): string {
...
function parseRecord(raw: string): RemoteTargetVaultRecord {
    ...
    if (Reflect.get(value, "status") === "pending") {
        const signingPrivateKeyJwk = Reflect.get(value, "signingPrivateKeyJwk") as JsonWebKey;
        const encryptionPrivateKeyJwk = Reflect.get(value, "encryptionPrivateKeyJwk") as JsonWebKey;
        if (!isIdentifier(Reflect.get(value, "ownerId")) ||
            ...
            (Reflect.get(value, "platform") !== undefined &&
                !["macos", "windows", "linux"].includes(Reflect.get(value, "platform") as string))) {
            throw new Error("Stored remote target identity is corrupt.");
        }
        return value as PendingRemoteTargetVaultRecord;
    }
    if (Reflect.get(value, "status") !== "enrolled" ||
        !isIdentifier(Reflect.get(value, "deviceId")) ||
...
export class RemoteTargetVault {
    ...
    constructor(private readonly secureStore: PlatformSecureStore = createNodePlatformSecureStore(), private readonly vaultId = remoteTargetVaultId()) { }
    ...
    async prepare(input: {
        ...
        platform: "macos" | "windows" | "linux";
        now: number;
    }): Promise<PendingRemoteTargetVaultRecord | EnrolledRemoteTargetVaultRecord> {
        if (!isIdentifier(input.ownerId) ||
        ...
        return this.serialize(async () => {
            ...
            if (existing) {
                const existingOwner = existing.status === "enrolled"
            ...
            const stored = await this.secureStore.set(this.vaultId, TARGET_VAULT_KIND, JSON.stringify(pending));
            if (!stored.ok) {
                throw new Error("Secure remote target storage is unavailable.");
            }
            return pending;
    ...
    async commitEnrollment(input: {
        ...
        return this.serialize(async () => {
            const existing = await this.load();
            if (!existing)
                throw new Error("Remote target identity is missing.");
            if (existing.status === "enrolled") {
                if (existing.identity.runtimeId !== input.hostId ||
                    existing.hostToken !== input.hostToken) {
                    throw new Error("Remote target enrollment conflicts with stored state.");
                }
                return existing;
            }
            if (!isIdentifier(input.hostId) ||
            ...
            const stored = await this.secureStore.set(this.vaultId, TARGET_VAULT_KIND, JSON.stringify(enrolled));
            if (!stored.ok) {
                throw new Error("Secure remote target storage is unavailable.");
            }
            return enrolled;
    ...
    async recordManagedNetwork(input: {
        ...
        return this.serialize(async () => {
            ...
            if (existing?.status !== "enrolled" ||
                ...
                throw new Error("Managed network enrollment does not match stored state.");
            }
            const updated: EnrolledRemoteTargetVaultRecord = {
                ...existing,
                managedNetwork: { hostname: input.hostname, loginServer },
            };
            const stored = await this.secureStore.set(this.vaultId, TARGET_VAULT_KIND, JSON.stringify(updated));
            if (!stored.ok) {
                throw new Error("Secure remote target storage is unavailable.");
            }
            return updated;
    ...
    async delete(): Promise<boolean> {
        return this.serialize(async () => {
            const result = await this.secureStore.delete(this.vaultId, TARGET_VAULT_KIND);
