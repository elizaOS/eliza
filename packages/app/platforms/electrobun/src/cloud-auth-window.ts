...
export interface CloudAuthWindowLike {
    ...
    webview: {
        ...
        on: (event: "dom-ready" | "will-navigate" | "host-message", handler: (event?: unknown) => void) => void;
    };
}
...
type NavigationEventLike = string | {
    url?: string;
    data?: {
        detail?: string;
    };
...
export class CloudAuthWindowManager {
    ...
    open(url: string): boolean {
        if (!isTrustedElizaUrl(url)) {
            console.warn(`[CloudAuthWindow] Rejected open(): URL is not a trusted Eliza origin: ${url}`);
            return false;
        }
        if (this.window) {
