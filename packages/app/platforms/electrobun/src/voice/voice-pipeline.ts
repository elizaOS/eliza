...
import { type VoiceComponentRole } from "./types";
import { type VoiceComponentSnapshot } from "./types";
import { type VoiceLatencyMark } from "./types";
import { type VoiceLatencySummary } from "./types";
...
function latestVoiceVersions(): Map<VoiceModelId, VoiceModelVersion> {
    ...
    for (const version of VOICE_MODEL_VERSIONS) {
        if (!latest.has(version.id))
            latest.set(version.id, version);
    }
    return latest;
}
...
function firstCatalogComponent(key: "voice" | "asr" | "vad"): {
    ...
    for (const model of MODEL_CATALOG) {
        ...
        if (component)
            return { model, file: component.file };
    }
    return null;
}
function addSeed(components: Map<string, VoiceComponentSnapshot>, seed: ComponentSeed): void {
    if (components.has(seed.id))
        return;
...
export function discoverStaticVoiceComponents(): VoiceComponentSnapshot[] {
    ...
    const backends = new Set(MODEL_CATALOG.flatMap((model) => [...(model.voiceBackends ?? [])]));
    ...
    return Array.from(components.values()).sort((a, b) => a.id.localeCompare(b.id));
}
...
export function summarizeVoiceLatency(turn: VoiceTurn | undefined): VoiceLatencySummary | undefined {
    if (!turn)
        return undefined;
