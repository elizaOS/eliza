/** Exercises api base behavior with deterministic app test fixtures. */
...
/**
 * Electrobun is excluded from app's default vitest run
 * (`platforms/electrobun/**` in `packages/app/vitest.config.ts`). Run this
 ...
 *
 * (or `bun run --cwd packages/app/platforms/electrobun test` to run the
