...
import { isRemoteControlIdentifier } from "@elizaos/core/contracts/remote-control";
import { isRemoteControllerGrant } from "@elizaos/core/contracts/remote-control";
...
export type RemoteTargetCommandStatus = "reserved" | "started" | "completed" | "rejected" | "cancelled" | "execution_ambiguous";
...
export interface RemoteTargetStateStore {
    ...
    clear(): Promise<void>;
    transact<T>(operation: (state: RemoteTargetDurableState) => T | Promise<T>): Promise<T>;
}
...
function assertState(value: unknown): asserts value is RemoteTargetDurableState {
    ...
    for (const [sessionId, candidate] of Object.entries(sessions)) {
        ...
        if (!isRemoteControllerGrant(grant) ||
            ...
            Object.values(nonces).some((expiry) => !Number.isSafeInteger(expiry) || (expiry as number) <= 0) ||
    ...
    for (const [commandId, candidate] of Object.entries(commands)) {
        ...
        if (!isSignedRemoteCommand(signed) ||
        ...
        const hasExecution = typeof command.executionId === "string" &&
            Number.isSafeInteger(command.startedAt);
        if ((command.claimAttempt === null) !== (command.claimToken === null) ||
            (reserved &&
                (command.executionId !== null ||
        ...
        try {
            if (command.resultPresent)
                canonicalizeRemoteControlValue(command.result);
        }
        catch {
...
export function resolveRemoteTargetJournalPath(env: NodeJS.ProcessEnv = process.env): string {
...
export class JsonFileRemoteTargetStateStore implements RemoteTargetStateStore {
    ...
    async transact<T>(operation: (state: RemoteTargetDurableState) => T | Promise<T>): Promise<T> {
