...
import { type FetchLike } from "./menu-reset-from-main";
...
export function buildMainApiHeaders(contentType?: string, bearerTokenOverride?: string | null): Record<string, string> {
    ...
    if (!apiToken) {
        const rt = resolveDesktopRuntimeMode(process.env as Record<string, string | undefined>);
        if (rt.mode === "local") {
...
export async function postCloudDisconnectFromMain(options?: {
    ...
    const buildHeaders = (contentType?: string) => buildMainApiHeaders(contentType, bearer);
