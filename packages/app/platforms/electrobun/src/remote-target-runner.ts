 ...
 */
import { REMOTE_CONTROL_MAX_REPLAY_ENTRIES_PER_SESSION } from "@elizaos/core/contracts/remote-control";
import { REMOTE_CONTROL_PROTOCOL_VERSION } from "@elizaos/core/contracts/remote-control";
import { RemoteTargetTransportError } from "./remote-target-transport";
import { copyRemoteCommandBinding } from "@elizaos/core/contracts/remote-control";
import { digestRemoteResultValue } from "../../../src/security/remote-control-crypto";
import { isSignedRemoteCommand } from "@elizaos/core/contracts/remote-control";
import { logger } from "./logger";
import { openRemoteControlMessage } from "../../../src/security/remote-control-crypto";
import { randomUUID } from "node:crypto";
import { sealRemoteControlMessage } from "../../../src/security/remote-control-crypto";
import { signRemoteCommandResult } from "../../../src/security/remote-control-crypto";
import { signRemoteCommandStartReceipt } from "../../../src/security/remote-control-crypto";
...
import { type RemoteCommandAction } from "@elizaos/core/contracts/remote-control";
import { type RemoteCommandResultStatus } from "@elizaos/core/contracts/remote-control";
import { type RemoteControllerGrant } from "@elizaos/core/contracts/remote-control";
import { type RemoteJsonValue } from "@elizaos/core/contracts/remote-control";
import { type RemoteTargetActivationResponse } from "./remote-target-transport";
import { type RemoteTargetClaim } from "./remote-target-transport";
import { type RemoteTargetRelayTransport } from "./remote-target-transport";
import { type RemoteTargetStateStore } from "./remote-target-store";
import { type RemoteTargetStoredCommand } from "./remote-target-store";
import { type RemoteTargetStoredSession } from "./remote-target-store";
import { type RemoteTargetVault } from "./remote-target-vault";
import { type SignedRemoteCommand } from "@elizaos/core/contracts/remote-control";
import { verifyRemoteCommandAuthenticity } from "../../../src/security/remote-control-crypto";
...
type PollDisposition = "empty" | "completed" | "duplicate" | "delivery_pending" | "offline";
...
function resultEnvelope(command: SignedRemoteCommand, stored: RemoteTargetStoredCommand, enrollment: EnrolledRemoteTargetVaultRecord, controllerEncryptionPublicKeyJwk: JsonWebKey): EncryptedRemoteControlEnvelope {
    const body = {
        ...
        resultDigest: digestRemoteResultValue(stored.resultPresent ? stored.result : undefined, stored.errorCode ?? undefined),
    };
    const signed = signRemoteCommandResult(body, enrollment.signingPrivateKeyJwk);
    return sealRemoteControlMessage(signed, {
        ...copyRemoteCommandBinding(command.body),
        messageKind: "result",
...
function startEnvelope(command: SignedRemoteCommand, stored: RemoteTargetStoredCommand, enrollment: EnrolledRemoteTargetVaultRecord, controllerEncryptionPublicKeyJwk: JsonWebKey): EncryptedRemoteControlEnvelope {
...
export class RemoteTargetRunner {
    ...
    constructor(private readonly vault: RemoteTargetVault, private readonly stateStore: RemoteTargetStateStore, private readonly transport: RemoteTargetRelayTransport, private readonly executor: RemoteTargetCommandExecutor, private readonly options: {
        now?: () => number;
    ...
    async installActivation(activation: RemoteTargetActivationResponse): Promise<void> {
        const enrollment = await this.requireEnrollment();
        if (activation.ownerId !== enrollment.identity.ownerId ||
        ...
        await this.stateStore.transact((state) => {
            ...
            state.sessions[activation.sessionId] = {
                ...
                stoppedAt: null,
                activationState: current?.activationState === "active" ? "active" : "staged",
            };
        });
    }
    ...
    async recoverStagedActivations(): Promise<string | null> {
        ...
        const staged = Object.values(state.sessions)
            .filter((session) => session.activationState === "staged" &&
            session.stoppedAt === null &&
            session.grant.revokedAt === null)
        ...
        for (const session of staged) {
            try {
                ...
                if (error instanceof RemoteTargetTransportError &&
                    ...
                    logger.warn("[RemoteTargetRunner] Staged activation commit is offline", {
                        sessionId: session.grant.sessionId,
                        error,
                    });
                    continue;
    ...
    async pollOnce(expectedGeneration?: number): Promise<PollDisposition> {
        ...
        this.pollTail = run.then(() => undefined, () => undefined);
        await run;
        return disposition;
    }
    private async pollOnceSerialized(expectedGeneration?: number): Promise<PollDisposition> {
        const enrollment = await this.requireEnrollment();
        ...
        try {
            ...
            for (const session of sessions) {
                ...
                const disposition = await this.processClaim(enrollment, claim, expectedGeneration);
    ...
    private async processClaim(enrollment: EnrolledRemoteTargetVaultRecord, claim: RemoteTargetClaim, expectedGeneration?: number): Promise<PollDisposition> {
        ...
        if (!session)
            throw new Error("Remote command session is unknown.");
        ...
        const opened = openRemoteControlMessage(claim.envelope, enrollment.encryptionPrivateKeyJwk, expected);
        ...
        if (!authenticity.ok) {
            throw new Error(`Remote command authentication failed: ${authenticity.reason}.`);
        }
        if (claim.commandId !== opened.body.commandId ||
        ...
        const admission = await this.stateStore.transact((state) => {
            ...
            if (!authority ||
                ...
                !authority.grant.targetRuntimeIds.includes(opened.body.targetRuntimeId) ||
        ...
        const started = await this.stateStore.transact((state) => {
            const record = state.commands[opened.body.commandId];
            if (!record)
                throw new Error("Reserved remote command disappeared.");
            if (record.status !== "reserved")
                return { began: false, record };
            record.status = "started";
            record.executionId = randomUUID();
            record.startedAt = this.now();
            record.claimAttempt = claim.claimAttempt;
            record.claimToken = claim.claimToken;
            record.startEnvelope = startEnvelope(opened, record, enrollment, session.controller.encryptionPublicKeyJwk);
    ...
    private async resumeStartedCommand(enrollment: EnrolledRemoteTargetVaultRecord, commandId: string): Promise<PollDisposition> {
        const snapshot = await this.stateStore.read();
        ...
        if (!record.startDelivered) {
            try {
                ...
                if (error instanceof RemoteTargetTransportError &&
                    ...
                    await this.stateStore.transact((state) => {
                        ...
                        if (!session)
                            throw new Error("Remote command session is missing.");
                        current.resultEnvelope = resultEnvelope(current.command, current, enrollment, session.controller.encryptionPublicKeyJwk);
            ...
            await this.stateStore.transact((state) => {
                const current = state.commands[commandId];
                if (current?.startEnvelope?.messageDigest ===
        ...
        let effect: RemoteTargetEffectResult | {
        ...
        const terminal = await this.stateStore.transact((state) => {
            ...
            current.resultEnvelope = resultEnvelope(current.command, current, enrollment, activeSession.controller.encryptionPublicKeyJwk);
            return current;
        });
    ...
    private canProcessGeneration(expectedGeneration: number | undefined): boolean {
        return (expectedGeneration === undefined ||
            (this.running && expectedGeneration === this.loopGeneration));
    }
    private async flushPending(enrollment: EnrolledRemoteTargetVaultRecord): Promise<void> {
        const state = await this.stateStore.read();
        const pending = Object.values(state.commands).filter((record) => record.status !== "reserved" &&
            record.status !== "started" &&
            !record.resultDelivered &&
            record.resultEnvelope !== null);
        for (const record of pending)
            await this.deliverResult(enrollment, record);
    }
    private async deliverResult(enrollment: EnrolledRemoteTargetVaultRecord, record: RemoteTargetStoredCommand): Promise<void> {
    ...
    async revokeSession(sessionId: string): Promise<void> {
        ...
        await this.stateStore.transact((state) => {
            ...
            for (const record of Object.values(state.commands)) {
                if (record.command.body.sessionId !== sessionId)
                    continue;
                if (record.status === "reserved") {
                    ...
                    record.resultEnvelope = resultEnvelope(record.command, record, enrollment, session.controller.encryptionPublicKeyJwk);
    ...
    async start(): Promise<void> {
        ...
        this.pollTail = startup.then(() => undefined, () => undefined);
        await startup;
        if (!this.running || generation !== this.loopGeneration)
            return;
    ...
    async status(): Promise<RemoteTargetRunnerStatus> {
        ...
        return {
            ...
            activeSessions: Object.values(state.sessions).filter((session) => isCommittedSession(session) &&
                session.stoppedAt === null &&
                session.grant.revokedAt === null).length,
            pendingResults: Object.values(state.commands).filter((record) => record.resultEnvelope && !record.resultDelivered).length,
