...
import { HttpRemoteTargetRelayTransport } from "./remote-target-transport";
...
import { RemoteTargetRunner } from "./remote-target-runner";
...
import { normalizeRemoteTargetApiBase } from "./remote-target-transport";
import { normalizeRemoteTargetLoopbackBase } from "./remote-target-executor";
import { remoteTargetVaultInternals } from "./remote-target-vault";
...
import { type RemoteTargetManagedNetworkJoiner } from "./remote-target-managed-network";
import { type RemoteTargetPairingChallenge } from "./remote-target-transport";
import { type RemoteTargetPairingChallengeStatus } from "./remote-target-transport";
...
import { type RemoteTargetRunnerStatus } from "./remote-target-runner";
...
export interface DesktopRemoteTargetResumeResult {
    resumed: boolean;
    reason: "active_authority" | "activation_recovery" | "not_enrolled" | "no_active_authority";
}
...
export class RemoteTargetDesktopService {
    ...
    constructor(private readonly vault: RemoteTargetVault = new RemoteTargetVault(), private readonly stateStore: RemoteTargetStateStore = new JsonFileRemoteTargetStateStore(), private readonly transport: RemoteTargetRelayTransport = new HttpRemoteTargetRelayTransport(), private readonly now: () => number = Date.now, private readonly managedNetworkJoiner: RemoteTargetManagedNetworkJoiner = new TailscaleCliManagedNetworkJoiner()) { }
    private prepareLoopbackConfiguration(input: RemoteTargetLoopbackConfiguration): PreparedRemoteTargetLoopbackConfiguration {
    ...
    private enqueueConfiguration<T>(operation: () => Promise<T>): Promise<T> {
        ...
        this.configurationTail = queued.then(() => undefined, () => undefined);
        return queued;
    }
    private async installLoopbackConfiguration(prepared: PreparedRemoteTargetLoopbackConfiguration): Promise<RemoteTargetRunner> {
        if (this.runner && this.loopbackConfigurationKey === prepared.key) {
            return this.runner;
        }
        const replacement = new RemoteTargetRunner(this.vault, this.stateStore, this.transport, prepared.executor, { now: this.now, pollIntervalMs: prepared.pollIntervalMs });
        await this.runner?.stop();
        this.runner = replacement;
        this.loopbackConfigurationKey = prepared.key;
        return replacement;
    }
    async configureLoopback(input: RemoteTargetLoopbackConfiguration): Promise<void> {
     ...
     */
    async resumeEligibleLoopback(input: RemoteTargetLoopbackConfiguration): Promise<DesktopRemoteTargetResumeResult> {
        const prepared = this.prepareLoopbackConfiguration(input);
        return this.enqueueConfiguration(async () => {
            ...
            const activeSessions = Object.values(state.sessions).filter((session) => session.activationState !== "staged" &&
                session.stoppedAt === null &&
                session.grant.revokedAt === null &&
                (session.grant.expiresAt === null ||
                    session.grant.expiresAt >= this.now()));
            const stagedSessions = Object.values(state.sessions).filter((session) => session.activationState === "staged" &&
                session.stoppedAt === null &&
                session.grant.revokedAt === null);
            if (enrollment?.status !== "enrolled") {
                if (activeSessions.length > 0) {
                    throw new Error("Remote target credentials are unavailable for the durable active session.");
                }
                return {
                    resumed: false,
    ...
    async enroll(params: unknown): Promise<DesktopRemoteTargetEnrollmentResult> {
        ...
        const apiBaseUrl = normalizeRemoteTargetApiBase(requireString(value.apiBaseUrl, "API URL", 2048));
        const ownerId = requireString(value.ownerId, "owner", 256);
        const ownerAccessToken = requireString(value.ownerAccessToken, "owner authentication", 16384);
        ...
        if (managedNetworkValue !== undefined &&
        ...
        try {
            response = await this.transport.enroll({
                ...
                signingPublicKeyJwk: remoteTargetVaultInternals.publicJwk(prepared.signingPrivateKeyJwk),
                encryptionPublicKeyJwk: remoteTargetVaultInternals.publicJwk(prepared.encryptionPrivateKeyJwk),
                managedNetwork,
            });
        }
        catch (error) {
            if (error instanceof RemoteTargetTransportError &&
                ...
                if (!(await this.vault.delete())) {
                    throw new Error("Stale remote target identity could not be cleared.", {
                        cause: error,
                    });
                }
                throw new Error("The revoked enrollment was cleared locally. Retry to create a fresh host identity.", { cause: error });
            }
            if (error instanceof RemoteTargetTransportError &&
                error.code === "MANAGED_ENROLLMENT_PENDING") {
                throw new Error("A prior managed enrollment is still pending. Revoke its host card, then retry.", { cause: error });
            }
            throw error;
        }
            ...
            try {
                ...
                try {
                    ...
                    for (let pageNumber = 0; pageNumber < 10; pageNumber += 1) {
                        ...
                        if (!page.cleanup.more) {
                            if (cleanupFailures.length === 0 &&
                                ...
                                throw new Error("Managed enrollment was revoked, but local credentials could not be removed.");
                            }
                    ...
                    if (!cloudCleanupComplete) {
                        throw new Error("Managed enrollment cleanup exceeded its retry bound.");
                    }
                }
                catch (cleanupCause) {
                ...
                if (cleanupFailures.length > 0) {
                    throw new AggregateError([cause, ...cleanupFailures], "Managed enrollment failed and cleanup must be retried from Devices & Runtimes.", { cause });
                }
                throw cause;
            }
        }
    ...
    async readPairingChallenge(params: unknown): Promise<RemoteTargetPairingChallengeStatus> {
        const value = requireObject(params);
    ...
    private async finishActivation(enrollment: EnrolledRemoteTargetVaultRecord, activation: RemoteTargetActivationResponse): Promise<DesktopRemoteTargetActivationResult> {
        const installer = this.runner ??
            new RemoteTargetRunner(this.vault, this.stateStore, this.transport, {
                execute: async () => ({
                    status: "rejected",
        ...
        try {
            ...
            try {
                ...
                const failure = new ElizaError("Remote activation failed locally and Cloud compensation is required", {
                ...
                logger.error("[RemoteTargetDesktopService] Activation compensation requires retry", failure);
                return {
                    sessionId: activation.sessionId,
                    status: "compensation_required",
                    errorCode: "REMOTE_ACTIVATION_COMPENSATION_REQUIRED",
            ...
            throw new ElizaError("Remote activation failed locally and was rolled back in Cloud", {
        ...
        try {
            ...
            logger.warn("[RemoteTargetDesktopService] Activation commit requires retry", { sessionId: activation.sessionId, error: commitError });
            return {
                sessionId: activation.sessionId,
                status: "commit_required",
                errorCode: "REMOTE_ACTIVATION_COMMIT_REQUIRED",
    ...
    async confirmPairing(params: unknown): Promise<DesktopRemoteTargetActivationResult> {
        const value = requireObject(params);
        const sessionId = requireString(value.sessionId, "session id", 256);
        return this.enqueueConfiguration(async () => {
            const enrollment = await this.vault.load();
    ...
    async activate(params: unknown): Promise<DesktopRemoteTargetActivationResult> {
        const value = requireObject(params);
    ...
    async status(): Promise<RemoteTargetRunnerStatus> {
        ...
        return {
            ...
            activeSessions: Object.values(state.sessions).filter((session) => session.activationState !== "staged" &&
                session.stoppedAt === null &&
                session.grant.revokedAt === null).length,
            pendingResults: Object.values(state.commands).filter((command) => command.resultEnvelope && !command.resultDelivered).length,
            lastPollAt: null,
            lastErrorCode: null,
        };
    }
    ...
    async finalizeHostRevoke(params: unknown): Promise<{
        ...
        return await this.enqueueConfiguration(async () => {
            ...
            if (enrollment.identity.runtimeId !== hostId) {
                throw new ElizaError("Remote host cleanup target does not match this device", {
            ...
            while (true) {
                ...
                if (!page.cleanup.more)
                    break;
                if (page.cleanup.sessions === 0 && page.cleanup.commands === 0) {
                    throw new ElizaError("Remote host revocation made no cleanup progress", {
...
export function desktopRemoteTargetEnroll(params: unknown): Promise<DesktopRemoteTargetEnrollmentResult> {
...
export function desktopRemoteTargetReadPairingChallenge(params: unknown): Promise<RemoteTargetPairingChallengeStatus> {
    return desktopRemoteTargetService.readPairingChallenge(params);
}
export function desktopRemoteTargetConfirmPairing(params: unknown): Promise<DesktopRemoteTargetActivationResult> {
    return desktopRemoteTargetService.confirmPairing(params);
}
export function desktopRemoteTargetActivate(params: unknown): Promise<DesktopRemoteTargetActivationResult> {
    return desktopRemoteTargetService.activate(params);
}
export function desktopRemoteTargetCompensateActivation(params: unknown): Promise<{
    sessionId: string;
    status: "denied" | "revoked";
    alreadyCompensated: boolean;
}> {
...
export function desktopRemoteTargetFinalizeHostRevoke(params: unknown): Promise<{
    cleaned: true;
}> {
