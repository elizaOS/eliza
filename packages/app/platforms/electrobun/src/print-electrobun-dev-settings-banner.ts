...
import { firstWinningEnvString } from "@elizaos/core/runtime-env";
...
import { resolveDesktopApiPortPreference } from "@elizaos/core/runtime-env";
import { resolveDesktopRuntimeMode } from "./api-base";
import { resolveMainWindowPartition } from "./main-window-session";
import { type DevSettingsRow } from "@elizaos/app/dev-settings-table";
 ...
 */
export function printElectrobunDevSettingsBanner(env: Record<string, string | undefined>): void {
    if (!shouldPrintElectrobunDevSettingsBanner())
        return;
    ...
    const rows: DevSettingsRow[] = [
        {
            ...
            source: runtime.mode === "external" && runtime.externalApi.source
            ...
            change: "set ELIZA_DESKTOP_TEST_API_BASE / ELIZA_DESKTOP_API_BASE / ELIZA_API_BASE_URL / ELIZA_API_BASE for external; ELIZA_DESKTOP_SKIP_EMBEDDED_AGENT=1 for disabled",
        },
        {
            ...
            change: "export ELIZA_RENDERER_URL=http://127.0.0.1:<vite>/ (or VITE_DEV_SERVER_URL)",
        },
        {
        ...
        },
        {
            ...
            source: env.ELIZA_DESKTOP_TEST_API_BASE?.trim()
                ...
                : "default (unset)",
            change: "export ELIZA_DESKTOP_TEST_API_BASE=https://… for partition tests",
        },
        {
            setting: "resolveMainWindowPartition",
            effective: partition ?? "—",
            source: "derived — main-window-session.ts",
            change: "export ELIZA_DESKTOP_TEST_PARTITION or ELIZA_DESKTOP_TEST_API_BASE (see source)",
        },
        {
        ...
        },
        {
            ...
            source: screenshotOff
                ...
                : "default (on)",
            change: "export ELIZA_DESKTOP_SCREENSHOT_SERVER=0 to disable; ELIZA_SCREENSHOT_SERVER_PORT for port",
        },
        {
            setting: "NODE_ENV",
    ...
    console.log(colorizeDevSettingsStartupBanner(prependDevSubsystemFigletHeading("electrobun", formatDevSettingsTable("Electrobun shell — effective settings", rows))));
}
