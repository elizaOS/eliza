...
import { isWildcardBindHost } from "@elizaos/core/runtime-env";
...
function isConfiguredExternalApiBaseUrl(parsed: URL): boolean {
    if (parsed.protocol !== "http:")
        return false;
    const configured = resolveExternalApiBase(process.env as Record<string, string | undefined>).base;
...
function isTrustedElizaCloudHttpsUrl(parsed: URL): boolean {
    ...
    return (isElizaCloudControlPlaneHostname(hostname) ||
...
export function normalizeDesktopHttpRequest(params: unknown): {
    ...
    if (!isExternalPlainHttpUrl(parsed) &&
        ...
        throw new Error("desktopHttpRequest supports only external or configured desktop API plain HTTP URLs, or trusted Eliza Cloud HTTPS URLs.");
    }
    const method = typeof record.method === "string" ? record.method : "GET";
    const headers = record.headers && typeof record.headers === "object"
        ? Object.fromEntries(Object.entries(record.headers as Record<string, unknown>)
...
export async function desktopHttpRequest(params: unknown): Promise<{
    ...
    const timeout = new Promise<never>((_, reject) => {
        timeoutId = setTimeout(() => {
            ...
            reject(new Error(`desktopHttpRequest timed out after ${request.timeoutMs}ms.`));
        }, request.timeoutMs);
    });
    try {
