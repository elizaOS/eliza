...
const proofSource = fs.readFileSync(
  path.join(
    repoRoot,
    "packages/app/platforms/electrobun/scripts/test-browser-bridge-windows-native-host.ps1",
  ),
  "utf8",
);
const installerProofSource = fs.readFileSync(
  path.join(
    repoRoot,
    "packages/app/platforms/electrobun/scripts/verify-windows-installer-proof.ps1",
  ),
  "utf8",
);
...
describe("browser bridge hosted Windows native-host proof", () => {
  it("runs a staged compiled-host probe and registration lifecycle contracts", () => {
    ...
    expect(workflowSource).toContain(
      "- name: Compile and probe staged Windows native host\n        shell: powershell\n        run: .\\packages/app/platforms/electrobun/scripts/test-browser-bridge-windows-native-host.ps1",
    );
