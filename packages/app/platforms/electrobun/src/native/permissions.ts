...
import { SYSTEM_PERMISSIONS } from "./permissions-shared";
import { checkNotificationPermission as checkNativeNotificationPermission } from "./mac-window-effects";
import { existsSync } from "node:fs";
import { getMacPermissionDeepLink } from "@elizaos/core/utils/permission-deep-links";
import { isPermissionApplicable } from "./permissions-shared";
import { pathToFileURL } from "node:url";
import { requestNotificationPermission as requestNativeNotificationPermission } from "./mac-window-effects";
import { resolveRuntimeDistPath } from "./agent";
import { type AllPermissionsState } from "./permissions-shared";
import { type PermissionState } from "./permissions-shared";
import { type SendToWebview } from "../types.js";
import { type SystemPermissionId } from "./permissions-shared";
...
let probersByIdPromise: Promise<Map<SystemPermissionId, DesktopPermissionProber>> | null = null;
...
function isPermissionProber(value: unknown): value is DesktopPermissionProber {
    ...
    return (isKnownPermissionId(record.id) &&
...
async function importPermissionProbersModule(): Promise<PermissionProbersModule> {
    ...
    const bundledProbersPath = path.join(runtimeDistPath, "node_modules", "@elizaos", "agent", "dist", "services", "permissions", "probers", "index.js");
    if (existsSync(bundledProbersPath)) {
        return parsePermissionProbersModule(await import(pathToFileURL(bundledProbersPath).href));
    }
    try {
        return parsePermissionProbersModule(await import("@elizaos/agent/services/permissions/probers/index"));
    }
    catch (packageImportError) {
        const cause = packageImportError instanceof Error
            ? packageImportError.message
            : String(packageImportError);
        throw new Error(`Permission probers are unavailable at ${bundledProbersPath}; package import failed: ${cause}`);
    }
}
async function getProbersById(): Promise<Map<SystemPermissionId, DesktopPermissionProber>> {
    probersByIdPromise ??= importPermissionProbersModule().then(({ ALL_PROBERS }) => new Map(ALL_PROBERS.map((prober) => [prober.id, prober])));
    return probersByIdPromise;
}
function buildPermissionState(id: SystemPermissionId, status: PermissionState["status"], options: Partial<Omit<PermissionState, "id" | "status">> = {}): PermissionState {
    return {
        id,
        status,
...
function notificationStateFromNativeStatus(status: number | null, lastRequested?: number): PermissionState {
    if (status === null)
        throw nativeNotificationBridgeUnavailable();
    if (status < 0) {
        throw new ElizaError("macOS rejected the notification authorization request", {
            code: "NOTIFICATION_AUTHORIZATION_FAILED",
            context: { nativeStatus: status },
            severity: "fatal",
        });
    }
    const mapped = status === 2
        ? "granted"
        : status === 1
            ? "denied"
            : status === 3
...
export async function waitForNativeNotificationStatus(initialStatus: number, readStatus: () => number | null, options: {
    operation: "check" | "request";
    timeoutMs: number;
    pollIntervalMs?: number;
}): Promise<number> {
    const pollIntervalMs = options.pollIntervalMs ?? NOTIFICATION_AUTHORIZATION_POLL_INTERVAL_MS;
    const deadline = Date.now() + options.timeoutMs;
    let status = initialStatus;
    const isPending = (value: number) => value === NATIVE_NOTIFICATION_QUERY_PENDING ||
        (options.operation === "request" && value === 0);
    while (isPending(status)) {
        if (Date.now() >= deadline) {
            throw new ElizaError(options.operation === "request"
...
async function checkMacNotificationPermission(): Promise<PermissionState> {
    ...
    if (initialStatus === null)
        throw nativeNotificationBridgeUnavailable();
    const status = await waitForNativeNotificationStatus(initialStatus, checkNativeNotificationPermission, {
        operation: "check",
...
const LEGACY_MAC_NOTIFICATIONS_DEEP_LINK = "x-apple.systempreferences:com.apple.preference.notifications";
interface SettingsCommandProcess {
    exited: Promise<number>;
    stderr: ReadableStream<Uint8Array>;
}
type SettingsCommandSpawner = (argv: string[], options: {
    stdout: "ignore";
    stderr: "pipe";
}) => SettingsCommandProcess;
function spawnSettingsCommand(command: string[], options: {
    stdout: "ignore";
    stderr: "pipe";
}): SettingsCommandProcess {
...
export async function runSettingsCommand(argv: string[], spawnCommand: SettingsCommandSpawner = spawnSettingsCommand): Promise<void> {
    ...
    if (exitCode !== 0) {
        ...
        throw new ElizaError(`Settings command exited ${exitCode}${detail ? `: ${detail}` : ""}`, {
...
export function buildPermissionSettingsCommand(id: SystemPermissionId, targetPlatform: typeof platform = platform): string[] | null {
...
async function openPermissionSettings(id: SystemPermissionId): Promise<void> {
    if (platform === "darwin") {
        try {
            ...
            if (command)
                await runSettingsCommand(command);
        }
        catch (cause) {
