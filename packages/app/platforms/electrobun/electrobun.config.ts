...
export function hasElectrobunWorkspaceRoot(candidateDir: string): boolean {
	return (
		...
		(fs.existsSync(
			path.join(candidateDir, "packages/app/platforms/electrobun/package.json"),
		) ||
			fs.existsSync(
				path.join(
					candidateDir,
					"eliza/packages/app/platforms/electrobun/package.json",
				),
			))
	);
}
...
const rendererDistDir = path.relative(
	...
	fs.existsSync(path.join(repoRoot, "packages/app/package.json"))
		...
		: path.join(repoRoot, "apps/app/web-dist"),
);
