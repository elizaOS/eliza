...
import { MAX_CAPTURE_DURATION_SECONDS } from "./lib/capture-output.ts";
import {
  captureIos,
  captureIosDevice,
  computeExitCode,
...
describe("real CLI rejection for --duration", () => {
  const scriptPath = fileURLToPath(
    new URL("./walkthrough-device-matrix.ts", import.meta.url),
  );
...
describe("iOS device lane (captureIosDevice)", () => {
  ...
  it("runs the real capture and records `captured` with the output dir when device + staged app are present", () => {
    ...
    assert.equal(invoked.rel, "ios-device-capture.ts");
    assert.deepEqual(
      [
    ...
    const result = captureIos({
      ...
      deps: {
        ...
        run: (rel) => {
          ...
          return rel === "mobile-local-chat-smoke.ts" ? 23 : 0;
        },
      },
    });
    assert.equal(result.status, "error");
    assert.equal(result.reason, "mobile-local-chat-smoke.ts");
