 ...
 *
 * Run:
 *   bun run --cwd packages/app voice:interactive                       # real mic interactive
 *   bun run --cwd packages/app voice:interactive -- --list-active      # print active optimizations + exit
 *   bun run --cwd packages/app voice:interactive -- --say "hi there"   # skip ASR, inject text (LLM -> TTS half)
 *   bun run --cwd packages/app voice:interactive -- --wav speech.wav   # feed a WAV through the path once
 *   bun run --cwd packages/app voice:interactive -- --no-audio         # write out-<ts>.wav instead of playing
 *   bun run --cwd packages/app voice:interactive -- --no-mtp           # disable MTP (loud warning per AGENTS.md)
 *   bun run --cwd packages/app voice:interactive -- --room my-room     # set the conversation id
 *
...
async function inspectActiveOptimizations(args) {
  ...
  if (ttsBackend === "fused") {
    ...
    missing.push({
      ...
      fix: "build it: see packages/app/scripts/omnivoice-merged/README.md (the fused build ships real OmniVoice TTS + local ASR on macOS-Metal; stub elsewhere)",
    });
    active.push({
      name: "streaming OmniVoice TTS",
      on: false,
    ...
    backends: [
      {
        ...
        verified:
          "needs an Xcode build (macOS + Xcode): packages/app/scripts/ios-xcframework/build-xcframework.mjs + a physical-device smoke",
      },
    ],
  },
  {
