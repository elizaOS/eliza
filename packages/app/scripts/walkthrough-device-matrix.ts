...
/**
 * walkthrough-device-matrix.ts — drive + capture the full-journey walkthrough
 * across the native device matrix (#10198 / #10204).
 *
 * The web/desktop lane (`walkthrough-e2e.ts`) runs the full 25-step DOM-driven
 ...
 *     (`android-e2e.ts` → `playwright.android.config.ts`). The driven journey +
 *     route coverage + on-device chat run there; `capture-android-emu.ts`
 ...
 *     (`ios-onboarding-smoke.ts`, `mobile-local-chat-smoke.ts`) and captured
 *     with `xcrun simctl io` (`capture-ios-sim.ts`). This asymmetry is inherent
 ...
 *
 * Usage:
 *   node scripts/walkthrough-device-matrix.ts --platform ios|android|device|all
...
import { resolveDeviceId } from "./ios-device-lib.ts";
import {
 ...
 * (`ios-device-deploy.ts` stages to `ios/build/device-deploy-stage/App.app`). */
...
export function captureIosDevice({ iosDevice, deps = {} } = {}) {
  ...
  const code = run("ios-device-capture.ts", [
    "--platform",
    "device",
    "--device",
    deviceKey,
  ...
  return lane(code === 0 ? "captured" : "error", null, {
    outputDir,
    note: "On-device XCUITest capture (boot + walkthrough suites) against the signed staged app via ios-device-capture.ts --platform device; WKWebView has no CDP, so the in-app narrative parity runs through the committed AppUITests/BootCaptureUITests harness. See DEVICE_MATRIX.md.",
    device: deviceKey,
...
export function captureIos({ duration, deps = {} }) {
  ...
  const phaseSpecs = [
    ["ios-onboarding-smoke.ts", ["--app-path", app]],
    [
  ...
  return lane(failed ? "error" : "captured", failed?.script ?? null, {
    ...
    note: failed
      ...
      : "iOS WKWebView has no CDP, so the walkthrough is driven in-app via ios-onboarding-smoke.ts and mobile-local-chat-smoke.ts before simctl capture. See DEVICE_MATRIX.md.",
    simUdid: sim,
    appPath: app,
    phases,
  });
...
async function captureAndroid({
  ...
  if (drive) {
    ...
    const driveCode = runScript("android-e2e.ts", driveArgs, {
      ANDROID_SERIAL: chosen,
    });
    if (driveCode !== 0) {
      return lane(
        "error",
        "android-e2e.ts --skip-local-chat failed before capture",
        {
          serial: chosen,
          phase: "drive",
          exitCode: driveCode,
  ...
  return lane(code === 0 ? "captured" : "error", null, {
    ...
    note: drive
      ? "Android WebView is CDP-drivable: this leg first runs `android-e2e.ts --skip-local-chat`, then captures the driven app state from the same device."
      : "Passive Android screen capture only; rerun without `--skip-android-drive` to drive `android-e2e.ts --skip-local-chat` before capture.",
    serial: chosen,
