 ...
 * startup trace from `capture-startup-trace.ts` (per-phase `timeline` marks)
 * and the build-time JSON from `run-mobile-build.ts` — turns them into named
 ...
 *
 * Usage:
 *   node scripts/check-startup-budget.ts \
 ...
 *
 *   --trace          capture-startup-trace.ts artifact; the cold run's TTI mark
 *                    becomes the `tti/<tti-target>` metric
 *   --tti-target     which tti budget the trace measures (default ci-web)
 *   --build-timing   run-mobile-build.ts timing json ({ target, buildMs })
