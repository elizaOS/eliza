 ...
 * (`bun run --cwd packages/app voice:duet`).
 *
 ...
 *
 * Run:
 *   bun run --cwd packages/app voice:duet                                  # 0.6b, endless, in-process
 *   bun run --cwd packages/app voice:duet -- --turns 20 --report out.json  # 20 round-trips -> bench JSON
 *   bun run --cwd packages/app voice:duet -- --model eliza-1-2b --two-process
 *   bun run --cwd packages/app voice:duet -- --list-active                 # prereq report, then exit
 *   bun run --cwd packages/app voice:duet -- --platform-report             # cross-platform matrix, then exit
 *   bun run --cwd packages/app voice:duet -- --character-a a.json --character-b b.json --seed-text "hey there"
 *   bun run --cwd packages/app voice:duet -- --parallel 2 --draft-max 16 --ring-ms 240 --prewarm-lead-ms 0
 *
...
const USAGE = `Usage: bun run --cwd packages/app voice:duet [-- <options>]
...
// ---------------------------------------------------------------------------
// Prereq inspection (reuses voice-interactive.ts's inspector)
// ---------------------------------------------------------------------------
...
async function inspectDuetPrereqs(args) {
  ...
  const { inspectActiveOptimizations } = await import("./voice-interactive.ts");
...
async function main() {
  ...
  if (args.platformReport) {
    const { printPlatformReport } = await import("./voice-interactive.ts");
  ...
  // (the same step `voice-interactive.ts` does before `engine.load`).
  try {
    const { ensureBundleRegistered } = await import("./voice-interactive.ts");
