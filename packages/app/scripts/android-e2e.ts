...
import { startAndroidScreenRecord } from "./lib/android-capture.ts";
import {
...
import { resolveAndroidE2eBuildScript } from "./lib/android-e2e-build.ts";
import {
...
import {
  ...
  setBundleDevice,
  startBundleStep,
} from "./lib/device-e2e-bundle.ts";
import { acquireDeviceLease, isDeviceLeased } from "./lib/device-lease.ts";
  ...
  try {
    ...
    if (!has("--skip-local-chat")) {
      ...
      run(
        ...
        "node",
        [
          "scripts/mobile-local-chat-smoke.ts",
          "--platform",
          "android",
          "--require-installed",
          "--live",
