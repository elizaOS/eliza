 ...
 * `codesign-mas.ts` has finished — confirms that what shipped matches what we
 ...
 *
 * Usage:
 *   node mas-smoke.ts --app=path/to/Built.app
 ...
 *
 * Wired into `desktop-build.ts` behind `--verify-mas` / `ELIZA_VERIFY_MAS=1`.
 */
...
import { BUN_HELPER_BINARY_NAMES, isBunHelperBinary } from "./codesign-mas.ts";
...
function printUsage() {
  const usage = `Usage: node mas-smoke.ts --app=<path/to/Built.app> [options]
