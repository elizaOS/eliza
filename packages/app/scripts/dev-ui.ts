 ...
 *
 * Usage:
 *   bun eliza/packages/app/scripts/dev-ui.ts            # from Eliza repo root — API + UI
 *   bun packages/app/scripts/dev-ui.ts                  # from eliza repo root — same
 *   bun …/dev-ui.ts --ui-only                                # Vite only (API assumed running)
 *   bun …/dev-ui.ts --check-acp-hot-reload=31337             # one-shot reload safety probe
 */
...
import { capacitorPluginsBuildNeeded } from "./lib/capacitor-plugin-build-needed.ts";
import {
  probeApiHealth,
  probeApiHealthWithConfirmation,
} from "./lib/dev-api-health.ts";
import {
  applyDevCloudTarget,
...
} from "./lib/dev-process-lifecycle.ts";
import { createDevTrajectoryRecoveryCoordinator } from "./lib/dev-trajectory-recovery.ts";
...
import { resolveSupervisedViteCommand } from "./lib/dev-ui-vite.ts";
...
import { chooseElizaRuntime, resolveNodeExecPath } from "./run-node-runtime.ts";
