 ...
 *   node eliza/packages/app/scripts/build-capacitor-app.ts
 *
 * `ELIZA_BUILD_FULL_SETUP=1` (or `<PREFIX>_BUILD_FULL_SETUP=1`) prepends
 * `bun install --ignore-scripts` + `run-repo-setup.ts` (CI-style).
 *
...
const repoSetupScript = path.join(__dirname, "run-repo-setup.ts");
const pruneCdnAssetsScript = path.join(__dirname, "prune-cdn-local-assets.ts");
...
const syncPublicAssetsScript = path.resolve(
  __dirname,
  "../../shared/scripts/sync-to-public.ts",
);
