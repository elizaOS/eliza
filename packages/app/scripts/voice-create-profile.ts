...
/**
 * `bun run --cwd packages/app voice:create-profile` — build-time / fine-tune-time voice
 * profile creation.
 *
 * This is NOT a runtime UI. Profile creation happens at build time:
 *   - Invokes the I6 freeze pipeline (freeze-voice.ts) to produce an
 *     ELZ2 v2 preset binary.
 *   - Emits an entry into `models/voice/profiles/catalog.json`.
 *   - Appends to `models/voice/CHANGELOG.md`.
 *
 * Usage:
 *   bun packages/app/scripts/voice-create-profile.ts \
...
function usage(code = 0) {
  console.log(
    [
      "Usage: bun packages/app/scripts/voice-create-profile.ts [flags]",
      "",
      "Flags:",
...
async function main() {
  ...
  console.log(
    `\n[voice:create-profile] step 1: run freeze-voice.ts → ${presetPath}`,
  );
  ...
  if (!existsSync(FREEZE_VOICE_SCRIPT)) {
    console.error(
      `[voice:create-profile] freeze-voice.ts not found at ${FREEZE_VOICE_SCRIPT}`,
    );
    process.exit(1);
  }
  ...
  if (result.status !== 0) {
    console.error(
      `[voice:create-profile] freeze-voice.ts exited with code ${result.status}`,
    );
