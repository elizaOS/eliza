...
} from "./smoke-view-declarations.ts";
...
// `smokeViewDeclarations` is imported from ./smoke-view-declarations.ts so it
...
const server = http.createServer(async (req, res) => {
          ...
          ? [
              {
                ...
                hasProfiles: false,
                kind: "secret",
                lastModified: Date.parse(SMOKE_GENERATED_AT),
              },
              {
  ...
  if (req.method === "GET" && url.pathname === "/api/i18n/locale") {
    ...
    // (packages/app/src/api/i18n-locale-routes.ts). The SPA polls this on
