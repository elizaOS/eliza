...
const CELLS = [
  ...
  },
  {
    ...
    cwd: "packages/app/platforms/android",
  ...
  },
  {
    ...
    command: ["node", "packages/app/scripts/voice/stage-b-stt-bench.mjs"],
  ...
  },
  {
    ...
    class: "stt-evaluation",
    command: ["node", "packages/app/scripts/voice/voice-stage-b-eval.mjs"],
    evidence: [
...
function hasPackagedDesktopLauncher(platform) {
  ...
  const roots = [
    path.join(REPO_ROOT, "packages/app/platforms/electrobun/build"),
    path.join(REPO_ROOT, "packages/app/platforms/electrobun/artifacts"),
  ];
  if (platform === "darwin") {
    return roots.some(
      (root) =>
  ...
  switch (cell.probe) {
    ...
    case "macosElectrobun":
      ...
      if (!process.env.ELIZA_VOICE_DESKTOP_API_BASE?.trim()) {
        return {
          available: false,
          reason:
            "ELIZA_VOICE_DESKTOP_API_BASE is not set to a real app API base",
        };
      }
    ...
    case "androidGradle": {
      const androidDir = path.join(
        REPO_ROOT,
        cell.cwd ?? "packages/app/platforms/android",
      );
