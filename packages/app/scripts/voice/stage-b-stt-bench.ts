...
function renderMarkdown(report) {
  ...
  lines.push(
    "  `ELIZA_VOICE_STAGE_B_REPORT` with `packages/app/scripts/voice/voice-stage-b-eval.ts`.",
  );
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    "The complete #9958 Stage-B decision is green only when `voice-stage-b-eval.ts`",
  );
  lines.push(
