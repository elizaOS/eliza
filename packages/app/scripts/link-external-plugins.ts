 ...
 * setup-upstreams.ts so the same symlink semantics (relative
...
import { createPackageLink } from "./setup-upstreams.ts";
 ...
 * setup-upstreams.ts so dynamic imports work from every entry
...
export function linkExternalPlugins(
  ...
  for (const sourceDir of sources) {
    ...
    // `patch-deps.ts` earlier removed the nested @elizaos/core from
