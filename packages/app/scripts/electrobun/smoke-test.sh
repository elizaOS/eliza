...
#
# Usage:
#   bash packages/app/scripts/electrobun/smoke-test.sh
#
...
ELECTROBUN_DIR="$(cd "$SCRIPT_DIR/../../platforms/electrobun" && pwd)"
...
has_electrobun_workspace_root() {
  ...
  [[ -f "$candidate/packages/app/platforms/electrobun/package.json" || -f "$candidate/eliza/packages/app/platforms/electrobun/package.json" ]] || return 1
}

has_outer_eliza_electrobun_checkout() {
  local candidate="$1"
  [[ -f "$candidate/eliza/packages/app/platforms/electrobun/package.json" ]]
}
...
if [[ -f "$REPO_ROOT/eliza/packages/app/package.json" && -z "${ELIZA_ELIZA_SOURCE:-}" && -z "${ELIZA_SOURCE:-}" ]]; then
