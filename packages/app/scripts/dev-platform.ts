 ...
 *    says `packages/app/web-dist` is missing or older than sources (cheap mtime heuristic). Override:
 ...
 * `eliza/packages/app/scripts/lib/allocate-loopback-port.ts` resolves **ELIZA_API_PORT** (default
...
import {
  ...
  stopMacApplication,
} from "./lib/macos-launch-services-lifecycle.ts";
import { resolveMacNativeEffectsDevPlan } from "./lib/macos-native-effects-dev.ts";
import { extendNodePathEnv } from "./lib/node-path-env.ts";
import { formatOrchestratorDesktopDevBanner } from "./lib/orchestrator-desktop-dev-banner.ts";
import { appIdentityEnv } from "./lib/read-app-identity.ts";
import { resolveRendererBuildAction } from "./lib/renderer-build-action.ts";
import {
  drainSpawnedChildren,
  resolveShutdownDrainWindowMs,
} from "./lib/shutdown-drain.ts";
...
// outer wrapper. dev-platform.ts originally only handled the wrapper
...
const isStandaloneEliza =
  ...
  existsSync(path.join(_elizaRoot, "packages", "app", "package.json"));
...
const devServerEntry = isElizaMonorepo
  ? "eliza/packages/app/src/runtime/dev-server.ts"
  : "packages/app/src/runtime/dev-server.ts";
...
function syncRendererPublicAssets() {
  const syncScript = path.join(
    bundleRoot,
    isElizaMonorepo
      ? "eliza/packages/ui/scripts/sync-to-public.ts"
      : "packages/ui/scripts/sync-to-public.ts",
  );
...
if (process.argv.includes("--help") || process.argv.includes("-h")) {
  console.log(`Usage: bun run dev:desktop [options]
       ELIZA_DESKTOP_VITE_WATCH=1 bun eliza/packages/app/scripts/dev-platform.ts   # same as bun run dev
...
if (!existsSync(rootDistEntry)) {
  ...
  // that re-exports the canonical app entry. Electrobun's bun
  ...
  const entryTs = isStandaloneEliza
    ...
    : "../eliza/packages/app/src/entry.ts";
  const entryStub = `// auto-generated by dev-platform.ts — re-exports the eliza entry
...
// Same bounded window as dev-ui.ts (#18435): covers the longest bounded
