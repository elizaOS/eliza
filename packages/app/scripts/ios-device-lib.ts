 ...
 * (ios-device-deploy.ts / ios-device-logs.ts / ios-device-capture.ts).
 *
 * Everything exported here is deterministic, or takes impure edges as injected
 * dependencies, so it can be unit-tested (see ios-device-lib.test.ts, run by
 ...
 * `ios-device-logs.ts --pull-boot-trace`.
 *
 ...
 * consumed only by ios-device-logs.ts (native code does not read it).
 */
