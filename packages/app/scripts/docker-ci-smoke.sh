...
#
# Usage:
#   bash packages/app/scripts/docker-ci-smoke.sh [--tag TAG] [--version VERSION] [--skip-smoke]
#   # Verify an already-built/pulled image without rebuilding:
#   DOCKER_IMAGE=ghcr.io/... bash packages/app/scripts/docker-ci-smoke.sh --boot-verify-only
#
# Environment:
...
on_error() {
  ...
  if [[ "${GITHUB_ACTIONS:-}" == "true" ]]; then
    printf '::error file=packages/app/scripts/docker-ci-smoke.sh,line=%s::docker-ci-smoke command failed with exit code %s: %s\n' "$line" "$status" "$command" >&2
  fi
}
