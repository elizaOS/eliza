...
// throws — `build-llama-cpp-mtp.ts` then exits non-zero rather than
  ...
  if (found.length === 0) {
    throw new Error(
      `[mtp-build] server-structured-output: no llama-server source found ` +
        ...
        `changed; update kernel-patches/server-structured-output.ts.`,
    );
  }
  return found;
}
    ...
    if (!target) {
      ...
      throw new Error(
        `[mtp-build] server-structured-output: could not find a llama-server ` +
          ...
          `kernel-patches/server-structured-output.ts to re-anchor the MTP ` +
