...
// build-llama-cpp-mtp.ts; its applyForkPatches() calls patchCudaKernels +
...
// (consumed by build-llama-cpp-mtp.ts's `backend === "cuda"` branch).
...
export function patchCudaKernels(cacheDir, { dryRun = false } = {}) {
  ...
  for (const name of CUDA_KERNEL_FILES) {
    ...
    if (dryRun) {
      ...
      const stamped =
        ...
        `// build-llama-cpp-mtp.ts. Frozen — do not edit in fork. Body gated by\n` +
        `// GGML_CUDA_FUSED_ATTN_QJL (build flag set in build-llama-cpp-mtp.ts).\n` +
        text;
  ...
  console.log(
    `[cuda-kernels] ${dryRun ? "(dry-run) " : ""}staged ${copied.length} CUDA kernel(s): ${copied.join(", ")}` +
      ` (needs ${CUDA_KERNEL_CMAKE_FLAGS.join(" ")} from build-llama-cpp-mtp.ts)`,
  );
