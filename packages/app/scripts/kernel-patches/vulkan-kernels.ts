...
} from "./metal-kernels.ts";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// packages/app/scripts/kernel-patches/  →  plugin-local-inference/native/vulkan/
  ...
  for (const name of VULKAN_ALL_STAGED_FILES) {
    ...
    if (dryRun) {
      ...
      const stamped =
        ...
        `// build-llama-cpp-mtp.ts. Frozen — do not edit in fork.\n` +
        text;
...
// Public entry point used by build-llama-cpp-mtp.ts.
export function patchVulkanKernels(
  ...
  // build-llama-cpp-mtp.ts post-build via the requiredKernels audit.
  console.log(
