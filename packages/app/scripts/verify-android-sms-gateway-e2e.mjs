...
function usage() {
  return [
    "Usage: node packages/app/scripts/verify-android-sms-gateway-e2e.mjs [options]",
    "",
    "Options:",
...
async function resolveSerial(args) {
  ...
  throw new Error(
    `Timed out waiting ${args.waitDeviceSeconds}s for an adb device.${services} ` +
      ...
      "node packages/app/scripts/install-android-sms-gateway.mjs --pair auto --wait-pair 300 --connect auto --wait-device 60 --grant-role --clear-logcat --watch-logs 60",
  );
}
