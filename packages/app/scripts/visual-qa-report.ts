 ...
 *
 *   visual-qa-report.ts <image.png> [--baseline <prev.png>] \
...
import { analyzeScreenshot } from "./lib/visual-qa.ts";
...
async function main() {
  ...
  if (!argv[0] || argv[0] === "--help") {
    process.stdout.write(
      `${import.meta.url}\nusage: visual-qa-report.ts <image.png> [--baseline p] [--expect spec.json] [--out report.json]\n`,
    );
