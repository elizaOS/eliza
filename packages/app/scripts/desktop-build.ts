...
} from "./lib/native-activity-tracker-packaging.ts";
...
const BUILD_EXPERIMENTAL_EXACT_WINDOW_HELPER_SCRIPT = path.join(
  SCRIPT_DIR,
  "build-experimental-exact-window-helper.ts",
);
  ...
  run(
    ...
    [
      path.join(
        ...
        "packages",
        "scripts",
        "patch-electrobun-linux-cef-profile.ts",
      ),
 ...
 * the way through staging + packaging only to die in codesign-mas.ts
...
function ensureRootRuntimeBundle() {
  ...
  const entrySource = [
    "// auto-generated by desktop-build.ts",
...
function ensureWorkspaceRuntimePackagesBuilt() {
  ...
  runBun(
    [
      path.join(workspaceRoot, "packages", "scripts", "run-turbo.ts"),
      "run",
      "build:dist",
      "--filter=@elizaos/app",
      "--force",
...
function stageDesktopBuild() {
  ...
  runBun([path.join(SCRIPT_DIR, "ensure-avatars.ts"), `--app=${appName}`], {
    cwd: ROOT,
...
function packageDesktopBuild() {
  ...
  if (
    ...
    const appBundlePath = findLatestMacAppBundle();
    const codesignArgs = [
      path.join(SCRIPT_DIR, "codesign-mas.ts"),
      `--app=${appBundlePath}`,
    ];
    ...
    if (
      ...
      run(
        "node",
        [path.join(SCRIPT_DIR, "mas-smoke.ts"), `--app=${appBundlePath}`],
        {
          cwd: ROOT,
          env: packageEnv,
...
function printUsage() {
  console.log(`Usage: node eliza/packages/app/scripts/desktop-build.ts <command> [options]
...
Options:
  ...
  --verify-mas                     After MAS codesign, walk the bundle and verify the tightened
                                   entitlements via mas-smoke.ts. Off by default; ELIZA_VERIFY_MAS=1
                                   also enables it.
