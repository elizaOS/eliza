 ...
 *   2. `scripts/setup-upstreams.ts` may run `bun install --cwd eliza`
...
export const repoSetupSteps = [
  "patch-workspace-plugins.ts",
  "patch-deps.ts",
  "ensure-type-package-aliases.ts",
  "scripts/setup-upstreams.ts",
  "scripts/sync-workspace-default-skills.mjs",
  "ensure-avatars.ts",
  "link-browser-server.ts",
  "link-external-plugins.ts",
  "ensure-vision-deps.ts",
...
function resolveRepoSetupStepPath(repoRoot, step) {
  ...
  // root (e.g. Eliza's /scripts/setup-upstreams.ts). Unprefixed names
  // resolve to this app/scripts/ directory. This split lets Eliza
