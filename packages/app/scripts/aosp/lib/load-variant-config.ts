...
export function resolveAppConfigPath({
  ...
  return path.join(repoRoot, "packages", "app", "app.config.ts");
}
 ...
 * fine.
 */
function extractObjectBlockBody(src: string, key: string) {
...
function skipStringLiteral(src: string, start: number) {
...
function matchString(body: string, key: string) {
  const re = new RegExp(`\\b${key}\\s*:\\s*["'\`]([^"'\`]+)["'\`]`);
