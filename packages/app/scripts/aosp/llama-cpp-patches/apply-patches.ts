#!/usr/bin/env node
// apply-patches.ts — apply the QJL + PolarQuant patch series on top
// of a checked-out apothic/llama.cpp-1bit-turboquant tree.
//
// Called from compile-libllama.ts after the fork is cloned + checked
...
//
// Usage:
//   node apply-patches.ts --repo <path-to-llama.cpp-checkout>
//   node apply-patches.ts --repo <path> --series qjl,polarquant
...
if (!repo) {
  console.error(
    "usage: apply-patches.ts --repo <llama.cpp-checkout> [--series qjl,polarquant]",
  );
  process.exit(1);
}
