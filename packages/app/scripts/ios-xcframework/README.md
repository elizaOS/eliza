...
`packages/app/scripts/build-llama-cpp-mtp.mjs` into a
...
node packages/app/scripts/build-llama-cpp-mtp.mjs --target ios-arm64-metal
node packages/app/scripts/build-llama-cpp-mtp.mjs --target ios-arm64-simulator-metal

# Assemble the xcframework with full kernel verification.
node packages/app/scripts/ios-xcframework/build-xcframework.mjs \
...
- `node packages/app/scripts/run-mobile-build.mjs ios` (or
...
ELIZA_IOS_INCLUDE_LLAMA=1 \
  node packages/app/scripts/run-mobile-build.mjs ios
```
...
ELIZA_IOS_DEVELOPMENT_TEAM=<Apple Team ID> \
  node packages/app/scripts/ios-xcframework/run-physical-device-smoke.mjs \
    --build-if-missing \
