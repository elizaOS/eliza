 ...
 * `run-mobile-build.ts` invoked cmake against the upstream npm package's
 ...
 * by `build-llama-cpp-mtp.ts --target ios-arm64-metal` and
 * `--target ios-arm64-simulator-metal` and assembles them into a
 * well-formed `.xcframework` bundle.
 *
 * Invocation:
 *   node build-xcframework.ts --output <dir>
 ...
 *     [--build-if-missing]              run build-llama-cpp-mtp.ts first
 ...
 *     --verify (matches the build-llama-cpp-mtp.ts CAPABILITIES gate).
...
const MTP_BUILD_SCRIPT = path.join(SCRIPTS_DIR, "build-llama-cpp-mtp.ts");
...
const REQUIRED_IOS_KERNEL_SYMBOLS = [
  ...
  },
  {
    ...
    // MTLLibraryError Code=5. build-llama-cpp-mtp.ts defaults to metal3.1;
...
function printHelp() {
  process.stdout.write(`Usage: build-xcframework.ts --output <dir> [options]
...
Options:
  ...
  --build-if-missing            Invoke build-llama-cpp-mtp.ts for any
...
function loadSlice(dir, sliceName) {
  ...
  if (archives.length === 0) {
    throw new Error(
      `[ios-xcframework] slice ${sliceName} (${dir}) contains no .a archives — re-run build-llama-cpp-mtp.ts.`,
    );
  }
  ...
  if (missing.length > 0) {
    ...
    throw new Error(
      `[ios-xcframework] AGENTS.md §3 kernel-symbol audit FAILED:\n${lines.join(
        ...
        `The static archives produced by build-llama-cpp-mtp.ts do not\n` +
        ...
        `kernel-patches/metal-kernels.ts to handle the EMBED_LIBRARY=ON\n` +
