 ...
 * `lib/ios-device-e2e-lib.ts`:
 *   1. ios-device-deploy.ts (build → sign full app + appexes → install). The deploy
 *      also writes the renderer-freshness assert + the deploy ledger row.
 *   2. ios-device-capture.ts --platform device  (BootCapture — the ON-DEVICE
 ...
 *   3. ios-device-logs.ts --no-console --pull-boot-trace  (full-Bun engine
 ...
 * shared `lib/device-e2e-bundle.ts` framework, the same one `android-e2e.ts`
 * and `ios-e2e.ts` use, so all three device lanes emit one summary shape
 ...
 *
 * Usage:
 *   node scripts/ios-device-e2e.ts [--device <id>] [--skip-appexes]
...
} from "./lib/ios-device-e2e-lib.ts";
import {
...
async function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node scripts/ios-device-e2e.ts [--device <id>] [--skip-appexes] [--skip-logs] [--require-chat] [--bundle-id <id>] [--output <dir>]",
    );
    return;
  }
