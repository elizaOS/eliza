#!/usr/bin/env bun
/**
 * stage-desktop-fused-lib.ts — build + stage the fused `libelizainference` for
 ...
 * already existed for the mobile slices — `stage-elizavoice-lib.ts` (Android
 * NDK → jniLibs) and `build-llama-cpp-mtp.ts` (iOS xcframework). Before this,
 ...
 *
 * Usage:
 *   bun packages/app/scripts/stage-desktop-fused-lib.ts \
...
} from "./lib/fused-artifact-integrity.ts";
