 ...
 *   1. supervisor-recovery — spawn the real run-node.ts with a fake child that
 ...
 * run in their own lanes (android-e2e.ts, the chainsaw suite).
 *
 * Usage: node packages/app/scripts/stability-suite.ts [--scoreboard <path>]
 */
...
const MAX_RESTARTS_IN_WINDOW = 5; // mirror run-node.ts
...
> Regenerated by \`node packages/app/scripts/stability-suite.ts\`. Raw run
...
| --- | --- | --- | --- |
| Android device | \`ElizaAgentService\` \`WatchdogThread\` (health poll + \`scheduleRestart\`) | \`packages/app/scripts/android-e2e.ts\` (needs \`ANDROID_SERIAL\`) | code present; device-connected run gated on a phone |
