...
function usage() {
  return [
    "Usage: node packages/app/scripts/continue-sms-gateway-work.ts [options]",
    "",
    "Options:",
...
function main() {
  ...
  runStep("software-contracts", "node", [
    script("test-sms-gateway-software.ts"),
  ]);
  if (args.applyDns) {
  ...
  if (!args.skipWatch && args.watchSeconds > 0) {
    runStep(
      ...
      "node",
      [
        script("watch-sms-gateway-readiness.ts"),
        "--timeout",
  ...
  const status = runStep("status", "node", [script("sms-gateway-status.ts")], {
    allowFailure: true,
    timeout: 240_000,
  });
