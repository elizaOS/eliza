...
/**
 * walkthrough-e2e.ts — the single runnable entrypoint for the full-journey
 ...
 *   2. Runs the per-step vision review (`packages/scripts/ai-qa/review-walkthrough.ts`)
 ...
 *
 * Usage:
 *   node scripts/walkthrough-e2e.ts [--live] [--viewports desktop,mobile]
...
async function main() {
  ...
  const childEnv = {
    ...
    WALKTHROUGH_COMMAND: `walkthrough-e2e.ts --platform web${args.live ? " --live" : ""} --viewports ${args.viewports}`,
  };
