...
import {
  ...
  resolveAdb,
} from "./lib/android-device.ts";
import {
  assertMarkerSurvivedRelaunch,
  buildRelaunchMarker,
} from "./lib/chat-history-persistence.ts";
import {
  MAX_TIMER_DELAY_MS,
  parseNonNegativeSafeInteger,
  parsePositiveSafeInteger,
  startDeviceE2eHostAgent,
} from "./lib/host-agent.ts";
import {
  assertIosFullBunSmokeSuccess,
  iosFullBunSmokeResultTimeMs,
  parseIosFullBunSmokeResult,
} from "./lib/ios-full-bun-smoke-contract.ts";
import { assertInstalledIosAppRendererFresh } from "./lib/ios-renderer-stamp.ts";
import {
...
} from "./lib/ios-sim-defaults-hygiene.ts";
import { evaluateLocalInferenceReadiness } from "./lib/local-inference-readiness.ts";
import { resolveSmokeCommand } from "./lib/smoke-command-proxy.ts";
...
function printHelp() {
  console.log(`Usage: node packages/app/scripts/mobile-local-chat-smoke.ts [options]
...
async function requireLocalInferenceReady(baseUrl, authToken) {
  ...
  for (
    ...
    // Three accepted serving paths (see local-inference-readiness.ts):
