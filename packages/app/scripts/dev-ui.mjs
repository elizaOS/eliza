 ...
 *   bun packages/app/scripts/dev-ui.mjs                  # from eliza repo root — same
...
function resolveCapacitorPluginNamesPath(devCwd) {
  ...
  const elizaSubmodulePresent = existsSync(
    path.join(devCwd, "eliza", "packages", "app", "package.json"),
  );
...
function resolveDevServerEntryRelativePath(devCwd) {
  ...
  if (existsSync(elizaMonorepoEntry)) {
    return "eliza/packages/app/src/runtime/dev-server.ts";
  }
  ...
  if (existsSync(elizaRepoEntry)) {
    return "packages/app/src/runtime/dev-server.ts";
  }
  ...
  throw new Error(
    `[dev-ui] dev-server entry not found under ${devCwd}. Expected eliza/packages/app/... (Eliza-style checkout), packages/app/... (eliza repo root), or a packaged @elizaos/app runtime entry.`,
  );
}
...
function createApiChildEnv(baseEnv) {
  ...
  // setting: app must first honor an explicit shell value or persisted
...
function startVite() {
  ...
  // The API runtime is selected independently. Vite's HTTP proxy depends on
  // Node socket methods that Bun does not implement, so the UI child uses the
  // repository's required Node 24+ runtime even when the API runs under Bun.
