...
function hasPath(url: string, expectedPath: string) {
  try {
...
export function isLifeOpsActivitySignals503(status: number, url: string) {
  return status === 503 && hasPath(url, LIFEOPS_ACTIVITY_SIGNALS_PATH);
}

export function isBrowserBridgeCompanions503(status: number, url: string) {
  return status === 503 && hasPath(url, BROWSER_BRIDGE_COMPANIONS_PATH);
}

export function isExpectedDevSmokeResponseCandidate(
  status: number,
  url: string,
) {
  return (
...
export function browserResourceConsoleErrorStatus(text: string) {
...
export function isExpectedEmbeddedBrowserConsoleError(
  text: string,
  locationUrl: string,
) {
...
export class ExpectedBrowserResponseConsoleMatcher {
  #expectedResponses = new Map<string, number>();

  recordResponse(status: number, url: string) {
  ...
  consumeConsoleError(text: string, locationUrl: string) {
...
export class ExpectedDevSmokeFailureMatcher {
  ...
  recordResponse(status: number, url: string, body: string | null | undefined) {
