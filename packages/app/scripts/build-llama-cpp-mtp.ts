#!/usr/bin/env node
/**
 * build-llama-cpp-mtp.ts — per-target static llama.cpp slice builder.
 *
 ...
 * `ios-xcframework/build-xcframework.ts` packages into the
 * `LlamaCpp.xcframework` consumed by `llama-cpp-capacitor`.
 *
 * Output layout (consumed by build-xcframework.ts loadSlice()):
 *
 ...
 * `ensureMtpIosTarget()` / build-xcframework.ts both fail closed. There is no
 * fallback to a stock (kernel-less) framework.
 *
 * Usage:
 *   node build-llama-cpp-mtp.ts --target ios-arm64-metal
 *   node build-llama-cpp-mtp.ts --target ios-arm64-simulator-metal
 *   node build-llama-cpp-mtp.ts --list
 *
 * Env overrides:
 *   ELIZA_STATE_DIR                 install root (default ~/.eliza) — MUST match
 *                                   run-mobile-build.ts mtpTargetOutDir()
...
import {
  ...
  sha256File,
} from "./lib/ios-fused-slice-cache.ts";
...
// ── State dir — MUST mirror run-mobile-build.ts elizaStateDirForBuild() and
//    ios-xcframework/build-xcframework.ts elizaStateDir() EXACTLY, otherwise
...
//    ios-xcframework/build-xcframework.ts REQUIRED_IOS_KERNEL_SYMBOLS so the
...
//    mirror the eliza_inference_* half of ios-xcframework/build-xcframework.ts
...
const REQUIRED_FUSED_SYMBOLS = [
  ...
  // verify-fused-symbols.ts gate. A slice missing these throws at synth time on
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--target") target = argv[++i];
      ...
      process.stdout.write(
        `Usage: build-llama-cpp-mtp.ts --target <${SUPPORTED_TARGETS.join("|")}>\n`,
      );
      return;
