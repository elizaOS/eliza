...
`packages/app/scripts/build-llama-cpp-mtp.mjs` invokes when one of
...
`packages/app/src/services/local-inference/voice/ffi-bindings.ts`
...
```bash
bun packages/app/scripts/ffi-stub/tts-stream-ffi-smoke.ts \
...
import { loadElizaInferenceFfi } from
  "@elizaos/app/services/local-inference/voice/ffi-bindings";
...
```sh
make -C packages/app/scripts/ffi-stub
...
make -C packages/app/scripts/ffi-stub verify-stub-rejected
...
```sh
node packages/app/scripts/build-helpers/verify-fused-symbols.mjs \
