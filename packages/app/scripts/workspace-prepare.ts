 ...
 *
 * Usage:
 *   bun scripts/workspace-prepare.ts
 *   bun scripts/workspace-prepare.ts --remote         # submodule update --remote
 *   bun scripts/workspace-prepare.ts --skip-fix-deps  # no rewrite at all
 *   bun scripts/workspace-prepare.ts --skip-install   # no bun install
 *   bun scripts/workspace-prepare.ts --leave-on-disk  # opt out of restore (rare)
 *   bun scripts/workspace-prepare.ts --help
 */
...
if (process.argv.includes("--help") || process.argv.includes("-h")) {
  ...
  bun scripts/workspace-prepare.ts [options]
...
if (!SKIP_FIX) {
  run("fix in-repo deps → workspace:*", "bun", [
    resolve(root, "scripts/fix-workspace-deps.ts"),
  ]);
} else {
  console.log(
