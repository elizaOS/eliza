...
import { nativeLibraryInventory } from "./lib/fused-artifact-integrity.ts";
...
const fixtureScripts = path.join(fixtureRoot, "packages/app/scripts");
fs.mkdirSync(path.join(fixtureScripts, "lib"), { recursive: true });
for (const relative of [
  "stage-desktop-fused-lib.ts",
  "lib/fused-artifact-integrity.ts",
...
const script = path.join(fixtureScripts, "stage-desktop-fused-lib.ts");
