    ...
    backends: [
      {
        ...
        verified:
          "needs an Xcode build (macOS + Xcode): packages/app/scripts/ios-xcframework/build-xcframework.ts + a physical-device smoke",
      },
    ],
  },
  {
    ...
    backends: [
      {
        gpu: "cpu",
        runtime:
          "in-process FFI (@elizaos/plugin-native-inference compile-libllama.ts → libllama .so + aosp-llama/mtp adapters) — NOT llama-server-spawn",
        kernels:
        ...
        verified:
          "needs an Android Studio / NDK build (compile-libllama.ts cross-compiles)",
      },
      {
        gpu: "vulkan",
        runtime:
...
// Re-used by the two-agents duet harness (`voice-duet.ts`): the prereq
