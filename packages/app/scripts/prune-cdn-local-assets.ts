...
import { APP_DIST_BOOTSTRAP_ASSETS } from "./lib/static-asset-manifest.ts";
...
function resolveCleanupHelperScript() {
  const candidates = [
    path.join(repoRoot, "packages", "scripts", "rm-path-recursive.ts"),
    path.join(repoRoot, "eliza", "packages", "scripts", "rm-path-recursive.ts"),
  ];
