 ...
 * in `packages/app/scripts/aosp/compile-shim.mjs`. We resolve from
...
export async function stageAndroidAgentRuntime({
    ...
    if (fs.existsSync(llamaServerStaged)) {
      ...
      tlog(
        `No llama-server staged for ${androidAbi}; MTP spec-decode on AOSP ` +
          ...
          `packages/app/scripts/aosp/compile-libllama.mjs\` to build it.`,
      );
    }
  ...
  // it relative to THIS script's location (eliza/packages/app/scripts/lib/)
  ...
  const runtimeProvenance = {
    ...
    generated_by: "packages/app/scripts/lib/stage-android-agent.mjs",
    claim_boundary:
