...
describe.skipIf(process.platform === "win32")(
  ...
  () => {
    ...
    it("escalates a detached group after its launcher exits but a native descendant survives", async () => {
      const child = spawn(
        process.execPath,
        [
          "-e",
...
describe("dev-platform supervisor wiring", () => {
  const devPlatformSource = readFileSync(
    path.join(libDir, "..", "dev-platform.ts"),
    "utf8",
  );
