 ...
 *
 * `ios-device-deploy.ts` stages `App.app` locally before install, so the ledger
 ...
 * resolution, its schema, and its read/write path — `devices-status.ts` (the
 * reader) and `ios-device-deploy.ts` (the writer) both go through it, so there
 ...
 * `lib/ios-renderer-stamp.ts` (the single source of truth the simulator lanes
