 ...
 *
 * `dev-ui.ts` used to SIGTERM its children and then SIGKILL every child
 ...
 *   Optional liveness probe for a target that can outlive its launcher handle.
...
export function drainSpawnedChildren(options) {
  ...
  return new Promise((resolve) => {
    ...
    // A detached launcher can exit before its descendants. Poll the target so
    // cooperative groups finish promptly and stubborn descendants remain in
    // the bounded SIGKILL escalation set.
