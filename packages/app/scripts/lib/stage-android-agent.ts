...
/**
 * Default cache dir for compile-shim.ts's outputs. Mirrors the default
 * in `packages/app/scripts/aosp/compile-shim.ts`. We resolve from
 * `os.homedir()` directly instead of importing `compile-shim.ts` to
...
#
# Staged into the APK by run-mobile-build.ts and copied to the app's
...
function stageNativeLlamaAssetsForAbi({ androidAbi, abiAssetsDir, log }) {
  ...
  // build-helpers/omnivoice-merged.ts fusedExtraCmakeFlags + the bionic JNI
...
/**
 * If `compile-shim.ts` has produced shim + loader-wrap artifacts in the
 ...
 * compile-shim.ts for the ABI. Returns true when the shim cache exists
  ...
  if (!fs.existsSync(cachedWrap) || !fs.existsSync(cachedShim)) {
    throw new Error(
      `[stage-android-agent] Missing compiled SIGSYS shim for ${androidAbi}. ` +
        ...
        `\`node packages/app/scripts/aosp/compile-shim.ts --abi ${androidAbi}\` ` +
...
export async function stageAndroidAgentRuntime({
    ...
    if (fs.existsSync(llamaServerStaged)) {
      ...
      tlog(
        `No llama-server staged for ${androidAbi}; MTP spec-decode on AOSP ` +
          ...
          `packages/app/scripts/aosp/compile-libllama.ts\` to build it.`,
      );
    }
  ...
  const runtimeProvenance = {
    ...
    generated_by: "packages/app/scripts/lib/stage-android-agent.ts",
    claim_boundary:
