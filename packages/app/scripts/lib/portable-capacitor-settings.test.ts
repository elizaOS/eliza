...
import { normalizeCapacitorSettings } from "./portable-capacitor-settings.ts";
...
describe("normalizeCapacitorSettings", () => {
  ...
  it("resolves packages from the host app for isolated app-dir builds", () => {
    const generated = `// DO NOT EDIT THIS FILE! IT IS GENERATED EACH TIME "capacitor update" IS RUN\nproject(':capacitor-android').projectDir = new File('../../node_modules/@capacitor/android/capacitor')\n`;
    const normalized = normalizeCapacitorSettings(generated, {
      appPackageRootRelative: "..",
    });

    expect(normalized).toContain("new File(settingsDir, '..')");
    expect(normalized).not.toContain("new File(settingsDir, '../../../app')");
    expect(
      normalizeCapacitorSettings(normalized, {
        appPackageRootRelative: "..",
      }),
    ).toBe(normalized);
  });
