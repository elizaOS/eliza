 ...
 * scripts/run-ui-playwright.ts and the playwright.*.config.ts webServer
 * commands. Deterministic: candidate probes are injected fixtures mirroring
 * app's run-node-runtime.test.ts, plus one real-probe acceptance case
...
import { parseNodeMajor } from "./run-node-runtime.ts";
