...
function ensureRootRuntimeBundle() {
  ...
  const entrySource = [
    ...
    'import "./node_modules/@elizaos/app/dist/entry.js";',
    "",
  ].join("\n");
...
// carries it; app's runtime probe (ensureBundledFusedLibDir) then finds
...
function stageDesktopBuild() {
  ...
  // Direct Vite builds need the same public asset preparation as web builds.
  runBun(["run", "prebuild"], {
    cwd: APP_DIR,
    label: "Preparing renderer public assets",
  });
  ...
  assertRendererRebuiltSince(path.join(APP_DIR, "web-dist"), {
...
function printUsage() {
  console.log(`Usage: node eliza/packages/app/scripts/desktop-build.mjs <command> [options]
