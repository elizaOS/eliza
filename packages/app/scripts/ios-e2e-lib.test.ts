 ...
 * e2e orchestrator (`ios-e2e.ts`, wired as `test:e2e:ios`). Deterministic, no
...
describe("parseIosE2eArgs", () => {
  it("defaults to a full run with no flags", () => {
    const f = parseIosE2eArgs(["node", "ios-e2e.ts"]);
    expect(f).toEqual({
      device: undefined,
      appPath: undefined,
      output: undefined,
...
describe("leg command builders", () => {
  ...
  it("targets the booted udid for the auth leg", () => {
    expect(buildAuthSmokeCommand(UDID)).toEqual({
      cmd: "node",
      args: [
        "../../packages/app/scripts/mobile-auth-simulator-smoke.ts",
        "--platform",
        "ios",
        "--device",
        UDID,
  ...
  it("builds the cloud provisioning command", () => {
    expect(buildCloudProvisioningCommand()).toEqual({
      cmd: "node",
      args: ["scripts/cloud-provisioning-e2e.ts"],
    });
  });
});
