...
#
# Usage:
#   bash eliza/packages/app/scripts/build-image.sh [OPTIONS]
#
# Options:
...
#
# Examples:
#   bash eliza/packages/app/scripts/build-image.sh                             # Build alpha.89 locally
#   bash eliza/packages/app/scripts/build-image.sh --tag latest --push         # Build + push to server
#   bash eliza/packages/app/scripts/build-image.sh --remote                    # Build on eliza-core-1
#   bash eliza/packages/app/scripts/build-image.sh --version 2.0.0-alpha.54    # Build specific version
#
# Context:
...
#   4. Builds the Vite UI → packages/app/web-dist/
...
load_env_file "eliza/packages/app/deploy/deploy.defaults.env"
...
# ── Select Dockerfile ─────────────────────────────────────────────────────────
if [[ -f "eliza/packages/app/deploy/Dockerfile.ci" ]]; then
  DOCKERFILE="eliza/packages/app/deploy/Dockerfile.ci"
  log "Dockerfile: ${YELLOW}${DOCKERFILE}${NC} (canonical production image)"
else
  die "No Dockerfile found. Expected eliza/packages/app/deploy/Dockerfile.ci."
fi
...
#   "packages/app/src/index.ts"                 ← monorepo path
...
#   "app/src/index.ts"     (packages/ prefix removed)
...
patch_vite() {
  ...
  # 3. Remove "packages/" prefix from elizaos/app paths
  sedi 's|"packages/app/src/index\.ts"|"app/src/index.ts"|g' "$file"
  sedi 's|packages/app/src/\$1|app/src/$1|g' "$file"

  # 4. @elizaos/app → use dist/ (npm package ships compiled output, no src/)
  ...
  # 5. @elizaos/app → keep src/ (local repo source), just remove packages/ prefix
...
hdr "Step 1c: Run postinstall patches"
...
# packages/app/web-dist/ is the compiled React frontend served by the agent
...
if ! $DRY_RUN && [[ ! -d "packages/app/web-dist" ]]; then
  die "packages/app/web-dist not found after vite build — check vite output above"
fi
ok "Vite UI built"
...
if [[ "$DOCKERFILE" == "eliza/packages/app/deploy/Dockerfile.ci" ]]; then
  [[ -f "eliza/packages/app/deploy/.dockerignore.ci" ]] || die "eliza/packages/app/deploy/.dockerignore.ci is required for Dockerfile.ci builds"
  if $DRY_RUN; then
    warn "[dry-run] Would copy eliza/packages/app/deploy/.dockerignore.ci → .dockerignore for Dockerfile.ci"
  else
    ...
    cp eliza/packages/app/deploy/.dockerignore.ci .dockerignore
    ok "Using eliza/packages/app/deploy/.dockerignore.ci for canonical image build"
  fi
fi
...
if $REMOTE; then
  ...
  echo ""
  echo -e "  To deploy:  ${YELLOW}bash eliza/packages/app/scripts/deploy-image.sh --all --image ${IMAGE_NAME}${NC}"
elif $DO_PUSH; then
  ...
  echo ""
  echo -e "  To push:    ${YELLOW}bash eliza/packages/app/scripts/build-image.sh --tag ${TAG} --push${NC}"
  echo -e "  To deploy:  ${YELLOW}bash eliza/packages/app/scripts/deploy-image.sh --all --image ${IMAGE_NAME}${NC}"
fi
