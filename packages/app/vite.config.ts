...
import { ANDROID_CLOUD_ROUTING_MARKERS } from "./scripts/lib/android-cloud-routing-markers.mjs";
import { CAPACITOR_PLUGIN_NAMES } from "./scripts/capacitor-plugin-names.mjs";
import { DEFAULT_APP_ROUTE_PLUGIN_MODULES } from "@elizaos/core/utils/env";
...
import { findAndroidCloudRoutingMarkers } from "./scripts/lib/android-cloud-routing-markers.mjs";
...
import { generateNodeBuiltinStub } from "./vite/native-module-stub-plugin.ts";
import { loadEnv } from "vite";
import { nativeModuleStubPlugin } from "./vite/native-module-stub-plugin.ts";
...
import { removeEmittedBuildStamp } from "./scripts/build-stamp.mjs";
import { removePublicBuildStamp } from "./scripts/build-stamp.mjs";
...
import { resolveDesktopUiPortPreference } from "@elizaos/core/runtime-env";
import { resolveViteDevServerRuntime } from "./vite-dev-origin.ts";
import { shouldSkipBuildStamp } from "./scripts/build-stamp.mjs";
import { swBuildRevPlugin } from "./vite/sw-build-rev-plugin.ts";
import { syncElizaEnvAliases } from "@elizaos/core/utils/env";
...
export function resolveAndroidCloudPrebootLockupDataUri(): string {
    const androidCloudPrebootLockupSvg = fs
        .readFileSync(path.join(here, "public", "brand", "logos", "eliza_logotext.svg"), "utf8")
        .replace(/\s*<rect x="0\.081543" y="1\.84143" width="101\.919" height="101\.919" fill="#FF5800"\s*\/?>\s*/, "\n");
    if (androidCloudPrebootLockupSvg.includes("#FF5800")) {
        throw new Error("Android Cloud preboot lockup still contains its orange backing rect");
    }
...
const bunLinkedPackageCacheRoot = path.join(os.homedir(), ".bun/install/cache/links");
...
function getLucideIconFileMap(): Map<string, string> {
    if (lucideIconFileMap)
        return lucideIconFileMap;
    const map = new Map<string, string>();
    try {
        const barrelPath = path.resolve(elizaRoot, "packages/ui/node_modules/lucide-react/dist/esm/lucide-react.mjs");
        const src = fs.readFileSync(barrelPath, "utf8");
        const lineRe = /export\s*\{([^}]*)\}\s*from\s*['"]\.\/icons\/([\w-]+)\.mjs['"]/g;
...
function buildLucideUsedBarrelSource(): string {
    if (lucideUsedBarrelSource !== null)
    ...
    const roots = ["packages", "plugins", "apps"].map((d) => path.join(elizaRoot, d));
    ...
    const lines = [...used.entries()]
        ...
        .map(([name, file]) => `export { default as ${name} } from "lucide-react/dist/esm/icons/${file}.mjs";`);
...
const pluginBrowserBridgeSrcRoot = path.join(elizaRoot, "plugins/plugin-browser/src");
const uiPkgRoot = path.join(elizaRoot, "packages/ui");
const capacitorCoreEntry = path.join(path.dirname(_require.resolve("@capacitor/core/package.json")), "dist/index.js");
...
const SOLANA_WALLET_CSS_RESOLVED = path.resolve(here, "src/shims/solana-wallet-adapter-react-ui.css");
function resolveBunStorePackageEntry(packageName: string, entryPath: string): string | undefined {
    try {
        const bunDir = path.join(elizaRoot, "node_modules/.bun");
        const versions = fs
            .readdirSync(bunDir)
            ...
            .sort();
        for (const dir of versions.reverse()) {
            const entry = path.join(bunDir, dir, "node_modules", packageName, entryPath);
            if (fs.existsSync(entry))
                return entry;
        }
    }
...
function bufferEsmShimPlugin(): Plugin {
    return {
        ...
        load(id) {
            if (id !== BUFFER_ESM_SHIM_RESOLVED)
                return null;
...
function tryResolvePackageModuleEntryFrom(id: string, fromFile: string): string | undefined {
    try {
...
const capacitorKeyboardEntry = tryResolvePackageModuleEntry("@capacitor/keyboard");
const capacitorPreferencesEntry = tryResolvePackageModuleEntry("@capacitor/preferences");
const capacitorAppEntry = tryResolvePackageModuleEntry("@capacitor/app");
const json5EsmEntry = path.join(path.dirname(_require.resolve("json5/package.json")), "dist/index.mjs");
const markedEntry = path.join(elizaRoot, "plugins/plugin-agent-orchestrator/node_modules/marked/lib/marked.esm.js");
const rechartsEntry = path.join(uiPkgRoot, "node_modules/recharts/es6/index.js");
const nprogressEntry = path.join(uiPkgRoot, "node_modules/nprogress/nprogress.js");
...
const reactRouterDomEntry = tryResolvePackageModuleEntryFrom("react-router-dom", uiPackageJsonPath);
...
const yamlBrowserEntry = (() => {
    try {
        return path.join(path.dirname(_require.resolve("yaml/package.json")), "browser/index.js");
    }
    catch {
        return undefined;
    }
...
const uuidBrowserEntry = (() => {
    try {
        return path.join(path.dirname(_require.resolve("uuid/package.json")), "dist/index.js");
    }
    catch {
        return undefined;
    }
...
const reactDayPickerEntry = tryResolvePackageModuleEntryFrom("react-day-picker", uiPackageJsonPath);
const resolveDateFnsDir = (id: string): string | undefined => {
    if (!reactDayPickerEntry)
        return undefined;
...
const otelApiEntry = (() => {
    ...
    // 3. core's nested node_modules.
    try {
        candidateRoots.push(path.join(path.dirname(_require.resolve("@elizaos/core/package.json")), "node_modules"));
    }
    catch {
        /* core not resolvable */
    }
...
function isExpectedWsProxySocketError(message: unknown, error: unknown): boolean {
    ...
    const errorLike = error && typeof error === "object"
        ...
        : null;
    return (errorLike?.code === "ECONNRESET" ||
 ...
 */
function isExpectedApiProxyConnectError(message: unknown, error: unknown): boolean {
    const text = typeof message === "string" ? message : String(message ?? "");
...
function isKnownToleratedBuildWarning(message: unknown): boolean {
    ...
    if (text.includes("Sourcemap for") &&
        text.includes("@stwd+") &&
        text.includes("points to missing source files")) {
        return true;
    }
    if (text.includes("IMPORT_IS_UNDEFINED") &&
        text.includes("Import `tslFn`") &&
        text.includes("three.webgpu")) {
        return true;
    }
    if (text.includes("Use of direct eval") &&
...
function resolveLocalPackageSourceExportTarget(packageDir: string, exportTarget: string): string | null {
    if (!exportTarget.startsWith("./dist/") || !exportTarget.endsWith(".js")) {
        return null;
    }
    const sourceTarget = path.join(packageDir, "src", `${exportTarget.slice("./dist/".length, -".js".length)}.ts`);
    return fs.existsSync(sourceTarget) ? sourceTarget : null;
}
function isAppPluginPackage(packageRootName: string, entryName: string, pkg: Record<string, unknown>): boolean {
    if (packageRootName !== "plugins")
        return true;
    if (entryName.startsWith("app-"))
        return true;
    const elizaos = pkg.elizaos;
    if (!elizaos || typeof elizaos !== "object")
        return false;
...
function createWorkspacePackageAliases(packageRoots: string[]) {
    ...
    for (const packageRoot of packageRoots) {
        ...
        for (const entry of fs.readdirSync(packageRoot, { withFileTypes: true })) {
            ...
            if (!isAppPluginPackage(packageRootName, entry.name, pkg))
                continue;
            const pkgName = pkg.name;
            if (typeof pkgName !== "string")
                continue;
            const pkgExports = pkg.exports && typeof pkg.exports === "object"
...
function createWorkspacePackageExportAliases(packageDirs: string[]) {
    ...
    for (const packageDir of packageDirs) {
        ...
        for (const [key, value] of Object.entries(pkgExports)) {
            ...
            if (!exportTarget)
                continue;
            const packageSpecifier = key === "." ? pkgName : `${pkgName}/${key.slice(2)}`;
            ...
            if (keyWildcard >= 0 || targetWildcard >= 0) {
                ...
                const targetPattern = path.resolve(packageDir, `${exportTarget.slice(0, targetWildcard)}${wildcardPlaceholder}${exportTarget.slice(targetWildcard + 1)}`);
                aliases.push({
                    find: new RegExp(`^${escapeRegExp(keyPrefix)}(.+)${escapeRegExp(keySuffix)}$`),
...
function createAppPluginBrowserAliases() {
    ...
    for (const entry of fs.readdirSync(pluginsRoot, { withFileTypes: true })) {
        ...
        if (!fs.existsSync(pkgPath))
            continue;
        const pkg = JSON.parse(fs.readFileSync(pkgPath, "utf8")) as Record<string, unknown>;
        if (!isAppPluginPackage("plugins", entry.name, pkg))
            continue;
        const pkgName = pkg.name;
        if (typeof pkgName !== "string")
            continue;
...
const APP_ENV_PREFIX = normalizeEnvPrefix(appConfig.envPrefix?.trim() || appConfig.cliName.trim());
...
const NATIVE_PLUGIN_ALIAS_ENTRIES = CAPACITOR_PLUGIN_NAMES.map((name) => ({
    ...
    replacement: path.join(nativePluginsRoot, `${NATIVE_PLUGIN_DIR_PREFIX}${name}/src/index.ts`),
}));
const CAPACITOR_BUILD_TARGET = process.env.ELIZA_CAPACITOR_BUILD_TARGET ?? "";
const IS_CAPACITOR_MOBILE_BUILD = CAPACITOR_BUILD_TARGET === "ios" || CAPACITOR_BUILD_TARGET === "android";
const IS_ANDROID_CLOUD_RENDERER_BUILD = CAPACITOR_BUILD_TARGET === "android" &&
 ...
 */
export function resolveAppShellLocalCspSources(capacitorBuildTarget: string, isIosStoreBuild: boolean, isAndroidCloudBuild = false): {
...
export const ANDROID_CLOUD_FORBIDDEN_ROUTING_MARKERS = ANDROID_CLOUD_ROUTING_MARKERS;
 ...
 */
export function findAndroidCloudEmittedRoutingFindings(bundle: Record<string, AndroidCloudAuditOutput>): string[] {
...
function androidCloudRendererPolicyPlugin(): Plugin {
    return {
        ...
        generateBundle(_options, bundle) {
            if (!IS_ANDROID_CLOUD_RENDERER_BUILD)
                return;
            const findings = findAndroidCloudEmittedRoutingFindings(bundle);
            if (findings.length > 0) {
                throw new Error(`Android Cloud renderer contains forbidden local routing markers:\n${findings
                    .sort()
                    .map((finding) => `  - ${finding}`)
                    .join("\n")}`);
            }
 ...
 */
export function androidCloudCuratedAssetsPlugin(androidCloudBuild = IS_ANDROID_CLOUD_RENDERER_BUILD): Plugin {
    return {
        name: "android-cloud-curated-assets",
        generateBundle() {
            if (!androidCloudBuild)
                return;
...
export const VIEWPORT_META_NATIVE = "width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover";
export const VIEWPORT_META_WEB = "width=device-width, initial-scale=1.0, viewport-fit=cover";
const NATIVE_AGENT_IPC_BRIDGE_BLOCK = /\s*<!-- ELIZA_NATIVE_AGENT_IPC_BRIDGE_START -->[\s\S]*?<!-- ELIZA_NATIVE_AGENT_IPC_BRIDGE_END -->\s*/;
/**
...
export function stripAndroidCloudIpcBootstrap(html: string): string {
    ...
    return withoutBridge.replace(/(connect-src\s[^;]*?)\s+eliza-local-agent:/, "$1");
}
/** Removes browser-only assets whose public tree is not packaged. */
export function stripAndroidCloudPublicAssetReferences(html: string, prebootLockupDataUri = resolveAndroidCloudPrebootLockupDataUri()): string {
    return html
        .replace(/\s*<link\b[^>]*\brel=["'](?:icon|apple-touch-icon|manifest)["'][^>]*>\s*/gi, "\n")
        .replace(/<img\b[^>]*\bclass=["'][^"']*\beliza-preboot-shell__mark\b[^"']*["'][^>]*>\s*<span\b[^>]*\bclass=["'][^"']*\beliza-preboot-shell__name\b[^"']*["'][^>]*>[^<]*<\/span>/gi, `<img class="eliza-preboot-shell__lockup" src="${prebootLockupDataUri}" alt="" decoding="sync" fetchpriority="high" />`)
        .replace(/\s*<img\b[^>]*\bclass=["'][^"']*\beliza-preboot-shell__mark\b[^"']*["'][^>]*>\s*/gi, "\n");
}
 ...
 */
export function selectAndroidCloudRendererEntry(html: string, androidCloudBuild: boolean): string {
    if (!androidCloudBuild)
        return html;
    if (!html.includes(DEFAULT_RENDERER_ENTRY)) {
        throw new Error(`Android Cloud HTML is missing the expected ${DEFAULT_RENDERER_ENTRY} module entry`);
    }
    return html;
}
/** Runs before Vite discovers HTML module imports, enforcing graph isolation. */
export function androidCloudRendererEntryPlugin(androidCloudBuild = IS_ANDROID_CLOUD_RENDERER_BUILD): Plugin {
    return {
...
export function appShellMetadataPlugin(options: {
    androidCloudBuild?: boolean;
    capacitorBuildTarget?: string;
    resolveAndroidCloudPrebootLockup?: () => string;
} = {}): Plugin {
    const capacitorBuildTarget = options.capacitorBuildTarget ?? CAPACITOR_BUILD_TARGET;
    const isCapacitorMobileBuild = capacitorBuildTarget === "ios" || capacitorBuildTarget === "android";
    const isIosStoreBuild = capacitorBuildTarget === "ios" &&
        (process.env.ELIZA_BUILD_VARIANT === "store" ||
            process.env.ELIZA_RELEASE_AUTHORITY === "apple-app-store");
    const isAndroidCloudBuild = options.androidCloudBuild ??
        (capacitorBuildTarget === "android" &&
            process.env.VITE_ELIZA_ANDROID_RUNTIME_MODE === "cloud");
    const { localHttpSources, localConnectSources } = resolveAppShellLocalCspSources(capacitorBuildTarget, isIosStoreBuild, isAndroidCloudBuild);
    ...
    return {
        ...
        transformIndexHtml(html) {
            ...
            if (isAndroidCloudBuild) {
                ...
                next = stripAndroidCloudPublicAssetReferences(next, (options.resolveAndroidCloudPrebootLockup ??
...
export function devViewStudioPlugin(): Plugin {
    ...
    const assets: ReadonlyMap<string, readonly [
        string,
        string
    ]> = new Map([
...
function productionBuildStampGuardPlugin(): Plugin {
    ...
    const shouldRemoveStamp = () => shouldSkipBuildStamp(process.env, { viteProductionBuild });
    return {
...
function forcedHostModeFlagGuardPlugin(): Plugin {
    return {
        ...
        configResolved(config) {
            ...
            const offending = forbiddenForcedHostModeFlags(loadEnv(config.mode, config.envDir, "VITE_FORCE_"));
            if (offending.length > 0) {
                throw new Error(`${offending.join(", ")} must not be set in a production-mode build: ` +
...
const localVoiceGatewayPort = resolveOptionalLocalVoiceGatewayPort(process.env.ELIZA_LOCAL_VOICE_GATEWAY_PORT);
const viteDevServerRuntime = resolveViteDevServerRuntime(process.env, uiPort, APP_ENV_PREFIX);
...
function resolveOptionalLocalVoiceGatewayPort(raw: string | undefined): number | null {
    if (typeof raw !== "string" || raw.trim() === "")
        return null;
    const port = Number(raw.trim());
    if (!Number.isInteger(port) || port < 1 || port > 65535) {
        throw new Error("ELIZA_LOCAL_VOICE_GATEWAY_PORT must be an integer TCP port");
    }
    return port;
}
/**
 ...
 */
export function resolveLocalRealtimeVoiceDefines(command: string, gatewayPort: number | null, env: NodeJS.ProcessEnv): Record<string, string> {
    if (command !== "serve" || gatewayPort === null)
        return {};
...
export function resolveLocalRealtimeVoiceDefinesFromEnv(command: string, mode: string, gatewayPort: number | null, envDir: string): Record<string, string> {
    return resolveLocalRealtimeVoiceDefines(command, gatewayPort, loadEnv(mode, envDir, "VITE_VOICE_REALTIME_"));
}
export function appDevWsBasePlugin(): Plugin {
    ...
    const wsBaseExpr = "((location.protocol==='https:'?'wss://':'ws://')+location.host)";
    return {
...
const VENDOR_WALLET_TEST = /\/node_modules\/(wagmi|@wagmi\/[^/]+|viem|@rainbow-me\/[^/]+|@walletconnect\/[^/]+|@reown\/[^/]+|@coinbase\/wallet[^/]*|mipd|eventemitter3)(\/|$)/;
...
const VENDOR_REACT_TEST = /\/node_modules\/(react|react-dom|react-is|scheduler|@react-spring)(\/|$)/;
const VENDOR_REACT_SPRING_THREE_TEST = /\/node_modules\/@react-spring\/three(\/|$)/;
...
function resolveManualChunk(id: string): string | undefined {
    ...
    if (normalizedId.includes("/node_modules/@noble/") ||
    ...
    if (/\/node_modules\/@tanstack\/(react-query|query-core)\//.test(normalizedId) ||
        /\/node_modules\/(react-remove-scroll|react-remove-scroll-bar|react-style-singleton|use-callback-ref|use-sidecar|get-nonce|detect-node-es)\//.test(normalizedId)) {
    ...
    if (normalizedId.includes("/node_modules/")) {
        ...
        if (VENDOR_CRYPTO_TEST.test(normalizedId) ||
    ...
    if (normalizedId.includes("/node_modules/")) {
        ...
        if (VENDOR_REACT_TEST.test(normalizedId))
            return "vendor-react";
        if (VENDOR_VRM_TEST.test(normalizedId))
            return "vendor-vrm";
        if (VENDOR_THREE_TEST.test(normalizedId))
            return "vendor-three";
        if (VENDOR_DRACO_TEST.test(normalizedId))
            return "vendor-draco";
    }
    return undefined;
}
...
function buildViteDevSettingsRows(mode: "dev-server" | "build-watch"): DevSettingsRow[] {
    const apiPref = resolveDesktopApiPortPreference(process.env);
    const uiPref = resolveDesktopUiPortPreference(process.env);
    const apiPort = resolveDesktopApiPort(process.env);
    const uiPort = resolveDesktopUiPort(process.env);
    const assetBase = process.env.VITE_ASSET_BASE_URL?.trim() ||
    ...
    return [
        ...
        },
        {
            ...
            change: "set by dev orchestrator for Rollup watch; unset for normal dev server",
        },
        {
        ...
        },
        {
            ...
            effective: process.env[BRANDED_ENV.settingsDebug]?.trim() ||
        ...
        {
            setting: "Mode",
            effective: mode === "dev-server" ? "vite dev (HMR)" : "vite build --watch",
            source: "derived",
            change: mode === "dev-server"
...
function appDevSettingsBannerPlugin(): Plugin {
    ...
    return {
        ...
        configureServer() {
            return () => {
                console.log(colorizeDevSettingsStartupBanner(prependDevSubsystemFigletHeading("vite", formatDevSettingsTable("Vite — effective settings (dev server)", buildViteDevSettingsRows("dev-server")))));
            };
        },
        buildStart() {
            if (process.env[BRANDED_ENV.desktopFastDist] === "1" && !printedWatch) {
                printedWatch = true;
                console.log(colorizeDevSettingsStartupBanner(prependDevSubsystemFigletHeading("vite", formatDevSettingsTable("Vite — effective settings (build --watch)", buildViteDevSettingsRows("build-watch")))));
            }
        },
    };
}
    ...
    return {
        ...
        configureServer(server) {
            server.middlewares.use((req, res, next) => {
                ...
                res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Eliza-Token, X-Api-Key, X-Eliza-Export-Token, X-Eliza-Client-Id, X-Eliza-Terminal-Token, X-Eliza-UI-Language, X-Eliza-Platform");
...
function asyncLocalStoragePatchPlugin(): Plugin {
    return {
        ...
        renderChunk(code) {
            ...
            const re = /var\s*\{\s*AsyncLocalStorage\s*:\s*(\w+)\s*\}\s*=\s*\(\s*\(\s*\)\s*=>\s*\(\s*\{\s*\}\s*\)\s*\)/g;
            if (!re.test(code))
                return null;
            re.lastIndex = 0;
...
function isIgnoredWorkspaceGeneratedOutput(normalizedFile: string): boolean {
    return (normalizedFile.includes("/packages/app/.vite/") ||
    ...
    return {
        ...
        configureServer(server) {
            ...
            server.watcher.on("change", (file) => {
                ...
                if (isIgnoredWorkspaceGeneratedOutput(normalizedFile))
                    return;
                const stat = fs.statSync(file, { throwIfNoEntry: false });
                if (!stat)
                    return;
                if (stat.mtimeMs < watcherStartedAt - 1000)
                    return;
                if (seenMtimes.get(normalizedFile) === stat.mtimeMs)
                    return;
    ...
    return {
        ...
        async transform(code, id) {
            ...
            if (!cleanId.endsWith(".js"))
                return null;
            if (!normalizedId.startsWith(`${normalizedAppCoreSrcRoot}/`))
                return null;
...
const DEV_CJS_INTEROP_SHIM_ALIASES: Array<[
    RegExp,
    string
]> = [
...
function makeEsToolkitCompatEsmPlugin(pluginName: string, isRaw = false): Plugin {
    ...
    return {
        ...
        resolveId(source, importer) {
            ...
            try {
                const req = createRequire(importer || path.join(elizaRoot, "node_modules", "x.js"));
        ...
        load(id) {
            ...
            return (`export { ${name} as default } from ${spec};\n` +
                `export * from ${spec};\n`);
        },
    };
}
...
export default defineConfig(({ command, mode }) => ({
    ...
    define: {
        global: "globalThis",
        ...resolveLocalRealtimeVoiceDefinesFromEnv(command, mode, localVoiceGatewayPort, here),
        ...
        __ELIZA_BUILD_VARIANT__: JSON.stringify(process.env.ELIZA_BUILD_VARIANT === "store" ? "store" : "direct"),
        ...
        __ELIZA_WEB_SHELL__: JSON.stringify(!IS_CAPACITOR_MOBILE_BUILD && process.env.ELIZA_DISABLE_WEB_SHELL !== "1"),
        ...
        __ELIZA_CHAT_UI_HARNESS__: JSON.stringify(process.env.ELIZA_CHAT_UI_HARNESS === "1"),
        // Mirror the branded TTS debug env into the client bundle so one env
        // enables UI + server TTS logs in dev.
        [`import.meta.env.${BRANDED_ENV.ttsDebug}`]: JSON.stringify(process.env[BRANDED_ENV.ttsDebug] ?? ""),
        [`import.meta.env.${BRANDED_ENV.settingsDebug}`]: JSON.stringify(process.env[BRANDED_ENV.settingsDebug] ?? ""),
        [`import.meta.env.${BRANDED_ENV.viteSettingsDebug}`]: JSON.stringify(process.env[BRANDED_ENV.viteSettingsDebug] ?? ""),
            ...
            ? [
                {
                    ...
                    load(id: string) {
                        ...
                        if (/\/packages\/ui\/src\/cloud\/settings\/index\.tsx?$/.test(p)) {
                        ...
                        const broken = /\/packages\/ui\/src\/cloud\/(account-security|admin|billing|instances|organization)\//.test(p) || /\/packages\/ui\/src\/cloud\/index\.tsx?$/.test(p);
        ...
        makeEsToolkitCompatEsmPlugin("raw", true),
        {
            ...
            transform(code: string, id: string) {
                ...
                if (map.size === 0)
                    return null;
                let changed = false;
                let out = code.replace(/import\s*\{([^}]*)\}\s*from\s*['"]lucide-react['"];?/g, (full: string, inner: string) => {
                    const specs = inner
                        .split(",")
                        .map((s) => s.trim())
                        .filter(Boolean);
                    ...
                    for (const spec of specs) {
                        ...
                        if (!file)
                            return full;
                        valueLines.push(`import ${local} from "lucide-react/dist/esm/icons/${file}.mjs";`);
                    }
                    changed = true;
                    if (typeSpecs.length > 0) {
                        valueLines.push(`import type { ${typeSpecs.join(", ")} } from "lucide-react";`);
                    }
    ...
    resolve: {
        ...
        alias: [
            ...
            },
            {
                ...
                replacement: path.resolve(here, "src/shims/use-sync-external-store-with-selector.ts"),
            },
            ...
            {
                find: /^lucide-react$/,
                replacement: path.resolve(elizaRoot, "packages/ui/node_modules/lucide-react"),
            },
            ...
            ...(process.env.ELIZA_DISABLE_WEB_SHELL === "1"
                ? [
                    ...
                    },
                    {
                        ...
                        replacement: path.join(here, "src/shims/cloud-register-all-stub.ts"),
                    },
                    {
            ...
            // browser bundle (those are pulled in by src/index.ts / src/plugin.ts).
            {
                ...
                replacement: path.resolve(elizaRoot, "plugins/plugin-personal-assistant/src/ui.ts"),
            },
            {
            ...
            // `/ui` subpath needs an explicit alias to its source entry.
            {
                ...
                replacement: path.resolve(elizaRoot, "plugins/plugin-health/src/ui/index.ts"),
            },
            ...
            // production browser build resolves it without a built plugin-health dist.
            {
                ...
                replacement: path.resolve(elizaRoot, "plugins/plugin-health/src/screen-time/mobile-signal-setup.ts"),
            },
            ...
            ...(() => {
                const cloudSdkSrcDir = path.resolve(elizaRoot, "packages/cloud/sdk/src");
            ...
            ...(() => {
                const appCorePkgPath = path.resolve(elizaRoot, "packages/app/package.json");
                const appCorePkgDir = path.dirname(appCorePkgPath);
                const appCoreBrowserEntry = path.resolve(appCorePkgDir, "src/browser.ts");
                ...
                for (const [key, value] of Object.entries(appCorePkg.exports || {})) {
                    ...
                    if (!key.startsWith("./"))
                        continue;
                    const sourceTarget = resolveLocalPackageSourceExportTarget(appCorePkgDir, exportTarget);
                    if (!sourceTarget)
                        continue;
                    generatedAliases.push({
                        find: new RegExp(`^${escapeRegExp(`@elizaos/app/${key.slice(2)}`)}$`),
                ...
                return [
                    ...
                    },
                    {
                        ...
                        replacement: path.join(appCoreSrcRoot, "first-run/first-run-config.ts"),
                    },
                    {
                        find: /^@elizaos\/app\/api\/ios-local-agent-transport$/,
                        replacement: path.join(appCoreSrcRoot, "api/ios-local-agent-transport.ts"),
                    },
            ...
            plugins: [
                {
                    ...
                    async transform(code, id) {
                        ...
                        if (!id.endsWith(".js") ||
                            !normalizedPath?.startsWith(`${appCoreSrcRoot.split(path.sep).join("/")}/`)) {
                            return null;
                        }
                        return transformWithOxc(code, id, {
                            lang: "jsx",
                ...
                },
                {
                    ...
                    load(id) {
                        if (!id.startsWith("\0node-stub:"))
                            return null;
                        return generateNodeBuiltinStub(id.slice("\0node-stub:".length), _require);
                    },
                },
...
`;
            ...
            external: (id) => {
                if ([
                    ...
                    return true;
                if (/^@node-llama-cpp\//.test(id))
                    return true;
                if (/^@napi-rs\/keyring/.test(id))
                    return true;
                return false;
            },
            input: {
