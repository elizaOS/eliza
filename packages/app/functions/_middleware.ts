...
import { LANDING_AB_HOSTNAMES } from "@elizaos/plugin-elizacloud/cloud-config/domain-contract";
...
import { proxyToApiWorker } from "./_proxy";
import { type PagesProxyEnv } from "./_proxy";
...
export function resolveCanonicalPageRedirect(requestUrl: string): string | null {
    ...
    if (LANDING_AB_HOSTNAMES.includes(url.hostname.toLowerCase()))
        return null;
    ...
    const canonicalDashboardPath = canonicalCloudPathForLegacyDashboard(url.pathname, url.search);
...
const isSpaFallback = (response: Response): boolean => (response.headers.get("Content-Type") ?? "")
    .toLowerCase()
    .includes("text/html");
const serveAsset = async (context: MiddlewareContext): Promise<Response> => {
    ...
    const response = await context.next(new Request(context.request, { headers }));
...
const serveCrawlAsset = async (context: MiddlewareContext, contentType: string): Promise<Response> => {
...
const isEmbedPlatform = (value: string | null): value is EmbedPlatform => value === "telegram" || value === "discord";
...
export const embedFrameAncestors = (platform: string | null): string => isEmbedPlatform(platform)
 ...
 */
export const swapCspFrameAncestors = (csp: string, frameAncestorsDirective: string): string => {
    const directives = csp
        .split(";")
        .map((directive) => directive.trim())
        .filter((directive) => directive.length > 0);
    const index = directives.findIndex((directive) => directive.split(/\s+/, 1)[0]?.toLowerCase() === "frame-ancestors");
    if (index >= 0) {
...
const isEmbedPath = (pathname: string): boolean => pathname === "/embed" || pathname.startsWith("/embed/");
export const onRequest = async (context: MiddlewareContext): Promise<Response> => {
    ...
    headers.set("Content-Security-Policy", inheritedCsp
