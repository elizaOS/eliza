...
Host APIs, runtime startup, and platform tooling live in this package; see [host documentation](docs/host.md). `build:dist` emits the Node host to `dist/`; `build:renderer` emits the renderer to `web-dist/`. `build` runs both. `test` and `typecheck` run both host and renderer lanes.
