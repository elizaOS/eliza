...
This package exports the application host and has no CLI binary. It is the
composition root: it wires the `@elizaos/*` plugins and UI packages into a bootable
app and hands off rendering to `@elizaos/ui`. The packaged `web-dist/` output is consumed
by the Electrobun desktop shell and by Capacitor for mobile. The same package owns host APIs, CLI startup, and native packaging. Node consumers import the host exports from `@elizaos/app`; browser startup uses `src/renderer-entry.ts`.
...
public/ web-dist/    Static assets / Vite build output
```

Root URLs boot the app. Public authentication routes use the lightweight Cloud shell.
...
bun run build          # Host and renderer builds
...
bun run typecheck      # Host and renderer typechecks
bun run test           # Renderer and host test lanes
...
to start both the host API and renderer together.
...
## Application host and build outputs

Host APIs, runtime startup, and platform tooling live in this package; see the [application host guide](AGENTS.md#application-host). `build:dist` emits the Node host to `dist/`; `build:renderer` emits the renderer to `web-dist/`. `build` runs both. `test` and `typecheck` run both host and renderer lanes.
