...
import { createTestPgliteDataDir } from "@elizaos/plugin-sql/database-utils/pglite-storage";
import { fileURLToPath } from "node:url";
import { installHttpPluginLifecycle } from "@elizaos/core/api/http-plugin-runtime";
import { isInMemoryPgliteDataDir } from "@elizaos/plugin-sql/database-utils/pglite-storage";
import { logger } from "@elizaos/core";
import { pathToFileURL } from "node:url";
import { type LiveProviderConfig } from "./live-provider";
import { type LiveProviderName } from "./live-provider";
...
function importOptionalPlugin(specifier: string): Promise<Record<string, unknown>> {
...
function getPendingTrajectoryWrites(service: unknown): Promise<void>[] {
    ...
    return Array.from(writeQueues.values()).filter((pending): pending is Promise<void> => pending instanceof Promise);
}
function extractPlugin(moduleExports: unknown, exportNames: readonly string[]): Plugin | null {
...
async function importPluginSql(): Promise<Plugin> {
    try {
        ...
        const fallbackPath = path.resolve(helperDir, "../../../../plugins/plugin-sql/src/index.ts");
        try {
            const { default: pluginSql } = await import(pathToFileURL(fallbackPath).href);
            return pluginSql as Plugin;
        }
        catch (fallbackError) {
            const packageMessage = packageError instanceof Error
                ? packageError.message
                : String(packageError);
            const fallbackMessage = fallbackError instanceof Error
                ? fallbackError.message
                : String(fallbackError);
            throw new Error(`Failed to import @elizaos/plugin-sql. Package import: ${packageMessage}; fallback ${fallbackPath}: ${fallbackMessage}`);
        }
    }
}
...
function applyRuntimeSettings(runtime: AgentRuntime, settings: Record<string, string>): void {
    for (const [key, value] of Object.entries(settings)) {
        runtime.setSetting(key, value, /(API_KEY|TOKEN|SECRET|PASSWORD)/i.test(key));
    }
}
async function flushPendingTrajectoryWrites(runtime: AgentRuntime): Promise<void> {
    try {
        const { flushTrajectoryWrites } = await import("../../../agent/src/runtime/trajectory-storage");
...
function createCerebrasProviderConfigFromEnv(): LiveProviderConfig | null {
    ...
    const smallModel = process.env.ELIZA_LIVE_TEST_SMALL_MODEL?.trim() ||
        process.env.OPENAI_SMALL_MODEL?.trim() ||
        DEFAULT_CEREBRAS_TEXT_MODEL;
    const largeModel = process.env.ELIZA_LIVE_TEST_LARGE_MODEL?.trim() ||
        process.env.OPENAI_LARGE_MODEL?.trim() ||
        DEFAULT_CEREBRAS_TEXT_MODEL;
    const mediumModel = process.env.OPENAI_MEDIUM_MODEL?.trim() ||
        process.env.MEDIUM_MODEL?.trim() ||
        largeModel;
    const actionPlannerModel = process.env.OPENAI_ACTION_PLANNER_MODEL?.trim() ||
...
export async function createRealTestRuntime(options?: RealTestRuntimeOptions): Promise<RealTestRuntimeResult> {
    const pgliteDir = options?.pgliteDir ?? createTestPgliteDataDir("eliza-real-test-");
    const removePgliteDirOnCleanup = options?.removePgliteDirOnCleanup ??
    ...
    const prevWebsiteBlockerHostsPath = process.env.WEBSITE_BLOCKER_HOSTS_FILE_PATH;
    const prevSelfControlHostsPath = process.env.SELFCONTROL_HOSTS_FILE_PATH;
    process.env.PGLITE_DATA_DIR = pgliteDir;
    if (!hasConfiguredHostsPath(process.env.WEBSITE_BLOCKER_HOSTS_FILE_PATH) &&
        !hasConfiguredHostsPath(process.env.SELFCONTROL_HOSTS_FILE_PATH)) {
        selfControlTempDir = fs.mkdtempSync(path.join(os.tmpdir(), "eliza-real-selfcontrol-"));
    ...
    try {
            ...
            if (providerConfig) {
                ...
                try {
                    ...
                    if (plugin) {
                        ...
                        logger.info(`[real-runtime] Registered LLM plugin: ${providerConfig.pluginPackage} (${providerName})`);
                    }
                    else {
                        logger.warn(`[real-runtime] Loaded ${providerConfig.pluginPackage} but could not find a plugin export`);
                    }
                }
                catch (err) {
                    logger.warn(`[real-runtime] Failed to register LLM plugin ${providerConfig.pluginPackage}: ${err}`);
                    providerName = null;
                    providerConfig = null;
                }
            }
        ...
        if (options?.withLLM &&
            ...
            try {
                const { default: localEmbeddingPlugin } = await importOptionalPlugin("@elizaos/plugin-local-inference");
                ...
                logger.info("[real-runtime] Registered local embedding plugin for TEXT_EMBEDDING");
            }
            catch (err) {
                logger.warn(`[real-runtime] Failed to register local embedding plugin: ${err}`);
            }
        }
        // Register Discord plugin if requested and token available
        if (options?.withDiscord && process.env.DISCORD_BOT_TOKEN?.trim()) {
            try {
                const { default: discordPlugin } = await importOptionalPlugin("@elizaos/plugin-discord");
        ...
        if (options?.withTelegram && process.env.TELEGRAM_BOT_TOKEN?.trim()) {
            try {
                const { default: telegramPlugin } = await importOptionalPlugin("@elizaos/plugin-telegram");
        ...
        for (const plugin of options?.plugins ?? []) {
            for (const service of plugin.services ?? []) {
                const serviceType = (service as unknown as {
                    serviceType?: string;
                })
                    .serviceType;
                if (!serviceType)
                ...
                try {
                    ...
                    logger.debug(`[real-runtime] declared service '${serviceType}' from ${plugin.name} did not start during boot barrier: ${err}`);
                }
            }
        }
        ...
        try {
            const { OptimizedPromptService } = await import("@elizaos/plugin-assistant");
            ...
            logger.warn(`[real-runtime] OptimizedPromptService eager start failed: ${err}`);
        }
