...
const RESET = "\x1b[0m";
export type LiveProviderId = "openai" | "anthropic" | "groq" | "openrouter" | "xai" | "elizacloud" | "cerebras";
...
const DEFAULT_SYSTEM_PROMPT = "Concise, helpful assistant for end-to-end testing. " +
 ...
 */
async function importWorkspacePlugin(relativeFromHere: string, bareSpecifier: string): Promise<Record<string, unknown> | null> {
    try {
        const mod = (await import(new URL(relativeFromHere, import.meta.url).href)) as Record<string, unknown>;
        return mod;
    }
    catch {
        try {
...
async function resolveProviderPlugin(provider: LiveProviderId): Promise<Plugin | null> {
...
async function loadExtraPlugin(entry: string | {
    path: string;
    name?: string;
}): Promise<Plugin | null> {
...
function applyProviderSettings(runtime: AgentRuntime, provider: LiveProviderId): void {
    switch (provider) {
        case "openai":
            ...
            if (process.env.OPENAI_ACTION_PLANNER_MODEL) {
                runtime.setSetting("OPENAI_ACTION_PLANNER_MODEL", process.env.OPENAI_ACTION_PLANNER_MODEL);
            }
            break;
        case "anthropic":
            ...
            break;
        case "google":
            runtime.setSetting("GOOGLE_GENERATIVE_AI_API_KEY", process.env.GOOGLE_API_KEY ??
        ...
        case "groq":
            ...
            runtime.setSetting("GROQ_SMALL_MODEL", process.env.GROQ_SMALL_MODEL ?? "openai/gpt-oss-120b");
            runtime.setSetting("GROQ_LARGE_MODEL", process.env.GROQ_LARGE_MODEL ?? "openai/gpt-oss-120b");
            break;
        case "openrouter":
            runtime.setSetting("OPENROUTER_API_KEY", process.env.OPENROUTER_API_KEY ?? "", true);
            break;
        case "ollama": {
            const endpoint = process.env.OLLAMA_API_ENDPOINT?.trim() ||
                process.env.OLLAMA_API_URL?.trim() ||
                "";
            runtime.setSetting("OLLAMA_API_ENDPOINT", endpoint, true);
            if (process.env.OLLAMA_SMALL_MODEL) {
                runtime.setSetting("OLLAMA_SMALL_MODEL", process.env.OLLAMA_SMALL_MODEL);
            }
            if (process.env.OLLAMA_LARGE_MODEL) {
                runtime.setSetting("OLLAMA_LARGE_MODEL", process.env.OLLAMA_LARGE_MODEL);
            }
            if (process.env.OLLAMA_EMBEDDING_MODEL) {
                runtime.setSetting("OLLAMA_EMBEDDING_MODEL", process.env.OLLAMA_EMBEDDING_MODEL);
            }
            break;
        }
        case "xai":
            ...
            break;
        case "elizacloud":
            runtime.setSetting("ELIZAOS_CLOUD_API_KEY", process.env.ELIZAOS_CLOUD_API_KEY ?? "", true);
            if (process.env.ELIZAOS_CLOUD_BASE_URL) {
                runtime.setSetting("ELIZAOS_CLOUD_BASE_URL", process.env.ELIZAOS_CLOUD_BASE_URL);
            }
            if (process.env.ELIZAOS_CLOUD_LARGE_MODEL) {
                runtime.setSetting("ELIZAOS_CLOUD_LARGE_MODEL", process.env.ELIZAOS_CLOUD_LARGE_MODEL);
            }
            if (process.env.ELIZAOS_CLOUD_SMALL_MODEL) {
                runtime.setSetting("ELIZAOS_CLOUD_SMALL_MODEL", process.env.ELIZAOS_CLOUD_SMALL_MODEL);
            }
            break;
        case "cerebras": {
            ...
            runtime.setSetting("OPENAI_BASE_URL", process.env.OPENAI_BASE_URL || "https://api.cerebras.ai/v1");
            runtime.setSetting("OPENAI_LARGE_MODEL", process.env.OPENAI_LARGE_MODEL || DEFAULT_CEREBRAS_TEXT_MODEL);
            runtime.setSetting("OPENAI_MEDIUM_MODEL", process.env.OPENAI_MEDIUM_MODEL ||
                process.env.OPENAI_LARGE_MODEL ||
                DEFAULT_CEREBRAS_TEXT_MODEL);
            runtime.setSetting("OPENAI_SMALL_MODEL", process.env.OPENAI_SMALL_MODEL || DEFAULT_CEREBRAS_TEXT_MODEL);
            runtime.setSetting("OPENAI_ACTION_PLANNER_MODEL", process.env.OPENAI_ACTION_PLANNER_MODEL ||
                process.env.OPENAI_LARGE_MODEL ||
                DEFAULT_CEREBRAS_TEXT_MODEL);
            runtime.setSetting("OPENAI_PLANNER_MODEL", process.env.OPENAI_PLANNER_MODEL ||
...
function effectiveRequiredEnv(opts: LiveAgentTestOptions): {
    ...
    const hasCerebrasFallback = provider === "openai" &&
...
export async function buildLiveHarness(opts: LiveAgentTestOptions): Promise<LiveAgentHarness> {
    const provider = opts.provider ?? "openai";
    const restoreEnv = maybeApplyCerebrasAlias(provider);
    const providerPlugin = await resolveProviderPlugin(provider);
    if (!providerPlugin) {
        restoreEnv();
        throw new Error(`[live-agent-test] failed to resolve provider plugin for ${provider}`);
    }
    const plugins: Plugin[] = [providerPlugin];
    for (const entry of opts.extraPlugins ?? []) {
        const extra = await loadExtraPlugin(entry);
        if (!extra) {
            throw new Error(`[live-agent-test] failed to load extra plugin: ${typeof entry === "string" ? entry : entry.path}`);
        }
        plugins.push(extra);
    }
...
function emitSkip(name: string, reason: string): void {
    ...
    console.warn(`${YELLOW}[live-agent-test] ${name} skipped — ${reason}${RESET}`);
    describe(name, () => {
 ...
 */
export async function describeLive(name: string, opts: LiveAgentTestOptions, body: (ctx: {
