...
function isVaultRef(value: string): boolean {
    return (value.startsWith(VAULT_REF_PREFIX) && value.length > VAULT_REF_PREFIX.length);
}
...
async function resolveMaybeVaultRef(value: string, opts: {
    stateDir?: string;
} = {}): Promise<string | null> {
    if (!isVaultRef(value))
...
function providerKeyMatchesSelection(providerName: LiveProviderName, apiKey: string): boolean {
...
function getLiveTestModelOverride(kind: "small" | "large"): string | null {
    return getTrimmedEnv(kind === "small"
        ? "ELIZA_LIVE_TEST_SMALL_MODEL"
        : "ELIZA_LIVE_TEST_LARGE_MODEL");
}
function getLiveTestBaseUrlOverride(providerName: LiveProviderName): string | null {
...
export type LiveProviderName = "cerebras" | "groq" | "openai" | "anthropic" | "openrouter" | "local-llama-cpp";
...
export function getFirstRunProviderForLiveProvider(provider: Pick<LiveProviderConfig, "name">): string {
...
function buildLiveProviderConfig(def: (typeof PROVIDERS)[number], apiKey: string): LiveProviderConfig {
 ...
 */
export function selectLiveProvider(preferredProvider?: LiveProviderName): LiveProviderConfig | null {
    ...
    for (const def of candidates) {
        ...
        if (def.name === "cerebras" && !preferredProvider) {
            ...
            const baseUrlIsCerebras = !!explicitBaseUrl && /cerebras\.ai(?:\/|$)/i.test(explicitBaseUrl);
 ...
 */
export async function selectLiveProviderAsync(preferredProvider?: LiveProviderName): Promise<LiveProviderConfig | null> {
    const candidates = preferredProvider
        ? PROVIDERS.filter((p) => p.name === preferredProvider)
        : PROVIDERS;
    for (const def of candidates) {
 ...
 */
export function requireLiveProvider(preferredProvider?: LiveProviderName): LiveProviderConfig | null {
...
export function availableProviderNames(): LiveProviderName[] {
    const providers = new Set<LiveProviderName>(PROVIDERS.filter((def) => providerKeyEnvCandidates(def).some((key) => {
        const value = getTrimmedEnv(key);
        return value ? providerKeyMatchesSelection(def.name, value) : false;
    })).map((def) => def.name));
    return [...providers];
}
export function buildIsolatedLiveProviderEnv(baseEnv: NodeJS.ProcessEnv, provider: Pick<LiveProviderConfig, "env"> | null | undefined): NodeJS.ProcessEnv {
