/** Exercises live provider behavior with deterministic app test fixtures. */
...
describe("selectLiveProvider", () => {
  ...
  afterEach(() => {
    vi.resetModules();
    vi.doUnmock("@elizaos/auth/vault");
    vi.unstubAllEnvs();
  });
  ...
  it("loads a vault-referenced Cerebras key from local eliza.json in the async selector", async () => {
    ...
    vi.doMock("@elizaos/auth/vault", () => ({ createVault }));
