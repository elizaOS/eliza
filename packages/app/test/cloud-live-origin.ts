...
import { type ElizaCloudEnvironment } from "@elizaos/plugin-elizacloud/cloud-config/domain-contract";
...
export function resolveCloudLiveOriginContract(env: NodeJS.ProcessEnv = process.env): CloudLiveOriginContract {
    const expectedRaw = env.ELIZA_UI_SMOKE_CLOUD_EXPECTED_ENV?.trim().toLowerCase();
    const expected: ElizaCloudEnvironment | null = expectedRaw === "staging" || expectedRaw === "production"
        ? expectedRaw
        : null;
    ...
    if (expected === "staging" && !env.ELIZAOS_CLOUD_BASE_URL?.trim()) {
        return {
            ...
            expected,
            ok: false,
            reason: "staging lane requires an explicit ELIZAOS_CLOUD_BASE_URL; refusing the production default",
        };
    }
