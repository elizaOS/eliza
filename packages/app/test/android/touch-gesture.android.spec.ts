...
async function androidTouchDrag(
  ...
  writeStage("android-touch-drag", { selector, box, startX, startY, endX, endY });
...
async function ensureCollapsedHome(page: Page, adb: string, serial: string) {
  ...
  if ((await overlay.getAttribute("data-open")) === "true") {
    await page
      ...
      .dispatchEvent("keydown", { key: "Escape", bubbles: true, cancelable: true });
    await expect
...
let recording: Awaited<ReturnType<typeof startChunkedAndroidScreenRecord>> | null =
  null;
const matrixLog: Record<string, unknown>[] = [];

test.describe.serial("android chat gesture matrix (real WebView)", () => {
  ...
  test("sheet grabber drag opens and collapses the chat sheet", async ({
    ...
    matrixLog.push({
      leg: "sheet-detents",
      openedDetent,
      openTouchEvents: openTouch.touchEventCount + openTouch.pointerTouchCount,
      closeTouchEvents:
  ...
  test("horizontal rail swipe pages home↔launcher and back", async ({
    ...
    for (let attempt = 1; attempt <= 3 && !delivered; attempt++) {
      ...
      delivered = await page
        ...
        .then(() => true)
        .catch(() => false);
      if (!delivered) await waitForResponsiveMainThread(page, { timeoutMs: 30_000 });
    }
    ...
    matrixLog.push({
      ...
      backTouchEvents: backTouch.touchEventCount + backTouch.pointerTouchCount,
    });
  ...
  test("push-to-talk hold arms dictation without toggling hands-free", async ({
    ...
    await installTouchRecorder(page);
    const point = await selectorDevicePoint(page, '[data-testid="chat-composer-mic"]');
    // Press-and-hold ~700ms (> the 200ms PTT arm threshold), polling the label
    // mid-hold for the "release to insert" dictation state.
    const press = androidTouchPressBackground(adb, serial, point.startX, point.startY, 700);
  ...
  test("focusing the composer lifts it clear of the software keyboard", async ({
    ...
    const point = await selectorDevicePoint(page, '[data-testid="chat-composer-textarea"]');
    adbDevice(adb, serial, ["shell", "input", "tap", String(point.startX), String(point.startY)]);
    ...
    expect(layout.composerBottom, "composer rect is measurable").not.toBeNull();
    expect(
  ...
  test("attaching an image shows a pending preview via the real intake path", async ({
    ...
    await installTouchRecorder(page);
    const point = await selectorDevicePoint(page, '[data-testid="chat-composer-attach"]');
    adbDevice(adb, serial, ["shell", "input", "tap", String(point.startX), String(point.startY)]);
  ...
  test("long press on a sent message reveals the copy affordance", async ({
    ...
    await androidTouchPressBackground(adb, serial, point.startX, point.startY, 800);
    await page.waitForTimeout(400);
