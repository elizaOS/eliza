 ...
 * `AgentRuntime` on a real PGLite database behind the real app HTTP API
...
describe("POST /api/agent/reset (real HTTP handler)", () => {
  ...
  it("resets the agent: 200 {ok:true}, state nulled, first-run cleared, data dir gone, cloud secrets cleared", async () => {
    ...
    const agentsBefore = await req(port, "GET", "/api/agents");
    expect(agentsBefore.data.agents[0].status).toBe("running");
    ...
    const agentsAfter = await req(port, "GET", "/api/agents");
    expect(agentsAfter.data.agents[0].status).toBe("stopped");
