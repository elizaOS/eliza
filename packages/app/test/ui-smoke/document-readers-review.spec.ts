/** Browser acceptance for document-reader review over the real app with controlled HTTP authority responses; no real document is shared. */
...
for (const width of [1280, 390]) {
  test(`document readers require a fresh review after conflict at ${width}px`, async ({
    page,
  }, testInfo) => {
    ...
    await page.route("**/api/documents**", async (route) => {
      ...
      if (path !== `/api/documents/${document.id}/access`)
        return route.fallback();
      if (route.request().method() === "GET")
        return route.fulfill({
          json: {
            documentId: document.id,
            directGrantEntityIds: readers,
      ...
      expect(payload.directGrantEntityIds).toEqual([readerId]);
      expect(payload.expectedAccessRevision).toBe(revision);
      if (writes === 1) {
        revision = `dar1_${"b".repeat(64)}`;
        return route.fulfill({
          status: 409,
          json: { error: "Access changed" },
        });
      }
      readers = payload.directGrantEntityIds;
    ...
    await page.route("**/api/relationships/people**", (route) =>
      route.fulfill({
        json: {
          data: [
            {
              primaryEntityId: readerId,
              groupId: "not-an-entity",
              memberEntityIds: [readerId, "not-selected"],
              displayName: "Casey — synthetic co-parent",
            },
          ],
          stats: {},
        },
    ...
    await page
      ...
      .first()
      .click();
    await page.getByRole("button", { name: "Manage document readers" }).click();
    const sharing = page.getByRole("region", { name: "Document sharing" });
    const reader = sharing.getByRole("checkbox", { name: /Casey/ });
    ...
    const save = sharing.getByRole("button", { name: "Save reviewed readers" });
    ...
    await save.click();
    await expect(sharing.getByRole("alert")).toContainText("Access changed");
    await expect(save).toHaveCount(0);
    await page.screenshot({
      path: testInfo.outputPath(`conflict-${width}.png`),
      fullPage: true,
    });
    await sharing.getByRole("button", { name: "Reload readers" }).click();
    await reader.check();
    await sharing
      .getByRole("button", { name: "Review reader changes" })
      .click();
    ...
    await save.click();
    await expect(sharing.getByRole("status")).toContainText("Sharing saved");
