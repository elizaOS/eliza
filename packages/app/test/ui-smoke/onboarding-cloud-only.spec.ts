...
test.describe("cloud-only onboarding (production default)", () => {
  ...
  test("an existing Dedicated row shows its exact quote and waits for the in-chat confirmation before POST", async ({
    page,
  }) => {
    await injectCloudAuthToken(page);
    ...
    await expect(page.getByText("Balance: $115.54059")).toBeVisible();
