/**
 * Exercises Account sign-in through the production renderer and auth store,
...
for (const viewport of [
  ...
  test(`Account starts browser sign-in with a connected agent on ${viewport.name}`, async ({
    page,
    baseURL,
  }, testInfo) => {
    const logs: string[] = [];
    ...
    await page.route("**/api/cloud/login", async (route) => {
      if (rejectLogin) {
        await route.fulfill({
          status: 503,
          json: { ok: false, error: "Sign-in service temporarily unavailable" },
        });
        return;
      }
      await route.fulfill({
    ...
    await page.evaluate(() => {
      window.location.hash = "cloud-account";
    });
    const signIn = page.getByRole("button", { name: /^sign in$/i });
    await expect(signIn).toBeVisible({ timeout: 60_000 });
    await page.screenshot({
      path: testInfo.outputPath(`${viewport.name}-account-rest.jpg`),
      fullPage: true,
    });
    await signIn.hover();
    await page.screenshot({
      path: testInfo.outputPath(`${viewport.name}-account-hover.jpg`),
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: testInfo.outputPath(`${viewport.name}-account-signin-error.jpg`),
      fullPage: true,
    });
    rejectLogin = false;
    ...
    await expect(page).toHaveURL(/\/settings#cloud-account$/);
    ...
    if (video) {
      const artifact = await saveBrowserVideoArtifact({
        video,
        testInfo,
        basename: `${viewport.name}-account-signin-retry`,
      });
