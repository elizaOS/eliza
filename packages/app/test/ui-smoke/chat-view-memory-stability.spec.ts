...
import { expectNoPageDiagnostics } from "./helpers";
import { expectNoRenderTelemetryErrors } from "./helpers";
import { installDefaultAppRoutes } from "./helpers";
import { installPageDiagnosticsGuard } from "./helpers";
...
function parseRouteCycleCount(): number {
    ...
    if (!raw)
        return DEFAULT_ROUTE_CYCLES;
    const parsed = Number(raw);
    if (!Number.isInteger(parsed) || parsed <= 0)
        return DEFAULT_ROUTE_CYCLES;
    return Math.min(parsed, MAX_ROUTE_CYCLES);
}
function metricMap(metrics: Array<{
    name: string;
    value: number;
}>): Map<string, number> {
...
function expectBoundedRuntimeGrowth(before: RuntimeMetrics, after: RuntimeMetrics): void {
    const heapGrowth = growth(before.jsHeapUsedSize, after.jsHeapUsedSize);
    const nodeGrowth = growth(before.nodes, after.nodes);
    const listenerGrowth = growth(before.jsEventListeners, after.jsEventListeners);
    const documentGrowth = growth(before.documents, after.documents);
    const summary = JSON.stringify({ before, after }, null, 2);
    expect(heapGrowth, `expected retained JS heap growth to stay bounded after repeated chat/view switching; metrics=${summary}`).toBeLessThanOrEqual(96 * MI_B);
    expect(after.jsHeapUsedSize, `expected final JS heap to stay near the warmed baseline; metrics=${summary}`).toBeLessThanOrEqual(Math.max(before.jsHeapUsedSize * 2.75, before.jsHeapUsedSize + 128 * MI_B));
    expect(nodeGrowth, `expected DOM node growth to stay bounded after route cycles; metrics=${summary}`).toBeLessThanOrEqual(8000);
    expect(after.nodes, `expected final DOM node count to stay near the warmed baseline; metrics=${summary}`).toBeLessThanOrEqual(Math.max(before.nodes * 2.5, before.nodes + 12000));
    expect(listenerGrowth, `expected event-listener growth to stay bounded after route cycles; metrics=${summary}`).toBeLessThanOrEqual(2000);
    expect(documentGrowth, `expected document count not to grow repeatedly across SPA route changes; metrics=${summary}`).toBeLessThanOrEqual(8);
}
async function navigateInPlace(page: Page, targetPath: string): Promise<void> {
    await page.evaluate(({ eventName, path }) => {
        window.dispatchEvent(new CustomEvent(eventName, { detail: { viewPath: path } }));
    }, { eventName: NAVIGATE_VIEW_EVENT, path: targetPath });
    await expect(page).toHaveURL(new RegExp(`${targetPath.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(?:[?#].*)?$`));
}
async function waitForRoute(page: Page, targetPath: string, marker: Locator): Promise<void> {
...
test("chat and routed views keep heap, DOM, and listeners bounded", async ({ browserName, page, }) => {
    const routeCycles = parseRouteCycleCount();
    test.setTimeout(Math.max(180000, 45000 + routeCycles * 8000));
    test.skip(browserName !== "chromium", "CDP runtime metrics are Chromium-only.");
