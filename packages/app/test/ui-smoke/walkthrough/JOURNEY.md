...
| 06 | Open settings | The Settings shell opens and account inventory finishes loading. | `settings-shell` visible; "Models & Providers" opened; `GET /api/accounts` completed; `account-management-panel` reached `data-state=ready` or `error` on desktop and mobile. | `06-settings-open.png` |
| 07 | Wallet view | The Wallet view renders. | `wallet-shell` (or Wallet heading) and the `wallets-sidebar` balances/chains surface visible at `/wallet`. | `07-wallet.png` |
