...
import { seedFirstRunCompleteBeforeLoad } from "../helpers";
...
const WALKTHROUGH_VOICE_MODEL_INSTALLATIONS = Array.from(new Set(VOICE_MODEL_VERSIONS.map((version) => version.id)))
    .sort()
    .map((id) => ({
    id,
...
const CHAT_COMPOSER_SELECTOR = '[data-testid="chat-composer-textarea"], textarea[aria-label="message"]';
const CHAT_SEND_SELECTOR = '[data-testid="chat-composer-action"], button[aria-label="Send"], button[aria-label="Send message"]';
...
export class WalkthroughRecorder {
    ...
    constructor(readonly page: Page, readonly lane: Lane, readonly viewport: ViewportProfile, readonly runDir: string, private readonly nowIso: () => string) { }
    attach(): void {
        ...
        this.page.on("requestfailed", (request) => {
            ...
            if (failure.includes("net::ERR_ABORTED") || isIgnorableUrl(url))
                return;
    ...
    private gateConsoleErrors(): ConsoleRecord[] {
        ...
        return this.console.filter((c) => (c.type === "console.error" || c.type === "pageerror") &&
            !allow.some((a) => c.text.includes(a)) &&
            !OPTIONAL_RESOURCE_ALLOWLIST.some((url) => c.location.includes(url)));
    }
    async captureStep(step: JourneyStep, result: StepRunResult, testInfo: TestInfo): Promise<void> {
    ...
    gateSummary(): {
        ...
        const consoleErrors = this.gateConsoleErrors().map((c) => `${c.type}${c.atStep ? ` @${c.atStep}` : ""}: ${c.text}`);
        const serverErrors = this.serverErrors.map((r) => `${r.status}${r.atStep ? ` @${r.atStep}` : ""} ${r.method} ${r.url}`);
        return {
    ...
    async finalize(): Promise<void> {
        ...
        await writeFile(join(dir, "logs", "console.log"), this.console
            .map((c) => `[${c.atStep ?? "-"}] ${c.type} ${c.text}${c.location ? ` (${c.location})` : ""}`)
            .join("\n") || "(no console.warn/error/pageerror recorded)\n");
        await writeFile(join(dir, "logs", "network.log"), this.network
            .map((r) => `[${r.atStep ?? "-"}] ${r.status ?? "FAIL"} ${r.method} ${r.url}${r.failure ? ` ${r.failure}` : ""}`)
            .join("\n") || "(no network recorded)\n");
        if (this.trajectory) {
            await mkdir(join(dir, "trajectory"), { recursive: true });
            await writeFile(join(dir, "trajectory", "chat-step.json"), JSON.stringify(this.trajectory, null, 2));
        }
    }
}
// ---------------------------------------------------------------------------
// Route installers
// ---------------------------------------------------------------------------
async function fulfillJson(route: Route, status: number, body: Record<string, unknown>): Promise<void> {
    await route.fulfill({
        status,
...
async function installConversationStore(page: Page): Promise<ConversationStore> {
    const assistantText = "Saved — walkthrough reply captured.";
    const conversations: Array<{
        id: string;
        title: string;
    }> = [];
    const messagesById = new Map<string, Array<{
        id: string;
        role: "user" | "assistant";
        text: string;
        timestamp: number;
    ...
    await page.route(/\/api\/conversations\/([^/?#]+)(\/[^?#]*)?(\?.*)?$/, async (route) => {
        ...
        if (suffix.startsWith("/messages/stream")) {
            ...
            await route.fulfill({
                ...
                body: `data: ${JSON.stringify({ type: "token", text: assistantText, fullText: assistantText })}\n\n` +
...
export async function installJourneyRoutes(page: Page, lane: Lane): Promise<JourneyRoutes> {
...
async function installMockLaneWrites(page: Page): Promise<void> {
    ...
    await page.route("**/api/models**", async (route) => {
        if (route.request().method() === "GET") {
            const url = new URL(route.request().url());
            await fulfillJson(route, 200, url.pathname.endsWith("/config")
                ? { config: {}, providers: [] }
                : { models: [] });
            return;
        }
...
async function pullChatSheetToFull(page: Page): Promise<void> {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
        if ((await sheet.getAttribute("data-detent")) === "full")
            return;
        const box = await page.getByTestId("chat-sheet-grabber").boundingBox();
        if (!box)
            throw new Error("no bounding box for chat-sheet-grabber");
...
async function navigateViaAgentEvent(page: Page, detail: Record<string, unknown>): Promise<void> {
...
async function domMarkers(page: Page, markers: Record<string, string>): Promise<Record<string, boolean>> {
...
export const JOURNEY_STEPS: readonly JourneyStep[] = [
    {
        ...
        expectation: "First app load from / with first-run incomplete: the real chat overlay renders first-run choices in the transcript. No render failure, no stack trace.",
        async run({ page, lane }) {
            ...
            await expect(page.getByText("First, where should your agent run?", { exact: false })).toBeVisible({ timeout: FIRST_RUN_TEXT_TIMEOUT_MS[lane] });
    ...
    },
    {
        ...
        expectation: "The chat transcript asks how the agent should run, with Eliza Cloud, on-device, and remote-agent options (Bring your own keys is a provider sub-choice, not a location — #11509).",
        async run({ page, lane }) {
            await expect(page.getByText("First, where should your agent run?", { exact: false })).toBeVisible({ timeout: FIRST_RUN_TEXT_TIMEOUT_MS[lane] });
            ...
            await expect(page.getByTestId("choice-__first_run__:runtime:other")).toHaveCount(0);
            return {
                assertions: [
    ...
    },
    {
        ...
        expectation: "Choosing the local runtime advances first-run to the provider step and resolves to a ready agent: the chat overlay + composer are reachable.",
        async run(ctx) {
            const { page } = ctx;
    ...
    },
    {
        ...
        expectation: "A typed 'start tutorial' command in the chat composer starts the chat-native tour: one conversational turn per step lands in the live transcript, the send-message step auto-advances on a real composer send, and Done completes the run.",
        async run({ page }) {
    ...
    },
    {
        ...
        expectation: "Typing 'restart tutorial' in the composer starts a fresh run (new welcome turn) and 'stop tutorial' ends it with a stopped acknowledgment — neither reaches the agent as a chat message.",
        async run({ page }) {
            const box = composer(page);
    ...
    },
    {
        ...
        id: "settings-open",
        title: "Open settings",
        expectation: "The Settings shell opens, Models & Providers loads /api/accounts, and the account panel reaches a ready or explicit error state.",
        async run({ page }) {
            const accountsResponse = page.waitForResponse((response) => response.request().method() === "GET" &&
                new URL(response.url()).pathname === "/api/accounts", { timeout: 30000 });
            ...
            await expect(accountPanel).toHaveAttribute("data-state", /^(ready|error)$/, { timeout: 30000 });
            return {
                assertions: [
    ...
    },
    {
        ...
        id: "wallet",
        title: "Wallet view",
        expectation: "The Wallet view renders its shell with balances/chains (mock fixtures in the keyless lane; real wallet state in the live lane).",
        async run({ page }) {
    ...
    },
    {
        ...
        expectation: "A real conversation: the typed message renders as a user thread line and an assistant reply renders. In the live lane the reply comes from the real model (no canned text).",
        async run(ctx) {
            const { page, lane } = ctx;
            await reachChatReady(ctx);
            const userText = lane === "live"
            ...
            const trajectory: Record<string, unknown> = {
                ...
                model: process.env.ANTHROPIC_LARGE_MODEL ??
    ...
    },
    {
        ...
        expectation: "An upward flick on the sheet grabber — the pull gesture that replaced the removed maximize button (#13531) — expands the chat overlay to its full-height detent (data-detent=full).",
        async run({ page }) {
            const sheet = page.getByTestId("chat-sheet");
    ...
    },
    {
        ...
        expectation: "A chat-driven navigation switches the route to the character editor view.",
        async run({ page }) {
    ...
    },
    {
        ...
        expectation: "The Personality panel opens and the About Me / personality field accepts an edit. In the live lane the edit persists (PUT /api/character) and reads back after reload.",
        async run({ page, lane }) {
            ...
            if (await bio.isVisible().catch(() => false)) {
                ...
                let putCount = 0;
                page.on("response", (r) => {
                    if (r.url().includes("/api/character") &&
    ...
    },
    {
        ...
        expectation: "A fresh conversation can be started without losing the prior thread; the composer is empty for the new thread.",
        async run(ctx) {
            const { page } = ctx;
            await reachChatReady(ctx);
            ...
            if (await newChat.isVisible().catch(() => false)) {
                ...
                const createdId = await page.evaluate(async () => {
                    ...
                    if (!response.ok) {
                        throw new Error(`POST /api/conversations failed with ${response.status}: ${await response.text()}`);
                    }
                    ...
                    if (typeof body.conversation?.id !== "string") {
                        throw new Error("POST /api/conversations returned no conversation id");
                    }
                    return body.conversation.id;
                });
                await expect
                    .poll(() => page.evaluate(async (expectedId) => {
                    const response = await fetch("/api/conversations");
                    if (!response.ok)
                        return false;
                    const body = (await response.json()) as {
                    ...
                    return Boolean(body.conversations?.some((conversation) => conversation.id === expectedId));
                }, createdId))
                    .toBe(true);
            }
    ...
    },
    {
        ...
        expectation: "The app leaves full chat and shows the home/dashboard surface with the chat collapsed.",
        async run({ page }) {
    ...
    },
    {
        ...
        expectation: "Reopening chat restores the previous thread: the prior user + assistant lines reappear.",
        async run(ctx) {
            const { page } = ctx;
            await openAppPath(page, "/chat");
            const overlay = page.getByTestId("chat-overlay");
    ...
    },
    {
        ...
        desktopOnly: false,
        expectation: "A rendered message exposes selectable transcript text that can be copied.",
        async run({ page }) {
    ...
    },
    {
        ...
        expectation: "Pasting a large block into the composer collapses it into a pasted-text.md attachment chip rather than a huge composer value.",
        async run({ page }) {
            const input = composer(page);
            await expect(input).toBeVisible({ timeout: 15000 });
            await input.click();
            const bigText = "The quick brown fox jumps over the lazy dog. ".repeat(60);
    ...
    },
    {
        ...
        expectation: "Clearing the composer/draft leaves it empty with no pending attachment chip.",
        async run({ page }) {
            const input = composer(page);
            await input.fill("");
    ...
    },
    {
        ...
        expectation: "The overlay collapses to its pill/rest state while the composer remains reachable.",
        async run({ page }) {
            const overlay = page.getByTestId("chat-overlay");
    ...
    },
    {
        ...
        expectation: "The overlay expands from rest back to the open/full state with the thread visible.",
        async run({ page }) {
            const grabber = page.getByTestId("chat-sheet-grabber");
            const overlay = page.getByTestId("chat-overlay");
    ...
    },
    {
        ...
        expectation: "Clicking into the composer focuses it (document.activeElement is the composer) with no layout overlap.",
        async run({ page }) {
            const input = composer(page);
            await input.click();
            const focused = await page.evaluate(() => {
                const active = document.activeElement as HTMLElement | null;
                return (active?.getAttribute("data-testid") === "chat-composer-textarea" ||
    ...
    },
    {
        ...
        expectation: "The launcher surface shows the view grid with at least one launcher tile.",
        async run({ page }) {
            await openAppPath(page, "/views");
            await expect(page.getByTestId("launcher")).toBeVisible({
                timeout: 30000,
            });
            await expect(page.locator('[data-testid^="launcher-tile-"]').first()).toBeVisible({ timeout: 15000 });
    ...
    },
    {
        ...
        id: "launch-view",
        title: "Launch a view",
        expectation: "Clicking a launcher tile launches a real view and the route leaves /views.",
        async run({ page }) {
    ...
    },
    {
        ...
        expectation: "The chat overlay opens over the current view without remounting it; the composer is reachable and the view remains behind.",
        async run({ page }) {
    ...
    },
    {
        ...
        expectation: "A voice selection is saved through PUT /api/config, survives a reload, and is restored. The mock lane uses a stateful config fixture; the live lane uses the backend.",
        async run({ page, lane }) {
            await openAppPath(page, "/settings");
            if (lane === "mock") {
                ...
                if (!seedVoice)
                    throw new Error("No supported voice fixture available");
                let config = {
                    ...initial,
                    messages: {
                        ...initial.messages,
            ...
            if (!before)
                throw new Error("Selected voice is unnamed");
            const alternative = page.getByRole("option", { selected: false }).first();
            const after = (await alternative.textContent())?.trim();
            if (!after)
                throw new Error("Alternative voice is unnamed");
            expect(after).not.toBe(before);
            await alternative.click();
            const save = async () => {
                const [response] = await Promise.all([
                    page.waitForResponse((r) => new URL(r.url()).pathname === "/api/config" &&
                        r.request().method() === "PUT", { timeout: 15000 }),
                    page
            ...
            try {
                ...
                await expect(page
                    ...
                    .filter({ hasText: after })).toBeVisible();
            }
            finally {
                ...
                await expect(page
                    .getByRole("option", { selected: true })
                    .filter({ hasText: before })).toBeVisible();
                await page.keyboard.press("Escape");
            }
            return {
                assertions: [
    ...
    },
    {
        ...
        expectation: "The app returns to the home/dashboard surface with chat at rest and no page diagnostics accumulated through the journey.",
        async run({ page }) {
            await openAppPath(page, "/");
            const home = page
                .getByTestId("widget-host-home")
