...
async function pageSemanticState(page: Page): Promise<string> {
  return page.evaluate((observedAttributes: readonly string[]) => {
    const root = document.querySelector("main") ?? document.body;
    const clone = root.cloneNode(true) as HTMLElement;

    // Clocks advance independently of an interaction and cannot prove that the
    // clicked control worked. Everything else is retained so cross-control
    // effects such as dialer text, route surfaces, and form state are visible.
    for (const node of clone.querySelectorAll("time,script,style")) {
      node.remove();
    }
    ...
    const fields = Array.from(
      ...
      return {
        ...
        selectedIndex: "selectedIndex" in select ? select.selectedIndex : null,
      };
    });
    ...
    return JSON.stringify({ text, fields, controls });
...
async function snapshotControl(
  ...
  return {
    ...
    pageSemanticState: await pageSemanticState(page),
    details,
  };
}
...
function semanticDelta(
  ...
  if (after.pageSemanticState !== before.pageSemanticState) {
    return "page semantic state changed";
  }
  ...
  if (before.details.connected && !after.details.connected) {
    return "clicked control detached or was replaced";
  }
...
function documentedClickNoop(
  ...
  const agentId = details.attributes["data-agent-id"];
  const pathname = new URL(before.url).pathname;
  if (
    agentId === "request-sms-role" ||
    (pathname === "/phone" &&
      (agentId === "call" ||
        agentId === "open-dialer" ||
        agentId?.startsWith("call:")))
  ) {
    return "native phone or Android role action has no browser DOM outcome";
  }
  if (
...
function documentedPreClickNoop(before: ControlSnapshot): string | null {
  if (
    before.details?.attributes["data-testid"] === "pendant-transcript-connect"
  ) {
    return "Web Bluetooth connection requires a human device choice in the browser-native chooser";
  }
  return null;
}
...
test.describe("every-view interaction coverage", () => {
  test("the seeded todo completion route matches the stateful LifeOps contract", async ({
    page,
  }) => {
    ...
    const contract = await page.evaluate(async () => {
      const readTodos = async () => {
        const response = await fetch("/api/lifeops/todos");
        return {
      ...
      const complete = async () => {
        const response = await fetch(
          "/api/lifeops/occurrences/todo-smoke-1/complete",
          {
            method: "POST",
      ...
      const after = await readTodos();
      const repeat = await complete();
      return { before, first, after, repeat };
    });

    expect(contract.before.status).toBe(200);
    expect(contract.before.body.todos).toContainEqual(
      expect.objectContaining({ id: "todo-smoke-1", status: "pending" }),
    );
    expect(contract.first.status).toBe(200);
    expect(contract.first.body.occurrence).toMatchObject({
      id: "todo-smoke-1",
      state: "completed",
      snoozedUntil: null,
      completionPayload: {
    ...
    expect(contract.after.status).toBe(200);
    expect(contract.after.body.todos).not.toContainEqual(
      expect.objectContaining({ id: "todo-smoke-1" }),
    );
    expect(contract.repeat).toEqual(contract.first);
  });
  ...
  for (const view of VIEW_ROUTES) {
    test(`${view.id} — exercise every control with semantic outcomes`, async ({
      ...
      // Reacquire each live position before clicking so re-renders and
      // navigation cannot leave the harness holding detached element handles.
      const clickables = page.locator(CLICK_SELECTOR);
      const clickCount = Math.min(await clickables.count(), MAX_CLICKS);
      const clickAgentIds = await clickables.evaluateAll(
        (elements, limit) =>
          elements
            .slice(0, limit)
      ...
      const clickOrder = Array.from(
        { length: clickCount },
        (_, index) => index,
      );
      // A leading plus is meaningful only while the dialer is empty, so exercise
      // that state-dependent control before the numeric keys populate it.
      clickOrder.sort(
        (left, right) =>
          Number(clickAgentIds[right] === "plus") -
          Number(clickAgentIds[left] === "plus"),
      );
      for (const i of clickOrder) {
        ...
        const preClickNoop = documentedPreClickNoop(before);
        if (preClickNoop) {
          if (!(await control.isEnabled())) {
            semanticFailures.push(
              `click ${i}: ${describeControl(before.details)} native affordance was disabled`,
            );
          } else {
            documentedNoops.push(
              `click ${i}: ${describeControl(before.details)}: ${preClickNoop}`,
            );
          }
        ...
        try {
          await controlHandle.click({ noWaitAfter: true, timeout: 2_000 });
        } catch (error) {
          actionFailures.push(
