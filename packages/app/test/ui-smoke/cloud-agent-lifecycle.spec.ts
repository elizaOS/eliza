 ...
 * `migrating → switched` transition (and the retryable failure path) render in
 * the live app shell — the on-device-legible pill, not a silent swap.
 */
test("cloud handoff: the migrating→switched pill is visible, and failures offer Retry", async ({
  ...
  const emitPhase = (detail: Record<string, unknown>, withRetry = false) =>
    page.evaluate(
      ({ detail, withRetry }) => {
        const full = withRetry
          ? {
              ...detail,
              onRetry: () => {
                (globalThis as Record<string, unknown>).__handoffRetried = true;
              },
            }
          : detail;
        window.dispatchEvent(
          new CustomEvent("eliza:cloud-handoff-phase", { detail: full }),
        );
      },
      { detail, withRetry },
    );
  ...
  await emitPhase(
    { agentId: "agent-keep", phase: "failed", error: "boot timeout" },
    true,
  );
