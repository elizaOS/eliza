...
const CORE_ROUTE_PROBES: readonly RouteProbe[] = [
  ...
  },
  {
    ...
    //
    // Assert the destination view, not only a shell. `expectMainShell` already
    // requires `main, [role='main']` on every probe, so that selector alone
    // lets a wrong view, or a stranded shell at the right URL, pass.
    // `[data-testid="relationships-view"]` is the readiness selector
    ...
    // await for this route.
    //
    // `mode: "all"` is load-bearing: ready checks default to "any", which
    // short-circuits on the first passing selector, so keeping the shell check
    // without it would let the generic selector satisfy the probe by itself.
