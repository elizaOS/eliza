...
| `odysseus` | `gui` | `/odysseus` | Conversation-centric workbench keeps the Odysseus port available while the orchestrator replacement is reviewed. | Needs screenshot review against the upstream clone and proof that chat/sidebar controls stay usable without a Python backend. | Verify shell, sidebar, composer, model controls, and honest empty states in visual smoke artifacts. |
| `odysseus` | `tui` | `/odysseus/tui` | TUI exposes Odysseus as an operator terminal alongside the visual workbench. | Needs command-output proof for the shared orchestrator task operations. | Verify terminal command buttons render stable structured results. |
