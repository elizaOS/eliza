...
- **Dual-pressure shouldRespond classifier.** The ambiguous-channel path now asks the LLM for `speak_up` and `hold_back` (0–100) plus `action` (REPLY / RESPOND / IGNORE / STOP). Net is `speak_up - hold_back`. **Why:** A single boolean or one score is easy to game; explicit “pressure to speak” vs “pressure to stay quiet” matches group-chat dynamics and makes contradictions visible in logs.
- **Post-classifier consistency (gaslighting clamp).** When both scores parse and action is REPLY/RESPOND, if `net <= -DUAL_PRESSURE_THRESHOLD` the runtime clamps to **IGNORE** and logs a warning. **STOP** is never overridden by net. **Why:** Stops the failure mode where the model claims strong reluctance but still replies; STOP must remain a user-intent escape hatch.
- **Soft warning on high net + IGNORE.** When `net >= threshold` and action is IGNORE, the decision stands but a warning is logged. **Why:** Silence can still be correct (safety, norms); operators need visibility without forcing a reply.
- **`DUAL_PRESSURE_THRESHOLD` (default 20).** Runtime setting `runtime.getSetting("DUAL_PRESSURE_THRESHOLD")`, integer 1–100, aligns with the prompt’s default band. **Why:** One knob to tune aggressiveness of the clamp without editing the template.
- **`MessageProcessingResult` fields `dualPressure` and `shouldRespondClassifierAction`.** Set when the LLM classifier path runs (scores may be `null` if parsing was incomplete). **Why:** Same data as structured logs, available to integrations and tests without scraping log lines.
- **`ResponseDecision` optional `pressure` and `classifierAction`.** For custom `shouldRespond` implementations that want parity with the core contract. **Why:** Keeps a single typed story for “scores + action” without forcing the rule-based `shouldRespond()` to fabricate LLM fields.
- **Documentation:** [docs/SHOULD_RESPOND_DUAL_PRESSURE.md](./docs/SHOULD_RESPOND_DUAL_PRESSURE.md), [ROADMAP.md](./ROADMAP.md) entries, README and DESIGN cross-links. **Why:** Preserves WHY for operators and future refactors.
...
- **shouldRespond prompt (`shouldRespondTemplate`).** Rewritten for dual-pressure, consistency band, anti-gaming hints, context routing, and examples; `action_space` is ordered before the TOON `output:` block. **Why:** Clear action definitions before format constraints reduce label confusion; rubric text replaces over-reliance on a separate anxiety provider for this gate.
- **Initial message `composeState` provider list.** Should-respond evaluation context no longer requests **ANXIETY** in that compose call (uses ENTITIES, CHARACTER, RECENT_MESSAGES, ACTIONS). **Why:** Rubric lives in the classifier prompt; dropping ANXIETY here cuts noise and coupling for the gate specifically (characters can still add ANXIETY elsewhere if registered).
- **`basic-capabilities` `shouldRespond`.** Return type is `ResponseDecision`; `ALWAYS_RESPOND_CHANNELS` / `ALWAYS_RESPOND_SOURCES` fall back to `SHOULD_RESPOND_BYPASS_TYPES` / `SHOULD_RESPOND_BYPASS_SOURCES` like `DefaultMessageService`. **Why:** One mental model and env surface for bypass rules across entry points.
