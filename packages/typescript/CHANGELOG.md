...
- **Prevent memory saving: DISABLE_MEMORY_CREATION and ALLOW_MEMORY_SOURCE_IDS**
  - **What:** When `DISABLE_MEMORY_CREATION` is true, the message service skips persisting the incoming message (and embedding queue), response memories, and ignores response-to-memory entirely (no sources are persisted). Optional `ALLOW_MEMORY_SOURCE_IDS` whitelist: when memory creation is enabled (i.e. `DISABLE_MEMORY_CREATION` is false) and `ALLOW_MEMORY_SOURCE_IDS` is set, only messages whose `metadata.sourceId` is in the comma-separated list are persisted; messages from other sources are skipped. If `ALLOW_MEMORY_SOURCE_IDS` is not set, all sources are persisted normally.
