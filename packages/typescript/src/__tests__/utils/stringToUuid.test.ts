...
describe("stringToUuid SHA-1 implementation", () => {
  ...
  it("should handle URL-unsafe characters consistently", () => {
    const urlTests = [
      {
        input: "test?query=value&param=123",
        expected: stringToUuid("test?query=value&param=123"),
      },
      {
        input: "path/to/resource#anchor",
        expected: stringToUuid("path/to/resource#anchor"),
      },
    ];
