...
import { createTestDatabaseAdapter, createTestCharacter } from "./test-utils";
...
describe("runtime-composition", () => {
  ...
  describe("mergeSettingsInto", () => {
    ...
    it("merges secrets: db base, character overrides", () => {
      ...
      const result = mergeSettingsInto(character, agentRecord);
      expect((result.settings?.secrets as Record<string, string>)?.API_KEY).toBe("char-secret");
      expect((result.settings?.secrets as Record<string, string>)?.DB_SECRET).toBe("only-in-db");
    });
  });
  ...
  describe("loadCharacters", () => {
    ...
    it("throws on invalid character object (validation failure)", async () => {
      const invalidInput = { name: 123, bio: "not-an-array" } as unknown as { name: string; bio: string };
    ...
    it("loads one character from file path when loadCharacterFile returns character", async () => {
      ...
      expect(loadCharacterFile).toHaveBeenCalledWith("/some/path/character.json");
    });
    ...
    it("throws when loadCharacterFile returns null (file load failure)", async () => {
      ...
      await expect(loadCharacters(["/missing.json"])).rejects.toThrow("Failed to load character file");
    });
  });
  ...
  describe("createRuntimes", () => {
    ...
    it("returns one initialized runtime when given one character and adapter override", async () => {
      ...
      expect(runtimes[0]).toBeDefined();
      expect(runtimes[0]!.character.name).toBe("CompTest");
      expect(runtimes[0]!.adapter).toBe(adapter);
    });
    ...
    it("uses merged character (from mergeSettingsInto) when adapter returns agent", async () => {
      ...
      expect(runtimes[0]!.character.settings?.FROM_DB).toBe("db-val");
      expect(runtimes[0]!.character.settings?.FROM_CHAR).toBe("char-val");
    });
  });
});
