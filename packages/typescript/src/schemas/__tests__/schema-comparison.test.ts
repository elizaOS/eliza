...
const mockBuildTable = (schema: any, adapter: any) => {
  // Simple mock that creates an object with column properties
  const table: any = {};
  for (const [key, col] of Object.entries(schema.columns as Record<string, any>)) {
...
const mockPgAdapter = {
  createTable: (name: string, columns: any) => columns,
  buildColumn: (col: any) => col,
  buildIndex: (name: string) => ({ on: (...cols: any[]) => ({}) }),
};
