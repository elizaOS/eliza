...
export async function ensureConnections(
  ...
  const entityMap = new Map<
    string,
    { entityId: UUID; names: string[]; metadata: Record<string, unknown>; agentId: UUID }
  >();
  ...
  for (const c of connections) {
    ...
    roomParticipants.get(c.roomId)!.add(c.entityId);
    roomParticipants.get(c.roomId)!.add(agentId);
  }
  ...
  for (const [roomId, entityIdsSet] of roomParticipants) {
    const currentResult = await adapter.getParticipantsForRooms([roomId as UUID]);
