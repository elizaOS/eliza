...
export const readAttachmentAction: Action = {
	...
	handler: async (
		...
		try {
			...
			if (clipboardResult.requested && clipboardResult.stored) {
				clipboardSummary = `${clipboardResult.replaced ? "Updated" : "Added"} clipboard item ${clipboardResult.item.id}: ${clipboardResult.item.title}`;
			} else if (clipboardResult.requested && "reason" in clipboardResult) {
				clipboardSummary = `Clipboard add skipped: ${clipboardResult.reason}`;
			}
