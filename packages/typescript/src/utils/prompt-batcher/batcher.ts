...
export class PromptBatcher {
  ...
  private async _ensureAffinityTask(affinityKey: string): Promise<void> {
    ...
    const match = existing.find(
      (t) => (t.metadata as Record<string, unknown>)?.affinityKey === affinityKey,
    );
    if (match?.id) {
  ...
  private async _syncAffinityTask(affinityKey: string): Promise<void> {
    ...
    if (!taskId) return;
    if (typeof this.runtime.getTask !== "function" || typeof this.runtime.updateTask !== "function") return;
    ...
    const current = (task.metadata as Record<string, unknown>)?.updateInterval as number | undefined;
  ...
  private async _drainAffinityGroupUnlocked(affinityKey: string): Promise<void> {
    if (this.disposed || !this.enabled) {
      return;
    }
  ...
  private async _resolveContext(
    ...
    if (providers.length === 0) {
      ...
      const state = await this.runtime.composeState(anchorMessage, providers, true);
      pieces.push(state.text ?? "");
    }
    ...
    if (
      ...
      Array.from(this.sections.values()).some(
        (other) =>
          other.id !== section.id &&
          (other.affinityKey ?? "default") === (section.affinityKey ?? "default") &&
