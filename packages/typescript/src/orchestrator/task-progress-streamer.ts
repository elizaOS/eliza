...
export function installTaskProgressStreamer(
	...
	svc.onSessionEvent((sessionId, event, data) => {
		if (!sessionStartedAt.has(sessionId)) {
			...
			if (meta?.roomId) {
				void (async () => {
					const roomId = meta.roomId as NonNullable<typeof meta.roomId>;
					...
					if (!roomId && meta?.threadId) {
						const coordinator = runtime.getService("SWARM_COORDINATOR") as
							| {
									getTaskThread?: (
										threadId: string,
									) => Promise<{ roomId?: UUID | null } | null>;
							  }
							| undefined;
						const thread = await coordinator
							?.getTaskThread?.(meta.threadId)
							.catch(() => null);
						if (thread?.roomId) {
...
async function postFinalReport(
	...
	const entry = workdir ? await readLatestAssistantFromWorkdir(workdir) : null;
