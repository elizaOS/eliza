...
export function validateModelConfig(runtime?: IAgentRuntime): ModelConfig {
			...
			if (runtime) {
				...
				const normalizedRuntimeValue =
					typeof runtimeValue === "string"
						...
						: undefined;
				return normalizedRuntimeValue ?? normalizeEnvValue(process.env[key]) ?? defaultValue;
			}
    ...
    const inferredLocalEmbeddings = !embeddingProvider &&
      Boolean(localEmbeddingModel || localEmbeddingDimensions);
    const resolvedEmbeddingProvider = embeddingProvider || (inferredLocalEmbeddings ? "local" : undefined);
    ...
    const textEmbeddingModel =
      ...
      (resolvedEmbeddingProvider === "local" ? "local-embedding" : "text-embedding-3-small");
