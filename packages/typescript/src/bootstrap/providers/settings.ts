  ...
  try {
    ...
    if (isOnboarding) {
      ...
      if (requiredUnconfigured > 0) {
        const senderName = state && state.senderName ? state.senderName : "user";
  ...
  get: async (
    ...
    try {
      ...
      if (isOnboarding) {
        ...
        world = (userWorlds && userWorlds.find(
        ...
        if (world.metadata && world.metadata.settings) {
          const salt = getSalt();
