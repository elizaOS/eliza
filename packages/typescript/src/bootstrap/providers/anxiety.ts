import type { IAgentRuntime, Memory, Provider } from "../../types/index.ts";
import { ChannelType } from "../../types/primitives.ts";
import { addHeader } from "../../utils.ts";

/**
 * Anxiety provider: channel-specific guidance to reduce AI verbosity and over-eagerness.
 * WHY: Message service already requests ANXIETY in initial state; this provider fulfills that
 * so the model gets concrete guidance (e.g. be brief in groups, use IGNORE when unsure).
 * Randomly selects three examples per channel type so the model sees variety across turns.
 */
