...
async function handler(
	...
	const result = await runtime.promptBatcher.onDrain<ReflectionFields>(
	...
	if (result) {
		...
		const relationshipsArray = Array.isArray(fields.relationships)
		...
		for (const relationship of relationshipsArray) {
			...
			const tags = Array.isArray(relationship.tags)
				? relationship.tags.map((tag) => String(tag).trim()).filter(Boolean)
