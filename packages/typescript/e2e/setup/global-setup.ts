...
async function resolveProviderPlugin(providerName: string): Promise<Plugin | null> {
  switch (providerName) {
    case "openai": {
      const mod = await import("../../../../plugins/plugin-openai/typescript/index");
      return mod.openaiPlugin ?? mod.default ?? null;
    }
    case "anthropic": {
      const mod = await import("../../../../plugins/plugin-anthropic/typescript/index");
      return mod.anthropicPlugin ?? mod.default ?? null;
    }
    case "google": {
      try {
        const mod = await import("../../../../plugins/plugin-google-genai/typescript/index");
    ...
    for (const [modelType, handler] of Object.entries(handlers)) {
      if (handler) {
        runtime.registerModel(
          modelType,
          handler as (rt: AgentRuntime, p: Record<string, unknown>) => Promise<unknown>,
          "ollama",
        );
      }
    }
