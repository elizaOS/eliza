...
test.describe("sleep / wake", () => {
  test("running agent sleeps (durable backup + freed compute) then wakes back to running", async ({
    ...
    const sandboxId = await createCloudAgent(api, seededUser.apiKey, "e2e-sleep-wake");
    ...
    await agentLifecycleAction(api, seededUser.apiKey, sandboxId, "sleep", [202]);
    ...
    expect(backups.length, "sleep must leave at least one restore point").toBeGreaterThanOrEqual(
      1,
    );

    // Wake: a fresh container is provisioned and state restored, status → running.
    await agentLifecycleAction(api, seededUser.apiKey, sandboxId, "wake", [202]);
    await pollSandboxStatus(api, seededUser.apiKey, sandboxId, "running", {
      timeoutMs: 30_000,
      onTick: onTick(stack.urls.api),
    });
  ...
  test("sleep is idempotent for an already-sleeping agent", async ({ stack, seededUser }) => {
    const api = { apiUrl: stack.urls.api };
    const sandboxId = await createCloudAgent(api, seededUser.apiKey, "e2e-sleep-idem");
    await startAgentProvisioning(api, seededUser.apiKey, sandboxId);
    await pollSandboxStatus(api, seededUser.apiKey, sandboxId, "running", {
      timeoutMs: 30_000,
      onTick: onTick(stack.urls.api),
  ...
  test("waking an already-running agent is a no-op", async ({ stack, seededUser }) => {
    const api = { apiUrl: stack.urls.api };
    const sandboxId = await createCloudAgent(api, seededUser.apiKey, "e2e-wake-noop");
    await startAgentProvisioning(api, seededUser.apiKey, sandboxId);
    await pollSandboxStatus(api, seededUser.apiKey, sandboxId, "running", {
      timeoutMs: 30_000,
      onTick: onTick(stack.urls.api),
    ...
    const { body } = await agentLifecycleAction(api, seededUser.apiKey, sandboxId, "wake", [200]);
