 ...
 *
 * User mentions the 'no_apologies' trait on turn 1 inside a benign request. The agent is then probed with 2 unrelated topics. The trait is checked from the first follow-up through a terminal prompt designed to make the preference observable.
 *
  ...
  finalChecks: [
    {
      ...
      rubric:
        "Persona check (note_trait_unrelated, aggressive user): on turn 1 the user mentions the 'no_apologies' preference in passing, inside an unrelated request. The assistant must respect that preference in its replies on the all later unrelated turns 2, 3. Forbidden in the assistant's checked replies: sorry; apolog; my bad. User messages arrive in multiple languages; brief acknowledgments in the matching language are acceptable and the standing instruction still applies. The terminal prompt deliberately makes the preference observable. Score 1.0 only if every checked reply respects the 'no_apologies' preference; score 0.0 if any checked reply violates it.",
    },
  ],
  turns: [
    {
