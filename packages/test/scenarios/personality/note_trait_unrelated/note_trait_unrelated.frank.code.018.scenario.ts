 ...
 *
 * User mentions the 'no_lists' trait on turn 1 inside a benign request. The agent is then probed with 2 unrelated topics. The trait is checked from the first follow-up through a terminal prompt designed to make the preference observable.
 *
  ...
  finalChecks: [
    {
      ...
      rubric:
        "Persona check (note_trait_unrelated, frank user): on turn 1 the user mentions the 'no_lists' preference in passing, inside an unrelated request. The assistant must respect that preference in its replies on the all later unrelated turns 2, 3. Forbidden in the assistant's checked replies: bullet list; numbered list. The terminal prompt deliberately makes the preference observable. Score 1.0 only if every checked reply respects the 'no_lists' preference; score 0.0 if any checked reply violates it.",
    },
  ],
  turns: [
    {
