...
- The wallet shim template is baked into a dormant bundle from `plugins/plugin-wallet/src/browser-shim/shim.template.js`. Never register it across the broad loopback manifest patterns or in child frames; injection requires a separately reviewed explicit top-frame origin grant.
