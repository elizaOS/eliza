...
Browser extension (Chrome, Firefox, and Safari) that pairs a user's browser profile with an Eliza agent so the agent can read open tabs and execute owner-approved browser actions.
...
bun run --cwd packages/browser-bridge-extension lint                    # Biome check --write --unsafe
bun run --cwd packages/browser-bridge-extension lint:check              # Biome check (read-only)
bun run --cwd packages/browser-bridge-extension format                  # Biome format --write
bun run --cwd packages/browser-bridge-extension format:check            # Biome format (read-only)
bun run --cwd packages/browser-bridge-extension package:chrome          # .zip for Chrome Web Store
bun run --cwd packages/browser-bridge-extension package:firefox         # .zip/XPI-ready Firefox archive
...
bun run --cwd packages/browser-bridge-extension test                    # unit tests + Chrome dist smoke
