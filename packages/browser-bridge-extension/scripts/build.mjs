...
const supportedBrowserKinds = new Set(["chrome", "firefox", "safari"]);
const requestedBrowserKind = process.argv[2] ?? "chrome";
if (!supportedBrowserKinds.has(requestedBrowserKind)) {
  throw new Error(
    `Unsupported browser extension target: ${requestedBrowserKind}`,
  );
}
const browserKind = requestedBrowserKind;
