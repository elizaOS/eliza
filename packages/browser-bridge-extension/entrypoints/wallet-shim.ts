...
type WalletShimExtensionApi = {
  storage?: {
    local?: {
      get?: (
        keys: string[],
        callback: (items: unknown) => void,
      ) => Promise<{ walletShim?: WalletShimStored }> | undefined;
    };
  };
};
  ...
  return new Promise((resolve) => {
    try {
      const api =
        (
          globalThis as {
            chrome?: WalletShimExtensionApi;
            browser?: WalletShimExtensionApi;
          }
        ).chrome ??
        (globalThis as { browser?: WalletShimExtensionApi }).browser;
