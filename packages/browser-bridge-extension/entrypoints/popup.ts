...
function popupResponseError(response: PopupResponse): string {
  return "error" in response
    ? response.error
    : "Agent Browser Bridge returned an invalid response.";
}
...
function readConfig(refs: FormRefs): Partial<CompanionConfig> {
  return {
    ...
    browser:
      refs.browser.value === "safari"
        ? "safari"
        : refs.browser.value === "firefox"
          ? "firefox"
          : "chrome",
...
async function refresh(refs: FormRefs): Promise<void> {
  ...
  if (!response.state.config && !autoPairAttempted) {
    ...
    if (!autoPairResponse.ok) {
      refs.statusDetail.textContent = popupResponseError(autoPairResponse);
    }
  }
}
