...
import { FROM_AI_AUTO_GENERATED_MARKER } from './constants.js';
import { isExpression } from './expressions/expression-helpers.js';
import type { INodeProperties } from './interfaces.js';
...
export function generateZodSchema(placeholder: FromAIArgument): z.ZodTypeAny {
	...
	switch (placeholder.type?.toLowerCase()) {
		...
		case 'json': {
			...
			const typedSchema = customSchema as unknown as z.ZodType<
				Record<string, unknown> | unknown[]
			> & { _def: { jsonSchema?: unknown } };
...
function parseDefaultValue(
	...
	if (type === 'number' && !Number.isNaN(Number(value))) return Number(value);
...
export function traverseNodeParameters(payload: unknown, collectedArgs: FromAIArgument[]) {
	if (typeof payload === 'string') {
		...
		collectedArgs.push(...fromAICalls);
	} else if (Array.isArray(payload)) {
		payload.forEach((item: unknown) => {
			traverseNodeParameters(item, collectedArgs);
		});
	} else if (typeof payload === 'object' && payload !== null) {
		Object.values(payload).forEach((value) => {
			traverseNodeParameters(value, collectedArgs);
		});
	}
}
...
export function traverseNodeParametersWithParamNames(
	...
	if (typeof payload === 'string') {
		const fromAICalls = extractFromAICalls(payload);
		fromAICalls.forEach((call) => {
			collectedArgs.set(name as string, call);
		});
	} else if (Array.isArray(payload)) {
