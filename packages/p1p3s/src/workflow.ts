...
export class Workflow {
	...
	getHighestNode(
		...
		if (!Object.hasOwn(this.connectionsByDestinationNode[nodeName], NodeConnectionTypes.Main)) {
	...
	searchNodesBFS(connections: IConnections, sourceNode: string, maxDepth = -1): IConnectedNode[] {
		...
		while (queue.length > 0) {
			...
			toAdd.forEach((curr) => {
				...
				if (
					!Object.hasOwn(connections, curr.name) ||
					!Object.hasOwn(connections[curr.name], type)
				) {
					return;
				}
