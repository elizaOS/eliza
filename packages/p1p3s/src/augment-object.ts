import type { IDataObject } from './interfaces.js';

const defaultPropertyDescriptor = Object.freeze({ enumerable: true, configurable: true });

const { hasOwnProperty: objectHasOwnProperty } = Object.prototype;
...
export function augmentObject<T extends object>(data: T): T {
	...
	const proxy = new Proxy(data, {
		get(target, key: string, receiver): unknown {
			...
			if (objectHasOwnProperty.call(newData, key)) {
				return newData[key];
			}
		...
		deleteProperty(_target, key: string) {
			...
			if (objectHasOwnProperty.call(data, key)) {
		...
		has(_target, key) {
			...
			const target = objectHasOwnProperty.call(newData, key) ? newData : data;
