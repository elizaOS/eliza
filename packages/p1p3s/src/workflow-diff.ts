...
import { type ConnectionsDiff, compareConnections } from './connections-diff.js';
import type {
	IConnections,
	INode,
	INodeParameters,
...
export function parametersAreSuperset(prev: unknown, next: unknown): boolean {
	...
	if (params.some((x) => !Object.hasOwn(next, x))) return false;
...
function makeMergeDependingOnSizeRule<_W extends DiffableWorkflow>(mapping: Map<number, number>) {
