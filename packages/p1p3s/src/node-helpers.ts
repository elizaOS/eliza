...
import { EXECUTE_WORKFLOW_NODE_TYPE } from './constants.js';
import { isExpression } from './expressions/expression-helpers.js';
...
const getPropertyValues = (
	...
	if (value && typeof value === 'object' && '__rl' in value) {
		const resourceLocator = value as { __rl?: unknown; value?: unknown };
		if (resourceLocator.__rl) {
			value = resourceLocator.value;
		}
	}
...
function resolveResourceAndOperation(
	...
	if (nodeTypeDescription.name === 'workflows-nodes-base.code') {
...
const TRIGGER_NODE_TYPES = new Set([
	'workflows-nodes-base.webhook',
	'workflows-nodes-base.cron', // Legacy schedule trigger
	'workflows-nodes-base.emailReadImap', // Email polling trigger
	'workflows-nodes-base.telegramBot', // Can act as webhook trigger
	'workflows-nodes-base.start', // Legacy trigger
]);
