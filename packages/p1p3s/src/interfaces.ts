// Structural opaque type for callback managers supplied by host applications.
...
import type { IDeferredPromise } from './deferred-promise.js';
import type { ExpressionError } from './errors/expression.error.js';
import type { ExecutionCancelledError } from './errors/index.js';
import type { NodeApiError } from './errors/node-api.error.js';
import type { NodeOperationError } from './errors/node-operation.error.js';
import type { WorkflowActivationError } from './errors/workflow-activation.error.js';
import type { WorkflowOperationError } from './errors/workflow-operation.error.js';
import type {
	IExecutionContext,
...
export type { WorkflowExecuteModeValues as WorkflowExecuteMode } from './execution-context.js';
...
export namespace DeclarativeRestApiSettings {
	...
	export type ResultOptions = {
		...
		requestOperations?: IWorkflowsRequestOperations;
	};
}
...
export type IExecuteResponsePromiseData = IDataObject | IWorkflowsHttpFullResponse;
 ...
 * @retired Prefer using IHttpRequestOptions
 */
...
export type IWorkflowsHttpResponse = IDataObject | Buffer | GenericValue | GenericValue[] | null;

export interface IWorkflowsHttpFullResponse {
	body: IWorkflowsHttpResponse | Readable;
...
export interface IWorkflowsRequestOperations {
	pagination?:
		| IWorkflowsRequestOperationPaginationGeneric
		| IWorkflowsRequestOperationPaginationOffset
		| ((
...
export interface IWorkflowsRequestOperationPaginationBase {
	type: string;
	properties: {
		[key: string]: unknown;
	};
...
export interface IWorkflowsRequestOperationPaginationGeneric
	extends IWorkflowsRequestOperationPaginationBase {
	type: 'generic';
	properties: {
		continue: boolean | string;
		request: IRequestOptionsSimplifiedAuth;
...
export interface IWorkflowsRequestOperationPaginationOffset
	extends IWorkflowsRequestOperationPaginationBase {
	type: 'offset';
	properties: {
...
namespace ExecuteFunctions {
	...
	export type GetNodeParameterFn = {
		// @TECH_DEBT: Refactor to remove this barely used overload - WORKFLOWS-5632
...
export interface INodeCredentialsDetails {
	...
	name: string;
	/**
	 * Set automatically when the credential slot is managed by the workflows AI Gateway.
...
export interface INodeExecutionData {
	 ...
	 *
	 * See example in
	 * chat trigger implementations
	 */
	sendMessage?: ChatNodeMessage;

	/**
	 * @retired This key was added by accident and should not be used as it
...
export interface ILoadOptions {
	routing?: {
		operations?: IWorkflowsRequestOperations;
...
export interface INodeProperties {
	 ...
	 * truthy. The value is the flag suffix without the `WORKFLOWS_ENV_FEAT_` prefix
	 */
...
export interface INodePropertyRouting {
	operations?: IWorkflowsRequestOperations; // Should be changed, does not sound right
	output?: INodeRequestOutput;
	request?: DeclarativeRestApiSettings.HttpRequestOptions;
...
export type PostReceiveAction =
	| ((
			...
			response: IWorkflowsHttpFullResponse
	  ) => Promise<INodeExecutionData[]>)
...
/**
 * @retired Use INodeFilter instead
 */
...
export type NodeDefaults = Partial<{
	/**
	 * @retired Use {@link INodeTypeBaseDescription.iconColor|iconColor} instead. `iconColor` supports dark mode and uses preset colors from workflows's design system.
	 */
	color: string;
	name: string;
}>;
...
export interface IRelatedNode {
	/** The node type ID (e.g., 'workflows workflow node') */
	nodeType: string;
 ...
 * filesystem-backed `DirectoryLoader` in `workflows-core` and in modules
 */
...
export interface IWorkflowExecutionDataProcess {
	 ...
	 * ignores WORKFLOWS_MINIMIZE_EXECUTION_DATA_FETCHING environment variable if set
	 */
...
export interface INodeGraphItem {
	...
	agent?: string; //workflows workflow node
	is_streaming?: boolean; //workflows workflow node
	prompts?: IDataObject[] | IDataObject; //ai node's prompts, cloud only
	use_responses_api?: boolean; //workflows workflow node
	toolSettings?: IDataObject; //various workflow tool's settings
	sql?: string; //merge node combineBySql, cloud only
	workflow_id?: string; //workflows workflow node and workflows-nodes-base.executeWorkflow
	response_mode?: string; // workflows workflow node, workflows-nodes-base.webhook selected response mode
	public_chat?: boolean; // workflows workflow node
	runs?: number;
	items_total?: number;
	metric_names?: string[];
	language?: string; // only for Code node: 'javascript' or 'python' or 'pythonNative'
	package_version?: string; // only for community nodes
	used_guardrails?: string[]; // only for workflows workflow node
	mcp_client_auth_method?: string; // for workflows workflow node and workflows workflow node
	...
	ai_gateway_credentials?: boolean; // true if node used workflows Connect / AI Gateway managed credentials
}
...
export type WorkflowsAIProviderType = 'openai' | 'unknown';

export type Functionality = 'regular' | 'configuration-node' | 'pairedItem';

export type CallbackManager = CallbackManagerPlaceholder;

export type IPersonalizationSurveyAnswersV4 = {
	version: 'v4';
	personalization_survey_submitted_at: string;
	personalization_survey_workflows_version: string;
