...
describe("parseWindowsPowerLineOutput", () => {
  ...
  it("takes the last non-empty line and rejects garbage", () => {
    expect(
      parseWindowsPowerLineOutput("warning line\nOffline"),
    ).toEqual({ onBattery: true, known: true });
...
describe("parseMacOsPowerSourceOutput", () => {
  it("detects battery and AC power", () => {
    expect(parseMacOsPowerSourceOutput("Current Power Source: Battery Power")).toEqual({
      onBattery: true,
      known: true,
    });
    expect(parseMacOsPowerSourceOutput("Current Power Source: AC Power")).toEqual({
      onBattery: false,
      known: true,
    });
...
describe("parseMacOsSessionLockedOutput", () => {
  it("parses locked/unlocked", () => {
    expect(parseMacOsSessionLockedOutput("CGSSessionScreenIsLocked = 1")).toBe(true);
