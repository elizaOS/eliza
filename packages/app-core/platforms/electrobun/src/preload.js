(() => {
  ...
  function j(w = {}) {
    ...
    const { maxRequestTime: G = 1000 } = w;
    if (w.transport) B(w.transport);
    if (w.requestHandler) A(w.requestHandler);
    if (w._debugHooks) _ = w._debugHooks;
    let J = 0;
    function Q() {
      if (J <= 10000000000) return ++J;
      return (J = 0);
    }
    const W = new Map(),
      $ = new Map();
    function C(f, ...S) {
      ...
      return new Promise((Z, K) => {
        if (!b.send) throw V(["send"], "make requests");
        const X = Q(),
          L = { type: "request", id: X, method: f, params: I };
        if ((W.set(X, { resolve: Z, reject: K }), G !== 1 / 0))
          $.set(
            X,
            setTimeout(() => {
              $.delete(X), W.delete(X), K(Error("RPC request timed out."));
            }, G),
          );
        _.onSend?.(L), b.send(L);
      });
    ...
    const D = new Proxy(C, {
        get: (f, S, I) => {
          if (S in f) return Reflect.get(f, S, I);
          return (Z) => C(S, Z);
        },
      }),
      M = D;
    function H(f, ...S) {
      const I = S[0];
      if (!b.send) throw V(["send"], "send messages");
      const Z = { type: "message", id: f, payload: I };
      _.onSend?.(Z), b.send(Z);
    }
    const R = new Proxy(H, {
        get: (f, S, I) => {
    ...
    function d(f, S) {
      if (!b.registerHandler)
        throw V(["registerHandler"], "register message listeners");
      if (f === "*") {
        U.add(S);
        return;
      }
    ...
    function l(f, S) {
      ...
      if ((T.get(f)?.delete(S), T.get(f)?.size === 0)) T.delete(f);
    }
    async function p(f) {
      ...
      if (f.type === "request") {
        if (!b.send || !Y)
          throw V(["send", "requestHandler"], "handle requests");
        let { id: S, method: I, params: Z } = f,
          K;
        try {
          K = { type: "response", id: S, success: !0, payload: await Y(I, Z) };
        } catch (X) {
          if (!(X instanceof Error)) throw X;
          K = { type: "response", id: S, success: !1, error: X.message };
        }
        _.onSend?.(K), b.send(K);
        return;
      }
      if (f.type === "response") {
        ...
        $.delete(f.id);
        const { resolve: I, reject: Z } = W.get(f.id) ?? {};
        if ((W.delete(f.id), !f.success)) Z?.(Error(f.error));
        else I?.(f.payload);
        return;
      }
      if (f.type === "message") {
        for (const I of U) I(f.id, f.payload);
        const S = T.get(f.id);
        if (!S) return;
        for (const I of S) I(f.payload);
        return;
      }
  ...
  function O(w, _) {
    const b = {
      ...
      Y = j(b),
      B = _.handlers.messages;
    if (B)
      Y.addMessageListener("*", (A, G) => {
        const J = B["*"];
        if (J) J(A, G);
        const Q = B[A];
        if (Q) Q(G);
  ...
  var { __electrobunWebviewId: N, __electrobunRpcSocketPort: h } = window;
  class z {
    bunSocket;
    rpc;
    rpcHandler;
    ...
    initSocketToBun() {
      if (!h || !N) return;
      const w = new WebSocket(`ws://localhost:${h}/socket?webviewId=${N}`);
      (this.bunSocket = w),
      ...
      if (this.bunSocket?.readyState === WebSocket.OPEN)
        try {
          const {
            ...
            A = JSON.stringify({ encryptedData: _, iv: b, tag: Y });
              ...
              try {
                ...
                if (B instanceof Promise)
                  B.then((A) => {
                    ...
                    console.error("bun: async script execution failed", A),
                      b(String(A));
                  });
                else b(B);
              } catch (Y) {
  ...
  var o = Symbol.for("elizaos.app.boot-config"),
    u = o;
  function n(w, _) {
    const Y = {
  ...
  function g(w, _) {
    if (w === "apiBaseUpdate") {
      const Y = _;
      if (((window.__ELIZA_API_BASE__ = Y.base), Y.token))
      ...
      n(window, { apiBase: Y.base, ...(Y.token ? { apiToken: Y.token } : {}) });
    }
    const b = F[w];
    if (!b) return;
      ...
      requests: {
        browserWorkspaceRendererEvaluate: async (w) => {
          const _ = i(w),
            ...
            Y = P(_, "script"),
            B = m(_, "timeoutMs");
          return await x().evaluate(b, Y, B);
        },
        browserWorkspaceRendererGetTabRect: async (w) => {
          const _ = i(w);
          return x().getTabRect(P(_, "id"));
        },
      },
      messages: { "*": t },
    },
  ...
  var r = new Proxy(E.request, {
    ...
    a = {
      ...
      offMessage: (w, _) => {
        if ((F[w]?.delete(_), F[w]?.size === 0)) delete F[w];
      },
    };
  ...
  function s() {
    ...
    for (const Y of b) {
      const B = console[Y].bind(console);
      console[Y] = (...A) => {
        B(...A),
          _(
            Y,
          ...
          Q =
            typeof G === "string"
              ...
              : G instanceof Request
                ? G.url
                : String(G),
          W = J?.method ?? (G instanceof Request ? G.method : void 0) ?? "GET";
        try {
          const $ = await Y(...B);
          if (!$.ok)
            _(
              ...
              { url: Q, method: W, durationMs: Date.now() - A },
            );
          return $;
        } catch ($) {
          throw (
    ...
    if (typeof XMLHttpRequest < "u") {
      ...
      (XMLHttpRequest.prototype.open = function (A, G, ...J) {
        return (
          (this.__elizaDiag = {
            method: A,
            url: String(G),
          ...
          const J = () => {
            ...
            Q = () => {
              ...
              _("error", "xhr", "XMLHttpRequest failed", {
                url: W?.url,
                method: W?.method,
                durationMs: W ? Date.now() - W.startedAt : void 0,
              });
            };
          return (
            this.addEventListener("loadend", J, { once: !0 }),
