...
function normalizeAgent(value: unknown): AgentSummary | null {
	if (!isRecord(value)) return null;
	const id = asString(value.id) ?? asString(value.agentId) ?? asString(value.uuid);
	...
	return {
		...
		...(metadataWithout(value, ["id", "agentId", "uuid", "name", "status"])
			? { metadata: metadataWithout(value, ["id", "agentId", "uuid", "name", "status"]) }
			: {}),
	};
}
...
function normalizeConversation(value: unknown): ConversationSummary | null {
	if (!isRecord(value)) return null;
	const id =
		asString(value.id) ?? asString(value.conversationId) ?? asString(value.uuid);
	if (id === undefined) return null;
	return {
		id,
		...(asString(value.title) ? { title: asString(value.title) } : {}),
		...(asString(value.agentId) ? { agentId: asString(value.agentId) } : {}),
		...(asString(value.updatedAt) ? { updatedAt: asString(value.updatedAt) } : {}),
...
export class ElizaRuntimeApiClient {
	...
	async sendMessage(params: AgentMessageParams): Promise<AgentMessageResult> {
		...
		const conversationId =
			params.conversationId ?? (await this.createConversation(text, params.agentId));
		...
		return {
			...
			...(idFromResponse(raw, ["messageId", "id"]) !== undefined
				...
				: {}),
			...(textFromResponse(raw) !== undefined ? { text: textFromResponse(raw) } : {}),
			raw,
		};
	}
	...
	async listConversations(): Promise<ConversationSummary[]> {
		...
		if (array === null) {
			throw createApiBridgeError({
				code: "DECODE_FAILED",
				message: "Conversation list response did not contain a conversation array.",
				method: "GET",
				path: route.path,
				details: raw,
			});
	...
	private async requestJson(options: ApiRequestOptions): Promise<unknown> {
		...
		try {
			...
			const message =
				error instanceof Error ? errorMessage(error) : "Runtime API request failed";
			throw createApiBridgeError({
				code: "REQUEST_FAILED",
				message,
				method,
	...
	private async createConversation(text: string, agentId?: string): Promise<string> {
		const raw = await this.requestJson({
			method: "POST",
			path: "/api/conversations",
			body: {
				title: text.replace(/\s+/g, " ").trim().split(" ").slice(0, 5).join(" "),
				...(agentId === undefined
					? {}
