...
const RUNTIME_REMOTE_ID = "eliza.runtime";
...
function invokeRuntime(
	...
	return new Promise((resolve, reject) => {
		const timeout = setTimeout(() => {
			...
			reject(new Error(`Runtime Remote request timed out: ${method}`));
		}, 30_000);
...
function completeHostRequest(message: HostResponse): void {
	...
	request.reject(new Error(message.error ?? "Runtime Remote request failed."));
}
