...
`host:manage-carrots` is required by the existing module host for cross-Remote calls through the upstream `invoke-carrot` host request. No filesystem, shell, Git, model-download, or child-process permission is requested by Surface.
