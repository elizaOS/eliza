...
export class GitCommandRunner {
	...
	constructor(options: { env?: NodeJS.ProcessEnv; history?: GitOperationHistory } = {}) {
	...
	async run(input: GitRunInput): Promise<GitRunOutput> {
		...
		try {
			...
			throwGitError({
				code: input.args[0] === "--version" ? "GIT_NOT_AVAILABLE" : "GIT_COMMAND_FAILED",
				message,
				cwd,
				command,
		...
		if (exitCode !== 0) {
			const message = stderr.trim() || stdout.trim() || `git exited with ${exitCode}`;
