...
function parseShowParams(params?: JsonValue): { cwd?: string; ref: string; path?: string } {
...
function parseRestoreParams(params?: JsonValue): GitRestoreParams {
	if (!isRecord(params)) throw new Error("git.restore params must be an object.");
	return {
		paths: stringArrayParam(params, "paths"),
		...(typeof params.cwd === "string" ? { cwd: params.cwd } : {}),
		...(typeof params.staged === "boolean" ? { staged: params.staged } : {}),
...
function parseCheckoutParams(params?: JsonValue): GitCheckoutParams {
	if (!isRecord(params)) throw new Error("git.checkout params must be an object.");
	return {
		ref: stringParam(params, "ref"),
		...(typeof params.cwd === "string" ? { cwd: params.cwd } : {}),
...
function parseBranchCreateParams(params?: JsonValue): GitBranchCreateParams {
	if (!isRecord(params)) throw new Error("git.branch.create params must be an object.");
	return {
		name: stringParam(params, "name"),
		...(typeof params.cwd === "string" ? { cwd: params.cwd } : {}),
		...(typeof params.checkout === "boolean" ? { checkout: params.checkout } : {}),
...
function parseBranchDeleteParams(params?: JsonValue): GitBranchDeleteParams {
	if (!isRecord(params)) throw new Error("git.branch.delete params must be an object.");
	return {
		name: stringParam(params, "name"),
		...(typeof params.cwd === "string" ? { cwd: params.cwd } : {}),
...
function parseCommitParams(params?: JsonValue): GitCommitParams {
	if (!isRecord(params)) throw new Error("git.commit params must be an object.");
	return {
		message: stringParam(params, "message"),
		...(typeof params.cwd === "string" ? { cwd: params.cwd } : {}),
		...(typeof params.amend === "boolean" ? { amend: params.amend } : {}),
		...(typeof params.noVerify === "boolean" ? { noVerify: params.noVerify } : {}),
	};
}

function parseRemoteOperationParams(params?: JsonValue): GitRemoteOperationParams {
	if (params === undefined) return {};
	if (!isRecord(params)) throw new Error("Git remote operation params must be an object.");
	return {
		...(typeof params.cwd === "string" ? { cwd: params.cwd } : {}),
...
function parseOperationListParams(params?: JsonValue): { limit?: number } {
	if (params === undefined) return {};
	if (!isRecord(params)) throw new Error("git.operation.list params must be an object.");
	return {
		...(typeof params.limit === "number" ? { limit: params.limit } : {}),
	};
}
...
function parseCommandRunParams(params?: JsonValue): GitCommandRunParams {
	if (!isRecord(params)) throw new Error("git.command.run params must be an object.");
	return {
