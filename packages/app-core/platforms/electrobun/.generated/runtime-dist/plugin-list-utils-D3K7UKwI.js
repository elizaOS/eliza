  ...
  for (const p of params) {
    ...
    if (
      ...
      if (pipeMatch) {
        ...
        const safeQuoted = [...desc.matchAll(/'([a-z0-9_-]+)'/gi)]
          .map((m) => m[1])
          .filter((v) => /^[a-z0-9_-]+$/i.test(v));
...
function isPluginReady(plugin) {
  ...
  return !(
    plugin.parameters?.some((param) => param.required && !param.isSet) ?? false
  );
}
...
//#endregion
export {
  ALWAYS_ON_PLUGIN_IDS as t,
  buildPluginListState as a,
  getPluginResourceLinks as o,
  iconImageSource as s,
  paramsToSchema as c,
  pluginResourceLinkLabel as l,
  resolveIcon as u,
  SUBGROUP_LABELS as n,
  SUBGROUP_NAV_ICONS as r,
  subgroupForPlugin as d,
  TELEGRAM_ALLOW_ALL_HIDDEN as i,
};
