import {
	DEFAULT_WALLET_RPC_SELECTIONS as DEFAULT_WALLET_RPC_SELECTIONS$1,
	isElizaSettingsDebugEnabled,
	normalizeWalletRpcProviderId,
	normalizeWalletRpcSelections as normalizeWalletRpcSelections$1,
	packageNameToAppRouteSlug,
	sanitizeForSettingsDebug,
	settingsDebugCloudSummary,
	WALLET_RPC_PROVIDER_OPTIONS as WALLET_RPC_PROVIDER_OPTIONS$1,
} from "@elizaos/shared";
import { createContext, useContext } from "react";
import {
	r as getAppBlockerPlugin,
	v as getWebsiteBlockerPlugin,
	S as invokeDesktopBridgeRequest,
	n as isElectrobunRuntime,
	E as require_index_cjs,
...
function resolveCharacterCatalog(catalog) {
	...
	return {
		...
		getInjectedCharacter: (catchphrase) =>
...
function tidyAssistantTextSpacing(input) {
	return (input.length > 2e5 ? input.slice(0, 2e5) : input)
		.replace(/[ \t]+\n/g, "\n")
		.replace(/\n[ \t]+/g, "\n")
		.replace(/[ \t]{2,}/g, " ")
		.replace(/ ?([,.;!?])/g, "$1")
...
function isLikelySnapshotReplacement(existing, incoming) {
	...
	const sharedLength =
		sharedPrefixLength +
		commonSuffixLength(existing, incoming, sharedPrefixLength);
...
function isNativeDirectCloudApiUrl(url) {
	try {
		...
		return (
			import_index_cjs.Capacitor.isNativePlatform() &&
...
var ElizaClient = class ElizaClient {
	...
	async rawRequest(path, init, options) {
		...
		const makeRequest = async (token) => {
			...
			try {
				const nativeCloudResponse = await requestWithNativeCloudHttp(
					requestUrl,
					requestInit,
					{ timeoutMs },
				);
	...
	onWsEvent(type, handler) {
		if (!this.wsHandlers.has(type))
			this.wsHandlers.set(type, /* @__PURE__ */ new Set());
	...
	normalizeAssistantText(text) {
		...
		if (trimmed.length === 0) {
			if (text.trim().length === 0 || /^\(?no response\)?$/i.test(text.trim()))
			...
			try {
				...
				const timeoutPromise = new Promise((_, reject) => {
					const id = setTimeout(
						() =>
							reject(
								/* @__PURE__ */ new Error("SSE idle timeout — no data for 60s"),
							),
						SSE_IDLE_TIMEOUT_MS,
					);
...
ElizaClient.prototype.getRelationshipsGraph = async function (query) {
	...
	const qs = params.toString();
	return (await this.fetch(`/api/relationships/graph${qs ? `?${qs}` : ""}`))
		.data;
};
...
ElizaClient.prototype.getRelationshipsPerson = async function (id) {
	return (
		await this.fetch(`/api/relationships/people/${encodeURIComponent(id)}`)
	).data;
};
...
ElizaClient.prototype.getExperienceGraph = async function (options) {
	...
	return {
		graph: (
			await this.fetch(`/api/character/experiences/graph${qs ? `?${qs}` : ""}`)
		).data,
	};
};
ElizaClient.prototype.runExperienceMaintenance = async function (options) {
	return {
		result: (
			await this.fetch("/api/character/experiences/maintenance", {
				method: "POST",
				body: JSON.stringify(options ?? {}),
			})
		).data,
...
ElizaClient.prototype.getExperience = async function (id) {
	return {
		experience: (
			await this.fetch(`/api/character/experiences/${encodeURIComponent(id)}`)
		).data,
	};
};
...
ElizaClient.prototype.startAppBlock = async (options) => {
	const plugin = getNativeAppBlockerPluginIfAvailable();
...
ElizaClient.prototype.spawnShellSession = async function (workdir) {
	return {
		sessionId: (
			await this.fetch("/api/coding-agents/spawn", {
				method: "POST",
				body: JSON.stringify({
					agentType: "shell",
					...(workdir ? { workdir } : {}),
...
ElizaClient.prototype.sendChatRest = async function (
	...
	const sendToConversation = async (conversationId) =>
...
ElizaClient.prototype.sendChatStream = async function (
	...
	const streamConversation = async (conversationId) =>
...
ElizaClient.prototype.getConversationMessages = async function (id) {
	return {
		messages: (
			await this.fetch(`/api/conversations/${encodeURIComponent(id)}/messages`)
...
ElizaClient.prototype.getInboxMessages = async function (options) {
	...
	if (typeof options?.roomSource === "string" && options.roomSource.length > 0)
...
ElizaClient.prototype.exportTrajectories = async function (options) {
	return (
		await this.rawRequest("/api/trajectories/export", {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify(options),
		})
...
function resolveDirectCloudWebBase(cloudBase) {
	...
	try {
		if (
			new URL(normalized).hostname.toLowerCase() === DIRECT_ELIZA_CLOUD_API_HOST
		)
...
function isDirectCloudBase(client) {
	...
	if (
		originsMatch(baseUrl, DEFAULT_DIRECT_CLOUD_API_BASE_URL) ||
		originsMatch(baseUrl, DEFAULT_DIRECT_CLOUD_BASE_URL)
	)
		return true;
	try {
...
function toCloudCompatJob(input) {
	...
	const createdAt =
		stringOrNull(input.createdAt) ?? /* @__PURE__ */ new Date(0).toISOString();
	...
	async function (agentId, request = {}) {
		try {
			...
			return {
				...
				data: {
					authorizeUrl: (
						await this.initiateCloudOauth("github", {
...
ElizaClient.prototype.getSandboxScreenshot = async function (region) {
	if (!region)
		return this.fetch("/api/sandbox/screen/screenshot", { method: "POST" });
...
ElizaClient.prototype.cloudLoginDirect = async (cloudApiBase) => {
...
ElizaClient.prototype.cloudLoginPollDirect = async (
	cloudApiBase,
	sessionId,
) => {
	const authApiBase = resolveDirectCloudAuthApiBase(cloudApiBase);
...
ElizaClient.prototype.sendIMessage = async function (request) {
	const attachmentPaths =
		request.attachmentPaths ?? (request.mediaUrl ? [request.mediaUrl] : void 0);
	const body = {
		to: request.to,
		text: request.text,
...
ElizaClient.prototype.deactivateN8nWorkflow = async function (id) {
	return this.fetch(`/api/n8n/workflows/${encodeURIComponent(id)}/deactivate`, {
		method: "POST",
	});
};
...
ElizaClient.prototype.createCustomAction = async function (action) {
	return (
		await this.fetch("/api/custom-actions", {
			method: "POST",
			body: JSON.stringify(action),
		})
	).action;
...
export {
	ApiError as y,
	AppContext as t,
	clearElizaApiBase as T,
	clearElizaApiToken as E,
	client as d,
	computeStreamingDelta as C,
	DEFAULT_BOOT_CONFIG as M,
	DEFAULT_WALLET_RPC_SELECTIONS$1 as l,
	ElizaClient as v,
	formatByteSize as r,
	formatDateTime as i,
	formatDurationMs as a,
	formatShortDate as o,
	formatTime as s,
	formatUptime as c,
	getBootConfig as N,
	getElizaApiBase as D,
	getElizaApiToken as O,
	isApiError as b,
	mapPtySessionsToCodingAgentSessions as x,
	mapServerTasksToSessions as _,
	mapTaskThreadsToCodingAgentSessions as S,
	mergeStreamingText as w,
	normalizeWalletRpcProviderId as f,
	normalizeWalletRpcSelections$1 as p,
	PULSE_STATUSES as m,
	resolveCharacterCatalog as P,
	STATUS_DOT as h,
	setBootConfig as F,
	setElizaApiBase as k,
	setElizaApiToken as A,
	stripAssistantStageDirections as j,
	syncBrandEnvToEliza as I,
	syncElizaEnvToBrand as L,
	TERMINAL_STATUSES as g,
	useApp as n,
	WALLET_RPC_PROVIDER_OPTIONS$1 as u,
};
