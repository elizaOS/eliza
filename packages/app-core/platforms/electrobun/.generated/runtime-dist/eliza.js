...
import {
  applyCloudConfigToEnv as applyCloudConfigToEnv$1,
  applyN8nConfigToEnv as applyN8nConfigToEnv$1,
  bootElizaRuntime as bootElizaRuntime$1,
  configureLocalEmbeddingPlugin,
  shutdownRuntime as shutdownRuntime$1,
  startEliza as startEliza$1,
...
import {
  CHANNEL_PLUGIN_MAP as CHANNEL_PLUGIN_MAP$1,
  collectPluginNames as collectPluginNames$1,
 ...
 */
const EMBEDDING_PRESETS$1 = {
  ...EMBEDDING_PRESETS,
  performance: {
    ...EMBEDDING_PRESETS.performance,
    label: "Efficient (compact text embedding)",
    description:
      "384-dim compact text-embedding model (~133MB). Powers memory / knowledge vectors only — not chat. The framework keeps the default SQL-safe and fast instead of auto-selecting a multi-GB embedding GGUF.",
  },
};
...
function findExistingEmbeddingModelForWarmupReuse(modelsDir) {
  ...
  for (const c of warmupReuseEmbeddingCandidates())
    if (embeddingGgufFilePresent(dir, c.model)) return c;
  return null;
}
    ...
    const request = (reqUrl) => {
      ...
      try {
        ...
        reject(
          error instanceof Error
            ? error
            : /* @__PURE__ */ new Error("Invalid download URL"),
        );
        return;
      }
      ...
      const settleSuccess = () => {
        ...
        if (expectedBytes != null && bytesReceived !== expectedBytes) {
          settleError(
            /* @__PURE__ */ new Error(
              `${getLogPrefix()} Download failed: bytes received (${bytesReceived}) does not match Content-Length (${expectedBytes})`,
            ),
          );
          return;
        }
  ...
  await downloadFile(
    ...
    onProgress
      ? (downloaded, total) => {
          ...
          onProgress(
            "downloading",
            `${safeFilename} ${total ? Math.round((downloaded / total) * 100) : 0}% of ${totalStr}`,
          );
        }
      : void 0,
  );
      ...
      socket.on("message", (raw) => {
        ...
        if (!registered) {
          ...
          if (
            ...
            logger.warn(
              "[device-bridge] Rejecting register: bad pairing token",
            );
            socket.close(4001, "unauthorized");
            return;
          }
...
init_types$1();
const ROUTER_PROVIDER = "eliza-router";
/**
...
function filterUnavailableLocalInferenceCandidates(
  candidates,
...
async function filterUnavailableLocalInference(
  slot,
  policy,
  preferredProvider,
  candidates,
...
var tslib_es6_exports = /* @__PURE__ */ __exportAll({
  ...
  __read: () => __read,
  __rest: () => __rest,
  __rewriteRelativeImportExtension: () => __rewriteRelativeImportExtension$1,
...
function __exportStar(m, o) {
  for (var p in m)
    if (p !== "default" && !Object.hasOwn(o, p)) __createBinding(o, m, p);
}
function __values(o) {
...
function __rewriteRelativeImportExtension$1(path, preserveJsx) {
...
var require_index_cjs = /* @__PURE__ */ __commonJSMin((exports) => {
      ...
      const createPluginMethod = (impl, prop) => {
        ...
        if (pluginHeader) {
          ...
          if (methodHeader)
            ...
            return (_a = impl[prop]) === null || _a === void 0
              ? void 0
              : _a.bind(impl);
        } else if (impl)
          return (_b = impl[prop]) === null || _b === void 0
            ? void 0
            : _b.bind(impl);
        else
  ...
  var WebPlugin = class {
    ...
    hasListeners(eventName) {
      var _a;
      return !!((_a = this.listeners[eventName]) === null || _a === void 0
        ? void 0
        : _a.length);
    }
    ...
    async setCookie(options) {
      try {
        ...
        const expires = options.expires
          ? `; expires=${options.expires.replace("expires=", "")}`
          : "";
...
var require_plugin_cjs = /* @__PURE__ */ __commonJSMin((exports) => {
  ...
  (_c =
    (_b =
      (_a =
        LlamaCpp === null || LlamaCpp === void 0
          ? void 0
          : LlamaCpp.toggleNativeLog) === null || _a === void 0
        ? void 0
        : _a.call(LlamaCpp, { enabled: false })) === null || _b === void 0
      ? void 0
      : _b.catch) === null ||
    _c === void 0 ||
    _c.call(_b, () => {});
  ...
  const getJsonSchema = (responseFormat) => {
    var _a;
    if (
      (responseFormat === null || responseFormat === void 0
        ? void 0
        : responseFormat.type) === "json_schema"
    )
      return (_a = responseFormat.json_schema) === null || _a === void 0
        ? void 0
        : _a.schema;
    if (
      (responseFormat === null || responseFormat === void 0
  ...
  const jsonSchemaToGrammar = async (schema) => {
    try {
      return await LlamaCpp.convertJsonSchemaToGrammar({
        schema: JSON.stringify(schema),
      });
    } catch (error) {
      console.warn(
        "Failed to convert JSON schema to GBNF, using fallback:",
        error,
      );
...
ws ::= [ \\t\\n]*`;
  ...
  var LlamaContext = class {
    ...
    isJinjaSupported() {
      ...
      return (
        !!(minja === null || minja === void 0 ? void 0 : minja.toolUse) ||
        !!(minja === null || minja === void 0 ? void 0 : minja.default)
      );
    }
    async getFormattedChat(messages, template, params) {
        ...
        if (Array.isArray(msg.content)) {
          const content = msg.content.map((part) => {
            ...
            if (part.type === "image_url") {
              let path =
                ((_a = part.image_url) === null || _a === void 0
                  ? void 0
                  : _a.url) || "";
              if (
      ...
      const useJinja =
        this.isJinjaSupported() &&
        (params === null || params === void 0 ? void 0 : params.jinja);
      let tmpl;
      if (template) tmpl = template;
      const jsonSchema = getJsonSchema(
        params === null || params === void 0 ? void 0 : params.response_format,
      );
      const result = await LlamaCpp.getFormattedChat({
        ...
        params: {
          ...
          parallel_tool_calls: (
            ...
            : void 0,
          tool_choice:
            params === null || params === void 0 ? void 0 : params.tool_choice,
          enable_thinking:
            (_a =
              params === null || params === void 0
                ? void 0
                : params.enable_thinking) !== null && _a !== void 0
              ? _a
              : true,
          add_generation_prompt:
            params === null || params === void 0
              ? void 0
              : params.add_generation_prompt,
          now:
      ...
      if (params.grammar) nativeParams.grammar = params.grammar;
        ...
        if (jsonSchema)
          try {
            ...
            console.warn(
              "Failed to convert JSON schema to grammar, falling back to json_schema parameter:",
              error,
            );
  ...
  async function initLlama(_a, onProgress) {
    ...
    let loraPath = lora;
    if (
      loraPath === null || loraPath === void 0
        ? void 0
        : loraPath.startsWith("file://")
    )
    ...
    if (rest.cache_type_k && !validCacheTypes.includes(rest.cache_type_k)) {
      console.warn(
        `[LlamaCpp] initLlama: Invalid cache K type: ${rest.cache_type_k}, falling back to f16`,
      );
      delete rest.cache_type_k;
    }
    if (rest.cache_type_v && !validCacheTypes.includes(rest.cache_type_v)) {
      console.warn(
        `[LlamaCpp] initLlama: Invalid cache V type: ${rest.cache_type_v}, falling back to f16`,
      );
  ...
  exports.LLAMACPP_MTMD_DEFAULT_MEDIA_MARKER =
    LLAMACPP_MTMD_DEFAULT_MEDIA_MARKER;
...
function isCapacitorNative() {
  ...
  return Boolean(
    (_a = cap === null || cap === void 0 ? void 0 : cap.isNativePlatform) ===
      null || _a === void 0
      ? void 0
      : _a.call(cap),
  );
...
function detectPlatform() {
  ...
  const cap = globalThis.Capacitor;
  const platform =
    (_a = cap === null || cap === void 0 ? void 0 : cap.getPlatform) === null ||
    _a === void 0
      ? void 0
      : _a.call(cap);
  ...
  CapacitorLlamaAdapter = class {
    ...
    async loadPlugin() {
      ...
      this.pluginLoadPromise = (async () => {
        const plugin = resolveLlamaCppPlugin(
          await Promise.resolve().then(() =>
            /* @__PURE__ */ __toESM(require_plugin_cjs()),
          ),
        );
        if (!plugin)
          throw new Error(
        ...
        const tokenListenerHandle = await plugin.addListener(
          ...
          (data) => {
            ...
            const token =
              (_b =
                (_a = data.tokenResult) === null || _a === void 0
                  ? void 0
                  : _a.token) !== null && _b !== void 0
                ? _b
                : data.token;
            if (!token) return;
            this.tokenIndex += 1;
        ...
        this.pluginListenerHandle =
          tokenListenerHandle !== null && tokenListenerHandle !== void 0
            ? tokenListenerHandle
            : null;
        this.plugin = plugin;
        return plugin;
      ...
      return {
        ...
        cpuCores:
          (_a =
            nav === null || nav === void 0
              ? void 0
              : nav.hardwareConcurrency) !== null && _a !== void 0
            ? _a
            : 0,
        gpu: null,
    ...
    async load(options) {
      ...
      await plugin.initContext({
        ...
        params: {
          ...
          n_ctx:
            (_a = options.contextSize) !== null && _a !== void 0 ? _a : 4096,
          n_gpu_layers: options.useGpu === false ? 0 : 99,
          n_threads:
            (_b = options.maxThreads) !== null && _b !== void 0 ? _b : 0,
          use_mmap: true,
        },
      });
    ...
    async generate(options) {
      ...
      const params = {
        n_predict:
          (_a = options.maxTokens) !== null && _a !== void 0 ? _a : 2048,
        temperature:
          (_b = options.temperature) !== null && _b !== void 0 ? _b : 0.7,
      ...
      const duration =
        ((_d = result.timings) === null || _d === void 0
          ? void 0
          : _d.predicted_ms) != null
    ...
    async embed(options) {
      ...
      const params = {
        embd_normalize:
          (_a = options.embdNormalize) !== null && _a !== void 0 ? _a : 0,
      };
...
var init_device_bridge_client = __esmMin(() => {
  ...
  DeviceBridgeClient = class {
    ...
    connect() {
      ...
      (_b = (_a = this.config).onStateChange) === null ||
        _b === void 0 ||
        _b.call(_a, "connecting");
      const url = this.buildUrl();
      let ws;
      try {
        ws = new WebSocket(url);
      } catch (err) {
        (_d = (_c = this.config).onStateChange) === null ||
          _d === void 0 ||
          _d.call(
            _c,
            "error",
      ...
      ws.onerror = () => {
        ...
        (_b = (_a = this.config).onStateChange) === null ||
          _b === void 0 ||
          _b.call(_a, "error", "websocket error");
      };
      ws.onclose = () => {
        var _a, _b;
        this.socket = null;
...
const DEVICE_BRIDGE_PROVIDER$1 = "eliza-device-bridge";
const CAPACITOR_LLAMA_PROVIDER$1 = "capacitor-llama";
...
async function ensureAssignedModelLoaded(loader, slot) {
  ...
  if (!loader && localInferenceEngine.currentModelPath()) {
    if (
      (await listInstalledModels()).find(
  ...
  if (loader) {
    ...
    if (currentPath) {
      if (
        (await listInstalledModels()).find((m) => m.path === currentPath)
          ?.id === assignedId
      )
        return;
    }
...
async function ensureLocalInferenceHandler(runtime) {
  ...
  const provider = aospRegistered
    ? AOSP_LLAMA_PROVIDER
    : capacitorRegistered
      ? CAPACITOR_LLAMA_PROVIDER$1
      : deviceBridgeEnabled
        ? DEVICE_BRIDGE_PROVIDER$1
 ...
 */
var n8n_auth_bridge_exports = /* @__PURE__ */ __exportAll({
  startN8nAuthBridge: () => startN8nAuthBridge,
});
...
function startN8nAuthBridge(runtime, config, options = {}) {
  ...
  const handleTransition = (next) => {
    ...
    if (prev === false && next === true) {
      ...
      if (sidecar && (status === "starting" || status === "ready")) {
        ...
        disposeN8nSidecar().catch((err) => {
          logger.warn(
            `[n8n-auth-bridge] disposeN8nSidecar failed: ${err instanceof Error ? err.message : String(err)}`,
          );
        });
      }
      return;
    ...
    if (prev === true && next === false) {
      ...
      getN8nSidecar(resolveSidecarConfig$1(cfg))
        .start()
        .catch((err) => {
          logger.warn(
            `[n8n-auth-bridge] sidecar.start failed: ${err instanceof Error ? err.message : String(err)}`,
          );
        });
    }
  };
 ...
 */
var n8n_autostart_exports = /* @__PURE__ */ __exportAll({
  startN8nAutoStart: () => startN8nAutoStart,
});
  ...
  const evaluate = async () => {
    ...
    try {
      (await getSidecar(resolveSidecarConfig(cfg))).start().catch((err) => {
        logger.warn(
          `[n8n-autostart] boot start failed: ${err instanceof Error ? err.message : String(err)}`,
        );
      });
    } catch (err) {
      logger.warn(
 ...
 */
var n8n_dispatch_exports = /* @__PURE__ */ __exportAll({
  createN8nDispatchService: () => createN8nDispatchService,
});
...
async function defaultGetLocalOwnerCookie(host, config) {
  ...
  try {
    ...
    logger.warn(
      `[n8n-dispatch] failed to read local n8n owner credentials: ${error instanceof Error ? error.message : String(error)}`,
    );
    return null;
  }
  ...
  const response = await fetch(`${host.replace(/\/+$/, "")}/rest/login`, {
    ...
    logger.warn(
      `[n8n-dispatch] local n8n owner login failed: ${error instanceof Error ? error.message : String(error)}`,
    );
    return null;
  });
  ...
  const execute = async (workflowId, payload = {}) => {
    ...
    if (mode === "cloud") {
      ...
      const workflow = extractWorkflowBody(
        await readJsonBody$2(workflowResponse),
      );
      if (!workflow)
        return {
          ok: false,
  ...
  const getCachedTriggers = async () => {
    ...
    for (const task of tasks) {
      ...
      if (
        ...
        newEventTypes.add(trigger.eventKind);
    }
...
var DEFAULT_MIN_INTERVAL_MS,
  TRIGGER_CACHE_TTL_MS,
  PASSIVE_CONNECTOR_SOURCES,
  EXPOSED_EVENTS;
...
function startElizaN8nRuntimeContextProvider(runtime, options) {
  ...
  const fetchDiscordFacts = async (botToken) => {
    return formatDiscordEnumerationAsFacts(
      await fetchDiscordEnumeration(botToken, {
        fetchImpl,
        now,
        cache: discordCache,
  ...
  const getRuntimeContext = async (input) => {
    ...
    if (
      input.relevantNodes.some((n) => n.name.startsWith("n8n-nodes-base.gmail"))
    ) {
...
var connector_target_catalog_exports = /* @__PURE__ */ __exportAll({
  createElizaConnectorTargetCatalog: () => createElizaConnectorTargetCatalog,
});
...
var CLOUD_HEALTH_CACHE_TTL_MS,
  CLOUD_HEALTH_PROBE_TIMEOUT_MS,
  cloudHealthCache,
  DEFAULT_CLOUD_API_BASE_URL$1;
...
var WORKFLOW_DRAFT_TITLE,
  SYSTEM_TASK_NAMES,
  BLOCKED_AUTOMATION_PROVIDER_NODES,
...
var init_automations_compat_routes = __esmMin(() => {
  ...
  BLOCKED_AUTOMATION_PROVIDER_NODES = new Set([
...
var OPENAI_STYLE_VOICE_ALIASES,
  DEFAULT_ELIZA_CLOUD_TTS_VOICE_ID,
  DEFAULT_ELIZA_CLOUD_TTS_MODEL_ID,
  ELIZA_CLOUD_TTS_MAX_TEXT_CHARS,
  cachedCloudBaseUrlFromConfig,
  hasResolvedCloudBaseUrlFromConfig;
...
function resolveCompatWalletExportRejection(...args) {
  ...
  return runWithCompatAuthContext(req, () =>
    normalizeCompatRejection(resolveWalletExportRejection(...args)),
  );
}
/**
...
function resolveMcpTerminalAuthorizationRejection$1(...args) {
  const [req] = args;
  return runWithCompatAuthContext(req, () =>
    normalizeCompatRejection(resolveMcpTerminalAuthorizationRejection(...args)),
  );
}
function resolveTerminalRunRejection$1(...args) {
  const [req] = args;
  return runWithCompatAuthContext(req, () =>
    normalizeCompatRejection(resolveTerminalRunRejection(...args)),
  );
}
function resolveWebSocketUpgradeRejection$1(...args) {
  const [req] = args;
  return runWithCompatAuthContext(req, () =>
    resolveWebSocketUpgradeRejection(...args),
  );
}
function resolveTerminalRunClientId$1(...args) {
  const [req] = args;
  return runWithCompatAuthContext(req, () =>
    resolveTerminalRunClientId(...args),
  );
}
...
var init_server_startup = __esmMin(() => {
  init_env();
  PACKAGE_ROOT_NAMES = new Set(["eliza", "elizaai", "elizaos", "eliza"]);
});
...
var SENSITIVE_RATE_LIMIT_WINDOW_MS,
...
var DEFAULT_RATE_LIMIT_BACKOFF_MS,
  QUOTA_AWARE_SKIP_PCT,
  SESSION_AFFINITY_MAX_ATTEMPTS,
  AccountPool,
  ANTHROPIC_POOL_SHIM_SYMBOL,
...
var init_account_pool = __esmMin(() => {
  ...
  ORCHESTRATOR_POOL_SHIM_SYMBOL = Symbol.for(
    "eliza.account-pool.orchestrator.v1",
  );
  SUBSCRIPTION_SELECTOR_SHIM_SYMBOL = Symbol.for(
...
var DEFAULT_ELEVENLABS_TTS_MODEL,
  ELEVENLABS_VOICE_ID_BY_PRESET,
...
var init_server_onboarding_compat = __esmMin(() => {
  ...
  ELEVENLABS_VOICE_ID_BY_PRESET = new Map(
...
var DEFAULT_CLOUD_API_BASE_URL,
  CLOUD_BILLING_URL,
  CLOUD_ENV_KEYS,
...
var CLOUD_LOGIN_POLL_TIMEOUT_MS,
  DEFAULT_CLOUD_ROUTE_SERVICES,
  cloudDisconnectEpoch;
...
var init_providers = __esmMin(() => {
  ...
  OPENAI_CODEX_PROVIDER = {
    ...
    kind: "cloud-subscription",
    description:
      "Codex and ChatGPT subscription access through linked accounts.",
    supportedSlots: [
      "TEXT_SMALL",
      "TEXT_LARGE",
      "OBJECT_SMALL",
...
var init_downloader = __esmMin(() => {
  ...
  Downloader = class {
    ...
    async start(modelIdOrSpec) {
      ...
      if (
        existing &&
        (existing.job.state === "queued" ||
          existing.job.state === "downloading")
      )
        return { ...existing.job };
      await ensureDirs();
...
var init_service = __esmMin(() => {
  ...
  LocalInferenceService = class {
    ...
    async setActive(runtime, modelId) {
      const installed = (await this.getInstalled()).find(
        (m) => m.id === modelId,
      );
...
var init_secrets_manager_routes = __esmMin(() => {
  ...
  init_response();
  _manager = null;
  INSTALLABLE_BACKENDS = ["1password", "bitwarden", "protonpass"];
...
var MARKET_OVERVIEW_PATH,
  CLOUD_MARKET_OVERVIEW_PREVIEW_PATH,
  MARKET_OVERVIEW_CACHE_TTL_MS,
  MARKET_OVERVIEW_FETCH_TIMEOUT_MS,
  MARKET_OVERVIEW_REFRESH_WINDOW_MS,
  MARKET_OVERVIEW_REFRESH_LIMIT,
  ...
  cachedWalletMarketOverview,
  walletMarketOverviewInFlight,
  walletMarketRefreshBuckets,
...
var init_wallet_market_overview_route = __esmMin(() => {
  ...
  MARKET_PRICE_IDS = ["bitcoin", "ethereum", "solana"];
...
var execFileAsync,
  MacOSKeychainPlatformSecureStore,
  LinuxSecretToolPlatformSecureStore,
  NonePlatformSecureStore;
...
var server_exports = /* @__PURE__ */ __exportAll({
  ...
  resolveMcpTerminalAuthorizationRejection: () =>
    resolveMcpTerminalAuthorizationRejection$1,
  resolvePluginConfigMutationRejections: () =>
    resolvePluginConfigMutationRejections,
...
var __rewriteRelativeImportExtension =
  (void 0 && (void 0).__rewriteRelativeImportExtension) ||
  ((path, preserveJsx) => {
    if (typeof path === "string" && /^\.\.?\//.test(path))
      return path.replace(
        /\.(tsx)$|((?:\.d)?)((?:\.[^./]+?)?)\.([cm]?)ts$/i,
...
const shutdownRuntime = shutdownRuntime$1;
...
function collectPluginNames(...args) {
  syncBrandEnvAliases();
  const [config] = args;
  const result = collectPluginNames$1(...args);
  if (
...
function applyCloudConfigToEnv(...args) {
  syncBrandEnvAliases();
  const result = applyCloudConfigToEnv$1(...args);
  syncBrandEnvAliases();
  return result;
}
function applyN8nConfigToEnv(...args) {
  syncBrandEnvAliases();
  if (isNativeServerPlatform() || isMobilePlatform()) {
    const [config, agentId] = args;
    const result = applyN8nConfigToEnv$1(
  ...
  const result = applyN8nConfigToEnv$1(...args);
  syncBrandEnvAliases();
  return result;
}
async function ensureAutonomyBootstrapContext(runtime) {
  ...
  if (!existingEntity) {
    if (
      !(await runtimeWithCompat.createEntity?.(autonomyEntity)) &&
      adapter?.upsertEntities
    )
...
async function registerTrainingRuntimeHooks(runtime) {
  ...
  try {
    hookMod = await import(
      __rewriteRelativeImportExtension(TRAINING_RUNTIME_HOOKS_SPECIFIER)
    );
  } catch (err) {
    logger.warn(
...
let _n8nAuthBridge = null;
let _n8nAutoStart = null;
let _n8nDispatch = null;
let _triggerEventBridge = null;
let _n8nRuntimeContextProvider = null;
let _discordEnumerationCache = null;
let _connectorTargetCatalog = null;
  ...
  try {
    const [{ startN8nAuthBridge }, config] = await Promise.all([
      Promise.resolve().then(
        () => (init_n8n_auth_bridge(), n8n_auth_bridge_exports),
      ),
    ...
    logger.warn(
      `[eliza] Failed to start n8n auth bridge: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
async function ensureN8nAutoStart(runtime) {
  ...
  try {
    const [{ startN8nAutoStart }, config] = await Promise.all([
      Promise.resolve().then(
        () => (init_n8n_autostart(), n8n_autostart_exports),
      ),
      Promise.resolve(loadElizaConfig()),
    ]);
    ...
    logger.warn(
      `[eliza] Failed to start n8n autostart: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
async function ensureN8nDispatchService(runtime) {
  ...
  try {
    const { createN8nDispatchService } = await Promise.resolve().then(
      () => (init_n8n_dispatch(), n8n_dispatch_exports),
    );
    ...
    logger.warn(
      `[eliza] Failed to register n8n dispatch service: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
async function ensureTriggerEventBridge(runtime) {
  ...
  try {
    const { startTriggerEventBridge } = await Promise.resolve().then(
      () => (init_trigger_event_bridge(), trigger_event_bridge_exports),
    );
    _triggerEventBridge = startTriggerEventBridge(runtime);
    logger.info("[eliza] trigger event bridge armed");
  } catch (err) {
    logger.warn(
      `[eliza] Failed to start trigger event bridge: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
async function ensureN8nRuntimeContextProvider(runtime) {
  ...
  const { createDiscordSourceCache } = await Promise.resolve().then(
    () => (init_discord_target_source(), discord_target_source_exports),
  );
  _discordEnumerationCache = createDiscordSourceCache();
  try {
    const { startElizaN8nRuntimeContextProvider } =
      await Promise.resolve().then(
        () => (
          init_n8n_runtime_context_provider(),
          n8n_runtime_context_provider_exports
        ),
      );
    const credProviderInstance = (runtime.services.get(
      "n8n_credential_provider",
    ) ?? [])[0];
    _n8nRuntimeContextProvider = startElizaN8nRuntimeContextProvider(runtime, {
      getConfig: () => loadElizaConfig(),
      credProvider:
        credProviderInstance &&
        typeof credProviderInstance.resolve === "function"
          ? credProviderInstance
          : void 0,
    ...
    logger.warn(
      `[eliza] Failed to register n8n runtime-context provider: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
async function ensureConnectorTargetCatalog(runtime) {
  ...
  try {
    const { createElizaConnectorTargetCatalog } = await Promise.resolve().then(
      () => (init_connector_target_catalog(), connector_target_catalog_exports),
    );
    ...
    logger.warn(
      `[eliza] Failed to register connector-target-catalog: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
async function ensureTelegramBotPolling(runtime) {
  ...
  try {
    ...
    const bot = new Telegraf(botToken, {
      telegram: {
        apiRoot: process$1.env.TELEGRAM_API_ROOT || "https://api.telegram.org",
      },
    });
...
async function warmupEmbeddingModel(onProgress) {
  ...
  let modelRepo =
    process$1.env.LOCAL_EMBEDDING_MODEL_REPO?.trim() || preset.modelRepo;
  if (
    ...
    if (reuse) {
      logger.info(
        `[eliza] Embedding warmup: configured file "${model}" not found in MODELS_DIR — reusing existing ${reuse.model} to avoid a large re-download. Set LOCAL_EMBEDDING_MODEL or ELIZA_EMBEDDING_WARMUP_NO_REUSE=1 to force the configured model.`,
      );
      ...
      process$1.env.LOCAL_EMBEDDING_USE_MMAP =
  ...
  logger.info(
    `[eliza] Local embedding warmup: ${model} (hardware tier preset: ${preset.label}). This file is for TEXT_EMBEDDING / memory only (not your conversation model).`,
  );
...
async function bootElizaRuntime(opts = {}) {
  ...
  try {
    ...
    if (!process$1.env.EMBEDDING_DIMENSION)
      process$1.env.EMBEDDING_DIMENSION = "384";
    const runtime = await bootElizaRuntime$1(opts);
...
async function upstreamStartElizaWithPgliteCompat(options) {
  try {
    return await startEliza$1(options);
  } catch (err) {
...
async function startEliza(options) {
  ...
  try {
    ...
    if (options?.serverOnly) {
      ...
      const { startApiServer } = await Promise.resolve().then(
        () => (init_server(), server_exports),
      );
      const { port: actualApiPort } = await startApiServer({
        ...
        onRestart: async () => {
          ...
          await shutdownRuntime$1(currentRuntime, "server-only restart");
          const restarted =
            (await upstreamStartElizaWithPgliteCompat({
              ...options,
              headless: true,
      ...
      syncResolvedApiPort(process$1.env, actualApiPort, {
        overwriteUiPort: true,
      });
      try {
        const { invalidateCorsAllowedPorts } = await Promise.resolve().then(
          () => (init_server_cors(), server_cors_exports),
        );
      ...
      const cleanup = async () => {
        ...
        if (currentRuntime)
          await shutdownRuntime$1(currentRuntime, "server-only shutdown");
        ...
        try {
          const { disposeN8nSidecar } = await Promise.resolve().then(
            () => (init_n8n_sidecar(), n8n_sidecar_exports),
          );
