...
import { AGENT_EVENT_ALLOWED_STREAMS, CONFIG_WRITE_ALLOWED_TOP_KEYS, CONNECTOR_ENV_MAP, EMBEDDING_PRESETS, applyCanonicalOnboardingConfig, applyOnboardingCredentialPersistence, applyPluginRuntimeMutation, buildCharacterFromConfig, clearPersistedOnboardingConfig, cloneWithoutBlockedObjectKeys, createIntegrationTelemetrySpan, detectEmbeddingTier, discoverInstalledPlugins, discoverPluginsFromManifest, discoverPluginsFromManifest as discoverPluginsFromManifest$1, ensureApiTokenForBindHost, extractAuthToken, fetchWithTimeoutGuard, fetchWithTimeoutGuard as fetchWithTimeoutGuard$1, findPrimaryEnvKey, handleCloudBillingRoute, handleCloudCompatRoute, handleCloudRoute, initStewardWalletCache, injectApiBaseIntoHtml, isAdvancedCapabilityPluginId, isAllowedHost, isAuthorized, isSafeResetStateDir, loadElizaConfig, normalizeCloudSiteUrl, normalizeWsClientId, persistConversationRoomTitle, readBundledPluginPackageMetadata, resolveAdvancedCapabilitiesEnabled, resolveAppHeroImage, resolveCloudApiBaseUrl, resolveCorsOrigin, resolveDefaultAgentWorkspaceDir, resolveMcpServersRejection, resolveMcpTerminalAuthorizationRejection, resolvePluginConfigMutationRejections, resolveTerminalRunClientId, resolveTerminalRunRejection, resolveUserPath, resolveWalletExportRejection, resolveWebSocketUpgradeRejection, routeAutonomyTextToUser, saveElizaConfig, startApiServer, streamResponseBodyWithByteLimit, validateCloudBaseUrl, validateMcpServerConfig } from "@elizaos/agent";
import { applyCloudConfigToEnv as applyCloudConfigToEnv$1, applyN8nConfigToEnv as applyN8nConfigToEnv$1, bootElizaRuntime as bootElizaRuntime$1, configureLocalEmbeddingPlugin, shutdownRuntime as shutdownRuntime$1, startEliza as startEliza$1 } from "@elizaos/agent/runtime/eliza";
import { CHANNEL_PLUGIN_MAP as CHANNEL_PLUGIN_MAP$1, collectPluginNames as collectPluginNames$1 } from "@elizaos/agent/runtime/plugin-collector";
...
import { AgentRuntime, AutonomyService, ChannelType, EventType, ModelType, lifeOpsPassiveConnectorsEnabled, logger, stringToUuid } from "@elizaos/core";
import { asRecord, deriveOnboardingCredentialPersistencePlan, getDefaultStylePreset, getStylePresets, isCloudInferenceSelectedInConfig, isElizaCloudServiceSelectedInConfig, isElizaSettingsDebugEnabled, isLoopbackBindHost, isMobilePlatform, migrateLegacyRuntimeConfig, normalizeCharacterLanguage, normalizeDeploymentTargetConfig, normalizeLinkedAccountFlagsConfig, normalizeLinkedAccountsConfig, normalizeOnboardingCredentialInputs, normalizeOnboardingProviderId, normalizeServiceRoutingConfig, resolveApiBindHost, resolveApiToken, resolveDeploymentTargetInConfig, resolveLinkedAccountsInConfig, resolveServerOnlyPort, resolveServiceRoutingInConfig, resolveStylePresetByAvatarIndex, resolveStylePresetById, resolveStylePresetByName, sanitizeForSettingsDebug, sanitizeSpeechText, settingsDebugCloudSummary, syncResolvedApiPort } from "@elizaos/shared";
import { z } from "zod";
...
const APP_ROUTE_PLUGIN_REGISTRY_KEY = Symbol.for("elizaos.app.route-plugin-registry");
...
const EMBEDDING_META_DIR = process.env.ELIZA_EMBEDDING_META_DIR ?? path.join(os.homedir(), ".eliza", "state");
const EMBEDDING_META_PATH = process.env.ELIZA_EMBEDDING_META_PATH ?? path.join(EMBEDDING_META_DIR, "embedding-meta.json");
let _logger;
function getLogger() {
...
init_sql_compat();
init_startup_overlay();
var __rewriteRelativeImportExtension = void 0 && (void 0).__rewriteRelativeImportExtension || function(path, preserveJsx) {
	if (typeof path === "string" && /^\.\.?\//.test(path)) return path.replace(/\.(tsx)$|((?:\.d)?)((?:\.[^./]+?)?)\.([cm]?)ts$/i, function(m, tsx, d, ext, cm) {
		return tsx ? preserveJsx ? ".jsx" : ".js" : d && (!ext || !cm) ? m : d + ext + "." + cm.toLowerCase() + "js";
	});
	return path;
...
const DIRECT_HELP_FLAGS = new Set([
	"-h",
	"--help",
	"help"
]);
const DIRECT_VERSION_FLAGS = new Set([
	"-v",
	"-V",
	"--version",
	"version"
]);
const PLUGIN_SQL_GLOBAL_SINGLETONS = Symbol.for("@elizaos/plugin-sql/global-singletons");
...
function applyN8nConfigToEnv(...args) {
	...
	if (isNativeServerPlatform() || isMobilePlatform()) {
		...
		const result = applyN8nConfigToEnv$1(config?.n8n?.localEnabled === false ? config : {
			...config,
			n8n: {
				...config.n8n ?? {},
				localEnabled: false
...
async function loadAppRoutePluginFromSpecifier(specifier, exportName) {
	return resolvePluginExport(await import(__rewriteRelativeImportExtension(
		/* webpackIgnore: true */
		specifier
	)), exportName);
}
...
const TRAINING_RUNTIME_HOOKS_SPECIFIER = "@elizaos/app-training/register-runtime";
	...
	try {
		...
		if (options?.serverOnly) {
			...
			const cleanup = async () => {
				...
				if (currentRuntime) await shutdownRuntime$1(currentRuntime, "server-only shutdown");
...
//#endregion
export { CHANNEL_PLUGIN_MAP, CUSTOM_PLUGINS_DIRNAME, applyCloudConfigToEnv, applyN8nConfigToEnv, attemptPgliteAutoReset, bootElizaRuntime, collectPluginNames, getPgliteRecoveryRetrySkipPlugins, resolvePackageEntry, scanDropInPlugins, shutdownRuntime, startEliza };