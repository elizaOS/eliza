...
const cloudHealthCache = /* @__PURE__ */ new Map();
...
const ELIZA_CLOUD_TTS_MAX_TEXT_CHARS = 5e3;
...
let cachedCloudBaseUrlFromConfig;
...
const isAllowedLocalOrigin = isAllowedOrigin;
...
function injectApiBaseIntoHtml(...args) {
	return injectApiBaseIntoHtml$1(...args);
}
...
function ensureApiTokenForBindHost(...args) {
	syncAppEnvToEliza();
	const result = ensureApiTokenForBindHost$1(...args);
	syncElizaEnvAliases();
	return result;
}
...
function resolveCorsOrigin(...args) {
	syncElizaEnvAliases();
	syncAppEnvToEliza();
	const result = resolveCorsOrigin$1(...args);
	syncAppEnvToEliza();
	syncElizaEnvAliases();
	return result;
}
...
init_compat_route_shared();
init_response();
const PAIRING_TTL_MS = 600 * 1e3;
const PAIRING_WINDOW_MS = 600 * 1e3;
...
const AUTH_ATTEMPT_WINDOW_MS = 6e4;
const AUTH_ATTEMPT_MAX = 20;
const sessionRouteAttempts = /* @__PURE__ */ new Map();
...
const DEFAULT_CLOUD_ROUTE_SERVICES = {
...
const localInferenceEngine = new LocalInferenceEngine();
...
const localInferenceService = new LocalInferenceService();
...
const DEFAULT_SIGNIN_TIMEOUT_MS = 6e4;
var SecretsManagerInstaller = class {
...
var LinuxSecretToolPlatformSecureStore = class {
...
var NonePlatformSecureStore = class {
	backend;
