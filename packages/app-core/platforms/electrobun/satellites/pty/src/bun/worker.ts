...
import { TerminalSatelliteService } from "./pty-service.ts";
import type {
	JsonValue,
	PtyCommandRunParams,
...
function parseRequest(value: unknown): PtyWorkerRequestMessage | null {
	...
	if (
		...
		throw new Error("Invalid Terminal Satellite request.");
	}
...
async function dispatch(
	...
	throw new Error(`Unsupported Terminal Satellite method: ${exhaustive}`);
}
...
function parseCreateSessionParams(params?: JsonValue): PtyCreateSessionParams {
	...
	if (!isRecord(params)) throw new Error("pty.session.create params must be an object.");
	return {
...
function parseResizeParams(params?: JsonValue): PtyResizeParams {
	if (!isRecord(params)) throw new Error("pty.session.resize params must be an object.");
	return {
		sessionId: stringParam(params, "sessionId"),
...
function parseKillParams(params?: JsonValue): PtyKillParams {
	if (!isRecord(params)) throw new Error("pty.session.kill params must be an object.");
	return {
		sessionId: stringParam(params, "sessionId"),
...
function parseOutputTailParams(params?: JsonValue): PtyOutputTailParams {
	if (!isRecord(params)) throw new Error("pty.session.output.tail params must be an object.");
	return {
		sessionId: stringParam(params, "sessionId"),
