...
Eliza Orbit is the runtime environment that coordinates Satellites. Satellites are separately installable desktop capabilities with stable IDs.

`eliza.runtime` is the Runtime Satellite. In this first-party Electrobun location it adapts to the existing desktop `AgentManager` for production runtime lifecycle calls, while keeping the standalone subprocess manager for isolated smoke tests. Phase 2 added a local API bridge so the Satellite can discover the existing runtime routes and call supported lifecycle, status, agent, conversation, plugin, memory, and config endpoints. Phase 3 added a streaming chat bridge that normalizes the current local SSE stream into stable worker events. Phase 5 added brokered filesystem capability methods that forward to `eliza.fs`. Phase 6 added brokered terminal methods that forward to `eliza.pty`. Phase 7 adds brokered Git methods that forward to `eliza.git`.

This package does not migrate the production dashboard, add Swift, add MLX, add Model Satellites, or rewrite elizaOS core.

## Satellite IDs

- `eliza.runtime` - Runtime Satellite
- `eliza.surface` - Surface Satellite
- `eliza.fs` - File Satellite
- `eliza.pty` - Terminal Satellite
- `eliza.git` - Git Satellite
- `eliza.local-model` - future Model Satellite

## Runtime Supervision

When installed as a bundled first-party Satellite inside `packages/app-core/platforms/electrobun`, `runtime.start`, `runtime.stop`, `runtime.restart`, `runtime.status`, `runtime.health`, and `runtime.logs.tail` call the host `AgentManager` through host requests. Production mode does not start a second elizaOS runtime process.
...
`ElizaRuntimeApiClient` uses the Runtime Satellite `apiBase` and probes the existing local API. Route discovery does not assume every route exists. GET routes are probed with GET. POST routes are listed but not marked available from generic OPTIONS preflight alone, because the current local server answers CORS preflight before route matching.
...
The Runtime Satellite stream bridge starts that route, parses SSE safely, and emits worker events. It also understands the existing OpenAI-compatible `/v1/chat/completions` and Anthropic-compatible `/v1/messages` SSE shapes as fallback parsers. It does not make non-streaming `agent.message` streaming.
...
If `eliza.fs` is unavailable, Runtime returns `CAPABILITY_UNAVAILABLE` with the message `File Satellite eliza.fs is not available`.
...
If `eliza.pty` is unavailable, Runtime returns `CAPABILITY_UNAVAILABLE` with the message `Terminal Satellite eliza.pty is not available`.
...
If `eliza.git` is unavailable, Runtime returns `CAPABILITY_UNAVAILABLE` with the message `Git Satellite eliza.git is not available`.
...
```sh
bun run --cwd elizalaunch/satellites/runtime build
```

Phase 1 smoke:

```sh
bun run --cwd elizalaunch/satellites/runtime smoke
bun run --cwd elizalaunch/satellites/runtime smoke:phase1
```

Phase 2 smoke:

```sh
bun run --cwd elizalaunch/satellites/runtime smoke:phase2
```

Phase 3 smoke:

```sh
bun run --cwd elizalaunch/satellites/runtime smoke:phase3
```
...
```sh
packages/app-core/platforms/electrobun/satellites/runtime
```
...
After install, start `eliza.runtime` from the existing module manager. If the current app UI still labels this surface with upstream terminology, that is host UI copy outside this package; this Satellite keeps Eliza-facing package naming to ElizaLaunch, Eliza Orbit, and Satellites.
...
Runtime brokers these methods to the Model Satellite and passes its current `apiBase` when the caller did not provide one:
