...
const streamSnapshot = document.querySelector<HTMLPreElement>("#stream-snapshot");
const activeStreamId = document.querySelector<HTMLElement>("#active-stream-id");
const buttons = Array.from(document.querySelectorAll<HTMLButtonElement>("button[data-method]"));
...
const messageTextInput = document.querySelector<HTMLInputElement>("#message-text");
const memoryQueryInput = document.querySelector<HTMLInputElement>("#memory-query");
...
function requestParams(method: RuntimeMethod): RuntimeWorkerRequestMessage["params"] {
...
function send(method: RuntimeMethod): void {
	const request: RuntimeWorkerRequestMessage = {
		...
		method,
		...(requestParams(method) === undefined ? {} : { params: requestParams(method) }),
	};
	append(`> ${method}`);
...
function handleStreamEvent(event: AgentMessageStreamEvent): void {
	...
	if (event.kind === "snapshot") {
		setSnapshot(event.text ?? JSON.stringify(event.raw ?? event.payload, null, 2));
		return;
	}
...
worker.addEventListener("message", (event: MessageEvent<RuntimeWorkerOutboundMessage>) => {
	const message = event.data;
	if (message.type === "ready") {
		append("Runtime Satellite worker ready.");
		return;
	}
	if (message.type === "event") {
		...
		append(`event ${message.name}: ${JSON.stringify(message.payload, null, 2)}`);
		return;
	}
	if (
