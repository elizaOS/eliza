...
import { GitSatelliteService } from "./git-service.ts";
import type {
	GitAddParams,
...
const service = new GitSatelliteService();
...
function parseRequest(value: unknown): GitWorkerRequestMessage | null {
	...
	if (
		...
		throw new Error("Invalid Git Satellite request.");
	}
...
async function dispatch(
	...
	throw new Error(`Unsupported Git Satellite method: ${exhaustive}`);
}
