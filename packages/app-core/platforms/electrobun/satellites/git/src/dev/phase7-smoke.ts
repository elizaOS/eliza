...
import { GitSatelliteService } from "../bun/git-service.ts";

const tempRoot = mkdtempSync(path.join(os.tmpdir(), "eliza-git-"));
const runner = new GitCommandRunner();
const service = new GitSatelliteService({ runner });
...
try {
	...
	writeFileSync(path.join(tempRoot, "hello.txt"), "hello from git satellite\n", "utf8");
	...
	writeFileSync(path.join(tempRoot, "hello.txt"), "updated git satellite\n", "utf8");
	const diff = await service.diff({ cwd: tempRoot, path: "hello.txt" });
	assert(diff.raw.includes("updated git satellite"), "diff works");

	const show = await service.show({ cwd: tempRoot, ref: "HEAD", path: "hello.txt" });
	assert(show.raw.includes("hello from git satellite"), "show works");
...
function defaultBranch(branches: Array<{ name: string; current: boolean }>): string {
