...
class MockRuntimeBridge implements RuntimeSatelliteBridge {
	...
	async invoke(
		...
		if (method === "agent.message.stream") {
		...
		if (method === "pty.session.kill") {
			return {
				...
				exitedAt: "2026-05-17T00:00:03.000Z",
			};
		}
		...
		if (method === "model.catalog.eliza1") {
			return [
				{
					id: "eliza-1-2b",
					displayName: "Eliza-1 2B",
		...
		if (method === "model.active") {
			return { modelId: null, status: "idle" };
		}
		if (method === "model.downloads") {
			return [];
		}
...
for (const eventName of [
	...
	client.on(eventName, (payload) => applyRuntimeEvent(state, eventName, payload));
}
...
assert(state.fileList.entries[0]?.name === "hello.txt", "file listing is stored");
state.fileText = await client.fsReadText({ path: "/tmp/eliza/hello.txt" });
assert(state.fileText.text === "hello file", "file text is stored");
state.fileSearch = await client.fsSearch({ path: "/tmp/eliza", query: "hello" });
...
const ptyTail = await client.ptyOutputTail({ sessionId: ptySession.session.id });
state.ptyOutput = ptyTail.entries;
state.ptyNextSequence = ptyTail.nextSequence;
assert(state.ptyOutput[0]?.data.includes("pty smoke"), "terminal output is stored");
...
const killedSession = await client.ptyKill({ sessionId: ptySession.session.id });
...
assert(state.modelHub.catalog[0]?.hfRepo === "elizaos/eliza-1", "model hub is stored");
...
bridge.emit("agent.message.stream.delta", streamEvent("delta", { delta: "Hi" }));
bridge.emit(
	"agent.message.stream.delta",
...
let tailedDelta = "";
const unsubscribeTail = tailClient.on("agent.message.stream.delta", (payload) => {
