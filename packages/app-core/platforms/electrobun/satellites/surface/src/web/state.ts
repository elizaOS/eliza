...
export function applyRuntimeEvent(
	...
	if (eventName === "runtime.statusChanged" && isRuntimeState(payload)) {
		state.runtimeState = payload;
		return;
	}

	if (eventName === "runtime.started" && isRuntimeState(payload)) {
		state.runtimeState = payload;
		return;
	}

	if (eventName === "runtime.stopped" && isRuntimeState(payload)) {
		state.runtimeState = payload;
		return;
	}
	...
	if (eventName === "runtime.error") {
		addError(state, "Runtime Satellite error.", payload);
		return;
	}
	...
	if (eventName === "agent.message.stream.done") {
		...
		if (state.activeStreamId === payload.streamId) state.activeStreamId = null;
		return;
	}
	...
	if (eventName === "agent.message.stream.error") {
