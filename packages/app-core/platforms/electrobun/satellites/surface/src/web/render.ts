	...
	element.replaceChildren(
		...routes.map((route) => {
			...
			row.append(
				...
				cell(route.name),
				cell(route.path),
				cell(route.available ? `available ${route.status ?? ""}` : route.error ?? "unavailable"),
			);
			return row;
		}),
	);
...
function renderTimeline(
	...
	const latest = state.actionTimeline.find((event) => event.kind === "snapshot");
...
function renderFilePanel(
	...
	elements.fileRoots.textContent =
		state.fileRoots.length === 0
			? "No roots loaded."
			: state.fileRoots
					.map((root) => `${root.id}: ${root.path}${root.label ? ` (${root.label})` : ""}`)
					.join("\n");
