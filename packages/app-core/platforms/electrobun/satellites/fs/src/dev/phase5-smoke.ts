...
import { FileSatelliteException } from "../bun/errors.ts";
import { FileSatelliteService } from "../bun/fs-service.ts";
...
try {
	...
	writeFileSync(path.join(tempRoot, "hello.txt"), "hello from ElizaLaunch\n", "utf8");
	writeFileSync(path.join(tempRoot, "nested", "note.txt"), "nested note\n", "utf8");
	writeFileSync(path.join(tempRoot, ".env"), "SECRET=denied\n", "utf8");
	writeFileSync(path.join(tempRoot, "binary.dat"), Buffer.from([0, 1, 2, 3]));
	writeFileSync(path.join(tempRoot, "node_modules", "ignored.txt"), "ignored text\n", "utf8");
	writeFileSync(path.join(tempRoot, "..", "outside.txt"), "outside via parent\n", "utf8");
	writeFileSync(path.join(outsideRoot, "secret.txt"), "outside root\n", "utf8");
	try {
		symlinkSync(path.join(outsideRoot, "secret.txt"), path.join(tempRoot, "escape-link"));
	} catch {
	}

	process.env.ELIZA_FS_ROOTS = tempRoot;
	process.env.ELIZA_FS_ENABLE_WRITES = "0";
	const service = new FileSatelliteService();
	...
	const read = await service.readText({ path: path.join(tempRoot, "hello.txt") });
	...
	const ignoredSearch = await service.search({ path: tempRoot, query: "ignored" });
	...
	if (originalWrites === "1") {
		...
		const writeService = new FileSatelliteService();
...
async function expectFileError(
	...
	try {
		...
		if (error instanceof FileSatelliteException && error.code === code) return;
		throw error;
	}
