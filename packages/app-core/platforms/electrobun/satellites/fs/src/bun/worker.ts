...
import { FileSatelliteService } from "./fs-service.ts";
import type {
	FileListParams,
	FileMethod,
	FileReadTextParams,
...
function parseRequest(value: unknown): FileWorkerRequestMessage | null {
	...
	if (
		...
		throw new Error("Invalid File Satellite request.");
	}
...
async function dispatch(request: FileWorkerRequestMessage): Promise<unknown> {
	...
	throw new Error(`Unsupported File Satellite method: ${exhaustive}`);
}
