# Eliza File Satellite

`eliza.fs` is the File Satellite for ElizaLaunch. It exposes scoped filesystem operations for Eliza Orbit through the Satellite boundary.

Phase 5 keeps filesystem access brokered:

```text
Surface Satellite -> eliza.runtime -> eliza.fs
```

The Surface Satellite does not call this Satellite directly.

## Roots

Allowed roots come from `ELIZA_FS_ROOTS`, a comma-separated list of absolute paths. If it is unset, the File Satellite uses:
...
```sh
bun run --cwd elizalaunch/satellites/fs build
bun run --cwd elizalaunch/satellites/fs smoke
bun run --cwd elizalaunch/satellites/fs smoke:phase5
bun run --cwd elizalaunch/satellites/fs typecheck
```
