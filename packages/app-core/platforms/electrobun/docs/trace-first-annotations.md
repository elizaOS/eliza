...
| `plugin-computeruse` | native semantic plugin | screenshot, OCR, approval request/decision, click/key/window action, denial, failure | scene/approval/task inspection view | use `eliza.computer` for host-owned screen/input/window implementation | Do not fold semantic desktop automation into Electrobun. The plugin remains the agent-facing layer. |
...
| `plugin-native-screencapture` | native semantic plugin | capture request, permission result, capture complete, recording start/stop, file artifact, failure | capture result inspection view | use `eliza.computer` for host-owned capture implementation | Keep the plugin as the agent-facing capture action surface. |
