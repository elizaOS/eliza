...
> proxy child to a Node 24+ executable (the Vite proxy relies on Node-only
> socket methods that Bun does not implement), so a Bun-only checkout without a
> compliant Node fails at supervisor start. See
