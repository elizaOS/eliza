...
check "Runtime inventory is closure-derived, text-bearing, and fail-closed" \
  bash -c "grep -Fq 'elizaos-runtime-dependencies.json' '$SCRIPT_DIR/generate-license-inventory.mjs' && grep -Fq 'runtimeDependencyInventorySha256' '$SCRIPT_DIR/generate-license-inventory.mjs' && grep -Fq 'payloadSha256' '$SCRIPT_DIR/generate-license-inventory.mjs' && grep -Fq 'licenseTexts' '$SCRIPT_DIR/generate-license-inventory.mjs' && grep -Fq 'Third-party dependency has no retained license terms' '$SCRIPT_DIR/generate-license-inventory.mjs' && grep -Fq 'Dependency uses a prohibited license' '$SCRIPT_DIR/generate-license-inventory.mjs' && grep -Fq 'licensePolicyCounts' '$SCRIPT_DIR/generate-license-inventory.mjs'"
