...
When `MILADY_BENCHMARK_USE_MOCKS=1` is set, runtime construction routes through
...
2. Snapshots and overrides `process.env` with each provider's `MILADY_MOCK_*_BASE`
...
MILADY_BENCHMARK_USE_MOCKS=1 \
  MILADY_DUMP_TRAJECTORIES=1 \
...
```bash
MILADY_BENCHMARK_USE_MOCKS=1 \
  MILADY_BENCHMARK_FILTER="cal-next-event,password-manager-lookup" \
  bun test test/benchmarks/action-selection.real.test.ts
```
...
| --------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------ |
| Google OAuth + Calendar + Gmail   | `*.googleapis.com`, `accounts.google.com`            | In-process HTTP server (`environments/google.json` + `googleDynamicFixture`) with stateful calendar+gmail fixture data | `MILADY_MOCK_GOOGLE_BASE`, `MILADY_BLOCK_REAL_GMAIL_WRITES=1`                                                              | done   |
| Eliza Cloud managed Google        | `cloud.elizaos.com`                                  | `environments/cloud-managed.json`                                                                                      | `ELIZA_CLOUD_BASE_URL`                                                                                                     | done   |
| Twilio Programmable Voice/SMS     | `api.twilio.com`                                     | `environments/twilio.json` (static Mockoon routes)                                                                     | `MILADY_MOCK_TWILIO_BASE`, `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_PHONE_NUMBER` (FAKE_CREDS)                    | done   |
| WhatsApp Business Cloud           | `graph.facebook.com`                                 | `environments/whatsapp.json` + dynamic webhook ingest                                                                  | `MILADY_MOCK_WHATSAPP_BASE`, `ELIZA_WHATSAPP_ACCESS_TOKEN`, `ELIZA_WHATSAPP_PHONE_NUMBER_ID`, `ELIZA_WHATSAPP_API_VERSION`  | done   |
| Calendly v2                       | `api.calendly.com`                                   | `environments/calendly.json`                                                                                           | `MILADY_MOCK_CALENDLY_BASE`, `ELIZA_CALENDLY_TOKEN`                                                                        | done   |
| X (Twitter) v2                    | `api.x.com`                                          | `environments/x-twitter.json` + `xDynamicFixture`                                                                      | `MILADY_MOCK_X_BASE`, `TWITTER_API_KEY`, `TWITTER_API_SECRET_KEY`, `TWITTER_ACCESS_TOKEN`, `TWITTER_ACCESS_TOKEN_SECRET`    | done   |
...
| GitHub REST                       | `api.github.com`                                     | `environments/github.json` + `githubDynamicFixture`                                                                    | `MILADY_MOCK_GITHUB_BASE`, `GITHUB_API_URL`                                                                                | done   |
