...
function runFixture(
  ...
  try {
    const helper = path.join(
      root,
      "packages/app-core/test/live-agent/helpers/connector-imports.ts",
    );
...
describe("connector filesystem imports", () => {
  it.each(["Telegram", "Lens", "Feishu"] as const)(
    ...
    (connector) => {
      ...
      const directory = `packages/app-core/node_modules/${packageName}`;
      expect(
  ...
  it("prefers the installed canonical package to a local checkout", () => {
    const directory = "packages/app-core/node_modules/@elizaos/plugin-telegram";
    expect(
      runFixture("Telegram", {
        [`${directory}/package.json`]: JSON.stringify({
          type: "module",
  ...
  it("imports the alternate Lens package before checkout fallbacks", () => {
    const directory =
      "packages/app-core/node_modules/@elizaos-plugins/client-lens";
    expect(
      runFixture("Lens", {
        [`${directory}/package.json`]: JSON.stringify({
          type: "module",
