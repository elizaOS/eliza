...
if (argv._.length === 0 && !argv.all) {
  console.error(
    `Warning: No specs requested. Did you forget to toggle '--all'?`,
  );
  process.exitCode = 1;
} else {
  Promise.all(
    (argv.all
      ? glob.sync(pattern)
      : argv._.map((name) =>
          fs.existsSync(name) ? name : pattern.replaceAll("*", name),
        )
    ).map((conf, i, { length }) =>
      limit(
        () =>
          new Promise((resolve) => {
            if (argv.verbose)
              console.log(`[${i + 1}/${length}] Running ${conf}`);
            execFile("certoraRun", [conf], (error, stdout, stderr) => {
              ...
              if (error) {
                ...
                console.error(
                  `[ERR] Could not parse stdout for ${conf}:\n${stdout}`,
                );
                process.exitCode = 1;
              }
              resolve();
