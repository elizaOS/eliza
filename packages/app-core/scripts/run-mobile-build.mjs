...
// Auto-hide the iOS home indicator (the bottom "pull" bar) so it doesn't sit
// over the chat composer. iOS still reveals it on the next upward swipe (it
// can't be permanently removed), but it fades while the app is in use. We do
// this by overriding `prefersHomeIndicatorAutoHidden` on a CAPBridgeViewController
// subclass and pointing the storyboard at it. The ios/ tree is gitignored +
// regenerated by `cap add`/clean, so this runs on EVERY build (idempotent) to
// survive regeneration — appending the subclass to the already-compiled
// AppDelegate.swift avoids any Xcode project (pbxproj) change.
function ensureIosHomeIndicatorAutoHidden() {
  ...
  if (fs.existsSync(appDelegatePath)) {
    ...
    if (!src.includes("class MainViewController")) {
      const subclass = [
        "",
        "",
        '/// Auto-hides the iOS home indicator (the bottom "pull" bar) so it does',
        "/// not sit over the chat composer. iOS still reveals it on the next",
        "/// upward swipe — it cannot be permanently removed.",
        "class MainViewController: CAPBridgeViewController {",
        "    override var prefersHomeIndicatorAutoHidden: Bool { return true }",
        "}",
        "",
      ].join("\n");
      fs.writeFileSync(appDelegatePath, src + subclass);
      console.log(
        "[mobile-build] iOS: added MainViewController (home-indicator auto-hide).",
      );
    }
  }
  ...
  if (fs.existsSync(storyboardPath)) {
    ...
    if (xml.includes('customClass="CAPBridgeViewController"')) {
      ...
      if (patched !== xml) {
        fs.writeFileSync(storyboardPath, patched);
        console.log(
          "[mobile-build] iOS: pointed storyboard at MainViewController.",
        );
      }
    }
  }
...
async function buildIos({ local = false } = {}) {
  ...
  ensureIosHomeIndicatorAutoHidden();
