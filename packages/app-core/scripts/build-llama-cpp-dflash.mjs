...
function patchGgmlBaseForWindowsQjl(cacheDir) {
  ...
  const qjlFileLines = QJL_GGML_BASE_LINK_FILES.map(
    (f) => `            ${f}`,
  ).join("\n");
...
function patchGgmlCudaForFusedAttn(cacheDir, { dryRun = false } = {}) {
  ...
  // last of them.
  const anchorRe = /if\s*\(\s*GGML_CUDA_TBQ3_TCQ\s*\)[\s\S]*?endif\s*\(\s*\)/;
...
// Returns true when the in-repo submodule checkout exists and has a worktree
// (`.git` present + a tracked source file). When this is the case the build
// defaults to it; otherwise it falls back to a per-user clone.
...
function applyForkPatches(cacheDir, backend, target, { dryRun = false } = {}) {
  ...
  // builds are byte-for-byte unchanged; the merged tools/omnivoice CMake path
  // links `omnivoice_lib` into `llama-server` and sets that define for fused
...
function ensureLegacyDflashDrafterRuntime({ args }) {
  ...
  return {
    ...buildLegacyDflashDrafterRuntime({ args }),
    reusedPrebuilt: false,
  };
}
...
function buildIosRuntimeSymbolShim({ target, outDir }) {
  ...
  if (!fs.existsSync(source)) {
    throw new Error(
      `[dflash-build] iOS runtime symbol shim missing: ${source}`,
    );
  }
...
function buildTarget({ target, args, ctx }) {
  ...
  if (shouldUseLegacyDflashDrafterRuntime(target)) {
    if (args.dryRun) {
      ...
      console.log(`  install ${legacyDflashDrafterBinDir()} -> ${outDir}`);
      return null;
    }
