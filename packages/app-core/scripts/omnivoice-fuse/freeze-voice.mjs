 ...
 *     --voice samantha \
 *     --ai-voices-dir packages/training/data/voice/samantha/audio/ \
 *     --out <bundle>/cache/voice-preset-samantha.bin \
 *     --instruct "young adult female, warm, soft, neutral us-american"
 *
 * Flags:
 *   --voice <id>           Voice identifier (default: `samantha`).
...
} from "../../../../plugins/plugin-local-inference/src/services/voice/voice-preset-format.ts";
...
function usage(code = 0) {
	console.log(
		[
			...
			"",
			"Flags:",
			"  --voice <id>           Voice id (default: samantha)",
...
function parseArgs(argv) {
	...
	if (!args.out) {
		args.out = path.join(args.bundle, "cache", `voice-preset-${args.voice}.bin`);
	}
 ...
 *  samantha corpus ships (verified via `audit_samantha.sh` per R12). */
...
function discoverClips(args) {
	...
	if (args.corpus && existsSync(path.join(args.corpus, "manifest.jsonl"))) {
		const lines = readFileSync(
			path.join(args.corpus, "manifest.jsonl"),
			"utf8",
		)
			.split("\n")
...
async function main() {
	...
	console.log(`[freeze-voice] audio-dir=${args.audioDir ?? "(default audio/)"}`);
	...
	const refText = clips.map((c) => c.transcript).join(" ").trim();
	const pcm24k = loadAndConcat24k(clips);
	console.log(`[freeze-voice] concatenated PCM: ${pcm24k.length} samples @ 24 kHz (${(pcm24k.length / 24000).toFixed(2)} s)`);

	const { K, refT, tokens } = await maybeEncodeReference(pcm24k, args);
	console.log(`[freeze-voice] encoded: K=${K}, ref_T=${refT}, ${tokens.length} tokens`);
