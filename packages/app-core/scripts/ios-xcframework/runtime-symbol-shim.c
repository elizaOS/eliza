 ...
 * build-llama-cpp-dflash.mjs, not from the upstream llama-cpp-capacitor CMake
...
int eliza_inference_mmap_acquire(
    ...
    if (strcmp(region_name, "text") == 0 || strcmp(region_name, "dflash") == 0) {
        return ELIZA_OK;
    }
...
int32_t eliza_llama_speculative_draft_gen(
    ...
    (void) draft_max;
    (void) out_capacity;
    if (out_drafted != NULL) *out_drafted = 0;
    return 0;
}
...
void * eliza_llama_sampler_init_token_tree(
    ...
    (void) trie_bytes;
    (void) trie_size;
    set_last_error("iOS token-tree sampler is not wired in this static slice");
    return NULL;
}
