 ...
 * Code and Codex sub-agents but depends only on `@elizaos/core` — it
 ...
 *
 * Claude and Codex are optional external CLIs. Eliza Code uses the runtime's
 * own provider route; z.ai / Kimi / GLM accounts likewise serve runtime API-key
 * routing and are deliberately absent from this external-CLI selector.
 */
...
async function buildEnvPatch(
  ...
  switch (providerId) {
    ...
    default: {
      // Direct API providers map to their narrow provider key.
