 ...
 */
function pluginWithParams(params: Array<{ key: string; sensitive: boolean }>): {
...
function alwaysFailingVault(): Vault {
  const stub: Partial<Vault> = {
    ...
    async get() {
      throw new VaultMissError("_manager.preferences");
    },
    async reveal() {
...
function selectivelyFailingVault(failKeys: ReadonlySet<string>): {
  ...
  const stub: Partial<Vault> = {
    ...
    async get(key) {
      ...
      if (v === undefined) throw new VaultMissError(key);
      return v;
    },
    async reveal(key) {
      const v = written.get(key);
...
describe("plugins-compat reveal route — vault-first source guard", () => {
  ...
  it("calls sharedVault().reveal(key, caller) inside the reveal route", () => {
    const src = readSource();
    expect(src).toMatch(/\/api\/plugins\/[^/]+\/reveal/);
    expect(src).toMatch(/sharedVault\(\)\s*\.\s*reveal\s*\(/);
  });
  ...
  it("falls back to process.env after the vault attempt", () => {
    ...
    const vaultIdx = after.indexOf("sharedVault().reveal(");
