...
describe("decideResponse", () => {
  it("silent when vad is inactive regardless of other signals", () => {
    expect(
      decideResponse(
        signals({ vadActive: false, directAddress: true, wakeIntent: 1, ownerConfidence: 1 }),
      ),
    ).toBe("silent");
  });

  it("responds on direct address with sufficient owner confidence", () => {
    expect(decideResponse(signals({ directAddress: true, ownerConfidence: 0.6 }))).toBe(
      "respond",
    );
  });

  it("observes on direct address with insufficient owner confidence", () => {
    expect(decideResponse(signals({ directAddress: true, ownerConfidence: 0.4 }))).toBe(
      "observe",
    );
  });
  ...
  it("responds when context expects reply with floor owner confidence", () => {
    expect(
      decideResponse(signals({ contextExpectsReply: true, ownerConfidence: 0.5 })),
    ).toBe("respond");
  });

  it("observes when context expects reply but owner confidence too low", () => {
    expect(
      decideResponse(signals({ contextExpectsReply: true, ownerConfidence: 0.49 })),
    ).toBe("observe");
  });

  it("respects a custom direct-address threshold", () => {
    expect(
      decideResponse(signals({ directAddress: true, ownerConfidence: 0.7 }), 0.8),
    ).toBe("observe");
    expect(
      decideResponse(signals({ directAddress: true, ownerConfidence: 0.85 }), 0.8),
    ).toBe("respond");
  });
});
