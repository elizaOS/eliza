import type { ChatSourceMeta } from "@elizaos/ui";
import {
...
const APP_CHAT_SOURCE_META: Record<string, ChatSourceMeta> = {
  eliza: {
...
registerChatSourceMetaEntries(APP_CHAT_SOURCE_META);
registerChatReactionEmojiRenderer((emoji) => {
  ...
  const extension = match[1] === "a" ? "gif" : "png";
  const src = `https://cdn.discordapp.com/emojis/${match[3]}.${extension}?size=64&quality=lossless`;
  return (
    <img src={src} alt={`:${match[2]}:`} className="h-4 w-4 object-contain" />
  );
});
