 ...
 *
 * Contributor discovery, deferral policy, and server-only lifecycle live in
 * focused startup modules; this host retains runtime repair and lifecycle
 * composition. Mobile platforms take a trimmed boot path. Boot resources are
 * scoped to one runtime so embedded hosts cannot supersede or tear down each
 * other's services.
 */
