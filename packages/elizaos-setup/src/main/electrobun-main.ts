...
//      bundle script runs, by passing a small preload script string to the
...
function buildPreloadScript(serverUrl: string): string {
  ...
  // Keep this as a string literal — Electrobun passes it directly to the
  // webview's preload hook.
  return `(() => {
  try {
    ...
    // to a plain assignment. The renderer just needs the value to be set
    // before getServerUrl() is called.
...
function resolveRendererIndexUrl(): string {
  ...
  // output to `renderer/` next to the bun bundle. `import.meta.dir` is
  // the directory containing this compiled entrypoint.
...
async function main(): Promise<void> {
  ...
  // Reference `win` so the GC does not collect the window handle.
  void win;
}
