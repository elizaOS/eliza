...
impl Deployment for DockerDeployment {
    async fn start(&mut self) -> Result<()> {
        ...
        // Add volume mounts
        let volume_args: Vec<String> = self.config.volumes
            .iter()
        ...
        let env_args: Vec<String> = self.config.environment
            .iter()
            .map(|(k, v)| format!("{}={}", k, v))
            .collect();
        for env in &env_args {
    ...
    async fn execute(&self, command: &str, timeout: Option<u64>) -> Result<String> {
        let container_id = self.container_id.as_ref().ok_or_else(|| {
            SWEAgentError::EnvironmentError("Container not running".to_string())
        })?;
    ...
    async fn read_file(&self, path: &str) -> Result<String> {
        ...
        self.docker_cmd(&["exec", container_id, "cat", path]).await
    ...
    async fn write_file(&self, path: &str, content: &str) -> Result<()> {
        ...
        if let Some(parent) = std::path::Path::new(path).parent() {
            ...
            let _ = self.docker_cmd(&["exec", container_id, "bash", "-c", &mkdir_cmd]).await;
        }
        
        // Use heredoc to write content - escape single quotes
        let escaped_content = content.replace('\'', "'\\''");
        let write_cmd = format!("cat > '{}' << 'SWEAGENT_EOF'\n{}\nSWEAGENT_EOF", path, escaped_content);
        
        self.docker_cmd(&["exec", container_id, "bash", "-c", &write_cmd]).await?;
    ...
    async fn interrupt(&self) -> Result<()> {
        if let Some(ref container_id) = self.container_id {
            ...
            let _ = self.docker_cmd(&["exec", container_id, "pkill", "-INT", "-f", "."]).await;
        }
        Ok(())
    }
}
...
impl Deployment for MockDeployment {
    ...
    async fn execute(&self, command: &str, _timeout: Option<u64>) -> Result<String> {
        ...
        if command.starts_with("cat ") {
            ...
            let files = self.files.lock().map_err(|e| SWEAgentError::RuntimeError(e.to_string()))?;
    ...
    async fn read_file(&self, path: &str) -> Result<String> {
        ...
        files.get(path).cloned().ok_or_else(|| {
            SWEAgentError::FileNotFound(path.to_string())
        })
    }
    
    async fn write_file(&self, path: &str, content: &str) -> Result<()> {
        let mut files = self.files.lock().map_err(|e| SWEAgentError::RuntimeError(e.to_string()))?;
...
mod tests {
    ...
    async fn test_mock_deployment() {
        ...
        // Write a file
        deployment.write_file("/test.txt", "hello world").await.unwrap();
    ...
    async fn test_local_deployment_file_ops() {
        ...
        deployment.write_file(test_file, "test content").await.unwrap();
