...
fn test_version_format() {
    ...
    for part in parts {
        assert!(part.parse::<u32>().is_ok(), "Each version part should be a number");
    }
}
...
fn test_version_matches_python_and_typescript() {
    ...
    assert_eq!(VERSION, "1.1.0", "Version should match Python and TypeScript");
}
...
fn test_crate_exports() {
    ...
    use elizaos_sweagent::{
        AgentConfig, AgentRunResult, DefaultAgent, DefaultAgentConfig,
        RetryAgent, RetryAgentConfig,
    };
    use elizaos_sweagent::{
        DeploymentConfig,
        EnvironmentConfig, RepoConfig, SWEEnv,
    };
    use elizaos_sweagent::{
        Bundle, BundleConfig, ParseFunction, ToolConfig, ToolHandler,
    };
    use elizaos_sweagent::{
        RunBatch, RunBatchConfig, RunSingle, RunSingleConfig,
    };
    use elizaos_sweagent::types::{
        History, HistoryItem, StepOutput, Trajectory, TrajectoryStep,
    };
