...
export function warnProbablyWrongJinjaSyntax(template: string | null): void {
  ...
  const patterns = [
    {
      pattern: /\{\{[^}]*\{[^}]*\}\}/,
      message: "Nested braces detected in template",
    },
    {
      pattern: /\{%[^%]*\{[^%]*%\}/,
      message: "Nested braces in control structure",
    },
  ...
  if (openingTags !== closingTags) {
    logger.warning(
      `Mismatched Jinja tags: ${openingTags} opening, ${closingTags} closing`,
    );
  }
  ...
  if (openingControlTags !== closingControlTags) {
    logger.warning(
      `Mismatched Jinja control tags: ${openingControlTags} opening, ${closingControlTags} closing`,
    );
  }
}
