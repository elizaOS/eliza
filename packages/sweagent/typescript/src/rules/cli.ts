...
import { formatValidationResults, validateFiles } from "./validators";

const program = new Command();

program
  .name("swe-rules")
  .description("SWE-agent rules validation and management CLI")
  .version("1.0.0");
...
program
  ...
  .option(
    "-l, --language <lang>",
    "Language to validate (python or typescript)",
    "auto",
  )
  ...
  .action((options: { language?: string }) => {
    ...
    for (const lang of languages) {
      ...
      const rules = getApplicableRules(
        `dummy.${lang === "python" ? "py" : "ts"}`,
        lang as "python" | "typescript",
      );
