...
export const SWE_ENV_CLASS: ClassInfo = {
  ...
  description:
    "Class that interfaces with the SWE-ReX project to interface with sandboxed docker containers",
};
...
export const EXECUTION_ENVIRONMENT: ExecutionEnvironment = {
  type: "docker",
  description:
    "AI agent proposes actions that are executed in sandboxed docker containers",
...
export const TOOLS_INFO: ToolsInfo = {
  ...
  deployment:
    "Copied to sandboxed container and made available in $PATH variable",
};
...
export function getComponentByPath(
  path: string,
): { component: string; description: string } | null {
  // Check entry points
  ...
  if (path === MAIN_AGENT_CLASS.path) {
    return {
      ...
      description: MAIN_AGENT_CLASS.description,
    };
  }
