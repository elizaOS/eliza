...
export const MODEL_PRICING: Record<string, ModelPricing> = {
  ...
  "azure/gpt-5": {
    inputCostPer1k: 0.005, // Same as gpt-5
    outputCostPer1k: 0.015, // Same as gpt-5
    maxInputTokens: 128000,
    maxOutputTokens: 4096,
    provider: "azure",
  },
  "azure/gpt-5-mini": {
    inputCostPer1k: 0.00015, // Same as gpt-5-mini
...
export function calculateCost(
  ...
  if (!pricing) {
    // Handle versioned models (e.g., gpt-5-0613 -> gpt-5)
  ...
  if (!pricing) {
    for (const [key, value] of Object.entries(MODEL_PRICING)) {
      if (
        modelName.includes(
          key.replace(
            /^(azure|bedrock|together|mistral|perplexity|deepseek|ollama)\//,
            "",
          ),
        )
      ) {
  ...
  if (!pricing) {
    console.warn(
      `Model pricing not found for ${modelName}, using default pricing`,
    );
    pricing = {
      inputCostPer1k: 0.001,
