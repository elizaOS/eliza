...
import type { Command, ModelOutput } from "../types";
  ...
  call(
    ...
    const message =
      typeof modelResponse === "string"
        ? modelResponse
        : modelResponse.message || modelResponse.content || "";
    ...
    const thoughtMatch = message.match(
      /💭\s*THOUGHT[:\s]*([^\n]*(?:\n(?!🎬|ACTION)[^\n]*)*)/i,
    );
...
export class EditFormatParser extends ThoughtActionParser {
  ...
  call(
    ...
    if (
      action.includes("str_replace_editor") ||
      action.includes("str_replace_based_edit_tool")
    ) {
...
export class FunctionCallingParser extends AbstractParseFunction {
  ...
  call(
    ...
    const toolCalls =
      typeof modelResponse !== "string"
        ? (modelResponse.toolCalls ?? modelResponse.tool_calls ?? null)
        : null;
    ...
    if (toolCalls && toolCalls.length > 0) {
        ...
        try {
          ...
          if (
            ...
            if (argsObj.command) {
              ...
              const values = Object.values(argsObj).filter(
      ...
      const thought =
        typeof modelResponse !== "string" ? modelResponse.message || "" : "";
      return [thought, action];
    }
  ...
  call(
    ...
    if (strict) {
      throw new FormatError(
        "Could not find bash code blocks in model response",
      );
    }
  ...
  call(
    ...
    try {
      ...
      if (!parsed.thought || !parsed.command) {
        if (strict) {
          throw new FormatError(
            'JSON must contain both "thought" and "command" keys',
          );
        }
        return ["", message];
      }
