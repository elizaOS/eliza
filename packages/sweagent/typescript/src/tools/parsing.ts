  ...
  formatErrorMessage(error: FunctionCallingFormatError): string {
    ...
    if (errorCode === "missing") {
      ...
      const missingMatch = message.match(
        /{%- if error_code == "missing" -%}([\s\S]*?){%- elif/,
      );
      if (missingMatch) {
      ...
      const multipleMatch = message.match(
        /{%- elif error_code == "multiple" -%}([\s\S]*?){%- else/,
      );
      if (multipleMatch) {
