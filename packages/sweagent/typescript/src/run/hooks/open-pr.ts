...
import { AbstractRunHook, type RunHookInit } from "./types";
...
export interface OpenPRConfig {
  // Skip action if there are already commits claiming to fix the issue
...
function formatTrajectoryMarkdown(
  ...
  const steps: string[] = [];
  let currentLength = prefixText.length + suffixText.length;
  ...
  for (let i = 0; i < trajectory.length; i++) {
    ...
    if (
      charLimit !== undefined &&
      currentLength + separatorLength + stepText.length > charLimit
    ) {
      if (i > 0) {
...
async function openPR(params: {
  ...
  const { logger, token, env, githubUrl, trajectory, dryRun = false } = params;
  ...
  try {
    ...
    if (error instanceof InvalidGithubURL) {
      throw new Error(
        "Data path must be a github issue URL if open_pr is set to True.",
      );
    }
    throw error;
  }
  ...
  await env.communicate(
    "git config user.email 'noemail@swe-agent.com' && git config user.name 'SWE-agent'",
    10,
    {
  ...
  const out = await env.communicate(
    `git commit -m "${commitMsg[0]}" -m "${commitMsg[1]}" ${dryRunFlag}`,
    10,
    {
  ...
  let forker = owner;
  let remote = "origin";
  const pushRepoUrl = (env.repo as { push_gh_repo_url?: string })
    ?.push_gh_repo_url;
  ...
  const pushOut = await env.communicate(
    `${dryRunPrefix}git push ${remote} ${branchName}`,
    10,
    {
      errorMsg:
  ...
  logger.debug(
    `Pushed commit to remote=${remote} branch=${branchName}: ${pushOut}`,
  );
  ...
  if (!dryRun) {
    ...
    logger.info(
      `🎉 PR created as a draft at ${pr.data.html_url}. Please review it carefully, push ` +
 ...
 */
interface ProblemStatementWithGithubUrl {
  githubUrl: string;
}

export class OpenPRHook extends AbstractRunHook {
  ...
  onInit(run: RunHookInit): void {
    const envVal = run.env;
    this.env = envVal;
    this.token = process.env.GITHUB_TOKEN || "";

    const ps = run.problemStatement;
    if (ps && typeof ps === "object" && "githubUrl" in ps) {
      const githubUrl = (ps as { githubUrl?: JsonValue }).githubUrl;
      if (typeof githubUrl === "string") {
        this.problemStatement = ps as ProblemStatementWithGithubUrl;
      }
    }
  }
  ...
  async onInstanceCompleted(params: { result: AgentRunResult }): Promise<void> {
    try {
      if (await this.shouldOpenPR(params.result)) {
        ...
        const trajectory: TrajectoryStep[] = params.result.trajectory.map(
          (step) => ({
            ...step,
          }),
        );
  ...
  private async shouldOpenPR(result: AgentRunResult): Promise<boolean> {
    ...
    if (result.info.exitStatus !== "submitted") {
      logger.info(
        `Not opening PR because exit status was ${result.info.exitStatus} and not submitted.`,
      );
      return false;
    }
    ...
    if (!this.problemStatement || !this.problemStatement.githubUrl) {
      logger.info(
        "Problem statement not initialized or missing GitHub URL. Skipping PR creation.",
      );
      return false;
    }
    ...
    let issue: GitHubIssue;
    try {
      issue = (await getGhIssueData(
        this.problemStatement.githubUrl,
        this.token,
      )) as GitHubIssue;
    } catch (error) {
      if (error instanceof InvalidGithubURL) {
    ...
    if (issue.state !== "open") {
      logger.info(
        `Issue is not open (state=${issue.state}). Skipping PR creation.`,
      );
      return false;
    }
    ...
    try {
      ...
      associatedCommits = await getAssociatedCommitUrls(
        org,
        repo,
        issueNumber,
        this.token,
    ...
    if (associatedCommits.length > 0) {
      ...
      if (this.config.skipIfCommitsReferenceIssue) {
        logger.info(
          `Issue already has associated commits (see ${commitUrlStrs}). Skipping PR creation.`,
        );
        return false;
      } else {
        logger.warn(
