...
import { GithubIssue, TextProblemStatement } from "../agent/problem-statement";
import type { GithubRepoConfig, LocalRepoConfig } from "../environment/repo";
import type { EnvironmentConfig } from "../environment/swe-env";
import { RunBatch, type RunBatchConfig } from "./run-batch";
import {
  RunSingle,
...
program
  ...
  .alias("r")
  .description(
    "Run swe-agent on a single problem statement instance, for example a github issue",
  )
...
program
  ...
  .alias("b")
  .description(
    "Run swe-agent on a batch of problem statements, e.g., on SWE-Bench",
  )
  .option(
    "--instances.type <type>",
    "Instance source type (swe_bench, file, huggingface)",
  )
  .option(
    "--instances.subset <subset>",
    "SWE-bench subset (lite, verified, full, multimodal, multilingual)",
  )
  ...
  .option("--agent.model.name <name>", "Model to use")
  .option(
    "--agent.model.per_instance_cost_limit <limit>",
  ...
  .option(
    "--random_delay_multiplier <n>",
    "Random delay multiplier for workers",
    "0.3",
  )
  .action(async (options) => {
    try {
      ...
      const config: Partial<RunBatchConfig> = {
        ...
        randomDelayMultiplier: parseFloat(
          options.random_delay_multiplier || "0.3",
        ),
      };
...
program
  ...
  .option(
    "-d, --data_path <path>",
    "Path to the data file to load gold patches from",
  )
...
program
  ...
  .option(
    ...
    "config/exotic/default_shell.yaml",
  )
