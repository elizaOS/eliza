...
function viewImage(imagePath: string): void {
  ...
  if (!mimeType || !VALID_MIME_TYPES.has(mimeType)) {
    console.error(
      `Error: Unsupported image type: ${mimeType}. Valid types are: ${Array.from(VALID_MIME_TYPES).join(", ")}`,
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      `![${path.basename(imagePath)}](data:${mimeType};base64,${base64})`,
    );
  } catch (error) {
...
if (
  require.main === module ||
  require.main?.filename?.endsWith("/bin/view_image")
) {
  setupCLI();
}
