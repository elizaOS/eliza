...
describe("History Processors", () => {
  ...
  describe("LastNObservations", () => {
    it("should keep last N observations", () => {
      ...
      const elidedCount = newHistory.filter((entry) =>
        entry.content?.toString().includes("Old environment output"),
      ).length;

      // We have 5 observations total, keeping 3 + 1 (instance template), so 1 should be elided
      const totalObservations = testHistory.filter(
        (entry) => entry.messageType === "observation",
      ).length;
  ...
  describe("TagToolCallObservations", () => {
    it("should add tags to edit actions", () => {
      ...
      newHistory.forEach((entry) => {
        if (entry.action?.startsWith("edit ")) {
    ...
    it("should handle multiple tags and function names", () => {
      ...
      newHistory.forEach((entry) => {
        if (
          entry.action &&
          (entry.action.startsWith("edit ") || entry.action.startsWith("cat "))
        ) {
  ...
  describe("Multiple processors", () => {
    it("should chain processors correctly", () => {
      ...
      const elidedCount = processedHistory.filter((entry) =>
        entry.content?.toString().includes("Old environment output"),
      ).length;
      ...
      const taggedEdits = processedHistory.filter(
        (entry) =>
          entry.action?.startsWith("edit ") &&
          entry.tags?.includes("processed"),
      );
