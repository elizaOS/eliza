...
function toTemplateConfig(partial: Partial<TemplateConfig>): TemplateConfig {
  return {
    systemTemplate: partial.systemTemplate ?? "",
    instanceTemplate: partial.instanceTemplate ?? "",
    nextStepTemplate:
      partial.nextStepTemplate ?? "Observation: {{observation}}",
    nextStepTruncatedObservationTemplate:
      partial.nextStepTruncatedObservationTemplate ??
      "Observation: {{observation}}",
    maxObservationLength: partial.maxObservationLength ?? 100000,
    nextStepNoOutputTemplate: partial.nextStepNoOutputTemplate,
    strategyTemplate: partial.strategyTemplate,
    demonstrationTemplate: partial.demonstrationTemplate,
    demonstrations: partial.demonstrations ?? [],
    putDemosInHistory: partial.putDemosInHistory ?? false,
    disableImageProcessing: partial.disableImageProcessing ?? false,
    shellCheckErrorTemplate: partial.shellCheckErrorTemplate ?? "",
    commandCancelledTimeoutTemplate:
      partial.commandCancelledTimeoutTemplate ?? "",
  };
}
...
describe("Agent Tests", () => {
  ...
  beforeEach(() => {
    ...
    const templates = toTemplateConfig(DEFAULT_TEMPLATE_CONFIG);

    dummyEnv = new SWEEnv({
      deployment: new TestDeployment(new TestRuntime()),
      repo: undefined,
      postStartupCommands: [],
    });
    ...
    jest
      .spyOn(dummyEnv, "writeFile")
      .mockImplementation(() => Promise.resolve());
    jest
      .spyOn(dummyEnv, "communicate")
      .mockImplementation(() => Promise.resolve(""));
    ...
    functionCallingAgentConfig = {
      ...
      tools: {
        ...defaultToolConfig,
        parseFunction: "function_calling",
  ...
  describe("Exit conditions", () => {
    it("should exit on cost limit", async () => {
      ...
      jest
        .spyOn(agent, "attemptAutosubmissionAfterError")
  ...
  describe("Step execution", () => {
    it("should run step by step checking history", async () => {
      ...
      await agent.setup(
        dummyEnv,
        new TextProblemStatement({ text: "asdf123" }),
      );
  ...
  describe("Auto-submission", () => {
    it("should auto-submit when patch is found", async () => {
      ...
      jest
        .spyOn(dummyEnv, "writeFile")
        .mockImplementation(async (path, _content) => {
  ...
  describe("Retry logic", () => {
    it("should retry on format errors up to max retries", async () => {
      ...
      expect(agent.trajectory.length).toBeGreaterThanOrEqual(
        agent.maxRequeries,
      );
    });
  });
