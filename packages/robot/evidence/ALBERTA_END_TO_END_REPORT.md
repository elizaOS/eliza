...
| `unified_robot_interface_all_profiles` | `proved` | objective_audit_passed=`True`, video_profiles=`['asimov-1', 'hiwonder-ainex', 'unitree-g1', 'unitree-h1', 'unitree-r1']`, manifest_review_consistent=`True` | none |
| `traditional_and_sota_baselines` | `proved` | stable_baselines3_ppo_artifact=`True`, stable_baselines3_sac_artifact=`True`, stable_baselines3_sac_ok=`True`, brax_mjx_ppo_preflight_ok=`True`, brax_mjx_ppo_manifest_present=`False`, brax_mjx_ppo_contract_artifact_ok=`True`, brax_mjx_ppo_contract_only=`True`, brax_mjx_ppo_production_training=`False` | none |
...
| `alberta_vs_baselines_side_by_side` | `proved` | backend_comparison_count=`3`, green_backend_comparison_count=`2`, green_backend_profiles=`['asimov-1', 'hiwonder-ainex']`, sac_learners=`['alberta', 'ppo', 'sac']` | none |
| `continual_unseen_obstacle_learning_no_forgetting` | `proved` | obstacle_validation_ok=`True`, alberta_acc_minus_ppo=`1.20296`, alberta_forgetting_minus_ppo=`-0.033605`, obstacle_demo_ok=`True`, sac_demo_ok=`True`, alberta_new_task_gain=`3.50647`, sac_alberta_new_task_gain=`0.373737` | none |
| `robot_action_videos_self_reviewed` | `proved` | video_review_ok=`True`, video_count=`25`, profiles=`['asimov-1', 'hiwonder-ainex', 'unitree-g1', 'unitree-h1', 'unitree-r1']`, all_videos_reviewed_good=`True`, manifest_review_consistent=`True`, failed_review_count=`0`, checkpoint_bound_local_policy_videos_ok=`True`, checkpoint_bound_video_count=`15`, checkpoint_bound_profiles=`['asimov-1', 'hiwonder-ainex', 'unitree-g1', 'unitree-h1', 'unitree-r1']` | none |
| `checkpoint_bound_local_policy_videos_reviewed` | `proved` | checkpoint_video_validation_ok=`True`, checkpoint_bound_local_policy_videos_ok=`True`, checkpoint_bound_video_count=`15`, checkpoint_bound_expected_video_count=`15`, checkpoint_bound_profiles=`['asimov-1', 'hiwonder-ainex', 'unitree-g1', 'unitree-h1', 'unitree-r1']`, checkpoint_bound_actions=`['combined_actions', 'stand_up', 'walk_forward']`, policy_source_ok_count=`15`, task_signal_ok_count=`15`, all_expected_reviewed=`True`, telemetry_failed_count=`0` | none |
...
| `nebius_production_training_complete` | `missing` | objective_audit_failed=`True`, production_blocker=`nebius_cli_auth_required` | Nebius production training is still gated by CLI auth or missing production artifacts |
...
| steps | `4000` |
| winner | `ppo` |
| untrained mean reward | `409.255` |
| Alberta mean reward | `409.193` |
| PPO mean reward | `409.255` |
| Alberta minus untrained | `-0.062007` |
| PPO minus untrained | `-0.000465055` |
| Alberta minus PPO | `-0.0615419` |
| Alberta >= PPO | `False` |
| min mean steps survived | `120` |
| Alberta min mean steps survived | `120` |
| PPO min mean steps survived | `120` |
...
| `brax_mjx_ppo` | SOTA-style accelerator PPO baseline | preflight ok: `True`, script present: `True`, manifest present: `False`, contract artifact ok: `True`, contract only: `True`, production training: `False`, contract profile: `asimov-1`, contract steps: `8` |
...
| field | value |
|---|---:|
| path | `/home/shaw/milady/eliza/packages/robot/evidence/alberta_obstacle_course_sac_smoke` |
| env | `obstacle_course` |
| tasks | `2` |
| steps per task | `40` |
| eval episodes | `1` |
...
| PPO BWT | `0.561449` |
| Alberta forgetting | `0` |
| PPO forgetting | `0.033605` |
| Alberta FWT | `0` |
| PPO FWT | `0.569066` |
| ACC delta | `1.20296` |
| forgetting delta | `-0.033605` |
| Alberta new-task gain | `3.50647` |
| PPO new-task gain | `1.87553` |
| Alberta task-0 retention delta | `0` |
| PPO task-0 retention delta | `1.78516` |
| Alberta ACC >= PPO | `True` |
| Alberta forgetting <= PPO | `True` |
| demo video ok | `True` |
| demo video | `evidence/alberta_obstacle_course_local_4task/obstacle_course_demo.mp4` |
| demo frames | `72` |
| visual review | `good` |
| demo video file exists | `True` |
| demo video bytes match | `True` |
...
| min frame count | `40` |
| min visual progress | `0.00164222` |
| mean visual progress | `0.00557321` |
| contact sheets | `25` |
| existing contact sheets | `25` |
...
|---|---|---|---:|---:|---|---|
| `asimov-1` | `combined_actions` | `good` | `160` | `0.00441142` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/asimov-1_asimov-1_combined_actions_contact.jpg` exists=`True` | Sampled frames show a nonblank robot sequence across the combined action script with measurable frame-to-frame or centroid progress. |
| `asimov-1` | `stand_up` | `good` | `40` | `0.00465354` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/asimov-1_asimov-1_stand_up_contact.jpg` exists=`True` | Sampled frames show nonblank robot motion for `stand up` with measurable frame-to-frame or centroid progress. |
| `asimov-1` | `turn_left` | `good` | `40` | `0.00441353` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/asimov-1_asimov-1_turn_left_contact.jpg` exists=`True` | Sampled frames show nonblank robot motion for `turn left` with measurable frame-to-frame or centroid progress. |
| `asimov-1` | `turn_right` | `good` | `40` | `0.004595` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/asimov-1_asimov-1_turn_right_contact.jpg` exists=`True` | Sampled frames show nonblank robot motion for `turn right` with measurable frame-to-frame or centroid progress. |
| `asimov-1` | `walk_forward` | `good` | `40` | `0.00447918` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/asimov-1_asimov-1_walk_forward_contact.jpg` exists=`True` | Sampled frames show nonblank robot motion for `walk forward` with measurable frame-to-frame or centroid progress. |
| `hiwonder-ainex` | `combined_actions` | `good` | `160` | `0.00273278` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/hiwonder-ainex_hiwonder-ainex_combined_actions_contact.jpg` exists=`True` | Sampled frames show a nonblank robot sequence across the combined action script with measurable frame-to-frame or centroid progress. |
| `hiwonder-ainex` | `stand_up` | `good` | `40` | `0.00536232` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/hiwonder-ainex_hiwonder-ainex_stand_up_contact.jpg` exists=`True` | Sampled frames show nonblank robot motion for `stand up` with measurable frame-to-frame or centroid progress. |
| `hiwonder-ainex` | `turn_left` | `good` | `40` | `0.00536232` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/hiwonder-ainex_hiwonder-ainex_turn_left_contact.jpg` exists=`True` | Sampled frames show nonblank robot motion for `turn left` with measurable frame-to-frame or centroid progress. |
| `hiwonder-ainex` | `turn_right` | `good` | `40` | `0.00536232` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/hiwonder-ainex_hiwonder-ainex_turn_right_contact.jpg` exists=`True` | Sampled frames show nonblank robot motion for `turn right` with measurable frame-to-frame or centroid progress. |
| `hiwonder-ainex` | `walk_forward` | `good` | `40` | `0.00536232` | `/home/shaw/milady/eliza/packages/robot/evidence/video_review/hiwonder-ainex_hiwonder-ainex_walk_forward_contact.jpg` exists=`True` | Sampled frames show nonblank robot motion for `walk forward` with measurable frame-to-frame or centroid progress. |
...
| Alberta obstacle-course advantage | `True` |
...
| obstacle continual learning | Continual obstacle-course evidence supports Alberta over PPO on adaptation/retention. |
| methods compared | `stable-baselines3 PPO, stable-baselines3 SAC, Brax/MJX PPO preflight, Brax/MJX PPO contract artifact` |
| Alberta >= PPO robot-backend count | `0` / `2` |
| obstacle advantage supported | `True` |
