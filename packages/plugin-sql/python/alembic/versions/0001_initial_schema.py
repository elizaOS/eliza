...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('message_server_id', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('world_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('worlds.id', ondelete='SET NULL'), nullable=True),
        sa.Column('metadata', postgresql.JSON(), default={}),
        sa.Column('created_at', sa.DateTime(), server_default=sa.func.now()),
    )
    ...
    op.create_unique_constraint('uq_participants_entity_room', 'participants', ['entity_id', 'room_id'])
