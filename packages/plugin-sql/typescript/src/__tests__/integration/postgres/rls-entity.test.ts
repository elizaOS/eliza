...
describe.skipIf(!process.env.POSTGRES_URL)(
  "PostgreSQL RLS Entity Integration",
  () => {
    ...
    beforeAll(async () => {
      ...
      `);
        ...
        throw new Error(
          "RLS tests require superuser access for cleanup and data setup",
        );
      }
      ...
      try {
        ...
        try {
          ...
          await applyEntityRLSToAllTables(mockAdapter);
          console.log(
            "[RLS Test] RLS policies applied (functions already exist)",
          );
        } catch (policyErr) {
          console.log(
      ...
      try {
        await setupClient.query(
          `GRANT ALL ON ALL TABLES IN SCHEMA public TO eliza_test`,
        );
        await setupClient.query(
          `GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO eliza_test`,
        );
      } catch (_err) {
        // Ignore if already granted or permission denied (we're already eliza_test)
        console.log(
          "[RLS Test] Permission grant skipped (may already be granted)",
        );
      }
    ...
    afterAll(async () => {
      ...
      try {
        await superuserClient.query(
          `DELETE FROM memories WHERE room_id IN ($1, $2)`,
          [room1Id, room2Id],
        );
        await superuserClient.query(
          `DELETE FROM participants WHERE room_id IN ($1, $2)`,
          [room1Id, room2Id],
        );
        await superuserClient.query(`DELETE FROM rooms WHERE id IN ($1, $2)`, [
          room1Id,
          room2Id,
        ]);
        await superuserClient.query(
        ...
        await superuserClient.query(`DELETE FROM agents WHERE id = $1`, [
          agentId,
        ]);
        await superuserClient.query(`DELETE FROM servers WHERE id = $1`, [
          serverId,
        ]);
      } catch (err) {
    ...
    it("should allow Alice to see room1 memories (tests withEntityContext + sql.raw fix)", async () => {
      ...
      const result = await manager.withEntityContext(
        ...
        async (tx) => {
          return await tx.execute(
            sql`SELECT id, room_id, content FROM memories`,
          );
        },
      );
      ...
      expect((result.rows[0].content as { text: string }).text).toContain(
        "room1",
      );
    });
    ...
    it("should allow Bob to see BOTH room1 and room2 memories", async () => {
      const result = await manager.withEntityContext(
        ...
        async (tx) => {
          return await tx.execute(
            sql`SELECT id, room_id, content FROM memories ORDER BY room_id`,
          );
        },
      );
      ...
      expect(result.rows.map((r: { room_id: string }) => r.room_id)).toContain(
        room1Id,
      );
      expect(result.rows.map((r: { room_id: string }) => r.room_id)).toContain(
        room2Id,
    ...
    it("should combine Server RLS + Entity RLS (double isolation)", async () => {
      ...
      const wrongServerManager = new PostgresConnectionManager(
        POSTGRES_URL,
        wrongServerId,
      );
      ...
      try {
        ...
        const result = await wrongServerManager.withEntityContext(
          aliceId as UUID,
          async (tx) => {
            return await tx.execute(
