  ...
  describe("Agent Tests", () => {
    ...
    describe("createAgent", () => {
      ...
      it("should create agent with complex settings structure", async () => {
        ...
        const settings = createdAgent?.settings as
          | Record<string, unknown>
          | undefined;
        const apiSettings = settings?.apiSettings as
          | Record<string, unknown>
          | undefined;
        const endpoints = apiSettings?.endpoints as
          | Record<string, unknown>
          | undefined;
        const auth = apiSettings?.auth as Record<string, unknown> | undefined;
    ...
    describe("updateAgent", () => {
      ...
      it("should handle deep nested settings objects", async () => {
        ...
        const updatedAgent = await adapter.getAgent(newAgent.id);
        if (updatedAgent?.settings?.level1) {
    ...
    describe("deleteAgent", () => {
      ...
      it("should cascade delete all related data when deleting an agent", async () => {
        ...
        try {
          ...
          expect(
            (await cascadeAdapter.getEntitiesByIds([entityId1, entityId2]))
              ?.length,
          ).toBe(2);
