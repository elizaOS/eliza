...
const worldId = stringToUuid("example-app-world");
const userId = stringToUuid("example-app-user");
...
async function getOrCreateRuntime(config: AppConfig): Promise<RuntimeBundle> {
  ...
  initializing = (async () => {
    ...
    const roomId = stringToUuid("example-app-room");
...
export async function getHistory(config: AppConfig): Promise<ChatMessage[]> {
