...
export class App {
  ...
  private async checkInterruptedTasks(): Promise<void> {
    ...
    try {
      ...
      // error-policy:J4 Startup recovery failures remain visible in the chat
      // without preventing the user from starting a new task.
      const state = useStore.getState();
      state.addMessage(
        state.currentRoomId,
        "system",
        `Unable to inspect interrupted tasks: ${error instanceof Error ? error.message : String(error)}`,
      );
      this.tui.requestRender();
    }
  }
...
During a turn: Enter queues the message, Esc/Ctrl+C aborts (queued messages are discarded), second Ctrl+C quits`,
  ...
  private async handleSendMessage(text: string): Promise<void> {
    ...
    // Queue-and-send behavior: a submission made while a turn is
