...
2. Else auto-detect: `OPENAI_API_KEY`, `ELIZA_CODE_API_KEY`, or
   `CEREBRAS_API_KEY` → `openai`; `ANTHROPIC_API_KEY` → `anthropic`.

`applyElizaCodeProviderEnv(env)` maps the `ELIZA_CODE_*` knobs onto the
`OPENAI_*` ones the provider plugin reads (and pins
`ELIZA_CODE_PROVIDER=openai` when it inherits the elizaos key), so the
