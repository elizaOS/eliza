...
function reqKey(body) {
  ...
  return createHash("sha256")
    .update(JSON.stringify({ model: body.model, msgs, tools: body.tools ?? null }))
    .digest("hex");
}
...
if (MODE === "replay") {
  ...
  for (const r of recorded) byKey.set(r.key, r.response);
  ...
  req.on("end", async () => {
    if (!req.url.includes("/chat/completions")) {
      ...
      if (MODE === "record") {
        ...
        res.writeHead(up?.status ?? 200, { "Content-Type": "application/json" });
        return res.end(txt);
      }
    ...
    if (MODE === "replay") {
      const hit = byKey.get(key) ?? recorded[seq]?.rec;
      seq += 1;
      if (!hit) {
        console.error(`[llm-proxy] replay MISS turn ${seq} key=${key.slice(0, 12)}`);
        res.writeHead(500);
        return res.end('{"error":"no recorded response for this request"}');
      }
      res.writeHead(hit.status, { "Content-Type": hit.contentType });
      return res.end(Buffer.from(hit.bodyB64, "base64"));
    }
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      console.error(`[llm-proxy] 429 tpm-limit, waiting ${retryAfter}s (attempt ${attempt + 1}/8)`);
      await new Promise((r) => setTimeout(r, retryAfter * 1000));
    }
    const raw = Buffer.from(await up.arrayBuffer());
    const contentType = up.headers.get("content-type") || "application/json";
    const rec = { status: up.status, contentType, bodyB64: raw.toString("base64") };
    recorded.push({ key, seq: seq++, rec });
    if (recorded.length % 1 === 0) writeFileSync(RECORDING, JSON.stringify(recorded));
