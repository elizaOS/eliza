...
- **`scripts/write-dist-tsconfig.mjs` runs as the last `build` step** and emits a paths-free `dist/tsconfig.json`. Bun applies the nearest tsconfig's `compilerOptions.paths` **at runtime**; source typechecking resolves built workspace package declarations through `node_modules`, and the built package resolves the corresponding runtime exports there. The shadow config keeps `dist/` independent of any caller-provided workspace aliases. Removing the step silently re-breaks every cockpit terminal spawn (#11043).
