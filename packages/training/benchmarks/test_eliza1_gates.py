...
def test_all_passing_blob_passes_and_skips_hardware_gates() -> None:
    rep: GateReport = apply_gates(
        {"tier": "2b", "mode": "full", "results": _full_results()}
    )
...
def test_missing_required_measurement_is_publish_blocking() -> None:
    ...
    rep = apply_gates({"tier": "2b", "mode": "full", "results": results})
...
def test_failing_numeric_gate_blocks() -> None:
    # 2b text_eval threshold is 0.60; 0.40 fails (>=).
    rep = apply_gates({"tier": "2b", "mode": "full", "results": _full_results(text_eval=0.40)})
    assert rep.passed is False
...
def test_voice_rtf_uses_le_comparison() -> None:
    # voice_rtf is "<=": 0.30 passes the 0.45 2b threshold, 0.80 fails
    # visibly but does not block while the threshold remains provisional.
    ok = apply_gates({"tier": "2b", "mode": "full", "results": _full_results(voice_rtf=0.30)})
    bad = apply_gates({"tier": "2b", "mode": "full", "results": _full_results(voice_rtf=0.80)})
...
def test_bool_gate_requires_true() -> None:
    bad = apply_gates({"tier": "2b", "mode": "full", "results": _full_results(thirty_turn_ok=False)})
...
def test_non_required_gate_failure_does_not_block() -> None:
    # first_token_latency_ms is required:false for 2b -> failing it must not
    # flip the verdict.
    rep = apply_gates({"tier": "2b", "mode": "full", "results": _full_results(first_token_latency_ms=99999.0)})
    assert rep.passed is True
...
def test_bare_results_with_explicit_tier() -> None:
    rep = apply_gates(_full_results(), "eliza-1-2b")
    assert rep.passed is True


def test_smoke_mode_runs_only_structural_gates() -> None:
    rep = apply_gates(
        {"tier": "2b", "mode": "smoke", "results": {"format_ok": 0.7, "format_ok_base": 0.5}}
    )
...
def test_report_to_dict_shape() -> None:
    rep = apply_gates({"tier": "2b", "mode": "full", "results": _full_results()})
    d = rep.to_dict()
...
def test_different_tiers_have_different_thresholds() -> None:
    # 4b text_eval threshold (0.62) is tighter than 2b (0.60); 0.61
    # passes 2b but fails 4b.
    res = _full_results(text_eval=0.61)
    assert apply_gates({"tier": "2b", "mode": "full", "results": res}).passed is True
    assert apply_gates({"tier": "4b", "mode": "full", "results": res}).passed is False
