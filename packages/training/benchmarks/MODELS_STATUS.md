...
|---|---|---|---|---|
| `eliza-1-0_6b` | ✅ APOLLO `apollo_mini` full-param, 8000 samples / 1 epoch, eval_loss 1.315 (`checkpoints/eliza-1-0_6b-apollo-1778551769/final/`). Bench vs base `Qwen3-0.6B`: **beats baseline on every metric, regresses none** (`format_ok` 0.0857→0.20, `reply` parse-errors 8→0, `claude_distill` format 27.3%→63.6%, gen tps +33%) — but does NOT clear the absolute `format_ok` publish floor on the 35-row smoke corpus → **conditional go**: PUBLISH blocks `defaultEligible` until a full-corpus run lands a green `format_ok` gate; see `reports/eliza1-0_6b-apollo-sft-2026-05-11.md` (+ `.manifest.json`). Full-corpus run on H200: `bash scripts/cloud/run-on-cloud.sh --provider nebius --task train --gpu h200 --tier 0_6b --yes-i-will-pay` (delegates to `train_nebius.sh full`; fail-closed, needs `NEBIUS_PROJECT_ID`). No DFlash drafter for this tier (no smaller Qwen3 base) → nothing to re-stamp. | ✅ Q4_K_M, 396 MB | ✅ sidecars applied (`polarquant_artifacts.safetensors` + `qjl_config.json` + `turboquant.json` + `eliza1_manifest.json`); GGUF body is **Q8_0**, not native `Q4_POLAR` — `weight_quant.deferred: true` (the fork's `convert_hf_to_gguf.py` doesn't emit `q4_polar` yet; runtime kernels exist) | Q4_K_M: ~27.8 k pp512 / ~384 tg128 @ d0, ~6.6 k pp / ~125 tg @ d16k. eliza1-bundle Q8_0: ~31 k pp512 / ~392 tg128 @ d0 |
