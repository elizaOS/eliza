...
generate trajectories from generic seeds. This module closes the loop:
  ...
  1. read recent benchmark failures from JSON artifacts written by
     benchmark adapters,
...
Strict layering, no business logic in the CLI:
  ...
  - :class:`FailureSource` (Protocol) — yields :class:`BenchmarkFailure`
    rows. :class:`JsonArtifactFailureSource` reads JSON / JSONL files.
...
::
    ...
    python -m synth.adaptive_synth \\
        --since 2025-05-01 \\
        --variations 3 \\
        --artifacts-dir data/benchmarks/runs/ \\
...
class BenchmarkFailure:
    """A single failed benchmark run pulled from a JSON artifact.
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        required=True,
        help=(
            "Root directory of benchmark JSON/JSONL run artifacts to read "
            "failures from."
        ),
    )
    ap.add_argument(
