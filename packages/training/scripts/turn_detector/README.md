...
- `finetune_turn_detector.py` — LoRA / APOLLO finetune entrypoint. Reads a
  ...
  text-only path (Option A in the R1 research note — internal, not committed to this repository)
...
`BargeInCancelToken.signal` with reason `"turn-suppressed"`. See the R11 cancellation research note (internal, not committed to this
repository).

## See also

- R1 research report (internal, not committed to this repository) — full
