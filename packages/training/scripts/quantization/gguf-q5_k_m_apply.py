...
Wraps llama.cpp's two-stage GGUF conversion:
  ...
  2. ``llama-quantize`` — quantizes that f16 GGUF down to Q5_K_M (5-bit
     K-quant, mixed precision: 5-bit body + 6-bit attention.w + Q6_K
     embed_tokens). ~5.5 bits per weight average; near-lossless against
     bf16 on most ppl/wikitext metrics.

Output is written to ``<output>/eliza-1-<size>-Q5_K_M.gguf``, matching the
sibling K-quant levels (``-Q3_K_M``, ``-Q4_K_M``, ``-Q6_K``) so the publish
...
importance matrix when present.
"""
...
log = logging.getLogger("gguf_q5_k_m_apply")
...
def _resolve_output_basename(model_id_or_path: str, output_dir: Path) -> str:
    """Pick ``eliza-1-<size>-Q5_K_M.gguf`` (or fall back to last segment)."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    sidecar = {
        ...
        "smoke_load": smoke,
        "notes": (
            "Q5_K_M is the second-largest K-quant in the canonical "
            "llama.cpp ladder (~5.5 bits per weight on average). "
            "Near-lossless vs bf16 on standard ppl/wikitext metrics; "
            "the high-quality option for users with more VRAM than the "
            "Q4_K_M sweet-spot target."
        ),
    }
    sidecar_path = write_sidecar(args.output, "gguf_q5_k_m.json", sidecar)
