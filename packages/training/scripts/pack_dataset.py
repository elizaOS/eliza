...
def main() -> int:
    ...
    # Tier-based per-source caps (DATASET_REVIEW.md §"Concrete filter").
    # Eliza-aligned data gets full coverage; over-represented sources get
    # capped; broken sources are weight=0 in datasets.yaml so they
    # naturally produce 0 records.
    TIER_S = {  # gold standard
        "nubilio-trajectories",
    }
    TIER_A = {  # eliza-aligned bench
    ...
    TIER_D = {  # pure reasoning/coding, over-represented
    ...
    e_sources = [(s, n) for s, n in counts.items() if tier_for(s) == "E"]
    f_sources = [(s, n) for s, n in counts.items() if tier_for(s) == "F"]
    e_total = sum(n for _, n in e_sources)
    f_total = sum(n for _, n in f_sources)
    ...
    for slug, n in counts.items():
        tier = tier_for(slug)
        if tier == "E":
            # Proportional split of the combined budget.
        ...
        # CLI override: when --per-source-cap is set < tier cap, respect it.
        # When --no-weights is set, fall back to a flat cap.
        if args.no_weights:
            t = min(args.per_source_cap, n)
        elif args.per_source_cap and args.per_source_cap < t:
            t = args.per_source_cap
    ...
    with train_path.open("w", encoding="utf-8") as ftr, \
        ...
        for slug, path in sources:
            ...
            if not path.exists():
                ...
                continue
            log.info("  sampling %s: %d/%d", slug, k, n)
            keep = reservoir_sample_indices(n, k, rng)
