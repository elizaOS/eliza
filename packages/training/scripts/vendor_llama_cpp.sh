...
#
# packages/training/vendor/llama.cpp is a git submodule (see .gitmodules,
# pinned to upstream tag b6650). This script materializes it (git submodule
# update --init, or a plain clone if run outside the repo), builds the
...
# --- obtain the checkout (idempotent) -------------------------------------
# Preferred: this is a git submodule (packages/training/vendor/llama.cpp,
# pinned in .gitmodules) — `git submodule update --init` materializes it at
# the pinned commit. Fallback: a plain clone of $LLAMA_CPP_TAG when this
# script is run outside the repo (or LLAMA_CPP_VENDOR_DIR was overridden to a
# non-submodule path).
REPO_ROOT="$(git -C "$TRAINING_ROOT" rev-parse --show-toplevel 2>/dev/null || echo '')"
if [[ -d "$VENDOR_DIR/.git" || -f "$VENDOR_DIR/.git" ]]; then
  log "checkout already present at $VENDOR_DIR — reusing"
elif [[ -n "$REPO_ROOT" && -f "$REPO_ROOT/.gitmodules" ]] \
     && git -C "$REPO_ROOT" config -f .gitmodules --get-regexp 'submodule\..*\.path' 2>/dev/null \
        | grep -q "packages/training/vendor/llama.cpp" \
     && [[ "$VENDOR_DIR" == "$TRAINING_ROOT/vendor/llama.cpp" ]]; then
  log "initializing submodule packages/training/vendor/llama.cpp"
  git -C "$REPO_ROOT" submodule update --init --depth 1 -- packages/training/vendor/llama.cpp \
    || git -C "$REPO_ROOT" submodule update --init -- packages/training/vendor/llama.cpp \
    || die "git submodule update failed for packages/training/vendor/llama.cpp"
else
  mkdir -p "$(dirname "$VENDOR_DIR")"
  log "cloning $GIT_URL @ $LLAMA_CPP_TAG -> $VENDOR_DIR (non-submodule path)"
...
[[ -f "$VENDOR_DIR/convert_hf_to_gguf.py" ]] || die "no convert_hf_to_gguf.py in $VENDOR_DIR after checkout"
