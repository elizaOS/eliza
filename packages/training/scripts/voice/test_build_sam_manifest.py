...
def test_dry_run_emits_manifest_and_excludes_hallucination(tmp_path: Path) -> None:
    ...
    s002 = next(r for r in records if r["id"] == "samantha_002")
    ...
    assert all(not row.startswith("samantha_002|") for row in csv_rows)
...
def test_gitignore_carve_out_tracks_manifest_but_ignores_audio(tmp_path: Path) -> None:
    ...
    tracked = [
        samantha_root / "manifest.jsonl",
        samantha_root / "source.json",
        samantha_root / "README.md",
        samantha_root / "ljspeech" / "metadata.csv",
    ]
    for path in tracked:
    ...
    ignored = [
        samantha_root / "audio" / "samantha_001.wav",
        samantha_root / "raw" / "samantha_001.wav",
        samantha_root / "raw" / "samantha_001.txt",
        samantha_root / "ljspeech" / "wavs" / "samantha_001.wav",
    ]
    for path in ignored:
        rel = path.relative_to(repo_root)
        proc = subprocess.run(
