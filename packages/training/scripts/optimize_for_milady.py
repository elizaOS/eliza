...
``milady_manifest.json`` declares the full applied stack so phones can
...
log = logging.getLogger("optimize_for_milady")
...
def _build_milady_manifest(
    ...
    return {
        "schema_version": 1,
        "produced_by": "scripts/optimize_for_milady.py",
        ...
        "runtime": {
            ...
            # intentionally a flat command shape — no Milady-side string
...
def _push_to_hf(
    ...
    """Drive ``push_model_to_hf.py --milady-manifest`` to publish.

    We surface the new ``--milady-manifest`` flag added in this change
...
def execute_plan(plan: OptimizationPlan) -> int:
    ...
    gguf_filename = f"{base_slug}-milady-Q4_POLAR.gguf"
    ...
    manifest = _build_milady_manifest(plan, stages, gguf_path, None)
    manifest_path = gguf_dir / "milady_manifest.json"
    if not plan.dry_run:
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--base-model",
        required=True,
        help="HF repo id or local path to the base model (e.g. elizaos/eliza-1-lite-0_6b).",
    )
    p.add_argument(
        "--output-dir",
        type=Path,
    ...
    p.add_argument(
        "--hf-repo",
        default=None,
        help="HuggingFace repo to publish to (e.g. elizaos/eliza-1-lite-0_6b). "
