 ...
 *
 * Wraps `@elizaos/credentials/vault`'s master-key resolution (OS keychain → scrypt passphrase)
...
export interface LocalKmsOptions {
  /** 32-byte root key. Caller resolves via `@elizaos/credentials/vault` master-key API. */
  rootKey: Uint8Array;
}
