...
import { evaluateExpression, setErrorHandler } from './expression-evaluator-proxy.js';
import { sanitizer, sanitizerName } from './expression-sandboxing.js';
import { isExpression } from './expressions/expression-helpers.js';
import { extendSyntax } from './extensions/expression-extension.js';
import { extendedFunctions } from './extensions/extended-functions.js';
import { extend, extendOptional } from './extensions/index.js';
import type {
	IDataObject,
	INodeParameters,
...
export class Expression {
	...
	/**
	 * Sandbox-isolate lifecycle hooks. The isolated-vm path is opt-in and
	 * disabled by default in the in-process embedded engine, so these are
	 * no-ops here. Consumers that wire a real isolate pool can override.
	 */
	...
	resolveSimpleParameterValue(
		...
		data.process =
			typeof process !== 'undefined'
				? {
						arch: process.arch,
						env: process.env.WORKFLOWS_BLOCK_ENV_ACCESS_IN_NODE !== 'false' ? {} : process.env,
