...
import { augmentArray, augmentObject } from './augment-object.js';
import { BINARY_MODE_COMBINED, SCRIPTING_NODE_TYPES } from './constants.js';
...
export class WorkflowDataProxy {
		...
		return new Proxy(node?.parameters ?? {}, {
			...
			get(target, name) {
				...
				if (name[0] === '&') {
					...
					if (!Object.hasOwn(that.siblingParameters, key)) {
					...
					if (!Object.hasOwn(node.parameters, name)) {
	...
	private getNodeExecutionData(
		...
		if (!shortSyntax) {
			...
			if (!this.workflow.getNode(nodeName)) {
			...
			if (
				!Object.hasOwn(this.runExecutionData.resultData.runData, nodeName) &&
				!getPinDataIfManualExecution(this.workflow, nodeName, this.mode)
			) {
				throw new ExpressionError(`Node '${nodeName}' hasn't been executed`, {
					...
					descriptionKey: isScriptingNode(nodeName, this.workflow)
			...
			runIndex = runIndex === undefined ? this.defaultReturnRunIndex : runIndex;
			runIndex =
				runIndex === -1 ? this.runExecutionData.resultData.runData[nodeName].length - 1 : runIndex;

			if (this.runExecutionData.resultData.runData[nodeName].length <= runIndex) {
			...
			if (outputIndex === undefined) {
				const nodeConnection = this.workflow.getNodeConnectionIndexes(
					this.contextNodeName,
					nodeName,
					NodeConnectionTypes.Main
				);

				if (nodeConnection === undefined) {
					throw new ExpressionError(`connect "${this.contextNodeName}" to "${nodeName}"`, {
						runIndex: this.runIndex,
						itemIndex: this.itemIndex,
					});
				}
			...
			executionData = this.connectionInputData;
		}
	...
	getDataProxy(opts?: { throwOnMissingExecutionData: boolean }): IWorkflowDataProxyData {
		...
		const createExpressionError = (
			...
			if (context?.nodeCause) {
				...
				if (context.moreInfoLink && (pinData || isScriptingNode(nodeName, that.workflow))) {
					const moreInfoLink =
						' <a target="_blank" href="https://docs.workflows.io/data/data-mapping/data-item-linking/item-linking-errors/">More info</a>';
		...
		const handleFromAi = (
			...
			let placeholdersDataInputData: IDataObject | undefined;

			if (!resultData) {
				const inputData = this.connectionInputData?.[runIndex];
		...
		const base = {
			...
			// this is historical syntax that is not documented
			...
			$fromai: handleFromAi,
			$fromAi: handleFromAi,
			// this is a historical syntax that is not documented
