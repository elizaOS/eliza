 ...
 *
 * Lives in workflows-workflow so it can be shared by:
...
export const formatWorkflowStructureIssuePath = (path: ReadonlyArray<PropertyKey>): string => {
	if (path.length === 0) return 'workflow';

	return path.reduce<string>((acc, segment) => {
		if (typeof segment === 'number') return `${acc}[${segment}]`;
		const key = typeof segment === 'symbol' ? (segment.description ?? String(segment)) : segment;
