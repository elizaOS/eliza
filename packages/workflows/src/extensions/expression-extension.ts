// We pin ast-types to 0.15.2 to match the version bundled by recast 0.22,
// so node-builder return types unify between our imports and recast's.
...
import { isSafeObjectProperty } from '../utils.js';
import { arrayExtensions } from './array-extensions.js';
import { booleanExtensions } from './boolean-extensions.js';
...
import { checkIfValueDefinedOrThrow } from './utils.js';
...
function isDate(input: unknown): boolean {
	...
	return d instanceof Date && !Number.isNaN(d.valueOf()) && d.toISOString() === input;
}
