...
function parseArgs(argv) {
  const args = {
    ...
    steps: (process.env.ELIZA_TTS_SWEEP_STEPS || "4,6,8,10,12,16").split(",").map((s) => Number.parseInt(s, 10)).filter(Number.isFinite),
    chunkStep: Number.parseInt(process.env.ELIZA_TTS_SWEEP_CHUNK_STEP || "8", 10),
    ngl: process.env.ELIZA_TTS_SWEEP_NGL || "99",
    threads: Number.parseInt(process.env.ELIZA_TTS_SWEEP_THREADS || String(Math.min(os.cpus().length, 12)), 10),
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (a === "--bundle") args.bundle = next();
    ...
    else if (a === "--steps") args.steps = next().split(",").map((s) => Number.parseInt(s, 10)).filter(Number.isFinite);
      ...
      console.log("Usage: bun packages/inference/verify/tts_step_sweep.mjs --bundle <dir> [--report out.json] [--steps 4,6,8] [--skip-chunk-sweep]");
      process.exit(0);
    } else {
...
function stateRoot() {
  return process.env.ELIZA_STATE_DIR?.trim() || process.env.MILADY_STATE_DIR?.trim() || path.join(os.homedir(), ".eliza");
}

function platformTag() {
  const sys = { darwin: "darwin", linux: "linux", win32: "windows" }[process.platform] || process.platform;
...
function discoverEngine(backend, explicitBinDir) {
  ...
  const dirs = fs.existsSync(root)
    ...
    : [];
  const pick = dirs.find((d) => d === prefer) || dirs.find((d) => d.startsWith(plat) && d.includes(`-${backend}`) && d.includes("-fused"));
...
function validateEngineDir(dir) {
  ...
  if (fs.existsSync(capsPath)) caps = JSON.parse(fs.readFileSync(capsPath, "utf8"));
...
function bundleFiles(bundleDir) {
  ...
  return {
    text: firstExisting(...fs.readdirSync(textDir).filter((f) => f.endsWith(".gguf")).sort().map((f) => path.join(textDir, f))),
    drafter: firstExisting(...fs.readdirSync(dflashDir).filter((f) => f.endsWith(".gguf")).map((f) => path.join(dflashDir, f))),
    ttsModel: path.join(ttsDir, ttsGgufs.find((f) => !/token/i.test(f)) || ttsGgufs[0]),
    ttsCodec: path.join(ttsDir, ttsGgufs.find((f) => /token/i.test(f)) || ttsGgufs[0]),
    asr: firstExisting(path.join(asrDir, "eliza-1-asr.gguf"), ...fs.readdirSync(asrDir).filter((f) => f.endsWith(".gguf") && !/mmproj/i.test(f)).map((f) => path.join(asrDir, f))),
    manifest: fs.existsSync(path.join(bundleDir, "eliza-1.manifest.json"))
      ? JSON.parse(fs.readFileSync(path.join(bundleDir, "eliza-1.manifest.json"), "utf8"))
      : null,
  };
}
...
async function loadFfi(libPath, libDir) {
  ...
  if (process.platform === "darwin") process.env.DYLD_LIBRARY_PATH = `${libDir}${sep}${process.env.DYLD_LIBRARY_PATH || ""}`;
  ...
  const lib = ffi.dlopen(libPath, {
    ...
    eliza_inference_asr_transcribe: { args: [T.ptr, T.ptr, T.usize, T.i32, T.ptr, T.usize, T.ptr], returns: T.i32 },
...
function readErrAndFree(ffi, s, ptrBuf) {
  let p = 0n;
  try { p = ffi.read.ptr(ptrBuf, 0); } catch {}
  if (!p || p === 0n) return "(no diagnostic)";
  let msg = "(unreadable diagnostic)";
  try { msg = ffi.CString(p); } catch {}
  try { s.eliza_inference_free_string(p); } catch {}
  return msg;
}
...
function asrTranscribeFloat32(ffiCtx, ffi, s, samples, sampleRate) {
  ...
  const rc = s.eliza_inference_asr_transcribe(ffiCtx, ffi.ptr(pcmBuf), BigInt(pcm16k.length), 16000, ffi.ptr(outBuf), BigInt(outBuf.length), ffi.ptr(errBuf));
  const latencyMs = performance.now() - t0;
  if (rc < 0) throw new Error(`asr_transcribe rc=${rc}: ${readErrAndFree(ffi, s, errBuf)}`);
...
async function synth(port, text, step, duration) {
  ...
  const res = await fetch(`http://127.0.0.1:${port}/v1/audio/speech`, {
    ...
    body: JSON.stringify({ input: text, response_format: "pcm", num_step: step, duration }),
    signal: AbortSignal.timeout(180000),
  });
  if (!res.ok) throw new Error(`/v1/audio/speech ${res.status}: ${(await res.text()).slice(0, 200)}`);
  ...
  return { sampleRate, samples, wallMs, audioSec, rtf: wallMs / 1000 / audioSec };
}
...
function parseCodecBackendLog(logText) {
  const fallback = logText.match(/\[PipelineCodec\] Metal codec fallback: requested=([^\s]+) selected=([^\s]+) reason=([^\n]+)/);
  const loaded = logText.match(/\[PipelineCodec\] Loaded codec: sr=(\d+) hop=(\d+) backend=([^\s]+)/);
  return {
...
async function main() {
  if (typeof Bun === "undefined") throw new Error("tts_step_sweep.mjs requires Bun");
  ...
  const audioDir = args.audioDir || path.join(path.dirname(args.report || path.join(__dirname, "bench_results", "tts-step-sweep.json")), "tts-step-sweep-audio");
  ...
  try {
    ...
    const serverArgs = [
      "-m", files.text, "-md", files.drafter, "--spec-type", "dflash",
      "--draft-min", "2", "--draft-max", "6", "--port", String(port),
      "-c", String(args.ctx), "-cd", String(args.ctx), "-ngl", String(args.ngl),
      "-t", String(args.threads), "--no-warmup", "--metrics",
    ];
    const srvLog = fs.createWriteStream(path.join(tmpDir, "server.log"));
    child = spawn(engine.server, serverArgs, { cwd: engine.dir, env, stdio: ["ignore", "pipe", "pipe"] });
    ...
    asrErr.fill(0);
    const asrRc = s.eliza_inference_mmap_acquire(ffiCtx, Buffer.from("asr\0", "utf8"), ffi.ptr(asrErr));
    if (asrRc < 0) throw new Error(`mmap_acquire(asr) rc=${asrRc}: ${readErrAndFree(ffi, s, asrErr)}`);

    const stepPhrase = { id: "chunk4_meeting", text: "The meeting is to", duration: 1.18, kind: "step-sweep" };
    const capitalPhrase = { id: "chunk4_capital", text: "The capital of France", duration: 1.18, kind: "step-sweep" };
    const chunkCases = [
      stepPhrase,
      { id: "chunk8_roadmap", text: "The meeting is to discuss the product roadmap", duration: 1.94, kind: "chunk-sweep" },
      { id: "chunk12_roadmap", text: "The meeting is to discuss the product roadmap before lunch", duration: 2.51, kind: "chunk-sweep" },
    ];
    const cases = [
      ...args.steps.flatMap((step) => [stepPhrase, capitalPhrase].map((c) => ({ ...c, step }))),
      ...(args.skipChunkSweep ? [] : chunkCases.filter((c) => c.id !== stepPhrase.id).map((c) => ({ ...c, step: args.chunkStep }))),
    ];
    ...
    for (const c of cases) {
      ...
      const asrWer = asr ? round4(wordErrorRate(c.text, asr.transcript)) : null;
      rows.push({
        id: c.id,
        kind: c.kind,
        text: c.text,
      ...
      console.log(`[tts-sweep] ${c.id} steps=${c.step} rtf=${rows.at(-1).rtf} wer=${asrWer == null ? "n/a" : asrWer}`);
    }

    const logText = fs.existsSync(path.join(tmpDir, "server.log")) ? fs.readFileSync(path.join(tmpDir, "server.log"), "utf8") : "";
    const report = {
      schemaVersion: 1,
      ...
      engine: { dir: engine.dir, backend: engine.backend, caps: engine.caps?.kernels || null },
      codecBackend: parseCodecBackendLog(logText),
      summary: {
        stepSweep: args.steps.map((step) => {
          const v = rows.filter((r) => r.kind === "step-sweep" && r.steps === step);
          return {
            steps: step,
        ...
        chunkSweep: args.skipChunkSweep
          ? []
          : rows.filter((r) => r.kind === "chunk-sweep" || r.id === stepPhrase.id && r.steps === args.chunkStep),
      },
      rows,
    };
    const reportPath = args.report || path.join(__dirname, "bench_results", `tts-step-sweep-${new Date().toISOString().slice(0, 10)}.json`);
