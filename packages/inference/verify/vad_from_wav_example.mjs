 ...
 * through the app-core VAD implementation and records the emitted events.
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (a === "--bundle") args.bundle = argv[++i] ?? args.bundle;
      ...
      console.log(
        "Usage: node vad_from_wav_example.mjs [--bundle PATH] [--wav PATH] [--report PATH] [--json]",
      );
      process.exit(0);
    }
  }
...
import { createSileroVadDetector, resolveSileroVadPath } from ${JSON.stringify(vadUrl)};
...
const detector = await createSileroVadDetector({
  modelPath,
  bundleRoot,
  config: {
    sampleRate: SR,
...
function main() {
  ...
  if (!runnerRuntime) {
    throw new Error("bun or node --import tsx is required to run app-core VAD");
  }
