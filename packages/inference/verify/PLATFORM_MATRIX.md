...
| `android-arm64-vulkan` | `node packages/app-core/scripts/aosp/compile-libllama.mjs` (NDK cross-build) | `ANDROID_SERIAL=27051JEGR10034 make -C …/verify android-vulkan-smoke` — standalone fixtures **8/8 PASS on Pixel 6a / Mali-G78**, fused QJL/TBQ and QJL/Polar attention normal+causal fixtures **PASS**, built-fork graph dispatch **7/7 PASS** (`hardware-results/android-vulkan-smoke-20260513T021447Z.log`, `hardware-results/android-vulkan-graph-smoke-20260513T021457Z.log`); `vulkan-runtime-dispatch-evidence.json` now marks `android-arm64-vulkan` runtime-ready | `adb`-pushed `vulkan_bench` / `llama-bench` | **verified-here on Mali-G78** for standalone + built-fork graph dispatch | Run the same smoke on a physical Adreno device before claiming Snapdragon coverage. |
...
| `darwin-arm64-metal-fused` | `…/build-llama-cpp-dflash.mjs --target darwin-arm64-metal-fused --jobs 10` — links `omnivoice-core` + `libelizainference.dylib` + `llama-omnivoice-server` + `libmtmd` + `default.metallib`; `verify-symbols.mjs` (`omnivoice=10 abi=8`) | `metal-verify metal-verify-multiblock dispatch-smoke` (same as above) + `verify-symbols.mjs` | Bun FFI smoke against `~/.eliza/local-inference/models/eliza-1-2b.bundle` (loads real OmniVoice Q4_K_M + Eliza-1 ASR for TTS + ASR); `e2e_loop_bench.mjs` | **verified-here on macOS Metal** for the fused dylib FFI smoke (real GGUF-backed TTS + ASR in one fused process) | Built-fork graph-dispatch smoke + full latency/RSS/thermal gates; the fused `llama-server` route on macOS (currently the macOS evidence is the FFI path, not the HTTP route). |
...
~/.cache/eliza-dflash/milady-llama-cpp/build-cuda/bin/llama-bench \
  -m ~/.eliza/local-inference/models/eliza-1-2b.bundle/text/eliza-1-2b-32k.gguf -ngl 99 -p 16,512 -n 32 -fa 1
```
...
**No.** The eliza-1 text backbone (0.8B smallest, fp16/Q4) does not fit a Coral
...
cover. The sidecars (Silero VAD and the Eliza-1 ASR/embedding regions) don't
