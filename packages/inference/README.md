...
> | CUDA (all 5) | `verify/cuda_verify.cu` — self-contained: ports the C reference (`reference/turbo_kernels.c` + `verify/qjl_polar_ref.c`) into `__device__` kernels operating on the on-disk fixture byte layouts, same approach as `metal_verify` / `vulkan_verify`; plus `llama-bench --cache-type-k tbq3_0/tbq4_0 -ngl 99` for the `GGML_OP_ATTN_SCORE_TBQ` built-fork graph route | YES (against `reference/turbo_kernels.c` + `verify/qjl_polar_ref.c`; `make cuda-preprocess-check` also asserts the harness covers every kernel family + the generated TCQ codebook) | YES — `make cuda` builds with distro `nvcc` 12.0 (sm_90/89/86/80 SASS + `compute_90` PTX; the 580 driver JITs the PTX to Blackwell `sm_120`; `nvcc` ≥ 12.8 adds native `sm_120` SASS) | **YES — verified on RTX 5080 (sm_90 PTX JIT to sm_120):** `make cuda-verify` 8/8 PASS for turbo3 (2.9e-6), turbo4 (5.7e-6), turbo3_tcq (5.7e-6), qjl (9.5e-6), polar (7.6e-6), polar_qjl (5.7e-6); built-fork `GGML_OP_ATTN_SCORE_TBQ` graph dispatch via `llama-bench --cache-type-k tbq3_0/tbq4_0 -ngl 99` runs `backend=CUDA`. Evidence: `verify/hardware-results/cuda-linux-rtx5080-2026-05-11.json`, `verify/cuda-runtime-dispatch-evidence.json` |
>
