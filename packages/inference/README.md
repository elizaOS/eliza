...
| `fused_attn_qjl_tbq.metal` | n/a | n/a | yes (`cases` array) | YES (byte-faithful mirror of `fused_attn_qjl_tbq.comp` / `eliza_fused_attn_qjl_tbq3`) | yes (Apple Metal runtime JIT; ships in `default.metallib` via `metal-kernels.mjs`) | AUTHORED — hardware-verify pending (no Apple HW; `make metal-verify-fused` on an Apple-Silicon Mac) | AUTHORED — hardware-verify pending |
| `fused_attn_qjl_polar.metal` | n/a | n/a | yes (`cases` array) | YES (byte-faithful mirror of `fused_attn_qjl_polar.comp` / `eliza_fused_attn_qjl_polar`) | yes (Apple Metal runtime JIT; ships in `default.metallib`) | AUTHORED — hardware-verify pending (no Apple HW; `make metal-verify-fused`) | AUTHORED — hardware-verify pending |
| `polar_preht.metal` (`kernel_attn_score_q4_polar_preht_f32` + `_multi`) | n/a | n/a | yes (`polar_preht.json`) | YES (`dot(H·x, q) == dot(x, H·q)`; mirrors `vulkan/polar_preht.comp` + `eliza_polar_mul_mv`) | yes (Apple Metal runtime JIT; ships in `default.metallib`) | AUTHORED — hardware-verify pending (no Apple HW; `make metal-verify-fused`) | AUTHORED — hardware-verify pending |
