...
| `make -C packages/inference/verify dispatch-smoke` | PASS: exercises the built fork, not JIT, through `GGML_OP_ATTN_SCORE_QJL` -> `kernel_attn_score_qjl1_256`; 32 scores matched the local packed-QJL reference, max diff `2.384e-07`. This replaces the earlier unsafe generic `MILADY-DISPATCH-V1` smoke route. |
