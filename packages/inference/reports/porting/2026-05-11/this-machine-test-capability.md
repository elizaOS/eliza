...
| Caches present | `~/.cache/eliza-dflash/{vulkan-headers,spirv-headers,milady-llama-cpp,buun-llama-cpp,mingw-toolchain}`. `milady-llama-cpp` is at detached `2baad86`; its `ggml-cuda/` has only `turboquant.cuh` — `turbo-tcq.cuh`, `qjl.cuh`, `polarquant.cuh` are produced by the build script's CUDA patch step and are **not yet staged**. |
