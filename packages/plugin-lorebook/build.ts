#!/usr/bin/env bun
/**
 * Dual build script for @elizaos/plugin-character-lore (Node + Browser)
 */

async function build() {
  const externalDeps = ['@elizaos/core', 'drizzle-orm', 'uuid'];
  const totalStart = Date.now();

  // Node build
  const nodeStart = Date.now();
  console.log('🔨 Building @elizaos/plugin-character-lore for Node...');
  ...
  console.log('🌐 Building @elizaos/plugin-character-lore for Browser...');
  ...
  console.log('🧱 Building @elizaos/plugin-character-lore for Node (CJS)...');
