...
The landing phone is demand-rendered and its code/model transfer starts only
after the viewport is visible and the browser is idle, or after user
interaction. `prefers-reduced-motion` settles the shader, phone camera/model,
chat surface, and intro transitions to static states. Model provenance,
reproduction, and geometry bounds are documented in
[`docs/phone-model-provenance.md`](docs/phone-model-provenance.md).

`postbuild` removes optional hydrated background/product directories only after
scanning the complete built output. Any reference fails the build before
deletion, preventing a pruning-induced deployment 404.
