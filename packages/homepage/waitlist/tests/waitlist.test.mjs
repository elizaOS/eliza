...
test("stores a normalized valid signup without exposing the address in the key", async () => {
  ...
  const response = await onRequestPost({
    request: request({ email: " Hello@Example.COM ", source: "eliza.app" }),
    env: { WAITLIST },
  });
  assert.equal(response.status, 200);
  assert.deepEqual(await response.json(), { ok: true });
  const emailEntry = [...WAITLIST.values.entries()].find(([key]) =>
...
test("rejects invalid email, cross-origin requests, and unsupported methods", async () => {
  ...
  assert.equal(
    (await onRequestPost({ request: request({ email: "bad" }), env })).status,
    400,
  );
  assert.equal(
    (
...
test("returns success for honeypot submissions without storing an email", async () => {
  ...
  assert.equal(response.status, 200);
  assert.equal(
    [...WAITLIST.values.keys()].some((key) => key.startsWith("email:")),
    false,
  );
});
...
test("deduplicates a signup after the short rate-limit window", async () => {
  ...
  for (const key of [...WAITLIST.values.keys()])
    if (key.startsWith("rate:")) WAITLIST.values.delete(key);
