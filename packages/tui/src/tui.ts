...
// Re-export types and utilities from core modules
export type {
	Component,
	Focusable,
	OverlayAnchor,
...
import type { Component, OverlayEntry, OverlayHandle, OverlayOptions } from "./core/types.js";
import { CURSOR_MARKER, isFocusable } from "./core/types.js";
...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.TUI_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.TUI_CLEAR_ON_SHRINK === "1";
	...
	private isOverlayVisible(entry: OverlayEntry): boolean {
		return isOverlayVisibleFn(entry, this.terminal.columns, this.terminal.rows);
	}
	...
	private compositeOverlays(lines: string[], termWidth: number, termHeight: number): string[] {
		...
		// Ensure no composited line exceeds terminal width
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.TUI_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".eliza", "tui-debug.log");
