...
const RAW_DATA_DIR = path.join(
  PROJECT_ROOT,
  "src/tests/benchmarks/data/price-talk-trenches",
);
const PROCESSED_DATA_DIR = path.join(
  PROJECT_ROOT,
  "src/tests/benchmarks/data_processed",
);
...
async function loadEnrichedCalls(): Promise<EnrichedCall[]> {
  ...
  const enrichedFile = path.join(
    PROCESSED_DATA_DIR,
    "enriched/enriched_calls_complete.json",
  );
...
async function buildMessagesDataset(
  ...
  const callIdByMessageId = new Map(
    calls.map((c) => [c.originalMessageId, c.callId]),
  );
  ...
  logger.info(
    `Built messages dataset with ${messageArray.length} messages (${callMessageIds.size} are calls)`,
  );
  return messageArray;
}
...
async function buildCallsDataset(calls: EnrichedCall[]): Promise<CallRecord[]> {
  ...
  for (const [key, group] of callGroups) {
    if (group.length === 1) {
      ...
      for (let i = 1; i < group.length; i++) {
        if (group[i].timestamp - lastKeptTime > 3600000) {
          // 1 hour in ms
  ...
  logger.info(
    `Built calls dataset with ${callRecords.length} calls (filtered ${ai16zFiltered} AI16Z, ${dedupFiltered} duplicates)`,
  );
  return callRecords;
}
...
async function buildUserManifest(
...
async function main() {
  ...
  try {
    ...
    const userManifest = await buildUserManifest(
    ...
    logger.info(
      `- Date range: ${new Date(messagesDataset[0].timestamp).toISOString()} to ${new Date(messagesDataset[messagesDataset.length - 1].timestamp).toISOString()}`,
    );
