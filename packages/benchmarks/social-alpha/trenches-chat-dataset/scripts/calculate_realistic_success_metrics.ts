...
async function loadEnrichedCallsData(): Promise<EnrichedCall[]> {
  ...
  const enrichedPath = path.join(
    process.cwd(),
    "../src/tests/benchmarks/data_processed/enriched/enriched_calls_complete.json",
  );
...
async function calculateRealisticMetrics() {
  ...
  const calls = enrichedCalls.filter(
    (call) => call.tokenMentioned?.toUpperCase() !== NATIVE_TOKEN,
  );
  ...
  for (const call of calls) {
    ...
    if (
      ...
      const timeToBestPrice =
        call.priceData.bestPriceTimestamp - call.priceData.calledPriceTimestamp;
      ...
      if (isRug) {
        ...
        const worstPrice =
          call.priceData.worstPrice || call.priceData.calledPrice * 0.9;
        realisticProfitLoss =
          ((worstPrice - call.priceData.calledPrice) /
            call.priceData.calledPrice) *
          100;
      } else {
        // Use the actual best price if it happened after 1 hour
        realisticProfitLoss =
          ((call.priceData.bestPrice - call.priceData.calledPrice) /
            call.priceData.calledPrice) *
          100;
      }
  ...
  const outputPath = path.join(
    process.cwd(),
    "data/realistic_success_metrics.json",
  );
  ...
  userMetricsArray.forEach((user) => {
    const bucket = successRateBuckets.find(
      (b) => user.successRate >= b.min && user.successRate < b.max,
    );
  ...
  successRateBuckets.forEach((bucket) => {
    ...
    logger.info(
      `${bucket.min}-${bucket.max}%: ${bucket.count} users (${pct}%)`,
    );
  });
