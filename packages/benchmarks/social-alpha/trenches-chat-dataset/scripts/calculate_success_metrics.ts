...
async function calculateSuccessMetrics() {
  ...
  const calls = allCalls.filter(
    (call) => call.token_mentioned?.toUpperCase() !== NATIVE_TOKEN,
  );

  const ai16zFiltered = allCalls.length - calls.length;
  logger.info(
    `Loaded ${calls.length} calls (filtered ${ai16zFiltered} AI16Z calls)`,
  );
  ...
  for (const call of calls) {
    ...
    if (
      ...
      const timeToBestPrice =
        call.price_data.bestPriceTimestamp -
        call.price_data.calledPriceTimestamp;
      ...
      if (isRug) {
        ...
        const worstPrice =
          call.price_data.worstPrice || call.price_data.calledPrice * 0.9;
        realisticProfitLoss =
          ((worstPrice - call.price_data.calledPrice) /
            call.price_data.calledPrice) *
          100;
      } else {
        // Use the actual best price if it happened after 1 hour
        realisticProfitLoss =
          ((call.price_data.bestPrice - call.price_data.calledPrice) /
            call.price_data.calledPrice) *
          100;
      }
  ...
  for (const [_userId, metrics] of userMetricsMap) {
    ...
    if (
      metrics.callsWithPriceData > 0 &&
      metrics.uniqueTokenCount >= MIN_DIFFERENT_TOKENS
    ) {
      metrics.averageProfitLossPercent =
        metrics.totalProfitLossPercent / metrics.callsWithPriceData;
      metrics.successRate =
        (metrics.successfulCalls / metrics.callsWithPriceData) * 100;
      metrics.rugRate =
  ...
  const outputPath = path.join(
    process.cwd(),
    "data/calculated_success_metrics.json",
  );
  ...
  logger.info(
    `Users with price data and ${MIN_DIFFERENT_TOKENS}+ different tokens: ${userMetricsArray.length}`,
  );

  const avgSuccessRate =
    userMetricsArray.reduce((sum, u) => sum + u.successRate, 0) /
    userMetricsArray.length;
  const avgRugRate =
    userMetricsArray.reduce((sum, u) => sum + u.rugRate, 0) /
    userMetricsArray.length;
  ...
  userMetricsArray.slice(0, 10).forEach((user, i) => {
    logger.info(
      `${i + 1}. ${user.username}: Success ${user.successRate.toFixed(1)}%, Rugs ${user.rugRate.toFixed(1)}%, Avg P/L ${user.averageProfitLossPercent.toFixed(1)}%`,
    );
  });
...
async function updateUsersWithMetrics(calculatedMetrics: UserMetrics[]) {
  ...
  // Save updated users
  const updatedUsersPath = path.join(
    process.cwd(),
    "data/users_with_metrics.json",
  );
