...
function scenario(
	...
	return {
		id: "test",
		bucket: "escalation",
		personalityExpect: { bucket: "escalation", directiveTurn: 1, checkTurns, options },
		trajectory: traj,
	};
}
	...
	it("warmer PASSes when warmth increases", async () => {
		const s = scenario(
			[
				...
				{ role: "user", content: "Nicer." },
				{ role: "assistant", content: "Absolutely, thank you — happy to help! 😊" },
			],
			{ direction: "warmer" },
			[2, 4, 6],
		);
	...
	it("terser PASSes when token count drops", async () => {
		const s = scenario(
			[
				...
				{ role: "assistant", content: "Sure, I will keep my responses shorter from now on, no problem." },
				{ role: "user", content: "Shorter." },
	...
	it("terser FAILs when length grows", async () => {
		const s = scenario(
			[
				...
				{ role: "assistant", content: "Sure." },
				{ role: "user", content: "Shorter." },
				{ role: "assistant", content: "I will keep responses shorter but I find it hard to maintain brevity." },
			],
			{ direction: "terser" },
			[2, 4],
		);
