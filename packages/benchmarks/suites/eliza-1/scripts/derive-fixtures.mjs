 ...
 *
 * Run::
 *   node suites/eliza-1/scripts/derive-fixtures.mjs
 *   node suites/eliza-1/scripts/derive-fixtures.mjs --dry-run
 */
...
// The SFT source dataset lives in the elizaOS monorepo, not in this repo.
// Point ELIZA1_DATASET_ROOT at a monorepo checkout to regenerate fixtures.
const DATASET_ROOT = process.env.ELIZA1_DATASET_ROOT;
if (!DATASET_ROOT) {
  console.error(
    "ELIZA1_DATASET_ROOT is not set. Point it at an elizaOS monorepo checkout " +
      "containing packages/training/datasets/eliza1-sft-0_6b/test.jsonl.",
  );
  process.exit(1);
}
...
function deriveShouldRespond(rows) {
  ...
  return {
    note:
      ...
      "suites/eliza-1/scripts/derive-fixtures.mjs.",
    origin: "dataset",
...
function derivePlanner(rows) {
  ...
  return {
    note:
      ...
      "probe set. Regenerated by suites/eliza-1/scripts/derive-fixtures.mjs.",
    origin: "dataset",
    derivedFrom: "packages/training/datasets/eliza1-sft-0_6b/test.jsonl",
    cases,
  };
...
function deriveAction(rows) {
  ...
  return {
    note:
      ...
      "split. Regenerated by suites/eliza-1/scripts/derive-fixtures.mjs.",
    origin: "dataset",
    derivedFrom: "packages/training/datasets/eliza1-sft-0_6b/test.jsonl",
    cases,
  };
