 ...
 */
import { METRIC_SCHEMA } from "./metric-schema.mjs";
import { MODALITIES } from "./metric-schema.mjs";
import { REPO_ROOT } from "./lib.mjs";
import { THROUGHPUT_UNIT } from "./metric-schema.mjs";
import { join } from "node:path";
import { loadBudgets } from "./lib.mjs";
import { ms } from "./lib.mjs";
import { recordResult } from "./lib.mjs";
import { rssMb } from "./lib.mjs";
import { skippedModalityRow } from "./metric-schema.mjs";
import { type ArbiterCapability } from "@elizaos/plugin-local-inference/services/memory-arbiter";
import { type ArbiterEvent } from "@elizaos/plugin-local-inference/services/memory-arbiter";
...
import { type InstalledModel } from "@elizaos/plugin-local-inference/services";
...
const services = await import(join(REPO_ROOT, "plugins/plugin-local-inference/src/services/index.ts"));
const { classifyDeviceTier, findCatalogModel, localInferenceEngine, MemoryArbiter, probeHardware, } = services;
const { resolveLocalInferenceLoadArgs } = await import(join(REPO_ROOT, "plugins/plugin-local-inference/src/services/active-model.ts"));
const { capacitorPressureSource } = await import(join(REPO_ROOT, "plugins/plugin-local-inference/src/services/memory-pressure.ts"));
const { resolveRamBudget } = await import(join(REPO_ROOT, "plugins/plugin-local-inference/src/services/ram-budget.ts"));
const { listInstalledModels } = await import(join(REPO_ROOT, "plugins/plugin-local-inference/src/services/registry.ts"));
const { ELIZA_1_TIER_IDS } = await import(join(REPO_ROOT, "plugins/plugin-native-inference/src/model-catalog/catalog.ts"));
 ...
 */
async function measureText(tier: Eliza1TierId, installedPath: string): Promise<ModalityMetric> {
    const catalog = findCatalogModel(tier);
    const installed = (await listInstalledModels()).find((m: InstalledModel) => m.path === installedPath);
    ...
    const { result, peakMb } = await withPeakRss(async () => {
        ...
        return {
            ...
            throughput: genMs > 0 ? Number(((tokens / genMs) * 1000).toFixed(2)) : null,
        };
    });
...
async function runCoResidency(): Promise<CoResidencyMetric> {
    ...
    const evictions = events
        .filter((e): e is Extract<ArbiterEvent, {
        type: "eviction";
    }> => e.type === "eviction")
        .map((e) => ({
...
function checkBudgets(rows: ModalityMetric[], co: CoResidencyMetric): BudgetCheck[] {
    const b = loadBudgets();
    ...
    for (const row of rows) {
        if (!row.measured || row.modality !== "text")
            continue;
        const tierBudget = b.tiers?.[row.tier]?.peakRssMb;
        if (typeof tierBudget !== "number")
            continue;
        checks.push({
...
async function main(): Promise<void> {
    ...
    const tiersToConsider = (tierFilter.length > 0 ? tierFilter : (ELIZA_1_TIER_IDS as readonly string[])) as Eliza1TierId[];
    ...
    console.log(`host tier:   ${tierAssessment.tier}  (${result.summary.host.totalRamGb} GB RAM, ` +
        `${result.summary.host.vramGb ? `${result.summary.host.vramGb} GB VRAM, ` : ""}` +
        `FFI ${ffiAvailable ? "available" : "unavailable"})`);
    console.log(`measured:    ${measuredCount} / ${rows.length} (tier × modality) rows`);
    console.log("\n-- per (tier × modality) --");
    for (const row of rows) {
        if (row.measured) {
            console.log(`  ${row.tier} / ${row.modality}: load ${ms(row.loadMs)}, ` +
    ...
    for (const e of coResidency.evictions) {
        console.log(`    evicted ${e.capability}/${e.modelKey} reason=${e.reason} (~${e.estimatedMb} MB)`);
    }
    console.log("\n-- budget checks --");
    if (checks.length === 0) {
    ...
    for (const c of checks) {
        ...
        console.log(`  ${c.pass ? "PASS" : "FAIL"}  ${c.name}: ${v} / ${cmp} ${c.budget} ${c.unit}`);
    }
    console.log(`\nresult: ${pass ? "PASS" : "FAIL"}   recorded -> ${file}`);
    if (measuredCount === 0) {
        console.log("note: no model bundle measured — co-residency telemetry self-check ran instead.\n");
    }
    else {
        console.log("");
    }
...
main().catch((err) => {
    console.error(`[memperf-kpi] fatal: ${err instanceof Error ? err.stack : String(err)}`);
    process.exit(1);
});
