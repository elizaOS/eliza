...
// this exporter reads. The real types live in the monorepo checkout and are
// loaded dynamically, so they cannot be imported statically here.
type ActionLike = {
  name: string;
  tags?: string[];
  contexts?: string[];
...
const benchmarkPackageRoot = resolve(dirname(scriptPath), "..");
...
function resolveElizaRepoDir(): string {
  // ELIZA_REPO_DIR overrides; the containing monorepo is the default.
  const raw =
...
function usage(): string {
  return [
    "Usage: ELIZA_REPO_DIR=/path/to/eliza node --conditions=eliza-source --conditions=development --import tsx suites/lifeops-bench/scripts/export-action-manifest.ts [options]",
    "",
    "Options:",
...
function examplesCount(action: ActionLike): number {
  return Array.isArray(action.examples) ? action.examples.length : 0;
}

function actionToEntry(
  action: ActionLike,
  pluginName: string,
...
function renderSummary(manifest: Manifest): string {
  ...
  const lines: string[] = [
    ...
    "",
    "Generated by `suites/lifeops-bench/scripts/export-action-manifest.ts`.",
...
async function main(): Promise<void> {
  ...
  // never match a Linux CI regeneration — both sides were "correct" for their
  // own host.
  //
  ...
  const platformIndependentScreenTimePlugin: PluginLike = {
  ...
  const entries = filterEntries(
    collectEntries(sourcePlugins, buildPlannerToolsFromActions),
    options,
  );
  const manifest: Manifest = {
    schemaVersion: 1,
    generator: "suites/lifeops-bench/scripts/export-action-manifest.ts",
