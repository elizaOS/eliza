 ...
 */
import { HouseholdOperationsRepository } from "@elizaos/plugin-personal-assistant";
import { SELF_ENTITY_ID } from "@elizaos/core/knowledge-graph/entity-types";
import { createHash } from "node:crypto";
import { getHouseholdOperationsService } from "@elizaos/plugin-personal-assistant";
import { getSchoolSourceFactRuntimeService } from "@elizaos/plugin-personal-assistant";
...
import { type HouseholdSourceProvenance } from "@elizaos/plugin-personal-assistant";
import { type ResponsibilityAssignmentDefinition } from "@elizaos/plugin-personal-assistant";
import { type SourceArtifactInput } from "@elizaos/plugin-personal-assistant";
export const TRUSTED_PARENT_CONTRACT_STATE_SCHEMA = "lifeops.trusted-parent-contract-state.v1" as const;
...
export const G38_ASSIGNMENT_RECORD_ID = "gutter-responsibility-assignment" as const;
...
interface EvidenceSession {
    readonly scenarioId: typeof G15_SCENARIO_ID | typeof G30_SCENARIO_ID | typeof G38_SCENARIO_ID;
...
function scenarioIdForTask(taskId: string): EvidenceSession["scenarioId"] | null {
    for (const scenarioId of [
        G15_SCENARIO_ID,
        G30_SCENARIO_ID,
        G38_SCENARIO_ID,
    ] as const) {
        if (taskId.startsWith(`${scenarioId}:`))
            return scenarioId;
    }
    return null;
}
...
function textField(value: Record<string, unknown>, field: string): string | null {
    const candidate = value[field];
    return typeof candidate === "string" && candidate.trim()
        ? candidate.trim()
        : null;
}
function actionObservation(actionName: string, parameters: Record<string, unknown>, result: TrustedActionResult): SessionActionObservation {
    ...
    const discriminator = typeof parameters.action === "string" ? parameters.action : null;
    return {
        actionName,
        discriminator,
...
async function ensureHouseholdEntity(runtime: AgentRuntime, input: {
    entityId: string;
    ...
    if (!graph) {
        throw new Error("trusted parent-contract evidence requires the production knowledge graph");
    }
...
function schoolExtraction(input: {
    ...
    const nextDate = input.startDate === "2026-05-20" ? "2026-05-21" : "2026-05-22";
    return {
        noticeKey: input.noticeKey,
        kind: input.kind,
        title: input.kind === "correction"
...
async function prepareG15(runtime: AgentRuntime): Promise<void> {
    ...
    if (!runtimeService) {
        throw new Error("trusted G15 evidence requires the school source-fact runtime service");
    }
    ...
    await runtimeService.ingestSchoolNotice({
        artifact: schoolArtifact({
            ...
            content: "Signed correction from Lincoln School: early release is May 21, 2026, not May 20.",
        }),
        extraction: schoolExtraction({
...
async function prepareG30(runtime: AgentRuntime): Promise<void> {
    ...
    if (!operations) {
        throw new Error("trusted G30 evidence requires the household-operations runtime service");
    }
    const repository = new HouseholdOperationsRepository(runtime, runtime.agentId);
    const threshold = await repository.getCurrentRevision("item_threshold", G30_THRESHOLD_RECORD_ID);
    if (!threshold) {
...
function responsibilityDefinition(minimumStandard: string): ResponsibilityAssignmentDefinition {
    return {
...
async function prepareG38(runtime: AgentRuntime): Promise<void> {
    ...
    const operations = getHouseholdOperationsService(runtime);
    if (!operations) {
        throw new Error("trusted G38 evidence requires the household-operations runtime service");
    }
    const repository = new HouseholdOperationsRepository(runtime, runtime.agentId);
    let assignment = await repository.getCurrentRevision("responsibility_assignment", G38_ASSIGNMENT_RECORD_ID);
    if (!assignment) {
        assignment = await operations.putRevision({
            principalEntityId: SELF_ENTITY_ID,
            expectedRevision: 0,
            definition: responsibilityDefinition("The gutter task remains assigned until an accepted successor agreement exists."),
        });
    }
    if (assignment.revision === 1) {
        assignment = await operations.putRevision({
            principalEntityId: SELF_ENTITY_ID,
            expectedRevision: 1,
            definition: responsibilityDefinition("Two ignored alerts trigger a household responsibility review without reassignment."),
        });
    }
    if (assignment.kind !== "responsibility_assignment" ||
        assignment.revision !== 2) {
        throw new Error("trusted G38 responsibility fixture has an unexpected current revision");
    }
...
async function prepareScenario(runtime: AgentRuntime, scenarioId: EvidenceSession["scenarioId"]): Promise<void> {
...
export async function prepareTrustedParentContractEvidenceSession(runtime: AgentRuntime, session: BenchmarkSession): Promise<void> {
...
async function captureG15(runtime: AgentRuntime, actionHistory: readonly SessionActionObservation[]): Promise<Record<string, unknown>> {
    const runtimeService = getSchoolSourceFactRuntimeService(runtime);
    const graph = resolveKnowledgeGraphService(runtime);
    if (!runtimeService || !graph) {
        throw new Error("trusted G15 production services became unavailable");
    }
    const facts = await runtimeService.school.listFacts(`school.notice:${G15_NOTICE_KEY}`);
    const artifacts = await Promise.all(facts.map((fact) => runtimeService.school.getArtifact(fact.artifactId)));
    if (artifacts.some((artifact) => artifact === null)) {
        throw new Error("trusted G15 source-fact provenance is incomplete");
    }
    const authoritative = facts.find((fact) => String(fact.authority) === "signed_school_correction");
    if (!authoritative) {
...
function commerceAudit(actionHistory: readonly SessionActionObservation[]): Record<string, number> {
    ...
    return {
        ...
        orderCount: kinds.filter((kind) => /(^|[._])order([._]|$)/u.test(kind))
            .length,
        paymentArtifactCount: kinds.filter((kind) => /(^|[._])payment([._]|$)/u.test(kind)).length,
    };
}
async function captureG30(runtime: AgentRuntime, actionHistory: readonly SessionActionObservation[]): Promise<Record<string, unknown>> {
    const operations = getHouseholdOperationsService(runtime);
    if (!operations) {
        throw new Error("trusted G30 household-operations service became unavailable");
    }
    ...
    const sizes = history.map((observation) => observation.value.kind === "child_item_size"
    ...
    return {
        ...
        sizeHistory: history,
        currentObservation: resolution.state === "known" ? resolution.observation : null,
        appendOnly: history.length === 2 &&
...
async function captureG38(runtime: AgentRuntime, actionHistory: readonly SessionActionObservation[]): Promise<Record<string, unknown>> {
    const repository = new HouseholdOperationsRepository(runtime, runtime.agentId);
    const revisions = await repository.listRevisionHistory("responsibility_assignment", G38_ASSIGNMENT_RECORD_ID);
    const current = await repository.getCurrentRevision("responsibility_assignment", G38_ASSIGNMENT_RECORD_ID);
    const signals = await repository.listResponsibilitySignals(G38_HOUSEHOLD_ID, G38_ASSIGNMENT_RECORD_ID);
    const reviews = await repository.listResponsibilityReviews(G38_HOUSEHOLD_ID, G38_ASSIGNMENT_RECORD_ID);
    ...
    for (let index = 1; index < revisions.length; index += 1) {
        ...
        if (previous?.kind !== "responsibility_assignment" ||
        ...
        if (affectedOwnerIds.every((entityId) => candidate.acceptedByEntityIds.includes(entityId))) {
    ...
    const revisionIdentity = (revision: (typeof revisions)[number]) => `${revision.recordId}:${revision.revision}:${revision.contentSha256}`;
    return {
        schemaVersion: TRUSTED_PARENT_CONTRACT_STATE_SCHEMA,
        ...
        historyImmutable: revisions.length >= 2 &&
            revisions.every((revision, index) => revision.revision === index + 1 &&
                /^[0-9a-f]{64}$/u.test(revision.contentSha256)),
        proposalIds: reviews
            .filter((review) => review.state === "proposed" && review.ownerChanges.length === 0)
            .map((review) => review.reviewId),
        acceptedSuccessorAgreementCount,
        unapprovedOwnerChangeCount,
    };
}
export async function captureTrustedParentContractFinalState(runtime: AgentRuntime, session: BenchmarkSession, actionName: string, parameters: Record<string, unknown>, result: TrustedActionResult): Promise<Record<string, unknown> | null> {
