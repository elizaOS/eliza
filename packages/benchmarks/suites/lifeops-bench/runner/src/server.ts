...
import { LIFECYCLE_TASKS_TOOL_CONTRACT } from "./lifecycle-task-action.js";
import { LIFECYCLE_TASK_CONTEXTS } from "./lifecycle-task-action.js";
import { LifeOpsBenchHandler } from "./lifeops-bench-handler.js";
import { ModelType } from "@elizaos/core";
import { UsageCapture } from "./usage-capture.js";
import { assertClaudeSubscriptionModelRegistry } from "./subscription-chat-capabilities.js";
import { autoWireCerebras } from "./cerebras-autowire.js";
import { benchmarkRuntimeActionNames } from "./server-utils.js";
import { benchmarkTurnMetadata } from "./server-utils.js";
import { capturedActionToParams } from "./server-utils.js";
import { capturedActionsToToolCalls } from "./server-utils.js";
import { claudeSubscriptionChatOnlyPlugin } from "./subscription-chat-capabilities.js";
import { clearCapturedAction } from "./plugin";
import { coerceActions } from "./server-utils.js";
import { coerceParams } from "./server-utils.js";
import { composeBenchmarkPrompt } from "./server-utils.js";
import { configureBenchmarkToolCallPolicy } from "./server-utils.js";
...
import { ensureBenchmarkSessionContext } from "./server-utils.js";
import { extractBenchmarkName } from "./server-utils.js";
import { extractRecord } from "./server-utils.js";
import { extractTaskId } from "./server-utils.js";
import { formatUnknownError } from "./server-utils.js";
import { getCapturedAction } from "./plugin";
import { getCapturedActions } from "./plugin";
import { hasLifecycleTaskAction } from "./server-utils.js";
import { lifecycleBenchmarkProviderPayloadIsNeutral } from "./plugin";
import { lifecycleCaptureOnlyPlugin } from "./lifecycle-task-action.js";
import { normalizeBenchmarkContext } from "./server-utils.js";
import { normalizeBenchmarkModelUsage } from "./server-utils.js";
import { projectLifecycleTaskExecutions } from "./lifecycle-task-action.js";
import { readAliasedEnv } from "@elizaos/core/utils/env";
import { resolveElizaPluginImportSpecifier } from "@elizaos/agent/runtime/plugin-types";
import { resolveHost } from "./server-utils.js";
import { resolvePort } from "./server-utils.js";
import { retainOnlyLifecycleTaskAction } from "./lifecycle-task-action.js";
import { runWithBenchmarkContext } from "./plugin";
import { runWithLifecycleTaskCapture } from "./lifecycle-task-action.js";
import { runWithLlmInputSubstringAttestation } from "@elizaos/core";
import { sessionKey } from "./server-utils.js";
import { setBenchmarkContext } from "./plugin";
import { stringToUuid } from "@elizaos/core";
import { summarizeBenchmarkTurnUsage } from "./server-utils.js";
import { toPlugin } from "./server-utils.js";
import { type BenchmarkLlmCallUsage } from "./server-utils.js";
import { type BenchmarkOutboxEntry } from "./server-utils.js";
import { type BenchmarkSession } from "./server-utils.js";
import { type BenchmarkTrajectoryStep } from "./server-utils.js";
import { type ChatMessage } from "@elizaos/core";
import { type ChatMessageRole } from "@elizaos/core";
...
import { type MessageProcessingResult } from "@elizaos/core";
import { type Plugin } from "@elizaos/core";
import { type ToolCall } from "@elizaos/core";
import { type ToolChoice } from "@elizaos/core";
...
const dotenvDisabled = process.env.ELIZA_BENCH_DISABLE_DOTENV === "1" ||
 ...
 */
function toToolDefinitions(raw: Array<Record<string, unknown>>): ToolDefinition[] {
...
function normalizeActionCallingNativeMessages(text: string, context: Record<string, unknown>): Array<Record<string, unknown>> {
    return normalizeGenericToolMessages(context.messages, text);
}
function normalizeWooBenchNativeMessages(text: string, context: Record<string, unknown>): Array<Record<string, unknown>> {
...
function normalizeBfclNativeMessages(text: string, context: Record<string, unknown>): Array<Record<string, unknown>> {
...
function _normalizeTauNativeMessages(text: string, context: Record<string, unknown>): Array<Record<string, unknown>> {
    const rawMessages = Array.isArray(context.messages) ? context.messages : [];
...
function normalizeLocaNativeMessages(rawMessages: unknown): Array<Record<string, unknown>> {
...
function normalizeGenericToolMessages(rawMessages: unknown, fallbackText: string): Array<Record<string, unknown>> {
    const input = Array.isArray(rawMessages) ? rawMessages : [];
    const toolNamesById = new Map<string, string>();
    const normalized: Array<Record<string, unknown>> = [];
    for (const item of input) {
...
function normalizeLocaIncomingToolCall(raw: unknown): Record<string, unknown> | null {
...
function firstLocaBenchmarkActionFromToolCalls(toolCalls: Array<{
    function: {
        name: string;
        arguments: string;
    };
...
function firstWooBenchActionFromToolCalls(toolCalls: Array<{
    function: {
        name: string;
        arguments: string;
    };
...
function bfclBenchmarkActionFromToolCalls(toolCalls: Array<{
    function: {
        name: string;
        arguments: string;
    };
...
function webshopBenchmarkActionFromToolCalls(toolCalls: Array<{
    function: {
        name: string;
        arguments: string;
    };
...
function buildLifeOpsBenchmarkContext(backend: LifeOpsFakeBackend, previousTurns: LifeOpsBenchTurnRecord[]): Record<string, unknown> {
...
function lifeOpsToolCallsFromNativeToolCalls(toolCalls: Array<{
    id: string;
    function: {
        name: string;
        arguments: string;
...
function isAllowedOrigin(origin: string | undefined): boolean {
    ...
    try {
        ...
        if (LOCALHOST_ORIGINS.has(canonical))
            return true;
...
function checkBenchAuth(req: http.IncomingMessage, res: http.ServerResponse): boolean {
    ...
    if (!expected) {
        ...
        res.end(JSON.stringify({
            error: "Benchmark server requires ELIZA_BENCH_TOKEN to be set. " +
...
async function collectSessionDiagnostics(runtime: AgentRuntime, session: BenchmarkSession): Promise<Record<string, unknown>> {
    const room = await runtime.getRoom(session.roomId);
    const rawLastCompactionAt = room?.metadata?.lastCompactionAt;
    const lastCompactionAt = typeof rawLastCompactionAt === "number" ? rawLastCompactionAt : null;
    const [allMessages, recentMessages, factsInRoom, factsForUser] = await Promise.all([
        runtime.getMemories({
            tableName: "messages",
    ...
    const evaluatorNames = (runtime as {
        evaluators?: Array<{
            name?: string;
        }>;
    }).evaluators
    ...
    return {
        ...
        capability_flags: {
            ...
            has_reflection_evaluator: evaluatorNames.some((name) => name.toUpperCase().includes("REFLECTION")),
            has_relationship_evaluator: evaluatorNames.some((name) => name.toUpperCase().includes("RELATIONSHIP")),
            has_manual_compaction_action: actionNames.includes(COMPACT_CONVERSATION_ACTION_NAME),
        },
...
export async function startBenchmarkServer() {
    ...
    const subscriptionChatOnly = initialBenchProvider === "claude-subscription" &&
        process.env.ELIZA_BENCH_SUBSCRIPTION_CHAT_ONLY === "1";
    ...
    if (subscriptionChatOnly && skipEmbeddingPlugin) {
        throw new Error("Claude-subscription chat-only mode cannot use a stub embedding handler");
    }
    ...
    const corePluginsToLoadBase = lifecycleProfile
        ...
        : skipCorePlugins
            ? ["@elizaos/plugin-sql"]
            : CORE_PLUGINS;
    const shouldLoadTaskAgentPlugin = Boolean(process.env.ELIZA_BENCH_REQUIRE_ORCHESTRATOR === "1" ||
    ...
    if (lifecycleProfile) {
        elizaLogger.info("[bench] Loading lifecycle-scoped plugin profile (SQL + native TASKS + selected model provider)");
    }
    ...
    for (const pluginName of corePluginsToLoad) {
        ...
        try {
            const pluginModule = (await import(resolveElizaPluginImportSpecifier(pluginName))) as Record<string, unknown>;
            const plugin = pluginModule.default ?? pluginModule[Object.keys(pluginModule)[0]];
            if (plugin) {
                const resolvedPlugin = toPlugin(plugin, pluginName);
                plugins.push(lifecycleProfile &&
    ...
    elizaLogger.info(`[bench] Loaded ${loadedPlugins.length}/${corePluginsToLoad.length} core plugins`);
    ...
    if (lifecycleProfile ||
        ...
        try {
            const { createElizaPlugin } = await import("@elizaos/agent/runtime/eliza-plugin");
            ...
            elizaLogger.info(`[bench] Loaded eliza plugin with workspace: ${workspaceDir}`);
        }
        catch (error: unknown) {
            elizaLogger.error(`[bench] Failed to load eliza plugin: ${formatUnknownError(error)}`);
        }
    }
    // Load benchmark plugin — provides benchmark provider + BENCHMARK_ACTION
    try {
        const benchmarkPlugin = createBenchmarkPlugin();
        const resolvedBenchmarkPlugin = toPlugin(benchmarkPlugin, "benchmark-plugin");
        plugins.push(lifecycleProfile
    ...
    if (skipEmbeddingPlugin) {
        ...
        const benchEmbeddingPlugin: Plugin = {
            ...
            priority: 100,
            models: {
                TEXT_EMBEDDING: async () => new Array<number>(EMBEDDING_DIMENSIONS).fill(0),
            },
        };
    ...
    if (hasOpenAiCompatibleKey) {
        ...
        try {
            ...
            plugins.push(subscriptionChatOnly
            ...
            if (subscriptionChatOnly) {
                elizaLogger.info("[bench] Claude-subscription capability profile: chat generation and tokenization only; startup validation, embeddings, media, and research are omitted");
            }
            ...
            elizaLogger.warn(`[bench] OpenAI plugin not available: ${formatUnknownError(error)}`);
        }
    }
    else {
        elizaLogger.warn(`[bench] Skipping @elizaos/plugin-openai: no usable key found ` +
    ...
    if (openRouterApiKey && !subscriptionChatOnly) {
        ...
        try {
            const { default: openrouterPlugin } = await import(OPENROUTER_PLUGIN_MODULE);
            ...
            elizaLogger.warn(`[bench] OpenRouter plugin not available: ${formatUnknownError(error)}`);
        }
    }
    else if (openRouterApiKey) {
        elizaLogger.info("[bench] Skipping @elizaos/plugin-openrouter: Claude-subscription capability profile permits only the gateway-backed openai handler");
    }
    ...
    if (anthropicApiKey && !subscriptionChatOnly) {
        ...
        elizaLogger.info("[bench] Skipping @elizaos/plugin-anthropic: Claude-subscription capability profile permits only the gateway-backed openai handler");
    }
    ...
    if (process.env.COMPUTER_USE_ENABLED === "1" && !lifecycleProfile) {
        try {
            ...
            const computeruseModule = (await import(resolveElizaPluginImportSpecifier(computeruseName))) as Record<string, unknown>;
        ...
        elizaLogger.info("[bench] Skipping computer-use plugin for the lifecycle-scoped profile");
    }
    ...
    if (mockBenchmarkEnabled && !lifecycleProfile) {
        ...
        throw new Error("Lifecycle profile cannot load the mock plugin; use the subscription gateway");
    }
    ...
    const registeredTaskActions = registeredActionCatalog.filter((name) => name === "TASKS" || name.startsWith("TASKS_"));
    if (process.env.ELIZA_BENCH_REQUIRE_ORCHESTRATOR === "1" &&
        !lifecycleTaskActionRegistered) {
        throw new Error("ELIZA_BENCH_REQUIRE_ORCHESTRATOR=1 but TASKS/TASKS_* was not registered");
    }
    if (lifecycleProfile &&
        (registeredActionCatalog.length !== 1 ||
            registeredActionCatalog[0] !== "TASKS")) {
        throw new Error(`Lifecycle profile requires an exact one-action TASKS catalog; registered ${JSON.stringify(registeredActionCatalog)}`);
    }
    ...
    if (!skipEmbeddingPlugin && !subscriptionChatOnly && !lifecycleProfile) {
        ...
        elizaLogger.info("[bench] Skipping local-inference wiring for the Claude-subscription chat-only capability profile");
    }
    else if (lifecycleProfile) {
        elizaLogger.info("[bench] Skipping local-inference wiring for the lifecycle-scoped plugin profile");
    }
    else {
    ...
    const modelHandlers = (runtime as {
    ...
    if (subscriptionChatOnly) {
        const embeddingHandlers = modelHandlers?.get(ModelType.TEXT_EMBEDDING) ?? [];
        if (embeddingHandlers.length > 0) {
            throw new Error("Claude-subscription chat-only mode unexpectedly registered a TEXT_EMBEDDING handler");
        }
        assertClaudeSubscriptionModelRegistry(modelHandlers as ReadonlyMap<string, readonly {
            provider?: string;
        }[]> | undefined);
        elizaLogger.info("[bench] Claude-subscription model registry verified: every chat/planner/tokenizer type has exactly one gateway-backed openai handler; RECENT_MESSAGES preserves in-room dialogue while semantic vector generation and recall are disabled");
    }
    ...
    try {
        const registerEvent = runtime.registerEvent.bind(runtime) as (type: string, handler: (payload: unknown) => void | Promise<void>) => void;
        if (typeof registerEvent === "function") {
            ...
            elizaLogger.warn("[bench] runtime.registerEvent is not available; trajectory usage will be unset");
        }
    }
    catch (err: unknown) {
        ...
        for (const [key, createdAt] of sessionCreatedAt.entries()) {
            if (now - createdAt > SESSION_TTL_MS) {
                ...
                for (const [k, v] of roomToSession.entries()) {
                    if (v === key)
                        roomToSession.delete(k);
                }
                for (const [k, v] of entityToSession.entries()) {
                    if (v === key)
                        entityToSession.delete(k);
                }
            }
        }
    };
    ...
    const getLastSession = (): BenchmarkSession | null => lastSessionKey ? (sessions.get(lastSessionKey) ?? null) : null;
    const resolveSession = (taskId: string, benchmark: string, createIfMissing = true): BenchmarkSession | null => {
    ...
    const lifeopsBenchHandler = new LifeOpsBenchHandler({
        ...
        invokePlanner: async ({ taskId, userText, toolManifest, backend, previousTurns, }) => {
            ...
            if (Array.isArray(toolManifest) && toolManifest.length > 0) {
                ...
                const modelTurn = await usageCapture.run(modelUsageBuffer, async () => {
                    const nativeResult = await runtime.useModel(ModelType.TEXT_LARGE, {
                        messages: toChatMessages(buildLifeOpsActionCallingMessages({
                            userText,
                            lifeopsContext,
                        })),
                    ...
                    const nativeRecord = nativeResult && typeof nativeResult === "object"
                        ...
                        : {};
                    const toolCalls = lifeOpsToolCallsFromNativeToolCalls(normalizeLocaNativeToolCalls(nativeRecord.toolCalls));
                    if (toolCalls.length > 0) {
                        const usage = summarizeBenchmarkTurnUsage(modelUsageBuffer);
                        return {
                            text: typeof nativeRecord.text === "string"
                                ? nativeRecord.text
            ...
            try {
                result = await usageCapture.run(turnUsageBuffer, () => messageService.handleMessage(runtime, incomingMessage, callback));
            }
            finally {
            ...
            for (const name of actions) {
                ...
                const argumentsObj: Record<string, unknown> = paramsForAction &&
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (pathname === "/api/benchmark/health" && req.method === "GET") {
            ...
            res.end(JSON.stringify({
                ...
                lifecycle_force_tool_call_disabled: lifecycleProfile && process.env.ELIZA_BENCH_FORCE_TOOL_CALL === "0",
                ...
                lifecycle_benchmark_provider_registered: lifecycleProfile
                    ? runtime.providers.some((provider) => provider.name === "ELIZA_BENCHMARK")
                    : null,
        ...
        if (pathname === "/api/benchmark/diagnostics" && req.method === "GET") {
            try {
                ...
                elizaLogger.error(`[bench] Diagnostics error: ${formatUnknownError(err)}`);
                res.writeHead(500, { "Content-Type": "application/json" });
                res.end(JSON.stringify({ error: "Internal benchmark error" }));
            }
            return;
        ...
        if (pathname === "/api/benchmark/message" && req.method === "POST") {
            ...
            req.on("end", async () => {
                ...
                try {
                    ...
                    const messageService = runtime.messageService;
                    clearCapturedAction();
                    const lifecycleSystemHint = typeof benchmarkContext.system_hint === "string"
                        ? benchmarkContext.system_hint.trim()
                        : "";
                    if (lifecycleProfile && !lifecycleSystemHint) {
                        throw new ElizaError("Lifecycle benchmark request omitted its shared system hint", {
                            code: "BENCHMARK_LIFECYCLE_SYSTEM_HINT_INVALID",
                            severity: "fatal",
                        });
                    }
                    const turnUsageBuffer: BenchmarkLlmCallUsage[] = [];
                    const handleNativeTurn = () => runWithBenchmarkContext(benchmarkContext, () => usageCapture.run(turnUsageBuffer, () => messageService.handleMessage(runtime, incomingMessage, callback)));
                    const lifecycleDispatch = lifecycleProfile
                        ? await runWithLlmInputSubstringAttestation(lifecycleSystemHint, () => runWithLifecycleTaskCapture(handleNativeTurn))
                        : null;
                    ...
                    if (lifecycleProfile) {
                        ...
                        const modelTypeCallCount = Object.values(attestation?.modelTypeCallCounts ?? {}).reduce((total, count) => total + count, 0);
                        if (!attestation ||
                            ...
                            throw new ElizaError("Lifecycle model-boundary hint attestation disagrees with turn usage", {
                    ...
                    if (lifecycleProfile && !lifecycleTurn) {
                        throw new Error("Lifecycle profile completed without request-scoped TASKS capture");
                    }
                    const lifecycleProjection = lifecycleTurn
                        ? projectLifecycleTaskExecutions(lifecycleTurn.executions, result.actionResults)
                        : null;
                    const responseText = typeof result.responseContent?.text === "string"
                        ? result.responseContent.text
                        : callbackTexts.join("\n\n");
                    const thought = typeof result.responseContent?.thought === "string"
    ...
    const benchRequestTimeoutMs = Number(process.env.ELIZA_BENCH_REQUEST_TIMEOUT_MS ?? 30 * 60 * 1000);
    const benchHeadersTimeoutMs = Number(process.env.ELIZA_BENCH_HEADERS_TIMEOUT_MS ?? 30 * 60 * 1000);
    const benchKeepAliveTimeoutMs = Number(process.env.ELIZA_BENCH_KEEPALIVE_TIMEOUT_MS ?? 5 * 60 * 1000);
    ...
    server.listen(port, host, () => {
        elizaLogger.info(`[bench] Eliza benchmark server listening on ${host}:${port} ` +
