...
JSON report on the shared `METRIC_SCHEMA` (shared with #8800), compares against
the reference thresholds in `budgets.json`, and exits non-zero when a measurement
is over threshold. Not registered in the suite orchestrator — run directly with
`node` / `bun`.
...
- **Honesty contract.** A row is `measured: true` only on a real load+run;
  ...
  is never a tier metric and can never satisfy the real eviction-regression
  threshold, but it DOES fail loudly if the arbiter stops counting evictions.
...
- **Exit codes:** `0` within threshold, `1` a measurement over threshold or a
  telemetry regression, `2` nothing measurable (self-check passed) — run it to
  measure regressions and read the exit code.
