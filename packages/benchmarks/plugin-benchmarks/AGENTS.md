...
plugin-benchmarks/
  ...
  __tests__/
    ...
    actions.test.ts            Vitest suite — exercises handlers, discriminator pinning/rejection
...
```bash
bun run --cwd plugin-benchmarks build       # compile to dist/
bun run --cwd plugin-benchmarks dev         # hot-rebuild during development
bun run --cwd plugin-benchmarks test        # run vitest suite
bun run --cwd plugin-benchmarks typecheck   # tsgo --noEmit
bun run --cwd plugin-benchmarks lint        # biome check --write --unsafe
bun run --cwd plugin-benchmarks lint:check  # biome check (read-only)
bun run --cwd plugin-benchmarks format      # biome format --write
bun run --cwd plugin-benchmarks clean       # rm -rf dist .turbo
```
...
- **Promoted virtuals reject contradictory discriminators.** `rejectContradictoryDiscriminator` in `src/index.ts` wraps every promoted virtual: calling e.g. `VENDING_MACHINE_SET_PRICE` with `action: "restock_slot"` fails with `success: false` instead of being silently rewritten to the pinned value (which is what bare `promoteSubactionsToActions` from the published `@elizaos/core` beta does).
