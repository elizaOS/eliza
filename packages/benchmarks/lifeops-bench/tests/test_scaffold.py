...
def test_archive_email_thread_alias_matches_message_manage_archive() -> None:
    ...
    alias_world = factory(2026, "2026-05-10T12:00:00Z")

    assert "ARCHIVE_EMAIL_THREAD" in supported_actions()
    assert any(
        email.thread_id == thread_id and email.folder != "archive"
        for email in alias_world.emails.values()
    )
    ...
    alias = _execute_action(
        Action(name="ARCHIVE_EMAIL_THREAD", kwargs={"threadId": thread_id}),
        alias_world,
    )
