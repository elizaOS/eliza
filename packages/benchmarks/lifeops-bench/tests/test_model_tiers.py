...
def test_resolve_tier_small() -> None:
    ...
    assert spec.model_name == "gemma-4-e2b-q4_k_m"
    assert spec.bundle_path is not None
    assert "eliza-1-2b.bundle" in spec.bundle_path


def test_resolve_tier_mid() -> None:
    spec = resolve_tier({"MODEL_TIER": "mid"})
    assert spec.tier == "mid"
    assert spec.model_name == "gemma-4-e4b-q4_k_m"
...
def test_model_name_override() -> None:
    spec = resolve_tier(
        {"MODEL_TIER": "small", "MODEL_NAME_OVERRIDE": "gemma-4-e2b-q4_k_s"}
    )
    assert spec.model_name == "gemma-4-e2b-q4_k_s"
...
def test_override_does_not_mutate_registry() -> None:
    ...
    assert DEFAULT_TIERS["small"].model_name == "gemma-4-e2b-q4_k_m"
