...
def test_generator_is_deterministic_across_calls() -> None:
    h1 = (
        WorldGenerator(seed=42, now_iso=NOW_ISO, scale="small")
...
def test_different_seeds_produce_different_hashes() -> None:
    h1 = (
        WorldGenerator(seed=1, now_iso=NOW_ISO, scale="small")
        .generate_default_world()
        .state_hash()
    )
    h2 = (
        WorldGenerator(seed=2, now_iso=NOW_ISO, scale="small")
        .generate_default_world()
        .state_hash()
    )
    assert h1 != h2
...
def test_different_now_iso_produces_different_hash() -> None:
    ...
    h2 = (
        WorldGenerator(seed=42, now_iso="2026-01-01T00:00:00Z", scale="small")
        .generate_default_world()
        .state_hash()
    )
    assert h1 != h2
...
def test_world_state_hash_helper_matches_method() -> None:
    world = WorldGenerator(
        seed=7, now_iso=NOW_ISO, scale="tiny"
    ).generate_default_world()
...
def test_snapshot_roundtrip_preserves_hash() -> None:
    world = WorldGenerator(
        seed=42, now_iso=NOW_ISO, scale="small"
    ).generate_default_world()
...
def test_small_scale_populates_every_entity_kind() -> None:
    ...
    for kind in set(EntityKind) - action_created_kinds:
...
def _seed_one_of_every_kind(world: LifeWorld) -> None:
    ...
    world.add(
        ...
        EmailThread(
            ...
            subject="hi",
            message_ids=["em1"],
            participants=["a@b.test", "c@d.test"],
...
@pytest.mark.parametrize(
    "kind,entity_id,patch_field,patch_value",
    [
...
def test_add_with_wrong_type_raises() -> None:
    world = LifeWorld(seed=1, now_iso=NOW_ISO)
    note = Note(
        id="n", title="t", body_markdown="", created_at=NOW_ISO, updated_at=NOW_ISO
    )
...
def test_world_snapshot_dataclass_is_hashable_via_dict() -> None:
    world = WorldGenerator(
        seed=3, now_iso=NOW_ISO, scale="tiny"
    ).generate_default_world()
