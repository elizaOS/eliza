...
| `model-tiers.ts` | `DEFAULT_TIERS` registry (`small` → local eliza-1 2B GGUF, `mid` → local eliza-1 4B GGUF, `large` → Cerebras `gemma-4-31b`, `frontier` → Anthropic Opus); `resolveTier()` reads `MODEL_TIER` + override env vars |
