...
function makeBundle(overrides: ManifestOverrides = {}): {
  ...
  const bundlePath = path.join(root, "eliza-1-2b.bundle");
  ...
  const manifest: Record<string, unknown> = {
    bundleId: overrides.bundleId ?? "eliza-1-2b",
    modelSize: overrides.modelSize ?? "2b",
...
describe("readElizaOneBundle", () => {
  it("reads a local-standin manifest with resolved absolute weights/drafters paths", async () => {
    ...
    expect(m.bundleId).toBe("eliza-1-2b");
    expect(m.modelSize).toBe("2b");
