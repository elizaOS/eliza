...
describe("resolveTier", () => {
  ...
  it("returns the small tier when MODEL_TIER=small", () => {
    ...
    expect(spec.modelName).toBe("qwen3.5-2b-q4_k_m");
  ...
  it("returns the mid tier when MODEL_TIER=mid", () => {
    ...
    expect(spec.modelName).toBe("qwen3.5-4b-q4_k_m");
  ...
  it("applies MODEL_NAME_OVERRIDE", () => {
    const spec = resolveTier({
      MODEL_TIER: "small",
      MODEL_NAME_OVERRIDE: "qwen3.5-2b-q4_k_s",
    });
    expect(spec.modelName).toBe("qwen3.5-2b-q4_k_s");
  });
  ...
  it("does not mutate the DEFAULT_TIERS registry on override", () => {
    ...
    expect(DEFAULT_TIERS.small.modelName).toBe("qwen3.5-2b-q4_k_m");
  });
});
