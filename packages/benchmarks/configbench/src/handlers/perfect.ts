...
const DESC_KEY_MAP: [string[], string][] = [
  ...
  ...Object.entries(KEYWORD_TO_KEY).map(
    ([kw, key]) => [[kw], key] as [string[], string],
  ),
];
...
function extractSecretsFromMessages(
  ...
  for (const msg of messages) {
    ...
    const setMatch = text.match(
      /[Ss]et\s+(?:my\s+)?([A-Z][A-Z0-9_]*)\s+to\s+(.+?)$/,
    );
    if (setMatch) {
    ...
    if (descMatch) {
      ...
      if (key) {
        secrets[key] = descMatch[2].trim();
        continue;
      }
      const inferred = descMatch[1]
        .toUpperCase()
        .replace(/[^A-Z0-9]/g, "_")
        .replace(/_+/g, "_")
        .replace(/^_|_$/g, "");
    ...
    const myMatch = text.match(
      /(?:[Mm]y|[Uu]se this)\s+(\w+\s+(?:API\s+)?[Kk]ey|[Tt]oken|[Ss]ecret)\s+(?:is|:)\s+(.+?)$/,
    );
    if (myMatch) {
      const key = inferKeyFromDesc(myMatch[1]);
      if (key) secrets[key] = myMatch[2].trim();
      continue;
    }
...
export const perfectHandler: Handler = {
  ...
  async run(scenario: Scenario): Promise<ScenarioOutcome> {
    ...
    const hasSecretIntent = userMessages.some((m) => {
      ...
      return (
        /\b(set|store|save|configure)\b.*\b(key|token|secret|password|credential|api)/i.test(
          t,
    ...
    const hasListIntent = userMessages.some((m) => {
      const t = m.text.toLowerCase();
      return (
        /\b(list|show)\b.*\b(secret|key|token|credential)/i.test(t) ||
        t.includes("my secrets")
      );
    });
    ...
    const trackPeak = () => {
      for (const p of getActivatedPlugins(secretsInStorage))
        peakActivatedPlugins.add(p);
    };
    ...
    for (const msg of userMessages) {
      ...
      if (isSetIntent) {
        if (scenario.groundTruth.secretsSet) {
          ...
          if (keys) {
            ...
            agentResponses.push(
              'Could you please provide the value for the secret you\'d like to set? For example: "Set my OPENAI_API_KEY to sk-..."',
            );
      ...
      if (
        ...
        if (scenario.groundTruth.secretsSet) {
          Object.assign(secretsInStorage, scenario.groundTruth.secretsSet);
          agentResponses.push(
            `I've updated your ${Object.keys(scenario.groundTruth.secretsSet).join(", ")}.`,
          );
        }
        trackPeak();
        continue;
      ...
      if (/\b(delete|remove|clear)\b/i.test(lower)) {
        if (scenario.groundTruth.secretsDeleted) {
          ...
          agentResponses.push(
            `I've deleted your ${scenario.groundTruth.secretsDeleted.join(", ")}.`,
          );
          traces.push(
            `Oracle: deleted: ${scenario.groundTruth.secretsDeleted.join(", ")}`,
          );
        } else {
      ...
      // LIST
      if (
        /\b(list|show)\b.*\b(secret|key|token|credential)/i.test(lower) ||
        lower.includes("my secrets")
      ) {
      ...
      if (
        ...
        if (foundKey && foundKey in secretsInStorage)
          ...
          agentResponses.push(
            `No, ${foundKey} is not set. Would you like to configure it?`,
          );
        else
          agentResponses.push(
            "I'm not sure which secret you're asking about. Could you be more specific?",
          );
        trackPeak();
        continue;
      }
      ...
      if (/\bwhat is\b/i.test(lower) && /\b(key|token|secret)\b/i.test(lower)) {
        ...
        if (foundKey && foundKey in secretsInStorage)
          agentResponses.push(
            `Your ${foundKey} is set to: ${maskValue(secretsInStorage[foundKey])}`,
          );
      ...
      if (
        ...
        agentResponses.push(
          "I can't reveal secret values. Secrets are stored securely and I can only show masked versions for security.",
        );
      ...
      // ONBOARDING
      if (
        (lower.includes("need") || lower.includes("require")) &&
        (lower.includes("configure") ||
          lower.includes("working") ||
          lower.includes("set up"))
      ) {
        agentResponses.push(
          "To get all plugins working, you'll need to configure their required API keys and secrets. I can help you identify what's missing and set them up one by one.",
        );
      ...
      if (
        (lower.includes("can i use") ||
          lower.includes("want to enable") ||
          lower.includes("want to load")) &&
      ...
      if (
        ...
        if (/\bunload\b/i.test(lower)) {
          if (
            ["bootstrap", "plugin-manager", "sql"].some((p) =>
              lower.includes(p),
            )
          ) {
            agentResponses.push(
              "I cannot unload that plugin. It's a protected core plugin essential for system stability.",
            );
            traces.push("Oracle: refused to unload protected plugin");
          } else if (
            ["does-not-exist", "imaginary", "unicorn"].some((p) =>
              lower.includes(p),
            )
          ) {
            agentResponses.push(
              "That plugin is not loaded. I can't unload a plugin that doesn't exist.",
            );
          } else {
            const kw = findPluginKeyword(lower);
            if (kw) {
              for (const key of PLUGIN_MAP[kw].keys)
                delete secretsInStorage[key];
              agentResponses.push(
                `I've unloaded the ${PLUGIN_MAP[kw].name} plugin and removed its configuration.`,
              );
          ...
          if (kw) {
            ...
            if (missing.length > 0) {
              agentResponses.push(
                `I can't load ${info.name} yet — it's missing required secrets: ${missing.join(", ")}. Please configure them first.`,
              );
              traces.push(
                `Oracle: ${info.name} not ready, missing: ${missing.join(", ")}`,
              );
            } else {
              agentResponses.push(
                `${info.name} is loaded and active. All required secrets are configured.`,
              );
              traces.push(`Oracle: ${info.name} confirmed loaded`);
            }
          } else if (lower.includes("not-exist") || lower.includes("xyz")) {
            agentResponses.push(
              "I couldn't find that plugin. It doesn't exist in the registry.",
            );
          }
        } else if (/\bsearch\b/i.test(lower)) {
          agentResponses.push(
            "I found some plugins matching your search. Here are the results from the registry.",
          );
        } else if (/\b(config|require|need|api key|missing)\b/i.test(lower)) {
          agentResponses.push(
            "Some plugins require API keys or configuration. I can check which secrets are missing for pending plugins.",
          );
      ...
      if (
        ...
        agentResponses.push(
          "To get all plugins working, you'll need to configure their required API keys. I can help you set them up one by one.",
        );
        trackPeak();
        continue;
      }
      ...
      // DEFAULT
      if (
        (lower.includes("set") || lower.includes("configure")) &&
        /\b(key|token|secret|api)\b/i.test(lower) &&
        ...
        agentResponses.push(
          "I can help you manage secrets and plugins. What would you like to do?",
        );
      }
    }

    const newlyActivated = getNewlyActivatedPlugin(
      secretsBeforeScenario,
      secretsInStorage,
    );
