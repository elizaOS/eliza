...
| `compute_costs.py` | Cost projection tool. Reads token-usage data from `benchmark_results/latest/<benchmark>__<harness>.json` snapshots and emits a markdown cost report (`benchmark_results/COST_REPORT.md`) priced against Cerebras `gpt-oss-120b` and Anthropic `claude-opus-4-8`. Also prints a per-harness summary to stdout. |
