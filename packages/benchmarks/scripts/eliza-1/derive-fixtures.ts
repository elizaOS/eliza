#!/usr/bin/env node
/**
 * derive-fixtures.ts — derive sibling bench fixtures from the canonical
 ...
 *
 * Run::
 *   node scripts/eliza-1/derive-fixtures.ts
 *   node scripts/eliza-1/derive-fixtures.ts --dry-run
 */
...
const BENCH_ROOT = resolve(dirname(SELF), "../../suites/eliza-1");
...
function deriveShouldRespond(rows) {
  ...
  return {
    note:
      ...
      "scripts/eliza-1/derive-fixtures.ts.",
    origin: "dataset",
...
function derivePlanner(rows) {
  ...
  return {
    note:
      ...
      "probe set. Regenerated by scripts/eliza-1/derive-fixtures.ts.",
    origin: "dataset",
    derivedFrom: "packages/training/datasets/eliza1-sft-0_6b/test.jsonl",
    cases,
  };
...
function deriveAction(rows) {
  ...
  return {
    note:
      ...
      "split. Regenerated by scripts/eliza-1/derive-fixtures.ts.",
    origin: "dataset",
    derivedFrom: "packages/training/datasets/eliza1-sft-0_6b/test.jsonl",
    cases,
  };
