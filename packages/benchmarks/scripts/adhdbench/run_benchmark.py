...
Usage:
    python ../../scripts/adhdbench/run_benchmark.py run --model gpt-4o --levels 0 1 2
    python ../../scripts/adhdbench/run_benchmark.py run --quick
    python ../../scripts/adhdbench/run_benchmark.py run --full
    python ../../scripts/adhdbench/run_benchmark.py baselines
"""
...
# Source execution resolves the suite and adapter from the canonical scripts tree.
# Wheels install this module beside the benchmark package, where imports resolve normally.
_BENCHMARKS_DIR = Path(__file__).resolve().parents[2]
_BENCHMARK_DIR = _BENCHMARKS_DIR / "suites" / "adhdbench"
if _BENCHMARK_DIR.is_dir():
    sys.path.insert(0, str(_BENCHMARK_DIR))
    sys.path.insert(0, str(_BENCHMARKS_DIR / "harnesses" / "eliza"))
