...
Usage:
    ...
    python ../../scripts/clawbench/run_batch.py --start --wait --stop

    # If services are already running:
    python ../../scripts/clawbench/run_batch.py --wait

    # Just one scenario (both variants):
    python ../../scripts/clawbench/run_batch.py --wait --only morning_brief

    # Dry run (no API calls — just verify fixtures load):
    python ../../scripts/clawbench/run_batch.py --dry-run
...
# Paths
# ---------------------------------------------------------------------------
SANDBOX_DIR = (Path(__file__).resolve().parents[2] / "suites" / "clawbench")
