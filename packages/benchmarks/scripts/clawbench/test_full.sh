...
#
# Usage:
#   ./../../scripts/clawbench/test_full.sh                    # run all layers
#   ./../../scripts/clawbench/test_full.sh --quick            # skip Docker (layers 1-3 only)
#   ./../../scripts/clawbench/test_full.sh --docker-only      # skip offline tests, run Docker only
#   ./../../scripts/clawbench/test_full.sh --scenario NAME    # use a specific scenario (default: client_escalation)
#   ./../../scripts/clawbench/test_full.sh --keep             # don't stop Docker after test
#
# Prerequisites:
...
SANDBOX_DIR="$(cd "$SCRIPT_DIR/../../suites/clawbench" && pwd)"
cd "$SANDBOX_DIR"
...
if [ "$DOCKER_ONLY" = false ]; then
    ...
    if python ../../scripts/clawbench/test_handlers.py --scenario "$SCENARIO"; then
...
if [ "$DOCKER_ONLY" = false ]; then
    ...
    if python ../../scripts/clawbench/test_scoring.py; then
...
if [ "$DOCKER_ONLY" = false ]; then
    ...
    if curl -sf http://localhost:3001/health > /dev/null 2>&1; then
        if python ../../scripts/clawbench/test_mock_tools.py --base-url http://localhost:3001; then
            layer_pass "3-http"
        else
            layer_fail "3-http"
        fi
                ...
                if [ "$HEALTHY" = false ]; then
                    ...
                    if python ../../scripts/clawbench/test_mock_tools.py --base-url http://localhost:3001; then
                        ...
                        if [ "$GW_READY" = true ]; then
                            ...
                            if python ../../scripts/clawbench/run_episode.py \
                                --scenario "$SCENARIO" \
