...
lane still yields a full result set for fairness/starvation accounting. Other
exceptions propagate because infrastructure failures are not scored outcomes.
"""
...
async def _run_task(
    ...
    ``wait_for``. A timeout becomes a resultless ``TaskRun`` whose
    ``terminated_reason`` records why, keeping "did not complete" distinct
    from "completed with score 0". Any other exception invalidates the lane.
    """
    ...
    *,
    ...
    for wave_index, wave in enumerate(waves):
        wave_runs = await asyncio.gather(
            *(
                _run_task(
                    runner, scenario, sample_seed(scenario), wave_index, timeout_s
                )
