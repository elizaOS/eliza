"""
Trajectory logging integration for WebShop.

This mirrors the Tau-bench integration style:
- Manual instrumentation around canonical `runtime.message_service.handle_message()`
- Provider accesses (WEBSHOP_CONTEXT)
- Action attempts (WEBSHOP_ACTION / REPLY)
- LLM calls via wrapping `runtime.use_model`
- Export to OpenPipe ART JSONL or GRPO grouped JSON
"""
...
class WebShopTrajectoryIntegration:
    ...
    def start_task(self, task: WebShopTask, *, agent_id: str, trial_number: int) -> str | None:
        if not self._logger:
            return None
        ...
        trajectory_id = self._logger.start_trajectory(
            ...
            metadata={
                ...
                "has_budget": task.budget is not None,
            },
        )
    ...
    def start_turn(
        ...
        *,
        ...
        if observation is not None:
            custom["observation"] = _safe_jsonable(observation)
    ...
    def log_action_attempt(
        ...
        *,
        ...
        self._logger.complete_step(trajectory_id, step_id, action=attempt, reward=reward)

    async def end_task(self, *, result: WebShopResult) -> Trajectory | None:
        if not self._logger or not self._current_trajectory_id:
            return None
        ...
        await self._logger.end_trajectory(
            ...
            final_metrics={
                ...
                "purchased_product_id": result.purchased_product_id or "",
            },
        )

        traj = self._logger.get_active_trajectory(trajectory_id)
        if traj:
            traj.total_reward = float(result.reward)
            traj.reward_components.environment_reward = float(result.reward)
    ...
    def flush_llm_calls_to_step(self, *, step_id: str, system_prompt: str) -> None:
        ...
        for entry in self._llm_call_buffer:
            params_obj = entry.get("params")
            prompt = ""
            temperature = 0.0
            max_tokens = 0

            if isinstance(params_obj, dict):
                p = params_obj.get("prompt")
                if isinstance(p, str):
                    prompt = p
                t = params_obj.get("temperature")
                if isinstance(t, (int, float)):
                    temperature = float(t)
                mt = params_obj.get("max_tokens") or params_obj.get("maxTokens")
