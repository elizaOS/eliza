...
latency). Set `OPENAI_API_KEY` or `CEREBRAS_API_KEY` to load
`@elizaos/plugin-openai` (Cerebras uses its OpenAI-compatible endpoint).
The retired CLI inference plugin is no longer supported by this harness;
subscription campaigns use the orchestrator gateway.
