...
```bash
toolchains/bun-riscv64/validate.sh
```
...
qemu-riscv64-static \
  linux/elizaos/artifacts/riscv64/elizaos-app/musl-runtime/ld-musl-riscv64.so.1.real \
  --library-path linux/elizaos/artifacts/riscv64/elizaos-app/musl-runtime \
  linux/elizaos/artifacts/riscv64/elizaos-app/musl-runtime/bun \
  /tmp/nfkc.js
```
...
`make -C linux/elizaos qemu-virt-smoke ARCH=riscv64`.
...
# 3. Export as patches
git format-patch -o "$REPO_ROOT/toolchains/bun-riscv64/webkit-patches/" \
    --start-number=1 \
    5488984d20e0dbfe4be2c3ba8fb18eb81a5e0e8b..HEAD

# 4. Remove the recipes
rm "$REPO_ROOT/toolchains/bun-riscv64/webkit-patches/"*.recipe
