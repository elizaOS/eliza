...
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../linux/elizaos" && pwd)"
cd "${HERE}"
...
python3 -c 'import ast,pathlib; ast.parse(pathlib.Path("../../scripts/linux/check_release_manifest.py").read_text())' \
    || { echo "PY COMPILE FAIL: ../../scripts/linux/check_release_manifest.py"; fail=1; }
...
if ! make -n release-check ARCH=riscv64 2>/dev/null | grep -q '../../scripts/linux/check_release_manifest.py'; then
    echo "BAD RELEASE CHECK TARGET: release-check must invoke ../../scripts/linux/check_release_manifest.py"
    fail=1
fi
