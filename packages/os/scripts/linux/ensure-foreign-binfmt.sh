...
if ! grep -qx enabled /proc/sys/fs/binfmt_misc/status; then
    echo "ERROR: binfmt_misc is disabled; enable it before cross-architecture builds." >&2
    exit 65
fi

if [ -e "/proc/sys/fs/binfmt_misc/$handler" ] &&
    ! grep -qx enabled "/proc/sys/fs/binfmt_misc/$handler"; then
...
if ! grep -Eq '^flags: [A-Z]*F[A-Z]*$' "/proc/sys/fs/binfmt_misc/$handler"; then
    echo "ERROR: $handler needs the F flag (fixed interpreter) for image chroots; replace its registration before building." >&2
    exit 65
fi
