...
//
// The mapping is the shared confidential-enforcement-map.ts. Drift in either
...
//   node scripts/check-confidential-artifacts.ts
...
async function main() {
  ...
  if (!result.ok) {
    ...
    console.error(
      "confidential-artifacts-check: FAIL-CLOSED (regenerate with generate-confidential-artifacts.ts)",
    );
    process.exit(1);
  }
