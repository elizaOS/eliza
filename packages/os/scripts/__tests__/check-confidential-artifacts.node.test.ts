...
//   node --test scripts/__tests__/check-confidential-artifacts.test.ts
...
import {
  ...
  loadArtifacts,
} from "../check-confidential-artifacts.ts";
import {
  expectedCmdlineTokens,
  expectedMaskedUnits,
  expectedSysctlEntries,
} from "../confidential-enforcement-map.ts";
import { renderArtifacts } from "../generate-confidential-artifacts.ts";
import { readJson, repoRoot } from "../os-release-lib.ts";

const policyPath = path.join(
  repoRoot,
  "linux/confidential/policy/confidential-policy.json",
);
