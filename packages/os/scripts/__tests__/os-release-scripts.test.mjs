...
releaseFixtureTest(
  ...
  async () => {
    ...
    await execFileAsync(
      process.execPath,
      [
        "scripts/generate-os-homepage-data.mjs",
        "--manifest",
        defaultManifestPath,
        "--output",
        output,
...
test("legacy checksum updater preserves valid candidate manifest status", async () => {
  ...
  const manifest = {
    ...
    checksumPolicy: {
      ...
      verificationScript: "scripts/verify-release-checksums.mjs",
    },
    validation: {
  ...
  await execFileAsync(
    process.execPath,
    [
      "scripts/update-manifest-checksums.mjs",
      "--manifest",
      manifestPath,
      "--artifacts-dir",
      artifactRoot,
...
test("TEE measurement generation hashes required release inputs", async () => {
  ...
  await execFileAsync(
    process.execPath,
    [
      "scripts/generate-tee-measurements.mjs",
      "--output",
      output,
      "--boot",
      inputs.boot,
