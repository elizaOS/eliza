...
//   node --test scripts/__tests__/verify-image-reproducibility.test.ts
//
...
import {
  ...
  recomputeComponents,
  verifyManifest,
} from "../verify-image-reproducibility.ts";
...
// are the real sha256 of those bytes — mirroring generate-tee-measurements.ts.
async function buildFixture(
  ...
  return {
    ...
    gate: {
      ...
      provingCommand: "node scripts/verify-image-reproducibility.ts",
    },
  };
}
...
test("double-build CLI cannot report reproducible without both sets of component bytes", async () => {
  ...
  const command = [
    new URL("../verify-image-reproducibility.ts", import.meta.url).pathname,
    "--build-a",
    manifestPath,
    "--build-b",
    manifestPath,
