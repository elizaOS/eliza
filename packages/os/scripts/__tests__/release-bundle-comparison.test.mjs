...
import { assertReleaseBundlesEqual } from "../release-asset-inventory.mjs";
...
test("compares the complete bundle through the production CLI", async (t) => {
  ...
  const script = new URL("../release-asset-inventory.mjs", import.meta.url);
...
test("a symlinked CLI executes verification instead of silently succeeding", async (t) => {
  ...
  await symlink(
    new URL("../release-asset-inventory.mjs", import.meta.url).pathname,
    entrypoint,
  );
  const args = [
    entrypoint,
