...
//
// Usage:
//   node update-release-manifest.ts \
//     --manifest  release/v0.1.0-beta.1/manifest.json \
...
if (missing.length > 0) {
  ...
  console.error(
    "Usage: node update-release-manifest.ts --manifest PATH --artifact ID [--sha256 HASH] [--size BYTES] [--url URL]",
  );
  process.exit(1);
}
