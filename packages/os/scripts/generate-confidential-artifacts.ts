...
//
//   linux/confidential/cmdline.conf            kernel-cmdline fragment
//   linux/confidential/sysctl.d/99-confidential.conf   sysctl drop-in
//   linux/confidential/masked-units.txt        systemd masked units
//
// The mapping (which policy setting produces which line) lives in
// confidential-enforcement-map.ts so the generator and the checker share one
// source of truth. Re-run this whenever the policy changes; the
// check-confidential-artifacts.ts gate fails closed if the artifacts drift.
//
