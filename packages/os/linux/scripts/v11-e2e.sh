...
#
# Full end-to-end harness for usbeliza live ISOs.
#
# Exercises every Action surface + multi-turn flows + dream-world wallpaper
# + LLM-rephrase ON-path (via a fake claude auth marker) on a single headless
# QEMU boot. Extends v9-smoke.sh with twice as many probes and on-disk
# file-existence checks.
#
# Usage:
#   scripts/v11-e2e.sh                                # default: out/usbeliza-v*-final-amd64.iso, newest first
#   scripts/v11-e2e.sh out/usbeliza-v11-final-amd64.iso
#
# Exit codes:
#   0   all probes passed
...
#   3   setup error
#
# Artifacts: vm/snapshots/v11-e2e-<TS>/{screen-<n>.png, chat-<n>.json, summary.md}
...
ARTIFACTS="vm/snapshots/v11-e2e-$TS"
mkdir -p "$ARTIFACTS"

QMP_SOCK="vm/snapshots/v11-e2e-qmp.sock"
SERIAL_SOCK="vm/snapshots/v11-e2e-serial.sock"
SSH_PORT=2225
...
cleanup() {
    ...
    sudo pkill -9 -f "qemu-system-x86_64.*v11-e2e" 2>/dev/null || true
...
PASS=0; FAIL=0
{
    echo "# v11 E2E results — $(date -u +%FT%TZ)"
    echo
    echo "ISO: \`$ISO\`"
    echo
