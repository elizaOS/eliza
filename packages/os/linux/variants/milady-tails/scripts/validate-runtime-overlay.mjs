...
function parseArgs(argv) {
  const parsed = {
    stage: process.env.ELIZAOS_MILADY_APP_STAGE ?? defaultStage,
...
function validatePorts(manifest) {
  ...
  const liveLauncher = path.join(osOverlayRoot, "usr/local/bin/milady");
...
function validateBranding() {
  ...
  if (version?.identifier !== "ai.elizaos.app") {
    fail(`${versionPath}: identifier must be ai.elizaos.app`);
  }
