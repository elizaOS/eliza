...
function listPackageInventory() {
  const appManifestPath = path.join(
    variantRoot,
    "tails/config/chroot_local-includes/usr/share/elizaos/milady-app/Resources/app/elizaos-live-overlay-manifest.json",
  );
...
console.log(`wrote ${path.relative(variantRoot, sbomPath)}`);
console.log(`wrote ${path.relative(variantRoot, provenancePath)}`);
