...
start = text.index("class MiladyData")
end = text.index("class WelcomeScreen", start)
milady = text[start:end]
for name, target in re.findall(r'Binding\("([^"]+)", "([^"]+)"', milady):
    if not name.startswith("milady/"):
        continue
...
if "uses_symlinks=True" in milady:
...
root_unit=tails/config/chroot_local-includes/etc/systemd/system/milady.service
require_fixed 'ExecStart=/usr/local/lib/elizaos/milady-keeper' "${root_unit}" \
...
if grep -Eq 'ExecStart=.*(/usr/local/bin/milady|/opt/milady|launcher)' "${root_unit}"; then
