...
- `just milady-app` staged the rebuilt app into the live-build overlay.
- `node scripts/prepare-milady-app-overlay.mjs --check` passed.
...
- Work in `packages/os/linux/variants/milady-tails/` for this distro path.
...
- Keep user-facing branding as elizaOS. Internal `tails` and `/opt/milady`
