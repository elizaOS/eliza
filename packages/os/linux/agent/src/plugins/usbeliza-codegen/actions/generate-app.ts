...
export async function generateApp(
  ...
  for (let attempt = 0; attempt <= MAX_AUTO_RETRIES; attempt++) {
    try {
      ...
      if (lastError.stage !== "parse" && lastError.stage !== "validate") {
        throw lastError;
      }
...
async function writeStaged(
  ...
  for (const [relPath, contents] of Object.entries(parsed.files)) {
    const rerooted = relPath.startsWith("src/") ? relPath.slice(4) : relPath;
...
function runClaudeArgs(brief: GenerationBrief): string[] {
  return [
    ...
    buildUserPrompt({
      slug: brief.slug,
      intent: brief.intent,
      ...(brief.existingSrc !== undefined
        ? { existingSrc: brief.existingSrc }
        : {}),
  ...
  return await new Promise<CodegenOutput>((resolveOutput, rejectOutput) => {
    ...
    child.on("close", (code) => {
      ...
      if (wrapper === undefined) {
        rejectOutput(
          new CodegenError("claude stdout was not valid JSON", "parse"),
        );
        return;
      }
      if (wrapper.is_error === true) {
        rejectOutput(
          new CodegenError("claude reported is_error=true", "spawn"),
        );
        return;
      }
      if (
      ...
      if (typeof wrapper.result === "string" && wrapper.result.length > 0) {
        ...
        rejectOutput(
          new CodegenError("claude `result` was not valid inner JSON", "parse"),
        );
        return;
      }
      rejectOutput(
