...
export async function handleOnboarding(
  ...
  if (trimmed === "") {
    return {
      reply: await rephraseOnboardingTurn(q.prompt, String(q.id), state, ""),
      completed: false,
    };
  }
  ...
  if (parsed !== undefined) {
    ...
    // code paste.
    if (
      q.id === "claudeOfferAccepted" &&
      parsed === true &&
      process.env.USBELIZA_STATE_DIR === undefined
    ) {
      try {
        const { beginClaudeFlow } = await import(
          "../runtime/flows/claude-flow.ts"
        );
...
function maybeFireBuildApp(brief: string): void {
  ...
  void (async () => {
    try {
      ...
      await BUILD_APP_ACTION.handler(
        runtime,
        memory,
        undefined,
        undefined,
