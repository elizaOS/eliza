...
export async function generateViaClaude(
  ...
  const binaryExistsFn =
    options.binaryExistsFn ?? ((p: string) => existsSync(p));
  ...
  const _maxTokensReserved =
    options.maxTokens ?? params.maxTokens ?? DEFAULT_MAX_TOKENS;
  void _maxTokensReserved;
  const model =
    options.model ?? process.env.USBELIZA_CLAUDE_MODEL ?? DEFAULT_MODEL;
 ...
 */
export function createClaudeCloudPlugin(
  options: ClaudeCloudOptions = {},
): Plugin {
  return {
