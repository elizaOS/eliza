...
export const LIST_WIFI_ACTION: Action = {
  ...
  handler: async (_runtime, _message, _state, _options, callback) => {
    if (!(await isNmcliAvailable())) {
      const text =
        "I can't see Wi-Fi controls right now — nmcli isn't available on this system.";
...
export const CONNECT_WIFI_ACTION: Action = {
  ...
  handler: async (_runtime, message, _state, _options, callback) => {
    ...
    if (parsed === null) {
      return {
        success: false,
        text: "I couldn't parse the Wi-Fi network name.",
      };
    }
    if (!(await isNmcliAvailable())) {
      const text =
        "I can't manage Wi-Fi right now — nmcli isn't available on this system.";
    ...
    try {
      ...
      return {
        success: false,
        text: shapeNetworkError(err, `connect to ${parsed.ssid}`),
      };
    }
  },
};
