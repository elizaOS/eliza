  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0) resolve();
        ...
        reject(
          new Error(`ImageMagick exited ${code}: ${stderr.slice(0, 400)}`),
        );
    });
  });
  return outPath;
...
function briefFromMessage(text: string): string | null {
  ...
  const cleaned = lower
    .replace(/^(please\s+)?/, "")
    .replace(
      /^(make|set|create|build|generate|paint|put)\s+(me\s+)?(a|an|my|the)?\s*/,
      "",
    )
    .replace(
      /(my )?(wallpaper|background|desktop)\s*(to|with|that's|that is)?\s*/,
      "",
    )
    .trim();
...
export const SET_WALLPAPER_ACTION: Action = {
  ...
  handler: async (_runtime, message, _state, _options, callback) => {
    ...
    try {
      ...
      try {
        ...
        if (callback)
          await callback({ text: reply, actions: ["SET_WALLPAPER"] });
  ...
  examples: [
    [
      {
        name: "{{user}}",
        content: { text: "make me a space wallpaper with stars" },
      },
      {
        name: "Eliza",
        content: { text: 'Done. New wallpaper based on "space with stars".' },
      },
    ],
    [
      {
        name: "{{user}}",
        content: { text: "change my background to a sunset gradient" },
      },
      {
        name: "Eliza",
        content: { text: 'Done. New wallpaper based on "sunset gradient".' },
      },
    ],
  ],
};
