...
import { type AuthProvider, markSignedIn } from "../auth/state.ts";
...
function resolveBinary(provider: LoginProvider): string {
  return existsSync(provider.binaryPath)
    ? provider.binaryPath
    : provider.binaryName;
}
 ...
 * weekly.
 */
export function extractLoginPrompt(buf: string): {
  url: string | null;
  code: string | null;
} {
...
export async function runLoginFlow(
  ...
  const openUrlFn =
    options.openUrlFn ??
    ((url: string, opts?: { appIdSuffix?: string }) =>
      openUrl(
        url,
        opts?.appIdSuffix !== undefined
          ? { appIdSuffix: opts.appIdSuffix }
          : {},
      ));
  const closeUrlFn = options.closeUrlFn ?? ((url: string) => closeUrl(url));
  const tokenExists =
    options.tokenExists ?? ((path: string) => existsSync(path));
  const onSignedIn =
    options.onSignedIn ?? ((p: LoginProvider) => markSignedIn(p.authKey));
  ...
  let prompt: { url: string | null; code: string | null } = {
    url: null,
    code: null,
  };
  ...
  while (Date.now() < pollDeadline) {
    if (
      tokenExists(provider.tokenFile) ||
      (await probeLoggedIn(provider, spawnFn))
    ) {
      handle.kill();
...
function readLoginOptions(options: unknown): LoginRuntimeOptions {
  ...
  if (typeof o["urlTimeoutMs"] === "number")
    out.urlTimeoutMs = o["urlTimeoutMs"];
  if (typeof o["timeoutMs"] === "number") out.timeoutMs = o["timeoutMs"];
  if (typeof o["pollIntervalMs"] === "number")
    out.pollIntervalMs = o["pollIntervalMs"];
  ...
  return {
    ...
    handler: async (_runtime, _message, _state, options, callback) => {
      ...
      const flowOpts: RunLoginOptions = {
        ...
        ...(opts.closeUrlFn !== undefined
          ? { closeUrlFn: opts.closeUrlFn }
          : {}),
        ...(opts.tokenExists !== undefined
          ? { tokenExists: opts.tokenExists }
          : {}),
        ...(opts.onSignedIn !== undefined
  ...
  [
    ...
    [
      { name: "{{user}}", content: { text: "claude login" } },
      {
        name: "Eliza",
        content: {
          text: "You're already signed into Claude. I'll use it for app generation.",
        },
      },
    ],
  ],
  ...
  [
    ...
    [
      { name: "{{user}}", content: { text: "codex login" } },
      {
        name: "Eliza",
        content: {
          text: "Signed in to Codex. You can now ask me to 'build me a calendar' with real codegen.",
        },
      },
    ],
  ],
