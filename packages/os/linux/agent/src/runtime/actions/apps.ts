 ...
 */
export async function readAppEntry(
  root: string,
  slug: string,
  ...
  try {
    ...
    if (
      typeof parsed.last_built_by === "string" &&
      parsed.last_built_by.length > 0
    ) {
      builtBy = parsed.last_built_by;
    }
    if (
      typeof parsed.last_built_at === "string" &&
      parsed.last_built_at.length > 0
    ) {
...
export const LIST_APPS_ACTION: Action = {
  ...
  handler: async (_runtime, _message, _state, _options, callback) => {
    ...
    if (apps.length === 0) {
      if (callback)
        await callback({ text: EMPTY_LIST_REPLY, actions: ["LIST_APPS"] });
      return { success: true, text: EMPTY_LIST_REPLY };
    }
    const now = new Date();
    const header =
      apps.length === 1 ? "You have 1 app:" : `You have ${apps.length} apps:`;
  ...
  examples: [
    ...
    [
      { name: "{{user}}", content: { text: "what apps do i have" } },
      {
        name: "Eliza",
        content: {
          text: "You haven't built any apps yet. Try 'build me a calendar' to start.",
        },
      },
    ],
  ],
...
export const DELETE_APP_ACTION: Action = {
  ...
  handler: async (_runtime, message, _state, _options, callback) => {
    ...
    if (slug === "") {
      return {
        success: false,
        text: "That doesn't look like an app I can remove.",
      };
    }
