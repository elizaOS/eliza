...
import { clearFlow, type FlowState, getFlowState, setFlow } from "./state.ts";
 ...
 */
export function matchSsidReply(
  reply: string,
...
async function handleAwaitingSsid(
  ...
  const ssids = Array.isArray(state.data.networks)
    ? ((state.data.networks as unknown[]).filter(
        (s) => typeof s === "string",
      ) as string[])
    : [];
...
async function tryConnect(
  ...
  try {
    ...
    if (code === "auth") {
      const prior =
        typeof state.data.attempts === "number" ? state.data.attempts : 0;
...
async function handleAwaitingPassword(
  ...
  if (ssid === "") {
    clearFlow();
    return {
      reply: "I lost track of which network — try again from the top.",
      done: true,
    };
  }
...
export async function continueWifiFlow(
  ...
  if (state.flowId !== "wifi-setup") {
    return {
      reply: "I lost track — start over with 'connect to wifi'.",
      done: true,
    };
  }
  switch (state.step) {
