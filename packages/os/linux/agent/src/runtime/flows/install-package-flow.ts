...
function isYes(message: string): boolean {
  ...
  return yes.some(
    (y) => norm === y || norm.startsWith(y + " ") || norm.startsWith(y + "."),
  );
}
...
function isNo(message: string): boolean {
  ...
  return no.some(
    (n) => norm === n || norm.startsWith(n + " ") || norm.startsWith(n + "."),
  );
}
  ...
  const promise: Promise<InstallResult> = (async () => {
    ...
    if (cancelled) {
      return {
        ...
        exitCode,
        installedCount: tracker.installedCount,
        durationMs,
        lastError,
      };
    }
...
async function handleConfirm(
  ...
  const packages = Array.isArray(state.data.packages)
    ? ((state.data.packages as unknown[]).filter(
        (p) => typeof p === "string",
      ) as string[])
    : [];
  if (packages.length === 0) {
    clearFlow();
    return {
      reply: "I lost track of which packages — try the install again.",
      done: true,
    };
  }
  if (isNo(message)) {
...
function formatInstallResult(
  ...
  if (result.status === "ok") {
    ...
    const extras =
      result.installedCount > packages.length
