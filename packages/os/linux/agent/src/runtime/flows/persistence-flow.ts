...
const DEFAULT_RUNNER: PersistenceRunner = {
  async run(passphrase) {
    return await new Promise<PersistenceResult>((resolve) => {
      const script =
        process.env.USBELIZA_PERSIST_SCRIPT ??
        "/usr/local/bin/usbeliza-persistence-setup";
...
async function handleAwaitingConfirm(
  ...
  if (isNo(message)) {
    ...
    return {
      reply:
        "OK, no encryption for now. Say 'set up persistence' anytime to start.",
      done: true,
    };
  }
...
async function handleAwaitingConfirm2(
  ...
  const passphrase =
    typeof state.data.passphrase === "string" ? state.data.passphrase : "";
  if (passphrase === "") {
    clearFlow();
    return {
      reply:
        "I lost track of the passphrase — let's start over with 'set up persistence'.",
      done: true,
    };
  }
