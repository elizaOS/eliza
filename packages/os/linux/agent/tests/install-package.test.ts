...
afterEach(() => {
  ...
  if (originalDangerous === undefined)
    delete process.env.USBELIZA_ALLOW_DANGEROUS_PACKAGES;
    ...
    const stream: SpawnStream = {
      ...
      exit: new Promise<number | null>((resolve) => {
        if (opts.delayMs !== undefined) {
          setTimeout(
            () => resolve(killed ? 130 : (opts.exitCode ?? 0)),
            opts.delayMs,
          );
        } else {
...
describe("hasInstallIntent", () => {
  ...
  test("validate() agrees with hasInstallIntent", async () => {
    expect(
      await INSTALL_PACKAGE_ACTION.validate(
        fakeRuntime,
...
describe("INSTALL_PACKAGE handler (confirmation flow entry)", () => {
  ...
  test("group-phrase expansion installs all in the group", async () => {
    ...
    const flow = getFlowState();
    expect(flow?.data.packages).toEqual([
      "i3",
      "i3status",
      "i3lock",
      "dmenu",
  ...
  test("missing package returns 'I can't find' error", async () => {
    ...
    expect(result?.success).toBe(false);
    expect(captured.some((t) => t.toLowerCase().includes("can't find"))).toBe(
      true,
    );
  });
