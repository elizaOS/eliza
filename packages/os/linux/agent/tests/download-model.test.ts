...
afterEach(() => {
  ...
  if (originalActiveToml === undefined)
    delete process.env.USBELIZA_ACTIVE_MODEL_TOML;
...
describe("parseCurlPercent", () => {
  test("returns the leading percent from a progress row", () => {
    expect(parseCurlPercent("  3 4760M  3  166M  0 0  73.2M 0 0:01:04")).toBe(
      3,
    );
    expect(parseCurlPercent("100 4760M 100 4760M 0 0  90M 0 --:--:--")).toBe(
      100,
    );
  });
...
describe("buildActiveModelToml", () => {
  test('emits `path = "<abs>"` newline', () => {
    const toml = buildActiveModelToml(
      "/home/eliza/.eliza/models/eliza-1-4b.gguf",
    );
