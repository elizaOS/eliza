...
describe("formatRelativeTime", () => {
  ...
  test("under 1 hour → minutes", () => {
    expect(formatRelativeTime(new Date("2026-05-11T11:30:00Z"), now)).toBe(
      "30 min ago",
    );
  ...
  test("under 1 day → hours", () => {
    expect(formatRelativeTime(new Date("2026-05-11T10:00:00Z"), now)).toBe(
      "2 hours ago",
    );
  ...
  test("under 7 days → days", () => {
    ...
    expect(formatRelativeTime(new Date("2026-05-08T12:00:00Z"), now)).toBe(
      "3 days ago",
    );
  });

  test("older than 7 days → absolute YYYY-MM-DD", () => {
    expect(formatRelativeTime(new Date("2026-04-01T12:00:00Z"), now)).toBe(
      "on 2026-04-01",
    );
  });

  test("future timestamps fall through to absolute (clock skew)", () => {
    expect(formatRelativeTime(new Date("2026-06-01T12:00:00Z"), now)).toBe(
      "on 2026-06-01",
    );
  });
});
...
describe("DELETE_APP handler", () => {
  test("validate rejects messages without a delete verb", async () => {
    const ok = await DELETE_APP_ACTION.validate(
      fakeRuntime,
      memoryOf("hello there"),
    );
