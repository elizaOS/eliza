...
afterEach(() => {
  ...
  if (originalPowerRoot === undefined)
    delete process.env.USBELIZA_POWER_SUPPLY_ROOT;
  else process.env.USBELIZA_POWER_SUPPLY_ROOT = originalPowerRoot;
  if (originalCalibrationFile === undefined)
    delete process.env.USBELIZA_CALIBRATION_FILE;
...
describe("BATTERY_STATUS_ACTION handler", () => {
  ...
  test("validate is always true (deterministic responder)", async () => {
    expect(
      await BATTERY_STATUS_ACTION.validate?.(fakeRuntime, memoryOf("anything")),
    ).toBe(true);
  });
...
describe("formatTimeReply", () => {
  ...
  test("honors a specified IANA timezone", () => {
    ...
    const reply = formatTimeReply(
      new Date("2026-05-11T20:00:00Z"),
      "America/Los_Angeles",
    );
...
describe("CURRENT_TIME_ACTION handler", () => {
  ...
  test("validate is always true", async () => {
    expect(
      await CURRENT_TIME_ACTION.validate?.(fakeRuntime, memoryOf("anything")),
    ).toBe(true);
  });
