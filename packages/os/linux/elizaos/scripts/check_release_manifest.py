...
REPO_ROOT = VARIANT_DIR.parents[1]
DEFAULT_REPORT = REPO_ROOT / "upstreams/research/chip/build/reports/release_manifest.json"
RELEASE_SCHEMA = (
    REPO_ROOT / "release/schema/elizaos-os-release-manifest.schema.json"
)
...
RISCV64_BUN_VERSION_INPUT = "toolchains/bun-riscv64/bun-version.json"
...
def _recapture_commands(manifest_path: Path, message: str) -> list[str]:
    ...
    iso = f"linux/elizaos/out/{filename}" if isinstance(filename, str) else "<iso>"
    qemu_evidence = "linux/elizaos/evidence/qemu_virt_boot.json"
    transcript = "linux/elizaos/evidence/qemu_virt_boot.transcript.log"
    qemu_report = "upstreams/research/chip/build/reports/qemu_virt_smoke.json"
    commands = [
        (
            "python3 linux/elizaos/scripts/qemu_virt_smoke.py "
            f"--iso {iso} --evidence {qemu_evidence} --transcript {transcript} "
            f"--report {qemu_report}"
        ),
        (
            "python3 linux/elizaos/scripts/check_release_manifest.py "
