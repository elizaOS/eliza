...
`linux/build.sh`, and its Debian-only images are not release
...
Two build orchestrators remain in this tree during migration:

- **live-build (retired release path; documented below).** Historical Debian
  ISO work via `lb config` / `lb build`; it cannot produce a v1 release.
- **mkosi (canonical v1 source, still under qualification).** See
  `mkosi/README.md`. It produces persistent `*.raw[.zst]` GPT disk images.
  Targets: `make mkosi-build ARCH=… PROFILE=…`, `make mkosi-summary`, and
  `make mkosi-lint`. No mkosi output is promotable until the signed desktop
  artifact, QEMU, installer, persistence, and hardware gates in the accepted
  v1 architecture contract are satisfied for the advertised architecture.
...
`toolchains/bun-riscv64/run-build.sh` and stage it with the
Debian wrapper plus the matching musl runtime:

```sh
make -C linux/elizaos stage-agent-artifacts ARCH=riscv64
make -C linux/elizaos riscv64-agent-runtime-smoke
```
...
```sh
make -C linux/elizaos stage-agent-artifacts ARCH=riscv64 RISCV64_RUNTIME=node
make -C linux/elizaos riscv64-agent-runtime-smoke
```
...
```sh
make -C linux/elizaos riscv64-gui-kiosk-iso-check
make -C linux/elizaos arm64-gui-kiosk-iso-check
```
...
against the schema at `release/schema/`. It is fail-closed:
...
considered for release promotion. See `CLAUDE.md` for the
