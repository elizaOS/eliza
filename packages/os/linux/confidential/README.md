...
`docs/tee-os-implementation-plan.md` §1.3 / OS-1. It is **separate
from** the Debian live-build product under
`linux/`. The live-build
...
```
node scripts/generate-tee-measurements.mjs --policy policy/confidential-policy.json ...
...
- The `tee` measured-boot block in the confidential release manifest
  (`release/confidential-2026-05-21/manifest.json`), validated by
  `scripts/validate-release-manifest.mjs` and
...
- The runtime-evidence bridge with mock fixtures
  (`scripts/tee-evidence-bridge.mjs`), which emits the normalized
...
| Real CVM boot + quote | `tdx-cvm-boot-smoke` | 4th/5th-gen Xeon TDX host | `node scripts/tee-evidence-bridge.mjs --quote-source tappd --socket /var/run/dstack.sock` |
