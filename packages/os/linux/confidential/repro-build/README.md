...
`scripts/verify-image-reproducibility.mjs` and its tests against
...
These must be vendored under `linux/confidential/` (pinned by commit)
...
| meta-elizaos | linux/confidential/meta-elizaos | `buildInputs.layers[].commit` |
...
#    linux/confidential/ (git submodules pinned by commit).
...
node scripts/generate-tee-measurements.mjs ...  # golden measurements
...
node scripts/verify-image-reproducibility.mjs \
  --input linux/confidential/image-manifest.example.json \
