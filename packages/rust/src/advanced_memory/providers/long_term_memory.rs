...
use crate::types::components::{ProviderHandler, ProviderResult, ProviderDefinition};
    ...
    fn empty_result() -> ProviderResult {
        ProviderResult {
            ...
            values: Some(HashMap::from([
                ("longTermMemories".to_string(), serde_json::Value::String(String::new())),
            ])),
            data: Some(HashMap::from([
                ("memoryCount".to_string(), serde_json::Value::Number(0.into())),
            ])),
        }
    }

    /// Format memories grouped by category, matching TypeScript/Python behavior.
    async fn get_formatted_memories(&self, service: &MemoryService, entity_id: UUID) -> (String, Vec<LongTermMemory>) {
    ...
    async fn get(
        ...
        if let Some(service_arc) = service_opt {
             if let Some(memory_service) = service_arc.as_any().downcast_ref::<MemoryService>() {
                 ...
                 let category_list = category_counts.iter()
                 ...
                 return Ok(ProviderResult {
                     ...
                     values: Some(HashMap::from([
                         ("longTermMemories".to_string(), serde_json::Value::String(text)),
                         ("memoryCategories".to_string(), serde_json::Value::String(category_list.clone())),
                     ])),
                     data: Some(HashMap::from([
                         ("memoryCount".to_string(), serde_json::Value::Number(memories.len().into())),
                         ("categories".to_string(), serde_json::Value::String(category_list)),
                     ])),
                 });
             }
        }
