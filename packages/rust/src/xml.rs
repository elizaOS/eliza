...
pub fn parse_key_value_xml(xml: &str) -> Option<HashMap<String, String>> {
    ...
    // Parse individual tags (direct children)
    let mut pos = 0;
    while pos < response_content.len() {
        if let Some(tag_start_off) = response_content[pos..].find('<') {
            let tag_start = pos + tag_start_off;
            ...
            // Find tag name end
            if let Some(tag_end_off) = response_content[tag_start..].find('>') {
                let tag_end = tag_start + tag_end_off;
...
pub fn parse_action_params(
    ...
    if inner_trimmed.starts_with('{') || inner_trimmed.starts_with('[') {
        ...
        match v {
            serde_json::Value::Object(map) => {
                for (action_name, params_v) in map {
                    if let serde_json::Value::Object(pmap) = params_v {
                        let mut param_map: HashMap<String, serde_json::Value> = HashMap::new();
                        for (k, val) in pmap {
                            param_map.insert(k, val);
                        }
    ...
    for (action_name, action_body) in extract_xml_children(inner_trimmed) {
        let mut param_map: HashMap<String, serde_json::Value> = HashMap::new();
...
mod tests {
    ...
    #[test]
    fn test_parse_action_params_json_object_wrapped() {
        let xml = r#"<params>{"POST":{"text":"hi","replyTo":"123"}}</params>"#;
        let parsed = parse_action_params(xml);
        assert_eq!(
            parsed
                .get("POST")
    ...
    #[test]
    fn test_parse_action_params_json_array_wrapped() {
        let xml = r#"<params>[{"POST":{"text":"a"}},{"REPLY":{"text":"b"}}]</params>"#;
        let parsed = parse_action_params(xml);
        assert_eq!(
            parsed
                .get("POST")
