...
import { PostSubscriptionAnthropicExchangeRequestSchema } from "@elizaos/core/contracts/subscription-routes";
import { PostSubscriptionAnthropicSetupTokenRequestSchema } from "@elizaos/core/contracts/subscription-routes";
import { PostSubscriptionOpenAIExchangeRequestSchema } from "@elizaos/core/contracts/subscription-routes";
...
import { type OAuthCredentials } from "@elizaos/auth/auth/types";
import { type RouteRequestContext } from "@elizaos/core/api/route-helpers";
import { type SubscriptionProvider } from "@elizaos/auth/auth/types";
import { updateAccountMetadata } from "@elizaos/auth/auth/account-storage";
type AuthModule = typeof import("@elizaos/auth/auth");
export type SubscriptionAuthApi = Pick<AuthModule, "getSubscriptionStatus" | "exchangeAnthropicAuthorizationCode" | "fetchAnthropicOAuthProfile" | "startAnthropicLogin" | "startCodexLogin" | "submitProviderFlowCode" | "saveCredentials" | "applySubscriptionCredentials" | "deleteCredentials" | "deleteProviderCredentials">;
...
export async function handleSubscriptionRoutes(ctx: SubscriptionRouteContext): Promise<boolean> {
    ...
    if (method === "GET" && pathname === "/api/subscription/status") {
        try {
            ...
            const rows = baseRows.map((row) => {
                const linked = linkedAccounts[`${row.provider}:${row.accountId}`] ??
    ...
    if (method === "POST" &&
        ...
        if (rawAxe === null)
            return true;
        const parsedAxe = PostSubscriptionAnthropicExchangeRequestSchema.safeParse(rawAxe);
        if (!parsedAxe.success) {
            error(res, parsedAxe.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        const body = parsedAxe.data;
        try {
            const { saveCredentials, applySubscriptionCredentials, exchangeAnthropicAuthorizationCode, fetchAnthropicOAuthProfile, } = await loadSubscriptionAuth();
            ...
            const stored = saveCredentials("anthropic-subscription", credentials, accountId, storagePolicy);
            if (profile.email) {
                const metadata = updateAccountMetadata("anthropic-subscription", accountId, {
                    label: profile.email,
                    email: profile.email,
                    ...(profile.organizationId
                        ? { organizationId: profile.organizationId }
                        : {}),
                }, storagePolicy, stored.credentialGeneration);
                if (metadata.kind !== "updated") {
                    error(res, "Account login changed before its profile could be saved; retry login", 409);
                    return true;
                }
            }
    ...
    if (method === "POST" &&
        ...
        if (rawTok === null)
            return true;
        const parsedTok = PostSubscriptionAnthropicSetupTokenRequestSchema.safeParse(rawTok);
        if (!parsedTok.success) {
            error(res, parsedTok.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        ...
        try {
            ...
            if (!state.config.env)
                state.config.env = {};
            (state.config.env as Record<string, unknown>).__anthropicSubscriptionToken = trimmedToken;
            ctx.saveConfig(state.config);
            logger.info("[api] Saved Anthropic setup token for task agents (not applied to runtime — TOS restriction)");
    ...
    if (method === "POST" && pathname === "/api/subscription/openai/exchange") {
        ...
        if (rawOaeb === null)
            return true;
        const parsedOaeb = PostSubscriptionOpenAIExchangeRequestSchema.safeParse(rawOaeb);
        if (!parsedOaeb.success) {
            error(res, parsedOaeb.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        const body = parsedOaeb.data;
        try {
            const { saveCredentials, applySubscriptionCredentials, submitProviderFlowCode, } = await loadSubscriptionAuth();
            ...
            if (!flow) {
                ...
                try {
                    ...
                    logger.error(`[api] OpenAI account-flow exchange failed: ${String(err)}`);
    ...
    if (method === "DELETE" && pathname.startsWith("/api/subscription/")) {
        ...
        if (isSubscriptionProvider(provider)) {
            try {
                ...
                const deletedProviderId = subscriptionSelectionIdForStoredProvider(provider);
                ...
                if ((llmBackend === deletedProviderId || llmBackend === provider) &&
...
function subscriptionSelectionIdForStoredProvider(provider: SubscriptionProvider): string {
    switch (provider) {
        case "openai-codex":
 ...
 */
async function readRichLinkedAccountsFromPool(): Promise<Record<string, LinkedAccountConfig>> {
    try {
