...
function directRuntime(settings: Record<string, string | undefined> = {}) {
  return {
    character: {},
    plugins: [],
...
function effectiveDirectConfig(
  backend: string,
  primaryModel: string,
  env?: ElizaConfig["env"],
): ElizaConfig {
...
describe("detectRuntimeModel effective direct-provider status", () => {
  ...
  it("reports the effective Cerebras small/response model over a stale route default", () => {
    ...
    expect(detectRuntimeModel(directRuntime(), config)).toBe("gemma-4-31b");
  });
  ...
  it("matches plugin-openai response-handler override precedence", () => {
    ...
    expect(
      detectRuntimeModel(
        directRuntime({ OPENAI_RESPONSE_HANDLER_MODEL: "zai-glm-4.7" }),
        config,
      ),
    ).toBe("zai-glm-4.7");
  });
  ...
  it("leaves non-Cerebras direct-provider behavior unchanged", () => {
    const config = effectiveDirectConfig("openai", "gpt-5.6-luna", {
      CEREBRAS_SMALL_MODEL: "inactive-cerebras-model",
    });
    expect(detectRuntimeModel(directRuntime(), config)).toBe("gpt-5.6-luna");
  });
});
