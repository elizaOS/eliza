...
describe("scope helpers", () => {
  ...
  it("reads a room id only from string metadata", () => {
    expect(documentRoomId({ roomId: "room-1" })).toBe("room-1");
    expect(documentRoomId({ roomId: 42 })).toBeUndefined();
    expect(documentRoomId(undefined)).toBeUndefined();
  });
