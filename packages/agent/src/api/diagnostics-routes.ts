...
type DiagnosticsSseWriteJson = (res: http.ServerResponse, payload: unknown, event?: string) => void;
export interface DiagnosticsRouteContext extends RouteRequestMeta, Pick<RouteHelpers, "json"> {
    url: URL;
    ...
    subscribeAuditFeed: (subscriber: (entry: AuditEntryLike) => void) => () => void;
}
...
function defaultWriteSseData(res: http.ServerResponse, data: string, event?: string): void {
    if (event) {
...
function defaultWriteSseJson(res: http.ServerResponse, payload: unknown, event?: string): void {
...
function applyLogFilter(buffer: readonly LogEntryLike[], filter: LogFilter): LogEntryLike[] {
...
function matchesAuditFilter(entry: AuditEntryLike, filters: {
    type?: string;
    severity?: string;
    sinceMs?: number;
}): boolean {
...
export async function handleDiagnosticsRoutes(ctx: DiagnosticsRouteContext): Promise<boolean> {
    const { req, res, method, pathname, url, logBuffer, eventBuffer, initSse, writeSseJson, auditEventTypes, auditSeverities, getAuditFeedSize, queryAuditFeed, subscribeAuditFeed, json, } = ctx;
    ...
    if (method === "POST" && pathname === "/api/logs/export") {
        ...
        if (!parsedExp.success) {
            errorFn(res, parsedExp.error.issues[0]?.message ?? 'format must be "json" or "csv"', 400);
            return true;
        }
        ...
        if (typeof body.since === "string" && body.since.trim()) {
            ...
            if (!Number.isSafeInteger(body.since)) {
                errorFn(res, 'Invalid "since" filter: expected epoch ms or ISO timestamp.', 400);
                return true;
            }
            sinceMs = body.since;
        }
        ...
        if (Array.isArray(body.tags)) {
            const first = body.tags.find((entry): entry is string => typeof entry === "string" && entry.trim().length > 0);
            if (first)
                tag = first.trim();
        }
    ...
    if (method === "GET" && pathname === "/api/agent/events") {
        ...
        if (runIdFilter) {
            autonomyEvents = autonomyEvents.filter((event) => event.runId === runIdFilter);
        }
        if (fromSeq !== undefined) {
            autonomyEvents = autonomyEvents.filter((event) => typeof event.seq === "number" && event.seq >= fromSeq);
        }
        let startIndex = 0;
        if (afterEventId) {
        ...
        const latestEventId = events.length > 0 ? events[events.length - 1].eventId : null;
        json(res, {
            events,
            latestEventId,
    ...
    if (method === "GET" && pathname === "/api/security/audit") {
        ...
        const streamRequested = isTruthyQueryParam(url.searchParams.get("stream")) ||
