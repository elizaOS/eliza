 ...
 */
import { MEDIA_URL_IN_TEXT_RE } from "./media-store.ts";
import { ensureThumbnailForStoredFile } from "./media-store.ts";
import { fetchRemoteMedia } from "@elizaos/core/media";
import { gcUnreferencedMedia } from "./media-store.ts";
import { handleMediaRouteRequest } from "./media-store.ts";
import { isStoredMediaUrl } from "./media-store.ts";
import { logger } from "@elizaos/core";
import { mediaFileNameFromUrl } from "./media-store.ts";
import { nodeLookupFn } from "@elizaos/core";
import { nodePinnedFetch } from "@elizaos/core";
import { persistAttachmentUrlIfInline } from "./media-store.ts";
 ...
 */
async function rehostRemoteMediaUrl(url: string, options: {
    signal?: AbortSignal;
    timeoutMs?: number;
} = {}): Promise<string | null> {
...
export const mediaFileRoute: Route = {
    ...
    public: true,
    name: "media-file",
    publicReason: "Media URLs are content-addressed capability links served pre-auth.",
    routeHandler: async (ctx) => {
        const filename = ctx.params?.filename ?? "";
        const result = handleMediaRouteRequest(`${MEDIA_URL_PREFIX}${filename}`, ctx.method ?? "GET", 
...
export function registerMediaPipelineHook(runtime: IAgentRuntime): void {
    runtime.registerPipelineHook({
        ...
        handler: async (_rt, ctx) => {
            if (ctx.phase !== "outgoing_before_deliver")
                return;
...
function transcriptContentFromMemory(memory: Memory): unknown {
    return (memory.content as {
        transcript?: unknown;
    } | undefined)?.transcript;
}
function collectTranscriptAudioReference(memory: Memory, addUrl: (value: unknown) => void, diagnostics?: MediaGcDiagnostics): void {
...
export function collectReferencedMedia(memories: Memory[], diagnostics?: MediaGcDiagnostics): Set<string> {
    ...
    for (const memory of memories) {
        const attachments = (memory.content as {
            attachments?: Array<{
                url?: unknown;
...
export async function scheduleMediaGc(runtime: IAgentRuntime): Promise<void> {
    ...
    if (existing.some((task) => task.name === MEDIA_GC_TASK_NAME))
        return;
