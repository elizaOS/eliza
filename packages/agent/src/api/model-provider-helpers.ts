...
import { DEFAULT_ELIZA_CLOUD_FREE_TEXT_MODEL } from "@elizaos/core/contracts/service-routing";
...
export function getModelOptions(): {
    ...
    const freeDefaultMatchesRecommended = DEFAULT_ELIZA_CLOUD_FREE_TEXT_MODEL === DEFAULT_ELIZA_CLOUD_TEXT_MODEL;
    const allModels = [
        {
...
export function paramKeyToCategory(paramKey: string): ModelCategory {
    ...
    if (k.includes("TTS"))
        return "tts";
    if (k.includes("STT") || k.includes("TRANSCRIPTION"))
        return "stt";
    return "chat";
}
const MODELS_CACHE_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours
const PROVIDER_ENV_KEYS: Record<string, {
    envKey: string;
    altEnvKeys?: string[];
    baseUrl?: string;
}> = {
...
export function providerCachePath(providerId: string): string {
    ...
    if (path.dirname(result) !== cacheDir) {
        throw new ElizaError(`Model provider cache path escapes the cache directory: ${providerId}`, { code: "INVALID_MODEL_PROVIDER_ID", context: { providerId } });
    }
    return result;
}
export function readProviderCache(providerId: string): ProviderCache | null {
    try {
        ...
        if (cache.version !== 1 || !cache.fetchedAt || !cache.models)
            return null;
...
export function writeProviderCache(cache: ProviderCache): void {
    try {
        ...
        logger.warn(`[model-catalog] Failed to write cache for ${cache.providerId}: ${e instanceof Error ? e.message : e}`);
    }
}
// ── Provider fetchers ────────────────────────────────────────────────────
/** Fetch models from unknown provider's /v1/models endpoint (standard REST). */
export async function fetchModelsREST(providerId: string, apiKey: string, baseUrl: string): Promise<CachedModel[]> {
    try {
        ...
        logger.warn(`[model-catalog] Failed to fetch models for ${providerId}: ${e instanceof Error ? e.message : e}`);
        return [];
    }
}
export function restTypeToCategory(type: string): ModelCategory {
    ...
    if (t.includes("embed"))
        return "embedding";
    if (t === "image" || t.includes("image-generation"))
        return "image";
    if (t.includes("tts") || t.includes("speech"))
        return "tts";
    if (t.includes("stt") || t.includes("transcription") || t.includes("whisper"))
        return "stt";
    if (t === "language" || t === "chat" || t.includes("text"))
        return "chat";
...
export async function fetchAnthropicModels(apiKey: string): Promise<CachedModel[]> {
    try {
        ...
        // error-policy:J4 an unavailable catalog is an explicit empty provider list.
        logger.warn(`[model-catalog] Failed to fetch Anthropic models: ${e instanceof Error ? e.message : e}`);
        return [];
    }
}
export async function fetchGoogleModels(apiKey: string): Promise<CachedModel[]> {
    try {
        ...
        // error-policy:J4 an unavailable catalog is an explicit empty provider list.
        logger.warn(`[model-catalog] Failed to fetch Google models: ${e instanceof Error ? e.message : e}`);
        return [];
    }
}
...
export async function fetchOpenRouterModels(apiKey: string): Promise<CachedModel[]> {
    ...
    const [chatRes, embedRes] = await Promise.all([
        fetch("https://openrouter.ai/api/v1/models?output_modalities=all", {
            ...
            logger.warn(`[model-catalog] OpenRouter text catalog unavailable: ${error instanceof Error ? error.message : String(error)}`);
            return null;
        }),
        fetch("https://openrouter.ai/api/v1/embeddings/models", {
            ...
            // error-policy:J4 the combined catalog can remain partially available.
            logger.warn(`[model-catalog] OpenRouter embedding catalog unavailable: ${error instanceof Error ? error.message : String(error)}`);
            return null;
        }),
    ]);
    ...
    if (chatRes?.ok) {
        try {
            ...
            for (const m of data.data ?? []) {
                const outputs = (m.architecture?.output_modalities ?? []).map((value) => value.toLowerCase());
                ...
                else if (outputs.includes("image"))
                    category = "image";
                else if (outputs.includes("audio"))
                    category = "tts";
            ...
            logger.warn(`[model-catalog] Invalid OpenRouter text catalog: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    if (embedRes?.ok) {
        try {
            ...
            logger.warn(`[model-catalog] Invalid OpenRouter embedding catalog: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
...
export async function fetchNearAIModels(apiKey: string, baseUrl: string): Promise<CachedModel[]> {
    try {
        ...
        // error-policy:J4 an unavailable catalog is an explicit empty provider list.
        logger.warn(`[model-catalog] Failed to fetch NEAR AI models: ${e instanceof Error ? e.message : e}`);
        return [];
    }
}
export async function fetchProviderModels(providerId: string, apiKey: string, baseUrl?: string): Promise<CachedModel[]> {
    switch (providerId) {
        case "anthropic":
        ...
        case "deepseek":
            return fetchModelsREST(providerId, apiKey, baseUrl ?? "https://api.deepseek.com");
        case "zai":
            return fetchModelsREST(providerId, apiKey, baseUrl ?? "https://api.z.ai/api/paas/v4");
        case "nearai":
            return fetchNearAIModels(apiKey, baseUrl ?? "https://cloud-api.near.ai/v1");
        case "moonshot":
            return fetchModelsREST(providerId, apiKey, baseUrl ?? "https://api.moonshot.ai/v1");
        default:
            return [];
    }
}
/** Fetch + cache a single provider. Returns cached models or empty array. */
export async function getOrFetchProvider(providerId: string, force = false): Promise<CachedModel[]> {
    if (!force) {
...
export async function getOrFetchAllProviders(force = false): Promise<Record<string, CachedModel[]>> {
    const result: Record<string, CachedModel[]> = {};
    const fetches: Array<Promise<void>> = [];
    for (const providerId of Object.keys(PROVIDER_ENV_KEYS)) {
        fetches.push(getOrFetchProvider(providerId, force).then((models) => {
            if (models.length > 0)
