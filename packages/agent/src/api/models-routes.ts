...
export interface ModelsRouteContext extends RouteRequestMeta, Pick<RouteHelpers, "json"> {
    url: URL;
    providerCachePath: (provider: string) => string;
    getOrFetchProvider: (provider: string, force: boolean) => Promise<unknown[]>;
    getOrFetchAllProviders: (force: boolean) => Promise<Record<string, unknown[]>>;
...
export async function handleModelsRoutes(ctx: ModelsRouteContext): Promise<boolean> {
    ...
    if (method !== "GET" || pathname !== "/api/models")
        return false;
    const catalogOnlyParsed = parseOptionalBooleanQuery(url.searchParams.get("catalogOnly"));
