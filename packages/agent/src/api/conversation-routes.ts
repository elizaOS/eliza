...
import { ChatIdempotencyWaitAbortedError } from "./chat-routes.ts";
import { ELIZA_TRACE_ID_HEADER } from "./conversation-trace.ts";
import { ElizaError } from "@elizaos/core";
import { InferenceTurnTimer } from "@elizaos/core";
import { LOCAL_VOICE_RUNTIME_AGENT_HEADER } from "@elizaos/core/voice";
import { LOCAL_VOICE_RUNTIME_CONVERSATION_HEADER } from "@elizaos/core/voice";
import { MESSAGE_SOURCE_AGENT_GREETING } from "@elizaos/core";
import { MESSAGE_SOURCE_CLIENT_CHAT } from "@elizaos/core";
import { PatchConversationRequestSchema } from "@elizaos/core/contracts/conversation-routes";
import { PostConversationCleanupEmptyRequestSchema } from "@elizaos/core/contracts/conversation-routes";
import { PostConversationRequestSchema } from "@elizaos/core/contracts/conversation-routes";
import { PostConversationTruncateRequestSchema } from "@elizaos/core/contracts/conversation-routes";
import { PostSeedMessagesRequestSchema } from "@elizaos/core/contracts/conversation-routes";
import { RoomHandlerQueueClosedError } from "@elizaos/core";
import { RoomHandlerQueueGlobalSaturatedError } from "@elizaos/core";
import { RoomHandlerQueueSaturatedError } from "@elizaos/core";
import { TodoCutoverContractError } from "@elizaos/core/todo-cutover";
import { admitChatMessageId } from "./chat-routes.ts";
import { assertConversationConnectionRuntime } from "./conversation-connection-readiness.ts";
import { attestAuthenticatedApiDeliveryAudience } from "@elizaos/core";
import { authorizeOwnerExclusiveDisclosure } from "@elizaos/core";
import { buildConversationRoomMetadata } from "./conversation-metadata.ts";
import { buildUserMessages } from "./server-helpers.ts";
import { captureConversationConnectionDescriptor } from "./conversation-connection-readiness.ts";
import { classifyChatFailure } from "./chat-routes.ts";
import { compareConversationsByRecency } from "./conversation-sort.ts";
import { compareMemoriesByCreatedAt } from "./conversation-sort.ts";
...
import { createChatTokenStreamWriter } from "./chat-stream-writer.ts";
import { createMessageMemory } from "@elizaos/core";
import { createUniqueUuid } from "@elizaos/core";
import { decodePathComponent } from "./server-helpers.ts";
import { deleteConversationMemories } from "../services/conversation-message-service.ts";
import { deleteConversationMessage } from "../services/conversation-message-service.ts";
import { enforceTrustedDeliveryAudienceAtEgress } from "@elizaos/plugin-assistant";
import { evaluatePlannedReplyEgress } from "@elizaos/plugin-assistant";
import { evictOldestConversation } from "./memory-bounds.ts";
import { generateChatResponse } from "./chat-routes.ts";
import { generateConversationTitle } from "./chat-routes.ts";
import { generateMessageCorpus } from "./message-corpus.ts";
import { getChatFailureReply } from "./chat-routes.ts";
import { getChatMessageIdOutcome } from "./chat-routes.ts";
...
import { isChatFailureKind } from "@elizaos/core/contracts/chat";
import { isConversationConnectionError } from "./conversation-connection-readiness.ts";
import { isIntentionalNoResponseResult } from "./chat-routes.ts";
import { isScheduledTask } from "@elizaos/plugin-scheduling";
import { logger } from "@elizaos/core";
import { mergeEffectReceipts } from "@elizaos/core";
import { nextInferenceTurnId } from "@elizaos/core";
import { normalizeAccountConnectRequest } from "./chat-routes.ts";
import { normalizeActionFailureProvenance } from "@elizaos/core";
import { normalizeActionReplyFailure } from "@elizaos/core";
import { normalizeChatResponseText } from "./chat-routes.ts";
...
import { parseReplyRecoveryHistorySelection } from "@elizaos/plugin-assistant";
import { parseSharedTodoCutoverSnapshot } from "@elizaos/core/todo-cutover";
import { persistAssistantConversationMemory } from "./chat-routes.ts";
import { persistConversationMemory } from "./chat-routes.ts";
import { persistExactConversationMemory } from "./chat-routes.ts";
import { persistExactConversationMemoryResult } from "./chat-routes.ts";
import { persistInterruptedAssistantReceipt } from "./chat-routes.ts";
import { prepareConversationConnectionRoom } from "./conversation-connection-readiness.ts";
import { projectCompleteToolValueForModel } from "@elizaos/core";
import { readChatRequestPayload } from "./chat-routes.ts";
import { readDurableConversationChatMarker } from "@elizaos/core/conversation-chat-marker";
import { recordOwnerGrant } from "@elizaos/core";
import { recordRoleGrant } from "@elizaos/core";
import { releaseChatMessageId } from "./chat-routes.ts";
import { resolveAppUserName } from "./server-helpers.ts";
import { resolveAppliedUserFacingEffectReceipts } from "@elizaos/core";
import { resolveClientChatAdminEntityId } from "./client-chat-admin.ts";
import { resolveConversationTraceContext } from "./conversation-trace.ts";
import { resolveHttpAccessContext } from "./http-access-context.ts";
import { resolveNoResponseFallback } from "./chat-routes.ts";
import { resolvePlannedReplyEgress } from "@elizaos/plugin-assistant";
import { resolveStateDir } from "../config/paths.ts";
import { resolveTrustedApiPrincipal } from "./chat-routes.ts";
import { resolveWaifuChatAccess } from "./waifu-chat-role-resolver.ts";
import { runWithInferenceTiming } from "@elizaos/core";
import { sanitizeConversationMetadata } from "./conversation-metadata.ts";
import { scheduleConversationConnectionEnsure } from "./conversation-connection-readiness.ts";
import { scheduleImportedConversationEmbeddings } from "./conversation-import-embeddings.ts";
import { seedMessageCorpus } from "./message-corpus.ts";
import { selectAttachmentsForViewer } from "./attachment-disclosure.ts";
import { serializeConversationConnectionRoomDeletion } from "./conversation-connection-readiness.ts";
import { setChatMessageIdOutcome } from "./chat-routes.ts";
...
import { truncateConversationMessages } from "../services/conversation-message-service.ts";
import { type AccountConnectRequest } from "./chat-routes.ts";
import { type ActionResult } from "@elizaos/core";
import { type AgentHttpRequestAuthorization } from "../runtime/host-bridge.ts";
import { type AgentRuntime } from "@elizaos/core";
import { type ChatFailureKind } from "@elizaos/core/contracts/chat";
import { type ChatGenerationResult } from "./chat-routes.ts";
import { type ChatMessageIdOutcome } from "./chat-routes.ts";
import { type ChatMessageIdReservation } from "./chat-routes.ts";
import { type ChatTerminalFailure } from "@elizaos/core/contracts/chat";
import { type Content } from "@elizaos/core";
import { type ConversationConnectionDescriptor } from "./conversation-connection-readiness.ts";
import { type ConversationMeta } from "./server-types.ts";
import { type DurableConversationChatMarker } from "@elizaos/core/conversation-chat-marker";
import { type ElizaConfig } from "../config/config.ts";
import { type LogEntry } from "./chat-routes.ts";
import { type Memory } from "@elizaos/core";
import { type RoleGrantSource } from "@elizaos/core";
...
import { type SerializedMessageAttachment } from "./attachment-disclosure.ts";
import { type SharedTodoImportReceipt } from "./todo-cutover-import.ts";
import { type TrustedApiPrincipal } from "@elizaos/core";
import { type UUID } from "@elizaos/core";
import { type WaifuChatAccess } from "./waifu-chat-role-resolver.ts";
import { type WaifuChatWorldRole } from "./waifu-chat-role-resolver.ts";
import { validateUuid } from "@elizaos/core";
import { waifuChatRoleToWorldRole } from "./waifu-chat-role-resolver.ts";
import { withStandaloneTrajectory } from "@elizaos/core";
import { writeChatStatusSse } from "./chat-stream-writer.ts";
import { writeChatTokenSse } from "./chat-stream-writer.ts";
import { writeChatToolSse } from "./chat-stream-writer.ts";
import { writeSse } from "./chat-stream-writer.ts";
...
type DiscordConversationModule = {
    cacheDiscordAvatarForRuntime: (runtime: AgentRuntime, avatarUrl: string | undefined, userId?: string) => Promise<string | undefined>;
    isCanonicalDiscordSource: (source: unknown) => boolean;
    resolveDiscordMessageAuthorProfile: (runtime: AgentRuntime, channelId: string, messageId: string) => Promise<DiscordProfileLike | null>;
    resolveDiscordUserProfile: (runtime: AgentRuntime, userId: string) => Promise<DiscordProfileLike | null>;
    resolveStoredDiscordEntityProfile: (runtime: AgentRuntime, entityId: string | undefined) => Promise<DiscordProfileLike | null>;
};
let discordConversationPromise: Promise<DiscordConversationModule> | null = null;
function getDiscordConversationApi(): Promise<DiscordConversationModule> {
    discordConversationPromise ??= import("@elizaos/plugin-discord") as Promise<unknown> as Promise<DiscordConversationModule>;
...
function persistDeletedConversationIdsToState(ids: Set<string>): void {
    ...
    fs.writeFileSync(path.join(dir, DELETED_CONVERSATIONS_FILENAME), JSON.stringify(payload, null, 2), { encoding: "utf-8", mode: 0o600 });
}
// ---------------------------------------------------------------------------
...
function readCanonicalSingleHeader(req: Pick<http.IncomingMessage, "headers">, name: string): string | null | "invalid" {
...
function resolveLocalVoiceRuntimeFence(req: Pick<http.IncomingMessage, "headers">, state: ConversationRouteState, conversationId: string): LocalVoiceRuntimeFenceResolution {
    const expectedAgentId = readCanonicalSingleHeader(req, LOCAL_VOICE_RUNTIME_AGENT_HEADER);
    const expectedConversationId = readCanonicalSingleHeader(req, LOCAL_VOICE_RUNTIME_CONVERSATION_HEADER);
...
function isLocalVoiceRuntimeFenceCurrent(state: ConversationRouteState, fence: LocalVoiceRuntimeFence | null, conversation?: ConversationMeta): boolean {
    return (fence === null ||
...
function assertLocalVoiceTurnFenceCurrent(state: ConversationRouteState, fence: LocalVoiceRuntimeFence | null, conversation: ConversationMeta): void {
    if (fence === null)
        return;
    ...
    if (state.conversations.get(conversation.id) !== conversation ||
        state.deletedConversationIds.has(conversation.id)) {
...
function readViewInteractionClientId(req: Pick<http.IncomingMessage, "headers">): string | null {
...
function withViewInteractionClient(message: Memory, req: Pick<http.IncomingMessage, "headers">): Memory {
...
function buildConversationChatIdempotencyScope(runtime: AgentRuntime, roomId: UUID, principalId: UUID): string {
    return `${runtime.agentId}:${roomId}:${principalId}`;
}
function isRoomQueueBackpressureError(error: unknown): boolean {
    return (error instanceof RoomHandlerQueueSaturatedError ||
        error instanceof RoomHandlerQueueGlobalSaturatedError);
}
function roomQueueAdmissionStatus(error: unknown): number {
    if (isRoomQueueBackpressureError(error))
        return 429;
    if (error instanceof RoomHandlerQueueClosedError)
        return 503;
    return 500;
}
async function awaitConversationChatAdmission(scope: string, clientMessageId: string | null, fingerprint: string, signal: AbortSignal): Promise<ConversationChatAdmission> {
    while (true) {
...
// ---------------------------------------------------------------------------
export function resolveConversationAdminEntityId(state: ConversationRouteState): UUID {
...
function createRequestDisconnectAbortTracker({ req, res, operation, }: {
    ...
    const abort = (reason?: unknown) => {
        if (completed || aborted)
            return;
        aborted = true;
        abortController.abort(reason instanceof Error ? reason : new Error(`${operation} aborted`));
    };
    const register = (source: unknown, event: string, listener: StreamEventListener) => {
        if (!isStreamEventSource(source))
            return;
    ...
    const onResponseClose = () => {
        const ended = Boolean((res as http.ServerResponse & {
...
function createConversationStreamDisconnectTracker({ req, res, conversationId, roomId, }: {
    req: http.IncomingMessage;
    res: http.ServerResponse;
    ...
    let aborted = false;
    let completed = false;
    const requestSocket = isStreamSocketLike((req as http.IncomingMessage & {
        socket?: unknown;
    }).socket)
        ? ((req as http.IncomingMessage & {
            ...
            null)
        : null;
    const responseSocket = isStreamSocketLike((res as http.ServerResponse & {
        socket?: unknown;
    }).socket)
        ? ((res as http.ServerResponse & {
            ...
            null)
        : null;
    const responseEnded = () => Boolean((res as http.ServerResponse & {
        writableEnded?: boolean;
    }).writableEnded);
    const abort = (reason?: unknown) => {
        if (completed || aborted)
...
function writeConversationStreamHeartbeat(res: http.ServerResponse, disconnectTracker: ConversationStreamDisconnectTracker): void {
...
function isTurnAbortError(err: unknown): boolean {
    ...
    return (code === "TURN_ABORTED" ||
        err.name === "TurnAbortedError" ||
        err.message.startsWith("Turn aborted:"));
}
function ensureAdminEntityIdForRuntime(state: ConversationRouteState, runtime: AgentRuntime | null): UUID {
 ...
 */
export function resolveConversationCaller(req: http.IncomingMessage, state: ConversationRouteState, principal: TrustedApiPrincipal, runtime: AgentRuntime | null = state.runtime): ConversationCaller {
    const access = resolveWaifuChatAccess(req);
    if (access) {
        return {
            entityId: stringToUuid(`waifu-wallet:${access.walletAddress.toLowerCase()}`),
...
function addWaifuConversationOwnerMetadata(req: http.IncomingMessage, metadata: ConversationMeta["metadata"]): ConversationMeta["metadata"] {
    const access = resolveWaifuChatAccess(req);
    if (!access)
        return metadata;
    return {
...
function canWaifuAccessConversation(access: WaifuChatAccess | null, conv: ConversationMeta): boolean {
    if (!access || access.role === "admin")
        return true;
    return getWaifuChatOwnerWallet(conv) === access.walletAddress.toLowerCase();
}
function rejectWaifuConversationAccessIfNeeded(req: http.IncomingMessage, conv: ConversationMeta, error: ConversationRouteContext["error"], res: http.ServerResponse): boolean {
    const access = resolveWaifuChatAccess(req);
    if (canWaifuAccessConversation(access, conv))
        return false;
    error(res, "Conversation not found", 404);
    return true;
}
function rejectWaifuNonAdminMutationIfNeeded(req: http.IncomingMessage, error: ConversationRouteContext["error"], res: http.ServerResponse): boolean {
    const access = resolveWaifuChatAccess(req);
    if (!access || access.role === "admin")
        return false;
...
async function ensureWorldOwnershipAndRoles(runtime: AgentRuntime, worldId: UUID, ownerId: UUID, callerId: UUID, callerRole: WaifuChatWorldRole, callerGrantSource: RoleGrantSource, assertCurrent?: () => void): Promise<void> {
    const world = await runtime.getWorld(worldId);
    assertCurrent?.();
    if (!world) {
        throw new ElizaError("Conversation world is missing after connection initialization", {
    ...
    if (callerId !== ownerId) {
        if (callerGrantSource === "session") {
            ...
            // grant by the owner).
            if (!hasAtLeastRole(getEntityRole(metadata, callerId), callerRole) &&
                recordRoleGrant(metadata, callerId, callerRole, "session")) {
                needsUpdate = true;
            }
        }
        else if (recordRoleGrant(metadata, callerId, callerRole, callerGrantSource)) {
            needsUpdate = true;
        }
    }
    if (needsUpdate) {
...
function findPersistedGeneratedAssistantTurn(runtime: AgentRuntime, roomId: UUID, result: ChatGenerationResult): PersistedAssistantMemory | null {
    if (!Array.isArray(result.persistedResponseMessageIds) ||
...
async function resolvePersistedAssistantTurn(runtime: AgentRuntime, roomId: UUID, turnStartedAt: number, result: ChatGenerationResult, text: string, channelType: ChannelType, roomHandlerLease: RoomHandlerLease, userMessageId?: UUID, assertCurrent?: () => void): Promise<{
    kind: "durable";
    id: UUID;
    text: string;
} | {
    ...
    const generatedTurn = findPersistedGeneratedAssistantTurn(runtime, roomId, result);
    if (generatedTurn) {
        const generatedText = typeof generatedTurn.content.text === "string"
            ? generatedTurn.content.text
            : "";
        const persistedContent = buildPersistedAssistantContent(text, result, userMessageId);
        const generatedTerminalFailure = parseChatTerminalFailure(generatedTurn.content.terminalFailure);
        const terminalFailureNeedsReconciliation = result.terminalFailure !== undefined &&
        ...
        if (generatedText !== text ||
            ...
            try {
                ...
                throw new AssistantReplyPersistenceError("Failed to reconcile the persisted assistant reply", cause);
            }
        }
    ...
    if (shouldSkipResponseMemoryPersistence({
        content,
        roomId,
    ...
    try {
        persisted = await persistAssistantConversationMemory(runtime, roomId, content, channelType, turnStartedAt, crypto.randomUUID() as UUID, roomHandlerLease, assertCurrent);
    }
    catch (cause) {
        // error-policy:J2 attach the durable-turn boundary before the route
        // translates this into a terminal SSE error.
        throw new AssistantReplyPersistenceError("Failed to persist the assistant reply", cause);
    }
    if (!persisted?.id) {
        throw new AssistantReplyPersistenceError("Assistant reply persistence returned no durable message id");
    }
    return { kind: "durable", id: persisted.id as UUID, text };
}
function markConversationDeleted(state: ConversationRouteState, conversationId: string): void {
    const normalizedId = conversationId.trim();
    if (!normalizedId)
        return;
    if (state.deletedConversationIds.has(normalizedId))
        return;
...
async function deleteConversationRoomData(runtime: AgentRuntime, roomId: UUID): Promise<void> {
    await serializeConversationConnectionRoomDeletion(runtime, roomId, async () => {
...
function captureConversationConnection(state: ConversationRouteState, runtime: AgentRuntime, conv: ConversationMeta, caller: ConversationCaller, requestFence?: () => void): ConversationConnectionDescriptor {
...
async function establishConversationConnection(descriptor: ConversationConnectionDescriptor): Promise<void> {
    ...
    await ensureWorldOwnershipAndRoles(descriptor.runtime, descriptor.worldId, descriptor.ownerId, descriptor.callerEntityId, descriptor.callerRole, descriptor.callerGrantSource, descriptor.requestFence);
    descriptor.requestFence?.();
}
/**
 * Exported for the machine-session conversation-attribution regression suite;
 * runtime callers are the conversation route handlers in this module.
 */
export async function ensureConversationRoom(state: ConversationRouteState, runtime: AgentRuntime, conv: ConversationMeta, caller: ConversationCaller): Promise<ConversationConnectionDescriptor> {
    const descriptor = captureConversationConnection(state, runtime, conv, caller);
    await scheduleConversationConnectionEnsure(descriptor, () => establishConversationConnection(descriptor));
    assertConversationConnectionRuntime(state.runtime, descriptor);
    return descriptor;
}
async function syncConversationRoomState(state: ConversationRouteState, conv: ConversationMeta): Promise<void> {
    if (!state.runtime)
        return;
    const runtime = state.runtime;
    ...
    const nextMetadata = buildConversationRoomMetadata(conv, ownerId, room.metadata);
    const nextName = conv.title;
    const metadataChanged = JSON.stringify(room.metadata ?? null) !== JSON.stringify(nextMetadata);
...
async function waitForConversationRestore(state: ConversationRouteState): Promise<void> {
...
function mergeActionCallbackHistory(existing: readonly string[], incoming: readonly string[]): string[] {
    return normalizeActionCallbackHistory([...existing, ...incoming]);
}
export function formatConversationMessageText(text: string, actionCallbackHistory: readonly string[] = []): string {
...
export function buildPersistedAssistantContent(text: string, result: {
...
} | null | undefined, userMessageId?: UUID): Content {
    ...
    const responseMessageContent = Array.isArray(result?.responseMessages)
        ? (result.responseMessages
            .map((entry) => entry.content && typeof entry.content === "object"
            ? entry.content
            : null)
            .filter((content): content is Content => content !== null)
            .at(-1) ?? null)
        : null;
    const actionCallbackHistory = normalizeActionCallbackHistory(result?.actionCallbackHistory);
    const transcriptVisibility = result?.transcriptVisibility === "internal"
        ? ("internal" as const)
        : undefined;
...
type DurableConversationReplyRecovery = NonNullable<ChatGenerationResult["replyRecovery"]> & {
...
function clearRecoveredReplyFailureMarkers<T extends object>(record: T): T {
    ...
    for (const key of ["elizaSyntheticFailure", "syntheticChatFailure"] as const)
        if (recovered[key] === true)
            delete recovered[key];
    for (const key of ["failureKind", "chatFailureKind"] as const)
        if (isChatFailureKind(recovered[key]) ||
            recovered[key] === "no_response" ||
            recovered[key] === "transient_failure")
            delete recovered[key];
    return recovered;
}
function parseDurableConversationReplyRecovery(serialized: string): DurableConversationReplyRecovery | null {
    try {
        ...
        for (const result of value.actionResults) {
            ...
            if (result.failureProvenance !== undefined)
                result.failureProvenance = normalizeActionFailureProvenance(result.failureProvenance);
            if (result.success && result.failureProvenance !== undefined)
                return null;
        }
        if (value.reply !== undefined &&
            (!isRecord(value.reply) ||
                ...
                !value.reply.effectReceiptIds.every((id) => typeof id === "string" && id.length > 0)))
            return null;
        const historySelection = parseReplyRecoveryHistorySelection(value.historySelection, value.context);
        return {
            ...
            ...(value.reply !== undefined
                ? {
                    reply: value.reply as NonNullable<DurableConversationReplyRecovery["reply"]>,
                }
                : {}),
        };
    }
...
function conversationReplyRecoveryIsEligible(recovery: DurableConversationReplyRecovery): boolean {
    const receipts = mergeEffectReceipts(...recovery.actionResults.map((result) => result.effectReceipts));
    const unknownCommit = recovery.actionResults.some((result) => result.data?.reconciliationRequired === true ||
    ...
    return (!unknownCommit &&
        receipts.some((receipt) => resolveAppliedUserFacingEffectReceipts({
            verifiedUserFacing: true,
            userFacingText: "Receipt eligibility",
            userFacingEffectReceiptIds: [receipt.receiptId],
        }, receipts) !== null));
}
async function persistConversationReplyRecovery(runtime: AgentRuntime, roomId: UUID, userMessageId: UUID | undefined, assistantMessageId: UUID | undefined, result: ChatGenerationResult, lease: RoomHandlerLease, assertCurrent?: () => void): Promise<boolean> {
    if (!result.replyRecovery || !userMessageId || !assistantMessageId)
        return false;
    try {
        const [user] = await runtime.getMemoriesByIds([userMessageId], "messages");
        assertCurrent?.();
        const marker = readDurableConversationChatMarker(user?.content.chatIdempotency);
        if (!user ||
            user.roomId !== roomId ||
            !marker ||
            conversationClientUserMemoryId(marker.scope, marker.clientMessageId) !==
                userMessageId)
            return false;
        const [assistant] = await runtime.getMemoriesByIds([assistantMessageId], "messages");
        if (!assistant ||
            ...
            throw new TypeError("Reply recovery assistant does not match the original turn");
        ...
        const serialized = JSON.stringify(projected, (_key, value) => {
            if (typeof value === "number" && !Number.isFinite(value))
                throw new TypeError("Reply recovery evidence contains a non-finite number");
        ...
        if (!recovery)
            throw new TypeError("Reply recovery evidence is invalid");
        ...
        assertCurrent?.();
        if (!conversationReplyRecoveryIsEligible(recovery))
            return false;
        await runtime.roomHandlerQueue.runInLease(roomId, lease, () => {
            assertCurrent?.();
            return runtime.updateMemory({
...
const INCOMPLETE_CHAT_RECOVERY_TEXT = "The previous attempt ended before its final response was saved. It was not run again; send a new message if you want to retry.";
...
function isDurableChatActionResult(value: unknown): boolean {
    if (!isRecord(value) || typeof value.success !== "boolean")
        return false;
    return ((value.actionName === undefined || typeof value.actionName === "string") &&
        (value.text === undefined || typeof value.text === "string") &&
        (value.error === undefined || typeof value.error === "string") &&
        (value.values === undefined || isRecord(value.values)));
}
function parseDurableConversationChatOutcome(serialized: string): ChatMessageIdOutcome | null {
    ...
    if ((outcome.messageId !== undefined && !validateUuid(outcome.messageId)) ||
    ...
    const accountConnect = outcome.accountConnect === undefined
        ? undefined
    ...
    return {
        ...
        ...(outcome.actionResults !== undefined
            ? {
                actionResults: outcome.actionResults as NonNullable<ChatMessageIdOutcome["actionResults"]>,
            }
            : {}),
        ...
        ...(outcome.localInference !== undefined
            ? {
                localInference: outcome.localInference as NonNullable<ChatMessageIdOutcome["localInference"]>,
            }
            : {}),
        ...(outcome.noResponseReason === "ignored"
...
function buildRecoveredConversationChatOutcome(memory: Memory & {
    id: UUID;
}, userMessageId: UUID, agentName: string): ChatMessageIdOutcome {
    ...
    const localInference = content.localInference && typeof content.localInference === "object"
...
async function persistDurableConversationChatOutcome(runtime: AgentRuntime, roomId: UUID, scope: string, clientMessageId: string | null | undefined, fingerprint: string, outcome: ChatMessageIdOutcome, roomHandlerLease: RoomHandlerLease, assertCurrent?: () => void): Promise<void> {
    if (!clientMessageId)
        return;
    const userMessageId = conversationClientUserMemoryId(scope, clientMessageId);
    const [userMemory] = await runtime.getMemoriesByIds([userMessageId], "messages");
    assertCurrent?.();
    ...
    const existingMarker = readDurableConversationChatMarker(userMemory.content.chatIdempotency);
    if (!existingMarker ||
        existingMarker.scope !== scope ||
        existingMarker.clientMessageId !== clientMessageId ||
        existingMarker.fingerprint !== fingerprint) {
        throw new ElizaError("Durable chat outcome does not match its original turn", {
...
async function recoverDurableConversationChatOutcome(runtime: AgentRuntime, roomId: UUID, scope: string, clientMessageId: string | null | undefined, fingerprint: string, agentName: string, roomHandlerLease: RoomHandlerLease, assertCurrent?: () => void): Promise<DurableConversationChatRecovery> {
    if (!clientMessageId)
        return { kind: "none" };
    const userMessageId = conversationClientUserMemoryId(scope, clientMessageId);
    const [userMemory] = await runtime.getMemoriesByIds([userMessageId], "messages");
    ...
    const marker = readDurableConversationChatMarker(userMemory.content.chatIdempotency);
    if (!marker ||
        marker.scope !== scope ||
        ...
        return {
            kind: "conflict",
            error: new ElizaError("Idempotency key was reused for a different durable chat request", {
                code: "CHAT_IDEMPOTENCY_CONFLICT",
                context: { roomId, userMessageId, clientMessageId },
            }),
        };
    ...
    if (!assistant) {
        ...
        if (!isChannelType(channelType)) {
            return {
                kind: "conflict",
                error: new ElizaError("Incomplete durable chat request has no valid channel type", {
                    code: "CHAT_IDEMPOTENCY_INCOMPLETE_INVALID",
                    context: { roomId, userMessageId, clientMessageId, channelType },
                }),
            };
        }
        const recoveryMessageId = stringToUuid(`conversation-incomplete-recovery:${userMessageId}`) as UUID;
        assertCurrent?.();
        const persisted = await persistAssistantConversationMemory(runtime, roomId, {
            text: INCOMPLETE_CHAT_RECOVERY_TEXT,
            inReplyTo: userMessageId,
            chatIdempotencyRecovery: "incomplete",
        }, channelType, undefined, recoveryMessageId, roomHandlerLease, assertCurrent);
        assertCurrent?.();
        if (!persisted?.id) {
            throw new ElizaError("Failed to persist the incomplete chat recovery terminal", {
        ...
        await persistDurableConversationChatOutcome(runtime, roomId, scope, clientMessageId, fingerprint, outcome, roomHandlerLease, assertCurrent);
        return { kind: "settled", outcome };
    }
    ...
    const outcome = buildRecoveredConversationChatOutcome(assistant, userMessageId, agentName);
    await persistDurableConversationChatOutcome(runtime, roomId, scope, clientMessageId, fingerprint, outcome, roomHandlerLease, assertCurrent);
    return { kind: "settled", outcome };
}
function bindClientUserMemoryId(clientMessageId: string | null | undefined, scope: string, fingerprint: string, messages: Awaited<ReturnType<typeof buildUserMessages>>): void {
    if (!clientMessageId)
        return;
...
async function persistClientUserMemory(runtime: AgentRuntime, memory: ReturnType<typeof createMessageMemory>, clientMessageId: string | null | undefined, roomHandlerLease: RoomHandlerLease, assertCurrent?: () => void): Promise<void> {
    if (!clientMessageId) {
        await persistConversationMemory(runtime, memory, roomHandlerLease, assertCurrent);
        return;
    }
    await persistExactConversationMemory(runtime, memory, roomHandlerLease, assertCurrent);
}
function writeConversationDoneSse(res: http.ServerResponse, outcome: ChatMessageIdOutcome): void {
...
function buildGenerationMessageIdOutcome(result: ChatGenerationResult, text: string, messageId?: UUID, terminal?: Pick<ChatMessageIdOutcome, "userMessageId" | "assistantEphemeral" | "historyRefreshRequired">): ChatMessageIdOutcome {
    return {
        text,
...
function buildConversationJsonOutcome(outcome: ChatMessageIdOutcome): ChatMessageIdOutcome {
    return {
        text: outcome.text,
        agentName: outcome.agentName,
...
function isCallbackHistoryPersistenceError(error: unknown): error is ElizaError {
    return (error instanceof ElizaError &&
        error.code === "CONVERSATION_CALLBACK_HISTORY_WRITE_FAILED");
}
export async function persistRecentAssistantActionCallbackHistory(runtime: AgentRuntime, roomId: UUID, actionCallbackHistory: readonly string[], sinceMs: number, targetMemoryId?: UUID, roomHandlerLease?: RoomHandlerLease, assertCurrent?: () => void): Promise<boolean> {
    const normalizedHistory = normalizeActionCallbackHistory(actionCallbackHistory);
    ...
    const persist = async (): Promise<boolean> => {
        ...
        if (!target || typeof target.id !== "string") {
            if (targetMemoryId) {
                throw new ElizaError("Exact assistant memory for callback history was not found", {
        ...
        const content = target.content && typeof target.content === "object"
            ? (target.content as Content)
            : ({ text: "" } satisfies Content);
        const existingHistory = normalizeActionCallbackHistory((content as Record<string, unknown>).actionCallbackHistory);
        const mergedHistory = mergeActionCallbackHistory(existingHistory, normalizedHistory);
    ...
    try {
        if (runtime.roomHandlerQueue.ownsLease(roomId, roomHandlerLease)) {
            return await runtime.roomHandlerQueue.runInLease(roomId, roomHandlerLease, persist);
        }
        return await runtime.roomHandlerQueue.withLease(roomId, async (lease) => runtime.roomHandlerQueue.runInLease(roomId, lease, persist));
    }
    catch (cause) {
...
async function getConversationWithRestore(state: ConversationRouteState, convId: string): Promise<ConversationMeta | undefined> {
 ...
 */
async function loadConversationMessagesAround(runtime: AgentRuntime, roomId: UUID, aroundMessageId: UUID): Promise<Memory[]> {
    const [pivot] = await runtime.getMemoriesByIds([aroundMessageId], "messages");
    if (!pivot || pivot.roomId !== roomId) {
        logger.warn(`[conversations] around=${aroundMessageId} is not in room ${roomId}; serving the recent window instead`);
 ...
 * trims it.
 */
async function loadConversationMessagesBefore(runtime: AgentRuntime, roomId: UUID, before: number, limit: number): Promise<{
    memories: Memory[];
    hasMore: boolean;
}> {
...
function extractConversationMetaString(memory: {
    metadata?: unknown;
...
async function ensureConversationGreetingStored(state: ConversationRouteState, conv: ConversationMeta, lang: string, roomHandlerLease?: RoomHandlerLease): Promise<{
    text: string;
    agentName: string;
    generated: boolean;
    persisted: boolean;
    ...
    if (roomHandlerLease) {
        return runtime.roomHandlerQueue.runInLease(conv.roomId, roomHandlerLease, () => ensureConversationGreetingStoredUnlocked(state, conv, lang, roomHandlerLease));
    }
    return runtime.roomHandlerQueue.withLease(conv.roomId, (lease) => ensureConversationGreetingStoredUnlocked(state, conv, lang, lease));
}
async function ensureConversationGreetingStoredUnlocked(state: ConversationRouteState, conv: ConversationMeta, _lang: string, _roomHandlerLease?: RoomHandlerLease): Promise<{
    text: string;
    agentName: string;
    generated: boolean;
    persisted: boolean;
    ...
    if (existingGreeting) {
        return {
            text: String((existingGreeting.content as Record<string, unknown> | undefined)
                ?.text ?? ""),
            agentName,
            generated: true,
            persisted: false,
...
function isLegacyViewsInventoryContent(content: Record<string, unknown>): boolean {
    const text = typeof content.text === "string" ? content.text.trim() : "";
    if (!/^available_views:\s*(?:\n|$)/.test(text))
        return false;
    const callbackHistory = normalizeActionCallbackHistory(content.actionCallbackHistory);
...
function buildMessageSearchSnippet(text: string, query: string): string {
    ...
    const end = Math.min(normalizedText.length, index + query.length + MESSAGE_SEARCH_SNIPPET_RADIUS);
...
export async function handleConversationRoutes(ctx: ConversationRouteContext): Promise<boolean> {
    const requestStartedAt = Date.now();
    const endpoint = conversationEndpoints.find(({ method, path }) => method === ctx.method && path.test(ctx.pathname));
    if (!endpoint)
        return false;
    return endpoint.handle({
        ...ctx,
        requestStartedAt,
        trustedApiPrincipal: resolveTrustedApiPrincipal(ctx.req, ctx.callerAuthorization),
        requestUrl: new URL(ctx.req.url ?? "", `http://${ctx.req.headers.host ?? "localhost"}`),
    });
}
async function listConversations(ctx: ConversationHandlerContext): Promise<boolean> {
...
async function searchConversationMessages(ctx: ConversationHandlerContext): Promise<boolean> {
    ...
    if (since === "invalid" || until === "invalid") {
        error(res, "since/until must be an epoch-ms timestamp or an ISO 8601 date", 400);
        return true;
    }
    ...
    for (const conv of state.conversations.values()) {
        if (state.deletedConversationIds.has(conv.id))
            continue;
    ...
    try {
            ...
            return [
                {
                    ...
                    roomId,
                    role: (memory.entityId === runtime.agentId ? "assistant" : "user") as "assistant" | "user",
                    text: rawText,
        ...
        logger.error({ error: getErrorMessage(err) }, "[ConversationSearch] keyword message search failed");
        error(res, "Failed to search conversation messages", 500);
        return true;
    }
}
async function seedConversationMessages(ctx: ConversationHandlerContext): Promise<boolean> {
    ...
    if (!parsedSeed.success) {
        error(res, parsedSeed.error.issues[0]?.message ?? "Invalid request body", 400);
        return true;
    }
    await waitForConversationRestore(state);
...
async function createConversation(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, readJsonBody, json, error, state, trustedApiPrincipal } = ctx;
    ...
    if (!parsedConv.success) {
        error(res, parsedConv.error.issues[0]?.message ?? "Invalid request body", 400);
        return true;
    }
    ...
    const metadata = addWaifuConversationOwnerMetadata(req, sanitizeConversationMetadata(body.metadata));
    ...
    if (runtime) {
        try {
            ...
            await ensureConversationRoom(state, runtime, conv, resolveConversationCaller(req, state, trustedApiPrincipal, runtime));
            await syncConversationRoomState(state, conv);
            if (body.includeGreeting === true) {
                const storedGreeting = await ensureConversationGreetingStored(state, conv, typeof body.lang === "string" ? body.lang : "en");
            ...
            error(res, `Failed to initialize conversation: ${getErrorMessage(err)}`, 500);
            return true;
        }
    }
...
async function listConversationMessages(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, pathname, json, error, state, requestUrl } = ctx;
    const convId = decodePathComponent(pathname.split("/")[3], res, "conversation id");
    if (convId === null)
        return true;
    if (!state.runtime) {
        error(res, "Agent runtime not available", 503);
    ...
    try {
        ...
        const beforeParam = parseBeforeCursor(requestUrl.searchParams.get("before"));
        const olderLimit = clampOlderPageLimit(requestUrl.searchParams.get("limit"));
        let hasMore = false;
        let memories: Memory[];
        if (!aroundParam && beforeParam !== null) {
            const page = await loadConversationMessagesBefore(runtime, conv.roomId, beforeParam, olderLimit);
            memories = page.memories;
            hasMore = page.hasMore;
        }
        else {
            memories = aroundParam
                ? await loadConversationMessagesAround(runtime, conv.roomId, aroundParam)
                : await runtime.getMemories({
                    roomId: conv.roomId,
                    tableName: "messages",
                    limit: CONVERSATION_MESSAGE_WINDOW,
        ...
        const messages = memories
            ...
            const replyToAuthor = meta?.replyToAuthor && typeof meta.replyToAuthor === "object"
                ? (meta.replyToAuthor as Record<string, unknown>)
                : null;
            const normalizedSource = typeof contentSource === "string" &&
                contentSource.length > 0 &&
                contentSource !== MESSAGE_SOURCE_CLIENT_CHAT
                ? contentSource
                : undefined;
            const actionName = typeof content.action === "string" && content.action.length > 0
                ? content.action
                : undefined;
            const actionCallbackHistory = normalizeActionCallbackHistory(content.actionCallbackHistory);
            const transcriptVisibility = content.transcriptVisibility === "internal"
                ? ("internal" as const)
                : undefined;
            ...
            const rawFailureKind = typeof content.failureKind === "string"
                ? content.failureKind
            ...
            const rawText = formatConversationMessageText((m.content as {
                text?: string;
            })?.text ?? "", actionCallbackHistory);
            ...
            const text = transcriptVisibility === "internal" ||
                (role === "assistant" &&
                    isIntentionalNoResponseResult({ responseContent: m.content }, rawText))
                ? ""
                : role === "assistant" && !interrupted
                    ? normalizeChatResponseText(rawText, state.logBuffer, runtime)
                    : rawText;
            const attachments = selectAttachmentsForViewer(m, viewerAccessContext, agentId);
            const topics = Array.isArray(meta?.topics) && meta.topics.length > 0
                ? (meta.topics as unknown[]).filter((topic): topic is string => typeof topic === "string")
                : undefined;
            return {
                id: m.id ?? "",
                role,
                ...
                actionCallbackHistory: actionCallbackHistory.length > 0
                    ...
                    : undefined,
                from: typeof entityName === "string" && entityName.length > 0
                    ? entityName
                    : undefined,
                fromUserName: typeof meta?.entityUserName === "string" &&
                    meta.entityUserName.length > 0
                    ? meta.entityUserName
                    : undefined,
                avatarUrl: typeof meta?.entityAvatarUrl === "string" &&
                    meta.entityAvatarUrl.length > 0
                    ? meta.entityAvatarUrl
                    : undefined,
                replyToMessageId: typeof content.inReplyTo === "string" &&
                    ...
                    : typeof meta?.replyToMessageId === "string" &&
                        ...
                        : undefined,
                replyToSenderName: typeof meta?.replyToSenderName === "string" &&
                    ...
                    : typeof replyToAuthor?.displayName === "string" &&
                        ...
                        : typeof replyToAuthor?.username === "string" &&
                            ...
                            : undefined,
                replyToSenderUserName: typeof meta?.replyToSenderUserName === "string" &&
                    ...
                    : typeof replyToAuthor?.username === "string" &&
                        ...
                        ? replyToAuthor.username
                        : undefined,
                rawDiscordChannelId: extractConversationMetaString(m, "discordChannelId"),
                rawDiscordMessageId: extractConversationMetaString(m, "discordMessageId"),
                rawSenderId: extractConversationMetaString(m, "fromId"),
                senderEntityId: typeof m.entityId === "string" ? m.entityId : undefined,
                ...(failureKind ? { failureKind } : {}),
                ...(terminalFailure ? { terminalFailure } : {}),
                ...(accountConnect ? { accountConnect } : {}),
            ...
            m.interrupted === true);
        const discordMessages = messages.filter((message) => mayNeedDiscordMessageEnrichment(message.source));
        const discord = discordMessages.length > 0
            ? await getDiscordConversationApi().catch((err) => {
                logger.debug(`[conversations] Discord metadata enrichment unavailable: ${getErrorMessage(err)}`);
                return null;
            })
            : null;
        await Promise.all(discordMessages.map(async (message) => {
            if (!discord) {
                return;
            }
            ...
            try {
                const storedSenderProfile = await discord.resolveStoredDiscordEntityProfile(runtime, message.senderEntityId);
                ...
                const messageAuthorProfile = message.rawDiscordChannelId && message.rawDiscordMessageId
                    ? await discord.resolveDiscordMessageAuthorProfile(runtime, message.rawDiscordChannelId, message.rawDiscordMessageId)
                    : null;
                ...
                if (rawSenderId) {
                    const profile = await discord.resolveDiscordUserProfile(runtime, rawSenderId);
                    if (profile) {
                ...
                message.avatarUrl = await discord.cacheDiscordAvatarForRuntime(runtime, message.avatarUrl, rawSenderId);
            }
            catch (err) {
                logger.debug(`[conversations] Failed to enrich Discord message metadata: ${getErrorMessage(err)}`);
            }
        }));
        json(res, {
            messages: messages.map(({ rawDiscordChannelId: _rawDiscordChannelId, rawDiscordMessageId: _rawDiscordMessageId, rawSenderId: _rawSenderId, senderEntityId: _senderEntityId, ...message }) => message),
        ...
        logger.warn(`[conversations] Failed to fetch messages: ${err instanceof Error ? err.message : String(err)}`);
        json(res, { messages: [], error: "Failed to fetch messages" }, 500);
    }
    return true;
}
async function importConversation(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, pathname, readJsonBody, json, error, state, trustedApiPrincipal, } = ctx;
    const convId = decodePathComponent(pathname.split("/")[3], res, "conversation id");
    if (convId === null)
        return true;
    ...
    const importMessages = rawMessages
        ...
        const role = rec.role === "assistant"
            ...
            : rec.role === "user"
                ? "user"
                : null;
        const rawText = typeof rec.text === "string"
            ? rec.text
        ...
        if (!role || !rawText.trim())
            return null;
        const text = rawText;
        const timestamp = typeof rec.timestamp === "number" && Number.isFinite(rec.timestamp)
            ? rec.timestamp
            : undefined;
        const sourceId = typeof rec.sourceId === "string" &&
            rec.sourceId.trim() &&
    ...
    const cutoverToken = typeof rawImport.cutoverToken === "string" &&
    ...
    if (rawImport.cutoverToken !== undefined && !cutoverToken) {
        error(res, "A cutoverToken must be a non-empty string of at most 512 characters", 400);
        return true;
    }
    ...
    if (activateScheduledTasks !== undefined &&
    ...
    let todoSnapshot: Awaited<ReturnType<typeof parseSharedTodoCutoverSnapshot>> | null = null;
    if (rawTodoSnapshot !== undefined) {
        try {
            ...
            error(res, err instanceof TodoCutoverContractError
                ? err.message
                : `Todo snapshot validation failed: ${getErrorMessage(err)}`, 400);
            return true;
        }
    ...
    if (!conv) {
        ...
        conv = {
            id: convId,
            title: typeof rawImport.title === "string" && rawImport.title.trim()
    ...
    const caller = resolveConversationCaller(req, state, trustedApiPrincipal, runtime);
    ...
    try {
        historyLease = await runtime.roomHandlerQueue.acquire(conv.roomId, importAbortTracker.signal);
    }
    catch (err) {
        ...
        if (importAbortTracker.isAborted())
            return true;
        error(res, isRoomQueueBackpressureError(err)
            ? "Conversation is busy; retry import after pending turns finish"
            : `Failed to serialize conversation import: ${getErrorMessage(err)}`, roomQueueAdmissionStatus(err));
        return true;
    }
    importAbortTracker.markCompleted();
    importAbortTracker.dispose();
    try {
        if (state.conversations.get(conv.id) !== conv ||
        ...
        try {
            ...
            error(res, `Failed to initialize conversation room: ${getErrorMessage(err)}`, 500);
            return true;
        }
        if (createdConversation) {
            evictOldestConversation(state.conversations, 500);
        ...
        if (todoSnapshot && cutoverToken) {
            try {
                todoReceipt = await (ctx.todoCutoverImporter ?? importSharedTodoCutover)({
                    runtime,
        ...
        for (let i = 0; i < importMessages.length; i += 1) {
            ...
            const entityId = m.role === "assistant" ? runtime.agentId : caller.entityId;
            const createdAt = m.timestamp ?? anchor + i;
            try {
                const memory = createMessageMemory({
                    id: m.sourceId
                        ? createUniqueUuid(runtime, `handoff-import:${convId}:${m.sourceId}`)
                ...
                if (m.sourceId) {
                    const result = await persistExactConversationMemoryResult(runtime, memory, historyLease);
                    ...
                    if (!result.created &&
                        ...
                        const updated = await runtime.roomHandlerQueue.runInLease(conv.roomId, historyLease, () => runtime.updateMemory({ id: result.memory.id!, metadata }));
                        if (!updated) {
                    ...
                    const persisted = result.created
                        ? (await runtime.getMemoriesByIds([result.memory.id!], "messages"))[0]
                        : result.memory;
                    if (!persisted)
                    ...
                    const result = await persistConversationMemory(runtime, memory, historyLease);
                    const [persisted] = await runtime.getMemoriesByIds([result.id!], "messages");
                    if (!persisted)
                        throw new Error("Imported message was not persisted");
                    await runtime.queueEmbeddingGeneration(persisted, "low");
                    inserted += 1;
                ...
                error(res, `Conversation import failed at message ${i}: ${getErrorMessage(err)}`, 500);
                return true;
            }
        }
        ...
        if (importTasks.length > 0 && cutoverToken) {
            ...
            for (let i = 0; i < importTasks.length; i += 1) {
                try {
                    ...
                    error(res, `Scheduled task import failed at task ${i}: ${getErrorMessage(err)}`, 500);
                    return true;
                }
            }
            if (activateScheduledTasks === true) {
                for (let i = 0; i < importTasks.length; i += 1) {
                    try {
                        const result = await runner.activateImportedTask(importTasks[i].taskId, {
                            sourceAgentId: convId,
                            cutoverToken,
                        });
                        if (result.activated)
                            ...
                            skippedActivatedScheduledTasks += 1;
                    }
                    catch (err) {
                        // error-policy:J1 the conversation import boundary reports the exact failing task.
                        error(res, `Scheduled task activation failed at task ${i}: ${getErrorMessage(err)}`, 500);
                        return true;
                    }
                }
            }
...
async function truncateConversationMessagesRoute(ctx: ConversationHandlerContext): Promise<boolean> {
    ...
    if (rejectWaifuNonAdminMutationIfNeeded(req, error, res))
        return true;
    ...
    if (!parsedTrunc.success) {
        error(res, parsedTrunc.error.issues[0]?.message ?? "Invalid request body", 400);
        return true;
    }
    ...
    let historyLease: RoomHandlerLease;
    try {
        historyLease = await runtime.roomHandlerQueue.acquire(conv.roomId, truncateAbortTracker.signal);
    }
    catch (err) {
        truncateAbortTracker.dispose();
        if (truncateAbortTracker.isAborted())
            return true;
        error(res, isRoomQueueBackpressureError(err)
            ? "Conversation is busy; retry after the pending turns finish"
            : `Failed to serialize conversation history: ${getErrorMessage(err)}`, roomQueueAdmissionStatus(err));
        return true;
    }
    ...
    try {
        ...
        const result = await runtime.roomHandlerQueue.runInLease(conv.roomId, historyLease, () => truncateConversationMessages(runtime, conv, messageId, {
...
async function deleteConversationMessageRoute(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, pathname, json, error, state } = ctx;
    const segments = pathname.split("/");
    const convId = decodePathComponent(segments[3], res, "conversation id");
    if (convId === null)
        return true;
    const messageId = decodePathComponent(segments[5], res, "conversation message id");
    if (messageId === null)
        return true;
    const conv = await getConversationWithRestore(state, convId);
    if (!conv) {
    ...
    let historyLease: RoomHandlerLease;
    try {
        historyLease = await runtime.roomHandlerQueue.acquire(conv.roomId, deleteMessageAbortTracker.signal);
    }
    catch (err) {
        deleteMessageAbortTracker.dispose();
        if (deleteMessageAbortTracker.isAborted())
            return true;
        error(res, isRoomQueueBackpressureError(err)
            ? "Conversation is busy; retry after the pending turns finish"
            : `Failed to serialize conversation history: ${getErrorMessage(err)}`, roomQueueAdmissionStatus(err));
    ...
    try {
        ...
        const result = await runtime.roomHandlerQueue.runInLease(conv.roomId, historyLease, () => deleteConversationMessage(runtime, conv, messageId));
        conv.updatedAt = new Date().toISOString();
        state.broadcastWs?.({
            type: "conversation-updated",
            conversation: conv,
...
async function retryConversationReply(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, pathname, readJsonBody, json, error, state, trustedApiPrincipal, } = ctx;
    const segments = pathname.split("/");
    const convId = decodePathComponent(segments[3], res, "conversation id");
    const rawMessageId = decodePathComponent(segments[5], res, "conversation message id");
    if (convId === null || rawMessageId === null)
        return true;
    ...
    if (!isRecord(body) || Object.keys(body).length > 0) {
        error(res, "Reply recovery does not accept replacement text or evidence", 400);
        return true;
    }
    const conv = await getConversationWithRestore(state, convId);
    if (!conv) {
    ...
    const assertCurrent = () => {
        if (state.runtime !== runtime ||
            ...
            state.deletedConversationIds.has(conv.id))
            throw new ElizaError("Conversation runtime changed during reply recovery", { code: "CHAT_REPLY_RECOVERY_CONFLICT" });
    };
    try {
        lease = await runtime.roomHandlerQueue.acquire(conv.roomId, disconnect.signal);
        assertCurrent();
        endActive = beginActiveChatTurn(state);
        const [assistant] = await runtime.getMemoriesByIds([assistantId], "messages");
        ...
        const originalAssistantHash = conversationReplyContentHash(assistant.content);
        const [storedUser] = await runtime.getMemoriesByIds([userId], "messages");
        const scope = buildConversationChatIdempotencyScope(runtime, conv.roomId, caller.entityId);
        const marker = readDurableConversationChatMarker(storedUser?.content.chatIdempotency);
        if (!storedUser ||
            ...
            throw new ElizaError("Reply recovery belongs to another authenticated turn", { code: "CHAT_REPLY_RECOVERY_IDENTITY" });
        const recovery = marker.replyRecoveryJson === undefined
            ? null
        ...
        if (!recovery ||
            ...
            throw new ElizaError("Reply recovery is unavailable or requires outcome reconciliation", { code: "CHAT_REPLY_RECOVERY_UNAVAILABLE" });
        const outcome = marker.outcomeJson === undefined
            ? null
            : parseDurableConversationChatOutcome(marker.outcomeJson);
        if ((marker.outcomeJson !== undefined && !outcome) ||
            (outcome &&
                (outcome.messageId !== assistantId || outcome.userMessageId !== userId)))
            throw new ElizaError("Stored reply outcome does not match its original turn", { code: "CHAT_REPLY_RECOVERY_INVALID" });
        const alreadyRecovered = recovery.reply !== undefined &&
            outcome !== null &&
            outcome.terminalFailure === undefined &&
            outcome.replyRecoveryAvailable !== true;
        if (alreadyRecovered && outcome?.text !== recovery.reply?.text)
            throw new ElizaError("Stored recovered prose does not match its outcome", { code: "CHAT_REPLY_RECOVERY_INVALID" });
        if (alreadyRecovered &&
            originalAssistantHash !== recovery.reply?.contentHash)
            throw new ElizaError("The recovered assistant reply was edited after recovery completed", { code: "CHAT_REPLY_RECOVERY_CONFLICT" });
        if (!alreadyRecovered &&
            originalAssistantHash !== recovery.assistantContentHash &&
            originalAssistantHash !== recovery.reply?.contentHash)
            throw new ElizaError("The failed assistant reply was edited after its outcomes settled", { code: "CHAT_REPLY_RECOVERY_CONFLICT" });
        if (!recovery.reply &&
            (assistant.content.replyRecoveryAvailable !== true ||
                !parseChatTerminalFailure(assistant.content.terminalFailure)))
            throw new ElizaError("The selected assistant turn does not have a missing reply", { code: "CHAT_REPLY_RECOVERY_UNAVAILABLE" });
        ...
        const authorizeRecoveryAudience = async () => {
            await attestAuthenticatedApiDeliveryAudience(runtime, message, trustedApiPrincipal);
            assertCurrent();
            if (recovery.ownerExclusiveDisclosureUsed) {
                const disclosure = await authorizeOwnerExclusiveDisclosure(runtime, message);
                if (!disclosure.allowed)
                    throw new ElizaError("The current conversation cannot receive this private reply", { code: "CHAT_REPLY_RECOVERY_IDENTITY" });
            }
            for (const actionResult of recovery.actionResults) {
                if (!actionResult.data || !("disclosureSubject" in actionResult.data))
                    continue;
                if (actionResult.data.disclosureSubject === null ||
                ...
                const checked = await enforceTrustedDeliveryAudienceAtEgress(runtime, message, {
                    data: {
                        disclosureSubject: actionResult.data.disclosureSubject,
                    } as Content,
                });
                if (isRecord(checked.data) && checked.data.privacyDenied === true)
                    throw new ElizaError("The current audience cannot receive these action results", { code: "CHAT_REPLY_RECOVERY_IDENTITY" });
            }
        };
        ...
        const recoverAttempt = async () => {
            const reply = recovery.reply ??
                (await resolvePlannedReplyEgress({
                    ...
                    beforeContextRestore: async () => {
                        await authorizeRecoveryAudience();
                        const currentSources = await runtime.getMemoriesByIds([userId, assistantId], "messages");
                        assertCurrent();
                        const currentUser = currentSources.find((source) => source.id === userId);
                        const currentAssistant = currentSources.find((source) => source.id === assistantId);
                        if (!currentUser ||
                            !currentAssistant ||
                            ...
                            throw new ElizaError("The original turn changed before reply context restoration", { code: "CHAT_REPLY_RECOVERY_CONFLICT" });
                        }
                    },
                }));
            if (reply.effectReceiptIds.length > 0 &&
                !resolveAppliedUserFacingEffectReceipts({
                    verifiedUserFacing: true,
                    userFacingText: reply.text,
                    userFacingEffectReceiptIds: reply.effectReceiptIds,
                }, mergeEffectReceipts(...recovery.actionResults.map((result) => result.effectReceipts))))
            ...
            if (evaluatePlannedReplyEgress({
                reply: reply.text,
                actionResults: [...recovery.actionResults, replyBinding],
                actions: runtime.actions,
            }).verdict !== "allow")
                throw new ElizaError("The saved reply is not grounded in its original effects", { code: "CHAT_REPLY_RECOVERY_INVALID" });
            await authorizeRecoveryAudience();
            assertCurrent();
            let content: Content = {
                ...assistant.content,
            ...
            if (isRecord(normalizedContent.metadata))
                normalizedContent.metadata = clearRecoveredReplyFailureMarkers(normalizedContent.metadata);
            ...
            const normalizeFailureMetadata = !recovery.reply ||
                recovery.reply.contentHash ===
                    conversationReplyContentHash(normalizedContent);
            if (normalizeFailureMetadata)
                content = normalizedContent;
            if (recovery.reply &&
                recovery.reply.contentHash !== conversationReplyContentHash(content))
                throw new ElizaError("Stored recovered content does not match its prepared revision", { code: "CHAT_REPLY_RECOVERY_INVALID" });
            const checked = await enforceTrustedDeliveryAudienceAtEgress(runtime, message, content);
            if (checked !== content)
                throw new ElizaError("The current audience cannot receive the recovered reply", { code: "CHAT_REPLY_RECOVERY_IDENTITY" });
            assertCurrent();
            ...
            const latestRows = await runtime.getMemoriesByIds([userId, assistantId], "messages");
            const latestUser = latestRows.find((memory) => memory.id === userId);
            const latestAssistant = latestRows.find((memory) => memory.id === assistantId);
            const latestMarker = readDurableConversationChatMarker(latestUser?.content.chatIdempotency);
            if (!latestUser ||
                ...
                conversationReplyContentHash(latestAssistant.content) !==
                    originalAssistantHash)
                throw new ElizaError("The original turn changed while its reply was recovering", { code: "CHAT_REPLY_RECOVERY_CONFLICT" });
            assertCurrent();
            if (!recovery.reply) {
                ...
                await runtime.roomHandlerQueue.runInLease(conv.roomId, recoveryLease, () => {
                    assertCurrent();
                    return runtime.updateMemory({
                        id: userId,
                        content: {
            ...
            await persistDurableConversationChatOutcome(runtime, conv.roomId, scope, marker.clientMessageId, marker.fingerprint, recoveredOutcome, recoveryLease, assertCurrent);
            assertCurrent();
            conv.updatedAt = new Date().toISOString();
            state.broadcastWs?.({
                type: "conversation-updated",
        ...
        const recoveredOutcome = recovery.reply
            ...
            : await withStandaloneTrajectory(runtime, {
                source: MESSAGE_SOURCE_CLIENT_CHAT,
                metadata: {
                    roomId: conv.roomId,
                    entityId: caller.entityId,
        ...
        if (!disconnect.isAborted()) {
            ...
            error(res, getErrorMessage(cause), code === "CHAT_REPLY_RECOVERY_IDENTITY"
                ? 403
...
async function streamConversationMessage(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, pathname, readJsonBody, error, state, requestStartedAt, trustedApiPrincipal, } = ctx;
    ...
    const localVoiceRuntimeFence = fenceResolution.kind === "valid" ? fenceResolution.fence : null;
    const conv = await getConversationWithRestore(state, convId);
    ...
    const { prompt, channelType, images, preferredLanguage, source, metadata: chatMetadata, streamProtocol, clientMessageId, } = chatPayload;
    logger.info({ traceId: trace.traceId, traceSource: trace.source }, "[ConversationStream] accepted validated trace context");
    ...
    const runtime = state.runtime;
    const releaseTurnReservation = () => releaseChatMessageId(chatIdempotencyScope, clientMessageId ?? null, chatReservation);
    try {
        ...
        const inferenceTimer = getInferenceTimer() ??
        ...
        chatIdempotencyScope = buildConversationChatIdempotencyScope(runtime, conv.roomId, caller.entityId);
        ...
        const assertLocalVoiceTurnFence = () => assertLocalVoiceTurnFenceCurrent(state, localVoiceRuntimeFence, conv);
        const settleTurnReservationInMemory = (outcome: ChatMessageIdOutcome): void => {
            setChatMessageIdOutcome(chatIdempotencyScope, clientMessageId ?? null, outcome, chatReservation);
            reservationSettled = true;
        };
        const settleTurnReservation = async (outcome: ChatMessageIdOutcome): Promise<void> => {
            if (clientMessageId) {
                if (!runtimeTurnLease) {
                ...
                await persistDurableConversationChatOutcome(runtime, conv.roomId, chatIdempotencyScope, clientMessageId, chatFingerprint, outcome, runtimeTurnLease, assertLocalVoiceTurnFence);
                assertLocalVoiceTurnFence();
            }
            settleTurnReservationInMemory(outcome);
        };
        const settleDurableAssistantOutcome = async (outcome: ChatMessageIdOutcome): Promise<void> => {
            try {
                ...
                settleTurnReservationInMemory(outcome);
                runtime.reportError("ConversationStream.durableReplySettlement", settlementError, {
                    conversationId: conv.id,
                    roomId: conv.roomId,
                    clientMessageId,
        ...
        const idempotencyAdmission = await awaitConversationChatAdmission(chatIdempotencyScope, clientMessageId ?? null, chatFingerprint, disconnectTracker.signal);
        if (!isLocalVoiceRuntimeFenceCurrent(state, localVoiceRuntimeFence, conv)) {
        ...
        try {
            runtimeTurnLease = await runWithInferenceTiming(inferenceTimer, () => timeInferenceSpan("chat:room-lease-wait", () => runtime.roomHandlerQueue.acquire(conv.roomId, disconnectTracker.signal)));
            if (!isLocalVoiceRuntimeFenceCurrent(state, localVoiceRuntimeFence, conv)) {
                return failStream("Local voice agent runtime changed");
            }
        }
            ...
            return failStream(isRoomQueueBackpressureError(err)
                ? "Conversation is busy; retry after the pending turns finish"
        ...
        try {
            ...
            try {
                durableRecovery = await recoverDurableConversationChatOutcome(runtime, conv.roomId, chatIdempotencyScope, clientMessageId, chatFingerprint, state.agentName, runtimeTurnLease, assertLocalVoiceTurnFence);
            }
            catch (err) {
            ...
            try {
                ...
                const handled = failStream(`Failed to prepare user message: ${getErrorMessage(err)}`);
                return handled;
            }
            bindClientUserMemoryId(clientMessageId ?? null, chatIdempotencyScope, chatFingerprint, userMessages);
            const { userMessage, messageToStore } = userMessages;
            const connectionDescriptor = captureConversationConnection(state, runtime, conv, caller, assertLocalVoiceTurnFence);
            try {
                await scheduleConversationConnectionEnsure(connectionDescriptor, () => establishConversationConnection(connectionDescriptor));
                assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                await attestAuthenticatedApiDeliveryAudience(runtime, userMessage, trustedApiPrincipal);
            }
            catch (err) {
                releaseTurnReservation();
                const handled = failStream(`Failed to initialize conversation room: ${getErrorMessage(err)}`);
                return handled;
            }
            ...
            try {
                assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                await persistClientUserMemory(runtime, messageToStore, clientMessageId ?? null, runtimeTurnLease, assertLocalVoiceTurnFence);
                assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
            }
            catch (err) {
                ...
                const handled = failStream(`${connectionFailed ? "Failed to refresh conversation room" : "Failed to store user message"}: ${getErrorMessage(err)}`);
                return handled;
            }
            ...
            try {
                const assertCurrentGenerationOwner = () => assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                const publishReplyReady = async (result: ChatGenerationResult) => {
                    if (replyReadyPublished ||
                    ...
                    resolvedGenerationText = normalizeChatResponseText(result.text, state.logBuffer, runtime);
                    writeSse(res, {
                        type: "reply_ready",
                        fullText: result.transcriptVisibility === "internal"
                            ? ""
                ...
                const completeGeneration = (result: ChatGenerationResult): Promise<void> => {
                    ...
                    generationCompletion = (async () => {
                        ...
                        if (result.noResponseReason !== "ignored") {
                            const resolvedText = resolvedGenerationText ??
                                normalizeChatResponseText(result.text, state.logBuffer, runtime);
                            const visibleResolvedText = result.transcriptVisibility === "internal" ? "" : resolvedText;
                            if (!disconnectTracker.isAborted() &&
                                !result.terminalFailure &&
                                !streamedText &&
                                resolvedText &&
                            ...
                            const persistedAssistant = await resolvePersistedAssistantTurn(runtime, conv.roomId, turnStartedAt, result, resolvedText, channelType, generationLease, messageToStore.id, assertLocalVoiceTurnFence);
                            assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                            const persistedAssistantId = persistedAssistant.kind === "durable"
                                ? persistedAssistant.id
                                : undefined;
                            if (result.actionCallbackHistory?.length &&
                                persistedAssistantId) {
                                await persistRecentAssistantActionCallbackHistory(runtime, conv.roomId, result.actionCallbackHistory, turnStartedAt, persistedAssistantId, generationLease, assertLocalVoiceTurnFence);
                            }
                            assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                            const replyRecoveryAvailable = await persistConversationReplyRecovery(runtime, conv.roomId, messageToStore.id, persistedAssistantId, result, generationLease, assertLocalVoiceTurnFence);
                            const outcome = buildGenerationMessageIdOutcome(result, visibleResolvedText, persistedAssistantId, {
                ...
                const result = await generateChatResponse(runtime, routedUserMessage, state.agentName, {
                    traceId: trace.traceId,
                    inferenceTimer,
                ...
                if (isConversationConnectionError(terminalError)) {
                    ...
                    if (!getChatMessageIdOutcome(chatIdempotencyScope, clientMessageId ?? null)) {
                        try {
                            assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                            const receiptId = crypto.randomUUID() as UUID;
                            const persisted = await persistInterruptedAssistantReceipt(runtime, conv.roomId, streamedText, channelType, messageToStore.id, receiptId, runtimeTurnLease, assertLocalVoiceTurnFence);
                            conv.updatedAt = new Date().toISOString();
                            ...
                            try {
                                ...
                                runtime.reportError("ConversationStream.interruptedReceiptSettlement", settlementError, {
                                    conversationId: conv.id,
                                    roomId: conv.roomId,
                                    clientMessageId,
                ...
                else if (isCallbackHistoryPersistenceError(terminalError) ||
                    terminalError instanceof AssistantReplyPersistenceError) {
                    releaseTurnReservation();
                    if (!disconnectTracker.isAborted()) {
                        writeSse(res, {
                            type: "error",
                            message: getErrorMessage(terminalError instanceof AssistantReplyPersistenceError
                    ...
                    if (streamedText) {
                        ...
                        try {
                            ...
                            const persisted = await persistAssistantConversationMemory(runtime, conv.roomId, { text: streamedText, inReplyTo: messageToStore.id }, channelType, turnStartedAt, routeOwnedId, runtimeTurnLease, assertLocalVoiceTurnFence);
                            assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                            conv.updatedAt = new Date().toISOString();
                            const outcome: ChatMessageIdOutcome = {
                                text: streamedText,
                        ...
                        logger.warn({
                            err: getErrorMessage(terminalError),
                            stack: terminalError instanceof Error
                                ? terminalError.stack
                                : undefined,
                        ...
                        try {
                            ...
                            const generationResolvedText = generationResult
                                ? normalizeChatResponseText(generationResult.text, state.logBuffer, runtime)
                                : "";
                            const exactPersistedResponse = generationResult &&
                                generationResult.transcriptVisibility !== "internal" &&
                                generationResolvedText
                                ? findPersistedGeneratedAssistantTurn(runtime, conv.roomId, generationResult)
                                : null;
                            assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                            ...
                            if (generationResult &&
                                ...
                                exactPersistedId) {
                                if (exactPersistedResponse.content.text !== generationResolvedText) {
                                    await runtime.roomHandlerQueue.runInLease(conv.roomId, runtimeTurnLease, async () => {
                                        assertLocalVoiceTurnFence();
                                        await runtime.updateMemory({
                                            ...exactPersistedResponse,
                                            content: buildPersistedAssistantContent(generationResolvedText, generationResult, messageToStore.id),
                                        });
                                        assertLocalVoiceTurnFence();
                                    });
                                    assertLocalVoiceTurnFence();
                                ...
                                if (generationResult.actionCallbackHistory?.length) {
                                    await persistRecentAssistantActionCallbackHistory(runtime, conv.roomId, generationResult.actionCallbackHistory, turnStartedAt, exactPersistedId, runtimeTurnLease, assertLocalVoiceTurnFence);
                                }
                                assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                                const outcome = buildGenerationMessageIdOutcome(generationResult, generationResolvedText, exactPersistedId, {
                                    userMessageId: messageToStore.id,
                        ...
                        const providerIssueReply = getChatFailureReply(terminalError, state.logBuffer);
                        const failureKind = classifyChatFailure(terminalError, state.logBuffer);
                        try {
                            assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                            const routeOwnedId = crypto.randomUUID() as UUID;
                            const persisted = await persistAssistantConversationMemory(runtime, conv.roomId, { text: providerIssueReply, inReplyTo: messageToStore.id }, channelType, undefined, routeOwnedId, runtimeTurnLease, assertLocalVoiceTurnFence);
                            assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                            conv.updatedAt = new Date().toISOString();
                            const outcome: ChatMessageIdOutcome = {
        ...
        try {
            ...
            logger.warn(`[conversation-stream] terminal error frame undeliverable: ${getErrorMessage(frameError)}`);
        }
        throw streamError;
    }
    finally {
...
async function sendConversationMessage(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, pathname, readJsonBody, json, error, state, requestStartedAt, trustedApiPrincipal, } = ctx;
    const convId = decodePathComponent(pathname.split("/")[3], res, "conversation id");
    if (convId === null)
        return true;
    const conv = await getConversationWithRestore(state, convId);
    ...
    if (!chatPayload)
        return true;
    const { prompt, channelType, images, preferredLanguage, source, metadata: restMetadata, clientMessageId, } = chatPayload;
    const runtime = state.runtime;
    if (!runtime) {
        error(res, "Agent is not running", 503);
        return true;
    ...
    const caller = resolveConversationCaller(req, state, trustedApiPrincipal, runtime);
    const userId = caller.entityId;
    const chatIdempotencyScope = buildConversationChatIdempotencyScope(runtime, conv.roomId, caller.entityId);
    const chatFingerprint = buildConversationChatFingerprint({
        prompt,
        images,
        source,
    ...
    const idempotencyAdmission = await awaitConversationChatAdmission(chatIdempotencyScope, clientMessageId ?? null, chatFingerprint, admissionDisconnectTracker.signal);
    if (idempotencyAdmission.kind === "aborted") {
    ...
    const chatReservation = idempotencyAdmission.kind === "owner"
        ? idempotencyAdmission.reservation
        : null;
    let reservationSettled = false;
    let runtimeTurnLease: RoomHandlerLease | null = null;
    ...
    const settleTurnReservation = async (outcome: ChatMessageIdOutcome): Promise<void> => {
        if (clientMessageId) {
            ...
            await persistDurableConversationChatOutcome(runtime, conv.roomId, chatIdempotencyScope, clientMessageId, chatFingerprint, outcome, runtimeTurnLease);
        }
        setChatMessageIdOutcome(chatIdempotencyScope, clientMessageId ?? null, outcome, chatReservation);
        reservationSettled = true;
    };
    try {
        try {
            runtimeTurnLease = await runWithInferenceTiming(inferenceTimer, () => timeInferenceSpan("chat:room-lease-wait", () => runtime.roomHandlerQueue.acquire(conv.roomId, admissionDisconnectTracker.signal)));
        }
        catch (err) {
            admissionDisconnectTracker.dispose();
            releaseTurnReservation();
            ...
            error(res, isRoomQueueBackpressureError(err)
                ? "Conversation is busy; retry after the pending turns finish"
                : `Failed to serialize conversation turn: ${getErrorMessage(err)}`, roomQueueAdmissionStatus(err));
            return true;
        }
        admissionDisconnectTracker.markCompleted();
        admissionDisconnectTracker.dispose();
        try {
            ...
            const durableRecovery = await recoverDurableConversationChatOutcome(runtime, conv.roomId, chatIdempotencyScope, clientMessageId, chatFingerprint, state.agentName, runtimeTurnLease);
            if (durableRecovery.kind === "conflict") {
                releaseTurnReservation();
            ...
            try {
                connectionDescriptor = await ensureConversationRoom(state, runtime, conv, caller);
            }
            catch (err) {
                releaseTurnReservation();
                error(res, `Failed to initialize conversation room: ${getErrorMessage(err)}`, 500);
            ...
            try {
                ...
                releaseTurnReservation();
                error(res, `Failed to prepare user message: ${getErrorMessage(err)}`, 500);
                return true;
            }
            bindClientUserMemoryId(clientMessageId ?? null, chatIdempotencyScope, chatFingerprint, userMessages);
            const { userMessage, messageToStore } = userMessages;
            try {
                ...
                releaseTurnReservation();
                error(res, `Failed to attest conversation audience: ${getErrorMessage(err)}`, 500);
                return true;
            }
            const routedUserMessage = withViewInteractionClient(userMessage, req);
            const turnStartedAt = Date.now();
            try {
                assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                await persistClientUserMemory(runtime, messageToStore, clientMessageId ?? null, runtimeTurnLease);
                assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
            }
            catch (err) {
                releaseTurnReservation();
            ...
            try {
                ...
                const completeGeneration = (result: ChatGenerationResult): Promise<void> => {
                    ...
                    generationCompletion = (async () => {
                        ...
                        conv.updatedAt = new Date().toISOString();
                        if (result.noResponseReason !== "ignored") {
                            const resolvedText = normalizeChatResponseText(result.text, state.logBuffer, runtime);
                            const persistedAssistant = await resolvePersistedAssistantTurn(runtime, conv.roomId, turnStartedAt, result, resolvedText, channelType, deliveryLease, messageToStore.id);
                            assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                            const persistedAssistantId = persistedAssistant.kind === "durable"
                                ? persistedAssistant.id
                                : undefined;
                            if (result.actionCallbackHistory?.length &&
                                persistedAssistantId) {
                                await persistRecentAssistantActionCallbackHistory(runtime, conv.roomId, result.actionCallbackHistory, turnStartedAt, persistedAssistantId, deliveryLease);
                            }
                            assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                            const visibleResolvedText = result.transcriptVisibility === "internal" ? "" : resolvedText;
                            const replyRecoveryAvailable = await persistConversationReplyRecovery(runtime, conv.roomId, messageToStore.id, persistedAssistantId, result, deliveryLease);
                            const outcome = buildGenerationMessageIdOutcome(result, visibleResolvedText, persistedAssistantId, {
                                userMessageId: messageToStore.id,
                                ...(persistedAssistant.kind === "ephemeral"
                                    ? { assistantEphemeral: true }
                ...
                if (isCallbackHistoryPersistenceError(err) ||
                    ...
                    releaseTurnReservation();
                    error(res, getErrorMessage(err instanceof AssistantReplyPersistenceError
                        ? (err.cause ?? err)
                        : err), 500);
                    return true;
                }
                logger.warn(`[conversations] POST /messages failed: ${err instanceof Error ? err.message : String(err)}`);
                if (isConversationConnectionError(err)) {
                    releaseTurnReservation();
                    error(res, `Failed to refresh conversation room: ${getErrorMessage(err)}`, 500);
                    return true;
                }
                ...
                try {
                    assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                    const routeOwnedId = crypto.randomUUID() as UUID;
                    const persisted = await persistAssistantConversationMemory(runtime, conv.roomId, { text: providerIssueReply, inReplyTo: messageToStore.id }, channelType, undefined, routeOwnedId, runtimeTurnLease);
                    assertConversationConnectionRuntime(state.runtime, connectionDescriptor);
                    conv.updatedAt = new Date().toISOString();
                    const outcome: ChatMessageIdOutcome = {
                        text: providerIssueReply,
...
async function greetConversation(ctx: ConversationHandlerContext): Promise<boolean> {
    ...
    let historyLease: RoomHandlerLease;
    try {
        historyLease = await runtime.roomHandlerQueue.acquire(conv.roomId, greetingAbortTracker.signal);
    }
    catch (err) {
        greetingAbortTracker.dispose();
        if (greetingAbortTracker.isAborted())
            return true;
        error(res, isRoomQueueBackpressureError(err)
            ? "Conversation is busy; retry after the pending turns finish"
            : `Failed to serialize conversation history: ${getErrorMessage(err)}`, roomQueueAdmissionStatus(err));
    ...
    try {
        ...
        const greeting = await ensureConversationGreetingStored(state, conv, lang, historyLease);
        json(res, {
            text: greeting.text,
...
async function patchConversation(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, pathname, readJsonBody, json, error, state } = ctx;
    const convId = decodePathComponent(pathname.split("/")[3], res, "conversation id");
    if (convId === null)
        return true;
    ...
    if (!parsedPatch.success) {
        error(res, parsedPatch.error.issues[0]?.message ?? "Invalid request body", 400);
        return true;
    }
    ...
    if (body.generate) {
        ...
        const lastUserMemory = memories.find((m) => m.entityId !== state.runtime?.agentId);
        ...
        try {
            newTitle = await generateConversationTitle(state.runtime, prompt, state.agentName, { signal: titleAbortTracker.signal });
        }
        finally {
            titleAbortTracker.markCompleted();
            titleAbortTracker.dispose();
        }
        if (titleAbortTracker.isAborted())
            return true;
...
async function cleanupEmptyConversations(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, readJsonBody, json, error, state } = ctx;
    if (rejectWaifuNonAdminMutationIfNeeded(req, error, res))
        return true;
    ...
    if (rawCleanup === null)
        return true;
    const parsedCleanup = PostConversationCleanupEmptyRequestSchema.safeParse(rawCleanup);
    if (!parsedCleanup.success) {
        error(res, parsedCleanup.error.issues[0]?.message ?? "Invalid request body", 400);
        return true;
    }
    await waitForConversationRestore(state);
    const runtime = state.runtime;
    ...
    for (const conv of Array.from(state.conversations.values())) {
        ...
        let historyLease: RoomHandlerLease;
        try {
            historyLease = await runtime.roomHandlerQueue.acquire(conv.roomId, cleanupAbortTracker.signal);
        }
        catch (err) {
            cleanupAbortTracker.dispose();
            if (cleanupAbortTracker.isAborted())
                return true;
            error(res, isRoomQueueBackpressureError(err)
                ? "Conversation is busy; retry cleanup after pending turns finish"
                : `Failed to serialize conversation cleanup: ${getErrorMessage(err)}`, roomQueueAdmissionStatus(err));
            return true;
        }
        ...
        try {
            ...
            const memoryIds = memories
                ...
                .filter((memoryId): memoryId is UUID => typeof memoryId === "string" && memoryId.trim().length > 0);
            if (memoryIds.length > 0) {
                await runtime.roomHandlerQueue.runInLease(conv.roomId, historyLease, () => deleteConversationMemories(runtime, memoryIds));
            }
...
async function deleteConversation(ctx: ConversationHandlerContext): Promise<boolean> {
    const { req, res, pathname, json, error, state } = ctx;
    if (rejectWaifuNonAdminMutationIfNeeded(req, error, res))
        return true;
    const convId = decodePathComponent(pathname.split("/")[3], res, "conversation id");
    ...
    if (conv?.roomId && runtime) {
        ...
        let historyLease: RoomHandlerLease;
        try {
            historyLease = await runtime.roomHandlerQueue.acquire(conv.roomId, deleteConversationAbortTracker.signal);
        }
        catch (err) {
            deleteConversationAbortTracker.dispose();
            if (deleteConversationAbortTracker.isAborted())
                return true;
            error(res, isRoomQueueBackpressureError(err)
                ? "Conversation is busy; retry deletion after pending turns finish"
                : `Failed to serialize conversation deletion: ${getErrorMessage(err)}`, roomQueueAdmissionStatus(err));
            return true;
        }
        ...
        try {
            ...
            try {
                ...
                error(res, `Failed to delete conversation messages: ${getErrorMessage(err)}`, 500);
                return true;
            }
            try {
                await deleteConversationRoomData(runtime, conv.roomId);
                ...
                if (isConversationConnectionError(err)) {
                    error(res, `Failed to serialize conversation deletion: ${getErrorMessage(err)}`, 503);
                    return true;
                }
                // error-policy:J1 an incomplete room deletion is a route failure, not a
                // successful tombstone-only delete.
                error(res, `Failed to delete conversation room: ${getErrorMessage(err)}`, 500);
                return true;
            }
            state.conversations.delete(convId);
