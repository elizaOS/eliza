 ...
 * so "go home" turns in a dashboard conversation still failed the USER surface
 * minRole:USER gate (live tj-92e47fe5ea3e5c: deterministic USER surface call ->
 * "Action USER surface is not allowed for the current role"). This suite drives the
 * production conversation route + connection seam with a stubbed runtime
 * adapter, then core checkSenderRole and the real USER surface action gate.
 */
...
describe("machine-session attribution on the conversation surface", () => {
  ...
  it("keeps a plain external principal GUEST and USER surface-denied", async () => {
    const harness = createRuntimeHarness();
