    ...
    waitFor(requestId: string, timeoutMs: number): Promise<ViewInteractResult> {
        return new Promise<ViewInteractResult>((resolve, reject) => {
            ...
            if (existing) {
                ...
                existing.reject(new ElizaError(`Pending request "${requestId}" was superseded by a newer waiter`, {
            ...
            const timer = setTimeout(() => {
                if (this.map.get(requestId)?.timer !== timer)
                    return;
                this.map.delete(requestId);
                reject(new ElizaError(`Pending request "${requestId}" timed out after ${timeoutMs}ms`, {
