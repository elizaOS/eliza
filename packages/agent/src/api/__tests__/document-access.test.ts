...
describe("scope helpers", () => {
  ...
  it("defaults missing scope to global", () => {
    ...
    expect(getDocumentVisibilityScope({ scope: "user-private" })).toBe(
      "user-private",
    );
  });
...
describe("matchesDocumentFilter", () => {
  ...
  it("matches by scopedToEntityId, addedBy, roomId", () => {
    expect(matchesDocumentFilter(mem, { scopedToEntityId: "user-1" })).toBe(
      true,
    );
    expect(matchesDocumentFilter(mem, { scopedToEntityId: "other" })).toBe(
      false,
    );
  ...
  it("matches by tags (all required)", () => {
    ...
    expect(
      matchesDocumentFilter(mem, { tags: ["finance", "media-format:pdf"] }),
    ).toBe(true);
    expect(matchesDocumentFilter(mem, { tags: ["finance", "missing"] })).toBe(
      false,
    );
  });
...
describe("canReadDocumentMemory (scope wall)", () => {
  ...
  const userPriv = {
    entityId: "m1",
    metadata: { scope: "user-private", scopedToEntityId: "user-1" },
  };
  ...
  it("user-private matches the scoped entity for USER", () => {
    ...
    expect(
      canReadDocumentMemory(userPriv, { ...USER, entityId: "other" }),
    ).toBe(false);
  });
  ...
  it("OWNER reads user-private via scopedToEntityId filter or own entityId", () => {
    expect(
      canReadDocumentMemory(userPriv, OWNER, { scopedToEntityId: "user-1" }),
    ).toBe(true);
    expect(
      canReadDocumentMemory(
        {
          ...userPriv,
          metadata: { ...userPriv.metadata, scopedToEntityId: "owner-1" },
        },
        OWNER,
      ),
    ).toBe(true);
  ...
  it("user-private with no scoped entity is unreadable", () => {
    expect(
      canReadDocumentMemory({ metadata: { scope: "user-private" } }, USER),
    ).toBe(false);
  });
});
...
describe("canMutateDocumentMemory", () => {
  it("global/owner-private require OWNER/RUNTIME", () => {
    expect(
      canMutateDocumentMemory(
        { metadata: { scope: "global" } } as never,
        OWNER,
      ),
    ).toBe(true);
    expect(
      canMutateDocumentMemory({ metadata: { scope: "global" } } as never, USER),
    ).toBe(false);
    expect(
      canMutateDocumentMemory(
        { metadata: { scope: "owner-private" } } as never,
        RUNTIME,
      ),
    ).toBe(true);
  });
  ...
  it("agent-private requires OWNER/AGENT/RUNTIME", () => {
    expect(
      canMutateDocumentMemory(
        { metadata: { scope: "agent-private" } } as never,
        AGENT,
      ),
    ).toBe(true);
    expect(
  ...
  it("user-private allows scoped user or agent-class actors", () => {
    ...
    expect(canMutateDocumentMemory(mem, { ...USER, entityId: "other" })).toBe(
      false,
    );
  });
});
