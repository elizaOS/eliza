 ...
 */
import { AGENT_SURFACE_CAPABILITY_IDS } from "@elizaos/core/views/view-interact-protocol";
import { ElizaError } from "@elizaos/core";
import { EventType } from "@elizaos/core";
import { STANDARD_CAPABILITIES } from "@elizaos/core/views/view-interact-protocol";
...
import { createShellNavigateViewWsFrame } from "@elizaos/core/events";
import { createViewClientStore } from "../runtime/view-client-context.ts";
import { detectClientPlatform } from "./platform-detect.ts";
import { findHeroOnDisk } from "./views-registry.ts";
import { generateViewHeroSvg } from "./views-registry.ts";
...
import { isDynamicLoadingAllowed } from "./platform-detect.ts";
...
import { setActiveViewElements } from "../runtime/view-action-affinity.ts";
import { type ActiveViewElement } from "../runtime/view-action-affinity.ts";
import { type AgentHttpRequestAuthorization } from "../runtime/host-bridge.ts";
import { type IAgentRuntime } from "@elizaos/core";
import { type RendererViewInteractResult } from "./view-interaction-host.ts";
...
import { type ShellNavigateViewPayload } from "@elizaos/core/events";
import { type ViewClientScope } from "../runtime/view-client-context.ts";
import { type ViewInteractResult } from "./pending-request-map.ts";
import { type ViewRegistryEntry } from "./view-registry-types.ts";
import { type ViewType } from "@elizaos/core";
import { viewInteractionHost } from "./view-interaction-host.ts";
...
function resolveViewTypeQuery(raw: string | null, res: http.ServerResponse, error: ViewsRouteContext["error"]): {
    reject: true;
} | {
    viewType: ViewType | undefined;
} {
...
function resolveViewTypePair(bodyValue: unknown, queryRaw: string | null, res: http.ServerResponse, error: ViewsRouteContext["error"]): {
    reject: true;
} | {
    viewType: ViewType | undefined;
} {
...
function isSurfaceBrokeredCapability(capability: string): boolean {
    return (STANDARD_CAPABILITY_IDS.has(capability) ||
...
function viewManifestAllowsCapability(entry: ViewRegistryEntry, capability: string): boolean {
    if (!isSurfaceBrokeredCapability(capability))
        return true;
    if (isReadOnlyViewCapability(capability))
        return true;
    return entry.surface?.capabilities?.includes("agent-surface") === true;
}
function viewManifestAllowsAgentAuthority(entry: ViewRegistryEntry, capability: string): boolean {
    return (entry.capabilities?.find((declared) => declared.id === capability)
        ?.authority !== "human");
}
function capabilityAuthorityDeniedMessage(viewId: string, capability: string): string {
...
function capabilityDeniedMessage(viewId: string, capability: string): string {
    return (`View "${viewId}" is not granted capability "${capability}" ` +
        "(its surface manifest does not grant `agent-surface`)");
}
export { getViewsBroadcastWs, getViewsBroadcastWsToClientId, setViewsBroadcastWs, } from "./view-interaction-host.ts";
...
export function isViewSwitchFresh(state: CurrentViewState | null, now: number = Date.now()): boolean {
    if (!state?.switchedAt)
        return false;
...
export function getCurrentViewState(runtime: IAgentRuntime, scope: ViewClientScope | undefined = getViewClientScope()): CurrentViewState | null {
    return currentViews.get(runtime, scope);
}
export function clearCurrentViewState(runtime: IAgentRuntime, scope: ViewClientScope | undefined = getViewClientScope()): void {
    currentViews.delete(runtime, scope);
    clearActiveViewContext(runtime, scope);
}
function clientScope(hostKey: object, clientId: string | null): ViewClientScope | undefined {
 ...
 */
export function resolveViewInteractResult(runtime: IAgentRuntime, hostKey: object, clientId: string, result: RendererViewInteractResult): void {
    viewInteractionHost(runtime, hostKey).resolve(clientId, result);
}
export interface ViewsRouteContext extends RouteRequestMeta, Pick<RouteHelpers, "json" | "error"> {
    url: URL;
    hostKey: object;
...
export async function handleViewsRoutes(ctx: ViewsRouteContext): Promise<boolean> {
    ...
    let currentViewState = getCurrentViewState(viewRuntime, clientScope(ctx.hostKey, resolveViewInteractClientId(req, undefined)));
    ...
    if (method === "GET" && pathname === `${PREFIX}/search`) {
        ...
        const parsedSearchViewType = resolveViewTypeQuery(url.searchParams.get("viewType"), res, error);
        if ("reject" in parsedSearchViewType)
            return true;
        if (!query.trim()) {
        ...
        for (const v of allViews) {
            ...
            if (label === q)
                ...
                score = 80;
            else if ((v.tags ?? []).some((t) => t.toLowerCase() === q))
                score = 60;
            else if ((v.description ?? "").toLowerCase().includes(q))
                score = 40;
        ...
        if (ctx.runtime) {
            try {
                const semResults = await getViewSearchIndex(viewRuntime).search(query, topK * 2);
                for (const { viewId, viewType, score } of semResults) {
                    // Cosine similarity in [−1, 1]; normalise to [0, 100].
                    semanticMap.set(`${viewType ?? "gui"}:${viewId}`, ((score + 1) / 2) * 100);
                }
            }
            catch (err) {
                logger.debug({ src: "ViewsRoutes", err }, "[ViewsRoutes] Semantic search unavailable — using keyword only");
            }
        }
    ...
    if (method === "GET" && (pathname === PREFIX || pathname === `${PREFIX}/`)) {
        ...
        const parsedListViewType = resolveViewTypeQuery(url.searchParams.get("viewType"), res, error);
        if ("reject" in parsedListViewType)
            return true;
        ...
        const filtered = dynamicAllowed
            ? allViews
            : allViews.map((view) => view.bundleUrl || view.frameUrl
                ? {
                    ...view,
                    bundleUrl: undefined,
                    frameUrl: undefined,
    ...
    if (method === "POST" && pathname === `${PREFIX}/events/broadcast`) {
        ...
        logger.info({ src: "ViewsRoutes", viewEventType: type }, `[ViewsRoutes] Broadcast view event "${type}"`);
    ...
    if (method === "GET" && subResource === "") {
        const parsedDetailViewType = resolveViewTypeQuery(url.searchParams.get("viewType"), res, error);
        if ("reject" in parsedDetailViewType)
            return true;
    ...
    if ((method === "GET" || method === "HEAD") &&
        ...
        return handleViewAssetRequest(ctx, viewRuntime, id, subResource, (entry) => satisfiesRoleGate(callerRoles(ctx), entry.roleGate));
    }
    // ── GET /api/views/:id/hero ───────────────────────────────────────────────
    if (method === "GET" && subResource === "hero") {
        const parsedHeroViewType = resolveViewTypeQuery(url.searchParams.get("viewType"), res, error);
        if ("reject" in parsedHeroViewType)
            return true;
    ...
    if (method === "POST" && subResource === "navigate") {
        ...
        const parsedNavigateViewType = resolveViewTypePair(body?.viewType, url.searchParams.get("viewType"), res, error);
        if ("reject" in parsedNavigateViewType)
            return true;
        ...
        const viewPath = (typeof body?.path === "string" ? body.path : null) ??
            entry?.path ??
        ...
        const subview = typeof body?.subview === "string" && body.subview.trim().length > 0
            ? body.subview.trim()
        ...
        const layoutViews = Array.isArray(body?.views)
            ? body.views.filter((value): value is string => typeof value === "string" && value.trim().length > 0)
            : undefined;
        const layout = typeof body?.layout === "string" && body.layout.trim().length > 0
            ? body.layout.trim()
            : undefined;
        const placement = typeof body?.placement === "string" && body.placement.trim().length > 0
            ? body.placement.trim()
            : undefined;
        const payload = body && Object.hasOwn(body, "payload") ? body.payload : undefined;
        const callerOwnedDelivery = body?.delivery === "originating-client" ||
        ...
        logger.info({ src: "ViewsRoutes", viewId: id, viewPath, action, subview }, `[ViewsRoutes] Navigate to view "${id}"${action ? ` (action=${action})` : ""}${subview ? ` (subview=${subview})` : ""}`);
        ...
        const commitCurrentViewState = (committedViewPath: string | null) => {
            ...
            setActiveViewContext(viewRuntime, {
                hostKey: ctx.hostKey,
                viewId: id,
                viewLabel,
                viewType: resolvedViewType,
            ...
            if (viewChanged && ctx.runtime) {
                void ctx.runtime
                    ...
                    .catch((err) => {
                    logger.debug({ src: "ViewsRoutes", err }, "[ViewsRoutes] VIEW_SWITCHED emit failed");
                });
            }
        };
        ...
        const shouldTargetCompletedAction = body?.delivery === "completed-action" && Boolean(originatingClientId);
            ...
            if (originatingClientId) {
                ...
                if (shouldTargetCompletedAction) {
                    try {
                        ...
                        logger.warn({ src: "ViewsRoutes", err, viewId: id }, "[ViewsRoutes] Early completed-action navigation delivery failed");
                    }
                }
                else {
                    delivered = ctx.broadcastWsToClientId?.(originatingClientId, frame);
                ...
                if (!shouldTargetCompletedAction &&
                    ...
                    error(res, `No connected view client "${originatingClientId}" is available for "${id}".`, 409);
                    return true;
                }
    ...
    if (method === "POST" && subResource === "elements") {
        ...
        if (!body)
            return true;
        const parsedElementsViewType = resolveViewTypePair(body.viewType, url.searchParams.get("viewType"), res, error);
        if ("reject" in parsedElementsViewType)
            return true;
        ...
        if (!clientId ||
            ...
            error(res, "Element report does not belong to the current view installation", 409);
            return true;
        }
    ...
    if (method === "POST" && subResource === "activate") {
        ...
        const parsedActivateViewType = resolveViewTypePair(body.viewType, url.searchParams.get("viewType"), res, error);
        if ("reject" in parsedActivateViewType)
            return true;
        ...
        const active = getActiveViewContext(viewRuntime, clientScope(ctx.hostKey, resolveViewInteractClientId(req, body)));
        ...
        const dispatch = await dispatchViewInteract(entry, id, capability, params, {
            ...
            clientId: resolveTargetViewClientId(viewRuntime, ctx.hostKey, id, req, body),
            runtime: viewRuntime,
    ...
    if (method === "POST" && id === "interact-claim" && subResource === "") {
        ...
        const claimId = viewInteractionHost(viewRuntime, ctx.hostKey).claim(clientId, {
        ...
        if (!claimId) {
            error(res, "View interaction is no longer available to this renderer", 409);
            return true;
        }
        json(res, { claimId });
        return true;
    ...
    if (method === "POST" && id === "interact-result" && subResource === "") {
        const body = await readJsonBody<Record<string, unknown>>(req, res);
        if (!body)
            return true; // readJsonBody already sent the error response
        ...
        const result: RendererViewInteractResult = {
            ...
            installationId: typeof body.installationId === "string"
                ? body.installationId
                : undefined,
    ...
    if (method === "POST" && subResource === "interact") {
        ...
        if (!body)
            return true;
        const parsedInteractViewType = resolveViewTypePair(body.viewType, url.searchParams.get("viewType"), res, error);
        if ("reject" in parsedInteractViewType)
            return true;
        ...
        if (entry.capabilities?.length &&
            ...
            if (!declared) {
                error(res, `Capability "${capability}" is not declared for view "${id}"`, 400);
                return true;
            }
        }
        ...
        const timeoutMs = typeof body.timeoutMs === "number" && body.timeoutMs > 0
            ? body.timeoutMs
            : 5000;
        logger.info({ src: "ViewsRoutes", viewId: id, capability }, `[ViewsRoutes] Interact with view "${id}" capability="${capability}"`);
        ...
        const targetClientId = resolveTargetViewClientId(viewRuntime, ctx.hostKey, id, req, body);
        const dispatch = await dispatchViewInteract(entry, id, capability, params, {
            hostKey: ctx.hostKey,
            broadcastWs: ctx.broadcastWs,
            broadcastWsToClientId: ctx.broadcastWsToClientId,
        ...
        if (!dispatch.success && dispatch.failureKind === "timeout") {
            error(res, `View "${id}" did not respond to capability "${capability}" within ${timeoutMs}ms`, 504);
        }
        else {
            json(res, dispatch);
        }
...
const VIEW_INTERACTION_FAILURE_CODES: Readonly<Record<string, NonNullable<ViewInteractDispatchResult["failureKind"]>>> = {
    PENDING_REQUEST_TIMEOUT: "timeout",
    VIEW_HOST_CLOSED: "unavailable",
    VIEW_INSTALLATION_INVALID: "revoked",
    VIEW_REGISTRY_CLOSED: "revoked",
};
function interactionFailureKind(error: unknown): NonNullable<ViewInteractDispatchResult["failureKind"]> {
 ...
 */
export async function dispatchViewInteract(entry: ViewRegistryEntry, viewId: string, capability: string, params: Record<string, unknown> | undefined, transport: ViewInteractTransport, timeoutMs = 5000): Promise<ViewInteractDispatchResult> {
    ...
    const preferFrontend = !hasServerInteract || isSurfaceBrokeredCapability(capability);
    if (preferFrontend &&
        transport.clientId &&
        ...
        const resultPromise = host.waitFor(requestId, transport.clientId, entry, timeoutMs);
        const frame = {
            type: "view:interact",
            installationId: entry.installationId,
            viewId,
        ...
        try {
            ...
            logger.warn({ src: "ViewsRoutes", viewId, capability, requestId, err }, `[ViewsRoutes] Targeted interaction delivery failed for view "${viewId}"`);
            host.cancel(requestId, new ElizaError("Renderer delivery outcome is unknown", {
            ...
            return {
                ...
                success: false,
                failureKind: "unknown",
                error: "Renderer delivery outcome is unknown; do not replay the operation.",
            };
        }
        if (delivered > 0) {
            try {
                ...
                logger.warn({ src: "ViewsRoutes", viewId, capability, requestId, err }, `[ViewsRoutes] Interaction outcome unavailable for view "${viewId}"`);
                return {
                    requestId,
                    success: false,
        ...
        host.cancel(requestId, new ElizaError(unavailable, { code: "VIEW_CLIENT_UNAVAILABLE" }));
    ...
    if (!transport.clientId) {
        return {
            requestId,
            success: false,
            error: "Missing client id for frontend view interaction. Provide X-ElizaOS-Client-Id or clientId.",
        };
    }
    return {
        requestId,
...
function resolveViewInteractClientId(req: Pick<http.IncomingMessage, "headers">, body: Record<string, unknown> | null | undefined): string | null {
    const headers = req.headers ?? {};
    return (normalizeWsClientId(firstHeaderValue(headers["x-elizaos-client-id"])) ??
        normalizeWsClientId(firstHeaderValue(headers["x-eliza-client-id"])) ??
        normalizeWsClientId(body?.clientId));
}
function resolveTargetViewClientId(runtime: IAgentRuntime, hostKey: object, viewId: string, req: Pick<http.IncomingMessage, "headers">, body: Record<string, unknown> | null | undefined): string | null {
    ...
    const mountedOwner = active?.viewId === viewId ? (active.clientId ?? null) : null;
    if (!mountedOwner)
        return explicit;
...
function streamHeroImage(res: http.ServerResponse, data: Buffer, contentType: string, req: http.IncomingMessage): true {
...
function sendGeneratedHero(res: http.ServerResponse, label: string, icon?: string): true {
