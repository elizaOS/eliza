import { buildLegacyShim } from "./dispatch-route.ts";
import { capturedToResult } from "./dispatch-route.ts";
...
export function registerInProcessApi(runtime: object, kernel: RouteKernel): () => void {
    kernels.set(runtime, kernel);
    return () => {
        if (kernels.get(runtime) === kernel)
            kernels.delete(runtime);
    };
}
/** Use the full server routing and authentication boundary without a TCP listener. */
export async function dispatchApiRoute(args: DispatchRouteArgs): Promise<RouteHandlerResult> {
