...
async function readBodyForDispatch(request: Request, method: string): Promise<{
    body: unknown;
    rawBody?: string;
}> {
 ...
 */
export function mountRoutesOnHono(app: Hono, runtime: IAgentRuntime, options: HonoAdapterOptions): void {
    const routes = getHttpRuntime(runtime).routes;
    for (const route of routes as Route[]) {
        if (!honoMethod(route.type))
            continue;
        if (!route.handler && !route.routeHandler)
            continue;
        ...
        const honoHandler = async (ctx: Context): Promise<Response> => {
            ...
            const { body, rawBody } = await readBodyForDispatch(request, request.method);
...
export function buildHonoAppForRuntime(runtime: IAgentRuntime, options: HonoAdapterOptions): Hono {
    const app = new Hono();
