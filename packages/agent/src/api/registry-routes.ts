...
import { type RegistryPluginInfo } from "../services/plugin-manager-types.ts";
import { type RegistrySearchResult } from "../services/plugin-manager-types.ts";
...
export interface RegistryRouteContext extends RouteRequestMeta, Pick<RouteHelpers, "json" | "error"> {
    url: URL;
...
export async function handleRegistryRoutes(ctx: RegistryRouteContext): Promise<boolean> {
    const { res, method, pathname, url, json, error, getPluginManager, getLoadedPluginNames, getBundledPluginIds, classifyRegistryPluginRelease, } = ctx;
        ...
        try {
            ...
            const plugins = Array.from(registry.values()).map((plugin) => {
                ...
                return {
                    ...
                    installedVersion: installed.find((entry) => entry.name === plugin.name)?.version ??
                        null,
                    releaseStream: installed.find((entry) => entry.name === plugin.name)
                        ?.releaseStream ?? null,
                    betaVersion: installed.find((entry) => entry.name === plugin.name)
                        ?.betaVersion ?? null,
                    latestVersion: installed.find((entry) => entry.name === plugin.name)
                        ?.latestVersion ??
                        plugin.npm.v2Version ??
                        null,
                    loaded: loadedNames.has(plugin.name) ||
    ...
    if (method === "GET" &&
        ...
        const name = decodePathComponent(pathname.slice("/api/registry/plugins/".length), res, "plugin name");
        if (name === null)
            return true;
