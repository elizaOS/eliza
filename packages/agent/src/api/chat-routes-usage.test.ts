...
> & {
  messageService?: Pick<MessageService, "handleMessage">;
  actions?: Array<Pick<Action, "name"> & Partial<Action>>;
};

function unexpectedFixtureCall(): never {
  throw new Error("Chat route called an unconfigured fixture collaborator");
}

function createRuntime(overrides: RuntimeOverrides = {}): AgentRuntime {
  const { messageService, actions = [], ...runtimeOverrides } = overrides;
  const runtimeLogger = logger.child({ test: "chat-routes-usage" });
  for (const level of ["info", "warn", "error", "debug"] as const) {
    vi.spyOn(runtimeLogger, level).mockImplementation(() => undefined);
  }
  const runtime: Partial<AgentRuntime> = {
    ...
    actions: actions.map((action) => ({
      description: "Chat route action fixture",
      validate: unexpectedFixtureCall,
      handler: unexpectedFixtureCall,
      ...action,
    })),
    plugins: [],
    ...
    ...(messageService
      ? {
          messageService: {
            ...messageService,
            shouldRespond: unexpectedFixtureCall,
            deleteMessage: unexpectedFixtureCall,
            clearChannel: unexpectedFixtureCall,
          },
        }
      : {}),
  };
...
describe("generateChatResponse usage reporting", () => {
    ...
    const runtime = createRuntime({
      emitEvent: async (event, payload) => {
        ...
        ingress.push({
          ...
          traceId:
            message.metadata && "traceId" in message.metadata
      ...
      } satisfies Pick<MessageService, "handleMessage">,
    });
    await Promise.all([
      generateChatResponse(
        runtime,
  ...
  it("returns actual provider usage when a provider event is emitted", async () => {
    ...
    runtime = createRuntime({
      messageService: {
        handleMessage: vi.fn<MessageService["handleMessage"]>(async () => {
  ...
  it("isolates provider usage across concurrent requests on one runtime", async () => {
    ...
    runtime = createRuntime({
      messageService: {
        handleMessage: vi.fn<MessageService["handleMessage"]>(
          async (_runtime, message) => {
            started += 1;
