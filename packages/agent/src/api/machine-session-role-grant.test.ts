/**
 * Pairing -> chat -> USER surface attribution flow for authenticated machine-session
 ...
 * a USER-gated action descriptor over a stubbed runtime adapter.
 */
...
const userSurface: GateableFixture = {
  name: "USER_SURFACE",
...
/**
 * Mirrors the role/context stages of core's actionGateRejection (USER surface
...
describe("machine-session chat attribution (pairing -> chat -> USER surface)", () => {
  ...
  it("machine-session turns resolve USER and pass the USER action gate; OWNER gates still deny", async () => {
    const harness = createRuntimeHarness();
    ...
    expect(gateAllows(userSurface, userRoles, activeContexts)).toBe(true);
    expect(
      gateAllows(
  ...
  it("keeps an unauthenticated external principal GUEST and USER surface-denied", async () => {
    const harness = createRuntimeHarness();
