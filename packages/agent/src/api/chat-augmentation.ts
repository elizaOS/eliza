...
import { aliasRecallQuery, embedRecallQuery } from "@elizaos/core";
...
export async function maybeAugmentChatMessageWithDocuments(
  ...
  // DocumentService derives authorization from the search message requester.
  // Missing identities must not become agent self-reads, which have global
  // visibility; the nil UUID resolves as a least-privileged requester.
  const searchRequesterEntityId =
    trimmedEntityId.length > 0
      ? (trimmedEntityId as UUID)
      : UNRESOLVED_REQUESTER_ENTITY_ID;
  ...
  const searchMessage = {
    ...
    agentId,
    entityId: searchRequesterEntityId,
    roomId,
    content: {
      ...(message.content as Content),
      text: userPrompt,
