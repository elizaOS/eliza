 ...
 *
 * Harness mirrors `agent-message-route.test.ts`: it invokes the actual
...
import { handleConversationRouteGroup } from "./server-route-dispatch.ts";
...
function createMessageService(reply: string): MessageService {
  return {
    async handleMessage(
      _runtime: Parameters<MessageService["handleMessage"]>[0],
      _message: Parameters<MessageService["handleMessage"]>[1],
      _callback: Parameters<MessageService["handleMessage"]>[2],
      _options: Parameters<MessageService["handleMessage"]>[3],
    ) {
      return {
