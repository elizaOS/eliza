...
type ExperienceRecord = Record<string, unknown> & { id: UUID };
...
type ExperienceServiceLike = {
  listExperiences(query?: ExperienceQuery): Promise<ExperienceRecord[]>;
  getExperience(id: UUID): Promise<ExperienceRecord | null>;
  updateExperience(
    id: UUID,
...
function experienceService(
  runtime: AgentRuntime | null,
): ExperienceServiceLike | null {
  return (
    (runtime?.getService("EXPERIENCE") as ExperienceServiceLike | null) ?? null
  );
}
...
function parseExperienceQuery(url: URL): {
  ...
  const limit = boundedNumber(url.searchParams.get("limit"), {
    min: 1,
    max: 500,
    integer: true,
  });
  const offset = boundedNumber(url.searchParams.get("offset"), {
    min: 0,
    max: 100_000,
    integer: true,
  });
  const minImportance = boundedNumber(url.searchParams.get("minImportance"), {
    min: 0,
    max: 1,
  });
  const minConfidence = boundedNumber(url.searchParams.get("minConfidence"), {
    min: 0,
    max: 1,
  });
  if ([limit, offset, minImportance, minConfidence].includes(null)) {
    return { query: {}, offset: 0, error: "Invalid experience query." };
  }
  const queryText =
...
function presentExperience(
  ...
  const { embedding, ...safe } = experience;
  return {
    ...safe,
...
export async function handleCharacterRoutes(
  ...
  if (pathname.startsWith("/api/character/experiences")) {
    const service = experienceService(state.runtime);
    if (!service) {
      error(res, "Experience service is not available.", 503);
      return true;
    }
    ...
    if (method === "GET" && pathname === "/api/character/experiences") {
      const records = await service.listExperiences(parsedQuery.query);
      ...
      json(res, { data: page.map(presentExperience), total: records.length });
      return true;
    }

    if (method === "GET" && pathname === "/api/character/experiences/graph") {
      const graph = await service.getExperienceGraph(parsedQuery.query);
      json(res, { data: graph });
      return true;
    }
    ...
    if (
      method === "POST" &&
      pathname === "/api/character/experiences/maintenance"
    ) {
      ...
      const limit =
        body.limit === undefined
          ? undefined
          : boundedNumber(String(body.limit), {
              min: 1,
              max: 10_000,
              integer: true,
            });
      if (
        limit === null ||
        (body.deleteDuplicates !== undefined &&
          typeof body.deleteDuplicates !== "boolean")
      ) {
        error(res, "Invalid experience maintenance options.", 400);
        return true;
      }
    ...
    const match = /^\/api\/character\/experiences\/([^/]+)$/.exec(pathname);
    if (!match) return false;
    let decodedId: string;
    try {
      decodedId = decodeURIComponent(match[1] ?? "");
    } catch {
      // error-policy:J3 Malformed percent-encoding is invalid route input.
      error(res, "Invalid experience id.", 400);
      return true;
    }
    ...
    if (method === "GET") {
      ...
      json(res, { data: presentExperience(experience) });
      return true;
    }
    ...
    if (method === "PATCH") {
      ...
      if (Object.keys(body).some((key) => !allowed.has(key))) {
        error(res, "Unsupported experience update field.", 400);
        return true;
      }
      if (body.learning !== undefined && typeof body.learning !== "string") {
        error(res, "Experience learning must be text.", 400);
        return true;
      }
      for (const key of ["importance", "confidence"] as const) {
        if (
          ...
          error(res, `Experience ${key} must be between 0 and 1.`, 400);
          return true;
        }
      }
      for (const key of ["tags", "keywords"] as const) {
        if (
          body[key] !== undefined &&
          (!Array.isArray(body[key]) ||
            body[key].some((value) => typeof value !== "string"))
        ) {
          error(res, `Experience ${key} must be a list of text values.`, 400);
          return true;
        }
      }
      if (
        body.associatedEntityIds !== undefined &&
        (!Array.isArray(body.associatedEntityIds) ||
          body.associatedEntityIds.some(
            (value) => typeof value !== "string" || !validateUuid(value),
          ))
      ) {
        error(res, "Experience associatedEntityIds must contain UUIDs.", 400);
        return true;
      }
      const experience = await service.updateExperience(
        id,
        body as Partial<ExperienceRecord>,
      );
      if (!experience) {
        error(res, "Experience not found.", 404);
        return true;
    ...
    if (method === "DELETE") {
      if (!(await service.deleteExperience(id))) {
        error(res, "Experience not found.", 404);
        return true;
      }
      json(res, { ok: true });
