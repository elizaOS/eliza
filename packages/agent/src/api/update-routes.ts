...
export async function handleUpdateRoutes(ctx: UpdateRouteContext): Promise<boolean> {
    ...
    if (method === "GET" && pathname === "/api/update/status") {
        ...
        const { resolveChannel, checkForUpdate, fetchAllChannelVersions, CHANNEL_DIST_TAGS, } = await import("../services/update-checker.ts");
        const { isTrustedLocalRequest } = await import("./server-helpers-auth.ts");
        const { detectInstallMethod, getUpdateActionPlan } = await import("../services/self-updater.ts");
    ...
    if (method === "PUT" && pathname === "/api/update/channel") {
        ...
        if (!parsedCh.success) {
            error(res, parsedCh.error.issues[0]?.message ??
