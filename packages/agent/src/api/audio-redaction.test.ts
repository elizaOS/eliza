...
describe("parseWavPcm16", () => {
  ...
  it("rejects multiple data chunks instead of leaving samples unredacted", () => {
    const base = makeWav(100);
    const secondData = Buffer.alloc(10);
    secondData.write("data", 0, "ascii");
    secondData.writeUInt32LE(2, 4);
    const malformed = Buffer.concat([base, secondData]);
    malformed.writeUInt32LE(malformed.length - 8, 4);
    expect(() => parseWavPcm16(malformed)).toThrow(/multiple data chunks/);
  });

  it("rejects truncated chunks and inconsistent RIFF geometry", () => {
    const truncated = makeWav(100).subarray(0, -1);
    expect(() => parseWavPcm16(truncated)).toThrow(/RIFF length/);
    const badChunk = makeWav(100);
    badChunk.writeUInt32LE(badChunk.length, 40);
    expect(() => parseWavPcm16(badChunk)).toThrow(/exceeds RIFF bounds/);
  });
});
  ...
  it("throws typed on empty or malformed span lists", async () => {
    ...
    await expect(
      redactAudioBytes({
        bytes: input,
        containerExt: "wav",
        spans: [{ startMs: 1500, endMs: 2000 }],
        mode: "mute",
      }),
    ).rejects.toThrow(/does not overlap/);
  });

  it.skipIf(audioRedactionCapability().lossyContainers)(
    "fails typed (not silently) for lossy containers when the host has no ffmpeg",
    async () => {
      await expect(
        redactAudioBytes({
...
describe("content-addressed redacted variant store", () => {
  ...
  it("stores the variant as a second object keyed off the original sha and is idempotent", async () => {
    ...
    const preparedFirst = await prepareRedactedAudioVariant({
      originalFileName: stored.fileName,
      spans,
      mode: "mute",
      rulesetVersion,
    });
    const first = persistVerifiedRedactedAudioVariant(preparedFirst, VERIFIED);
    ...
    const second = findRedactedAudioVariant({
      originalSha: stored.hash,
      spans,
      mode: "mute",
      rulesetVersion,
    ...
    const preparedThird = await prepareRedactedAudioVariant({
      originalFileName: stored.fileName,
      spans,
      mode: "mute",
      rulesetVersion,
    });
    const third = persistVerifiedRedactedAudioVariant(preparedThird, VERIFIED);
  ...
  it("keys strictly on original sha + spans + mode + ruleset version", async () => {
    ...
    const variant = persistVerifiedRedactedAudioVariant(prepared, VERIFIED);
  ...
  it("rejects unsafe store names and refuses to persist failed verification", async () => {
    await expect(
      prepareRedactedAudioVariant({
    ...
    const stored = persistMediaBytes(makeWav(1500), "audio/wav");
    const prepared = await prepareRedactedAudioVariant({
      originalFileName: stored.fileName,
      spans,
      mode: "mute",
    ...
    expect(() =>
      persistVerifiedRedactedAudioVariant(prepared, {
        ok: false,
        findings: [
          {
            verifierId: "leak-detector",
            transcript: "PII remains",
            piiFound: ["PII"],
            sentinelsMissing: [],
            ok: false,
          },
    ...
    ).toThrow(/mandatory re-transcription verification/);
  });

  it("never accepts a corrupted memo entry that points at the original hash", () => {
    const stored = persistMediaBytes(makeWav(1700), "audio/wav");
    const parts = {
      originalSha: stored.hash,
      spans,
      mode: "mute" as const,
      rulesetVersion: "corrupt-memo-fixture",
    };
    fs.writeFileSync(
      path.join(stateDir, "media", "audio-redactions.json"),
      JSON.stringify([
        {
          key: audioRedactionKey(parts),
