...
import { expandConnectorSourceFilter } from "@elizaos/core";
import { getConnectorAccountManager } from "@elizaos/core";
import { normalizeConnectorSource } from "@elizaos/core";
...
import { setRoomMuteUntil } from "@elizaos/plugin-assistant";
import { setWorldMuteState } from "@elizaos/plugin-assistant";
...
import { type World } from "@elizaos/core";
import { z } from "zod";
let discordModulePromise: Promise<{
    cacheDiscordAvatarUrl: (avatarUrl: string | undefined, options: {
...
function getDiscordModule() {
    if (!discordModulePromise) {
        ...
        /* @vite-ignore */ "@elizaos/plugin-discord") as Promise<{
            cacheDiscordAvatarUrl: (avatarUrl: string | undefined, options: {
                fetchImpl: typeof fetch;
                userId?: string;
            }) => Promise<string | undefined>;
...
const ORPHAN_ROOM_MEMORY_SCAN_LIMIT = 10000;
/**
...
function getRuntimeSendHandlers(runtime: AgentRuntime): Map<string, unknown> | null {
...
function runtimeHasAnySendHandler(runtime: AgentRuntime, source: string): boolean {
    ...
    return Array.from(handlers.keys()).some((key) => key.startsWith(accountPrefix));
}
function runtimeHasUnscopedSendHandler(runtime: AgentRuntime, source: string): boolean {
    return getRuntimeSendHandlers(runtime)?.has(source) ?? false;
}
function runtimeHasAccountScopedSendHandler(runtime: AgentRuntime, source: string): boolean {
    const handlers = getRuntimeSendHandlers(runtime);
    if (!handlers)
        return false;
    const accountPrefix = `${source}${CONNECTOR_ACCOUNT_KEY_SEPARATOR}`;
...
function runtimeHasSendHandler(runtime: AgentRuntime, source: string, accountId?: string): boolean {
    const handlers = getRuntimeSendHandlers(runtime);
    if (!handlers)
        return false;
    if (accountId &&
    ...
    for (const key of handlers.keys()) {
        ...
        if (scopedHandlerCount > 1)
            return false;
    }
    return scopedHandlerCount === 1;
}
type InboxAccountRoutingErrorCode = "INBOX_CONNECTOR_ACCOUNT_AMBIGUOUS" | "INBOX_CONNECTOR_ACCOUNT_CALLER_UNAUTHORIZED" | "INBOX_CONNECTOR_ACCOUNT_LOOKUP_FAILED" | "INBOX_CONNECTOR_ACCOUNT_NOT_FOUND" | "INBOX_CONNECTOR_ACCOUNT_OWNER_BINDING_REQUIRED" | "INBOX_CONNECTOR_ACCOUNT_REQUIRED" | "INBOX_CONNECTOR_ACCOUNT_SOURCE_MISMATCH" | "INBOX_CONNECTOR_ACCOUNT_UNAVAILABLE";
...
type InboxAccountRoutingResolution = InboxAccountRoutingFailure | InboxAccountRoutingSuccess;
...
function accountRoutingFailure(code: InboxAccountRoutingErrorCode, error: string, status: number, context: Record<string, unknown>): InboxAccountRoutingFailure {
...
async function validateInboxAccount(manager: ConnectorAccountManager, source: string, account: ConnectorAccount, callerAuthorization: InboxRouteCallerAuthorization): Promise<InboxAccountRoutingFailure | null> {
    ...
    if (accountProvider !== provider) {
        return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_SOURCE_MISMATCH", `Connector account ${account.id} belongs to ${accountProvider}, not ${provider}`, 409, {
    ...
    if (unavailableReason) {
        return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_UNAVAILABLE", `Connector account ${account.id} is unavailable: ${unavailableReason}`, 409, {
            accountId: account.id,
            source: provider,
            reason: unavailableReason,
        });
    }
    const ownerAuthorityRequired = account.accessGate === "owner_binding" ||
        account.role.trim().toUpperCase() === "OWNER";
    if (ownerAuthorityRequired &&
        roleRank(callerAuthorization.role) < roleRank("OWNER")) {
        return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_CALLER_UNAUTHORIZED", `Authenticated caller cannot use owner connector account ${account.id}`, 403, {
            accountId: account.id,
            source: provider,
    ...
    if (!evaluation.allowed ||
        ...
        return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_OWNER_BINDING_REQUIRED", `Connector account ${account.id} requires a verified owner binding`, 403, {
            accountId: account.id,
            source: provider,
            reason: evaluation.reason ?? "owner binding has not been verified",
        });
    }
    const instanceId = typeof account.metadata?.instanceId === "string"
        ? account.metadata.instanceId
        : undefined;
    const storage = manager.getStorage();
    const verifiedBinding = account.externalId && storage.findOwnerBinding
        ...
        : null;
    const boundIdentityId = verifiedBinding?.identityId ??
    ...
    if (callerAuthorization.identityId &&
        callerAuthorization.identityId !== boundIdentityId) {
        return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_CALLER_UNAUTHORIZED", `Authenticated caller cannot use connector account ${account.id}`, 403, {
            accountId: account.id,
            source: provider,
...
async function findAccountSourceByExactId(manager: ConnectorAccountManager, requestedSource: string, accountId: string): Promise<string | null> {
    const source = normalizeAccountProvider(requestedSource);
    const storedAccounts = await manager.getStorage().listAccounts();
    const storedMatch = storedAccounts.find((account) => account.id === accountId &&
...
async function resolveInboxAccountRouting(runtime: AgentRuntime, source: string, callerAuthorization: InboxRouteCallerAuthorization, requestedAccountId?: string): Promise<InboxAccountRoutingResolution> {
    ...
    if (requestedAccountId) {
        ...
        if (!account || account.id !== requestedAccountId) {
            const actualSource = await findAccountSourceByExactId(manager, provider, requestedAccountId);
            if (actualSource) {
                return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_SOURCE_MISMATCH", `Connector account ${requestedAccountId} belongs to ${actualSource}, not ${provider}`, 409, {
                    accountId: requestedAccountId,
                    requestedSource: provider,
                    accountSource: actualSource,
                });
            }
            return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_NOT_FOUND", `Connector account ${requestedAccountId} was not found for ${provider}`, 404, { accountId: requestedAccountId, source: provider });
        }
        const failure = await validateInboxAccount(manager, provider, account, callerAuthorization);
        return (failure ?? {
            ok: true,
            accountId: account.id,
            mode: "explicit",
    ...
    const accounts = (await manager.listAccounts(provider)).sort((a, b) => a.id.localeCompare(b.id));
    if (accounts.length === 0) {
        if (!runtimeHasUnscopedSendHandler(runtime, provider) &&
            runtimeHasAccountScopedSendHandler(runtime, provider)) {
            return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_REQUIRED", `A registered connector account is required for ${provider}`, 409, { source: provider });
        }
    ...
    if (usable.length === 0) {
        ...
        return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_UNAVAILABLE", `No usable messaging account is available for ${provider}`, 409, {
            source: provider,
    ...
    const defaults = usable.filter((account) => account.metadata?.isDefault === true);
    ...
    return accountRoutingFailure("INBOX_CONNECTOR_ACCOUNT_AMBIGUOUS", `Multiple usable messaging accounts are available for ${provider}; select an account explicitly`, 409, {
        source: provider,
...
function readLooseStringValue(record: Record<string, unknown> | undefined, keys: string[]): string | null {
    if (!record)
        return null;
...
async function resolveTrustedRoomSource(runtime: AgentRuntime, room: Room): Promise<string | null> {
...
function muteUntilIsoFromDuration(durationMinutes: number | undefined): string | undefined {
 ...
 */
async function resolveDiscordParentRoomId(runtime: AgentRuntime, room: Room | undefined): Promise<UUID | undefined> {
    if (!isDiscordConnectorSource(readRoomSource(room)))
        return undefined;
...
async function resolveInboxRoomMuteState(runtime: AgentRuntime, room: Room | undefined, roomId: UUID): Promise<Pick<InboxChat, "muted" | "mutedScope">> {
...
function equalsNormalizedTitle(left: string | null, right: string | null): boolean {
    if (!left || !right)
        return false;
...
function extractReplyAuthorRecord(memory: Memory): Record<string, unknown> | null {
    const meta = memory.metadata as Record<string, unknown> | undefined;
...
function extractDiscordReactionEvent(memory: Memory): DiscordReactionEvent | null {
...
function buildMessageReactionMap(memories: Memory[]): Map<string, InboxReaction[]> {
    const stateByTargetId = new Map<string, Map<string, ReactionAggregateState>>();
    ...
    for (const [targetMessageId, byEmoji] of stateByTargetId) {
        const reactions = Array.from(byEmoji.values())
            .map((aggregate) => {
            const users = Array.from(aggregate.users.values()).filter((value): value is string => typeof value === "string" && value.trim().length > 0);
            return {
...
const DISCORD_PROFILE_CACHE_TTL_MS = 5 * 60000;
const discordRoomProfileCache = new Map<string, {
    expiresAt: number;
    value: DiscordRoomProfile | null;
}>();
const discordUserProfileCache = new Map<string, {
    expiresAt: number;
    value: DiscordUserProfile | null;
}>();
const discordMessageAuthorProfileCache = new Map<string, {
    expiresAt: number;
    value: DiscordMessageAuthorProfile | null;
}>();
function readCachedValue<T>(cache: Map<string, {
    expiresAt: number;
    value: T;
 ...
 */
function writeCachedValue<T>(cache: Map<string, {
    expiresAt: number;
    value: T;
    ...
    if (cache.size <= MAX_DISCORD_PROFILE_CACHE_ENTRIES)
        return;
    const now = Date.now();
...
function getDiscordClient(runtime: AgentRuntime): DiscordClientLike | null {
    ...
    const service = runtimeWithServices.getService(DISCORD_SERVICE_NAME) as IDiscordService | null | undefined;
...
function readStoredDiscordEntityProfile(entity: unknown): StoredDiscordEntityProfile | null {
    if (!entity || typeof entity !== "object") {
        return null;
    }
    const metadata = (entity as {
        metadata?: unknown;
    }).metadata;
...
async function resolveStoredDiscordEntityProfile(runtime: AgentRuntime, entityId: string | undefined): Promise<StoredDiscordEntityProfile | null> {
    if (!entityId)
        return null;
    ...
    try {
        const entity = await runtimeWithEntityLookup.getEntityById(entityId as UUID);
...
async function cacheInboxDiscordAvatar(runtime: AgentRuntime, avatarUrl: string | undefined, userId?: string): Promise<string | undefined> {
...
async function resolveDiscordMessageAuthorProfile(runtime: AgentRuntime, channelId: string, messageId: string): Promise<DiscordMessageAuthorProfile | null> {
...
async function resolveDiscordUserProfile(runtime: AgentRuntime, userId: string): Promise<DiscordUserProfile | null> {
...
async function resolveDiscordRoomProfile(runtime: AgentRuntime, room: Room | undefined, channelIdHint?: string): Promise<DiscordRoomProfile | null> {
    ...
    if (channel && typeof channel === "object") {
        ...
        const parentRecord = channel as {
            parentId?: unknown;
        };
        if (typeof parentRecord.parentId === "string" &&
...
async function collectAgentWorlds(runtime: AgentRuntime): Promise<Map<UUID, World>> {
    const worlds = await runtime.getAllWorlds();
...
async function collectAgentRooms(runtime: AgentRuntime): Promise<Room[]> {
    ...
    if (worldsById.size === 0)
        return [];
    const worldIds = Array.from(worldsById.keys()).filter((id): id is UUID => typeof id === "string");
    if (worldIds.length === 0)
        return [];
    return runtime.getRoomsByWorlds(worldIds, MAX_ROOMS_SCANNED, 0);
}
function resolveInboxWorldLabel(room: Room | undefined, world: World | undefined): string {
...
async function loadRelevantRooms(runtime: AgentRuntime, requestedRoomId: UUID | null): Promise<Map<string, Room>> {
...
function applyInboxChatMemory(accumulator: Map<string, {
...
}>, memory: Memory, room: Room | undefined, source: string): void {
    ...
    if (!existing) {
        accumulator.set(key, {
            ...
            source,
            lastMessageText: truncateWellFormed(toWellFormedUnicode(text), INBOX_CHAT_PREVIEW_LENGTH),
            lastMessageAt: ts,
            messageCount: 1,
    ...
    if (ts > existing.lastMessageAt) {
        ...
        existing.lastMessageText = truncateWellFormed(toWellFormedUnicode(text), INBOX_CHAT_PREVIEW_LENGTH);
...
async function loadLatestRoomMemory(runtime: AgentRuntime, roomId: UUID): Promise<Memory | null> {
    try {
        ...
        const candidates = memories
            ...
            .sort((left, right) => {
            const rightCreated = typeof right.createdAt === "number" &&
                Number.isFinite(right.createdAt)
                ? right.createdAt
                : 0;
            const leftCreated = typeof left.createdAt === "number" && Number.isFinite(left.createdAt)
                ? left.createdAt
                : 0;
...
async function augmentRoomsFromRecentMemories(runtime: AgentRuntime, roomById: Map<UUID, Room>, sourceFilter: Set<string>): Promise<void> {
 ...
 * via this endpoint.
 */
async function loadInboxMessages(runtime: AgentRuntime, limit: number, sourceFilter: Set<string>, roomId: UUID | null, roomSourceHint: string | null): Promise<InboxMessage[]> {
...
function isConnectorVisibleDiscordAssistantMessage(message: InboxMessageRecord): boolean {
    return (isDiscordConnectorSource(message.source) &&
        message.role === "assistant" &&
        (message.hasExplicitSource || message.hasExternalUrl));
}
function isImplicitDiscordAssistantShadow(message: InboxMessageRecord): boolean {
    return (isDiscordConnectorSource(message.source) &&
        message.role === "assistant" &&
...
function suppressUnsentDiscordAssistantShadows(messages: InboxMessageRecord[]): InboxMessageRecord[] {
...
function areLikelyConnectorAssistantDuplicates(left: InboxMessageRecord, right: InboxMessageRecord): boolean {
    ...
    if (Math.abs(left.timestamp - right.timestamp) > 15000) {
        return false;
    }
...
function dedupeInboxMessages(messages: InboxMessageRecord[]): InboxMessageRecord[] {
 ...
 */
async function loadInboxChats(runtime: AgentRuntime, sourceFilter: Set<string>): Promise<InboxChat[]> {
    const worldsById = await collectAgentWorlds(runtime);
    ...
    const backfilledRooms = await Promise.all(Array.from(roomById.entries())
        ...
        const latestMemory = await loadLatestRoomMemory(runtime, roomIdKey as UUID);
    ...
    for (const [roomIdKey, entry] of accumulator) {
        ...
        const muteState = await resolveInboxRoomMuteState(runtime, room, roomIdKey as UUID);
    ...
    chats.sort((a, b) => {
        const bLast = typeof b.lastMessageAt === "number" && Number.isFinite(b.lastMessageAt)
            ? b.lastMessageAt
            : 0;
        const aLast = typeof a.lastMessageAt === "number" && Number.isFinite(a.lastMessageAt)
            ? a.lastMessageAt
            : 0;
 ...
 */
export async function handleInboxRoute(req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, state: InboxRouteState, helpers: RouteHelpers): Promise<boolean> {
    ...
    if (method === "POST" && pathname === "/api/inbox/messages") {
        ...
        if (!callerAuthorization?.ok ||
            ...
            helpers.json(res, {
                error: "Authenticated caller is not allowed to send connector messages",
                code: "INBOX_CALLER_UNAUTHORIZED",
            }, 403);
            return true;
        }
        const rawBody = await helpers.readJsonBody<Record<string, unknown>>(req, res, { maxBytes: 256 * 1024 });
        if (rawBody === null) {
            return true;
        }
        ...
        if (!parsed.success) {
            ...
            helpers.error(res, `Invalid request body at ${issuePath}: ${issue?.message}`, 400);
            return true;
        }
        ...
        try {
            accountRouting = await resolveInboxAccountRouting(runtime, source, callerAuthorization, accountId);
        }
        catch (error) {
        ...
        if (!runtimeHasSendHandler(runtime, source, accountRouting.accountId)) {
            helpers.error(res, accountRouting.accountId
                ? `no send handler registered for inbox source: ${source} accountId: ${accountRouting.accountId}`
                : `no send handler registered for inbox source: ${source}`, 409);
            return true;
        }
        try {
            requireConfirmedSendHandlerDelivery(await runtime.sendMessageToTarget({
            ...
            const [message] = await loadInboxMessages(runtime, 1, new Set([source]), room.id as UUID, source);
    ...
    if (method === "POST" && pathname === "/api/inbox/chats/mute") {
        ...
        const rawBody = await helpers.readJsonBody<Record<string, unknown>>(req, res, { maxBytes: 16 * 1024 });
        if (rawBody === null) {
            return true;
        }
        ...
        try {
            if (scope === "server") {
                ...
                await setWorldMuteState(runtime, worldId as UUID, action === "mute" ? { ...(untilIso ? { untilIso } : {}) } : null);
            }
            else {
                await runtime.updateParticipantUserState(targetRoomId, runtime.agentId, action === "mute" ? "MUTED" : null);
                await setRoomMuteUntil(runtime, targetRoomId, action === "mute"
                    ? (muteUntilIsoFromDuration(durationMinutes) ?? null)
                    : null);
            }
            const latestRoom = (await runtime.getRoom(targetRoomId)) ?? room;
            const muteState = await resolveInboxRoomMuteState(runtime, latestRoom, targetRoomId);
            helpers.json(res, {
                ok: true,
                roomId: targetRoomId,
                scope,
            ...
            helpers.error(res, `failed to update inbox mute state: ${err instanceof Error ? err.message : String(err)}`, 500);
        }
        return true;
    }
    // ── GET /api/inbox/chats ──────────────────────────────────────────
