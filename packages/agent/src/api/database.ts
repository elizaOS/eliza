...
import { normalizeIpForPolicy } from "@elizaos/core";
...
import { type ColumnInfo } from "@elizaos/core/api/agent-api-types";
import { type ConnectionTestResult } from "@elizaos/core/api/agent-api-types";
import { type DatabaseConfig } from "../config/types.eliza.ts";
import { type DatabaseProviderType } from "../config/types.eliza.ts";
import { type DatabaseStatus } from "@elizaos/core/api/agent-api-types";
import { type PostgresCredentials } from "../config/types.eliza.ts";
import { type QueryResult } from "@elizaos/core/api/agent-api-types";
import { type TableInfo } from "@elizaos/core/api/agent-api-types";
export { stripSqlBlockComments, stripSqlDollarQuotedLiterals, stripSqlLineComments, } from "../shared/sql-sanitizers.ts";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
 ...
 */
function withPinnedHost(creds: PostgresCredentials, pinnedHost: string): PostgresCredentials {
 ...
 */
async function validateDbHost(creds: PostgresCredentials, opts: {
...
async function executeRawSql(runtime: AgentRuntime, sqlText: string): Promise<{
...
async function assertTableExists(runtime: AgentRuntime, tableName: string): Promise<boolean> {
 ...
 */
async function handleGetStatus(_req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime | null): Promise<void> {
...
async function handlePutConfig(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    const effectiveProvider = body.provider ?? existingDb.provider ?? ("pglite" as DatabaseProviderType);
 ...
 */
async function handleGetTables(_req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime): Promise<void> {
    // Get all user tables
 ...
 */
export function parseRowsPagination(offsetRaw: string | null, limitRaw: string | null): {
    offset: number;
    limit: number;
} {
    return {
 ...
 */
async function handleGetRows(req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime, tableName: string): Promise<void> {
    const url = new URL(req.url ?? "/", `http://${req.headers.host ?? "localhost"}`);
    const { offset, limit } = parseRowsPagination(url.searchParams.get("offset"), url.searchParams.get("limit"));
 ...
 */
async function handleInsertRow(req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime, tableName: string): Promise<void> {
 ...
 */
async function handleUpdateRow(req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime, tableName: string): Promise<void> {
    const body = await readJsonBody<{
 ...
 */
async function handleDeleteRow(req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime, tableName: string): Promise<void> {
    const body = await readJsonBody<{
        where: Record<string, unknown>;
    }>(req, res);
    if (!body)
 ...
 */
async function handleQuery(req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime): Promise<void> {
    const body = await readJsonBody<{
        sql: string;
        readOnly?: boolean;
    }>(req, res);
    ...
    if (body.readOnly !== false) {
        ...
        if (/[uU]&"/.test(noLiterals)) {
            sendJsonError(res, 'Query rejected: Unicode-escaped identifiers (U&"...") are not allowed in read-only mode: they can hide a dangerous function name from the guard.');
            return;
        }
 ...
 *   POST   /api/database/query
 */
export async function handleDatabaseRoute(req: http.IncomingMessage, res: http.ServerResponse, runtime: AgentRuntime | null, pathname: string): Promise<boolean> {
    ...
    if (rowsMatch) {
        const tableNameDecoded = decodePathComponent(rowsMatch[1], res, "database table name");
        if (tableNameDecoded === null)
            return true;
        if (method === "GET") {
