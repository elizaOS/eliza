...
import { type HttpPlugin } from "@elizaos/shared";
import { type IdentityPersonLinkActorRole } from "@elizaos/core";
...
async function attest(ctx: RouteHandlerContext): Promise<RouteHandlerResult> {
    ...
    if (!service)
        return json(503, { error: "IDENTITY_AUTHORITY_UNAVAILABLE" });
