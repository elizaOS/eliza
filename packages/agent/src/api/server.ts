...
import { AUDIT_EVENT_TYPES } from "../security/audit-log.ts";
import { AUDIT_SEVERITIES } from "../security/audit-log.ts";
import { AgentBackupClientDisconnectedError } from "./backup-json-response.ts";
import { AgentExportError } from "../services/agent-export.ts";
import { CONFIG_WRITE_ALLOWED_TOP_KEYS } from "./plugin-discovery-helpers.ts";
import { CharacterSchema } from "../config/zod-schema.ts";
import { DEFAULT_REPLAY_LIMIT } from "./ws-event-replay.ts";
import { DISABLED_TRIGGER_INTERVAL_MS } from "../triggers/scheduling.ts";
import { DefaultRuntimeOperationManager } from "../runtime/operations/index.ts";
import { ElizaError } from "@elizaos/core";
import { EventType } from "@elizaos/core";
import { MAX_PTY_INPUT_MESSAGE_LENGTH } from "./pty-ws-bridge.ts";
...
import { PGLITE_SNAPSHOT_UNAVAILABLE_TRANSIENT } from "../services/agent-backup.ts";
import { PGLITE_SNAPSHOT_UNAVAILABLE_TRANSIENT_CODE } from "../services/agent-backup.ts";
import { PROACTIVE_INTERACTION_SOURCE } from "../services/proactive-interaction-decider.ts";
...
import { TRIGGER_TASK_TAGS } from "../triggers/runtime.ts";
import { WS_AUTH_GRACE_TIMEOUT_MS } from "./server-helpers-auth.ts";
import { WebSocket } from "ws";
import { WebSocketServer } from "ws";
import { applyCors } from "./server-helpers-auth.ts";
import { applyFirstRunVoicePreset } from "./server-helpers-config.ts";
import { applyWalletRpcConfigUpdate } from "./wallet-rpc.ts";
import { asObject } from "./workbench-helpers.ts";
import { attachPtySessionWsBridge } from "./pty-ws-bridge.ts";
import { bindViewRequestHost } from "./view-interaction-host.ts";
import { buildPluginDiagnosticEntry } from "./plugin-diagnostic.ts";
import { buildTriggerConfig } from "../triggers/scheduling.ts";
import { buildTriggerMetadata } from "../triggers/scheduling.ts";
import { canUseLocalTradeExecution } from "./trade-safety.ts";
import { cancelPendingPtySessionStop } from "./pty-ws-bridge.ts";
import { classifyRegistryPluginRelease } from "../runtime/release-plugin-policy.ts";
import { clearPairing } from "./server-helpers-auth.ts";
...
import { createAgentSnapshot } from "../services/agent-backup.ts";
import { createApiEventHub } from "./event-hub.ts";
import { createColdStrategy } from "../runtime/operations/index.ts";
import { createConfigPluginManager } from "../services/config-plugin-manager.ts";
import { createConnectorHealthMonitor } from "./server-lazy-routes.ts";
import { createDevCloudConfigAuthorityView } from "../config/dev-cloud-env-authority.ts";
import { createHotStrategy } from "../runtime/operations/index.ts";
import { createIntegrationTelemetrySpan } from "../diagnostics/integration-observability.ts";
import { createLocalAgentBackup } from "../services/agent-backup.ts";
...
import { defaultClassifier } from "../runtime/operations/index.ts";
import { detectRuntimeModel } from "./agent-model.ts";
import { discoverPluginsFromManifest } from "./plugin-discovery-helpers.ts";
import { ensureApiTokenForBindHost } from "./server-helpers-auth.ts";
import { ensurePairingCode } from "./server-helpers-auth.ts";
import { ensureWalletKeysInEnvAndConfig } from "./server-helpers-config.ts";
import { estimateExportSize } from "../services/agent-export.ts";
import { executeTriggerTask } from "../triggers/runtime.ts";
import { exportAgent } from "../services/agent-export.ts";
import { extractWebSocketHandshakeToken } from "./server-helpers-auth.ts";
import { flushEarlyLogs } from "./early-logs.ts";
import { formatError } from "@elizaos/core";
import { getAgentEventService } from "../runtime/agent-event-service.ts";
import { getAgentHostBridge } from "../runtime/host-bridge.ts";
import { getAuditFeedSize } from "../security/audit-log.ts";
import { getCloudProviderOptions } from "./server-helpers-config.ts";
import { getConfiguredApiToken } from "./server-helpers-auth.ts";
import { getDefaultHealthChecker } from "../runtime/operations/index.ts";
import { getDefaultRepository } from "../runtime/operations/index.ts";
import { getHttpRuntime } from "@elizaos/core/api/http-plugin-runtime";
import { getInventoryProviderOptions } from "./wallet-rpc.ts";
import { getModelOptions } from "./model-provider-helpers.ts";
import { getOrFetchAllProviders } from "./model-provider-helpers.ts";
import { getOrFetchProvider } from "./model-provider-helpers.ts";
import { getPairingExpiresAt } from "./server-helpers-auth.ts";
import { getProviderOptions } from "./server-helpers-config.ts";
import { getPtyConsoleBridge } from "./server-helpers-swarm.ts";
import { getPtyService } from "./server-helpers-swarm.ts";
import { getReleaseBundledPluginIds } from "./plugin-discovery-helpers.ts";
import { getStoredWalletRpcSelections } from "./wallet-rpc.ts";
import { getStylePresets } from "@elizaos/core/character-presets";
import { getTriggerHealthSnapshot } from "../triggers/runtime.ts";
import { getTriggerLimit } from "../triggers/runtime.ts";
import { getViewClientScope } from "../runtime/view-client-context.ts";
import { handleAccountsRoutes } from "./server-lazy-routes.ts";
import { handleAgentAdminRoutes } from "./server-lazy-routes.ts";
import { handleAgentBackupV2SnapshotRequest } from "./backup-v2-stream-response.ts";
import { handleAgentLifecycleRoutes } from "./server-lazy-routes.ts";
import { handleAgentStatusRoutes } from "./server-lazy-routes.ts";
import { handleAgentTransferRoutes } from "./server-lazy-routes.ts";
import { handleAuthRoutes } from "./server-lazy-routes.ts";
import { handleAvatarRoutes } from "./server-lazy-routes.ts";
import { handleBackgroundTasksRoute } from "./server-lazy-routes.ts";
import { handleBugReportRoutes } from "./server-lazy-routes.ts";
import { handleCharacterRoutes } from "./server-lazy-routes.ts";
import { handleCloudAndCoreRouteGroup } from "./server-lazy-routes.ts";
import { handleConfigRoutes } from "./server-lazy-routes.ts";
import { handleConnectorRoutes } from "./server-lazy-routes.ts";
import { handleContextInspectorRoute } from "./context-inspector-routes.ts";
import { handleConversationRouteGroup } from "./server-lazy-routes.ts";
import { handleDatabaseRouteGroup } from "./server-lazy-routes.ts";
import { handleDiagnosticsRoutes } from "./server-lazy-routes.ts";
import { handleFirstRunRoutes } from "./server-lazy-routes.ts";
import { handleHealthRoutes } from "./server-lazy-routes.ts";
import { handleInboxAndCloudRelayRouteGroup } from "./server-lazy-routes.ts";
import { handleInteractionsRoutes } from "./server-lazy-routes.ts";
import { handleLifeOpsRuntimePluginRoute } from "./server-lazy-routes.ts";
import { handleMemoryRoutes } from "./server-lazy-routes.ts";
import { handleMiscRoutes } from "./server-lazy-routes.ts";
import { handleMobileOptionalRoutes } from "./server-lazy-routes.ts";
import { handleModelConfigRoutes } from "./server-lazy-routes.ts";
import { handleModelsRoutes } from "./server-lazy-routes.ts";
import { handlePermissionRoutes } from "./server-lazy-routes.ts";
import { handlePermissionsExtraRoutes } from "./server-lazy-routes.ts";
import { handlePluginDirectoryRoutes } from "./plugin-directory-routes.ts";
import { handleProjectRoutes } from "./server-lazy-routes.ts";
import { handleProviderSwitchRoutes } from "./server-lazy-routes.ts";
import { handleRegistryRoutes } from "./server-lazy-routes.ts";
import { handleRelationshipsRoutes } from "./server-lazy-routes.ts";
import { handleRemoteCapabilityRoutes } from "./server-lazy-routes.ts";
...
import { handleRuntimeModeRemoteForward } from "./runtime-mode/pre-dispatch.ts";
import { handleRuntimeSwitchRoutes } from "./runtime-switch-routes.ts";
import { handleSandboxRouteGroup } from "./server-lazy-routes.ts";
import { handleStandaloneCloudPairRoute } from "./cloud-pair-route.ts";
import { handleSubscriptionRoutes } from "./server-lazy-routes.ts";
import { handleUpdateRoutes } from "./server-lazy-routes.ts";
import { handleViewsRoutes } from "./server-lazy-routes.ts";
import { handleWorkbenchRoutes } from "./server-lazy-routes.ts";
import { hasBlockedObjectKeyDeep } from "./server-helpers.ts";
import { hasPersistedFirstRunState } from "./server-helpers.ts";
import { importAgent } from "../services/agent-export.ts";
import { injectApiBaseIntoHtml } from "./static-file-server.ts";
import { isAllowedHost } from "./server-helpers-auth.ts";
import { isAuthProtectedRoute } from "./static-file-server.ts";
import { isAuthorized } from "./server-helpers-auth.ts";
import { isBlockedEnvKey } from "./plugin-discovery-helpers.ts";
import { isBlockedObjectKey as isBlockedObjectKeyFromConfig } from "./server-helpers-config.ts";
import { isBoundaryRoleAuthorized } from "./server-helpers-auth.ts";
import { isCloudWalletEnabled } from "../config/feature-flags.ts";
import { isCoreManagerLike } from "../services/plugin-manager-types.ts";
import { isCredentialedCorsOrigin } from "./server-helpers-auth.ts";
import { isLifeOpsCloudPluginRoute } from "./server-autonomy-helpers.ts";
import { isMobilePlatform } from "@elizaos/core/runtime-env";
import { isPluginManagerLike } from "../services/plugin-manager-types.ts";
import { isPublicRuntimePluginRoute } from "./server-lazy-routes.ts";
import { isServerTokenAuthorized } from "./server-helpers-auth.ts";
import { isSharedTerminalClientId } from "./server-helpers-auth.ts";
...
import { isWebSocketAuthorized } from "./server-helpers-auth.ts";
import { isWebSocketSessionTokenAuthorized } from "./server-helpers-auth.ts";
import { isWebSocketUpgradeSessionAuthorized } from "./server-helpers-auth.ts";
import { listLocalAgentBackups } from "../services/agent-backup.ts";
import { listTriggerTasks } from "../triggers/runtime.ts";
import { listenForUiLogs } from "./early-logs.ts";
...
import { loadLocalInferenceVoiceRouteApi } from "./local-inference-server-api.ts";
import { logger } from "@elizaos/core";
import { markWebSocketUpgradeSessionAuthorized } from "./server-helpers-auth.ts";
import { materializeDevCloudConfigAuthorityView } from "../config/dev-cloud-env-authority.ts";
import { maybeCapRequestStorm } from "./request-storm-cap.ts";
import { maybeRouteAutonomyEventToConversation } from "./server-autonomy-helpers.ts";
import { mergeDevCloudConfigAuthorityMutation } from "../config/dev-cloud-env-authority.ts";
import { normalizeCharacterLanguage } from "@elizaos/core/character-presets";
import { normalizePairingCode } from "./server-helpers-auth.ts";
import { normalizeTags } from "./workbench-helpers.ts";
import { normalizeTriggerDraft } from "../triggers/scheduling.ts";
import { normalizeWsClientId } from "./server-helpers-auth.ts";
import { pairingEnabled } from "./server-helpers-auth.ts";
import { parseClampedInteger } from "@elizaos/core/utils/number-parsing";
import { parseEventCursor } from "./ws-event-replay.ts";
import { parseNullableNumber } from "./workbench-helpers.ts";
import { patchTouchesProviderSelection } from "./server-helpers.ts";
import { persistConfigEnv } from "./config-env.ts";
import { pickRandomNames } from "../runtime/first-run-names.ts";
import { providerCachePath } from "./model-provider-helpers.ts";
import { queryAuditFeed } from "../security/audit-log.ts";
import { quiesceRuntimeBeforeReplacement } from "./runtime-replacement-ownership.ts";
import { rateLimitPairing } from "./server-helpers-auth.ts";
import { readAliasedEnv } from "@elizaos/core/utils/env";
import { readJsonBody as parseJsonBody } from "@elizaos/core/api/http-helpers";
import { readRequestBody } from "@elizaos/core/api/http-helpers";
import { readTaskCompleted } from "./workbench-helpers.ts";
import { readTaskMetadata } from "./workbench-helpers.ts";
import { readTriggerConfig } from "../triggers/runtime.ts";
import { readTriggerRuns } from "../triggers/runtime.ts";
import { readUiLanguageHeader } from "./server-helpers-config.ts";
import { redactConfigSecrets } from "./server-helpers-config.ts";
import { redactDeep } from "./server-helpers-config.ts";
import { registerBuiltinViews } from "./server-lazy-routes.ts";
...
import { registerProactiveInteractionDecider } from "../services/proactive-interaction-decider.ts";
import { rejectWebSocketUpgrade } from "./server-helpers-auth.ts";
import { releasePendingWebSocket } from "./server-helpers-auth.ts";
...
import { resolveConfiguredCharacterLanguage } from "./server-helpers-config.ts";
import { resolveConnectorHealthIntervalMs } from "./connector-health.ts";
import { resolveDefaultAgentName } from "./server-helpers-config.ts";
...
import { resolveMcpServersRejection } from "./server-helpers-mcp.ts";
...
import { resolvePrimaryModel } from "../runtime/model-resolution.ts";
import { resolveProviderFromModel } from "./agent-model.ts";
import { resolvePtyDisconnectGraceMs } from "./pty-ws-bridge.ts";
import { resolveRuntimeMode } from "./runtime-mode/runtime-mode.ts";
import { resolveServerOnlyPort } from "@elizaos/core/runtime-env";
import { resolveStateDir } from "../config/paths.ts";
import { resolveTerminalRunClientId } from "./server-helpers-auth.ts";
import { resolveTerminalRunRejection } from "./server-helpers-auth.ts";
import { resolveWalletAutomationMode as resolveAgentAutomationModeFromConfig } from "./wallet-capability.ts";
import { resolveWalletCapabilityStatus } from "./wallet-capability.ts";
import { resolveWalletDiagnosticStatus } from "./plugin-diagnostic.ts";
...
import { resolveWebSocketUpgradeRejection } from "./server-helpers-auth.ts";
import { restoreAgentSnapshot } from "../services/agent-backup.ts";
import { restoreConversationsFromDb as restoreConversationsFromDbImpl } from "./conversation-restore.ts";
import { restoreLocalAgentBackup } from "../services/agent-backup.ts";
...
import { schedulePtySessionStopAfterGrace } from "./pty-ws-bridge.ts";
import { selectReplayEvents } from "./ws-event-replay.ts";
...
import { serveStaticUi } from "./static-file-server.ts";
import { stripRedactedPlaceholderValuesDeep } from "./server-helpers-config.ts";
import { subscribeAuditFeed } from "../security/audit-log.ts";
import { taskToTriggerSummary } from "../triggers/runtime.ts";
import { toWorkbenchTodo } from "./workbench-helpers.ts";
import { triggersFeatureEnabled } from "../triggers/runtime.ts";
import { tryAcquirePendingWebSocket } from "./server-helpers-auth.ts";
import { tryHandleHonoRuntimeRoute } from "./server-lazy-routes.ts";
import { tryHandleLifeOpsInboxFallbackLazy } from "./server-lazy-routes.ts";
import { tryHandleRuntimePluginRoute } from "./server-lazy-routes.ts";
...
import { type AgentStartupDiagnostics } from "./server-types.ts";
import { type AwarenessRegistryLike } from "./agent-status-routes.ts";
import { type ClassifyContext } from "../runtime/operations/index.ts";
...
import { type LocalInferenceVoiceRouteApi } from "./local-inference-server-api.ts";
import { type LogEntry } from "./server-types.ts";
import { type PluginEntry } from "./plugin-discovery-helpers.ts";
import { type PluginManagerLike } from "../services/plugin-manager-types.ts";
import { type ProactiveOffer } from "../services/proactive-interaction-decider.ts";
import { type ReadJsonBodyOptions } from "@elizaos/core/api/route-helpers";
import { type Route } from "@elizaos/shared";
import { type RuntimeModeSnapshot } from "./runtime-mode/runtime-mode.ts";
import { type RuntimeOperationManager } from "../runtime/operations/index.ts";
import { type ServerState } from "./server-types.ts";
import { type TradePermissionMode } from "./trade-safety.ts";
import { type X402PluginModule } from "./x402-contract.ts";
import { type captureEarlyLogs } from "./early-logs.ts";
import { walletDiagnosticDescriptor } from "@elizaos/plugin-wallet/diagnostic";
import { wireCodingAgentChatBridge } from "./server-helpers-swarm.ts";
import { wireCodingAgentSwarmSynthesis } from "./server-helpers-swarm.ts";
import { wireCodingAgentWsBridge } from "./server-helpers-swarm.ts";
import { wireCoordinatorBridgesWhenReady } from "./coordinator-wiring.ts";
import { wireCoordinatorEventRouting } from "./server-helpers-swarm.ts";
import { writeAgentBackupJsonResponse } from "./backup-json-response.ts";
import { writeJsonError } from "@elizaos/core/api/http-helpers";
import { writeJsonResponse } from "@elizaos/core/api/http-helpers";
...
async function getBrowserPlugin(): Promise<BrowserPluginModule> {
    ...
    browserPluginModulePromise ??= importOptionalPlugin<BrowserPluginModule>("@elizaos/plugin-browser").then((browser) => {
...
function wireNativeBrowserPageReader(runtime: AgentRuntime | null): void {
    if (!runtime || nativeReaderWiredRuntimes.has(runtime))
        return;
    ...
    const requestingHost = (clientId: string): object => {
        ...
        if (!scope || scope.clientId !== clientId) {
            throw new ElizaError("Native browser requires the requesting view client", { code: "VIEW_CLIENT_REQUIRED" });
        }
    ...
    const bindReader = () => {
        const browser = runtime.getService("browser") as InstanceType<BrowserPluginModule["BrowserService"]> | null;
        ...
        browser.setNativeClientTransport({
            navigate: async (clientId, url) => {
                ...
                const [{ getViewsBroadcastWsToClientId }, { createShellNavigateViewWsFrame },] = await Promise.all([
                    import("./views-routes.ts"),
                    import("@elizaos/core/events"),
                ]);
                const send = getViewsBroadcastWsToClientId(hostKey);
                if (!send ||
                    send(clientId, createShellNavigateViewWsFrame({
                        viewId: "browser",
                        viewType: "gui",
                        viewLabel: "Browser",
                        source: "agent",
                    ...
                    throw new Error("The requesting native Browser client is not connected.");
            },
            readPage: async (clientId, selector) => {
                const hostKey = requestingHost(clientId);
                const [{ dispatchViewInteract, getViewsBroadcastWsToClientId }, { getView },] = await Promise.all([
                    import("./views-routes.ts"),
                ...
                if (!entry || !sendToClient)
                    throw new Error("Native Browser interaction transport is unavailable.");
                const reply = await dispatchViewInteract(entry, "browser", "get-text", {
                    nativeOnly: true,
...
// otherwise.
async function resolveDesktopBrowserPlugin(method: keyof BrowserPluginModule): Promise<BrowserPluginModule | null> {
    if (isMobilePlatform())
        return null;
...
async function getX402Plugin(): Promise<X402PluginModule | null> {
    ...
    x402PluginModulePromise ??= importOptionalPlugin<X402PluginModule>("@elizaos/plugin-x402")
        .then((x402) => {
...
async function getOptionalPluginApi<T>(key: keyof typeof optionalPluginImports): Promise<T> {
    try {
        ...
        return resolveOptionalPluginImportFailure<T>(String(key), err, optionalPluginSpecifiers[key]);
    }
}
type BrowserWorkspaceCommand = Parameters<BrowserPluginModule["executeBrowserWorkspaceCommand"]>[0];
type BrowserWorkspaceTabKind = NonNullable<Parameters<BrowserPluginModule["openBrowserWorkspaceTab"]>[0]["kind"]>;
let walletApiPromise: Promise<typeof import("@elizaos/plugin-wallet")> | undefined;
function getWalletApi(): Promise<typeof import("@elizaos/plugin-wallet")> {
    walletApiPromise ??= importOptionalPlugin<typeof import("@elizaos/plugin-wallet")>("@elizaos/plugin-wallet").catch((err) => {
        ...
        return resolveOptionalPluginImportFailure<typeof import("@elizaos/plugin-wallet")>("@elizaos/plugin-wallet", err, "@elizaos/plugin-wallet");
    });
...
export { isClientVisibleNoResponse, isNoResponsePlaceholder, stripAssistantStageDirections, } from "./chat-text-helpers.ts";
export { cloneWithoutBlockedObjectKeys, decodePathComponent, findOwnPackageRoot, getErrorMessage, isUuidLike, persistConversationRoomTitle, } from "./server-helpers.ts";
// Re-export for downstream consumers (e.g. @elizaos/app)
export { AGENT_EVENT_ALLOWED_STREAMS, CONFIG_WRITE_ALLOWED_TOP_KEYS, discoverInstalledPlugins, discoverPluginsFromManifest, findPrimaryEnvKey, readBundledPluginPackageMetadata, } from "./plugin-discovery-helpers.ts";
// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------
function getAgentEventSvc(runtime: AgentRuntime | null): AgentEventServiceLike | null {
    return getAgentEventService(runtime);
}
function _requirePluginManager(runtime: AgentRuntime | null): PluginManagerLike {
 ...
 */
function wrapPluginManagerWithLocalFallback(pm: PluginManagerLike): PluginManagerLike {
...
export type { AgentStartupDiagnostics, ConversationMeta, LogEntry, ServerState, ShareIngestItem, SkillEntry, StreamEventEnvelope, StreamEventType, } from "./server-types.ts";
export { fetchWithTimeoutGuard, streamResponseBodyWithByteLimit, } from "./server-helpers-fetch.ts";
/**
 ...
 */
async function readJsonBody<T = Record<string, unknown>>(req: http.IncomingMessage, res: http.ServerResponse, options: ReadJsonBodyOptions = {}): Promise<T | null> {
    return parseJsonBody(req, res, {
        maxBytes: MAX_BODY_BYTES,
        ...options,
    });
}
const readBody = (req: http.IncomingMessage): Promise<string> => readRequestBody(req, { maxBytes: MAX_BODY_BYTES }).then((value) => value ?? "");
...
async function readBackupJsonBody(req: http.IncomingMessage, res: http.ServerResponse): Promise<unknown | null> {
    try {
        ...
        error(res, tooLarge ? BACKUP_BODY_TOO_LARGE : "Invalid backup request body", tooLarge ? 413 : 400);
        return null;
    }
}
...
async function handleBuiltinOptionalRoutes(req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, runtimeAgentId?: string | null): Promise<boolean> {
    ...
    if (absentPluginStub) {
        ...
        json(res, absentPluginStub.buildBody(req), absentPluginStub.statusCode ?? 200);
        return true;
    }
    ...
    if (pathname === "/api/browser-workspace/command" && method === "POST") {
        const browserPlugin = await getBrowserWorkspacePlugin();
        const body = (await readJsonBody<BrowserWorkspaceCommand>(req, res)) ?? null;
        if (!body?.subaction) {
    ...
    const decodedTabId = decodePathComponent(tabMatch[1], res, "browser workspace tab id");
...
function _writeFavoriteAppsToConfig(config: ElizaConfig, apps: string[]): string[] {
...
export { resolveMcpServersRejection, resolveMcpTerminalAuthorizationRejection, } from "./server-helpers-mcp.ts";
 ...
 */
export function resolveTradePermissionMode(config: ElizaConfig): TradePermissionMode {
...
export { AGENT_AUTO_MAX_DAILY_TRADES, agentAutoDailyTrades, assertQuoteFresh, canUseLocalTradeExecution, getAgentAutoTradeDate, QUOTE_MAX_AGE_MS, recordAgentAutoTrade, type TradePermissionMode, } from "./trade-safety.ts";
// ---------------------------------------------------------------------------
...
function persistAgentAutomationMode(state: Pick<ServerState, "config" | "agentAutomationMode">, mode: AgentAutomationMode): void {
    state.agentAutomationMode = mode;
    if (!state.config.features) {
        state.config.features = {};
    }
    const features = state.config.features as Record<string, boolean | {
        enabled?: boolean;
        [k: string]: unknown;
    }>;
 ...
 */
function _buildPluginEvmDiagnosticEntry(state: Pick<ServerState, "config" | "runtime">): PluginEntry {
    return buildPluginDiagnosticEntry(walletDiagnosticDescriptor, resolveWalletDiagnosticStatus(walletDiagnosticDescriptor, state));
}
export { type PluginConfigMutationRejection, resolvePluginConfigMutationRejections, resolvePluginConfigReply, } from "./server-helpers-plugin.ts";
export { resolveWalletExportRejection, type WalletExportRejection, } from "./server-helpers-wallet.ts";
// ---------------------------------------------------------------------------
// Route handler
// ---------------------------------------------------------------------------
...
interface RequestContext {
    ...
    onRestart: ((options?: RuntimeRestartOptions) => Promise<AgentRuntime | null>) | null;
    onRuntimeSwapped?: () => void;
    onRuntimeActivated?: (previousRuntime: AgentRuntime | null, activeRuntime: AgentRuntime) => void | Promise<void>;
}
...
export { ensureApiTokenForBindHost, extractAuthToken, isAllowedHost, isAuthorized, normalizeWsClientId, resolveCorsOrigin, resolveTerminalRunClientId, resolveTerminalRunRejection, resolveWebSocketUpgradeRejection, type TerminalRunRejection, type WebSocketUpgradeRejection, } from "./server-helpers-auth.ts";
...
function getOrCreateRuntimeOperationManager(state: ServerState, restartRuntime: (reason: string) => Promise<boolean>): RuntimeOperationManager {
...
export { handleSwarmSynthesis, routeAutonomyTextToUser, } from "./server-helpers-swarm.ts";
...
async function notifyProactiveInteraction(rt: IAgentRuntime, offer: ProactiveOffer): Promise<void> {
 ...
 */
function wireProactiveInteractionDecider(rt: IAgentRuntime, state: ServerState): void {
...
async function applyRuntimeRestart(state: ServerState, ctx: RequestContext | undefined, reason: string, options?: RuntimeRestartOptions): Promise<boolean> {
    if (!ctx?.onRestart) {
        return false;
    }
    ...
    try {
        ...
        if (!newRuntime) {
            ...
            if (options?.disposeCurrentBeforeBuild) {
                state.startup = {
                    ...state.startup,
                    phase: "error",
                    lastError: "Runtime replacement failed after the current runtime was disposed",
        ...
        try {
            ...
            logger.warn(`[eliza-api] Post-swap runtime cleanup failed: ${err instanceof Error ? err.message : String(err)}`);
        }
        state.broadcastStatus?.();
        return true;
    }
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse, state: ServerState, ctx?: RequestContext): Promise<void> {
    const rawClientId = req.headers["x-elizaos-client-id"] ?? req.headers["x-eliza-client-id"];
    const clientId = normalizeWsClientId(Array.isArray(rawClientId) ? rawClientId[0] : rawClientId);
    return runWithViewClient(clientId ? { hostKey: state, clientId } : undefined, () => handleRequestForViewClient(req, res, state, ctx));
}
async function handleRequestForViewClient(req: http.IncomingMessage, res: http.ServerResponse, state: ServerState, ctx?: RequestContext): Promise<void> {
    ...
    let handleCloudStatusRoutes = async (_args: unknown): Promise<boolean> => false;
    if (
    ...
    const allowHostCookieAuth = requestOrigin === undefined || isCredentialedCorsOrigin(requestOrigin);
    let hostSessionAuthorization: AgentHttpRequestAuthorization = {
        ok: false,
        role: "NONE",
    };
    let hostSessionAuthorizationAttempted = false;
    const resolveHostSessionAuthorization = async (): Promise<AgentHttpRequestAuthorization> => {
        if (hostSessionAuthorizationAttempted)
        ...
        if (typeof resolveAuthorization === "function") {
            hostSessionAuthorization = await resolveAuthorization(req, state.runtime, {
        ...
        const authorized = allowHostCookieAuth && typeof authorize === "function"
    ...
    const isHostSessionAuthorized = async (): Promise<boolean> => (await resolveHostSessionAuthorization()).ok;
    ...
    const restartRuntime = (reason: string, options?: RuntimeRestartOptions): Promise<boolean> => applyRuntimeRestart(state, ctx, reason, options);
    ...
    const handleDesktopAuthBootstrapRoute = getAgentHostBridge().handleDesktopAuthBootstrapRoute;
    if (typeof handleDesktopAuthBootstrapRoute === "function" &&
    ...
    if (method !== "OPTIONS" &&
        (await handleRuntimeModePreDispatch(req, res, state.runtime, ctx?.hostRuntimeMode))) {
        return;
    }
    ...
    if (method !== "OPTIONS" &&
        ...
        !isTrajectoryOwnerRequest(req, method, pathname, await resolveHostSessionAuthorization())) {
    ...
    if (method === "POST" && pathname === "/api/backups/restore") {
        ...
        const fileName = typeof bodyRecord?.fileName === "string" ? bodyRecord.fileName : null;
        if (!fileName) {
    ...
    if (method === "POST" && pathname === "/api/snapshot") {
        ...
        try {
            ...
            if (err instanceof AgentBackupClientDisconnectedError ||
                ...
                logger.warn({ err: message }, "[agent-backup] Snapshot download aborted");
                return;
            }
            if (message === PGLITE_SNAPSHOT_UNAVAILABLE_TRANSIENT) {
                ...
                logger.warn({ err: message }, "[agent-backup] Snapshot temporarily unavailable");
                json(res, {
    ...
    if (method === "POST" && pathname === "/api/restore") {
        ...
        try {
            ...
            if (!restarted) {
                throw new Error("Backup restored, but the runtime could not restart on the restored database");
            }
    ...
    if (!isMobilePlatform() && pathname.startsWith("/api/computer-use/")) {
        const { handleComputerUseRoutes } = await getOptionalPluginApi<{
            handleComputerUseRoutes: (req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string) => Promise<boolean>;
        }>("computerUse");
    ...
    if (await handleSubscriptionRoutes({
        ...
        loadSubscriptionAuth: async () => (await import("@elizaos/auth/auth")) as never,
    } as never)) {
        return;
    }
    ...
    const firstRunGetWalletAddresses = pathname === "/api/wallet/keys"
    ...
    if (await handlePluginDirectoryRoutes({
        req,
        res,
        method,
        pathname,
    ...
    if (method === "POST" && pathname === "/api/plugins/unload-from-directory") {
        ...
        try {
            const { unloadPluginFromDirectory } = await import("../runtime/load-plugin-from-directory.ts");
            const result = await unloadPluginFromDirectory({
                runtime: state.runtime as Parameters<typeof unloadPluginFromDirectory>[0]["runtime"],
                pluginName,
            });
    ...
    if (await handleDiagnosticsRoutes({
        ...
        queryAuditFeed: (query) => queryAuditFeed({
            type: (AUDIT_EVENT_TYPES as readonly string[]).includes(query.type ?? "")
                ? (query.type as (typeof AUDIT_EVENT_TYPES)[number])
                : undefined,
            severity: (AUDIT_SEVERITIES as readonly string[]).includes(query.severity ?? "")
    ...
    if (!isMobilePlatform() && pathname.startsWith("/api/wallet/")) {
        ...
        const { deriveSolanaAddress, fetchEvmBalances, fetchSolanaBalances, fetchSolanaNativeBalanceViaRpc, generateWalletForChain, getWalletAddresses: getCoreWalletAddresses, importWallet, setSolanaWalletEnv, validatePrivateKey, } = await getCoreWalletApi();
        const durableWalletConfig = loadElizaConfig();
        const walletUsesCloudNetwork = method === "GET" || pathname === "/api/wallet/refresh-cloud";
        const walletAuthorityView = walletUsesCloudNetwork
            ? createDevCloudConfigAuthorityView(durableWalletConfig)
            : durableWalletConfig;
        const walletConfig = materializeDevCloudConfigAuthorityView(walletAuthorityView);
        const saveWalletConfig = (nextConfig: ElizaConfig): void => {
            const persistable = mergeDevCloudConfigAuthorityMutation(durableWalletConfig, walletAuthorityView, nextConfig);
        ...
        if (await handleWalletRoutes({
            ...
            deps: {
                ...
                getWalletAddresses: () => getCoreWalletAddresses(state.runtime?.agentId),
                validatePrivateKey,
                importWallet,
                ...
                resolveWalletCapabilityStatus: (args) => ({
                    ...resolveWalletCapabilityStatus({
                        ...
                        getWalletAddresses: () => getCoreWalletAddresses(args.runtime?.agentId),
                    }),
                }),
                isCloudWalletEnabled,
                persistConfigEnv,
                createIntegrationTelemetrySpan: (args) => createIntegrationTelemetrySpan({
                    boundary: "wallet",
    ...
    if ((pathname === "/api/agent/self-status" ||
        ...
        (await (async () => {
            ...
            const getCoreWalletAddresses = pathname === "/api/agent/self-status"
                ? (await getCoreWalletApi()).getWalletAddresses
                : null;
            return handleAgentStatusRoutes({
                ...
                pathname,
                url,
                state: pathname === "/api/agent/self-status"
                    ? {
                        ...state,
                ...
                deps: {
                    ...
                    getAwarenessRegistry: (): AwarenessRegistryLike | null => {
                        ...
                        if (!service || typeof service !== "object")
                            return null;
                        const composeSummary = (service as {
                            composeSummary?: unknown;
                        })
                            .composeSummary;
                        if (typeof composeSummary !== "function")
                            return null;
                        return {
                            composeSummary: (activeRuntime) => Promise.resolve(composeSummary.call(service, activeRuntime)),
                        };
                    },
                    RegistryService,
                },
    ...
    if (pathname.startsWith("/api/inbox")) {
        ...
        inboxCallerAuthorization = resolveInboxRequestAuthorization(req, method, pathname, hostAuthorization);
    }
    ...
    const viewsCallerAuthorization = resolveInboxRequestAuthorization(req, method, pathname, await resolveHostSessionAuthorization());
    ...
    const runtimeManagementCallerAuthorization = resolveInboxRequestAuthorization(req, method, pathname, await resolveHostSessionAuthorization());
    if (await handleRuntimeManagementRoutes({
        req,
        res,
        method,
        pathname,
    ...
    if (await handleRuntimeSwitchRoutes({
        req,
        res,
        method,
        pathname,
    ...
    if (pathname.startsWith("/api/mcp")) {
        ...
        if (await handleMcpRoutes({
            ...
            // other agent routes.
            json: (response: http.ServerResponse, data: unknown, status?: number) => writeJsonResponse(response, data, status).catch((err) => {
                logger.warn(`[api] MCP JSON response write failed: ${err}`);
            }),
            error: (response: http.ServerResponse, message: string, status?: number) => writeJsonError(response, message, status).catch((err) => {
    ...
    if (await handleMiscRoutes({
        ...
        tryAcquireTerminalRunSlot: (scopeId: string, runId: string, maxConcurrent: number) => {
            const now = Date.now();
            if (now - lastTerminalRunIdSweepAt >= TERMINAL_RUN_ID_SWEEP_INTERVAL_MS ||
            ...
            if (terminalRunIdReservations.size >= MAX_TERMINAL_RUN_ID_RESERVATIONS) {
                return { rejection: "registry-capacity" as const };
            }
            terminalRunIdReservations.set(reservationKey, now + TERMINAL_RUN_ID_RESERVATION_TTL_MS);
    ...
    const runtimeRouteConfig = pathname.startsWith("/api/cloud/")
        ? materializeDevCloudConfigAuthorityView(createDevCloudConfigAuthorityView(state.config))
        : state.config;
    ...
    if (await handleBuiltinOptionalRoutes(req, res, pathname, method, state.runtime?.agentId)) {
        return;
    }
    ...
    if (pathname === "/api/context-inspector") {
        ...
        if (await handleContextInspectorRoute({
            req,
            res,
            pathname,
            method,
    ...
    if (await tryHandleLifeOpsInboxFallbackLazy({
        pathname,
        method,
        url,
        res,
    ...
    // platform.
    if (await tryHandleTrajectoryReadRoutes({
        pathname,
        method,
        url,
        runtime: state.runtime,
    ...
    if (await tryHandleHonoRuntimeRoute({
        ...
        isAuthorized: () => hostSessionAuthorization.ok ||
            isAuthorized(req) ||
            isBoundaryRoleAuthorized(req, method, pathname),
        ...
        accessContext: () => {
            if (hostSessionAuthorization.ok && state.runtime) {
                return resolveHostSessionAccessContext(hostSessionAuthorization, state.runtime);
            }
...
// Server start
// ---------------------------------------------------------------------------
export type ApiRequestMiddleware = (req: http.IncomingMessage, res: http.ServerResponse, next: () => Promise<void>) => Promise<void>;
export type ApiServerConfigurator = (server: http.Server) => void | Promise<void>;
/** Mandatory host policy precedes built-in authentication; true grants no identity. */
export type ApiHostAdmission = (request: http.IncomingMessage, boundary: "request" | "upgrade" | "websocket-send" | "websocket-message") => boolean | Promise<boolean>;
export type WebSocketAuthorizer = (request: http.IncomingMessage, url: URL) => boolean | Promise<boolean>;
...
export async function startApiServer(opts?: {
    ...
    updateStartup: (update: Partial<AgentStartupDiagnostics> & {
        phase?: string;
        attempt?: number;
    ...
    const hostConfig = opts?.hostConfig === undefined
        ? undefined
        : structuredClone(opts.hostConfig);
    async function admitHostRequest(request: http.IncomingMessage, boundary: "request" | "upgrade" | "websocket-send" | "websocket-message"): Promise<403 | 503 | null> {
        if (!hostAdmission)
            return null;
        try {
    ...
    const connectorHealthIntervalMs = resolveConnectorHealthIntervalMs(process.env.CONNECTOR_HEALTH_INTERVAL_MS);
    ...
    const _workspaceDir = config.agents?.defaults?.workspace ?? resolveDefaultAgentWorkspaceDir();
    ...
    const addLog = (level: string, message: string, source = "system", tags: string[] = []) => {
    ...
    const requestContext: RequestContext = {
        hostRuntimeMode: hostConfig === undefined ? undefined : resolveRuntimeMode(hostConfig),
        onRestart,
        onRuntimeActivated,
        onRuntimeSwapped: () => {
            bindInProcessApi();
    ...
    const runtimeOperations: Pick<RuntimeOperationManager, "start" | "get"> = {
        start: async (request) => {
            ...
            return getOrCreateRuntimeOperationManager(state, (reason) => applyRuntimeRestart(state, requestContext, reason)).start(request);
        },
        get: (id) => getOrCreateRuntimeOperationManager(state, (reason) => applyRuntimeRestart(state, requestContext, reason)).get(id),
    };
    ...
    if (deviceBridgeAttachAllowed &&
        ...
        setImmediate(() => {
            void getOptionalPluginApi<{
                attachMobileDeviceBridgeToServer: (server: http.Server) => Promise<boolean>;
            }>("capacitor")
    ...
    server.timeout = 0;
    logger.debug("[eliza-api] Server lifecycle: requestTimeout=300000ms, idleTimeout=disabled, headersTimeout=60000ms, keepAliveTimeout=60000ms");
    ...
    const admitWebSocket = async (ws: WebSocket, request: http.IncomingMessage, boundary: "websocket-send" | "websocket-message"): Promise<boolean> => {
    ...
    wss.on("error", (err: unknown) => {
        logger.warn(`[eliza-api] WebSocketServer error: ${err instanceof Error ? err.message : err}`);
    });
    /**
    ...
    const wsPtyDisconnectGraceMs = resolvePtyDisconnectGraceMs(process.env.ELIZA_PTY_WS_DISCONNECT_GRACE_MS);
    /**
    ...
    const isDuplicateWsMessage = (clientId: string | undefined, msgId: unknown): boolean => {
    ...
    const isDeviceBridgeDelegationExpected = (): boolean => {
        ...
        return Boolean(process.env.ELIZA_DEVICE_PAIRING_TOKEN?.trim() ||
    ...
    server.on("upgrade", async (request, socket, head) => {
        ...
        socket.on("error", (err: unknown) => {
            logger.warn(`[eliza-api] WS upgrade socket error: ${err instanceof Error ? err.message : err}`);
            try {
                socket.destroy();
            }
            catch {
        ...
        try {
            ...
            if (rejection?.status === 401) {
                ...
                if (handshakeToken) {
                    ...
                    if (!tryAcquirePendingWebSocket(lookupPeer)) {
                        rejectWebSocketUpgrade(socket, 401, "Too many unauthenticated WebSocket connections");
                        return;
                    }
                    try {
                        if (await isWebSocketSessionTokenAuthorized(handshakeToken, state.runtime)) {
            ...
            if (!staticallyAuthorized &&
                ...
                try {
                    ...
                    logger.error(`[eliza-api] host WebSocket authorization failed: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            if (hostAuthorized) {
            ...
            try {
                wss.handleUpgrade(request, socket, head, (ws: WebSocket) => {
                    ...
                    ws.on("error", (err: unknown) => {
                        logger.warn(`[eliza-api] WebSocket error: ${err instanceof Error ? err.message : err}`);
                    });
    ...
    wss.on("connection", (ws: WebSocket, request: http.IncomingMessage) => {
        ...
        let isAuthenticated = hostAuthorized ||
        ...
        if (!isAuthenticated) {
            authGraceTimer = setTimeout(() => {
                authGraceTimer = null;
                logger.warn("[eliza-api] closing WebSocket that did not authenticate within the grace period");
        ...
        const activateAuthenticatedConnection = () => {
            ...
            if (wsClientId &&
                ...
                logger.info(`[eliza-api] client ${wsClientId} reconnected within the PTY grace window; keeping its PTY sessions alive`);
            }
            ...
            try {
                sendWebSocket(ws, JSON.stringify({
                    type: "status",
        ...
        const stopOwnedPtySessions = (reason: string): void => {
            if (!wsClientId)
                return;
            const service = getPtyService(state);
            if (!service?.listSessions || !service.stopSession)
                return;
            const owned = service
                .listSessions()
            ...
            for (const session of owned) {
                void service.stopSession(session.sessionId).catch((err) => {
                    logger.warn(`[eliza-api] failed to stop PTY session ${session.sessionId} on ${reason}: ${err instanceof Error ? err.message : err}`);
                });
            }
        };
        /**
        ...
        ws.on("message", async (data: unknown) => {
            try {
                ...
                if (!isAuthenticated) {
                    ...
                    const providedToken = msg.type === "auth" && typeof msg.token === "string"
                        ? msg.token.trim()
                        : "";
                    let authorized = Boolean(expected && providedToken && tokenMatches(expected, providedToken));
                    if (!authorized && providedToken) {
                        ...
                        try {
                            authorized = await isWebSocketSessionTokenAuthorized(providedToken, state.runtime);
                        }
                        finally {
                ...
                if (msg.type === "ping") {
                    ...
                    if (bridge) {
                        if (!currentClientOwnsPtySession(msg.sessionId)) {
                            logger.warn(`[eliza-api] pty-subscribe rejected: client ${wsClientId ?? "unknown"} does not own session ${msg.sessionId}`);
                            return;
                        }
                    ...
                    if (!subs?.has(msg.sessionId)) {
                        ...
                        logger.warn(`[eliza-api] pty-input rejected: client ${wsClientId ?? "unknown"} does not own session ${msg.sessionId}`);
                    }
                        ...
                        logger.warn(`[eliza-api] pty-input rejected: payload too large (${msg.data.length} chars) for session ${msg.sessionId}`);
                        if (ws.readyState === 1) {
                            sendWebSocket(ws, JSON.stringify({
                                type: "pty-error",
                    ...
                    if (!subs?.has(msg.sessionId)) {
                        ...
                        logger.warn(`[eliza-api] pty-resize rejected: client ${wsClientId ?? "unknown"} does not own session ${msg.sessionId}`);
                    }
                    else {
                        const bridge = getPtyConsoleBridge(state);
                        if (bridge &&
                    ...
                    void import("./views-routes.ts")
                        ...
                        if (!state.runtime || !wsClientId)
                            return;
                        resolveViewInteractResult(state.runtime, state, wsClientId, {
                            requestId: msg.requestId,
                            viewId: typeof msg.viewId === "string" ? msg.viewId : undefined,
                            viewType: typeof msg.viewType === "string" ? msg.viewType : undefined,
                            installationId: typeof msg.installationId === "string"
                                ? msg.installationId
                                : undefined,
                            claimId: typeof msg.claimId === "string" ? msg.claimId : undefined,
    ...
    state.broadcastWsToClientId = (clientId: string, data: object) => eventHub.sendToClient(clientId, data);
    ...
    void import("./views-routes.ts")
        ...
        setViewsBroadcastWs(state, state.broadcastWs ?? null, state.broadcastWsToClientId ?? null);
    })
        .catch((err) => {
        logger.error(`[eliza-api] failed to wire views broadcaster: ${err instanceof Error ? err.message : err}`);
    });
    state.broadcastWsToConversation = (conversationId: string, data: object) => eventHub.sendToConversation(conversationId, data);
     ...
     */
    const overlayDbCharacter = async (rt: AgentRuntime, st: typeof state): Promise<void> => {
        try {
            ...
            const agentRecord = dbAgent && typeof dbAgent === "object" && !Array.isArray(dbAgent)
                ...
                : null;
            const saved = agentRecord?.character as Record<string, unknown> | undefined;
            ...
            if (Array.isArray(saved.messageExamples)) {
                c.messageExamples = saved.messageExamples as NonNullable<typeof c.messageExamples>;
            }
    ...
    const assertX402RoutesValid = async (rt: AgentRuntime | null | undefined): Promise<void> => {
        ...
        if (!x402)
            return; // x402 module unavailable (e.g. mobile bundle) — nothing to validate
        const { validateX402Startup } = x402;
        if (!validateX402Startup)
            return;
        const result = validateX402Startup(getHttpRuntime(rt).routes as Route[], rt.character, {
            agentId,
        });
    ...
    const updateStartup = (update: Partial<AgentStartupDiagnostics> & {
        phase?: string;
        attempt?: number;
        state?: ServerState["agentState"];
    }): void => {
    ...
    addLog("info", "Structured logger connected — agent logs will stream to the UI", "system", ["system", "agent"]);
    ...
    if (opts?.skipListen) {
        ...
        addLog("info", "API server initialized without a TCP listener (skipListen)", "system", ["server", "system"]);
        logger.info("[eliza-api] Started without binding a TCP listener (skipListen; local-agent IPC mode)");
    ...
    const listener = await listenHttpServer({
        ...
        onPortInUse: (occupiedPort, willFallback) => {
            logger.warn(`[eliza-api] Port ${occupiedPort} is already in use. Checking fallback...`);
            if (willFallback) {
                logger.warn("[eliza-api] Retrying with dynamic port (0)...");
            }
            else {
                logger.error(`[eliza-api] Strict port binding is enabled; refusing dynamic fallback from ${occupiedPort}.`);
            }
        },
        onServerError: (err) => {
