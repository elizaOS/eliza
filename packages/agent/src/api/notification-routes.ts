...
import { type NotificationInput } from "@elizaos/core";
import { type NotificationPriority } from "@elizaos/core";
import { type NotificationServiceLifecycleRuntime } from "@elizaos/core";
...
function respondServiceUnavailable(res: http.ServerResponse, state: NotificationRouteState, method: string, pathname: string, helpers: RouteHelpers): boolean {
...
function parseNotificationInput(body: Record<string, unknown>): {
    ...
    const input: NotificationInput = {
        ...
        source: typeof body.source === "string"
            ...
            : undefined,
        deepLink: typeof body.deepLink === "string"
            ? body.deepLink.trim() || undefined
            : undefined,
        groupKey: typeof body.groupKey === "string"
...
export async function handleNotificationRoute(req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, state: NotificationRouteState, helpers: RouteHelpers): Promise<boolean> {
    ...
    if (method === "GET" && pathname === "/api/notifications") {
        ...
        if (requestedUnreadValues.length > 1 ||
