...
import { PostConnectorRequestSchema } from "@elizaos/core/contracts/connector-routes";
...
function getConfiguredConnectorsFromEnv(): Record<string, {
    enabled: true;
    configuredViaEnv: true;
}> {
    const configured: Record<string, {
        enabled: true;
        configuredViaEnv: true;
    }> = {};
...
function listVisibleConnectors(config: ElizaConfig): Record<string, unknown> {
    const rawConnectors = config.connectors ??
        ...
        {};
    const visibleConnectors = rawConnectors &&
        typeof rawConnectors === "object" &&
        !Array.isArray(rawConnectors)
        ? { ...rawConnectors }
        : {};
    for (const [connectorName, summary] of Object.entries(getConfiguredConnectorsFromEnv())) {
...
// Route handler
// ---------------------------------------------------------------------------
export async function handleConnectorRoutes(ctx: ConnectorRouteContext): Promise<boolean> {
    const { req, res, method, pathname, state, json, error, readJsonBody, saveElizaConfig, redactConfigSecrets, isBlockedObjectKey, hasBlockedObjectKeyDeep, cloneWithoutBlockedObjectKeys, onConnectorDisconnect, } = ctx;
    // ── GET /api/connectors ──────────────────────────────────────────────
    ...
    if (method === "POST" && pathname === "/api/connectors") {
        ...
        const sanitizedConfig = cloneWithoutBlockedObjectKeys(config) as ConnectorConfig;
        ...
        try {
            ...
            error(res, err instanceof Error
                ? `Failed to save connector config: ${err.message}`
                : "Failed to save connector config", 500);
            return true;
        }
        ...
        json(res, {
            connectors: redactConfigSecrets(state.config.connectors as Record<string, unknown>),
        });
        return true;
    }
    ...
    if (method === "DELETE" && pathname.startsWith("/api/connectors/")) {
        ...
        const previousConnector = state.config.connectors && Object.hasOwn(state.config.connectors, name)
        ...
        const channels = stateConfigRecord.channels &&
            typeof stateConfigRecord.channels === "object"
            ? (stateConfigRecord.channels as Record<string, unknown>)
            : undefined;
        const previousChannel = channels && Object.hasOwn(channels, name) ? channels[name] : undefined;
