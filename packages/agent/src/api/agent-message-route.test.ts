...
function createCallbackMessageService(
  ...
  return {
    async handleMessage(
      _runtime: Parameters<MessageService["handleMessage"]>[0],
      _message: Parameters<MessageService["handleMessage"]>[1],
      callback: Parameters<MessageService["handleMessage"]>[2],
    ) {
      if (streamedReply) {
        await callback?.({
          text: streamedReply,
...
describe("POST /api/agents/:id/message (issue #7680)", () => {
  ...
  it("scopes hosted waifu conversations to the holder wallet while admin can see all", async () => {
    ...
    const otherMessages = createCtx({
      ...
      state: {
        ...created.state,
        runtime: createRuntime(stringToUuid("holder-chat-agent")),
      },
    });
    expect(await otherMessages.invoke()).toBe(true);
    expect(otherMessages.record.status).toBe(404);

    // Missing runtime is uniformly unavailable, regardless of conversation
    // ownership or existence; it must not become an existence oracle.
    for (const [targetId, wallet] of [
      [conversationId, ownerWallet],
      [conversationId, otherWallet],
      [stringToUuid("missing-holder-conversation"), otherWallet],
    ] as const) {
      const unavailable = createCtx({
        method: "GET",
        pathname: `/api/conversations/${targetId}/messages`,
        runtime: null,
        headers: waifuAuthHeaders("guest", wallet),
        state: created.state,
      });
      expect(await unavailable.invoke()).toBe(true);
      expect(unavailable.record.status).toBe(503);
    }
    ...
    const runtime = createRuntime(agentId, {
      messageService: {
        async handleMessage(
          ...
          _message: Parameters<MessageService["handleMessage"]>[1],
          _callback: Parameters<MessageService["handleMessage"]>[2],
          options: Parameters<MessageService["handleMessage"]>[3],
        ) {
