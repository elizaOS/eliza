...
function validateStreamSettings(raw: unknown): {
    ...
    if ("avatarIndex" in input) {
        if (typeof input.avatarIndex !== "number" ||
...
function getRuntimeModeFallbackSnapshot(): {
    ...
    const deploymentTarget = normalizeDeploymentTargetConfig(loadEffectiveElizaConfig().deploymentTarget);
    const deploymentRuntime = deploymentTarget?.runtime ?? "local";
    return {
        mode: deploymentRuntime,
        deploymentRuntime,
        isRemoteController: deploymentRuntime === "remote",
        remoteApiBaseConfigured: Boolean(deploymentRuntime === "remote" && deploymentTarget?.remoteApiBase?.trim()),
    };
}
...
function sendEmptyComputerUseApprovalStream(req: http.IncomingMessage, res: http.ServerResponse): void {
    res.writeHead(200, {
...
export async function handleMobileOptionalRoutes(req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string): Promise<boolean> {
    ...
    if (method === "POST" && pathname === "/api/computer-use/approval-mode") {
        try {
            ...
            sendJsonError(res, err instanceof Error ? err.message : "Invalid approval mode payload", 400);
            return true;
        }
    ...
    if (method === "POST" && pathname === "/api/stream/settings") {
        try {
            ...
            sendJsonError(res, err instanceof Error ? err.message : "Invalid stream settings", 400);
        }
        return true;
    }
    ...
    if (absentPluginStub) {
        ...
        sendJson(res, absentPluginStub.buildBody(req), absentPluginStub.statusCode ?? 200);
        return true;
    }
    return false;
}
