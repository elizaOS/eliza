...
import { findProtectedNamespace } from "./route-mode-matrix.ts";
import { findRouteModeRule } from "./route-mode-matrix.ts";
...
function isRuntimeModeList(value: unknown): value is ReadonlyArray<RuntimeMode> {
    return (Array.isArray(value) &&
        value.every((mode) => mode === "local" ||
            mode === "local-only" ||
            mode === "cloud" ||
            mode === "remote"));
}
export function findRegisteredRouteModeRule(args: {
    ...
    for (const route of routes) {
        if (route.type === "STATIC" || route.type !== method)
            continue;
        if (!isRuntimeModeList(route.modes) || route.modes.length === 0)
            continue;
        if (matchPluginRoutePath(route.path, args.pathname) === null)
            continue;
        return {
            path: route.path,
            method: route.type,
...
export function applyRouteModeGuard(req: http.IncomingMessage, res: http.ServerResponse, runtime?: RouteModeRuntimeLike | null, snapshot: RuntimeModeSnapshot = getRuntimeModeSnapshot()): ModeGateOutcome {
