...
function currentConfigMtimeMs(): number {
  try {
    ...
    // error-policy:J4 missing config file is the documented no-config state
    // (defaults apply); a stable sentinel keeps the cache working and file
    // creation still invalidates it.
    return 0;
  }
}
