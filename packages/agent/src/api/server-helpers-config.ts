...
import { FIRST_RUN_CLOUD_PROVIDER_OPTIONS } from "@elizaos/core/contracts/first-run-options";
import { FIRST_RUN_PROVIDER_CATALOG } from "@elizaos/core/contracts/first-run-options";
import { generateWalletKeys } from "./wallet-keygen.ts";
...
function failConfigSecretFilterUnbounded(context: Record<string, unknown>, cause?: unknown): never {
    throw new ElizaError("Config response exceeds the secret-key filter walk budget", {
...
function ownEnumerableStringKeys(value: object): string[] {
    ...
    for (const key of inspectFilter("ownKeys", () => Reflect.ownKeys(value))) {
        if (typeof key !== "string")
            continue;
        const descriptor = inspectFilter("getOwnPropertyDescriptor", () => Object.getOwnPropertyDescriptor(value, key));
...
function ownValueDescriptor(value: object, key: string): PropertyDescriptor | undefined {
    const descriptor = inspectFilter("getOwnPropertyDescriptor", () => Object.getOwnPropertyDescriptor(value, key));
    if (!descriptor)
        return undefined;
...
function walkConfigSecretFilter(value: unknown, depth: number, ctx: FilterWalkContext, redactAll: boolean, visitAlreadyReserved = false): unknown {
    ...
    if (!visitAlreadyReserved)
        reserveFilterVisits(ctx, 1);
    if (value === null || value === undefined)
        return value;
    ...
    try {
        if (Array.isArray(value)) {
            ...
            for (let index = 0; index < length; index += 1) {
                ...
                if (!descriptor)
                    continue;
                next[index] = walkConfigSecretFilter(descriptor.value, depth + 1, ctx, redactAll, true);
            }
            return next;
        }
        ...
        for (const key of keys) {
            ...
            out[key] = walkConfigSecretFilter(descriptor.value, depth + 1, ctx, childRedactAll, true);
        }
        return out;
    }
    finally {
...
function walkStripRedactedPlaceholders(value: unknown, depth: number, ctx: FilterWalkContext, visitAlreadyReserved = false): void {
    if (depth > MAX_CONFIG_SECRET_FILTER_DEPTH) {
        failConfigSecretFilterUnbounded({
            depth,
            max: MAX_CONFIG_SECRET_FILTER_DEPTH,
...
export function validateSkillId(skillId: string, res: http.ServerResponse): string | null {
    if (!skillId ||
...
export function readUiLanguageHeader(req: http.IncomingMessage | undefined): string | undefined {
    if (!req) {
        return undefined;
    }
...
export function resolveConfiguredCharacterLanguage(config?: ElizaConfig, req?: http.IncomingMessage) {
    const uiLanguage = readUiLanguageHeader(req) ??
        ((config?.ui as {
            language?: unknown;
        } | undefined)?.language as string | undefined);
    return normalizeCharacterLanguage(uiLanguage);
}
export function resolveFirstRunStylePreset(body: Record<string, unknown>, language: string) {
...
export function applyFirstRunVoicePreset(config: ElizaConfig, body: Record<string, unknown>, language: string) {
...
export function resolveDefaultAgentName(config?: ElizaConfig, req?: http.IncomingMessage): string {
...
export function isSafeResetStateDir(resolvedState: string, homeDir: string): boolean {
