...
import { NOTIFICATION_PUSH_SERVICE_TYPE } from "../services/push/notification-push-service.ts";
import { NotificationPushService } from "../services/push/notification-push-service.ts";
import { isPushTokenValidationError } from "../services/push/push-token-registry.ts";
import { type PushPlatform } from "../services/push/push-token-registry.ts";
...
export async function handlePushTokenRoute(req: http.IncomingMessage, res: http.ServerResponse, pathname: string, method: string, state: PushTokenRouteState, helpers: RouteHelpers): Promise<boolean> {
    if (!pathname.startsWith(PUSH_TOKENS_PREFIX))
        return false;
    ...
    const tokenMatch = pathname.match(/^\/api\/notifications\/push-tokens\/([^/]+)$/);
 ...
 */
async function unregisterOrError(registry: PushTokenRegistry, token: string, res: http.ServerResponse, helpers: RouteHelpers): Promise<boolean> {
    try {
