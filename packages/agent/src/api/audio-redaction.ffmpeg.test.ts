...
/** Decode a real window to PCM16 and calculate its RMS level in dBFS. */
function meanVolumeDb(
  ...
  if (result.status !== 0) {
    throw new Error(`PCM probe decode failed with status ${result.status}`);
  }
  ...
  if (sampleCount === 0 || energy === 0) return -120;
  return 20 * Math.log10(Math.sqrt(energy / sampleCount));
}
...
describe.skipIf(!capability.lossyContainers)(
  ...
  () => {
    ...
    it("store-level idempotency holds on the ffmpeg lane (memo present or wiped)", async () => {
      ...
      const preparedFirst = await prepareRedactedAudioVariant(job);
      const first = persistVerifiedRedactedAudioVariant(
        preparedFirst,
        VERIFIED,
      );
      ...
      const preparedThird = await prepareRedactedAudioVariant(job);
      const third = persistVerifiedRedactedAudioVariant(
        preparedThird,
        VERIFIED,
      );
