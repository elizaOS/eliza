...
import { PatchMemoryRequestSchema } from "@elizaos/shared";
import { PostMemoryRememberRequestSchema } from "@elizaos/shared";
...
import { getDocumentsService } from "@elizaos/plugin-assistant";
...
import { type DocumentsServiceResult } from "@elizaos/plugin-assistant";
...
async function ensureMemoryConnection(runtime: AgentRuntime, agentName: string): Promise<{
    roomId: UUID;
    entityId: UUID;
}> {
    ...
    const messageServerId = stringToUuid(`${agentName}-hash-memory-server`) as UUID;
 ...
 */
export function rankByKeyword<T>(query: string, items: T[], getText: (item: T) => string): Array<{
    item: T;
    score: number;
}> {
    if (items.length === 0)
    ...
    const bm25 = new BM25(items.map((item) => ({ content: getText(item) })), { stemming: true });
 ...
 */
function rankWithIndex<T>(query: string, items: T[], bm25: BM25): Array<{
    item: T;
    score: number;
}> {
    if (items.length === 0)
 ...
 */
let memorySearchCorpusCaches = new WeakMap<AgentRuntime, Map<string, MemorySearchCacheSlot>>();
type MemorySearchBuildAdmission = {
    active: number;
    waiters: Array<() => void>;
};
let memorySearchBuildAdmissions = new WeakMap<AgentRuntime, MemorySearchBuildAdmission>();
...
export function invalidateMemorySearchCache(runtime?: AgentRuntime, roomId?: UUID): void {
    if (!runtime) {
...
async function countRoomMessages(runtime: AgentRuntime, roomId: UUID): Promise<number | null> {
...
function runtimeMemorySearchCache(runtime: AgentRuntime): Map<string, MemorySearchCacheSlot> {
...
function retainMemorySearchCacheSlot(cache: Map<string, MemorySearchCacheSlot>, roomId: string, slot: MemorySearchCacheSlot): void {
    // Map insertion order is the LRU order.
    cache.delete(roomId);
    cache.set(roomId, slot);
}
async function acquireMemorySearchCacheSlot(runtime: AgentRuntime, roomId: string): Promise<{
    ...
    while (true) {
        ...
        const evictable = [...cache].find(([, candidate]) => candidate.builds.size === 0 && candidate.leases === 0);
        if (evictable) {
...
async function withMemorySearchBuildPermit<T>(runtime: AgentRuntime, task: () => Promise<T>): Promise<T> {
    ...
    try {
        ...
        else if (admission.active === 0 &&
            memorySearchBuildAdmissions.get(runtime) === admission) {
            memorySearchBuildAdmissions.delete(runtime);
        }
    }
}
async function buildMemorySearchCorpus(runtime: AgentRuntime, roomId: UUID): Promise<MemorySearchCorpusBuild> {
    ...
    const countAfter = countBefore === null ? null : await countRoomMessages(runtime, roomId);
    const candidates: MemorySearchCandidate[] = [];
    const textEncoder = new TextEncoder();
    let retainedTextBytes = 0;
    for (const memory of memories) {
        const text = (memory.content as {
            text?: string;
        } | undefined)?.text?.trim();
        if (!text)
            continue;
        const source = (memory.content as {
            source?: string;
        } | undefined)?.source;
        if (source !== HASH_MEMORY_SOURCE)
            continue;
        if (!memory.id || typeof memory.createdAt !== "number")
            continue;
    ...
    const countsMatch = countBefore !== null && countAfter !== null && countBefore === countAfter;
    return {
        corpus: {
            candidates,
            bm25: new BM25(candidates.map((candidate) => ({ content: candidate.text })), { stemming: true }),
            rowCount: countAfter ?? memories.length,
            builtAt: Date.now(),
        },
        disposition: countBefore !== null && countAfter !== null && !countsMatch
            ? "obsolete"
...
function startMemorySearchCorpusBuild(runtime: AgentRuntime, roomId: UUID, slot: MemorySearchCacheSlot): Promise<MemorySearchCorpusBuild> {
    if (slot.inFlight)
        return slot.inFlight;
    const generation = slot.generation;
    const build = withMemorySearchBuildPermit(runtime, async () => buildMemorySearchCorpus(runtime, roomId)).then((result) => {
...
async function awaitCurrentMemorySearchCorpusBuild(runtime: AgentRuntime, roomId: UUID, cache: Map<string, MemorySearchCacheSlot>, slot: MemorySearchCacheSlot, retryBudget: MemorySearchBuildRetryBudget): Promise<MemorySearchCorpus> {
    while (true) {
        const generation = slot.generation;
        ...
        // in-flight request.
        if (memorySearchCorpusCaches.get(runtime) !== cache ||
        ...
        if (result.disposition !== "obsolete")
...
function consumeMemorySearchRetryBudget(retryBudget: MemorySearchBuildRetryBudget, roomId: UUID, reason: "count_mismatch" | "generation" | "map_identity", context: {
    countBefore?: number | null;
    countAfter?: number | null;
} = {}): void {
    retryBudget.unstableAttempts++;
    if (retryBudget.unstableAttempts < MEMORY_SEARCH_MAX_UNSTABLE_BUILD_ATTEMPTS) {
        return;
    }
    throw new ElizaError("Memory search corpus could not obtain a stable room snapshot", {
...
async function getMemorySearchCorpus(runtime: AgentRuntime, roomId: UUID, retryBudget: MemorySearchBuildRetryBudget = { unstableAttempts: 0 }): Promise<MemorySearchCorpus> {
    ...
    try {
        ...
        if (cached) {
            ...
            if (slot.corpus !== cached) {
                consumeMemorySearchRetryBudget(retryBudget, roomId, "generation");
                return await awaitCurrentMemorySearchCorpusBuild(runtime, roomId, cache, slot, retryBudget);
            }
            if (rowCount !== null && rowCount === cached.rowCount) {
                const age = Date.now() - cached.builtAt;
                if (age <= MEMORY_SEARCH_CACHE_TTL_MS)
                    return cached;
...
async function searchMemoryNotes(runtime: AgentRuntime, roomId: UUID, query: string, limit: number): Promise<MemorySearchHit[]> {
    const corpus = await getMemorySearchCorpus(runtime, roomId);
    const hits: MemorySearchHit[] = rankWithIndex(query, corpus.candidates, corpus.bm25)
...
async function searchDocuments(runtime: AgentRuntime, query: string, limit: number): Promise<DocumentSearchHit[]> {
    const documents: DocumentsServiceResult = await getDocumentsService(runtime);
    const documentsService = documents.service;
    if (!documentsService || !runtime.agentId)
        return [];
    ...
    const matches: DocumentSearchMatch[] = await documentsService.searchDocuments(searchMessage, {
        roomId: agentId,
    });
    return matches
        .filter((match) => (match.similarity ?? 0) >= QUICK_CONTEXT_DOCUMENTS_THRESHOLD)
        .slice(0, limit)
        .map((match) => {
        ...
        return {
            ...
            documentId: typeof metadata?.documentId === "string"
                ? metadata.documentId
                : undefined,
            documentTitle: typeof metadata?.filename === "string"
                ? metadata.filename
                : typeof metadata?.title === "string"
                    ? metadata.title
                    : undefined,
            position: typeof metadata?.position === "number"
                ? metadata.position
                : undefined,
        };
    });
...
async function fetchMemoriesFromTables(runtime: AgentRuntime, params: {
    entityIds?: UUID[];
    roomId?: UUID;
    ...
    const maxScannedRowsPerTable = Math.max(1, Math.floor(MEMORY_BROWSE_MAX_SCAN_ROWS / Math.max(tables.length, 1)));
    ...
    const tableResults = await Promise.all(tables.map(async (tableName) => {
        const eligible: TaggedMemory[] = [];
        let cursor: {
            createdAt: number;
            id: UUID;
        } | undefined = params.before !== undefined && params.beforeId !== undefined
            ? { createdAt: params.before, id: params.beforeId }
            : undefined;
        let batchSize = 200;
        let scannedRows = 0;
        for (;;) {
            const queryLimit = Math.min(batchSize, maxScannedRowsPerTable - scannedRows);
            if (queryLimit <= 0) {
                throw new ElizaError("Memory browse exceeded its bounded scan budget before proving the page", {
            ...
            for (const memory of memories) {
                if (!memory.id) {
                    throw new ElizaError("A paged memory row did not contain the required id", {
                ...
                if (nextCursor &&
                    ...
                    throw new ElizaError("Memory adapter did not advance the requested keyset cursor", {
            ...
            for (const memory of memories) {
                ...
                if (searchQuery &&
                    !matchesKeyword((tagged.content as {
                        text?: string;
                    } | undefined)?.text ?? "", searchQuery)) {
                    continue;
                }
...
export async function handleMemoryRoutes(ctx: MemoryRouteContext): Promise<boolean> {
    const { req, res, method, pathname, url, runtime, agentName, json, error, readJsonBody, } = ctx;
    ...
    const { roomId, entityId } = await ensureMemoryConnection(runtime, resolvedAgentName);
    if (method === "POST" && pathname === "/api/memory/remember") {
        ...
        const memoryId = parsedRem.data.idempotencyKey
            ? (stringToUuid(`${HASH_MEMORY_SOURCE}:${runtime.agentId}:${parsedRem.data.idempotencyKey}`) as UUID)
    ...
    if (method === "GET" && pathname === "/api/memory/search") {
        ...
        const requestedLimit = parsePositiveInteger(url.searchParams.get("limit"), MEMORY_SEARCH_DEFAULT_LIMIT);
        const limit = Math.min(Math.max(requestedLimit, 1), MEMORY_SEARCH_MAX_LIMIT);
    ...
    if (method === "GET" && pathname === "/api/context/quick") {
        ...
        const requestedLimit = parsePositiveInteger(url.searchParams.get("limit"), QUICK_CONTEXT_DEFAULT_LIMIT);
        const limit = Math.min(Math.max(requestedLimit, 1), QUICK_CONTEXT_MAX_LIMIT);
    ...
    if (method === "GET" && pathname === "/api/memories/feed") {
        const requestedLimit = parsePositiveInteger(url.searchParams.get("limit"), MEMORY_FEED_DEFAULT_LIMIT);
    ...
    if (method === "GET" && pathname === "/api/memories/browse") {
        const requestedLimit = parsePositiveInteger(url.searchParams.get("limit"), MEMORY_BROWSE_DEFAULT_LIMIT);
    ...
    if (method === "GET" && pathname.startsWith("/api/memories/by-entity/")) {
        const primaryEntityId = decodePathComponent(pathname.slice("/api/memories/by-entity/".length), res, "entity identifier");
    ...
    if (memoryIdMatch && (method === "DELETE" || method === "PATCH")) {
        ...
        return runtime.roomHandlerQueue.withLease(existing.roomId, async () => {
            ...
            const existingContent = (current.content as Record<string, unknown> | undefined) ?? {};
            ...
            json(res, {
                ...
                memory: projectCompleteToolValueForModel(updated, composeToolDiagnosticRedactor(runtime)),
            });
            return true;
        });
    }
