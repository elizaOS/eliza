...
import { isPathWithinRoot } from "./realpath-confinement.ts";
import { isTruthyEnvValue } from "@elizaos/core";
import { logger } from "@elizaos/core";
import { resolveApiToken } from "@elizaos/core/runtime-env";
import { resolveRealPathSync } from "./realpath-confinement.ts";
...
// Response helpers
// ---------------------------------------------------------------------------
export function sendStaticResponse(req: http.IncomingMessage, res: http.ServerResponse, status: number, headers: Record<string, string | number>, body?: Buffer): void {
...
const staticFileCache = new Map<string, {
    body: Buffer;
    mtimeMs: number;
}>();
function getCachedFile(filePath: string, mtimeMs: number): Buffer {
    return getOrReadCachedFile(staticFileCache, filePath, mtimeMs, (p) => fs.readFileSync(p), STATIC_CACHE_MAX, STATIC_CACHE_FILE_LIMIT);
}
// ---------------------------------------------------------------------------
 ...
 */
export function injectApiBaseIntoHtml(html: Buffer, externalBase?: string | null, opts?: {
    ...
    if (!trimmedBase && !trimmedToken && !trimmedVapid)
        return html;
    ...
    if (trimmedBase)
        bootOverrides.apiBase = trimmedBase;
    ...
    if (trimmedVapid)
        bootOverrides.webPushVapidPublicKey = trimmedVapid;
    if (Object.keys(bootOverrides).length > 0) {
        ...
        parts.push(`(function(){var k=Symbol.for("elizaos.app.boot-config"),w=window,prev=w.__ELIZAOS_APP_BOOT_CONFIG__||(w[k]&&w[k].current)||{},next=Object.assign({},prev,${serializeInlineScriptValue(bootOverrides)});w.__ELIZAOS_APP_BOOT_CONFIG__=next;w[k]={current:next};})();`);
    }
    if (trimmedToken) {
...
export function resolveInjectedDashboardToken(): string | null {
    ...
    if (cloudProvisioned || !forceInjectToken)
        return null;
    if (!warnedForceInjectToken) {
        warnedForceInjectToken = true;
        logger.warn("[static-file-server] ELIZA_FORCE_INJECT_TOKEN is set — embedding the API token in served dashboard HTML. Ensure the dashboard is fronted by your own auth gate; do not enable this on a directly exposed agent port.");
    }
...
export function buildServedDashboardHtml(html: Buffer): Buffer {
    ...
    const webPushVapidPublicKey = process.env.ELIZA_WEB_PUSH_VAPID_PUBLIC_KEY?.trim() || null;
    const injectOpts = injectedToken || webPushVapidPublicKey
        ? {
            ...(injectedToken ? { apiToken: injectedToken } : {}),
            ...(webPushVapidPublicKey ? { webPushVapidPublicKey } : {}),
        }
        : undefined;
    return injectApiBaseIntoHtml(html, process.env.ELIZA_EXTERNAL_BASE_URL, injectOpts);
}
// ---------------------------------------------------------------------------
// SPA serving
// ---------------------------------------------------------------------------
export function serveStaticUi(req: http.IncomingMessage, res: http.ServerResponse, pathname: string): boolean {
    ...
    try {
        ...
        if (stat.isFile()) {
            ...
            const isPreviewOrBinaryAsset = relativePath.startsWith("vrms/previews/") ||
