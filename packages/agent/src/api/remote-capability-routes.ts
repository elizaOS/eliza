...
import { CAPABILITY_ROUTER_SERVICE_TYPE } from "@elizaos/core";
import { CapabilityError } from "@elizaos/core";
import { connectCloudCapabilitySandbox } from "../services/remote-capability-cloud-sandbox.ts";
import { connectRemoteCapabilityEndpointProvider } from "../services/remote-capability-endpoint-provider.ts";
import { decodeUrlPathComponent } from "@elizaos/core/utils/path-component";
import { desktopCompanionCapabilityEndpointProvider } from "../services/remote-capability-url-endpoint-providers.ts";
import { detectClientPlatform } from "./platform-detect.ts";
import { directRemoteCapabilityEndpointProvider } from "../services/remote-capability-endpoint-provider.ts";
import { homeMachineCapabilityEndpointProvider } from "../services/remote-capability-url-endpoint-providers.ts";
import { isDynamicLoadingAllowed } from "./platform-detect.ts";
import { isLoopbackHost } from "@elizaos/core";
import { isPrivateIpAddress } from "@elizaos/core";
import { mobileCompanionCapabilityEndpointProvider } from "../services/remote-capability-url-endpoint-providers.ts";
import { normalizeEndpointTrustPolicyOptions } from "../services/remote-capability-endpoint-provider.ts";
import { normalizeHostLike } from "@elizaos/core";
import { type ConnectCloudCapabilitySandboxOptions } from "../services/remote-capability-cloud-sandbox.ts";
import { type ConnectCloudCapabilitySandboxResult } from "../services/remote-capability-cloud-sandbox.ts";
import { type ConnectRemoteCapabilityEndpointProviderOptions } from "../services/remote-capability-endpoint-provider.ts";
import { type ConnectRemoteCapabilityEndpointProviderResult } from "../services/remote-capability-endpoint-provider.ts";
import { type ElizaCapabilityRouter } from "@elizaos/core";
...
import { type RemoteCapabilityEndpointProvider } from "../services/remote-capability-endpoint-provider.ts";
import { type RemoteCapabilityEndpointTrustPolicyOptions } from "../services/remote-capability-endpoint-provider.ts";
import { type RemotePluginSyncResult } from "../services/remote-plugin-adapter.ts";
import { type RemotePluginTrustPolicy } from "../services/remote-plugin-adapter.ts";
...
import { type UrlRemoteCapabilityEndpointProviderOptions } from "../services/remote-capability-url-endpoint-providers.ts";
type JsonBodyReader = <T = Record<string, unknown>>(req: http.IncomingMessage, res: http.ServerResponse, options?: {
    requireObject?: boolean;
}) => Promise<T | null>;
export interface RemoteCapabilityRouteContext extends RouteRequestMeta, Pick<RouteHelpers, "json" | "error"> {
    ...
    connectEndpointProvider?: <TOptions>(runtime: IAgentRuntime, options: ConnectRemoteCapabilityEndpointProviderOptions<TOptions>) => Promise<ConnectRemoteCapabilityEndpointProviderResult>;
    connectCloudSandbox?: (runtime: IAgentRuntime, options: ConnectCloudCapabilitySandboxOptions) => Promise<ConnectCloudCapabilitySandboxResult>;
}
type ConnectBody = {
    endpoint?: unknown;
    cloud?: unknown;
...
type EndpointProviderMode = "direct" | "home-machine" | "mobile-companion" | "desktop-companion";
...
type EndpointProviderOptions = DirectEndpointProviderOptions | UrlRemoteCapabilityEndpointProviderOptions;
type CloudBody = {
...
export async function handleRemoteCapabilityRoutes(ctx: RemoteCapabilityRouteContext): Promise<boolean> {
    const { req, res, method, pathname, runtime, readJsonBody, json, error } = ctx;
    if (pathname.startsWith("/api/capability-router/assets/")) {
        ...
        if (!isDynamicLoadingAllowed(detectClientPlatform(req))) {
            error(res, "Dynamic capability asset loading is not permitted on this platform.", 403);
            return true;
        }
        try {
            ...
            error(res, err instanceof Error ? err.message : "Failed to load remote asset.", err instanceof CapabilityError ? 502 : 400);
        }
        return true;
    }
    ...
    try {
        const unloadMissing = typeof body.unloadMissing === "boolean" ? body.unloadMissing : true;
        const persist = typeof body.persist === "boolean" ? body.persist : true;
        const allowedModuleIds = parseOptionalStringArray(body.allowedModuleIds, "allowedModuleIds");
        const trustPolicy = parseOptionalEndpointTrustPolicy(body.trustPolicy, "trustPolicy");
        const requestTimeoutMs = optionalPositiveInteger(body.requestTimeoutMs, "requestTimeoutMs");
        ...
        if (body.endpoint !== undefined) {
            ...
            const connectEndpointProvider = ctx.connectEndpointProvider ?? connectRemoteCapabilityEndpointProvider;
            const provider = getEndpointProvider(providerMode);
            const result = await connectEndpointProvider(runtime, {
                provider,
                provisionOptions: buildEndpointProvisionOptions(providerMode, endpoint, allowedModuleIds),
                unloadMissing,
        ...
        if (body.cloud !== undefined) {
            ...
            if (trustPolicy !== undefined && cloud.trustPolicy !== undefined) {
                error(res, "Cloud requests must set trustPolicy either at the top level or inside 'cloud', not both.", 400);
                return true;
            }
            if (allowedModuleIds !== undefined &&
                cloud.allowedModuleIds !== undefined) {
                error(res, "Cloud requests must set allowedModuleIds either at the top level or inside 'cloud', not both.", 400);
                return true;
            }
            const cloudAllowedModuleIds = allowedModuleIds ?? cloud.allowedModuleIds;
            const cloudTrustPolicy = trustPolicy ?? cloud.trustPolicy;
            const connectCloudSandbox = ctx.connectCloudSandbox ?? connectCloudCapabilitySandbox;
        ...
        error(res, err instanceof Error
            ? err.message
            : "Failed to connect capability router endpoint.", 400);
        return true;
    }
}
async function serveCapabilityRouterAssetProxy(ctx: RemoteCapabilityRouteContext, runtime: IAgentRuntime): Promise<void> {
...
function parseAssetProxyPath(pathname: string): {
    ...
    const [encodedEndpointId, encodedModuleId, ...assetParts] = remainder.split("/");
    if (!encodedEndpointId || !encodedModuleId || assetParts.length === 0) {
        throw new Error("Capability asset URL must include endpoint id, module id, and path.");
    }
    ...
    const assetSegments = assetParts.map((part) => decodeAssetComponent(part, "asset path"));
    const hasUnsafeSegment = assetSegments.some((part) => !part || part === "." || part === ".." || part.includes("\\"));
...
function getRuntimeCapabilityRouter(runtime: IAgentRuntime): ElizaCapabilityRouter {
    const router = runtime.getService?.(CAPABILITY_ROUTER_SERVICE_TYPE) as ElizaCapabilityRouter | null;
    if (!router) {
...
function persistEndpoint(ctx: Pick<RemoteCapabilityRouteContext, "config" | "saveConfig" | "persistConfigEnv">, endpoint: RemoteCapabilityEndpointConfig, allowedModuleIds?: string[], trustPolicy?: RemoteCapabilityEndpointTrustPolicyOptions, audit?: CapabilityRouterTrustAuditInput): Promise<void> {
    const config = ctx.config;
    const saveConfig = ctx.saveConfig;
    const persistConfigEnv = ctx.persistConfigEnv;
    if (!config || !saveConfig || !persistConfigEnv) {
        throw new Error("Capability router endpoint persistence is unavailable in this runtime.");
    }
    return persistEndpointInner({ config, saveConfig, persistConfigEnv }, endpoint, allowedModuleIds, trustPolicy, audit);
}
async function persistEndpointInner(ctx: {
    config: CapabilityRouterPersistConfig;
    saveConfig: (config: CapabilityRouterPersistConfig) => void;
    persistConfigEnv: (key: string, value: string) => Promise<void>;
}, endpoint: RemoteCapabilityEndpointConfig, allowedModuleIds?: string[], trustPolicy?: RemoteCapabilityEndpointTrustPolicyOptions, audit?: CapabilityRouterTrustAuditInput): Promise<void> {
    const env = ctx.config.env ?? {};
    const vars = { ...(env.vars ?? {}) };
    const endpoints = mergePersistedEndpoints(readPersistedEndpoints(process.env.ELIZA_CAPABILITY_ROUTER_URLS ??
        vars.ELIZA_CAPABILITY_ROUTER_URLS), endpoint);
    const moduleAllowlists = mergePersistedModuleAllowlists(readPersistedModuleAllowlists(process.env.ELIZA_CAPABILITY_ROUTER_ALLOWED_MODULES ??
        vars.ELIZA_CAPABILITY_ROUTER_ALLOWED_MODULES), endpoint.id, allowedModuleIds);
    const persistedTrustPolicy = mergePersistedTrustPolicies(readPersistedTrustPolicies(process.env.ELIZA_CAPABILITY_ROUTER_TRUST_POLICY ??
        vars.ELIZA_CAPABILITY_ROUTER_TRUST_POLICY), endpoint.id, trustPolicy);
    const sanitizedEndpoints = endpoints.map(({ token: _token, ...item }) => item);
    await ctx.persistConfigEnv("ELIZA_CAPABILITY_ROUTER_ENABLED", "true");
    await ctx.persistConfigEnv("ELIZA_CAPABILITY_ROUTER_URLS", JSON.stringify(endpoints));
    ...
    if (Object.keys(persistedTrustPolicy).length > 0) {
        await ctx.persistConfigEnv("ELIZA_CAPABILITY_ROUTER_TRUST_POLICY", JSON.stringify(persistedTrustPolicy));
    ...
    if (audit !== undefined) {
        vars.ELIZA_CAPABILITY_ROUTER_TRUST_AUDIT = JSON.stringify(appendTrustAuditRecord(readTrustAuditRecords(process.env.ELIZA_CAPABILITY_ROUTER_TRUST_AUDIT ??
            vars.ELIZA_CAPABILITY_ROUTER_TRUST_AUDIT), audit));
    }
    ctx.config.env = {
        ...env,
        vars,
...
function readTrustAuditRecords(value: string | undefined): CapabilityRouterTrustAuditRecord[] {
    if (!value?.trim())
        return [];
    try {
...
function appendTrustAuditRecord(existing: CapabilityRouterTrustAuditRecord[], audit: CapabilityRouterTrustAuditInput): CapabilityRouterTrustAuditRecord[] {
    return [
        ...existing,
        {
...
function isTrustAuditRecord(value: unknown): value is CapabilityRouterTrustAuditRecord {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
        return false;
    }
    const record = value as Record<string, unknown>;
    return (typeof record.recordedAt === "string" &&
...
function readPersistedModuleAllowlists(value: string | undefined): Record<string, string[]> {
    if (!value?.trim())
        return {};
    try {
        const parsed = JSON.parse(value) as unknown;
        ...
        for (const [endpointId, moduleIds] of Object.entries(parsed)) {
            if (!endpointId.trim() || !Array.isArray(moduleIds))
                continue;
...
function mergePersistedModuleAllowlists(existing: Record<string, string[]>, endpointId: string, allowedModuleIds: string[] | undefined): Record<string, string[]> {
    const next = { ...existing };
    if (allowedModuleIds === undefined) {
...
function readPersistedTrustPolicies(value: string | undefined): Record<string, RemoteCapabilityEndpointTrustPolicyOptions> {
    if (!value?.trim())
        return {};
    try {
        const parsed = JSON.parse(value) as unknown;
        ...
        for (const [endpointId, candidate] of Object.entries(parsed)) {
            if (!endpointId.trim())
                continue;
            const trustPolicy = parseOptionalEndpointTrustPolicy(candidate, `ELIZA_CAPABILITY_ROUTER_TRUST_POLICY.${endpointId}`);
...
function mergePersistedTrustPolicies(existing: Record<string, RemoteCapabilityEndpointTrustPolicyOptions>, endpointId: string, trustPolicy: RemoteCapabilityEndpointTrustPolicyOptions | undefined): Record<string, RemoteCapabilityEndpointTrustPolicyOptions> {
    const next = { ...existing };
...
function readPersistedEndpoints(value: string | undefined): RemoteCapabilityEndpointConfig[] {
    if (!value?.trim())
        return [];
    try {
        const parsed = JSON.parse(value) as unknown;
        ...
        return parsed
            ...
            .filter((endpoint): endpoint is RemoteCapabilityEndpointConfig => endpoint !== null);
    }
    catch {
        return [];
    }
}
function mergePersistedEndpoints(existing: RemoteCapabilityEndpointConfig[], next: RemoteCapabilityEndpointConfig): RemoteCapabilityEndpointConfig[] {
...
function getEndpointProvider(providerMode: EndpointProviderMode): RemoteCapabilityEndpointProvider<EndpointProviderOptions> {
    switch (providerMode) {
        case "direct":
...
function buildEndpointProvisionOptions(providerMode: EndpointProviderMode, endpoint: RemoteCapabilityEndpointConfig, allowedModuleIds: string[] | undefined): EndpointProviderOptions {
...
function parseCloudOptions(value: unknown): Omit<ConnectCloudCapabilitySandboxOptions, "unloadMissing" | "requestTimeoutMs" | "fetch" | "onProgress"> {
    const body = requireObject(value, "cloud") as CloudBody;
    const bio = parseOptionalStringArray(body.bio, "cloud.bio");
    const allowedModuleIds = parseOptionalStringArray(body.allowedModuleIds, "cloud.allowedModuleIds");
    const trustPolicy = parseOptionalEndpointTrustPolicy(body.trustPolicy, "cloud.trustPolicy");
    const endpointId = optionalNonEmptyString(body.endpointId, "cloud.endpointId");
    const timeoutMs = optionalPositiveInteger(body.timeoutMs, "cloud.timeoutMs");
    if (timeoutMs instanceof Error)
        throw timeoutMs;
    const pollIntervalMs = optionalPositiveInteger(body.pollIntervalMs, "cloud.pollIntervalMs");
    if (pollIntervalMs instanceof Error)
        throw pollIntervalMs;
    return {
...
function optionalPositiveInteger(value: unknown, field: string): number | undefined | Error {
    if (value === undefined)
        return undefined;
...
function parseOptionalEndpointTrustPolicy(value: unknown, field: string): RemoteCapabilityEndpointTrustPolicyOptions | undefined {
    if (value === undefined)
        return undefined;
    const body = requireObject(value, field);
    const allowedProvenanceIssuers = parseOptionalStringArray(body.allowedProvenanceIssuers, `${field}.allowedProvenanceIssuers`);
    const trustedProvenancePublicKeys = parseOptionalStringRecord(body.trustedProvenancePublicKeys, `${field}.trustedProvenancePublicKeys`);
    const trustPolicy = normalizeEndpointTrustPolicyOptions({
        ...
        ...optionalBooleanField(body.requireSignedProvenance, `${field}.requireSignedProvenance`, "requireSignedProvenance"),
        ...optionalBooleanField(body.requireVerifiedProvenance, `${field}.requireVerifiedProvenance`, "requireVerifiedProvenance"),
        ...optionalBooleanField(body.requireProvenanceDigestMatch, `${field}.requireProvenanceDigestMatch`, "requireProvenanceDigestMatch"),
    });
    return Object.keys(trustPolicy).length === 0 ? undefined : trustPolicy;
}
function optionalBooleanField<TKey extends keyof RemotePluginTrustPolicy>(value: unknown, field: string, key: TKey): Partial<Pick<RemotePluginTrustPolicy, TKey>> {
    if (value === undefined)
        return {};
