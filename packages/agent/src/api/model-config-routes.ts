 ...
 */
import { CODING_MODEL_DEFAULTS } from "./model-catalog.ts";
...
import { isCerebrasMode } from "@elizaos/plugin-openai";
...
import { lastServingTextProvider } from "./agent-model.ts";
import { logger } from "@elizaos/core";
import { resolveDevCloudEnvAuthority } from "../config/dev-cloud-env-authority.ts";
...
export interface ModelConfigRouteContext extends RouteRequestMeta, Pick<RouteHelpers, "json" | "readJsonBody"> {
    state: {
        config: ElizaConfig;
 ...
 */
function resolveAnthropicBaseURL(readSetting: (key: string) => string | undefined, options: {
    mockBaseURL?: string;
} = {}): string {
    const mockBaseURL = options.mockBaseURL?.trim();
    if (mockBaseURL)
        return mockBaseURL;
    return (readSetting("ANTHROPIC_BASE_URL")?.trim() || DEFAULT_ANTHROPIC_BASE_URL);
}
...
function invalid(message: string, context: Record<string, unknown>): ElizaError {
...
function findEntry(catalog: ModelCatalog, provider: string, model: string): ModelCatalogEntry | undefined {
...
function validateEffort(entry: ModelCatalogEntry, effort: string): void {
    ...
    if (!entry.efforts.includes(effort)) {
        throw invalid(`Effort "${effort}" is not supported by model "${entry.id}" (supported: ${entry.efforts.join(", ")})`, { model: entry.id, effort, supported: entry.efforts });
    }
}
...
function readConfigEnvString(config: ElizaConfig, key: string): {
    value: string;
 ...
 */
function writeModelEnvKey(config: ElizaConfig, processEnv: NodeJS.ProcessEnv, key: string, value: string): boolean {
    ...
    return (typeof priorProcess === "string" &&
        priorProcess !== value &&
...
function resolveChatWrites(catalog: ModelCatalog, body: ModelConfigWriteBody, activeProvider?: string): ResolvedWrite[] {
    if (body.backend !== undefined) {
        throw invalid(`backend is a coding-target field; target "${body.target}" selects a chat provider`, { target: body.target, backend: body.backend });
    }
    ...
    if (provider !== undefined && !(provider in CHAT_PROVIDER_KEY_FAMILY)) {
        throw invalid(`Unknown chat provider "${provider}" (expected one of: ${Object.keys(CHAT_PROVIDER_KEY_FAMILY).join(", ")})`, { provider });
    }
    if (provider === undefined) {
        const matches = Object.keys(CHAT_PROVIDER_KEY_FAMILY).filter((candidate) => findEntry(catalog, candidate, model)?.roles.includes(body.target as "small" | "large"));
        if (matches.length === 0) {
        ...
        if (matches.length > 1) {
            ...
            if (activeProvider !== undefined && matches.includes(activeProvider)) {
                ...
                throw invalid(`Model "${model}" is served by multiple providers (${matches.join(", ")}); specify provider`, { model, providers: matches });
            }
        }
        else {
    ...
    if (!entry.roles.includes(body.target as "small" | "large")) {
        throw invalid(`Model "${model}" is not offered for the "${body.target}" role on provider "${provider}"`, { model, provider, target: body.target, roles: entry.roles });
    }
...
function resolveCodingWrites(catalog: ModelCatalog, body: ModelConfigWriteBody): ResolvedWrite[] {
    ...
    if (body.model === undefined) {
        if (body.backend !== undefined ||
            ...
            throw invalid("a defaultBackend-only write must not carry backend, provider, or effort", {
    ...
    if (backend === undefined || !CODING_BACKENDS.has(backend)) {
        throw invalid(`target "coding" requires backend (one of: ${[...CODING_BACKENDS].join(", ")})`, { backend: backend ?? null });
    }
    const seam = CODING_BACKEND_SEAMS[backend];
    if (body.provider !== undefined && body.provider !== seam.catalogProvider) {
        throw invalid(`provider "${body.provider}" does not match coding backend "${backend}"`, { provider: body.provider, backend });
    }
    if (seam.catalogProvider) {
        ...
        if (body.effort !== undefined) {
            ...
            if (backend === "codex" && !MANAGED_CODEX_ACP_EFFORTS.has(body.effort)) {
                throw invalid(`Effort "${body.effort}" is valid for ${entry.id} but not supported by the managed codex-acp contract (supported: ${[...MANAGED_CODEX_ACP_EFFORTS].join(", ")})`, {
                    model: entry.id,
                    effort: body.effort,
...
function resolveDefaultBackendWrites(body: ModelConfigWriteBody): ResolvedWrite[] {
...
function parseWriteBody(raw: Record<string, unknown>): ModelConfigWriteBody {
    ...
    const defaultBackendOnly = target === "coding" &&
        model === undefined &&
    ...
    if (backend !== undefined && !CODING_BACKENDS.has(backend as CodingBackend)) {
        throw invalid(`Unknown backend "${backend}" (expected one of: ${[...CODING_BACKENDS].join(", ")})`, { backend });
    }
    ...
    if (defaultBackend !== undefined &&
        ...
        throw invalid(`Unknown defaultBackend "${defaultBackend}" (expected one of: ${[...CODING_BACKENDS].join(", ")})`, { defaultBackend });
    }
    return {
...
function resolveEffective(config: ElizaConfig, processEnv: NodeJS.ProcessEnv, key: string): EffectiveValue {
...
function hasTextHandlerRegistered(runtime: AgentRuntime, provider: string): boolean {
    try {
        return (runtime.getModelRegistrations?.() ?? []).some((entry) => (entry.modelType === ModelType.TEXT_SMALL ||
...
const DIRECT_CHAT_SERVING_REQUIREMENTS: Readonly<Record<string, {
...
function isConfiguredCerebrasMode(runtime: AgentRuntime, processEnv: NodeJS.ProcessEnv): boolean {
    try {
...
function hasDirectProviderServingEvidence(provider: string, processEnv: NodeJS.ProcessEnv, runtime: AgentRuntime): boolean {
    ...
    for (const credentialKey of requirement.credentialKeys) {
        ...
        if (isUsableProviderCredential(runtimeCredential))
            return true;
        if (runtimeCredential !== null && runtimeCredential !== undefined) {
            if (String(runtimeCredential).trim().length > 0)
                return false;
            continue;
        }
        const environmentCredential = processEnv[credentialKey];
        if (isUsableProviderCredential(environmentCredential))
            return true;
...
export function resolveActiveChat(config: ElizaConfig, processEnv: NodeJS.ProcessEnv, runtime?: AgentRuntime | null): ActiveChatInfo | null {
    const routing = resolveServiceRoutingInConfig(config as Record<string, unknown>);
    const llmText = routing?.llmText;
    const backend = typeof llmText?.backend === "string" ? llmText.backend : undefined;
    if (llmText?.transport === "direct" && backend === "openai-subscription") {
        ...
        return {
            ...
            family: "OPENAI",
            endpoint: hostOf(typeof runtimeBase === "string"
                ? runtimeBase
                : resolveEffective(config, processEnv, "CODEX_BASE_URL")?.value) ?? "chatgpt.com",
        };
    }
    const cloudProxyConfigured = llmText?.transport === "cloud-proxy" && backend === "elizacloud";
    ...
    if (cloudProxyConfigured) {
        ...
        const explicitProvider = resolveEffective(config, processEnv, "ELIZA_PROVIDER")?.value.toLowerCase();
        if (explicitProvider === "cerebras" &&
            runtime &&
            hasDirectProviderServingEvidence("cerebras", processEnv, runtime)) {
            provider = "cerebras";
        }
    }
    const family = provider === "openai"
        ? "OPENAI"
        : provider !== undefined
            ? CHAT_PROVIDER_KEY_FAMILY[provider]
            : undefined;
    if (provider === undefined || family === undefined)
        return null;
    const readSetting = (key: string): string | undefined => resolveEffective(config, processEnv, key)?.value;
    const baseURL = family === "ELIZAOS_CLOUD"
        ? resolveElizaCloudBaseURL(readSetting)
        : family === "ANTHROPIC"
            ? resolveAnthropicBaseURL(readSetting, {
                mockBaseURL: processEnv.ELIZA_MOCK_ANTHROPIC_BASE,
            })
            : resolveOpenAIBaseURL((key) => key === "ELIZA_PROVIDER" && provider === "cerebras"
                ? "cerebras"
                : readSetting(key), { mockBaseURL: processEnv.ELIZA_MOCK_OPENAI_BASE });
    const endpoint = hostOf(baseURL) ??
        new URL(family === "ELIZAOS_CLOUD"
            ? "https://api.eliza.app/api/v1"
            : family === "ANTHROPIC"
                ? "https://api.anthropic.com/v1"
                : provider === "cerebras"
                    ? "https://api.cerebras.ai/v1"
                    : "https://api.openai.com/v1").hostname;
    return { provider, family, endpoint };
}
function buildEffectiveConfig(config: ElizaConfig, processEnv: NodeJS.ProcessEnv, activeChat: ActiveChatInfo | null): Record<string, Record<string, EffectiveValue>> {
    const resolve = (key: string): EffectiveValue => resolveEffective(config, processEnv, key);
    ...
    return {
        ...
        coding: {
            ...
            ELIZA_CODEX_MODEL_POWERFUL: resolve("ELIZA_CODEX_MODEL_POWERFUL") ??
 ...
 * handled.
 */
export async function handleModelConfigRoutes(ctx: ModelConfigRouteContext): Promise<boolean> {
    ...
    if (method === "GET") {
        const activeChat = resolveActiveChat(state.config, processEnv, state.runtime ?? null);
        json(res, {
    ...
    try {
        ...
        if (body.target === "coding") {
            ...
            for (const write of writes) {
                if (writeModelEnvKey(state.config, processEnv, write.key, write.value)) {
                    conflicts.push(write.key);
                }
            }
            ctx.saveElizaConfig(state.config);
            logger.info(`[ModelConfigRoutes] coding model config applied: ${writes.map((w) => `${w.key}=${w.value}`).join(" ")}`);
        ...
        const writes = resolveChatWrites(catalog, body, resolveActiveChat(state.config, processEnv, state.runtime ?? null)
            ?.provider);
        if (devCloudAuthority &&
            writes.some((write) => isDevCloudEnvOwnedKey(write.key))) {
            json(res, {
                error: "Cloud model selection is owned by the immutable local dev launch target; restart with the intended Cloud model configuration.",
            }, 409);
            return true;
        }
        const conflicts: string[] = [];
        ...
        logger.info(`[ModelConfigRoutes] ${body.target} model config applied: ${writes.map((w) => `${w.key}=${w.value}`).join(" ")} op=${outcome.operation.id}`);
        json(res, {
        ...
        if (err instanceof ElizaError) {
            json(res, { error: err.message, code: err.code, context: err.context }, 400);
            return true;
        }
        logger.error(`[ModelConfigRoutes] model config write failed: ${err instanceof Error ? err.stack : String(err)}`);
