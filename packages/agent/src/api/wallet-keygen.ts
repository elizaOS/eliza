...
import { assertSolanaBase58CharBudget } from "./solana-secret-budget.ts";
...
import { type WalletGenerateResult } from "@elizaos/core/contracts/wallet-types";
import { type WalletKeys } from "@elizaos/core/contracts/wallet-types";
...
export function decodeSolanaBase58(str: string): Buffer {
    ...
    for (const c of str) {
        ...
        if (i === -1)
            throw new Error(`Invalid base58: ${c}`);
...
const PLACEHOLDER_RE = /^\[?\s*(REDACTED|PLACEHOLDER|T(?:O)D(?:O)|CHANGEME|EMPTY)\s*]?$/i;
...
export function syncSolanaPublicKeyEnv(privateKey = process.env.SOLANA_PRIVATE_KEY): string | null {
    const trimmed = privateKey?.trim();
