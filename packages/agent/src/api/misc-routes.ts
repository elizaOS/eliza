...
import { MAX_TERMINAL_CAPTURE_BYTES } from "./terminal-output-contract.ts";
import { ModelType } from "@elizaos/core";
import { PostAgentEventRequestSchema } from "@elizaos/shared";
import { PostCustomActionGenerateRequestSchema } from "@elizaos/shared";
import { PostCustomActionRequestSchema } from "@elizaos/shared";
import { PostCustomActionTestRequestSchema } from "@elizaos/shared";
import { PostIngestShareRequestSchema } from "@elizaos/shared";
import { PostTerminalRunRequestSchema } from "@elizaos/shared";
import { PutCustomActionRequestSchema } from "@elizaos/shared";
...
import { capturedTerminalOutputIsSafe } from "./terminal-output-contract.ts";
...
import { registerCustomActionLive } from "../runtime/custom-actions.ts";
...
type MiscRouteState = Pick<ServerState, "config" | "runtime" | "agentState" | "agentName" | "shellEnabled" | "broadcastWs" | "broadcastWsToClientId" | "nextEventId" | "eventBuffer" | "shareIngestQueue" | "startup" | "broadcastStatus" | "pendingRestartReasons">;
// ---------------------------------------------------------------------------
...
function ipGeoServiceUrls(): string[] {
    ...
    if (!raw)
        return DEFAULT_IP_GEO_SERVICES;
    const urls = raw
        .split(",")
...
async function lookupApproximateLocation(): Promise<ApproximateLocation | null> {
    for (const url of ipGeoServiceUrls()) {
        try {
            ...
            if (!resp.ok) {
                logger.warn(`[MiscRoutes] IP geolocation provider returned ${resp.status} (${url})`);
                continue;
            }
            ...
            if (typeof lat !== "number" ||
                ...
                !Number.isFinite(lat) ||
                !Number.isFinite(lon)) {
                logger.warn(`[MiscRoutes] IP geolocation provider returned no usable coordinates (${url})`);
                continue;
            }
            return {
                lat,
            ...
            // coordinates.
            logger.warn({ err }, `[MiscRoutes] IP geolocation provider failed (${url})`);
        }
    }
    return null;
}
function resolveTerminalShellCommand(): {
    ...
    return {
        command: process.env.CODING_TOOLS_SHELL ||
            process.env.SHELL ||
...
function toTerminalRunRequestBody(body: Record<string, unknown>): TerminalRunRequestBody {
    return {
        command: typeof body.command === "string" ? body.command : undefined,
        clientId: body.clientId,
        terminalToken: typeof body.terminalToken === "string" ? body.terminalToken : undefined,
    };
}
export interface MiscRouteContext {
    ...
    resolveTerminalRunRejection: (req: http.IncomingMessage, body: TerminalRunRequestBody) => {
        reason: string;
        status: number;
    } | null;
    resolveTerminalRunClientId: (req: http.IncomingMessage, body: TerminalRunRequestBody) => string | null;
...
// Route handler
// ---------------------------------------------------------------------------
export async function handleMiscRoutes(ctx: MiscRouteContext): Promise<boolean> {
    ...
    if (method === "POST" && pathname === "/api/ingest/share") {
        ...
        if (!parsedShare.success) {
            error(res, parsedShare.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
    ...
    if (method === "GET" && pathname === "/api/ingest/share") {
        const consumeParsed = parseOptionalBooleanQuery(url.searchParams.get("consume"));
        if (!consumeParsed.ok) {
    ...
    if (method === "POST" &&
        ...
        if (agentEventMatch) {
            const decodedAgentId = decodePathComponent(agentEventMatch[1] ?? "", res, "agent id");
        ...
        if (!parsedEvent.success) {
            error(res, parsedEvent.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        const body = parsedEvent.data;
        if (!ctx.AGENT_EVENT_ALLOWED_STREAMS.has(body.stream)) {
            error(res, `Invalid stream: ${body.stream}. Allowed: ${[...ctx.AGENT_EVENT_ALLOWED_STREAMS].join(", ")}`, 400);
            return true;
        }
    ...
    if (method === "POST" && pathname === "/api/terminal/run") {
        ...
        if (!parsedTerm.success) {
            error(res, parsedTerm.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        ...
        if (command.includes("\n") ||
            command.includes("\r") ||
            command.includes("\0")) {
            error(res, "Command must be a single line without control characters", 400);
            return true;
        }
        ...
        const emitTerminalEvent = (payload: object) => {
            try {
                ...
                if (typeof state.broadcastWsToClientId !== "function")
                    return;
                ...
                logger.warn({ error, runClientId: targetClientId }, "[terminal] Failed to broadcast terminal event");
            }
        };
        const captureOutput = body.captureOutput === true;
        const MAX_CAPTURE_BYTES = MAX_TERMINAL_CAPTURE_BYTES;
        const runId = resolveRequestedTerminalRunId(req.headers["x-eliza-terminal-run-id"]);
        if (!runId) {
        ...
        if ("rejection" in admission) {
            ...
            error(res, `Too many active terminal runs (${maxConcurrent}). Wait for a command to finish.`, 429);
            return true;
        }
        ...
        Promise.resolve()
            .then(() => {
            const shell = (ctx.resolveTerminalShellCommand ?? resolveTerminalShellCommand)();
            return runShell({
            ...
            if (captureOutput) {
                if (captureOverflowed) {
                    ...
                    error(res, `Terminal output exceeded the ${MAX_CAPTURE_BYTES}-byte complete-capture safety limit; no partial result was returned.`, 413);
                    return;
                }
                if (runTimedOut) {
                    error(res, "Terminal execution timed out; no partial result was returned.", 504);
                    return;
                }
                if (!capturedTerminalOutputIsSafe(stdout, stderr)) {
                    ...
                    error(res, "Terminal output was not valid safe text; no result was returned.", 422);
                    return;
                }
                if (stdout) {
                    emitTerminalEvent({
    ...
    if (method === "POST" && pathname === "/api/custom-actions") {
        ...
        if (!parsedAction.success) {
            error(res, parsedAction.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        const body = parsedAction.data;
        if (body.handler.type === "shell" || body.handler.type === "code") {
            const terminalRejection = ctx.resolveTerminalRunRejection(req, toTerminalRunRequestBody(rawAction));
            if (terminalRejection) {
                error(res, `Creating ${body.handler.type} actions requires terminal authorization. ${terminalRejection.reason}`, terminalRejection.status);
                return true;
            }
        }
    ...
    if (method === "POST" && customActionTestMatch) {
        const decodedActionId = decodePathComponent(customActionTestMatch[1], res, "custom action id");
        if (decodedActionId === null)
            return true;
        ...
        if (!parsedTest.success) {
            error(res, parsedTest.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        ...
        if (def.handler.type === "shell" || def.handler.type === "code") {
            const terminalRejection = ctx.resolveTerminalRunRejection(req, toTerminalRunRequestBody(rawTest));
            if (terminalRejection) {
                error(res, `Testing ${def.handler.type} actions requires terminal authorization. ${terminalRejection.reason}`, terminalRejection.status);
                return true;
            }
        }
    ...
    if (method === "PUT" && customActionMatch) {
        const decodedActionId = decodePathComponent(customActionMatch[1], res, "custom action id");
        if (decodedActionId === null)
            return true;
        const actionId = validateUuid(decodedActionId);
        if (!actionId) {
        ...
        if (!parsedUpdate.success) {
            error(res, parsedUpdate.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
        ...
        if (newHandler.type === "shell" || newHandler.type === "code") {
            const terminalRejection = ctx.resolveTerminalRunRejection(req, toTerminalRunRequestBody(rawUpdate));
            if (terminalRejection) {
                error(res, `Updating to ${newHandler.type} handler requires terminal authorization. ${terminalRejection.reason}`, terminalRejection.status);
                return true;
            }
        }
