...
import { PutPermissionsShellRequestSchema } from "@elizaos/core/contracts/permissions-routes";
import { PutPermissionsStateRequestSchema } from "@elizaos/core/contracts/permissions-routes";
import { assertNativePersonalDataProjectionMetadataOnly } from "@elizaos/core/contracts/native-personal-data";
...
import { projectNativePersonalDataCapabilities } from "@elizaos/core/contracts/native-personal-data";
...
import { type PermissionRestrictedReason } from "@elizaos/core/contracts/permissions";
...
import { type Platform } from "@elizaos/core/contracts/permissions";
...
function isPermissionsRegistry(service: unknown): service is IPermissionsRegistry {
...
function getPermissionRegistry(runtime: AgentRuntime | null): IPermissionsRegistry | null {
...
function validatePermissionStates(raw: Record<string, Record<string, unknown>>): {
    ...
    for (const [key, entry] of Object.entries(raw)) {
        ...
        if (typeof entry.restrictedReason === "string" &&
            PERMISSION_RESTRICTED_REASONS.includes(entry.restrictedReason as PermissionRestrictedReason)) {
...
async function openSystemPermissionSettings(id: PermissionId): Promise<boolean> {
    ...
    if (platform === "darwin") {
        ...
        if (panel)
            argv = ["sh", "-lc", `gnome-control-center ${panel}`];
    }
    if (!argv)
        return false;
    try {
...
async function buildPermissionsPayload(state: PermissionRouteState, refresh = false): Promise<Record<PermissionId, PermissionState> & {
    ...
    await Promise.all(PERMISSION_IDS.map(async (id) => {
...
export async function handlePermissionRoutes(ctx: PermissionRouteContext): Promise<boolean> {
    ...
    if (method === "GET" && pathname === "/api/permissions/native-projection") {
        ...
        const states: PermissionState[] = PERMISSION_IDS.map((id) => payload[id]).filter((entry): entry is PermissionState => entry !== undefined);
        const projection = projectNativePersonalDataCapabilities(states, { platform: currentPlatform(), online: true, appState: "foreground" }, new Date().toISOString());
    ...
    if (method === "POST" &&
        ...
        if (registry) {
            try {
                json(res, await registry.request(permId, {
    ...
    if (method === "POST" &&
        ...
        json(res, {
            opened,
            id: permId,
            permission: state.permissionStates?.[permId] ?? unavailableSystemPermission(permId),
        });
        return true;
    }
    if (method === "PUT" && pathname === "/api/permissions/shell") {
        ...
        if (!parsedShell.success) {
            error(res, parsedShell.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
    ...
    if (method === "PUT" && pathname === "/api/permissions/state") {
        ...
        if (rawPermState === null)
            return true;
        const parsedPermState = PutPermissionsStateRequestSchema.safeParse(rawPermState);
        if (!parsedPermState.success) {
            error(res, parsedPermState.error.issues[0]?.message ?? "Invalid request body", 400);
            return true;
        }
