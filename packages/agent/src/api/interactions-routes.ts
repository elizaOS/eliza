...
export function parseShortcutBody(raw: string): ShortcutInteractionRequest | null {
    if (!raw.trim())
        return null;
    let parsed: unknown;
    try {
...
function readStringWithinLimit(value: unknown, maxChars: number): string | undefined {
...
export function parseComposerBody(raw: string): ComposerInteractionRequest | null {
    if (!raw.trim())
        return null;
    let parsed: unknown;
    try {
    ...
    const body = parsed as Record<string, unknown>;
    const activity = typeof body.activity === "string" &&
    ...
    if (!surface || !SURFACE_ID_PATTERN.test(surface))
        return null;
    const draftLength = readNonNegativeInteger(body.draftLength);
    if (draftLength === null || draftLength > MAX_DRAFT_LENGTH)
        return null;
    const idleForMs = body.idleForMs === undefined
        ? undefined
    ...
    const reason = body.reason === "cleared" ||
        body.reason === "blurred" ||
    ...
    if (body.conversationId !== undefined) {
        conversationId = readStringWithinLimit(body.conversationId, MAX_CONVERSATION_ID_CHARS);
        if (!conversationId)
            return null;
    }
    return {
...
export async function handleInteractionsRoutes(ctx: InteractionsRouteContext): Promise<boolean> {
