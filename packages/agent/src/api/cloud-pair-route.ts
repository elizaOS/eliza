...
function isStagingCloudHostname(hostname: string): boolean {
    ...
    return (STAGING_CLOUD_HOSTS.has(host) ||
...
function renderErrorHtml(title: string, message: string, recoveryUrl?: string): string {
...
function sendHtml(res: http.ServerResponse, status: number, body: string, extraHeaders?: Record<string, string>): void {
...
export async function handleStandaloneCloudPairRoute(req: http.IncomingMessage, res: http.ServerResponse): Promise<boolean> {
    ...
    if (!rateLimitConsume(ip)) {
        sendHtml(res, 429, renderErrorHtml("Too many sign-in attempts", "Wait a minute and try opening your agent again."));
        return true;
    }
    const token = url.searchParams.get("token")?.trim();
    if (!token) {
        sendHtml(res, 400, renderErrorHtml("Missing pairing token", "Open the agent from Eliza Cloud so a fresh sign-in link is generated."));
        return true;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            logger.warn(`[cloud-pair] exchange returned non-2xx status=${status} exchangeUrl=${exchangeUrl} requestOrigin=${origin}`);
        }
    }
    catch (err) {
        // error-policy:J1 transport/abort failures become a structured 503 page.
        const timedOut = err instanceof Error && err.name === "AbortError";
        logger.error(`[cloud-pair] exchange ${timedOut ? "timed out" : "failed"} url=${exchangeUrl} error=${err instanceof Error ? err.message : String(err)}`);
        sendHtml(res, 503, renderErrorHtml("Eliza Cloud is unreachable", timedOut
            ...
            : "We could not reach Eliza Cloud or confirm whether sign-in completed. Open your agent again from Eliza Cloud to get a fresh link."), { "retry-after": "60" });
        return true;
    }
    finally {
    ...
    if (status === 401 || status === 403 || status === 410) {
        ...
        logger.warn(`[cloud-pair] pairing link rejected status=${status} exchangeUrl=${exchangeUrl} requestOrigin=${origin}`);
        sendHtml(res, 403, renderErrorHtml("Sign-in link could not be verified", "Eliza Cloud could not verify this pairing link. It may have already been used, or does not match this agent. Open your agent again from Eliza Cloud to get a fresh link."));
        return true;
    }
    if (status === 429) {
        sendHtml(res, 429, renderErrorHtml("Too many sign-in attempts", "Wait a minute and try opening your agent again."));
    ...
    if (status >= 500) {
        ...
        sendHtml(res, 502, renderErrorHtml("Eliza Cloud hit an error", "Eliza Cloud returned an internal error, so we could not confirm whether sign-in completed. Wait a moment, then open your agent again from Eliza Cloud to get a fresh link."), { "retry-after": "60" });
        return true;
    }
    if (!exchanged) {
        ...
        sendHtml(res, 502, renderErrorHtml("Sign-in failed", accepted
