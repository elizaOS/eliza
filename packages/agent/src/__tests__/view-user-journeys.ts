...
export const PLUGIN_VIEW_LLM_MOCK_CASES: PluginViewMockCase[] = [
  // Collapsed plugins draw gui + tui (+ xr) from one declaration on the SAME
  // route (`/<id>`); the tui modality is no longer a standalone `/<id>/tui`
  // route. This list stays in lockstep with the visual matrix
  // (packages/app/test/ui-smoke/plugin-view-cases.ts) and the XR ratchet
  // (KNOWN_XR_VIEW_CASES) — see plugin-view-llm-mock-coverage.test.ts.
  ...
  { id: "waifu-imagegen", viewType: "xr", path: "/waifu-imagegen" },
