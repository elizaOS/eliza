...
import { RemoteCapabilityRouterService } from "./remote-capability-router.ts";
import { Service } from "@elizaos/core";
import { createHash } from "node:crypto";
import { createPublicKey } from "node:crypto";
import { getCapabilityRouter } from "@elizaos/core";
import { getHttpRuntime } from "@elizaos/core/api/http-plugin-runtime";
import { getPluginHttpRoutes } from "@elizaos/core/api/http-plugin-runtime";
import { hasRuntimeAppRouteModule } from "./app-package-modules.ts";
import { packageNameToAppRouteSlug } from "@elizaos/core/contracts/apps";
import { registerRuntimeAppRouteModule } from "./app-package-modules.ts";
...
import { type AppRouteModule } from "./app-package-modules.ts";
...
import { type PluginAppBridge } from "@elizaos/core";
import { type PluginAppLaunchDiagnostic } from "@elizaos/core";
import { type PluginAppLaunchPreparation } from "@elizaos/core";
import { type PluginAppSessionState } from "@elizaos/core";
import { type PluginAppViewerAuthMessage } from "@elizaos/core";
import { type PluginCallRouteParams } from "@elizaos/core";
import { type PluginCallServiceParams } from "@elizaos/core";
import { type PluginInvokeActionParams } from "@elizaos/core";
...
import { type ResponseHandlerFieldEffect } from "@elizaos/core";
...
import { type RuntimeEventStorage } from "@elizaos/core";
import { type ServiceClass } from "@elizaos/core";
import { type ViewDeclaration } from "@elizaos/core";
import { unregisterRuntimeAppRouteModule } from "./app-package-modules.ts";
...
export type RemotePluginTrustDecision = {
    ...
    endpointId?: string;
    trusted: boolean;
    reason: "no-policy" | "allowed" | "missing-endpoint-id" | "endpoint-not-allowed" | "module-not-allowed" | "missing-provenance" | "provenance-issuer-not-allowed" | "missing-provenance-public-key" | "invalid-provenance-signature" | "invalid-provenance-digest" | "provenance-walk-bound";
...
export async function registerRemoteCapabilityPlugins(runtime: IAgentRuntime, options: RemotePluginAdapterOptions = {}): Promise<Plugin[]> {
    const router = requireCapabilityRouter(runtime);
    const modules = options.modules ?? (await router.plugin.listModules()).modules;
...
export async function bootstrapRemoteCapabilityPlugins(runtime: IAgentRuntime, options: RemotePluginBootstrapOptions = {}): Promise<RemotePluginSyncResult> {
    ...
    return await syncRemoteCapabilityPlugins(runtime, {
        ...options,
        trustPolicy: options.trustPolicy ?? resolveConfiguredRemotePluginTrustPolicy(runtime),
        modules: options.modules ?? (await router.plugin.listModules()).modules,
    });
}
export async function syncRemoteCapabilityPlugins(runtime: IAgentRuntime, options: RemotePluginAdapterOptions & {
    unloadMissing?: boolean;
} = {}): Promise<RemotePluginSyncResult> {
    const router = requireCapabilityRouter(runtime);
    const modules = options.modules ?? (await router.plugin.listModules()).modules;
    const trustDecisions = evaluateRemotePluginTrustPolicy(modules, options.trustPolicy);
    validateRemotePluginNameCollisions(runtime, modules, options);
    validateRemotePluginComponentCollisions(runtime, modules, options);
    validateRemotePluginServiceCollisions(runtime, modules, options);
    validateRemotePluginModelCollisions(runtime, modules, options);
    ...
    validateRemotePluginNavTabCollisions(runtime, modules, options);
    validateRemotePluginAppBridgeIdentifierCollisions(runtime, modules, options);
    const nextPlugins = modules.map((module) => createRemoteCapabilityPlugin(module));
    ...
    if (options.unloadMissing) {
        const unloadEndpointIds = options.unloadMissingEndpointIds === undefined
            ? undefined
            : new Set(options.unloadMissingEndpointIds);
        for (const pluginName of getRegisteredRemoteCapabilityPluginNames(runtime, unloadEndpointIds)) {
            if (nextPluginNames.has(pluginName))
                continue;
...
export function createRemoteCapabilityPlugin(module: RemotePluginModuleManifest): Plugin {
    const endpointId = remoteModuleEndpointId(module);
    const services = (module.services ?? []).map((service) => createRemoteServiceClass(module.id, endpointId, service));
    const routes = (module.routes ?? []).map((route): Route => {
        const baseRoute = {
            ...
            routeHandler: async (ctx: RouteHandlerContext) => {
                const result = await requireCapabilityRouter(ctx.runtime).plugin.callRoute({
        ...
        if (route.public) {
            if (!route.publicReason?.trim()) {
                throw new Error(`[RemotePluginAdapter] public route ${route.path} must declare publicReason`);
            }
            return {
                ...baseRoute,
                public: true,
    ...
    const views = (module.views ?? []).map((view): ViewDeclaration => ({
        id: view.id,
        label: view.label,
    ...
    const evaluators = (module.evaluators ?? []).map((evaluator): RegisteredEvaluator => ({
        name: evaluator.name,
        ...
        shouldRun: async ({ runtime, message, state, options }) => (await requireCapabilityRouter(runtime).plugin.shouldRunEvaluator({
            ...endpointSelection(endpointId),
            moduleId: module.id,
        ...
        ...(evaluator.hasPrepare
            ? {
                prepare: async ({ runtime, message, state, options }) => (await requireCapabilityRouter(runtime).plugin.prepareEvaluator({
                    ...endpointSelection(endpointId),
                    moduleId: module.id,
                    evaluator: evaluator.name,
                    message: toJsonObject(message),
                ...
                processors: [
                    {
                        ...
                        process: async ({ runtime, message, state, options, prepared, output, }) => requireRemoteEvaluatorProcessResult((await requireCapabilityRouter(runtime).plugin.processEvaluator({
                            ...endpointSelection(endpointId),
                            moduleId: module.id,
                            evaluator: evaluator.name,
                            message: toJsonObject(message),
                        ...
                        })).result, module.id, evaluator.name) as never,
                    },
                ],
            }
            : {}),
    }));
    const responseHandlerEvaluators = (module.responseHandlerEvaluators ?? []).map((evaluator): ResponseHandlerEvaluator => ({
        name: evaluator.name,
        description: evaluator.description,
        priority: evaluator.priority,
        deterministicActions: (module.actions ?? []).map((action) => action.name),
        shouldRun: async (context) => (await requireCapabilityRouter(context.runtime).plugin.shouldRunResponseHandlerEvaluator({
            ...endpointSelection(endpointId),
            moduleId: module.id,
            evaluator: evaluator.name,
            context: responseHandlerContextToJsonObject(context),
        })).shouldRun,
        evaluate: async (context) => requireRemoteResponseHandlerPatch((await requireCapabilityRouter(context.runtime).plugin.evaluateResponseHandlerEvaluator({
            ...endpointSelection(endpointId),
            moduleId: module.id,
            evaluator: evaluator.name,
            context: responseHandlerContextToJsonObject(context),
        })).patch, module.id, evaluator.name) as never,
    }));
    const responseHandlerFieldEvaluators = (module.responseHandlerFieldEvaluators ?? []).map((field): ResponseHandlerFieldEvaluator => ({
        name: field.name,
        description: field.description,
        priority: field.priority,
        schema: field.schema,
        shouldRun: async (context) => (await requireCapabilityRouter(context.runtime).plugin.shouldRunResponseHandlerFieldEvaluator({
            ...endpointSelection(endpointId),
            moduleId: module.id,
            field: field.name,
        ...
        ...(field.hasParse
            ? {
                parse: async (value, context) => {
                    const result = await requireCapabilityRouter(context.runtime).plugin.parseResponseHandlerFieldEvaluator({
                        ...endpointSelection(endpointId),
                        moduleId: module.id,
                        field: field.name,
        ...
        ...(field.hasHandle
            ? {
                handle: async (context) => {
                    const result = await requireCapabilityRouter(context.runtime).plugin.handleResponseHandlerFieldEvaluator({
                        ...endpointSelection(endpointId),
                        moduleId: module.id,
                        field: field.name,
    ...
    const events = (module.events ?? []).reduce<RuntimeEventStorage>((accumulator, event) => {
    ...
    const models = (module.models ?? []).reduce<NonNullable<Plugin["models"]> & Record<string, (runtime: IAgentRuntime, params: unknown) => Promise<never>>>((accumulator, model) => {
        accumulator[model.modelType as ModelTypeName] = async (_runtime, params) => (await requireCapabilityRouter(_runtime).plugin.invokeModel({
            ...endpointSelection(endpointId),
            moduleId: module.id,
    ...
    const widgets = (module.widgets ?? []).map((widget): PluginWidgetDeclaration => ({
        id: widget.id,
    ...
    const appBridge = module.appBridge === undefined
        ? undefined
        : createRemoteAppBridge(module.id, endpointId, module.appBridge.hooks);
    const lifecycleHooks = new Set(module.lifecycle?.hooks ?? []);
    return {
        name: module.name,
        description: module.description ?? `Remote capability plugin module ${module.name}`,
        ...
        actions: (module.actions ?? []).map((action) => ({
            ...
            handler: async (runtime, message, _state, options, callback) => {
                const result = await requireCapabilityRouter(runtime).plugin.invokeAction({
                    ...endpointSelection(endpointId),
                    moduleId: module.id,
                    action: action.name,
        ...
        providers: (module.providers ?? []).map((provider) => ({
            ...
            get: async (runtime, _message, state): Promise<ProviderResult> => await requireCapabilityRouter(runtime).plugin.getProvider({
                ...endpointSelection(endpointId),
                moduleId: module.id,
            ...
            ? {
                init: async (config: Record<string, string>, runtime: IAgentRuntime) => {
                    ...
                    if (lifecycleHooks.has("init")) {
                        await callRemoteLifecycle(runtime, module.id, endpointId, "init", {
                            config,
                        });
                    }
                },
                ...
                dispose: async (runtime: IAgentRuntime) => {
                    try {
                        if (lifecycleHooks.has("dispose")) {
                            await callRemoteLifecycle(runtime, module.id, endpointId, "dispose");
                        }
                    }
                    finally {
                        if (appBridge !== undefined) {
        ...
        ...(lifecycleHooks.has("applyConfig")
            ? {
                applyConfig: async (config: Record<string, string>, runtime: IAgentRuntime) => {
                    await callRemoteLifecycle(runtime, module.id, endpointId, "applyConfig", { config });
                },
            }
            : {}),
        routes,
...
function createRemoteAppBridge(moduleId: string, endpointId: string | undefined, hooks: string[]): PluginAppBridge & AppRouteModule {
    const bridge: PluginAppBridge & AppRouteModule = {};
    const hookSet = new Set(hooks);
    const call = async (runtime: IAgentRuntime | null | undefined, hook: string, ctx: unknown) => await requireCapabilityRouterFromNullable(runtime).plugin.callAppBridge({
        ...endpointSelection(endpointId),
        moduleId,
        hook: hook as never,
    ...
    if (hookSet.has("prepareLaunch")) {
        bridge.prepareLaunch = async (ctx) => requireRemoteLaunchPreparation((await call(ctx.runtime, "prepareLaunch", ctx)).result, moduleId, "prepareLaunch");
    }
    if (hookSet.has("resolveViewerAuthMessage")) {
        bridge.resolveViewerAuthMessage = async (ctx) => requireRemoteViewerAuthMessage((await call(ctx.runtime, "resolveViewerAuthMessage", ctx)).result, moduleId, "resolveViewerAuthMessage");
    }
    ...
    if (hookSet.has("collectLaunchDiagnostics")) {
        bridge.collectLaunchDiagnostics = async (ctx) => requireRemoteLaunchDiagnostics((await call(ctx.runtime, "collectLaunchDiagnostics", ctx)).result, moduleId, "collectLaunchDiagnostics");
    }
    if (hookSet.has("resolveLaunchSession")) {
        bridge.resolveLaunchSession = async (ctx) => requireRemoteLaunchSession((await call(ctx.runtime, "resolveLaunchSession", ctx)).result, moduleId, "resolveLaunchSession");
    }
    if (hookSet.has("refreshRunSession")) {
        bridge.refreshRunSession = async (ctx) => requireRemoteLaunchSession((await call(ctx.runtime, "refreshRunSession", ctx)).result, moduleId, "refreshRunSession");
    }
    if (hookSet.has("stopRun")) {
        bridge.stopRun = async (ctx: unknown) => {
            const runtime = ctx && typeof ctx === "object" && "runtime" in ctx
                ? (ctx as {
    ...
    if (hookSet.has("handleAppRoutes")) {
        bridge.handleAppRoutes = async (ctx) => await callRemoteAppRoutes(moduleId, endpointId, ctx);
    }
    return bridge;
}
function createRemoteServiceClass(moduleId: string, endpointId: string | undefined, service: NonNullable<RemotePluginModuleManifest["services"]>[number]): ServiceClass {
    ...
    class RemoteCapabilityService extends Service {
        ...
        async stop(): Promise<void> {
            if (!methodNames.has("stop"))
                return;
            await this.callRemote("stop", []);
        }
        async callRemote(method: string, args: unknown[]): Promise<JsonValue | undefined> {
            const jsonArgs = args.map((arg) => toJsonValue(arg) ?? null);
            const result = await requireCapabilityRouter(this.runtime).plugin.callService({
                ...endpointSelection(endpointId),
                moduleId,
    ...
    for (const method of methodNames) {
        if (method === "stop" || method === "constructor")
            continue;
        Object.defineProperty(RemoteCapabilityService.prototype, method, {
            configurable: true,
            value: async function remoteServiceMethod(this: RemoteCapabilityService, ...args: unknown[]) {
...
async function callRemoteLifecycle(runtime: IAgentRuntime, moduleId: string, endpointId: string | undefined, hook: "init" | "dispose" | "applyConfig", options: {
...
async function callRemoteAppRoutes(moduleId: string, endpointId: string | undefined, ctx: AppPackageRouteContext): Promise<boolean> {
    const body = shouldReadRouteBody(ctx.method)
        ? await ctx.readJsonBody<Record<string, JsonValue>>()
        : undefined;
    const result = (await requireCapabilityRouterFromNullable(ctx.runtime as IAgentRuntime | null | undefined).plugin.callAppBridge({
        ...endpointSelection(endpointId),
        moduleId,
        ...
        context: {
            ...
            headers: routeHeadersToJsonObject(sanitizeForwardedRouteHeaders(ctx.req.headers)),
            ...(body === undefined ? {} : { body: toJsonValue(body) ?? null }),
        },
    })).result;
    if (!isJsonObject(result)) {
        throw remoteDecodeError(moduleId, "handleAppRoutes", "returned a non-object route response");
    }
    if (result.handled === false) {
        return false;
    }
    if (result.handled !== true) {
        throw remoteDecodeError(moduleId, "handleAppRoutes", "must return handled: true or handled: false");
    }
    const status = requireRemoteRouteStatus(result.status, moduleId);
    const headers = sanitizeRemoteRouteResponseHeaders(requireRemoteRouteHeaders(result.headers, moduleId));
...
function remoteAppBridgeIdentifiers(module: RemotePluginModuleManifest): string[] {
    return Array.from(new Set([module.name, module.app?.runtimePlugin, module.app?.displayName].filter((value): value is string => typeof value === "string" && value.trim().length > 0)));
}
function maxModelPriority(module: RemotePluginModuleManifest): number | undefined {
...
function remoteModuleEndpointId(module: RemotePluginModuleManifest): string | undefined {
...
function shouldRegisterPlugin(runtime: IAgentRuntime, plugin: Plugin, options: Pick<RemotePluginAdapterOptions, "reloadExisting">): boolean {
    if (options.reloadExisting)
        return true;
    return !runtime.plugins?.some((existing) => existing.name === plugin.name);
}
function validateRemotePluginNameCollisions(runtime: IAgentRuntime, modules: RemotePluginModuleManifest[], options: Pick<RemotePluginAdapterOptions, "reloadExisting">): void {
    ...
    if (options.reloadExisting)
        return;
    const remotePluginNames = new Set(getRegisteredRemoteCapabilityPluginNames(runtime));
    for (const module of modules) {
        const existing = runtime.plugins?.find((plugin) => plugin.name === module.name);
...
function evaluateRemotePluginTrustPolicy(modules: RemotePluginModuleManifest[], policy: RemotePluginTrustPolicy | undefined): RemotePluginTrustDecision[] {
    if (!policy) {
    ...
    const allowedEndpointIds = policy.allowedEndpointIds === undefined
        ? null
        : new Set(policy.allowedEndpointIds);
    const allowedModuleIds = policy.allowedModuleIds === undefined
        ? null
        : new Set(policy.allowedModuleIds);
    const allowedProvenanceIssuers = policy.allowedProvenanceIssuers === undefined
        ? null
        : new Set(policy.allowedProvenanceIssuers);
    const decisions: RemotePluginTrustDecision[] = [];
    for (const module of modules) {
        const endpointId = remoteModuleEndpointId(module);
        const provenanceIssuer = typeof module.provenance?.issuer === "string"
            ? module.provenance.issuer
            : undefined;
        if (policy.requireEndpointId && endpointId === undefined) {
            const decision = trustDecision(module, endpointId, false, "missing-endpoint-id");
        ...
        if (allowedEndpointIds &&
            (endpointId === undefined || !allowedEndpointIds.has(endpointId))) {
            const decision = trustDecision(module, endpointId, false, "endpoint-not-allowed");
            decisions.push(decision);
            throw new CapabilityError({
                code: "CAPABILITY_UNAVAILABLE",
        ...
        if (allowedModuleIds && !allowedModuleIds.has(module.id)) {
            const decision = trustDecision(module, endpointId, false, "module-not-allowed");
            decisions.push(decision);
            throw new CapabilityError({
                code: "CAPABILITY_UNAVAILABLE",
        ...
        if (policy.requireSignedProvenance && module.provenance === undefined) {
            const decision = trustDecision(module, endpointId, false, "missing-provenance");
            decisions.push(decision);
            throw new CapabilityError({
                code: "CAPABILITY_UNAVAILABLE",
        ...
        if (allowedProvenanceIssuers &&
            (provenanceIssuer === undefined ||
                !allowedProvenanceIssuers.has(provenanceIssuer))) {
            const decision = trustDecision(module, endpointId, false, "provenance-issuer-not-allowed");
            decisions.push(decision);
            throw new CapabilityError({
                code: "CAPABILITY_UNAVAILABLE",
        ...
        if (policy.requireVerifiedProvenance) {
            ...
            const publicKey = policy.trustedProvenancePublicKeys?.[module.provenance.issuer];
            if (!publicKey) {
                const decision = trustDecision(module, endpointId, false, "missing-provenance-public-key");
                decisions.push(decision);
                throw new CapabilityError({
                    code: "CAPABILITY_UNAVAILABLE",
            ...
            if (!verifyRemotePluginModuleProvenance(module, publicKey)) {
                const decision = trustDecision(module, endpointId, false, "invalid-provenance-signature");
                decisions.push(decision);
                throw new CapabilityError({
                    code: "CAPABILITY_UNAVAILABLE",
        ...
        if (policy.requireProvenanceDigestMatch) {
            ...
            if (!digestMatch.ok) {
                const reason = digestMatch.reason === "walk-bound"
                ...
                throw new CapabilityError({
                    code: "CAPABILITY_UNAVAILABLE",
                    message: digestMatch.reason === "walk-bound"
...
function verifyRemotePluginModuleProvenance(module: RemotePluginModuleManifest, publicKeyPem: string): boolean {
    ...
    try {
        return verifySignature(null, Buffer.from(remotePluginModuleProvenancePayload(provenance), "utf8"), createPublicKey(publicKeyPem), Buffer.from(provenance.signature, "base64"));
    }
    catch {
        return false;
    }
}
function remotePluginModuleProvenancePayload(provenance: NonNullable<RemotePluginModuleManifest["provenance"]>): string {
    return [
...
type RemotePluginProvenanceWalkBound = "cycle" | "depth" | "nodes" | "reflection";
...
class RemotePluginProvenanceWalkBoundError extends Error {
    ...
    constructor(reason: RemotePluginProvenanceWalkBound, options?: {
        cause?: unknown;
    }) {
        super(reason === "reflection"
            ? "Remote plugin provenance walk could not reflect on the module value"
            : `Remote plugin provenance walk exceeded ${reason} bound`, options);
...
function chargeRemotePluginProvenanceNodes(budget: RemotePluginProvenanceWalkBudget, cost: number): void {
 ...
 */
function remotePluginProvenanceDescriptorValue(descriptor: PropertyDescriptor | undefined): unknown {
...
function canonicalizeForRemotePluginProvenance(value: unknown, ancestors: Set<object> = new Set(), depth = 0, budget: RemotePluginProvenanceWalkBudget = {
    ...
    try {
        return isArray
            ...
            : canonicalizeRemotePluginProvenanceObject(value, ancestors, depth, budget);
    }
    finally {
        ancestors.delete(value);
    }
}
function canonicalizeRemotePluginProvenanceArray(value: object, ancestors: Set<object>, depth: number, budget: RemotePluginProvenanceWalkBudget): unknown[] {
    const lengthDescriptor = reflectForRemotePluginProvenance(() => Object.getOwnPropertyDescriptor(value, "length"));
    ...
    for (let index = 0; index < length; index += 1) {
        const descriptor = reflectForRemotePluginProvenance(() => Object.getOwnPropertyDescriptor(value, String(index)));
        canonical[index] = canonicalizeForRemotePluginProvenance(remotePluginProvenanceDescriptorValue(descriptor), ancestors, depth + 1, budget);
    }
    return canonical;
}
function canonicalizeRemotePluginProvenanceObject(value: object, ancestors: Set<object>, depth: number, budget: RemotePluginProvenanceWalkBudget): Record<string, unknown> {
    ...
    for (const key of keys) {
        if (typeof key !== "string")
            continue;
        const descriptor = reflectForRemotePluginProvenance(() => Object.getOwnPropertyDescriptor(value, key));
    ...
    for (const [key, entry] of entries) {
        canonical[key] = canonicalizeForRemotePluginProvenance(entry, ancestors, depth + 1, budget);
    }
    return canonical;
}
function remotePluginModuleProvenanceDigestResult(module: RemotePluginModuleManifest): {
    ok: true;
} | {
    ok: false;
    reason: "mismatch";
    ...
    try {
        if (hashRemotePluginModuleForProvenance(module) ===
...
function hashRemotePluginModuleForProvenance(module: RemotePluginModuleManifest): string {
    return createHash("sha256")
        .update(canonicalJsonForRemotePluginProvenance(module), "utf8")
        .digest("hex");
}
function canonicalJsonForRemotePluginProvenance(module: RemotePluginModuleManifest): string {
    const rest = reflectForRemotePluginProvenance(() => {
        const { capabilityEndpointId: _endpointId, provenance: _provenance, ...clone } = module;
        return clone;
    });
...
function trustDecision(module: RemotePluginModuleManifest, endpointId: string | undefined, trusted: boolean, reason: RemotePluginTrustDecision["reason"]): RemotePluginTrustDecision {
    return {
        moduleId: module.id,
        pluginName: module.name,
...
function resolveConfiguredRemotePluginTrustPolicy(runtime: IAgentRuntime): RemotePluginTrustPolicy | undefined {
    ...
    const trustPolicy = configuredRemotePluginTrustPolicyOptions(runtime, endpointIds);
    return {
...
function configuredEndpointIds(config: {
    ...
    for (const endpoint of config.endpoints ?? []) {
        if (endpoint.id.trim())
            ids.add(endpoint.id.trim());
    }
    return [...ids];
}
function configuredAllowedModuleIds(runtime: IAgentRuntime, endpointIds: string[]): string[] {
    const configured = runtime.getSetting?.("ELIZA_CAPABILITY_ROUTER_ALLOWED_MODULES");
...
function configuredRemotePluginTrustPolicyOptions(runtime: IAgentRuntime, endpointIds: string[]): RemotePluginTrustPolicy {
    const configured = runtime.getSetting?.("ELIZA_CAPABILITY_ROUTER_TRUST_POLICY");
    const raw = typeof configured === "string" && configured.trim()
        ? configured
    ...
    try {
        ...
        const globalCandidate = "allowedProvenanceIssuers" in record ||
...
function mergeConfiguredTrustPolicyOptions(values: Array<Record<string, unknown>>): RemotePluginTrustPolicy {
...
function validateRemotePluginComponentCollisions(runtime: IAgentRuntime, modules: RemotePluginModuleManifest[], options: Pick<RemotePluginAdapterOptions, "reloadExisting">): void {
    validateNamedRemoteComponents({
        ...
        existingRemoteOwners: getRegisteredRemoteCapabilityComponentOwners(runtime, "actions", (action) => action.name),
        namesForModule: (module) => (module.actions ?? []).map((action) => action.name),
    });
    validateNamedRemoteComponents({
        runtime,
        modules,
        ...
        existingRemoteOwners: getRegisteredRemoteCapabilityComponentOwners(runtime, "providers", (provider) => provider.name),
        namesForModule: (module) => (module.providers ?? []).map((provider) => provider.name),
    });
    validateNamedRemoteComponents({
        runtime,
        modules,
        ...
        existingRemoteOwners: getRegisteredRemoteCapabilityComponentOwners(runtime, "evaluators", (evaluator) => evaluator.name),
        namesForModule: (module) => (module.evaluators ?? []).map((evaluator) => evaluator.name),
    });
    validateNamedRemoteComponents({
        runtime,
        modules,
        ...
        existingNames: runtime.responseHandlerEvaluators?.map((evaluator) => evaluator.name) ??
            [],
        existingRemoteOwners: getRegisteredRemoteCapabilityComponentOwners(runtime, "responseHandlerEvaluators", (evaluator) => evaluator.name),
        namesForModule: (module) => (module.responseHandlerEvaluators ?? []).map((evaluator) => evaluator.name),
    });
    validateNamedRemoteComponents({
        runtime,
        modules,
        ...
        existingNames: runtime.responseHandlerFieldEvaluators?.map((evaluator) => evaluator.name) ?? [],
        existingRemoteOwners: getRegisteredRemoteCapabilityComponentOwners(runtime, "responseHandlerFieldEvaluators", (evaluator) => evaluator.name),
        namesForModule: (module) => (module.responseHandlerFieldEvaluators ?? []).map((evaluator) => evaluator.name),
    });
}
function validateNamedRemoteComponents(args: {
    ...
    kind: "action" | "provider" | "evaluator" | "response-handler evaluator" | "response-handler field evaluator";
    ...
    if (args.options.reloadExisting)
        return;
    for (const module of args.modules) {
        for (const name of args.namesForModule(module)) {
            ...
            if (existingRemoteModuleId === module.id)
                continue;
            if (!args.existingNames.includes(name))
                continue;
            throw new CapabilityError({
                code: "CAPABILITY_DECODE_FAILED",
...
function validateRemotePluginServiceCollisions(runtime: IAgentRuntime, modules: RemotePluginModuleManifest[], options: Pick<RemotePluginAdapterOptions, "reloadExisting">): void {
    const seen = new Map<string, string>();
    for (const module of modules) {
    ...
    if (options.reloadExisting)
        return;
    const registeredRemoteServiceTypes = new Set(getRegisteredRemoteCapabilityServiceTypes(runtime));
    for (const module of modules) {
        for (const service of module.services ?? []) {
            if (!runtime.hasService?.(service.serviceType))
                continue;
            if (registeredRemoteServiceTypes.has(service.serviceType))
                continue;
            throw new CapabilityError({
                code: "CAPABILITY_DECODE_FAILED",
...
function validateRemotePluginModelCollisions(runtime: IAgentRuntime, modules: RemotePluginModuleManifest[], options: Pick<RemotePluginAdapterOptions, "reloadExisting">): void {
    const seen = new Map<string, string>();
    for (const module of modules) {
        for (const model of module.models ?? []) {
            ...
            if (existingModuleId) {
                throw new CapabilityError({
                    code: "CAPABILITY_DECODE_FAILED",
                    message: existingModuleId === module.id
    ...
    for (const module of modules) {
        for (const model of module.models ?? []) {
            ...
            if (existingRemoteModuleId === module.id)
                continue;
            if (!localModelTypes.has(model.modelType))
                continue;
            throw new CapabilityError({
                code: "CAPABILITY_DECODE_FAILED",
...
function validateRemotePluginRouteDeclarations(modules: RemotePluginModuleManifest[]): void {
    for (const module of modules) {
...
function validateRemotePluginRouteCollisions(runtime: IAgentRuntime, modules: RemotePluginModuleManifest[], options: Pick<RemotePluginAdapterOptions, "reloadExisting">): void {
    const seen = new Map<string, string>();
    for (const module of modules) {
        for (const route of module.routes ?? []) {
    ...
    if (options.reloadExisting)
        return;
    const registeredRemoteRouteKeys = new Set(getRegisteredRemoteCapabilityRoutes(runtime).map((route) => routeCollisionKey(route.type, route.path)));
    for (const module of modules) {
        for (const route of module.routes ?? []) {
            const key = routeCollisionKey(route.method, route.path);
            const existing = getHttpRuntime(runtime).routes?.find((runtimeRoute) => routeCollisionKey(runtimeRoute.type, runtimeRoute.path) === key);
            if (!existing)
                continue;
            if (registeredRemoteRouteKeys.has(key))
                continue;
            throw new CapabilityError({
                code: "CAPABILITY_DECODE_FAILED",
...
function validateRemotePluginViewCollisions(runtime: IAgentRuntime, modules: RemotePluginModuleManifest[], options: Pick<RemotePluginAdapterOptions, "reloadExisting">): void {
    const seen = new Map<string, string>();
    for (const module of modules) {
    ...
    if (options.reloadExisting)
        return;
    const registeredRemoteViewKeys = new Set(getRegisteredRemoteCapabilityViews(runtime).map(viewCollisionKey));
    for (const module of modules) {
        for (const view of module.views ?? []) {
            const key = viewCollisionKey(view);
            ...
            if (!existing)
                continue;
            if (registeredRemoteViewKeys.has(key))
                continue;
            throw new CapabilityError({
                code: "CAPABILITY_DECODE_FAILED",
...
function validateRemotePluginWidgetCollisions(runtime: IAgentRuntime, modules: RemotePluginModuleManifest[], options: Pick<RemotePluginAdapterOptions, "reloadExisting">): void {
    const seen = new Map<string, string>();
    for (const module of modules) {
    ...
    if (options.reloadExisting)
        return;
    const registeredRemoteWidgetKeys = new Set(getRegisteredRemoteCapabilityWidgets(runtime).map(widgetDeclarationKey));
    for (const module of modules) {
        for (const widget of module.widgets ?? []) {
            const key = widgetCollisionKey(module, widget);
            const existing = (runtime.plugins ?? [])
            ...
            if (!existing)
                continue;
            if (registeredRemoteWidgetKeys.has(key))
                continue;
            throw new CapabilityError({
                code: "CAPABILITY_DECODE_FAILED",
...
function validateRemotePluginNavTabCollisions(runtime: IAgentRuntime, modules: RemotePluginModuleManifest[], options: Pick<RemotePluginAdapterOptions, "reloadExisting">): void {
    const seen = new Map<string, string>();
    for (const module of modules) {
    ...
    if (options.reloadExisting)
        return;
    const registeredRemoteNavTabKeys = new Set(getRegisteredRemoteCapabilityNavTabs(runtime).map((navTab) => navTab.id));
    for (const module of modules) {
        for (const navTab of module.app?.navTabs ?? []) {
            const key = navTab.id;
            const existing = (runtime.plugins ?? [])
            ...
            if (!existing)
                continue;
            if (registeredRemoteNavTabKeys.has(key))
                continue;
            throw new CapabilityError({
                code: "CAPABILITY_DECODE_FAILED",
...
function validateRemotePluginAppBridgeIdentifierCollisions(runtime: IAgentRuntime, modules: RemotePluginModuleManifest[], options: Pick<RemotePluginAdapterOptions, "reloadExisting">): void {
    const seen = new Map<string, {
        moduleId: string;
        identifier: string;
    }>();
    for (const module of modules) {
        if (module.appBridge === undefined)
            continue;
        for (const identifier of remoteAppBridgeIdentifiers(module)) {
    ...
    if (options.reloadExisting)
        return;
    const registeredRemoteBridgeKeys = new Set(getRegisteredRemoteCapabilityAppBridgeKeys(runtime));
    for (const module of modules) {
        if (module.appBridge === undefined)
            continue;
        for (const identifier of remoteAppBridgeIdentifiers(module)) {
            const key = appBridgeIdentifierKey(identifier);
            if (!hasRuntimeAppRouteModule(identifier))
                continue;
            if (registeredRemoteBridgeKeys.has(key))
                continue;
            throw new CapabilityError({
                code: "CAPABILITY_DECODE_FAILED",
...
function widgetCollisionKey(module: RemotePluginModuleManifest, widget: NonNullable<RemotePluginModuleManifest["widgets"]>[number]): string {
...
async function ensureConfiguredCapabilityRouter(runtime: IAgentRuntime, options: Pick<RemotePluginBootstrapOptions, "registerRouterService">): Promise<ElizaCapabilityRouter | null> {
    ...
    if (options.registerRouterService !== false) {
        ...
        const service = await runtime.getServiceLoadPromise(CAPABILITY_ROUTER_SERVICE_TYPE);
...
function getRegisteredRemoteCapabilityViews(runtime: IAgentRuntime): NonNullable<Plugin["views"]> {
...
function getRegisteredRemoteCapabilityWidgets(runtime: IAgentRuntime): NonNullable<Plugin["widgets"]> {
...
function getRegisteredRemoteCapabilityNavTabs(runtime: IAgentRuntime): NonNullable<NonNullable<Plugin["app"]>["navTabs"]> {
...
function getRegisteredRemoteCapabilityAppBridgeKeys(runtime: IAgentRuntime): string[] {
    const keys = new Set<string>();
    for (const item of runtime.getAllPluginOwnership?.() ?? []) {
        const moduleId = remotePluginModuleId(item.plugin);
        if (!moduleId || item.plugin.appBridge === undefined)
            continue;
    ...
    for (const plugin of runtime.plugins ?? []) {
        ...
        if (!moduleId || plugin.appBridge === undefined)
            continue;
...
function getRegisteredRemoteCapabilityComponentOwners<K extends "actions" | "providers" | "evaluators" | "responseHandlerEvaluators" | "responseHandlerFieldEvaluators">(runtime: IAgentRuntime, field: K, nameForComponent: (component: NonNullable<Plugin[K]>[number]) => string): Map<string, string> {
...
function getRegisteredRemoteCapabilityModelOwners(runtime: IAgentRuntime): Map<string, string> {
    const owners = new Map<string, string>();
    for (const item of runtime.getAllPluginOwnership?.() ?? []) {
        const moduleId = remotePluginModuleId(item.plugin);
        if (!moduleId)
...
function getLocalRuntimeModelTypes(runtime: IAgentRuntime): Set<string> {
    ...
    for (const item of runtime.getAllPluginOwnership?.() ?? []) {
        if (remotePluginModuleId(item.plugin))
            continue;
        for (const model of item.models ?? []) {
    ...
    for (const plugin of runtime.plugins ?? []) {
        if (remotePluginModuleId(plugin))
            continue;
        for (const modelType of Object.keys(plugin.models ?? {})) {
            modelTypes.add(modelType);
        }
...
function remoteAppBridgeIdentifiersForPlugin(plugin: Plugin): string[] {
    return [plugin.name, packageNameToAppRouteSlug(plugin.name)].filter((value): value is string => typeof value === "string" && value.length > 0);
}
function getRegisteredRemoteCapabilityServiceTypes(runtime: IAgentRuntime): string[] {
    return (runtime.getAllPluginOwnership?.() ?? [])
        .filter((item) => {
        const config = item.plugin.config as Record<string, unknown> | undefined;
        return typeof config?.remoteCapabilityModuleId === "string";
...
function getRegisteredRemoteCapabilityPluginNames(runtime: IAgentRuntime, endpointIds?: Set<string>): string[] {
...
function remotePluginEndpointMatches(config: Record<string, unknown>, endpointIds: Set<string> | undefined): boolean {
    if (endpointIds === undefined)
        return true;
    return (typeof config.remoteCapabilityEndpointId === "string" &&
        endpointIds.has(config.remoteCapabilityEndpointId));
}
function requireCapabilityRouter(runtime: IAgentRuntime): ElizaCapabilityRouter {
...
function requireCapabilityRouterFromNullable(runtime: IAgentRuntime | null | undefined): ElizaCapabilityRouter {
    if (!runtime) {
        throw new CapabilityError({
            code: "CAPABILITY_UNAVAILABLE",
            message: `Runtime does not have ${CAPABILITY_ROUTER_SERVICE_TYPE} service.`,
...
function remoteDecodeError(moduleId: string, hook: string, reason: string): CapabilityError {
...
function requireRemoteEvaluatorProcessResult(result: JsonObject | undefined, moduleId: string, evaluatorName: string): ActionResult | undefined {
    if (result === undefined)
        return undefined;
    if (typeof result.success !== "boolean") {
        throw remoteDecodeError(moduleId, `evaluator.${evaluatorName}.process`, "returned an action result without boolean success");
    }
    return { ...result, success: result.success };
}
function requireRemoteResponseHandlerPatch(patch: JsonObject | undefined, moduleId: string, evaluatorName: string): JsonObject | undefined {
    if (patch === undefined)
        return undefined;
    ...
    for (const [key, value] of Object.entries(patch)) {
        ...
        if (!validate) {
            throw remoteDecodeError(moduleId, `responseHandler.${evaluatorName}.evaluate`, `returned unknown patch field "${key}"`);
        }
        if (!validate(value)) {
            throw remoteDecodeError(moduleId, `responseHandler.${evaluatorName}.evaluate`, `returned invalid patch field "${key}"`);
        }
    }
    return patch;
}
function requireRemoteLaunchPreparation(result: JsonValue | undefined, moduleId: string, hook: string): PluginAppLaunchPreparation | null {
    if (result === null)
        return null;
    if (!isJsonObject(result)) {
        throw remoteDecodeError(moduleId, hook, "returned invalid launch data");
    }
    if (result.launchUrl !== undefined &&
...
function requireRemoteViewerAuthMessage(result: JsonValue | undefined, moduleId: string, hook: string): PluginAppViewerAuthMessage | null {
    if (result === null)
        return null;
    if (!isJsonObject(result)) {
        throw remoteDecodeError(moduleId, hook, "returned invalid viewer auth message");
    }
    if (!isRemoteViewerAuthMessage(result)) {
        throw remoteDecodeError(moduleId, hook, "returned malformed viewer auth message");
    }
    return result;
}
function isRemoteViewerAuthMessage(value: JsonObject): value is JsonObject & PluginAppViewerAuthMessage {
...
function requireRemoteLaunchDiagnostics(result: JsonValue | undefined, moduleId: string, hook: string): PluginAppLaunchDiagnostic[] {
    if (result === undefined) {
...
function requireRemoteLaunchSession(result: JsonValue | undefined, moduleId: string, hook: string): PluginAppSessionState | null {
    if (result === null)
        return null;
    if (!isJsonObject(result)) {
...
function isRemoteLaunchSession(value: JsonObject): value is JsonObject & PluginAppSessionState {
...
function requireRemoteRouteStatus(status: JsonValue | undefined, moduleId: string): number {
    if (status === undefined)
        return 200;
    if (typeof status !== "number" ||
        ...
        status < 100 ||
        status > 599) {
        throw remoteDecodeError(moduleId, "handleAppRoutes", "returned invalid status");
    }
    return status;
}
function requireRemoteRouteHeaders(headers: JsonValue | undefined, moduleId: string): Record<string, string> {
    if (headers === undefined)
        return {};
    if (!isJsonObject(headers)) {
        throw remoteDecodeError(moduleId, "handleAppRoutes", "returned invalid headers");
    }
    ...
    for (const [key, value] of Object.entries(headers)) {
        if (typeof value !== "string" &&
            ...
            throw remoteDecodeError(moduleId, "handleAppRoutes", `returned invalid header "${key}"`);
        }
...
function eventPayloadToJsonObject(value: unknown): JsonObject | undefined {
    ...
    const { runtime: _runtime, ...serializable } = value as Record<string, unknown>;
...
function responseHandlerFieldEffectFromJson(effect: {
    patch?: JsonObject;
    preempt?: {
    ...
    debug?: string[];
} | undefined): ResponseHandlerFieldEffect | undefined {
    if (!effect)
        return undefined;
    return {
...
function routeHeadersToJsonObject(headers: AppPackageRouteContext["req"]["headers"]): JsonObject {
...
function sanitizeForwardedRouteHeaders<T extends Record<string, string | string[] | undefined>>(headers: T | undefined): T {
    const result: Record<string, string | string[] | undefined> = {};
    for (const [key, value] of Object.entries(headers ?? {})) {
        if (SENSITIVE_FORWARDED_ROUTE_HEADERS.has(key.toLowerCase()))
            continue;
        result[key] = value;
    }
    return result as T;
}
function sanitizeRemoteRouteResponseHeaders<T extends Record<string, string | undefined>>(headers: T): T {
