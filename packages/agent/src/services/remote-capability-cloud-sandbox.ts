...
import { buildRemoteCapabilityEndpointTrustPolicy } from "./remote-capability-endpoint-provider.ts";
import { classifyElizaHostname } from "@elizaos/plugin-elizacloud/cloud-config/domain-contract";
import { connectRemoteCapabilityEndpointProvider } from "./remote-capability-endpoint-provider.ts";
import { installRemoteCapabilityEndpoint } from "./remote-capability-endpoint-provider.ts";
import { toWellFormedUnicode } from "@elizaos/core";
import { trimEndCharacters } from "../utils/string-boundaries.ts";
import { truncateWellFormed } from "@elizaos/core";
import { type IAgentRuntime } from "@elizaos/core";
import { type ProvisionedRemoteCapabilityEndpoint } from "./remote-capability-endpoint-provider.ts";
import { type RemoteCapabilityEndpointConfig } from "./remote-capability-router.ts";
import { type RemoteCapabilityEndpointProvider } from "./remote-capability-endpoint-provider.ts";
import { type RemoteCapabilityEndpointTrustPolicyOptions } from "./remote-capability-endpoint-provider.ts";
...
export type ConnectCloudCapabilitySandboxOptions = CloudCapabilitySandboxProvisionOptions & {
    unloadMissing?: boolean;
    requestTimeoutMs?: number;
};
export type ConnectCloudCapabilitySandboxResult = CloudCapabilitySandboxProvisionResult & {
    providerId: string;
...
export const cloudCapabilityEndpointProvider: RemoteCapabilityEndpointProvider<CloudCapabilitySandboxProvisionOptions> = {
    id: "cloud",
    provision: async (options): Promise<ProvisionedRemoteCapabilityEndpoint> => ({
        providerId: "cloud",
...
export async function provisionCloudCapabilitySandbox(options: CloudCapabilitySandboxProvisionOptions): Promise<CloudCapabilitySandboxProvisionResult> {
    ...
    const create = await cloudJson<CreateAgentResponse>(request, `${baseUrl}/api/v1/eliza/agents`, {
        method: "POST",
        headers,
        body: JSON.stringify({
    ...
    if (!create.ok) {
        throw new Error(`Failed to create cloud capability sandbox: ${create.text ?? create.status}`);
    }
    const agentId = create.data?.data?.id ?? create.data?.id;
    if (!agentId) {
        throw new Error("Failed to create cloud capability sandbox: missing agent id.");
    }
    options.onProgress?.("provisioning", "Provisioning cloud capability endpoint.");
    const provision = await cloudJson<ProvisionResponse>(request, `${baseUrl}/api/v1/eliza/agents/${encodeURIComponent(agentId)}/provision`, {
        method: "POST",
        headers,
    });
    if (!provision.ok) {
        throw new Error(`Failed to provision cloud capability sandbox: ${provision.text ?? provision.status}`);
    }
    ...
    if (!jobId) {
        throw new Error("Failed to provision cloud capability sandbox: missing job id and endpoint URL.");
    }
    const deadline = Date.now() + (options.timeoutMs ?? DEFAULT_CLOUD_PROVISION_TIMEOUT_MS);
    const pollIntervalMs = options.pollIntervalMs ?? DEFAULT_CLOUD_PROVISION_POLL_MS;
    let lastStatus: string | undefined;
    let lastError: string | undefined;
    while (Date.now() < deadline) {
        await sleep(pollIntervalMs);
        const job = await cloudJson<JobResponse>(request, `${baseUrl}/api/v1/jobs/${encodeURIComponent(jobId)}`, {
            method: "GET",
            headers,
        });
        if (!job.ok)
        ...
        if (status === "failed") {
            throw new Error(`Cloud capability sandbox provisioning failed: ${error ?? "unknown error"}`);
        }
        options.onProgress?.("provisioning", `Cloud capability sandbox status: ${status ?? "pending"}.`);
    }
    throw new Error(`Cloud capability sandbox provisioning timed out.${lastStatus ? ` Last status: ${lastStatus}.` : ""}${lastError ? ` Last error: ${lastError}.` : ""}`);
}
export async function connectCloudCapabilitySandbox(runtime: IAgentRuntime, options: ConnectCloudCapabilitySandboxOptions): Promise<ConnectCloudCapabilitySandboxResult> {
...
export async function waitForCloudCapabilityEndpointAvailability(options: WaitForCloudCapabilityEndpointAvailabilityOptions): Promise<void> {
    const request = options.fetch ?? fetch;
    ...
    do {
        try {
            ...
            try {
                const response = await request(new URL("/v1/capabilities", options.endpoint.baseUrl), {
                    method: "GET",
                    headers: {
        ...
        options.onProgress?.(`waiting for endpoint ${options.endpoint.id}: ${lastError}`);
        const remainingMs = deadline - Date.now();
        if (remainingMs <= 0)
            break;
        await sleep(Math.min(pollIntervalMs, remainingMs));
    } while (Date.now() < deadline);
    throw new Error(`Cloud capability endpoint ${options.endpoint.id} did not report plugin availability within ${timeoutMs}ms. Last error: ${lastError}`);
}
...
export { buildRemoteCapabilityEndpointTrustPolicy as buildEndpointTrustPolicy, installRemoteCapabilityEndpoint, };
function assertCloudProvisionResult(result: ProvisionedRemoteCapabilityEndpoint): asserts result is ProvisionedRemoteCapabilityEndpoint & {
    agentId: string;
} {
    if (!result.agentId) {
        throw new Error("Cloud capability sandbox provisioning returned no agent id.");
    }
}
function endpointFromProvisionPayload(options: Pick<CloudCapabilitySandboxProvisionOptions, "endpointId" | "token">, payload: ProvisionResponse | ProvisionResponse["data"] | null | undefined): RemoteCapabilityEndpointConfig | null {
    const data = payload && "data" in payload && payload.data ? payload.data : payload;
    const baseUrl = firstString(data?.capabilityRouterUrl, data?.capability_router_url, data?.bridgeUrl, data?.bridge_url);
    if (!baseUrl)
        return null;
    const token = firstString(options.token, data?.capabilityRouterToken, data?.capability_router_token, data?.token);
    return {
...
async function cloudJson<T>(request: typeof fetch, url: string, init: RequestInit): Promise<CloudJsonResponse<T>> {
...
function normalizeCloudApiBase(value: string): string {
    ...
    try {
        ...
        if (classified.environment &&
            ...
            url.hostname = new URL(ELIZA_DOMAIN_CONTRACTS[classified.environment].cloudApiOrigin).hostname;
        }
