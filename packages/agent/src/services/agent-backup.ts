...
import { AGENT_BACKUP_AUTHORITY_DIRECTORY } from "./agent-backup-authority.ts";
import { AGENT_BACKUP_CANONICAL_JSON } from "@elizaos/core/canonical-json";
import { AGENT_BACKUP_CAPTURE_V2_FRAME_FORMAT } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { AGENT_BACKUP_CAPTURE_V2_LIMITS } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { AGENT_BACKUP_CAPTURE_V2_SCHEMA_VERSION } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { AgentBackupCaptureV2ComponentDescriptorSchema } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { ElizaError } from "@elizaos/core";
import { INITIAL_AGENT_BACKUP_GENERATION } from "./agent-backup-authority.ts";
...
import { compareAgentBackupCaptureV2FilePaths } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { createKmsClient } from "@elizaos/auth/kms";
import { isBackupAuthorityPath } from "./agent-backup-authority.ts";
import { logger } from "@elizaos/core";
import { parseAgentBackupCaptureV2Request } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { readAgentBackupCaptureV2FrameDigest } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { resolveConfigPath } from "../config/paths.ts";
import { resolveDefaultAgentWorkspaceDir } from "../shared/workspace-resolution.ts";
import { resolveStateDir } from "../config/paths.ts";
import { resolveUserPath } from "../config/paths.ts";
import { serializeAgentBackupCaptureV2Frame } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { stableJsonString } from "@elizaos/core/canonical-json";
import { systemKey } from "@elizaos/auth/kms";
import { timeInferenceSpan } from "@elizaos/core";
import { type AgentBackupCaptureV2ComponentDescriptor } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { type AgentBackupCaptureV2FileEntry } from "@elizaos/core/contracts/agent-backup-capture-v2";
import { type AgentBackupCaptureV2FrameHeader } from "@elizaos/core/contracts/agent-backup-capture-v2";
...
export class AgentSnapshotBudgetExceededError extends ElizaError {
    ...
    constructor(readonly stage: string, readonly observedBytes: number, readonly limitBytes: number) {
...
export class SnapshotBudget {
    ...
    private fileCount = 0;
    constructor(private readonly maxRawBytes: number, private readonly maxFiles: number, private readonly signal?: AbortSignal) { }
    ...
    reserve(declaredBytes: number): SnapshotReservation {
        ...
        if (projected > this.maxRawBytes) {
            throw new AgentSnapshotBudgetExceededError("file capture", projected, this.maxRawBytes);
        }
    ...
    assertWireSize(value: unknown): void {
        ...
        if (wireBytes > this.maxRawBytes) {
            throw new AgentSnapshotBudgetExceededError("final wire serialization", wireBytes, this.maxRawBytes);
        }
    }
    private chargeFileEntry(base64Bytes: number): void {
        ...
        if (this.fileCount > this.maxFiles) {
            throw new AgentSnapshotBudgetExceededError("file capture", this.fileCount, this.maxFiles);
        }
    ...
    private charge(bytes: number, stage: string): void {
        ...
        if (this.chargedBytes + this.reservedBytes > this.maxRawBytes) {
            throw new AgentSnapshotBudgetExceededError(stage, this.chargedBytes + this.reservedBytes, this.maxRawBytes);
        }
    }
}
...
async function readFileEntry(root: string, absolutePath: string, budget?: SnapshotBudget): Promise<AgentBackupFileEntry> {
...
function fileEntryFromBytes(relativePath: string, bytes: Buffer): AgentBackupFileEntry {
...
async function collectFileSet(params: {
    ...
    async function visit(dir: string): Promise<void> {
        ...
        for (const entry of entries) {
            ...
            if (params.include && !params.include(relative))
                continue;
...
function baseStateFileInclude(relativePath: string): boolean {
    ...
    if (relativePath === "plugins/.runtime-imports" ||
    ...
    if (relativePath === "skills/.cache/catalog.json")
        return false;
    const first = relativePath.split("/")[0];
    if (first === AGENT_BACKUP_AUTHORITY_DIRECTORY ||
...
function vaultFileInclude(relativePath: string): boolean {
    return (relativePath === VAULT_JSON_PATH ||
...
function pgliteFileInclude(relativePath: string): boolean {
    const first = relativePath.split("/")[0];
    if (PGLITE_VOLATILE_ROOT_FILES.has(relativePath))
        return false;
    if (first.startsWith(".s.PGSQL."))
        return false;
...
async function removePgliteVolatileFiles(root: string): Promise<void> {
    await Promise.all([...PGLITE_VOLATILE_ROOT_FILES].map((fileName) => fs.rm(path.join(root, fileName), { force: true })));
...
function relativeRootWithin(root: string, target: string | null): string | null {
    if (!target)
        return null;
    const relative = path.relative(root, target);
...
function makeStateFileInclude(stateDir: string, pgliteDir: string | null): (relativePath: string) => boolean {
    const pgliteRelativeRoot = relativeRootWithin(path.resolve(stateDir), pgliteDir ? path.resolve(pgliteDir) : null);
    return (relativePath: string): boolean => {
        if (!baseStateFileInclude(relativePath))
            return false;
        if (pgliteRelativeRoot &&
...
function hasPostgresUrl(runtime?: IAgentRuntime | AgentRuntime | null): string | null {
...
function getTableColumnsBucket(tableColumns: Map<string, string[]>, tableName: string): string[] {
 ...
 */
export async function fetchAgentScopedRowsBatched(pool: {
    query: (text: string, values: unknown[]) => Promise<{
        rows: unknown[];
    }>;
}, buildSql: (keysetClause: string) => string, baseParams: unknown[], budget: SnapshotBudget | undefined, tableName: string, 
// Qualified for a join (`e."id"`), bare otherwise — the ORDER BY must be
// unambiguous or Postgres rejects the statement.
idExpression: string): Promise<JsonRecord[]> {
    const rows: JsonRecord[] = [];
    let lastId: unknown = null;
    for (;;) {
        budget?.check();
        const params = lastId === null ? baseParams : [...baseParams, lastId];
        const keysetClause = lastId === null
        ...
        if (batch.length === 0)
            break;
        budget?.chargeRaw(Buffer.byteLength(JSON.stringify(batch), "utf8"), `postgres table ${tableName}`);
        rows.push(...batch);
...
async function capturePostgresRows(postgresUrl: string, agentId: string, budget?: SnapshotBudget): Promise<AgentBackupPostgresDump> {
       ...
       ORDER BY table_name, ordinal_position`);
        ...
        for (const [tableName, columns] of tableColumns) {
            ...
            if (tableName === POSTGRES_AGENT_TABLE && columnSet.has("id")) {
                const result = await pool.query(`SELECT * FROM ${quoteIdentifier(tableName)} WHERE ${quoteIdentifier("id")} = $1`, [agentId]);
                rows = result.rows as JsonRecord[];
            }
            else if (tableName === POSTGRES_EMBEDDINGS_TABLE &&
                columnSet.has("memory_id")) {
                if (columnSet.has("id")) {
                    rows = await fetchAgentScopedRowsBatched(pool, (keyset) => `SELECT e.*
               ...
               WHERE m.${quoteIdentifier("agent_id")} = $1 ${keyset}`, [agentId], budget, tableName, `e.${quoteIdentifier("id")}`);
                    charged = true;
                }
                else {
             ...
             INNER JOIN ${quoteIdentifier(POSTGRES_MEMORIES_TABLE)} m
               ON e.${quoteIdentifier("memory_id")} = m.${quoteIdentifier("id")}
             WHERE m.${quoteIdentifier("agent_id")} = $1`, [agentId]);
                    rows = result.rows as JsonRecord[];
                }
            }
            else {
                ...
                if (!ownerColumn)
                    continue;
                if (columnSet.has("id")) {
                    rows = await fetchAgentScopedRowsBatched(pool, (keyset) => `SELECT * FROM ${quoteIdentifier(tableName)} WHERE ${quoteIdentifier(ownerColumn)} = $1 ${keyset}`, [agentId], budget, tableName, quoteIdentifier("id"));
                    charged = true;
                }
                else {
                    ...
                    const result = await pool.query(`SELECT * FROM ${quoteIdentifier(tableName)} WHERE ${quoteIdentifier(ownerColumn)} = $1`, [agentId]);
                    rows = result.rows as JsonRecord[];
                }
            }
            if (tableName === POSTGRES_AGENT_TABLE ||
                ...
                if (!charged) {
                    budget?.chargeRaw(Buffer.byteLength(JSON.stringify(rows), "utf8"), `postgres table ${tableName}`);
                }
                tables.push({ name: tableName, columns, rows });
            }
        }
        tables.sort((left, right) => tableRestoreRank(left.name) - tableRestoreRank(right.name));
...
function withPostgresHash(dump: AgentBackupPostgresDump): AgentBackupPostgresDump {
    return {
        ...dump,
        sha256: sha256Json(dump.tables.map((table) => ({
            name: table.name,
...
function withPgliteDumpHash(dump: AgentBackupPgliteDump): AgentBackupPgliteDump {
    return {
        ...dump,
        sha256: sha256Json({
            kind: dump.kind,
 ...
 */
export const PGLITE_SNAPSHOT_UNAVAILABLE_TRANSIENT = "PGlite snapshot temporarily unavailable (connection closing)";
export const PGLITE_SNAPSHOT_UNAVAILABLE_TRANSIENT_CODE = "PGLITE_SNAPSHOT_UNAVAILABLE_TRANSIENT";
...
function isPgliteClosingError(err: unknown): boolean {
    ...
    return (message === "closing" ||
        /\b(?:pglite|database|connection)\s+(?:is\s+)?(?:closed|closing)\b/.test(message));
}
async function capturePgliteDump(runtime: IAgentRuntime | AgentRuntime, pgliteDir: string, signal: AbortSignal, budget?: SnapshotBudget): Promise<AgentBackupPgliteDump | null> {
    const adapter = runtime.adapter as {
        dumpPgliteDataDirAfterPreflight?: (preflight: () => Promise<PglitePhysicalPreflight>, compression?: "gzip") => Promise<unknown>;
    ...
    if (typeof adapter?.getPgliteDataDir !== "function") {
        throw new Error("The bounded PGlite exporter cannot attest its physical data directory");
    }
    const managedDataDir = adapter.getPgliteDataDir();
    if (typeof managedDataDir !== "string" ||
        managedDataDir.length === 0 ||
        managedDataDir === ":memory:" ||
        managedDataDir.includes("://")) {
        throw new Error("The bounded PGlite exporter is not backed by a physical data directory");
    }
    ...
    if (physicalPgliteDir !== physicalManagedDataDir) {
        throw new Error("The bounded PGlite exporter data directory does not match backup configuration");
    }
    let bounded: unknown;
    let provenPreflight: PglitePhysicalPreflight | undefined;
    try {
        bounded = await managedDump.call(adapter, async () => {
            const proof = await preflightPglitePhysicalDirectory(physicalPgliteDir, signal, runtime.agentId);
    ...
    if (typeof release !== "function") {
        throw new Error("The bounded PGlite exporter did not provide a consumer-lifetime lease");
    }
    let released = false;
    const releaseOnce = () => {
        if (released)
    ...
    try {
        if (preflight !== provenPreflight || !provenPreflight) {
            throw new Error("The bounded PGlite exporter skipped its required preflight");
        }
        if (!isBlobLike(dump)) {
        ...
        const legacyAdditionalMemoryBudgetBytes = dump.size * LEGACY_PGLITE_POST_DUMP_COPY_FACTOR;
        const legacyRequiredAvailableMemoryBytes = legacyAdditionalMemoryBudgetBytes +
            AGENT_BACKUP_V2_PGLITE_CAPTURE_LIMITS.availableMemoryHeadroomBytes;
        if (resolveAgentBackupAvailableMemoryBytes() <
...
async function captureDatabaseComponent(runtime: IAgentRuntime | AgentRuntime, signal: AbortSignal, budget?: SnapshotBudget): Promise<AgentBackupDatabaseComponent> {
    const postgresUrl = hasPostgresUrl(runtime);
    if (postgresUrl) {
        const postgres = await capturePostgresRows(postgresUrl, runtime.agentId, budget);
        return {
            kind: "postgres-rows",
            postgres,
    ...
    const pgliteDump = await capturePgliteDump(runtime, pgliteDir, signal, budget);
    if (pgliteDump) {
        return {
            kind: "pglite-dump",
            pgliteDump,
...
async function captureCharacterComponent(runtime: IAgentRuntime | AgentRuntime, budget?: SnapshotBudget): Promise<AgentBackupManifest["components"]["character"]> {
    ...
    budget?.chargeRaw(Buffer.byteLength(JSON.stringify(component.runtimeCharacter), "utf8"), "runtime character");
...
export async function createAgentSnapshot(runtime: IAgentRuntime | AgentRuntime, config: ElizaConfig, options?: {
    signal?: AbortSignal;
    maxRawBytes?: number;
    maxFiles?: number;
}): Promise<AgentBackupStateData> {
    return withAgentBackupAuthority(resolveStateDir(), async (authority) => captureAgentSnapshot(runtime, config, await authority.generation(runtime.agentId), options));
}
async function captureAgentSnapshot(runtime: IAgentRuntime | AgentRuntime, config: ElizaConfig, restoreGeneration: string, options?: {
    signal?: AbortSignal;
    maxRawBytes?: number;
    maxFiles?: number;
}): Promise<AgentBackupStateData> {
    ...
    const signal = options?.signal
        ...
        : controller.signal;
    const budget = new SnapshotBudget(options?.maxRawBytes ?? MAX_RESTORABLE_AGENT_BACKUP_BYTES, options?.maxFiles ?? DEFAULT_SNAPSHOT_MAX_FILES, signal);
    const stateDir = resolveStateDir();
    const pgliteDirForStateFiles = hasPostgresUrl(runtime)
        ? null
        : await resolvePgliteDir();
    const stateFileInclude = makeStateFileInclude(stateDir, pgliteDirForStateFiles);
    ...
    const [database, media, vault, character, stateFiles] = await Promise.all(captures);
    ...
    budget.chargeRaw(Buffer.byteLength(JSON.stringify(snapshot.config), "utf8"), "legacy config");
    budget.assertWireSize(snapshot);
    return snapshot;
}
async function encryptLocalBackupEnvelope(snapshot: AgentBackupStateData): Promise<AgentBackupFileEnvelope> {
    ...
    const encrypted = await kms.encrypt(keyId, textEncoder.encode(stableJson(snapshot)), localBackupAad(manifest.agentId, stateSha256));
    return {
        schemaVersion: 1,
...
async function decryptLocalBackupEnvelope(envelope: AgentBackupFileEnvelope): Promise<AgentBackupStateData> {
    if (envelope.format !== LOCAL_BACKUP_FORMAT ||
        envelope.schemaVersion !== 1 ||
        envelope.encryption.algorithm !== "kms-aes-256-gcm") {
        throw new Error("Unsupported local agent backup file");
    }
    const kms = getLocalBackupKmsClient();
    const plaintext = await kms.decrypt(envelope.encryption.kmsKeyId, b64decode(envelope.encryption.ciphertext), b64decode(envelope.encryption.nonce), b64decode(envelope.encryption.authTag), localBackupAad(envelope.agentId, envelope.stateSha256), envelope.encryption.kmsKeyVersion);
    const snapshot = JSON.parse(textDecoder.decode(plaintext)) as AgentBackupStateData;
    const actual = sha256Json(snapshot);
    if (actual !== envelope.stateSha256) {
        throw new Error(`Local backup state hash mismatch: expected ${envelope.stateSha256}, got ${actual}`);
    }
    assertManifest(snapshot);
    return snapshot;
}
async function readLocalBackupEnvelope(fileName: string): Promise<AgentBackupFileEnvelope> {
    const filePath = resolveLocalBackupPath(fileName);
    return JSON.parse(await fs.readFile(filePath, "utf8")) as AgentBackupFileEnvelope;
}
export async function createLocalAgentBackup(runtime: IAgentRuntime | AgentRuntime, config: ElizaConfig): Promise<LocalAgentBackupMetadata> {
    return withAgentBackupAuthority(resolveStateDir(), async (authority) => {
        const snapshot = await captureAgentSnapshot(runtime, config, await authority.generation(runtime.agentId));
        return persistLocalAgentBackup(snapshot);
    });
}
async function persistLocalAgentBackup(snapshot: AgentBackupStateData): Promise<LocalAgentBackupMetadata> {
...
async function pruneLocalBackups(agentId: string, keepFileName: string): Promise<void> {
...
const localBackupMetadataCache = new Map<string, {
    version: string;
...
export async function listLocalAgentBackups(agentId?: string): Promise<LocalAgentBackupMetadata[]> {
    const root = localBackupsDir();
    let entries: Dirent[];
    try {
        entries = await timeInferenceSpan("local-backups:directory-list", () => fs.readdir(root, { withFileTypes: true }));
    }
    catch (error) {
        ...
        if ((error as NodeJS.ErrnoException).code === "ENOENT" &&
            !(await timeInferenceSpan("local-backups:directory-stat", () => pathExists(root))))
            return [];
        throw error;
    }
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            const envelope = JSON.parse(await fs.readFile(filePath, "utf8")) as AgentBackupFileEnvelope;
            if (envelope.format !== LOCAL_BACKUP_FORMAT ||
                envelope.schemaVersion !== 1)
                continue;
            ...
            if (localBackupFileVersion(await fs.stat(filePath, { bigint: true })) !==
                version)
                continue;
            ...
            if (localBackupMetadataCache.size >= LOCAL_BACKUP_METADATA_CACHE_SIZE) {
                ...
                if (oldest !== undefined)
                    localBackupMetadataCache.delete(oldest);
            }
...
export async function reviewRetiredLocalAgentBackups(agentId: string): Promise<{
    ...
    return withReviewedRetiredLocalAgentBackups(agentId, async (review) => review);
}
/** Keeps inventory stable until the caller durably admits its reviewed identities. */
export async function withReviewedRetiredLocalAgentBackups<T>(agentId: string, operation: (review: {
    generation: string;
    archives: RetiredLocalAgentBackup[];
}) => Promise<T>): Promise<T> {
    return withAgentBackupAuthority(resolveStateDir(), async (authority) => {
...
async function readRetiredLocalAgentBackups(agentId: string, generation: string): Promise<{
    generation: string;
    archives: RetiredLocalAgentBackup[];
}> {
    const root = localBackupsDir();
    ...
    try {
        ...
        throw new ElizaError("[AgentBackup] Backup directory is unavailable for review", {
            code: "AGENT_BACKUP_REVIEW_UNAVAILABLE",
            cause,
        });
    }
    if (!directory.isDirectory() || directory.isSymbolicLink())
        throw new ElizaError("[AgentBackup] Backup review requires a real directory", {
            code: "AGENT_BACKUP_REVIEW_UNAVAILABLE",
        });
    const archives: RetiredLocalAgentBackup[] = [];
    const entries = await fs.readdir(root, { withFileTypes: true });
    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
        if (!entry.name.endsWith(LOCAL_BACKUP_EXTENSION))
            continue;
        try {
            if (!entry.isFile() || entry.isSymbolicLink())
                throw new Error("Archive is not a regular file");
            const handle = await fs.open(resolveLocalBackupPath(entry.name), constants.O_RDONLY | constants.O_NOFOLLOW);
            let bytes: Buffer;
            try {
            ...
            const envelope = cleanupEnvelopeSchema.parse(JSON.parse(bytes.toString("utf8")));
            const snapshot = await decryptLocalBackupEnvelope(envelope);
            const manifest = assertManifest(snapshot);
            if (manifest.agentId !== envelope.agentId)
                throw new Error("Authenticated archive identity disagrees with its envelope");
            // Authenticate every candidate before excluding another agent's archive.
            if (manifest.agentId !== agentId)
                continue;
            const restoreGeneration = manifest.restoreGeneration === undefined
            ...
            throw new ElizaError("[AgentBackup] Reconcile the unreadable archive before reviewing backup cleanup", {
                code: "AGENT_BACKUP_REVIEW_UNAVAILABLE",
 ...
 */
export async function purgeAdmittedRetiredLocalAgentBackups(agentId: string, loadAdmission: () => Promise<{
    generation: string;
    notBefore: string;
    archives: RetiredLocalAgentBackup[];
}>): Promise<void> {
    return withAgentBackupAuthority(resolveStateDir(), async (authority) => {
        ...
        if (generation !== z.string().uuid().parse(admission.generation))
            throw new ElizaError("[AgentBackup] Backup generation changed; review cleanup again", {
                code: "AGENT_BACKUP_CLEANUP_STALE",
            });
        const deadline = Date.parse(z.string().datetime().parse(admission.notBefore));
        ...
        for (const archive of admission.archives) {
            ...
            if (admitted.has(archive.fileName) ||
                archive.restoreGeneration === generation)
                throw new ElizaError("[AgentBackup] Cleanup admission is inconsistent", {
                    code: "AGENT_BACKUP_CLEANUP_STALE",
                });
        ...
        for (const archive of review.archives) {
            ...
            if (!expected || stableJson(expected) !== stableJson(archive))
                throw new ElizaError("[AgentBackup] Archive inventory changed; review cleanup again", {
                    code: "AGENT_BACKUP_CLEANUP_STALE",
                });
        }
        ...
        for (const archive of admission.archives) {
            ...
            try {
                ...
                if (cause instanceof Error &&
                    ...
                    cause.code === "ENOENT")
                    continue;
                throw new ElizaError("[AgentBackup] Admitted archive cannot be reconciled", {
                    code: "AGENT_BACKUP_CLEANUP_STALE",
                    cause,
                });
            }
        ...
        if (review.archives.length) {
            const directory = await fs.open(localBackupsDir(), constants.O_RDONLY | constants.O_NOFOLLOW);
            try {
        ...
        if (remaining.archives.length)
            throw new ElizaError("[AgentBackup] Retired archive removal is incomplete", {
                code: "AGENT_BACKUP_CLEANUP_INCOMPLETE",
            });
    });
}
export async function restoreLocalAgentBackup(runtime: IAgentRuntime | AgentRuntime, fileName: string): Promise<{
    restored: true;
    requiresRestart: true;
}> {
    const snapshot = await decryptLocalBackupEnvelope(await readLocalBackupEnvelope(fileName));
...
function verifyFileEntry(entry: AgentBackupFileEntry): Buffer {
    ...
    if (actual !== entry.sha256) {
        throw new Error(`Backup file hash mismatch for ${entry.path}: expected ${entry.sha256}, got ${actual}`);
    }
    if (entry.size !== bytes.length) {
        throw new Error(`Backup file size mismatch for ${entry.path}: expected ${entry.size}, got ${bytes.length}`);
    }
    return bytes;
}
...
function verifyPgliteDump(dump: AgentBackupPgliteDump): Buffer {
    ...
    if (expected !== dump.sha256) {
        throw new Error(`PGlite dump hash mismatch: expected ${dump.sha256}, got ${expected}`);
    }
    return verifyFileEntry(dump.file);
}
async function pruneExtraFiles(root: string, include: (relativePath: string) => boolean, keepPaths: Set<string>): Promise<void> {
    if (!(await pathExists(root)))
        return;
    async function visit(dir: string): Promise<void> {
        const entries = await fs.readdir(dir, { withFileTypes: true });
...
async function prepareVaultRestoreDirectories(vault: AgentBackupFileSet): Promise<string[]> {
    const version = vault.files.find((file) => file.path === `${VAULT_PGLITE_DIR_NAME}/PG_VERSION`);
    if (!version)
        return [];
    const { PGlite } = await import("@electric-sql/pglite");
    const template = await fs.mkdtemp(path.join(os.tmpdir(), "eliza-vault-restore-layout-"));
    try {
        const database = await PGlite.create(template);
        await database.close();
        if (!(await fs.readFile(path.join(template, "PG_VERSION"))).equals(verifyFileEntry(version))) {
            throw new ElizaError("[AgentBackup] Vault database version does not match the installed PGlite version", {
        ...
        const visit = async (directory: string): Promise<void> => {
            for (const entry of await fs.readdir(directory, {
                ...
                const relative = normalizeRelativePath(path.relative(template, absolute));
                directories.push(`${VAULT_PGLITE_DIR_NAME}/${relative}`);
                await visit(absolute);
            }
        };
        await visit(template);
        const files = new Set(vault.files.map((file) => normalizeRelativePath(file.path)));
        for (const directory of directories) {
            if (files.has(directory)) {
                throw new ElizaError("[AgentBackup] Vault archive replaces a required database directory with a file", {
...
async function restoreFileSet(root: string, fileSet: AgentBackupFileSet, options: {
    replaceRoot?: boolean;
    include?: (relativePath: string) => boolean;
    ...
    const filesToRestore = options.include
        ? fileSet.files.filter((entry) => options.include?.(normalizeRelativePath(entry.path)))
        : fileSet.files;
    const keepPaths = new Set(filesToRestore.map((entry) => normalizeRelativePath(entry.path)));
...
async function restorePgliteDump(pgliteDir: string, dump: AgentBackupPgliteDump): Promise<void> {
...
function sortedTablesForRestore(tables: AgentBackupPostgresTable[]): AgentBackupPostgresTable[] {
    return [...tables].sort((left, right) => tableRestoreRank(left.name) - tableRestoreRank(right.name));
}
function sortedTablesForDelete(tables: AgentBackupPostgresTable[]): AgentBackupPostgresTable[] {
...
function verifyPostgresDump(dump: AgentBackupPostgresDump): void {
    ...
    if (expected !== dump.sha256) {
        throw new Error(`Postgres dump hash mismatch: expected ${dump.sha256}, got ${expected}`);
    }
}
async function restorePostgresRows(postgresUrl: string, agentId: string, dump: AgentBackupPostgresDump): Promise<void> {
    ...
    try {
        ...
        await client
            .query(`DELETE FROM ${quoteIdentifier(POSTGRES_EMBEDDINGS_TABLE)}
       WHERE ${quoteIdentifier("memory_id")} IN (
        ...
        for (const table of sortedTablesForDelete(dump.tables)) {
            if (table.name === POSTGRES_EMBEDDINGS_TABLE)
                continue;
            if (table.name === POSTGRES_AGENT_TABLE)
                continue;
            const columnSet = new Set(table.columns);
            const ownerColumn = agentIdColumn(columnSet);
            if (!ownerColumn)
                continue;
            await client.query(`DELETE FROM ${quoteIdentifier(table.name)} WHERE ${quoteIdentifier(ownerColumn)} = $1`, [agentId]);
        }
        await client
            .query(`DELETE FROM ${quoteIdentifier(POSTGRES_AGENT_TABLE)} WHERE ${quoteIdentifier("id")} = $1`, [agentId])
            .catch(() => undefined);
        for (const table of sortedTablesForRestore(dump.tables)) {
            ...
            for (const row of table.rows) {
                ...
                await client.query(`INSERT INTO ${quoteIdentifier(table.name)} (${quotedColumns.join(", ")})
...
function assertManifest(snapshot: AgentBackupStateData): AgentBackupManifest {
    ...
    if (manifest?.format !== "elizaos.agent-backup" ||
    ...
    if (stableJson(actualHashes) !== stableJson(manifest.integrity.componentHashes)) {
        throw new Error("Backup manifest component hash index is inconsistent");
    }
    return manifest;
}
export async function restoreAgentSnapshot(runtime: IAgentRuntime | AgentRuntime, snapshot: AgentBackupStateData): Promise<{
    restored: true;
    requiresRestart: true;
}> {
    return withAgentBackupAuthority(resolveStateDir(), async (authority) => {
        ...
        if ((manifest.restoreGeneration ?? INITIAL_AGENT_BACKUP_GENERATION) !==
            current)
            throw new ElizaError("[AgentBackup] This snapshot predates a data-deletion boundary and cannot be restored", {
        ...
        if (sets.some((set) => set.files.some((file) => isBackupAuthorityPath(file.path))))
            throw new ElizaError("[AgentBackup] A snapshot cannot replace backup authority", {
        ...
        const databaseRoot = manifest.components.database.kind === "postgres-rows"
            ? null
        ...
        if ((databaseRoot &&
            ...
            throw new ElizaError("[AgentBackup] Restore targets overlap backup authority; configure independent database and configuration paths", {
                code: "AGENT_BACKUP_AUTHORITY_INVALID",
            });
        return restoreAuthorizedAgentSnapshot(runtime, snapshot);
    });
}
async function restoreAuthorizedAgentSnapshot(runtime: IAgentRuntime | AgentRuntime, snapshot: AgentBackupStateData): Promise<{
    restored: true;
    requiresRestart: true;
}> {
    const manifest = assertManifest(snapshot);
    if (manifest.agentId !== runtime.agentId) {
        throw new Error(`Backup belongs to agent ${manifest.agentId}, not ${runtime.agentId}`);
    }
    const stateDir = resolveStateDir();
    ...
    const vaultDirectories = await prepareVaultRestoreDirectories(manifest.components.vault);
    let pgliteDirForStateFiles: string | null = null;
    if (database.kind === "postgres-rows") {
        const postgresUrl = hasPostgresUrl(runtime);
        if (!postgresUrl) {
            throw new Error("Backup contains Postgres rows but POSTGRES_URL is not configured");
        }
        ...
        if (pgliteDir === ":memory:" || pgliteDir.includes("://")) {
            throw new Error(`Cannot restore PGlite backup into non-filesystem data dir ${pgliteDir}`);
        }
        verifyPgliteDump(database.pgliteDump);
        await stopRuntimeBeforeDatabaseRestore(runtime);
        if (typeof (runtime.adapter as {
            close?: () => Promise<void>;
        }).close ===
            "function") {
            await (runtime.adapter as {
                close: () => Promise<void>;
            }).close();
        }
        ...
        for (const file of database.pglite.files)
            normalizeRelativePath(file.path);
        await stopRuntimeBeforeDatabaseRestore(runtime);
        if (typeof (runtime.adapter as {
            close?: () => Promise<void>;
        ...
        throw new Error(database.reason ?? "Backup did not capture a database component");
    }
    await restoreFileSet(path.join(stateDir, MEDIA_DIR_NAME), manifest.components.media, {
        replaceRoot: true,
    });
    ...
    if (manifest.components.character.configFile) {
        ...
        await fs.writeFile(configPath, bytes, {
            mode: typeof manifest.components.character.configFile.mode === "number"
...
async function stopRuntimeBeforeDatabaseRestore(runtime: IAgentRuntime | AgentRuntime): Promise<void> {
...
function captureError(message: string, code: string, context?: Record<string, unknown>, options?: {
    cause?: unknown;
    severity?: "ephemeral" | "fatal";
}): never {
...
function sourceAbortError(signal: AbortSignal): Error {
    return signal.reason instanceof Error
        ? signal.reason
        : new AgentBackupV2CaptureError("Agent backup capture was cancelled", "AGENT_BACKUP_V2_CAPTURE_ABORTED", undefined, { severity: "ephemeral" });
}
function assertCaptureActive(request: AgentBackupCaptureV2Request, signal: AbortSignal | undefined, now: () => number): void {
    if (signal?.aborted) {
        if (signal.reason instanceof AgentBackupV2CaptureError) {
            throw signal.reason;
        }
        captureError("Agent backup capture was cancelled", "AGENT_BACKUP_V2_CAPTURE_ABORTED", { operationId: request.operationId }, { cause: abortReason(signal), severity: "ephemeral" });
    }
    if (now() >= request.deadlineEpochMs) {
        captureError("Agent backup capture deadline exceeded", "AGENT_BACKUP_V2_CAPTURE_DEADLINE_EXCEEDED", {
            operationId: request.operationId,
            deadlineEpochMs: request.deadlineEpochMs,
        }, { severity: "ephemeral" });
    }
}
async function awaitWithCaptureControl<T>(operation: () => PromiseLike<T>, request: AgentBackupCaptureV2Request, signal: AbortSignal | undefined, now: () => number): Promise<T> {
    ...
    const interrupted = new Promise<never>((_resolve, reject) => {
        timeout = setTimeout(() => reject(new AgentBackupV2CaptureError("Agent backup capture deadline exceeded", "AGENT_BACKUP_V2_CAPTURE_DEADLINE_EXCEEDED", {
            operationId: request.operationId,
            deadlineEpochMs: request.deadlineEpochMs,
        }, { severity: "ephemeral" })), Math.min(remainingMs, 2147483647));
        if (signal) {
            abortListener = () => reject(signal.reason instanceof AgentBackupV2CaptureError
                ? signal.reason
                : new AgentBackupV2CaptureError("Agent backup capture was cancelled", "AGENT_BACKUP_V2_CAPTURE_ABORTED", { operationId: request.operationId }, { cause: abortReason(signal), severity: "ephemeral" }));
...
function normalizeCaptureRelativePath(input: string): string {
    ...
    if (!normalized ||
        ...
        normalized.startsWith("../") ||
        path.posix.isAbsolute(normalized)) {
        captureError(`Invalid backup path: ${input}`, "AGENT_BACKUP_V2_INVALID_PATH", { path: input }, { severity: "fatal" });
    }
    return normalized;
}
function resolveCaptureDirectoryIdentity(directory: string, role: "pglite" | "state"): string {
    const resolved = path.resolve(directory);
    try {
        const physical = nodeFs.realpathSync.native(resolved);
        if (!nodeFs.statSync(physical).isDirectory()) {
            captureError(`Agent backup ${role} path is not a directory`, "AGENT_BACKUP_V2_DIRECTORY_IDENTITY_INVALID", { role, path: resolved }, { severity: "fatal" });
        }
        return physical;
    }
    catch (error) {
        if (error instanceof AgentBackupV2CaptureError)
            throw error;
        ...
        throw new AgentBackupV2CaptureError(`Agent backup could not resolve the ${role} directory identity`, "AGENT_BACKUP_V2_DIRECTORY_IDENTITY_UNRESOLVED", { role, path: resolved }, { cause: error, severity: "fatal" });
    }
}
function pathsOverlap(left: string, right: string): boolean {
    return isWithin(left, right) || isWithin(right, left);
}
function resolveStateFilesPgliteExclusion(stateDir: string, pgliteDir: string): string | null {
    const physicalStateDir = resolveCaptureDirectoryIdentity(stateDir, "state");
    const physicalPgliteDir = resolveCaptureDirectoryIdentity(pgliteDir, "pglite");
    if (isWithin(physicalPgliteDir, physicalStateDir)) {
        captureError("PGlite cannot contain or equal the agent state directory during capture", "AGENT_BACKUP_V2_PGLITE_STATE_OVERLAP", { stateDir: physicalStateDir, pgliteDir: physicalPgliteDir }, { severity: "fatal" });
    }
    if (!isWithin(physicalStateDir, physicalPgliteDir))
        return null;
    ...
    if (pathsOverlap(mediaDir, physicalPgliteDir) ||
        pathsOverlap(vaultPgliteDir, physicalPgliteDir) ||
        physicalPgliteDir === vaultAuditDir) {
        captureError("PGlite overlaps another dedicated backup component", "AGENT_BACKUP_V2_PGLITE_COMPONENT_OVERLAP", { stateDir: physicalStateDir, pgliteDir: physicalPgliteDir }, { severity: "fatal" });
    }
    return normalizeCaptureRelativePath(path.relative(physicalStateDir, physicalPgliteDir));
}
...
export function resolveAgentBackupAvailableMemoryBytes(): number {
    ...
    if (typeof processAvailableMemory === "number" &&
    ...
    captureError("Available memory could not be proven before PGlite export", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_UNPROVEN", undefined, { severity: "fatal" });
}
...
export async function preflightPglitePhysicalDirectory(physicalRoot: string, signal: AbortSignal, agentId: string, options: {
    ...
    try {
        while (pendingDirectories.length > 0) {
            ...
            if (!before.isDirectory() || before.isSymbolicLink()) {
                captureError("PGlite physical-size preflight encountered an unsafe directory", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_UNPROVEN", { agentId }, { severity: "fatal" });
            }
            ...
            for await (const entry of entries) {
                ...
                if (entryCount > AGENT_BACKUP_CAPTURE_V2_LIMITS.maxFiles) {
                    captureError("PGlite physical-size preflight exceeds the entry-count limit", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_ENTRY_LIMIT", { agentId, entryCount }, { severity: "fatal" });
                }
                const absolutePath = path.join(directory, entry.name);
                if (!isWithin(physicalRoot, absolutePath)) {
                    captureError("PGlite physical-size preflight escaped its configured root", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_UNPROVEN", { agentId }, { severity: "fatal" });
                }
                const stats = await fs.lstat(absolutePath, { bigint: true });
                if (stats.isSymbolicLink()) {
                    captureError("PGlite physical-size preflight refuses symbolic links", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_UNPROVEN", { agentId }, { severity: "fatal" });
                }
                ...
                if (!stats.isFile() || stats.size < 0n) {
                    captureError("PGlite physical-size preflight encountered a non-regular entry", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_UNPROVEN", { agentId }, { severity: "fatal" });
                }
            ...
            if (!sameDirectoryIdentity(before, after)) {
                captureError("PGlite changed while its physical size was being proven", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_CHANGED", { agentId }, { severity: "ephemeral" });
            }
        }
    }
    catch (error) {
        if (error instanceof AgentBackupV2CaptureError)
            throw error;
        throw new AgentBackupV2CaptureError("PGlite physical size could not be proven before export", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_UNPROVEN", { agentId }, { cause: error, severity: "fatal" });
    }
    const estimatedArchive = Number(estimatedArchiveBytes);
    const archiveCopyFactor = options.archiveCopyFactor ?? limits.archiveCopyFactor;
    if (!Number.isSafeInteger(archiveCopyFactor) ||
        archiveCopyFactor < limits.archiveCopyFactor) {
        captureError("PGlite export requested an invalid archive-copy budget", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_UNPROVEN", { agentId, archiveCopyFactor }, { severity: "fatal" });
    }
    const additionalMemoryBudgetBytes = estimatedArchive * archiveCopyFactor;
    const requiredAvailableMemoryBytes = additionalMemoryBudgetBytes + limits.availableMemoryHeadroomBytes;
    const availableMemoryBytes = resolveAgentBackupAvailableMemoryBytes();
    if (availableMemoryBytes < requiredAvailableMemoryBytes) {
        captureError("PGlite export would exceed the available-memory budget", 
...
async function* splitOpaqueBytes(bytes: Uint8Array): AsyncGenerator<AgentBackupV2CaptureSourceChunk> {
    for (let offset = 0; offset < bytes.length; offset += AGENT_BACKUP_CAPTURE_V2_LIMITS.maxFramePayloadBytes) {
        yield {
            bytes: bytes.subarray(offset, Math.min(bytes.length, offset + AGENT_BACKUP_CAPTURE_V2_LIMITS.maxFramePayloadBytes)),
        };
    }
}
async function* walkFiles(root: string, include: ((relativePath: string) => boolean) | undefined, signal: AbortSignal): AsyncGenerator<{
    absolutePath: string;
    relativePath: string;
}> {
    const resolvedRoot = path.resolve(root);
    ...
    async function* visit(directory: string): AsyncGenerator<{
        ...
        if (signal.aborted) {
            captureError("Agent backup file walk was cancelled", "AGENT_BACKUP_V2_CAPTURE_ABORTED", undefined, { cause: abortReason(signal), severity: "ephemeral" });
        }
        const entries = await fs.readdir(directory, {
            withFileTypes: true,
        });
        entries.sort((left, right) => compareAgentBackupCaptureV2FilePaths(`${left.name}${left.isDirectory() ? "/" : ""}`, `${right.name}${right.isDirectory() ? "/" : ""}`));
        for (const entry of entries) {
            const absolutePath = path.join(directory, entry.name);
            if (!isWithin(resolvedRoot, absolutePath))
                continue;
            const relativePath = normalizeCaptureRelativePath(path.relative(resolvedRoot, absolutePath));
...
function fileSetSource(descriptor: AgentBackupCaptureV2ComponentDescriptor, root: string, include?: (relativePath: string) => boolean): AgentBackupV2CaptureComponentSource {
    return {
        descriptor,
        async *open(signal) {
            let fileCount = 0;
            for await (const file of walkFiles(root, include, signal)) {
                ...
                if (fileCount > AGENT_BACKUP_CAPTURE_V2_LIMITS.maxFiles) {
                    captureError(`Component ${descriptor.name} exceeds the file-count limit`, "AGENT_BACKUP_V2_FILE_LIMIT", { componentName: descriptor.name, fileCount }, { severity: "fatal" });
                }
                ...
                if (before.size === 0) {
                    ...
                    if (signal.aborted)
                        stream.destroy(sourceAbortError(signal));
                    ...
                    try {
                        for await (const chunk of stream) {
                            ...
                            if (bytes.length === 0) {
                                captureError(`File stream made no progress for ${file.relativePath}`, "AGENT_BACKUP_V2_ZERO_PROGRESS", { componentName: descriptor.name, path: file.relativePath }, { severity: "fatal" });
                            }
                            yield {
                                bytes,
                    ...
                    if (fileOffsetBytes !== before.size) {
                        captureError(`File size changed while capturing ${file.relativePath}`, "AGENT_BACKUP_V2_FILE_CHANGED", {
                ...
                if (after.size !== before.size ||
                    ...
                    captureError(`File changed while capturing ${file.relativePath}`, "AGENT_BACKUP_V2_FILE_CHANGED", { componentName: descriptor.name, path: file.relativePath }, { severity: "ephemeral" });
                }
            }
        },
    };
}
function jsonSource(descriptor: AgentBackupCaptureV2ComponentDescriptor, value: unknown): AgentBackupV2CaptureComponentSource {
    return {
        descriptor,
        open() {
...
function acquirePgliteDumpSlot(physicalPgliteDir: string, agentId: string): () => void {
    if (activePgliteDumpByPhysicalDirectory.has(physicalPgliteDir)) {
        captureError("A previous PGlite export is still active or settling", "AGENT_BACKUP_V2_PGLITE_DUMP_BUSY", { agentId }, { severity: "ephemeral" });
    }
...
function pgliteDumpSource(runtime: AgentBackupV2CaptureRuntime, physicalPgliteDir: string): AgentBackupV2CaptureComponentSource {
    const adapter = runtime.adapter as {
        dumpPgliteDataDirAfterPreflight?: (preflight: () => Promise<PglitePhysicalPreflight>, compression?: "gzip") => Promise<{
            dump: unknown;
    ...
    const managedDump = adapter?.dumpPgliteDataDirAfterPreflight;
    if (typeof managedDump !== "function") {
        captureError("Capture v2 requires the fenced, lifecycle-managed PGlite dump exporter", "AGENT_BACKUP_V2_PGLITE_MANAGED_DUMP_UNAVAILABLE", { agentId: runtime.agentId }, { severity: "fatal" });
    }
    if (typeof adapter?.getPgliteDataDir !== "function") {
        captureError("The PGlite exporter cannot attest its physical data directory", "AGENT_BACKUP_V2_PGLITE_DIRECTORY_UNATTESTED", { agentId: runtime.agentId }, { severity: "fatal" });
    }
    const managedDataDir = adapter.getPgliteDataDir();
    if (typeof managedDataDir !== "string" ||
        managedDataDir.length === 0 ||
        managedDataDir === ":memory:" ||
        managedDataDir.includes("://")) {
        captureError("The PGlite exporter did not attest a filesystem-backed data directory", "AGENT_BACKUP_V2_PGLITE_DIRECTORY_UNATTESTED", { agentId: runtime.agentId }, { severity: "fatal" });
    }
    const managedPhysicalDir = resolveCaptureDirectoryIdentity(resolveUserPath(managedDataDir), "pglite");
    if (managedPhysicalDir !== physicalPgliteDir) {
        captureError("The PGlite exporter data directory does not match capture configuration", "AGENT_BACKUP_V2_PGLITE_DIRECTORY_MISMATCH", { agentId: runtime.agentId }, { severity: "fatal" });
    }
    ...
    const prepare = async (signal: AbortSignal): Promise<void> => {
        if (signal.aborted)
            throw sourceAbortError(signal);
        if (disposed) {
            captureError("The PGlite capture source is already closed", "AGENT_BACKUP_V2_PGLITE_DUMP_ALREADY_CONSUMED", { agentId: runtime.agentId }, { severity: "fatal" });
        }
        if (prepared)
            return;
        if (!preparing) {
            preparing = (async () => {
                const releaseDumpSlot = acquirePgliteDumpSlot(physicalPgliteDir, runtime.agentId);
                ...
                try {
                    ...
                    let provenPreflight: PglitePhysicalPreflight | undefined;
                    try {
                        const dumpPromise = Promise.resolve().then(() => runManagedDump(async () => {
                            const proof = await preflightPglitePhysicalDirectory(physicalPgliteDir, signal, runtime.agentId);
                            provenPreflight = proof;
                            return proof;
                        }, "gzip"));
                        ...
                        if (error instanceof AgentBackupV2CaptureError)
                            throw error;
                        if (pgliteManagedExportErrorCode(error) ===
                            "PGLITE_DATA_DIR_EXPORT_BUSY") {
                            captureError("A previous PGlite export is still active or settling", "AGENT_BACKUP_V2_PGLITE_DUMP_BUSY", { agentId: runtime.agentId }, { severity: "ephemeral" });
                        }
                        throw new AgentBackupV2CaptureError("PGlite could not create a managed data-dir export", "AGENT_BACKUP_V2_PGLITE_DUMP_FAILED", { agentId: runtime.agentId }, { cause: error, severity: "ephemeral" });
                    }
                    if (typeof boundedDump.release !== "function") {
                        captureError("The managed PGlite exporter did not provide a consumer-lifetime lease", "AGENT_BACKUP_V2_PGLITE_MANAGED_DUMP_UNAVAILABLE", { agentId: runtime.agentId }, { severity: "fatal" });
                    }
                    releaseManagedLease = boundedDump.release;
                    if (signal.aborted)
                        throw sourceAbortError(signal);
                    if (disposed) {
                        captureError("The PGlite capture source closed while export was settling", "AGENT_BACKUP_V2_CAPTURE_CLOSED", { agentId: runtime.agentId }, { severity: "ephemeral" });
                    }
                    const { dump, preflight } = boundedDump;
                    if (!isPgliteDump(dump)) {
                        captureError("PGlite dumpDataDir() did not return a streamable Blob/File", "AGENT_BACKUP_V2_PGLITE_DUMP_NOT_STREAMABLE", { agentId: runtime.agentId }, { severity: "fatal" });
                    }
                    if (!provenPreflight ||
                        preflight !== provenPreflight ||
                        !Number.isSafeInteger(preflight.estimatedArchiveBytes) ||
                        preflight.estimatedArchiveBytes < 0) {
                        captureError("The managed PGlite exporter skipped its required preflight", "AGENT_BACKUP_V2_PGLITE_PREFLIGHT_UNPROVEN", { agentId: runtime.agentId }, { severity: "fatal" });
                    }
                    if (dump.size > preflight.estimatedArchiveBytes) {
                        captureError("PGlite export exceeds its preflighted archive bound", "AGENT_BACKUP_V2_PGLITE_DUMP_EXCEEDS_PREFLIGHT", {
                            agentId: runtime.agentId,
                            dumpBytes: dump.size,
    ...
    return {
        ...
        async *open(signal) {
            ...
            if (opened) {
                captureError("A prepared PGlite export can only be consumed once", "AGENT_BACKUP_V2_PGLITE_DUMP_ALREADY_CONSUMED", { agentId: runtime.agentId }, { severity: "fatal" });
            }
            opened = true;
            const dump = prepared?.dump;
            if (!dump) {
                captureError("The prepared PGlite export is unavailable", "AGENT_BACKUP_V2_PGLITE_DUMP_FAILED", { agentId: runtime.agentId }, { severity: "fatal" });
            }
            ...
            try {
                ...
                abort = () => {
                    ...
                    void reader?.cancel(abortReason(signal)).catch((error) => {
                        logger.warn({ err: error instanceof Error ? error.message : String(error) }, "[agent-backup-v2] PGlite reader cancellation failed");
                    });
                };
                signal.addEventListener("abort", abort, { once: true });
                if (signal.aborted)
                ...
                for (;;) {
                    ...
                    if (next.value.length === 0) {
                        captureError("PGlite export made no progress", "AGENT_BACKUP_V2_ZERO_PROGRESS", { componentName: "database" }, { severity: "fatal" });
                    }
...
function resolveCapturePgliteDir(config: ElizaConfig): string {
    ...
    const workspace = config.agents?.defaults?.workspace ?? resolveDefaultAgentWorkspaceDir();
...
function captureHasPostgresUrl(runtime: AgentBackupV2CaptureRuntime): boolean {
    const runtimeSetting = runtime.getSetting?.("POSTGRES_URL");
    return ((typeof runtimeSetting === "string" && runtimeSetting.trim().length > 0) ||
        Boolean(process.env.POSTGRES_URL?.trim() || process.env.DATABASE_URL?.trim()));
}
function captureStateFileInclude(relativePath: string, pgliteRelativePath: string | null): boolean {
    // Plugin import generations are rebuilt from installed sources on boot.
    if (relativePath === "plugins/.runtime-imports" ||
        relativePath.startsWith("plugins/.runtime-imports/"))
        return false;
...
export function createDefaultAgentBackupV2CaptureSources(runtime: AgentBackupV2CaptureRuntime, config: ElizaConfig): readonly AgentBackupV2CaptureComponentSource[] {
    if (captureHasPostgresUrl(runtime)) {
        captureError("Capture v2 currently requires the sandbox-local PGlite database export", "AGENT_BACKUP_V2_POSTGRES_UNSUPPORTED", { agentId: runtime.agentId }, { severity: "fatal" });
    }
    const stateDir = resolveStateDir();
    const pgliteDir = resolveCapturePgliteDir(config);
    if (pgliteDir === ":memory:" || pgliteDir.includes("://")) {
        captureError("Capture v2 requires a filesystem-backed PGlite database", "AGENT_BACKUP_V2_PGLITE_NOT_FILESYSTEM", { pgliteDir }, { severity: "fatal" });
    }
    const pgliteStateFilesExclusion = resolveStateFilesPgliteExclusion(stateDir, pgliteDir);
    const physicalPgliteDir = resolveCaptureDirectoryIdentity(pgliteDir, "pglite");
    ...
    return Object.freeze([
        ...
        fileSetSource({
            ...
            contentKind: "file-set",
            consistency: "best-effort",
        }, stateDir, (relativePath) => captureStateFileInclude(relativePath, pgliteStateFilesExclusion)),
        fileSetSource({
            name: "vault",
            format: "file-set-v1",
            compression: "none",
            contentKind: "file-set",
            consistency: "best-effort",
        }, stateDir, vaultFileInclude),
    ]);
}
function assertComponentSources(components: readonly AgentBackupV2CaptureComponentSource[]): void {
    if (components.length === 0 ||
        components.length > AGENT_BACKUP_CAPTURE_V2_LIMITS.maxComponents) {
        captureError("Capture component count is outside its bound", "AGENT_BACKUP_V2_COMPONENT_COUNT", { componentCount: components.length }, { severity: "fatal" });
    }
    ...
    for (const source of components) {
        ...
        if (previousName && source.descriptor.name <= previousName) {
            captureError("Capture components must be unique and lexicographically ordered", "AGENT_BACKUP_V2_COMPONENT_ORDER", { previousName, componentName: source.descriptor.name }, { severity: "fatal" });
        }
        previousName = source.descriptor.name;
    }
}
/** Stream an injected capture source; used by production and large real tests. */
export async function* streamAgentBackupV2Capture(options: Readonly<StreamAgentBackupV2CaptureOptions>): AsyncGenerator<Uint8Array> {
    const request = parseAgentBackupCaptureV2Request(options.request);
    const now = options.now ?? Date.now;
    if (request.agentId !== options.agentId) {
        captureError("Capture request agent does not match the active runtime", "AGENT_BACKUP_V2_AGENT_MISMATCH", { requestedAgentId: request.agentId, activeAgentId: options.agentId }, { severity: "fatal" });
    }
    const deadlineAheadMs = request.deadlineEpochMs - now();
    if (deadlineAheadMs <= 0 ||
        deadlineAheadMs > AGENT_BACKUP_CAPTURE_V2_LIMITS.maxDeadlineAheadMs) {
        captureError("Capture request deadline is expired or too far in the future", "AGENT_BACKUP_V2_INVALID_DEADLINE", { deadlineEpochMs: request.deadlineEpochMs, deadlineAheadMs }, { severity: "fatal" });
    }
    ...
    const signal = options.signal
        ? AbortSignal.any([options.signal, controller.signal])
        : controller.signal;
    const deadlineTimer = setTimeout(() => controller.abort(new AgentBackupV2CaptureError("Agent backup capture deadline exceeded", "AGENT_BACKUP_V2_CAPTURE_DEADLINE_EXCEEDED", { operationId: request.operationId }, { severity: "ephemeral" })), Math.min(deadlineAheadMs, 2147483647));
    ...
    const serialize = async (header: AgentBackupCaptureV2FrameHeader, payload?: Uint8Array, includeInChain = true): Promise<Uint8Array> => {
        assertCaptureActive(request, signal, now);
        const wire = await serializeAgentBackupCaptureV2Frame({ header, payload }, nodeSha256Digest);
        if (includeInChain) {
    ...
    try {
        for (const source of options.components) {
            if (!source.prepare)
                continue;
            try {
                await awaitWithCaptureControl(() => source.prepare?.(signal) ?? Promise.resolve(), request, signal, now);
            }
            catch (error) {
                if (error instanceof AgentBackupV2CaptureError)
                    throw error;
                throw new AgentBackupV2CaptureError(`Capture source preparation failed for component ${source.descriptor.name}`, "AGENT_BACKUP_V2_SOURCE_PREPARE_FAILED", {
                    operationId: request.operationId,
        ...
        for (const [componentIndex, source] of options.components.entries()) {
            ...
            try {
                for (;;) {
                    const next = await awaitWithCaptureControl(() => iterator.next(), request, signal, now);
                    if (next.done) {
                    ...
                    if (!(bytes instanceof Uint8Array)) {
                        captureError(`Component ${source.descriptor.name} yielded non-byte data`, "AGENT_BACKUP_V2_INVALID_SOURCE_CHUNK", { componentName: source.descriptor.name }, { severity: "fatal" });
                    }
                    if (bytes.length > AGENT_BACKUP_CAPTURE_V2_LIMITS.maxFramePayloadBytes) {
                        captureError(`Component ${source.descriptor.name} exceeded the frame payload bound`, "AGENT_BACKUP_V2_SOURCE_CHUNK_TOO_LARGE", { componentName: source.descriptor.name, bytes: bytes.length }, { severity: "fatal" });
                    }
                    if (bytes.length === 0 &&
                        (entry?.fileSizeBytes !== 0 || entry.fileOffsetBytes !== 0)) {
                        captureError(`Component ${source.descriptor.name} made no progress`, "AGENT_BACKUP_V2_ZERO_PROGRESS", { componentName: source.descriptor.name }, { severity: "fatal" });
                    }
                    if (totalPlainBytes >
                        AGENT_BACKUP_CAPTURE_V2_LIMITS.maxPlainBytes - bytes.length) {
                        captureError("Capture exceeds the plaintext byte limit", "AGENT_BACKUP_V2_PLAIN_BYTES_LIMIT", { observedBytes: totalPlainBytes + bytes.length }, { severity: "fatal" });
                    }
                    if (totalDataFrames >= AGENT_BACKUP_CAPTURE_V2_LIMITS.maxDataFrames) {
                        captureError("Capture exceeds the data-frame limit", "AGENT_BACKUP_V2_DATA_FRAME_LIMIT", { observedFrames: totalDataFrames + 1 }, { severity: "fatal" });
                    }
                ...
                if (error instanceof AgentBackupV2CaptureError)
                    throw error;
                throw new AgentBackupV2CaptureError(`Capture source failed for component ${source.descriptor.name}`, "AGENT_BACKUP_V2_SOURCE_FAILED", {
                    operationId: request.operationId,
                    componentName: source.descriptor.name,
                }, { cause: error, severity: "ephemeral" });
            }
                ...
                if (!sourceCompleted && !controller.signal.aborted) {
                    controller.abort(new AgentBackupV2CaptureError("Agent backup capture source closed before completion", "AGENT_BACKUP_V2_CAPTURE_CLOSED", {
                        operationId: request.operationId,
                        componentName: source.descriptor.name,
                    }, { severity: "ephemeral" }));
                }
        ...
        if (!controller.signal.aborted) {
            controller.abort(new AgentBackupV2CaptureError("Agent backup capture iterator closed", "AGENT_BACKUP_V2_CAPTURE_CLOSED", { operationId: request.operationId }, { severity: "ephemeral" }));
        }
    }
}
/** Create the production capture stream for one authenticated runtime. */
export function createAgentBackupV2Capture(runtime: AgentBackupV2CaptureRuntime, config: ElizaConfig, request: AgentBackupCaptureV2Request, options: Readonly<CreateAgentBackupV2CaptureOptions> = {}): AsyncIterable<Uint8Array> {
