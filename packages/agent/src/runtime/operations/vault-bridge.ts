...
async function readOptimizedPromptIntegrityKeyOrRecover(
  ...
  try {
    ...
    // error-policy:J4 This one internal random key is safely regenerable. The
    // unreadable ciphertext remains quarantined for forensic inspection, while
    // every user/provider secret and every other Vault error stays fail-closed.
