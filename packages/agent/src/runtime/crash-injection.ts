...
export type CrashInjectionConfig = Map<CrashInjectionPoint, CrashInjectionFault>;
 ...
 */
export function parseCrashInjectionSpec(raw: string | undefined): CrashInjectionConfig {
    ...
    for (const entry of raw.split(",")) {
        ...
        const arg = argNum !== undefined && Number.isFinite(argNum) ? argNum : undefined;
...
function isProductionRuntime(env: NodeJS.ProcessEnv): boolean {
    return (env.NODE_ENV === "production" || env.ELIZA_BUILD_VARIANT === "production");
}
/**
 ...
 */
export function resolveCrashInjectionConfig(env: NodeJS.ProcessEnv = process.env): CrashInjectionConfig {
    const spec = env[ENV_SPEC];
    if (!spec?.trim())
        return new Map();
    if (isProductionRuntime(env) && env[ENV_ALLOW_PROD] !== "1") {
        warn(`${ENV_SPEC} is set in a production runtime but ${ENV_ALLOW_PROD} != 1 — refusing to arm fault injection.`);
        return new Map();
    }
 ...
 */
export function armCrashInjection(env: NodeJS.ProcessEnv = process.env): CrashInjectionConfig {
    armed = resolveCrashInjectionConfig(env);
    if (armed.size > 0) {
        const plan = [...armed.entries()]
            .map(([p, f]) => `${p}=${f.mode}${f.arg !== undefined ? `:${f.arg}` : ""}`)
            .join(", ");
...
function executeFault(point: CrashInjectionPoint, fault: CrashInjectionFault): void {
    ...
    switch (fault.mode) {
        ...
        case "reject":
            ...
            Promise.reject(new Error(`[crash-injection] injected unhandled rejection at "${point}"`));
            break;
        case "oom": {
 ...
 */
export function maybeInjectFault(point: CrashInjectionPoint): undefined | Promise<never> {
    if (armed === null)
        armCrashInjection();
    const fault = armed?.get(point);
    if (!fault || tripped.has(point))
        return;
    tripped.add(point);
    if (fault.mode === "hang") {
        const ms = Number.isFinite(fault.arg) ? (fault.arg as number) : undefined;
        warn(`injecting hang at "${point}"${ms ? ` for ${ms}ms` : " (indefinite)"}`);
