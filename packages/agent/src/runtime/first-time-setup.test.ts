...
describe("bindCloudProvider", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "eliza-bind-cloud-"));
  ...
  it("writes WALLET_SOURCE_* to config.env when flag on + cloud bound", async () => {
    ...
    expect(process.env.ELIZA_CLOUD_EVM_ADDRESS).toBe("0xabc");
    expect(process.env.ELIZA_CLOUD_SOLANA_ADDRESS).toBe("So1abc");
    ...
    expect(raw).toContain("ELIZA_CLOUD_EVM_ADDRESS=0xabc");
    expect(raw).toContain("ELIZA_CLOUD_SOLANA_ADDRESS=So1abc");
  ...
  it("skips writes when env is already bound to cloud (idempotent)", async () => {
    process.env.ENABLE_CLOUD_WALLET = "1";
    process.env.ELIZA_CLOUD_EVM_ADDRESS = "0xabc";
