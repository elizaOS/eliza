...
import { packageExportCandidates } from "./workspace-plugin-source.ts";
import { pathToFileURL } from "node:url";
import { type Dirent } from "node:fs";
import { type ElizaConfig } from "../config/config.ts";
import { type HttpPlugin as Plugin } from "@elizaos/shared";
import { type PackageExportEntry } from "./workspace-plugin-source.ts";
 ...
 */
export const STATIC_ELIZA_PLUGIN_LOADERS: Record<string, () => Promise<unknown>> = {};
 ...
 */
export async function scanDropInPlugins(dir: string): Promise<Record<string, PluginInstallRecord>> {
...
export function resolveElizaPluginImportSpecifier(pluginName: string, runtimeModuleUrl = import.meta.url): string {
...
export function repairBrokenInstallRecord(config: ElizaConfig, pluginName: string): boolean {
...
/** @internal Exported for testing. */
export async function resolvePackageEntry(pkgRoot: string, exportSubpath = "."): Promise<string> {
    ...
    try {
        ...
        if (typeof pkg.exports === "object" &&
            ...
            const resolved = packageExportCandidates(entry).map((candidate) => path.resolve(pkgRoot, candidate));
