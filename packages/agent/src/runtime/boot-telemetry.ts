...
function telemetryDisabled(): boolean {
    return (process.env.ELIZA_DISABLE_TELEMETRY === "1" ||
 ...
 */
async function writeTelemetryFile(dir: string, file: string, data: unknown): Promise<void> {
    ...
    try {
        ...
        logger.warn(`[boot-telemetry] Failed to write ${targetPath}: ${err instanceof Error ? err.message : String(err)}`);
    }
}
/**
...
export async function recordBootEvent(label: string): Promise<void> {
    ...
    const event: BootEvent = {
        ...
        at: Date.now(),
        pid: process.pid,
        spawnedAtMs: Number.isFinite(spawnedAtMs) && spawnedAtMs > 0 ? spawnedAtMs : null,
    ...
    try {
        ...
        if (existing !== null) {
            try {
                ...
                if (Array.isArray(parsed)) {
                    ...
                    logger.warn(`[boot-telemetry] Ignoring invalid restart history in ${targetPath}: expected an array`);
                }
            }
            catch (error) {
                // error-policy:J3 corrupt local telemetry is explicit before replacement.
                logger.warn(`[boot-telemetry] Replacing invalid restart history in ${targetPath}: ${error instanceof Error ? error.message : String(error)}`);
                events = [];
            }
        }
        events.push(event);
        ...
        // error-policy:J7 telemetry failure is warned but cannot kill runtime boot.
        logger.warn(`[boot-telemetry] Failed to record boot event: ${err instanceof Error ? err.message : String(err)}`);
    }
}
