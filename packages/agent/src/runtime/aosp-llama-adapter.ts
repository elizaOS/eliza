...
class AospLlamaAdapter implements AospLoader {
  ...
  async loadModel(args: AospLlamaLoadOptions): Promise<void> {
    ...
    const contextSize =
      args.contextSize ?? readEnvInt("ELIZA_LLAMA_N_CTX", 8192);
    const maxThreads = args.maxThreads ?? readEnvInt("ELIZA_LLAMA_THREADS", 0);
    ...
    try {
      ...
      const nBatchParam = readEnvInt("ELIZA_LLAMA_N_BATCH", 2048);
