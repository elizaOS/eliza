...
const loadOptionalPlugin = async (packageName: string): Promise<unknown> => {
  try {
    ...
    if (packageName === "@elizaos/plugin-commands") {
      // plugin-commands does not ship type declarations
      // (dist/index.d.ts is not generated by its build script).
      return await import(
        // @ts-expect-error — no declaration file; resolved at runtime.
