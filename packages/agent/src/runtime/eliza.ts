...
import { AutonomyService } from "@elizaos/plugin-assistant";
...
import { CUSTOM_PLUGINS_DIRNAME as CUSTOM_RUNTIME_PLUGINS_DIRNAME } from "./plugin-types.ts";
import { ChannelType } from "@elizaos/core";
import { DEFAULT_ELIZA_CLOUD_TEXT_MODEL } from "@elizaos/core/contracts/service-routing";
import { DEFERRED_CORE_PLUGINS } from "./core-plugins.ts";
import { ElizaError } from "@elizaos/core";
import { EmbeddingDimensionProbeError } from "@elizaos/core";
import { LEAN_CHAT_PLUGINS } from "./core-plugins.ts";
import { MESSAGE_SOURCE_CLIENT_CHAT } from "@elizaos/core";
import { OPTIONAL_CORE_PLUGINS } from "./core-plugins.ts";
import { OPTIONAL_STATIC_PLUGIN_OVERRIDES } from "./optional-plugins.ts";
import { OPTIONAL_STATIC_PLUGIN_REGISTRATIONS } from "./optional-plugins.ts";
import { PGLITE_ERROR_CODES } from "./pglite-error-compat.ts";
import { PROVIDER_PLUGIN_MAP } from "./plugin-collector.ts";
import { SQLITE_PLUGIN } from "./database-selection.ts";
...
import { abandonAgentWalletAddressCacheSession } from "../api/wallet.ts";
import { activateAgentWalletAddressCacheSession } from "../api/wallet.ts";
import { addLogListener } from "@elizaos/core";
import { applyDevCloudConfigAuthority } from "../config/dev-cloud-env-authority.ts";
...
import { beginAgentWalletAddressCacheSession } from "../api/wallet.ts";
import { beginDeferredAgentWalletAddressCacheSession } from "../api/wallet.ts";
...
import { cacheAgentWalletAddresses } from "../api/wallet.ts";
import { cancelAndDrainDeferredBoot } from "./deferred-boot-owner.ts";
...
import { configFileExists } from "../config/config.ts";
import { constructWithRuntimeInstallationIdentity } from "./runtime-installation-id.ts";
import { createAgentProcessLifecycle } from "./process-lifecycle.ts";
import { createAssistantPlugins } from "./assistant-plugins.ts";
import { createBootContext } from "./boot-pipeline.ts";
import { createDevCloudConfigAuthorityView } from "../config/dev-cloud-env-authority.ts";
import { createDevCloudRuntimeSettingsAuthorityOverlay } from "../config/dev-cloud-env-authority.ts";
import { createElizaPlugin } from "./eliza-plugin.ts";
import { createHookEvent } from "../hooks/index.ts";
import { createMessageMemory } from "@elizaos/core";
import { createPgliteInitError } from "./pglite-error-compat.ts";
import { createRequire } from "node:module";
import { debugLogResolvedContext } from "../api/plugin-validation.ts";
...
import { getPgliteErrorCode } from "./pglite-error-compat.ts";
import { getWalletAddresses } from "../api/wallet.ts";
import { hasDurableHostVault } from "./host-bridge.ts";
import { hydrateConfigEnvForBoot } from "./runtime-settings.ts";
import { hydrateSelectedProviderCredentialFromVault } from "./provider-vault-credential.ts";
import { initializeBlockingCoreRuntimeForBoot } from "./blocking-core-boot.ts";
import { installDatabaseTrajectoryLogger } from "./trajectory-persistence.ts";
import { installProcessSignalHandlers } from "./process-lifecycle.ts";
...
import { isDevCloudEnvOwnedKey } from "../config/dev-cloud-env-authority.ts";
import { isElizaSettingsDebugEnabled } from "@elizaos/core/settings-debug";
import { isLikelyOpenAiTextModel } from "./provider-model-defaults.ts";
import { isMobilePlatform } from "@elizaos/core/runtime-env";
import { isSQLiteSelected } from "./database-selection.ts";
import { isVaultRef } from "./operations/vault-bridge.ts";
import { isWorkspacePluginSourceFallbackAllowed } from "./plugin-resolver.ts";
import { loadEffectiveElizaConfig } from "../config/config.ts";
import { loadElizaConfig } from "../config/config.ts";
import { loadHooks } from "../hooks/index.ts";
...
import { pendingDeferredBootTaskCount } from "./deferred-boot-owner.ts";
import { preregisterCorePluginsInDependencyWaves } from "./blocking-core-boot.ts";
import { readAliasedEnv } from "@elizaos/core/utils/env";
import { readFileSync } from "node:fs";
import { recordBootEvent } from "./boot-telemetry.ts";
import { recordBootTelemetry } from "./boot-telemetry.ts";
...
import { resolveBootPlan } from "./boot-pipeline.ts";
import { resolveConfigEnvForProcess } from "./operations/vault-bridge.ts";
import { resolveConnectorSecretSettings } from "./operations/vault-bridge.ts";
...
import { resolveOptimizedPromptIntegrityKey } from "./operations/vault-bridge.ts";
import { resolvePlugins } from "./plugin-resolver.ts";
import { resolvePreferredProviderId } from "./model-resolution.ts";
import { resolvePreferredProviderPluginName } from "./model-resolution.ts";
import { resolvePrimaryModel } from "./model-resolution.ts";
import { resolveSandboxRouteAgentId } from "./sandbox-character.ts";
...
import { restoreDevCloudEnvAuthority } from "../config/dev-cloud-env-authority.ts";
...
import { runtimeDocumentsEnabled } from "./native-runtime-features.ts";
import { runtimeTrajectoriesEnabled } from "./native-runtime-features.ts";
import { seedBundledDocuments } from "./default-documents.ts";
import { selectDatabasePluginNames } from "./database-selection.ts";
import { selectedDatabasePlugin } from "./database-selection.ts";
import { setEnvIfMissing } from "./provider-model-defaults.ts";
import { setTeeBootGateState } from "../services/tee-boot-gate-state.ts";
import { settingsDebugCloudSummary } from "@elizaos/core/settings-debug";
import { shouldBootstrapWorkspaceInitFiles } from "../shared/workspace-resolution.ts";
...
import { startMemorySampler } from "./boot-telemetry.ts";
import { startMemoryWatchdog } from "./memory-watchdog.ts";
import { stopMemorySampler } from "./boot-telemetry.ts";
import { stringToUuid } from "@elizaos/core";
import { subAgentCredentialsPlugin } from "@elizaos/plugin-assistant";
import { syncSolanaPublicKeyEnv } from "../api/wallet.ts";
import { teeBootGateBlocksSecrets } from "../services/tee-boot-gate-state.ts";
import { trackDeferredBootTask } from "./deferred-boot-owner.ts";
import { triggerHook } from "../hooks/index.ts";
import { type AgentHostBridge } from "./host-bridge.ts";
import { type AgentProcessLifecycle } from "./process-lifecycle.ts";
import { type AgentWalletAddressCacheSession } from "../api/wallet.ts";
import { type BootContext } from "./boot-pipeline.ts";
import { type BootPhaseName } from "./boot-pipeline.ts";
import { type Component } from "@elizaos/core";
import { type ElizaBootResult } from "./boot-pipeline.ts";
import { type ElizaConfig } from "../config/config.ts";
import { type Entity } from "@elizaos/core";
import { type IAgentRuntime } from "@elizaos/core";
import { type LoadHooksOptions } from "../hooks/index.ts";
import { type LogEntry } from "@elizaos/core";
import { type Plugin } from "@elizaos/core";
import { type PluginResolutionPhase } from "./plugin-resolver.ts";
import { type Provider } from "@elizaos/core";
import { type ResolvedPlugin as RuntimeResolvedPlugin } from "./plugin-types.ts";
import { type RuntimeSettingsProjectionOptions } from "./runtime-settings.ts";
import { type RuntimeStopOptions } from "@elizaos/core";
import { type SandboxMode } from "../services/sandbox-manager.ts";
import { type TargetInfo } from "@elizaos/core";
import { type TeeBootGate } from "../services/tee-boot-gate.ts";
import { type UUID } from "@elizaos/core";
import { unlinkSync } from "node:fs";
import { validateRuntimeContext } from "../api/plugin-validation.ts";
import { validateViewActionMap } from "./view-action-affinity.ts";
import { warnOnUnmatchedActionRolePolicyKeys } from "@elizaos/core";
...
export { hydrateConfigEnvForBoot, isEnvKeyAllowedForForwarding, } from "./runtime-settings.ts";
export { CUSTOM_PLUGINS_DIRNAME, EJECTED_PLUGINS_DIRNAME, findRuntimePluginExport, mergeDropInPlugins, type PluginModuleShape, type ResolvedPlugin, repairBrokenInstallRecord, resolveElizaPluginImportSpecifier, resolvePackageEntry, STATIC_ELIZA_PLUGINS, scanDropInPlugins, } from "./plugin-types.ts";
...
async function runVaultBootHydration(): Promise<void> {
    ...
    try {
        ...
        logger.warn(`[wallet][os-store] deferred boot hydrate skipped: ${err instanceof Error ? err.message : String(err)}`);
    }
...
type RemoteCodingRunnerModule = typeof import("../services/remote-coding-runner.ts");
async function loadRemoteCodingRunnerModule(): Promise<RemoteCodingRunnerModule> {
    ...
    return (await import(
    /* @vite-ignore */ moduleId)) as RemoteCodingRunnerModule;
}
...
async function loadRequiredPluginSql(): Promise<typeof import("@elizaos/plugin-sql")> {
    try {
        return await import(/* @vite-ignore */ "@elizaos/plugin-sql");
    }
    catch (err) {
        const sourceEntry = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../../plugins/plugin-sql/src/index.ts");
        ...
        return (await import(pathToFileURL(sourceEntry).href)) as typeof import("@elizaos/plugin-sql");
    }
}
...
let _pluginSqlPromise: Promise<typeof import("@elizaos/plugin-sql")> | null = null;
...
async function getPluginSqlite(): Promise<typeof import("@elizaos/plugin-sqlite")> {
    if (isMobilePlatform()) {
        throw new ElizaError("SQLite storage requires the standalone Node 24.15.0 host", { code: "SQLITE_RUNTIME_UNSUPPORTED" });
    }
    return import(/* @vite-ignore */ String(SQLITE_PLUGIN));
}
let _pluginLocalEmbeddingPromise: Promise<typeof import("@elizaos/plugin-local-inference") | null> | null = null;
async function getPluginLocalEmbedding(): Promise<typeof import("@elizaos/plugin-local-inference") | null> {
...
function getOptionalPlugin(packageName: string): Promise<unknown> {
    ...
    if (cached)
        return cached;
    const promise = loadOptionalPlugin(packageName, path.dirname(fileURLToPath(import.meta.url)));
...
const BLOCKING_STATIC_PLUGIN_LOADERS: Readonly<Record<string, {
    required: boolean;
    load: () => Promise<unknown>;
}>> = {
...
function buildBlockingStaticRegistrations(): CoreStaticPluginRegistration[] {
    return BLOCKING_CORE_PLUGINS.map((packageName) => {
        ...
        if (!loader) {
            ...
            throw new Error(`[boot] BLOCKING_CORE_PLUGINS lists ${packageName} but no blocking static loader is declared for it in eliza.ts`);
        }
        return {
            packageName,
...
function awaitOwnedPromise<T>(promise: Promise<T>, abortSignal?: AbortSignal): Promise<T> {
    if (!abortSignal)
        return promise;
...
async function registerStaticPluginPhase(phase: CoreStaticPluginPhase): Promise<void> {
    const registrations = CORE_STATIC_PLUGIN_REGISTRATIONS.filter((registration) => registration.phase === phase &&
        (!["@elizaos/plugin-sql", SQLITE_PLUGIN].includes(registration.packageName) ||
            registration.packageName === selectedDatabasePlugin()));
    logger.info(`[boot] resolving ${phase} plugins (${registrations.length})`);
    const trackImport = async (registration: CoreStaticPluginRegistration): Promise<void> => {
        ...
        try {
            ...
            if (!mod) {
                ...
                logger.debug(`[boot] ${registration.packageName} skipped after ${Date.now() - startedAt}ms: module unavailable`);
                return;
            }
            STATIC_ELIZA_PLUGINS[registration.registryName ?? registration.packageName] = mod;
            logger.debug(`[boot] ${registration.packageName} loaded in ${Date.now() - startedAt}ms`);
        }
        catch (err) {
            ...
            logger.debug(`[boot] ${registration.packageName} skipped after ${elapsed}ms: ${formatError(err)}`);
        }
    };
    if (phase === "deferred") {
...
async function ensureStaticPluginsRegisteredByName(packageNames: readonly string[], abortSignal?: AbortSignal): Promise<void> {
    ...
    if (missing.length > 0) {
        logger.debug(`[boot] no static registration for configured provider plugin(s): ${missing.join(", ")}`);
    }
    const registrationsPromise = Promise.all(registrations.map(async (registration) => {
        abortSignal?.throwIfAborted();
        ...
        try {
            ...
            if (mod) {
                ...
                logger.debug(`[boot] configured provider plugin ${registration.packageName} loaded before runtime initialization`);
            }
        }
        catch (err) {
            if (abortSignal?.aborted)
                throw err;
            logger.warn(`[boot] configured provider plugin ${registration.packageName} unavailable before runtime initialization: ${formatError(err)}`);
        }
    }));
    await awaitOwnedPromise(registrationsPromise, abortSignal);
}
async function ensureBlockingCoreStaticPluginsRegistered(abortSignal?: AbortSignal): Promise<void> {
    ...
    await awaitOwnedPromise(_blockingStaticPluginsRegistrationPromise, abortSignal);
}
export async function ensureDeferredCoreStaticPluginsRegistered(abortSignal?: AbortSignal): Promise<void> {
    ...
    await awaitOwnedPromise(_deferredStaticPluginsRegistrationPromise, abortSignal);
}
/**
 ...
 */
export async function ensureCoreStaticPluginsRegistered(abortSignal?: AbortSignal): Promise<void> {
...
export async function configureLocalEmbeddingPlugin(_plugin: Plugin, config?: ElizaConfig): Promise<void> {
    const { detectEmbeddingPreset, selectEmbeddingPresetFromHardware } = await import("@elizaos/plugin-local-inference/runtime/embedding-presets");
    let detectedPreset = detectEmbeddingPreset();
    let detectedGpuBackend: "cuda" | "metal" | "vulkan" | null = null;
    try {
        const { probeHardware } = await import("@elizaos/plugin-local-inference/services");
        ...
        logger.warn(`[eliza] Local embedding hardware probe failed; using sync preset fallback: ${formatError(err)}`);
    }
    const SQL_COMPATIBLE_EMBEDDING_DIMENSIONS = new Set([
        384, 512, 768, 1024, 1536, 2048, 3072,
    ]);
    const normalizeEmbeddingDimensions = (rawValue: string | undefined): string | undefined => {
        if (!rawValue)
            return undefined;
    ...
    const resolvedGpuLayers = configuredGpuLayers ??
        process.env.LOCAL_EMBEDDING_GPU_LAYERS ??
        String(detectedPreset.gpuLayers);
    const shouldDisableMmap = resolvedGpuLayers === "auto" &&
        (detectedGpuBackend === "metal" ||
            (detectedGpuBackend === null && process.platform === "darwin"));
    setEnvIfMissing("LOCAL_EMBEDDING_USE_MMAP", shouldDisableMmap ? "false" : "true");
    ...
    const embeddingConfigSummary = `${process.env.LOCAL_EMBEDDING_MODEL} (repo: ${process.env.LOCAL_EMBEDDING_MODEL_REPO ?? "auto"}, ` +
    ...
    if (lastLoggedLocalEmbeddingConfig !== embeddingConfigSummary) {
        ...
        logger.info(`[eliza] Configured local embedding env: ${embeddingConfigSummary}`);
    }
    else {
 ...
 */
export async function configureLocalEmbeddingEnvEarlyIfNeeded(config?: ElizaConfig): Promise<void> {
    ...
    try {
        ...
        logger.warn(`[eliza] Early local-embedding env configuration failed (deferred warmup will retry): ${formatError(err)}`);
    }
}
// ---------------------------------------------------------------------------
// Helpers
...
function readEffectiveEnvValue(config: ElizaConfig, key: string, env: NodeJS.ProcessEnv = process.env): string | undefined {
    return trimEnvString(env[key]) ?? readConfigEnvValue(config, key);
}
function readEffectiveCloudCredential(config: ElizaConfig, key: string, env: NodeJS.ProcessEnv = process.env): string | undefined {
    return (trimCloudCredential(env[key]) ??
...
function hasExplicitEmbeddingProviderConfig(config: ElizaConfig, env: NodeJS.ProcessEnv = process.env): boolean {
    return Boolean(readEffectiveEnvValue(config, "EMBEDDING_BASE_URL", env) ||
        readEffectiveEnvValue(config, "EMBEDDING_API_KEY", env));
}
const CLOUD_ROUTING_MODEL_ENV: ReadonlyArray<[
    string,
    string
]> = [
...
function mergeMissingCloudRoutingModelPins(config: ElizaConfig, env: NodeJS.ProcessEnv): boolean {
    const existingRouting = resolveServiceRoutingInConfig(config as Record<string, unknown>);
    const llmText = existingRouting?.llmText as Record<string, unknown> | undefined;
    if (!llmText)
        return false;
    const patch: Record<string, string> = {};
    for (const [envKey, routingField] of CLOUD_ROUTING_MODEL_ENV) {
        if (trimEnvString(llmText[routingField]))
            continue;
        const value = readEffectiveEnvValue(config, envKey, env);
        if (value)
            patch[routingField] = value;
    }
...
/** @internal Exported for regression coverage. */
export function ensureProvisionedCloudContainerConfig(config: ElizaConfig, env: NodeJS.ProcessEnv = process.env): boolean {
    ...
    const apiKey = trimCloudCredential(env.ELIZAOS_CLOUD_API_KEY) ??
    ...
    const baseUrl = trimEnvString(env.ELIZAOS_CLOUD_BASE_URL) ??
        trimEnvString(config.cloud?.baseUrl) ??
        readEffectiveEnvValue(config, "ELIZAOS_CLOUD_BASE_URL", env);
    const agentId = trimEnvString(config.cloud?.agentId) ??
    ...
    const directInferenceDisabledCloud = isExplicitFalseEnvValue(readEffectiveEnvValue(config, "ELIZAOS_CLOUD_USE_INFERENCE", env));
    ...
    if (directInferenceDisabledCloud && directProvider) {
        ...
        logger.info(`[eliza][cloud-topology] provisioned=true directProvider=${directProvider} -> runtime=local inference=false`);
        return true;
    }
    ...
    if (!topology.services.inference) {
        ...
        const cloudRouting = buildDefaultElizaCloudServiceRouting({
            ...
            excludeServices: isExplicitFalseEnvValue(readEffectiveEnvValue(config, "ELIZAOS_CLOUD_USE_EMBEDDINGS", env)) && hasExplicitEmbeddingProviderConfig(config, env)
                ? ["embeddings"]
                : undefined,
            nanoModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_NANO_MODEL", env),
            smallModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_SMALL_MODEL", env),
            mediumModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_MEDIUM_MODEL", env),
            largeModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_LARGE_MODEL", env),
            megaModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_MEGA_MODEL", env),
            responseHandlerModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_RESPONSE_HANDLER_MODEL", env),
            shouldRespondModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_SHOULD_RESPOND_MODEL", env),
            actionPlannerModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_ACTION_PLANNER_MODEL", env),
            plannerModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_PLANNER_MODEL", env),
            responseModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_RESPONSE_MODEL", env),
            mediaDescriptionModel: readEffectiveEnvValue(config, "ELIZAOS_CLOUD_MEDIA_DESCRIPTION_MODEL", env),
        });
        config.serviceRouting = {
            ...(existingRouting ?? {}),
            ...cloudRouting,
    ...
    if (changed) {
        logger.info("[eliza] Provisioned cloud container missing managed runtime topology; forcing Eliza Cloud routing in memory");
    }
    const finalTopology = resolveElizaCloudTopology(config as Record<string, unknown>);
    logger.info(`[eliza][cloud-topology] provisioned=true changed=${changed} -> runtime=${finalTopology.runtime} inference=${finalTopology.services.inference}`);
    return changed;
}
/** @internal Exported for regression coverage. */
export function shouldStartElizaCloudThinClient(config: ElizaConfig, env: NodeJS.ProcessEnv = process.env): boolean {
    if (isProvisionedCloudContainer(env)) {
        return false;
    }
    const deploymentTarget = resolveDeploymentTargetInConfig(config as Record<string, unknown>);
    return Boolean(deploymentTarget.runtime === "cloud" &&
        deploymentTarget.provider === "elizacloud" &&
        config.cloud?.apiKey &&
        config.cloud?.agentId?.trim());
}
function setConfigEnvValue(config: ElizaConfig, key: string, value: string): void {
    if (!config.env ||
        typeof config.env !== "object" ||
        Array.isArray(config.env)) {
        config.env = {};
...
export function normalizeOpenAiCompatibleProviderConfig(config: ElizaConfig, env: NodeJS.ProcessEnv = process.env): boolean {
    const cloudInferenceEnabled = resolveElizaCloudTopology(config as Record<string, unknown>).services.inference;
 ...
 */
export async function shutdownRuntime(runtime: AgentRuntime | null | undefined, context: string, options: RuntimeStopOptions = {}): Promise<void> {
    ...
    try {
        ...
        firstError = err;
        logger.warn(`[eliza] ${context}: memory telemetry flush failed: ${formatError(err)}`);
    }
    if (!runtime) {
        if (firstError)
            throw firstError;
    ...
    try {
        ...
        if (pendingDeferredBoot > 0) {
            logger.info(`[eliza] ${context}: cancelling and draining ${pendingDeferredBoot} deferred boot task(s)`);
        }
        await cancelAndDrainDeferredBoot(runtime);
    }
    catch (err) {
        firstError = err;
        logger.warn(`[eliza] ${context}: deferred boot drain failed: ${formatError(err)}`);
    }
    try {
...
type TrajectoryLoggerRegistrationStatus = "pending" | "registering" | "registered" | "failed" | "unknown";
...
async function startAndRegisterAutonomyService(runtime: AgentRuntime): Promise<AutonomyServiceLike> {
...
type TrajectoryLoggerRuntimeLike = {
    ...
    getServiceRegistrationStatus?: (serviceType: string) => TrajectoryLoggerRegistrationStatus;
};
async function waitForTrajectoriesService(runtime: AgentRuntime, context: string, abortSignal?: AbortSignal): Promise<void> {
    ...
    if (typeof runtimeLike.getServiceLoadPromise !== "function")
        return;
    try {
        await awaitOwnedPromise(runtimeLike.getServiceLoadPromise("trajectories"), abortSignal);
    }
    catch (err) {
        if (abortSignal?.aborted)
            throw err;
...
function ensureTrajectoryLoggerEnabled(runtime: AgentRuntime, context: string): void {
    if (!runtimeTrajectoriesEnabled(runtime)) {
        logger.info(`[eliza] Native trajectories disabled (${context})`);
        return;
    }
    const trajectoryLogger = runtime.getService("trajectories") as TrajectoryLoggerControl | null | undefined;
    if (!trajectoryLogger) {
...
async function installPromptOptimizationLayer(runtime: AgentRuntime, context: string, config?: ElizaConfig): Promise<void> {
    try {
        const { installPromptOptimizations } = await import("./prompt-optimization.ts");
 ...
 * tradeoff.
 */
async function wireTrajectoryCaptureService(runtime: AgentRuntime, context: string, abortSignal?: AbortSignal): Promise<void> {
...
// ---------------------------------------------------------------------------
function assertPersistentDatabaseRequired(runtime: Pick<AgentRuntime, "getSetting" | "agentId">): void {
...
export function applyConnectorSecretsToEnv(config: ElizaConfig): void {
    ...
    const connectors = config.connectors ?? (config as Record<string, unknown>).channels ?? {};
    for (const [channelName, channelConfig] of Object.entries(connectors)) {
        ...
        if (channelName === "whatsapp") {
            ...
            if (accounts &&
                ...
                const firstEnabledAccount = Object.values(accounts as Record<string, unknown>).find((account) => {
                    if (!account ||
 ...
 */
/** @internal Exported for testing. */
export async function resolveConnectorSecretsOverlayForBoot(config: ElizaConfig): Promise<Record<string, string>> {
    ...
    if (refKeys.length === 0)
        return {};
    if (isMobilePlatform() || readAliasedEnv("ELIZA_CLOUD_PROVISIONED") === "1") {
        logger.error(`[vault-bootstrap] connector vault ref(s) present but the vault is unavailable on this platform (fail-closed): ${refKeys.join(", ")}`);
        return {};
    }
    const { sharedVault } = importAppCoreRuntime();
    const { resolved, failures } = await resolveConnectorSecretSettings(connectorEnvVars, sharedVault());
    if (failures.length > 0) {
        // Key names only — never values, never the underlying vault error.
        logger.error(`[vault-bootstrap] connector vault ref(s) failed to resolve (fail-closed): ${failures.join(", ")}`);
    }
    return resolved;
}
/** @internal Exported for vault-backed cloud/provider boot coverage. */
export async function resolveConfigEnvVaultRefsForBoot(config: ElizaConfig): Promise<void> {
    if (isMobilePlatform() || readAliasedEnv("ELIZA_CLOUD_PROVISIONED") === "1") {
        return;
    }
    if (!config.env ||
    ...
    const { resolved, missing } = await resolveConfigEnvForProcess(configEnv, vault);
    ...
    if (varsBag && typeof varsBag === "object" && !Array.isArray(varsBag)) {
        const varsResult = await resolveConfigEnvForProcess(varsBag as Record<string, unknown>, vault);
    ...
    if (unresolved.length > 0) {
        logger.warn(`[vault-bootstrap] sentinel(s) without vault entry: ${unresolved.join(", ")}`);
    }
}
 ...
 */
export async function fetchJsonWithCloudTimeout(url: string, init: RequestInit = {}, options: {
    timeoutMs?: number;
    signal?: AbortSignal;
 ...
 */
/** @internal Exported for testing. */
export async function autoResolveDiscordAppId(tokenOverride?: string, timeoutMs = DEFAULT_CLOUD_FETCH_TIMEOUT_MS, callerSignal?: AbortSignal, fetchImpl: typeof fetch = globalThis.fetch): Promise<void> {
    if (process.env.DISCORD_APPLICATION_ID)
        return;
    const discordToken = tokenOverride ||
    ...
    const signal = callerSignal
        ...
        : timeoutSignal;
    try {
        const res = await fetchImpl("https://discord.com/api/v10/oauth2/applications/@me", {
 ...
 */
/** @internal Exported for testing. */
export async function autoFetchCloudGithubToken(agentId?: string, timeoutMs = DEFAULT_CLOUD_FETCH_TIMEOUT_MS, callerSignal?: AbortSignal, fetchImpl: typeof fetch = globalThis.fetch): Promise<CloudGithubTokenResult | null> {
    ...
    const cloudApiKey = resolveDevCloudAuthorityEnvValue("ELIZAOS_CLOUD_API_KEY")?.trim();
    const cloudBaseUrl = resolveDevCloudAuthorityEnvValue("ELIZAOS_CLOUD_BASE_URL")?.trim() ||
        "https://api.eliza.app";
    if (!cloudApiKey || !agentId)
        return null;
    const managedNs = resolveDevCloudAuthorityEnvValue("ELIZA_CLOUD_MANAGED_AGENTS_API_SEGMENT")?.trim() ||
        resolveDevCloudAuthorityEnvValue("ELIZAOS_CLOUD_MANAGED_AGENTS_API_SEGMENT")?.trim();
    if (!managedNs)
        return null;
    const timeoutSignal = AbortSignal.timeout(timeoutMs);
    const signal = callerSignal
 ...
 */
/** @internal Exported for testing. */
export function bindCloudGithubTokenToRuntime(runtime: Pick<IAgentRuntime, "getSetting" | "setSetting">, result: CloudGithubTokenResult | null): boolean {
...
export function applyCloudConfigToEnv(config: ElizaConfig): void {
    ...
    const devCloudSnapshot = applyDevCloudConfigAuthority(config as Record<string, unknown>);
    try {
...
function applyCloudConfigToEnvResolved(config: ElizaConfig): void {
    ...
    const topology = resolveElizaCloudTopology(config as Record<string, unknown>);
    const serviceRouting = resolveServiceRoutingInConfig(config as Record<string, unknown>);
    ...
    if (!cloud && !isCloudContainer && !topology.shouldLoadPlugin)
        return;
    ...
    const effectiveCloudApiKey = configuredCloudApiKey ??
    ...
    const inferenceAvailable = (topology.services.inference &&
        (isCloudContainer || Boolean(effectiveCloudApiKey))) ||
        (isCloudContainer &&
            !isExplicitFalseEnvValue(readEffectiveEnvValue(config, "ELIZAOS_CLOUD_USE_INFERENCE")));
    ...
    if (isElizaSettingsDebugEnabled()) {
        ...
        logger.debug(`[eliza][settings][runtime] applyCloudConfigToEnv inferenceConfigured=${inferenceConfigured} inferenceAvailable=${inferenceAvailable} shouldLoadPlugin=${shouldLoadCloudPlugin} isCloudContainer=${isCloudContainer} cloud=${JSON.stringify(settingsDebugCloudSummary(c))}`);
    }
    ...
    if (topology.servicesUnreconciled.length > 0) {
        logger.warn({ services: topology.servicesUnreconciled }, `[eliza][cloud] Config routes ${topology.servicesUnreconciled.join(", ")} to Eliza Cloud, but no Cloud credential is linked. Those capabilities fall back to their local/other providers until you sign in.`);
    }
    ...
    const hasExplicitCloudEmbeddingRoute = Boolean(embeddingRoute?.transport === "cloud-proxy" &&
        normalizeFirstRunProviderId(embeddingRoute.backend) === "elizacloud");
    const cloudEmbeddingsPolicy = readEffectiveEnvValue(config, "ELIZAOS_CLOUD_USE_EMBEDDINGS")
        ?.trim()
        .toLowerCase();
    ...
    if (shouldLoadCloudPlugin) {
        ...
        const effectiveCloudBaseUrl = configuredCloudBaseUrl ??
            readEffectiveEnvValue(config, "ELIZAOS_CLOUD_BASE_URL");
        logger.info(`[eliza] Cloud config: inference=${topology.services.inference}, runtime=${topology.runtime}, hasApiKey=${Boolean(effectiveCloudApiKey)}, apiKey=${cloudApiKeyFingerprint(effectiveCloudApiKey)}, baseUrl=${effectiveCloudBaseUrl ?? "(default)"}, isCloudContainer=${isCloudContainer}`);
    ...
    if (inferenceAvailable) {
        ...
        const actionPlannerModel = llmText?.actionPlannerModel ||
 ...
 */
function resolveEffectiveDbProvider(config: ElizaConfig): "postgres" | "pglite" | "sqlite" {
    if (isSQLiteSelected()) {
        if (config.database?.provider)
            throw new ElizaError("Remove the PostgreSQL/PGlite database provider when selecting SQLite", { code: "DATABASE_PROVIDER_CONFLICT" });
        return "sqlite";
    }
    if (config.database?.provider) {
...
export function applyDatabaseConfigToEnv(config: ElizaConfig): void {
    ...
    if (provider === "sqlite") {
        ...
        if (!sqlitePath || !path.isAbsolute(sqlitePath))
            throw new ElizaError("SQLite requires an absolute SQLITE_DATABASE_PATH inside agent state storage", { code: "SQLITE_PATH_REQUIRED" });
    ...
    if (provider === "postgres") {
        ...
        if (dataDir) {
            ...
            logger.debug(`[eliza] PGlite data dir: ${dataDir} (${alreadyExisted ? "existed" : "created"})`);
...
type PglitePidFileStatus = "missing" | "active" | "active-unconfirmed" | "cleared-stale" | "cleared-malformed" | "check-failed";
type PgliteRecoveryAction = "none" | "retry-without-reset" | "fail-active-lock" | "fail-manual-reset";
...
/** @internal Exported for testing. */
export function getPgliteRecoveryAction(err: unknown, dataDir: string): PgliteRecoveryAction {
    const code = getPgliteErrorCode(err);
    if (code === PGLITE_ERROR_CODES.ACTIVE_LOCK) {
...
function createManualResetRequiredPgliteError(dataDir: string, err: unknown): Error {
...
async function initializeDatabaseAdapter(runtime: AgentRuntime, config: ElizaConfig): Promise<void> {
...
async function withEntityCreateMutex<T>(runtimeWithBindings: RuntimeWithMethodBindings, fn: () => Promise<T>): Promise<T> {
    const previous = runtimeWithBindings.__elizaEntityCreateMutex;
    let release: () => void = () => { };
    runtimeWithBindings.__elizaEntityCreateMutex = new Promise<void>((resolve) => {
        release = resolve;
    });
    if (previous) {
        await previous;
...
function uniqueEntitiesById(entities: Entity[]): Entity[] {
    ...
    for (const entity of entities) {
        if (entity?.id)
            uniqueById.set(entity.id as UUID, entity);
    }
    return Array.from(uniqueById.values());
}
async function findMissingEntities(runtimeWithEntityWrites: RuntimeWithEntityWrites, deduped: Entity[]): Promise<Entity[]> {
    if (typeof runtimeWithEntityWrites.getEntitiesByIds !== "function") {
        return deduped;
    }
    try {
        const existing = (await runtimeWithEntityWrites.getEntitiesByIds(deduped.map((entity) => entity.id as UUID))) ?? [];
        const existingIds = new Set<UUID>();
        for (const entity of existing) {
            if (entity?.id)
                existingIds.add(entity.id as UUID);
        }
...
async function recoverMissingEntities(runtimeWithEntityWrites: RuntimeWithEntityWrites, missing: Entity[]): Promise<boolean> {
...
export function installRuntimeMethodBindings(runtime: AgentRuntime, devCloudAuthorityOverlay: Readonly<Record<string, string>> = {}): void {
    ...
    const hasDevCloudAuthorityOverlay = Object.keys(frozenDevCloudAuthorityOverlay).length > 0;
    ...
    runtime.getSetting = (key: string) => {
        ...
        if (hasDevCloudAuthorityOverlay && isDevCloudEnvOwnedKey(key)) {
            const authorityValue = frozenDevCloudAuthorityOverlay[key.toUpperCase()] ?? "";
            if (authorityValue === "true")
                return true;
            if (authorityValue === "false")
                return false;
...
async function registerSqlPluginWithRecovery(runtime: AgentRuntime, sqlPlugin: RuntimeResolvedPlugin, config: ElizaConfig): Promise<void> {
...
const REQUIRED_BLOCKING_CORE_PLUGINS = new Set(Object.entries(BLOCKING_STATIC_PLUGIN_LOADERS)
 ...
 */
export function resolveVisionModeSetting(config: ElizaConfig, env: NodeJS.ProcessEnv = process.env): string | undefined {
...
/** @internal Exported for testing. */
export function resolveWalletRuntimeSettings(config?: Partial<ElizaConfig>, env: NodeJS.ProcessEnv = process.env): Record<string, string> {
 ...
 */
export function buildRuntimeSettings(config: ElizaConfig, options: RuntimeSettingsProjectionOptions = {}): Record<string, string> {
...
export function resolveRuntimeProviderName(resolvedPlugins: readonly RuntimeResolvedPlugin[], pluginPackageName: string | undefined): string | undefined {
...
/** @internal Exported for routing regression coverage. */
export function resolveEmbeddingProviderPluginName(config: ElizaConfig): string | undefined {
    const route = resolveServiceRoutingInConfig(config as Record<string, unknown>)?.embeddings;
 ...
 */
export function resolveDeferredPluginRegistrationTimeoutMs(rawEnv?: string | null): number {
    ...
    if (!(parsed >= 1 && parsed <= MAX_DEFERRED_PLUGIN_REGISTRATION_TIMEOUT_MS)) {
        throw new ElizaError(`Invalid ELIZA_DEFERRED_PLUGIN_REGISTRATION_TIMEOUT_MS: "${rawEnv}". Expected a decimal integer between 1 and ${MAX_DEFERRED_PLUGIN_REGISTRATION_TIMEOUT_MS}.`, {
...
const walletAddressCacheSessionsByRuntime = new WeakMap<AgentRuntime, AgentWalletAddressCacheSession>();
/**
 ...
 */
export async function bootEliza(opts: StartElizaOptions = {}): Promise<ElizaBootResult<AgentRuntime, CloudRuntimeProxyLike>> {
    ...
    throw new ElizaError("Boot completed without a local runtime or cloud proxy", {
        code: "AGENT_BOOT_RESULT_MISSING",
        severity: "fatal",
    });
}
/** Builds the same initialized local runtime used by cold boot for replacement. */
export async function buildInitializedRuntime(options: BuildInitializedRuntimeOptions): Promise<AgentRuntime> {
    ...
    try {
        ...
        if (result.mode === "cloud") {
            throw new ElizaError("A local runtime replacement cannot switch to cloud thin-client mode", {
...
export async function startElizaProcess(opts: StartElizaOptions = {}): Promise<AgentRuntime | undefined> {
 ...
 */
export async function bootElizaRuntime(opts: BootElizaRuntimeOptions = {}): Promise<AgentRuntime> {
    ...
    try {
        ...
        if (result.mode === "cloud") {
            throw new ElizaError("bootElizaRuntime requires a local runtime but configuration selected cloud mode", {
 ...
 */
export async function startEliza(opts?: StartElizaOptions): Promise<AgentRuntime | undefined> {
    ...
    const bootContext = opts?.bootContext ?? createBootContext({ observePhase: opts?.onBootPhase });
    ...
    {
        const { applySandboxConnectorOwnership } = await import("./sandbox-character.ts");
    ...
    const connectorSecretsOverlay = await resolveConnectorSecretsOverlayForBoot(config);
    const discordAppIdPromise = autoResolveDiscordAppId(connectorSecretsOverlay.DISCORD_API_TOKEN, bootContext.policy.providerProbeTimeoutMs);
    const cloudGithubTokenPromise = autoFetchCloudGithubToken(config.cloud?.agentId?.trim(), bootContext.policy.providerProbeTimeoutMs);
    ...
    const deferredWatchdogTimeoutMs = resolveDeferredPluginRegistrationTimeoutMs(process.env.ELIZA_DEFERRED_PLUGIN_REGISTRATION_TIMEOUT_MS);
    ...
    if (readAliasedEnv("ELIZA_CLOUD_PROVISIONED") !== "1")
        try {
            ...
            await accountPool.applyAccountPoolApiCredentials({
                activeBackend: resolveServiceRoutingInConfig(config as Record<string, unknown>)?.llmText?.backend,
                accountStrategies: (config as Record<string, unknown> & {
                    accountStrategies?: Record<string, unknown>;
                }).accountStrategies,
                serviceRouting: resolveServiceRoutingInConfig(config as Record<string, unknown>),
            });
    ...
    try {
        const { applySubscriptionCredentialsLocal } = await import("@elizaos/auth/auth");
    ...
    subscriptionCredentialsDeferredPromise = (async () => {
        const { applySubscriptionCredentialsDeferred } = await import("@elizaos/auth/auth");
    ...
    {
        const { applyCanonicalFileBootToConfig } = await import("./canonical-file-boot.ts");
    ...
    if (process.env.ELIZA_DISABLE_VAULT_PROFILE_RESOLVER !== "1" &&
        ...
        try {
            const { sharedVault } = await importAppCoreRuntime();
            const { applyVaultProfilesForAgent } = await import("./vault-profile-resolver.ts");
    ...
    if (preferredProviderId &&
        ...
        if (providerCredential.status === "hydrated") {
            logger.info(`[provider-vault] hydrated selected provider=${providerCredential.providerId} env=${providerCredential.envKey} key=${providerCredential.vaultKey}`);
        }
        else if (providerCredential.status === "missing") {
            logger.warn(`[provider-vault] selected provider=${providerCredential.providerId} credential=missing env=${providerCredential.envKey}`);
        }
    }
    ...
    const blockDeferredPluginImports = bootContext.policy.blockDeferredPluginImports;
    const initialPluginResolutionPhase: PluginResolutionPhase = blockDeferredPluginImports ? "all" : "blocking";
    ...
    const initialForceIncludePluginNames = !blockDeferredPluginImports
        ...
        : [];
    await ensureStaticPluginsRegisteredByName(initialForceIncludePluginNames, opts?.abortSignal);
    opts?.abortSignal?.throwIfAborted();
    ...
    const sqlPlugin = resolvedPlugins.find((p) => p.name === selectedDatabasePlugin());
    const otherPlugins = resolvedPlugins.filter((p) => !PREREGISTER_PLUGINS.has(p.name));
    const preferredTextRuntimeProviderName = resolveRuntimeProviderName(resolvedPlugins, preferredProviderPluginName);
    const preferredEmbeddingRuntimeProviderName = resolveRuntimeProviderName(resolvedPlugins, resolveEmbeddingProviderPluginName(config));
    ...
    const sandboxMode: SandboxMode = sandboxModeStr === "light" ||
    ...
    if (isSandboxActive) {
        ...
        if (sandboxMode === "standard" || sandboxMode === "max") {
            const dockerSettings = (sandboxConfig as Record<string, unknown> | undefined)?.docker as Record<string, unknown> | undefined;
            const browserSettings = (sandboxConfig as Record<string, unknown> | undefined)?.browser as Record<string, unknown> | undefined;
            sandboxManager = new SandboxManager({
                mode: sandboxMode,
                image: (dockerSettings?.image as string) ?? undefined,
                containerPrefix: (dockerSettings?.containerPrefix as string) ?? undefined,
                ...
                browser: browserSettings
                    ? {
                        ...
                        enableNoVnc: (browserSettings.enableNoVnc as boolean) ?? undefined,
                        autoStart: (browserSettings.autoStart as boolean) ?? true,
                        autoStartTimeoutMs: (browserSettings.autoStartTimeoutMs as number) ?? undefined,
                    }
                    : undefined,
            });
            try {
    ...
    const devCloudRuntimeSettingsAuthorityOverlay = Object.freeze(createDevCloudRuntimeSettingsAuthorityOverlay(process.env, config));
    let runtime = await constructWithRuntimeInstallationIdentity({
        stateDirectory: resolveStateDir(),
        abortSignal: opts?.abortSignal,
        construct: (runtimeInstanceId) => new AgentRuntime({
            character,
            runtimeInstanceId,
    ...
    installRuntimeMethodBindings(runtime, devCloudRuntimeSettingsAuthorityOverlay);
    opts?.onRuntimeCreated?.(runtime);
    opts?.abortSignal?.throwIfAborted();
    ...
    if (process.env.ELIZA_LOCAL_LLAMA?.trim() === "1") {
        try {
            const { ensureAospLocalInferenceHandlers } = await import("@elizaos/plugin-native-inference");
        ...
        try {
            const { ensureMobileDeviceBridgeInferenceHandlers } = await import("@elizaos/plugin-native-inference/mobile-device-bridge-bootstrap");
    ...
    if (sqlPlugin) {
        ...
        logger.error(`[eliza] ${selectedDatabasePlugin()} was NOT found among resolved plugins. ` +
            `Loaded: [${loadedNames}]`);
        throw new Error(`${selectedDatabasePlugin()} is required but was not loaded. ` +
    ...
    const registerConnectorSetupService = async (): Promise<void> => {
        try {
            const { ConnectorSetupService } = await import("../services/connector-setup-service.ts");
    ...
    const registerConnectorCredentialStoreService = async (): Promise<void> => {
        try {
            const { ConnectorCredentialStoreService } = await import("../services/connector-credential-store.ts");
            await runtime.registerService(ConnectorCredentialStoreService);
        }
        catch (err) {
            logger.debug(`[eliza] ConnectorCredentialStoreService registration skipped: ${formatError(err)}`);
        }
    };
    ...
    const ensureConnectorCredentialStoreStarted = async (): Promise<void> => {
        ...
        try {
            ...
            logger.warn(`[eliza] ConnectorCredentialStoreService failed to start; connector OAuth credential writes will fail until it is restored (no non-durable fallback): ${formatError(err)}`);
        }
    };
    const registerRemoteCodingRunner = async (): Promise<void> => {
        ...
        if (!shouldLoadRemoteCodingRunnerForBoot(runtime))
            return;
        try {
            const { registerRemoteCodingCapabilityRouterIfEnabled } = await loadRemoteCodingRunnerModule();
            const result = await registerRemoteCodingCapabilityRouterIfEnabled(runtime);
    ...
    const initializeCoreRuntime = async (): Promise<void> => {
        ...
        if (runtime.isEmbeddingGenerationDisabled()) {
            const message = "[eliza] boot continuing with embedding generation disabled: memory writes persist without vectors until the deferred re-probe finds a working provider";
        ...
        await installPromptOptimizationLayer(runtime, "runtime.initialize()", config);
        ...
        trajectoryCaptureWiring = wireTrajectoryCaptureService(runtime, "runtime.initialize()", opts?.abortSignal);
    };
    ...
    const registerRemoteSigningIfEnabled = async (): Promise<void> => {
        ...
        try {
            const { sharedVault } = await importAppCoreRuntime();
            const { VaultSignerBackend } = await import("../services/vault-signer-backend.ts");
            const { createTeeGatedRemoteSigningService, RemoteSigningRuntimeService, } = await import("../services/remote-signing-service.ts");
            ...
            const svc = runtime.getService(RemoteSigningRuntimeService.serviceType) as InstanceType<typeof RemoteSigningRuntimeService> | null;
            if (!svc) {
    ...
    const applyPluginRoleGatingIfAvailable = async (): Promise<void> => {
        try {
            ...
            logger.error(`[eliza] Plugin provider role gating FAILED — sensitive providers may be ungated: ${formatError(err)}`);
        }
    };
    const registerConversationProximityProvider = async (): Promise<void> => {
        try {
            const { conversationProximityProvider } = await import("../providers/conversation-proximity.ts");
    ...
    const installServerSideWebSearchIfAvailable = async (): Promise<void> => {
        try {
            const { installServerSideWebSearch } = await import("./web-search-tools.ts");
    ...
    const registerWebFetchActionIfEnabled = async (): Promise<void> => {
        try {
            const { webFetch, isWebFetchEnabled } = await import("./actions/web-fetch.ts");
            ...
            if (registerFallbackActionIfAbsent(runtime, webFetch)) {
                ...
                logger.debug("[eliza] WEB_FETCH already provided by a loaded plugin; keyless fallback skipped");
            }
        }
        catch (err) {
    ...
    const registerWebSearchActionIfEnabled = async (): Promise<void> => {
        try {
            const { webSearch, isWebSearchEnabled } = await import("./actions/web-search.ts");
            ...
            if (registerFallbackActionIfAbsent(runtime, webSearch)) {
                ...
                logger.debug("[eliza] WEB_SEARCH already provided by a loaded plugin; keyless fallback skipped");
            }
        }
        catch (err) {
    ...
    const isAutonomyEnabled = (): boolean => ["true", "1"].includes((process.env.ENABLE_AUTONOMY ?? "").toLowerCase());
    const startAutonomyServiceIfEnabled = async (autonomyEnabled: boolean): Promise<void> => {
    ...
    const enableAutonomyLoopIfAvailable = async (autonomyEnabled: boolean): Promise<void> => {
        if (!autonomyEnabled)
            return;
    ...
    const warmEmbeddingModel = async (abortSignal: AbortSignal): Promise<void> => {
        abortSignal.throwIfAborted();
        if (process.env.NODE_ENV === "test")
            return;
    ...
    const startEmbeddingWarmup = async (abortSignal: AbortSignal): Promise<void> => {
        const canonicalEmbeddings = runtime.getSetting("ELIZA_CANONICAL_EMBEDDINGS_ENABLED");
        if (canonicalEmbeddings === false ||
            String(canonicalEmbeddings).trim().toLowerCase() === "false") {
            logger.info("[eliza] Skipping local embedding warmup — canonical service routing omits embeddings.");
            return;
        }
        try {
    ...
    const ensureAgentWalletsLazy = (abortSignal: AbortSignal): Promise<readonly import("./agent-wallets.ts").AgentWalletDescriptor[]> => {
        ...
        walletInitPromise = (async () => {
            try {
                ...
                abortSignal.throwIfAborted();
                const descriptors = await ensureAgentWallets(sharedVault(), agentId, "agent-bootstrap", abortSignal);
                abortSignal.throwIfAborted();
                const published = cacheAgentWalletAddresses(walletAddressCacheSession, {
                    evmAddress: descriptors.find((descriptor) => descriptor.chain === "evm")
                        ?.address ?? null,
                    solanaAddress: descriptors.find((descriptor) => descriptor.chain === "solana")
                        ?.address ?? null,
                });
                if (!published)
                    return descriptors;
    ...
    const initializeRuntimeServices = async (): Promise<void> => {
        ...
        await initializeBlockingCoreRuntimeForBoot({
            ...
            runtime,
            resolvedPlugins,
            requiredPluginNames: new Set(selectDatabasePluginNames(REQUIRED_BLOCKING_CORE_PLUGINS)),
    ...
    const registerDeferredRuntimePlugins = async (deferredResolvedPlugins: RuntimeResolvedPlugin[], abortSignal: AbortSignal): Promise<void> => {
        ...
        const registerDeferredPlugin = async (plugin: (typeof deferredPluginsForRuntime)[number]): Promise<void> => {
            const startedAt = Date.now();
            ...
            try {
                ...
                registrationWatchdog = setTimeout(() => {
                    ...
                    const error = new Error(`Registration exceeded ${deferredWatchdogTimeoutMs / 1000}s watchdog`);
                    logger.warn(`[eliza] deferred: Plugin ${plugin.name} registration exceeded the ${deferredWatchdogTimeoutMs / 1000}s watchdog; still waiting for a definitive result`);
                    ...
                    runtime.reportError("eliza.deferredPluginRegistrationWatchdog", error, {
                        plugin: plugin.name,
                        phase: "deferred-boot",
                ...
                logger.info(`[eliza] deferred: ✓ ${plugin.name} registered (${Date.now() - startedAt}ms${exceededWatchdog ? ", after watchdog" : ""})`);
            }
            catch (err) {
                if (abortSignal.aborted)
                    throw err;
        ...
        const preferredPlugin = preferredProviderPluginName
            ? deferredPluginsForRuntime.find((plugin) => plugin.name === preferredProviderPluginName)
            : undefined;
        const registrationQueue = preferredPlugin
            ? [
                preferredPlugin,
                ...deferredPluginsForRuntime.filter((plugin) => plugin.name !== preferredPlugin.name),
            ]
            : deferredPluginsForRuntime;
    ...
    const resolveDeferredPluginsForBoot = async (abortSignal: AbortSignal): Promise<RuntimeResolvedPlugin[]> => {
        if (blockDeferredPluginImports) {
    ...
    const runDeferredBoot = async (abortSignal: AbortSignal): Promise<void> => {
        ...
        if (!blockDeferredPluginImports) {
            const deferredResolvedPlugins = await resolveDeferredPluginsForBoot(abortSignal);
            abortSignal.throwIfAborted();
            ...
            await registerDeferredRuntimePlugins(deferredResolvedPlugins, abortSignal);
        ...
        try {
            ...
            if (err instanceof EmbeddingDimensionProbeError) {
                ...
                logger.warn({ attempts: err.attempts }, "[eliza] deferred embedding-dimension re-probe: every registered TEXT_EMBEDDING provider failed; embedding generation stays disabled (memory writes persist without vectors)");
            }
            else {
        ...
        try {
            const { seedOnboardingNotifications } = await import("./onboarding-notifications.ts");
            await seedOnboardingNotifications(runtime);
        }
        catch (err) {
            if (abortSignal.aborted)
                throw err;
            logger.warn(`[eliza] Failed to seed onboarding notifications: ${formatError(err)}`);
        }
        ...
        if (!isMobilePlatform()) {
            try {
                const { registerWalletBalanceDeltaProducer } = await import("./wallet-balance-delta.ts");
                ...
                logger.warn(`[eliza] Failed to register the wallet balance-delta watcher: ${formatError(err)}`);
        ...
        if (isProvisionedCloudContainer()) {
            const { registerSharedCutoverReminderDispatcher } = await import("./shared-cutover-reminder-dispatch.ts");
        ...
        validateViewActionMap(runtime, runtime.actions.map((a) => a.name), runtime.logger);
    ...
    try {
        ...
        const { port: actualApiPort } = await startApiServer({
            ...
            onRuntimeActivated: async (previousRuntime, activeRuntime) => {
                const walletAddressCacheSession = walletAddressCacheSessionsByRuntime.get(activeRuntime);
                if (!walletAddressCacheSession ||
                    !activateAgentWalletAddressCacheSession(walletAddressCacheSession)) {
                    throw new ElizaError("Replacement runtime wallet-address cache session is no longer current", {
                ...
                try {
                    ...
                    logger.warn(`[eliza] Previous runtime cleanup failed after swap: ${formatError(err)}`);
                }
            },
        });
        if (skipApiListen) {
            logger.info("[eliza] Local-agent IPC mode — API route kernel ready in-process, no TCP listener bound");
        }
        else {
    ...
    if (opts?.serverOnly) {
        ...
        if (sandboxRegistry) {
            ...
            sandboxRegistry.startHeartbeat(bootContext.policy.sandboxHeartbeatIntervalMs);
        }
...
export async function startInCloudMode(config: ElizaConfig, agentId: string, opts?: StartElizaOptions): Promise<AgentRuntime | undefined> {
