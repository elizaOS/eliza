...
import { resolveTrajectoryGate } from "@elizaos/core";
import { sanitizeTrajectoryJsonObject } from "@elizaos/core";
...
import { type TrajectoryProviderAccess } from "../types/trajectory.ts";
import { type TrajectorySkillInvocation } from "../types/trajectory.ts";
import { type TrajectoryStatus } from "../types/trajectory.ts";
import { type TrajectoryStep } from "../types/trajectory.ts";
import { type TrajectoryStepKind } from "../types/trajectory.ts";
 ...
 */
export function appendCompleteTrajectoryTextRecords(existing: unknown, additions: readonly string[], field: string): string[] {
...
export function inferTrajectoryLlmStepType(params: {
    ...
    if (purpose === "should_respond")
        return "should_respond";
    if (purpose === "compose_state" ||
        purpose === "evaluation" ||
        purpose === "reasoning" ||
        purpose === "response" ||
...
export function inferTrajectoryLlmTags(params: {
    ...
    if (stepType === "should_respond")
        push("routing");
    if (stepType === "compose_state")
        push("context");
    if (ORCHESTRATOR_STEP_TYPES.has(stepType) ||
        actionType.startsWith("orchestrator_")) {
        push("orchestrator");
    }
    return tags;
}
export function enrichTrajectoryLlmCall<T extends Record<string, unknown>>(call: T): T & {
    stepType?: string;
    tags?: string[];
} {
...
export function readRecordValue(record: Record<string, unknown>, keys: string[]): unknown {
...
function readGroupingValue(metadata: Record<string, unknown>, keys: string[]): string | undefined {
    return toOptionalText(readRecordValue(metadata, keys));
}
export function resolveTrajectoryGrouping(metadata: Record<string, unknown> | undefined, fallback?: {
    scenarioId?: unknown;
    batchId?: unknown;
}): {
    scenarioId?: string;
    ...
    const scenarioId = readGroupingValue(record, TRAJECTORY_SCENARIO_METADATA_KEYS) ??
        toOptionalText(fallback?.scenarioId);
    const batchId = readGroupingValue(record, TRAJECTORY_BATCH_METADATA_KEYS) ??
        toOptionalText(fallback?.batchId);
    return { scenarioId, batchId };
}
export function normalizeTrajectoryMetadata(metadata: Record<string, unknown> | undefined, fallback?: {
    scenarioId?: unknown;
    batchId?: unknown;
}): {
    metadata: Record<string, unknown>;
    ...
    const { scenarioId, batchId } = resolveTrajectoryGrouping(normalizedMetadata, fallback);
    if (scenarioId) {
...
// Insight extraction
// ---------------------------------------------------------------------------
export function extractInsightsFromResponse(response: string, purpose: string): string[] {
...
export function shouldRunObservationExtraction(runtime: IAgentRuntime): boolean {
    const explicitSetting = runtime.getSetting("TRAJECTORY_OBSERVATION_EXTRACTION");
...
const observationFlushTimers = new WeakMap<object, ReturnType<typeof setTimeout>>();
...
export function pushChatExchange(runtime: IAgentRuntime, exchange: BufferedExchange): void {
...
export async function flushObservationBuffer(runtime: IAgentRuntime): Promise<string[]> {
    const key = runtime as object;
    ...
    const runtimeRecord = runtime as IAgentRuntime & RuntimeWithOrchestratorTrajectoryContext;
    try {
        ...
        if (trajectory) {
            ...
            meta.observations = appendCompleteTrajectoryTextRecords(meta.observations, observations, "observations");
        ...
        warnRuntime(runtime, "[trajectory-persistence] observation flush failed", err);
        return [];
    }
    finally {
...
export function parsePersistedMetadata(value: unknown, trajectoryId: string): Record<string, unknown> {
    if (value === undefined || value === null)
        return {};
    const parsed = parseJsonValue(value);
    const record = asRecord(parsed);
...
function parseCanonicalJsonObject(value: unknown, field: "metrics" | "rewardComponents", trajectoryId: string): Record<string, JsonValue> {
    if (value === undefined || value === null)
        return {};
    const parsed = parseJsonValue(value);
    const record = asRecord(parsed);
...
export function parseSteps(value: unknown, trajectoryId = "unknown"): PersistedStep[] {
    if (value === undefined || value === null)
        return [];
    const parsed = parseJsonValue(value);
    ...
    return records.map((step, index) => parsePersistedStepObject(step, trajectoryId, index));
}
...
export async function getSqlRaw(): Promise<(query: string) => {
    queryChunks: object[];
}> {
    if (cachedSqlRaw)
        return cachedSqlRaw;
...
export async function executeRawSql(runtime: IAgentRuntime, sqlText: string): Promise<unknown> {
...
export async function executeRawSqlTransaction<T>(runtime: IAgentRuntime, work: (execute: RawSqlExecutor) => Promise<T>): Promise<T> {
    const db = getRuntimeDb(runtime);
    ...
    return timeInferenceSpan("trajectory:db-transaction", () => transaction((tx) => work((sqlText) => tx.execute(raw(sqlText)))));
}
...
export function extractRequiredRows(result: unknown, context: Record<string, unknown> = {}): unknown[] {
    if (Array.isArray(result))
        return result;
    const record = asRecord(result);
...
export async function computeBySource(runtime: IAgentRuntime): Promise<Record<string, number>> {
    const result = await executeRawSql(runtime, `SELECT source, count(*) AS cnt FROM trajectories
...
export function warnRuntime(runtime: IAgentRuntime, message: string, err?: unknown): void {
    if (runtime.logger.warn) {
        runtime.logger.warn({ err, src: "eliza", subsystem: "trajectory-db" }, message);
    }
}
// ---------------------------------------------------------------------------
// Schema management
...
function databaseErrorMatches(error: unknown, patterns: RegExp[]): boolean {
    ...
    return patterns.some((pattern) => messages.some((message) => pattern.test(message)));
}
...
async function addColumnIfMissing(runtime: IAgentRuntime, table: string, name: string, definition: string): Promise<void> {
    try {
        await executeRawSql(runtime, `ALTER TABLE ${table} ADD COLUMN ${name} ${definition}`);
    }
    catch (error) {
...
const trajectorySchemaInitializationPromises = new WeakMap<object, Promise<boolean>>();
export async function ensureTrajectoriesTable(runtime: IAgentRuntime): Promise<boolean> {
    const key = runtime as object;
...
async function initializeTrajectoriesTable(runtime: IAgentRuntime): Promise<boolean> {
    const key = runtime as object;
    ...
    try {
        ...
        try {
            ...
            if (!isMissingTableError(error))
                throw error;
            needsRecreate = true;
            coreLogger.warn("[trajectory-persistence] Trajectories table does not exist, creating...");
        }
        ...
        await addColumnIfMissing(runtime, "trajectory_archive", "archive_blob_path", "TEXT");
        await addColumnIfMissing(runtime, "trajectory_archive", "total_cache_read_input_tokens", "INTEGER NOT NULL DEFAULT 0");
        await addColumnIfMissing(runtime, "trajectory_archive", "total_cache_creation_input_tokens", "INTEGER NOT NULL DEFAULT 0");
        // Best-effort forward migration for grouping columns.
        await addColumnIfMissing(runtime, "trajectories", "scenario_id", "TEXT");
        await executeRawSql(runtime, `CREATE INDEX IF NOT EXISTS idx_trajectories_scenario_id ON trajectories(scenario_id)`);
        await addColumnIfMissing(runtime, "trajectories", "batch_id", "TEXT");
        await executeRawSql(runtime, `CREATE INDEX IF NOT EXISTS idx_trajectories_batch_id ON trajectories(batch_id)`);
        await addColumnIfMissing(runtime, "trajectory_archive", "scenario_id", "TEXT");
        ...
        await executeRawSql(runtime, `CREATE TABLE IF NOT EXISTS trajectory_steps (
        id TEXT PRIMARY KEY,
        ...
        await ensureTrajectoryStepsConstraintSchema(runtime, trajectoryStepsExisted);
        await executeRawSql(runtime, `CREATE INDEX IF NOT EXISTS idx_trajectory_steps_trajectory_id ON trajectory_steps(trajectory_id)`);
        await executeRawSql(runtime, `CREATE INDEX IF NOT EXISTS idx_trajectory_steps_ordinal ON trajectory_steps(trajectory_id, ordinal)`);
        ...
        await forwardMigrateStepsJsonToRows(runtime);
        if (needsRecreate) {
            coreLogger.warn("[trajectory-persistence] Recreated trajectories table with updated schema");
        }
...
async function ensureTrajectoryStepsConstraintSchema(runtime: IAgentRuntime, trajectoryStepsExisted: boolean): Promise<void> {
    await executeRawSql(runtime, `CREATE TABLE IF NOT EXISTS trajectory_schema_migrations (
      id TEXT PRIMARY KEY,
      created_at TEXT NOT NULL
    )`);
    await executeRawSqlTransaction(runtime, async (execute) => {
        const claimed = extractRequiredRows(await execute(`INSERT INTO trajectory_schema_migrations (id, created_at)
         VALUES (
           ${sqlQuote(TRAJECTORY_STEPS_CONSTRAINT_MIGRATION)},
           ${sqlQuote(new Date().toISOString())}
         )
         ON CONFLICT (id) DO NOTHING
         RETURNING id`), { operation: "claim trajectory step schema migration" });
        if (claimed.length === 0 || !trajectoryStepsExisted)
            return;
...
async function forwardMigrateStepsJsonToRows(runtime: IAgentRuntime): Promise<boolean> {
    const key = runtime as object;
    if (stepsForwardMigrationRan.has(key))
        return true;
...
async function runForwardStepsMigration(runtime: IAgentRuntime): Promise<boolean> {
    try {
        // Find trajectories that have steps_json content but no rows yet.
        const result = await executeRawSql(runtime, `SELECT t.id AS id, CAST(t.steps_json AS TEXT) AS steps_json
       FROM trajectories t
       ...
       WHERE s.id IS NULL
        ...
        for (const row of rows) {
            ...
            try {
                const steps = parsed.map((stepValue, index) => normalizeMigratedStep(stepValue, trajectoryId, index));
                await executeRawSqlTransaction(runtime, async (execute) => {
                    await replaceStepsForTrajectoryInternal(trajectoryId, steps, execute, true);
                });
                ...
                failed = true;
                warnRuntime(runtime, `forwardMigrateStepsJsonToRows: failed trajectory ${trajectoryId}`, error);
        ...
        if (migrated > 0) {
            coreLogger.info(`[trajectory-persistence] Forward-migrated ${migrated} step rows from steps_json into trajectory_steps`);
        }
        return !failed;
    }
    catch (err) {
        ...
        warnRuntime(runtime, "forwardMigrateStepsJsonToRows: migration query failed; legacy steps_json still readable", err);
...
function normalizeMigratedStep(value: unknown, trajectoryId: string, index: number): PersistedStep {
    return parsePersistedStepObject(value, trajectoryId, index);
}
// ---------------------------------------------------------------------------
// Normalization helpers
// ---------------------------------------------------------------------------
export function normalizeStatus(value: unknown, fallback: TrajectoryStatus): TrajectoryStatus {
...
export function parsePersistedTrajectoryStatus(value: unknown, trajectoryId: string): TrajectoryStatus {
    const status = toText(value, "").toLowerCase();
    if (status === "active" ||
        status === "completed" ||
        status === "error" ||
...
export function normalizePersistedTrajectoryTiming(input: {
    ...
    const existingEndTime = typeof input.endTime === "number" &&
    ...
    const endTime = existingEndTime ??
        [
            ...
            candidate >= startTime) ??
        startTime;
    const durationMs = existingEndTime !== null &&
...
export function normalizePersistedUpdatedAt(input: {
    ...
    const startTime = Number.isFinite(input.startTime) ? input.startTime : 0;
    const floorTime = typeof input.endTime === "number" && Number.isFinite(input.endTime)
        ? input.endTime
        : startTime;
    const updatedAtMs = toOptionalEpochMs(input.updatedAt);
    const createdAtMs = toOptionalEpochMs(input.createdAt);
    const timestamp = (typeof updatedAtMs === "number" &&
        updatedAtMs > 0 &&
...
function redactTrajectoryParams(params: Record<string, unknown>): Record<string, unknown> {
...
function snapshotCaptureParams(params: Record<string, unknown>, stepId: string): Record<string, unknown> {
 ...
 */
function snapshotLlmCaptureParams(params: Record<string, unknown>, stepId: string): Record<string, unknown> {
 ...
 */
function coerceAbsentLlmResponse(params: Record<string, unknown>): Record<string, unknown> {
...
export function normalizeLlmCallPayload(args: unknown[]): {
    ...
    if (typeof args[0] === "string") {
        ...
        const params = redactTrajectoryParams(coerceAbsentLlmResponse({
            ...details,
            stepId,
        }));
    ...
    const normalizedParams = params.stepId === stepId ? params : { ...params, stepId };
    const redactedParams = redactTrajectoryParams(coerceAbsentLlmResponse(normalizedParams));
...
function requireCaptureString(params: Record<string, unknown>, field: string, stepId: string, allowEmpty = false): void {
    const value = params[field];
...
function validateLlmCapture(params: Record<string, unknown>, stepId: string): void {
    ...
    if (params.providerAttributions !== undefined &&
 ...
 */
function snapshotProviderCaptureParams(params: Record<string, unknown>, stepId: string): Record<string, unknown> {
    return snapshotCaptureParams({
...
function validateProviderCapture(params: Record<string, unknown>, stepId: string): void {
    ...
    if (params.overlapsWith !== undefined &&
...
export function shouldSuppressNoInputEmbeddingCall(params: Record<string, unknown>): boolean {
...
export async function resolveTrajectoryLogger(runtime: IAgentRuntime): Promise<TrajectoryLoggerLike | null> {
...
// ---------------------------------------------------------------------------
export function enqueueStepWrite(runtime: IAgentRuntime, stepId: string, work: () => Promise<void>): Promise<void> {
    ...
    const current = previous
        ...
        try {
            ...
            warnRuntime(runtime, "Failed to write trajectory update to database", error);
            try {
                ...
                warnRuntime(runtime, "Failed to report trajectory database write error", reportError);
            }
            throw error;
        }
    })
...
export function createBaseTrajectory(stepId: string, now: number, agentId: string, source?: string, metadata?: Record<string, unknown>): PersistedTrajectory {
...
export function ensureStep(trajectory: PersistedTrajectory, stepId: string, now: number): PersistedStep {
...
export function mergeMetadata(existing: Record<string, unknown>, incoming?: Record<string, unknown>): Record<string, unknown> {
    if (!incoming)
        return existing;
...
export function collectTrajectoryTimestamps(trajectory: PersistedTrajectory): number[] {
...
export function parsePersistedTrajectoryRow(row: Record<string, unknown>, fallbackId: string): PersistedTrajectory {
    const id = requiredPersistedString(readRecordValue(row, ["id", "trajectory_id", "trajectoryId"]), fallbackId, "id");
    const agentId = requiredPersistedString(readRecordValue(row, ["agent_id", "agentId"]), id, "agent_id");
    const source = requiredPersistedString(readRecordValue(row, ["source"]), id, "source");
    const startTime = requiredPersistedNumber(readRecordValue(row, ["start_time", "startTime"]), id, "start_time");
    const status = parsePersistedTrajectoryStatus(readRecordValue(row, ["status"]), fallbackId);
    ...
    const endTime = rawEndTime === undefined || rawEndTime === null
        ? null
        : requiredPersistedNumber(rawEndTime, id, "end_time");
    const durationMs = rawDurationMs === undefined || rawDurationMs === null
        ? null
        : requiredPersistedNumber(rawDurationMs, id, "duration_ms");
    if ((status === "active" && (endTime !== null || durationMs !== null)) ||
        (status !== "active" &&
            (endTime === null ||
                endTime < startTime ||
                durationMs === null ||
    ...
    const steps = parseSteps(readRecordValue(row, ["steps_json", "stepsJson", "steps"]), id);
    const optionalRowString = (keys: string[], field: string): string | undefined => {
    ...
    const scenarioId = optionalRowString(["scenario_id", "scenarioId"], "scenario_id");
    const traceId = optionalRowString(["trace_id", "traceId"], "trace_id");
    const episodeId = optionalRowString(["episode_id", "episodeId"], "episode_id");
    const batchId = optionalRowString(["batch_id", "batchId"], "batch_id");
    const rawGroupIndex = readRecordValue(row, ["group_index", "groupIndex"]);
    const groupIndex = rawGroupIndex === undefined || rawGroupIndex === null
        ? undefined
    ...
    const normalizedMetadata = normalizeTrajectoryMetadata(parsePersistedMetadata(readRecordValue(row, [
        "metadata_json",
        "metadataJson",
        "metadata",
        "meta",
    ...
    const totalReward = requiredPersistedNumber(readRecordValue(row, ["total_reward", "totalReward"]), id, "total_reward");
    const parsedMetrics = parseCanonicalJsonObject(readRecordValue(row, ["metrics_json", "metricsJson", "metrics"]), "metrics", fallbackId);
    const parsedRewardComponents = parseCanonicalJsonObject(readRecordValue(row, [
        "reward_components_json",
        "rewardComponentsJson",
        "rewardComponents",
    ]), "rewardComponents", fallbackId);
    ...
    else if (typeof rewardComponents.environmentReward !== "number" ||
...
// ---------------------------------------------------------------------------
export async function loadTrajectoryById(runtime: IAgentRuntime, stepId: string): Promise<PersistedTrajectory | null> {
    const safeId = sqlQuote(stepId);
    try {
        const result = await executeRawSql(runtime, `SELECT * FROM trajectories
       ...
       LIMIT 1`);
        ...
        const stepsFromTable = await loadAllStepsFromDedicatedTable(runtime, stepId);
 ...
 */
async function loadAllStepsFromDedicatedTable(runtime: IAgentRuntime, trajectoryId: string): Promise<PersistedStep[] | null> {
    const safeId = sqlQuote(trajectoryId);
    try {
        const result = await executeRawSql(runtime, `SELECT * FROM trajectory_steps
...
export function stepRowToPersistedStep(row: Record<string, unknown>): PersistedStep {
    const payload = parseJsonValue(readRecordValue(row, ["payload"]));
    const payloadRecord = asRecord(payload);
    const trajectoryId = toText(readRecordValue(row, ["trajectory_id", "trajectoryId"]), "").trim();
    ...
    const parentStepId = parentStepValue === undefined || parentStepValue === null
        ? undefined
    ...
    const kind = kindRaw === "llm" || kindRaw === "action" || kindRaw === "evaluator"
        ? kindRaw
        : null;
    const scriptValue = readRecordValue(row, ["script"]);
    const script = scriptValue === undefined || scriptValue === null
        ? undefined
    ...
    const scriptHash = typeof payloadRecord.scriptHash === "string"
    ...
    return parsePersistedStepObject({
        ...
        ...(kind === "evaluator" && payloadRecord.evaluatorName === undefined
            ...
            : {}),
    }, trajectoryId, stepNumber);
}
export function buildTrajectoryStepUpsertSql(trajectoryId: string, step: PersistedStep, parentStepIdOverride?: string | null, conflictAction = "DO UPDATE SET"): string {
    const stepType = step.kind === "llm" || step.kind === "action" || step.kind === "evaluator"
        ? step.kind
        : "llm";
    const script = typeof step.script === "string" && step.script.length > 0
        ? step.script
        : null;
    const { script: _script, ...payloadObject } = step;
    const startedAt = Number.isFinite(step.timestamp) ? step.timestamp : null;
    const name = step.kind === "evaluator" && step.evaluatorName
        ? step.evaluatorName
        : (step.llmCalls[0]?.purpose ??
            step.providerAccesses[0]?.providerName ??
            null);
    const parentStepId = parentStepIdOverride !== undefined
        ? parentStepIdOverride
        : typeof step.parentStepId === "string" && step.parentStepId.length > 0
            ? step.parentStepId
            : null;
    const updateClause = conflictAction === "DO NOTHING"
        ? "DO NOTHING"
        : `DO UPDATE SET
...
export async function assertTrajectoryStepOwnership(execute: RawSqlExecutor, trajectoryId: string, stepId: string): Promise<void> {
    const result = await execute(`SELECT trajectory_id FROM trajectory_steps WHERE id = ${sqlQuote(stepId)} LIMIT 1`);
    const existing = asRecord(extractRequiredRows(result, {
...
export async function assertTrajectoryAgentOwnership(execute: RawSqlExecutor, trajectoryId: string, agentId: string, allowMissing = false): Promise<void> {
    const result = await execute(`SELECT agent_id FROM trajectories WHERE id = ${sqlQuote(trajectoryId)} LIMIT 1`);
    const owner = toOptionalText(asRecord(extractRequiredRows(result, {
        operation: "check trajectory ownership",
        trajectoryId,
        agentId,
    })[0])?.agent_id);
    if (!owner && allowMissing)
        return;
    if (!owner) {
...
export async function assertTrajectoryStepParentOwnership(execute: RawSqlExecutor, trajectoryId: string, step: PersistedStep): Promise<void> {
    ...
    const parentOwner = toOptionalText(asRecord(extractRequiredRows(result, {
...
export function parsePersistedEvaluatorName(value: unknown): string | undefined {
    if (value === undefined || value === null)
        return undefined;
    if (typeof value !== "string" || value.trim().length === 0) {
...
export function parsePersistedSkillInvocations(value: unknown): TrajectorySkillInvocation[] | undefined {
    if (value === undefined || value === null)
        return undefined;
    if (!Array.isArray(value)) {
    ...
    return value.map((entry, index) => {
        ...
        for (const field of optionalStrings) {
            if (record[field] !== undefined && typeof record[field] !== "string") {
                throw new ElizaError(`Stored trajectory skill invocation ${field} is invalid`, {
                    code: "TRAJECTORY_SKILL_INVOCATION_INVALID",
        ...
        if (record.mode !== undefined &&
            ...
            throw new ElizaError("Stored trajectory skill invocation mode is invalid", {
                code: "TRAJECTORY_SKILL_INVOCATION_INVALID",
        ...
        if (record.truncated !== undefined) {
            if (!Array.isArray(record.truncated)) {
                throw new ElizaError("Stored trajectory skill invocation truncation is invalid", {
                    code: "TRAJECTORY_SKILL_INVOCATION_INVALID",
            ...
            truncated = record.truncated.map((marker, markerIndex) => {
                ...
                if (!markerRecord ||
                    ...
                    capBytes === undefined ||
                    capBytes < 0) {
                    throw new ElizaError("Stored trajectory skill invocation truncation marker is invalid", {
                        code: "TRAJECTORY_SKILL_INVOCATION_INVALID",
...
function parsePersistedActionAttempt(value: unknown): TrajectoryActionAttempt | undefined {
    if (value === undefined || value === null)
        return undefined;
    const record = asRecord(value);
    ...
    if (record.immediateReward !== undefined &&
...
function persistedRowError(trajectoryId: string, field: string, stepId?: string, index?: number): ElizaError {
...
function requiredPersistedString(value: unknown, trajectoryId: string, field: string, stepId?: string, allowEmpty = false): string {
    if (typeof value !== "string" || (!allowEmpty && value.trim().length === 0)) {
        throw persistedRowError(trajectoryId, field, stepId);
    }
    return value;
}
function requiredPersistedNumber(value: unknown, trajectoryId: string, field: string, stepId?: string): number {
...
function requiredPersistedJsonNumber(value: unknown, trajectoryId: string, field: string, stepId?: string): number {
    if (typeof value !== "number" || !Number.isFinite(value)) {
        throw persistedRowError(trajectoryId, field, stepId);
    }
    return value;
}
function parseOptionalPersistedString(record: Record<string, unknown>, field: string, trajectoryId: string, stepId: string): string | undefined {
...
function parseOptionalPersistedNumber(record: Record<string, unknown>, field: string, trajectoryId: string, stepId: string): number | undefined {
    const value = record[field];
    if (value === undefined)
        return undefined;
    if (typeof value !== "number" || !Number.isFinite(value)) {
...
export function parsePersistedLlmCall(value: unknown, trajectoryId: string, stepId: string, index: number): PersistedLlmCall {
    const record = asRecord(value);
    if (!record) {
        throw persistedRowError(trajectoryId, "llmCalls", stepId, index);
    }
    const callId = requiredPersistedString(record.callId, trajectoryId, `llmCalls[${index}].callId`, stepId);
    const timestamp = requiredPersistedJsonNumber(record.timestamp, trajectoryId, `llmCalls[${index}].timestamp`, stepId);
    const model = requiredPersistedString(record.model, trajectoryId, `llmCalls[${index}].model`, stepId);
    const response = requiredPersistedString(record.response, trajectoryId, `llmCalls[${index}].response`, stepId, true);
    const purpose = requiredPersistedString(record.purpose, trajectoryId, `llmCalls[${index}].purpose`, stepId);
    const actionType = requiredPersistedString(record.actionType, trajectoryId, `llmCalls[${index}].actionType`, stepId);
    const stringFields = [
        "provider",
        "modelVersion",
        "modelType",
    ...
    for (const field of stringFields) {
        const parsed = parseOptionalPersistedString(record, field, trajectoryId, stepId);
        if (parsed !== undefined)
            optional[field] = parsed;
    }
    for (const field of numericFields) {
        const parsed = parseOptionalPersistedNumber(record, field, trajectoryId, stepId);
        if (parsed !== undefined)
            optional[field] = parsed;
    }
    for (const field of [
        ...
        if (record[field] !== undefined && !Array.isArray(record[field])) {
            throw persistedRowError(trajectoryId, `llmCalls[${index}].${field}`, stepId);
        }
    }
    for (const field of ["maxTokensOmitted", "tokenUsageEstimated"] as const) {
...
export function parsePersistedProviderAccess(value: unknown, trajectoryId: string, stepId: string, index: number): PersistedProviderAccess {
    const record = asRecord(value);
    if (!record) {
        throw persistedRowError(trajectoryId, "providerAccesses", stepId, index);
    }
    const providerId = requiredPersistedString(record.providerId, trajectoryId, `providerAccesses[${index}].providerId`, stepId);
    const providerName = requiredPersistedString(record.providerName, trajectoryId, `providerAccesses[${index}].providerName`, stepId);
    const timestamp = requiredPersistedJsonNumber(record.timestamp, trajectoryId, `providerAccesses[${index}].timestamp`, stepId);
    const purpose = requiredPersistedString(record.purpose, trajectoryId, `providerAccesses[${index}].purpose`, stepId);
    const data = asRecord(record.data);
    if (!data || !isJsonValue(data)) {
        throw persistedRowError(trajectoryId, `providerAccesses[${index}].data`, stepId);
    }
    const optional: Record<string, unknown> = {};
    for (const field of [
        "startedAt",
    ...
    if (record.query !== undefined &&
        ...
        throw persistedRowError(trajectoryId, `providerAccesses[${index}].query`, stepId);
    }
    if (record.overlapsWith !== undefined &&
        ...
        throw persistedRowError(trajectoryId, `providerAccesses[${index}].overlapsWith`, stepId);
    }
    return {
        ...record,
        ...optional,
...
export function parsePersistedStepObject(value: unknown, trajectoryId: string, index: number): PersistedStep {
    const record = asRecord(value);
    if (!record)
        throw persistedRowError(trajectoryId, "steps", undefined, index);
    const stepId = requiredPersistedString(record.stepId, trajectoryId, `steps[${index}].stepId`);
    const stepNumber = requiredPersistedJsonNumber(record.stepNumber, trajectoryId, `steps[${index}].stepNumber`, stepId);
    if (!Number.isInteger(stepNumber) || stepNumber < 0) {
        throw persistedRowError(trajectoryId, `steps[${index}].stepNumber`, stepId);
    }
    const timestamp = requiredPersistedJsonNumber(record.timestamp, trajectoryId, `steps[${index}].timestamp`, stepId);
    if (!Array.isArray(record.llmCalls) ||
    ...
    const parentStepId = parseOptionalPersistedString(record, "parentStepId", trajectoryId, stepId);
    ...
    const script = parseOptionalPersistedString(record, "script", trajectoryId, stepId);
    const scriptHash = parseOptionalPersistedString(record, "scriptHash", trajectoryId, stepId);
    const evaluatorName = parsePersistedEvaluatorName(record.evaluatorName);
    const skillInvocations = parsePersistedSkillInvocations(record.skillInvocations);
    ...
    return {
        ...
        stepNumber,
        timestamp,
        llmCalls: record.llmCalls.map((call, callIndex) => parsePersistedLlmCall(call, trajectoryId, stepId, callIndex)),
        providerAccesses: record.providerAccesses.map((access, accessIndex) => parsePersistedProviderAccess(access, trajectoryId, stepId, accessIndex)),
...
export async function loadTrajectoryByStepId(runtime: IAgentRuntime, stepId: string): Promise<PersistedTrajectory | null> {
    ...
    const dedicatedResult = await executeRawSql(runtime, `SELECT s.trajectory_id
     ...
     LIMIT 1`);
    const dedicatedTrajectoryId = toOptionalText(asRecord(extractRequiredRows(dedicatedResult, {
...
function normalizeStepForPersistence(trajectoryId: string, step: PersistedStep): PersistedStep {
    const { llmCalls, providerAccesses, childSteps, usedSkills, skillInvocations, semanticStages, script, ...scalarFields } = step;
    ...
    const normalizeRecord = (value: unknown, field: "llmCalls" | "providerAccesses", index: number): Record<string, unknown> => {
    ...
    const normalizeProviderRecord = (access: PersistedProviderAccess, index: number): PersistedProviderAccess => {
        const { providerId, providerName, timestamp, purpose, data, ...extras } = access;
        ...
        return parsePersistedProviderAccess(bounded, trajectoryId, step.stepId, index);
    };
    return {
        ...
        llmCalls: llmCalls.map((call, index) => normalizeRecord(call, "llmCalls", index) as unknown as PersistedLlmCall),
        providerAccesses: providerAccesses.map(normalizeProviderRecord),
        ...(childSteps !== undefined ? { childSteps: [...childSteps] } : {}),
        ...(usedSkills !== undefined ? { usedSkills: [...usedSkills] } : {}),
        ...(skillInvocations !== undefined
            ? {
                skillInvocations: parsePersistedSkillInvocations(skillInvocations) ?? [],
            }
            : {}),
...
export async function saveTrajectory(runtime: IAgentRuntime, trajectory: PersistedTrajectory, options: {
    ...
    if (options.createOnly &&
        ...
        throw new ElizaError("Insert-only trajectory writes cannot update an existing row", {
    ...
    const persistedEndTime = typeof trajectory.endTime === "number" &&
        Number.isFinite(trajectory.endTime) &&
        trajectory.endTime >= summary.startTime
        ? trajectory.endTime
        : undefined;
    const summaryEndTime = Number.isFinite(summary.endTime) && summary.endTime >= summary.startTime
        ? summary.endTime
        : summary.startTime;
    ...
    const boundedSteps = trajectory.steps.map((step) => normalizeStepForPersistence(trajectory.id, step));
    ...
    const serializeLegacySteps = (): string => {
        if (serializedLegacySteps !== undefined)
    ...
    const boundedRewardComponents = sanitizeTrajectoryJsonObject(trajectory.rewardComponents);
    ...
    const serializedRewardComponents = sqlQuote(JSON.stringify(boundedRewardComponents));
    ...
    const updateLegacyStepsSql = replaceAllSteps || options.updateLegacySnapshot
        ? "steps_json = EXCLUDED.steps_json,"
        : "";
    const updateLegacyStepsValueSql = replaceAllSteps || options.updateLegacySnapshot
    ...
    try {
        await persistTrajectoryAndSteps(runtime, currentSchemaSql, currentSchemaUpdateSql, trajectory.id, stepsToPersist, replaceAllSteps, {
        ...
        if (currentSchemaError instanceof ElizaError &&
            [
        ...
        try {
            await persistTrajectoryAndSteps(runtime, legacySchemaSql, legacySchemaUpdateSql, trajectory.id, stepsToPersist, replaceAllSteps, {
                requireActiveExisting: options.requireActiveExisting === true,
                expectedUpdatedAt: options.expectedUpdatedAt,
                createOnly: options.createOnly === true,
            });
            ...
            if (legacySchemaError instanceof ElizaError &&
                [
                    "TRAJECTORY_STEPS_SAVE_FAILED",
                    "TRAJECTORY_AGENT_OWNERSHIP_CONFLICT",
                    "TRAJECTORY_STEP_OWNERSHIP_CONFLICT",
...
async function persistTrajectoryAndSteps(runtime: IAgentRuntime, parentUpsertSql: () => string, parentUpdateSql: string, trajectoryId: string, steps: PersistedStep[], replaceAllSteps: boolean, precondition: {
    requireActiveExisting: boolean;
    expectedUpdatedAt?: string;
    createOnly: boolean;
}): Promise<void> {
    const requiresConditionalWrite = precondition.requireActiveExisting ||
    ...
    await executeRawSqlTransaction(runtime, async (execute) => {
        if (!requiresConditionalWrite) {
            await assertTrajectoryAgentOwnership(execute, trajectoryId, runtime.agentId, true);
         ...
         RETURNING id`);
            ...
            if (parentWriteRows.length === 0) {
                const parentResult = await execute(`SELECT agent_id, status, updated_at FROM trajectories
        ...
        try {
            ...
            await replaceStepsForTrajectoryInternal(trajectoryId, steps, execute, replaceAllSteps);
        }
        catch (error) {
            if (error instanceof ElizaError &&
                [
...
async function replaceStepsForTrajectoryInternal(trajectoryId: string, steps: PersistedStep[], execute: RawSqlExecutor, replaceAllSteps: boolean): Promise<void> {
    if (replaceAllSteps) {
        const safeId = sqlQuote(trajectoryId);
        await execute(`DELETE FROM trajectory_steps WHERE trajectory_id = ${safeId}`);
    }
 ...
 */
export function readOrchestratorTrajectoryContext(runtime: unknown): OrchestratorTrajectoryContext | undefined {
...
export async function resolveTrajectoryArchiveDirectory(): Promise<string> {
    ...
    try {
        ...
        const fallback = path.join(process.env.TMPDIR || os.tmpdir(), "eliza", TRAJECTORY_ARCHIVE_DIRNAME);
...
export async function writeCompressedJsonlRows(archivePath: string, rows: Record<string, unknown>[]): Promise<void> {
 ...
 */
export function shouldEnableTrajectoryLoggingByDefault(env: NodeJS.ProcessEnv = process.env): boolean {
