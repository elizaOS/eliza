...
function knowledgeRowFingerprint(memory: Memory): string {
  try {
...
async function readStableCompleteDocumentRows(
  ...
  const firstFingerprints = first.map(knowledgeRowFingerprint);
  const secondFingerprints = second.map(knowledgeRowFingerprint);
  if (
