...
const PAGE_SCOPE_BRIEF: Record<string, string> = {
  ...
  "page-automations":
    "The user is in the Automations view. They can create coordinator-text triggers, one-off tasks, recurring tasks, and workflows; set cron or interval schedules; configure wake mode (inject_now / schedule_at / interval), max-runs, and enabled state; browse templates; inspect existing automations; and troubleshoot failed runs. Action vocabulary: WORKFLOW (op-based dispatch — create/modify/activate/deactivate/toggle_active/delete/executions for workflows), TRIGGER (op-based dispatch — create/update/delete/run/toggle for scheduled triggers), and manageTasksAction. When the user asks what to do, recommend trigger vs task vs workflow based on the event, schedule, and desired result. Triggers and workflows already in the system are listed in live state below; reference them by display name when answering.",
  "page-apps":
  ...
  "automation-draft":
    ...
    '- Deterministic pipeline of integration steps (e.g. "when a Slack message matches X, post to Discord") → create a workflow via the WORKFLOW actions.\n' +
