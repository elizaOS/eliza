...
import { historyRetentionContext } from "../../../../plugins/plugin-assistant/src/services/history-retention.ts";
import {
...
const {
  ...
  const getEvaluatorProgressState = vi.fn<
    typeof import("@elizaos/plugin-assistant").getEvaluatorProgressState
  >(async () => undefined);
...
describe("relevantConversationsProvider — shared recall embed fail-open", () => {
  ...
  it.each([
    ...
    async (mode) => {
      ...
      if (["valid", "missing", "stale"].includes(mode)) {
        expect(getEvaluatorProgressState).toHaveBeenCalledWith(
          runtime,
          expect.objectContaining({
            roomId: OTHER_ROOM,
            entityId: message.entityId,
      ...
      } else expect(getEvaluatorProgressState).not.toHaveBeenCalled();
