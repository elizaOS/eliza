 ...
 */
import { CONNECTOR_IDS } from "@elizaos/core/config/schema";
...
export type PluginUiMetadata = {
    ...
    name?: string;
    description?: string;
    configUiHints?: Record<string, Pick<ConfigUiHint, "label" | "help" | "advanced" | "sensitive" | "placeholder" | "type" | "icon" | "readonly" | "hidden" | "width" | "pattern" | "patternError" | "showIf" | "requires" | "requiresAny" | "visible" | "options" | "min" | "max" | "step" | "unit" | "modeToggle">>;
...
const FIELD_LABELS: Record<string, string> = {
    ...
    "connectors.telegram.draftChunk.breakPreference": "Telegram Draft Chunk Break Preference",
...
const FIELD_HELP: Record<string, string> = {
    "meta.firstRunComplete": "Explicit first-run completion marker used to keep the app out of setup until reset.",
    ...
    "gateway.controlUi.basePath": "Optional URL prefix where the Control UI is served (e.g. /eliza).",
    ...
    "diagnostics.cacheTrace.filePath": "JSONL output path for cache trace logs (default: $ELIZA_STATE_DIR/logs/cache-trace.jsonl).",
    ...
    "tools.web.search.enabled": "WEB_SEARCH is keyless (Parallel, then Exa). This legacy toggle is retained for compatibility and does not control tool availability.",
    ...
    "tools.cache.enabled": "Enable the cross-tool call cache (default: enabled). Caches results of cacheable tools (web_search, web_fetch, file_read, rag_search, knowledge_lookup) by canonicalized argument hash to a two-tier in-memory + on-disk store. Side-effect tools never cache.",
    "tools.cache.memoryCapacity": "In-memory LRU capacity for the tool-call cache. Default 1000.",
    "tools.cache.diskRoot": "Override the on-disk root for the tool-call cache. Default: <stateDir>/tool-cache.",
    ...
    "connectors.slack.allowBots": "Allow bot-authored messages to trigger Slack replies (default: false).",
    "connectors.slack.thread.historyScope": 'Scope for Slack thread history context ("thread" isolates per thread; "channel" reuses channel history).',
    "connectors.slack.thread.inheritParent": "If true, Slack thread sessions inherit the parent channel transcript (default: false).",
    "connectors.mattermost.botToken": "Bot token from Mattermost System Console -> Integrations -> Bot Accounts.",
    "connectors.mattermost.baseUrl": "Base URL for your Mattermost server (e.g., https://chat.example.com).",
    "connectors.mattermost.chatmode": 'Reply to channel messages on mention ("oncall"), on trigger chars (">" or "!") ("onchar"), or on every message ("onmessage").',
    "connectors.mattermost.oncharPrefixes": 'Trigger prefixes for onchar mode (default: [">", "!"]).',
    "connectors.mattermost.requireMention": "Require @mention in channels before responding (default: true).",
    ...
    "agents.defaults.enableInitProviders": "Enable init providers (attachments, entities, facts) that add context but consume tokens. Useful to disable for small context windows (default: true).",
    ...
    "agents.defaults.memorySearch.local.modelPath": "Local GGUF model path or hf: URI (capacitor-llama).",
    "agents.defaults.memorySearch.fallback": 'Fallback provider when embeddings fail ("openai", "gemini", "local", or "none").',
    "agents.defaults.memorySearch.store.path": "SQLite index path (default: <stateDir>/memory/{agentId}.sqlite).",
    ...
    "embedding.model": "GGUF model filename for local embeddings (default: bundles/2b/text/eliza-1-2b-32k.gguf).",
    "embedding.modelRepo": "HuggingFace repo for auto-downloading the embedding model (default: elizaos/eliza-1).",
    "embedding.dimensions": "Embedding vector dimensions (must match the model; default: 1024).",
    "embedding.contextSize": "Embedding context window size (must match the model; default inferred from selected model).",
    "embedding.gpuLayers": 'GPU layers for embedding model loading: "auto" (Metal on macOS), "max", or a number (default: "auto" on macOS, 0 elsewhere).',
    "embedding.idleTimeoutMinutes": "Minutes of inactivity before unloading the embedding model from memory (default: 30, 0 = never unload).",
    memory: "Memory backend configuration (global).",
    "memory.backend": 'Memory backend ("builtin" for Eliza embeddings, "qmd" for QMD sidecar).',
    ...
    "plugins.installs": "CLI-managed install metadata (used by `eliza plugins update` to locate install sources).",
    "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
    "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
    "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
    "plugins.installs.*.installPath": "Resolved install directory (usually <stateDir>/plugins/<id>).",
    ...
    "connectors.telegram.configWrites": "Allow Telegram to write config in response to channel events/commands (default: true).",
    "connectors.slack.configWrites": "Allow Slack to write config in response to channel events/commands (default: true).",
    "connectors.mattermost.configWrites": "Allow Mattermost to write config in response to channel events/commands (default: true).",
    "connectors.discord.configWrites": "Allow Discord to write config in response to channel events/commands (default: true).",
    "connectors.whatsapp.configWrites": "Allow WhatsApp to write config in response to channel events/commands (default: true).",
    "connectors.imessage.configWrites": "Allow iMessage to write config in response to channel events/commands (default: true).",
    "connectors.msteams.configWrites": "Allow Microsoft Teams to write config in response to channel events/commands (default: true).",
    "connectors.discord.commands.native": 'Override native commands for Discord (bool or "auto").',
    "connectors.discord.commands.nativeSkills": 'Override native skill commands for Discord (bool or "auto").',
    "connectors.telegram.commands.native": 'Override native commands for Telegram (bool or "auto").',
    "connectors.telegram.commands.nativeSkills": 'Override native skill commands for Telegram (bool or "auto").',
    "connectors.slack.commands.native": 'Override native commands for Slack (bool or "auto").',
    "connectors.slack.commands.nativeSkills": 'Override native skill commands for Slack (bool or "auto").',
    "session.agentToAgent.maxPingPongTurns": "Max reply-back turns between requester and target (0–5).",
    "connectors.telegram.customCommands": "Additional Telegram bot menu commands (merged with native; conflicts ignored).",
    ...
    "connectors.telegram.dmPolicy": 'Direct message access control ("pairing" recommended). "open" requires connectors.telegram.allowFrom=["*"].',
    "connectors.telegram.streamMode": "Draft streaming mode for Telegram replies (off | partial | block). Separate from block streaming; requires private topics + sendMessageDraft.",
    "connectors.telegram.draftChunk.minChars": 'Minimum chars before emitting a Telegram draft update when connectors.telegram.streamMode="block" (default: 200).',
    "connectors.telegram.draftChunk.maxChars": 'Target max size for a Telegram draft update chunk when connectors.telegram.streamMode="block" (default: 800; clamped to connectors.telegram.textChunkLimit).',
    "connectors.telegram.draftChunk.breakPreference": "Preferred breakpoints for Telegram draft chunks (paragraph | newline | sentence). Default: paragraph.",
    "connectors.telegram.retry.attempts": "Max retry attempts for outbound Telegram API calls (default: 3).",
    "connectors.telegram.retry.minDelayMs": "Minimum retry delay in ms for Telegram outbound calls.",
    "connectors.telegram.retry.maxDelayMs": "Maximum retry delay cap in ms for Telegram outbound calls.",
    "connectors.telegram.retry.jitter": "Jitter factor (0-1) applied to Telegram retry delays.",
    "connectors.telegram.network.autoSelectFamily": "Override Node autoSelectFamily for Telegram (true=enable, false=disable).",
    "connectors.telegram.timeoutSeconds": "Max seconds before Telegram API requests are aborted (default: 500 per grammY).",
    "connectors.whatsapp.dmPolicy": 'Direct message access control ("pairing" recommended). "open" requires connectors.whatsapp.allowFrom=["*"].',
    "connectors.whatsapp.selfChatMode": "Same-phone setup (bot uses your personal WhatsApp number).",
    "connectors.whatsapp.debounceMs": "Debounce window (ms) for batching rapid consecutive messages from the same sender (0 to disable).",
    "connectors.imessage.dmPolicy": 'Direct message access control ("pairing" recommended). "open" requires connectors.imessage.allowFrom=["*"].',
    "connectors.discord.dm.policy": 'Direct message access control ("pairing" recommended). "open" requires connectors.discord.dm.allowFrom=["*"].',
    "connectors.discord.retry.attempts": "Max retry attempts for outbound Discord API calls (default: 3).",
    "connectors.discord.retry.minDelayMs": "Minimum retry delay in ms for Discord outbound calls.",
    "connectors.discord.retry.maxDelayMs": "Maximum retry delay cap in ms for Discord outbound calls.",
    "connectors.discord.retry.jitter": "Jitter factor (0-1) applied to Discord retry delays.",
    "connectors.discord.maxLinesPerMessage": "Soft max line count per Discord message (default: 17).",
    "connectors.discord.intents.presence": "Enable the Guild Presences privileged intent. Must also be enabled in the Discord Developer Portal. Allows tracking user activities (e.g. Spotify). Default: false.",
    "connectors.discord.intents.guildMembers": "Enable the Guild Members privileged intent. Must also be enabled in the Discord Developer Portal. Default: false.",
    "connectors.discord.pluralkit.enabled": "Resolve PluralKit proxied messages and treat system members as distinct senders.",
    "connectors.discord.pluralkit.token": "Optional PluralKit token for resolving private systems or members.",
    "connectors.discord.syncProfile": "Synchronize the Discord bot username/avatar on startup from connector settings (default: true).",
    "connectors.discord.profileName": "Explicit Discord bot username override; falls back to the active character name when omitted.",
    "connectors.discord.profileAvatar": "Discord bot avatar source (https URL, data URI, or local file path).",
    "connectors.slack.dm.policy": 'Direct message access control ("pairing" recommended). "open" requires connectors.slack.dm.allowFrom=["*"].',
};
...
function applyConnectorHints(hints: ConfigUiHints, connectors: ConnectorUiMetadata[]): ConfigUiHints {
    const next: ConfigUiHints = { ...hints };
...
function applyHeartbeatTargetHints(hints: ConfigUiHints, connectors: ConnectorUiMetadata[]): ConfigUiHints {
    const next: ConfigUiHints = { ...hints };
...
function applyConnectorSchemas(schema: ConfigSchema, connectors: ConnectorUiMetadata[]): ConfigSchema {
    const next = cloneSchema(schema);
    const root = asSchemaObject(next);
...
export function buildConfigSchema(params?: {
    ...
    const mergedHints = applySensitiveHints(applyHeartbeatTargetHints(applyConnectorHints(applyPluginHints(base.uiHints, plugins), connectors), connectors));
    const mergedSchema = applyConnectorSchemas(applyPluginSchemas(base.schema, plugins), connectors);
    return {
        ...base,
        schema: mergedSchema,
        uiHints: mergedHints,
