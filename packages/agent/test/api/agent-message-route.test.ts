...
describe("AgentRuntime model dispatch", () => {
  it("dispatches TEXT_LARGE through its registered provider", async () => {
    const { createTestRuntime } = await import("@elizaos/core/testing");
    const { runtime, cleanup } = await createTestRuntime({
      characterName: "ModelDispatchTest",
    });
    ...
    try {
      ...
      await expect(
        runtime.useModel(ModelType.TEXT_LARGE, { prompt: "test" }),
      ).resolves.toBe("from-local-inference");
