...
import {
  ...
  replayResponseBody,
  runSemanticFixture,
} from "./cerebras-evaluator-semantics.ts";
...
it("persists both owned semantic fixtures and rejects incomplete replay without effects", async ({
  ...
  let phase = "initial runtime";
  onTestFailed(() => console.error(`Semantic replay failed during ${phase}`));
  const fixtures = createSemanticFixtures();
  const wireFixtures = [...fixtures];
  ...
  const server = createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      requests.push(body);
      const fixture = wireFixtures.find((entry) => body.includes(entry.handle));
      if (!fixture) {
  ...
  try {
    ...
    phase = "initial semantic effects";
    ...
    const moduleUrl = new URL(
      "./cerebras-evaluator-semantics.ts",
      import.meta.url,
    ).href;
    ...
    phase = "retained evidence corruption checks";
    ...
    const replayService = await prepareSemanticEvaluators(live.runtime);
    // Every replay has fresh participants, room and message IDs. They share
    // one migrated database while their observable effects remain disjoint.
    const freshFixture = () => {
      const fixture = createSemanticFixtures()[0];
      if (!fixture) throw new Error("Missing replay fixture");
      wireFixtures.push(fixture);
      return fixture;
    };
    for (const rejected of ["length", "content_filter", "malformed"] as const) {
      phase = `rejected ${rejected} replay`;
      finish = rejected;
    ...
    phase = "conflicting relationship replay";
    finish = "original";
    conflictingRelationship = true;
    const conflictFixture = freshFixture();
