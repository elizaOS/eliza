import { parseBooleanFromText, IAgentRuntime } from "@ai16z/eliza";
...
export async function validateTwitterConfig(
    ...
    if (!runtime) {
        throw new Error("Runtime is required for Twitter configuration");
    }
    ...
    try {
        ...
        const parsedConfig = twitterEnvSchema.parse(twitterConfig);

        // Additional validation checks
        if (parsedConfig.POST_INTERVAL_MIN >= parsedConfig.POST_INTERVAL_MAX) {
            throw new Error("POST_INTERVAL_MIN must be less than POST_INTERVAL_MAX");
        }
        ...
        // Enhance error handling for non-Zod errors
        if (error instanceof Error) {
            throw new Error(`Twitter configuration error: ${error.message}`);
        }
        throw new Error("Unknown error during Twitter configuration validation");
    }
}
