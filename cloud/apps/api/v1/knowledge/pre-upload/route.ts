...
app.post("/", async (c) => {
  try {
    ...
    const invalid = validateKnowledgeFiles(files);
    if (invalid) return invalid;

    const uploaded = [];
    for (const file of files) {
      const filename = sanitizeFilename(file.name || "knowledge.txt");
      const key = `knowledge-pre-upload/${user.id}/${Date.now()}-${crypto.randomUUID()}-${filename}`;
...
app.delete("/", async (c) => {
  try {
    ...
    if (!key.startsWith(`knowledge-pre-upload/${user.id}/`)) {
