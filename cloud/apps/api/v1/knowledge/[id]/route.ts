...
import { resolveKnowledgeScope } from "../_worker-knowledge";

interface KnowledgeMemoryRef {
  id: string;
  agentId?: string;
  roomId?: string;
  type?: string;
...
app.delete("/", rateLimit(RateLimitPresets.STANDARD), async (c) => {
  try {
    ...
    const memory = (await memoriesRepository.findById(id)) as unknown as KnowledgeMemoryRef | null;
