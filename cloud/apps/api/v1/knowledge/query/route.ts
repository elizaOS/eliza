...
app.post("/", async (c) => {
  try {
    ...
    const body = (await c.req.json().catch(() => null)) as KnowledgeQueryBody | null;
    ...
    const documents = await listKnowledgeDocuments(scope, 200, 0);
    const results = documents
      .map((doc) => ({
        id: doc.id,
        content: doc.content.text,
        similarity: scoreKnowledgeText(doc.content.text, query),
