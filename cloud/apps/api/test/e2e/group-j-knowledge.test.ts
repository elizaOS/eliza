...
beforeAll(async () => {
  ...
  if (!serverReachable) {
    console.warn(
      `[group-j-knowledge] ${getBaseUrl()} did not respond to /api/health. Tests will skip.`,
    );
  }
  if (!hasTestApiKey) {
    console.warn("[group-j-knowledge] TEST_API_KEY is not set; auth-gated tests will skip.");
  }
});

describe("Group J — /api/v1/knowledge", () => {
  ...
  test("text document lifecycle: create, list, query, delete", async () => {
    if (!shouldRunAuthed()) return;
    const marker = `knowledge-e2e-${crypto.randomUUID()}`;
    ...
    const list = await api.get("/api/v1/knowledge", { headers: bearerHeaders() });
    ...
    const deleted = await api.delete(`/api/v1/knowledge/${documentId}`, {
      headers: bearerHeaders(),
    });
  ...
  test("file upload stores documents without the Node runtime", async () => {
    if (!shouldRunAuthed()) return;
    const marker = `knowledge-file-${crypto.randomUUID()}`;
    ...
    const res = await fetch(url("/api/v1/knowledge/upload-file"), {
      method: "POST",
    ...
    if (body.documents?.[0]?.id) {
      await api.delete(`/api/v1/knowledge/${body.documents[0].id}`, { headers: bearerHeaders() });
    }
  });
  ...
  test("pre-upload route is live and supports cleanup when object storage is configured", async () => {
    ...
    const res = await fetch(url("/api/v1/knowledge/pre-upload"), {
      method: "POST",
      headers: { Authorization: bearerHeaders().Authorization },
      body: form,
      signal: AbortSignal.timeout(30_000),
    ...
    if (res.status === 200) {
      ...
      const deleted = await api.delete("/api/v1/knowledge/pre-upload", {
        headers: bearerHeaders(),
        body: { blobUrl },
      });
      expect(deleted.status).toBe(200);
  ...
  test("submit route validates required character and file payload", async () => {
    if (!shouldRunAuthed()) return;
    const res = await api.post("/api/v1/knowledge/submit", {}, { headers: bearerHeaders() });
    expect(res.status).toBe(400);
  });
});
