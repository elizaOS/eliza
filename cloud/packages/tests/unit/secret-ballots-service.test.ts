  ...
  const repo: SecretBallotsRepository = {
    ...
    async updateBallot(id: string, patch: UpdateBallotPatch): Promise<SecretBallotRow | null> {
      ...
      const next: SecretBallotRow = {
        ...
        ...(patch.tallyResult !== undefined ? { tallyResult: patch.tallyResult } : {}),
    ...
    async recordVote(input: RecordVoteInput): Promise<RecordVoteOutcome> {
      const existing = votes.find(
        (v) =>
          v.ballotId === input.ballotId && v.participantIdentityId === input.participantIdentityId,
      );
      if (existing) {
...
describe("secretBallotsService", () => {
  ...
  test("submitVote records new votes and is idempotent on replay with same value", async () => {
    ...
    const u1Token = created.participantTokens.find((t) => t.identityId === "u1")!.scopedToken;
