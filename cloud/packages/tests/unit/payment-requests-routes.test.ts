...
function installMocks(harness: Harness): void {
  ...
  mock.module("@/lib/api/cloud-worker-errors", () => ({
    failureResponse: (_c: unknown, error: unknown) => {
      const status =
        (error as { status?: number }).status === 401 ? 401 : 500;
...
async function loadRoute(relPath: string, mount: string) {
  const mod = await import(
    new URL(
      `../../../apps/api/${relPath}?test=${Date.now()}-${Math.random()}`,
      import.meta.url,
    ).href
  );
...
describe("payment-requests routes", () => {
  ...
  test("GET / rejects invalid status enum", async () => {
    ...
    const response = await route.request(
      "https://api.test/api/v1/payment-requests?status=bogus",
    );
    expect(response.status).toBe(400);
  ...
  test("POST /:id/cancel cancels via service", async () => {
    ...
    const response = await route.request(
      `https://api.test/api/v1/payment-requests/${ID}/cancel`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    expect(body.success).toBe(true);
    expect(body.paymentRequest?.status).toBe("cancelled");
  ...
  test("POST /:id/expire invokes expirePast and returns refreshed row", async () => {
    ...
    const response = await route.request(
      `https://api.test/api/v1/payment-requests/${ID}/expire`,
      { method: "POST" },
    );
  ...
  test("GET /:id authed returns the full row", async () => {
    ...
    const response = await route.request(
      `https://api.test/api/v1/payment-requests/${ID}`,
    );
    expect(response.status).toBe(200);
    const body = (await response.json()) as {
      success?: boolean;
