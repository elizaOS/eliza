...
} from "@/lib/eliza/plugin-cloud-bootstrap/utils/native-planner-guards";

describe("cloud bootstrap native-planner guards", () => {
  ...
  test("accepts valid known actions with JSON parameters", () => {
    const result = validateNativePlannerDecision(
      {
        action: "WEB_SEARCH",
  ...
  test("parses v5 native tool-call-compatible planner output", () => {
    const parsed = parseNativePlannerDecision(
      JSON.stringify({
  ...
  test("normalizes legacy cloud arg shapes into native params", () => {
    ...
    expect(
      normalizeCloudActionArgs("WEB_SEARCH", {
        params: { query: "direct native args" },
      }),
    ).toEqual({ query: "direct native args" });
  });
});
