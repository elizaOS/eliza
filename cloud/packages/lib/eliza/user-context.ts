 ...
 * context. Today only `createSystemContext` is consumed externally
 * (memory.ts, agents.ts, gateway-discord/event-router.ts); the
 * authenticated-user `buildContext` path was unused dead code (verified
 * via grep) and was removed during the Clean Architecture migration so
 * this module no longer depends on `apiKeysService` / `oauthService`.
 */
