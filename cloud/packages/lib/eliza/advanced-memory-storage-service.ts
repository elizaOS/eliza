...
export class AdvancedMemoryStorageService extends Service implements MemoryStorageProvider {
  ...
  private async ensureMemoryWorld(): Promise<UUID> {
    ...
    const existing = await this.runtime.getWorld(worldId);
    if (existing) {
      return worldId;
    }
  ...
  private async ensureLongTermRoom(entityId: UUID, worldId: UUID): Promise<UUID> {
    ...
    await this.runtime.createRooms([room]);
    return roomId;
  }
  ...
  private parseSessionSummary(memory: Memory): SessionSummary | null {
    const envelope = getAdvancedMemoryEnvelope(memory);
    if (
      !envelope ||
  ...
  private sortLongTermMemories(memories: LongTermMemory[]): LongTermMemory[] {
  ...
  private sortSessionSummaries(summaries: SessionSummary[]): SessionSummary[] {
  ...
  async updateLongTermMemory(
    ...
    await this.runtime.updateMemory({
      id,
      content: { text: updates.content ?? parsed.content },
      metadata: buildCustomMemoryMetadata({
  ...
  async updateSessionSummary(
    ...
    const advancedMemory = toJsonRecord({
      ...(currentEnvelope ?? {}),
      kind: "session_summary",
      originalEntityId: updates.entityId ?? parsed.entityId,
    ...
    await this.runtime.updateMemory({
      id,
      content: { text: updates.summary ?? parsed.summary },
      metadata: buildCustomMemoryMetadata({
        existing: asRecord(existing.metadata),
        scope: "room",
        timestamp: updatedAt.getTime(),
  ...
  async getSessionSummaries(agentId: UUID, roomId: UUID, limit = 5): Promise<SessionSummary[]> {
    ...
    return this.sortSessionSummaries(summaries).slice(0, limit);
  }
}
