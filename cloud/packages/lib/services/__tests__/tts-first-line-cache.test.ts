...
describe("hashCloudCacheKey", () => {
	...
	test("changes when any key field changes (regression: F3 voice-swap safety)", () => {
		...
		expect(hashCloudCacheKey({ ...base, voiceRevision: "rev-bbbb" })).not.toBe(
			baseHash,
		);
		expect(hashCloudCacheKey({ ...base, normalizedText: "sure thing" })).not.toBe(
			baseHash,
		);
...
describe("fingerprintCloudVoiceSettings", () => {
	...
	test("empty / null produce the same baseline fingerprint", () => {
		expect(fingerprintCloudVoiceSettings({})).toBe(
			fingerprintCloudVoiceSettings(null),
		);
		expect(fingerprintCloudVoiceSettings({})).toBe(
			fingerprintCloudVoiceSettings(undefined),
		);
	});
});
...
describe("shouldBypassCloudFirstLineCache", () => {
	...
	test("returns true for realtime models (non-deterministic output)", () => {
		expect(
			shouldBypassCloudFirstLineCache({ modelId: "eleven_flash_v2_5_realtime" }),
		).toBe(true);
		expect(
			shouldBypassCloudFirstLineCache({
	...
	test("returns false for default flash model", () => {
		expect(shouldBypassCloudFirstLineCache({ modelId: "eleven_flash_v2_5" })).toBe(
			false,
		);
