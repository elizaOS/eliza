 ...
 *   GET  /accounts/{id}/registrar/domains/{domain}
 *
...
export async function registerDomain(domain: string): Promise<RegistrationStartResult> {
  ...
  const result = await cloudflareApiRequest<CfRegister>(
  ...
  return {
    domain: result.domain_name ?? result.name ?? domain,
    registrationId: result.registration_id ?? result.id ?? "",
    status: normalizeRegistrationStatus(result.status ?? "pending"),
  };
}
...
export async function getRegistrationStatus(domain: string): Promise<RegistrationStatus> {
  ...
  const result = await cloudflareApiRequest<CfStatus>(
    `/accounts/${cfg.accountId}/registrar/registrations/${encodeURIComponent(domain)}/registration-status`,
    cfg.apiToken,
  );

  const rawStatus =
    result.status ?? result.registration_status ?? result.state ?? result.current_status ?? null;
  let status = normalizeRegistrationStatus(rawStatus);
...
export async function getRegisteredDomain(domain: string): Promise<RegisteredDomain> {
  ...
  const result = await cloudflareApiRequest<CfDomain>(
    `/accounts/${cfg.accountId}/registrar/domains/${encodeURIComponent(domain)}`,
    cfg.apiToken,
  );
