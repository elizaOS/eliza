...
export async function POST(req: NextRequest) {
  ...
  try {
    const { faceImageUrl, name, description, howYouMet, gender, sceneType = 'fullbody' } = await req.json();
    ...
    if (!falKey) {
      return new Response(
        JSON.stringify({ success: false, error: 'Image generation not available. Please configure FAL_KEY.' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
      ...
      async start(controller) {
        try {
          ...
          fal.config({ credentials: falKey });
            ...
            try {
              controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'progress', message: 'Creating scene...' })}\n\n`));
            } catch (e) {
            ...
            if (!imageUrl) {
              ...
              try {
                controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'error', error: 'No image generated' })}\n\n`));
              } catch (e) {
...
function extractActivity(description: string, howYouMet: string): string {
  ...
  if (text.includes('music') || text.includes('concert')) return 'at a music venue, enjoying a show';
  ...
  if (text.includes('college') || text.includes('class')) return 'on a college campus, carrying books';
