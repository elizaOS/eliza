...
<Tip>
  **One plugin, one concern.** Twitter integration? Plugin. Blockchain actions?
...
### Bootstrap (in @elizaos/core)

The core message handler and event system is built into `@elizaos/core`. Import `bootstrapPlugin` from `@elizaos/core` to use it. It provides essential functionality for message processing, knowledge management, and basic agent operations:
