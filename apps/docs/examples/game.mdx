...
<Tabs>
  <Tab title="TypeScript">
```typescript
import { AgentRuntime, ModelType, bootstrapPlugin } from "@elizaos/core";
...
Respond with ONLY the action command, nothing else.""",
    ...
    while state.health > 0:
        ...
        # Get AI decision
        decision = await runtime.use_model("TEXT_SMALL", {
            "prompt": prompt,
            "max_tokens": 50,
        })
...
let \_ = dotenvy::dotenv();
    ...
    while state.health > 0 {
        ...
        // Get AI decision
        let decision = runtime.use_model("TEXT_SMALL", &prompt).await?;
