...
<Note>
  **No API keys required!** These examples use `plugin-eliza-classic` for pattern-matching responses and `plugin-localdb` for local storage.
</Note>
...
async fn chat(
    ...
    let response = data.runtime
        .use_model("TEXT_LARGE", &req.message)
        .await
