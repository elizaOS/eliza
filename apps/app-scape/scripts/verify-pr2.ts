...
function iframeSrcMatches(html: string, expected: string): boolean {
  ...
  try {
    ...
    return (
      ...
      actual.pathname.replace(/\/$/, "") === target.pathname.replace(/\/$/, "")
    );
  } catch {
    return false;
  }
...
async function main(): Promise<void> {
  ...
  assertTrue("response statusCode = 200", res.statusCodeFinal === 200);
  assertTrue(
  ...
  assertTrue(
    "handler returned false for unknown subroute",
    notHandled === false,
  );
  ...
  assertTrue("scape slug = 'scape'", scapeEntry?.slug === "scape");
  assertTrue(
