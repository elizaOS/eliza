...
import { aggregateSecrets, discoverPluginsFromManifest } from "@elizaos/agent";
...
function main(): void {
  ...
  const clientParam = scape.parameters.find(
    (p) => p.key === "SCAPE_CLIENT_URL",
  );
  assertTrue(
  ...
  const loopParam = scape.parameters.find(
    (p) => p.key === "SCAPE_LOOP_INTERVAL_MS",
  );
  assertTrue(
