...
function envExecutionMode() {
  ...
  if (raw === "live_buy_only" || raw === "live_full" || raw === "paper")
    return raw;
...
function parseTakeProfitRules(raw: string) {
  return raw
    ...
    .map((chunk, index) => {
      ...
      if (
        Number.isNaN(gainPct) ||
        Number.isNaN(sellPct) ||
        gainPct <= 0 ||
        sellPct <= 0
      ) {
        return null;
...
export function getDiscoveryConfig(): DiscoveryConfig {
  return {
    ...
    runOnStartup: envBool(
      "ELIZAOK_DISCOVERY_RUN_ON_STARTUP",
      DEFAULT_RUN_ON_STARTUP,
    ),
    intervalMs: envInt(
    ...
    newPoolsLimit: envInt(
      "ELIZAOK_DISCOVERY_NEW_POOLS_LIMIT",
      DEFAULT_NEW_POOLS_LIMIT,
      1,
    ),
    ...
    execution: {
      ...
      kol: {
        ...
        publicCachePath:
          envString("ELIZAOK_KOL_PUBLIC_CACHE_PATH") ||
          DEFAULT_EXECUTION_KOL_PUBLIC_CACHE_PATH,
      },
    },
    distribution: {
      ...
      poolPct: envInt(
        "ELIZAOK_DISTRIBUTION_POOL_PCT",
        DEFAULT_DISTRIBUTION_POOL_PCT,
        0,
      ),
      startBlock: process.env.ELIZAOK_DISTRIBUTION_START_BLOCK?.trim()
        ? envInt(
            "ELIZAOK_DISTRIBUTION_START_BLOCK",
            DEFAULT_DISTRIBUTION_START_BLOCK,
            0,
          )
        : null,
