...
function buildReadinessChecks(config: ExecutionConfig): ExecutionReadinessCheck[] {
  const checks: ExecutionReadinessCheck[] = [
    ...
    },
    {
      ...
      detail: config.rpcUrl ? "BNB Chain RPC is configured." : "Add ELIZAOK_BSC_RPC_URL.",
    },
  ];
...
function buildCandidatePlan(
  ...
  const previousRun = previousDetail?.history.find((entry) => entry.runId !== previousDetail.latest.runId) ?? previousDetail?.history[1];
  const previousHeatConfirmed = previousRun ? qualifiesForSustainedHeat(previousRun, config) : false;
  const eligible =
    candidate.recommendation === "simulate_buy" &&
    effectiveMcap !== null &&
    effectiveMcap >= config.risk.minEntryMcapUsd &&
    ...
    (!config.risk.allowedQuoteOnly ||
      DEFAULT_QUOTE_TOKEN_ADDRESSES.has(candidate.quoteTokenAddress.toLowerCase()));

  if (candidate.recommendation !== "simulate_buy") {
    reasons.push("Candidate is not in simulate_buy state.");
  }
  if (effectiveMcap === null) {
    reasons.push("Market cap / FDV is unavailable, so early-entry range cannot be verified.");
  } else {
    if (effectiveMcap < config.risk.minEntryMcapUsd) {
      reasons.push(`Valuation ${Math.round(effectiveMcap)} is below the minimum sustainable-entry floor.`);
    }
    if (effectiveMcap > config.risk.maxEntryMcapUsd) {
      reasons.push(`Valuation ${Math.round(effectiveMcap)} is above the early-entry ceiling.`);
    }
  }
  if (candidate.reserveUsd < config.risk.minLiquidityUsd) {
    reasons.push(`Liquidity ${Math.round(candidate.reserveUsd)} is below the minimum liquidity gate.`);
  }
  if (candidate.volumeUsdM5 < config.risk.minVolumeUsdM5) {
    reasons.push(`5m volume ${Math.round(candidate.volumeUsdM5)} is below the minimum volume gate.`);
  }
  if (candidate.volumeUsdH1 < config.risk.minVolumeUsdH1) {
    reasons.push(`1h volume ${Math.round(candidate.volumeUsdH1)} is below the sustained-heat gate.`);
  }
  if (candidate.buyersM5 < config.risk.minBuyersM5) {
    reasons.push(`5m buyers ${candidate.buyersM5} is below the holder-quality gate.`);
  }
  ...
  if (candidate.poolAgeMinutes < config.risk.minPoolAgeMinutes) {
    reasons.push(`Pool age ${candidate.poolAgeMinutes}m is too fresh for stable entry.`);
  }
  if (candidate.poolAgeMinutes > config.risk.maxPoolAgeMinutes) {
    reasons.push(`Pool age ${candidate.poolAgeMinutes}m is beyond the early-entry window.`);
  }
  if (Math.abs(candidate.priceChangeH1) > config.risk.maxPriceChangeH1Pct) {
    reasons.push(`1h price change ${Math.round(candidate.priceChangeH1)}% is too unstable for entry.`);
  }
  if (!previousRun) {
    reasons.push("Candidate has not yet shown sustained heat across two scans.");
  } else if (!previousHeatConfirmed) {
    reasons.push("Candidate failed the sustained-heat gate in the previous scan.");
  }
  if (
    config.risk.allowedQuoteOnly &&
    !DEFAULT_QUOTE_TOKEN_ADDRESSES.has(candidate.quoteTokenAddress.toLowerCase())
  ) {
    reasons.push(`Quote token ${candidate.quoteTokenSymbol} is outside the allowed quote set.`);
  }
  if (eligible) {
    reasons.push("Candidate passes the phase-1 readiness gates for a live buy lane.");
  }
  ...
  return {
    ...
    plannedBuyBnb: Math.min(config.risk.maxBuyBnb, config.risk.maxDailyDeployBnb),
    route: config.router,
    eligible,
    routeTradable: "unchecked",
    routeReason: eligible ? "Awaiting route preflight." : reasons[0] || undefined,
    resolvedRoute: null,
    reasons,
  };
}
...
export function buildExecutionState(
  ...
  const previousMap = new Map(previousCandidateDetails.map((detail) => [detail.tokenAddress, detail]));
  ...
  const liveTradingArmed =
    config.enabled && configured && (config.mode === "live_buy_only" || config.mode === "live_full");
  const plans = candidates
    .slice(0, 10)
    .map((candidate) => buildCandidatePlan(candidate, config, previousMap.get(candidate.tokenAddress)))
  ...
  return {
    ...
    nextAction: configured
      ...
      : readinessChecks.find((check) => !check.ready)?.detail || "Complete remaining execution checks.",
    risk: config.risk,
    gooLane,
    plans,
    cycleSummary: {
