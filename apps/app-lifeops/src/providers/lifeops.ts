  ...
  async get(
    ...
    return {
      text: [
        ...
        "Use CALL_USER or CALL_EXTERNAL for phone-call escalation or booking calls. These actions can draft or request confirmation first; they do not require the dial to happen on the first turn. Requests like 'call the dentist and reschedule my appointment' or 'phone my cable company about the outage' belong to CALL_EXTERNAL, not OWNER_CALENDAR, LIFE, or OWNER_SEND_MESSAGE.",
        ...
        "Route examples: sleep/no-call windows -> OWNER_CALENDAR(update_preferences); daily brief additions, missed-call repair, Gmail reply drafts, or group-chat handoff -> OWNER_INBOX; subscription audits or cancellations -> SUBSCRIPTIONS; travel preference memory -> UPDATE_OWNER_PROFILE; clinic-doc reminders or multi-device meeting ladders -> PUBLISH_DEVICE_INTENT; portal upload or browser filing -> LIFEOPS_COMPUTER_USE; if the agent gets stuck and should phone the owner -> CALL_USER.",
