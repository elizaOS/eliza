  ...
  async get(
    ...
    return {
      text: [
        ...
        "Use SCHEDULING only for an explicit multi-turn negotiation record that already has proposal/response state to manage. Do not use SCHEDULING for first-turn recurring blocks, bundled travel scheduling, or missed-call repair.",
        ...
        "Use CROSS_PLATFORM_GATEWAY for real cross-platform handoffs: creating shared group chats on a messaging platform, or escalating something back to the owner when direct negotiation, signature, or personal intervention is required. Examples: 'create a group chat with Alice on Discord' or 'I need you to escalate this back to me because I have to sign it myself'. Do not route these through OWNER_INBOX just to suggest a handoff.",
        ...
        "Route examples: sleep/no-call windows -> UPDATE_MEETING_PREFERENCES; daily brief additions, missed-call repair, or group-chat handoff -> INBOX; 'if direct relaying gets messy here, suggest making a group chat handoff instead' -> INBOX; subscription audits or cancellations -> SUBSCRIPTIONS; travel preference memory -> UPDATE_OWNER_PROFILE; clinic-doc reminders or multi-device meeting ladders -> PUBLISH_DEVICE_INTENT; portal upload or browser filing -> LIFEOPS_COMPUTER_USE; if the agent gets stuck and should phone the owner -> CALL_USER.",
