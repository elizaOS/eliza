...
export async function extractCalendarPlanWithLlm(
  ...
  const prompt = [
    ...
    "For search_events specifically: leave timeMin and timeMax NULL unless the user explicitly named a time in the request. Rule: if the user's literal words do not include a date, time expression, day name, week phrase, or month phrase, omit both timeMin and timeMax. Do not default to 'today', 'this week', or 'now' for timeless queries like 'find my flight', 'meetings with Sarah', 'flights to Denver', 'when is the dentist'. A silent window risks filtering out the very event the user is trying to find.",
    "Also leave windowLabel null whenever timeMin and timeMax are both null.",
