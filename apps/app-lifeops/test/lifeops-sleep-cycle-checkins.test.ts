...
type PatchableLifeOpsService = LifeOpsService & {
  getInbox?: LifeOpsService["getInbox"];
  getGmailTriage?: LifeOpsService["getGmailTriage"];
  getCalendarFeed?: LifeOpsService["getCalendarFeed"];
  syncXDms?: LifeOpsService["syncXDms"];
  getXDms?: LifeOpsService["getXDms"];
  syncXFeed?: LifeOpsService["syncXFeed"];
  getXFeedItems?: LifeOpsService["getXFeedItems"];
};
...
async function createFixture(name: string): Promise<Fixture> {
  ...
  const service = new LifeOpsService(testRuntime.runtime);
  const patchableService = service as PatchableLifeOpsService;
  patchableService.getInbox = async () => ({
    messages: [],
    channelCounts: {},
    unreadCount: 0,
    generatedAt: new Date(0).toISOString(),
  });
  patchableService.getGmailTriage = async () => ({
    messages: [],
    summary: {
      totalCount: 0,
      unreadCount: 0,
  ...
  patchableService.getCalendarFeed = async () => ({
    events: [],
    calendars: [],
    generatedAt: new Date(0).toISOString(),
  });
  patchableService.syncXDms = async () => ({ synced: 0 });
  patchableService.getXDms = async () => [];
  patchableService.syncXFeed = async () => ({ synced: 0 });
  patchableService.getXFeedItems = async () => [];
