...
async function ensureSafariDevelopMenu() {
  await run("defaults", ["write", "com.apple.Safari", "IncludeDevelopMenu", "-bool", "true"]);
}
...
async function readSafariDeveloperFlags() {
  ...
  `);
  const [remoteAutomation, unsignedExtensions, jsFromAppleEvents, authPromptVisible] =
    raw.split(",");
  return {
...
async function readSafariWebExtensionsPlist() {
  try {
    const { stdout } = await run("plutil", ["-convert", "json", "-o", "-", safariExtensionsPlist], {
      cwd: extensionRoot,
    });
...
export function lifeOpsSafariPopupCandidates(extensionKeys) {
...
async function installTemporarySafariExtension() {
  ...
  const addedKeys = Object.keys(after).filter((key) => !Object.hasOwn(before, key));
  const lifeOpsKeys = Object.keys(after).filter((key) =>
    /lifeops/i.test(key) || /ai\.lifeops/i.test(key),
  );
  return [...new Set([...addedKeys, ...lifeOpsKeys])];
}
...
async function enableLifeOpsExtensionInSafari() {
          ...
          tell table 1 of scroll area 1 of group 2 of group 1 of group 1
            repeat with currentRow in rows
              try
                ...
                if rowLabel contains "LifeOps" then
  ...
  if (!rowName) {
    throw new Error(
      "Safari did not show a LifeOps extension row after the temporary install step. Reopen Safari > Settings > Extensions and verify that LifeOps Browser appears there.",
    );
  }
  return rowName;
}
...
async function startMockLifeOpsServer() {
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && url.pathname === "/api/lifeops/browser/companions/auto-pair") {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({
          companion: {
            id: "lifeops-browser-safari-smoke",
            agentId: "agent-safari-smoke",
            browser: "safari",
            profileId: "default",
            profileLabel: "Default",
            label: "LifeOps Browser Safari smoke",
          ...
          config: {
            ...
            companionId: "lifeops-browser-safari-smoke",
    ...
    if (req.method === "POST" && url.pathname === "/api/lifeops/browser/companions/sync") {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({
          companion: {
...
async function resolveSafariPopupUrl(extensionKeys) {
  const candidates = lifeOpsSafariPopupCandidates(extensionKeys);
  for (const candidate of candidates) {
    await openSafariUrl(candidate, true);
    const state = await readSafariFrontTabState().catch(() => null);
    if (state && typeof state.title === "string" && state.title.trim().length > 0) {
      return candidate;
    }
  }
  throw new Error(
    `Could not resolve a working Safari popup URL for LifeOps Browser. Tried: ${candidates.join(", ")}`,
  );
}
...
export async function main() {
  ...
  if (extensionKeys.length === 0) {
    throw new Error(
      "Safari did not register a LifeOps temporary extension after the install step.",
    );
  }
  await enableLifeOpsExtensionInSafari();
  ...
  try {
    ...
    await waitForSafariPopup((state) => state.title && state.title !== "Loading extension state…", 20_000);
    ...
    console.log("LifeOps Browser Safari smoke checks passed.");
  } finally {
