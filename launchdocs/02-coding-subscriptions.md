...
- Native desktop Codex credential scan is stale for newer Codex CLI OAuth shape. Runtime `readCodexCliAuthAccessToken()` supports `tokens.access_token`, but native scanning only detects `.codex/auth.json` when `OPENAI_API_KEY` exists (`credentials.ts:113-132`). This can make desktop onboarding/preflight miss a valid Codex CLI login.
...
- Update native credential scan to parse Codex `tokens.access_token` and normalize/report `openai-codex`; add `Z_AI_API_KEY` to native env scanning.
