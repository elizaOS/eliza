...
Android system utilities exist as real overlay app packages: Contacts, Phone, and WiFi each declare `androidOnly: true`, register an overlay app component, and expose a runtime plugin half (`apps/app-contacts/package.json:31`, `apps/app-phone/package.json:31`, `apps/app-wifi/package.json:31`). However, I could not find their renderer side-effect imports in `packages/app/src/main.tsx` or package deps in `packages/app/package.json`, even though runtime plugin loading comments say overlay UI registration happens through those imports (`packages/agent/src/runtime/plugin-collector.ts:324`, `packages/agent/src/runtime/core-plugins.ts:35`).

Screenshare has the most complete new-style app metadata among hidden utility apps: `kind: "app"`, launch type, viewer URL, sandbox, session features, capabilities, runtime plugin, and bridge export are all declared (`apps/app-screenshare/package.json:29`). It is intentionally hidden from Apps browse (`packages/app-core/src/components/apps/helpers.ts:50`), and I did not find a renderer import/package dependency that would make it user-discoverable through the new catalog shell.

Workflow Builder is exposed to users through the internal tool list and opens `/automations`, but the package under `apps/app-workflow-builder` is only a minimal metadata shell with a stub `registerWorkflowBuilderApp()` (`apps/app-workflow-builder/package.json:15`, `apps/app-workflow-builder/src/register.ts:8`).
...
- Android utility app packages: `apps/app-contacts/package.json:31`, `apps/app-contacts/src/register.ts:13`, `apps/app-contacts/src/components/contacts-app.ts:14`, `apps/app-phone/package.json:31`, `apps/app-phone/src/register.ts:14`, `apps/app-phone/src/components/phone-app.ts:14`, `apps/app-wifi/package.json:31`, `apps/app-wifi/src/register.ts:14`, `apps/app-wifi/src/components/wifi-app.ts:14`.
- Hidden utility/productivity packages: `apps/app-screenshare/package.json:29`, `apps/app-screenshare/src/routes.ts:76`, `apps/app-screenshare/test/routes.test.ts:122`, `apps/app-workflow-builder/package.json:15`, `apps/app-workflow-builder/src/register.ts:8`, `apps/app-browser/package.json:1`, `apps/app-form/package.json:20`, `apps/app-knowledge/package.json:22`, `apps/app-task-coordinator/package.json:22`.
...
- Android system utility plugin tests passed when run from each app directory:
  - `apps/app-contacts`: `bunx vitest run --config ./vitest.config.ts test/plugin.test.ts` passed, 3 tests.
  - `apps/app-phone`: `bunx vitest run --config ./vitest.config.ts test/plugin.test.ts` passed, 3 tests.
  - `apps/app-wifi`: `bunx vitest run --config ./vitest.config.ts test/plugin.test.ts` passed, 3 tests.
- `apps/app-workflow-builder` typechecked cleanly with `bun run typecheck`.
...
- I could not validate Screenshare routes through a targeted test command. `apps/app-screenshare` has no local `vitest.config.ts`, and direct root Vitest filters did not discover `apps/app-screenshare/test/routes.test.ts` from this repo cwd. The test file itself exists and covers route/session behavior (`apps/app-screenshare/test/routes.test.ts:122`).
...
- Workflow Builder is user-visible through the internal tool table, but the package registration is a stub (`apps/app-workflow-builder/src/register.ts:8`). The current user-facing app is effectively an alias to `/automations`, not a complete standalone app package.

### P3

- New app formatting is uneven across utility packages. Screenshare has rich metadata (`apps/app-screenshare/package.json:29`), Contacts/Phone/WiFi have enough Android-only system metadata (`apps/app-contacts/package.json:31`), Workflow Builder has only display/category/hero (`apps/app-workflow-builder/package.json:15`), and hidden plugin-style apps such as Form, Knowledge, and Task Coordinator mostly declare only a hero image (`apps/app-form/package.json:20`, `apps/app-knowledge/package.json:22`, `apps/app-task-coordinator/package.json:22`).
...
- Either make `apps/app-workflow-builder` a real self-registering app package or document/remove the stub so the internal `/automations` route is clearly the source of truth.
...
| Browser | Plugin/runtime package, hidden from Apps browse, no app-card metadata. | Static package review only; deeper browser QA is in TODO 11. | OK as hidden infrastructure unless product expects app tile. |
