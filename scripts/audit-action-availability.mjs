...
const actions = files.flatMap(scanFile).sort((a, b) =>
...
function needsHardStateReview({ name, contexts, flags, hasConnectorAccountPolicy }) {
	...
	if (flags.kind === "keyword_or_intent" || flags.kind === "missing") return true;
	if (hasConnectorAccountPolicy) return false;
	if (sensitiveContext && flags.kind === "always_available") return true;
...
function buildSummary(items) {
	return {
		...
		missingRoleGate: items.filter((item) => !item.hasRoleGate).length,
...
function getProperty(node, name) {
	return node.properties.find((property) => propertyName(property.name) === name);
}
