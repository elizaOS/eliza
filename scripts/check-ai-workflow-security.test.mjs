...
describe("AI workflow security policy", () => {
  ...
  it("validates untrusted changed skills without secrets, models, writes, or persistent runners", () => {
    ...
    assert.match(source, /--requirement trusted\/packages\/skills\/skills\/skill-creator\/requirements\.txt/);
    assert.match(source, /--require-hashes/);
  });

  it("pins PyYAML with hashes in the skill-creator requirements file", () => {
    const reqs = readFileSync(new URL(`../${REQUIREMENTS_PATH}`, import.meta.url), "utf8");
    assert.match(reqs, /PyYAML==6\.0\.3/);
    assert.match(reqs, /--hash=sha256:[0-9a-f]{64}/);
  });
