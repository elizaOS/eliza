...
function openAICompatiblePayload({ model, callIndex, provider }) {
  ...
  return {
    ...
    messages: [
      ...
      {
        role: "user",
        content: `Dynamic message ${callIndex}: choose whether to reply or call a music action.`,
      },
    ],
    tools: [plannerTool()],
    tool_choice: "auto",
...
function anthropicPayload({ model, callIndex }) {
  return {
    model,
    max_tokens: 64,
    temperature: 0,
...
function dryRun(args) {
  ...
  const payload =
    config.kind === "anthropic"
      ? anthropicPayload({ model: config.model, callIndex: 1 })
      : openAICompatiblePayload({ model: config.model, callIndex: 1, provider });
  const observations = [
...
async function main() {
  ...
  for (let index = 1; index <= calls; index += 1) {
    const payload =
      config.kind === "anthropic"
        ? anthropicPayload({ model: config.model, callIndex: index })
        : openAICompatiblePayload({ model: config.model, callIndex: index, provider });
