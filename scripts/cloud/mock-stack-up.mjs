...
async function waitForHttp(url, { timeoutMs = 60_000, intervalMs = 500 } = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      ...
      // Any HTTP response (even 404/501) means the server is listening.
...
const services = []; // { name, color, proc, logPath, tail: string[] }
...
function dumpTail(svcName) {
  ...
  for (const line of svc.tail.slice(-30)) {
...
async function main() {
  ...
  // Reset PGlite if asked.
  if (flags.reset) {
    if (existsSync(PGDATA_DIR)) {
  ...
  // Install signal handlers up front so partial-boot Ctrl+C still cleans up.
  process.on("SIGINT", () => void shutdown(0));
  process.on("SIGTERM", () => void shutdown(0));

  // Migrations (synchronous boot-time step).
  ...
  // Watch for unexpected child exits.
  for (const svc of services) {
