...
# the single active `elizaos/eliza-1` bundle repo if it does not exist yet.
#
# Context: the active publisher writes all tiers to `elizaos/eliza-1`
...
# a `repo move`, so moving (not re-uploading) is the right operation. This helper
# only handles the active Qwen3.5 0.8B/2B/4B Eliza-1 release line; larger 9B/27B
# experiments stay hidden until final weights, evals, licenses, checksums, and
...
#   scripts/hf-transfer-eliza1.sh --execute --skip-moves     # only create the active eliza-1 repo
#
...
# --- canonical active model repo to create (elizaos/eliza-1) ------------------
# Do not create new per-tier model repos for the active release line. The app and
# publisher consume the single `elizaos/eliza-1` model repo under
# `bundles/<tier>/...`.
TIER_REPOS=(
  "eliza-1"
)
...
if [[ "$SKIP_CREATES" -eq 0 ]]; then
  echo "--- 2. Create canonical active model repo under ${DST_ORG} (empty; publish fills it) ---"
