...
async function listTrajectoryFiles(): Promise<TrajectoryFile[]> {
	...
	let stack: string[] = [TRAJECTORY_DIR];
...
async function cmdList(opts: ListOptions): Promise<void> {
	...
	for (const file of files.slice(0, opts.limit ?? 20)) {
		try {
			...
			console.error(c.yellow(`skip ${file.filePath}: ${(err as Error).message}`));
		}
	}
	...
	const header =
		`${"id".padEnd(widths.id)}  ${"started".padEnd(widths.started)}  ${"dur".padEnd(widths.duration)}  ${"stages".padEnd(widths.stages)}  ${"tools".padEnd(widths.tools)}  ${"cost".padEnd(widths.cost)}  ${"decision".padEnd(widths.decision)}  ${"agent".padEnd(widths.agent)}`;
...
function printStage(stage: RecordedStage, idx: number, opts: PrintOptions): void {
	...
	if (stage.tool) {
		...
		console.log(`${c.dim("tool:")}     ${t.name} · ${tag} · ${formatDuration(t.durationMs)}`);
		console.log(`${c.dim("args:")}     ${JSON.stringify(t.args)}`);
		console.log(`${c.dim("result:")}   ${truncate(JSON.stringify(t.result), opts.full ? Number.POSITIVE_INFINITY : 1500)}`);
	}

	if (stage.evaluation) {
		const e = stage.evaluation;
		const tag = e.success ? c.green("success: true") : c.red("success: false");
		console.log(`${c.dim("eval:")}     ${tag} · decision: ${c.cyan(e.decision)}`);
		if (e.thought) console.log(`${c.dim("thought:")}  ${e.thought}`);
		if (e.messageToUser) console.log(`${c.dim("user:")}     ${e.messageToUser}`);
	}
...
async function cmdPrint(idOrPath: string, opts: PrintOptions): Promise<void> {
	...
	stages.forEach((stage, idx) => printStage(stage, idx, opts));
}
...
async function cmdStats(idOrPath: string): Promise<void> {
	...
	console.log(
		`Stages: ${t.stages.length} (${stageBreakdown || "—"})`,
	);
	console.log(
...
async function cmdFailures(idOrPath: string): Promise<void> {
	...
	for (const stage of failed) {
		printStage(stage, t.stages.indexOf(stage), { full: true, stageId: undefined });
	}
}
...
async function cmdDiff(idOrPath: string, opts: DiffOptions): Promise<void> {
	...
	console.log(`  ${a.stageId}  ${a.kind}${a.iteration ? ` iter ${a.iteration}` : ""}`);
	console.log(c.bold("Stage B"));
	console.log(`  ${b.stageId}  ${b.kind}${b.iteration ? ` iter ${b.iteration}` : ""}`);
	console.log("");
	...
	console.log("");
	console.log(c.dim("Cache implication: B can hit A's cached prefix where shared."));
}
...
function exportMarkdown(t: RecordedTrajectory): string {
	...
	lines.push(`- total: ${formatDuration(t.metrics?.totalLatencyMs ?? 0)} · ${formatUsd(t.metrics?.totalCostUsd ?? 0)}`);
	lines.push(`- root message: ${t.rootMessage?.text ?? "—"}`);
	lines.push("");

	t.stages.forEach((stage, idx) => {
		lines.push(`## Stage ${idx + 1}: ${stage.kind}${stage.iteration ? ` iter ${stage.iteration}` : ""} (${stage.stageId})`);
		lines.push("");
		lines.push(`- latency: ${formatDuration(stage.latencyMs)}`);
		if (stage.model) {
			lines.push(`- model: \`${stage.model.modelName ?? stage.model.modelType}\` (${stage.model.provider})`);
			if (stage.model.usage) {
				lines.push(`- usage: ${stage.model.usage.promptTokens} in · ${stage.model.usage.completionTokens} out · cache-read ${stage.model.usage.cacheReadInputTokens ?? 0}`);
			}
			lines.push("```");
			lines.push("PROMPT:");
		...
		if (stage.tool) {
			lines.push(`- tool \`${stage.tool.name}\` ${stage.tool.success ? "ok" : "FAIL"} ${formatDuration(stage.tool.durationMs)}`);
			lines.push("```json");
			lines.push(JSON.stringify({ args: stage.tool.args, result: stage.tool.result }, null, 2));
			lines.push("```");
		}
		if (stage.evaluation) {
			lines.push(`- evaluation: success=${stage.evaluation.success}, decision=${stage.evaluation.decision}`);
			if (stage.evaluation.thought) lines.push(`  - thought: ${stage.evaluation.thought}`);
...
async function cmdAggregate(opts: AggregateOptions): Promise<void> {
	...
	const totalCost = trajectories.reduce((sum, traj) => {
		...
		const derived = traj.stages.reduce((stageSum, stage) => {
			if (stage.model?.usage) {
				return (
					stageSum + computeCallCostUsd(stage.model.modelName, stage.model.usage)
				);
			}
			return stageSum;
		}, 0);
	...
	console.log(`Median duration: ${formatDuration(median)} · p95: ${formatDuration(p95)}`);
...
function flagInt(flags: Map<string, string | true>, key: string): number | undefined {
...
function flagDate(flags: Map<string, string | true>, key: string): number | undefined {
	const v = flagString(flags, key);
	if (v === undefined) return undefined;
