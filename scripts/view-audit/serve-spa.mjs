...
function mockApi(path) {
  if (path === "/api/status") return J({ state: "running", agentName: "Eliza", model: "local", canRespond: true, startedAt: 0, uptime: 1, startup: { phase: "running", attempt: 0 }, cloud: { connectionStatus: "disconnected", cloudProvisioned: false, hasApiKey: false } });
  if (path === "/api/auth/status") return J({ required: false, pairingEnabled: false });
  if (path.startsWith("/api/views")) return J({ views: [] });
  if (path.startsWith("/api/plugins")) return J({ plugins: [] });
  if (path.startsWith("/api/settings") || path.startsWith("/api/secrets")) return J({ entries: [], settings: {} });
...
serve({
  ...
  async fetch(req) {
    ...
    if (u.pathname === "/" || !existsSync(p) || !u.pathname.includes(".")) p = ROOT + "/index.html";
    const f = Bun.file(p);
