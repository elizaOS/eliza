...
const DESTRUCTIVE = /delete|remove|reset|disconnect|sign ?out|log ?out|\bclear\b|wipe|revoke|leave|uninstall|deactivate|forget|erase|unpair|trash|destroy|purge|\bdrop\b|stop agent|shut ?down|factory|delete account/i;
...
async function connect() {
  ...
  ws.onmessage = (e) => {
    ...
    if (m.method === "Runtime.exceptionThrown") { const d = m.params?.exceptionDetails; exc.push((d?.exception?.description || d?.text || "exc").slice(0, 220)); }
    else if (m.method === "Runtime.consoleAPICalled" && m.params?.type === "error") { const t = (m.params.args || []).map((a) => a.value ?? a.description ?? "").join(" "); if (t.trim()) exc.push("console:" + t.slice(0, 220)); }
  };
  const send = (method, params) => { const mid = ++id; try { ws.send(JSON.stringify({ id: mid, method, params })); } catch { return Promise.resolve({ __dead: 1 }); } return Promise.race([new Promise((r) => pending.set(mid, r)), sleep(8000).then(() => ({ __timeout: 1 }))]); };
  const ev = async (expr) => { const r = await send("Runtime.evaluate", { expression: expr, awaitPromise: true, returnByValue: true }); if (r.__timeout || r.__dead) return { __fail: 1 }; if (r.result?.result?.value === undefined && r.result?.exceptionDetails) return { __fail: 1 }; return r.result?.result?.value; };
  await new Promise((r) => { ws.onopen = r; setTimeout(r, 3500); });
  await send("Runtime.enable", {});
  return { ev, exc, close: () => { try { ws.close(); } catch {} } };
}
...
const nav = (u) => `(()=>{history.pushState(null,"",${JSON.stringify(u)});window.dispatchEvent(new PopStateEvent("popstate"));if(${JSON.stringify(u)}.includes("#"))window.dispatchEvent(new HashChangeEvent("hashchange"));return 1})()`;

async function main() {
  let conn = await connect();
  const reconnect = async () => { conn.close(); await sleep(2000); for (let t = 0; t < 6; t++) { try { conn = await connect(); return true; } catch { await sleep(3000); } } return false; };
  const health = async () => { const r = await conn.ev("location.pathname"); if (r && r.__fail === undefined) return true; return await reconnect(); };
  const scan = async () => { try { return JSON.parse(await conn.ev(SCAN) || "{}"); } catch { return {}; } };

  const FROM = +(args.from || 0), TO = +(args.to || ROUTES.length);
  const SLICE = ROUTES.slice(FROM, TO);
  const REPORT = args.report || `${OUT}/report.json`;
  const results = [];
  for (const [route, slug] of SLICE) {
    if (!(await health())) { await conn.ev(nav("/chat")); await sleep(2500); }
    await conn.ev(nav(route)); await sleep(2400); // settle + data fetch
    conn.exc.length = 0;
    const land = await scan();
    if (land.__fail) { if (!(await health())) break; continue; }
    const landErrs = [...new Set(conn.exc)];
    let controls = [];
    try { controls = JSON.parse(await conn.ev(DISCOVER) || "[]"); } catch {}
    ...
    for (const c of controls) {
      ...
      if (ok && ok.__fail) { if (!(await health())) break; continue; }
      await sleep(800);
      const s = await scan();
      if (s.__fail) { if (!(await health())) break; await conn.ev(nav(route)); await sleep(1500); continue; }
      const errs = [...new Set(conn.exc)];
      const navd = s.path && s.path !== route && s.path !== route.split("#")[0];
      if ((s.reds || []).length || errs.length || s.dialog || navd || c.k === "tab") {
        subStates.push({ trigger: c.l, kind: c.k, reds: s.reds || [], errors: errs, txt404: s.txt404, modal: s.dialog, navigated: navd ? s.path : null, snip: (s.snip || "").slice(0, 90) });
      }
      // soft recover
      if (s.dialog) { await conn.ev(closeModal); await conn.ev("document.activeElement&&document.activeElement.blur()"); await sleep(250); const st = await scan(); if (st.dialog) { await conn.ev(nav(route)); await sleep(1300); } }
      else if (navd) { await conn.ev(nav(route)); await sleep(1300); }
      else { await sleep(150); }
    }
    const f = { slug, route, landReds: land.reds || [], land404: !!land.txt404, landErrors: landErrs, landSnip: (land.snip || "").slice(0, 100), controls: controls.length, subStates };
    results.push(f);
    const redTotal = (f.landReds.length) + f.subStates.reduce((a, s) => a + s.reds.length, 0);
    const errTotal = f.landErrors.length + f.subStates.reduce((a, s) => a + s.errors.length, 0);
    ...
    console.error(line); appendFileSync(`${OUT}/progress.log`, line + "\n");
    writeFileSync(REPORT, JSON.stringify({ count: results.length, results }, null, 2));
  }
