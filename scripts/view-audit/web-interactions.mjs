...
const DESTRUCTIVE = /delete|remove|reset|disconnect|sign out|log ?out|clear|wipe|revoke|leave|uninstall|deactivate|danger|forget|erase|drop|unpair|trash|destroy/i;
...
async function main() {
  ...
  for (const [route, slug] of ROUTES) {
    const goView = () => page.evaluate((u) => { history.pushState(null, "", u); window.dispatchEvent(new PopStateEvent("popstate")); if (u.includes("#")) window.dispatchEvent(new HashChangeEvent("hashchange")); }, route).catch(() => {});
    await goView();
    await sleep(1500);
    ...
    try {
      controls = await page.evaluate((destrSrc) => {
        ...
        return [...document.querySelectorAll('button,[role="button"]')]
          .map((e, i) => ({ i, l: lbl(e), dis: !!e.disabled || e.getAttribute("aria-disabled") === "true", vis: !!(e.offsetParent || e.getClientRects().length) }))
    ...
    for (let k = 0; k < max; k++) {
      ...
      try {
        const handle = await page.evaluateHandle((lab) => {
          const lbl = (e) => ((e.getAttribute("aria-label") || e.getAttribute("title") || e.textContent || "").replace(/\s+/g, " ").trim());
          return [...document.querySelectorAll('button,[role="button"]')].find((e) => lbl(e) === lab && (e.offsetParent || e.getClientRects().length) && !e.disabled);
        }, target.l);
      ...
      try {
        errBoundary = await page.evaluate(() => /something went wrong|failed to load|is not a function|cannot read prop/i.test(document.body.innerText || ""));
      } catch {}
      page.off("pageerror", onErr);
      if (clicked && (pageErrors.length || errBoundary)) {
        issues.push({ button: target.l.slice(0, 40), jsError: pageErrors[0] || null, errorBoundary: errBoundary });
      }
      // recover: if path moved or a dialog/overlay opened, go back to the view + dismiss
      await page.keyboard.press("Escape").catch(() => {});
      const onView = await page.evaluate((r) => location.pathname + location.hash === r || location.pathname === r.split("#")[0], route).catch(() => true);
      if (!onView) { await goView(); await sleep(900); }
    }
    ...
    const f = { slug, route, buttonsTested: max, buttonsAvailable: controls.length, buttonIssues: issues, inputIssues };
    results.push(f);
    ...
    writeFileSync(`${OUT}/report-web-interactions.json`, JSON.stringify({ count: results.length, results }, null, 2));
  }
  await browser.close();
  console.error("DONE");
}
