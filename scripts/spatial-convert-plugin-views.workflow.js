...
5. Run only your new test: bun run --cwd plugins/${spec.plugin} vitest run src/components/<Name>SpatialView.test.tsx (or the path you used). Iterate until it passes.
...
Report created files, whether the test passed, and a one-line summary. If you cannot make the test pass, set testPassed=false and include the failure output.`,
      {
