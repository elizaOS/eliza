#!/usr/bin/env node
/**
 * Strips fields from a `package.json` that are safe in a workspace but unsafe
 * to publish to npm:
 *
 *   - `overrides` entries that use a local protocol (workspace:, file:, link:,
 *     portal:) or that conflict with a direct dependency's specifier.
 *   - Top-level `bundleDependencies` / `bundledDependencies` arrays.
 *
 * Operates on the calling cwd by default. Pass `--dry-run` to preview without
 * writing.
 */
