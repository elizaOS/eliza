...
const agentEnv = {
  ...
  OLLAMA_BASE_URL: envDefault("OLLAMA_BASE_URL", "http://127.0.0.1:11434"),
...
const services = [
  ...
  },
  {
    name: "agent",
    cwd: ".",
    command: ["bun", "run", "--cwd", "packages/agent", "start"],
    env: agentEnv,
  },
  {
    name: "frontend",
...
const defaultPrepareCommands = [
  ["ui package build", "packages/ui", ["bun", "run", "build:dist"], commonEnv],
  [
    "wallet plugin build",
  ...
  [
    "cloud dev vars",
    "cloud",
    ["bun", "run", "packages/scripts/sync-api-dev-vars.ts"],
    cloudSharedEnv,
  ],
];
const prepareCommands = fullPrepare
  ? [["dev:prepare", ".", ["bun", "run", "dev:prepare"], commonEnv]]
