...
| Eliza APK installed | The kit pushes onto an existing app; it does not install one. | `adb install path/to/eliza-arm64.apk` once. |
...
|---|---|---|
| Chat | `elizalabs/eliza-1` | `bundles/2b/text/eliza-1-2b-32k.gguf` |
...
4. `~/.eliza/local-inference/models/`

If it is not found, download it with:

```bash
hf download elizalabs/eliza-1 \
...
| `ANDROID_RUNNER_PACKAGE` | `ai.eliza.eliza` | Package id. |
