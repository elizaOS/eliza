...
- **DFlash** speculative decoding via milady-ai/qwen3.5-4b-milady-drafter

## Status

**Placeholder.** The GGUF is not yet uploaded. W5-Pipeline produces
the file from `Qwen/Qwen3.5-4B`; once it lands, the publisher
...
# Catalog id: qwen3.5-4b-milady-optimized

# Manual download:
huggingface-cli download milady-ai/qwen3.5-4b-milady-optimized qwen3.5-4b-milady-optimized.gguf

# llama-server (with the milady fork):
./llama-server --hf-repo milady-ai/qwen3.5-4b-milady-optimized \
  --hf-file qwen3.5-4b-milady-optimized.gguf \
  --alias milady --ctx-size 8192 --flash-attn
```

## Repo layout

- `qwen3.5-4b-milady-optimized.gguf` — the fused-stack GGUF.
