...
Historical note: this directory was formerly `eliza-ai-repos/` and the
repos lived under the `eliza-ai` HF namespace. The canonical namespace
is now `elizaos/eliza-1-*` (the `eliza-ai` GitHub/HF org was transferred
to `elizaos`). The `-eliza-optimized` / `-eliza-drafter` bundle infix
