...
import { useResetGame, useSetRunning, useStartGameRound } from "../hooks/useTownControls";
...
export default function PlayerDetails({
  ...
  const selectedProfile = TOWN_AGENTS.find((agent) => agent.id === selectedAgentId);
  ...
  const assignedObjectives = selectedAgentId
    ? objectives.filter((objective) => objective.assignedAgentIds.includes(selectedAgentId))
    : [];
  const assignedCompleted = assignedObjectives.filter(
    (objective) => objective.status === "completed",
  ).length;
  const objectiveCompleted = objectives.filter((objective) => objective.status === "completed")
    .length;

  const sheriffId = gameState?.players.find((player) => player.role === "detective")?.agentId;
  ...
  const aliveCount = gameState?.players.filter((player) => player.alive).length ?? 0;
  const deadCount = gameState?.players.length ? gameState.players.length - aliveCount : 0;
  const canStartRound = Boolean(
    gameState && canRun && (gameState.phase === "setup" || gameState.phase === "ended"),
  );
  const canResumeRound = Boolean(
    gameState && canRun && (gameState.phase === "night" || gameState.phase === "day"),
  );
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          <div className="h-full bg-clay-700 text-center px-3 py-1">
            {isRunning ? "Pause Round" : canResumeRound ? "Resume Round" : "Start Round"}
          </div>
        </button>
        <button
          className="button text-white shadow-solid text-xs cursor-pointer"
        ...
        >
          <div className="h-full bg-clay-700 text-center px-3 py-1">Reset Round</div>
        </button>
        <button
          className="button text-white shadow-solid text-xs cursor-pointer"
          onClick={onOpenSettings}
        >
          <div className="h-full bg-clay-700 text-center px-3 py-1">Settings</div>
        </button>
      </div>
          ...
          {gameState ? (
            <>
              <div className="flex flex-wrap gap-3 text-xs uppercase tracking-wider opacity-80">
                ...
                {gameState.winner !== "none" && <span>Winner: {gameState.winner}</span>}
              </div>
              ...
              {gameState.lastEvent && (
                <div className="text-xs opacity-80">Last event: {gameState.lastEvent}</div>
              )}
              {sheriffId && (
              ...
              <div className="mt-2 text-xs uppercase tracking-wider opacity-70">Players</div>
              <div className="flex flex-col gap-2">
                {gameState.players.map((player) => {
                  const name = agentNameById.get(player.agentId) ?? player.agentId;
                  ...
                  const revealedRole = player.revealedRole
                    ...
                    : null;
                  const showRole =
                    showAllRoles || revealedRole !== null || player.role === "detective";
                  ...
                  return (
                    <div
                      ...
                      className={`rounded border px-3 py-2 ${
                        isDead ? "border-brown-600/60 opacity-70" : "border-brown-700/80"
                      } ${isSelected ? "ring-2 ring-amber-300" : ""}`}
                    >
                      <button
                        className="w-full text-left"
                        onClick={() => setSelectedElement({ kind: "player", id: player.agentId })}
                      >
                        ...
                        {isSelected && selectedAgent && (
                          <div className="mt-1 text-xs opacity-80">
                            Vision: {selectedAgent.visionRangeTiles} tiles · Hearing:{" "}
                            {selectedAgent.audioRangeTiles} tiles
                          </div>
                        )}
                      ...
                      {suspectedMafia.includes(player.agentId) && (
                        <div className="text-xs text-amber-200">Marked as mafia</div>
                      )}
                      {!isDead && (
                        <button
              ...
              {gameState.eliminations.length > 0 && (
                <div className="mt-3 rounded border border-brown-700/80 p-3 text-xs space-y-2">
                  <div className="uppercase tracking-wider opacity-80">Eliminations</div>
                  {gameState.eliminations.map((entry, index) => (
                    <div key={`${entry.agentId}-${entry.round}-${index}`}>
                      ...
                      {entry.revealedRole} · {entry.reason} · round {entry.round} ({entry.phase})
                    </div>
                  ))}
                </div>
              )}
              ...
              {gameState.phase === "night" && nightStatus && (
                <div className="mt-3 rounded border border-brown-700/80 p-3 text-xs space-y-2">
                  <div className="uppercase tracking-wider opacity-80">Night actions</div>
                  <div className="opacity-80">
                    Goal: mafia kill, sheriff investigate, doctor protect.
                  </div>
                  <div>
                    Mafia kill:{" "}
                    {formatActionStatus(nightStatus.mafiaRequired, nightStatus.mafiaSubmitted)}
                  </div>
                  <div>
                    Sheriff investigate:{" "}
                    {formatActionStatus(
              ...
              {gameState.phase === "day" && voteStatus && (
                <div className="mt-3 rounded border border-brown-700/80 p-3 text-xs space-y-2">
                  <div className="uppercase tracking-wider opacity-80">Day vote</div>
                  <div className="opacity-80">
                  ...
                  {voteStatus.isTie && <div className="opacity-80">Tie detected.</div>}
                  {voteStatus.votesByTarget.length > 0 && (
                    <div className="space-y-1">
                      {voteStatus.votesByTarget.map((entry) => (
                        <div key={entry.target}>
                          {agentNameById.get(entry.target) ?? entry.target}: {entry.count}
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <div className="mt-3 rounded border border-brown-700/80 p-3 text-xs space-y-2">
                <div className="uppercase tracking-wider opacity-80">Objectives</div>
                {selectedAgentId ? (
                  <>
                    <div className="opacity-80">
                      {assignedCompleted}/{assignedObjectives.length} completed for{" "}
