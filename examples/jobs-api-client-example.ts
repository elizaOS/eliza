...
async function example2_createAndPoll() {
  ...
  try {
    ...
    if (result.success && result.job.result) {
      ...
      console.log(
        "Processing time:",
        result.job.result.processingTimeMs,
        "ms",
      );
    ...
    const result = await client.jobs.createAndPollWithBackoff(
      {
        userId,
        content:
          "Perform a comprehensive market analysis across multiple chains",
...
async function example7_healthMonitoring() {
  ...
  try {
    ...
    console.log(
      ...
      health.metrics.averageProcessingTimeMs,
      "ms",
    );
    console.log(
      "  Success rate:",
      (health.metrics.successRate * 100).toFixed(2),
      "%",
    );
    console.log(
      "  Failure rate:",
      (health.metrics.failureRate * 100).toFixed(2),
      "%",
    );
    console.log(
      "  Timeout rate:",
      (health.metrics.timeoutRate * 100).toFixed(2),
      "%",
    );
  } catch (error) {
    console.error("Error:", error.message);
