...
async function getClaimablePositions(wallet: string, apiKey: string): Promise<ClaimablePosition[]> {
...
async function generateClaimTx(wallet: string, tokenMint: string, apiKey: string): Promise<ClaimTxResponse[]> {
...
async function exportPrivateKey(jwtToken: string, walletAddress: string): Promise<string> {
...
async function signAndSendTx(txBase58: string, privateKeyBase58: string): Promise<string> {
...
async function claimAllFees(creds: BagsCredentials): Promise<{ claimed: number; tokens: string[]; sigs: string[] }> {
  ...
  for (const wallet of creds.wallets) {
    ...
    const claimable = positions.filter(p => p.totalClaimableLamportsUserShare >= MIN_CLAIMABLE_LAMPORTS);
...
async function main(): Promise<void> {
  ...
  console.log(`Mode:     ${RUN_ONCE ? "Single run" : `Continuous (every ${CLAIM_INTERVAL_MS / 1000 / 60} min)`}`);
