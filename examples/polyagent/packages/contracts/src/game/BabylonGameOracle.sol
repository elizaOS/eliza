...
/**
 * @title PolyagentGameOracle
 * @notice Oracle for Polyagent prediction game outcomes
 * @dev Extends PredictionOracle with Polyagent-specific features
 * 
 * Polyagent Game Integration:
 ...
 *   oracle.commitPolyagentGame(questionId, questionNumber, question, commitment, category);
 *   
 *   // 2. Users trade on Diamond PredictionMarketFacet or custom contracts
 *   
 *   // 3. Game server reveals after resolution
 *   oracle.revealPolyagentGame(sessionId, outcome, salt, teeQuote, winners, totalPayout);
 *   
 ...
 */
contract PolyagentGameOracle is PredictionOracle, Ownable, Pausable {
    ...
    mapping(bytes32 => address[]) private _polyagentWinners;
    ...
    // ============ Polyagent-Specific Functions ============
    
    /**
     * @notice Commit a Polyagent game question
    ...
    function commitPolyagentGame(
        ...
        sessionId = keccak256(abi.encodePacked("polyagent", questionId, block.timestamp));
        ...
        emit PolyagentGameCommitted(sessionId, questionId, questionNumber, question, commitment);
        
        return sessionId;
    }
    
    /**
     * @notice Reveal a Polyagent game outcome
     * @param sessionId Session ID from commitPolyagentGame
    ...
    function revealPolyagentGame(
        ...
        _polyagentWinners[sessionId] = winners;
        game.totalPayout = totalPayout;
        game.finalized = true;
        
        totalGamesRevealed++;
        
        emit PolyagentGameRevealed(sessionId, questionId, outcome, winners.length);
    }
     ...
     */
    function batchCommitPolyagentGames(
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            bytes32 _sessionId = keccak256(abi.encodePacked("polyagent", questionId, block.timestamp, i));
            ...
            emit PolyagentGameCommitted(_sessionId, questionId, questionNumbers[i], questions[i], _commitment);
     ...
     */
    function batchRevealPolyagentGames(
    ...
    function getWinners(bytes32 sessionId) external view override returns (address[] memory) {
        return _polyagentWinners[sessionId];
    }
    ...
    function getContractMetadata() external pure returns (string memory) {
        return '{"type":"prediction-oracle","subtype":"polyagent-game","name":"Polyagent Game Oracle","category":"social-prediction","version":"1.0.0"}';
    }
}
