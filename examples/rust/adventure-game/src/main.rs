...
impl AdventureGame {
    ...
    fn handle_attack(&mut self, with_sword: bool) -> String {
        ...
        let enemy = match &mut room.enemy {
            Some(e) if e.health > 0 => e,
            _ => return "There is nothing to attack here.".to_string(),
        };
...
async fn decide_action(session: &mut GameSession) -> Result<String> {
    ...
    let game_context = format!(
        r#"You are playing a text adventure game. Your goal is to explore the dungeon, collect items, defeat enemies, and find the dragon's treasure.
