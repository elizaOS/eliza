...
class AdventureAgent {
  ...
  static async initialize(): Promise<GameSession> {
    ...
    try {
      ...
      return { runtime, game, roomId, agentId };
    } catch (error) {
  ...
  static async decideAction(session: GameSession): Promise<string> {
    const { runtime, game, roomId, agentId } = session;
    ...
    const gameContext = `You are playing a text adventure game. Your goal is to explore the dungeon, collect items, defeat enemies, and find the dragon's treasure.
...
Respond with ONLY the exact action text you want to take (e.g., "go north" or "attack with sword"). Do not include any explanation, just the action.
`;
    ...
    let chosenAction = String(response).trim();
