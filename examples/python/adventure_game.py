...
async def create_session() -> GameSession:
    ...
    return GameSession(runtime=runtime, game=game)
...
async def decide_action(session: GameSession) -> str:
    ...
    game_context = f"""You are playing a text adventure game. Your goal is to explore the dungeon, collect items, defeat enemies, and find the dragon's treasure.
...
"""
    ...
    result = await runtime.use_model(
        ...
        {"prompt": game_context, "maxTokens": 50, "temperature": 0.3},
    )
