...
Certification is an operator-run release/review activity. Two manual workflows
share the same signing boundary:

- `certification-vast.yml` runs the chain on a cost-capped vast.ai instance and
  may run nightly only when `ELIZA_VAST_CERT_ENABLED` is explicitly `true`.
- `certification-hosted.yml` is the GitHub-hosted fallback when vast.ai is
  unavailable. It has no schedule and no pull-request trigger.

Both produce a signed artifact for offline verification:
