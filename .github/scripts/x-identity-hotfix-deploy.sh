...
ssh -i "$work/key" -p "$node_port" -o BatchMode=yes \
  ...
  "$node_user@$node_host" \
  "set -euo pipefail; test \"\$(docker inspect --format '{{.Config.Image}}' '$container_name')\" = '$expected_image'; docker exec '$container_name' sh -lc 'cd /app && bun -e '\''import postgres from \"postgres\"; const sql=postgres(process.env.ELIZA_MANAGED_DATABASE_URL,{max:1,ssl:{rejectUnauthorized:false}}); const query=\"SELECT r.id::text AS room_id,r.world_id::text AS room_world_id,w.id::text AS world_id,w.name,w.metadata FROM rooms r LEFT JOIN worlds w ON w.id=r.world_id WHERE r.id IN (\\\$1::uuid,\\\$2::uuid) OR jsonb_extract_path_text(w.metadata,\\\$5,\\\$6) IN (\\\$3,\\\$4) ORDER BY r.created_at DESC\"; const rows=await sql.unsafe(query,[\"47f5526f-c192-04ca-aa63-12283ea6f010\",\"d81c82a8-6992-0632-94bd-c23a925f1c04\",\"1519007261917650945\",\"1830340867737178112\",\"ownership\",\"ownerId\"]); console.log(JSON.stringify(rows)); await sql.end();'\'''"
