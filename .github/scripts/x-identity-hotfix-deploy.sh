#!/usr/bin/env bash
# Retries Eliza X after stale duplicate Headscale registrations were retired.
set -euo pipefail

agent_id=4602b3be-2c01-4e7e-9cdc-849604e1bef7
failed_job=9c1d6878-bf6f-46b0-b42a-0ec40b12804c
...
BEGIN;
UPDATE jobs
SET status = 'failed', error = 'Operator retrying after stale Headscale registrations retired',
...
TARGET_AGENT_ID="$agent_id" TARGET_ORG_ID="$organization_id" \
  /home/deploy/.bun/bin/bun --env-file=cloud/.env.local -e '
    ...
    process.stdout.write("last stale X candidate retired\n");
    process.exit(0);
  '
...
BEGIN
  ...
  IF changed <> 1 THEN RAISE EXCEPTION 'last retired X placement did not match'; END IF;
  ...
  GET DIAGNOSTICS changed = ROW_COUNT;
  IF changed <> 1 THEN RAISE EXCEPTION 'last X restart admission failed'; END IF;
END \$\$;
COMMIT;
