#!/usr/bin/env bash
# Replaces the exact serving Eliza X image with the complete identity hotfix.
set -euo pipefail

work="$(mktemp -d)"
trap 'rm -rf "$work"' EXIT
agent_id=4602b3be-2c01-4e7e-9cdc-849604e1bef7
old_image=ghcr.io/elizaos/eliza-demo@sha256:7ac3474cc7520a489918a506fb8fd48c3ddd5bafe7d91c4c2e43a976872a3ba0
new_digest=sha256:ef8891f02762177004a9250d3b297dee9999ec50d61b615937774802a7ad8e86
...
BEGIN
  ...
  SET status = 'provisioning', lifecycle_job_id = NULL,
      ...
      error_message = 'Operator staging complete X identity hotfix',
      updated_at = NOW()
...
BEGIN
  ...
  WHERE id = '$agent_id' AND status = 'provisioning'
    ...
    AND NOT EXISTS (SELECT 1 FROM jobs WHERE agent_id = '$agent_id' AND status IN ('pending','in_progress'));
  GET DIAGNOSTICS changed = ROW_COUNT;
  IF changed <> 1 THEN RAISE EXCEPTION 'staged complete X placement did not match'; END IF;
  INSERT INTO jobs (
  ...
  GET DIAGNOSTICS changed = ROW_COUNT;
  IF changed <> 1 THEN RAISE EXCEPTION 'complete X restart admission failed'; END IF;
END \$\$;
COMMIT;
