#!/usr/bin/env bash
# Replaces only the pinned Eliza X runtime image and enqueues its durable restart.
set -euo pipefail

work="$(mktemp -d)"
trap 'rm -rf "$work"' EXIT
agent_id=4602b3be-2c01-4e7e-9cdc-849604e1bef7
old_image=ghcr.io/elizaos/eliza-demo@sha256:ae4d87e30f85f7a8c662751ba16c71522f309ed163b16da519cb931aa305965a
new_digest=sha256:9b4baaaa657d1c79cee47eb00b6e2f46e2b24729ce7d9b0357e9611f27abdc49
...
BEGIN
  ...
  SET status = 'provisioning',
      ...
      error_message = 'Operator staging canonical X identity hotfix',
      updated_at = NOW()
  WHERE id = '$agent_id'
    AND status = 'running'
    AND node_id = '$node_id'
  ...
  IF changed <> 1 THEN
    RAISE EXCEPTION 'exact running X placement did not match';
  END IF;
END \$\$;
COMMIT;
SQL
...
BEGIN
  ...
  GET DIAGNOSTICS changed = ROW_COUNT;
  IF changed <> 1 THEN
    RAISE EXCEPTION 'exact staged X placement did not match';
  END IF;
  INSERT INTO jobs (
  ...
  WHERE sandbox.id = '$agent_id'
    ...
    AND sandbox.docker_image = '$new_image'
    AND sandbox.user_id IS NOT NULL;
  GET DIAGNOSTICS changed = ROW_COUNT;
  IF changed <> 1 THEN
    RAISE EXCEPTION 'exact X restart job admission failed';
  END IF;
END \$\$;
COMMIT;
