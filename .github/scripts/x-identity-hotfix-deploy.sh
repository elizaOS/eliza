#!/usr/bin/env bash
# Inspects the exact Eliza X restart lifecycle without mutating it.
set -euo pipefail
...
psql "$database_url" -v ON_ERROR_STOP=1 -x -c \
  "SELECT id, status, node_id, container_name, sandbox_id, bridge_url, docker_image, image_digest, lifecycle_job_id, lifecycle_execution_generation, error_message, replacement_cleanup_sandbox_id, replacement_cleanup_node_id, replacement_cleanup_container_name, updated_at FROM agent_sandboxes WHERE id = '$agent_id'"
psql "$database_url" -v ON_ERROR_STOP=1 -x -c \
