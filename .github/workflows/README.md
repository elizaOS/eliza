...
`repository-ruleset-drift.yml` performs an on-demand, read-only semantic
readback of the active main manifest only through the
`repository_ruleset_drift` external repository-dispatch event. It has no
schedule or arbitrary-ref manual dispatch, checks out the immutable event
`${{ github.sha }}` rather than a floating branch tip, and refuses any event
whose ref is not exact `refs/heads/develop`. It references no GitHub Environment,
creates no Deployment, and contains no apply route. A green readback proves
configuration parity only; owner audit-log review plus red/green and direct-push
