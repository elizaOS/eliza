...
>   tracking) — #11205.
>
> In flight: archive/reopen/pause wiring #11216; terminal-status dedup #11220.
...
- **`archive` / `reopen` / control-`pause`** are wired sub-actions with schema
  enum entries, similes, and a **few-shot example that trains the planner to call
  `control.pause`** — but every handler returns `UNSUPPORTED_OPERATION`. The
  plugin teaches the planner to pick dead paths.
