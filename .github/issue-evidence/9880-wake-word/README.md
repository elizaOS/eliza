...
| **Physical device** | — | Remaining: requires a signed Capacitor app build installed on the paired iPhone for on-device openWakeWord. The device path (head fast-path / two-stage ASR) is covered by the unit + host real-audio tiers; the shipped `hey-eliza` head's on-device accuracy is ~98.8% TA / ~3.6% FA (held-out 250+250) per the head-provenance commits. |
