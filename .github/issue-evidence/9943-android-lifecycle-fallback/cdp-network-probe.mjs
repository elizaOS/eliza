...
(async () => {
  ...
  const init = await evalx(ws, `(async () => {
    window.__net = [];
  ...
  let net, capnet;
  try { net = await evalx(ws, "JSON.stringify(window.__net||[])"); capnet = await evalx(ws, "JSON.stringify(window.__capnet||[])"); }
  catch { const w2 = await connect(); net = await evalx(w2, "JSON.stringify(window.__net||[])"); capnet = await evalx(w2, "JSON.stringify(window.__capnet||[])"); }
