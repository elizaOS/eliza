/**
 * UI shell for the ArkLib Clone app.
 *
 * Displays the ArkLib repository details and proximity-prize branch status,
 * giving the user a starting point for working with the δ* formal verification campaign.
 */
...
type SetupState = "idle" | "cloning" | "done" | "error";
...
function App() {
  ...
  const handleSetup = () => {
    setSetupState("cloning");
    setErrorMsg("");
    // Simulate the clone + checkout operation (actual git is done server-side via the plugin action).
    setTimeout(() => {
      try {
        setSetupState("done");
      } catch (err) {
