...
const FAULT_RULES = (label, fault) => [
  { target: "header", modifier: "X-Mockoon-Fault", value: fault, invert: false, operator: "equals" },
  { target: "query", modifier: "_fault", value: fault, invert: false, operator: "equals" },
];
...
function faultResponse(label, statusCode, body, extraHeaders = []) {
  return {
    ...
    statusCode,
    label,
    headers: [{ key: "Content-Type", value: "application/json" }, ...extraHeaders],
    bodyType: "INLINE",
    filePath: "",
    databucketID: "",
    sendFileAsBody: false,
...
function defaultFaults(provider) {
  const rate = (() => {
    switch (provider) {
      ...
      case "discord":
        return { message: "You are being rate limited.", retry_after: 1.5, global: false, code: 0 };
      case "telegram":
        return { ok: false, error_code: 429, description: "Too Many Requests: retry after 1", parameters: { retry_after: 1 } };
      case "github":
        return { message: "API rate limit exceeded.", documentation_url: "https://docs.github.com/rest/overview/resources-in-the-rest-api#rate-limiting" };
      case "notion":
        return { object: "error", status: 429, code: "rate_limited", message: "You have been rate limited. Please try again in a few minutes." };
      case "twilio":
        return { code: 20429, message: "Too Many Requests", more_info: "https://www.twilio.com/docs/errors/20429", status: 429 };
      case "plaid":
        return { error_type: "RATE_LIMIT_EXCEEDED", error_code: "RATE_LIMIT", error_message: "rate limit exceeded for client_id" };
      case "duffel":
        return { errors: [{ type: "rate_limit_error", title: "Rate limit exceeded", code: "rate_limit_exceeded", message: "Too many requests, slow down." }] };
      case "anthropic":
        return { type: "error", error: { type: "rate_limit_error", message: "Number of request tokens has exceeded your per-minute rate limit" } };
      case "cerebras":
        return { error: { type: "rate_limit_exceeded", code: "rate_limit_exceeded", message: "Rate limit reached" } };
      case "spotify":
        return { error: { status: 429, message: "API rate limit exceeded" } };
      case "ntfy":
        return { code: 42909, http: 429, error: "rate limit reached" };
      case "elizacloud":
        return { error: "rate_limited", message: "Too many requests; back off and retry." };
      default:
  ...
  const auth = (() => {
    switch (provider) {
      ...
      case "github":
        return { message: "Bad credentials", documentation_url: "https://docs.github.com/rest" };
      case "notion":
        return { object: "error", status: 401, code: "unauthorized", message: "API token is invalid." };
      case "twilio":
        return { code: 20003, message: "Authentication Error - No credentials provided", more_info: "https://www.twilio.com/docs/errors/20003", status: 401 };
      case "plaid":
        return { error_type: "INVALID_INPUT", error_code: "INVALID_ACCESS_TOKEN", error_message: "the provided access token is invalid" };
      case "duffel":
        return { errors: [{ type: "authentication_error", title: "Invalid access token", code: "invalid_access_token", message: "Your access token is invalid." }] };
      case "anthropic":
        return { type: "error", error: { type: "authentication_error", message: "invalid x-api-key" } };
      case "cerebras":
        return { error: { type: "invalid_request_error", code: "invalid_api_key", message: "Incorrect API key provided." } };
      case "spotify":
        return { error: { status: 401, message: "The access token expired" } };
      case "ntfy":
        return { code: 40101, http: 401, error: "unauthorized" };
      case "elizacloud":
        return { error: "unauthenticated", message: "Eliza Cloud API key is missing or expired." };
      default:
  ...
  return [
    faultResponse("rate_limit", 429, rate, [{ key: "Retry-After", value: "1" }]),
...
function route(method, endpoint, happyBody, provider, statusCode = 200) {
  return {
    ...
    method,
    endpoint,
    responses: [happyResponse(happyBody, statusCode), ...defaultFaults(provider)],
    responseMode: null,
  };
}
...
const PEOPLE = [
  { name: "Priya Raman", email: "priya.raman@bluepine.dev", phone: "+14155551234" },
  { name: "Marcus Okafor", email: "marcus@delta-four.io", phone: "+12025553456" },
  { name: "Mei Tanaka", email: "mei.t@nautilus.studio", phone: "+15555550101" },
  { name: "Sasha Kowalski", email: "sasha.k@northwind.work", phone: "+16175557890" },
    ...
    route("get", "channels/:channelId/messages", [
      {
        ...
        author: { id: "5550000000000001", username: "priya", discriminator: "0", global_name: PEOPLE[0].name },
...
const telegram = envelope({
  ...
  routes: [
    ...
    route("post", "bot:token/sendChatAction", { ok: true, result: true }, "telegram"),
  ],
});
    ...
    route("get", "repos/:owner/:repo/commits", [
      {
        ...
        commit: {
          author: { name: PEOPLE[0].name, email: PEOPLE[0].email, date: "2026-05-09T10:18:00Z" },
          message: "fix(lifeops): keep candidate when only one signal is fresh",
        },
...
const notion = envelope({
  ...
  routes: [
      ...
      results: [
        {
          ...
          properties: {
            title: { type: "title", title: [{ type: "text", text: { content: "Lifeops design doc" } }] },
          },
    ...
    route("patch", "v1/blocks/:blockId/children", {
      object: "list",
      results: [
        { object: "block", id: "block-aaaa-0001", type: "paragraph", paragraph: { rich_text: [{ type: "text", text: { content: "Mock appended block" } }] } },
      ],
    }, "notion"),
    route("get", "v1/databases/:databaseId", {
      object: "database",
  ...
  routes: [
    ...
    route("post", "v1/eliza/plaid/exchange", {
      ...
      institution: {
        ...
        accounts: [
          { accountId: "acct_001", name: "Plaid Checking", mask: "0000", type: "depository", subtype: "checking" },
          { accountId: "acct_002", name: "Plaid Saving", mask: "1111", type: "depository", subtype: "savings" },
        ],
      },
    }, "plaid"),
    route("post", "v1/eliza/plaid/sync", {
      added: [
        {
          ...
          pending: false,
          category: { primary: "FOOD_AND_DRINK", detailed: "FOOD_AND_DRINK_COFFEE" },
        },
        {
          ...
          merchantName: "Lyft",
          pending: true,
          category: { primary: "TRANSPORTATION", detailed: "TRANSPORTATION_RIDESHARE" },
        },
        {
...
const appleReminders = envelope({
  ...
  port: 18810,
  routes: [
    route("get", "reminders/lists", [
      { id: "list-default", name: "Reminders", color: "red", isDefault: true },
      { id: "list-followups", name: "Followups", color: "blue", isDefault: false },
      ...
      data: [
        {
          ...
          participants: [{ address: PEOPLE[0].phone, contact: { displayName: PEOPLE[0].name } }],
          lastMessage: { text: "got it, see you Thursday", dateCreated: 1746810400000 },
        },
        {
          guid: "iMessage;-;+12025553456",
          chatIdentifier: PEOPLE[1].phone,
          displayName: PEOPLE[1].name,
          participants: [{ address: PEOPLE[1].phone, contact: { displayName: PEOPLE[1].name } }],
          lastMessage: { text: "thanks!", dateCreated: 1746810000000 },
        },
      ],
    }, "default"),
    route("get", "api/v1/chat/:guid/message", {
      status: 200,
      data: [
        { guid: "msg_aaa1", text: "hey are you free thursday?", isFromMe: false, dateCreated: 1746810000000 },
        { guid: "msg_aaa2", text: "yeah, what time?", isFromMe: true, dateCreated: 1746810060000 },
        { guid: "msg_aaa3", text: "9:30 work?", isFromMe: false, dateCreated: 1746810120000 },
      ],
    }, "default"),
    route("post", "api/v1/message/text", {
      status: 200,
      message: "Message sent!",
      data: { guid: "msg_outbound_zzz1", text: "Mock outbound iMessage", isFromMe: true, dateCreated: 1746810500000 },
    }, "default"),
  ],
});
  ...
  routes: [
    route("post", "air/offer_requests", {
      data: {
        ...
        live_mode: false,
        slices: [
          { origin: { iata_code: "JFK", name: "John F Kennedy" }, destination: { iata_code: "LHR", name: "London Heathrow" }, departure_date: "2026-06-12" },
          { origin: { iata_code: "LHR", name: "London Heathrow" }, destination: { iata_code: "JFK", name: "John F Kennedy" }, departure_date: "2026-06-19" },
        ],
        ...
        offers: [
          {
            ...
            slices: [
              { origin: { iata_code: "JFK" }, destination: { iata_code: "LHR" }, departing_at: "2026-06-12T22:30:00", duration: "PT7H05M" },
              { origin: { iata_code: "LHR" }, destination: { iata_code: "JFK" }, departing_at: "2026-06-19T11:55:00", duration: "PT8H10M" },
            ],
          },
        ],
      },
    }, "duffel"),
    route("get", "air/offers", {
      data: [
        {
          id: "off_0000A2bcDe3FgHi4JkLmN5_1",
          total_amount: "642.18",
    ...
    route("post", "air/orders", {
      data: {
        id: "ord_0000A2bcDe3FgHi4JkLmN5_X",
        booking_reference: "ABC123",
        total_amount: "642.18",
        total_currency: "USD",
        passengers: [{ id: "pas_001", given_name: "Shaw", family_name: "Walters" }],
      },
    }, "duffel"),
  ],
});
      ...
      // is loud. Tests should always send a fault toggle.
      {
        ...happyResponse({
          ...
          error: {
            ...
            message: "Mockoon anthropic env is failure-injection only — point the SDK at cerebras for happy-path completions.",
          },
        }, 503),
...
const cerebras = envelope({
  ...
  routes: [
      ...
      choices: [
        {
          ...
          message: {
            role: "assistant",
            content: "{\"action\":\"route_to_inbox\",\"confidence\":0.91,\"rationale\":\"Routine standup confirmation; not urgent.\"}",
          },
          finish_reason: "stop",
        },
      ],
    ...
    route("post", "v1/embeddings", {
      object: "list",
      data: [
        { object: "embedding", index: 0, embedding: [0.0123, -0.0456, 0.0789, -0.0234, 0.0567, -0.0890, 0.0345, -0.0678] },
      ],
...
const elizaCloud = envelope({
  ...
  port: 18816,
  routes: [
    route("post", "api/v1/eliza/auth/token", {
      apiKey: "elc_test_2026_05_09_shaw",
      expiresAt: "2026-08-09T00:00:00.000Z",
      agentId: "agent-shaw-default",
    }, "elizacloud"),
    route("get", "api/v1/eliza/agents/me", {
    ...
    route("get", "api/v1/eliza/billing/balance", {
      currency: "USD",
      balance: 18.42,
      reservedCredits: 1.0,
    ...
    route("post", "api/v1/eliza/plaid/link-token", {
      linkToken: "link-sandbox-mock-2026-05-09",
      expiration: "2026-05-09T22:00:00.000Z",
      environment: "sandbox",
    }, "elizacloud"),
    route("post", "api/v1/eliza/paypal/authorize", {
      authorizationUrl: "https://www.paypal.com/connect?flowEntry=static&client_id=mock&scope=openid",
      state: "state-mock-2026-05-09",
    }, "elizacloud"),
    route("post", "api/v1/eliza/schedule/sync", {
      synced: 12,
      pruned: 1,
    ...
    route("get", "v1/receive/:account", [
      {
        envelope: {
          ...
          dataMessage: { timestamp: 1746810400000, message: "Are we still on for tomorrow?", expiresInSeconds: 0, viewOnce: false },
        },
      },
    ], "default"),
    route("post", "v2/send", {
      results: [{ recipientAddress: { number: PEOPLE[2].phone }, type: "SUCCESS" }],
