...
From the repository root, with the pinned Electrobun checkout available at
`upstreams/electrobun`, run:

```bash
bash patches/electrobun/apply.sh
```

The script skips patches that are already applied. This source-patch bundle is
separate from `patches/electrobun@1.18.1.patch`, which Bun applies to the npm
