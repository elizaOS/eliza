...
Legend: `[x]` done in this checkout · `[ ]` pending · `[hw]` pending, needs GPU / reference hardware.

> v1 is the upstream BASE models, GGUF-converted via the `elizaOS/llama.cpp`
> fork with all §3 kernel optimizations — NOT fine-tuned. So the **text eval is
> perplexity-vs-the-upstream-GGUF (parity)**, not a fine-tuned-quality
> threshold. The 0.55 / 0.60 `text_eval` floors in `eliza1_gates.yaml` are the
> v2 fine-tuned gate, not the v1 gate.

## A. Host-side / CPU (no GPU, runs in CI)

- [x] `make -C packages/inference/verify reference-test` — C reference clean, `gen_fixture --self-test` finite (incl. fused-attn + TBQ V-cache parity).
...
- [ ] `python3 packages/training/scripts/manifest/eliza1_platform_plan.py --out ... --readiness-md ...` and `node .../render-ios-smoke-report.mjs` regenerate cleanly and idempotently (run on every status change).
- [hw] `make -C packages/inference/verify cuda-preprocess-check` — host-side CUDA API/layout surface check (needs the fork's CUDA headers staged via `build-llama-cpp-dflash.mjs --target linux-x64-cuda --no-build`; no `nvcc` needed).

## B. Kernel verification (per supported backend, against the SHIPPED quantized bytes)
...
- [hw] `make -C packages/inference/verify metal-verify-fused` — Metal fused-attn kernel hardware verify (needs an Apple-Silicon Mac).
...
- [hw] `node packages/app-core/scripts/build-llama-cpp-dflash.mjs --target <each target>` — the fork actually builds and the post-build capability gate is green for the bundle's `dtype` on each supported backend (the build MUST fail if any required §3 kernel is missing — there is no kernels-missing fallback build).
...
- [hw] **VAD latency / boundary / endpoint / false-barge-in** → `evals.vadLatencyMs` (+ the related VAD slots).
...
- [ ] (NOT a v1 gate) Fine-tuned text quality — ships in v2 (`releaseState=finetuned-v2`).
