...
function getCacheKey(organizationId: string, connectionId: string): string {
  return `oauth_token:${organizationId}:${connectionId}`;
}
...
export const tokenCache = {
  async get(organizationId: string, connectionId: string): Promise<TokenResult | null> {
    const cached = await cache.get<CachedToken>(getCacheKey(organizationId, connectionId));
    ...
    if (expiresAt && Date.now() >= expiresAt.getTime() - EXPIRY_BUFFER_MS) {
      await this.invalidate(organizationId, connectionId);
      return null;
    }
  ...
  async set(organizationId: string, connectionId: string, token: TokenResult): Promise<void> {
    ...
    const key = getCacheKey(organizationId, connectionId);
    await cache.set(key, { token: { ...token, fromCache: false }, cachedAt: Date.now() }, ttl);
    logger.debug("[TokenCache] Cached token", { connectionId, ttlSeconds: ttl });
  },

  async invalidate(organizationId: string, connectionId: string): Promise<void> {
    await cache.del(getCacheKey(organizationId, connectionId));
  },
   ...
   */
  async invalidateByPlatform(organizationId: string, platform: string): Promise<void> {
    // Secrets-based connection IDs are `platform:orgId`, so the cache key is:
    // `oauth_token:orgId:platform:orgId`
    const secretsConnectionId = `${platform}:${organizationId}`;
    await cache.del(getCacheKey(organizationId, secretsConnectionId));
    logger.debug("[TokenCache] Invalidated platform token", { organizationId, platform });
  },
};
