 ...
 * complex mocking of discord.js and @vercel/blob. Integration tests
 * should cover the full flow with real dependencies.
 */
...
describe("Voice attachment detection logic", () => {
  ...
  test("non-audio content types are not detected as voice", () => {
    const isVoiceAttachment = (att: { contentType?: string | null; name?: string | null }): boolean => {
      return !!(att.contentType?.startsWith("audio/") || att.name?.endsWith(".ogg"));
    };
