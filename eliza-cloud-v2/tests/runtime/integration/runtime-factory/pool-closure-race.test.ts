...
describe("Pool Closure Race Condition", () => {
    ...
    it(
      ...
      async () => {
        ...
        if (!errorOccurred) {
          ...
          expect(errorMessage.toLowerCase()).toMatch(/pool|end|closed|cannot/);
        }
  ...
  describe("Race Condition: In-Flight Query During Pool Close", () => {
    /**
     * This test simulates the REAL production scenario:
     * A long-running database operation is interrupted when the pool is closed.
     * This is different from starting a new query after the pool is closed.
     *
    ...
    it(
      ...
      async () => {
        ...
        // Start multiple queries that will be "in-flight" when we close the pool
        const errors: string[] = [];
        const successes: number[] = [];

        // Create a promise that will query repeatedly
        const queryLoop = async (index: number): Promise<void> => {
          ...
          while (Date.now() - startTime < 500) {
            ...
            try {
              ...
              errors.push(`Query ${index}: ${(e as Error).message}`);
        ...
        // Start multiple query loops
        const queryPromises = Array.from({ length: 3 }, (_, i) => queryLoop(i));

        // After a short delay, close the pool while queries are running
        const closePromise = (async () => {
          await new Promise((r) => setTimeout(r, 100)); // Wait for queries to start
          console.log("Closing adapter while queries are in-flight...");
          await _testing.closeAdapterDirectly(agentId);
          console.log("Adapter closed");
        })();

        // Wait for everything to complete
        await Promise.all([...queryPromises, closePromise]);

        console.log(`\nResults:`);
        console.log(`  Successful query cycles: ${successes.length}`);
        console.log(`  Errors: ${errors.length}`);

        if (errors.length > 0) {
          console.log("\n*** IN-FLIGHT QUERY FAILURE REPRODUCED ***");
          errors.forEach((err) => console.log(`  ${err}`));

          // At least one query should have failed
    ...
    it(
      ...
      async () => {
        ...
        // Close the pool while queries are in progress
        const closePromise = (async () => {
        ...
        if (errors.length > 0) {
          ...
          const hasPoolError = errors.some(
            (err) =>
              ...
              err.toLowerCase().includes("closed") ||
              err.toLowerCase().includes("cannot") ||
              err.toLowerCase().includes("terminated"),
          );

          if (hasPoolError) {
            console.log("!!! EXACT PRODUCTION ERROR TYPE FOUND !!!");
          }
        }
