 ...
 * WHY THIS EXISTS:
 * ----------------
 * 1. PROVIDER AGNOSTIC: Generic path allows voice provider changes without
 *    breaking client code that lists available voices.
 *
 * 2. VOICE SELECTION: Developers building TTS applications need to programmatically
 *    list available voices to offer selection in their UIs or select appropriate
 *    voices based on context (language, tone, gender).
 *
 * 3. VOICE INVENTORY: Organizations with multiple cloned voices need API access
 *    to audit, manage, and integrate their voice library into workflows.
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.info(`[Voice List API] Fetching voices for user ${user.id}`, {
