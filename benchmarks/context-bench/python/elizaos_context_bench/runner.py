...
async def run_eliza_agent_benchmark(
    ...
    # Generate all tasks
    tasks: list[tuple[str, str, str, str, str, str, int, NeedlePosition]] = []
    task_counter = 0

    # NIAH tasks
    if benchmark_config.run_niah_basic:
        for length in benchmark_config.context_lengths:
            for position in benchmark_config.positions:
                for _ in range(benchmark_config.tasks_per_position):
                    task_counter += 1
                    task = generator.generate_niah_task(
                        task_id=f"agent_niah_{task_counter}",
                    ...
                    tasks.append((
                        ...
                        "niah_basic",
                        task.context_length,
                        task.needle_position,
                    ))

    # Multi-hop tasks
    if benchmark_config.run_multi_hop:
        for length in benchmark_config.context_lengths[:3]:  # Limit lengths for multi-hop
            for depth in benchmark_config.multi_hop_depths:
                task_counter += 1
                task = generator.generate_multi_hop_task(
                    task_id=f"agent_multihop_{task_counter}",
                    context_length=length,
                    num_hops=depth,
                )
                tasks.append((
    ...
    async def run_single(
        task_data: tuple[str, str, str, str, str, str, int, NeedlePosition],
    ) -> ContextBenchResult:
        """Run a single task through the agent loop."""
        nonlocal completed
        task_id, context, question, expected, needle, bench_type, ctx_len, position = task_data
        ...
        async with semaphore:
            ...
            try:
                ...
                result = ContextBenchResult(
                    task_id=task_id,
                    bench_type=ContextBenchType(bench_type) if bench_type.startswith("niah") else ContextBenchType.MULTI_HOP,
                    context_length=ctx_len,
                    needle_position=position,
                    actual_position_pct=0.5,  # Will be updated if needed
                    ...
                    num_hops=int(bench_type.split("_")[-1]) if bench_type.startswith("multi_hop") else 1,
...
async def setup_and_run_agent_benchmark(
    ...
    # Create character optimized for benchmarking
    character = Character(
        name="BenchmarkAgent",
        bio="A precise agent for context retrieval benchmarking.",
        system=(
            "You answer questions based ONLY on provided context. "
            "Be brief and precise. Return ONLY the answer."
        ),
    )
    ...
    if model_plugin_factory is not None:
        plugin = model_plugin_factory()
        if isinstance(plugin, Plugin) and plugin.models:
