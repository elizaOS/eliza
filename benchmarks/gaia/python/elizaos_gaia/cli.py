...
Examples:
  ...
  gaia-benchmark --provider openai --model gpt-4o
...
async def run_benchmark_async(args: argparse.Namespace) -> int:
    ...
    try:
        if args.quick_test:
            ...
            print(f"Running quick test (5 questions) with {provider or 'auto'}/{model_name}...")
            config = GAIAConfig(
                split=args.split,
                dataset_source=args.dataset,
                dataset_path=args.dataset_path,
            ...
            results = await run_quick_test(config, hf_token=hf_token)
        else:
