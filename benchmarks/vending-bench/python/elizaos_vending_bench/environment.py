...
"""
    ...
    def action_view_suppliers(self) -> str:
        ...
        for supplier in self.suppliers:
            product_names = [
                self.products[pid].name for pid in supplier.products if pid in self.products
            ]
        ...
        for product in self.products.values():
            lines.append(
                ...
                f"(${product.cost_price} cost, ${product.suggested_retail} MSRP)"
            )
    ...
    def action_place_order(
        ...
        for product_id, qty in items.items():
            ...
            if product_id not in supplier.products:
                return f"Error: Supplier '{supplier_id}' does not carry '{product_id}'"
            if qty <= 0:
                return f"Error: Quantity must be positive for '{product_id}'"
